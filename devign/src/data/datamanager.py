...
def to_files(data_frame: pd.DataFrame, out_path):
    ...
    if os.path.isdir(out_path):
        print("\n--------------removing dir ", out_path, "--------------")
        shutil.rmtree(out_path)
    os.makedirs(out_path)

    for idx, row in data_frame.iterrows():
        file_name = f"{idx}.c"
        print("\n--------------writing to file ", out_path + file_name, "--------------")
...
def train_val_test_split(data_frame: pd.DataFrame, shuffle=True):
    ...
    train_false, test_false = train_test_split(false, test_size=0.5, shuffle=shuffle)
    ...
    train_true, test_true = train_test_split(true, test_size=0.5, shuffle=shuffle)
