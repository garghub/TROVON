...
ti = proto_tree_add_text(tree, tvb, offset, 0, "Pixmap");
...
ti = proto_tree_add_text(tree, tvb, offset, 2, "RedCursor");
...
proto_tree_add_text(LZ_PLT_tree, tvb, offset, 1, "LZ_PLT Flag");
offset += 1;
...
proto_tree_add_text(LZ_PLT_tree, tvb, offset, 4, "pallete offset: %u bytes", pal_size);
offset += 4;
...
ti = proto_tree_add_text(tree, tvb, offset, 8, "Client RECORD_MODE message");
...
ti = proto_tree_add_text(tree, tvb, offset, 0, "RECORD_VOLUME message");
...
break;
case VD_AGENT_REPLY:
proto_tree_add_text(tree, tvb, offset, message_len, "VD_AGENT_REPLY message");
offset += message_len;
break;
case VD_AGENT_CLIPBOARD:
proto_tree_add_text(tree, tvb, offset, message_len, "VD_AGENT_CLIPBOARD message");
offset += message_len;
break;
...
proto_tree_add_text(tree, tvb, offset, message_len, "VD_AGENT_ANNOUNCE_CAPABILITIES message");
offset += message_len;
break;
...
proto_tree_add_text(tree, tvb, offset, 4, "Client MOUSE_MODE_REQUEST message");
offset += 2;
break;
case SPICEC_MAIN_ATTACH_CHANNELS:
proto_tree_add_text(tree, tvb, offset, 4, "Client MAIN_ATTACH_CHANNEL message");
break;
...
proto_tree_add_boolean(cap_tree, hf_main_cap_vm_name_uuid, tvb, offset, 4, val);
break;
default:
break;
}
...
GET_PDU_FROM_OFFSET(offset)
pdu_len = tvb_get_letohl(tvb, offset + 14);
if (pdu_len == 0) {
