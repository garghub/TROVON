...
dcm_header=(guint8 *)ep_alloc0(DCM_HEADER_MAX);
offset=128;
...
guint8 abort_source;
guint8 abort_reason;
buf_desc = (gchar *)ep_alloc0(MAX_BUF_LEN);
...
col_set_str(pinfo->cinfo, COL_INFO, se_strdup(buf_desc));
...
(*pdv)->pctx_id = pctx_id;
desc_header=(gchar *)se_alloc0(MAX_BUF_LEN);
switch (flags) {
case 0:
...
tag_vl = ep_strdup_printf("%10u", vl);
}
...
buf_desc=(gchar *)se_alloc0(MAX_BUF_LEN);
while (offset < endpos) {
