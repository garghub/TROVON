...
guint subset_tvb_offset;
guint subset_tvb_length;
check_offset_length(backing->length, backing->reported_length, backing_offset, -1 ,
&subset_tvb_offset,
&subset_tvb_length);
tvb = tvb_new_with_subset(subset_tvb_offset, subset_tvb_length);
tvb_set_subset_no_exceptions(tvb, backing, -1 );
tvb->ds_tvb = backing->ds_tvb;
return tvb;
}
tvbuff_t*
...
add_to_chain((tvbuff_t *)composite->tvbs->data, tvb);
tvb->initialized = TRUE;
}
guint
