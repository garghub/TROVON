...
switch(tag) {
case BER_UNI_TAG_INTEGER:
ret = asn1_int32_value_decode(&asn1, len, &value);
asn1_close(&asn1, &offset);
if (asn1_debug) {
if ( (props.value_id == -1) ||
...
break;
case BER_UNI_TAG_BOOLEAN:
ret = asn1_bool_decode(&asn1, len, (gboolean *)&value);
...
if (len > 4)
goto dostring;
ret = asn1_int32_value_decode(&asn1, len, (gint32 *)&value);
asn1_close(&asn1, &offset);
if (asn1_debug) {
if ( (props.value_id == -1) ||
(tbl_types_wireshark[props.type] != FT_UINT32) )
...
if (def) {
asn1_oid_value_decode(&asn1, len, &oid, &con);
oid = g_realloc(oid, con + sizeof(guint));
...
p->typename = tbl_types_asn1[p->type];
p->typenum = -1;
p->mytype = -1;
