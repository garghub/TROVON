...
if (!pinfo->fd->flags.visited && length > 0) {
if (pinfo->phdr->presence_flags & WTAP_HAS_INTERFACE_ID)
wireshark_interface_id = pinfo->phdr->interface_id;
key[0].length = 1;
key[0].key = &wireshark_interface_id;
...
if (!pinfo->fd->flags.visited && (length == -1 || (client_request && client_request->service_in == -1 && tvb_reported_length_remaining(tvb, offset) > 0))) {
if (!client_request) {
if (pinfo->phdr->presence_flags & WTAP_HAS_INTERFACE_ID)
wireshark_interface_id = pinfo->phdr->interface_id;
key[0].length = 1;
...
} else if (pinfo->srcport == server_port) {
guint32 wireshark_interface_id = 0;
guint8 *service = SERVICE_NONE;
wmem_tree_t *subtree;
wmem_tree_key_t key[5];
