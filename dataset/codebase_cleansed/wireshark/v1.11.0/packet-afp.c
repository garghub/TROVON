...
proto_tree_add_item(tree, hf_afp_session_token_type, tvb, offset, 2, ENC_BIG_ENDIAN);
offset += 2;
if (token == kLoginWithoutID || token == kGetKerberosSessionKey)
return offset;
len = tvb_get_ntohl(tvb, offset);
proto_tree_add_item(tree, hf_afp_session_token_len, tvb, offset, 4, ENC_BIG_ENDIAN);
offset += 4;
...
case SQ_CPX_TYPE_FILEMETA:
if (query_length <= 8) {
proto_tree_add_text(tree, tvb, offset, query_length, "filemeta (empty)");
} else {
...
case SQ_TYPE_CNIDS:
if (query_length <= 8) {
proto_tree_add_text(tree, tvb, offset, query_length, "CNID Array (empty)");
} else {
