...
*type = HTTP_REQUEST;
isHttpRequestOrReply = TRUE;
} else if (strncmp(data, "SSTP_DUPLEX_POST", indx) == 0) {
*type = HTTP_REQUEST;
isHttpRequestOrReply = TRUE;
...
http_ssl_handle = register_dissector("http-over-tls", dissect_http_ssl, proto_http);
...
http_follow_tap = register_tap("http_follow");
