...
pi_present = tvb_get_guint8(tvb, 2) & 0x01;
if (pi_present) {
if (tfi == 0x00 && !pi_length_found) {
guint8 pi_bit_length;
pi_byte_length = length - 6;
...
tb_byte_length = (length - (pi_byte_length + 6));
...
expected_total_size = (num_of_pdus * mac_d_pdu_length / 8) + 7 + 2 ;
...
fpi->edch_ddi[i] = fp_edch_channel_info->edch_ddi[i];
fpi->edch_macd_pdu_size[i] = fp_edch_channel_info->edch_macd_pdu_size[i];
fpi->edch_lchId[i] = fp_edch_channel_info->edch_lchId[i];
}
