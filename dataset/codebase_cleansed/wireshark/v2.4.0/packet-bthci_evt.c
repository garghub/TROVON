...
offset += 64;
add_opcode(opcode_list, 0x2025, COMMAND_STATUS_NORMAL);
break;
case 0x09:
proto_tree_add_item(tree, hf_bthci_evt_status, tvb, offset, 1, ENC_LITTLE_ENDIAN);
send_hci_summary_status_tap(tvb_get_guint8(tvb, offset), pinfo, bluetooth_data);
...
offset += 32;
add_opcode(opcode_list, 0x2026, COMMAND_STATUS_NORMAL);
break;
case 0x0A:
proto_tree_add_item(tree, hf_bthci_evt_status, tvb, offset, 1, ENC_LITTLE_ENDIAN);
status = tvb_get_guint8(tvb, offset);
...
add_opcode(opcode_list, 0x041F, COMMAND_STATUS_NORMAL);
break;
case 0x1d:
