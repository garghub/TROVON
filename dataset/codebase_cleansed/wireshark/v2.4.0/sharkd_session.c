...
analyser.protocols_set = g_hash_table_new(NULL , NULL );
printf("{\"frames\":%u", cfile.count);
printf(",\"protocols\":[");
...
if (field_data == NULL || field_data->type == TABLE_ITEM_NONE)
continue;
printf("%s[", sepa);
...
printf("%s{", sepa);
if (rtd_data->stat_table.num_rtds == 1)
type_str = val_to_str_const(j, vs, "Other");
else
type_str = val_to_str_const(i, vs, "Other");
printf("\"type\":");
...
memcpy(&wav_hdr[4], "\xFF\xFF\xFF\xFF", 4);
memcpy(&wav_hdr[8], "WAVE", 4);
memcpy(&wav_hdr[12], "fmt ", 4);
memcpy(&wav_hdr[16], "\x10\x00\x00\x00", 4);
memcpy(&wav_hdr[20], "\x01\x00", 2);
tmp16 = channels;
...
memcpy(&wav_hdr[40], "\xFF\xFF\xFF\xFF", 4);
...
printf(",\"data\":");
json_print_base64(eo_entry->payload_data, (int) eo_entry->payload_len);
printf("}\n");
}
}
