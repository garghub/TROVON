...
if (undecorated_smoothed_ofdm <= 0) {
...
break;
case 6:
return -48 + 2 * (2 - VGA_idx);
case 5:
return -42 + 2 * (7 - VGA_idx);
case 4:
return -36 + 2 * (7 - VGA_idx);
case 3:
return -24 + 2 * (7 - VGA_idx);
case 2:
if (cck_highpwr)
return -12 + 2 * (5 - VGA_idx);
else
...
odm_signal_scale_mapping(dm, pwdb_all));
} else {
if (rf_rx_num != 0) {
...
struct dm_phy_status_info *phy_info)
{
phy_info->rx_power = rx_power;
phy_info->recv_signal_power = rx_power;
phy_info->channel = channel;
phy_info->is_beamformed = is_beamformed;
phy_info->is_mu_packet = is_mu_packet;
phy_info->rxsc = rxsc;
phy_info->rx_pwdb_all =
odm_query_rx_pwr_percentage(rx_power);
phy_info->signal_quality = signal_quality;
phy_info->band_width = bandwidth;
}
