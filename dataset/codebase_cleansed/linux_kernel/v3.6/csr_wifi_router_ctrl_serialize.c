...
size_t bufferSize = 2;
bufferSize += 2;
bufferSize += primitive->mlmeCommandLength;
bufferSize += 2;
bufferSize += primitive->dataRef1Length;
bufferSize += 2;
bufferSize += primitive->dataRef2Length;
return bufferSize;
}
...
bufferSize += 4;
bufferSize += (primitive->smeVersions.smeBuild ? strlen(primitive->smeVersions.smeBuild) : 0) + 1;
bufferSize += 4;
bufferSize += 1;
return bufferSize;
}
...
bufferSize += 2;
bufferSize += 2;
bufferSize += primitive->signalLength;
bufferSize += 2;
bufferSize += primitive->dataLength;
return bufferSize;
}
...
CsrUint16Des((u16 *) &primitive->clientData, buffer, &offset);
CsrUint16Des((u16 *) &primitive->result, buffer, &offset);
primitive->byteRead = NULL;
offset += 4;
primitive->byteWrite = NULL;
offset += 4;
primitive->firmwareDownload = NULL;
offset += 4;
primitive->reset = NULL;
offset += 4;
primitive->coreDumpPrepare = NULL;
offset += 4;
primitive->byteBlockRead = NULL;
offset += 4;
primitive->gpRead16 = NULL;
offset += 4;
primitive->gpWrite16 = NULL;
offset += 4;
return primitive;
}
