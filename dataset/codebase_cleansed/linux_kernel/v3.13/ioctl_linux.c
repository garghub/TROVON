...
struct adapter *padapter = (struct adapter *)rtw_netdev_priv(dev);
struct wifidirect_info *pwdinfo = &(padapter->wdinfo);
u8 listen_ch = pwdinfo->listen_channel;
switch (wrqu->data.length) {
case 1:
listen_ch = extra[0] - '0';
break;
...
struct adapter *padapter = (struct adapter *)rtw_netdev_priv(dev);
struct wifidirect_info *pwdinfo = &(padapter->wdinfo);
u8 op_ch = pwdinfo->operating_channel;
switch (wrqu->data.length) {
case 1:
op_ch = extra[0] - '0';
break;
...
uint dev_len = 0;
u8 dev_name_str[WPS_MAX_DEVICE_NAME_LEN + 5] = {0x00};
DBG_88E("[%s] data = %s\n", __func__, (char *)extra);
...
int err;
u8 org_fw_iol = padapter->registrypriv.fw_iol;
...
lps_mode = pwrctrlpriv->power_mgnt;
...
padapter->registrypriv.fw_iol = 0;
...
padapter->registrypriv.fw_iol = org_fw_iol;
return err;
}
...
return -EFAULT;
DBG_88E("%s: in =%s\n", __func__, extra);
countPkTx = strncmp(extra, "count =", 5);
...
DBG_88E("%s: %s\n", __func__, input);
bStartRx = (strncmp(input, "start", 5) == 0) ? 1 : 0;
bStopRx = (strncmp(input, "stop", 5) == 0) ? 1 : 0;
bQueryPhy = (strncmp(input, "phy", 3) == 0) ? 1 : 0;
if (bStartRx) {
sprintf(extra, "start");
...
return -EFAULT;
DBG_88E("%s:iwpriv in =%s\n", __func__, input);
bMain = strncmp(input, "1", 2);
bTurnoff = strncmp(input, "0", 3);
if (bMain == 0) {
