static int F_1 ( struct V_1 * V_2 )
{
struct V_3 * V_4 = F_2 ( F_3 ( V_2 ) ,
F_4 ( * V_4 ) , V_2 ) ;
struct V_5 * V_6 = F_5 ( V_2 ) ;
unsigned int V_7 = F_6 ( V_2 ) ;
T_1 V_8 = 0 ;
T_1 * V_9 = NULL ;
unsigned int V_10 [ 2 ] ;
T_1 V_11 ;
const bool V_12 = ( ( V_6 -> V_13 . V_14 & V_15 ) ==
V_16 ) ;
const bool V_17 = V_4 -> V_18 . V_19 ;
if ( ! V_6 -> V_13 . V_20 || ! V_6 -> V_21 )
return 0 ;
if ( V_12 )
V_8 = 16 ;
if ( V_17 ) {
V_8 = 16 + V_22 ;
V_9 = ( T_1 * ) ( ( void * ) V_6 -> V_23 + V_6 -> V_24 . V_25 +
V_6 -> V_13 . V_20 - V_22 ) ;
}
V_10 [ 0 ] = V_6 -> V_24 . V_25 ;
V_10 [ 1 ] = V_6 -> V_13 . V_20 ;
if ( V_4 -> V_18 . V_26 )
goto V_27;
if ( F_7 ( V_28 +
( V_17 ? V_29 : 0 ) ,
V_30 , V_10 , & V_11 ,
F_8 ( V_10 ) ) < 0 )
return - V_31 ;
if ( V_11 & 1 )
V_6 -> V_24 . V_32 = V_6 -> V_23 ;
else
V_6 -> V_24 . V_33 = V_6 -> V_33 ;
if ( V_11 & 2 )
V_6 -> V_13 . V_32 = V_6 -> V_23 + V_6 -> V_24 . V_25 ;
else
V_6 -> V_13 . V_33 = V_6 -> V_33 + V_6 -> V_24 . V_25 ;
V_6 -> V_24 . V_34 = ! ! ( V_11 & 1 ) ;
V_6 -> V_13 . V_34 = ! ! ( V_11 & 2 ) ;
F_9 ( V_6 -> V_35 , & V_6 -> V_13 , & V_6 -> V_24 ,
V_7 , V_6 -> V_21 , V_17 , V_9 ,
V_8 , true ) ;
V_27:
if ( F_7 ( V_36 +
( V_17 ? V_29 : 0 ) ,
V_30 , V_10 , & V_11 ,
F_8 ( V_10 ) ) < 0 )
return - V_31 ;
if ( V_11 & 1 )
V_6 -> V_24 . V_32 = V_6 -> V_23 ;
else
V_6 -> V_24 . V_33 = V_6 -> V_33 ;
if ( V_11 & 2 )
V_6 -> V_13 . V_32 = V_6 -> V_23 + V_6 -> V_24 . V_25 ;
else
V_6 -> V_13 . V_33 = V_6 -> V_33 + V_6 -> V_24 . V_25 ;
V_6 -> V_24 . V_34 = ! ! ( V_11 & 1 ) ;
V_6 -> V_13 . V_34 = ! ! ( V_11 & 2 ) ;
F_10 ( V_6 -> V_37 , & V_6 -> V_13 , & V_6 -> V_24 ,
V_7 , V_6 -> V_21 , V_4 -> V_18 . V_26 ,
V_17 , V_9 , V_8 , true ) ;
if ( ! V_4 -> V_18 . V_26 )
goto V_38;
if ( F_7 ( V_39 +
( V_17 ? V_29 : 0 ) ,
V_30 , V_10 , & V_11 ,
F_8 ( V_10 ) ) < 0 )
return - V_31 ;
if ( V_11 & 1 )
V_6 -> V_24 . V_32 = V_6 -> V_23 ;
else
V_6 -> V_24 . V_33 = V_6 -> V_33 ;
if ( V_11 & 2 )
V_6 -> V_13 . V_32 = V_6 -> V_23 + V_6 -> V_24 . V_25 ;
else
V_6 -> V_13 . V_33 = V_6 -> V_33 + V_6 -> V_24 . V_25 ;
V_6 -> V_24 . V_34 = ! ! ( V_11 & 1 ) ;
V_6 -> V_13 . V_34 = ! ! ( V_11 & 2 ) ;
F_11 ( V_6 -> V_35 , & V_6 -> V_13 , & V_6 -> V_24 ,
V_7 , V_6 -> V_21 , V_17 , V_9 ,
V_8 , true ) ;
V_38:
return 0 ;
}
static int F_12 ( struct V_1 * V_40 , unsigned int V_21 )
{
struct V_5 * V_6 = F_5 ( V_40 ) ;
V_6 -> V_21 = V_21 ;
F_1 ( V_40 ) ;
return 0 ;
}
static int F_13 ( struct V_1 * V_2 , const T_2 * V_23 ,
unsigned int V_20 )
{
struct V_5 * V_6 = F_5 ( V_2 ) ;
struct V_41 * V_42 = V_6 -> V_42 ;
struct V_43 V_44 ;
int V_45 = 0 ;
if ( F_14 ( & V_44 , V_23 , V_20 ) != 0 )
goto V_46;
#ifdef F_15
F_16 ( V_42 , L_1 ,
V_44 . V_47 + V_44 . V_48 , V_44 . V_48 ,
V_44 . V_47 ) ;
F_17 ( V_49 , L_2 F_18 ( __LINE__ ) L_3 ,
V_50 , 16 , 4 , V_23 , V_20 , 1 ) ;
#endif
V_45 = F_19 ( V_42 , V_6 -> V_23 , & V_6 -> V_24 , V_44 . V_51 ,
V_44 . V_47 , V_52 -
V_44 . V_48 ) ;
if ( V_45 )
goto V_46;
memcpy ( V_6 -> V_23 + V_6 -> V_24 . V_25 , V_44 . V_53 , V_44 . V_48 ) ;
F_20 ( V_42 , V_6 -> V_33 , V_6 -> V_24 . V_25 +
V_44 . V_48 , V_54 ) ;
#ifdef F_15
F_17 ( V_49 , L_4 F_18 ( __LINE__ ) L_3 ,
V_50 , 16 , 4 , V_6 -> V_23 ,
V_6 -> V_24 . V_25 + V_44 . V_48 , 1 ) ;
#endif
V_6 -> V_13 . V_20 = V_44 . V_48 ;
V_45 = F_1 ( V_2 ) ;
if ( V_45 )
goto V_46;
if ( V_6 -> V_55 [ V_56 ] ) {
V_45 = F_21 ( V_6 -> V_55 [ V_56 ] ,
V_6 -> V_35 ) ;
if ( V_45 ) {
F_16 ( V_42 , L_5 ) ;
goto V_46;
}
}
if ( V_6 -> V_55 [ V_57 ] ) {
V_45 = F_21 ( V_6 -> V_55 [ V_57 ] ,
V_6 -> V_37 ) ;
if ( V_45 ) {
F_16 ( V_42 , L_6 ) ;
goto V_46;
}
}
return V_45 ;
V_46:
F_22 ( V_2 , V_58 ) ;
return - V_31 ;
}
static int F_23 ( struct V_59 * V_60 ,
const T_2 * V_23 , unsigned int V_20 )
{
struct V_5 * V_6 = F_24 ( V_60 ) ;
struct V_61 * V_62 = F_25 ( V_60 ) ;
const char * V_63 = F_26 ( V_62 ) ;
struct V_41 * V_42 = V_6 -> V_42 ;
unsigned int V_7 = F_27 ( V_60 ) ;
T_1 V_8 = 0 ;
const bool V_12 = ( ( V_6 -> V_13 . V_14 & V_15 ) ==
V_16 ) ;
const bool V_17 = ( V_12 && strstr ( V_63 , L_7 ) ) ;
int V_45 = 0 ;
memcpy ( V_6 -> V_23 , V_23 , V_20 ) ;
#ifdef F_15
F_17 ( V_49 , L_2 F_18 ( __LINE__ ) L_3 ,
V_50 , 16 , 4 , V_23 , V_20 , 1 ) ;
#endif
if ( V_12 )
V_8 = 16 ;
if ( V_17 ) {
V_8 = 16 + V_22 ;
V_20 -= V_22 ;
}
F_20 ( V_42 , V_6 -> V_33 , V_20 , V_54 ) ;
V_6 -> V_13 . V_20 = V_20 ;
V_6 -> V_13 . V_32 = V_6 -> V_23 ;
V_6 -> V_13 . V_34 = true ;
F_28 ( V_6 -> V_35 , & V_6 -> V_13 , V_7 ,
V_17 , V_8 ) ;
F_29 ( V_6 -> V_37 , & V_6 -> V_13 , V_7 ,
V_17 , V_8 ) ;
F_30 ( V_6 -> V_64 , & V_6 -> V_13 ,
V_7 , V_17 , V_8 ) ;
if ( V_6 -> V_55 [ V_56 ] ) {
V_45 = F_21 ( V_6 -> V_55 [ V_56 ] ,
V_6 -> V_35 ) ;
if ( V_45 ) {
F_16 ( V_42 , L_5 ) ;
goto V_46;
}
}
if ( V_6 -> V_55 [ V_57 ] ) {
V_45 = F_21 ( V_6 -> V_55 [ V_57 ] ,
V_6 -> V_37 ) ;
if ( V_45 ) {
F_16 ( V_42 , L_6 ) ;
goto V_46;
}
}
if ( V_6 -> V_55 [ V_65 ] ) {
V_45 = F_21 ( V_6 -> V_55 [ V_65 ] ,
V_6 -> V_64 ) ;
if ( V_45 ) {
F_16 ( V_42 , L_8 ) ;
goto V_46;
}
}
return V_45 ;
V_46:
F_31 ( V_60 , V_58 ) ;
return - V_31 ;
}
static int F_32 ( struct V_59 * V_60 ,
const T_2 * V_23 , unsigned int V_20 )
{
struct V_5 * V_6 = F_24 ( V_60 ) ;
struct V_41 * V_42 = V_6 -> V_42 ;
int V_45 = 0 ;
if ( V_20 != 2 * V_66 && V_20 != 2 * V_67 ) {
F_31 ( V_60 ,
V_58 ) ;
F_16 ( V_42 , L_9 ) ;
return - V_31 ;
}
memcpy ( V_6 -> V_23 , V_23 , V_20 ) ;
F_20 ( V_42 , V_6 -> V_33 , V_20 , V_54 ) ;
V_6 -> V_13 . V_20 = V_20 ;
V_6 -> V_13 . V_32 = V_6 -> V_23 ;
V_6 -> V_13 . V_34 = true ;
F_33 ( V_6 -> V_35 , & V_6 -> V_13 ) ;
F_34 ( V_6 -> V_37 , & V_6 -> V_13 ) ;
if ( V_6 -> V_55 [ V_56 ] ) {
V_45 = F_21 ( V_6 -> V_55 [ V_56 ] ,
V_6 -> V_35 ) ;
if ( V_45 ) {
F_16 ( V_42 , L_5 ) ;
goto V_46;
}
}
if ( V_6 -> V_55 [ V_57 ] ) {
V_45 = F_21 ( V_6 -> V_55 [ V_57 ] ,
V_6 -> V_37 ) ;
if ( V_45 ) {
F_16 ( V_42 , L_6 ) ;
goto V_46;
}
}
return V_45 ;
V_46:
F_31 ( V_60 , V_58 ) ;
return 0 ;
}
static struct V_68 * F_35 ( struct V_5 * V_6 ,
enum V_69 type )
{
struct V_68 * V_55 = V_6 -> V_55 [ type ] ;
T_1 * V_70 ;
if ( F_36 ( ! V_55 ) ) {
F_37 ( & V_6 -> V_71 ) ;
V_55 = V_6 -> V_55 [ type ] ;
if ( ! V_55 ) {
int V_72 ;
if ( type == V_56 )
V_70 = V_6 -> V_35 ;
else if ( type == V_57 )
V_70 = V_6 -> V_37 ;
else
V_70 = V_6 -> V_64 ;
V_72 = F_38 () ;
V_55 = F_39 ( V_6 -> V_73 , & V_72 , V_70 ) ;
if ( F_40 ( ! F_41 ( V_55 ) ) )
V_55 -> V_74 = type ;
V_6 -> V_55 [ type ] = V_55 ;
}
F_42 ( & V_6 -> V_71 ) ;
}
return V_55 ;
}
static void F_43 ( struct V_41 * V_75 , struct V_76 * V_77 ,
struct V_76 * V_78 , int V_79 ,
int V_80 , T_3 V_81 , int V_7 ,
enum V_69 V_74 , T_3 V_82 ,
int V_83 )
{
if ( V_78 != V_77 ) {
if ( V_79 )
F_44 ( V_75 , V_77 , V_79 , V_54 ) ;
F_44 ( V_75 , V_78 , V_80 , V_84 ) ;
} else {
F_44 ( V_75 , V_77 , V_79 , V_85 ) ;
}
if ( V_81 )
F_45 ( V_75 , V_81 , V_7 ,
V_74 == V_65 ? V_84 :
V_54 ) ;
if ( V_83 )
F_45 ( V_75 , V_82 , V_83 , V_54 ) ;
}
static void F_46 ( struct V_41 * V_75 ,
struct V_86 * V_87 ,
struct V_88 * V_89 )
{
struct V_1 * V_2 = F_47 ( V_89 ) ;
int V_7 = F_6 ( V_2 ) ;
F_43 ( V_75 , V_89 -> V_77 , V_89 -> V_78 , V_87 -> V_79 , V_87 -> V_80 ,
V_87 -> V_81 , V_7 , V_87 -> V_90 . V_55 -> V_74 ,
V_87 -> V_82 , V_87 -> V_83 ) ;
F_45 ( V_75 , V_87 -> V_91 , 4 , V_54 ) ;
}
static void F_48 ( struct V_41 * V_75 ,
struct V_92 * V_87 ,
struct V_93 * V_89 )
{
struct V_59 * V_60 = F_49 ( V_89 ) ;
int V_7 = F_27 ( V_60 ) ;
F_43 ( V_75 , V_89 -> V_77 , V_89 -> V_78 , V_87 -> V_79 , V_87 -> V_80 ,
V_87 -> V_81 , V_7 , V_87 -> V_90 . V_55 -> V_74 ,
V_87 -> V_82 , V_87 -> V_83 ) ;
}
static void F_50 ( struct V_94 * V_90 , T_1 V_95 )
{
struct V_41 * V_73 ;
struct V_86 * V_87 ;
struct V_88 * V_96 = V_90 -> V_97 ;
struct V_1 * V_2 = F_47 ( V_96 ) ;
struct V_5 * V_5 = F_5 ( V_2 ) ;
int V_98 = 0 ;
V_73 = V_5 -> V_73 ;
if ( F_36 ( V_95 ) ) {
F_51 ( V_73 , V_95 ) ;
V_98 = - V_99 ;
}
V_87 = F_2 ( V_90 , F_4 ( * V_87 ) , V_90 ) ;
F_46 ( V_73 , V_87 , V_96 ) ;
F_52 ( V_96 , V_98 ) ;
F_53 ( V_87 ) ;
}
static struct V_86 * F_54 ( struct V_88 * V_89 ,
bool V_100 )
{
struct V_1 * V_2 = F_47 ( V_89 ) ;
struct V_5 * V_6 = F_5 ( V_2 ) ;
struct V_3 * V_4 = F_2 ( F_3 ( V_2 ) ,
F_4 ( * V_4 ) , V_2 ) ;
struct V_41 * V_73 = V_6 -> V_73 ;
T_4 V_101 = ( V_89 -> V_102 . V_101 & V_103 ) ?
V_104 : V_105 ;
int V_79 , V_106 , V_80 = 0 , V_107 = 0 ;
struct V_86 * V_87 ;
T_3 V_82 , V_81 = 0 ;
int V_7 = 0 ;
unsigned int V_21 = V_6 -> V_21 ;
int V_108 = 0 , V_109 = 0 , V_83 ;
int V_110 , V_111 ;
struct V_112 * V_113 , * V_114 ;
struct V_68 * V_55 ;
enum V_69 V_74 = V_100 ? V_56 : V_57 ;
V_55 = F_35 ( V_6 , V_74 ) ;
if ( F_36 ( F_41 ( V_55 ) ) )
return (struct V_86 * ) V_55 ;
V_87 = F_55 ( V_115 | V_101 ) ;
if ( F_36 ( ! V_87 ) ) {
F_16 ( V_73 , L_10 ) ;
return F_56 ( - V_116 ) ;
}
if ( F_40 ( V_89 -> V_77 == V_89 -> V_78 ) ) {
V_79 = F_57 ( V_89 -> V_77 , V_89 -> V_117 +
V_89 -> V_118 +
( V_100 ? V_21 : 0 ) ) ;
if ( F_36 ( V_79 < 0 ) ) {
F_16 ( V_73 , L_11 ,
V_89 -> V_117 + V_89 -> V_118 +
( V_100 ? V_21 : 0 ) ) ;
F_53 ( V_87 ) ;
return F_56 ( V_79 ) ;
}
V_106 = F_58 ( V_73 , V_89 -> V_77 , V_79 ,
V_85 ) ;
if ( F_36 ( ! V_106 ) ) {
F_16 ( V_73 , L_12 ) ;
F_53 ( V_87 ) ;
return F_56 ( - V_116 ) ;
}
} else {
V_79 = F_57 ( V_89 -> V_77 , V_89 -> V_117 +
V_89 -> V_118 ) ;
if ( F_36 ( V_79 < 0 ) ) {
F_16 ( V_73 , L_11 ,
V_89 -> V_117 + V_89 -> V_118 ) ;
F_53 ( V_87 ) ;
return F_56 ( V_79 ) ;
}
V_80 = F_57 ( V_89 -> V_78 , V_89 -> V_117 +
V_89 -> V_118 +
( V_100 ? V_21 :
( - V_21 ) ) ) ;
if ( F_36 ( V_80 < 0 ) ) {
F_16 ( V_73 , L_13 ,
V_89 -> V_117 + V_89 -> V_118 +
( V_100 ? V_21 : ( - V_21 ) ) ) ;
F_53 ( V_87 ) ;
return F_56 ( V_80 ) ;
}
if ( V_79 ) {
V_106 = F_58 ( V_73 , V_89 -> V_77 ,
V_79 , V_54 ) ;
if ( F_36 ( ! V_106 ) ) {
F_16 ( V_73 , L_12 ) ;
F_53 ( V_87 ) ;
return F_56 ( - V_116 ) ;
}
} else {
V_106 = 0 ;
}
V_107 = F_58 ( V_73 , V_89 -> V_78 , V_80 ,
V_84 ) ;
if ( F_36 ( ! V_107 ) ) {
F_16 ( V_73 , L_14 ) ;
F_44 ( V_73 , V_89 -> V_77 , V_79 , V_54 ) ;
F_53 ( V_87 ) ;
return F_56 ( - V_116 ) ;
}
}
if ( ( V_4 -> V_18 . V_19 && V_100 ) || ! V_4 -> V_18 . V_26 ) {
V_7 = F_6 ( V_2 ) ;
V_81 = F_59 ( V_73 , V_89 -> V_119 , V_7 , V_54 ) ;
if ( F_60 ( V_73 , V_81 ) ) {
F_16 ( V_73 , L_15 ) ;
F_43 ( V_73 , V_89 -> V_77 , V_89 -> V_78 , V_79 ,
V_80 , 0 , 0 , V_74 , 0 , 0 ) ;
F_53 ( V_87 ) ;
return F_56 ( - V_116 ) ;
}
}
V_109 = 1 + ! ! V_7 + V_106 +
( V_107 > 1 ? V_107 : 0 ) ;
if ( F_36 ( V_109 > V_120 ) ) {
F_16 ( V_73 , L_16 ,
V_109 , V_120 ) ;
F_43 ( V_73 , V_89 -> V_77 , V_89 -> V_78 , V_79 , V_80 ,
V_81 , V_7 , V_74 , 0 , 0 ) ;
F_53 ( V_87 ) ;
return F_56 ( - V_116 ) ;
}
V_113 = & V_87 -> V_121 [ 0 ] ;
V_83 = V_109 * sizeof( * V_113 ) ;
V_87 -> V_79 = V_79 ;
V_87 -> V_80 = V_80 ;
V_87 -> V_81 = V_81 ;
V_87 -> V_90 . V_97 = V_89 ;
V_87 -> V_90 . V_122 = F_50 ;
V_87 -> V_90 . V_55 = V_55 ;
V_87 -> V_117 = F_61 ( V_89 -> V_117 ) ;
V_87 -> V_91 = F_59 ( V_73 , & V_87 -> V_117 , 4 ,
V_54 ) ;
if ( F_60 ( V_73 , V_87 -> V_91 ) ) {
F_16 ( V_73 , L_17 ) ;
F_43 ( V_73 , V_89 -> V_77 , V_89 -> V_78 , V_79 , V_80 ,
V_81 , V_7 , V_74 , 0 , 0 ) ;
F_53 ( V_87 ) ;
return F_56 ( - V_116 ) ;
}
F_62 ( V_113 , V_87 -> V_91 , 4 , 0 ) ;
V_108 ++ ;
if ( V_7 ) {
F_62 ( V_113 + V_108 , V_81 , V_7 , 0 ) ;
V_108 ++ ;
}
F_63 ( V_89 -> V_77 , V_106 , V_113 + V_108 , 0 ) ;
V_108 += V_106 ;
if ( V_107 > 1 )
F_63 ( V_89 -> V_78 , V_107 , V_113 +
V_108 , 0 ) ;
V_82 = F_59 ( V_73 , V_113 , V_83 , V_54 ) ;
if ( F_60 ( V_73 , V_82 ) ) {
F_16 ( V_73 , L_18 ) ;
F_45 ( V_73 , V_87 -> V_91 , 4 , V_54 ) ;
F_43 ( V_73 , V_89 -> V_77 , V_89 -> V_78 , V_79 , V_80 ,
V_81 , V_7 , V_74 , 0 , 0 ) ;
F_53 ( V_87 ) ;
return F_56 ( - V_116 ) ;
}
V_87 -> V_82 = V_82 ;
V_87 -> V_83 = V_83 ;
V_111 = V_89 -> V_117 + V_89 -> V_118 +
( V_100 ? V_6 -> V_21 : ( - V_6 -> V_21 ) ) ;
V_110 = 4 + V_7 + V_89 -> V_117 + V_89 -> V_118 ;
V_114 = & V_87 -> V_90 . V_114 [ 0 ] ;
F_64 ( & V_114 [ 1 ] , V_82 , V_110 , 0 ) ;
if ( V_89 -> V_78 == V_89 -> V_77 ) {
if ( V_106 == 1 )
F_62 ( & V_114 [ 0 ] , F_65 ( V_89 -> V_77 ) ,
V_111 , 0 ) ;
else
F_66 ( & V_114 [ 0 ] , V_82 +
( 1 + ! ! V_7 ) * sizeof( * V_113 ) ,
V_111 , 0 ) ;
} else if ( V_107 == 1 ) {
F_62 ( & V_114 [ 0 ] , F_65 ( V_89 -> V_78 ) , V_111 ,
0 ) ;
} else {
F_66 ( & V_114 [ 0 ] , V_82 + sizeof( * V_113 ) *
V_108 , V_111 , 0 ) ;
}
return V_87 ;
}
static inline int F_67 ( struct V_88 * V_89 , bool V_100 )
{
struct V_86 * V_87 ;
struct V_1 * V_2 = F_47 ( V_89 ) ;
struct V_5 * V_6 = F_5 ( V_2 ) ;
int V_45 ;
if ( F_36 ( V_123 ) )
return - V_124 ;
V_87 = F_54 ( V_89 , V_100 ) ;
if ( F_41 ( V_87 ) )
return F_68 ( V_87 ) ;
V_45 = F_69 ( V_6 -> V_73 , & V_87 -> V_90 ) ;
if ( ! V_45 ) {
V_45 = - V_125 ;
} else {
F_46 ( V_6 -> V_73 , V_87 , V_89 ) ;
F_53 ( V_87 ) ;
}
return V_45 ;
}
static int F_70 ( struct V_88 * V_89 )
{
return F_67 ( V_89 , true ) ;
}
static int F_71 ( struct V_88 * V_89 )
{
return F_67 ( V_89 , false ) ;
}
static void F_72 ( struct V_94 * V_90 , T_1 V_95 )
{
struct V_92 * V_87 ;
struct V_93 * V_89 = V_90 -> V_97 ;
struct V_59 * V_60 = F_49 ( V_89 ) ;
struct V_5 * V_5 = F_24 ( V_60 ) ;
struct V_41 * V_73 = V_5 -> V_73 ;
int V_7 = F_27 ( V_60 ) ;
#ifdef F_15
F_16 ( V_73 , L_19 , V_126 , __LINE__ , V_95 ) ;
#endif
V_87 = F_2 ( V_90 , F_4 ( * V_87 ) , V_90 ) ;
if ( V_95 )
F_51 ( V_73 , V_95 ) ;
#ifdef F_15
F_17 ( V_49 , L_20 F_18 ( __LINE__ ) L_3 ,
V_50 , 16 , 4 , V_89 -> V_127 ,
V_87 -> V_79 > 1 ? 100 : V_7 , 1 ) ;
F_73 ( V_49 , L_21 F_18 ( __LINE__ ) L_3 ,
V_50 , 16 , 4 , V_89 -> V_78 ,
V_87 -> V_80 > 1 ? 100 : V_89 -> V_128 , 1 ) ;
#endif
F_48 ( V_73 , V_87 , V_89 ) ;
F_53 ( V_87 ) ;
F_74 ( V_89 -> V_127 , V_89 -> V_78 , V_89 -> V_128 - V_7 ,
V_7 , 0 ) ;
F_75 ( V_89 , V_95 ) ;
}
static struct V_92 * F_76 ( struct V_93
* V_89 , bool V_100 )
{
struct V_59 * V_60 = F_49 ( V_89 ) ;
struct V_5 * V_6 = F_24 ( V_60 ) ;
struct V_41 * V_73 = V_6 -> V_73 ;
T_4 V_101 = ( V_89 -> V_102 . V_101 & V_103 ) ?
V_104 : V_105 ;
int V_79 , V_106 , V_80 = 0 , V_107 = 0 ;
struct V_92 * V_87 ;
T_3 V_81 ;
bool V_129 ;
int V_7 = F_27 ( V_60 ) ;
int V_130 , V_109 ;
struct V_112 * V_113 , * V_114 ;
struct V_68 * V_55 ;
enum V_69 V_74 = V_100 ? V_56 : V_57 ;
V_55 = F_35 ( V_6 , V_74 ) ;
if ( F_36 ( F_41 ( V_55 ) ) )
return (struct V_92 * ) V_55 ;
V_79 = F_57 ( V_89 -> V_77 , V_89 -> V_128 ) ;
if ( F_36 ( V_79 < 0 ) ) {
F_16 ( V_73 , L_11 ,
V_89 -> V_128 ) ;
return F_56 ( V_79 ) ;
}
if ( F_36 ( V_89 -> V_77 != V_89 -> V_78 ) ) {
V_80 = F_57 ( V_89 -> V_78 , V_89 -> V_128 ) ;
if ( F_36 ( V_80 < 0 ) ) {
F_16 ( V_73 , L_13 ,
V_89 -> V_128 ) ;
return F_56 ( V_80 ) ;
}
V_106 = F_58 ( V_73 , V_89 -> V_77 , V_79 ,
V_54 ) ;
if ( F_36 ( ! V_106 ) ) {
F_16 ( V_73 , L_12 ) ;
return F_56 ( - V_116 ) ;
}
V_107 = F_58 ( V_73 , V_89 -> V_78 , V_80 ,
V_84 ) ;
if ( F_36 ( ! V_107 ) ) {
F_16 ( V_73 , L_14 ) ;
F_44 ( V_73 , V_89 -> V_77 , V_79 , V_54 ) ;
return F_56 ( - V_116 ) ;
}
} else {
V_106 = F_58 ( V_73 , V_89 -> V_77 , V_79 ,
V_85 ) ;
if ( F_36 ( ! V_106 ) ) {
F_16 ( V_73 , L_12 ) ;
return F_56 ( - V_116 ) ;
}
}
V_81 = F_59 ( V_73 , V_89 -> V_127 , V_7 , V_54 ) ;
if ( F_60 ( V_73 , V_81 ) ) {
F_16 ( V_73 , L_15 ) ;
F_43 ( V_73 , V_89 -> V_77 , V_89 -> V_78 , V_79 , V_80 , 0 ,
0 , 0 , 0 , 0 ) ;
return F_56 ( - V_116 ) ;
}
if ( V_106 == 1 &&
V_81 + V_7 == F_65 ( V_89 -> V_77 ) ) {
V_129 = true ;
V_109 = 0 ;
} else {
V_129 = false ;
V_109 = 1 + V_106 ;
}
V_130 = V_109 ;
V_109 += V_107 > 1 ? V_107 : 0 ;
if ( F_36 ( V_109 > V_131 ) ) {
F_16 ( V_73 , L_16 ,
V_109 , V_131 ) ;
F_43 ( V_73 , V_89 -> V_77 , V_89 -> V_78 , V_79 , V_80 ,
V_81 , V_7 , V_74 , 0 , 0 ) ;
return F_56 ( - V_116 ) ;
}
V_87 = F_55 ( V_115 | V_101 ) ;
if ( F_36 ( ! V_87 ) ) {
F_16 ( V_73 , L_10 ) ;
F_43 ( V_73 , V_89 -> V_77 , V_89 -> V_78 , V_79 , V_80 ,
V_81 , V_7 , V_74 , 0 , 0 ) ;
return F_56 ( - V_116 ) ;
}
V_87 -> V_79 = V_79 ;
V_87 -> V_80 = V_80 ;
V_87 -> V_81 = V_81 ;
V_113 = & V_87 -> V_121 [ 0 ] ;
V_87 -> V_83 = V_109 * sizeof( * V_113 ) ;
V_87 -> V_90 . V_97 = V_89 ;
V_87 -> V_90 . V_122 = F_72 ;
V_87 -> V_90 . V_55 = V_55 ;
if ( ! V_129 ) {
F_62 ( V_113 , V_81 , V_7 , 0 ) ;
F_63 ( V_89 -> V_77 , V_106 , V_113 + 1 , 0 ) ;
}
if ( V_107 > 1 )
F_63 ( V_89 -> V_78 , V_107 , V_113 +
V_130 , 0 ) ;
V_87 -> V_82 = F_59 ( V_73 , V_113 , V_87 -> V_83 ,
V_54 ) ;
if ( F_60 ( V_73 , V_87 -> V_82 ) ) {
F_16 ( V_73 , L_18 ) ;
F_43 ( V_73 , V_89 -> V_77 , V_89 -> V_78 , V_79 , V_80 ,
V_81 , V_7 , V_74 , 0 , 0 ) ;
F_53 ( V_87 ) ;
return F_56 ( - V_116 ) ;
}
V_114 = & V_87 -> V_90 . V_114 [ 0 ] ;
if ( ! V_129 )
F_64 ( & V_114 [ 1 ] , V_87 -> V_82 ,
V_7 + V_89 -> V_128 , 0 ) ;
else
F_77 ( & V_114 [ 1 ] , V_81 , V_7 + V_89 -> V_128 ,
0 ) ;
if ( V_89 -> V_77 == V_89 -> V_78 ) {
if ( ! V_129 )
F_66 ( & V_114 [ 0 ] , V_87 -> V_82 +
sizeof( * V_113 ) , V_89 -> V_128 , 0 ) ;
else
F_62 ( & V_114 [ 0 ] , F_65 ( V_89 -> V_77 ) ,
V_89 -> V_128 , 0 ) ;
} else if ( V_107 > 1 ) {
F_66 ( & V_114 [ 0 ] , V_87 -> V_82 + V_130 *
sizeof( * V_113 ) , V_89 -> V_128 , 0 ) ;
} else {
F_62 ( & V_114 [ 0 ] , F_65 ( V_89 -> V_78 ) ,
V_89 -> V_128 , 0 ) ;
}
return V_87 ;
}
static struct V_92 * F_78 (
struct V_132 * V_133 )
{
struct V_93 * V_89 = & V_133 -> V_133 ;
struct V_59 * V_60 = F_49 ( V_89 ) ;
struct V_5 * V_6 = F_24 ( V_60 ) ;
struct V_41 * V_73 = V_6 -> V_73 ;
T_4 V_101 = ( V_89 -> V_102 . V_101 & V_103 ) ?
V_104 : V_105 ;
int V_79 , V_106 , V_80 , V_107 ;
struct V_92 * V_87 ;
T_3 V_81 ;
bool V_134 ;
int V_7 = F_27 ( V_60 ) ;
struct V_112 * V_113 , * V_114 ;
int V_130 , V_109 ;
struct V_68 * V_55 ;
V_55 = F_35 ( V_6 , V_65 ) ;
if ( F_36 ( F_41 ( V_55 ) ) )
return (struct V_92 * ) V_55 ;
V_79 = F_57 ( V_89 -> V_77 , V_89 -> V_128 ) ;
if ( F_36 ( V_79 < 0 ) ) {
F_16 ( V_73 , L_11 ,
V_89 -> V_128 ) ;
return F_56 ( V_79 ) ;
}
if ( F_36 ( V_89 -> V_77 != V_89 -> V_78 ) ) {
V_80 = F_57 ( V_89 -> V_78 , V_89 -> V_128 ) ;
if ( F_36 ( V_80 < 0 ) ) {
F_16 ( V_73 , L_13 ,
V_89 -> V_128 ) ;
return F_56 ( V_80 ) ;
}
V_106 = F_58 ( V_73 , V_89 -> V_77 , V_79 ,
V_54 ) ;
if ( F_36 ( ! V_106 ) ) {
F_16 ( V_73 , L_12 ) ;
return F_56 ( - V_116 ) ;
}
V_107 = F_58 ( V_73 , V_89 -> V_78 , V_80 ,
V_84 ) ;
if ( F_36 ( ! V_107 ) ) {
F_16 ( V_73 , L_14 ) ;
F_44 ( V_73 , V_89 -> V_77 , V_79 , V_54 ) ;
return F_56 ( - V_116 ) ;
}
} else {
V_106 = F_58 ( V_73 , V_89 -> V_77 , V_79 ,
V_85 ) ;
if ( F_36 ( ! V_106 ) ) {
F_16 ( V_73 , L_12 ) ;
return F_56 ( - V_116 ) ;
}
V_80 = V_79 ;
V_107 = V_79 ;
}
V_81 = F_59 ( V_73 , V_133 -> V_135 , V_7 , V_84 ) ;
if ( F_60 ( V_73 , V_81 ) ) {
F_16 ( V_73 , L_15 ) ;
F_43 ( V_73 , V_89 -> V_77 , V_89 -> V_78 , V_79 , V_80 , 0 ,
0 , 0 , 0 , 0 ) ;
return F_56 ( - V_116 ) ;
}
V_109 = V_106 > 1 ? V_106 : 0 ;
V_130 = V_109 ;
if ( V_107 == 1 &&
V_81 + V_7 == F_65 ( V_89 -> V_78 ) ) {
V_134 = true ;
} else {
V_134 = false ;
V_109 += 1 + V_107 ;
}
if ( F_36 ( V_109 > V_131 ) ) {
F_16 ( V_73 , L_16 ,
V_109 , V_131 ) ;
F_43 ( V_73 , V_89 -> V_77 , V_89 -> V_78 , V_79 , V_80 ,
V_81 , V_7 , V_65 , 0 , 0 ) ;
return F_56 ( - V_116 ) ;
}
V_87 = F_55 ( V_115 | V_101 ) ;
if ( ! V_87 ) {
F_16 ( V_73 , L_10 ) ;
F_43 ( V_73 , V_89 -> V_77 , V_89 -> V_78 , V_79 , V_80 ,
V_81 , V_7 , V_65 , 0 , 0 ) ;
return F_56 ( - V_116 ) ;
}
V_87 -> V_79 = V_79 ;
V_87 -> V_80 = V_80 ;
V_87 -> V_81 = V_81 ;
V_113 = & V_87 -> V_121 [ 0 ] ;
V_87 -> V_83 = V_109 * sizeof( * V_113 ) ;
V_87 -> V_90 . V_97 = V_89 ;
V_87 -> V_90 . V_122 = F_72 ;
V_87 -> V_90 . V_55 = V_55 ;
if ( V_106 > 1 )
F_63 ( V_89 -> V_77 , V_106 , V_113 , 0 ) ;
if ( ! V_134 ) {
F_62 ( V_113 + V_130 , V_81 , V_7 , 0 ) ;
F_63 ( V_89 -> V_78 , V_107 , V_113 +
V_130 + 1 , 0 ) ;
}
V_87 -> V_82 = F_59 ( V_73 , V_113 , V_87 -> V_83 ,
V_54 ) ;
if ( F_60 ( V_73 , V_87 -> V_82 ) ) {
F_16 ( V_73 , L_18 ) ;
F_43 ( V_73 , V_89 -> V_77 , V_89 -> V_78 , V_79 , V_80 ,
V_81 , V_7 , V_65 , 0 , 0 ) ;
F_53 ( V_87 ) ;
return F_56 ( - V_116 ) ;
}
V_114 = & V_87 -> V_90 . V_114 [ 0 ] ;
if ( V_106 > 1 )
F_66 ( & V_114 [ 1 ] , V_87 -> V_82 , V_89 -> V_128 ,
0 ) ;
else
F_62 ( & V_114 [ 1 ] , F_65 ( V_89 -> V_77 ) ,
V_89 -> V_128 , 0 ) ;
if ( ! V_134 )
F_66 ( & V_114 [ 0 ] , V_87 -> V_82 + V_130 *
sizeof( * V_113 ) , V_7 + V_89 -> V_128 ,
0 ) ;
else
F_62 ( & V_114 [ 0 ] , F_65 ( V_89 -> V_78 ) ,
V_7 + V_89 -> V_128 , 0 ) ;
return V_87 ;
}
static inline int F_79 ( struct V_93 * V_89 , bool V_100 )
{
struct V_92 * V_87 ;
struct V_59 * V_60 = F_49 ( V_89 ) ;
struct V_5 * V_6 = F_24 ( V_60 ) ;
int V_45 ;
if ( F_36 ( V_123 ) )
return - V_124 ;
V_87 = F_76 ( V_89 , V_100 ) ;
if ( F_80 ( V_87 ) )
return F_68 ( V_87 ) ;
V_45 = F_69 ( V_6 -> V_73 , & V_87 -> V_90 ) ;
if ( ! V_45 ) {
V_45 = - V_125 ;
} else {
F_48 ( V_6 -> V_73 , V_87 , V_89 ) ;
F_53 ( V_87 ) ;
}
return V_45 ;
}
static int F_81 ( struct V_93 * V_89 )
{
return F_79 ( V_89 , true ) ;
}
static int F_82 ( struct V_93 * V_89 )
{
return F_79 ( V_89 , false ) ;
}
static int F_83 ( struct V_132 * V_133 )
{
struct V_93 * V_89 = & V_133 -> V_133 ;
struct V_92 * V_87 ;
struct V_59 * V_60 = F_49 ( V_89 ) ;
struct V_5 * V_6 = F_24 ( V_60 ) ;
int V_45 ;
if ( F_36 ( V_123 ) )
return - V_124 ;
V_87 = F_78 ( V_133 ) ;
if ( F_80 ( V_87 ) )
return F_68 ( V_87 ) ;
V_45 = F_69 ( V_6 -> V_73 , & V_87 -> V_90 ) ;
if ( ! V_45 ) {
V_45 = - V_125 ;
} else {
F_48 ( V_6 -> V_73 , V_87 , V_89 ) ;
F_53 ( V_87 ) ;
}
return V_45 ;
}
static int F_84 ( struct V_5 * V_6 , struct V_136 * V_18 )
{
struct V_137 * V_138 ;
V_6 -> V_42 = F_85 () ;
if ( F_80 ( V_6 -> V_42 ) ) {
F_86 ( L_22 ) ;
return F_68 ( V_6 -> V_42 ) ;
}
V_6 -> V_33 = F_59 ( V_6 -> V_42 , V_6 -> V_23 , sizeof( V_6 -> V_23 ) ,
V_54 ) ;
if ( F_60 ( V_6 -> V_42 , V_6 -> V_33 ) ) {
F_16 ( V_6 -> V_42 , L_23 ) ;
F_87 ( V_6 -> V_42 ) ;
return - V_116 ;
}
V_6 -> V_13 . V_14 = V_139 | V_18 -> V_140 ;
V_6 -> V_24 . V_14 = V_141 | V_18 -> V_142 ;
V_138 = F_88 ( V_6 -> V_42 -> V_143 ) ;
V_6 -> V_73 = V_138 -> V_73 ;
F_89 ( & V_6 -> V_71 ) ;
V_6 -> V_55 [ V_56 ] = NULL ;
V_6 -> V_55 [ V_57 ] = NULL ;
V_6 -> V_55 [ V_65 ] = NULL ;
return 0 ;
}
static int F_90 ( struct V_61 * V_62 )
{
struct V_144 * V_4 = V_62 -> V_145 ;
struct V_146 * V_147 = F_2 ( V_4 , F_4 ( * V_147 ) ,
V_144 ) ;
struct V_5 * V_6 = F_91 ( V_62 ) ;
return F_84 ( V_6 , & V_147 -> V_18 ) ;
}
static int F_92 ( struct V_1 * V_62 )
{
struct V_148 * V_4 = F_3 ( V_62 ) ;
struct V_3 * V_147 = F_2 ( V_4 , F_4 ( * V_147 ) ,
V_2 ) ;
struct V_5 * V_6 = F_5 ( V_62 ) ;
return F_84 ( V_6 , & V_147 -> V_18 ) ;
}
static void F_93 ( struct V_5 * V_6 )
{
F_94 ( V_6 -> V_55 [ V_56 ] ) ;
F_94 ( V_6 -> V_55 [ V_57 ] ) ;
F_94 ( V_6 -> V_55 [ V_65 ] ) ;
F_45 ( V_6 -> V_42 , V_6 -> V_33 , sizeof( V_6 -> V_23 ) ,
V_54 ) ;
F_87 ( V_6 -> V_42 ) ;
}
static void F_95 ( struct V_61 * V_62 )
{
F_93 ( F_91 ( V_62 ) ) ;
}
static void F_96 ( struct V_1 * V_62 )
{
F_93 ( F_5 ( V_62 ) ) ;
}
static void T_5 F_97 ( void )
{
struct V_146 * V_149 , * V_150 ;
int V_151 ;
for ( V_151 = 0 ; V_151 < F_8 ( V_152 ) ; V_151 ++ ) {
struct V_3 * V_149 = V_152 + V_151 ;
if ( V_149 -> V_153 )
F_98 ( & V_149 -> V_2 ) ;
}
if ( ! V_154 . V_155 )
return;
F_99 (t_alg, n, &alg_list, entry) {
F_100 ( & V_149 -> V_144 ) ;
F_101 ( & V_149 -> V_156 ) ;
F_102 ( V_149 ) ;
}
}
static struct V_146 * F_103 ( struct V_157
* V_158 )
{
struct V_146 * V_149 ;
struct V_144 * V_4 ;
V_149 = F_104 ( sizeof( * V_149 ) , V_104 ) ;
if ( ! V_149 )
return F_56 ( - V_116 ) ;
V_4 = & V_149 -> V_144 ;
snprintf ( V_4 -> V_159 , V_160 , L_24 , V_158 -> V_161 ) ;
snprintf ( V_4 -> V_162 , V_160 , L_24 ,
V_158 -> V_163 ) ;
V_4 -> V_164 = V_165 ;
V_4 -> V_166 = F_90 ;
V_4 -> V_167 = F_95 ;
V_4 -> V_168 = V_169 ;
V_4 -> V_170 = V_158 -> V_171 ;
V_4 -> V_172 = 0 ;
V_4 -> V_173 = sizeof( struct V_5 ) ;
V_4 -> V_174 = V_175 | V_176 |
V_158 -> type ;
switch ( V_158 -> type ) {
case V_177 :
V_4 -> V_178 = & V_179 ;
V_4 -> V_180 = V_158 -> V_181 ;
break;
case V_182 :
V_4 -> V_178 = & V_183 ;
V_4 -> V_180 = V_158 -> V_181 ;
break;
}
V_149 -> V_18 . V_140 = V_158 -> V_140 ;
V_149 -> V_18 . V_142 = V_158 -> V_142 ;
return V_149 ;
}
static void F_105 ( struct V_3 * V_149 )
{
struct V_148 * V_4 = & V_149 -> V_2 ;
V_4 -> V_102 . V_164 = V_165 ;
V_4 -> V_102 . V_168 = V_169 ;
V_4 -> V_102 . V_173 = sizeof( struct V_5 ) ;
V_4 -> V_102 . V_174 = V_175 | V_176 ;
V_4 -> V_184 = F_92 ;
V_4 -> exit = F_96 ;
}
static int T_6 F_106 ( void )
{
struct V_185 * V_186 ;
struct V_187 * V_188 ;
struct V_41 * V_189 ;
struct V_137 * V_138 ;
int V_151 = 0 , V_190 = 0 ;
T_1 V_191 , V_192 , V_193 , V_194 , V_195 ;
unsigned int V_196 = V_197 ;
bool V_153 = false ;
V_186 = F_107 ( NULL , NULL , L_25 ) ;
if ( ! V_186 ) {
V_186 = F_107 ( NULL , NULL , L_26 ) ;
if ( ! V_186 )
return - V_198 ;
}
V_188 = F_108 ( V_186 ) ;
F_109 ( V_186 ) ;
if ( ! V_188 )
return - V_198 ;
V_189 = & V_188 -> V_75 ;
V_138 = F_88 ( V_189 ) ;
if ( ! V_138 || ! V_138 -> V_199 )
return - V_198 ;
F_110 ( & V_154 ) ;
V_191 = F_111 ( & V_138 -> V_200 -> V_201 . V_202 ) ;
V_192 = F_111 ( & V_138 -> V_200 -> V_201 . V_203 ) ;
V_193 = ( V_192 & V_204 ) >> V_205 ;
V_194 = ( V_192 & V_206 ) >> V_207 ;
V_195 = ( V_192 & V_208 ) >> V_209 ;
if ( V_195 && ( ( V_191 & V_208 ) == V_210 ) )
V_196 = V_211 ;
for ( V_151 = 0 ; V_151 < F_8 ( V_212 ) ; V_151 ++ ) {
struct V_146 * V_149 ;
struct V_157 * V_4 = V_212 + V_151 ;
T_1 V_213 = V_4 -> V_140 & V_214 ;
if ( ! V_193 &&
( ( V_213 == V_215 ) ||
( V_213 == V_216 ) ) )
continue;
if ( ! V_194 && ( V_213 == V_217 ) )
continue;
V_149 = F_103 ( V_4 ) ;
if ( F_80 ( V_149 ) ) {
V_190 = F_68 ( V_149 ) ;
F_112 ( V_138 -> V_73 , L_27 ,
V_4 -> V_163 ) ;
continue;
}
V_190 = F_113 ( & V_149 -> V_144 ) ;
if ( V_190 ) {
F_112 ( V_138 -> V_73 , L_28 ,
V_149 -> V_144 . V_162 ) ;
F_102 ( V_149 ) ;
continue;
}
F_114 ( & V_149 -> V_156 , & V_154 ) ;
V_153 = true ;
}
for ( V_151 = 0 ; V_151 < F_8 ( V_152 ) ; V_151 ++ ) {
struct V_3 * V_149 = V_152 + V_151 ;
T_1 V_218 = V_149 -> V_18 . V_140 &
V_214 ;
T_1 V_219 = V_149 -> V_18 . V_142 &
V_214 ;
T_1 V_220 = V_149 -> V_18 . V_140 & V_15 ;
if ( ! V_193 &&
( ( V_218 == V_215 ) ||
( V_218 == V_216 ) ) )
continue;
if ( ! V_194 && ( V_218 == V_217 ) )
continue;
if ( ( ( V_191 & V_206 ) == V_221 ) &&
( V_220 == V_222 ) )
continue;
if ( V_219 &&
( ! V_195 || ( V_149 -> V_2 . V_223 > V_196 ) ) )
continue;
F_105 ( V_149 ) ;
V_190 = F_115 ( & V_149 -> V_2 ) ;
if ( V_190 ) {
F_116 ( L_28 ,
V_149 -> V_2 . V_102 . V_162 ) ;
continue;
}
V_149 -> V_153 = true ;
V_153 = true ;
}
if ( V_153 )
F_117 ( V_138 -> V_73 , L_29 ) ;
return V_190 ;
}
