static int F_1 ( struct V_1 * V_2 , bool V_3 ,
bool V_4 , unsigned char V_5 ,
unsigned V_6 , unsigned V_7 )
{
T_1 * V_8 = V_2 -> V_8 ;
struct V_9 V_10 = {
. V_11 = V_8 ,
. V_12 = V_6 + 1 ,
. V_4 = V_4 ,
} ;
struct V_13 V_14 ;
V_8 [ 0 ] = V_15 | F_2 ( V_5 ) ;
switch ( V_6 ) {
case 3 :
V_8 [ 1 ] = V_7 >> 16 ;
V_8 [ 2 ] = V_7 >> 8 ;
V_8 [ 3 ] = V_7 ;
break;
case 2 :
V_8 [ 1 ] = V_7 >> 8 ;
V_8 [ 2 ] = V_7 ;
break;
case 1 :
V_8 [ 1 ] = V_7 ;
break;
default:
return - V_16 ;
}
F_3 ( & V_14 ) ;
F_4 ( & V_10 , & V_14 ) ;
if ( V_3 )
return F_5 ( V_2 -> V_17 , & V_14 ) ;
else
return F_6 ( V_2 -> V_17 , & V_14 ) ;
}
static int F_7 ( struct V_1 * V_2 ,
unsigned V_5 , unsigned V_6 , unsigned V_7 )
{
return F_1 ( V_2 , false , false , V_5 , V_6 , V_7 ) ;
}
static int F_8 ( struct V_1 * V_2 , bool V_3 ,
bool V_4 , unsigned char V_5 ,
int * V_7 , unsigned V_6 )
{
T_1 * V_8 = V_2 -> V_8 ;
int V_18 ;
struct V_9 V_10 [] = {
{
. V_11 = V_8 ,
. V_12 = 1 ,
} , {
. V_19 = V_8 ,
. V_12 = V_6 ,
. V_4 = V_4 ,
} ,
} ;
struct V_13 V_14 ;
V_8 [ 0 ] = V_20 | F_2 ( V_5 ) ;
F_3 ( & V_14 ) ;
F_4 ( & V_10 [ 0 ] , & V_14 ) ;
F_4 ( & V_10 [ 1 ] , & V_14 ) ;
if ( V_3 )
V_18 = F_5 ( V_2 -> V_17 , & V_14 ) ;
else
V_18 = F_6 ( V_2 -> V_17 , & V_14 ) ;
if ( V_18 < 0 )
return V_18 ;
switch ( V_6 ) {
case 3 :
* V_7 = V_8 [ 0 ] << 16 | V_8 [ 1 ] << 8 | V_8 [ 2 ] ;
break;
case 2 :
* V_7 = V_8 [ 0 ] << 8 | V_8 [ 1 ] ;
break;
case 1 :
* V_7 = V_8 [ 0 ] ;
break;
default:
return - V_16 ;
}
return 0 ;
}
static int F_9 ( struct V_1 * V_2 ,
unsigned V_5 , int * V_7 , unsigned V_6 )
{
return F_8 ( V_2 , 0 , 0 , V_5 , V_7 , V_6 ) ;
}
static int F_10 ( struct V_1 * V_2 , unsigned V_21 ,
unsigned V_12 , int * V_7 )
{
int V_18 ;
V_2 -> V_22 = ( V_2 -> V_22 & ~ F_11 ( - 1 ) ) |
F_11 ( 1 << V_21 ) ;
V_2 -> V_23 = ( V_2 -> V_23 & ~ F_12 ( - 1 ) ) |
F_12 ( V_24 ) ;
F_7 ( V_2 , V_25 , 3 , V_2 -> V_22 ) ;
F_13 ( V_2 -> V_17 -> V_26 ) ;
V_2 -> V_27 = false ;
V_18 = F_1 ( V_2 , 1 , 1 , V_28 , 3 , V_2 -> V_23 ) ;
if ( V_18 < 0 )
goto V_29;
V_2 -> V_30 = false ;
F_14 ( V_2 -> V_17 -> V_31 ) ;
F_15 ( V_2 -> V_32 , V_2 -> V_27 ) ;
V_18 = F_8 ( V_2 , 1 , 0 , V_33 , V_7 , V_12 ) ;
V_29:
F_16 ( V_2 -> V_17 -> V_26 ) ;
return V_18 ;
}
static int F_17 ( struct V_1 * V_2 , unsigned V_23 , unsigned V_21 )
{
int V_18 ;
V_2 -> V_22 = ( V_2 -> V_22 & ~ F_11 ( - 1 ) ) |
F_11 ( 1 << V_21 ) ;
V_2 -> V_23 = ( V_2 -> V_23 & ~ F_12 ( - 1 ) ) | F_12 ( V_23 ) ;
F_7 ( V_2 , V_25 , 3 , V_2 -> V_22 ) ;
F_13 ( V_2 -> V_17 -> V_26 ) ;
V_2 -> V_27 = false ;
V_18 = F_1 ( V_2 , 1 , 1 , V_28 , 3 ,
( V_2 -> V_34 != V_35 ) ?
V_2 -> V_23 | V_36 :
V_2 -> V_23 ) ;
if ( V_18 < 0 )
goto V_29;
V_2 -> V_30 = false ;
F_14 ( V_2 -> V_17 -> V_31 ) ;
F_15 ( V_2 -> V_32 , V_2 -> V_27 ) ;
V_2 -> V_23 = ( V_2 -> V_23 & ~ F_12 ( - 1 ) ) |
F_12 ( V_37 ) ;
V_18 = F_1 ( V_2 , 1 , 0 , V_28 , 3 , V_2 -> V_23 ) ;
V_29:
F_16 ( V_2 -> V_17 -> V_26 ) ;
return V_18 ;
}
static int F_18 ( struct V_1 * V_2 )
{
int V_38 , V_18 ;
for ( V_38 = 0 ; V_38 < F_19 ( V_39 ) ; V_38 ++ ) {
V_18 = F_17 ( V_2 , V_39 [ V_38 ] [ 0 ] ,
V_39 [ V_38 ] [ 1 ] ) ;
if ( V_18 )
goto V_29;
}
return 0 ;
V_29:
F_20 ( & V_2 -> V_17 -> V_40 , L_1 ) ;
return V_18 ;
}
static int F_21 ( struct V_1 * V_2 )
{
struct V_41 * V_42 = F_22 ( V_2 -> V_17 ) ;
struct V_43 * V_44 = V_2 -> V_44 ;
unsigned long long V_45 ;
int V_38 , V_18 , V_46 ;
T_1 V_47 [ 6 ] ;
memset ( & V_47 , 0xFF , 6 ) ;
V_18 = F_23 ( V_2 -> V_17 , & V_47 , 6 ) ;
if ( V_18 < 0 )
goto V_29;
F_24 ( 1 ) ;
V_18 = F_9 ( V_2 , V_48 , & V_46 , 1 ) ;
if ( V_18 )
goto V_29;
V_46 &= V_49 ;
if ( V_46 != V_2 -> V_34 )
F_25 ( & V_2 -> V_17 -> V_40 , L_2 , V_46 ) ;
switch ( V_44 -> V_50 ) {
case V_51 :
case V_52 :
V_2 -> V_53 = V_54 ;
break;
case V_55 :
case V_56 :
if ( V_44 -> V_57 )
V_2 -> V_53 = V_44 -> V_57 ;
else
V_2 -> V_53 = V_54 ;
break;
default:
V_18 = - V_16 ;
goto V_29;
}
V_2 -> V_23 = F_12 ( V_37 ) |
F_26 ( V_44 -> V_50 ) |
F_27 ( 480 ) ;
V_2 -> V_22 = F_28 ( 0 ) ;
if ( V_44 -> V_58 )
V_2 -> V_23 |= V_59 ;
if ( V_44 -> V_60 )
V_2 -> V_23 |= V_61 ;
if ( V_44 -> V_62 && ( V_2 -> V_34 != V_35 ) )
V_2 -> V_22 |= V_63 ;
if ( V_44 -> V_64 ) {
V_2 -> V_22 |= V_65 ;
if ( V_44 -> V_60 )
V_2 -> V_66 = 3 ;
else
V_2 -> V_66 = 4 ;
} else {
V_2 -> V_66 = 1 ;
}
if ( V_44 -> V_67 )
V_2 -> V_22 |= V_68 ;
if ( V_44 -> V_69 )
V_2 -> V_22 |= V_70 ;
if ( V_44 -> V_71 )
V_2 -> V_22 |= V_72 ;
V_18 = F_7 ( V_2 , V_28 , 3 , V_2 -> V_23 ) ;
if ( V_18 )
goto V_29;
V_18 = F_7 ( V_2 , V_25 , 3 , V_2 -> V_22 ) ;
if ( V_18 )
goto V_29;
V_18 = F_18 ( V_2 ) ;
if ( V_18 )
goto V_29;
for ( V_38 = 0 ; V_38 < F_19 ( V_2 -> V_73 ) ; V_38 ++ ) {
V_45 = ( ( V_74 ) V_2 -> V_75 * 100000000 )
>> ( V_42 -> V_76 [ 0 ] . V_77 . V_78 -
( ( V_2 -> V_22 & V_70 ) ? 0 : 1 ) ) ;
V_45 >>= V_38 ;
V_2 -> V_73 [ V_38 ] [ 1 ] = F_29 ( V_45 , 100000000 ) * 10 ;
V_2 -> V_73 [ V_38 ] [ 0 ] = V_45 ;
}
return 0 ;
V_29:
F_20 ( & V_2 -> V_17 -> V_40 , L_3 ) ;
return V_18 ;
}
static int F_30 ( struct V_41 * V_42 )
{
struct V_1 * V_2 = F_31 ( V_42 ) ;
struct V_79 * V_80 = V_42 -> V_81 ;
T_2 V_82 ;
unsigned V_83 ;
if ( F_32 ( V_42 -> V_84 , V_42 -> V_85 ) )
return - V_16 ;
V_83 = F_33 ( V_42 -> V_84 ,
V_42 -> V_85 ) ;
V_82 = F_34 ( V_42 -> V_84 ,
V_42 -> V_85 ) *
V_42 -> V_76 [ 0 ] . V_77 . V_86 / 8 ;
if ( V_80 -> V_87 ) {
V_82 += sizeof( V_88 ) ;
if ( V_82 % sizeof( V_88 ) )
V_82 += sizeof( V_88 ) - ( V_82 % sizeof( V_88 ) ) ;
}
if ( V_42 -> V_81 -> V_89 -> V_90 )
V_42 -> V_81 -> V_89 ->
V_90 ( V_42 -> V_81 , V_82 ) ;
V_2 -> V_23 = ( V_2 -> V_23 & ~ F_12 ( - 1 ) ) |
F_12 ( V_91 ) ;
V_2 -> V_22 = ( V_2 -> V_22 & ~ F_11 ( - 1 ) ) |
F_11 ( 1 << V_42 -> V_76 [ V_83 ] . V_92 ) ;
F_7 ( V_2 , V_25 , 3 , V_2 -> V_22 ) ;
F_13 ( V_2 -> V_17 -> V_26 ) ;
F_1 ( V_2 , 1 , 1 , V_28 , 3 , V_2 -> V_23 ) ;
V_2 -> V_30 = false ;
F_14 ( V_2 -> V_17 -> V_31 ) ;
return 0 ;
}
static int F_35 ( struct V_41 * V_42 )
{
struct V_1 * V_2 = F_31 ( V_42 ) ;
V_2 -> V_23 = ( V_2 -> V_23 & ~ F_12 ( - 1 ) ) |
F_12 ( V_37 ) ;
V_2 -> V_27 = false ;
F_15 ( V_2 -> V_32 , V_2 -> V_27 ) ;
if ( ! V_2 -> V_30 )
F_36 ( V_2 -> V_17 -> V_31 ) ;
F_1 ( V_2 , 1 , 0 , V_28 , 3 , V_2 -> V_23 ) ;
return F_16 ( V_2 -> V_17 -> V_26 ) ;
}
static T_3 F_37 ( int V_31 , void * V_93 )
{
struct V_94 * V_95 = V_93 ;
struct V_41 * V_42 = V_95 -> V_42 ;
struct V_79 * V_80 = V_42 -> V_81 ;
struct V_1 * V_2 = F_31 ( V_42 ) ;
V_88 V_96 [ 2 ] ;
T_4 * V_97 = ( T_4 * ) V_96 ;
if ( ! F_32 ( V_42 -> V_84 , V_42 -> V_85 ) )
F_8 ( V_2 , 1 , 1 , V_33 ,
V_97 ,
V_42 -> V_76 [ 0 ] . V_77 . V_78 / 8 ) ;
if ( V_80 -> V_87 )
V_96 [ 1 ] = V_95 -> V_98 ;
V_80 -> V_89 -> V_99 ( V_80 , ( T_1 * ) V_96 , V_95 -> V_98 ) ;
F_38 ( V_42 -> V_100 ) ;
V_2 -> V_30 = false ;
F_14 ( V_2 -> V_17 -> V_31 ) ;
return V_101 ;
}
static int F_39 ( struct V_41 * V_42 )
{
int V_18 ;
V_42 -> V_81 = F_40 ( V_42 ) ;
if ( ! V_42 -> V_81 ) {
V_18 = - V_102 ;
goto V_103;
}
V_42 -> V_81 -> V_89 = & V_104 ;
V_42 -> V_105 = F_41 ( & V_106 ,
& F_37 ,
V_107 ,
V_42 ,
L_4 ,
V_42 -> V_46 ) ;
if ( V_42 -> V_105 == NULL ) {
V_18 = - V_102 ;
goto V_108;
}
V_42 -> V_109 = & V_110 ;
V_42 -> V_111 |= V_112 ;
return 0 ;
V_108:
F_42 ( V_42 -> V_81 ) ;
V_103:
return V_18 ;
}
static void F_43 ( struct V_41 * V_42 )
{
F_44 ( V_42 -> V_105 ) ;
F_42 ( V_42 -> V_81 ) ;
}
static T_3 F_45 ( int V_31 , void * V_113 )
{
struct V_1 * V_2 = F_31 ( V_113 ) ;
V_2 -> V_27 = true ;
F_46 ( & V_2 -> V_32 ) ;
F_36 ( V_31 ) ;
V_2 -> V_30 = true ;
F_47 ( V_2 -> V_100 , F_48 () ) ;
return V_101 ;
}
static int F_49 ( struct V_41 * V_42 )
{
struct V_1 * V_2 = F_31 ( V_42 ) ;
int V_18 ;
V_2 -> V_100 = F_50 ( L_5 ,
F_51 ( V_2 -> V_17 ) -> V_114 ,
V_42 -> V_46 ) ;
if ( V_2 -> V_100 == NULL ) {
V_18 = - V_102 ;
goto V_103;
}
V_2 -> V_100 -> V_115 = & V_116 ;
V_18 = F_52 ( V_2 -> V_17 -> V_31 ,
F_45 ,
V_117 ,
F_51 ( V_2 -> V_17 ) -> V_114 ,
V_42 ) ;
if ( V_18 )
goto V_118;
F_36 ( V_2 -> V_17 -> V_31 ) ;
V_2 -> V_30 = true ;
V_2 -> V_100 -> V_40 . V_119 = & V_2 -> V_17 -> V_40 ;
V_2 -> V_100 -> V_120 = V_42 ;
V_18 = F_53 ( V_2 -> V_100 ) ;
V_42 -> V_100 = V_2 -> V_100 ;
if ( V_18 )
goto V_121;
return 0 ;
V_121:
F_54 ( V_2 -> V_17 -> V_31 , V_42 ) ;
V_118:
F_55 ( V_2 -> V_100 ) ;
V_103:
return V_18 ;
}
static void F_56 ( struct V_41 * V_42 )
{
struct V_1 * V_2 = F_31 ( V_42 ) ;
F_57 ( V_2 -> V_100 ) ;
F_54 ( V_2 -> V_17 -> V_31 , V_42 ) ;
F_55 ( V_2 -> V_100 ) ;
}
static T_5 F_58 ( struct V_122 * V_40 ,
struct V_123 * V_124 ,
char * V_125 )
{
struct V_41 * V_42 = F_59 ( V_40 ) ;
struct V_1 * V_2 = F_31 ( V_42 ) ;
return sprintf ( V_125 , L_6 , V_2 -> V_53 /
( V_2 -> V_66 * 1024 * F_27 ( V_2 -> V_23 ) ) ) ;
}
static T_5 F_60 ( struct V_122 * V_40 ,
struct V_123 * V_124 ,
const char * V_125 ,
T_2 V_12 )
{
struct V_41 * V_42 = F_59 ( V_40 ) ;
struct V_1 * V_2 = F_31 ( V_42 ) ;
unsigned long V_126 ;
int div , V_18 ;
V_18 = F_61 ( V_125 , 10 , & V_126 ) ;
if ( V_18 )
return V_18 ;
F_62 ( & V_42 -> V_127 ) ;
if ( F_63 ( V_42 ) ) {
F_64 ( & V_42 -> V_127 ) ;
return - V_128 ;
}
div = V_2 -> V_53 / ( V_126 * V_2 -> V_66 * 1024 ) ;
if ( div < 1 || div > 1023 ) {
V_18 = - V_16 ;
goto V_29;
}
V_2 -> V_23 &= ~ F_27 ( - 1 ) ;
V_2 -> V_23 |= F_27 ( div ) ;
F_7 ( V_2 , V_28 , 3 , V_2 -> V_23 ) ;
V_29:
F_64 ( & V_42 -> V_127 ) ;
return V_18 ? V_18 : V_12 ;
}
static T_5 F_65 ( struct V_122 * V_40 ,
struct V_123 * V_124 , char * V_125 )
{
struct V_41 * V_42 = F_59 ( V_40 ) ;
struct V_1 * V_2 = F_31 ( V_42 ) ;
int V_38 , V_12 = 0 ;
for ( V_38 = 0 ; V_38 < F_19 ( V_2 -> V_73 ) ; V_38 ++ )
V_12 += sprintf ( V_125 + V_12 , L_7 , V_2 -> V_73 [ V_38 ] [ 0 ] ,
V_2 -> V_73 [ V_38 ] [ 1 ] ) ;
V_12 += sprintf ( V_125 + V_12 , L_8 ) ;
return V_12 ;
}
static T_5 F_66 ( struct V_122 * V_40 ,
struct V_123 * V_124 ,
char * V_125 )
{
struct V_41 * V_42 = F_59 ( V_40 ) ;
struct V_1 * V_2 = F_31 ( V_42 ) ;
return sprintf ( V_125 , L_6 , ! ! ( V_2 -> V_23 & V_129 ) ) ;
}
static T_5 F_67 ( struct V_122 * V_40 ,
struct V_123 * V_124 ,
char * V_125 )
{
struct V_41 * V_42 = F_59 ( V_40 ) ;
struct V_1 * V_2 = F_31 ( V_42 ) ;
return sprintf ( V_125 , L_6 , ! ! ( V_2 -> V_130 & V_131 ) ) ;
}
static T_5 F_68 ( struct V_122 * V_40 ,
struct V_123 * V_124 ,
const char * V_125 ,
T_2 V_12 )
{
struct V_41 * V_42 = F_59 ( V_40 ) ;
struct V_1 * V_2 = F_31 ( V_42 ) ;
struct V_132 * V_133 = F_69 ( V_124 ) ;
int V_18 ;
bool V_7 ;
V_18 = F_70 ( V_125 , & V_7 ) ;
if ( V_18 < 0 )
return V_18 ;
F_62 ( & V_42 -> V_127 ) ;
if ( F_63 ( V_42 ) ) {
F_64 ( & V_42 -> V_127 ) ;
return - V_128 ;
}
switch ( ( V_134 ) V_133 -> V_92 ) {
case V_135 :
if ( V_7 )
V_2 -> V_130 |= V_131 ;
else
V_2 -> V_130 &= ~ V_131 ;
F_7 ( V_2 , V_135 , 1 , V_2 -> V_130 ) ;
break;
case V_28 :
if ( V_7 )
V_2 -> V_23 |= V_129 ;
else
V_2 -> V_23 &= ~ V_129 ;
F_7 ( V_2 , V_135 , 3 , V_2 -> V_23 ) ;
break;
default:
V_18 = - V_16 ;
}
F_64 ( & V_42 -> V_127 ) ;
return V_18 ? V_18 : V_12 ;
}
static T_6 F_71 ( struct V_136 * V_137 ,
struct V_138 * V_124 , int V_139 )
{
struct V_122 * V_40 = F_72 ( V_137 , struct V_122 , V_137 ) ;
struct V_41 * V_42 = F_59 ( V_40 ) ;
struct V_1 * V_2 = F_31 ( V_42 ) ;
T_6 V_23 = V_124 -> V_23 ;
if ( ( V_2 -> V_34 != V_35 ) &&
( V_124 == & V_140 . V_141 . V_124 ) )
V_23 = 0 ;
return V_23 ;
}
static int F_73 ( struct V_41 * V_42 ,
struct V_142 const * V_143 ,
int * V_7 ,
int * V_144 ,
long V_14 )
{
struct V_1 * V_2 = F_31 ( V_42 ) ;
int V_18 , V_145 = 0 ;
bool V_146 = ! ! ( V_2 -> V_22 & V_70 ) ;
switch ( V_14 ) {
case 0 :
F_62 ( & V_42 -> V_127 ) ;
if ( F_63 ( V_42 ) )
V_18 = - V_128 ;
else
V_18 = F_10 ( V_2 , V_143 -> V_92 ,
V_143 -> V_77 . V_78 / 8 , & V_145 ) ;
F_64 ( & V_42 -> V_127 ) ;
if ( V_18 < 0 )
return V_18 ;
* V_7 = ( V_145 >> V_143 -> V_77 . V_147 ) &
( ( 1 << ( V_143 -> V_77 . V_78 ) ) - 1 ) ;
switch ( V_143 -> type ) {
case V_148 :
if ( ! V_146 )
* V_7 -= ( 1 << ( V_143 -> V_77 . V_78 - 1 ) ) ;
break;
case V_149 :
* V_7 -= 0x800000 ;
* V_7 /= 2815 ;
* V_7 -= 273 ;
break;
default:
return - V_16 ;
}
return V_150 ;
case V_151 :
switch ( V_143 -> type ) {
case V_148 :
F_62 ( & V_42 -> V_127 ) ;
* V_7 = V_2 -> V_73 [ F_28 ( V_2 -> V_22 ) ] [ 0 ] ;
* V_144 = V_2 -> V_73 [ F_28 ( V_2 -> V_22 ) ] [ 1 ] ;
F_64 ( & V_42 -> V_127 ) ;
return V_152 ;
case V_149 :
* V_7 = 1000 ;
return V_150 ;
default:
return - V_16 ;
}
}
return - V_16 ;
}
static int F_74 ( struct V_41 * V_42 ,
struct V_142 const * V_143 ,
int V_7 ,
int V_144 ,
long V_153 )
{
struct V_1 * V_2 = F_31 ( V_42 ) ;
int V_18 , V_38 ;
unsigned int V_154 ;
F_62 ( & V_42 -> V_127 ) ;
if ( F_63 ( V_42 ) ) {
F_64 ( & V_42 -> V_127 ) ;
return - V_128 ;
}
switch ( V_153 ) {
case V_151 :
V_18 = - V_16 ;
for ( V_38 = 0 ; V_38 < F_19 ( V_2 -> V_73 ) ; V_38 ++ )
if ( V_144 == V_2 -> V_73 [ V_38 ] [ 1 ] ) {
V_154 = V_2 -> V_22 ;
V_2 -> V_22 &= ~ F_28 ( - 1 ) ;
V_2 -> V_22 |= F_28 ( V_38 ) ;
if ( V_154 != V_2 -> V_22 ) {
F_7 ( V_2 , V_25 ,
3 , V_2 -> V_22 ) ;
F_18 ( V_2 ) ;
}
V_18 = 0 ;
}
default:
V_18 = - V_16 ;
}
F_64 ( & V_42 -> V_127 ) ;
return V_18 ;
}
static int F_75 ( struct V_41 * V_42 ,
struct V_155 * V_100 )
{
if ( V_42 -> V_100 != V_100 )
return - V_16 ;
return 0 ;
}
static int F_76 ( struct V_41 * V_42 ,
struct V_142 const * V_143 ,
long V_153 )
{
return V_152 ;
}
static int T_7 F_77 ( struct V_156 * V_17 )
{
struct V_43 * V_44 = V_17 -> V_40 . V_157 ;
struct V_1 * V_2 ;
struct V_41 * V_42 ;
int V_18 , V_38 , V_158 = 0 ;
if ( ! V_44 ) {
F_20 ( & V_17 -> V_40 , L_9 ) ;
return - V_159 ;
}
if ( ! V_17 -> V_31 ) {
F_20 ( & V_17 -> V_40 , L_10 ) ;
return - V_159 ;
}
V_42 = F_78 ( sizeof( * V_2 ) ) ;
if ( V_42 == NULL )
return - V_102 ;
V_2 = F_31 ( V_42 ) ;
V_2 -> V_5 = F_79 ( & V_17 -> V_40 , L_11 ) ;
if ( ! F_80 ( V_2 -> V_5 ) ) {
V_18 = F_81 ( V_2 -> V_5 ) ;
if ( V_18 )
goto V_160;
V_158 = F_82 ( V_2 -> V_5 ) ;
}
V_2 -> V_44 = V_44 ;
if ( V_44 && V_44 -> V_161 )
V_2 -> V_75 = V_44 -> V_161 ;
else if ( V_158 )
V_2 -> V_75 = V_158 / 1000 ;
else
F_25 ( & V_17 -> V_40 , L_12 ) ;
F_83 ( V_17 , V_42 ) ;
V_2 -> V_17 = V_17 ;
V_2 -> V_34 = F_51 ( V_17 ) -> V_162 ;
V_42 -> V_40 . V_119 = & V_17 -> V_40 ;
V_42 -> V_114 = F_51 ( V_17 ) -> V_114 ;
V_42 -> V_111 = V_163 ;
V_42 -> V_76 = V_164 ;
V_42 -> V_165 = F_19 ( V_164 ) ;
V_42 -> V_166 = V_2 -> V_166 ;
V_42 -> V_167 = & V_168 ;
for ( V_38 = 0 ; V_38 < V_42 -> V_165 ; V_38 ++ )
V_2 -> V_166 [ V_38 ] = ( 1 << V_38 ) | ( 1 <<
V_42 -> V_76 [ V_42 -> V_165 - 1 ] .
V_169 ) ;
F_84 ( & V_2 -> V_32 ) ;
V_18 = F_39 ( V_42 ) ;
if ( V_18 )
goto V_170;
V_18 = F_49 ( V_42 ) ;
if ( V_18 )
goto V_171;
V_18 = F_85 ( V_42 ,
V_42 -> V_76 ,
V_42 -> V_165 ) ;
if ( V_18 )
goto V_172;
V_18 = F_21 ( V_2 ) ;
if ( V_18 )
goto V_173;
V_18 = F_86 ( V_42 ) ;
if ( V_18 < 0 )
goto V_173;
return 0 ;
V_173:
F_87 ( V_42 ) ;
V_172:
F_56 ( V_42 ) ;
V_171:
F_43 ( V_42 ) ;
V_170:
if ( ! F_80 ( V_2 -> V_5 ) )
F_88 ( V_2 -> V_5 ) ;
V_160:
if ( ! F_80 ( V_2 -> V_5 ) )
F_89 ( V_2 -> V_5 ) ;
F_90 ( V_42 ) ;
return V_18 ;
}
static int F_91 ( struct V_156 * V_17 )
{
struct V_41 * V_42 = F_22 ( V_17 ) ;
struct V_1 * V_2 = F_31 ( V_42 ) ;
F_92 ( V_42 ) ;
F_87 ( V_42 ) ;
F_56 ( V_42 ) ;
F_43 ( V_42 ) ;
if ( ! F_80 ( V_2 -> V_5 ) ) {
F_88 ( V_2 -> V_5 ) ;
F_89 ( V_2 -> V_5 ) ;
}
return 0 ;
}
