static void F_1 ( T_1 * V_1 , int V_2 )
{
F_2 ( V_1 -> V_3 , V_4 ,
F_3 ( V_1 -> V_3 , V_4 ) | ( 0x1 << V_2 ) ) ;
}
static void F_4 ( T_1 * V_1 , int V_2 )
{
F_2 ( V_1 -> V_3 , V_4 ,
F_3 ( V_1 -> V_3 , V_4 ) & ~ ( 0x1 << V_2 ) ) ;
}
static void
F_5 ( T_1 * V_1 , unsigned char V_5 ,
unsigned char V_6 )
{
int V_7 ;
F_2 ( V_1 -> V_3 , V_8 + ( V_5 << 2 ) , V_6 ) ;
if ( 1 ) {
V_7 = F_3 ( V_1 -> V_3 , V_9 + ( V_5 << 2 ) ) ;
if ( ( V_7 != 0x80 ) || ( V_6 == 0x80 ) )
return;
}
F_2 ( V_1 -> V_3 , V_9 + ( V_5 << 2 ) , V_6 ) ;
}
static void
F_6 ( T_1 * V_1 , unsigned char V_5 ,
int V_10 , unsigned char V_6 )
{
int V_7 ;
F_2 ( V_1 -> V_3 ,
V_11 + ( ( ( V_5 << 5 ) + V_10 ) << 2 ) , V_6 ) ;
if ( 1 ) {
V_7 =
F_3 ( V_1 -> V_3 ,
V_12 + ( ( ( V_5 << 5 ) + V_10 ) << 2 ) ) ;
if ( ( V_7 != 0x80 ) || ( V_6 == 0x80 ) )
return;
}
F_2 ( V_1 -> V_3 ,
V_12 + ( ( ( V_5 << 5 ) + V_10 ) << 2 ) , V_6 ) ;
}
static void
F_7 ( T_1 * V_1 , unsigned char V_5 , int V_10 , int V_13 )
{
int V_7 , V_14 ;
if ( V_10 < 0 )
V_14 = ( V_10 + 3 ) ;
else
V_14 = V_10 ;
V_14 = ( ( V_5 << 3 ) + ( V_14 >> 2 ) ) << 2 ;
V_7 = F_3 ( V_1 -> V_3 , V_15 + V_14 ) ;
if ( V_13 )
V_7 |= ( 1 << ( V_10 & 3 ) ) ;
else
V_7 &= ~ ( 1 << ( V_10 & 3 ) ) ;
F_2 ( V_1 -> V_3 ,
V_12 + ( ( ( V_5 << 5 ) + V_10 ) << 2 ) , 0x80 ) ;
F_2 ( V_1 -> V_3 , V_16 + ( V_10 << 2 ) , 0x0 ) ;
F_2 ( V_1 -> V_3 , V_16 + 4 + ( V_10 << 2 ) , 0x0 ) ;
F_2 ( V_1 -> V_3 , V_15 + V_14 , V_7 ) ;
}
static void
F_8 ( T_1 * V_1 , unsigned char V_5 , int V_10 )
{
V_17 [ V_5 ] &= ~ ( 1 << V_10 ) ;
F_6 ( V_1 , V_5 , V_10 , 0x80 ) ;
V_18 [ V_5 ] &= ~ ( 1 << V_10 ) ;
F_7 ( V_1 , V_5 , V_10 , 0 ) ;
}
static void
F_9 ( T_1 * V_1 , unsigned char V_5 , int V_10 )
{
F_8 ( V_1 , V_5 , V_10 ) ;
if ( ( V_18 [ V_5 ] & ( 1 << V_10 ) ) == 0 ) {
F_6 ( V_1 , V_5 , V_10 , 0x80 ) ;
V_18 [ V_5 ] |= ( 1 << V_10 ) ;
}
F_7 ( V_1 , V_5 , V_10 , 1 ) ;
}
static void
F_10 ( T_1 * V_1 , unsigned char V_5 , int V_2 ,
int V_19 )
{
if ( V_19 ) {
V_17 [ V_5 ] |= ( 1 << V_2 ) ;
F_8 ( V_1 , V_5 , V_2 ) ;
} else
F_8 ( V_1 , V_5 , V_2 ) ;
}
static int
F_11 ( T_1 * V_1 , unsigned char V_5 , unsigned char V_20 )
{
int V_7 , V_21 = 0 , V_22 ;
V_7 = F_3 ( V_1 -> V_3 , V_4 ) ;
if ( ( V_7 & ( 1 << V_20 ) ) == 0 ) {
F_2 ( V_1 -> V_3 , V_23 + ( V_20 << 2 ) , V_5 ) ;
F_1 ( V_1 , V_20 ) ;
return 1 ;
}
V_22 = V_23 + ( V_20 << 2 ) ;
V_7 = F_3 ( V_1 -> V_3 , V_22 ) ;
while ( V_7 & 0x10 ) {
V_22 = V_24 + ( ( V_7 & 0xf ) << 2 ) ;
V_7 = F_3 ( V_1 -> V_3 , V_22 ) ;
if ( ( ++ V_21 ) > 0xf ) {
F_12 ( V_25
L_1 ) ;
return 0 ;
}
}
F_2 ( V_1 -> V_3 , V_24 + ( ( V_7 & 0xf ) << 2 ) , V_5 ) ;
F_2 ( V_1 -> V_3 , V_22 , ( V_7 & 0xf ) | 0x10 ) ;
return 1 ;
}
static int
F_13 ( T_1 * V_1 , unsigned char V_5 , unsigned char V_20 )
{
int V_26 = - 1 , V_27 , V_28 , V_29 , V_30 , V_31 , V_32 = 0 ;
V_28 = F_3 ( V_1 -> V_3 , V_4 ) ;
if ( ( ( 1 << V_20 ) & V_28 ) == 0 ) {
F_12 ( V_25 L_2 , V_28 ) ;
return 0 ;
}
V_31 = V_23 + ( V_20 << 2 ) ;
V_27 = F_3 ( V_1 -> V_3 , V_31 ) ;
if ( V_27 & 0x10 ) {
V_29 = ( V_27 & 0xf ) ;
if ( V_5 == V_29 ) {
V_29 = V_24 + ( V_5 << 2 ) ;
V_30 = F_3 ( V_1 -> V_3 , V_29 ) ;
F_2 ( V_1 -> V_3 , V_31 , V_30 ) ;
F_2 ( V_1 -> V_3 , V_29 , 0 ) ;
} else {
V_30 =
F_3 ( V_1 -> V_3 ,
V_24 + ( V_29 << 2 ) ) ;
while ( ( V_30 & 0xf ) != V_5 ) {
if ( ( V_32 ) > 0xf ) {
F_12 ( V_25
L_3 ) ;
return 0 ;
}
V_26 = V_29 ;
V_29 = V_30 & 0xf ;
V_31 = V_29 << 2 ;
V_30 =
F_3 ( V_1 -> V_3 ,
V_24 + V_31 ) ;
V_32 ++ ;
}
V_31 = V_29 << 2 ;
if ( V_30 & 0x10 ) {
V_29 = V_24 + ( ( V_30 & 0xf ) << 2 ) ;
V_30 = F_3 ( V_1 -> V_3 , V_29 ) ;
F_2 ( V_1 -> V_3 ,
V_24 + V_31 , V_30 ) ;
F_2 ( V_1 -> V_3 , V_29 , 0 ) ;
} else {
if ( V_26 == - 1 )
F_2 ( V_1 -> V_3 ,
V_23 +
( V_20 << 2 ) , V_27 & 0xef ) ;
else {
V_29 = ( 0xffffffe0 & V_30 ) | ( 0xf & V_29 ) ;
F_2 ( V_1 -> V_3 ,
V_24 +
( V_26 << 2 ) , V_29 ) ;
}
F_2 ( V_1 -> V_3 ,
V_24 + V_31 , 0 ) ;
return 1 ;
}
}
} else {
F_4 ( V_1 , V_20 ) ;
F_2 ( V_1 -> V_3 , V_31 , 0 ) ;
}
return 1 ;
}
static void F_14 ( T_1 * V_1 )
{
T_2 V_14 ;
int V_33 ;
memset ( V_18 , 0 , V_34 * sizeof( int ) ) ;
memset ( V_17 , 0 , V_34 * sizeof( int ) ) ;
V_14 = V_16 + 0x17c ;
for ( V_33 = 0x5f ; V_33 >= 0 ; V_33 -- ) {
F_2 ( V_1 -> V_3 , V_14 , 0 ) ;
V_14 -= 4 ;
}
V_14 = V_15 + 0x1fc ;
for ( V_33 = 0x7f ; V_33 >= 0 ; V_33 -- ) {
F_2 ( V_1 -> V_3 , V_14 , 0 ) ;
V_14 -= 4 ;
}
V_14 = V_16 + 0x17c ;
for ( V_33 = 0x5f ; V_33 >= 0 ; V_33 -- ) {
F_2 ( V_1 -> V_3 , V_14 , 0 ) ;
V_14 -= 4 ;
}
V_14 = V_11 + 0x7fc ;
for ( V_33 = 0x1ff ; V_33 >= 0 ; V_33 -- ) {
F_2 ( V_1 -> V_3 , V_14 , 0x80 ) ;
V_14 -= 4 ;
}
V_14 = V_8 + 0x3c ;
for ( V_33 = 0xf ; V_33 >= 0 ; V_33 -- ) {
F_2 ( V_1 -> V_3 , V_14 , 0x80 ) ;
V_14 -= 4 ;
}
V_14 = V_12 + 0x7fc ;
for ( V_33 = 0x1ff ; V_33 >= 0 ; V_33 -- ) {
F_2 ( V_1 -> V_3 , V_14 , 0x80 ) ;
V_14 -= 4 ;
}
V_14 = V_9 + 0x3c ;
for ( V_33 = 0xf ; V_33 >= 0 ; V_33 -- ) {
F_2 ( V_1 -> V_3 , V_14 , 0x80 ) ;
V_14 -= 4 ;
}
V_14 = V_24 + ( V_35 - 1 ) * 4 ;
for ( V_33 = ( V_35 - 1 ) ; V_33 >= 0 ; V_33 -- ) {
F_2 ( V_1 -> V_3 , V_14 , 0x0 ) ;
V_14 -= 4 ;
}
F_2 ( V_1 -> V_3 , V_4 , 0 ) ;
}
static void F_15 ( T_1 * V_1 , int V_2 )
{
F_2 ( V_1 -> V_3 , V_36 ,
F_3 ( V_1 -> V_3 , V_36 ) | ( 0x1 << V_2 ) ) ;
}
static void F_16 ( T_1 * V_1 , int V_2 )
{
F_2 ( V_1 -> V_3 , V_36 ,
F_3 ( V_1 -> V_3 , V_36 ) & ~ ( 0x1 << V_2 ) ) ;
}
static void F_17 ( T_1 * V_1 , unsigned char V_37 )
{
int V_38 ;
for ( V_38 = 0x1f ; V_38 >= 0 ; V_38 -- )
F_2 ( V_1 -> V_3 ,
V_39 + ( V_37 << 7 ) + ( V_38 << 2 ) , 0 ) ;
F_2 ( V_1 -> V_3 , V_40 + ( V_37 << 3 ) , 0 ) ;
F_2 ( V_1 -> V_3 , V_40 + ( V_37 << 3 ) + 4 , 0 ) ;
}
static void F_18 ( T_1 * V_1 , unsigned char V_37 )
{
F_2 ( V_1 -> V_3 , V_41 + ( V_37 << 2 ) , 0 ) ;
F_2 ( V_1 -> V_3 , V_42 + ( V_37 << 2 ) , 0 ) ;
F_2 ( V_1 -> V_3 , V_43 + ( V_37 << 2 ) , 1 ) ;
}
static void
F_19 ( T_1 * V_1 , unsigned char V_37 , int V_13 )
{
int V_7 ;
V_7 = F_3 ( V_1 -> V_3 , V_44 ) ;
if ( V_13 )
V_7 |= 1 << V_37 ;
else
V_7 &= ~ ( 1 << V_37 ) ;
F_2 ( V_1 -> V_3 , V_44 , V_7 ) ;
}
static int
F_20 ( T_1 * V_1 , unsigned char V_37 , int V_45 )
{
int V_7 , V_21 = 0 ;
do {
F_2 ( V_1 -> V_3 , V_46 + ( V_37 << 2 ) , V_45 ) ;
V_7 = F_3 ( V_1 -> V_3 , V_46 + ( V_37 << 2 ) ) ;
if ( ( ++ V_21 ) > 0x9 ) {
F_12 ( V_25 L_4 ) ;
break;
}
}
while ( V_7 != V_45 );
return V_7 ;
}
static void F_21 ( T_1 * V_47 , unsigned char V_37 ,
unsigned int V_48 , unsigned int V_49 , int V_50 , int V_51 ,
int V_52 , int V_53 , unsigned int V_54 , int V_55 )
{
int V_32 , V_31 = 0 , V_56 ;
F_17 ( V_47 , V_37 ) ;
if ( V_50 ) {
if ( ( V_54 & 0x10000 ) && ( V_54 != 0x10000 ) ) {
V_54 = 0 ;
V_32 = 0x7 ;
} else {
if ( ( ( ( short ) V_54 ) < 0 ) && ( V_54 != 0x8000 ) ) {
V_54 = 0 ;
V_32 = 0x8 ;
} else {
V_54 = 1 ;
V_32 = 0xc ;
}
}
} else {
if ( ( V_48 & 0x10000 ) && ( V_48 != 0x10000 ) ) {
V_54 = 0 ;
V_32 = 0x11 - ( ( V_48 >> 0xe ) & 7 ) ;
if ( V_48 & 0x3fff )
V_32 -= 1 ;
else
V_32 -= 2 ;
} else {
V_54 = 1 ;
V_32 = 0xc ;
}
}
F_18 ( V_47 , V_37 ) ;
F_19 ( V_47 , V_37 , V_55 ) ;
if ( ( V_52 == 0 ) && ( V_50 == 0 ) ) {
if ( V_54 )
V_56 = 0xf ;
else
V_56 = 0xc ;
V_31 = 0 ;
} else {
if ( V_54 )
V_31 = 0xf ;
else
V_31 = 0xc ;
V_56 = 0 ;
}
F_2 ( V_47 -> V_3 , V_57 + ( V_37 << 2 ) ,
( V_53 << 0x9 ) | ( V_50 << 0x8 ) | ( ( V_32 & 0xf ) << 4 ) | V_51 ) ;
F_20 ( V_47 , V_37 , V_48 ) ;
F_2 ( V_47 -> V_3 , V_58 + ( V_37 << 2 ) , V_49 & 0xffff ) ;
F_2 ( V_47 -> V_3 , V_59 + ( V_37 << 2 ) ,
( V_54 << 0x11 ) | ( V_52 << 0x10 ) | ( V_31 << 0x8 ) | V_56 ) ;
}
static void F_22 ( T_1 * V_1 )
{
T_2 V_14 ;
int V_33 ;
F_2 ( V_1 -> V_3 , V_60 , 0x1ff ) ;
V_14 = V_61 + 0x3c ;
for ( V_33 = 0xf ; V_33 >= 0 ; V_33 -- ) {
F_2 ( V_1 -> V_3 , V_14 , 0 ) ;
V_14 -= 4 ;
}
V_14 = V_62 + 0x54 ;
for ( V_33 = 0x15 ; V_33 >= 0 ; V_33 -- ) {
F_2 ( V_1 -> V_3 , V_14 , 0 ) ;
V_14 -= 4 ;
}
}
static int
F_23 ( T_1 * V_1 , unsigned char V_37 , unsigned char V_20 )
{
int V_7 , V_21 = 0 , V_22 ;
V_7 = F_3 ( V_1 -> V_3 , V_36 ) ;
if ( ( V_7 & ( 1 << V_20 ) ) == 0 ) {
F_2 ( V_1 -> V_3 , V_62 + ( V_20 << 2 ) , V_37 ) ;
F_15 ( V_1 , V_20 ) ;
return 1 ;
}
V_22 = V_62 + ( V_20 << 2 ) ;
V_7 = F_3 ( V_1 -> V_3 , V_22 ) ;
while ( V_7 & 0x10 ) {
V_22 = V_61 + ( ( V_7 & 0xf ) << 2 ) ;
V_7 = F_3 ( V_1 -> V_3 , V_22 ) ;
if ( ( ++ V_21 ) > 0xf ) {
F_12 ( V_25
L_5 ) ;
return 0 ;
}
}
F_2 ( V_1 -> V_3 , V_61 + ( ( V_7 & 0xf ) << 2 ) , V_37 ) ;
F_2 ( V_1 -> V_3 , V_22 , ( V_7 & 0xf ) | 0x10 ) ;
return 1 ;
}
static int
F_24 ( T_1 * V_1 , unsigned char V_37 , unsigned char V_20 )
{
int V_26 = - 1 , V_27 , V_28 , V_29 , V_30 , V_31 , V_32 = 0 ;
V_28 = F_3 ( V_1 -> V_3 , V_36 ) ;
if ( ( ( 1 << V_20 ) & V_28 ) == 0 ) {
F_12 ( V_25 L_6 ) ;
return 0 ;
}
V_31 = V_62 + ( V_20 << 2 ) ;
V_27 = F_3 ( V_1 -> V_3 , V_31 ) ;
if ( V_27 & 0x10 ) {
V_29 = ( V_27 & 0xf ) ;
if ( V_37 == V_29 ) {
V_29 = V_61 + ( V_37 << 2 ) ;
V_30 = F_3 ( V_1 -> V_3 , V_29 ) ;
F_2 ( V_1 -> V_3 , V_31 , V_30 ) ;
F_2 ( V_1 -> V_3 , V_29 , 0 ) ;
} else {
V_30 =
F_3 ( V_1 -> V_3 ,
V_61 + ( V_29 << 2 ) ) ;
while ( ( V_30 & 0xf ) != V_37 ) {
if ( ( V_32 ) > 0xf ) {
F_12
( L_7 ) ;
return 0 ;
}
V_26 = V_29 ;
V_29 = V_30 & 0xf ;
V_31 = V_29 << 2 ;
V_30 =
F_3 ( V_1 -> V_3 ,
V_61 + V_31 ) ;
V_32 ++ ;
}
V_31 = V_29 << 2 ;
if ( V_30 & 0x10 ) {
V_29 = V_61 + ( ( V_30 & 0xf ) << 2 ) ;
V_30 = F_3 ( V_1 -> V_3 , V_29 ) ;
F_2 ( V_1 -> V_3 ,
V_61 + V_31 , V_30 ) ;
F_2 ( V_1 -> V_3 , V_29 , 0 ) ;
} else {
if ( V_26 == - 1 )
F_2 ( V_1 -> V_3 ,
V_62 +
( V_20 << 2 ) , V_27 & 0xef ) ;
else {
V_29 = ( 0xffffffe0 & V_30 ) | ( 0xf & V_29 ) ;
F_2 ( V_1 -> V_3 ,
V_61 +
( V_26 << 2 ) , V_29 ) ;
}
F_2 ( V_1 -> V_3 ,
V_61 + V_31 , 0 ) ;
return 1 ;
}
}
} else {
F_16 ( V_1 , V_20 ) ;
F_2 ( V_1 -> V_3 , V_31 , 0 ) ;
}
return 1 ;
}
static void
F_25 ( T_1 * V_1 , int V_63 , int V_33 )
{
for ( V_33 -- ; V_33 >= 0 ; V_33 -- )
F_2 ( V_1 -> V_3 ,
V_64 +
( ( ( V_63 << V_65 ) + V_33 ) << 2 ) , 0 ) ;
}
static void F_26 ( T_1 * V_1 , int V_63 , int V_13 )
{
F_2 ( V_1 -> V_3 , V_66 + ( V_63 << 2 ) ,
( F_3 ( V_1 -> V_3 , V_66 + ( V_63 << 2 ) ) &
0xffffffef ) | ( ( 1 & V_13 ) << 4 ) | V_67 ) ;
}
static void
F_27 ( T_1 * V_1 , int V_63 , int V_68 , int V_69 ,
int V_70 , int V_71 , int V_72 )
{
int V_7 , V_21 = 0 ;
int V_73 = 0x2 ;
do {
V_7 = F_3 ( V_1 -> V_3 , V_66 + ( V_63 << 2 ) ) ;
if ( V_21 ++ > 0xbb8 ) {
F_12 ( V_25
L_8 ) ;
break;
}
}
while ( V_7 & V_74 );
if ( V_71 ) {
if ( ( V_7 & V_75 ) == 0 ) {
F_25 ( V_1 , V_63 , V_76 ) ;
#ifdef F_28
V_7 = ( V_73 & 0x1f ) << 0xb ;
#else
V_7 = ( V_73 & 0x3f ) << 0xc ;
#endif
V_7 = ( V_7 & 0xfffffffd ) | ( ( V_68 & 1 ) << 1 ) ;
V_7 = ( V_7 & 0xfffffff3 ) | ( ( V_69 & 3 ) << 2 ) ;
V_7 = ( V_7 & 0xffffffef ) | ( ( V_71 & 1 ) << 4 ) ;
V_7 |= V_67 ;
V_7 = ( V_7 & 0xffffffdf ) | ( ( V_70 & 1 ) << 5 ) ;
#ifdef F_28
V_7 = ( V_7 & 0xfffbffff ) | ( ( V_72 & 1 ) << 0x12 ) ;
#endif
#ifdef F_29
V_7 = ( V_7 & 0xf7ffffff ) | ( ( V_72 & 1 ) << 0x1b ) ;
V_7 = ( V_7 & 0xefffffff ) | ( ( V_72 & 1 ) << 0x1c ) ;
#endif
#ifdef F_30
V_7 = ( V_7 & 0xfeffffff ) | ( ( V_72 & 1 ) << 0x18 ) ;
V_7 = ( V_7 & 0xfdffffff ) | ( ( V_72 & 1 ) << 0x19 ) ;
#endif
}
} else {
if ( V_7 & V_75 ) {
#ifdef F_28
V_7 = ( ( V_72 & 1 ) << 0x12 ) | ( V_7 & 0xfffbffef ) ;
#endif
#ifdef F_29
V_7 =
( ( V_72 & 1 ) << 0x1b ) | ( V_7 & 0xe7ffffef ) | V_77 ;
#endif
#ifdef F_30
V_7 =
( ( V_72 & 1 ) << 0x18 ) | ( V_7 & 0xfcffffef ) | V_77 ;
#endif
} else
F_25 ( V_1 , V_63 , V_76 ) ;
}
F_2 ( V_1 -> V_3 , V_66 + ( V_63 << 2 ) , V_7 ) ;
F_3 ( V_1 -> V_3 , V_66 + ( V_63 << 2 ) ) ;
}
static void F_31 ( T_1 * V_1 , int V_63 , int V_33 )
{
if ( V_33 < 1 )
return;
for ( V_33 -- ; V_33 >= 0 ; V_33 -- )
F_2 ( V_1 -> V_3 ,
V_78 +
( ( ( V_63 << V_65 ) + V_33 ) << 2 ) , 0 ) ;
}
static void F_32 ( T_1 * V_1 , int V_63 , int V_79 )
{
F_31 ( V_1 , V_63 , V_76 ) ;
#ifdef F_28
F_2 ( V_1 -> V_3 , V_80 + ( V_63 << 2 ) ,
( V_67 | ( ( V_79 & V_81 ) << 0xb ) ) ) ;
#else
F_2 ( V_1 -> V_3 , V_80 + ( V_63 << 2 ) ,
( V_67 | ( ( V_79 & V_81 ) << 0xc ) ) ) ;
#endif
}
static void F_33 ( T_1 * V_1 , int V_63 , int V_13 )
{
F_2 ( V_1 -> V_3 , V_80 + ( V_63 << 2 ) ,
( F_3 ( V_1 -> V_3 , V_80 + ( V_63 << 2 ) ) &
0xffffffef ) | ( ( V_13 & 1 ) << 4 ) | V_67 ) ;
}
static void
F_34 ( T_1 * V_1 , int V_63 , int V_82 , int V_69 ,
int V_70 , int V_71 , int V_72 )
{
int V_7 = 0 , V_21 = 0 ;
int V_73 = 2 ;
do {
V_7 = F_3 ( V_1 -> V_3 , V_80 + ( V_63 << 2 ) ) ;
if ( V_21 ++ > 0xbb8 ) {
F_12 ( V_25 L_9 ) ;
break;
}
}
while ( V_7 & V_74 );
if ( V_71 ) {
if ( ( V_7 & V_75 ) == 0 ) {
F_31 ( V_1 , V_63 , V_76 ) ;
#ifdef F_28
V_7 = ( V_73 & 0x1f ) << 0xb ;
#else
V_7 = ( V_73 & 0x3f ) << 0xc ;
#endif
V_7 = ( V_7 & 0xfffffffd ) | ( ( V_82 & 1 ) << 1 ) ;
V_7 = ( V_7 & 0xfffffff3 ) | ( ( V_69 & 3 ) << 2 ) ;
V_7 = ( V_7 & 0xffffffef ) | ( ( V_71 & 1 ) << 4 ) ;
V_7 |= V_67 ;
V_7 = ( V_7 & 0xffffffdf ) | ( ( V_70 & 1 ) << 5 ) ;
#ifdef F_28
V_7 = ( V_7 & 0xfffbffff ) | ( ( V_72 & 1 ) << 0x12 ) ;
#endif
#ifdef F_29
V_7 = ( V_7 & 0xf7ffffff ) | ( ( V_72 & 1 ) << 0x1b ) ;
V_7 = ( V_7 & 0xefffffff ) | ( ( V_72 & 1 ) << 0x1c ) ;
#endif
#ifdef F_30
V_7 = ( V_7 & 0xfeffffff ) | ( ( V_72 & 1 ) << 0x18 ) ;
V_7 = ( V_7 & 0xfdffffff ) | ( ( V_72 & 1 ) << 0x19 ) ;
#endif
}
} else {
if ( V_7 & V_75 ) {
#ifdef F_28
V_7 = ( ( V_72 & 1 ) << 0x12 ) | ( V_7 & 0xfffbffef ) ;
#endif
#ifdef F_29
V_7 =
( ( V_72 & 1 ) << 0x1b ) | ( V_7 & 0xe7ffffef ) | V_77 ;
#endif
#ifdef F_30
V_7 =
( ( V_72 & 1 ) << 0x18 ) | ( V_7 & 0xfcffffef ) | V_77 ;
#endif
} else
F_31 ( V_1 , V_63 , V_76 ) ;
}
F_2 ( V_1 -> V_3 , V_80 + ( V_63 << 2 ) , V_7 ) ;
F_3 ( V_1 -> V_3 , V_80 + ( V_63 << 2 ) ) ;
}
static void F_35 ( T_1 * V_1 )
{
int V_33 ;
T_2 V_14 ;
V_14 = V_66 + ( ( V_83 - 1 ) * 4 ) ;
for ( V_33 = V_83 - 1 ; V_33 >= 0 ; V_33 -- ) {
F_2 ( V_1 -> V_3 , V_14 , ( V_84 | V_67 ) ) ;
if ( F_3 ( V_1 -> V_3 , V_14 ) != ( V_84 | V_67 ) )
F_12 ( V_25 L_10 ) ;
F_25 ( V_1 , V_33 , V_76 ) ;
V_14 -= 4 ;
}
#ifndef F_30
V_14 = V_80 + ( ( V_85 - 1 ) * 4 ) ;
for ( V_33 = V_85 - 1 ; V_33 >= 0 ; V_33 -- ) {
F_2 ( V_1 -> V_3 , V_14 , V_84 ) ;
if ( F_3 ( V_1 -> V_3 , V_14 ) != V_84 )
F_12 ( V_25
L_11 ,
V_14 , F_3 ( V_1 -> V_3 , V_14 ) ) ;
F_31 ( V_1 , V_33 , V_76 ) ;
V_14 -= 4 ;
}
#endif
#ifdef F_28
F_2 ( V_1 -> V_3 , 0xf8c0 , 0xd03 ) ;
#else
#ifdef F_29
F_2 ( V_1 -> V_3 , 0x17000 , 0x61 ) ;
F_2 ( V_1 -> V_3 , 0x17004 , 0x61 ) ;
#endif
F_2 ( V_1 -> V_3 , 0x17008 , 0x61 ) ;
#endif
}
static void F_36 ( T_1 * V_1 )
{
}
static void F_37 ( T_1 * V_1 , int V_86 )
{
T_3 * V_87 = & V_1 -> V_88 [ V_86 ] ;
F_2 ( V_1 -> V_3 , V_89 + ( V_86 << 2 ) ,
V_87 -> V_90 ) ;
}
static void F_38 ( T_1 * V_1 , int V_86 , int V_91 )
{
T_3 * V_87 = & V_1 -> V_88 [ V_86 ] ;
F_2 ( V_1 -> V_3 , V_92 + ( V_86 << 2 ) ,
V_91 << ( ( 0xf - ( V_86 & 0xf ) ) * 2 ) ) ;
V_87 -> V_93 = V_87 -> V_94 = V_91 ;
}
static void
F_39 ( T_1 * V_1 , int V_86 ,
int V_95 , int V_96 )
{
T_3 * V_87 = & V_1 -> V_88 [ V_86 ] ;
V_87 -> V_97 = V_95 ;
V_87 -> V_98 = V_96 ;
V_87 -> V_99 = 0 ;
V_87 -> V_100 = 0 ;
switch ( V_96 ) {
default:
case 4 :
V_87 -> V_100 |= 0x88000000 | 0x44000000 | 0x30000000 | ( V_95 - 1 ) ;
F_2 ( V_1 -> V_3 ,
V_101 + ( V_86 << 4 ) + 0xc ,
F_40 ( V_87 -> V_102 , V_95 * 3 ) ) ;
case 3 :
V_87 -> V_99 |= 0x12000000 ;
V_87 -> V_100 |= 0x80000000 | 0x40000000 | ( ( V_95 - 1 ) << 0xc ) ;
F_2 ( V_1 -> V_3 ,
V_101 + ( V_86 << 4 ) + 0x8 ,
F_40 ( V_87 -> V_102 , V_95 * 2 ) ) ;
case 2 :
V_87 -> V_99 |= 0x88000000 | 0x44000000 | 0x10000000 | ( V_95 - 1 ) ;
F_2 ( V_1 -> V_3 ,
V_101 + ( V_86 << 4 ) + 0x4 ,
F_40 ( V_87 -> V_102 , V_95 ) ) ;
case 1 :
V_87 -> V_99 |= 0x80000000 | 0x40000000 | ( ( V_95 - 1 ) << 0xc ) ;
F_2 ( V_1 -> V_3 ,
V_101 + ( V_86 << 4 ) ,
F_40 ( V_87 -> V_102 , 0 ) ) ;
break;
}
F_2 ( V_1 -> V_3 , V_103 + ( V_86 << 3 ) , V_87 -> V_99 ) ;
F_2 ( V_1 -> V_3 , V_104 + ( V_86 << 3 ) , V_87 -> V_100 ) ;
F_37 ( V_1 , V_86 ) ;
F_38 ( V_1 , V_86 , 0 ) ;
}
static void
F_41 ( T_1 * V_1 , int V_86 , int V_105 , int V_106 ,
int V_107 , int V_68 , T_2 V_108 )
{
T_3 * V_87 = & V_1 -> V_88 [ V_86 ] ;
V_87 -> V_109 = V_68 ;
V_87 -> V_90 =
( ( V_108 & V_110 ) | ( V_87 -> V_90 & ~ V_110 ) ) ;
V_87 -> V_90 =
( V_87 -> V_90 & ~ V_111 ) | ( ( V_105 << V_112 ) & V_111 ) ;
V_87 -> V_90 =
( V_87 -> V_90 & ~ V_113 ) | ( ( V_106 << V_114 ) & V_113 ) ;
V_87 -> V_90 =
( V_87 -> V_90 & ~ V_115 ) | ( ( V_107 << V_116 ) & V_115 ) ;
F_2 ( V_1 -> V_3 , V_89 + ( V_86 << 2 ) ,
V_87 -> V_90 ) ;
F_3 ( V_1 -> V_3 , V_89 + ( V_86 << 2 ) ) ;
}
static int F_42 ( T_1 * V_1 , int V_86 )
{
T_3 * V_87 = & V_1 -> V_88 [ V_86 ] ;
int V_117 , V_118 , V_119 , V_120 , V_38 ;
V_117 =
( F_3 ( V_1 -> V_3 , V_121 + ( V_86 << 2 ) ) &
V_122 ) >> V_123 ;
if ( V_87 -> V_98 >= 4 )
V_120 = ( V_117 - V_87 -> V_93 ) & 3 ;
else {
V_120 = ( V_117 - V_87 -> V_93 ) ;
if ( V_120 < 0 )
V_120 += V_87 -> V_98 ;
}
if ( V_120 == 0 )
return 0 ;
if ( V_87 -> V_98 > 4 ) {
for ( V_38 = 0 ; V_38 < V_120 ; V_38 ++ ) {
V_118 = V_87 -> V_94 + V_38 + 4 ;
if ( V_118 >= V_87 -> V_98 )
V_118 -= V_87 -> V_98 ;
V_119 = V_87 -> V_93 + V_38 ;
if ( V_119 >= 4 )
V_119 -= 4 ;
F_2 ( V_1 -> V_3 ,
V_101 + ( ( ( V_86 << 2 ) + V_119 ) << 2 ) ,
F_40 ( V_87 -> V_102 ,
V_87 -> V_97 * V_118 ) ) ;
F_3 ( V_1 -> V_3 , V_101 +
( ( ( V_86 << 2 ) + V_119 ) << 2 ) ) ;
}
}
V_87 -> V_94 += V_120 ;
V_87 -> V_93 = V_117 ;
if ( V_87 -> V_94 >= V_87 -> V_98 )
V_87 -> V_94 -= V_87 -> V_98 ;
if ( V_120 != 1 )
F_12 ( V_124 L_12 ,
V_86 , V_87 -> V_94 , V_87 -> V_93 , V_120 ) ;
return V_120 ;
}
static void F_43 ( T_1 * V_1 , int V_86 ) {
T_3 * V_87 = & V_1 -> V_88 [ V_86 ] ;
int V_118 , V_119 , V_38 ;
for ( V_38 = 0 ; V_38 < 4 && V_38 < V_87 -> V_98 ; V_38 ++ ) {
V_118 = V_87 -> V_94 + V_38 ;
if ( V_118 >= V_87 -> V_98 )
V_118 -= V_87 -> V_98 ;
V_119 = V_87 -> V_93 + V_38 ;
if ( V_87 -> V_98 < 4 ) {
if ( V_119 >= V_87 -> V_98 )
V_119 -= V_87 -> V_98 ;
}
else {
if ( V_119 >= 4 )
V_119 -= 4 ;
}
F_2 ( V_1 -> V_3 ,
V_101 + ( ( ( V_86 << 2 ) + V_119 ) << 2 ) ,
F_40 ( V_87 -> V_102 ,
V_87 -> V_97 * V_118 ) ) ;
F_3 ( V_1 -> V_3 , V_101 + ( ( ( V_86 << 2 ) + V_119 ) << 2 ) ) ;
}
}
static inline int F_44 ( T_1 * V_1 , int V_86 )
{
T_3 * V_87 = & V_1 -> V_88 [ V_86 ] ;
int V_7 , V_117 , V_120 ;
V_7 = F_3 ( V_1 -> V_3 , V_121 + ( V_86 << 2 ) ) ;
V_117 = ( V_7 & V_122 ) >> V_123 ;
if ( V_87 -> V_98 >= 4 )
V_120 = ( V_117 - V_87 -> V_93 ) & 3 ;
else {
V_120 = ( V_117 - V_87 -> V_93 ) ;
if ( V_120 < 0 )
V_120 += V_87 -> V_98 ;
}
return ( V_87 -> V_94 + V_120 ) * V_87 -> V_97
+ ( V_7 & ( V_87 -> V_97 - 1 ) ) ;
}
static void F_45 ( T_1 * V_1 , int V_86 )
{
int V_125 = 0 , V_73 = 0 ;
T_3 * V_87 = & V_1 -> V_88 [ V_86 ] ;
switch ( V_87 -> V_126 ) {
case V_127 :
F_26 ( V_1 , V_86 ,
V_87 -> V_128 ? 1 : 0 ) ;
break;
case V_129 :
V_125 = 1 ;
F_2 ( V_1 -> V_3 , V_89 + ( V_86 << 2 ) ,
V_87 -> V_90 ) ;
F_27 ( V_1 , V_86 , V_87 -> V_109 ,
V_73 , V_125 ,
V_87 -> V_128 ? 1 : 0 , 0 ) ;
break;
case V_130 :
F_27 ( V_1 , V_86 , V_87 -> V_109 ,
V_73 , V_125 ,
V_87 -> V_128 ? 1 : 0 , 0 ) ;
break;
}
V_87 -> V_126 = V_127 ;
}
static void F_46 ( T_1 * V_1 , int V_86 )
{
T_3 * V_87 = & V_1 -> V_88 [ V_86 ] ;
int V_125 = 1 , V_73 = 0 ;
switch ( V_87 -> V_126 ) {
case V_129 :
F_2 ( V_1 -> V_3 , V_89 + ( V_86 << 2 ) ,
V_87 -> V_90 ) ;
F_27 ( V_1 , V_86 , V_87 -> V_109 ,
V_73 , V_125 ,
V_87 -> V_128 ? 1 : 0 , 0 ) ;
break;
case V_130 :
F_27 ( V_1 , V_86 , V_87 -> V_109 ,
V_73 , V_125 ,
V_87 -> V_128 ? 1 : 0 , 0 ) ;
break;
}
V_87 -> V_126 = V_127 ;
}
static void F_47 ( T_1 * V_1 , int V_86 )
{
T_3 * V_87 = & V_1 -> V_88 [ V_86 ] ;
int V_125 = 0 , V_73 = 0 ;
switch ( V_87 -> V_126 ) {
case V_127 :
F_27 ( V_1 , V_86 , V_87 -> V_109 ,
V_73 , V_125 , 0 , 0 ) ;
break;
case V_129 :
F_2 ( V_1 -> V_3 , V_89 + ( V_86 << 2 ) ,
V_87 -> V_90 ) ;
F_27 ( V_1 , V_86 , V_87 -> V_109 ,
V_73 , V_125 , 0 , 0 ) ;
break;
}
V_87 -> V_126 = V_130 ;
}
static void F_48 ( T_1 * V_1 , int V_86 )
{
T_3 * V_87 = & V_1 -> V_88 [ V_86 ] ;
int V_73 = 0 , V_125 = 0 ;
if ( V_87 -> V_126 == V_127 )
F_27 ( V_1 , V_86 , V_87 -> V_109 ,
V_73 , V_125 , 0 , 0 ) ;
else if ( V_87 -> V_126 == V_129 )
return;
V_87 -> V_126 = V_129 ;
V_87 -> V_128 = 0 ;
}
static void F_49 ( T_1 * V_1 , int V_131 )
{
T_3 * V_87 = & V_1 -> V_132 [ V_131 ] ;
F_2 ( V_1 -> V_3 , V_133 + ( V_131 << 2 ) , V_87 -> V_90 ) ;
}
static void F_50 ( T_1 * V_1 , int V_131 , int V_91 )
{
T_3 * V_87 = & V_1 -> V_132 [ V_131 ] ;
F_2 ( V_1 -> V_3 , V_134 + ( V_131 << 2 ) ,
V_91 << ( ( 0xf - ( V_131 & 0xf ) ) * 2 ) ) ;
V_87 -> V_93 = V_87 -> V_94 = V_91 ;
}
static void
F_51 ( T_1 * V_1 , int V_131 ,
int V_95 , int V_96 )
{
T_3 * V_87 = & V_1 -> V_132 [ V_131 ] ;
V_87 -> V_97 = V_95 ;
V_87 -> V_98 = V_96 ;
V_87 -> V_99 = 0 ;
V_87 -> V_100 = 0 ;
switch ( V_96 ) {
default:
case 4 :
V_87 -> V_100 |= 0x88000000 | 0x44000000 | 0x30000000 | ( V_95 - 1 ) ;
F_2 ( V_1 -> V_3 , V_135 + ( V_131 << 4 ) + 0xc ,
F_40 ( V_87 -> V_102 , V_95 * 3 ) ) ;
case 3 :
V_87 -> V_99 |= 0x12000000 ;
V_87 -> V_100 |= 0x80000000 | 0x40000000 | ( ( V_95 - 1 ) << 0xc ) ;
F_2 ( V_1 -> V_3 , V_135 + ( V_131 << 4 ) + 0x8 ,
F_40 ( V_87 -> V_102 , V_95 * 2 ) ) ;
case 2 :
V_87 -> V_99 |= 0x88000000 | 0x44000000 | 0x10000000 | ( V_95 - 1 ) ;
F_2 ( V_1 -> V_3 , V_135 + ( V_131 << 4 ) + 0x4 ,
F_40 ( V_87 -> V_102 , V_95 ) ) ;
case 1 :
V_87 -> V_99 |= 0x80000000 | 0x40000000 | ( ( V_95 - 1 ) << 0xc ) ;
F_2 ( V_1 -> V_3 , V_135 + ( V_131 << 4 ) ,
F_40 ( V_87 -> V_102 , 0 ) ) ;
break;
}
F_2 ( V_1 -> V_3 , V_136 + ( V_131 << 3 ) , V_87 -> V_99 ) ;
F_2 ( V_1 -> V_3 , V_137 + ( V_131 << 3 ) , V_87 -> V_100 ) ;
F_49 ( V_1 , V_131 ) ;
F_50 ( V_1 , V_131 , 0 ) ;
}
static void
F_52 ( T_1 * V_1 , int V_131 , int V_105 , int V_107 , int V_51 ,
T_2 V_108 )
{
T_3 * V_87 = & V_1 -> V_132 [ V_131 ] ;
V_87 -> V_109 = V_51 ;
V_87 -> V_90 = 0 ;
V_87 -> V_90 =
( ( V_108 & V_110 ) | ( V_87 -> V_90 & ~ V_110 ) ) ;
V_87 -> V_90 =
( V_87 -> V_90 & ~ V_111 ) | ( ( V_105 << V_112 ) & V_111 ) ;
V_87 -> V_90 |= ( 1 << V_114 ) ;
V_87 -> V_90 =
( V_87 -> V_90 & V_115 ) | ( ( V_107 << V_116 ) & V_115 ) ;
F_2 ( V_1 -> V_3 , V_133 + ( V_131 << 2 ) , V_87 -> V_90 ) ;
}
static int F_53 ( T_1 * V_1 , int V_131 )
{
T_3 * V_87 = & V_1 -> V_132 [ V_131 ] ;
int V_117 , V_118 , V_119 , V_120 , V_38 ;
V_117 =
( F_3 ( V_1 -> V_3 , V_138 + ( V_131 << 2 ) ) &
V_139 )
>> V_140 ;
if ( V_87 -> V_98 >= 4 )
V_120 = ( V_117 - V_87 -> V_93 ) & 3 ;
else {
V_120 = ( V_117 - V_87 -> V_93 ) ;
if ( V_120 < 0 )
V_120 += V_87 -> V_98 ;
}
if ( V_120 == 0 )
return 0 ;
if ( V_87 -> V_98 > 4 ) {
for ( V_38 = 0 ; V_38 < V_120 ; V_38 ++ ) {
V_118 = V_87 -> V_94 + V_38 + 4 ;
if ( V_118 >= V_87 -> V_98 )
V_118 -= V_87 -> V_98 ;
V_119 = V_87 -> V_93 + V_38 ;
if ( V_119 >= 4 )
V_119 -= 4 ;
F_2 ( V_1 -> V_3 ,
V_135 +
( ( ( V_131 << 2 ) + V_119 ) << 2 ) ,
F_40 ( V_87 -> V_102 ,
V_87 -> V_97 * V_118 ) ) ;
F_3 ( V_1 -> V_3 , V_135 +
( ( ( V_131 << 2 ) + V_119 ) << 2 ) ) ;
}
}
V_87 -> V_94 += V_120 ;
if ( V_87 -> V_94 >= V_87 -> V_98 )
V_87 -> V_94 -= V_87 -> V_98 ;
V_87 -> V_93 = V_117 ;
if ( V_120 != 1 )
F_12 ( V_141 L_13 ,
V_87 -> V_94 , V_120 ) ;
return V_120 ;
}
static inline int F_54 ( T_1 * V_1 , int V_131 )
{
T_3 * V_87 = & V_1 -> V_132 [ V_131 ] ;
int V_7 ;
V_7 = F_3 ( V_1 -> V_3 , V_138 + ( V_131 << 2 ) ) ;
V_7 = ( V_87 -> V_94 * V_87 -> V_97 ) + ( V_7 & ( V_87 -> V_97 - 1 ) ) ;
return V_7 ;
}
static void F_55 ( T_1 * V_1 , int V_131 )
{
T_3 * V_87 = & V_1 -> V_132 [ V_131 ] ;
int V_125 = 0 , V_73 = 0 ;
switch ( V_87 -> V_126 ) {
case V_127 :
F_33 ( V_1 , V_131 ,
V_87 -> V_128 ? 1 : 0 ) ;
break;
case V_129 :
V_125 = 1 ;
F_2 ( V_1 -> V_3 , V_133 + ( V_131 << 2 ) ,
V_87 -> V_90 ) ;
F_34 ( V_1 , V_131 , V_87 -> V_109 ,
V_73 , V_125 ,
V_87 -> V_128 ? 1 : 0 , 0 ) ;
break;
case V_130 :
F_34 ( V_1 , V_131 , V_87 -> V_109 ,
V_73 , V_125 ,
V_87 -> V_128 ? 1 : 0 , 0 ) ;
break;
}
V_87 -> V_126 = V_127 ;
}
static void F_56 ( T_1 * V_1 , int V_131 )
{
T_3 * V_87 = & V_1 -> V_132 [ V_131 ] ;
int V_125 = 0 , V_73 = 0 ;
switch ( V_87 -> V_126 ) {
case V_129 :
F_2 ( V_1 -> V_3 , V_133 + ( V_131 << 2 ) ,
V_87 -> V_90 ) ;
F_34 ( V_1 , V_131 , V_87 -> V_109 ,
V_73 , V_125 ,
V_87 -> V_128 ? 1 : 0 , 0 ) ;
break;
case V_130 :
F_34 ( V_1 , V_131 , V_87 -> V_109 ,
V_73 , V_125 ,
V_87 -> V_128 ? 1 : 0 , 0 ) ;
break;
}
V_87 -> V_126 = V_127 ;
}
static void F_57 ( T_1 * V_1 , int V_131 )
{
T_3 * V_87 = & V_1 -> V_132 [ V_131 ] ;
int V_125 = 0 , V_73 = 0 ;
switch ( V_87 -> V_126 ) {
case V_127 :
F_34 ( V_1 , V_131 , V_87 -> V_109 ,
V_73 , V_125 , 0 , 0 ) ;
break;
case V_129 :
F_2 ( V_1 -> V_3 , V_133 + ( V_131 << 2 ) ,
V_87 -> V_90 ) ;
F_34 ( V_1 , V_131 , V_87 -> V_109 ,
V_73 , V_125 , 0 , 0 ) ;
break;
}
V_87 -> V_126 = V_130 ;
}
static void F_58 ( T_1 * V_1 , int V_131 )
{
T_3 * V_87 = & V_1 -> V_132 [ V_131 ] ;
int V_73 = 0 , V_125 = 0 ;
if ( V_87 -> V_126 == V_127 )
F_34 ( V_1 , V_131 , V_87 -> V_109 ,
V_73 , V_125 , 0 , 0 ) ;
else if ( V_87 -> V_126 == V_129 )
return;
V_87 -> V_126 = V_129 ;
V_87 -> V_128 = 0 ;
}
static void F_59 ( T_1 * V_1 )
{
int V_38 ;
F_2 ( V_1 -> V_3 , V_142 , 0 ) ;
for ( V_38 = 0 ; V_38 < V_143 ; V_38 ++ )
F_2 ( V_1 -> V_3 , V_144 + ( V_38 << 2 ) ,
F_3 ( V_1 -> V_3 ,
V_144 + ( V_38 << 2 ) ) | V_145 ) ;
for ( V_38 = 0 ; V_38 < V_146 ; V_38 ++ ) {
F_2 ( V_1 -> V_3 , V_147 + ( V_38 << 2 ) ,
F_3 ( V_1 -> V_3 ,
V_147 + ( V_38 << 2 ) ) | V_145 ) ;
}
}
static void F_60 ( T_1 * V_1 , int V_2 )
{
F_2 ( V_1 -> V_3 , V_142 ,
F_3 ( V_1 -> V_3 , V_142 ) | ( 0x1 << V_2 ) ) ;
}
static void F_61 ( T_1 * V_1 , int V_2 )
{
F_2 ( V_1 -> V_3 , V_142 ,
F_3 ( V_1 -> V_3 , V_142 ) & ~ ( 0x1 << V_2 ) ) ;
}
static void
F_62 ( T_1 * V_1 , unsigned char V_2 ,
T_4 * V_148 , int V_149 )
{
int V_7 , V_22 , V_21 = 0 ;
if ( ( V_149 <= 0 ) || ( V_148 == NULL ) )
return;
V_149 -- ;
F_2 ( V_1 -> V_3 ,
V_144 + ( ( V_148 [ V_149 ] & V_150 ) << 2 ) ,
V_145 ) ;
while ( V_149 > 0 ) {
F_2 ( V_1 -> V_3 ,
V_144 +
( ( V_148 [ V_149 - 1 ] & V_150 ) << 2 ) , V_148 [ V_149 ] ) ;
V_149 -- ;
}
V_7 =
F_3 ( V_1 -> V_3 ,
V_147 + ( V_2 << 2 ) ) & V_150 ;
if ( V_7 == V_150 ) {
F_2 ( V_1 -> V_3 , V_147 + ( V_2 << 2 ) ,
V_148 [ 0 ] ) ;
F_60 ( V_1 , V_2 ) ;
return;
}
do {
V_22 = V_7 ;
V_7 =
F_3 ( V_1 -> V_3 ,
V_144 + ( V_7 << 2 ) ) & V_150 ;
if ( ( V_21 ++ ) > V_150 ) {
F_12 ( V_25
L_14 ,
* V_148 ) ;
return;
}
}
while ( V_7 != V_150 );
F_2 ( V_1 -> V_3 , V_144 + ( V_22 << 2 ) , V_148 [ 0 ] ) ;
}
static void
F_63 ( T_1 * V_1 , unsigned char V_2 ,
T_4 V_151 , T_4 V_152 )
{
int V_7 , V_21 = 0 , V_22 ;
V_7 =
F_3 ( V_1 -> V_3 ,
V_147 + ( V_2 << 2 ) ) & V_150 ;
if ( V_7 == ( V_151 & V_150 ) ) {
V_7 =
F_3 ( V_1 -> V_3 ,
V_144 + ( ( V_152 & V_150 ) << 2 ) ) ;
if ( ( V_7 & V_150 ) == V_150 )
F_61 ( V_1 , V_2 ) ;
F_2 ( V_1 -> V_3 , V_147 + ( V_2 << 2 ) ,
V_7 ) ;
return;
}
do {
V_22 = V_7 ;
V_7 =
F_3 ( V_1 -> V_3 ,
V_144 + ( V_22 << 2 ) ) & V_150 ;
if ( ( ( V_21 ++ ) > V_150 ) || ( V_7 == V_150 ) ) {
F_12 ( V_25
L_15 ,
V_151 ) ;
return;
}
}
while ( V_7 != ( V_151 & V_150 ) );
V_7 = F_3 ( V_1 -> V_3 , V_144 + ( V_7 << 2 ) ) ;
if ( ( V_7 & V_150 ) == V_152 )
V_7 = F_3 ( V_1 -> V_3 , V_144 + ( V_7 << 2 ) ) ;
F_2 ( V_1 -> V_3 , V_144 + ( V_22 << 2 ) , V_7 ) ;
}
static void
F_64 ( T_1 * V_1 , int V_13 , unsigned char V_2 ,
unsigned char V_153 , unsigned char V_154 )
{
T_4 V_148 ;
V_148 = ( ( V_153 & V_150 ) << V_155 ) | ( V_154 & V_150 ) ;
if ( V_13 ) {
F_62 ( V_1 , V_2 , & V_148 , 1 ) ;
if ( ( V_153 < ( V_156 + V_157 ) )
&& ( V_153 >= V_156 ) )
F_23 ( V_1 , ( V_153 - V_156 ) ,
V_2 ) ;
else if ( ( V_153 < ( V_158 + V_34 ) )
&& ( V_153 >= V_158 ) )
F_11 ( V_1 ,
( V_153 - V_158 ) , V_2 ) ;
} else {
F_63 ( V_1 , V_2 , V_148 , V_148 ) ;
if ( ( V_153 < ( V_156 + V_157 ) )
&& ( V_153 >= V_156 ) )
F_24 ( V_1 , ( V_153 - V_156 ) ,
V_2 ) ;
else if ( ( V_153 < ( V_158 + V_34 ) )
&& ( V_153 >= V_158 ) )
F_13 ( V_1 ,
( V_153 - V_158 ) , V_2 ) ;
}
}
static void
F_65 ( T_1 * V_1 , int V_13 , unsigned char V_20 ,
unsigned char V_159 , unsigned char V_160 ,
unsigned char V_154 )
{
T_4 V_148 [ 2 ] ;
V_148 [ 0 ] = ( ( V_159 & V_150 ) << V_155 ) | ( V_154 & V_150 ) ;
V_148 [ 1 ] = ( ( V_160 & V_150 ) << V_155 ) | ( V_154 & V_150 ) ;
if ( V_154 < 0x10 )
V_148 [ 1 ] = ( V_148 [ 1 ] & ~ V_150 ) | ( V_154 + 0x20 ) ;
if ( V_13 ) {
F_62 ( V_1 , V_20 , V_148 , 2 ) ;
if ( ( V_159 < ( V_156 + V_157 ) )
&& ( V_159 >= V_156 ) ) {
F_23 ( V_1 ,
( V_159 - V_156 ) , V_20 ) ;
F_23 ( V_1 ,
( V_160 - V_156 ) , V_20 ) ;
} else if ( ( V_159 < ( V_158 + V_34 ) )
&& ( V_159 >= V_158 ) ) {
F_11 ( V_1 ,
( V_159 - V_158 ) , V_20 ) ;
F_11 ( V_1 ,
( V_160 - V_158 ) , V_20 ) ;
}
} else {
F_63 ( V_1 , V_20 , V_148 [ 0 ] , V_148 [ 1 ] ) ;
if ( ( V_159 < ( V_156 + V_157 ) )
&& ( V_159 >= V_156 ) ) {
F_24 ( V_1 ,
( V_159 - V_156 ) , V_20 ) ;
F_24 ( V_1 ,
( V_160 - V_156 ) , V_20 ) ;
} else if ( ( V_159 < ( V_158 + V_34 ) )
&& ( V_159 >= V_158 ) ) {
F_13 ( V_1 ,
( V_159 - V_158 ) , V_20 ) ;
F_13 ( V_1 ,
( V_160 - V_158 ) , V_20 ) ;
}
}
}
static void
F_66 ( T_1 * V_1 , int V_13 , unsigned char V_20 ,
unsigned char V_86 , unsigned char V_37 )
{
F_64 ( V_1 , V_13 , V_20 , F_67 ( V_86 ) , F_68 ( V_37 ) ) ;
}
static void
F_69 ( T_1 * V_1 , int V_13 ,
unsigned char V_2 , unsigned char V_37 ,
unsigned char V_10 )
{
F_64 ( V_1 , V_13 , V_2 , F_70 ( V_37 ) , F_71 ( V_10 ) ) ;
}
static void
F_72 ( T_1 * V_1 , int V_13 , unsigned char V_10 ,
unsigned char V_5 , int V_161 )
{
if ( V_13 ) {
F_9 ( V_1 , V_5 , V_10 ) ;
F_6 ( V_1 , V_5 , V_10 , V_162 ) ;
} else
F_10 ( V_1 , V_5 , V_10 , V_161 ) ;
}
static void
F_73 ( T_1 * V_1 , int V_13 ,
unsigned char V_2 , unsigned char V_153 ,
unsigned char V_10 )
{
F_64 ( V_1 , V_13 , V_2 , V_153 , F_71 ( V_10 ) ) ;
}
static void
F_74 ( T_1 * V_1 , int V_13 , unsigned char V_20 ,
unsigned char V_37 , unsigned char V_86 )
{
F_64 ( V_1 , V_13 , V_20 , F_70 ( V_37 ) , F_67 ( V_86 ) ) ;
}
static void
F_75 ( T_1 * V_1 , int V_13 ,
unsigned char V_20 , unsigned char V_163 ,
unsigned char V_164 , unsigned char V_86 )
{
F_65 ( V_1 , V_13 , V_20 , F_70 ( V_163 ) , F_70 ( V_164 ) ,
F_67 ( V_86 ) ) ;
}
static void
F_76 ( T_1 * V_1 , int V_13 , unsigned char V_20 ,
unsigned char V_5 , unsigned char V_154 )
{
F_64 ( V_1 , V_13 , V_20 , F_77 ( V_5 ) , V_154 ) ;
F_5 ( V_1 , V_5 , V_165 ) ;
}
static void
F_78 ( T_1 * V_1 , int V_13 , unsigned char V_20 ,
unsigned char V_5 , unsigned char V_37 )
{
F_64 ( V_1 , V_13 , V_20 , F_77 ( V_5 ) , F_68 ( V_37 ) ) ;
F_5 ( V_1 , V_5 , V_165 ) ;
}
static void
F_79 ( T_1 * V_1 , int V_13 , unsigned char V_166 [] )
{
#ifdef F_28
F_76 ( V_1 , V_13 , 0x11 , V_166 [ 0 ] , F_80 ( 0 ) ) ;
F_76 ( V_1 , V_13 , 0x11 , V_166 [ 1 ] , F_80 ( 1 ) ) ;
#else
#if 1
F_76 ( V_1 , V_13 , 0x11 , V_166 [ 0 ] , F_81 ( 0 ) ) ;
F_76 ( V_1 , V_13 , 0x11 , V_166 [ 1 ] , F_81 ( 1 ) ) ;
F_5 ( V_1 , V_166 [ 0 ] , 0 ) ;
F_5 ( V_1 , V_166 [ 1 ] , 0 ) ;
F_64 ( V_1 , V_13 , 0x11 , F_82 ( 0 ) , F_80 ( 0 ) ) ;
F_64 ( V_1 , V_13 , 0x11 , F_82 ( 1 ) , F_80 ( 1 ) ) ;
if ( F_83 ( V_1 ) ) {
F_76 ( V_1 , V_13 , 0x11 , V_166 [ 2 ] ,
F_80 ( 0 + 4 ) ) ;
F_76 ( V_1 , V_13 , 0x11 , V_166 [ 3 ] ,
F_80 ( 1 + 4 ) ) ;
}
#else
F_76 ( V_1 , V_13 , 0x11 , V_166 [ 0 ] , F_80 ( 0 ) ) ;
F_76 ( V_1 , V_13 , 0x11 , V_166 [ 1 ] , F_80 ( 1 ) ) ;
#endif
#endif
}
static void
F_84 ( T_1 * V_1 , int V_13 , unsigned char V_167 ,
unsigned char V_168 )
{
F_73 ( V_1 , V_13 , 0x11 , F_85 ( 0 ) , V_167 ) ;
F_73 ( V_1 , V_13 , 0x11 , F_85 ( 1 ) , V_168 ) ;
}
static char
F_86 ( T_1 * V_1 , int V_169 [] , int V_170 , int V_171 )
{
int V_38 , V_172 = V_173 [ V_171 ] , V_174 = 0 ;
if ( V_170 ) {
for ( V_38 = 0 ; V_38 < V_83 ; V_38 ++ ) {
V_174 |= V_1 -> V_88 [ V_38 ] . V_175 [ V_171 ] ;
}
V_174 |= V_1 -> V_176 [ V_171 ] ;
for ( V_38 = 0 ; V_38 < V_172 ; V_38 ++ ) {
if ( ( V_174 & ( 1 << V_38 ) ) == 0 ) {
if ( V_169 != NULL )
V_169 [ V_171 ] |= ( 1 << V_38 ) ;
else
V_1 -> V_88 [ V_38 ] . V_175 [ V_171 ] |= ( 1 << V_38 ) ;
return V_38 ;
}
}
} else {
if ( V_169 == NULL )
return - V_177 ;
for ( V_38 = 0 ; V_38 < V_172 ; V_38 ++ ) {
if ( V_169 [ V_171 ] & ( 1 << V_38 ) ) {
V_169 [ V_171 ] &= ~ ( 1 << V_38 ) ;
return V_38 ;
}
}
}
F_12 ( V_25 L_16 , V_171 ) ;
return - V_178 ;
}
static void F_87 ( T_1 * V_1 , int V_13 )
{
V_1 -> V_179 [ 0 ] = F_86 ( V_1 , V_1 -> V_176 , V_13 ,
V_180 ) ;
V_1 -> V_179 [ 1 ] = F_86 ( V_1 , V_1 -> V_176 , V_13 ,
V_180 ) ;
if ( F_83 ( V_1 ) ) {
V_1 -> V_179 [ 2 ] = F_86 ( V_1 , V_1 -> V_176 , V_13 ,
V_180 ) ;
V_1 -> V_179 [ 3 ] = F_86 ( V_1 , V_1 -> V_176 , V_13 ,
V_180 ) ;
}
F_79 ( V_1 , V_13 , V_1 -> V_179 ) ;
V_1 -> V_181 [ 0 ] = F_86 ( V_1 , V_1 -> V_176 , V_13 ,
V_182 ) ;
V_1 -> V_181 [ 1 ] = F_86 ( V_1 , V_1 -> V_176 , V_13 ,
V_182 ) ;
F_84 ( V_1 , V_13 , F_88 ( 0 ) , F_88 ( 1 ) ) ;
#ifndef F_28
V_1 -> V_183 [ 0 ] = F_86 ( V_1 , V_1 -> V_176 , V_13 ,
V_180 ) ;
V_1 -> V_183 [ 1 ] = F_86 ( V_1 , V_1 -> V_176 , V_13 ,
V_180 ) ;
F_76 ( V_1 , V_13 , 0x14 , V_1 -> V_183 [ 0 ] ,
F_89 ( 0 ) ) ;
F_76 ( V_1 , V_13 , 0x14 , V_1 -> V_183 [ 1 ] ,
F_89 ( 1 ) ) ;
#endif
#ifndef F_30
F_90 ( V_1 , V_13 ) ;
#endif
#ifndef F_28
F_91 ( V_1 , V_13 ) ;
#endif
}
static int
F_92 ( T_1 * V_1 , int V_87 , int V_184 , int V_106 , int type )
{
T_3 * V_185 ;
int V_38 , V_13 ;
if ( ( V_184 == 3 )
|| ( ( V_106 == V_186 ) && ( V_184 > 2 ) ) )
return - V_187 ;
if ( V_87 >= 0 ) {
V_13 = 0 ;
F_86 ( V_1 ,
V_1 -> V_88 [ V_87 ] . V_175 , V_13 ,
V_188 ) ;
} else {
V_13 = 1 ;
if ( ( V_87 =
F_86 ( V_1 , NULL , V_13 ,
V_188 ) ) < 0 )
return - V_187 ;
}
V_185 = & V_1 -> V_88 [ V_87 ] ;
V_185 -> V_87 = V_87 ;
V_185 -> V_106 = V_106 ;
V_185 -> type = type ;
if ( V_106 == V_189 ) {
int V_37 [ 4 ] , V_5 [ 4 ] , V_190 ;
#ifndef F_28
int V_191 = 0 ;
#endif
if ( V_185 -> type != V_192 ) {
for ( V_38 = 0 ; V_38 < V_184 ; V_38 ++ ) {
if ( ( V_37 [ V_38 ] = F_86 ( V_1 ,
V_185 -> V_175 , V_13 ,
V_193 ) ) < 0 ) {
memset ( V_185 -> V_175 , 0 ,
sizeof( unsigned char ) *
V_194 ) ;
return - V_187 ;
}
if ( V_185 -> type != V_195 ) {
if ( ( V_5 [ V_38 ] = F_86 ( V_1 ,
V_185 -> V_175 ,
V_13 ,
V_182 ) ) < 0 ) {
memset ( V_185 -> V_175 ,
0 ,
sizeof( unsigned char ) * V_194 ) ;
return - V_187 ;
}
}
}
}
#ifndef F_28
if ( V_185 -> type == V_195 ) {
if ( ( V_191 =
F_86 ( V_1 ,
V_185 -> V_175 , V_13 ,
V_196 ) ) < 0 ) {
memset ( V_185 -> V_175 , 0 ,
sizeof( unsigned char ) *
V_194 ) ;
F_12 ( V_25 L_17 ) ;
return - V_187 ;
}
F_93 ( & ( V_1 -> V_191 [ V_191 ] ) , V_13 ) ;
}
if ( ( V_185 -> type == V_192 ) && ( V_13 ) ) {
F_64 ( V_1 , 0 , 0x14 ,
F_77 ( V_1 -> V_183 [ 0 ] ) ,
F_89 ( 0 ) ) ;
F_64 ( V_1 , 0 , 0x14 ,
F_77 ( V_1 -> V_183 [ 1 ] ) ,
F_89 ( 1 ) ) ;
}
#endif
for ( V_38 = 0 ; V_38 < V_184 ; V_38 ++ ) {
if ( V_185 -> type == V_197 ) {
F_66 ( V_1 , V_13 ,
V_37 [ V_184 - 1 ] ,
V_87 ,
V_37 [ V_38 ] ) ;
F_69 ( V_1 , V_13 ,
0x11 , V_37 [ V_38 ] ,
V_5 [ V_38 ] ) ;
F_72 ( V_1 , V_13 ,
V_5 [ V_38 ] ,
F_94 ( V_38 ) , 0 ) ;
#ifndef F_28
F_72 ( V_1 , V_13 ,
V_5 [ V_38 ] ,
F_95 ( V_38 % 2 ) , 0 ) ;
F_6 ( V_1 ,
F_95 ( V_38 % 2 ) ,
V_5 [ V_38 ] ,
V_162 ) ;
#endif
}
#ifndef F_28
if ( V_185 -> type == V_195 ) {
F_66 ( V_1 , V_13 ,
V_37 [ V_184 - 1 ] ,
V_87 ,
V_37 [ V_38 ] ) ;
F_64 ( V_1 , V_13 , 0x11 , F_70 ( V_37 [ V_38 ] ) , F_96 ( V_191 ) ) ;
}
if ( V_185 -> type == V_192 )
F_64 ( V_1 , V_13 , 0x14 ,
F_67 ( V_185 -> V_87 ) ,
F_89 ( V_38 ) ) ;
#endif
}
if ( V_185 -> type != V_192 && V_185 -> type != V_195 ) {
V_190 = ( F_83 ( V_1 ) ? 4 : 2 ) ;
for ( V_38 = V_184 ; V_38 < V_190 ; V_38 ++ ) {
F_72 ( V_1 , V_13 ,
V_5 [ V_38 % V_184 ] ,
F_94 ( V_38 ) , 0 ) ;
#ifndef F_28
F_72 ( V_1 , V_13 ,
V_5 [ V_38 % V_184 ] ,
F_95 ( V_38 % 2 ) ,
0 ) ;
F_6 ( V_1 ,
F_95 ( V_38 % 2 ) ,
V_5 [ V_38 % V_184 ] ,
V_162 ) ;
#endif
}
}
#ifndef F_28
else {
if ( V_184 == 1 && V_185 -> type == V_192 )
F_64 ( V_1 , V_13 , 0x14 ,
F_67 ( V_185 -> V_87 ) ,
F_89 ( 1 ) ) ;
}
if ( ( V_185 -> type == V_192 ) && ( ! V_13 ) ) {
F_64 ( V_1 , 1 , 0x14 ,
F_77 ( V_1 -> V_183 [ 0 ] ) ,
F_89 ( 0 ) ) ;
F_64 ( V_1 , 1 , 0x14 ,
F_77 ( V_1 -> V_183 [ 1 ] ) ,
F_89 ( 1 ) ) ;
}
#endif
} else {
int V_37 [ 2 ] , V_5 [ 2 ] ;
for ( V_38 = 0 ; V_38 < V_184 ; V_38 ++ ) {
if ( ( V_5 [ V_38 ] =
F_86 ( V_1 ,
V_185 -> V_175 , V_13 ,
V_180 ) )
< 0 ) {
memset ( V_185 -> V_175 , 0 ,
sizeof( unsigned char ) *
V_194 ) ;
return - V_187 ;
}
if ( ( V_37 [ V_38 ] =
F_86 ( V_1 ,
V_185 -> V_175 , V_13 ,
V_193 ) ) < 0 ) {
memset ( V_185 -> V_175 , 0 ,
sizeof( unsigned char ) *
V_194 ) ;
return - V_187 ;
}
}
F_72 ( V_1 , V_13 , F_88 ( 0 ) , V_5 [ 0 ] , 0 ) ;
F_78 ( V_1 , V_13 , 0x11 , V_5 [ 0 ] , V_37 [ 0 ] ) ;
if ( V_184 == 1 ) {
F_72 ( V_1 , V_13 ,
F_88 ( 1 ) , V_5 [ 0 ] , 0 ) ;
F_74 ( V_1 , V_13 ,
V_37 [ 0 ] ,
V_37 [ 0 ] , V_87 ) ;
} else {
F_72 ( V_1 , V_13 ,
F_88 ( 1 ) , V_5 [ 1 ] , 0 ) ;
F_78 ( V_1 , V_13 , 0x11 , V_5 [ 1 ] ,
V_37 [ 1 ] ) ;
F_75 ( V_1 , V_13 ,
V_37 [ 1 ] , V_37 [ 0 ] ,
V_37 [ 1 ] , V_87 ) ;
}
}
V_1 -> V_88 [ V_87 ] . V_184 = V_184 ;
#if 0
if (nr_ch < 4) {
snd_ac97_write_cache(vortex->codec,
AC97_SIGMATEL_DAC2INVERT,
snd_ac97_read(vortex->codec,
AC97_SIGMATEL_DAC2INVERT)
| 4);
} else {
snd_ac97_write_cache(vortex->codec,
AC97_SIGMATEL_DAC2INVERT,
snd_ac97_read(vortex->codec,
AC97_SIGMATEL_DAC2INVERT)
& ~((u32)
4));
}
#endif
return V_87 ;
}
static void
F_97 ( T_1 * V_1 , int V_86 , unsigned int V_198 , int V_106 )
{
T_3 * V_185 = & ( V_1 -> V_88 [ V_86 ] ) ;
int V_38 , V_199 ;
if ( V_106 )
V_199 = F_98 ( V_198 , 48000 ) ;
else
V_199 = F_98 ( 48000 , V_198 ) ;
for ( V_38 = 0 ; V_38 < V_157 ; V_38 ++ ) {
if ( V_185 -> V_175 [ V_193 ] & ( 1 << V_38 ) )
F_21 ( V_1 , V_38 , V_199 , 0 , 0 , V_38 , V_106 , 1 , V_199 , V_106 ) ;
}
}
static void F_99 ( T_1 * V_1 , int V_200 )
{
F_2 ( V_1 -> V_3 , V_201 , V_200 ) ;
}
static void F_100 ( T_1 * V_47 )
{
F_2 ( V_47 -> V_3 , V_202 ,
F_3 ( V_47 -> V_3 , V_202 ) | V_203 ) ;
F_2 ( V_47 -> V_3 , V_204 ,
( F_3 ( V_47 -> V_3 , V_204 ) & 0xffffefc0 ) | 0x24 ) ;
}
static void F_101 ( T_1 * V_47 )
{
F_2 ( V_47 -> V_3 , V_202 ,
F_3 ( V_47 -> V_3 , V_202 ) & ~ V_203 ) ;
}
static T_5 F_102 ( int V_205 , void * V_206 )
{
T_1 * V_1 = V_206 ;
int V_38 , V_207 ;
T_2 V_153 ;
if ( ! ( F_3 ( V_1 -> V_3 , V_208 ) & 0x1 ) )
return V_209 ;
if ( ! ( F_3 ( V_1 -> V_3 , V_202 ) & V_203 ) )
return V_209 ;
V_153 = F_3 ( V_1 -> V_3 , V_210 ) ;
F_2 ( V_1 -> V_3 , V_210 , V_153 ) ;
F_3 ( V_1 -> V_3 , V_210 ) ;
if ( V_153 == 0 ) {
F_12 ( V_25 L_18 ) ;
return V_209 ;
}
V_207 = 0 ;
if ( F_103 ( V_153 & V_211 ) ) {
if ( V_153 & V_212 ) {
F_12 ( V_25 L_19 ) ;
}
if ( V_153 & V_213 ) {
F_12 ( V_25 L_20 ) ;
}
if ( V_153 & V_214 ) {
F_12 ( V_25 L_21 ) ;
}
if ( V_153 & V_215 ) {
F_12 ( V_25 L_22 ) ;
}
if ( V_153 & V_216 ) {
F_12 ( V_25 L_23 ) ;
}
V_207 = 1 ;
}
if ( V_153 & V_217 ) {
F_104 ( & V_1 -> V_218 ) ;
for ( V_38 = 0 ; V_38 < V_83 ; V_38 ++ ) {
if ( V_1 -> V_88 [ V_38 ] . V_126 == V_127 ) {
if ( ! F_42 ( V_1 , V_38 ) )
continue;
F_105 ( & V_1 -> V_218 ) ;
F_106 ( V_1 -> V_88 [ V_38 ] .
V_102 ) ;
F_104 ( & V_1 -> V_218 ) ;
}
}
#ifndef F_30
for ( V_38 = 0 ; V_38 < V_85 ; V_38 ++ ) {
if ( V_1 -> V_132 [ V_38 ] . V_126 == V_127 ) {
if ( F_53 ( V_1 , V_38 ) ) ;
F_105 ( & V_1 -> V_218 ) ;
F_106 ( V_1 -> V_132 [ V_38 ] .
V_102 ) ;
F_104 ( & V_1 -> V_218 ) ;
}
}
#endif
F_105 ( & V_1 -> V_218 ) ;
V_207 = 1 ;
}
if ( V_153 & V_219 ) {
F_3 ( V_1 -> V_3 , V_201 ) ;
V_207 = 1 ;
}
if ( V_153 & V_220 ) {
F_107 ( V_1 -> V_205 ,
V_1 -> V_221 -> V_222 ) ;
V_207 = 1 ;
}
if ( ! V_207 ) {
F_12 ( V_25 L_24 , V_153 ) ;
}
return F_108 ( V_207 ) ;
}
static void F_109 ( T_1 * V_1 )
{
int V_38 ;
for ( V_38 = 0 ; V_38 < 32 ; V_38 ++ ) {
F_2 ( V_1 -> V_3 , ( V_223 + ( V_38 << 2 ) ) , - V_38 ) ;
F_110 ( 2 ) ;
}
if ( 0 ) {
F_2 ( V_1 -> V_3 , V_224 , 0x8068 ) ;
F_110 ( 1 ) ;
F_2 ( V_1 -> V_3 , V_224 , 0x00e8 ) ;
F_110 ( 1 ) ;
} else {
F_2 ( V_1 -> V_3 , V_224 , 0x00a8 ) ;
F_110 ( 2 ) ;
F_2 ( V_1 -> V_3 , V_224 , 0x80a8 ) ;
F_110 ( 2 ) ;
F_2 ( V_1 -> V_3 , V_224 , 0x80e8 ) ;
F_110 ( 2 ) ;
F_2 ( V_1 -> V_3 , V_224 , 0x80a8 ) ;
F_110 ( 2 ) ;
F_2 ( V_1 -> V_3 , V_224 , 0x00a8 ) ;
F_110 ( 2 ) ;
F_2 ( V_1 -> V_3 , V_224 , 0x00e8 ) ;
}
for ( V_38 = 0 ; V_38 < 32 ; V_38 ++ ) {
F_2 ( V_1 -> V_3 , ( V_223 + ( V_38 << 2 ) ) , - V_38 ) ;
F_110 ( 5 ) ;
}
F_2 ( V_1 -> V_3 , V_224 , 0xe8 ) ;
F_110 ( 1 ) ;
F_2 ( V_1 -> V_3 , V_225 ,
F_3 ( V_1 -> V_3 , V_225 ) | V_226 ) ;
}
static void
F_111 ( struct V_227 * V_228 , unsigned short V_14 , unsigned short V_229 )
{
T_1 * V_47 = ( T_1 * ) V_228 -> V_222 ;
unsigned int V_21 = 0 ;
while ( ! ( F_3 ( V_47 -> V_3 , V_224 ) & 0x100 ) ) {
F_112 ( 100 ) ;
if ( V_21 ++ > V_230 ) {
F_12 ( V_25 L_25 ) ;
return;
}
}
F_2 ( V_47 -> V_3 , V_231 ,
( ( V_14 << V_232 ) & V_233 ) |
( ( V_229 << V_234 ) & V_235 ) |
V_236 |
( V_228 -> V_237 << V_238 ) ) ;
F_3 ( V_47 -> V_3 , V_231 ) ;
}
static unsigned short F_113 ( struct V_227 * V_228 , unsigned short V_14 )
{
T_1 * V_47 = ( T_1 * ) V_228 -> V_222 ;
T_2 V_239 , V_229 ;
unsigned V_21 = 0 ;
while ( ! ( F_3 ( V_47 -> V_3 , V_224 ) & 0x100 ) ) {
F_112 ( 100 ) ;
if ( V_21 ++ > V_230 ) {
F_12 ( V_25 L_25 ) ;
return 0xffff ;
}
}
V_239 = ( ( V_14 << V_232 ) & V_233 ) |
( V_228 -> V_237 << V_238 ) ;
F_2 ( V_47 -> V_3 , V_231 , V_239 ) ;
do {
F_112 ( 100 ) ;
V_229 = F_3 ( V_47 -> V_3 , V_231 ) ;
if ( V_21 ++ > V_230 ) {
F_12 ( V_25 L_26 ) ;
return 0xffff ;
}
} while ( ( V_229 & V_233 ) !=
( V_14 << V_232 ) );
return ( V_240 ) ( V_229 & V_235 ) ;
}
static void F_114 ( T_1 * V_1 , int V_241 , int V_242 )
{
int V_38 , V_243 = 0 , V_244 = 0 , V_245 = 0 , V_246 = 0 ;
F_2 ( V_1 -> V_3 , V_247 ,
F_3 ( V_1 -> V_3 , V_247 ) & 0xfff3fffd ) ;
for ( V_38 = 0 ; V_38 < 11 ; V_38 ++ )
F_2 ( V_1 -> V_3 , V_248 + ( V_38 << 2 ) , 0 ) ;
F_2 ( V_1 -> V_3 , V_225 ,
F_3 ( V_1 -> V_3 , V_225 ) | V_249 ) ;
if ( V_244 && V_245 ) {
int V_250 ;
V_38 = ( ( ( 0x5DC00000 / V_241 ) + 1 ) >> 1 ) ;
if ( V_38 > 0x800 ) {
if ( V_38 < 0x1ffff )
V_250 = ( V_38 >> 1 ) ;
else
V_250 = 0x1ffff ;
} else {
V_38 = V_250 = 0x800 ;
}
F_21 ( V_1 , V_244 , V_250 , 0 , 1 ,
V_246 , 1 , 0 , V_250 , 1 ) ;
F_21 ( V_1 , V_245 , V_250 , 0 , 1 ,
V_246 , 1 , 0 , V_250 , 1 ) ;
}
V_38 = V_241 ;
V_241 |= 0x8c ;
switch ( V_38 ) {
case 32000 :
V_243 &= 0xFFFFFFFE ;
V_243 &= 0xFFFFFFFD ;
V_243 &= 0xF3FFFFFF ;
V_243 |= 0x03000000 ;
V_243 &= 0xFFFFFF3F ;
V_241 &= 0xFFFFFFFD ;
V_241 |= 1 ;
break;
case 44100 :
V_243 &= 0xFFFFFFFE ;
V_243 &= 0xFFFFFFFD ;
V_243 &= 0xF0FFFFFF ;
V_243 |= 0x03000000 ;
V_243 &= 0xFFFFFF3F ;
V_241 &= 0xFFFFFFFC ;
break;
case 48000 :
if ( V_242 == 1 ) {
V_243 &= 0xFFFFFFFE ;
V_243 &= 0xFFFFFFFD ;
V_243 &= 0xF2FFFFFF ;
V_243 |= 0x02000000 ;
V_243 &= 0xFFFFFF3F ;
} else {
V_243 |= 0x00000003 ;
V_243 &= 0xFFFFFFBF ;
V_243 |= 0x80 ;
}
V_241 |= 2 ;
V_241 &= 0xFFFFFFFE ;
break;
}
F_2 ( V_1 -> V_3 , V_251 , V_243 & 0xffff ) ;
F_2 ( V_1 -> V_3 , V_248 , V_243 >> 0x10 ) ;
F_2 ( V_1 -> V_3 , V_252 , V_241 ) ;
}
static int T_6 F_115 ( T_1 * V_1 )
{
F_12 ( V_124 L_27 ) ;
F_2 ( V_1 -> V_3 , V_202 , 0xffffffff ) ;
F_110 ( 5 ) ;
F_2 ( V_1 -> V_3 , V_202 ,
F_3 ( V_1 -> V_3 , V_202 ) & 0xffdfffff ) ;
F_110 ( 5 ) ;
F_2 ( V_1 -> V_3 , V_210 , 0xffffffff ) ;
F_3 ( V_1 -> V_3 , V_201 ) ;
F_109 ( V_1 ) ;
#ifdef F_29
F_2 ( V_1 -> V_3 , V_202 ,
F_3 ( V_1 -> V_3 , V_202 ) | 0x1000000 ) ;
#endif
F_36 ( V_1 ) ;
F_2 ( V_1 -> V_3 , V_253 , 0x0 ) ;
F_59 ( V_1 ) ;
F_35 ( V_1 ) ;
F_14 ( V_1 ) ;
F_22 ( V_1 ) ;
#ifndef F_28
F_116 ( V_1 ) ;
F_114 ( V_1 , 48000 , 1 ) ;
F_117 ( V_1 ) ;
#endif
#ifndef F_30
F_118 ( V_1 ) ;
#endif
F_99 ( V_1 , 0x90 ) ;
F_12 ( V_124 L_28 ) ;
F_119 ( & V_1 -> V_218 ) ;
return 0 ;
}
static int F_120 ( T_1 * V_1 )
{
F_12 ( V_124 L_29 ) ;
#ifndef F_28
F_121 ( V_1 ) ;
F_122 ( V_1 ) ;
#endif
F_101 ( V_1 ) ;
F_87 ( V_1 , 0 ) ;
F_35 ( V_1 ) ;
F_59 ( V_1 ) ;
F_2 ( V_1 -> V_3 , V_204 , 0 ) ;
F_2 ( V_1 -> V_3 , V_202 , 0 ) ;
F_110 ( 5 ) ;
F_2 ( V_1 -> V_3 , V_210 , 0xffff ) ;
F_12 ( V_124 L_28 ) ;
return 0 ;
}
static int F_123 ( int V_254 )
{
int V_107 ;
switch ( V_254 ) {
case V_255 :
V_107 = 0x1 ;
break;
case V_256 :
V_107 = 0x2 ;
break;
case V_257 :
V_107 = 0x3 ;
break;
case V_258 :
V_107 = 0x4 ;
break;
case V_259 :
V_107 = 0x5 ;
break;
case V_260 :
V_107 = 0x8 ;
break;
case V_261 :
V_107 = 0x9 ;
break;
default:
V_107 = 0x8 ;
F_12 ( V_25 L_30 , V_254 ) ;
break;
}
return V_107 ;
}
