static inline void F_1 ( int V_1 , int V_2 , int V_3 )
{
F_2 ( V_2 , V_1 ) ;
F_2 ( V_3 , V_1 + 1 ) ;
}
static inline int F_3 ( int V_1 , int V_2 )
{
F_2 ( V_2 , V_1 ) ;
return F_4 ( V_1 + 1 ) ;
}
static inline void F_5 ( int V_1 )
{
F_2 ( 0x02 , V_1 ) ;
F_2 ( 0x02 , V_1 + 1 ) ;
}
static inline int F_6 ( struct V_4 * V_5 , T_1 V_6 , T_1 V_2 )
{
return F_4 ( V_5 -> V_7 [ V_6 ] + V_2 ) ;
}
static inline int F_7 ( struct V_4 * V_5 , T_1 V_6 ,
T_1 V_8 , T_1 V_2 )
{
F_2 ( V_8 , V_5 -> V_7 [ V_6 ] + V_9 ) ;
return F_4 ( V_5 -> V_7 [ V_6 ] + V_2 ) ;
}
static inline void F_8 ( struct V_4 * V_5 , T_1 V_6 ,
T_1 V_8 , T_1 V_2 , T_1 V_10 )
{
F_2 ( V_8 , V_5 -> V_7 [ V_6 ] + V_9 ) ;
F_2 ( V_10 , V_5 -> V_7 [ V_6 ] + V_2 ) ;
}
static void F_9 ( struct V_4 * V_5 , T_1 V_11 )
{
int V_12 = V_5 -> V_7 [ V_13 ] ;
F_2 ( F_10 ( V_11 ) , V_12 + V_9 ) ;
V_5 -> V_14 [ V_11 ] = F_11 ( V_12 + V_15 ) ;
V_5 -> V_16 [ V_11 ] = F_11 ( V_12 + V_17 ) ;
V_5 -> V_18 [ V_11 ] = F_4 ( V_12 + V_19 ) ;
F_2 ( V_5 -> V_18 [ V_11 ] , V_12 + V_19 ) ;
}
static inline unsigned long F_12 ( T_2 V_2 )
{
V_2 &= 0xfffc ;
if ( V_2 == 0x0000 || V_2 == 0xfffc )
return 0 ;
return 5400000UL / V_2 ;
}
static inline T_2 F_13 ( unsigned long V_3 )
{
if ( V_3 < 83UL )
return 0xffff ;
if ( V_3 >= 1350000UL )
return 0x0004 ;
return ( ( 1350000UL + V_3 / 2 ) / V_3 ) << 2 ;
}
static void F_14 ( struct V_4 * V_5 , T_1 V_11 )
{
int V_12 = V_5 -> V_7 [ V_13 ] ;
F_2 ( F_15 ( V_11 ) , V_12 + V_9 ) ;
V_5 -> V_20 [ V_11 ] = F_4 ( V_12 + V_21 ) ;
V_5 -> V_22 [ V_11 ] = F_4 ( V_12 + V_23 ) ;
}
static inline int F_16 ( T_1 V_2 )
{
switch ( V_2 & V_24 ) {
case V_25 :
return 0 ;
case V_26 :
case V_27 :
return 1 ;
case V_28 :
return 2 ;
default:
return - V_29 ;
}
}
static inline T_1 F_17 ( unsigned long V_3 , T_1 V_30 )
{
switch ( V_3 ) {
default:
return V_25 ;
case 1 :
return V_30 ? V_26 : V_27 ;
case 2 :
return V_28 ;
}
}
static void F_18 ( struct V_4 * V_5 , T_1 V_11 )
{
int V_12 = V_5 -> V_7 [ V_31 ] ;
F_2 ( F_19 ( V_11 ) , V_12 + V_9 ) ;
V_5 -> V_32 [ V_11 ] = F_20 ( F_11 ( V_12 + V_33 ) ) ;
V_5 -> V_34 [ V_11 ] = F_4 ( V_12 + V_35 ) ;
V_5 -> V_36 [ V_11 ] = F_4 ( V_12 + V_37 ) ;
V_5 -> V_38 [ V_11 ] = F_4 ( V_12 + V_39 ) ;
V_5 -> V_40 [ V_11 ] = F_4 ( V_12 + V_41 ) ;
V_5 -> V_42 [ V_11 ] = F_4 ( V_12 + V_43 ) ;
F_2 ( V_5 -> V_42 [ V_11 ] , V_12 + V_43 ) ;
}
static inline unsigned int F_21 ( T_1 V_2 )
{
switch ( V_2 & V_44 ) {
case V_45 :
return 4 ;
case V_46 :
case V_47 :
return 3 ;
default:
return 0 ;
}
}
static inline long F_22 ( T_3 V_2 )
{
return V_2 * 1000 / 256 ;
}
static inline long F_23 ( T_4 V_2 )
{
return V_2 * 1000 ;
}
static struct V_4 * F_24 ( struct V_48 * V_49 )
{
struct V_4 * V_5 = F_25 ( V_49 ) ;
int V_50 ;
F_26 ( & V_5 -> V_51 ) ;
if ( ! F_27 ( V_52 , V_5 -> V_53 + V_54 )
&& V_5 -> V_53 )
goto V_55;
for ( V_50 = 0 ; V_50 < 8 ; V_50 ++ ) {
if ( ! ( V_5 -> V_56 & ( 1 << V_50 ) ) )
continue;
F_9 ( V_5 , V_50 ) ;
}
for ( V_50 = 0 ; V_50 < 4 ; V_50 ++ ) {
if ( ! ( V_5 -> V_57 & ( 1 << V_50 ) ) )
continue;
F_14 ( V_5 , V_50 ) ;
}
for ( V_50 = 0 ; V_50 < 6 ; V_50 ++ ) {
if ( ! ( V_5 -> V_58 & ( 1 << V_50 ) ) )
continue;
F_18 ( V_5 , V_50 ) ;
}
V_5 -> V_53 = V_52 ;
V_55:
F_28 ( & V_5 -> V_51 ) ;
return V_5 ;
}
static T_5 F_29 ( struct V_48 * V_49 , struct V_59
* V_60 , char * V_61 )
{
struct V_4 * V_5 = F_24 ( V_49 ) ;
int V_11 = F_30 ( V_60 ) -> V_62 ;
return sprintf ( V_61 , L_1 , F_12 ( V_5 -> V_14 [ V_11 ] ) ) ;
}
static T_5 F_31 ( struct V_48 * V_49 , struct V_59
* V_60 , char * V_61 )
{
struct V_4 * V_5 = F_24 ( V_49 ) ;
int V_11 = F_30 ( V_60 ) -> V_62 ;
return sprintf ( V_61 , L_1 , F_12 ( V_5 -> V_16 [ V_11 ] ) ) ;
}
static T_5 F_32 ( struct V_48 * V_49 , struct V_59
* V_60 , char * V_61 )
{
struct V_4 * V_5 = F_24 ( V_49 ) ;
int V_11 = F_30 ( V_60 ) -> V_62 ;
return sprintf ( V_61 , L_2 , ! ! ( V_5 -> V_18 [ V_11 ]
& V_63 ) ) ;
}
static T_5 F_33 ( struct V_48 * V_49 , struct V_59
* V_60 , char * V_61 )
{
struct V_4 * V_5 = F_24 ( V_49 ) ;
int V_11 = F_30 ( V_60 ) -> V_62 ;
return sprintf ( V_61 , L_2 , ! ! ( V_5 -> V_18 [ V_11 ]
& V_64 ) ) ;
}
static T_5 F_34 ( struct V_48 * V_49 , struct V_59
* V_60 , const char * V_61 , T_6 V_65 )
{
struct V_4 * V_5 = F_25 ( V_49 ) ;
int V_11 = F_30 ( V_60 ) -> V_62 ;
unsigned long V_3 ;
int V_12 = V_5 -> V_7 [ V_13 ] ;
if ( F_35 ( V_61 , 10 , & V_3 ) < 0 )
return - V_66 ;
F_26 ( & V_5 -> V_51 ) ;
F_2 ( F_10 ( V_11 ) , V_12 + V_9 ) ;
F_2 ( 0 , V_12 + V_19 ) ;
V_5 -> V_16 [ V_11 ] = F_13 ( V_3 ) ;
F_36 ( V_5 -> V_16 [ V_11 ] , V_12 + V_17 ) ;
F_2 ( V_67 , V_12 + V_19 ) ;
F_28 ( & V_5 -> V_51 ) ;
return V_65 ;
}
static void F_37 ( struct V_4 * V_5 , int V_11 , T_1 V_68 )
{
int V_12 = V_5 -> V_7 [ V_13 ] ;
V_5 -> V_20 [ V_11 ] &= ~ V_24 ;
V_5 -> V_20 [ V_11 ] |= V_68 ;
F_2 ( V_5 -> V_20 [ V_11 ] , V_12 + V_21 ) ;
}
static T_5 F_38 ( struct V_48 * V_49 , struct V_59
* V_60 , char * V_61 )
{
struct V_4 * V_5 = F_24 ( V_49 ) ;
int V_11 = F_30 ( V_60 ) -> V_62 ;
int V_20 ;
V_20 = F_16 ( V_5 -> V_20 [ V_11 ] ) ;
if ( V_20 < 0 )
return V_20 ;
return sprintf ( V_61 , L_2 , V_20 ) ;
}
static T_5 F_39 ( struct V_48 * V_49 , struct V_59
* V_60 , const char * V_61 , T_6 V_65 )
{
struct V_4 * V_5 = F_25 ( V_49 ) ;
int V_11 = F_30 ( V_60 ) -> V_62 ;
unsigned long V_3 ;
if ( F_35 ( V_61 , 10 , & V_3 ) < 0 || V_3 > 2 )
return - V_66 ;
if ( V_3 == 2 && ! ( V_5 -> V_69 & ( 1 << V_11 ) ) )
return - V_66 ;
F_26 ( & V_5 -> V_51 ) ;
F_14 ( V_5 , V_11 ) ;
F_37 ( V_5 , V_11 , F_17 ( V_3 , V_5 -> V_22 [ V_11 ] ) ) ;
F_28 ( & V_5 -> V_51 ) ;
return V_65 ;
}
static T_5 F_40 ( struct V_48 * V_49 , struct V_59
* V_60 , char * V_61 )
{
struct V_4 * V_5 = F_24 ( V_49 ) ;
int V_11 = F_30 ( V_60 ) -> V_62 ;
return sprintf ( V_61 , L_2 , ( int ) V_5 -> V_22 [ V_11 ] ) ;
}
static T_5 F_41 ( struct V_48 * V_49 , struct V_59
* V_60 , const char * V_61 , T_6 V_65 )
{
struct V_4 * V_5 = F_25 ( V_49 ) ;
int V_11 = F_30 ( V_60 ) -> V_62 ;
unsigned long V_3 ;
int V_12 = V_5 -> V_7 [ V_13 ] ;
T_1 V_68 ;
if ( F_35 ( V_61 , 10 , & V_3 ) < 0 || V_3 > 0xff )
return - V_66 ;
F_26 ( & V_5 -> V_51 ) ;
F_14 ( V_5 , V_11 ) ;
V_68 = V_5 -> V_20 [ V_11 ] & V_24 ;
if ( V_68 != V_26 && V_68 != V_27 ) {
F_42 ( V_49 , L_3
L_4 , V_11 + 1 ) ;
F_28 ( & V_5 -> V_51 ) ;
return - V_70 ;
}
if ( V_68 == V_26 && V_3 == 0 ) {
F_37 ( V_5 , V_11 , V_27 ) ;
V_68 = V_27 ;
F_43 ( V_49 , L_5 , V_11 + 1 ,
L_6 , L_7 ) ;
} else if ( V_68 == V_27 && V_3 != 0 ) {
F_37 ( V_5 , V_11 , V_26 ) ;
V_68 = V_26 ;
F_43 ( V_49 , L_5 , V_11 + 1 ,
L_7 , L_6 ) ;
}
V_5 -> V_22 [ V_11 ] = V_3 ;
if ( V_68 == V_26 )
F_2 ( V_3 , V_12 + V_23 ) ;
F_28 ( & V_5 -> V_51 ) ;
return V_65 ;
}
static T_5 F_44 ( struct V_48 * V_49 , struct V_59
* V_60 , char * V_61 )
{
struct V_4 * V_5 = F_24 ( V_49 ) ;
int V_11 = F_30 ( V_60 ) -> V_62 ;
return sprintf ( V_61 , L_8 , F_22 ( V_5 -> V_32 [ V_11 ] ) ) ;
}
static T_5 F_45 ( struct V_48 * V_49 , struct V_59
* V_60 , char * V_61 )
{
struct V_4 * V_5 = F_24 ( V_49 ) ;
int V_11 = F_30 ( V_60 ) -> V_62 ;
return sprintf ( V_61 , L_8 , F_23 ( V_5 -> V_36 [ V_11 ] ) ) ;
}
static T_5 F_46 ( struct V_48 * V_49 , struct V_59
* V_60 , char * V_61 )
{
struct V_4 * V_5 = F_24 ( V_49 ) ;
int V_11 = F_30 ( V_60 ) -> V_62 ;
return sprintf ( V_61 , L_8 , F_23 ( V_5 -> V_34 [ V_11 ] ) ) ;
}
static T_5 F_47 ( struct V_48 * V_49 , struct V_59
* V_60 , char * V_61 )
{
struct V_4 * V_5 = F_24 ( V_49 ) ;
int V_11 = F_30 ( V_60 ) -> V_62 ;
return sprintf ( V_61 , L_8 , F_23 ( V_5 -> V_38 [ V_11 ] ) ) ;
}
static T_5 F_48 ( struct V_48 * V_49 , struct V_59
* V_60 , char * V_61 )
{
struct V_4 * V_5 = F_24 ( V_49 ) ;
int V_11 = F_30 ( V_60 ) -> V_62 ;
return sprintf ( V_61 , L_9 , F_21 ( V_5 -> V_40 [ V_11 ] ) ) ;
}
static T_5 F_49 ( struct V_48 * V_49 , struct V_59
* V_60 , char * V_61 )
{
struct V_4 * V_5 = F_24 ( V_49 ) ;
int V_11 = F_30 ( V_60 ) -> V_62 ;
return sprintf ( V_61 , L_2 , ! ! ( V_5 -> V_42 [ V_11 ]
& V_71 ) ) ;
}
static T_5 F_50 ( struct V_48 * V_49 , struct V_59
* V_60 , char * V_61 )
{
struct V_4 * V_5 = F_24 ( V_49 ) ;
int V_11 = F_30 ( V_60 ) -> V_62 ;
return sprintf ( V_61 , L_2 , ! ! ( V_5 -> V_42 [ V_11 ]
& V_72 ) ) ;
}
static T_5 F_51 ( struct V_48 * V_49 , struct V_59
* V_60 , char * V_61 )
{
struct V_4 * V_5 = F_24 ( V_49 ) ;
int V_11 = F_30 ( V_60 ) -> V_62 ;
return sprintf ( V_61 , L_2 , ! ! ( V_5 -> V_42 [ V_11 ]
& V_73 ) ) ;
}
static T_5 F_52 ( struct V_48 * V_49 , struct V_59
* V_60 , char * V_61 )
{
struct V_4 * V_5 = F_24 ( V_49 ) ;
int V_11 = F_30 ( V_60 ) -> V_62 ;
return sprintf ( V_61 , L_2 , ! ! ( V_5 -> V_42 [ V_11 ]
& V_74 ) ) ;
}
static T_5 F_53 ( struct V_48 * V_49 , struct V_59
* V_60 , char * V_61 )
{
struct V_4 * V_5 = F_25 ( V_49 ) ;
return sprintf ( V_61 , L_10 , V_5 -> V_75 ) ;
}
static void F_54 ( struct V_76 * V_77 , int V_65 )
{
struct V_78 * V_79 ;
int V_50 ;
for ( V_50 = 0 ; V_50 < V_65 ; V_50 ++ ) {
V_79 = F_55 ( V_77 , V_80 , V_50 ) ;
F_56 ( V_79 -> V_81 , F_57 ( V_79 ) ) ;
}
}
static int T_7 F_58 ( struct V_76 * V_77 ,
int V_65 )
{
struct V_78 * V_79 ;
int V_50 , V_82 = 0 ;
for ( V_50 = 0 ; V_50 < V_65 ; V_50 ++ ) {
V_79 = F_55 ( V_77 , V_80 , V_50 ) ;
if ( ! V_79 ) {
V_82 = - V_83 ;
F_59 ( & V_77 -> V_49 , L_11 , V_50 ) ;
break;
}
if ( ! F_60 ( V_79 -> V_81 , F_57 ( V_79 ) , V_84 ) ) {
V_82 = - V_85 ;
F_59 ( & V_77 -> V_49 ,
L_12 ,
( unsigned long ) V_79 -> V_81 ,
( unsigned long ) V_79 -> V_86 ) ;
break;
}
}
if ( V_82 && V_50 )
F_54 ( V_77 , V_50 ) ;
return V_82 ;
}
static void T_7 F_61 ( struct V_48 * V_49 )
{
struct V_87 * V_88 = V_49 -> V_89 ;
struct V_4 * V_5 = F_25 ( V_49 ) ;
int V_50 ;
T_1 V_2 ;
V_2 = F_6 ( V_5 , V_13 , V_9 ) ;
if ( ! ( V_2 & 0x80 ) )
F_62 ( V_49 , L_13 , L_14 ) ;
for ( V_50 = 0 ; V_50 < 8 ; V_50 ++ ) {
if ( ! ( V_88 -> V_90 & ( 1 << V_50 ) ) )
continue;
V_2 = F_7 ( V_5 , V_13 , F_10 ( V_50 ) ,
V_19 ) ;
if ( V_2 & V_67 )
V_5 -> V_56 |= ( 1 << V_50 ) ;
}
if ( ! V_5 -> V_56 ) {
F_43 ( V_49 , L_15 ) ;
for ( V_50 = 0 ; V_50 < 8 ; V_50 ++ ) {
if ( ! ( V_88 -> V_90 & ( 1 << V_50 ) ) )
continue;
F_8 ( V_5 , V_13 , F_10 ( V_50 ) ,
V_19 ,
V_67 ) ;
}
V_5 -> V_56 = V_88 -> V_90 ;
}
for ( V_50 = 0 ; V_50 < 4 ; V_50 ++ ) {
if ( ! ( V_88 -> V_91 & ( 1 << V_50 ) ) )
continue;
V_2 = F_7 ( V_5 , V_13 , F_15 ( V_50 ) ,
V_21 ) ;
if ( V_2 & V_92 )
V_5 -> V_57 |= ( 1 << V_50 ) ;
if ( ( V_2 & V_24 ) == V_28 ) {
F_43 ( V_49 , L_16 ,
V_50 + 1 ) ;
V_5 -> V_69 |= ( 1 << V_50 ) ;
}
}
V_2 = F_6 ( V_5 , V_31 , V_9 ) ;
if ( ! ( V_2 & 0x80 ) )
F_62 ( V_49 , L_13 , L_17 ) ;
for ( V_50 = 0 ; V_50 < 6 ; V_50 ++ ) {
V_2 = F_7 ( V_5 , V_31 , F_19 ( V_50 ) ,
V_43 ) ;
if ( V_2 & V_93 )
V_5 -> V_58 |= ( 1 << V_50 ) ;
}
}
static void F_63 ( struct V_48 * V_49 )
{
struct V_4 * V_5 = F_25 ( V_49 ) ;
int V_50 ;
F_64 ( V_49 , & V_94 ) ;
for ( V_50 = 0 ; V_50 < 8 ; V_50 ++ ) {
if ( ! ( V_5 -> V_56 & ( 1 << V_50 ) ) )
continue;
F_65 ( & V_49 -> V_95 , & V_96 [ V_50 ] ) ;
}
for ( V_50 = 0 ; V_50 < 4 ; V_50 ++ ) {
if ( ! ( V_5 -> V_57 & ( 1 << V_50 ) ) )
continue;
F_65 ( & V_49 -> V_95 , & V_97 [ V_50 ] ) ;
}
for ( V_50 = 0 ; V_50 < 6 ; V_50 ++ ) {
if ( ! ( V_5 -> V_58 & ( 1 << V_50 ) ) )
continue;
F_65 ( & V_49 -> V_95 , & V_98 [ V_50 ] ) ;
}
}
static int T_7 F_66 ( struct V_76 * V_77 )
{
struct V_87 * V_88 = V_77 -> V_49 . V_89 ;
struct V_4 * V_5 ;
int V_50 , V_82 , V_99 ;
V_5 = F_67 ( sizeof( struct V_4 ) , V_100 ) ;
if ( ! V_5 ) {
V_82 = - V_101 ;
F_68 ( L_18 ) ;
goto exit;
}
V_5 -> V_7 [ 0 ] = V_88 -> V_7 [ 0 ] ;
V_5 -> V_7 [ 1 ] = V_88 -> V_7 [ 1 ] ;
V_99 = ( V_5 -> V_7 [ 0 ] != 0 ) + ( V_5 -> V_7 [ 1 ] != 0 ) ;
V_82 = F_58 ( V_77 , V_99 ) ;
if ( V_82 )
goto V_102;
F_69 ( & V_5 -> V_51 ) ;
V_5 -> V_75 = L_19 ;
F_70 ( V_77 , V_5 ) ;
F_61 ( & V_77 -> V_49 ) ;
V_82 = F_71 ( & V_77 -> V_49 , & V_94 ) ;
if ( V_82 )
goto V_103;
for ( V_50 = 0 ; V_50 < 8 ; V_50 ++ ) {
if ( ! ( V_5 -> V_56 & ( 1 << V_50 ) ) )
continue;
V_82 = F_72 ( & V_77 -> V_49 . V_95 ,
& V_96 [ V_50 ] ) ;
if ( V_82 )
goto V_104;
}
for ( V_50 = 0 ; V_50 < 4 ; V_50 ++ ) {
if ( ! ( V_5 -> V_57 & ( 1 << V_50 ) ) )
continue;
V_82 = F_72 ( & V_77 -> V_49 . V_95 ,
& V_97 [ V_50 ] ) ;
if ( V_82 )
goto V_104;
}
for ( V_50 = 0 ; V_50 < 6 ; V_50 ++ ) {
if ( ! ( V_5 -> V_58 & ( 1 << V_50 ) ) )
continue;
V_82 = F_72 ( & V_77 -> V_49 . V_95 ,
& V_98 [ V_50 ] ) ;
if ( V_82 )
goto V_104;
}
V_5 -> V_105 = F_73 ( & V_77 -> V_49 ) ;
if ( F_74 ( V_5 -> V_105 ) ) {
V_82 = F_75 ( V_5 -> V_105 ) ;
F_59 ( & V_77 -> V_49 , L_20 , V_82 ) ;
goto V_104;
}
return 0 ;
V_104:
F_63 ( & V_77 -> V_49 ) ;
V_103:
F_54 ( V_77 , V_99 ) ;
V_102:
F_70 ( V_77 , NULL ) ;
F_76 ( V_5 ) ;
exit:
return V_82 ;
}
static int T_8 F_77 ( struct V_76 * V_77 )
{
struct V_4 * V_5 = F_78 ( V_77 ) ;
int V_99 ;
V_99 = ( V_5 -> V_7 [ 0 ] != 0 ) + ( V_5 -> V_7 [ 1 ] != 0 ) ;
F_79 ( V_5 -> V_105 ) ;
F_63 ( & V_77 -> V_49 ) ;
F_70 ( V_77 , NULL ) ;
F_76 ( V_5 ) ;
F_54 ( V_77 , V_99 ) ;
return 0 ;
}
static int T_9 F_80 ( const struct V_87 * V_88 )
{
struct V_78 V_79 [ 2 ] = {
{ . V_106 = V_80 } ,
{ . V_106 = V_80 } ,
} ;
int V_82 , V_50 , V_99 ;
V_99 = 0 ;
for ( V_50 = 0 ; V_50 < 2 ; V_50 ++ ) {
if ( ! V_88 -> V_7 [ V_50 ] )
continue;
V_79 [ V_99 ] . V_81 = V_88 -> V_7 [ V_50 ] ;
V_79 [ V_99 ] . V_86 = V_88 -> V_7 [ V_50 ] + V_107 - 1 ;
V_79 [ V_99 ] . V_75 = V_108 [ V_50 ] ;
V_82 = F_81 ( & V_79 [ V_99 ] ) ;
if ( V_82 )
goto exit;
V_99 ++ ;
}
V_77 = F_82 ( V_84 , V_79 [ 0 ] . V_81 ) ;
if ( ! V_77 ) {
V_82 = - V_101 ;
F_68 ( L_21 ) ;
goto exit;
}
V_82 = F_83 ( V_77 , V_79 , V_99 ) ;
if ( V_82 ) {
F_68 ( L_22 , V_82 ) ;
goto V_109;
}
V_82 = F_84 ( V_77 , V_88 ,
sizeof( struct V_87 ) ) ;
if ( V_82 ) {
F_68 ( L_23 ) ;
goto V_109;
}
V_82 = F_85 ( V_77 ) ;
if ( V_82 ) {
F_68 ( L_24 , V_82 ) ;
goto V_109;
}
return 0 ;
V_109:
F_86 ( V_77 ) ;
exit:
return V_82 ;
}
static int T_9 F_87 ( int V_1 , struct V_87 * V_88 )
{
T_2 V_3 ;
T_1 V_110 , V_111 ;
int V_50 , V_82 = 0 ;
V_3 = V_112 ? V_112 : F_3 ( V_1 , V_113 ) ;
if ( V_3 != 0xf2 ) {
V_82 = - V_114 ;
goto exit;
}
for ( V_50 = 0 ; V_50 < 2 ; V_50 ++ ) {
V_88 -> V_7 [ V_50 ] = 0 ;
F_1 ( V_1 , V_115 , V_116 [ V_50 ] ) ;
V_3 = F_3 ( V_1 , V_117 ) ;
if ( ! ( V_3 & 0x01 ) ) {
F_88 ( L_25 ,
V_116 [ V_50 ] ) ;
continue;
}
V_3 = F_3 ( V_1 , V_118 ) ;
if ( V_3 & 0x01 ) {
F_89 ( L_26
L_27 , V_116 [ V_50 ] ) ;
continue;
}
V_3 = ( F_3 ( V_1 , V_119 ) << 8 )
| F_3 ( V_1 , V_119 + 1 ) ;
if ( ! V_3 ) {
F_88 ( L_28
L_29 , V_116 [ V_50 ] ) ;
continue;
}
V_88 -> V_7 [ V_50 ] = V_3 ;
}
if ( ! V_88 -> V_7 [ 0 ] && ! V_88 -> V_7 [ 1 ] ) {
V_82 = - V_114 ;
goto exit;
}
V_88 -> V_90 = ( 1 << 2 ) | ( 1 << 3 ) ;
V_110 = F_3 ( V_1 , V_120 ) ;
if ( ! ( V_110 & ( 1 << 3 ) ) )
V_88 -> V_90 |= ( 1 << 0 ) ;
if ( ! ( V_110 & ( 1 << 2 ) ) )
V_88 -> V_90 |= ( 1 << 4 ) ;
V_110 = F_3 ( V_1 , V_121 ) ;
if ( ! ( V_110 & ( 1 << 0 ) ) )
V_88 -> V_90 |= ( 1 << 1 ) ;
V_110 = F_3 ( V_1 , V_122 ) ;
if ( ! ( V_110 & ( 1 << 0 ) ) )
V_88 -> V_90 |= ( 1 << 7 ) ;
V_111 = F_3 ( V_1 , V_123 ) ;
if ( ! ( V_110 & ( 1 << 1 ) ) && ( V_111 & ( 1 << 3 ) ) )
V_88 -> V_90 |= ( 1 << 5 ) ;
V_110 = F_3 ( V_1 , V_124 ) ;
if ( ( V_110 & ( 1 << 3 ) ) && ! ( V_111 & ( 1 << 5 ) ) )
V_88 -> V_90 |= ( 1 << 6 ) ;
V_88 -> V_91 = ( 1 << 0 ) ;
if ( V_111 & ( 1 << 0 ) )
V_88 -> V_91 |= ( 1 << 3 ) ;
V_110 = F_3 ( V_1 , V_125 ) ;
if ( ! ( V_110 & ( 1 << 4 ) ) ) {
if ( V_111 & ( 1 << 1 ) )
V_88 -> V_91 |= ( 1 << 1 ) ;
if ( V_111 & ( 1 << 2 ) )
V_88 -> V_91 |= ( 1 << 2 ) ;
}
V_110 = F_3 ( V_1 , V_126 ) ;
if ( V_110 & ( 1 << 6 ) )
V_88 -> V_91 |= ( 1 << 1 ) ;
if ( V_110 & ( 1 << 5 ) )
V_88 -> V_91 |= ( 1 << 2 ) ;
exit:
F_5 ( V_1 ) ;
return V_82 ;
}
static int T_9 F_90 ( void )
{
int V_82 ;
struct V_87 V_88 ;
if ( F_87 ( 0x2e , & V_88 )
&& F_87 ( 0x4e , & V_88 ) )
return - V_114 ;
V_82 = F_91 ( & V_127 ) ;
if ( V_82 )
goto exit;
V_82 = F_80 ( & V_88 ) ;
if ( V_82 )
goto V_128;
return 0 ;
V_128:
F_92 ( & V_127 ) ;
exit:
return V_82 ;
}
static void T_10 F_93 ( void )
{
F_94 ( V_77 ) ;
F_92 ( & V_127 ) ;
}
