static int F_1 ( struct V_1 * V_2 , struct V_3 * V_4 )
{
int V_5 ;
unsigned int V_6 ;
T_1 V_7 ;
T_1 * V_8 ;
V_8 = F_2 ( V_4 -> V_9 , V_10 ) ;
if ( ! V_8 ) {
V_5 = - V_11 ;
goto V_12;
}
if ( V_4 -> V_13 & V_14 ) {
memcpy ( V_8 , V_4 -> V_15 , V_4 -> V_9 ) ;
V_7 = ( V_16 | V_17 ) ;
V_6 = F_3 ( V_2 -> V_18 , 0 ) ;
} else {
V_7 = ( V_16 | V_19 ) ;
V_6 = F_4 ( V_2 -> V_18 , 0 ) ;
}
V_5 = F_5 ( V_2 -> V_18 , V_6 , 0 , V_7 , V_4 -> V_20 ,
V_4 -> V_13 , V_8 , V_4 -> V_9 , 1000 ) ;
F_6 ( V_2 -> V_18 , 0 , V_7 , V_4 -> V_20 ,
V_4 -> V_13 , V_8 , V_4 -> V_9 ) ;
if ( V_5 > 0 )
V_5 = 0 ;
if ( ! V_5 && V_7 == ( V_16 | V_19 ) )
memcpy ( V_4 -> V_15 , V_8 , V_4 -> V_9 ) ;
F_7 ( V_8 ) ;
if ( V_5 )
goto V_12;
return V_5 ;
V_12:
F_8 ( & V_2 -> V_18 -> V_21 , L_1 , V_22 , V_5 ) ;
return V_5 ;
}
static int F_9 ( struct V_1 * V_2 , T_2 V_23 , T_1 * V_24 , int V_25 )
{
struct V_3 V_4 ;
if ( V_23 < 0x3000 )
V_4 . V_13 = V_26 ;
else if ( V_23 < 0x4000 )
V_4 . V_13 = V_27 ;
else
V_4 . V_13 = V_28 ;
V_4 . V_20 = V_23 ;
V_4 . V_9 = V_25 ;
V_4 . V_15 = V_24 ;
return F_1 ( V_2 , & V_4 ) ;
}
static int F_10 ( struct V_1 * V_2 , T_2 V_23 , T_1 * V_24 , int V_25 )
{
struct V_3 V_4 ;
if ( V_23 < 0x3000 )
V_4 . V_13 = V_29 ;
else if ( V_23 < 0x4000 )
V_4 . V_13 = V_30 ;
else
V_4 . V_13 = V_31 ;
V_4 . V_20 = V_23 ;
V_4 . V_9 = V_25 ;
V_4 . V_15 = V_24 ;
return F_1 ( V_2 , & V_4 ) ;
}
static int F_11 ( struct V_1 * V_2 , T_2 V_23 , T_1 V_24 )
{
return F_9 ( V_2 , V_23 , & V_24 , 1 ) ;
}
static int F_12 ( struct V_1 * V_2 , T_2 V_23 , T_1 * V_24 )
{
return F_10 ( V_2 , V_23 , V_24 , 1 ) ;
}
static int F_13 ( struct V_1 * V_2 , T_2 V_23 , T_1 V_24 ,
T_1 V_32 )
{
int V_5 ;
T_1 V_33 ;
if ( V_32 != 0xff ) {
V_5 = F_12 ( V_2 , V_23 , & V_33 ) ;
if ( V_5 )
return V_5 ;
V_24 &= V_32 ;
V_33 &= ~ V_32 ;
V_24 |= V_33 ;
}
return F_11 ( V_2 , V_23 , V_24 ) ;
}
static int F_14 ( struct V_34 * V_35 , struct V_36 V_37 [] ,
int V_38 )
{
int V_5 ;
struct V_1 * V_2 = F_15 ( V_35 ) ;
struct V_39 * V_40 = V_2 -> V_40 ;
struct V_3 V_4 ;
if ( F_16 ( & V_2 -> V_41 ) < 0 )
return - V_42 ;
if ( V_38 == 2 && ! ( V_37 [ 0 ] . V_43 & V_44 ) &&
( V_37 [ 1 ] . V_43 & V_44 ) ) {
if ( V_37 [ 0 ] . V_25 > 24 || V_37 [ 1 ] . V_25 > 24 ) {
V_5 = - V_45 ;
goto V_46;
} else if ( V_37 [ 0 ] . V_47 == 0x10 ) {
V_4 . V_20 = ( V_37 [ 0 ] . V_8 [ 0 ] << 8 ) | ( V_37 [ 0 ] . V_47 << 1 ) ;
V_4 . V_13 = V_48 | V_40 -> V_49 ;
V_4 . V_9 = V_37 [ 1 ] . V_25 ;
V_4 . V_15 = & V_37 [ 1 ] . V_8 [ 0 ] ;
V_5 = F_1 ( V_2 , & V_4 ) ;
} else if ( V_37 [ 0 ] . V_25 < 2 ) {
V_4 . V_20 = ( V_37 [ 0 ] . V_8 [ 0 ] << 8 ) | ( V_37 [ 0 ] . V_47 << 1 ) ;
V_4 . V_13 = V_50 ;
V_4 . V_9 = V_37 [ 1 ] . V_25 ;
V_4 . V_15 = & V_37 [ 1 ] . V_8 [ 0 ] ;
V_5 = F_1 ( V_2 , & V_4 ) ;
} else {
V_4 . V_20 = ( V_37 [ 0 ] . V_47 << 1 ) ;
V_4 . V_13 = V_51 ;
V_4 . V_9 = V_37 [ 0 ] . V_25 ;
V_4 . V_15 = V_37 [ 0 ] . V_8 ;
V_5 = F_1 ( V_2 , & V_4 ) ;
if ( V_5 )
goto V_46;
V_4 . V_20 = ( V_37 [ 0 ] . V_47 << 1 ) ;
V_4 . V_13 = V_52 ;
V_4 . V_9 = V_37 [ 1 ] . V_25 ;
V_4 . V_15 = V_37 [ 1 ] . V_8 ;
V_5 = F_1 ( V_2 , & V_4 ) ;
}
} else if ( V_38 == 1 && ! ( V_37 [ 0 ] . V_43 & V_44 ) ) {
if ( V_37 [ 0 ] . V_25 > 22 ) {
V_5 = - V_45 ;
goto V_46;
} else if ( V_37 [ 0 ] . V_47 == 0x10 ) {
if ( V_37 [ 0 ] . V_8 [ 0 ] == 0x00 ) {
V_40 -> V_49 = V_37 [ 0 ] . V_8 [ 1 ] ;
V_5 = 0 ;
} else {
V_4 . V_20 = ( V_37 [ 0 ] . V_8 [ 0 ] << 8 ) |
( V_37 [ 0 ] . V_47 << 1 ) ;
V_4 . V_13 = V_53 | V_40 -> V_49 ;
V_4 . V_9 = V_37 [ 0 ] . V_25 - 1 ;
V_4 . V_15 = & V_37 [ 0 ] . V_8 [ 1 ] ;
V_5 = F_1 ( V_2 , & V_4 ) ;
}
} else if ( V_37 [ 0 ] . V_25 < 23 ) {
V_4 . V_20 = ( V_37 [ 0 ] . V_8 [ 0 ] << 8 ) | ( V_37 [ 0 ] . V_47 << 1 ) ;
V_4 . V_13 = V_54 ;
V_4 . V_9 = V_37 [ 0 ] . V_25 - 1 ;
V_4 . V_15 = & V_37 [ 0 ] . V_8 [ 1 ] ;
V_5 = F_1 ( V_2 , & V_4 ) ;
} else {
V_4 . V_20 = ( V_37 [ 0 ] . V_47 << 1 ) ;
V_4 . V_13 = V_51 ;
V_4 . V_9 = V_37 [ 0 ] . V_25 ;
V_4 . V_15 = V_37 [ 0 ] . V_8 ;
V_5 = F_1 ( V_2 , & V_4 ) ;
}
} else {
V_5 = - V_55 ;
}
V_46:
F_17 ( & V_2 -> V_41 ) ;
return V_5 ? V_5 : V_38 ;
}
static T_3 F_18 ( struct V_34 * V_56 )
{
return V_57 ;
}
static int F_19 ( struct V_1 * V_2 )
{
struct V_39 * V_40 = F_20 ( V_2 ) ;
int V_5 ;
T_1 V_8 [ 1 ] ;
struct V_3 V_58 = { 0x0120 , 0x0011 , 0x0001 , L_2 } ;
struct V_3 V_59 = { 0x00c0 , V_50 , 1 , V_8 } ;
struct V_3 V_60 = { 0x0fc4 , V_50 , 1 , V_8 } ;
F_8 ( & V_2 -> V_18 -> V_21 , L_3 , V_22 ) ;
V_5 = F_11 ( V_2 , V_61 , 0x0a ) ;
if ( V_5 )
goto V_12;
V_5 = F_11 ( V_2 , V_62 , 0x15 ) ;
if ( V_5 )
goto V_12;
F_21 ( 20 ) ;
V_40 -> V_63 = L_4 ;
V_5 = F_1 ( V_2 , & V_58 ) ;
if ( V_5 )
goto V_12;
V_5 = F_1 ( V_2 , & V_60 ) ;
if ( V_5 == 0 && V_8 [ 0 ] == 0x2c ) {
V_40 -> V_64 = V_65 ;
V_40 -> V_63 = L_5 ;
goto V_66;
}
V_5 = F_1 ( V_2 , & V_58 ) ;
if ( V_5 )
goto V_12;
V_5 = F_1 ( V_2 , & V_59 ) ;
if ( V_5 == 0 && V_8 [ 0 ] == 0x63 ) {
V_40 -> V_64 = V_67 ;
V_40 -> V_63 = L_6 ;
goto V_66;
}
V_40 -> V_64 = V_68 ;
V_40 -> V_63 = L_7 ;
goto V_66;
V_66:
F_8 ( & V_2 -> V_18 -> V_21 , L_8 , V_22 , V_40 -> V_63 ) ;
return 0 ;
V_12:
F_8 ( & V_2 -> V_18 -> V_21 , L_1 , V_22 , V_5 ) ;
return V_5 ;
}
static int F_22 ( struct V_1 * V_2 )
{
struct V_39 * V_40 = F_20 ( V_2 ) ;
int V_5 ;
T_1 V_8 [ 2 ] ;
struct V_3 V_58 = { 0x0120 , 0x0011 , 0x0001 , L_9 } ;
struct V_3 V_69 = { 0x0120 , 0x0011 , 0x0001 , L_10 } ;
struct V_3 V_70 = { 0x00c6 , V_50 , 1 , V_8 } ;
struct V_3 V_71 = { 0x00c6 , V_50 , 1 , V_8 } ;
struct V_3 V_72 = { 0x00c0 , V_50 , 1 , V_8 } ;
struct V_3 V_73 = { 0x01ac , V_50 , 1 , V_8 } ;
struct V_3 V_74 = { 0x00c0 , V_50 , 1 , V_8 } ;
struct V_3 V_75 = { 0x00c0 , V_50 , 1 , V_8 } ;
struct V_3 V_76 = { 0x7ec0 , V_50 , 2 , V_8 } ;
struct V_3 V_77 = { 0xd9c0 , V_50 , 1 , V_8 } ;
struct V_3 V_78 = { 0x02c8 , V_50 , 1 , V_8 } ;
struct V_3 V_79 = { 0x00c0 , V_50 , 2 , V_8 } ;
F_8 ( & V_2 -> V_18 -> V_21 , L_3 , V_22 ) ;
V_5 = F_13 ( V_2 , V_61 , 0x00 , 0x40 ) ;
if ( V_5 )
goto V_12;
V_5 = F_13 ( V_2 , V_62 , 0x48 , 0x48 ) ;
if ( V_5 )
goto V_12;
V_5 = F_1 ( V_2 , & V_58 ) ;
if ( V_5 )
goto V_12;
V_40 -> V_63 = L_4 ;
V_5 = F_1 ( V_2 , & V_70 ) ;
if ( V_5 == 0 && V_8 [ 0 ] == 0xa1 ) {
V_40 -> V_64 = V_80 ;
V_40 -> V_63 = L_11 ;
goto V_66;
}
V_5 = F_1 ( V_2 , & V_71 ) ;
if ( V_5 == 0 && V_8 [ 0 ] == 0xa3 ) {
V_40 -> V_64 = V_81 ;
V_40 -> V_63 = L_12 ;
goto V_66;
}
V_5 = F_1 ( V_2 , & V_72 ) ;
if ( V_5 == 0 && V_8 [ 0 ] == 0x85 ) {
V_40 -> V_64 = V_82 ;
V_40 -> V_63 = L_13 ;
goto V_66;
}
V_5 = F_1 ( V_2 , & V_73 ) ;
if ( V_5 == 0 && V_8 [ 0 ] == 0x56 ) {
V_40 -> V_64 = V_83 ;
V_40 -> V_63 = L_14 ;
goto V_66;
}
V_5 = F_1 ( V_2 , & V_74 ) ;
if ( V_5 == 0 && ( V_8 [ 0 ] == 0x9e || V_8 [ 0 ] == 0x9c ) ) {
V_40 -> V_64 = V_84 ;
V_40 -> V_63 = L_15 ;
goto V_66;
}
V_5 = F_1 ( V_2 , & V_75 ) ;
if ( V_5 == 0 && V_8 [ 0 ] == 0x38 ) {
V_40 -> V_64 = V_85 ;
V_40 -> V_63 = L_16 ;
goto V_66;
}
V_5 = F_1 ( V_2 , & V_76 ) ;
if ( V_5 == 0 && V_8 [ 0 ] == 0x23 && V_8 [ 1 ] == 0x28 ) {
V_40 -> V_64 = V_86 ;
V_40 -> V_63 = L_17 ;
goto V_66;
}
V_5 = F_1 ( V_2 , & V_77 ) ;
if ( V_5 == 0 && V_8 [ 0 ] == 0x14 ) {
V_40 -> V_64 = V_87 ;
V_40 -> V_63 = L_18 ;
goto V_66;
}
V_5 = F_1 ( V_2 , & V_78 ) ;
if ( V_5 == 0 && V_8 [ 0 ] == 0x40 ) {
V_40 -> V_64 = V_88 ;
V_40 -> V_63 = L_19 ;
goto V_66;
}
V_5 = F_1 ( V_2 , & V_79 ) ;
if ( V_5 == 0 && ( V_8 [ 0 ] == 0xc7 || V_8 [ 1 ] == 0x60 ) ) {
V_40 -> V_64 = V_89 ;
V_40 -> V_63 = L_20 ;
goto V_66;
}
V_66:
F_8 ( & V_2 -> V_18 -> V_21 , L_8 , V_22 , V_40 -> V_63 ) ;
V_5 = F_1 ( V_2 , & V_69 ) ;
if ( V_5 < 0 )
goto V_12;
return 0 ;
V_12:
F_8 ( & V_2 -> V_18 -> V_21 , L_1 , V_22 , V_5 ) ;
return V_5 ;
}
static int F_23 ( struct V_90 * V_35 )
{
struct V_1 * V_2 = F_24 ( V_35 ) ;
struct V_39 * V_40 = F_20 ( V_2 ) ;
struct V_91 * V_91 ;
int V_5 ;
F_8 ( & V_2 -> V_18 -> V_21 , L_3 , V_22 ) ;
switch ( V_40 -> V_64 ) {
case V_65 :
V_91 = & V_92 ;
break;
case V_67 :
V_91 = & V_93 ;
break;
case V_68 :
V_91 = & V_94 ;
break;
default:
F_25 ( & V_2 -> V_18 -> V_21 , L_21 ,
V_95 , V_40 -> V_63 ) ;
V_5 = - V_96 ;
goto V_12;
}
V_35 -> V_97 [ 0 ] = F_26 ( V_98 , V_91 , & V_2 -> V_99 ) ;
if ( ! V_35 -> V_97 [ 0 ] ) {
V_5 = - V_96 ;
goto V_12;
}
return 0 ;
V_12:
F_8 ( & V_2 -> V_18 -> V_21 , L_1 , V_22 , V_5 ) ;
return V_5 ;
}
static int F_27 ( struct V_1 * V_2 ,
int V_100 , int V_101 )
{
int V_5 ;
T_1 V_24 ;
F_8 ( & V_2 -> V_18 -> V_21 , L_22 , V_22 , V_100 , V_101 ) ;
switch ( V_100 ) {
case V_102 :
V_5 = F_12 ( V_2 , V_103 , & V_24 ) ;
if ( V_5 )
goto V_12;
if ( V_101 )
V_24 &= 0xbf ;
else
V_24 |= 0x40 ;
V_5 = F_11 ( V_2 , V_103 , V_24 ) ;
if ( V_5 )
goto V_12;
break;
default:
V_5 = - V_55 ;
goto V_12;
}
return 0 ;
V_12:
F_8 ( & V_2 -> V_18 -> V_21 , L_1 , V_22 , V_5 ) ;
return V_5 ;
}
static int F_28 ( struct V_1 * V_2 ,
int V_100 , int V_101 )
{
int V_5 ;
T_1 V_24 ;
F_8 ( & V_2 -> V_18 -> V_21 , L_22 , V_22 , V_100 , V_101 ) ;
switch ( V_100 ) {
case V_104 :
if ( V_101 )
V_24 = ( 1 << 4 ) ;
else
V_24 = ( 0 << 4 ) ;
V_5 = F_13 ( V_2 , V_103 , V_24 , 0x10 ) ;
if ( V_5 )
goto V_12;
break;
case V_105 :
if ( V_101 )
V_24 = ( 1 << 1 ) ;
else
V_24 = ( 0 << 1 ) ;
V_5 = F_13 ( V_2 , V_103 , V_24 , 0x02 ) ;
if ( V_5 )
goto V_12;
break;
}
return 0 ;
V_12:
F_8 ( & V_2 -> V_18 -> V_21 , L_1 , V_22 , V_5 ) ;
return V_5 ;
}
static int F_29 ( struct V_1 * V_2 , int V_100 , int V_101 )
{
struct V_39 * V_40 = V_2 -> V_40 ;
switch ( V_40 -> V_64 ) {
case V_80 :
return F_27 ( V_2 , V_100 , V_101 ) ;
case V_86 :
return F_28 ( V_2 , V_100 , V_101 ) ;
default:
break;
}
return 0 ;
}
static int F_30 ( void * V_106 , int V_107 ,
int V_100 , int V_101 )
{
struct V_34 * V_35 = V_106 ;
struct V_1 * V_2 = F_15 ( V_35 ) ;
F_8 ( & V_2 -> V_18 -> V_21 , L_23 ,
V_22 , V_107 , V_100 , V_101 ) ;
switch ( V_107 ) {
case V_108 :
return F_29 ( V_2 , V_100 , V_101 ) ;
default:
break;
}
return 0 ;
}
static int F_31 ( struct V_90 * V_35 )
{
int V_5 ;
struct V_1 * V_2 = F_24 ( V_35 ) ;
struct V_39 * V_40 = F_20 ( V_2 ) ;
struct V_109 * V_109 ;
F_8 ( & V_2 -> V_18 -> V_21 , L_3 , V_22 ) ;
switch ( V_40 -> V_64 ) {
case V_80 :
V_109 = & V_110 ;
break;
case V_81 :
V_109 = & V_111 ;
break;
case V_83 :
V_109 = & V_110 ;
break;
case V_86 :
V_109 = & V_112 ;
break;
case V_88 :
V_109 = & V_113 ;
break;
default:
F_25 ( & V_2 -> V_18 -> V_21 , L_21 ,
V_95 , V_40 -> V_63 ) ;
V_5 = - V_96 ;
goto V_12;
}
V_35 -> V_97 [ 0 ] = F_26 ( V_114 , V_109 , & V_2 -> V_99 ) ;
if ( ! V_35 -> V_97 [ 0 ] ) {
V_5 = - V_96 ;
goto V_12;
}
V_35 -> V_97 [ 0 ] -> V_115 = F_30 ;
return 0 ;
V_12:
F_8 ( & V_2 -> V_18 -> V_21 , L_1 , V_22 , V_5 ) ;
return V_5 ;
}
static int F_32 ( struct V_90 * V_35 )
{
int V_5 ;
struct V_1 * V_2 = F_24 ( V_35 ) ;
struct V_39 * V_40 = F_20 ( V_2 ) ;
struct V_34 * V_116 ;
struct V_117 * V_97 ;
F_8 ( & V_2 -> V_18 -> V_21 , L_3 , V_22 ) ;
V_116 = F_33 ( V_35 -> V_97 [ 0 ] ) ;
switch ( V_40 -> V_64 ) {
case V_65 :
V_97 = F_26 ( V_118 , V_35 -> V_97 [ 0 ] ,
V_116 , & V_119 ) ;
break;
case V_67 :
V_97 = F_26 ( V_120 , V_35 -> V_97 [ 0 ] ,
V_116 , & V_121 ,
1220 ) ;
break;
case V_68 :
V_97 = F_26 ( V_122 , V_35 -> V_97 [ 0 ] ,
V_116 , & V_123 ) ;
break;
default:
V_97 = NULL ;
F_25 ( & V_2 -> V_18 -> V_21 , L_24 , V_95 ,
V_40 -> V_64 ) ;
}
if ( V_97 == NULL ) {
V_5 = - V_96 ;
goto V_12;
}
return 0 ;
V_12:
F_8 ( & V_2 -> V_18 -> V_21 , L_1 , V_22 , V_5 ) ;
return V_5 ;
}
static int F_34 ( struct V_90 * V_35 )
{
int V_5 ;
struct V_1 * V_2 = F_24 ( V_35 ) ;
struct V_39 * V_40 = F_20 ( V_2 ) ;
struct V_117 * V_97 ;
F_8 ( & V_2 -> V_18 -> V_21 , L_3 , V_22 ) ;
switch ( V_40 -> V_64 ) {
case V_80 :
V_97 = F_26 ( V_124 , V_35 -> V_97 [ 0 ] ,
& V_2 -> V_99 , & V_125 ) ;
V_35 -> V_97 [ 0 ] -> V_126 . V_127 =
V_35 -> V_97 [ 0 ] -> V_126 . V_128 . V_129 ;
return 0 ;
break;
case V_81 :
V_97 = F_26 ( V_130 , V_35 -> V_97 [ 0 ] ,
& V_2 -> V_99 , 0xc6 >> 1 , 0 , V_131 ) ;
V_35 -> V_97 [ 0 ] -> V_126 . V_127 =
V_35 -> V_97 [ 0 ] -> V_126 . V_128 . V_129 ;
return 0 ;
case V_88 :
V_97 = F_26 ( V_132 , V_35 -> V_97 [ 0 ] , & V_2 -> V_99 ,
& V_133 ) ;
break;
case V_83 :
V_97 = F_26 ( V_134 , V_35 -> V_97 [ 0 ] , & V_2 -> V_99 ,
& V_135 ) ;
break;
case V_86 :
V_5 = F_13 ( V_2 , V_61 , 0x00 , 0x12 ) ;
if ( V_5 )
goto V_12;
V_5 = F_13 ( V_2 , V_62 , 0x12 , 0x12 ) ;
if ( V_5 )
goto V_12;
V_97 = F_26 ( V_136 , V_35 -> V_97 [ 0 ] , & V_2 -> V_99 ,
& V_137 ) ;
break;
default:
V_97 = NULL ;
F_25 ( & V_2 -> V_18 -> V_21 , L_24 , V_95 ,
V_40 -> V_64 ) ;
}
if ( V_97 == NULL ) {
V_5 = - V_96 ;
goto V_12;
}
return 0 ;
V_12:
F_8 ( & V_2 -> V_18 -> V_21 , L_1 , V_22 , V_5 ) ;
return V_5 ;
}
static int F_35 ( struct V_1 * V_2 )
{
int V_5 ;
T_1 V_24 ;
F_8 ( & V_2 -> V_18 -> V_21 , L_3 , V_22 ) ;
V_5 = F_12 ( V_2 , V_138 , & V_24 ) ;
if ( V_5 )
goto V_12;
V_24 |= 0x09 ;
V_5 = F_11 ( V_2 , V_138 , V_24 ) ;
if ( V_5 )
goto V_12;
V_5 = F_9 ( V_2 , V_139 , L_25 , 4 ) ;
if ( V_5 )
goto V_12;
V_5 = F_9 ( V_2 , V_140 , L_26 , 4 ) ;
if ( V_5 )
goto V_12;
return V_5 ;
V_12:
F_8 ( & V_2 -> V_18 -> V_21 , L_1 , V_22 , V_5 ) ;
return V_5 ;
}
static int F_36 ( struct V_1 * V_2 , int V_141 )
{
int V_5 ;
T_1 V_142 , V_143 , V_144 [ 2 ] ;
F_8 ( & V_2 -> V_18 -> V_21 , L_27 , V_22 , V_141 ) ;
V_5 = F_12 ( V_2 , V_145 , & V_143 ) ;
if ( V_5 )
goto V_12;
V_5 = F_12 ( V_2 , V_103 , & V_142 ) ;
if ( V_5 )
goto V_12;
F_8 ( & V_2 -> V_18 -> V_21 , L_28 , V_22 ,
V_143 , V_142 ) ;
if ( V_141 ) {
V_142 |= 0x01 ;
V_142 &= ( ~ 0x10 ) ;
V_142 |= 0x04 ;
V_143 = V_143 & 0x0f ;
V_143 |= 0xe0 ;
V_144 [ 0 ] = 0x00 ;
V_144 [ 1 ] = 0x00 ;
} else {
V_142 &= ( ~ 0x01 ) ;
V_142 |= 0x10 ;
V_142 &= ( ~ 0x04 ) ;
V_143 = V_143 & ( ~ 0xc0 ) ;
V_144 [ 0 ] = 0x10 ;
V_144 [ 1 ] = 0x02 ;
}
F_8 ( & V_2 -> V_18 -> V_21 , L_29 , V_22 ,
V_143 , V_142 ) ;
V_5 = F_11 ( V_2 , V_145 , V_143 ) ;
if ( V_5 )
goto V_12;
V_5 = F_11 ( V_2 , V_103 , V_142 ) ;
if ( V_5 )
goto V_12;
V_5 = F_9 ( V_2 , V_146 , V_144 , 2 ) ;
if ( V_5 )
goto V_12;
if ( V_141 )
F_37 ( V_2 -> V_18 , F_38 ( V_2 -> V_18 , 0x81 ) ) ;
return V_5 ;
V_12:
F_8 ( & V_2 -> V_18 -> V_21 , L_1 , V_22 , V_5 ) ;
return V_5 ;
}
static int F_39 ( struct V_1 * V_2 , int V_141 )
{
int V_5 ;
T_1 V_24 ;
F_8 ( & V_2 -> V_18 -> V_21 , L_27 , V_22 , V_141 ) ;
if ( V_141 ) {
V_5 = F_12 ( V_2 , V_103 , & V_24 ) ;
if ( V_5 )
goto V_12;
V_24 |= 0x08 ;
V_24 &= 0xef ;
V_5 = F_11 ( V_2 , V_103 , V_24 ) ;
if ( V_5 )
goto V_12;
V_5 = F_12 ( V_2 , V_147 , & V_24 ) ;
if ( V_5 )
goto V_12;
V_24 &= 0xef ;
V_5 = F_11 ( V_2 , V_147 , V_24 ) ;
if ( V_5 )
goto V_12;
V_5 = F_12 ( V_2 , V_148 , & V_24 ) ;
if ( V_5 )
goto V_12;
V_24 |= 0x80 ;
V_5 = F_11 ( V_2 , V_148 , V_24 ) ;
if ( V_5 )
goto V_12;
V_5 = F_12 ( V_2 , V_148 , & V_24 ) ;
if ( V_5 )
goto V_12;
V_24 |= 0x20 ;
V_5 = F_11 ( V_2 , V_148 , V_24 ) ;
if ( V_5 )
goto V_12;
F_40 ( 5 ) ;
V_5 = F_12 ( V_2 , V_148 , & V_24 ) ;
if ( V_5 )
goto V_12;
V_24 |= 0x48 ;
V_5 = F_11 ( V_2 , V_148 , V_24 ) ;
if ( V_5 )
goto V_12;
V_5 = F_9 ( V_2 , V_146 , L_30 , 2 ) ;
if ( V_5 )
goto V_12;
V_5 = F_37 ( V_2 -> V_18 , F_38 ( V_2 -> V_18 , 0x81 ) ) ;
if ( V_5 )
goto V_12;
} else {
V_5 = F_12 ( V_2 , V_147 , & V_24 ) ;
if ( V_5 )
goto V_12;
V_24 |= 0x0c ;
V_5 = F_11 ( V_2 , V_147 , V_24 ) ;
if ( V_5 )
goto V_12;
V_5 = F_12 ( V_2 , V_103 , & V_24 ) ;
if ( V_5 )
goto V_12;
V_24 |= 0x10 ;
V_5 = F_11 ( V_2 , V_103 , V_24 ) ;
if ( V_5 )
goto V_12;
V_5 = F_12 ( V_2 , V_148 , & V_24 ) ;
if ( V_5 )
goto V_12;
V_24 &= 0x37 ;
V_5 = F_11 ( V_2 , V_148 , V_24 ) ;
if ( V_5 )
goto V_12;
V_5 = F_9 ( V_2 , V_146 , L_31 , 2 ) ;
if ( V_5 )
goto V_12;
}
return V_5 ;
V_12:
F_8 ( & V_2 -> V_18 -> V_21 , L_1 , V_22 , V_5 ) ;
return V_5 ;
}
static int F_41 ( struct V_1 * V_2 )
{
int V_5 , V_149 ;
struct V_39 * V_40 = V_2 -> V_40 ;
T_1 V_8 [ 5 ] ;
T_3 V_150 ;
struct V_151 V_152 [] = {
{ 0x3033 , 0x80 } ,
{ 0x3020 , 0x43 } ,
{ 0x3021 , 0x16 } ,
{ 0x3022 , 0x16 } ,
{ 0x3023 , 0x5a } ,
{ 0x3024 , 0x2d } ,
{ 0x3025 , 0x16 } ,
{ 0x3026 , 0x01 } ,
{ 0x3028 , 0xb0 } ,
{ 0x3029 , 0x04 } ,
{ 0x302c , 0x88 } ,
{ 0x302e , 0x13 } ,
{ 0x3030 , 0xdf } ,
{ 0x3031 , 0x05 } ,
} ;
if ( ! V_40 -> V_153 ) {
for ( V_149 = 0 ; V_149 < F_42 ( V_152 ) ; V_149 ++ ) {
V_5 = F_11 ( V_2 , V_152 [ V_149 ] . V_23 ,
V_152 [ V_149 ] . V_24 ) ;
if ( V_5 )
goto V_12;
}
V_40 -> V_153 = true ;
}
V_5 = F_10 ( V_2 , V_154 , V_8 , 5 ) ;
if ( V_5 )
goto V_12;
if ( V_8 [ 4 ] & 0x01 ) {
if ( V_8 [ 2 ] == ( T_1 ) ~ V_8 [ 3 ] ) {
if ( V_8 [ 0 ] == ( T_1 ) ~ V_8 [ 1 ] ) {
V_150 = V_8 [ 0 ] << 8 | V_8 [ 2 ] ;
} else {
V_150 = V_8 [ 0 ] << 16 |
V_8 [ 1 ] << 8 | V_8 [ 2 ] ;
}
} else {
V_150 = V_8 [ 0 ] << 24 | V_8 [ 1 ] << 16 |
V_8 [ 2 ] << 8 | V_8 [ 3 ] ;
}
F_43 ( V_2 -> V_155 , V_150 , 0 ) ;
V_5 = F_11 ( V_2 , V_156 , 1 ) ;
if ( V_5 )
goto V_12;
V_5 = F_11 ( V_2 , V_156 , 1 ) ;
if ( V_5 )
goto V_12;
}
return V_5 ;
V_12:
F_8 ( & V_2 -> V_18 -> V_21 , L_1 , V_22 , V_5 ) ;
return V_5 ;
}
static int F_44 ( struct V_1 * V_2 ,
struct V_157 * V_158 )
{
V_158 -> V_159 = V_160 ;
V_158 -> V_161 = V_162 ;
V_158 -> V_163 = F_41 ;
V_158 -> V_164 = 400 ;
return 0 ;
}
static int F_45 ( struct V_1 * V_2 )
{
int V_5 , V_149 ;
struct V_39 * V_40 = V_2 -> V_40 ;
T_1 V_8 [ 128 ] ;
int V_25 ;
struct V_151 V_152 [] = {
{ V_165 , 0x20 } ,
{ V_166 , 0x80 } ,
{ V_167 , 0xff } ,
{ V_168 , 0xff } ,
{ V_169 , 0xd0 } ,
{ V_170 , 0x07 } ,
{ V_171 , 0xc0 } ,
{ V_172 , 0x00 } ,
{ V_173 , 0x03 } ,
{ V_174 , 0x09 } ,
{ V_175 , 0x1c } ,
{ V_176 , 0x1e } ,
{ V_177 , 0x1e } ,
{ V_165 , 0x80 } ,
} ;
if ( ! V_40 -> V_153 ) {
for ( V_149 = 0 ; V_149 < F_42 ( V_152 ) ; V_149 ++ ) {
V_5 = F_11 ( V_2 , V_152 [ V_149 ] . V_23 ,
V_152 [ V_149 ] . V_24 ) ;
if ( V_5 )
goto V_12;
}
V_40 -> V_153 = true ;
}
V_5 = F_12 ( V_2 , V_167 , & V_8 [ 0 ] ) ;
if ( V_5 )
goto V_12;
if ( V_8 [ 0 ] != 0x83 )
goto exit;
V_5 = F_12 ( V_2 , V_178 , & V_8 [ 0 ] ) ;
if ( V_5 )
goto V_12;
V_25 = V_8 [ 0 ] ;
V_5 = F_10 ( V_2 , V_179 , V_8 , V_25 ) ;
V_5 = F_11 ( V_2 , V_167 , 0x03 ) ;
V_5 = F_11 ( V_2 , V_166 , 0x80 ) ;
V_5 = F_11 ( V_2 , V_165 , 0x80 ) ;
exit:
return V_5 ;
V_12:
F_8 ( & V_2 -> V_18 -> V_21 , L_1 , V_22 , V_5 ) ;
return V_5 ;
}
static int F_46 ( struct V_1 * V_2 ,
struct V_157 * V_158 )
{
V_158 -> V_159 = V_160 ;
V_158 -> V_161 = V_162 ;
V_158 -> V_163 = F_45 ;
V_158 -> V_164 = 400 ;
return 0 ;
}
