static inline struct V_1 * F_1 ( struct V_1 * V_2 )
{
if ( V_2 )
F_2 ( & V_2 -> V_3 ) ;
return V_2 ;
}
static void F_3 ( struct V_1 * V_2 )
{
unsigned long V_4 ;
if ( F_4 ( ! V_2 ) )
return;
F_5 ( & V_5 , V_4 ) ;
if ( F_6 ( & V_2 -> V_3 ) ) {
struct V_6 * V_7 = V_2 -> V_8 ;
V_7 -> V_9 -- ;
F_7 ( V_7 -> V_10 ) ;
F_8 ( & V_5 , V_4 ) ;
F_9 ( V_2 ) ;
return;
}
F_8 ( & V_5 , V_4 ) ;
if ( F_10 ( & V_2 -> V_11 ) )
F_11 ( & V_2 -> V_11 ) ;
}
int F_12 ( int V_12 , struct V_6 * V_13 )
{
unsigned long V_4 ;
int V_14 = 0 ;
if ( V_12 < V_15 || V_12 >= V_16 )
return - V_17 ;
F_5 ( & V_5 , V_4 ) ;
V_18 [ V_12 ] = V_13 ;
V_13 -> V_19 = V_12 ;
V_13 -> V_9 = 0 ;
F_8 ( & V_5 , V_4 ) ;
return V_14 ;
}
int F_13 ( int V_12 )
{
unsigned long V_4 ;
int V_14 = 0 ;
if ( V_12 < V_15 || V_12 >= V_16 )
return - V_17 ;
F_5 ( & V_5 , V_4 ) ;
if ( V_18 [ V_12 ] -> V_9 )
V_14 = - V_20 ;
else
V_18 [ V_12 ] = NULL ;
F_8 ( & V_5 , V_4 ) ;
return V_14 ;
}
static struct V_6 * F_14 ( int V_12 )
{
unsigned long V_4 ;
struct V_6 * V_21 , * V_14 ;
F_5 ( & V_5 , V_4 ) ;
V_14 = F_15 ( - V_17 ) ;
V_21 = V_18 [ V_12 ] ;
if ( V_21 ) {
V_14 = F_15 ( - V_22 ) ;
if ( F_16 ( V_21 -> V_10 ) ) {
V_21 -> V_9 ++ ;
V_14 = V_21 ;
}
}
F_8 ( & V_5 , V_4 ) ;
return V_14 ;
}
static void F_17 ( struct V_6 * V_21 )
{
unsigned long V_4 ;
F_5 ( & V_5 , V_4 ) ;
V_21 -> V_9 -- ;
F_7 ( V_21 -> V_10 ) ;
F_8 ( & V_5 , V_4 ) ;
}
static struct V_1 * F_18 ( int V_12 )
{
struct V_1 * V_2 ;
struct V_6 * V_21 ;
if ( V_12 < V_15 || V_12 >= V_16 )
return F_15 ( - V_17 ) ;
V_21 = F_14 ( V_12 ) ;
if ( F_19 ( V_21 ) ) {
F_20 ( L_1 , V_12 ) ;
V_21 = F_14 ( V_12 ) ;
if ( F_19 ( V_21 ) )
return F_21 ( V_21 ) ;
}
V_2 = F_22 ( sizeof( struct V_1 ) , V_23 ) ;
if ( V_2 == NULL ) {
F_17 ( V_21 ) ;
return F_15 ( - V_24 ) ;
}
V_2 -> V_8 = V_21 ;
F_23 ( & V_2 -> V_3 , 1 ) ;
F_24 ( & V_2 -> V_11 ) ;
return V_2 ;
}
static void * F_25 ( struct V_25 * V_26 , T_1 * V_27 )
{
return ( * V_27 < V_16 ) ? V_27 : NULL ;
}
static void * F_26 ( struct V_25 * V_26 , void * V_28 , T_1 * V_27 )
{
( * V_27 ) ++ ;
return ( * V_27 < V_16 ) ? V_27 : NULL ;
}
static void F_27 ( struct V_25 * V_26 , void * V_28 )
{
}
static int F_28 ( struct V_25 * V_26 , void * V_28 )
{
int V_29 = * ( T_1 * ) V_28 ;
struct V_6 * V_21 ;
V_21 = F_14 ( V_29 ) ;
if ( F_19 ( V_21 ) )
return 0 ;
F_29 ( V_26 , L_2 , V_21 -> V_30 ? V_21 -> V_30 : L_3 , V_29 ) ;
F_17 ( V_21 ) ;
return 0 ;
}
static int F_30 ( struct V_31 * V_31 , struct V_32 * V_32 )
{
return F_31 ( V_32 , & V_33 ) ;
}
static void F_32 ( struct V_34 * V_35 , struct V_1 * V_2 )
{
V_35 -> V_36 = V_2 ;
}
static struct V_1 * F_33 ( struct V_34 * V_35 )
{
unsigned long V_4 ;
struct V_1 * V_2 ;
F_5 ( & V_5 , V_4 ) ;
V_2 = NULL ;
if ( F_34 ( V_37 , & V_35 -> V_4 ) )
V_2 = F_1 ( V_35 -> V_36 ) ;
F_8 ( & V_5 , V_4 ) ;
return V_2 ;
}
struct V_1 * F_35 ( struct V_34 * V_35 )
{
struct V_1 * V_2 ;
F_36 ( V_38 , ( V_2 = F_33 ( V_35 ) ) != NULL ) ;
return V_2 ;
}
struct V_1 * F_37 ( struct V_34 * V_35 )
{
return F_33 ( V_35 ) ;
}
void F_38 ( struct V_1 * V_2 )
{
F_3 ( V_2 ) ;
}
static inline void F_39 ( struct V_1 * V_2 )
{
F_3 ( V_2 ) ;
}
void F_40 ( struct V_34 * V_35 )
{
F_41 ( V_37 , & V_35 -> V_4 ) ;
F_42 ( V_39 , & V_35 -> V_4 ) ;
F_11 ( & V_38 ) ;
}
void F_43 ( struct V_34 * V_35 )
{
struct V_1 * V_2 = F_37 ( V_35 ) ;
if ( V_2 ) {
if ( V_2 -> V_8 -> V_40 )
V_2 -> V_8 -> V_40 ( V_35 ) ;
F_38 ( V_2 ) ;
}
F_44 ( V_35 ) ;
}
static void F_45 ( struct V_34 * V_35 , int V_19 )
{
F_46 ( & V_35 -> V_41 ) ;
V_35 -> V_42 . V_43 = V_19 ;
F_47 ( & V_35 -> V_41 ) ;
}
static int F_48 ( struct V_34 * V_35 , struct V_1 * V_2 )
{
F_49 ( F_50 ( V_44 , & V_35 -> V_4 ) ) ;
if ( V_2 -> V_8 -> V_45 ) {
int V_14 ;
V_14 = V_2 -> V_8 -> V_45 ( V_35 ) ;
if ( V_14 )
F_42 ( V_44 , & V_35 -> V_4 ) ;
return V_14 ;
}
return 0 ;
}
static void F_51 ( struct V_34 * V_35 , struct V_1 * V_2 )
{
F_49 ( ! F_34 ( V_44 , & V_35 -> V_4 ) ) ;
F_42 ( V_44 , & V_35 -> V_4 ) ;
if ( V_2 -> V_8 -> V_46 )
V_2 -> V_8 -> V_46 ( V_35 ) ;
}
static void F_52 ( struct V_34 * V_35 , struct V_1 * V_47 )
{
char V_48 [ 64 ] ;
struct V_1 * V_13 ;
int V_49 ;
V_47 = F_18 ( V_47 -> V_8 -> V_19 ) ;
F_49 ( F_19 ( V_47 ) ) ;
F_32 ( V_35 , V_47 ) ;
F_45 ( V_35 , V_47 -> V_8 -> V_19 ) ;
if ( F_48 ( V_35 , V_47 ) < 0 ) {
F_39 ( V_47 ) ;
V_13 = F_18 ( V_15 ) ;
if ( F_19 ( V_13 ) )
F_53 ( L_4 ) ;
F_32 ( V_35 , V_13 ) ;
F_45 ( V_35 , V_15 ) ;
V_49 = F_48 ( V_35 , V_13 ) ;
if ( V_49 < 0 )
F_53 ( L_5
L_6 ,
F_54 ( V_35 , V_48 ) , V_49 ) ;
}
}
static int F_55 ( struct V_34 * V_35 )
{
F_42 ( V_37 , & V_35 -> V_4 ) ;
return F_56 ( & V_35 -> V_50 -> V_48 . V_51 ) ;
}
static void F_57 ( struct V_34 * V_35 )
{
F_58 ( & V_35 -> V_52 ) ;
F_58 ( & V_35 -> V_53 ) ;
F_58 ( & V_35 -> V_50 -> V_48 . V_51 ) ;
}
static int F_59 ( struct V_34 * V_35 , long V_54 )
{
long V_14 ;
V_14 = F_60 ( V_35 -> V_36 -> V_11 ,
F_61 ( & V_35 -> V_36 -> V_3 ) == 1 , V_54 ) ;
return V_14 > 0 ? 0 : - V_20 ;
}
int F_62 ( struct V_34 * V_35 , int V_36 )
{
int V_55 ;
struct V_1 * V_56 , * V_13 ;
int V_51 , V_57 = 0 ;
struct V_34 * V_58 ;
V_13 = F_18 ( V_36 ) ;
if ( F_19 ( V_13 ) )
return F_63 ( V_13 ) ;
F_64 ( V_35 ) ;
V_58 = V_35 -> V_59 ;
if ( V_35 -> V_36 -> V_8 -> V_19 == V_36 ) {
F_65 ( V_35 ) ;
F_39 ( V_13 ) ;
return 0 ;
}
F_65 ( V_35 ) ;
F_66 ( V_35 , 0 ) ;
F_64 ( V_35 ) ;
F_46 ( & V_35 -> V_60 ) ;
while ( F_34 ( V_39 , & V_35 -> V_4 ) ) {
F_47 ( & V_35 -> V_60 ) ;
F_65 ( V_35 ) ;
F_36 ( V_38 ,
F_34 ( V_39 , & V_35 -> V_4 ) == 0 ) ;
F_64 ( V_35 ) ;
F_46 ( & V_35 -> V_60 ) ;
}
F_41 ( V_39 , & V_35 -> V_4 ) ;
V_35 -> V_61 = 0 ;
V_56 = V_35 -> V_36 ;
F_65 ( V_35 ) ;
V_51 = F_55 ( V_35 ) ;
if ( V_58 )
V_57 = F_55 ( V_58 ) ;
F_47 ( & V_35 -> V_60 ) ;
F_57 ( V_35 ) ;
V_55 = F_59 ( V_35 , 5 * V_62 ) ;
F_64 ( V_35 ) ;
F_46 ( & V_35 -> V_60 ) ;
if ( V_55 ) {
F_39 ( V_13 ) ;
goto V_63;
}
if ( F_34 ( V_64 , & V_35 -> V_4 ) ) {
F_42 ( V_39 , & V_35 -> V_4 ) ;
F_47 ( & V_35 -> V_60 ) ;
F_39 ( V_13 ) ;
F_65 ( V_35 ) ;
return - V_65 ;
}
F_51 ( V_35 , V_56 ) ;
F_32 ( V_35 , V_13 ) ;
F_45 ( V_35 , V_36 ) ;
V_55 = F_48 ( V_35 , V_13 ) ;
if ( V_55 < 0 ) {
F_39 ( V_13 ) ;
F_52 ( V_35 , V_56 ) ;
}
if ( V_35 -> V_36 -> V_8 -> V_19 != V_56 -> V_8 -> V_19 && V_35 -> V_8 -> V_66 )
V_35 -> V_8 -> V_66 ( V_35 ) ;
F_39 ( V_56 ) ;
V_63:
F_40 ( V_35 ) ;
if ( V_58 )
F_40 ( V_58 ) ;
if ( V_51 )
F_67 ( & V_35 -> V_50 -> V_48 . V_51 ) ;
if ( V_57 )
F_67 ( & V_58 -> V_50 -> V_48 . V_51 ) ;
F_47 ( & V_35 -> V_60 ) ;
F_65 ( V_35 ) ;
return V_55 ;
}
static void F_68 ( struct V_34 * V_35 )
{
F_46 ( & V_35 -> V_41 ) ;
V_35 -> V_42 = V_35 -> V_67 -> V_68 ;
V_35 -> V_42 . V_69 = F_69 ( & V_35 -> V_42 ) ;
V_35 -> V_42 . V_70 = F_70 ( & V_35 -> V_42 ) ;
F_47 ( & V_35 -> V_41 ) ;
}
static int F_71 ( struct V_34 * V_35 , int V_36 )
{
struct V_1 * V_2 = F_18 ( V_36 ) ;
if ( F_19 ( V_2 ) )
return - 1 ;
F_51 ( V_35 , V_35 -> V_36 ) ;
F_39 ( V_35 -> V_36 ) ;
V_35 -> V_36 = NULL ;
F_32 ( V_35 , V_2 ) ;
F_45 ( V_35 , V_36 ) ;
return 0 ;
}
void F_72 ( struct V_34 * V_35 )
{
struct V_1 * V_2 ;
int V_71 = V_35 -> V_67 -> V_4 & V_72 ;
int V_73 = 0 ;
V_2 = F_37 ( V_35 ) ;
if ( V_2 != NULL ) {
if ( V_2 -> V_8 -> V_40 )
V_2 -> V_8 -> V_40 ( V_35 ) ;
F_73 ( V_35 ) ;
if ( ( F_34 ( V_74 , & V_35 -> V_4 ) ) &&
V_2 -> V_8 -> V_75 )
V_2 -> V_8 -> V_75 ( V_35 ) ;
if ( V_2 -> V_8 -> V_76 )
V_2 -> V_8 -> V_76 ( V_35 ) ;
F_38 ( V_2 ) ;
}
F_74 ( & V_35 -> V_77 , V_78 ) ;
F_74 ( & V_35 -> V_79 , V_80 ) ;
F_46 ( & V_35 -> V_60 ) ;
F_42 ( V_37 , & V_35 -> V_4 ) ;
F_65 ( V_35 ) ;
F_56 ( & V_35 -> V_50 -> V_48 . V_51 ) ;
F_47 ( & V_35 -> V_60 ) ;
V_81:
F_64 ( V_35 ) ;
F_46 ( & V_35 -> V_60 ) ;
if ( V_35 -> V_36 ) {
if ( F_61 ( & V_35 -> V_36 -> V_3 ) != 1 ) {
char V_82 [ V_83 ] , V_84 [ 64 ] ;
long V_54 = 3 * V_62 ;
F_65 ( V_35 ) ;
while ( F_59 ( V_35 , V_54 ) == - V_20 ) {
V_54 = V_85 ;
F_75 ( V_86
L_7 ,
V_87 , F_76 ( V_82 , V_88 ) ,
F_54 ( V_35 , V_84 ) ) ;
}
F_47 ( & V_35 -> V_60 ) ;
goto V_81;
}
if ( V_71 == 0 ) {
if ( ! F_71 ( V_35 , V_35 -> V_42 . V_43 ) )
V_73 = F_48 ( V_35 , V_35 -> V_36 ) ;
else
V_73 = 1 ;
}
if ( V_71 || V_73 ) {
F_77 ( F_71 ( V_35 , V_15 ) ) ;
F_49 ( F_48 ( V_35 , V_35 -> V_36 ) ) ;
}
F_40 ( V_35 ) ;
}
F_47 ( & V_35 -> V_60 ) ;
if ( V_71 )
F_68 ( V_35 ) ;
}
int F_78 ( struct V_34 * V_35 , struct V_34 * V_58 )
{
struct V_1 * V_2 = V_35 -> V_36 ;
int V_55 ;
V_55 = F_48 ( V_35 , V_2 ) ;
if ( V_55 )
return V_55 ;
if ( V_58 ) {
V_55 = F_48 ( V_58 , V_58 -> V_36 ) ;
if ( V_55 ) {
F_51 ( V_35 , V_2 ) ;
return V_55 ;
}
F_40 ( V_58 ) ;
}
F_40 ( V_35 ) ;
return 0 ;
}
static void F_79 ( struct V_34 * V_35 )
{
F_59 ( V_35 , V_85 ) ;
F_46 ( & V_35 -> V_60 ) ;
F_51 ( V_35 , V_35 -> V_36 ) ;
F_39 ( V_35 -> V_36 ) ;
V_35 -> V_36 = NULL ;
F_45 ( V_35 , V_15 ) ;
F_47 ( & V_35 -> V_60 ) ;
}
void F_80 ( struct V_34 * V_35 , struct V_34 * V_58 )
{
F_55 ( V_35 ) ;
if ( V_58 )
F_55 ( V_58 ) ;
F_57 ( V_35 ) ;
if ( V_58 )
F_57 ( V_58 ) ;
F_81 ( V_35 , V_58 ) ;
F_79 ( V_35 ) ;
if ( V_58 )
F_79 ( V_58 ) ;
F_82 ( V_35 , V_58 ) ;
}
void F_83 ( struct V_34 * V_35 )
{
struct V_1 * V_2 = F_18 ( V_15 ) ;
if ( F_19 ( V_2 ) )
F_53 ( L_8 ) ;
F_32 ( V_35 , V_2 ) ;
}
void F_84 ( struct V_34 * V_35 )
{
F_3 ( V_35 -> V_36 ) ;
F_32 ( V_35 , NULL ) ;
}
void F_85 ( void )
{
( void ) F_12 ( V_15 , & V_89 ) ;
}
