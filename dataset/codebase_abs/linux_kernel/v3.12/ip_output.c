void F_1 ( struct V_1 * V_2 )
{
V_2 -> V_3 = 0 ;
V_2 -> V_3 = F_2 ( ( unsigned char * ) V_2 , V_2 -> V_4 ) ;
}
int F_3 ( struct V_5 * V_6 )
{
struct V_1 * V_2 = F_4 ( V_6 ) ;
V_2 -> V_7 = F_5 ( V_6 -> V_8 ) ;
F_1 ( V_2 ) ;
return F_6 ( V_9 , V_10 , V_6 , NULL ,
F_7 ( V_6 ) -> V_11 , V_12 ) ;
}
int F_8 ( struct V_5 * V_6 )
{
int V_13 ;
V_13 = F_3 ( V_6 ) ;
if ( F_9 ( V_13 == 1 ) )
V_13 = V_12 ( V_6 ) ;
return V_13 ;
}
static inline int F_10 ( struct V_14 * V_15 , struct V_16 * V_17 )
{
int V_18 = V_15 -> V_19 ;
if ( V_18 < 0 )
V_18 = F_11 ( V_17 ) ;
return V_18 ;
}
int F_12 ( struct V_5 * V_6 , struct V_20 * V_21 ,
T_1 V_22 , T_1 V_23 , struct V_24 * V_25 )
{
struct V_14 * V_15 = F_13 ( V_21 ) ;
struct V_26 * V_27 = F_14 ( V_6 ) ;
struct V_1 * V_2 ;
F_15 ( V_6 , sizeof( struct V_1 ) + ( V_25 ? V_25 -> V_25 . V_28 : 0 ) ) ;
F_16 ( V_6 ) ;
V_2 = F_4 ( V_6 ) ;
V_2 -> V_29 = 4 ;
V_2 -> V_4 = 5 ;
V_2 -> V_30 = V_15 -> V_30 ;
if ( F_17 ( V_21 , & V_27 -> V_17 ) )
V_2 -> V_31 = F_5 ( V_32 ) ;
else
V_2 -> V_31 = 0 ;
V_2 -> V_18 = F_10 ( V_15 , & V_27 -> V_17 ) ;
V_2 -> V_23 = ( V_25 && V_25 -> V_25 . V_33 ? V_25 -> V_25 . V_34 : V_23 ) ;
V_2 -> V_22 = V_22 ;
V_2 -> V_35 = V_21 -> V_36 ;
F_18 ( V_6 , & V_27 -> V_17 , V_21 ) ;
if ( V_25 && V_25 -> V_25 . V_28 ) {
V_2 -> V_4 += V_25 -> V_25 . V_28 >> 2 ;
F_19 ( V_6 , & V_25 -> V_25 , V_23 , V_27 , 0 ) ;
}
V_6 -> V_37 = V_21 -> V_38 ;
V_6 -> V_39 = V_21 -> V_40 ;
return F_8 ( V_6 ) ;
}
static inline int F_20 ( struct V_5 * V_6 )
{
struct V_16 * V_17 = F_7 ( V_6 ) ;
struct V_26 * V_27 = (struct V_26 * ) V_17 ;
struct V_41 * V_11 = V_17 -> V_11 ;
unsigned int V_42 = F_21 ( V_11 ) ;
struct V_43 * V_44 ;
T_2 V_45 ;
if ( V_27 -> V_46 == V_47 ) {
F_22 ( F_23 ( V_11 ) , V_48 , V_6 -> V_8 ) ;
} else if ( V_27 -> V_46 == V_49 )
F_22 ( F_23 ( V_11 ) , V_50 , V_6 -> V_8 ) ;
if ( F_24 ( F_25 ( V_6 ) < V_42 && V_11 -> V_51 ) ) {
struct V_5 * V_52 ;
V_52 = F_26 ( V_6 , F_21 ( V_11 ) ) ;
if ( V_52 == NULL ) {
F_27 ( V_6 ) ;
return - V_53 ;
}
if ( V_6 -> V_21 )
F_28 ( V_52 , V_6 -> V_21 ) ;
F_29 ( V_6 ) ;
V_6 = V_52 ;
}
F_30 () ;
V_45 = ( V_54 T_2 ) F_31 ( V_27 , F_4 ( V_6 ) -> V_23 ) ;
V_44 = F_32 ( V_11 , V_45 ) ;
if ( F_24 ( ! V_44 ) )
V_44 = F_33 ( & V_55 , & V_45 , V_11 , false ) ;
if ( ! F_34 ( V_44 ) ) {
int V_56 = F_35 ( V_17 , V_44 , V_6 ) ;
F_36 () ;
return V_56 ;
}
F_36 () ;
F_37 ( L_1 ,
V_57 ) ;
F_27 ( V_6 ) ;
return - V_58 ;
}
static int F_38 ( struct V_5 * V_6 )
{
#if F_39 ( V_59 ) && F_39 ( V_60 )
if ( F_7 ( V_6 ) -> V_61 != NULL ) {
F_40 ( V_6 ) -> V_62 |= V_63 ;
return V_12 ( V_6 ) ;
}
#endif
if ( V_6 -> V_8 > F_41 ( V_6 ) && ! F_42 ( V_6 ) )
return F_43 ( V_6 , F_20 ) ;
else
return F_20 ( V_6 ) ;
}
int F_44 ( struct V_5 * V_6 )
{
struct V_20 * V_21 = V_6 -> V_21 ;
struct V_26 * V_27 = F_14 ( V_6 ) ;
struct V_41 * V_11 = V_27 -> V_17 . V_11 ;
F_22 ( F_23 ( V_11 ) , V_64 , V_6 -> V_8 ) ;
V_6 -> V_11 = V_11 ;
V_6 -> V_35 = F_5 ( V_65 ) ;
if ( V_27 -> V_66 & V_67 ) {
if ( F_45 ( V_21 )
#ifdef F_46
&&
( ( V_27 -> V_66 & V_68 ) ||
! ( F_40 ( V_6 ) -> V_62 & V_69 ) )
#endif
) {
struct V_5 * V_70 = F_47 ( V_6 , V_71 ) ;
if ( V_70 )
F_48 ( V_9 , V_72 ,
V_70 , NULL , V_70 -> V_11 ,
V_73 ) ;
}
if ( F_4 ( V_6 ) -> V_18 == 0 ) {
F_27 ( V_6 ) ;
return 0 ;
}
}
if ( V_27 -> V_66 & V_74 ) {
struct V_5 * V_70 = F_47 ( V_6 , V_71 ) ;
if ( V_70 )
F_48 ( V_9 , V_72 , V_70 ,
NULL , V_70 -> V_11 , V_73 ) ;
}
return F_49 ( V_9 , V_72 , V_6 , NULL ,
V_6 -> V_11 , F_38 ,
! ( F_40 ( V_6 ) -> V_62 & V_63 ) ) ;
}
int F_50 ( struct V_5 * V_6 )
{
struct V_41 * V_11 = F_7 ( V_6 ) -> V_11 ;
F_22 ( F_23 ( V_11 ) , V_64 , V_6 -> V_8 ) ;
V_6 -> V_11 = V_11 ;
V_6 -> V_35 = F_5 ( V_65 ) ;
return F_49 ( V_9 , V_72 , V_6 , NULL , V_11 ,
F_38 ,
! ( F_40 ( V_6 ) -> V_62 & V_63 ) ) ;
}
static void F_51 ( struct V_1 * V_2 , const struct V_75 * V_76 )
{
F_52 ( F_53 ( F_54 ( * V_76 ) , V_23 ) !=
F_53 ( F_54 ( * V_76 ) , V_22 ) + sizeof( V_76 -> V_22 ) ) ;
memcpy ( & V_2 -> V_22 , & V_76 -> V_22 ,
sizeof( V_76 -> V_22 ) + sizeof( V_76 -> V_23 ) ) ;
}
int F_55 ( struct V_5 * V_6 , struct V_77 * V_78 )
{
struct V_20 * V_21 = V_6 -> V_21 ;
struct V_14 * V_15 = F_13 ( V_21 ) ;
struct V_24 * V_79 ;
struct V_75 * V_76 ;
struct V_26 * V_27 ;
struct V_1 * V_2 ;
int V_56 ;
F_56 () ;
V_79 = F_57 ( V_15 -> V_79 ) ;
V_76 = & V_78 -> V_80 . V_81 ;
V_27 = F_14 ( V_6 ) ;
if ( V_27 != NULL )
goto V_82;
V_27 = (struct V_26 * ) F_58 ( V_21 , 0 ) ;
if ( V_27 == NULL ) {
T_1 V_23 ;
V_23 = V_15 -> V_83 ;
if ( V_79 && V_79 -> V_25 . V_33 )
V_23 = V_79 -> V_25 . V_34 ;
V_27 = F_59 ( F_60 ( V_21 ) , V_76 , V_21 ,
V_23 , V_15 -> V_84 ,
V_15 -> V_85 ,
V_15 -> V_86 ,
V_21 -> V_36 ,
F_61 ( V_21 ) ,
V_21 -> V_87 ) ;
if ( F_34 ( V_27 ) )
goto V_88;
F_62 ( V_21 , & V_27 -> V_17 ) ;
}
F_63 ( V_6 , & V_27 -> V_17 ) ;
V_82:
if ( V_79 && V_79 -> V_25 . V_89 && V_27 -> V_90 )
goto V_88;
F_15 ( V_6 , sizeof( struct V_1 ) + ( V_79 ? V_79 -> V_25 . V_28 : 0 ) ) ;
F_16 ( V_6 ) ;
V_2 = F_4 ( V_6 ) ;
* ( ( V_91 * ) V_2 ) = F_5 ( ( 4 << 12 ) | ( 5 << 8 ) | ( V_15 -> V_30 & 0xff ) ) ;
if ( F_17 ( V_21 , & V_27 -> V_17 ) && ! V_6 -> V_92 )
V_2 -> V_31 = F_5 ( V_32 ) ;
else
V_2 -> V_31 = 0 ;
V_2 -> V_18 = F_10 ( V_15 , & V_27 -> V_17 ) ;
V_2 -> V_35 = V_21 -> V_36 ;
F_51 ( V_2 , V_76 ) ;
if ( V_79 && V_79 -> V_25 . V_28 ) {
V_2 -> V_4 += V_79 -> V_25 . V_28 >> 2 ;
F_19 ( V_6 , & V_79 -> V_25 , V_15 -> V_83 , V_27 , 0 ) ;
}
F_64 ( V_6 , & V_27 -> V_17 , V_21 ,
( F_65 ( V_6 ) -> V_93 ? : 1 ) - 1 ) ;
V_6 -> V_37 = V_21 -> V_38 ;
V_6 -> V_39 = V_21 -> V_40 ;
V_56 = F_8 ( V_6 ) ;
F_66 () ;
return V_56 ;
V_88:
F_66 () ;
F_67 ( F_60 ( V_21 ) , V_94 ) ;
F_27 ( V_6 ) ;
return - V_95 ;
}
static void F_68 ( struct V_5 * V_96 , struct V_5 * V_97 )
{
V_96 -> V_98 = V_97 -> V_98 ;
V_96 -> V_37 = V_97 -> V_37 ;
V_96 -> V_35 = V_97 -> V_35 ;
F_69 ( V_96 ) ;
F_70 ( V_96 , V_97 ) ;
V_96 -> V_11 = V_97 -> V_11 ;
V_96 -> V_39 = V_97 -> V_39 ;
F_40 ( V_96 ) -> V_62 = F_40 ( V_97 ) -> V_62 ;
#ifdef F_71
V_96 -> V_99 = V_97 -> V_99 ;
#endif
F_72 ( V_96 , V_97 ) ;
#if F_73 ( V_100 )
V_96 -> V_101 = V_97 -> V_101 ;
#endif
#if F_39 ( V_102 ) || F_39 ( V_103 )
V_96 -> V_104 = V_97 -> V_104 ;
#endif
F_74 ( V_96 , V_97 ) ;
}
int F_43 ( struct V_5 * V_6 , int (* F_75)( struct V_5 * ) )
{
struct V_1 * V_2 ;
int V_105 ;
struct V_41 * V_11 ;
struct V_5 * V_52 ;
unsigned int V_106 , V_107 , V_108 , V_8 , V_109 ;
int V_110 ;
V_91 V_111 ;
struct V_26 * V_27 = F_14 ( V_6 ) ;
int V_13 = 0 ;
V_11 = V_27 -> V_17 . V_11 ;
V_2 = F_4 ( V_6 ) ;
if ( F_24 ( ( ( V_2 -> V_31 & F_5 ( V_32 ) ) && ! V_6 -> V_92 ) ||
( F_40 ( V_6 ) -> V_112 &&
F_40 ( V_6 ) -> V_112 > F_76 ( & V_27 -> V_17 ) ) ) ) {
F_67 ( F_23 ( V_11 ) , V_113 ) ;
F_77 ( V_6 , V_114 , V_115 ,
F_78 ( F_41 ( V_6 ) ) ) ;
F_27 ( V_6 ) ;
return - V_116 ;
}
V_107 = V_2 -> V_4 * 4 ;
V_106 = F_76 ( & V_27 -> V_17 ) - V_107 ;
#ifdef F_79
if ( V_6 -> V_117 )
V_106 -= F_80 ( V_6 ) ;
#endif
F_40 ( V_6 ) -> V_62 |= V_118 ;
if ( F_81 ( V_6 ) ) {
struct V_5 * V_119 , * V_120 ;
int V_121 = F_82 ( V_6 ) ;
if ( V_121 - V_107 > V_106 ||
( ( V_121 - V_107 ) & 7 ) ||
F_83 ( V_2 ) ||
F_84 ( V_6 ) )
goto V_122;
F_85 (skb, frag) {
if ( V_119 -> V_8 > V_106 ||
( ( V_119 -> V_8 & 7 ) && V_119 -> V_123 ) ||
F_25 ( V_119 ) < V_107 )
goto V_124;
if ( F_86 ( V_119 ) )
goto V_124;
F_87 ( V_119 -> V_21 ) ;
if ( V_6 -> V_21 ) {
V_119 -> V_21 = V_6 -> V_21 ;
V_119 -> V_125 = V_126 ;
}
V_6 -> V_127 -= V_119 -> V_127 ;
}
V_13 = 0 ;
V_110 = 0 ;
V_119 = F_65 ( V_6 ) -> V_128 ;
F_88 ( V_6 ) ;
V_6 -> V_129 = V_121 - F_89 ( V_6 ) ;
V_6 -> V_8 = V_121 ;
V_2 -> V_7 = F_5 ( V_121 ) ;
V_2 -> V_31 = F_5 ( V_130 ) ;
F_1 ( V_2 ) ;
for (; ; ) {
if ( V_119 ) {
V_119 -> V_131 = V_132 ;
F_90 ( V_119 ) ;
F_91 ( V_119 , V_107 ) ;
F_16 ( V_119 ) ;
memcpy ( F_92 ( V_119 ) , V_2 , V_107 ) ;
V_2 = F_4 ( V_119 ) ;
V_2 -> V_7 = F_5 ( V_119 -> V_8 ) ;
F_68 ( V_119 , V_6 ) ;
if ( V_110 == 0 )
F_93 ( V_119 ) ;
V_110 += V_6 -> V_8 - V_107 ;
V_2 -> V_31 = F_5 ( V_110 >> 3 ) ;
if ( V_119 -> V_123 != NULL )
V_2 -> V_31 |= F_5 ( V_130 ) ;
F_1 ( V_2 ) ;
}
V_13 = F_75 ( V_6 ) ;
if ( ! V_13 )
F_67 ( F_23 ( V_11 ) , V_133 ) ;
if ( V_13 || ! V_119 )
break;
V_6 = V_119 ;
V_119 = V_6 -> V_123 ;
V_6 -> V_123 = NULL ;
}
if ( V_13 == 0 ) {
F_67 ( F_23 ( V_11 ) , V_134 ) ;
return 0 ;
}
while ( V_119 ) {
V_6 = V_119 -> V_123 ;
F_27 ( V_119 ) ;
V_119 = V_6 ;
}
F_67 ( F_23 ( V_11 ) , V_113 ) ;
return V_13 ;
V_124:
F_85 (skb, frag2) {
if ( V_120 == V_119 )
break;
V_120 -> V_21 = NULL ;
V_120 -> V_125 = NULL ;
V_6 -> V_127 += V_120 -> V_127 ;
}
}
V_122:
if ( ( V_6 -> V_131 == V_135 ) && F_94 ( V_6 ) )
goto V_136;
V_2 = F_4 ( V_6 ) ;
V_108 = V_6 -> V_8 - V_107 ;
V_105 = V_107 ;
V_109 = F_95 ( V_27 -> V_17 . V_11 , F_96 ( V_6 ) ) ;
V_110 = ( F_97 ( V_2 -> V_31 ) & V_137 ) << 3 ;
V_111 = V_2 -> V_31 & F_5 ( V_130 ) ;
while ( V_108 > 0 ) {
V_8 = V_108 ;
if ( V_8 > V_106 )
V_8 = V_106 ;
if ( V_8 < V_108 ) {
V_8 &= ~ 7 ;
}
if ( ( V_52 = F_98 ( V_8 + V_107 + V_109 , V_71 ) ) == NULL ) {
F_99 ( V_138 L_2 ) ;
V_13 = - V_53 ;
goto V_136;
}
F_68 ( V_52 , V_6 ) ;
F_100 ( V_52 , V_109 ) ;
F_101 ( V_52 , V_8 + V_107 ) ;
F_16 ( V_52 ) ;
V_52 -> V_139 = V_52 -> V_140 + V_107 ;
if ( V_6 -> V_21 )
F_28 ( V_52 , V_6 -> V_21 ) ;
F_102 ( V_6 , F_92 ( V_52 ) , V_107 ) ;
if ( F_103 ( V_6 , V_105 , F_104 ( V_52 ) , V_8 ) )
F_105 () ;
V_108 -= V_8 ;
V_2 = F_4 ( V_52 ) ;
V_2 -> V_31 = F_5 ( ( V_110 >> 3 ) ) ;
if ( V_110 == 0 )
F_93 ( V_6 ) ;
if ( V_108 > 0 || V_111 )
V_2 -> V_31 |= F_5 ( V_130 ) ;
V_105 += V_8 ;
V_110 += V_8 ;
V_2 -> V_7 = F_5 ( V_8 + V_107 ) ;
F_1 ( V_2 ) ;
V_13 = F_75 ( V_52 ) ;
if ( V_13 )
goto V_136;
F_67 ( F_23 ( V_11 ) , V_133 ) ;
}
F_29 ( V_6 ) ;
F_67 ( F_23 ( V_11 ) , V_134 ) ;
return V_13 ;
V_136:
F_27 ( V_6 ) ;
F_67 ( F_23 ( V_11 ) , V_113 ) ;
return V_13 ;
}
int
F_106 ( void * V_97 , char * V_96 , int V_110 , int V_8 , int V_141 , struct V_5 * V_6 )
{
struct V_142 * V_143 = V_97 ;
if ( V_6 -> V_131 == V_135 ) {
if ( F_107 ( V_96 , V_143 , V_110 , V_8 ) < 0 )
return - V_144 ;
} else {
T_3 V_145 = 0 ;
if ( F_108 ( V_96 , V_143 , V_110 , V_8 , & V_145 ) < 0 )
return - V_144 ;
V_6 -> V_145 = F_109 ( V_6 -> V_145 , V_145 , V_141 ) ;
}
return 0 ;
}
static inline T_3
F_110 ( struct V_146 * V_146 , int V_110 , int V_147 )
{
char * V_148 ;
T_3 V_145 ;
V_148 = F_111 ( V_146 ) ;
V_145 = F_112 ( V_148 + V_110 , V_147 , 0 ) ;
F_113 ( V_146 ) ;
return V_145 ;
}
static inline int F_114 ( struct V_20 * V_21 ,
struct V_149 * V_150 ,
int F_115 ( void * V_97 , char * V_96 , int V_110 , int V_8 ,
int V_141 , struct V_5 * V_6 ) ,
void * V_97 , int V_151 , int V_42 , int V_152 ,
int V_153 , int V_154 , unsigned int V_62 )
{
struct V_5 * V_6 ;
int V_13 ;
if ( ( V_6 = F_116 ( V_150 ) ) == NULL ) {
V_6 = F_117 ( V_21 ,
V_42 + V_152 + V_153 + 20 ,
( V_62 & V_155 ) , & V_13 ) ;
if ( V_6 == NULL )
return V_13 ;
F_100 ( V_6 , V_42 ) ;
F_101 ( V_6 , V_152 + V_153 ) ;
F_16 ( V_6 ) ;
V_6 -> V_139 = V_6 -> V_140 + V_152 ;
V_6 -> V_145 = 0 ;
F_118 ( V_150 , V_6 ) ;
} else if ( F_42 ( V_6 ) ) {
goto V_156;
}
V_6 -> V_131 = V_135 ;
F_65 ( V_6 ) -> V_157 = V_154 - V_152 ;
F_65 ( V_6 ) -> V_158 = V_159 ;
V_156:
return F_119 ( V_21 , V_6 , F_115 , V_97 ,
( V_151 - V_153 ) ) ;
}
static int F_120 ( struct V_20 * V_21 ,
struct V_75 * V_76 ,
struct V_149 * V_150 ,
struct V_160 * V_161 ,
struct V_162 * V_163 ,
int F_115 ( void * V_97 , char * V_96 , int V_110 ,
int V_8 , int V_141 , struct V_5 * V_6 ) ,
void * V_97 , int V_151 , int V_153 ,
unsigned int V_62 )
{
struct V_14 * V_15 = F_13 ( V_21 ) ;
struct V_5 * V_6 ;
struct V_164 * V_25 = V_161 -> V_25 ;
int V_42 ;
int V_165 ;
int V_106 ;
int V_147 ;
int V_13 ;
int V_110 = 0 ;
unsigned int V_154 , V_152 ;
int V_166 = V_132 ;
struct V_26 * V_27 = (struct V_26 * ) V_161 -> V_17 ;
V_6 = F_116 ( V_150 ) ;
V_165 = ! V_6 ? V_27 -> V_17 . V_167 : 0 ;
V_106 = V_161 -> V_168 ;
V_42 = F_21 ( V_27 -> V_17 . V_11 ) ;
V_152 = sizeof( struct V_1 ) + ( V_25 ? V_25 -> V_28 : 0 ) ;
V_154 = ( ( V_106 - V_152 ) & ~ 7 ) + V_152 ;
if ( V_161 -> V_151 + V_151 > 0xFFFF - V_152 ) {
F_121 ( V_21 , V_116 , V_76 -> V_23 , V_15 -> V_85 ,
V_106 - V_165 ) ;
return - V_116 ;
}
if ( V_153 &&
V_151 + V_152 <= V_106 &&
V_27 -> V_17 . V_11 -> V_169 & V_170 &&
! V_165 )
V_166 = V_135 ;
V_161 -> V_151 += V_151 ;
if ( ( ( V_151 > V_106 ) || ( V_6 && F_42 ( V_6 ) ) ) &&
( V_21 -> V_36 == V_171 ) &&
( V_27 -> V_17 . V_11 -> V_169 & V_172 ) && ! V_27 -> V_17 . V_167 ) {
V_13 = F_114 ( V_21 , V_150 , F_115 , V_97 , V_151 ,
V_42 , V_152 , V_153 ,
V_154 , V_62 ) ;
if ( V_13 )
goto error;
return 0 ;
}
if ( ! V_6 )
goto V_173;
while ( V_151 > 0 ) {
V_147 = V_106 - V_6 -> V_8 ;
if ( V_147 < V_151 )
V_147 = V_154 - V_6 -> V_8 ;
if ( V_147 <= 0 ) {
char * V_174 ;
unsigned int V_175 ;
unsigned int V_176 ;
unsigned int V_177 ;
unsigned int V_178 ;
struct V_5 * V_179 ;
V_173:
V_179 = V_6 ;
if ( V_179 )
V_177 = V_179 -> V_8 - V_154 ;
else
V_177 = 0 ;
V_175 = V_151 + V_177 ;
if ( V_175 > V_106 - V_152 )
V_175 = V_154 - V_152 ;
V_176 = V_175 + V_152 ;
if ( ( V_62 & V_180 ) &&
! ( V_27 -> V_17 . V_11 -> V_169 & V_181 ) )
V_178 = V_106 ;
else
V_178 = V_176 ;
V_178 += V_165 ;
if ( V_175 == V_151 + V_177 )
V_178 += V_27 -> V_17 . V_182 ;
if ( V_153 ) {
V_6 = F_117 ( V_21 ,
V_178 + V_42 + 15 ,
( V_62 & V_155 ) , & V_13 ) ;
} else {
V_6 = NULL ;
if ( F_122 ( & V_21 -> V_183 ) <=
2 * V_21 -> V_184 )
V_6 = F_123 ( V_21 ,
V_178 + V_42 + 15 , 1 ,
V_21 -> V_185 ) ;
if ( F_24 ( V_6 == NULL ) )
V_13 = - V_186 ;
else
V_161 -> V_187 = 0 ;
}
if ( V_6 == NULL )
goto error;
V_6 -> V_131 = V_166 ;
V_6 -> V_145 = 0 ;
F_100 ( V_6 , V_42 ) ;
F_65 ( V_6 ) -> V_187 = V_161 -> V_187 ;
V_174 = F_101 ( V_6 , V_176 + V_165 ) ;
F_124 ( V_6 , V_165 ) ;
V_6 -> V_139 = ( V_6 -> V_140 +
V_152 ) ;
V_174 += V_152 + V_165 ;
if ( V_177 ) {
V_6 -> V_145 = F_125 (
V_179 , V_154 ,
V_174 + V_153 , V_177 , 0 ) ;
V_179 -> V_145 = F_126 ( V_179 -> V_145 ,
V_6 -> V_145 ) ;
V_174 += V_177 ;
F_127 ( V_179 , V_154 ) ;
}
V_147 = V_175 - V_153 - V_177 ;
if ( V_147 > 0 && F_115 ( V_97 , V_174 + V_153 , V_110 , V_147 , V_177 , V_6 ) < 0 ) {
V_13 = - V_144 ;
F_27 ( V_6 ) ;
goto error;
}
V_110 += V_147 ;
V_151 -= V_175 - V_177 ;
V_153 = 0 ;
V_165 = 0 ;
V_166 = V_132 ;
F_118 ( V_150 , V_6 ) ;
continue;
}
if ( V_147 > V_151 )
V_147 = V_151 ;
if ( ! ( V_27 -> V_17 . V_11 -> V_169 & V_181 ) ) {
unsigned int V_188 ;
V_188 = V_6 -> V_8 ;
if ( F_115 ( V_97 , F_101 ( V_6 , V_147 ) ,
V_110 , V_147 , V_188 , V_6 ) < 0 ) {
F_128 ( V_6 , V_188 ) ;
V_13 = - V_144 ;
goto error;
}
} else {
int V_189 = F_65 ( V_6 ) -> V_190 ;
V_13 = - V_53 ;
if ( ! F_129 ( V_21 , V_163 ) )
goto error;
if ( ! F_130 ( V_6 , V_189 , V_163 -> V_146 ,
V_163 -> V_110 ) ) {
V_13 = - V_116 ;
if ( V_189 == V_191 )
goto error;
F_131 ( V_6 , V_189 , V_163 -> V_146 ,
V_163 -> V_110 , 0 ) ;
F_65 ( V_6 ) -> V_190 = ++ V_189 ;
F_132 ( V_163 -> V_146 ) ;
}
V_147 = F_133 ( int , V_147 , V_163 -> V_192 - V_163 -> V_110 ) ;
if ( F_115 ( V_97 ,
F_134 ( V_163 -> V_146 ) + V_163 -> V_110 ,
V_110 , V_147 , V_6 -> V_8 , V_6 ) < 0 )
goto V_193;
V_163 -> V_110 += V_147 ;
F_135 ( & F_65 ( V_6 ) -> V_194 [ V_189 - 1 ] , V_147 ) ;
V_6 -> V_8 += V_147 ;
V_6 -> V_129 += V_147 ;
V_6 -> V_127 += V_147 ;
F_136 ( V_147 , & V_21 -> V_183 ) ;
}
V_110 += V_147 ;
V_151 -= V_147 ;
}
return 0 ;
V_193:
V_13 = - V_144 ;
error:
V_161 -> V_151 -= V_151 ;
F_67 ( F_60 ( V_21 ) , V_195 ) ;
return V_13 ;
}
static int F_137 ( struct V_20 * V_21 , struct V_160 * V_161 ,
struct V_196 * V_197 , struct V_26 * * V_198 )
{
struct V_14 * V_15 = F_13 ( V_21 ) ;
struct V_24 * V_25 ;
struct V_26 * V_27 ;
V_25 = V_197 -> V_25 ;
if ( V_25 ) {
if ( V_161 -> V_25 == NULL ) {
V_161 -> V_25 = F_138 ( sizeof( struct V_164 ) + 40 ,
V_21 -> V_185 ) ;
if ( F_24 ( V_161 -> V_25 == NULL ) )
return - V_186 ;
}
memcpy ( V_161 -> V_25 , & V_25 -> V_25 , sizeof( struct V_164 ) + V_25 -> V_25 . V_28 ) ;
V_161 -> V_62 |= V_199 ;
V_161 -> V_200 = V_197 -> V_200 ;
}
V_27 = * V_198 ;
if ( F_24 ( ! V_27 ) )
return - V_144 ;
* V_198 = NULL ;
V_161 -> V_168 = V_15 -> V_201 == V_202 ?
V_27 -> V_17 . V_11 -> V_106 : F_76 ( & V_27 -> V_17 ) ;
V_161 -> V_17 = & V_27 -> V_17 ;
V_161 -> V_151 = 0 ;
V_161 -> V_187 = V_197 -> V_187 ;
return 0 ;
}
int F_139 ( struct V_20 * V_21 , struct V_75 * V_76 ,
int F_115 ( void * V_97 , char * V_96 , int V_110 , int V_8 ,
int V_141 , struct V_5 * V_6 ) ,
void * V_97 , int V_151 , int V_153 ,
struct V_196 * V_197 , struct V_26 * * V_198 ,
unsigned int V_62 )
{
struct V_14 * V_15 = F_13 ( V_21 ) ;
int V_13 ;
if ( V_62 & V_203 )
return 0 ;
if ( F_140 ( & V_21 -> V_204 ) ) {
V_13 = F_137 ( V_21 , & V_15 -> V_161 . V_205 , V_197 , V_198 ) ;
if ( V_13 )
return V_13 ;
} else {
V_153 = 0 ;
}
return F_120 ( V_21 , V_76 , & V_21 -> V_204 , & V_15 -> V_161 . V_205 ,
F_141 ( V_21 ) , F_115 ,
V_97 , V_151 , V_153 , V_62 ) ;
}
T_4 F_142 ( struct V_20 * V_21 , struct V_75 * V_76 , struct V_146 * V_146 ,
int V_110 , T_5 V_192 , int V_62 )
{
struct V_14 * V_15 = F_13 ( V_21 ) ;
struct V_5 * V_6 ;
struct V_26 * V_27 ;
struct V_164 * V_25 = NULL ;
struct V_160 * V_161 ;
int V_42 ;
int V_106 ;
int V_8 ;
int V_13 ;
unsigned int V_154 , V_152 , V_177 ;
if ( V_15 -> V_206 )
return - V_207 ;
if ( V_62 & V_203 )
return 0 ;
if ( F_140 ( & V_21 -> V_204 ) )
return - V_58 ;
V_161 = & V_15 -> V_161 . V_205 ;
V_27 = (struct V_26 * ) V_161 -> V_17 ;
if ( V_161 -> V_62 & V_199 )
V_25 = V_161 -> V_25 ;
if ( ! ( V_27 -> V_17 . V_11 -> V_169 & V_181 ) )
return - V_208 ;
V_42 = F_21 ( V_27 -> V_17 . V_11 ) ;
V_106 = V_161 -> V_168 ;
V_152 = sizeof( struct V_1 ) + ( V_25 ? V_25 -> V_28 : 0 ) ;
V_154 = ( ( V_106 - V_152 ) & ~ 7 ) + V_152 ;
if ( V_161 -> V_151 + V_192 > 0xFFFF - V_152 ) {
F_121 ( V_21 , V_116 , V_76 -> V_23 , V_15 -> V_85 , V_106 ) ;
return - V_116 ;
}
if ( ( V_6 = F_116 ( & V_21 -> V_204 ) ) == NULL )
return - V_58 ;
V_161 -> V_151 += V_192 ;
if ( ( V_192 + V_6 -> V_8 > V_106 ) &&
( V_21 -> V_36 == V_171 ) &&
( V_27 -> V_17 . V_11 -> V_169 & V_172 ) ) {
F_65 ( V_6 ) -> V_157 = V_106 - V_152 ;
F_65 ( V_6 ) -> V_158 = V_159 ;
}
while ( V_192 > 0 ) {
int V_189 ;
if ( F_42 ( V_6 ) )
V_8 = V_192 ;
else {
V_8 = V_106 - V_6 -> V_8 ;
if ( V_8 < V_192 )
V_8 = V_154 - V_6 -> V_8 ;
}
if ( V_8 <= 0 ) {
struct V_5 * V_179 ;
int V_178 ;
V_179 = V_6 ;
V_177 = V_179 -> V_8 - V_154 ;
V_178 = V_152 + V_42 + V_177 + 15 ;
V_6 = F_123 ( V_21 , V_178 , 1 , V_21 -> V_185 ) ;
if ( F_24 ( ! V_6 ) ) {
V_13 = - V_186 ;
goto error;
}
V_6 -> V_131 = V_132 ;
V_6 -> V_145 = 0 ;
F_100 ( V_6 , V_42 ) ;
F_101 ( V_6 , V_152 + V_177 ) ;
F_16 ( V_6 ) ;
V_6 -> V_139 = ( V_6 -> V_140 +
V_152 ) ;
if ( V_177 ) {
V_6 -> V_145 = F_125 ( V_179 ,
V_154 ,
F_104 ( V_6 ) ,
V_177 , 0 ) ;
V_179 -> V_145 = F_126 ( V_179 -> V_145 ,
V_6 -> V_145 ) ;
F_127 ( V_179 , V_154 ) ;
}
F_118 ( & V_21 -> V_204 , V_6 ) ;
continue;
}
V_189 = F_65 ( V_6 ) -> V_190 ;
if ( V_8 > V_192 )
V_8 = V_192 ;
if ( F_130 ( V_6 , V_189 , V_146 , V_110 ) ) {
F_135 ( & F_65 ( V_6 ) -> V_194 [ V_189 - 1 ] , V_8 ) ;
} else if ( V_189 < V_191 ) {
F_132 ( V_146 ) ;
F_143 ( V_6 , V_189 , V_146 , V_110 , V_8 ) ;
} else {
V_13 = - V_116 ;
goto error;
}
if ( V_6 -> V_131 == V_132 ) {
T_3 V_145 ;
V_145 = F_110 ( V_146 , V_110 , V_8 ) ;
V_6 -> V_145 = F_109 ( V_6 -> V_145 , V_145 , V_6 -> V_8 ) ;
}
V_6 -> V_8 += V_8 ;
V_6 -> V_129 += V_8 ;
V_6 -> V_127 += V_8 ;
F_136 ( V_8 , & V_21 -> V_183 ) ;
V_110 += V_8 ;
V_192 -= V_8 ;
}
return 0 ;
error:
V_161 -> V_151 -= V_192 ;
F_67 ( F_60 ( V_21 ) , V_195 ) ;
return V_13 ;
}
static void F_144 ( struct V_160 * V_161 )
{
V_161 -> V_62 &= ~ V_199 ;
F_145 ( V_161 -> V_25 ) ;
V_161 -> V_25 = NULL ;
F_146 ( V_161 -> V_17 ) ;
V_161 -> V_17 = NULL ;
}
struct V_5 * F_147 ( struct V_20 * V_21 ,
struct V_75 * V_76 ,
struct V_149 * V_150 ,
struct V_160 * V_161 )
{
struct V_5 * V_6 , * V_209 ;
struct V_5 * * V_210 ;
struct V_14 * V_15 = F_13 ( V_21 ) ;
struct V_211 * V_211 = F_60 ( V_21 ) ;
struct V_164 * V_25 = NULL ;
struct V_26 * V_27 = (struct V_26 * ) V_161 -> V_17 ;
struct V_1 * V_2 ;
V_91 V_212 = 0 ;
T_6 V_18 ;
if ( ( V_6 = F_148 ( V_150 ) ) == NULL )
goto V_213;
V_210 = & ( F_65 ( V_6 ) -> V_128 ) ;
if ( V_6 -> V_174 < F_92 ( V_6 ) )
F_149 ( V_6 , F_150 ( V_6 ) ) ;
while ( ( V_209 = F_148 ( V_150 ) ) != NULL ) {
F_149 ( V_209 , F_151 ( V_6 ) ) ;
* V_210 = V_209 ;
V_210 = & ( V_209 -> V_123 ) ;
V_6 -> V_8 += V_209 -> V_8 ;
V_6 -> V_129 += V_209 -> V_8 ;
V_6 -> V_127 += V_209 -> V_127 ;
V_209 -> V_125 = NULL ;
V_209 -> V_21 = NULL ;
}
if ( V_15 -> V_201 < V_214 )
V_6 -> V_92 = 1 ;
if ( V_15 -> V_201 >= V_214 ||
( V_6 -> V_8 <= F_76 ( & V_27 -> V_17 ) &&
F_17 ( V_21 , & V_27 -> V_17 ) ) )
V_212 = F_5 ( V_32 ) ;
if ( V_161 -> V_62 & V_199 )
V_25 = V_161 -> V_25 ;
if ( V_27 -> V_46 == V_47 )
V_18 = V_15 -> V_215 ;
else
V_18 = F_10 ( V_15 , & V_27 -> V_17 ) ;
V_2 = F_4 ( V_6 ) ;
V_2 -> V_29 = 4 ;
V_2 -> V_4 = 5 ;
V_2 -> V_30 = V_15 -> V_30 ;
V_2 -> V_31 = V_212 ;
V_2 -> V_18 = V_18 ;
V_2 -> V_35 = V_21 -> V_36 ;
F_51 ( V_2 , V_76 ) ;
F_18 ( V_6 , & V_27 -> V_17 , V_21 ) ;
if ( V_25 ) {
V_2 -> V_4 += V_25 -> V_28 >> 2 ;
F_19 ( V_6 , V_25 , V_161 -> V_200 , V_27 , 0 ) ;
}
V_6 -> V_37 = V_21 -> V_38 ;
V_6 -> V_39 = V_21 -> V_40 ;
V_161 -> V_17 = NULL ;
F_152 ( V_6 , & V_27 -> V_17 ) ;
if ( V_2 -> V_35 == V_216 )
F_153 ( V_211 , ( (struct V_217 * )
F_104 ( V_6 ) ) -> type ) ;
F_144 ( V_161 ) ;
V_213:
return V_6 ;
}
int F_154 ( struct V_211 * V_211 , struct V_5 * V_6 )
{
int V_13 ;
V_13 = F_8 ( V_6 ) ;
if ( V_13 ) {
if ( V_13 > 0 )
V_13 = F_155 ( V_13 ) ;
if ( V_13 )
F_67 ( V_211 , V_195 ) ;
}
return V_13 ;
}
int F_156 ( struct V_20 * V_21 , struct V_75 * V_76 )
{
struct V_5 * V_6 ;
V_6 = F_157 ( V_21 , V_76 ) ;
if ( ! V_6 )
return 0 ;
return F_154 ( F_60 ( V_21 ) , V_6 ) ;
}
static void F_158 ( struct V_20 * V_21 ,
struct V_149 * V_150 ,
struct V_160 * V_161 )
{
struct V_5 * V_6 ;
while ( ( V_6 = F_159 ( V_150 ) ) != NULL )
F_27 ( V_6 ) ;
F_144 ( V_161 ) ;
}
void F_160 ( struct V_20 * V_21 )
{
F_158 ( V_21 , & V_21 -> V_204 , & F_13 ( V_21 ) -> V_161 . V_205 ) ;
}
struct V_5 * F_161 ( struct V_20 * V_21 ,
struct V_75 * V_76 ,
int F_115 ( void * V_97 , char * V_96 , int V_110 ,
int V_8 , int V_141 , struct V_5 * V_6 ) ,
void * V_97 , int V_151 , int V_153 ,
struct V_196 * V_197 , struct V_26 * * V_198 ,
unsigned int V_62 )
{
struct V_160 V_161 ;
struct V_149 V_150 ;
int V_13 ;
if ( V_62 & V_203 )
return NULL ;
F_162 ( & V_150 ) ;
V_161 . V_62 = 0 ;
V_161 . V_200 = 0 ;
V_161 . V_25 = NULL ;
V_13 = F_137 ( V_21 , & V_161 , V_197 , V_198 ) ;
if ( V_13 )
return F_163 ( V_13 ) ;
V_13 = F_120 ( V_21 , V_76 , & V_150 , & V_161 ,
& V_218 -> V_219 , F_115 ,
V_97 , V_151 , V_153 , V_62 ) ;
if ( V_13 ) {
F_158 ( V_21 , & V_150 , & V_161 ) ;
return F_163 ( V_13 ) ;
}
return F_147 ( V_21 , V_76 , & V_150 , & V_161 ) ;
}
static int F_164 ( void * V_220 , char * V_96 , int V_110 ,
int V_8 , int V_141 , struct V_5 * V_6 )
{
T_3 V_145 ;
V_145 = F_165 ( V_220 + V_110 , V_96 , V_8 , 0 ) ;
V_6 -> V_145 = F_109 ( V_6 -> V_145 , V_145 , V_141 ) ;
return 0 ;
}
void F_166 ( struct V_211 * V_211 , struct V_5 * V_6 , T_1 V_23 ,
T_1 V_22 , const struct V_221 * V_222 ,
unsigned int V_8 )
{
struct V_223 V_224 ;
struct V_196 V_197 ;
struct V_75 V_76 ;
struct V_26 * V_27 = F_14 ( V_6 ) ;
struct V_5 * V_225 ;
struct V_20 * V_21 ;
struct V_14 * V_15 ;
if ( F_167 ( & V_224 . V_25 . V_25 , V_6 ) )
return;
V_197 . V_200 = V_23 ;
V_197 . V_25 = NULL ;
V_197 . V_187 = 0 ;
if ( V_224 . V_25 . V_25 . V_28 ) {
V_197 . V_25 = & V_224 . V_25 ;
if ( V_224 . V_25 . V_25 . V_33 )
V_23 = V_224 . V_25 . V_25 . V_34 ;
}
F_168 ( & V_76 , V_222 -> V_226 , 0 ,
F_169 ( V_222 -> V_30 ) ,
V_227 , F_4 ( V_6 ) -> V_35 ,
F_170 ( V_222 ) ,
V_23 , V_22 ,
F_171 ( V_6 ) -> V_228 , F_171 ( V_6 ) -> V_229 ) ;
F_172 ( V_6 , F_173 ( & V_76 ) ) ;
V_27 = F_174 ( V_211 , & V_76 ) ;
if ( F_34 ( V_27 ) )
return;
V_15 = & F_175 ( V_230 ) ;
V_15 -> V_30 = V_222 -> V_30 ;
V_21 = & V_15 -> V_21 ;
V_21 -> V_38 = V_6 -> V_37 ;
V_21 -> V_36 = F_4 ( V_6 ) -> V_35 ;
V_21 -> V_87 = V_222 -> V_226 ;
F_176 ( V_21 , V_211 ) ;
F_162 ( & V_21 -> V_204 ) ;
V_21 -> V_184 = V_231 ;
F_139 ( V_21 , & V_76 , F_164 , V_222 -> V_143 -> V_232 , V_8 , 0 ,
& V_197 , & V_27 , V_155 ) ;
V_225 = F_177 ( & V_21 -> V_204 ) ;
if ( V_225 ) {
if ( V_222 -> V_233 >= 0 )
* ( ( V_234 * ) F_104 ( V_225 ) +
V_222 -> V_233 ) = F_178 ( F_179 ( V_225 -> V_145 ,
V_222 -> V_145 ) ) ;
V_225 -> V_131 = V_132 ;
F_180 ( V_225 ) ;
F_181 ( V_225 , F_182 ( V_6 ) ) ;
F_156 ( V_21 , & V_76 ) ;
}
F_183 ( V_230 ) ;
F_184 ( V_27 ) ;
}
void T_7 F_185 ( void )
{
F_186 () ;
F_187 () ;
#if F_39 ( V_235 ) && F_39 ( V_236 )
F_188 () ;
#endif
}
