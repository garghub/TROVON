static int F_1 ( struct V_1 * V_2 , struct V_3 * V_3 )
{
struct V_4 * V_5 = V_2 -> V_5 ;
struct V_6 * V_7 = V_5 -> V_8 ;
if ( V_7 -> type -> V_9 . V_10 )
return V_7 -> type -> V_9 . V_10 ( V_5 , V_2 , V_3 ) ;
return 1 ;
}
bool F_2 ( struct V_1 * V_2 , struct V_3 * V_3 )
{
if ( ! F_3 ( V_2 , V_3 ) )
return 0 ;
if ( ! F_1 ( V_2 , V_3 ) )
return 0 ;
return 1 ;
}
static struct V_11 * F_4 ( const char * V_12 )
{
struct V_11 * V_7 ;
F_5 (e, &elv_list, list) {
if ( ! strcmp ( V_7 -> V_13 , V_12 ) )
return V_7 ;
}
return NULL ;
}
static void F_6 ( struct V_11 * V_7 )
{
F_7 ( V_7 -> V_14 ) ;
}
static struct V_11 * F_8 ( const char * V_12 , bool V_15 )
{
struct V_11 * V_7 ;
F_9 ( & V_16 ) ;
V_7 = F_4 ( V_12 ) ;
if ( ! V_7 && V_15 ) {
F_10 ( & V_16 ) ;
F_11 ( L_1 , V_12 ) ;
F_9 ( & V_16 ) ;
V_7 = F_4 ( V_12 ) ;
}
if ( V_7 && ! F_12 ( V_7 -> V_14 ) )
V_7 = NULL ;
F_10 ( & V_16 ) ;
return V_7 ;
}
static int T_1 F_13 ( char * V_17 )
{
strncpy ( V_18 , V_17 , sizeof( V_18 ) - 1 ) ;
return 1 ;
}
void T_1 F_14 ( void )
{
struct V_11 * V_7 ;
if ( ! V_18 [ 0 ] )
return;
F_9 ( & V_16 ) ;
V_7 = F_4 ( V_18 ) ;
F_10 ( & V_16 ) ;
if ( ! V_7 )
F_11 ( L_1 , V_18 ) ;
}
struct V_6 * F_15 ( struct V_4 * V_5 ,
struct V_11 * V_7 )
{
struct V_6 * V_19 ;
V_19 = F_16 ( sizeof( * V_19 ) , V_20 , V_5 -> V_21 ) ;
if ( F_17 ( ! V_19 ) )
goto V_22;
V_19 -> type = V_7 ;
F_18 ( & V_19 -> V_23 , & V_24 ) ;
F_19 ( & V_19 -> V_25 ) ;
F_20 ( V_19 -> V_26 ) ;
return V_19 ;
V_22:
F_21 ( V_19 ) ;
F_6 ( V_7 ) ;
return NULL ;
}
static void F_22 ( struct V_27 * V_23 )
{
struct V_6 * V_7 ;
V_7 = F_23 ( V_23 , struct V_6 , V_23 ) ;
F_6 ( V_7 -> type ) ;
F_21 ( V_7 ) ;
}
int F_24 ( struct V_4 * V_5 , char * V_12 )
{
struct V_11 * V_7 = NULL ;
int V_22 ;
if ( F_17 ( V_5 -> V_8 ) )
return 0 ;
F_25 ( & V_5 -> V_28 ) ;
V_5 -> V_29 = NULL ;
V_5 -> V_30 = 0 ;
V_5 -> V_31 = NULL ;
if ( V_12 ) {
V_7 = F_8 ( V_12 , true ) ;
if ( ! V_7 )
return - V_32 ;
}
if ( ! V_7 && * V_18 ) {
V_7 = F_8 ( V_18 , false ) ;
if ( ! V_7 )
F_26 ( V_33 L_2 ,
V_18 ) ;
}
if ( ! V_7 ) {
V_7 = F_8 ( V_34 , false ) ;
if ( ! V_7 ) {
F_26 ( V_33
L_3 \
L_4 ) ;
V_7 = F_8 ( L_5 , false ) ;
}
}
V_22 = V_7 -> V_9 . V_35 ( V_5 , V_7 ) ;
return 0 ;
}
void F_27 ( struct V_6 * V_7 )
{
F_28 ( & V_7 -> V_25 ) ;
if ( V_7 -> type -> V_9 . V_36 )
V_7 -> type -> V_9 . V_36 ( V_7 ) ;
F_29 ( & V_7 -> V_25 ) ;
F_30 ( & V_7 -> V_23 ) ;
}
static inline void F_31 ( struct V_1 * V_2 )
{
F_32 ( & V_2 -> V_26 ) ;
}
static void F_33 ( struct V_4 * V_5 , struct V_1 * V_2 )
{
if ( F_34 ( V_2 ) )
F_31 ( V_2 ) ;
}
static void F_35 ( struct V_4 * V_5 , struct V_1 * V_2 )
{
struct V_6 * V_7 = V_5 -> V_8 ;
F_36 ( F_34 ( V_2 ) ) ;
F_37 ( V_7 -> V_26 , & V_2 -> V_26 , F_38 ( V_2 ) ) ;
}
static void F_39 ( struct V_4 * V_5 , struct V_1 * V_2 )
{
F_31 ( V_2 ) ;
F_35 ( V_5 , V_2 ) ;
}
static struct V_1 * F_40 ( struct V_4 * V_5 , T_2 V_37 )
{
struct V_6 * V_7 = V_5 -> V_8 ;
struct V_38 * V_39 ;
struct V_1 * V_2 ;
F_41 (e->hash, rq, next, hash, offset) {
F_36 ( ! F_34 ( V_2 ) ) ;
if ( F_17 ( ! F_42 ( V_2 ) ) ) {
F_31 ( V_2 ) ;
continue;
}
if ( F_38 ( V_2 ) == V_37 )
return V_2 ;
}
return NULL ;
}
void F_43 ( struct V_40 * V_41 , struct V_1 * V_2 )
{
struct V_42 * * V_43 = & V_41 -> V_42 ;
struct V_42 * V_44 = NULL ;
struct V_1 * V_45 ;
while ( * V_43 ) {
V_44 = * V_43 ;
V_45 = F_44 ( V_44 , struct V_1 , V_42 ) ;
if ( F_45 ( V_2 ) < F_45 ( V_45 ) )
V_43 = & ( * V_43 ) -> V_46 ;
else if ( F_45 ( V_2 ) >= F_45 ( V_45 ) )
V_43 = & ( * V_43 ) -> V_47 ;
}
F_46 ( & V_2 -> V_42 , V_44 , V_43 ) ;
F_47 ( & V_2 -> V_42 , V_41 ) ;
}
void F_48 ( struct V_40 * V_41 , struct V_1 * V_2 )
{
F_36 ( F_49 ( & V_2 -> V_42 ) ) ;
F_50 ( & V_2 -> V_42 , V_41 ) ;
F_51 ( & V_2 -> V_42 ) ;
}
struct V_1 * F_52 ( struct V_40 * V_41 , T_2 V_48 )
{
struct V_42 * V_49 = V_41 -> V_42 ;
struct V_1 * V_2 ;
while ( V_49 ) {
V_2 = F_44 ( V_49 , struct V_1 , V_42 ) ;
if ( V_48 < F_45 ( V_2 ) )
V_49 = V_49 -> V_46 ;
else if ( V_48 > F_45 ( V_2 ) )
V_49 = V_49 -> V_47 ;
else
return V_2 ;
}
return NULL ;
}
void F_53 ( struct V_4 * V_5 , struct V_1 * V_2 )
{
T_2 V_50 ;
struct V_51 * V_52 ;
int V_53 ;
if ( V_5 -> V_29 == V_2 )
V_5 -> V_29 = NULL ;
F_33 ( V_5 , V_2 ) ;
V_5 -> V_54 -- ;
V_50 = V_5 -> V_30 ;
V_53 = V_55 | V_56 ;
F_54 (entry, &q->queue_head) {
struct V_1 * V_57 = F_55 ( V_52 ) ;
if ( ( V_2 -> V_58 & V_59 ) !=
( V_57 -> V_58 & V_59 ) )
break;
if ( F_56 ( V_2 ) != F_56 ( V_57 ) )
break;
if ( V_57 -> V_58 & V_53 )
break;
if ( F_45 ( V_2 ) >= V_50 ) {
if ( F_45 ( V_57 ) < V_50 )
continue;
} else {
if ( F_45 ( V_57 ) >= V_50 )
break;
}
if ( F_45 ( V_2 ) >= F_45 ( V_57 ) )
break;
}
F_57 ( & V_2 -> V_60 , V_52 ) ;
}
void F_58 ( struct V_4 * V_5 , struct V_1 * V_2 )
{
if ( V_5 -> V_29 == V_2 )
V_5 -> V_29 = NULL ;
F_33 ( V_5 , V_2 ) ;
V_5 -> V_54 -- ;
V_5 -> V_30 = F_59 ( V_2 ) ;
V_5 -> V_31 = V_2 ;
F_60 ( & V_2 -> V_60 , & V_5 -> V_28 ) ;
}
int F_61 ( struct V_4 * V_5 , struct V_1 * * V_61 , struct V_3 * V_3 )
{
struct V_6 * V_7 = V_5 -> V_8 ;
struct V_1 * V_45 ;
int V_62 ;
if ( F_62 ( V_5 ) )
return V_63 ;
if ( V_5 -> V_29 && F_2 ( V_5 -> V_29 , V_3 ) ) {
V_62 = F_63 ( V_5 -> V_29 , V_3 ) ;
if ( V_62 != V_63 ) {
* V_61 = V_5 -> V_29 ;
return V_62 ;
}
}
if ( F_64 ( V_5 ) )
return V_63 ;
V_45 = F_40 ( V_5 , V_3 -> V_64 ) ;
if ( V_45 && F_2 ( V_45 , V_3 ) ) {
* V_61 = V_45 ;
return V_65 ;
}
if ( V_7 -> type -> V_9 . V_66 )
return V_7 -> type -> V_9 . V_66 ( V_5 , V_61 , V_3 ) ;
return V_63 ;
}
static bool F_65 ( struct V_4 * V_5 ,
struct V_1 * V_2 )
{
struct V_1 * V_45 ;
bool V_62 ;
if ( F_62 ( V_5 ) )
return false ;
if ( V_5 -> V_29 && F_66 ( V_5 , V_5 -> V_29 , V_2 ) )
return true ;
if ( F_64 ( V_5 ) )
return false ;
V_62 = false ;
while ( 1 ) {
V_45 = F_40 ( V_5 , F_45 ( V_2 ) ) ;
if ( ! V_45 || ! F_66 ( V_5 , V_45 , V_2 ) )
break;
V_62 = true ;
V_2 = V_45 ;
}
return V_62 ;
}
void F_67 ( struct V_4 * V_5 , struct V_1 * V_2 , int type )
{
struct V_6 * V_7 = V_5 -> V_8 ;
if ( V_7 -> type -> V_9 . V_67 )
V_7 -> type -> V_9 . V_67 ( V_5 , V_2 , type ) ;
if ( type == V_65 )
F_39 ( V_5 , V_2 ) ;
V_5 -> V_29 = V_2 ;
}
void F_68 ( struct V_4 * V_5 , struct V_1 * V_2 ,
struct V_1 * V_39 )
{
struct V_6 * V_7 = V_5 -> V_8 ;
const int V_68 = V_39 -> V_58 & V_69 ;
if ( V_68 && V_7 -> type -> V_9 . V_70 )
V_7 -> type -> V_9 . V_70 ( V_5 , V_2 , V_39 ) ;
F_39 ( V_5 , V_2 ) ;
if ( V_68 ) {
F_33 ( V_5 , V_39 ) ;
V_5 -> V_54 -- ;
}
V_5 -> V_29 = V_2 ;
}
void F_69 ( struct V_4 * V_5 , struct V_1 * V_2 ,
struct V_3 * V_3 )
{
struct V_6 * V_7 = V_5 -> V_8 ;
if ( V_7 -> type -> V_9 . V_71 )
V_7 -> type -> V_9 . V_71 ( V_5 , V_2 , V_3 ) ;
}
static void F_70 ( struct V_1 * V_2 )
{
if ( V_2 -> V_5 -> V_72 && ! ( V_2 -> V_58 & V_73 ) )
V_2 -> V_5 -> V_74 -- ;
}
static void F_71 ( struct V_4 * V_5 , struct V_1 * V_2 )
{
if ( V_5 -> V_72 && ! ( V_2 -> V_58 & V_73 ) && V_5 -> V_74 ++ == 0 &&
( V_5 -> V_75 == V_76 || V_5 -> V_75 == V_77 ) )
F_72 ( V_5 -> V_72 ) ;
}
static inline void F_70 ( struct V_1 * V_2 ) {}
static inline void F_71 ( struct V_4 * V_5 ,
struct V_1 * V_2 )
{
}
void F_73 ( struct V_4 * V_5 , struct V_1 * V_2 )
{
if ( F_74 ( V_2 ) ) {
V_5 -> V_78 [ F_75 ( V_2 ) ] -- ;
if ( V_2 -> V_58 & V_69 )
F_76 ( V_5 , V_2 ) ;
}
V_2 -> V_58 &= ~ V_56 ;
F_70 ( V_2 ) ;
F_77 ( V_5 , V_2 , V_79 ) ;
}
void F_78 ( struct V_4 * V_5 )
{
static int V_80 ;
F_79 ( V_5 -> V_81 ) ;
while ( V_5 -> V_8 -> type -> V_9 . V_82 ( V_5 , 1 ) )
;
if ( V_5 -> V_54 && V_80 ++ < 10 ) {
F_26 ( V_33 L_6
L_7 ,
V_5 -> V_8 -> type -> V_13 , V_5 -> V_54 ) ;
}
}
void F_77 ( struct V_4 * V_5 , struct V_1 * V_2 , int V_83 )
{
F_80 ( V_5 , V_2 ) ;
F_71 ( V_5 , V_2 ) ;
V_2 -> V_5 = V_5 ;
if ( V_2 -> V_58 & V_55 ) {
if ( V_2 -> V_84 == V_85 ) {
V_5 -> V_30 = F_59 ( V_2 ) ;
V_5 -> V_31 = V_2 ;
}
} else if ( ! ( V_2 -> V_58 & V_86 ) &&
( V_83 == V_87 ||
V_83 == V_88 ) )
V_83 = V_89 ;
switch ( V_83 ) {
case V_79 :
case V_90 :
V_2 -> V_58 |= V_55 ;
F_57 ( & V_2 -> V_60 , & V_5 -> V_28 ) ;
break;
case V_89 :
V_2 -> V_58 |= V_55 ;
F_78 ( V_5 ) ;
F_60 ( & V_2 -> V_60 , & V_5 -> V_28 ) ;
F_81 ( V_5 ) ;
break;
case V_88 :
if ( F_65 ( V_5 , V_2 ) )
break;
case V_87 :
F_36 ( V_2 -> V_84 != V_85 ) ;
V_2 -> V_58 |= V_69 ;
V_5 -> V_54 ++ ;
if ( F_42 ( V_2 ) ) {
F_35 ( V_5 , V_2 ) ;
if ( ! V_5 -> V_29 )
V_5 -> V_29 = V_2 ;
}
V_5 -> V_8 -> type -> V_9 . V_91 ( V_5 , V_2 ) ;
break;
case V_92 :
V_2 -> V_58 |= V_55 ;
F_82 ( V_2 ) ;
break;
default:
F_26 ( V_33 L_8 ,
V_93 , V_83 ) ;
F_83 () ;
}
}
void F_84 ( struct V_4 * V_5 , struct V_1 * V_2 , int V_83 )
{
unsigned long V_94 ;
F_85 ( V_5 -> V_81 , V_94 ) ;
F_77 ( V_5 , V_2 , V_83 ) ;
F_86 ( V_5 -> V_81 , V_94 ) ;
}
struct V_1 * F_87 ( struct V_4 * V_5 , struct V_1 * V_2 )
{
struct V_6 * V_7 = V_5 -> V_8 ;
if ( V_7 -> type -> V_9 . V_95 )
return V_7 -> type -> V_9 . V_95 ( V_5 , V_2 ) ;
return NULL ;
}
struct V_1 * F_88 ( struct V_4 * V_5 , struct V_1 * V_2 )
{
struct V_6 * V_7 = V_5 -> V_8 ;
if ( V_7 -> type -> V_9 . V_96 )
return V_7 -> type -> V_9 . V_96 ( V_5 , V_2 ) ;
return NULL ;
}
int F_89 ( struct V_4 * V_5 , struct V_1 * V_2 ,
struct V_3 * V_3 , T_3 V_97 )
{
struct V_6 * V_7 = V_5 -> V_8 ;
if ( V_7 -> type -> V_9 . V_98 )
return V_7 -> type -> V_9 . V_98 ( V_5 , V_2 , V_3 , V_97 ) ;
return 0 ;
}
void F_90 ( struct V_4 * V_5 , struct V_1 * V_2 )
{
struct V_6 * V_7 = V_5 -> V_8 ;
if ( V_7 -> type -> V_9 . V_99 )
V_7 -> type -> V_9 . V_99 ( V_2 ) ;
}
int F_91 ( struct V_4 * V_5 , int V_100 )
{
struct V_6 * V_7 = V_5 -> V_8 ;
if ( V_7 -> type -> V_9 . V_101 )
return V_7 -> type -> V_9 . V_101 ( V_5 , V_100 ) ;
return V_102 ;
}
void F_92 ( struct V_4 * V_5 )
{
struct V_1 * V_2 ;
F_93 ( V_5 ) ;
while ( ! F_94 ( & V_5 -> V_28 ) ) {
V_2 = F_55 ( V_5 -> V_28 . V_39 ) ;
V_2 -> V_58 |= V_103 ;
F_95 ( V_5 , V_2 ) ;
F_96 ( V_2 ) ;
F_97 ( V_2 , - V_104 ) ;
}
}
void F_98 ( struct V_4 * V_5 , struct V_1 * V_2 )
{
struct V_6 * V_7 = V_5 -> V_8 ;
if ( F_74 ( V_2 ) ) {
V_5 -> V_78 [ F_75 ( V_2 ) ] -- ;
if ( ( V_2 -> V_58 & V_69 ) &&
V_7 -> type -> V_9 . V_105 )
V_7 -> type -> V_9 . V_105 ( V_5 , V_2 ) ;
}
}
static T_4
F_99 ( struct V_27 * V_23 , struct V_106 * V_107 , char * V_108 )
{
struct V_109 * V_52 = F_100 ( V_107 ) ;
struct V_6 * V_7 ;
T_4 error ;
if ( ! V_52 -> V_110 )
return - V_104 ;
V_7 = F_23 ( V_23 , struct V_6 , V_23 ) ;
F_28 ( & V_7 -> V_25 ) ;
error = V_7 -> type ? V_52 -> V_110 ( V_7 , V_108 ) : - V_111 ;
F_29 ( & V_7 -> V_25 ) ;
return error ;
}
static T_4
F_101 ( struct V_27 * V_23 , struct V_106 * V_107 ,
const char * V_108 , T_5 V_112 )
{
struct V_109 * V_52 = F_100 ( V_107 ) ;
struct V_6 * V_7 ;
T_4 error ;
if ( ! V_52 -> V_113 )
return - V_104 ;
V_7 = F_23 ( V_23 , struct V_6 , V_23 ) ;
F_28 ( & V_7 -> V_25 ) ;
error = V_7 -> type ? V_52 -> V_113 ( V_7 , V_108 , V_112 ) : - V_111 ;
F_29 ( & V_7 -> V_25 ) ;
return error ;
}
int F_102 ( struct V_4 * V_5 )
{
struct V_6 * V_7 = V_5 -> V_8 ;
int error ;
error = F_103 ( & V_7 -> V_23 , & V_5 -> V_23 , L_9 , L_10 ) ;
if ( ! error ) {
struct V_109 * V_107 = V_7 -> type -> V_114 ;
if ( V_107 ) {
while ( V_107 -> V_107 . V_12 ) {
if ( F_104 ( & V_7 -> V_23 , & V_107 -> V_107 ) )
break;
V_107 ++ ;
}
}
F_105 ( & V_7 -> V_23 , V_115 ) ;
V_7 -> V_116 = 1 ;
}
return error ;
}
void F_106 ( struct V_4 * V_5 )
{
if ( V_5 ) {
struct V_6 * V_7 = V_5 -> V_8 ;
F_105 ( & V_7 -> V_23 , V_117 ) ;
F_107 ( & V_7 -> V_23 ) ;
V_7 -> V_116 = 0 ;
}
}
int F_108 ( struct V_11 * V_7 )
{
char * V_118 = L_11 ;
if ( V_7 -> V_119 ) {
if ( F_109 ( V_7 -> V_119 < sizeof( struct V_120 ) ) ||
F_109 ( V_7 -> V_121 < F_110 ( struct V_120 ) ) )
return - V_32 ;
snprintf ( V_7 -> V_122 , sizeof( V_7 -> V_122 ) ,
L_12 , V_7 -> V_13 ) ;
V_7 -> V_123 = F_111 ( V_7 -> V_122 , V_7 -> V_119 ,
V_7 -> V_121 , 0 , NULL ) ;
if ( ! V_7 -> V_123 )
return - V_124 ;
}
F_9 ( & V_16 ) ;
if ( F_4 ( V_7 -> V_13 ) ) {
F_10 ( & V_16 ) ;
if ( V_7 -> V_123 )
F_112 ( V_7 -> V_123 ) ;
return - V_125 ;
}
F_60 ( & V_7 -> V_126 , & V_127 ) ;
F_10 ( & V_16 ) ;
if ( ! strcmp ( V_7 -> V_13 , V_18 ) ||
( ! * V_18 &&
! strcmp ( V_7 -> V_13 , V_34 ) ) )
V_118 = L_13 ;
F_26 ( V_128 L_14 , V_7 -> V_13 ,
V_118 ) ;
return 0 ;
}
void F_113 ( struct V_11 * V_7 )
{
F_9 ( & V_16 ) ;
F_114 ( & V_7 -> V_126 ) ;
F_10 ( & V_16 ) ;
if ( V_7 -> V_123 ) {
F_115 () ;
F_112 ( V_7 -> V_123 ) ;
V_7 -> V_123 = NULL ;
}
}
static int F_116 ( struct V_4 * V_5 , struct V_11 * V_129 )
{
struct V_6 * V_130 = V_5 -> V_8 ;
bool V_116 = V_130 -> V_116 ;
int V_22 ;
F_117 ( V_5 ) ;
if ( V_116 )
F_106 ( V_5 ) ;
F_118 ( V_5 -> V_81 ) ;
F_119 ( V_5 ) ;
F_120 ( V_5 -> V_81 ) ;
V_22 = V_129 -> V_9 . V_35 ( V_5 , V_129 ) ;
if ( V_22 )
goto V_131;
if ( V_116 ) {
V_22 = F_102 ( V_5 ) ;
if ( V_22 )
goto V_132;
}
F_27 ( V_130 ) ;
F_121 ( V_5 ) ;
F_122 ( V_5 , L_15 , V_129 -> V_13 ) ;
return 0 ;
V_132:
F_27 ( V_5 -> V_8 ) ;
V_131:
V_5 -> V_8 = V_130 ;
F_102 ( V_5 ) ;
F_121 ( V_5 ) ;
return V_22 ;
}
int F_123 ( struct V_4 * V_5 , const char * V_12 )
{
char V_13 [ V_133 ] ;
struct V_11 * V_7 ;
if ( ! V_5 -> V_8 )
return - V_134 ;
F_124 ( V_13 , V_12 , sizeof( V_13 ) ) ;
V_7 = F_8 ( F_125 ( V_13 ) , true ) ;
if ( ! V_7 ) {
F_26 ( V_33 L_16 , V_13 ) ;
return - V_32 ;
}
if ( ! strcmp ( V_13 , V_5 -> V_8 -> type -> V_13 ) ) {
F_6 ( V_7 ) ;
return 0 ;
}
return F_116 ( V_5 , V_7 ) ;
}
T_4 F_126 ( struct V_4 * V_5 , const char * V_12 ,
T_5 V_135 )
{
int V_62 ;
if ( ! V_5 -> V_8 )
return V_135 ;
V_62 = F_123 ( V_5 , V_12 ) ;
if ( ! V_62 )
return V_135 ;
F_26 ( V_33 L_17 , V_12 ) ;
return V_62 ;
}
T_4 F_127 ( struct V_4 * V_5 , char * V_12 )
{
struct V_6 * V_7 = V_5 -> V_8 ;
struct V_11 * V_136 ;
struct V_11 * V_137 ;
int V_138 = 0 ;
if ( ! V_5 -> V_8 || ! F_128 ( V_5 ) )
return sprintf ( V_12 , L_18 ) ;
V_136 = V_7 -> type ;
F_9 ( & V_16 ) ;
F_5 (__e, &elv_list, list) {
if ( ! strcmp ( V_136 -> V_13 , V_137 -> V_13 ) )
V_138 += sprintf ( V_12 + V_138 , L_19 , V_136 -> V_13 ) ;
else
V_138 += sprintf ( V_12 + V_138 , L_20 , V_137 -> V_13 ) ;
}
F_10 ( & V_16 ) ;
V_138 += sprintf ( V_138 + V_12 , L_21 ) ;
return V_138 ;
}
struct V_1 * F_129 ( struct V_4 * V_5 ,
struct V_1 * V_2 )
{
struct V_42 * V_139 = F_130 ( & V_2 -> V_42 ) ;
if ( V_139 )
return F_131 ( V_139 ) ;
return NULL ;
}
struct V_1 * F_132 ( struct V_4 * V_5 ,
struct V_1 * V_2 )
{
struct V_42 * V_140 = F_133 ( & V_2 -> V_42 ) ;
if ( V_140 )
return F_131 ( V_140 ) ;
return NULL ;
}
