void
F_1 (
T_1 * V_1 ,
int V_2 ,
T_2 V_3 )
{
T_3 V_4 ;
T_4 V_5 ;
int V_6 ;
for ( V_6 = 0 ; V_6 < V_2 ; V_6 ++ ) {
T_4 * V_7 = F_2 ( V_1 , V_6 ) ;
V_5 . V_8 = F_3 ( & V_7 -> V_8 ) ;
V_5 . V_9 = F_3 ( & V_7 -> V_9 ) ;
F_4 ( & V_5 , & V_4 ) ;
if ( V_3 == V_10 )
ASSERT ( V_4 . V_11 == V_12 ) ;
}
}
int
F_5 (
T_5 * V_13 ,
T_6 * V_14 )
{
T_7 * V_15 ;
int V_16 ;
int error = 0 ;
T_8 V_17 ;
if ( F_6 ( F_7 ( V_14 -> V_18 ) +
F_8 ( V_14 -> V_19 ) >
F_9 ( V_14 -> V_20 ) ) ) {
F_10 ( V_13 -> V_21 ,
L_1 ,
( unsigned long long ) V_13 -> V_22 ,
( int ) ( F_7 ( V_14 -> V_18 ) +
F_8 ( V_14 -> V_19 ) ) ,
( unsigned long long )
F_9 ( V_14 -> V_20 ) ) ;
F_11 ( L_2 , V_23 ,
V_13 -> V_21 , V_14 ) ;
return F_12 ( V_24 ) ;
}
if ( F_6 ( V_14 -> V_25 > V_13 -> V_21 -> V_26 . V_27 ) ) {
F_10 ( V_13 -> V_21 , L_3 ,
( unsigned long long ) V_13 -> V_22 ,
V_14 -> V_25 ) ;
F_11 ( L_4 , V_23 ,
V_13 -> V_21 , V_14 ) ;
return F_12 ( V_24 ) ;
}
if ( F_6 ( ( V_13 -> V_28 . V_29 & V_30 ) &&
! V_13 -> V_21 -> V_31 ) ) {
F_10 ( V_13 -> V_21 ,
L_5 ,
V_13 -> V_22 ) ;
F_11 ( L_6 ,
V_23 , V_13 -> V_21 , V_14 ) ;
return F_12 ( V_24 ) ;
}
switch ( V_13 -> V_28 . V_32 & V_33 ) {
case V_34 :
case V_35 :
case V_36 :
case V_37 :
if ( F_6 ( V_14 -> V_38 != V_39 ) ) {
F_11 ( L_7 , V_23 ,
V_13 -> V_21 , V_14 ) ;
return F_12 ( V_24 ) ;
}
V_13 -> V_28 . V_17 = 0 ;
V_13 -> V_40 . V_41 . V_42 = F_13 ( V_14 ) ;
break;
case V_43 :
case V_44 :
case V_45 :
switch ( V_14 -> V_38 ) {
case V_46 :
if ( F_6 ( F_14 ( F_8 ( V_14 -> V_32 ) ) ) ) {
F_10 ( V_13 -> V_21 ,
L_8 ,
( unsigned long long ) V_13 -> V_22 ) ;
F_11 ( L_9 ,
V_23 ,
V_13 -> V_21 , V_14 ) ;
return F_12 ( V_24 ) ;
}
V_17 = F_9 ( V_14 -> V_17 ) ;
if ( F_6 ( V_17 < 0 ||
V_17 > F_15 ( V_14 , V_13 -> V_21 ) ) ) {
F_10 ( V_13 -> V_21 ,
L_10 ,
( unsigned long long ) V_13 -> V_22 ,
( long long ) V_17 ) ;
F_11 ( L_11 ,
V_23 ,
V_13 -> V_21 , V_14 ) ;
return F_12 ( V_24 ) ;
}
V_16 = ( int ) V_17 ;
error = F_16 ( V_13 , V_14 , V_47 , V_16 ) ;
break;
case V_48 :
error = F_17 ( V_13 , V_14 , V_47 ) ;
break;
case V_49 :
error = F_18 ( V_13 , V_14 , V_47 ) ;
break;
default:
F_19 ( L_12 , V_23 ,
V_13 -> V_21 ) ;
return F_12 ( V_24 ) ;
}
break;
default:
F_19 ( L_13 , V_23 , V_13 -> V_21 ) ;
return F_12 ( V_24 ) ;
}
if ( error ) {
return error ;
}
if ( ! F_20 ( V_14 ) )
return 0 ;
ASSERT ( V_13 -> V_50 == NULL ) ;
V_13 -> V_50 = F_21 ( V_51 , V_52 | V_53 ) ;
switch ( V_14 -> V_54 ) {
case V_46 :
V_15 = ( T_7 * ) F_22 ( V_14 ) ;
V_16 = F_8 ( V_15 -> V_55 . V_56 ) ;
if ( F_6 ( V_16 < sizeof( struct V_57 ) ) ) {
F_10 ( V_13 -> V_21 ,
L_14 ,
( unsigned long long ) V_13 -> V_22 ,
( long long ) V_16 ) ;
F_11 ( L_15 ,
V_23 ,
V_13 -> V_21 , V_14 ) ;
return F_12 ( V_24 ) ;
}
error = F_16 ( V_13 , V_14 , V_58 , V_16 ) ;
break;
case V_48 :
error = F_17 ( V_13 , V_14 , V_58 ) ;
break;
case V_49 :
error = F_18 ( V_13 , V_14 , V_58 ) ;
break;
default:
error = F_12 ( V_24 ) ;
break;
}
if ( error ) {
F_23 ( V_51 , V_13 -> V_50 ) ;
V_13 -> V_50 = NULL ;
F_24 ( V_13 , V_47 ) ;
}
return error ;
}
STATIC int
F_16 (
T_5 * V_13 ,
T_6 * V_14 ,
int V_59 ,
int V_16 )
{
T_1 * V_1 ;
int V_60 ;
if ( F_6 ( V_16 > F_25 ( V_14 , V_13 -> V_21 , V_59 ) ) ) {
F_10 ( V_13 -> V_21 ,
L_16 ,
( unsigned long long ) V_13 -> V_22 , V_16 ,
F_25 ( V_14 , V_13 -> V_21 , V_59 ) ) ;
F_11 ( L_17 , V_23 ,
V_13 -> V_21 , V_14 ) ;
return F_12 ( V_24 ) ;
}
V_1 = F_26 ( V_13 , V_59 ) ;
V_60 = 0 ;
if ( V_16 == 0 )
V_1 -> V_61 . V_62 = NULL ;
else if ( V_16 <= sizeof( V_1 -> V_41 . V_63 ) )
V_1 -> V_61 . V_62 = V_1 -> V_41 . V_63 ;
else {
V_60 = F_27 ( V_16 , 4 ) ;
V_1 -> V_61 . V_62 = F_28 ( V_60 , V_52 | V_53 ) ;
}
V_1 -> V_64 = V_16 ;
V_1 -> V_65 = V_60 ;
if ( V_16 )
memcpy ( V_1 -> V_61 . V_62 , F_29 ( V_14 , V_59 ) , V_16 ) ;
V_1 -> V_66 &= ~ V_67 ;
V_1 -> V_66 |= V_68 ;
return 0 ;
}
STATIC int
F_17 (
T_5 * V_13 ,
T_6 * V_14 ,
int V_59 )
{
T_9 * V_69 ;
T_1 * V_1 ;
int V_70 ;
int V_16 ;
int V_6 ;
V_1 = F_26 ( V_13 , V_59 ) ;
V_70 = F_30 ( V_14 , V_59 ) ;
V_16 = V_70 * ( V_71 ) sizeof( T_9 ) ;
if ( F_6 ( V_16 < 0 || V_16 > F_25 ( V_14 , V_13 -> V_21 , V_59 ) ) ) {
F_10 ( V_13 -> V_21 , L_18 ,
( unsigned long long ) V_13 -> V_22 , V_70 ) ;
F_11 ( L_19 , V_23 ,
V_13 -> V_21 , V_14 ) ;
return F_12 ( V_24 ) ;
}
V_1 -> V_65 = 0 ;
if ( V_70 == 0 )
V_1 -> V_61 . V_72 = NULL ;
else if ( V_70 <= V_73 )
V_1 -> V_61 . V_72 = V_1 -> V_41 . V_74 ;
else
F_31 ( V_1 , 0 , V_70 ) ;
V_1 -> V_64 = V_16 ;
if ( V_16 ) {
V_69 = ( T_9 * ) F_29 ( V_14 , V_59 ) ;
F_1 ( V_1 , V_70 , F_32 ( V_13 ) ) ;
for ( V_6 = 0 ; V_6 < V_70 ; V_6 ++ , V_69 ++ ) {
T_4 * V_7 = F_2 ( V_1 , V_6 ) ;
V_7 -> V_8 = F_33 ( & V_69 -> V_8 ) ;
V_7 -> V_9 = F_33 ( & V_69 -> V_9 ) ;
}
F_34 ( V_13 , V_70 , V_59 ) ;
if ( V_59 != V_47 ||
F_32 ( V_13 ) == V_10 )
if ( F_6 ( F_35 (
V_1 , 0 , V_70 ) ) ) {
F_19 ( L_20 ,
V_23 ,
V_13 -> V_21 ) ;
return F_12 ( V_24 ) ;
}
}
V_1 -> V_66 |= V_67 ;
return 0 ;
}
STATIC int
F_18 (
T_5 * V_13 ,
T_6 * V_14 ,
int V_59 )
{
struct V_75 * V_76 = V_13 -> V_21 ;
T_10 * V_77 ;
T_1 * V_1 ;
int V_2 ;
int V_16 ;
V_1 = F_26 ( V_13 , V_59 ) ;
V_77 = ( T_10 * ) F_29 ( V_14 , V_59 ) ;
V_16 = F_36 ( V_76 , V_77 ) ;
V_2 = F_8 ( V_77 -> V_78 ) ;
if ( F_6 ( F_37 ( V_13 , V_59 ) <=
F_38 ( V_13 , V_59 ) ||
F_39 ( V_2 ) >
F_25 ( V_14 , V_76 , V_59 ) ||
F_37 ( V_13 , V_59 ) > V_13 -> V_28 . V_20 ) ) {
F_10 ( V_76 , L_21 ,
( unsigned long long ) V_13 -> V_22 ) ;
F_11 ( L_22 , V_23 ,
V_76 , V_14 ) ;
return F_12 ( V_24 ) ;
}
V_1 -> V_79 = V_16 ;
V_1 -> V_80 = F_28 ( V_16 , V_52 | V_53 ) ;
ASSERT ( V_1 -> V_80 != NULL ) ;
F_40 ( V_13 , V_77 , F_25 ( V_14 , V_13 -> V_21 , V_59 ) ,
V_1 -> V_80 , V_16 ) ;
V_1 -> V_66 &= ~ V_67 ;
V_1 -> V_66 |= V_81 ;
return 0 ;
}
int
F_41 (
T_11 * V_82 ,
T_5 * V_13 ,
int V_59 )
{
int error ;
T_1 * V_1 ;
T_12 V_83 ;
if ( F_6 ( F_42 ( V_13 , V_59 ) != V_49 ) ) {
F_19 ( L_23 , V_23 ,
V_13 -> V_21 ) ;
return F_12 ( V_24 ) ;
}
V_83 = F_37 ( V_13 , V_59 ) ;
V_1 = F_26 ( V_13 , V_59 ) ;
V_1 -> V_64 = V_1 -> V_65 = 0 ;
V_1 -> V_66 |= V_67 ;
F_31 ( V_1 , 0 , V_83 ) ;
error = F_43 ( V_82 , V_13 , V_59 ) ;
if ( error ) {
F_44 ( V_1 ) ;
V_1 -> V_66 &= ~ V_67 ;
return error ;
}
F_1 ( V_1 , V_83 , F_32 ( V_13 ) ) ;
return 0 ;
}
void
F_45 (
T_5 * V_13 ,
int V_84 ,
int V_59 )
{
struct V_75 * V_76 = V_13 -> V_21 ;
int V_85 ;
T_1 * V_1 ;
struct V_86 * V_87 ;
int V_88 ;
T_13 V_89 ;
char * V_90 ;
char * V_91 ;
if ( V_84 == 0 ) {
return;
}
V_1 = F_26 ( V_13 , V_59 ) ;
if ( V_84 > 0 ) {
if ( V_1 -> V_79 == 0 ) {
V_89 = F_46 ( V_76 , V_84 ) ;
V_1 -> V_80 = F_28 ( V_89 , V_52 | V_53 ) ;
V_1 -> V_79 = ( int ) V_89 ;
return;
}
V_85 = F_47 ( V_76 , V_1 -> V_79 , 0 ) ;
V_88 = V_85 + V_84 ;
V_89 = F_46 ( V_76 , V_88 ) ;
V_1 -> V_80 = F_48 ( V_1 -> V_80 , V_89 ,
F_46 ( V_76 , V_85 ) ,
V_52 | V_53 ) ;
V_91 = ( char * ) F_49 ( V_76 , V_1 -> V_80 , 1 ,
V_1 -> V_79 ) ;
V_90 = ( char * ) F_49 ( V_76 , V_1 -> V_80 , 1 ,
( int ) V_89 ) ;
V_1 -> V_79 = ( int ) V_89 ;
ASSERT ( F_50 ( V_1 -> V_80 ) <=
F_51 ( V_13 , V_59 ) ) ;
memmove ( V_90 , V_91 , V_85 * ( V_71 ) sizeof( V_92 ) ) ;
return;
}
ASSERT ( ( V_1 -> V_80 != NULL ) && ( V_1 -> V_79 > 0 ) ) ;
V_85 = F_47 ( V_76 , V_1 -> V_79 , 0 ) ;
V_88 = V_85 + V_84 ;
ASSERT ( V_88 >= 0 ) ;
if ( V_88 > 0 )
V_89 = F_46 ( V_76 , V_88 ) ;
else
V_89 = 0 ;
if ( V_89 > 0 ) {
V_87 = F_28 ( V_89 , V_52 | V_53 ) ;
memcpy ( V_87 , V_1 -> V_80 ,
F_52 ( V_13 -> V_21 ) ) ;
} else {
V_87 = NULL ;
V_1 -> V_66 &= ~ V_81 ;
}
if ( V_88 > 0 ) {
V_91 = ( char * ) F_53 ( V_76 , V_1 -> V_80 , 1 ) ;
V_90 = ( char * ) F_53 ( V_76 , V_87 , 1 ) ;
memcpy ( V_90 , V_91 , V_88 * ( V_71 ) sizeof( T_9 ) ) ;
V_91 = ( char * ) F_49 ( V_76 , V_1 -> V_80 , 1 ,
V_1 -> V_79 ) ;
V_90 = ( char * ) F_49 ( V_76 , V_87 , 1 ,
( int ) V_89 ) ;
memcpy ( V_90 , V_91 , V_88 * ( V_71 ) sizeof( V_92 ) ) ;
}
F_54 ( V_1 -> V_80 ) ;
V_1 -> V_80 = V_87 ;
V_1 -> V_79 = ( int ) V_89 ;
if ( V_1 -> V_80 )
ASSERT ( F_50 ( V_1 -> V_80 ) <=
F_51 ( V_13 , V_59 ) ) ;
return;
}
void
F_55 (
T_5 * V_13 ,
int V_93 ,
int V_59 )
{
T_1 * V_1 ;
int V_89 ;
int V_60 ;
if ( V_93 == 0 ) {
return;
}
V_1 = F_26 ( V_13 , V_59 ) ;
V_89 = ( int ) V_1 -> V_64 + V_93 ;
ASSERT ( V_89 >= 0 ) ;
if ( V_89 == 0 ) {
if ( V_1 -> V_61 . V_62 != V_1 -> V_41 . V_63 ) {
F_54 ( V_1 -> V_61 . V_62 ) ;
}
V_1 -> V_61 . V_62 = NULL ;
V_60 = 0 ;
} else if ( V_89 <= sizeof( V_1 -> V_41 . V_63 ) ) {
if ( V_1 -> V_61 . V_62 == NULL ) {
V_1 -> V_61 . V_62 = V_1 -> V_41 . V_63 ;
} else if ( V_1 -> V_61 . V_62 != V_1 -> V_41 . V_63 ) {
ASSERT ( V_1 -> V_65 != 0 ) ;
memcpy ( V_1 -> V_41 . V_63 , V_1 -> V_61 . V_62 ,
V_89 ) ;
F_54 ( V_1 -> V_61 . V_62 ) ;
V_1 -> V_61 . V_62 = V_1 -> V_41 . V_63 ;
}
V_60 = 0 ;
} else {
V_60 = F_27 ( V_89 , 4 ) ;
if ( V_1 -> V_61 . V_62 == NULL ) {
ASSERT ( V_1 -> V_65 == 0 ) ;
V_1 -> V_61 . V_62 = F_28 ( V_60 ,
V_52 | V_53 ) ;
} else if ( V_1 -> V_61 . V_62 != V_1 -> V_41 . V_63 ) {
if ( V_1 -> V_65 != V_60 ) {
V_1 -> V_61 . V_62 =
F_48 ( V_1 -> V_61 . V_62 ,
V_60 ,
V_1 -> V_65 ,
V_52 | V_53 ) ;
}
} else {
ASSERT ( V_1 -> V_65 == 0 ) ;
V_1 -> V_61 . V_62 = F_28 ( V_60 ,
V_52 | V_53 ) ;
memcpy ( V_1 -> V_61 . V_62 , V_1 -> V_41 . V_63 ,
V_1 -> V_64 ) ;
}
}
V_1 -> V_65 = V_60 ;
V_1 -> V_64 = V_89 ;
ASSERT ( V_1 -> V_64 <= F_51 ( V_13 , V_59 ) ) ;
}
void
F_24 (
T_5 * V_13 ,
int V_59 )
{
T_1 * V_1 ;
V_1 = F_26 ( V_13 , V_59 ) ;
if ( V_1 -> V_80 != NULL ) {
F_54 ( V_1 -> V_80 ) ;
V_1 -> V_80 = NULL ;
}
if ( F_42 ( V_13 , V_59 ) == V_46 ) {
if ( ( V_1 -> V_61 . V_62 != V_1 -> V_41 . V_63 ) &&
( V_1 -> V_61 . V_62 != NULL ) ) {
ASSERT ( V_1 -> V_65 != 0 ) ;
F_54 ( V_1 -> V_61 . V_62 ) ;
V_1 -> V_61 . V_62 = NULL ;
V_1 -> V_65 = 0 ;
}
} else if ( ( V_1 -> V_66 & V_67 ) &&
( ( V_1 -> V_66 & V_94 ) ||
( ( V_1 -> V_61 . V_72 != NULL ) &&
( V_1 -> V_61 . V_72 != V_1 -> V_41 . V_74 ) ) ) ) {
ASSERT ( V_1 -> V_65 != 0 ) ;
F_44 ( V_1 ) ;
}
ASSERT ( V_1 -> V_61 . V_72 == NULL ||
V_1 -> V_61 . V_72 == V_1 -> V_41 . V_74 ) ;
ASSERT ( V_1 -> V_65 == 0 ) ;
if ( V_59 == V_58 ) {
F_23 ( V_51 , V_13 -> V_50 ) ;
V_13 -> V_50 = NULL ;
}
}
int
F_56 (
T_5 * V_13 ,
T_9 * V_69 ,
int V_59 )
{
int V_95 ;
int V_6 ;
T_1 * V_1 ;
int V_2 ;
T_14 V_96 ;
V_1 = F_26 ( V_13 , V_59 ) ;
ASSERT ( F_57 ( V_13 , V_97 | V_98 ) ) ;
ASSERT ( V_1 -> V_64 > 0 ) ;
V_2 = V_1 -> V_64 / ( V_71 ) sizeof( T_9 ) ;
F_34 ( V_13 , V_2 , V_59 ) ;
ASSERT ( V_2 > 0 ) ;
V_95 = 0 ;
for ( V_6 = 0 ; V_6 < V_2 ; V_6 ++ ) {
T_4 * V_7 = F_2 ( V_1 , V_6 ) ;
V_96 = F_58 ( V_7 ) ;
if ( F_59 ( V_96 ) ) {
continue;
}
F_60 ( V_7 -> V_8 , & V_69 -> V_8 ) ;
F_60 ( V_7 -> V_9 , & V_69 -> V_9 ) ;
V_69 ++ ;
V_95 ++ ;
}
ASSERT ( V_95 != 0 ) ;
F_1 ( V_1 , V_95 , F_32 ( V_13 ) ) ;
return ( V_95 * ( V_71 ) sizeof( T_9 ) ) ;
}
void
F_61 (
T_5 * V_13 ,
T_6 * V_14 ,
T_15 * V_99 ,
int V_59 ,
T_16 * V_100 )
{
char * V_101 ;
T_1 * V_1 ;
T_17 * V_76 ;
static const short V_102 [ 2 ] =
{ V_103 , V_104 } ;
static const short V_105 [ 2 ] =
{ V_106 , V_107 } ;
static const short V_108 [ 2 ] =
{ V_109 , V_110 } ;
if ( ! V_99 )
return;
V_1 = F_26 ( V_13 , V_59 ) ;
if ( ! V_1 ) {
ASSERT ( V_59 == V_58 ) ;
return;
}
V_101 = F_29 ( V_14 , V_59 ) ;
V_76 = V_13 -> V_21 ;
switch ( F_42 ( V_13 , V_59 ) ) {
case V_46 :
if ( ( V_99 -> V_111 & V_105 [ V_59 ] ) &&
( V_1 -> V_64 > 0 ) ) {
ASSERT ( V_1 -> V_61 . V_62 != NULL ) ;
ASSERT ( V_1 -> V_64 <= F_51 ( V_13 , V_59 ) ) ;
memcpy ( V_101 , V_1 -> V_61 . V_62 , V_1 -> V_64 ) ;
}
break;
case V_48 :
ASSERT ( ( V_1 -> V_66 & V_67 ) ||
! ( V_99 -> V_111 & V_108 [ V_59 ] ) ) ;
if ( ( V_99 -> V_111 & V_108 [ V_59 ] ) &&
( V_1 -> V_64 > 0 ) ) {
ASSERT ( F_2 ( V_1 , 0 ) ) ;
ASSERT ( F_37 ( V_13 , V_59 ) > 0 ) ;
( void ) F_56 ( V_13 , ( T_9 * ) V_101 ,
V_59 ) ;
}
break;
case V_49 :
if ( ( V_99 -> V_111 & V_102 [ V_59 ] ) &&
( V_1 -> V_79 > 0 ) ) {
ASSERT ( V_1 -> V_80 != NULL ) ;
ASSERT ( F_50 ( V_1 -> V_80 ) <=
F_51 ( V_13 , V_59 ) ) ;
F_62 ( V_76 , V_1 -> V_80 , V_1 -> V_79 ,
( T_10 * ) V_101 ,
F_25 ( V_14 , V_76 , V_59 ) ) ;
}
break;
case V_39 :
if ( V_99 -> V_111 & V_112 ) {
ASSERT ( V_59 == V_47 ) ;
F_63 ( V_14 , V_13 -> V_40 . V_41 . V_42 ) ;
}
break;
case V_113 :
if ( V_99 -> V_111 & V_114 ) {
ASSERT ( V_59 == V_47 ) ;
memcpy ( F_64 ( V_14 ) ,
& V_13 -> V_40 . V_41 . V_115 ,
sizeof( V_116 ) ) ;
}
break;
default:
ASSERT ( 0 ) ;
break;
}
}
T_4 *
F_2 (
T_1 * V_1 ,
T_12 V_117 )
{
ASSERT ( V_117 >= 0 ) ;
ASSERT ( V_117 < V_1 -> V_64 / sizeof( T_9 ) ) ;
if ( ( V_1 -> V_66 & V_94 ) && ( V_117 == 0 ) ) {
return V_1 -> V_61 . V_118 -> V_119 ;
} else if ( V_1 -> V_66 & V_94 ) {
T_18 * V_120 ;
int V_121 = 0 ;
T_12 V_122 = V_117 ;
V_120 = F_65 ( V_1 , & V_122 , & V_121 , 0 ) ;
return & V_120 -> V_119 [ V_122 ] ;
} else if ( V_1 -> V_64 ) {
return & V_1 -> V_61 . V_72 [ V_117 ] ;
} else {
return NULL ;
}
}
void
F_66 (
T_5 * V_13 ,
T_12 V_117 ,
T_12 V_123 ,
T_3 * V_124 ,
int V_125 )
{
T_1 * V_1 = ( V_125 & V_126 ) ? V_13 -> V_50 : & V_13 -> V_40 ;
T_12 V_6 ;
F_67 ( V_13 , V_117 , V_124 , V_125 , V_127 ) ;
ASSERT ( V_1 -> V_66 & V_67 ) ;
F_31 ( V_1 , V_117 , V_123 ) ;
for ( V_6 = V_117 ; V_6 < V_117 + V_123 ; V_6 ++ , V_124 ++ )
F_68 ( F_2 ( V_1 , V_6 ) , V_124 ) ;
}
void
F_31 (
T_1 * V_1 ,
T_12 V_117 ,
int V_128 )
{
int V_93 ;
int V_89 ;
T_12 V_83 ;
V_83 = V_1 -> V_64 / ( V_71 ) sizeof( T_9 ) ;
ASSERT ( ( V_117 >= 0 ) && ( V_117 <= V_83 ) ) ;
V_93 = V_128 * sizeof( T_9 ) ;
V_89 = V_1 -> V_64 + V_93 ;
if ( V_83 + V_128 <= V_73 ) {
if ( V_117 < V_83 ) {
memmove ( & V_1 -> V_41 . V_74 [ V_117 + V_128 ] ,
& V_1 -> V_41 . V_74 [ V_117 ] ,
( V_83 - V_117 ) * sizeof( T_9 ) ) ;
memset ( & V_1 -> V_41 . V_74 [ V_117 ] , 0 , V_93 ) ;
}
V_1 -> V_61 . V_72 = V_1 -> V_41 . V_74 ;
V_1 -> V_65 = 0 ;
}
else if ( V_83 + V_128 <= V_129 ) {
F_69 ( V_1 , V_89 ) ;
if ( V_117 < V_83 ) {
memmove ( & V_1 -> V_61 . V_72 [ V_117 + V_128 ] ,
& V_1 -> V_61 . V_72 [ V_117 ] ,
( V_83 - V_117 ) * sizeof( T_9 ) ) ;
memset ( & V_1 -> V_61 . V_72 [ V_117 ] , 0 , V_93 ) ;
}
}
else {
T_18 * V_120 ;
int V_121 = 0 ;
int V_122 = V_117 ;
ASSERT ( V_83 + V_128 > V_129 ) ;
if ( V_1 -> V_66 & V_94 ) {
V_120 = F_65 ( V_1 , & V_122 , & V_121 , 1 ) ;
} else {
F_70 ( V_1 ) ;
ASSERT ( V_1 -> V_66 & V_94 ) ;
V_120 = V_1 -> V_61 . V_118 ;
}
if ( V_120 && V_120 -> V_130 + V_128 <= V_129 ) {
if ( V_122 < V_120 -> V_130 ) {
memmove ( & V_120 -> V_119 [ V_122 + V_128 ] ,
& V_120 -> V_119 [ V_122 ] ,
( V_120 -> V_130 - V_122 ) *
sizeof( T_9 ) ) ;
memset ( & V_120 -> V_119 [ V_122 ] , 0 , V_93 ) ;
}
V_120 -> V_130 += V_128 ;
F_71 ( V_1 , V_121 + 1 , V_128 ) ;
}
else if ( V_120 ) {
F_72 ( V_1 ,
V_121 , V_122 , V_128 ) ;
}
else {
int V_123 = V_128 ;
while ( V_123 ) {
V_120 = F_73 ( V_1 , V_121 ) ;
V_120 -> V_130 = V_123 ;
V_123 -= F_74 ( V_123 , ( int ) V_129 ) ;
if ( V_123 ) {
V_121 ++ ;
}
}
}
}
V_1 -> V_64 = V_89 ;
}
void
F_72 (
T_1 * V_1 ,
int V_121 ,
T_12 V_117 ,
int V_123 )
{
int V_93 ;
T_18 * V_120 ;
T_12 V_128 ;
T_12 V_131 ;
T_12 V_132 ;
T_9 * V_133 = NULL ;
int V_134 ;
ASSERT ( V_1 -> V_66 & V_94 ) ;
V_120 = & V_1 -> V_61 . V_118 [ V_121 ] ;
V_132 = V_120 -> V_130 - V_117 ;
V_134 = V_1 -> V_65 / V_135 ;
if ( V_132 ) {
V_93 = V_132 * sizeof( T_9 ) ;
V_133 = ( T_9 * ) F_28 ( V_93 , V_53 ) ;
memmove ( V_133 , & V_120 -> V_119 [ V_117 ] , V_93 ) ;
V_120 -> V_130 -= V_132 ;
F_71 ( V_1 , V_121 + 1 , - V_132 ) ;
memset ( & V_120 -> V_119 [ V_117 ] , 0 , V_93 ) ;
}
V_131 = V_123 ;
V_128 = F_74 ( V_131 , ( int ) V_129 - V_120 -> V_130 ) ;
if ( V_128 ) {
V_120 -> V_130 += V_128 ;
F_71 ( V_1 , V_121 + 1 , V_128 ) ;
V_131 -= V_128 ;
}
while ( V_131 ) {
V_121 ++ ;
V_120 = F_73 ( V_1 , V_121 ) ;
V_128 = F_74 ( V_131 , ( int ) V_129 ) ;
V_120 -> V_130 = V_128 ;
F_71 ( V_1 , V_121 + 1 , V_128 ) ;
V_131 -= V_128 ;
}
if ( V_132 ) {
T_12 V_136 ;
int V_6 ;
V_93 = V_132 * sizeof( T_9 ) ;
V_136 = V_129 - V_120 -> V_130 ;
V_6 = 0 ;
if ( V_132 <= V_136 ) {
V_6 = V_120 -> V_130 ;
}
else if ( ( V_121 < V_134 - 1 ) &&
( V_132 <= ( V_136 = V_129 -
V_1 -> V_61 . V_118 [ V_121 + 1 ] . V_130 ) ) ) {
V_121 ++ ;
V_120 ++ ;
memmove ( & V_120 -> V_119 [ V_132 ] , V_120 -> V_119 ,
V_120 -> V_130 * sizeof( T_9 ) ) ;
}
else {
V_121 ++ ;
V_120 = F_73 ( V_1 , V_121 ) ;
}
memmove ( & V_120 -> V_119 [ V_6 ] , V_133 , V_93 ) ;
F_54 ( V_133 ) ;
V_120 -> V_130 += V_132 ;
F_71 ( V_1 , V_121 + 1 , V_132 ) ;
}
}
void
F_75 (
T_5 * V_13 ,
T_12 V_117 ,
int V_128 ,
int V_125 )
{
T_1 * V_1 = ( V_125 & V_126 ) ? V_13 -> V_50 : & V_13 -> V_40 ;
T_12 V_83 ;
int V_89 ;
F_76 ( V_13 , V_117 , V_125 , V_127 ) ;
ASSERT ( V_128 > 0 ) ;
V_83 = V_1 -> V_64 / ( V_71 ) sizeof( T_9 ) ;
V_89 = ( V_83 - V_128 ) * sizeof( T_9 ) ;
if ( V_89 == 0 ) {
F_44 ( V_1 ) ;
} else if ( V_1 -> V_66 & V_94 ) {
F_77 ( V_1 , V_117 , V_128 ) ;
} else if ( V_1 -> V_65 ) {
F_78 ( V_1 , V_117 , V_128 ) ;
} else {
F_79 ( V_1 , V_117 , V_128 ) ;
}
V_1 -> V_64 = V_89 ;
}
void
F_79 (
T_1 * V_1 ,
T_12 V_117 ,
int V_128 )
{
int V_83 ;
ASSERT ( ! ( V_1 -> V_66 & V_94 ) ) ;
ASSERT ( V_117 < V_73 ) ;
V_83 = V_1 -> V_64 / ( V_71 ) sizeof( T_9 ) ;
ASSERT ( ( ( V_83 - V_128 ) > 0 ) &&
( V_83 - V_128 ) < V_73 ) ;
if ( V_117 + V_128 < V_83 ) {
memmove ( & V_1 -> V_41 . V_74 [ V_117 ] ,
& V_1 -> V_41 . V_74 [ V_117 + V_128 ] ,
( V_83 - ( V_117 + V_128 ) ) *
sizeof( T_9 ) ) ;
memset ( & V_1 -> V_41 . V_74 [ V_83 - V_128 ] ,
0 , V_128 * sizeof( T_9 ) ) ;
} else {
memset ( & V_1 -> V_41 . V_74 [ V_117 ] , 0 ,
V_128 * sizeof( T_9 ) ) ;
}
}
void
F_78 (
T_1 * V_1 ,
T_12 V_117 ,
int V_128 )
{
T_12 V_83 ;
int V_89 ;
ASSERT ( ! ( V_1 -> V_66 & V_94 ) ) ;
V_89 = V_1 -> V_64 -
( V_128 * sizeof( T_9 ) ) ;
V_83 = V_1 -> V_64 / ( V_71 ) sizeof( T_9 ) ;
if ( V_89 == 0 ) {
F_44 ( V_1 ) ;
return;
}
if ( V_117 + V_128 < V_83 ) {
memmove ( & V_1 -> V_61 . V_72 [ V_117 ] ,
& V_1 -> V_61 . V_72 [ V_117 + V_128 ] ,
( V_83 - ( V_117 + V_128 ) ) *
sizeof( T_9 ) ) ;
}
memset ( & V_1 -> V_61 . V_72 [ V_83 - V_128 ] ,
0 , V_128 * sizeof( T_9 ) ) ;
F_69 ( V_1 , V_89 ) ;
V_1 -> V_64 = V_89 ;
}
void
F_77 (
T_1 * V_1 ,
T_12 V_117 ,
int V_123 )
{
T_18 * V_120 ;
int V_121 = 0 ;
T_12 V_131 ;
T_12 V_128 ;
T_12 V_137 ;
T_12 V_132 ;
int V_122 = V_117 ;
ASSERT ( V_1 -> V_66 & V_94 ) ;
V_120 = F_65 ( V_1 , & V_122 , & V_121 , 0 ) ;
ASSERT ( V_120 != NULL ) ;
V_137 = V_122 ;
V_131 = V_123 ;
while ( V_131 ) {
V_132 = F_80 ( ( V_120 -> V_130 - ( V_137 + V_131 ) ) , 0 ) ;
V_128 = F_74 ( V_131 , ( V_120 -> V_130 - V_137 ) ) ;
if ( V_128 == V_120 -> V_130 ) {
F_81 ( V_1 , V_121 ) ;
V_131 -= V_128 ;
V_137 = 0 ;
if ( V_131 ) {
ASSERT ( V_121 < V_1 -> V_65 /
V_135 ) ;
V_120 = & V_1 -> V_61 . V_118 [ V_121 ] ;
V_137 = 0 ;
continue;
} else {
break;
}
}
if ( V_132 ) {
memmove ( & V_120 -> V_119 [ V_137 ] ,
& V_120 -> V_119 [ V_137 + V_128 ] ,
V_132 * sizeof( T_9 ) ) ;
}
memset ( & V_120 -> V_119 [ V_137 + V_132 ] , 0 , ( V_135 -
( ( V_137 + V_132 ) * sizeof( T_9 ) ) ) ) ;
V_120 -> V_130 -= V_128 ;
F_71 ( V_1 , V_121 + 1 , - V_128 ) ;
V_131 -= V_128 ;
V_137 = 0 ;
V_121 ++ ;
V_120 ++ ;
}
V_1 -> V_64 -= V_123 * sizeof( T_9 ) ;
F_82 ( V_1 ) ;
}
void
F_69 (
T_1 * V_1 ,
int V_89 )
{
int V_138 ;
V_138 = V_89 ;
ASSERT ( ! ( V_1 -> V_66 & V_94 ) ||
( ( V_89 >= 0 ) && ( V_89 <= V_135 ) &&
( V_89 != V_1 -> V_65 ) ) ) ;
if ( V_89 == 0 ) {
F_44 ( V_1 ) ;
}
else if ( V_1 -> V_65 ) {
if ( V_89 <= V_73 * sizeof( T_9 ) ) {
F_83 ( V_1 , V_89 /
( V_71 ) sizeof( T_9 ) ) ;
V_1 -> V_64 = V_89 ;
return;
}
if ( ! F_84 ( V_89 ) ) {
V_138 = F_85 ( V_89 ) ;
}
if ( V_138 != V_1 -> V_65 ) {
V_1 -> V_61 . V_72 =
F_48 ( V_1 -> V_61 . V_72 ,
V_138 ,
V_1 -> V_65 , V_53 ) ;
}
if ( V_138 > V_1 -> V_65 ) {
memset ( & V_1 -> V_61 . V_72 [ V_1 -> V_64 /
( V_71 ) sizeof( T_9 ) ] , 0 ,
V_138 - V_1 -> V_65 ) ;
}
}
else {
V_89 += V_1 -> V_64 ;
if ( ! F_84 ( V_89 ) ) {
V_138 = F_85 ( V_89 ) ;
}
F_86 ( V_1 , V_138 ) ;
}
V_1 -> V_65 = V_138 ;
V_1 -> V_64 = V_89 ;
}
void
F_83 (
T_1 * V_1 ,
T_12 V_83 )
{
ASSERT ( V_1 -> V_66 & V_67 ) ;
ASSERT ( V_83 <= V_73 ) ;
memcpy ( V_1 -> V_41 . V_74 , V_1 -> V_61 . V_72 ,
V_83 * sizeof( T_9 ) ) ;
F_54 ( V_1 -> V_61 . V_72 ) ;
V_1 -> V_61 . V_72 = V_1 -> V_41 . V_74 ;
V_1 -> V_65 = 0 ;
}
void
F_86 (
T_1 * V_1 ,
int V_89 )
{
V_1 -> V_61 . V_72 = F_28 ( V_89 , V_53 ) ;
memset ( V_1 -> V_61 . V_72 , 0 , V_89 ) ;
if ( V_1 -> V_64 ) {
memcpy ( V_1 -> V_61 . V_72 , V_1 -> V_41 . V_74 ,
V_1 -> V_64 ) ;
memset ( V_1 -> V_41 . V_74 , 0 , V_73 *
sizeof( T_9 ) ) ;
}
V_1 -> V_65 = V_89 ;
}
STATIC void
F_87 (
T_1 * V_1 ,
int V_89 )
{
int V_134 ;
int V_16 ;
ASSERT ( V_1 -> V_66 & V_94 ) ;
V_134 = V_1 -> V_65 / V_135 ;
V_16 = V_134 * sizeof( T_18 ) ;
ASSERT ( V_1 -> V_65 ) ;
ASSERT ( ( V_89 >= 0 ) && ( V_89 != V_16 ) ) ;
if ( V_89 == 0 ) {
F_44 ( V_1 ) ;
} else {
V_1 -> V_61 . V_118 = ( T_18 * )
F_48 ( V_1 -> V_61 . V_118 ,
V_89 , V_16 , V_53 ) ;
}
}
STATIC void
F_88 (
T_1 * V_1 )
{
T_4 * V_7 ;
T_12 V_83 ;
int V_16 ;
ASSERT ( V_1 -> V_66 & V_94 ) ;
V_83 = V_1 -> V_64 / ( V_71 ) sizeof( T_9 ) ;
ASSERT ( V_83 <= V_129 ) ;
V_16 = V_83 * sizeof( T_9 ) ;
F_89 ( V_1 ) ;
ASSERT ( V_1 -> V_65 == V_135 ) ;
V_7 = V_1 -> V_61 . V_118 -> V_119 ;
F_54 ( V_1 -> V_61 . V_118 ) ;
V_1 -> V_66 &= ~ V_94 ;
V_1 -> V_61 . V_72 = V_7 ;
V_1 -> V_64 = V_16 ;
if ( V_83 < V_129 ) {
F_69 ( V_1 , V_16 ) ;
}
}
void
F_44 (
T_1 * V_1 )
{
if ( V_1 -> V_66 & V_94 ) {
int V_121 ;
int V_134 ;
V_134 = V_1 -> V_65 / V_135 ;
for ( V_121 = V_134 - 1 ; V_121 >= 0 ; V_121 -- ) {
F_81 ( V_1 , V_121 ) ;
}
V_1 -> V_66 &= ~ V_94 ;
} else if ( V_1 -> V_65 ) {
F_54 ( V_1 -> V_61 . V_72 ) ;
} else if ( V_1 -> V_64 ) {
memset ( V_1 -> V_41 . V_74 , 0 , V_73 *
sizeof( T_9 ) ) ;
}
V_1 -> V_61 . V_72 = NULL ;
V_1 -> V_65 = 0 ;
V_1 -> V_64 = 0 ;
}
T_4 *
F_90 (
T_1 * V_1 ,
T_19 V_139 ,
T_12 * V_140 )
{
T_4 * V_141 ;
T_20 V_142 = 0 ;
T_4 * V_7 = NULL ;
T_18 * V_120 = NULL ;
int V_143 ;
T_12 V_117 = 0 ;
int V_144 ;
T_12 V_83 ;
T_19 V_145 = 0 ;
V_83 = V_1 -> V_64 / ( V_71 ) sizeof( T_9 ) ;
if ( V_83 == 0 ) {
* V_140 = 0 ;
return NULL ;
}
V_144 = 0 ;
if ( V_1 -> V_66 & V_94 ) {
int V_121 = 0 ;
V_120 = F_91 ( V_1 , V_139 , & V_121 ) ;
V_141 = V_120 -> V_119 ;
V_143 = V_120 -> V_130 - 1 ;
} else {
V_141 = V_1 -> V_61 . V_72 ;
V_143 = V_83 - 1 ;
}
while ( V_144 <= V_143 ) {
V_117 = ( V_144 + V_143 ) >> 1 ;
V_7 = V_141 + V_117 ;
V_145 = F_92 ( V_7 ) ;
V_142 = F_93 ( V_7 ) ;
if ( V_139 < V_145 ) {
V_143 = V_117 - 1 ;
} else if ( V_139 >= V_145 + V_142 ) {
V_144 = V_117 + 1 ;
} else {
if ( V_1 -> V_66 & V_94 ) {
V_117 += V_120 -> V_146 ;
}
* V_140 = V_117 ;
return V_7 ;
}
}
if ( V_1 -> V_66 & V_94 ) {
V_117 += V_120 -> V_146 ;
}
if ( V_139 >= V_145 + V_142 ) {
if ( ++ V_117 == V_83 ) {
V_7 = NULL ;
} else {
V_7 = F_2 ( V_1 , V_117 ) ;
}
}
* V_140 = V_117 ;
return V_7 ;
}
T_18 *
F_91 (
T_1 * V_1 ,
T_19 V_139 ,
int * V_147 )
{
T_18 * V_120 = NULL ;
T_18 * V_148 ;
int V_121 ;
int V_134 ;
int V_143 ;
int V_144 ;
ASSERT ( V_1 -> V_66 & V_94 ) ;
V_134 = V_1 -> V_65 / V_135 ;
V_121 = 0 ;
V_144 = 0 ;
V_143 = V_134 - 1 ;
while ( V_144 <= V_143 ) {
V_121 = ( V_144 + V_143 ) >> 1 ;
V_120 = & V_1 -> V_61 . V_118 [ V_121 ] ;
V_148 = V_121 < V_134 - 1 ? V_120 + 1 : NULL ;
if ( V_139 < F_92 ( V_120 -> V_119 ) ) {
V_143 = V_121 - 1 ;
} else if ( V_148 && V_139 >=
F_92 ( V_148 -> V_119 ) ) {
V_144 = V_121 + 1 ;
} else {
break;
}
}
* V_147 = V_121 ;
return V_120 ;
}
T_18 *
F_65 (
T_1 * V_1 ,
T_12 * V_140 ,
int * V_147 ,
int realloc )
{
T_18 * V_149 ;
T_18 * V_120 = NULL ;
int V_121 ;
int V_134 ;
int V_143 ;
int V_144 ;
T_12 V_122 = * V_140 ;
ASSERT ( V_1 -> V_66 & V_94 ) ;
ASSERT ( V_122 >= 0 ) ;
ASSERT ( V_122 <= V_1 -> V_64 / sizeof( T_9 ) ) ;
ASSERT ( V_122 < V_1 -> V_64 / sizeof( T_9 ) || realloc ) ;
V_134 = V_1 -> V_65 / V_135 ;
V_121 = 0 ;
V_144 = 0 ;
V_143 = V_134 - 1 ;
while ( V_144 <= V_143 ) {
V_121 = ( V_144 + V_143 ) >> 1 ;
V_120 = & V_1 -> V_61 . V_118 [ V_121 ] ;
V_149 = V_121 > 0 ? V_120 - 1 : NULL ;
if ( V_122 < V_120 -> V_146 || ( V_122 == V_120 -> V_146 &&
realloc && V_149 && V_149 -> V_130 < V_129 ) ) {
V_143 = V_121 - 1 ;
} else if ( V_122 > V_120 -> V_146 + V_120 -> V_130 ||
( V_122 == V_120 -> V_146 + V_120 -> V_130 &&
! realloc ) ) {
V_144 = V_121 + 1 ;
} else if ( V_122 == V_120 -> V_146 + V_120 -> V_130 &&
V_120 -> V_130 == V_129 ) {
ASSERT ( realloc ) ;
V_122 = 0 ;
V_121 ++ ;
V_120 = V_121 < V_134 ? V_120 + 1 : NULL ;
break;
} else {
V_122 -= V_120 -> V_146 ;
break;
}
}
* V_140 = V_122 ;
* V_147 = V_121 ;
return ( V_120 ) ;
}
void
F_70 (
T_1 * V_1 )
{
T_18 * V_120 ;
T_12 V_83 ;
ASSERT ( ! ( V_1 -> V_66 & V_94 ) ) ;
V_83 = V_1 -> V_64 / ( V_71 ) sizeof( T_9 ) ;
ASSERT ( V_83 <= V_129 ) ;
V_120 = F_28 ( sizeof( T_18 ) , V_53 ) ;
if ( V_83 == 0 ) {
V_1 -> V_61 . V_72 = F_28 ( V_135 , V_53 ) ;
} else if ( ! V_1 -> V_65 ) {
F_86 ( V_1 , V_135 ) ;
} else if ( V_1 -> V_65 < V_135 ) {
F_69 ( V_1 , V_135 ) ;
}
V_120 -> V_119 = V_1 -> V_61 . V_72 ;
V_120 -> V_130 = V_83 ;
V_120 -> V_146 = 0 ;
V_1 -> V_66 |= V_94 ;
V_1 -> V_65 = V_135 ;
V_1 -> V_64 = V_83 * sizeof( T_9 ) ;
V_1 -> V_61 . V_118 = V_120 ;
return;
}
T_18 *
F_73 (
T_1 * V_1 ,
int V_121 )
{
T_18 * V_120 ;
int V_6 ;
int V_134 ;
ASSERT ( V_1 -> V_66 & V_94 ) ;
V_134 = V_1 -> V_65 / V_135 ;
F_87 ( V_1 , ++ V_134 *
sizeof( T_18 ) ) ;
V_120 = V_1 -> V_61 . V_118 ;
for ( V_6 = V_134 - 1 ; V_6 > V_121 ; V_6 -- ) {
memmove ( & V_120 [ V_6 ] , & V_120 [ V_6 - 1 ] , sizeof( T_18 ) ) ;
}
ASSERT ( V_6 == V_121 ) ;
V_120 = V_1 -> V_61 . V_118 ;
V_120 [ V_121 ] . V_119 = F_28 ( V_135 , V_53 ) ;
V_1 -> V_65 = V_134 * V_135 ;
memset ( V_120 [ V_121 ] . V_119 , 0 , V_135 ) ;
V_120 [ V_121 ] . V_130 = 0 ;
V_120 [ V_121 ] . V_146 = V_121 > 0 ?
V_120 [ V_121 - 1 ] . V_146 + V_120 [ V_121 - 1 ] . V_130 : 0 ;
return ( & V_120 [ V_121 ] ) ;
}
void
F_81 (
T_1 * V_1 ,
int V_121 )
{
T_18 * V_120 ;
int V_6 ;
int V_134 ;
ASSERT ( V_1 -> V_66 & V_94 ) ;
V_134 = V_1 -> V_65 / V_135 ;
V_120 = & V_1 -> V_61 . V_118 [ V_121 ] ;
if ( V_120 -> V_119 ) {
F_71 ( V_1 , V_121 + 1 ,
- V_120 -> V_130 ) ;
F_54 ( V_120 -> V_119 ) ;
}
V_120 = V_1 -> V_61 . V_118 ;
for ( V_6 = V_121 ; V_6 < V_134 - 1 ; V_6 ++ ) {
memmove ( & V_120 [ V_6 ] , & V_120 [ V_6 + 1 ] , sizeof( T_18 ) ) ;
}
if ( -- V_134 ) {
F_87 ( V_1 ,
V_134 * sizeof( T_18 ) ) ;
} else {
F_54 ( V_1 -> V_61 . V_118 ) ;
}
V_1 -> V_65 = V_134 * V_135 ;
}
void
F_82 (
T_1 * V_1 )
{
T_12 V_83 ;
int V_134 ;
ASSERT ( V_1 -> V_66 & V_94 ) ;
V_134 = V_1 -> V_65 / V_135 ;
V_83 = V_1 -> V_64 / ( V_71 ) sizeof( T_9 ) ;
if ( V_83 == 0 ) {
F_44 ( V_1 ) ;
} else if ( V_83 <= V_73 ) {
F_88 ( V_1 ) ;
F_83 ( V_1 , V_83 ) ;
} else if ( V_83 <= V_129 ) {
F_88 ( V_1 ) ;
} else if ( V_83 < ( V_134 * V_129 ) > > 1 ) {
F_89 ( V_1 ) ;
}
}
void
F_89 (
T_1 * V_1 )
{
T_18 * V_120 , * V_148 ;
int V_121 = 0 ;
int V_134 ;
ASSERT ( V_1 -> V_66 & V_94 ) ;
V_134 = V_1 -> V_65 / V_135 ;
while ( V_121 < V_134 - 1 ) {
V_120 = & V_1 -> V_61 . V_118 [ V_121 ] ;
V_148 = V_120 + 1 ;
if ( V_148 -> V_130 <=
( V_129 - V_120 -> V_130 ) ) {
memcpy ( & V_120 -> V_119 [ V_120 -> V_130 ] ,
V_148 -> V_119 , V_148 -> V_130 *
sizeof( T_9 ) ) ;
V_120 -> V_130 += V_148 -> V_130 ;
F_54 ( V_148 -> V_119 ) ;
V_148 -> V_119 = NULL ;
F_81 ( V_1 , V_121 + 1 ) ;
V_134 = V_1 -> V_65 / V_135 ;
} else {
V_121 ++ ;
}
}
}
void
F_71 (
T_1 * V_1 ,
int V_121 ,
int V_128 )
{
int V_6 ;
int V_134 ;
ASSERT ( V_1 -> V_66 & V_94 ) ;
V_134 = V_1 -> V_65 / V_135 ;
for ( V_6 = V_121 ; V_6 < V_134 ; V_6 ++ ) {
V_1 -> V_61 . V_118 [ V_6 ] . V_146 += V_128 ;
}
}
