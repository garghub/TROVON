static void F_1 ( T_1 * V_1 ,
int V_2 ,
int V_3 )
{
memcpy ( V_1 , V_4 , sizeof V_4 ) ;
V_1 [ V_5 + 0 ] = V_2 >> 8 ;
V_1 [ V_5 + 1 ] = V_2 ;
V_1 [ V_5 + 2 ] = V_3 >> 8 ;
V_1 [ V_5 + 3 ] = V_3 ;
}
static void F_2 ( T_1 * V_1 ,
int V_6 )
{
int V_7 , V_8 ;
if ( V_6 < 50 )
V_8 = 5000 / V_6 ;
else
V_8 = 200 - V_6 * 2 ;
for ( V_7 = 0 ; V_7 < 64 ; V_7 ++ ) {
V_1 [ V_9 + V_7 ] =
( V_4 [ V_9 + V_7 ] * V_8 + 50 ) / 100 ;
V_1 [ V_10 + V_7 ] =
( V_4 [ V_10 + V_7 ] * V_8 + 50 ) / 100 ;
}
}
static void F_3 ( struct V_11 * V_11 , T_1 V_12 , T_1 V_13 )
{
struct V_14 * V_15 = V_11 -> V_15 ;
int V_16 ;
if ( V_11 -> V_17 < 0 )
return;
V_16 = F_4 ( V_15 , F_5 ( V_15 , 0 ) ,
0x0e ,
V_18 | V_19 | V_20 ,
V_13 , V_12 , NULL , 0 , 500 ) ;
if ( V_16 < 0 ) {
F_6 ( L_1 , V_16 ) ;
V_11 -> V_17 = V_16 ;
}
}
static void F_7 ( struct V_11 * V_11 , T_1 V_12 )
{
struct V_14 * V_15 = V_11 -> V_15 ;
int V_16 ;
if ( V_11 -> V_17 < 0 )
return;
V_16 = F_4 ( V_15 , F_8 ( V_15 , 0 ) ,
0x0d ,
V_21 | V_19 | V_20 ,
0 , V_12 , V_11 -> V_22 , 1 , 500 ) ;
if ( V_16 < 0 ) {
F_6 ( L_2 , V_16 ) ;
V_11 -> V_17 = V_16 ;
}
}
static void F_9 ( struct V_11 * V_11 ,
const struct V_23 * V_24 , int V_25 )
{
do {
F_3 ( V_11 , V_24 -> V_26 , V_24 -> V_27 ) ;
V_24 ++ ;
} while ( -- V_25 > 0 );
}
static int F_10 ( struct V_11 * V_11 , T_1 V_12 , T_1 V_13 )
{
struct V_28 * V_28 = (struct V_28 * ) V_11 ;
F_3 ( V_11 , V_29 , 0x00 ) ;
F_3 ( V_11 , V_30 , V_12 ) ;
F_3 ( V_11 , V_31 , V_13 ) ;
F_3 ( V_11 , V_29 , 0x01 ) ;
if ( V_28 -> V_32 == V_33 )
return 0 ;
F_11 ( 5 ) ;
F_7 ( V_11 , V_29 ) ;
if ( V_11 -> V_22 [ 0 ] == 0 )
return 0 ;
F_3 ( V_11 , V_29 , 0x00 ) ;
return - 1 ;
}
static void F_12 ( struct V_11 * V_11 ,
const struct V_23 * V_24 , int V_25 )
{
do {
F_10 ( V_11 , V_24 -> V_26 , V_24 -> V_27 ) ;
V_24 ++ ;
} while ( -- V_25 > 0 );
}
static int F_13 ( struct V_11 * V_11 , T_1 V_12 , int V_34 )
{
struct V_28 * V_28 = (struct V_28 * ) V_11 ;
int V_35 ;
F_3 ( V_11 , V_30 , V_12 ) ;
F_3 ( V_11 , V_29 , 0x02 ) ;
F_11 ( 5 ) ;
F_7 ( V_11 , V_36 ) ;
V_35 = V_11 -> V_22 [ 0 ] ;
if ( V_28 -> V_32 == V_33 )
return V_35 ;
if ( V_34 > 1 ) {
F_7 ( V_11 , V_37 ) ;
V_35 |= ( V_11 -> V_22 [ 0 ] << 8 ) ;
}
F_7 ( V_11 , V_29 ) ;
if ( V_11 -> V_22 [ 0 ] == 0 )
return V_35 ;
F_3 ( V_11 , V_29 , 0x00 ) ;
return - 1 ;
}
static void F_14 ( struct V_11 * V_11 ,
T_1 V_38 ,
const T_1 * V_39 ,
int V_40 )
{
struct V_14 * V_15 = V_11 -> V_15 ;
int V_41 , V_42 , V_16 ;
if ( V_11 -> V_17 < 0 )
return;
for (; ; ) {
V_41 = V_40 > V_43 - 1
? V_43 - 1 : V_40 ;
V_11 -> V_22 [ 0 ] = V_38 ;
memcpy ( & V_11 -> V_22 [ 1 ] , V_39 , V_41 ) ;
V_16 = F_15 ( V_15 ,
F_16 ( V_15 , 3 ) ,
V_11 -> V_22 , V_41 + 1 ,
& V_42 , 500 ) ;
if ( V_16 < 0 ) {
F_6 ( L_3 ,
V_16 , V_38 ) ;
V_11 -> V_17 = V_16 ;
return;
}
V_40 -= V_41 ;
if ( V_40 <= 0 )
break;
V_39 += V_41 ;
}
}
static int F_17 ( struct V_11 * V_11 )
{
T_1 V_44 ;
int V_16 ;
F_7 ( V_11 , V_45 ) ;
V_44 = V_11 -> V_22 [ 0 ] ;
F_3 ( V_11 , V_45 , V_44 ) ;
F_3 ( V_11 , V_45 , V_44 | 0x20 ) ;
F_3 ( V_11 , V_45 , V_44 ) ;
F_3 ( V_11 , V_46 , 0x04 ) ;
F_3 ( V_11 , V_47 , 0x21 ) ;
F_3 ( V_11 , V_48 , 0x00 ) ;
if ( F_10 ( V_11 , 0x00 , 0x00 ) >= 0 )
return V_49 ;
F_3 ( V_11 , V_45 , V_44 | 0x20 ) ;
F_3 ( V_11 , V_46 , 0x00 ) ;
F_3 ( V_11 , V_47 , 0x7f ) ;
if ( F_10 ( V_11 , 0x00 , 0x00 ) >= 0 )
return - 2 ;
F_3 ( V_11 , V_45 , V_44 | 0x20 ) ;
F_3 ( V_11 , V_45 , V_44 ) ;
F_3 ( V_11 , V_45 , V_44 | 0x20 ) ;
F_3 ( V_11 , V_46 , 0x00 ) ;
F_3 ( V_11 , V_47 , 0x11 ) ;
V_16 = F_13 ( V_11 , 0x00 , 1 ) ;
if ( V_16 > 0 )
return - 3 ;
F_3 ( V_11 , V_45 , V_44 | 0x20 ) ;
F_3 ( V_11 , V_45 , V_44 ) ;
F_3 ( V_11 , V_45 , V_44 | 0x20 ) ;
F_3 ( V_11 , V_47 , 0x6e ) ;
V_16 = F_13 ( V_11 , 0x00 , 1 ) ;
if ( V_16 > 0 )
return - 4 ;
V_16 = F_13 ( V_11 , 0x01 , 1 ) ;
if ( V_16 > 0 )
return - 5 ;
F_3 ( V_11 , V_45 , V_44 | 0x20 ) ;
F_3 ( V_11 , V_45 , V_44 ) ;
F_3 ( V_11 , V_45 , V_44 | 0x20 ) ;
F_3 ( V_11 , V_46 , 0x04 ) ;
F_3 ( V_11 , V_47 , 0x5d ) ;
V_16 = F_13 ( V_11 , 0x00 , 2 ) ;
if ( V_16 > 0 )
return - 6 ;
F_3 ( V_11 , V_45 , V_44 | 0x20 ) ;
F_3 ( V_11 , V_45 , V_44 ) ;
F_3 ( V_11 , V_45 , V_44 | 0x20 ) ;
F_3 ( V_11 , V_47 , 0x5c ) ;
V_16 = F_13 ( V_11 , 0x36 , 2 ) ;
if ( V_16 > 0 )
return - 7 ;
F_3 ( V_11 , V_45 , V_44 ) ;
F_3 ( V_11 , V_45 , V_44 | 0x20 ) ;
F_3 ( V_11 , V_45 , V_44 ) ;
F_3 ( V_11 , V_47 , 0x61 ) ;
F_3 ( V_11 , V_48 , 0x10 ) ;
if ( F_10 ( V_11 , 0xff , 0x00 ) >= 0 )
return - 8 ;
F_3 ( V_11 , V_45 , V_44 | 0x20 ) ;
F_3 ( V_11 , V_45 , V_44 ) ;
F_3 ( V_11 , V_45 , V_44 | 0x20 ) ;
F_3 ( V_11 , V_46 , 0x00 ) ;
F_3 ( V_11 , V_47 , 0x20 ) ;
V_16 = F_13 ( V_11 , 0x00 , 1 ) ;
if ( V_16 > 0 )
return V_50 ;
return - 9 ;
}
static void F_18 ( struct V_11 * V_11 )
{
static const struct V_23 V_51 [] = {
{ V_52 , 0x2f } ,
{ 0x25 , 0x02 } ,
{ V_53 , 0x00 } ,
{ V_54 , 0x80 } ,
{ V_52 , 0x2f } ,
{ V_45 , 0xe1 } ,
{ V_45 , 0xc1 } ,
{ V_45 , 0xe1 } ,
{ V_29 , 0x00 } ,
} ;
static const struct V_23 V_55 [] = {
{ V_56 , 0x48 } ,
{ V_29 , 0x00 } ,
} ;
static const struct V_23 V_57 [] = {
{ V_58 , 0x07 } ,
{ V_59 , 0x58 } ,
{ V_60 , 0x28 } ,
{ V_61 , 0x28 } ,
{ V_62 , 0x50 } ,
{ V_63 , 0x03 } ,
{ V_64 , 0xff } ,
{ V_65 , 0x07 } ,
{ V_66 , 0xff } ,
{ V_67 , 0x07 } ,
{ V_68 , 0x5c } ,
{ V_69 , 0x01 } ,
{ V_70 , 0xfc } ,
{ V_71 , 0x03 } ,
{ V_72 , 0x00 } ,
{ V_73 , 0x00 } ,
{ V_74 , 0x40 } ,
{ V_75 , 0x01 } ,
{ V_76 , 0x00 } ,
{ V_74 , 0x02 } ,
{ V_77 , 0x05 } ,
{ V_78 , 0x00 } ,
{ V_79 , 0x00 } ,
{ V_80 , 0x01 } ,
{ V_81 , 0x0b } ,
{ V_82 , 0x0b } ,
{ V_83 , 0x07 } ,
{ V_84 , 0x40 } ,
{ V_85 , 0x02 } ,
{ V_86 , 0x81 } ,
{ V_87 , 0x01 } ,
} ;
F_9 ( V_11 , V_51 , F_19 ( V_51 ) ) ;
F_9 ( V_11 , V_88 ,
F_19 ( V_88 ) ) ;
F_9 ( V_11 , V_55 , F_19 ( V_55 ) ) ;
F_3 ( V_11 , V_47 , 0x20 ) ;
F_12 ( V_11 , V_57 , F_19 ( V_57 ) ) ;
F_12 ( V_11 , V_89 , F_19 ( V_89 ) ) ;
}
static void F_20 ( struct V_11 * V_11 )
{
static const struct V_23 V_51 [] = {
{ V_52 , 0x2f } ,
{ V_45 , 0xe1 } ,
{ 0x25 , 0x02 } ,
{ V_53 , 0x00 } ,
{ V_54 , 0x80 } ,
{ V_52 , 0x2f } ,
{ V_45 , 0xc1 } ,
} ;
static const struct V_23 V_55 [] = {
{ V_56 , 0x54 } ,
} ;
static const struct V_23 V_57 [] = {
{ 0x00 , 0x00 } ,
{ 0x01 , 0x80 } ,
{ 0x02 , 0x80 } ,
{ 0x03 , 0x90 } ,
{ 0x04 , 0x20 } ,
{ 0x05 , 0x20 } ,
{ 0x06 , 0x80 } ,
{ 0x07 , 0x00 } ,
{ 0x08 , 0xff } ,
{ 0x09 , 0xff } ,
{ 0x0a , 0x76 } ,
{ 0x0b , 0x30 } ,
{ 0x0c , 0x20 } ,
{ 0x0d , 0x20 } ,
{ 0x0e , 0xff } ,
{ 0x0f , 0xff } ,
{ 0x10 , 0x41 } ,
{ 0x15 , 0x14 } ,
{ 0x11 , 0x40 } ,
{ 0x12 , 0x48 } ,
{ 0x13 , 0x80 } ,
{ 0x14 , 0x80 } ,
{ 0x16 , 0x03 } ,
{ 0x28 , 0xb0 } ,
{ 0x71 , 0x20 } ,
{ 0x75 , 0x8e } ,
{ 0x17 , 0x1b } ,
{ 0x18 , 0xbd } ,
{ 0x19 , 0x05 } ,
{ 0x1a , 0xf6 } ,
{ 0x1b , 0x04 } ,
{ 0x1c , 0x7f } ,
{ 0x1d , 0xa2 } ,
{ 0x1e , 0x00 } ,
{ 0x1f , 0x00 } ,
{ 0x20 , 0x45 } ,
{ 0x21 , 0x80 } ,
{ 0x22 , 0x80 } ,
{ 0x23 , 0xee } ,
{ 0x24 , 0x50 } ,
{ 0x25 , 0x7a } ,
{ 0x26 , 0xa0 } ,
{ 0x27 , 0x9a } ,
{ 0x29 , 0x30 } ,
{ 0x2a , 0x80 } ,
{ 0x2b , 0x00 } ,
{ 0x2c , 0xac } ,
{ 0x2d , 0x05 } ,
{ 0x2e , 0x80 } ,
{ 0x2f , 0x3c } ,
{ 0x30 , 0x22 } ,
{ 0x31 , 0x00 } ,
{ 0x32 , 0x86 } ,
{ 0x33 , 0x08 } ,
{ 0x34 , 0xff } ,
{ 0x35 , 0xff } ,
{ 0x36 , 0xff } ,
{ 0x37 , 0xff } ,
{ 0x38 , 0xff } ,
{ 0x39 , 0xff } ,
{ 0x3a , 0xfe } ,
{ 0x3b , 0xfe } ,
{ 0x3c , 0xfe } ,
{ 0x3d , 0xfe } ,
{ 0x3e , 0xfe } ,
{ 0x3f , 0x71 } ,
{ 0x40 , 0xff } ,
{ 0x41 , 0xff } ,
{ 0x42 , 0xff } ,
{ 0x43 , 0xff } ,
{ 0x44 , 0xff } ,
{ 0x45 , 0xff } ,
{ 0x46 , 0xff } ,
{ 0x47 , 0xff } ,
{ 0x48 , 0xff } ,
{ 0x49 , 0xff } ,
{ 0x4a , 0xfe } ,
{ 0x4b , 0xff } ,
{ 0x4c , 0x00 } ,
{ 0x4d , 0x00 } ,
{ 0x4e , 0xff } ,
{ 0x4f , 0xff } ,
{ 0x50 , 0xff } ,
{ 0x51 , 0xff } ,
{ 0x52 , 0xff } ,
{ 0x53 , 0xff } ,
{ 0x54 , 0xff } ,
{ 0x55 , 0xff } ,
{ 0x56 , 0xff } ,
{ 0x57 , 0xff } ,
{ 0x58 , 0xff } ,
{ 0x59 , 0xff } ,
{ 0x5a , 0xff } ,
{ 0x5b , 0xfe } ,
{ 0x5c , 0xff } ,
{ 0x5d , 0x8f } ,
{ 0x5e , 0xff } ,
{ 0x5f , 0x8f } ,
{ 0x60 , 0xa2 } ,
{ 0x61 , 0x4a } ,
{ 0x62 , 0xf3 } ,
{ 0x63 , 0x75 } ,
{ 0x64 , 0xf0 } ,
{ 0x65 , 0x00 } ,
{ 0x66 , 0x55 } ,
{ 0x67 , 0x92 } ,
{ 0x68 , 0xa0 } ,
{ 0x69 , 0x4a } ,
{ 0x6a , 0x22 } ,
{ 0x6b , 0x00 } ,
{ 0x6c , 0x33 } ,
{ 0x6d , 0x44 } ,
{ 0x6e , 0x22 } ,
{ 0x6f , 0x84 } ,
{ 0x70 , 0x0b } ,
{ 0x72 , 0x10 } ,
{ 0x73 , 0x50 } ,
{ 0x74 , 0x21 } ,
{ 0x76 , 0x00 } ,
{ 0x77 , 0xa5 } ,
{ 0x78 , 0x80 } ,
{ 0x79 , 0x80 } ,
{ 0x7a , 0x80 } ,
{ 0x7b , 0xe2 } ,
{ 0x7c , 0x00 } ,
{ 0x7d , 0xf7 } ,
{ 0x7e , 0x00 } ,
{ 0x7f , 0x00 } ,
} ;
F_9 ( V_11 , V_51 , F_19 ( V_51 ) ) ;
F_9 ( V_11 , V_90 ,
F_19 ( V_90 ) ) ;
F_9 ( V_11 , V_55 , F_19 ( V_55 ) ) ;
F_10 ( V_11 , 0x12 , 0x80 ) ;
F_11 ( 10 ) ;
F_12 ( V_11 , V_57 , F_19 ( V_57 ) ) ;
}
static void F_21 ( struct V_11 * V_11 , T_2 V_91 , T_2 V_92 ,
T_2 V_93 , T_2 V_94 )
{
struct V_28 * V_28 = (struct V_28 * ) V_11 ;
if ( V_28 -> V_95 == V_50 ) {
V_91 = ( V_91 << 2 ) - 1 ;
F_10 ( V_11 , V_96 , V_91 ) ;
F_10 ( V_11 , V_97 , V_91 >> 8 ) ;
if ( V_28 -> V_32 == V_33 )
F_10 ( V_11 , V_98 ,
V_92 >> 8 ) ;
F_10 ( V_11 , V_99 , V_92 ) ;
if ( V_28 -> V_32 == V_33 )
F_10 ( V_11 , V_100 ,
V_92 >> 8 ) ;
F_10 ( V_11 , V_101 , V_92 ) ;
if ( V_28 -> V_95 == V_50 ) {
if ( V_28 -> V_32 == V_33 )
F_10 ( V_11 , V_102 ,
V_93 >> 8 ) ;
F_10 ( V_11 , V_103 , V_93 ) ;
if ( V_28 -> V_32 == V_33 )
F_10 ( V_11 , V_104 ,
V_94 >> 8 ) ;
F_10 ( V_11 , V_105 , V_94 ) ;
}
F_10 ( V_11 , V_74 ,
V_28 -> V_32 == V_33 ? 0x80 : 0x81 ) ;
return;
}
F_10 ( V_11 , 0x10 ,
V_91 ) ;
F_10 ( V_11 , 0x00 ,
V_92 ) ;
}
static void F_22 ( struct V_11 * V_11 , T_1 V_106 )
{
struct V_28 * V_28 = (struct V_28 * ) V_11 ;
F_23 ( V_107 , L_4 , V_28 -> V_6 , V_106 ) ;
V_28 -> V_6 = V_106 ;
if ( V_106 > 16 )
V_106 = 16 ;
if ( V_28 -> V_95 == V_49 )
F_2 ( V_28 -> V_1 , V_108 [ V_106 ] ) ;
else
memcpy ( & V_28 -> V_1 [ V_9 - 1 ] ,
V_109 [ V_106 ] , sizeof V_109 [ 0 ] ) ;
}
static void F_24 ( struct V_11 * V_11 , T_2 V_106 )
{
struct V_28 * V_28 = (struct V_28 * ) V_11 ;
if ( V_106 != 16 )
V_106 = 15 - V_106 ;
F_3 ( V_11 , V_110 , 0x00 ) ;
F_3 ( V_11 , V_111 , 0x04 ) ;
F_3 ( V_11 , V_111 , V_106 ) ;
if ( V_106 == 15 && V_28 -> V_32 == V_112 ) {
F_11 ( 4 ) ;
F_3 ( V_11 , V_110 , 0x19 ) ;
}
}
static void F_25 ( struct V_11 * V_11 , T_2 gamma )
{
struct V_28 * V_28 = (struct V_28 * ) V_11 ;
#define F_26 6
static const T_1 V_113 [ F_26 ] [ 3 ] [ 1024 ] = {
{
{  } ,
{  } ,
{  }
} ,
{
{  } ,
{  } ,
{  }
} ,
{
{  } ,
{  } ,
{  }
} ,
{
{  } ,
{  } ,
{  }
} ,
{
{  } ,
{  } ,
{  }
} ,
{
{  } ,
{  } ,
{  } ,
} ,
} ;
F_3 ( V_11 , V_53 , 0x00 ) ;
if ( V_28 -> V_32 == V_112 )
F_3 ( V_11 , 0x02 , 0x28 ) ;
F_14 ( V_11 , 0x00 , V_113 [ gamma ] [ 0 ] , 1024 ) ;
F_14 ( V_11 , 0x01 , V_113 [ gamma ] [ 1 ] , 1024 ) ;
F_14 ( V_11 , 0x02 , V_113 [ gamma ] [ 2 ] , 1024 ) ;
if ( V_28 -> V_32 == V_112 ) {
int V_7 ;
F_3 ( V_11 , 0x02 , 0x2b ) ;
F_3 ( V_11 , 0x02 , 0x28 ) ;
for ( V_7 = 0 ; V_7 < 6 ; V_7 ++ )
F_3 ( V_11 , V_114 ,
V_113 [ gamma ] [ 0 ] [ V_7 ] ) ;
F_3 ( V_11 , 0x02 , 0x2b ) ;
F_3 ( V_11 , 0x02 , 0x28 ) ;
for ( V_7 = 0 ; V_7 < 6 ; V_7 ++ )
F_3 ( V_11 , V_115 ,
V_113 [ gamma ] [ 1 ] [ V_7 ] ) ;
F_3 ( V_11 , 0x02 , 0x2b ) ;
F_3 ( V_11 , 0x02 , 0x28 ) ;
for ( V_7 = 0 ; V_7 < 6 ; V_7 ++ )
F_3 ( V_11 , V_116 ,
V_113 [ gamma ] [ 2 ] [ V_7 ] ) ;
F_3 ( V_11 , 0x02 , 0x28 ) ;
}
F_3 ( V_11 , V_53 , 0x03 ) ;
}
static void F_27 ( struct V_11 * V_11 , T_2 V_27 )
{
struct V_28 * V_28 = (struct V_28 * ) V_11 ;
if ( V_28 -> V_32 == V_33 ) {
V_27 |= 0x08 ;
if ( V_11 -> V_3 == 640 )
F_3 ( V_11 , V_56 , 0x00 ) ;
else
V_27 |= 0x04 ;
F_3 ( V_11 , V_117 , V_27 ) ;
} else {
V_27 = ( V_27 << 5 ) | 0x08 ;
F_3 ( V_11 , 0x59 , V_27 ) ;
}
}
static void F_28 ( struct V_11 * V_11 , T_2 V_27 )
{
struct V_28 * V_28 = (struct V_28 * ) V_11 ;
V_28 -> V_118 = V_27 ? V_119 : - 1 ;
}
static void F_29 ( struct V_11 * V_11 )
{
struct V_28 * V_28 = (struct V_28 * ) V_11 ;
F_3 ( V_11 , V_53 , 0x00 ) ;
if ( V_11 -> V_3 == 320 ) {
F_3 ( V_11 , V_54 , 0x06 ) ;
F_11 ( 100 ) ;
F_10 ( V_11 , V_86 , 0x01 ) ;
F_11 ( 100 ) ;
F_3 ( V_11 , V_53 , 0x03 ) ;
F_3 ( V_11 , V_56 , 0x01 ) ;
F_3 ( V_11 , V_117 , 0x0d ) ;
F_10 ( V_11 , V_96 , 0x37 ) ;
F_10 ( V_11 , V_97 , 0x01 ) ;
} else {
F_3 ( V_11 , V_54 , 0x05 ) ;
F_11 ( 100 ) ;
F_10 ( V_11 , V_86 , 0x01 ) ;
F_11 ( 100 ) ;
F_3 ( V_11 , V_53 , 0x03 ) ;
F_3 ( V_11 , V_56 , 0x00 ) ;
F_3 ( V_11 , V_117 , 0x09 ) ;
F_10 ( V_11 , V_96 , 0xcf ) ;
F_10 ( V_11 , V_97 , 0x00 ) ;
}
F_10 ( V_11 , V_74 , 0x01 ) ;
F_14 ( V_11 , 0x03 , V_120 [ V_50 ] ,
F_19 ( V_120 [ 0 ] ) ) ;
F_25 ( V_11 , F_30 ( V_28 -> gamma ) ) ;
if ( V_28 -> V_95 == V_49 )
F_24 ( V_11 , F_30 ( V_28 -> V_121 ) ) ;
}
static int F_31 ( struct V_11 * V_11 )
{
struct V_28 * V_28 = (struct V_28 * ) V_11 ;
int V_7 ;
if ( V_28 -> V_32 == V_33 ) {
for ( V_7 = 0 ; V_7 < F_19 ( V_122 ) - 1 ; V_7 ++ ) {
if ( V_28 -> V_123 >= V_122 [ V_7 ] )
break;
}
V_7 = 6 - V_7 ;
if ( V_7 == 6
&& V_11 -> V_3 == 640 )
V_7 = 0x05 ;
} else {
for ( V_7 = 0 ; V_7 < F_19 ( V_124 ) - 1 ; V_7 ++ ) {
if ( V_28 -> V_123 >= V_124 [ V_7 ] )
break;
}
V_7 = 7 - V_7 ;
if ( V_7 == 7
&& V_11 -> V_3 == 640 )
V_7 = 6 ;
V_7 |= 0x80 ;
}
return V_7 ;
}
static void F_32 ( struct V_11 * V_11 , T_2 V_27 )
{
struct V_28 * V_28 = (struct V_28 * ) V_11 ;
T_1 V_125 ;
V_125 = F_31 ( V_11 ) ;
if ( V_28 -> V_32 == V_112 ) {
F_7 ( V_11 , 0x7b ) ;
F_3 ( V_11 , 0x7b ,
V_28 -> V_95 == V_50 ? 0x10 : 0x90 ) ;
if ( V_27 >= 128 )
V_125 = 0xf0 ;
}
F_3 ( V_11 , V_54 , V_125 ) ;
if ( V_28 -> V_95 == V_50 )
F_10 ( V_11 , V_86 , 0x01 ) ;
}
static void F_33 ( struct V_11 * V_11 , T_2 V_126 )
{
F_10 ( V_11 , V_104 , V_126 >> 8 ) ;
F_10 ( V_11 , V_105 , V_126 ) ;
F_10 ( V_11 , V_74 , 0x80 ) ;
}
static int F_34 ( struct V_11 * V_11 )
{
struct V_28 * V_28 = (struct V_28 * ) V_11 ;
T_2 V_27 = V_11 -> V_92 -> V_27 ;
if ( V_28 -> V_95 == V_50 ) {
T_2 V_127 = V_11 -> V_92 -> V_128 . V_27 ?
V_11 -> V_92 -> V_128 . V_27 : 1 ;
V_28 -> V_93 -> V_27 = V_28 -> V_93 -> V_27 * V_27 / V_127 ;
if ( V_28 -> V_93 -> V_27 > 4095 )
V_28 -> V_93 -> V_27 = 4095 ;
V_28 -> V_94 -> V_27 = V_28 -> V_94 -> V_27 * V_27 / V_127 ;
if ( V_28 -> V_94 -> V_27 > 4095 )
V_28 -> V_94 -> V_27 = 4095 ;
}
if ( V_11 -> V_129 ) {
if ( V_28 -> V_95 == V_50 )
F_21 ( V_11 , V_11 -> V_130 -> V_27 ,
V_11 -> V_92 -> V_27 ,
V_28 -> V_93 -> V_27 , V_28 -> V_94 -> V_27 ) ;
else
F_21 ( V_11 , V_11 -> V_130 -> V_27 ,
V_11 -> V_92 -> V_27 , 0 , 0 ) ;
}
return V_11 -> V_17 ;
}
static void F_35 ( struct V_11 * V_11 , T_2 V_131 )
{
F_10 ( V_11 , V_102 , V_131 >> 8 ) ;
F_10 ( V_11 , V_103 , V_131 ) ;
F_10 ( V_11 , V_74 , 0x80 ) ;
}
static int F_36 ( struct V_11 * V_11 ,
const struct V_132 * V_133 )
{
struct V_28 * V_28 = (struct V_28 * ) V_11 ;
V_28 -> V_32 = V_133 -> V_134 ;
V_11 -> V_135 . V_136 = V_137 ;
V_11 -> V_135 . V_138 = F_19 ( V_137 ) ;
V_11 -> V_135 . V_139 = V_28 -> V_32 == V_33 ?
V_140 : V_141 ;
V_28 -> V_123 = 30 ;
return 0 ;
}
static int F_37 ( struct V_11 * V_11 )
{
struct V_28 * V_28 = (struct V_28 * ) V_11 ;
static const struct V_23 V_142 [] = {
{ V_46 , 0x01 } ,
{ V_29 , 0x01 } ,
{ V_143 , 0x9f } ,
{ V_144 , 0x9f } ,
{ V_145 , 0x80 } ,
{ V_45 , 0x40 } ,
} ;
static const struct V_23 V_146 [] = {
{ V_52 , 0x2f } ,
{ V_143 , 0x6f } ,
{ V_144 , 0x40 } ,
{ V_145 , 0x9f } ,
{ V_45 , 0xc1 } ,
} ;
if ( V_28 -> V_32 == V_33 )
F_9 ( V_11 , V_142 ,
F_19 ( V_142 ) ) ;
else
F_9 ( V_11 , V_146 ,
F_19 ( V_146 ) ) ;
F_11 ( 15 ) ;
F_7 ( V_11 , V_45 ) ;
F_23 ( V_147 , L_5 , V_11 -> V_22 [ 0 ] ) ;
if ( V_148 >= 0 ) {
V_28 -> V_95 = V_148 ;
} else {
if ( V_28 -> V_32 == V_33 ) {
switch ( V_11 -> V_22 [ 0 ] & 0x07 ) {
case 0 :
V_28 -> V_95 = V_49 ;
break;
case 1 :
V_28 -> V_95 = V_50 ;
break;
}
} else {
int V_95 ;
V_95 = F_17 ( V_11 ) ;
if ( V_95 < 0 ) {
F_38 ( L_6 ,
- V_95 ) ;
V_95 = V_49 ;
}
V_28 -> V_95 = V_95 ;
}
}
if ( V_28 -> V_95 == V_49 ) {
F_39 ( L_7 ) ;
if ( V_28 -> V_32 == V_112 ) {
F_20 ( V_11 ) ;
}
} else {
F_39 ( L_8 ) ;
if ( V_28 -> V_32 == V_112 ) {
F_18 ( V_11 ) ;
}
}
F_22 ( V_11 , 0 ) ;
return 0 ;
}
static int F_40 ( struct V_11 * V_11 )
{
struct V_28 * V_28 = (struct V_28 * ) V_11 ;
static const struct V_23 V_149 [] = {
{ V_86 , 0x81 } ,
{ V_96 , 0x37 } ,
{ V_97 , 0x01 } ,
{ V_101 , 0x00 } ,
{ V_99 , 0x00 } ,
{ V_105 , 0x00 } ,
{ V_103 , 0x00 } ,
{ V_74 , 0x81 } ,
} ;
static const struct V_23 V_150 [] = {
{ 0x4d , 0x00 } ,
{ 0x4c , 0xff } ,
{ 0x4e , 0xff } ,
{ 0x4f , 0x00 } ,
} ;
static const struct V_23 V_151 [] = {
{ 0x10 , 0x75 } ,
{ 0x76 , 0x03 } ,
{ 0x00 , 0x00 } ,
} ;
static const struct V_23 V_152 [] = {
{ 0x7b , 0x90 } ,
{ V_54 , 0x87 } ,
} ;
if ( V_28 -> V_32 == V_33 )
return 0 ;
if ( V_28 -> V_95 == V_50 ) {
F_3 ( V_11 , V_47 , 0x20 ) ;
F_3 ( V_11 , V_54 , 0x87 ) ;
F_12 ( V_11 , V_149 ,
F_19 ( V_149 ) ) ;
F_9 ( V_11 , V_150 ,
F_19 ( V_150 ) ) ;
F_14 ( V_11 , 0x03 , V_153 , sizeof V_153 ) ;
F_3 ( V_11 , 0x59 , 0x40 ) ;
} else {
F_3 ( V_11 , V_47 , 0x21 ) ;
F_12 ( V_11 , V_151 ,
F_19 ( V_151 ) ) ;
F_7 ( V_11 , 0x7b ) ;
F_9 ( V_11 , V_152 ,
F_19 ( V_152 ) ) ;
}
F_3 ( V_11 , V_56 ,
V_11 -> V_154 ? 0x00 : 0x01 ) ;
return V_11 -> V_17 ;
}
static void F_41 ( struct V_11 * V_11 , int V_155 )
{
T_1 V_39 ;
F_7 ( V_11 , V_45 ) ;
V_39 = V_11 -> V_22 [ 0 ] ;
if ( V_155 )
V_39 &= ~ 0x40 ;
else
V_39 |= 0x40 ;
F_3 ( V_11 , V_45 , V_39 ) ;
}
static void F_42 ( struct V_11 * V_11 )
{
struct V_28 * V_28 = (struct V_28 * ) V_11 ;
static const struct V_23 V_156 [] = {
{ V_145 , 0x9f } ,
{ V_144 , 0x40 } ,
{ V_46 , 0x00 } ,
{ V_157 , 0x00 } ,
{ V_158 , 0x00 } ,
{ V_159 , 0xff } ,
{ V_160 , 0x03 } ,
{ V_161 , 0x07 } ,
{ V_162 , 0x40 } ,
{ V_110 , 0x40 } ,
{ V_52 , 0x17 } ,
{ V_163 , 0x18 } ,
{ V_164 , 0x00 } ,
{ V_165 , 0x00 } ,
{ V_166 , 0x00 } ,
{ V_167 , 0x00 } ,
{ V_168 , 0x00 } ,
{ V_169 , 0x00 } ,
{ V_170 , 0x00 } ,
{ V_171 , 0x00 } ,
{ V_53 , 0x03 } ,
{ V_172 , 0x0b } ,
{ V_173 , 0x8a } ,
{ V_174 , 0x02 } ,
{ V_175 , 0x0e } ,
{ V_176 , 0xf4 } ,
{ V_177 , 0x01 } ,
{ V_56 , 0x00 } ,
{ V_47 , 0x20 } ,
} ;
static const struct V_23 V_57 [] = {
{ V_58 , 0x07 } ,
{ V_59 , 0x58 } ,
{ V_60 , 0x16 } ,
{ V_61 , 0x16 } ,
{ V_62 , 0xc0 } ,
{ V_63 , 0x03 } ,
{ V_64 , 0xff } ,
{ V_65 , 0x07 } ,
{ V_66 , 0xff } ,
{ V_67 , 0x07 } ,
{ V_68 , 0x5c } ,
{ V_69 , 0x01 } ,
{ V_70 , 0xfc } ,
{ V_71 , 0x03 } ,
{ V_72 , 0x00 } ,
{ V_73 , 0x00 } ,
{ V_74 , 0x40 } ,
{ V_75 , 0x01 } ,
{ V_76 , 0x00 } ,
{ V_74 , 0x02 } ,
{ V_77 , 0x05 } ,
{ V_78 , 0x00 } ,
{ V_79 , 0x00 } ,
{ V_80 , 0x01 } ,
{ V_81 , 0x0b } ,
{ V_82 , 0x0b } ,
{ V_83 , 0x07 } ,
{ V_84 , 0x40 } ,
{ V_85 , 0x02 } ,
} ;
F_9 ( V_11 , V_156 , F_19 ( V_156 ) ) ;
F_12 ( V_11 , V_57 , F_19 ( V_57 ) ) ;
F_12 ( V_11 , V_89 , F_19 ( V_89 ) ) ;
F_3 ( V_11 , V_162 , 0x10 ) ;
F_3 ( V_11 , V_161 , 0x00 ) ;
F_10 ( V_11 , V_97 , 0x00 ) ;
F_10 ( V_11 , V_74 , 0x01 ) ;
if ( V_28 -> V_95 == V_50 )
F_21 ( V_11 , F_30 ( V_11 -> V_130 ) ,
F_30 ( V_11 -> V_92 ) ,
F_30 ( V_28 -> V_93 ) ,
F_30 ( V_28 -> V_94 ) ) ;
else
F_21 ( V_11 , F_30 ( V_11 -> V_130 ) ,
F_30 ( V_11 -> V_92 ) , 0 , 0 ) ;
F_41 ( V_11 , 1 ) ;
F_29 ( V_11 ) ;
}
static void F_43 ( struct V_11 * V_11 )
{
struct V_28 * V_28 = (struct V_28 * ) V_11 ;
static const struct V_23 V_178 [] = {
{ V_96 , 0x6f } ,
{ V_97 , 0x02 } ,
{ V_101 , 0x00 } ,
{ V_99 , 0x00 } ,
{ V_105 , 0x00 } ,
{ V_103 , 0x00 } ,
{ V_74 , 0x81 } ,
} ;
static const struct V_23 V_179 [] = {
{ 0x4d , 0x00 } ,
{ 0x4c , 0xff } ,
{ 0x4e , 0xff } ,
{ 0x4f , 0x00 } ,
{ V_110 , 0x00 } ,
{ V_111 , 0x04 } ,
{ V_111 , 0x01 } ,
} ;
static const struct V_23 V_180 [] = {
{ V_172 , 0x08 } ,
{ V_173 , 0x87 } ,
{ V_174 , 0x02 } ,
{ V_175 , 0x0e } ,
{ V_176 , 0xf4 } ,
{ V_177 , 0x01 } ,
} ;
static const struct V_23 V_181 [] = {
{ V_86 , 0x81 } ,
{ V_96 , 0x6f } ,
{ V_97 , 0x02 } ,
{ V_101 , 0x00 } ,
{ V_99 , 0x00 } ,
{ V_105 , 0x00 } ,
{ V_103 , 0x00 } ,
{ V_74 , 0x81 } ,
} ;
static const struct V_23 V_182 [] = {
{ 0x4d , 0x00 } ,
{ 0x4c , 0xff } ,
{ 0x4e , 0xff } ,
{ 0x4f , 0x00 } ,
} ;
static const struct V_23 V_183 [] = {
{ V_96 , 0xd3 } ,
{ V_97 , 0x01 } ,
{ V_101 , 0x40 } ,
{ V_99 , 0x40 } ,
{ V_105 , 0x40 } ,
{ V_103 , 0x40 } ,
{ V_74 , 0x81 } ,
} ;
static const struct V_23 V_184 [] = {
{ V_77 , 0x05 } ,
{ V_78 , 0x00 } ,
{ V_79 , 0x00 } ,
{ V_80 , 0x01 } ,
{ V_81 , 0x0b } ,
{ V_82 , 0x0b } ,
{ V_83 , 0x07 } ,
{ V_84 , 0x40 } ,
{ V_85 , 0x02 } ,
{ V_86 , 0x81 } ,
} ;
F_3 ( V_11 , 0x22 , V_11 -> V_185 ) ;
F_12 ( V_11 , V_178 , F_19 ( V_178 ) ) ;
F_9 ( V_11 , V_179 , F_19 ( V_179 ) ) ;
F_9 ( V_11 , V_88 ,
F_19 ( V_88 ) ) ;
F_9 ( V_11 , V_180 , F_19 ( V_180 ) ) ;
if ( V_11 -> V_154 ) {
F_3 ( V_11 , 0x4a , 0x7f ) ;
F_3 ( V_11 , 0x07 , 0x05 ) ;
F_3 ( V_11 , V_56 , 0x00 ) ;
} else {
F_3 ( V_11 , 0x4a , 0xff ) ;
F_3 ( V_11 , 0x07 , 0x85 ) ;
F_3 ( V_11 , V_56 , 0x01 ) ;
}
F_25 ( V_11 , F_30 ( V_28 -> gamma ) ) ;
F_9 ( V_11 , V_186 ,
F_19 ( V_186 ) ) ;
F_27 ( V_11 , F_30 ( V_28 -> V_187 ) ) ;
F_14 ( V_11 , 0x03 , V_120 [ V_50 ] ,
F_19 ( V_120 [ 0 ] ) ) ;
F_3 ( V_11 , V_54 , 0x87 ) ;
F_12 ( V_11 , V_181 , F_19 ( V_181 ) ) ;
F_9 ( V_11 , V_182 , F_19 ( V_182 ) ) ;
F_12 ( V_11 , V_183 , F_19 ( V_183 ) ) ;
F_9 ( V_11 , V_182 , F_19 ( V_182 ) ) ;
F_12 ( V_11 , V_184 , F_19 ( V_184 ) ) ;
F_41 ( V_11 , 1 ) ;
}
static void F_44 ( struct V_11 * V_11 )
{
struct V_28 * V_28 = (struct V_28 * ) V_11 ;
static const struct V_23 V_156 [] = {
{ V_111 , 0x04 } ,
{ V_111 , 0x01 } ,
{ V_46 , 0x00 } ,
{ V_157 , 0x00 } ,
{ V_158 , 0x00 } ,
{ V_159 , 0xff } ,
{ V_160 , 0x03 } ,
{ V_161 , 0x07 } ,
{ V_162 , 0x40 } ,
{ V_111 , 0x03 } ,
{ V_110 , 0x40 } ,
{ V_52 , 0x46 } ,
{ V_163 , 0x10 } ,
{ V_164 , 0x00 } ,
{ V_165 , 0x00 } ,
{ V_166 , 0x00 } ,
{ V_167 , 0x00 } ,
{ V_168 , 0x00 } ,
{ V_169 , 0x00 } ,
{ V_170 , 0x00 } ,
{ V_171 , 0x00 } ,
{ V_53 , 0x03 } ,
{ V_54 , 0x04 } ,
{ V_117 , 0x0e } ,
{ V_172 , 0x1b } ,
{ V_173 , 0x9a } ,
{ V_174 , 0x02 } ,
{ V_175 , 0x0f } ,
{ V_176 , 0xf4 } ,
{ V_177 , 0x01 } ,
{ V_56 , 0x01 } ,
{ V_47 , 0x21 } ,
{ V_48 , 0x00 } ,
} ;
static const struct V_23 V_57 [] = {
{ 0x12 , 0x48 } ,
{ 0x13 , 0xa0 } ,
{ 0x03 , 0xa4 } ,
{ 0x04 , 0x30 } ,
{ 0x05 , 0x88 } ,
{ 0x06 , 0x60 } ,
{ 0x10 , 0x41 } ,
{ 0x11 , 0x40 } ,
{ 0x13 , 0xa0 } ,
{ 0x14 , 0x00 } ,
{ 0x15 , 0x14 } ,
{ 0x1f , 0x41 } ,
{ 0x20 , 0x80 } ,
{ 0x23 , 0xee } ,
{ 0x24 , 0x50 } ,
{ 0x25 , 0x7a } ,
{ 0x26 , 0x00 } ,
{ 0x27 , 0xe2 } ,
{ 0x28 , 0xb0 } ,
{ 0x2a , 0x00 } ,
{ 0x2b , 0x00 } ,
{ 0x2d , 0x81 } ,
{ 0x2f , 0x9d } ,
{ 0x60 , 0x80 } ,
{ 0x61 , 0x00 } ,
{ 0x62 , 0x88 } ,
{ 0x63 , 0x11 } ,
{ 0x64 , 0x89 } ,
{ 0x65 , 0x00 } ,
{ 0x67 , 0x94 } ,
{ 0x68 , 0x7a } ,
{ 0x69 , 0x0f } ,
{ 0x6c , 0x80 } ,
{ 0x6d , 0x80 } ,
{ 0x6e , 0x80 } ,
{ 0x6f , 0xff } ,
{ 0x71 , 0x20 } ,
{ 0x74 , 0x20 } ,
{ 0x75 , 0x86 } ,
{ 0x77 , 0xb5 } ,
{ 0x17 , 0x18 } ,
{ 0x18 , 0xbf } ,
{ 0x19 , 0x03 } ,
{ 0x1a , 0xf8 } ,
{ 0x01 , 0x80 } ,
{ 0x02 , 0x80 } ,
} ;
F_9 ( V_11 , V_156 , F_19 ( V_156 ) ) ;
F_10 ( V_11 , 0x12 , 0x80 ) ;
F_11 ( 10 ) ;
F_12 ( V_11 , V_57 , F_19 ( V_57 ) ) ;
F_3 ( V_11 , V_162 , 0x10 ) ;
F_3 ( V_11 , V_161 , 0x00 ) ;
F_27 ( V_11 , F_30 ( V_28 -> V_187 ) ) ;
F_14 ( V_11 , 0x03 , V_120 [ V_49 ] ,
F_19 ( V_120 [ 0 ] ) ) ;
F_41 ( V_11 , 1 ) ;
if ( V_28 -> V_95 == V_50 )
F_21 ( V_11 , F_30 ( V_11 -> V_130 ) ,
F_30 ( V_11 -> V_92 ) ,
F_30 ( V_28 -> V_93 ) ,
F_30 ( V_28 -> V_94 ) ) ;
else
F_21 ( V_11 , F_30 ( V_11 -> V_130 ) ,
F_30 ( V_11 -> V_92 ) , 0 , 0 ) ;
if ( V_28 -> V_95 == V_49 )
F_24 ( V_11 , F_30 ( V_28 -> V_121 ) ) ;
F_25 ( V_11 , F_30 ( V_28 -> gamma ) ) ;
}
static void F_45 ( struct V_11 * V_11 )
{
struct V_28 * V_28 = (struct V_28 * ) V_11 ;
static const struct V_23 V_179 [] = {
{ V_110 , 0x00 } ,
{ V_111 , 0x04 } ,
{ V_111 , 0x01 } ,
} ;
static const struct V_23 V_180 [] = {
{ V_172 , 0x20 } ,
{ V_173 , 0x9f } ,
{ V_174 , 0x02 } ,
{ V_175 , 0x13 } ,
{ V_176 , 0xf8 } ,
{ V_177 , 0x01 } ,
} ;
static const struct V_23 V_188 [] = {
{ 0x08 , 0xff } ,
{ 0x09 , 0xff } ,
{ 0x0a , 0x5f } ,
{ 0x0b , 0x80 } ,
} ;
F_3 ( V_11 , 0x22 , V_11 -> V_185 ) ;
F_14 ( V_11 , 0x03 , V_153 , sizeof V_153 ) ;
F_3 ( V_11 , 0x59 , 0x40 ) ;
if ( V_28 -> V_95 == V_50 )
F_21 ( V_11 , F_30 ( V_11 -> V_130 ) ,
F_30 ( V_11 -> V_92 ) ,
F_30 ( V_28 -> V_93 ) ,
F_30 ( V_28 -> V_94 ) ) ;
else
F_21 ( V_11 , F_30 ( V_11 -> V_130 ) ,
F_30 ( V_11 -> V_92 ) , 0 , 0 ) ;
F_9 ( V_11 , V_179 , F_19 ( V_179 ) ) ;
F_9 ( V_11 , V_90 ,
F_19 ( V_90 ) ) ;
F_9 ( V_11 , V_180 , F_19 ( V_180 ) ) ;
if ( V_11 -> V_154 ) {
F_3 ( V_11 , 0x4a , 0x7f ) ;
F_3 ( V_11 , 0x07 , 0x05 ) ;
F_3 ( V_11 , V_56 , 0x00 ) ;
} else {
F_3 ( V_11 , 0x4a , 0xff ) ;
F_3 ( V_11 , 0x07 , 0x85 ) ;
F_3 ( V_11 , V_56 , 0x01 ) ;
}
F_25 ( V_11 , F_30 ( V_28 -> gamma ) ) ;
F_9 ( V_11 , V_186 ,
F_19 ( V_186 ) ) ;
if ( V_11 -> V_154 ) {
F_3 ( V_11 , 0x4f , 0x00 ) ;
F_3 ( V_11 , 0x4e , 0x7c ) ;
}
F_3 ( V_11 , 0x00 , 0x00 ) ;
F_27 ( V_11 , F_30 ( V_28 -> V_187 ) ) ;
F_14 ( V_11 , 0x03 , V_120 [ V_49 ] ,
F_19 ( V_120 [ 0 ] ) ) ;
F_41 ( V_11 , 1 ) ;
F_3 ( V_11 , V_54 , 0xf0 ) ;
if ( V_28 -> V_95 == V_50 )
F_21 ( V_11 , F_30 ( V_11 -> V_130 ) ,
F_30 ( V_11 -> V_92 ) ,
F_30 ( V_28 -> V_93 ) ,
F_30 ( V_28 -> V_94 ) ) ;
else
F_21 ( V_11 , F_30 ( V_11 -> V_130 ) ,
F_30 ( V_11 -> V_92 ) , 0 , 0 ) ;
F_9 ( V_11 , V_188 , F_19 ( V_188 ) ) ;
}
static int F_46 ( struct V_11 * V_11 )
{
struct V_28 * V_28 = (struct V_28 * ) V_11 ;
F_1 ( V_28 -> V_1 , V_11 -> V_2 , V_11 -> V_3 ) ;
F_22 ( V_11 , V_28 -> V_6 ) ;
if ( V_28 -> V_32 == V_33 ) {
if ( V_28 -> V_95 == V_50 )
F_42 ( V_11 ) ;
else
F_44 ( V_11 ) ;
} else {
if ( V_28 -> V_95 == V_50 )
F_43 ( V_11 ) ;
else
F_45 ( V_11 ) ;
F_9 ( V_11 , V_189 ,
F_19 ( V_189 ) ) ;
F_3 ( V_11 , 0x80 , 0x03 ) ;
F_3 ( V_11 , 0x82 , V_11 -> V_154 ? 0x0a : 0x0e ) ;
if ( V_28 -> V_95 == V_50 )
F_21 ( V_11 ,
F_30 ( V_11 -> V_130 ) ,
F_30 ( V_11 -> V_92 ) ,
F_30 ( V_28 -> V_93 ) ,
F_30 ( V_28 -> V_94 ) ) ;
else
F_21 ( V_11 ,
F_30 ( V_11 -> V_130 ) ,
F_30 ( V_11 -> V_92 ) , 0 , 0 ) ;
if ( V_28 -> V_95 == V_49 )
F_24 ( V_11 ,
F_30 ( V_28 -> V_121 ) ) ;
if ( V_28 -> V_32 == V_112 )
F_28 ( V_11 ,
F_30 ( V_11 -> V_190 ) ) ;
}
F_32 ( V_11 , F_30 ( V_11 -> V_130 ) ) ;
return V_11 -> V_17 ;
}
static void F_47 ( struct V_11 * V_11 )
{
struct V_28 * V_28 = (struct V_28 * ) V_11 ;
if ( V_28 -> V_32 == V_33 )
F_3 ( V_11 , V_52 , 0x03 ) ;
F_41 ( V_11 , 0 ) ;
F_3 ( V_11 , V_53 , 0x00 ) ;
}
static void F_48 ( struct V_11 * V_11 ,
T_1 * V_39 ,
int V_34 )
{
struct V_28 * V_28 = (struct V_28 * ) V_11 ;
if ( V_28 -> V_32 == V_112 ) {
if ( * V_39 != 0x5a ) {
if ( * V_39 == 0xaa || * V_39 == 0x00 )
return;
if ( * V_39 > 0xc0 ) {
F_23 ( V_191 , L_9 ) ;
V_11 -> V_192 = V_193 ;
return;
}
}
V_39 ++ ;
V_34 -- ;
if ( * V_39 == 0xff && V_39 [ 1 ] == 0xd8 ) {
if ( ( V_39 [ 6 ] & 0x0f ) != V_28 -> V_6 )
F_22 ( V_11 , V_39 [ 6 ] & 0x0f ) ;
F_49 ( V_11 , V_194 ,
V_28 -> V_1 , V_195 ) ;
F_49 ( V_11 , V_196 ,
V_39 + 7 , V_34 - 7 ) ;
} else if ( V_39 [ V_34 - 2 ] == 0xff && V_39 [ V_34 - 1 ] == 0xd9 ) {
F_49 ( V_11 , V_197 ,
V_39 , V_34 ) ;
} else {
F_49 ( V_11 , V_196 ,
V_39 , V_34 ) ;
}
return;
}
switch ( * V_39 ) {
case 0x55 :
F_49 ( V_11 , V_197 , V_39 , 0 ) ;
if ( V_34 < 8
|| V_39 [ 1 ] != 0xff || V_39 [ 2 ] != 0xd8
|| V_39 [ 3 ] != 0xff || V_39 [ 4 ] != 0xfe ) {
V_11 -> V_192 = V_193 ;
return;
}
if ( V_39 [ 7 ] != V_28 -> V_6 )
F_22 ( V_11 , V_39 [ 7 ] ) ;
F_49 ( V_11 , V_194 ,
V_28 -> V_1 , V_195 ) ;
F_49 ( V_11 , V_196 ,
V_39 + 8 , V_34 - 8 ) ;
break;
case 0xaa :
V_11 -> V_192 = V_193 ;
break;
case 0xcc :
if ( V_39 [ 1 ] != 0xff || V_39 [ 2 ] != 0xd8 )
F_49 ( V_11 , V_196 ,
V_39 + 1 , V_34 - 1 ) ;
else
V_11 -> V_192 = V_193 ;
break;
}
}
static void F_50 ( struct V_11 * V_11 )
{
struct V_28 * V_28 = (struct V_28 * ) V_11 ;
int V_16 , V_198 ;
int V_199 , V_91 ;
if ( V_28 -> V_118 < 0 )
return;
if ( -- V_28 -> V_118 > 5 )
return;
switch ( V_28 -> V_118 ) {
default:
F_3 ( V_11 , 0x7d , 0x00 ) ;
break;
case 4 :
F_3 ( V_11 , 0x27 , 0xb0 ) ;
break;
case 3 :
F_3 ( V_11 , 0x0c , 0x01 ) ;
break;
case 2 :
V_16 = F_15 ( V_11 -> V_15 ,
F_51 ( V_11 -> V_15 , 0x02 ) ,
V_11 -> V_22 ,
32 ,
& V_198 ,
500 ) ;
if ( V_16 < 0 ) {
F_6 ( L_10 , V_16 ) ;
break;
}
break;
case 1 :
F_3 ( V_11 , 0x27 , 0xd0 ) ;
break;
case 0 :
V_16 = F_15 ( V_11 -> V_15 ,
F_51 ( V_11 -> V_15 , 0x02 ) ,
V_11 -> V_22 ,
32 ,
& V_198 ,
500 ) ;
if ( V_16 < 0 ) {
F_6 ( L_10 , V_16 ) ;
break;
}
V_199 = ( ( V_11 -> V_22 [ 8 ] << 8 ) + V_11 -> V_22 [ 7 ] +
( V_11 -> V_22 [ 11 ] << 8 ) + V_11 -> V_22 [ 10 ] +
( V_11 -> V_22 [ 14 ] << 8 ) + V_11 -> V_22 [ 13 ] +
( V_11 -> V_22 [ 17 ] << 8 ) + V_11 -> V_22 [ 16 ] +
( V_11 -> V_22 [ 20 ] << 8 ) + V_11 -> V_22 [ 19 ] +
( V_11 -> V_22 [ 23 ] << 8 ) + V_11 -> V_22 [ 22 ] +
( V_11 -> V_22 [ 26 ] << 8 ) + V_11 -> V_22 [ 25 ] +
( V_11 -> V_22 [ 29 ] << 8 ) + V_11 -> V_22 [ 28 ] )
/ 8 ;
if ( V_11 -> V_3 == 640 )
V_199 /= 4 ;
F_3 ( V_11 , 0x7d , 0x00 ) ;
V_91 = F_30 ( V_11 -> V_130 ) ;
V_16 = F_52 ( V_11 , V_199 ,
60 ,
6 ,
2 ,
70 ) ;
V_28 -> V_118 = V_119 ;
if ( V_28 -> V_32 == V_112 ) {
int V_200 = F_30 ( V_11 -> V_130 ) ;
if ( ( V_91 >= 128 && V_200 < 128 )
|| ( V_91 < 128 && V_200 >= 128 ) )
F_32 ( V_11 , V_200 ) ;
}
break;
}
}
static void F_53 ( struct V_11 * V_11 ,
struct V_201 * V_202 )
{
struct V_28 * V_28 = (struct V_28 * ) V_11 ;
struct V_203 * V_204 = & V_202 -> V_202 . V_205 ;
struct V_206 * V_207 = & V_204 -> V_208 ;
int V_209 , V_7 ;
V_204 -> V_210 |= V_211 ;
V_207 -> V_212 = 1 ;
V_7 = F_31 ( V_11 ) ;
if ( V_7 & 0x80 ) {
if ( V_28 -> V_32 == V_33 )
V_209 = V_122 [ 6 - ( V_7 & 0x07 ) ] ;
else
V_209 = V_124 [ 7 - ( V_7 & 0x07 ) ] ;
} else {
V_209 = V_122 [ 6 - V_7 ] ;
}
V_207 -> V_213 = V_209 ;
}
static void F_54 ( struct V_11 * V_11 ,
struct V_201 * V_202 )
{
struct V_28 * V_28 = (struct V_28 * ) V_11 ;
struct V_203 * V_204 = & V_202 -> V_202 . V_205 ;
struct V_206 * V_207 = & V_204 -> V_208 ;
int V_209 , V_7 ;
V_28 -> V_123 = V_207 -> V_213 / V_207 -> V_212 ;
if ( V_11 -> V_129 )
F_32 ( V_11 , F_30 ( V_11 -> V_130 ) ) ;
V_7 = F_31 ( V_11 ) ;
if ( V_7 & 0x80 )
V_209 = V_124 [ 7 - ( V_7 & 0x07 ) ] ;
else
V_209 = V_122 [ 6 - V_7 ] ;
V_207 -> V_212 = 1 ;
V_207 -> V_213 = V_209 ;
}
static int F_55 ( struct V_11 * V_11 ,
struct V_214 * V_215 )
{
struct V_28 * V_28 = (struct V_28 * ) V_11 ;
if ( V_28 -> V_95 != V_49 )
return - V_216 ;
F_56 ( V_28 -> V_121 , V_215 -> V_6 ) ;
return 0 ;
}
static int F_57 ( struct V_11 * V_11 ,
struct V_214 * V_215 )
{
struct V_28 * V_28 = (struct V_28 * ) V_11 ;
if ( V_28 -> V_95 != V_49 )
return - V_216 ;
memset ( V_215 , 0 , sizeof *V_215 ) ;
V_215 -> V_6 = F_30 ( V_28 -> V_121 ) ;
V_215 -> V_217 = V_218
| V_219 ;
return 0 ;
}
static int F_58 ( struct V_220 * V_221 )
{
struct V_11 * V_11 =
F_59 ( V_221 -> V_222 , struct V_11 , V_223 ) ;
struct V_28 * V_28 = (struct V_28 * ) V_11 ;
V_11 -> V_17 = 0 ;
if ( ! V_11 -> V_129 )
return 0 ;
switch ( V_221 -> V_133 ) {
case V_224 :
F_27 ( V_11 , V_221 -> V_27 ) ;
break;
case V_225 :
F_25 ( V_11 , V_221 -> V_27 ) ;
break;
case V_226 :
F_35 ( V_11 , V_221 -> V_27 ) ;
break;
case V_227 :
F_33 ( V_11 , V_221 -> V_27 ) ;
break;
case V_228 :
F_34 ( V_11 ) ;
break;
case V_229 :
if ( V_221 -> V_27 )
break;
F_34 ( V_11 ) ;
break;
case V_230 :
F_2 ( V_28 -> V_1 , V_221 -> V_27 ) ;
break;
}
return V_11 -> V_17 ;
}
static int F_60 ( struct V_11 * V_11 )
{
struct V_28 * V_28 = (struct V_28 * ) V_11 ;
struct V_231 * V_232 = & V_11 -> V_223 ;
V_11 -> V_233 . V_223 = V_232 ;
F_61 ( V_232 , 4 ) ;
V_11 -> V_130 = F_62 ( V_232 , & V_234 ,
V_228 , 1 , 0xdc , 1 , 0x4e ) ;
if ( V_28 -> V_95 == V_50 ) {
V_28 -> V_94 = F_62 ( V_232 , & V_234 ,
V_227 , 0 , 4095 , 1 , 256 ) ;
V_28 -> V_93 = F_62 ( V_232 , & V_234 ,
V_226 , 0 , 4095 , 1 , 256 ) ;
}
if ( V_28 -> V_95 == V_49 )
V_11 -> V_92 = F_62 ( V_232 , & V_234 ,
V_235 , 0 , 15 , 1 , 3 ) ;
else
V_11 -> V_92 = F_62 ( V_232 , & V_234 ,
V_235 , 0 , 4095 , 1 , 256 ) ;
V_28 -> V_187 = F_62 ( V_232 , & V_234 ,
V_224 , 0 , 3 , 1 , 2 ) ;
V_28 -> gamma = F_62 ( V_232 , & V_234 ,
V_225 , 0 , F_26 - 1 , 1 ,
( V_28 -> V_95 == V_49 &&
V_28 -> V_32 == V_33 ) ? 0 : 1 ) ;
if ( V_28 -> V_32 == V_112 )
V_11 -> V_190 = F_62 ( V_232 , & V_234 ,
V_229 , 0 , 1 , 1 , 1 ) ;
if ( V_28 -> V_95 == V_49 )
V_28 -> V_121 = F_62 ( V_232 , & V_234 ,
V_230 ,
0 , 15 , 1 , ( V_28 -> V_32 == V_112 ) ? 0 : 13 ) ;
if ( V_232 -> error ) {
F_6 ( L_11 ) ;
return V_232 -> error ;
}
if ( V_11 -> V_190 )
F_63 ( 3 , & V_11 -> V_190 , 0 , false ) ;
else
F_64 ( 2 , & V_11 -> V_130 ) ;
return 0 ;
}
static int F_65 ( struct V_236 * V_237 ,
const struct V_132 * V_133 )
{
return F_66 ( V_237 , V_133 , & V_238 , sizeof( struct V_28 ) ,
V_239 ) ;
}
