static inline T_1 F_1 ( struct V_1 * V_2 ,
unsigned int V_3 )
{
return F_2 ( V_2 -> V_4 + F_3 ( V_3 ) ) ;
}
static inline T_2 F_4 ( struct V_1 * V_2 ,
unsigned int V_3 )
{
return F_5 ( V_2 -> V_4 + F_3 ( V_3 ) ) ;
}
static inline void F_6 ( struct V_1 * V_2 ,
T_1 V_5 , unsigned int V_3 )
{
F_7 ( V_5 , V_2 -> V_4 + F_3 ( V_3 ) ) ;
}
static inline void F_8 ( struct V_1 * V_2 ,
T_2 V_5 , unsigned int V_3 )
{
F_9 ( V_5 , V_2 -> V_4 + F_3 ( V_3 ) ) ;
}
static int F_10 ( struct V_6 * V_7 ,
struct V_8 * V_9 )
{
T_1 V_10 ;
V_10 = ( V_9 ) ? V_9 -> V_10 : V_7 -> V_10 ;
if ( V_10 != 8 ) {
F_11 ( & V_7 -> V_11 , L_1 ,
V_12 , V_10 ) ;
return - V_13 ;
}
if ( V_7 -> V_14 > V_7 -> V_15 -> V_16 ) {
F_11 ( & V_7 -> V_11 , L_2 ,
V_12 , V_7 -> V_14 ) ;
return - V_13 ;
}
return 0 ;
}
static void F_12 ( struct V_6 * V_7 ,
struct V_8 * V_9 )
{
struct V_1 * V_2 = F_13 ( V_7 -> V_15 ) ;
T_3 V_17 ;
T_1 V_18 , V_19 ;
int V_20 ;
V_17 = ( V_9 ) ? V_9 -> V_21 : V_7 -> V_22 ;
for ( V_20 = 0 ; V_20 < V_23 ; V_20 ++ ) {
if ( V_17 >= V_24 [ V_20 ] [ 0 ] ) {
V_18 = V_24 [ V_20 ] [ 1 ] ;
break;
}
}
if ( V_20 == V_23 )
V_18 = V_25 ;
V_19 = F_1 ( V_2 , V_26 ) ;
V_19 &= ~ V_23 ;
V_19 |= V_18 ;
F_6 ( V_2 , V_19 , V_26 ) ;
F_14 ( & V_7 -> V_11 , L_3 ,
V_18 , V_17 ) ;
}
static int F_15 ( struct V_6 * V_7 )
{
struct V_1 * V_2 ;
int V_27 ;
V_2 = F_13 ( V_7 -> V_15 ) ;
if ( ! V_7 -> V_10 )
V_7 -> V_10 = 8 ;
if ( V_7 -> V_28 & ~ V_29 ) {
F_11 ( & V_7 -> V_11 , L_4 ,
V_12 , V_7 -> V_28 & ~ V_29 ) ;
return - V_13 ;
}
V_27 = F_10 ( V_7 , NULL ) ;
if ( V_27 < 0 ) {
F_11 ( & V_7 -> V_11 , L_5 ,
V_7 -> V_28 & ~ V_29 ) ;
return V_27 ;
}
F_14 ( & V_7 -> V_11 , L_6 ,
V_12 , V_7 -> V_28 & V_29 , V_7 -> V_10 , 0 ) ;
return 0 ;
}
static void F_16 ( struct V_1 * V_2 )
{
T_1 V_30 ;
V_30 = V_2 -> V_31 < V_2 -> V_32 ? V_2 -> V_31 :
V_2 -> V_32 ;
F_17 ( V_2 -> V_33 , V_2 -> V_34 , V_30 ) ;
V_2 -> V_31 -= V_30 ;
}
static unsigned int F_18 ( struct V_6 * V_7 ,
struct V_8 * V_9 )
{
struct V_1 * V_2 = F_13 ( V_7 -> V_15 ) ;
T_2 V_35 ;
T_2 V_36 ;
F_6 ( V_2 , 0 , V_37 ) ;
F_14 ( & V_7 -> V_11 , L_7 ,
V_9 -> V_38 , V_9 -> V_39 , V_9 -> V_40 ) ;
V_2 -> V_34 = V_9 -> V_38 ;
V_2 -> V_41 = V_9 -> V_39 ;
if ( V_9 -> V_38 ) {
V_2 -> V_31 = V_9 -> V_40 ;
F_16 ( V_2 ) ;
}
F_19 ( & V_2 -> V_42 ) ;
V_35 = ( V_9 -> V_40 << V_43 ) ;
if ( V_9 -> V_39 && V_9 -> V_38 )
V_35 |= ( V_44 << V_2 -> V_45 ) ;
else if ( V_9 -> V_39 )
V_35 |= ( V_46 << V_2 -> V_45 ) ;
else if ( V_9 -> V_38 )
V_35 |= ( V_47 << V_2 -> V_45 ) ;
switch ( V_2 -> V_48 ) {
case 8 :
F_6 ( V_2 , V_35 , V_49 ) ;
break;
case 16 :
F_8 ( V_2 , V_35 , V_49 ) ;
break;
}
V_36 = V_50 ;
V_36 |= ( 0 << V_51 ) ;
V_36 |= ( V_7 -> V_14 << V_52 ) ;
F_8 ( V_2 , V_36 , V_53 ) ;
F_6 ( V_2 , V_54 , V_37 ) ;
return V_9 -> V_40 - V_2 -> V_31 ;
}
static int F_20 ( struct V_55 * V_15 )
{
struct V_1 * V_2 = F_13 ( V_15 ) ;
F_21 ( & V_2 -> V_56 -> V_11 ) ;
return 0 ;
}
static int F_22 ( struct V_55 * V_15 )
{
struct V_1 * V_2 = F_13 ( V_15 ) ;
F_23 ( & V_2 -> V_56 -> V_11 ) ;
return 0 ;
}
static int F_24 ( struct V_55 * V_15 ,
struct V_57 * V_58 )
{
struct V_1 * V_2 = F_13 ( V_15 ) ;
struct V_8 * V_9 ;
struct V_6 * V_7 = V_58 -> V_7 ;
int V_59 = 0 ;
unsigned int V_60 = 0 ;
F_25 (t, &m->transfers, transfer_list) {
unsigned int V_40 = V_9 -> V_40 ;
T_1 V_61 ;
V_59 = F_10 ( V_7 , V_9 ) ;
if ( V_59 < 0 )
goto exit;
F_12 ( V_7 , V_9 ) ;
while ( V_40 ) {
V_40 -= F_18 ( V_7 , V_9 ) ;
V_60 = F_26 ( & V_2 -> V_42 , V_62 ) ;
if ( ! V_60 ) {
V_59 = - V_63 ;
goto exit;
}
V_61 = F_1 ( V_2 , V_64 ) ;
if ( V_61 )
F_27 ( V_2 -> V_41 , V_2 -> V_65 , V_61 ) ;
}
V_58 -> V_66 += V_9 -> V_40 ;
}
exit:
V_58 -> V_59 = V_59 ;
F_28 ( V_15 ) ;
return 0 ;
}
static T_4 F_29 ( int V_67 , void * V_68 )
{
struct V_55 * V_15 = (struct V_55 * ) V_68 ;
struct V_1 * V_2 = F_13 ( V_15 ) ;
T_1 V_69 ;
V_69 = F_1 ( V_2 , V_70 ) ;
F_6 ( V_2 , V_71 , V_70 ) ;
F_6 ( V_2 , 0 , V_37 ) ;
if ( V_69 & V_54 )
F_30 ( & V_2 -> V_42 ) ;
return V_72 ;
}
static int T_5 F_31 ( struct V_73 * V_56 )
{
struct V_74 * V_75 ;
struct V_76 * V_11 = & V_56 -> V_11 ;
struct V_77 * V_78 = V_56 -> V_11 . V_79 ;
int V_67 ;
struct V_55 * V_15 ;
struct V_80 * V_80 ;
struct V_1 * V_2 ;
int V_27 ;
V_75 = F_32 ( V_56 , V_81 , 0 ) ;
if ( ! V_75 ) {
F_11 ( V_11 , L_8 ) ;
V_27 = - V_82 ;
goto V_83;
}
V_67 = F_33 ( V_56 , 0 ) ;
if ( V_67 < 0 ) {
F_11 ( V_11 , L_9 ) ;
V_27 = - V_82 ;
goto V_83;
}
V_80 = F_34 ( V_11 , L_10 ) ;
if ( F_35 ( V_80 ) ) {
F_11 ( V_11 , L_11 ) ;
V_27 = F_36 ( V_80 ) ;
goto V_83;
}
V_15 = F_37 ( V_11 , sizeof( * V_2 ) ) ;
if ( ! V_15 ) {
F_11 ( V_11 , L_12 ) ;
V_27 = - V_84 ;
goto V_85;
}
V_2 = F_13 ( V_15 ) ;
F_38 ( V_56 , V_15 ) ;
V_2 -> V_56 = V_56 ;
if ( ! F_39 ( & V_56 -> V_11 , V_75 -> V_86 ,
F_40 ( V_75 ) , V_87 ) ) {
F_11 ( V_11 , L_13 ) ;
V_27 = - V_82 ;
goto V_88;
}
V_2 -> V_4 = F_41 ( & V_56 -> V_11 , V_75 -> V_86 ,
F_40 ( V_75 ) ) ;
if ( ! V_2 -> V_4 ) {
F_11 ( V_11 , L_14 ) ;
V_27 = - V_84 ;
goto V_88;
}
V_2 -> V_67 = V_67 ;
V_2 -> V_80 = V_80 ;
V_2 -> V_32 = V_78 -> V_32 ;
V_27 = F_42 ( & V_56 -> V_11 , V_67 , F_29 , 0 ,
V_56 -> V_89 , V_15 ) ;
if ( V_27 ) {
F_11 ( V_11 , L_15 ) ;
goto V_88;
}
V_15 -> V_90 = V_78 -> V_90 ;
V_15 -> V_16 = V_78 -> V_16 ;
V_15 -> V_91 = F_15 ;
V_15 -> V_92 = F_20 ;
V_15 -> V_93 = F_22 ;
V_15 -> V_94 = F_24 ;
V_15 -> V_95 = V_29 ;
V_2 -> V_21 = V_78 -> V_21 ;
V_2 -> V_45 = V_78 -> V_45 ;
V_2 -> V_48 = V_78 -> V_48 ;
V_2 -> V_33 = ( T_1 * ) ( V_2 -> V_4 + F_3 ( V_96 ) ) ;
V_2 -> V_65 = ( const T_1 * ) ( V_2 -> V_4 + F_3 ( V_97 ) ) ;
switch ( V_2 -> V_48 ) {
case 8 :
case 16 :
break;
default:
F_11 ( V_11 , L_16 ,
V_2 -> V_48 ) ;
goto V_98;
}
F_43 ( V_2 -> V_80 ) ;
F_6 ( V_2 , V_71 , V_70 ) ;
V_27 = F_44 ( V_15 ) ;
if ( V_27 ) {
F_11 ( V_11 , L_17 ) ;
goto V_98;
}
F_45 ( V_11 , L_18 ,
V_75 -> V_86 , V_67 , V_2 -> V_32 , V_99 ) ;
return 0 ;
V_98:
F_46 ( V_80 ) ;
V_88:
F_38 ( V_56 , NULL ) ;
F_47 ( V_15 ) ;
V_85:
F_48 ( V_80 ) ;
V_83:
return V_27 ;
}
static int T_6 F_49 ( struct V_73 * V_56 )
{
struct V_55 * V_15 = F_50 ( F_51 ( V_56 ) ) ;
struct V_1 * V_2 = F_13 ( V_15 ) ;
F_52 ( V_15 ) ;
F_6 ( V_2 , 0 , V_37 ) ;
F_46 ( V_2 -> V_80 ) ;
F_48 ( V_2 -> V_80 ) ;
F_38 ( V_56 , 0 ) ;
F_47 ( V_15 ) ;
return 0 ;
}
static int F_53 ( struct V_76 * V_11 )
{
struct V_55 * V_15 =
F_51 ( F_54 ( V_11 ) ) ;
struct V_1 * V_2 = F_13 ( V_15 ) ;
F_46 ( V_2 -> V_80 ) ;
return 0 ;
}
static int F_55 ( struct V_76 * V_11 )
{
struct V_55 * V_15 =
F_51 ( F_54 ( V_11 ) ) ;
struct V_1 * V_2 = F_13 ( V_15 ) ;
F_43 ( V_2 -> V_80 ) ;
return 0 ;
}
