static T_1 F_1 ( struct V_1 * V_2 )
{
struct V_3 * V_4 = V_2 -> V_5 ;
T_1 V_6 ;
if ( F_2 ( V_2 , false ) ) {
V_6 = F_3 ( V_7 ) ;
F_4 ( V_2 ) ;
} else
V_6 = V_4 -> V_8 ;
V_6 = ( V_6 & V_9 ) >>
V_10 ;
V_6 *= 2 ;
if ( V_6 == 0 )
F_5 ( V_2 -> V_2 , L_1 ,
F_3 ( V_7 ) , V_4 -> V_8 ) ;
return V_6 ;
}
static int F_6 ( struct V_1 * V_2 ,
unsigned int V_11 )
{
struct V_3 * V_4 =
(struct V_3 * ) V_2 -> V_5 ;
struct V_12 * V_13 = V_4 -> V_13 ;
T_2 V_14 [ 2 ] ;
unsigned int V_15 ;
struct V_16 V_17 [] = {
{
. V_18 = V_13 -> V_19 ,
. V_20 = 0 ,
. V_21 = 2 ,
. V_22 = V_14 ,
}
} ;
V_15 = V_23 & ( ( unsigned int ) V_11 *
V_23 /
V_24 ) ;
if ( V_4 -> V_25 -> V_26 == V_27 )
V_15 = V_23 - V_15 ;
V_14 [ 0 ] = V_4 -> V_25 -> V_28 ;
V_14 [ 1 ] = ( T_2 ) V_15 ;
if ( F_7 ( & V_13 -> V_29 , V_17 , 1 ) == 1 ) {
F_8 ( V_2 -> V_2 , L_2 ,
V_4 -> V_25 -> V_28 ,
V_15 ) ;
return 0 ;
}
F_5 ( V_2 -> V_2 , L_3 ) ;
return - 1 ;
}
static int F_9 ( struct V_1 * V_2 , int V_11 )
{
struct V_3 * V_4 =
(struct V_3 * ) V_2 -> V_5 ;
T_1 V_30 ;
T_1 V_31 ;
V_30 = F_1 ( V_2 ) ;
F_10 ( V_30 == 0 ) ;
V_31 = V_11 * V_30 / V_24 ;
if ( V_4 -> V_25 -> V_26 == V_27 )
V_31 = V_30 - V_31 ;
V_31 &= V_32 ;
F_11 ( V_7 ,
( V_30 << V_33 ) |
( V_31 ) ) ;
F_12 ( V_2 -> V_2 , L_4 ,
( V_30 << V_33 ) |
( V_31 ) ) ;
return 0 ;
}
void F_13 ( struct V_1 * V_2 , int V_11 )
{
struct V_3 * V_4 = V_2 -> V_5 ;
F_8 ( V_2 -> V_2 , L_5 , V_11 ) ;
if ( ! V_4 -> V_25 ) {
F_5 ( V_2 -> V_2 , L_6 ) ;
return;
}
if ( V_4 -> V_25 -> type == V_34 )
F_6 ( V_2 , V_11 ) ;
else
F_9 ( V_2 , V_11 ) ;
}
static void F_14 ( struct V_1 * V_2 , int V_11 )
{
struct V_3 * V_4 = V_2 -> V_5 ;
T_1 V_35 ;
if ( F_2 ( V_2 , false ) ) {
V_35 = F_3 ( V_7 ) ;
V_35 &= ~ V_36 ;
F_11 ( V_7 ,
( V_35 |
( V_11 << V_37 ) ) ) ;
V_4 -> V_8 = ( V_35 |
( V_11 << V_37 ) ) ;
F_4 ( V_2 ) ;
} else {
V_35 = V_4 -> V_8 &
~ V_36 ;
V_4 -> V_8 = ( V_35 |
( V_11 << V_37 ) ) ;
}
}
static void F_15 ( struct V_1 * V_2 ,
struct V_38 * V_39 , bool V_40 )
{
T_1 V_41 ;
if ( ! F_2 ( V_2 , true ) ) {
F_5 ( V_2 -> V_2 , L_7 ) ;
return;
}
if ( V_40 ) {
F_11 ( V_42 , F_3 ( V_42 ) |
V_43 ) ;
do {
V_41 = F_3 ( V_44 ) ;
} while ( ( V_41 & V_45 ) == 0 );
F_14 ( V_2 ,
V_39 ->
V_46 -> V_47 ) ;
} else {
F_14 ( V_2 , 0 ) ;
F_11 ( V_42 , F_3 ( V_42 ) &
~ V_43 ) ;
do {
V_41 = F_3 ( V_44 ) ;
} while ( V_41 & V_45 );
}
F_4 ( V_2 ) ;
}
static void F_16 ( struct V_48 * V_49 , int V_50 )
{
struct V_1 * V_2 = V_49 -> V_2 ;
struct V_38 * V_39 = F_17 ( V_49 ) ;
if ( V_50 == V_51 )
F_15 ( V_2 , V_39 , true ) ;
else
F_15 ( V_2 , V_39 , false ) ;
}
static void F_18 ( struct V_52 * V_53 )
{
struct V_1 * V_2 = V_53 -> V_2 ;
struct V_3 * V_4 =
(struct V_3 * ) V_2 -> V_5 ;
struct V_38 * V_38 =
F_19 ( V_53 ) ;
struct V_54 * V_55 =
(struct V_54 * ) V_38 -> V_4 ;
V_55 -> V_56 = F_3 ( V_57 ) ;
V_55 -> V_58 = F_3 ( V_59 ) ;
V_55 -> V_60 = F_3 ( V_61 ) ;
V_55 -> V_62 = F_3 ( V_42 ) ;
V_55 -> V_63 = F_3 ( V_64 ) ;
V_55 -> V_8 = F_3 ( V_7 ) ;
V_55 -> V_65 = F_3 ( V_66 ) ;
V_55 -> V_67 = F_3 ( V_68 ) ;
V_4 -> V_47 = ( V_4 -> V_8 &
V_36 ) ;
if ( V_4 -> V_47 == 0 )
V_4 -> V_47 =
F_1 ( V_2 ) ;
F_8 ( V_2 -> V_2 , L_8 ,
V_55 -> V_56 ,
V_55 -> V_58 ,
V_55 -> V_60 ,
V_55 -> V_62 ,
V_55 -> V_63 ,
V_55 -> V_8 ) ;
}
static void F_20 ( struct V_52 * V_53 )
{
struct V_1 * V_2 = V_53 -> V_2 ;
T_1 V_41 ;
struct V_38 * V_38 =
F_19 ( V_53 ) ;
struct V_54 * V_55 =
(struct V_54 * ) V_38 -> V_4 ;
F_8 ( V_2 -> V_2 , L_8 ,
V_55 -> V_56 ,
V_55 -> V_58 ,
V_55 -> V_60 ,
V_55 -> V_62 ,
V_55 -> V_63 ,
V_55 -> V_8 ) ;
F_11 ( V_7 , V_55 -> V_8 ) ;
F_11 ( V_66 , V_55 -> V_65 ) ;
F_11 ( V_68 , V_55 -> V_67 ) ;
F_11 ( V_57 , V_55 -> V_56 ) ;
F_11 ( V_59 , V_55 -> V_58 ) ;
F_11 ( V_64 , V_55 -> V_63 ) ;
F_11 ( V_42 , V_55 -> V_62 ) ;
F_11 ( V_61 , V_55 -> V_60 ) ;
if ( V_55 -> V_62 & V_43 ) {
F_11 ( V_42 , F_3 ( V_42 ) |
V_43 ) ;
do {
V_41 = F_3 ( V_44 ) ;
} while ( ( V_41 & V_45 ) == 0 );
} else {
F_11 ( V_42 , F_3 ( V_42 ) &
~ V_43 ) ;
do {
V_41 = F_3 ( V_44 ) ;
} while ( V_41 & V_45 );
}
}
int F_21 ( struct V_52 * V_53 ,
struct V_69 * V_50 )
{
struct V_38 * V_38 =
F_19 ( V_53 ) ;
struct V_69 * V_70 =
V_38 -> V_46 -> V_71 ;
if ( V_38 -> type == V_72 )
V_70 = V_38 -> V_46 -> V_73 ;
if ( V_50 -> V_20 & V_74 )
return V_75 ;
if ( V_50 -> V_20 & V_76 )
return V_77 ;
if ( V_70 ) {
if ( V_50 -> V_78 > V_70 -> V_78 )
return V_79 ;
if ( V_50 -> V_80 > V_70 -> V_80 )
return V_79 ;
}
return V_81 ;
}
bool F_22 ( struct V_48 * V_49 ,
struct V_69 * V_50 ,
struct V_69 * V_82 )
{
struct V_83 * V_46 =
F_17 ( V_49 ) -> V_46 ;
struct V_1 * V_2 = V_49 -> V_2 ;
struct V_84 * V_84 =
F_23 ( V_49 -> V_85 ) ;
struct V_48 * V_86 ;
struct V_69 * V_71 = V_46 -> V_71 ;
struct V_38 * V_38 =
F_17 ( V_49 ) ;
if ( V_38 -> type == V_72 )
V_71 = V_46 -> V_73 ;
if ( ! F_24 ( V_2 ) && V_84 -> V_87 == 0 ) {
F_25 ( V_88 L_9 ) ;
return false ;
}
if ( F_24 ( V_2 ) && V_84 -> V_87 != 0 ) {
F_25 ( V_88 L_10 ) ;
return false ;
}
F_26 (tmp_encoder, &dev->mode_config.encoder_list,
head) {
if ( V_86 != V_49
&& V_86 -> V_85 == V_49 -> V_85 ) {
F_25 ( V_88 L_11
L_12 ) ;
return false ;
}
}
if ( V_71 != NULL ) {
V_82 -> V_78 = V_71 -> V_78 ;
V_82 -> V_89 = V_71 -> V_89 ;
V_82 -> V_90 = V_71 -> V_90 ;
V_82 -> V_91 = V_71 -> V_91 ;
V_82 -> V_80 = V_71 -> V_80 ;
V_82 -> V_92 = V_71 -> V_92 ;
V_82 -> V_93 = V_71 -> V_93 ;
V_82 -> V_94 = V_71 -> V_94 ;
V_82 -> clock = V_71 -> clock ;
F_27 ( V_82 ,
V_95 ) ;
}
return true ;
}
static void F_28 ( struct V_48 * V_49 )
{
struct V_1 * V_2 = V_49 -> V_2 ;
struct V_38 * V_39 = F_17 ( V_49 ) ;
struct V_83 * V_46 = V_39 -> V_46 ;
if ( ! F_2 ( V_2 , true ) )
return;
V_46 -> V_8 = F_3 ( V_7 ) ;
V_46 -> V_47 = ( V_46 -> V_8 &
V_36 ) ;
F_15 ( V_2 , V_39 , false ) ;
F_4 ( V_2 ) ;
}
static void F_29 ( struct V_48 * V_49 )
{
struct V_1 * V_2 = V_49 -> V_2 ;
struct V_38 * V_39 = F_17 ( V_49 ) ;
struct V_83 * V_46 = V_39 -> V_46 ;
if ( V_46 -> V_47 == 0 )
V_46 -> V_47 =
F_1 ( V_2 ) ;
F_15 ( V_2 , V_39 , true ) ;
}
static void F_30 ( struct V_48 * V_49 ,
struct V_69 * V_50 ,
struct V_69 * V_82 )
{
struct V_1 * V_2 = V_49 -> V_2 ;
struct V_3 * V_4 = V_2 -> V_5 ;
T_1 V_96 ;
if ( V_50 -> V_78 != V_82 -> V_78 ||
V_50 -> V_80 != V_82 -> V_80 )
V_96 = ( V_97 | V_98 |
V_99 | V_100 |
V_101 ) ;
else
V_96 = 0 ;
if ( V_4 -> V_102 )
V_96 |= V_103 ;
F_11 ( V_66 , V_96 ) ;
}
static enum V_104 F_31 ( struct V_52
* V_53 , bool V_105 )
{
return V_106 ;
}
static int F_32 ( struct V_52 * V_53 )
{
struct V_1 * V_2 = V_53 -> V_2 ;
struct V_38 * V_38 =
F_19 ( V_53 ) ;
struct V_83 * V_46 =
V_38 -> V_46 ;
int V_6 = 0 ;
if ( ! F_24 ( V_2 ) )
V_6 = F_33 ( V_38 ) ;
if ( V_6 )
return V_6 ;
V_53 -> V_107 . V_108 = 0 ;
V_53 -> V_107 . V_109 = 200 ;
V_53 -> V_107 . V_110 = 0 ;
V_53 -> V_107 . V_111 = 200 ;
if ( V_46 -> V_71 != NULL ) {
struct V_69 * V_50 =
F_34 ( V_2 , V_46 -> V_71 ) ;
F_35 ( V_53 , V_50 ) ;
return 1 ;
}
return 0 ;
}
void F_36 ( struct V_52 * V_53 )
{
struct V_38 * V_38 =
F_19 ( V_53 ) ;
if ( V_38 -> V_112 )
F_37 ( V_38 -> V_112 ) ;
F_38 ( V_53 ) ;
F_39 ( V_53 ) ;
F_40 ( V_53 ) ;
}
int F_41 ( struct V_52 * V_53 ,
struct V_113 * V_114 ,
T_3 V_115 )
{
struct V_48 * V_49 = V_53 -> V_49 ;
if ( ! V_49 )
return - 1 ;
if ( ! strcmp ( V_114 -> V_116 , L_13 ) ) {
struct V_84 * V_85 =
F_23 ( V_49 -> V_85 ) ;
T_3 V_117 ;
if ( ! V_85 )
goto V_118;
switch ( V_115 ) {
case V_119 :
break;
case V_120 :
break;
case V_121 :
break;
default:
goto V_118;
}
if ( F_42 ( V_53 ,
V_114 ,
& V_117 ) )
goto V_118;
if ( V_117 == V_115 )
goto V_122;
if ( F_43 ( V_53 ,
V_114 ,
V_115 ) )
goto V_118;
if ( V_85 -> V_123 . V_78 != 0 &&
V_85 -> V_123 . V_80 != 0 ) {
if ( ! F_44 ( V_49 -> V_85 ,
& V_85 -> V_123 ,
V_49 -> V_85 -> V_124 ,
V_49 -> V_85 -> V_125 ,
V_49 -> V_85 -> V_126 ) )
goto V_118;
}
} else if ( ! strcmp ( V_114 -> V_116 , L_14 ) ) {
if ( F_43 ( V_53 ,
V_114 ,
V_115 ) )
goto V_118;
else {
#ifdef F_45
struct V_3 * V_127 =
V_49 -> V_2 -> V_5 ;
struct V_128 * V_129 = V_127 -> V_128 ;
if ( V_129 ) {
V_129 -> V_130 . V_131 = V_115 ;
F_46 ( V_129 ) ;
}
#endif
}
} else if ( ! strcmp ( V_114 -> V_116 , L_15 ) ) {
struct V_132 * V_133
= V_49 -> V_134 ;
V_133 -> V_135 ( V_49 , V_115 ) ;
}
V_122:
return 0 ;
V_118:
return - 1 ;
}
static void F_47 ( struct V_48 * V_49 )
{
F_48 ( V_49 ) ;
}
void F_49 ( struct V_1 * V_2 ,
struct V_83 * V_46 )
{
struct V_38 * V_38 ;
struct V_54 * V_55 ;
struct V_52 * V_53 ;
struct V_48 * V_49 ;
struct V_69 * V_136 ;
struct V_137 * V_85 ;
struct V_3 * V_4 = V_2 -> V_5 ;
T_1 V_138 ;
int V_87 ;
V_38 = F_50 ( sizeof( struct V_38 ) , V_139 ) ;
if ( ! V_38 )
return;
V_55 = F_50 ( sizeof( struct V_54 ) , V_139 ) ;
if ( ! V_55 ) {
F_40 ( V_38 ) ;
F_5 ( V_2 -> V_2 , L_16 ) ;
return;
}
V_38 -> V_4 = V_55 ;
V_38 -> V_46 = V_46 ;
V_53 = & V_38 -> V_140 ;
V_49 = & V_38 -> V_141 ;
F_51 ( V_2 , & V_38 -> V_140 ,
& V_142 ,
V_143 ) ;
F_52 ( V_2 , & V_38 -> V_141 ,
& V_144 ,
V_145 ) ;
F_53 ( & V_38 -> V_140 ,
& V_38 -> V_141 ) ;
V_38 -> type = V_146 ;
F_54 ( V_49 , & V_147 ) ;
F_55 ( V_53 ,
& V_148 ) ;
V_53 -> V_107 . V_149 = V_150 ;
V_53 -> V_151 = false ;
V_53 -> V_152 = false ;
F_56 ( V_53 ,
V_2 -> V_153 . V_154 ,
V_119 ) ;
F_56 ( V_53 ,
V_4 -> V_155 ,
V_24 ) ;
V_38 -> V_156 = F_57 ( V_2 ,
V_157 ,
L_17 ) ;
if ( ! V_38 -> V_156 ) {
F_58 ( V_88 ,
& V_2 -> V_158 -> V_2 , L_18 ) ;
goto V_159;
}
V_38 -> V_156 -> V_19 = 0x2C ;
V_4 -> V_13 = V_38 -> V_156 ;
V_38 -> V_112 = F_57 ( V_2 ,
V_160 ,
L_19 ) ;
if ( ! V_38 -> V_112 ) {
F_58 ( V_88 , & V_2 -> V_158 -> V_2 ,
L_20 L_21 ) ;
goto V_161;
}
F_33 ( V_38 ) ;
F_26 (scan, &connector->probed_modes, head) {
if ( V_136 -> type & V_162 ) {
V_46 -> V_71 =
F_34 ( V_2 , V_136 ) ;
goto V_163;
}
}
if ( V_46 -> V_164 )
V_46 -> V_71 =
F_34 ( V_2 , V_46 -> V_164 ) ;
if ( ! V_46 -> V_71 )
if ( V_4 -> V_165 )
V_46 -> V_71 =
F_34 ( V_2 ,
V_4 -> V_165 ) ;
V_138 = F_3 ( V_61 ) ;
V_87 = ( V_138 & V_166 ) ? 1 : 0 ;
V_85 = F_59 ( V_2 , V_87 ) ;
if ( V_85 && ( V_138 & V_167 ) ) {
V_46 -> V_71 =
F_60 ( V_2 , V_85 ) ;
if ( V_46 -> V_71 ) {
V_46 -> V_71 -> type |=
V_162 ;
goto V_163;
}
}
if ( ! V_46 -> V_71 ) {
F_5 ( V_2 -> V_2 , L_22 ) ;
goto V_168;
}
V_163:
F_61 ( V_53 ) ;
return;
V_168:
if ( V_38 -> V_112 )
F_37 ( V_38 -> V_112 ) ;
V_161:
if ( V_38 -> V_156 )
F_37 ( V_38 -> V_156 ) ;
V_159:
F_48 ( V_49 ) ;
F_39 ( V_53 ) ;
F_40 ( V_53 ) ;
}
