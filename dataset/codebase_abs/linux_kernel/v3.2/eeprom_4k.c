static int F_1 ( struct V_1 * V_2 )
{
return ( ( V_2 -> V_3 . V_4 . V_5 . V_6 >> 12 ) & 0xF ) ;
}
static int F_2 ( struct V_1 * V_2 )
{
return ( ( V_2 -> V_3 . V_4 . V_5 . V_6 ) & 0xFFF ) ;
}
static bool F_3 ( struct V_1 * V_2 )
{
struct V_7 * V_8 = F_4 ( V_2 ) ;
T_1 * V_9 = ( T_1 * ) & V_2 -> V_3 . V_4 ;
int V_10 , V_11 = 64 ;
for ( V_10 = 0 ; V_10 < V_12 ; V_10 ++ ) {
if ( ! F_5 ( V_8 , V_10 + V_11 , V_9 ) ) {
F_6 ( V_8 , V_13 ,
L_1 ) ;
return false ;
}
V_9 ++ ;
}
return true ;
}
static bool F_7 ( struct V_1 * V_2 )
{
T_1 * V_9 = ( T_1 * ) & V_2 -> V_3 . V_4 ;
F_8 ( V_2 , V_9 , 64 , V_12 ) ;
return true ;
}
static bool F_9 ( struct V_1 * V_2 )
{
struct V_7 * V_8 = F_4 ( V_2 ) ;
if ( ! F_10 ( V_2 ) ) {
F_6 ( V_8 , V_13 ,
L_2 ) ;
}
if ( V_8 -> V_14 -> V_15 == V_16 )
return F_7 ( V_2 ) ;
else
return F_3 ( V_2 ) ;
}
static T_2 F_11 ( char * V_17 , T_2 V_18 , T_2 V_19 ,
struct V_20 * V_21 )
{
F_12 ( L_3 , V_21 -> V_22 [ 0 ] ) ;
F_12 ( L_4 , V_21 -> V_23 ) ;
F_12 ( L_5 , V_21 -> V_24 [ 0 ] ) ;
F_12 ( L_6 , V_21 -> V_25 ) ;
F_12 ( L_7 , V_21 -> V_26 [ 0 ] ) ;
F_12 ( L_8 , V_21 -> V_27 [ 0 ] ) ;
F_12 ( L_9 , V_21 -> V_28 ) ;
F_12 ( L_10 , V_21 -> V_29 ) ;
F_12 ( L_11 , V_21 -> V_30 [ 0 ] ) ;
F_12 ( L_12 , V_21 -> V_31 ) ;
F_12 ( L_13 , V_21 -> V_32 ) ;
F_12 ( L_14 , V_21 -> V_33 ) ;
F_12 ( L_15 , V_21 -> V_34 ) ;
F_12 ( L_16 , V_21 -> V_35 [ 0 ] ) ;
F_12 ( L_17 , V_21 -> V_36 ) ;
F_12 ( L_18 , V_21 -> V_37 ) ;
F_12 ( L_19 , V_21 -> V_38 [ 0 ] ) ;
F_12 ( L_20 , V_21 -> V_39 [ 0 ] ) ;
F_12 ( L_21 , V_21 -> V_40 ) ;
F_12 ( L_22 , V_21 -> V_6 ) ;
F_12 ( L_23 , V_21 -> V_41 ) ;
F_12 ( L_24 , V_21 -> V_42 ) ;
F_12 ( L_25 , V_21 -> V_43 ) ;
F_12 ( L_26 , V_21 -> V_44 ) ;
F_12 ( L_27 , V_21 -> V_45 ) ;
F_12 ( L_28 , V_21 -> V_46 ) ;
F_12 ( L_29 , V_21 -> V_47 ) ;
F_12 ( L_30 , V_21 -> V_48 ) ;
F_12 ( L_31 , V_21 -> V_49 ) ;
F_12 ( L_32 , V_21 -> V_50 ) ;
F_12 ( L_33 , V_21 -> V_51 ) ;
F_12 ( L_34 , V_21 -> V_52 ) ;
F_12 ( L_35 , V_21 -> V_53 ) ;
F_12 ( L_36 , V_21 -> V_54 ) ;
F_12 ( L_37 , V_21 -> V_55 ) ;
F_12 ( L_38 , V_21 -> V_56 ) ;
F_12 ( L_39 , V_21 -> V_57 ) ;
F_12 ( L_40 , V_21 -> V_58 ) ;
F_12 ( L_41 , V_21 -> V_59 ) ;
F_12 ( L_42 , V_21 -> V_60 [ 0 ] ) ;
F_12 ( L_43 , V_21 -> V_61 [ 0 ] ) ;
F_12 ( L_44 , V_21 -> V_62 ) ;
F_12 ( L_45 , V_21 -> V_63 [ 0 ] ) ;
F_12 ( L_46 , V_21 -> V_64 [ 0 ] ) ;
F_12 ( L_47 , V_21 -> V_65 ) ;
F_12 ( L_48 , V_21 -> V_66 ) ;
F_12 ( L_49 , V_21 -> V_67 ) ;
return V_18 ;
}
static T_2 F_13 ( struct V_1 * V_2 , bool V_68 ,
T_3 * V_17 , T_2 V_18 , T_2 V_19 )
{
struct V_69 * V_70 = & V_2 -> V_3 . V_4 ;
struct V_71 * V_72 = & V_70 -> V_5 ;
if ( ! V_68 ) {
V_18 += snprintf ( V_17 + V_18 , V_19 - V_18 ,
L_50 , L_51 ) ;
V_18 += F_11 ( V_17 , V_18 , V_19 ,
& V_70 -> V_73 ) ;
goto V_74;
}
F_12 ( L_52 , V_72 -> V_6 >> 12 ) ;
F_12 ( L_53 , V_72 -> V_6 & 0xFFF ) ;
F_12 ( L_54 , V_72 -> V_75 ) ;
F_12 ( L_55 , V_72 -> V_76 ) ;
F_12 ( L_56 , V_72 -> V_77 [ 0 ] ) ;
F_12 ( L_57 , V_72 -> V_77 [ 1 ] ) ;
F_12 ( L_58 , V_72 -> V_78 ) ;
F_12 ( L_59 , V_72 -> V_79 ) ;
F_12 ( L_60 , ! ! ( V_72 -> V_80 & V_81 ) ) ;
F_12 ( L_61 , ! ! ( V_72 -> V_80 & V_82 ) ) ;
F_12 ( L_62 , ! ! ( V_72 -> V_80 &
V_83 ) ) ;
F_12 ( L_63 , ! ! ( V_72 -> V_80 &
V_84 ) ) ;
F_12 ( L_64 , ! ! ( V_72 -> V_80 &
V_85 ) ) ;
F_12 ( L_65 , ! ! ( V_72 -> V_80 &
V_86 ) ) ;
F_12 ( L_66 , ! ! ( V_72 -> V_87 & 0x01 ) ) ;
F_12 ( L_67 , ( V_72 -> V_88 >> 24 ) & 0xFF ) ;
F_12 ( L_68 , ( V_72 -> V_88 >> 16 ) & 0xFF ) ;
F_12 ( L_69 , ( V_72 -> V_88 >> 8 ) & 0xFF ) ;
F_12 ( L_70 , V_72 -> V_89 ) ;
V_18 += snprintf ( V_17 + V_18 , V_19 - V_18 , L_71 , L_72 ,
V_72 -> V_90 ) ;
V_74:
if ( V_18 > V_19 )
V_18 = V_19 ;
return V_18 ;
}
static T_2 F_13 ( struct V_1 * V_2 , bool V_68 ,
T_3 * V_17 , T_2 V_18 , T_2 V_19 )
{
return 0 ;
}
static int F_14 ( struct V_1 * V_2 )
{
#define F_15 (sizeof(struct ar5416_eeprom_4k) / sizeof(u16))
struct V_7 * V_8 = F_4 ( V_2 ) ;
struct V_69 * V_70 =
(struct V_69 * ) & V_2 -> V_3 . V_4 ;
T_1 * V_91 , V_92 , V_93 , V_94 ;
T_2 V_95 = 0 , V_96 ;
bool V_97 = false ;
int V_98 , V_10 ;
if ( ! F_10 ( V_2 ) ) {
if ( ! F_5 ( V_8 , V_99 ,
& V_93 ) ) {
F_16 ( V_8 , L_73 ) ;
return false ;
}
F_6 ( V_8 , V_13 ,
L_74 , V_93 ) ;
if ( V_93 != V_100 ) {
V_94 = F_17 ( V_93 ) ;
if ( V_94 == V_100 ) {
V_97 = true ;
V_91 = ( T_1 * ) ( & V_2 -> V_3 ) ;
for ( V_10 = 0 ; V_10 < F_15 ; V_10 ++ ) {
V_92 = F_17 ( * V_91 ) ;
* V_91 = V_92 ;
V_91 ++ ;
}
} else {
F_16 ( V_8 ,
L_75 ) ;
return - V_101 ;
}
}
}
F_6 ( V_8 , V_13 , L_76 ,
V_97 ? L_77 : L_78 ) ;
if ( V_97 )
V_96 = F_17 ( V_2 -> V_3 . V_4 . V_5 . V_76 ) ;
else
V_96 = V_2 -> V_3 . V_4 . V_5 . V_76 ;
if ( V_96 > sizeof( struct V_69 ) )
V_96 = sizeof( struct V_69 ) / sizeof( T_1 ) ;
else
V_96 = V_96 / sizeof( T_1 ) ;
V_91 = ( T_1 * ) ( & V_2 -> V_3 ) ;
for ( V_98 = 0 ; V_98 < V_96 ; V_98 ++ )
V_95 ^= * V_91 ++ ;
if ( V_97 ) {
T_2 integer ;
T_1 V_102 ;
F_6 ( V_8 , V_13 ,
L_79 ) ;
V_102 = F_17 ( V_70 -> V_5 . V_76 ) ;
V_70 -> V_5 . V_76 = V_102 ;
V_102 = F_17 ( V_70 -> V_5 . V_75 ) ;
V_70 -> V_5 . V_75 = V_102 ;
V_102 = F_17 ( V_70 -> V_5 . V_6 ) ;
V_70 -> V_5 . V_6 = V_102 ;
V_102 = F_17 ( V_70 -> V_5 . V_77 [ 0 ] ) ;
V_70 -> V_5 . V_77 [ 0 ] = V_102 ;
V_102 = F_17 ( V_70 -> V_5 . V_77 [ 1 ] ) ;
V_70 -> V_5 . V_77 [ 1 ] = V_102 ;
V_102 = F_17 ( V_70 -> V_5 . V_103 ) ;
V_70 -> V_5 . V_103 = V_102 ;
V_102 = F_17 ( V_70 -> V_5 . V_104 ) ;
V_70 -> V_5 . V_104 = V_102 ;
V_102 = F_17 ( V_70 -> V_5 . V_105 ) ;
V_70 -> V_5 . V_105 = V_102 ;
integer = F_18 ( V_70 -> V_73 . V_23 ) ;
V_70 -> V_73 . V_23 = integer ;
for ( V_98 = 0 ; V_98 < V_106 ; V_98 ++ ) {
integer = F_18 ( V_70 -> V_73 . V_22 [ V_98 ] ) ;
V_70 -> V_73 . V_22 [ V_98 ] = integer ;
}
for ( V_98 = 0 ; V_98 < V_107 ; V_98 ++ ) {
V_102 = F_17 ( V_70 -> V_73 . V_108 [ V_98 ] . V_109 ) ;
V_70 -> V_73 . V_108 [ V_98 ] . V_109 = V_102 ;
}
}
if ( V_95 != 0xffff || V_2 -> V_110 -> V_111 ( V_2 ) != V_112 ||
V_2 -> V_110 -> V_113 ( V_2 ) < V_114 ) {
F_16 ( V_8 , L_80 ,
V_95 , V_2 -> V_110 -> V_111 ( V_2 ) ) ;
return - V_101 ;
}
return 0 ;
#undef F_15
}
static T_2 F_19 ( struct V_1 * V_2 ,
enum V_115 V_116 )
{
struct V_69 * V_70 = & V_2 -> V_3 . V_4 ;
struct V_20 * V_117 = & V_70 -> V_73 ;
struct V_71 * V_72 = & V_70 -> V_5 ;
T_1 V_118 ;
V_118 = V_72 -> V_6 & V_119 ;
switch ( V_116 ) {
case V_120 :
return V_117 -> V_35 [ 0 ] ;
case V_121 :
return F_20 ( V_72 -> V_90 ) ;
case V_122 :
return F_20 ( V_72 -> V_90 + 2 ) ;
case V_123 :
return F_20 ( V_72 -> V_90 + 4 ) ;
case V_124 :
return V_72 -> V_77 [ 0 ] ;
case V_125 :
return V_72 -> V_105 ;
case V_126 :
return V_72 -> V_80 ;
case V_127 :
return V_72 -> V_103 ;
case V_128 :
return V_117 -> V_41 ;
case V_129 :
return V_117 -> V_47 ;
case V_130 :
return V_118 ;
case V_131 :
return V_72 -> V_78 ;
case V_132 :
return V_72 -> V_79 ;
case V_133 :
return 0 ;
case V_134 :
return V_135 ;
case V_136 :
return V_117 -> V_6 ;
case V_137 :
return V_117 -> V_65 ;
case V_138 :
return V_72 -> V_89 ;
case V_139 :
return V_117 -> V_24 [ 0 ] ;
default:
return 0 ;
}
}
static void F_21 ( struct V_1 * V_2 ,
struct V_140 * V_141 )
{
struct V_7 * V_8 = F_4 ( V_2 ) ;
struct V_69 * V_142 = & V_2 -> V_3 . V_4 ;
struct V_143 * V_144 ;
T_3 * V_145 = NULL ;
T_1 V_146 ;
static T_3 V_147 [ V_148 ] ;
T_1 V_149 [ V_150 ] ;
T_1 V_151 , V_98 , V_152 ;
T_1 V_153 , V_154 ;
T_1 V_155 [ V_156 ] = { 0 , 0 } ;
T_2 V_157 , V_158 , V_159 ;
V_154 = V_142 -> V_73 . V_36 ;
if ( ( V_142 -> V_5 . V_6 & V_119 ) >=
V_160 ) {
V_146 =
V_142 -> V_73 . V_40 ;
} else {
V_146 = ( T_1 ) ( F_22 ( F_23 ( V_2 , V_161 ) ,
V_162 ) ) ;
}
V_145 = V_142 -> V_163 ;
V_151 = V_164 ;
V_153 = 0 ;
for ( V_98 = 1 ; V_98 <= V_150 ; V_98 ++ ) {
if ( ( V_154 >> ( V_150 - V_98 ) ) & 1 ) {
if ( V_153 >= V_156 )
break;
V_155 [ V_153 ] =
( T_1 ) ( V_150 - V_98 ) ;
V_153 ++ ;
}
}
F_24 ( V_2 , V_165 , V_166 ,
( V_153 - 1 ) & 0x3 ) ;
F_24 ( V_2 , V_165 , V_167 ,
V_155 [ 0 ] ) ;
F_24 ( V_2 , V_165 , V_168 ,
V_155 [ 1 ] ) ;
F_24 ( V_2 , V_165 , V_169 , 0 ) ;
for ( V_98 = 0 ; V_98 < V_106 ; V_98 ++ ) {
V_159 = V_98 * 0x1000 ;
if ( V_142 -> V_5 . V_78 & ( 1 << V_98 ) ) {
V_144 = V_142 -> V_170 [ V_98 ] ;
F_25 ( V_2 , V_141 ,
V_144 , V_145 ,
V_151 , V_146 ,
V_149 ,
V_147 , V_153 ) ;
F_26 ( V_2 ) ;
F_27 ( V_2 , V_161 + V_159 ,
F_28 ( V_146 ,
V_162 )
| F_28 ( V_149 [ 0 ] ,
V_171 )
| F_28 ( V_149 [ 1 ] ,
V_172 )
| F_28 ( V_149 [ 2 ] ,
V_173 )
| F_28 ( V_149 [ 3 ] ,
V_174 ) ) ;
V_158 = V_175 + ( 672 << 2 ) + V_159 ;
for ( V_152 = 0 ; V_152 < 32 ; V_152 ++ ) {
V_157 = F_29 ( & V_147 [ 4 * V_152 ] ) ;
F_27 ( V_2 , V_158 , V_157 ) ;
F_6 ( V_8 , V_13 ,
L_81 ,
V_98 , V_159 , V_158 ,
V_157 ) ;
F_6 ( V_8 , V_13 ,
L_82
L_83
L_83
L_83
L_84 ,
V_98 , 4 * V_152 , V_147 [ 4 * V_152 ] ,
4 * V_152 + 1 , V_147 [ 4 * V_152 + 1 ] ,
4 * V_152 + 2 , V_147 [ 4 * V_152 + 2 ] ,
4 * V_152 + 3 , V_147 [ 4 * V_152 + 3 ] ) ;
V_158 += 4 ;
}
F_30 ( V_2 ) ;
}
}
}
static void F_31 ( struct V_1 * V_2 ,
struct V_140 * V_141 ,
T_4 * V_176 ,
T_1 V_177 ,
T_1 V_178 ,
T_1 V_179 )
{
#define F_32 \
(((cfgCtl & ~CTL_MODE_M)| (pCtlMode[ctlMode] & CTL_MODE_M)) == \
pEepData->ctlIndex[i]) \
|| (((cfgCtl & ~CTL_MODE_M) | (pCtlMode[ctlMode] & CTL_MODE_M)) == \
((pEepData->ctlIndex[i] & CTL_MODE_M) | SD_NO_CTL))
int V_98 ;
T_1 V_180 ;
T_1 V_181 = V_182 ;
T_1 V_183 = 0 , V_184 ;
T_1 V_185 ;
const T_1 * V_186 ;
T_1 V_187 , V_188 ;
struct V_189 V_190 ;
struct V_191 * V_192 ;
struct V_69 * V_142 = & V_2 -> V_3 . V_4 ;
struct V_193 V_194 , V_195 = {
0 , { 0 , 0 , 0 , 0 }
} ;
struct V_193 V_196 = {
0 , { 0 , 0 , 0 , 0 } } , V_197 = {
0 , { 0 , 0 , 0 , 0 }
} ;
struct V_198 V_199 , V_200 = {
0 , { 0 , 0 , 0 , 0 }
} ;
static const T_1 V_201 [] = {
V_202 , V_203 , V_204 ,
V_205 , V_206 , V_207
} ;
F_33 ( V_2 , V_141 , & V_190 ) ;
V_183 = V_179 - V_178 ;
V_185 = F_34 ( V_201 ) - V_208 ;
V_186 = V_201 ;
F_35 ( V_2 , V_141 ,
V_142 -> V_209 ,
V_210 ,
& V_195 , 4 , false ) ;
F_35 ( V_2 , V_141 ,
V_142 -> V_211 ,
V_212 ,
& V_194 , 4 , false ) ;
F_36 ( V_2 , V_141 ,
V_142 -> V_213 ,
V_212 ,
& V_199 , 8 , false ) ;
if ( F_37 ( V_141 ) ) {
V_185 = F_34 ( V_201 ) ;
F_36 ( V_2 , V_141 ,
V_142 -> V_214 ,
V_215 ,
& V_200 , 8 , true ) ;
F_35 ( V_2 , V_141 ,
V_142 -> V_209 ,
V_210 ,
& V_197 , 4 , true ) ;
F_35 ( V_2 , V_141 ,
V_142 -> V_211 ,
V_212 ,
& V_196 , 4 , true ) ;
}
for ( V_187 = 0 ; V_187 < V_185 ; V_187 ++ ) {
bool V_216 = ( V_186 [ V_187 ] == V_217 ) ||
( V_186 [ V_187 ] == V_207 ) ;
if ( V_216 )
V_188 = V_190 . V_218 ;
else if ( V_186 [ V_187 ] & V_219 )
V_188 = V_190 . V_220 ;
else
V_188 = V_190 . V_221 ;
if ( V_2 -> V_110 -> V_111 ( V_2 ) == 14 &&
V_2 -> V_110 -> V_113 ( V_2 ) <= 2 )
V_181 = V_182 ;
for ( V_98 = 0 ; ( V_98 < V_222 ) &&
V_142 -> V_223 [ V_98 ] ; V_98 ++ ) {
if ( F_32 ) {
V_192 = & ( V_142 -> V_224 [ V_98 ] ) ;
V_180 = F_38 (
V_188 ,
V_192 -> V_225 [
F_39 ( V_2 -> V_226 ) - 1 ] ,
F_40 ( V_141 ) ,
V_227 ) ;
if ( ( V_177 & ~ V_228 ) == V_229 ) {
V_181 =
F_41 ( V_181 ,
V_180 ) ;
} else {
V_181 = V_180 ;
break;
}
}
}
V_184 = ( T_3 ) F_41 ( V_181 , V_183 ) ;
switch ( V_186 [ V_187 ] ) {
case V_202 :
for ( V_98 = 0 ; V_98 < F_34 ( V_195 . V_230 ) ; V_98 ++ ) {
V_195 . V_230 [ V_98 ] =
F_41 ( ( T_1 ) V_195 . V_230 [ V_98 ] ,
V_184 ) ;
}
break;
case V_203 :
for ( V_98 = 0 ; V_98 < F_34 ( V_194 . V_230 ) ; V_98 ++ ) {
V_194 . V_230 [ V_98 ] =
F_41 ( ( T_1 ) V_194 . V_230 [ V_98 ] ,
V_184 ) ;
}
break;
case V_204 :
for ( V_98 = 0 ; V_98 < F_34 ( V_199 . V_230 ) ; V_98 ++ ) {
V_199 . V_230 [ V_98 ] =
F_41 ( ( T_1 ) V_199 . V_230 [ V_98 ] ,
V_184 ) ;
}
break;
case V_205 :
V_197 . V_230 [ 0 ] =
F_41 ( ( T_1 ) V_197 . V_230 [ 0 ] ,
V_184 ) ;
break;
case V_206 :
V_196 . V_230 [ 0 ] =
F_41 ( ( T_1 ) V_196 . V_230 [ 0 ] ,
V_184 ) ;
break;
case V_207 :
for ( V_98 = 0 ; V_98 < F_34 ( V_200 . V_230 ) ; V_98 ++ ) {
V_200 . V_230 [ V_98 ] =
F_41 ( ( T_1 ) V_200 . V_230 [ V_98 ] ,
V_184 ) ;
}
break;
default:
break;
}
}
V_176 [ V_231 ] =
V_176 [ V_232 ] =
V_176 [ V_233 ] =
V_176 [ V_234 ] =
V_176 [ V_235 ] =
V_194 . V_230 [ 0 ] ;
V_176 [ V_236 ] = V_194 . V_230 [ 1 ] ;
V_176 [ V_237 ] = V_194 . V_230 [ 2 ] ;
V_176 [ V_238 ] = V_194 . V_230 [ 3 ] ;
V_176 [ V_239 ] = V_194 . V_230 [ 0 ] ;
for ( V_98 = 0 ; V_98 < F_34 ( V_199 . V_230 ) ; V_98 ++ )
V_176 [ V_240 + V_98 ] = V_199 . V_230 [ V_98 ] ;
V_176 [ V_241 ] = V_195 . V_230 [ 0 ] ;
V_176 [ V_242 ] = V_176 [ V_243 ] = V_195 . V_230 [ 1 ] ;
V_176 [ V_244 ] = V_176 [ V_245 ] = V_195 . V_230 [ 2 ] ;
V_176 [ V_246 ] = V_176 [ V_247 ] = V_195 . V_230 [ 3 ] ;
if ( F_37 ( V_141 ) ) {
for ( V_98 = 0 ; V_98 < F_34 ( V_200 . V_230 ) ; V_98 ++ ) {
V_176 [ V_248 + V_98 ] =
V_200 . V_230 [ V_98 ] ;
}
V_176 [ V_249 ] = V_200 . V_230 [ 0 ] ;
V_176 [ V_250 ] = V_200 . V_230 [ 0 ] ;
V_176 [ V_251 ] = V_196 . V_230 [ 0 ] ;
V_176 [ V_252 ] = V_197 . V_230 [ 0 ] ;
}
#undef F_32
}
static void F_42 ( struct V_1 * V_2 ,
struct V_140 * V_141 ,
T_1 V_177 ,
T_3 V_253 ,
T_3 V_179 , bool V_254 )
{
struct V_255 * V_256 = F_43 ( V_2 ) ;
struct V_69 * V_142 = & V_2 -> V_3 . V_4 ;
struct V_20 * V_117 = & V_142 -> V_73 ;
T_4 V_176 [ V_257 ] ;
T_3 V_59 = 2 ;
int V_98 ;
memset ( V_176 , 0 , sizeof( V_176 ) ) ;
if ( ( V_142 -> V_5 . V_6 & V_119 ) >=
V_160 ) {
V_59 = V_117 -> V_59 ;
}
F_31 ( V_2 , V_141 ,
& V_176 [ 0 ] , V_177 ,
V_253 ,
V_179 ) ;
F_21 ( V_2 , V_141 ) ;
V_256 -> V_258 = 0 ;
for ( V_98 = 0 ; V_98 < F_34 ( V_176 ) ; V_98 ++ ) {
if ( V_176 [ V_98 ] > V_182 )
V_176 [ V_98 ] = V_182 ;
if ( V_176 [ V_98 ] > V_256 -> V_258 )
V_256 -> V_258 = V_176 [ V_98 ] ;
}
if ( V_254 )
return;
for ( V_98 = 0 ; V_98 < V_257 ; V_98 ++ )
V_176 [ V_98 ] -= V_135 * 2 ;
F_26 ( V_2 ) ;
F_27 ( V_2 , V_259 ,
F_44 ( V_176 [ V_234 ] , 24 )
| F_44 ( V_176 [ V_233 ] , 16 )
| F_44 ( V_176 [ V_232 ] , 8 )
| F_44 ( V_176 [ V_231 ] , 0 ) ) ;
F_27 ( V_2 , V_260 ,
F_44 ( V_176 [ V_238 ] , 24 )
| F_44 ( V_176 [ V_237 ] , 16 )
| F_44 ( V_176 [ V_236 ] , 8 )
| F_44 ( V_176 [ V_235 ] , 0 ) ) ;
F_27 ( V_2 , V_261 ,
F_44 ( V_176 [ V_242 ] , 24 )
| F_44 ( V_176 [ V_243 ] , 16 )
| F_44 ( V_176 [ V_239 ] , 8 )
| F_44 ( V_176 [ V_241 ] , 0 ) ) ;
F_27 ( V_2 , V_262 ,
F_44 ( V_176 [ V_246 ] , 24 )
| F_44 ( V_176 [ V_247 ] , 16 )
| F_44 ( V_176 [ V_244 ] , 8 )
| F_44 ( V_176 [ V_245 ] , 0 ) ) ;
F_27 ( V_2 , V_263 ,
F_44 ( V_176 [ V_264 ] , 24 )
| F_44 ( V_176 [ V_265 ] , 16 )
| F_44 ( V_176 [ V_266 ] , 8 )
| F_44 ( V_176 [ V_240 ] , 0 ) ) ;
F_27 ( V_2 , V_267 ,
F_44 ( V_176 [ V_268 ] , 24 )
| F_44 ( V_176 [ V_269 ] , 16 )
| F_44 ( V_176 [ V_270 ] , 8 )
| F_44 ( V_176 [ V_271 ] , 0 ) ) ;
if ( F_37 ( V_141 ) ) {
F_27 ( V_2 , V_272 ,
F_44 ( V_176 [ V_273 ] +
V_59 , 24 )
| F_44 ( V_176 [ V_274 ] +
V_59 , 16 )
| F_44 ( V_176 [ V_275 ] +
V_59 , 8 )
| F_44 ( V_176 [ V_248 ] +
V_59 , 0 ) ) ;
F_27 ( V_2 , V_276 ,
F_44 ( V_176 [ V_277 ] +
V_59 , 24 )
| F_44 ( V_176 [ V_278 ] +
V_59 , 16 )
| F_44 ( V_176 [ V_279 ] +
V_59 , 8 )
| F_44 ( V_176 [ V_280 ] +
V_59 , 0 ) ) ;
F_27 ( V_2 , V_281 ,
F_44 ( V_176 [ V_251 ] , 24 )
| F_44 ( V_176 [ V_252 ] , 16 )
| F_44 ( V_176 [ V_249 ] , 8 )
| F_44 ( V_176 [ V_250 ] , 0 ) ) ;
}
F_30 ( V_2 ) ;
}
static void F_45 ( struct V_1 * V_2 ,
struct V_20 * V_117 ,
struct V_69 * V_70 ,
T_3 V_282 )
{
F_27 ( V_2 , V_283 ,
V_117 -> V_22 [ 0 ] ) ;
F_27 ( V_2 , F_46 ( 0 ) ,
( F_23 ( V_2 , F_46 ( 0 ) ) &
~ ( V_284 |
V_285 ) ) |
F_28 ( V_117 -> V_38 [ 0 ] , V_285 ) |
F_28 ( V_117 -> V_39 [ 0 ] , V_284 ) ) ;
if ( ( V_70 -> V_5 . V_6 & V_119 ) >=
V_286 ) {
V_282 = V_117 -> V_26 [ 0 ] ;
F_24 ( V_2 , V_287 ,
V_288 , V_117 -> V_61 [ 0 ] ) ;
F_24 ( V_2 , V_287 ,
V_289 , V_117 -> V_60 [ 0 ] ) ;
F_24 ( V_2 , V_287 ,
V_290 ,
V_117 -> V_64 [ 0 ] ) ;
F_24 ( V_2 , V_287 ,
V_291 , V_117 -> V_63 [ 0 ] ) ;
F_24 ( V_2 , V_287 + 0x1000 ,
V_288 ,
V_117 -> V_61 [ 0 ] ) ;
F_24 ( V_2 , V_287 + 0x1000 ,
V_289 , V_117 -> V_60 [ 0 ] ) ;
F_24 ( V_2 , V_287 + 0x1000 ,
V_290 ,
V_117 -> V_64 [ 0 ] ) ;
F_24 ( V_2 , V_287 + 0x1000 ,
V_291 ,
V_117 -> V_63 [ 0 ] ) ;
}
F_24 ( V_2 , V_292 ,
V_293 , V_282 ) ;
F_24 ( V_2 , V_292 ,
V_294 , V_117 -> V_27 [ 0 ] ) ;
F_24 ( V_2 , V_292 + 0x1000 ,
V_293 , V_282 ) ;
F_24 ( V_2 , V_292 + 0x1000 ,
V_294 , V_117 -> V_27 [ 0 ] ) ;
}
static void F_47 ( struct V_1 * V_2 ,
struct V_140 * V_141 )
{
struct V_20 * V_117 ;
struct V_69 * V_70 = & V_2 -> V_3 . V_4 ;
struct V_71 * V_72 = & V_70 -> V_5 ;
T_3 V_282 ;
T_3 V_295 [ 5 ] , V_296 [ 5 ] , V_297 [ 5 ] ;
T_3 V_298 , V_299 ;
T_3 V_300 ;
T_2 V_301 ;
V_117 = & V_70 -> V_73 ;
V_282 = 23 ;
F_27 ( V_2 , V_302 , V_117 -> V_23 ) ;
F_45 ( V_2 , V_117 , V_70 , V_282 ) ;
if ( V_117 -> V_6 >= 3 ) {
V_298 = V_117 -> V_65 ;
V_299 = V_117 -> V_66 ;
V_301 = F_23 ( V_2 , V_303 ) ;
V_301 &= ( ~ ( V_304 ) ) ;
V_301 |= F_28 ( V_298 ,
V_305 ) ;
V_301 |= F_28 ( V_299 ,
V_306 ) ;
V_301 |= F_28 ( ( V_299 >> 2 ) ,
V_307 ) ;
V_301 |= F_28 ( ( V_298 >> 1 ) ,
V_308 ) ;
V_301 |= F_28 ( ( V_298 >> 2 ) ,
V_309 ) ;
F_27 ( V_2 , V_303 , V_301 ) ;
V_301 = F_23 ( V_2 , V_303 ) ;
V_301 = F_23 ( V_2 , V_310 ) ;
V_301 &= ( ~ V_311 ) ;
V_301 |= F_28 ( ( V_298 >> 3 ) ,
V_311 ) ;
F_27 ( V_2 , V_310 , V_301 ) ;
V_301 = F_23 ( V_2 , V_310 ) ;
}
if ( V_117 -> V_6 >= 2 ) {
V_295 [ 0 ] = V_117 -> V_41 ;
V_295 [ 1 ] = V_117 -> V_42 ;
V_295 [ 2 ] = V_117 -> V_43 ;
V_295 [ 3 ] = V_117 -> V_44 ;
V_295 [ 4 ] = V_117 -> V_45 ;
V_296 [ 0 ] = V_117 -> V_46 ;
V_296 [ 1 ] = V_117 -> V_47 ;
V_296 [ 2 ] = V_117 -> V_48 ;
V_296 [ 3 ] = V_117 -> V_49 ;
V_296 [ 4 ] = V_117 -> V_50 ;
V_297 [ 0 ] = V_117 -> V_51 ;
V_297 [ 1 ] = V_117 -> V_52 ;
V_297 [ 2 ] = V_117 -> V_53 ;
V_297 [ 3 ] = V_117 -> V_54 ;
V_297 [ 4 ] = V_117 -> V_55 ;
} else if ( V_117 -> V_6 == 1 ) {
V_295 [ 0 ] = V_117 -> V_41 ;
V_295 [ 1 ] = V_295 [ 2 ] = V_295 [ 3 ] = V_295 [ 4 ] = V_117 -> V_42 ;
V_296 [ 0 ] = V_117 -> V_46 ;
V_296 [ 1 ] = V_296 [ 2 ] = V_296 [ 3 ] = V_296 [ 4 ] = V_117 -> V_47 ;
V_297 [ 0 ] = V_117 -> V_51 ;
V_297 [ 1 ] = V_297 [ 2 ] = V_297 [ 3 ] = V_297 [ 4 ] = V_117 -> V_52 ;
} else {
int V_98 ;
for ( V_98 = 0 ; V_98 < 5 ; V_98 ++ ) {
V_295 [ V_98 ] = V_117 -> V_41 ;
V_296 [ V_98 ] = V_117 -> V_46 ;
V_297 [ V_98 ] = V_117 -> V_46 ;
}
}
if ( F_48 ( V_2 ) ) {
F_49 ( V_2 ,
V_312 ,
V_313 ,
V_314 ,
V_295 [ 0 ] ) ;
F_49 ( V_2 ,
V_312 ,
V_315 ,
V_316 ,
V_295 [ 1 ] ) ;
F_49 ( V_2 ,
V_312 ,
V_317 ,
V_318 ,
V_295 [ 2 ] ) ;
F_49 ( V_2 ,
V_312 ,
V_319 ,
V_320 ,
V_296 [ 0 ] ) ;
F_49 ( V_2 ,
V_321 ,
V_322 ,
V_323 ,
V_297 [ 0 ] ) ;
} else {
F_49 ( V_2 ,
V_312 ,
V_324 ,
V_325 ,
V_295 [ 0 ] ) ;
F_49 ( V_2 ,
V_312 ,
V_326 ,
V_327 ,
V_295 [ 1 ] ) ;
F_49 ( V_2 ,
V_312 ,
V_328 ,
V_329 ,
V_295 [ 2 ] ) ;
F_49 ( V_2 ,
V_312 ,
V_330 ,
V_331 ,
V_295 [ 3 ] ) ;
F_49 ( V_2 ,
V_312 ,
V_332 ,
V_333 ,
V_295 [ 4 ] ) ;
F_49 ( V_2 ,
V_312 ,
V_334 ,
V_335 ,
V_296 [ 0 ] ) ;
F_49 ( V_2 ,
V_312 ,
V_336 ,
V_337 ,
V_296 [ 1 ] ) ;
F_49 ( V_2 ,
V_312 ,
V_338 ,
V_339 ,
V_296 [ 2 ] ) ;
F_49 ( V_2 ,
V_321 ,
V_340 ,
V_341 ,
V_296 [ 3 ] ) ;
F_49 ( V_2 ,
V_321 ,
V_342 ,
V_343 , V_296 [ 4 ] ) ;
F_49 ( V_2 ,
V_321 ,
V_344 ,
V_345 ,
V_297 [ 0 ] ) ;
F_49 ( V_2 ,
V_321 ,
V_346 ,
V_347 ,
V_297 [ 1 ] ) ;
F_49 ( V_2 ,
V_321 ,
V_348 ,
V_349 ,
V_297 [ 2 ] ) ;
F_49 ( V_2 ,
V_321 ,
V_350 ,
V_351 ,
V_297 [ 3 ] ) ;
F_49 ( V_2 ,
V_321 ,
V_352 ,
V_353 ,
V_297 [ 4 ] ) ;
}
F_24 ( V_2 , V_354 , V_355 ,
V_117 -> V_25 ) ;
F_24 ( V_2 , V_356 , V_357 ,
V_117 -> V_28 ) ;
F_27 ( V_2 , V_358 ,
F_28 ( V_117 -> V_31 , V_359 ) |
F_28 ( V_117 -> V_31 , V_360 ) |
F_28 ( V_117 -> V_33 , V_361 ) |
F_28 ( V_117 -> V_33 , V_362 ) ) ;
F_24 ( V_2 , V_363 , V_364 ,
V_117 -> V_32 ) ;
if ( F_50 ( V_2 ) )
F_24 ( V_2 , V_363 , V_364 ,
V_117 -> V_32 ) ;
F_24 ( V_2 , V_365 , V_366 ,
V_117 -> V_34 ) ;
F_24 ( V_2 , V_367 , V_368 ,
V_117 -> V_34 ) ;
if ( ( V_70 -> V_5 . V_6 & V_119 ) >=
V_160 ) {
F_24 ( V_2 , V_369 , V_370 ,
V_117 -> V_57 ) ;
F_24 ( V_2 , V_369 , V_371 ,
V_117 -> V_58 ) ;
}
if ( ( V_70 -> V_5 . V_6 & V_119 ) >=
V_286 ) {
if ( F_37 ( V_141 ) )
F_24 ( V_2 , V_354 ,
V_355 ,
V_117 -> V_62 ) ;
}
V_300 = ( V_117 -> V_372 &
V_373 ) ;
if ( ( V_72 -> V_89 == 0 ) && ( V_300 != 0 ) ) {
T_2 V_374 , V_375 , V_376 ;
V_375 = F_51 ( 0 ) | F_51 ( 5 ) | F_51 ( 10 ) | F_51 ( 15 ) | F_51 ( 20 ) | F_51 ( 25 ) ;
V_374 = V_375 * V_300 ;
V_376 = V_375 * 0x1f ;
F_52 ( V_2 , V_377 , V_374 , V_376 ) ;
F_52 ( V_2 , V_378 , V_374 , V_376 ) ;
F_52 ( V_2 , V_379 , V_374 , V_376 ) ;
V_375 = F_51 ( 0 ) | F_51 ( 5 ) | F_51 ( 15 ) ;
V_374 = V_375 * V_300 ;
V_376 = V_375 * 0x1f ;
F_52 ( V_2 , V_380 , V_374 , V_376 ) ;
V_375 = F_51 ( 0 ) | F_51 ( 5 ) ;
V_374 = V_375 * V_300 ;
V_376 = V_375 * 0x1f ;
F_52 ( V_2 , V_381 , V_374 , V_376 ) ;
F_52 ( V_2 , V_382 , V_374 , V_376 ) ;
}
}
static T_1 F_53 ( struct V_1 * V_2 , T_1 V_98 , bool V_383 )
{
#define F_54 \
(ah->eeprom.map4k.modalHeader.spurChans[i].spurChan)
struct V_7 * V_8 = F_4 ( V_2 ) ;
T_1 V_384 = V_385 ;
F_6 ( V_8 , V_386 ,
L_85 ,
V_98 , V_383 , V_2 -> V_387 . V_388 [ V_98 ] [ V_383 ] ) ;
switch ( V_2 -> V_387 . V_389 ) {
case V_390 :
break;
case V_391 :
V_384 = V_2 -> V_387 . V_388 [ V_98 ] [ V_383 ] ;
F_6 ( V_8 , V_386 ,
L_86 , V_384 ) ;
break;
case V_392 :
V_384 = F_54 ;
break;
}
return V_384 ;
#undef F_54
}
