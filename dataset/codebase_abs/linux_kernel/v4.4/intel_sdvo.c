static struct V_1 * F_1 ( struct V_2 * V_3 )
{
return F_2 ( V_3 , struct V_1 , V_4 ) ;
}
static struct V_1 * F_3 ( struct V_5 * V_6 )
{
return F_1 ( F_4 ( V_6 ) ) ;
}
static struct V_7 * F_5 ( struct V_5 * V_6 )
{
return F_2 ( F_6 ( V_6 ) , struct V_7 , V_4 ) ;
}
static void F_7 ( struct V_1 * V_1 , T_1 V_8 )
{
struct V_9 * V_10 = V_1 -> V_4 . V_4 . V_10 ;
struct V_11 * V_12 = V_10 -> V_13 ;
T_1 V_14 = V_8 , V_15 = V_8 ;
int V_16 ;
if ( V_1 -> V_17 == V_18 ) {
F_8 ( V_1 -> V_17 , V_8 ) ;
F_9 ( V_1 -> V_17 ) ;
if ( F_10 ( V_10 ) ) {
F_8 ( V_1 -> V_17 , V_8 ) ;
F_9 ( V_1 -> V_17 ) ;
}
return;
}
if ( V_1 -> V_17 == V_19 )
V_15 = F_11 ( V_20 ) ;
else
V_14 = F_11 ( V_19 ) ;
for ( V_16 = 0 ; V_16 < 2 ; V_16 ++ )
{
F_8 ( V_19 , V_14 ) ;
F_9 ( V_19 ) ;
F_8 ( V_20 , V_15 ) ;
F_9 ( V_20 ) ;
}
}
static bool F_12 ( struct V_1 * V_1 , T_2 V_21 , T_2 * V_22 )
{
struct V_23 V_24 [] = {
{
. V_21 = V_1 -> V_25 ,
. V_26 = 0 ,
. V_27 = 1 ,
. V_28 = & V_21 ,
} ,
{
. V_21 = V_1 -> V_25 ,
. V_26 = V_29 ,
. V_27 = 1 ,
. V_28 = V_22 ,
}
} ;
int V_30 ;
if ( ( V_30 = F_13 ( V_1 -> V_31 , V_24 , 2 ) ) == 2 )
return true ;
F_14 ( L_1 , V_30 ) ;
return false ;
}
static void F_15 ( struct V_1 * V_1 , T_2 V_32 ,
const void * args , int V_33 )
{
int V_16 , V_34 = 0 ;
#define F_16 256
char V_35 [ F_16 ] ;
#define F_17 ( args ... ) \
pos += snprintf(buffer + pos, max_t(int, BUF_LEN - pos, 0), args)
for ( V_16 = 0 ; V_16 < V_33 ; V_16 ++ ) {
F_17 ( L_2 , ( ( T_2 * ) args ) [ V_16 ] ) ;
}
for (; V_16 < 8 ; V_16 ++ ) {
F_17 ( L_3 ) ;
}
for ( V_16 = 0 ; V_16 < F_18 ( V_36 ) ; V_16 ++ ) {
if ( V_32 == V_36 [ V_16 ] . V_32 ) {
F_17 ( L_4 , V_36 [ V_16 ] . V_37 ) ;
break;
}
}
if ( V_16 == F_18 ( V_36 ) ) {
F_17 ( L_5 , V_32 ) ;
}
F_19 ( V_34 >= F_16 - 1 ) ;
#undef F_17
#undef F_16
F_14 ( L_6 , F_20 ( V_1 ) , V_32 , V_35 ) ;
}
static bool F_21 ( struct V_1 * V_1 , T_2 V_32 ,
const void * args , int V_33 )
{
T_2 * V_28 , V_38 ;
struct V_23 * V_24 ;
int V_16 , V_30 = true ;
V_28 = F_22 ( V_33 * 2 + 2 , V_39 ) ;
if ( ! V_28 )
return false ;
V_24 = F_23 ( V_33 + 3 , sizeof( * V_24 ) , V_39 ) ;
if ( ! V_24 ) {
F_24 ( V_28 ) ;
return false ;
}
F_15 ( V_1 , V_32 , args , V_33 ) ;
for ( V_16 = 0 ; V_16 < V_33 ; V_16 ++ ) {
V_24 [ V_16 ] . V_21 = V_1 -> V_25 ;
V_24 [ V_16 ] . V_26 = 0 ;
V_24 [ V_16 ] . V_27 = 2 ;
V_24 [ V_16 ] . V_28 = V_28 + 2 * V_16 ;
V_28 [ 2 * V_16 + 0 ] = V_40 - V_16 ;
V_28 [ 2 * V_16 + 1 ] = ( ( T_2 * ) args ) [ V_16 ] ;
}
V_24 [ V_16 ] . V_21 = V_1 -> V_25 ;
V_24 [ V_16 ] . V_26 = 0 ;
V_24 [ V_16 ] . V_27 = 2 ;
V_24 [ V_16 ] . V_28 = V_28 + 2 * V_16 ;
V_28 [ 2 * V_16 + 0 ] = V_41 ;
V_28 [ 2 * V_16 + 1 ] = V_32 ;
V_38 = V_42 ;
V_24 [ V_16 + 1 ] . V_21 = V_1 -> V_25 ;
V_24 [ V_16 + 1 ] . V_26 = 0 ;
V_24 [ V_16 + 1 ] . V_27 = 1 ;
V_24 [ V_16 + 1 ] . V_28 = & V_38 ;
V_24 [ V_16 + 2 ] . V_21 = V_1 -> V_25 ;
V_24 [ V_16 + 2 ] . V_26 = V_29 ;
V_24 [ V_16 + 2 ] . V_27 = 1 ;
V_24 [ V_16 + 2 ] . V_28 = & V_38 ;
V_30 = F_13 ( V_1 -> V_31 , V_24 , V_16 + 3 ) ;
if ( V_30 < 0 ) {
F_14 ( L_7 , V_30 ) ;
V_30 = false ;
goto V_43;
}
if ( V_30 != V_16 + 3 ) {
F_14 ( L_8 , V_30 , V_16 + 3 ) ;
V_30 = false ;
}
V_43:
F_24 ( V_24 ) ;
F_24 ( V_28 ) ;
return V_30 ;
}
static bool F_25 ( struct V_1 * V_1 ,
void * V_44 , int V_45 )
{
T_2 V_46 = 15 ;
T_2 V_38 ;
int V_16 , V_34 = 0 ;
#define F_16 256
char V_35 [ F_16 ] ;
if ( ! F_12 ( V_1 ,
V_42 ,
& V_38 ) )
goto V_47;
while ( ( V_38 == V_48 ||
V_38 == V_49 ) && -- V_46 ) {
if ( V_46 < 10 )
F_26 ( 15 ) ;
else
F_27 ( 15 ) ;
if ( ! F_12 ( V_1 ,
V_42 ,
& V_38 ) )
goto V_47;
}
#define F_17 ( args ... ) \
pos += snprintf(buffer + pos, max_t(int, BUF_LEN - pos, 0), args)
if ( V_38 <= V_50 )
F_17 ( L_4 , V_51 [ V_38 ] ) ;
else
F_17 ( L_9 , V_38 ) ;
if ( V_38 != V_52 )
goto V_47;
for ( V_16 = 0 ; V_16 < V_45 ; V_16 ++ ) {
if ( ! F_12 ( V_1 ,
V_53 + V_16 ,
& ( ( T_2 * ) V_44 ) [ V_16 ] ) )
goto V_47;
F_17 ( L_10 , ( ( T_2 * ) V_44 ) [ V_16 ] ) ;
}
F_19 ( V_34 >= F_16 - 1 ) ;
#undef F_17
#undef F_16
F_14 ( L_11 , F_20 ( V_1 ) , V_35 ) ;
return true ;
V_47:
F_14 ( L_12 , F_20 ( V_1 ) ) ;
return false ;
}
static int F_28 ( const struct V_54 * V_55 )
{
if ( V_55 -> V_56 >= 100000 )
return 1 ;
else if ( V_55 -> V_56 >= 50000 )
return 2 ;
else
return 4 ;
}
static bool F_29 ( struct V_1 * V_1 ,
T_2 V_57 )
{
return F_21 ( V_1 ,
V_58 ,
& V_57 , 1 ) ;
}
static bool F_30 ( struct V_1 * V_1 , T_2 V_32 , const void * V_59 , int V_27 )
{
if ( ! F_21 ( V_1 , V_32 , V_59 , V_27 ) )
return false ;
return F_25 ( V_1 , NULL , 0 ) ;
}
static bool
F_31 ( struct V_1 * V_1 , T_2 V_32 , void * V_60 , int V_27 )
{
if ( ! F_21 ( V_1 , V_32 , NULL , 0 ) )
return false ;
return F_25 ( V_1 , V_60 , V_27 ) ;
}
static bool F_32 ( struct V_1 * V_1 )
{
struct V_61 V_62 = { 0 } ;
return F_30 ( V_1 ,
V_63 ,
& V_62 , sizeof( V_62 ) ) ;
}
static bool F_33 ( struct V_1 * V_1 , bool * V_64 , bool * V_65 )
{
struct V_66 V_44 ;
F_34 ( sizeof( V_44 ) != 1 ) ;
if ( ! F_31 ( V_1 , V_67 ,
& V_44 , sizeof( V_44 ) ) )
return false ;
* V_64 = V_44 . V_68 ;
* V_65 = V_44 . V_69 ;
return true ;
}
static bool F_35 ( struct V_1 * V_1 ,
T_3 V_70 )
{
return F_30 ( V_1 ,
V_71 ,
& V_70 , sizeof( V_70 ) ) ;
}
static bool F_36 ( struct V_1 * V_1 ,
T_3 * V_70 )
{
return F_31 ( V_1 ,
V_72 ,
V_70 , sizeof( * V_70 ) ) ;
}
static bool F_37 ( struct V_1 * V_1 ,
int V_73 )
{
T_2 V_74 = V_75 ;
switch ( V_73 ) {
case V_76 :
V_74 = V_75 ;
break;
case V_77 :
V_74 = V_78 ;
break;
case V_79 :
V_74 = V_80 ;
break;
case V_81 :
V_74 = V_82 ;
break;
}
return F_30 ( V_1 ,
V_83 , & V_74 , sizeof( V_74 ) ) ;
}
static bool F_38 ( struct V_1 * V_1 ,
int * V_84 ,
int * V_85 )
{
struct V_86 V_87 ;
F_34 ( sizeof( V_87 ) != 4 ) ;
if ( ! F_31 ( V_1 ,
V_88 ,
& V_87 , sizeof( V_87 ) ) )
return false ;
* V_84 = V_87 . V_89 * 10 ;
* V_85 = V_87 . V_90 * 10 ;
return true ;
}
static bool F_39 ( struct V_1 * V_1 ,
T_3 V_70 )
{
return F_30 ( V_1 ,
V_91 ,
& V_70 , sizeof( V_70 ) ) ;
}
static bool F_40 ( struct V_1 * V_1 , T_2 V_32 ,
struct V_92 * V_93 )
{
return F_30 ( V_1 , V_32 , & V_93 -> V_94 , sizeof( V_93 -> V_94 ) ) &&
F_30 ( V_1 , V_32 + 1 , & V_93 -> V_95 , sizeof( V_93 -> V_95 ) ) ;
}
static bool F_41 ( struct V_1 * V_1 , T_2 V_32 ,
struct V_92 * V_93 )
{
return F_31 ( V_1 , V_32 , & V_93 -> V_94 , sizeof( V_93 -> V_94 ) ) &&
F_31 ( V_1 , V_32 + 1 , & V_93 -> V_95 , sizeof( V_93 -> V_95 ) ) ;
}
static bool F_42 ( struct V_1 * V_1 ,
struct V_92 * V_93 )
{
return F_40 ( V_1 ,
V_96 , V_93 ) ;
}
static bool F_43 ( struct V_1 * V_1 ,
struct V_92 * V_93 )
{
return F_40 ( V_1 ,
V_97 , V_93 ) ;
}
static bool F_44 ( struct V_1 * V_1 ,
struct V_92 * V_93 )
{
return F_41 ( V_1 ,
V_98 , V_93 ) ;
}
static bool
F_45 ( struct V_1 * V_1 ,
T_4 clock ,
T_4 V_99 ,
T_4 V_100 )
{
struct V_101 args ;
memset ( & args , 0 , sizeof( args ) ) ;
args . clock = clock ;
args . V_99 = V_99 ;
args . V_100 = V_100 ;
args . V_102 = 0 ;
if ( V_1 -> V_103 &&
( V_1 -> V_104 -> V_105 != V_99 ||
V_1 -> V_104 -> V_106 != V_100 ) )
args . V_107 = 1 ;
return F_30 ( V_1 ,
V_108 ,
& args , sizeof( args ) ) ;
}
static bool F_46 ( struct V_1 * V_1 ,
struct V_92 * V_93 )
{
F_34 ( sizeof( V_93 -> V_94 ) != 8 ) ;
F_34 ( sizeof( V_93 -> V_95 ) != 8 ) ;
return F_31 ( V_1 , V_109 ,
& V_93 -> V_94 , sizeof( V_93 -> V_94 ) ) &&
F_31 ( V_1 , V_110 ,
& V_93 -> V_95 , sizeof( V_93 -> V_95 ) ) ;
}
static bool F_47 ( struct V_1 * V_1 , T_2 V_8 )
{
return F_30 ( V_1 , V_111 , & V_8 , 1 ) ;
}
static void F_48 ( struct V_92 * V_93 ,
const struct V_54 * V_73 )
{
T_4 V_99 , V_100 ;
T_4 V_112 , V_113 , V_114 , V_115 ;
T_4 V_116 , V_117 ;
int V_118 ;
memset ( V_93 , 0 , sizeof( * V_93 ) ) ;
V_99 = V_73 -> V_105 ;
V_100 = V_73 -> V_106 ;
V_112 = V_73 -> V_119 - V_73 -> V_105 ;
V_113 = V_73 -> V_120 - V_73 -> V_121 ;
V_114 = V_73 -> V_122 - V_73 -> V_106 ;
V_115 = V_73 -> V_123 - V_73 -> V_124 ;
V_116 = V_73 -> V_121 - V_73 -> V_105 ;
V_117 = V_73 -> V_124 - V_73 -> V_106 ;
V_118 = V_73 -> clock ;
V_118 /= 10 ;
V_93 -> V_94 . clock = V_118 ;
V_93 -> V_94 . V_125 = V_99 & 0xff ;
V_93 -> V_94 . V_126 = V_112 & 0xff ;
V_93 -> V_94 . V_127 = ( ( ( V_99 >> 8 ) & 0xf ) << 4 ) |
( ( V_112 >> 8 ) & 0xf ) ;
V_93 -> V_94 . V_128 = V_100 & 0xff ;
V_93 -> V_94 . V_129 = V_114 & 0xff ;
V_93 -> V_94 . V_130 = ( ( ( V_100 >> 8 ) & 0xf ) << 4 ) |
( ( V_114 >> 8 ) & 0xf ) ;
V_93 -> V_95 . V_131 = V_116 & 0xff ;
V_93 -> V_95 . V_132 = V_113 & 0xff ;
V_93 -> V_95 . V_133 = ( V_117 & 0xf ) << 4 |
( V_115 & 0xf ) ;
V_93 -> V_95 . V_134 = ( ( V_116 & 0x300 ) >> 2 ) |
( ( V_113 & 0x300 ) >> 4 ) | ( ( V_117 & 0x30 ) >> 2 ) |
( ( V_115 & 0x30 ) >> 4 ) ;
V_93 -> V_95 . V_135 = 0x18 ;
if ( V_73 -> V_26 & V_136 )
V_93 -> V_95 . V_135 |= V_137 ;
if ( V_73 -> V_26 & V_138 )
V_93 -> V_95 . V_135 |= V_139 ;
if ( V_73 -> V_26 & V_140 )
V_93 -> V_95 . V_135 |= V_141 ;
V_93 -> V_95 . V_142 = V_117 & 0xc0 ;
}
static void F_49 ( struct V_54 * V_143 ,
const struct V_92 * V_93 )
{
struct V_54 V_73 = {} ;
V_73 . V_105 = V_93 -> V_94 . V_125 ;
V_73 . V_105 += ( ( V_93 -> V_94 . V_127 >> 4 ) & 0x0f ) << 8 ;
V_73 . V_121 = V_73 . V_105 + V_93 -> V_95 . V_131 ;
V_73 . V_121 += ( V_93 -> V_95 . V_134 & 0xc0 ) << 2 ;
V_73 . V_120 = V_73 . V_121 + V_93 -> V_95 . V_132 ;
V_73 . V_120 += ( V_93 -> V_95 . V_134 & 0x30 ) << 4 ;
V_73 . V_119 = V_73 . V_105 + V_93 -> V_94 . V_126 ;
V_73 . V_119 += ( V_93 -> V_94 . V_127 & 0xf ) << 8 ;
V_73 . V_106 = V_93 -> V_94 . V_128 ;
V_73 . V_106 += ( ( V_93 -> V_94 . V_130 >> 4 ) & 0x0f ) << 8 ;
V_73 . V_124 = V_73 . V_106 ;
V_73 . V_124 += ( V_93 -> V_95 . V_133 >> 4 ) & 0xf ;
V_73 . V_124 += ( V_93 -> V_95 . V_134 & 0x0c ) << 2 ;
V_73 . V_124 += V_93 -> V_95 . V_142 & 0xc0 ;
V_73 . V_123 = V_73 . V_124 +
( V_93 -> V_95 . V_133 & 0xf ) ;
V_73 . V_123 += ( V_93 -> V_95 . V_134 & 0x3 ) << 4 ;
V_73 . V_122 = V_73 . V_106 + V_93 -> V_94 . V_129 ;
V_73 . V_122 += ( V_93 -> V_94 . V_130 & 0xf ) << 8 ;
V_73 . clock = V_93 -> V_94 . clock * 10 ;
if ( V_93 -> V_95 . V_135 & V_137 )
V_73 . V_26 |= V_136 ;
if ( V_93 -> V_95 . V_135 & V_139 )
V_73 . V_26 |= V_138 ;
else
V_73 . V_26 |= V_144 ;
if ( V_93 -> V_95 . V_135 & V_141 )
V_73 . V_26 |= V_140 ;
else
V_73 . V_26 |= V_145 ;
F_50 ( & V_73 , 0 ) ;
F_51 ( V_143 , & V_73 ) ;
}
static bool F_52 ( struct V_1 * V_1 )
{
struct V_146 V_147 ;
F_34 ( sizeof( V_147 ) != 2 ) ;
return F_31 ( V_1 ,
V_148 ,
& V_147 , sizeof( V_147 ) ) ;
}
static bool F_53 ( struct V_1 * V_1 ,
T_5 V_73 )
{
return F_30 ( V_1 , V_149 , & V_73 , 1 ) ;
}
static bool F_54 ( struct V_1 * V_1 ,
T_5 V_73 )
{
return F_30 ( V_1 , V_150 , & V_73 , 1 ) ;
}
static bool F_55 ( struct V_1 * V_1 ,
unsigned V_151 , T_5 V_152 ,
const T_5 * V_59 , unsigned V_153 )
{
T_5 V_154 [ 2 ] = { V_151 , 0 } ;
T_5 V_155 , V_156 [ 8 ] ;
int V_16 ;
if ( ! F_30 ( V_1 ,
V_157 ,
V_154 , 2 ) )
return false ;
if ( ! F_31 ( V_1 , V_158 ,
& V_155 , 1 ) )
return false ;
V_155 ++ ;
F_14 ( L_13 ,
V_151 , V_153 , V_155 ) ;
for ( V_16 = 0 ; V_16 < V_155 ; V_16 += 8 ) {
memset ( V_156 , 0 , 8 ) ;
if ( V_16 < V_153 )
memcpy ( V_156 , V_59 + V_16 , F_56 ( unsigned , 8 , V_153 - V_16 ) ) ;
if ( ! F_30 ( V_1 ,
V_159 ,
V_156 , 8 ) )
return false ;
}
return F_30 ( V_1 ,
V_160 ,
& V_152 , 1 ) ;
}
static bool F_57 ( struct V_1 * V_1 ,
const struct V_54 * V_55 )
{
T_5 V_161 [ F_58 ( V_162 ) ] ;
struct V_163 * V_164 = V_1 -> V_4 . V_4 . V_164 ;
struct V_165 * V_165 = F_59 ( V_164 ) ;
union V_166 V_167 ;
int V_30 ;
T_6 V_27 ;
V_30 = F_60 ( & V_167 . V_168 ,
V_55 ) ;
if ( V_30 < 0 ) {
F_61 ( L_14 ) ;
return false ;
}
if ( V_1 -> V_169 ) {
if ( V_165 -> V_170 -> V_171 )
V_167 . V_168 . V_172 =
V_173 ;
else
V_167 . V_168 . V_172 =
V_174 ;
}
V_27 = F_62 ( & V_167 , V_161 , sizeof( V_161 ) ) ;
if ( V_27 < 0 )
return false ;
return F_55 ( V_1 , V_175 ,
V_176 ,
V_161 , sizeof( V_161 ) ) ;
}
static bool F_63 ( struct V_1 * V_1 )
{
struct V_177 V_178 ;
T_7 V_179 ;
V_179 = 1 << V_1 -> V_180 ;
memset ( & V_178 , 0 , sizeof( V_178 ) ) ;
memcpy ( & V_178 , & V_179 , V_89 ( sizeof( V_178 ) , sizeof( V_179 ) ) ) ;
F_34 ( sizeof( V_178 ) != 6 ) ;
return F_30 ( V_1 ,
V_181 ,
& V_178 , sizeof( V_178 ) ) ;
}
static bool
F_64 ( struct V_1 * V_1 ,
const struct V_54 * V_73 )
{
struct V_92 V_182 ;
if ( ! F_39 ( V_1 ,
V_1 -> V_183 ) )
return false ;
F_48 ( & V_182 , V_73 ) ;
if ( ! F_43 ( V_1 , & V_182 ) )
return false ;
return true ;
}
static bool
F_65 ( struct V_1 * V_1 ,
const struct V_54 * V_73 ,
struct V_54 * V_55 )
{
struct V_92 V_184 ;
if ( ! F_32 ( V_1 ) )
return false ;
if ( ! F_45 ( V_1 ,
V_73 -> clock / 10 ,
V_73 -> V_105 ,
V_73 -> V_106 ) )
return false ;
if ( ! F_46 ( V_1 ,
& V_184 ) )
return false ;
F_49 ( V_55 , & V_184 ) ;
V_1 -> V_185 = V_184 . V_95 . V_186 ;
return true ;
}
static void F_66 ( struct V_187 * V_188 )
{
unsigned V_189 = V_188 -> V_190 ;
struct V_191 * clock = & V_188 -> V_191 ;
if ( V_189 >= 100000 && V_189 < 140500 ) {
clock -> V_192 = 2 ;
clock -> V_193 = 10 ;
clock -> V_194 = 3 ;
clock -> V_195 = 16 ;
clock -> V_196 = 8 ;
} else if ( V_189 >= 140500 && V_189 <= 200000 ) {
clock -> V_192 = 1 ;
clock -> V_193 = 10 ;
clock -> V_194 = 6 ;
clock -> V_195 = 12 ;
clock -> V_196 = 8 ;
} else {
F_67 ( 1 , L_15 , V_189 ) ;
}
V_188 -> V_197 = true ;
}
static bool F_68 ( struct V_2 * V_3 ,
struct V_187 * V_188 )
{
struct V_1 * V_1 = F_1 ( V_3 ) ;
struct V_54 * V_55 = & V_188 -> V_4 . V_55 ;
struct V_54 * V_73 = & V_188 -> V_4 . V_73 ;
F_14 ( L_16 ) ;
V_188 -> V_198 = 8 * 3 ;
if ( F_69 ( V_3 -> V_4 . V_10 ) )
V_188 -> V_199 = true ;
if ( V_1 -> V_200 ) {
if ( ! F_64 ( V_1 , V_73 ) )
return false ;
( void ) F_65 ( V_1 ,
V_73 ,
V_55 ) ;
V_188 -> V_201 = true ;
} else if ( V_1 -> V_103 ) {
if ( ! F_64 ( V_1 ,
V_1 -> V_104 ) )
return false ;
( void ) F_65 ( V_1 ,
V_73 ,
V_55 ) ;
}
V_188 -> V_202 =
F_28 ( V_55 ) ;
V_188 -> V_203 = V_1 -> V_204 ;
if ( V_1 -> V_205 ) {
if ( V_188 -> V_203 &&
F_70 ( V_55 ) > 1 )
V_188 -> V_171 = true ;
} else {
if ( V_188 -> V_203 &&
V_1 -> V_206 == V_207 )
V_188 -> V_171 = true ;
}
if ( V_1 -> V_200 )
F_66 ( V_188 ) ;
if ( V_1 -> V_208 )
V_55 -> V_209 = V_1 -> V_210 ;
return true ;
}
static void F_71 ( struct V_2 * V_2 )
{
struct V_9 * V_10 = V_2 -> V_4 . V_10 ;
struct V_11 * V_12 = V_10 -> V_13 ;
struct V_165 * V_164 = F_59 ( V_2 -> V_4 . V_164 ) ;
const struct V_54 * V_55 = & V_164 -> V_170 -> V_4 . V_55 ;
struct V_54 * V_73 = & V_164 -> V_170 -> V_4 . V_73 ;
struct V_1 * V_1 = F_1 ( V_2 ) ;
T_1 V_211 ;
struct V_212 V_213 ;
struct V_92 V_184 , V_182 ;
int V_214 ;
if ( ! V_73 )
return;
V_213 . V_215 = V_1 -> V_183 ;
V_213 . V_216 = 0 ;
F_30 ( V_1 ,
V_217 ,
& V_213 , sizeof( V_213 ) ) ;
if ( ! F_39 ( V_1 ,
V_1 -> V_183 ) )
return;
if ( V_1 -> V_103 )
F_48 ( & V_182 ,
V_1 -> V_104 ) ;
else
F_48 ( & V_182 , V_73 ) ;
if ( ! F_43 ( V_1 , & V_182 ) )
F_72 ( L_17 ,
F_20 ( V_1 ) ) ;
if ( ! F_32 ( V_1 ) )
return;
if ( V_164 -> V_170 -> V_203 ) {
F_53 ( V_1 , V_218 ) ;
F_54 ( V_1 ,
V_219 ) ;
F_57 ( V_1 , V_55 ) ;
} else
F_53 ( V_1 , V_220 ) ;
if ( V_1 -> V_200 &&
! F_63 ( V_1 ) )
return;
F_48 ( & V_184 , V_55 ) ;
if ( V_1 -> V_200 || V_1 -> V_103 )
V_184 . V_95 . V_186 = V_1 -> V_185 ;
if ( ! F_42 ( V_1 , & V_184 ) )
F_72 ( L_18 ,
F_20 ( V_1 ) ) ;
switch ( V_164 -> V_170 -> V_202 ) {
default:
F_67 ( 1 , L_19 ) ;
case 1 : V_214 = V_221 ; break;
case 2 : V_214 = V_222 ; break;
case 4 : V_214 = V_223 ; break;
}
if ( ! F_47 ( V_1 , V_214 ) )
return;
if ( F_73 ( V_10 ) -> V_224 >= 4 ) {
V_211 = V_225 | V_226 ;
if ( ! F_69 ( V_10 ) && V_164 -> V_170 -> V_171 )
V_211 |= V_207 ;
if ( F_73 ( V_10 ) -> V_224 < 5 )
V_211 |= V_227 ;
} else {
V_211 = F_11 ( V_1 -> V_17 ) ;
switch ( V_1 -> V_17 ) {
case V_19 :
V_211 &= V_228 ;
break;
case V_20 :
V_211 &= V_229 ;
break;
}
V_211 |= ( 9 << 19 ) | V_227 ;
}
if ( F_74 ( V_10 ) >= V_230 )
V_211 |= F_75 ( V_164 -> V_231 ) ;
else
V_211 |= F_76 ( V_164 -> V_231 ) ;
if ( V_1 -> V_232 )
V_211 |= V_233 ;
if ( F_73 ( V_10 ) -> V_224 >= 4 ) {
} else if ( F_77 ( V_10 ) || F_78 ( V_10 ) || F_79 ( V_10 ) ) {
} else {
V_211 |= ( V_164 -> V_170 -> V_202 - 1 )
<< V_234 ;
}
if ( V_184 . V_95 . V_186 & V_235 &&
F_73 ( V_10 ) -> V_224 < 5 )
V_211 |= V_236 ;
F_7 ( V_1 , V_211 ) ;
}
static bool F_80 ( struct V_237 * V_6 )
{
struct V_7 * V_7 =
F_5 ( & V_6 -> V_4 ) ;
struct V_1 * V_1 = F_3 ( & V_6 -> V_4 ) ;
T_3 V_238 = 0 ;
F_36 ( V_1 , & V_238 ) ;
if ( V_238 & V_7 -> V_239 )
return true ;
else
return false ;
}
static bool F_81 ( struct V_2 * V_3 ,
enum V_231 * V_231 )
{
struct V_9 * V_10 = V_3 -> V_4 . V_10 ;
struct V_11 * V_12 = V_10 -> V_13 ;
struct V_1 * V_1 = F_1 ( V_3 ) ;
T_3 V_238 = 0 ;
T_1 V_156 ;
V_156 = F_11 ( V_1 -> V_17 ) ;
F_36 ( V_1 , & V_238 ) ;
if ( ! ( V_156 & V_240 ) && ( V_238 == 0 ) )
return false ;
if ( F_82 ( V_10 ) )
* V_231 = F_83 ( V_156 ) ;
else
* V_231 = F_84 ( V_156 ) ;
return true ;
}
static void F_85 ( struct V_2 * V_3 ,
struct V_187 * V_188 )
{
struct V_9 * V_10 = V_3 -> V_4 . V_10 ;
struct V_11 * V_12 = V_10 -> V_13 ;
struct V_1 * V_1 = F_1 ( V_3 ) ;
struct V_92 V_93 ;
int V_241 = 0 ;
int V_189 ;
T_1 V_26 = 0 , V_211 ;
T_2 V_8 ;
bool V_30 ;
V_211 = F_11 ( V_1 -> V_17 ) ;
V_30 = F_44 ( V_1 , & V_93 ) ;
if ( ! V_30 ) {
F_86 ( L_20 ) ;
V_188 -> V_242 |= V_243 ;
} else {
if ( V_93 . V_95 . V_135 & V_139 )
V_26 |= V_138 ;
else
V_26 |= V_144 ;
if ( V_93 . V_95 . V_135 & V_141 )
V_26 |= V_140 ;
else
V_26 |= V_145 ;
}
V_188 -> V_4 . V_55 . V_26 |= V_26 ;
if ( F_87 ( V_10 ) || F_88 ( V_10 ) ) {
V_188 -> V_202 =
( ( V_211 & V_244 )
>> V_234 ) + 1 ;
}
V_189 = V_188 -> V_190 ;
if ( V_188 -> V_202 )
V_189 /= V_188 -> V_202 ;
if ( F_69 ( V_10 ) )
F_89 ( V_188 , V_189 ) ;
V_188 -> V_4 . V_55 . V_56 = V_189 ;
if ( F_31 ( V_1 , V_245 ,
& V_8 , 1 ) ) {
switch ( V_8 ) {
case V_221 :
V_241 = 1 ;
break;
case V_222 :
V_241 = 2 ;
break;
case V_223 :
V_241 = 4 ;
break;
}
}
if ( V_211 & V_207 )
V_188 -> V_171 = true ;
if ( F_31 ( V_1 , V_246 ,
& V_8 , 1 ) ) {
if ( V_8 == V_218 )
V_188 -> V_203 = true ;
}
F_67 ( V_241 != V_188 -> V_202 ,
L_21 ,
V_188 -> V_202 , V_241 ) ;
}
static void F_90 ( struct V_2 * V_3 )
{
struct V_11 * V_12 = V_3 -> V_4 . V_10 -> V_13 ;
struct V_1 * V_1 = F_1 ( V_3 ) ;
struct V_165 * V_164 = F_59 ( V_3 -> V_4 . V_164 ) ;
T_1 V_247 ;
F_35 ( V_1 , 0 ) ;
if ( 0 )
F_37 ( V_1 ,
V_81 ) ;
V_247 = F_11 ( V_1 -> V_17 ) ;
V_247 &= ~ V_240 ;
F_7 ( V_1 , V_247 ) ;
if ( F_10 ( V_12 ) && V_164 -> V_231 == V_248 ) {
V_247 &= ~ V_249 ;
V_247 |= V_240 ;
F_7 ( V_1 , V_247 ) ;
V_247 &= ~ V_240 ;
F_7 ( V_1 , V_247 ) ;
}
}
static void F_91 ( struct V_2 * V_3 )
{
}
static void F_92 ( struct V_2 * V_3 )
{
F_90 ( V_3 ) ;
}
static void F_93 ( struct V_2 * V_3 )
{
struct V_9 * V_10 = V_3 -> V_4 . V_10 ;
struct V_11 * V_12 = V_10 -> V_13 ;
struct V_1 * V_1 = F_1 ( V_3 ) ;
struct V_165 * V_165 = F_59 ( V_3 -> V_4 . V_164 ) ;
T_1 V_247 ;
bool V_250 , V_251 ;
int V_16 ;
bool V_252 ;
V_247 = F_11 ( V_1 -> V_17 ) ;
V_247 |= V_240 ;
F_7 ( V_1 , V_247 ) ;
for ( V_16 = 0 ; V_16 < 2 ; V_16 ++ )
F_94 ( V_10 , V_165 -> V_231 ) ;
V_252 = F_33 ( V_1 , & V_250 , & V_251 ) ;
if ( V_252 && ! V_250 ) {
F_14 ( L_22
L_23 , F_20 ( V_1 ) ) ;
}
if ( 0 )
F_37 ( V_1 ,
V_76 ) ;
F_35 ( V_1 , V_1 -> V_183 ) ;
}
static enum V_253
F_95 ( struct V_5 * V_6 ,
struct V_54 * V_73 )
{
struct V_1 * V_1 = F_3 ( V_6 ) ;
if ( V_73 -> V_26 & V_254 )
return V_255 ;
if ( V_1 -> V_256 > V_73 -> clock )
return V_257 ;
if ( V_1 -> V_258 < V_73 -> clock )
return V_259 ;
if ( V_1 -> V_103 ) {
if ( V_73 -> V_105 > V_1 -> V_104 -> V_105 )
return V_260 ;
if ( V_73 -> V_106 > V_1 -> V_104 -> V_106 )
return V_260 ;
}
return V_261 ;
}
static bool F_96 ( struct V_1 * V_1 , struct V_262 * V_263 )
{
F_34 ( sizeof( * V_263 ) != 8 ) ;
if ( ! F_31 ( V_1 ,
V_264 ,
V_263 , sizeof( * V_263 ) ) )
return false ;
F_14 ( L_24
L_25
L_26
L_27
L_28
L_29
L_30
L_31
L_32
L_33
L_34
L_35
L_36 ,
V_263 -> V_265 ,
V_263 -> V_266 ,
V_263 -> V_267 ,
V_263 -> V_268 ,
V_263 -> V_269 ,
V_263 -> V_270 ,
V_263 -> V_271 ,
V_263 -> V_272 ,
V_263 -> V_273 ,
V_263 -> V_274 ,
V_263 -> V_275 ,
V_263 -> V_276 ) ;
return true ;
}
static T_4 F_97 ( struct V_1 * V_1 )
{
struct V_9 * V_10 = V_1 -> V_4 . V_4 . V_10 ;
T_4 V_277 ;
if ( ! F_98 ( V_10 ) )
return 0 ;
if ( F_77 ( V_10 ) || F_78 ( V_10 ) )
return 0 ;
if ( ! F_31 ( V_1 , V_278 ,
& V_277 , sizeof( V_277 ) ) )
return 0 ;
return V_277 ;
}
static void F_99 ( struct V_2 * V_3 )
{
struct V_1 * V_1 = F_1 ( V_3 ) ;
F_21 ( V_1 , V_279 ,
& V_1 -> V_280 , 2 ) ;
}
static bool
F_100 ( struct V_1 * V_1 )
{
return F_101 ( V_1 -> V_263 . V_276 ) > 1 ;
}
static struct V_281 *
F_102 ( struct V_5 * V_6 )
{
struct V_1 * V_282 = F_3 ( V_6 ) ;
return F_103 ( V_6 , & V_282 -> V_283 ) ;
}
static struct V_281 *
F_104 ( struct V_5 * V_6 )
{
struct V_11 * V_12 = V_6 -> V_10 -> V_13 ;
return F_103 ( V_6 ,
F_105 ( V_12 ,
V_12 -> V_284 . V_285 ) ) ;
}
static enum V_286
F_106 ( struct V_5 * V_6 )
{
struct V_1 * V_1 = F_3 ( V_6 ) ;
enum V_286 V_38 ;
struct V_281 * V_281 ;
V_281 = F_102 ( V_6 ) ;
if ( V_281 == NULL && F_100 ( V_1 ) ) {
T_2 V_283 , V_287 = V_1 -> V_57 ;
for ( V_283 = V_1 -> V_57 >> 1 ; V_283 > 1 ; V_283 >>= 1 ) {
V_1 -> V_57 = V_283 ;
V_281 = F_102 ( V_6 ) ;
if ( V_281 )
break;
}
if ( V_281 == NULL )
V_1 -> V_57 = V_287 ;
}
if ( V_281 == NULL )
V_281 = F_104 ( V_6 ) ;
V_38 = V_288 ;
if ( V_281 != NULL ) {
if ( V_281 -> V_289 & V_290 ) {
V_38 = V_291 ;
if ( V_1 -> V_208 ) {
V_1 -> V_204 = F_107 ( V_281 ) ;
V_1 -> V_232 = F_108 ( V_281 ) ;
V_1 -> V_169 =
F_109 ( V_281 ) ;
}
} else
V_38 = V_292 ;
F_24 ( V_281 ) ;
}
if ( V_38 == V_291 ) {
struct V_7 * V_7 = F_5 ( V_6 ) ;
if ( V_7 -> V_293 != V_294 )
V_1 -> V_232 = ( V_7 -> V_293 == V_295 ) ;
}
return V_38 ;
}
static bool
F_110 ( struct V_7 * V_282 ,
struct V_281 * V_281 )
{
bool V_296 = ! ! ( V_281 -> V_289 & V_290 ) ;
bool V_297 = ! ! F_111 ( V_282 ) ;
F_14 ( L_37 ,
V_297 , V_296 ) ;
return V_297 == V_296 ;
}
static enum V_286
F_112 ( struct V_5 * V_6 , bool V_298 )
{
T_4 V_44 ;
struct V_1 * V_1 = F_3 ( V_6 ) ;
struct V_7 * V_7 = F_5 ( V_6 ) ;
enum V_286 V_30 ;
F_14 ( L_38 ,
V_6 -> V_4 . V_299 , V_6 -> V_37 ) ;
if ( ! F_31 ( V_1 ,
V_300 ,
& V_44 , 2 ) )
return V_288 ;
F_14 ( L_39 ,
V_44 & 0xff , V_44 >> 8 ,
V_7 -> V_239 ) ;
if ( V_44 == 0 )
return V_292 ;
V_1 -> V_183 = V_44 ;
V_1 -> V_204 = false ;
V_1 -> V_232 = false ;
V_1 -> V_169 = false ;
if ( ( V_7 -> V_239 & V_44 ) == 0 )
V_30 = V_292 ;
else if ( F_113 ( V_7 ) )
V_30 = F_106 ( V_6 ) ;
else {
struct V_281 * V_281 ;
V_281 = F_102 ( V_6 ) ;
if ( V_281 == NULL )
V_281 = F_104 ( V_6 ) ;
if ( V_281 != NULL ) {
if ( F_110 ( V_7 ,
V_281 ) )
V_30 = V_291 ;
else
V_30 = V_292 ;
F_24 ( V_281 ) ;
} else
V_30 = V_291 ;
}
if ( V_30 == V_291 ) {
V_1 -> V_200 = false ;
V_1 -> V_103 = false ;
if ( V_44 & V_301 )
V_1 -> V_200 = true ;
if ( V_44 & V_302 )
V_1 -> V_103 = V_1 -> V_104 != NULL ;
}
return V_30 ;
}
static void F_114 ( struct V_5 * V_6 )
{
struct V_281 * V_281 ;
F_14 ( L_38 ,
V_6 -> V_4 . V_299 , V_6 -> V_37 ) ;
V_281 = F_102 ( V_6 ) ;
if ( V_281 == NULL )
V_281 = F_104 ( V_6 ) ;
if ( V_281 != NULL ) {
if ( F_110 ( F_5 ( V_6 ) ,
V_281 ) ) {
F_115 ( V_6 , V_281 ) ;
F_116 ( V_6 , V_281 ) ;
}
F_24 ( V_281 ) ;
}
}
static void F_117 ( struct V_5 * V_6 )
{
struct V_1 * V_1 = F_3 ( V_6 ) ;
struct V_303 V_304 ;
T_7 V_305 = 0 , V_179 = 0 ;
int V_16 ;
F_14 ( L_38 ,
V_6 -> V_4 . V_299 , V_6 -> V_37 ) ;
V_179 = 1 << V_1 -> V_180 ;
memcpy ( & V_304 , & V_179 ,
V_89 ( sizeof( V_179 ) , sizeof( struct V_303 ) ) ) ;
if ( ! F_39 ( V_1 , V_1 -> V_183 ) )
return;
F_34 ( sizeof( V_304 ) != 3 ) ;
if ( ! F_21 ( V_1 ,
V_306 ,
& V_304 , sizeof( V_304 ) ) )
return;
if ( ! F_25 ( V_1 , & V_305 , 3 ) )
return;
for ( V_16 = 0 ; V_16 < F_18 ( V_307 ) ; V_16 ++ )
if ( V_305 & ( 1 << V_16 ) ) {
struct V_54 * V_308 ;
V_308 = F_118 ( V_6 -> V_10 ,
& V_307 [ V_16 ] ) ;
if ( V_308 )
F_119 ( V_6 , V_308 ) ;
}
}
static void F_120 ( struct V_5 * V_6 )
{
struct V_1 * V_1 = F_3 ( V_6 ) ;
struct V_11 * V_12 = V_6 -> V_10 -> V_13 ;
struct V_54 * V_309 ;
F_14 ( L_38 ,
V_6 -> V_4 . V_299 , V_6 -> V_37 ) ;
if ( V_12 -> V_284 . V_310 != NULL ) {
V_309 = F_118 ( V_6 -> V_10 ,
V_12 -> V_284 . V_310 ) ;
if ( V_309 != NULL ) {
V_309 -> type = ( V_311 |
V_312 ) ;
F_119 ( V_6 , V_309 ) ;
}
}
F_121 ( V_6 , & V_1 -> V_283 ) ;
F_122 (newmode, &connector->probed_modes, head) {
if ( V_309 -> type & V_311 ) {
V_1 -> V_104 =
F_118 ( V_6 -> V_10 , V_309 ) ;
V_1 -> V_103 = true ;
break;
}
}
}
static int F_123 ( struct V_5 * V_6 )
{
struct V_7 * V_7 = F_5 ( V_6 ) ;
if ( F_124 ( V_7 ) )
F_117 ( V_6 ) ;
else if ( F_125 ( V_7 ) )
F_120 ( V_6 ) ;
else
F_114 ( V_6 ) ;
return ! F_126 ( & V_6 -> V_313 ) ;
}
static void F_127 ( struct V_5 * V_6 )
{
struct V_7 * V_7 = F_5 ( V_6 ) ;
F_128 ( V_6 ) ;
F_24 ( V_7 ) ;
}
static bool F_129 ( struct V_5 * V_6 )
{
struct V_1 * V_1 = F_3 ( V_6 ) ;
struct V_281 * V_281 ;
bool V_314 = false ;
if ( ! V_1 -> V_208 )
return false ;
V_281 = F_102 ( V_6 ) ;
if ( V_281 != NULL && V_281 -> V_289 & V_290 )
V_314 = F_108 ( V_281 ) ;
F_24 ( V_281 ) ;
return V_314 ;
}
static int
F_130 ( struct V_5 * V_6 ,
struct V_315 * V_316 ,
T_8 V_8 )
{
struct V_1 * V_1 = F_3 ( V_6 ) ;
struct V_7 * V_7 = F_5 ( V_6 ) ;
struct V_11 * V_12 = V_6 -> V_10 -> V_13 ;
T_4 V_317 ;
T_5 V_32 ;
int V_30 ;
V_30 = F_131 ( & V_6 -> V_4 , V_316 , V_8 ) ;
if ( V_30 )
return V_30 ;
if ( V_316 == V_12 -> V_318 ) {
int V_16 = V_8 ;
bool V_314 ;
if ( V_16 == V_7 -> V_293 )
return 0 ;
V_7 -> V_293 = V_16 ;
if ( V_16 == V_294 )
V_314 = F_129 ( V_6 ) ;
else
V_314 = ( V_16 == V_295 ) ;
if ( V_314 == V_1 -> V_232 )
return 0 ;
V_1 -> V_232 = V_314 ;
goto V_319;
}
if ( V_316 == V_12 -> V_320 ) {
bool V_321 = V_1 -> V_205 ;
T_7 V_322 = V_1 -> V_206 ;
switch ( V_8 ) {
case V_323 :
V_1 -> V_205 = true ;
break;
case V_324 :
V_1 -> V_205 = false ;
V_1 -> V_206 = 0 ;
break;
case V_325 :
V_1 -> V_205 = false ;
V_1 -> V_206 = V_207 ;
break;
default:
return - V_326 ;
}
if ( V_321 == V_1 -> V_205 &&
V_322 == V_1 -> V_206 )
return 0 ;
goto V_319;
}
if ( V_316 == V_6 -> V_10 -> V_327 . V_328 ) {
switch ( V_8 ) {
case V_329 :
V_1 -> V_210 = V_330 ;
break;
case V_331 :
V_1 -> V_210 = V_332 ;
break;
case V_333 :
V_1 -> V_210 = V_334 ;
break;
default:
return - V_326 ;
}
goto V_319;
}
#define F_132 ( V_37 , T_9 ) \
if (intel_sdvo_connector->name == property) { \
if (intel_sdvo_connector->cur_##name == temp_value) return 0; \
if (intel_sdvo_connector->max_##name < temp_value) return -EINVAL; \
cmd = SDVO_CMD_SET_##NAME; \
intel_sdvo_connector->cur_##name = temp_value; \
goto set_value; \
}
if ( V_316 == V_7 -> V_335 ) {
if ( V_8 >= V_336 )
return - V_326 ;
if ( V_1 -> V_180 ==
V_7 -> V_337 [ V_8 ] )
return 0 ;
V_1 -> V_180 = V_7 -> V_337 [ V_8 ] ;
goto V_319;
} else if ( F_133 ( V_7 ) ) {
V_317 = V_8 ;
if ( V_7 -> V_338 == V_316 ) {
F_131 ( & V_6 -> V_4 ,
V_7 -> V_339 , V_8 ) ;
if ( V_7 -> V_340 == V_317 )
return 0 ;
V_7 -> V_340 = V_317 ;
V_7 -> V_341 = V_317 ;
V_317 = V_7 -> V_342 -
V_7 -> V_340 ;
V_32 = V_343 ;
goto V_344;
} else if ( V_7 -> V_339 == V_316 ) {
F_131 ( & V_6 -> V_4 ,
V_7 -> V_338 , V_8 ) ;
if ( V_7 -> V_341 == V_317 )
return 0 ;
V_7 -> V_340 = V_317 ;
V_7 -> V_341 = V_317 ;
V_317 = V_7 -> V_342 -
V_7 -> V_340 ;
V_32 = V_343 ;
goto V_344;
} else if ( V_7 -> V_345 == V_316 ) {
F_131 ( & V_6 -> V_4 ,
V_7 -> V_346 , V_8 ) ;
if ( V_7 -> V_347 == V_317 )
return 0 ;
V_7 -> V_347 = V_317 ;
V_7 -> V_348 = V_317 ;
V_317 = V_7 -> V_349 -
V_7 -> V_347 ;
V_32 = V_350 ;
goto V_344;
} else if ( V_7 -> V_346 == V_316 ) {
F_131 ( & V_6 -> V_4 ,
V_7 -> V_345 , V_8 ) ;
if ( V_7 -> V_348 == V_317 )
return 0 ;
V_7 -> V_347 = V_317 ;
V_7 -> V_348 = V_317 ;
V_317 = V_7 -> V_349 -
V_7 -> V_347 ;
V_32 = V_350 ;
goto V_344;
}
F_132 (hpos, HPOS)
F_132 (vpos, VPOS)
F_132 (saturation, SATURATION)
F_132 (contrast, CONTRAST)
F_132 (hue, HUE)
F_132 (brightness, BRIGHTNESS)
F_132 (sharpness, SHARPNESS)
F_132 (flicker_filter, FLICKER_FILTER)
F_132 (flicker_filter_2d, FLICKER_FILTER_2D)
F_132 (flicker_filter_adaptive, FLICKER_FILTER_ADAPTIVE)
F_132 (tv_chroma_filter, TV_CHROMA_FILTER)
F_132 (tv_luma_filter, TV_LUMA_FILTER)
F_132 (dot_crawl, DOT_CRAWL)
}
return - V_326 ;
V_344:
if ( ! F_30 ( V_1 , V_32 , & V_317 , 2 ) )
return - V_351 ;
V_319:
if ( V_1 -> V_4 . V_4 . V_164 )
F_134 ( V_1 -> V_4 . V_4 . V_164 ) ;
return 0 ;
#undef F_132
}
static void F_135 ( struct V_352 * V_3 )
{
struct V_1 * V_1 = F_1 ( F_136 ( V_3 ) ) ;
if ( V_1 -> V_104 != NULL )
F_137 ( V_3 -> V_10 ,
V_1 -> V_104 ) ;
F_138 ( & V_1 -> V_283 ) ;
F_139 ( V_3 ) ;
}
static void
F_140 ( struct V_1 * V_282 )
{
T_4 V_353 = 0 ;
unsigned int V_354 ;
switch ( V_282 -> V_355 ) {
case V_356 :
V_353 |= V_356 ;
case V_357 :
V_353 |= V_357 ;
case V_358 :
V_353 |= V_358 ;
case V_359 :
V_353 |= V_359 ;
case V_360 :
V_353 |= V_360 ;
case V_361 :
V_353 |= V_361 ;
break;
}
V_353 &= V_282 -> V_263 . V_276 ;
V_354 = F_101 ( V_353 ) ;
if ( V_354 > 3 )
V_354 = 3 ;
V_282 -> V_57 = 1 << V_354 ;
}
static void
F_141 ( struct V_11 * V_12 ,
struct V_1 * V_282 )
{
struct V_362 * V_363 ;
if ( V_282 -> V_364 )
V_363 = & ( V_12 -> V_365 [ 0 ] ) ;
else
V_363 = & ( V_12 -> V_365 [ 1 ] ) ;
if ( V_363 -> V_366 )
V_282 -> V_57 = 1 << ( ( V_363 -> V_367 & 0xf0 ) >> 4 ) ;
else
F_140 ( V_282 ) ;
}
static void
F_142 ( struct V_11 * V_12 ,
struct V_1 * V_282 )
{
struct V_362 * V_363 ;
T_2 V_368 ;
if ( V_282 -> V_364 )
V_363 = & V_12 -> V_365 [ 0 ] ;
else
V_363 = & V_12 -> V_365 [ 1 ] ;
if ( V_363 -> V_366 &&
F_143 ( V_12 , V_363 -> V_369 ) )
V_368 = V_363 -> V_369 ;
else
V_368 = V_370 ;
V_282 -> V_31 = F_105 ( V_12 , V_368 ) ;
F_144 ( V_282 -> V_31 , true ) ;
}
static void
F_145 ( struct V_1 * V_282 )
{
F_144 ( V_282 -> V_31 , false ) ;
}
static bool
F_146 ( struct V_1 * V_1 , int V_371 )
{
return F_52 ( V_1 ) ;
}
static T_2
F_147 ( struct V_9 * V_10 , struct V_1 * V_282 )
{
struct V_11 * V_12 = V_10 -> V_13 ;
struct V_362 * V_372 , * V_373 ;
if ( V_282 -> V_364 ) {
V_372 = & V_12 -> V_365 [ 0 ] ;
V_373 = & V_12 -> V_365 [ 1 ] ;
} else {
V_372 = & V_12 -> V_365 [ 1 ] ;
V_373 = & V_12 -> V_365 [ 0 ] ;
}
if ( V_372 -> V_25 )
return V_372 -> V_25 ;
if ( V_373 -> V_25 ) {
if ( V_373 -> V_25 == 0x70 )
return 0x72 ;
else
return 0x70 ;
}
if ( V_282 -> V_364 )
return 0x70 ;
else
return 0x72 ;
}
static void
F_148 ( struct V_237 * V_237 )
{
struct V_5 * V_5 ;
struct V_1 * V_374 ;
V_5 = & V_237 -> V_4 ;
V_374 = F_3 ( & V_237 -> V_4 ) ;
F_149 ( & V_5 -> V_375 -> V_376 ,
V_374 -> V_283 . V_10 . V_376 . V_37 ) ;
F_150 ( V_237 ) ;
}
static int
F_151 ( struct V_7 * V_6 ,
struct V_1 * V_3 )
{
struct V_5 * V_5 ;
int V_30 ;
V_5 = & V_6 -> V_4 . V_4 ;
V_30 = F_152 ( V_3 -> V_4 . V_4 . V_10 ,
V_5 ,
& V_377 ,
V_6 -> V_4 . V_4 . V_378 ) ;
if ( V_30 < 0 )
return V_30 ;
F_153 ( V_5 ,
& V_379 ) ;
V_6 -> V_4 . V_4 . V_380 = 1 ;
V_6 -> V_4 . V_4 . V_381 = 0 ;
V_6 -> V_4 . V_4 . V_382 . V_383 = V_384 ;
V_6 -> V_4 . V_385 = F_80 ;
V_6 -> V_4 . V_386 = F_148 ;
F_154 ( & V_6 -> V_4 , & V_3 -> V_4 ) ;
V_30 = F_155 ( V_5 ) ;
if ( V_30 < 0 )
goto V_387;
V_30 = F_156 ( & V_5 -> V_375 -> V_376 ,
& V_3 -> V_283 . V_10 . V_376 ,
V_3 -> V_283 . V_10 . V_376 . V_37 ) ;
if ( V_30 < 0 )
goto V_388;
return 0 ;
V_388:
F_157 ( V_5 ) ;
V_387:
F_128 ( V_5 ) ;
return V_30 ;
}
static void
F_158 ( struct V_1 * V_1 ,
struct V_7 * V_6 )
{
struct V_9 * V_10 = V_6 -> V_4 . V_4 . V_10 ;
F_159 ( & V_6 -> V_4 . V_4 ) ;
if ( F_73 ( V_10 ) -> V_224 >= 4 && F_160 ( V_10 ) ) {
F_161 ( & V_6 -> V_4 . V_4 ) ;
V_1 -> V_205 = true ;
}
F_162 ( & V_6 -> V_4 . V_4 ) ;
V_1 -> V_210 = V_330 ;
}
static struct V_7 * F_163 ( void )
{
struct V_7 * V_389 ;
V_389 = F_22 ( sizeof( * V_389 ) , V_39 ) ;
if ( ! V_389 )
return NULL ;
if ( F_164 ( & V_389 -> V_4 ) < 0 ) {
F_24 ( V_389 ) ;
return NULL ;
}
return V_389 ;
}
static bool
F_165 ( struct V_1 * V_1 , int V_371 )
{
struct V_352 * V_3 = & V_1 -> V_4 . V_4 ;
struct V_5 * V_6 ;
struct V_2 * V_2 = F_136 ( V_3 ) ;
struct V_237 * V_237 ;
struct V_7 * V_7 ;
F_14 ( L_40 , V_371 ) ;
V_7 = F_163 () ;
if ( ! V_7 )
return false ;
if ( V_371 == 0 ) {
V_1 -> V_355 |= V_359 ;
V_7 -> V_239 = V_359 ;
} else if ( V_371 == 1 ) {
V_1 -> V_355 |= V_358 ;
V_7 -> V_239 = V_358 ;
}
V_237 = & V_7 -> V_4 ;
V_6 = & V_237 -> V_4 ;
if ( F_97 ( V_1 ) &
V_7 -> V_239 ) {
V_1 -> V_280 |= V_7 -> V_239 ;
V_2 -> V_390 = F_99 ;
F_99 ( V_2 ) ;
} else {
V_237 -> V_391 = V_392 | V_393 ;
}
V_3 -> V_394 = V_395 ;
V_6 -> V_378 = V_396 ;
if ( F_146 ( V_1 , V_371 ) ) {
V_6 -> V_378 = V_397 ;
V_1 -> V_208 = true ;
}
if ( F_151 ( V_7 , V_1 ) < 0 ) {
F_24 ( V_7 ) ;
return false ;
}
if ( V_1 -> V_208 )
F_158 ( V_1 , V_7 ) ;
return true ;
}
static bool
F_166 ( struct V_1 * V_1 , int type )
{
struct V_352 * V_3 = & V_1 -> V_4 . V_4 ;
struct V_5 * V_6 ;
struct V_237 * V_237 ;
struct V_7 * V_7 ;
F_14 ( L_41 , type ) ;
V_7 = F_163 () ;
if ( ! V_7 )
return false ;
V_237 = & V_7 -> V_4 ;
V_6 = & V_237 -> V_4 ;
V_3 -> V_394 = V_398 ;
V_6 -> V_378 = V_399 ;
V_1 -> V_355 |= type ;
V_7 -> V_239 = type ;
V_1 -> V_200 = true ;
if ( F_151 ( V_7 , V_1 ) < 0 ) {
F_24 ( V_7 ) ;
return false ;
}
if ( ! F_167 ( V_1 , V_7 , type ) )
goto V_400;
if ( ! F_168 ( V_1 , V_7 ) )
goto V_400;
return true ;
V_400:
F_157 ( V_6 ) ;
F_127 ( V_6 ) ;
return false ;
}
static bool
F_169 ( struct V_1 * V_1 , int V_371 )
{
struct V_352 * V_3 = & V_1 -> V_4 . V_4 ;
struct V_5 * V_6 ;
struct V_237 * V_237 ;
struct V_7 * V_7 ;
F_14 ( L_42 , V_371 ) ;
V_7 = F_163 () ;
if ( ! V_7 )
return false ;
V_237 = & V_7 -> V_4 ;
V_6 = & V_237 -> V_4 ;
V_237 -> V_391 = V_392 ;
V_3 -> V_394 = V_401 ;
V_6 -> V_378 = V_402 ;
if ( V_371 == 0 ) {
V_1 -> V_355 |= V_361 ;
V_7 -> V_239 = V_361 ;
} else if ( V_371 == 1 ) {
V_1 -> V_355 |= V_360 ;
V_7 -> V_239 = V_360 ;
}
if ( F_151 ( V_7 , V_1 ) < 0 ) {
F_24 ( V_7 ) ;
return false ;
}
return true ;
}
static bool
F_170 ( struct V_1 * V_1 , int V_371 )
{
struct V_352 * V_3 = & V_1 -> V_4 . V_4 ;
struct V_5 * V_6 ;
struct V_237 * V_237 ;
struct V_7 * V_7 ;
F_14 ( L_43 , V_371 ) ;
V_7 = F_163 () ;
if ( ! V_7 )
return false ;
V_237 = & V_7 -> V_4 ;
V_6 = & V_237 -> V_4 ;
V_3 -> V_394 = V_403 ;
V_6 -> V_378 = V_404 ;
if ( V_371 == 0 ) {
V_1 -> V_355 |= V_357 ;
V_7 -> V_239 = V_357 ;
} else if ( V_371 == 1 ) {
V_1 -> V_355 |= V_356 ;
V_7 -> V_239 = V_356 ;
}
if ( F_151 ( V_7 , V_1 ) < 0 ) {
F_24 ( V_7 ) ;
return false ;
}
if ( ! F_168 ( V_1 , V_7 ) )
goto V_400;
return true ;
V_400:
F_157 ( V_6 ) ;
F_127 ( V_6 ) ;
return false ;
}
static bool
F_171 ( struct V_1 * V_1 , T_4 V_26 )
{
V_1 -> V_200 = false ;
V_1 -> V_103 = false ;
if ( V_26 & V_359 )
if ( ! F_165 ( V_1 , 0 ) )
return false ;
if ( ( V_26 & V_405 ) == V_405 )
if ( ! F_165 ( V_1 , 1 ) )
return false ;
if ( V_26 & V_406 )
if ( ! F_166 ( V_1 , V_406 ) )
return false ;
if ( V_26 & V_407 )
if ( ! F_166 ( V_1 , V_407 ) )
return false ;
if ( V_26 & V_408 )
if ( ! F_166 ( V_1 , V_408 ) )
return false ;
if ( V_26 & V_361 )
if ( ! F_169 ( V_1 , 0 ) )
return false ;
if ( ( V_26 & V_409 ) == V_409 )
if ( ! F_169 ( V_1 , 1 ) )
return false ;
if ( V_26 & V_357 )
if ( ! F_170 ( V_1 , 0 ) )
return false ;
if ( ( V_26 & V_302 ) == V_302 )
if ( ! F_170 ( V_1 , 1 ) )
return false ;
if ( ( V_26 & V_410 ) == 0 ) {
unsigned char V_411 [ 2 ] ;
V_1 -> V_355 = 0 ;
memcpy ( V_411 , & V_1 -> V_263 . V_276 , 2 ) ;
F_14 ( L_44 ,
F_20 ( V_1 ) ,
V_411 [ 0 ] , V_411 [ 1 ] ) ;
return false ;
}
V_1 -> V_4 . V_412 = ( 1 << 0 ) | ( 1 << 1 ) | ( 1 << 2 ) ;
return true ;
}
static void F_172 ( struct V_1 * V_1 )
{
struct V_9 * V_10 = V_1 -> V_4 . V_4 . V_10 ;
struct V_5 * V_6 , * V_156 ;
F_173 (connector, tmp,
&dev->mode_config.connector_list, head) {
if ( F_4 ( V_6 ) == & V_1 -> V_4 ) {
F_157 ( V_6 ) ;
F_127 ( V_6 ) ;
}
}
}
static bool F_167 ( struct V_1 * V_1 ,
struct V_7 * V_7 ,
int type )
{
struct V_9 * V_10 = V_1 -> V_4 . V_4 . V_10 ;
struct V_177 V_178 ;
T_7 V_179 , V_16 ;
if ( ! F_39 ( V_1 , type ) )
return false ;
F_34 ( sizeof( V_178 ) != 6 ) ;
if ( ! F_31 ( V_1 ,
V_413 ,
& V_178 , sizeof( V_178 ) ) )
return false ;
memcpy ( & V_179 , & V_178 , V_89 ( sizeof( V_179 ) , sizeof( V_178 ) ) ) ;
if ( V_179 == 0 )
return false ;
V_7 -> V_414 = 0 ;
for ( V_16 = 0 ; V_16 < V_336 ; V_16 ++ )
if ( V_179 & ( 1 << V_16 ) )
V_7 -> V_337 [ V_7 -> V_414 ++ ] = V_16 ;
V_7 -> V_335 =
F_174 ( V_10 , V_415 ,
L_45 , V_7 -> V_414 ) ;
if ( ! V_7 -> V_335 )
return false ;
for ( V_16 = 0 ; V_16 < V_7 -> V_414 ; V_16 ++ )
F_175 (
V_7 -> V_335 , V_16 ,
V_16 , V_416 [ V_7 -> V_337 [ V_16 ] ] ) ;
V_1 -> V_180 = V_7 -> V_337 [ 0 ] ;
F_176 ( & V_7 -> V_4 . V_4 . V_4 ,
V_7 -> V_335 , 0 ) ;
return true ;
}
static bool
F_177 ( struct V_1 * V_1 ,
struct V_7 * V_7 ,
struct V_417 V_418 )
{
struct V_9 * V_10 = V_1 -> V_4 . V_4 . V_10 ;
struct V_5 * V_6 = & V_7 -> V_4 . V_4 ;
T_4 V_44 , V_419 [ 2 ] ;
if ( V_418 . V_420 ) {
if ( ! F_31 ( V_1 ,
V_421 ,
& V_419 , 4 ) )
return false ;
if ( ! F_31 ( V_1 ,
V_422 ,
& V_44 , 2 ) )
return false ;
V_7 -> V_342 = V_419 [ 0 ] ;
V_7 -> V_340 = V_419 [ 0 ] - V_44 ;
V_7 -> V_341 = V_7 -> V_340 ;
V_7 -> V_338 =
F_178 ( V_10 , 0 , L_46 , 0 , V_419 [ 0 ] ) ;
if ( ! V_7 -> V_338 )
return false ;
F_176 ( & V_6 -> V_4 ,
V_7 -> V_338 ,
V_7 -> V_340 ) ;
V_7 -> V_339 =
F_178 ( V_10 , 0 , L_47 , 0 , V_419 [ 0 ] ) ;
if ( ! V_7 -> V_339 )
return false ;
F_176 ( & V_6 -> V_4 ,
V_7 -> V_339 ,
V_7 -> V_341 ) ;
F_14 ( L_48
L_49 ,
V_419 [ 0 ] , V_419 [ 1 ] , V_44 ) ;
}
if ( V_418 . V_423 ) {
if ( ! F_31 ( V_1 ,
V_424 ,
& V_419 , 4 ) )
return false ;
if ( ! F_31 ( V_1 ,
V_425 ,
& V_44 , 2 ) )
return false ;
V_7 -> V_349 = V_419 [ 0 ] ;
V_7 -> V_347 = V_419 [ 0 ] - V_44 ;
V_7 -> V_348 = V_7 -> V_347 ;
V_7 -> V_345 =
F_178 ( V_10 , 0 ,
L_50 , 0 , V_419 [ 0 ] ) ;
if ( ! V_7 -> V_345 )
return false ;
F_176 ( & V_6 -> V_4 ,
V_7 -> V_345 ,
V_7 -> V_347 ) ;
V_7 -> V_346 =
F_178 ( V_10 , 0 ,
L_51 , 0 , V_419 [ 0 ] ) ;
if ( ! V_7 -> V_346 )
return false ;
F_176 ( & V_6 -> V_4 ,
V_7 -> V_346 ,
V_7 -> V_348 ) ;
F_14 ( L_52
L_49 ,
V_419 [ 0 ] , V_419 [ 1 ] , V_44 ) ;
}
F_179 ( V_426 , V_427 ) ;
F_179 ( V_428 , V_429 ) ;
F_179 ( V_430 , V_431 ) ;
F_179 ( V_432 , V_433 ) ;
F_179 ( V_434 , V_435 ) ;
F_179 ( V_436 , V_437 ) ;
F_179 ( V_438 , V_439 ) ;
F_179 ( V_440 , V_441 ) ;
F_179 ( V_442 , V_443 ) ;
F_179 ( V_444 , V_445 ) ;
F_179 ( V_446 , V_447 ) ;
F_179 ( V_448 , V_449 ) ;
if ( V_418 . V_450 ) {
if ( ! F_31 ( V_1 , V_451 , & V_44 , 2 ) )
return false ;
V_7 -> V_452 = 1 ;
V_7 -> V_453 = V_44 & 0x1 ;
V_7 -> V_450 =
F_178 ( V_10 , 0 , L_53 , 0 , 1 ) ;
if ( ! V_7 -> V_450 )
return false ;
F_176 ( & V_6 -> V_4 ,
V_7 -> V_450 ,
V_7 -> V_453 ) ;
F_14 ( L_54 , V_44 ) ;
}
return true ;
}
static bool
F_180 ( struct V_1 * V_1 ,
struct V_7 * V_7 ,
struct V_417 V_418 )
{
struct V_9 * V_10 = V_1 -> V_4 . V_4 . V_10 ;
struct V_5 * V_6 = & V_7 -> V_4 . V_4 ;
T_4 V_44 , V_419 [ 2 ] ;
F_179 ( V_438 , V_439 ) ;
return true ;
}
static bool F_168 ( struct V_1 * V_1 ,
struct V_7 * V_7 )
{
union {
struct V_417 V_305 ;
T_4 V_44 ;
} V_418 ;
F_34 ( sizeof( V_418 ) != 2 ) ;
V_418 . V_44 = 0 ;
F_31 ( V_1 ,
V_454 ,
& V_418 , sizeof( V_418 ) ) ;
if ( V_418 . V_44 == 0 ) {
F_14 ( L_55 ) ;
return true ;
}
if ( F_124 ( V_7 ) )
return F_177 ( V_1 , V_7 , V_418 . V_305 ) ;
else if ( F_125 ( V_7 ) )
return F_180 ( V_1 , V_7 , V_418 . V_305 ) ;
else
return true ;
}
static int F_181 ( struct V_455 * V_456 ,
struct V_23 * V_24 ,
int V_457 )
{
struct V_1 * V_282 = V_456 -> V_458 ;
if ( ! F_29 ( V_282 , V_282 -> V_57 ) )
return - V_351 ;
return V_282 -> V_31 -> V_459 -> V_460 ( V_282 -> V_31 , V_24 , V_457 ) ;
}
static T_1 F_182 ( struct V_455 * V_456 )
{
struct V_1 * V_282 = V_456 -> V_458 ;
return V_282 -> V_31 -> V_459 -> V_461 ( V_282 -> V_31 ) ;
}
static bool
F_183 ( struct V_1 * V_282 ,
struct V_9 * V_10 )
{
V_282 -> V_283 . V_462 = V_463 ;
V_282 -> V_283 . V_464 = V_465 ;
snprintf ( V_282 -> V_283 . V_37 , V_466 , L_56 ) ;
V_282 -> V_283 . V_10 . V_467 = & V_10 -> V_468 -> V_10 ;
V_282 -> V_283 . V_458 = V_282 ;
V_282 -> V_283 . V_459 = & V_469 ;
return F_184 ( & V_282 -> V_283 ) == 0 ;
}
bool F_185 ( struct V_9 * V_10 , T_7 V_17 , bool V_364 )
{
struct V_11 * V_12 = V_10 -> V_13 ;
struct V_2 * V_2 ;
struct V_1 * V_1 ;
int V_16 ;
V_1 = F_22 ( sizeof( * V_1 ) , V_39 ) ;
if ( ! V_1 )
return false ;
V_1 -> V_17 = V_17 ;
V_1 -> V_364 = V_364 ;
V_1 -> V_25 = F_147 ( V_10 , V_1 ) >> 1 ;
F_142 ( V_12 , V_1 ) ;
if ( ! F_183 ( V_1 , V_10 ) )
goto V_470;
V_2 = & V_1 -> V_4 ;
V_2 -> type = V_471 ;
F_186 ( V_10 , & V_2 -> V_4 , & V_472 , 0 ) ;
for ( V_16 = 0 ; V_16 < 0x40 ; V_16 ++ ) {
T_2 V_473 ;
if ( ! F_12 ( V_1 , V_16 , & V_473 ) ) {
F_14 ( L_57 ,
F_20 ( V_1 ) ) ;
goto V_400;
}
}
V_2 -> V_474 = F_68 ;
if ( F_69 ( V_10 ) ) {
V_2 -> V_475 = F_91 ;
V_2 -> V_476 = F_92 ;
} else {
V_2 -> V_475 = F_90 ;
}
V_2 -> V_477 = F_71 ;
V_2 -> V_478 = F_93 ;
V_2 -> V_385 = F_81 ;
V_2 -> V_479 = F_85 ;
if ( ! F_96 ( V_1 , & V_1 -> V_263 ) )
goto V_400;
if ( F_171 ( V_1 ,
V_1 -> V_263 . V_276 ) != true ) {
F_14 ( L_58 ,
F_20 ( V_1 ) ) ;
goto V_480;
}
if ( V_1 -> V_280 ) {
V_2 -> V_481 =
V_1 -> V_364 ? V_482 : V_483 ;
}
V_1 -> V_4 . V_484 = 0 ;
F_141 ( V_12 , V_1 ) ;
if ( ! F_32 ( V_1 ) )
goto V_480;
if ( ! F_38 ( V_1 ,
& V_1 -> V_256 ,
& V_1 -> V_258 ) )
goto V_480;
F_14 ( L_59
L_60
L_61
L_62 ,
F_20 ( V_1 ) ,
V_1 -> V_263 . V_265 , V_1 -> V_263 . V_266 ,
V_1 -> V_263 . V_267 ,
V_1 -> V_256 / 1000 ,
V_1 -> V_258 / 1000 ,
( V_1 -> V_263 . V_270 & 0x1 ) ? 'Y' : 'N' ,
( V_1 -> V_263 . V_270 & 0x2 ) ? 'Y' : 'N' ,
V_1 -> V_263 . V_276 &
( V_359 | V_361 ) ? 'Y' : 'N' ,
V_1 -> V_263 . V_276 &
( V_358 | V_360 ) ? 'Y' : 'N' ) ;
return true ;
V_480:
F_172 ( V_1 ) ;
V_400:
F_187 ( & V_2 -> V_4 ) ;
F_138 ( & V_1 -> V_283 ) ;
V_470:
F_145 ( V_1 ) ;
F_24 ( V_1 ) ;
return false ;
}
