static T_1 F_1 ( T_1 V_1 )
{
T_1 V_2 ;
V_2 = F_2 ( V_1 ) - 1 ;
return V_2 ;
}
T_2 F_3 ( struct V_3 * V_4 , T_1 V_5 )
{
T_2 V_6 = 1 ;
struct V_7 * V_8 = F_4 ( V_4 ) ;
if ( V_8 -> V_9 == V_10 ) {
V_6 = 0 ;
} else if ( V_8 -> V_9 == V_11 ) {
if ( V_5 == V_12 || V_5 == V_13 )
V_6 = 1 ;
else if ( V_5 == V_14 || V_5 == V_15 )
V_6 = 0 ;
}
return V_6 ;
}
void F_5 ( struct V_3 * V_4 , T_1 V_16 , T_1 V_1 ,
T_1 V_17 )
{
T_1 V_18 , V_19 ;
if ( V_1 != V_20 ) {
F_6 ( V_4 , V_16 , & V_18 ) ;
V_19 = F_1 ( V_1 ) ;
V_18 &= ~ V_1 ;
V_18 |= V_17 << V_19 ;
F_7 ( V_4 , V_16 , V_18 ) ;
} else {
F_7 ( V_4 , V_16 , V_17 ) ;
}
}
T_1 F_8 ( struct V_3 * V_4 , T_1 V_16 , T_1 V_1 )
{
T_1 V_18 , V_19 ;
F_6 ( V_4 , V_16 , & V_18 ) ;
V_19 = F_1 ( V_1 ) ;
return ( V_18 & V_1 ) >> V_19 ;
}
static T_1 F_9 ( struct V_3 * V_4 ,
T_3 V_5 , T_1 V_21 )
{
struct V_7 * V_8 = F_4 ( V_4 ) ;
T_1 V_6 = 0 ;
T_1 V_22 = 0 ;
T_4 * V_23 = & V_8 -> V_24 [ V_5 ] ;
F_5 ( V_4 , V_23 -> V_25 , V_26 , 0 ) ;
V_21 &= 0x3f ;
if ( V_8 -> V_27 == V_28 ) {
if ( V_21 >= 31 ) {
V_8 -> V_29 [ V_5 ] |= 0x140 ;
F_5 ( V_4 , V_23 -> V_30 ,
V_20 ,
V_8 -> V_29 [ V_5 ] << 16 ) ;
V_22 = V_21 - 30 ;
} else if ( V_21 >= 16 ) {
V_8 -> V_29 [ V_5 ] |= 0x100 ;
V_8 -> V_29 [ V_5 ] &= ( ~ 0x40 ) ;
F_5 ( V_4 , V_23 -> V_30 ,
V_20 ,
V_8 -> V_29 [ V_5 ] << 16 ) ;
V_22 = V_21 - 15 ;
} else {
V_22 = V_21 ;
}
} else {
F_10 ( ( V_31 | V_32 ) ,
L_1 ) ;
V_22 = V_21 ;
}
F_5 ( V_4 , V_23 -> V_33 , V_34 ,
V_22 ) ;
F_5 ( V_4 , V_23 -> V_33 , V_35 , 0x0 ) ;
F_5 ( V_4 , V_23 -> V_33 , V_35 , 0x1 ) ;
F_11 ( 1000 , 1000 ) ;
V_6 = F_8 ( V_4 , V_23 -> V_25 ,
V_26 ) ;
if ( V_8 -> V_27 == V_28 ) {
V_8 -> V_29 [ V_5 ] &= 0xebf ;
F_5 ( V_4 , V_23 -> V_30 , V_20 ,
V_8 -> V_29 [ V_5 ] << 16 ) ;
}
return V_6 ;
}
static void F_12 ( struct V_3 * V_4 ,
T_3 V_5 , T_1 V_21 ,
T_1 V_17 )
{
struct V_7 * V_8 = F_4 ( V_4 ) ;
T_1 V_36 = 0 , V_22 = 0 ;
T_4 * V_23 = & V_8 -> V_24 [ V_5 ] ;
V_21 &= 0x3f ;
if ( V_8 -> V_27 == V_28 ) {
if ( V_21 >= 31 ) {
V_8 -> V_29 [ V_5 ] |= 0x140 ;
F_5 ( V_4 , V_23 -> V_30 ,
V_20 ,
V_8 -> V_29 [ V_5 ] << 16 ) ;
V_22 = V_21 - 30 ;
} else if ( V_21 >= 16 ) {
V_8 -> V_29 [ V_5 ] |= 0x100 ;
V_8 -> V_29 [ V_5 ] &= ( ~ 0x40 ) ;
F_5 ( V_4 , V_23 -> V_30 ,
V_20 ,
V_8 -> V_29 [ V_5 ] << 16 ) ;
V_22 = V_21 - 15 ;
} else {
V_22 = V_21 ;
}
} else {
F_10 ( ( V_31 | V_32 ) ,
L_1 ) ;
V_22 = V_21 ;
}
V_36 = ( V_17 << 16 ) | ( V_22 & 0x3f ) ;
F_5 ( V_4 , V_23 -> V_30 , V_20 , V_36 ) ;
if ( V_21 == 0x0 )
V_8 -> V_29 [ V_5 ] = V_17 ;
if ( V_8 -> V_27 == V_28 ) {
if ( V_21 != 0 ) {
V_8 -> V_29 [ V_5 ] &= 0xebf ;
F_5 ( V_4 , V_23 -> V_30 ,
V_20 ,
V_8 -> V_29 [ V_5 ] << 16 ) ;
}
}
}
void F_13 ( struct V_3 * V_4 , T_3 V_5 ,
T_1 V_16 , T_1 V_1 , T_1 V_17 )
{
struct V_7 * V_8 = F_4 ( V_4 ) ;
T_1 V_18 , V_19 ;
if ( ! F_3 ( V_4 , V_5 ) )
return;
if ( V_8 -> V_37 == V_38 ) {
if ( V_1 != V_39 ) {
V_18 = F_14 ( V_4 , V_5 , V_16 ) ;
V_19 = F_1 ( V_1 ) ;
V_18 &= ~ V_1 ;
V_18 |= V_17 << V_19 ;
F_15 ( V_4 , V_5 , V_16 , V_18 ) ;
} else {
F_15 ( V_4 , V_5 , V_16 , V_17 ) ;
}
F_16 ( 200 ) ;
} else {
if ( V_1 != V_39 ) {
V_18 = F_9 ( V_4 , V_5 , V_16 ) ;
V_19 = F_1 ( V_1 ) ;
V_18 &= ~ V_1 ;
V_18 |= V_17 << V_19 ;
F_12 ( V_4 , V_5 , V_16 , V_18 ) ;
} else {
F_12 ( V_4 , V_5 , V_16 , V_17 ) ;
}
}
}
T_1 F_17 ( struct V_3 * V_4 , T_3 V_5 ,
T_1 V_16 , T_1 V_1 )
{
T_1 V_18 , V_19 ;
struct V_7 * V_8 = F_4 ( V_4 ) ;
if ( ! F_3 ( V_4 , V_5 ) )
return 0 ;
if ( V_8 -> V_37 == V_38 ) {
V_18 = F_14 ( V_4 , V_5 , V_16 ) ;
F_16 ( 200 ) ;
} else {
V_18 = F_9 ( V_4 , V_5 , V_16 ) ;
}
V_19 = F_1 ( V_1 ) ;
V_18 = ( V_18 & V_1 ) >> V_19 ;
return V_18 ;
}
static T_1 F_14 ( struct V_3 * V_4 , T_3 V_5 ,
T_1 V_21 )
{
T_1 V_18 = 0 ;
T_1 V_17 = 0 ;
T_2 time = 0 ;
T_1 V_40 ;
V_17 |= ( ( V_21 & 0xFF ) << 12 ) ;
V_17 |= ( ( V_5 & 0x3 ) << 20 ) ;
V_17 |= 0x80000000 ;
F_6 ( V_4 , V_41 , & V_40 ) ;
while ( V_40 & 0x80000000 ) {
if ( time ++ < 100 ) {
F_16 ( 10 ) ;
F_6 ( V_4 , V_41 , & V_40 ) ;
} else {
break;
}
}
F_7 ( V_4 , V_41 , V_17 ) ;
F_6 ( V_4 , V_41 , & V_40 ) ;
while ( V_40 & 0x80000000 ) {
if ( time ++ < 100 ) {
F_16 ( 10 ) ;
F_6 ( V_4 , V_41 , & V_40 ) ;
} else {
return 0 ;
}
}
F_6 ( V_4 , V_42 , & V_18 ) ;
return V_18 ;
}
static void F_15 ( struct V_3 * V_4 ,
T_3 V_5 , T_1 V_21 , T_1 V_17 )
{
T_2 time = 0 ;
T_1 V_40 ;
V_17 |= ( ( V_21 & 0xFF ) << 12 ) ;
V_17 |= ( ( V_5 & 0x3 ) << 20 ) ;
V_17 |= 0x400000 ;
V_17 |= 0x80000000 ;
F_6 ( V_4 , V_41 , & V_40 ) ;
while ( V_40 & 0x80000000 ) {
if ( time ++ < 100 ) {
F_16 ( 10 ) ;
F_6 ( V_4 , V_41 , & V_40 ) ;
} else {
break;
}
}
F_7 ( V_4 , V_41 , V_17 ) ;
}
void F_18 ( struct V_3 * V_4 )
{
T_1 V_43 = 0 , V_2 ;
T_1 * V_44 = NULL ;
struct V_7 * V_8 = F_4 ( V_4 ) ;
if ( V_8 -> V_45 ) {
F_10 ( V_31 , L_2 ) ;
V_43 = V_46 ;
V_44 = V_47 ;
} else {
F_10 ( V_31 , L_3 ) ;
V_43 = V_48 ;
V_44 = V_49 ;
}
for ( V_2 = 0 ; V_2 < V_43 ; V_2 = V_2 + 3 ) {
if ( V_44 [ V_2 ] == 0x318 )
V_44 [ V_2 + 2 ] = 0x00000800 ;
F_10 ( V_50 ,
L_4 ,
V_44 [ V_2 ] , V_44 [ V_2 + 1 ] , V_44 [ V_2 + 2 ] ) ;
F_5 ( V_4 , V_44 [ V_2 ] , V_44 [ V_2 + 1 ] ,
V_44 [ V_2 + 2 ] ) ;
}
}
void F_19 ( struct V_3 * V_4 , T_2 V_51 )
{
T_1 V_2 ;
#ifdef F_20
T_1 * V_52 = NULL , * V_53 = NULL ;
if ( V_54 -> V_55 ) {
V_56 = V_57 ;
V_58 = V_59 ;
V_60 = V_61 ;
V_62 = V_63 ;
}
#endif
if ( V_51 == V_64 ) {
for ( V_2 = 0 ; V_2 < V_65 ; V_2 += 2 ) {
F_5 ( V_4 , V_66 [ V_2 ] ,
V_20 ,
V_66 [ V_2 + 1 ] ) ;
F_10 ( V_50 ,
L_5 ,
V_2 , V_66 [ V_2 ] ,
V_66 [ V_2 + 1 ] ) ;
}
} else if ( V_51 == V_67 ) {
for ( V_2 = 0 ; V_2 < V_68 ; V_2 += 2 ) {
F_5 ( V_4 , V_69 [ V_2 ] ,
V_20 , V_69 [ V_2 + 1 ] ) ;
F_10 ( V_50 ,
L_6 ,
V_2 , V_69 [ V_2 ] ,
V_69 [ V_2 + 1 ] ) ;
}
}
}
static void F_21 ( struct V_3 * V_4 )
{
struct V_7 * V_8 = F_4 ( V_4 ) ;
V_8 -> V_24 [ V_12 ] . V_70 = V_71 ;
V_8 -> V_24 [ V_13 ] . V_70 = V_71 ;
V_8 -> V_24 [ V_14 ] . V_70 = V_72 ;
V_8 -> V_24 [ V_15 ] . V_70 = V_72 ;
V_8 -> V_24 [ V_12 ] . V_73 = V_74 ;
V_8 -> V_24 [ V_13 ] . V_73 = V_74 ;
V_8 -> V_24 [ V_14 ] . V_73 = V_75 ;
V_8 -> V_24 [ V_15 ] . V_73 = V_75 ;
V_8 -> V_24 [ V_12 ] . V_76 = V_77 ;
V_8 -> V_24 [ V_13 ] . V_76 = V_78 ;
V_8 -> V_24 [ V_14 ] . V_76 = V_79 ;
V_8 -> V_24 [ V_15 ] . V_76 = V_80 ;
V_8 -> V_24 [ V_12 ] . V_81 = V_77 ;
V_8 -> V_24 [ V_13 ] . V_81 = V_78 ;
V_8 -> V_24 [ V_14 ] . V_81 = V_79 ;
V_8 -> V_24 [ V_15 ] . V_81 = V_80 ;
V_8 -> V_24 [ V_12 ] . V_30 = V_82 ;
V_8 -> V_24 [ V_13 ] . V_30 = V_83 ;
V_8 -> V_24 [ V_14 ] . V_30 = V_84 ;
V_8 -> V_24 [ V_15 ] . V_30 = V_85 ;
V_8 -> V_24 [ V_12 ] . V_86 = V_87 ;
V_8 -> V_24 [ V_13 ] . V_86 = V_87 ;
V_8 -> V_24 [ V_14 ] . V_86 = V_88 ;
V_8 -> V_24 [ V_15 ] . V_86 = V_88 ;
V_8 -> V_24 [ V_12 ] . V_89 = V_90 ;
V_8 -> V_24 [ V_13 ] . V_89 = V_90 ;
V_8 -> V_24 [ V_14 ] . V_89 = V_90 ;
V_8 -> V_24 [ V_15 ] . V_89 = V_90 ;
V_8 -> V_24 [ V_12 ] . V_91 = V_92 ;
V_8 -> V_24 [ V_13 ] . V_91 = V_93 ;
V_8 -> V_24 [ V_14 ] . V_91 = V_94 ;
V_8 -> V_24 [ V_15 ] . V_91 = V_95 ;
V_8 -> V_24 [ V_12 ] . V_33 = V_96 ;
V_8 -> V_24 [ V_13 ] . V_33 = V_97 ;
V_8 -> V_24 [ V_14 ] . V_33 = V_98 ;
V_8 -> V_24 [ V_15 ] . V_33 = V_99 ;
V_8 -> V_24 [ V_12 ] . V_100 = V_101 ;
V_8 -> V_24 [ V_13 ] . V_100 = V_101 ;
V_8 -> V_24 [ V_14 ] . V_100 = V_102 ;
V_8 -> V_24 [ V_15 ] . V_100 = V_102 ;
V_8 -> V_24 [ V_12 ] . V_103 = V_104 ;
V_8 -> V_24 [ V_13 ] . V_103 = V_105 ;
V_8 -> V_24 [ V_14 ] . V_103 = V_106 ;
V_8 -> V_24 [ V_15 ] . V_103 = V_107 ;
V_8 -> V_24 [ V_12 ] . V_108 = V_109 ;
V_8 -> V_24 [ V_13 ] . V_108 = V_110 ;
V_8 -> V_24 [ V_14 ] . V_108 = V_111 ;
V_8 -> V_24 [ V_15 ] . V_108 = V_112 ;
V_8 -> V_24 [ V_12 ] . V_113 = V_114 ;
V_8 -> V_24 [ V_13 ] . V_113 = V_115 ;
V_8 -> V_24 [ V_14 ] . V_113 = V_116 ;
V_8 -> V_24 [ V_15 ] . V_113 = V_117 ;
V_8 -> V_24 [ V_12 ] . V_118 = V_119 ;
V_8 -> V_24 [ V_13 ] . V_118 = V_120 ;
V_8 -> V_24 [ V_14 ] . V_118 = V_121 ;
V_8 -> V_24 [ V_15 ] . V_118 = V_122 ;
V_8 -> V_24 [ V_12 ] . V_123 = V_124 ;
V_8 -> V_24 [ V_13 ] . V_123 = V_125 ;
V_8 -> V_24 [ V_14 ] . V_123 = V_126 ;
V_8 -> V_24 [ V_15 ] . V_123 = V_127 ;
V_8 -> V_24 [ V_12 ] . V_128 = V_129 ;
V_8 -> V_24 [ V_13 ] . V_128 = V_130 ;
V_8 -> V_24 [ V_14 ] . V_128 = V_131 ;
V_8 -> V_24 [ V_15 ] . V_128 = V_132 ;
V_8 -> V_24 [ V_12 ] . V_25 = V_133 ;
V_8 -> V_24 [ V_13 ] . V_25 = V_134 ;
V_8 -> V_24 [ V_14 ] . V_25 = V_135 ;
V_8 -> V_24 [ V_15 ] . V_25 = V_136 ;
}
T_2 F_22 ( struct V_3 * V_4 , T_5 V_137 ,
T_3 V_5 )
{
T_2 V_6 = 0 ;
T_1 V_2 , V_138 = 4 , V_18 = 0 ;
T_1 V_139 [ 4 ] ;
T_1 V_140 [] = { 0xfffff027 , 0xaa55a02f , 0x00000027 , 0x55aa502f } ;
V_139 [ V_141 ] = 0x100 ;
V_139 [ V_142 ] = 0x900 ;
V_139 [ V_143 ] = 0x800 ;
V_139 [ V_144 ] = 0x3 ;
F_10 ( V_31 , L_7 , V_145 , V_137 ) ;
for ( V_2 = 0 ; V_2 < V_138 ; V_2 ++ ) {
switch ( V_137 ) {
case V_141 :
F_10 ( V_32 ,
L_8 ) ;
break;
case V_142 :
case V_143 :
F_7 ( V_4 , V_139 [ V_137 ] ,
V_140 [ V_2 ] ) ;
F_6 ( V_4 , V_139 [ V_137 ] , & V_18 ) ;
break;
case V_144 :
V_140 [ V_2 ] &= 0xfff ;
F_13 ( V_4 , V_5 ,
V_139 [ V_144 ] ,
V_39 , V_140 [ V_2 ] ) ;
F_11 ( 1000 , 1000 ) ;
V_18 = F_17 ( V_4 , V_5 ,
V_139 [ V_144 ] ,
V_39 ) ;
F_11 ( 1000 , 1000 ) ;
break;
default:
V_6 = 1 ;
break;
}
if ( V_18 != V_140 [ V_2 ] ) {
F_10 ( ( V_31 | V_32 ) ,
L_9 ,
V_18 , V_140 [ V_2 ] ) ;
V_6 = 1 ;
break;
}
}
return V_6 ;
}
static void F_23 ( struct V_3 * V_4 )
{
struct V_7 * V_8 = F_4 ( V_4 ) ;
T_2 V_146 = 0 , V_147 = 0 , V_148 = 0 ;
T_1 V_149 = 0 ;
F_24 ( V_4 , V_150 , & V_146 ) ;
F_25 ( V_4 , V_150 , ( V_146 | V_151 ) ) ;
F_26 ( 50 ) ;
F_6 ( V_4 , V_152 , & V_149 ) ;
F_7 ( V_4 , V_152 , ( V_149 & ( ~ V_153 ) ) ) ;
for ( V_147 = ( T_5 ) V_142 ;
V_147 <= V_143 ; V_147 ++ ) {
V_148 = F_22 ( V_4 , ( T_5 ) V_147 ,
( T_3 ) 0 ) ;
if ( V_148 != 0 ) {
F_10 ( ( V_32 | V_31 ) ,
L_10 ,
V_147 - 1 ) ;
return;
}
}
F_5 ( V_4 , V_154 , V_155 | V_156 , 0x0 ) ;
F_19 ( V_4 , V_64 ) ;
F_6 ( V_4 , V_152 , & V_149 ) ;
F_7 ( V_4 , V_152 , ( V_149 | V_153 ) ) ;
F_19 ( V_4 , V_67 ) ;
F_27 ( V_4 , 0x5e , 0x00 ) ;
if ( V_8 -> V_157 == ( T_2 ) V_158 ) {
V_149 = V_8 -> V_159 [ 1 ] << 4 |
V_8 -> V_159 [ 0 ] ;
F_5 ( V_4 , V_90 , ( V_160 | V_161 ) ,
V_149 ) ;
V_149 = V_8 -> V_162 & 0xf ;
F_5 ( V_4 , V_163 , V_164 ,
V_149 ) ;
}
V_8 -> V_165 = ( T_2 ) F_8 ( V_4 ,
V_96 ,
0x200 ) ;
}
void F_28 ( struct V_3 * V_4 )
{
F_21 ( V_4 ) ;
F_23 ( V_4 ) ;
}
void F_29 ( struct V_3 * V_4 )
{
struct V_7 * V_8 = F_4 ( V_4 ) ;
T_2 V_40 ;
F_6 ( V_4 , V_166 ,
& V_8 -> V_167 [ 0 ] ) ;
F_6 ( V_4 , V_168 ,
& V_8 -> V_167 [ 1 ] ) ;
F_6 ( V_4 , V_169 ,
& V_8 -> V_167 [ 2 ] ) ;
F_6 ( V_4 , V_170 ,
& V_8 -> V_167 [ 3 ] ) ;
F_6 ( V_4 , V_171 ,
& V_8 -> V_167 [ 4 ] ) ;
F_6 ( V_4 , V_172 ,
& V_8 -> V_167 [ 5 ] ) ;
F_24 ( V_4 , V_104 , & V_8 -> V_173 [ 0 ] ) ;
F_24 ( V_4 , V_105 , & V_8 -> V_173 [ 1 ] ) ;
F_24 ( V_4 , V_106 , & V_8 -> V_173 [ 2 ] ) ;
F_24 ( V_4 , V_107 , & V_8 -> V_173 [ 3 ] ) ;
F_10 ( V_174 ,
L_11 ,
V_8 -> V_173 [ 0 ] , V_8 -> V_173 [ 1 ] ,
V_8 -> V_173 [ 2 ] , V_8 -> V_173 [ 3 ] ) ;
F_24 ( V_4 , V_175 , & V_8 -> V_176 ) ;
F_24 ( V_4 , V_177 , & V_40 ) ;
V_8 -> V_178 = V_40 ;
F_10 ( V_174 , L_12 ,
V_175 , V_8 -> V_176 ) ;
F_30 ( V_4 , V_179 , & V_8 -> V_180 ) ;
}
void F_31 ( struct V_3 * V_4 , T_2 V_181 )
{
struct V_7 * V_8 = F_4 ( V_4 ) ;
T_2 V_182 = V_8 -> V_183 [ V_181 - 1 ] ;
T_2 V_184 = V_8 -> V_185 [ V_181 - 1 ] ;
switch ( V_8 -> V_27 ) {
case V_28 :
F_32 ( V_4 , V_182 ) ;
F_33 ( V_4 , V_184 ) ;
break;
default:
F_10 ( ( V_31 | V_32 ) ,
L_13 ,
V_145 ) ;
break;
}
}
void F_34 ( struct V_3 * V_4 )
{
struct V_7 * V_8 = F_4 ( V_4 ) ;
switch ( V_8 -> V_27 ) {
case V_28 :
F_35 ( V_4 ) ;
break;
default:
F_10 ( V_32 , L_14 ) ;
break;
}
}
void F_36 ( struct V_3 * V_4 )
{
}
T_2 F_37 ( struct V_3 * V_4 ,
T_3 V_5 )
{
int V_2 ;
switch ( V_5 ) {
case V_12 :
for ( V_2 = 0 ; V_2 < V_186 ; V_2 = V_2 + 2 ) {
if ( V_187 [ V_2 ] == 0xfe ) {
F_26 ( 100 ) ;
continue;
}
F_13 ( V_4 , V_5 ,
V_187 [ V_2 ] ,
V_39 ,
V_187 [ V_2 + 1 ] ) ;
F_26 ( 1 ) ;
}
break;
case V_13 :
for ( V_2 = 0 ; V_2 < V_188 ; V_2 = V_2 + 2 ) {
if ( V_189 [ V_2 ] == 0xfe ) {
F_26 ( 100 ) ;
continue;
}
F_13 ( V_4 , V_5 ,
V_189 [ V_2 ] ,
V_39 ,
V_189 [ V_2 + 1 ] ) ;
F_26 ( 1 ) ;
}
break;
case V_14 :
for ( V_2 = 0 ; V_2 < V_190 ; V_2 = V_2 + 2 ) {
if ( V_191 [ V_2 ] == 0xfe ) {
F_26 ( 100 ) ;
continue;
}
F_13 ( V_4 , V_5 ,
V_191 [ V_2 ] ,
V_39 ,
V_191 [ V_2 + 1 ] ) ;
F_26 ( 1 ) ;
}
break;
case V_15 :
for ( V_2 = 0 ; V_2 < V_192 ; V_2 = V_2 + 2 ) {
if ( V_193 [ V_2 ] == 0xfe ) {
F_26 ( 100 ) ;
continue;
}
F_13 ( V_4 , V_5 ,
V_193 [ V_2 ] ,
V_39 ,
V_193 [ V_2 + 1 ] ) ;
F_26 ( 1 ) ;
}
break;
default:
break;
}
return 0 ;
}
static void F_38 ( struct V_3 * V_4 , T_2 V_181 )
{
struct V_7 * V_8 = F_4 ( V_4 ) ;
T_2 V_182 = V_8 -> V_183 [ V_181 - 1 ] ;
T_2 V_184 = V_8 -> V_185 [ V_181 - 1 ] ;
switch ( V_8 -> V_27 ) {
case V_194 :
#ifdef F_20
F_39 ( V_54 , V_182 ) ;
F_40 ( V_54 , V_184 ) ;
#endif
break;
case V_28 :
F_32 ( V_4 , V_182 ) ;
F_33 ( V_4 , V_184 ) ;
break;
case V_195 :
break;
default:
F_10 ( V_32 , L_15 , V_145 ) ;
break;
}
}
bool F_41 ( struct V_3 * V_4 ,
T_6 V_196 )
{
bool V_197 = true ;
struct V_7 * V_8 = F_4 ( V_4 ) ;
if ( V_196 == V_8 -> V_198 -> V_196 )
return false ;
if ( V_8 -> V_199 )
return false ;
V_8 -> V_199 = true ;
switch ( V_8 -> V_27 ) {
case V_28 :
switch ( V_196 ) {
case V_200 :
F_5 ( V_4 , V_77 , F_42 ( 4 ) ,
0x1 ) ;
F_5 ( V_4 , V_201 , 0x300 ,
0x3 ) ;
F_5 ( V_4 , V_163 , 0x18 ,
0x3 ) ;
F_5 ( V_4 , V_202 , 0x3 , 0x3 ) ;
F_5 ( V_4 , V_203 , 0x3 , 0x3 ) ;
F_5 ( V_4 , V_163 , 0x60 ,
0x3 ) ;
break;
case V_204 :
break;
case V_205 :
F_5 ( V_4 , V_77 , F_42 ( 4 ) ,
0x0 ) ;
F_5 ( V_4 , V_201 , 0xf00 ,
0x0 ) ;
F_5 ( V_4 , V_163 , 0x18 ,
0x0 ) ;
F_5 ( V_4 , V_202 , 0xf , 0x0 ) ;
F_5 ( V_4 , V_203 , 0xf , 0x0 ) ;
F_5 ( V_4 , V_163 , 0x60 ,
0x0 ) ;
break;
default:
V_197 = false ;
F_10 ( V_32 , L_16 ,
V_145 , V_196 ) ;
break;
}
break;
default:
F_10 ( V_32 , L_17 , V_8 -> V_27 ) ;
break;
}
#ifdef F_20
if ( V_197 ) {
V_206 -> V_196 = V_196 ;
switch ( V_206 -> V_207 ) {
case V_28 :
switch ( V_206 -> V_196 ) {
case V_205 :
if ( V_208 -> V_209 == V_210 )
V_54 -> V_211 . V_212 ( V_54 , V_213 ) ;
else
V_54 -> V_211 . V_212 ( V_54 , V_214 ) ;
break;
case V_200 :
if ( V_208 -> V_215 )
V_54 -> V_211 . V_212 ( V_54 , V_216 ) ;
else
V_54 -> V_211 . V_212 ( V_54 , V_213 ) ;
break;
default:
break;
}
break;
default:
F_10 ( V_217 , V_218 , L_18 ,
V_145 ) ;
break;
}
}
#endif
V_8 -> V_199 = false ;
return V_197 ;
}
static T_2 F_43 ( T_7 * V_219 , T_1 V_220 ,
T_1 V_221 , T_8 V_222 ,
T_1 V_223 , T_1 V_224 , T_1 V_225 )
{
T_7 * V_226 ;
if ( V_219 == NULL ) {
F_10 ( V_32 , L_19 , V_145 ) ;
return false ;
}
if ( V_220 >= V_221 ) {
F_10 ( V_32 , L_20 ,
V_145 , V_220 , V_221 ) ;
return false ;
}
V_226 = V_219 + V_220 ;
V_226 -> V_222 = V_222 ;
V_226 -> V_223 = V_223 ;
V_226 -> V_224 = V_224 ;
V_226 -> V_225 = V_225 ;
return true ;
}
static T_2 F_44 ( struct V_3 * V_4 , T_2 V_181 ,
T_2 * V_227 , T_2 * V_228 , T_1 * V_229 )
{
struct V_7 * V_8 = F_4 ( V_4 ) ;
T_7 V_230 [ V_231 ] ;
T_1 V_232 ;
T_7 V_233 [ V_234 ] ;
T_1 V_235 ;
T_7 V_236 [ V_237 ] ;
T_1 V_238 ;
T_7 * V_239 = NULL ;
T_2 V_5 ;
F_10 ( V_240 , L_21 ,
V_145 , * V_227 , * V_228 , V_181 ) ;
if ( ! F_45 ( V_8 -> V_198 , V_181 ) ) {
F_10 ( V_32 , L_22 , V_181 ) ;
return true ;
}
V_232 = 0 ;
F_43 ( V_230 , V_232 ++ ,
V_231 , V_241 ,
0 , 0 , 0 ) ;
F_43 ( V_230 , V_232 ++ ,
V_231 , V_242 , 0 , 0 , 0 ) ;
V_235 = 0 ;
F_43 ( V_233 , V_235 ++ ,
V_234 , V_242 , 0 , 0 , 0 ) ;
V_238 = 0 ;
switch ( V_8 -> V_27 ) {
case V_194 :
if ( ! ( V_181 >= 1 && V_181 <= 14 ) ) {
F_10 ( V_32 ,
L_23 ,
V_181 ) ;
return true ;
}
F_43 ( V_236 , V_238 ++ ,
V_237 ,
V_243 ,
V_244 ,
V_245 [ V_181 ] ,
10 ) ;
F_43 ( V_236 , V_238 ++ ,
V_237 ,
V_242 , 0 , 0 , 0 ) ;
break;
case V_28 :
if ( ! ( V_181 >= 1 && V_181 <= 14 ) ) {
F_10 ( V_32 ,
L_24 ,
V_181 ) ;
return true ;
}
F_43 ( V_236 , V_238 ++ ,
V_237 ,
V_243 ,
V_244 , V_181 , 10 ) ;
F_43 ( V_236 , V_238 ++ ,
V_237 ,
V_242 , 0 , 0 , 0 ) ;
break;
case V_195 :
break;
default:
F_10 ( V_32 , L_25 , V_8 -> V_27 ) ;
return true ;
}
do {
switch ( * V_227 ) {
case 0 :
V_239 = & V_230 [ * V_228 ] ;
break;
case 1 :
V_239 = & V_236 [ * V_228 ] ;
break;
case 2 :
V_239 = & V_233 [ * V_228 ] ;
break;
}
if ( V_239 -> V_222 == V_242 ) {
if ( ( * V_227 ) == 2 ) {
( * V_229 ) = V_239 -> V_225 ;
return true ;
}
( * V_227 ) ++ ;
( * V_228 ) = 0 ;
continue;
}
switch ( V_239 -> V_222 ) {
case V_241 :
if ( V_8 -> V_157 == ( T_2 ) V_158 )
F_38 ( V_4 , V_181 ) ;
break;
case V_246 :
F_7 ( V_4 , V_239 -> V_223 ,
V_239 -> V_224 ) ;
break;
case V_247 :
F_46 ( V_4 , V_239 -> V_223 ,
( V_248 ) V_239 -> V_224 ) ;
break;
case V_249 :
F_25 ( V_4 , V_239 -> V_223 ,
( T_2 ) V_239 -> V_224 ) ;
break;
case V_243 :
for ( V_5 = 0 ; V_5 < V_250 ; V_5 ++ ) {
F_13 ( V_4 ,
( T_3 ) V_5 ,
V_239 -> V_223 ,
V_251 ,
V_239 -> V_224 ) ;
}
break;
default:
break;
}
break;
} while ( true );
( * V_229 ) = V_239 -> V_225 ;
( * V_228 ) ++ ;
return false ;
}
static void F_47 ( struct V_3 * V_4 , T_2 V_181 )
{
struct V_7 * V_8 = F_4 ( V_4 ) ;
T_1 V_229 = 0 ;
while ( ! F_44 ( V_4 , V_181 , & V_8 -> V_252 ,
& V_8 -> V_253 , & V_229 ) ) {
if ( ! V_8 -> V_254 )
break;
}
}
void F_48 ( struct V_3 * V_4 )
{
struct V_7 * V_8 = F_4 ( V_4 ) ;
F_10 ( V_240 , L_26 ,
V_8 -> V_255 ) ;
F_47 ( V_4 , V_8 -> V_255 ) ;
F_10 ( V_240 , L_27 ) ;
}
T_2 F_49 ( struct V_3 * V_4 , T_2 V_181 )
{
struct V_7 * V_8 = F_4 ( V_4 ) ;
F_10 ( V_240 , L_28 , V_145 ,
V_8 -> V_256 ) ;
if ( ! V_8 -> V_254 )
return false ;
if ( V_8 -> V_256 )
return false ;
switch ( V_8 -> V_198 -> V_257 ) {
case V_258 :
case V_259 :
if ( V_181 <= 14 ) {
F_10 ( V_32 , L_29 ) ;
return false ;
}
break;
case V_260 :
if ( V_181 > 14 ) {
F_10 ( V_32 , L_30 ) ;
return false ;
}
break;
case V_261 :
case V_262 :
if ( V_181 > 14 ) {
F_10 ( V_32 , L_31 ) ;
return false ;
}
break;
}
V_8 -> V_256 = true ;
if ( V_181 == 0 )
V_181 = 1 ;
V_8 -> V_255 = V_181 ;
V_8 -> V_252 = 0 ;
V_8 -> V_253 = 0 ;
if ( V_8 -> V_254 )
F_48 ( V_4 ) ;
V_8 -> V_256 = false ;
return true ;
}
void F_50 ( struct V_3 * V_4 )
{
struct V_7 * V_8 = F_4 ( V_4 ) ;
T_2 V_263 ;
F_10 ( V_264 , L_32 , V_145 ,
V_8 -> V_265 == V_266 ? L_33 : L_34 ) ;
if ( V_8 -> V_27 == V_267 ) {
V_8 -> V_268 = false ;
return;
}
F_24 ( V_4 , V_269 , & V_263 ) ;
switch ( V_8 -> V_265 ) {
case V_266 :
V_263 |= V_270 ;
F_25 ( V_4 , V_269 , V_263 ) ;
break;
case V_271 :
V_263 &= ~ V_270 ;
F_25 ( V_4 , V_269 , V_263 ) ;
break;
default:
F_10 ( V_32 ,
L_35 ,
V_8 -> V_265 ) ;
break;
}
switch ( V_8 -> V_265 ) {
case V_266 :
F_5 ( V_4 , V_154 , V_272 , 0x0 ) ;
F_5 ( V_4 , V_273 , V_272 , 0x0 ) ;
F_5 ( V_4 , V_163 ,
0x00100000 , 1 ) ;
V_8 -> V_274 =
V_8 -> V_275 +
V_8 -> V_276 ;
if ( V_8 -> V_274 > 22 )
V_8 -> V_274 = 22 ;
if ( V_8 -> V_274 < 0 )
V_8 -> V_274 = 0 ;
F_10 ( V_174 ,
L_36 ,
V_8 -> V_274 ) ;
if ( V_8 -> V_255 == 14 && ! V_8 -> V_277 ) {
V_8 -> V_277 = true ;
F_51 ( V_4 , V_8 -> V_277 ) ;
} else if ( V_8 -> V_255 != 14 && V_8 -> V_277 ) {
V_8 -> V_277 = false ;
F_51 ( V_4 , V_8 -> V_277 ) ;
} else {
F_51 ( V_4 , V_8 -> V_277 ) ;
}
break;
case V_271 :
F_5 ( V_4 , V_154 , V_272 , 0x1 ) ;
F_5 ( V_4 , V_273 , V_272 , 0x1 ) ;
F_5 ( V_4 , V_278 , V_279 ,
V_8 -> V_280 >> 1 ) ;
F_5 ( V_4 , V_163 , 0x00100000 , 0 ) ;
F_5 ( V_4 , V_281 , 0xC00 ,
V_8 -> V_280 ) ;
V_8 -> V_274 =
V_8 -> V_282 +
V_8 -> V_276 ;
if ( V_8 -> V_274 > 22 )
V_8 -> V_274 = 22 ;
if ( V_8 -> V_274 < 0 )
V_8 -> V_274 = 0 ;
F_10 ( V_174 ,
L_37 ,
V_8 -> V_274 ) ;
if ( V_8 -> V_255 == 14 && ! V_8 -> V_277 ) {
V_8 -> V_277 = true ;
F_51 ( V_4 , V_8 -> V_277 ) ;
} else if ( V_8 -> V_255 != 14 && V_8 -> V_277 ) {
V_8 -> V_277 = false ;
F_51 ( V_4 , V_8 -> V_277 ) ;
} else {
F_51 ( V_4 , V_8 -> V_277 ) ;
}
break;
default:
F_10 ( V_32 ,
L_35 ,
V_8 -> V_265 ) ;
break;
}
switch ( V_8 -> V_27 ) {
case V_194 :
#ifdef F_20
F_52 ( V_54 , V_206 -> V_265 ) ;
#endif
break;
case V_28 :
F_53 ( V_4 , V_8 -> V_265 ) ;
break;
case V_195 :
break;
case V_267 :
break;
default:
F_10 ( V_32 , L_25 , V_8 -> V_27 ) ;
break;
}
V_8 -> V_268 = false ;
F_10 ( V_264 , L_38 ,
F_54 ( & V_8 -> V_198 -> V_283 ) ) ;
}
void F_55 ( struct V_3 * V_4 , T_9 V_284 ,
T_10 V_21 )
{
struct V_7 * V_8 = F_4 ( V_4 ) ;
if ( V_8 -> V_268 )
return;
V_8 -> V_268 = true ;
V_8 -> V_265 = V_284 ;
if ( V_21 == V_285 )
V_8 -> V_280 = V_286 ;
else if ( V_21 == V_287 )
V_8 -> V_280 = V_288 ;
else
V_8 -> V_280 = V_289 ;
F_50 ( V_4 ) ;
}
void F_56 ( struct V_3 * V_4 , T_2 V_290 )
{
struct V_7 * V_8 = F_4 ( V_4 ) ;
V_8 -> V_291 = V_290 ;
if ( V_8 -> V_254 )
F_57 ( V_8 -> V_292 , & V_8 -> V_293 , 0 ) ;
}
void F_58 ( struct V_294 * V_295 )
{
struct V_296 * V_297 = F_59 ( V_295 , struct V_296 ,
V_295 ) ;
struct V_7 * V_8 = F_59 ( V_297 , struct V_7 ,
V_293 ) ;
struct V_3 * V_4 = V_8 -> V_198 -> V_4 ;
#define F_60 0x17
#define F_61 0x08
T_1 V_1 ;
T_2 V_298 ;
T_2 V_290 ;
V_290 = V_8 -> V_291 ;
switch ( V_290 ) {
case V_299 :
F_10 ( V_300 , L_39 ) ;
V_298 = F_60 ;
V_1 = V_301 ;
if ( V_302 . V_303 == V_304 )
F_5 ( V_4 , V_305 , V_306 , 0x8 ) ;
V_8 -> V_307 . V_308 =
( T_2 ) F_8 ( V_4 , V_104 , V_1 ) ;
V_8 -> V_307 . V_309 =
( T_2 ) F_8 ( V_4 , V_105 , V_1 ) ;
V_8 -> V_307 . V_310 =
( T_2 ) F_8 ( V_4 , V_106 , V_1 ) ;
V_8 -> V_307 . V_311 =
( T_2 ) F_8 ( V_4 , V_107 , V_1 ) ;
V_1 = V_312 ;
V_8 -> V_307 . V_313 =
( T_2 ) F_8 ( V_4 , V_314 , V_1 ) ;
F_10 ( V_300 , L_40 ,
V_8 -> V_307 . V_308 ) ;
F_10 ( V_300 , L_41 ,
V_8 -> V_307 . V_309 ) ;
F_10 ( V_300 , L_42 ,
V_8 -> V_307 . V_310 ) ;
F_10 ( V_300 , L_43 ,
V_8 -> V_307 . V_311 ) ;
F_10 ( V_300 , L_44 ,
V_8 -> V_307 . V_313 ) ;
F_10 ( V_300 , L_45 ,
V_298 ) ;
F_25 ( V_4 , V_104 , V_298 ) ;
F_25 ( V_4 , V_105 , V_298 ) ;
F_25 ( V_4 , V_106 , V_298 ) ;
F_25 ( V_4 , V_107 , V_298 ) ;
F_10 ( V_300 , L_46 ,
F_61 ) ;
F_25 ( V_4 , 0xa0a , F_61 ) ;
break;
case V_315 :
F_10 ( V_300 , L_47 ) ;
V_1 = 0x7f ;
if ( V_302 . V_303 == V_304 )
F_5 ( V_4 , V_305 , V_306 , 0x8 ) ;
F_5 ( V_4 , V_104 , V_1 ,
( T_1 ) V_8 -> V_307 . V_308 ) ;
F_5 ( V_4 , V_105 , V_1 ,
( T_1 ) V_8 -> V_307 . V_309 ) ;
F_5 ( V_4 , V_106 , V_1 ,
( T_1 ) V_8 -> V_307 . V_310 ) ;
F_5 ( V_4 , V_107 , V_1 ,
( T_1 ) V_8 -> V_307 . V_311 ) ;
V_1 = V_312 ;
F_5 ( V_4 , V_314 , V_1 ,
( T_1 ) V_8 -> V_307 . V_313 ) ;
F_10 ( V_300 , L_48 ,
V_8 -> V_307 . V_308 ) ;
F_10 ( V_300 , L_49 ,
V_8 -> V_307 . V_309 ) ;
F_10 ( V_300 , L_50 ,
V_8 -> V_307 . V_310 ) ;
F_10 ( V_300 , L_51 ,
V_8 -> V_307 . V_311 ) ;
F_10 ( V_300 , L_52 ,
V_8 -> V_307 . V_313 ) ;
F_31 ( V_4 , V_8 -> V_198 -> V_316 . V_181 ) ;
if ( V_302 . V_303 == V_304 )
F_5 ( V_4 , V_305 , V_306 , 0x1 ) ;
break;
default:
F_10 ( V_300 , L_53 ) ;
break;
}
}
