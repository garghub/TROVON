static int T_1 F_1 ( void )
{
T_2 V_1 ;
T_3 * V_2 ;
int V_3 ;
int V_4 ;
int V_5 , V_6 ;
int V_7 ;
int V_8 = 0 ;
V_3 = F_2 ( & V_9 ) ;
if ( V_3 ) {
F_3 ( L_1 ,
V_10 ) ;
return V_3 ;
}
V_3 = F_4 ( & V_11 ) ;
if ( ! V_3 )
V_12 = 1 ;
V_3 = - V_13 ;
for ( V_2 = V_14 ; V_2 -> V_15 ; V_2 ++ ) {
F_5 ( L_2 , V_16 ,
V_2 -> V_15 ) ;
for ( V_5 = 0 ; V_5 < F_6 ( V_2 -> V_5 ) ; V_5 ++ ) {
V_4 = V_2 -> V_5 [ V_5 ] ;
if ( ! V_4 )
continue;
V_7 = F_7 ( V_4 ) ;
if ( V_7 == 0xff ) {
F_5 ( L_3 ,
V_16 , V_4 ) ;
continue;
}
F_8 ( V_2 -> V_17 , V_4 ) ;
V_6 = F_7 ( V_4 + 1 ) ;
if ( ( V_6 & V_2 -> V_18 ) == V_2 -> V_19 ) {
F_5 ( L_4 ,
V_16 , V_2 -> V_15 ,
V_6 & ~ V_2 -> V_18 ) ;
if ( V_20 ) {
memset ( & V_1 , 0 , sizeof( T_2 ) ) ;
V_1 . V_4 = V_4 ;
V_1 . V_21 = V_22 . V_21 ;
V_1 . V_23 = V_22 . V_23 ;
V_1 . V_24 = V_22 . V_24 ;
if ( V_1 . V_21 < 0x2000 ) {
F_9 ( L_5 ,
V_10 ) ;
V_2 -> V_25 ( V_2 , & V_1 ) ;
} else
V_2 -> V_26 ( V_2 , & V_1 ) ;
if ( F_10 ( & V_1 ) >= 0 )
V_3 = 0 ;
}
if ( V_3 ) {
F_5 ( L_6 ,
V_10 ) ;
memset ( & V_1 , 0 , sizeof( T_2 ) ) ;
V_1 . V_4 = V_4 ;
V_1 . V_21 = V_27 [ V_8 ] ;
V_1 . V_23 = V_23 [ V_8 ] ;
V_1 . V_24 = V_24 [ V_8 ] ;
if ( V_27 [ V_8 ] < 0x2000 ) {
V_2 -> V_25 ( V_2 , & V_1 ) ;
} else
V_2 -> V_26 ( V_2 , & V_1 ) ;
if ( F_10 ( & V_1 ) >= 0 )
V_3 = 0 ;
}
V_8 ++ ;
} else {
F_5 ( L_7 ,
V_16 , V_6 ) ;
}
}
}
if ( V_3 ) {
F_11 ( & V_9 ) ;
F_12 ( & V_11 ) ;
V_12 = 0 ;
}
return V_3 ;
}
static void T_4 F_13 ( void )
{
int V_8 ;
for ( V_8 = 0 ; V_8 < F_6 ( V_28 ) ; V_8 ++ ) {
if ( V_28 [ V_8 ] )
F_14 ( V_28 [ V_8 ] ) ;
}
F_11 ( & V_9 ) ;
if ( V_12 )
F_12 ( & V_11 ) ;
V_12 = 0 ;
}
static int T_1 F_10 ( T_2 * V_1 )
{
struct V_29 * V_30 ;
struct V_31 * V_32 ;
void * V_3 ;
int V_33 , V_34 ;
for ( V_34 = 0 ; V_34 < F_6 ( V_28 ) ; V_34 ++ ) {
if ( ! V_28 [ V_34 ] )
break;
}
if ( V_34 == F_6 ( V_28 ) ) {
F_3 ( L_8 ,
V_16 ) ;
return - V_35 ;
}
F_9 ( L_9 ,
V_10 , V_1 -> V_4 ) ;
if ( ( F_15 ( V_1 ) ) == - 1 )
return - 1 ;
F_9 ( L_10 , V_10 ) ;
V_30 = F_16 ( sizeof( struct V_31 ) ) ;
if ( V_30 == NULL ) {
F_3 ( L_11 ,
V_16 ) ;
return - V_35 ;
}
V_32 = F_17 ( V_30 ) ;
V_32 -> V_36 = V_30 ;
F_18 ( & V_32 -> V_37 ) ;
V_28 [ V_34 ] = V_32 ;
V_32 -> V_38 = V_34 ;
V_32 -> V_27 . V_4 = V_1 -> V_4 ;
V_32 -> V_27 . V_21 = V_1 -> V_21 ;
V_32 -> V_27 . V_24 = V_1 -> V_24 ;
V_32 -> V_27 . V_39 = V_40 ;
V_32 -> V_27 . V_23 = V_1 -> V_23 ;
V_32 -> V_27 . V_41 = 32 ;
V_3 = F_19 ( V_32 -> V_27 . V_21 , V_32 -> V_27 . V_39 , V_10 ) ;
if ( ! V_3 ) {
F_20 ( L_12 ,
V_16 , V_32 -> V_27 . V_21 ) ;
V_33 = - V_13 ;
goto V_42;
}
F_21 ( & V_32 -> V_43 ) ;
V_32 -> V_43 . V_44 . V_45 = V_46 | V_47 | V_48 | V_49 |
V_50 | V_51 | V_52 | ( V_53 << 8 ) ;
V_32 -> V_43 . V_54 . V_45 = V_55 ;
F_22 ( & V_32 -> V_43 ) ;
V_32 -> V_56 . V_57 = 14384 ;
V_32 -> V_58 . V_57 = 14384 ;
V_32 -> V_56 . V_59 =
F_23 ( NULL , V_32 -> V_56 . V_57 ,
& V_32 -> V_60 , V_61 ) ;
if ( V_32 -> V_56 . V_59 == NULL ) {
V_33 = - V_35 ;
goto V_62;
}
V_32 -> V_58 . V_59 =
F_23 ( NULL , V_32 -> V_58 . V_57 ,
& V_32 -> V_63 , V_61 ) ;
if ( V_32 -> V_58 . V_59 == NULL ) {
V_33 = - V_35 ;
goto V_64;
}
V_32 -> V_56 . V_65 = FALSE ;
V_32 -> V_56 . V_66 = V_67 ;
V_32 -> V_58 . V_68 = V_32 -> V_58 . V_59 ;
V_32 -> V_56 . V_68 = V_32 -> V_56 . V_59 ;
V_32 -> V_69 . V_70 = V_32 -> V_69 . V_71 = V_32 -> V_69 . free = 0 ;
V_32 -> V_69 . V_72 = V_32 -> V_58 . V_59 ;
V_30 -> V_73 = & V_74 ;
V_33 = F_24 ( V_30 ) ;
if ( V_33 ) {
F_3 ( L_13 ,
V_16 ) ;
goto V_75;
}
F_9 ( L_14 , V_30 -> V_15 ) ;
if ( ( V_76 <= 0 ) ||
( V_76 >= F_6 ( V_77 ) ) ) {
V_76 = F_25 ( V_32 -> V_27 . V_21 ) ;
F_9 ( L_15 ,
V_10 , V_77 [ V_76 ] ) ;
} else {
F_9 ( L_16 ,
V_10 , V_77 [ V_76 ] ) ;
}
V_32 -> V_27 . V_76 = V_76 ;
F_26 ( V_32 -> V_27 . V_21 , V_76 ) ;
V_32 -> V_78 = F_27 ( V_79 ,
V_32 -> V_38 , NULL , 0 ) ;
if ( F_28 ( V_32 -> V_78 ) ) {
V_33 = F_29 ( V_32 -> V_78 ) ;
goto V_80;
}
F_30 ( V_32 -> V_78 , V_32 ) ;
return V_34 ;
V_80:
F_31 ( V_30 ) ;
V_75:
F_32 ( NULL , V_32 -> V_58 . V_57 ,
V_32 -> V_58 . V_59 , V_32 -> V_63 ) ;
V_64:
F_32 ( NULL , V_32 -> V_56 . V_57 ,
V_32 -> V_56 . V_59 , V_32 -> V_60 ) ;
V_62:
F_33 ( V_32 -> V_27 . V_21 , V_32 -> V_27 . V_39 ) ;
V_42:
F_34 ( V_30 ) ;
V_28 [ V_34 ] = NULL ;
return V_33 ;
}
static int T_4 F_14 ( struct V_31 * V_32 )
{
int V_81 ;
F_35 (self != NULL, return -1;) ;
V_81 = V_32 -> V_27 . V_21 ;
F_36 ( V_32 -> V_78 ) ;
F_31 ( V_32 -> V_36 ) ;
F_5 ( L_17 ,
V_16 , V_32 -> V_27 . V_21 ) ;
F_33 ( V_32 -> V_27 . V_21 , V_32 -> V_27 . V_39 ) ;
if ( V_32 -> V_58 . V_59 )
F_32 ( NULL , V_32 -> V_58 . V_57 ,
V_32 -> V_58 . V_59 , V_32 -> V_63 ) ;
if ( V_32 -> V_56 . V_59 )
F_32 ( NULL , V_32 -> V_56 . V_57 ,
V_32 -> V_56 . V_59 , V_32 -> V_60 ) ;
V_28 [ V_32 -> V_38 ] = NULL ;
F_34 ( V_32 -> V_36 ) ;
return 0 ;
}
static int F_37 ( T_3 * V_2 , T_2 * V_1 )
{
int V_4 = V_1 -> V_4 ;
T_5 V_82 = 0 ;
F_8 ( 2 , V_4 ) ;
F_8 ( 0x00 , V_4 + 1 ) ;
F_8 ( V_83 , V_4 ) ;
switch ( V_1 -> V_21 ) {
case 0x3e8 : F_8 ( 0x14 , V_4 + 1 ) ; break;
case 0x2e8 : F_8 ( 0x15 , V_4 + 1 ) ; break;
case 0x3f8 : F_8 ( 0x16 , V_4 + 1 ) ; break;
case 0x2f8 : F_8 ( 0x17 , V_4 + 1 ) ; break;
default: F_3 ( L_18 , V_16 ) ;
}
switch ( V_1 -> V_24 ) {
case 3 : V_82 = 0x01 ; break;
case 4 : V_82 = 0x02 ; break;
case 5 : V_82 = 0x03 ; break;
case 7 : V_82 = 0x04 ; break;
case 9 : V_82 = 0x05 ; break;
case 11 : V_82 = 0x06 ; break;
case 15 : V_82 = 0x07 ; break;
default: F_3 ( L_19 , V_16 ) ;
}
F_8 ( V_84 , V_4 ) ;
switch ( V_1 -> V_23 ) {
case 0 : F_8 ( 0x08 + V_82 , V_4 + 1 ) ; break;
case 1 : F_8 ( 0x10 + V_82 , V_4 + 1 ) ; break;
case 3 : F_8 ( 0x18 + V_82 , V_4 + 1 ) ; break;
default: F_3 ( L_20 , V_16 ) ;
}
F_8 ( V_85 , V_4 ) ;
F_8 ( 0x03 , V_4 + 1 ) ;
return 0 ;
}
static int F_38 ( T_3 * V_2 , T_2 * V_1 )
{
int V_4 = V_1 -> V_4 ;
int V_7 ;
F_8 ( V_83 , V_4 ) ;
V_7 = F_7 ( V_4 + 1 ) ;
switch ( V_7 & 0x03 ) {
case 0 :
V_1 -> V_21 = 0x3e8 ;
break;
case 1 :
V_1 -> V_21 = 0x2e8 ;
break;
case 2 :
V_1 -> V_21 = 0x3f8 ;
break;
case 3 :
V_1 -> V_21 = 0x2f8 ;
break;
}
V_1 -> V_86 = V_1 -> V_21 ;
F_5 ( L_21 , V_16 ,
V_1 -> V_21 ) ;
F_8 ( V_84 , V_4 ) ;
V_7 = F_7 ( V_4 + 1 ) ;
switch ( V_7 & 0x07 ) {
case 0 :
V_1 -> V_24 = - 1 ;
break;
case 1 :
V_1 -> V_24 = 3 ;
break;
case 2 :
V_1 -> V_24 = 4 ;
break;
case 3 :
V_1 -> V_24 = 5 ;
break;
case 4 :
V_1 -> V_24 = 7 ;
break;
case 5 :
V_1 -> V_24 = 9 ;
break;
case 6 :
V_1 -> V_24 = 11 ;
break;
case 7 :
V_1 -> V_24 = 15 ;
break;
}
F_5 ( L_22 , V_16 , V_1 -> V_24 ) ;
switch ( ( V_7 >> 3 ) & 0x03 ) {
case 0 :
V_1 -> V_23 = - 1 ;
break;
case 1 :
V_1 -> V_23 = 0 ;
break;
case 2 :
V_1 -> V_23 = 1 ;
break;
case 3 :
V_1 -> V_23 = 3 ;
break;
}
F_5 ( L_23 , V_16 , V_1 -> V_23 ) ;
F_8 ( V_85 , V_4 ) ;
V_7 = F_7 ( V_4 + 1 ) ;
V_1 -> V_87 = V_7 & 0x01 ;
V_1 -> V_88 = ! ( ( V_7 >> 1 ) & 0x01 ) ;
return 0 ;
}
static int F_39 ( T_3 * V_2 , T_2 * V_1 )
{
return 0 ;
}
static int F_40 ( T_3 * V_2 , T_2 * V_1 )
{
int V_4 = V_1 -> V_4 ;
int V_7 , V_89 = 0 ;
int V_90 ;
F_8 ( V_91 , V_4 ) ;
V_7 = F_7 ( V_4 + 1 ) ;
V_1 -> V_87 = ( V_7 >> 2 ) & 0x01 ;
F_8 ( V_92 , V_4 ) ;
V_7 = F_7 ( V_4 + 1 ) ;
V_90 = ( V_7 >> 3 ) & 0x01 ;
if ( V_90 ) {
F_5 ( L_24 ) ;
F_8 ( 0x46 , V_4 ) ;
V_7 = ( F_7 ( V_4 + 1 ) & 0xfe ) << 2 ;
F_8 ( 0x47 , V_4 ) ;
V_7 |= ( ( F_7 ( V_4 + 1 ) & 0xfc ) << 8 ) ;
V_1 -> V_21 = V_7 ;
} else {
F_8 ( V_93 , V_4 ) ;
V_7 = F_7 ( V_4 + 1 ) ;
switch ( ( V_7 >> 4 ) & 0x03 ) {
case 0 :
V_1 -> V_21 = 0x3f8 ;
break;
case 1 :
V_1 -> V_21 = 0x2f8 ;
break;
case 2 :
V_89 = 3 ;
break;
case 3 :
V_89 = 4 ;
break;
}
if ( V_89 ) {
switch ( ( V_7 >> 6 ) & 0x03 ) {
case 0 :
if ( V_89 == 3 )
V_1 -> V_21 = 0x3e8 ;
else
V_1 -> V_21 = 0x2e8 ;
break;
case 1 :
if ( V_89 == 3 )
V_1 -> V_21 = 0x338 ;
else
V_1 -> V_21 = 0x238 ;
break;
case 2 :
if ( V_89 == 3 )
V_1 -> V_21 = 0x2e8 ;
else
V_1 -> V_21 = 0x2e0 ;
break;
case 3 :
if ( V_89 == 3 )
V_1 -> V_21 = 0x220 ;
else
V_1 -> V_21 = 0x228 ;
break;
}
}
}
V_1 -> V_86 = V_1 -> V_21 ;
F_8 ( V_94 , V_4 ) ;
V_7 = F_7 ( V_4 + 1 ) ;
V_1 -> V_24 = V_7 >> 4 ;
F_8 ( V_95 , V_4 ) ;
V_7 = F_7 ( V_4 + 1 ) ;
V_1 -> V_23 = ( V_7 & 0x07 ) - 1 ;
F_8 ( V_96 , V_4 ) ;
V_7 = F_7 ( V_4 + 1 ) ;
V_1 -> V_88 = V_7 & 0x01 ;
return 0 ;
}
static int F_41 ( T_3 * V_2 , T_2 * V_1 )
{
int V_4 = V_1 -> V_4 ;
int V_87 ;
F_5 ( L_25 ,
V_16 , V_1 -> V_21 , V_1 -> V_24 , V_1 -> V_23 ) ;
F_8 ( V_97 , V_4 ) ;
F_8 ( 0x02 , V_4 + 1 ) ;
F_8 ( V_98 , V_4 ) ;
V_87 = F_7 ( V_4 + 1 ) & 0x01 ;
if ( ! V_87 ) {
F_8 ( V_99 , V_4 ) ;
F_8 ( 0x01 , V_4 + 1 ) ;
}
F_8 ( V_100 , V_4 ) ;
F_8 ( 0x82 , V_4 + 1 ) ;
return 0 ;
}
static int F_42 ( T_3 * V_2 , T_2 * V_1 )
{
int V_4 = V_1 -> V_4 ;
int V_101 , V_102 , V_24 , V_103 , V_104 , V_105 ;
int V_87 , V_106 ;
F_5 ( L_26 ,
V_16 , V_4 ) ;
F_8 ( V_97 , V_4 ) ;
F_8 ( 0x02 , V_4 + 1 ) ;
F_8 ( V_107 , V_4 ) ;
V_101 = F_7 ( V_4 + 1 ) ;
F_8 ( V_108 , V_4 ) ;
V_102 = F_7 ( V_4 + 1 ) ;
V_1 -> V_21 = ( V_101 << 8 ) | V_102 ;
F_8 ( V_109 , V_4 ) ;
V_24 = F_7 ( V_4 + 1 ) ;
F_8 ( V_110 , V_4 ) ;
V_103 = F_7 ( V_4 + 1 ) ;
V_1 -> V_24 = V_24 ;
F_8 ( V_111 , V_4 ) ;
V_104 = F_7 ( V_4 + 1 ) ;
F_8 ( V_112 , V_4 ) ;
V_105 = F_7 ( V_4 + 1 ) ;
V_1 -> V_23 = V_104 - 1 ;
F_8 ( V_98 , V_4 ) ;
V_1 -> V_87 = V_87 = F_7 ( V_4 + 1 ) & 0x01 ;
F_8 ( V_100 , V_4 ) ;
V_106 = 1 - ( ( F_7 ( V_4 + 1 ) & 0x02 ) >> 1 ) ;
F_5 ( L_27 ,
V_16 , V_101 , V_102 , V_24 , V_103 , V_104 , V_105 , V_87 , V_106 ) ;
F_8 ( V_98 , V_4 ) ;
V_87 = F_7 ( V_4 + 1 ) & 0x01 ;
if ( ! V_87 ) {
F_8 ( V_99 , V_4 ) ;
F_8 ( 0x01 , V_4 + 1 ) ;
}
F_8 ( V_100 , V_4 ) ;
F_8 ( 0x82 , V_4 + 1 ) ;
return 0 ;
}
static int F_43 ( struct V_113 * V_30 , const struct V_114 * V_6 )
{
memset ( & V_22 , 0 , sizeof( T_2 ) ) ;
V_22 . V_24 = - 1 ;
V_22 . V_23 = - 1 ;
V_20 = 1 ;
if ( V_6 -> V_115 & V_116 )
V_76 = 0x9 ;
if ( F_44 ( V_30 , 0 ) &&
! ( F_45 ( V_30 , 0 ) & V_117 ) )
V_22 . V_21 = F_46 ( V_30 , 0 ) ;
if ( F_47 ( V_30 , 0 ) &&
! ( F_48 ( V_30 , 0 ) & V_117 ) )
V_22 . V_24 = F_49 ( V_30 , 0 ) ;
if ( F_50 ( V_30 , 0 ) &&
! ( F_51 ( V_30 , 0 ) & V_117 ) )
V_22 . V_23 = F_52 ( V_30 , 0 ) ;
F_5 ( L_28 ,
V_16 , V_22 . V_21 , V_22 . V_24 , V_22 . V_23 ) ;
if( ( V_22 . V_21 == 0 ) ||
( V_22 . V_24 == - 1 ) || ( V_22 . V_23 == - 1 ) ) {
V_20 = 0 ;
}
return 0 ;
}
static int F_15 ( T_2 * V_1 )
{
int V_118 ;
int V_81 = V_1 -> V_21 ;
F_53 ( V_81 , V_119 ) ;
V_118 = F_7 ( V_81 + V_120 ) ;
F_5 ( L_29 ,
V_16 , V_10 , V_118 ) ;
if ( 0x20 != ( V_118 & 0xf0 ) ) {
F_3 ( L_30 ,
V_10 , V_118 ) ;
return - 1 ;
}
F_53 ( V_81 , V_121 ) ;
F_8 ( V_122 , V_81 + V_123 ) ;
F_53 ( V_81 , V_124 ) ;
F_53 ( V_81 , V_124 ) ;
F_8 ( V_125 | V_126 | V_127 | V_128 | V_129 , V_81 + V_130 ) ;
F_8 ( 0x03 , V_81 + V_131 ) ;
F_8 ( V_132 , V_81 + V_133 ) ;
F_53 ( V_81 , V_121 ) ;
F_8 ( V_134 | V_135 , V_81 + V_136 ) ;
F_53 ( V_81 , V_137 ) ;
F_8 ( 0x02 , V_81 + 4 ) ;
F_53 ( V_81 , V_138 ) ;
F_8 ( 0x20 , V_81 + 0 ) ;
F_8 ( 0x0a , V_81 + 1 ) ;
F_8 ( 0x0d , V_81 + 2 ) ;
F_8 ( 0x2a , V_81 + 4 ) ;
F_53 ( V_81 , V_124 ) ;
F_8 ( V_139 , V_81 + V_140 ) ;
return 0 ;
}
static int F_25 ( int V_81 )
{
int V_76 ;
T_5 V_141 ;
V_141 = F_7 ( V_81 + V_142 ) ;
F_53 ( V_81 , V_143 ) ;
F_8 ( 0x00 , V_81 + 7 ) ;
F_54 ( 50 ) ;
V_76 = F_7 ( V_81 + 4 ) & 0x0f ;
#ifdef F_55
if ( V_76 == 0x0a )
V_76 = 0x09 ;
#endif
F_53 ( V_81 , V_124 ) ;
F_8 ( V_141 , V_81 + V_142 ) ;
return V_76 ;
}
static void F_26 ( int V_81 , int V_76 )
{
int V_141 ;
V_141 = F_7 ( V_81 + V_142 ) ;
F_53 ( V_81 , V_143 ) ;
switch ( V_76 ) {
case 0x00 :
case 0x01 :
F_5 ( L_31 ,
V_16 , V_77 [ V_76 ] ) ;
break;
case 0x02 :
case 0x03 :
F_5 ( L_31 ,
V_16 , V_77 [ V_76 ] ) ;
break;
case 0x04 :
break;
case 0x05 :
F_5 ( L_31 ,
V_16 , V_77 [ V_76 ] ) ;
break;
case 0x06 :
F_5 ( L_31 ,
V_16 , V_77 [ V_76 ] ) ;
break;
case 0x07 :
F_5 ( L_32 ,
V_16 , V_77 [ V_76 ] ) ;
break;
case 0x08 :
F_5 ( L_33 ,
V_16 , V_77 [ V_76 ] ) ;
break;
case 0x09 :
F_8 ( 0x28 , V_81 + 7 ) ;
break;
case 0x0A :
case 0x0B :
F_5 ( L_31 ,
V_16 , V_77 [ V_76 ] ) ;
break;
case 0x0C :
case 0x0D :
F_8 ( 0x48 , V_81 + 7 ) ;
break;
case 0x0E :
F_8 ( 0x28 , V_81 + 7 ) ;
break;
case 0x0F :
F_5 ( L_33 ,
V_16 , V_77 [ V_76 ] ) ;
F_53 ( V_81 , V_124 ) ;
F_8 ( 0x62 , V_81 + V_133 ) ;
break;
default:
F_5 ( L_34 ,
V_16 , V_76 ) ;
}
F_8 ( 0x00 , V_81 + 4 ) ;
F_8 ( V_141 , V_81 + V_142 ) ;
}
static void F_56 ( int V_81 , int V_144 , int V_76 )
{
T_5 V_141 ;
V_141 = F_7 ( V_81 + V_142 ) ;
F_53 ( V_81 , V_143 ) ;
switch ( V_76 ) {
case 0x00 :
case 0x01 :
F_5 ( L_31 ,
V_16 , V_77 [ V_76 ] ) ;
break;
case 0x02 :
case 0x03 :
F_5 ( L_31 ,
V_16 , V_77 [ V_76 ] ) ;
break;
case 0x04 :
break;
case 0x05 :
F_5 ( L_31 ,
V_16 , V_77 [ V_76 ] ) ;
break;
case 0x06 :
F_5 ( L_31 ,
V_16 , V_77 [ V_76 ] ) ;
break;
case 0x07 :
F_5 ( L_32 ,
V_16 , V_77 [ V_76 ] ) ;
break;
case 0x08 :
F_5 ( L_33 ,
V_16 , V_77 [ V_76 ] ) ;
F_8 ( 0x00 , V_81 + 4 ) ;
if ( V_144 > 115200 )
F_8 ( 0x01 , V_81 + 4 ) ;
break;
case 0x09 :
F_8 ( 0x01 , V_81 + 4 ) ;
if ( V_144 == 4000000 ) {
F_8 ( 0x81 , V_81 + 4 ) ;
F_8 ( 0x80 , V_81 + 4 ) ;
} else
F_8 ( 0x00 , V_81 + 4 ) ;
break;
case 0x0A :
case 0x0B :
F_5 ( L_31 ,
V_16 , V_77 [ V_76 ] ) ;
break;
case 0x0C :
case 0x0D :
break;
case 0x0E :
break;
case 0x0F :
F_5 ( L_32 ,
V_16 , V_77 [ V_76 ] ) ;
F_53 ( V_81 , V_124 ) ;
F_8 ( 0x62 , V_81 + V_133 ) ;
break;
default:
F_5 ( L_35 , V_16 ) ;
}
F_8 ( V_141 , V_81 + V_142 ) ;
}
static T_5 F_57 ( struct V_31 * V_32 , T_6 V_144 )
{
struct V_29 * V_30 ;
T_5 V_145 = V_132 ;
int V_81 ;
T_5 V_141 ;
T_5 V_146 ;
F_5 ( L_36 , V_16 , V_144 ) ;
F_35 (self != NULL, return 0;) ;
V_30 = V_32 -> V_36 ;
V_81 = V_32 -> V_27 . V_21 ;
V_32 -> V_27 . V_144 = V_144 ;
V_141 = F_7 ( V_81 + V_142 ) ;
F_53 ( V_81 , V_124 ) ;
F_8 ( 0 , V_81 + V_140 ) ;
F_53 ( V_81 , V_121 ) ;
F_8 ( 0x00 , V_81 + V_147 ) ;
switch ( V_144 ) {
case 9600 : F_8 ( 0x0c , V_81 + V_148 ) ; break;
case 19200 : F_8 ( 0x06 , V_81 + V_148 ) ; break;
case 38400 : F_8 ( 0x03 , V_81 + V_148 ) ; break;
case 57600 : F_8 ( 0x02 , V_81 + V_148 ) ; break;
case 115200 : F_8 ( 0x01 , V_81 + V_148 ) ; break;
case 576000 :
F_53 ( V_81 , V_137 ) ;
F_8 ( F_7 ( V_81 + 4 ) | 0x04 , V_81 + 4 ) ;
V_145 = V_149 ;
F_5 ( L_37 , V_16 ) ;
break;
case 1152000 :
V_145 = V_149 ;
F_5 ( L_38 , V_16 ) ;
break;
case 4000000 :
V_145 = V_150 ;
F_5 ( L_39 , V_16 ) ;
break;
default:
V_145 = V_150 ;
F_5 ( L_40 ,
V_16 , V_144 ) ;
break;
}
F_53 ( V_81 , V_124 ) ;
F_8 ( V_145 | V_151 , V_81 + V_133 ) ;
F_56 ( V_81 , V_144 , V_32 -> V_27 . V_76 ) ;
F_53 ( V_81 , V_124 ) ;
F_8 ( 0x00 , V_81 + V_130 ) ;
F_8 ( V_129 , V_81 + V_130 ) ;
F_8 ( V_125 |
V_126 |
V_127 |
V_128 |
V_129 ,
V_81 + V_130 ) ;
F_53 ( V_81 , V_121 ) ;
F_8 ( V_134 | V_135 , V_81 + V_136 ) ;
F_53 ( V_81 , V_124 ) ;
if ( V_144 > 115200 ) {
V_30 -> V_73 = & V_152 ;
V_146 = V_153 ;
F_58 ( V_32 ) ;
} else {
V_30 -> V_73 = & V_74 ;
V_146 = V_139 ;
}
F_8 ( V_146 , V_81 + V_140 ) ;
F_8 ( V_141 , V_81 + V_142 ) ;
return V_146 ;
}
static T_7 F_59 ( struct V_154 * V_155 ,
struct V_29 * V_30 )
{
struct V_31 * V_32 ;
unsigned long V_156 ;
int V_81 ;
T_8 V_144 ;
T_5 V_141 ;
V_32 = F_17 ( V_30 ) ;
F_35 (self != NULL, return NETDEV_TX_OK;) ;
V_81 = V_32 -> V_27 . V_21 ;
F_60 ( V_30 ) ;
F_61 ( & V_32 -> V_37 , V_156 ) ;
V_144 = F_62 ( V_155 ) ;
if ( ( V_144 != V_32 -> V_27 . V_144 ) && ( V_144 != - 1 ) ) {
if ( ! V_155 -> V_70 ) {
if ( V_32 -> V_27 . V_157 == V_158 ) {
F_57 ( V_32 , V_144 ) ;
F_63 ( V_30 ) ;
} else {
V_32 -> V_159 = V_144 ;
}
F_64 ( V_30 ) ;
F_65 ( & V_32 -> V_37 , V_156 ) ;
F_66 ( V_155 ) ;
return V_160 ;
} else
V_32 -> V_159 = V_144 ;
}
V_141 = F_7 ( V_81 + V_142 ) ;
V_32 -> V_58 . V_68 = V_32 -> V_58 . V_59 ;
V_32 -> V_58 . V_70 = F_67 ( V_155 , V_32 -> V_58 . V_68 ,
V_32 -> V_58 . V_57 ) ;
V_30 -> V_161 . V_162 += V_32 -> V_58 . V_70 ;
F_53 ( V_81 , V_124 ) ;
F_8 ( V_163 , V_81 + V_140 ) ;
F_8 ( V_141 , V_81 + V_142 ) ;
F_64 ( V_30 ) ;
F_65 ( & V_32 -> V_37 , V_156 ) ;
F_66 ( V_155 ) ;
return V_160 ;
}
static T_7 F_68 ( struct V_154 * V_155 ,
struct V_29 * V_30 )
{
struct V_31 * V_32 ;
unsigned long V_156 ;
int V_81 ;
T_8 V_144 ;
T_5 V_141 ;
int V_164 , V_165 ;
V_32 = F_17 ( V_30 ) ;
V_81 = V_32 -> V_27 . V_21 ;
F_60 ( V_30 ) ;
F_61 ( & V_32 -> V_37 , V_156 ) ;
V_144 = F_62 ( V_155 ) ;
if ( ( V_144 != V_32 -> V_27 . V_144 ) && ( V_144 != - 1 ) ) {
if ( ! V_155 -> V_70 ) {
if( V_32 -> V_69 . V_70 == 0 ) {
F_57 ( V_32 , V_144 ) ;
F_63 ( V_30 ) ;
} else {
V_32 -> V_159 = V_144 ;
}
F_64 ( V_30 ) ;
F_65 ( & V_32 -> V_37 , V_156 ) ;
F_66 ( V_155 ) ;
return V_160 ;
} else {
V_32 -> V_159 = V_144 ;
}
}
V_141 = F_7 ( V_81 + V_142 ) ;
V_32 -> V_69 . V_166 [ V_32 -> V_69 . free ] . V_167 = V_32 -> V_69 . V_72 ;
V_32 -> V_69 . V_166 [ V_32 -> V_69 . free ] . V_70 = V_155 -> V_70 ;
V_32 -> V_69 . V_72 += V_155 -> V_70 ;
V_30 -> V_161 . V_162 += V_155 -> V_70 ;
F_69 ( V_155 , V_32 -> V_69 . V_166 [ V_32 -> V_69 . free ] . V_167 ,
V_155 -> V_70 ) ;
V_32 -> V_69 . V_70 ++ ;
V_32 -> V_69 . free ++ ;
if ( V_32 -> V_69 . V_70 == 1 ) {
V_164 = F_70 ( V_155 ) ;
if ( V_164 ) {
V_165 = F_71 ( F_72 () , V_32 -> V_168 ) ;
if ( V_164 > V_165 ) {
V_164 -= V_165 ;
if ( V_164 > 125 ) {
V_164 = V_164 / 125 ;
F_53 ( V_81 , V_169 ) ;
F_8 ( V_164 & 0xff , V_81 + V_170 ) ;
F_8 ( ( V_164 >> 8 ) & 0x0f , V_81 + V_171 ) ;
F_8 ( V_172 , V_81 + V_173 ) ;
V_32 -> V_27 . V_157 = V_174 ;
F_53 ( V_81 , V_124 ) ;
F_8 ( V_175 , V_81 + V_140 ) ;
goto V_176;
} else
F_54 ( V_164 ) ;
}
}
F_53 ( V_81 , V_124 ) ;
F_8 ( V_177 , V_81 + V_140 ) ;
F_73 ( V_32 , V_81 ) ;
}
V_176:
if ( ( V_32 -> V_69 . free < V_178 ) && ( V_32 -> V_159 == 0 ) )
F_63 ( V_32 -> V_36 ) ;
F_8 ( V_141 , V_81 + V_142 ) ;
F_64 ( V_30 ) ;
F_65 ( & V_32 -> V_37 , V_156 ) ;
F_66 ( V_155 ) ;
return V_160 ;
}
static void F_73 ( struct V_31 * V_32 , int V_81 )
{
int V_179 ;
V_179 = F_7 ( V_81 + V_142 ) ;
F_53 ( V_81 , V_124 ) ;
F_8 ( F_7 ( V_81 + V_133 ) & ~ V_180 , V_81 + V_133 ) ;
V_32 -> V_27 . V_157 = V_174 ;
F_53 ( V_81 , V_121 ) ;
F_8 ( V_181 | V_182 | V_122 , V_81 + V_123 ) ;
F_74 ( V_32 -> V_27 . V_23 ,
( ( V_183 * ) V_32 -> V_69 . V_166 [ V_32 -> V_69 . V_71 ] . V_167 -
V_32 -> V_58 . V_59 ) + V_32 -> V_63 ,
V_32 -> V_69 . V_166 [ V_32 -> V_69 . V_71 ] . V_70 ,
V_184 ) ;
F_53 ( V_81 , V_124 ) ;
F_8 ( F_7 ( V_81 + V_133 ) | V_151 | V_180 | V_185 , V_81 + V_133 ) ;
F_8 ( V_179 , V_81 + V_142 ) ;
}
static int F_75 ( int V_81 , T_5 * V_186 , int V_70 , int V_41 )
{
int V_187 = 0 ;
T_5 V_141 ;
V_141 = F_7 ( V_81 + V_142 ) ;
F_53 ( V_81 , V_124 ) ;
if ( ! ( F_76 ( V_81 + V_188 ) & V_189 ) ) {
F_5 ( L_41 ,
V_16 ) ;
V_41 -= 17 ;
}
while ( ( V_41 -- > 0 ) && ( V_187 < V_70 ) ) {
F_8 ( V_186 [ V_187 ++ ] , V_81 + V_190 ) ;
}
F_5 ( L_42 ,
V_16 , V_41 , V_187 , V_70 ) ;
F_8 ( V_141 , V_81 + V_142 ) ;
return V_187 ;
}
static int F_77 ( struct V_31 * V_32 )
{
int V_81 ;
T_5 V_141 ;
int V_3 = TRUE ;
V_81 = V_32 -> V_27 . V_21 ;
V_141 = F_7 ( V_81 + V_142 ) ;
F_53 ( V_81 , V_124 ) ;
F_8 ( F_7 ( V_81 + V_133 ) & ~ V_180 , V_81 + V_133 ) ;
if ( F_7 ( V_81 + V_191 ) & V_192 ) {
V_32 -> V_36 -> V_161 . V_193 ++ ;
V_32 -> V_36 -> V_161 . V_194 ++ ;
F_8 ( V_192 , V_81 + V_191 ) ;
} else {
V_32 -> V_36 -> V_161 . V_195 ++ ;
}
V_32 -> V_69 . V_71 ++ ;
V_32 -> V_69 . V_70 -- ;
if ( V_32 -> V_69 . V_70 ) {
F_73 ( V_32 , V_81 ) ;
V_3 = FALSE ;
} else {
V_32 -> V_69 . V_70 = V_32 -> V_69 . V_71 = V_32 -> V_69 . free = 0 ;
V_32 -> V_69 . V_72 = V_32 -> V_58 . V_59 ;
}
if ( ( V_32 -> V_69 . free < V_178 ) && ( V_32 -> V_159 == 0 ) ) {
F_63 ( V_32 -> V_36 ) ;
}
F_8 ( V_141 , V_81 + V_142 ) ;
return V_3 ;
}
static int F_58 ( struct V_31 * V_32 )
{
int V_81 ;
T_5 V_179 ;
V_81 = V_32 -> V_27 . V_21 ;
V_32 -> V_69 . V_70 = V_32 -> V_69 . V_71 = V_32 -> V_69 . free = 0 ;
V_32 -> V_69 . V_72 = V_32 -> V_58 . V_59 ;
V_179 = F_7 ( V_81 + V_142 ) ;
F_53 ( V_81 , V_124 ) ;
F_8 ( F_7 ( V_81 + V_133 ) & ~ V_180 , V_81 + V_133 ) ;
F_53 ( V_81 , V_121 ) ;
F_8 ( V_182 | V_122 , V_81 + V_123 ) ;
V_32 -> V_27 . V_157 = V_158 ;
V_32 -> V_56 . V_68 = V_32 -> V_56 . V_59 ;
F_53 ( V_81 , V_124 ) ;
F_8 ( V_128 | V_129 , V_81 + V_130 ) ;
V_32 -> V_196 . V_70 = V_32 -> V_196 . V_197 = 0 ;
V_32 -> V_196 . V_72 = V_32 -> V_196 . V_59 = 0 ;
F_74 ( V_32 -> V_27 . V_23 , V_32 -> V_60 , V_32 -> V_56 . V_57 ,
V_198 ) ;
F_53 ( V_81 , V_124 ) ;
F_8 ( F_7 ( V_81 + V_133 ) | V_180 , V_81 + V_133 ) ;
F_8 ( V_179 , V_81 + V_142 ) ;
return 0 ;
}
static int F_78 ( struct V_31 * V_32 , int V_81 )
{
struct V_196 * V_196 ;
struct V_154 * V_155 ;
T_5 V_199 ;
T_5 V_141 ;
int V_70 ;
V_196 = & V_32 -> V_196 ;
V_141 = F_7 ( V_81 + V_142 ) ;
F_53 ( V_81 , V_137 ) ;
while ( ( V_199 = F_7 ( V_81 + V_200 ) ) & V_201 ) {
V_70 = F_7 ( V_81 + V_202 ) | ( ( F_7 ( V_81 + V_203 ) & 0x1f ) << 8 ) ;
if ( V_196 -> V_72 >= V_204 ) {
F_5 ( L_43 , V_16 ) ;
continue;
}
V_196 -> V_205 [ V_196 -> V_72 ] . V_199 = V_199 ;
V_196 -> V_205 [ V_196 -> V_72 ] . V_70 = V_70 ;
V_196 -> V_197 += V_70 ;
V_196 -> V_72 ++ ;
V_196 -> V_70 ++ ;
}
while ( V_196 -> V_70 > 0 ) {
V_199 = V_196 -> V_205 [ V_196 -> V_59 ] . V_199 ;
V_70 = V_196 -> V_205 [ V_196 -> V_59 ] . V_70 ;
V_196 -> V_197 -= V_70 ;
V_196 -> V_59 ++ ;
V_196 -> V_70 -- ;
if ( V_199 & V_206 ) {
if ( V_199 & V_207 ) {
V_32 -> V_36 -> V_161 . V_208 += V_70 ;
} else {
V_32 -> V_36 -> V_161 . V_208 ++ ;
V_32 -> V_56 . V_68 += V_70 ;
if ( V_199 & V_209 )
V_32 -> V_36 -> V_161 . V_210 ++ ;
if ( V_199 & V_211 )
V_32 -> V_36 -> V_161 . V_212 ++ ;
if ( V_199 & V_213 )
V_32 -> V_36 -> V_161 . V_214 ++ ;
}
if ( V_199 & V_215 )
V_32 -> V_36 -> V_161 . V_216 ++ ;
if ( V_199 & V_217 )
V_32 -> V_36 -> V_161 . V_216 ++ ;
} else {
if ( V_196 -> V_197 < V_32 -> V_27 . V_41 ) {
F_53 ( V_81 , V_124 ) ;
if ( F_7 ( V_81 + V_188 ) & V_218 ) {
V_196 -> V_59 -- ;
V_196 -> V_70 ++ ;
V_196 -> V_197 += V_70 ;
V_196 -> V_205 [ V_196 -> V_59 ] . V_199 = V_199 ;
V_196 -> V_205 [ V_196 -> V_59 ] . V_70 = V_70 ;
F_53 ( V_81 , V_169 ) ;
F_8 ( 0x02 , V_81 + V_170 ) ;
F_8 ( 0x00 , V_81 + V_171 ) ;
F_8 ( V_172 , V_81 + V_173 ) ;
F_8 ( V_141 , V_81 + V_142 ) ;
return FALSE ;
}
}
V_32 -> V_168 = F_72 () ;
V_155 = F_79 ( V_70 + 1 ) ;
if ( V_155 == NULL ) {
V_32 -> V_36 -> V_161 . V_219 ++ ;
F_8 ( V_141 , V_81 + V_142 ) ;
return FALSE ;
}
F_80 ( V_155 , 1 ) ;
if ( V_32 -> V_27 . V_144 < 4000000 ) {
F_81 ( V_155 , V_70 - 2 ) ;
F_82 ( V_155 ,
V_32 -> V_56 . V_68 ,
V_70 - 2 ) ;
} else {
F_81 ( V_155 , V_70 - 4 ) ;
F_82 ( V_155 ,
V_32 -> V_56 . V_68 ,
V_70 - 4 ) ;
}
V_32 -> V_56 . V_68 += V_70 ;
V_32 -> V_36 -> V_161 . V_220 += V_70 ;
V_32 -> V_36 -> V_161 . V_221 ++ ;
V_155 -> V_30 = V_32 -> V_36 ;
F_83 ( V_155 ) ;
V_155 -> V_222 = F_84 ( V_223 ) ;
F_85 ( V_155 ) ;
}
}
F_8 ( V_141 , V_81 + V_142 ) ;
return TRUE ;
}
static void F_86 ( struct V_31 * V_32 )
{
T_5 V_224 ;
int V_81 ;
V_81 = V_32 -> V_27 . V_21 ;
do {
V_224 = F_7 ( V_81 + V_225 ) ;
F_87 ( V_32 -> V_36 , & V_32 -> V_36 -> V_161 ,
& V_32 -> V_56 , V_224 ) ;
} while ( F_7 ( V_81 + V_188 ) & V_218 );
}
static void F_88 ( struct V_31 * V_32 , int V_226 )
{
int V_187 ;
if ( V_226 & V_227 ) {
V_187 = F_75 ( V_32 -> V_27 . V_21 ,
V_32 -> V_58 . V_68 ,
V_32 -> V_58 . V_70 ,
V_32 -> V_27 . V_41 ) ;
V_32 -> V_58 . V_68 += V_187 ;
V_32 -> V_58 . V_70 -= V_187 ;
V_32 -> V_27 . V_157 = V_174 ;
if ( V_32 -> V_58 . V_70 > 0 )
V_32 -> V_146 = V_163 ;
else {
V_32 -> V_36 -> V_161 . V_195 ++ ;
F_63 ( V_32 -> V_36 ) ;
V_32 -> V_146 = V_228 ;
}
}
if ( V_226 & V_229 ) {
V_32 -> V_27 . V_157 = V_158 ;
V_32 -> V_146 = V_139 ;
if ( V_32 -> V_159 ) {
F_5 ( L_44 , V_16 ) ;
V_32 -> V_146 = F_57 ( V_32 ,
V_32 -> V_159 ) ;
V_32 -> V_159 = 0 ;
F_63 ( V_32 -> V_36 ) ;
if ( V_32 -> V_27 . V_144 > 115200 ) {
return;
}
}
}
if ( V_226 & V_230 ) {
F_86 ( V_32 ) ;
V_32 -> V_146 = V_139 ;
}
}
static void F_89 ( struct V_31 * V_32 , int V_81 ,
int V_226 )
{
T_5 V_141 ;
V_141 = F_7 ( V_81 + V_142 ) ;
if ( V_226 & V_231 ) {
if ( F_78 ( V_32 , V_81 ) ) {
V_32 -> V_146 = V_153 ;
} else {
V_32 -> V_146 = V_153 | V_175 ;
}
} else if ( V_226 & V_232 ) {
F_53 ( V_81 , V_169 ) ;
F_8 ( 0 , V_81 + V_173 ) ;
F_53 ( V_81 , V_124 ) ;
F_8 ( V_233 , V_81 + V_191 ) ;
if ( V_32 -> V_27 . V_157 == V_174 ) {
F_73 ( V_32 , V_81 ) ;
V_32 -> V_146 = V_177 ;
} else {
if ( F_78 ( V_32 , V_81 ) ) {
V_32 -> V_146 = V_153 ;
} else {
V_32 -> V_146 = V_153 | V_175 ;
}
}
} else if ( V_226 & V_234 ) {
if ( F_77 ( V_32 ) ) {
if( V_32 -> V_159 != 0 ) {
V_32 -> V_146 = V_228 ;
} else {
if ( F_90 ( V_32 -> V_36 ) ) {
F_58 ( V_32 ) ;
V_32 -> V_146 = V_153 ;
} else
F_20 ( L_45 ,
V_16 ) ;
}
} else {
V_32 -> V_146 = V_177 ;
}
} else if ( V_226 & V_229 ) {
V_32 -> V_146 = F_57 ( V_32 , V_32 -> V_159 ) ;
V_32 -> V_159 = 0 ;
F_63 ( V_32 -> V_36 ) ;
}
F_8 ( V_141 , V_81 + V_142 ) ;
}
static T_9 F_91 ( int V_24 , void * V_235 )
{
struct V_29 * V_30 = V_235 ;
struct V_31 * V_32 ;
T_5 V_179 , V_226 ;
int V_81 ;
V_32 = F_17 ( V_30 ) ;
F_92 ( & V_32 -> V_37 ) ;
V_81 = V_32 -> V_27 . V_21 ;
V_179 = F_7 ( V_81 + V_142 ) ;
F_53 ( V_81 , V_124 ) ;
V_32 -> V_146 = F_7 ( V_81 + V_140 ) ;
V_226 = F_7 ( V_81 + V_236 ) & V_32 -> V_146 ;
F_8 ( 0 , V_81 + V_140 ) ;
if ( V_226 ) {
if ( V_32 -> V_27 . V_144 > 115200 )
F_89 ( V_32 , V_81 , V_226 ) ;
else
F_88 ( V_32 , V_226 ) ;
}
F_8 ( V_32 -> V_146 , V_81 + V_140 ) ;
F_8 ( V_179 , V_81 + V_142 ) ;
F_93 ( & V_32 -> V_37 ) ;
return F_94 ( V_226 ) ;
}
static int F_95 ( struct V_31 * V_32 )
{
unsigned long V_156 ;
int V_199 = FALSE ;
int V_81 ;
T_5 V_141 ;
F_35 (self != NULL, return FALSE;) ;
F_61 ( & V_32 -> V_37 , V_156 ) ;
if ( V_32 -> V_27 . V_144 > 115200 ) {
V_81 = V_32 -> V_27 . V_21 ;
V_141 = F_7 ( V_81 + V_142 ) ;
F_53 ( V_81 , V_121 ) ;
if ( ( F_7 ( V_81 + V_237 ) & 0x3f ) != 0 ) {
V_199 = TRUE ;
}
F_8 ( V_141 , V_81 + V_142 ) ;
} else
V_199 = ( V_32 -> V_56 . V_66 != V_67 ) ;
F_65 ( & V_32 -> V_37 , V_156 ) ;
return V_199 ;
}
static int F_96 ( struct V_29 * V_30 )
{
struct V_31 * V_32 ;
int V_81 ;
char V_238 [ 32 ] ;
T_5 V_141 ;
F_35 (dev != NULL, return -1;) ;
V_32 = F_17 ( V_30 ) ;
F_35 (self != NULL, return 0;) ;
V_81 = V_32 -> V_27 . V_21 ;
if ( F_97 ( V_32 -> V_27 . V_24 , F_91 , 0 , V_30 -> V_15 , V_30 ) ) {
F_20 ( L_46 ,
V_10 , V_32 -> V_27 . V_24 ) ;
return - V_239 ;
}
if ( F_98 ( V_32 -> V_27 . V_23 , V_30 -> V_15 ) ) {
F_20 ( L_47 ,
V_10 , V_32 -> V_27 . V_23 ) ;
F_99 ( V_32 -> V_27 . V_24 , V_30 ) ;
return - V_239 ;
}
V_141 = F_7 ( V_81 + V_142 ) ;
F_53 ( V_81 , V_124 ) ;
F_8 ( V_240 | V_139 , V_81 + V_140 ) ;
F_8 ( V_141 , V_81 + V_142 ) ;
F_100 ( V_30 ) ;
sprintf ( V_238 , L_48 , V_32 -> V_27 . V_21 ) ;
V_32 -> V_241 = F_101 ( V_30 , & V_32 -> V_43 , V_238 ) ;
return 0 ;
}
static int F_102 ( struct V_29 * V_30 )
{
struct V_31 * V_32 ;
int V_81 ;
T_5 V_141 ;
F_35 (dev != NULL, return -1;) ;
V_32 = F_17 ( V_30 ) ;
F_35 (self != NULL, return 0;) ;
F_60 ( V_30 ) ;
if ( V_32 -> V_241 )
F_103 ( V_32 -> V_241 ) ;
V_32 -> V_241 = NULL ;
V_81 = V_32 -> V_27 . V_21 ;
F_104 ( V_32 -> V_27 . V_23 ) ;
V_141 = F_7 ( V_81 + V_142 ) ;
F_53 ( V_81 , V_124 ) ;
F_8 ( 0 , V_81 + V_140 ) ;
F_99 ( V_32 -> V_27 . V_24 , V_30 ) ;
F_105 ( V_32 -> V_27 . V_23 ) ;
F_8 ( V_141 , V_81 + V_142 ) ;
return 0 ;
}
static int F_106 ( struct V_29 * V_30 , struct V_242 * V_243 , int V_244 )
{
struct V_245 * V_24 = (struct V_245 * ) V_243 ;
struct V_31 * V_32 ;
unsigned long V_156 ;
int V_3 = 0 ;
F_35 (dev != NULL, return -1;) ;
V_32 = F_17 ( V_30 ) ;
F_35 (self != NULL, return -1;) ;
F_5 ( L_49 , V_16 , V_30 -> V_15 , V_244 ) ;
switch ( V_244 ) {
case V_246 :
if ( ! F_107 ( V_247 ) ) {
V_3 = - V_248 ;
break;
}
F_61 ( & V_32 -> V_37 , V_156 ) ;
F_57 ( V_32 , V_24 -> V_249 ) ;
F_65 ( & V_32 -> V_37 , V_156 ) ;
break;
case V_250 :
if ( ! F_107 ( V_247 ) ) {
V_3 = - V_248 ;
break;
}
F_108 ( V_32 -> V_36 , TRUE ) ;
break;
case V_251 :
V_24 -> V_252 = F_95 ( V_32 ) ;
break;
default:
V_3 = - V_253 ;
}
return V_3 ;
}
static int F_109 ( struct V_254 * V_30 , T_10 V_66 )
{
struct V_31 * V_32 = F_110 ( V_30 ) ;
int V_141 ;
unsigned long V_156 ;
int V_81 = V_32 -> V_27 . V_21 ;
if ( V_32 -> V_27 . V_88 )
return 0 ;
F_5 ( L_50 , V_10 ) ;
F_111 () ;
if ( F_112 ( V_32 -> V_36 ) ) {
F_113 ( V_32 -> V_36 ) ;
F_61 ( & V_32 -> V_37 , V_156 ) ;
V_141 = F_7 ( V_81 + V_142 ) ;
F_53 ( V_81 , V_124 ) ;
F_8 ( 0 , V_81 + V_140 ) ;
F_8 ( V_141 , V_81 + V_142 ) ;
F_65 ( & V_32 -> V_37 , V_156 ) ;
F_99 ( V_32 -> V_27 . V_24 , V_32 -> V_36 ) ;
F_104 ( V_32 -> V_27 . V_23 ) ;
}
V_32 -> V_27 . V_88 = 1 ;
F_114 () ;
return 0 ;
}
static int F_115 ( struct V_254 * V_30 )
{
struct V_31 * V_32 = F_110 ( V_30 ) ;
unsigned long V_156 ;
if ( ! V_32 -> V_27 . V_88 )
return 0 ;
F_5 ( L_51 , V_10 ) ;
F_111 () ;
F_15 ( & V_32 -> V_27 ) ;
F_26 ( V_32 -> V_27 . V_21 , V_32 -> V_27 . V_76 ) ;
if ( F_112 ( V_32 -> V_36 ) ) {
if ( F_97 ( V_32 -> V_27 . V_24 , F_91 , 0 ,
V_32 -> V_36 -> V_15 , V_32 -> V_36 ) ) {
F_20 ( L_46 ,
V_10 , V_32 -> V_27 . V_24 ) ;
F_116 ( V_32 -> V_36 ) ;
} else {
F_61 ( & V_32 -> V_37 , V_156 ) ;
F_57 ( V_32 , V_32 -> V_27 . V_144 ) ;
F_65 ( & V_32 -> V_37 , V_156 ) ;
F_117 ( V_32 -> V_36 ) ;
}
} else {
F_61 ( & V_32 -> V_37 , V_156 ) ;
F_57 ( V_32 , 9600 ) ;
F_65 ( & V_32 -> V_37 , V_156 ) ;
}
V_32 -> V_27 . V_88 = 0 ;
F_114 () ;
return 0 ;
}
