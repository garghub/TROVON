static void F_1 ( unsigned long V_1 )
{
struct V_2 * V_3 = (struct V_2 * ) V_1 ;
if ( ! V_3 )
return;
F_2 ( V_3 -> V_4 , L_1 ) ;
if ( V_3 -> V_5 )
F_3 ( V_3 -> V_5 ) ;
}
static T_1 F_4 ( int V_6 , void * V_1 )
{
struct V_2 * V_3 = (struct V_2 * ) V_1 ;
F_5 ( & V_3 -> V_7 ) ;
return V_8 ;
}
static void F_6 ( struct V_4 * V_9 )
{
struct V_2 * V_3 = F_7 ( V_9 ) ;
int V_10 ;
for ( V_10 = 0 ; V_10 < V_3 -> V_11 ; V_10 ++ ) {
F_8 ( & V_9 -> V_12 , V_3 -> V_13 [ V_10 ] . V_14 ) ;
F_9 ( V_3 -> V_13 [ V_10 ] . V_15 ) ;
}
}
static int F_10 ( struct V_4 * V_9 )
{
struct V_16 * V_17 = V_9 -> V_18 ;
struct V_2 * V_3 = F_7 ( V_9 ) ;
int V_19 , V_20 , V_10 , V_21 ;
V_19 = F_11 ( V_17 ) ;
if ( V_19 < 0 ) {
F_12 ( V_9 , L_2 , V_19 ) ;
return V_19 ;
}
V_20 = F_13 ( V_17 , L_3 ) ;
if ( V_19 != V_20 ) {
F_12 ( V_9 , L_4 ) ;
return - V_22 ;
}
V_3 -> V_13 = F_14 ( V_9 , V_19 , sizeof( * V_3 -> V_13 ) ,
V_23 ) ;
if ( ! V_3 -> V_13 )
return - V_24 ;
V_3 -> V_11 = V_19 ;
for ( V_10 = 0 ; V_10 < V_19 ; V_10 ++ ) {
V_3 -> V_13 [ V_10 ] . V_15 = F_15 ( V_9 , NULL , V_10 ,
V_25 ) ;
if ( F_16 ( V_3 -> V_13 [ V_10 ] . V_15 ) ) {
F_12 ( V_9 , L_5 , V_10 ) ;
return F_17 ( V_3 -> V_13 [ V_10 ] . V_15 ) ;
}
V_21 = F_18 ( V_17 , L_3 , V_10 ,
& ( V_3 -> V_13 [ V_10 ] . V_14 ) ) ;
if ( V_21 ) {
F_12 ( V_9 , L_6 , V_10 ) ;
return V_21 ;
}
V_21 = F_19 ( V_3 -> V_13 [ V_10 ] . V_15 , 0 ) ;
if ( V_21 )
return V_21 ;
V_21 = F_20 ( V_9 , V_3 -> V_13 [ V_10 ] . V_14 ,
V_3 -> V_13 [ V_10 ] . V_15 ) ;
if ( V_21 )
return V_21 ;
}
return 0 ;
}
static int F_21 ( struct V_4 * V_9 )
{
struct V_16 * V_17 ;
struct V_2 * V_3 ;
struct V_26 * V_27 = F_22 ( V_9 ) ;
struct V_28 * V_29 = F_23 ( V_27 ) ;
int V_6 , V_30 , V_21 ;
struct V_31 V_32 ;
struct V_31 V_33 ;
V_17 = V_9 -> V_18 ;
if ( ! V_17 ) {
F_12 ( V_9 , L_7 ) ;
return - V_34 ;
}
V_3 = F_24 ( V_9 , sizeof( * V_3 ) , V_23 ) ;
if ( ! V_3 )
return - V_24 ;
F_25 ( V_9 , V_3 ) ;
V_3 -> V_4 = V_9 ;
V_6 = F_26 ( V_17 , 0 ) ;
if ( ! V_6 ) {
F_12 ( V_9 , L_8 , V_6 ) ;
return - V_22 ;
}
V_3 -> V_35 = V_6 ;
V_30 = F_27 ( V_6 ) ;
F_28 ( & V_3 -> V_7 ,
F_1 , ( unsigned long ) V_3 ) ;
V_21 = F_29 ( V_9 , V_6 , F_4 ,
V_30 , L_9 , V_3 ) ;
if ( V_21 < 0 ) {
F_12 ( V_9 , L_10 ,
V_6 , V_30 ) ;
return V_21 ;
}
F_30 ( V_6 ) ;
if ( V_36 ) {
V_21 = F_10 ( V_9 ) ;
if ( V_21 < 0 ) {
F_12 ( V_9 , L_11 ) ;
goto V_37;
}
}
V_32 . V_14 = L_12 ;
V_32 . V_38 = V_27 -> V_38 ;
V_32 . V_39 = V_27 -> V_39 ;
V_32 . V_40 = NULL ;
V_32 . V_41 = NULL ;
V_3 -> V_5 = F_31 ( V_29 , & V_32 ) ;
if ( ! V_3 -> V_5 ) {
F_12 ( V_9 , L_13 ) ;
V_21 = - V_24 ;
goto V_42;
}
V_21 = F_32 ( & V_3 -> V_5 -> V_4 ) ;
if ( V_21 == 0 ) {
F_33 ( V_9 , L_14 ) ;
V_21 = - V_43 ;
goto V_44;
} else if ( V_21 < 0 ) {
F_12 ( V_9 , L_15 , V_21 ) ;
goto V_44;
}
V_33 . V_14 = L_16 ;
V_33 . V_38 = V_27 -> V_38 ;
V_33 . V_39 = V_27 -> V_39 ;
V_33 . V_40 = NULL ;
V_33 . V_41 = NULL ;
V_3 -> V_45 = F_31 ( V_29 , & V_33 ) ;
if ( ! V_3 -> V_45 ) {
F_12 ( V_9 , L_17 ) ;
V_21 = - V_24 ;
goto V_44;
}
V_21 = F_32 ( & V_3 -> V_45 -> V_4 ) ;
if ( V_21 == 0 ) {
F_33 ( V_9 , L_18 ) ;
V_21 = - V_43 ;
goto V_46;
} else if ( V_21 < 0 ) {
F_12 ( V_9 , L_19 , V_21 ) ;
goto V_46;
}
F_2 ( V_9 , L_20 ) ;
return 0 ;
V_46:
F_34 ( & V_3 -> V_45 -> V_4 , NULL ) ;
V_44:
F_34 ( & V_3 -> V_5 -> V_4 , NULL ) ;
V_42:
F_6 ( V_9 ) ;
V_37:
F_35 ( V_3 -> V_35 ) ;
F_36 ( & V_3 -> V_7 ) ;
return V_21 ;
}
static int F_37 ( struct V_4 * V_9 )
{
struct V_2 * V_3 = F_7 ( V_9 ) ;
if ( ! V_3 )
return 0 ;
if ( V_3 -> V_45 ) {
F_34 ( & V_3 -> V_45 -> V_4 , NULL ) ;
V_3 -> V_45 = NULL ;
}
if ( V_3 -> V_5 ) {
F_34 ( & V_3 -> V_5 -> V_4 , NULL ) ;
V_3 -> V_5 = NULL ;
}
F_6 ( V_9 ) ;
F_25 ( V_9 , NULL ) ;
F_35 ( V_3 -> V_35 ) ;
F_36 ( & V_3 -> V_7 ) ;
return 0 ;
}
static int T_2 F_38 ( void )
{
return F_39 ( & V_47 ) ;
}
static void T_3 F_40 ( void )
{
F_41 ( & V_47 ) ;
}
