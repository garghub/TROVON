static int F_1 ( const char * V_1 ,
const struct V_2 * V_3 )
{
if ( V_4 < F_2 ( V_5 ) )
V_5 [ V_4 ++ ] = V_1 ;
else
return - V_6 ;
return 0 ;
}
static void F_3 ( struct V_7 * V_8 , unsigned long V_9 ,
unsigned long V_10 , char * V_11 , int V_12 )
{
unsigned long V_13 = V_9 << V_14 ;
unsigned long V_15 = V_10 << V_14 ;
if ( V_13 > V_8 -> V_16 || V_8 -> V_16 - V_13 < V_15 ) {
F_4 ( L_1 ,
V_12 ? L_2 : L_3 , V_13 , V_15 ) ;
return;
}
F_5 ( & V_8 -> V_17 ) ;
while ( V_15 > 0 ) {
unsigned long V_18 ;
F_6 ( V_8 -> V_19 , V_13 , V_20 ) ;
F_7 ( * V_11 ) ;
if ( V_12 )
V_18 = F_8 ( V_8 -> V_19 , V_11 , V_15 ) ;
else
V_18 = F_9 ( V_8 -> V_19 , V_11 , V_15 ) ;
if ( V_18 == - 1 ) {
F_10 ( L_4 , V_21 ) ;
break;
}
V_11 += V_18 ;
V_13 += V_18 ;
V_15 -= V_18 ;
}
F_11 ( & V_8 -> V_17 ) ;
}
static T_1 F_12 ( struct V_22 * V_23 , struct V_24 * V_24 )
{
struct V_7 * V_8 = V_23 -> V_25 ;
struct V_26 V_27 ;
struct V_28 V_29 ;
T_2 V_9 = V_24 -> V_30 . V_31 ;
F_13 (bvec, bio, iter) {
char * V_11 = F_14 ( V_24 , V_29 ) ;
unsigned V_32 = V_27 . V_33 >> V_14 ;
F_3 ( V_8 , V_9 , V_32 , V_11 ,
F_15 ( V_24 ) == V_34 ) ;
V_9 += V_32 ;
F_16 ( V_11 ) ;
}
F_17 ( V_24 ) ;
return V_35 ;
}
static int F_18 ( struct V_36 * V_37 , T_3 V_38 )
{
struct V_7 * V_8 = V_37 -> V_39 -> V_40 ;
F_5 ( & V_8 -> V_17 ) ;
if ( ! V_8 -> V_41 )
F_19 ( V_37 ) ;
++ V_8 -> V_41 ;
F_11 ( & V_8 -> V_17 ) ;
return 0 ;
}
static void F_20 ( struct V_42 * V_43 , T_3 V_38 )
{
struct V_7 * V_8 = V_43 -> V_40 ;
F_5 ( & V_8 -> V_17 ) ;
-- V_8 -> V_41 ;
F_11 ( & V_8 -> V_17 ) ;
}
static int F_21 ( struct V_7 * V_8 , const char * V_5 )
{
int V_44 = 0 ;
V_5 = F_22 ( V_5 , V_45 ) ;
if ( V_5 == NULL )
return - V_46 ;
F_5 ( & V_8 -> V_17 ) ;
if ( V_8 -> V_19 != - 1 ) {
V_44 = - V_47 ;
goto V_48;
}
V_8 -> V_19 = F_23 ( V_5 , V_49 , 0 ) ;
if ( V_8 -> V_19 == - 1 ) {
F_10 ( L_5 , V_5 , V_21 ) ;
V_44 = - V_50 ;
goto V_48;
}
V_8 -> V_16 = F_6 ( V_8 -> V_19 , 0 , V_51 ) ;
F_24 ( V_8 -> V_52 , V_8 -> V_16 >> V_14 ) ;
V_8 -> V_5 = V_5 ;
F_25 ( L_6 , V_8 -> V_52 -> V_53 , V_8 -> V_5 ) ;
V_48:
if ( V_44 )
F_26 ( V_5 ) ;
F_11 ( & V_8 -> V_17 ) ;
return V_44 ;
}
static int F_27 ( struct V_7 * V_8 )
{
int V_44 = 0 ;
F_5 ( & V_8 -> V_17 ) ;
if ( V_8 -> V_41 != 0 ) {
V_44 = - V_47 ;
} else if ( V_8 -> V_19 != - 1 ) {
if ( F_28 ( V_8 -> V_19 ) ) {
F_10 ( L_7 ,
V_8 -> V_5 , V_21 ) ;
V_44 = - V_54 ;
} else {
F_25 ( L_8 ,
V_8 -> V_52 -> V_53 , V_8 -> V_5 ) ;
V_8 -> V_19 = - 1 ;
F_26 ( V_8 -> V_5 ) ;
V_8 -> V_5 = NULL ;
}
}
F_11 ( & V_8 -> V_17 ) ;
return V_44 ;
}
static T_4 F_29 ( struct V_55 * V_55 , char T_5 * V_56 ,
T_6 V_16 , T_7 * V_57 )
{
struct V_7 * V_8 = F_30 ( F_31 ( V_55 ) ) ;
const char * V_58 = V_8 -> V_5 ;
if ( V_58 ) {
T_4 V_59 = F_32 ( V_56 , V_16 , V_57 ,
V_58 , strlen ( V_58 ) ) ;
if ( V_59 < 0 )
return V_59 ;
V_56 += V_59 ;
V_16 -= V_59 ;
}
return F_32 ( V_56 , V_16 , V_57 , L_9 , 1 ) ;
}
static T_4 F_33 ( struct V_55 * V_55 , const char T_5 * V_56 ,
T_6 V_60 , T_7 * V_57 )
{
char * V_61 = F_34 ( V_56 , V_60 ) ;
struct V_7 * V_8 = F_30 ( F_31 ( V_55 ) ) ;
int V_44 ;
if ( F_35 ( V_61 ) )
return F_36 ( V_61 ) ;
V_44 = F_27 ( V_8 ) ;
if ( V_44 != 0 )
goto V_62;
if ( V_60 > 0 && V_61 [ V_60 - 1 ] == '\n' )
V_61 [ V_60 - 1 ] = 0 ;
if ( V_61 [ 0 ] )
V_44 = F_21 ( V_8 , V_61 ) ;
if ( V_44 == 0 )
V_44 = V_60 ;
V_62:
F_26 ( V_61 ) ;
return V_44 ;
}
static int T_8 F_37 ( struct V_7 * V_8 , int V_63 ,
struct V_64 * V_65 )
{
char V_61 [ 2 ] = { '0' + V_63 , 0 } ;
V_8 -> V_19 = - 1 ;
V_8 -> V_5 = NULL ;
F_38 ( & V_8 -> V_17 ) ;
V_8 -> V_41 = 0 ;
V_8 -> V_66 = F_39 ( V_45 ) ;
if ( V_8 -> V_66 == NULL ) {
F_10 ( L_10 ) ;
goto V_67;
}
F_40 ( V_8 -> V_66 , F_12 ) ;
V_8 -> V_66 -> V_25 = V_8 ;
V_8 -> V_52 = F_41 ( V_68 ) ;
if ( V_8 -> V_52 == NULL ) {
F_10 ( L_11 ) ;
goto V_69;
}
V_8 -> V_52 -> V_70 = V_71 ;
V_8 -> V_52 -> V_72 = V_63 ;
V_8 -> V_52 -> V_73 = & V_74 ;
V_8 -> V_52 -> V_66 = V_8 -> V_66 ;
V_8 -> V_52 -> V_40 = V_8 ;
snprintf ( V_8 -> V_52 -> V_53 , 32 , L_12 , V_63 ) ;
F_24 ( V_8 -> V_52 , 0 ) ;
F_42 ( V_8 -> V_52 ) ;
V_8 -> V_75 = F_43 ( V_61 , 0644 , V_65 , & V_73 , V_8 ) ;
return 0 ;
V_69:
F_44 ( V_8 -> V_66 ) ;
V_8 -> V_66 = NULL ;
V_67:
F_28 ( V_8 -> V_19 ) ;
return - V_54 ;
}
static int T_8 F_45 ( void )
{
int V_76 ;
if ( F_46 ( V_71 , L_13 ) < 0 ) {
F_10 ( L_14 , V_71 ) ;
return - V_54 ;
}
F_25 ( L_15 , V_71 ) ;
if ( V_4 > V_77 )
V_77 = V_4 ;
if ( V_77 > V_78 )
V_77 = V_78 ;
V_79 = F_47 ( V_77 , sizeof( * V_79 ) , V_45 ) ;
if ( V_79 == NULL )
goto V_80;
V_81 = F_48 ( L_13 , 0 ) ;
if ( V_81 == NULL )
goto V_82;
for ( V_76 = 0 ; V_76 < V_77 ; ++ V_76 ) {
if ( F_37 ( V_79 + V_76 , V_76 , V_81 ) == 0 ) {
if ( V_5 [ V_76 ] != NULL && V_5 [ V_76 ] [ 0 ] != 0 &&
( V_4 == 0 || V_76 < V_4 ) )
F_21 ( V_79 + V_76 , V_5 [ V_76 ] ) ;
}
}
return 0 ;
V_82:
F_26 ( V_79 ) ;
V_80:
F_49 ( V_71 , L_13 ) ;
return - V_46 ;
}
static void F_50 ( struct V_7 * V_8 , int V_63 ,
struct V_64 * V_65 )
{
char V_61 [ 2 ] = { '0' + V_63 , 0 } ;
F_27 ( V_8 ) ;
if ( V_8 -> V_52 )
F_51 ( V_8 -> V_52 ) ;
if ( V_8 -> V_66 )
F_44 ( V_8 -> V_66 ) ;
F_52 ( V_61 , V_65 ) ;
}
static void T_9 F_53 ( void )
{
int V_76 ;
for ( V_76 = 0 ; V_76 < V_77 ; ++ V_76 )
F_50 ( V_79 + V_76 , V_76 , V_81 ) ;
F_52 ( L_13 , 0 ) ;
F_26 ( V_79 ) ;
F_49 ( V_71 , L_13 ) ;
}
