static inline void F_1 ( struct V_1 * V_2 ,
const T_1 V_3 [ 16 ] , T_1 V_4 [ 16 ] )
{
F_2 ( V_2 , V_4 , V_3 ) ;
}
static void * F_3 ( int V_5 )
{
struct V_6 * V_7 ;
V_7 = F_4 ( sizeof( * V_7 ) , V_8 ) ;
if ( V_7 == NULL )
goto V_9;
V_7 -> V_5 = V_5 ;
V_7 -> V_2 = F_5 ( L_1 , 0 , V_10 ) ;
if ( F_6 ( V_7 -> V_2 ) ) {
V_7 -> V_2 = NULL ;
goto V_9;
}
return V_7 ;
V_9:
if ( V_7 ) {
if ( V_7 -> V_2 )
F_7 ( V_7 -> V_2 ) ;
F_8 ( V_7 ) ;
}
return NULL ;
}
static void F_9 ( void * V_7 )
{
struct V_6 * V_11 = V_7 ;
if ( V_11 && V_11 -> V_2 )
F_7 ( V_11 -> V_2 ) ;
F_8 ( V_7 ) ;
}
static inline void F_10 ( T_1 * V_12 , T_1 * V_13 , T_2 V_14 )
{
int V_15 ;
for ( V_15 = 0 ; V_15 < V_14 ; V_15 ++ )
V_12 [ V_15 ] ^= V_13 [ V_15 ] ;
}
static void F_11 ( struct V_1 * V_2 ,
struct V_16 * V_17 ,
T_1 * V_18 , T_2 V_19 , T_1 * V_20 , T_1 * V_21 , T_1 * V_22 )
{
T_1 * V_23 , V_24 = 0 ;
T_2 V_25 ;
int V_26 , V_27 ;
T_1 V_28 [ 2 * V_29 ] ;
V_26 = F_12 ( V_17 -> V_30 ) ;
V_27 = F_13 ( V_17 -> V_30 ) ;
V_25 = 22 ;
if ( V_26 )
V_25 += 6 ;
if ( V_27 ) {
V_23 = ( T_1 * ) & V_17 -> V_31 ;
if ( V_26 )
V_23 += 6 ;
V_24 = * V_23 & 0x0f ;
V_25 += 2 ;
}
V_20 [ 0 ] = 0x59 ;
V_20 [ 1 ] = V_24 ;
memcpy ( V_20 + 2 , V_17 -> V_32 , V_33 ) ;
memcpy ( V_20 + 8 , V_18 , V_34 ) ;
V_20 [ 14 ] = ( V_19 >> 8 ) & 0xff ;
V_20 [ 15 ] = V_19 & 0xff ;
V_23 = ( T_1 * ) V_17 ;
V_28 [ 0 ] = 0 ;
V_28 [ 1 ] = V_25 & 0xff ;
V_28 [ 2 ] = V_23 [ 0 ] & 0x8f ;
V_28 [ 3 ] = V_23 [ 1 ] & 0xc7 ;
memcpy ( V_28 + 4 , V_17 -> V_35 , 3 * V_33 ) ;
V_23 = ( T_1 * ) & V_17 -> V_36 ;
V_28 [ 22 ] = V_23 [ 0 ] & 0x0f ;
V_28 [ 23 ] = 0 ;
memset ( V_28 + 24 , 0 , 8 ) ;
if ( V_26 )
memcpy ( V_28 + 24 , V_17 -> V_31 , V_33 ) ;
if ( V_27 ) {
V_28 [ V_26 ? 30 : 24 ] = V_24 ;
}
F_1 ( V_2 , V_20 , V_21 ) ;
F_10 ( V_21 , V_28 , V_29 ) ;
F_1 ( V_2 , V_21 , V_21 ) ;
F_10 ( V_21 , & V_28 [ V_29 ] , V_29 ) ;
F_1 ( V_2 , V_21 , V_21 ) ;
V_20 [ 0 ] &= 0x07 ;
V_20 [ 14 ] = V_20 [ 15 ] = 0 ;
F_1 ( V_2 , V_20 , V_22 ) ;
}
static int F_14 ( struct V_37 * V_38 , int V_39 ,
T_1 * V_40 , int V_41 , void * V_7 )
{
struct V_6 * V_42 = V_7 ;
int V_15 ;
T_1 * V_23 ;
if ( F_15 ( V_38 ) < V_43 || V_38 -> V_14 < V_39 )
return - 1 ;
if ( V_40 != NULL && V_41 >= V_44 )
memcpy ( V_40 , V_42 -> V_42 , V_44 ) ;
V_23 = F_16 ( V_38 , V_43 ) ;
memmove ( V_23 , V_23 + V_43 , V_39 ) ;
V_23 += V_39 ;
V_15 = V_34 - 1 ;
while ( V_15 >= 0 ) {
V_42 -> V_45 [ V_15 ] ++ ;
if ( V_42 -> V_45 [ V_15 ] != 0 )
break;
V_15 -- ;
}
* V_23 ++ = V_42 -> V_45 [ 5 ] ;
* V_23 ++ = V_42 -> V_45 [ 4 ] ;
* V_23 ++ = 0 ;
* V_23 ++ = ( V_42 -> V_5 << 6 ) | ( 1 << 5 ) ;
* V_23 ++ = V_42 -> V_45 [ 3 ] ;
* V_23 ++ = V_42 -> V_45 [ 2 ] ;
* V_23 ++ = V_42 -> V_45 [ 1 ] ;
* V_23 ++ = V_42 -> V_45 [ 0 ] ;
return V_43 ;
}
static int F_17 ( struct V_37 * V_38 , int V_39 , void * V_7 )
{
struct V_6 * V_42 = V_7 ;
int V_46 , V_15 , V_47 , V_48 , V_14 ;
T_1 * V_23 , * V_49 ;
struct V_16 * V_17 ;
T_1 * V_20 = V_42 -> V_50 ;
T_1 * V_12 = V_42 -> V_51 ;
T_1 * V_52 = V_42 -> V_53 ;
T_1 * V_22 = V_42 -> V_54 ;
if ( F_18 ( V_38 ) < V_55 || V_38 -> V_14 < V_39 )
return - 1 ;
V_46 = V_38 -> V_14 - V_39 ;
V_14 = F_14 ( V_38 , V_39 , NULL , 0 , V_7 ) ;
if ( V_14 < 0 )
return - 1 ;
V_23 = V_38 -> V_56 + V_39 + V_43 ;
V_17 = (struct V_16 * ) V_38 -> V_56 ;
F_11 ( V_42 -> V_2 , V_17 , V_42 -> V_45 , V_46 , V_20 , V_12 , V_22 ) ;
V_47 = F_19 ( V_46 , V_29 ) ;
V_48 = V_46 % V_29 ;
for ( V_15 = 1 ; V_15 <= V_47 ; V_15 ++ ) {
V_14 = ( V_15 == V_47 && V_48 ) ? V_48 : V_29 ;
F_10 ( V_12 , V_23 , V_14 ) ;
F_1 ( V_42 -> V_2 , V_12 , V_12 ) ;
V_20 [ 14 ] = ( V_15 >> 8 ) & 0xff ;
V_20 [ 15 ] = V_15 & 0xff ;
F_1 ( V_42 -> V_2 , V_20 , V_52 ) ;
F_10 ( V_23 , V_52 , V_14 ) ;
V_23 += V_14 ;
}
V_49 = F_20 ( V_38 , V_55 ) ;
for ( V_15 = 0 ; V_15 < V_55 ; V_15 ++ )
V_49 [ V_15 ] = V_12 [ V_15 ] ^ V_22 [ V_15 ] ;
return 0 ;
}
static inline int F_21 ( T_1 * V_57 , T_1 * V_58 )
{
T_3 V_59 , V_60 ;
T_3 V_61 , V_62 ;
V_59 = ( V_57 [ 0 ] << 24 ) | ( V_57 [ 1 ] << 16 ) | ( V_57 [ 2 ] << 8 ) | V_57 [ 3 ] ;
V_60 = ( V_57 [ 4 ] << 8 ) | V_57 [ 5 ] ;
V_61 = ( V_58 [ 0 ] << 24 ) | ( V_58 [ 1 ] << 16 ) | ( V_58 [ 2 ] << 8 ) | V_58 [ 3 ] ;
V_62 = ( V_58 [ 4 ] << 8 ) | V_58 [ 5 ] ;
if ( ( V_63 ) V_59 - ( V_63 ) V_61 < 0 ||
( V_59 == V_61 && V_60 <= V_62 ) )
return 1 ;
return 0 ;
}
static int F_22 ( struct V_37 * V_38 , int V_39 , void * V_7 )
{
struct V_6 * V_42 = V_7 ;
T_1 V_64 , * V_23 ;
struct V_16 * V_17 ;
T_1 * V_20 = V_42 -> V_65 ;
T_1 * V_12 = V_42 -> V_66 ;
T_1 * V_13 = V_42 -> V_67 ;
T_1 V_18 [ 6 ] ;
int V_15 , V_47 , V_48 , V_14 ;
T_2 V_46 = V_38 -> V_14 - V_39 - V_43 - V_55 ;
T_1 * V_49 = V_38 -> V_56 + V_38 -> V_14 - V_55 ;
if ( V_38 -> V_14 < V_39 + V_43 + V_55 ) {
V_42 -> V_68 ++ ;
return - 1 ;
}
V_17 = (struct V_16 * ) V_38 -> V_56 ;
V_23 = V_38 -> V_56 + V_39 ;
V_64 = V_23 [ 3 ] ;
if ( ! ( V_64 & ( 1 << 5 ) ) ) {
F_23 ( L_2 ,
V_17 -> V_32 ) ;
V_42 -> V_68 ++ ;
return - 2 ;
}
V_64 >>= 6 ;
if ( V_42 -> V_5 != V_64 ) {
F_23 ( L_3 ,
V_42 -> V_5 , V_64 ) ;
return - 6 ;
}
if ( ! V_42 -> V_69 ) {
F_23 ( L_4 ,
V_17 -> V_32 , V_64 ) ;
return - 3 ;
}
V_18 [ 0 ] = V_23 [ 7 ] ;
V_18 [ 1 ] = V_23 [ 6 ] ;
V_18 [ 2 ] = V_23 [ 5 ] ;
V_18 [ 3 ] = V_23 [ 4 ] ;
V_18 [ 4 ] = V_23 [ 1 ] ;
V_18 [ 5 ] = V_23 [ 0 ] ;
V_23 += 8 ;
if ( F_21 ( V_18 , V_42 -> V_70 ) ) {
#ifdef F_24
F_23 ( L_5 ,
V_17 -> V_32 ,
V_42 -> V_70 [ 0 ] , V_42 -> V_70 [ 1 ] , V_42 -> V_70 [ 2 ] ,
V_42 -> V_70 [ 3 ] , V_42 -> V_70 [ 4 ] , V_42 -> V_70 [ 5 ] ,
V_18 [ 0 ] , V_18 [ 1 ] , V_18 [ 2 ] , V_18 [ 3 ] , V_18 [ 4 ] , V_18 [ 5 ] ) ;
#endif
V_42 -> V_71 ++ ;
return - 4 ;
}
F_11 ( V_42 -> V_2 , V_17 , V_18 , V_46 , V_20 , V_13 , V_12 ) ;
F_10 ( V_49 , V_12 , V_55 ) ;
V_47 = F_19 ( V_46 , V_29 ) ;
V_48 = V_46 % V_29 ;
for ( V_15 = 1 ; V_15 <= V_47 ; V_15 ++ ) {
V_14 = ( V_15 == V_47 && V_48 ) ? V_48 : V_29 ;
V_20 [ 14 ] = ( V_15 >> 8 ) & 0xff ;
V_20 [ 15 ] = V_15 & 0xff ;
F_1 ( V_42 -> V_2 , V_20 , V_12 ) ;
F_10 ( V_23 , V_12 , V_14 ) ;
F_10 ( V_13 , V_23 , V_14 ) ;
F_1 ( V_42 -> V_2 , V_13 , V_13 ) ;
V_23 += V_14 ;
}
if ( memcmp ( V_49 , V_13 , V_55 ) != 0 ) {
F_23 ( L_6 ,
V_17 -> V_32 ) ;
V_42 -> V_72 ++ ;
return - 5 ;
}
memcpy ( V_42 -> V_70 , V_18 , V_34 ) ;
memmove ( V_38 -> V_56 + V_43 , V_38 -> V_56 , V_39 ) ;
F_25 ( V_38 , V_43 ) ;
F_26 ( V_38 , V_38 -> V_14 - V_55 ) ;
return V_64 ;
}
static int F_27 ( void * V_42 , int V_14 , T_1 * V_73 , void * V_7 )
{
struct V_6 * V_56 = V_7 ;
int V_64 ;
struct V_1 * V_2 = V_56 -> V_2 ;
V_64 = V_56 -> V_5 ;
memset ( V_56 , 0 , sizeof( * V_56 ) ) ;
V_56 -> V_5 = V_64 ;
V_56 -> V_2 = V_2 ;
if ( V_14 == V_44 ) {
memcpy ( V_56 -> V_42 , V_42 , V_44 ) ;
V_56 -> V_69 = 1 ;
if ( V_73 ) {
V_56 -> V_70 [ 0 ] = V_73 [ 5 ] ;
V_56 -> V_70 [ 1 ] = V_73 [ 4 ] ;
V_56 -> V_70 [ 2 ] = V_73 [ 3 ] ;
V_56 -> V_70 [ 3 ] = V_73 [ 2 ] ;
V_56 -> V_70 [ 4 ] = V_73 [ 1 ] ;
V_56 -> V_70 [ 5 ] = V_73 [ 0 ] ;
}
F_28 ( V_56 -> V_2 , V_56 -> V_42 , V_44 ) ;
} else if ( V_14 == 0 )
V_56 -> V_69 = 0 ;
else
return - 1 ;
return 0 ;
}
static int F_29 ( void * V_42 , int V_14 , T_1 * V_73 , void * V_7 )
{
struct V_6 * V_56 = V_7 ;
if ( V_14 < V_44 )
return - 1 ;
if ( ! V_56 -> V_69 )
return 0 ;
memcpy ( V_42 , V_56 -> V_42 , V_44 ) ;
if ( V_73 ) {
V_73 [ 0 ] = V_56 -> V_45 [ 5 ] ;
V_73 [ 1 ] = V_56 -> V_45 [ 4 ] ;
V_73 [ 2 ] = V_56 -> V_45 [ 3 ] ;
V_73 [ 3 ] = V_56 -> V_45 [ 2 ] ;
V_73 [ 4 ] = V_56 -> V_45 [ 1 ] ;
V_73 [ 5 ] = V_56 -> V_45 [ 0 ] ;
}
return V_44 ;
}
static void F_30 ( struct V_74 * V_75 , void * V_7 )
{
struct V_6 * V_76 = V_7 ;
F_31 ( V_75 ,
L_7
L_8
L_9
L_10 ,
V_76 -> V_5 , V_76 -> V_69 ,
V_76 -> V_45 [ 0 ] , V_76 -> V_45 [ 1 ] , V_76 -> V_45 [ 2 ] ,
V_76 -> V_45 [ 3 ] , V_76 -> V_45 [ 4 ] , V_76 -> V_45 [ 5 ] ,
V_76 -> V_70 [ 0 ] , V_76 -> V_70 [ 1 ] , V_76 -> V_70 [ 2 ] ,
V_76 -> V_70 [ 3 ] , V_76 -> V_70 [ 4 ] , V_76 -> V_70 [ 5 ] ,
V_76 -> V_68 ,
V_76 -> V_71 ,
V_76 -> V_72 ) ;
}
static int T_4 F_32 ( void )
{
return F_33 ( & V_77 ) ;
}
static void T_5 F_34 ( void )
{
F_35 ( & V_77 ) ;
}
