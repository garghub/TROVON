static inline int F_1 ( T_1 * V_1 , unsigned long V_2 )
{
unsigned long V_3 ;
V_3 = V_4 + V_2 ;
while ( F_2 ( V_1 -> V_5 + V_6 ) != 0xffffffff ) {
if ( ! F_3 ( V_4 , V_3 ) )
return - 1 ;
F_4 () ;
}
return 0 ;
}
static int F_5 ( T_1 * V_1 , unsigned long V_7 , unsigned long V_8 )
{
if ( F_1 ( V_1 , V_9 / 10 ) < 0 )
return - 1 ;
F_6 ( V_1 -> V_5 + V_10 , V_7 ) ;
F_6 ( V_1 -> V_5 + V_6 , V_11 ) ;
if ( F_1 ( V_1 , V_9 / 10 ) < 0 )
return - 1 ;
F_6 ( V_1 -> V_5 + V_10 , V_8 ) ;
F_6 ( V_1 -> V_5 + V_6 , V_12 | V_11 ) ;
return 0 ;
}
static int F_7 ( T_1 * V_1 , unsigned long V_7 , unsigned long * V_13 )
{
if ( F_1 ( V_1 , V_9 / 10 ) < 0 )
return - 1 ;
F_6 ( V_1 -> V_5 + V_10 , V_7 ) ;
F_6 ( V_1 -> V_5 + V_6 , V_14 | V_11 ) ;
if ( F_1 ( V_1 , V_9 / 10 ) < 0 )
return - 1 ;
* V_13 = F_2 ( V_1 -> V_5 + V_10 ) ;
return 0 ;
}
static int F_8 ( T_1 * V_1 , T_2 * V_15 )
{
T_3 V_16 ;
unsigned char * V_17 ;
T_4 V_18 ;
T_3 V_19 = 0 ;
V_17 = V_15 -> V_20 ;
V_18 = V_15 -> V_21 ;
while ( V_18 >= sizeof( T_3 ) ) {
if ( V_15 -> V_22 ) {
if ( F_9 ( & V_16 , V_17 , sizeof( V_16 ) ) )
return - V_23 ;
} else {
memcpy ( & V_16 , V_17 , sizeof( V_16 ) ) ;
}
if ( F_5 ( V_1 , V_19 , V_16 ) ) {
F_10 ( V_24 L_1 ,
V_1 -> V_25 ) ;
return - V_26 ;
}
V_18 -= sizeof( T_3 ) ;
V_17 += sizeof( T_3 ) ;
V_19 += sizeof( T_3 ) ;
}
if ( V_18 ) {
V_16 = 0 ;
if ( V_15 -> V_22 ) {
if ( F_9 ( & V_16 , V_17 , V_18 ) )
return - V_23 ;
} else {
memcpy ( & V_16 , V_17 , V_18 ) ;
}
if ( F_5 ( V_1 , V_19 , V_16 ) ) {
F_10 ( V_24 L_1 ,
V_1 -> V_25 ) ;
return - V_26 ;
}
}
return 0 ;
}
static inline void F_11 ( void * * V_27 , T_5 V_16 )
{
T_5 * V_28 = * V_27 ;
* V_28 ++ = V_16 ;
* V_27 = V_28 ;
}
static inline void F_12 ( void * * V_27 , T_3 V_16 )
{
T_5 * V_28 = * V_27 ;
* V_28 ++ = V_16 & 0xff ;
* V_28 ++ = ( V_16 >> 8 ) & 0xff ;
* V_28 ++ = ( V_16 >> 16 ) & 0xff ;
* V_28 ++ = ( V_16 >> 24 ) & 0xff ;
* V_27 = V_28 ;
}
static inline void F_13 ( void * * V_27 , unsigned char * V_17 , unsigned int V_21 )
{
unsigned V_29 = V_21 ;
F_12 ( V_27 , V_29 ) ;
while ( V_29 -- > 0 )
F_11 ( V_27 , * V_17 ++ ) ;
}
static inline T_5 F_14 ( void * * V_27 )
{
T_5 * V_28 = * V_27 ;
T_5 V_16 ;
V_16 = * V_28 ++ ;
* V_27 = V_28 ;
return V_16 ;
}
static inline T_3 F_15 ( void * * V_27 )
{
T_5 * V_28 = * V_27 ;
T_3 V_16 ;
V_16 = * V_28 ++ ;
V_16 |= ( * V_28 ++ << 8 ) ;
V_16 |= ( * V_28 ++ << 16 ) ;
V_16 |= ( * V_28 ++ << 24 ) ;
* V_27 = V_28 ;
return V_16 ;
}
static inline T_3 F_16 ( void * * V_27 , unsigned char * V_17 )
{
unsigned int V_21 , V_29 ;
V_21 = V_29 = F_15 ( V_27 ) ;
while ( V_29 -- > 0 ) * V_17 ++ = F_14 ( V_27 ) ;
return V_21 ;
}
static void F_17 ( T_1 * V_1 )
{
unsigned long V_3 ;
F_6 ( V_1 -> V_5 + V_6 , V_30 ) ;
V_3 = V_4 + V_9 * 10 ;
while ( F_2 ( V_1 -> V_5 + V_6 ) != 0xffffffff ) {
if ( ! F_3 ( V_4 , V_3 ) )
return;
F_6 ( V_1 -> V_5 + V_6 , V_11 ) ;
F_4 () ;
}
F_5 ( V_1 , V_31 + V_32 , 0 ) ;
F_5 ( V_1 , V_31 + V_33 , 0 ) ;
}
static int F_18 ( T_1 * V_1 )
{
unsigned long V_3 , V_34 ;
F_6 ( V_1 -> V_5 + V_35 , 0x0c ) ;
if ( F_2 ( V_1 -> V_5 + V_35 ) != 0x0c )
return 1 ;
F_6 ( V_1 -> V_5 + V_6 , V_30 ) ;
V_3 = V_4 + V_9 * 10 ;
while ( F_2 ( V_1 -> V_5 + V_6 ) != 0xffffffff ) {
if ( ! F_3 ( V_4 , V_3 ) )
return 2 ;
F_6 ( V_1 -> V_5 + V_6 , V_11 ) ;
F_4 () ;
}
F_5 ( V_1 , V_31 + V_32 , 0 ) ;
F_5 ( V_1 , V_31 + V_33 , 0 ) ;
F_6 ( V_1 -> V_5 + V_36 , 0x55aa55aa ) ;
if ( F_2 ( V_1 -> V_5 + V_36 ) != 0x55aa55aa ) return 3 ;
F_6 ( V_1 -> V_5 + V_36 , 0xaa55aa55 ) ;
if ( F_2 ( V_1 -> V_5 + V_36 ) != 0xaa55aa55 ) return 4 ;
if ( F_5 ( V_1 , V_31 + V_37 , 0 ) ) return 5 ;
if ( F_5 ( V_1 , V_31 + V_38 , 0 ) ) return 6 ;
if ( F_5 ( V_1 , V_31 + V_39 , V_40 ) )
return 7 ;
if ( F_5 ( V_1 , V_31 + V_41 , V_42 ) )
return 8 ;
if ( F_5 ( V_1 , V_31 + V_43 , V_44 ) )
return 8 ;
if ( F_5 ( V_1 , V_31 + V_45 , 0 ) ) return 9 ;
F_19 ( 1 ) ;
if ( F_7 ( V_1 , V_46 , & V_34 ) ) return 10 ;
if ( F_7 ( V_1 , V_47 , & V_34 ) ) return 11 ;
if ( F_7 ( V_1 , V_48 , & V_34 ) ) return 12 ;
if ( F_7 ( V_1 , V_49 , & V_34 ) ) return 13 ;
if ( F_5 ( V_1 , V_46 + V_50 , 0 ) ) return 14 ;
if ( F_5 ( V_1 , V_47 + V_50 , 0 ) ) return 15 ;
if ( F_5 ( V_1 , V_48 + V_50 , 0 ) ) return 16 ;
if ( F_5 ( V_1 , V_49 + V_50 , 0 ) ) return 17 ;
F_19 ( 1 ) ;
if ( F_5 ( V_1 , V_31 + V_45 , V_51 ) )
return 18 ;
if ( F_5 ( V_1 , V_31 + V_52 , V_53 ) )
return 19 ;
if ( F_5 ( V_1 , V_31 + V_54 , V_55 ) )
return 20 ;
if ( F_5 ( V_1 , V_31 + V_56 , V_55 ) )
return 21 ;
if ( F_5 ( V_1 , V_31 + V_57 , V_55 ) )
return 22 ;
if ( F_5 ( V_1 , 0x000000 , 0x11111111 )
|| F_5 ( V_1 , 0x400000 , 0x22222222 )
|| F_5 ( V_1 , 0x800000 , 0x33333333 )
|| F_5 ( V_1 , 0xC00000 , 0x44444444 ) )
return 23 ;
if ( F_7 ( V_1 , 0x000000 , & V_34 ) || V_34 != 0x11111111
|| F_7 ( V_1 , 0x400000 , & V_34 ) || V_34 != 0x22222222
|| F_7 ( V_1 , 0x800000 , & V_34 ) || V_34 != 0x33333333
|| F_7 ( V_1 , 0xC00000 , & V_34 ) || V_34 != 0x44444444 )
return 24 ;
if ( F_5 ( V_1 , 0x000000 , 0x55555555 )
|| F_5 ( V_1 , 0x400000 , 0x66666666 )
|| F_5 ( V_1 , 0x800000 , 0x77777777 )
|| F_5 ( V_1 , 0xC00000 , 0x88888888 ) )
return 25 ;
if ( F_7 ( V_1 , 0x000000 , & V_34 ) || V_34 != 0x55555555
|| F_7 ( V_1 , 0x400000 , & V_34 ) || V_34 != 0x66666666
|| F_7 ( V_1 , 0x800000 , & V_34 ) || V_34 != 0x77777777
|| F_7 ( V_1 , 0xC00000 , & V_34 ) || V_34 != 0x88888888 )
return 26 ;
return 0 ;
}
static void F_20 ( T_1 * V_1 )
{
T_6 * V_58 = V_1 -> V_58 ;
struct V_59 * V_60 ;
T_5 V_61 , V_62 ;
T_7 V_21 ;
T_3 V_63 ;
void * V_64 ;
if ( V_1 -> V_65 & V_66 ) {
return;
}
V_60 = F_21 ( & V_58 -> V_67 ) ;
if ( ! V_60 ) {
#ifdef F_22
F_10 ( V_68 L_2 , V_1 -> V_25 ) ;
#endif
return;
}
V_21 = F_23 ( V_60 -> V_20 ) ;
if ( V_21 ) {
V_61 = F_24 ( V_60 -> V_20 ) ;
V_62 = F_25 ( V_60 -> V_20 ) ;
V_64 = V_58 -> V_69 . V_70 ;
if ( F_26 ( V_61 , V_62 ) == V_71 ) {
T_7 V_72 = F_27 ( V_60 -> V_20 ) ;
F_11 ( & V_64 , V_73 ) ;
F_13 ( & V_64 , V_60 -> V_20 , V_21 ) ;
F_13 ( & V_64 , V_60 -> V_20 + V_21 , V_72 ) ;
} else {
F_11 ( & V_64 , V_74 ) ;
F_13 ( & V_64 , V_60 -> V_20 , V_21 ) ;
}
V_63 = ( T_5 * ) V_64 - ( T_5 * ) V_58 -> V_69 . V_70 ;
#ifdef F_22
F_10 ( V_68 L_3 , V_1 -> V_25 , V_63 ) ;
#endif
} else {
V_63 = V_60 -> V_21 - 2 ;
#ifdef F_28
if ( V_60 -> V_20 [ 2 ] == V_75 )
F_10 ( V_76 L_4 , V_1 -> V_25 ) ;
#endif
#ifdef F_22
F_10 ( V_68 L_5 ,
V_1 -> V_25 , V_60 -> V_20 [ 2 ] , V_63 ) ;
#endif
F_29 ( V_60 , 2 , V_58 -> V_69 . V_70 ,
V_60 -> V_21 - 2 ) ;
}
V_63 = ( V_63 + 3 ) & ~ 3 ;
F_6 ( V_1 -> V_5 + V_77 , V_58 -> V_69 . V_78 ) ;
F_6 ( V_1 -> V_5 + V_79 , V_63 ) ;
V_1 -> V_65 |= V_66 ;
F_6 ( V_1 -> V_5 + V_6 , V_66 ) ;
F_30 ( V_60 ) ;
}
static void F_31 ( T_1 * V_1 )
{
struct V_59 * V_60 ;
void * V_64 ;
V_60 = F_32 ( 3 , V_80 ) ;
if ( ! V_60 ) {
F_10 ( V_81 L_6 ,
V_1 -> V_25 ) ;
return;
}
V_64 = V_60 -> V_20 ;
F_11 ( & V_64 , 0 ) ;
F_11 ( & V_64 , 0 ) ;
F_11 ( & V_64 , V_75 ) ;
F_33 ( V_60 , ( T_5 * ) V_64 - ( T_5 * ) V_60 -> V_20 ) ;
F_34 ( & V_1 -> V_58 -> V_67 , V_60 ) ;
F_20 ( V_1 ) ;
}
static void F_35 ( T_1 * V_1 )
{
T_6 * V_58 = V_1 -> V_58 ;
struct V_82 * V_83 ;
T_8 * V_84 ;
struct V_59 * V_60 ;
void * V_64 = V_58 -> V_85 . V_70 ;
T_3 V_86 , V_87 , V_88 , V_89 , V_90 ;
T_5 V_91 = F_14 ( & V_64 ) ;
T_3 V_92 ;
#ifdef F_22
F_10 ( V_68 L_7 , V_1 -> V_25 ,
V_91 , ( unsigned long ) V_58 -> V_93 ) ;
#endif
switch ( V_91 ) {
case V_94 :
V_86 = ( unsigned ) F_15 ( & V_64 ) ;
V_87 = F_16 ( & V_64 , V_1 -> V_95 ) ;
V_88 = F_16 ( & V_64 , V_1 -> V_96 ) ;
V_92 = F_36 ( V_1 -> V_95 ) - V_1 -> V_97 ;
if ( V_92 >= V_1 -> V_98 ) V_92 = 0 ;
V_83 = & V_1 -> V_99 [ V_92 ] . V_100 ;
if ( V_87 < 30 ) {
memset ( V_1 -> V_95 + V_87 , 0 , 30 - V_87 ) ;
V_87 = 30 ;
F_37 ( V_1 -> V_95 , 30 ) ;
}
if ( ! ( V_60 = F_32 ( V_88 + V_87 , V_80 ) ) ) {
F_10 ( V_24 L_8 ,
V_1 -> V_25 ) ;
} else {
F_38 ( V_60 , V_1 -> V_95 , V_87 ) ;
F_38 ( V_60 , V_1 -> V_96 , V_88 ) ;
F_39 ( V_83 , V_86 , V_60 ) ;
}
break;
case V_101 :
V_86 = ( unsigned ) F_15 ( & V_64 ) ;
V_87 = F_16 ( & V_64 , V_1 -> V_95 ) ;
V_92 = F_36 ( V_1 -> V_95 ) - V_1 -> V_97 ;
if ( V_92 >= V_1 -> V_98 ) V_92 = 0 ;
V_84 = & V_1 -> V_99 [ V_92 ] ;
V_83 = & V_1 -> V_99 [ V_92 ] . V_100 ;
if ( ! ( V_60 = F_32 ( V_87 , V_80 ) ) ) {
F_10 ( V_24 L_8 ,
V_1 -> V_25 ) ;
} else {
F_38 ( V_60 , V_1 -> V_95 , V_87 ) ;
if ( F_40 ( V_60 -> V_20 ) == V_102 )
F_41 ( & V_84 -> V_103 , V_86 ,
F_42 ( V_60 -> V_20 ) ,
F_43 ( V_60 -> V_20 ) ) ;
F_39 ( V_83 , V_86 , V_60 ) ;
}
break;
case V_104 :
V_86 = F_15 ( & V_64 ) ;
V_89 = F_15 ( & V_64 ) ;
V_90 = F_15 ( & V_64 ) ;
V_92 = ( V_89 & 0x7f ) - V_1 -> V_97 ;
if ( V_92 >= V_1 -> V_98 ) V_92 = 0 ;
F_44 ( & V_1 -> V_99 [ V_92 ] . V_103 , V_86 , V_89 , V_90 ) ;
break;
case V_105 :
V_86 = F_15 ( & V_64 ) ;
V_89 = F_15 ( & V_64 ) ;
if ( V_89 != 0xffffffff ) {
V_92 = ( V_89 & 0x7f ) - V_1 -> V_97 ;
if ( V_92 >= V_1 -> V_98 ) V_92 = 0 ;
F_45 ( & V_1 -> V_99 [ V_92 ] . V_103 , V_86 , V_89 ) ;
}
break;
case V_106 :
#ifdef F_28
F_10 ( V_76 L_9 , V_1 -> V_25 ) ;
#endif
if ( ! V_107 )
F_31 ( V_1 ) ;
for ( V_92 = 0 ; V_92 < V_1 -> V_108 ; V_92 ++ ) {
V_83 = & V_1 -> V_99 [ V_92 ] . V_100 ;
F_46 ( V_83 ) ;
}
break;
case V_109 :
for ( V_92 = 0 ; V_92 < V_1 -> V_108 ; V_92 ++ ) {
V_83 = & V_1 -> V_99 [ V_92 ] . V_100 ;
F_47 ( V_83 ) ;
}
break;
case V_110 :
V_92 = V_1 -> V_98 ;
if ( V_92 >= V_1 -> V_108 ) {
F_10 ( V_24 L_10 ,
V_1 -> V_25 , V_92 + 1 ) ;
break;
}
V_1 -> V_98 ++ ;
V_84 = & V_1 -> V_99 [ V_92 ] ;
V_83 = & V_84 -> V_100 ;
V_84 -> V_111 = F_16 ( & V_64 , V_84 -> V_112 ) ;
F_48 ( V_84 ) ;
F_10 ( V_76 L_11 ,
V_1 -> V_25 ,
V_84 -> V_113 [ V_114 ] ,
V_84 -> V_113 [ V_115 ] ) ;
F_49 ( & V_84 -> V_100 ) ;
break;
case V_116 :
V_86 = ( unsigned ) F_15 ( & V_64 ) ;
V_87 = F_16 ( & V_64 , V_1 -> V_95 ) ;
V_1 -> V_95 [ V_87 ] = 0 ;
while ( V_87 > 0
&& ( V_1 -> V_95 [ V_87 - 1 ] == '\n'
|| V_1 -> V_95 [ V_87 - 1 ] == '\r' ) ) {
V_1 -> V_95 [ V_87 - 1 ] = 0 ;
V_87 -- ;
}
F_10 ( V_76 L_12 ,
V_1 -> V_25 , V_86 , V_1 -> V_95 ) ;
break;
case V_117 :
V_87 = F_16 ( & V_64 , V_1 -> V_95 ) ;
V_1 -> V_95 [ V_87 ] = 0 ;
while ( V_87 > 0
&& ( V_1 -> V_95 [ V_87 - 1 ] == '\n'
|| V_1 -> V_95 [ V_87 - 1 ] == '\r' ) ) {
V_1 -> V_95 [ V_87 - 1 ] = 0 ;
V_87 -- ;
}
F_10 ( V_76 L_13 , V_1 -> V_25 , V_1 -> V_95 ) ;
break;
default:
F_10 ( V_24 L_14 ,
V_1 -> V_25 , V_91 ) ;
return;
}
}
static T_9 F_50 ( T_1 * V_1 )
{
unsigned long V_118 ;
T_3 V_119 ;
F_51 ( & V_1 -> V_120 , V_118 ) ;
V_119 = F_2 ( V_1 -> V_5 + V_6 ) ;
if ( V_119 & V_121 ) {
T_4 V_29 ;
F_6 ( V_1 -> V_5 + V_35 , 0x0c ) ;
F_52 ( & V_1 -> V_120 , V_118 ) ;
if ( V_1 -> V_98 == 0 )
return V_122 ;
F_10 ( V_24 L_15 , V_1 -> V_25 ) ;
for ( V_29 = 0 ; V_29 < V_1 -> V_108 ; V_29 ++ ) {
T_8 * V_84 = & V_1 -> V_99 [ V_29 ] ;
memset ( V_84 -> V_113 , 0 , sizeof( V_84 -> V_113 ) ) ;
F_51 ( & V_1 -> V_120 , V_118 ) ;
F_53 ( & V_84 -> V_103 ) ;
F_52 ( & V_1 -> V_120 , V_118 ) ;
F_54 ( & V_84 -> V_100 ) ;
}
V_1 -> V_98 = 0 ;
return V_122 ;
}
V_119 &= ( V_123 | V_124 ) ;
if ( ! V_119 ) {
F_52 ( & V_1 -> V_120 , V_118 ) ;
return V_122 ;
}
F_6 ( V_1 -> V_5 + V_6 , V_119 ) ;
if ( ( V_119 & V_123 ) != 0 ) {
V_1 -> V_58 -> V_93 = F_2 ( V_1 -> V_5 + V_125 ) ;
F_6 ( V_1 -> V_5 + V_125 , 0 ) ;
F_35 ( V_1 ) ;
V_1 -> V_58 -> V_93 = 0 ;
F_6 ( V_1 -> V_5 + V_125 , V_1 -> V_58 -> V_85 . V_126 ) ;
F_6 ( V_1 -> V_5 + V_6 , V_127 ) ;
}
if ( ( V_119 & V_124 ) != 0 ) {
V_1 -> V_65 &= ~ V_66 ;
F_20 ( V_1 ) ;
} else if ( V_1 -> V_65 & V_124 ) {
if ( F_2 ( V_1 -> V_5 + V_79 ) == 0 ) {
V_1 -> V_65 &= ~ V_66 ;
F_20 ( V_1 ) ;
}
}
F_52 ( & V_1 -> V_120 , V_118 ) ;
return V_122 ;
}
static T_9 F_55 ( int V_128 , void * V_129 )
{
T_1 * V_1 = V_129 ;
return F_50 ( V_1 ) ;
}
static void F_56 ( T_1 * V_1 )
{
struct V_59 * V_60 ;
void * V_64 ;
unsigned long V_118 ;
V_60 = F_32 ( 15 , V_80 ) ;
if ( ! V_60 ) {
F_10 ( V_81 L_16 ,
V_1 -> V_25 ) ;
return;
}
V_64 = V_60 -> V_20 ;
F_11 ( & V_64 , 0 ) ;
F_11 ( & V_64 , 0 ) ;
F_11 ( & V_64 , V_130 ) ;
F_12 ( & V_64 , V_131 ) ;
F_12 ( & V_64 , V_132 * 30 ) ;
F_12 ( & V_64 , V_1 -> V_97 - 1 ) ;
F_33 ( V_60 , ( T_5 * ) V_64 - ( T_5 * ) V_60 -> V_20 ) ;
F_34 ( & V_1 -> V_58 -> V_67 , V_60 ) ;
F_51 ( & V_1 -> V_120 , V_118 ) ;
F_20 ( V_1 ) ;
F_52 ( & V_1 -> V_120 , V_118 ) ;
}
static int F_57 ( T_1 * V_1 , T_3 V_16 )
{
struct V_59 * V_60 ;
unsigned long V_118 ;
void * V_64 ;
V_60 = F_32 ( 3 + 4 , V_80 ) ;
if ( ! V_60 ) {
F_10 ( V_81 L_17 ,
V_1 -> V_25 ) ;
return - V_133 ;
}
V_64 = V_60 -> V_20 ;
F_11 ( & V_64 , 0 ) ;
F_11 ( & V_64 , 0 ) ;
F_11 ( & V_64 , V_134 ) ;
F_12 ( & V_64 , V_16 ) ;
F_33 ( V_60 , ( T_5 * ) V_64 - ( T_5 * ) V_60 -> V_20 ) ;
F_34 ( & V_1 -> V_58 -> V_67 , V_60 ) ;
F_51 ( & V_1 -> V_120 , V_118 ) ;
F_20 ( V_1 ) ;
F_52 ( & V_1 -> V_120 , V_118 ) ;
return 0 ;
}
static int F_58 ( T_1 * V_1 , char V_135 [ 4 ] )
{
struct V_59 * V_60 ;
unsigned long V_118 ;
void * V_64 ;
V_60 = F_32 ( 3 + 4 , V_80 ) ;
if ( ! V_60 ) {
F_10 ( V_81 L_17 ,
V_1 -> V_25 ) ;
return - V_133 ;
}
V_64 = V_60 -> V_20 ;
F_11 ( & V_64 , 0 ) ;
F_11 ( & V_64 , 0 ) ;
F_11 ( & V_64 , V_134 ) ;
F_11 ( & V_64 , V_135 [ 0 ] ) ;
F_11 ( & V_64 , V_135 [ 1 ] ) ;
F_11 ( & V_64 , V_135 [ 2 ] ) ;
F_11 ( & V_64 , V_135 [ 3 ] ) ;
F_33 ( V_60 , ( T_5 * ) V_64 - ( T_5 * ) V_60 -> V_20 ) ;
F_34 ( & V_1 -> V_58 -> V_67 , V_60 ) ;
F_51 ( & V_1 -> V_120 , V_118 ) ;
F_20 ( V_1 ) ;
F_52 ( & V_1 -> V_120 , V_118 ) ;
return 0 ;
}
static int F_59 ( T_1 * V_1 , T_2 * V_136 )
{
T_5 V_16 [ 4 ] ;
unsigned char * V_17 ;
T_4 V_18 ;
int V_137 ;
if ( ( V_137 = F_57 ( V_1 , 1 ) ) != 0 )
return V_137 ;
if ( ( V_137 = F_57 ( V_1 , V_136 -> V_21 ) ) != 0 )
return V_137 ;
V_17 = V_136 -> V_20 ;
V_18 = V_136 -> V_21 ;
while ( V_18 >= sizeof( T_3 ) ) {
if ( V_136 -> V_22 ) {
if ( F_9 ( V_16 , V_17 , sizeof( V_16 ) ) )
return - V_23 ;
} else {
memcpy ( V_16 , V_17 , sizeof( V_16 ) ) ;
}
if ( ( V_137 = F_58 ( V_1 , V_16 ) ) != 0 )
return V_137 ;
V_18 -= sizeof( V_16 ) ;
V_17 += sizeof( V_16 ) ;
}
if ( V_18 ) {
memset ( V_16 , 0 , sizeof( V_16 ) ) ;
if ( V_136 -> V_22 ) {
if ( F_9 ( & V_16 , V_17 , V_18 ) )
return - V_23 ;
} else {
memcpy ( & V_16 , V_17 , V_18 ) ;
}
if ( ( V_137 = F_58 ( V_1 , V_16 ) ) != 0 )
return V_137 ;
}
return 0 ;
}
static int F_60 ( struct V_82 * V_83 , T_10 * V_20 )
{
T_8 * V_84 = ( T_8 * ) ( V_83 -> V_138 ) ;
T_1 * V_1 = V_84 -> V_1 ;
int V_137 ;
if ( ( V_137 = F_8 ( V_1 , & V_20 -> V_139 ) ) ) {
F_10 ( V_24 L_18 ,
V_1 -> V_25 ) ;
F_17 ( V_1 ) ;
return V_137 ;
}
V_1 -> V_65 = 0 ;
F_6 ( V_1 -> V_5 + V_125 , 0 ) ;
F_6 ( V_1 -> V_5 + V_79 , 0 ) ;
F_6 ( V_1 -> V_5 + V_6 , V_140 ) ;
F_19 ( 1 ) ;
F_6 ( V_1 -> V_5 + V_6 ,
V_123 | V_124 | V_121 ) ;
F_6 ( V_1 -> V_5 + V_35 , 0x08 ) ;
V_1 -> V_58 -> V_93 = 0 ;
F_6 ( V_1 -> V_5 + V_141 , V_1 -> V_58 -> V_85 . V_78 ) ;
F_6 ( V_1 -> V_5 + V_125 , V_1 -> V_58 -> V_85 . V_126 ) ;
F_6 ( V_1 -> V_5 + V_6 , V_127 ) ;
if ( V_20 -> V_142 . V_21 > 0 && V_20 -> V_142 . V_20 ) {
V_137 = F_59 ( V_1 , & V_20 -> V_142 ) ;
if ( V_137 ) {
F_10 ( V_24 L_19 ,
V_1 -> V_25 ) ;
F_17 ( V_1 ) ;
return V_137 ;
}
}
F_56 ( V_1 ) ;
return 0 ;
}
static void F_61 ( struct V_82 * V_83 )
{
T_1 * V_1 = ( ( T_8 * ) ( V_83 -> V_138 ) ) -> V_1 ;
T_8 * V_84 ;
T_4 V_29 ;
unsigned long V_118 ;
F_51 ( & V_1 -> V_120 , V_118 ) ;
F_17 ( V_1 ) ;
F_52 ( & V_1 -> V_120 , V_118 ) ;
for ( V_29 = 0 ; V_29 < V_1 -> V_108 ; V_29 ++ ) {
V_84 = & V_1 -> V_99 [ V_29 ] ;
memset ( V_84 -> V_113 , 0 , sizeof( V_84 -> V_113 ) ) ;
F_54 ( & V_84 -> V_100 ) ;
}
V_1 -> V_98 = 0 ;
}
static void F_62 ( struct V_143 * V_144 )
{
T_1 * V_1 = F_63 ( V_144 ) ;
T_8 * V_84 ;
T_4 V_29 ;
if ( ! V_1 )
return;
F_17 ( V_1 ) ;
for ( V_29 = 0 ; V_29 < V_1 -> V_108 ; V_29 ++ ) {
V_84 = & V_1 -> V_99 [ V_29 ] ;
F_64 ( & V_84 -> V_100 ) ;
}
F_65 ( V_1 -> V_145 , V_1 ) ;
F_66 ( V_1 -> V_5 ) ;
F_67 ( V_1 -> V_146 , V_147 ) ;
F_68 ( V_1 -> V_58 ) ;
F_69 ( V_144 , NULL ) ;
F_70 ( V_1 ) ;
}
static void F_71 ( struct V_82 * V_83 ,
T_7 V_148 ,
T_11 * V_149 )
{
T_8 * V_84 = ( T_8 * ) ( V_83 -> V_138 ) ;
T_1 * V_1 = V_84 -> V_1 ;
struct V_59 * V_60 ;
int V_150 = V_149 -> V_151 ;
unsigned long V_118 ;
int V_152 ;
void * V_64 ;
if ( V_83 -> V_153 == V_1 -> V_97 ) {
if ( V_150 > 0 ) V_152 = V_150 ;
else V_152 = V_83 -> V_154 . V_155 * 4 * - V_150 ;
if ( V_152 == 0 ) V_152 = V_83 -> V_154 . V_155 * 4 ;
V_60 = F_32 ( 23 , V_80 ) ;
if ( ! V_60 ) {
F_10 ( V_81 L_16 ,
V_1 -> V_25 ) ;
return;
}
V_64 = V_60 -> V_20 ;
F_11 ( & V_64 , 0 ) ;
F_11 ( & V_64 , 0 ) ;
F_11 ( & V_64 , V_156 ) ;
F_12 ( & V_64 , V_148 ) ;
F_12 ( & V_64 , 1024 * ( V_152 + 1 ) ) ;
F_12 ( & V_64 , V_152 ) ;
F_12 ( & V_64 , V_149 -> V_157 ) ;
F_12 ( & V_64 , V_149 -> V_158 ) ;
F_33 ( V_60 , ( T_5 * ) V_64 - ( T_5 * ) V_60 -> V_20 ) ;
F_34 ( & V_1 -> V_58 -> V_67 , V_60 ) ;
F_51 ( & V_1 -> V_120 , V_118 ) ;
F_20 ( V_1 ) ;
F_52 ( & V_1 -> V_120 , V_118 ) ;
}
}
static void F_72 ( struct V_82 * V_83 , T_7 V_148 )
{
T_8 * V_84 = ( T_8 * ) ( V_83 -> V_138 ) ;
T_1 * V_1 = V_84 -> V_1 ;
unsigned long V_118 ;
struct V_59 * V_60 ;
void * V_64 ;
F_51 ( & V_1 -> V_120 , V_118 ) ;
F_73 ( & V_84 -> V_103 , V_148 ) ;
F_52 ( & V_1 -> V_120 , V_118 ) ;
if ( V_83 -> V_153 == V_1 -> V_97 ) {
V_60 = F_32 ( 7 , V_80 ) ;
if ( ! V_60 ) {
F_10 ( V_81 L_20 ,
V_1 -> V_25 ) ;
return;
}
V_64 = V_60 -> V_20 ;
F_11 ( & V_64 , 0 ) ;
F_11 ( & V_64 , 0 ) ;
F_11 ( & V_64 , V_159 ) ;
F_12 ( & V_64 , V_148 ) ;
F_33 ( V_60 , ( T_5 * ) V_64 - ( T_5 * ) V_60 -> V_20 ) ;
F_34 ( & V_1 -> V_58 -> V_67 , V_60 ) ;
F_51 ( & V_1 -> V_120 , V_118 ) ;
F_20 ( V_1 ) ;
F_52 ( & V_1 -> V_120 , V_118 ) ;
}
}
static T_7 F_74 ( struct V_82 * V_83 , struct V_59 * V_60 )
{
T_8 * V_84 = ( T_8 * ) ( V_83 -> V_138 ) ;
T_1 * V_1 = V_84 -> V_1 ;
T_7 V_137 = V_160 ;
unsigned long V_118 ;
F_51 ( & V_1 -> V_120 , V_118 ) ;
if ( F_40 ( V_60 -> V_20 ) == V_71 ) {
V_137 = F_75 ( & V_84 -> V_103 ,
F_76 ( V_60 -> V_20 ) ,
F_42 ( V_60 -> V_20 ) ,
F_43 ( V_60 -> V_20 ) ) ;
}
if ( V_137 == V_160 ) {
F_34 ( & V_1 -> V_58 -> V_67 , V_60 ) ;
F_20 ( V_1 ) ;
}
F_52 ( & V_1 -> V_120 , V_118 ) ;
return V_137 ;
}
static char * F_77 ( struct V_82 * V_83 )
{
T_8 * V_84 = ( T_8 * ) ( V_83 -> V_138 ) ;
if ( ! V_84 )
return L_21 ;
sprintf ( V_84 -> V_161 , L_22 ,
V_84 -> V_162 [ 0 ] ? V_84 -> V_162 : L_23 ,
V_84 -> V_113 [ V_115 ] ? V_84 -> V_113 [ V_115 ] : L_23 ,
V_84 -> V_1 ? V_84 -> V_1 -> V_146 : 0x0 ,
V_84 -> V_1 ? V_84 -> V_1 -> V_145 : 0 ,
V_84 -> V_1 ? V_84 -> V_1 -> V_163 : 0
) ;
return V_84 -> V_161 ;
}
static int F_78 ( struct V_164 * V_165 , void * V_166 )
{
struct V_82 * V_83 = V_165 -> V_167 ;
T_8 * V_84 = ( T_8 * ) ( V_83 -> V_138 ) ;
T_1 * V_1 = V_84 -> V_1 ;
T_5 V_168 ;
char * V_28 ;
F_79 ( V_165 , L_24 , L_25 , V_1 -> V_25 ) ;
F_79 ( V_165 , L_26 , L_27 , V_1 -> V_146 ) ;
F_79 ( V_165 , L_28 , L_29 , V_1 -> V_145 ) ;
F_79 ( V_165 , L_30 , L_31 , V_1 -> V_163 ) ;
switch ( V_1 -> V_169 ) {
case V_170 : V_28 = L_32 ; break;
case V_171 : V_28 = L_33 ; break;
case V_172 : V_28 = L_34 ; break;
case V_173 : V_28 = L_35 ; break;
case V_174 : V_28 = L_36 ; break;
case V_175 : V_28 = L_37 ; break;
case V_176 : V_28 = L_38 ; break;
case V_177 : V_28 = L_39 ; break;
case V_178 : V_28 = L_40 ; break;
default: V_28 = L_41 ; break;
}
F_79 ( V_165 , L_24 , L_42 , V_28 ) ;
if ( ( V_28 = V_84 -> V_113 [ V_115 ] ) != NULL )
F_79 ( V_165 , L_24 , L_43 , V_28 ) ;
if ( ( V_28 = V_84 -> V_113 [ V_114 ] ) != NULL )
F_79 ( V_165 , L_24 , L_44 , V_28 ) ;
if ( ( V_28 = V_84 -> V_113 [ V_179 ] ) != NULL )
F_79 ( V_165 , L_24 , L_45 , V_28 ) ;
if ( V_1 -> V_169 != V_173 ) {
V_168 = ( ( T_5 * ) ( V_83 -> V_154 . V_180 ) ) [ 3 ] ;
if ( V_168 )
F_79 ( V_165 , L_46 ,
L_47 ,
( V_168 & 0x01 ) ? L_48 : L_21 ,
( V_168 & 0x02 ) ? L_49 : L_21 ,
( V_168 & 0x04 ) ? L_50 : L_21 ,
( V_168 & 0x08 ) ? L_51 : L_21 ,
( V_168 & 0x10 ) ? L_52 : L_21 ,
( V_168 & 0x20 ) ? L_53 : L_21 ,
( V_168 & 0x40 ) ? L_54 : L_21
) ;
}
if ( V_1 -> V_169 != V_173 ) {
V_168 = ( ( T_5 * ) ( V_83 -> V_154 . V_180 ) ) [ 5 ] ;
if ( V_168 )
F_79 ( V_165 , L_55 ,
L_56 ,
( V_168 & 0x01 ) ? L_57 : L_21 ,
( V_168 & 0x02 ) ? L_58 : L_21 ,
( V_168 & 0x08 ) ? L_59 : L_21 ,
( V_168 & 0x04 ) ? L_60 : L_21
) ;
}
F_79 ( V_165 , L_24 , L_61 , V_84 -> V_162 ) ;
return 0 ;
}
static int F_80 ( struct V_181 * V_181 , struct V_182 * V_182 )
{
return F_81 ( V_182 , F_78 , F_82 ( V_181 ) ) ;
}
static int F_83 ( struct V_183 * V_64 , struct V_143 * V_184 ,
int V_108 )
{
T_1 * V_1 ;
T_8 * V_84 ;
int V_137 ;
int V_29 ;
V_1 = F_84 ( V_108 ) ;
if ( ! V_1 ) {
F_10 ( V_185 L_62 ) ;
V_137 = - V_133 ;
goto V_186;
}
V_1 -> V_58 = F_85 ( L_63 , V_184 , 2048 + 128 , 2048 + 128 ) ;
if ( ! V_1 -> V_58 ) {
F_10 ( V_185 L_62 ) ;
V_137 = - V_133 ;
goto V_187;
}
sprintf ( V_1 -> V_25 , L_64 , V_108 , V_64 -> V_146 ) ;
V_1 -> V_146 = V_64 -> V_146 ;
V_1 -> V_145 = V_64 -> V_145 ;
V_1 -> V_163 = V_64 -> V_163 ;
V_1 -> V_169 = ( V_108 == 4 ) ? V_177 : V_178 ;
if ( ! F_86 ( V_1 -> V_146 , V_147 , V_1 -> V_25 ) ) {
F_10 ( V_185 L_65 ,
V_1 -> V_146 , V_1 -> V_146 + V_147 ) ;
V_137 = - V_188 ;
goto V_189;
}
V_1 -> V_5 = F_87 ( V_1 -> V_163 , 128 ) ;
if ( V_1 -> V_5 == NULL ) {
F_10 ( V_190 L_66 ,
V_1 -> V_163 ) ;
V_137 = - V_26 ;
goto V_191;
}
V_137 = F_18 ( V_1 ) ;
if ( V_137 != 0 ) {
F_10 ( V_190 L_67 ,
V_1 -> V_146 , V_137 ) ;
V_137 = - V_26 ;
goto V_192;
}
F_17 ( V_1 ) ;
V_137 = F_88 ( V_1 -> V_145 , F_55 , V_193 , V_1 -> V_25 , V_1 ) ;
if ( V_137 ) {
F_10 ( V_24 L_68 , V_1 -> V_145 ) ;
V_137 = - V_188 ;
goto V_192;
}
for ( V_29 = 0 ; V_29 < V_108 ; V_29 ++ ) {
V_84 = & V_1 -> V_99 [ V_29 ] ;
V_84 -> V_100 . V_194 = V_195 ;
V_84 -> V_100 . V_196 = L_63 ;
V_84 -> V_100 . V_138 = V_84 ;
V_84 -> V_100 . V_197 = F_71 ;
V_84 -> V_100 . V_198 = F_72 ;
V_84 -> V_100 . V_199 = F_74 ;
V_84 -> V_100 . V_200 = F_60 ;
V_84 -> V_100 . V_201 = F_61 ;
V_84 -> V_100 . V_202 = F_77 ;
V_84 -> V_100 . V_203 = & V_204 ;
strcpy ( V_84 -> V_100 . V_25 , V_1 -> V_25 ) ;
V_137 = F_89 ( & V_84 -> V_100 ) ;
if ( V_137 ) {
F_10 ( V_24 L_69 , V_29 ) ;
for ( V_29 -- ; V_29 >= 0 ; V_29 -- ) {
V_84 = & V_1 -> V_99 [ V_29 ] ;
F_64 ( & V_84 -> V_100 ) ;
}
goto V_205;
}
if ( V_29 == 0 )
V_1 -> V_97 = V_84 -> V_100 . V_153 ;
}
F_10 ( V_76 L_70 ,
V_108 , V_1 -> V_146 , V_1 -> V_145 ,
V_1 -> V_163 ) ;
F_69 ( V_184 , V_1 ) ;
return 0 ;
V_205:
F_65 ( V_1 -> V_145 , V_1 ) ;
V_192:
F_66 ( V_1 -> V_5 ) ;
V_191:
F_67 ( V_1 -> V_146 , V_147 ) ;
V_189:
F_68 ( V_1 -> V_58 ) ;
V_187:
F_70 ( V_1 ) ;
V_186:
return V_137 ;
}
static int F_90 ( struct V_143 * V_184 , const struct V_206 * V_207 )
{
int V_208 = V_207 -> V_209 ;
int V_137 = 0 ;
struct V_183 V_210 ;
if ( F_91 ( V_184 ) < 0 ) {
F_10 ( V_24 L_71 , V_208 ) ;
return - V_211 ;
}
F_92 ( V_184 ) ;
V_210 . V_146 = F_93 ( V_184 , 1 ) ;
V_210 . V_145 = V_184 -> V_145 ;
V_210 . V_163 = F_93 ( V_184 , 0 ) ;
F_10 ( V_76 L_72 ,
V_208 , V_210 . V_146 , V_210 . V_145 , V_210 . V_163 ) ;
V_137 = F_83 ( & V_210 , V_184 , V_208 ) ;
if ( V_137 != 0 ) {
F_10 ( V_24 L_73 ,
V_208 , V_210 . V_146 , V_210 . V_145 , V_210 . V_163 ) ;
F_94 ( V_184 ) ;
return - V_211 ;
}
return 0 ;
}
static int T_12 F_95 ( void )
{
char * V_64 ;
char V_212 [ 32 ] ;
int V_186 ;
if ( ( V_64 = strchr ( V_213 , ':' ) ) != NULL && V_64 [ 1 ] ) {
F_96 ( V_212 , V_64 + 2 , 32 ) ;
if ( ( V_64 = strchr ( V_212 , '$' ) ) != NULL && V_64 > V_212 )
* ( V_64 - 1 ) = 0 ;
} else
strcpy ( V_212 , L_74 ) ;
V_186 = F_97 ( & V_214 ) ;
if ( ! V_186 ) {
F_96 ( V_215 . V_213 , V_212 , 32 ) ;
F_98 ( & V_215 ) ;
F_96 ( V_216 . V_213 , V_212 , 32 ) ;
F_98 ( & V_216 ) ;
F_10 ( V_76 L_75 , V_212 ) ;
}
return V_186 ;
}
static void T_13 F_99 ( void )
{
F_100 ( & V_215 ) ;
F_100 ( & V_216 ) ;
F_101 ( & V_214 ) ;
}
