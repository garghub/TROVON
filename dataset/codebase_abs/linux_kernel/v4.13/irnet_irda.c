static void F_1 ( struct V_1 * V_2 )
{
T_1 * V_3 =
F_2 ( V_2 , T_1 , V_4 ) ;
if ( V_3 == NULL )
return;
if ( V_3 -> V_5 && ! V_3 -> V_6 && ! V_3 -> V_7 ) {
F_3 ( & V_3 -> V_8 ) ;
V_3 -> V_5 = 0 ;
}
}
static void
F_4 ( T_1 * V_9 ,
T_2 V_10 ,
T_3 V_11 ,
T_3 V_12 ,
char * V_13 ,
T_4 V_14 )
{
int V_15 ;
F_5 ( V_16 , L_1 ,
V_9 , V_10 , V_12 , V_13 ) ;
F_6 ( & V_17 . V_18 ) ;
V_15 = V_17 . V_15 ;
V_17 . log [ V_15 ] . V_10 = V_10 ;
V_17 . log [ V_15 ] . V_12 = V_12 ;
V_17 . log [ V_15 ] . V_11 = V_11 ;
if( V_13 )
strcpy ( V_17 . log [ V_15 ] . V_13 , V_13 ) ;
else
V_17 . log [ V_15 ] . V_13 [ 0 ] = '\0' ;
V_17 . log [ V_15 ] . V_14 . V_19 = V_14 ;
if( ( V_9 != ( T_1 * ) NULL ) && ( V_9 -> V_5 ) )
V_17 . log [ V_15 ] . V_20 = F_7 ( & V_9 -> V_8 ) ;
else
V_17 . log [ V_15 ] . V_20 = - 1 ;
V_17 . V_15 = ( V_15 + 1 ) % V_21 ;
F_8 ( V_22 , L_2 , V_17 . V_15 ) ;
F_9 ( & V_17 . V_18 ) ;
F_10 ( & V_17 . V_23 ) ;
F_11 ( V_16 , L_3 ) ;
}
static inline int
F_12 ( T_1 * V_3 )
{
T_5 V_24 ;
F_5 ( V_25 , L_4 , V_3 ) ;
F_13 ( V_3 -> V_26 != NULL , - V_27 , V_28 , L_5 ) ;
F_14 ( & V_24 ) ;
V_24 . V_29 = V_30 ;
V_24 . V_31 = V_32 ;
V_24 . V_33 = V_34 ;
V_24 . V_35 = V_36 ;
V_24 . V_37 = V_38 ;
V_24 . V_39 = V_40 ;
V_24 . V_41 = V_3 ;
F_15 ( V_24 . V_13 , V_42 , sizeof( V_24 . V_13 ) ) ;
V_3 -> V_26 = F_16 ( V_43 , V_44 ,
& V_24 ) ;
F_13 ( V_3 -> V_26 == NULL , - V_45 ,
V_28 , L_6 ) ;
V_3 -> V_46 = V_3 -> V_26 -> V_46 ;
F_11 ( V_25 , L_7 ,
V_3 -> V_26 , V_3 -> V_46 ) ;
return 0 ;
}
static inline T_6
F_17 ( T_1 * V_3 ,
int V_47 ,
struct V_48 * V_49 )
{
T_6 V_50 = 0 ;
F_5 ( V_25 , L_4 , V_3 ) ;
V_3 -> V_51 = 0 ;
switch( V_47 )
{
case V_52 :
case V_53 :
F_8 ( V_54 , L_8 , V_47 ) ;
V_3 -> V_51 = - V_55 ;
break;
default :
F_8 ( V_54 , L_9 , V_47 ) ;
V_3 -> V_51 = - V_56 ;
break;
case V_57 :
break;
}
if( V_49 != NULL )
{
switch( V_49 -> type )
{
case V_58 :
F_8 ( V_54 , L_10 , V_49 -> V_59 . integer ) ;
if( V_49 -> V_59 . integer != - 1 )
V_50 = V_49 -> V_59 . integer ;
else
V_3 -> V_51 = - V_55 ;
break;
default:
V_3 -> V_51 = - V_55 ;
F_18 ( V_28 , L_11 , V_49 -> type ) ;
break;
}
F_19 ( V_49 ) ;
}
else
{
if( ! ( V_3 -> V_51 ) )
{
F_18 ( V_28 ,
L_12 ) ;
V_3 -> V_51 = - V_56 ;
}
}
F_11 ( V_25 , L_3 ) ;
return V_50 ;
}
static inline int
F_20 ( T_1 * V_3 )
{
F_5 ( V_25 , L_4 , V_3 ) ;
F_13 ( V_3 -> V_60 , - V_27 , V_28 , L_13 ) ;
V_3 -> V_60 = F_21 ( V_43 , V_61 , V_3 ,
V_62 ) ;
V_3 -> V_51 = - V_56 ;
F_22 ( V_3 -> V_60 , V_3 -> V_63 , V_3 -> V_12 ,
V_64 , V_65 ) ;
F_11 ( V_25 , L_3 ) ;
return 0 ;
}
static inline int
F_23 ( T_1 * V_3 )
{
int V_66 ;
F_5 ( V_25 , L_4 , V_3 ) ;
V_66 = F_12 ( V_3 ) ;
if( V_66 != 0 )
{
F_24 ( 0 , & V_3 -> V_7 ) ;
F_18 ( V_28 , L_14 ) ;
return V_66 ;
}
V_66 = F_25 ( V_3 -> V_26 , V_3 -> V_50 ,
V_3 -> V_63 , V_3 -> V_12 , NULL ,
V_3 -> V_67 , NULL ) ;
if( V_66 != 0 )
{
F_24 ( 0 , & V_3 -> V_7 ) ;
F_18 ( V_28 , L_14 ) ;
return V_66 ;
}
F_11 ( V_25 , L_3 ) ;
return V_66 ;
}
static inline int
F_26 ( T_1 * V_3 )
{
if( V_3 -> V_60 )
{
F_27 ( V_3 -> V_60 ) ;
V_3 -> V_60 = NULL ;
}
V_3 -> V_60 = F_21 ( V_43 , V_61 , V_3 ,
V_68 ) ;
if( V_3 -> V_60 == NULL )
return - V_45 ;
V_3 -> V_69 ++ ;
if( V_3 -> V_69 < V_3 -> V_70 )
{
F_22 ( V_3 -> V_60 ,
V_3 -> V_71 [ V_3 -> V_69 ] . V_11 ,
V_3 -> V_71 [ V_3 -> V_69 ] . V_12 ,
V_64 , V_65 ) ;
return 0 ;
}
else
return 1 ;
}
static inline int
F_28 ( T_1 * V_3 )
{
int V_72 ;
F_5 ( V_25 , L_4 , V_3 ) ;
V_3 -> V_71 = F_29 ( & V_3 -> V_70 , V_3 -> V_73 ,
V_74 ) ;
if( V_3 -> V_71 == NULL )
{
V_3 -> V_70 = - 1 ;
F_24 ( 0 , & V_3 -> V_7 ) ;
F_30 ( - V_75 , V_54 , L_15 ) ;
}
F_8 ( V_54 , L_16 ,
V_3 -> V_71 , V_3 -> V_70 ) ;
V_3 -> V_69 = - 1 ;
V_3 -> V_12 = V_76 ;
V_72 = F_26 ( V_3 ) ;
if( V_72 )
{
if( V_3 -> V_60 )
F_27 ( V_3 -> V_60 ) ;
V_3 -> V_60 = NULL ;
F_31 ( V_3 -> V_71 ) ;
V_3 -> V_71 = NULL ;
F_24 ( 0 , & V_3 -> V_7 ) ;
F_30 ( - V_75 , V_54 , L_17 ) ;
}
F_11 ( V_25 , L_3 ) ;
return 0 ;
}
static inline int
F_32 ( T_1 * V_3 )
{
struct V_77 * V_71 ;
int V_78 ;
int V_79 ;
F_5 ( V_25 , L_4 , V_3 ) ;
V_71 = F_29 ( & V_78 , 0xffff ,
V_74 ) ;
if( V_71 == NULL )
F_30 ( - V_75 , V_54 , L_17 ) ;
for( V_79 = 0 ; V_79 < V_78 ; V_79 ++ )
{
if( ! strncmp ( V_71 [ V_79 ] . V_80 , V_3 -> V_81 , V_82 ) )
{
V_3 -> V_12 = V_71 [ V_79 ] . V_12 ;
F_8 ( V_54 , L_18 ,
V_3 -> V_81 , V_3 -> V_12 ) ;
F_31 ( V_71 ) ;
F_11 ( V_25 , L_3 ) ;
return 0 ;
}
}
F_8 ( V_54 , L_19 , V_3 -> V_81 ) ;
F_31 ( V_71 ) ;
return - V_55 ;
}
int
F_33 ( T_1 * V_3 )
{
F_5 ( V_83 , L_4 , V_3 ) ;
V_3 -> V_84 = V_85 ;
V_3 -> V_6 = 0 ;
V_3 -> V_7 = 0 ;
V_3 -> V_81 [ 0 ] = '\0' ;
V_3 -> V_86 = V_76 ;
V_3 -> V_63 = V_76 ;
V_3 -> V_12 = V_76 ;
V_3 -> V_11 = V_76 ;
V_3 -> V_67 = V_87 ;
V_3 -> V_88 = F_34 ( 0 , NULL , NULL , NULL ) ;
#ifdef F_35
V_3 -> V_73 = 0xffff ;
#else
V_3 -> V_73 = F_36 ( V_89 ) ;
#endif
V_3 -> V_90 = V_91 ;
F_37 ( & V_3 -> V_4 , F_1 ) ;
F_11 ( V_83 , L_3 ) ;
return 0 ;
}
int
F_38 ( T_1 * V_3 )
{
int V_66 ;
F_5 ( V_83 , L_4 , V_3 ) ;
if( F_39 ( 0 , & V_3 -> V_7 ) )
F_30 ( - V_27 , V_92 , L_20 ) ;
if( ( V_3 -> V_60 != NULL ) || ( V_3 -> V_26 != NULL ) )
F_18 ( V_93 , L_21 ) ;
if( ( V_94 . V_95 ) && ( V_3 -> V_96 . V_97 == NULL ) )
{
F_6 ( & V_94 . V_18 ) ;
F_40 ( V_94 . V_98 , ( V_99 * ) V_3 , 0 , V_3 -> V_81 ) ;
F_9 ( & V_94 . V_18 ) ;
F_8 ( V_92 , L_22 , V_3 -> V_81 ) ;
}
if( ( V_3 -> V_86 == V_76 ) && ( V_3 -> V_81 [ 0 ] == '\0' ) )
{
if( ( V_66 = F_28 ( V_3 ) ) != 0 )
F_30 ( V_66 , V_92 , L_23 ) ;
}
else
{
if( V_3 -> V_86 == V_76 )
{
if( ( V_66 = F_32 ( V_3 ) ) != 0 )
F_30 ( V_66 , V_92 , L_24 ) ;
}
else
V_3 -> V_12 = V_3 -> V_86 ;
F_20 ( V_3 ) ;
}
F_11 ( V_83 , L_3 ) ;
return 0 ;
}
void
F_41 ( T_1 * V_3 )
{
F_5 ( V_83 , L_4 , V_3 ) ;
if( V_3 == NULL )
return;
if( ( V_94 . V_95 ) && ( V_3 -> V_96 . V_97 != NULL ) )
{
struct T_1 * V_100 ;
F_8 ( V_92 , L_25 ) ;
F_6 ( & V_94 . V_18 ) ;
V_100 = F_42 ( V_94 . V_98 , ( V_99 * ) V_3 ) ;
V_3 -> V_96 . V_97 = NULL ;
F_9 ( & V_94 . V_18 ) ;
F_43 ( V_100 == V_3 , , V_93 , L_26 ) ;
}
if( F_44 ( 0 , & V_3 -> V_6 ) )
{
F_4 ( NULL , V_101 ,
V_3 -> V_11 , V_3 -> V_12 , V_3 -> V_81 , 0 ) ;
}
F_24 ( 0 , & V_3 -> V_7 ) ;
F_24 ( 0 , & V_3 -> V_6 ) ;
F_45 ( V_3 -> V_88 ) ;
if( V_3 -> V_60 )
{
F_27 ( V_3 -> V_60 ) ;
V_3 -> V_60 = NULL ;
}
if( V_3 -> V_71 != NULL )
{
F_31 ( V_3 -> V_71 ) ;
V_3 -> V_71 = NULL ;
}
if( V_3 -> V_26 )
{
F_8 ( V_92 , L_27 ) ;
F_46 ( V_3 -> V_26 , NULL , V_102 ) ;
F_47 ( V_3 -> V_26 ) ;
V_3 -> V_26 = NULL ;
}
V_3 -> V_46 = 0 ;
F_11 ( V_83 , L_3 ) ;
}
static inline int
F_48 ( T_1 * V_3 )
{
struct V_77 * V_71 ;
int V_78 ;
int V_79 ;
F_5 ( V_103 , L_4 , V_3 ) ;
V_71 = F_29 ( & V_78 , 0xffff ,
V_74 ) ;
if ( V_71 == NULL )
F_30 ( - V_75 , V_104 , L_17 ) ;
for( V_79 = 0 ; V_79 < V_78 ; V_79 ++ )
{
if( V_71 [ V_79 ] . V_12 == V_3 -> V_12 )
{
F_15 ( V_3 -> V_81 , V_71 [ V_79 ] . V_80 , sizeof( V_3 -> V_81 ) ) ;
V_3 -> V_81 [ sizeof( V_3 -> V_81 ) - 1 ] = '\0' ;
F_8 ( V_104 , L_28 ,
V_3 -> V_12 , V_3 -> V_81 ) ;
F_31 ( V_71 ) ;
F_11 ( V_103 , L_3 ) ;
return 0 ;
}
}
F_11 ( V_104 , L_29 , V_3 -> V_12 ) ;
F_31 ( V_71 ) ;
return - V_55 ;
}
static inline T_1 *
F_49 ( T_1 * V_3 )
{
T_1 * V_105 = ( T_1 * ) NULL ;
int V_66 ;
F_5 ( V_103 , L_4 , V_3 ) ;
V_3 -> V_12 = F_50 ( V_3 -> V_26 ) ;
V_3 -> V_11 = F_51 ( V_3 -> V_26 ) ;
V_66 = F_48 ( V_3 ) ;
F_6 ( & V_94 . V_18 ) ;
if( V_66 == 0 )
{
V_105 = ( T_1 * ) F_52 ( V_94 . V_98 ,
0 , V_3 -> V_81 ) ;
if( V_105 )
F_8 ( V_104 , L_30 ,
V_105 , V_105 -> V_81 ) ;
}
if( V_105 == ( T_1 * ) NULL )
{
V_105 = ( T_1 * ) F_53 ( V_94 . V_98 ) ;
while( V_105 != ( T_1 * ) NULL )
{
if( ( V_105 -> V_86 == V_3 -> V_12 ) || ( V_105 -> V_12 == V_3 -> V_12 ) )
{
F_8 ( V_104 , L_31 ,
V_105 , V_3 -> V_12 ) ;
break;
}
V_105 = ( T_1 * ) F_54 ( V_94 . V_98 ) ;
}
}
if( V_105 == ( T_1 * ) NULL )
{
V_105 = ( T_1 * ) F_53 ( V_94 . V_98 ) ;
while( V_105 != ( T_1 * ) NULL )
{
if( ! ( F_44 ( 0 , & V_105 -> V_6 ) ) && ( V_105 -> V_86 == V_76 ) &&
( V_105 -> V_81 [ 0 ] == '\0' ) && ( V_105 -> V_5 ) )
{
F_8 ( V_104 , L_32 ,
V_105 ) ;
break;
}
V_105 = ( T_1 * ) F_54 ( V_94 . V_98 ) ;
}
}
F_9 ( & V_94 . V_18 ) ;
F_11 ( V_103 , L_33 , V_105 ) ;
return V_105 ;
}
static inline int
F_55 ( T_1 * V_106 ,
T_1 * V_105 ,
struct V_107 * V_108 ,
T_3 V_109 ,
T_6 V_110 )
{
F_5 ( V_103 , L_34 ,
V_106 , V_105 ) ;
V_105 -> V_26 = F_56 ( V_106 -> V_26 , V_105 ) ;
F_13 ( V_105 -> V_26 == NULL , - 1 , V_111 , L_35 ) ;
V_105 -> V_46 = V_105 -> V_26 -> V_46 ;
V_105 -> V_50 = V_105 -> V_26 -> V_50 ;
V_105 -> V_11 = F_51 ( V_105 -> V_26 ) ;
V_105 -> V_12 = F_50 ( V_105 -> V_26 ) ;
V_105 -> V_110 = V_110 ;
V_105 -> V_112 = V_109 ;
V_105 -> V_113 = V_109 ;
#ifdef F_57
if( V_109 == 0 )
V_105 -> V_113 = F_58 ( V_105 -> V_26 ) ;
#endif
F_59 ( V_106 -> V_26 ) ;
F_60 ( V_105 -> V_26 , V_105 -> V_67 , NULL ) ;
F_61 ( 0 , & V_105 -> V_6 ) ;
F_24 ( 0 , & V_105 -> V_7 ) ;
if( V_105 -> V_60 )
{
F_27 ( V_105 -> V_60 ) ;
V_105 -> V_60 = NULL ;
}
if( V_105 -> V_71 != NULL )
{
F_31 ( V_105 -> V_71 ) ;
V_105 -> V_71 = NULL ;
}
#ifdef F_62
F_63 ( & V_105 -> V_8 ) ;
#endif
F_4 ( V_105 , V_114 ,
V_105 -> V_11 , V_105 -> V_12 , V_106 -> V_81 , 0 ) ;
F_11 ( V_103 , L_3 ) ;
return 0 ;
}
static inline void
F_64 ( T_1 * V_3 ,
struct V_115 * V_116 )
{
F_5 ( V_103 , L_4 , V_3 ) ;
F_65 ( V_116 ) ;
#ifdef F_66
F_46 ( V_3 -> V_26 , NULL , V_102 ) ;
#endif
F_4 ( NULL , V_117 ,
V_3 -> V_11 , V_3 -> V_12 , V_3 -> V_81 , 0 ) ;
F_59 ( V_3 -> V_26 ) ;
F_11 ( V_103 , L_3 ) ;
}
static inline int
F_67 ( void )
{
T_4 V_14 ;
F_5 ( V_103 , L_36 ) ;
F_33 ( & V_94 . V_118 ) ;
F_12 ( & V_94 . V_118 ) ;
V_94 . V_118 . V_5 = 0 ;
V_94 . V_118 . V_8 . V_119 = NULL ;
V_94 . V_118 . V_120 = NULL ;
V_14 = F_36 ( V_89 ) ;
#ifdef F_68
V_94 . V_121 = F_69 ( V_14 ) ;
#endif
V_94 . V_122 = F_70 ( V_64 , V_123 ) ;
F_71 ( V_94 . V_122 , V_65 ,
V_94 . V_118 . V_46 , V_124 ) ;
F_72 ( V_94 . V_122 ) ;
#ifdef F_73
F_74 ( V_94 . V_118 . V_88 , V_14 ,
V_125 , V_126 ,
( void * ) & V_94 . V_118 ) ;
#endif
F_11 ( V_103 , L_37 , & V_94 . V_118 ) ;
return 0 ;
}
static inline void
F_75 ( void )
{
F_5 ( V_103 , L_36 ) ;
#ifdef F_68
F_76 ( V_94 . V_121 ) ;
#endif
if( V_94 . V_122 )
F_77 ( V_94 . V_122 ) ;
F_41 ( & V_94 . V_118 ) ;
F_11 ( V_103 , L_3 ) ;
}
static int
V_36 ( void * V_41 ,
void * V_127 ,
struct V_115 * V_116 )
{
T_1 * V_9 = ( T_1 * ) V_41 ;
unsigned char * V_128 ;
int V_129 = 0 ;
F_5 ( V_130 , L_38 ,
V_9 , V_116 ) ;
F_43 ( V_116 != NULL , 0 , V_131 , L_39 ) ;
if( ! V_9 -> V_5 )
{
F_18 ( V_131 , L_40 ) ;
return - V_45 ;
}
V_128 = V_116 -> V_132 ;
if( ( V_128 [ 0 ] == V_133 ) && ( V_128 [ 1 ] == V_134 ) )
{
if( V_116 -> V_135 < 3 )
goto V_136;
V_128 = F_78 ( V_116 , 2 ) ;
}
if( V_128 [ 0 ] & 1 )
{
* ( V_137 * ) F_79 ( V_116 , 1 ) = 0 ;
}
else
if( V_116 -> V_135 < 2 )
goto V_136;
F_80 ( & V_9 -> V_8 , V_116 ) ;
F_11 ( V_130 , L_3 ) ;
return 0 ;
V_136:
F_18 ( V_131 , L_41 ) ;
F_65 ( V_116 ) ;
F_81 ( & V_9 -> V_8 , V_129 ) ;
return 0 ;
}
static void
V_34 ( void * V_41 ,
void * V_127 ,
T_7 V_138 ,
struct V_115 * V_116 )
{
T_1 * V_3 = ( T_1 * ) V_41 ;
int V_139 ;
int V_140 ;
F_5 ( V_130 , L_4 , V_3 ) ;
F_43 ( V_3 != NULL , , V_131 , L_42 ) ;
if( V_116 )
F_82 ( V_116 ) ;
V_139 = F_83 ( 0 , & V_3 -> V_6 ) ;
V_140 = F_83 ( 0 , & V_3 -> V_7 ) ;
if( ! ( V_139 || V_140 ) )
{
F_18 ( V_131 , L_43 ) ;
return;
}
if( V_139 )
F_4 ( V_3 , V_141 ,
V_3 -> V_11 , V_3 -> V_12 , V_3 -> V_81 , 0 ) ;
else
if( ( V_3 -> V_26 ) && ( V_3 != & V_94 . V_118 ) )
F_4 ( V_3 , V_142 ,
V_3 -> V_11 , V_3 -> V_12 , V_3 -> V_81 , 0 ) ;
if( ( V_3 -> V_26 ) && ( V_3 != & V_94 . V_118 ) )
{
F_8 ( V_143 , L_27 ) ;
F_47 ( V_3 -> V_26 ) ;
V_3 -> V_26 = NULL ;
}
V_3 -> V_46 = 0 ;
V_3 -> V_12 = V_76 ;
V_3 -> V_90 = V_91 ;
if( V_3 -> V_5 )
{
if( V_139 )
{
F_84 ( & V_3 -> V_4 ) ;
}
else
{
F_63 ( & V_3 -> V_8 ) ;
}
}
F_11 ( V_130 , L_3 ) ;
}
static void
V_30 ( void * V_41 ,
void * V_127 ,
struct V_107 * V_108 ,
T_3 V_109 ,
T_6 V_110 ,
struct V_115 * V_116 )
{
T_1 * V_3 = ( T_1 * ) V_41 ;
F_5 ( V_130 , L_4 , V_3 ) ;
if( ! F_44 ( 0 , & V_3 -> V_7 ) )
{
F_18 ( V_131 , L_44 ) ;
return;
}
V_3 -> V_110 = V_110 ;
V_3 -> V_112 = V_109 ;
V_3 -> V_113 = V_109 ;
#ifdef F_57
if( V_109 == 0 )
V_3 -> V_113 = F_58 ( V_3 -> V_26 ) ;
#endif
V_3 -> V_11 = F_51 ( V_3 -> V_26 ) ;
F_61 ( 0 , & V_3 -> V_6 ) ;
F_24 ( 0 , & V_3 -> V_7 ) ;
F_63 ( & V_3 -> V_8 ) ;
if( V_116 -> V_135 > 0 )
{
#ifdef F_85
F_8 ( V_143 , L_45 ) ;
V_36 ( V_41 , V_127 , V_116 ) ;
#else
F_18 ( V_131 , L_46 ) ;
F_65 ( V_116 ) ;
#endif
}
else
F_65 ( V_116 ) ;
F_4 ( V_3 , V_144 ,
V_3 -> V_11 , V_3 -> V_12 , V_3 -> V_81 , 0 ) ;
F_11 ( V_130 , L_3 ) ;
}
static void
V_38 ( void * V_41 ,
void * V_127 ,
T_8 V_145 )
{
T_1 * V_3 = ( T_1 * ) V_41 ;
T_8 V_146 = V_3 -> V_90 ;
F_5 ( V_130 , L_47 , V_3 , V_145 ) ;
V_3 -> V_90 = V_145 ;
switch( V_145 )
{
case V_91 :
F_8 ( V_143 , L_48 ) ;
if( V_146 == V_147 )
F_63 ( & V_3 -> V_8 ) ;
else
F_8 ( V_143 , L_49 ) ;
break;
case V_147 :
F_8 ( V_143 , L_50 ) ;
break;
default:
F_8 ( V_143 , L_51 ) ;
break;
}
F_11 ( V_130 , L_3 ) ;
}
static void
V_40 ( void * V_41 ,
T_9 V_148 ,
T_10 V_149 )
{
T_1 * V_3 = ( T_1 * ) V_41 ;
F_5 ( V_130 , L_4 , V_3 ) ;
F_43 ( V_3 != NULL , , V_131 , L_42 ) ;
switch( V_148 )
{
case V_150 :
F_4 ( V_3 , V_151 ,
V_3 -> V_11 , V_3 -> V_12 , V_3 -> V_81 , 0 ) ;
break;
default:
F_8 ( V_143 , L_52 ) ;
}
F_11 ( V_130 , L_3 ) ;
}
static void
V_32 ( void * V_41 ,
void * V_127 ,
struct V_107 * V_108 ,
T_3 V_109 ,
T_6 V_110 ,
struct V_115 * V_116 )
{
T_1 * V_106 = & V_94 . V_118 ;
T_1 * V_105 = ( T_1 * ) NULL ;
F_5 ( V_130 , L_53 , V_106 ) ;
F_43 ( V_41 == & V_94 , , V_131 ,
L_54 , V_41 ) ;
F_43 ( V_127 == V_94 . V_118 . V_26 , , V_131 , L_55 ) ;
V_105 = F_49 ( V_106 ) ;
if( V_105 == ( T_1 * ) NULL )
{
F_11 ( V_143 , L_56 ) ;
F_64 ( V_106 , V_116 ) ;
return;
}
if( F_44 ( 0 , & V_105 -> V_6 ) )
{
F_11 ( V_143 , L_57 ) ;
F_64 ( V_106 , V_116 ) ;
return;
}
if( 0
#ifdef F_86
|| ( ( F_87 ( V_106 -> V_26 ) == 1 ) &&
( F_83 ( 0 , & V_105 -> V_7 ) ) )
#endif
)
{
F_18 ( V_131 , L_58 ) ;
if( V_105 -> V_26 != NULL )
{
F_47 ( V_105 -> V_26 ) ;
V_105 -> V_26 = NULL ;
}
}
else
{
if( ( F_44 ( 0 , & V_105 -> V_7 ) ) || ( V_105 -> V_26 != NULL ) )
{
F_18 ( V_131 , L_59 ) ;
F_64 ( V_106 , V_116 ) ;
return;
}
}
F_55 ( V_106 , V_105 , V_108 , V_109 , V_110 ) ;
if( V_116 -> V_135 > 0 )
{
#ifdef F_85
F_8 ( V_143 , L_45 ) ;
V_36 ( V_105 , V_105 -> V_26 , V_116 ) ;
#else
F_18 ( V_131 , L_46 ) ;
F_65 ( V_116 ) ;
#endif
}
else
F_65 ( V_116 ) ;
F_11 ( V_130 , L_3 ) ;
}
static void
V_62 ( int V_47 ,
T_4 V_152 ,
struct V_48 * V_49 ,
void * V_153 )
{
T_1 * V_3 = ( T_1 * ) V_153 ;
F_5 ( V_154 , L_4 , V_3 ) ;
F_43 ( V_3 != NULL , , V_155 , L_42 ) ;
if( ! F_44 ( 0 , & V_3 -> V_7 ) )
{
F_18 ( V_155 , L_44 ) ;
return;
}
F_27 ( V_3 -> V_60 ) ;
V_3 -> V_60 = NULL ;
V_3 -> V_50 = F_17 ( V_3 , V_47 , V_49 ) ;
if( V_3 -> V_51 )
{
F_24 ( 0 , & V_3 -> V_7 ) ;
F_18 ( V_155 , L_60 , V_3 -> V_51 ) ;
return;
}
F_8 ( V_156 , L_61 ,
V_3 -> V_12 , V_3 -> V_50 ) ;
F_23 ( V_3 ) ;
F_11 ( V_154 , L_3 ) ;
}
static void
V_68 ( int V_47 ,
T_4 V_152 ,
struct V_48 * V_49 ,
void * V_153 )
{
T_1 * V_3 = ( T_1 * ) V_153 ;
T_6 V_50 ;
F_5 ( V_154 , L_4 , V_3 ) ;
F_43 ( V_3 != NULL , , V_155 , L_42 ) ;
if( ! F_44 ( 0 , & V_3 -> V_7 ) )
{
F_18 ( V_155 , L_44 ) ;
return;
}
V_50 = F_17 ( V_3 , V_47 , V_49 ) ;
if( V_3 -> V_51 == 0 )
{
if( V_3 -> V_12 != V_76 )
{
F_18 ( V_155 , L_62 ) ;
}
else
{
V_3 -> V_12 = V_3 -> V_71 [ V_3 -> V_69 ] . V_12 ;
V_3 -> V_50 = V_50 ;
}
}
if( ( V_3 -> V_51 == - V_55 ) || ( V_3 -> V_51 == 0 ) )
{
int V_72 ;
V_72 = F_26 ( V_3 ) ;
if( ! V_72 )
{
return;
}
}
F_27 ( V_3 -> V_60 ) ;
V_3 -> V_60 = NULL ;
F_8 ( V_156 , L_63 ,
V_3 -> V_71 ) ;
if( V_3 -> V_71 != NULL )
{
F_31 ( V_3 -> V_71 ) ;
V_3 -> V_71 = NULL ;
}
V_3 -> V_70 = - 1 ;
if( V_3 -> V_12 == V_76 )
{
V_3 -> V_12 = V_76 ;
F_24 ( 0 , & V_3 -> V_7 ) ;
F_11 ( V_154 , L_64 ) ;
return;
}
F_8 ( V_156 , L_61 ,
V_3 -> V_12 , V_3 -> V_50 ) ;
F_23 ( V_3 ) ;
F_11 ( V_154 , L_3 ) ;
}
static void
V_125 ( T_11 * V_157 ,
T_12 V_158 ,
void * V_153 )
{
T_1 * V_3 = & V_94 . V_118 ;
F_5 ( V_154 , L_4 , V_3 ) ;
F_43 ( V_153 == & V_94 , , V_155 ,
L_54 , V_153 ) ;
F_8 ( V_156 , L_65 ,
V_157 -> V_80 ) ;
F_4 ( NULL , V_159 ,
V_157 -> V_11 , V_157 -> V_12 , V_157 -> V_80 ,
F_88 ( ( T_4 * ) V_157 -> V_14 ) ) ;
F_11 ( V_154 , L_3 ) ;
}
static void
V_126 ( T_11 * V_160 ,
T_12 V_158 ,
void * V_153 )
{
T_1 * V_3 = & V_94 . V_118 ;
F_5 ( V_154 , L_4 , V_3 ) ;
F_43 ( V_153 == & V_94 , , V_155 ,
L_54 , V_153 ) ;
F_8 ( V_156 , L_66 ,
V_160 -> V_80 ) ;
F_4 ( NULL , V_161 ,
V_160 -> V_11 , V_160 -> V_12 , V_160 -> V_80 ,
F_88 ( ( T_4 * ) V_160 -> V_14 ) ) ;
F_11 ( V_154 , L_3 ) ;
}
static int
F_89 ( struct V_162 * V_163 , void * V_164 )
{
T_1 * V_3 ;
char * V_165 ;
int V_79 = 0 ;
F_90 ( V_163 , L_67 ) ;
F_90 ( V_163 , L_68 ,
( V_94 . V_95 ? L_69 : L_70 ) ) ;
F_90 ( V_163 , L_71 , V_94 . V_118 . V_46 ) ;
F_90 ( V_163 , L_72 , V_94 . V_118 . V_50 ) ;
if( ! V_94 . V_95 )
return 0 ;
F_6 ( & V_94 . V_18 ) ;
V_3 = ( T_1 * ) F_53 ( V_94 . V_98 ) ;
while( V_3 != NULL )
{
F_90 ( V_163 , L_73 , V_79 ++ ) ;
F_90 ( V_163 , L_74 , V_3 -> V_81 ) ;
F_90 ( V_163 , L_75 , V_3 -> V_86 ) ;
F_90 ( V_163 , L_76 , V_3 -> V_63 ) ;
F_90 ( V_163 , L_77 ,
( V_3 -> V_5 ? L_78 : L_79 ) ) ;
if( V_3 -> V_5 )
{
F_90 ( V_163 , L_80 ,
F_7 ( & V_3 -> V_8 ) ) ;
F_90 ( V_163 , L_81 ,
F_91 ( & V_3 -> V_8 ) ) ;
F_90 ( V_163 , L_82 ,
V_3 -> V_166 ) ;
}
if( V_3 -> V_6 )
V_165 = L_83 ;
else
if( V_3 -> V_26 != NULL )
V_165 = L_84 ;
else
if( V_3 -> V_60 != NULL )
V_165 = L_85 ;
else
if( V_3 -> V_7 )
V_165 = L_86 ;
else
V_165 = L_87 ;
F_90 ( V_163 , L_88 , V_165 ) ;
F_90 ( V_163 , L_75 , V_3 -> V_12 ) ;
F_90 ( V_163 , L_71 , V_3 -> V_46 ) ;
F_90 ( V_163 , L_72 , V_3 -> V_50 ) ;
V_3 = ( T_1 * ) F_54 ( V_94 . V_98 ) ;
}
F_9 ( & V_94 . V_18 ) ;
return 0 ;
}
static int F_92 ( struct V_167 * V_167 , struct V_120 * V_120 )
{
return F_93 ( V_120 , F_89 , NULL ) ;
}
int T_13
F_94 ( void )
{
int V_66 = 0 ;
F_5 ( V_168 , L_36 ) ;
memset ( & V_94 , 0 , sizeof( struct V_169 ) ) ;
V_94 . V_98 = F_95 ( V_170 ) ;
F_13 ( V_94 . V_98 == NULL , - V_45 ,
V_171 , L_89 ) ;
F_96 ( & V_94 . V_18 ) ;
F_97 ( & V_17 . V_23 ) ;
V_17 . V_15 = 0 ;
F_96 ( & V_17 . V_18 ) ;
#ifdef F_98
F_99 ( L_90 , 0 , V_172 , & V_173 ) ;
#endif
V_66 = F_67 () ;
if( ! V_66 )
V_94 . V_95 = 1 ;
F_11 ( V_168 , L_3 ) ;
return V_66 ;
}
void T_14
F_100 ( void )
{
F_5 ( V_168 , L_36 ) ;
V_94 . V_95 = 0 ;
#ifdef F_98
F_101 ( L_90 , V_172 ) ;
#endif
F_75 () ;
F_102 ( V_94 . V_98 , ( V_174 ) F_41 ) ;
F_11 ( V_168 , L_3 ) ;
}
