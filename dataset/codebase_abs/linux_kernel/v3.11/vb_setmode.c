void F_1 ( unsigned char V_1 , struct V_2 * V_3 )
{
V_3 -> V_4 = V_5 ;
V_3 -> V_6 = 0 ;
V_3 -> V_7 = 0 ;
V_3 -> V_8 = 0 ;
V_3 -> V_9 = 0 ;
V_3 -> V_10 = 0 ;
V_3 -> V_11 = V_12 ;
V_3 -> V_13 = V_14 ;
if ( V_1 < V_15 )
F_2 ( V_3 ) ;
if ( ( V_3 -> V_16 & V_17 ) || ( V_3 -> V_16 & V_18 ) )
V_3 -> V_19 = V_20 ;
else
V_3 -> V_19 = V_21 ;
if ( V_1 >= V_15 )
V_3 -> V_22 = 0x10 ;
if ( V_1 == V_23 ) {
unsigned char V_24 ;
V_3 -> V_4 = V_25 ;
V_3 -> V_13 = V_26 ;
V_3 -> V_22 = 0xc1 ;
V_3 -> V_11 = V_27 ;
V_24 = F_3 ( V_3 -> V_28 , 0x3B ) ;
if ( ( ( V_24 & 0x88 ) == 0x80 ) || ( ( V_24 & 0x88 ) == 0x08 ) )
V_3 -> V_22 = 0x80 ;
}
}
static void F_4 ( unsigned short V_29 ,
unsigned short V_30 ,
struct V_2 * V_3 )
{
unsigned char V_31 , V_32 ;
F_5 ( V_3 -> V_28 , 0x00 , 0x03 ) ;
for ( V_32 = 0 ; V_32 < 4 ; V_32 ++ ) {
V_31 = V_33 . V_34 [ V_32 ] ;
F_5 ( V_3 -> V_28 , V_32 + 1 , V_31 ) ;
}
}
static void F_6 ( struct V_35 * V_36 ,
struct V_2 * V_3 )
{
unsigned char V_37 ;
unsigned short V_32 ;
V_37 = F_3 ( V_3 -> V_38 , 0x11 ) ;
V_37 &= 0x7f ;
F_5 ( V_3 -> V_38 , 0x11 , V_37 ) ;
for ( V_32 = 0 ; V_32 <= 0x18 ; V_32 ++ ) {
V_37 = V_33 . V_39 [ V_32 ] ;
F_5 ( V_3 -> V_38 , V_32 , V_37 ) ;
}
}
static void F_7 ( unsigned short V_29 ,
unsigned short V_30 ,
struct V_2 * V_3 )
{
unsigned char V_40 ;
unsigned short V_32 , V_41 ;
V_41 = V_42 [ V_30 ] . V_43 ;
for ( V_32 = 0 ; V_32 <= 0x13 ; V_32 ++ ) {
V_40 = V_33 . V_44 [ V_32 ] ;
if ( ( V_41 & V_45 ) && V_32 == 0x13 ) {
if ( V_3 -> V_9 & V_46 ) {
V_40 = 0 ;
} else if ( ( V_3 -> V_9 &
( V_47 | V_48 ) ) &&
( V_3 -> V_9 & V_49 ) ) {
V_40 = 0 ;
}
}
F_8 ( V_3 -> V_50 ) ;
F_9 ( V_32 , V_3 -> V_51 ) ;
F_9 ( V_40 , V_3 -> V_51 ) ;
}
F_8 ( V_3 -> V_50 ) ;
F_9 ( 0x14 , V_3 -> V_51 ) ;
F_9 ( 0x00 , V_3 -> V_51 ) ;
F_8 ( V_3 -> V_50 ) ;
F_9 ( 0x20 , V_3 -> V_51 ) ;
}
static void F_10 ( struct V_2 * V_3 )
{
unsigned char V_52 ;
unsigned short V_32 ;
for ( V_32 = 0 ; V_32 <= 0x08 ; V_32 ++ ) {
V_52 = V_33 . V_53 [ V_32 ] ;
F_5 ( V_3 -> V_54 , V_32 , V_52 ) ;
}
if ( V_3 -> V_55 > V_56 ) {
V_52 = F_3 ( V_3 -> V_54 , 0x05 ) ;
V_52 &= 0xBF ;
F_5 ( V_3 -> V_54 , 0x05 , V_52 ) ;
}
}
static void F_11 ( struct V_2 * V_3 )
{
unsigned short V_32 ;
for ( V_32 = 0x0A ; V_32 <= 0x0E ; V_32 ++ )
F_5 ( V_3 -> V_28 , V_32 , 0x00 ) ;
}
static unsigned char F_12 ( struct V_2 * V_3 )
{
F_13 ( V_3 -> V_28 , 0x31 , ~ 0x30 , 0x20 ) ;
F_5 ( V_3 -> V_28 , 0x2B , V_57 [ 0 ] . V_58 ) ;
F_5 ( V_3 -> V_28 , 0x2C , V_57 [ 0 ] . V_59 ) ;
F_13 ( V_3 -> V_28 , 0x31 , ~ 0x30 , 0x10 ) ;
F_5 ( V_3 -> V_28 , 0x2B , V_57 [ 1 ] . V_58 ) ;
F_5 ( V_3 -> V_28 , 0x2C , V_57 [ 1 ] . V_59 ) ;
F_14 ( V_3 -> V_28 , 0x31 , ~ 0x30 ) ;
return 0 ;
}
static unsigned char F_15 ( unsigned short V_29 ,
unsigned short V_30 ,
unsigned short V_60 , unsigned short * V_32 ,
struct V_2 * V_3 )
{
unsigned short V_61 , V_62 , V_63 , V_41 , V_64 ;
V_41 = V_42 [ V_30 ] . V_43 ;
V_63 = V_42 [ V_30 ] . V_65 ;
V_62 = V_66 [ V_60 + ( * V_32 ) ] . V_67 ;
V_61 = 0 ;
if ( V_3 -> V_9 & V_68 ) {
V_61 |= V_69 ;
if ( V_3 -> V_16 & V_70 )
V_61 |= V_71 ;
}
if ( V_3 -> V_9 & ( V_48 | V_46 ) ) {
V_61 |= V_72 ;
if ( V_3 -> V_6 != V_73 &&
V_3 -> V_6 != V_74 &&
( V_3 -> V_8 & V_75 ) &&
V_63 >= 9 )
return 0 ;
}
if ( V_3 -> V_9 & V_76 ) {
V_61 |= V_77 ;
if ( ( V_3 -> V_9 & V_49 ) &&
( ( V_63 == 4 ) ||
( V_63 == 3 && ( V_3 -> V_78 & V_79 ) ) ||
( V_63 > 7 ) ) )
return 0 ;
} else if ( V_3 -> V_9 & ( V_80 | V_81 |
V_82 | V_83 |
V_76 ) ) {
V_61 |= V_84 ;
if ( V_3 -> V_16 & ( V_85 | V_86 | V_17 |
V_18 | V_70 ) )
V_61 |= V_87 ;
if ( ! ( V_3 -> V_9 & V_88 ) &&
( V_41 & V_89 ) &&
( V_3 -> V_9 & V_49 ) &&
( ! ( V_3 -> V_9 & V_90 ) ) )
return 0 ;
}
for (; V_66 [ V_60 + ( * V_32 ) ] . V_67 ==
V_62 ; ( * V_32 ) -- ) {
V_64 = V_66 [ V_60 + ( * V_32 ) ] .
V_91 ;
if ( V_64 & V_61 )
return 1 ;
if ( ( * V_32 ) == 0 )
break;
}
for ( ( * V_32 ) = 0 ; ; ( * V_32 ) ++ ) {
V_64 = V_66 [ V_60 + ( * V_32 ) ] .
V_91 ;
if ( V_66 [ V_60 + ( * V_32 ) ] . V_67
!= V_62 ) {
return 0 ;
}
if ( V_64 & V_61 )
return 1 ;
}
return 1 ;
}
static void F_16 ( unsigned short V_60 ,
struct V_2 * V_3 )
{
unsigned short V_92 , V_24 ;
V_92 = V_66 [ V_60 ] . V_91 >> 8 ;
V_92 &= 0xC0 ;
V_24 = 0x2F ;
V_24 |= V_92 ;
F_9 ( V_24 , V_3 -> V_93 ) ;
}
static void F_17 ( struct V_2 * V_3 ,
struct V_35 * V_36 )
{
unsigned char V_94 , V_95 , V_96 ;
unsigned short V_32 , V_97 ;
V_94 = F_3 ( V_3 -> V_38 , 0x11 ) ;
V_94 &= 0x7F ;
F_5 ( V_3 -> V_38 , 0x11 , V_94 ) ;
V_94 = V_3 -> V_98 . V_94 [ 0 ] ;
F_5 ( V_3 -> V_38 , 0 , V_94 ) ;
for ( V_32 = 0x01 ; V_32 <= 0x04 ; V_32 ++ ) {
V_94 = V_3 -> V_98 . V_94 [ V_32 ] ;
F_5 ( V_3 -> V_38 , ( unsigned short ) ( V_32 + 1 ) , V_94 ) ;
}
for ( V_32 = 0x05 ; V_32 <= 0x06 ; V_32 ++ ) {
V_94 = V_3 -> V_98 . V_94 [ V_32 ] ;
F_5 ( V_3 -> V_28 , ( unsigned short ) ( V_32 + 6 ) , V_94 ) ;
}
V_97 = F_3 ( V_3 -> V_28 , 0x0e ) ;
V_97 &= 0x1F ;
V_94 = V_3 -> V_98 . V_94 [ 7 ] ;
V_94 &= 0xE0 ;
V_94 |= V_97 ;
F_5 ( V_3 -> V_28 , 0x0e , V_94 ) ;
if ( V_36 -> V_99 >= V_15 ) {
V_94 = F_3 ( V_3 -> V_38 , 0x04 ) ;
V_94 = V_94 - 1 ;
F_5 ( V_3 -> V_38 , 0x04 , V_94 ) ;
V_94 = F_3 ( V_3 -> V_38 , 0x05 ) ;
V_95 = V_94 ;
V_95 &= 0xE0 ;
V_94 &= 0x1F ;
if ( V_94 == 0 ) {
V_96 = V_94 ;
V_94 = F_3 ( V_3 -> V_28 , 0x0c ) ;
V_94 &= 0xFB ;
F_5 ( V_3 -> V_28 , 0x0c , V_94 ) ;
V_94 = V_96 ;
}
V_94 = V_94 - 1 ;
V_94 |= V_95 ;
F_5 ( V_3 -> V_38 , 0x05 , V_94 ) ;
V_94 = F_3 ( V_3 -> V_28 , 0x0e ) ;
V_94 = V_94 >> 5 ;
V_94 = V_94 + 3 ;
if ( V_94 > 7 )
V_94 = V_94 - 7 ;
V_94 = V_94 << 5 ;
F_13 ( V_3 -> V_28 , 0x0e , ~ 0xE0 , V_94 ) ;
}
}
static void F_18 ( unsigned short V_30 ,
unsigned short V_29 ,
struct V_2 * V_3 )
{
unsigned char V_94 ;
unsigned short V_32 , V_97 ;
for ( V_32 = 0x00 ; V_32 <= 0x01 ; V_32 ++ ) {
V_94 = V_3 -> V_100 . V_94 [ V_32 ] ;
F_5 ( V_3 -> V_38 , ( unsigned short ) ( V_32 + 6 ) , V_94 ) ;
}
for ( V_32 = 0x02 ; V_32 <= 0x03 ; V_32 ++ ) {
V_94 = V_3 -> V_100 . V_94 [ V_32 ] ;
F_5 ( V_3 -> V_38 , ( unsigned short ) ( V_32 + 0x0e ) , V_94 ) ;
}
for ( V_32 = 0x04 ; V_32 <= 0x05 ; V_32 ++ ) {
V_94 = V_3 -> V_100 . V_94 [ V_32 ] ;
F_5 ( V_3 -> V_38 , ( unsigned short ) ( V_32 + 0x11 ) , V_94 ) ;
}
V_97 = F_3 ( V_3 -> V_28 , 0x0a ) ;
V_97 &= 0xC0 ;
V_94 = V_3 -> V_100 . V_94 [ 6 ] ;
V_94 &= 0x3F ;
V_94 |= V_97 ;
F_5 ( V_3 -> V_28 , 0x0a , V_94 ) ;
V_94 = V_3 -> V_100 . V_94 [ 6 ] ;
V_94 &= 0x80 ;
V_94 = V_94 >> 2 ;
V_32 = V_42 [ V_30 ] . V_43 ;
V_32 &= V_101 ;
if ( V_32 )
V_94 |= 0x80 ;
V_97 = F_3 ( V_3 -> V_38 , 0x09 ) ;
V_97 &= 0x5F ;
V_94 |= V_97 ;
F_5 ( V_3 -> V_38 , 0x09 , V_94 ) ;
}
static void F_19 ( unsigned short V_29 , unsigned short V_30 ,
unsigned short V_60 ,
struct V_2 * V_3 ,
struct V_35 * V_36 )
{
unsigned char V_102 , V_94 ;
unsigned short V_32 ;
V_102 = V_66 [ V_60 ] . V_103 ;
V_102 = V_102 & V_104 ;
V_94 = F_3 ( V_3 -> V_38 , 0x11 ) ;
V_94 &= 0x7F ;
F_5 ( V_3 -> V_38 , 0x11 , V_94 ) ;
for ( V_32 = 0 ; V_32 < 8 ; V_32 ++ )
V_3 -> V_98 . V_94 [ V_32 ]
= V_105 [ V_102 ] . V_106 [ V_32 ] ;
for ( V_32 = 0 ; V_32 < 7 ; V_32 ++ )
V_3 -> V_100 . V_94 [ V_32 ]
= V_105 [ V_102 ] . V_106 [ V_32 + 8 ] ;
F_17 ( V_3 , V_36 ) ;
F_18 ( V_30 , V_29 , V_3 ) ;
if ( V_3 -> V_55 > 0x03 )
F_5 ( V_3 -> V_38 , 0x14 , 0x4F ) ;
}
static void F_20 ( unsigned short V_29 , unsigned short V_30 ,
unsigned short V_60 ,
struct V_2 * V_3 )
{
unsigned char V_102 , V_107 , V_108 , V_109 , V_110 ;
unsigned short V_111 , V_112 , V_113 ;
V_102 = V_66 [ V_60 ] . V_103 ;
V_107 = V_105 [ V_102 ] . V_106 [ 3 ] ;
V_109 = V_107 ;
F_5 ( V_3 -> V_28 , 0x2E , V_107 ) ;
V_110 = V_105 [ V_102 ] . V_106 [ 5 ] ;
V_110 &= 0xC0 ;
V_111 = V_110 ;
V_111 <<= 2 ;
V_111 |= V_107 ;
V_107 = V_105 [ V_102 ] . V_106 [ 4 ] ;
V_107 &= 0x1F ;
V_108 = V_105 [ V_102 ] . V_106 [ 6 ] ;
V_108 &= 0x04 ;
V_108 <<= 3 ;
V_107 |= V_108 ;
V_112 = V_111 & 0x3C0 ;
V_112 |= V_107 ;
V_109 &= 0x3F ;
if ( V_107 < V_109 )
V_112 |= 0x40 ;
V_112 &= 0xFF ;
V_107 = ( unsigned char ) V_112 ;
V_107 <<= 2 ;
V_110 >>= 6 ;
V_107 |= V_110 ;
F_5 ( V_3 -> V_28 , 0x2F , V_107 ) ;
F_13 ( V_3 -> V_28 , 0x30 , 0xE3 , 00 ) ;
V_107 = V_105 [ V_102 ] . V_106 [ 10 ] ;
V_108 = V_107 ;
V_107 &= 0x01 ;
F_21 ( V_3 -> V_28 , 0x33 , V_107 ) ;
V_107 = V_105 [ V_102 ] . V_106 [ 9 ] ;
V_109 = V_108 >> 1 ;
V_110 = V_107 & 0x04 ;
V_110 <<= 5 ;
V_109 |= V_110 ;
F_5 ( V_3 -> V_28 , 0x34 , V_109 ) ;
V_111 = V_110 ;
V_111 <<= 1 ;
V_111 |= V_108 ;
V_107 &= 0x80 ;
V_112 = V_107 << 2 ;
V_111 |= V_112 ;
V_107 = V_105 [ V_102 ] . V_106 [ 14 ] ;
V_107 &= 0x08 ;
V_112 = V_107 ;
V_112 <<= 7 ;
V_111 |= V_112 ;
V_107 = V_105 [ V_102 ] . V_106 [ 11 ] ;
V_107 &= 0x0F ;
V_108 = V_105 [ V_102 ] . V_106 [ 14 ] ;
V_108 &= 0x20 ;
V_108 >>= 1 ;
V_107 |= V_108 ;
V_112 = V_111 & 0x7E0 ;
V_112 |= V_107 ;
V_113 = V_111 & 0x1F ;
if ( V_107 < V_113 )
V_112 |= 0x20 ;
V_112 &= 0xFF ;
V_107 = ( unsigned char ) V_112 ;
V_107 <<= 2 ;
V_111 &= 0x600 ;
V_111 >>= 9 ;
V_108 = ( unsigned char ) V_111 ;
V_107 |= V_108 ;
V_107 &= 0x7F ;
F_5 ( V_3 -> V_28 , 0x3F , V_107 ) ;
}
static void F_22 ( unsigned short V_29 ,
unsigned short V_30 ,
unsigned short V_60 ,
struct V_2 * V_3 )
{
unsigned short V_102 , V_107 , V_108 , V_109 ;
V_102 = V_66 [ V_60 ] . V_103 ;
V_107 = V_105 [ V_102 ] . V_106 [ 3 ] ;
V_108 = V_107 ;
F_5 ( V_3 -> V_28 , 0x2E , V_107 ) ;
V_107 = V_105 [ V_102 ] . V_106 [ 5 ] ;
V_107 &= 0xC0 ;
V_108 |= ( V_107 << 2 ) ;
V_107 = V_105 [ V_102 ] . V_106 [ 4 ] ;
V_107 &= 0x1F ;
V_109 = V_107 ;
V_107 = V_105 [ V_102 ] . V_106 [ 6 ] ;
V_107 &= 0x04 ;
V_107 <<= 3 ;
V_109 |= V_107 ;
V_108 = V_108 & 0x3C0 ;
V_108 |= V_109 ;
V_107 = V_105 [ V_102 ] . V_106 [ 3 ] ;
V_107 &= 0x3F ;
if ( V_109 <= V_107 )
V_108 += 0x40 ;
V_107 = V_105 [ V_102 ] . V_106 [ 5 ] ;
V_107 &= 0xC0 ;
V_107 >>= 6 ;
V_107 |= ( ( V_108 << 2 ) & 0xFF ) ;
F_5 ( V_3 -> V_28 , 0x2F , V_107 ) ;
F_13 ( V_3 -> V_28 , 0x30 , 0xE3 , 00 ) ;
V_107 = V_105 [ V_102 ] . V_106 [ 10 ] ;
F_5 ( V_3 -> V_28 , 0x34 , V_107 ) ;
V_109 = V_107 ;
V_107 = V_105 [ V_102 ] . V_106 [ 9 ] ;
V_108 = V_107 ;
V_107 = V_107 & 0x04 ;
V_107 >>= 2 ;
F_13 ( V_3 -> V_28 , 0x35 , ~ 0x01 , V_107 ) ;
V_109 |= ( V_107 << 8 ) ;
V_109 |= ( ( V_108 & 0x80 ) << 2 ) ;
V_107 = V_105 [ V_102 ] . V_106 [ 14 ] ;
V_107 &= 0x08 ;
V_109 |= ( V_107 << 7 ) ;
V_107 = V_105 [ V_102 ] . V_106 [ 11 ] ;
V_107 &= 0x0F ;
V_108 = V_105 [ V_102 ] . V_106 [ 14 ] ;
V_108 &= 0x20 ;
V_108 >>= 1 ;
V_107 |= V_108 ;
V_108 = V_109 ;
V_108 &= 0x7E0 ;
V_108 |= V_107 ;
if ( V_108 <= V_109 )
V_108 |= 0x20 ;
V_107 = ( V_108 << 2 ) & 0xFF ;
F_13 ( V_3 -> V_28 , 0x3F , ~ 0xFC , V_107 ) ;
V_107 = V_109 >> 8 ;
F_13 ( V_3 -> V_28 , 0x35 , ~ 0x07 , V_107 ) ;
}
static void F_23 ( struct V_2 * V_3 )
{
unsigned char V_24 ;
V_24 = F_3 ( V_3 -> V_38 , 0x37 ) ;
V_24 = ( V_24 & 3 ) << 6 ;
F_13 ( V_3 -> V_28 , 0x06 , ~ 0xc0 , V_24 & 0x80 ) ;
F_13 ( V_3 -> V_28 , 0x09 , ~ 0xc0 , V_24 | 0x80 ) ;
}
static void F_24 ( int V_114 ,
struct V_2 * V_3 ,
unsigned short V_60 ,
unsigned short V_29 )
{
unsigned short V_24 ;
F_5 ( V_3 -> V_38 , 0x2E , 0x00 ) ;
F_5 ( V_3 -> V_38 , 0x2F , 0x00 ) ;
F_5 ( V_3 -> V_38 , 0x46 , 0x00 ) ;
F_5 ( V_3 -> V_38 , 0x47 , 0x00 ) ;
if ( V_114 == V_23 ) {
V_24 = F_3 ( V_3 -> V_38 , 0x37 ) ;
if ( ( V_24 & 0x03 ) == 0 ) {
F_5 ( V_3 -> V_38 , 0x46 , 0x13 ) ;
F_5 ( V_3 -> V_38 , 0x47 , 0x13 ) ;
}
}
if ( V_114 == V_23 ) {
F_23 ( V_3 ) ;
} else {
V_24 = F_3 ( V_3 -> V_38 , 0x37 ) ;
if ( V_24 & 0x01 ) {
F_21 ( V_3 -> V_28 , 0x06 , 0x40 ) ;
F_21 ( V_3 -> V_28 , 0x09 , 0x40 ) ;
}
}
F_21 ( V_3 -> V_28 , 0x1E , 0x01 ) ;
F_14 ( V_3 -> V_28 , 0x30 , ~ 0x20 ) ;
F_14 ( V_3 -> V_28 , 0x35 , ~ 0x80 ) ;
V_24 = V_66 [ V_60 ] . V_91 ;
if ( V_24 & 0x4000 )
F_21 ( V_3 -> V_28 , 0x30 , 0x20 ) ;
if ( V_24 & 0x8000 )
F_21 ( V_3 -> V_28 , 0x35 , 0x80 ) ;
}
static void F_25 ( unsigned short V_29 ,
struct V_2 * V_3 ,
unsigned short V_60 )
{
int V_102 = - 1 ;
F_14 ( V_3 -> V_38 , 0x11 , 0x7F ) ;
if ( V_29 == 0x2E &&
( V_66 [ V_60 ] . V_103 ==
V_115 ) )
V_102 = 12 ;
else if ( V_29 == 0x2E && ( V_66 [ V_60 ] .
V_103 == V_116 ) )
V_102 = 13 ;
else if ( V_29 == 0x2F )
V_102 = 14 ;
else if ( V_29 == 0x50 )
V_102 = 15 ;
else if ( V_29 == 0x59 )
V_102 = 16 ;
if ( V_102 != - 1 ) {
F_5 ( V_3 -> V_38 , 0x02 ,
V_117 [ V_102 ] . V_118 ) ;
F_5 ( V_3 -> V_38 , 0x03 ,
V_117 [ V_102 ] . V_119 ) ;
F_5 ( V_3 -> V_38 , 0x15 ,
V_117 [ V_102 ] . V_120 ) ;
F_5 ( V_3 -> V_38 , 0x16 ,
V_117 [ V_102 ] . V_121 ) ;
}
}
static void F_26 ( struct V_35 * V_36 ,
unsigned short V_29 , unsigned short V_30 ,
unsigned short V_60 ,
struct V_2 * V_3 )
{
unsigned short V_122 , V_61 , V_62 , V_123 , V_24 , V_41 ;
unsigned char V_94 ;
V_122 = V_42 [ V_30 ] . V_65 ;
V_41 = V_42 [ V_30 ] . V_43 ;
V_61 = V_124 [ V_122 ] . V_125 ;
V_62 = V_124 [ V_122 ] . V_126 ;
if ( V_41 & V_127 )
V_61 = V_61 >> 1 ;
if ( V_41 & V_127 )
V_61 = V_61 << 1 ;
V_24 = V_66 [ V_60 ] . V_91 ;
if ( V_24 & V_128 )
V_62 = V_62 >> 1 ;
if ( V_41 & V_101 )
V_62 = V_62 << 1 ;
V_123 = 8 ;
V_61 /= V_123 ;
V_61 -= 1 ;
V_62 -= 1 ;
V_123 = V_61 ;
V_24 = F_3 ( V_3 -> V_38 , 0x11 ) ;
V_94 = F_3 ( V_3 -> V_38 , 0x11 ) ;
V_94 &= 0x7F ;
F_5 ( V_3 -> V_38 , 0x11 , V_94 ) ;
F_5 ( V_3 -> V_38 , 0x01 , ( unsigned short ) ( V_123 & 0xff ) ) ;
F_13 ( V_3 -> V_38 , 0x0b , ~ 0x0c ,
( unsigned short ) ( ( V_123 & 0x0ff00 ) >> 10 ) ) ;
F_5 ( V_3 -> V_38 , 0x12 , ( unsigned short ) ( V_62 & 0xff ) ) ;
V_61 = 0 ;
V_62 = V_62 >> 8 ;
if ( V_62 & 0x01 )
V_61 |= 0x02 ;
if ( V_62 & 0x02 )
V_61 |= 0x40 ;
F_13 ( V_3 -> V_38 , 0x07 , ~ 0x42 , V_61 ) ;
V_94 = F_3 ( V_3 -> V_38 , 0x07 ) ;
V_61 = 0 ;
if ( V_62 & 0x04 )
V_61 |= 0x02 ;
F_13 ( V_3 -> V_38 , 0x0a , ~ 0x02 , V_61 ) ;
F_5 ( V_3 -> V_38 , 0x11 , V_24 ) ;
}
static void F_27 ( unsigned short V_29 ,
unsigned short V_30 ,
unsigned short V_60 ,
struct V_35 * V_36 ,
struct V_2 * V_3 )
{
unsigned short V_24 , V_129 , V_130 , V_131 , V_32 , V_132 ;
V_24 = V_42 [ V_30 ] . V_133 ;
V_24 = V_24 >> 8 ;
V_24 = V_134 [ V_24 ] ;
V_131 = V_66 [ V_60 ] . V_91 ;
V_131 &= V_128 ;
if ( V_131 )
V_24 = V_24 << 1 ;
V_131 = V_3 -> V_55 - V_135 ;
switch ( V_131 ) {
case 0 :
V_131 = 1 ;
break;
case 1 :
V_131 = 2 ;
break;
case 2 :
V_131 = 4 ;
break;
case 3 :
V_131 = 4 ;
break;
case 4 :
V_131 = 6 ;
break;
case 5 :
V_131 = 8 ;
break;
default:
break;
}
if ( ( V_29 >= 0x26 ) && ( V_29 <= 0x28 ) )
V_24 = V_24 * V_131 + V_131 / 2 ;
else
V_24 *= V_131 ;
V_132 = V_24 ;
V_131 = V_24 ;
V_24 = V_24 >> 8 ;
V_24 &= 0x0F ;
V_32 = F_3 ( V_3 -> V_28 , 0x0E ) ;
V_32 &= 0xF0 ;
V_32 |= V_24 ;
F_5 ( V_3 -> V_28 , 0x0E , V_32 ) ;
V_24 = ( unsigned char ) V_131 ;
V_24 &= 0xFF ;
F_5 ( V_3 -> V_38 , 0x13 , V_24 ) ;
V_131 = V_66 [ V_60 ] . V_91 ;
V_131 &= V_128 ;
if ( V_131 )
V_132 >>= 1 ;
V_132 = V_132 << 5 ;
V_129 = ( V_132 & 0xff00 ) >> 8 ;
V_130 = V_132 & 0x00ff ;
if ( V_130 == 0 )
V_129 += 1 ;
else
V_129 += 2 ;
if ( V_36 -> V_99 >= V_15 )
if ( ( V_29 == 0x4A ) | ( V_29 == 0x49 ) )
V_129 -= 1 ;
F_5 ( V_3 -> V_28 , 0x10 , V_129 ) ;
}
static unsigned short F_28 ( unsigned short V_29 ,
unsigned short V_30 ,
unsigned short V_60 ,
struct V_35 * V_36 ,
struct V_2 * V_3 )
{
unsigned short V_136 , V_41 ;
V_41 = V_42 [ V_30 ] . V_43 ;
if ( V_3 -> V_9 & ( V_48 | V_46 ) ) {
if ( V_3 -> V_6 != V_137 )
V_136 = V_138 + 5 ;
else
V_136 = V_139 + 2 ;
} else if ( V_3 -> V_9 & V_76 ) {
if ( V_3 -> V_78 & V_140 )
V_136 = V_141 + V_142 ;
else
V_136 = V_141 + V_143 ;
if ( V_3 -> V_78 & V_79 ) {
if ( V_41 & V_45 ) {
V_136 = V_141 + V_144 ;
} else {
V_136 = V_141 + V_145 ;
}
}
if ( V_3 -> V_16 & V_17 ) {
if ( V_3 -> V_78 & V_140 )
V_136 = V_146 ;
else
V_136 = V_147 ;
}
} else if ( V_3 -> V_9 & V_47 ) {
if ( V_3 -> V_78 & V_140 )
V_136 = V_141 + V_148 ;
else
V_136 = V_141 + V_149 ;
} else {
V_136 = V_66 [ V_60 ] . V_150 ;
V_136 &= V_104 ;
}
return V_136 ;
}
static void F_29 ( unsigned short V_29 ,
unsigned short V_30 ,
struct V_35 * V_36 ,
unsigned short V_60 ,
struct V_2 * V_3 )
{
unsigned char V_102 , V_94 ;
unsigned short V_151 ;
if ( ( V_3 -> V_152 == 0 ) &&
( V_3 -> V_16 & ( V_85 | V_86 | V_17 |
V_18 | V_70 ) ) &&
( V_3 -> V_9 & V_46 ) ) {
V_151 = F_28 ( V_29 , V_30 ,
V_60 , V_36 ,
V_3 ) ;
V_94 = F_3 ( V_3 -> V_28 , 0x31 ) & 0xCF ;
F_5 ( V_3 -> V_28 , 0x31 , V_94 ) ;
V_94 = V_153 [ V_151 ] . V_154 ;
F_5 ( V_3 -> V_28 , 0x2B , V_94 ) ;
V_94 = V_153 [ V_151 ] . V_155 ;
F_5 ( V_3 -> V_28 , 0x2C , V_94 ) ;
F_5 ( V_3 -> V_28 , 0x2D , 0x01 ) ;
} else {
V_102 = V_66 [ V_60 ] . V_150 ;
V_94 = F_3 ( V_3 -> V_28 , 0x31 ) & 0xCF ;
F_5 ( V_3 -> V_28 , 0x31 , V_94 ) ;
F_5 ( V_3 -> V_28 , 0x2B , V_57 [ V_102 ] . V_58 ) ;
F_5 ( V_3 -> V_28 , 0x2C , V_57 [ V_102 ] . V_59 ) ;
F_5 ( V_3 -> V_28 , 0x2D , 0x01 ) ;
}
if ( V_36 -> V_99 >= V_15 ) {
if ( V_42 [ V_30 ] . V_43 &
V_127 ) {
V_94 = F_3 ( V_3 -> V_28 , 0x2B ) ;
F_5 ( V_3 -> V_28 , 0x2B , V_94 ) ;
V_94 = F_3 ( V_3 -> V_28 , 0x2C ) ;
V_102 = V_94 ;
V_102 &= 0xE0 ;
V_94 &= 0x1F ;
V_94 = V_94 << 1 ;
V_94 += 1 ;
V_94 |= V_102 ;
F_5 ( V_3 -> V_28 , 0x2C , V_94 ) ;
}
}
}
static void F_30 ( struct V_2 * V_3 )
{
unsigned char V_24 ;
V_24 = F_3 ( V_3 -> V_38 , 0x37 ) ;
V_24 = ( V_24 & 1 ) << 6 ;
F_13 ( V_3 -> V_28 , 0x06 , ~ 0x40 , V_24 ) ;
F_13 ( V_3 -> V_28 , 0x09 , ~ 0xc0 , V_24 | 0x80 ) ;
}
static void F_31 ( unsigned short V_29 ,
struct V_35 * V_36 ,
struct V_2 * V_3 )
{
unsigned short V_94 ;
V_94 = F_3 ( V_3 -> V_28 , 0x3D ) ;
V_94 &= 0xfe ;
F_5 ( V_3 -> V_28 , 0x3D , V_94 ) ;
F_5 ( V_3 -> V_28 , 0x08 , 0x34 ) ;
V_94 = F_3 ( V_3 -> V_28 , 0x09 ) ;
V_94 &= 0xC0 ;
F_5 ( V_3 -> V_28 , 0x09 , V_94 | 0x30 ) ;
V_94 = F_3 ( V_3 -> V_28 , 0x3D ) ;
V_94 |= 0x01 ;
F_5 ( V_3 -> V_28 , 0x3D , V_94 ) ;
if ( V_36 -> V_99 == V_156 )
F_30 ( V_3 ) ;
}
static void F_32 ( struct V_35 * V_36 ,
unsigned short V_29 , unsigned short V_60 ,
struct V_2 * V_3 )
{
unsigned short V_94 , V_157 = 0 ;
short V_158 ;
unsigned char V_102 ;
V_102 = V_66 [ V_60 ] . V_150 ;
V_102 &= V_104 ;
V_158 = V_57 [ V_102 ] . CLOCK ;
V_94 = F_3 ( V_3 -> V_28 , 0x32 ) ;
V_94 &= 0xf3 ;
if ( V_158 >= 200 )
V_94 |= 0x0c ;
if ( V_36 -> V_99 >= V_15 )
V_94 &= ~ 0x04 ;
F_5 ( V_3 -> V_28 , 0x32 , V_94 ) ;
if ( V_36 -> V_99 < V_15 ) {
V_94 = F_3 ( V_3 -> V_28 , 0x1F ) ;
V_94 &= 0xE7 ;
if ( V_158 < 200 )
V_94 |= 0x10 ;
F_5 ( V_3 -> V_28 , 0x1F , V_94 ) ;
}
V_157 = 0x00 ;
F_13 ( V_3 -> V_28 , 0x07 , 0xFC , V_157 ) ;
if ( V_36 -> V_99 >= V_23 )
F_13 ( V_3 -> V_28 , 0x40 , 0xFC , V_157 & 0x03 ) ;
}
static void F_33 ( struct V_35 * V_36 ,
unsigned short V_29 , unsigned short V_30 ,
unsigned short V_60 ,
struct V_2 * V_3 )
{
unsigned short V_94 , V_157 , V_159 , V_64 = 0 , V_41 , V_122 ,
V_160 ;
V_41 = V_42 [ V_30 ] . V_43 ;
V_64 = V_66 [ V_60 ] . V_91 ;
if ( F_3 ( V_3 -> V_38 , 0x31 ) & 0x01 )
F_13 ( V_3 -> V_28 , 0x1F , 0x3F , 0x00 ) ;
V_94 = V_64 ;
V_157 = 0 ;
V_157 |= 0x02 ;
V_159 = V_3 -> V_55 - V_56 ;
V_159 = V_159 << 2 ;
V_157 |= V_159 ;
V_94 &= V_128 ;
if ( V_94 )
V_157 |= 0x20 ;
F_13 ( V_3 -> V_28 , 0x06 , ~ 0x3F , V_157 ) ;
V_122 = V_42 [ V_30 ] . V_65 ;
V_160 = V_124 [ V_122 ] . V_125 ;
V_94 = 0x0000 ;
if ( V_64 & V_128 ) {
if ( V_160 == 1024 )
V_94 = 0x0035 ;
else if ( V_160 == 1280 )
V_94 = 0x0048 ;
}
F_13 ( V_3 -> V_38 , 0x19 , 0xFF , V_94 ) ;
F_13 ( V_3 -> V_38 , 0x19 , 0xFC , 0 ) ;
if ( V_41 & V_127 )
F_13 ( V_3 -> V_28 , 0x01 , 0xF7 , 0x08 ) ;
V_157 = 0 ;
if ( V_41 & V_161 )
V_157 |= 0x08 ;
F_13 ( V_3 -> V_28 , 0x0F , ~ 0x48 , V_157 ) ;
V_94 = 0x60 ;
V_94 = V_94 ^ 0x60 ;
V_94 = V_94 ^ 0xA0 ;
F_13 ( V_3 -> V_28 , 0x21 , 0x1F , V_94 ) ;
F_32 ( V_36 , V_29 , V_60 ,
V_3 ) ;
V_94 = F_3 ( V_3 -> V_38 , 0x31 ) ;
if ( V_36 -> V_99 == V_23 ) {
if ( V_94 & 0x40 )
V_94 = 0x2c ;
else
V_94 = 0x6c ;
F_5 ( V_3 -> V_38 , 0x52 , V_94 ) ;
F_21 ( V_3 -> V_38 , 0x51 , 0x10 ) ;
} else if ( V_36 -> V_99 >= V_15 ) {
if ( V_94 & 0x40 )
V_94 = 0x33 ;
else
V_94 = 0x73 ;
F_5 ( V_3 -> V_38 , 0x52 , V_94 ) ;
F_5 ( V_3 -> V_38 , 0x51 , 0x02 ) ;
} else {
if ( V_94 & 0x40 )
V_94 = 0x2c ;
else
V_94 = 0x6c ;
F_5 ( V_3 -> V_38 , 0x52 , V_94 ) ;
}
}
static void F_34 ( unsigned short V_162 ,
unsigned short V_129 ,
unsigned short V_130 ,
unsigned short V_163 ,
struct V_2 * V_3 )
{
unsigned short V_24 , V_164 , V_165 ;
V_164 = V_129 ;
V_165 = V_130 ;
if ( V_162 != 0 ) {
V_24 = V_164 ;
V_164 = V_163 ;
V_163 = V_24 ;
if ( V_162 == 1 ) {
V_24 = V_165 ;
V_165 = V_163 ;
V_163 = V_24 ;
} else {
V_24 = V_165 ;
V_165 = V_164 ;
V_164 = V_24 ;
}
}
F_9 ( ( unsigned short ) V_163 , V_3 -> V_166 ) ;
F_9 ( ( unsigned short ) V_164 , V_3 -> V_166 ) ;
F_9 ( ( unsigned short ) V_165 , V_3 -> V_166 ) ;
}
static void F_35 ( unsigned short V_29 , unsigned short V_30 ,
struct V_2 * V_3 )
{
unsigned short V_94 , V_157 , V_32 , V_167 , V_168 , V_169 , V_170 , V_171 , V_172 , V_173 , V_162 , V_130 , V_129 , V_163 ;
const unsigned short * V_174 = V_175 ;
F_9 ( 0xFF , V_3 -> V_176 ) ;
F_9 ( 0x00 , V_3 -> V_177 ) ;
for ( V_32 = 0 ; V_32 < 16 ; V_32 ++ ) {
V_94 = V_174 [ V_32 ] ;
for ( V_167 = 0 ; V_167 < 3 ; V_167 ++ ) {
V_157 = 0 ;
if ( V_94 & 0x01 )
V_157 = 0x2A ;
if ( V_94 & 0x02 )
V_157 += 0x15 ;
F_9 ( V_157 , V_3 -> V_166 ) ;
V_94 = V_94 >> 2 ;
}
}
for ( V_32 = 16 ; V_32 < 32 ; V_32 ++ ) {
V_94 = V_174 [ V_32 ] ;
for ( V_167 = 0 ; V_167 < 3 ; V_167 ++ )
F_9 ( V_94 , V_3 -> V_166 ) ;
}
V_171 = 32 ;
for ( V_168 = 0 ; V_168 < 9 ; V_168 ++ ) {
V_172 = V_171 ;
V_173 = V_171 + 0x04 ;
V_162 = 0 ;
for ( V_169 = 0 ; V_169 < 3 ; V_169 ++ ) {
for ( V_170 = 0 ; V_170 < 5 ; V_170 ++ ) {
V_163 = V_174 [ V_171 ] ;
V_129 = V_174 [ V_172 ] ;
V_130 = V_174 [ V_173 ] ;
V_171 ++ ;
F_34 ( V_162 , V_129 , V_130 , V_163 , V_3 ) ;
}
V_171 -= 2 ;
for ( V_170 = 0 ; V_170 < 3 ; V_170 ++ ) {
V_163 = V_174 [ V_173 ] ;
V_129 = V_174 [ V_172 ] ;
V_130 = V_174 [ V_171 ] ;
V_171 -- ;
F_34 ( V_162 , V_129 , V_130 , V_163 , V_3 ) ;
}
V_162 ++ ;
}
V_171 += 5 ;
}
}
static void F_36 ( unsigned short V_29 ,
unsigned short V_30 ,
struct V_2 * V_3 )
{
unsigned short V_122 , V_160 , V_178 , V_41 ;
V_41 = V_42 [ V_30 ] . V_65 ;
V_122 = V_42 [ V_30 ] . V_65 ;
V_160 = V_124 [ V_122 ] . V_125 ;
V_178 = V_124 [ V_122 ] . V_126 ;
if ( V_41 & V_127 )
V_160 = V_160 << 1 ;
if ( V_41 & V_101 )
V_178 = V_178 << 1 ;
if ( V_160 == 720 )
V_160 = 640 ;
V_3 -> V_179 = V_160 ;
V_3 -> V_180 = V_160 ;
V_3 -> V_181 = V_178 ;
V_3 -> V_182 = V_178 ;
}
static void const * F_37 ( struct V_183 const * V_174 ,
unsigned short V_29 ,
unsigned short V_30 ,
unsigned short V_60 ,
struct V_2 * V_3 )
{
unsigned short V_32 , V_184 , V_62 , V_41 ;
V_62 = 0 ;
V_41 = V_42 [ V_30 ] . V_43 ;
V_32 = 0 ;
while ( V_174 [ V_32 ] . V_185 != 0xff ) {
V_184 = V_3 -> V_6 ;
if ( V_62 & 0x0080 ) {
V_62 &= ( ~ 0x0080 ) ;
V_184 = V_3 -> V_7 ;
}
if ( V_3 -> V_8 & V_186 )
V_184 &= ( ~ V_187 ) ;
if ( V_174 [ V_32 ] . V_185 == V_184 ) {
V_62 = V_174 [ V_32 ] . V_188 ;
V_184 = V_3 -> V_8 ;
if ( V_41 & V_127 )
V_184 |= V_189 ;
V_62 &= V_184 ;
if ( V_62 == V_174 [ V_32 ] . V_190 )
break;
}
V_32 ++ ;
}
return V_174 [ V_32 ] . V_191 ;
}
static struct V_192 const * F_38 ( unsigned short V_29 ,
unsigned short V_30 ,
unsigned short V_60 ,
struct V_2 * V_3 )
{
unsigned short V_32 , V_184 , V_193 , V_41 ;
V_41 = V_42 [ V_30 ] . V_43 ;
V_193 = V_66 [ V_60 ] . V_194 ;
V_193 = V_193 & 0x3f ;
V_184 = V_3 -> V_10 ;
if ( V_3 -> V_9 & V_49 )
V_184 = V_184 | V_195 ;
if ( V_41 & V_127 )
V_184 = V_184 | V_196 ;
V_32 = 0 ;
while ( V_197 [ V_32 ] . V_188 != 0xffff ) {
if ( ( V_184 & V_197 [ V_32 ] . V_188 ) ==
V_197 [ V_32 ] . V_190 )
break;
V_32 ++ ;
}
return & V_197 [ V_32 ] . V_191 [ V_193 ] ;
}
static void F_39 ( unsigned short V_29 , unsigned short V_30 ,
unsigned short V_60 ,
struct V_2 * V_3 )
{
struct V_198 const * V_199 ;
if ( ! ( V_3 -> V_9 & ( V_48 | V_46 ) ) )
return;
V_199 = F_37 ( V_200 , V_29 , V_30 ,
V_60 , V_3 ) ;
V_3 -> V_201 = V_199 -> V_201 ;
V_3 -> V_202 = V_199 -> V_202 ;
V_3 -> V_203 = V_199 -> V_204 ;
V_3 -> V_205 = V_199 -> V_206 ;
if ( V_3 -> V_8 & ( V_207 | V_186 ) )
return;
if ( ( V_3 -> V_6 == V_137 ) ||
( V_3 -> V_6 == V_208 ) ) {
V_3 -> V_180 = 1024 ;
V_3 -> V_182 = 768 ;
} else if ( ( V_3 -> V_6 == V_73 ) ||
( V_3 -> V_6 == V_209 ) ) {
V_3 -> V_180 = 1280 ;
V_3 -> V_182 = 1024 ;
} else if ( V_3 -> V_6 == V_210 ) {
V_3 -> V_180 = 1400 ;
V_3 -> V_182 = 1050 ;
} else {
V_3 -> V_180 = 1600 ;
V_3 -> V_182 = 1200 ;
}
}
static void F_40 ( unsigned short V_29 , unsigned short V_30 ,
unsigned short V_60 ,
struct V_35 * V_36 ,
struct V_2 * V_3 )
{
unsigned short V_32 ;
struct V_211 const * V_199 = NULL ;
struct V_212 const * V_213 = NULL ;
if ( V_3 -> V_9 & ( V_48 | V_46 ) ) {
V_199 = F_37 ( V_214 , V_29 , V_30 ,
V_60 , V_3 ) ;
for ( V_32 = 0 ; V_32 < 8 ; V_32 ++ )
V_3 -> V_98 . V_94 [ V_32 ] = V_199 [ 0 ] . V_215 [ V_32 ] ;
}
F_17 ( V_3 , V_36 ) ;
if ( V_3 -> V_9 & ( V_48 | V_46 ) ) {
V_213 = F_37 ( V_216 , V_29 ,
V_30 , V_60 ,
V_3 ) ;
for ( V_32 = 0 ; V_32 < 7 ; V_32 ++ )
V_3 -> V_100 . V_94 [ V_32 ] = V_213 [ 0 ] . V_215 [ V_32 ] ;
}
F_18 ( V_30 , V_29 , V_3 ) ;
}
static unsigned short F_41 ( struct V_2 * V_3 )
{
unsigned char V_193 , V_217 , V_218 , V_32 ;
V_217 = F_3 ( V_3 -> V_38 , 0x36 ) ;
V_193 = V_217 & 0x0F ;
V_217 = V_217 & 0xF0 ;
V_32 = 0 ;
V_218 = V_3 -> V_19 [ V_32 ] . V_219 ;
while ( V_218 != 0xFF ) {
if ( V_218 & 0x80 ) {
V_193 = V_217 ;
V_218 = V_218 & ~ ( 0x80 ) ;
}
if ( V_193 == V_218 )
break;
V_32 ++ ;
V_218 = V_3 -> V_19 [ V_32 ] . V_219 ;
}
return V_32 ;
}
static unsigned short F_42 ( struct V_2 * V_3 )
{
unsigned short V_217 , V_193 , V_218 , V_32 ;
V_193 = V_3 -> V_6 ;
V_217 = V_3 -> V_7 ;
V_32 = 0 ;
V_218 = V_3 -> V_19 [ V_32 ] . V_219 ;
while ( V_218 != 0xFF ) {
if ( ( V_218 & 0x80 ) && ( V_218 != 0x80 ) ) {
V_193 = V_217 ;
V_218 &= ~ 0x80 ;
}
if ( V_193 == V_218 )
break;
V_32 ++ ;
V_218 = V_3 -> V_19 [ V_32 ] . V_219 ;
}
if ( V_218 == 0xFF ) {
V_3 -> V_6 = V_137 ;
V_3 -> V_7 = 0 ;
V_32 = 0 ;
}
return V_32 ;
}
static void F_43 ( unsigned short * V_220 ,
unsigned short * V_221 ,
struct V_2 * V_3 )
{
unsigned short V_222 ;
V_222 = F_41 ( V_3 ) ;
* V_220 = V_3 -> V_19 [ V_222 ] . V_223 ;
* V_221 = V_3 -> V_19 [ V_222 ] . V_224 ;
}
static void F_44 ( unsigned short V_29 , unsigned short V_30 ,
unsigned short V_60 ,
struct V_2 * V_3 )
{
unsigned short V_62 , V_61 , V_123 , V_184 , V_225 , V_226 , V_41 ;
unsigned long V_24 , V_227 , V_131 , V_228 , V_229 ;
struct V_230 const * V_213 = NULL ;
V_41 = V_42 [ V_30 ] . V_43 ;
V_213 = F_37 ( V_231 , V_29 , V_30 ,
V_60 , V_3 ) ;
F_43 ( & V_61 , & V_62 , V_3 ) ;
V_225 = V_62 ;
V_226 = V_61 ;
if ( ( V_3 -> V_6 == V_137 ) ||
( V_3 -> V_6 == V_208 ) ) {
V_61 = 1024 ;
V_62 = 768 ;
} else if ( ( V_3 -> V_6 == V_73 ) ||
( V_3 -> V_6 == V_209 ) ) {
V_61 = 1280 ;
V_62 = 1024 ;
} else if ( V_3 -> V_6 == V_210 ) {
V_61 = 1400 ;
V_62 = 1050 ;
} else {
V_61 = 1600 ;
V_62 = 1200 ;
}
if ( V_3 -> V_8 & V_207 ) {
V_3 -> V_180 = V_61 ;
V_3 -> V_182 = V_62 ;
V_3 -> V_179 = V_61 ;
V_3 -> V_181 = V_62 ;
}
V_61 = V_3 -> V_203 ;
V_62 = V_213 -> V_232 ;
V_123 = V_3 -> V_180 ;
V_62 = V_62 & 0x0fff ;
V_123 += V_62 ;
if ( V_123 >= V_61 )
V_123 -= V_61 ;
F_5 ( V_3 -> V_233 , 0x1A , V_62 & 0x07 ) ;
V_123 = V_123 >> 3 ;
V_62 = V_62 >> 3 ;
F_5 ( V_3 -> V_233 , 0x16 ,
( unsigned short ) ( V_62 & 0xff ) ) ;
F_5 ( V_3 -> V_233 , 0x17 ,
( unsigned short ) ( V_123 & 0xff ) ) ;
V_61 = V_3 -> V_203 ;
V_62 = V_213 -> V_234 ;
V_123 = V_226 ;
if ( V_3 -> V_8 & V_186 )
V_123 = V_213 -> V_235 ;
V_123 += V_62 ;
if ( V_123 >= V_61 )
V_123 -= V_61 ;
V_61 = V_62 & 0x07 ;
V_61 = V_61 >> 5 ;
V_123 = V_123 >> 3 ;
V_62 = V_62 >> 3 ;
V_123 &= 0x1f ;
V_61 |= V_123 ;
F_5 ( V_3 -> V_233 , 0x15 , V_61 ) ;
F_5 ( V_3 -> V_233 , 0x14 ,
( unsigned short ) ( V_62 & 0xff ) ) ;
V_61 = V_3 -> V_205 ;
V_62 = V_213 -> V_236 ;
V_123 = V_3 -> V_182 ;
V_62 = V_62 & 0x0fff ;
V_123 += V_62 ;
if ( V_123 >= V_61 )
V_123 -= V_61 ;
F_5 ( V_3 -> V_233 , 0x1b ,
( unsigned short ) ( V_62 & 0xff ) ) ;
F_5 ( V_3 -> V_233 , 0x1c ,
( unsigned short ) ( V_123 & 0xff ) ) ;
V_62 = ( V_62 >> 8 ) & 0x07 ;
V_123 = ( V_123 >> 8 ) & 0x07 ;
F_5 ( V_3 -> V_233 , 0x1d ,
( unsigned short ) ( ( V_123 << 3 )
| V_62 ) ) ;
V_61 = V_3 -> V_205 ;
V_62 = V_213 -> V_237 ;
V_123 = V_225 ;
if ( V_3 -> V_8 & V_186 )
V_123 = V_213 -> V_238 ;
V_123 += V_62 ;
if ( V_123 >= V_61 )
V_123 -= V_61 ;
F_5 ( V_3 -> V_233 , 0x18 ,
( unsigned short ) ( V_62 & 0xff ) ) ;
F_13 ( V_3 -> V_233 , 0x19 , ~ 0x0f ,
( unsigned short ) ( V_123 & 0x0f ) ) ;
V_61 = ( ( V_62 >> 8 ) & 0x07 ) << 3 ;
V_62 = V_3 -> V_181 ;
if ( V_62 != V_3 -> V_182 )
V_61 |= 0x40 ;
if ( V_3 -> V_8 & V_239 )
V_61 |= 0x40 ;
F_13 ( V_3 -> V_233 , 0x1a , 0x07 ,
V_61 ) ;
V_62 = V_3 -> V_182 ;
V_61 = V_3 -> V_181 ;
V_24 = V_61 ;
V_227 = ( V_24 << 18 ) / V_62 ;
V_184 = ( unsigned short ) ( ( V_24 << 18 ) % V_62 ) ;
if ( V_184 != 0 )
V_227 += 1 ;
V_131 = V_227 ;
V_229 = V_131 ;
F_5 ( V_3 -> V_233 , 0x37 ,
( unsigned short ) ( V_131 & 0xff ) ) ;
F_5 ( V_3 -> V_233 , 0x36 ,
( unsigned short ) ( ( V_131 >> 8 ) & 0xff ) ) ;
V_62 = ( unsigned short ) ( V_131 >> 16 ) ;
V_61 = V_62 & 0x03 ;
V_62 = V_3 -> V_181 ;
if ( V_62 == V_3 -> V_182 )
V_61 |= 0x04 ;
F_5 ( V_3 -> V_233 , 0x35 , V_61 ) ;
if ( V_3 -> V_16 & V_70 ) {
V_131 = V_229 ;
F_5 ( V_3 -> V_240 ,
0x3c ,
( unsigned short ) ( V_131 & 0xff ) ) ;
F_5 ( V_3 -> V_240 ,
0x3b ,
( unsigned short ) ( ( V_131 >> 8 ) &
0xff ) ) ;
V_62 = ( unsigned short ) ( V_131 >> 16 ) ;
F_13 ( V_3 -> V_240 , 0x3a ,
~ 0xc0 ,
( unsigned short ) ( ( V_62 &
0xff ) << 6 ) ) ;
V_123 = V_3 -> V_181 ;
if ( V_123 == V_3 -> V_182 )
F_13 ( V_3 -> V_240 ,
0x30 , ~ 0x0c , 0x00 ) ;
else
F_13 ( V_3 -> V_240 ,
0x30 , ~ 0x0c , 0x08 ) ;
}
V_123 = V_3 -> V_179 ;
V_62 = V_3 -> V_180 ;
V_227 = V_123 << 16 ;
V_61 = ( unsigned short ) ( V_227 / V_62 ) ;
if ( ( V_62 & 0xffff ) == ( V_123 & 0xffff ) )
V_61 = 65535 ;
V_228 = V_61 ;
V_227 = V_3 -> V_179 << 16 ;
V_227 /= V_228 ;
V_228 = V_228 << 16 ;
V_227 -= 1 ;
V_228 = ( V_228 & 0xffff0000 ) + ( V_227 & 0xffff ) ;
V_61 = ( unsigned short ) ( V_228 & 0xff ) ;
F_5 ( V_3 -> V_233 , 0x1f , V_61 ) ;
V_227 = V_3 -> V_181 << 18 ;
V_227 = V_227 / V_229 ;
V_62 = ( unsigned short ) ( V_227 & 0xffff ) ;
if ( V_3 -> V_6 == V_137 )
V_62 -= 1 ;
V_61 = ( ( V_62 >> 8 ) & 0xff ) << 3 ;
V_61 |= ( unsigned short ) ( ( V_228 >> 8 ) & 0x07 ) ;
F_5 ( V_3 -> V_233 , 0x20 ,
( unsigned short ) ( V_61 & 0xff ) ) ;
F_5 ( V_3 -> V_233 , 0x21 ,
( unsigned short ) ( V_62 & 0xff ) ) ;
V_228 = V_228 >> 16 ;
if ( V_41 & V_127 )
V_228 = V_228 >> 1 ;
F_5 ( V_3 -> V_233 , 0x22 ,
( unsigned short ) ( ( V_228 >> 8 ) & 0xff ) ) ;
F_5 ( V_3 -> V_233 , 0x23 ,
( unsigned short ) ( V_228 & 0xff ) ) ;
}
static void F_45 ( unsigned char * V_241 , unsigned char * V_242 ,
struct V_2 * V_3 )
{
unsigned short V_102 ;
if ( V_3 -> V_9 & ( V_48 | V_46 ) ) {
V_102 = F_42 ( V_3 ) ;
if ( V_3 -> V_9 & V_48 ) {
* V_241 = V_3 -> V_19 [ V_102 ] . V_243 ;
* V_242 = V_3 -> V_19 [ V_102 ] . V_244 ;
} else {
* V_241 = V_3 -> V_19 [ V_102 ] . V_245 ;
* V_242 = V_3 -> V_19 [ V_102 ] . V_246 ;
}
}
}
static unsigned char F_46 ( unsigned short V_60 ,
unsigned short V_29 , unsigned short V_30 ,
struct V_2 * V_3 )
{
unsigned short V_102 , V_41 ;
unsigned char V_193 ;
V_41 = V_42 [ V_30 ] . V_43 ;
if ( ( V_3 -> V_78 & V_247 ) &&
( ! ( V_3 -> V_8 & V_186 ) ) ) {
V_102 = F_41 ( V_3 ) ;
V_193 = V_3 -> V_19 [ V_102 ] . V_248 ;
if ( V_3 -> V_9 & ( V_48 | V_46 ) )
return V_193 ;
if ( V_3 -> V_16 &
( V_85 |
V_86 |
V_17 |
V_18 |
V_70 ) ) {
if ( V_3 -> V_9 & V_76 ) {
V_193 = V_249 + V_142 ;
if ( ! ( V_3 -> V_10 & V_140 ) )
V_193 = V_249 + V_143 ;
if ( V_3 -> V_10 & V_79 ) {
V_193 = V_249 + V_144 ;
if ( ! ( V_41 & V_45 ) )
V_193 = V_249 +
V_145 ;
}
return V_193 ;
}
if ( V_3 -> V_10 & V_250 ) {
V_193 = V_251 ;
return V_193 ;
}
if ( V_3 -> V_10 & V_252 ) {
V_193 = V_253 ;
return V_193 ;
}
V_193 = V_254 ;
if ( ! ( V_3 -> V_10 & V_255 ) ) {
V_193 = V_249 + V_148 ;
if ( ! ( V_3 -> V_10 & V_140 ) )
V_193 = V_249 + V_149 ;
}
if ( V_3 -> V_9 & V_47 )
return V_193 ;
}
}
F_8 ( ( V_3 -> V_256 + 0x02 ) ) ;
V_193 = V_66 [ V_60 ] . V_150 ;
return V_193 ;
}
static void F_47 ( unsigned char V_193 , unsigned char * V_241 ,
unsigned char * V_242 , struct V_2 * V_3 )
{
if ( V_3 -> V_16 & ( V_257 | V_85 | V_86
| V_17 | V_18 | V_70 ) ) {
if ( ( ! ( V_3 -> V_9 & V_46 ) ) &&
( V_3 -> V_78 & V_247 ) ) {
* V_241 = V_153 [ V_193 ] . V_154 ;
* V_242 = V_153 [ V_193 ] . V_155 ;
}
} else {
* V_241 = V_57 [ V_193 ] . V_58 ;
* V_242 = V_57 [ V_193 ] . V_59 ;
}
}
static void F_48 ( unsigned short V_29 , unsigned short V_30 ,
unsigned short V_60 ,
struct V_2 * V_3 )
{
unsigned char V_241 , V_242 , V_193 ;
int V_32 ;
V_193 = F_46 ( V_60 , V_29 , V_30 ,
V_3 ) ;
F_47 ( V_193 , & V_241 , & V_242 , V_3 ) ;
F_45 ( & V_241 , & V_242 , V_3 ) ;
for ( V_32 = 0 ; V_32 < 4 ; V_32 ++ ) {
F_13 ( V_3 -> V_38 , 0x31 , ~ 0x30 ,
( unsigned short ) ( 0x10 * V_32 ) ) ;
if ( ( ! ( V_3 -> V_9 & V_46 ) )
&& ( ! ( V_3 -> V_9 & V_49 ) ) ) {
F_5 ( V_3 -> V_28 , 0x2e , V_241 ) ;
F_5 ( V_3 -> V_28 , 0x2f , V_242 ) ;
} else {
F_5 ( V_3 -> V_28 , 0x2b , V_241 ) ;
F_5 ( V_3 -> V_28 , 0x2c , V_242 ) ;
}
}
}
static void F_49 ( struct V_35 * V_36 ,
struct V_2 * V_3 )
{
unsigned short V_258 , V_259 , V_24 , V_218 , V_61 ;
if ( V_3 -> V_16 & ( V_85 | V_86 | V_17
| V_18 | V_70 ) ) {
V_258 = 0 ;
V_259 = 0 ;
V_24 = F_3 ( V_3 -> V_28 , 0x01 ) ;
if ( ! ( V_24 & 0x20 ) ) {
V_24 = F_3 ( V_3 -> V_38 , 0x17 ) ;
if ( V_24 & 0x80 ) {
V_24 = F_3 ( V_3 -> V_38 , 0x53 ) ;
if ( ! ( V_24 & 0x40 ) )
V_258 |= V_260 ;
}
}
V_24 = F_3 ( V_3 -> V_233 , 0x2e ) ;
V_24 &= 0x0f ;
if ( ! ( V_24 == 0x08 ) ) {
V_61 = F_3 ( V_3 -> V_233 , 0x13 ) ;
if ( V_61 & 0x04 )
V_258 = V_258 | V_261 ;
V_24 &= 0x05 ;
if ( ! ( V_258 & V_261 ) )
if ( V_24 == 0x01 )
V_258 |= V_262 ;
if ( V_24 == 0x04 )
V_258 |= V_261 ;
if ( V_24 == 0x05 ) {
V_24 = F_3 ( V_3 -> V_263 , 0x00 ) ;
if ( ! ( V_24 & 0x08 ) )
V_259 |= V_264 ;
if ( ! ( V_24 & 0x04 ) )
V_259 |= V_265 ;
if ( V_24 & 0x02 )
V_259 |= V_266 ;
if ( V_3 -> V_9 & V_76 ) {
if ( V_24 & 0x01 )
V_259 |= V_267 ;
}
if ( V_3 -> V_9 & V_83 ) {
V_24 = F_3 (
V_3 -> V_263 ,
0x4d ) ;
if ( V_24 & 0x10 )
V_259 |= V_268 ;
}
if ( V_259 != 0 )
V_258 |= V_269 ;
}
}
V_24 = F_3 ( V_3 -> V_38 , 0x3d ) ;
if ( V_258 & V_261 ) {
if ( ( V_3 -> V_78 & V_270 ) ) {
if ( V_24 & V_269 )
V_258 |= V_269 ;
}
}
V_24 = V_258 ;
V_218 = ~ V_271 ;
F_13 ( V_3 -> V_38 , 0x3d , V_218 , V_24 ) ;
if ( ! ( V_3 -> V_78 & V_270 ) )
F_5 ( V_3 -> V_38 , 0x3e , V_259 ) ;
}
}
void F_2 ( struct V_2 * V_3 )
{
unsigned short V_272 , V_62 , V_217 ;
V_62 = V_86 ;
V_272 = F_3 ( V_3 -> V_240 , 0x00 ) ;
if ( V_272 == 0x02 )
goto V_273;
V_62 = V_257 ;
V_272 = F_3 ( V_3 -> V_240 , 0x01 ) ;
if ( V_272 < 0xB0 )
goto V_273;
V_62 = V_85 ;
if ( V_272 < 0xC0 )
goto V_274;
V_62 = V_70 ;
if ( V_272 < 0xD0 )
goto V_274;
V_62 = V_17 ;
if ( V_272 < 0xE0 )
goto V_274;
V_62 = V_18 ;
V_217 = F_3 ( V_3 -> V_240 , 0x39 ) ;
if ( V_217 != 0xFF )
V_62 = V_70 ;
V_274:
if ( V_62 & ( V_85 | V_86 ) ) {
V_272 = F_3 ( V_3 -> V_240 , 0x23 ) ;
if ( ! ( V_272 & 0x02 ) )
V_62 = V_62 | V_275 ;
}
V_273:
V_3 -> V_16 = V_62 ;
}
static void F_50 ( unsigned short V_29 , unsigned short V_30 ,
struct V_35 * V_36 ,
struct V_2 * V_3 )
{
unsigned short V_61 , V_276 , V_62 , V_24 , V_41 ;
V_41 = V_42 [ V_30 ] . V_43 ;
V_3 -> V_78 = 0 ;
V_3 -> V_55 = V_41 & V_277 ;
V_62 = 0 ;
if ( ! ( V_3 -> V_16 & 0xFFFF ) )
return;
V_24 = F_3 ( V_3 -> V_38 , 0x30 ) ;
V_62 = V_62 | V_24 ;
V_24 = F_3 ( V_3 -> V_38 , 0x31 ) ;
V_276 = V_24 ;
V_276 = V_276 << 8 ;
V_61 = V_24 << 8 ;
V_62 = V_62 | V_61 ;
V_24 = ( V_278 | V_83 | V_46
| V_49 | V_279 ) ;
V_24 = 0xFFFF ^ V_24 ;
V_62 &= V_24 ;
V_24 = F_3 ( V_3 -> V_38 , 0x38 ) ;
if ( V_3 -> V_16 & ( V_86 | V_17 | V_18 |
V_70 ) ) {
if ( V_24 & V_280 ) {
V_62 |= V_278 ;
if ( V_24 & V_281 )
V_62 |= V_46 ;
}
}
if ( V_3 -> V_16 & ( V_17 | V_18 | V_70 ) ) {
if ( V_24 & V_282 ) {
V_24 = F_3 ( V_3 -> V_38 , 0x35 ) ;
V_24 &= V_283 ;
V_62 |= V_76 ;
if ( V_24 != V_284 ) {
V_62 &= ( ~ V_76 ) ;
V_62 |= V_83 ;
}
}
}
V_61 = V_276 ;
V_24 = 0x09FC ;
if ( ! ( V_62 & V_24 ) ) {
V_61 |= V_279 ;
V_62 = 0 ;
}
if ( ! ( V_3 -> V_16 & V_275 ) ) {
if ( V_62 & V_46 ) {
if ( V_62 & V_285 )
V_62 &= ( ~ ( V_48 | V_68 |
V_286 ) ) ;
else
V_62 &= ( ~ ( V_48 | V_68 |
V_47 | V_286 ) ) ;
}
}
if ( ! ( V_62 & ( V_286 | V_285 ) ) ) {
if ( V_62 & V_68 ) {
V_62 &= ( 0xFF00 | V_68 |
V_286 | V_285 ) ;
V_62 &= ( 0x00FF | ( ~ V_83 ) ) ;
}
}
if ( ! ( V_3 -> V_16 & V_275 ) ) {
if ( V_62 & V_48 ) {
V_62 &= ( 0xFF00 | V_48 | V_286 |
V_285 ) ;
V_62 &= ( 0x00FF | ( ~ V_83 ) ) ;
}
}
if ( V_62 & V_82 ) {
V_62 &= ( 0xFF00 | V_82 | V_286 |
V_285 ) ;
V_62 &= ( 0x00FF | ( ~ V_83 ) ) ;
}
if ( V_62 & V_83 )
V_62 &= ( 0xFF00 | V_286 | V_285 ) ;
if ( V_62 & V_76 )
V_62 &= ( 0xFF00 | V_76 | V_286 |
V_285 ) ;
if ( V_61 & V_279 ) {
if ( ! ( V_62 & ( V_286 | V_285 ) ) )
V_62 = V_279 ;
}
if ( ! ( V_62 & V_279 ) ) {
if ( ( ! ( V_62 & V_287 ) ) || ( ! ( V_41 & V_288 ) ) ) {
if ( ! ( V_62 & V_46 ) )
V_62 |= ( V_49 | V_285 ) ;
}
if ( ( V_62 & V_49 ) && ( V_62 & V_46 ) ) {
V_62 ^= ( V_48 | V_46 |
V_278 ) ;
V_3 -> V_78 |= V_270 ;
}
}
V_3 -> V_9 = V_62 ;
}
static void F_51 ( unsigned short V_29 , unsigned short V_30 ,
struct V_2 * V_3 )
{
unsigned short V_62 = 0 , V_63 = 0 , V_41 , V_289 ;
if ( V_3 -> V_9 & V_47 ) {
V_41 = V_42 [ V_30 ] . V_43 ;
V_63 = V_42 [ V_30 ] . V_65 ;
V_62 = F_3 ( V_3 -> V_38 , 0x35 ) ;
if ( V_62 & V_88 ) {
V_62 &= ( V_290 |
V_291 |
V_292 |
V_88 ) ;
if ( V_62 & V_291 )
V_62 &= ~ V_88 ;
} else
V_62 &= ( V_290 |
V_293 |
V_88 ) ;
if ( V_3 -> V_9 & V_82 )
V_62 |= V_88 ;
if ( V_3 -> V_9 & V_83 ) {
V_289 = F_3 ( V_3 -> V_38 , 0x35 ) ;
V_289 &= V_283 ;
if ( V_289 == V_294 )
V_62 |= V_295 ;
if ( V_289 == V_296 )
V_62 = V_62 | V_252 ;
if ( V_289 == V_297 )
V_62 = V_62 | V_250 ;
}
if ( V_3 -> V_9 & V_76 )
V_62 = V_62 | V_298 | V_88 ;
if ( ( V_3 -> V_9 & V_49 ) &&
( ! ( V_3 -> V_9 & V_90 ) ) )
V_62 |= V_79 ;
if ( ! ( V_62 & V_88 ) && ( V_41 > 13 ) && ( V_63 == 8 ) )
V_62 |= V_255 ;
V_62 |= V_140 ;
if ( V_3 -> V_9 & V_76 ) {
if ( V_3 -> V_9 & V_49 )
V_62 &= ( ~ V_140 ) ;
} else if ( V_62 & ( V_252 | V_250 ) ) {
V_62 &= ( ~ V_140 ) ;
} else if ( ! ( V_3 -> V_16 & ( V_85 | V_86 |
V_17 | V_18 |
V_70 ) ) ) {
if ( V_62 & V_79 )
V_62 &= ( ~ V_140 ) ;
}
}
V_3 -> V_10 = V_62 ;
}
static unsigned char F_52 ( unsigned short V_29 ,
unsigned short V_30 , struct V_2 * V_3 )
{
unsigned short V_24 , V_61 , V_62 , V_63 = 0 , V_299 ;
V_3 -> V_6 = 0 ;
V_3 -> V_7 = 0 ;
V_3 -> V_8 = 0 ;
V_63 = V_42 [ V_30 ] . V_65 ;
V_24 = F_3 ( V_3 -> V_38 , 0x36 ) ;
V_62 = V_24 & 0x0F ;
if ( V_62 == 0 )
V_62 = V_137 ;
if ( ( V_62 == V_137 ) || ( V_62 == V_73 ) ) {
if ( V_3 -> V_9 & V_287 ) {
V_61 = F_3 ( V_3 -> V_38 , 0x33 ) ;
if ( V_3 -> V_9 & V_46 )
V_61 &= 0x0F ;
else
V_61 = V_61 >> 4 ;
if ( ( V_63 == 6 ) || ( V_63 == 9 ) ) {
if ( V_61 >= 3 )
V_62 |= V_300 ;
} else if ( ( V_63 == 7 ) || ( V_63 == 8 ) ) {
if ( V_61 >= 4 )
V_62 |= V_300 ;
}
}
}
V_3 -> V_6 = V_62 ;
if ( ! ( V_3 -> V_9 & ( V_48 | V_46 ) ) )
return 0 ;
V_62 = 0 ;
V_24 = F_3 ( V_3 -> V_38 , 0x37 ) ;
V_24 &= ( V_301 | V_75 | V_302 | V_303 ) ;
V_62 |= V_24 ;
V_299 = F_42 ( V_3 ) ;
V_61 = V_3 -> V_19 [ V_299 ] . V_304 ;
if ( ( ( V_3 -> V_16 & V_18 ) ||
( V_3 -> V_16 & V_70 ) ) && ( V_61 & V_305 ) )
V_62 |= V_306 ;
if ( ( V_3 -> V_6 == V_210 ) &&
( V_3 -> V_9 & V_48 ) && ( V_63 == 9 ) &&
( ! ( V_62 & V_186 ) ) )
V_62 |= V_207 ;
if ( V_3 -> V_9 & V_49 ) {
if ( V_3 -> V_9 & V_90 )
V_62 |= V_307 ;
} else {
V_62 |= V_307 ;
}
V_3 -> V_8 = V_62 ;
return 1 ;
}
unsigned char F_53 ( unsigned short V_29 ,
unsigned short * V_30 , struct V_2 * V_3 )
{
for ( * V_30 = 0 ; ; ( * V_30 ) ++ ) {
if ( V_42 [ * V_30 ] . V_308 == V_29 )
break;
if ( V_42 [ * V_30 ] . V_308 == 0xFF )
return 0 ;
}
return 1 ;
}
static unsigned char F_54 ( unsigned char V_309 )
{
unsigned char V_310 = 0 ;
unsigned char V_32 = 0 ;
for ( V_32 = 0 ; V_32 < 8 ; V_32 ++ ) {
V_310 = V_310 << 1 ;
V_310 |= ( V_309 >> V_32 ) & 1 ;
}
return V_310 ;
}
static unsigned char F_55 ( struct V_2 * V_3 )
{
unsigned char V_311 , V_24 ;
V_311 = F_3 ( V_3 -> V_38 , 0x4A ) ;
F_14 ( V_3 -> V_38 , 0x4A , ~ 0x23 ) ;
V_24 = F_3 ( V_3 -> V_38 , 0x48 ) ;
V_24 = F_54 ( V_24 ) ;
V_24 &= 0x23 ;
F_5 ( V_3 -> V_38 , 0x4A , V_311 ) ;
return V_24 ;
}
static unsigned char F_56 ( struct V_2 * V_3 )
{
unsigned char V_311 , V_312 , V_24 ;
V_311 = F_3 ( V_3 -> V_38 , 0x4A ) ;
F_14 ( V_3 -> V_38 , 0x4A , ~ 0x0C ) ;
V_24 = F_3 ( V_3 -> V_38 , 0x48 ) ;
V_24 &= 0x0C ;
V_24 >>= 2 ;
F_5 ( V_3 -> V_38 , 0x4A , V_311 ) ;
V_312 = F_3 ( V_3 -> V_38 , 0xB4 ) ;
V_24 |= ( ( V_312 & 0x04 ) << 3 ) ;
return V_24 ;
}
static void F_57 ( unsigned short V_313 , unsigned short V_218 ,
struct V_2 * V_3 )
{
unsigned char V_311 , V_24 ;
V_311 = F_3 ( V_3 -> V_38 , 0x4A ) ;
V_313 &= 0x23 ;
V_218 &= 0x23 ;
F_14 ( V_3 -> V_38 , 0x4A , ~ V_313 ) ;
if ( V_313 & 0x20 ) {
V_24 = ( V_218 >> 4 ) & 0x02 ;
F_13 ( V_3 -> V_38 , 0xB4 , ~ 0x02 , V_24 ) ;
}
V_24 = F_3 ( V_3 -> V_38 , 0x48 ) ;
V_24 = F_54 ( V_24 ) ;
V_24 &= ~ V_313 ;
V_24 |= V_218 ;
F_5 ( V_3 -> V_38 , 0x48 , V_24 ) ;
}
static void F_58 ( unsigned short V_313 , unsigned short V_218 ,
struct V_2 * V_3 )
{
unsigned char V_311 , V_24 ;
unsigned short V_314 , V_315 ;
V_314 = V_313 ;
V_315 = V_218 ;
V_314 &= 0x20 ;
V_315 &= 0x20 ;
V_314 >>= 3 ;
V_315 >>= 3 ;
if ( V_313 & 0x20 ) {
V_24 = ( V_218 >> 4 ) & 0x02 ;
F_13 ( V_3 -> V_38 , 0xB4 , ~ 0x02 , V_24 ) ;
}
F_13 ( V_3 -> V_38 , 0xB4 , ~ V_314 , V_315 ) ;
V_311 = F_3 ( V_3 -> V_38 , 0x4A ) ;
V_313 &= 0x03 ;
V_218 &= 0x03 ;
V_313 <<= 2 ;
V_218 <<= 2 ;
F_14 ( V_3 -> V_38 , 0x4A , ~ V_313 ) ;
F_13 ( V_3 -> V_38 , 0x48 , ~ V_313 , V_218 ) ;
}
static void F_59 ( struct V_316 * V_317 ,
struct V_35 * V_318 ,
struct V_2 * V_3 )
{
F_13 ( V_3 -> V_28 , 0x01 , 0xDF , 0x00 ) ;
if ( V_318 -> V_99 == V_156 ) {
if ( V_3 -> V_152 == 1 ) {
if ( ! ( F_55 ( V_3 ) & 0x1 ) ) {
F_57 ( 0x01 , 0x01 , V_3 ) ;
F_60 ( V_317 -> V_319 . V_320 ) ;
}
if ( ! ( F_55 ( V_3 ) & 0x20 ) )
F_57 ( 0x20 , 0x20 , V_3 ) ;
F_60 ( V_317 -> V_319 . V_321 ) ;
F_57 ( 0x02 , 0x02 , V_3 ) ;
} else {
F_57 ( 0x20 , 0x20 , V_3 ) ;
}
}
if ( V_318 -> V_99 == V_23 ) {
if ( V_3 -> V_152 == 1 ) {
if ( ! ( F_56 ( V_3 ) & 0x1 ) ) {
F_58 ( 0x01 , 0x01 , V_3 ) ;
F_60 ( V_317 -> V_319 . V_320 ) ;
}
if ( ! ( F_56 ( V_3 ) & 0x20 ) )
F_58 ( 0x20 , 0x20 , V_3 ) ;
F_60 ( V_317 -> V_319 . V_321 ) ;
F_58 ( 0x02 , 0x02 , V_3 ) ;
} else {
F_58 ( 0x20 , 0x20 , V_3 ) ;
}
}
}
void F_61 ( struct V_316 * V_317 ,
struct V_35 * V_318 ,
struct V_2 * V_3 )
{
if ( V_318 -> V_99 == V_156 ) {
if ( V_3 -> V_152 == 1 ) {
F_57 ( 0x02 , 0x00 , V_3 ) ;
F_60 ( V_317 -> V_319 . V_321 ) ;
} else {
F_57 ( 0x20 , 0x00 , V_3 ) ;
}
}
if ( V_318 -> V_99 == V_23 ) {
if ( ( F_56 ( V_3 ) & 0x2 ) ) {
F_58 ( 0x02 , 0x00 , V_3 ) ;
F_60 ( V_317 -> V_319 . V_321 ) ;
}
if ( V_3 -> V_152 == 0 )
F_58 ( 0x20 , 0x00 , V_3 ) ;
}
F_13 ( V_3 -> V_28 , 0x01 , 0xDF , 0x20 ) ;
}
static void F_62 ( struct V_2 * V_3 )
{
while ( ( F_8 ( V_3 -> V_50 ) & 0x01 ) )
break;
while ( ! ( F_8 ( V_3 -> V_50 ) & 0x01 ) )
break;
}
static void F_63 ( struct V_2 * V_3 )
{
F_21 ( V_3 -> V_233 , 0x01 , 0x40 ) ;
}
static void F_64 ( unsigned short V_29 ,
struct V_2 * V_3 )
{
unsigned short V_227 , V_131 ;
F_5 ( V_3 -> V_38 , 0x34 , V_29 ) ;
V_227 = ( V_3 -> V_9 & V_49 ) >> 8 ;
V_131 = ~ ( V_49 >> 8 ) ;
F_13 ( V_3 -> V_38 , 0x31 , V_131 , V_227 ) ;
}
static void F_65 ( unsigned short V_29 ,
unsigned short V_30 ,
struct V_2 * V_3 )
{
unsigned short V_160 , V_178 , V_41 , V_122 ;
V_122 = V_42 [ V_30 ] . V_65 ;
V_160 = V_124 [ V_122 ] . V_125 ;
V_178 = V_124 [ V_122 ] . V_126 ;
V_41 = V_42 [ V_30 ] . V_43 ;
if ( V_41 & V_127 )
V_160 *= 2 ;
if ( V_41 & V_101 )
V_178 *= 2 ;
if ( ! ( V_3 -> V_9 & V_48 ) )
goto exit;
if ( V_3 -> V_6 == V_322 ) {
if ( ! ( V_3 -> V_8 & V_307 ) ) {
if ( V_178 == 1024 )
V_178 = 1056 ;
}
}
if ( V_3 -> V_6 == V_73 ) {
if ( V_178 == 400 )
V_178 = 405 ;
else if ( V_178 == 350 )
V_178 = 360 ;
if ( V_3 -> V_8 & V_307 ) {
if ( V_178 == 360 )
V_178 = 375 ;
}
}
if ( V_3 -> V_6 == V_137 ) {
if ( ! ( V_3 -> V_8 & V_307 ) ) {
if ( ! ( V_3 -> V_8 & V_75 ) ) {
if ( V_178 == 350 )
V_178 = 357 ;
else if ( V_178 == 400 )
V_178 = 420 ;
else if ( V_178 == 480 )
V_178 = 525 ;
}
}
}
if ( V_160 == 720 )
V_160 = 640 ;
exit:
V_3 -> V_179 = V_160 ;
V_3 -> V_180 = V_160 ;
V_3 -> V_181 = V_178 ;
V_3 -> V_182 = V_178 ;
}
static unsigned char F_66 ( struct V_2 * V_3 )
{
if ( ( V_3 -> V_9 & ( V_48 | V_46 ) ) &&
( V_3 -> V_8 & V_306 ) )
return 1 ;
return 0 ;
}
static void F_67 ( unsigned short V_29 ,
unsigned short V_30 ,
unsigned short V_60 ,
struct V_2 * V_3 )
{
unsigned short V_61 , V_62 , V_227 , V_131 , V_41 = 0 , V_123 ,
V_323 ;
V_3 -> V_324 = 1 ;
V_3 -> V_325 = 1 ;
V_41 = V_42 [ V_30 ] . V_43 ;
V_323 = V_66 [ V_60 ] . V_103 ;
V_323 &= V_104 ;
V_227 = ( unsigned short ) V_105 [ V_323 ] . V_106 [ 0 ] ;
V_131 = ( unsigned short ) V_105 [ V_323 ] . V_106 [ 5 ] ;
V_61 = ( V_227 & 0xFF ) | ( ( V_131 & 0x03 ) << 8 ) ;
V_62 = ( unsigned short ) V_105 [ V_323 ] . V_106 [ 8 ] ;
V_123 = ( unsigned short )
V_105 [ V_323 ] . V_106 [ 14 ] << 8 ;
V_123 &= 0x0100 ;
V_123 = V_123 << 2 ;
V_62 |= V_123 ;
V_227 = ( unsigned short ) V_105 [ V_323 ] . V_106 [ 9 ] ;
if ( V_227 & 0x01 )
V_62 |= 0x0100 ;
if ( V_227 & 0x20 )
V_62 |= 0x0200 ;
V_61 += 5 ;
if ( V_41 & V_45 )
V_61 *= 8 ;
else
V_61 *= 9 ;
V_3 -> V_201 = V_61 ;
V_3 -> V_203 = V_61 ;
V_62 ++ ;
V_3 -> V_202 = V_62 ;
V_3 -> V_205 = V_62 ;
}
static void F_68 ( unsigned short V_29 , unsigned short V_30 ,
unsigned short V_60 ,
struct V_2 * V_3 )
{
unsigned short V_61 = 0 , V_62 = 0 , V_41 , V_63 ;
struct V_326 const * V_199 = NULL ;
V_41 = V_42 [ V_30 ] . V_43 ;
V_63 = V_42 [ V_30 ] . V_65 ;
V_3 -> V_327 = 0 ;
V_3 -> V_328 = 50 ;
if ( V_3 -> V_9 & V_68 ) {
F_67 ( V_29 , V_30 , V_60 ,
V_3 ) ;
return;
}
if ( V_3 -> V_9 & ( V_48 | V_46 ) ) {
V_199 = F_37 ( V_329 , V_29 , V_30 ,
V_60 , V_3 ) ;
V_3 -> V_324 = V_199 -> V_324 ;
V_3 -> V_325 = V_199 -> V_325 ;
V_3 -> V_201 = V_199 -> V_201 ;
V_3 -> V_202 = V_199 -> V_202 ;
V_3 -> V_203 = V_199 -> V_204 ;
V_3 -> V_205 = V_199 -> V_206 ;
if ( V_3 -> V_6 == V_137 ) {
V_61 = 1024 ;
V_62 = 768 ;
if ( ! ( V_3 -> V_8 & V_307 ) ) {
if ( V_3 -> V_181 == 357 )
V_62 = 527 ;
else if ( V_3 -> V_181 == 420 )
V_62 = 620 ;
else if ( V_3 -> V_181 == 525 )
V_62 = 775 ;
else if ( V_3 -> V_181 == 600 )
V_62 = 775 ;
}
} else if ( V_3 -> V_6 == V_208 ) {
V_61 = 1024 ;
V_62 = 768 ;
} else if ( V_3 -> V_6 == V_73 ) {
V_61 = 1280 ;
if ( V_3 -> V_181 == 360 )
V_62 = 768 ;
else if ( V_3 -> V_181 == 375 )
V_62 = 800 ;
else if ( V_3 -> V_181 == 405 )
V_62 = 864 ;
else
V_62 = 1024 ;
} else if ( V_3 -> V_6 == V_209 ) {
V_61 = 1280 ;
V_62 = 1024 ;
} else if ( V_3 -> V_6 == V_74 ) {
V_61 = 1280 ;
if ( V_3 -> V_181 == 350 )
V_62 = 700 ;
else if ( V_3 -> V_181 == 400 )
V_62 = 800 ;
else if ( V_3 -> V_181 == 1024 )
V_62 = 960 ;
else
V_62 = 960 ;
} else if ( V_3 -> V_6 == V_210 ) {
V_61 = 1400 ;
V_62 = 1050 ;
if ( V_3 -> V_181 == 1024 ) {
V_61 = 1280 ;
V_62 = 1024 ;
}
} else if ( V_3 -> V_6 == V_322 ) {
V_61 = 1600 ;
V_62 = 1200 ;
if ( ! ( V_3 -> V_8 & V_307 ) ) {
if ( V_3 -> V_181 == 350 )
V_62 = 875 ;
else if ( V_3 -> V_181 == 400 )
V_62 = 1000 ;
}
}
if ( V_3 -> V_8 & V_75 ) {
V_61 = V_3 -> V_179 ;
V_62 = V_3 -> V_181 ;
}
V_3 -> V_180 = V_61 ;
V_3 -> V_182 = V_62 ;
return;
}
if ( V_3 -> V_9 & ( V_47 ) ) {
struct V_192 const * V_330 ;
V_330 = F_38 ( V_29 , V_30 , V_60 ,
V_3 ) ;
V_3 -> V_324 = V_330 -> V_324 ;
V_3 -> V_325 = V_330 -> V_325 ;
V_3 -> V_201 = V_330 -> V_201 ;
V_3 -> V_202 = V_330 -> V_202 ;
V_3 -> V_180 = V_330 -> V_331 ;
V_3 -> V_182 = V_330 -> V_332 ;
V_3 -> V_328 = V_330 -> V_328 ;
V_3 -> V_327 = V_330 -> V_333 ;
if ( V_3 -> V_9 & V_76 ) {
if ( V_63 == 0x08 )
V_3 -> V_327 = 0x40 ;
else if ( V_63 == 0x09 )
V_3 -> V_327 = 0x40 ;
else if ( V_63 == 0x12 )
V_3 -> V_327 = 0x40 ;
if ( V_3 -> V_181 == 350 )
V_3 -> V_10 |= V_79 ;
V_61 = V_334 ;
V_62 = V_335 ;
if ( V_3 -> V_9 & V_49 ) {
if ( V_3 -> V_10 & V_79 ) {
V_61 = V_336 ;
V_62 = V_337 ;
if ( ! ( V_41 & V_45 ) ) {
V_61 = V_338 ;
V_62 = V_339 ;
}
}
}
} else if ( V_3 -> V_9 & V_83 ) {
if ( V_3 -> V_10 & V_250 ) {
V_61 = V_340 ;
V_62 = V_341 ;
}
if ( V_3 -> V_10 & V_252 ) {
V_61 = V_342 ;
V_62 = V_343 ;
} else if ( V_3 -> V_10 & V_295 ) {
V_61 = V_344 ;
V_62 = V_345 ;
if ( V_3 -> V_10 & V_255 )
V_61 = V_346 ;
}
} else {
V_61 = V_347 ;
V_62 = V_348 ;
if ( ! ( V_3 -> V_10 & V_88 ) ) {
V_61 = V_349 ;
V_62 = V_350 ;
if ( V_3 -> V_10 & V_255 )
V_61 = V_346 ;
}
}
V_3 -> V_203 = V_61 ;
V_3 -> V_205 = V_62 ;
}
}
static void F_69 ( unsigned short V_29 , unsigned short V_30 ,
unsigned short V_60 ,
struct V_2 * V_3 )
{
unsigned char V_241 , V_242 , V_193 ;
V_193 = F_46 ( V_60 , V_29 , V_30 ,
V_3 ) ;
F_47 ( V_193 , & V_241 , & V_242 , V_3 ) ;
F_45 ( & V_241 , & V_242 , V_3 ) ;
if ( V_3 -> V_16 & V_257 ) {
F_5 ( V_3 -> V_240 , 0x0A , 0x10 ) ;
F_5 ( V_3 -> V_240 , 0x0B , V_242 ) ;
F_5 ( V_3 -> V_240 , 0x0A , V_241 ) ;
} else {
F_5 ( V_3 -> V_240 , 0x0A , V_241 ) ;
F_5 ( V_3 -> V_240 , 0x0B , V_242 ) ;
}
F_5 ( V_3 -> V_240 , 0x00 , 0x12 ) ;
if ( V_3 -> V_9 & V_68 )
F_21 ( V_3 -> V_240 , 0x12 , 0x28 ) ;
else
F_21 ( V_3 -> V_240 , 0x12 , 0x08 ) ;
}
static unsigned short F_70 ( unsigned short V_29 ,
unsigned short V_30 , struct V_2 * V_3 )
{
unsigned short V_351 [ 6 ] = { 1 , 2 , 4 , 4 , 6 , 8 } ;
short V_102 ;
unsigned short V_41 ;
V_41 = V_42 [ V_30 ] . V_43 ;
V_102 = ( V_41 & V_277 ) - V_135 ;
if ( V_102 < 0 )
V_102 = 0 ;
return V_351 [ V_102 ] ;
}
static unsigned short F_71 ( unsigned short V_29 ,
unsigned short V_30 ,
unsigned short V_60 ,
struct V_35 * V_36 ,
struct V_2 * V_3 )
{
unsigned short V_24 , V_352 , V_353 , V_102 , V_64 ,
V_351 [] = { 0x01 , 0x02 , 0x04 } ;
V_353 = V_42 [ V_30 ] . V_133 ;
V_64 = V_66 [ V_60 ] . V_91 ;
V_102 = ( V_353 >> 8 ) & 0xFF ;
V_24 = V_134 [ V_102 ] ;
if ( V_64 & V_128 )
V_24 = V_24 << 1 ;
V_352 = F_70 ( V_29 , V_30 , V_3 ) ;
if ( ( V_29 >= 0x7C ) && ( V_29 <= 0x7E ) ) {
V_24 = V_29 - 0x7C ;
V_352 = V_351 [ V_24 ] ;
V_24 = 0x6B ;
if ( V_64 & V_128 )
V_24 = V_24 << 1 ;
}
return V_24 * V_352 ;
}
static void F_72 ( unsigned short V_29 ,
unsigned short V_30 ,
unsigned short V_60 ,
struct V_35 * V_36 ,
struct V_2 * V_3 )
{
unsigned short V_354 ;
unsigned char V_24 ;
if ( V_3 -> V_9 & V_49 )
return;
V_354 = F_71 ( V_29 , V_30 , V_60 ,
V_36 , V_3 ) ;
V_24 = ( unsigned char ) ( V_354 & 0xFF ) ;
F_5 ( V_3 -> V_233 , 0x07 , V_24 ) ;
V_24 = ( unsigned char ) ( ( V_354 & 0xFF00 ) >> 8 ) ;
F_5 ( V_3 -> V_233 , 0x09 , V_24 ) ;
V_24 = ( unsigned char ) ( ( ( V_354 >> 3 ) & 0xFF ) + 1 ) ;
F_5 ( V_3 -> V_233 , 0x03 , V_24 ) ;
}
static void F_73 ( struct V_2 * V_3 )
{
F_5 ( V_3 -> V_233 , 0x01 , 0x3B ) ;
F_13 ( V_3 -> V_233 , 0x02 , ~ ( 0x3F ) , 0x04 ) ;
}
static void F_74 ( unsigned short V_29 , unsigned short V_30 ,
struct V_35 * V_36 ,
unsigned short V_60 ,
struct V_2 * V_3 )
{
T_1 V_123 ;
F_72 ( V_29 , V_30 , V_60 ,
V_36 , V_3 ) ;
F_73 ( V_3 ) ;
for ( V_123 = 4 ; V_123 < 7 ; V_123 ++ )
F_5 ( V_3 -> V_233 , V_123 , 0x0 ) ;
F_5 ( V_3 -> V_233 , 0x50 , 0x00 ) ;
F_5 ( V_3 -> V_233 , 0x02 , 0x44 ) ;
}
static void F_75 ( unsigned short V_29 , unsigned short V_30 ,
struct V_35 * V_36 ,
unsigned short V_60 ,
struct V_2 * V_3 )
{
unsigned short V_24 = 0 , V_61 = 0 , V_62 = 0 , V_123 = 0 ,
V_355 = 0 , V_323 , V_41 ;
V_323 = V_66 [ V_60 ] . V_103 ;
V_323 &= V_104 ;
V_41 = V_42 [ V_30 ] . V_43 ;
if ( V_41 & V_127 ) {
V_24 = ( V_3 -> V_201 / 2 - 1 ) & 0x0FF ;
F_5 ( V_3 -> V_233 , 0x08 , V_24 ) ;
V_24 = ( ( ( V_3 -> V_201 / 2 - 1 ) & 0xFF00 ) >> 8 ) << 4 ;
F_13 ( V_3 -> V_233 , 0x09 , ~ 0x0F0 , V_24 ) ;
V_24 = ( V_3 -> V_179 / 2 + 16 ) & 0x0FF ;
F_5 ( V_3 -> V_233 , 0x0A , V_24 ) ;
V_123 = ( ( V_3 -> V_201 - V_3 -> V_179 ) / 2 ) >> 2 ;
V_355 = V_3 -> V_179 / 2 + 16 ;
V_123 = V_123 >> 1 ;
V_62 = V_355 + V_123 ;
V_123 += V_62 ;
if ( V_3 -> V_9 & V_68 ) {
V_62 = V_105 [ V_323 ] . V_106 [ 4 ] ;
V_62 |= ( ( V_105 [ V_323 ] . V_106 [ 14 ] &
0xC0 ) << 2 ) ;
V_62 = ( V_62 - 3 ) << 3 ;
V_123 = V_105 [ V_323 ] . V_106 [ 5 ] ;
V_123 &= 0x1F ;
V_24 = V_105 [ V_323 ] . V_106 [ 15 ] ;
V_24 = ( V_24 & 0x04 ) << ( 5 - 2 ) ;
V_123 = ( ( V_123 | V_24 ) - 3 ) << 3 ;
}
V_62 += 4 ;
V_123 += 4 ;
if ( V_123 > ( V_3 -> V_201 / 2 ) )
V_123 = V_3 -> V_201 / 2 ;
V_24 = V_62 & 0x00FF ;
F_5 ( V_3 -> V_233 , 0x0B , V_24 ) ;
} else {
V_24 = ( V_3 -> V_201 - 1 ) & 0x0FF ;
F_5 ( V_3 -> V_233 , 0x08 , V_24 ) ;
V_24 = ( ( ( V_3 -> V_201 - 1 ) & 0xFF00 ) >> 8 ) << 4 ;
F_13 ( V_3 -> V_233 , 0x09 , ~ 0x0F0 , V_24 ) ;
V_24 = ( V_3 -> V_179 + 16 ) & 0x0FF ;
F_5 ( V_3 -> V_233 , 0x0A , V_24 ) ;
V_123 = ( V_3 -> V_201 - V_3 -> V_179 ) >> 2 ;
V_355 = V_3 -> V_179 + 16 ;
V_123 = V_123 >> 1 ;
V_62 = V_355 + V_123 ;
V_123 += V_62 ;
if ( V_3 -> V_9 & V_68 ) {
V_62 = V_105 [ V_323 ] . V_106 [ 3 ] ;
V_62 |= ( ( V_105 [ V_323 ] . V_106 [ 5 ] &
0xC0 ) << 2 ) ;
V_62 = ( V_62 - 3 ) << 3 ;
V_123 = V_105 [ V_323 ] . V_106 [ 4 ] ;
V_123 &= 0x1F ;
V_24 = V_105 [ V_323 ] . V_106 [ 6 ] ;
V_24 = ( V_24 & 0x04 ) << ( 5 - 2 ) ;
V_123 = ( ( V_123 | V_24 ) - 3 ) << 3 ;
V_62 += 16 ;
V_123 += 16 ;
}
if ( V_123 > V_3 -> V_201 )
V_123 = V_3 -> V_201 ;
V_24 = V_62 & 0x00FF ;
F_5 ( V_3 -> V_233 , 0x0B , V_24 ) ;
}
V_61 = ( V_61 & 0x00FF ) | ( V_62 & 0xFF00 ) ;
V_62 = V_355 ;
V_62 = ( V_62 & 0x00FF ) | ( ( V_62 & 0xFF00 ) << 4 ) ;
V_61 |= ( V_62 & 0xFF00 ) ;
V_24 = ( V_61 & 0xFF00 ) >> 8 ;
F_5 ( V_3 -> V_233 , 0x0C , V_24 ) ;
V_24 = V_123 & 0x00FF ;
F_5 ( V_3 -> V_233 , 0x0D , V_24 ) ;
V_123 = ( V_3 -> V_202 - 1 ) ;
V_24 = V_123 & 0x00FF ;
F_5 ( V_3 -> V_233 , 0x0E , V_24 ) ;
V_62 = V_3 -> V_181 - 1 ;
V_24 = V_62 & 0x00FF ;
F_5 ( V_3 -> V_233 , 0x0F , V_24 ) ;
V_24 = ( ( V_62 & 0xFF00 ) << 3 ) >> 8 ;
V_24 |= ( ( V_123 & 0xFF00 ) >> 8 ) ;
F_5 ( V_3 -> V_233 , 0x12 , V_24 ) ;
V_62 = ( V_3 -> V_202 + V_3 -> V_181 ) >> 1 ;
V_123 = ( ( V_3 -> V_202 - V_3 -> V_181 ) >> 4 ) + V_62 + 1 ;
if ( V_3 -> V_9 & V_68 ) {
V_62 = V_105 [ V_323 ] . V_106 [ 10 ] ;
V_24 = V_105 [ V_323 ] . V_106 [ 9 ] ;
if ( V_24 & 0x04 )
V_62 |= 0x0100 ;
if ( V_24 & 0x080 )
V_62 |= 0x0200 ;
V_24 = V_105 [ V_323 ] . V_106 [ 14 ] ;
if ( V_24 & 0x08 )
V_62 |= 0x0400 ;
V_24 = V_105 [ V_323 ] . V_106 [ 11 ] ;
V_123 = ( V_123 & 0xFF00 ) | ( V_24 & 0x00FF ) ;
}
V_24 = V_62 & 0x00FF ;
F_5 ( V_3 -> V_233 , 0x10 , V_24 ) ;
V_24 = ( ( V_62 & 0xFF00 ) >> 8 ) << 4 ;
V_24 = ( ( V_123 & 0x000F ) | ( V_24 ) ) ;
F_5 ( V_3 -> V_233 , 0x11 , V_24 ) ;
V_61 = 0 ;
if ( V_41 & V_101 )
V_61 |= 0x80 ;
if ( V_41 & V_127 )
V_61 |= 0x40 ;
F_13 ( V_3 -> V_233 , 0x2C , ~ 0x0C0 , V_61 ) ;
}
static unsigned short F_76 ( struct V_2 * V_3 )
{
unsigned long V_61 , V_62 ;
V_62 = ( ( V_3 -> V_202 - V_3 -> V_181 ) * V_3 -> V_324 )
& 0xFFFF ;
V_61 = ( V_3 -> V_205 - V_3 -> V_182 ) * V_3 -> V_325 ;
V_61 = ( V_61 * V_3 -> V_203 ) / V_62 ;
return ( unsigned short ) V_61 ;
}
static void F_77 ( unsigned short V_29 , unsigned short V_30 ,
struct V_35 * V_36 ,
unsigned short V_60 ,
struct V_2 * V_3 )
{
unsigned short V_225 , V_226 , V_61 , V_62 = 0 , V_123 , V_24 , V_63 ,
V_41 ;
V_41 = V_42 [ V_30 ] . V_43 ;
V_63 = V_42 [ V_30 ] . V_65 ;
if ( ! ( V_3 -> V_9 & V_49 ) )
return;
V_24 = 0xFF ;
F_5 ( V_3 -> V_233 , 0x03 , V_24 ) ;
V_123 = 0x08 ;
if ( V_3 -> V_16 & ( V_17 | V_18 | V_70 ) )
V_41 |= V_45 ;
V_61 = V_3 -> V_179 ;
if ( V_41 & V_127 )
V_61 = V_61 >> 1 ;
V_61 = ( V_61 / V_123 ) - 1 ;
V_62 |= ( ( V_61 & 0x00FF ) << 8 ) ;
V_24 = V_61 & 0x00FF ;
F_5 ( V_3 -> V_233 , 0x04 , V_24 ) ;
V_24 = ( V_62 & 0xFF00 ) >> 8 ;
if ( V_3 -> V_9 & V_47 ) {
if ( ! ( V_3 -> V_16 & ( V_85 | V_86 | V_17
| V_18 | V_70 ) ) )
V_24 += 2 ;
if ( ( V_3 -> V_9 & V_76 ) &&
! ( V_3 -> V_16 & V_17 ) && ( V_63 == 7 ) )
V_24 -= 2 ;
}
F_5 ( V_3 -> V_233 , 0x05 , V_24 ) ;
F_5 ( V_3 -> V_233 , 0x06 , 0x03 ) ;
if ( ! ( V_3 -> V_9 & V_279 ) ) {
if ( V_3 -> V_9 & V_47 )
V_61 = V_3 -> V_201 ;
else
V_61 = F_76 ( V_3 ) ;
}
if ( V_61 >= V_3 -> V_201 )
V_61 = V_3 -> V_201 ;
if ( V_41 & V_127 )
V_61 = V_61 >> 1 ;
V_61 = ( V_61 / V_123 ) - 5 ;
V_123 = V_61 ;
if ( V_3 -> V_9 & V_76 ) {
V_24 = ( V_62 & 0x00FF ) - 1 ;
if ( ! ( V_41 & V_127 ) ) {
V_24 -= 6 ;
if ( V_3 -> V_10 & V_79 ) {
V_24 -= 4 ;
V_24 -= 10 ;
}
}
} else {
V_62 = ( V_62 & 0xFF00 ) >> 8 ;
V_123 = ( V_123 + V_62 ) >> 1 ;
V_24 = ( V_123 & 0x00FF ) + 2 ;
if ( V_3 -> V_9 & V_47 ) {
V_24 -= 1 ;
if ( ! ( V_41 & V_127 ) ) {
if ( ( V_41 & V_45 ) ) {
V_24 += 4 ;
if ( V_3 -> V_179 >= 800 )
V_24 -= 6 ;
}
}
} else if ( ! ( V_41 & V_127 ) ) {
V_24 -= 4 ;
if ( V_3 -> V_6 != V_74 &&
V_3 -> V_179 >= 800 ) {
V_24 -= 7 ;
if ( V_3 -> V_179 >= 1280 &&
V_3 -> V_6 != V_74 &&
( V_3 -> V_8 & V_75 ) )
V_24 += 28 ;
}
}
}
F_5 ( V_3 -> V_233 , 0x07 , V_24 ) ;
F_5 ( V_3 -> V_233 , 0x08 , 0 ) ;
if ( V_3 -> V_9 & V_47 ) {
if ( V_3 -> V_10 & V_79 ) {
if ( V_29 == 0x50 ) {
if ( V_3 -> V_10 == V_356 ) {
F_5 ( V_3 -> V_233 ,
0x07 , 0x30 ) ;
F_5 ( V_3 -> V_233 ,
0x08 , 0x03 ) ;
} else {
F_5 ( V_3 -> V_233 ,
0x07 , 0x2f ) ;
F_5 ( V_3 -> V_233 ,
0x08 , 0x02 ) ;
}
}
}
}
F_5 ( V_3 -> V_233 , 0x18 , 0x03 ) ;
F_13 ( V_3 -> V_233 , 0x19 , 0xF0 , 0x00 ) ;
F_5 ( V_3 -> V_233 , 0x09 , 0xFF ) ;
V_62 = V_3 -> V_202 ;
V_225 = V_62 ;
V_123 = 0x121 ;
V_62 = V_3 -> V_181 ;
if ( V_62 == 357 )
V_62 = 350 ;
if ( V_62 == 360 )
V_62 = 350 ;
if ( V_62 == 375 )
V_62 = 350 ;
if ( V_62 == 405 )
V_62 = 400 ;
if ( V_62 == 525 )
V_62 = 480 ;
V_226 = V_62 ;
if ( V_3 -> V_9 & V_48 ) {
if ( V_3 -> V_6 == V_137 ) {
if ( ! ( V_3 -> V_8 & V_307 ) ) {
if ( V_62 == 350 )
V_62 += 5 ;
if ( V_62 == 480 )
V_62 += 5 ;
}
}
}
V_62 -- ;
V_62 -- ;
V_24 = V_62 & 0x00FF ;
F_5 ( V_3 -> V_233 , 0x10 , V_24 ) ;
V_62 = V_226 ;
V_62 -- ;
V_24 = V_62 & 0x00FF ;
F_5 ( V_3 -> V_233 , 0x0E , V_24 ) ;
if ( V_62 & 0x0100 )
V_123 |= 0x0002 ;
V_61 = 0x000B ;
if ( V_41 & V_101 )
V_61 |= 0x08000 ;
if ( V_62 & 0x0200 )
V_123 |= 0x0040 ;
V_24 = ( V_61 & 0xFF00 ) >> 8 ;
F_5 ( V_3 -> V_233 , 0x0B , V_24 ) ;
if ( V_62 & 0x0400 )
V_123 |= 0x0600 ;
F_5 ( V_3 -> V_233 , 0x11 , 0x00 ) ;
V_61 = V_225 ;
V_61 -= V_62 ;
V_61 = V_61 >> 2 ;
V_225 = V_61 ;
if ( V_63 != 0x09 ) {
V_61 = V_61 << 1 ;
V_62 += V_61 ;
}
if ( V_3 -> V_9 & V_76 ) {
if ( ( V_3 -> V_16 & V_17 ) &&
! ( V_3 -> V_10 & V_298 ) ) {
if ( ( V_3 -> V_10 & V_79 ) &&
( V_3 -> V_10 & V_88 ) ) {
if ( ! ( V_3 -> V_16 & V_17 ) ||
! ( V_3 -> V_10 &
( V_252 |
V_250 |
V_298 ) ) )
V_62 += 40 ;
}
} else {
V_62 -= 10 ;
}
} else if ( V_3 -> V_10 & V_79 ) {
if ( V_3 -> V_10 & V_88 ) {
if ( V_3 -> V_16 & V_17 ) {
if ( ! ( V_3 -> V_10 &
( V_252 |
V_250 |
V_298 ) ) )
V_62 += 40 ;
} else {
V_62 += 40 ;
}
}
}
V_61 = V_225 ;
V_61 = V_61 >> 2 ;
V_61 ++ ;
V_61 += V_62 ;
V_225 = V_61 ;
if ( ( V_3 -> V_10 & V_88 ) ) {
if ( V_62 <= 513 ) {
if ( V_61 >= 513 )
V_62 = 513 ;
}
}
V_24 = V_62 & 0x00FF ;
F_5 ( V_3 -> V_233 , 0x0C , V_24 ) ;
V_62 -- ;
V_24 = V_62 & 0x00FF ;
F_5 ( V_3 -> V_233 , 0x10 , V_24 ) ;
if ( V_62 & 0x0100 )
V_123 |= 0x0008 ;
if ( V_62 & 0x0200 )
F_13 ( V_3 -> V_233 , 0x0B , 0x0FF , 0x20 ) ;
V_62 ++ ;
if ( V_62 & 0x0100 )
V_123 |= 0x0004 ;
if ( V_62 & 0x0200 )
V_123 |= 0x0080 ;
if ( V_62 & 0x0400 )
V_123 |= 0x0C00 ;
V_62 = V_225 ;
V_24 = V_62 & 0x00FF ;
V_24 &= 0x0F ;
F_5 ( V_3 -> V_233 , 0x0D , V_24 ) ;
if ( V_62 & 0x0010 )
V_123 |= 0x2000 ;
V_24 = V_123 & 0x00FF ;
F_5 ( V_3 -> V_233 , 0x0A , V_24 ) ;
V_24 = ( V_123 & 0x0FF00 ) >> 8 ;
F_5 ( V_3 -> V_233 , 0x17 , V_24 ) ;
V_61 = V_41 ;
V_24 = ( V_61 & 0xFF00 ) >> 8 ;
V_24 = ( V_24 >> 1 ) & 0x09 ;
if ( V_3 -> V_16 & ( V_17 | V_18 | V_70 ) )
V_24 |= 0x01 ;
F_5 ( V_3 -> V_233 , 0x16 , V_24 ) ;
F_5 ( V_3 -> V_233 , 0x0F , 0 ) ;
F_5 ( V_3 -> V_233 , 0x12 , 0 ) ;
if ( V_3 -> V_8 & V_357 )
V_24 = 0x80 ;
else
V_24 = 0x00 ;
F_5 ( V_3 -> V_233 , 0x1A , V_24 ) ;
}
static void F_78 ( unsigned short V_29 , unsigned short V_30 ,
unsigned short V_60 ,
struct V_35 * V_36 ,
struct V_2 * V_3 )
{
unsigned short V_32 , V_97 , V_61 , V_62 , V_123 , V_24 , V_225 , V_226 ,
V_41 ;
unsigned char const * V_358 ;
unsigned long V_359 , V_360 , V_361 , V_131 , V_362 ;
V_41 = V_42 [ V_30 ] . V_43 ;
V_61 = 0 ;
if ( ! ( V_3 -> V_9 & V_80 ) )
V_61 |= 0x0800 ;
if ( ! ( V_3 -> V_9 & V_81 ) )
V_61 |= 0x0400 ;
if ( V_3 -> V_9 & V_82 )
V_61 |= 0x0200 ;
if ( ! ( V_3 -> V_10 & V_88 ) )
V_61 |= 0x1000 ;
if ( V_3 -> V_9 & V_76 )
V_61 |= 0x0100 ;
if ( V_3 -> V_10 & ( V_252 | V_250 ) )
V_61 &= 0xfe00 ;
V_61 = ( V_61 & 0xff00 ) >> 8 ;
F_5 ( V_3 -> V_263 , 0x0 , V_61 ) ;
V_358 = V_363 ;
if ( V_3 -> V_10 & V_88 )
V_358 = V_364 ;
if ( V_3 -> V_9 & V_76 ) {
V_358 = V_365 ;
if ( V_3 -> V_9 & V_49 )
V_358 = V_366 ;
if ( V_3 -> V_78 & V_79 )
V_358 = V_367 ;
if ( ! ( V_41 & V_45 ) )
V_358 = V_368 ;
}
if ( V_3 -> V_9 & V_83 ) {
if ( V_3 -> V_10 & V_295 )
V_358 = V_369 ;
if ( V_3 -> V_10 & V_252 )
V_358 = V_370 ;
if ( V_3 -> V_10 & V_250 )
V_358 = V_371 ;
}
for ( V_32 = 0x01 , V_97 = 0 ; V_32 <= 0x2D ; V_32 ++ , V_97 ++ )
F_5 ( V_3 -> V_263 , V_32 , V_358 [ V_97 ] ) ;
for ( V_32 = 0x39 ; V_32 <= 0x45 ; V_32 ++ , V_97 ++ )
F_5 ( V_3 -> V_263 , V_32 , V_358 [ V_97 ] ) ;
if ( V_3 -> V_9 & V_47 )
F_13 ( V_3 -> V_263 , 0x3A , 0x1F , 0x00 ) ;
V_24 = V_3 -> V_327 ;
V_24 &= 0x80 ;
F_13 ( V_3 -> V_263 , 0x0A , 0xFF , V_24 ) ;
if ( V_3 -> V_10 & V_88 )
V_61 = 520 ;
else
V_61 = 440 ;
if ( V_3 -> V_182 <= V_61 ) {
V_61 -= V_3 -> V_182 ;
V_61 = V_61 >> 2 ;
V_61 = ( V_61 & 0x00FF ) | ( ( V_61 & 0x00FF ) << 8 ) ;
V_225 = V_61 ;
V_24 = ( V_61 & 0xFF00 ) >> 8 ;
V_24 += ( unsigned short ) V_358 [ 0 ] ;
if ( V_3 -> V_16 & ( V_85 | V_86 | V_17
| V_18 | V_70 ) ) {
if ( V_3 -> V_9 & ( V_80
| V_81 | V_82
| V_83 ) ) {
V_123 = V_3 -> V_179 ;
if ( V_123 >= 1024 ) {
V_24 = 0x17 ;
if ( V_3 -> V_10 & V_88 )
V_24 = 0x19 ;
}
}
}
F_5 ( V_3 -> V_263 , 0x01 , V_24 ) ;
V_61 = V_225 ;
V_24 = ( V_61 & 0xFF00 ) >> 8 ;
V_24 += V_358 [ 1 ] ;
if ( V_3 -> V_16 & ( V_85 | V_86 | V_17
| V_18 | V_70 ) ) {
if ( ( V_3 -> V_9 & ( V_80
| V_81 | V_82
| V_83 ) ) ) {
V_123 = V_3 -> V_179 ;
if ( V_123 >= 1024 ) {
V_24 = 0x1D ;
if ( V_3 -> V_10 & V_88 )
V_24 = 0x52 ;
}
}
}
F_5 ( V_3 -> V_263 , 0x02 , V_24 ) ;
}
V_123 = V_3 -> V_203 ;
if ( F_66 ( V_3 ) )
V_123 = V_123 >> 1 ;
V_123 -= 2 ;
V_24 = V_123 & 0x00FF ;
F_5 ( V_3 -> V_263 , 0x1B , V_24 ) ;
V_24 = ( V_123 & 0xFF00 ) >> 8 ;
F_13 ( V_3 -> V_263 , 0x1D , ~ 0x0F , V_24 ) ;
V_123 = V_3 -> V_203 >> 1 ;
V_225 = V_123 ;
V_123 += 7 ;
if ( V_3 -> V_9 & V_76 )
V_123 -= 4 ;
V_24 = V_123 & 0x00FF ;
V_24 = V_24 << 4 ;
F_13 ( V_3 -> V_263 , 0x22 , 0x0F , V_24 ) ;
V_62 = V_358 [ V_97 ] | ( ( V_358 [ V_97 + 1 ] ) << 8 ) ;
V_62 += V_123 ;
V_226 = V_62 ;
V_24 = V_62 & 0x00FF ;
F_5 ( V_3 -> V_263 , 0x24 , V_24 ) ;
V_24 = ( V_62 & 0xFF00 ) >> 8 ;
V_24 = V_24 << 4 ;
F_13 ( V_3 -> V_263 , 0x25 , 0x0F , V_24 ) ;
V_62 = V_226 ;
V_62 = V_62 + 8 ;
if ( V_3 -> V_9 & V_76 ) {
V_62 = V_62 - 4 ;
V_123 = V_62 ;
}
V_24 = ( V_62 & 0x00FF ) << 4 ;
F_13 ( V_3 -> V_263 , 0x29 , 0x0F , V_24 ) ;
V_97 += 2 ;
V_123 += ( V_358 [ V_97 ] | ( ( V_358 [ V_97 + 1 ] ) << 8 ) ) ;
V_24 = V_123 & 0x00FF ;
F_5 ( V_3 -> V_263 , 0x27 , V_24 ) ;
V_24 = ( ( V_123 & 0xFF00 ) >> 8 ) << 4 ;
F_13 ( V_3 -> V_263 , 0x28 , 0x0F , V_24 ) ;
V_123 += 8 ;
if ( V_3 -> V_9 & V_76 )
V_123 -= 4 ;
V_24 = V_123 & 0xFF ;
V_24 = V_24 << 4 ;
F_13 ( V_3 -> V_263 , 0x2A , 0x0F , V_24 ) ;
V_123 = V_225 ;
V_97 += 2 ;
V_24 = V_358 [ V_97 ] | ( ( V_358 [ V_97 + 1 ] ) << 8 ) ;
V_123 -= V_24 ;
V_24 = V_123 & 0x00FF ;
V_24 = V_24 << 4 ;
F_13 ( V_3 -> V_263 , 0x2D , 0x0F , V_24 ) ;
V_123 -= 11 ;
if ( ! ( V_3 -> V_9 & V_47 ) ) {
V_61 = F_76 ( V_3 ) ;
V_123 = V_61 - 1 ;
}
V_24 = V_123 & 0x00FF ;
F_5 ( V_3 -> V_263 , 0x2E , V_24 ) ;
V_62 = V_3 -> V_182 ;
if ( V_3 -> V_181 == 360 )
V_62 = 746 ;
if ( V_3 -> V_181 == 375 )
V_62 = 746 ;
if ( V_3 -> V_181 == 405 )
V_62 = 853 ;
if ( V_3 -> V_9 & V_47 ) {
if ( V_3 -> V_16 &
( V_17 | V_18 | V_70 ) ) {
if ( ! ( V_3 -> V_10 &
( V_252 | V_250 ) ) )
V_62 = V_62 >> 1 ;
} else
V_62 = V_62 >> 1 ;
}
V_62 -= 2 ;
V_24 = V_62 & 0x00FF ;
if ( V_3 -> V_9 & V_76 ) {
if ( V_3 -> V_16 & V_17 ) {
if ( V_3 -> V_10 & V_298 ) {
if ( V_3 -> V_9 & V_49 ) {
if ( V_29 == 0x2f )
V_24 += 1 ;
}
}
} else if ( V_3 -> V_9 & V_49 ) {
if ( V_29 == 0x2f )
V_24 += 1 ;
}
}
F_5 ( V_3 -> V_263 , 0x2F , V_24 ) ;
V_24 = ( V_123 & 0xFF00 ) >> 8 ;
V_24 |= ( ( V_62 & 0xFF00 ) >> 8 ) << 6 ;
if ( ! ( V_3 -> V_9 & V_76 ) ) {
if ( V_3 -> V_16 & V_17 ) {
if ( V_3 -> V_10 & V_298 ) {
V_24 |= 0x10 ;
if ( ! ( V_3 -> V_9 & V_81 ) )
V_24 |= 0x20 ;
}
} else {
V_24 |= 0x10 ;
if ( ! ( V_3 -> V_9 & V_81 ) )
V_24 |= 0x20 ;
}
}
F_5 ( V_3 -> V_263 , 0x30 , V_24 ) ;
if ( V_3 -> V_16 & ( V_85 | V_86 | V_17
| V_18 | V_70 ) ) {
V_62 = V_3 -> V_182 ;
V_123 = V_62 - 2 ;
if ( V_3 -> V_9 & V_47 ) {
if ( ! ( V_3 -> V_10 & ( V_252
| V_250 ) ) )
V_62 = V_62 >> 1 ;
}
if ( V_3 -> V_16 & ( V_18 | V_70 ) ) {
V_24 = 0 ;
if ( V_123 & 0x0400 )
V_24 |= 0x20 ;
if ( V_62 & 0x0400 )
V_24 |= 0x40 ;
F_5 ( V_3 -> V_240 , 0x10 , V_24 ) ;
}
V_24 = ( ( ( V_62 - 3 ) & 0x0300 ) >> 8 ) << 5 ;
F_5 ( V_3 -> V_263 , 0x46 , V_24 ) ;
V_24 = ( V_62 - 3 ) & 0x00FF ;
F_5 ( V_3 -> V_263 , 0x47 , V_24 ) ;
}
V_62 = V_62 & 0x00FF ;
if ( ! ( V_41 & V_127 ) ) {
V_123 = V_3 -> V_179 ;
if ( V_123 >= V_3 -> V_180 ) {
V_62 |= 0x2000 ;
V_61 &= 0x00FF ;
}
}
V_123 = 0x0101 ;
if ( V_3 -> V_9 & V_47 ) {
if ( V_3 -> V_179 >= 1024 ) {
V_123 = 0x1920 ;
if ( V_3 -> V_179 >= 1280 ) {
V_123 = 0x1420 ;
V_62 = V_62 & 0xDFFF ;
}
}
}
if ( ! ( V_62 & 0x2000 ) ) {
if ( V_41 & V_127 )
V_123 = ( V_123 & 0xFF00 ) | ( ( V_123 & 0x00FF ) << 1 ) ;
V_225 = V_62 ;
V_360 = V_3 -> V_179 ;
V_361 = ( V_123 & 0xFF00 ) >> 8 ;
V_359 = V_360 * V_361 ;
V_362 = V_123 & 0x00FF ;
V_359 = V_359 / V_362 ;
V_362 = 8 * 1024 ;
if ( V_3 -> V_16 & ( V_85 | V_86 | V_17
| V_18 | V_70 ) ) {
V_362 = V_362 * 8 ;
}
V_359 = V_359 * V_362 ;
V_362 = V_3 -> V_180 ;
V_131 = V_359 % V_362 ;
V_360 = V_359 / V_362 ;
if ( V_131 != 0 )
V_360 += 1 ;
V_61 = ( unsigned short ) V_360 ;
if ( V_3 -> V_16 & ( V_85 | V_86 | V_17
| V_18 | V_70 ) ) {
V_123 = ( ( V_61 & 0xFF00 ) >> 5 ) >> 8 ;
}
V_62 = V_225 ;
V_62 = ( unsigned short ) ( ( ( V_360 & 0x0000FF00 ) & 0x1F00 )
| ( V_62 & 0x00FF ) ) ;
V_61 = ( unsigned short ) ( ( ( V_360 & 0x000000FF ) << 8 )
| ( V_61 & 0x00FF ) ) ;
V_24 = ( V_61 & 0xFF00 ) >> 8 ;
} else {
V_24 = ( V_61 & 0x00FF ) >> 8 ;
}
F_5 ( V_3 -> V_263 , 0x44 , V_24 ) ;
V_24 = ( V_62 & 0xFF00 ) >> 8 ;
F_13 ( V_3 -> V_263 , 0x45 , ~ 0x03F , V_24 ) ;
V_24 = V_123 & 0x00FF ;
if ( V_62 & 0x2000 )
V_24 = 0 ;
if ( ! ( V_3 -> V_9 & V_48 ) )
V_24 |= 0x18 ;
F_13 ( V_3 -> V_263 , 0x46 , ~ 0x1F , V_24 ) ;
if ( V_3 -> V_10 & V_88 ) {
V_62 = 0x0382 ;
V_123 = 0x007e ;
} else {
V_62 = 0x0369 ;
V_123 = 0x0061 ;
}
V_24 = V_62 & 0x00FF ;
F_5 ( V_3 -> V_263 , 0x4b , V_24 ) ;
V_24 = V_123 & 0x00FF ;
F_5 ( V_3 -> V_263 , 0x4c , V_24 ) ;
V_24 = ( ( V_123 & 0xFF00 ) >> 8 ) & 0x03 ;
V_24 = V_24 << 2 ;
V_24 |= ( ( V_62 & 0xFF00 ) >> 8 ) & 0x03 ;
if ( V_3 -> V_9 & V_83 ) {
V_24 |= 0x10 ;
if ( V_3 -> V_10 & V_252 )
V_24 |= 0x20 ;
if ( V_3 -> V_10 & V_250 )
V_24 |= 0x60 ;
}
F_5 ( V_3 -> V_263 , 0x4d , V_24 ) ;
V_24 = F_3 ( V_3 -> V_263 , 0x43 ) ;
F_5 ( V_3 -> V_263 , 0x43 , ( unsigned short ) ( V_24 - 3 ) ) ;
if ( ! ( V_3 -> V_10 & ( V_252 | V_250 ) ) ) {
if ( V_3 -> V_10 & V_255 ) {
V_358 = V_372 ;
for ( V_32 = 0x1c , V_97 = 0 ; V_32 <= 0x30 ; V_32 ++ , V_97 ++ ) {
F_5 ( V_3 -> V_263 , V_32 ,
V_358 [ V_97 ] ) ;
}
F_5 ( V_3 -> V_263 , 0x43 , 0x72 ) ;
}
}
if ( V_3 -> V_16 & V_70 ) {
if ( V_3 -> V_10 & V_291 )
F_13 ( V_3 -> V_263 , 0x4E , ~ 0x08 ,
0x08 ) ;
}
if ( V_3 -> V_10 & V_291 ) {
V_61 = F_3 ( V_3 -> V_263 , 0x01 ) ;
V_61 -- ;
F_14 ( V_3 -> V_263 , 0x01 , V_61 ) ;
F_14 ( V_3 -> V_263 , 0x00 , 0xEF ) ;
}
if ( V_3 -> V_9 & V_76 ) {
if ( ! ( V_3 -> V_9 & V_49 ) )
F_5 ( V_3 -> V_263 , 0x0B , 0x00 ) ;
}
}
static void F_79 ( unsigned short V_29 , unsigned short V_30 ,
struct V_35 * V_36 ,
unsigned short V_60 ,
struct V_2 * V_3 )
{
unsigned short V_355 , V_61 , V_62 , V_123 , V_24 , V_217 ,
V_313 , V_259 ;
struct V_373 const * V_374 = NULL ;
if ( ! ( V_3 -> V_9 & V_48 ) )
return;
V_62 = V_3 -> V_180 ;
if ( F_66 ( V_3 ) )
V_62 = V_62 >> 1 ;
V_62 -= 1 ;
V_24 = V_62 & 0x00FF ;
F_5 ( V_3 -> V_263 , 0x2C , V_24 ) ;
V_24 = ( V_62 & 0xFF00 ) >> 8 ;
V_24 = V_24 << 4 ;
F_13 ( V_3 -> V_263 , 0x2B , 0x0F , V_24 ) ;
V_24 = 0x01 ;
F_5 ( V_3 -> V_263 , 0x0B , V_24 ) ;
V_62 = V_3 -> V_182 ;
V_62 -- ;
V_24 = V_62 & 0x00FF ;
F_5 ( V_3 -> V_263 , 0x03 , V_24 ) ;
V_24 = ( ( V_62 & 0xFF00 ) >> 8 ) & 0x07 ;
F_13 ( V_3 -> V_263 , 0x0C , ~ 0x07 , V_24 ) ;
V_123 = V_3 -> V_205 - 1 ;
V_24 = V_123 & 0x00FF ;
F_5 ( V_3 -> V_263 , 0x19 , V_24 ) ;
V_24 = ( V_123 & 0xFF00 ) >> 8 ;
V_24 = V_24 << 5 ;
F_5 ( V_3 -> V_263 , 0x1A , V_24 ) ;
F_13 ( V_3 -> V_263 , 0x09 , 0xF0 , 0x00 ) ;
F_13 ( V_3 -> V_263 , 0x0A , 0xF0 , 0x00 ) ;
F_13 ( V_3 -> V_263 , 0x17 , 0xFB , 0x00 ) ;
F_13 ( V_3 -> V_263 , 0x18 , 0xDF , 0x00 ) ;
if ( ( V_3 -> V_16 & V_17 ) || ( V_3 -> V_16 & V_18 ) )
V_374 = F_37 ( V_375 , V_29 , V_30 ,
V_60 , V_3 ) ;
else
V_374 = F_37 ( V_376 , V_29 ,
V_30 , V_60 ,
V_3 ) ;
V_217 = V_3 -> V_6 ;
V_217 &= V_187 ;
if ( ( V_217 == V_137 ) || ( V_217 == V_208 ) ) {
V_62 = 1024 ;
V_123 = 768 ;
} else if ( ( V_217 == V_73 ) ||
( V_217 == V_209 ) ) {
V_62 = 1280 ;
V_123 = 1024 ;
} else if ( V_217 == V_210 ) {
V_62 = 1400 ;
V_123 = 1050 ;
} else {
V_62 = 1600 ;
V_123 = 1200 ;
}
if ( V_3 -> V_8 & V_186 ) {
V_62 = V_3 -> V_180 ;
V_123 = V_3 -> V_182 ;
}
V_355 = V_62 ;
V_61 = V_3 -> V_205 ;
V_3 -> V_232 = V_374 -> V_232 ;
V_3 -> V_234 = V_374 -> V_234 ;
V_3 -> V_236 = V_374 -> V_236 ;
V_3 -> V_237 = V_374 -> V_237 ;
V_62 = V_3 -> V_236 ;
V_123 += V_62 ;
if ( V_123 >= V_61 )
V_123 -= V_61 ;
V_24 = V_62 & 0x00FF ;
F_5 ( V_3 -> V_263 , 0x05 , V_24 ) ;
V_24 = V_123 & 0x00FF ;
F_5 ( V_3 -> V_263 , 0x06 , V_24 ) ;
V_259 = ( ( V_123 & 0xFF00 ) >> 8 ) & 0x07 ;
V_313 = ( ( V_62 & 0xFF00 ) >> 8 ) & 0x07 ;
V_217 = V_259 ;
V_217 = V_217 << 3 ;
V_217 |= V_313 ;
F_5 ( V_3 -> V_263 , 0x02 , V_217 ) ;
F_43 ( & V_61 , & V_62 , V_3 ) ;
V_123 = V_62 ;
V_61 = V_3 -> V_205 ;
V_62 = V_3 -> V_237 ;
V_123 += V_62 ;
if ( V_123 >= V_61 )
V_123 -= V_61 ;
V_24 = V_62 & 0x00FF ;
F_5 ( V_3 -> V_263 , 0x04 , V_24 ) ;
V_24 = ( V_62 & 0xFF00 ) >> 8 ;
V_24 = V_24 << 4 ;
V_24 |= ( V_123 & 0x000F ) ;
F_5 ( V_3 -> V_263 , 0x01 , V_24 ) ;
V_123 = V_355 ;
V_61 = V_3 -> V_203 ;
V_62 = V_3 -> V_232 ;
V_62 &= 0x0FFF ;
if ( F_66 ( V_3 ) ) {
V_61 = V_61 >> 1 ;
V_62 = V_62 >> 1 ;
V_123 = V_123 >> 1 ;
}
if ( V_3 -> V_16 & V_18 )
V_62 += 1 ;
if ( V_3 -> V_16 & V_70 )
V_62 += 1 ;
V_123 += V_62 ;
if ( V_123 >= V_61 )
V_123 -= V_61 ;
V_24 = V_62 & 0x00FF ;
F_5 ( V_3 -> V_263 , 0x1F , V_24 ) ;
V_24 = ( ( V_62 & 0xFF00 ) >> 8 ) << 4 ;
F_5 ( V_3 -> V_263 , 0x20 , V_24 ) ;
V_24 = V_123 & 0x00FF ;
F_5 ( V_3 -> V_263 , 0x23 , V_24 ) ;
V_24 = ( V_123 & 0xFF00 ) >> 8 ;
F_5 ( V_3 -> V_263 , 0x25 , V_24 ) ;
F_43 ( & V_61 , & V_62 , V_3 ) ;
V_123 = V_61 ;
V_61 = V_3 -> V_203 ;
V_62 = V_3 -> V_234 ;
if ( F_66 ( V_3 ) ) {
V_61 = V_61 >> 1 ;
V_62 = V_62 >> 1 ;
V_123 = V_123 >> 1 ;
}
if ( V_3 -> V_16 & V_18 )
V_62 += 1 ;
V_123 += V_62 ;
if ( V_123 >= V_61 )
V_123 -= V_61 ;
V_24 = V_62 & 0x00FF ;
F_5 ( V_3 -> V_263 , 0x1C , V_24 ) ;
V_24 = ( V_62 & 0xFF00 ) >> 8 ;
V_24 = V_24 << 4 ;
F_13 ( V_3 -> V_263 , 0x1D , ~ 0x0F0 , V_24 ) ;
V_24 = V_123 & 0x00FF ;
F_5 ( V_3 -> V_263 , 0x21 , V_24 ) ;
if ( ! ( V_3 -> V_8 & V_307 ) ) {
if ( V_3 -> V_181 == 525 ) {
if ( V_3 -> V_16 & ( V_85 | V_86
| V_17 | V_18
| V_70 ) ) {
V_24 = 0xC6 ;
} else
V_24 = 0xC4 ;
F_5 ( V_3 -> V_263 , 0x2f , V_24 ) ;
F_5 ( V_3 -> V_263 , 0x30 , 0xB3 ) ;
}
if ( V_3 -> V_181 == 420 ) {
if ( V_3 -> V_16 & ( V_85 | V_86
| V_17 | V_18
| V_70 ) ) {
V_24 = 0x4F ;
} else
V_24 = 0x4E ;
F_5 ( V_3 -> V_263 , 0x2f , V_24 ) ;
}
}
}
static struct V_377 const
* F_80 ( unsigned short V_123 , struct V_2 * V_3 )
{
unsigned short V_61 , V_62 , V_32 ;
struct V_377 const * V_378 ;
if ( V_123 == 0 ) {
V_61 = V_3 -> V_179 ;
V_62 = V_3 -> V_180 ;
} else {
V_61 = V_3 -> V_181 ;
V_62 = V_3 -> V_182 ;
}
if ( V_61 <= V_62 )
return & V_379 [ 0 ] ;
else
V_378 = V_380 ;
if ( V_3 -> V_10 & V_88 )
V_378 = V_381 ;
if ( V_3 -> V_9 & V_83 ) {
if ( ( V_3 -> V_10 & V_295 ) ||
( V_3 -> V_10 & V_252 ) )
V_378 = V_380 ;
if ( V_3 -> V_10 & V_250 )
V_378 = V_382 ;
}
if ( V_3 -> V_9 & V_76 )
V_378 = V_379 ;
V_32 = 0 ;
while ( V_378 [ V_32 ] . V_383 != 0xFFFF ) {
if ( V_378 [ V_32 ] . V_383 == V_61 )
break;
V_32 ++ ;
}
return & V_378 [ V_32 ] ;
}
static void F_81 ( struct V_2 * V_3 )
{
unsigned short V_32 , V_97 ;
struct V_377 const * V_378 ;
if ( ! ( V_3 -> V_16 & V_70 ) )
return;
V_378 = F_80 ( 0 , V_3 ) ;
for ( V_32 = 0x80 , V_97 = 0 ; V_32 <= 0xBF ; V_32 ++ , V_97 ++ )
F_5 ( V_3 -> V_263 , V_32 , V_378 -> V_215 [ V_97 ] ) ;
if ( ( V_3 -> V_9 & V_47 ) &&
( ! ( V_3 -> V_9 & V_76 ) ) ) {
V_378 = F_80 ( 1 , V_3 ) ;
for ( V_32 = 0xC0 , V_97 = 0 ; V_32 < 0xFF ; V_32 ++ , V_97 ++ )
F_5 ( V_3 -> V_263 ,
V_32 ,
V_378 -> V_215 [ V_97 ] ) ;
}
if ( ( V_3 -> V_9 & V_47 ) &&
( ! ( V_3 -> V_9 & V_76 ) ) )
F_13 ( V_3 -> V_263 , 0x4E , ~ 0x14 , 0x04 ) ;
else
F_13 ( V_3 -> V_263 , 0x4E , ~ 0x14 , 0x10 ) ;
}
static void F_82 ( unsigned short V_29 , unsigned short V_30 ,
struct V_2 * V_3 )
{
unsigned short V_32 ;
unsigned char const * V_384 ;
unsigned short V_41 ;
V_41 = V_42 [ V_30 ] . V_43 ;
F_5 ( V_3 -> V_385 , 0x00 , 0x00 ) ;
if ( V_3 -> V_10 & V_88 ) {
F_5 ( V_3 -> V_385 , 0x13 , 0xFA ) ;
F_5 ( V_3 -> V_385 , 0x14 , 0xC8 ) ;
} else {
F_5 ( V_3 -> V_385 , 0x13 , 0xF5 ) ;
F_5 ( V_3 -> V_385 , 0x14 , 0xB7 ) ;
}
if ( ! ( V_3 -> V_9 & V_47 ) )
return;
if ( V_3 -> V_10 & V_291 ) {
F_5 ( V_3 -> V_385 , 0x13 , 0xFA ) ;
F_5 ( V_3 -> V_385 , 0x14 , 0xC8 ) ;
F_5 ( V_3 -> V_385 , 0x3D , 0xA8 ) ;
}
if ( ( V_3 -> V_9 & V_76 ) || ( V_3 -> V_9
& V_83 ) ) {
if ( V_3 -> V_10 & V_295 )
return;
V_384 = V_386 ;
if ( V_3 -> V_78 & V_79 ) {
V_384 = V_387 ;
if ( ! ( V_41 & V_45 ) )
V_384 = V_388 ;
}
if ( V_3 -> V_10 & V_252 )
V_384 = V_389 ;
if ( V_3 -> V_10 & V_250 )
V_384 = V_390 ;
for ( V_32 = 0 ; V_32 <= 0x3E ; V_32 ++ )
F_5 ( V_3 -> V_385 , V_32 , V_384 [ V_32 ] ) ;
if ( V_3 -> V_16 & V_70 ) {
if ( V_3 -> V_10 & V_252 )
F_5 ( V_3 -> V_385 , 0x28 , 0x3f ) ;
}
}
}
static void F_83 ( unsigned short V_29 , unsigned short V_30 ,
unsigned short V_60 ,
struct V_35 * V_36 ,
struct V_2 * V_3 )
{
unsigned short V_61 , V_123 , V_62 , V_41 , V_24 , V_131 ;
unsigned long V_361 , V_360 , V_391 ;
V_41 = V_42 [ V_30 ] . V_43 ;
V_24 = V_3 -> V_325 ;
F_5 ( V_3 -> V_240 , 0x13 , V_24 ) ;
V_62 = V_3 -> V_324 ;
V_24 = V_62 & 0x00FF ;
F_5 ( V_3 -> V_240 , 0x14 , V_24 ) ;
V_131 = ( ( V_62 & 0xFF00 ) >> 8 ) << 7 ;
V_123 = V_3 -> V_201 - 1 ;
V_24 = V_123 & 0x00FF ;
F_5 ( V_3 -> V_240 , 0x16 , V_24 ) ;
V_24 = ( ( V_123 & 0xFF00 ) >> 8 ) << 3 ;
V_131 |= V_24 ;
V_123 = V_3 -> V_202 - 1 ;
if ( ! ( V_3 -> V_9 & V_47 ) )
V_123 -= 5 ;
V_24 = V_123 & 0x00FF ;
F_5 ( V_3 -> V_240 , 0x17 , V_24 ) ;
V_24 = V_131 | ( ( V_123 & 0xFF00 ) >> 8 ) ;
F_5 ( V_3 -> V_240 , 0x15 , V_24 ) ;
F_21 ( V_3 -> V_240 , 0x0D , 0x08 ) ;
V_123 = V_3 -> V_9 ;
V_62 = V_3 -> V_179 ;
if ( V_41 & V_127 )
V_62 = V_62 >> 1 ;
if ( F_66 ( V_3 ) )
V_62 = V_62 >> 1 ;
if ( V_123 & V_76 ) {
V_24 = 0 ;
if ( V_62 <= 1024 )
V_24 = 0xA0 ;
if ( V_62 == 1280 )
V_24 = 0xC0 ;
} else if ( V_123 & V_47 ) {
V_24 = 0xA0 ;
if ( V_62 <= 800 )
V_24 = 0x80 ;
} else {
V_24 = 0x80 ;
if ( V_3 -> V_9 & V_48 ) {
V_24 = 0 ;
if ( V_62 > 800 )
V_24 = 0x60 ;
}
}
if ( V_3 -> V_10 & ( V_252 | V_250 ) ) {
V_24 = 0x00 ;
if ( V_3 -> V_179 == 1280 )
V_24 = 0x40 ;
if ( V_3 -> V_179 == 1024 )
V_24 = 0x20 ;
}
F_13 ( V_3 -> V_240 , 0x0E , ~ 0xEF , V_24 ) ;
V_361 = V_3 -> V_182 ;
V_123 = V_3 -> V_328 ;
V_24 = V_123 & 0x00FF ;
F_5 ( V_3 -> V_240 , 0x18 , V_24 ) ;
V_360 = V_3 -> V_181 ;
V_123 |= 0x04000 ;
if ( V_360 <= V_361 ) {
V_123 = ( V_123 & ( ~ 0x4000 ) ) ;
V_360 = V_3 -> V_181 ;
} else {
V_360 -= V_361 ;
}
V_391 = ( V_360 * 256 * 1024 ) % V_361 ;
V_360 = ( V_360 * 256 * 1024 ) / V_361 ;
V_361 = V_360 ;
if ( V_391 != 0 )
V_361 ++ ;
V_24 = ( unsigned short ) ( V_361 & 0x000000FF ) ;
F_5 ( V_3 -> V_240 , 0x1B , V_24 ) ;
V_24 = ( unsigned short ) ( ( V_361 & 0x0000FF00 ) >> 8 ) ;
F_5 ( V_3 -> V_240 , 0x1A , V_24 ) ;
V_62 = ( unsigned short ) ( V_361 >> 16 ) ;
V_24 = V_62 & 0x00FF ;
V_24 = V_24 << 4 ;
V_24 |= ( ( V_123 & 0xFF00 ) >> 8 ) ;
F_5 ( V_3 -> V_240 , 0x19 , V_24 ) ;
if ( V_3 -> V_16 & ( V_85 | V_86 | V_17
| V_18 | V_70 ) ) {
V_24 = 0x0028 ;
F_5 ( V_3 -> V_240 , 0x1C , V_24 ) ;
V_61 = V_3 -> V_179 ;
if ( V_41 & V_127 )
V_61 = V_61 >> 1 ;
if ( F_66 ( V_3 ) )
V_61 = V_61 >> 1 ;
if ( V_3 -> V_9 & V_48 ) {
if ( V_61 > 800 )
V_61 -= 800 ;
} else if ( V_3 -> V_179 > 800 ) {
if ( V_3 -> V_179 == 1024 )
V_61 = ( V_61 * 25 / 32 ) - 1 ;
else
V_61 = ( V_61 * 20 / 32 ) - 1 ;
}
V_61 -= 1 ;
V_24 = ( V_61 & 0xFF00 ) >> 8 ;
V_24 = ( ( V_24 & 0x0003 ) << 4 ) ;
F_5 ( V_3 -> V_240 , 0x1E , V_24 ) ;
V_24 = ( V_61 & 0x00FF ) ;
F_5 ( V_3 -> V_240 , 0x1D , V_24 ) ;
if ( V_3 -> V_9 & ( V_47 | V_76 ) ) {
if ( V_3 -> V_179 > 800 )
F_21 ( V_3 -> V_240 , 0x1E , 0x08 ) ;
}
V_24 = 0x0036 ;
if ( V_3 -> V_9 & V_47 ) {
if ( ! ( V_3 -> V_10 & ( V_255
| V_252 | V_250
| V_298 ) ) ) {
V_24 |= 0x0001 ;
if ( ( V_3 -> V_9 & V_49 )
&& ( ! ( V_3 -> V_10
& V_79 ) ) )
V_24 &= ( ~ 0x0001 ) ;
}
}
F_13 ( V_3 -> V_240 , 0x1F , 0x00C0 , V_24 ) ;
V_62 = V_3 -> V_203 ;
if ( F_66 ( V_3 ) )
V_62 = V_62 >> 1 ;
V_62 = ( V_62 >> 1 ) - 2 ;
V_24 = ( ( V_62 & 0x0700 ) >> 8 ) << 3 ;
F_13 ( V_3 -> V_240 , 0x21 , 0x00C0 , V_24 ) ;
V_24 = V_62 & 0x00FF ;
F_5 ( V_3 -> V_240 , 0x22 , V_24 ) ;
}
F_69 ( V_29 , V_30 , V_60 , V_3 ) ;
}
static void F_84 ( struct V_2 * V_3 )
{
F_13 ( V_3 -> V_28 , 0x1E , 0xFF , 0x20 ) ;
}
static void F_85 ( unsigned short V_29 , unsigned short V_30 ,
struct V_2 * V_3 )
{
if ( V_3 -> V_55 == V_56 ) {
if ( ! ( V_3 -> V_9 & ( V_49 | V_392
| V_279 ) ) ) {
F_84 ( V_3 ) ;
}
}
}
static void F_86 ( struct V_35 * V_36 ,
struct V_2 * V_3 )
{
F_13 ( V_3 -> V_38 , 0x63 , 0xBF , 0x00 ) ;
}
static unsigned char F_87 ( struct V_316 * V_317 ,
unsigned short V_29 , unsigned short V_30 ,
struct V_2 * V_3 )
{
unsigned short V_160 , V_178 , V_352 , V_41 , V_122 ;
V_122 = V_42 [ V_30 ] . V_65 ;
V_160 = V_124 [ V_122 ] . V_125 ;
V_178 = V_124 [ V_122 ] . V_126 ;
V_41 = V_42 [ V_30 ] . V_43 ;
if ( ! ( V_41 & V_45 ) ) {
V_160 /= 9 ;
V_160 *= 8 ;
}
if ( ( V_29 > 0x13 ) && ( V_41 & V_127 ) )
V_160 *= 2 ;
if ( ( V_29 > 0x13 ) && ( V_41 & V_101 ) )
V_178 *= 2 ;
if ( V_160 > V_317 -> V_319 . V_393 )
return 0 ;
if ( V_178 > V_317 -> V_319 . V_394 )
return 0 ;
if ( V_160 != V_317 -> V_319 . V_393 ||
V_178 != V_317 -> V_319 . V_394 ) {
V_352 = F_70 ( V_29 , V_30 , V_3 ) ;
if ( V_352 > 2 )
return 0 ;
}
return 1 ;
}
static void F_88 ( struct V_316 * V_317 ,
int V_114 ,
unsigned short V_29 ,
unsigned short V_30 ,
struct V_2 * V_3 )
{
unsigned char V_24 , V_395 ;
unsigned short V_160 , V_178 , V_41 , V_122 ;
unsigned short V_396 , V_397 , V_398 , V_399 , V_400 ;
unsigned short V_401 , V_402 , V_403 , V_404 , V_405 ;
unsigned short V_406 ;
V_24 = ( unsigned char ) ( ( V_317 -> V_319 . V_407 &
( V_408 << 8 ) ) >> 8 ) ;
V_24 &= V_408 ;
V_395 = F_8 ( V_3 -> V_409 ) ;
F_9 ( ( V_395 & 0x3F ) | V_24 , V_3 -> V_93 ) ;
V_24 = V_317 -> V_319 . V_407 & V_408 ;
F_13 ( V_3 -> V_28 , 0x35 , ~ 0x80 , V_24 & 0x80 ) ;
F_13 ( V_3 -> V_28 , 0x30 , ~ 0x20 , ( V_24 & 0x40 ) >> 1 ) ;
if ( V_114 == V_23 )
F_23 ( V_3 ) ;
else
F_30 ( V_3 ) ;
V_122 = V_42 [ V_30 ] . V_65 ;
V_160 = V_124 [ V_122 ] . V_125 ;
V_178 = V_124 [ V_122 ] . V_126 ;
V_41 = V_42 [ V_30 ] . V_43 ;
if ( ! ( V_41 & V_45 ) )
V_160 = V_160 * 8 / 9 ;
V_396 = V_317 -> V_319 . V_396 ;
V_397 = V_160 + ( V_317 -> V_319 . V_393 - V_160 ) / 2 ;
if ( V_397 > V_396 )
V_397 -= V_396 ;
V_398 = V_397 + V_317 -> V_319 . V_410 ;
if ( V_398 > V_396 )
V_398 -= V_396 ;
V_399 = V_398 + V_317 -> V_319 . V_411 ;
if ( V_399 > V_396 )
V_399 -= V_396 ;
V_400 = V_397 + V_396 - V_317 -> V_319 . V_393 ;
V_401 = V_317 -> V_319 . V_401 ;
V_402 = V_178 + ( V_317 -> V_319 . V_394 - V_178 ) / 2 ;
if ( V_41 & V_101 )
V_402 += V_178 / 2 ;
if ( V_402 > V_401 )
V_402 -= V_401 ;
V_403 = V_402 + V_317 -> V_319 . V_412 ;
if ( V_403 > V_401 )
V_403 -= V_401 ;
V_404 = V_403 + V_317 -> V_319 . V_413 ;
if ( V_404 > V_401 )
V_404 -= V_401 ;
V_405 = V_402 + V_401 - V_317 -> V_319 . V_394 ;
V_24 = F_3 ( V_3 -> V_38 , 0x11 ) ;
F_5 ( V_3 -> V_38 , 0x11 , V_24 & 0x7f ) ;
if ( ! ( V_41 & V_45 ) )
F_21 ( V_3 -> V_28 , 0x1 , 0x1 ) ;
V_406 = ( V_396 >> 3 ) - 5 ;
F_13 ( V_3 -> V_28 , 0x0B , ~ 0x03 , ( V_406 & 0x300 ) >> 8 ) ;
F_5 ( V_3 -> V_38 , 0x0 , ( V_406 & 0xFF ) ) ;
V_406 = ( V_397 >> 3 ) - 1 ;
F_13 ( V_3 -> V_28 , 0x0B , ~ 0x30 , ( V_406 & 0x300 ) >> 4 ) ;
F_5 ( V_3 -> V_38 , 0x2 , ( V_406 & 0xFF ) ) ;
V_406 = ( V_400 >> 3 ) - 1 ;
F_13 ( V_3 -> V_28 , 0x0C , ~ 0x03 , ( V_406 & 0xC0 ) >> 6 ) ;
F_13 ( V_3 -> V_38 , 0x05 , ~ 0x80 , ( V_406 & 0x20 ) << 2 ) ;
F_13 ( V_3 -> V_38 , 0x03 , ~ 0x1F , V_406 & 0x1F ) ;
V_406 = ( V_398 >> 3 ) + 2 ;
F_13 ( V_3 -> V_28 , 0x0B , ~ 0xC0 , ( V_406 & 0x300 ) >> 2 ) ;
F_5 ( V_3 -> V_38 , 0x4 , ( V_406 & 0xFF ) ) ;
V_406 -- ;
F_13 ( V_3 -> V_28 , 0x2F , ~ 0x03 , ( V_406 & 0x300 ) >> 8 ) ;
F_5 ( V_3 -> V_28 , 0x2E , ( V_406 & 0xFF ) ) ;
V_406 = ( V_399 >> 3 ) + 2 ;
F_13 ( V_3 -> V_28 , 0x0C , ~ 0x04 , ( V_406 & 0x20 ) >> 3 ) ;
F_13 ( V_3 -> V_38 , 0x05 , ~ 0x1F , V_406 & 0x1F ) ;
V_406 -- ;
F_13 ( V_3 -> V_28 , 0x2F , ~ 0xFC , V_406 << 2 ) ;
V_406 = V_401 - 2 ;
F_13 ( V_3 -> V_28 , 0x0A , ~ 0x01 , ( V_406 & 0x400 ) >> 10 ) ;
F_13 ( V_3 -> V_38 , 0x07 , ~ 0x20 , ( V_406 & 0x200 ) >> 4 ) ;
F_13 ( V_3 -> V_38 , 0x07 , ~ 0x01 , ( V_406 & 0x100 ) >> 8 ) ;
F_5 ( V_3 -> V_38 , 0x06 , ( V_406 & 0xFF ) ) ;
V_406 = V_402 - 1 ;
F_13 ( V_3 -> V_28 , 0x0A , ~ 0x04 , ( V_406 & 0x400 ) >> 8 ) ;
F_13 ( V_3 -> V_38 , 0x09 , ~ 0x20 , ( V_406 & 0x200 ) >> 4 ) ;
F_13 ( V_3 -> V_38 , 0x07 , ~ 0x08 , ( V_406 & 0x100 ) >> 5 ) ;
F_5 ( V_3 -> V_38 , 0x15 , ( V_406 & 0xFF ) ) ;
V_406 = V_405 - 1 ;
F_13 ( V_3 -> V_28 , 0x0A , ~ 0x10 , ( V_406 & 0x100 ) >> 4 ) ;
F_5 ( V_3 -> V_38 , 0x16 , ( V_406 & 0xFF ) ) ;
V_406 = V_403 - 1 ;
F_13 ( V_3 -> V_28 , 0x0A , ~ 0x08 , ( V_406 & 0x400 ) >> 7 ) ;
F_13 ( V_3 -> V_38 , 0x07 , ~ 0x80 , ( V_406 & 0x200 ) >> 2 ) ;
F_13 ( V_3 -> V_38 , 0x07 , ~ 0x04 , ( V_406 & 0x100 ) >> 6 ) ;
F_5 ( V_3 -> V_38 , 0x10 , ( V_406 & 0xFF ) ) ;
if ( V_114 == V_23 ) {
F_13 ( V_3 -> V_28 , 0x35 , ~ 0x07 ,
( V_406 & 0x700 ) >> 8 ) ;
F_5 ( V_3 -> V_28 , 0x34 , V_406 & 0xFF ) ;
} else {
F_13 ( V_3 -> V_28 , 0x3F , ~ 0x03 ,
( V_406 & 0x600 ) >> 9 ) ;
F_5 ( V_3 -> V_28 , 0x34 , ( V_406 >> 1 ) & 0xFF ) ;
F_13 ( V_3 -> V_38 , 0x33 , ~ 0x01 , V_406 & 0x01 ) ;
}
V_406 = V_404 - 1 ;
F_13 ( V_3 -> V_28 , 0x0A , ~ 0x20 , ( V_406 & 0x10 ) << 1 ) ;
F_13 ( V_3 -> V_38 , 0x11 , ~ 0x0F , V_406 & 0x0F ) ;
if ( V_114 == V_23 )
F_13 ( V_3 -> V_28 , 0x3F , ~ 0xFC ,
( V_406 << 2 ) & 0xFC ) ;
else
F_13 ( V_3 -> V_28 , 0x3F , ~ 0xFC ,
( V_406 << 2 ) & 0x7C ) ;
for ( V_24 = 0 , V_406 = 0 ; V_24 < 3 ; V_24 ++ ) {
F_13 ( V_3 -> V_28 , 0x31 , ~ 0x30 , V_406 ) ;
F_5 ( V_3 -> V_28 ,
0x2B , V_317 -> V_319 . V_414 ) ;
F_5 ( V_3 -> V_28 ,
0x2C , V_317 -> V_319 . V_415 ) ;
V_406 += 0x10 ;
}
if ( ! ( V_41 & V_45 ) ) {
F_8 ( V_3 -> V_50 ) ;
F_9 ( 0x13 , V_3 -> V_51 ) ;
F_9 ( 0x00 , V_3 -> V_51 ) ;
F_8 ( V_3 -> V_50 ) ;
F_9 ( 0x20 , V_3 -> V_51 ) ;
F_8 ( V_3 -> V_50 ) ;
}
}
static unsigned char F_89 ( struct V_2 * V_3 )
{
unsigned short V_61 ;
V_61 = V_3 -> V_9 ;
if ( V_61 & V_278 )
return 0 ;
else if ( V_61 & ( V_279 | V_286 | V_285 ) )
return 1 ;
return 0 ;
}
static void F_90 ( struct V_316 * V_317 ,
struct V_35 * V_36 ,
struct V_2 * V_3 )
{
unsigned short V_217 = 0 ;
if ( V_3 -> V_16 & ( V_85 | V_86 | V_17
| V_18 | V_70 ) ) {
V_217 = 0x3F ;
if ( ! ( V_3 -> V_9 &
( V_279 | V_285 ) ) ) {
if ( V_3 -> V_9 & V_46 ) {
if ( V_3 -> V_9 & V_278 )
V_217 = 0x7F ;
}
}
F_14 ( V_3 -> V_240 , 0x1F , V_217 ) ;
if ( V_3 -> V_16 & ( V_18 | V_70 ) ) {
if ( ( ( V_3 -> V_9 &
( V_48 | V_46 ) ) ) ||
( F_89 ( V_3 ) ) )
F_21 ( V_3 -> V_240 , 0x30 , 0x80 ) ;
}
if ( V_3 -> V_9 & ( V_279 | V_46 |
V_285 ) )
F_61 ( V_317 , V_36 , V_3 ) ;
if ( V_3 -> V_9 & V_46 )
F_14 ( V_3 -> V_233 , 0x1e , 0xdf ) ;
F_14 ( V_3 -> V_28 , 0x32 , 0xdf ) ;
if ( ( V_3 -> V_9 & ( V_285 | V_278 ) ) )
F_14 ( V_3 -> V_263 , 0x00 , 0xdf ) ;
if ( ( V_3 -> V_9 &
( V_279 | V_285 ) ) ||
( ( ! ( V_3 -> V_9 & V_46 ) ) &&
( V_3 -> V_9 &
( V_68 | V_48 | V_47 ) ) ) )
F_21 ( V_3 -> V_233 , 0x00 , 0x80 ) ;
if ( ( V_3 -> V_9 &
( V_279 | V_285 ) ) ||
( ! ( V_3 -> V_9 & V_46 ) ) ||
( V_3 -> V_9 &
( V_68 | V_48 | V_47 ) ) ) {
V_217 = F_3 ( V_3 -> V_233 , 0x00 ) ;
F_21 ( V_3 -> V_233 , 0x00 , 0x10 ) ;
F_14 ( V_3 -> V_233 , 0x1E , 0xDF ) ;
F_5 ( V_3 -> V_233 , 0x00 , V_217 ) ;
}
} else {
if ( V_3 -> V_9 & ( V_48 | V_47 ) ) {
F_21 ( V_3 -> V_233 , 0x00 , 0x80 ) ;
F_14 ( V_3 -> V_233 , 0x1E , 0xDF ) ;
F_14 ( V_3 -> V_28 , 0x32 , 0xDF ) ;
}
if ( V_3 -> V_9 & ( V_279 | V_46
| V_285 ) )
F_61 ( V_317 , V_36 , V_3 ) ;
}
}
static unsigned short F_91 ( struct V_2 * V_3 )
{
unsigned short V_62 = 0 ;
if ( V_3 -> V_10 & V_88 )
V_62 = 2 ;
if ( V_3 -> V_10 & V_298 )
V_62 = 4 ;
if ( V_3 -> V_10 & V_295 )
V_62 = 6 ;
if ( V_3 -> V_10 & V_252 )
V_62 = 8 ;
if ( V_3 -> V_10 & V_250 )
V_62 = 10 ;
if ( V_3 -> V_10 & V_79 )
V_62 ++ ;
return V_62 ;
}
static void F_92 ( unsigned short * V_62 , unsigned char * V_258 ,
unsigned char * V_259 , struct V_2 * V_3 )
{
* V_62 = 0 ;
* V_258 = 0 ;
* V_259 = 0 ;
if ( V_3 -> V_10 & V_88 )
* V_62 = 1 ;
if ( V_3 -> V_10 & V_291 )
* V_62 = 2 ;
if ( V_3 -> V_10 & V_292 )
* V_62 = 3 ;
if ( V_3 -> V_10 & V_255 ) {
* V_62 = 4 ;
if ( V_3 -> V_10 & V_291 )
* V_62 = 5 ;
}
if ( V_3 -> V_16 & ( V_85 | V_86 | V_17
| V_18 | V_70 ) ) {
if ( ( ! ( V_3 -> V_9 & V_49 ) ) || ( V_3 -> V_10
& V_79 ) ) {
* V_62 += 8 ;
* V_258 += 1 ;
}
}
if ( V_3 -> V_16 & ( V_85 | V_86 | V_17
| V_18 | V_70 ) )
( * V_259 ) ++ ;
}
static void F_93 ( struct V_2 * V_3 )
{
unsigned char V_217 , V_218 , V_313 ;
if ( V_3 -> V_16 & ( V_85 | V_86 | V_17
| V_18 | V_70 ) ) {
if ( V_3 -> V_9 & ( V_48 | V_46
| V_47 | V_68 ) ) {
V_313 = 0 ;
V_218 = V_416 ;
if ( V_3 -> V_9 & V_278 )
V_218 = V_218 >> 4 ;
if ( V_3 -> V_9 &
( V_48 | V_46 ) ) {
V_313 = V_417 ;
if ( ! ( V_3 -> V_9 & V_46 ) )
V_218 = V_313 ;
}
V_218 &= 0x0F ;
V_313 &= 0xF0 ;
V_217 = F_3 ( V_3 -> V_233 , 0x2D ) ;
if ( V_3 -> V_9 & ( V_68 | V_48
| V_47 ) ) {
V_217 &= 0xF0 ;
V_217 |= V_218 ;
}
if ( V_3 -> V_9 & V_46 ) {
V_217 &= 0x0F ;
V_217 |= V_313 ;
}
F_5 ( V_3 -> V_233 , 0x2D , V_217 ) ;
}
}
}
static void F_94 ( unsigned short V_123 ,
struct V_2 * V_3 )
{
unsigned short V_24 ;
V_24 = F_3 ( V_3 -> V_38 , 0x37 ) ;
if ( V_24 & V_357 ) {
F_13 ( V_3 -> V_233 , 0x19 , 0x0F ,
( unsigned short ) ( 0x20 | ( V_123 & 0x00C0 ) ) ) ;
F_13 ( V_3 -> V_233 , 0x1A , 0x7F , 0x80 ) ;
} else {
F_13 ( V_3 -> V_233 , 0x19 , 0x0F ,
( unsigned short ) ( 0x30 | ( V_123 & 0x00C0 ) ) ) ;
F_13 ( V_3 -> V_233 , 0x1A , 0x7F , 0x00 ) ;
}
}
static void F_95 ( unsigned short V_123 ,
struct V_2 * V_3 )
{
if ( V_123 & V_418 )
F_13 ( V_3 -> V_263 , 0x1A , 0xE0 ,
( unsigned short ) ( ( ( V_123 & 0x00ff ) >> 6 )
| 0x0c ) ) ;
else
F_13 ( V_3 -> V_263 , 0x1A , 0xE0 ,
( unsigned short ) ( ( ( V_123 & 0x00ff ) >> 6 )
| 0x18 ) ) ;
}
static void F_96 ( struct V_2 * V_3 )
{
unsigned short V_32 ;
V_32 = F_3 ( V_3 -> V_28 , 0x1F ) ;
if ( ! ( V_32 & 0xC0 ) ) {
for ( V_32 = 0 ; V_32 < 0xFFFF ; V_32 ++ ) {
if ( ! ( F_8 ( V_3 -> V_50 ) & 0x08 ) )
break;
}
for ( V_32 = 0 ; V_32 < 0xFFFF ; V_32 ++ ) {
if ( ( F_8 ( V_3 -> V_50 ) & 0x08 ) )
break;
}
}
}
static void F_97 ( struct V_2 * V_3 )
{
unsigned short V_102 ;
V_102 = F_41 ( V_3 ) ;
F_14 ( V_3 -> V_240 , 0x30 , 0x8F ) ;
F_96 ( V_3 ) ;
F_21 ( V_3 -> V_240 , 0x30 , 0x20 ) ;
F_96 ( V_3 ) ;
F_5 ( V_3 -> V_240 , 0x31 ,
V_3 -> V_19 [ V_102 ] . V_419 ) ;
F_5 ( V_3 -> V_240 , 0x32 ,
V_3 -> V_19 [ V_102 ] . V_420 ) ;
F_5 ( V_3 -> V_240 , 0x33 ,
V_3 -> V_19 [ V_102 ] . V_421 ) ;
F_5 ( V_3 -> V_240 , 0x34 ,
V_3 -> V_19 [ V_102 ] . V_422 ) ;
F_96 ( V_3 ) ;
F_21 ( V_3 -> V_240 , 0x30 , 0x40 ) ;
}
static void F_98 ( struct V_2 * V_3 )
{
unsigned short V_123 ;
V_123 = V_3 -> V_19 [ F_41 ( V_3 ) ] . V_304 ;
if ( V_3 -> V_16 & ( V_85 | V_86 | V_17 |
V_18 | V_70 ) ) {
if ( V_3 -> V_16 &
( V_17 | V_18 | V_70 ) ) {
F_5 ( V_3 -> V_240 , 0x24 ,
( unsigned char ) ( V_123 & 0x1F ) ) ;
}
F_13 ( V_3 -> V_240 , 0x0D ,
~ ( ( V_423 | V_424 ) >> 8 ) ,
( unsigned short ) ( ( V_123 & ( V_423
| V_424 ) ) >> 8 ) ) ;
if ( V_3 -> V_9 & V_48 )
F_95 ( V_123 , V_3 ) ;
else if ( V_3 -> V_9 & V_46 )
F_94 ( V_123 , V_3 ) ;
if ( V_3 -> V_16 & ( V_18 | V_70 ) ) {
if ( V_123 & V_425 )
F_97 ( V_3 ) ;
}
} else {
F_94 ( V_123 , V_3 ) ;
}
}
static void F_99 ( unsigned short V_29 ,
unsigned short V_30 ,
struct V_2 * V_3 )
{
unsigned short V_62 ;
unsigned char V_217 ;
if ( V_3 -> V_10 & ( V_252 | V_250 ) )
return;
V_62 = F_91 ( V_3 ) ;
V_62 &= 0xFE ;
V_217 = V_426 [ V_62 ] ;
V_217 = V_217 << 4 ;
F_13 ( V_3 -> V_263 , 0x0A , 0x8F , V_217 ) ;
}
static void F_100 ( unsigned short V_29 ,
unsigned short V_30 ,
struct V_2 * V_3 )
{
unsigned short V_62 ;
unsigned char V_217 ;
V_62 = F_91 ( V_3 ) ;
V_62 &= 0xFE ;
V_217 = V_427 [ V_62 ] ;
V_217 = V_217 << 5 ;
F_13 ( V_3 -> V_263 , 0x3A , 0x1F , V_217 ) ;
}
static void F_101 ( struct V_2 * V_3 )
{
unsigned short V_62 ;
unsigned char V_258 , V_259 ;
unsigned long V_428 ;
F_92 ( & V_62 , & V_258 , & V_259 , V_3 ) ;
V_428 = V_429 [ V_62 ] ;
F_5 ( V_3 -> V_263 , 0x31 , ( unsigned short ) ( V_428
& 0x000000FF ) ) ;
F_5 ( V_3 -> V_263 , 0x32 , ( unsigned short ) ( ( V_428
& 0x0000FF00 ) >> 8 ) ) ;
F_5 ( V_3 -> V_263 , 0x33 , ( unsigned short ) ( ( V_428
& 0x00FF0000 ) >> 16 ) ) ;
F_5 ( V_3 -> V_263 , 0x34 , ( unsigned short ) ( ( V_428
& 0xFF000000 ) >> 24 ) ) ;
}
static void F_102 ( unsigned short V_29 , unsigned short V_30 ,
struct V_2 * V_3 )
{
unsigned short V_62 , V_102 ;
unsigned char const * V_430 ;
unsigned char V_258 , V_259 , V_193 ;
F_92 ( & V_62 , & V_258 , & V_259 , V_3 ) ;
switch ( V_62 ) {
case 0x00 :
case 0x04 :
V_430 = V_431 ;
break;
case 0x01 :
V_430 = V_432 ;
break;
case 0x02 :
case 0x05 :
case 0x0D :
case 0x03 :
V_430 = V_433 ;
break;
case 0x08 :
case 0x0C :
case 0x0A :
case 0x0B :
case 0x09 :
V_430 = V_434 ;
break;
default:
return;
}
V_193 = V_42 [ V_30 ] . V_435 ;
if ( V_258 == 0 )
V_102 = V_193 * 4 ;
else
V_102 = V_193 * 7 ;
if ( ( V_258 == 0 ) && ( V_259 == 1 ) ) {
F_5 ( V_3 -> V_263 , 0x35 , 0 ) ;
F_5 ( V_3 -> V_263 , 0x36 , 0 ) ;
F_5 ( V_3 -> V_263 , 0x37 , 0 ) ;
F_5 ( V_3 -> V_263 , 0x38 , V_430 [ V_102 ++ ] ) ;
} else {
F_5 ( V_3 -> V_263 , 0x35 , V_430 [ V_102 ++ ] ) ;
F_5 ( V_3 -> V_263 , 0x36 , V_430 [ V_102 ++ ] ) ;
F_5 ( V_3 -> V_263 , 0x37 , V_430 [ V_102 ++ ] ) ;
F_5 ( V_3 -> V_263 , 0x38 , V_430 [ V_102 ++ ] ) ;
}
if ( V_3 -> V_16 & ( V_85 | V_86 | V_17
| V_18 | V_70 ) ) {
F_5 ( V_3 -> V_263 , 0x48 , V_430 [ V_102 ++ ] ) ;
F_5 ( V_3 -> V_263 , 0x49 , V_430 [ V_102 ++ ] ) ;
F_5 ( V_3 -> V_263 , 0x4A , V_430 [ V_102 ++ ] ) ;
}
}
static void F_103 ( unsigned short V_29 ,
unsigned short V_30 ,
struct V_2 * V_3 )
{
F_93 ( V_3 ) ;
if ( V_3 -> V_9 & ( V_48 | V_46 ) )
F_98 ( V_3 ) ;
if ( V_3 -> V_9 & V_47 ) {
F_101 ( V_3 ) ;
F_102 ( V_29 , V_30 , V_3 ) ;
F_99 ( V_29 , V_30 , V_3 ) ;
if ( V_3 -> V_16 & V_257 )
F_100 ( V_29 , V_30 , V_3 ) ;
}
}
static void F_104 ( unsigned short V_29 ,
struct V_35 * V_36 ,
struct V_2 * V_3 )
{
unsigned short V_218 ;
short V_258 ;
unsigned char V_217 ;
V_217 = 0 ;
if ( ! ( V_3 -> V_9 & V_279 ) ) {
V_217 = F_3 ( V_3 -> V_233 , 0x00 ) ;
V_217 &= ~ 0x10 ;
V_217 |= 0x40 ;
if ( V_3 -> V_9 & ( V_68 | V_47
| V_48 ) ) {
V_217 = 0x40 ;
V_258 = V_3 -> V_55 ;
V_258 -= V_56 ;
if ( V_258 >= 0 ) {
V_217 = ( 0x008 >> V_258 ) ;
if ( V_217 == 0 )
V_217 = 1 ;
V_217 |= 0x040 ;
}
if ( V_3 -> V_9 & V_49 )
V_217 ^= 0x50 ;
}
}
F_5 ( V_3 -> V_233 , 0x00 , V_217 ) ;
V_217 = 0x08 ;
V_218 = 0xf0 ;
if ( V_3 -> V_9 & V_279 )
goto V_436;
V_217 = 0x00 ;
V_218 = 0xff ;
if ( ! ( V_3 -> V_9 & ( V_68 | V_47 |
V_48 | V_46 ) ) )
goto V_436;
if ( ( V_3 -> V_9 & V_46 ) &&
( ! ( V_3 -> V_9 & V_285 ) ) ) {
V_218 &= 0xf7 ;
V_217 |= 0x01 ;
goto V_436;
}
if ( V_3 -> V_9 & V_46 ) {
V_218 &= 0xf7 ;
V_217 |= 0x01 ;
}
if ( ! ( V_3 -> V_9 & ( V_68 | V_47 | V_48 ) ) )
goto V_436;
V_218 &= 0xf8 ;
V_217 = 0x01 ;
if ( ! ( V_3 -> V_9 & V_49 ) )
V_217 |= 0x02 ;
if ( ! ( V_3 -> V_9 & V_68 ) ) {
V_217 = V_217 ^ 0x05 ;
if ( ! ( V_3 -> V_9 & V_48 ) )
V_217 = V_217 ^ 0x01 ;
}
if ( ! ( V_3 -> V_9 & V_278 ) )
V_217 |= 0x08 ;
V_436:
F_13 ( V_3 -> V_233 , 0x2e , V_218 , V_217 ) ;
if ( V_3 -> V_9 & ( V_68 | V_47 | V_48
| V_46 ) ) {
V_217 &= ( ~ 0x08 ) ;
if ( ( V_3 -> V_55 == V_56 ) && ( ! ( V_3 -> V_9
& V_49 ) ) ) {
V_217 |= 0x010 ;
}
V_217 |= 0x080 ;
if ( V_3 -> V_9 & V_47 ) {
V_217 |= 0x020 ;
if ( V_3 -> V_9 & V_287 )
V_217 = V_217 ^ 0x20 ;
}
F_13 ( V_3 -> V_240 , 0x0D , ~ 0x0BF , V_217 ) ;
V_217 = 0 ;
if ( V_3 -> V_8 & V_306 )
V_217 |= 0x40 ;
if ( V_3 -> V_9 & V_47 ) {
if ( V_3 -> V_10 & V_140 )
V_217 |= 0x40 ;
}
if ( ( V_3 -> V_6 == V_73 )
|| ( V_3 -> V_6 == V_209 ) )
V_217 |= 0x80 ;
if ( V_3 -> V_6 == V_74 )
V_217 |= 0x80 ;
F_5 ( V_3 -> V_240 , 0x0C , V_217 ) ;
}
if ( V_3 -> V_16 & ( V_85 | V_86 | V_17
| V_18 | V_70 ) ) {
V_217 = 0 ;
V_218 = 0xfb ;
if ( V_3 -> V_9 & V_278 ) {
V_218 = 0xff ;
if ( V_3 -> V_9 & V_46 )
V_217 |= 0x04 ;
}
F_13 ( V_3 -> V_233 , 0x13 , V_218 , V_217 ) ;
V_217 = 0x00 ;
V_218 = 0xcf ;
if ( ! ( V_3 -> V_9 & V_279 ) ) {
if ( V_3 -> V_9 & V_278 )
V_217 |= 0x30 ;
}
F_13 ( V_3 -> V_233 , 0x2c , V_218 , V_217 ) ;
V_217 = 0 ;
V_218 = 0x3f ;
if ( ! ( V_3 -> V_9 & V_279 ) ) {
if ( V_3 -> V_9 & V_278 )
V_217 |= 0xc0 ;
}
F_13 ( V_3 -> V_240 , 0x21 , V_218 , V_217 ) ;
}
V_217 = 0 ;
V_218 = 0x7f ;
if ( ! ( V_3 -> V_9 & V_46 ) ) {
V_218 = 0xff ;
if ( ! ( V_3 -> V_9 & V_278 ) )
V_217 |= 0x80 ;
}
F_13 ( V_3 -> V_240 , 0x23 , V_218 , V_217 ) ;
if ( V_3 -> V_16 & ( V_18 | V_70 ) ) {
if ( V_3 -> V_8 & V_306 ) {
F_21 ( V_3 -> V_240 , 0x27 , 0x20 ) ;
F_21 ( V_3 -> V_240 , 0x34 , 0x10 ) ;
}
}
}
void F_105 ( struct V_35 * V_36 ,
struct V_2 * V_3 )
{
F_13 ( V_3 -> V_233 , 0x2f , 0xFF , 0x01 ) ;
}
void F_106 ( struct V_35 * V_36 ,
struct V_2 * V_3 )
{
F_13 ( V_3 -> V_233 , 0x2F , 0xFE , 0x00 ) ;
}
unsigned short F_107 ( struct V_35 * V_318 ,
unsigned short V_29 , unsigned short V_30 ,
struct V_2 * V_3 )
{
const T_1 V_437 [] = {
0x00 , 0x00 , 0x03 , 0x01 , 0x01 , 0x01 , 0x01 , 0x00 } ;
unsigned short V_60 , V_32 , V_102 , V_24 ;
V_102 = F_3 ( V_3 -> V_38 , 0x33 ) ;
V_102 = V_102 >> V_3 -> V_438 ;
V_102 &= 0x0F ;
if ( V_3 -> V_8 & V_75 )
V_102 = 0 ;
if ( V_102 > 0 )
V_102 -- ;
if ( V_3 -> V_78 & V_247 ) {
if ( V_3 -> V_9 & ( V_48 | V_46 ) ) {
V_24 = V_437 [ V_3 -> V_6 & 0x07 ] ;
if ( V_102 > V_24 )
V_102 = V_24 ;
}
}
V_60 = V_42 [ V_30 ] . V_439 ;
V_29 = V_66 [ V_60 ] . V_67 ;
if ( V_318 -> V_99 >= V_15 ) {
if ( ( V_66 [ V_60 ] . V_440 == 800 ) &&
( V_66 [ V_60 ] . V_441 == 600 ) ) {
V_102 ++ ;
}
if ( ( V_66 [ V_60 ] . V_440 == 1024 ) &&
( V_66 [ V_60 ] . V_441 == 768 ) ) {
V_102 ++ ;
}
if ( ( V_66 [ V_60 ] . V_440 == 1280 ) &&
( V_66 [ V_60 ] . V_441 == 1024 ) ) {
V_102 ++ ;
}
}
V_32 = 0 ;
do {
if ( V_66 [ V_60 + V_32 ] .
V_67 != V_29 )
break;
V_24 = V_66 [ V_60 + V_32 ] . V_91 ;
V_24 &= V_277 ;
if ( V_24 < V_3 -> V_55 )
break;
V_32 ++ ;
V_102 -- ;
} while ( V_102 != 0xFFFF );
if ( ! ( V_3 -> V_9 & V_68 ) ) {
if ( V_3 -> V_9 & V_49 ) {
V_24 = V_66 [ V_60 + V_32 - 1 ] .
V_91 ;
if ( V_24 & V_128 )
V_32 ++ ;
}
}
V_32 -- ;
if ( ( V_3 -> V_78 & V_247 ) ) {
V_24 = F_15 ( V_29 , V_30 ,
V_60 , & V_32 , V_3 ) ;
}
return V_60 + V_32 ;
}
static void F_108 ( unsigned short V_29 , unsigned short V_30 ,
struct V_35 * V_36 ,
struct V_2 * V_3 )
{
unsigned short V_60 ;
V_3 -> V_78 |= V_247 ;
V_60 = F_107 ( V_36 , V_29 ,
V_30 , V_3 ) ;
F_36 ( V_29 , V_30 , V_3 ) ;
F_39 ( V_29 , V_30 , V_60 , V_3 ) ;
F_40 ( V_29 , V_30 , V_60 ,
V_36 , V_3 ) ;
F_44 ( V_29 , V_30 , V_60 , V_3 ) ;
F_48 ( V_29 , V_30 , V_60 , V_3 ) ;
}
static unsigned char F_109 ( unsigned short V_29 ,
struct V_35 * V_36 ,
struct V_2 * V_3 )
{
unsigned short V_30 , V_60 ;
V_3 -> V_78 |= V_247 ;
F_53 ( V_29 , & V_30 , V_3 ) ;
V_3 -> V_438 = 4 ;
V_60 = F_107 ( V_36 , V_29 ,
V_30 , V_3 ) ;
F_64 ( V_29 , V_3 ) ;
F_65 ( V_29 , V_30 , V_3 ) ;
F_68 ( V_29 , V_30 , V_60 , V_3 ) ;
F_74 ( V_29 , V_30 , V_36 ,
V_60 , V_3 ) ;
F_75 ( V_29 , V_30 , V_36 ,
V_60 , V_3 ) ;
F_77 ( V_29 , V_30 , V_36 ,
V_60 , V_3 ) ;
F_78 ( V_29 , V_30 , V_60 ,
V_36 , V_3 ) ;
F_79 ( V_29 , V_30 , V_36 ,
V_60 , V_3 ) ;
F_81 ( V_3 ) ;
F_82 ( V_29 , V_30 , V_3 ) ;
F_83 ( V_29 , V_30 , V_60 ,
V_36 , V_3 ) ;
F_69 ( V_29 , V_30 , V_60 , V_3 ) ;
F_85 ( V_29 , V_30 , V_3 ) ;
F_63 ( V_3 ) ;
return 1 ;
}
void F_110 ( struct V_2 * V_3 )
{
unsigned char V_442 [ 17 ] = { 0x5F , 0x4F , 0x50 , 0x82 , 0x55 , 0x81 ,
0x0B , 0x3E , 0xE9 , 0x0B , 0xDF , 0xE7 , 0x04 , 0x00 , 0x00 ,
0x05 , 0x00 } ;
unsigned char V_443 = 0 , V_444 = 0 , V_445 = 0 , V_446 = 0 ;
unsigned char V_447 , V_448 , V_449 ;
unsigned short V_24 ;
int V_32 ;
F_5 ( V_3 -> V_28 , 0x05 , 0x86 ) ;
F_5 ( V_3 -> V_38 , 0x57 , 0x4A ) ;
F_5 ( V_3 -> V_38 , 0x53 , ( F_3 (
V_3 -> V_38 , 0x53 ) | 0x02 ) ) ;
V_449 = F_3 ( V_3 -> V_28 , 0x31 ) ;
V_448 = F_3 ( V_3 -> V_38 , 0x63 ) ;
V_443 = F_3 ( V_3 -> V_28 , 0x01 ) ;
F_5 ( V_3 -> V_28 , 0x01 , ( unsigned char ) ( V_443 & 0xDF ) ) ;
F_5 ( V_3 -> V_38 , 0x63 , ( unsigned char ) ( V_448 & 0xBF ) ) ;
V_447 = F_3 ( V_3 -> V_38 , 0x17 ) ;
F_5 ( V_3 -> V_38 , 0x17 , ( unsigned char ) ( V_447 | 0x80 ) ) ;
V_444 = F_3 ( V_3 -> V_28 , 0x1F ) ;
F_5 ( V_3 -> V_28 , 0x1F , ( unsigned char ) ( V_444 | 0x04 ) ) ;
V_445 = F_3 ( V_3 -> V_28 , 0x07 ) ;
F_5 ( V_3 -> V_28 , 0x07 , ( unsigned char ) ( V_445 & 0xFB ) ) ;
V_446 = F_3 ( V_3 -> V_28 , 0x06 ) ;
F_5 ( V_3 -> V_28 , 0x06 , ( unsigned char ) ( V_446 & 0xC3 ) ) ;
F_5 ( V_3 -> V_38 , 0x11 , 0x00 ) ;
for ( V_32 = 0 ; V_32 < 8 ; V_32 ++ )
F_5 ( V_3 -> V_38 , ( unsigned short ) V_32 , V_442 [ V_32 ] ) ;
for ( V_32 = 8 ; V_32 < 11 ; V_32 ++ )
F_5 ( V_3 -> V_38 , ( unsigned short ) ( V_32 + 8 ) ,
V_442 [ V_32 ] ) ;
for ( V_32 = 11 ; V_32 < 13 ; V_32 ++ )
F_5 ( V_3 -> V_38 , ( unsigned short ) ( V_32 + 4 ) ,
V_442 [ V_32 ] ) ;
for ( V_32 = 13 ; V_32 < 16 ; V_32 ++ )
F_5 ( V_3 -> V_28 , ( unsigned short ) ( V_32 - 3 ) ,
V_442 [ V_32 ] ) ;
F_5 ( V_3 -> V_28 , 0x0E , ( unsigned char ) ( V_442 [ 16 ]
& 0xE0 ) ) ;
F_5 ( V_3 -> V_28 , 0x31 , 0x00 ) ;
F_5 ( V_3 -> V_28 , 0x2B , 0x1B ) ;
F_5 ( V_3 -> V_28 , 0x2C , 0xE1 ) ;
F_9 ( 0x00 , V_3 -> V_177 ) ;
for ( V_32 = 0 ; V_32 < 256 * 3 ; V_32 ++ ) {
F_9 ( 0x0F , ( V_3 -> V_177 + 1 ) ) ;
}
F_60 ( 1 ) ;
F_62 ( V_3 ) ;
V_24 = F_8 ( V_3 -> V_93 ) ;
if ( V_24 & 0x10 )
F_13 ( V_3 -> V_38 , 0x32 , 0xDF , 0x20 ) ;
else
F_13 ( V_3 -> V_38 , 0x32 , 0xDF , 0x00 ) ;
F_9 ( 0x00 , V_3 -> V_177 ) ;
for ( V_32 = 0 ; V_32 < 256 * 3 ; V_32 ++ ) {
F_9 ( 0 , ( V_3 -> V_177 + 1 ) ) ;
}
F_5 ( V_3 -> V_28 , 0x01 , V_443 ) ;
F_5 ( V_3 -> V_38 , 0x63 , V_448 ) ;
F_5 ( V_3 -> V_28 , 0x31 , V_449 ) ;
F_5 ( V_3 -> V_38 , 0x53 , ( F_3 (
V_3 -> V_38 , 0x53 ) & 0xFD ) ) ;
F_5 ( V_3 -> V_28 , 0x1F , ( unsigned char ) V_444 ) ;
}
static void F_111 ( struct V_316 * V_317 ,
struct V_35 * V_36 ,
struct V_2 * V_3 )
{
unsigned short V_217 ;
if ( V_3 -> V_16 & ( V_85 | V_86 | V_17
| V_18 | V_70 ) ) {
if ( V_3 -> V_9 & V_278 )
F_5 ( V_3 -> V_233 , 0x1E , 0x20 ) ;
if ( V_3 -> V_9 & ( V_48 | V_47 |
V_68 ) ) {
V_217 = F_3 ( V_3 -> V_28 , 0x32 ) ;
V_217 &= 0xDF ;
if ( V_3 -> V_9 & V_49 ) {
if ( ! ( V_3 -> V_9 & V_68 ) )
V_217 |= 0x20 ;
}
F_5 ( V_3 -> V_28 , 0x32 , V_217 ) ;
F_21 ( V_3 -> V_28 , 0x1E , 0x20 ) ;
V_217 = F_3 ( V_3 -> V_233 , 0x2E ) ;
if ( ! ( V_217 & 0x80 ) )
F_21 ( V_3 -> V_233 , 0x2E , 0x80 ) ;
F_14 ( V_3 -> V_233 , 0x00 , 0x7F ) ;
}
if ( ! ( V_3 -> V_9 & V_279 ) ) {
F_13 ( V_3 -> V_263 , 0x00 , ~ 0xE0 ,
0x20 ) ;
if ( V_3 -> V_16 & ( V_18 | V_70 ) ) {
if ( V_3 -> V_9 &
( V_48 | V_46 ) )
F_14 ( V_3 -> V_240 , 0x2A ,
0x7F ) ;
F_14 ( V_3 -> V_240 , 0x30 , 0x7F ) ;
}
}
V_217 = 0x00 ;
if ( ! ( V_3 -> V_9 & V_279 ) ) {
V_217 = 0xc0 ;
if ( ! ( V_3 -> V_9 & V_285 ) &&
( V_3 -> V_9 & V_46 ) &&
( V_3 -> V_9 & V_278 ) ) {
V_217 = V_217 & 0x40 ;
if ( V_3 -> V_9 & V_46 )
V_217 = V_217 ^ 0xC0 ;
}
}
F_21 ( V_3 -> V_240 , 0x1F , V_217 ) ;
F_86 ( V_36 , V_3 ) ;
F_59 ( V_317 , V_36 , V_3 ) ;
}
else {
if ( V_3 -> V_9 & ( V_47 | V_48
| V_46 ) )
F_21 ( V_3 -> V_233 , 0x1E , 0x20 ) ;
V_217 = F_3 ( V_3 -> V_233 , 0x2E ) ;
if ( ! ( V_217 & 0x80 ) )
F_21 ( V_3 -> V_233 , 0x2E , 0x80 ) ;
F_14 ( V_3 -> V_233 , 0x00 , 0x7F ) ;
F_59 ( V_317 , V_36 , V_3 ) ;
}
}
static void F_112 ( struct V_316 * V_317 ,
struct V_35 * V_36 ,
unsigned short V_29 , unsigned short V_30 ,
struct V_2 * V_3 )
{
unsigned short V_60 , V_24 ;
F_4 ( V_29 , V_30 , V_3 ) ;
F_9 ( V_33 . V_450 , V_3 -> V_93 ) ;
F_6 ( V_36 , V_3 ) ;
F_7 ( V_29 , V_30 , V_3 ) ;
F_10 ( V_3 ) ;
F_11 ( V_3 ) ;
if ( V_36 -> V_99 == V_23 ) {
if ( V_3 -> V_152 == 0 )
F_12 ( V_3 ) ;
}
V_24 = ~ V_247 ;
V_3 -> V_78 &= V_24 ;
V_3 -> V_438 = 0 ;
if ( V_3 -> V_16 & ( V_85 | V_86 | V_17
| V_18 | V_70 ) ) {
if ( V_3 -> V_9 & ( V_285 | V_46
| V_49 ) ) {
V_3 -> V_78 |= V_247 ;
}
}
V_60 = F_107 ( V_36 , V_29 ,
V_30 , V_3 ) ;
if ( V_60 != 0xFFFF ) {
F_16 ( V_60 , V_3 ) ;
F_19 ( V_29 , V_30 , V_60 ,
V_3 , V_36 ) ;
F_26 ( V_36 , V_29 , V_30 ,
V_60 , V_3 ) ;
F_27 ( V_29 , V_30 , V_60 ,
V_36 , V_3 ) ;
F_29 ( V_29 , V_30 , V_36 ,
V_60 , V_3 ) ;
}
if ( V_36 -> V_99 >= V_156 ) {
V_24 = F_3 ( V_3 -> V_38 , 0x38 ) ;
if ( V_24 & 0xA0 ) {
if ( V_36 -> V_99 == V_23 )
F_22 ( V_29 , V_30 ,
V_60 , V_3 ) ;
else
F_20 ( V_29 , V_30 ,
V_60 , V_3 ) ;
F_25 ( V_29 , V_3 ,
V_60 ) ;
F_24 ( V_36 -> V_99 ,
V_3 , V_60 , V_29 ) ;
if ( V_3 -> V_152 == 1 )
F_88 ( V_317 ,
V_36 -> V_99 ,
V_29 , V_30 , V_3 ) ;
}
}
V_3 -> V_78 &= ( ~ V_247 ) ;
F_31 ( V_29 , V_36 , V_3 ) ;
F_33 ( V_36 , V_29 , V_30 ,
V_60 , V_3 ) ;
F_35 ( V_29 , V_30 , V_3 ) ;
}
unsigned char F_113 ( struct V_316 * V_317 ,
struct V_35 * V_36 ,
unsigned short V_29 )
{
unsigned short V_30 ;
struct V_2 V_451 ;
struct V_2 * V_3 = & V_451 ;
V_3 -> V_152 = 0 ;
if ( V_36 -> V_99 >= V_15 )
V_3 -> V_16 = 0 ;
F_114 ( V_3 , V_317 -> V_452 ) ;
if ( V_36 -> V_99 == V_156 ) {
if ( ( F_3 ( V_3 -> V_38 , 0x38 ) & 0xE0 ) == 0xC0 )
V_3 -> V_152 = 1 ;
}
if ( V_36 -> V_99 == V_23 ) {
if ( ( F_3 ( V_3 -> V_38 , 0x38 ) & 0xE0 ) == 0xC0 ) {
if ( F_3 ( V_3 -> V_38 , 0x30 ) & 0x20 )
V_3 -> V_152 = 1 ;
}
}
F_1 ( V_36 -> V_99 , V_3 ) ;
if ( V_29 & 0x80 )
V_29 = V_29 & 0x7F ;
F_5 ( V_3 -> V_28 , 0x05 , 0x86 ) ;
if ( V_36 -> V_99 < V_15 )
F_105 ( V_36 , V_3 ) ;
F_53 ( V_29 , & V_30 , V_3 ) ;
if ( V_36 -> V_99 < V_15 ) {
F_50 ( V_29 , V_30 , V_36 , V_3 ) ;
F_51 ( V_29 , V_30 , V_3 ) ;
F_52 ( V_29 , V_30 , V_3 ) ;
F_90 ( V_317 , V_36 , V_3 ) ;
if ( V_3 -> V_9 & ( V_285 | V_46 ) ||
( ! ( V_3 -> V_9 & V_286 ) ) ) {
F_112 ( V_317 , V_36 , V_29 ,
V_30 , V_3 ) ;
if ( V_3 -> V_9 & V_46 ) {
F_108 ( V_29 , V_30 ,
V_36 , V_3 ) ;
}
}
if ( V_3 -> V_9 & ( V_285 | V_286 ) ) {
switch ( V_36 -> V_453 ) {
case V_454 :
case V_455 :
F_109 ( V_29 , V_36 ,
V_3 ) ;
break;
default:
break;
}
}
F_104 ( V_29 , V_36 , V_3 ) ;
F_103 ( V_29 , V_30 , V_3 ) ;
F_111 ( V_317 , V_36 , V_3 ) ;
}
else {
if ( V_3 -> V_152 == 1 )
if ( ! F_87 ( V_317 , V_29 ,
V_30 ,
V_3 ) )
return 0 ;
V_3 -> V_55 = V_42 [ V_30 ] .
V_43 & V_277 ;
V_3 -> V_78 = 0 ;
V_3 -> V_9 = V_279 ;
F_61 ( V_317 , V_36 , V_3 ) ;
F_112 ( V_317 , V_36 , V_29 ,
V_30 , V_3 ) ;
F_59 ( V_317 , V_36 , V_3 ) ;
}
F_49 ( V_36 , V_3 ) ;
if ( V_36 -> V_99 < V_15 )
F_106 ( V_36 , V_3 ) ;
return 1 ;
}
