static inline struct V_1 * F_1 ( struct V_2 * V_3 )
{
return F_2 ( V_3 , struct V_1 , V_3 ) ;
}
static void F_3 ( struct V_1 * V_4 , int V_5 )
{
int V_6 ;
F_4 ( V_4 , V_7 , 0x00 ) ;
F_4 ( V_4 , V_8 , 0x00 ) ;
F_4 ( V_4 , V_9 , 0x00 ) ;
F_4 ( V_4 , V_10 , 0x00 ) ;
F_4 ( V_4 , V_11 , 0x00 ) ;
F_4 ( V_4 , V_12 , 0x00 ) ;
F_4 ( V_4 , V_13 , 0x00 ) ;
F_4 ( V_4 , V_14 , 0x00 ) ;
F_4 ( V_4 , V_15 , 0x00 ) ;
F_4 ( V_4 , V_16 ,
0x00 ) ;
F_4 ( V_4 , V_17 ,
0x00 ) ;
F_4 ( V_4 , V_18 ,
0x00 ) ;
for ( V_6 = 0x30 ; V_6 < 0x60 ; V_6 ++ )
F_4 ( V_4 , V_6 , 0x40 ) ;
F_4 ( V_4 , 0x44 , 0x60 ) ;
F_4 ( V_4 , V_7 ,
V_19 ) ;
}
static void F_5 ( struct V_1 * V_4 , T_1 V_20 )
{
int V_6 ;
int V_21 ;
F_4 ( V_4 , V_22 , 0x07 ) ;
F_4 ( V_4 , V_23 , 0xed ) ;
F_4 ( V_4 , V_24 , 0x79 ) ;
F_4 ( V_4 , V_25 , 0x80 ) ;
F_4 ( V_4 , V_26 , 0x80 ) ;
F_4 ( V_4 , V_27 , 0x00 ) ;
F_4 ( V_4 , V_28 , 0x00 ) ;
F_4 ( V_4 , V_29 , 0x00 ) ;
if ( V_20 == 0x23 ) {
F_4 ( V_4 , V_30 , 0x04 ) ;
} else {
F_4 ( V_4 , V_30 , 0x00 ) ;
}
F_4 ( V_4 , V_31 ,
V_32 ) ;
F_4 ( V_4 , V_33 ,
V_34 ) ;
F_4 ( V_4 , V_35 ,
V_36 ) ;
F_4 ( V_4 , V_37 ,
V_38 |
V_39 |
V_40 ) ;
F_4 ( V_4 , V_41 ,
V_42 ) ;
F_4 ( V_4 , V_43 ,
V_44 ) ;
F_4 ( V_4 , V_45 ,
V_46 ) ;
F_4 ( V_4 , V_47 ,
V_48 ) ;
F_4 ( V_4 , V_49 ,
V_50 ) ;
F_4 ( V_4 , V_51 ,
V_52 ) ;
F_4 ( V_4 , V_53 ,
V_54 ) ;
F_4 ( V_4 , V_55 ,
V_56 ) ;
F_4 ( V_4 , V_57 ,
V_58 ) ;
F_4 ( V_4 , V_59 ,
V_60 ) ;
if ( V_20 == V_61 ||
V_20 == V_62 ) {
F_4 ( V_4 , V_63 ,
V_64 ) ;
F_4 ( V_4 , V_65 ,
V_66 ) ;
} else {
F_4 ( V_4 , V_63 ,
V_67 ) ;
F_4 ( V_4 , V_65 ,
V_68 ) ;
}
F_4 ( V_4 , V_69 ,
V_70 ) ;
F_4 ( V_4 , V_71 ,
V_72 ) ;
F_4 ( V_4 , V_73 ,
V_74 ) ;
F_4 ( V_4 , V_75 , V_76 ) ;
F_4 ( V_4 , V_77 , V_78 ) ;
F_4 ( V_4 , V_79 ,
V_80 ) ;
F_4 ( V_4 , V_81 , V_82 ) ;
F_4 ( V_4 , V_83 , V_84 ) ;
F_4 ( V_4 , V_85 ,
V_86 ) ;
F_4 ( V_4 , V_87 ,
V_88 ) ;
F_4 ( V_4 , V_89 ,
V_90 ) ;
F_4 ( V_4 , V_91 ,
V_92 ) ;
F_4 ( V_4 , V_93 ,
V_94 ) ;
F_4 ( V_4 , V_95 ,
V_96 ) ;
F_4 ( V_4 , V_97 ,
V_98 ) ;
F_4 ( V_4 , V_99 , V_100 ) ;
F_3 ( V_4 , 0 ) ;
if ( V_20 == V_61 ||
V_20 == V_62 ) {
V_21 = 5 ;
} else {
V_21 = 5 ;
}
for ( V_6 = 0 ; V_6 < V_101 ; V_6 ++ ) {
F_4 ( V_4 , V_102 [ V_6 ] . V_103 ,
V_102 [ V_6 ] . V_104 [ V_21 ] ) ;
}
F_4 ( V_4 , V_105 , 0x87 ) ;
F_4 ( V_4 , V_106 , 0xa2 ) ;
F_4 ( V_4 , V_107 , 0xbf ) ;
F_4 ( V_4 , V_108 , 0xcb ) ;
F_4 ( V_4 , V_109 , 0xa1 ) ;
F_4 ( V_4 , V_110 , 0x41 ) ;
F_4 ( V_4 , V_111 , 0x88 ) ;
F_4 ( V_4 , V_112 , 0xc2 ) ;
F_4 ( V_4 , V_113 , 0x3c ) ;
}
static void F_6 ( struct V_1 * V_4 )
{
F_4 ( V_4 , V_114 ,
V_115 ) ;
F_4 ( V_4 , V_116 , 0x00 ) ;
F_4 ( V_4 , V_117 , 0x00 ) ;
F_4 ( V_4 , V_118 ,
V_119 ) ;
F_5 ( V_4 , V_119 ) ;
F_4 ( V_4 , V_120 ,
V_121 ) ;
}
static void F_7 ( struct V_1 * V_4 )
{
F_4 ( V_4 , V_114 ,
V_115 ) ;
F_4 ( V_4 , V_116 , 0x00 ) ;
F_4 ( V_4 , V_117 , 0x0e ) ;
F_4 ( V_4 , V_116 , 0x10 ) ;
F_4 ( V_4 , V_118 ,
V_122 ) ;
F_5 ( V_4 ,
V_122 ) ;
F_4 ( V_4 , V_120 ,
V_121 ) ;
}
static void F_8 ( struct V_1 * V_4 )
{
F_4 ( V_4 , V_114 ,
V_123 ) ;
F_4 ( V_4 , V_118 ,
V_61 ) ;
F_4 ( V_4 , V_116 , 0x00 ) ;
F_4 ( V_4 , V_117 , 0x00 ) ;
F_5 ( V_4 ,
V_61 ) ;
F_4 ( V_4 , V_120 ,
V_121 ) ;
}
static void F_9 ( struct V_1 * V_4 )
{
F_4 ( V_4 , V_124 , 0x00 ) ;
F_4 ( V_4 , V_125 , 0x00 ) ;
F_4 ( V_4 , V_126 , 0x00 ) ;
F_4 ( V_4 , V_127 , 0x04 ) ;
F_4 ( V_4 , V_128 , 0x02 ) ;
F_4 ( V_4 , V_120 ,
V_129 ) ;
}
static void F_10 ( struct V_1 * V_4 , int V_5 )
{
int V_6 ;
if ( V_5 == V_130 ) {
F_9 ( V_4 ) ;
return;
}
if ( V_5 != V_131 ) {
F_11 ( V_132 L_1 ,
V_5 ) ;
return;
}
for ( V_6 = 0 ; V_6 < V_133 ; V_6 ++ ) {
F_4 ( V_4 , V_134 [ V_6 ] . V_103 ,
V_134 [ V_6 ] . V_104 [ 0 ] ) ;
}
F_4 ( V_4 , V_124 , 0x00 ) ;
F_4 ( V_4 , V_125 , 0x00 ) ;
F_4 ( V_4 , V_126 , 0x00 ) ;
F_4 ( V_4 , V_135 , 0x80 ) ;
F_4 ( V_4 , V_136 , 0x84 ) ;
F_12 ( 150 ) ;
F_4 ( V_4 , V_120 , 0x00 ) ;
F_12 ( 1 ) ;
F_4 ( V_4 , V_120 , 0x9d ) ;
F_12 ( 50 ) ;
F_4 ( V_4 , V_124 , 0x7F ) ;
F_4 ( V_4 , V_125 , 0x7F ) ;
F_4 ( V_4 , V_126 , 0xff ) ;
F_12 ( 80 ) ;
F_4 ( V_4 , V_124 , 0x7F ) ;
F_4 ( V_4 , V_125 , 0x7F ) ;
F_4 ( V_4 , V_137 , V_138 ) ;
F_4 ( V_4 , V_139 , 0x82 ) ;
F_12 ( 70 ) ;
F_4 ( V_4 , V_127 , 0x09 ) ;
F_4 ( V_4 , V_140 , 0x03 ) ;
F_4 ( V_4 , V_128 , 0xc2 ) ;
}
static int F_13 ( struct V_141 * V_142 )
{
struct V_1 * V_4 =
F_2 ( V_142 -> V_143 , struct V_1 , V_144 ) ;
switch ( V_142 -> V_145 ) {
case V_146 :
F_4 ( V_4 , V_23 ,
V_142 -> V_147 - 128 ) ;
break;
case V_148 :
F_4 ( V_4 , V_24 ,
V_142 -> V_147 ) ;
break;
case V_149 :
F_4 ( V_4 , V_25 ,
V_142 -> V_147 ) ;
F_4 ( V_4 , V_26 ,
V_142 -> V_147 ) ;
break;
case V_150 :
F_4 ( V_4 , V_27 ,
V_142 -> V_147 >> 8 ) ;
F_4 ( V_4 , V_28 ,
V_142 -> V_147 & 0xFF ) ;
break;
default:
return - V_151 ;
}
return 0 ;
}
static int F_14 ( struct V_2 * V_3 ,
struct V_152 * V_153 )
{
struct V_1 * V_4 = F_1 ( V_3 ) ;
V_153 -> V_147 = F_15 ( V_4 , V_153 -> V_153 & 0xffff ) ;
return 0 ;
}
static int F_16 ( struct V_2 * V_3 ,
const struct V_152 * V_153 )
{
struct V_1 * V_4 = F_1 ( V_3 ) ;
F_4 ( V_4 , V_153 -> V_153 , V_153 -> V_147 & 0xff ) ;
return 0 ;
}
static int F_17 ( struct V_2 * V_3 , int V_154 )
{
struct V_1 * V_4 = F_1 ( V_3 ) ;
if ( V_154 ) {
F_4 ( V_4 , V_120 ,
0x01 ) ;
F_12 ( 1 ) ;
F_4 ( V_4 , V_120 ,
V_121 ) ;
} else {
F_4 ( V_4 , V_120 ,
1 << 5 ) ;
}
return 0 ;
}
static int F_18 ( struct V_2 * V_3 , T_2 V_147 )
{
struct V_1 * V_4 = F_1 ( V_3 ) ;
V_4 -> V_155 = V_156 ;
V_4 -> V_157 = 0 ;
F_4 ( V_4 , 0xa4 , 1 << 5 ) ;
return 0 ;
}
static int F_19 ( struct V_2 * V_3 ,
T_2 V_158 , T_2 V_159 , T_2 V_160 )
{
struct V_1 * V_4 = F_1 ( V_3 ) ;
F_18 ( V_3 , 0 ) ;
if ( V_158 == V_161 ) {
F_6 ( V_4 ) ;
} else if ( V_158 == V_162 ) {
F_8 ( V_4 ) ;
} else if ( V_158 == V_163 ) {
F_7 ( V_4 ) ;
} else {
F_11 ( V_132 L_2 ) ;
return - V_151 ;
}
return 0 ;
}
static int F_20 ( struct V_2 * V_3 ,
T_2 V_158 , T_2 V_159 , T_2 V_160 )
{
struct V_1 * V_4 = F_1 ( V_3 ) ;
F_10 ( V_4 , V_158 ) ;
return 0 ;
}
static int F_21 ( struct V_2 * V_3 , struct V_164 * V_165 )
{
int V_147 = 0 ;
struct V_1 * V_4 = F_1 ( V_3 ) ;
T_1 V_166 ;
V_166 = F_15 ( V_4 , 0x00 ) ;
if ( V_166 == 0xa2 )
V_165 -> signal = 0xffff ;
else
V_165 -> signal = 0x00 ;
V_165 -> V_167 |=
V_168 | V_169 |
V_170 | V_171 ;
V_147 = V_172 ;
V_165 -> V_173 = V_147 ;
V_165 -> V_174 = V_175 ;
return 0 ;
}
static int F_22 ( struct V_176 * V_177 ,
const struct V_178 * V_179 )
{
struct V_1 * V_4 ;
struct V_180 * V_144 ;
struct V_2 * V_3 ;
int V_181 ;
struct V_182 * V_183 ;
if ( ! F_23 ( V_177 -> V_184 ,
V_185 ) ) {
return - V_186 ;
}
V_181 = F_24 ( & V_4 , V_177 -> V_184 , V_177 -> V_187 ) ;
switch ( V_181 ) {
case 0 :
F_11 ( V_132 L_3 ) ;
return - V_186 ;
case 1 :
F_11 ( V_188 L_4 ) ;
break;
default:
F_11 ( V_188 L_5 ) ;
break;
}
V_183 = F_25 ( sizeof( struct V_182 ) , V_189 ) ;
if ( V_183 == NULL ) {
if ( V_181 == 1 )
F_26 ( V_4 ) ;
return - V_190 ;
}
V_183 -> V_191 = 0x8e >> 1 ;
V_4 -> V_160 = V_183 ;
V_4 -> V_192 = V_177 -> V_184 ;
V_3 = & V_4 -> V_3 ;
F_27 ( V_3 , V_177 , & V_193 ) ;
V_144 = & V_4 -> V_144 ;
F_28 ( V_144 , 4 ) ;
F_29 ( V_144 , & V_194 ,
V_146 , 0 , 255 , 1 , 109 ) ;
F_29 ( V_144 , & V_194 ,
V_148 , 0 , 255 , 1 ,
V_195 ) ;
F_29 ( V_144 , & V_194 ,
V_149 , 0 , 255 , 1 , 128 ) ;
F_29 ( V_144 , & V_194 ,
V_150 , - 32768 , 32767 , 1 , 0 ) ;
V_3 -> V_196 = V_144 ;
if ( V_144 -> error ) {
int V_197 = V_144 -> error ;
F_30 ( V_144 ) ;
F_26 ( V_183 ) ;
F_26 ( V_4 ) ;
return V_197 ;
}
V_4 -> V_198 = V_177 ;
V_4 -> V_199 = V_161 ;
V_4 -> V_5 = V_130 ;
V_4 -> V_145 = 8522 ;
V_4 -> V_200 = 0 ;
F_4 ( V_4 , 0x106 , 1 ) ;
return 0 ;
}
static int F_31 ( struct V_176 * V_177 )
{
struct V_2 * V_3 = F_32 ( V_177 ) ;
F_33 ( V_3 ) ;
F_30 ( V_3 -> V_196 ) ;
F_34 ( F_1 ( V_3 ) ) ;
return 0 ;
}
