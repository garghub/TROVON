static struct V_1 * F_1 ( const char * V_2 )
{
struct V_1 * V_3 , * V_4 ;
V_3 = NULL ;
F_2 (temp_pwrdm, &pwrdm_list, node) {
if ( ! strcmp ( V_2 , V_4 -> V_2 ) ) {
V_3 = V_4 ;
break;
}
}
return V_3 ;
}
static int F_3 ( struct V_1 * V_3 )
{
int V_5 ;
struct V_6 * V_7 ;
if ( ! V_3 || ! V_3 -> V_2 )
return - V_8 ;
if ( F_4 () &&
V_3 -> V_9 == V_10 ) {
F_5 ( L_1 ,
V_3 -> V_2 ) ;
return - V_8 ;
}
if ( F_1 ( V_3 -> V_2 ) )
return - V_11 ;
if ( V_12 && V_12 -> V_13 )
if ( ! V_12 -> V_13 () )
goto V_14;
V_7 = F_6 ( V_3 -> V_7 . V_2 ) ;
if ( ! V_7 ) {
F_5 ( L_2 ,
V_3 -> V_2 , V_3 -> V_7 . V_2 ) ;
return - V_8 ;
}
V_3 -> V_7 . V_15 = V_7 ;
F_7 ( & V_3 -> V_16 ) ;
F_8 ( V_7 , V_3 ) ;
V_14:
F_9 ( & V_3 -> V_17 ) ;
F_10 ( & V_3 -> V_18 , & V_19 ) ;
for ( V_5 = 0 ; V_5 < V_20 ; V_5 ++ )
V_3 -> V_21 [ V_5 ] = 0 ;
V_3 -> V_22 = 0 ;
for ( V_5 = 0 ; V_5 < V_3 -> V_23 ; V_5 ++ )
V_3 -> V_24 [ V_5 ] = 0 ;
V_12 -> V_25 ( V_3 ) ;
V_3 -> V_26 = F_11 ( V_3 ) ;
V_3 -> V_21 [ V_3 -> V_26 ] = 1 ;
F_12 ( L_3 , V_3 -> V_2 ) ;
return 0 ;
}
static void F_13 ( struct V_1 * V_3 )
{
int V_5 ;
T_1 V_27 , V_28 ;
V_27 = F_14 ( V_3 ) ;
if ( ( V_3 -> V_29 == V_30 ) &&
( V_27 == V_31 ) )
V_3 -> V_22 ++ ;
for ( V_5 = 0 ; V_5 < V_3 -> V_23 ; V_5 ++ ) {
V_28 = F_15 ( V_3 , V_5 ) ;
if ( ( V_3 -> V_32 [ V_5 ] == V_30 ) &&
( V_28 == V_31 ) )
V_3 -> V_24 [ V_5 ] ++ ;
}
}
static int F_16 ( struct V_1 * V_3 , int V_33 )
{
int V_34 , V_35 , V_26 , V_36 = 0 ;
if ( V_3 == NULL )
return - V_8 ;
V_26 = F_11 ( V_3 ) ;
switch ( V_33 ) {
case V_37 :
V_34 = V_3 -> V_26 ;
break;
case V_38 :
V_34 = F_17 ( V_3 ) ;
if ( V_3 -> V_26 != V_34 )
V_3 -> V_21 [ V_34 ] ++ ;
if ( V_34 == V_39 )
F_13 ( V_3 ) ;
V_35 = F_18 ( V_3 ) ;
if ( V_35 != V_34 ) {
V_36 = ( V_40 |
( ( V_35 & V_41 ) << 8 ) |
( ( V_34 & V_41 ) << 0 ) ) ;
F_19 ( V_3 -> V_2 , V_36 ,
F_20 () ) ;
}
break;
default:
return - V_8 ;
}
if ( V_26 != V_34 )
V_3 -> V_21 [ V_26 ] ++ ;
F_21 ( V_3 , V_34 ) ;
V_3 -> V_26 = V_26 ;
return 0 ;
}
static int F_22 ( struct V_1 * V_3 , void * V_42 )
{
F_23 ( V_3 ) ;
F_16 ( V_3 , V_37 ) ;
return 0 ;
}
static int F_24 ( struct V_1 * V_3 , void * V_42 )
{
F_16 ( V_3 , V_38 ) ;
return 0 ;
}
static T_1 F_25 ( struct V_1 * V_3 ,
T_1 V_43 , T_1 V_44 ,
bool * V_45 )
{
T_1 V_46 ;
if ( V_43 < V_47 ) {
if ( V_43 > V_44 &&
V_3 -> V_48 & V_49 &&
V_12 -> V_50 ) {
V_46 = V_51 ;
} else {
* V_45 = F_26 ( V_3 -> V_52 [ 0 ] ) ;
F_27 ( V_3 -> V_52 [ 0 ] ) ;
V_46 = V_53 ;
}
} else {
V_46 = V_54 ;
}
return V_46 ;
}
static void F_28 ( struct V_1 * V_3 ,
T_1 V_46 , bool V_45 )
{
switch ( V_46 ) {
case V_53 :
if ( V_45 )
F_29 ( V_3 -> V_52 [ 0 ] ) ;
else
F_30 ( V_3 -> V_52 [ 0 ] ) ;
break;
case V_51 :
if ( V_3 -> V_48 & V_49 &&
V_12 -> V_50 )
V_12 -> V_50 ( V_3 ) ;
F_31 ( V_3 ) ;
break;
}
}
int F_32 ( struct V_55 * V_56 )
{
if ( ! V_56 )
return - V_8 ;
if ( V_12 )
return - V_11 ;
V_12 = V_56 ;
return 0 ;
}
int F_33 ( struct V_1 * * V_57 )
{
struct V_1 * * V_58 = NULL ;
if ( ! V_12 )
return - V_11 ;
if ( ! V_57 )
return - V_8 ;
for ( V_58 = V_57 ; * V_58 ; V_58 ++ )
F_3 ( * V_58 ) ;
return 0 ;
}
int F_34 ( void )
{
struct V_1 * V_59 ;
if ( F_35 ( & V_19 ) )
return - V_60 ;
F_2 (temp_p, &pwrdm_list, node)
F_36 ( V_59 , V_47 ) ;
return 0 ;
}
void F_37 ( struct V_1 * V_3 )
__acquires( &pwrdm->_lock
void F_38 ( struct V_1 * V_3 )
__releases( &pwrdm->_lock
struct V_1 * F_39 ( const char * V_2 )
{
struct V_1 * V_3 ;
if ( ! V_2 )
return NULL ;
V_3 = F_1 ( V_2 ) ;
return V_3 ;
}
int F_40 ( int (* F_41)( struct V_1 * V_3 , void * V_61 ) ,
void * V_61 )
{
struct V_1 * V_4 ;
int V_62 = 0 ;
if ( ! F_41 )
return - V_8 ;
F_2 (temp_pwrdm, &pwrdm_list, node) {
V_62 = (* F_41)( V_4 , V_61 ) ;
if ( V_62 )
break;
}
return V_62 ;
}
int F_42 ( struct V_1 * V_3 , struct V_63 * V_64 )
{
int V_5 ;
int V_62 = - V_8 ;
if ( ! V_3 || ! V_64 )
return - V_8 ;
F_12 ( L_4 ,
V_3 -> V_2 , V_64 -> V_2 ) ;
for ( V_5 = 0 ; V_5 < V_65 ; V_5 ++ ) {
if ( ! V_3 -> V_52 [ V_5 ] )
break;
#ifdef F_43
if ( V_3 -> V_52 [ V_5 ] == V_64 ) {
V_62 = - V_8 ;
goto V_66;
}
#endif
}
if ( V_5 == V_65 ) {
F_12 ( L_5 ,
V_3 -> V_2 , V_64 -> V_2 ) ;
F_44 ( 1 ) ;
V_62 = - V_67 ;
goto V_66;
}
V_3 -> V_52 [ V_5 ] = V_64 ;
V_62 = 0 ;
V_66:
return V_62 ;
}
int F_45 ( struct V_1 * V_3 , struct V_63 * V_64 )
{
int V_62 = - V_8 ;
int V_5 ;
if ( ! V_3 || ! V_64 )
return - V_8 ;
F_12 ( L_6 ,
V_3 -> V_2 , V_64 -> V_2 ) ;
for ( V_5 = 0 ; V_5 < V_65 ; V_5 ++ )
if ( V_3 -> V_52 [ V_5 ] == V_64 )
break;
if ( V_5 == V_65 ) {
F_12 ( L_7 ,
V_3 -> V_2 , V_64 -> V_2 ) ;
V_62 = - V_68 ;
goto V_69;
}
V_3 -> V_52 [ V_5 ] = NULL ;
V_62 = 0 ;
V_69:
return V_62 ;
}
int F_46 ( struct V_1 * V_3 ,
int (* F_41)( struct V_1 * V_3 ,
struct V_63 * V_64 ) )
{
int V_62 = 0 ;
int V_5 ;
if ( ! F_41 )
return - V_8 ;
for ( V_5 = 0 ; V_5 < V_65 && ! V_62 ; V_5 ++ )
V_62 = (* F_41)( V_3 , V_3 -> V_52 [ V_5 ] ) ;
return V_62 ;
}
struct V_6 * F_47 ( struct V_1 * V_3 )
{
return V_3 -> V_7 . V_15 ;
}
int F_48 ( struct V_1 * V_3 )
{
if ( ! V_3 )
return - V_8 ;
return V_3 -> V_23 ;
}
int F_36 ( struct V_1 * V_3 , T_1 V_44 )
{
int V_62 = - V_8 ;
if ( ! V_3 )
return - V_8 ;
if ( ! ( V_3 -> V_70 & ( 1 << V_44 ) ) )
return - V_8 ;
F_12 ( L_8 ,
V_3 -> V_2 , V_44 ) ;
if ( V_12 && V_12 -> F_36 ) {
F_19 ( V_3 -> V_2 , V_44 ,
F_20 () ) ;
V_62 = V_12 -> F_36 ( V_3 , V_44 ) ;
}
return V_62 ;
}
int F_18 ( struct V_1 * V_3 )
{
int V_62 = - V_8 ;
if ( ! V_3 )
return - V_8 ;
if ( V_12 && V_12 -> F_18 )
V_62 = V_12 -> F_18 ( V_3 ) ;
return V_62 ;
}
int F_11 ( struct V_1 * V_3 )
{
int V_62 = - V_8 ;
if ( ! V_3 )
return - V_8 ;
if ( V_3 -> V_70 == V_71 )
return V_47 ;
if ( V_12 && V_12 -> F_11 )
V_62 = V_12 -> F_11 ( V_3 ) ;
return V_62 ;
}
int F_17 ( struct V_1 * V_3 )
{
int V_62 = - V_8 ;
if ( ! V_3 )
return - V_8 ;
if ( V_12 && V_12 -> F_17 )
V_62 = V_12 -> F_17 ( V_3 ) ;
return V_62 ;
}
int F_49 ( struct V_1 * V_3 , T_1 V_44 )
{
int V_62 = - V_8 ;
if ( ! V_3 )
return - V_8 ;
if ( ! ( V_3 -> V_29 & ( 1 << V_44 ) ) )
return - V_8 ;
F_12 ( L_9 ,
V_3 -> V_2 , V_44 ) ;
if ( V_12 && V_12 -> F_49 )
V_62 = V_12 -> F_49 ( V_3 , V_44 ) ;
return V_62 ;
}
int F_50 ( struct V_1 * V_3 , T_1 V_72 , T_1 V_44 )
{
int V_62 = - V_8 ;
if ( ! V_3 )
return - V_8 ;
if ( V_3 -> V_23 < ( V_72 + 1 ) )
return - V_11 ;
if ( ! ( V_3 -> V_73 [ V_72 ] & ( 1 << V_44 ) ) )
return - V_8 ;
F_12 ( L_10 ,
V_3 -> V_2 , V_72 , V_44 ) ;
if ( V_12 && V_12 -> F_50 )
V_62 = V_12 -> F_50 ( V_3 , V_72 , V_44 ) ;
return V_62 ;
}
int F_51 ( struct V_1 * V_3 , T_1 V_72 , T_1 V_44 )
{
int V_62 = - V_8 ;
if ( ! V_3 )
return - V_8 ;
if ( V_3 -> V_23 < ( V_72 + 1 ) )
return - V_11 ;
if ( ! ( V_3 -> V_32 [ V_72 ] & ( 1 << V_44 ) ) )
return - V_8 ;
F_12 ( L_11 ,
V_3 -> V_2 , V_72 , V_44 ) ;
if ( V_12 && V_12 -> F_51 )
V_62 = V_12 -> F_51 ( V_3 , V_72 , V_44 ) ;
return V_62 ;
}
int F_52 ( struct V_1 * V_3 )
{
int V_62 = - V_8 ;
if ( ! V_3 )
return - V_8 ;
if ( V_12 && V_12 -> F_52 )
V_62 = V_12 -> F_52 ( V_3 ) ;
return V_62 ;
}
int F_14 ( struct V_1 * V_3 )
{
int V_62 = - V_8 ;
if ( ! V_3 )
return - V_8 ;
if ( V_12 && V_12 -> F_14 )
V_62 = V_12 -> F_14 ( V_3 ) ;
return V_62 ;
}
int F_53 ( struct V_1 * V_3 )
{
int V_62 = - V_8 ;
if ( ! V_3 )
return - V_8 ;
if ( V_12 && V_12 -> F_53 )
V_62 = V_12 -> F_53 ( V_3 ) ;
return V_62 ;
}
int F_54 ( struct V_1 * V_3 , T_1 V_72 )
{
int V_62 = - V_8 ;
if ( ! V_3 )
return V_62 ;
if ( V_3 -> V_23 < ( V_72 + 1 ) )
return V_62 ;
if ( V_3 -> V_48 & V_74 )
V_72 = 1 ;
if ( V_12 && V_12 -> F_54 )
V_62 = V_12 -> F_54 ( V_3 , V_72 ) ;
return V_62 ;
}
int F_15 ( struct V_1 * V_3 , T_1 V_72 )
{
int V_62 = - V_8 ;
if ( ! V_3 )
return V_62 ;
if ( V_3 -> V_23 < ( V_72 + 1 ) )
return V_62 ;
if ( V_3 -> V_48 & V_74 )
V_72 = 1 ;
if ( V_12 && V_12 -> F_15 )
V_62 = V_12 -> F_15 ( V_3 , V_72 ) ;
return V_62 ;
}
int F_55 ( struct V_1 * V_3 , T_1 V_72 )
{
int V_62 = - V_8 ;
if ( ! V_3 )
return V_62 ;
if ( V_3 -> V_23 < ( V_72 + 1 ) )
return V_62 ;
if ( V_12 && V_12 -> F_55 )
V_62 = V_12 -> F_55 ( V_3 , V_72 ) ;
return V_62 ;
}
int F_23 ( struct V_1 * V_3 )
{
int V_62 = - V_8 ;
if ( ! V_3 )
return V_62 ;
F_12 ( L_12 ,
V_3 -> V_2 ) ;
if ( V_12 && V_12 -> F_23 )
V_62 = V_12 -> F_23 ( V_3 ) ;
return V_62 ;
}
int F_56 ( struct V_1 * V_3 )
{
int V_62 = - V_8 ;
if ( ! V_3 )
return V_62 ;
if ( ! ( V_3 -> V_48 & V_75 ) )
return V_62 ;
F_12 ( L_13 , V_3 -> V_2 ) ;
if ( V_12 && V_12 -> F_56 )
V_62 = V_12 -> F_56 ( V_3 ) ;
return V_62 ;
}
int F_57 ( struct V_1 * V_3 )
{
int V_62 = - V_8 ;
if ( ! V_3 )
return V_62 ;
if ( ! ( V_3 -> V_48 & V_75 ) )
return V_62 ;
F_12 ( L_14 , V_3 -> V_2 ) ;
if ( V_12 && V_12 -> F_57 )
V_62 = V_12 -> F_57 ( V_3 ) ;
return V_62 ;
}
bool F_58 ( struct V_1 * V_3 )
{
return ( V_3 && V_3 -> V_48 & V_75 ) ? 1 : 0 ;
}
int F_31 ( struct V_1 * V_3 )
{
int V_62 ;
if ( ! V_3 || ! V_12 )
return - V_8 ;
V_62 = V_12 -> V_25 ( V_3 ) ;
if ( ! V_62 )
V_62 = F_16 ( V_3 , V_37 ) ;
return V_62 ;
}
int T_2 F_59 ( struct V_1 * V_3 )
{
int V_62 ;
F_37 ( V_3 ) ;
V_62 = F_31 ( V_3 ) ;
F_38 ( V_3 ) ;
return V_62 ;
}
int F_60 ( struct V_1 * V_3 )
{
if ( V_3 )
F_22 ( V_3 , NULL ) ;
else
F_40 ( F_22 , NULL ) ;
return 0 ;
}
int F_61 ( struct V_1 * V_3 )
{
if ( V_3 )
F_24 ( V_3 , NULL ) ;
else
F_40 ( F_24 , NULL ) ;
return 0 ;
}
int F_62 ( struct V_1 * V_3 , T_1 V_44 )
{
T_1 V_76 , V_46 ;
int V_43 ;
int V_62 = 0 ;
bool V_45 = false ;
if ( ! V_3 || F_63 ( V_3 ) )
return - V_8 ;
while ( ! ( V_3 -> V_70 & ( 1 << V_44 ) ) ) {
if ( V_44 == V_31 )
return V_62 ;
V_44 -- ;
}
F_37 ( V_3 ) ;
V_43 = F_11 ( V_3 ) ;
if ( V_43 < 0 ) {
V_62 = - V_8 ;
goto V_77;
}
V_76 = F_18 ( V_3 ) ;
if ( V_43 == V_44 && V_76 == V_44 )
goto V_77;
V_46 = F_25 ( V_3 , V_43 ,
V_44 , & V_45 ) ;
V_62 = F_36 ( V_3 , V_44 ) ;
if ( V_62 )
F_5 ( L_15 ,
V_78 , V_3 -> V_2 ) ;
F_28 ( V_3 , V_46 , V_45 ) ;
V_77:
F_38 ( V_3 ) ;
return V_62 ;
}
int F_64 ( struct V_1 * V_3 )
{
int V_5 , V_79 ;
if ( ! V_3 ) {
F_65 ( 1 , L_16 , V_78 ) ;
return - V_80 ;
}
V_79 = V_3 -> V_21 [ V_31 ] ;
V_79 += V_3 -> V_22 ;
for ( V_5 = 0 ; V_5 < V_3 -> V_23 ; V_5 ++ )
V_79 += V_3 -> V_24 [ V_5 ] ;
V_79 &= V_81 ;
F_12 ( L_17 ,
V_3 -> V_2 , V_79 ) ;
return V_79 ;
}
bool F_66 ( struct V_1 * V_3 )
{
int V_5 ;
if ( ! V_3 ) {
F_12 ( L_18 ,
V_78 ) ;
return 1 ;
}
if ( V_3 -> V_70 & V_82 )
return 1 ;
if ( V_3 -> V_70 & V_83 ) {
if ( V_3 -> V_29 & V_82 )
return 1 ;
for ( V_5 = 0 ; V_5 < V_3 -> V_23 ; V_5 ++ )
if ( V_3 -> V_32 [ V_5 ] & V_82 )
return 1 ;
}
for ( V_5 = 0 ; V_5 < V_3 -> V_23 ; V_5 ++ )
if ( V_3 -> V_73 [ V_5 ] & V_82 )
return 1 ;
return 0 ;
}
