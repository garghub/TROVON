static int F_1 ( struct V_1 * V_2 )
{
unsigned char * V_3 = V_2 -> V_3 ;
struct V_4 * V_5 = V_2 -> V_5 ;
switch ( V_3 [ 0 ] ) {
case 1 :
if ( V_3 [ 5 ] & 0x80 ) {
V_2 -> V_6 [ 0 ] = ( V_3 [ 5 ] & 0x20 ) ? V_7 : V_8 ;
V_2 -> V_9 [ 0 ] = ( V_3 [ 5 ] & 0x20 ) ? V_10 : V_11 ;
F_2 ( V_5 , V_2 -> V_6 [ 0 ] , 1 ) ;
F_3 ( V_5 , V_12 , V_2 -> V_9 [ 0 ] ) ;
F_3 ( V_5 , V_13 , F_4 ( & V_3 [ 1 ] ) ) ;
F_3 ( V_5 , V_14 , F_4 ( & V_3 [ 3 ] ) ) ;
F_3 ( V_5 , V_15 , ( signed char ) V_3 [ 6 ] + 127 ) ;
F_2 ( V_5 , V_16 , ( ( signed char ) V_3 [ 6 ] > - 127 ) ) ;
F_2 ( V_5 , V_17 , ( V_3 [ 5 ] & 0x40 ) ) ;
} else {
F_2 ( V_5 , V_2 -> V_6 [ 0 ] , 0 ) ;
F_3 ( V_5 , V_12 , 0 ) ;
F_3 ( V_5 , V_15 , - 1 ) ;
F_2 ( V_5 , V_16 , 0 ) ;
}
break;
case 2 :
F_2 ( V_5 , V_8 , 1 ) ;
F_3 ( V_5 , V_12 , V_11 ) ;
F_3 ( V_5 , V_13 , F_4 ( & V_3 [ 1 ] ) ) ;
F_3 ( V_5 , V_14 , F_4 ( & V_3 [ 3 ] ) ) ;
F_3 ( V_5 , V_15 , ( signed char ) V_3 [ 6 ] + 127 ) ;
F_2 ( V_5 , V_16 , ( ( signed char ) V_3 [ 6 ] > - 80 ) && ! ( V_3 [ 5 ] & 0x20 ) ) ;
F_2 ( V_5 , V_17 , ( V_3 [ 5 ] & 0x40 ) ) ;
break;
default:
F_5 ( V_5 -> V_18 . V_19 ,
L_1 , V_20 , V_3 [ 0 ] ) ;
return 0 ;
}
return 1 ;
}
static int F_6 ( struct V_1 * V_2 )
{
struct V_21 * V_22 = & V_2 -> V_22 ;
unsigned char * V_3 = V_2 -> V_3 ;
struct V_4 * V_5 = V_2 -> V_5 ;
int V_23 , V_24 ;
if ( V_3 [ 0 ] != V_25 ) {
F_5 ( V_5 -> V_18 . V_19 ,
L_1 , V_20 , V_3 [ 0 ] ) ;
return 0 ;
}
V_23 = V_3 [ 1 ] & 0x40 ;
if ( V_23 ) {
V_2 -> V_9 [ 0 ] = V_10 ;
V_24 = ( signed char ) ( ( V_3 [ 7 ] << 1 ) | ( ( V_3 [ 4 ] >> 2 ) & 1 ) ) ;
if ( V_22 -> V_26 > 255 )
V_24 = ( V_24 << 1 ) | ( ( V_3 [ 4 ] >> 6 ) & 1 ) ;
V_24 += ( V_22 -> V_26 + 1 ) / 2 ;
if ( ! V_2 -> V_6 [ 0 ] ) {
if ( V_3 [ 1 ] & 0x10 )
V_2 -> V_6 [ 1 ] = V_7 ;
else
V_2 -> V_6 [ 1 ] = ( V_3 [ 4 ] & 0x20 ) ? V_7 : V_8 ;
} else {
if ( V_2 -> V_6 [ 1 ] == V_7 && ! ( V_3 [ 4 ] & 0x20 ) ) {
F_2 ( V_5 , V_2 -> V_6 [ 1 ] , 0 ) ;
F_7 ( V_5 ) ;
V_2 -> V_6 [ 1 ] = V_8 ;
return 0 ;
}
}
if ( V_2 -> V_6 [ 1 ] != V_7 ) {
V_2 -> V_6 [ 1 ] = V_8 ;
V_2 -> V_9 [ 0 ] = V_11 ;
}
F_2 ( V_5 , V_2 -> V_6 [ 1 ] , V_23 ) ;
F_3 ( V_5 , V_12 , V_2 -> V_9 [ 0 ] ) ;
F_3 ( V_5 , V_13 , V_3 [ 3 ] | ( V_3 [ 2 ] << 7 ) | ( ( V_3 [ 1 ] & 0x03 ) << 14 ) ) ;
F_3 ( V_5 , V_14 , V_3 [ 6 ] | ( V_3 [ 5 ] << 7 ) | ( ( V_3 [ 4 ] & 0x03 ) << 14 ) ) ;
F_3 ( V_5 , V_15 , V_24 ) ;
F_2 ( V_5 , V_16 , V_3 [ 4 ] & 0x08 ) ;
F_2 ( V_5 , V_17 , V_3 [ 4 ] & 0x10 ) ;
F_2 ( V_5 , V_27 , ( V_2 -> V_6 [ 1 ] == V_8 ) && ( V_3 [ 4 ] & 0x20 ) ) ;
} else {
if ( V_2 -> V_6 [ 1 ] != V_7 ) {
V_2 -> V_6 [ 1 ] = V_8 ;
}
F_2 ( V_5 , V_2 -> V_6 [ 1 ] , V_23 ) ;
}
V_2 -> V_6 [ 0 ] = V_23 ;
return 1 ;
}
static int F_8 ( struct V_1 * V_2 )
{
unsigned char * V_3 = V_2 -> V_3 ;
struct V_4 * V_5 = V_2 -> V_5 ;
if ( V_3 [ 0 ] != V_25 ) {
F_5 ( V_5 -> V_18 . V_19 ,
L_1 , V_20 , V_3 [ 0 ] ) ;
return 0 ;
}
if ( V_3 [ 1 ] & 0x04 ) {
F_2 ( V_5 , V_7 , V_3 [ 1 ] & 0x20 ) ;
F_2 ( V_5 , V_16 , V_3 [ 1 ] & 0x08 ) ;
V_2 -> V_9 [ 0 ] = V_10 ;
} else {
F_2 ( V_5 , V_8 , V_3 [ 1 ] & 0x20 ) ;
F_2 ( V_5 , V_16 , V_3 [ 1 ] & 0x01 ) ;
V_2 -> V_9 [ 0 ] = V_11 ;
}
F_3 ( V_5 , V_12 , V_2 -> V_9 [ 0 ] ) ;
F_3 ( V_5 , V_13 , F_9 ( ( V_28 * ) & V_3 [ 2 ] ) ) ;
F_3 ( V_5 , V_14 , F_9 ( ( V_28 * ) & V_3 [ 4 ] ) ) ;
F_3 ( V_5 , V_15 , F_9 ( ( V_28 * ) & V_3 [ 6 ] ) ) ;
F_2 ( V_5 , V_17 , V_3 [ 1 ] & 0x02 ) ;
F_2 ( V_5 , V_27 , V_3 [ 1 ] & 0x10 ) ;
return 1 ;
}
static int F_10 ( struct V_1 * V_2 )
{
struct V_21 * V_22 = & V_2 -> V_22 ;
char * V_3 = V_2 -> V_3 ;
struct V_4 * V_5 = V_2 -> V_5 ;
int V_23 = V_3 [ 1 ] & 0x20 , V_24 ;
F_5 ( V_5 -> V_18 . V_19 ,
L_2 , V_20 , V_3 [ 0 ] ) ;
if ( V_23 ) {
V_2 -> V_6 [ 0 ] = ( V_3 [ 1 ] & 0x0c ) ? V_7 : V_8 ;
if ( V_2 -> V_6 [ 0 ] == V_8 )
V_2 -> V_9 [ 0 ] = V_11 ;
else
V_2 -> V_9 [ 0 ] = V_10 ;
}
F_2 ( V_5 , V_17 , V_3 [ 1 ] & 0x02 ) ;
F_2 ( V_5 , V_27 , V_3 [ 1 ] & 0x10 ) ;
F_3 ( V_5 , V_13 , F_9 ( ( V_28 * ) & V_3 [ 2 ] ) ) ;
F_3 ( V_5 , V_14 , F_9 ( ( V_28 * ) & V_3 [ 4 ] ) ) ;
V_24 = ( ( V_3 [ 7 ] & 0x01 ) << 8 ) | V_3 [ 6 ] ;
if ( V_24 < 0 )
V_24 = V_22 -> V_26 + V_24 + 1 ;
F_3 ( V_5 , V_15 , V_24 ) ;
F_2 ( V_5 , V_16 , V_3 [ 1 ] & 0x05 ) ;
if ( ! V_23 )
V_2 -> V_9 [ 0 ] = 0 ;
F_2 ( V_5 , V_2 -> V_6 [ 0 ] , V_23 ) ;
F_3 ( V_5 , V_12 , V_2 -> V_9 [ 0 ] ) ;
return 1 ;
}
static int F_11 ( struct V_1 * V_2 )
{
struct V_21 * V_22 = & V_2 -> V_22 ;
unsigned char * V_3 = V_2 -> V_3 ;
struct V_4 * V_5 = V_2 -> V_5 ;
int V_23 ;
int V_29 = 0 ;
int V_30 = 0 ;
if ( V_3 [ 0 ] != V_25 ) {
F_5 ( V_5 -> V_18 . V_19 ,
L_1 , V_20 , V_3 [ 0 ] ) ;
goto exit;
}
V_23 = V_3 [ 1 ] & 0x80 ;
if ( V_23 || V_2 -> V_9 [ 0 ] ) {
if ( V_23 ) {
switch ( ( V_3 [ 1 ] >> 5 ) & 3 ) {
case 0 :
V_2 -> V_6 [ 0 ] = V_8 ;
V_2 -> V_9 [ 0 ] = V_11 ;
break;
case 1 :
V_2 -> V_6 [ 0 ] = V_7 ;
V_2 -> V_9 [ 0 ] = V_10 ;
break;
case 2 :
F_2 ( V_5 , V_31 , V_3 [ 1 ] & 0x04 ) ;
case 3 :
V_2 -> V_6 [ 0 ] = V_32 ;
V_2 -> V_9 [ 0 ] = V_33 ;
break;
}
}
F_3 ( V_5 , V_13 , F_9 ( ( V_28 * ) & V_3 [ 2 ] ) ) ;
F_3 ( V_5 , V_14 , F_9 ( ( V_28 * ) & V_3 [ 4 ] ) ) ;
if ( V_2 -> V_6 [ 0 ] != V_32 ) {
F_3 ( V_5 , V_15 , V_3 [ 6 ] | ( ( V_3 [ 7 ] & 0x03 ) << 8 ) ) ;
F_2 ( V_5 , V_16 , V_3 [ 1 ] & 0x01 ) ;
F_2 ( V_5 , V_17 , V_3 [ 1 ] & 0x02 ) ;
F_2 ( V_5 , V_27 , V_3 [ 1 ] & 0x04 ) ;
} else {
F_2 ( V_5 , V_34 , V_3 [ 1 ] & 0x01 ) ;
F_2 ( V_5 , V_35 , V_3 [ 1 ] & 0x02 ) ;
if ( V_22 -> type == V_36 ||
V_22 -> type == V_37 ) {
F_3 ( V_5 , V_38 , V_3 [ 6 ] & 0x3f ) ;
V_29 = ( V_3 [ 7 ] & 0x04 ) - ( V_3 [ 7 ] & 0x03 ) ;
} else {
F_3 ( V_5 , V_38 , V_3 [ 7 ] & 0x3f ) ;
V_29 = - ( signed char ) V_3 [ 6 ] ;
}
F_12 ( V_5 , V_39 , V_29 ) ;
}
if ( ! V_23 )
V_2 -> V_9 [ 0 ] = 0 ;
F_3 ( V_5 , V_12 , V_2 -> V_9 [ 0 ] ) ;
F_2 ( V_5 , V_2 -> V_6 [ 0 ] , V_23 ) ;
F_13 ( V_5 , V_40 , V_41 , 1 ) ;
F_7 ( V_5 ) ;
}
switch ( V_22 -> type ) {
case V_36 :
V_23 = V_3 [ 7 ] & 0xf8 ;
if ( V_23 || V_2 -> V_9 [ 1 ] ) {
V_2 -> V_9 [ 1 ] = V_42 ;
F_2 ( V_5 , V_43 , ( V_3 [ 7 ] & 0x40 ) ) ;
F_2 ( V_5 , V_44 , ( V_3 [ 7 ] & 0x80 ) ) ;
V_29 = ( ( V_3 [ 7 ] & 0x18 ) >> 3 ) - ( ( V_3 [ 7 ] & 0x20 ) >> 3 ) ;
F_12 ( V_5 , V_39 , V_29 ) ;
if ( ! V_23 )
V_2 -> V_9 [ 1 ] = 0 ;
F_3 ( V_5 , V_12 , V_2 -> V_9 [ 1 ] ) ;
F_13 ( V_5 , V_40 , V_41 , 0xf0 ) ;
V_30 = 1 ;
}
break;
case V_37 :
V_23 = ( V_3 [ 7 ] & 0xf8 ) || V_3 [ 8 ] ;
if ( V_23 || V_2 -> V_9 [ 1 ] ) {
V_2 -> V_9 [ 1 ] = V_42 ;
F_2 ( V_5 , V_43 , ( V_3 [ 7 ] & 0x08 ) ) ;
F_2 ( V_5 , V_34 , ( V_3 [ 7 ] & 0x20 ) ) ;
F_2 ( V_5 , V_44 , ( V_3 [ 7 ] & 0x10 ) ) ;
F_2 ( V_5 , V_35 , ( V_3 [ 7 ] & 0x40 ) ) ;
F_3 ( V_5 , V_45 , ( V_3 [ 8 ] & 0x7f ) ) ;
if ( ! V_23 )
V_2 -> V_9 [ 1 ] = 0 ;
F_3 ( V_5 , V_12 , V_2 -> V_9 [ 1 ] ) ;
F_13 ( V_5 , V_40 , V_41 , 0xf0 ) ;
V_30 = 1 ;
}
break;
}
exit:
return V_30 ;
}
static int F_14 ( struct V_1 * V_2 )
{
struct V_21 * V_22 = & V_2 -> V_22 ;
unsigned char * V_3 = V_2 -> V_3 ;
struct V_4 * V_5 = V_2 -> V_5 ;
int V_46 = 0 ;
if ( V_22 -> type == V_47 )
V_46 = V_3 [ 1 ] & 0x01 ;
if ( ( V_3 [ 1 ] & 0xfc ) == 0xc0 ) {
if ( V_22 -> V_48 == V_49 )
V_2 -> V_50 -> V_51 = true ;
V_2 -> V_52 [ V_46 ] = ( ( V_3 [ 3 ] & 0x0f ) << 28 ) +
( V_3 [ 4 ] << 20 ) + ( V_3 [ 5 ] << 12 ) +
( V_3 [ 6 ] << 4 ) + ( V_3 [ 7 ] >> 4 ) ;
V_2 -> V_9 [ V_46 ] = ( V_3 [ 2 ] << 4 ) | ( V_3 [ 3 ] >> 4 ) |
( ( V_3 [ 7 ] & 0x0f ) << 20 ) | ( ( V_3 [ 8 ] & 0xf0 ) << 12 ) ;
switch ( V_2 -> V_9 [ V_46 ] ) {
case 0x812 :
case 0x801 :
case 0x120802 :
case 0x012 :
V_2 -> V_6 [ V_46 ] = V_53 ;
break;
case 0x822 :
case 0x842 :
case 0x852 :
case 0x823 :
case 0x813 :
case 0x885 :
case 0x802 :
case 0x804 :
case 0x022 :
case 0x100804 :
case 0x140802 :
case 0x160802 :
case 0x180802 :
case 0x100802 :
V_2 -> V_6 [ V_46 ] = V_8 ;
break;
case 0x832 :
case 0x032 :
V_2 -> V_6 [ V_46 ] = V_54 ;
break;
case 0x007 :
case 0x09c :
case 0x094 :
case 0x017 :
case 0x806 :
V_2 -> V_6 [ V_46 ] = V_32 ;
break;
case 0x096 :
case 0x097 :
case 0x006 :
V_2 -> V_6 [ V_46 ] = V_55 ;
break;
case 0x82a :
case 0x85a :
case 0x91a :
case 0xd1a :
case 0x0fa :
case 0x82b :
case 0x81b :
case 0x91b :
case 0x80c :
case 0x80a :
case 0x90a :
case 0x14080a :
case 0x10090a :
case 0x10080c :
case 0x16080a :
case 0x18080a :
case 0x10080a :
V_2 -> V_6 [ V_46 ] = V_7 ;
break;
case 0xd12 :
case 0x912 :
case 0x112 :
case 0x913 :
case 0x902 :
case 0x100902 :
V_2 -> V_6 [ V_46 ] = V_56 ;
break;
default:
V_2 -> V_6 [ V_46 ] = V_8 ;
break;
}
return 1 ;
}
if ( ! ( ( V_2 -> V_9 [ V_46 ] >> 20 ) & 0x01 ) &&
( V_22 -> type == V_57 ) )
return 1 ;
if ( ( V_3 [ 1 ] & 0xfe ) == 0x80 ) {
if ( V_22 -> V_48 == V_49 )
V_2 -> V_50 -> V_51 = false ;
F_3 ( V_5 , V_13 , 0 ) ;
F_3 ( V_5 , V_14 , 0 ) ;
F_3 ( V_5 , V_38 , 0 ) ;
F_3 ( V_5 , V_58 , 0 ) ;
F_3 ( V_5 , V_59 , 0 ) ;
if ( V_2 -> V_6 [ V_46 ] >= V_32 ) {
F_2 ( V_5 , V_34 , 0 ) ;
F_2 ( V_5 , V_31 , 0 ) ;
F_2 ( V_5 , V_35 , 0 ) ;
F_2 ( V_5 , V_60 , 0 ) ;
F_2 ( V_5 , V_61 , 0 ) ;
F_3 ( V_5 , V_62 , 0 ) ;
F_3 ( V_5 , V_63 , 0 ) ;
} else {
F_3 ( V_5 , V_15 , 0 ) ;
F_2 ( V_5 , V_17 , 0 ) ;
F_2 ( V_5 , V_27 , 0 ) ;
F_2 ( V_5 , V_16 , 0 ) ;
F_3 ( V_5 , V_45 , 0 ) ;
if ( V_22 -> type >= V_64 )
F_3 ( V_5 , V_65 , 0 ) ;
}
F_2 ( V_5 , V_2 -> V_6 [ V_46 ] , 0 ) ;
F_3 ( V_5 , V_12 , 0 ) ;
F_13 ( V_5 , V_40 , V_41 , V_2 -> V_52 [ V_46 ] ) ;
V_2 -> V_9 [ V_46 ] = 0 ;
return 2 ;
}
return 0 ;
}
static void F_15 ( struct V_1 * V_2 )
{
struct V_21 * V_22 = & V_2 -> V_22 ;
unsigned char * V_3 = V_2 -> V_3 ;
struct V_4 * V_5 = V_2 -> V_5 ;
unsigned int V_66 ;
if ( ( V_3 [ 1 ] & 0xb8 ) == 0xa0 ) {
V_66 = ( V_3 [ 6 ] << 2 ) | ( ( V_3 [ 7 ] >> 6 ) & 3 ) ;
if ( V_22 -> type >= V_67 && V_22 -> type <= V_68 ) {
V_66 = ( V_66 << 1 ) | ( V_3 [ 1 ] & 1 ) ;
}
F_3 ( V_5 , V_15 , V_66 ) ;
F_3 ( V_5 , V_58 ,
( ( V_3 [ 7 ] << 1 ) & 0x7e ) | ( V_3 [ 8 ] >> 7 ) ) ;
F_3 ( V_5 , V_59 , V_3 [ 8 ] & 0x7f ) ;
F_2 ( V_5 , V_17 , V_3 [ 1 ] & 2 ) ;
F_2 ( V_5 , V_27 , V_3 [ 1 ] & 4 ) ;
F_2 ( V_5 , V_16 , V_66 > 10 ) ;
}
if ( ( V_3 [ 1 ] & 0xbc ) == 0xb4 ) {
F_3 ( V_5 , V_45 ,
( V_3 [ 6 ] << 2 ) | ( ( V_3 [ 7 ] >> 6 ) & 3 ) ) ;
F_3 ( V_5 , V_58 ,
( ( V_3 [ 7 ] << 1 ) & 0x7e ) | ( V_3 [ 8 ] >> 7 ) ) ;
F_3 ( V_5 , V_59 , V_3 [ 8 ] & 0x7f ) ;
}
}
static int F_16 ( struct V_1 * V_2 )
{
struct V_21 * V_22 = & V_2 -> V_22 ;
unsigned char * V_3 = V_2 -> V_3 ;
struct V_4 * V_5 = V_2 -> V_5 ;
unsigned int V_66 ;
int V_46 = 0 , V_69 ;
if ( V_3 [ 0 ] != V_25 &&
V_3 [ 0 ] != V_70 &&
V_3 [ 0 ] != V_71 &&
V_3 [ 0 ] != V_72 &&
V_3 [ 0 ] != V_73 ) {
F_5 ( V_5 -> V_18 . V_19 ,
L_1 , V_20 , V_3 [ 0 ] ) ;
return 0 ;
}
if ( V_22 -> type == V_47 )
V_46 = V_3 [ 1 ] & 0x01 ;
if ( V_3 [ 0 ] == V_72 || V_3 [ 0 ] == V_73 ) {
if ( V_22 -> type >= V_67 && V_22 -> type <= V_74 ) {
F_2 ( V_5 , V_75 , ( V_3 [ 2 ] & 0x01 ) ) ;
F_2 ( V_5 , V_76 , ( V_3 [ 3 ] & 0x01 ) ) ;
F_2 ( V_5 , V_77 , ( V_3 [ 3 ] & 0x02 ) ) ;
F_2 ( V_5 , V_78 , ( V_3 [ 3 ] & 0x04 ) ) ;
F_2 ( V_5 , V_79 , ( V_3 [ 3 ] & 0x08 ) ) ;
F_2 ( V_5 , V_80 , ( V_3 [ 3 ] & 0x10 ) ) ;
F_2 ( V_5 , V_81 , ( V_3 [ 3 ] & 0x20 ) ) ;
if ( V_3 [ 1 ] & 0x80 ) {
F_3 ( V_5 , V_45 , ( V_3 [ 1 ] & 0x7f ) ) ;
} else {
F_3 ( V_5 , V_45 , 0 ) ;
}
if ( V_22 -> type != V_67 ) {
F_2 ( V_5 , V_82 , ( V_3 [ 3 ] & 0x40 ) ) ;
F_2 ( V_5 , V_83 , ( V_3 [ 3 ] & 0x80 ) ) ;
}
if ( V_3 [ 1 ] | ( V_3 [ 2 ] & 0x01 ) | V_3 [ 3 ] ) {
F_3 ( V_5 , V_12 , V_42 ) ;
} else {
F_3 ( V_5 , V_12 , 0 ) ;
}
} else if ( V_22 -> type == V_84 ) {
F_2 ( V_5 , V_75 , ( V_3 [ 6 ] & 0x01 ) ) ;
F_2 ( V_5 , V_76 , ( V_3 [ 6 ] & 0x02 ) ) ;
F_2 ( V_5 , V_77 , ( V_3 [ 6 ] & 0x04 ) ) ;
F_2 ( V_5 , V_78 , ( V_3 [ 6 ] & 0x08 ) ) ;
F_2 ( V_5 , V_79 , ( V_3 [ 6 ] & 0x10 ) ) ;
F_2 ( V_5 , V_80 , ( V_3 [ 6 ] & 0x20 ) ) ;
if ( V_3 [ 6 ] & 0x3f ) {
F_3 ( V_5 , V_12 , V_42 ) ;
} else {
F_3 ( V_5 , V_12 , 0 ) ;
}
} else if ( V_22 -> type == V_85 ) {
F_2 ( V_5 , V_75 , ( V_3 [ 3 ] & 0x01 ) ) ;
F_2 ( V_5 , V_76 , ( V_3 [ 4 ] & 0x01 ) ) ;
F_2 ( V_5 , V_77 , ( V_3 [ 4 ] & 0x02 ) ) ;
F_2 ( V_5 , V_78 , ( V_3 [ 4 ] & 0x04 ) ) ;
F_2 ( V_5 , V_79 , ( V_3 [ 4 ] & 0x08 ) ) ;
F_2 ( V_5 , V_80 , ( V_3 [ 4 ] & 0x10 ) ) ;
F_2 ( V_5 , V_81 , ( V_3 [ 4 ] & 0x20 ) ) ;
F_2 ( V_5 , V_82 , ( V_3 [ 4 ] & 0x40 ) ) ;
F_2 ( V_5 , V_83 , ( V_3 [ 4 ] & 0x80 ) ) ;
if ( ( V_3 [ 3 ] & 0x01 ) | V_3 [ 4 ] ) {
F_3 ( V_5 , V_12 , V_42 ) ;
} else {
F_3 ( V_5 , V_12 , 0 ) ;
}
} else if ( V_22 -> type == V_68 ) {
F_2 ( V_5 , V_75 , ( V_3 [ 6 ] & 0x01 ) ) ;
F_2 ( V_5 , V_76 , ( V_3 [ 6 ] & 0x02 ) ) ;
F_2 ( V_5 , V_77 , ( V_3 [ 6 ] & 0x04 ) ) ;
F_2 ( V_5 , V_78 , ( V_3 [ 6 ] & 0x08 ) ) ;
F_2 ( V_5 , V_79 , ( V_3 [ 6 ] & 0x10 ) ) ;
F_2 ( V_5 , V_80 , ( V_3 [ 6 ] & 0x20 ) ) ;
F_2 ( V_5 , V_81 , ( V_3 [ 6 ] & 0x40 ) ) ;
F_2 ( V_5 , V_82 , ( V_3 [ 6 ] & 0x80 ) ) ;
F_2 ( V_5 , V_83 , ( V_3 [ 8 ] & 0x01 ) ) ;
F_2 ( V_5 , V_86 , ( V_3 [ 8 ] & 0x02 ) ) ;
F_2 ( V_5 , V_87 , ( V_3 [ 8 ] & 0x04 ) ) ;
F_2 ( V_5 , V_88 , ( V_3 [ 8 ] & 0x08 ) ) ;
F_2 ( V_5 , V_89 , ( V_3 [ 8 ] & 0x10 ) ) ;
F_2 ( V_5 , V_90 , ( V_3 [ 8 ] & 0x20 ) ) ;
F_2 ( V_5 , V_91 , ( V_3 [ 8 ] & 0x40 ) ) ;
F_2 ( V_5 , V_92 , ( V_3 [ 8 ] & 0x80 ) ) ;
F_2 ( V_5 , V_93 , V_3 [ 4 ] & 0x07 ) ;
F_2 ( V_5 , V_94 , V_3 [ 4 ] & 0xE0 ) ;
F_2 ( V_5 , V_95 , V_3 [ 3 ] & 0x1C ) ;
if ( V_3 [ 1 ] & 0x80 ) {
F_3 ( V_5 , V_45 , ( V_3 [ 1 ] & 0x7f ) ) ;
} else {
F_3 ( V_5 , V_45 , 0 ) ;
}
if ( V_3 [ 2 ] & 0x80 ) {
F_3 ( V_5 , V_62 , ( V_3 [ 2 ] & 0x7f ) ) ;
} else {
F_3 ( V_5 , V_62 , 0 ) ;
}
if ( V_3 [ 1 ] | V_3 [ 2 ] | ( V_3 [ 3 ] & 0x1f ) | V_3 [ 4 ] | V_3 [ 6 ] | V_3 [ 8 ] ) {
F_3 ( V_5 , V_12 , V_42 ) ;
} else {
F_3 ( V_5 , V_12 , 0 ) ;
}
} else if ( V_22 -> type >= V_96 && V_22 -> type <= V_97 ) {
int V_98 ;
F_2 ( V_5 , V_75 , ( V_3 [ 3 ] & 0x01 ) ) ;
for ( V_98 = 0 ; V_98 < 8 ; V_98 ++ )
F_2 ( V_5 , V_76 + V_98 , V_3 [ 4 ] & ( 1 << V_98 ) ) ;
if ( V_3 [ 2 ] & 0x80 ) {
F_3 ( V_5 , V_45 , ( V_3 [ 2 ] & 0x7f ) ) ;
} else {
F_3 ( V_5 , V_45 , 0 ) ;
}
if ( V_3 [ 2 ] | ( V_3 [ 3 ] & 0x01 ) | V_3 [ 4 ] | V_3 [ 5 ] ) {
F_3 ( V_5 , V_12 , V_42 ) ;
} else {
F_3 ( V_5 , V_12 , 0 ) ;
}
} else {
if ( V_22 -> type == V_57 || V_22 -> type == V_99 ) {
F_2 ( V_5 , V_75 , ( V_3 [ 5 ] & 0x01 ) ) ;
F_2 ( V_5 , V_76 , ( V_3 [ 6 ] & 0x01 ) ) ;
F_2 ( V_5 , V_77 , ( V_3 [ 6 ] & 0x02 ) ) ;
F_2 ( V_5 , V_78 , ( V_3 [ 6 ] & 0x04 ) ) ;
F_2 ( V_5 , V_79 , ( V_3 [ 6 ] & 0x08 ) ) ;
F_2 ( V_5 , V_80 , ( V_3 [ 6 ] & 0x10 ) ) ;
F_2 ( V_5 , V_81 , ( V_3 [ 6 ] & 0x20 ) ) ;
F_2 ( V_5 , V_82 , ( V_3 [ 6 ] & 0x40 ) ) ;
F_2 ( V_5 , V_83 , ( V_3 [ 6 ] & 0x80 ) ) ;
F_2 ( V_5 , V_86 , ( V_3 [ 7 ] & 0x01 ) ) ;
F_2 ( V_5 , V_87 , ( V_3 [ 8 ] & 0x01 ) ) ;
F_2 ( V_5 , V_88 , ( V_3 [ 8 ] & 0x02 ) ) ;
F_2 ( V_5 , V_89 , ( V_3 [ 8 ] & 0x04 ) ) ;
F_2 ( V_5 , V_90 , ( V_3 [ 8 ] & 0x08 ) ) ;
F_2 ( V_5 , V_91 , ( V_3 [ 8 ] & 0x10 ) ) ;
F_2 ( V_5 , V_92 , ( V_3 [ 8 ] & 0x20 ) ) ;
F_2 ( V_5 , V_100 , ( V_3 [ 8 ] & 0x40 ) ) ;
F_2 ( V_5 , V_101 , ( V_3 [ 8 ] & 0x80 ) ) ;
if ( V_22 -> type == V_99 ) {
F_2 ( V_5 , V_93 , V_3 [ 9 ] & 0x01 ) ;
F_2 ( V_5 , V_94 , V_3 [ 9 ] & 0x02 ) ;
F_2 ( V_5 , V_95 , V_3 [ 9 ] & 0x04 ) ;
}
} else {
F_2 ( V_5 , V_75 , ( V_3 [ 5 ] & 0x01 ) ) ;
F_2 ( V_5 , V_76 , ( V_3 [ 5 ] & 0x02 ) ) ;
F_2 ( V_5 , V_77 , ( V_3 [ 5 ] & 0x04 ) ) ;
F_2 ( V_5 , V_78 , ( V_3 [ 5 ] & 0x08 ) ) ;
F_2 ( V_5 , V_79 , ( V_3 [ 6 ] & 0x01 ) ) ;
F_2 ( V_5 , V_80 , ( V_3 [ 6 ] & 0x02 ) ) ;
F_2 ( V_5 , V_81 , ( V_3 [ 6 ] & 0x04 ) ) ;
F_2 ( V_5 , V_82 , ( V_3 [ 6 ] & 0x08 ) ) ;
F_2 ( V_5 , V_83 , ( V_3 [ 5 ] & 0x10 ) ) ;
F_2 ( V_5 , V_86 , ( V_3 [ 6 ] & 0x10 ) ) ;
}
F_3 ( V_5 , V_102 , ( ( V_3 [ 1 ] & 0x1f ) << 8 ) | V_3 [ 2 ] ) ;
F_3 ( V_5 , V_103 , ( ( V_3 [ 3 ] & 0x1f ) << 8 ) | V_3 [ 4 ] ) ;
if ( ( V_3 [ 5 ] & 0x1f ) | V_3 [ 6 ] | ( V_3 [ 1 ] & 0x1f ) |
V_3 [ 2 ] | ( V_3 [ 3 ] & 0x1f ) | V_3 [ 4 ] | V_3 [ 8 ] |
( V_3 [ 7 ] & 0x01 ) ) {
F_3 ( V_5 , V_12 , V_42 ) ;
} else {
F_3 ( V_5 , V_12 , 0 ) ;
}
}
F_13 ( V_5 , V_40 , V_41 , 0xffffffff ) ;
return 1 ;
}
V_69 = F_14 ( V_2 ) ;
if ( V_69 )
return V_69 - 1 ;
if ( ! V_2 -> V_9 [ V_46 ] )
return 0 ;
if ( V_2 -> V_6 [ V_46 ] == V_55 &&
( V_22 -> type == V_104 ||
V_22 -> type == V_64 ||
V_22 -> type == V_105 ||
V_22 -> type == V_67 ||
V_22 -> type == V_106 ||
V_22 -> type == V_96 ) ) {
return 0 ;
}
if ( V_22 -> type == V_107 && ! ( V_3 [ 1 ] & 0x40 ) )
return 0 ;
if ( V_22 -> type >= V_64 ) {
F_3 ( V_5 , V_13 , ( V_3 [ 2 ] << 9 ) | ( V_3 [ 3 ] << 1 ) | ( ( V_3 [ 9 ] >> 1 ) & 1 ) ) ;
F_3 ( V_5 , V_14 , ( V_3 [ 4 ] << 9 ) | ( V_3 [ 5 ] << 1 ) | ( V_3 [ 9 ] & 1 ) ) ;
F_3 ( V_5 , V_38 , ( ( V_3 [ 9 ] >> 2 ) & 0x3f ) ) ;
} else {
F_3 ( V_5 , V_13 , F_17 ( ( V_108 * ) & V_3 [ 2 ] ) ) ;
F_3 ( V_5 , V_14 , F_17 ( ( V_108 * ) & V_3 [ 4 ] ) ) ;
F_3 ( V_5 , V_38 , ( ( V_3 [ 9 ] >> 3 ) & 0x1f ) ) ;
}
F_15 ( V_2 ) ;
if ( ( V_3 [ 1 ] & 0xbc ) == 0xa8 || ( V_3 [ 1 ] & 0xbe ) == 0xb0 || ( V_3 [ 1 ] & 0xbc ) == 0xac ) {
if ( V_3 [ 1 ] & 0x02 ) {
if ( V_22 -> type >= V_64 ) {
V_66 = ( V_3 [ 6 ] << 3 ) | ( ( V_3 [ 7 ] >> 5 ) & 7 ) ;
V_66 = ( V_3 [ 7 ] & 0x20 ) ? ( ( V_66 > 900 ) ? ( ( V_66 - 1 ) / 2 - 1350 ) :
( ( V_66 - 1 ) / 2 + 450 ) ) : ( 450 - V_66 / 2 ) ;
F_3 ( V_5 , V_65 , V_66 ) ;
} else {
V_66 = ( V_3 [ 6 ] << 3 ) | ( ( V_3 [ 7 ] >> 5 ) & 7 ) ;
F_3 ( V_5 , V_63 , ( V_3 [ 7 ] & 0x20 ) ?
( ( V_66 - 1 ) / 2 ) : - V_66 / 2 ) ;
}
} else if ( ! ( V_3 [ 1 ] & 0x10 ) && V_22 -> type < V_64 ) {
F_2 ( V_5 , V_34 , V_3 [ 8 ] & 0x01 ) ;
F_2 ( V_5 , V_31 , V_3 [ 8 ] & 0x02 ) ;
F_2 ( V_5 , V_35 , V_3 [ 8 ] & 0x04 ) ;
F_2 ( V_5 , V_60 , V_3 [ 8 ] & 0x20 ) ;
F_2 ( V_5 , V_61 , V_3 [ 8 ] & 0x10 ) ;
V_66 = ( V_3 [ 6 ] << 2 ) | ( ( V_3 [ 7 ] >> 6 ) & 3 ) ;
F_3 ( V_5 , V_62 , ( V_3 [ 8 ] & 0x08 ) ? - V_66 : V_66 ) ;
} else if ( V_2 -> V_6 [ V_46 ] == V_32 ) {
if ( ( V_22 -> type >= V_67 && V_22 -> type <= V_74 ) ||
( V_22 -> type >= V_96 && V_22 -> type <= V_97 ) ) {
F_2 ( V_5 , V_34 , V_3 [ 6 ] & 0x01 ) ;
F_2 ( V_5 , V_31 , V_3 [ 6 ] & 0x02 ) ;
F_2 ( V_5 , V_35 , V_3 [ 6 ] & 0x04 ) ;
F_12 ( V_5 , V_39 , ( ( V_3 [ 7 ] & 0x80 ) >> 7 )
- ( ( V_3 [ 7 ] & 0x40 ) >> 6 ) ) ;
F_2 ( V_5 , V_60 , V_3 [ 6 ] & 0x08 ) ;
F_2 ( V_5 , V_61 , V_3 [ 6 ] & 0x10 ) ;
F_3 ( V_5 , V_58 ,
( ( V_3 [ 7 ] << 1 ) & 0x7e ) | ( V_3 [ 8 ] >> 7 ) ) ;
F_3 ( V_5 , V_59 , V_3 [ 8 ] & 0x7f ) ;
} else {
F_2 ( V_5 , V_34 , V_3 [ 8 ] & 0x04 ) ;
F_2 ( V_5 , V_31 , V_3 [ 8 ] & 0x08 ) ;
F_2 ( V_5 , V_35 , V_3 [ 8 ] & 0x10 ) ;
F_12 ( V_5 , V_39 , ( V_3 [ 8 ] & 0x01 )
- ( ( V_3 [ 8 ] & 0x02 ) >> 1 ) ) ;
if ( V_22 -> type >= V_64 && V_22 -> type <= V_109 ) {
F_2 ( V_5 , V_60 , V_3 [ 8 ] & 0x40 ) ;
F_2 ( V_5 , V_61 , V_3 [ 8 ] & 0x20 ) ;
}
}
} else if ( ( V_22 -> type < V_64 || V_22 -> type == V_109 ||
V_22 -> type == V_74 || V_22 -> type == V_97 ) &&
V_2 -> V_6 [ V_46 ] == V_55 ) {
F_2 ( V_5 , V_34 , V_3 [ 8 ] & 0x01 ) ;
F_2 ( V_5 , V_31 , V_3 [ 8 ] & 0x02 ) ;
F_2 ( V_5 , V_35 , V_3 [ 8 ] & 0x04 ) ;
F_2 ( V_5 , V_60 , V_3 [ 8 ] & 0x10 ) ;
F_2 ( V_5 , V_61 , V_3 [ 8 ] & 0x08 ) ;
}
}
F_3 ( V_5 , V_12 , V_2 -> V_9 [ V_46 ] ) ;
F_2 ( V_5 , V_2 -> V_6 [ V_46 ] , 1 ) ;
F_13 ( V_5 , V_40 , V_41 , V_2 -> V_52 [ V_46 ] ) ;
return 1 ;
}
static int F_18 ( int V_110 , int y1 , int V_111 , int V_112 )
{
int V_113 = V_111 - V_110 ;
int V_114 = V_112 - y1 ;
return F_19 ( V_113 * V_113 + V_114 * V_114 ) ;
}
static int F_20 ( struct V_1 * V_2 )
{
struct V_4 * V_5 = V_2 -> V_5 ;
char * V_3 = V_2 -> V_3 ;
int V_98 ;
int V_115 = V_3 [ 61 ] ;
int V_116 = 0 ;
if ( V_115 )
V_2 -> V_117 = V_115 ;
V_116 = F_21 ( 4 , V_2 -> V_117 ) ;
for ( V_98 = 0 ; V_98 < V_116 ; V_98 ++ ) {
int V_118 = ( V_119 * V_98 ) + 1 ;
bool V_120 = V_3 [ V_118 ] & 0x1 && ! V_2 -> V_50 -> V_51 ;
int V_121 = F_22 ( V_5 , V_3 [ V_118 + 1 ] ) ;
if ( V_121 < 0 )
continue;
F_23 ( V_5 , V_121 ) ;
F_24 ( V_5 , V_122 , V_120 ) ;
if ( V_120 ) {
int V_123 = F_9 ( ( V_28 * ) & V_3 [ V_118 + 2 ] ) ;
int V_124 = F_9 ( ( V_28 * ) & V_3 [ V_118 + 4 ] ) ;
int V_125 = F_9 ( ( V_28 * ) & V_3 [ V_118 + 6 ] ) ;
int V_126 = F_9 ( ( V_28 * ) & V_3 [ V_118 + 8 ] ) ;
int V_127 = F_9 ( ( V_28 * ) & V_3 [ V_118 + 10 ] ) ;
int V_128 = F_9 ( ( V_28 * ) & V_3 [ V_118 + 12 ] ) ;
F_3 ( V_5 , V_129 , V_123 ) ;
F_3 ( V_5 , V_130 , V_125 ) ;
F_3 ( V_5 , V_131 , F_21 ( V_127 , V_128 ) ) ;
F_3 ( V_5 , V_132 , F_21 ( V_127 , V_128 ) + F_18 ( V_123 , V_125 , V_124 , V_126 ) ) ;
F_3 ( V_5 , V_133 , F_21 ( V_127 , V_128 ) ) ;
F_3 ( V_5 , V_134 , V_127 > V_128 ) ;
}
}
F_25 ( V_5 , true ) ;
V_2 -> V_117 -= V_116 ;
if ( V_2 -> V_117 <= 0 )
V_2 -> V_117 = 0 ;
return 1 ;
}
static int F_26 ( struct V_1 * V_2 )
{
struct V_4 * V_5 = V_2 -> V_5 ;
char * V_3 = V_2 -> V_3 ;
int V_98 ;
int V_115 = V_3 [ 2 ] ;
int V_116 = 0 ;
int V_135 = 0 ;
if ( V_2 -> V_22 . type == V_136 )
V_135 = - 4 ;
if ( V_115 )
V_2 -> V_117 = V_115 ;
V_116 = F_21 ( 5 , V_2 -> V_117 ) ;
for ( V_98 = 0 ; V_98 < V_116 ; V_98 ++ ) {
int V_118 = ( V_137 + V_135 ) * V_98 + 3 ;
bool V_120 = V_3 [ V_118 ] & 0x1 ;
int V_9 = F_9 ( ( V_28 * ) & V_3 [ V_118 + 1 ] ) ;
int V_121 = F_22 ( V_5 , V_9 ) ;
if ( V_121 < 0 )
continue;
F_23 ( V_5 , V_121 ) ;
F_24 ( V_5 , V_122 , V_120 ) ;
if ( V_120 ) {
int V_113 = F_9 ( ( V_28 * ) & V_3 [ V_118 + V_135 + 7 ] ) ;
int V_114 = F_9 ( ( V_28 * ) & V_3 [ V_118 + V_135 + 9 ] ) ;
F_3 ( V_5 , V_129 , V_113 ) ;
F_3 ( V_5 , V_130 , V_114 ) ;
}
}
F_25 ( V_5 , true ) ;
V_2 -> V_117 -= V_116 ;
if ( V_2 -> V_117 < 0 )
V_2 -> V_117 = 0 ;
return 1 ;
}
static int F_27 ( struct V_1 * V_2 )
{
struct V_4 * V_5 = V_2 -> V_5 ;
unsigned char * V_3 = V_2 -> V_3 ;
int V_138 = 0 ;
int V_98 ;
for ( V_98 = 0 ; V_98 < 2 ; V_98 ++ ) {
int V_139 = V_3 [ 1 ] & ( 1 << V_98 ) ;
bool V_120 = V_139 && ! V_2 -> V_50 -> V_51 ;
F_23 ( V_5 , V_98 ) ;
F_24 ( V_5 , V_122 , V_120 ) ;
if ( V_120 ) {
int V_113 = F_9 ( ( V_28 * ) & V_3 [ V_98 * 2 + 2 ] ) & 0x7fff ;
int V_114 = F_9 ( ( V_28 * ) & V_3 [ V_98 * 2 + 6 ] ) & 0x7fff ;
F_3 ( V_5 , V_129 , V_113 ) ;
F_3 ( V_5 , V_130 , V_114 ) ;
V_138 ++ ;
}
}
F_25 ( V_5 , true ) ;
V_2 -> V_50 -> V_140 = ( V_138 > 0 ) ;
return 1 ;
}
static int F_28 ( struct V_1 * V_2 , T_1 V_141 )
{
char * V_3 = V_2 -> V_3 ;
struct V_4 * V_5 = V_2 -> V_5 ;
bool V_23 ;
int V_113 = 0 , V_114 = 0 ;
if ( V_2 -> V_22 . V_142 > 1 || V_141 > V_143 )
return 0 ;
if ( ! V_2 -> V_50 -> V_51 ) {
if ( V_141 == V_144 ) {
V_23 = V_3 [ 0 ] & 0x01 ;
V_113 = F_4 ( & V_3 [ 1 ] ) ;
V_114 = F_4 ( & V_3 [ 3 ] ) ;
} else {
V_23 = V_3 [ 1 ] & 0x01 ;
V_113 = F_9 ( ( V_28 * ) & V_3 [ 2 ] ) ;
V_114 = F_9 ( ( V_28 * ) & V_3 [ 4 ] ) ;
}
} else
V_23 = 0 ;
if ( V_23 ) {
F_3 ( V_5 , V_13 , V_113 ) ;
F_3 ( V_5 , V_14 , V_114 ) ;
}
F_2 ( V_5 , V_16 , V_23 ) ;
V_2 -> V_50 -> V_140 = V_23 ;
return 1 ;
}
static int F_29 ( struct V_1 * V_2 )
{
struct V_21 * V_22 = & V_2 -> V_22 ;
char * V_3 = V_2 -> V_3 ;
struct V_4 * V_5 = V_2 -> V_5 ;
int V_24 ;
bool V_23 = V_3 [ 1 ] & 0x20 ;
if ( ! V_2 -> V_50 -> V_51 )
V_2 -> V_6 [ 0 ] = ( V_3 [ 1 ] & 0x0c ) ? V_7 : V_8 ;
V_2 -> V_50 -> V_51 = V_23 ;
if ( ! V_2 -> V_50 -> V_140 ) {
F_2 ( V_5 , V_17 , V_3 [ 1 ] & 0x02 ) ;
F_2 ( V_5 , V_27 , V_3 [ 1 ] & 0x10 ) ;
F_3 ( V_5 , V_13 , F_9 ( ( V_28 * ) & V_3 [ 2 ] ) ) ;
F_3 ( V_5 , V_14 , F_9 ( ( V_28 * ) & V_3 [ 4 ] ) ) ;
V_24 = ( ( V_3 [ 7 ] & 0x01 ) << 8 ) | V_3 [ 6 ] ;
if ( V_24 < 0 )
V_24 = V_22 -> V_26 + V_24 + 1 ;
F_3 ( V_5 , V_15 , V_24 ) ;
F_2 ( V_5 , V_16 , V_3 [ 1 ] & 0x05 ) ;
F_2 ( V_5 , V_2 -> V_6 [ 0 ] , V_23 ) ;
return 1 ;
}
return 0 ;
}
static int F_30 ( struct V_1 * V_2 , T_1 V_141 )
{
char * V_3 = V_2 -> V_3 ;
F_5 ( V_2 -> V_5 -> V_18 . V_19 ,
L_3 , V_20 , V_3 [ 0 ] ) ;
switch ( V_141 ) {
case V_144 :
return F_28 ( V_2 , V_141 ) ;
case V_143 :
return F_27 ( V_2 ) ;
default:
switch ( V_3 [ 0 ] ) {
case V_145 :
case V_146 :
case V_147 :
case V_148 :
return F_28 ( V_2 , V_141 ) ;
case V_149 :
return F_26 ( V_2 ) ;
case V_25 :
return F_29 ( V_2 ) ;
}
}
return 0 ;
}
static int F_31 ( struct V_1 * V_2 )
{
struct V_21 * V_22 = & V_2 -> V_22 ;
struct V_4 * V_5 = V_2 -> V_5 ;
unsigned char * V_3 = V_2 -> V_3 ;
int V_98 ;
if ( V_3 [ 0 ] != 0x02 )
return 0 ;
for ( V_98 = 0 ; V_98 < 2 ; V_98 ++ ) {
int V_118 = ( V_3 [ 1 ] & 0x80 ) ? ( 8 * V_98 ) : ( 9 * V_98 ) ;
bool V_120 = V_3 [ V_118 + 3 ] & 0x80 ;
V_120 = V_120 && ! V_2 -> V_50 -> V_51 ;
F_23 ( V_5 , V_98 ) ;
F_24 ( V_5 , V_122 , V_120 ) ;
if ( V_120 ) {
int V_113 = F_32 ( & V_3 [ V_118 + 3 ] ) & 0x7ff ;
int V_114 = F_32 ( & V_3 [ V_118 + 5 ] ) & 0x7ff ;
if ( V_22 -> V_48 & V_150 ) {
V_113 <<= 5 ;
V_114 <<= 5 ;
}
F_3 ( V_5 , V_129 , V_113 ) ;
F_3 ( V_5 , V_130 , V_114 ) ;
}
}
F_25 ( V_5 , true ) ;
F_2 ( V_5 , V_34 , ( V_3 [ 1 ] & 0x08 ) != 0 ) ;
F_2 ( V_5 , V_44 , ( V_3 [ 1 ] & 0x04 ) != 0 ) ;
F_2 ( V_5 , V_43 , ( V_3 [ 1 ] & 0x02 ) != 0 ) ;
F_2 ( V_5 , V_35 , ( V_3 [ 1 ] & 0x01 ) != 0 ) ;
F_7 ( V_5 ) ;
return 0 ;
}
static void F_33 ( struct V_1 * V_2 , unsigned char * V_3 )
{
struct V_4 * V_5 = V_2 -> V_5 ;
bool V_120 = V_3 [ 1 ] & 0x80 ;
int V_121 = F_22 ( V_5 , V_3 [ 0 ] ) ;
if ( V_121 < 0 )
return;
V_120 = V_120 && ! V_2 -> V_50 -> V_51 ;
F_23 ( V_5 , V_121 ) ;
F_24 ( V_5 , V_122 , V_120 ) ;
if ( V_120 ) {
int V_113 = ( V_3 [ 2 ] << 4 ) | ( V_3 [ 4 ] >> 4 ) ;
int V_114 = ( V_3 [ 3 ] << 4 ) | ( V_3 [ 4 ] & 0x0f ) ;
int V_151 = V_3 [ 5 ] ;
int V_152 = F_34 ( V_5 , V_13 ) ;
int V_153 = F_34 ( V_5 , V_14 ) ;
int V_154 = 2 * F_19 ( V_151 * V_155 ) ;
int V_156 = V_154 * V_153 / V_152 ;
F_3 ( V_5 , V_129 , V_113 ) ;
F_3 ( V_5 , V_130 , V_114 ) ;
F_3 ( V_5 , V_131 , V_154 ) ;
F_3 ( V_5 , V_157 , V_156 ) ;
}
}
static void F_35 ( struct V_1 * V_2 , unsigned char * V_3 )
{
struct V_4 * V_5 = V_2 -> V_5 ;
F_2 ( V_5 , V_34 , ( V_3 [ 1 ] & 0x08 ) != 0 ) ;
F_2 ( V_5 , V_44 , ( V_3 [ 1 ] & 0x04 ) != 0 ) ;
F_2 ( V_5 , V_43 , ( V_3 [ 1 ] & 0x02 ) != 0 ) ;
F_2 ( V_5 , V_35 , ( V_3 [ 1 ] & 0x01 ) != 0 ) ;
}
static int F_36 ( struct V_1 * V_2 )
{
struct V_4 * V_5 = V_2 -> V_5 ;
unsigned char * V_3 = V_2 -> V_3 ;
int V_158 = V_3 [ 1 ] & 0x07 ;
int V_98 ;
if ( V_3 [ 0 ] != 0x02 )
return 0 ;
for ( V_98 = 0 ; V_98 < V_158 ; V_98 ++ ) {
int V_118 = ( 8 * V_98 ) + 2 ;
int V_159 = V_3 [ V_118 ] ;
if ( V_159 >= 2 && V_159 <= 17 )
F_33 ( V_2 , V_3 + V_118 ) ;
else if ( V_159 == 128 )
F_35 ( V_2 , V_3 + V_118 ) ;
}
F_25 ( V_5 , true ) ;
F_7 ( V_5 ) ;
return 0 ;
}
static int F_37 ( struct V_1 * V_2 )
{
struct V_4 * V_5 = V_2 -> V_5 ;
unsigned char * V_3 = V_2 -> V_3 ;
int V_23 = 0 , V_113 = 0 , V_114 = 0 , V_139 = 0 , V_160 = 0 , V_161 = 0 , V_162 = 0 , V_163 = 0 ;
if ( V_3 [ 0 ] != 0x02 )
return 0 ;
V_23 = ( V_3 [ 1 ] & 0x20 ) == 0x20 ;
if ( V_23 ) {
if ( ! V_2 -> V_50 -> V_51 ) {
if ( V_3 [ 1 ] & 0x08 ) {
V_2 -> V_6 [ 0 ] = V_7 ;
V_2 -> V_9 [ 0 ] = V_10 ;
} else {
V_2 -> V_6 [ 0 ] = V_8 ;
V_2 -> V_9 [ 0 ] = V_11 ;
}
V_2 -> V_50 -> V_51 = true ;
}
V_113 = F_9 ( ( V_28 * ) & V_3 [ 2 ] ) ;
V_114 = F_9 ( ( V_28 * ) & V_3 [ 4 ] ) ;
V_139 = F_9 ( ( V_28 * ) & V_3 [ 6 ] ) ;
if ( V_3 [ 8 ] <= V_2 -> V_22 . V_164 )
V_160 = V_2 -> V_22 . V_164 - V_3 [ 8 ] ;
V_161 = V_3 [ 1 ] & 0x01 ;
V_162 = V_3 [ 1 ] & 0x02 ;
V_163 = V_3 [ 1 ] & 0x04 ;
}
F_2 ( V_5 , V_16 , V_161 ) ;
F_2 ( V_5 , V_17 , V_162 ) ;
F_2 ( V_5 , V_27 , V_163 ) ;
F_3 ( V_5 , V_13 , V_113 ) ;
F_3 ( V_5 , V_14 , V_114 ) ;
F_3 ( V_5 , V_15 , V_139 ) ;
F_3 ( V_5 , V_38 , V_160 ) ;
if ( ! V_23 ) {
V_2 -> V_9 [ 0 ] = 0 ;
V_2 -> V_50 -> V_51 = false ;
}
F_2 ( V_5 , V_2 -> V_6 [ 0 ] , V_23 ) ;
F_3 ( V_5 , V_12 , V_2 -> V_9 [ 0 ] ) ;
return 1 ;
}
static int F_38 ( struct V_1 * V_2 , T_1 V_141 )
{
if ( V_141 == V_165 )
return F_31 ( V_2 ) ;
else if ( V_141 == V_166 )
return F_36 ( V_2 ) ;
else if ( V_141 == V_167 || V_141 == V_168 )
return F_37 ( V_2 ) ;
return 0 ;
}
static int F_39 ( struct V_1 * V_2 , T_1 V_141 )
{
unsigned char * V_3 = V_2 -> V_3 ;
int V_169 ;
if ( V_141 != V_170 || V_3 [ 0 ] != 0x80 )
return 0 ;
V_169 = V_3 [ 1 ] & 0x01 ;
if ( V_169 ) {
int V_171 , V_172 ;
V_171 = F_32 ( & V_3 [ 6 ] ) ;
V_172 = V_3 [ 5 ] & 0x3f ;
if ( V_2 -> V_171 != V_171 ) {
V_2 -> V_171 = V_171 ;
F_40 ( V_2 ) ;
}
V_2 -> V_173 = V_172 ;
} else if ( V_2 -> V_171 != 0 ) {
V_2 -> V_171 = 0 ;
F_40 ( V_2 ) ;
V_2 -> V_173 = 0 ;
}
return 0 ;
}
void F_41 ( struct V_1 * V_1 , T_1 V_141 )
{
bool V_174 ;
switch ( V_1 -> V_22 . type ) {
case V_175 :
V_174 = F_1 ( V_1 ) ;
break;
case V_176 :
V_174 = F_6 ( V_1 ) ;
break;
case V_36 :
case V_177 :
case V_37 :
V_174 = F_11 ( V_1 ) ;
break;
case V_178 :
V_174 = F_8 ( V_1 ) ;
break;
case V_179 :
V_174 = F_10 ( V_1 ) ;
break;
case V_47 :
case V_64 :
case V_104 :
case V_109 :
case V_67 :
case V_105 :
case V_74 :
case V_107 :
case V_180 :
case V_85 :
case V_57 :
case V_99 :
case V_68 :
case V_84 :
V_174 = F_16 ( V_1 ) ;
break;
case V_181 :
V_174 = F_20 ( V_1 ) ;
break;
case V_96 :
case V_106 :
case V_97 :
if ( V_141 == V_166 )
V_174 = F_36 ( V_1 ) ;
else
V_174 = F_16 ( V_1 ) ;
break;
case V_182 :
case V_183 :
case V_184 :
case V_185 :
case V_136 :
V_174 = F_30 ( V_1 , V_141 ) ;
break;
case V_186 :
V_174 = F_38 ( V_1 , V_141 ) ;
break;
case V_187 :
V_174 = F_39 ( V_1 , V_141 ) ;
break;
default:
V_174 = false ;
break;
}
if ( V_174 )
F_7 ( V_1 -> V_5 ) ;
}
static void F_42 ( struct V_1 * V_1 )
{
struct V_4 * V_4 = V_1 -> V_5 ;
F_43 ( V_4 , V_40 , V_41 ) ;
F_44 ( V_7 , V_4 -> V_188 ) ;
F_44 ( V_8 , V_4 -> V_188 ) ;
F_44 ( V_54 , V_4 -> V_188 ) ;
F_44 ( V_53 , V_4 -> V_188 ) ;
F_44 ( V_56 , V_4 -> V_188 ) ;
F_44 ( V_17 , V_4 -> V_188 ) ;
F_44 ( V_27 , V_4 -> V_188 ) ;
F_45 ( V_4 , V_38 ,
0 , V_1 -> V_22 . V_164 , 0 , 0 ) ;
F_45 ( V_4 , V_45 , 0 , 1023 , 0 , 0 ) ;
F_45 ( V_4 , V_58 , 0 , 127 , 0 , 0 ) ;
F_45 ( V_4 , V_59 , 0 , 127 , 0 , 0 ) ;
}
static void F_46 ( struct V_1 * V_1 )
{
struct V_4 * V_4 = V_1 -> V_5 ;
F_43 ( V_4 , V_189 , V_39 ) ;
F_42 ( V_1 ) ;
F_44 ( V_34 , V_4 -> V_188 ) ;
F_44 ( V_35 , V_4 -> V_188 ) ;
F_44 ( V_31 , V_4 -> V_188 ) ;
F_44 ( V_60 , V_4 -> V_188 ) ;
F_44 ( V_61 , V_4 -> V_188 ) ;
F_44 ( V_32 , V_4 -> V_188 ) ;
F_44 ( V_55 , V_4 -> V_188 ) ;
F_45 ( V_4 , V_63 , - 900 , 899 , 0 , 0 ) ;
F_45 ( V_4 , V_62 , - 1023 , 1023 , 0 , 0 ) ;
}
void F_47 ( struct V_21 * V_22 )
{
if ( V_22 -> V_190 == V_191 && ! V_22 -> V_192 ) {
V_22 -> V_192 = 1023 ;
V_22 -> V_193 = 1023 ;
}
if ( V_22 -> type >= V_187 ||
( V_22 -> type >= V_96 && V_22 -> type <= V_97 ) ||
( V_22 -> V_194 && V_22 -> V_195 ) )
V_22 -> V_48 |= V_49 ;
if ( V_22 -> type == V_186 &&
V_22 -> V_196 == V_165 ) {
V_22 -> V_192 <<= 5 ;
V_22 -> V_193 <<= 5 ;
V_22 -> V_197 <<= 5 ;
V_22 -> V_198 <<= 5 ;
V_22 -> V_48 |= V_150 ;
}
if ( V_22 -> type == V_187 ) {
V_22 -> V_48 |= V_199 ;
if ( ! V_22 -> V_190 )
V_22 -> V_48 |= V_200 ;
}
}
static unsigned int F_48 ( unsigned int V_201 ,
unsigned int V_202 )
{
return ( V_201 * 100 ) / V_202 ;
}
static void F_49 ( struct V_4 * V_4 ,
struct V_1 * V_1 )
{
struct V_21 * V_22 = & V_1 -> V_22 ;
if ( V_22 -> V_190 == V_8 ) {
F_45 ( V_4 , V_13 , 0 , V_22 -> V_192 ,
V_22 -> V_197 , 0 ) ;
F_45 ( V_4 , V_14 , 0 , V_22 -> V_193 ,
V_22 -> V_198 , 0 ) ;
F_45 ( V_4 , V_15 , 0 ,
V_22 -> V_26 , V_22 -> V_203 , 0 ) ;
F_50 ( V_4 , V_13 , V_22 -> V_204 ) ;
F_50 ( V_4 , V_14 , V_22 -> V_205 ) ;
} else {
if ( V_22 -> V_142 <= 2 ) {
F_45 ( V_4 , V_13 , 0 ,
V_22 -> V_192 , V_22 -> V_197 , 0 ) ;
F_45 ( V_4 , V_14 , 0 ,
V_22 -> V_193 , V_22 -> V_198 , 0 ) ;
F_50 ( V_4 , V_13 ,
F_48 ( V_22 -> V_192 ,
V_22 -> V_206 ) ) ;
F_50 ( V_4 , V_14 ,
F_48 ( V_22 -> V_193 ,
V_22 -> V_207 ) ) ;
}
if ( V_22 -> V_142 > 1 ) {
F_45 ( V_4 , V_129 , 0 ,
V_22 -> V_192 , V_22 -> V_197 , 0 ) ;
F_45 ( V_4 , V_130 , 0 ,
V_22 -> V_193 , V_22 -> V_198 , 0 ) ;
F_50 ( V_4 , V_129 ,
F_48 ( V_22 -> V_192 ,
V_22 -> V_206 ) ) ;
F_50 ( V_4 , V_130 ,
F_48 ( V_22 -> V_193 ,
V_22 -> V_207 ) ) ;
}
}
}
int F_51 ( struct V_4 * V_4 ,
struct V_1 * V_1 )
{
struct V_21 * V_22 = & V_1 -> V_22 ;
int V_98 ;
V_4 -> V_208 [ 0 ] |= F_52 ( V_209 ) | F_52 ( V_210 ) ;
F_44 ( V_16 , V_4 -> V_188 ) ;
F_44 ( V_12 , V_4 -> V_211 ) ;
F_49 ( V_4 , V_1 ) ;
switch ( V_1 -> V_22 . type ) {
case V_37 :
F_45 ( V_4 , V_45 , 0 , 71 , 0 , 0 ) ;
case V_36 :
F_43 ( V_4 , V_40 , V_41 ) ;
F_44 ( V_43 , V_4 -> V_188 ) ;
F_44 ( V_44 , V_4 -> V_188 ) ;
case V_177 :
F_43 ( V_4 , V_189 , V_39 ) ;
F_44 ( V_34 , V_4 -> V_188 ) ;
F_44 ( V_35 , V_4 -> V_188 ) ;
F_44 ( V_31 , V_4 -> V_188 ) ;
F_44 ( V_7 , V_4 -> V_188 ) ;
F_44 ( V_8 , V_4 -> V_188 ) ;
F_44 ( V_32 , V_4 -> V_188 ) ;
F_44 ( V_17 , V_4 -> V_188 ) ;
F_44 ( V_27 , V_4 -> V_188 ) ;
F_44 ( V_212 , V_4 -> V_213 ) ;
break;
case V_68 :
F_44 ( V_87 , V_4 -> V_188 ) ;
F_44 ( V_88 , V_4 -> V_188 ) ;
F_44 ( V_89 , V_4 -> V_188 ) ;
F_44 ( V_90 , V_4 -> V_188 ) ;
F_44 ( V_91 , V_4 -> V_188 ) ;
F_44 ( V_92 , V_4 -> V_188 ) ;
for ( V_98 = 6 ; V_98 < 10 ; V_98 ++ )
F_44 ( V_75 + V_98 , V_4 -> V_188 ) ;
F_44 ( V_93 , V_4 -> V_188 ) ;
F_44 ( V_94 , V_4 -> V_188 ) ;
F_44 ( V_95 , V_4 -> V_188 ) ;
F_45 ( V_4 , V_65 , - 900 , 899 , 0 , 0 ) ;
F_45 ( V_4 , V_62 , 0 , 71 , 0 , 0 ) ;
case V_84 :
for ( V_98 = 0 ; V_98 < 6 ; V_98 ++ )
F_44 ( V_75 + V_98 , V_4 -> V_188 ) ;
F_44 ( V_214 , V_4 -> V_213 ) ;
F_42 ( V_1 ) ;
break;
case V_99 :
F_44 ( V_93 , V_4 -> V_188 ) ;
F_44 ( V_94 , V_4 -> V_188 ) ;
F_44 ( V_95 , V_4 -> V_188 ) ;
case V_57 :
F_44 ( V_87 , V_4 -> V_188 ) ;
F_44 ( V_88 , V_4 -> V_188 ) ;
F_44 ( V_89 , V_4 -> V_188 ) ;
F_44 ( V_90 , V_4 -> V_188 ) ;
F_44 ( V_91 , V_4 -> V_188 ) ;
F_44 ( V_92 , V_4 -> V_188 ) ;
F_44 ( V_100 , V_4 -> V_188 ) ;
F_44 ( V_101 , V_4 -> V_188 ) ;
case V_180 :
F_44 ( V_83 , V_4 -> V_188 ) ;
F_44 ( V_86 , V_4 -> V_188 ) ;
case V_107 :
for ( V_98 = 0 ; V_98 < 8 ; V_98 ++ )
F_44 ( V_75 + V_98 , V_4 -> V_188 ) ;
F_45 ( V_4 , V_102 , 0 , 4096 , 0 , 0 ) ;
F_45 ( V_4 , V_103 , 0 , 4096 , 0 , 0 ) ;
F_45 ( V_4 , V_65 , - 900 , 899 , 0 , 0 ) ;
F_44 ( V_214 , V_4 -> V_213 ) ;
F_42 ( V_1 ) ;
break;
case V_85 :
for ( V_98 = 0 ; V_98 < 9 ; V_98 ++ )
F_44 ( V_75 + V_98 , V_4 -> V_188 ) ;
F_45 ( V_4 , V_65 , - 900 , 899 , 0 , 0 ) ;
F_44 ( V_214 , V_4 -> V_213 ) ;
F_42 ( V_1 ) ;
break;
case V_104 :
case V_109 :
F_44 ( V_79 , V_4 -> V_188 ) ;
F_44 ( V_80 , V_4 -> V_188 ) ;
F_44 ( V_81 , V_4 -> V_188 ) ;
F_44 ( V_82 , V_4 -> V_188 ) ;
F_45 ( V_4 , V_103 , 0 , 4096 , 0 , 0 ) ;
case V_64 :
F_44 ( V_75 , V_4 -> V_188 ) ;
F_44 ( V_76 , V_4 -> V_188 ) ;
F_44 ( V_77 , V_4 -> V_188 ) ;
F_44 ( V_78 , V_4 -> V_188 ) ;
F_45 ( V_4 , V_102 , 0 , 4096 , 0 , 0 ) ;
F_45 ( V_4 , V_65 , - 900 , 899 , 0 , 0 ) ;
case V_47 :
F_44 ( V_212 , V_4 -> V_213 ) ;
F_46 ( V_1 ) ;
break;
case V_106 :
case V_97 :
if ( V_22 -> V_190 == V_8 ) {
F_44 ( V_82 , V_4 -> V_188 ) ;
F_44 ( V_83 , V_4 -> V_188 ) ;
}
case V_96 :
F_44 ( V_212 , V_4 -> V_213 ) ;
if ( V_22 -> V_190 == V_8 ) {
for ( V_98 = 0 ; V_98 < 7 ; V_98 ++ )
F_44 ( V_75 + V_98 , V_4 -> V_188 ) ;
F_45 ( V_4 , V_38 , 0 ,
V_22 -> V_164 ,
0 , 0 ) ;
F_45 ( V_4 , V_65 , - 900 , 899 , 0 , 0 ) ;
F_46 ( V_1 ) ;
} else if ( V_22 -> V_190 == V_191 ) {
F_53 ( V_12 , V_4 -> V_211 ) ;
F_45 ( V_4 , V_131 ,
0 , V_22 -> V_192 , 0 , 0 ) ;
F_45 ( V_4 , V_157 ,
0 , V_22 -> V_193 , 0 , 0 ) ;
F_54 ( V_4 , V_22 -> V_142 , V_215 ) ;
}
break;
case V_105 :
case V_74 :
F_44 ( V_82 , V_4 -> V_188 ) ;
F_44 ( V_83 , V_4 -> V_188 ) ;
case V_67 :
for ( V_98 = 0 ; V_98 < 7 ; V_98 ++ )
F_44 ( V_75 + V_98 , V_4 -> V_188 ) ;
F_45 ( V_4 , V_65 , - 900 , 899 , 0 , 0 ) ;
F_46 ( V_1 ) ;
F_44 ( V_212 , V_4 -> V_213 ) ;
break;
case V_181 :
if ( V_22 -> V_190 == V_191 ) {
F_45 ( V_4 , V_131 , 0 , V_22 -> V_192 , 0 , 0 ) ;
F_45 ( V_4 , V_132 , 0 , V_22 -> V_192 , 0 , 0 ) ;
F_45 ( V_4 , V_133 , 0 , V_22 -> V_193 , 0 , 0 ) ;
F_45 ( V_4 , V_134 , 0 , 1 , 0 , 0 ) ;
}
case V_185 :
case V_136 :
case V_184 :
if ( V_22 -> V_190 == V_191 ) {
unsigned int V_216 = V_217 ;
if ( V_1 -> V_22 . type == V_184 )
V_216 = 0 ;
F_54 ( V_4 , V_22 -> V_142 , V_216 ) ;
}
case V_182 :
case V_183 :
F_53 ( V_12 , V_4 -> V_211 ) ;
F_44 ( V_214 , V_4 -> V_213 ) ;
if ( V_22 -> V_190 != V_8 )
break;
case V_176 :
case V_179 :
F_44 ( V_8 , V_4 -> V_188 ) ;
F_44 ( V_7 , V_4 -> V_188 ) ;
F_44 ( V_17 , V_4 -> V_188 ) ;
F_44 ( V_27 , V_4 -> V_188 ) ;
F_44 ( V_214 , V_4 -> V_213 ) ;
break;
case V_178 :
F_44 ( V_27 , V_4 -> V_188 ) ;
case V_175 :
F_44 ( V_8 , V_4 -> V_188 ) ;
F_44 ( V_7 , V_4 -> V_188 ) ;
F_44 ( V_17 , V_4 -> V_188 ) ;
F_44 ( V_212 , V_4 -> V_213 ) ;
break;
case V_186 :
F_53 ( V_12 , V_4 -> V_211 ) ;
F_44 ( V_212 , V_4 -> V_213 ) ;
if ( V_22 -> V_190 == V_191 ) {
unsigned int V_216 = V_215 ;
F_44 ( V_34 , V_4 -> V_188 ) ;
F_44 ( V_44 , V_4 -> V_188 ) ;
F_44 ( V_43 , V_4 -> V_188 ) ;
F_44 ( V_35 , V_4 -> V_188 ) ;
if ( V_22 -> V_196 == V_166 ) {
F_45 ( V_4 ,
V_131 ,
0 , V_22 -> V_192 , 0 , 0 ) ;
F_45 ( V_4 ,
V_157 ,
0 , V_22 -> V_193 , 0 , 0 ) ;
} else {
F_44 ( V_191 , V_4 -> V_188 ) ;
F_44 ( V_218 , V_4 -> V_188 ) ;
V_216 = 0 ;
}
F_54 ( V_4 , V_22 -> V_142 , V_216 ) ;
} else if ( V_22 -> V_190 == V_8 ) {
F_44 ( V_7 , V_4 -> V_188 ) ;
F_44 ( V_8 , V_4 -> V_188 ) ;
F_44 ( V_17 , V_4 -> V_188 ) ;
F_44 ( V_27 , V_4 -> V_188 ) ;
F_45 ( V_4 , V_38 , 0 ,
V_22 -> V_164 ,
0 , 0 ) ;
}
break;
}
return 0 ;
}
