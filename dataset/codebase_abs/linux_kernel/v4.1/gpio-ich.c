static int F_1 ( int V_1 , unsigned V_2 , int V_3 , int V_4 )
{
unsigned long V_5 ;
T_1 V_6 , V_7 ;
int V_8 = V_2 / 32 ;
int V_9 = V_2 & 0x1f ;
int V_10 = 0 ;
F_2 ( & V_11 . V_12 , V_5 ) ;
if ( V_1 == V_13 && V_11 . V_14 -> V_15 )
V_6 = V_11 . V_16 [ V_8 ] ;
else
V_6 = F_3 ( V_11 . V_14 -> V_17 [ V_1 ] [ V_8 ] ,
V_11 . V_18 ) ;
if ( V_3 )
V_6 |= 1 << V_9 ;
else
V_6 &= ~ ( 1 << V_9 ) ;
F_4 ( V_6 , V_11 . V_14 -> V_17 [ V_1 ] [ V_8 ] ,
V_11 . V_18 ) ;
if ( V_1 == V_13 && V_11 . V_14 -> V_15 )
V_11 . V_16 [ V_8 ] = V_6 ;
V_7 = F_3 ( V_11 . V_14 -> V_17 [ V_1 ] [ V_8 ] ,
V_11 . V_18 ) ;
if ( V_4 && V_6 != V_7 )
V_10 = - V_19 ;
F_5 ( & V_11 . V_12 , V_5 ) ;
return V_10 ;
}
static int F_6 ( int V_1 , unsigned V_2 )
{
unsigned long V_5 ;
T_1 V_6 ;
int V_8 = V_2 / 32 ;
int V_9 = V_2 & 0x1f ;
F_2 ( & V_11 . V_12 , V_5 ) ;
V_6 = F_3 ( V_11 . V_14 -> V_17 [ V_1 ] [ V_8 ] ,
V_11 . V_18 ) ;
if ( V_1 == V_13 && V_11 . V_14 -> V_15 )
V_6 = V_11 . V_16 [ V_8 ] | V_6 ;
F_5 ( & V_11 . V_12 , V_5 ) ;
return V_6 & ( 1 << V_9 ) ? 1 : 0 ;
}
static bool F_7 ( struct V_20 * V_21 , unsigned V_2 )
{
return ! ! ( V_11 . V_22 & ( 1 << ( V_2 / 32 ) ) ) ;
}
static int F_8 ( struct V_20 * V_21 , unsigned V_2 )
{
return F_6 ( V_23 , V_2 ) ? V_24 : V_25 ;
}
static int F_9 ( struct V_20 * V_21 , unsigned V_2 )
{
if ( F_1 ( V_23 , V_2 , 1 , 1 ) )
return - V_26 ;
return 0 ;
}
static int F_10 ( struct V_20 * V_21 , unsigned V_2 ,
int V_3 )
{
if ( V_2 < 32 && V_11 . V_14 -> V_27 )
F_1 ( V_28 , V_2 , 0 , 0 ) ;
F_1 ( V_13 , V_2 , V_3 , 0 ) ;
if ( F_1 ( V_23 , V_2 , 0 , 1 ) )
return - V_26 ;
return 0 ;
}
static int F_11 ( struct V_20 * V_29 , unsigned V_2 )
{
return F_6 ( V_13 , V_2 ) ;
}
static int F_12 ( struct V_20 * V_29 , unsigned V_2 )
{
unsigned long V_5 ;
T_1 V_6 ;
if ( V_2 < 16 ) {
if ( ! V_11 . V_30 )
return - V_31 ;
F_2 ( & V_11 . V_12 , V_5 ) ;
F_4 ( 1 << ( 16 + V_2 ) , 0 , V_11 . V_30 ) ;
V_6 = F_3 ( 0 , V_11 . V_30 ) ;
F_5 ( & V_11 . V_12 , V_5 ) ;
return ( V_6 >> 16 ) & ( 1 << V_2 ) ? 1 : 0 ;
} else {
return F_11 ( V_29 , V_2 ) ;
}
}
static int F_13 ( struct V_20 * V_29 , unsigned V_2 )
{
if ( ! F_7 ( V_29 , V_2 ) )
return - V_31 ;
if ( V_11 . V_14 -> V_32 [ V_2 / 32 ] & ( 1 << ( V_2 & 0x1f ) ) )
return 0 ;
return F_6 ( V_33 , V_2 ) ? 0 : - V_34 ;
}
static int F_14 ( struct V_20 * V_29 , unsigned V_2 )
{
if ( V_2 == 16 || V_2 == 17 )
V_2 -= 16 ;
return F_13 ( V_29 , V_2 ) ;
}
static void F_15 ( struct V_20 * V_29 , unsigned V_2 , int V_3 )
{
F_1 ( V_13 , V_2 , V_3 , 0 ) ;
}
static void F_16 ( struct V_20 * V_29 )
{
V_29 -> V_35 = V_36 ;
V_29 -> V_37 = V_38 ;
V_29 -> V_39 = & V_11 . V_39 -> V_39 ;
V_29 -> V_40 = V_11 . V_14 -> V_40 ?
V_11 . V_14 -> V_40 : F_13 ;
V_29 -> V_41 = V_11 . V_14 -> V_41 ?
V_11 . V_14 -> V_41 : F_11 ;
V_29 -> V_42 = F_15 ;
V_29 -> V_43 = F_8 ;
V_29 -> V_44 = F_9 ;
V_29 -> V_45 = F_10 ;
V_29 -> V_46 = V_47 ;
V_29 -> V_48 = V_11 . V_14 -> V_48 ;
V_29 -> V_49 = false ;
V_29 -> V_50 = NULL ;
}
static int F_17 ( struct V_51 * V_52 ,
const char * V_53 , T_2 V_22 )
{
int V_54 ;
if ( ! V_52 || ! V_52 -> V_55 || ! V_52 -> V_56 )
return - V_34 ;
for ( V_54 = 0 ; V_54 < F_18 ( V_11 . V_14 -> V_17 [ 0 ] ) ; V_54 ++ ) {
if ( ! ( V_22 & ( 1 << V_54 ) ) )
continue;
if ( ! F_19 (
V_52 -> V_55 + V_11 . V_14 -> V_17 [ 0 ] [ V_54 ] ,
V_11 . V_14 -> V_57 [ V_54 ] , V_53 ) )
goto V_58;
}
return 0 ;
V_58:
for ( V_54 -- ; V_54 >= 0 ; V_54 -- ) {
if ( ! ( V_22 & ( 1 << V_54 ) ) )
continue;
F_20 ( V_52 -> V_55 + V_11 . V_14 -> V_17 [ 0 ] [ V_54 ] ,
V_11 . V_14 -> V_57 [ V_54 ] ) ;
}
return - V_59 ;
}
static void F_21 ( struct V_51 * V_52 , T_2 V_22 )
{
int V_54 ;
for ( V_54 = 0 ; V_54 < F_18 ( V_11 . V_14 -> V_17 [ 0 ] ) ; V_54 ++ ) {
if ( ! ( V_22 & ( 1 << V_54 ) ) )
continue;
F_20 ( V_52 -> V_55 + V_11 . V_14 -> V_17 [ 0 ] [ V_54 ] ,
V_11 . V_14 -> V_57 [ V_54 ] ) ;
}
}
static int F_22 ( struct V_60 * V_61 )
{
struct V_51 * V_52 , * V_62 ;
int V_63 ;
struct V_64 * V_65 = F_23 ( & V_61 -> V_39 ) ;
if ( ! V_65 )
return - V_34 ;
V_11 . V_39 = V_61 ;
switch ( V_65 -> V_66 ) {
case V_67 :
V_11 . V_14 = & V_68 ;
break;
case V_69 :
V_11 . V_14 = & V_70 ;
break;
case V_71 :
V_11 . V_14 = & V_72 ;
break;
case V_73 :
V_11 . V_14 = & V_74 ;
break;
case V_75 :
V_11 . V_14 = & V_76 ;
break;
case V_77 :
V_11 . V_14 = & V_78 ;
break;
case V_79 :
V_11 . V_14 = & V_80 ;
break;
case V_81 :
V_11 . V_14 = & V_82 ;
break;
default:
return - V_34 ;
}
F_24 ( & V_11 . V_12 ) ;
V_52 = F_25 ( V_61 , V_83 , V_84 ) ;
V_11 . V_22 = V_65 -> V_22 ;
V_63 = F_17 ( V_52 , V_61 -> V_53 ,
V_11 . V_22 ) ;
if ( V_63 )
return V_63 ;
V_11 . V_18 = V_52 ;
if ( ! V_11 . V_14 -> V_85 )
goto V_86;
V_62 = F_25 ( V_61 , V_83 , V_87 ) ;
if ( ! V_62 ) {
F_26 ( L_1 ) ;
goto V_86;
}
if ( ! F_19 ( V_62 -> V_55 , F_27 ( V_62 ) ,
V_61 -> V_53 ) ) {
F_26 ( L_2 ) ;
goto V_86;
}
V_11 . V_30 = V_62 ;
V_86:
F_16 ( & V_11 . V_29 ) ;
V_63 = F_28 ( & V_11 . V_29 ) ;
if ( V_63 ) {
F_29 ( L_3 ) ;
goto V_88;
}
F_30 ( L_4 , V_11 . V_29 . V_46 ,
V_11 . V_29 . V_46 + V_11 . V_29 . V_48 - 1 , V_38 ) ;
return 0 ;
V_88:
F_21 ( V_11 . V_18 , V_11 . V_22 ) ;
if ( V_11 . V_30 )
F_20 ( V_11 . V_30 -> V_55 ,
F_27 ( V_11 . V_30 ) ) ;
return V_63 ;
}
static int F_31 ( struct V_60 * V_61 )
{
F_32 ( & V_11 . V_29 ) ;
F_21 ( V_11 . V_18 , V_11 . V_22 ) ;
if ( V_11 . V_30 )
F_20 ( V_11 . V_30 -> V_55 ,
F_27 ( V_11 . V_30 ) ) ;
return 0 ;
}
