static int F_1 ( struct V_1 * V_2 , int type , T_1 V_3 ,
struct V_4 * V_5 )
{
return F_2 ( type , V_2 -> V_6 -> V_7 , V_3 , V_5 ) ;
}
static void F_3 ( struct V_1 * V_2 , struct V_4 * V_5 )
{
F_4 ( V_5 ) ;
}
static int F_5 ( struct V_1 * V_2 ,
struct V_8 * V_9 ,
T_1 V_3 )
{
struct V_10 * V_10 = F_6 ( V_9 ) ;
V_10 -> V_11 = 0 ;
V_10 -> V_12 = 0 ;
V_10 -> V_13 = 0 ;
return F_7 ( V_9 , V_3 ) ;
}
static int F_8 ( struct V_1 * V_2 ,
struct V_8 * V_9 )
{
return F_9 ( V_9 ) ;
}
static void F_10 ( T_2 V_14 , T_2 * V_15 )
{
F_11 ( V_14 , V_15 ) ;
}
static T_2 F_12 ( T_2 * V_15 )
{
return F_13 ( V_15 ) ;
}
static void F_14 ( T_3 V_14 , T_3 * V_15 )
{
unsigned int V_16 = ( ( unsigned long ) ( V_15 ) & 0x3 ) << 3 ;
void * V_17 = ( void * ) ( ( unsigned long ) ( V_15 ) & ~ 0x3 ) ;
T_2 V_18 ;
V_18 = F_13 ( V_17 ) ;
V_18 &= ~ ( 0xffff << V_16 ) ;
V_18 |= V_14 << V_16 ;
F_11 ( V_18 , V_17 ) ;
}
static T_3 F_15 ( T_3 * V_15 )
{
unsigned int V_16 = ( ( unsigned long ) ( V_15 ) & 0x3 ) << 3 ;
void * V_17 = ( void * ) ( ( unsigned long ) ( V_15 ) & ~ 0x3 ) ;
T_2 V_18 ;
V_18 = F_13 ( V_17 ) ;
return ( V_18 >> V_16 ) & 0xffff ;
}
static void F_16 ( T_4 V_14 , T_4 * V_15 )
{
unsigned int V_16 = ( ( unsigned long ) ( V_15 ) & 0x3 ) << 3 ;
void * V_17 = ( void * ) ( ( unsigned long ) ( V_15 ) & ~ 0x3 ) ;
T_2 V_18 ;
V_18 = F_13 ( V_17 ) ;
V_18 &= ~ ( 0xff << V_16 ) ;
V_18 |= V_14 << V_16 ;
F_11 ( V_18 , V_17 ) ;
}
static T_4 F_17 ( T_4 * V_15 )
{
unsigned int V_16 = ( ( unsigned long ) ( V_15 ) & 0x3 ) << 3 ;
void * V_17 = ( void * ) ( ( unsigned long ) ( V_15 ) & ~ 0x3 ) ;
T_2 V_18 ;
V_18 = F_13 ( V_17 ) ;
return ( V_18 >> V_16 ) & 0xff ;
}
static void F_18 ( struct V_19 * V_20 )
{
T_2 V_18 ;
V_18 = F_13 ( V_20 -> V_21 + V_22 ) ;
V_18 |= V_23 ;
F_11 ( V_18 , V_20 -> V_21 + V_22 ) ;
V_18 = F_13 ( V_20 -> V_21 + V_24 ) ;
V_18 &= ~ V_25 ;
V_18 |= V_26 | V_27 |
V_28 | V_29 ;
F_11 ( V_18 , V_20 -> V_21 + V_24 ) ;
F_11 ( V_30 , V_20 -> V_21 + V_31 ) ;
F_11 ( V_32 , V_20 -> V_21 + V_31 ) ;
F_11 ( V_33 , V_20 -> V_21 + V_34 ) ;
V_18 = F_13 ( V_20 -> V_21 + V_35 ) ;
V_18 |= V_36 ;
F_11 ( V_18 , V_20 -> V_21 + V_35 ) ;
}
static int F_19 ( struct V_19 * V_37 )
{
int V_38 ;
V_38 = F_20 ( V_37 -> V_39 ) ;
if ( V_38 )
return V_38 ;
V_38 = F_20 ( V_37 -> V_40 ) ;
if ( V_38 )
goto V_41;
V_38 = F_20 ( V_37 -> V_42 ) ;
if ( V_38 )
goto V_43;
return 0 ;
V_43:
F_21 ( V_37 -> V_40 ) ;
V_41:
F_21 ( V_37 -> V_39 ) ;
return V_38 ;
}
static void F_22 ( struct V_19 * V_37 )
{
F_21 ( V_37 -> V_42 ) ;
F_21 ( V_37 -> V_40 ) ;
F_21 ( V_37 -> V_39 ) ;
}
static int F_23 ( struct V_44 * V_7 )
{
struct V_45 * V_6 = F_24 ( V_7 ) ;
struct V_1 * V_2 = V_6 -> V_46 ;
struct V_19 * V_20 = F_25 ( V_2 , struct V_19 , V_2 ) ;
F_26 ( V_6 , V_47 ) ;
F_27 ( V_2 ) ;
F_28 ( V_2 ) ;
F_22 ( V_20 ) ;
return 0 ;
}
static int F_29 ( struct V_44 * V_7 )
{
struct V_45 * V_6 = F_24 ( V_7 ) ;
struct V_1 * V_2 = V_6 -> V_46 ;
struct V_19 * V_20 = F_25 ( V_2 , struct V_19 , V_2 ) ;
F_19 ( V_20 ) ;
F_18 ( V_20 ) ;
F_30 ( V_2 , 1 ) ;
F_26 ( V_6 , V_48 ) ;
return 0 ;
}
static int F_31 ( struct V_49 * V_44 )
{
int V_50 ;
struct V_1 * V_2 = V_44 -> V_51 ;
if ( V_2 -> V_52 ) {
for ( V_50 = 0 ; V_50 < V_2 -> V_53 ; V_50 ++ )
F_32 ( V_2 , & V_2 -> V_10 [ V_50 ] ) ;
F_27 ( V_2 ) ;
}
F_33 ( V_2 ) ;
return 0 ;
}
static int F_34 ( struct V_1 * V_2 , struct V_54 * V_55 )
{
struct V_19 * V_20 = F_25 ( V_2 , struct V_19 , V_2 ) ;
struct V_44 * V_7 = V_20 -> V_7 ;
struct V_56 * V_57 ;
int V_58 ;
V_20 -> V_39 = F_35 ( V_7 , L_1 ) ;
if ( F_36 ( V_20 -> V_39 ) )
return F_37 ( V_20 -> V_39 ) ;
V_20 -> V_40 = F_35 ( V_7 , L_2 ) ;
if ( F_36 ( V_20 -> V_40 ) )
return F_37 ( V_20 -> V_40 ) ;
V_20 -> V_42 = F_35 ( V_7 , L_3 ) ;
if ( F_36 ( V_20 -> V_42 ) )
return F_37 ( V_20 -> V_42 ) ;
V_57 = F_38 ( V_55 , V_59 , 0 ) ;
V_20 -> V_21 = F_39 ( V_7 , V_57 ) ;
if ( F_36 ( V_20 -> V_21 ) )
return F_37 ( V_20 -> V_21 ) ;
V_2 -> V_60 = V_20 -> V_21 + V_61 ;
V_2 -> V_15 = V_57 -> V_62 + V_61 ;
V_58 = F_19 ( V_20 ) ;
if ( V_58 )
return V_58 ;
F_18 ( V_20 ) ;
return 0 ;
}
static int F_40 ( struct V_1 * V_2 , struct V_54 * V_55 )
{
struct V_45 * V_6 = V_2 -> V_6 ;
int V_58 ;
unsigned short V_63 ;
int V_64 = F_41 ( V_55 , 0 ) ;
V_58 = F_34 ( V_2 , V_55 ) ;
if ( V_58 )
return V_58 ;
V_58 = F_42 ( V_2 -> V_6 -> V_7 , V_64 , V_65 ,
V_66 , V_67 , V_2 ) ;
if ( V_58 ) {
F_43 ( V_2 -> V_6 -> V_7 ,
L_4 ,
V_64 ) ;
return V_58 ;
}
V_2 -> V_68 = V_64 ;
F_44 ( V_2 -> V_68 ) ;
V_63 = F_45 ( V_2 , V_69 ) ;
F_46 ( V_6 -> V_7 , L_5 , V_63 ) ;
V_2 -> V_70 = ( V_63 >> 8 ) & 0x0f ;
V_2 -> V_71 = ( V_63 >> 12 ) & 0x0f ;
if ( ! V_2 -> V_71 && ! V_2 -> V_70 ) {
V_2 -> V_71 = V_72 ;
V_2 -> V_70 = V_73 ;
}
V_2 -> V_74 = 0 ;
V_2 -> V_75 = V_2 -> V_70 ;
V_2 -> V_53 = V_2 -> V_71 + V_2 -> V_70 ;
V_2 -> V_10 = F_47 ( V_6 -> V_7 , V_2 -> V_53 ,
sizeof( * V_2 -> V_10 ) , V_76 ) ;
if ( ! V_2 -> V_10 )
return - V_77 ;
V_58 = F_48 ( V_2 ) ;
if ( V_58 < 0 )
return V_58 ;
F_49 ( V_2 ) ;
F_30 ( V_2 , 1 ) ;
if ( ! V_2 -> V_78 ) {
F_43 ( V_6 -> V_7 , L_6 ) ;
return - V_79 ;
}
strcpy ( V_6 -> V_80 , L_7 ) ;
strcpy ( V_6 -> V_81 , L_7 ) ;
snprintf ( V_6 -> V_82 , sizeof( V_6 -> V_82 ) ,
L_8 ,
V_6 -> V_81 , V_2 -> V_15 , V_2 -> V_68 ) ;
return 0 ;
}
static int F_50 ( struct V_45 * V_6 ,
unsigned int V_83 ,
const struct V_84 * V_85 ,
struct V_19 * V_20 )
{
static struct V_86 V_87 = {
. V_88 = F_31 ,
} ;
struct V_1 * V_2 ;
int V_58 ;
V_2 = & V_20 -> V_2 ;
F_51 ( & V_2 -> V_89 ) ;
F_52 ( & V_2 -> V_90 ) ;
V_2 -> V_6 = V_6 ;
V_2 -> V_87 = V_85 ;
V_2 -> V_68 = - 1 ;
V_2 -> V_83 = V_83 ;
V_2 -> V_91 = V_83 & 0xff ;
V_2 -> V_92 = 0 ;
F_53 ( & V_2 -> V_93 ) ;
V_2 -> V_94 = - 1 ;
V_2 -> V_95 = false ;
V_2 -> V_96 = true ;
V_58 = F_54 ( V_6 , V_97 , V_2 , & V_87 ) ;
if ( V_58 < 0 ) {
F_43 ( V_6 -> V_7 , L_9 ) ;
return V_58 ;
}
return 0 ;
}
static int F_55 ( struct V_54 * V_55 )
{
struct V_45 * V_6 ;
struct V_1 * V_2 ;
struct V_19 * V_20 ;
int V_58 ;
const unsigned int V_98 = V_99 ;
V_20 = F_56 ( & V_55 -> V_7 , sizeof( * V_20 ) , V_76 ) ;
if ( ! V_20 )
return - V_77 ;
V_20 -> V_7 = & V_55 -> V_7 ;
V_2 = & V_20 -> V_2 ;
V_58 = F_57 ( & V_55 -> V_7 , V_100 , V_101 ,
V_102 , 0 , & V_6 ) ;
if ( V_58 < 0 ) {
F_43 ( & V_55 -> V_7 , L_10 ) ;
return V_58 ;
}
V_58 = F_50 ( V_6 , V_98 , & V_103 , V_20 ) ;
if ( V_58 < 0 )
goto V_104;
V_6 -> V_46 = V_2 ;
F_58 ( & V_55 -> V_7 , V_6 ) ;
V_58 = F_40 ( V_2 , V_55 ) ;
if ( V_58 < 0 )
goto V_104;
V_58 = F_59 ( V_2 , NULL ) ;
if ( V_58 < 0 )
goto V_104;
V_58 = F_60 ( V_2 , 0 ) ;
if ( V_58 < 0 )
goto V_104;
V_58 = F_61 ( V_2 ) ;
if ( V_58 < 0 )
goto V_104;
V_58 = F_62 ( V_2 -> V_6 ) ;
if ( V_58 < 0 )
goto V_104;
V_2 -> V_105 = 1 ;
F_63 ( V_2 -> V_106 , V_107 * 1000 ) ;
return 0 ;
V_104:
F_64 ( V_6 ) ;
return V_58 ;
}
static int F_65 ( struct V_54 * V_55 )
{
return F_64 ( F_24 ( & V_55 -> V_7 ) ) ;
}
static void F_66 ( struct V_54 * V_55 )
{
struct V_45 * V_6 = F_24 ( & V_55 -> V_7 ) ;
struct V_1 * V_2 ;
if ( ! V_6 )
return;
V_2 = V_6 -> V_46 ;
if ( V_2 && V_2 -> V_105 )
F_27 ( V_2 ) ;
}
