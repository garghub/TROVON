static void F_1 ( struct V_1 * V_2 ,
struct V_3 * V_4 )
{
strncpy ( V_4 -> V_5 , V_6 , sizeof( V_4 -> V_5 ) ) ;
strncpy ( V_4 -> V_7 , V_8 , sizeof( V_4 -> V_7 ) ) ;
}
static T_1 F_2 ( struct V_1 * V_2 )
{
struct V_9 * V_10 = F_3 ( V_2 ) ;
return V_10 -> V_11 ;
}
static void F_4 ( struct V_1 * V_2 , T_1 V_12 )
{
struct V_9 * V_10 = F_3 ( V_2 ) ;
V_10 -> V_11 = V_12 ;
}
static struct V_13 * F_5 ( struct V_9 * V_10 )
{
struct V_13 * V_13 ;
V_13 = F_6 ( & V_14 , V_10 ) ;
if ( ! V_13 )
V_13 = F_6 ( & V_15 , V_10 ) ;
return V_13 ;
}
static void F_7 ( struct V_1 * V_2 ,
T_2 V_16 , T_3 * V_17 )
{
struct V_9 * V_10 = F_3 ( V_2 ) ;
struct V_13 * V_13 ;
struct V_18 * V_19 ;
int V_20 ;
V_13 = F_5 ( V_10 ) ;
if ( ! V_13 )
return;
V_19 = V_13 -> V_19 ;
switch ( V_16 ) {
case V_21 :
for ( V_20 = 0 ; V_20 < V_19 -> V_22 ; V_20 ++ ) {
memcpy ( V_17 , V_19 -> V_23 [ V_20 ] . V_24 ,
V_25 ) ;
V_17 += V_25 ;
}
break;
case V_26 :
break;
}
}
static int F_8 ( struct V_1 * V_2 , int V_16 )
{
struct V_9 * V_10 = F_3 ( V_2 ) ;
struct V_13 * V_13 ;
struct V_18 * V_19 ;
V_13 = F_5 ( V_10 ) ;
if ( ! V_13 )
return - V_27 ;
V_19 = V_13 -> V_19 ;
switch ( V_16 ) {
case V_26 :
return 0 ;
case V_21 :
return V_19 -> V_22 ;
default:
return - V_27 ;
}
}
static void F_9 ( struct V_18 * V_19 , int V_28 )
{
void T_4 * V_29 = V_19 -> V_30 [ V_28 ] ;
T_1 T_4 * V_31 ;
int V_20 ;
for ( V_20 = 0 ; V_20 < V_19 -> V_22 ; V_20 ++ ) {
if ( V_19 -> V_23 [ V_20 ] . type == V_28 ) {
V_31 = V_29 + V_19 -> V_23 [ V_20 ] . V_32 ;
V_19 -> V_33 [ V_20 ] = 0 ;
V_19 -> V_34 [ V_20 ] = F_10 ( V_31 ) ;
}
}
}
static inline void F_11 ( struct V_18 * V_19 ,
int V_35 )
{
void T_4 * V_29 = NULL ;
T_1 T_4 * V_31 ;
T_1 V_36 , V_37 ;
V_29 = V_19 -> V_30 [ V_19 -> V_23 [ V_35 ] . type ] ;
V_31 = V_29 + V_19 -> V_23 [ V_35 ] . V_32 ;
V_36 = F_10 ( V_31 ) ;
V_37 = V_36 - V_19 -> V_34 [ V_35 ] ;
V_19 -> V_34 [ V_35 ] = V_36 ;
V_19 -> V_33 [ V_35 ] += V_37 ;
}
static void F_12 ( struct V_18 * V_19 , T_5 * V_17 )
{
int V_20 ;
for ( V_20 = 0 ; V_20 < V_19 -> V_22 ; V_20 ++ ) {
F_11 ( V_19 , V_20 ) ;
if ( V_17 )
V_17 [ V_20 ] = V_19 -> V_33 [ V_20 ] ;
}
}
static inline void F_13 ( struct V_18 * V_19 ,
int V_28 )
{
T_1 V_38 ;
V_38 = F_10 ( F_14 ( V_19 , V_39 , V_40 ) ) ;
switch ( V_28 ) {
case V_41 :
case V_42 :
V_38 &= ~ V_43 ;
break;
case V_44 :
case V_45 :
V_38 |= V_43 ;
break;
default:
return;
}
F_15 ( V_38 , F_14 ( V_19 , V_39 , V_40 ) ) ;
}
static void F_16 ( struct V_18 * V_19 , int V_28 )
{
F_13 ( V_19 , V_28 ) ;
F_9 ( V_19 , V_28 ) ;
}
static void F_17 ( struct V_18 * V_19 , T_5 * V_17 )
{
T_1 V_46 = ( V_19 -> V_22 / 2 ) ;
int V_47 , V_48 , V_49 ;
for ( V_49 = 0 ; V_49 < 2 ; V_49 ++ ) {
F_13 ( V_19 , ( V_49 ?
V_44 :
V_41 ) ) ;
for ( V_48 = 0 ; V_48 < V_46 ; V_48 ++ ) {
V_47 = V_49 * V_46 + V_48 ;
F_11 ( V_19 , V_47 ) ;
if ( V_17 )
V_17 [ V_47 ] = V_19 -> V_33 [ V_47 ] ;
}
}
}
static void F_18 ( struct V_1 * V_2 ,
struct V_50 * V_51 ,
T_5 * V_17 )
{
struct V_9 * V_10 = F_3 ( V_2 ) ;
struct V_13 * V_13 ;
struct V_18 * V_19 ;
V_13 = F_5 ( V_10 ) ;
if ( ! V_13 )
return;
V_19 = V_13 -> V_19 ;
F_19 ( & V_19 -> V_52 ) ;
if ( V_19 -> V_53 == V_54 )
F_17 ( V_19 , V_17 ) ;
else
F_12 ( V_19 , V_17 ) ;
F_20 ( & V_19 -> V_52 ) ;
}
static int F_21 ( struct V_1 * V_2 ,
struct V_55 * V_56 )
{
struct V_9 * V_10 = F_3 ( V_2 ) ;
struct V_57 * V_58 = V_2 -> V_59 ;
struct V_13 * V_13 ;
int V_60 ;
if ( ! V_58 )
return - V_27 ;
V_13 = F_5 ( V_10 ) ;
if ( ! V_13 )
return - V_27 ;
if ( ! V_13 -> V_61 )
return - V_27 ;
V_60 = F_22 ( V_58 , V_56 ) ;
if ( ! V_60 )
V_56 -> V_29 . V_62 = V_13 -> V_61 -> V_63 ;
return V_60 ;
}
static int F_23 ( struct V_1 * V_2 ,
const struct V_55 * V_56 )
{
struct V_9 * V_10 = F_3 ( V_2 ) ;
struct V_57 * V_58 = V_2 -> V_59 ;
struct V_13 * V_13 ;
T_6 V_62 = V_56 -> V_29 . V_62 ;
T_1 V_64 , V_65 ;
T_1 V_66 ;
F_24 ( & V_64 ,
V_56 -> V_67 . V_64 ) ;
F_24 ( & V_65 ,
V_56 -> V_67 . V_65 ) ;
V_66 = V_64 & V_65 ;
if ( ! V_58 )
return - V_27 ;
V_13 = F_5 ( V_10 ) ;
if ( ! V_13 )
return - V_27 ;
if ( ! V_13 -> V_61 )
return - V_27 ;
if ( V_62 != V_13 -> V_61 -> V_63 ) {
if ( ( V_62 == V_68 ) && ! ( V_66 & V_69 ) )
return - V_27 ;
if ( ( V_62 == V_70 ) && ! ( V_66 & V_71 ) )
return - V_27 ;
if ( ( V_62 == V_72 ) && ! ( V_66 & V_73 ) )
return - V_27 ;
if ( ( V_62 == V_74 ) && ! ( V_66 & V_75 ) )
return - V_27 ;
if ( ( V_62 == V_76 ) && ! ( V_66 & V_77 ) )
return - V_27 ;
}
V_13 -> V_61 -> V_63 = V_62 ;
return F_25 ( V_58 , V_56 ) ;
}
static int F_26 ( struct V_1 * V_2 ,
struct V_78 * V_4 )
{
struct V_9 * V_10 = F_3 ( V_2 ) ;
struct V_13 * V_13 ;
V_13 = F_6 ( & V_14 , V_10 ) ;
if ( ! V_13 || ! V_13 -> V_19 -> V_79 )
return - V_27 ;
V_4 -> V_80 =
V_81 |
V_82 |
V_83 |
V_84 |
V_85 |
V_86 ;
V_4 -> V_87 = V_13 -> V_19 -> V_79 -> V_87 ;
V_4 -> V_88 =
( 1 << V_89 ) |
( 1 << V_90 ) ;
V_4 -> V_91 =
( 1 << V_92 ) |
( 1 << V_93 ) |
( 1 << V_94 ) ;
return 0 ;
}
static int F_26 ( struct V_1 * V_2 ,
struct V_78 * V_4 )
{
V_4 -> V_80 =
V_82 |
V_84 |
V_85 ;
V_4 -> V_87 = - 1 ;
V_4 -> V_88 = 0 ;
V_4 -> V_91 = 0 ;
return 0 ;
}
static void F_27 ( struct V_95 * V_61 ,
struct V_13 * V_13 )
{
struct V_1 * V_2 = V_13 -> V_2 ;
F_15 ( F_28 ( V_2 -> V_96 ) , F_14 ( V_61 , V_97 , V_98 ) ) ;
F_15 ( F_29 ( V_2 -> V_96 ) , F_14 ( V_61 , V_97 , V_99 ) ) ;
}
static int F_30 ( struct V_18 * V_100 , T_1 V_101 )
{
if ( V_100 -> V_102 == 0 )
return V_101 + 1 ;
return V_101 ;
}
static void F_31 ( struct V_18 * V_19 ,
struct V_1 * V_2 ,
struct V_95 * V_61 ,
int V_103 )
{
struct V_57 * V_58 = V_61 -> V_58 ;
T_1 V_104 = 0 ;
if ( V_103 ) {
V_104 = V_61 -> V_104 ;
if ( V_58 && ( V_58 -> V_105 == V_106 ) ) {
V_104 |= V_107 ;
V_104 &= ~ V_108 ;
} else if ( V_58 && ( V_58 -> V_105 == V_109 ) ) {
V_104 |= V_108 ;
V_104 &= ~ V_107 ;
}
F_15 ( V_104 , F_14 ( V_61 , V_110 ,
V_104 ) ) ;
F_32 ( V_19 -> V_111 , V_61 -> V_112 ,
V_113 ,
V_114 ) ;
if ( V_2 && V_61 -> V_115 &&
V_61 -> V_116 != V_117 &&
V_61 -> V_116 != V_118 )
F_33 ( V_2 ) ;
} else {
F_15 ( V_104 , F_14 ( V_61 , V_110 ,
V_104 ) ) ;
F_32 ( V_19 -> V_111 , V_61 -> V_112 ,
V_113 ,
V_119 ) ;
if ( V_2 &&
V_61 -> V_116 != V_117 &&
V_61 -> V_116 != V_118 )
F_34 ( V_2 ) ;
}
if ( V_58 )
F_35 ( V_58 ) ;
}
static bool F_36 ( struct V_95 * V_61 )
{
return ! V_61 -> V_58 || V_61 -> V_58 -> V_120 ;
}
static void F_37 ( struct V_18 * V_19 ,
struct V_95 * V_61 ,
struct V_1 * V_2 )
{
int V_121 = V_61 -> V_101 ;
int V_122 , V_123 = 1 , V_124 ;
if ( ! V_61 -> V_115 )
return;
if ( ! F_38 ( V_61 ) ) {
V_123 =
F_39 ( F_40 ( V_19 , V_121 ) , V_121 ) ;
}
V_122 = F_36 ( V_61 ) ;
V_124 = V_122 & V_123 ;
if ( F_41 ( & V_61 -> V_124 , V_124 ) != V_124 )
F_31 ( V_19 , V_2 , V_61 ,
V_124 ) ;
}
static void F_42 ( struct V_1 * V_2 )
{
struct V_9 * V_10 = F_3 ( V_2 ) ;
struct V_13 * V_13 ;
V_13 = F_6 ( & V_15 , V_10 ) ;
if ( ! V_13 )
return;
F_37 ( V_13 -> V_19 , V_13 -> V_61 ,
V_2 ) ;
}
static void F_43 ( struct V_1 * V_2 )
{
struct V_9 * V_10 = F_3 ( V_2 ) ;
struct V_13 * V_13 ;
V_13 = F_6 ( & V_14 , V_10 ) ;
if ( ! V_13 )
return;
F_37 ( V_13 -> V_19 , V_13 -> V_61 ,
V_2 ) ;
}
static void F_44 ( struct V_1 * V_2 )
{
struct V_18 * V_19 = F_3 ( V_2 ) ;
struct V_95 * V_61 ;
F_45 (slave, gbe_dev)
F_37 ( V_19 , V_61 , NULL ) ;
}
static int F_46 ( struct V_95 * V_61 )
{
T_1 V_20 , V_125 ;
F_15 ( V_126 , F_14 ( V_61 , V_110 , V_127 ) ) ;
for ( V_20 = 0 ; V_20 < V_128 ; V_20 ++ ) {
V_125 = F_10 ( F_14 ( V_61 , V_110 , V_127 ) ) ;
if ( ( V_125 & V_129 ) != V_126 )
return 0 ;
}
return V_130 ;
}
static void F_47 ( struct V_18 * V_19 , struct V_95 * V_61 ,
int V_131 )
{
void T_4 * V_132 ;
T_1 V_133 ;
if ( V_131 > V_134 )
V_131 = V_134 ;
if ( ( V_19 -> V_53 == V_135 ) &&
( V_61 -> V_116 >= V_118 ) ) {
V_133 = F_10 ( F_14 ( V_19 , V_136 , V_137 ) ) ;
V_133 |= ( 1 << V_61 -> V_101 ) ;
F_15 ( V_133 , F_14 ( V_19 , V_136 , V_137 ) ) ;
}
if ( F_48 ( V_19 ) )
V_132 = F_14 ( V_61 , V_97 , V_138 ) ;
else
V_132 = F_14 ( V_61 , V_110 , V_138 ) ;
F_15 ( V_131 , V_132 ) ;
F_15 ( V_61 -> V_104 , F_14 ( V_61 , V_110 , V_104 ) ) ;
}
static void F_49 ( struct V_18 * V_100 ,
struct V_95 * V_61 , bool V_139 )
{
if ( F_38 ( V_61 ) )
return;
F_50 ( F_40 ( V_100 , V_61 -> V_101 ) ,
V_61 -> V_101 , V_139 ) ;
}
static void F_51 ( struct V_13 * V_140 )
{
struct V_18 * V_19 = V_140 -> V_19 ;
struct V_95 * V_61 = V_140 -> V_61 ;
F_49 ( V_19 , V_61 , true ) ;
F_46 ( V_61 ) ;
F_32 ( V_19 -> V_111 , V_61 -> V_112 ,
V_113 , V_119 ) ;
F_52 ( V_19 -> V_111 , V_140 -> V_2 -> V_141 ,
1 << V_61 -> V_112 , 0 , 0 ) ;
if ( ! V_61 -> V_58 )
return;
F_53 ( V_61 -> V_58 ) ;
F_54 ( V_61 -> V_58 ) ;
V_61 -> V_58 = NULL ;
}
static void F_55 ( struct V_18 * V_100 , struct V_95 * V_61 )
{
if ( F_38 ( V_61 ) )
return;
F_56 ( F_40 ( V_100 , V_61 -> V_101 ) , V_61 -> V_101 ) ;
F_57 ( F_40 ( V_100 , V_61 -> V_101 ) , V_61 -> V_101 ,
V_61 -> V_116 ) ;
}
static int F_58 ( struct V_13 * V_13 )
{
struct V_18 * V_100 = V_13 -> V_19 ;
struct V_95 * V_61 = V_13 -> V_61 ;
T_7 V_142 ;
bool V_143 = false ;
void (* F_59)( struct V_1 * ) = F_43 ;
F_55 ( V_100 , V_61 ) ;
F_46 ( V_61 ) ;
F_49 ( V_100 , V_61 , false ) ;
F_47 ( V_100 , V_61 , V_100 -> V_144 ) ;
F_27 ( V_61 , V_13 ) ;
F_32 ( V_100 -> V_111 , V_61 -> V_112 ,
V_113 , V_114 ) ;
F_60 ( V_100 -> V_111 , V_13 -> V_2 -> V_141 ,
1 << V_61 -> V_112 , 0 , 0 , V_145 ) ;
if ( V_61 -> V_116 == V_117 ) {
V_143 = true ;
V_142 = V_146 ;
V_61 -> V_63 = V_74 ;
} else if ( V_61 -> V_116 == V_118 ) {
V_143 = true ;
V_142 = V_147 ;
V_61 -> V_63 = V_76 ;
}
if ( V_143 ) {
if ( V_100 -> V_53 == V_135 )
F_59 = F_42 ;
V_61 -> V_58 = F_61 ( V_13 -> V_2 ,
V_61 -> V_148 ,
F_59 , 0 ,
V_142 ) ;
if ( ! V_61 -> V_58 ) {
F_62 ( V_100 -> V_149 , L_1 ,
V_61 -> V_101 ) ;
return - V_150 ;
}
F_63 ( V_100 -> V_149 , L_2 ,
F_64 ( V_61 -> V_58 ) ) ;
F_65 ( V_61 -> V_58 ) ;
}
return 0 ;
}
static void F_66 ( struct V_18 * V_100 )
{
int V_151 = 1 ;
if ( F_67 ( V_100 ) || F_68 ( V_100 ) )
F_15 ( V_152 ,
F_14 ( V_100 , V_153 , V_154 ) ) ;
F_15 ( V_134 , F_14 ( V_100 , V_153 ,
V_138 ) ) ;
F_69 ( V_100 -> V_111 ) ;
if ( V_100 -> V_155 )
V_151 = 0 ;
F_32 ( V_100 -> V_111 , 0 , V_156 , V_151 ) ;
F_32 ( V_100 -> V_111 , 0 , V_157 , 1 ) ;
F_32 ( V_100 -> V_111 , V_100 -> V_102 ,
V_113 , V_114 ) ;
F_32 ( V_100 -> V_111 , 0 ,
V_158 ,
F_70 ( V_100 -> V_159 ) ) ;
F_32 ( V_100 -> V_111 , 0 ,
V_160 ,
F_70 ( V_100 -> V_159 - 1 ) ) ;
F_32 ( V_100 -> V_111 , 0 ,
V_161 ,
F_70 ( V_100 -> V_159 ) ) ;
F_32 ( V_100 -> V_111 , 0 ,
V_162 ,
F_70 ( V_100 -> V_159 ) ) ;
}
static void F_71 ( struct V_13 * V_13 , T_6 * V_163 )
{
struct V_18 * V_19 = V_13 -> V_19 ;
T_8 V_164 ;
F_60 ( V_19 -> V_111 , V_163 ,
F_70 ( V_19 -> V_159 ) , 0 , 0 ,
V_145 ) ;
F_72 (vlan_id, gbe_intf->active_vlans, VLAN_N_VID) {
F_60 ( V_19 -> V_111 , V_163 ,
F_70 ( V_19 -> V_159 ) ,
V_165 , V_164 , V_145 ) ;
}
}
static void F_73 ( struct V_13 * V_13 , T_6 * V_163 )
{
struct V_18 * V_19 = V_13 -> V_19 ;
T_8 V_164 ;
F_74 ( V_19 -> V_111 , V_163 , V_19 -> V_102 , 0 , 0 ) ;
F_72 (vlan_id, gbe_intf->active_vlans, VLAN_N_VID)
F_74 ( V_19 -> V_111 , V_163 , V_19 -> V_102 ,
V_165 , V_164 ) ;
}
static void F_75 ( struct V_13 * V_13 , T_6 * V_163 )
{
struct V_18 * V_19 = V_13 -> V_19 ;
T_8 V_164 ;
F_52 ( V_19 -> V_111 , V_163 , 0 , 0 , 0 ) ;
F_72 (vlan_id, gbe_intf->active_vlans, VLAN_N_VID) {
F_52 ( V_19 -> V_111 , V_163 , 0 , V_165 , V_164 ) ;
}
}
static void F_76 ( struct V_13 * V_13 , T_6 * V_163 )
{
struct V_18 * V_19 = V_13 -> V_19 ;
T_8 V_164 ;
F_77 ( V_19 -> V_111 , V_163 , V_19 -> V_102 , 0 , 0 ) ;
F_72 (vlan_id, gbe_intf->active_vlans, VLAN_N_VID) {
F_77 ( V_19 -> V_111 , V_163 , V_19 -> V_102 ,
V_165 , V_164 ) ;
}
}
static int F_78 ( void * V_166 , struct V_167 * V_168 )
{
struct V_13 * V_13 = V_166 ;
struct V_18 * V_19 = V_13 -> V_19 ;
F_63 ( V_19 -> V_149 , L_3 ,
V_168 -> V_163 , V_168 -> type ) ;
switch ( V_168 -> type ) {
case V_169 :
case V_170 :
F_71 ( V_13 , V_168 -> V_163 ) ;
break;
case V_171 :
case V_172 :
F_73 ( V_13 , V_168 -> V_163 ) ;
break;
case V_173 :
default:
break;
}
return 0 ;
}
static int F_79 ( void * V_166 , struct V_167 * V_168 )
{
struct V_13 * V_13 = V_166 ;
struct V_18 * V_19 = V_13 -> V_19 ;
F_63 ( V_19 -> V_149 , L_4 ,
V_168 -> V_163 , V_168 -> type ) ;
switch ( V_168 -> type ) {
case V_169 :
case V_170 :
F_75 ( V_13 , V_168 -> V_163 ) ;
break;
case V_171 :
case V_172 :
F_76 ( V_13 , V_168 -> V_163 ) ;
break;
case V_173 :
default:
break;
}
return 0 ;
}
static int F_80 ( void * V_166 , int V_174 )
{
struct V_13 * V_13 = V_166 ;
struct V_18 * V_19 = V_13 -> V_19 ;
F_81 ( V_174 , V_13 -> V_175 ) ;
F_82 ( V_19 -> V_111 , V_174 ,
F_70 ( V_19 -> V_159 ) ,
V_176 ,
F_70 ( V_19 -> V_159 ) ,
F_70 ( V_19 -> V_159 - 1 ) ) ;
return 0 ;
}
static int F_83 ( void * V_166 , int V_174 )
{
struct V_13 * V_13 = V_166 ;
struct V_18 * V_19 = V_13 -> V_19 ;
F_84 ( V_19 -> V_111 , V_174 , 0 ) ;
F_85 ( V_174 , V_13 -> V_175 ) ;
return 0 ;
}
static void F_86 ( void * V_177 , struct V_178 * V_179 )
{
struct V_13 * V_13 = V_177 ;
struct V_18 * V_19 = V_13 -> V_19 ;
F_87 ( V_19 -> V_79 , V_179 ) ;
}
static bool F_88 ( struct V_13 * V_13 ,
const struct V_180 * V_181 )
{
struct V_178 * V_179 = V_181 -> V_179 ;
unsigned int V_182 = F_89 ( V_179 ) ;
if ( V_182 == V_183 )
return false ;
switch ( V_182 ) {
case V_184 :
case V_185 :
case V_186 :
case V_187 :
case V_188 :
case ( V_189 | V_190 ) :
case ( V_189 | V_191 ) :
case ( V_189 | V_192 ) :
return true ;
}
return false ;
}
static int F_90 ( struct V_13 * V_13 ,
struct V_180 * V_181 )
{
struct V_57 * V_59 = V_181 -> V_179 -> V_149 -> V_59 ;
struct V_18 * V_19 = V_13 -> V_19 ;
if ( ! ( F_91 ( V_181 -> V_179 ) -> V_193 & V_194 ) ||
! F_92 ( V_19 -> V_79 ) )
return 0 ;
if ( V_59 && F_93 ( V_59 ) ) {
F_91 ( V_181 -> V_179 ) -> V_193 |= V_195 ;
return 0 ;
}
if ( F_88 ( V_13 , V_181 ) ) {
V_181 -> V_196 = F_86 ;
V_181 -> V_197 = ( void * ) V_13 ;
F_91 ( V_181 -> V_179 ) -> V_193 |= V_195 ;
}
return 0 ;
}
static int F_94 ( struct V_13 * V_13 , struct V_180 * V_181 )
{
struct V_57 * V_59 = V_181 -> V_179 -> V_149 -> V_59 ;
struct V_18 * V_19 = V_13 -> V_19 ;
if ( V_181 -> V_198 )
return 0 ;
if ( V_59 && F_95 ( V_59 ) ) {
V_181 -> V_198 = true ;
return 0 ;
}
F_96 ( V_19 -> V_79 , V_181 -> V_179 ) ;
V_181 -> V_198 = true ;
return 0 ;
}
static int F_97 ( struct V_13 * V_13 , struct V_199 * V_200 )
{
struct V_18 * V_19 = V_13 -> V_19 ;
struct V_79 * V_79 = V_19 -> V_79 ;
struct V_201 V_202 ;
if ( ! V_79 )
return - V_203 ;
V_202 . V_204 = 0 ;
V_202 . V_205 = F_92 ( V_79 ) ?
V_90 : V_89 ;
V_202 . V_206 = ( F_98 ( V_79 ) ?
V_79 -> V_207 : V_92 ) ;
return F_99 ( V_200 -> V_208 , & V_202 , sizeof( V_202 ) ) ? - V_209 : 0 ;
}
static void F_100 ( struct V_13 * V_13 )
{
struct V_18 * V_19 = V_13 -> V_19 ;
struct V_95 * V_61 = V_13 -> V_61 ;
T_1 V_210 , V_211 , V_212 ;
if ( ! F_98 ( V_19 -> V_79 ) &&
! F_92 ( V_19 -> V_79 ) ) {
F_15 ( 0 , F_14 ( V_61 , V_97 , V_213 ) ) ;
return;
}
V_211 = ( 30 << V_214 ) | V_215 ;
V_210 = V_216 << V_217 ;
V_212 = V_215 | V_218 |
( V_61 -> V_213 . V_219 << V_220 ) |
( V_61 -> V_213 . V_221 ? V_222 :
V_61 -> V_213 . V_223 << V_224 ) ;
if ( F_92 ( V_19 -> V_79 ) )
V_210 |= ( V_225 | V_226 ) ;
if ( F_98 ( V_19 -> V_79 ) )
V_210 |= ( V_227 | V_228 ) ;
F_15 ( V_210 , F_14 ( V_61 , V_97 , V_213 ) ) ;
F_15 ( V_211 , F_14 ( V_61 , V_97 , V_229 ) ) ;
F_15 ( V_212 , F_14 ( V_61 , V_97 , V_230 ) ) ;
}
static int F_101 ( struct V_13 * V_13 , struct V_199 * V_200 )
{
struct V_18 * V_19 = V_13 -> V_19 ;
struct V_79 * V_79 = V_19 -> V_79 ;
struct V_201 V_202 ;
if ( ! V_79 )
return - V_203 ;
if ( F_102 ( & V_202 , V_200 -> V_208 , sizeof( V_202 ) ) )
return - V_209 ;
if ( V_202 . V_204 )
return - V_27 ;
switch ( V_202 . V_205 ) {
case V_89 :
F_103 ( V_79 , 0 ) ;
break;
case V_90 :
F_103 ( V_79 , 1 ) ;
break;
default:
return - V_231 ;
}
switch ( V_202 . V_206 ) {
case V_92 :
F_104 ( V_79 , 0 ) ;
break;
case V_93 :
case V_232 :
case V_233 :
F_104 ( V_79 , V_93 ) ;
V_202 . V_206 = V_93 ;
break;
case V_234 :
case V_235 :
case V_236 :
case V_237 :
case V_238 :
case V_239 :
case V_94 :
case V_240 :
case V_241 :
F_104 ( V_79 , V_94 ) ;
V_202 . V_206 = V_94 ;
break;
default:
return - V_231 ;
}
F_100 ( V_13 ) ;
return F_99 ( V_200 -> V_208 , & V_202 , sizeof( V_202 ) ) ? - V_209 : 0 ;
}
static void F_105 ( struct V_18 * V_19 )
{
if ( ! V_19 -> V_79 )
return;
if ( V_19 -> V_242 > 0 )
goto V_243;
if ( F_106 ( V_19 -> V_79 ) ) {
F_62 ( V_19 -> V_149 , L_5 ) ;
return;
}
V_243:
++ V_19 -> V_242 ;
}
static void F_107 ( struct V_18 * V_19 )
{
if ( ! V_19 -> V_79 || ( V_19 -> V_242 <= 0 ) )
return;
if ( -- V_19 -> V_242 )
return;
F_108 ( V_19 -> V_79 ) ;
}
static inline int F_90 ( struct V_13 * V_13 ,
struct V_180 * V_181 )
{
return 0 ;
}
static inline int F_94 ( struct V_13 * V_13 ,
struct V_180 * V_181 )
{
return 0 ;
}
static inline int F_100 ( struct V_13 * V_13 ,
struct V_199 * V_200 , int V_56 )
{
return - V_203 ;
}
static inline void F_105 ( struct V_18 * V_19 )
{
}
static inline void F_107 ( struct V_18 * V_19 )
{
}
static inline int F_97 ( struct V_13 * V_13 , struct V_199 * V_244 )
{
return - V_203 ;
}
static inline int F_101 ( struct V_13 * V_13 , struct V_199 * V_244 )
{
return - V_203 ;
}
static int F_109 ( void * V_166 , struct V_199 * V_244 , int V_56 )
{
struct V_13 * V_13 = V_166 ;
struct V_57 * V_58 = V_13 -> V_61 -> V_58 ;
if ( ! V_58 || ! V_58 -> V_245 -> V_246 ) {
switch ( V_56 ) {
case V_247 :
return F_97 ( V_13 , V_244 ) ;
case V_248 :
return F_101 ( V_13 , V_244 ) ;
}
}
if ( V_58 )
return F_110 ( V_58 , V_244 , V_56 ) ;
return - V_203 ;
}
static void F_111 ( unsigned long V_249 )
{
struct V_18 * V_19 = (struct V_18 * ) V_249 ;
struct V_13 * V_13 ;
struct V_95 * V_61 ;
F_112 (gbe_intf, gbe_dev) {
if ( ! V_13 -> V_61 -> V_115 )
continue;
F_37 ( V_19 , V_13 -> V_61 ,
V_13 -> V_2 ) ;
}
F_45 (slave, gbe_dev) {
F_37 ( V_19 , V_61 , NULL ) ;
}
F_113 ( & V_19 -> V_52 ) ;
if ( V_19 -> V_53 == V_54 )
F_17 ( V_19 , NULL ) ;
else
F_12 ( V_19 , NULL ) ;
F_114 ( & V_19 -> V_52 ) ;
V_19 -> V_250 . V_251 = V_252 + V_253 ;
F_115 ( & V_19 -> V_250 ) ;
}
static int F_116 ( int V_254 , void * V_17 , struct V_180 * V_181 )
{
struct V_13 * V_13 = V_17 ;
V_181 -> V_255 = & V_13 -> V_255 ;
return F_90 ( V_13 , V_181 ) ;
}
static int F_117 ( int V_254 , void * V_17 , struct V_180 * V_181 )
{
struct V_13 * V_13 = V_17 ;
return F_94 ( V_13 , V_181 ) ;
}
static int F_118 ( void * V_166 , struct V_1 * V_2 )
{
struct V_13 * V_13 = V_166 ;
struct V_18 * V_19 = V_13 -> V_19 ;
struct V_9 * V_10 = F_3 ( V_2 ) ;
struct V_95 * V_61 = V_13 -> V_61 ;
int V_112 = V_61 -> V_112 ;
T_1 V_256 , V_38 ;
int V_60 ;
V_256 = F_10 ( F_14 ( V_19 , V_39 , V_257 ) ) ;
F_63 ( V_19 -> V_149 , L_6 ,
F_119 ( V_256 ) , F_120 ( V_256 ) ,
F_121 ( V_256 ) , F_122 ( V_256 ) ) ;
if ( ( V_19 -> V_53 == V_135 ) || F_48 ( V_19 ) )
V_13 -> V_255 . V_204 = V_258 ;
if ( V_19 -> V_155 )
V_13 -> V_255 . V_259 = 0 ;
else
V_13 -> V_255 . V_259 = V_112 ;
F_63 ( V_19 -> V_149 ,
L_7 ,
V_13 -> V_255 . V_260 ,
V_13 -> V_255 . V_261 ,
V_13 -> V_255 . V_259 ,
V_13 -> V_255 . V_204 ) ;
F_51 ( V_13 ) ;
F_15 ( 0 , F_14 ( V_19 , V_39 , V_262 ) ) ;
V_38 = V_263 ;
if ( F_48 ( V_19 ) ) {
V_38 |= V_264 ;
V_10 -> V_265 = V_266 ;
}
F_15 ( V_38 , F_14 ( V_19 , V_39 , V_137 ) ) ;
F_15 ( V_19 -> V_267 , F_14 ( V_19 , V_39 ,
V_40 ) ) ;
V_60 = F_58 ( V_13 ) ;
if ( V_60 )
goto V_268;
F_123 ( V_10 , V_269 , F_116 , V_13 ) ;
F_124 ( V_10 , V_270 , F_117 , V_13 ) ;
V_61 -> V_115 = true ;
F_37 ( V_19 , V_61 , V_2 ) ;
F_105 ( V_19 ) ;
return 0 ;
V_268:
F_51 ( V_13 ) ;
return V_60 ;
}
static int F_125 ( void * V_166 , struct V_1 * V_2 )
{
struct V_13 * V_13 = V_166 ;
struct V_9 * V_10 = F_3 ( V_2 ) ;
struct V_18 * V_19 = V_13 -> V_19 ;
F_107 ( V_19 ) ;
F_51 ( V_13 ) ;
F_126 ( V_10 , V_270 , F_117 , V_13 ) ;
F_127 ( V_10 , V_269 , F_116 , V_13 ) ;
V_13 -> V_61 -> V_115 = false ;
F_128 ( & V_13 -> V_61 -> V_124 , V_271 ) ;
return 0 ;
}
static void F_129 ( struct V_95 * V_61 )
{
V_61 -> V_213 . V_221 = 1 ;
V_61 -> V_213 . V_219 =
( V_272 >> V_220 ) & 0x3 ;
V_61 -> V_213 . V_223 =
( V_273 >> V_224 ) & 0x1f ;
}
static void F_129 ( struct V_95 * V_61 )
{
}
static int F_130 ( struct V_18 * V_19 , struct V_95 * V_61 ,
struct V_274 * V_275 )
{
int V_276 ;
T_1 V_277 , V_278 ;
T_1 V_279 , V_280 ;
if ( F_131 ( V_275 , L_8 , & V_61 -> V_101 ) ) {
F_62 ( V_19 -> V_149 , L_9 ) ;
return - V_27 ;
}
if ( F_131 ( V_275 , L_10 ,
& V_61 -> V_116 ) ) {
F_132 ( V_19 -> V_149 ,
L_11 ) ;
V_61 -> V_116 = V_117 ;
}
V_61 -> V_115 = false ;
if ( ( V_61 -> V_116 == V_117 ) ||
( V_61 -> V_116 == V_118 ) )
V_61 -> V_148 = F_133 ( V_275 , L_12 , 0 ) ;
V_61 -> V_112 = F_30 ( V_19 , V_61 -> V_101 ) ;
if ( V_61 -> V_116 >= V_118 )
V_61 -> V_104 = V_281 ;
else
V_61 -> V_104 = V_282 ;
V_276 = V_61 -> V_101 ;
if ( V_19 -> V_53 == V_54 ) {
if ( V_61 -> V_101 > 1 ) {
V_277 = V_283 ;
V_276 -= 2 ;
} else {
V_277 = V_284 ;
}
V_278 = V_285 ;
V_279 = 0x30 ;
V_280 = 0x40 ;
} else if ( F_48 ( V_19 ) ) {
V_277 = V_286 ;
V_278 = V_287 ;
V_279 = 0x1000 ;
V_280 = 0x1000 ;
} else if ( V_19 -> V_53 == V_135 ) {
V_277 = V_288 ;
V_278 = V_289 ;
V_279 = 0x30 ;
V_280 = 0x40 ;
} else {
F_62 ( V_19 -> V_149 , L_13 ,
V_19 -> V_53 ) ;
return - V_27 ;
}
V_61 -> V_97 = V_19 -> V_39 + V_277 +
( V_279 * V_276 ) ;
V_61 -> V_110 = V_19 -> V_39 + V_278 +
( V_280 * V_61 -> V_101 ) ;
if ( V_19 -> V_53 == V_54 ) {
F_134 ( V_61 , V_97 , V_290 ) ;
F_134 ( V_61 , V_97 , V_154 ) ;
F_134 ( V_61 , V_97 , V_99 ) ;
F_134 ( V_61 , V_97 , V_98 ) ;
F_134 ( V_61 , V_97 , V_213 ) ;
F_134 ( V_61 , V_97 , V_229 ) ;
F_134 ( V_61 , V_97 , V_291 ) ;
F_134 ( V_61 , V_97 , V_230 ) ;
F_134 ( V_61 , V_97 , V_292 ) ;
F_134 ( V_61 , V_110 , V_104 ) ;
F_134 ( V_61 , V_110 , V_127 ) ;
F_134 ( V_61 , V_110 , V_138 ) ;
} else if ( F_48 ( V_19 ) ) {
F_135 ( V_61 , V_97 , V_290 ) ;
F_135 ( V_61 , V_97 , V_154 ) ;
F_135 ( V_61 , V_97 , V_99 ) ;
F_135 ( V_61 , V_97 , V_98 ) ;
F_135 ( V_61 , V_97 , V_213 ) ;
F_135 ( V_61 , V_97 , V_229 ) ;
F_135 ( V_61 , V_97 , V_291 ) ;
F_135 ( V_61 , V_97 , V_230 ) ;
F_135 ( V_61 , V_97 , V_292 ) ;
F_135 ( V_61 , V_97 , V_138 ) ;
F_135 ( V_61 , V_110 , V_104 ) ;
F_135 ( V_61 , V_110 , V_127 ) ;
} else if ( V_19 -> V_53 == V_135 ) {
F_136 ( V_61 , V_97 , V_290 ) ;
F_136 ( V_61 , V_97 , V_154 ) ;
F_136 ( V_61 , V_97 , V_99 ) ;
F_136 ( V_61 , V_97 , V_98 ) ;
F_136 ( V_61 , V_97 , V_213 ) ;
F_136 ( V_61 , V_97 , V_229 ) ;
F_136 ( V_61 , V_97 , V_291 ) ;
F_136 ( V_61 , V_97 , V_230 ) ;
F_136 ( V_61 , V_97 , V_292 ) ;
F_136 ( V_61 , V_110 , V_104 ) ;
F_136 ( V_61 , V_110 , V_127 ) ;
F_136 ( V_61 , V_110 , V_138 ) ;
}
F_128 ( & V_61 -> V_124 , V_271 ) ;
F_129 ( V_61 ) ;
return 0 ;
}
static void F_137 ( struct V_18 * V_19 ,
struct V_274 * V_275 )
{
struct V_293 * V_149 = V_19 -> V_149 ;
T_7 V_142 ;
struct V_18 * * V_100 ;
struct V_274 * V_62 ;
struct V_95 * V_61 ;
bool V_294 = false ;
F_138 (node, port) {
V_61 = F_139 ( V_149 , sizeof( * V_61 ) , V_295 ) ;
if ( ! V_61 ) {
F_62 ( V_149 , L_14 ,
V_62 -> V_296 ) ;
continue;
}
if ( F_130 ( V_19 , V_61 , V_62 ) ) {
F_62 ( V_149 ,
L_15 ,
V_62 -> V_296 ) ;
F_140 ( V_149 , V_61 ) ;
continue;
}
F_55 ( V_19 , V_61 ) ;
F_46 ( V_61 ) ;
F_47 ( V_19 , V_61 , V_19 -> V_144 ) ;
F_141 ( & V_61 -> V_297 , & V_19 -> V_298 ) ;
V_19 -> V_299 ++ ;
if ( ( V_61 -> V_116 == V_117 ) ||
( V_61 -> V_116 == V_118 ) )
V_294 = true ;
V_61 -> V_115 = true ;
if ( V_19 -> V_299 >= V_19 -> V_300 ) {
F_142 ( V_62 ) ;
break;
}
}
if ( ! V_294 )
return;
V_19 -> V_301 = F_143 ( sizeof( V_19 ) , L_16 ,
V_302 , V_303 ) ;
if ( ! V_19 -> V_301 ) {
F_62 ( V_149 ,
L_17 ) ;
return;
}
V_100 = F_3 ( V_19 -> V_301 ) ;
* V_100 = V_19 ;
if ( V_61 -> V_116 == V_117 ) {
V_142 = V_146 ;
V_61 -> V_63 = V_74 ;
} else {
V_142 = V_147 ;
V_61 -> V_63 = V_76 ;
}
F_45 (slave, gbe_dev) {
if ( ( V_61 -> V_116 != V_117 ) &&
( V_61 -> V_116 != V_118 ) )
continue;
V_61 -> V_58 =
F_61 ( V_19 -> V_301 ,
V_61 -> V_148 ,
F_44 ,
0 , V_142 ) ;
if ( ! V_61 -> V_58 ) {
F_62 ( V_149 , L_18 ,
V_61 -> V_101 ) ;
V_61 -> V_58 = NULL ;
} else {
F_63 ( V_149 , L_2 ,
F_64 ( V_61 -> V_58 ) ) ;
F_65 ( V_61 -> V_58 ) ;
}
}
}
static void F_144 ( struct V_18 * V_19 )
{
struct V_95 * V_61 ;
while ( ! F_145 ( & V_19 -> V_298 ) ) {
V_61 = F_146 ( V_19 ) ;
if ( V_61 -> V_58 )
F_54 ( V_61 -> V_58 ) ;
F_147 ( & V_61 -> V_297 ) ;
}
if ( V_19 -> V_301 )
F_148 ( V_19 -> V_301 ) ;
}
static int F_149 ( struct V_18 * V_19 ,
struct V_274 * V_275 )
{
struct V_304 V_305 ;
void T_4 * V_306 ;
int V_60 , V_20 ;
V_60 = F_150 ( V_275 , V_307 , & V_305 ) ;
if ( V_60 ) {
F_62 ( V_19 -> V_149 ,
L_19 ,
V_275 -> V_296 , V_307 ) ;
return V_60 ;
}
V_306 = F_151 ( V_19 -> V_149 , & V_305 ) ;
if ( F_152 ( V_306 ) ) {
F_62 ( V_19 -> V_149 , L_20 ) ;
return F_153 ( V_306 ) ;
}
V_19 -> V_136 = V_306 ;
V_60 = F_150 ( V_275 , V_308 , & V_305 ) ;
if ( V_60 ) {
F_62 ( V_19 -> V_149 ,
L_21 ,
V_275 -> V_296 , V_308 ) ;
return V_60 ;
}
V_306 = F_151 ( V_19 -> V_149 , & V_305 ) ;
if ( F_152 ( V_306 ) ) {
F_62 ( V_19 -> V_149 , L_22 ) ;
return F_153 ( V_306 ) ;
}
V_19 -> V_39 = V_306 ;
V_60 = F_150 ( V_275 , V_309 , & V_305 ) ;
if ( V_60 ) {
F_62 ( V_19 -> V_149 ,
L_23 ,
V_275 -> V_296 , V_309 ) ;
return V_60 ;
}
V_306 = F_151 ( V_19 -> V_149 , & V_305 ) ;
if ( F_152 ( V_306 ) ) {
F_62 ( V_19 -> V_149 , L_24 ) ;
return F_153 ( V_306 ) ;
}
V_19 -> V_310 = V_306 ;
V_19 -> V_311 = V_19 -> V_312 ;
V_19 -> V_23 = V_313 ;
V_19 -> V_22 = F_154 ( V_313 ) ;
V_19 -> V_33 = F_139 ( V_19 -> V_149 ,
V_19 -> V_22 * sizeof( V_314 ) ,
V_295 ) ;
if ( ! V_19 -> V_33 ) {
F_62 ( V_19 -> V_149 , L_25 ) ;
return - V_315 ;
}
V_19 -> V_34 =
F_139 ( V_19 -> V_149 ,
V_19 -> V_22 * sizeof( T_1 ) ,
V_295 ) ;
if ( ! V_19 -> V_34 ) {
F_62 ( V_19 -> V_149 ,
L_26 ) ;
return - V_315 ;
}
V_19 -> V_53 = V_135 ;
V_19 -> V_316 = V_19 -> V_136 +
V_317 ;
V_19 -> V_153 = V_19 -> V_136 + V_318 ;
for ( V_20 = 0 ; V_20 < V_19 -> V_312 ; V_20 ++ )
V_19 -> V_30 [ V_20 ] = V_19 -> V_39 +
V_319 + ( V_320 * V_20 ) ;
V_19 -> V_321 = V_19 -> V_39 + V_322 ;
V_19 -> V_323 = V_19 -> V_39 + V_324 ;
V_19 -> V_159 = V_19 -> V_312 ;
V_19 -> V_102 = V_325 ;
V_19 -> V_326 = V_327 ;
V_19 -> V_267 = ( 1 << ( V_19 -> V_312 ) ) - 1 ;
F_136 ( V_19 , V_136 , V_257 ) ;
F_136 ( V_19 , V_136 , V_137 ) ;
F_136 ( V_19 , V_39 , V_257 ) ;
F_136 ( V_19 , V_39 , V_137 ) ;
F_136 ( V_19 , V_39 , V_262 ) ;
F_136 ( V_19 , V_39 , V_40 ) ;
F_136 ( V_19 , V_39 , V_328 ) ;
F_136 ( V_19 , V_153 , V_290 ) ;
F_136 ( V_19 , V_153 , V_154 ) ;
F_136 ( V_19 , V_153 , V_138 ) ;
return 0 ;
}
static int F_155 ( struct V_18 * V_19 ,
struct V_274 * V_275 )
{
struct V_304 V_305 ;
void T_4 * V_306 ;
int V_60 ;
V_60 = F_150 ( V_275 , V_329 , & V_305 ) ;
if ( V_60 ) {
F_62 ( V_19 -> V_149 ,
L_27 ,
V_275 -> V_296 , V_329 ) ;
return V_60 ;
}
V_306 = F_151 ( V_19 -> V_149 , & V_305 ) ;
if ( F_152 ( V_306 ) ) {
F_62 ( V_19 -> V_149 , L_28 ) ;
return F_153 ( V_306 ) ;
}
V_19 -> V_136 = V_306 ;
V_19 -> V_53 = F_10 ( V_19 -> V_136 ) ;
return 0 ;
}
static int F_156 ( struct V_18 * V_19 ,
struct V_274 * V_275 )
{
struct V_304 V_305 ;
void T_4 * V_306 ;
int V_20 , V_60 ;
V_60 = F_150 ( V_275 , V_330 , & V_305 ) ;
if ( V_60 ) {
F_62 ( V_19 -> V_149 ,
L_29 ,
V_275 -> V_296 , V_330 ) ;
return V_60 ;
}
V_306 = F_151 ( V_19 -> V_149 , & V_305 ) ;
if ( F_152 ( V_306 ) ) {
F_62 ( V_19 -> V_149 ,
L_30 ) ;
return F_153 ( V_306 ) ;
}
V_19 -> V_331 = V_306 ;
V_60 = F_150 ( V_275 , V_332 , & V_305 ) ;
if ( V_60 ) {
F_62 ( V_19 -> V_149 ,
L_29 ,
V_275 -> V_296 , V_332 ) ;
return V_60 ;
}
V_306 = F_151 ( V_19 -> V_149 , & V_305 ) ;
if ( F_152 ( V_306 ) ) {
F_62 ( V_19 -> V_149 ,
L_31 ) ;
return F_153 ( V_306 ) ;
}
V_19 -> V_39 = V_306 ;
V_19 -> V_311 = V_19 -> V_300 ;
V_19 -> V_23 = V_333 ;
V_19 -> V_22 = F_154 ( V_333 ) ;
V_19 -> V_33 = F_139 ( V_19 -> V_149 ,
V_19 -> V_22 * sizeof( V_314 ) ,
V_295 ) ;
if ( ! V_19 -> V_33 ) {
F_62 ( V_19 -> V_149 , L_25 ) ;
return - V_315 ;
}
V_19 -> V_34 =
F_139 ( V_19 -> V_149 ,
V_19 -> V_22 * sizeof( T_1 ) ,
V_295 ) ;
if ( ! V_19 -> V_34 ) {
F_62 ( V_19 -> V_149 ,
L_26 ) ;
return - V_315 ;
}
V_19 -> V_316 = V_19 -> V_136 + V_334 ;
V_19 -> V_153 = V_19 -> V_39 + V_335 ;
for ( V_20 = 0 ; V_20 < V_19 -> V_300 ; V_20 ++ ) {
V_19 -> V_30 [ V_20 ] =
V_19 -> V_39 + V_336 +
( V_320 * ( V_20 & 0x1 ) ) ;
}
V_19 -> V_323 = V_19 -> V_39 + V_337 ;
V_19 -> V_321 = V_19 -> V_39 + V_338 ;
V_19 -> V_159 = V_19 -> V_312 ;
V_19 -> V_102 = V_339 ;
V_19 -> V_326 = V_340 ;
V_19 -> V_267 = V_341 ;
F_134 ( V_19 , V_136 , V_257 ) ;
F_134 ( V_19 , V_39 , V_257 ) ;
F_134 ( V_19 , V_39 , V_137 ) ;
F_134 ( V_19 , V_39 , V_127 ) ;
F_134 ( V_19 , V_39 , V_40 ) ;
F_134 ( V_19 , V_39 , V_262 ) ;
F_134 ( V_19 , V_39 , V_328 ) ;
F_134 ( V_19 , V_153 , V_290 ) ;
F_134 ( V_19 , V_153 , V_138 ) ;
return 0 ;
}
static int F_157 ( struct V_18 * V_19 ,
struct V_274 * V_275 )
{
struct V_304 V_305 ;
void T_4 * V_306 ;
int V_20 , V_60 ;
V_19 -> V_311 = V_19 -> V_312 ;
V_19 -> V_23 = V_342 ;
if ( F_67 ( V_19 ) )
V_19 -> V_22 = V_343 +
( V_19 -> V_300 * V_344 ) ;
else
V_19 -> V_22 = V_343 +
V_344 ;
V_19 -> V_33 = F_139 ( V_19 -> V_149 ,
V_19 -> V_22 * sizeof( V_314 ) ,
V_295 ) ;
if ( ! V_19 -> V_33 ) {
F_62 ( V_19 -> V_149 , L_25 ) ;
return - V_315 ;
}
V_19 -> V_34 =
F_139 ( V_19 -> V_149 ,
V_19 -> V_22 * sizeof( T_1 ) ,
V_295 ) ;
if ( ! V_19 -> V_34 ) {
F_62 ( V_19 -> V_149 ,
L_26 ) ;
return - V_315 ;
}
V_60 = F_150 ( V_275 , V_345 , & V_305 ) ;
if ( V_60 ) {
F_62 ( V_19 -> V_149 ,
L_32 ,
V_275 -> V_296 , V_345 ) ;
return V_60 ;
}
V_306 = F_151 ( V_19 -> V_149 , & V_305 ) ;
if ( F_152 ( V_306 ) ) {
F_62 ( V_19 -> V_149 ,
L_33 ) ;
return F_153 ( V_306 ) ;
}
V_19 -> V_39 = V_306 ;
V_19 -> V_316 = V_19 -> V_136 + V_346 ;
V_19 -> V_331 = V_19 -> V_316 +
( 2 * V_347 ) ;
V_19 -> V_153 = V_19 -> V_39 + V_348 ;
for ( V_20 = 0 ; V_20 < ( V_19 -> V_312 ) ; V_20 ++ )
V_19 -> V_30 [ V_20 ] = V_19 -> V_39 +
V_349 + ( V_350 * V_20 ) ;
V_19 -> V_323 = V_19 -> V_39 + V_351 ;
V_19 -> V_321 = V_19 -> V_39 + V_352 ;
V_19 -> V_159 = V_19 -> V_312 ;
V_19 -> V_102 = V_353 ;
V_19 -> V_267 = ( 1 << ( V_19 -> V_312 ) ) - 1 ;
F_135 ( V_19 , V_136 , V_257 ) ;
F_135 ( V_19 , V_39 , V_257 ) ;
F_135 ( V_19 , V_39 , V_137 ) ;
F_135 ( V_19 , V_39 , V_40 ) ;
F_135 ( V_19 , V_39 , V_262 ) ;
F_135 ( V_19 , V_153 , V_290 ) ;
F_135 ( V_19 , V_153 , V_138 ) ;
F_135 ( V_19 , V_153 , V_154 ) ;
return 0 ;
}
static int F_158 ( struct V_354 * V_354 , struct V_293 * V_149 ,
struct V_274 * V_275 , void * * V_355 )
{
struct V_274 * V_356 , * V_357 ;
struct V_274 * V_358 ;
struct V_359 V_360 ;
struct V_18 * V_19 ;
T_1 V_101 ;
int V_20 , V_60 = 0 ;
if ( ! V_275 ) {
F_62 ( V_149 , L_34 ) ;
return - V_150 ;
}
V_19 = F_139 ( V_149 , sizeof( struct V_18 ) , V_295 ) ;
if ( ! V_19 )
return - V_315 ;
if ( F_159 ( V_275 , L_35 ) ||
F_159 ( V_275 , L_36 ) ) {
V_19 -> V_300 = 4 ;
} else if ( F_159 ( V_275 , L_37 ) ) {
V_19 -> V_300 = 8 ;
} else if ( F_159 ( V_275 , L_38 ) ) {
V_19 -> V_300 = 1 ;
} else if ( F_159 ( V_275 , L_39 ) ) {
V_19 -> V_300 = 2 ;
} else {
F_62 ( V_149 , L_40 ) ;
return - V_27 ;
}
V_19 -> V_312 = V_19 -> V_300 + 1 ;
V_19 -> V_149 = V_149 ;
V_19 -> V_354 = V_354 ;
V_19 -> V_144 = V_134 ;
F_160 ( & V_19 -> V_52 ) ;
if ( F_161 ( V_275 , L_41 , NULL ) ) {
V_19 -> V_155 = true ;
F_162 ( V_149 , L_42 ) ;
} else {
V_19 -> V_155 = false ;
F_63 ( V_149 , L_43 ) ;
}
V_60 = F_131 ( V_275 , L_44 ,
& V_19 -> V_361 ) ;
if ( V_60 < 0 ) {
F_62 ( V_149 , L_45 ) ;
V_19 -> V_361 = V_362 ;
}
V_60 = F_163 ( V_275 , L_46 ,
& V_19 -> V_260 ) ;
if ( V_60 < 0 ) {
F_62 ( V_149 , L_47 ) ;
return - V_27 ;
}
if ( ! strcmp ( V_275 -> V_296 , L_48 ) ) {
V_60 = F_155 ( V_19 , V_275 ) ;
if ( V_60 )
return V_60 ;
F_63 ( V_149 , L_49 , V_19 -> V_53 ) ;
if ( V_19 -> V_53 == V_54 )
V_60 = F_156 ( V_19 , V_275 ) ;
else if ( F_48 ( V_19 ) )
V_60 = F_157 ( V_19 , V_275 ) ;
else
V_60 = - V_150 ;
} else if ( ! strcmp ( V_275 -> V_296 , L_50 ) ) {
V_60 = F_149 ( V_19 , V_275 ) ;
if ( V_60 )
return V_60 ;
V_60 = F_164 ( V_19 -> V_310 ,
V_19 -> V_136 ) ;
} else {
F_62 ( V_149 , L_51 , V_275 -> V_296 ) ;
V_60 = - V_150 ;
}
if ( V_60 )
return V_60 ;
V_356 = F_165 ( V_275 , L_52 ) ;
if ( ! V_356 )
F_62 ( V_149 , L_53 ) ;
V_60 = F_166 ( & V_19 -> V_255 , V_354 ,
V_19 -> V_260 , V_19 -> V_361 ) ;
if ( V_60 )
return V_60 ;
V_60 = F_167 ( & V_19 -> V_255 ) ;
if ( V_60 )
return V_60 ;
F_168 ( & V_19 -> V_363 ) ;
F_138 (interfaces, interface) {
V_60 = F_131 ( V_357 , L_8 , & V_101 ) ;
if ( V_60 ) {
F_62 ( V_149 , L_54 ,
V_357 -> V_296 ) ;
continue;
}
V_19 -> V_299 ++ ;
if ( V_19 -> V_299 >= V_19 -> V_300 ) {
F_142 ( V_357 ) ;
break;
}
}
F_142 ( V_356 ) ;
if ( ! V_19 -> V_299 )
F_132 ( V_149 , L_55 ) ;
V_358 = F_165 ( V_275 , L_56 ) ;
F_168 ( & V_19 -> V_298 ) ;
if ( V_358 && ( V_19 -> V_299 < V_19 -> V_300 ) )
F_137 ( V_19 , V_358 ) ;
F_142 ( V_358 ) ;
if ( ! V_19 -> V_299 ) {
F_62 ( V_149 ,
L_57 ) ;
V_60 = - V_150 ;
goto V_364;
}
memset ( & V_360 , 0 , sizeof( V_360 ) ) ;
V_360 . V_149 = V_19 -> V_149 ;
V_360 . V_365 = V_19 -> V_321 ;
V_360 . V_366 = V_367 ;
V_360 . V_326 = V_19 -> V_326 ;
V_360 . V_159 = V_19 -> V_159 ;
if ( F_48 ( V_19 ) ) {
V_360 . V_368 = 0x7 ;
V_360 . V_369 = true ;
}
V_19 -> V_111 = F_169 ( & V_360 ) ;
if ( ! V_19 -> V_111 ) {
F_62 ( V_19 -> V_149 , L_58 ) ;
V_60 = - V_150 ;
goto V_364;
} else {
F_63 ( V_19 -> V_149 , L_59 ) ;
}
V_19 -> V_79 = F_170 ( V_19 -> V_149 , V_19 -> V_323 , V_275 ) ;
if ( F_171 ( V_370 ) && F_152 ( V_19 -> V_79 ) ) {
V_60 = F_153 ( V_19 -> V_79 ) ;
goto V_364;
}
F_66 ( V_19 ) ;
F_19 ( & V_19 -> V_52 ) ;
for ( V_20 = 0 ; V_20 < V_19 -> V_311 ; V_20 ++ ) {
if ( V_19 -> V_53 == V_54 )
F_16 ( V_19 , V_20 ) ;
else
F_9 ( V_19 , V_20 ) ;
}
F_20 ( & V_19 -> V_52 ) ;
F_172 ( & V_19 -> V_250 ) ;
V_19 -> V_250 . V_17 = ( unsigned long ) V_19 ;
V_19 -> V_250 . V_371 = F_111 ;
V_19 -> V_250 . V_251 = V_252 + V_253 ;
F_115 ( & V_19 -> V_250 ) ;
* V_355 = V_19 ;
return 0 ;
V_364:
F_144 ( V_19 ) ;
return V_60 ;
}
static int F_173 ( void * V_355 , struct V_1 * V_2 ,
struct V_274 * V_275 , void * * V_166 )
{
struct V_18 * V_19 = V_355 ;
struct V_13 * V_13 ;
int V_60 ;
if ( ! V_275 ) {
F_62 ( V_19 -> V_149 , L_60 ) ;
return - V_150 ;
}
V_13 = F_139 ( V_19 -> V_149 , sizeof( * V_13 ) , V_295 ) ;
if ( ! V_13 )
return - V_315 ;
V_13 -> V_2 = V_2 ;
V_13 -> V_149 = V_19 -> V_149 ;
V_13 -> V_19 = V_19 ;
V_13 -> V_61 = F_139 ( V_19 -> V_149 ,
sizeof( * V_13 -> V_61 ) ,
V_295 ) ;
if ( ! V_13 -> V_61 ) {
V_60 = - V_315 ;
goto V_268;
}
if ( F_130 ( V_19 , V_13 -> V_61 , V_275 ) ) {
V_60 = - V_150 ;
goto V_268;
}
V_13 -> V_255 = V_19 -> V_255 ;
V_2 -> V_372 = & V_373 ;
F_141 ( & V_13 -> V_374 , & V_19 -> V_363 ) ;
* V_166 = V_13 ;
return 0 ;
V_268:
if ( V_13 -> V_61 )
F_140 ( V_19 -> V_149 , V_13 -> V_61 ) ;
if ( V_13 )
F_140 ( V_19 -> V_149 , V_13 ) ;
return V_60 ;
}
static int F_174 ( void * V_166 )
{
struct V_13 * V_13 = V_166 ;
V_13 -> V_2 -> V_372 = NULL ;
F_147 ( & V_13 -> V_374 ) ;
F_140 ( V_13 -> V_149 , V_13 -> V_61 ) ;
F_140 ( V_13 -> V_149 , V_13 ) ;
return 0 ;
}
static int F_175 ( struct V_354 * V_354 , void * V_355 )
{
struct V_18 * V_19 = V_355 ;
F_176 ( & V_19 -> V_250 ) ;
F_177 ( V_19 -> V_79 ) ;
F_178 ( V_19 -> V_111 ) ;
F_179 ( V_19 -> V_111 ) ;
F_180 ( & V_19 -> V_255 ) ;
F_144 ( V_19 ) ;
if ( ! F_145 ( & V_19 -> V_363 ) )
F_181 ( V_19 -> V_149 ,
L_61 ) ;
return 0 ;
}
static int T_9 F_182 ( void )
{
int V_60 ;
V_60 = F_183 ( & V_14 ) ;
if ( V_60 )
return V_60 ;
V_60 = F_183 ( & V_15 ) ;
if ( V_60 )
return V_60 ;
return 0 ;
}
static void T_10 F_184 ( void )
{
F_185 ( & V_14 ) ;
F_185 ( & V_15 ) ;
}
