static inline void F_1 ( void T_1 * V_1 , T_2 V_2 )
{
F_2 ( F_3 ( V_1 ) | V_2 , V_1 ) ;
}
static inline void F_4 ( void T_1 * V_1 , T_2 V_2 )
{
F_2 ( F_3 ( V_1 ) & ~ V_2 , V_1 ) ;
}
static void F_5 ( struct V_3 * V_4 ,
enum V_5 V_6 )
{
switch ( V_6 ) {
case V_7 :
F_4 ( & V_4 -> V_8 -> V_9 , V_10 ) ;
break;
case V_11 :
F_1 ( & V_4 -> V_8 -> V_9 , V_10 ) ;
break;
default:
F_6 ( V_4 -> V_12 , L_1 , V_13 ) ;
break;
}
}
static void F_7 ( struct V_3 * V_4 )
{
T_2 V_14 = F_3 ( & V_4 -> V_8 -> V_15 ) ;
if ( V_4 -> V_16 . V_17 & V_18 )
V_14 |= V_19 ;
if ( V_4 -> V_16 . V_17 & V_20 )
V_14 |= V_21 ;
F_1 ( & V_4 -> V_8 -> V_9 , V_22 ) ;
F_2 ( V_14 , & V_4 -> V_8 -> V_15 ) ;
}
static void F_8 ( void T_1 * V_23 , T_2 V_24 )
{
int V_25 = V_26 ;
T_2 V_27 ;
F_2 ( V_24 , V_23 ) ;
while ( V_25 ) {
V_27 = F_3 ( V_23 ) & V_28 ;
if ( ! V_27 )
break;
V_25 -- ;
F_9 ( 1 ) ;
}
if ( ! V_25 )
F_10 ( L_2 , V_13 ) ;
}
static void F_11 ( struct V_3 * V_4 ,
enum V_5 V_29 )
{
switch ( V_29 ) {
case V_30 :
F_4 ( & V_4 -> V_8 -> V_9 , V_31 ) ;
break;
case V_32 :
F_1 ( & V_4 -> V_8 -> V_9 , V_33 ) ;
break;
case V_34 :
F_4 ( & V_4 -> V_8 -> V_9 , V_33 ) ;
break;
default:
F_6 ( V_4 -> V_12 , L_3 ) ;
break;
}
}
static void F_12 ( struct V_3 * V_4 , T_2 V_35 ,
int V_36 , enum V_37 V_38 )
{
T_2 V_39 ;
if ( V_38 )
V_39 = V_40 ;
else
V_39 = V_41 ;
F_2 ( V_42 , & V_4 -> V_8 -> V_43 [ V_38 ] . V_44 ) ;
F_8 ( & V_4 -> V_8 -> V_43 [ V_38 ] . V_45 , V_35 ) ;
F_2 ( V_46 | V_47 | V_48 ,
& V_4 -> V_8 -> V_43 [ V_38 ] . V_44 ) ;
if ( V_36 ) {
F_1 ( & V_4 -> V_8 -> V_43 [ V_38 ] . V_49 , V_39 ) ;
F_1 ( & V_4 -> V_8 -> V_43 [ V_38 ] . V_50 , V_51 ) ;
} else {
F_4 ( & V_4 -> V_8 -> V_43 [ V_38 ] . V_49 , V_39 ) ;
F_4 ( & V_4 -> V_8 -> V_43 [ V_38 ] . V_50 , V_51 ) ;
}
F_8 ( & V_4 -> V_8 -> V_43 [ V_38 ] . V_45 , V_35 ) ;
}
static void F_13 ( struct V_3 * V_4 , int V_36 )
{
int V_52 ;
for ( V_52 = V_53 ; V_52 <= V_54 ; V_52 ++ )
F_12 ( V_4 , V_52 , V_36 , V_55 ) ;
}
static void F_14 ( struct V_3 * V_4 , int V_36 )
{
int V_52 ;
for ( V_52 = V_56 ; V_52 <= V_57 ; V_52 ++ )
F_12 ( V_4 , V_52 , V_36 , V_58 ) ;
}
static T_2 F_15 ( struct V_3 * V_4 )
{
return F_3 ( & V_4 -> V_8 -> V_59 ) & 0xffff ;
}
static void F_16 ( struct V_3 * V_4 )
{
int V_52 ;
for ( V_52 = V_53 ; V_52 <= V_57 ; V_52 ++ ) {
F_2 ( V_60 , & V_4 -> V_8 -> V_43 [ 0 ] . V_44 ) ;
F_2 ( 0xffff , & V_4 -> V_8 -> V_43 [ 0 ] . V_61 ) ;
F_2 ( 0xffff , & V_4 -> V_8 -> V_43 [ 0 ] . V_62 ) ;
F_2 ( 0x0 , & V_4 -> V_8 -> V_43 [ 0 ] . V_63 ) ;
F_2 ( 0x0 , & V_4 -> V_8 -> V_43 [ 0 ] . V_50 ) ;
F_2 ( 0x0 , & V_4 -> V_8 -> V_43 [ 0 ] . V_49 ) ;
F_2 ( 0x0 , & V_4 -> V_8 -> V_43 [ 0 ] . V_64 [ 0 ] ) ;
F_2 ( 0x0 , & V_4 -> V_8 -> V_43 [ 0 ] . V_64 [ 1 ] ) ;
F_2 ( 0x0 , & V_4 -> V_8 -> V_43 [ 0 ] . V_64 [ 2 ] ) ;
F_2 ( 0x0 , & V_4 -> V_8 -> V_43 [ 0 ] . V_64 [ 3 ] ) ;
F_2 ( V_46 | V_65 |
V_47 | V_48 ,
& V_4 -> V_8 -> V_43 [ 0 ] . V_44 ) ;
F_8 ( & V_4 -> V_8 -> V_43 [ 0 ] . V_45 , V_52 ) ;
}
}
static void F_17 ( struct V_3 * V_4 )
{
int V_52 ;
for ( V_52 = V_53 ; V_52 <= V_54 ; V_52 ++ ) {
F_2 ( V_42 , & V_4 -> V_8 -> V_43 [ 0 ] . V_44 ) ;
F_8 ( & V_4 -> V_8 -> V_43 [ 0 ] . V_45 , V_52 ) ;
F_2 ( 0x0 , & V_4 -> V_8 -> V_43 [ 0 ] . V_63 ) ;
F_2 ( 0x0 , & V_4 -> V_8 -> V_43 [ 0 ] . V_50 ) ;
F_1 ( & V_4 -> V_8 -> V_43 [ 0 ] . V_49 ,
V_66 ) ;
if ( V_52 == V_54 )
F_1 ( & V_4 -> V_8 -> V_43 [ 0 ] . V_49 ,
V_67 ) ;
else
F_4 ( & V_4 -> V_8 -> V_43 [ 0 ] . V_49 ,
V_67 ) ;
F_2 ( 0 , & V_4 -> V_8 -> V_43 [ 0 ] . V_61 ) ;
F_4 ( & V_4 -> V_8 -> V_43 [ 0 ] . V_62 ,
0x1fff | V_68 ) ;
F_2 ( V_46 | V_65 | V_47 |
V_48 , & V_4 -> V_8 -> V_43 [ 0 ] . V_44 ) ;
F_8 ( & V_4 -> V_8 -> V_43 [ 0 ] . V_45 , V_52 ) ;
}
for ( V_52 = V_56 ; V_52 <= V_57 ; V_52 ++ ) {
F_2 ( V_42 , & V_4 -> V_8 -> V_43 [ 1 ] . V_44 ) ;
F_8 ( & V_4 -> V_8 -> V_43 [ 1 ] . V_45 , V_52 ) ;
F_2 ( 0x0 , & V_4 -> V_8 -> V_43 [ 1 ] . V_63 ) ;
F_2 ( V_69 , & V_4 -> V_8 -> V_43 [ 1 ] . V_50 ) ;
F_2 ( V_67 | V_66 ,
& V_4 -> V_8 -> V_43 [ 1 ] . V_49 ) ;
F_2 ( 0 , & V_4 -> V_8 -> V_43 [ 1 ] . V_61 ) ;
F_4 ( & V_4 -> V_8 -> V_43 [ 1 ] . V_62 , 0x1fff ) ;
F_2 ( V_46 | V_65 | V_47 |
V_48 , & V_4 -> V_8 -> V_43 [ 1 ] . V_44 ) ;
F_8 ( & V_4 -> V_8 -> V_43 [ 1 ] . V_45 , V_52 ) ;
}
}
static void F_18 ( struct V_3 * V_4 )
{
F_5 ( V_4 , V_11 ) ;
F_16 ( V_4 ) ;
F_17 ( V_4 ) ;
F_11 ( V_4 , V_32 ) ;
}
static void F_19 ( struct V_3 * V_4 )
{
F_5 ( V_4 , V_11 ) ;
F_11 ( V_4 , V_34 ) ;
F_13 ( V_4 , 0 ) ;
F_14 ( V_4 , 0 ) ;
}
static void F_20 ( struct V_3 * V_4 , T_2 V_2 )
{
if ( ( V_2 >= V_53 ) && ( V_2 <= V_54 ) ) {
F_2 ( V_46 | V_48 | V_47 ,
& V_4 -> V_8 -> V_43 [ 0 ] . V_44 ) ;
F_4 ( & V_4 -> V_8 -> V_43 [ 0 ] . V_50 , V_69 ) ;
F_4 ( & V_4 -> V_8 -> V_43 [ 0 ] . V_49 ,
V_70 | V_71 ) ;
F_8 ( & V_4 -> V_8 -> V_43 [ 0 ] . V_45 , V_2 ) ;
} else if ( ( V_2 >= V_56 ) && ( V_2 <= V_57 ) ) {
F_2 ( V_46 | V_48 | V_47 ,
& V_4 -> V_8 -> V_43 [ 1 ] . V_44 ) ;
F_1 ( & V_4 -> V_8 -> V_43 [ 1 ] . V_50 ,
V_69 | ( 0x7ff << 2 ) ) ;
F_2 ( 0x0 , & V_4 -> V_8 -> V_43 [ 1 ] . V_63 ) ;
F_4 ( & V_4 -> V_8 -> V_43 [ 1 ] . V_49 ,
V_70 | V_71 |
V_72 ) ;
F_8 ( & V_4 -> V_8 -> V_43 [ 1 ] . V_45 , V_2 ) ;
}
}
static void F_21 ( struct V_3 * V_4 )
{
F_2 ( 1 , & V_4 -> V_8 -> V_73 ) ;
F_2 ( 0 , & V_4 -> V_8 -> V_73 ) ;
}
static void F_22 ( struct V_74 * V_12 , T_2 V_75 )
{
struct V_76 * V_77 ;
struct V_3 * V_4 = F_23 ( V_12 ) ;
struct V_78 * V_79 ;
T_2 V_80 , V_81 ;
struct V_82 * V_83 = & ( V_4 -> V_12 -> V_83 ) ;
enum V_84 V_85 = V_4 -> V_16 . V_85 ;
V_77 = F_24 ( V_12 , & V_79 ) ;
if ( ! V_77 )
return;
if ( V_75 & V_86 ) {
F_14 ( V_4 , 0 ) ;
F_13 ( V_4 , 0 ) ;
V_85 = V_87 ;
V_79 -> V_88 |= V_89 ;
V_4 -> V_16 . V_90 . V_91 ++ ;
F_25 ( V_12 ) ;
}
V_80 = F_3 ( & V_4 -> V_8 -> V_80 ) ;
if ( V_75 & V_92 ) {
V_85 = V_93 ;
V_4 -> V_16 . V_90 . V_94 ++ ;
V_79 -> V_88 |= V_95 ;
if ( ( ( V_80 & V_96 ) >> 8 ) > 96 )
V_79 -> V_64 [ 1 ] |= V_97 ;
if ( ( V_80 & V_98 ) > 96 )
V_79 -> V_64 [ 1 ] |= V_99 ;
F_26 ( V_12 ,
L_4 , V_13 ) ;
}
if ( V_75 & V_100 ) {
V_4 -> V_16 . V_90 . V_101 ++ ;
V_85 = V_102 ;
V_79 -> V_88 |= V_95 ;
if ( V_80 & V_103 )
V_79 -> V_64 [ 1 ] |= V_104 ;
if ( ( V_80 & V_98 ) > 127 )
V_79 -> V_64 [ 1 ] |= V_105 ;
F_26 ( V_12 ,
L_5 , V_13 ) ;
}
V_81 = V_75 & V_106 ;
switch ( V_81 ) {
case V_107 :
V_79 -> V_64 [ 2 ] |= V_108 ;
V_4 -> V_16 . V_90 . V_109 ++ ;
V_83 -> V_110 ++ ;
break;
case V_111 :
V_79 -> V_64 [ 2 ] |= V_112 ;
V_4 -> V_16 . V_90 . V_109 ++ ;
V_83 -> V_110 ++ ;
break;
case V_113 :
V_79 -> V_88 |= V_114 ;
V_4 -> V_16 . V_90 . V_109 ++ ;
V_83 -> V_110 ++ ;
break;
case V_115 :
case V_116 :
V_79 -> V_64 [ 2 ] |= V_117 ;
V_4 -> V_16 . V_90 . V_109 ++ ;
V_83 -> V_110 ++ ;
break;
case V_118 :
V_79 -> V_64 [ 3 ] = V_119 ;
V_4 -> V_16 . V_90 . V_109 ++ ;
V_83 -> V_110 ++ ;
break;
case V_106 :
break;
}
V_79 -> V_64 [ 6 ] = V_80 & V_98 ;
V_79 -> V_64 [ 7 ] = ( V_80 & V_96 ) >> 8 ;
V_4 -> V_16 . V_85 = V_85 ;
F_27 ( V_77 ) ;
V_83 -> V_120 ++ ;
V_83 -> V_121 += V_79 -> V_122 ;
}
static T_3 F_28 ( int V_123 , void * V_124 )
{
struct V_74 * V_12 = (struct V_74 * ) V_124 ;
struct V_3 * V_4 = F_23 ( V_12 ) ;
if ( ! F_15 ( V_4 ) )
return V_125 ;
F_11 ( V_4 , V_34 ) ;
F_29 ( & V_4 -> V_126 ) ;
return V_127 ;
}
static void F_30 ( struct V_3 * V_4 , int V_128 )
{
if ( V_128 < V_129 ) {
F_2 ( V_46 | V_48 |
V_47 , & V_4 -> V_8 -> V_43 [ 0 ] . V_44 ) ;
F_4 ( & V_4 -> V_8 -> V_43 [ 0 ] . V_50 , V_69 ) ;
F_4 ( & V_4 -> V_8 -> V_43 [ 0 ] . V_49 ,
V_71 ) ;
F_8 ( & V_4 -> V_8 -> V_43 [ 0 ] . V_45 , V_128 ) ;
} else if ( V_128 > V_129 ) {
F_20 ( V_4 , V_128 ) ;
} else if ( V_128 == V_129 ) {
int V_130 ;
for ( V_130 = 0 ; V_130 < V_129 ; V_130 ++ )
F_20 ( V_4 , V_130 + 1 ) ;
}
}
static void F_31 ( struct V_74 * V_12 , int V_128 )
{
struct V_3 * V_4 = F_23 ( V_12 ) ;
struct V_82 * V_83 = & ( V_4 -> V_12 -> V_83 ) ;
struct V_76 * V_77 ;
struct V_78 * V_79 ;
F_26 ( V_4 -> V_12 , L_6 ) ;
F_4 ( & V_4 -> V_8 -> V_43 [ 0 ] . V_49 ,
V_131 ) ;
F_2 ( V_46 | V_48 ,
& V_4 -> V_8 -> V_43 [ 0 ] . V_44 ) ;
F_8 ( & V_4 -> V_8 -> V_43 [ 0 ] . V_45 , V_128 ) ;
V_77 = F_24 ( V_12 , & V_79 ) ;
if ( ! V_77 )
return;
V_79 -> V_88 |= V_95 ;
V_79 -> V_64 [ 1 ] = V_132 ;
V_83 -> V_133 ++ ;
V_83 -> V_110 ++ ;
F_27 ( V_77 ) ;
}
static int F_32 ( struct V_74 * V_12 , T_2 V_134 , int V_135 )
{
T_2 V_136 ;
T_4 V_137 ;
int V_138 = 0 ;
struct V_76 * V_77 ;
struct V_78 * V_79 ;
struct V_3 * V_4 = F_23 ( V_12 ) ;
struct V_82 * V_83 = & ( V_4 -> V_12 -> V_83 ) ;
int V_52 ;
T_2 V_50 ;
T_5 V_139 ;
do {
F_2 ( V_42 , & V_4 -> V_8 -> V_43 [ 0 ] . V_44 ) ;
F_8 ( & V_4 -> V_8 -> V_43 [ 0 ] . V_45 , V_134 ) ;
V_136 = F_3 ( & V_4 -> V_8 -> V_43 [ 0 ] . V_49 ) ;
if ( V_136 & V_67 )
break;
if ( V_136 & V_131 ) {
F_31 ( V_12 , V_134 ) ;
V_138 ++ ;
V_135 -- ;
V_134 ++ ;
continue;
} else if ( ! ( V_136 & V_70 ) ) {
V_134 ++ ;
continue;
}
V_77 = F_33 ( V_4 -> V_12 , & V_79 ) ;
if ( ! V_77 ) {
F_6 ( V_12 , L_7 ) ;
return V_138 ;
}
V_50 = F_3 ( & V_4 -> V_8 -> V_43 [ 0 ] . V_50 ) ;
if ( V_50 & V_140 ) {
V_137 = ( F_3 ( & V_4 -> V_8 -> V_43 [ 0 ] . V_63 ) & 0xffff ) ;
V_137 |= ( ( ( V_50 ) & 0x1fff ) << 16 ) ;
V_79 -> V_88 = V_137 | V_141 ;
} else {
V_137 = ( V_50 >> 2 ) & V_142 ;
V_79 -> V_88 = V_137 ;
}
if ( V_50 & V_69 )
V_79 -> V_88 |= V_143 ;
V_79 -> V_122 = F_34 ( ( F_3 ( & V_4 -> V_8 ->
V_43 [ 0 ] . V_49 ) ) & 0xF ) ;
for ( V_52 = 0 ; V_52 < V_79 -> V_122 ; V_52 += 2 ) {
V_139 = F_35 ( & V_4 -> V_8 -> V_43 [ 0 ] . V_64 [ V_52 / 2 ] ) ;
V_79 -> V_64 [ V_52 ] = V_139 ;
V_79 -> V_64 [ V_52 + 1 ] = V_139 >> 8 ;
}
F_27 ( V_77 ) ;
V_138 ++ ;
V_83 -> V_120 ++ ;
V_135 -- ;
V_83 -> V_121 += V_79 -> V_122 ;
F_30 ( V_4 , V_134 ) ;
V_134 ++ ;
} while ( V_135 > 0 );
return V_138 ;
}
static void F_36 ( struct V_74 * V_12 , T_2 V_144 )
{
struct V_3 * V_4 = F_23 ( V_12 ) ;
struct V_82 * V_83 = & ( V_4 -> V_12 -> V_83 ) ;
T_2 V_145 ;
F_37 ( V_12 , V_144 - V_54 - 1 ) ;
F_2 ( V_42 | V_146 ,
& V_4 -> V_8 -> V_43 [ 1 ] . V_44 ) ;
F_8 ( & V_4 -> V_8 -> V_43 [ 1 ] . V_45 , V_144 ) ;
V_145 = F_34 ( F_3 ( & V_4 -> V_8 -> V_43 [ 1 ] . V_49 ) &
V_147 ) ;
V_83 -> V_148 += V_145 ;
V_83 -> V_149 ++ ;
if ( V_144 == V_57 )
F_38 ( V_12 ) ;
}
static int F_39 ( struct V_150 * V_126 , int V_135 )
{
struct V_74 * V_12 = V_126 -> V_151 ;
struct V_3 * V_4 = F_23 ( V_12 ) ;
T_2 V_144 ;
T_2 V_152 ;
int V_153 = V_135 ;
V_144 = F_15 ( V_4 ) ;
if ( ! V_144 )
goto V_154;
if ( V_144 == V_155 ) {
V_152 = F_3 ( & V_4 -> V_8 -> V_156 ) ;
if ( ( V_152 & ( V_86 | V_106 ) ) &&
( ( V_152 & V_106 ) != V_106 ) ) {
F_22 ( V_12 , V_152 ) ;
V_135 -- ;
}
if ( V_152 & ( V_157 | V_158 ) )
F_4 ( & V_4 -> V_8 -> V_156 ,
V_152 & ( V_157 | V_158 ) ) ;
V_144 = F_15 ( V_4 ) ;
}
if ( V_135 == 0 )
goto V_154;
if ( ( V_144 >= V_53 ) && ( V_144 <= V_54 ) ) {
V_135 -= F_32 ( V_12 , V_144 , V_135 ) ;
} else if ( ( V_144 >= V_56 ) &&
( V_144 <= V_57 ) ) {
F_36 ( V_12 , V_144 ) ;
}
V_154:
F_40 ( V_126 ) ;
F_11 ( V_4 , V_32 ) ;
return V_153 - V_135 ;
}
static int F_41 ( struct V_74 * V_12 )
{
struct V_3 * V_4 = F_23 ( V_12 ) ;
const struct V_159 * V_160 = & V_4 -> V_16 . V_161 ;
T_2 V_162 ;
T_2 V_163 ;
F_1 ( & V_4 -> V_8 -> V_9 , V_164 ) ;
V_162 = ( V_160 -> V_165 - 1 ) & V_166 ;
V_162 |= ( V_160 -> V_167 - 1 ) << V_168 ;
V_162 |= ( V_160 -> V_169 + V_160 -> V_170 - 1 ) << V_171 ;
V_162 |= ( V_160 -> V_172 - 1 ) << V_173 ;
V_163 = ( ( V_160 -> V_165 - 1 ) & V_174 ) >> V_175 ;
F_2 ( V_162 , & V_4 -> V_8 -> V_176 ) ;
F_2 ( V_163 , & V_4 -> V_8 -> V_177 ) ;
F_4 ( & V_4 -> V_8 -> V_9 , V_164 ) ;
return 0 ;
}
static void F_42 ( struct V_74 * V_12 )
{
struct V_3 * V_4 = F_23 ( V_12 ) ;
if ( V_4 -> V_16 . V_85 != V_178 )
F_21 ( V_4 ) ;
F_41 ( V_12 ) ;
F_7 ( V_4 ) ;
F_14 ( V_4 , 1 ) ;
F_13 ( V_4 , 1 ) ;
F_5 ( V_4 , V_7 ) ;
V_4 -> V_16 . V_85 = V_179 ;
return;
}
static int F_43 ( struct V_74 * V_12 , enum V_180 V_6 )
{
int V_181 = 0 ;
switch ( V_6 ) {
case V_182 :
F_42 ( V_12 ) ;
F_38 ( V_12 ) ;
break;
default:
V_181 = - V_183 ;
break;
}
return V_181 ;
}
static int F_44 ( struct V_74 * V_12 )
{
struct V_3 * V_4 = F_23 ( V_12 ) ;
int V_184 ;
V_184 = F_45 ( V_4 -> V_151 -> V_123 , F_28 , V_185 ,
V_12 -> V_186 , V_12 ) ;
if ( V_184 ) {
F_6 ( V_12 , L_8 ) ;
goto V_187;
}
V_184 = F_46 ( V_12 ) ;
if ( V_184 ) {
F_6 ( V_12 , L_9 , V_184 ) ;
goto V_188;
}
F_18 ( V_4 ) ;
F_42 ( V_12 ) ;
F_47 ( & V_4 -> V_126 ) ;
F_48 ( V_12 ) ;
return 0 ;
V_188:
F_49 ( V_4 -> V_151 -> V_123 , V_12 ) ;
V_187:
F_19 ( V_4 ) ;
return V_184 ;
}
static int F_50 ( struct V_74 * V_12 )
{
struct V_3 * V_4 = F_23 ( V_12 ) ;
F_51 ( V_12 ) ;
F_52 ( & V_4 -> V_126 ) ;
F_19 ( V_4 ) ;
F_49 ( V_4 -> V_151 -> V_123 , V_12 ) ;
F_53 ( V_12 ) ;
V_4 -> V_16 . V_85 = V_178 ;
return 0 ;
}
static T_6 F_54 ( struct V_76 * V_77 , struct V_74 * V_12 )
{
struct V_3 * V_4 = F_23 ( V_12 ) ;
struct V_78 * V_79 = (struct V_78 * ) V_77 -> V_64 ;
int V_189 ;
int V_52 ;
T_2 V_50 ;
if ( F_55 ( V_12 , V_77 ) )
return V_190 ;
V_189 = V_4 -> V_191 ;
if ( V_4 -> V_191 == V_57 ) {
if ( F_3 ( & V_4 -> V_8 -> V_192 ) & V_193 )
F_51 ( V_12 ) ;
V_4 -> V_191 = V_56 ;
} else {
V_4 -> V_191 ++ ;
}
F_1 ( & V_4 -> V_8 -> V_43 [ 1 ] . V_44 , V_194 ) ;
if ( V_79 -> V_88 & V_141 ) {
F_2 ( V_79 -> V_88 & 0xffff , & V_4 -> V_8 -> V_43 [ 1 ] . V_63 ) ;
V_50 = ( ( V_79 -> V_88 >> 16 ) & 0x1fff ) | V_140 ;
} else {
F_2 ( 0 , & V_4 -> V_8 -> V_43 [ 1 ] . V_63 ) ;
V_50 = ( V_79 -> V_88 & V_142 ) << 2 ;
}
V_50 |= V_51 ;
if ( ! ( V_79 -> V_88 & V_143 ) )
V_50 |= V_69 ;
F_2 ( V_50 , & V_4 -> V_8 -> V_43 [ 1 ] . V_50 ) ;
for ( V_52 = 0 ; V_52 < V_79 -> V_122 ; V_52 += 2 ) {
F_56 ( V_79 -> V_64 [ V_52 ] | ( V_79 -> V_64 [ V_52 + 1 ] << 8 ) ,
& V_4 -> V_8 -> V_43 [ 1 ] . V_64 [ V_52 / 2 ] ) ;
}
F_57 ( V_77 , V_12 , V_189 - V_54 - 1 ) ;
F_2 ( V_79 -> V_122 | V_70 | V_72 |
V_40 , & V_4 -> V_8 -> V_43 [ 1 ] . V_49 ) ;
F_8 ( & V_4 -> V_8 -> V_43 [ 1 ] . V_45 , V_189 ) ;
return V_190 ;
}
static void F_58 ( struct V_195 * V_196 )
{
struct V_74 * V_12 = F_59 ( V_196 ) ;
struct V_3 * V_4 = F_23 ( V_12 ) ;
F_60 ( V_4 -> V_12 ) ;
if ( V_4 -> V_197 )
F_61 ( V_4 -> V_151 ) ;
F_62 ( V_196 ) ;
F_63 ( V_196 ) ;
F_21 ( V_4 ) ;
F_64 ( V_196 , V_4 -> V_8 ) ;
F_65 ( V_4 -> V_12 ) ;
}
static void F_66 ( struct V_3 * V_4 )
{
F_4 ( & V_4 -> V_8 -> V_9 , V_33 ) ;
F_1 ( & V_4 -> V_8 -> V_9 ,
( ( V_4 -> V_198 & V_199 ) << 1 ) ) ;
}
static T_2 F_67 ( struct V_3 * V_4 )
{
return ( F_3 ( & V_4 -> V_8 -> V_9 ) & V_33 ) >> 1 ;
}
static T_2 F_68 ( struct V_3 * V_4 , T_2 V_35 ,
enum V_37 V_38 )
{
T_2 V_39 , V_200 ;
if ( V_38 )
V_39 = V_41 ;
else
V_39 = V_40 ;
F_2 ( V_42 , & V_4 -> V_8 -> V_43 [ V_38 ] . V_44 ) ;
F_8 ( & V_4 -> V_8 -> V_43 [ V_38 ] . V_45 , V_35 ) ;
if ( ( ( F_3 ( & V_4 -> V_8 -> V_43 [ V_38 ] . V_50 ) ) & V_51 ) &&
( ( F_3 ( & V_4 -> V_8 -> V_43 [ V_38 ] . V_49 ) ) & V_39 ) )
V_200 = 1 ;
else
V_200 = 0 ;
return V_200 ;
}
static void F_69 ( struct V_3 * V_4 ,
T_2 V_201 , int V_36 )
{
F_2 ( V_42 , & V_4 -> V_8 -> V_43 [ 0 ] . V_44 ) ;
F_8 ( & V_4 -> V_8 -> V_43 [ 0 ] . V_45 , V_201 ) ;
F_2 ( V_46 | V_48 ,
& V_4 -> V_8 -> V_43 [ 0 ] . V_44 ) ;
if ( V_36 )
F_4 ( & V_4 -> V_8 -> V_43 [ 0 ] . V_49 ,
V_67 ) ;
else
F_1 ( & V_4 -> V_8 -> V_43 [ 0 ] . V_49 , V_67 ) ;
F_8 ( & V_4 -> V_8 -> V_43 [ 0 ] . V_45 , V_201 ) ;
}
static T_2 F_70 ( struct V_3 * V_4 , T_2 V_201 )
{
T_2 V_202 ;
F_2 ( V_42 , & V_4 -> V_8 -> V_43 [ 0 ] . V_44 ) ;
F_8 ( & V_4 -> V_8 -> V_43 [ 0 ] . V_45 , V_201 ) ;
if ( F_3 ( & V_4 -> V_8 -> V_43 [ 0 ] . V_49 ) & V_67 )
V_202 = 0 ;
else
V_202 = 1 ;
return V_202 ;
}
static int F_71 ( struct V_3 * V_4 )
{
return ( F_3 ( & V_4 -> V_8 -> V_203 ) & 0xffff ) |
( F_3 ( & V_4 -> V_8 -> V_192 ) << 16 ) ;
}
static int F_72 ( struct V_195 * V_196 , T_7 V_85 )
{
int V_52 ;
int V_184 ;
T_2 V_204 ;
int V_25 = V_26 ;
struct V_74 * V_151 = F_59 ( V_196 ) ;
struct V_3 * V_4 = F_23 ( V_151 ) ;
F_5 ( V_4 , V_11 ) ;
V_4 -> V_16 . V_85 = V_178 ;
while ( V_25 ) {
V_204 = F_71 ( V_4 ) ;
if ( ! V_204 )
break;
V_25 -- ;
F_9 ( 1 ) ;
}
if ( ! V_25 )
F_73 ( & V_196 -> V_151 , L_10 , V_13 ) ;
V_4 -> V_198 = F_67 ( V_4 ) ;
F_11 ( V_4 , V_30 ) ;
for ( V_52 = V_56 ; V_52 <= V_57 ; V_52 ++ )
V_4 -> V_205 [ V_52 - 1 ] = F_68 ( V_4 , V_52 ,
V_58 ) ;
F_14 ( V_4 , 0 ) ;
for ( V_52 = V_53 ; V_52 <= V_54 ; V_52 ++ ) {
V_4 -> V_206 [ V_52 - 1 ] = F_68 ( V_4 , V_52 ,
V_55 ) ;
V_4 -> V_207 [ V_52 - 1 ] = F_70 ( V_4 , V_52 ) ;
}
F_13 ( V_4 , 0 ) ;
V_184 = F_74 ( V_196 ) ;
if ( V_184 ) {
F_73 ( & V_196 -> V_151 , L_11 ) ;
} else {
F_75 ( V_196 , V_208 , 0 ) ;
F_63 ( V_196 ) ;
F_76 ( V_196 , F_77 ( V_196 , V_85 ) ) ;
}
return V_184 ;
}
static int F_78 ( struct V_195 * V_196 )
{
int V_52 ;
int V_184 ;
struct V_74 * V_151 = F_59 ( V_196 ) ;
struct V_3 * V_4 = F_23 ( V_151 ) ;
F_76 ( V_196 , V_209 ) ;
F_79 ( V_196 ) ;
V_184 = F_80 ( V_196 ) ;
if ( V_184 ) {
F_73 ( & V_196 -> V_151 , L_12 ) ;
return V_184 ;
}
F_75 ( V_196 , V_208 , 0 ) ;
V_4 -> V_16 . V_85 = V_179 ;
F_11 ( V_4 , V_30 ) ;
F_5 ( V_4 , V_11 ) ;
F_17 ( V_4 ) ;
F_41 ( V_151 ) ;
F_7 ( V_4 ) ;
for ( V_52 = V_56 ; V_52 <= V_57 ; V_52 ++ )
F_12 ( V_4 , V_52 , V_4 -> V_205 [ V_52 - 1 ] , V_58 ) ;
for ( V_52 = V_53 ; V_52 <= V_54 ; V_52 ++ ) {
F_69 ( V_4 , V_52 , V_4 -> V_207 [ V_52 - 1 ] ) ;
F_12 ( V_4 , V_52 , V_4 -> V_206 [ V_52 - 1 ] , V_55 ) ;
}
F_66 ( V_4 ) ;
F_5 ( V_4 , V_7 ) ;
return V_184 ;
}
static int F_81 ( const struct V_74 * V_151 ,
struct V_210 * V_211 )
{
struct V_3 * V_4 = F_23 ( V_151 ) ;
T_2 V_80 = F_3 ( & V_4 -> V_8 -> V_80 ) ;
V_211 -> V_212 = V_80 & V_98 ;
V_211 -> V_213 = ( V_80 & V_96 ) >> 8 ;
return 0 ;
}
static int F_82 ( struct V_195 * V_196 ,
const struct V_214 * V_137 )
{
struct V_74 * V_12 ;
struct V_3 * V_4 ;
int V_215 ;
void T_1 * V_1 ;
V_215 = F_80 ( V_196 ) ;
if ( V_215 ) {
F_73 ( & V_196 -> V_151 , L_13 , V_215 ) ;
goto V_216;
}
V_215 = F_83 ( V_196 , V_217 ) ;
if ( V_215 ) {
F_73 ( & V_196 -> V_151 , L_14 , V_215 ) ;
goto V_218;
}
V_1 = F_84 ( V_196 , 1 , 0 ) ;
if ( ! V_1 ) {
V_215 = - V_219 ;
F_73 ( & V_196 -> V_151 , L_15 ) ;
goto V_220;
}
V_12 = F_85 ( sizeof( struct V_3 ) , V_57 ) ;
if ( ! V_12 ) {
V_215 = - V_221 ;
F_73 ( & V_196 -> V_151 , L_16 ) ;
goto V_222;
}
V_4 = F_23 ( V_12 ) ;
V_4 -> V_12 = V_12 ;
V_4 -> V_8 = V_1 ;
V_4 -> V_151 = V_196 ;
V_4 -> V_16 . V_223 = & V_224 ;
V_4 -> V_16 . V_225 = F_43 ;
V_4 -> V_16 . V_226 = F_81 ;
V_4 -> V_16 . V_227 = V_18 |
V_20 ;
V_4 -> V_191 = V_56 ;
V_12 -> V_123 = V_196 -> V_123 ;
V_12 -> V_228 |= V_229 ;
F_86 ( V_196 , V_12 ) ;
F_87 ( V_12 , & V_196 -> V_151 ) ;
V_12 -> V_230 = & V_231 ;
V_4 -> V_16 . clock . V_232 = V_233 ;
F_88 ( V_12 , & V_4 -> V_126 , F_39 , V_54 ) ;
V_215 = F_89 ( V_4 -> V_151 ) ;
if ( V_215 ) {
F_6 ( V_12 , L_17 ) ;
V_4 -> V_197 = 0 ;
} else {
F_6 ( V_12 , L_18 ) ;
F_90 ( V_196 ) ;
V_4 -> V_197 = 1 ;
}
V_215 = F_91 ( V_12 ) ;
if ( V_215 ) {
F_73 ( & V_196 -> V_151 , L_19 , V_215 ) ;
goto V_234;
}
return 0 ;
V_234:
if ( V_4 -> V_197 )
F_61 ( V_4 -> V_151 ) ;
F_65 ( V_12 ) ;
V_222:
F_64 ( V_196 , V_1 ) ;
V_220:
F_62 ( V_196 ) ;
V_218:
F_63 ( V_196 ) ;
V_216:
return V_215 ;
}
