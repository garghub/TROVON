void F_1 ( const struct V_1 * V_2 , const char * V_3 ,
unsigned int V_4 , const char * V_5 ,
const char * V_6 , const char * V_7 , ... )
{
struct V_8 V_9 ;
T_1 args ;
if ( V_4 != V_10 && ! ( V_11 & V_4 ) )
return;
va_start ( args , V_7 ) ;
V_9 . V_12 = V_7 ;
V_9 . V_13 = & args ;
if ( V_2 )
F_2 ( V_3 , V_2 , V_14 , V_5 , V_6 ,
& V_9 ) ;
else
F_3 ( L_1 V_14 , V_3 , V_5 , V_6 , & V_9 ) ;
va_end ( args ) ;
}
void F_4 ( const char * V_3 , unsigned int V_4 ,
const char * V_7 , ... )
{
struct V_8 V_9 ;
T_1 args ;
if ( V_4 != V_10 && ! ( V_11 & V_4 ) )
return;
va_start ( args , V_7 ) ;
V_9 . V_12 = V_7 ;
V_9 . V_13 = & args ;
F_3 ( L_1 L_2 V_15 L_3 ,
V_3 , F_5 ( 0 ) ,
strcmp ( V_3 , V_16 ) == 0 ? L_4 : L_5 , & V_9 ) ;
va_end ( args ) ;
}
static struct V_17 * * F_6 ( struct V_18 * V_2 ,
unsigned int type )
{
switch ( type ) {
case V_19 :
return & V_2 -> V_20 ;
case V_21 :
return & V_2 -> V_22 ;
case V_23 :
return & V_2 -> V_24 ;
default:
return NULL ;
}
}
static int F_7 ( struct V_18 * V_2 , unsigned int type )
{
struct V_17 * V_25 ;
unsigned long V_26 ;
int V_27 ;
V_25 = F_8 ( sizeof( * V_25 ) , V_28 ) ;
if ( ! V_25 )
return - V_29 ;
V_25 -> type = type ;
V_25 -> V_2 = V_2 ;
F_9 ( V_28 ) ;
F_10 ( & V_30 , V_26 ) ;
V_27 = F_11 ( & V_31 ,
NULL ,
64 * type ,
64 * ( type + 1 ) ,
V_32 ) ;
F_12 ( & V_30 , V_26 ) ;
F_13 () ;
if ( V_27 < 0 )
goto V_33;
V_25 -> V_34 = V_27 ;
V_25 -> V_35 = F_14 ( V_25 ) ;
if ( F_15 ( V_25 -> V_35 ) ) {
V_27 = F_16 ( V_25 -> V_35 ) ;
goto V_36;
}
* F_6 ( V_2 , type ) = V_25 ;
return 0 ;
V_36:
F_10 ( & V_30 , V_26 ) ;
F_17 ( & V_31 , V_25 -> V_34 ) ;
F_12 ( & V_30 , V_26 ) ;
V_33:
F_18 ( V_25 ) ;
return V_27 ;
}
static void F_19 ( struct V_18 * V_2 , unsigned int type )
{
struct V_17 * * V_37 , * V_25 ;
unsigned long V_26 ;
V_37 = F_6 ( V_2 , type ) ;
V_25 = * V_37 ;
if ( ! V_25 )
return;
F_20 ( V_25 -> V_35 ) ;
F_10 ( & V_30 , V_26 ) ;
F_17 ( & V_31 , V_25 -> V_34 ) ;
F_12 ( & V_30 , V_26 ) ;
F_18 ( V_25 ) ;
* V_37 = NULL ;
}
static int F_21 ( struct V_18 * V_2 , unsigned int type )
{
struct V_17 * V_25 ;
unsigned long V_26 ;
int V_38 ;
F_22 ( L_6 ) ;
V_25 = * F_6 ( V_2 , type ) ;
if ( ! V_25 )
return 0 ;
V_38 = F_23 ( V_25 , V_25 -> V_34 , V_39 ) ;
if ( V_38 ) {
F_24 ( L_7 ) ;
return V_38 ;
}
V_38 = F_25 ( V_25 -> V_35 ) ;
if ( V_38 )
goto V_40;
F_10 ( & V_30 , V_26 ) ;
F_26 ( & V_31 , V_25 , V_25 -> V_34 ) ;
F_12 ( & V_30 , V_26 ) ;
F_22 ( L_8 , V_25 -> V_34 ) ;
return 0 ;
V_40:
F_27 ( V_25 ) ;
return V_38 ;
}
static void F_28 ( struct V_18 * V_2 , unsigned int type )
{
struct V_17 * V_25 ;
unsigned long V_26 ;
V_25 = * F_6 ( V_2 , type ) ;
if ( ! V_25 || ! F_29 ( V_25 -> V_35 ) )
return;
F_10 ( & V_30 , V_26 ) ;
F_26 ( & V_31 , NULL , V_25 -> V_34 ) ;
F_12 ( & V_30 , V_26 ) ;
F_30 ( V_25 -> V_35 ) ;
F_31 ( V_25 -> V_35 , NULL ) ;
F_27 ( V_25 ) ;
}
struct V_17 * F_32 ( unsigned int V_41 )
{
struct V_17 * V_25 ;
unsigned long V_26 ;
F_10 ( & V_30 , V_26 ) ;
V_25 = F_33 ( & V_31 , V_41 ) ;
if ( V_25 )
F_34 ( V_25 -> V_2 ) ;
F_12 ( & V_30 , V_26 ) ;
if ( ! V_25 ) {
return F_35 ( - V_42 ) ;
} else if ( F_36 ( V_25 -> V_2 ) ) {
F_37 ( V_25 -> V_2 ) ;
return F_35 ( - V_42 ) ;
}
return V_25 ;
}
void F_38 ( struct V_17 * V_25 )
{
F_37 ( V_25 -> V_2 ) ;
}
void F_39 ( struct V_18 * V_2 )
{
F_22 ( L_6 ) ;
if ( ! V_2 ) {
F_24 ( L_9 ) ;
return;
}
F_40 ( V_2 ) ;
F_37 ( V_2 ) ;
}
void F_41 ( struct V_18 * V_2 )
{
F_40 ( V_2 ) ;
F_42 ( & V_43 ) ;
F_43 ( V_2 ) ;
if ( V_2 -> V_44 == 0 ) {
F_39 ( V_2 ) ;
}
F_44 ( & V_43 ) ;
}
static struct V_45 * F_45 ( struct V_46 * V_47 , int V_26 ,
const char * V_48 , void * V_49 )
{
return F_46 ( V_47 ,
L_10 ,
& V_50 ,
& V_51 ,
0x010203ff ) ;
}
static struct V_52 * F_47 ( void )
{
struct V_52 * V_52 ;
int V_27 ;
V_27 = F_48 ( & V_53 , & V_54 , & V_55 ) ;
if ( V_27 < 0 ) {
F_24 ( L_11 , V_27 ) ;
return F_35 ( V_27 ) ;
}
V_52 = F_49 ( V_54 -> V_56 ) ;
if ( F_15 ( V_52 ) )
F_50 ( & V_54 , & V_55 ) ;
return V_52 ;
}
static void F_51 ( struct V_52 * V_52 )
{
if ( V_52 ) {
F_52 ( V_52 ) ;
F_50 ( & V_54 , & V_55 ) ;
}
}
int F_53 ( struct V_18 * V_2 ,
struct V_57 * V_58 ,
struct V_1 * V_59 )
{
int V_38 ;
F_54 ( & V_2 -> V_60 ) ;
V_2 -> V_2 = V_59 ;
V_2 -> V_58 = V_58 ;
F_55 ( & V_2 -> V_61 ) ;
F_55 ( & V_2 -> V_62 ) ;
F_55 ( & V_2 -> V_63 ) ;
F_55 ( & V_2 -> V_64 ) ;
F_55 ( & V_2 -> V_65 ) ;
F_56 ( & V_2 -> V_66 ) ;
F_56 ( & V_2 -> V_67 ) ;
F_57 ( & V_2 -> V_68 ) ;
F_57 ( & V_2 -> V_69 ) ;
F_57 ( & V_2 -> V_70 ) ;
F_57 ( & V_2 -> V_71 ) ;
V_2 -> V_72 = F_47 () ;
if ( F_15 ( V_2 -> V_72 ) ) {
V_38 = F_16 ( V_2 -> V_72 ) ;
F_24 ( L_12 , V_38 ) ;
goto V_33;
}
if ( F_58 ( V_2 , V_73 ) ) {
V_38 = F_7 ( V_2 , V_21 ) ;
if ( V_38 )
goto V_74;
}
V_38 = F_7 ( V_2 , V_19 ) ;
if ( V_38 )
goto V_74;
V_38 = F_59 ( & V_2 -> V_75 , 12 ) ;
if ( V_38 )
goto V_74;
F_60 ( V_2 ) ;
if ( F_58 ( V_2 , V_76 ) ) {
V_38 = F_61 ( V_2 ) ;
if ( V_38 ) {
F_24 ( L_13 ) ;
goto V_77;
}
}
V_38 = F_62 ( V_2 , V_59 ? V_48 ( V_59 ) : V_58 -> V_78 ) ;
if ( V_38 )
goto V_79;
return 0 ;
V_79:
if ( F_58 ( V_2 , V_76 ) )
F_63 ( V_2 ) ;
V_77:
F_64 ( V_2 ) ;
F_65 ( & V_2 -> V_75 ) ;
V_74:
F_19 ( V_2 , V_19 ) ;
F_19 ( V_2 , V_21 ) ;
F_19 ( V_2 , V_23 ) ;
F_51 ( V_2 -> V_72 ) ;
V_33:
F_66 ( & V_2 -> V_71 ) ;
F_66 ( & V_2 -> V_70 ) ;
F_66 ( & V_2 -> V_69 ) ;
F_66 ( & V_2 -> V_68 ) ;
return V_38 ;
}
struct V_18 * F_67 ( struct V_57 * V_58 ,
struct V_1 * V_59 )
{
struct V_18 * V_2 ;
int V_38 ;
V_2 = F_8 ( sizeof( * V_2 ) , V_28 ) ;
if ( ! V_2 )
return F_35 ( - V_29 ) ;
V_38 = F_53 ( V_2 , V_58 , V_59 ) ;
if ( V_38 ) {
F_18 ( V_2 ) ;
return F_35 ( V_38 ) ;
}
return V_2 ;
}
static void F_68 ( struct V_80 * V_60 )
{
struct V_18 * V_2 = F_69 ( V_60 , struct V_18 , V_60 ) ;
if ( F_58 ( V_2 , V_76 ) )
F_63 ( V_2 ) ;
F_64 ( V_2 ) ;
F_65 ( & V_2 -> V_75 ) ;
F_51 ( V_2 -> V_72 ) ;
F_19 ( V_2 , V_19 ) ;
F_19 ( V_2 , V_21 ) ;
F_19 ( V_2 , V_23 ) ;
F_66 ( & V_2 -> V_71 ) ;
F_66 ( & V_2 -> V_70 ) ;
F_66 ( & V_2 -> V_69 ) ;
F_66 ( & V_2 -> V_68 ) ;
F_18 ( V_2 -> V_81 ) ;
F_18 ( V_2 ) ;
}
void F_34 ( struct V_18 * V_2 )
{
if ( V_2 )
F_70 ( & V_2 -> V_60 ) ;
}
void F_37 ( struct V_18 * V_2 )
{
if ( V_2 )
F_71 ( & V_2 -> V_60 , F_68 ) ;
}
static int F_72 ( struct V_18 * V_2 )
{
struct V_17 * V_25 ;
char * V_78 ;
int V_38 ;
if ( ! F_58 ( V_2 , V_82 ) )
return 0 ;
V_25 = * F_6 ( V_2 , V_19 ) ;
if ( ! V_25 )
return 0 ;
V_78 = F_73 ( V_28 , L_14 , V_25 -> V_34 + 64 ) ;
if ( ! V_78 )
return - V_29 ;
V_38 = F_74 ( V_25 -> V_35 -> V_83 . V_59 ,
& V_25 -> V_35 -> V_83 ,
V_78 ) ;
F_18 ( V_78 ) ;
return V_38 ;
}
static void F_75 ( struct V_18 * V_2 )
{
struct V_17 * V_25 ;
char * V_78 ;
if ( ! F_58 ( V_2 , V_82 ) )
return;
V_25 = * F_6 ( V_2 , V_19 ) ;
if ( ! V_25 )
return;
V_78 = F_73 ( V_28 , L_14 , V_25 -> V_34 ) ;
if ( ! V_78 )
return;
F_76 ( V_25 -> V_35 -> V_83 . V_59 , V_78 ) ;
F_18 ( V_78 ) ;
}
int F_77 ( struct V_18 * V_2 , unsigned long V_26 )
{
int V_38 ;
F_42 ( & V_43 ) ;
V_38 = F_21 ( V_2 , V_23 ) ;
if ( V_38 )
goto V_74;
V_38 = F_21 ( V_2 , V_21 ) ;
if ( V_38 )
goto V_74;
V_38 = F_21 ( V_2 , V_19 ) ;
if ( V_38 )
goto V_74;
V_38 = F_72 ( V_2 ) ;
if ( V_38 )
goto V_74;
V_2 -> V_84 = true ;
if ( V_2 -> V_58 -> V_85 ) {
V_38 = V_2 -> V_58 -> V_85 ( V_2 , V_26 ) ;
if ( V_38 )
goto V_74;
}
if ( F_58 ( V_2 , V_82 ) )
F_78 ( V_2 ) ;
V_38 = 0 ;
goto V_86;
V_74:
F_75 ( V_2 ) ;
F_28 ( V_2 , V_19 ) ;
F_28 ( V_2 , V_21 ) ;
F_28 ( V_2 , V_23 ) ;
V_86:
F_44 ( & V_43 ) ;
return V_38 ;
}
void F_40 ( struct V_18 * V_2 )
{
struct V_87 * V_88 , * V_89 ;
F_79 ( V_2 ) ;
V_2 -> V_84 = false ;
if ( F_58 ( V_2 , V_82 ) )
F_80 ( V_2 ) ;
if ( V_2 -> V_58 -> V_90 )
V_2 -> V_58 -> V_90 ( V_2 ) ;
if ( V_2 -> V_91 )
F_81 ( V_2 ) ;
F_82 ( V_2 ) ;
F_83 (r_list, list_temp, &dev->maplist, head)
F_84 ( V_2 , V_88 -> V_92 ) ;
F_75 ( V_2 ) ;
F_28 ( V_2 , V_19 ) ;
F_28 ( V_2 , V_21 ) ;
F_28 ( V_2 , V_23 ) ;
}
int F_62 ( struct V_18 * V_2 , const char * V_78 )
{
F_18 ( V_2 -> V_81 ) ;
V_2 -> V_81 = F_85 ( V_78 , V_28 ) ;
return V_2 -> V_81 ? 0 : - V_29 ;
}
static int F_86 ( struct V_52 * V_52 , struct V_93 * V_94 )
{
const struct V_95 * V_96 ;
struct V_17 * V_25 ;
int V_97 ;
F_22 ( L_6 ) ;
F_42 ( & V_43 ) ;
V_25 = F_32 ( F_87 ( V_52 ) ) ;
if ( F_15 ( V_25 ) ) {
V_97 = F_16 ( V_25 ) ;
goto V_86;
}
V_96 = F_88 ( V_25 -> V_2 -> V_58 -> V_98 ) ;
if ( ! V_96 ) {
V_97 = - V_42 ;
goto V_99;
}
F_89 ( V_94 , V_96 ) ;
if ( V_94 -> V_100 -> V_101 )
V_97 = V_94 -> V_100 -> V_101 ( V_52 , V_94 ) ;
else
V_97 = 0 ;
V_99:
F_38 ( V_25 ) ;
V_86:
F_44 ( & V_43 ) ;
return V_97 ;
}
static void F_90 ( void )
{
F_91 ( V_102 , L_15 ) ;
F_92 ( V_39 ) ;
F_93 () ;
F_94 ( & V_31 ) ;
F_95 () ;
F_96 () ;
}
static int T_2 F_97 ( void )
{
int V_38 ;
F_98 () ;
F_99 () ;
F_100 ( & V_31 ) ;
V_38 = F_101 () ;
if ( V_38 < 0 ) {
F_24 ( L_16 , V_38 ) ;
goto error;
}
V_39 = F_102 ( L_17 , NULL ) ;
if ( ! V_39 ) {
V_38 = - V_29 ;
F_24 ( L_18 , V_38 ) ;
goto error;
}
V_38 = F_103 ( V_102 , L_15 , & V_103 ) ;
if ( V_38 < 0 )
goto error;
F_104 ( L_19 ) ;
return 0 ;
error:
F_90 () ;
return V_38 ;
}
