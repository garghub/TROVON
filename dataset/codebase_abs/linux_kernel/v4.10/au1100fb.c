static int F_1 ( int V_1 , struct V_2 * V_3 )
{
struct V_4 * V_5 = F_2 ( V_3 ) ;
F_3 ( L_1 , V_1 , V_3 ) ;
switch ( V_1 ) {
case V_6 :
V_5 -> V_7 -> V_8 |= V_9 ;
F_4 () ;
break;
case V_10 :
case V_11 :
case V_12 :
V_5 -> V_7 -> V_8 &= ~ V_9 ;
F_4 () ;
break;
default:
break;
}
return 0 ;
}
int F_5 ( struct V_4 * V_5 )
{
struct V_2 * V_13 = & V_5 -> V_13 ;
T_1 V_14 ;
int V_15 ;
if ( ! V_5 )
return - V_16 ;
if ( F_6 ( V_5 -> V_17 ) || F_7 ( V_5 -> V_17 ) ) {
if ( V_13 -> V_18 . V_19 <= 8 ) {
V_13 -> V_18 . V_20 . V_21 = 0 ;
V_13 -> V_18 . V_20 . V_22 = V_13 -> V_18 . V_19 ;
V_13 -> V_18 . V_20 . V_23 = 0 ;
V_13 -> V_18 . V_24 . V_21 = 0 ;
V_13 -> V_18 . V_24 . V_22 = V_13 -> V_18 . V_19 ;
V_13 -> V_18 . V_24 . V_23 = 0 ;
V_13 -> V_18 . V_25 . V_21 = 0 ;
V_13 -> V_18 . V_25 . V_22 = V_13 -> V_18 . V_19 ;
V_13 -> V_18 . V_25 . V_23 = 0 ;
V_13 -> V_18 . V_26 . V_21 = 0 ;
V_13 -> V_18 . V_26 . V_22 = 0 ;
V_13 -> V_18 . V_26 . V_23 = 0 ;
V_13 -> V_27 . V_28 = V_29 ;
V_13 -> V_27 . V_30 = V_13 -> V_18 . V_31 /
( 8 / V_13 -> V_18 . V_19 ) ;
} else {
V_15 = ( V_5 -> V_17 -> V_32 & V_33 ) >> V_34 ;
V_13 -> V_18 . V_20 = V_35 [ V_15 ] [ 0 ] ;
V_13 -> V_18 . V_24 = V_35 [ V_15 ] [ 1 ] ;
V_13 -> V_18 . V_25 = V_35 [ V_15 ] [ 2 ] ;
V_13 -> V_18 . V_26 = V_35 [ V_15 ] [ 3 ] ;
V_13 -> V_27 . V_28 = V_36 ;
V_13 -> V_27 . V_30 = V_13 -> V_18 . V_31 << 1 ;
}
} else {
V_13 -> V_27 . V_28 = V_37 ;
V_13 -> V_27 . V_30 = V_13 -> V_18 . V_31 / 8 ;
}
V_13 -> V_38 = V_13 -> V_27 . V_30 * V_13 -> V_18 . V_39 ;
V_13 -> V_18 . V_40 = ( ( V_5 -> V_17 -> V_32 & V_41 ) \
>> V_42 ) * 90 ;
V_5 -> V_7 -> V_8 = V_5 -> V_17 -> V_32 ;
V_5 -> V_7 -> V_43 = V_5 -> V_17 -> V_44 ;
V_5 -> V_7 -> V_45 = V_5 -> V_17 -> V_46 ;
V_5 -> V_7 -> V_47 = V_5 -> V_17 -> V_48 ;
V_5 -> V_7 -> V_49 = 0 ;
V_5 -> V_7 -> V_50 = 0 ;
V_5 -> V_7 -> V_51 = F_8 ( V_5 -> V_52 ) ;
if ( F_9 ( V_5 -> V_17 ) ) {
if ( V_13 -> V_18 . V_39 >= ( V_13 -> V_18 . V_53 << 1 ) ) {
V_5 -> V_7 -> V_54 = F_8 ( V_5 -> V_52 +
( V_13 -> V_27 . V_30 *
( V_13 -> V_18 . V_39 >> 1 ) ) ) ;
} else {
V_5 -> V_7 -> V_54 = F_8 ( V_5 -> V_52 ) ;
}
}
V_14 = V_13 -> V_27 . V_30 / sizeof( T_1 ) ;
if ( ! V_13 -> V_18 . V_40 || ( V_13 -> V_18 . V_40 == 180 ) ) {
V_14 *= V_13 -> V_18 . V_39 ;
if ( V_13 -> V_18 . V_40 ) {
V_14 -= ( V_14 % 8 ) ;
}
}
V_5 -> V_7 -> V_55 = F_10 ( V_14 ) ;
V_5 -> V_7 -> V_56 = 0 ;
V_5 -> V_7 -> V_57 = 0 ;
V_5 -> V_7 -> V_8 |= V_9 ;
F_11 ( 10 ) ;
F_1 ( V_6 , V_13 ) ;
return 0 ;
}
int F_12 ( unsigned V_58 , unsigned V_20 , unsigned V_24 , unsigned V_25 , unsigned V_26 , struct V_2 * V_3 )
{
struct V_4 * V_5 ;
T_1 * V_59 ;
T_1 V_60 ;
V_5 = F_2 ( V_3 ) ;
V_59 = V_5 -> V_7 -> V_61 ;
if ( V_58 > ( V_62 - 1 ) )
return - V_16 ;
if ( V_3 -> V_18 . V_63 ) {
V_20 = V_24 = V_25 =
( 19595 * V_20 + 38470 * V_24 + 7471 * V_25 ) >> 16 ;
}
if ( V_3 -> V_27 . V_28 == V_36 ) {
if ( V_58 > 16 )
return - V_16 ;
V_59 = ( T_1 * ) V_3 -> V_64 ;
V_20 >>= ( 16 - V_3 -> V_18 . V_20 . V_22 ) ;
V_24 >>= ( 16 - V_3 -> V_18 . V_24 . V_22 ) ;
V_25 >>= ( 16 - V_3 -> V_18 . V_25 . V_22 ) ;
V_60 = ( V_20 << V_3 -> V_18 . V_20 . V_21 ) |
( V_24 << V_3 -> V_18 . V_24 . V_21 ) |
( V_25 << V_3 -> V_18 . V_25 . V_21 ) ;
V_60 &= 0xFFFF ;
} else if ( F_6 ( V_5 -> V_17 ) ) {
V_60 = ( V_20 & 0xF800 ) | ( ( V_24 >> 5 ) & 0x07E0 ) | ( ( V_25 >> 11 ) & 0x001F ) ;
V_60 &= 0xFFFF ;
} else if ( F_7 ( V_5 -> V_17 ) ) {
V_60 = ( ( ( F_13 ( V_5 -> V_17 ) ? V_25 : V_20 ) >> 12 ) & 0x000F ) |
( ( V_24 >> 8 ) & 0x00F0 ) |
( ( ( F_13 ( V_5 -> V_17 ) ? V_20 : V_25 ) >> 4 ) & 0x0F00 ) ;
V_60 &= 0xFFF ;
} else {
V_60 = ( V_24 >> 12 ) & 0x000F ;
V_60 &= 0xF ;
}
V_59 [ V_58 ] = V_60 ;
return 0 ;
}
int F_14 ( struct V_65 * V_18 , struct V_2 * V_3 )
{
struct V_4 * V_5 ;
int V_66 ;
V_5 = F_2 ( V_3 ) ;
F_3 ( L_2 , V_18 , V_3 ) ;
if ( ! V_18 || ! V_5 ) {
return - V_16 ;
}
if ( V_18 -> V_67 - V_3 -> V_18 . V_67 ) {
return - V_16 ;
}
F_3 ( L_3 , V_18 , V_3 ) ;
V_66 = V_18 -> V_68 - V_3 -> V_18 . V_68 ;
if ( V_66 ) {
T_1 V_69 ;
F_3 ( L_4 , V_66 ) ;
V_69 = V_5 -> V_7 -> V_51 ;
V_69 += ( V_3 -> V_27 . V_30 * V_66 ) ;
V_5 -> V_7 -> V_51 = F_8 ( V_69 ) ;
if ( F_9 ( V_5 -> V_17 ) ) {
V_69 = V_5 -> V_7 -> V_54 ;
V_69 += ( V_3 -> V_27 . V_30 * V_66 ) ;
V_5 -> V_7 -> V_51 = F_8 ( V_69 ) ;
}
}
F_3 ( L_5 , V_18 , V_3 ) ;
return 0 ;
}
int F_15 ( struct V_2 * V_3 , struct V_70 * V_71 )
{
struct V_4 * V_5 ;
V_5 = F_2 ( V_3 ) ;
V_71 -> V_72 = F_16 ( V_71 -> V_72 ) ;
F_17 ( V_71 -> V_72 ) |= ( 6 << 9 ) ;
return F_18 ( V_71 , V_5 -> V_52 , V_5 -> V_73 ) ;
}
static int F_19 ( struct V_4 * V_5 )
{
char * V_74 , * V_75 ;
int V_76 = F_20 ( V_77 ) ;
if ( V_76 <= 0 ) {
F_21 ( L_6 ) ;
return - V_78 ;
}
if ( F_22 ( V_79 , & V_75 ) )
return - V_78 ;
if ( ! V_75 )
return - V_78 ;
while ( ( V_74 = F_23 ( & V_75 , L_7 ) ) != NULL ) {
if ( ! strncmp ( V_74 , L_8 , 6 ) ) {
int V_80 ;
V_74 += 6 ;
for ( V_80 = 0 ; V_80 < V_76 ; V_80 ++ ) {
if ( ! strncmp ( V_74 , V_77 [ V_80 ] . V_81 ,
strlen ( V_74 ) ) ) {
V_5 -> V_17 = & V_77 [ V_80 ] ;
V_5 -> V_82 = V_80 ;
break;
}
}
if ( V_80 >= V_76 ) {
F_24 ( L_9 , V_74 ) ;
return - V_78 ;
}
}
else
F_24 ( L_10 , V_74 ) ;
}
F_25 ( L_11 , V_5 -> V_17 -> V_81 ) ;
return 0 ;
}
static int F_26 ( struct V_83 * V_84 )
{
struct V_4 * V_5 = NULL ;
struct V_85 * V_86 ;
unsigned long V_87 ;
struct V_88 * V_89 ;
V_5 = F_27 ( & V_84 -> V_84 , sizeof( struct V_4 ) ,
V_90 ) ;
if ( ! V_5 ) {
F_21 ( L_12 ) ;
return - V_91 ;
}
if ( F_19 ( V_5 ) )
goto V_92;
F_28 ( V_84 , ( void * ) V_5 ) ;
V_86 = F_29 ( V_84 , V_93 , 0 ) ;
if ( ! V_86 ) {
F_21 ( L_13 ) ;
return - V_94 ;
}
V_95 . V_96 = V_86 -> V_97 ;
V_95 . V_98 = F_30 ( V_86 ) ;
if ( ! F_31 ( & V_84 -> V_84 ,
V_95 . V_96 ,
V_95 . V_98 ,
V_79 ) ) {
F_21 ( L_14 ,
V_95 . V_96 ) ;
return - V_99 ;
}
V_5 -> V_7 = (struct V_100 * ) F_32 ( V_95 . V_96 ) ;
F_3 ( L_15 , V_5 -> V_7 ) ;
F_3 ( L_16 , V_5 -> V_101 , V_5 -> V_102 ) ;
V_89 = F_33 ( NULL , L_17 ) ;
if ( ! F_34 ( V_89 ) ) {
V_5 -> V_103 = V_89 ;
F_35 ( V_89 , 48000000 ) ;
F_36 ( V_89 ) ;
}
V_5 -> V_73 = V_5 -> V_17 -> V_104 * V_5 -> V_17 -> V_53 *
( V_5 -> V_17 -> V_105 >> 3 ) * V_106 ;
V_5 -> V_107 = F_37 ( & V_84 -> V_84 ,
F_38 ( V_5 -> V_73 ) ,
& V_5 -> V_52 , V_90 ) ;
if ( ! V_5 -> V_107 ) {
F_21 ( L_18 ,
V_5 -> V_73 / 1024 ) ;
return - V_91 ;
}
V_95 . V_108 = V_5 -> V_52 ;
V_95 . V_109 = V_5 -> V_73 ;
for ( V_87 = ( unsigned long ) V_5 -> V_107 ;
V_87 < F_38 ( ( unsigned long ) V_5 -> V_107 + V_5 -> V_73 ) ;
V_87 += V_110 ) {
#ifdef F_39
F_40 ( F_41 ( F_42 ( ( void * ) V_87 ) ) ) ;
#else
F_40 ( F_41 ( V_87 ) ) ;
#endif
}
F_3 ( L_19 , V_5 -> V_107 ) ;
F_3 ( L_20 , V_5 -> V_52 , V_5 -> V_73 / 1024 ) ;
V_111 . V_19 = V_5 -> V_17 -> V_105 ;
V_111 . V_104 = V_5 -> V_17 -> V_104 ;
V_111 . V_31 = V_111 . V_104 ;
V_111 . V_53 = V_5 -> V_17 -> V_53 ;
V_111 . V_39 = V_111 . V_53 ;
V_5 -> V_13 . V_112 = V_5 -> V_107 ;
V_5 -> V_13 . V_113 = & V_114 ;
V_5 -> V_13 . V_27 = V_95 ;
V_5 -> V_13 . V_64 =
F_27 ( & V_84 -> V_84 , sizeof( T_1 ) * 16 , V_90 ) ;
if ( ! V_5 -> V_13 . V_64 )
return - V_91 ;
if ( F_43 ( & V_5 -> V_13 . V_115 , V_62 , 0 ) < 0 ) {
F_21 ( L_21 ,
V_62 ) ;
return - V_94 ;
}
V_5 -> V_13 . V_18 = V_111 ;
F_5 ( V_5 ) ;
if ( F_44 ( & V_5 -> V_13 ) < 0 ) {
F_21 ( L_22 ) ;
goto V_92;
}
return 0 ;
V_92:
if ( V_5 -> V_103 ) {
F_45 ( V_5 -> V_103 ) ;
F_46 ( V_5 -> V_103 ) ;
}
if ( V_5 -> V_107 ) {
F_47 ( & V_84 -> V_84 , V_5 -> V_73 , V_5 -> V_107 ,
V_5 -> V_52 ) ;
}
if ( V_5 -> V_13 . V_115 . V_116 != 0 ) {
F_48 ( & V_5 -> V_13 . V_115 ) ;
}
return - V_78 ;
}
int F_49 ( struct V_83 * V_84 )
{
struct V_4 * V_5 = NULL ;
if ( ! V_84 )
return - V_78 ;
V_5 = F_50 ( V_84 ) ;
#if ! F_51 ( V_117 ) && F_51 ( V_118 )
F_1 ( V_12 , & V_5 -> V_13 ) ;
#endif
V_5 -> V_7 -> V_8 &= ~ V_9 ;
F_52 ( & V_5 -> V_13 ) ;
F_48 ( & V_5 -> V_13 . V_115 ) ;
if ( V_5 -> V_103 ) {
F_45 ( V_5 -> V_103 ) ;
F_46 ( V_5 -> V_103 ) ;
}
return 0 ;
}
int F_53 ( struct V_83 * V_84 , T_2 V_119 )
{
struct V_4 * V_5 = F_50 ( V_84 ) ;
if ( ! V_5 )
return 0 ;
F_1 ( V_12 , & V_5 -> V_13 ) ;
if ( V_5 -> V_103 )
F_54 ( V_5 -> V_103 ) ;
memcpy ( & V_120 , V_5 -> V_7 , sizeof( struct V_100 ) ) ;
return 0 ;
}
int F_55 ( struct V_83 * V_84 )
{
struct V_4 * V_5 = F_50 ( V_84 ) ;
if ( ! V_5 )
return 0 ;
memcpy ( V_5 -> V_7 , & V_120 , sizeof( struct V_100 ) ) ;
if ( V_5 -> V_103 )
F_56 ( V_5 -> V_103 ) ;
F_1 ( V_6 , & V_5 -> V_13 ) ;
return 0 ;
}
