static inline struct V_1 *
F_1 ( struct V_2 * V_3 )
{
return F_2 ( V_3 , struct V_1 , V_4 . V_4 ) ;
}
static inline struct V_5 *
F_3 ( struct V_6 * V_7 )
{
return F_2 ( V_7 , struct V_5 , V_4 ) ;
}
static void F_4 ( struct V_8 * V_9 )
{
int V_10 ;
for ( V_10 = 0 ; V_10 < F_5 ( V_11 ) ; V_10 ++ ) {
F_6 ( L_1 ,
V_11 [ V_10 ] . V_12 , V_11 [ V_10 ] . V_13 ,
F_7 ( V_11 [ V_10 ] . V_12 ) ) ;
}
}
int F_8 ( struct V_14 * V_15 , void * V_16 )
{
struct V_17 * V_18 = (struct V_17 * ) V_15 -> V_19 ;
struct V_20 * V_21 = V_18 -> V_22 -> V_21 ;
struct V_23 * V_24 = F_9 ( V_21 ) ;
struct V_8 * V_9 = V_24 -> V_9 ;
int V_10 ;
if ( ! V_9 )
return 0 ;
for ( V_10 = 0 ; V_10 < F_5 ( V_11 ) ; V_10 ++ ) {
F_10 ( V_15 , L_2 ,
V_11 [ V_10 ] . V_13 , V_11 [ V_10 ] . V_12 ,
F_7 ( V_11 [ V_10 ] . V_12 ) ) ;
}
return 0 ;
}
static enum V_25
F_11 ( struct V_6 * V_7 , bool V_26 )
{
struct V_5 * V_27 =
F_3 ( V_7 ) ;
struct V_8 * V_9 = V_27 -> V_9 ;
if ( V_9 -> V_28 )
return V_29 ;
else
return V_30 ;
}
static void F_12 ( struct V_6 * V_7 )
{
F_13 ( V_7 ) ;
F_14 ( V_7 ) ;
}
static int F_15 ( struct V_6 * V_7 )
{
struct V_5 * V_27 =
F_3 ( V_7 ) ;
struct V_8 * V_9 = V_27 -> V_9 ;
if ( V_9 -> V_28 )
return F_16 ( V_9 -> V_28 ) ;
return 0 ;
}
static struct V_6 * F_17 ( struct V_20 * V_21 ,
struct V_8 * V_9 )
{
struct V_6 * V_7 = NULL ;
struct V_5 * V_31 ;
V_31 = F_18 ( V_21 -> V_21 , sizeof( * V_31 ) ,
V_32 ) ;
if ( ! V_31 )
return F_19 ( - V_33 ) ;
V_7 = & V_31 -> V_4 ;
V_31 -> V_3 = V_9 -> V_3 ;
V_31 -> V_9 = V_9 ;
F_20 ( V_21 , V_7 , & V_34 ,
V_35 ) ;
F_21 ( V_7 , & V_36 ) ;
V_7 -> V_37 = 0 ;
V_7 -> V_38 = 0 ;
V_7 -> V_39 = 0 ;
F_22 ( V_7 , V_9 -> V_3 ) ;
return V_7 ;
}
static void F_23 ( struct V_2 * V_3 )
{
struct V_1 * V_40 = F_1 ( V_3 ) ;
struct V_8 * V_9 = V_40 -> V_9 ;
F_24 ( V_9 -> V_28 ) ;
F_25 ( V_9 -> V_41 ) ;
F_26 ( V_9 -> V_28 ) ;
}
static void F_27 ( struct V_2 * V_3 )
{
struct V_42 * V_43 = & V_3 -> V_44 -> V_43 ;
struct V_1 * V_40 = F_1 ( V_3 ) ;
struct V_8 * V_9 = V_40 -> V_9 ;
T_1 V_45 = V_46 | V_47 ;
int V_48 ;
V_48 = F_28 ( V_9 -> V_28 ) ;
if ( V_48 ) {
F_29 ( L_3 ) ;
return;
}
if ( V_9 -> V_7 -> V_49 . V_50 ) {
T_1 V_51 = V_9 -> V_7 -> V_49 . V_52 [ 0 ] ;
switch ( V_51 ) {
case V_53 :
V_45 |= F_30 ( V_54 ,
V_55 ) ;
break;
case V_56 :
V_45 |= F_30 ( V_54 ,
V_55 ) ;
V_45 |= F_30 ( V_57 , V_58 ) ;
break;
case V_59 :
V_45 |= F_30 ( V_60 ,
V_55 ) ;
break;
case V_61 :
V_45 |= F_30 ( V_62 ,
V_55 ) ;
break;
case V_63 :
V_45 |= F_30 ( V_64 ,
V_55 ) ;
break;
default:
F_29 ( L_4 , V_51 ) ;
break;
}
}
if ( V_43 -> V_65 & V_66 )
V_45 |= V_67 ;
else if ( ! ( V_43 -> V_65 & V_68 ) )
V_45 |= V_69 ;
if ( V_43 -> V_65 & V_70 )
V_45 |= V_71 ;
else if ( ! ( V_43 -> V_65 & V_72 ) )
V_45 |= V_73 ;
F_31 ( V_74 , V_45 ) ;
V_48 = F_32 ( V_9 -> V_41 , V_43 -> clock * 1000 ) ;
if ( V_48 )
F_29 ( L_5 , V_48 ) ;
V_48 = F_33 ( V_9 -> V_41 ) ;
if ( V_48 )
F_29 ( L_5 , V_48 ) ;
V_48 = F_34 ( V_9 -> V_28 ) ;
if ( V_48 ) {
F_29 ( L_6 ) ;
F_26 ( V_9 -> V_28 ) ;
return;
}
}
static bool F_35 ( struct V_2 * V_3 ,
const struct V_42 * V_43 ,
struct V_42 * V_75 )
{
if ( V_75 -> V_65 & V_76 )
return false ;
return true ;
}
static struct V_77 * F_36 ( struct V_78 * V_21 )
{
struct V_79 * V_80 , * V_81 ;
struct V_79 * V_82 = V_21 -> V_83 ;
struct V_77 * V_28 ;
V_80 = F_37 ( V_82 , NULL ) ;
if ( ! V_80 ) {
F_38 ( V_21 , L_7 ) ;
return NULL ;
}
V_81 = F_39 ( V_80 ) ;
F_40 ( V_80 ) ;
if ( ! V_81 ) {
F_38 ( V_21 , L_8 ) ;
return NULL ;
}
V_28 = F_41 ( V_81 ) ;
F_40 ( V_81 ) ;
return V_28 ;
}
static int F_42 ( struct V_78 * V_21 , struct V_78 * V_84 , void * V_85 )
{
struct V_86 * V_87 = F_43 ( V_21 ) ;
struct V_20 * V_88 = F_44 ( V_84 ) ;
struct V_23 * V_24 = F_9 ( V_88 ) ;
struct V_8 * V_9 ;
struct V_1 * V_1 ;
int V_48 ;
V_9 = F_18 ( V_21 , sizeof( * V_9 ) , V_32 ) ;
if ( ! V_9 )
return - V_33 ;
V_1 = F_18 ( V_21 , sizeof( * V_1 ) ,
V_32 ) ;
if ( ! V_1 )
return - V_33 ;
V_1 -> V_4 . type = V_89 ;
V_1 -> V_9 = V_9 ;
V_9 -> V_3 = & V_1 -> V_4 . V_4 ;
V_9 -> V_87 = V_87 ;
V_9 -> V_90 = F_45 ( V_87 , 0 ) ;
if ( F_46 ( V_9 -> V_90 ) )
return F_47 ( V_9 -> V_90 ) ;
F_4 ( V_9 ) ;
if ( F_7 ( V_91 ) != V_92 ) {
F_38 ( V_21 , L_9 ,
F_7 ( V_91 ) , V_92 ) ;
return - V_93 ;
}
V_9 -> V_94 = F_48 ( V_21 , L_10 ) ;
if ( F_46 ( V_9 -> V_94 ) ) {
V_48 = F_47 ( V_9 -> V_94 ) ;
if ( V_48 != - V_95 )
F_29 ( L_11 , V_48 ) ;
return V_48 ;
}
V_9 -> V_41 = F_48 ( V_21 , L_12 ) ;
if ( F_46 ( V_9 -> V_41 ) ) {
V_48 = F_47 ( V_9 -> V_41 ) ;
if ( V_48 != - V_95 )
F_29 ( L_13 , V_48 ) ;
return V_48 ;
}
V_48 = F_33 ( V_9 -> V_94 ) ;
if ( V_48 )
F_29 ( L_14 , V_48 ) ;
V_9 -> V_28 = F_36 ( V_21 ) ;
F_49 ( V_88 , V_9 -> V_3 , & V_96 ,
V_97 , NULL ) ;
F_50 ( V_9 -> V_3 , & V_98 ) ;
V_9 -> V_7 = F_17 ( V_88 , V_9 ) ;
if ( F_46 ( V_9 -> V_7 ) ) {
V_48 = F_47 ( V_9 -> V_7 ) ;
goto V_99;
}
if ( V_9 -> V_28 )
F_51 ( V_9 -> V_28 , V_9 -> V_7 ) ;
F_52 ( V_21 , V_9 ) ;
V_24 -> V_9 = V_9 ;
return 0 ;
V_99:
F_53 ( V_9 -> V_3 ) ;
F_25 ( V_9 -> V_94 ) ;
return V_48 ;
}
static void F_54 ( struct V_78 * V_21 , struct V_78 * V_84 ,
void * V_85 )
{
struct V_20 * V_88 = F_44 ( V_84 ) ;
struct V_23 * V_24 = F_9 ( V_88 ) ;
struct V_8 * V_9 = F_44 ( V_21 ) ;
if ( V_9 -> V_28 )
F_55 ( V_9 -> V_28 ) ;
F_12 ( V_9 -> V_7 ) ;
F_53 ( V_9 -> V_3 ) ;
F_25 ( V_9 -> V_94 ) ;
V_24 -> V_9 = NULL ;
}
static int F_56 ( struct V_86 * V_87 )
{
return F_57 ( & V_87 -> V_21 , & V_100 ) ;
}
static int F_58 ( struct V_86 * V_87 )
{
F_59 ( & V_87 -> V_21 , & V_100 ) ;
return 0 ;
}
