int F_1 ( void )
{
return V_1 ;
}
struct V_2 * F_2 (
struct V_3 * V_4 ,
unsigned int V_5 ,
unsigned int V_6 )
{
struct V_7 * V_8 = NULL ;
int V_9 ;
for ( V_9 = 0 ; V_9 < V_4 -> V_10 . V_11 ; V_9 ++ ) {
if ( V_4 -> V_8 [ V_9 ] -> V_12 [ 0 ] . V_10 . V_13
== V_5 ) {
V_8 = V_4 -> V_8 [ V_9 ] ;
break;
}
}
if ( ! V_8 )
return NULL ;
for ( V_9 = 0 ; V_9 < V_8 -> V_14 ; V_9 ++ )
if ( V_8 -> V_12 [ V_9 ] . V_10 . V_15 == V_6 )
return & V_8 -> V_12 [ V_9 ] ;
F_3 ( V_16 L_1
L_2 , V_6 , V_5 ,
V_4 -> V_10 . V_17 ) ;
return NULL ;
}
struct V_18 * F_4 ( const struct V_19 * V_20 ,
unsigned V_21 )
{
struct V_3 * V_4 = V_20 -> V_22 ;
int V_9 ;
if ( ! V_4 )
return NULL ;
for ( V_9 = 0 ; V_9 < V_4 -> V_10 . V_11 ; V_9 ++ )
if ( V_4 -> V_23 [ V_9 ] -> V_12 [ 0 ]
. V_10 . V_13 == V_21 )
return V_4 -> V_23 [ V_9 ] ;
return NULL ;
}
struct V_2 * F_5 (
const struct V_18 * V_24 ,
unsigned int V_25 )
{
int V_9 ;
for ( V_9 = 0 ; V_9 < V_24 -> V_14 ; V_9 ++ ) {
if ( V_24 -> V_12 [ V_9 ] . V_10 . V_15 == V_25 )
return & V_24 -> V_12 [ V_9 ] ;
}
return NULL ;
}
static int F_6 ( struct V_26 * V_20 , void * V_27 )
{
struct V_28 * V_29 = V_27 ;
struct V_18 * V_24 ;
if ( ! F_7 ( V_20 ) )
return 0 ;
if ( V_20 -> V_30 != V_29 -> V_31 )
return 0 ;
V_24 = F_8 ( V_20 ) ;
return V_24 -> V_32 == V_29 -> V_32 ;
}
struct V_18 * F_9 ( struct V_33 * V_31 , int V_32 )
{
struct V_28 V_34 ;
struct V_26 * V_20 ;
V_34 . V_32 = V_32 ;
V_34 . V_31 = & V_31 -> V_35 . V_30 ;
V_20 = F_10 ( & V_36 , NULL , & V_34 , F_6 ) ;
F_11 ( V_20 ) ;
return V_20 ? F_8 ( V_20 ) : NULL ;
}
static int F_12 ( struct V_26 * V_20 , void * V_27 )
{
struct V_37 * V_29 = (struct V_37 * ) V_27 ;
if ( ! F_13 ( V_20 ) )
return 0 ;
return V_29 -> V_38 ( F_14 ( V_20 ) , V_29 -> V_27 ) ;
}
int F_15 ( void * V_27 , int (* V_38)( struct V_19 * , void * ) )
{
struct V_37 V_29 = { V_27 , V_38 } ;
return F_16 ( & V_36 , NULL , & V_29 , F_12 ) ;
}
static void F_17 ( struct V_26 * V_20 )
{
struct V_19 * V_39 ;
struct V_40 * V_41 ;
V_39 = F_14 ( V_20 ) ;
V_41 = F_18 ( V_39 -> V_42 ) ;
F_19 ( V_39 ) ;
F_20 ( V_39 ) ;
F_21 ( V_41 ) ;
F_22 ( V_39 -> V_43 ) ;
F_22 ( V_39 -> V_44 ) ;
F_22 ( V_39 -> V_45 ) ;
F_22 ( V_39 ) ;
}
static int F_23 ( struct V_26 * V_20 , struct V_46 * V_47 )
{
struct V_19 * V_48 ;
V_48 = F_14 ( V_20 ) ;
if ( F_24 ( V_47 , L_3 , V_48 -> V_42 -> V_49 ) )
return - V_50 ;
if ( F_24 ( V_47 , L_4 , V_48 -> V_51 ) )
return - V_50 ;
return 0 ;
}
static int F_25 ( struct V_26 * V_20 )
{
return 0 ;
}
static void F_26 ( struct V_26 * V_20 )
{
F_27 ( V_20 ) ;
}
static int F_28 ( struct V_26 * V_20 )
{
return F_29 ( V_20 , V_52 ) ;
}
static int F_30 ( struct V_26 * V_20 )
{
return F_31 ( V_20 , V_53 ) ;
}
static int F_32 ( struct V_26 * V_20 )
{
return F_29 ( V_20 , V_54 ) ;
}
static int F_33 ( struct V_26 * V_20 )
{
return F_31 ( V_20 , V_55 ) ;
}
static int F_34 ( struct V_26 * V_20 )
{
return F_29 ( V_20 , V_56 ) ;
}
static int F_35 ( struct V_26 * V_20 )
{
return F_31 ( V_20 , V_57 ) ;
}
static char * F_36 ( struct V_26 * V_20 ,
T_1 * V_58 , T_2 * V_59 , T_3 * V_60 )
{
struct V_19 * V_48 ;
V_48 = F_14 ( V_20 ) ;
return F_37 ( V_61 , L_5 ,
V_48 -> V_42 -> V_49 , V_48 -> V_51 ) ;
}
static unsigned F_38 ( struct V_62 * V_42 )
{
struct V_40 * V_41 = F_18 ( V_42 ) ;
return V_41 -> V_63 ;
}
struct V_19 * F_39 ( struct V_19 * V_64 ,
struct V_62 * V_42 , unsigned V_65 )
{
struct V_19 * V_20 ;
struct V_40 * V_40 = F_18 ( V_42 ) ;
unsigned V_66 = 0 ;
unsigned V_67 = V_65 ;
V_20 = F_40 ( sizeof( * V_20 ) , V_61 ) ;
if ( ! V_20 )
return NULL ;
if ( ! F_41 ( V_40 ) ) {
F_22 ( V_20 ) ;
return NULL ;
}
if ( V_40 -> V_30 -> V_68 && V_64 &&
! V_40 -> V_30 -> V_68 ( V_40 , V_20 ) ) {
F_21 ( F_18 ( V_42 ) ) ;
F_22 ( V_20 ) ;
return NULL ;
}
F_42 ( & V_20 -> V_20 ) ;
V_20 -> V_20 . V_42 = & V_36 ;
V_20 -> V_20 . type = & V_69 ;
V_20 -> V_20 . V_70 = V_71 ;
V_20 -> V_20 . V_72 = V_42 -> V_73 -> V_72 ;
V_20 -> V_20 . V_74 = V_42 -> V_73 -> V_74 ;
F_43 ( & V_20 -> V_20 , F_44 ( V_42 -> V_73 ) ) ;
V_20 -> V_75 = V_76 ;
V_20 -> V_77 = 1 ;
F_45 ( & V_20 -> V_78 , 0 ) ;
F_46 ( & V_20 -> V_79 . V_80 ) ;
V_20 -> V_79 . V_10 . V_81 = V_82 ;
V_20 -> V_79 . V_10 . V_83 = V_84 ;
F_47 ( V_20 , & V_20 -> V_79 , false ) ;
V_20 -> V_85 = 1 ;
if ( F_48 ( ! V_64 ) ) {
V_20 -> V_86 [ 0 ] = '0' ;
V_20 -> V_87 = 0 ;
V_20 -> V_20 . V_64 = V_42 -> V_73 ;
F_49 ( & V_20 -> V_20 , L_6 , V_42 -> V_49 ) ;
V_66 = 1 ;
} else {
if ( V_64 -> V_86 [ 0 ] == '0' ) {
snprintf ( V_20 -> V_86 , sizeof V_20 -> V_86 ,
L_7 , V_65 ) ;
V_20 -> V_87 = 0 ;
} else {
snprintf ( V_20 -> V_86 , sizeof V_20 -> V_86 ,
L_8 , V_64 -> V_86 , V_65 ) ;
if ( V_65 < 15 )
V_20 -> V_87 = V_64 -> V_87 +
( V_65 << ( ( V_64 -> V_88 - 1 ) * 4 ) ) ;
else
V_20 -> V_87 = V_64 -> V_87 +
( 15 << ( ( V_64 -> V_88 - 1 ) * 4 ) ) ;
}
V_20 -> V_20 . V_64 = & V_64 -> V_20 ;
F_49 ( & V_20 -> V_20 , L_9 , V_42 -> V_49 , V_20 -> V_86 ) ;
if ( ! V_64 -> V_64 ) {
V_67 = F_50 ( V_40 ,
V_65 ) ;
}
V_20 -> V_20 . V_89 = F_51 ( V_64 -> V_20 . V_89 ,
V_67 ) ;
}
V_20 -> V_90 = V_65 ;
V_20 -> V_42 = V_42 ;
V_20 -> V_64 = V_64 ;
F_46 ( & V_20 -> V_91 ) ;
#ifdef F_52
F_53 ( & V_20 -> V_20 ,
V_92 * 1000 ) ;
V_20 -> V_93 = V_94 ;
V_20 -> V_95 = - V_94 ;
#endif
if ( V_66 )
V_20 -> V_96 = 1 ;
else {
V_20 -> V_96 = ! ! F_54 ( V_40 ) ;
V_20 -> V_97 = F_38 ( V_42 ) ? 1 : 0 ;
}
return V_20 ;
}
struct V_19 * F_55 ( struct V_19 * V_20 )
{
if ( V_20 )
F_56 ( & V_20 -> V_20 ) ;
return V_20 ;
}
void F_57 ( struct V_19 * V_20 )
{
if ( V_20 )
F_11 ( & V_20 -> V_20 ) ;
}
struct V_18 * F_58 ( struct V_18 * V_24 )
{
if ( V_24 )
F_56 ( & V_24 -> V_20 ) ;
return V_24 ;
}
void F_59 ( struct V_18 * V_24 )
{
if ( V_24 )
F_11 ( & V_24 -> V_20 ) ;
}
int F_60 ( struct V_19 * V_39 ,
const struct V_18 * V_98 )
{
unsigned long V_99 = V_94 + V_100 ;
if ( V_39 -> V_75 == V_101 )
return - V_102 ;
if ( V_39 -> V_75 == V_103 )
return - V_104 ;
if ( V_98 && ( V_98 -> V_105 == V_106 ||
V_98 -> V_105 == V_107 ) )
return - V_108 ;
while ( ! F_61 ( V_39 ) ) {
if ( F_62 ( V_94 , V_99 ) )
return - V_109 ;
F_63 ( 15 ) ;
if ( V_39 -> V_75 == V_101 )
return - V_102 ;
if ( V_39 -> V_75 == V_103 )
return - V_104 ;
if ( V_98 && ( V_98 -> V_105 == V_106 ||
V_98 -> V_105 == V_107 ) )
return - V_108 ;
}
return 0 ;
}
int F_64 ( struct V_19 * V_20 )
{
return F_65 ( V_20 ) ;
}
int F_66 ( char * V_110 , unsigned V_111 ,
unsigned char type , void * * V_112 )
{
struct V_113 * V_114 ;
while ( V_111 >= sizeof( struct V_113 ) ) {
V_114 = (struct V_113 * ) V_110 ;
if ( V_114 -> V_81 < 2 ) {
F_3 ( V_115
L_10 ,
V_116 ,
V_114 -> V_83 ,
V_114 -> V_81 ) ;
return - 1 ;
}
if ( V_114 -> V_83 == type ) {
* V_112 = V_114 ;
return 0 ;
}
V_110 += V_114 -> V_81 ;
V_111 -= V_114 -> V_81 ;
}
return - 1 ;
}
void * F_67 ( struct V_19 * V_20 , T_4 V_111 , T_5 V_117 ,
T_6 * V_118 )
{
if ( ! V_20 || ! V_20 -> V_42 )
return NULL ;
return F_68 ( V_20 -> V_42 , V_111 , V_117 , V_118 ) ;
}
void F_69 ( struct V_19 * V_20 , T_4 V_111 , void * V_119 ,
T_6 V_118 )
{
if ( ! V_20 || ! V_20 -> V_42 )
return;
if ( ! V_119 )
return;
F_70 ( V_20 -> V_42 , V_111 , V_119 , V_118 ) ;
}
static int F_71 ( struct V_120 * V_121 , unsigned long V_122 ,
void * V_27 )
{
struct V_26 * V_20 = V_27 ;
switch ( V_122 ) {
case V_123 :
if ( V_20 -> type == & V_69 )
( void ) F_72 ( F_14 ( V_20 ) ) ;
else if ( V_20 -> type == & V_124 )
F_73 ( F_8 ( V_20 ) ) ;
break;
case V_125 :
if ( V_20 -> type == & V_69 )
F_74 ( F_14 ( V_20 ) ) ;
else if ( V_20 -> type == & V_124 )
F_75 ( F_8 ( V_20 ) ) ;
break;
}
return 0 ;
}
static int F_76 ( void )
{
V_126 = F_77 ( L_11 , NULL ) ;
if ( ! V_126 )
return - V_127 ;
V_128 = F_78 ( L_12 , 0444 ,
V_126 , NULL ,
& V_129 ) ;
if ( ! V_128 ) {
F_79 ( V_126 ) ;
V_126 = NULL ;
return - V_127 ;
}
return 0 ;
}
static void F_80 ( void )
{
F_79 ( V_128 ) ;
F_79 ( V_126 ) ;
}
static int T_7 F_81 ( void )
{
int V_130 ;
if ( F_1 () ) {
F_82 ( L_13 , V_116 ) ;
return 0 ;
}
F_83 () ;
V_130 = F_76 () ;
if ( V_130 )
goto V_131;
F_84 () ;
V_130 = F_85 ( & V_36 ) ;
if ( V_130 )
goto V_132;
V_130 = F_86 ( & V_36 , & V_133 ) ;
if ( V_130 )
goto V_134;
V_130 = F_87 () ;
if ( V_130 )
goto V_135;
V_130 = F_88 ( & V_136 ) ;
if ( V_130 )
goto V_137;
V_130 = F_89 () ;
if ( V_130 )
goto V_138;
V_130 = F_90 () ;
if ( V_130 )
goto V_139;
V_130 = F_91 ( & V_140 , V_141 ) ;
if ( ! V_130 )
goto V_131;
F_92 () ;
V_139:
F_93 () ;
V_138:
F_94 ( & V_136 ) ;
V_137:
F_95 () ;
V_135:
F_96 ( & V_36 , & V_133 ) ;
V_134:
F_97 ( & V_36 ) ;
V_132:
F_98 () ;
F_80 () ;
V_131:
return V_130 ;
}
static void T_8 F_99 ( void )
{
if ( F_1 () )
return;
F_100 ( & V_140 ) ;
F_95 () ;
F_94 ( & V_136 ) ;
F_93 () ;
F_92 () ;
F_96 ( & V_36 , & V_133 ) ;
F_97 ( & V_36 ) ;
F_98 () ;
F_80 () ;
F_101 ( & V_142 ) ;
}
