static void T_1
F_1 ( void )
{
V_1 = 0 ;
V_2 = 0 ;
V_3 = - 1 ;
V_4 = - 1 ;
V_5 = - 1 ;
V_6 = - 1 ;
V_7 = - 1 ;
V_8 = - 1 ;
V_9 = - 1 ;
V_10 = 0 ;
V_11 = - 1 ;
V_12 = - 1 ;
V_13 = 0 ;
V_14 = 0xff ;
V_15 = 0xff ;
V_16 = - 1 ;
V_17 = V_18 ;
V_19 = - 1 ;
V_20 = 0 ;
V_21 = 0 ;
V_22 = - 1 ;
V_23 = - 1 ;
V_24 = 0 ;
V_25 = 0 ;
V_26 = 0 ;
#if ! F_2 ( V_27 ) && ! F_2 ( V_28 )
V_29 = 0 ;
V_30 = 0 ;
#endif
}
static void F_3 ( unsigned int V_31 , bool V_32 )
{
int V_33 = 0 , V_34 = 0 ;
if( V_31 == 0 ) {
if( ! V_32 )
F_4 ( V_35 L_1 ) ;
V_8 = V_36 ;
return;
}
V_31 &= 0x1dff ;
while( V_37 [ V_33 ++ ] . V_38 [ 0 ] != 0 ) {
if( ( V_37 [ V_33 - 1 ] . V_39 == V_31 ) ||
( V_37 [ V_33 - 1 ] . V_40 == V_31 ) ) {
if( V_21 ) {
if( V_37 [ V_33 - 1 ] . V_38 [ 1 ] == 0x50 ||
V_37 [ V_33 - 1 ] . V_38 [ 1 ] == 0x56 ||
V_37 [ V_33 - 1 ] . V_38 [ 1 ] == 0x53 )
continue;
} else {
if( V_37 [ V_33 - 1 ] . V_38 [ 1 ] == 0x5a ||
V_37 [ V_33 - 1 ] . V_38 [ 1 ] == 0x5b )
continue;
}
V_8 = V_33 - 1 ;
V_34 = 1 ;
break;
}
}
if( ( ! V_34 ) && ! V_32 )
F_4 ( V_35 L_2 , V_31 ) ;
}
static void F_5 ( char * V_41 , bool V_32 )
{
unsigned int V_34 = 0 , V_42 = 0 , V_43 = 0 , V_44 = 0 , V_45 = 0 ;
int V_33 = 0 ;
char V_46 [ 16 ] , V_47 [ 20 ] ;
char * V_48 = V_41 ;
if( V_41 == NULL ) {
if( ! V_32 )
F_4 ( V_35 L_3 ) ;
V_8 = V_36 ;
return;
}
if( ! strncasecmp ( V_41 , V_37 [ V_49 ] . V_41 , strlen ( V_41 ) ) ) {
if( ! V_32 )
F_4 ( V_35 L_4 ) ;
V_8 = V_36 ;
return;
}
if( strlen ( V_41 ) <= 19 ) {
strcpy ( V_47 , V_41 ) ;
for( V_33 = 0 ; V_33 < strlen ( V_47 ) ; V_33 ++ ) {
if( V_47 [ V_33 ] < '0' || V_47 [ V_33 ] > '9' ) V_47 [ V_33 ] = ' ' ;
}
if( sscanf ( V_47 , L_5 , & V_42 , & V_43 , & V_44 , & V_45 ) == 4 ) {
if( ( V_45 <= 32 ) || ( V_44 > 32 ) ) {
V_34 = V_45 ; V_45 = V_44 ; V_44 = V_34 ;
}
sprintf ( V_46 , L_6 , V_42 , V_43 , V_44 ) ;
V_48 = V_46 ;
V_9 = V_45 ;
} else if( sscanf ( V_47 , L_7 , & V_42 , & V_43 , & V_44 ) == 3 ) {
sprintf ( V_46 , L_6 , V_42 , V_43 , V_44 ) ;
V_48 = V_46 ;
} else {
V_42 = 0 ;
if( ( sscanf ( V_47 , L_8 , & V_42 , & V_43 ) == 2 ) && ( V_42 != 0 ) ) {
sprintf ( V_46 , L_9 , V_42 , V_43 ) ;
V_48 = V_46 ;
} else {
F_3 ( F_6 ( V_41 , NULL , 0 ) , V_32 ) ;
return;
}
}
}
V_33 = 0 ; V_34 = 0 ;
while( V_37 [ V_33 ] . V_38 [ 0 ] != 0 ) {
if( ! strncasecmp ( V_48 , V_37 [ V_33 ++ ] . V_41 , strlen ( V_48 ) ) ) {
if( V_21 ) {
if( V_37 [ V_33 - 1 ] . V_38 [ 1 ] == 0x50 ||
V_37 [ V_33 - 1 ] . V_38 [ 1 ] == 0x56 ||
V_37 [ V_33 - 1 ] . V_38 [ 1 ] == 0x53 )
continue;
} else {
if( V_37 [ V_33 - 1 ] . V_38 [ 1 ] == 0x5a ||
V_37 [ V_33 - 1 ] . V_38 [ 1 ] == 0x5b )
continue;
}
V_8 = V_33 - 1 ;
V_34 = 1 ;
break;
}
}
if( ( ! V_34 ) && ! V_32 )
F_4 ( V_35 L_10 , V_48 ) ;
}
static void F_7 ( void )
{
#ifdef F_8
char V_50 [ 32 ] ;
int V_51 = V_52 . V_53 ;
if( V_52 . V_54 != V_55 ) return;
if( ( V_52 . V_56 >= 320 ) && ( V_52 . V_56 <= 2048 ) &&
( V_52 . V_57 >= 200 ) && ( V_52 . V_57 <= 1536 ) &&
( V_51 >= 8 ) && ( V_51 <= 32 ) ) {
if( V_51 == 24 ) V_51 = 32 ;
sprintf ( V_50 , L_6 , V_52 . V_56 ,
V_52 . V_57 ,
V_51 ) ;
F_4 ( V_58
L_11 ,
V_50 ) ;
F_5 ( V_50 , true ) ;
}
#endif
return;
}
static void T_1
F_9 ( const char * V_41 )
{
int V_33 = 0 ;
if( V_41 == NULL ) return;
while( V_59 [ V_33 ] . V_60 != - 1 ) {
if( ! strncasecmp ( V_41 , V_59 [ V_33 ] . V_41 , strlen ( V_59 [ V_33 ] . V_41 ) ) ) {
V_12 = V_59 [ V_33 ] . V_60 ;
V_22 = V_59 [ V_33 ] . V_61 ;
V_13 = V_59 [ V_33 ] . V_62 ;
break;
}
V_33 ++ ;
}
V_20 = ( V_13 & V_63 ) ? 1 : 0 ;
V_21 = ( V_13 & V_64 ) ? 1 : 0 ;
if( V_12 < 0 )
F_4 ( V_35 L_12 , V_41 ) ;
}
static void T_1
F_10 ( const char * V_41 )
{
int V_33 = 0 ;
if( V_41 == NULL )
return;
while( V_65 [ V_33 ] . V_60 != - 1 ) {
if( ! strncasecmp ( V_41 , V_65 [ V_33 ] . V_41 , strlen ( V_65 [ V_33 ] . V_41 ) ) ) {
V_23 = V_65 [ V_33 ] . V_60 ;
break;
}
V_33 ++ ;
}
}
static void T_1
F_11 ( const char * V_41 )
{
int V_33 = 0 ;
bool V_66 = false ;
if( V_41 == NULL )
return;
if( ! strncasecmp ( V_41 , L_13 , 4 ) ) {
V_17 = V_67 ;
F_4 ( V_58 L_14 ) ;
} else {
while( V_68 [ V_33 ] . V_69 != 0 ) {
if( ! strncasecmp ( V_41 , V_68 [ V_33 ] . V_70 ,
strlen ( V_68 [ V_33 ] . V_70 ) ) ) {
V_17 = V_68 [ V_33 ] . V_71 ;
V_66 = true ;
F_4 ( V_72 L_15 ,
V_68 [ V_33 ] . V_73 ,
V_68 [ V_33 ] . V_74 ,
V_68 [ V_33 ] . V_70 ) ;
break;
}
V_33 ++ ;
}
if( ! V_66 ) {
F_4 ( V_75 L_16 ) ;
F_4 ( V_75 L_17 ) ;
V_33 = 0 ;
while( V_68 [ V_33 ] . V_69 != 0 ) {
F_4 ( V_75 L_18 ,
V_68 [ V_33 ] . V_70 ,
V_68 [ V_33 ] . V_73 ,
V_68 [ V_33 ] . V_74 ) ;
V_33 ++ ;
}
}
}
}
static void F_12 ( struct V_76 * V_77 )
{
unsigned char * V_78 = NULL ;
unsigned char * V_79 = NULL ;
bool V_80 ;
T_2 V_81 = 0 ;
int V_33 , V_34 ;
if( V_77 -> V_82 . V_83 ) {
V_78 = V_77 -> V_82 . V_84 + 0x06 ;
V_79 = V_77 -> V_82 . V_84 + 0x2c ;
for( V_33 = 0 ; V_33 < 32768 ; V_33 ++ )
V_81 += V_77 -> V_82 . V_84 [ V_33 ] ;
}
V_33 = 0 ;
do {
if( ( V_68 [ V_33 ] . V_69 == V_77 -> V_85 ) &&
( ( ! strlen ( V_68 [ V_33 ] . V_86 ) ) ||
( V_77 -> V_82 . V_83 &&
( ! strncmp ( V_68 [ V_33 ] . V_86 , V_78 ,
strlen ( V_68 [ V_33 ] . V_86 ) ) ) ) ) &&
( ( ! strlen ( V_68 [ V_33 ] . V_79 ) ) ||
( V_77 -> V_82 . V_83 &&
( ! strncmp ( V_68 [ V_33 ] . V_79 , V_79 ,
strlen ( V_68 [ V_33 ] . V_79 ) ) ) ) ) &&
( ( ! V_68 [ V_33 ] . V_87 ) ||
( V_77 -> V_82 . V_83 &&
( V_68 [ V_33 ] . V_87 == V_81 ) ) ) &&
( V_68 [ V_33 ] . V_88 == V_77 -> V_89 ) &&
( V_68 [ V_33 ] . V_90 == V_77 -> V_91 ) ) {
V_80 = true ;
for( V_34 = 0 ; V_34 < 5 ; V_34 ++ ) {
if( V_68 [ V_33 ] . V_92 [ V_34 ] ) {
if( V_77 -> V_82 . V_83 ) {
if( V_77 -> V_82 . V_84 [ V_68 [ V_33 ] . V_92 [ V_34 ] ] !=
V_68 [ V_33 ] . V_93 [ V_34 ] ) {
V_80 = false ;
}
} else
V_80 = false ;
}
}
if( V_80 ) {
V_77 -> V_82 . V_94 = V_68 [ V_33 ] . V_71 ;
F_4 ( V_58 L_19 ,
V_68 [ V_33 ] . V_73 ,
V_68 [ V_33 ] . V_74 ) ;
F_4 ( V_58 L_20 ,
V_68 [ V_33 ] . V_70 ) ;
break;
}
}
V_33 ++ ;
} while( V_68 [ V_33 ] . V_69 );
}
static bool F_13 ( struct V_95 * V_96 , T_3 * V_97 )
{
int V_33 , V_34 , V_42 , V_43 , V_98 , V_99 ;
T_2 V_100 ;
if( V_97 [ 0 ] != 0x00 || V_97 [ 1 ] != 0xff ||
V_97 [ 2 ] != 0xff || V_97 [ 3 ] != 0xff ||
V_97 [ 4 ] != 0xff || V_97 [ 5 ] != 0xff ||
V_97 [ 6 ] != 0xff || V_97 [ 7 ] != 0x00 ) {
F_4 ( V_58 L_21 ) ;
return false ;
}
if( V_97 [ 0x12 ] != 0x01 ) {
F_4 ( V_72 L_22 ,
V_97 [ 0x12 ] ) ;
return false ;
}
V_96 -> V_101 = V_97 [ 0x18 ] ;
if( ! ( V_97 [ 0x14 ] & 0x80 ) ) {
if( ! ( V_97 [ 0x14 ] & 0x08 ) ) {
F_4 ( V_72
L_23 ) ;
}
}
if( V_97 [ 0x13 ] >= 0x01 ) {
V_34 = 0x36 ;
for( V_33 = 0 ; V_33 < 4 ; V_33 ++ ) {
if( V_97 [ V_34 ] == 0x00 && V_97 [ V_34 + 1 ] == 0x00 &&
V_97 [ V_34 + 2 ] == 0x00 && V_97 [ V_34 + 3 ] == 0xfd &&
V_97 [ V_34 + 4 ] == 0x00 ) {
V_96 -> V_102 = V_97 [ V_34 + 7 ] ;
V_96 -> V_103 = V_97 [ V_34 + 8 ] ;
V_96 -> V_104 = V_97 [ V_34 + 5 ] ;
V_96 -> V_105 = V_97 [ V_34 + 6 ] ;
V_96 -> V_106 = V_97 [ V_34 + 9 ] * 10 * 1000 ;
V_96 -> V_107 = true ;
break;
}
V_34 += 18 ;
}
}
if( ! V_96 -> V_107 ) {
V_96 -> V_102 = 65535 ; V_96 -> V_103 = 0 ;
V_96 -> V_104 = 65535 ; V_96 -> V_105 = 0 ;
V_96 -> V_106 = 0 ;
V_100 = V_97 [ 0x23 ] | ( V_97 [ 0x24 ] << 8 ) | ( V_97 [ 0x25 ] << 16 ) ;
for( V_33 = 0 ; V_33 < 13 ; V_33 ++ ) {
if( V_100 & V_108 [ V_33 ] . V_109 ) {
if( V_96 -> V_102 > V_108 [ V_33 ] . V_110 ) V_96 -> V_102 = V_108 [ V_33 ] . V_110 ;
if( V_96 -> V_103 < V_108 [ V_33 ] . V_110 ) V_96 -> V_103 = V_108 [ V_33 ] . V_110 + 1 ;
if( V_96 -> V_104 > V_108 [ V_33 ] . V_111 ) V_96 -> V_104 = V_108 [ V_33 ] . V_111 ;
if( V_96 -> V_105 < V_108 [ V_33 ] . V_111 ) V_96 -> V_105 = V_108 [ V_33 ] . V_111 ;
if( V_96 -> V_106 < V_108 [ V_33 ] . V_112 ) V_96 -> V_106 = V_108 [ V_33 ] . V_112 ;
}
}
V_99 = 0x26 ;
for( V_33 = 0 ; V_33 < 8 ; V_33 ++ ) {
V_42 = ( V_97 [ V_99 ] + 31 ) * 8 ;
switch( V_97 [ V_99 + 1 ] & 0xc0 ) {
case 0xc0 : V_43 = ( V_42 * 9 ) / 16 ; break;
case 0x80 : V_43 = ( V_42 * 4 ) / 5 ; break;
case 0x40 : V_43 = ( V_42 * 3 ) / 4 ; break;
default: V_43 = V_42 ; break;
}
V_98 = ( V_97 [ V_99 + 1 ] & 0x3f ) + 60 ;
if( ( V_42 >= 640 ) && ( V_43 >= 480 ) ) {
for( V_34 = 0 ; V_34 < 8 ; V_34 ++ ) {
if( ( V_42 == V_113 [ V_34 ] . V_114 ) &&
( V_43 == V_113 [ V_34 ] . V_115 ) &&
( V_98 == V_113 [ V_34 ] . V_111 ) ) {
if( V_96 -> V_102 > V_113 [ V_34 ] . V_110 ) V_96 -> V_102 = V_113 [ V_34 ] . V_110 ;
if( V_96 -> V_103 < V_113 [ V_34 ] . V_110 ) V_96 -> V_103 = V_113 [ V_34 ] . V_110 + 1 ;
if( V_96 -> V_104 > V_108 [ V_34 ] . V_111 ) V_96 -> V_104 = V_108 [ V_34 ] . V_111 ;
if( V_96 -> V_105 < V_108 [ V_34 ] . V_111 ) V_96 -> V_105 = V_108 [ V_34 ] . V_111 ;
if( V_96 -> V_106 < V_108 [ V_34 ] . V_112 ) V_96 -> V_106 = V_108 [ V_34 ] . V_112 ;
}
}
}
V_99 += 2 ;
}
if( ( V_96 -> V_102 <= V_96 -> V_103 ) && ( V_96 -> V_104 <= V_96 -> V_105 ) ) {
V_96 -> V_107 = true ;
}
}
return V_96 -> V_107 ;
}
static void F_14 ( struct V_76 * V_77 ,
struct V_95 * V_96 , int V_116 )
{
unsigned short V_117 , V_33 , V_118 = V_116 ;
unsigned char V_97 [ 256 ] ;
V_96 -> V_107 = false ;
if( V_116 ) {
if( V_77 -> V_119 & V_120 ) V_118 = 1 ;
else if( V_77 -> V_119 & V_121 ) V_118 = 2 ;
else return;
}
if( ( V_77 -> V_10 ) && ( ! V_116 ) )
return;
V_117 = F_15 ( & V_77 -> V_82 , V_77 -> V_119 , V_77 -> V_122 ,
V_118 , 0 , & V_97 [ 0 ] , V_77 -> V_123 ) ;
if( ( ! V_117 ) || ( V_117 == 0xffff ) ) {
F_4 ( V_72 L_24 , V_116 + 1 ) ;
return;
} else {
F_4 ( V_72 L_25 , V_116 + 1 ) ;
F_4 ( V_72 L_26 ,
V_116 + 1 ,
( V_117 & 0x1a ) ? L_27 : L_28 ,
( V_117 & 0x02 ) ? L_29 : L_27 ,
( V_117 & 0x08 ) ? L_30 : L_27 ,
( V_117 & 0x10 ) ? L_31 : L_27 ) ;
if( V_117 & 0x02 ) {
V_33 = 3 ;
do {
V_117 = F_15 ( & V_77 -> V_82 , V_77 -> V_119 , V_77 -> V_122 ,
V_118 , 1 , & V_97 [ 0 ] , V_77 -> V_123 ) ;
} while( ( V_117 ) && V_33 -- );
if( ! V_117 ) {
if( F_13 ( V_96 , & V_97 [ 0 ] ) ) {
F_4 ( V_72 L_32 ,
V_96 -> V_102 , V_96 -> V_103 , V_96 -> V_104 , V_96 -> V_105 ,
V_96 -> V_106 / 1000 ) ;
} else {
F_4 ( V_72 L_33 , V_116 + 1 ) ;
}
} else {
F_4 ( V_72 L_34 , V_116 + 1 ) ;
}
} else {
F_4 ( V_72 L_35 ) ;
}
}
}
static bool
F_16 ( struct V_76 * V_77 , struct V_95 * V_96 ,
int V_124 , int V_125 , int V_45 )
{
int V_126 , V_127 ;
unsigned int V_128 , V_129 ;
if( ! V_96 -> V_107 )
return true ;
if( V_124 < 0 )
return false ;
switch( V_37 [ V_124 ] . V_38 [ V_77 -> V_130 ] ) {
case 0x59 :
case 0x41 :
case 0x4f :
case 0x50 :
case 0x56 :
case 0x53 :
case 0x2f :
case 0x5d :
case 0x5e :
return true ;
#ifdef F_17
case 0x5a :
case 0x5b :
if( V_77 -> V_122 == V_131 ) return true ;
#endif
}
if( V_45 < ( V_96 -> V_104 - 1 ) )
return false ;
if( V_45 > ( V_96 -> V_105 + 1 ) )
return false ;
if( F_18 ( & V_77 -> V_82 ,
V_37 [ V_124 ] . V_38 [ V_77 -> V_130 ] ,
& V_126 , & V_127 , V_125 ) ) {
V_128 = ( V_126 * V_127 * V_45 ) / 1000 ;
if( V_128 > ( V_96 -> V_106 + 1000 ) )
return false ;
V_129 = V_128 / V_126 ;
if( V_129 < ( V_96 -> V_102 - 1 ) )
return false ;
if( V_129 > ( V_96 -> V_103 + 1 ) )
return false ;
} else {
return false ;
}
return true ;
}
static int
F_19 ( struct V_76 * V_77 , int V_132 , T_2 V_119 )
{
T_4 V_42 = 0 , V_43 , V_133 ;
#ifdef F_20
if( V_77 -> V_122 == V_134 ) {
if( ! ( V_37 [ V_132 ] . V_135 & V_136 ) )
return - 1 ;
}
#endif
#ifdef F_17
if( V_77 -> V_122 == V_131 ) {
if( ! ( V_37 [ V_132 ] . V_135 & V_137 ) )
return - 1 ;
}
#endif
V_133 = V_37 [ V_132 ] . V_43 ;
switch( V_119 & V_138 ) {
case V_120 :
V_42 = V_77 -> V_139 ; V_43 = V_77 -> V_140 ;
if( ( V_77 -> V_82 . V_94 != V_141 ) &&
( V_77 -> V_82 . V_94 != V_142 ) ) {
if( V_37 [ V_132 ] . V_42 > V_42 )
return - 1 ;
if( V_133 > V_43 )
return - 1 ;
}
if( V_77 -> V_21 ) {
if( V_37 [ V_132 ] . V_42 == 320 ) {
if( V_133 == 240 ) {
switch( V_37 [ V_132 ] . V_38 [ 1 ] ) {
case 0x50 : V_132 = V_143 ; break;
case 0x56 : V_132 = V_144 ; break;
case 0x53 : return - 1 ;
}
}
}
}
if( F_21 ( V_77 -> V_122 , V_119 , V_37 [ V_132 ] . V_42 ,
V_37 [ V_132 ] . V_43 , 0 , V_77 -> V_21 ,
V_77 -> V_82 . V_94 , V_42 , V_43 , V_77 -> V_123 ) < 0x14 ) {
return - 1 ;
}
break;
case V_145 :
if( F_22 ( V_77 -> V_122 , V_119 , V_37 [ V_132 ] . V_42 ,
V_37 [ V_132 ] . V_43 , 0 , V_77 -> V_123 ) < 0x14 ) {
return - 1 ;
}
break;
case V_121 :
if( F_23 ( V_77 -> V_122 , V_119 , V_37 [ V_132 ] . V_42 ,
V_37 [ V_132 ] . V_43 , 0 , V_77 -> V_123 ) < 0x14 ) {
return - 1 ;
}
break;
}
return V_132 ;
}
static T_3
F_24 ( struct V_76 * V_77 , unsigned int V_45 , int V_124 )
{
int V_33 = 0 ;
T_4 V_42 = V_37 [ V_124 ] . V_42 ;
T_4 V_43 = V_37 [ V_124 ] . V_43 ;
V_77 -> V_125 = 0 ;
while( ( V_146 [ V_33 ] . V_147 != 0 ) && ( V_146 [ V_33 ] . V_42 <= V_42 ) ) {
if( ( V_146 [ V_33 ] . V_42 == V_42 ) && ( V_146 [ V_33 ] . V_43 == V_43 ) ) {
if( V_146 [ V_33 ] . V_98 == V_45 ) {
V_77 -> V_125 = V_146 [ V_33 ] . V_147 ;
break;
} else if( V_146 [ V_33 ] . V_98 > V_45 ) {
if( ( V_146 [ V_33 ] . V_98 - V_45 ) <= 3 ) {
F_25 ( L_36 ,
V_45 , V_146 [ V_33 ] . V_98 ) ;
V_77 -> V_125 = V_146 [ V_33 ] . V_147 ;
V_77 -> V_148 = V_146 [ V_33 ] . V_98 ;
} else if( ( V_146 [ V_33 ] . V_147 != 1 ) &&
( ( V_45 - V_146 [ V_33 - 1 ] . V_98 ) <= 2 ) ) {
F_25 ( L_37 ,
V_45 , V_146 [ V_33 - 1 ] . V_98 ) ;
V_77 -> V_125 = V_146 [ V_33 - 1 ] . V_147 ;
V_77 -> V_148 = V_146 [ V_33 - 1 ] . V_98 ;
}
break;
} else if( ( V_45 - V_146 [ V_33 ] . V_98 ) <= 2 ) {
F_25 ( L_37 ,
V_45 , V_146 [ V_33 ] . V_98 ) ;
V_77 -> V_125 = V_146 [ V_33 ] . V_147 ;
break;
}
}
V_33 ++ ;
}
if( V_77 -> V_125 > 0 ) {
return V_77 -> V_125 ;
} else {
F_4 ( V_72 L_38 ,
V_45 , V_42 , V_43 ) ;
return 0 ;
}
}
static bool
F_26 ( struct V_76 * V_77 )
{
unsigned char V_149 ;
if( ! ( V_77 -> V_123 & V_150 ) )
return false ;
V_149 = F_27 ( V_151 , 0x00 ) ;
if( ( ( V_77 -> V_122 == V_134 ) && ( V_149 & 0xa0 ) == 0x20 ) ||
( ( V_77 -> V_122 == V_131 ) && ( V_149 & 0x50 ) == 0x10 ) ) {
return true ;
} else {
return false ;
}
}
static bool
F_28 ( struct V_76 * V_77 )
{
T_3 V_117 ;
V_117 = F_27 ( V_152 , 0x17 ) ;
if( ! ( V_117 & 0x80 ) )
return false ;
V_117 = F_27 ( V_153 , 0x1f ) ;
if( V_117 & 0xc0 )
return false ;
return true ;
}
static bool
F_29 ( struct V_76 * V_77 )
{
if( ! F_28 ( V_77 ) )
return false ;
if ( F_30 ( V_154 ) & 0x08 )
return true ;
else
return false ;
}
static void
F_31 ( struct V_76 * V_77 )
{
int V_155 ;
if( ! F_28 ( V_77 ) )
return;
V_155 = 65536 ;
while ( ( ! ( F_30 ( V_154 ) & 0x08 ) ) && -- V_155 ) ;
V_155 = 65536 ;
while ( ( F_30 ( V_154 ) & 0x08 ) && -- V_155 ) ;
}
static bool
F_32 ( struct V_76 * V_77 )
{
unsigned char V_117 , V_156 ;
switch( V_77 -> V_122 ) {
case V_134 : V_156 = 0x25 ; break;
case V_131 : V_156 = 0x30 ; break;
default: return false ;
}
V_117 = F_27 ( V_151 , V_156 ) ;
if( V_117 & 0x02 )
return true ;
else
return false ;
}
static bool
F_33 ( struct V_76 * V_77 )
{
if( V_77 -> V_157 & V_138 ) {
if( ! F_26 ( V_77 ) ) {
return F_32 ( V_77 ) ;
}
}
return F_29 ( V_77 ) ;
}
static T_2
F_34 ( struct V_76 * V_77 , T_2 * V_158 , T_2 * V_159 )
{
T_3 V_147 , V_160 , V_161 , V_162 , V_163 ;
T_2 V_164 = 0 ;
( * V_158 ) = ( * V_159 ) = 0 ;
if( ( V_77 -> V_157 & V_138 ) && ( ! ( F_26 ( V_77 ) ) ) ) {
V_164 |= ( V_165 |
V_166 |
V_167 |
V_168 |
V_169 ) ;
switch( V_77 -> V_122 ) {
case V_134 : V_147 = 0x25 ; break;
default:
case V_131 : V_147 = 0x30 ; break;
}
V_160 = F_27 ( V_151 , ( V_147 + 0 ) ) ;
V_161 = F_27 ( V_151 , ( V_147 + 1 ) ) ;
V_162 = F_27 ( V_151 , ( V_147 + 2 ) ) ;
V_163 = F_27 ( V_151 , ( V_147 + 3 ) ) ;
if( V_160 & 0x01 ) V_164 |= V_170 ;
if( V_160 & 0x02 ) V_164 |= V_171 ;
if( V_163 & 0x80 ) V_164 |= V_172 ;
( * V_158 ) = V_162 | ( ( V_163 & 0x70 ) << 4 ) ;
( * V_159 ) = V_161 | ( ( V_163 & 0x0f ) << 8 ) ;
} else if( F_28 ( V_77 ) ) {
V_164 |= ( V_165 |
V_167 |
V_168 |
V_169 ) ;
V_160 = F_30 ( V_154 ) ;
if( V_160 & 0x08 ) V_164 |= V_171 ;
if( V_160 & 0x01 ) V_164 |= V_170 ;
V_160 = F_27 ( V_152 , 0x20 ) ;
V_160 = F_27 ( V_152 , 0x1b ) ;
V_161 = F_27 ( V_152 , 0x1c ) ;
V_162 = F_27 ( V_152 , 0x1d ) ;
( * V_158 ) = V_161 | ( ( V_162 & 0x07 ) << 8 ) ;
( * V_159 ) = ( V_160 | ( ( V_162 & 0x10 ) << 4 ) ) << 3 ;
}
return V_164 ;
}
static int
F_35 ( struct V_76 * V_77 , int V_173 )
{
T_3 V_174 , V_175 , V_176 , V_177 = 0 , V_178 , V_179 ;
bool V_180 = true ;
switch( V_173 ) {
case V_181 :
V_174 = 0x00 ;
V_175 = 0x00 ;
V_176 = 0x00 ;
V_177 = 0x00 ;
V_178 = 0x20 ;
V_179 = 0x00 ;
V_180 = true ;
break;
case V_182 :
V_174 = 0x20 ;
V_175 = 0x00 ;
V_176 = 0x00 ;
V_177 = 0x00 ;
V_178 = 0x20 ;
V_179 = 0x00 ;
V_180 = true ;
break;
case V_183 :
V_174 = 0x20 ;
V_175 = 0x08 ;
V_176 = 0x80 ;
V_177 = 0x40 ;
V_178 = 0x40 ;
V_179 = 0x80 ;
V_180 = false ;
break;
case V_184 :
V_174 = 0x20 ;
V_175 = 0x08 ;
V_176 = 0x40 ;
V_177 = 0x40 ;
V_178 = 0x80 ;
V_179 = 0x40 ;
V_180 = false ;
break;
case V_185 :
V_174 = 0x20 ;
V_175 = 0x08 ;
V_176 = 0xc0 ;
V_177 = 0x40 ;
V_178 = 0xc0 ;
V_179 = 0xc0 ;
V_180 = false ;
break;
default:
return 1 ;
}
if( V_77 -> V_157 & V_186 ) {
if( ( ! V_77 -> V_187 . V_107 ) ||
( ( V_77 -> V_187 . V_107 ) &&
( V_77 -> V_187 . V_101 & 0xe0 ) ) ) {
if( V_77 -> V_122 == V_131 ) {
F_36 ( V_152 , V_77 -> V_82 . V_188 , 0xbf , V_177 ) ;
}
if( ! ( F_26 ( V_77 ) ) ) {
F_36 ( V_153 , 0x01 , ~ 0x20 , V_174 ) ;
F_36 ( V_153 , 0x1f , 0x3f , V_176 ) ;
}
}
}
if( V_77 -> V_157 & V_120 ) {
if( V_77 -> V_123 & V_189 ) {
if( V_180 ) {
F_37 ( & V_77 -> V_82 ) ;
} else {
F_38 ( & V_77 -> V_82 ) ;
}
} else if( V_77 -> V_122 == V_131 ) {
#ifdef F_17
if( V_77 -> V_123 & V_190 ) {
if( V_180 ) {
F_39 ( & V_77 -> V_82 ) ;
} else {
F_40 ( & V_77 -> V_82 ) ;
}
}
#endif
}
if( ( ( V_77 -> V_122 == V_134 ) &&
( V_77 -> V_123 & ( V_191 | V_192 | V_193 ) ) ) ||
( ( V_77 -> V_122 == V_131 ) &&
( ( V_77 -> V_123 & ( V_193 | V_190 ) ) == V_193 ) ) ) {
F_36 ( V_153 , 0x11 , ~ 0x0c , V_175 ) ;
}
if( V_77 -> V_122 == V_134 ) {
if( ( V_77 -> V_123 & V_194 ) &&
( ! ( V_77 -> V_123 & V_192 ) ) ) {
F_36 ( V_151 , 0x13 , 0x3f , V_179 ) ;
}
} else if( V_77 -> V_122 == V_131 ) {
if( ( V_77 -> V_123 & V_194 ) &&
( ! ( V_77 -> V_123 & V_192 ) ) ) {
F_36 ( V_195 , 0x00 , 0x1f , V_178 ) ;
}
}
} else if( V_77 -> V_157 & V_121 ) {
if( V_77 -> V_123 & V_194 ) {
F_36 ( V_195 , 0x00 , 0x1f , V_178 ) ;
}
}
return 0 ;
}
unsigned int
F_41 ( struct V_196 * V_82 , int V_156 )
{
struct V_76 * V_77 = (struct V_76 * ) V_82 -> V_77 ;
T_2 V_197 = 0 ;
F_42 ( V_77 -> V_198 , V_156 , & V_197 ) ;
return ( unsigned int ) V_197 ;
}
void
F_43 ( struct V_196 * V_82 , int V_156 , unsigned int V_197 )
{
struct V_76 * V_77 = (struct V_76 * ) V_82 -> V_77 ;
F_44 ( V_77 -> V_198 , V_156 , ( T_2 ) V_197 ) ;
}
unsigned int
F_45 ( struct V_196 * V_82 , int V_156 )
{
struct V_76 * V_77 = (struct V_76 * ) V_82 -> V_77 ;
T_2 V_197 = 0 ;
if( ! V_77 -> V_199 ) return 0 ;
F_42 ( V_77 -> V_199 , V_156 , & V_197 ) ;
return ( unsigned int ) V_197 ;
}
void
F_46 ( struct V_196 * V_82 , int V_156 , unsigned char V_197 )
{
struct V_76 * V_77 = (struct V_76 * ) V_82 -> V_77 ;
F_47 ( V_77 -> V_198 , V_156 , ( T_3 ) V_197 ) ;
}
unsigned int
F_48 ( struct V_196 * V_82 , int V_156 )
{
struct V_76 * V_77 = (struct V_76 * ) V_82 -> V_77 ;
T_4 V_197 = 0 ;
if( ! V_77 -> V_199 ) return 0 ;
F_49 ( V_77 -> V_199 , V_156 , & V_197 ) ;
return ( unsigned int ) V_197 ;
}
static int
F_50 ( const struct V_200 * V_201 )
{
return ( V_201 -> V_202 == 8 ) ? 256 : 16 ;
}
static void
F_51 ( struct V_76 * V_77 )
{
switch( V_77 -> V_203 ) {
case 8 :
V_77 -> V_204 = 0x0000 ;
V_77 -> V_205 = 0x00000000 ;
V_77 -> V_206 = 256 ;
break;
case 16 :
V_77 -> V_204 = 0x8000 ;
V_77 -> V_205 = 0x00010000 ;
V_77 -> V_206 = 16 ;
break;
case 32 :
V_77 -> V_204 = 0xC000 ;
V_77 -> V_205 = 0x00020000 ;
V_77 -> V_206 = 16 ;
break;
default:
V_77 -> V_206 = 16 ;
F_4 ( V_35 L_39 , V_77 -> V_203 ) ;
V_77 -> V_207 = 0 ;
}
}
static int
F_52 ( struct V_76 * V_77 , struct V_200 * V_201 )
{
int V_208 = V_77 -> V_209 / ( V_201 -> V_210 * ( V_201 -> V_202 >> 3 ) ) ;
if( V_208 > 32767 ) V_208 = 32767 ;
return V_208 ;
}
static void
F_53 ( struct V_76 * V_77 , struct V_200 * V_201 )
{
V_77 -> V_211 = V_201 -> V_210 * ( V_201 -> V_202 >> 3 ) ;
V_77 -> V_212 = V_77 -> V_211 ;
if( ! ( V_77 -> V_157 & V_213 ) ) {
if( ( V_201 -> V_214 & V_215 ) == V_216 ) {
V_77 -> V_212 <<= 1 ;
}
}
}
static void
F_54 ( struct V_76 * V_77 )
{
bool V_217 = false ;
unsigned short V_218 = V_77 -> V_212 >> 3 ;
unsigned short V_219 = V_77 -> V_211 >> 3 ;
if( F_26 ( V_77 ) ) V_217 = true ;
if( ( V_77 -> V_157 & V_220 ) || ( V_217 ) ) {
F_55 ( V_152 , 0x13 , ( V_218 & 0xFF ) ) ;
F_36 ( V_153 , 0x0E , 0xF0 , ( V_218 >> 8 ) ) ;
}
if( ( V_77 -> V_157 & V_138 ) && ( ! V_217 ) ) {
F_56 ( V_151 , V_77 -> V_221 , 0x01 ) ;
F_55 ( V_151 , 0x07 , ( V_219 & 0xFF ) ) ;
F_36 ( V_151 , 0x09 , 0xF0 , ( V_219 >> 8 ) ) ;
}
}
static void
F_57 ( struct V_76 * V_77 , struct V_200 * V_201 )
{
V_77 -> V_206 = F_50 ( V_201 ) ;
switch( V_201 -> V_202 ) {
case 8 :
V_201 -> V_222 . V_223 = V_201 -> V_224 . V_223 = V_201 -> V_225 . V_223 = 0 ;
V_201 -> V_222 . V_226 = V_201 -> V_224 . V_226 = V_201 -> V_225 . V_226 = 8 ;
break;
case 16 :
V_201 -> V_222 . V_223 = 11 ;
V_201 -> V_222 . V_226 = 5 ;
V_201 -> V_224 . V_223 = 5 ;
V_201 -> V_224 . V_226 = 6 ;
V_201 -> V_225 . V_223 = 0 ;
V_201 -> V_225 . V_226 = 5 ;
V_201 -> V_227 . V_223 = 0 ;
V_201 -> V_227 . V_226 = 0 ;
break;
case 32 :
V_201 -> V_222 . V_223 = 16 ;
V_201 -> V_222 . V_226 = 8 ;
V_201 -> V_224 . V_223 = 8 ;
V_201 -> V_224 . V_226 = 8 ;
V_201 -> V_225 . V_223 = 0 ;
V_201 -> V_225 . V_226 = 8 ;
V_201 -> V_227 . V_223 = 24 ;
V_201 -> V_227 . V_226 = 8 ;
break;
}
}
static int
F_58 ( struct V_76 * V_77 , int V_228 )
{
unsigned short V_229 = V_77 -> V_38 ;
V_229 |= 0x80 ;
F_55 ( V_153 , V_230 , V_231 ) ;
F_59 ( V_77 ) ;
if( ! F_60 ( & V_77 -> V_82 , V_229 ) ) {
F_4 ( V_35 L_40 , V_77 -> V_38 ) ;
return - V_232 ;
}
F_55 ( V_153 , V_230 , V_231 ) ;
F_61 ( V_77 ) ;
return 0 ;
}
static int
F_62 ( struct V_200 * V_201 , int V_233 , struct V_234 * V_235 )
{
struct V_76 * V_77 = (struct V_76 * ) V_235 -> V_236 ;
unsigned int V_126 = 0 , V_127 = 0 ;
unsigned int V_237 = 0 , V_238 = 0 ;
int V_239 = 0 , V_164 ;
int V_240 ;
T_2 V_241 ;
V_126 = V_201 -> V_242 + V_201 -> V_42 + V_201 -> V_243 + V_201 -> V_244 ;
V_127 = V_201 -> V_245 + V_201 -> V_246 + V_201 -> V_247 ;
V_241 = V_201 -> V_241 ;
if( ( V_201 -> V_214 & V_215 ) == V_248 ) {
V_127 += V_201 -> V_43 ;
V_127 <<= 1 ;
} else if( ( V_201 -> V_214 & V_215 ) == V_249 ) {
V_127 += V_201 -> V_43 ;
V_127 <<= 2 ;
} else if( ( V_201 -> V_214 & V_215 ) == V_216 ) {
V_127 += V_201 -> V_43 ;
V_127 <<= 1 ;
} else V_127 += V_201 -> V_43 ;
if( ! ( V_126 ) || ! ( V_127 ) ) {
F_25 ( L_41 ) ;
return - V_232 ;
}
if( V_241 && V_126 && V_127 ) {
V_237 = 1000000000 / V_241 ;
V_238 = ( V_237 * 1000 ) / V_126 ;
V_77 -> V_148 = ( unsigned int ) ( V_238 * 2 / V_127 ) ;
} else {
V_77 -> V_148 = 60 ;
}
V_240 = V_77 -> V_8 ;
V_77 -> V_8 = 0 ;
while( ( V_37 [ V_77 -> V_8 ] . V_38 [ 0 ] != 0 ) &&
( V_37 [ V_77 -> V_8 ] . V_42 <= V_201 -> V_42 ) ) {
if( ( V_37 [ V_77 -> V_8 ] . V_42 == V_201 -> V_42 ) &&
( V_37 [ V_77 -> V_8 ] . V_43 == V_201 -> V_43 ) &&
( V_37 [ V_77 -> V_8 ] . V_250 == V_201 -> V_202 ) ) {
V_77 -> V_38 = V_37 [ V_77 -> V_8 ] . V_38 [ V_77 -> V_130 ] ;
V_239 = 1 ;
break;
}
V_77 -> V_8 ++ ;
}
if( V_239 ) {
V_77 -> V_8 = F_19 ( V_77 ,
V_77 -> V_8 , V_77 -> V_157 ) ;
} else {
V_77 -> V_8 = - 1 ;
}
if( V_77 -> V_8 < 0 ) {
F_4 ( V_35 L_42 , V_201 -> V_42 ,
V_201 -> V_43 , V_201 -> V_202 ) ;
V_77 -> V_8 = V_240 ;
return - V_232 ;
}
V_77 -> V_38 = V_37 [ V_77 -> V_8 ] . V_38 [ V_77 -> V_130 ] ;
if( F_24 ( V_77 , V_77 -> V_148 , V_77 -> V_8 ) == 0 ) {
V_77 -> V_125 = V_37 [ V_77 -> V_8 ] . V_125 ;
V_77 -> V_148 = 60 ;
}
if( V_233 ) {
V_77 -> V_207 = 0 ;
#if F_2 ( V_251 ) && F_2 ( V_252 )
#ifdef F_63
if( V_201 -> V_253 & V_254 ) {
V_235 -> V_62 &= ~ V_251 ;
} else {
V_235 -> V_62 |= V_251 ;
}
#endif
if( ! ( V_235 -> V_62 & V_251 ) ) V_77 -> V_207 = - 1 ;
#else
if( V_201 -> V_253 & V_254 ) V_77 -> V_207 = - 1 ;
#endif
if( ( V_164 = F_58 ( V_77 , 1 ) ) ) {
return V_164 ;
}
V_77 -> V_203 = V_37 [ V_77 -> V_8 ] . V_250 ;
V_77 -> V_255 = V_37 [ V_77 -> V_8 ] . V_42 ;
V_77 -> V_256 = V_37 [ V_77 -> V_8 ] . V_43 ;
F_53 ( V_77 , V_201 ) ;
F_54 ( V_77 ) ;
F_51 ( V_77 ) ;
V_77 -> V_257 = V_77 -> V_255 ;
V_77 -> V_258 = V_77 -> V_256 ;
V_77 -> V_259 = V_77 -> V_203 ;
V_77 -> V_260 = V_126 ;
V_77 -> V_261 = V_127 ;
V_77 -> V_262 = V_77 -> V_211 ;
V_77 -> V_263 = V_201 -> V_241 ;
V_77 -> V_264 = V_77 -> V_148 ;
V_77 -> V_265 [ V_77 -> V_38 ] = V_77 -> V_148 ;
}
return 0 ;
}
static void
F_64 ( struct V_76 * V_77 , unsigned int V_266 )
{
F_55 ( V_153 , V_230 , V_231 ) ;
F_55 ( V_152 , 0x0D , V_266 & 0xFF ) ;
F_55 ( V_152 , 0x0C , ( V_266 >> 8 ) & 0xFF ) ;
F_55 ( V_153 , 0x0D , ( V_266 >> 16 ) & 0xFF ) ;
if( V_77 -> V_122 == V_131 ) {
F_36 ( V_153 , 0x37 , 0xFE , ( V_266 >> 24 ) & 0x01 ) ;
}
}
static void
F_65 ( struct V_76 * V_77 , unsigned int V_266 )
{
if( V_77 -> V_157 & V_138 ) {
F_56 ( V_151 , V_77 -> V_221 , 0x01 ) ;
F_55 ( V_151 , 0x06 , ( V_266 & 0xFF ) ) ;
F_55 ( V_151 , 0x05 , ( ( V_266 >> 8 ) & 0xFF ) ) ;
F_55 ( V_151 , 0x04 , ( ( V_266 >> 16 ) & 0xFF ) ) ;
if( V_77 -> V_122 == V_131 ) {
F_36 ( V_151 , 0x02 , 0x7F , ( ( V_266 >> 24 ) & 0x01 ) << 7 ) ;
}
}
}
static int
F_66 ( struct V_76 * V_77 , struct V_234 * V_235 ,
struct V_200 * V_201 )
{
V_77 -> V_267 = V_201 -> V_268 * V_235 -> V_201 . V_210
+ V_201 -> V_269 ;
switch ( V_235 -> V_201 . V_202 ) {
case 32 :
break;
case 16 :
V_77 -> V_267 >>= 1 ;
break;
case 8 :
default:
V_77 -> V_267 >>= 2 ;
break;
}
V_77 -> V_267 += ( V_77 -> V_270 >> 2 ) ;
F_64 ( V_77 , V_77 -> V_267 ) ;
F_65 ( V_77 , V_77 -> V_267 ) ;
return 0 ;
}
static int
F_67 ( struct V_234 * V_235 , int V_271 )
{
return 0 ;
}
static int
F_68 ( struct V_234 * V_235 , int V_271 )
{
return 0 ;
}
static int
F_69 ( unsigned V_272 , unsigned V_222 , unsigned V_224 , unsigned V_225 ,
unsigned V_227 , struct V_234 * V_235 )
{
struct V_76 * V_77 = (struct V_76 * ) V_235 -> V_236 ;
if( V_272 >= F_50 ( & V_235 -> V_201 ) )
return 1 ;
switch( V_235 -> V_201 . V_202 ) {
case 8 :
F_70 ( V_273 , V_272 ) ;
F_70 ( V_274 , ( V_222 >> 10 ) ) ;
F_70 ( V_274 , ( V_224 >> 10 ) ) ;
F_70 ( V_274 , ( V_225 >> 10 ) ) ;
if( V_77 -> V_157 & V_138 ) {
F_70 ( V_275 , V_272 ) ;
F_70 ( V_276 , ( V_222 >> 8 ) ) ;
F_70 ( V_276 , ( V_224 >> 8 ) ) ;
F_70 ( V_276 , ( V_225 >> 8 ) ) ;
}
break;
case 16 :
if ( V_272 >= 16 )
break;
( ( T_2 * ) ( V_235 -> V_277 ) ) [ V_272 ] =
( V_222 & 0xf800 ) |
( ( V_224 & 0xfc00 ) >> 5 ) |
( ( V_225 & 0xf800 ) >> 11 ) ;
break;
case 32 :
if ( V_272 >= 16 )
break;
V_222 >>= 8 ;
V_224 >>= 8 ;
V_225 >>= 8 ;
( ( T_2 * ) ( V_235 -> V_277 ) ) [ V_272 ] =
( V_222 << 16 ) | ( V_224 << 8 ) | ( V_225 ) ;
break;
}
return 0 ;
}
static int
F_71 ( struct V_234 * V_235 )
{
int V_278 ;
if( ( V_278 = F_62 ( & V_235 -> V_201 , 1 , V_235 ) ) )
return V_278 ;
F_72 ( & V_235 -> V_279 , - 1 , V_235 ) ;
return 0 ;
}
static int
F_73 ( struct V_200 * V_201 , struct V_234 * V_235 )
{
struct V_76 * V_77 = (struct V_76 * ) V_235 -> V_236 ;
unsigned int V_126 = 0 , V_127 = 0 , V_280 = 0 ;
unsigned int V_237 = 0 , V_238 = 0 , V_208 ;
int V_239 = 0 ;
int V_148 , V_281 , V_282 ;
bool V_283 = false ;
T_2 V_241 ;
V_126 = V_201 -> V_242 + V_201 -> V_42 + V_201 -> V_243 + V_201 -> V_244 ;
V_127 = V_201 -> V_245 + V_201 -> V_246 + V_201 -> V_247 ;
V_241 = V_201 -> V_241 ;
if( ( V_201 -> V_214 & V_215 ) == V_248 ) {
V_127 += V_201 -> V_43 ;
V_127 <<= 1 ;
} else if( ( V_201 -> V_214 & V_215 ) == V_249 ) {
V_127 += V_201 -> V_43 ;
V_127 <<= 2 ;
} else if( ( V_201 -> V_214 & V_215 ) == V_216 ) {
V_127 += V_201 -> V_43 ;
V_127 <<= 1 ;
} else
V_127 += V_201 -> V_43 ;
if( ! ( V_126 ) || ! ( V_127 ) ) {
F_74 ( L_43 ) ;
}
V_281 = 0 ;
while( ( V_37 [ V_281 ] . V_38 [ 0 ] != 0 ) &&
( V_37 [ V_281 ] . V_42 <= V_201 -> V_42 ) ) {
if( ( V_37 [ V_281 ] . V_42 == V_201 -> V_42 ) &&
( V_37 [ V_281 ] . V_43 == V_201 -> V_43 ) &&
( V_37 [ V_281 ] . V_250 == V_201 -> V_202 ) ) {
if( ( V_282 = F_19 ( V_77 , V_281 ,
V_77 -> V_157 ) ) > 0 ) {
V_239 = 1 ;
V_281 = V_282 ;
break;
}
}
V_281 ++ ;
}
if( ! V_239 ) {
V_281 = 0 ;
while( V_37 [ V_281 ] . V_38 [ 0 ] != 0 ) {
if( ( V_201 -> V_42 <= V_37 [ V_281 ] . V_42 ) &&
( V_201 -> V_43 <= V_37 [ V_281 ] . V_43 ) &&
( V_201 -> V_202 == V_37 [ V_281 ] . V_250 ) ) {
if( ( V_282 = F_19 ( V_77 , V_281 ,
V_77 -> V_157 ) ) > 0 ) {
V_239 = 1 ;
V_281 = V_282 ;
break;
}
}
V_281 ++ ;
}
if( V_239 ) {
F_4 ( V_58
L_44 ,
V_201 -> V_42 , V_201 -> V_43 , V_201 -> V_202 ,
V_37 [ V_281 ] . V_42 ,
V_37 [ V_281 ] . V_43 ,
V_201 -> V_202 ) ;
V_201 -> V_42 = V_37 [ V_281 ] . V_42 ;
V_201 -> V_43 = V_37 [ V_281 ] . V_43 ;
} else {
F_4 ( V_35
L_45 ,
V_201 -> V_42 , V_201 -> V_43 , V_201 -> V_202 ) ;
return - V_232 ;
}
}
if( ( ( V_77 -> V_123 & V_193 ) ||
( ( V_77 -> V_123 & V_192 ) && ( V_77 -> V_157 & V_120 ) ) ) &&
( V_201 -> V_202 == 8 ) ) {
V_148 = 60 ;
V_283 = true ;
} else if( ( V_77 -> V_260 == V_126 ) &&
( V_77 -> V_261 == V_127 ) &&
( V_77 -> V_263 == V_241 ) ) {
V_237 = 1000000000 / V_241 ;
V_238 = ( V_237 * 1000 ) / V_126 ;
V_148 = ( unsigned int ) ( V_238 * 2 / V_127 ) ;
} else if( ( ( V_77 -> V_260 != V_126 ) ||
( V_77 -> V_261 != V_127 ) ) &&
( V_77 -> V_263 == V_201 -> V_241 ) ) {
if( V_77 -> V_265 [ V_37 [ V_281 ] . V_38 [ V_77 -> V_130 ] ] ) {
V_148 =
V_77 -> V_265 [ V_37 [ V_281 ] . V_38 [ V_77 -> V_130 ] ] ;
} else if( V_77 -> V_9 != - 1 ) {
V_148 = V_77 -> V_9 ;
} else {
V_148 = 60 ;
}
V_283 = true ;
} else if( ( V_241 ) && ( V_126 ) && ( V_127 ) ) {
V_237 = 1000000000 / V_241 ;
V_238 = ( V_237 * 1000 ) / V_126 ;
V_148 = ( unsigned int ) ( V_238 * 2 / V_127 ) ;
} else if( V_77 -> V_264 ) {
V_148 = V_77 -> V_264 ;
V_283 = true ;
} else {
V_148 = 60 ;
V_283 = true ;
}
V_280 = F_24 ( V_77 , V_148 , V_281 ) ;
if( V_283 ) {
if( ! V_280 ) V_280 = V_37 [ V_281 ] . V_125 ;
V_201 -> V_241 = ( T_2 ) ( 1000000000 / F_75 ( & V_77 -> V_82 ,
V_37 [ V_281 ] . V_38 [ V_77 -> V_130 ] ,
V_280 ) ) ;
F_76 ( & V_77 -> V_82 ,
V_37 [ V_281 ] . V_38 [ V_77 -> V_130 ] ,
V_280 , V_201 ) ;
if( ( V_201 -> V_214 & V_215 ) == V_249 ) {
V_201 -> V_241 <<= 1 ;
}
}
if( V_77 -> V_187 . V_107 ) {
if( ! F_16 ( V_77 , & V_77 -> V_187 , V_281 ,
V_280 , V_148 ) ) {
F_4 ( V_72
L_46 ) ;
}
}
F_57 ( V_77 , V_201 ) ;
if( V_201 -> V_42 > V_201 -> V_210 )
V_201 -> V_210 = V_201 -> V_42 ;
if( V_77 -> V_4 ) {
V_208 = F_52 ( V_77 , V_201 ) ;
if( V_77 -> V_5 ) {
V_201 -> V_284 = V_208 ;
} else {
if( V_201 -> V_284 > V_208 ) {
V_201 -> V_284 = V_208 ;
}
}
if( V_201 -> V_284 <= V_201 -> V_43 ) {
V_201 -> V_284 = V_201 -> V_43 ;
}
} else {
if( V_201 -> V_43 != V_201 -> V_284 ) {
V_201 -> V_284 = V_201 -> V_43 ;
}
V_201 -> V_269 = 0 ;
V_201 -> V_268 = 0 ;
}
if( V_201 -> V_269 > V_201 -> V_210 - V_201 -> V_42 ) {
V_201 -> V_269 = V_201 -> V_210 - V_201 -> V_42 - 1 ;
}
if( V_201 -> V_268 > V_201 -> V_284 - V_201 -> V_43 ) {
V_201 -> V_268 = V_201 -> V_284 - V_201 -> V_43 - 1 ;
}
V_201 -> V_222 . V_285 =
V_201 -> V_224 . V_285 =
V_201 -> V_225 . V_285 =
V_201 -> V_227 . V_223 =
V_201 -> V_227 . V_226 =
V_201 -> V_227 . V_285 = 0 ;
return 0 ;
}
static int
F_77 ( struct V_200 * V_201 , struct V_234 * V_235 )
{
struct V_76 * V_77 = (struct V_76 * ) V_235 -> V_236 ;
int V_278 ;
if ( V_201 -> V_214 & V_286 )
return - V_232 ;
if ( V_201 -> V_269 + V_235 -> V_201 . V_42 > V_235 -> V_201 . V_210 ||
V_201 -> V_268 + V_235 -> V_201 . V_43 > V_235 -> V_201 . V_284 )
return - V_232 ;
V_278 = F_66 ( V_77 , V_235 , V_201 ) ;
if ( V_278 < 0 )
return V_278 ;
V_235 -> V_201 . V_269 = V_201 -> V_269 ;
V_235 -> V_201 . V_268 = V_201 -> V_268 ;
return 0 ;
}
static int
F_78 ( int V_173 , struct V_234 * V_235 )
{
struct V_76 * V_77 = (struct V_76 * ) V_235 -> V_236 ;
return F_35 ( V_77 , V_173 ) ;
}
static int F_79 ( struct V_234 * V_235 , unsigned int V_287 ,
unsigned long V_288 )
{
struct V_76 * V_77 = (struct V_76 * ) V_235 -> V_236 ;
struct V_289 V_290 ;
struct V_291 V_292 ;
T_2 V_293 = 0 ;
#ifndef F_80
#define F_80
#endif
T_2 F_80 * V_294 = ( T_2 F_80 * ) V_288 ;
switch( V_287 ) {
case V_295 :
if( ! F_81 ( V_296 ) )
return - V_297 ;
if( F_82 ( & V_290 , ( void F_80 * ) V_288 , sizeof( V_290 ) ) )
return - V_298 ;
F_83 ( & V_290 ) ;
if( F_84 ( ( void F_80 * ) V_288 , & V_290 , sizeof( V_290 ) ) ) {
F_85 ( ( T_2 ) V_290 . V_223 ) ;
return - V_298 ;
}
break;
case V_299 :
if( ! F_81 ( V_296 ) )
return - V_297 ;
if( F_86 ( V_293 , V_294 ) )
return - V_298 ;
F_85 ( V_293 ) ;
break;
case V_300 :
memset ( & V_292 , 0 , sizeof( struct V_291 ) ) ;
V_292 . V_301 = 0 ;
V_292 . V_62 = F_34 ( V_77 , & V_292 . V_158 , & V_292 . V_159 ) ;
if( F_84 ( ( void F_80 * ) V_288 , & V_292 , sizeof( V_292 ) ) )
return - V_298 ;
break;
case V_302 :
return F_87 ( sizeof( struct V_303 ) , V_294 ) ;
case V_304 :
if( V_77 -> V_305 ++ < 10 )
F_4 ( V_72
L_47 ) ;
case V_306 :
V_77 -> V_307 . V_308 = V_309 ;
V_77 -> V_307 . V_310 = V_311 ;
V_77 -> V_307 . V_312 = V_313 ;
V_77 -> V_307 . V_314 = V_315 ;
V_77 -> V_307 . V_316 = V_77 -> V_316 ;
V_77 -> V_307 . V_317 = V_77 -> V_318 ;
V_77 -> V_307 . V_319 = V_77 -> V_320 / 1024 ;
V_77 -> V_307 . V_321 = V_77 -> V_321 / 1024 ;
if( V_77 -> V_322 ) {
V_77 -> V_307 . V_323 = V_77 -> V_38 ;
} else {
V_77 -> V_307 . V_323 = V_77 -> V_324 ;
}
V_77 -> V_307 . V_325 = V_77 -> V_326 ;
V_77 -> V_307 . V_327 = V_77 -> V_328 / 1024 ;
V_77 -> V_307 . V_329 = V_77 -> V_330 ;
V_77 -> V_307 . V_331 = V_77 -> V_332 ;
V_77 -> V_307 . V_333 = V_77 -> V_334 ;
V_77 -> V_307 . V_335 = V_77 -> V_336 ;
V_77 -> V_307 . V_337 = V_77 -> V_338 ;
V_77 -> V_307 . V_339 = V_77 -> V_340 ;
V_77 -> V_307 . V_341 = V_77 -> V_119 ;
V_77 -> V_307 . V_342 = V_77 -> V_157 ;
V_77 -> V_307 . V_16 = V_77 -> V_82 . V_343 ;
V_77 -> V_307 . V_17 = V_77 -> V_82 . V_94 ;
V_77 -> V_307 . V_344 = V_77 -> V_82 . V_345 ? 1 : 0 ;
V_77 -> V_307 . V_346 = V_77 -> V_82 . V_347 ? 1 : 0 ;
V_77 -> V_307 . V_348 = V_77 -> V_82 . V_349 ;
V_77 -> V_307 . V_350 = V_77 -> V_82 . V_351 ;
V_77 -> V_307 . V_352 = V_77 -> V_82 . V_353 ;
V_77 -> V_307 . V_354 = V_77 -> V_82 . V_355 ;
V_77 -> V_307 . V_356 = ( T_4 ) ( V_77 -> V_357 + 32 ) ;
V_77 -> V_307 . V_358 = ( T_4 ) ( V_77 -> V_359 + 32 ) ;
V_77 -> V_307 . V_360 = V_77 -> V_361 / 1024 ;
V_77 -> V_307 . V_362 = V_77 -> V_270 ;
V_77 -> V_307 . V_363 = V_77 -> V_364 ;
V_77 -> V_307 . V_365 = V_77 -> V_366 ;
V_77 -> V_307 . V_367 = V_77 -> V_123 ;
V_77 -> V_307 . V_368 = V_77 -> V_368 ? 1 : 0 ;
V_77 -> V_307 . V_369 = V_77 -> V_369 ? 1 : 0 ;
V_77 -> V_307 . V_370 = V_77 -> V_370 ? 1 : 0 ;
if( F_84 ( ( void F_80 * ) V_288 , & V_77 -> V_307 ,
sizeof( V_77 -> V_307 ) ) )
return - V_298 ;
break;
case V_371 :
if( V_77 -> V_305 ++ < 10 )
F_4 ( V_72
L_47 ) ;
case V_372 :
if( F_33 ( V_77 ) )
return F_87 ( ( T_2 ) 1 , V_294 ) ;
else
return F_87 ( ( T_2 ) 0 , V_294 ) ;
case V_373 :
if( V_77 -> V_305 ++ < 10 )
F_4 ( V_72
L_47 ) ;
case V_374 :
if( V_77 -> V_5 )
return F_87 ( ( T_2 ) 1 , V_294 ) ;
else
return F_87 ( ( T_2 ) 0 , V_294 ) ;
case V_375 :
if( V_77 -> V_305 ++ < 10 )
F_4 ( V_72
L_47 ) ;
case V_376 :
if( F_86 ( V_293 , V_294 ) )
return - V_298 ;
V_77 -> V_5 = ( V_293 ) ? 1 : 0 ;
break;
case V_377 :
if( F_86 ( V_293 , V_294 ) )
return - V_298 ;
F_88 ( V_77 , ( ( int ) ( V_293 >> 16 ) ) - 32 ) ;
F_89 ( V_77 , ( ( int ) ( V_293 & 0xffff ) ) - 32 ) ;
break;
case V_378 :
return F_87 ( ( T_2 ) ( ( ( V_77 -> V_357 + 32 ) << 16 ) | ( ( V_77 -> V_359 + 32 ) & 0xffff ) ) ,
V_294 ) ;
case V_379 :
if( F_82 ( & V_77 -> V_380 , ( void F_80 * ) V_288 ,
sizeof( struct V_381 ) ) )
return - V_298 ;
F_90 ( V_77 , & V_77 -> V_380 ) ;
if( F_84 ( ( void F_80 * ) V_288 , & V_77 -> V_380 ,
sizeof( struct V_381 ) ) )
return - V_298 ;
break;
case V_382 :
if( F_86 ( V_293 , V_294 ) )
return - V_298 ;
V_77 -> V_383 = ( V_293 ) ? 1 : 0 ;
break;
default:
#ifdef F_91
return - V_384 ;
#else
return - V_232 ;
#endif
}
return 0 ;
}
static int
F_72 ( struct V_385 * V_279 , int V_386 , struct V_234 * V_235 )
{
struct V_76 * V_77 = (struct V_76 * ) V_235 -> V_236 ;
memset ( V_279 , 0 , sizeof( struct V_385 ) ) ;
F_92 ( V_279 -> V_387 , V_77 -> V_388 , sizeof( V_279 -> V_387 ) ) ;
F_93 ( & V_235 -> V_389 ) ;
V_279 -> V_390 = V_77 -> V_391 + V_77 -> V_270 ;
V_279 -> V_392 = V_77 -> V_209 ;
F_94 ( & V_235 -> V_389 ) ;
V_279 -> type = V_393 ;
V_279 -> V_394 = 0 ;
V_279 -> V_395 = ( V_77 -> V_203 == 8 ) ? V_396 : V_397 ;
V_279 -> V_398 = 1 ;
V_279 -> V_399 = ( V_77 -> V_4 ) ? 1 : 0 ;
V_279 -> V_400 = 0 ;
V_279 -> V_401 = V_77 -> V_211 ;
V_279 -> V_402 = V_77 -> V_403 ;
V_279 -> V_404 = V_77 -> V_405 ;
if( V_77 -> V_122 == V_134 ) {
V_279 -> V_207 = V_406 ;
} else if( ( V_77 -> V_85 == V_407 ) ||
( V_77 -> V_85 == V_408 ) ||
( V_77 -> V_85 == V_409 ) ) {
V_279 -> V_207 = V_410 ;
} else if( V_77 -> V_85 == V_411 ) {
V_279 -> V_207 = V_412 ;
} else if( V_77 -> V_85 >= V_413 ) {
V_279 -> V_207 = V_414 ;
} else {
V_279 -> V_207 = V_415 ;
}
return 0 ;
}
static struct V_416 * F_95 ( int V_417 )
{
struct V_416 * V_418 = NULL ;
int V_419 , V_420 , V_33 ;
static const unsigned short V_421 [] = {
V_422 ,
V_423 ,
V_424 ,
V_425 ,
V_426 ,
V_427 ,
V_428 ,
V_429 ,
V_430 ,
V_431 ,
V_432 ,
V_433
} ;
switch( V_417 ) {
#ifdef F_20
case V_434 : V_420 = 0 ; V_419 = 1 ; break;
case V_435 : V_420 = 1 ; V_419 = 2 ; break;
#endif
#ifdef F_17
case V_436 : V_420 = 3 ; V_419 = 1 ; break;
case V_437 : V_420 = 4 ; V_419 = 3 ; break;
case V_438 : V_420 = 7 ; V_419 = 5 ; break;
#endif
default: return NULL ;
}
for( V_33 = 0 ; V_33 < V_419 ; V_33 ++ ) {
if( ( V_418 = F_96 ( V_439 ,
V_421 [ V_420 + V_33 ] , NULL ) ) )
break;
}
return V_418 ;
}
static int F_97 ( struct V_76 * V_77 )
{
#if F_2 ( F_20 ) || F_2 ( F_17 )
T_3 V_156 ;
#endif
V_77 -> V_320 = 0 ;
V_77 -> V_440 = V_77 -> V_441 = 0 ;
switch( V_77 -> V_85 ) {
#ifdef F_20
case V_442 :
V_156 = F_27 ( V_153 , 0x14 ) ;
V_77 -> V_320 = ( ( V_156 & 0x3F ) + 1 ) << 20 ;
break;
case V_434 :
case V_435 :
case V_443 :
if( ! V_77 -> V_198 )
return - 1 ;
F_98 ( V_77 -> V_198 , 0x63 , & V_156 ) ;
V_77 -> V_320 = 1 << ( ( ( V_156 & 0x70 ) >> 4 ) + 21 ) ;
break;
#endif
#ifdef F_17
case V_444 :
case V_445 :
case V_446 :
V_156 = F_27 ( V_153 , 0x14 ) ;
V_77 -> V_320 = ( 1 << ( ( V_156 & 0xf0 ) >> 4 ) ) << 20 ;
switch( ( V_156 >> 2 ) & 0x03 ) {
case 0x01 :
case 0x03 :
V_77 -> V_320 <<= 1 ;
break;
case 0x02 :
V_77 -> V_320 += ( V_77 -> V_320 / 2 ) ;
}
break;
case V_407 :
V_156 = F_27 ( V_153 , 0x14 ) ;
V_77 -> V_320 = ( 1 << ( ( V_156 & 0xf0 ) >> 4 ) ) << 20 ;
if( V_156 & 0x0c ) V_77 -> V_320 <<= 1 ;
break;
case V_436 :
case V_437 :
case V_447 :
V_156 = F_27 ( V_153 , 0x14 ) ;
V_77 -> V_320 = ( ( ( V_156 & 0x3f ) + 1 ) << 2 ) << 20 ;
break;
case V_448 :
case V_449 :
V_156 = F_27 ( V_152 , 0x79 ) ;
V_77 -> V_320 = ( 1 << ( ( V_156 & 0xf0 ) >> 4 ) ) << 20 ;
break;
case V_438 :
case V_408 :
case V_409 :
V_156 = F_27 ( V_152 , 0x79 ) ;
V_156 = ( V_156 & 0xf0 ) >> 4 ;
if( V_156 ) {
V_77 -> V_320 = ( 1 << V_156 ) << 20 ;
V_77 -> V_440 = V_77 -> V_320 ;
}
V_156 = F_27 ( V_152 , 0x78 ) ;
V_156 &= 0x30 ;
if( V_156 ) {
if( V_156 == 0x10 ) {
V_77 -> V_441 = ( 32 << 20 ) ;
} else {
V_77 -> V_441 = ( 64 << 20 ) ;
}
V_77 -> V_320 += V_77 -> V_441 ;
}
break;
case V_450 :
case V_411 :
case V_413 :
V_156 = F_27 ( V_153 , 0x14 ) ;
V_77 -> V_320 = ( 1 << ( ( V_156 & 0xf0 ) >> 4 ) ) << 20 ;
if( V_77 -> V_85 != V_411 ) {
V_156 = ( V_156 & 0x0c ) >> 2 ;
if( V_77 -> V_451 == 2 ) {
if( V_156 & 0x01 ) V_156 = 0x02 ;
else V_156 = 0x00 ;
}
if( V_156 == 0x02 ) V_77 -> V_320 <<= 1 ;
else if( V_156 == 0x03 ) V_77 -> V_320 <<= 2 ;
}
break;
#endif
default:
return - 1 ;
}
return 0 ;
}
static void F_99 ( struct V_76 * V_77 )
{
T_3 V_452 , V_117 ;
if( V_77 -> V_85 == V_411 ) {
V_77 -> V_10 = 0 ;
return;
}
#ifdef F_20
if( V_77 -> V_122 == V_134 ) {
V_117 = F_27 ( V_153 , 0x17 ) ;
if( ( V_117 & 0x0F ) && ( V_77 -> V_85 != V_442 ) ) {
if( ! ( V_77 -> V_119 & ( V_453 | V_454 | V_455 | V_456 ) ) ) {
V_117 = F_27 ( V_153 , 0x16 ) ;
if( V_117 & 0x20 )
V_77 -> V_119 |= V_453 ;
else
V_77 -> V_119 |= V_454 ;
}
}
}
#endif
V_452 = F_27 ( V_152 , 0x32 ) ;
if( V_452 & V_457 ) {
V_77 -> V_10 = 0 ;
} else {
V_77 -> V_10 = ( V_452 & 0xDF ) ? 1 : 0 ;
}
V_77 -> V_119 &= ~ ( V_145 | V_120 | V_121 ) ;
if( V_452 & V_458 ) V_77 -> V_119 |= V_145 ;
if( V_452 & V_459 ) V_77 -> V_119 |= V_120 ;
if( V_452 & V_460 ) V_77 -> V_119 |= V_121 ;
if( V_77 -> V_85 != V_436 ) {
V_77 -> V_20 = V_77 -> V_21 = 0 ;
}
if( V_77 -> V_22 != - 1 ) {
if( ( V_77 -> V_122 != V_131 ) ||
( ! ( V_77 -> V_123 & V_461 ) ) ) {
if( V_77 -> V_22 & V_462 ) {
V_77 -> V_22 = - 1 ;
F_4 ( V_35 L_48 ) ;
}
}
}
if( V_77 -> V_22 != - 1 ) {
if( ( V_77 -> V_122 != V_131 ) ||
( ! ( V_77 -> V_123 & V_463 ) ) ) {
if( V_77 -> V_22 & V_464 ) {
V_77 -> V_22 = - 1 ;
F_4 ( V_35 L_49 ) ;
}
}
}
if( V_77 -> V_23 != - 1 ) {
if( ( ! ( V_77 -> V_123 & V_465 ) ) &&
( ! ( ( V_77 -> V_122 == V_131 ) &&
( V_77 -> V_123 & V_190 ) ) ) ) {
if( V_77 -> V_23 & ( V_455 | V_456 | V_466 ) ) {
V_77 -> V_23 = - 1 ;
F_4 ( V_35 L_50 ) ;
}
}
}
if( V_77 -> V_22 != - 1 ) {
V_77 -> V_119 |= V_77 -> V_22 ;
} else {
if( V_452 & V_467 ) V_77 -> V_119 |= ( V_462 | V_468 ) ;
else if( V_452 & V_469 ) V_77 -> V_119 |= V_464 ;
else if( V_452 & V_470 ) V_77 -> V_119 |= V_471 ;
else {
if( V_452 & V_472 ) V_77 -> V_119 |= V_473 ;
if( V_452 & V_474 ) V_77 -> V_119 |= V_475 ;
}
}
if( ! ( V_77 -> V_119 & ( V_462 | V_464 ) ) ) {
if( V_77 -> V_23 != - 1 ) {
V_77 -> V_119 &= ~ ( V_454 | V_453 | V_455 | V_456 | V_466 ) ;
V_77 -> V_119 |= V_77 -> V_23 ;
}
if( V_77 -> V_119 & V_471 ) {
V_77 -> V_119 &= ~ ( V_454 | V_455 | V_456 | V_466 ) ;
V_77 -> V_119 |= V_453 ;
}
if( ! ( V_77 -> V_119 & ( V_453 | V_454 | V_455 | V_456 | V_466 ) ) ) {
if( V_77 -> V_122 == V_134 ) {
V_117 = F_27 ( V_153 , 0x38 ) ;
if( V_117 & 0x01 ) V_77 -> V_119 |= V_453 ;
else V_77 -> V_119 |= V_454 ;
} else if( ( V_77 -> V_85 <= V_445 ) || ( V_77 -> V_85 >= V_407 ) ) {
V_117 = F_27 ( V_153 , 0x38 ) ;
if( V_117 & 0x01 ) V_77 -> V_119 |= V_453 ;
else V_77 -> V_119 |= V_454 ;
} else {
V_117 = F_27 ( V_152 , 0x79 ) ;
if( V_117 & 0x20 ) V_77 -> V_119 |= V_453 ;
else V_77 -> V_119 |= V_454 ;
}
}
}
if( V_77 -> V_11 != - 1 ) {
V_77 -> V_10 = ( V_77 -> V_11 ) ? 0 : 1 ;
}
}
static bool F_100 ( struct V_76 * V_77 )
{
unsigned short V_476 ;
int V_301 = 48 ;
V_476 = F_101 ( & V_77 -> V_82 ) ;
do {
if( V_476 != F_101 ( & V_77 -> V_82 ) ) break;
} while( V_301 -- );
return ( V_301 != - 1 ) ;
}
static void F_102 ( struct V_76 * V_77 )
{
bool V_477 = false ;
T_3 V_478 , V_479 ;
#ifdef F_17
T_3 V_177 = 0 ;
#endif
T_4 V_117 = 0xffff ;
int V_33 ;
V_478 = F_27 ( V_153 , 0x1F ) ;
F_56 ( V_153 , 0x1F , 0x04 ) ;
F_103 ( V_153 , 0x1F , 0x3F ) ;
if( V_478 & 0xc0 ) V_477 = true ;
#ifdef F_17
if( V_77 -> V_122 == V_131 ) {
V_177 = F_27 ( V_152 , V_77 -> V_82 . V_188 ) ;
V_177 &= 0x40 ;
F_103 ( V_152 , V_77 -> V_82 . V_188 , 0xBF ) ;
}
#endif
V_479 = F_27 ( V_152 , 0x17 ) ;
V_479 &= 0x80 ;
if( ! V_479 ) {
F_56 ( V_152 , 0x17 , 0x80 ) ;
V_477 = true ;
F_55 ( V_153 , 0x00 , 0x01 ) ;
F_55 ( V_153 , 0x00 , 0x03 ) ;
}
if( V_477 ) {
for( V_33 = 0 ; V_33 < 10 ; V_33 ++ ) F_31 ( V_77 ) ;
}
#ifdef F_17
if( V_77 -> V_85 >= V_407 ) {
F_103 ( V_152 , 0x32 , ~ 0x20 ) ;
if( V_77 -> V_85 >= V_450 ) {
F_55 ( V_152 , 0x57 , 0x4a ) ;
} else {
F_55 ( V_152 , 0x57 , 0x5f ) ;
}
F_56 ( V_152 , 0x53 , 0x02 ) ;
while ( ( F_30 ( V_154 ) ) & 0x01 ) break;
while ( ! ( ( F_30 ( V_154 ) ) & 0x01 ) ) break;
if ( ( F_30 ( V_480 ) ) & 0x10 ) V_117 = 1 ;
F_103 ( V_152 , 0x53 , 0xfd ) ;
F_103 ( V_152 , 0x57 , 0x00 ) ;
}
#endif
if( V_117 == 0xffff ) {
V_33 = 3 ;
do {
V_117 = F_15 ( & V_77 -> V_82 , V_77 -> V_119 ,
V_77 -> V_122 , 0 , 0 , NULL , V_77 -> V_123 ) ;
} while( ( ( V_117 == 0 ) || ( V_117 == 0xffff ) ) && V_33 -- );
if( ( V_117 == 0 ) || ( V_117 == 0xffff ) ) {
if( F_100 ( V_77 ) ) V_117 = 1 ;
}
}
if( ( V_117 ) && ( V_117 != 0xffff ) ) {
F_56 ( V_152 , 0x32 , 0x20 ) ;
}
#ifdef F_17
if( V_77 -> V_122 == V_131 ) {
F_36 ( V_152 , V_77 -> V_82 . V_188 , 0xBF , V_177 ) ;
}
#endif
F_36 ( V_152 , 0x17 , 0x7F , V_479 ) ;
F_55 ( V_153 , 0x1F , V_478 ) ;
}
static void F_104 ( struct V_76 * V_77 )
{
unsigned char V_97 [ 256 ] ;
unsigned short V_117 , V_118 , V_33 ;
T_3 V_156 , V_481 = 0 , V_482 = 0 ;
T_4 V_42 , V_43 ;
V_77 -> V_82 . V_483 = false ;
if( ! ( V_77 -> V_123 & V_484 ) )
return;
if( V_77 -> V_123 & V_192 )
return;
V_156 = F_27 ( V_152 , 0x32 ) ;
if( V_156 & 0x08 )
return;
V_118 = 1 ;
if( V_77 -> V_82 . V_485 )
V_118 = 0 ;
V_117 = F_15 ( & V_77 -> V_82 , V_77 -> V_119 , V_77 -> V_122 ,
V_118 , 0 , & V_97 [ 0 ] , V_77 -> V_123 ) ;
if( ( ! V_117 ) || ( V_117 == 0xffff ) || ( ! ( V_117 & 0x02 ) ) )
return;
V_33 = 3 ;
do {
V_117 = F_15 ( & V_77 -> V_82 , V_77 -> V_119 ,
V_77 -> V_122 , V_118 , 1 ,
& V_97 [ 0 ] , V_77 -> V_123 ) ;
} while( ( V_117 ) && V_33 -- );
if( V_117 )
return;
if( ! ( V_97 [ 0x14 ] & 0x80 ) )
return;
if( ! ( V_97 [ 0x18 ] & 0x02 ) )
return;
V_42 = V_97 [ 0x38 ] | ( ( V_97 [ 0x3a ] & 0xf0 ) << 4 ) ;
V_43 = V_97 [ 0x3b ] | ( ( V_97 [ 0x3d ] & 0xf0 ) << 4 ) ;
switch( V_42 ) {
case 1024 :
if( V_43 == 768 )
V_482 = 0x02 ;
break;
case 1280 :
if( V_43 == 1024 )
V_482 = 0x03 ;
break;
case 1600 :
if( ( V_43 == 1200 ) && ( V_77 -> V_123 & V_486 ) )
V_482 = 0x0b ;
break;
}
if( ! V_482 )
return;
if( V_97 [ 0x23 ] )
V_481 |= 0x10 ;
if( ( V_97 [ 0x47 ] & 0x18 ) == 0x18 )
V_481 |= ( ( ( ( V_97 [ 0x47 ] & 0x06 ) ^ 0x06 ) << 5 ) | 0x20 ) ;
else
V_481 |= 0xc0 ;
F_55 ( V_152 , 0x36 , V_482 ) ;
V_481 &= 0xf1 ;
F_36 ( V_152 , 0x37 , 0x0c , V_481 ) ;
F_56 ( V_152 , 0x32 , 0x08 ) ;
V_77 -> V_82 . V_483 = true ;
}
static int F_105 ( struct V_76 * V_77 , T_4 type , T_4 V_487 )
{
int V_117 , V_488 , V_489 , V_33 , V_34 ;
for( V_34 = 0 ; V_34 < 10 ; V_34 ++ ) {
V_489 = 0 ;
for( V_33 = 0 ; V_33 < 3 ; V_33 ++ ) {
V_488 = V_487 ;
F_55 ( V_490 , 0x11 , ( type & 0x00ff ) ) ;
V_117 = ( type >> 8 ) | ( V_488 & 0x00ff ) ;
F_36 ( V_490 , 0x10 , 0xe0 , V_117 ) ;
F_106 ( & V_77 -> V_82 , 0x1500 ) ;
V_488 >>= 8 ;
V_488 &= 0x7f ;
V_117 = F_27 ( V_490 , 0x03 ) ;
V_117 ^= 0x0e ;
V_117 &= V_488 ;
if( V_117 == V_488 ) V_489 ++ ;
#if 1
F_55 ( V_490 , 0x11 , 0x00 ) ;
F_103 ( V_490 , 0x10 , 0xe0 ) ;
F_106 ( & V_77 -> V_82 , 0x1000 ) ;
#endif
}
if( ( V_489 == 0 ) || ( V_489 >= 2 ) ) break;
}
return V_489 ;
}
static void F_107 ( struct V_76 * V_77 )
{
T_3 V_491 , V_492 , V_493 , V_494 , V_495 = 0 ;
T_4 V_496 = 0 , V_497 = 0 ;
T_4 V_498 = 0 , V_499 = 0 ;
T_4 V_500 = 0 , V_501 = 0 ;
int V_502 , V_489 ;
char V_503 [] = L_51 ;
char V_504 [] = L_52 ;
if( V_77 -> V_123 & V_191 ) {
V_496 = 0x00b9 ; V_498 = 0x00b3 ; V_500 = 0x00d1 ;
V_502 = F_27 ( V_490 , 0x01 ) ;
if( V_502 & 0x04 ) {
V_496 = 0x00dd ; V_498 = 0x00ee ; V_500 = 0x00fd ;
}
} else if( V_77 -> V_123 & ( V_505 | V_506 ) ) {
V_496 = 0x016b ; V_498 = 0x0174 ; V_500 = 0x0190 ;
} else if( V_77 -> V_123 & ( V_507 | V_508 ) ) {
V_496 = 0x0200 ; V_498 = 0x0100 ;
} else if( V_77 -> V_123 & ( V_509 | V_510 | V_511 | V_512 ) ) {
V_496 = 0x016b ; V_498 = 0x0110 ; V_500 = 0x0190 ;
} else
return;
V_501 = 0x0e08 ; V_497 = 0x0404 ; V_499 = 0x0804 ;
if( V_77 -> V_119 & ( V_507 | V_508 | V_510 | V_512 ) ) {
V_497 = 0x0408 ; V_499 = 0x0808 ;
}
V_495 = 2 ;
if( V_77 -> V_513 ) {
V_495 = V_77 -> V_514 [ 0x58 ] & 0x03 ;
} else if( V_77 -> V_515 ) {
if( V_77 -> V_514 [ 0x5d ] & 0x04 ) V_495 |= 0x01 ;
} else if( V_77 -> V_122 == V_134 ) {
if( V_77 -> V_514 ) {
V_495 = V_77 -> V_514 [ 0xfe ] & 0x03 ;
}
}
if( V_77 -> V_85 == V_442 ) {
V_502 = F_27 ( V_153 , 0x3b ) ;
if( ! ( V_502 & 0x01 ) ) V_500 = V_501 = 0 ;
}
if( ! ( V_77 -> V_123 & V_516 ) ) {
V_500 = V_501 = 0 ;
}
V_494 = F_27 ( V_153 , 0x1e ) ;
F_56 ( V_153 , 0x1e , 0x20 ) ;
V_491 = F_27 ( V_490 , 0x0d ) ;
if( V_77 -> V_123 & V_486 ) {
F_36 ( V_490 , 0x0d , ~ 0x07 , 0x01 ) ;
} else {
F_56 ( V_490 , 0x0d , 0x04 ) ;
}
F_106 ( & V_77 -> V_82 , 0x2000 ) ;
V_492 = F_27 ( V_195 , 0x00 ) ;
F_55 ( V_195 , 0x00 , ( ( V_492 | 0x1c ) & 0xfc ) ) ;
V_493 = F_27 ( V_195 , 0x4d ) ;
if( V_77 -> V_123 & V_461 ) {
F_55 ( V_195 , 0x4d , ( V_493 & ~ 0x10 ) ) ;
}
if( ! ( V_77 -> V_123 & V_517 ) ) {
F_105 ( V_77 , 0 , 0 ) ;
}
F_103 ( V_152 , 0x32 , ~ 0x14 ) ;
if( V_501 || V_500 ) {
if( F_105 ( V_77 , V_500 , V_501 ) ) {
if( V_495 & 0x01 ) {
F_4 ( V_72 L_53 , V_503 , V_504 ) ;
F_56 ( V_152 , 0x32 , 0x04 ) ;
} else {
F_4 ( V_72 L_54 , V_503 ) ;
F_56 ( V_152 , 0x32 , 0x10 ) ;
}
}
}
F_103 ( V_152 , 0x32 , 0x3f ) ;
if( V_77 -> V_123 & V_517 ) {
F_56 ( V_490 , 0x0d , 0x04 ) ;
}
if( ( V_77 -> V_122 == V_131 ) && ( V_77 -> V_123 & V_461 ) ) {
F_55 ( V_195 , 0x4d , ( V_493 | 0x10 ) ) ;
F_106 ( & V_77 -> V_82 , 0x2000 ) ;
if( ( V_489 = F_105 ( V_77 , V_496 , 0x0604 ) ) ) {
if( ( V_489 = F_105 ( V_77 , V_498 , 0x0804 ) ) ) {
F_4 ( V_72 L_55 , V_503 , V_504 ) ;
F_56 ( V_152 , 0x32 , 0x80 ) ;
}
}
F_55 ( V_195 , 0x4d , V_493 ) ;
}
F_103 ( V_152 , 0x32 , ~ 0x03 ) ;
if( ! ( V_77 -> V_119 & V_462 ) ) {
if( ( V_489 = F_105 ( V_77 , V_496 , V_497 ) ) ) {
F_4 ( V_72 L_56 , V_503 , V_504 ) ;
F_56 ( V_152 , 0x32 , 0x02 ) ;
}
if( ( V_495 & 0x02 ) || ( ! V_489 ) ) {
if( F_105 ( V_77 , V_498 , V_499 ) ) {
F_4 ( V_72 L_57 , V_503 , V_504 ) ;
F_56 ( V_152 , 0x32 , 0x01 ) ;
}
}
}
F_105 ( V_77 , 0 , 0 ) ;
F_55 ( V_195 , 0x00 , V_492 ) ;
F_55 ( V_490 , 0x0d , V_491 ) ;
F_55 ( V_153 , 0x1e , V_494 ) ;
if( V_77 -> V_123 & V_517 ) {
V_495 = F_27 ( V_195 , 0x00 ) ;
if( V_495 & 0x20 ) {
for( V_502 = 2 ; V_502 > 0 ; V_502 -- ) {
V_495 ^= 0x20 ;
F_55 ( V_195 , 0x00 , V_495 ) ;
}
}
}
F_55 ( V_195 , 0x00 , V_492 ) ;
}
static void F_108 ( struct V_76 * V_77 )
{
#if F_2 ( F_20 ) || F_2 ( F_17 )
T_3 V_518 , V_519 ;
char V_503 [] = L_58 ;
#endif
#ifdef F_20
unsigned char V_487 [ 3 ] ;
int V_33 ;
#endif
if( V_77 -> V_85 < V_444 ) {
#ifdef F_20
V_77 -> V_82 . V_520 = 1 ;
F_109 ( & V_77 -> V_82 , 0x9c ) ;
F_106 ( & V_77 -> V_82 , 1000 ) ;
V_518 = F_110 ( & V_77 -> V_82 , 0x25 ) ;
V_519 = F_110 ( & V_77 -> V_82 , 0x0e ) ;
if( ( ( V_519 & 0x07 ) == 0x01 ) || ( V_519 & 0x04 ) ) {
F_111 ( & V_77 -> V_82 , 0x0e , 0x0b ) ;
F_106 ( & V_77 -> V_82 , 300 ) ;
}
V_519 = F_110 ( & V_77 -> V_82 , 0x25 ) ;
if( V_519 != V_518 ) V_518 = V_519 ;
if( ( V_518 >= 0x22 ) && ( V_518 <= 0x50 ) ) {
V_518 = F_110 ( & V_77 -> V_82 , 0x0e ) ;
if( ( V_518 & 0x03 ) != 0x03 ) {
F_111 ( & V_77 -> V_82 , 0x0e , 0x0b ) ;
F_106 ( & V_77 -> V_82 , 300 ) ;
}
for( V_33 = 0 ; V_33 < 3 ; V_33 ++ ) {
F_111 ( & V_77 -> V_82 , 0x10 , 0x01 ) ;
F_106 ( & V_77 -> V_82 , 0x96 ) ;
F_111 ( & V_77 -> V_82 , 0x10 , 0x00 ) ;
F_106 ( & V_77 -> V_82 , 0x96 ) ;
V_518 = F_110 ( & V_77 -> V_82 , 0x10 ) ;
if( ! ( V_518 & 0x08 ) ) V_487 [ V_33 ] = 0x02 ;
else if( ! ( V_518 & 0x02 ) ) V_487 [ V_33 ] = 0x01 ;
else V_487 [ V_33 ] = 0 ;
F_106 ( & V_77 -> V_82 , 0x96 ) ;
}
if( V_487 [ 0 ] == V_487 [ 1 ] ) V_518 = V_487 [ 0 ] ;
else if( V_487 [ 0 ] == V_487 [ 2 ] ) V_518 = V_487 [ 0 ] ;
else if( V_487 [ 1 ] == V_487 [ 2 ] ) V_518 = V_487 [ 1 ] ;
else {
F_4 ( V_72
L_59 ) ;
V_518 = V_487 [ 2 ] ;
}
if( V_518 == 0x02 ) {
F_4 ( V_72 L_60 , V_503 ) ;
V_77 -> V_119 |= V_473 ;
F_56 ( V_152 , 0x32 , 0x02 ) ;
F_103 ( V_152 , 0x32 , ~ 0x05 ) ;
} else if ( V_518 == 0x01 ) {
F_4 ( V_72 L_61 , V_503 ) ;
V_77 -> V_119 |= V_475 ;
F_56 ( V_152 , 0x32 , 0x01 ) ;
F_103 ( V_152 , 0x32 , ~ 0x06 ) ;
} else {
F_112 ( & V_77 -> V_82 , 0x0e , 0x01 , 0xF8 ) ;
F_103 ( V_152 , 0x32 , ~ 0x07 ) ;
}
} else if( V_518 == 0 ) {
F_112 ( & V_77 -> V_82 , 0x0e , 0x01 , 0xF8 ) ;
F_103 ( V_152 , 0x32 , ~ 0x07 ) ;
}
F_109 ( & V_77 -> V_82 , 0x00 ) ;
#endif
} else {
#ifdef F_17
V_77 -> V_82 . V_520 = 2 ;
V_518 = F_113 ( & V_77 -> V_82 , 0x49 ) ;
F_114 ( & V_77 -> V_82 , 0x49 , 0x20 ) ;
F_106 ( & V_77 -> V_82 , 0x96 ) ;
V_519 = F_113 ( & V_77 -> V_82 , 0x20 ) ;
V_519 |= 0x01 ;
F_114 ( & V_77 -> V_82 , 0x20 , V_519 ) ;
F_106 ( & V_77 -> V_82 , 0x96 ) ;
V_519 ^= 0x01 ;
F_114 ( & V_77 -> V_82 , 0x20 , V_519 ) ;
F_106 ( & V_77 -> V_82 , 0x96 ) ;
V_519 = F_113 ( & V_77 -> V_82 , 0x20 ) ;
F_114 ( & V_77 -> V_82 , 0x49 , V_518 ) ;
V_518 = 0 ;
if( V_519 & 0x02 ) V_518 |= 0x01 ;
if( V_519 & 0x10 ) V_518 |= 0x01 ;
if( V_519 & 0x04 ) V_518 |= 0x02 ;
if( ( V_518 & 0x01 ) && ( V_518 & 0x02 ) ) V_518 = 0x04 ;
switch( V_518 ) {
case 0x01 :
F_4 ( V_72 L_61 , V_503 ) ;
V_77 -> V_119 |= V_475 ;
F_56 ( V_152 , 0x32 , 0x01 ) ;
F_103 ( V_152 , 0x32 , ~ 0x06 ) ;
break;
case 0x02 :
F_4 ( V_72 L_60 , V_503 ) ;
V_77 -> V_119 |= V_473 ;
F_56 ( V_152 , 0x32 , 0x02 ) ;
F_103 ( V_152 , 0x32 , ~ 0x05 ) ;
break;
case 0x04 :
F_4 ( V_72 L_62 , V_503 ) ;
F_56 ( V_152 , 0x32 , 0x04 ) ;
F_103 ( V_152 , 0x32 , ~ 0x03 ) ;
break;
default:
F_103 ( V_152 , 0x32 , ~ 0x07 ) ;
}
#endif
}
}
static void F_115 ( struct V_76 * V_77 )
{
char V_503 [] = L_51 ;
char V_521 [] = L_63 ;
T_3 V_522 ;
T_3 V_156 ;
if( V_77 -> V_85 == V_411 )
return;
V_522 = F_27 ( V_490 , 0x00 ) ;
switch( V_522 ) {
case 0x01 :
V_156 = F_27 ( V_490 , 0x01 ) ;
if( V_156 < 0xb0 ) {
V_77 -> V_119 |= V_523 ;
V_77 -> V_123 |= V_191 ;
F_4 ( V_72 L_64 , V_503 , V_521 ) ;
} else if( V_156 < 0xc0 ) {
V_77 -> V_119 |= V_524 ;
V_77 -> V_123 |= V_505 ;
V_156 = F_27 ( V_490 , 0x23 ) ;
if( ! ( V_156 & 0x02 ) ) {
V_77 -> V_119 |= V_525 ;
V_77 -> V_123 |= V_192 ;
F_4 ( V_72 L_65 , V_503 , V_521 ) ;
} else {
F_4 ( V_72 L_66 , V_503 , V_521 ) ;
}
} else if( V_156 < 0xd0 ) {
V_77 -> V_119 |= V_526 ;
V_77 -> V_123 |= V_509 ;
F_4 ( V_72 L_67 , V_503 , V_521 ) ;
} else if( V_156 < 0xe0 ) {
V_77 -> V_119 |= V_527 ;
V_77 -> V_123 |= V_507 ;
F_4 ( V_72 L_68 , V_503 , V_521 ) ;
} else if( V_156 <= 0xe1 ) {
V_156 = F_27 ( V_490 , 0x39 ) ;
if( V_156 == 0xff ) {
V_77 -> V_119 |= V_528 ;
V_77 -> V_123 |= V_508 ;
F_4 ( V_72 L_69 , V_503 , V_521 ) ;
} else {
V_77 -> V_119 |= V_526 ;
V_77 -> V_123 |= V_509 ;
F_4 ( V_72 L_70 , V_503 , V_521 ) ;
#if 0
ivideo->vbflags |= VB_302ELV;
ivideo->vbflags2 |= VB2_302ELV;
printk(KERN_INFO "%s SiS302ELV %s\n", stdstr, bridgestr);
#endif
}
}
break;
case 0x02 :
V_77 -> V_119 |= V_529 ;
V_77 -> V_123 |= V_506 ;
F_4 ( V_72 L_71 , V_503 , V_521 ) ;
break;
}
if( ( ! ( V_77 -> V_123 & V_150 ) ) && ( V_77 -> V_85 != V_442 ) ) {
V_156 = F_27 ( V_152 , 0x37 ) ;
V_156 &= V_530 ;
V_156 >>= 1 ;
if( V_77 -> V_122 == V_134 ) {
#ifdef F_20
switch( V_156 ) {
case V_531 :
V_77 -> V_119 |= V_532 ;
V_77 -> V_123 |= V_193 ;
break;
case V_533 :
V_77 -> V_119 |= ( V_532 | V_534 ) ;
V_77 -> V_123 |= ( V_193 | V_535 ) ;
break;
case V_536 :
V_77 -> V_119 |= V_537 ;
V_77 -> V_123 |= V_190 ;
break;
case V_538 :
V_77 -> V_119 |= ( V_532 | V_537 ) ;
V_77 -> V_123 |= ( V_193 | V_190 ) ;
break;
}
if( V_77 -> V_123 & V_190 ) V_77 -> V_539 = 1 ;
#endif
} else if( V_77 -> V_85 < V_448 ) {
#ifdef F_17
switch ( V_156 ) {
case V_540 :
V_77 -> V_119 |= V_532 ;
V_77 -> V_123 |= V_193 ;
break;
case V_541 :
V_77 -> V_119 |= ( V_532 | V_537 ) ;
V_77 -> V_123 |= ( V_193 | V_190 ) ;
break;
}
if( V_77 -> V_123 & V_190 ) V_77 -> V_539 = 2 ;
#endif
} else if( V_77 -> V_85 >= V_448 ) {
#ifdef F_17
V_156 = F_27 ( V_152 , 0x38 ) ;
V_156 >>= 5 ;
switch( V_156 ) {
case 0x02 :
V_77 -> V_119 |= V_532 ;
V_77 -> V_123 |= V_193 ;
break;
case 0x03 :
V_77 -> V_119 |= ( V_532 | V_537 ) ;
V_77 -> V_123 |= ( V_193 | V_190 ) ;
break;
case 0x04 :
V_77 -> V_119 |= ( V_532 | V_542 ) ;
V_77 -> V_123 |= ( V_193 | V_543 ) ;
break;
}
if( V_77 -> V_123 & V_190 ) V_77 -> V_539 = 2 ;
#endif
}
if( V_77 -> V_123 & V_193 ) {
F_4 ( V_72 L_72 , V_503 ) ;
}
if( ( V_77 -> V_122 == V_134 ) && ( V_77 -> V_123 & V_535 ) ) {
F_4 ( V_72 L_73 , V_503 ) ;
}
if( V_77 -> V_123 & V_190 ) {
F_4 ( V_72 L_74 , V_503 ) ;
}
if( ( V_77 -> V_85 >= V_448 ) && ( V_77 -> V_123 & V_543 ) ) {
F_4 ( V_72 L_75 , V_503 ) ;
}
}
if( V_77 -> V_123 & V_465 ) {
F_104 ( V_77 ) ;
F_107 ( V_77 ) ;
} else if( V_77 -> V_123 & V_190 ) {
F_108 ( V_77 ) ;
}
}
static void
F_116 ( struct V_76 * V_77 )
{
V_77 -> V_326 &= ~ ( V_544 |
V_545 |
V_546 |
V_547 ) ;
#ifdef F_20
if( V_77 -> V_122 == V_134 ) {
T_2 V_548 ;
T_3 V_549 ;
V_548 = ( V_77 -> V_320 - V_77 -> V_328 ) / ( 64 * 1024 ) ;
V_549 = F_27 ( V_153 , V_550 ) ;
V_549 |= 0xf0 ;
V_549 &= 0xfc ;
V_549 |= ( T_3 ) ( V_548 >> 8 ) ;
F_55 ( V_153 , V_550 , V_549 ) ;
F_55 ( V_153 , V_551 , ( T_3 ) ( V_548 & 0xff ) ) ;
V_77 -> V_326 |= V_544 ;
}
#endif
#ifdef F_17
if( V_77 -> V_122 == V_131 ) {
T_2 V_552 = 0 , V_553 ;
T_3 V_117 ;
if( V_77 -> V_85 == V_411 ) {
switch( V_77 -> V_328 ) {
case ( 64 * 1024 ) :
V_117 = V_554 ;
break;
case ( 128 * 1024 ) :
default:
V_117 = V_555 ;
}
} else {
switch( V_77 -> V_328 ) {
case ( 4 * 1024 * 1024 ) :
V_117 = V_556 ;
break;
case ( 2 * 1024 * 1024 ) :
V_117 = V_557 ;
break;
case ( 1 * 1024 * 1024 ) :
V_117 = V_558 ;
break;
default:
case ( 512 * 1024 ) :
V_117 = V_559 ;
}
}
F_55 ( V_153 , V_560 , V_561 ) ;
F_55 ( V_153 , V_562 , V_563 ) ;
if( ( V_77 -> V_85 >= V_413 ) && V_77 -> V_322 ) {
if( ! ( ( V_553 = F_117 ( V_77 -> V_564 , 0x8240 ) ) & ( 1 << 10 ) ) ) {
F_118 ( V_77 -> V_564 , V_565 , 0 ) ;
F_55 ( V_153 , V_562 , ( V_117 | V_566 ) ) ;
V_552 = F_117 ( V_77 -> V_564 , V_567 ) ;
F_118 ( V_77 -> V_564 , V_565 , V_552 ) ;
V_552 = ( T_2 ) ( V_77 -> V_320 - V_77 -> V_328 ) ;
F_118 ( V_77 -> V_564 , V_568 , V_552 ) ;
F_119 ( 0x16800000 + 0x8240 , V_77 -> V_569 + V_552 ) ;
F_119 ( V_553 | ( 1 << 10 ) , V_77 -> V_569 + V_552 + 4 ) ;
F_119 ( 0x168F0000 , V_77 -> V_569 + V_552 + 8 ) ;
F_119 ( 0x168F0000 , V_77 -> V_569 + V_552 + 12 ) ;
F_118 ( V_77 -> V_564 , V_565 , ( V_552 + 16 ) ) ;
F_120 ( V_77 ) ;
F_55 ( V_153 , V_562 , V_563 ) ;
}
}
V_552 = F_117 ( V_77 -> V_564 , V_570 ) ;
F_118 ( V_77 -> V_564 , V_571 , V_552 ) ;
V_117 |= ( V_572 | V_573 ) ;
F_55 ( V_153 , V_562 , V_117 ) ;
V_552 = ( T_2 ) ( V_77 -> V_320 - V_77 -> V_328 ) ;
F_118 ( V_77 -> V_564 , V_574 , V_552 ) ;
V_77 -> V_326 |= V_545 ;
}
#endif
V_77 -> V_575 = 1 ;
}
static void F_121 ( struct V_76 * V_77 )
{
T_3 V_156 ;
int V_33 ;
V_156 = F_27 ( V_152 , 0x36 ) ;
V_156 &= 0x0f ;
if( V_77 -> V_122 == V_134 ) {
V_77 -> V_576 = V_577 [ V_156 ] ;
} else if( V_77 -> V_85 >= V_448 ) {
V_77 -> V_576 = V_578 [ V_156 ] ;
} else {
V_77 -> V_576 = V_579 [ V_156 ] ;
if( ( V_77 -> V_85 == V_436 ) && ( V_21 ) ) {
if( ( V_77 -> V_576 != V_580 ) &&
( V_77 -> V_576 != V_581 ) ) {
V_77 -> V_576 = V_582 ;
}
}
}
if( V_77 -> V_576 == V_583 ) {
V_77 -> V_576 = V_584 ;
F_36 ( V_152 , 0x36 , 0xf0 , 0x02 ) ;
F_36 ( V_152 , 0x37 , 0xee , 0x01 ) ;
F_4 ( V_58 L_76 , V_156 ) ;
}
for( V_33 = 0 ; V_33 < V_585 ; V_33 ++ ) {
if( V_77 -> V_576 == V_586 [ V_33 ] . V_587 ) {
V_77 -> V_139 = V_586 [ V_33 ] . V_42 ;
V_77 -> V_140 = V_586 [ V_33 ] . V_43 ;
V_77 -> V_588 = V_586 [ V_33 ] . V_589 ;
break;
}
}
#ifdef F_20
if( V_77 -> V_82 . V_94 == V_590 ) {
V_77 -> V_139 = 1360 ; V_77 -> V_140 = 1024 ;
V_77 -> V_588 = V_591 ;
} else if( V_77 -> V_82 . V_94 == V_141 ) {
V_77 -> V_139 = 848 ; V_77 -> V_140 = 480 ;
V_77 -> V_588 = V_592 ;
} else if( V_77 -> V_82 . V_94 == V_142 ) {
V_77 -> V_139 = 856 ; V_77 -> V_140 = 480 ;
V_77 -> V_588 = V_593 ;
}
#endif
F_4 ( V_58 L_77 ,
V_77 -> V_139 , V_77 -> V_140 ) ;
}
static void F_122 ( struct V_76 * V_77 )
{
#ifdef F_20
if( V_77 -> V_122 == V_134 ) {
if( V_77 -> V_123 & ( V_193 | V_192 ) ) {
int V_594 ;
V_594 = F_27 ( V_152 , 0x30 ) ;
if( V_594 & 0x20 ) {
V_77 -> V_336 = F_27 ( V_151 , 0x13 ) ;
V_77 -> V_336 &= 0x3c ;
if( V_77 -> V_82 . V_595 == - 1 ) {
V_77 -> V_82 . V_595 = V_77 -> V_336 ;
}
F_4 ( V_72 L_78 ,
V_77 -> V_336 ) ;
}
if( ( V_77 -> V_82 . V_595 != - 1 ) &&
( V_77 -> V_82 . V_595 != V_77 -> V_336 ) ) {
F_4 ( V_72 L_79 ,
V_77 -> V_82 . V_595 ) ;
}
}
}
#endif
#ifdef F_17
if( V_77 -> V_122 == V_131 ) {
if( V_77 -> V_123 & V_596 ) {
int V_594 ;
V_594 = F_27 ( V_151 , 0x13 ) ;
if( V_594 & 0x04 ) {
V_77 -> V_82 . V_597 = true ;
V_77 -> V_340 = 0x03 ;
}
}
if( V_77 -> V_123 & V_189 ) {
int V_594 ;
V_594 = F_27 ( V_152 , 0x30 ) ;
if( ( V_594 & 0x20 ) || ( V_77 -> V_340 != 0xff ) ) {
T_3 V_598 ;
V_598 = F_27 ( V_151 , 0x2D ) ;
V_77 -> V_336 = ( V_598 & 0x0f ) << 1 ;
V_77 -> V_338 = ( V_598 & 0xf0 ) >> 3 ;
V_598 = F_27 ( V_151 , 0x35 ) ;
V_77 -> V_336 |= ( ( V_598 >> 7 ) & 0x01 ) ;
V_598 = F_27 ( V_151 , 0x20 ) ;
V_77 -> V_338 |= ( ( V_598 >> 6 ) & 0x01 ) ;
if( V_77 -> V_515 ) {
if( V_77 -> V_340 != 0xff ) {
V_77 -> V_336 = 0xff ;
} else {
V_77 -> V_338 = 0xff ;
}
}
if( V_77 -> V_82 . V_595 == - 1 ) {
if( V_77 -> V_336 != 0xff ) {
V_77 -> V_82 . V_595 = V_77 -> V_336 ;
}
}
if( V_77 -> V_82 . V_599 == - 1 ) {
if( V_77 -> V_338 != 0xff ) {
V_77 -> V_82 . V_599 = V_77 -> V_338 ;
}
}
if( V_77 -> V_336 != 0xff ) {
F_4 ( V_72
L_80 ,
V_77 -> V_336 ) ;
}
if( V_77 -> V_338 != 0xff ) {
F_4 ( V_72
L_81 ,
V_77 -> V_338 ) ;
}
}
if( V_77 -> V_123 & V_600 ) {
V_77 -> V_82 . V_349 = F_27 ( V_490 , 0x30 ) ;
V_77 -> V_82 . V_351 = F_27 ( V_490 , 0x31 ) ;
V_77 -> V_82 . V_353 = F_27 ( V_490 , 0x32 ) ;
V_77 -> V_82 . V_355 = F_27 ( V_490 , 0x33 ) ;
V_77 -> V_82 . V_345 = true ;
if( ( V_594 & 0x20 ) || ( V_77 -> V_340 != 0xff ) ) {
V_77 -> V_82 . V_347 = true ;
}
}
}
if( V_77 -> V_123 & V_601 ) {
if( ( V_77 -> V_82 . V_595 != - 1 ) &&
( V_77 -> V_82 . V_595 != V_77 -> V_336 ) ) {
F_4 ( V_72 L_82 ,
V_77 -> V_82 . V_595 ) ;
}
if( ( V_77 -> V_82 . V_599 != - 1 ) &&
( V_77 -> V_82 . V_599 != V_77 -> V_338 ) ) {
F_4 ( V_72 L_83 ,
V_77 -> V_82 . V_599 ) ;
}
}
}
#endif
}
static T_2 F_123 ( struct V_76 * V_77 )
{
T_2 V_164 = V_77 -> V_2 * 1024 ;
T_2 V_602 = V_77 -> V_320 - V_77 -> V_603 - V_77 -> V_328 ;
T_2 V_604 ;
if( V_77 -> V_122 == V_134 ) {
if( V_77 -> V_320 > 0x1000000 ) {
V_604 = 0xc00000 ;
} else if( V_77 -> V_320 > 0x800000 ) {
V_604 = 0x800000 ;
} else {
V_604 = 0x400000 ;
}
} else if( V_77 -> V_440 && V_77 -> V_441 ) {
V_164 = V_604 = 0 ;
} else {
V_604 = V_602 - 0x8000 ;
}
if( ( ! V_164 ) || ( V_164 > V_602 ) || ( V_77 -> V_605 != 0 ) )
V_164 = V_604 ;
return V_164 ;
}
static T_2 F_124 ( struct V_76 * V_77 )
{
T_2 V_606 = V_77 -> V_320 - V_77 -> V_603 - V_77 -> V_328 ;
T_2 V_164 = 0 ;
if( V_77 -> V_440 && V_77 -> V_441 ) {
if( ( ! V_77 -> V_2 ) ||
( ( V_77 -> V_2 * 1024 ) > V_606 ) ||
( ( V_606 - ( V_77 -> V_2 * 1024 ) ) < V_77 -> V_440 ) ) {
V_164 = V_77 -> V_440 ;
V_606 -= V_77 -> V_440 ;
} else {
V_164 = V_606 - ( V_77 -> V_2 * 1024 ) ;
V_606 = V_77 -> V_2 * 1024 ;
}
V_77 -> V_270 = V_164 ;
V_77 -> V_209 = V_606 ;
} else {
V_164 = V_606 - V_77 -> V_321 ;
V_77 -> V_209 = V_77 -> V_321 ;
}
return V_164 ;
}
static int F_125 ( struct V_76 * V_77 )
{
struct V_607 * V_608 ;
V_77 -> V_270 = 0 ;
if( V_77 -> V_2 ) {
if( ( V_77 -> V_2 < ( 2 * 1024 * 1024 ) ) ||
( V_77 -> V_2 > V_77 -> V_320 ) ) {
V_77 -> V_2 = 0 ;
}
}
V_77 -> V_321 = F_123 ( V_77 ) ;
V_77 -> V_361 = F_124 ( V_77 ) ;
V_77 -> V_609 = V_77 -> V_569 + V_77 -> V_321 ;
V_77 -> V_610 = V_77 -> V_609 + V_77 -> V_361 ;
F_4 ( V_72 L_84 ,
( int ) ( V_77 -> V_321 / 1024 ) , ( int ) ( V_77 -> V_361 / 1024 ) ) ;
V_77 -> V_611 . V_612 = V_77 ;
V_77 -> V_611 . V_613 = NULL ;
V_77 -> V_611 . V_614 = NULL ;
V_608 = F_126 ( & V_77 -> V_611 ) ;
if( V_608 == NULL )
return 1 ;
V_608 -> V_615 = & V_77 -> V_611 . V_616 ;
V_608 -> V_617 = & V_77 -> V_611 . V_616 ;
V_608 -> V_618 = V_77 -> V_361 ;
V_608 -> V_223 = V_77 -> V_321 ;
V_77 -> V_611 . V_616 . V_615 = V_608 ;
V_77 -> V_611 . V_616 . V_617 = V_608 ;
V_77 -> V_611 . V_616 . V_618 = 0 ;
V_77 -> V_611 . V_619 = V_608 -> V_618 ;
V_77 -> V_611 . V_620 . V_615 = & V_77 -> V_611 . V_620 ;
V_77 -> V_611 . V_620 . V_617 = & V_77 -> V_611 . V_620 ;
V_77 -> V_611 . V_620 . V_618 = V_621 ;
if( V_77 -> V_605 == 0 ) {
V_611 = & V_77 -> V_611 ;
}
return 0 ;
}
static struct V_607 *
F_126 ( struct V_622 * V_623 )
{
struct V_624 * V_625 ;
struct V_607 * V_608 ;
unsigned long V_626 ;
int V_33 ;
if( V_623 -> V_614 == NULL ) {
V_625 = F_127 ( V_627 , V_628 ) ;
if( ! V_625 )
return NULL ;
V_625 -> V_629 = V_623 -> V_613 ;
V_623 -> V_613 = V_625 ;
V_626 = ( V_627 - sizeof( struct V_624 ) ) / sizeof( struct V_607 ) + 1 ;
V_608 = & V_625 -> V_630 [ 0 ] ;
for( V_33 = V_626 - 1 ; V_33 != 0 ; V_33 -- ) {
V_608 -> V_615 = V_608 + 1 ;
V_608 = V_608 + 1 ;
}
V_608 -> V_615 = NULL ;
V_623 -> V_614 = & V_625 -> V_630 [ 0 ] ;
}
V_608 = V_623 -> V_614 ;
V_623 -> V_614 = V_608 -> V_615 ;
return V_608 ;
}
static struct V_607 *
F_128 ( struct V_622 * V_623 , T_2 V_618 )
{
struct V_607 * V_631 ;
struct V_607 * V_632 ;
int V_633 = 0 ;
if( V_618 > V_623 -> V_619 ) {
F_25 ( L_85 ,
( unsigned int ) V_618 / 1024 ) ;
return NULL ;
}
V_631 = V_623 -> V_616 . V_615 ;
while( V_631 != & V_623 -> V_616 ) {
if( V_618 <= V_631 -> V_618 ) {
V_633 = 1 ;
break;
}
V_631 = V_631 -> V_615 ;
}
if( ! V_633 ) {
F_25 ( L_85 ,
( unsigned int ) V_618 / 1024 ) ;
return NULL ;
}
if( V_618 == V_631 -> V_618 ) {
V_632 = V_631 ;
F_129 ( V_631 ) ;
} else {
V_632 = F_126 ( V_623 ) ;
if( V_632 == NULL )
return NULL ;
V_632 -> V_223 = V_631 -> V_223 ;
V_632 -> V_618 = V_618 ;
V_631 -> V_223 += V_618 ;
V_631 -> V_618 -= V_618 ;
}
V_623 -> V_619 -= V_618 ;
V_631 = & V_623 -> V_620 ;
F_130 ( V_631 , V_632 ) ;
return V_632 ;
}
static void
F_129 ( struct V_607 * V_608 )
{
V_608 -> V_617 -> V_615 = V_608 -> V_615 ;
V_608 -> V_615 -> V_617 = V_608 -> V_617 ;
}
static void
F_130 ( struct V_607 * V_634 , struct V_607 * V_608 )
{
struct V_607 * V_635 = V_634 -> V_615 ;
V_634 -> V_615 = V_608 ;
V_635 -> V_617 = V_608 ;
V_608 -> V_617 = V_634 ;
V_608 -> V_615 = V_635 ;
}
static struct V_607 *
F_131 ( struct V_622 * V_623 , T_2 V_266 )
{
struct V_607 * V_631 ;
struct V_607 * V_636 ;
struct V_607 * V_617 ;
struct V_607 * V_615 ;
T_2 V_637 ;
T_2 V_638 ;
int V_639 = 0 ;
V_636 = V_623 -> V_620 . V_615 ;
while( V_636 != & V_623 -> V_620 ) {
if( V_636 -> V_223 == V_266 ) {
V_639 = 1 ;
break;
}
V_636 = V_636 -> V_615 ;
}
if( ! V_639 )
return NULL ;
V_623 -> V_619 += V_636 -> V_618 ;
V_617 = V_615 = NULL ;
V_637 = V_636 -> V_223 + V_636 -> V_618 ;
V_638 = V_636 -> V_223 ;
V_631 = V_623 -> V_616 . V_615 ;
while( V_631 != & V_623 -> V_616 ) {
if( V_631 -> V_223 == V_637 ) {
V_615 = V_631 ;
} else if( ( V_631 -> V_223 + V_631 -> V_618 ) == V_638 ) {
V_617 = V_631 ;
}
V_631 = V_631 -> V_615 ;
}
F_129 ( V_636 ) ;
if( V_617 && V_615 ) {
V_617 -> V_618 += ( V_636 -> V_618 + V_615 -> V_618 ) ;
F_129 ( V_615 ) ;
F_132 ( V_623 , V_636 ) ;
F_132 ( V_623 , V_615 ) ;
return V_617 ;
}
if( V_617 ) {
V_617 -> V_618 += V_636 -> V_618 ;
F_132 ( V_623 , V_636 ) ;
return V_617 ;
}
if( V_615 ) {
V_615 -> V_618 += V_636 -> V_618 ;
V_615 -> V_223 = V_636 -> V_223 ;
F_132 ( V_623 , V_636 ) ;
return V_615 ;
}
F_130 ( & V_623 -> V_616 , V_636 ) ;
return V_636 ;
}
static void
F_132 ( struct V_622 * V_623 , struct V_607 * V_608 )
{
if( V_608 == NULL )
return;
V_608 -> V_615 = V_623 -> V_614 ;
V_623 -> V_614 = V_608 ;
}
static void
F_133 ( struct V_76 * V_77 , struct V_289 * V_640 )
{
struct V_607 * V_608 = NULL ;
if( ( V_77 ) && ( V_77 -> V_308 == V_309 ) && ( ! V_77 -> V_641 ) )
V_608 = F_128 ( & V_77 -> V_611 , ( T_2 ) V_640 -> V_618 ) ;
if( V_608 == NULL ) {
V_640 -> V_223 = V_640 -> V_618 = 0 ;
F_25 ( L_86 ) ;
} else {
V_640 -> V_223 = V_608 -> V_223 ;
V_640 -> V_618 = V_608 -> V_618 ;
F_25 ( L_87 ,
( V_608 -> V_223 + V_77 -> V_569 ) ) ;
}
}
void
F_83 ( struct V_289 * V_640 )
{
struct V_76 * V_77 = V_611 -> V_612 ;
if( & V_77 -> V_611 == V_611 )
F_133 ( V_77 , V_640 ) ;
else
V_640 -> V_223 = V_640 -> V_618 = 0 ;
}
void
F_134 ( struct V_416 * V_418 , struct V_289 * V_640 )
{
struct V_76 * V_77 = F_135 ( V_418 ) ;
F_133 ( V_77 , V_640 ) ;
}
static void
F_136 ( struct V_76 * V_77 , T_2 V_266 )
{
struct V_607 * V_608 ;
if( ( ! V_77 ) || ( V_77 -> V_308 != V_309 ) || ( V_77 -> V_641 ) )
return;
V_608 = F_131 ( & V_77 -> V_611 , V_266 ) ;
if( V_608 == NULL ) {
F_25 ( L_88 ,
( unsigned int ) V_266 ) ;
}
}
void
F_85 ( T_2 V_266 )
{
struct V_76 * V_77 = V_611 -> V_612 ;
F_136 ( V_77 , V_266 ) ;
}
void
F_137 ( struct V_416 * V_418 , T_2 V_266 )
{
struct V_76 * V_77 = F_135 ( V_418 ) ;
F_136 ( V_77 , V_266 ) ;
}
static void
F_138 ( struct V_76 * V_77 )
{
T_3 V_642 , V_643 ;
V_642 = F_27 ( V_153 , V_644 ) ;
V_643 = F_27 ( V_153 , V_645 ) ;
if( ( V_642 & V_646 ) && ( V_643 & 0x42 ) ) {
#ifdef F_20
if( V_77 -> V_122 == V_134 ) {
F_120 ( V_77 ) ;
}
#endif
#ifdef F_17
if( V_77 -> V_122 == V_131 ) {
V_642 = F_27 ( V_153 , 0x26 ) ;
if( ( V_642 & 0xe0 ) && ( ! ( V_642 & 0x01 ) ) ) {
F_120 ( V_77 ) ;
}
}
#endif
}
}
static void
F_59 ( struct V_76 * V_77 )
{
T_3 V_642 = 0 , V_643 = 0 , V_647 = 0 , V_648 = 0 , V_649 = 0 ;
int V_650 = 0 ;
V_77 -> V_157 &= ( V_651 | V_138 ) ;
F_55 ( V_153 , 0x05 , 0x86 ) ;
V_643 = F_27 ( V_152 , 0x31 ) ;
V_643 &= ~ 0x60 ;
V_643 |= 0x04 ;
V_647 = V_77 -> V_125 & 0x0F ;
#ifdef F_17
if( V_77 -> V_122 == V_131 ) {
if( V_77 -> V_85 >= V_448 ) {
V_649 = F_27 ( V_152 , 0x38 ) ;
V_649 &= ~ 0x07 ;
} else {
V_650 = 0x38 ;
V_649 = F_27 ( V_152 , V_650 ) ;
V_649 &= ~ 0x3b ;
}
}
#endif
#ifdef F_20
if( V_77 -> V_122 == V_134 ) {
V_650 = 0x35 ;
V_649 = F_27 ( V_152 , V_650 ) ;
}
#endif
F_139 ( & V_77 -> V_82 , false ) ;
F_140 ( & V_77 -> V_82 , false ) ;
V_77 -> V_364 = V_77 -> V_366 = 0 ;
switch( V_77 -> V_157 & V_138 ) {
case V_145 :
V_649 &= ~ 0xc0 ;
if( ( V_77 -> V_119 & V_462 ) && ( V_77 -> V_123 & V_461 ) ) {
#ifdef F_17
if( V_77 -> V_85 >= V_448 ) {
V_649 |= 0x04 ;
if( V_77 -> V_119 & V_652 ) V_648 |= 0x20 ;
else if( V_77 -> V_119 & V_653 ) V_648 |= 0x40 ;
else if( V_77 -> V_119 & V_654 ) V_648 |= 0x60 ;
V_642 |= V_655 ;
V_648 &= ~ 0x01 ;
V_77 -> V_157 |= ( V_462 | ( V_77 -> V_119 & V_656 ) ) ;
} else if( V_77 -> V_122 == V_131 ) {
V_642 |= ( 0x80 | V_655 ) ;
V_649 |= 0x08 ;
if( V_77 -> V_119 & V_652 ) V_649 |= 0x10 ;
else if( V_77 -> V_119 & V_653 ) V_649 |= 0x20 ;
else if( V_77 -> V_119 & V_654 ) V_649 |= 0x30 ;
V_643 &= ~ 0x01 ;
V_77 -> V_157 |= ( V_462 | ( V_77 -> V_119 & V_656 ) ) ;
}
#endif
} else if( ( V_77 -> V_119 & V_464 ) &&
( V_77 -> V_123 & V_463 ) ) {
if( V_77 -> V_85 >= V_448 ) {
V_649 |= 0x04 ;
V_648 |= 0x60 ;
} else {
V_642 |= 0x80 ;
}
V_642 |= V_655 ;
V_643 |= 0x01 ;
V_648 |= 0x01 ;
V_77 -> V_157 |= V_464 ;
} else if( V_77 -> V_119 & V_471 ) {
V_642 = ( V_657 | V_655 ) ;
V_643 |= 0x01 ;
V_648 |= 0x01 ;
V_77 -> V_157 |= V_471 ;
} else {
if( V_77 -> V_119 & V_473 ) {
V_642 = ( V_658 | V_655 ) ;
V_77 -> V_157 |= V_473 ;
}
if( V_77 -> V_119 & V_475 ) {
V_642 = ( V_659 | V_655 ) ;
V_77 -> V_157 |= V_475 ;
}
}
V_643 |= V_660 ;
if( V_77 -> V_119 & ( V_475 | V_473 ) ) {
if( V_77 -> V_119 & V_453 ) {
V_643 |= 0x01 ; V_648 |= 0x01 ;
V_77 -> V_157 |= V_453 ;
if( V_77 -> V_119 & V_455 ) {
V_649 |= 0x40 ; V_648 |= 0x04 ;
V_77 -> V_157 |= V_455 ;
} else if( V_77 -> V_119 & V_456 ) {
V_649 |= 0x80 ; V_648 |= 0x08 ;
V_77 -> V_157 |= V_456 ;
}
} else {
V_643 &= ~ 0x01 ; V_648 &= ~ 0x01 ;
V_77 -> V_157 |= V_454 ;
if( V_77 -> V_119 & V_466 ) {
V_649 |= 0x40 ; V_648 |= 0x02 ;
V_77 -> V_157 |= V_466 ;
}
}
}
break;
case V_120 :
V_642 = ( V_661 | V_655 ) ;
V_643 |= V_660 ;
F_139 ( & V_77 -> V_82 , V_77 -> V_20 ) ;
F_140 ( & V_77 -> V_82 , V_77 -> V_21 ) ;
V_77 -> V_364 = V_77 -> V_21 ;
V_77 -> V_366 = V_77 -> V_20 ;
break;
case V_121 :
V_642 = ( V_662 | V_655 ) ;
V_643 |= V_660 ;
if( V_77 -> V_26 ) {
V_647 |= ( V_37 [ V_77 -> V_8 ] . V_125 << 4 ) ;
} else {
V_647 |= ( ( V_77 -> V_125 & 0x0F ) << 4 ) ;
}
break;
default:
V_642 = 0x00 ;
V_643 |= ( V_660 | V_663 ) ;
}
F_55 ( V_152 , 0x30 , V_642 ) ;
F_55 ( V_152 , 0x33 , V_647 ) ;
if( V_77 -> V_85 >= V_448 ) {
#ifdef F_17
V_643 &= ~ 0x01 ;
F_36 ( V_152 , 0x35 , ~ 0x10 , V_648 ) ;
V_649 &= 0x07 ;
F_36 ( V_152 , 0x38 , 0xf8 , V_649 ) ;
#endif
} else if( V_77 -> V_85 != V_442 ) {
F_55 ( V_152 , V_650 , V_649 ) ;
}
F_55 ( V_152 , 0x31 , V_643 ) ;
V_77 -> V_82 . V_664 = V_77 -> V_7 ;
F_138 ( V_77 ) ;
}
static void
F_141 ( struct V_76 * V_77 )
{
T_3 V_665 ;
if( V_77 -> V_85 >= V_448 ) {
V_665 = F_27 ( V_153 , 0x11 ) ;
if( V_665 & 0x20 ) {
V_665 = F_27 ( V_153 , 0x3e ) ;
V_665 = ( V_665 + 1 ) & 0xff ;
F_55 ( V_153 , 0x3e , V_665 ) ;
V_665 = F_27 ( V_153 , 0x11 ) ;
}
if( V_665 & 0xf0 ) {
F_103 ( V_153 , 0x11 , 0x0f ) ;
}
}
}
static void
F_88 ( struct V_76 * V_77 , int V_197 )
{
if( V_197 > 32 ) V_197 = 32 ;
if( V_197 < - 32 ) V_197 = - 32 ;
V_77 -> V_357 = V_197 ;
if( V_77 -> V_383 ) return;
if( ! V_77 -> V_322 ) return;
if( V_77 -> V_157 & V_145 ) {
if( V_77 -> V_123 & V_190 ) {
int V_114 = V_77 -> V_666 ;
switch( V_77 -> V_539 ) {
case 1 :
V_114 += V_197 ;
if( V_114 < 0 ) V_114 = 0 ;
F_55 ( V_153 , 0x05 , 0x86 ) ;
F_111 ( & V_77 -> V_82 , 0x0a , ( V_114 & 0xff ) ) ;
F_112 ( & V_77 -> V_82 , 0x08 , ( ( V_114 & 0x0100 ) >> 7 ) , 0xFD ) ;
break;
case 2 :
break;
}
} else if( V_77 -> V_123 & V_465 ) {
T_3 V_667 , V_668 , V_669 , V_670 , V_671 ;
unsigned short V_117 ;
V_667 = V_77 -> V_667 ;
V_668 = V_77 -> V_668 ;
V_669 = V_77 -> V_669 ;
V_670 = V_77 -> V_670 ;
V_671 = V_77 -> V_671 ;
V_117 = V_667 | ( ( V_668 & 0xf0 ) << 4 ) ;
V_117 += ( V_197 * 2 ) ;
V_667 = V_117 & 0xff ;
V_668 = ( V_117 & 0xf00 ) >> 4 ;
V_669 = ( ( V_669 & 0x0f ) + ( V_197 * 2 ) ) & 0x0f ;
V_117 = V_671 | ( ( V_670 & 0xf0 ) << 4 ) ;
V_117 += ( V_197 * 2 ) ;
V_671 = V_117 & 0xff ;
V_670 = ( V_117 & 0xf00 ) >> 4 ;
F_55 ( V_195 , 0x1f , V_667 ) ;
F_36 ( V_195 , 0x20 , 0x0F , V_668 ) ;
F_36 ( V_195 , 0x2b , 0xF0 , V_669 ) ;
F_36 ( V_195 , 0x42 , 0x0F , V_670 ) ;
F_55 ( V_195 , 0x43 , V_671 ) ;
}
}
}
static void
F_89 ( struct V_76 * V_77 , int V_197 )
{
if( V_197 > 32 ) V_197 = 32 ;
if( V_197 < - 32 ) V_197 = - 32 ;
V_77 -> V_359 = V_197 ;
if( V_77 -> V_383 ) return;
if( ! V_77 -> V_322 ) return;
if( V_77 -> V_157 & V_145 ) {
if( V_77 -> V_123 & V_190 ) {
int V_115 = V_77 -> V_672 ;
switch( V_77 -> V_539 ) {
case 1 :
V_115 -= V_197 ;
if( V_115 < 0 ) V_115 = 0 ;
F_55 ( V_153 , 0x05 , 0x86 ) ;
F_111 ( & V_77 -> V_82 , 0x0b , ( V_115 & 0xff ) ) ;
F_112 ( & V_77 -> V_82 , 0x08 , ( ( V_115 & 0x0100 ) >> 8 ) , 0xFE ) ;
break;
case 2 :
break;
}
} else if( V_77 -> V_123 & V_465 ) {
char V_673 , V_674 ;
V_197 /= 2 ;
V_673 = V_77 -> V_673 ;
V_674 = V_77 -> V_674 ;
V_673 += V_197 ;
V_674 += V_197 ;
if( ! ( V_77 -> V_157 & ( V_464 | V_462 ) ) ) {
while( ( V_673 <= 0 ) || ( V_674 <= 0 ) ) {
V_673 += 2 ;
V_674 += 2 ;
}
}
F_55 ( V_195 , 0x01 , V_673 ) ;
F_55 ( V_195 , 0x02 , V_674 ) ;
}
}
}
static void
F_61 ( struct V_76 * V_77 )
{
bool V_675 = false ;
bool V_676 = true ;
#if F_2 ( F_20 ) || F_2 ( F_17 )
T_3 V_156 ;
#endif
#ifdef F_17
T_3 V_160 ;
#endif
F_55 ( V_153 , 0x05 , 0x86 ) ;
#ifdef F_17
F_141 ( V_77 ) ;
#endif
V_77 -> V_322 = 1 ;
if( V_77 -> V_123 & V_150 ) {
if( F_26 ( V_77 ) ) V_676 = false ;
} else
V_77 -> V_10 = 0 ;
#ifdef F_20
if( V_77 -> V_122 == V_134 ) {
if( ( V_77 -> V_10 ) && ( V_676 ) ) {
V_675 = true ;
V_156 = 0x00 ;
} else {
V_675 = false ;
V_156 = 0x80 ;
}
F_36 ( V_152 , 0x17 , 0x7f , V_156 ) ;
}
#endif
#ifdef F_17
if( V_77 -> V_122 == V_131 ) {
if( ( V_77 -> V_10 ) && ( V_676 ) ) {
V_675 = true ;
V_156 = 0x40 ;
V_160 = 0xc0 ;
} else {
V_675 = false ;
V_156 = 0x00 ;
V_160 = 0x00 ;
}
F_36 ( V_152 , V_77 -> V_82 . V_188 , ~ 0x40 , V_156 ) ;
F_36 ( V_153 , 0x1f , 0x3f , V_160 ) ;
}
#endif
if( V_675 ) {
V_77 -> V_157 &= ~ V_186 ;
V_77 -> V_157 |= V_677 ;
} else {
V_77 -> V_157 |= V_186 ;
if( V_77 -> V_157 & V_678 ) {
V_77 -> V_157 |= V_679 ;
} else {
V_77 -> V_157 |= V_677 ;
}
}
F_103 ( V_153 , V_680 , ~ 0x04 ) ;
if( V_77 -> V_157 & V_145 ) {
if( V_77 -> V_123 & V_465 ) {
V_77 -> V_667 = F_27 ( V_195 , 0x1f ) ;
V_77 -> V_668 = F_27 ( V_195 , 0x20 ) ;
V_77 -> V_669 = F_27 ( V_195 , 0x2b ) ;
V_77 -> V_670 = F_27 ( V_195 , 0x42 ) ;
V_77 -> V_671 = F_27 ( V_195 , 0x43 ) ;
V_77 -> V_673 = F_27 ( V_195 , 0x01 ) ;
V_77 -> V_674 = F_27 ( V_195 , 0x02 ) ;
} else if( V_77 -> V_123 & V_190 ) {
if( V_77 -> V_539 == 1 ) {
V_77 -> V_666 = F_110 ( & V_77 -> V_82 , 0x0a ) ;
V_77 -> V_666 |= ( ( ( F_110 ( & V_77 -> V_82 , 0x08 ) & 0x02 ) >> 1 ) << 8 ) ;
V_77 -> V_672 = F_110 ( & V_77 -> V_82 , 0x0b ) ;
V_77 -> V_672 |= ( ( F_110 ( & V_77 -> V_82 , 0x08 ) & 0x01 ) << 8 ) ;
}
}
}
if( V_77 -> V_357 ) {
F_88 ( V_77 , V_77 -> V_357 ) ;
}
if( V_77 -> V_359 ) {
F_89 ( V_77 , V_77 -> V_359 ) ;
}
F_138 ( V_77 ) ;
if( V_77 -> V_207 ) {
F_116 ( V_77 ) ;
} else {
V_77 -> V_575 = 0 ;
}
}
static int
F_142 ( struct V_76 * V_77 )
{
if( F_58 ( V_77 , 0 ) )
return 1 ;
F_54 ( V_77 ) ;
F_64 ( V_77 , V_77 -> V_267 ) ;
F_65 ( V_77 , V_77 -> V_267 ) ;
return 0 ;
}
static void
F_90 ( struct V_76 * V_77 , struct V_381 * V_380 )
{
int V_681 ;
switch( V_380 -> V_381 ) {
case V_682 :
if( ! V_77 -> V_322 ) {
V_380 -> V_683 [ 0 ] = V_684 ;
} else {
V_380 -> V_683 [ 0 ] = V_685 ;
V_380 -> V_683 [ 1 ] = V_77 -> V_157 ;
V_380 -> V_683 [ 2 ] = V_77 -> V_123 ;
}
break;
case V_686 :
if( ! V_77 -> V_322 ) {
V_380 -> V_683 [ 0 ] = V_684 ;
} else if( V_380 -> V_687 [ 0 ] == 99 ) {
V_380 -> V_683 [ 1 ] = V_77 -> V_10 ? 0 : 1 ;
V_380 -> V_683 [ 0 ] = V_685 ;
} else if( V_77 -> V_383 ) {
V_380 -> V_683 [ 0 ] = V_688 ;
} else if( ( ! ( V_77 -> V_157 & V_689 ) ) &&
( V_380 -> V_687 [ 0 ] == 0 ) ) {
V_380 -> V_683 [ 0 ] = V_690 ;
} else {
V_380 -> V_683 [ 0 ] = V_685 ;
V_681 = V_380 -> V_687 [ 0 ] ? 0 : 1 ;
if( ( ( V_77 -> V_157 & V_186 ) && V_681 ) ||
( ( ! ( V_77 -> V_157 & V_186 ) ) && ! V_681 ) ) {
V_77 -> V_10 = V_681 ;
if( F_142 ( V_77 ) ) {
V_380 -> V_683 [ 0 ] = V_691 ;
}
}
V_380 -> V_683 [ 1 ] = V_77 -> V_10 ? 0 : 1 ;
}
break;
default:
V_380 -> V_683 [ 0 ] = V_692 ;
F_4 ( V_35 L_89 ,
V_380 -> V_381 ) ;
}
}
static int T_1 F_143 ( char * V_693 )
{
char * V_694 ;
F_1 () ;
if( ! V_693 || ! ( * V_693 ) )
return 0 ;
while( ( V_694 = F_144 ( & V_693 , L_90 ) ) != NULL ) {
if( ! ( * V_694 ) ) continue;
if( ! strncasecmp ( V_694 , L_91 , 3 ) ) {
V_1 = 1 ;
} else if( ! strncasecmp ( V_694 , L_92 , 14 ) ) {
F_9 ( V_694 + 14 ) ;
} else if( ! strncasecmp ( V_694 , L_93 , 7 ) ) {
F_10 ( V_694 + 7 ) ;
} else if( ! strncasecmp ( V_694 , L_94 , 11 ) ) {
F_10 ( V_694 + 11 ) ;
} else if( ! strncasecmp ( V_694 , L_95 , 5 ) ) {
F_5 ( V_694 + 5 , false ) ;
} else if( ! strncasecmp ( V_694 , L_96 , 5 ) ) {
F_3 ( F_6 ( V_694 + 5 , NULL , 0 ) , false ) ;
} else if( ! strncasecmp ( V_694 , L_97 , 5 ) ) {
V_9 = F_6 ( V_694 + 5 , NULL , 0 ) ;
} else if( ! strncasecmp ( V_694 , L_98 , 10 ) ) {
V_11 = ( int ) F_6 ( V_694 + 10 , NULL , 0 ) ;
} else if( ! strncasecmp ( V_694 , L_99 , 4 ) ) {
V_2 = F_6 ( V_694 + 4 , NULL , 0 ) ;
} else if( ! strncasecmp ( V_694 , L_100 , 4 ) ) {
V_14 = F_6 ( V_694 + 4 , NULL , 0 ) ;
} else if( ! strncasecmp ( V_694 , L_101 , 5 ) ) {
V_15 = F_6 ( V_694 + 5 , NULL , 0 ) ;
} else if( ! strncasecmp ( V_694 , L_102 , 7 ) ) {
V_3 = 0 ;
} else if( ! strncasecmp ( V_694 , L_103 , 5 ) ) {
V_3 = - 1 ;
} else if( ! strncasecmp ( V_694 , L_104 , 6 ) ) {
V_4 = 0 ;
} else if( ! strncasecmp ( V_694 , L_105 , 4 ) ) {
V_4 = - 1 ;
} else if( ! strncasecmp ( V_694 , L_106 , 5 ) ) {
V_5 = 0 ;
} else if( ! strncasecmp ( V_694 , L_107 , 3 ) ) {
V_5 = - 1 ;
} else if( ! strncasecmp ( V_694 , L_108 , 7 ) ) {
V_6 = ( int ) F_6 ( V_694 + 7 , NULL , 0 ) ;
} else if( ! strncasecmp ( V_694 , L_109 , 7 ) ) {
V_7 = ( int ) F_6 ( V_694 + 7 , NULL , 0 ) ;
} else if( ! strncasecmp ( V_694 , L_110 , 10 ) ) {
V_26 = 1 ;
} else if( ! strncasecmp ( V_694 , L_111 , 9 ) ) {
unsigned long V_117 = 2 ;
V_117 = F_6 ( V_694 + 9 , NULL , 0 ) ;
if( ( V_117 == 0 ) || ( V_117 == 1 ) ) {
V_16 = V_117 ^ 1 ;
}
} else if( ! strncasecmp ( V_694 , L_112 , 13 ) ) {
int V_117 = 0 ;
V_117 = ( int ) F_145 ( V_694 + 13 , NULL , 0 ) ;
if( ( V_117 >= - 32 ) && ( V_117 <= 32 ) ) {
V_24 = V_117 ;
}
} else if( ! strncasecmp ( V_694 , L_113 , 13 ) ) {
int V_117 = 0 ;
V_117 = ( int ) F_145 ( V_694 + 13 , NULL , 0 ) ;
if( ( V_117 >= - 32 ) && ( V_117 <= 32 ) ) {
V_25 = V_117 ;
}
} else if( ! strncasecmp ( V_694 , L_114 , 14 ) ) {
F_11 ( V_694 + 14 ) ;
} else if( ! strncasecmp ( V_694 , L_115 , 7 ) ) {
int V_117 = 4 ;
V_117 = F_6 ( V_694 + 7 , NULL , 0 ) ;
if( ( V_117 >= 0 ) && ( V_117 <= 3 ) ) {
V_19 = V_117 ;
}
} else if( V_694 [ 0 ] >= '0' && V_694 [ 0 ] <= '9' ) {
F_5 ( V_694 , true ) ;
#if ! F_2 ( V_27 ) && ! F_2 ( V_28 )
} else if( ! strncasecmp ( V_694 , L_116 , 9 ) ) {
V_29 = 1 ;
} else if( ! strncasecmp ( V_694 , L_117 , 9 ) ) {
V_30 = F_6 ( V_694 + 9 , NULL , 0 ) ;
#endif
} else {
F_4 ( V_72 L_118 , V_694 ) ;
}
}
return 0 ;
}
static int F_146 ( void T_5 * V_695 ,
struct V_76 * V_77 )
{
void T_5 * V_696 ;
int V_697 ;
if( ( F_147 ( V_695 ) != 0x55 ) || ( F_147 ( V_695 + 1 ) != 0xaa ) )
return 0 ;
V_697 = ( F_147 ( V_695 + 0x18 ) | ( F_147 ( V_695 + 0x19 ) << 8 ) ) ;
if( V_697 > ( 0x10000 - 8 ) )
return 0 ;
V_696 = V_695 + V_697 ;
if( ( F_147 ( V_696 ) != 'P' ) || ( F_147 ( V_696 + 1 ) != 'C' ) ||
( F_147 ( V_696 + 2 ) != 'I' ) || ( F_147 ( V_696 + 3 ) != 'R' ) )
return 0 ;
if( ( F_147 ( V_696 + 4 ) | ( F_147 ( V_696 + 5 ) << 8 ) ) != V_77 -> V_318 )
return 0 ;
if( ( F_147 ( V_696 + 6 ) | ( F_147 ( V_696 + 7 ) << 8 ) ) != V_77 -> V_316 )
return 0 ;
return 1 ;
}
static unsigned char * F_148 ( struct V_416 * V_418 )
{
struct V_76 * V_77 = F_135 ( V_418 ) ;
void T_5 * V_695 ;
unsigned char * V_698 = NULL ;
T_6 V_699 ;
if( ! V_77 -> V_198 ) {
if( ( V_695 = F_149 ( V_418 , & V_699 ) ) ) {
if( F_146 ( V_695 , V_77 ) ) {
if( ( V_698 = F_150 ( 65536 ) ) ) {
F_151 ( V_698 , V_695 ,
( V_699 > 65536 ) ? 65536 : V_699 ) ;
}
}
F_152 ( V_418 , V_695 ) ;
}
}
if( V_698 ) return V_698 ;
#if F_2 ( V_27 ) || F_2 ( V_28 )
{
T_2 V_117 ;
for ( V_117 = 0x000c0000 ; V_117 < 0x000f0000 ; V_117 += 0x00001000 ) {
V_695 = F_153 ( V_117 , 65536 ) ;
if ( ! V_695 )
continue;
if ( ! F_146 ( V_695 , V_77 ) ) {
F_154 ( V_695 ) ;
continue;
}
if ( ( V_698 = F_150 ( 65536 ) ) )
F_151 ( V_698 , V_695 , 65536 ) ;
F_154 ( V_695 ) ;
break;
}
}
#endif
return V_698 ;
}
static void F_155 ( struct V_76 * V_77 ,
unsigned int * V_700 , unsigned int V_701 )
{
if ( * V_700 < ( V_701 << 20 ) )
return;
V_77 -> V_569 = F_156 ( V_77 -> V_391 , ( * V_700 ) ) ;
if( ! V_77 -> V_569 ) {
F_4 ( V_35
L_119 ) ;
( * V_700 ) >>= 1 ;
while( ( ! ( V_77 -> V_569 = F_156 ( V_77 -> V_391 , ( * V_700 ) ) ) ) ) {
( * V_700 ) >>= 1 ;
if( ( * V_700 ) < ( V_701 << 20 ) )
break;
}
if( V_77 -> V_569 ) {
F_4 ( V_35
L_120 ,
( int ) ( ( * V_700 ) >> 20 ) ) ;
}
}
}
static int F_157 ( struct V_76 * V_77 )
{
void T_5 * V_702 = V_77 -> V_569 ;
unsigned short V_117 ;
unsigned char V_156 ;
int V_33 , V_34 ;
F_103 ( V_153 , 0x15 , 0xFB ) ;
F_56 ( V_153 , 0x15 , 0x04 ) ;
F_55 ( V_153 , 0x13 , 0x00 ) ;
F_55 ( V_153 , 0x14 , 0xBF ) ;
for( V_33 = 0 ; V_33 < 2 ; V_33 ++ ) {
V_117 = 0x1234 ;
for( V_34 = 0 ; V_34 < 4 ; V_34 ++ ) {
F_158 ( V_117 , V_702 ) ;
if( F_159 ( V_702 ) == V_117 )
break;
F_56 ( V_153 , 0x3c , 0x01 ) ;
V_156 = F_27 ( V_153 , 0x05 ) ;
V_156 = F_27 ( V_153 , 0x05 ) ;
F_103 ( V_153 , 0x3c , 0xfe ) ;
V_156 = F_27 ( V_153 , 0x05 ) ;
V_156 = F_27 ( V_153 , 0x05 ) ;
V_117 ++ ;
}
}
F_119 ( 0x01234567L , V_702 ) ;
F_119 ( 0x456789ABL , ( V_702 + 4 ) ) ;
F_119 ( 0x89ABCDEFL , ( V_702 + 8 ) ) ;
F_119 ( 0xCDEF0123L , ( V_702 + 12 ) ) ;
V_156 = F_27 ( V_153 , 0x3b ) ;
if( V_156 & 0x01 ) {
if( F_160 ( ( V_702 + 12 ) ) == 0xCDEF0123L )
return 4 ;
}
if( F_160 ( ( V_702 + 4 ) ) == 0x456789ABL )
return 2 ;
return 1 ;
}
static int F_161 ( struct V_76 * V_77 , int V_703 ,
int V_704 , int V_705 ,
int V_706 , unsigned int V_700 )
{
void T_5 * V_707 = V_77 -> V_569 ;
unsigned short V_708 ;
unsigned int V_709 , V_710 , V_711 , V_712 , V_713 ;
unsigned int V_714 , V_715 , V_716 ;
for( V_709 = 0 ; V_709 < F_162 ( V_717 ) ; V_709 ++ ) {
V_710 = V_704 * V_717 [ V_709 ] [ 3 ] ;
if( V_710 != V_705 )
continue;
if( ( V_717 [ V_709 ] [ 2 ] + V_717 [ V_709 ] [ 0 ] ) > V_706 )
continue;
V_712 = V_710 * 16 * V_703 - 1 ;
if( V_703 == 3 ) {
V_713 = V_710 * 16 - 1 ;
} else {
V_713 = V_710 * 16 * V_703 / 2 - 1 ;
}
V_711 = ( 1 << V_717 [ V_709 ] [ 1 ] ) * V_704 * 4 ;
V_715 = V_712 ;
V_716 = ( V_711 / 2 + V_715 ) % V_711 ;
V_714 = V_711 * V_717 [ V_709 ] [ 2 ] + V_715 ;
F_103 ( V_153 , 0x15 , 0xFB ) ;
F_56 ( V_153 , 0x15 , 0x04 ) ;
V_708 = ( V_717 [ V_709 ] [ 3 ] * V_704 ) - 1 ;
if( V_704 == 4 ) V_708 |= 0x80 ;
else if( V_704 == 2 ) V_708 |= 0x40 ;
F_55 ( V_153 , 0x13 , V_717 [ V_709 ] [ 4 ] ) ;
F_55 ( V_153 , 0x14 , V_708 ) ;
V_712 <<= 16 ;
V_713 <<= 16 ;
if( ( V_712 + V_715 >= V_700 ) ||
( V_713 + V_715 >= V_700 ) ||
( V_712 + V_716 >= V_700 ) ||
( V_712 + V_714 >= V_700 ) )
continue;
F_158 ( ( ( unsigned short ) V_715 ) ,
( V_707 + V_712 + V_715 ) ) ;
F_158 ( ( ( unsigned short ) V_713 ) ,
( V_707 + V_713 + V_715 ) ) ;
F_158 ( ( ( unsigned short ) V_716 ) ,
( V_707 + V_712 + V_716 ) ) ;
F_158 ( ( ( unsigned short ) V_714 ) ,
( V_707 + V_712 + V_714 ) ) ;
if( F_159 ( V_707 + V_712 + V_715 ) == V_715 )
return 1 ;
}
return 0 ;
}
static void F_163 ( struct V_416 * V_418 , unsigned int V_700 )
{
struct V_76 * V_77 = F_135 ( V_418 ) ;
int V_33 , V_34 , V_704 ;
int V_705 , V_706 ;
V_704 = F_157 ( V_77 ) ;
for( V_33 = 6 ; V_33 >= 0 ; V_33 -- ) {
V_705 = 1 << V_33 ;
for( V_34 = 4 ; V_34 >= 1 ; V_34 -- ) {
V_706 = 15 - V_34 ;
if( ( V_705 * V_34 ) <= 64 ) {
if( F_161 ( V_77 ,
V_34 ,
V_704 ,
V_705 ,
V_706 ,
V_700 ) )
return;
}
}
}
}
static void F_164 ( struct V_416 * V_418 )
{
struct V_76 * V_77 = F_135 ( V_418 ) ;
unsigned char * V_718 = V_77 -> V_82 . V_84 ;
T_3 V_156 , V_719 , V_720 , V_721 , V_722 , V_723 , V_724 , V_725 , V_726 ;
T_4 V_99 , V_727 , V_728 = 0 ;
unsigned int V_700 ;
if( ! V_77 -> V_82 . V_83 )
V_718 = NULL ;
F_55 ( V_153 , 0x05 , 0x86 ) ;
if( V_718 ) {
if( V_718 [ 0x52 ] & 0x80 ) {
V_728 = V_718 [ 0x52 ] ;
} else {
V_728 = F_27 ( V_153 , 0x3a ) ;
}
V_728 &= 0x07 ;
}
V_721 = 0x80 ; V_724 = 0x80 ;
if( V_77 -> V_451 <= 0x13 ) {
V_719 = 0x44 ; V_720 = 0x42 ;
V_722 = 0x44 ; V_723 = 0x42 ;
} else {
V_719 = 0x68 ; V_720 = 0x43 ;
V_722 = 0x68 ; V_723 = 0x43 ;
if( V_718 ) {
V_99 = V_728 * 5 ;
V_727 = V_99 + 0x54 ;
V_719 = V_718 [ V_727 ++ ] ;
V_720 = V_718 [ V_727 ++ ] ;
V_721 = V_718 [ V_727 ++ ] ;
V_727 = V_99 + 0x7c ;
V_722 = V_718 [ V_727 ++ ] ;
V_723 = V_718 [ V_727 ++ ] ;
V_724 = V_718 [ V_727 ++ ] ;
}
}
F_55 ( V_153 , 0x28 , V_719 ) ;
F_55 ( V_153 , 0x29 , V_720 ) ;
F_55 ( V_153 , 0x2a , V_721 ) ;
F_55 ( V_153 , 0x2e , V_722 ) ;
F_55 ( V_153 , 0x2f , V_723 ) ;
F_55 ( V_153 , 0x30 , V_724 ) ;
V_719 = 0x10 ;
if( V_718 )
V_719 = V_718 [ 0xa4 ] ;
F_55 ( V_153 , 0x07 , V_719 ) ;
F_55 ( V_153 , 0x11 , 0x0f ) ;
V_719 = 0x01 ; V_720 = 0x43 ; V_721 = 0x1e ; V_722 = 0x2a ;
V_723 = 0x06 ; V_724 = 0x00 ; V_725 = 0x00 ; V_726 = 0x00 ;
if( V_718 ) {
V_728 += 0xa5 ;
V_719 = V_718 [ V_728 ] ;
V_720 = V_718 [ V_728 + 8 ] ;
V_721 = V_718 [ V_728 + 16 ] ;
V_722 = V_718 [ V_728 + 24 ] ;
V_723 = V_718 [ V_728 + 32 ] ;
V_724 = V_718 [ V_728 + 40 ] ;
V_725 = V_718 [ V_728 + 48 ] ;
V_726 = V_718 [ V_728 + 56 ] ;
}
if( V_77 -> V_451 >= 0x80 )
V_721 &= 0xfd ;
F_55 ( V_153 , 0x15 , V_719 ) ;
F_55 ( V_153 , 0x16 , V_720 ) ;
F_55 ( V_153 , 0x17 , V_721 ) ;
F_55 ( V_153 , 0x18 , V_722 ) ;
F_55 ( V_153 , 0x19 , V_723 ) ;
F_55 ( V_153 , 0x1a , V_724 ) ;
F_55 ( V_153 , 0x1b , V_725 ) ;
F_55 ( V_153 , 0x1c , V_726 ) ;
F_103 ( V_153 , 0x15 , 0xfb ) ;
F_56 ( V_153 , 0x15 , 0x04 ) ;
if( V_718 ) {
if( V_718 [ 0x53 ] & 0x02 ) {
F_56 ( V_153 , 0x19 , 0x20 ) ;
}
}
V_719 = 0x04 ;
if( V_77 -> V_451 >= 0x80 )
V_719 |= 0x01 ;
F_55 ( V_153 , 0x1f , V_719 ) ;
F_55 ( V_153 , 0x20 , 0xa4 ) ;
V_719 = 0xf6 ; V_720 = 0x0d ; V_721 = 0x00 ;
if( V_718 ) {
V_719 = V_718 [ 0xe8 ] ;
V_720 = V_718 [ 0xe9 ] ;
V_721 = V_718 [ 0xea ] ;
}
F_55 ( V_153 , 0x23 , V_719 ) ;
F_55 ( V_153 , 0x24 , V_720 ) ;
F_55 ( V_153 , 0x25 , V_721 ) ;
F_55 ( V_153 , 0x21 , 0x84 ) ;
F_55 ( V_153 , 0x22 , 0x00 ) ;
F_55 ( V_152 , 0x37 , 0x00 ) ;
F_56 ( V_151 , 0x24 , 0x01 ) ;
F_55 ( V_151 , 0x00 , 0x00 ) ;
V_719 = 0x40 ; V_720 = 0x11 ;
if( V_718 ) {
V_719 = V_718 [ 0xec ] ;
V_720 = V_718 [ 0xeb ] ;
}
F_55 ( V_151 , 0x02 , V_719 ) ;
if( V_77 -> V_451 >= 0x80 )
V_720 &= ~ 0x01 ;
V_156 = F_27 ( V_490 , 0x00 ) ;
if( ( V_156 == 1 ) || ( V_156 == 2 ) ) {
F_55 ( V_152 , 0x37 , 0x02 ) ;
F_55 ( V_195 , 0x00 , 0x1c ) ;
V_722 = 0x00 ; V_723 = 0x00 ; V_724 = 0x10 ;
if( V_77 -> V_82 . V_83 ) {
V_722 = V_718 [ 0xf5 ] ;
V_723 = V_718 [ 0xf6 ] ;
V_724 = V_718 [ 0xf7 ] ;
}
F_55 ( V_490 , 0x0d , V_722 ) ;
F_55 ( V_490 , 0x0e , V_723 ) ;
F_55 ( V_490 , 0x10 , V_724 ) ;
F_55 ( V_490 , 0x0f , 0x3f ) ;
V_156 = F_27 ( V_490 , 0x01 ) ;
if( V_156 >= 0xb0 ) {
V_156 = F_27 ( V_490 , 0x23 ) ;
V_156 &= 0x20 ;
V_156 <<= 1 ;
F_55 ( V_490 , 0x23 , V_156 ) ;
}
} else {
V_720 &= ~ 0x10 ;
}
F_55 ( V_153 , 0x32 , V_720 ) ;
F_103 ( V_151 , 0x24 , 0xfe ) ;
V_156 = F_27 ( V_153 , 0x16 ) ;
V_156 &= 0xc3 ;
F_55 ( V_152 , 0x35 , V_156 ) ;
F_55 ( V_152 , 0x83 , 0x00 ) ;
#if ! F_2 ( V_27 ) && ! F_2 ( V_28 )
if( V_30 ) {
F_55 ( V_153 , 0x13 , 0x28 ) ;
V_156 = ( ( V_30 >> 10 ) - 1 ) | 0x40 ;
F_55 ( V_153 , 0x14 , V_156 ) ;
} else {
#endif
V_700 = V_77 -> V_320 ;
F_155 ( V_77 , & V_700 , 4 ) ;
if( V_77 -> V_569 ) {
F_163 ( V_418 , V_700 ) ;
F_154 ( V_77 -> V_569 ) ;
} else {
F_4 ( V_58
L_121 ) ;
F_55 ( V_153 , 0x13 , 0x28 ) ;
F_55 ( V_153 , 0x14 , 0x47 ) ;
}
#if ! F_2 ( V_27 ) && ! F_2 ( V_28 )
}
#endif
if( V_718 ) {
V_719 = V_718 [ 0xe6 ] ;
V_720 = V_718 [ 0xe7 ] ;
} else {
V_156 = F_27 ( V_153 , 0x3a ) ;
if( ( V_156 & 0x30 ) == 0x30 ) {
V_719 = 0x04 ;
V_720 = 0x92 ;
} else {
V_719 = 0x14 ;
V_720 = 0xb2 ;
}
}
F_55 ( V_153 , 0x21 , V_719 ) ;
F_55 ( V_153 , 0x22 , V_720 ) ;
F_102 ( V_77 ) ;
V_77 -> V_82 . V_664 = false ;
F_139 ( & V_77 -> V_82 , false ) ;
F_140 ( & V_77 -> V_82 , false ) ;
V_77 -> V_364 = V_77 -> V_366 = 0 ;
V_77 -> V_82 . V_729 = 8 << 20 ;
F_60 ( & V_77 -> V_82 , 0x2e | 0x80 ) ;
F_55 ( V_153 , 0x05 , 0x86 ) ;
F_56 ( V_153 , 0x01 , 0x20 ) ;
F_55 ( V_152 , 0x34 , 0x2e ) ;
V_77 -> V_324 = 0x2e ;
}
static inline int F_165 ( struct V_76 * V_77 )
{
return V_77 -> V_730 == V_731 ;
}
static void F_166 ( struct V_76 * V_77 , int V_732 )
{
unsigned int V_33 ;
T_3 V_156 ;
for( V_33 = 0 ; V_33 <= ( V_732 * 10 * 36 ) ; V_33 ++ ) {
V_156 = F_27 ( V_153 , 0x05 ) ;
V_156 ++ ;
}
}
static int F_167 ( struct V_76 * V_77 ,
struct V_416 * V_733 ,
unsigned short V_734 )
{
struct V_416 * V_418 = NULL ;
unsigned short V_117 ;
int V_164 = 0 ;
while( ( V_418 = F_168 ( V_735 , V_418 ) ) ) {
V_117 = V_418 -> V_736 ;
if( V_117 == V_734 ) {
V_164 = 1 ;
F_169 ( V_418 ) ;
break;
}
}
return V_164 ;
}
static int F_170 ( struct V_76 * V_77 , int V_737 ,
unsigned int V_738 , unsigned int V_700 )
{
unsigned int V_739 ;
int V_33 ;
F_119 ( 0 , V_77 -> V_569 ) ;
for( V_33 = V_737 ; V_33 <= V_738 ; V_33 ++ ) {
V_739 = 1 << V_33 ;
if( V_739 < V_700 )
F_119 ( V_739 , V_77 -> V_569 + V_739 ) ;
}
F_166 ( V_77 , 150 ) ;
if( F_160 ( V_77 -> V_569 ) != 0 )
return 0 ;
for( V_33 = V_737 ; V_33 <= V_738 ; V_33 ++ ) {
V_739 = 1 << V_33 ;
if( V_739 < V_700 ) {
if( F_160 ( V_77 -> V_569 + V_739 ) != V_739 )
return 0 ;
} else
return 0 ;
}
return 1 ;
}
static int F_171 ( struct V_76 * V_77 )
{
unsigned int V_704 , V_740 , V_741 , V_700 ;
int V_33 , V_34 , V_709 , V_742 , V_743 ;
T_3 V_156 , V_708 ;
static const T_3 V_744 [ 12 * 5 ] = {
0x02 , 0x0e , 0x0b , 0x80 , 0x5d ,
0x02 , 0x0e , 0x0a , 0x40 , 0x59 ,
0x02 , 0x0d , 0x0b , 0x40 , 0x4d ,
0x02 , 0x0e , 0x09 , 0x20 , 0x55 ,
0x02 , 0x0d , 0x0a , 0x20 , 0x49 ,
0x02 , 0x0c , 0x0b , 0x20 , 0x3d ,
0x02 , 0x0e , 0x08 , 0x10 , 0x51 ,
0x02 , 0x0d , 0x09 , 0x10 , 0x45 ,
0x02 , 0x0c , 0x0a , 0x10 , 0x39 ,
0x02 , 0x0d , 0x08 , 0x08 , 0x41 ,
0x02 , 0x0c , 0x09 , 0x08 , 0x35 ,
0x02 , 0x0c , 0x08 , 0x04 , 0x31
} ;
static const T_3 V_745 [ 4 * 5 ] = {
0x02 , 0x0d , 0x09 , 0x40 , 0x45 ,
0x02 , 0x0c , 0x09 , 0x20 , 0x35 ,
0x02 , 0x0c , 0x08 , 0x10 , 0x31 ,
0x02 , 0x0b , 0x08 , 0x08 , 0x21
} ;
F_56 ( V_153 , 0x20 , ( 0x80 | 0x04 ) ) ;
V_700 = V_77 -> V_320 ;
F_155 ( V_77 , & V_700 , 32 ) ;
if( ! V_77 -> V_569 ) {
F_4 ( V_35 L_122 ) ;
F_55 ( V_153 , 0x13 , 0x35 ) ;
F_55 ( V_153 , 0x14 , 0x41 ) ;
return - V_746 ;
}
F_55 ( V_153 , 0x15 , 0x00 ) ;
F_55 ( V_153 , 0x1c , 0x00 ) ;
if( V_77 -> V_85 == V_411 ) {
V_741 = 1 ;
V_156 = F_27 ( V_152 , 0x97 ) ;
if( ! ( V_156 & 0x01 ) ) {
V_704 = 32 ;
F_55 ( V_153 , 0x13 , 0xb1 ) ;
F_55 ( V_153 , 0x14 , 0x52 ) ;
F_166 ( V_77 , 1 ) ;
V_708 = 0x02 ;
if( F_170 ( V_77 , 23 , 24 , V_700 ) )
goto V_747;
F_55 ( V_153 , 0x13 , 0x31 ) ;
F_55 ( V_153 , 0x14 , 0x42 ) ;
F_166 ( V_77 , 1 ) ;
if( F_170 ( V_77 , 23 , 23 , V_700 ) )
goto V_747;
V_704 = 16 ;
F_55 ( V_153 , 0x13 , 0xb1 ) ;
F_55 ( V_153 , 0x14 , 0x41 ) ;
F_166 ( V_77 , 1 ) ;
V_708 = 0x01 ;
if( F_170 ( V_77 , 22 , 23 , V_700 ) )
goto V_747;
else
F_55 ( V_153 , 0x13 , 0x31 ) ;
} else {
V_704 = 16 ;
F_55 ( V_153 , 0x13 , 0xb1 ) ;
F_55 ( V_153 , 0x14 , 0x41 ) ;
F_166 ( V_77 , 1 ) ;
V_708 = 0x01 ;
if( F_170 ( V_77 , 22 , 23 , V_700 ) )
goto V_747;
F_55 ( V_153 , 0x13 , 0x31 ) ;
F_55 ( V_153 , 0x14 , 0x31 ) ;
F_166 ( V_77 , 1 ) ;
if( F_170 ( V_77 , 22 , 22 , V_700 ) )
goto V_747;
V_704 = 8 ;
F_55 ( V_153 , 0x13 , 0xb1 ) ;
F_55 ( V_153 , 0x14 , 0x30 ) ;
F_166 ( V_77 , 1 ) ;
V_708 = 0x00 ;
if( F_170 ( V_77 , 21 , 22 , V_700 ) )
goto V_747;
else
F_55 ( V_153 , 0x13 , 0x31 ) ;
}
} else {
V_156 = F_27 ( V_152 , 0x97 ) ;
if( ! ( V_156 & 0x10 ) ) {
V_156 = F_27 ( V_153 , 0x39 ) ;
V_156 >>= 1 ;
}
if( V_156 & 0x01 ) {
V_704 = 32 ;
if( V_77 -> V_451 == 2 ) {
V_741 = 2 ;
F_55 ( V_153 , 0x13 , 0xa1 ) ;
F_55 ( V_153 , 0x14 , 0x44 ) ;
V_708 = 0x04 ;
F_166 ( V_77 , 1 ) ;
if( F_170 ( V_77 , 23 , 24 , V_700 ) )
goto V_747;
F_55 ( V_153 , 0x13 , 0x21 ) ;
F_55 ( V_153 , 0x14 , 0x34 ) ;
if( F_170 ( V_77 , 22 , 23 , V_700 ) )
goto V_747;
V_741 = 1 ;
F_55 ( V_153 , 0x13 , 0xa1 ) ;
F_55 ( V_153 , 0x14 , 0x40 ) ;
V_708 = 0x00 ;
if( F_170 ( V_77 , 22 , 23 , V_700 ) )
goto V_747;
F_55 ( V_153 , 0x13 , 0x21 ) ;
F_55 ( V_153 , 0x14 , 0x30 ) ;
} else {
V_741 = 3 ;
F_55 ( V_153 , 0x13 , 0xa1 ) ;
F_55 ( V_153 , 0x14 , 0x4c ) ;
V_708 = 0x0c ;
F_166 ( V_77 , 1 ) ;
if( F_170 ( V_77 , 23 , 25 , V_700 ) )
goto V_747;
V_741 = 2 ;
F_55 ( V_153 , 0x14 , 0x48 ) ;
F_166 ( V_77 , 1 ) ;
V_708 = 0x08 ;
if( F_170 ( V_77 , 23 , 24 , V_700 ) )
goto V_747;
F_55 ( V_153 , 0x13 , 0x21 ) ;
F_55 ( V_153 , 0x14 , 0x3c ) ;
V_708 = 0x0c ;
if( F_170 ( V_77 , 23 , 24 , V_700 ) ) {
V_741 = 3 ;
} else {
V_741 = 2 ;
F_55 ( V_153 , 0x14 , 0x38 ) ;
V_708 = 0x08 ;
}
}
F_166 ( V_77 , 1 ) ;
} else {
V_704 = 64 ;
if( V_77 -> V_451 == 2 ) {
V_741 = 1 ;
F_55 ( V_153 , 0x13 , 0xa1 ) ;
F_55 ( V_153 , 0x14 , 0x52 ) ;
F_166 ( V_77 , 1 ) ;
V_708 = 0x02 ;
if( F_170 ( V_77 , 23 , 24 , V_700 ) )
goto V_747;
F_55 ( V_153 , 0x13 , 0x21 ) ;
F_55 ( V_153 , 0x14 , 0x42 ) ;
} else {
V_741 = 2 ;
F_55 ( V_153 , 0x13 , 0xa1 ) ;
F_55 ( V_153 , 0x14 , 0x5a ) ;
F_166 ( V_77 , 1 ) ;
V_708 = 0x0a ;
if( F_170 ( V_77 , 24 , 25 , V_700 ) )
goto V_747;
F_55 ( V_153 , 0x13 , 0x21 ) ;
F_55 ( V_153 , 0x14 , 0x4a ) ;
}
F_166 ( V_77 , 1 ) ;
}
}
V_747:
F_36 ( V_153 , 0x14 , 0xf0 , V_708 ) ;
F_166 ( V_77 , 1 ) ;
V_34 = ( V_77 -> V_85 == V_411 ) ? 5 : 9 ;
V_709 = ( V_77 -> V_85 == V_411 ) ? 12 : 4 ;
V_743 = - V_748 ;
for( V_33 = 0 ; V_33 < V_709 ; V_33 ++ ) {
V_156 = ( V_77 -> V_85 == V_411 ) ?
V_744 [ ( V_33 * 5 ) + 4 ] : V_745 [ ( V_33 * 5 ) + 4 ] ;
F_36 ( V_153 , 0x13 , 0x80 , V_156 ) ;
F_166 ( V_77 , 50 ) ;
V_740 = ( V_77 -> V_85 == V_411 ) ?
V_744 [ ( V_33 * 5 ) + 3 ] : V_745 [ ( V_33 * 5 ) + 3 ] ;
V_156 = F_27 ( V_153 , 0x13 ) ;
if( V_156 & 0x80 ) V_740 <<= 1 ;
if( V_77 -> V_85 == V_411 ) {
if( V_704 == 16 ) V_740 <<= 1 ;
else if( V_704 == 32 ) V_740 <<= 2 ;
} else {
if( V_704 == 64 ) V_740 <<= 1 ;
}
V_156 = 0 ;
V_742 = V_741 ;
if( V_742 == 3 ) V_742 = 4 ;
if( ( V_740 * V_742 ) <= 256 ) {
while( ( V_740 >>= 1 ) ) V_156 += 0x10 ;
}
if( ! V_156 ) continue;
F_36 ( V_153 , 0x14 , 0x0f , ( V_156 & 0xf0 ) ) ;
F_166 ( V_77 , 1 ) ;
if ( F_170 ( V_77 , V_34 , ( ( V_156 >> 4 ) + V_741 - 2 + 20 ) , V_700 ) ) {
V_743 = 0 ;
break;
}
}
F_154 ( V_77 -> V_569 ) ;
return V_743 ;
}
static void F_172 ( struct V_76 * V_77 , T_3 V_749 )
{
T_3 V_719 , V_720 , V_721 ;
int V_99 ;
static const T_3 V_750 [ 8 * 3 ] = {
0x16 , 0x01 , 0x01 ,
0x3e , 0x03 , 0x01 ,
0x7c , 0x08 , 0x01 ,
0x79 , 0x06 , 0x01 ,
0x29 , 0x01 , 0x81 ,
0x5c , 0x23 , 0x01 ,
0x5c , 0x23 , 0x01 ,
0x5c , 0x23 , 0x01
} ;
static const T_3 V_751 [ 8 * 3 ] = {
0x5c , 0x23 , 0x01 ,
0x29 , 0x01 , 0x01 ,
0x7c , 0x08 , 0x01 ,
0x79 , 0x06 , 0x01 ,
0x29 , 0x01 , 0x81 ,
0x5c , 0x23 , 0x01 ,
0x5c , 0x23 , 0x01 ,
0x5c , 0x23 , 0x01
} ;
V_749 = 0 ;
V_99 = V_749 * 3 ;
V_719 = V_750 [ V_99 ] ; V_720 = V_750 [ V_99 + 1 ] ; V_721 = V_750 [ V_99 + 2 ] ;
if( V_77 -> V_513 ) {
V_719 = V_77 -> V_514 [ 0x90 + V_99 ] ;
V_720 = V_77 -> V_514 [ 0x90 + V_99 + 1 ] ;
V_721 = V_77 -> V_514 [ 0x90 + V_99 + 2 ] ;
}
F_55 ( V_153 , 0x28 , V_719 ) ;
F_55 ( V_153 , 0x29 , V_720 ) ;
F_55 ( V_153 , 0x2a , V_721 ) ;
F_166 ( V_77 , 0x43 ) ;
F_166 ( V_77 , 0x43 ) ;
F_166 ( V_77 , 0x43 ) ;
V_99 = V_749 * 3 ;
V_719 = V_751 [ V_99 ] ; V_720 = V_751 [ V_99 + 1 ] ; V_721 = V_751 [ V_99 + 2 ] ;
if( V_77 -> V_513 ) {
V_719 = V_77 -> V_514 [ 0xb8 + V_99 ] ;
V_720 = V_77 -> V_514 [ 0xb8 + V_99 + 1 ] ;
V_721 = V_77 -> V_514 [ 0xb8 + V_99 + 2 ] ;
}
F_55 ( V_153 , 0x2e , V_719 ) ;
F_55 ( V_153 , 0x2f , V_720 ) ;
F_55 ( V_153 , 0x30 , V_721 ) ;
F_166 ( V_77 , 0x43 ) ;
F_166 ( V_77 , 0x43 ) ;
F_166 ( V_77 , 0x43 ) ;
}
static void F_173 ( struct V_76 * V_77 ,
T_3 V_749 )
{
unsigned char * V_718 = V_77 -> V_514 ;
T_3 V_719 ;
F_55 ( V_153 , 0x28 , 0x64 ) ;
F_55 ( V_153 , 0x29 , 0x63 ) ;
F_166 ( V_77 , 15 ) ;
F_55 ( V_153 , 0x18 , 0x00 ) ;
F_55 ( V_153 , 0x19 , 0x20 ) ;
F_55 ( V_153 , 0x16 , 0x00 ) ;
F_55 ( V_153 , 0x16 , 0x80 ) ;
F_55 ( V_153 , 0x18 , 0xc5 ) ;
F_55 ( V_153 , 0x19 , 0x23 ) ;
F_55 ( V_153 , 0x16 , 0x00 ) ;
F_55 ( V_153 , 0x16 , 0x80 ) ;
F_166 ( V_77 , 1 ) ;
F_55 ( V_152 , 0x97 , 0x11 ) ;
F_172 ( V_77 , V_749 ) ;
F_166 ( V_77 , 0x46 ) ;
F_55 ( V_153 , 0x18 , 0xc5 ) ;
F_55 ( V_153 , 0x19 , 0x23 ) ;
F_55 ( V_153 , 0x16 , 0x00 ) ;
F_55 ( V_153 , 0x16 , 0x80 ) ;
F_166 ( V_77 , 1 ) ;
F_55 ( V_153 , 0x1b , 0x04 ) ;
F_166 ( V_77 , 1 ) ;
F_55 ( V_153 , 0x1b , 0x00 ) ;
F_166 ( V_77 , 1 ) ;
V_719 = 0x31 ;
if ( V_77 -> V_513 ) {
V_719 = V_718 [ 0xf0 ] ;
}
F_55 ( V_153 , 0x18 , V_719 ) ;
F_55 ( V_153 , 0x19 , 0x06 ) ;
F_55 ( V_153 , 0x16 , 0x04 ) ;
F_55 ( V_153 , 0x16 , 0x84 ) ;
F_166 ( V_77 , 1 ) ;
}
static void F_174 ( struct V_76 * V_77 )
{
F_172 ( V_77 , 1 ) ;
F_55 ( V_152 , 0x97 , 0x11 ) ;
F_166 ( V_77 , 0x46 ) ;
F_55 ( V_153 , 0x18 , 0x00 ) ;
F_55 ( V_153 , 0x19 , 0x80 ) ;
F_55 ( V_153 , 0x16 , 0x05 ) ;
F_55 ( V_153 , 0x16 , 0x85 ) ;
F_55 ( V_153 , 0x18 , 0x00 ) ;
F_55 ( V_153 , 0x19 , 0xc0 ) ;
F_55 ( V_153 , 0x16 , 0x05 ) ;
F_55 ( V_153 , 0x16 , 0x85 ) ;
F_55 ( V_153 , 0x18 , 0x00 ) ;
F_55 ( V_153 , 0x19 , 0x40 ) ;
F_55 ( V_153 , 0x16 , 0x05 ) ;
F_55 ( V_153 , 0x16 , 0x85 ) ;
F_55 ( V_153 , 0x18 , 0x42 ) ;
F_55 ( V_153 , 0x19 , 0x02 ) ;
F_55 ( V_153 , 0x16 , 0x05 ) ;
F_55 ( V_153 , 0x16 , 0x85 ) ;
F_166 ( V_77 , 1 ) ;
F_55 ( V_153 , 0x1b , 0x04 ) ;
F_166 ( V_77 , 1 ) ;
F_55 ( V_153 , 0x1b , 0x00 ) ;
F_166 ( V_77 , 1 ) ;
F_55 ( V_153 , 0x18 , 0x42 ) ;
F_55 ( V_153 , 0x19 , 0x00 ) ;
F_55 ( V_153 , 0x16 , 0x05 ) ;
F_55 ( V_153 , 0x16 , 0x85 ) ;
F_166 ( V_77 , 1 ) ;
}
static void F_175 ( struct V_76 * V_77 , T_3 V_749 )
{
unsigned char * V_718 = V_77 -> V_514 ;
static const T_3 V_752 [ 8 ] = {
0x88 , 0xaa , 0x48 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00
} ;
static const T_3 V_753 [ 8 ] = {
0x44 , 0x77 , 0x77 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00
} ;
static const T_3 V_754 [ 8 ] = {
0x48 , 0x78 , 0x88 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00
} ;
T_3 V_156 ;
T_3 V_719 ;
T_3 V_720 ;
T_3 V_721 ;
F_55 ( V_152 , 0xb0 , 0x80 ) ;
F_55 ( V_152 , 0x82 , 0x77 ) ;
F_55 ( V_152 , 0x86 , 0x00 ) ;
V_156 = F_27 ( V_152 , 0x86 ) ;
F_55 ( V_152 , 0x86 , 0x88 ) ;
V_156 = F_27 ( V_152 , 0x86 ) ;
V_719 = V_754 [ V_749 ] ; V_720 = V_753 [ V_749 ] ; V_721 = V_752 [ V_749 ] ;
if ( V_77 -> V_513 ) {
V_719 = V_718 [ V_749 + 0x168 ] ;
V_720 = V_718 [ V_749 + 0x160 ] ;
V_721 = V_718 [ V_749 + 0x158 ] ;
}
F_55 ( V_152 , 0x86 , V_719 ) ;
F_55 ( V_152 , 0x82 , 0x77 ) ;
F_55 ( V_152 , 0x85 , 0x00 ) ;
V_156 = F_27 ( V_152 , 0x85 ) ;
F_55 ( V_152 , 0x85 , 0x88 ) ;
V_156 = F_27 ( V_152 , 0x85 ) ;
F_55 ( V_152 , 0x85 , V_720 ) ;
F_55 ( V_152 , 0x82 , V_721 ) ;
F_55 ( V_152 , 0x98 , 0x01 ) ;
F_55 ( V_152 , 0x9a , 0x02 ) ;
if ( F_165 ( V_77 ) )
F_174 ( V_77 ) ;
else
F_173 ( V_77 , V_749 ) ;
}
static T_3 F_176 ( struct V_76 * V_77 )
{
unsigned char * V_718 = V_77 -> V_514 ;
T_3 V_755 ;
T_3 V_156 ;
T_3 V_719 ;
V_755 = 0x00 ; V_719 = 0x10 ;
if ( V_77 -> V_513 ) {
V_755 = V_718 [ 0x62 ] ;
V_719 = V_718 [ 0x1d2 ] ;
}
if ( ! ( V_755 & 0x80 ) ) {
if ( F_165 ( V_77 ) ) {
F_103 ( V_152 , 0xb4 , 0xfd ) ;
F_56 ( V_152 , 0x4a , 0x80 ) ;
V_156 = F_27 ( V_152 , 0x48 ) ;
F_56 ( V_152 , 0xb4 , 0x02 ) ;
V_755 = V_156 & 0x01 ;
} else if ( V_77 -> V_85 == V_411 ) {
F_55 ( V_152 , 0x97 , V_719 ) ;
V_156 = F_27 ( V_152 , 0x97 ) ;
if ( V_156 & 0x10 ) {
V_755 = ( V_156 & 0x01 ) << 1 ;
}
} else {
V_156 = F_27 ( V_153 , 0x39 ) ;
V_755 = V_156 & 0x02 ;
if ( ! ( V_755 ) ) {
V_156 = F_27 ( V_153 , 0x3a ) ;
V_755 = ( V_156 >> 1 ) & 0x01 ;
}
}
}
V_755 &= 0x07 ;
return V_755 ;
}
static int F_177 ( struct V_416 * V_418 )
{
struct V_76 * V_77 = F_135 ( V_418 ) ;
unsigned char * V_718 = V_77 -> V_514 ;
struct V_416 * V_733 = NULL ;
const T_3 * V_756 , * V_757 ;
T_3 V_719 , V_720 , V_721 , V_722 , V_723 , V_156 , V_755 ;
T_2 V_758 , V_749 , V_759 ;
int V_33 , V_34 , V_709 , V_99 ;
static const T_3 V_760 [ 3 ] = { 0xf6 , 0x0d , 0x00 } ;
static const T_3 V_761 [ 2 ] = { 0xa3 , 0xfb } ;
static const T_3 V_762 [ 3 ] = { 0xc0 , 0x11 , 0x00 } ;
static const T_3 V_752 [ 8 ] = {
0x88 , 0xaa , 0x48 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00
} ;
static const T_3 V_753 [ 8 ] = {
0x44 , 0x77 , 0x77 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00
} ;
static const T_3 V_754 [ 8 ] = {
0x48 , 0x78 , 0x88 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00
} ;
static const T_3 V_763 [ 3 * 8 ] = {
0x90 , 0x28 , 0x24 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 ,
0x77 , 0x44 , 0x44 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 ,
0x77 , 0x44 , 0x44 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00
} ;
static const T_3 V_764 [ 2 * 8 ] = {
0x55 , 0x55 , 0x55 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 ,
0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00
} ;
static const T_3 V_765 [ 8 * 4 ] = {
0xaa , 0xaa , 0xaa , 0xaa , 0xaa , 0xaa , 0xaa , 0xaa ,
0xaa , 0xaa , 0xaa , 0xaa , 0x00 , 0x00 , 0x00 , 0x00 ,
0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 ,
0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00
} ;
static const T_3 V_766 [ 8 * 4 ] = {
0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 ,
0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 ,
0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 ,
0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00
} ;
static const T_3 V_767 [ 8 * 2 ] = {
0x00 , 0x00 , 0xa0 , 0x00 , 0xa0 , 0x00 , 0x00 , 0x00 ,
0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00
} ;
static const T_3 V_768 [ 7 * 8 ] = {
0x54 , 0x32 , 0x44 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 ,
0x54 , 0x43 , 0x44 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 ,
0x0a , 0x05 , 0x07 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 ,
0x44 , 0x34 , 0x44 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 ,
0x10 , 0x0a , 0x0a , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 ,
0x11 , 0x0c , 0x0c , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 ,
0x05 , 0x05 , 0x05 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00
} ;
static const T_3 V_769 [ 3 * 8 ] = {
0xf0 , 0xf0 , 0xf0 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 ,
0x05 , 0x02 , 0x02 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 ,
0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00
} ;
static const T_3 V_770 [ 2 * 8 ] = {
0xc4 , 0x04 , 0x84 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 ,
0xc4 , 0x04 , 0x84 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00
} ;
V_156 = F_30 ( V_771 ) | 0x01 ;
F_70 ( V_771 , V_156 ) ;
V_156 = F_30 ( V_772 ) | 0x01 ;
F_70 ( V_480 , V_156 ) ;
F_55 ( V_153 , 0x05 , 0x86 ) ;
V_156 = F_27 ( V_153 , 0x05 ) ;
if( V_156 != 0xa1 )
return 0 ;
for( V_33 = 0 ; V_33 < 0x22 ; V_33 ++ ) {
if( 0x06 + V_33 == 0x20 ) continue;
F_55 ( V_153 , 0x06 + V_33 , 0x00 ) ;
}
for( V_33 = 0 ; V_33 < 0x0b ; V_33 ++ ) {
F_55 ( V_153 , 0x31 + V_33 , 0x00 ) ;
}
for( V_33 = 0 ; V_33 < 0x10 ; V_33 ++ ) {
F_55 ( V_152 , 0x30 + V_33 , 0x00 ) ;
}
V_756 = V_760 ;
if( V_77 -> V_513 ) {
V_756 = ( const T_3 * ) & V_718 [ 0x78 ] ;
}
for( V_33 = 0 ; V_33 < 3 ; V_33 ++ ) {
F_55 ( V_153 , 0x23 + V_33 , V_756 [ V_33 ] ) ;
}
V_756 = V_761 ;
if( V_77 -> V_513 ) {
V_756 = ( const T_3 * ) & V_718 [ 0x76 ] ;
}
for( V_33 = 0 ; V_33 < 2 ; V_33 ++ ) {
F_55 ( V_153 , 0x21 + V_33 , V_756 [ V_33 ] ) ;
}
V_719 = 0x18 ; V_720 = 0x00 ;
if( V_77 -> V_513 ) {
V_719 = V_718 [ 0x74 ] ;
V_720 = V_718 [ 0x75 ] ;
}
F_55 ( V_153 , 0x07 , V_719 ) ;
F_55 ( V_153 , 0x11 , 0x0f ) ;
F_55 ( V_153 , 0x1f , V_720 ) ;
F_55 ( V_153 , 0x20 , 0x80 | 0x20 | 0x04 ) ;
F_55 ( V_153 , 0x27 , 0x74 ) ;
V_756 = V_762 ;
if( V_77 -> V_513 ) {
V_756 = ( const T_3 * ) & V_718 [ 0x7b ] ;
}
for( V_33 = 0 ; V_33 < 3 ; V_33 ++ ) {
F_55 ( V_153 , 0x31 + V_33 , V_756 [ V_33 ] ) ;
}
if( V_77 -> V_85 == V_413 ) {
if( V_77 -> V_451 == 2 ) {
F_36 ( V_153 , 0x3b , 0x3f , 0xc0 ) ;
}
F_55 ( V_152 , 0x7d , 0xfe ) ;
F_55 ( V_152 , 0x7e , 0x0f ) ;
}
if( V_77 -> V_451 == 0 ) {
F_103 ( V_152 , 0x58 , 0xd7 ) ;
V_156 = F_27 ( V_152 , 0xcb ) ;
if( V_156 & 0x20 ) {
F_36 ( V_152 , 0x58 , 0xd7 , ( V_156 & 0x10 ) ? 0x08 : 0x20 ) ;
}
}
V_156 = ( V_77 -> V_85 == V_413 ) ? 0x20 : 0x00 ;
F_36 ( V_152 , 0x38 , 0x1f , V_156 ) ;
if( V_77 -> V_85 == V_411 ) {
F_55 ( V_153 , 0x36 , 0x70 ) ;
} else {
F_55 ( V_773 , 0x00 , 0x86 ) ;
F_55 ( V_773 , 0x32 , 0x00 ) ;
F_55 ( V_773 , 0x30 , 0x00 ) ;
F_55 ( V_773 , 0x32 , 0x01 ) ;
F_55 ( V_773 , 0x30 , 0x00 ) ;
F_103 ( V_773 , 0x2f , 0xdf ) ;
F_103 ( V_774 , 0x00 , 0x3f ) ;
F_55 ( V_151 , 0x2f , 0x01 ) ;
F_55 ( V_151 , 0x00 , 0x00 ) ;
F_55 ( V_151 , 0x02 , V_718 [ 0x7e ] ) ;
F_55 ( V_151 , 0x2e , 0x08 ) ;
F_103 ( V_151 , 0x35 , 0x7f ) ;
F_103 ( V_151 , 0x50 , 0xfe ) ;
V_156 = F_27 ( V_490 , 0x00 ) ;
if( V_156 == 1 || V_156 == 2 ) {
F_55 ( V_195 , 0x00 , 0x1c ) ;
F_55 ( V_490 , 0x0d , V_718 [ 0x7f ] ) ;
F_55 ( V_490 , 0x0e , V_718 [ 0x80 ] ) ;
F_55 ( V_490 , 0x10 , V_718 [ 0x81 ] ) ;
F_103 ( V_490 , 0x0f , 0x3f ) ;
V_156 = F_27 ( V_490 , 0x01 ) ;
if( ( V_156 & 0xf0 ) >= 0xb0 ) {
V_156 = F_27 ( V_490 , 0x23 ) ;
if( V_156 & 0x20 ) V_156 |= 0x40 ;
F_55 ( V_490 , 0x23 , V_156 ) ;
V_156 = ( V_156 & 0x20 ) ? 0x02 : 0x00 ;
F_36 ( V_151 , 0x1e , 0xfd , V_156 ) ;
}
}
V_719 = V_718 [ 0x77 ] ;
V_156 = F_27 ( V_153 , 0x3b ) ;
if( V_156 & 0x02 ) {
V_156 = F_27 ( V_153 , 0x3a ) ;
V_720 = ( V_156 & 0x30 ) >> 3 ;
if( ! ( V_720 & 0x04 ) ) V_720 ^= 0x02 ;
V_156 = F_27 ( V_153 , 0x39 ) ;
if( V_156 & 0x80 ) V_720 |= 0x80 ;
V_720 |= 0x01 ;
if( ( V_733 = F_96 ( V_439 , 0x0730 , NULL ) ) ) {
F_169 ( V_733 ) ;
if( ( ( V_720 & 0x06 ) == 2 ) || ( ( V_720 & 0x06 ) == 4 ) )
V_720 &= 0xf9 ;
V_720 |= 0x08 ;
V_719 &= 0xfe ;
} else {
V_733 = F_96 ( V_439 , 0x0735 , NULL ) ;
if( ! V_733 )
V_733 = F_96 ( V_439 , 0x0645 , NULL ) ;
if( ! V_733 )
V_733 = F_96 ( V_439 , 0x0650 , NULL ) ;
if( V_733 ) {
F_42 ( V_733 , 0x94 , & V_759 ) ;
V_759 &= 0xfffffeff ;
F_44 ( V_733 , 0x94 , V_759 ) ;
V_719 &= 0xfe ;
F_169 ( V_733 ) ;
} else if( F_167 ( V_77 , V_418 , V_439 ) ) {
V_719 &= 0xfe ;
} else if( F_167 ( V_77 , V_418 , 0x1106 ) ||
F_167 ( V_77 , V_418 , 0x1022 ) ||
F_167 ( V_77 , V_418 , 0x700e ) ||
F_167 ( V_77 , V_418 , 0x10de ) ) {
if( ( V_720 & 0x06 ) == 4 )
V_720 ^= 0x06 ;
V_720 |= 0x08 ;
}
}
F_36 ( V_152 , 0x5f , 0xf0 , V_720 ) ;
}
F_55 ( V_153 , 0x22 , V_719 ) ;
if( V_77 -> V_451 == 2 ) {
V_719 = F_27 ( V_153 , 0x3b ) ;
V_720 = F_27 ( V_153 , 0x3a ) ;
V_759 = V_718 [ 0x90 + 3 ] | ( V_718 [ 0x90 + 4 ] << 8 ) ;
if( ( ! ( V_719 & 0x02 ) ) && ( V_720 & 0x30 ) && ( V_759 < 0xcf ) )
F_36 ( V_152 , 0x5f , 0xf1 , 0x01 ) ;
if( ( V_733 = F_96 ( 0x10de , 0x01e0 , NULL ) ) ) {
if( 0 )
F_36 ( V_152 , 0x5f , 0xf1 , 0x01 ) ;
F_169 ( V_733 ) ;
}
}
V_719 = 0x30 ;
V_156 = F_27 ( V_153 , 0x3b ) ;
V_720 = F_27 ( V_152 , 0x5f ) ;
if( ( ! ( V_156 & 0x02 ) ) && ( V_720 & 0x0e ) )
V_719 |= 0x08 ;
F_55 ( V_153 , 0x27 , V_719 ) ;
if( V_718 [ 0x64 ] & 0x01 ) {
F_36 ( V_152 , 0x5f , 0xf0 , V_718 [ 0x64 ] ) ;
}
V_719 = V_718 [ 0x4f7 ] ;
F_42 ( V_418 , 0x50 , & V_759 ) ;
V_759 = ( V_759 >> 20 ) & 0x0f ;
if( V_759 == 1 ) {
V_719 &= 0xfc ;
F_56 ( V_152 , 0x5f , 0x08 ) ;
}
F_55 ( V_152 , 0x48 , V_719 ) ;
F_36 ( V_152 , 0x47 , 0x04 , V_718 [ 0x4f6 ] & 0xfb ) ;
F_36 ( V_152 , 0x49 , 0xf0 , V_718 [ 0x4f8 ] & 0x0f ) ;
F_36 ( V_152 , 0x4a , 0x60 , V_718 [ 0x4f9 ] & 0x9f ) ;
F_36 ( V_152 , 0x4b , 0x08 , V_718 [ 0x4fa ] & 0xf7 ) ;
F_36 ( V_152 , 0x4c , 0x80 , V_718 [ 0x4fb ] & 0x7f ) ;
F_55 ( V_152 , 0x70 , V_718 [ 0x4fc ] ) ;
F_36 ( V_152 , 0x71 , 0xf0 , V_718 [ 0x4fd ] & 0x0f ) ;
F_55 ( V_152 , 0x74 , 0xd0 ) ;
F_36 ( V_152 , 0x74 , 0xcf , V_718 [ 0x4fe ] & 0x30 ) ;
F_36 ( V_152 , 0x75 , 0xe0 , V_718 [ 0x4ff ] & 0x1f ) ;
F_36 ( V_152 , 0x76 , 0xe0 , V_718 [ 0x500 ] & 0x1f ) ;
V_719 = V_718 [ 0x501 ] ;
if( ( V_733 = F_96 ( 0x8086 , 0x2530 , NULL ) ) ) {
V_719 = 0xf0 ;
F_169 ( V_733 ) ;
}
F_55 ( V_152 , 0x77 , V_719 ) ;
}
V_755 = F_176 ( V_77 ) ;
if ( ! F_165 ( V_77 ) && V_755 ) {
F_178 ( & V_418 -> V_775 ,
L_123 ,
V_755 ) ;
V_749 = 0 ;
} else {
V_749 = V_755 ;
}
V_719 = 0xff ;
if( V_77 -> V_513 ) {
V_719 = V_718 [ 0x140 + V_749 ] ;
}
F_55 ( V_152 , 0x6d , V_719 ) ;
V_756 = V_763 ;
if( V_77 -> V_513 ) {
V_756 = ( const T_3 * ) & V_718 [ 0x128 ] ;
}
for( V_33 = 0 , V_34 = 0 ; V_33 < 3 ; V_33 ++ , V_34 += 8 ) {
F_55 ( V_152 , 0x68 + V_33 , V_756 [ V_34 + V_749 ] ) ;
}
V_756 = V_765 ;
V_757 = V_766 ;
if( V_77 -> V_513 ) {
V_99 = ( V_77 -> V_85 == V_411 ) ? 0x31a : 0x3a6 ;
V_756 = ( const T_3 * ) & V_718 [ V_99 ] ;
V_757 = ( const T_3 * ) & V_718 [ V_99 + 0x20 ] ;
}
for( V_33 = 0 ; V_33 < 2 ; V_33 ++ ) {
if( V_33 == 0 ) {
V_759 = F_179 ( ( ( T_2 * ) V_756 ) [ V_749 ] ) ;
V_758 = 0x6b ;
} else {
V_759 = F_179 ( ( ( T_2 * ) V_757 ) [ V_749 ] ) ;
V_758 = 0x6e ;
}
V_156 = 0x00 ;
for( V_34 = 0 ; V_34 < 16 ; V_34 ++ ) {
V_156 &= 0xf3 ;
if( V_759 & 0x01 ) V_156 |= 0x04 ;
if( V_759 & 0x02 ) V_156 |= 0x08 ;
V_759 >>= 2 ;
F_55 ( V_152 , V_758 , V_156 ) ;
V_156 = F_27 ( V_152 , V_758 ) ;
V_156 = F_27 ( V_152 , V_758 ) ;
V_156 += 0x10 ;
}
}
F_103 ( V_152 , 0x6e , 0xfc ) ;
V_756 = NULL ;
if( V_77 -> V_513 ) {
V_99 = ( V_77 -> V_85 == V_411 ) ? 0x35a : 0x3e6 ;
V_756 = ( const T_3 * ) & V_718 [ V_99 ] ;
}
for( V_33 = 0 ; V_33 < 4 ; V_33 ++ ) {
F_36 ( V_152 , 0x6e , 0xfc , V_33 ) ;
V_156 = 0x00 ;
for( V_34 = 0 ; V_34 < 2 ; V_34 ++ ) {
V_759 = 0 ;
if( V_756 ) {
V_759 = F_179 ( ( ( T_2 * ) V_756 ) [ V_749 * 8 ] ) ;
V_756 += 4 ;
}
for( V_709 = 0 ; V_709 < 16 ; V_709 ++ ) {
V_156 &= 0xfc ;
if( V_759 & 0x01 ) V_156 |= 0x01 ;
if( V_759 & 0x02 ) V_156 |= 0x02 ;
V_759 >>= 2 ;
F_55 ( V_152 , 0x6f , V_156 ) ;
V_156 = F_27 ( V_152 , 0x6f ) ;
V_156 = F_27 ( V_152 , 0x6f ) ;
V_156 += 0x08 ;
}
}
}
V_756 = V_764 ;
if( V_77 -> V_513 ) {
V_756 = ( const T_3 * ) & V_718 [ 0x148 ] ;
}
for( V_33 = 0 , V_34 = 0 ; V_33 < 2 ; V_33 ++ , V_34 += 8 ) {
F_55 ( V_152 , 0x80 + V_33 , V_756 [ V_34 + V_749 ] ) ;
}
F_103 ( V_152 , 0x89 , 0x8f ) ;
V_756 = V_767 ;
if( V_77 -> V_513 ) {
V_99 = ( V_77 -> V_85 == V_411 ) ? 0x45a : 0x4e6 ;
V_756 = ( const T_3 * ) & V_718 [ V_99 ] ;
}
V_759 = F_180 ( ( ( const T_4 * ) V_756 ) [ V_749 ] ) ;
V_156 = 0x80 ;
for( V_33 = 0 ; V_33 < 5 ; V_33 ++ ) {
V_156 &= 0xfc ;
if( V_759 & 0x01 ) V_156 |= 0x01 ;
if( V_759 & 0x02 ) V_156 |= 0x02 ;
V_759 >>= 2 ;
F_55 ( V_152 , 0x89 , V_156 ) ;
V_156 = F_27 ( V_152 , 0x89 ) ;
V_156 = F_27 ( V_152 , 0x89 ) ;
V_156 += 0x10 ;
}
V_719 = 0xb5 ; V_720 = 0x20 ; V_721 = 0xf0 ; V_722 = 0x13 ;
if( V_77 -> V_513 ) {
V_719 = V_718 [ 0x118 + V_749 ] ;
V_720 = V_718 [ 0xf8 + V_749 ] ;
V_721 = V_718 [ 0x120 + V_749 ] ;
V_722 = V_718 [ 0x1ca ] ;
}
F_55 ( V_152 , 0x45 , V_719 & 0x0f ) ;
F_55 ( V_152 , 0x99 , ( V_719 >> 4 ) & 0x07 ) ;
F_56 ( V_152 , 0x40 , V_719 & 0x80 ) ;
F_55 ( V_152 , 0x41 , V_720 ) ;
V_756 = V_768 ;
if( V_77 -> V_513 ) {
V_756 = ( const T_3 * ) & V_718 [ 0x170 ] ;
}
for( V_33 = 0 , V_34 = 0 ; V_33 < 7 ; V_33 ++ , V_34 += 8 ) {
F_55 ( V_152 , 0x90 + V_33 , V_756 [ V_34 + V_749 ] ) ;
}
F_55 ( V_152 , 0x59 , V_721 ) ;
V_756 = V_769 ;
if( V_77 -> V_513 ) {
V_756 = ( const T_3 * ) & V_718 [ 0x1a8 ] ;
}
for( V_33 = 0 , V_34 = 0 ; V_33 < 3 ; V_33 ++ , V_34 += 8 ) {
F_55 ( V_152 , 0xc3 + V_33 , V_756 [ V_34 + V_749 ] ) ;
}
V_756 = V_770 ;
if( V_77 -> V_513 ) {
V_756 = ( const T_3 * ) & V_718 [ 0x100 ] ;
}
for( V_33 = 0 , V_34 = 0 ; V_33 < 2 ; V_33 ++ , V_34 += 8 ) {
F_55 ( V_152 , 0x8a + V_33 , V_756 [ V_34 + V_749 ] ) ;
}
F_55 ( V_152 , 0xcf , V_722 ) ;
F_55 ( V_152 , 0x83 , 0x09 ) ;
F_55 ( V_152 , 0x87 , 0x00 ) ;
if( V_77 -> V_85 == V_413 ) {
if( ( V_77 -> V_451 == 1 ) ||
( V_77 -> V_451 == 2 ) ) {
F_55 ( V_152 , 0x8c , 0x87 ) ;
}
}
if ( V_749 == 1 )
F_55 ( V_153 , 0x17 , 0x80 ) ;
else
F_55 ( V_153 , 0x17 , 0x00 ) ;
F_55 ( V_153 , 0x1a , 0x87 ) ;
if( V_77 -> V_85 == V_411 ) {
F_55 ( V_153 , 0x15 , 0x00 ) ;
F_55 ( V_153 , 0x1c , 0x00 ) ;
}
switch( V_755 ) {
case 0 :
F_172 ( V_77 , V_749 ) ;
if( ( V_77 -> V_85 == V_411 ) ||
( V_77 -> V_451 == 1 ) ||
( V_77 -> V_451 == 2 ) ) {
V_719 = V_752 [ V_749 ] ; V_720 = V_753 [ V_749 ] ; V_721 = V_754 [ V_749 ] ;
if( V_77 -> V_513 ) {
V_719 = V_718 [ V_749 + 0x158 ] ;
V_720 = V_718 [ V_749 + 0x160 ] ;
V_721 = V_718 [ V_749 + 0x168 ] ;
}
F_55 ( V_152 , 0x82 , V_719 ) ;
F_55 ( V_152 , 0x85 , V_720 ) ;
F_55 ( V_152 , 0x86 , V_721 ) ;
} else {
F_55 ( V_152 , 0x82 , 0x88 ) ;
F_55 ( V_152 , 0x86 , 0x00 ) ;
V_156 = F_27 ( V_152 , 0x86 ) ;
F_55 ( V_152 , 0x86 , 0x88 ) ;
V_156 = F_27 ( V_152 , 0x86 ) ;
F_55 ( V_152 , 0x86 , V_718 [ V_749 + 0x168 ] ) ;
F_55 ( V_152 , 0x82 , 0x77 ) ;
F_55 ( V_152 , 0x85 , 0x00 ) ;
V_156 = F_27 ( V_152 , 0x85 ) ;
F_55 ( V_152 , 0x85 , 0x88 ) ;
V_156 = F_27 ( V_152 , 0x85 ) ;
F_55 ( V_152 , 0x85 , V_718 [ V_749 + 0x160 ] ) ;
F_55 ( V_152 , 0x82 , V_718 [ V_749 + 0x158 ] ) ;
}
if( V_77 -> V_85 == V_413 ) {
F_55 ( V_152 , 0x97 , 0x00 ) ;
}
F_55 ( V_152 , 0x98 , 0x01 ) ;
F_55 ( V_152 , 0x9a , 0x02 ) ;
F_55 ( V_153 , 0x18 , 0x01 ) ;
if( ( V_77 -> V_85 == V_411 ) ||
( V_77 -> V_451 == 2 ) ) {
F_55 ( V_153 , 0x19 , 0x40 ) ;
} else {
F_55 ( V_153 , 0x19 , 0x20 ) ;
}
F_55 ( V_153 , 0x16 , 0x00 ) ;
F_55 ( V_153 , 0x16 , 0x80 ) ;
if( ( V_77 -> V_85 == V_411 ) || ( V_718 [ 0x1cb ] != 0x0c ) ) {
F_166 ( V_77 , 0x43 ) ;
F_166 ( V_77 , 0x43 ) ;
F_166 ( V_77 , 0x43 ) ;
F_55 ( V_153 , 0x18 , 0x00 ) ;
if( ( V_77 -> V_85 == V_411 ) ||
( V_77 -> V_451 == 2 ) ) {
F_55 ( V_153 , 0x19 , 0x40 ) ;
} else {
F_55 ( V_153 , 0x19 , 0x20 ) ;
}
} else if( ( V_77 -> V_85 == V_413 ) && ( V_718 [ 0x1cb ] == 0x0c ) ) {
}
F_55 ( V_153 , 0x16 , 0x00 ) ;
F_55 ( V_153 , 0x16 , 0x80 ) ;
F_166 ( V_77 , 4 ) ;
V_719 = 0x31 ; V_720 = 0x03 ; V_721 = 0x83 ; V_722 = 0x03 ; V_723 = 0x83 ;
if( V_77 -> V_513 ) {
V_719 = V_718 [ 0xf0 ] ;
V_99 = ( V_77 -> V_85 == V_411 ) ? 0x4b2 : 0x53e ;
V_720 = V_718 [ V_99 ] ;
V_721 = V_718 [ V_99 + 1 ] ;
V_722 = V_718 [ V_99 + 2 ] ;
V_723 = V_718 [ V_99 + 3 ] ;
}
F_55 ( V_153 , 0x18 , V_719 ) ;
F_55 ( V_153 , 0x19 , ( ( V_77 -> V_85 == V_411 ) ? 0x02 : 0x01 ) ) ;
F_55 ( V_153 , 0x16 , V_720 ) ;
F_55 ( V_153 , 0x16 , V_721 ) ;
F_166 ( V_77 , 0x43 ) ;
F_55 ( V_153 , 0x1b , 0x03 ) ;
F_166 ( V_77 , 0x22 ) ;
F_55 ( V_153 , 0x18 , V_719 ) ;
F_55 ( V_153 , 0x19 , 0x00 ) ;
F_55 ( V_153 , 0x16 , V_722 ) ;
F_55 ( V_153 , 0x16 , V_723 ) ;
F_55 ( V_153 , 0x1b , 0x00 ) ;
break;
case 1 :
F_175 ( V_77 , V_749 ) ;
break;
default:
F_172 ( V_77 , V_749 ) ;
if( ( V_77 -> V_85 == V_413 ) &&
( ( V_77 -> V_451 == 1 ) ||
( V_77 -> V_451 == 2 ) ) ) {
F_55 ( V_152 , 0x82 , V_718 [ V_749 + 0x158 ] ) ;
F_55 ( V_152 , 0x85 , V_718 [ V_749 + 0x160 ] ) ;
F_55 ( V_152 , 0x86 , V_718 [ V_749 + 0x168 ] ) ;
} else {
F_55 ( V_152 , 0x82 , 0x88 ) ;
F_55 ( V_152 , 0x86 , 0x00 ) ;
V_156 = F_27 ( V_152 , 0x86 ) ;
F_55 ( V_152 , 0x86 , 0x88 ) ;
F_55 ( V_152 , 0x82 , 0x77 ) ;
F_55 ( V_152 , 0x85 , 0x00 ) ;
V_156 = F_27 ( V_152 , 0x85 ) ;
F_55 ( V_152 , 0x85 , 0x88 ) ;
V_156 = F_27 ( V_152 , 0x85 ) ;
V_719 = V_753 [ V_749 ] ; V_720 = V_752 [ V_749 ] ;
if( V_77 -> V_513 ) {
V_719 = V_718 [ V_749 + 0x160 ] ;
V_720 = V_718 [ V_749 + 0x158 ] ;
}
F_55 ( V_152 , 0x85 , V_719 ) ;
F_55 ( V_152 , 0x82 , V_720 ) ;
}
if( V_77 -> V_85 == V_413 ) {
F_55 ( V_152 , 0x97 , 0x11 ) ;
}
if( ( V_77 -> V_85 == V_413 ) && ( V_77 -> V_451 == 2 ) ) {
F_55 ( V_152 , 0x98 , 0x01 ) ;
} else {
F_55 ( V_152 , 0x98 , 0x03 ) ;
}
F_55 ( V_152 , 0x9a , 0x02 ) ;
if( V_77 -> V_85 == V_413 ) {
F_55 ( V_153 , 0x18 , 0x01 ) ;
} else {
F_55 ( V_153 , 0x18 , 0x00 ) ;
}
F_55 ( V_153 , 0x19 , 0x40 ) ;
F_55 ( V_153 , 0x16 , 0x00 ) ;
F_55 ( V_153 , 0x16 , 0x80 ) ;
if( ( V_77 -> V_85 == V_413 ) && ( V_718 [ 0x1cb ] != 0x0c ) ) {
F_166 ( V_77 , 0x43 ) ;
F_166 ( V_77 , 0x43 ) ;
F_166 ( V_77 , 0x43 ) ;
F_55 ( V_153 , 0x18 , 0x00 ) ;
F_55 ( V_153 , 0x19 , 0x40 ) ;
F_55 ( V_153 , 0x16 , 0x00 ) ;
F_55 ( V_153 , 0x16 , 0x80 ) ;
}
F_166 ( V_77 , 4 ) ;
V_719 = 0x31 ;
if( V_77 -> V_513 ) {
V_719 = V_718 [ 0xf0 ] ;
}
F_55 ( V_153 , 0x18 , V_719 ) ;
F_55 ( V_153 , 0x19 , 0x01 ) ;
if( V_77 -> V_85 == V_413 ) {
F_55 ( V_153 , 0x16 , V_718 [ 0x53e ] ) ;
F_55 ( V_153 , 0x16 , V_718 [ 0x53f ] ) ;
} else {
F_55 ( V_153 , 0x16 , 0x05 ) ;
F_55 ( V_153 , 0x16 , 0x85 ) ;
}
F_166 ( V_77 , 0x43 ) ;
if( V_77 -> V_85 == V_413 ) {
F_55 ( V_153 , 0x1b , 0x01 ) ;
} else {
F_55 ( V_153 , 0x1b , 0x03 ) ;
}
F_166 ( V_77 , 0x22 ) ;
F_55 ( V_153 , 0x18 , V_719 ) ;
F_55 ( V_153 , 0x19 , 0x00 ) ;
if( V_77 -> V_85 == V_413 ) {
F_55 ( V_153 , 0x16 , V_718 [ 0x540 ] ) ;
F_55 ( V_153 , 0x16 , V_718 [ 0x541 ] ) ;
} else {
F_55 ( V_153 , 0x16 , 0x05 ) ;
F_55 ( V_153 , 0x16 , 0x85 ) ;
}
F_55 ( V_153 , 0x1b , 0x00 ) ;
}
V_749 = 0 ;
V_719 = 0x03 ;
if( V_77 -> V_513 ) {
V_719 = V_718 [ 0x110 + V_749 ] ;
}
F_55 ( V_153 , 0x1b , V_719 ) ;
V_719 = 0x00 ; V_720 = 0x00 ;
if( V_77 -> V_513 ) {
V_719 = V_718 [ 0x62 ] ;
V_720 = V_718 [ 0x63 ] ;
}
V_749 = 0 ;
V_759 = 1 << V_749 ;
if( ( V_719 & 0x40 ) && ( V_720 & V_759 ) && V_77 -> V_513 ) {
F_55 ( V_153 , 0x13 , V_718 [ V_749 + 0xe0 ] ) ;
F_55 ( V_153 , 0x14 , V_718 [ V_749 + 0xe0 + 8 ] ) ;
} else {
int V_278 ;
V_77 -> V_82 . V_664 = false ;
F_139 ( & V_77 -> V_82 , false ) ;
F_140 ( & V_77 -> V_82 , false ) ;
V_77 -> V_364 = V_77 -> V_366 = 0 ;
V_77 -> V_82 . V_729 = 8 << 20 ;
F_60 ( & V_77 -> V_82 , 0x2e | 0x80 ) ;
F_55 ( V_153 , 0x05 , 0x86 ) ;
F_103 ( V_153 , 0x21 , 0xdf ) ;
V_278 = F_171 ( V_77 ) ;
F_56 ( V_153 , 0x21 , 0x20 ) ;
if ( V_278 ) {
F_181 ( & V_418 -> V_775 ,
L_124 ,
V_776 , V_278 ) ;
return 0 ;
}
}
#if 0
printk(KERN_DEBUG "-----------------\n");
for(i = 0; i < 0xff; i++) {
reg = SiS_GetReg(SISCR, i);
printk(KERN_DEBUG "CR%02x(%x) = 0x%02x\n", i, SISCR, reg);
}
for(i = 0; i < 0x40; i++) {
reg = SiS_GetReg(SISSR, i);
printk(KERN_DEBUG "SR%02x(%x) = 0x%02x\n", i, SISSR, reg);
}
printk(KERN_DEBUG "-----------------\n");
#endif
if( V_77 -> V_85 == V_411 ) {
F_56 ( V_152 , 0x32 , 0x20 ) ;
} else {
V_156 = F_27 ( V_490 , 0x00 ) ;
if( ( V_156 == 1 ) || ( V_156 == 2 ) ) {
F_102 ( V_77 ) ;
} else {
F_56 ( V_152 , 0x32 , 0x20 ) ;
}
}
V_77 -> V_82 . V_664 = false ;
F_139 ( & V_77 -> V_82 , false ) ;
F_140 ( & V_77 -> V_82 , false ) ;
V_77 -> V_364 = V_77 -> V_366 = 0 ;
F_60 ( & V_77 -> V_82 , 0x2e | 0x80 ) ;
F_55 ( V_153 , 0x05 , 0x86 ) ;
F_56 ( V_153 , 0x01 , 0x20 ) ;
F_55 ( V_152 , 0x34 , 0x2e ) ;
V_77 -> V_324 = 0x2e ;
if( V_77 -> V_85 == V_413 ) {
V_156 = F_27 ( V_152 , 0xca ) ;
V_719 = F_27 ( V_152 , 0xcc ) ;
if( ( V_156 & 0x10 ) && ( ! ( V_719 & 0x04 ) ) ) {
F_4 ( V_35
L_125 ) ;
return 0 ;
}
}
return 1 ;
}
static int F_182 ( struct V_416 * V_418 , const struct V_777 * V_778 )
{
struct V_779 * V_780 = & V_779 [ V_778 -> V_781 ] ;
struct V_76 * V_77 = NULL ;
struct V_234 * V_782 = NULL ;
T_4 V_783 ;
T_3 V_156 ;
int V_33 , V_164 ;
if( V_1 )
return - V_784 ;
V_782 = F_183 ( sizeof( * V_77 ) , & V_418 -> V_775 ) ;
if( ! V_782 )
return - V_746 ;
V_77 = (struct V_76 * ) V_782 -> V_236 ;
V_77 -> V_785 = V_782 ;
V_77 -> V_308 = V_309 ;
if( V_786 == NULL ) {
V_77 -> V_605 = 0 ;
} else {
struct V_76 * V_787 = V_786 ;
V_77 -> V_605 = 1 ;
while( ( V_787 = V_787 -> V_788 ) != NULL )
V_77 -> V_605 ++ ;
}
F_92 ( V_77 -> V_388 , V_780 -> V_789 , sizeof( V_77 -> V_388 ) ) ;
V_77 -> V_305 = 0 ;
V_77 -> V_316 = V_418 -> V_790 ;
V_77 -> V_318 = V_418 -> V_736 ;
V_77 -> V_451 = V_418 -> V_791 ;
V_77 -> V_82 . V_792 = V_77 -> V_451 ;
F_49 ( V_418 , V_793 , & V_783 ) ;
V_77 -> V_794 = V_783 & 0x01 ;
V_77 -> V_330 = V_418 -> V_795 -> V_796 ;
V_77 -> V_332 = F_184 ( V_418 -> V_797 ) ;
V_77 -> V_334 = F_185 ( V_418 -> V_797 ) ;
V_77 -> V_89 = V_418 -> V_798 ;
V_77 -> V_91 = V_418 -> V_799 ;
#ifndef F_186
if( V_8 == - 1 ) {
F_7 () ;
}
#endif
V_77 -> V_85 = V_780 -> V_85 ;
V_77 -> V_730 = V_780 -> V_85 ;
V_77 -> V_122 = V_780 -> V_800 ;
V_77 -> V_603 = V_780 -> V_603 ;
V_77 -> V_221 = V_780 -> V_221 ;
V_77 -> V_130 = V_780 -> V_130 ;
V_77 -> V_336 = 0xff ;
V_77 -> V_338 = 0xff ;
V_77 -> V_340 = 0xff ;
V_77 -> V_187 . V_107 = false ;
V_77 -> V_267 = 0 ;
V_77 -> V_575 = 0 ;
V_77 -> V_370 = 0 ;
if( V_418 -> V_801 [ V_802 ] . V_62 & V_803 ) {
if( V_77 -> V_794 )
V_77 -> V_370 = 1 ;
else {
F_4 ( V_58 L_126
L_127 ) ;
F_4 ( V_58 L_128
L_129 ) ;
}
}
V_77 -> V_2 = V_2 ;
V_77 -> V_3 = V_3 ;
V_77 -> V_4 = V_4 ;
V_77 -> V_5 = V_5 ;
V_77 -> V_6 = V_6 ;
V_77 -> V_7 = V_7 ;
V_77 -> V_8 = V_8 ;
V_77 -> V_9 = V_9 ;
V_77 -> V_10 = V_10 ;
V_77 -> V_11 = V_11 ;
V_77 -> V_12 = V_12 ;
V_77 -> V_13 = V_13 ;
V_77 -> V_20 = V_20 ;
V_77 -> V_21 = V_21 ;
V_77 -> V_22 = V_22 ;
V_77 -> V_23 = V_23 ;
V_77 -> V_357 = V_24 ;
V_77 -> V_359 = V_25 ;
V_77 -> V_26 = V_26 ;
V_77 -> V_148 = 0 ;
if( V_77 -> V_9 != - 1 ) {
V_77 -> V_148 = V_77 -> V_9 ;
}
V_77 -> V_82 . V_343 = V_16 ;
V_77 -> V_82 . V_804 = - 1 ;
V_77 -> V_82 . V_94 = V_17 ;
V_77 -> V_82 . V_805 = V_19 ;
V_77 -> V_82 . V_806 = 0xff ;
V_77 -> V_82 . V_807 = - 1 ;
V_77 -> V_82 . V_808 = false ;
V_77 -> V_82 . V_597 = false ;
V_77 -> V_82 . V_345 = false ;
V_77 -> V_82 . V_347 = false ;
V_77 -> V_82 . V_809 = false ;
V_77 -> V_82 . V_810 = false ;
V_77 -> V_82 . V_188 = 0x63 ;
V_77 -> V_82 . V_595 = - 1 ;
V_77 -> V_82 . V_599 = - 1 ;
V_77 -> V_82 . V_485 = false ;
#ifdef F_17
if( V_77 -> V_85 >= V_407 ) {
V_77 -> V_82 . V_188 = 0x53 ;
if( V_77 -> V_85 >= V_448 ) {
V_77 -> V_82 . V_810 = true ;
}
}
#endif
memcpy ( & V_77 -> V_811 , & V_812 , sizeof( V_812 ) ) ;
F_187 ( V_418 , V_77 ) ;
if( ( V_77 -> V_198 = F_95 ( V_77 -> V_85 ) ) ) {
switch( V_77 -> V_198 -> V_790 ) {
#ifdef F_20
case V_424 :
V_77 -> V_85 = V_443 ;
strcpy ( V_77 -> V_388 , L_130 ) ;
break;
#endif
#ifdef F_17
case V_427 :
strcpy ( V_77 -> V_388 , L_131 ) ;
break;
case V_428 :
V_77 -> V_85 = V_447 ;
strcpy ( V_77 -> V_388 , L_132 ) ;
break;
case V_429 :
V_77 -> V_85 = V_448 ;
strcpy ( V_77 -> V_388 , L_133 ) ;
break;
case V_430 :
V_77 -> V_85 = V_449 ;
strcpy ( V_77 -> V_388 , L_134 ) ;
break;
case V_432 :
V_77 -> V_85 = V_408 ;
strcpy ( V_77 -> V_388 , L_135 ) ;
break;
case V_433 :
V_77 -> V_85 = V_409 ;
strcpy ( V_77 -> V_388 , L_136 ) ;
break;
#endif
default:
break;
}
}
V_77 -> V_82 . V_813 = V_77 -> V_85 ;
V_77 -> V_82 . V_77 = ( void * ) V_77 ;
#ifdef F_17
if( ( V_77 -> V_82 . V_813 == V_445 ) ||
( V_77 -> V_82 . V_813 == V_446 ) ) {
V_77 -> V_82 . V_813 = V_444 ;
}
#endif
if( ! V_77 -> V_794 ) {
if( F_188 ( V_418 ) ) {
F_169 ( V_77 -> V_198 ) ;
F_189 ( V_782 ) ;
return - V_748 ;
}
}
V_77 -> V_391 = F_190 ( V_418 , 0 ) ;
V_77 -> V_320 = F_191 ( V_418 , 0 ) ;
V_77 -> V_403 = F_190 ( V_418 , 1 ) ;
V_77 -> V_405 = F_191 ( V_418 , 1 ) ;
V_77 -> V_82 . V_814 = F_190 ( V_418 , 2 ) + 0x30 ;
V_77 -> V_82 . V_815 = V_77 -> V_816 = V_77 -> V_82 . V_814 ;
F_192 ( & V_77 -> V_82 , V_77 -> V_82 . V_815 ) ;
#ifdef F_20
if( V_77 -> V_85 == V_435 ) {
V_33 = 0 ;
do {
if( V_817 [ V_33 ] . V_818 == V_77 -> V_89 &&
V_817 [ V_33 ] . V_819 == V_77 -> V_91 ) {
V_77 -> V_82 . V_808 = true ;
F_4 ( V_58 L_137
L_138 ,
V_817 [ V_33 ] . V_73 ,
V_817 [ V_33 ] . V_74 ) ;
V_77 -> V_199 = F_96 ( V_439 , 0x0008 , NULL ) ;
break;
}
V_33 ++ ;
} while( V_817 [ V_33 ] . V_818 != 0 );
}
#endif
#ifdef F_17
if( ( V_77 -> V_85 == V_408 ) && ( V_77 -> V_198 ) ) {
V_77 -> V_199 = F_193 ( V_77 -> V_198 -> V_795 , ( 2 << 3 ) ) ;
}
#endif
F_55 ( V_153 , 0x05 , 0x86 ) ;
if( ( ! V_77 -> V_794 )
#if ! F_2 ( V_27 ) && ! F_2 ( V_28 )
|| ( V_29 )
#endif
) {
for( V_33 = 0x30 ; V_33 <= 0x3f ; V_33 ++ ) {
F_55 ( V_152 , V_33 , 0x00 ) ;
}
}
V_77 -> V_324 = 0x03 ;
V_156 = F_27 ( V_152 , 0x34 ) ;
if( V_156 & 0x7f ) {
V_77 -> V_324 = V_156 & 0x7f ;
} else if( V_77 -> V_794 ) {
#if F_2 ( V_27 ) || F_2 ( V_28 )
unsigned char T_5 * V_820 = F_153 ( 0x400 , 0x100 ) ;
if( V_820 ) {
V_77 -> V_324 = F_147 ( V_820 + 0x49 ) ;
F_154 ( V_820 ) ;
}
#endif
}
V_77 -> V_514 = NULL ;
V_77 -> V_82 . V_84 = NULL ;
V_77 -> V_82 . V_83 = false ;
V_77 -> V_513 = V_77 -> V_82 . V_821 = false ;
if( V_77 -> V_6 ) {
V_77 -> V_82 . V_84 = F_148 ( V_418 ) ;
V_77 -> V_514 = V_77 -> V_82 . V_84 ;
V_77 -> V_82 . V_83 = ( bool ) ( V_77 -> V_82 . V_84 ) ;
F_4 ( V_72 L_139 ,
V_77 -> V_82 . V_83 ? L_27 : L_140 ) ;
if( ( V_77 -> V_82 . V_83 ) && ( V_77 -> V_85 >= V_411 ) ) {
V_77 -> V_82 . V_83 = false ;
V_77 -> V_513 = V_77 -> V_82 . V_821 = true ;
if( ( V_77 -> V_451 == 2 ) &&
( ! ( V_77 -> V_514 [ 0x1d1 ] & 0x01 ) ) ) {
V_77 -> V_82 . V_485 = true ;
}
}
} else {
F_4 ( V_72 L_141 ) ;
}
if( V_77 -> V_82 . V_94 == V_18 ) {
F_12 ( V_77 ) ;
}
#ifdef F_17
if ( V_77 -> V_85 == V_411 ) {
F_56 ( V_152 , 0x4a , 0x40 ) ;
V_156 = F_27 ( V_152 , 0x48 ) ;
if ( V_156 & 0x02 ) {
V_77 -> V_730 = V_731 ;
F_194 ( & V_418 -> V_775 , L_142 ) ;
}
}
#endif
if( ( ! V_77 -> V_794 )
#if ! F_2 ( V_27 ) && ! F_2 ( V_28 )
|| ( V_29 )
#endif
) {
#ifdef F_20
if( V_77 -> V_122 == V_134 ) {
if( V_77 -> V_85 == V_442 ) {
F_164 ( V_418 ) ;
V_77 -> V_368 = 1 ;
}
}
#endif
#ifdef F_17
if( V_77 -> V_122 == V_131 ) {
int V_489 = 1 ;
if( V_77 -> V_85 == V_411 ) {
V_489 = F_177 ( V_418 ) ;
V_77 -> V_368 = 1 ;
} else if( ( V_77 -> V_85 == V_413 ) && V_77 -> V_513 ) {
V_489 = F_177 ( V_418 ) ;
V_77 -> V_368 = 1 ;
} else {
F_4 ( V_72 L_143
L_144 ) ;
}
if( ! V_489 ) {
F_4 ( V_35 L_145 ) ;
V_164 = - V_822 ;
goto V_823;
}
}
#endif
}
V_77 -> V_369 = 1 ;
if( F_97 ( V_77 ) ) {
F_4 ( V_72 L_146 ) ;
V_164 = - V_822 ;
goto V_823;
}
if( ( V_77 -> V_8 < 0 ) ||
( ( V_37 [ V_77 -> V_8 ] . V_38 [ V_77 -> V_130 ] ) != 0xFF ) ) {
F_56 ( V_153 , V_644 , ( V_824 | V_646 ) ) ;
F_56 ( V_153 , V_645 , V_825 ) ;
}
if( V_14 != 0xff ) {
if( V_77 -> V_122 == V_134 )
V_14 &= 0x3c ;
else
V_14 &= 0x1f ;
V_77 -> V_82 . V_595 = V_14 ;
}
#ifdef F_17
if( V_77 -> V_122 == V_131 ) {
if( V_15 != 0xff )
V_77 -> V_82 . V_599 = V_15 & 0x1f ;
}
#endif
if( ! F_195 ( V_77 -> V_391 , V_77 -> V_320 , L_147 ) ) {
F_4 ( V_35 L_148 ,
( int ) ( V_77 -> V_320 >> 20 ) ) ;
F_4 ( V_35 L_149 ) ;
V_164 = - V_822 ;
goto V_823;
}
if( ! F_195 ( V_77 -> V_403 , V_77 -> V_405 , L_150 ) ) {
F_4 ( V_35 L_151 ) ;
V_164 = - V_822 ;
goto V_826;
}
V_77 -> V_569 = F_156 ( V_77 -> V_391 , V_77 -> V_320 ) ;
V_77 -> V_82 . V_827 = V_77 -> V_569 ;
if( ! V_77 -> V_569 ) {
F_4 ( V_35 L_152 ) ;
V_164 = - V_822 ;
goto V_828;
}
V_77 -> V_564 = F_153 ( V_77 -> V_403 , V_77 -> V_405 ) ;
if( ! V_77 -> V_564 ) {
F_4 ( V_35 L_153 ) ;
V_164 = - V_822 ;
V_829: F_154 ( V_77 -> V_569 ) ;
V_828: F_196 ( V_77 -> V_391 , V_77 -> V_320 ) ;
V_826: F_196 ( V_77 -> V_403 , V_77 -> V_405 ) ;
V_823: F_197 ( V_77 -> V_514 ) ;
F_169 ( V_77 -> V_199 ) ;
F_169 ( V_77 -> V_198 ) ;
if( ! V_77 -> V_794 )
F_198 ( V_418 ) ;
F_189 ( V_782 ) ;
return V_164 ;
}
F_4 ( V_72 L_154 ,
V_77 -> V_391 , ( unsigned long ) V_77 -> V_569 , V_77 -> V_320 / 1024 ) ;
if( V_77 -> V_270 ) {
F_4 ( V_72 L_155 ,
V_77 -> V_270 / 1024 ) ;
}
F_4 ( V_72 L_156 ,
V_77 -> V_403 , ( unsigned long ) V_77 -> V_564 , V_77 -> V_405 / 1024 ) ;
if( V_77 -> V_122 == V_134 ) {
V_77 -> V_328 = V_830 ;
} else {
if( V_77 -> V_85 == V_411 ) {
V_77 -> V_328 = V_831 ;
} else {
V_77 -> V_328 = V_832 ;
}
}
V_77 -> V_833 = V_77 -> V_569
+ V_77 -> V_320
- V_77 -> V_328
- V_77 -> V_603 ;
V_77 -> V_326 |= V_834 ;
if( ( V_77 -> V_641 = F_125 ( V_77 ) ) ) {
F_4 ( V_75 L_157 ) ;
}
V_77 -> V_82 . V_827 += V_77 -> V_270 ;
V_77 -> V_82 . V_729 = V_77 -> V_209 ;
V_77 -> V_119 = 0 ;
V_77 -> V_588 = V_835 ;
V_77 -> V_836 = V_837 ;
V_77 -> V_838 = V_36 ;
V_77 -> V_515 = 0 ;
if( V_77 -> V_85 < V_411 ) {
if( V_77 -> V_514 ) {
V_77 -> V_515 = F_199 ( & V_77 -> V_82 ) ;
}
}
if( ( V_77 -> V_8 < 0 ) ||
( ( V_37 [ V_77 -> V_8 ] . V_38 [ V_77 -> V_130 ] ) != 0xFF ) ) {
F_102 ( V_77 ) ;
F_115 ( V_77 ) ;
if( V_77 -> V_123 & V_150 ) {
F_99 ( V_77 ) ;
}
V_77 -> V_157 = V_77 -> V_119 & ( V_651 | V_839 ) ;
if( V_77 -> V_123 & V_150 ) {
if( V_77 -> V_12 != - 1 ) {
if( ( V_77 -> V_12 == V_120 ) &&
( V_77 -> V_119 & V_120 ) ) {
V_77 -> V_157 |= V_120 ;
} else if( V_77 -> V_12 != V_120 ) {
V_77 -> V_157 |= V_77 -> V_12 ;
}
} else {
if( ( V_77 -> V_122 == V_134 ) &&
( V_77 -> V_123 & V_190 ) ) {
if( V_77 -> V_119 & V_120 )
V_77 -> V_157 |= V_120 ;
else if( V_77 -> V_119 & V_145 )
V_77 -> V_157 |= V_145 ;
else if( V_77 -> V_119 & V_121 )
V_77 -> V_157 |= V_121 ;
} else {
if( V_77 -> V_119 & V_145 )
V_77 -> V_157 |= V_145 ;
else if( V_77 -> V_119 & V_120 )
V_77 -> V_157 |= V_120 ;
else if( V_77 -> V_119 & V_121 )
V_77 -> V_157 |= V_121 ;
}
}
}
if( V_77 -> V_119 & V_120 ) {
F_121 ( V_77 ) ;
}
F_122 ( V_77 ) ;
if( ! V_77 -> V_10 ) {
F_14 ( V_77 , & V_77 -> V_187 , 0 ) ;
} else {
if( ( V_77 -> V_123 & V_484 ) &&
( V_77 -> V_119 & ( V_121 | V_120 ) ) ) {
F_14 ( V_77 , & V_77 -> V_187 , 1 ) ;
}
}
if( V_77 -> V_8 >= 0 ) {
int V_840 = V_77 -> V_8 ;
V_77 -> V_8 = F_19 ( V_77 ,
V_77 -> V_8 , V_77 -> V_157 ) ;
if( V_840 != V_77 -> V_8 ) {
F_4 ( V_35 L_158 ,
V_37 [ V_840 ] . V_42 ,
V_37 [ V_840 ] . V_43 ,
V_37 [ V_840 ] . V_250 ) ;
}
}
if( V_77 -> V_8 < 0 ) {
switch( V_77 -> V_157 & V_138 ) {
case V_120 :
V_77 -> V_8 = V_77 -> V_588 ;
break;
case V_145 :
V_77 -> V_8 = V_77 -> V_836 ;
break;
default:
V_77 -> V_8 = V_77 -> V_838 ;
break;
}
}
V_77 -> V_38 = V_37 [ V_77 -> V_8 ] . V_38 [ V_77 -> V_130 ] ;
if( V_77 -> V_148 != 0 ) {
F_24 ( V_77 , V_77 -> V_148 ,
V_77 -> V_8 ) ;
}
if( V_77 -> V_125 == 0 ) {
V_77 -> V_125 = V_37 [ V_77 -> V_8 ] . V_125 ;
V_77 -> V_148 = 60 ;
}
if( V_77 -> V_187 . V_107 ) {
if( ! F_16 ( V_77 , & V_77 -> V_187 ,
V_77 -> V_8 ,
V_77 -> V_125 ,
V_77 -> V_148 ) ) {
F_4 ( V_72 L_159
L_160 ) ;
}
}
V_77 -> V_203 = V_37 [ V_77 -> V_8 ] . V_250 ;
V_77 -> V_255 = V_37 [ V_77 -> V_8 ] . V_42 ;
V_77 -> V_256 = V_37 [ V_77 -> V_8 ] . V_43 ;
F_51 ( V_77 ) ;
F_4 ( V_72 L_161 ,
V_77 -> V_255 , V_77 -> V_256 , V_77 -> V_203 ,
V_77 -> V_148 ) ;
V_77 -> V_811 . V_42 = V_77 -> V_811 . V_210 = V_77 -> V_255 ;
V_77 -> V_811 . V_43 = V_77 -> V_811 . V_284 = V_77 -> V_256 ;
V_77 -> V_811 . V_202 = V_77 -> V_203 ;
F_57 ( V_77 , & V_77 -> V_811 ) ;
V_77 -> V_811 . V_241 = ( T_2 ) ( 1000000000 /
F_75 ( & V_77 -> V_82 , V_77 -> V_38 , V_77 -> V_125 ) ) ;
if( F_76 ( & V_77 -> V_82 , V_77 -> V_38 ,
V_77 -> V_125 , & V_77 -> V_811 ) ) {
if( ( V_77 -> V_811 . V_214 & V_215 ) == V_249 ) {
V_77 -> V_811 . V_241 <<= 1 ;
}
}
if( V_77 -> V_4 ) {
V_77 -> V_811 . V_284 =
F_52 ( V_77 , & V_77 -> V_811 ) ;
if( V_77 -> V_811 . V_284 < V_77 -> V_811 . V_43 ) {
V_77 -> V_811 . V_284 = V_77 -> V_811 . V_43 ;
}
}
F_53 ( V_77 , & V_77 -> V_811 ) ;
V_77 -> V_207 = 0 ;
if( V_77 -> V_3 ) {
V_77 -> V_207 = - 1 ;
#ifdef F_63
V_77 -> V_811 . V_253 |= V_254 ;
#endif
}
F_200 ( V_77 ) ;
#if F_2 ( V_251 ) && F_2 ( V_252 )
V_782 -> V_62 = V_841 |
V_842 |
V_252 |
V_843 |
V_844 |
( ( V_77 -> V_207 ) ? 0 : V_251 ) ;
#else
V_782 -> V_62 = V_845 ;
#endif
V_782 -> V_201 = V_77 -> V_811 ;
V_782 -> V_279 = V_77 -> V_846 ;
V_782 -> V_847 = V_77 -> V_569 + V_77 -> V_270 ;
V_782 -> V_848 = & V_849 ;
V_782 -> V_277 = V_77 -> V_277 ;
F_201 ( & V_782 -> V_850 , 256 , 0 ) ;
F_4 ( V_58 L_162 , ( int ) V_77 -> V_119 ) ;
V_77 -> V_851 = F_202 ( V_77 -> V_391 ,
V_77 -> V_320 ) ;
if( F_203 ( V_782 ) < 0 ) {
F_4 ( V_35 L_163 ) ;
V_164 = - V_232 ;
F_154 ( V_77 -> V_564 ) ;
goto V_829;
}
V_77 -> V_852 = 1 ;
V_77 -> V_788 = V_786 ;
V_786 = V_77 ;
F_4 ( V_72 L_164 ,
V_77 -> V_3 ? L_165 : L_166 ,
V_77 -> V_4 ?
( V_77 -> V_5 ? L_167 :
L_168 ) :
L_166 ) ;
V_234 ( V_782 , L_169 ,
V_77 -> V_388 , V_311 , V_313 , V_315 ) ;
F_4 ( V_72 L_170 ) ;
}
return 0 ;
}
static void F_204 ( struct V_416 * V_418 )
{
struct V_76 * V_77 = F_135 ( V_418 ) ;
struct V_234 * V_782 = V_77 -> V_785 ;
int V_852 = V_77 -> V_852 ;
int V_322 = V_77 -> V_322 ;
F_154 ( V_77 -> V_564 ) ;
F_154 ( V_77 -> V_569 ) ;
F_196 ( V_77 -> V_391 , V_77 -> V_320 ) ;
F_196 ( V_77 -> V_403 , V_77 -> V_405 ) ;
F_197 ( V_77 -> V_514 ) ;
F_169 ( V_77 -> V_199 ) ;
F_169 ( V_77 -> V_198 ) ;
F_205 ( V_77 -> V_851 ) ;
if( ! V_77 -> V_794 )
F_198 ( V_418 ) ;
if( V_77 -> V_852 ) {
F_206 ( V_782 ) ;
F_189 ( V_782 ) ;
}
if( V_852 && V_322 )
F_4 ( V_72
L_171 ) ;
}
static int T_1 F_207 ( void )
{
#ifndef F_186
char * V_693 = NULL ;
if( F_208 ( L_172 , & V_693 ) )
return - V_822 ;
F_143 ( V_693 ) ;
#endif
return F_209 ( & V_853 ) ;
}
static int T_1 F_210 ( void )
{
F_1 () ;
if( V_45 )
V_9 = V_45 ;
if( ( V_854 == 0 ) || ( V_854 == 1 ) )
V_16 = V_854 ^ 1 ;
if( V_855 )
F_9 ( V_855 ) ;
if( V_856 )
F_10 ( V_856 ) ;
if( V_857 )
F_5 ( V_857 , false ) ;
else if( V_858 != - 1 )
F_3 ( V_858 , false ) ;
V_10 = ( V_859 == 0 ) ? 1 : 0 ;
V_11 = V_860 ;
if( V_860 == 1 )
V_10 = 0 ;
else if( V_860 == 0 )
V_10 = 1 ;
if( V_861 == 1 )
V_3 = 0 ;
else if( V_861 == 0 )
V_3 = 1 ;
if( V_862 == 1 )
V_4 = 0 ;
else if( V_862 == 0 )
V_4 = 1 ;
if( V_863 == 1 )
V_5 = 0 ;
else if( V_863 == 0 )
V_5 = 1 ;
if( V_864 )
V_2 = V_864 ;
if( V_865 != - 1 )
V_6 = V_865 ;
if( V_866 != - 1 )
V_7 = V_866 ;
if( V_598 != - 1 )
V_14 = ( V_598 & 0x7f ) ;
if( V_867 != - 1 )
V_15 = ( V_867 & 0x1f ) ;
V_26 = V_868 ;
if( V_869 )
F_11 ( V_869 ) ;
if( ( V_870 >= 0 ) && ( V_870 <= 3 ) )
V_19 = V_870 ;
V_24 = V_871 ;
V_25 = V_872 ;
#if ! F_2 ( V_27 ) && ! F_2 ( V_28 )
V_29 = ( V_873 ) ? 1 : 0 ;
if( V_874 )
V_30 = V_874 ;
#endif
return F_207 () ;
}
static void T_7 F_211 ( void )
{
F_212 ( & V_853 ) ;
F_4 ( V_58 L_173 ) ;
}
