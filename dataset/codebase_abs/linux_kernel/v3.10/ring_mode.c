static unsigned int F_1 ( void * V_1 , struct V_2 * V_3 , int V_4 )
{
struct V_5 * V_6 = (struct V_5 * ) V_1 ;
unsigned int V_7 = V_6 -> V_8 ;
unsigned int V_9 = V_6 -> V_10 % V_7 ;
struct V_11 * V_12 = V_6 -> V_13 + V_9 ;
unsigned int V_14 = F_2 ( V_3 ) ;
unsigned int V_15 , V_16 ;
if ( V_6 -> V_17 -> V_18 )
V_15 = V_19 ;
else
V_15 = V_20 ;
V_16 = V_14 - V_15 ;
if ( V_14 > V_19 ) {
V_12 -> V_21 = F_3 ( V_6 -> V_22 , V_3 -> V_23 ,
V_15 , V_24 ) ;
V_6 -> V_25 [ V_9 ] = V_12 -> V_21 ;
V_12 -> V_26 = V_12 -> V_21 + V_27 ;
V_6 -> V_28 -> V_12 -> V_29 ( V_12 , 1 , V_15 , V_4 ,
V_30 ) ;
F_4 () ;
V_9 = ( ++ V_6 -> V_10 ) % V_7 ;
V_12 = V_6 -> V_13 + V_9 ;
V_12 -> V_21 = F_3 ( V_6 -> V_22 , V_3 -> V_23 + V_15 ,
V_16 , V_24 ) ;
V_6 -> V_25 [ V_9 ] = V_12 -> V_21 ;
V_12 -> V_26 = V_12 -> V_21 + V_27 ;
V_6 -> V_28 -> V_12 -> V_29 ( V_12 , 0 , V_16 , V_4 ,
V_30 ) ;
F_4 () ;
V_6 -> V_28 -> V_12 -> V_31 ( V_12 ) ;
V_6 -> V_32 [ V_9 ] = NULL ;
} else {
V_12 -> V_21 = F_3 ( V_6 -> V_22 , V_3 -> V_23 ,
V_14 , V_24 ) ;
V_6 -> V_25 [ V_9 ] = V_12 -> V_21 ;
V_12 -> V_26 = V_12 -> V_21 + V_27 ;
V_6 -> V_28 -> V_12 -> V_29 ( V_12 , 1 , V_14 , V_4 ,
V_30 ) ;
}
return V_9 ;
}
static unsigned int F_5 ( int V_16 , int V_18 )
{
unsigned int V_33 = 0 ;
if ( V_16 >= V_27 )
V_33 = 1 ;
return V_33 ;
}
static void F_6 ( void * V_34 , struct V_11 * V_1 )
{
struct V_5 * V_6 = (struct V_5 * ) V_34 ;
if ( F_7 ( V_6 -> V_17 -> V_35 ) )
if ( V_6 -> V_36 >= V_19 )
V_1 -> V_26 = V_1 -> V_21 + V_19 ;
}
static void F_8 ( struct V_11 * V_1 )
{
V_1 -> V_26 = V_1 -> V_21 + V_19 ;
}
static void F_9 ( void * V_34 , struct V_11 * V_1 )
{
if ( F_7 ( V_1 -> V_26 ) )
V_1 -> V_26 = 0 ;
}
static int F_10 ( int V_37 )
{
int V_33 = 0 ;
if ( F_7 ( V_37 >= V_19 ) )
V_33 = V_38 ;
return V_33 ;
}
