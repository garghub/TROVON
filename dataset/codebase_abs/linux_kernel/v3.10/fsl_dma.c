static void F_1 ( struct V_1 * V_2 )
{
unsigned long V_3 ;
F_2 ( V_2 , V_3 ) ;
if ( F_3 ( V_2 ) )
F_4 ( V_2 , V_4 ) ;
F_5 ( V_2 , V_3 ) ;
}
static void F_6 ( struct V_5 * V_6 )
{
struct V_7 * V_8 =
& V_6 -> V_8 [ V_6 -> V_9 ] ;
if ( V_6 -> V_2 -> V_10 == V_11 ) {
V_8 -> V_12 = F_7 ( V_6 -> V_13 ) ;
#ifdef F_8
V_8 -> V_14 = F_7 ( V_15 |
F_9 ( V_6 -> V_13 ) ) ;
#endif
} else {
V_8 -> V_16 = F_7 ( V_6 -> V_13 ) ;
#ifdef F_8
V_8 -> V_17 = F_7 ( V_15 |
F_9 ( V_6 -> V_13 ) ) ;
#endif
}
V_6 -> V_13 += V_6 -> V_18 ;
if ( V_6 -> V_13 >= V_6 -> V_19 )
V_6 -> V_13 = V_6 -> V_20 ;
if ( ++ V_6 -> V_9 >= V_21 )
V_6 -> V_9 = 0 ;
}
static T_1 F_10 ( int V_22 , void * V_23 )
{
struct V_5 * V_6 = V_23 ;
struct V_1 * V_2 = V_6 -> V_2 ;
struct V_24 * V_25 = V_2 -> V_26 ;
struct V_27 * V_28 = V_25 -> V_29 -> V_28 ;
struct V_30 T_2 * V_31 = V_6 -> V_31 ;
T_1 V_32 = V_33 ;
T_3 V_34 , V_35 = 0 ;
V_34 = F_11 ( & V_31 -> V_34 ) ;
if ( V_34 & V_36 ) {
F_12 ( V_28 , L_1 ) ;
F_1 ( V_2 ) ;
V_35 |= V_36 ;
V_32 = V_37 ;
}
if ( V_34 & V_38 )
V_32 = V_37 ;
if ( V_34 & V_39 ) {
F_12 ( V_28 , L_2 ) ;
F_1 ( V_2 ) ;
V_35 |= V_39 ;
V_32 = V_37 ;
}
if ( V_34 & V_40 ) {
V_35 |= V_40 ;
V_32 = V_37 ;
}
if ( V_34 & V_41 )
V_32 = V_37 ;
if ( V_34 & V_42 ) {
F_13 ( V_2 ) ;
if ( V_6 -> V_43 != V_21 )
F_6 ( V_6 ) ;
V_35 |= V_42 ;
V_32 = V_37 ;
}
if ( V_34 & V_44 ) {
V_35 |= V_44 ;
V_32 = V_37 ;
}
if ( V_35 )
F_14 ( & V_31 -> V_34 , V_35 ) ;
return V_32 ;
}
static int F_15 ( struct V_24 * V_25 )
{
struct V_45 * V_46 = V_25 -> V_46 -> V_45 ;
struct V_47 * V_48 = V_25 -> V_48 ;
static T_4 V_49 = F_16 ( 36 ) ;
int V_32 ;
if ( ! V_46 -> V_28 -> V_50 )
V_46 -> V_28 -> V_50 = & V_49 ;
if ( ! V_46 -> V_28 -> V_51 )
V_46 -> V_28 -> V_51 = V_49 ;
if ( V_48 -> V_52 [ V_11 ] . V_2 ) {
V_32 = F_17 ( V_53 , V_46 -> V_28 ,
V_54 . V_55 ,
& V_48 -> V_52 [ V_11 ] . V_2 -> V_56 ) ;
if ( V_32 ) {
F_12 ( V_46 -> V_28 , L_3 ) ;
return V_32 ;
}
}
if ( V_48 -> V_52 [ V_57 ] . V_2 ) {
V_32 = F_17 ( V_53 , V_46 -> V_28 ,
V_54 . V_55 ,
& V_48 -> V_52 [ V_57 ] . V_2 -> V_56 ) ;
if ( V_32 ) {
F_12 ( V_46 -> V_28 , L_4 ) ;
F_18 ( & V_48 -> V_52 [ V_11 ] . V_2 -> V_56 ) ;
return V_32 ;
}
}
return 0 ;
}
static int F_19 ( struct V_1 * V_2 )
{
struct V_58 * V_59 = V_2 -> V_59 ;
struct V_24 * V_25 = V_2 -> V_26 ;
struct V_27 * V_28 = V_25 -> V_29 -> V_28 ;
struct V_60 * V_61 =
F_20 ( V_25 -> V_29 -> V_62 , struct V_60 , V_63 ) ;
struct V_5 * V_6 ;
struct V_30 T_2 * V_31 ;
T_5 V_64 ;
T_4 V_65 ;
T_3 V_66 ;
unsigned int V_67 ;
int V_32 = 0 ;
unsigned int V_68 ;
V_32 = F_21 ( V_59 ,
V_69 ) ;
if ( V_32 < 0 ) {
F_12 ( V_28 , L_5 ) ;
return V_32 ;
}
V_67 = V_2 -> V_10 == V_11 ? 0 : 1 ;
if ( V_61 -> V_70 ) {
F_12 ( V_28 , L_6 ) ;
return - V_71 ;
}
V_6 = F_22 ( V_28 , sizeof( struct V_5 ) ,
& V_64 , V_72 ) ;
if ( ! V_6 ) {
F_12 ( V_28 , L_7 ) ;
return - V_73 ;
}
if ( V_2 -> V_10 == V_11 )
V_6 -> V_74 = V_61 -> V_75 ;
else
V_6 -> V_74 = V_61 -> V_76 ;
V_6 -> V_77 = V_61 -> V_77 ;
V_6 -> V_31 = V_61 -> V_67 ;
V_6 -> V_22 = V_61 -> V_22 ;
V_6 -> V_2 = V_2 ;
V_6 -> V_64 = V_64 ;
V_6 -> V_20 = V_2 -> V_56 . V_78 ;
V_32 = F_23 ( V_6 -> V_22 , F_10 , 0 , L_8 ,
V_6 ) ;
if ( V_32 ) {
F_12 ( V_28 , L_9 ,
V_6 -> V_22 , V_32 ) ;
F_24 ( V_28 , sizeof( struct V_5 ) ,
V_6 , V_6 -> V_64 ) ;
return V_32 ;
}
V_61 -> V_70 = 1 ;
F_25 ( V_2 , & V_2 -> V_56 ) ;
F_26 ( V_2 , & V_54 ) ;
V_59 -> V_26 = V_6 ;
V_31 = V_6 -> V_31 ;
V_65 = V_6 -> V_64 +
sizeof( struct V_7 ) ;
for ( V_68 = 0 ; V_68 < V_21 ; V_68 ++ ) {
V_6 -> V_8 [ V_68 ] . V_79 = F_27 ( V_65 ) ;
V_65 += sizeof( struct V_7 ) ;
}
V_6 -> V_8 [ V_68 - 1 ] . V_79 = F_27 ( V_6 -> V_64 ) ;
F_14 ( & V_31 -> V_80 ,
F_28 ( V_6 -> V_64 ) ) ;
F_14 ( & V_31 -> V_81 ,
F_29 ( V_6 -> V_64 ) ) ;
F_14 ( & V_31 -> V_82 , 0 ) ;
V_66 = F_11 ( & V_31 -> V_66 ) &
~ ( V_83 | V_84 | V_85 ) ;
V_66 |= V_86 | V_87 | V_88 |
V_89 ;
V_66 |= ( V_2 -> V_10 == V_11 ) ?
V_84 : V_85 ;
F_14 ( & V_31 -> V_66 , V_66 ) ;
return 0 ;
}
static int F_30 ( struct V_1 * V_2 ,
struct V_90 * V_91 )
{
struct V_58 * V_59 = V_2 -> V_59 ;
struct V_5 * V_6 = V_59 -> V_26 ;
struct V_24 * V_25 = V_2 -> V_26 ;
struct V_27 * V_28 = V_25 -> V_29 -> V_28 ;
unsigned int V_92 =
F_31 ( F_32 ( V_91 ) ) ;
unsigned int V_93 = V_92 / 8 ;
T_5 V_74 = V_6 -> V_74 ;
T_6 V_94 = F_33 ( V_91 ) ;
T_6 V_18 = F_34 ( V_91 ) ;
T_5 V_95 = V_2 -> V_56 . V_78 ;
struct V_30 T_2 * V_31 = V_6 -> V_31 ;
T_3 V_66 ;
unsigned int V_68 ;
V_6 -> V_18 = V_18 ;
V_6 -> V_43 = F_35 ( V_91 ) ;
V_6 -> V_19 = V_6 -> V_20 + V_94 ;
V_6 -> V_13 = V_6 -> V_20 +
( V_21 * V_18 ) ;
if ( V_6 -> V_13 >= V_6 -> V_19 )
V_6 -> V_13 = V_6 -> V_20 ;
V_66 = F_11 ( & V_31 -> V_66 ) & ~ ( V_96 |
V_97 | V_98 ) ;
switch ( V_92 ) {
case 8 :
V_66 |= V_99 | V_100 ;
V_74 += 3 ;
break;
case 16 :
V_66 |= V_101 | V_102 ;
V_74 += 2 ;
break;
case 32 :
V_66 |= V_103 | V_104 ;
break;
default:
F_12 ( V_28 , L_10 , V_92 ) ;
return - V_105 ;
}
V_66 |= F_36 ( ( V_6 -> V_77 - 2 ) * V_93 ) ;
F_14 ( & V_31 -> V_66 , V_66 ) ;
for ( V_68 = 0 ; V_68 < V_21 ; V_68 ++ ) {
struct V_7 * V_8 = & V_6 -> V_8 [ V_68 ] ;
V_8 -> V_106 = F_7 ( V_18 ) ;
if ( V_2 -> V_10 == V_11 ) {
V_8 -> V_12 = F_7 ( V_95 ) ;
V_8 -> V_14 = F_7 ( V_15 |
F_9 ( V_95 ) ) ;
V_8 -> V_16 = F_7 ( V_74 ) ;
V_8 -> V_17 = F_7 ( V_107 |
F_9 ( V_74 ) ) ;
} else {
V_8 -> V_12 = F_7 ( V_74 ) ;
V_8 -> V_14 = F_7 ( V_107 |
F_9 ( V_74 ) ) ;
V_8 -> V_16 = F_7 ( V_95 ) ;
V_8 -> V_17 = F_7 ( V_15 |
F_9 ( V_95 ) ) ;
}
V_95 += V_18 ;
}
return 0 ;
}
static T_7 F_37 ( struct V_1 * V_2 )
{
struct V_58 * V_59 = V_2 -> V_59 ;
struct V_5 * V_6 = V_59 -> V_26 ;
struct V_24 * V_25 = V_2 -> V_26 ;
struct V_27 * V_28 = V_25 -> V_29 -> V_28 ;
struct V_30 T_2 * V_31 = V_6 -> V_31 ;
T_5 V_108 ;
T_7 V_109 ;
if ( V_2 -> V_10 == V_11 ) {
V_108 = F_11 ( & V_31 -> V_110 ) ;
#ifdef F_8
V_108 |= ( T_4 ) ( F_11 ( & V_31 -> V_111 ) &
V_112 ) << 32 ;
#endif
} else {
V_108 = F_11 ( & V_31 -> V_113 ) ;
#ifdef F_8
V_108 |= ( T_4 ) ( F_11 ( & V_31 -> V_114 ) &
V_112 ) << 32 ;
#endif
}
if ( ! V_108 )
return 0 ;
if ( ( V_108 < V_6 -> V_20 ) ||
( V_108 > V_6 -> V_19 ) ) {
F_12 ( V_28 , L_11 ) ;
return V_115 ;
}
V_109 = F_38 ( V_59 , V_108 - V_6 -> V_20 ) ;
if ( V_109 == V_59 -> V_94 )
V_109 = 0 ;
return V_109 ;
}
static int F_39 ( struct V_1 * V_2 )
{
struct V_58 * V_59 = V_2 -> V_59 ;
struct V_5 * V_6 = V_59 -> V_26 ;
if ( V_6 ) {
struct V_30 T_2 * V_31 ;
V_31 = V_6 -> V_31 ;
F_14 ( & V_31 -> V_66 , V_83 ) ;
F_14 ( & V_31 -> V_66 , 0 ) ;
F_14 ( & V_31 -> V_34 , - 1 ) ;
F_14 ( & V_31 -> V_80 , 0 ) ;
F_14 ( & V_31 -> V_81 , 0 ) ;
F_14 ( & V_31 -> V_111 , 0 ) ;
F_14 ( & V_31 -> V_110 , 0 ) ;
F_14 ( & V_31 -> V_114 , 0 ) ;
F_14 ( & V_31 -> V_113 , 0 ) ;
F_14 ( & V_31 -> V_82 , 0 ) ;
F_14 ( & V_31 -> V_116 , 0 ) ;
F_14 ( & V_31 -> V_117 , 0 ) ;
}
return 0 ;
}
static int F_40 ( struct V_1 * V_2 )
{
struct V_58 * V_59 = V_2 -> V_59 ;
struct V_5 * V_6 = V_59 -> V_26 ;
struct V_24 * V_25 = V_2 -> V_26 ;
struct V_27 * V_28 = V_25 -> V_29 -> V_28 ;
struct V_60 * V_61 =
F_20 ( V_25 -> V_29 -> V_62 , struct V_60 , V_63 ) ;
if ( V_6 ) {
if ( V_6 -> V_22 )
F_41 ( V_6 -> V_22 , V_6 ) ;
F_24 ( V_28 , sizeof( struct V_5 ) ,
V_6 , V_6 -> V_64 ) ;
V_2 -> V_59 -> V_26 = NULL ;
}
V_61 -> V_70 = 0 ;
return 0 ;
}
static void F_42 ( struct V_47 * V_48 )
{
struct V_1 * V_2 ;
unsigned int V_68 ;
for ( V_68 = 0 ; V_68 < F_43 ( V_48 -> V_52 ) ; V_68 ++ ) {
V_2 = V_48 -> V_52 [ V_68 ] . V_2 ;
if ( V_2 ) {
F_18 ( & V_2 -> V_56 ) ;
V_2 -> V_56 . V_118 = NULL ;
V_2 -> V_56 . V_78 = 0 ;
}
}
}
static struct V_119 * F_44 ( struct V_119 * V_120 )
{
struct V_119 * V_121 , * V_122 ;
F_45 (ssi_np, NULL, L_12 ) {
V_122 = F_46 ( V_121 , L_13 , 0 ) ;
F_47 ( V_122 ) ;
if ( V_122 == V_120 )
return V_121 ;
V_122 = F_46 ( V_121 , L_14 , 0 ) ;
F_47 ( V_122 ) ;
if ( V_122 == V_120 )
return V_121 ;
}
return NULL ;
}
static int F_48 ( struct V_123 * V_124 )
{
struct V_60 * V_61 ;
struct V_119 * V_122 = V_124 -> V_28 . V_125 ;
struct V_119 * V_121 ;
struct V_126 V_127 ;
const T_8 * V_128 ;
int V_32 ;
V_121 = F_44 ( V_122 ) ;
if ( ! V_121 ) {
F_12 ( & V_124 -> V_28 , L_15 ) ;
return - V_129 ;
}
V_32 = F_49 ( V_121 , 0 , & V_127 ) ;
if ( V_32 ) {
F_12 ( & V_124 -> V_28 , L_16 ,
V_121 -> V_130 ) ;
F_47 ( V_121 ) ;
return V_32 ;
}
V_61 = F_50 ( sizeof( * V_61 ) + strlen ( V_122 -> V_130 ) , V_72 ) ;
if ( ! V_61 ) {
F_12 ( & V_124 -> V_28 , L_17 ) ;
F_47 ( V_121 ) ;
return - V_73 ;
}
strcpy ( V_61 -> V_131 , V_122 -> V_130 ) ;
V_61 -> V_63 . V_132 = & V_133 ;
V_61 -> V_63 . V_134 = F_15 ;
V_61 -> V_63 . V_135 = F_42 ;
V_61 -> V_75 = V_127 . V_136 + F_51 ( struct V_137 , V_138 ) ;
V_61 -> V_76 = V_127 . V_136 + F_51 ( struct V_137 , V_139 ) ;
V_128 = F_52 ( V_121 , L_18 , NULL ) ;
if ( V_128 )
V_61 -> V_77 = F_53 ( V_128 ) ;
else
V_61 -> V_77 = 8 ;
F_47 ( V_121 ) ;
V_32 = F_54 ( & V_124 -> V_28 , & V_61 -> V_63 ) ;
if ( V_32 ) {
F_12 ( & V_124 -> V_28 , L_19 ) ;
F_55 ( V_61 ) ;
return V_32 ;
}
V_61 -> V_67 = F_56 ( V_122 , 0 ) ;
V_61 -> V_22 = F_57 ( V_122 , 0 ) ;
F_58 ( & V_124 -> V_28 , V_61 ) ;
return 0 ;
}
static int F_59 ( struct V_123 * V_124 )
{
struct V_60 * V_61 = F_60 ( & V_124 -> V_28 ) ;
F_61 ( & V_124 -> V_28 ) ;
F_62 ( V_61 -> V_67 ) ;
F_63 ( V_61 -> V_22 ) ;
F_55 ( V_61 ) ;
return 0 ;
}
