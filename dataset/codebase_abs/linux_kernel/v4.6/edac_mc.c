unsigned F_1 ( struct V_1 * V_2 , char * V_3 ,
unsigned V_4 )
{
struct V_5 * V_6 = V_2 -> V_6 ;
int V_7 , V_8 , V_9 = 0 ;
char * V_10 = V_3 ;
for ( V_7 = 0 ; V_7 < V_6 -> V_11 ; V_7 ++ ) {
V_8 = snprintf ( V_10 , V_4 , L_1 ,
V_12 [ V_6 -> V_13 [ V_7 ] . type ] ,
V_2 -> V_14 [ V_7 ] ) ;
V_10 += V_8 ;
V_4 -= V_8 ;
V_9 += V_8 ;
if ( ! V_4 )
break;
}
return V_9 ;
}
static void F_2 ( struct V_15 * V_16 )
{
F_3 ( 4 , L_2 , V_16 -> V_17 ) ;
F_3 ( 4 , L_3 , V_16 ) ;
F_3 ( 4 , L_4 , V_16 -> V_18 ) ;
F_3 ( 4 , L_5 , V_16 -> V_2 ) ;
}
static void F_4 ( struct V_1 * V_2 , int V_19 )
{
char V_14 [ 80 ] ;
F_1 ( V_2 , V_14 , sizeof( V_14 ) ) ;
F_3 ( 4 , L_6 ,
V_2 -> V_6 -> V_20 ? L_7 : L_8 ,
V_19 , V_14 , V_2 -> V_18 , V_2 -> V_21 ) ;
F_3 ( 4 , L_9 , V_2 ) ;
F_3 ( 4 , L_10 , V_2 -> V_22 ) ;
F_3 ( 4 , L_11 , V_2 -> V_23 ) ;
F_3 ( 4 , L_12 , V_2 -> V_24 ) ;
F_3 ( 4 , L_11 , V_2 -> V_23 ) ;
}
static void F_5 ( struct V_25 * V_18 )
{
F_3 ( 4 , L_13 , V_18 -> V_26 ) ;
F_3 ( 4 , L_14 , V_18 ) ;
F_3 ( 4 , L_15 , V_18 -> V_27 ) ;
F_3 ( 4 , L_16 , V_18 -> V_28 ) ;
F_3 ( 4 , L_17 , V_18 -> V_29 ) ;
F_3 ( 4 , L_18 , V_18 -> V_30 ) ;
F_3 ( 4 , L_19 , V_18 -> V_31 ) ;
F_3 ( 4 , L_20 , V_18 -> V_6 ) ;
}
static void F_6 ( struct V_5 * V_6 )
{
F_3 ( 3 , L_21 , V_6 ) ;
F_3 ( 3 , L_22 , V_6 -> V_32 ) ;
F_3 ( 3 , L_23 , V_6 -> V_33 ) ;
F_3 ( 3 , L_24 , V_6 -> V_34 ) ;
F_3 ( 4 , L_25 , V_6 -> V_35 ) ;
F_3 ( 3 , L_26 ,
V_6 -> V_36 , V_6 -> V_37 ) ;
F_3 ( 3 , L_27 ,
V_6 -> V_38 , V_6 -> V_39 ) ;
F_3 ( 3 , L_28 , V_6 -> V_40 ) ;
F_3 ( 3 , L_29 ,
V_6 -> V_41 , V_6 -> V_42 ) ;
F_3 ( 3 , L_30 , V_6 -> V_43 ) ;
}
void * F_7 ( void * * V_10 , unsigned V_44 , int V_45 )
{
unsigned V_46 , V_47 ;
void * V_48 = * V_10 ;
* V_10 += V_44 * V_45 ;
if ( V_44 > sizeof( long ) )
V_46 = sizeof( long long ) ;
else if ( V_44 > sizeof( int ) )
V_46 = sizeof( long ) ;
else if ( V_44 > sizeof( short ) )
V_46 = sizeof( int ) ;
else if ( V_44 > sizeof( char ) )
V_46 = sizeof( short ) ;
else
return ( char * ) V_48 ;
V_47 = ( unsigned long ) V_10 % V_46 ;
if ( V_47 == 0 )
return ( char * ) V_48 ;
* V_10 += V_46 - V_47 ;
return ( void * ) ( ( ( unsigned long ) V_48 ) + V_46 - V_47 ) ;
}
static void F_8 ( struct V_5 * V_6 )
{
int V_7 , V_49 , V_50 ;
struct V_25 * V_51 ;
const unsigned int V_38 = V_6 -> V_38 ;
const unsigned int V_52 = V_6 -> V_53 ;
const unsigned int V_54 = V_6 -> V_36 ;
if ( V_6 -> V_39 ) {
for ( V_7 = 0 ; V_7 < V_38 ; V_7 ++ )
F_9 ( V_6 -> V_39 [ V_7 ] ) ;
F_9 ( V_6 -> V_39 ) ;
}
if ( V_6 -> V_37 ) {
for ( V_50 = 0 ; V_50 < V_54 ; V_50 ++ ) {
V_51 = V_6 -> V_37 [ V_50 ] ;
if ( V_51 ) {
if ( V_51 -> V_31 ) {
for ( V_49 = 0 ; V_49 < V_52 ; V_49 ++ )
F_9 ( V_51 -> V_31 [ V_49 ] ) ;
F_9 ( V_51 -> V_31 ) ;
}
F_9 ( V_51 ) ;
}
}
F_9 ( V_6 -> V_37 ) ;
}
F_9 ( V_6 ) ;
}
struct V_5 * F_10 ( unsigned V_55 ,
unsigned V_11 ,
struct V_56 * V_13 ,
unsigned V_57 )
{
struct V_5 * V_6 ;
struct V_56 * V_58 ;
struct V_25 * V_51 ;
struct V_15 * V_16 ;
struct V_1 * V_2 ;
T_1 * V_59 [ V_60 ] , * V_61 [ V_60 ] ;
unsigned V_62 [ V_60 ] ;
unsigned V_44 , V_38 = 1 , V_9 = 1 ;
unsigned V_54 = 1 , V_52 = 1 , V_63 = 0 ;
void * V_64 , * V_10 , * V_48 = NULL ;
int V_7 , V_65 , V_50 , V_49 , V_8 , V_4 , V_66 ;
bool V_67 = false ;
F_11 ( V_11 > V_60 || V_11 == 0 ) ;
for ( V_7 = 0 ; V_7 < V_11 ; V_7 ++ ) {
V_38 *= V_13 [ V_7 ] . V_44 ;
if ( V_13 [ V_7 ] . V_68 )
V_54 *= V_13 [ V_7 ] . V_44 ;
else
V_52 *= V_13 [ V_7 ] . V_44 ;
if ( V_13 [ V_7 ] . type == V_69 )
V_67 = true ;
}
V_6 = F_7 ( & V_48 , sizeof( * V_6 ) , 1 ) ;
V_58 = F_7 ( & V_48 , sizeof( * V_58 ) , V_11 ) ;
for ( V_7 = 0 ; V_7 < V_11 ; V_7 ++ ) {
V_9 *= V_13 [ V_7 ] . V_44 ;
F_3 ( 4 , L_31 , V_7 , V_9 ) ;
V_59 [ V_7 ] = F_7 ( & V_48 , sizeof( T_1 ) , V_9 ) ;
V_61 [ V_7 ] = F_7 ( & V_48 , sizeof( T_1 ) , V_9 ) ;
V_63 += 2 * V_9 ;
}
F_3 ( 4 , L_32 , V_63 ) ;
V_64 = F_7 ( & V_48 , V_57 , 1 ) ;
V_44 = ( ( unsigned long ) V_64 ) + V_57 ;
F_3 ( 1 , L_33 ,
V_44 ,
V_38 ,
V_67 ? L_34 : L_35 ,
V_54 * V_52 ) ;
V_6 = F_12 ( V_44 , V_70 ) ;
if ( V_6 == NULL )
return NULL ;
V_58 = (struct V_56 * ) ( ( ( char * ) V_6 ) + ( ( unsigned long ) V_58 ) ) ;
for ( V_7 = 0 ; V_7 < V_11 ; V_7 ++ ) {
V_6 -> V_59 [ V_7 ] = ( T_1 * ) ( ( char * ) V_6 + ( ( unsigned long ) V_59 [ V_7 ] ) ) ;
V_6 -> V_61 [ V_7 ] = ( T_1 * ) ( ( char * ) V_6 + ( ( unsigned long ) V_61 [ V_7 ] ) ) ;
}
V_64 = V_57 ? ( ( ( char * ) V_6 ) + ( ( unsigned long ) V_64 ) ) : NULL ;
V_6 -> V_71 = V_55 ;
V_6 -> V_38 = V_38 ;
V_6 -> V_43 = V_64 ;
V_6 -> V_11 = V_11 ;
V_6 -> V_13 = V_58 ;
memcpy ( V_6 -> V_13 , V_13 , sizeof( * V_58 ) * V_11 ) ;
V_6 -> V_36 = V_54 ;
V_6 -> V_53 = V_52 ;
V_6 -> V_20 = V_67 ;
V_6 -> V_37 = F_13 ( V_54 , sizeof( * V_6 -> V_37 ) , V_70 ) ;
if ( ! V_6 -> V_37 )
goto error;
for ( V_50 = 0 ; V_50 < V_54 ; V_50 ++ ) {
V_51 = F_12 ( sizeof( * * V_6 -> V_37 ) , V_70 ) ;
if ( ! V_51 )
goto error;
V_6 -> V_37 [ V_50 ] = V_51 ;
V_51 -> V_26 = V_50 ;
V_51 -> V_6 = V_6 ;
V_51 -> V_30 = V_52 ;
V_51 -> V_31 = F_13 ( V_52 , sizeof( * V_51 -> V_31 ) ,
V_70 ) ;
if ( ! V_51 -> V_31 )
goto error;
for ( V_49 = 0 ; V_49 < V_52 ; V_49 ++ ) {
V_16 = F_12 ( sizeof( * * V_51 -> V_31 ) , V_70 ) ;
if ( ! V_16 )
goto error;
V_51 -> V_31 [ V_49 ] = V_16 ;
V_16 -> V_17 = V_49 ;
V_16 -> V_18 = V_51 ;
}
}
V_6 -> V_39 = F_13 ( V_38 , sizeof( * V_6 -> V_39 ) , V_70 ) ;
if ( ! V_6 -> V_39 )
goto error;
memset ( & V_62 , 0 , sizeof( V_62 ) ) ;
V_50 = 0 ;
V_49 = 0 ;
for ( V_7 = 0 ; V_7 < V_38 ; V_7 ++ ) {
V_16 = V_6 -> V_37 [ V_50 ] -> V_31 [ V_49 ] ;
V_66 = F_14 ( V_58 , V_11 , V_62 [ 0 ] , V_62 [ 1 ] , V_62 [ 2 ] ) ;
if ( V_66 < 0 || V_66 >= V_38 ) {
F_15 ( V_6 , V_72 , L_36 ) ;
goto error;
}
V_2 = F_12 ( sizeof( * * V_6 -> V_39 ) , V_70 ) ;
if ( ! V_2 )
goto error;
V_6 -> V_39 [ V_66 ] = V_2 ;
V_2 -> V_6 = V_6 ;
V_4 = sizeof( V_2 -> V_22 ) ;
V_10 = V_2 -> V_22 ;
V_8 = snprintf ( V_10 , V_4 , L_37 , V_55 ) ;
V_10 += V_8 ;
V_4 -= V_8 ;
for ( V_65 = 0 ; V_65 < V_11 ; V_65 ++ ) {
V_8 = snprintf ( V_10 , V_4 , L_38 ,
V_12 [ V_13 [ V_65 ] . type ] ,
V_62 [ V_65 ] ) ;
V_10 += V_8 ;
V_4 -= V_8 ;
V_2 -> V_14 [ V_65 ] = V_62 [ V_65 ] ;
if ( V_4 <= 0 )
break;
}
V_16 -> V_2 = V_2 ;
V_2 -> V_18 = V_50 ;
V_2 -> V_21 = V_49 ;
if ( V_13 [ 0 ] . V_68 ) {
V_49 ++ ;
if ( V_49 == V_52 ) {
V_49 = 0 ;
V_50 ++ ;
}
} else {
V_50 ++ ;
if ( V_50 == V_54 ) {
V_50 = 0 ;
V_49 ++ ;
}
}
for ( V_65 = V_11 - 1 ; V_65 >= 0 ; V_65 -- ) {
V_62 [ V_65 ] ++ ;
if ( V_62 [ V_65 ] < V_13 [ V_65 ] . V_44 )
break;
V_62 [ V_65 ] = 0 ;
}
}
V_6 -> V_73 = V_74 ;
return V_6 ;
error:
F_8 ( V_6 ) ;
return NULL ;
}
void F_16 ( struct V_5 * V_6 )
{
F_3 ( 1 , L_39 ) ;
if ( ! F_17 ( & V_6 -> V_75 ) ) {
F_8 ( V_6 ) ;
return;
}
F_18 ( V_6 ) ;
}
struct V_5 * F_19 ( struct V_76 * V_75 )
{
struct V_5 * V_6 ;
struct V_77 * V_78 ;
F_3 ( 3 , L_39 ) ;
F_20 (item, &mc_devices) {
V_6 = F_21 ( V_78 , struct V_5 , V_79 ) ;
if ( V_6 -> V_40 == V_75 )
return V_6 ;
}
return NULL ;
}
static int F_22 ( void )
{
int V_80 ;
if ( V_81 == V_82 )
return 1 ;
V_80 = V_83 ;
V_83 = 0 ;
return V_80 ;
}
static void F_23 ( struct V_84 * V_85 )
{
struct V_86 * V_87 = F_24 ( V_85 ) ;
struct V_5 * V_6 = F_25 ( V_87 ) ;
F_26 ( & V_88 ) ;
if ( V_6 -> V_73 != V_89 ) {
F_27 ( & V_88 ) ;
return;
}
if ( F_22 () )
V_6 -> V_35 ( V_6 ) ;
F_27 ( & V_88 ) ;
F_28 ( & V_6 -> V_90 , F_29 ( F_30 () ) ) ;
}
void F_31 ( unsigned long V_91 )
{
struct V_5 * V_6 ;
struct V_77 * V_78 ;
F_26 ( & V_88 ) ;
F_20 (item, &mc_devices) {
V_6 = F_21 ( V_78 , struct V_5 , V_79 ) ;
F_32 ( & V_6 -> V_90 , V_91 ) ;
}
F_27 ( & V_88 ) ;
}
static int F_33 ( struct V_5 * V_6 )
{
struct V_77 * V_78 , * V_92 ;
struct V_5 * V_10 ;
V_92 = & V_93 ;
V_10 = F_19 ( V_6 -> V_40 ) ;
if ( F_34 ( V_10 != NULL ) )
goto V_94;
F_20 (item, &mc_devices) {
V_10 = F_21 ( V_78 , struct V_5 , V_79 ) ;
if ( V_10 -> V_71 >= V_6 -> V_71 ) {
if ( F_34 ( V_10 -> V_71 == V_6 -> V_71 ) )
goto V_95;
V_92 = V_78 ;
break;
}
}
F_35 ( & V_6 -> V_79 , V_92 ) ;
F_36 ( & V_96 ) ;
return 0 ;
V_94:
F_37 ( V_97 , V_98 ,
L_40 , F_38 ( V_10 -> V_40 ) ,
F_39 ( V_6 ) , V_10 -> V_41 , V_10 -> V_42 , V_10 -> V_71 ) ;
return 1 ;
V_95:
F_37 ( V_97 , V_98 ,
L_41
L_42 , V_10 -> V_71 , V_99 ) ;
return 1 ;
}
static int F_40 ( struct V_5 * V_6 )
{
int V_100 = F_41 ( & V_96 ) ;
F_42 ( & V_6 -> V_79 ) ;
F_43 () ;
F_44 ( & V_6 -> V_79 ) ;
return V_100 ;
}
struct V_5 * F_45 ( int V_101 )
{
struct V_77 * V_78 ;
struct V_5 * V_6 ;
F_20 (item, &mc_devices) {
V_6 = F_21 ( V_78 , struct V_5 , V_79 ) ;
if ( V_6 -> V_71 >= V_101 ) {
if ( V_6 -> V_71 == V_101 )
return V_6 ;
break;
}
}
return NULL ;
}
int F_46 ( struct V_5 * V_6 ,
const struct V_102 * * V_103 )
{
int V_104 = - V_105 ;
F_3 ( 0 , L_39 ) ;
if ( V_6 -> V_71 >= V_106 ) {
F_47 ( L_43 , V_6 -> V_71 ) ;
return - V_107 ;
}
#ifdef F_48
if ( V_108 >= 3 )
F_6 ( V_6 ) ;
if ( V_108 >= 4 ) {
int V_7 ;
for ( V_7 = 0 ; V_7 < V_6 -> V_36 ; V_7 ++ ) {
struct V_25 * V_18 = V_6 -> V_37 [ V_7 ] ;
T_1 V_23 = 0 ;
int V_65 ;
for ( V_65 = 0 ; V_65 < V_18 -> V_30 ; V_65 ++ )
V_23 += V_18 -> V_31 [ V_65 ] -> V_2 -> V_23 ;
if ( ! V_23 )
continue;
F_5 ( V_18 ) ;
for ( V_65 = 0 ; V_65 < V_18 -> V_30 ; V_65 ++ )
if ( V_18 -> V_31 [ V_65 ] -> V_2 -> V_23 )
F_2 ( V_18 -> V_31 [ V_65 ] ) ;
}
for ( V_7 = 0 ; V_7 < V_6 -> V_38 ; V_7 ++ )
if ( V_6 -> V_39 [ V_7 ] -> V_23 )
F_4 ( V_6 -> V_39 [ V_7 ] , V_7 ) ;
}
#endif
F_26 ( & V_88 ) ;
if ( V_109 && V_109 != V_6 -> V_41 ) {
V_104 = - V_110 ;
goto V_94;
}
if ( F_33 ( V_6 ) )
goto V_94;
V_6 -> V_111 = V_112 ;
V_6 -> V_113 = & V_114 [ V_6 -> V_71 ] ;
if ( F_49 ( V_6 , V_103 ) ) {
F_15 ( V_6 , V_97 ,
L_44 ) ;
goto V_95;
}
if ( V_6 -> V_35 ) {
V_6 -> V_73 = V_89 ;
F_50 ( & V_6 -> V_90 , F_23 ) ;
F_28 ( & V_6 -> V_90 , F_29 ( F_30 () ) ) ;
} else {
V_6 -> V_73 = V_115 ;
}
F_15 ( V_6 , V_116 ,
L_45 ,
V_6 -> V_41 , V_6 -> V_42 , V_6 -> F_38 ,
F_51 ( V_6 -> V_73 ) ) ;
V_109 = V_6 -> V_41 ;
F_27 ( & V_88 ) ;
return 0 ;
V_95:
F_40 ( V_6 ) ;
V_94:
F_27 ( & V_88 ) ;
return V_104 ;
}
struct V_5 * F_52 ( struct V_76 * V_75 )
{
struct V_5 * V_6 ;
F_3 ( 0 , L_39 ) ;
F_26 ( & V_88 ) ;
V_6 = F_19 ( V_75 ) ;
if ( V_6 == NULL ) {
F_27 ( & V_88 ) ;
return NULL ;
}
V_6 -> V_73 = V_117 ;
if ( ! F_40 ( V_6 ) )
V_109 = NULL ;
F_27 ( & V_88 ) ;
if ( V_6 -> V_35 )
F_53 ( & V_6 -> V_90 ) ;
F_54 ( V_6 ) ;
F_37 ( V_116 , V_98 ,
L_46 , V_6 -> V_71 ,
V_6 -> V_41 , V_6 -> V_42 , F_39 ( V_6 ) ) ;
return V_6 ;
}
static void F_55 ( unsigned long V_118 , unsigned long V_119 ,
T_1 V_44 )
{
struct V_118 * V_120 ;
void * V_121 ;
unsigned long V_122 = 0 ;
F_3 ( 3 , L_39 ) ;
if ( ! F_56 ( V_118 ) )
return;
V_120 = F_57 ( V_118 ) ;
if ( F_58 ( V_120 ) )
F_59 ( V_122 ) ;
V_121 = F_60 ( V_120 ) ;
F_61 ( V_121 + V_119 , V_44 ) ;
F_62 ( V_121 ) ;
if ( F_58 ( V_120 ) )
F_63 ( V_122 ) ;
}
int F_64 ( struct V_5 * V_6 , unsigned long V_118 )
{
struct V_25 * * V_37 = V_6 -> V_37 ;
int V_50 , V_7 , V_65 , V_8 ;
F_3 ( 1 , L_47 , V_6 -> V_71 , V_118 ) ;
V_50 = - 1 ;
for ( V_7 = 0 ; V_7 < V_6 -> V_36 ; V_7 ++ ) {
struct V_25 * V_18 = V_37 [ V_7 ] ;
V_8 = 0 ;
for ( V_65 = 0 ; V_65 < V_18 -> V_30 ; V_65 ++ ) {
struct V_1 * V_2 = V_18 -> V_31 [ V_65 ] -> V_2 ;
V_8 += V_2 -> V_23 ;
}
if ( V_8 == 0 )
continue;
F_3 ( 3 , L_48 ,
V_6 -> V_71 ,
V_18 -> V_27 , V_118 , V_18 -> V_28 ,
V_18 -> V_29 ) ;
if ( ( V_118 >= V_18 -> V_27 ) &&
( V_118 <= V_18 -> V_28 ) &&
( ( V_118 & V_18 -> V_29 ) ==
( V_18 -> V_27 & V_18 -> V_29 ) ) ) {
V_50 = V_7 ;
break;
}
}
if ( V_50 == - 1 )
F_15 ( V_6 , V_72 ,
L_49 ,
( unsigned long ) V_118 ) ;
return V_50 ;
}
static void F_65 ( struct V_5 * V_6 ,
bool V_123 ,
const int V_62 [ V_60 ] ,
const T_2 V_9 )
{
int V_7 , V_124 = 0 ;
V_6 -> V_125 += V_9 ;
if ( ! V_123 ) {
V_6 -> V_126 += V_9 ;
return;
}
for ( V_7 = 0 ; V_7 < V_6 -> V_11 ; V_7 ++ ) {
if ( V_62 [ V_7 ] < 0 )
break;
V_124 += V_62 [ V_7 ] ;
V_6 -> V_59 [ V_7 ] [ V_124 ] += V_9 ;
if ( V_7 < V_6 -> V_11 - 1 )
V_124 *= V_6 -> V_13 [ V_7 + 1 ] . V_44 ;
}
}
static void F_66 ( struct V_5 * V_6 ,
bool V_123 ,
const int V_62 [ V_60 ] ,
const T_2 V_9 )
{
int V_7 , V_124 = 0 ;
V_6 -> V_127 += V_9 ;
if ( ! V_123 ) {
V_6 -> V_126 += V_9 ;
return;
}
for ( V_7 = 0 ; V_7 < V_6 -> V_11 ; V_7 ++ ) {
if ( V_62 [ V_7 ] < 0 )
break;
V_124 += V_62 [ V_7 ] ;
V_6 -> V_61 [ V_7 ] [ V_124 ] += V_9 ;
if ( V_7 < V_6 -> V_11 - 1 )
V_124 *= V_6 -> V_13 [ V_7 + 1 ] . V_44 ;
}
}
static void F_67 ( struct V_5 * V_6 ,
const T_2 V_128 ,
const int V_62 [ V_60 ] ,
const char * V_129 ,
const char * V_14 ,
const char * V_22 ,
const char * V_130 ,
const char * V_131 ,
const bool V_123 ,
const unsigned long V_132 ,
const unsigned long V_133 ,
long V_24 )
{
unsigned long V_134 ;
char * V_135 = L_50 ;
if ( * V_129 )
V_135 = L_51 ;
if ( F_68 () ) {
if ( V_131 && * V_131 )
F_15 ( V_6 , V_97 ,
L_52 ,
V_128 , V_129 , V_135 , V_22 ,
V_14 , V_130 , V_131 ) ;
else
F_15 ( V_6 , V_97 ,
L_53 ,
V_128 , V_129 , V_135 , V_22 ,
V_14 , V_130 ) ;
}
F_65 ( V_6 , V_123 , V_62 , V_128 ) ;
if ( V_6 -> V_136 == V_137 ) {
V_134 = V_6 -> V_138 ?
V_6 -> V_138 ( V_6 , V_132 ) :
V_132 ;
F_55 ( V_134 ,
V_133 , V_24 ) ;
}
}
static void F_69 ( struct V_5 * V_6 ,
const T_2 V_128 ,
const int V_62 [ V_60 ] ,
const char * V_129 ,
const char * V_14 ,
const char * V_22 ,
const char * V_130 ,
const char * V_131 ,
const bool V_123 )
{
char * V_135 = L_50 ;
if ( * V_129 )
V_135 = L_51 ;
if ( F_70 () ) {
if ( V_131 && * V_131 )
F_15 ( V_6 , V_97 ,
L_54 ,
V_128 , V_129 , V_135 , V_22 ,
V_14 , V_130 , V_131 ) ;
else
F_15 ( V_6 , V_97 ,
L_55 ,
V_128 , V_129 , V_135 , V_22 ,
V_14 , V_130 ) ;
}
if ( F_71 () ) {
if ( V_131 && * V_131 )
F_72 ( L_56 ,
V_129 , V_135 , V_22 , V_14 , V_130 , V_131 ) ;
else
F_72 ( L_57 ,
V_129 , V_135 , V_22 , V_14 , V_130 ) ;
}
F_66 ( V_6 , V_123 , V_62 , V_128 ) ;
}
void F_73 ( const enum V_139 type ,
struct V_5 * V_6 ,
struct V_140 * V_141 )
{
char V_130 [ 80 ] ;
int V_62 [ V_60 ] = { V_141 -> V_142 , V_141 -> V_143 , V_141 -> V_144 } ;
if ( type == V_145 ) {
snprintf ( V_130 , sizeof( V_130 ) ,
L_58 ,
V_141 -> V_132 , V_141 -> V_133 ,
V_141 -> V_24 , V_141 -> V_146 ) ;
F_67 ( V_6 , V_141 -> V_128 , V_62 , V_141 -> V_129 , V_141 -> V_14 , V_141 -> V_22 ,
V_130 , V_141 -> V_131 , V_141 -> V_123 ,
V_141 -> V_132 , V_141 -> V_133 , V_141 -> V_24 ) ;
} else {
snprintf ( V_130 , sizeof( V_130 ) ,
L_59 ,
V_141 -> V_132 , V_141 -> V_133 , V_141 -> V_24 ) ;
F_69 ( V_6 , V_141 -> V_128 , V_62 , V_141 -> V_129 , V_141 -> V_14 , V_141 -> V_22 ,
V_130 , V_141 -> V_131 , V_141 -> V_123 ) ;
}
}
void F_74 ( const enum V_139 type ,
struct V_5 * V_6 ,
const T_2 V_128 ,
const unsigned long V_132 ,
const unsigned long V_133 ,
const unsigned long V_146 ,
const int V_142 ,
const int V_143 ,
const int V_144 ,
const char * V_129 ,
const char * V_131 )
{
char * V_10 ;
int V_50 = - 1 , V_16 = - 1 ;
int V_62 [ V_60 ] = { V_142 , V_143 , V_144 } ;
int V_7 , V_147 = 0 ;
T_3 V_148 ;
struct V_140 * V_141 = & V_6 -> V_149 ;
F_3 ( 3 , L_60 , V_6 -> V_71 ) ;
memset ( V_141 , 0 , sizeof ( * V_141 ) ) ;
V_141 -> V_128 = V_128 ;
V_141 -> V_142 = V_142 ;
V_141 -> V_143 = V_143 ;
V_141 -> V_144 = V_144 ;
V_141 -> V_132 = V_132 ;
V_141 -> V_133 = V_133 ;
V_141 -> V_146 = V_146 ;
V_141 -> V_129 = V_129 ;
V_141 -> V_131 = V_131 ;
for ( V_7 = 0 ; V_7 < V_6 -> V_11 ; V_7 ++ ) {
if ( V_62 [ V_7 ] >= ( int ) V_6 -> V_13 [ V_7 ] . V_44 ) {
F_15 ( V_6 , V_72 ,
L_61 ,
V_12 [ V_6 -> V_13 [ V_7 ] . type ] ,
V_62 [ V_7 ] , V_6 -> V_13 [ V_7 ] . V_44 ) ;
V_62 [ V_7 ] = - 1 ;
}
if ( V_62 [ V_7 ] >= 0 )
V_141 -> V_123 = true ;
}
V_10 = V_141 -> V_22 ;
* V_10 = '\0' ;
for ( V_7 = 0 ; V_7 < V_6 -> V_38 ; V_7 ++ ) {
struct V_1 * V_2 = V_6 -> V_39 [ V_7 ] ;
if ( V_142 >= 0 && V_142 != V_2 -> V_14 [ 0 ] )
continue;
if ( V_143 >= 0 && V_143 != V_2 -> V_14 [ 1 ] )
continue;
if ( V_144 >= 0 && V_144 != V_2 -> V_14 [ 2 ] )
continue;
if ( V_2 -> V_24 > V_141 -> V_24 )
V_141 -> V_24 = V_2 -> V_24 ;
if ( V_141 -> V_123 && V_2 -> V_23 ) {
if ( V_147 >= V_150 ) {
V_141 -> V_123 = false ;
break;
}
V_147 ++ ;
if ( V_10 != V_141 -> V_22 ) {
strcpy ( V_10 , V_151 ) ;
V_10 += strlen ( V_151 ) ;
}
strcpy ( V_10 , V_2 -> V_22 ) ;
V_10 += strlen ( V_10 ) ;
* V_10 = '\0' ;
F_3 ( 4 , L_62 ,
V_6 -> V_20 ? L_7 : L_8 ,
V_2 -> V_18 , V_2 -> V_21 ) ;
if ( V_50 == - 1 )
V_50 = V_2 -> V_18 ;
else if ( V_50 >= 0 && V_50 != V_2 -> V_18 )
V_50 = - 2 ;
if ( V_16 == - 1 )
V_16 = V_2 -> V_21 ;
else if ( V_16 >= 0 && V_16 != V_2 -> V_21 )
V_16 = - 2 ;
}
}
if ( ! V_141 -> V_123 ) {
strcpy ( V_141 -> V_22 , L_63 ) ;
} else {
F_3 ( 4 , L_64 , V_50 , V_16 ) ;
if ( V_10 == V_141 -> V_22 )
strcpy ( V_141 -> V_22 , L_65 ) ;
if ( type == V_145 ) {
if ( V_50 >= 0 ) {
V_6 -> V_37 [ V_50 ] -> V_152 += V_128 ;
if ( V_16 >= 0 )
V_6 -> V_37 [ V_50 ] -> V_31 [ V_16 ] -> V_152 += V_128 ;
}
} else
if ( V_50 >= 0 )
V_6 -> V_37 [ V_50 ] -> V_153 += V_128 ;
}
V_10 = V_141 -> V_14 ;
for ( V_7 = 0 ; V_7 < V_6 -> V_11 ; V_7 ++ ) {
if ( V_62 [ V_7 ] < 0 )
continue;
V_10 += sprintf ( V_10 , L_66 ,
V_12 [ V_6 -> V_13 [ V_7 ] . type ] ,
V_62 [ V_7 ] ) ;
}
if ( V_10 > V_141 -> V_14 )
* ( V_10 - 1 ) = '\0' ;
V_148 = F_75 ( V_141 -> V_24 ) + 1 ;
F_76 ( type , V_141 -> V_129 , V_141 -> V_22 , V_141 -> V_128 ,
V_6 -> V_71 , V_141 -> V_142 , V_141 -> V_143 , V_141 -> V_144 ,
( V_141 -> V_132 << V_154 ) | V_141 -> V_133 ,
V_148 , V_141 -> V_146 , V_141 -> V_131 ) ;
F_73 ( type , V_6 , V_141 ) ;
}
