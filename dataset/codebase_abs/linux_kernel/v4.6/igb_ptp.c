static T_1 F_1 ( const struct V_1 * V_2 )
{
struct V_3 * V_4 = F_2 ( V_2 , struct V_3 , V_2 ) ;
struct V_5 * V_6 = & V_4 -> V_6 ;
T_2 V_7 ;
T_3 V_8 , V_9 ;
V_8 = F_3 ( V_10 ) ;
V_9 = F_3 ( V_11 ) ;
V_7 = ( ( T_2 ) V_9 ) << 32 ;
V_7 |= V_8 ;
return V_7 ;
}
static T_1 F_4 ( const struct V_1 * V_2 )
{
struct V_3 * V_4 = F_2 ( V_2 , struct V_3 , V_2 ) ;
struct V_5 * V_6 = & V_4 -> V_6 ;
T_3 V_8 , V_9 ;
T_2 V_7 ;
F_3 ( V_12 ) ;
V_8 = F_3 ( V_10 ) ;
V_9 = F_3 ( V_11 ) ;
V_7 = ( ( T_2 ) V_9 ) << 32 ;
V_7 |= V_8 ;
return V_7 ;
}
static void F_5 ( struct V_3 * V_13 ,
struct V_14 * V_15 )
{
struct V_5 * V_6 = & V_13 -> V_6 ;
T_3 V_16 , V_17 ;
F_3 ( V_12 ) ;
V_17 = F_3 ( V_10 ) ;
V_16 = F_3 ( V_11 ) ;
V_15 -> V_18 = V_16 ;
V_15 -> V_19 = V_17 ;
}
static void F_6 ( struct V_3 * V_13 ,
const struct V_14 * V_15 )
{
struct V_5 * V_6 = & V_13 -> V_6 ;
F_7 ( V_10 , V_15 -> V_19 ) ;
F_7 ( V_11 , ( T_3 ) V_15 -> V_18 ) ;
}
static void F_8 ( struct V_3 * V_13 ,
struct V_20 * V_21 ,
T_2 V_22 )
{
unsigned long V_23 ;
T_2 V_24 ;
switch ( V_13 -> V_6 . V_25 . type ) {
case V_26 :
case V_27 :
case V_28 :
case V_29 :
F_9 ( & V_13 -> V_30 , V_23 ) ;
V_24 = F_10 ( & V_13 -> V_31 , V_22 ) ;
F_11 ( & V_13 -> V_30 , V_23 ) ;
memset ( V_21 , 0 , sizeof( * V_21 ) ) ;
V_21 -> V_32 = F_12 ( V_24 ) ;
break;
case V_33 :
case V_34 :
memset ( V_21 , 0 , sizeof( * V_21 ) ) ;
V_21 -> V_32 = F_13 ( V_22 >> 32 ,
V_22 & 0xFFFFFFFF ) ;
break;
default:
break;
}
}
static int F_14 ( struct V_35 * V_36 , T_4 V_37 )
{
struct V_3 * V_4 = F_2 ( V_36 , struct V_3 ,
V_38 ) ;
struct V_5 * V_6 = & V_4 -> V_6 ;
int V_39 = 0 ;
T_2 V_40 ;
T_3 V_41 ;
if ( V_37 < 0 ) {
V_39 = 1 ;
V_37 = - V_37 ;
}
V_40 = V_37 ;
V_40 <<= 14 ;
V_40 = F_15 ( V_40 , 1953125 ) ;
V_41 = 16 << V_42 ;
if ( V_39 )
V_41 -= V_40 ;
else
V_41 += V_40 ;
F_7 ( V_43 , V_44 | ( V_41 & V_45 ) ) ;
return 0 ;
}
static int F_16 ( struct V_35 * V_36 , T_4 V_37 )
{
struct V_3 * V_4 = F_2 ( V_36 , struct V_3 ,
V_38 ) ;
struct V_5 * V_6 = & V_4 -> V_6 ;
int V_39 = 0 ;
T_2 V_40 ;
T_3 V_46 ;
if ( V_37 < 0 ) {
V_39 = 1 ;
V_37 = - V_37 ;
}
V_40 = V_37 ;
V_40 <<= 26 ;
V_40 = F_15 ( V_40 , 1953125 ) ;
V_46 = V_40 & V_47 ;
if ( V_39 )
V_46 |= V_48 ;
F_7 ( V_43 , V_46 ) ;
return 0 ;
}
static int F_17 ( struct V_35 * V_36 , T_5 V_49 )
{
struct V_3 * V_4 = F_2 ( V_36 , struct V_3 ,
V_38 ) ;
unsigned long V_23 ;
F_9 ( & V_4 -> V_30 , V_23 ) ;
F_18 ( & V_4 -> V_31 , V_49 ) ;
F_11 ( & V_4 -> V_30 , V_23 ) ;
return 0 ;
}
static int F_19 ( struct V_35 * V_36 , T_5 V_49 )
{
struct V_3 * V_4 = F_2 ( V_36 , struct V_3 ,
V_38 ) ;
unsigned long V_23 ;
struct V_14 V_50 , V_51 = F_20 ( V_49 ) ;
F_9 ( & V_4 -> V_30 , V_23 ) ;
F_5 ( V_4 , & V_50 ) ;
V_50 = F_21 ( V_50 , V_51 ) ;
F_6 ( V_4 , ( const struct V_14 * ) & V_50 ) ;
F_11 ( & V_4 -> V_30 , V_23 ) ;
return 0 ;
}
static int F_22 ( struct V_35 * V_36 ,
struct V_14 * V_15 )
{
struct V_3 * V_4 = F_2 ( V_36 , struct V_3 ,
V_38 ) ;
unsigned long V_23 ;
T_2 V_24 ;
F_9 ( & V_4 -> V_30 , V_23 ) ;
V_24 = F_23 ( & V_4 -> V_31 ) ;
F_11 ( & V_4 -> V_30 , V_23 ) ;
* V_15 = F_20 ( V_24 ) ;
return 0 ;
}
static int F_24 ( struct V_35 * V_36 ,
struct V_14 * V_15 )
{
struct V_3 * V_4 = F_2 ( V_36 , struct V_3 ,
V_38 ) ;
unsigned long V_23 ;
F_9 ( & V_4 -> V_30 , V_23 ) ;
F_5 ( V_4 , V_15 ) ;
F_11 ( & V_4 -> V_30 , V_23 ) ;
return 0 ;
}
static int F_25 ( struct V_35 * V_36 ,
const struct V_14 * V_15 )
{
struct V_3 * V_4 = F_2 ( V_36 , struct V_3 ,
V_38 ) ;
unsigned long V_23 ;
T_2 V_24 ;
V_24 = F_26 ( V_15 ) ;
F_9 ( & V_4 -> V_30 , V_23 ) ;
F_27 ( & V_4 -> V_31 , & V_4 -> V_2 , V_24 ) ;
F_11 ( & V_4 -> V_30 , V_23 ) ;
return 0 ;
}
static int F_28 ( struct V_35 * V_36 ,
const struct V_14 * V_15 )
{
struct V_3 * V_4 = F_2 ( V_36 , struct V_3 ,
V_38 ) ;
unsigned long V_23 ;
F_9 ( & V_4 -> V_30 , V_23 ) ;
F_6 ( V_4 , V_15 ) ;
F_11 ( & V_4 -> V_30 , V_23 ) ;
return 0 ;
}
static void F_29 ( int V_52 , int V_53 , T_3 * V_54 , T_3 * V_55 )
{
T_3 * V_56 = V_52 < 2 ? V_54 : V_55 ;
static const T_3 V_57 [ V_58 ] = {
V_59 ,
V_60 ,
V_61 ,
V_62 ,
} ;
if ( V_53 )
* V_56 &= ~ V_57 [ V_52 ] ;
else
* V_56 |= V_57 [ V_52 ] ;
}
static void F_30 ( struct V_3 * V_4 , int V_63 , int V_52 )
{
static const T_3 V_64 [ V_58 ] = {
V_65 , V_66 , V_67 , V_68 ,
} ;
static const T_3 V_69 [ V_58 ] = {
V_70 , V_71 , V_72 , V_73 ,
} ;
static const T_3 V_74 [ V_58 ] = {
V_75 , V_76 , V_77 , V_78 ,
} ;
struct V_5 * V_6 = & V_4 -> V_6 ;
T_3 V_54 , V_55 , V_79 = 0 ;
V_54 = F_3 ( V_80 ) ;
V_55 = F_3 ( V_81 ) ;
V_79 = F_3 ( V_82 ) ;
F_29 ( V_52 , 1 , & V_54 , & V_55 ) ;
V_79 &= ~ V_74 [ V_52 ] ;
if ( V_63 == 1 ) {
V_79 &= ~ V_73 ;
V_79 |= V_69 [ V_52 ] | V_83 ;
} else {
V_79 &= ~ V_68 ;
V_79 |= V_64 [ V_52 ] | V_84 ;
}
F_7 ( V_82 , V_79 ) ;
F_7 ( V_80 , V_54 ) ;
F_7 ( V_81 , V_55 ) ;
}
static void F_31 ( struct V_3 * V_4 , int V_63 , int V_52 , int V_85 )
{
static const T_3 V_64 [ V_58 ] = {
V_65 , V_66 , V_67 , V_68 ,
} ;
static const T_3 V_69 [ V_58 ] = {
V_70 , V_71 , V_72 , V_73 ,
} ;
static const T_3 V_74 [ V_58 ] = {
V_75 , V_76 , V_77 , V_78 ,
} ;
static const T_3 V_86 [ V_58 ] = {
V_87 , V_88 ,
V_89 , V_90 ,
} ;
static const T_3 V_91 [ V_58 ] = {
V_92 , V_93 ,
V_94 , V_95 ,
} ;
static const T_3 V_96 [ V_58 ] = {
V_97 , V_98 ,
V_99 , V_100 ,
} ;
static const T_3 V_101 [ V_58 ] = {
V_102 , V_103 ,
V_104 , V_105 ,
} ;
static const T_3 V_106 [ V_58 ] = {
V_102 , V_103 ,
V_104 , V_105 ,
} ;
struct V_5 * V_6 = & V_4 -> V_6 ;
T_3 V_54 , V_55 , V_79 = 0 ;
V_54 = F_3 ( V_80 ) ;
V_55 = F_3 ( V_81 ) ;
V_79 = F_3 ( V_82 ) ;
F_29 ( V_52 , 0 , & V_54 , & V_55 ) ;
if ( ( V_79 & V_68 ) == V_64 [ V_52 ] )
V_79 &= ~ V_84 ;
if ( ( V_79 & V_73 ) == V_69 [ V_52 ] )
V_79 &= ~ V_83 ;
V_79 &= ~ V_106 [ V_52 ] ;
if ( V_85 ) {
if ( V_63 == 1 )
V_79 |= V_101 [ V_52 ] ;
else
V_79 |= V_96 [ V_52 ] ;
} else {
if ( V_63 == 1 )
V_79 |= V_91 [ V_52 ] ;
else
V_79 |= V_86 [ V_52 ] ;
}
V_79 |= V_74 [ V_52 ] ;
F_7 ( V_82 , V_79 ) ;
F_7 ( V_80 , V_54 ) ;
F_7 ( V_81 , V_55 ) ;
}
static int F_32 ( struct V_35 * V_36 ,
struct V_107 * V_108 , int V_109 )
{
struct V_3 * V_4 =
F_2 ( V_36 , struct V_3 , V_38 ) ;
struct V_5 * V_6 = & V_4 -> V_6 ;
T_3 V_110 , V_111 , V_112 , V_113 , V_114 , V_115 , V_116 ;
unsigned long V_23 ;
struct V_14 V_15 ;
int V_117 = 0 , V_52 = - 1 ;
T_5 V_24 ;
switch ( V_108 -> type ) {
case V_118 :
if ( V_109 ) {
V_52 = F_33 ( V_4 -> V_119 , V_120 ,
V_108 -> V_121 . V_122 ) ;
if ( V_52 < 0 )
return - V_123 ;
}
if ( V_108 -> V_121 . V_122 == 1 ) {
V_112 = V_124 ;
V_113 = V_125 ;
} else {
V_112 = V_126 ;
V_113 = V_127 ;
}
F_9 ( & V_4 -> V_30 , V_23 ) ;
V_110 = F_3 ( V_128 ) ;
V_111 = F_3 ( V_129 ) ;
if ( V_109 ) {
F_30 ( V_4 , V_108 -> V_121 . V_122 , V_52 ) ;
V_110 |= V_112 ;
V_111 |= V_113 ;
} else {
V_110 &= ~ V_112 ;
V_111 &= ~ V_113 ;
}
F_7 ( V_128 , V_110 ) ;
F_7 ( V_129 , V_111 ) ;
F_11 ( & V_4 -> V_30 , V_23 ) ;
return 0 ;
case V_130 :
if ( V_109 ) {
V_52 = F_33 ( V_4 -> V_119 , V_131 ,
V_108 -> V_132 . V_122 ) ;
if ( V_52 < 0 )
return - V_123 ;
}
V_15 . V_18 = V_108 -> V_132 . V_133 . V_16 ;
V_15 . V_19 = V_108 -> V_132 . V_133 . V_17 ;
V_24 = F_26 ( & V_15 ) ;
V_24 = V_24 >> 1 ;
if ( V_109 && ( ( V_24 <= 70000000LL ) || ( V_24 == 125000000LL ) ||
( V_24 == 250000000LL ) || ( V_24 == 500000000LL ) ) ) {
if ( V_24 < 8LL )
return - V_134 ;
V_117 = 1 ;
}
V_15 = F_20 ( V_24 ) ;
if ( V_108 -> V_132 . V_122 == 1 ) {
if ( V_117 ) {
V_112 = V_135 | V_136 ;
V_113 = 0 ;
} else {
V_112 = V_137 ;
V_113 = V_138 ;
}
V_114 = V_139 ;
V_115 = V_140 ;
V_116 = V_141 ;
} else {
if ( V_117 ) {
V_112 = V_142 | V_143 ;
V_113 = 0 ;
} else {
V_112 = V_144 ;
V_113 = V_145 ;
}
V_114 = V_146 ;
V_115 = V_147 ;
V_116 = V_148 ;
}
F_9 ( & V_4 -> V_30 , V_23 ) ;
V_110 = F_3 ( V_128 ) ;
V_111 = F_3 ( V_129 ) ;
if ( V_108 -> V_132 . V_122 == 1 ) {
V_110 &= ~ ( V_137 | V_135 | V_136 ) ;
V_111 &= ~ V_138 ;
} else {
V_110 &= ~ ( V_144 | V_142 | V_143 ) ;
V_111 &= ~ V_145 ;
}
if ( V_109 ) {
int V_149 = V_108 -> V_132 . V_122 ;
F_31 ( V_4 , V_149 , V_52 , V_117 ) ;
V_4 -> V_132 [ V_149 ] . V_150 . V_18 = V_108 -> V_132 . V_150 . V_16 ;
V_4 -> V_132 [ V_149 ] . V_150 . V_19 = V_108 -> V_132 . V_150 . V_17 ;
V_4 -> V_132 [ V_149 ] . V_133 . V_18 = V_15 . V_18 ;
V_4 -> V_132 [ V_149 ] . V_133 . V_19 = V_15 . V_19 ;
F_7 ( V_115 , V_108 -> V_132 . V_150 . V_16 ) ;
F_7 ( V_114 , V_108 -> V_132 . V_150 . V_17 ) ;
if ( V_117 )
F_7 ( V_116 , V_24 ) ;
V_110 |= V_112 ;
V_111 |= V_113 ;
}
F_7 ( V_128 , V_110 ) ;
F_7 ( V_129 , V_111 ) ;
F_11 ( & V_4 -> V_30 , V_23 ) ;
return 0 ;
case V_151 :
F_9 ( & V_4 -> V_30 , V_23 ) ;
V_111 = F_3 ( V_129 ) ;
if ( V_109 )
V_111 |= V_152 ;
else
V_111 &= ~ V_152 ;
F_7 ( V_129 , V_111 ) ;
F_11 ( & V_4 -> V_30 , V_23 ) ;
return 0 ;
}
return - V_153 ;
}
static int F_34 ( struct V_35 * V_36 ,
struct V_107 * V_108 , int V_109 )
{
return - V_153 ;
}
static int F_35 ( struct V_35 * V_36 , unsigned int V_52 ,
enum V_154 V_155 , unsigned int V_63 )
{
switch ( V_155 ) {
case V_156 :
case V_120 :
case V_131 :
break;
case V_157 :
return - 1 ;
}
return 0 ;
}
static void F_36 ( struct V_158 * V_159 )
{
struct V_3 * V_13 = F_2 ( V_159 , struct V_3 ,
V_160 ) ;
struct V_5 * V_6 = & V_13 -> V_6 ;
T_3 V_161 ;
if ( ! V_13 -> V_162 )
return;
if ( F_37 ( V_13 -> V_163 +
V_164 ) ) {
F_38 ( V_13 -> V_162 ) ;
V_13 -> V_162 = NULL ;
F_39 ( V_165 , & V_13 -> V_166 ) ;
V_13 -> V_167 ++ ;
F_40 ( & V_13 -> V_168 -> V_169 , L_1 ) ;
return;
}
V_161 = F_3 ( V_170 ) ;
if ( V_161 & V_171 )
F_41 ( V_13 ) ;
else
F_42 ( & V_13 -> V_160 ) ;
}
static void F_43 ( struct V_158 * V_159 )
{
struct V_3 * V_4 =
F_2 ( V_159 , struct V_3 , V_172 . V_159 ) ;
struct V_14 V_15 ;
V_4 -> V_38 . V_173 ( & V_4 -> V_38 , & V_15 ) ;
F_44 ( L_2 ,
( long long ) V_15 . V_18 , V_15 . V_19 ) ;
F_45 ( & V_4 -> V_172 ,
V_174 ) ;
}
void F_46 ( struct V_3 * V_13 )
{
struct V_5 * V_6 = & V_13 -> V_6 ;
T_3 V_175 = F_3 ( V_176 ) ;
unsigned long V_177 ;
if ( V_6 -> V_25 . type != V_26 )
return;
if ( ! ( V_175 & V_178 ) ) {
V_13 -> V_179 = V_180 ;
return;
}
V_177 = V_13 -> V_179 ;
if ( F_47 ( V_13 -> V_181 , V_177 ) )
V_177 = V_13 -> V_181 ;
if ( F_37 ( V_177 + 5 * V_182 ) ) {
F_3 ( V_183 ) ;
V_13 -> V_179 = V_180 ;
V_13 -> V_184 ++ ;
F_40 ( & V_13 -> V_168 -> V_169 , L_3 ) ;
}
}
static void F_41 ( struct V_3 * V_13 )
{
struct V_5 * V_6 = & V_13 -> V_6 ;
struct V_20 V_185 ;
T_2 V_186 ;
V_186 = F_3 ( V_187 ) ;
V_186 |= ( T_2 ) F_3 ( V_188 ) << 32 ;
F_8 ( V_13 , & V_185 , V_186 ) ;
F_48 ( V_13 -> V_162 , & V_185 ) ;
F_38 ( V_13 -> V_162 ) ;
V_13 -> V_162 = NULL ;
F_39 ( V_165 , & V_13 -> V_166 ) ;
}
void F_49 ( struct V_189 * V_190 ,
unsigned char * V_191 ,
struct V_192 * V_193 )
{
T_6 * V_186 = ( T_6 * ) V_191 ;
F_8 ( V_190 -> V_13 , F_50 ( V_193 ) ,
F_51 ( V_186 [ 1 ] ) ) ;
}
void F_52 ( struct V_189 * V_190 ,
struct V_192 * V_193 )
{
struct V_3 * V_13 = V_190 -> V_13 ;
struct V_5 * V_6 = & V_13 -> V_6 ;
T_2 V_186 ;
if ( ! ( F_3 ( V_176 ) & V_178 ) )
return;
V_186 = F_3 ( V_194 ) ;
V_186 |= ( T_2 ) F_3 ( V_183 ) << 32 ;
F_8 ( V_13 , F_50 ( V_193 ) , V_186 ) ;
V_13 -> V_181 = V_180 ;
}
int F_53 ( struct V_195 * V_196 , struct V_197 * V_198 )
{
struct V_3 * V_13 = F_54 ( V_196 ) ;
struct V_199 * V_200 = & V_13 -> V_201 ;
return F_55 ( V_198 -> V_202 , V_200 , sizeof( * V_200 ) ) ?
- V_203 : 0 ;
}
static int F_56 ( struct V_3 * V_13 ,
struct V_199 * V_200 )
{
struct V_5 * V_6 = & V_13 -> V_6 ;
T_3 V_204 = V_205 ;
T_3 V_206 = V_207 ;
T_3 V_208 = 0 ;
bool V_209 = false ;
bool V_210 = false ;
T_3 V_186 ;
if ( V_200 -> V_23 )
return - V_134 ;
switch ( V_200 -> V_211 ) {
case V_212 :
V_204 = 0 ;
case V_213 :
break;
default:
return - V_214 ;
}
switch ( V_200 -> V_215 ) {
case V_216 :
V_206 = 0 ;
break;
case V_217 :
V_206 |= V_218 ;
V_208 = V_219 ;
V_209 = true ;
break;
case V_220 :
V_206 |= V_218 ;
V_208 = V_221 ;
V_209 = true ;
break;
case V_222 :
case V_223 :
case V_224 :
case V_225 :
case V_226 :
case V_227 :
case V_228 :
case V_229 :
case V_230 :
V_206 |= V_231 ;
V_200 -> V_215 = V_222 ;
V_210 = true ;
V_209 = true ;
break;
case V_232 :
case V_233 :
if ( V_6 -> V_25 . type != V_26 ) {
V_206 |= V_234 ;
V_200 -> V_215 = V_233 ;
break;
}
default:
V_200 -> V_215 = V_216 ;
return - V_214 ;
}
if ( V_6 -> V_25 . type == V_235 ) {
if ( V_206 | V_204 )
return - V_134 ;
return 0 ;
}
if ( ( V_6 -> V_25 . type >= V_27 ) && V_206 ) {
V_206 = V_207 ;
V_206 |= V_234 ;
V_200 -> V_215 = V_233 ;
V_210 = true ;
V_209 = true ;
if ( ( V_6 -> V_25 . type == V_33 ) ||
( V_6 -> V_25 . type == V_34 ) ) {
V_186 = F_3 ( V_236 ) ;
V_186 |= V_237 ;
F_7 ( V_236 , V_186 ) ;
}
}
V_186 = F_3 ( V_170 ) ;
V_186 &= ~ V_205 ;
V_186 |= V_204 ;
F_7 ( V_170 , V_186 ) ;
V_186 = F_3 ( V_176 ) ;
V_186 &= ~ ( V_207 | V_238 ) ;
V_186 |= V_206 ;
F_7 ( V_176 , V_186 ) ;
F_7 ( V_239 , V_208 ) ;
if ( V_210 )
F_7 ( F_57 ( 3 ) ,
( V_240 |
V_241 |
V_242 ) ) ;
else
F_7 ( F_57 ( 3 ) , 0 ) ;
if ( V_209 ) {
T_3 V_243 = ( V_244
| V_245
| V_246
| V_247 ) ;
V_243 &= ~ V_248 ;
F_7 ( F_58 ( 3 ) , F_59 ( V_249 ) ) ;
F_7 ( F_60 ( 3 ) ,
( V_250 | V_251 ) ) ;
if ( V_6 -> V_25 . type == V_26 ) {
F_7 ( F_61 ( 3 ) , F_59 ( V_249 ) ) ;
V_243 &= ~ V_252 ;
}
F_7 ( F_62 ( 3 ) , V_243 ) ;
} else {
F_7 ( F_62 ( 3 ) , V_247 ) ;
}
F_63 () ;
V_186 = F_3 ( V_187 ) ;
V_186 = F_3 ( V_188 ) ;
V_186 = F_3 ( V_194 ) ;
V_186 = F_3 ( V_183 ) ;
return 0 ;
}
int F_64 ( struct V_195 * V_196 , struct V_197 * V_198 )
{
struct V_3 * V_13 = F_54 ( V_196 ) ;
struct V_199 V_200 ;
int V_253 ;
if ( F_65 ( & V_200 , V_198 -> V_202 , sizeof( V_200 ) ) )
return - V_203 ;
V_253 = F_56 ( V_13 , & V_200 ) ;
if ( V_253 )
return V_253 ;
memcpy ( & V_13 -> V_201 , & V_200 ,
sizeof( V_13 -> V_201 ) ) ;
return F_55 ( V_198 -> V_202 , & V_200 , sizeof( V_200 ) ) ?
- V_203 : 0 ;
}
void F_66 ( struct V_3 * V_13 )
{
struct V_5 * V_6 = & V_13 -> V_6 ;
struct V_195 * V_196 = V_13 -> V_196 ;
int V_149 ;
switch ( V_6 -> V_25 . type ) {
case V_26 :
snprintf ( V_13 -> V_38 . V_254 , 16 , L_4 , V_196 -> V_255 ) ;
V_13 -> V_38 . V_256 = V_257 ;
V_13 -> V_38 . V_258 = 999999881 ;
V_13 -> V_38 . V_259 = 0 ;
V_13 -> V_38 . V_260 = 0 ;
V_13 -> V_38 . V_261 = F_14 ;
V_13 -> V_38 . V_262 = F_17 ;
V_13 -> V_38 . V_173 = F_22 ;
V_13 -> V_38 . V_263 = F_25 ;
V_13 -> V_38 . V_264 = F_34 ;
V_13 -> V_2 . V_265 = F_1 ;
V_13 -> V_2 . V_57 = F_67 ( 64 ) ;
V_13 -> V_2 . V_266 = 1 ;
V_13 -> V_2 . V_267 = V_42 ;
F_7 ( V_43 , V_44 | V_268 ) ;
break;
case V_27 :
case V_28 :
case V_29 :
snprintf ( V_13 -> V_38 . V_254 , 16 , L_4 , V_196 -> V_255 ) ;
V_13 -> V_38 . V_256 = V_257 ;
V_13 -> V_38 . V_258 = 62499999 ;
V_13 -> V_38 . V_259 = 0 ;
V_13 -> V_38 . V_260 = 0 ;
V_13 -> V_38 . V_261 = F_16 ;
V_13 -> V_38 . V_262 = F_17 ;
V_13 -> V_38 . V_173 = F_22 ;
V_13 -> V_38 . V_263 = F_25 ;
V_13 -> V_38 . V_264 = F_34 ;
V_13 -> V_2 . V_265 = F_4 ;
V_13 -> V_2 . V_57 = F_67 ( V_269 ) ;
V_13 -> V_2 . V_266 = 1 ;
V_13 -> V_2 . V_267 = 0 ;
F_7 ( V_128 , 0x0 ) ;
break;
case V_33 :
case V_34 :
for ( V_149 = 0 ; V_149 < V_58 ; V_149 ++ ) {
struct V_270 * V_271 = & V_13 -> V_272 [ V_149 ] ;
snprintf ( V_271 -> V_254 , sizeof( V_271 -> V_254 ) , L_5 , V_149 ) ;
V_271 -> V_122 = V_149 ;
V_271 -> V_155 = V_156 ;
}
snprintf ( V_13 -> V_38 . V_254 , 16 , L_4 , V_196 -> V_255 ) ;
V_13 -> V_38 . V_256 = V_257 ;
V_13 -> V_38 . V_258 = 62499999 ;
V_13 -> V_38 . V_259 = V_273 ;
V_13 -> V_38 . V_274 = V_275 ;
V_13 -> V_38 . V_276 = V_58 ;
V_13 -> V_38 . V_260 = 1 ;
V_13 -> V_38 . V_277 = V_13 -> V_272 ;
V_13 -> V_38 . V_261 = F_16 ;
V_13 -> V_38 . V_262 = F_19 ;
V_13 -> V_38 . V_173 = F_24 ;
V_13 -> V_38 . V_263 = F_28 ;
V_13 -> V_38 . V_264 = F_32 ;
V_13 -> V_38 . V_278 = F_35 ;
F_7 ( V_128 , 0x0 ) ;
break;
default:
V_13 -> V_119 = NULL ;
return;
}
F_63 () ;
F_68 ( & V_13 -> V_30 ) ;
F_69 ( & V_13 -> V_160 , F_36 ) ;
if ( ( V_6 -> V_25 . type == V_33 ) || ( V_6 -> V_25 . type == V_34 ) ) {
struct V_14 V_15 = F_70 ( F_71 () ) ;
F_28 ( & V_13 -> V_38 , & V_15 ) ;
} else {
F_27 ( & V_13 -> V_31 , & V_13 -> V_2 ,
F_72 ( F_71 () ) ) ;
F_73 ( & V_13 -> V_172 ,
F_43 ) ;
F_45 ( & V_13 -> V_172 ,
V_174 ) ;
}
if ( V_6 -> V_25 . type >= V_27 ) {
F_7 ( V_129 , V_279 ) ;
F_7 ( V_280 , V_281 ) ;
}
V_13 -> V_201 . V_215 = V_216 ;
V_13 -> V_201 . V_211 = V_212 ;
V_13 -> V_119 = F_74 ( & V_13 -> V_38 ,
& V_13 -> V_168 -> V_169 ) ;
if ( F_75 ( V_13 -> V_119 ) ) {
V_13 -> V_119 = NULL ;
F_76 ( & V_13 -> V_168 -> V_169 , L_6 ) ;
} else {
F_77 ( & V_13 -> V_168 -> V_169 , L_7 ,
V_13 -> V_196 -> V_254 ) ;
V_13 -> V_23 |= V_282 ;
}
}
void F_78 ( struct V_3 * V_13 )
{
switch ( V_13 -> V_6 . V_25 . type ) {
case V_26 :
case V_27 :
case V_28 :
case V_29 :
F_79 ( & V_13 -> V_172 ) ;
break;
case V_33 :
case V_34 :
break;
default:
return;
}
F_80 ( & V_13 -> V_160 ) ;
if ( V_13 -> V_162 ) {
F_38 ( V_13 -> V_162 ) ;
V_13 -> V_162 = NULL ;
F_39 ( V_165 , & V_13 -> V_166 ) ;
}
if ( V_13 -> V_119 ) {
F_81 ( V_13 -> V_119 ) ;
F_77 ( & V_13 -> V_168 -> V_169 , L_8 ,
V_13 -> V_196 -> V_254 ) ;
V_13 -> V_23 &= ~ V_282 ;
}
}
void F_82 ( struct V_3 * V_13 )
{
struct V_5 * V_6 = & V_13 -> V_6 ;
unsigned long V_23 ;
if ( ! ( V_13 -> V_23 & V_282 ) )
return;
F_56 ( V_13 , & V_13 -> V_201 ) ;
F_9 ( & V_13 -> V_30 , V_23 ) ;
switch ( V_13 -> V_6 . V_25 . type ) {
case V_26 :
F_7 ( V_43 , V_44 | V_268 ) ;
break;
case V_27 :
case V_28 :
case V_29 :
case V_33 :
case V_34 :
F_7 ( V_128 , 0x0 ) ;
F_7 ( V_82 , 0x0 ) ;
F_7 ( V_129 , V_279 ) ;
F_7 ( V_280 , V_281 ) ;
break;
default:
goto V_283;
}
if ( ( V_6 -> V_25 . type == V_33 ) || ( V_6 -> V_25 . type == V_34 ) ) {
struct V_14 V_15 = F_70 ( F_71 () ) ;
F_6 ( V_13 , & V_15 ) ;
} else {
F_27 ( & V_13 -> V_31 , & V_13 -> V_2 ,
F_72 ( F_71 () ) ) ;
}
V_283:
F_11 ( & V_13 -> V_30 , V_23 ) ;
}
