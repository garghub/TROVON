static inline T_1 F_1 ( void T_2 * V_1 )
{
return F_2 ( V_1 ) ;
}
static inline void F_3 ( T_1 V_2 , void T_2 * V_1 )
{
F_4 ( V_2 , V_1 ) ;
}
static int F_5 ( struct V_3 * V_4 )
{
struct V_5 * V_6 = F_6 ( V_4 ) ;
T_1 V_2 ;
V_2 = F_1 ( V_6 -> V_7 + V_6 -> V_8 ) ;
F_7 ( L_1 , F_8 ( V_4 ) ,
V_2 & V_9 ? L_2 : L_3 ) ;
return V_2 & V_9 ? 0 : 1 ;
}
static unsigned long F_9 ( struct V_3 * V_4 ,
unsigned long V_10 )
{
struct V_5 * V_6 = F_6 ( V_4 ) ;
unsigned long V_11 ;
unsigned long V_12 ;
T_1 V_13 ;
T_1 V_14 ;
T_1 V_15 ;
T_1 V_16 ;
V_13 = F_1 ( V_6 -> V_7 + V_6 -> V_8 ) ;
if ( V_6 -> V_17 <= 1 ) {
if ( V_6 -> type == V_18 ) {
V_15 = 2 ;
V_12 = V_10 * ( F_10 ( V_13 ) + 4 ) ;
} else {
V_14 = F_11 ( V_13 ) + 1 ;
V_15 = F_12 ( V_13 ) + 1 ;
V_16 = F_13 ( V_13 ) ;
V_11 = V_10 / V_14 ;
V_12 = V_11 * V_16 ;
}
} else {
V_15 = F_14 ( V_13 ) ? 2 : 3 ;
V_12 = V_10 * F_15 ( V_13 ) ;
}
F_7 ( L_4 ,
F_8 ( V_4 ) , V_12 / V_15 , V_10 ,
V_6 -> V_17 ) ;
return V_12 / V_15 ;
}
static struct V_19 * F_16 ( struct V_20 * V_21 ,
const char * V_22 , const char * V_23 ,
unsigned long V_24 , void T_2 * V_7 , T_1 V_8 ,
T_1 type , T_3 * V_25 , int V_17 )
{
struct V_5 * V_26 ;
struct V_19 * V_19 ;
struct V_27 V_28 ;
V_26 = F_17 ( sizeof( * V_26 ) , V_29 ) ;
if ( ! V_26 ) {
F_18 ( L_5 , V_30 ) ;
return F_19 ( - V_31 ) ;
}
V_28 . V_22 = V_22 ;
V_28 . V_32 = & V_33 ;
V_28 . V_24 = V_24 ;
V_28 . V_34 = V_23 ? & V_23 : NULL ;
V_28 . V_35 = V_23 ? 1 : 0 ;
V_26 -> V_17 = V_17 ;
V_26 -> V_7 = V_7 ;
V_26 -> V_25 = V_25 ;
V_26 -> V_8 = V_8 ;
V_26 -> type = type ;
V_26 -> V_4 . V_28 = & V_28 ;
V_19 = F_20 ( V_21 , & V_26 -> V_4 ) ;
if ( F_21 ( V_19 ) ) {
F_18 ( L_6 , V_30 , V_22 ) ;
F_22 ( V_26 ) ;
return NULL ;
}
return V_19 ;
}
static int F_23 ( struct V_36 * V_37 )
{
if ( F_24 ( V_37 , L_7 ) )
return 1 ;
if ( F_24 ( V_37 , L_8 ) )
return 1 ;
return 2 ;
}
static void F_25 ( struct V_36 * V_37 , enum V_38 V_39 )
{
const char * V_40 = V_37 -> V_41 ;
struct V_19 * V_19 ;
void T_2 * V_7 ;
int V_17 = F_23 ( V_37 ) ;
V_7 = F_26 ( V_37 , 0 ) ;
if ( V_7 == NULL ) {
F_18 ( L_9 , V_37 -> V_41 ) ;
return;
}
F_27 ( V_37 , L_10 , & V_40 ) ;
V_19 = F_16 ( NULL ,
V_40 , F_28 ( V_37 , 0 ) ,
V_42 , V_7 , 0 , V_39 , & V_43 ,
V_17 ) ;
if ( ! F_21 ( V_19 ) ) {
F_29 ( V_37 , V_44 , V_19 ) ;
F_30 ( V_19 , V_40 , NULL ) ;
F_7 ( L_11 , V_40 ) ;
}
}
static void F_31 ( struct V_36 * V_37 )
{
F_25 ( V_37 , V_45 ) ;
}
static void F_32 ( struct V_36 * V_37 )
{
F_25 ( V_37 , V_18 ) ;
}
static int F_33 ( struct V_3 * V_4 )
{
struct V_46 * V_47 = F_34 ( V_4 ) ;
unsigned long V_24 = 0 ;
T_1 V_2 ;
T_4 V_7 ;
if ( V_47 -> V_25 )
F_35 ( V_47 -> V_25 , V_24 ) ;
if ( V_47 -> V_48 . V_49 != NULL ) {
F_7 ( L_12 , F_8 ( V_4 ) ) ;
V_7 = F_36 ( V_47 -> V_48 . V_49 ) ;
V_2 = F_1 ( V_47 -> V_48 . V_49 +
V_47 -> V_48 . V_50 ) ;
V_2 |= V_47 -> V_48 . V_51 ;
F_3 ( V_2 , V_47 -> V_48 . V_49 +
V_47 -> V_48 . V_50 ) ;
F_7 ( L_13 ,
F_8 ( V_4 ) , & V_7 ,
V_47 -> V_48 . V_50 , V_47 -> V_48 . V_51 ,
V_2 ) ;
V_2 = F_1 ( V_47 -> V_48 . V_49 +
V_47 -> V_48 . V_52 ) ;
V_2 &= ~ V_47 -> V_48 . V_53 ;
F_3 ( V_2 , V_47 -> V_48 . V_49 +
V_47 -> V_48 . V_52 ) ;
F_7 ( L_14 ,
F_8 ( V_4 ) , & V_7 ,
V_47 -> V_48 . V_52 , V_47 -> V_48 . V_53 ,
V_2 ) ;
}
if ( V_47 -> V_25 )
F_37 ( V_47 -> V_25 , V_24 ) ;
return 0 ;
}
static void F_38 ( struct V_3 * V_4 )
{
struct V_46 * V_47 = F_34 ( V_4 ) ;
unsigned long V_24 = 0 ;
T_1 V_2 ;
if ( V_47 -> V_25 )
F_35 ( V_47 -> V_25 , V_24 ) ;
if ( V_47 -> V_48 . V_49 != NULL ) {
F_7 ( L_15 , F_8 ( V_4 ) ) ;
V_2 = F_1 ( V_47 -> V_48 . V_49 +
V_47 -> V_48 . V_52 ) ;
V_2 |= V_47 -> V_48 . V_53 ;
F_3 ( V_2 , V_47 -> V_48 . V_49 +
V_47 -> V_48 . V_52 ) ;
V_2 = F_1 ( V_47 -> V_48 . V_49 +
V_47 -> V_48 . V_50 ) ;
V_2 &= ~ V_47 -> V_48 . V_51 ;
F_3 ( V_2 , V_47 -> V_48 . V_49 +
V_47 -> V_48 . V_50 ) ;
}
if ( V_47 -> V_25 )
F_37 ( V_47 -> V_25 , V_24 ) ;
}
static int F_39 ( struct V_3 * V_4 )
{
struct V_46 * V_47 = F_34 ( V_4 ) ;
T_1 V_2 = 0 ;
if ( V_47 -> V_48 . V_49 != NULL ) {
F_7 ( L_16 , F_8 ( V_4 ) ) ;
V_2 = F_1 ( V_47 -> V_48 . V_49 +
V_47 -> V_48 . V_50 ) ;
F_7 ( L_17 , F_8 ( V_4 ) ,
V_2 & V_47 -> V_48 . V_51 ? L_3 :
L_2 ) ;
}
if ( V_47 -> V_48 . V_49 == NULL )
return 1 ;
return V_2 & V_47 -> V_48 . V_51 ? 1 : 0 ;
}
static unsigned long F_40 ( struct V_3 * V_4 ,
unsigned long V_10 )
{
struct V_46 * V_47 = F_34 ( V_4 ) ;
T_1 V_2 ;
if ( V_47 -> V_48 . V_54 ) {
V_2 = F_1 ( V_47 -> V_48 . V_54 +
V_47 -> V_48 . V_55 ) ;
V_2 >>= V_47 -> V_48 . V_56 ;
V_2 &= ( 1 << V_47 -> V_48 . V_57 ) - 1 ;
F_7 ( L_18 ,
F_8 ( V_4 ) ,
V_10 / V_2 , V_10 ) ;
return V_10 / V_2 ;
} else {
F_7 ( L_18 ,
F_8 ( V_4 ) , V_10 , V_10 ) ;
return V_10 ;
}
}
static int F_41 ( struct V_3 * V_4 , unsigned long V_58 ,
unsigned long V_10 )
{
struct V_46 * V_47 = F_34 ( V_4 ) ;
unsigned long V_24 = 0 ;
T_1 V_2 ;
T_1 V_59 ;
T_1 V_60 ;
if ( V_47 -> V_25 )
F_35 ( V_47 -> V_25 , V_24 ) ;
if ( V_47 -> V_48 . V_54 ) {
if ( V_58 > V_10 )
V_58 = V_10 ;
V_60 = V_59 = V_10 / V_58 ;
V_59 &= ( 1 << V_47 -> V_48 . V_57 ) - 1 ;
V_59 <<= V_47 -> V_48 . V_56 ;
V_2 = F_1 ( V_47 -> V_48 . V_54 +
V_47 -> V_48 . V_55 ) ;
V_2 &= ~ ( ( ( 1 << V_47 -> V_48 . V_57 ) - 1 )
<< V_47 -> V_48 . V_56 ) ;
V_2 |= V_59 ;
F_3 ( V_2 , V_47 -> V_48 . V_54 +
V_47 -> V_48 . V_55 ) ;
F_7 ( L_19 , F_8 ( V_4 ) ,
V_10 / V_60 ) ;
} else {
V_60 = 1 ;
}
if ( V_47 -> V_25 )
F_37 ( V_47 -> V_25 , V_24 ) ;
return V_10 / V_60 ;
}
static long F_42 ( struct V_3 * V_4 , unsigned long V_58 ,
unsigned long * V_61 )
{
struct V_46 * V_47 = F_34 ( V_4 ) ;
unsigned long V_10 = * V_61 ;
T_1 V_59 ;
if ( V_47 -> V_48 . V_54 ) {
if ( V_58 > V_10 )
V_58 = V_10 ;
V_59 = V_10 / V_58 ;
} else {
V_59 = 1 ;
}
return V_10 / V_59 ;
}
static struct V_19 * F_43 ( struct V_20 * V_21 ,
const char * V_22 , const char * V_23 ,
struct V_62 * V_63 , T_3 * V_25 )
{
struct V_46 * V_26 ;
struct V_19 * V_19 ;
struct V_27 V_28 ;
int V_64 ;
V_26 = F_17 ( sizeof( * V_26 ) , V_29 ) ;
if ( ! V_26 ) {
F_18 ( L_5 , V_30 ) ;
return F_19 ( - V_31 ) ;
}
V_28 . V_22 = V_22 ;
V_28 . V_32 = & V_65 ;
V_28 . V_24 = 0 ;
V_28 . V_34 = V_23 ? & V_23 : NULL ;
V_28 . V_35 = V_23 ? 1 : 0 ;
V_26 -> V_25 = V_25 ;
V_26 -> V_4 . V_28 = & V_28 ;
V_26 -> V_48 = * V_63 ;
V_19 = F_20 ( V_21 , & V_26 -> V_4 ) ;
if ( F_21 ( V_19 ) ) {
F_18 ( L_6 , V_30 , V_22 ) ;
F_22 ( V_26 ) ;
return V_19 ;
}
V_64 = F_30 ( V_19 , V_22 , NULL ) ;
if ( V_64 != 0 ) {
F_18 ( L_20 ,
V_30 , V_22 ) ;
}
return V_19 ;
}
static void T_5 F_44 ( struct V_36 * V_37 )
{
const char * V_40 = V_37 -> V_41 ;
struct V_19 * V_19 ;
struct V_66 V_67 ;
int V_64 ;
struct V_62 V_63 ;
int V_68 ;
if ( ! F_45 ( V_37 ) )
return;
V_63 . V_49 = NULL ;
V_63 . V_54 = NULL ;
for ( V_68 = 0 ; V_68 < 2 ; V_68 ++ ) {
void T_2 * V_69 ;
V_64 = F_46 ( V_37 , V_68 , & V_67 ) ;
if ( V_64 != 0 ) {
if ( V_68 == 0 ) {
F_18 ( L_21 ,
V_37 -> V_41 ) ;
return;
}
break;
}
V_69 = F_26 ( V_37 , V_68 ) ;
if ( V_69 == NULL ) {
F_18 ( L_22 ,
V_68 , V_37 -> V_41 ) ;
goto V_70;
}
if ( strcmp ( V_67 . V_22 , L_23 ) == 0 )
V_63 . V_54 = V_69 ;
else
V_63 . V_49 = V_69 ;
}
if ( F_47 ( V_37 , L_24 , & V_63 . V_52 ) )
V_63 . V_52 = 0 ;
if ( F_47 ( V_37 , L_25 , & V_63 . V_53 ) )
V_63 . V_53 = 0xF ;
if ( F_47 ( V_37 , L_26 ,
& V_63 . V_50 ) )
V_63 . V_50 = 0x8 ;
if ( F_47 ( V_37 , L_27 , & V_63 . V_51 ) )
V_63 . V_51 = 0xF ;
if ( F_47 ( V_37 , L_28 ,
& V_63 . V_55 ) )
V_63 . V_55 = 0 ;
if ( F_47 ( V_37 , L_29 ,
& V_63 . V_57 ) )
V_63 . V_57 = 0 ;
if ( F_47 ( V_37 , L_30 ,
& V_63 . V_56 ) )
V_63 . V_56 = 0 ;
F_27 ( V_37 , L_10 , & V_40 ) ;
V_19 = F_43 ( NULL , V_40 ,
F_28 ( V_37 , 0 ) , & V_63 , & V_43 ) ;
if ( F_21 ( V_19 ) )
goto V_70;
F_7 ( L_31 , V_40 ) ;
V_64 = F_29 ( V_37 , V_44 , V_19 ) ;
if ( V_64 != 0 )
F_18 ( L_32 , V_30 ,
V_37 -> V_41 ) ;
return;
V_70:
if ( V_63 . V_49 )
F_48 ( V_63 . V_49 ) ;
if ( V_63 . V_54 )
F_48 ( V_63 . V_54 ) ;
}
