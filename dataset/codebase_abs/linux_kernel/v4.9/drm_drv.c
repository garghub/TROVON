void F_1 ( const struct V_1 * V_2 , const char * V_3 ,
unsigned int V_4 , const char * V_5 ,
const char * V_6 , const char * V_7 , ... )
{
struct V_8 V_9 ;
T_1 args ;
if ( V_4 != V_10 && ! ( V_11 & V_4 ) )
return;
va_start ( args , V_7 ) ;
V_9 . V_12 = V_7 ;
V_9 . V_13 = & args ;
if ( V_2 )
F_2 ( V_3 , V_2 , V_14 , V_5 , V_6 ,
& V_9 ) ;
else
F_3 ( L_1 V_14 , V_3 , V_5 , V_6 , & V_9 ) ;
va_end ( args ) ;
}
void F_4 ( const char * V_3 , unsigned int V_4 ,
const char * V_7 , ... )
{
struct V_8 V_9 ;
T_1 args ;
if ( V_4 != V_10 && ! ( V_11 & V_4 ) )
return;
va_start ( args , V_7 ) ;
V_9 . V_12 = V_7 ;
V_9 . V_13 = & args ;
F_3 ( L_1 L_2 V_15 L_3 ,
V_3 , F_5 ( 0 ) ,
strcmp ( V_3 , V_16 ) == 0 ? L_4 : L_5 , & V_9 ) ;
va_end ( args ) ;
}
static struct V_17 * * F_6 ( struct V_18 * V_2 ,
unsigned int type )
{
switch ( type ) {
case V_19 :
return & V_2 -> V_20 ;
case V_21 :
return & V_2 -> V_22 ;
case V_23 :
return & V_2 -> V_24 ;
default:
return NULL ;
}
}
static int F_7 ( struct V_18 * V_2 , unsigned int type )
{
struct V_17 * V_25 ;
unsigned long V_26 ;
int V_27 ;
V_25 = F_8 ( sizeof( * V_25 ) , V_28 ) ;
if ( ! V_25 )
return - V_29 ;
V_25 -> type = type ;
V_25 -> V_2 = V_2 ;
F_9 ( V_28 ) ;
F_10 ( & V_30 , V_26 ) ;
V_27 = F_11 ( & V_31 ,
NULL ,
64 * type ,
64 * ( type + 1 ) ,
V_32 ) ;
F_12 ( & V_30 , V_26 ) ;
F_13 () ;
if ( V_27 < 0 )
goto V_33;
V_25 -> V_34 = V_27 ;
V_25 -> V_35 = F_14 ( V_25 ) ;
if ( F_15 ( V_25 -> V_35 ) ) {
V_27 = F_16 ( V_25 -> V_35 ) ;
goto V_36;
}
* F_6 ( V_2 , type ) = V_25 ;
return 0 ;
V_36:
F_10 ( & V_30 , V_26 ) ;
F_17 ( & V_31 , V_25 -> V_34 ) ;
F_12 ( & V_30 , V_26 ) ;
V_33:
F_18 ( V_25 ) ;
return V_27 ;
}
static void F_19 ( struct V_18 * V_2 , unsigned int type )
{
struct V_17 * * V_37 , * V_25 ;
unsigned long V_26 ;
V_37 = F_6 ( V_2 , type ) ;
V_25 = * V_37 ;
if ( ! V_25 )
return;
F_20 ( V_25 -> V_35 ) ;
F_10 ( & V_30 , V_26 ) ;
F_17 ( & V_31 , V_25 -> V_34 ) ;
F_12 ( & V_30 , V_26 ) ;
F_18 ( V_25 ) ;
* V_37 = NULL ;
}
static int F_21 ( struct V_18 * V_2 , unsigned int type )
{
struct V_17 * V_25 ;
unsigned long V_26 ;
int V_38 ;
F_22 ( L_6 ) ;
V_25 = * F_6 ( V_2 , type ) ;
if ( ! V_25 )
return 0 ;
V_38 = F_23 ( V_25 , V_25 -> V_34 , V_39 ) ;
if ( V_38 ) {
F_24 ( L_7 ) ;
return V_38 ;
}
V_38 = F_25 ( V_25 -> V_35 ) ;
if ( V_38 )
goto V_40;
F_10 ( & V_30 , V_26 ) ;
F_26 ( & V_31 , V_25 , V_25 -> V_34 ) ;
F_12 ( & V_30 , V_26 ) ;
F_22 ( L_8 , V_25 -> V_34 ) ;
return 0 ;
V_40:
F_27 ( V_25 ) ;
return V_38 ;
}
static void F_28 ( struct V_18 * V_2 , unsigned int type )
{
struct V_17 * V_25 ;
unsigned long V_26 ;
V_25 = * F_6 ( V_2 , type ) ;
if ( ! V_25 || ! F_29 ( V_25 -> V_35 ) )
return;
F_10 ( & V_30 , V_26 ) ;
F_26 ( & V_31 , NULL , V_25 -> V_34 ) ;
F_12 ( & V_30 , V_26 ) ;
F_30 ( V_25 -> V_35 ) ;
F_31 ( V_25 -> V_35 , NULL ) ;
F_27 ( V_25 ) ;
}
struct V_17 * F_32 ( unsigned int V_41 )
{
struct V_17 * V_25 ;
unsigned long V_26 ;
F_10 ( & V_30 , V_26 ) ;
V_25 = F_33 ( & V_31 , V_41 ) ;
if ( V_25 )
F_34 ( V_25 -> V_2 ) ;
F_12 ( & V_30 , V_26 ) ;
if ( ! V_25 ) {
return F_35 ( - V_42 ) ;
} else if ( F_36 ( V_25 -> V_2 ) ) {
F_37 ( V_25 -> V_2 ) ;
return F_35 ( - V_42 ) ;
}
return V_25 ;
}
void F_38 ( struct V_17 * V_25 )
{
F_37 ( V_25 -> V_2 ) ;
}
static int F_39 ( struct V_18 * V_2 , const char * V_43 )
{
if ( ! V_43 )
return - V_44 ;
F_18 ( V_2 -> V_45 ) ;
V_2 -> V_45 = F_40 ( V_43 , V_28 ) ;
return V_2 -> V_45 ? 0 : - V_29 ;
}
void F_41 ( struct V_18 * V_2 )
{
F_22 ( L_6 ) ;
if ( ! V_2 ) {
F_24 ( L_9 ) ;
return;
}
F_42 ( V_2 ) ;
F_37 ( V_2 ) ;
}
void F_43 ( struct V_18 * V_2 )
{
F_42 ( V_2 ) ;
F_44 ( & V_46 ) ;
F_45 ( V_2 ) ;
if ( V_2 -> V_47 == 0 ) {
F_41 ( V_2 ) ;
}
F_46 ( & V_46 ) ;
}
static struct V_48 * F_47 ( struct V_49 * V_50 , int V_26 ,
const char * V_51 , void * V_52 )
{
return F_48 ( V_50 ,
L_10 ,
& V_53 ,
& V_54 ,
0x010203ff ) ;
}
static struct V_55 * F_49 ( void )
{
struct V_55 * V_55 ;
int V_27 ;
V_27 = F_50 ( & V_56 , & V_57 , & V_58 ) ;
if ( V_27 < 0 ) {
F_24 ( L_11 , V_27 ) ;
return F_35 ( V_27 ) ;
}
V_55 = F_51 ( V_57 -> V_59 ) ;
if ( F_15 ( V_55 ) )
F_52 ( & V_57 , & V_58 ) ;
return V_55 ;
}
static void F_53 ( struct V_55 * V_55 )
{
if ( V_55 ) {
F_54 ( V_55 ) ;
F_52 ( & V_57 , & V_58 ) ;
}
}
int F_55 ( struct V_18 * V_2 ,
struct V_60 * V_61 ,
struct V_1 * V_62 )
{
int V_38 ;
F_56 ( & V_2 -> V_63 ) ;
V_2 -> V_2 = V_62 ;
V_2 -> V_61 = V_61 ;
F_57 ( & V_2 -> V_64 ) ;
F_57 ( & V_2 -> V_65 ) ;
F_57 ( & V_2 -> V_66 ) ;
F_57 ( & V_2 -> V_67 ) ;
F_57 ( & V_2 -> V_68 ) ;
F_58 ( & V_2 -> V_69 ) ;
F_58 ( & V_2 -> V_70 ) ;
F_59 ( & V_2 -> V_71 ) ;
F_59 ( & V_2 -> V_72 ) ;
F_59 ( & V_2 -> V_73 ) ;
F_59 ( & V_2 -> V_74 ) ;
V_2 -> V_75 = F_49 () ;
if ( F_15 ( V_2 -> V_75 ) ) {
V_38 = F_16 ( V_2 -> V_75 ) ;
F_24 ( L_12 , V_38 ) ;
goto V_33;
}
if ( F_60 ( V_2 , V_76 ) ) {
V_38 = F_7 ( V_2 , V_23 ) ;
if ( V_38 )
goto V_77;
}
if ( F_60 ( V_2 , V_78 ) ) {
V_38 = F_7 ( V_2 , V_21 ) ;
if ( V_38 )
goto V_77;
}
V_38 = F_7 ( V_2 , V_19 ) ;
if ( V_38 )
goto V_77;
V_38 = F_61 ( & V_2 -> V_79 , 12 ) ;
if ( V_38 )
goto V_77;
F_62 ( V_2 ) ;
if ( F_60 ( V_2 , V_80 ) ) {
V_38 = F_63 ( V_2 ) ;
if ( V_38 ) {
F_24 ( L_13 ) ;
goto V_81;
}
}
V_38 = F_39 ( V_2 , V_62 ? V_51 ( V_62 ) : V_61 -> V_43 ) ;
if ( V_38 )
goto V_82;
return 0 ;
V_82:
if ( F_60 ( V_2 , V_80 ) )
F_64 ( V_2 ) ;
V_81:
F_65 ( V_2 ) ;
F_66 ( & V_2 -> V_79 ) ;
V_77:
F_19 ( V_2 , V_19 ) ;
F_19 ( V_2 , V_21 ) ;
F_19 ( V_2 , V_23 ) ;
F_53 ( V_2 -> V_75 ) ;
V_33:
F_67 ( & V_2 -> V_74 ) ;
return V_38 ;
}
struct V_18 * F_68 ( struct V_60 * V_61 ,
struct V_1 * V_62 )
{
struct V_18 * V_2 ;
int V_38 ;
V_2 = F_8 ( sizeof( * V_2 ) , V_28 ) ;
if ( ! V_2 )
return F_35 ( - V_29 ) ;
V_38 = F_55 ( V_2 , V_61 , V_62 ) ;
if ( V_38 ) {
F_18 ( V_2 ) ;
return F_35 ( V_38 ) ;
}
return V_2 ;
}
static void F_69 ( struct V_83 * V_63 )
{
struct V_18 * V_2 = F_70 ( V_63 , struct V_18 , V_63 ) ;
if ( F_60 ( V_2 , V_80 ) )
F_64 ( V_2 ) ;
F_65 ( V_2 ) ;
F_66 ( & V_2 -> V_79 ) ;
F_53 ( V_2 -> V_75 ) ;
F_19 ( V_2 , V_19 ) ;
F_19 ( V_2 , V_21 ) ;
F_19 ( V_2 , V_23 ) ;
F_67 ( & V_2 -> V_74 ) ;
F_18 ( V_2 -> V_45 ) ;
F_18 ( V_2 ) ;
}
void F_34 ( struct V_18 * V_2 )
{
if ( V_2 )
F_71 ( & V_2 -> V_63 ) ;
}
void F_37 ( struct V_18 * V_2 )
{
if ( V_2 )
F_72 ( & V_2 -> V_63 , F_69 ) ;
}
int F_73 ( struct V_18 * V_2 , unsigned long V_26 )
{
int V_38 ;
F_44 ( & V_46 ) ;
V_38 = F_21 ( V_2 , V_23 ) ;
if ( V_38 )
goto V_77;
V_38 = F_21 ( V_2 , V_21 ) ;
if ( V_38 )
goto V_77;
V_38 = F_21 ( V_2 , V_19 ) ;
if ( V_38 )
goto V_77;
if ( V_2 -> V_61 -> V_84 ) {
V_38 = V_2 -> V_61 -> V_84 ( V_2 , V_26 ) ;
if ( V_38 )
goto V_77;
}
if ( F_60 ( V_2 , V_76 ) )
F_74 ( V_2 ) ;
V_38 = 0 ;
goto V_85;
V_77:
F_28 ( V_2 , V_19 ) ;
F_28 ( V_2 , V_21 ) ;
F_28 ( V_2 , V_23 ) ;
V_85:
F_46 ( & V_46 ) ;
return V_38 ;
}
void F_42 ( struct V_18 * V_2 )
{
struct V_86 * V_87 , * V_88 ;
F_75 ( V_2 ) ;
if ( F_60 ( V_2 , V_76 ) )
F_76 ( V_2 ) ;
if ( V_2 -> V_61 -> V_89 )
V_2 -> V_61 -> V_89 ( V_2 ) ;
if ( V_2 -> V_90 )
F_77 ( V_2 ) ;
F_78 ( V_2 ) ;
F_79 (r_list, list_temp, &dev->maplist, head)
F_80 ( V_2 , V_87 -> V_91 ) ;
F_28 ( V_2 , V_19 ) ;
F_28 ( V_2 , V_21 ) ;
F_28 ( V_2 , V_23 ) ;
}
static int F_81 ( struct V_55 * V_55 , struct V_92 * V_93 )
{
const struct V_94 * V_95 ;
struct V_17 * V_25 ;
int V_96 ;
F_22 ( L_6 ) ;
F_44 ( & V_46 ) ;
V_25 = F_32 ( F_82 ( V_55 ) ) ;
if ( F_15 ( V_25 ) ) {
V_96 = F_16 ( V_25 ) ;
goto V_85;
}
V_95 = F_83 ( V_25 -> V_2 -> V_61 -> V_97 ) ;
if ( ! V_95 ) {
V_96 = - V_42 ;
goto V_98;
}
F_84 ( V_93 , V_95 ) ;
if ( V_93 -> V_99 -> V_100 )
V_96 = V_93 -> V_99 -> V_100 ( V_55 , V_93 ) ;
else
V_96 = 0 ;
V_98:
F_38 ( V_25 ) ;
V_85:
F_46 ( & V_46 ) ;
return V_96 ;
}
static void F_85 ( void )
{
F_86 ( V_101 , L_14 ) ;
F_87 ( V_39 ) ;
F_88 () ;
F_89 ( & V_31 ) ;
F_90 () ;
F_91 () ;
}
static int T_2 F_92 ( void )
{
int V_38 ;
F_93 () ;
F_94 () ;
F_95 ( & V_31 ) ;
V_38 = F_96 () ;
if ( V_38 < 0 ) {
F_24 ( L_15 , V_38 ) ;
goto error;
}
V_39 = F_97 ( L_16 , NULL ) ;
if ( ! V_39 ) {
V_38 = - V_29 ;
F_24 ( L_17 , V_38 ) ;
goto error;
}
V_38 = F_98 ( V_101 , L_14 , & V_102 ) ;
if ( V_38 < 0 )
goto error;
F_99 ( L_18 ) ;
return 0 ;
error:
F_85 () ;
return V_38 ;
}
