static struct V_1 * F_1 ( struct V_2 * V_3 ,
unsigned long V_4 ,
unsigned long V_5 )
{
struct V_6 * V_7 = V_3 -> V_8 . V_6 ;
unsigned long V_9 ;
struct V_1 * V_10 ;
struct V_1 * V_11 = NULL ;
while ( F_2 ( V_7 != NULL ) ) {
V_10 = F_3 ( V_7 , struct V_1 , V_12 ) ;
V_9 = V_10 -> V_13 -> V_14 ;
if ( V_4 >= V_9 ) {
V_7 = V_7 -> V_15 ;
V_11 = V_10 ;
if ( V_4 == V_9 )
break;
} else
V_7 = V_7 -> V_16 ;
}
if ( F_4 ( V_11 == NULL ) )
return NULL ;
if ( F_4 ( ( V_11 -> V_13 -> V_14 + V_11 -> V_5 ) <
( V_4 + V_5 ) ) )
return NULL ;
return V_11 ;
}
static int F_5 ( struct V_17 * V_18 , struct V_19 * V_20 )
{
struct V_1 * V_10 = (struct V_1 * )
V_18 -> V_21 ;
struct V_2 * V_3 = V_10 -> V_3 ;
unsigned long V_22 ;
unsigned long V_23 ;
unsigned long V_24 ;
struct V_25 * V_26 = NULL ;
struct V_27 * V_27 ;
int V_28 ;
int V_29 ;
unsigned long V_30 = ( unsigned long ) V_20 -> V_31 ;
int V_32 = V_33 ;
struct V_34 * V_35 =
& V_3 -> V_35 [ V_10 -> V_36 . V_37 ] ;
V_28 = F_6 ( V_10 , true , true , false , 0 ) ;
if ( F_4 ( V_28 != 0 ) ) {
if ( V_28 == - V_38 )
F_7 () ;
return V_33 ;
}
if ( V_3 -> V_39 -> V_40 ) {
V_28 = V_3 -> V_39 -> V_40 ( V_10 ) ;
switch ( V_28 ) {
case 0 :
break;
case - V_38 :
F_7 () ;
case - V_41 :
V_32 = V_33 ;
goto V_42;
default:
V_32 = V_43 ;
goto V_42;
}
}
F_8 ( & V_3 -> V_44 ) ;
if ( F_9 ( V_45 , & V_10 -> V_46 ) ) {
V_28 = F_10 ( V_10 , false , true , false ) ;
F_11 ( & V_3 -> V_44 ) ;
if ( F_4 ( V_28 != 0 ) ) {
V_32 = ( V_28 != - V_41 ) ?
V_43 : V_33 ;
goto V_42;
}
} else
F_11 ( & V_3 -> V_44 ) ;
V_28 = F_12 ( V_35 , true ) ;
if ( F_4 ( V_28 != 0 ) ) {
V_32 = V_33 ;
goto V_42;
}
V_28 = F_13 ( V_10 ) ;
if ( F_4 ( V_28 != 0 ) ) {
V_32 = V_43 ;
goto V_47;
}
V_22 = ( ( V_30 - V_18 -> V_48 ) >> V_49 ) +
V_10 -> V_13 -> V_14 - V_18 -> V_50 ;
V_23 = ( ( V_18 -> V_51 - V_18 -> V_48 ) >> V_49 ) +
V_10 -> V_13 -> V_14 - V_18 -> V_50 ;
if ( F_4 ( V_22 >= V_10 -> V_5 ) ) {
V_32 = V_43 ;
goto V_47;
}
if ( V_10 -> V_36 . V_52 . V_53 ) {
V_18 -> V_54 = F_14 ( V_10 -> V_36 . V_55 ,
V_18 -> V_54 ) ;
} else {
V_26 = V_10 -> V_26 ;
V_18 -> V_54 = ( V_10 -> V_36 . V_55 & V_56 ) ?
F_15 ( V_18 -> V_57 ) :
F_14 ( V_10 -> V_36 . V_55 , V_18 -> V_54 ) ;
if ( V_26 -> V_3 -> V_39 -> V_58 ( V_26 ) ) {
V_32 = V_59 ;
goto V_47;
}
}
for ( V_29 = 0 ; V_29 < V_60 ; ++ V_29 ) {
if ( V_10 -> V_36 . V_52 . V_53 )
V_24 = ( ( V_10 -> V_36 . V_52 . V_61 + V_10 -> V_36 . V_52 . V_62 ) >> V_49 ) + V_22 ;
else {
V_27 = V_26 -> V_63 [ V_22 ] ;
if ( F_4 ( ! V_27 && V_29 == 0 ) ) {
V_32 = V_59 ;
goto V_47;
} else if ( F_4 ( ! V_27 ) ) {
break;
}
V_24 = F_16 ( V_27 ) ;
}
V_28 = F_17 ( V_18 , V_30 , V_24 ) ;
if ( F_4 ( ( V_28 == - V_38 ) || ( V_28 != 0 && V_29 > 0 ) ) )
break;
else if ( F_4 ( V_28 != 0 ) ) {
V_32 =
( V_28 == - V_64 ) ? V_59 : V_43 ;
goto V_47;
}
V_30 += V_65 ;
if ( F_4 ( ++ V_22 >= V_23 ) )
break;
}
V_47:
F_18 ( V_35 ) ;
V_42:
F_19 ( V_10 ) ;
return V_32 ;
}
static void F_20 ( struct V_17 * V_18 )
{
struct V_1 * V_10 =
(struct V_1 * ) V_18 -> V_21 ;
( void ) F_21 ( V_10 ) ;
}
static void F_22 ( struct V_17 * V_18 )
{
struct V_1 * V_10 = (struct V_1 * ) V_18 -> V_21 ;
F_23 ( & V_10 ) ;
V_18 -> V_21 = NULL ;
}
int F_24 ( struct V_66 * V_67 , struct V_17 * V_18 ,
struct V_2 * V_3 )
{
struct V_68 * V_39 ;
struct V_1 * V_10 ;
int V_28 ;
F_25 ( & V_3 -> V_69 ) ;
V_10 = F_1 ( V_3 , V_18 -> V_50 ,
( V_18 -> V_51 - V_18 -> V_48 ) >> V_49 ) ;
if ( F_2 ( V_10 != NULL ) && ! F_26 ( & V_10 -> V_70 ) )
V_10 = NULL ;
F_27 ( & V_3 -> V_69 ) ;
if ( F_4 ( V_10 == NULL ) ) {
F_28 ( L_1 ) ;
return - V_71 ;
}
V_39 = V_10 -> V_3 -> V_39 ;
if ( F_4 ( ! V_39 -> V_72 ) ) {
V_28 = - V_73 ;
goto V_74;
}
V_28 = V_39 -> V_72 ( V_10 , V_67 ) ;
if ( F_4 ( V_28 != 0 ) )
goto V_74;
V_18 -> V_75 = & V_76 ;
V_18 -> V_21 = V_10 ;
V_18 -> V_57 |= V_77 | V_78 | V_79 | V_80 ;
return 0 ;
V_74:
F_23 ( & V_10 ) ;
return V_28 ;
}
int F_29 ( struct V_17 * V_18 , struct V_1 * V_10 )
{
if ( V_18 -> V_50 != 0 )
return - V_81 ;
V_18 -> V_75 = & V_76 ;
V_18 -> V_21 = F_21 ( V_10 ) ;
V_18 -> V_57 |= V_77 | V_78 | V_79 ;
return 0 ;
}
T_1 F_30 ( struct V_2 * V_3 , struct V_66 * V_67 ,
const char T_2 * V_82 , char T_2 * V_83 , T_3 V_84 ,
T_4 * V_85 , bool V_86 )
{
struct V_1 * V_10 ;
struct V_68 * V_39 ;
struct V_87 V_88 ;
unsigned long V_89 = ( * V_85 >> V_49 ) ;
unsigned long V_90 ;
unsigned long V_91 ;
unsigned long V_92 ;
T_3 V_93 ;
unsigned int V_22 ;
char * V_94 ;
int V_28 ;
bool V_95 = false ;
bool V_96 ;
F_25 ( & V_3 -> V_69 ) ;
V_10 = F_1 ( V_3 , V_89 , 1 ) ;
if ( F_2 ( V_10 != NULL ) )
F_21 ( V_10 ) ;
F_27 ( & V_3 -> V_69 ) ;
if ( F_4 ( V_10 == NULL ) )
return - V_97 ;
V_39 = V_10 -> V_3 -> V_39 ;
if ( F_4 ( ! V_39 -> V_72 ) ) {
V_28 = - V_73 ;
goto V_74;
}
V_28 = V_39 -> V_72 ( V_10 , V_67 ) ;
if ( F_4 ( V_28 != 0 ) )
goto V_74;
V_90 = V_89 - V_10 -> V_13 -> V_14 ;
if ( F_4 ( V_90 >= V_10 -> V_5 ) ) {
V_28 = - V_98 ;
goto V_74;
}
V_22 = * V_85 & ~ V_99 ;
V_93 = V_10 -> V_5 - V_90 ;
V_93 = ( V_93 << V_49 ) - V_22 ;
if ( V_84 < V_93 )
V_93 = V_84 ;
V_91 = ( * V_85 + V_84 - 1 ) >> V_49 ;
V_92 = V_91 - V_90 + 1 ;
V_28 = F_6 ( V_10 , true , V_95 , false , 0 ) ;
switch ( V_28 ) {
case 0 :
break;
case - V_38 :
V_28 = - V_100 ;
goto V_74;
default:
goto V_74;
}
V_28 = F_31 ( V_10 , V_90 , V_92 , & V_88 ) ;
if ( F_4 ( V_28 != 0 ) ) {
F_19 ( V_10 ) ;
goto V_74;
}
V_94 = F_32 ( & V_88 , & V_96 ) ;
V_94 += V_22 ;
if ( V_86 )
V_28 = F_33 ( V_94 , V_82 , V_93 ) ;
else
V_28 = F_34 ( V_83 , V_94 , V_93 ) ;
F_35 ( & V_88 ) ;
F_19 ( V_10 ) ;
F_23 ( & V_10 ) ;
if ( F_4 ( V_28 != 0 ) )
return - V_98 ;
* V_85 += V_93 ;
return V_93 ;
V_74:
F_23 ( & V_10 ) ;
return V_28 ;
}
T_1 F_36 ( struct V_1 * V_10 , const char T_2 * V_82 ,
char T_2 * V_83 , T_3 V_84 , T_4 * V_85 ,
bool V_86 )
{
struct V_87 V_88 ;
unsigned long V_90 ;
unsigned long V_91 ;
unsigned long V_92 ;
T_3 V_93 ;
unsigned int V_22 ;
char * V_94 ;
int V_28 ;
bool V_95 = false ;
bool V_96 ;
V_90 = ( * V_85 >> V_49 ) ;
if ( F_4 ( V_90 >= V_10 -> V_5 ) )
return - V_98 ;
V_22 = * V_85 & ~ V_99 ;
V_93 = V_10 -> V_5 - V_90 ;
V_93 = ( V_93 << V_49 ) - V_22 ;
if ( V_84 < V_93 )
V_93 = V_84 ;
V_91 = ( * V_85 + V_84 - 1 ) >> V_49 ;
V_92 = V_91 - V_90 + 1 ;
V_28 = F_6 ( V_10 , true , V_95 , false , 0 ) ;
switch ( V_28 ) {
case 0 :
break;
case - V_38 :
return - V_100 ;
default:
return V_28 ;
}
V_28 = F_31 ( V_10 , V_90 , V_92 , & V_88 ) ;
if ( F_4 ( V_28 != 0 ) ) {
F_19 ( V_10 ) ;
return V_28 ;
}
V_94 = F_32 ( & V_88 , & V_96 ) ;
V_94 += V_22 ;
if ( V_86 )
V_28 = F_33 ( V_94 , V_82 , V_93 ) ;
else
V_28 = F_34 ( V_83 , V_94 , V_93 ) ;
F_35 ( & V_88 ) ;
F_19 ( V_10 ) ;
F_23 ( & V_10 ) ;
if ( F_4 ( V_28 != 0 ) )
return V_28 ;
* V_85 += V_93 ;
return V_93 ;
}
