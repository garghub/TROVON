static struct V_1 *
F_1 ( struct V_2 * V_3 ,
const T_1 * V_4 ,
struct V_5 * V_6 ,
struct V_5 * V_7 , T_2 V_8 )
{
struct V_1 * V_9 ;
V_9 = F_2 ( V_3 , V_4 ,
F_3 ( V_8 ) ) ;
if ( ! V_9 )
goto V_10;
F_4 ( & V_9 -> V_11 ) ;
V_9 -> V_6 = V_7 ;
V_9 -> V_12 = V_3 ;
F_5 ( & V_6 -> V_13 ) ;
F_6 ( & V_9 -> V_14 , & V_6 -> V_15 ) ;
F_7 ( & V_6 -> V_13 ) ;
V_10:
return V_9 ;
}
static int F_8 ( struct V_2 * V_3 )
{
struct V_16 * V_16 ;
unsigned char * V_17 ;
T_3 V_18 ;
int V_19 = - V_20 ;
F_9 ( & V_18 , sizeof( V_18 ) ) ;
F_10 ( & V_3 -> V_21 . V_22 , V_18 ) ;
V_3 -> V_21 . V_23 = V_24 ;
V_17 = F_11 ( V_3 -> V_21 . V_23 , V_25 ) ;
if ( ! V_17 )
goto V_10;
V_3 -> V_21 . V_17 = V_17 ;
V_16 = (struct V_16 * ) V_17 ;
V_16 -> V_26 . V_27 = V_28 ;
V_16 -> V_26 . V_29 = V_30 ;
V_16 -> V_26 . V_31 = 2 ;
V_16 -> V_32 = V_33 ;
V_16 -> V_34 = V_35 ;
V_16 -> V_36 = 0 ;
V_16 -> V_37 = 0 ;
V_19 = 0 ;
V_10:
return V_19 ;
}
static void F_12 ( struct V_2 * V_3 )
{
F_13 ( V_3 -> V_21 . V_17 ) ;
V_3 -> V_21 . V_17 = NULL ;
}
static void F_14 ( struct V_2 * V_3 )
{
struct V_16 * V_16 ;
unsigned char * V_17 = V_3 -> V_21 . V_17 ;
V_16 = (struct V_16 * ) V_17 ;
memcpy ( V_16 -> V_38 ,
V_3 -> V_39 -> V_40 , V_41 ) ;
memcpy ( V_16 -> V_42 ,
V_3 -> V_39 -> V_40 , V_41 ) ;
}
static void
F_15 ( struct V_2 * V_3 )
{
struct V_16 * V_16 ;
unsigned char * V_17 = V_3 -> V_21 . V_17 ;
V_16 = (struct V_16 * ) V_17 ;
V_16 -> V_32 = V_43 ;
V_16 -> V_26 . V_31 = V_44 ;
}
static unsigned long
F_16 ( const struct V_45 * V_46 )
{
unsigned int V_47 ;
V_47 = F_17 ( & V_46 -> V_48 ) - V_49 ;
V_47 += F_18 () % ( 2 * V_49 ) ;
return V_50 + F_19 ( V_47 ) ;
}
static unsigned long F_20 ( void )
{
return V_50 + F_19 ( F_18 () % ( V_49 / 2 ) ) ;
}
static T_1 F_21 ( T_1 V_34 ,
const struct V_45 * V_46 )
{
int V_51 = F_17 ( & V_46 -> V_51 ) ;
int V_52 ;
V_52 = V_34 * ( V_35 - V_51 ) ;
V_52 /= V_35 ;
return V_52 ;
}
static int F_22 ( int V_53 , int V_54 ,
int V_36 )
{
int V_55 = 0 ;
V_55 += V_53 + V_24 ;
V_55 += F_23 ( V_36 ) ;
return ( V_55 <= V_54 ) &&
( V_55 <= V_56 ) ;
}
static void F_24 ( struct V_57 * V_58 ,
struct V_2 * V_3 )
{
struct V_45 * V_46 = F_25 ( V_3 -> V_59 ) ;
char * V_60 ;
T_1 V_61 ;
T_4 V_53 ;
struct V_16 * V_16 ;
struct V_62 * V_63 ;
T_1 * V_64 ;
if ( V_3 -> V_65 != V_66 )
return;
V_61 = 0 ;
V_53 = 0 ;
V_64 = V_58 -> V_63 -> V_67 ;
V_16 = (struct V_16 * ) V_64 ;
while ( F_22 ( V_53 , V_58 -> V_54 ,
V_16 -> V_36 ) ) {
if ( V_58 -> V_68 & F_26 ( V_61 ) &&
V_58 -> V_12 == V_3 )
V_16 -> V_32 |= V_69 ;
else
V_16 -> V_32 &= ~ V_69 ;
if ( V_61 > 0 || ! V_58 -> V_70 )
V_60 = L_1 ;
else
V_60 = L_2 ;
F_27 ( V_71 , V_46 ,
L_3 ,
V_60 , ( V_61 > 0 ? L_4 : L_5 ) ,
V_16 -> V_38 ,
F_3 ( V_16 -> V_8 ) ,
V_16 -> V_34 , V_16 -> V_26 . V_31 ,
( V_16 -> V_32 & V_69 ?
L_6 : L_7 ) ,
V_16 -> V_37 , V_3 -> V_39 -> V_72 ,
V_3 -> V_39 -> V_40 ) ;
V_53 += V_24 ;
V_53 += F_23 ( V_16 -> V_36 ) ;
V_61 ++ ;
V_64 = V_58 -> V_63 -> V_67 + V_53 ;
V_16 = (struct V_16 * ) V_64 ;
}
V_63 = F_28 ( V_58 -> V_63 , V_25 ) ;
if ( V_63 ) {
F_29 ( V_46 , V_73 ) ;
F_30 ( V_46 , V_74 ,
V_63 -> V_75 + V_76 ) ;
F_31 ( V_63 , V_3 , V_77 ) ;
}
}
static void F_32 ( struct V_57 * V_58 )
{
struct V_2 * V_3 ;
struct V_78 * V_59 ;
struct V_45 * V_46 ;
struct V_2 * V_79 = NULL ;
struct V_16 * V_16 ;
unsigned char V_80 ;
T_1 * V_64 ;
V_64 = V_58 -> V_63 -> V_67 ;
V_16 = (struct V_16 * ) V_64 ;
V_80 = ( V_16 -> V_32 & V_69 ? 1 : 0 ) ;
if ( ! V_58 -> V_12 ) {
F_33 ( L_8 ) ;
goto V_10;
}
V_59 = V_58 -> V_12 -> V_59 ;
V_46 = F_25 ( V_59 ) ;
if ( V_58 -> V_12 -> V_65 != V_66 )
goto V_10;
V_79 = F_34 ( V_46 ) ;
if ( ! V_79 )
goto V_10;
if ( ( V_80 && ( V_16 -> V_26 . V_31 == 1 ) ) ||
( V_58 -> V_70 && ( V_58 -> V_12 != V_79 ) ) ) {
F_27 ( V_71 , V_46 ,
L_9 ,
( V_58 -> V_70 ? L_2 : L_1 ) ,
V_16 -> V_38 ,
F_3 ( V_16 -> V_8 ) ,
V_16 -> V_26 . V_31 ,
V_58 -> V_12 -> V_39 -> V_72 ,
V_58 -> V_12 -> V_39 -> V_40 ) ;
F_31 ( V_58 -> V_63 ,
V_58 -> V_12 ,
V_77 ) ;
V_58 -> V_63 = NULL ;
goto V_10;
}
F_35 () ;
F_36 (hard_iface, &batadv_hardif_list, list) {
if ( V_3 -> V_59 != V_59 )
continue;
F_24 ( V_58 , V_3 ) ;
}
F_37 () ;
V_10:
if ( V_79 )
F_38 ( V_79 ) ;
}
static bool
F_39 ( const struct V_16 * V_81 ,
struct V_45 * V_46 ,
int V_54 , unsigned long V_82 ,
bool V_80 ,
const struct V_2 * V_12 ,
const struct V_57 * V_58 )
{
struct V_16 * V_16 ;
int V_83 = V_58 -> V_54 + V_54 ;
struct V_2 * V_79 = NULL ;
bool V_19 = false ;
unsigned long V_84 ;
V_16 = (struct V_16 * ) V_58 -> V_63 -> V_67 ;
V_84 = V_82 ;
V_84 += F_19 ( V_85 ) ;
if ( F_40 ( V_82 , V_58 -> V_82 ) &&
F_41 ( V_84 , V_58 -> V_82 ) &&
( V_83 <= V_56 ) ) {
V_79 = F_34 ( V_46 ) ;
if ( ! V_79 )
goto V_10;
if ( ( ! V_80 ) &&
( ! ( V_16 -> V_32 & V_69 ) ) &&
( V_16 -> V_26 . V_31 != 1 ) &&
( ( ! V_58 -> V_70 ) ||
( V_58 -> V_12 == V_79 ) ) ) {
V_19 = true ;
goto V_10;
}
if ( ( V_80 ) &&
( V_81 -> V_26 . V_31 == 1 ) &&
( V_58 -> V_12 == V_12 ) &&
( V_16 -> V_32 & V_69 ||
( V_58 -> V_70 &&
V_58 -> V_12 != V_79 ) ) ) {
V_19 = true ;
goto V_10;
}
}
V_10:
if ( V_79 )
F_38 ( V_79 ) ;
return V_19 ;
}
static void F_42 ( const unsigned char * V_86 ,
int V_54 , unsigned long V_82 ,
bool V_87 ,
struct V_2 * V_12 ,
int V_88 )
{
struct V_45 * V_46 = F_25 ( V_12 -> V_59 ) ;
struct V_57 * V_89 ;
unsigned char * V_90 ;
unsigned int V_91 ;
if ( ! F_43 ( & V_12 -> V_92 ) )
return;
if ( ! V_88 ) {
if ( ! F_44 ( & V_46 -> V_93 ) ) {
F_27 ( V_71 , V_46 ,
L_10 ) ;
goto V_10;
}
}
V_89 = F_11 ( sizeof( * V_89 ) , V_25 ) ;
if ( ! V_89 ) {
if ( ! V_88 )
F_45 ( & V_46 -> V_93 ) ;
goto V_10;
}
if ( ( F_17 ( & V_46 -> V_94 ) ) &&
( V_54 < V_56 ) )
V_91 = V_56 ;
else
V_91 = V_54 ;
V_91 += V_76 + V_95 ;
V_89 -> V_63 = F_46 ( V_91 ) ;
if ( ! V_89 -> V_63 ) {
if ( ! V_88 )
F_45 ( & V_46 -> V_93 ) ;
F_13 ( V_89 ) ;
goto V_10;
}
F_47 ( V_89 -> V_63 , V_76 + V_95 ) ;
F_48 ( & V_89 -> V_14 ) ;
V_90 = F_49 ( V_89 -> V_63 , V_54 ) ;
V_89 -> V_54 = V_54 ;
memcpy ( V_90 , V_86 , V_54 ) ;
V_89 -> V_70 = V_88 ;
V_89 -> V_12 = V_12 ;
V_89 -> V_96 = 0 ;
V_89 -> V_68 = V_33 ;
V_89 -> V_82 = V_82 ;
if ( V_87 )
V_89 -> V_68 |= 1 ;
F_5 ( & V_46 -> V_97 ) ;
F_50 ( & V_89 -> V_14 , & V_46 -> V_98 ) ;
F_7 ( & V_46 -> V_97 ) ;
F_51 ( & V_89 -> V_99 ,
V_100 ) ;
F_52 ( V_101 ,
& V_89 -> V_99 ,
V_82 - V_50 ) ;
return;
V_10:
F_38 ( V_12 ) ;
}
static void F_53 ( struct V_57 * V_89 ,
const unsigned char * V_86 ,
int V_54 , bool V_87 )
{
unsigned char * V_90 ;
unsigned long V_102 ;
V_90 = F_49 ( V_89 -> V_63 , V_54 ) ;
memcpy ( V_90 , V_86 , V_54 ) ;
V_89 -> V_54 += V_54 ;
V_89 -> V_96 ++ ;
if ( V_87 ) {
V_102 = F_26 ( V_89 -> V_96 ) ;
V_89 -> V_68 |= V_102 ;
}
}
static void F_54 ( struct V_45 * V_46 ,
unsigned char * V_86 ,
int V_54 ,
struct V_2 * V_12 ,
int V_88 , unsigned long V_82 )
{
struct V_57 * V_89 = NULL ;
struct V_57 * V_103 = NULL ;
struct V_104 * V_105 ;
struct V_16 * V_16 ;
bool V_87 ;
unsigned long V_106 ;
V_16 = (struct V_16 * ) V_86 ;
V_87 = V_16 -> V_32 & V_69 ? 1 : 0 ;
V_106 = F_19 ( V_85 ) ;
F_5 ( & V_46 -> V_97 ) ;
if ( ( F_17 ( & V_46 -> V_94 ) ) && ( ! V_88 ) ) {
F_55 (forw_packet_pos, tmp_node,
&bat_priv->forw_bat_list, list) {
if ( F_39 ( V_16 ,
V_46 , V_54 ,
V_82 , V_87 ,
V_12 ,
V_103 ) ) {
V_89 = V_103 ;
break;
}
}
}
if ( ! V_89 ) {
F_7 ( & V_46 -> V_97 ) ;
if ( ! V_88 && F_17 ( & V_46 -> V_94 ) )
V_82 += V_106 ;
F_42 ( V_86 , V_54 ,
V_82 , V_87 ,
V_12 , V_88 ) ;
} else {
F_53 ( V_89 , V_86 ,
V_54 , V_87 ) ;
F_7 ( & V_46 -> V_97 ) ;
}
}
static void F_56 ( struct V_5 * V_6 ,
const struct V_107 * V_107 ,
struct V_16 * V_16 ,
bool V_108 ,
bool V_109 ,
struct V_2 * V_12 )
{
struct V_45 * V_46 = F_25 ( V_12 -> V_59 ) ;
T_1 V_36 ;
if ( V_16 -> V_26 . V_31 <= 1 ) {
F_27 ( V_71 , V_46 , L_11 ) ;
return;
}
if ( ! V_109 ) {
if ( V_108 )
V_16 -> V_32 |= V_110 ;
else
return;
}
V_36 = V_16 -> V_36 ;
V_16 -> V_26 . V_31 -- ;
memcpy ( V_16 -> V_42 , V_107 -> V_111 , V_41 ) ;
V_16 -> V_34 = F_21 ( V_16 -> V_34 ,
V_46 ) ;
F_27 ( V_71 , V_46 ,
L_12 ,
V_16 -> V_34 , V_16 -> V_26 . V_31 ) ;
V_16 -> V_32 &= ~ V_43 ;
if ( V_108 )
V_16 -> V_32 |= V_69 ;
else
V_16 -> V_32 &= ~ V_69 ;
F_54 ( V_46 , ( unsigned char * ) V_16 ,
V_24 + F_23 ( V_36 ) ,
V_12 , 0 , F_20 () ) ;
}
static void F_57 ( struct V_2 * V_3 )
{
struct V_45 * V_46 = F_25 ( V_3 -> V_59 ) ;
unsigned char * * V_17 = & V_3 -> V_21 . V_17 ;
struct V_16 * V_16 ;
struct V_2 * V_79 ;
int * V_23 = & V_3 -> V_21 . V_23 ;
int V_112 , V_36 = 0 ;
T_3 V_8 ;
T_1 V_113 ;
V_112 = F_17 ( & V_46 -> V_114 ) ;
V_79 = F_34 ( V_46 ) ;
if ( V_3 == V_79 )
V_36 = F_58 ( V_46 , V_17 ,
V_23 ,
V_24 ) ;
V_16 = (struct V_16 * ) ( * V_17 ) ;
V_8 = ( T_3 ) F_17 ( & V_3 -> V_21 . V_22 ) ;
V_16 -> V_8 = F_59 ( V_8 ) ;
F_45 ( & V_3 -> V_21 . V_22 ) ;
V_16 -> V_37 = F_17 ( & V_46 -> V_115 . V_116 ) ;
V_16 -> V_117 = F_60 ( V_46 -> V_115 . V_118 ) ;
if ( V_36 >= 0 )
V_16 -> V_36 = V_36 ;
if ( V_112 == V_119 )
V_16 -> V_32 |= V_120 ;
else
V_16 -> V_32 &= ~ V_120 ;
if ( V_3 == V_79 &&
F_17 ( & V_46 -> V_121 ) == V_122 ) {
V_113 = ( T_1 ) F_17 ( & V_46 -> V_123 ) ;
V_16 -> V_124 = V_113 ;
} else {
V_16 -> V_124 = V_33 ;
}
F_61 ( V_3 ) ;
F_54 ( V_46 , V_3 -> V_21 . V_17 ,
V_3 -> V_21 . V_23 , V_3 , 1 ,
F_16 ( V_46 ) ) ;
if ( V_79 )
F_38 ( V_79 ) ;
}
static void
F_62 ( struct V_45 * V_46 ,
struct V_5 * V_6 ,
const struct V_107 * V_107 ,
const struct V_16 * V_16 ,
struct V_2 * V_12 ,
const unsigned char * V_125 ,
int V_126 )
{
struct V_1 * V_9 = NULL , * V_127 = NULL ;
struct V_1 * V_128 = NULL ;
struct V_5 * V_129 ;
struct V_104 * V_130 ;
int V_131 ;
T_1 V_132 , V_133 ;
T_1 * V_4 ;
T_1 V_134 ;
F_27 ( V_71 , V_46 ,
L_13 ) ;
F_35 () ;
F_63 (tmp_neigh_node, node,
&orig_node->neigh_list, list) {
V_4 = V_127 -> V_135 ;
if ( F_64 ( V_4 , V_107 -> V_111 ) &&
V_127 -> V_12 == V_12 &&
F_43 ( & V_127 -> V_92 ) ) {
if ( V_9 )
F_65 ( V_9 ) ;
V_9 = V_127 ;
continue;
}
if ( V_126 )
continue;
F_5 ( & V_127 -> V_136 ) ;
F_66 ( V_127 -> V_137 ,
& V_127 -> V_138 , 0 ) ;
V_134 = F_67 ( V_127 -> V_137 ) ;
V_127 -> V_134 = V_134 ;
F_7 ( & V_127 -> V_136 ) ;
}
if ( ! V_9 ) {
struct V_5 * V_139 ;
V_139 = F_68 ( V_46 , V_107 -> V_111 ) ;
if ( ! V_139 )
goto V_140;
V_9 = F_1 ( V_12 ,
V_107 -> V_111 ,
V_6 , V_139 ,
V_16 -> V_8 ) ;
F_69 ( V_139 ) ;
if ( ! V_9 )
goto V_140;
} else
F_27 ( V_71 , V_46 ,
L_14 ) ;
F_37 () ;
V_6 -> V_32 = V_16 -> V_32 ;
V_9 -> V_141 = V_50 ;
F_5 ( & V_9 -> V_136 ) ;
F_66 ( V_9 -> V_137 ,
& V_9 -> V_138 ,
V_16 -> V_34 ) ;
V_9 -> V_134 = F_67 ( V_9 -> V_137 ) ;
F_7 ( & V_9 -> V_136 ) ;
if ( ! V_126 ) {
V_6 -> V_142 = V_16 -> V_26 . V_31 ;
V_9 -> V_142 = V_16 -> V_26 . V_31 ;
}
F_70 ( V_6 , V_9 ) ;
V_128 = F_71 ( V_6 ) ;
if ( V_128 == V_9 )
goto V_143;
if ( V_128 && ( V_128 -> V_134 > V_9 -> V_134 ) )
goto V_143;
if ( V_128 && ( V_9 -> V_134 == V_128 -> V_134 ) ) {
V_129 = V_128 -> V_6 ;
F_5 ( & V_129 -> V_144 ) ;
V_131 = V_128 -> V_12 -> V_131 ;
V_132 = V_129 -> V_145 [ V_131 ] ;
F_7 ( & V_129 -> V_144 ) ;
V_129 = V_9 -> V_6 ;
F_5 ( & V_129 -> V_144 ) ;
V_131 = V_9 -> V_12 -> V_131 ;
V_133 = V_129 -> V_145 [ V_131 ] ;
F_7 ( & V_129 -> V_144 ) ;
if ( V_132 >= V_133 )
goto V_143;
}
F_72 ( V_46 , V_6 , V_9 ) ;
V_143:
if ( ( ( V_16 -> V_38 != V_107 -> V_111 ) &&
( V_16 -> V_26 . V_31 > 2 ) ) ||
( V_16 -> V_32 & V_43 ) )
F_73 ( V_46 , V_6 , V_125 ,
V_16 -> V_36 ,
V_16 -> V_37 ,
F_74 ( V_16 -> V_117 ) ) ;
if ( V_6 -> V_124 != V_16 -> V_124 )
F_75 ( V_46 , V_6 ,
V_16 -> V_124 ) ;
V_6 -> V_124 = V_16 -> V_124 ;
if ( ( V_6 -> V_124 ) &&
( F_17 ( & V_46 -> V_121 ) == V_146 ) &&
( F_17 ( & V_46 -> V_147 ) > 2 ) )
F_76 ( V_46 , V_6 ) ;
goto V_10;
V_140:
F_37 () ;
V_10:
if ( V_9 )
F_65 ( V_9 ) ;
if ( V_128 )
F_65 ( V_128 ) ;
}
static int F_77 ( struct V_5 * V_6 ,
struct V_5 * V_148 ,
struct V_16 * V_16 ,
struct V_2 * V_12 )
{
struct V_45 * V_46 = F_25 ( V_12 -> V_59 ) ;
struct V_1 * V_9 = NULL , * V_127 ;
struct V_104 * V_130 ;
T_1 V_149 ;
T_1 V_150 , V_151 , V_152 , V_153 ;
unsigned int V_154 , V_155 ;
int V_156 , V_157 , V_158 = 0 ;
unsigned int V_159 ;
F_35 () ;
F_63 (tmp_neigh_node, node,
&orig_neigh_node->neigh_list, list) {
if ( ! F_64 ( V_127 -> V_135 ,
V_148 -> V_38 ) )
continue;
if ( V_127 -> V_12 != V_12 )
continue;
if ( ! F_43 ( & V_127 -> V_92 ) )
continue;
V_9 = V_127 ;
break;
}
F_37 () ;
if ( ! V_9 )
V_9 = F_1 ( V_12 ,
V_148 -> V_38 ,
V_148 ,
V_148 ,
V_16 -> V_8 ) ;
if ( ! V_9 )
goto V_10;
if ( V_6 == V_148 )
V_9 -> V_141 = V_50 ;
V_6 -> V_141 = V_50 ;
F_5 ( & V_6 -> V_144 ) ;
V_150 = V_148 -> V_145 [ V_12 -> V_131 ] ;
V_151 = V_9 -> V_160 ;
F_7 ( & V_6 -> V_144 ) ;
if ( V_150 > V_151 )
V_149 = V_151 ;
else
V_149 = V_150 ;
if ( V_149 < V_161 ||
V_151 < V_162 )
V_153 = 0 ;
else
V_153 = ( V_35 * V_149 ) / V_151 ;
V_152 = V_163 - V_151 ;
V_154 = V_152 * V_152 * V_152 ;
V_155 = V_163 *
V_163 *
V_163 ;
V_157 = V_35 * V_154 ;
V_157 /= V_155 ;
V_156 = V_35 - V_157 ;
V_159 = V_16 -> V_34 * V_153 * V_156 ;
V_159 /= V_35 * V_35 ;
V_16 -> V_34 = V_159 ;
F_27 ( V_71 , V_46 ,
L_15 ,
V_6 -> V_38 , V_148 -> V_38 , V_149 ,
V_151 , V_153 ,
V_156 , V_16 -> V_34 ) ;
if ( V_16 -> V_34 >= V_164 )
V_158 = 1 ;
V_10:
if ( V_9 )
F_65 ( V_9 ) ;
return V_158 ;
}
static int
F_78 ( const struct V_107 * V_107 ,
const struct V_16 * V_16 ,
const struct V_2 * V_12 )
{
struct V_45 * V_46 = F_25 ( V_12 -> V_59 ) ;
struct V_5 * V_6 ;
struct V_1 * V_127 ;
struct V_104 * V_130 ;
int V_126 = 0 ;
T_5 V_165 ;
int V_166 = 0 ;
int V_167 , V_158 = - 1 ;
T_3 V_8 = F_3 ( V_16 -> V_8 ) ;
T_1 * V_4 ;
T_1 V_168 ;
V_6 = F_68 ( V_46 , V_16 -> V_38 ) ;
if ( ! V_6 )
return 0 ;
F_5 ( & V_6 -> V_144 ) ;
V_165 = V_8 - V_6 -> V_169 ;
if ( ! F_79 ( & V_6 -> V_15 ) &&
F_80 ( V_46 , V_165 ,
& V_6 -> V_170 ) )
goto V_10;
F_35 () ;
F_63 (tmp_neigh_node, node,
&orig_node->neigh_list, list) {
V_126 |= F_81 ( V_127 -> V_171 ,
V_6 -> V_169 ,
V_8 ) ;
V_4 = V_127 -> V_135 ;
if ( F_64 ( V_4 , V_107 -> V_111 ) &&
V_127 -> V_12 == V_12 )
V_167 = 1 ;
else
V_167 = 0 ;
V_166 |= F_82 ( V_46 ,
V_127 -> V_171 ,
V_165 , V_167 ) ;
V_168 = F_83 ( V_127 -> V_171 ,
V_163 ) ;
V_127 -> V_160 = V_168 ;
}
F_37 () ;
if ( V_166 ) {
F_27 ( V_71 , V_46 ,
L_16 ,
V_6 -> V_169 , V_8 ) ;
V_6 -> V_169 = V_8 ;
}
V_158 = V_126 ;
V_10:
F_7 ( & V_6 -> V_144 ) ;
F_69 ( V_6 ) ;
return V_158 ;
}
static void F_84 ( const struct V_107 * V_107 ,
struct V_16 * V_16 ,
const unsigned char * V_125 ,
struct V_2 * V_12 )
{
struct V_45 * V_46 = F_25 ( V_12 -> V_59 ) ;
struct V_2 * V_3 ;
struct V_5 * V_148 , * V_6 ;
struct V_1 * V_128 = NULL , * V_172 = NULL ;
struct V_1 * V_173 = NULL ;
int V_174 ;
int V_175 = 0 , V_176 = 0 , V_177 = 0 ;
int V_178 = 0 , V_179 ;
bool V_108 = false ;
bool V_109 = false ;
int V_126 , V_180 , V_181 ;
T_3 V_182 ;
T_1 * V_42 ;
if ( V_16 -> V_26 . V_27 != V_28 )
return;
V_182 = F_17 ( & V_12 -> V_21 . V_22 ) ;
if ( V_16 -> V_32 & V_69 )
V_174 = 1 ;
else
V_174 = 0 ;
if ( F_64 ( V_107 -> V_111 , V_16 -> V_38 ) )
V_108 = true ;
F_27 ( V_71 , V_46 ,
L_17 ,
V_107 -> V_111 , V_12 -> V_39 -> V_72 ,
V_12 -> V_39 -> V_40 , V_16 -> V_38 ,
V_16 -> V_42 ,
F_3 ( V_16 -> V_8 ) , V_16 -> V_37 ,
F_74 ( V_16 -> V_117 ) ,
V_16 -> V_36 , V_16 -> V_34 ,
V_16 -> V_26 . V_31 ,
V_16 -> V_26 . V_29 , V_174 ) ;
F_35 () ;
F_36 (hard_iface, &batadv_hardif_list, list) {
if ( V_3 -> V_65 != V_66 )
continue;
if ( V_3 -> V_59 != V_12 -> V_59 )
continue;
if ( F_64 ( V_107 -> V_111 ,
V_3 -> V_39 -> V_40 ) )
V_175 = 1 ;
if ( F_64 ( V_16 -> V_38 ,
V_3 -> V_39 -> V_40 ) )
V_176 = 1 ;
if ( F_64 ( V_16 -> V_42 ,
V_3 -> V_39 -> V_40 ) )
V_177 = 1 ;
if ( F_85 ( V_107 -> V_111 ) )
V_178 = 1 ;
}
F_37 () ;
if ( V_16 -> V_26 . V_29 != V_30 ) {
F_27 ( V_71 , V_46 ,
L_18 ,
V_16 -> V_26 . V_29 ) ;
return;
}
if ( V_175 ) {
F_27 ( V_71 , V_46 ,
L_19 ,
V_107 -> V_111 ) ;
return;
}
if ( V_178 ) {
F_27 ( V_71 , V_46 ,
L_20 ,
V_107 -> V_111 ) ;
return;
}
if ( V_176 ) {
unsigned long * V_183 ;
int V_184 ;
T_5 V_185 ;
T_4 V_131 ;
T_1 * V_186 ;
V_148 = F_68 ( V_46 ,
V_107 -> V_111 ) ;
if ( ! V_148 )
return;
if ( V_174 &&
F_64 ( V_12 -> V_39 -> V_40 ,
V_16 -> V_38 ) ) {
V_131 = V_12 -> V_131 ;
V_184 = V_131 * V_187 ;
F_5 ( & V_148 -> V_144 ) ;
V_183 = & ( V_148 -> V_188 [ V_184 ] ) ;
V_185 = V_182 - 2 ;
V_185 -= F_3 ( V_16 -> V_8 ) ;
F_86 ( V_183 , V_185 ) ;
V_186 = & V_148 -> V_145 [ V_131 ] ;
* V_186 = F_83 ( V_183 ,
V_163 ) ;
F_7 ( & V_148 -> V_144 ) ;
}
F_27 ( V_71 , V_46 ,
L_21 ) ;
F_69 ( V_148 ) ;
return;
}
if ( V_177 ) {
F_27 ( V_71 , V_46 ,
L_22 ,
V_107 -> V_111 ) ;
return;
}
if ( V_16 -> V_32 & V_110 ) {
F_27 ( V_71 , V_46 ,
L_23 ,
V_107 -> V_111 ) ;
return;
}
V_6 = F_68 ( V_46 , V_16 -> V_38 ) ;
if ( ! V_6 )
return;
V_126 = F_78 ( V_107 , V_16 ,
V_12 ) ;
if ( V_126 == - 1 ) {
F_27 ( V_71 , V_46 ,
L_24 ,
V_107 -> V_111 ) ;
goto V_10;
}
if ( V_16 -> V_34 == 0 ) {
F_27 ( V_71 , V_46 ,
L_25 ) ;
goto V_10;
}
V_128 = F_71 ( V_6 ) ;
if ( V_128 )
V_172 = F_71 ( V_128 -> V_6 ) ;
if ( ( V_128 && V_128 -> V_134 != 0 ) &&
( F_64 ( V_128 -> V_135 , V_107 -> V_111 ) ) )
V_109 = true ;
V_42 = V_16 -> V_42 ;
if ( V_128 && V_172 &&
( F_64 ( V_128 -> V_135 , V_42 ) ) &&
! ( F_64 ( V_16 -> V_38 , V_42 ) ) &&
( F_64 ( V_128 -> V_135 , V_172 -> V_135 ) ) ) {
F_27 ( V_71 , V_46 ,
L_26 ,
V_107 -> V_111 ) ;
goto V_10;
}
if ( V_108 )
V_148 = V_6 ;
else
V_148 = F_68 ( V_46 ,
V_107 -> V_111 ) ;
if ( ! V_148 )
goto V_10;
V_173 = F_71 ( V_148 ) ;
if ( ! V_108 && ( ! V_173 ) ) {
F_27 ( V_71 , V_46 ,
L_27 ) ;
goto V_189;
}
V_179 = F_77 ( V_6 , V_148 ,
V_16 , V_12 ) ;
F_87 ( V_6 , V_148 ,
V_16 ) ;
V_180 = V_6 -> V_169 == F_3 ( V_16 -> V_8 ) ;
V_181 = V_6 -> V_142 - 3 <= V_16 -> V_26 . V_31 ;
if ( V_179 && ( ! V_126 || ( V_180 && V_181 ) ) )
F_62 ( V_46 , V_6 , V_107 ,
V_16 , V_12 ,
V_125 , V_126 ) ;
if ( V_108 ) {
F_56 ( V_6 , V_107 , V_16 ,
V_108 ,
V_109 , V_12 ) ;
F_27 ( V_71 , V_46 ,
L_28 ) ;
goto V_189;
}
if ( ! V_179 ) {
F_27 ( V_71 , V_46 ,
L_29 ) ;
goto V_189;
}
if ( V_126 ) {
F_27 ( V_71 , V_46 ,
L_30 ) ;
goto V_189;
}
F_27 ( V_71 , V_46 ,
L_31 ) ;
F_56 ( V_6 , V_107 , V_16 ,
V_108 , V_109 ,
V_12 ) ;
V_189:
if ( ( V_148 ) && ( ! V_108 ) )
F_69 ( V_148 ) ;
V_10:
if ( V_128 )
F_65 ( V_128 ) ;
if ( V_172 )
F_65 ( V_172 ) ;
if ( V_173 )
F_65 ( V_173 ) ;
F_69 ( V_6 ) ;
}
static int F_88 ( struct V_62 * V_63 ,
struct V_2 * V_12 )
{
struct V_45 * V_46 = F_25 ( V_12 -> V_59 ) ;
struct V_16 * V_16 ;
struct V_107 * V_107 ;
int V_53 = 0 , V_54 ;
unsigned char * V_125 , * V_86 ;
bool V_158 ;
T_1 * V_64 ;
V_158 = F_89 ( V_63 , V_12 , V_24 ) ;
if ( ! V_158 )
return V_190 ;
if ( V_46 -> V_191 -> V_192 != F_32 )
return V_190 ;
F_29 ( V_46 , V_193 ) ;
F_30 ( V_46 , V_194 ,
V_63 -> V_75 + V_76 ) ;
V_54 = F_90 ( V_63 ) ;
V_107 = (struct V_107 * ) F_91 ( V_63 ) ;
V_86 = V_63 -> V_67 ;
V_16 = (struct V_16 * ) V_86 ;
do {
V_125 = V_86 + V_53 + V_24 ;
F_84 ( V_107 , V_16 , V_125 ,
V_12 ) ;
V_53 += V_24 ;
V_53 += F_23 ( V_16 -> V_36 ) ;
V_64 = V_86 + V_53 ;
V_16 = (struct V_16 * ) V_64 ;
} while ( F_22 ( V_53 , V_54 ,
V_16 -> V_36 ) );
F_92 ( V_63 ) ;
return V_195 ;
}
int T_6 F_93 ( void )
{
int V_158 ;
V_158 = F_94 ( V_28 ,
F_88 ) ;
if ( V_158 < 0 )
goto V_10;
V_158 = F_95 ( & V_196 ) ;
if ( V_158 < 0 )
goto V_197;
goto V_10;
V_197:
F_96 ( V_28 ) ;
V_10:
return V_158 ;
}
