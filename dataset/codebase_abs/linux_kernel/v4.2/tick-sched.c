struct V_1 * F_1 ( int V_2 )
{
return & F_2 ( V_3 , V_2 ) ;
}
static void F_3 ( T_1 V_4 )
{
unsigned long V_5 = 0 ;
T_1 V_6 ;
V_6 = F_4 ( V_4 , V_7 ) ;
if ( V_6 . V_8 < V_9 . V_8 )
return;
F_5 ( & V_10 ) ;
V_6 = F_4 ( V_4 , V_7 ) ;
if ( V_6 . V_8 >= V_9 . V_8 ) {
V_6 = F_4 ( V_6 , V_9 ) ;
V_7 = F_6 ( V_7 ,
V_9 ) ;
if ( F_7 ( V_6 . V_8 >= V_9 . V_8 ) ) {
T_2 V_11 = F_8 ( V_9 ) ;
V_5 = F_9 ( V_6 , V_11 ) ;
V_7 = F_10 ( V_7 ,
V_11 * V_5 ) ;
}
F_11 ( ++ V_5 ) ;
V_12 = F_6 ( V_7 , V_9 ) ;
} else {
F_12 ( & V_10 ) ;
return;
}
F_12 ( & V_10 ) ;
F_13 () ;
}
static T_1 F_14 ( void )
{
T_1 V_13 ;
F_5 ( & V_10 ) ;
if ( V_7 . V_8 == 0 )
V_7 = V_12 ;
V_13 = V_7 ;
F_12 ( & V_10 ) ;
return V_13 ;
}
static void F_15 ( T_1 V_4 )
{
int V_2 = F_16 () ;
#ifdef F_17
if ( F_7 ( V_14 == V_15 )
&& ! F_18 ( V_2 ) )
V_14 = V_2 ;
#endif
if ( V_14 == V_2 )
F_3 ( V_4 ) ;
}
static void F_19 ( struct V_1 * V_16 , struct V_17 * V_18 )
{
#ifdef F_17
if ( V_16 -> V_19 ) {
F_20 () ;
if ( F_21 ( V_20 ) )
V_16 -> V_21 ++ ;
}
#endif
F_22 ( F_23 ( V_18 ) ) ;
F_24 ( V_22 ) ;
}
static bool F_25 ( void )
{
F_26 ( ! F_27 () ) ;
if ( ! F_28 () ) {
F_29 ( 0 , L_1 ) ;
return false ;
}
if ( ! F_30 ( V_20 ) ) {
F_29 ( 0 , L_2 ) ;
return false ;
}
if ( ! F_31 () ) {
F_29 ( 0 , L_3 ) ;
return false ;
}
#ifdef F_32
if ( ! F_33 () ) {
F_29 ( 0 , L_4 ) ;
F_34 ( V_23 ,
L_5 ) ;
return false ;
}
#endif
return true ;
}
void F_35 ( void )
{
struct V_1 * V_16 = F_36 ( & V_3 ) ;
if ( F_18 ( F_16 () ) ) {
if ( V_16 -> V_19 && ! F_21 ( V_20 ) ) {
if ( ! F_25 () )
F_37 ( V_16 , F_38 () ) ;
}
}
}
static void F_39 ( struct V_24 * V_25 )
{
F_35 () ;
}
void F_40 ( void )
{
if ( ! F_18 ( F_16 () ) )
return;
F_41 ( F_36 ( & V_26 ) ) ;
}
void F_42 ( int V_2 )
{
if ( ! F_18 ( V_2 ) )
return;
F_43 ( & F_2 ( V_26 , V_2 ) , V_2 ) ;
}
static void F_44 ( void * V_27 )
{
F_35 () ;
}
void F_45 ( void )
{
if ( ! V_23 )
return;
F_46 () ;
F_47 ( V_28 ,
F_44 , NULL , false ) ;
F_40 () ;
F_48 () ;
}
void F_49 ( struct V_29 * V_30 )
{
unsigned long V_31 ;
F_50 ( V_31 ) ;
if ( ! F_18 ( F_16 () ) )
goto V_32;
if ( F_51 () && ! F_25 () )
F_40 () ;
V_32:
F_52 ( V_31 ) ;
}
static int T_3 F_53 ( char * V_33 )
{
F_54 ( & V_28 ) ;
if ( F_55 ( V_33 , V_28 ) < 0 ) {
F_56 ( L_6 ) ;
F_57 ( V_28 ) ;
return 1 ;
}
V_23 = true ;
return 1 ;
}
static int F_58 ( struct V_34 * V_35 ,
unsigned long V_36 ,
void * V_37 )
{
unsigned int V_2 = ( unsigned long ) V_37 ;
switch ( V_36 & ~ V_38 ) {
case V_39 :
if ( V_23 && V_14 == V_2 )
return V_40 ;
break;
}
return V_41 ;
}
static int F_59 ( void )
{
int V_42 = - 1 ;
#ifdef F_60
if ( ! F_61 ( & V_28 , V_43 ) ) {
F_62 ( 1 , L_7 ) ;
return V_42 ;
}
V_42 = 0 ;
F_63 ( V_28 ) ;
V_23 = true ;
#endif
return V_42 ;
}
void T_3 F_64 ( void )
{
int V_2 ;
if ( ! V_23 ) {
if ( F_59 () < 0 )
return;
}
if ( ! F_61 ( & V_44 , V_43 ) ) {
F_62 ( 1 , L_8 ) ;
F_65 ( V_28 ) ;
V_23 = false ;
return;
}
if ( ! F_66 () ) {
F_56 ( L_9
L_10 ) ;
F_65 ( V_28 ) ;
F_67 ( V_44 , V_45 ) ;
V_23 = false ;
return;
}
V_2 = F_16 () ;
if ( F_68 ( V_2 , V_28 ) ) {
F_56 ( L_11 , V_2 ) ;
F_69 ( V_2 , V_28 ) ;
}
F_70 ( V_44 ,
V_45 , V_28 ) ;
F_71 (cpu, tick_nohz_full_mask)
F_72 ( V_2 ) ;
F_73 ( F_58 , 0 ) ;
F_74 ( L_12 ,
F_75 ( V_28 ) ) ;
}
static int T_3 F_76 ( char * V_33 )
{
if ( ! strcmp ( V_33 , L_13 ) )
V_46 = 0 ;
else if ( ! strcmp ( V_33 , L_14 ) )
V_46 = 1 ;
else
return 0 ;
return 1 ;
}
int F_51 ( void )
{
return F_77 ( V_3 . V_19 ) ;
}
static void F_78 ( T_1 V_4 )
{
unsigned long V_31 ;
F_79 ( V_3 . V_47 , V_4 ) ;
F_50 ( V_31 ) ;
F_3 ( V_4 ) ;
F_52 ( V_31 ) ;
F_20 () ;
}
static void
F_80 ( int V_2 , struct V_1 * V_16 , T_1 V_4 , T_4 * V_48 )
{
T_1 V_6 ;
if ( V_16 -> V_49 ) {
V_6 = F_4 ( V_4 , V_16 -> V_50 ) ;
if ( F_81 ( V_2 ) > 0 )
V_16 -> V_51 = F_6 ( V_16 -> V_51 , V_6 ) ;
else
V_16 -> V_52 = F_6 ( V_16 -> V_52 , V_6 ) ;
V_16 -> V_50 = V_4 ;
}
if ( V_48 )
* V_48 = F_82 ( V_4 ) ;
}
static void F_83 ( struct V_1 * V_16 , T_1 V_4 )
{
F_80 ( F_16 () , V_16 , V_4 , NULL ) ;
V_16 -> V_49 = 0 ;
F_84 ( 0 ) ;
}
static T_1 F_85 ( struct V_1 * V_16 )
{
T_1 V_4 = F_38 () ;
V_16 -> V_50 = V_4 ;
V_16 -> V_49 = 1 ;
F_86 () ;
return V_4 ;
}
T_4 F_87 ( int V_2 , T_4 * V_48 )
{
struct V_1 * V_16 = & F_2 ( V_3 , V_2 ) ;
T_1 V_4 , V_53 ;
if ( ! V_54 )
return - 1 ;
V_4 = F_38 () ;
if ( V_48 ) {
F_80 ( V_2 , V_16 , V_4 , V_48 ) ;
V_53 = V_16 -> V_52 ;
} else {
if ( V_16 -> V_49 && ! F_81 ( V_2 ) ) {
T_1 V_6 = F_4 ( V_4 , V_16 -> V_50 ) ;
V_53 = F_6 ( V_16 -> V_52 , V_6 ) ;
} else {
V_53 = V_16 -> V_52 ;
}
}
return F_82 ( V_53 ) ;
}
T_4 F_88 ( int V_2 , T_4 * V_48 )
{
struct V_1 * V_16 = & F_2 ( V_3 , V_2 ) ;
T_1 V_4 , V_55 ;
if ( ! V_54 )
return - 1 ;
V_4 = F_38 () ;
if ( V_48 ) {
F_80 ( V_2 , V_16 , V_4 , V_48 ) ;
V_55 = V_16 -> V_51 ;
} else {
if ( V_16 -> V_49 && F_81 ( V_2 ) > 0 ) {
T_1 V_6 = F_4 ( V_4 , V_16 -> V_50 ) ;
V_55 = F_6 ( V_16 -> V_51 , V_6 ) ;
} else {
V_55 = V_16 -> V_51 ;
}
}
return F_82 ( V_55 ) ;
}
static void F_89 ( struct V_1 * V_16 , T_1 V_4 )
{
F_90 ( & V_16 -> V_56 ) ;
F_91 ( & V_16 -> V_56 , V_16 -> V_57 ) ;
F_92 ( & V_16 -> V_56 , V_4 , V_9 ) ;
if ( V_16 -> V_58 == V_59 )
F_93 ( & V_16 -> V_56 , V_60 ) ;
else
F_94 ( F_95 ( & V_16 -> V_56 ) , 1 ) ;
}
static T_1 F_96 ( struct V_1 * V_16 ,
T_1 V_4 , int V_2 )
{
struct V_61 * V_62 = F_77 ( V_63 . V_64 ) ;
T_4 V_65 , V_66 , V_67 , V_68 , V_6 , V_69 ;
unsigned long V_70 , V_71 ;
T_1 V_72 ;
do {
V_70 = F_97 ( & V_10 ) ;
V_65 = V_7 . V_8 ;
V_71 = V_73 ;
} while ( F_98 ( & V_10 , V_70 ) );
V_16 -> V_74 = V_71 ;
if ( F_99 ( V_65 , & V_68 ) ||
F_100 () || F_101 () ) {
V_66 = V_65 + V_75 ;
} else {
V_67 = F_102 ( V_71 , V_65 ) ;
V_16 -> V_76 = V_67 ;
V_66 = V_68 < V_67 ? V_68 : V_67 ;
}
V_6 = V_66 - V_65 ;
if ( V_6 <= ( T_4 ) V_75 ) {
V_72 . V_8 = 0 ;
if ( ! V_16 -> V_19 )
goto V_32;
if ( V_6 == 0 ) {
F_89 ( V_16 , V_4 ) ;
goto V_32;
}
}
V_6 = F_103 () ;
if ( V_2 == V_14 ) {
V_14 = V_15 ;
V_16 -> V_77 = 1 ;
} else if ( V_14 != V_15 ) {
V_6 = V_78 ;
V_16 -> V_77 = 0 ;
} else if ( ! V_16 -> V_77 ) {
V_6 = V_78 ;
}
#ifdef F_104
if ( ! V_16 -> V_79 )
V_6 = F_105 ( V_6 , F_106 () ) ;
#endif
if ( V_6 < ( V_78 - V_65 ) )
V_69 = V_65 + V_6 ;
else
V_69 = V_78 ;
V_69 = F_107 ( T_4 , V_69 , V_66 ) ;
V_72 . V_8 = V_69 ;
if ( V_16 -> V_19 && ( V_69 == V_62 -> V_80 . V_8 ) )
goto V_32;
if ( ! V_16 -> V_19 ) {
F_108 ( V_2 ) ;
F_109 () ;
V_16 -> V_57 = F_95 ( & V_16 -> V_56 ) ;
V_16 -> V_19 = 1 ;
F_29 ( 1 , L_15 ) ;
}
if ( F_7 ( V_69 == V_78 ) ) {
if ( V_16 -> V_58 == V_59 )
F_90 ( & V_16 -> V_56 ) ;
goto V_32;
}
if ( V_16 -> V_58 == V_59 )
F_110 ( & V_16 -> V_56 , V_72 , V_60 ) ;
else
F_94 ( V_72 , 1 ) ;
V_32:
V_16 -> V_81 = F_4 ( V_62 -> V_80 , V_4 ) ;
return V_72 ;
}
static void F_111 ( struct V_1 * V_16 )
{
#ifdef F_104
int V_2 = F_16 () ;
if ( ! F_18 ( V_2 ) || F_21 ( V_20 ) )
return;
if ( ! V_16 -> V_19 && V_16 -> V_58 == V_82 )
return;
if ( ! F_25 () )
return;
F_96 ( V_16 , F_38 () , V_2 ) ;
#endif
}
static bool F_112 ( int V_2 , struct V_1 * V_16 )
{
if ( F_7 ( ! F_113 ( V_2 ) ) ) {
if ( V_2 == V_14 )
V_14 = V_15 ;
return false ;
}
if ( F_7 ( V_16 -> V_58 == V_82 ) ) {
V_16 -> V_81 = ( T_1 ) { . V_8 = V_83 / V_84 } ;
return false ;
}
if ( F_114 () )
return false ;
if ( F_7 ( F_115 () && F_113 ( V_2 ) ) ) {
static int V_85 ;
if ( V_85 < 10 &&
( F_115 () & V_86 ) ) {
F_116 ( L_16 ,
( unsigned int ) F_115 () ) ;
V_85 ++ ;
}
return false ;
}
if ( F_117 () ) {
if ( V_14 == V_2 )
return false ;
if ( V_14 == V_15 )
return false ;
}
return true ;
}
static void F_118 ( struct V_1 * V_16 )
{
T_1 V_4 , V_69 ;
int V_2 = F_16 () ;
V_4 = F_85 ( V_16 ) ;
if ( F_112 ( V_2 , V_16 ) ) {
int V_87 = V_16 -> V_19 ;
V_16 -> V_88 ++ ;
V_69 = F_96 ( V_16 , V_4 , V_2 ) ;
if ( V_69 . V_8 > 0LL ) {
V_16 -> V_89 ++ ;
V_16 -> V_90 = V_69 ;
}
if ( ! V_87 && V_16 -> V_19 )
V_16 -> V_21 = V_16 -> V_74 ;
}
}
void F_119 ( void )
{
struct V_1 * V_16 ;
F_26 ( F_27 () ) ;
F_120 () ;
F_121 () ;
V_16 = F_36 ( & V_3 ) ;
V_16 -> V_79 = 1 ;
F_118 ( V_16 ) ;
F_122 () ;
}
void F_123 ( void )
{
struct V_1 * V_16 = F_36 ( & V_3 ) ;
if ( V_16 -> V_79 )
F_118 ( V_16 ) ;
else
F_111 ( V_16 ) ;
}
T_1 F_124 ( void )
{
struct V_1 * V_16 = F_36 ( & V_3 ) ;
return V_16 -> V_81 ;
}
static void F_37 ( struct V_1 * V_16 , T_1 V_4 )
{
F_3 ( V_4 ) ;
F_125 () ;
F_126 () ;
F_20 () ;
V_16 -> V_19 = 0 ;
V_16 -> V_91 = V_4 ;
F_89 ( V_16 , V_4 ) ;
}
static void F_127 ( struct V_1 * V_16 )
{
#ifndef F_128
unsigned long V_5 ;
if ( F_129 () )
return;
V_5 = V_73 - V_16 -> V_21 ;
if ( V_5 && V_5 < V_92 )
F_130 ( V_5 ) ;
#endif
}
void F_131 ( void )
{
struct V_1 * V_16 = F_36 ( & V_3 ) ;
T_1 V_4 ;
F_121 () ;
F_26 ( ! V_16 -> V_79 ) ;
V_16 -> V_79 = 0 ;
if ( V_16 -> V_49 || V_16 -> V_19 )
V_4 = F_38 () ;
if ( V_16 -> V_49 )
F_83 ( V_16 , V_4 ) ;
if ( V_16 -> V_19 ) {
F_37 ( V_16 , V_4 ) ;
F_127 ( V_16 ) ;
}
F_122 () ;
}
static void F_132 ( struct V_61 * V_62 )
{
struct V_1 * V_16 = F_36 ( & V_3 ) ;
struct V_17 * V_18 = F_133 () ;
T_1 V_4 = F_38 () ;
V_62 -> V_80 . V_8 = V_78 ;
F_15 ( V_4 ) ;
F_19 ( V_16 , V_18 ) ;
if ( F_7 ( V_16 -> V_19 ) )
return;
F_92 ( & V_16 -> V_56 , V_4 , V_9 ) ;
F_94 ( F_95 ( & V_16 -> V_56 ) , 1 ) ;
}
static inline void F_134 ( struct V_1 * V_16 , int V_93 )
{
if ( ! V_46 )
return;
V_16 -> V_58 = V_93 ;
if ( ! F_135 ( 0 , & V_54 ) )
F_136 ( true ) ;
}
static void F_137 ( void )
{
struct V_1 * V_16 = F_36 ( & V_3 ) ;
T_1 V_94 ;
if ( ! V_46 )
return;
if ( F_138 ( F_132 ) )
return;
F_139 ( & V_16 -> V_56 , V_95 , V_96 ) ;
V_94 = F_14 () ;
F_140 ( & V_16 -> V_56 , V_9 ) ;
F_91 ( & V_16 -> V_56 , V_94 ) ;
F_94 ( V_94 , 1 ) ;
F_134 ( V_16 , V_97 ) ;
}
static void F_141 ( struct V_1 * V_16 , T_1 V_4 )
{
#if 0
ktime_t delta;
delta = ktime_sub(hrtimer_get_expires(&ts->sched_timer), now);
if (delta.tv64 <= tick_period.tv64)
return;
tick_nohz_restart(ts, now);
#endif
}
static inline void F_142 ( void )
{
struct V_1 * V_16 = F_36 ( & V_3 ) ;
T_1 V_4 ;
if ( ! V_16 -> V_49 && ! V_16 -> V_19 )
return;
V_4 = F_38 () ;
if ( V_16 -> V_49 )
F_83 ( V_16 , V_4 ) ;
if ( V_16 -> V_19 ) {
F_78 ( V_4 ) ;
F_141 ( V_16 , V_4 ) ;
}
}
static inline void F_137 ( void ) { }
static inline void F_142 ( void ) { }
static inline void F_134 ( struct V_1 * V_16 , int V_93 ) { }
void F_143 ( void )
{
F_144 () ;
F_142 () ;
}
static enum V_98 F_145 ( struct V_99 * V_100 )
{
struct V_1 * V_16 =
F_146 ( V_100 , struct V_1 , V_56 ) ;
struct V_17 * V_18 = F_133 () ;
T_1 V_4 = F_38 () ;
F_15 ( V_4 ) ;
if ( V_18 )
F_19 ( V_16 , V_18 ) ;
if ( F_7 ( V_16 -> V_19 ) )
return V_101 ;
F_92 ( V_100 , V_4 , V_9 ) ;
return V_102 ;
}
static int T_3 F_147 ( char * V_33 )
{
F_148 ( & V_33 , & V_103 ) ;
return 0 ;
}
void F_149 ( void )
{
struct V_1 * V_16 = F_36 ( & V_3 ) ;
T_1 V_4 = F_38 () ;
F_139 ( & V_16 -> V_56 , V_95 , V_96 ) ;
V_16 -> V_56 . V_104 = F_145 ;
F_91 ( & V_16 -> V_56 , F_14 () ) ;
if ( V_103 ) {
T_4 V_105 = F_8 ( V_9 ) >> 1 ;
F_150 ( V_105 , F_151 () ) ;
V_105 *= F_16 () ;
F_152 ( & V_16 -> V_56 , V_105 ) ;
}
F_92 ( & V_16 -> V_56 , V_4 , V_9 ) ;
F_93 ( & V_16 -> V_56 , V_60 ) ;
F_134 ( V_16 , V_59 ) ;
}
void F_153 ( int V_2 )
{
struct V_1 * V_16 = & F_2 ( V_3 , V_2 ) ;
# ifdef F_154
if ( V_16 -> V_56 . V_106 )
F_90 ( & V_16 -> V_56 ) ;
# endif
memset ( V_16 , 0 , sizeof( * V_16 ) ) ;
}
void F_155 ( void )
{
int V_2 ;
F_156 (cpu)
F_157 ( 0 , & F_2 ( V_3 , V_2 ) . V_107 ) ;
}
void F_158 ( void )
{
struct V_1 * V_16 = F_36 ( & V_3 ) ;
F_157 ( 0 , & V_16 -> V_107 ) ;
}
int F_159 ( int V_108 )
{
struct V_1 * V_16 = F_36 ( & V_3 ) ;
if ( ! F_160 ( 0 , & V_16 -> V_107 ) )
return 0 ;
if ( V_16 -> V_58 != V_82 )
return 0 ;
if ( ! F_161 () || ! F_162 () )
return 0 ;
if ( ! V_108 )
return 1 ;
F_137 () ;
return 0 ;
}
