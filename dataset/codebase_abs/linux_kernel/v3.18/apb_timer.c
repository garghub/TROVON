static inline void T_1 * F_1 ( struct V_1 * V_2 )
{
return V_3 + V_2 -> V_4 * V_5 ;
}
static inline void F_2 ( void )
{
struct V_6 * V_7 ;
int V_8 = 0 ;
if ( V_3 ) {
F_3 ( L_1 ) ;
return;
}
V_7 = F_4 ( V_9 ) ;
if ( V_7 == NULL ) {
F_5 ( V_10 L_2 ,
V_9 ) ;
return;
}
V_11 = ( V_12 ) V_7 -> V_13 ;
if ( ! V_11 ) {
F_5 ( V_14 L_3 ) ;
V_11 = V_15 ;
}
V_3 = F_6 ( V_11 , V_16 ) ;
if ( ! V_3 ) {
F_3 ( L_4 ,\
( unsigned long ) V_11 ) ;
goto V_17;
}
V_18 = V_7 -> V_19 ;
F_7 ( V_7 ) ;
V_7 = F_4 ( V_20 ) ;
if ( V_7 == NULL )
goto V_17;
F_3 ( L_5 ,
( int ) ( V_7 -> V_13 & 0xff ) / V_5 ) ;
V_8 = ( unsigned int ) ( V_7 -> V_13 & 0xff ) /
V_5 ;
V_21 = F_8 ( V_22 ,
L_6 , V_3 + V_8 *
V_5 , V_18 ) ;
return;
V_17:
F_9 ( L_7 ) ;
}
static inline void F_10 ( void )
{
F_11 ( V_3 ) ;
V_3 = NULL ;
}
static inline int F_12 ( void )
{
return V_3 ? 1 : 0 ;
}
static int T_2 F_13 ( void )
{
struct V_6 * V_7 ;
struct V_1 * V_2 = F_14 ( & V_23 ) ;
V_7 = F_4 ( V_9 ) ;
if ( V_7 == NULL ) {
F_5 ( V_10 L_2 ,
V_9 ) ;
return - V_24 ;
}
V_2 -> V_4 = F_15 () ;
V_2 -> V_25 = F_16 ( F_15 () , L_6 ,
V_26 == V_27 ?
V_28 - 100 : V_28 ,
F_1 ( V_2 ) , 0 , V_18 ) ;
V_2 -> V_25 -> V_29 = NULL ;
if ( V_26 == V_27 ) {
V_30 = & V_2 -> V_25 -> V_31 ;
F_5 ( V_32 L_8 ,
V_30 -> V_33 ) ;
}
F_17 ( V_2 -> V_25 ) ;
F_7 ( V_7 ) ;
return 0 ;
}
static void F_18 ( struct V_1 * V_2 )
{
if ( V_2 -> V_34 == 0 )
return;
F_19 ( V_2 -> V_34 , 0 , V_35 ) ;
F_20 ( V_2 -> V_34 , F_21 ( V_2 -> V_36 ) ) ;
}
void F_22 ( void )
{
struct V_1 * V_2 ;
int V_36 ;
V_36 = F_15 () ;
if ( ! V_36 )
return;
V_2 = F_14 ( & V_23 ) ;
if ( ! V_2 -> V_25 ) {
V_2 -> V_25 = F_16 ( V_36 , V_2 -> V_33 ,
V_28 , F_1 ( V_2 ) ,
V_2 -> V_34 , V_18 ) ;
V_2 -> V_25 -> V_29 = NULL ;
} else {
F_23 ( V_2 -> V_25 ) ;
}
F_5 ( V_37 L_9 ,
V_36 , V_2 -> V_33 , V_2 -> V_36 ) ;
F_18 ( V_2 ) ;
F_17 ( V_2 -> V_25 ) ;
return;
}
static int F_24 ( struct V_38 * V_39 ,
unsigned long V_40 , void * V_41 )
{
unsigned long V_36 = ( unsigned long ) V_41 ;
struct V_1 * V_2 = & F_25 ( V_23 , V_36 ) ;
switch ( V_40 & 0xf ) {
case V_42 :
F_26 ( V_2 -> V_25 ) ;
if ( V_43 == V_44 ) {
F_3 ( L_10 , V_36 ) ;
} else {
F_3 ( L_11 , V_36 ) ;
F_27 ( V_2 -> V_25 ) ;
}
break;
default:
F_3 ( L_12 , V_40 ) ;
}
return V_45 ;
}
static T_2 int F_28 ( void )
{
if ( V_26 == V_27 ||
! V_46 )
return 0 ;
F_29 ( F_24 , - 20 ) ;
return 0 ;
}
void F_22 ( void ) {}
static int F_30 ( void )
{
T_3 V_47 , V_48 ;
T_4 V_49 ;
F_31 ( V_21 ) ;
V_49 = F_32 ( V_21 ) ;
F_33 ( V_47 ) ;
do {
F_34 () ;
F_33 ( V_48 ) ;
} while ( ( V_48 - V_47 ) < 200000UL );
if ( V_49 == F_32 ( V_21 ) )
F_9 ( L_13 ) ;
F_35 ( V_21 ) ;
return 0 ;
}
void T_2 F_36 ( void )
{
#ifdef F_37
int V_50 ;
struct V_6 * V_51 ;
struct V_1 * V_2 ;
#endif
if ( V_46 )
return;
F_2 () ;
if ( ! V_3 )
goto V_52;
if ( V_18 < V_53 || V_18 > V_54 ) {
F_3 ( L_14 , V_18 ) ;
goto V_52;
}
if ( F_30 () ) {
F_3 ( L_15 ) ;
goto V_52;
}
if ( ! F_13 () )
V_46 = 1 ;
else {
F_3 ( L_16 ) ;
goto V_52;
}
#ifdef F_37
if ( V_26 == V_27 ) {
F_5 ( V_37 L_17 ) ;
return;
}
F_3 ( L_18 , V_55 , F_38 () ) ;
if ( F_39 () <= V_56 )
V_57 = F_39 () ;
else
V_57 = 1 ;
F_3 ( L_19 , V_55 , V_57 ) ;
for ( V_50 = 0 ; V_50 < V_57 ; V_50 ++ ) {
V_2 = & F_25 ( V_23 , V_50 ) ;
V_2 -> V_4 = V_50 ;
V_2 -> V_36 = V_50 ;
V_51 = F_4 ( V_50 ) ;
if ( V_51 )
V_2 -> V_34 = V_51 -> V_34 ;
else
F_5 ( V_10 L_20 , V_50 ) ;
snprintf ( V_2 -> V_33 , sizeof( V_2 -> V_33 ) - 1 , L_21 , V_50 ) ;
}
#endif
return;
V_52:
F_10 () ;
V_46 = 0 ;
F_9 ( L_22 ) ;
}
unsigned long F_40 ( void )
{
int V_50 , V_58 ;
T_3 V_59 , V_60 ;
T_4 V_49 , V_61 ;
unsigned long V_62 = 0 ;
T_5 V_63 , V_64 ;
F_2 () ;
F_31 ( V_21 ) ;
V_59 = F_32 ( V_21 ) ;
V_50 = 10000 ;
while ( -- V_50 ) {
if ( V_59 != F_32 ( V_21 ) )
break;
}
if ( ! V_50 )
goto V_65;
V_63 = ( V_18 / 1000 ) << 4 ;
F_31 ( V_21 ) ;
V_59 = F_32 ( V_21 ) ;
V_59 += V_63 ;
V_49 = F_41 () ;
do {
V_60 = F_32 ( V_21 ) ;
} while ( V_60 < V_59 );
V_61 = F_41 () ;
V_64 = 5 ;
if ( F_42 ( V_63 >> V_64 == 0 ) ) {
F_5 ( V_37
L_23 ) ;
return 0 ;
}
V_58 = ( int ) F_43 ( ( V_61 - V_49 ) , V_63 >> V_64 ) ;
V_62 = ( V_58 * ( V_18 / 1000 ) ) >> V_64 ;
F_5 ( V_37 L_24 , V_62 ) ;
return V_62 ;
V_65:
return 0 ;
}
