static void F_1 ( struct V_1 T_1 * V_2 )
{
F_2 ( F_3 ( & V_2 -> V_3 ) | ( 1 << V_4 ) ,
& V_2 -> V_3 ) ;
F_2 ( F_3 ( & V_2 -> V_5 ) | ( 1 << V_4 ) ,
& V_2 -> V_5 ) ;
}
static void F_4 ( struct V_1 T_1 * V_2 )
{
F_2 ( F_3 ( & V_2 -> V_3 ) & ~ ( 1 << V_4 ) ,
& V_2 -> V_3 ) ;
F_2 ( F_3 ( & V_2 -> V_5 ) & ~ ( 1 << V_4 ) ,
& V_2 -> V_5 ) ;
}
static void F_5 ( struct V_6 * V_7 ,
int V_8 , int V_9 , T_2 * V_10 )
{
struct V_1 T_1 * V_2 = V_7 -> V_11 ;
T_3 V_12 ;
F_1 ( V_2 ) ;
V_12 = ( T_3 ) V_7 -> V_13 + ( V_8 & ~ 0x3 ) ;
* V_10 = F_3 ( V_12 ) ;
if ( V_9 == 1 )
* V_10 = ( * V_10 >> ( 8 * ( V_8 & 3 ) ) ) & 0xff ;
else if ( V_9 == 2 )
* V_10 = ( * V_10 >> ( 8 * ( V_8 & 3 ) ) ) & 0xffff ;
F_4 ( V_2 ) ;
}
static void F_6 ( struct V_6 * V_7 ,
int V_8 , int V_9 , T_2 V_10 )
{
struct V_1 T_1 * V_2 = V_7 -> V_11 ;
T_3 V_12 ;
F_1 ( V_2 ) ;
V_12 = ( T_3 ) V_7 -> V_13 + ( V_8 & ~ 0x3 ) ;
if ( V_9 == 4 )
F_2 ( V_10 , V_12 ) ;
else if ( V_9 == 2 )
F_7 ( V_10 , V_12 + ( V_8 & 2 ) ) ;
else if ( V_9 == 1 )
F_8 ( V_10 , V_12 + ( V_8 & 3 ) ) ;
F_4 ( V_2 ) ;
}
static int F_9 ( struct V_6 * V_7 ,
T_2 V_14 , int V_15 , int * V_16 )
{
T_2 V_17 ;
while ( ( * V_16 ) -- ) {
F_5 ( V_7 , V_14 , 1 , & V_14 ) ;
if ( V_14 < 0x40 )
break;
V_14 &= ~ 3 ;
F_5 ( V_7 , V_14 + V_18 , 1 , & V_17 ) ;
if ( V_17 == 0xff )
break;
if ( V_17 == V_15 )
return V_14 ;
V_14 += V_19 ;
}
return 0 ;
}
static int F_10 ( struct V_6 * V_7 ,
T_2 V_14 , int V_15 )
{
int V_16 = V_20 ;
return F_9 ( V_7 , V_14 , V_15 , & V_16 ) ;
}
static int F_11 ( struct V_6 * V_7 ,
T_4 V_21 )
{
T_2 V_22 ;
F_5 ( V_7 , V_23 , 2 , & V_22 ) ;
if ( ! ( V_22 & V_24 ) )
return 0 ;
switch ( V_21 ) {
case V_25 :
case V_26 :
return V_27 ;
case V_28 :
return V_29 ;
default:
return 0 ;
}
return 0 ;
}
static int F_12 ( struct V_6 * V_7 ,
int V_15 )
{
T_2 V_14 ;
T_2 V_21 ;
F_5 ( V_7 , V_30 , 1 , & V_21 ) ;
V_14 = F_11 ( V_7 , V_21 ) ;
if ( V_14 )
V_14 = F_10 ( V_7 , V_14 , V_15 ) ;
return V_14 ;
}
static T_5 F_13 ( int V_31 , void * V_32 )
{
return 0 ;
}
static T_6 F_14 (
struct V_6 * V_7 ,
char * V_33 )
{
struct V_1 T_1 * V_2 = V_7 -> V_11 ;
if ( F_3 ( & V_2 -> V_34 ) & ( ( T_2 ) 1 << V_35 ) )
return sprintf ( V_33 , L_1 ) ;
else
return sprintf ( V_33 , L_2 ) ;
}
static T_6 F_15 (
struct V_6 * V_7 ,
const char * V_33 , T_7 V_36 )
{
struct V_1 T_1 * V_2 = V_7 -> V_11 ;
if ( F_16 ( V_33 , L_1 ) )
F_2 ( F_3 ( & V_2 -> V_37 ) | ( 1 << V_38 ) ,
& V_2 -> V_37 ) ;
else if ( F_16 ( V_33 , L_2 ) )
F_2 ( F_3 ( & V_2 -> V_37 )
& ~ ( 1 << V_38 ) ,
& V_2 -> V_37 ) ;
else
return - V_39 ;
return V_36 ;
}
static T_6 F_17 (
struct V_6 * V_7 ,
char * V_33 )
{
return sprintf ( V_33 , L_3 , V_7 -> V_40 ) ;
}
static T_6 F_18 (
struct V_6 * V_7 ,
const char * V_33 , T_7 V_36 )
{
T_2 V_15 , V_41 , V_42 ;
T_3 V_43 ;
if ( F_16 ( V_33 , L_4 ) )
F_6 ( V_7 , V_44 , 1 , 1 ) ;
else if ( F_16 ( V_33 , L_5 ) ) {
V_43 = V_7 -> V_45 ;
V_41 = 0 ;
while ( V_43 > 1 ) {
V_43 /= 2 ;
V_41 ++ ;
}
F_6 ( V_7 , V_44 , 1 , 0 ) ;
V_15 = F_12 ( V_7 , V_46 ) ;
F_5 ( V_7 , V_15 + V_47 , 1 , & V_42 ) ;
V_42 &= ~ V_48 ;
V_42 |= V_41 << 1 ;
F_6 ( V_7 , V_15 + V_47 , 1 , V_42 ) ;
} else
return - V_39 ;
strcpy ( V_7 -> V_40 , V_33 ) ;
return V_36 ;
}
static T_6 F_19 (
struct V_6 * V_7 ,
char * V_33 )
{
struct V_1 T_1 * V_2 = V_7 -> V_11 ;
T_2 V_15 , V_41 , V_42 ;
T_3 V_43 ;
if ( ( F_3 ( & V_2 -> V_49 ) & ( 1 << V_50 ) )
!= ( 1 << V_50 ) )
V_43 = 0 ;
else {
V_15 = F_12 ( V_7 , V_46 ) ;
F_5 ( V_7 , V_15 + V_47 , 1 , & V_42 ) ;
V_42 &= ~ V_51 ;
V_41 = V_42 >> 4 ;
V_43 = 1 ;
while ( V_41 -- )
V_43 *= 2 ;
}
V_7 -> V_52 = V_43 ;
return sprintf ( V_33 , L_6 , V_43 ) ;
}
static T_6 F_20 (
struct V_6 * V_7 ,
const char * V_33 , T_7 V_36 )
{
int V_53 ;
V_53 = F_21 ( V_33 , 0 , & V_7 -> V_45 ) ;
if ( V_53 )
return V_53 ;
if ( V_7 -> V_45 > 32 )
V_7 -> V_45 = 32 ;
return V_36 ;
}
static T_6 F_22 (
struct V_6 * V_7 ,
const char * V_33 , T_7 V_36 )
{
struct V_1 T_1 * V_2 = V_7 -> V_11 ;
T_3 V_54 ;
int V_53 ;
V_53 = F_21 ( V_33 , 0 , & V_54 ) ;
if ( V_53 )
return V_53 ;
if ( V_54 )
F_2 ( F_3 ( & V_2 -> V_37 ) | ( 1 << V_55 ) ,
& V_2 -> V_37 ) ;
else
F_2 ( F_3 ( & V_2 -> V_37 ) & ~ ( 1 << V_55 ) ,
& V_2 -> V_37 ) ;
return V_36 ;
}
static T_6 F_23 (
struct V_6 * V_7 ,
const char * V_33 , T_7 V_36 )
{
struct V_1 T_1 * V_2 = V_7 -> V_11 ;
T_3 V_43 ;
T_2 V_56 ;
int V_53 ;
V_53 = F_21 ( V_33 , 0 , & V_43 ) ;
if ( V_53 )
return V_53 ;
if ( ! V_7 -> V_52 )
return - V_39 ;
if ( V_43 >= V_7 -> V_52 )
return - V_39 ;
V_56 = F_3 ( & V_2 -> V_57 ) ;
V_56 &= ~ V_58 ;
V_56 |= 0 << V_59 ;
V_56 &= ~ V_60 ;
V_56 |= 0 << V_61 ;
V_56 &= ~ V_62 ;
V_56 |= V_43 << V_63 ;
V_56 |= V_64 ;
F_2 ( V_56 , & V_2 -> V_57 ) ;
F_24 ( 1 ) ;
V_56 &= ~ V_64 ;
F_2 ( V_56 , & V_2 -> V_57 ) ;
return V_36 ;
}
static T_6 F_25 (
struct V_6 * V_7 ,
char * V_33 )
{
T_2 V_17 ;
F_5 ( V_7 , V_65 , 2 , & V_17 ) ;
return sprintf ( V_33 , L_7 , V_17 ) ;
}
static T_6 F_26 (
struct V_6 * V_7 ,
const char * V_33 , T_7 V_36 )
{
T_3 V_17 ;
int V_53 ;
V_53 = F_21 ( V_33 , 0 , & V_17 ) ;
if ( V_53 )
return V_53 ;
F_6 ( V_7 , V_65 , 2 , V_17 ) ;
return V_36 ;
}
static T_6 F_27 (
struct V_6 * V_7 ,
char * V_33 )
{
T_2 V_17 ;
F_5 ( V_7 , V_66 , 2 , & V_17 ) ;
return sprintf ( V_33 , L_7 , V_17 ) ;
}
static T_6 F_28 (
struct V_6 * V_7 ,
const char * V_33 , T_7 V_36 )
{
T_3 V_17 ;
int V_53 ;
V_53 = F_21 ( V_33 , 0 , & V_17 ) ;
if ( V_53 )
return V_53 ;
F_6 ( V_7 , V_66 , 2 , V_17 ) ;
return V_36 ;
}
static T_6 F_29 (
struct V_6 * V_7 ,
char * V_33 )
{
return sprintf ( V_33 , L_8 , V_7 -> V_67 ) ;
}
static T_6 F_30 (
struct V_6 * V_7 ,
const char * V_33 , T_7 V_36 )
{
T_3 V_9 ;
T_2 V_14 , V_68 ;
T_2 V_69 = 0 ;
int V_53 ;
V_53 = F_21 ( V_33 , 0 , & V_9 ) ;
if ( V_53 )
return V_53 ;
if ( V_9 <= 0x100 )
V_9 = 0x100 ;
else if ( V_9 >= 0x100000 )
V_9 = 0x100000 ;
else {
V_14 = 0 ;
V_68 = 0 ;
while ( V_14 < 21 ) {
V_14 = F_31 ( ( T_3 * ) & V_9 , 21 , V_14 ) ;
if ( V_14 != 21 )
V_68 = V_14 + 1 ;
V_14 ++ ;
V_69 ++ ;
}
if ( V_69 == 2 )
V_68 -- ;
V_9 = 1 << V_68 ;
}
V_7 -> V_67 = V_9 ;
F_6 ( V_7 , V_70 , 4 , V_9 - 1 ) ;
return V_36 ;
}
static T_6 F_32 (
struct V_6 * V_7 ,
char * V_33 )
{
struct V_1 T_1 * V_2 = V_7 -> V_11 ;
T_2 V_71 = F_3 ( & V_2 -> V_72 ) ;
return sprintf ( V_33 , L_7 , V_71 ) ;
}
static T_6 F_33 (
struct V_6 * V_7 ,
const char * V_33 , T_7 V_36 )
{
struct V_1 T_1 * V_2 = V_7 -> V_11 ;
T_3 V_71 ;
int V_53 ;
V_53 = F_21 ( V_33 , 0 , & V_71 ) ;
if ( V_53 )
return V_53 ;
V_71 &= ~ ( V_7 -> V_67 - 1 ) ;
if ( V_7 -> V_73 )
F_34 ( V_7 -> V_73 ) ;
V_7 -> V_73 = F_35 ( V_71 , V_7 -> V_67 ) ;
if ( ! V_7 -> V_73 )
return - V_74 ;
F_2 ( V_71 , & V_2 -> V_72 ) ;
return V_36 ;
}
static T_6 F_36 (
struct V_6 * V_7 ,
char * V_33 )
{
return sprintf ( V_33 , L_8 , V_7 -> V_75 ) ;
}
static T_6 F_37 (
struct V_6 * V_7 ,
const char * V_33 , T_7 V_36 )
{
T_3 V_76 ;
int V_53 ;
V_53 = F_21 ( V_33 , 0 , & V_76 ) ;
if ( V_53 )
return V_53 ;
if ( V_76 % 4 )
return - V_39 ;
V_7 -> V_75 = V_76 ;
return V_36 ;
}
static T_6 F_38 (
struct V_6 * V_7 ,
char * V_33 )
{
T_3 V_77 ;
if ( ! V_7 -> V_73 )
return - V_74 ;
V_77 = F_3 ( ( T_3 ) V_7 -> V_73 + V_7 -> V_75 ) ;
return sprintf ( V_33 , L_8 , V_77 ) ;
}
static T_6 F_39 (
struct V_6 * V_7 ,
const char * V_33 , T_7 V_36 )
{
T_3 V_77 ;
int V_53 ;
V_53 = F_21 ( V_33 , 0 , & V_77 ) ;
if ( V_53 )
return V_53 ;
if ( ! V_7 -> V_73 )
return - V_74 ;
F_2 ( V_77 , ( T_3 ) V_7 -> V_73 + V_7 -> V_75 ) ;
return V_36 ;
}
static struct V_78 * F_40 ( struct V_79 * V_80 )
{
return V_80 ?
F_41 ( F_42 ( F_43 ( V_80 ) ) ,
struct V_78 , V_81 ) : NULL ;
}
static T_6 F_44 ( struct V_79 * V_80 ,
struct V_82 * V_83 ,
char * V_33 )
{
T_6 V_53 = - V_39 ;
struct V_78 * V_84 = F_40 ( V_80 ) ;
struct V_85 * V_86 =
F_41 ( V_83 , struct V_85 , V_83 ) ;
if ( V_86 -> V_87 )
V_53 = V_86 -> V_87 ( & V_84 -> V_7 , V_33 ) ;
return V_53 ;
}
static T_6 F_45 ( struct V_79 * V_80 ,
struct V_82 * V_83 ,
const char * V_33 ,
T_7 V_36 )
{
T_6 V_53 = - V_39 ;
struct V_78 * V_84 = F_40 ( V_80 ) ;
struct V_85 * V_86 =
F_41 ( V_83 , struct V_85 , V_83 ) ;
if ( V_86 -> V_88 )
V_53 = V_86 -> V_88 ( & V_84 -> V_7 , V_33 , V_36 ) ;
return V_53 ;
}
static void F_46 ( struct V_6 * V_7 )
{
struct V_1 T_1 * V_2 = V_7 -> V_11 ;
F_2 ( V_7 -> V_89 , & V_2 -> V_90 ) ;
F_2 ( V_2 -> V_90 + V_91 ,
& V_2 -> V_92 ) ;
F_2 ( V_2 -> V_92 + 1 , & V_2 -> V_93 ) ;
F_2 ( V_2 -> V_93 + V_94 ,
& V_2 -> V_95 ) ;
F_2 ( V_2 -> V_95 + 1 , & V_2 -> V_96 ) ;
F_2 ( V_2 -> V_96 + V_97 ,
& V_2 -> V_98 ) ;
F_2 ( V_2 -> V_98 + 1 , & V_2 -> V_99 ) ;
F_2 ( V_2 -> V_99 + V_100 ,
& V_2 -> V_101 ) ;
F_2 ( V_2 -> V_101 + 1 , & V_2 -> V_102 ) ;
F_2 ( V_2 -> V_102 + V_103 ,
& V_2 -> V_104 ) ;
F_2 ( V_2 -> V_104 + 1 , & V_2 -> V_105 ) ;
F_2 ( V_2 -> V_105 + V_106 ,
& V_2 -> V_107 ) ;
F_2 ( V_2 -> V_90 , & V_2 -> V_108 ) ;
F_2 ( V_2 -> V_93 , & V_2 -> V_109 ) ;
F_2 ( V_2 -> V_96 , & V_2 -> V_110 ) ;
V_7 -> V_67 = V_111 + 1 ;
F_6 ( V_7 , V_70 , 4 , V_111 ) ;
F_6 ( V_7 , V_112 , 4 , 0xC ) ;
V_7 -> V_73 = F_35 ( V_113 ,
V_7 -> V_67 ) ;
F_2 ( V_113 , & V_2 -> V_72 ) ;
F_2 ( 0 , & V_2 -> V_114 ) ;
F_2 ( V_111 + 1 , & V_2 -> V_115 ) ;
F_2 ( 0x0 , & V_2 -> V_116 ) ;
F_2 ( 0x0 , & V_2 -> V_116 ) ;
F_2 ( 0x0 , & V_2 -> V_117 ) ;
F_2 ( V_118 | ( 1 << V_119 )
| ( ( T_2 ) 1 << V_120 ) ,
& V_2 -> V_37 ) ;
F_2 ( 0 , & V_2 -> V_121 ) ;
F_6 ( V_7 , V_44 , 1 , 1 ) ;
}
static int F_47 ( struct V_122 * V_123 )
{
struct V_124 * V_125 , * V_126 ;
unsigned int V_22 = 0 ;
int V_31 ;
struct V_127 * V_127 ;
static struct V_78 * V_84 ;
struct V_6 * V_7 ;
struct V_79 * V_128 ;
struct V_129 * V_81 ;
V_84 = F_48 ( & V_123 -> V_130 , sizeof( * V_84 ) , V_131 ) ;
if ( ! V_84 ) {
F_49 ( & V_123 -> V_130 , L_9 ) ;
return - V_74 ;
}
V_128 = & V_84 -> V_81 . V_132 . V_128 ;
sprintf ( V_128 -> V_133 , L_10 , V_123 -> V_17 ) ;
V_128 -> V_134 = & V_135 ;
V_7 = & V_84 -> V_7 ;
V_125 = F_50 ( V_123 , V_136 , 0 ) ;
V_7 -> V_11 = F_51 ( & V_123 -> V_130 , V_125 ) ;
if ( F_52 ( V_7 -> V_11 ) ) {
F_49 ( & V_123 -> V_130 , L_11 ) ;
return F_53 ( V_7 -> V_11 ) ;
}
V_126 = F_50 ( V_123 , V_136 , 1 ) ;
V_7 -> V_89 = ( void T_1 * ) V_126 -> V_137 ;
V_7 -> V_13 = F_51 ( & V_123 -> V_130 , V_126 ) ;
if ( F_52 ( V_7 -> V_13 ) ) {
F_49 ( & V_123 -> V_130 , L_11 ) ;
return F_53 ( V_7 -> V_13 ) ;
}
F_54 ( V_123 , V_84 ) ;
V_31 = F_55 ( V_123 , 0 ) ;
if ( V_31 < 0 ) {
F_49 ( & V_123 -> V_130 , L_12 ) ;
return V_31 ;
}
V_22 = F_56 ( & V_123 -> V_130 , V_31 , F_13 ,
0 , V_123 -> V_138 , NULL ) ;
if ( V_22 ) {
F_49 ( & V_123 -> V_130 ,
L_13 , V_31 ) ;
return V_22 ;
}
V_81 = & V_84 -> V_81 ;
F_57 ( & V_81 -> V_132 ) ;
F_58 ( & V_81 -> V_139 ) ;
V_22 = F_59 ( V_81 ) ;
if ( V_22 )
return V_22 ;
if ( V_123 -> V_17 == 1 ) {
V_127 = F_60 ( L_14 , NULL ) ;
if ( F_52 ( V_127 ) ) {
F_61 ( L_15 , V_140 ) ;
return F_53 ( V_127 ) ;
}
V_22 = F_62 ( V_127 ) ;
if ( V_22 ) {
F_61 ( L_16 , V_140 ) ;
return V_22 ;
}
} else if ( V_123 -> V_17 == 2 ) {
V_127 = F_60 ( L_17 , NULL ) ;
if ( F_52 ( V_127 ) ) {
F_61 ( L_18 , V_140 ) ;
return F_53 ( V_127 ) ;
}
V_22 = F_62 ( V_127 ) ;
if ( V_22 ) {
F_61 ( L_19 , V_140 ) ;
return V_22 ;
}
}
F_46 ( V_7 ) ;
return 0 ;
}
static int F_63 ( struct V_122 * V_123 )
{
static struct V_78 * V_84 ;
V_84 = F_64 ( V_123 ) ;
F_65 ( & V_84 -> V_81 ) ;
return 0 ;
}
static void F_66 ( struct V_122 * V_123 )
{
}
