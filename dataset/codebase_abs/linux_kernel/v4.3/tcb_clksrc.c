static T_1 F_1 ( struct V_1 * V_2 )
{
unsigned long V_3 ;
T_2 V_4 , V_5 ;
F_2 ( V_3 ) ;
do {
V_5 = F_3 ( V_6 + F_4 ( 1 , V_7 ) ) ;
V_4 = F_3 ( V_6 + F_4 ( 0 , V_7 ) ) ;
} while ( V_5 != F_3 ( V_6 + F_4 ( 1 , V_7 ) ) );
F_5 ( V_3 ) ;
return ( V_5 << 16 ) | V_4 ;
}
static T_1 F_6 ( struct V_1 * V_2 )
{
return F_3 ( V_6 + F_4 ( 0 , V_7 ) ) ;
}
static struct V_8 * F_7 ( struct V_9 * V_10 )
{
return F_8 ( V_10 , struct V_8 , V_10 ) ;
}
static int F_9 ( struct V_9 * V_11 )
{
struct V_8 * V_12 = F_7 ( V_11 ) ;
void T_3 * V_13 = V_12 -> V_13 ;
F_10 ( 0xff , V_13 + F_4 ( 2 , V_14 ) ) ;
F_10 ( V_15 , V_13 + F_4 ( 2 , V_16 ) ) ;
F_11 ( V_12 -> V_17 ) ;
return 0 ;
}
static int F_12 ( struct V_9 * V_11 )
{
struct V_8 * V_12 = F_7 ( V_11 ) ;
void T_3 * V_13 = V_12 -> V_13 ;
if ( F_13 ( V_11 ) || F_14 ( V_11 ) )
F_9 ( V_11 ) ;
F_15 ( V_12 -> V_17 ) ;
F_10 ( V_18 | V_19 | V_20 |
V_21 , V_13 + F_4 ( 2 , V_22 ) ) ;
F_10 ( V_23 , V_13 + F_4 ( 2 , V_24 ) ) ;
return 0 ;
}
static int F_16 ( struct V_9 * V_11 )
{
struct V_8 * V_12 = F_7 ( V_11 ) ;
void T_3 * V_13 = V_12 -> V_13 ;
if ( F_13 ( V_11 ) || F_14 ( V_11 ) )
F_9 ( V_11 ) ;
F_15 ( V_12 -> V_17 ) ;
F_10 ( V_18 | V_20 | V_21 ,
V_13 + F_4 ( 2 , V_22 ) ) ;
F_10 ( ( 32768 + V_25 / 2 ) / V_25 , V_6 + F_4 ( 2 , V_26 ) ) ;
F_10 ( V_23 , V_13 + F_4 ( 2 , V_24 ) ) ;
F_10 ( V_27 | V_28 , V_13 +
F_4 ( 2 , V_16 ) ) ;
return 0 ;
}
static int F_17 ( unsigned long V_29 , struct V_9 * V_11 )
{
F_10 ( V_29 , V_6 + F_4 ( 2 , V_26 ) ) ;
F_10 ( V_27 | V_28 ,
V_6 + F_4 ( 2 , V_16 ) ) ;
return 0 ;
}
static T_4 F_18 ( int V_30 , void * V_31 )
{
struct V_8 * V_32 = V_31 ;
unsigned int V_33 ;
V_33 = F_3 ( V_32 -> V_13 + F_4 ( 2 , V_34 ) ) ;
if ( V_33 & V_23 ) {
V_32 -> V_10 . V_35 ( & V_32 -> V_10 ) ;
return V_36 ;
}
return V_37 ;
}
static int T_5 F_19 ( struct V_38 * V_39 , int V_40 )
{
int V_41 ;
struct V_17 * V_42 = V_39 -> V_17 [ 2 ] ;
int V_30 = V_39 -> V_30 [ 2 ] ;
V_41 = F_20 ( V_42 ) ;
if ( V_41 )
return V_41 ;
F_11 ( V_42 ) ;
V_10 . V_13 = V_39 -> V_13 ;
V_10 . V_17 = V_42 ;
V_18 = V_40 ;
V_10 . V_10 . V_43 = F_21 ( 0 ) ;
V_41 = F_22 ( V_30 , F_18 , V_44 , L_1 , & V_10 ) ;
if ( V_41 ) {
F_23 ( V_42 ) ;
return V_41 ;
}
F_24 ( & V_10 . V_10 , 32768 , 1 , 0xffff ) ;
return V_41 ;
}
static int T_5 F_19 ( struct V_38 * V_39 , int V_40 )
{
return 0 ;
}
static void T_5 F_25 ( struct V_38 * V_39 , int V_45 )
{
F_10 ( V_45
| V_20
| V_46
| V_47
| V_48 ,
V_6 + F_4 ( 0 , V_22 ) ) ;
F_10 ( 0x0000 , V_6 + F_4 ( 0 , V_49 ) ) ;
F_10 ( 0x8000 , V_6 + F_4 ( 0 , V_26 ) ) ;
F_10 ( 0xff , V_6 + F_4 ( 0 , V_14 ) ) ;
F_10 ( V_27 , V_6 + F_4 ( 0 , V_16 ) ) ;
F_10 ( V_50
| V_20
| V_46 ,
V_6 + F_4 ( 1 , V_22 ) ) ;
F_10 ( 0xff , V_6 + F_4 ( 1 , V_14 ) ) ;
F_10 ( V_27 , V_6 + F_4 ( 1 , V_16 ) ) ;
F_10 ( V_51 , V_6 + V_52 ) ;
F_10 ( V_53 , V_6 + V_54 ) ;
}
static void T_5 F_26 ( struct V_38 * V_39 , int V_45 )
{
F_10 ( V_45
| V_20
| V_46 ,
V_6 + F_4 ( 0 , V_22 ) ) ;
F_10 ( 0xff , V_6 + F_4 ( 0 , V_14 ) ) ;
F_10 ( V_27 , V_6 + F_4 ( 0 , V_16 ) ) ;
F_10 ( V_53 , V_6 + V_54 ) ;
}
static int T_5 F_27 ( void )
{
static char V_55 [] V_56
= V_57 L_2 ;
struct V_58 * V_59 ;
struct V_38 * V_39 ;
struct V_17 * V_60 ;
T_2 V_61 , V_62 = 0 ;
int V_63 = - 1 ;
int V_40 = - 1 ;
int V_64 ;
int V_41 ;
V_39 = F_28 ( V_65 ) ;
if ( ! V_39 ) {
F_29 ( L_3 ) ;
return - V_66 ;
}
V_6 = V_39 -> V_13 ;
V_59 = V_39 -> V_59 ;
V_60 = V_39 -> V_17 [ 0 ] ;
V_41 = F_20 ( V_60 ) ;
if ( V_41 ) {
F_29 ( L_4 ) ;
goto V_67;
}
V_61 = ( T_2 ) F_30 ( V_60 ) ;
for ( V_64 = 0 ; V_64 < 5 ; V_64 ++ ) {
unsigned V_68 = V_69 [ V_64 ] ;
unsigned V_70 ;
if ( ! V_68 ) {
V_40 = V_64 ;
continue;
}
V_70 = V_61 / V_68 ;
F_29 ( L_5 , V_61 , V_68 , V_64 , V_70 ) ;
if ( V_63 > 0 ) {
if ( V_70 < 5 * 1000 * 1000 )
continue;
}
V_62 = V_70 ;
V_63 = V_64 ;
}
F_31 ( V_55 , V_71 . V_72 , V_65 ,
V_62 / 1000000 ,
( ( V_62 + 500000 ) % 1000000 ) / 1000 ) ;
if ( V_39 -> V_73 && V_39 -> V_73 -> V_74 == 32 ) {
V_71 . V_75 = F_6 ;
F_26 ( V_39 , V_63 ) ;
} else {
V_41 = F_20 ( V_39 -> V_17 [ 1 ] ) ;
if ( V_41 ) {
F_29 ( L_6 ) ;
goto V_76;
}
F_25 ( V_39 , V_63 ) ;
}
V_41 = F_32 ( & V_71 , V_62 ) ;
if ( V_41 )
goto V_77;
V_41 = F_19 ( V_39 , V_40 ) ;
if ( V_41 )
goto V_78;
return 0 ;
V_78:
F_33 ( & V_71 ) ;
V_77:
if ( ! V_39 -> V_73 || V_39 -> V_73 -> V_74 != 32 )
F_23 ( V_39 -> V_17 [ 1 ] ) ;
V_76:
F_23 ( V_60 ) ;
V_67:
F_34 ( V_39 ) ;
return V_41 ;
}
