static int T_1 F_1 ( char * V_1 )
{
if ( ! strncmp ( V_1 , L_1 , 5 ) )
V_2 = true ;
return 1 ;
}
static void F_2 ( struct V_3 * V_4 )
{
F_3 ( F_4 ( V_4 ) ) ;
}
static int F_5 (
struct V_5 * V_6 , int V_7 , int V_8 )
{
int V_9 ;
for ( V_9 = 0 ; V_9 < V_8 ; V_9 ++ )
if ( V_6 [ V_9 ] . V_10 == V_7 )
return V_9 ;
V_6 [ V_9 ] . V_10 = V_7 ;
return V_9 ;
}
static int F_6 ( struct V_11 * V_4 , int * V_12 , int V_13 )
{
struct V_5 * V_14 ;
int V_15 [ V_16 ] ;
int V_8 , V_17 , V_18 , V_19 , V_20 ;
T_2 V_21 ;
T_3 V_22 ;
V_8 = F_7 ( V_4 ) ;
if ( ! V_8 )
return - V_23 ;
if ( V_8 > V_24 )
V_8 = V_24 ;
V_14 = F_8 ( sizeof( * V_14 ) * V_8 , V_25 ) ;
if ( ! V_14 )
return - V_26 ;
for ( V_19 = 0 ; V_19 < V_8 ; V_19 ++ )
V_14 [ V_19 ] . V_10 = V_19 ;
V_17 = F_9 ( V_4 , V_14 , V_8 ) ;
if ( V_17 )
goto Exit;
for ( V_19 = 0 ; V_19 < V_16 ; V_19 ++ )
V_15 [ V_19 ] = - 1 ;
V_17 = - V_27 ;
V_20 = 0 ;
if ( V_13 & ( V_28 | V_29 ) ) {
int V_10 ;
V_18 = F_10 ( V_4 ) ;
F_11 ( V_4 , V_18 + V_30 , & V_21 ) ;
V_10 = ( V_21 & V_31 ) >> 9 ;
if ( V_10 >= V_8 )
goto Error;
V_19 = F_5 ( V_14 , V_10 , V_20 ) ;
if ( V_19 == V_20 )
V_20 ++ ;
V_15 [ V_32 ] = V_19 ;
V_15 [ V_33 ] = V_19 ;
}
if ( V_13 & V_34 ) {
int V_10 ;
V_18 = F_12 ( V_4 , V_35 ) ;
F_13 ( V_4 , V_18 + V_36 , & V_22 ) ;
V_10 = V_22 >> 27 ;
if ( V_10 >= V_8 )
goto Error;
V_19 = F_5 ( V_14 , V_10 , V_20 ) ;
if ( V_19 == V_20 )
V_20 ++ ;
V_15 [ V_37 ] = V_19 ;
}
if ( V_20 == V_8 ) {
V_17 = 0 ;
} else {
F_14 ( V_4 ) ;
V_17 = F_9 ( V_4 , V_14 , V_20 ) ;
if ( V_17 )
goto Exit;
}
for ( V_19 = 0 ; V_19 < V_16 ; V_19 ++ )
V_12 [ V_19 ] = V_15 [ V_19 ] >= 0 ? V_14 [ V_15 [ V_19 ] ] . V_38 : - 1 ;
Exit:
F_3 ( V_14 ) ;
return V_17 ;
Error:
F_14 ( V_4 ) ;
goto Exit;
}
static int F_15 ( struct V_11 * V_4 , int * V_39 , int V_13 )
{
int V_19 , V_40 = - 1 ;
if ( ( ( V_13 & V_28 ) && F_16 () ) ||
( ( V_13 & V_29 ) && F_17 () ) ) {
if ( V_4 -> V_40 )
V_40 = V_4 -> V_40 ;
goto V_41;
}
if ( ! F_6 ( V_4 , V_39 , V_13 ) )
return 0 ;
if ( ! F_18 ( V_4 ) || V_4 -> V_40 )
V_40 = V_4 -> V_40 ;
V_41:
for ( V_19 = 0 ; V_19 < V_16 ; V_19 ++ )
V_39 [ V_19 ] = V_40 ;
V_39 [ V_42 ] = - 1 ;
if ( V_40 < 0 )
return - V_43 ;
return 0 ;
}
static void F_19 ( struct V_11 * V_4 )
{
if ( V_4 -> V_44 )
F_14 ( V_4 ) ;
else if ( V_4 -> V_45 )
F_20 ( V_4 ) ;
}
static int F_21 ( struct V_11 * V_4 )
{
int V_46 = 0 ;
T_3 V_22 ;
int V_47 = 0 ;
int V_48 ;
if ( V_49 )
return 0 ;
V_48 = F_22 ( V_4 , & V_47 ) ;
if ( ! V_50 ) {
V_47 = V_28 | V_29
| V_51 ;
if ( F_23 () )
V_47 |= V_34 ;
} else if ( V_48 ) {
return 0 ;
}
if ( ( V_47 & V_29 ) &&
V_4 -> V_52 & V_53 ) {
F_24 ( V_4 , V_54 , & V_22 ) ;
if ( V_22 & V_55 ) {
V_46 |= V_29 ;
F_25 ( V_4 , V_56 ,
V_57 | V_58 ) ;
}
}
if ( ( V_47 & V_34 )
&& F_12 ( V_4 , V_35 ) ) {
V_46 |= V_34 ;
F_26 ( V_4 ) ;
}
if ( F_12 ( V_4 , V_59 ) )
V_46 |= V_51 ;
if ( ( V_47 & V_28 )
&& F_27 ( V_4 ) == V_60 ) {
V_46 |= V_28 ;
F_28 ( V_4 , false ) ;
}
return V_46 ;
}
static int F_29 ( struct V_11 * V_61 , int V_62 , int V_40 )
{
int V_63 ;
struct V_64 * V_65 ;
struct V_3 * V_3 ;
V_65 = F_8 ( sizeof( * V_65 ) , V_25 ) ;
if ( ! V_65 )
return - V_26 ;
V_65 -> V_66 = V_61 ;
V_65 -> V_40 = V_40 ;
V_65 -> V_62 = V_62 ;
V_3 = & V_65 -> V_3 ;
V_3 -> V_67 = & V_68 ;
V_3 -> V_69 = F_2 ;
F_30 ( V_3 , L_2 ,
F_31 ( V_61 ) ,
F_32 ( F_27 ( V_61 ) , V_62 ) ) ;
V_3 -> V_70 = & V_61 -> V_4 ;
F_33 ( V_3 ) ;
V_63 = F_34 ( V_3 ) ;
if ( V_63 )
F_3 ( V_65 ) ;
else
F_35 ( V_3 ) ;
return V_63 ;
}
int F_36 ( struct V_11 * V_4 )
{
int V_17 , V_71 , V_19 , V_72 ;
int V_39 [ V_16 ] ;
V_17 = F_37 ( V_4 ) ;
if ( V_17 )
return V_17 ;
V_71 = F_21 ( V_4 ) ;
if ( ! V_71 )
return 0 ;
F_38 ( V_4 ) ;
V_17 = F_15 ( V_4 , V_39 , V_71 ) ;
if ( V_17 ) {
V_71 &= V_51 ;
if ( ! V_71 )
goto V_73;
}
V_17 = - V_43 ;
V_72 = 0 ;
for ( V_19 = 0 ; V_19 < V_16 ; V_19 ++ ) {
int V_62 = 1 << V_19 ;
if ( ! ( V_71 & V_62 ) )
continue;
if ( ! F_29 ( V_4 , V_62 , V_39 [ V_19 ] ) )
V_72 ++ ;
}
if ( ! V_72 )
goto V_74;
return 0 ;
V_74:
F_19 ( V_4 ) ;
V_73:
F_39 ( V_4 ) ;
return V_17 ;
}
static int F_40 ( struct V_3 * V_4 , void * V_75 )
{
struct V_76 * V_77 ;
if ( ( V_4 -> V_67 == & V_68 ) && V_4 -> V_78 ) {
V_77 = F_41 ( V_4 -> V_78 ) ;
if ( V_77 -> V_79 )
V_77 -> V_79 ( F_4 ( V_4 ) ) ;
}
return 0 ;
}
int F_42 ( struct V_3 * V_4 )
{
return F_43 ( V_4 , NULL , F_40 ) ;
}
static int F_44 ( struct V_3 * V_4 , void * V_75 )
{
struct V_76 * V_77 ;
if ( ( V_4 -> V_67 == & V_68 ) &&
( V_4 -> V_78 ) ) {
V_77 = F_41 ( V_4 -> V_78 ) ;
if ( V_77 -> V_80 )
V_77 -> V_80 ( F_4 ( V_4 ) ) ;
}
return 0 ;
}
int F_45 ( struct V_3 * V_4 )
{
return F_43 ( V_4 , NULL , F_44 ) ;
}
static int F_46 ( struct V_3 * V_4 , void * V_75 )
{
if ( V_4 -> V_67 == & V_68 ) {
F_47 ( V_4 ) ;
F_48 ( V_4 ) ;
}
return 0 ;
}
void F_49 ( struct V_11 * V_4 )
{
F_43 ( & V_4 -> V_4 , NULL , F_46 ) ;
F_19 ( V_4 ) ;
F_39 ( V_4 ) ;
}
static int F_50 ( struct V_3 * V_4 )
{
struct V_64 * V_81 ;
struct V_76 * V_78 ;
int V_17 ;
if ( ! V_4 || ! V_4 -> V_78 )
return - V_43 ;
V_78 = F_41 ( V_4 -> V_78 ) ;
if ( ! V_78 || ! V_78 -> V_82 )
return - V_43 ;
V_81 = F_4 ( V_4 ) ;
V_17 = V_78 -> V_82 ( V_81 ) ;
if ( ! V_17 ) {
F_51 ( V_83 , V_4 , L_3 ,
V_78 -> V_84 ) ;
F_35 ( V_4 ) ;
}
return V_17 ;
}
static int F_52 ( struct V_3 * V_4 )
{
struct V_64 * V_81 ;
struct V_76 * V_78 ;
if ( ! V_4 || ! V_4 -> V_78 )
return 0 ;
V_81 = F_4 ( V_4 ) ;
V_78 = F_41 ( V_4 -> V_78 ) ;
if ( V_78 && V_78 -> remove ) {
F_51 ( V_83 , V_4 , L_4 ,
V_78 -> V_84 ) ;
V_78 -> remove ( V_81 ) ;
F_47 ( V_4 ) ;
}
return 0 ;
}
static void F_53 ( struct V_3 * V_4 ) {}
int F_54 ( struct V_76 * V_85 )
{
if ( V_49 )
return - V_43 ;
V_85 -> V_78 . V_84 = ( char * ) V_85 -> V_84 ;
V_85 -> V_78 . V_67 = & V_68 ;
V_85 -> V_78 . V_82 = F_50 ;
V_85 -> V_78 . remove = F_52 ;
V_85 -> V_78 . V_86 = F_53 ;
return F_55 ( & V_85 -> V_78 ) ;
}
void F_56 ( struct V_76 * V_87 )
{
F_57 ( & V_87 -> V_78 ) ;
}
