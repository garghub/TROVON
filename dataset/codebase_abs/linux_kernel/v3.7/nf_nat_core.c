inline const struct V_1 *
F_1 ( T_1 V_2 )
{
return F_2 ( V_3 [ V_2 ] ) ;
}
inline const struct V_4 *
F_3 ( T_1 V_2 , T_1 V_5 )
{
return F_2 ( V_6 [ V_2 ] [ V_5 ] ) ;
}
static void F_4 ( struct V_7 * V_8 , struct V_9 * V_10 )
{
const struct V_1 * V_11 ;
const struct V_12 * V_13 ;
enum V_14 V_15 ;
enum V_16 V_17 ;
unsigned long V_18 ;
T_1 V_2 ;
V_13 = F_5 ( V_8 , & V_15 ) ;
if ( V_13 == NULL )
return;
V_2 = V_13 -> V_19 [ V_20 ] . V_21 . V_22 . V_23 ;
F_6 () ;
V_11 = F_1 ( V_2 ) ;
if ( V_11 == NULL )
goto V_24;
V_17 = F_7 ( V_15 ) ;
if ( V_17 == V_20 )
V_18 = V_25 ;
else
V_18 = V_26 ;
V_11 -> V_27 ( V_8 , V_13 , V_17 , V_18 , V_10 ) ;
V_24:
F_8 () ;
}
int F_9 ( struct V_7 * V_8 , unsigned int V_2 )
{
struct V_9 V_10 ;
unsigned int V_28 ;
struct V_29 * V_30 ;
if ( F_10 ( V_8 , & V_10 , V_2 ) < 0 )
return - 1 ;
V_30 = F_11 ( V_8 ) ;
if ( V_30 -> V_31 )
V_30 = ( (struct V_32 * ) V_30 ) -> V_33 ;
F_12 ( V_30 ) ;
V_30 = F_13 ( F_14 ( V_30 -> V_34 ) , V_30 , & V_10 , V_8 -> V_35 , 0 ) ;
if ( F_15 ( V_30 ) )
return - 1 ;
F_16 ( V_8 ) ;
F_17 ( V_8 , V_30 ) ;
V_28 = F_11 ( V_8 ) -> V_34 -> V_36 ;
if ( F_18 ( V_8 ) < V_28 &&
F_19 ( V_8 , V_28 - F_18 ( V_8 ) , 0 , V_37 ) )
return - 1 ;
return 0 ;
}
static inline unsigned int
F_20 ( const struct V_38 * V_38 , T_2 V_39 ,
const struct V_40 * V_21 )
{
unsigned int V_41 ;
V_41 = F_21 ( ( V_42 * ) & V_21 -> V_22 , sizeof( V_21 -> V_22 ) / sizeof( V_42 ) ,
V_21 -> V_30 . V_5 ^ V_39 ^ V_43 ) ;
return ( ( V_44 ) V_41 * V_38 -> V_13 . V_45 ) >> 32 ;
}
int
F_22 ( const struct V_40 * V_21 ,
const struct V_12 * V_46 )
{
struct V_40 V_47 ;
F_23 ( & V_47 , V_21 ) ;
return F_24 ( & V_47 , V_46 ) ;
}
static int F_25 ( const struct V_1 * V_11 ,
const struct V_4 * V_48 ,
const struct V_40 * V_21 ,
const struct V_49 * V_50 )
{
if ( V_50 -> V_51 & V_52 &&
! V_11 -> F_25 ( V_21 , V_50 ) )
return 0 ;
if ( ! ( V_50 -> V_51 & V_53 ) ||
V_48 -> F_25 ( V_21 , V_54 ,
& V_50 -> V_55 , & V_50 -> V_56 ) )
return 1 ;
return 0 ;
}
static inline int
F_26 ( const struct V_12 * V_13 ,
const struct V_40 * V_21 )
{
const struct V_40 * V_57 ;
V_57 = & V_13 -> V_19 [ V_20 ] . V_21 ;
return ( V_57 -> V_30 . V_5 == V_21 -> V_30 . V_5 &&
F_27 ( & V_57 -> V_22 . V_58 , & V_21 -> V_22 . V_58 ) &&
V_57 -> V_22 . V_59 . V_60 == V_21 -> V_22 . V_59 . V_60 ) ;
}
static int
F_28 ( struct V_38 * V_38 , T_2 V_39 ,
const struct V_1 * V_11 ,
const struct V_4 * V_48 ,
const struct V_40 * V_21 ,
struct V_40 * V_61 ,
const struct V_49 * V_50 )
{
unsigned int V_62 = F_20 ( V_38 , V_39 , V_21 ) ;
const struct V_63 * V_64 ;
const struct V_12 * V_13 ;
const struct V_65 * V_66 ;
F_29 (nat, n, &net->ct.nat_bysource[h], bysource) {
V_13 = V_64 -> V_13 ;
if ( F_26 ( V_13 , V_21 ) && F_30 ( V_13 ) == V_39 ) {
F_23 ( V_61 ,
& V_13 -> V_19 [ V_67 ] . V_21 ) ;
V_61 -> V_30 = V_21 -> V_30 ;
if ( F_25 ( V_11 , V_48 , V_61 , V_50 ) )
return 1 ;
}
}
return 0 ;
}
static void
F_31 ( T_2 V_39 , struct V_40 * V_21 ,
const struct V_49 * V_50 ,
const struct V_12 * V_13 ,
enum V_68 V_69 )
{
union V_70 * V_71 ;
unsigned int V_72 , V_73 ;
V_42 V_74 , V_75 , V_76 , V_77 ;
bool V_78 ;
if ( ! ( V_50 -> V_51 & V_52 ) )
return;
if ( V_69 == V_54 )
V_71 = & V_21 -> V_22 . V_58 ;
else
V_71 = & V_21 -> V_30 . V_58 ;
if ( F_27 ( & V_50 -> V_79 , & V_50 -> V_80 ) ) {
* V_71 = V_50 -> V_79 ;
return;
}
if ( F_32 ( V_13 ) == V_81 )
V_73 = sizeof( V_71 -> V_82 ) / sizeof( V_42 ) - 1 ;
else
V_73 = sizeof( V_71 -> V_83 ) / sizeof( V_42 ) - 1 ;
V_76 = F_21 ( ( V_42 * ) & V_21 -> V_22 . V_58 , sizeof( V_21 -> V_22 . V_58 ) / sizeof( V_42 ) ,
V_50 -> V_51 & V_84 ?
0 : ( V_85 V_42 ) V_21 -> V_30 . V_58 . V_60 [ V_73 ] ^ V_39 ) ;
V_78 = false ;
for ( V_72 = 0 ; V_72 <= V_73 ; V_72 ++ ) {
if ( ! V_78 ) {
V_74 = F_33 ( ( V_85 V_86 ) V_50 -> V_79 . V_60 [ V_72 ] ) ;
V_75 = F_33 ( ( V_85 V_86 ) V_50 -> V_80 . V_60 [ V_72 ] ) ;
V_77 = V_75 - V_74 + 1 ;
} else {
V_74 = 0 ;
V_77 = ~ 0 ;
}
V_71 -> V_60 [ V_72 ] = ( V_85 V_87 )
F_34 ( V_74 + ( ( ( V_44 ) V_76 * V_77 ) >> 32 ) ) ;
if ( V_71 -> V_60 [ V_72 ] != V_50 -> V_80 . V_60 [ V_72 ] )
V_78 = true ;
if ( ! ( V_50 -> V_51 & V_84 ) )
V_76 ^= ( V_85 V_42 ) V_21 -> V_30 . V_58 . V_60 [ V_72 ] ;
}
}
static void
F_35 ( struct V_40 * V_21 ,
const struct V_40 * V_88 ,
const struct V_49 * V_50 ,
struct V_12 * V_13 ,
enum V_68 V_69 )
{
const struct V_1 * V_11 ;
const struct V_4 * V_48 ;
struct V_38 * V_38 = F_36 ( V_13 ) ;
T_2 V_39 = F_30 ( V_13 ) ;
F_6 () ;
V_11 = F_1 ( V_88 -> V_22 . V_23 ) ;
V_48 = F_3 ( V_88 -> V_22 . V_23 ,
V_88 -> V_30 . V_5 ) ;
if ( V_69 == V_54 &&
! ( V_50 -> V_51 & V_89 ) ) {
if ( F_25 ( V_11 , V_48 , V_88 , V_50 ) ) {
if ( ! F_22 ( V_88 , V_13 ) ) {
* V_21 = * V_88 ;
goto V_24;
}
} else if ( F_28 ( V_38 , V_39 , V_11 , V_48 ,
V_88 , V_21 , V_50 ) ) {
F_37 ( L_1 ) ;
if ( ! F_22 ( V_21 , V_13 ) )
goto V_24;
}
}
* V_21 = * V_88 ;
F_31 ( V_39 , V_21 , V_50 , V_13 , V_69 ) ;
if ( ! ( V_50 -> V_51 & V_89 ) ) {
if ( V_50 -> V_51 & V_53 ) {
if ( V_48 -> F_25 ( V_21 , V_69 ,
& V_50 -> V_55 ,
& V_50 -> V_56 ) &&
( V_50 -> V_55 . V_60 == V_50 -> V_56 . V_60 ||
! F_22 ( V_21 , V_13 ) ) )
goto V_24;
} else if ( ! F_22 ( V_21 , V_13 ) ) {
goto V_24;
}
}
V_48 -> V_90 ( V_11 , V_21 , V_50 , V_69 , V_13 ) ;
V_24:
F_8 () ;
}
unsigned int
F_38 ( struct V_12 * V_13 ,
const struct V_49 * V_50 ,
enum V_68 V_69 )
{
struct V_38 * V_38 = F_36 ( V_13 ) ;
struct V_40 V_91 , V_92 ;
struct V_63 * V_64 ;
V_64 = F_39 ( V_13 ) ;
if ( ! V_64 ) {
V_64 = F_40 ( V_13 , V_93 , V_37 ) ;
if ( V_64 == NULL ) {
F_37 ( L_2 ) ;
return V_94 ;
}
}
F_41 ( V_69 == V_54 ||
V_69 == V_95 ) ;
F_42 ( F_43 ( V_13 , V_69 ) ) ;
F_23 ( & V_91 ,
& V_13 -> V_19 [ V_67 ] . V_21 ) ;
F_35 ( & V_92 , & V_91 , V_50 , V_13 , V_69 ) ;
if ( ! F_44 ( & V_92 , & V_91 ) ) {
struct V_40 V_47 ;
F_23 ( & V_47 , & V_92 ) ;
F_45 ( V_13 , & V_47 ) ;
if ( V_69 == V_54 )
V_13 -> V_96 |= V_26 ;
else
V_13 -> V_96 |= V_25 ;
}
if ( V_69 == V_54 ) {
unsigned int V_97 ;
V_97 = F_20 ( V_38 , F_30 ( V_13 ) ,
& V_13 -> V_19 [ V_20 ] . V_21 ) ;
F_46 ( & V_98 ) ;
V_64 = F_39 ( V_13 ) ;
V_64 -> V_13 = V_13 ;
F_47 ( & V_64 -> V_99 ,
& V_38 -> V_13 . V_100 [ V_97 ] ) ;
F_48 ( & V_98 ) ;
}
if ( V_69 == V_95 )
V_13 -> V_96 |= V_101 ;
else
V_13 -> V_96 |= V_102 ;
return V_94 ;
}
unsigned int F_49 ( struct V_12 * V_13 ,
enum V_14 V_15 ,
unsigned int V_103 ,
struct V_7 * V_8 )
{
const struct V_1 * V_11 ;
const struct V_4 * V_48 ;
enum V_16 V_17 = F_7 ( V_15 ) ;
unsigned long V_18 ;
enum V_68 V_104 = F_50 ( V_103 ) ;
if ( V_104 == V_54 )
V_18 = V_26 ;
else
V_18 = V_25 ;
if ( V_17 == V_67 )
V_18 ^= V_105 ;
if ( V_13 -> V_96 & V_18 ) {
struct V_40 V_106 ;
F_23 ( & V_106 , & V_13 -> V_19 [ ! V_17 ] . V_21 ) ;
V_11 = F_1 ( V_106 . V_22 . V_23 ) ;
V_48 = F_3 ( V_106 . V_22 . V_23 ,
V_106 . V_30 . V_5 ) ;
if ( ! V_11 -> V_107 ( V_8 , 0 , V_48 , & V_106 , V_104 ) )
return V_108 ;
}
return V_94 ;
}
static int F_51 ( struct V_12 * V_72 , void * V_109 )
{
const struct F_51 * V_110 = V_109 ;
struct V_63 * V_64 = F_39 ( V_72 ) ;
if ( ! V_64 )
return 0 ;
if ( ! ( V_72 -> V_96 & V_102 ) )
return 0 ;
if ( ( V_110 -> V_11 && F_32 ( V_72 ) != V_110 -> V_11 ) ||
( V_110 -> V_48 && F_52 ( V_72 ) != V_110 -> V_48 ) )
return 0 ;
if ( V_110 -> V_41 ) {
F_46 ( & V_98 ) ;
F_53 ( & V_64 -> V_99 ) ;
F_48 ( & V_98 ) ;
} else {
memset ( V_64 , 0 , sizeof( * V_64 ) ) ;
V_72 -> V_96 &= ~ ( V_105 | V_111 |
V_112 ) ;
}
return 0 ;
}
static void F_54 ( T_1 V_11 , T_1 V_48 )
{
struct F_51 V_110 = {
. V_11 = V_11 ,
. V_48 = V_48 ,
} ;
struct V_38 * V_38 ;
F_55 () ;
V_110 . V_41 = true ;
F_56 (net)
F_57 ( V_38 , F_51 , & V_110 ) ;
F_58 () ;
V_110 . V_41 = false ;
F_56 (net)
F_57 ( V_38 , F_51 , & V_110 ) ;
F_59 () ;
}
static void F_60 ( T_1 V_11 )
{
struct F_51 V_110 = {
. V_11 = V_11 ,
} ;
struct V_38 * V_38 ;
F_55 () ;
V_110 . V_41 = true ;
F_56 (net)
F_57 ( V_38 , F_51 , & V_110 ) ;
F_58 () ;
V_110 . V_41 = false ;
F_56 (net)
F_57 ( V_38 , F_51 , & V_110 ) ;
F_59 () ;
}
int F_61 ( T_1 V_11 , const struct V_4 * V_48 )
{
const struct V_4 * * V_113 ;
unsigned int V_72 ;
int V_114 = 0 ;
F_62 ( & V_115 ) ;
if ( V_6 [ V_11 ] == NULL ) {
V_113 = F_63 ( V_116 * sizeof( struct V_4 * ) ,
V_117 ) ;
if ( V_113 == NULL ) {
V_114 = - V_118 ;
goto V_24;
}
for ( V_72 = 0 ; V_72 < V_116 ; V_72 ++ )
F_64 ( V_113 [ V_72 ] , & V_119 ) ;
F_65 () ;
V_6 [ V_11 ] = V_113 ;
}
if ( F_66 (
V_6 [ V_11 ] [ V_48 -> V_48 ] ,
F_67 ( & V_115 )
) != & V_119 ) {
V_114 = - V_120 ;
goto V_24;
}
F_64 ( V_6 [ V_11 ] [ V_48 -> V_48 ] , V_48 ) ;
V_24:
F_68 ( & V_115 ) ;
return V_114 ;
}
void F_69 ( T_1 V_11 , const struct V_4 * V_48 )
{
F_62 ( & V_115 ) ;
F_64 ( V_6 [ V_11 ] [ V_48 -> V_48 ] ,
& V_119 ) ;
F_68 ( & V_115 ) ;
F_58 () ;
F_54 ( V_11 , V_48 -> V_48 ) ;
}
int F_70 ( const struct V_1 * V_11 )
{
int V_121 ;
V_121 = F_71 ( V_11 -> V_11 ) ;
if ( V_121 < 0 )
return V_121 ;
F_62 ( & V_115 ) ;
F_64 ( V_6 [ V_11 -> V_11 ] [ V_122 ] ,
& V_123 ) ;
F_64 ( V_6 [ V_11 -> V_11 ] [ V_124 ] ,
& V_125 ) ;
F_68 ( & V_115 ) ;
F_64 ( V_3 [ V_11 -> V_11 ] , V_11 ) ;
return 0 ;
}
void F_72 ( const struct V_1 * V_11 )
{
F_62 ( & V_115 ) ;
F_64 ( V_3 [ V_11 -> V_11 ] , NULL ) ;
F_68 ( & V_115 ) ;
F_58 () ;
F_60 ( V_11 -> V_11 ) ;
F_73 ( V_11 -> V_11 ) ;
}
static void F_74 ( struct V_12 * V_13 )
{
struct V_63 * V_64 = F_75 ( V_13 , V_93 ) ;
if ( V_64 == NULL || V_64 -> V_13 == NULL )
return;
F_41 ( V_64 -> V_13 -> V_96 & V_102 ) ;
F_46 ( & V_98 ) ;
F_53 ( & V_64 -> V_99 ) ;
F_48 ( & V_98 ) ;
}
static void F_76 ( void * V_126 , void * V_127 )
{
struct V_63 * V_128 = V_126 ;
struct V_63 * V_129 = V_127 ;
struct V_12 * V_13 = V_129 -> V_13 ;
if ( ! V_13 || ! ( V_13 -> V_96 & V_102 ) )
return;
F_46 ( & V_98 ) ;
F_77 ( & V_129 -> V_99 , & V_128 -> V_99 ) ;
F_48 ( & V_98 ) ;
}
static int F_78 ( struct V_130 * V_131 ,
const struct V_12 * V_13 ,
struct V_49 * V_50 )
{
struct V_130 * V_132 [ V_133 + 1 ] ;
const struct V_4 * V_48 ;
int V_121 ;
V_121 = F_79 ( V_132 , V_133 , V_131 , V_134 ) ;
if ( V_121 < 0 )
return V_121 ;
V_48 = F_3 ( F_32 ( V_13 ) , F_52 ( V_13 ) ) ;
if ( V_48 -> V_135 )
V_121 = V_48 -> V_135 ( V_132 , V_50 ) ;
return V_121 ;
}
static int
F_80 ( const struct V_130 * V_64 ,
const struct V_12 * V_13 , struct V_49 * V_50 )
{
const struct V_1 * V_11 ;
struct V_130 * V_132 [ V_136 + 1 ] ;
int V_121 ;
memset ( V_50 , 0 , sizeof( * V_50 ) ) ;
V_121 = F_79 ( V_132 , V_136 , V_64 , V_137 ) ;
if ( V_121 < 0 )
return V_121 ;
F_6 () ;
V_11 = F_1 ( F_32 ( V_13 ) ) ;
if ( V_11 == NULL ) {
V_121 = - V_138 ;
goto V_24;
}
V_121 = V_11 -> V_135 ( V_132 , V_50 ) ;
if ( V_121 < 0 )
goto V_24;
if ( ! V_132 [ V_139 ] )
goto V_24;
V_121 = F_78 ( V_132 [ V_139 ] , V_13 , V_50 ) ;
V_24:
F_8 () ;
return V_121 ;
}
static int
F_81 ( struct V_12 * V_13 ,
enum V_68 V_140 ,
const struct V_130 * V_131 )
{
struct V_49 V_50 ;
int V_121 ;
V_121 = F_80 ( V_131 , V_13 , & V_50 ) ;
if ( V_121 < 0 )
return V_121 ;
if ( F_43 ( V_13 , V_140 ) )
return - V_141 ;
return F_38 ( V_13 , & V_50 , V_140 ) ;
}
static int
F_81 ( struct V_12 * V_13 ,
enum V_68 V_140 ,
const struct V_130 * V_131 )
{
return - V_142 ;
}
static int T_3 F_82 ( struct V_38 * V_38 )
{
V_38 -> V_13 . V_45 = V_38 -> V_13 . V_143 ;
V_38 -> V_13 . V_100 = F_83 ( & V_38 -> V_13 . V_45 , 0 ) ;
if ( ! V_38 -> V_13 . V_100 )
return - V_118 ;
return 0 ;
}
static void T_4 F_84 ( struct V_38 * V_38 )
{
struct F_51 V_110 = {} ;
F_57 ( V_38 , & F_51 , & V_110 ) ;
F_58 () ;
F_85 ( V_38 -> V_13 . V_100 , V_38 -> V_13 . V_45 ) ;
}
static int T_5 F_86 ( void )
{
int V_114 ;
V_114 = F_87 ( & V_144 ) ;
if ( V_114 < 0 ) {
F_88 ( V_145 L_3 ) ;
return V_114 ;
}
V_114 = F_89 ( & V_146 ) ;
if ( V_114 < 0 )
goto V_147;
F_90 ( & V_148 ) ;
F_91 ( V_111 ) ;
F_42 ( V_149 != NULL ) ;
F_64 ( V_149 , V_150 ) ;
F_42 ( V_151 != NULL ) ;
F_64 ( V_151 ,
F_81 ) ;
F_42 ( V_152 != NULL ) ;
F_64 ( V_152 , V_153 ) ;
F_64 ( V_154 , & V_155 ) ;
#ifdef F_92
F_42 ( V_156 != NULL ) ;
F_64 ( V_156 , F_4 ) ;
#endif
return 0 ;
V_147:
F_93 ( & V_144 ) ;
return V_114 ;
}
static void T_6 F_94 ( void )
{
unsigned int V_72 ;
F_95 ( & V_146 ) ;
F_93 ( & V_144 ) ;
F_96 ( & V_148 ) ;
F_64 ( V_149 , NULL ) ;
F_64 ( V_151 , NULL ) ;
F_64 ( V_152 , NULL ) ;
F_64 ( V_154 , NULL ) ;
#ifdef F_92
F_64 ( V_156 , NULL ) ;
#endif
for ( V_72 = 0 ; V_72 < V_157 ; V_72 ++ )
F_97 ( V_6 [ V_72 ] ) ;
F_98 () ;
}
