static unsigned long F_1 ( struct V_1 * V_2 )
{
#if F_2 ( V_3 ) || F_2 ( V_4 )
if ( V_2 -> V_5 -> V_6 . V_7 )
return F_3 () ;
else
#endif
return F_4 ( V_8 ) ;
}
static void F_5 ( unsigned long V_9 , unsigned long V_10 )
{
int V_11 ;
for ( V_11 = 0 ; V_11 < ( V_10 + 32 ) ; V_11 += 32 )
F_6 ( ( void * ) ( V_9 + V_11 ) ) ;
F_7 () ;
F_8 () ;
}
static void F_9 ( void )
{
unsigned long V_12 , V_13 , V_14 , V_15 , V_16 , V_17 , V_18 ;
long V_19 ;
V_18 = V_20 -> V_21 ;
V_14 = V_20 -> V_22 [ 0 ] ;
V_15 = V_20 -> V_22 [ 1 ] ;
V_16 = V_20 -> V_23 [ 0 ] ;
V_17 = V_20 -> V_23 [ 1 ] ;
F_10 ( V_19 ) ;
for ( V_12 = 0 ; V_12 < V_14 ; ++ V_12 ) {
for ( V_13 = 0 ; V_13 < V_15 ; ++ V_13 ) {
F_11 ( V_18 ) ;
V_18 += V_17 ;
}
V_18 += V_16 ;
}
F_12 ( V_19 ) ;
F_8 () ;
}
long F_13 ( T_1 * V_24 , T_1 * V_25 , T_1 * V_26 )
{
struct V_27 V_28 ;
F_14 ( V_28 , V_29 , ( T_1 ) V_24 , ( T_1 ) V_25 ,
( T_1 ) V_26 ) ;
return V_28 . V_30 ;
}
int F_15 ( void * V_31 )
{
long V_30 ;
long V_32 ;
unsigned long V_33 ;
unsigned long V_34 ;
int V_35 ;
V_33 = F_16 ( F_17 ( F_18 ( V_36 ) , V_37 ) ) ;
F_10 ( V_34 ) ;
V_35 = F_19 ( 0x3 , V_38 , V_33 , V_39 ) ;
F_12 ( V_34 ) ;
if ( V_35 < 0 )
return - V_40 ;
F_20 ( & V_41 ) ;
V_30 = F_21 ( V_42 ?
V_43 : V_44 ,
F_18 ( V_45 ) , V_46 , & V_32 ) ;
if ( V_30 != 0 ) {
F_22 ( & V_41 ) ;
F_23 ( V_47 L_1 ) ;
return - V_40 ;
}
if ( ! V_42 ) {
V_42 = V_32 ;
F_23 ( V_48 L_2 , V_42 ) ;
}
F_22 ( & V_41 ) ;
F_24 ( 0x3 , V_35 ) ;
return 0 ;
}
void F_25 ( void * V_31 )
{
long V_30 ;
int V_35 ;
unsigned long V_33 ;
unsigned long V_34 ;
unsigned long V_25 = F_4 ( V_49 ) ;
V_33 = F_16 ( F_17 ( F_18 ( V_36 ) ,
V_37 ) ) ;
F_10 ( V_34 ) ;
V_35 = F_19 ( 0x3 , V_38 , V_33 , V_39 ) ;
F_12 ( V_34 ) ;
if ( V_35 < 0 )
return;
V_30 = F_26 ( V_25 ) ;
if ( V_30 )
F_23 ( V_50 L_3 ,
V_30 ) ;
F_24 ( 0x3 , V_35 ) ;
}
void F_27 ( void * V_51 )
{
* ( int * ) V_51 = 0 ;
}
int F_28 ( long V_52 )
{
int V_53 ;
switch ( V_52 ) {
case V_54 :
case V_55 :
case V_56 :
V_53 = 1 ;
break;
case V_57 :
V_53 = V_58 ;
break;
case V_59 :
V_53 = F_29 ( & V_60 ) ;
break;
default:
V_53 = 0 ;
}
return V_53 ;
}
static int F_30 ( struct V_1 * V_2 , struct V_61 * V_61 )
{
V_61 -> V_62 = V_63 ;
V_61 -> V_64 . V_65 = 1 ;
return 0 ;
}
static int F_31 ( struct V_1 * V_2 , struct V_61 * V_61 )
{
struct V_66 * V_67 ;
struct V_68 * V_69 ;
int V_53 ;
V_67 = F_32 ( V_2 ) ;
if ( ( V_67 -> V_18 & V_70 ) == V_71 )
goto V_72;
V_2 -> V_73 = 1 ;
V_2 -> V_74 [ 0 ] . V_75 = V_61 -> V_72 . V_76 = V_67 -> V_18 ;
V_2 -> V_74 [ 0 ] . V_10 = V_61 -> V_72 . V_10 = V_67 -> V_77 ;
V_2 -> V_78 = V_61 -> V_72 . V_79 = ! V_67 -> V_80 ;
if ( V_2 -> V_78 )
memcpy ( V_2 -> V_6 . V_81 , & V_67 -> V_82 , V_67 -> V_77 ) ;
memcpy ( V_61 -> V_72 . V_82 , & V_67 -> V_82 , V_67 -> V_77 ) ;
V_61 -> V_62 = V_83 ;
return 0 ;
V_72:
if ( V_67 -> V_80 )
V_53 = F_33 ( V_2 -> V_5 , V_84 , V_67 -> V_18 ,
V_67 -> V_77 , & V_67 -> V_82 ) ;
else
V_53 = F_34 ( V_2 -> V_5 , V_84 , V_67 -> V_18 ,
V_67 -> V_77 , & V_67 -> V_82 ) ;
if ( V_53 )
F_23 ( V_85 L_4 , V_67 -> V_18 ) ;
V_67 -> V_86 = V_87 ;
return 1 ;
}
static int F_35 ( struct V_1 * V_2 , struct V_61 * V_61 )
{
struct V_88 * V_67 ;
V_67 = F_36 ( V_2 ) ;
if ( V_67 -> V_62 == V_89 )
return F_37 ( V_2 , V_61 ) ;
else {
V_61 -> V_62 = V_63 ;
V_61 -> V_64 . V_65 = 2 ;
return 0 ;
}
}
static int F_38 ( struct V_1 * V_2 , struct V_61 * V_61 )
{
struct V_88 * V_67 ;
V_67 = F_36 ( V_2 ) ;
if ( V_67 -> V_62 == V_90 ) {
F_39 ( V_2 ) ;
return 1 ;
} else {
V_61 -> V_62 = V_63 ;
V_61 -> V_64 . V_65 = 3 ;
return 0 ;
}
}
static int F_40 ( struct V_1 * V_2 , T_2 V_91 )
{
struct V_24 * V_24 = F_41 ( V_2 -> V_5 , V_2 -> V_6 . V_24 ) ;
if ( ! F_42 ( V_91 , & V_24 -> V_92 [ 0 ] ) ) {
V_2 -> V_6 . V_93 = 1 ;
F_43 ( V_2 ) ;
return 1 ;
}
return 0 ;
}
static void F_44 ( struct V_1 * V_2 , T_2 V_94 ,
T_2 V_91 )
{
switch ( V_94 ) {
case V_95 :
break;
case V_96 :
V_91 = 2 ;
break;
case V_97 :
V_91 = 0 ;
break;
case V_98 :
case V_99 :
default:
F_23 ( V_85 L_5 ) ;
return;
}
F_40 ( V_2 , V_91 ) ;
}
static struct V_1 * F_45 ( struct V_5 * V_5 , unsigned long V_100 ,
unsigned long V_101 )
{
union V_102 V_103 ;
int V_12 ;
struct V_1 * V_2 ;
F_46 (i, vcpu, kvm) {
V_103 . V_104 = F_47 ( V_2 ) ;
if ( V_103 . V_100 == V_100 && V_103 . V_101 == V_101 )
return V_2 ;
}
return NULL ;
}
static int F_48 ( struct V_1 * V_2 , struct V_61 * V_61 )
{
struct V_88 * V_67 = F_36 ( V_2 ) ;
struct V_1 * V_105 ;
struct V_106 * V_107 ;
union V_108 V_18 = V_67 -> V_109 . V_110 . V_18 ;
union V_111 V_82 = V_67 -> V_109 . V_110 . V_82 ;
V_105 = F_45 ( V_2 -> V_5 , V_18 . V_100 , V_18 . V_101 ) ;
if ( ! V_105 )
return F_30 ( V_2 , V_61 ) ;
if ( ! V_105 -> V_6 . V_112 ) {
V_107 = F_49 ( V_105 ) ;
V_107 -> V_113 = V_2 -> V_5 -> V_6 . V_114 . V_115 ;
V_107 -> V_116 = V_2 -> V_5 -> V_6 . V_114 . V_117 ;
V_105 -> V_6 . V_118 = V_119 ;
if ( F_50 ( & V_105 -> V_120 ) )
F_51 ( & V_105 -> V_120 ) ;
} else {
F_44 ( V_105 , V_82 . V_94 , V_82 . V_91 ) ;
if ( V_105 != V_2 )
F_43 ( V_105 ) ;
}
return 1 ;
}
static void F_52 ( void * V_121 )
{
struct V_122 * V_67 = (struct V_122 * ) V_121 ;
struct V_1 * V_2 = V_67 -> V_2 ;
if ( F_53 ( V_123 , & V_2 -> V_124 ) )
return;
F_54 ( V_125 , & V_2 -> V_124 ) ;
if ( V_2 -> V_6 . V_126 < V_127 ) {
V_2 -> V_6 . V_128 [ V_2 -> V_6 . V_126 ++ ] =
V_67 -> V_128 ;
} else {
F_55 ( V_125 , & V_2 -> V_124 ) ;
V_2 -> V_6 . V_126 = 0 ;
F_54 ( V_123 , & V_2 -> V_124 ) ;
}
}
static int F_56 ( struct V_1 * V_2 , struct V_61 * V_61 )
{
struct V_88 * V_67 = F_36 ( V_2 ) ;
struct V_5 * V_5 = V_2 -> V_5 ;
struct V_122 V_122 ;
int V_12 ;
struct V_1 * V_129 ;
V_122 . V_128 = V_67 -> V_109 . V_128 ;
F_46 (i, vcpui, kvm) {
if ( V_129 -> V_6 . V_118 == V_130 ||
V_2 == V_129 )
continue;
if ( F_50 ( & V_129 -> V_120 ) )
F_51 ( & V_129 -> V_120 ) ;
if ( V_129 -> V_131 != - 1 ) {
V_122 . V_2 = V_129 ;
F_57 ( V_129 -> V_131 ,
F_52 , & V_122 , 1 ) ;
} else
F_23 ( V_47 L_6 ) ;
}
return 1 ;
}
static int F_58 ( struct V_1 * V_2 , struct V_61 * V_61 )
{
return 1 ;
}
static int F_59 ( struct V_1 * V_2 )
{
unsigned long V_33 , V_132 , V_133 ;
int V_35 ;
V_133 = V_38 + ( 1UL << V_39 ) ;
V_132 = V_134 | V_135 ;
V_33 = F_16 ( F_17 ( V_132 , V_136 ) ) ;
V_35 = F_19 ( 0x3 , V_133 , V_33 , V_137 ) ;
V_2 -> V_6 . V_138 = V_35 ;
if ( V_35 < 0 ) {
F_23 ( V_85 L_7 ) ;
V_35 = 0 ;
}
return V_35 ;
}
int F_60 ( struct V_1 * V_2 )
{
T_3 V_139 ;
long V_140 ;
unsigned long V_141 ;
unsigned long V_142 ;
struct V_143 * V_144 = & V_2 -> V_6 . V_145 ;
unsigned long V_146 = V_20 -> V_146 ;
struct V_24 * V_24 = F_41 ( V_2 -> V_5 , V_2 -> V_6 . V_24 ) ;
if ( F_61 ( V_2 -> V_5 ) ) {
V_141 = F_1 ( V_2 ) + V_2 -> V_6 . V_147 ;
if ( F_62 ( V_141 , V_24 -> V_148 ) ) {
V_2 -> V_6 . V_149 = 1 ;
return 1 ;
}
V_140 = V_24 -> V_148 - V_141 ;
if ( V_140 < 0 )
V_140 = - V_140 ;
V_142 = F_63 ( V_140 , V_146 ) ;
V_139 = F_64 ( 0 , 1000 * V_142 ) ;
V_2 -> V_6 . V_150 = 1 ;
F_65 ( V_144 , V_139 , V_151 ) ;
V_2 -> V_6 . V_118 = V_152 ;
F_66 ( V_2 ) ;
F_67 ( V_144 ) ;
V_2 -> V_6 . V_150 = 0 ;
if ( F_68 ( V_153 , & V_2 -> V_124 ) ||
F_69 ( V_2 ) )
if ( V_2 -> V_6 . V_118 == V_152 )
V_2 -> V_6 . V_118 = V_119 ;
if ( V_2 -> V_6 . V_118 != V_119 )
return - V_154 ;
return 1 ;
} else {
F_23 ( V_85 L_8 ) ;
return 0 ;
}
}
static int F_70 ( struct V_1 * V_2 ,
struct V_61 * V_61 )
{
V_61 -> V_62 = V_155 ;
return 0 ;
}
static int F_71 ( struct V_1 * V_2 ,
struct V_61 * V_61 )
{
return 1 ;
}
static int F_72 ( struct V_1 * V_2 ,
struct V_61 * V_61 )
{
F_23 ( L_9 , V_2 -> V_6 . V_156 ) ;
return 1 ;
}
static T_4 F_73 ( struct V_1 * V_2 )
{
struct V_88 * V_157 ;
V_157 = F_36 ( V_2 ) ;
return V_157 -> V_62 ;
}
static int F_74 ( struct V_61 * V_61 , struct V_1 * V_2 )
{
T_5 V_62 = F_73 ( V_2 ) ;
V_2 -> V_6 . V_158 = V_62 ;
if ( V_62 < V_159
&& V_160 [ V_62 ] )
return V_160 [ V_62 ] ( V_2 , V_61 ) ;
else {
V_61 -> V_62 = V_63 ;
V_61 -> V_64 . V_65 = V_62 ;
}
return 0 ;
}
static inline void F_75 ( unsigned long V_161 )
{
F_76 ( V_162 , V_161 ) ;
F_8 () ;
}
static int F_77 ( struct V_1 * V_2 )
{
unsigned long V_33 ;
struct V_5 * V_5 = V_2 -> V_5 ;
int V_53 ;
V_33 = F_16 ( F_17 ( F_18 ( V_36 ) , V_37 ) ) ;
V_53 = F_19 ( 0x3 , V_38 , V_33 , V_39 ) ;
if ( V_53 < 0 )
goto V_163;
V_2 -> V_6 . V_164 = V_53 ;
V_33 = F_16 ( F_17 ( F_18 ( V_5 -> V_6 . V_165 ) , V_37 ) ) ;
V_53 = F_19 ( 0x3 , V_166 ,
V_33 , V_167 ) ;
if ( V_53 < 0 )
goto V_163;
V_2 -> V_6 . V_168 = V_53 ;
#if F_2 ( V_3 ) || F_2 ( V_4 )
if ( V_5 -> V_6 . V_7 ) {
V_53 = F_59 ( V_2 ) ;
if ( V_53 < 0 )
goto V_163;
}
#endif
V_53 = 0 ;
V_163:
return V_53 ;
}
static void F_78 ( struct V_1 * V_2 )
{
struct V_5 * V_5 = V_2 -> V_5 ;
F_24 ( 0x3 , V_2 -> V_6 . V_164 ) ;
F_24 ( 0x3 , V_2 -> V_6 . V_168 ) ;
#if F_2 ( V_3 ) || F_2 ( V_4 )
if ( V_5 -> V_6 . V_7 )
F_24 ( 0x3 , V_2 -> V_6 . V_138 ) ;
#endif
}
static int F_79 ( struct V_1 * V_2 )
{
unsigned long V_169 ;
int V_53 ;
int V_131 = F_80 () ;
if ( V_2 -> V_6 . V_170 != V_131 ||
F_81 ( V_171 , V_131 ) != V_2 ) {
F_81 ( V_171 , V_131 ) = V_2 ;
V_2 -> V_6 . V_170 = V_131 ;
F_9 () ;
}
V_2 -> V_6 . V_172 = F_82 ( V_162 ) ;
F_75 ( V_2 -> V_6 . V_173 ) ;
F_10 ( V_169 ) ;
V_53 = F_77 ( V_2 ) ;
F_12 ( V_169 ) ;
return V_53 ;
}
static void F_83 ( struct V_1 * V_2 )
{
F_78 ( V_2 ) ;
F_75 ( V_2 -> V_6 . V_172 ) ;
}
static int F_84 ( struct V_1 * V_2 , struct V_61 * V_61 )
{
union V_174 * V_175 , * V_176 ;
int V_53 , V_177 ;
V_177 = F_85 ( & V_2 -> V_5 -> V_178 ) ;
V_179:
if ( F_86 ( V_180 ) ) {
V_53 = - V_154 ;
V_61 -> V_62 = V_181 ;
goto V_163;
}
F_87 () ;
F_88 () ;
V_175 = F_89 ( V_2 ) ;
V_176 = F_90 ( V_2 ) ;
F_55 ( V_182 , & V_2 -> V_124 ) ;
V_53 = F_79 ( V_2 ) ;
if ( V_53 < 0 )
goto V_183;
F_91 ( & V_2 -> V_5 -> V_178 , V_177 ) ;
V_2 -> V_184 = V_185 ;
F_92 () ;
V_186 -> V_187 ( V_175 , V_176 ) ;
F_83 ( V_2 ) ;
V_2 -> V_6 . V_112 = 1 ;
F_54 ( V_182 , & V_2 -> V_124 ) ;
F_93 () ;
F_94 () ;
F_95 () ;
V_2 -> V_184 = V_188 ;
F_96 () ;
V_177 = F_85 ( & V_2 -> V_5 -> V_178 ) ;
V_53 = F_74 ( V_61 , V_2 ) ;
if ( V_53 > 0 ) {
if ( ! F_97 () )
goto V_179;
}
V_163:
F_91 ( & V_2 -> V_5 -> V_178 , V_177 ) ;
if ( V_53 > 0 ) {
F_98 ( V_2 ) ;
V_177 = F_85 ( & V_2 -> V_5 -> V_178 ) ;
goto V_179;
}
return V_53 ;
V_183:
F_93 () ;
F_96 () ;
V_61 -> V_62 = V_189 ;
goto V_163;
}
static void F_99 ( struct V_1 * V_2 )
{
struct V_66 * V_67 = F_32 ( V_2 ) ;
if ( ! V_2 -> V_78 )
memcpy ( & V_67 -> V_82 , V_2 -> V_6 . V_81 , 8 ) ;
V_67 -> V_86 = V_87 ;
}
int F_100 ( struct V_1 * V_2 , struct V_61 * V_61 )
{
int V_53 ;
T_6 V_190 ;
if ( V_2 -> V_191 )
F_101 ( V_192 , & V_2 -> V_193 , & V_190 ) ;
if ( F_102 ( V_2 -> V_6 . V_118 == V_130 ) ) {
F_66 ( V_2 ) ;
F_55 ( V_153 , & V_2 -> V_124 ) ;
V_53 = - V_194 ;
goto V_163;
}
if ( V_2 -> V_73 ) {
memcpy ( V_2 -> V_6 . V_81 , V_61 -> V_72 . V_82 , 8 ) ;
F_99 ( V_2 ) ;
V_2 -> V_195 = 1 ;
V_2 -> V_73 = 0 ;
}
V_53 = F_84 ( V_2 , V_61 ) ;
V_163:
if ( V_2 -> V_191 )
F_101 ( V_192 , & V_190 , NULL ) ;
return V_53 ;
}
struct V_5 * F_103 ( void )
{
struct V_5 * V_5 ;
T_2 V_165 ;
F_104 ( sizeof( struct V_5 ) > V_196 ) ;
V_165 = F_105 ( V_197 , F_106 ( V_198 ) ) ;
if ( ! V_165 )
return NULL ;
memset ( ( void * ) V_165 , 0 , V_198 ) ;
V_5 = (struct V_5 * ) ( V_165 +
F_107 ( struct V_199 , V_200 ) ) ;
V_5 -> V_6 . V_165 = V_165 ;
F_23 ( V_50 L_10 , V_165 ) ;
return V_5 ;
}
static void F_108 ( struct V_5 * V_5 )
{
unsigned long V_12 , V_13 ;
for ( V_12 = 0 ; V_12 < ( sizeof( V_201 ) / sizeof( struct V_202 ) ) ;
V_12 ++ ) {
for ( V_13 = V_201 [ V_12 ] . V_9 ;
V_13 < V_201 [ V_12 ] . V_9 + V_201 [ V_12 ] . V_77 ;
V_13 += V_203 )
F_109 ( V_5 , V_13 >> V_137 ,
V_201 [ V_12 ] . type , 0 ) ;
}
}
int F_110 ( struct V_5 * V_5 , unsigned long type )
{
F_104 ( ! V_5 ) ;
if ( type )
return - V_40 ;
V_5 -> V_6 . V_7 = F_111 ( L_11 ) ;
V_5 -> V_6 . V_204 = V_205 ;
V_5 -> V_6 . V_206 = V_207 ;
V_5 -> V_6 . V_208 = V_209 ;
F_108 ( V_5 ) ;
F_112 ( & V_5 -> V_6 . V_210 ) ;
F_54 ( V_211 , & V_5 -> V_6 . V_212 ) ;
return 0 ;
}
static int F_113 ( struct V_5 * V_5 ,
struct V_213 * V_214 )
{
int V_53 ;
V_53 = 0 ;
switch ( V_214 -> V_215 ) {
case V_216 :
V_53 = F_114 ( V_5 , & V_214 -> V_214 . V_217 ) ;
break;
default:
V_53 = - V_40 ;
break;
}
return V_53 ;
}
static int F_115 ( struct V_5 * V_5 , struct V_213 * V_214 )
{
int V_53 ;
V_53 = 0 ;
switch ( V_214 -> V_215 ) {
case V_216 :
V_53 = F_116 ( V_5 , & V_214 -> V_214 . V_217 ) ;
break;
default:
V_53 = - V_40 ;
break;
}
return V_53 ;
}
int F_117 ( struct V_1 * V_2 , struct V_218 * V_107 )
{
struct V_24 * V_24 = F_41 ( V_2 -> V_5 , V_2 -> V_6 . V_24 ) ;
int V_12 ;
for ( V_12 = 0 ; V_12 < 16 ; V_12 ++ ) {
V_24 -> V_219 [ V_12 ] = V_107 -> V_24 . V_219 [ V_12 ] ;
V_24 -> V_220 [ V_12 ] = V_107 -> V_24 . V_220 [ V_12 ] ;
}
for ( V_12 = 0 ; V_12 < 128 ; V_12 ++ )
V_24 -> V_221 [ V_12 ] = V_107 -> V_24 . V_221 [ V_12 ] ;
V_24 -> V_222 = V_107 -> V_24 . V_222 ;
V_24 -> V_223 = V_107 -> V_24 . V_223 ;
V_24 -> V_224 = V_107 -> V_24 . V_224 ;
V_24 -> V_225 = V_107 -> V_24 . V_225 ;
V_24 -> V_226 = V_107 -> V_24 . V_226 ;
memcpy ( & V_2 -> V_6 . V_227 , & V_107 -> V_228 , sizeof( union V_174 ) ) ;
F_118 ( V_118 ) ;
F_118 ( V_173 ) ;
memcpy ( V_2 -> V_6 . V_229 , V_107 -> V_229 , sizeof( struct V_230 ) * V_231 ) ;
memcpy ( V_2 -> V_6 . V_232 , V_107 -> V_232 , sizeof( struct V_230 ) * V_233 ) ;
F_118 ( V_234 ) ;
F_118 ( V_235 ) ;
F_118 ( V_236 ) ;
F_118 ( V_237 ) ;
F_118 ( V_238 ) ;
F_118 ( V_149 ) ;
F_118 ( V_239 ) ;
F_118 ( V_240 ) ;
for ( V_12 = 0 ; V_12 < 8 ; V_12 ++ ) {
V_2 -> V_6 . V_241 [ V_12 ] = V_107 -> V_241 [ V_12 ] ;
V_2 -> V_6 . V_242 [ V_12 ] = V_107 -> V_242 [ V_12 ] ;
V_2 -> V_6 . V_243 [ V_12 ] = V_107 -> V_243 [ V_12 ] ;
}
for ( V_12 = 0 ; V_12 < 4 ; V_12 ++ )
V_2 -> V_6 . V_244 [ V_12 ] = V_107 -> V_244 [ V_12 ] ;
F_118 ( V_245 ) ;
F_118 ( V_204 ) ;
F_118 ( V_206 ) ;
F_118 ( V_246 ) ;
F_118 ( V_247 ) ;
F_118 ( V_248 ) ;
F_118 ( V_249 ) ;
V_2 -> V_6 . V_93 = 1 ;
V_2 -> V_6 . V_147 = V_107 -> V_250 - F_1 ( V_2 ) ;
F_54 ( V_251 , & V_2 -> V_124 ) ;
return 0 ;
}
int F_119 ( struct V_5 * V_5 , struct V_252 * V_253 )
{
if ( ! F_61 ( V_5 ) )
return - V_254 ;
V_253 -> V_30 = F_120 ( V_5 , V_211 ,
V_253 -> V_255 , V_253 -> V_256 ) ;
return 0 ;
}
long F_121 ( struct V_257 * V_258 ,
unsigned int V_259 , unsigned long V_260 )
{
struct V_5 * V_5 = V_258 -> V_261 ;
void T_7 * V_262 = ( void T_7 * ) V_260 ;
int V_53 = - V_263 ;
switch ( V_259 ) {
case V_264 : {
struct V_265 V_266 ;
struct V_267 V_268 ;
V_53 = - V_269 ;
if ( F_122 ( & V_266 , V_262 , sizeof V_266 ) )
goto V_163;
V_268 . V_35 = V_266 . V_35 ;
V_268 . V_19 = V_266 . V_19 ;
V_268 . V_270 =
V_266 . V_270 ;
V_268 . V_271 = V_266 . V_271 ;
V_53 = F_123 ( V_5 ,
& V_268 , 0 ) ;
if ( V_53 )
goto V_163;
break;
}
case V_272 :
V_53 = - V_269 ;
V_53 = F_124 ( V_5 ) ;
if ( V_53 )
goto V_163;
V_53 = F_125 ( V_5 ) ;
if ( V_53 ) {
F_126 ( & V_5 -> V_273 ) ;
F_127 ( V_5 ) ;
F_128 ( & V_5 -> V_273 ) ;
goto V_163;
}
break;
case V_274 : {
struct V_213 V_214 ;
V_53 = - V_269 ;
if ( F_122 ( & V_214 , V_262 , sizeof V_214 ) )
goto V_163;
V_53 = - V_254 ;
if ( ! F_61 ( V_5 ) )
goto V_163;
V_53 = F_113 ( V_5 , & V_214 ) ;
if ( V_53 )
goto V_163;
V_53 = - V_269 ;
if ( F_129 ( V_262 , & V_214 , sizeof V_214 ) )
goto V_163;
V_53 = 0 ;
break;
}
case V_275 : {
struct V_213 V_214 ;
V_53 = - V_269 ;
if ( F_122 ( & V_214 , V_262 , sizeof V_214 ) )
goto V_163;
V_53 = - V_254 ;
if ( ! F_61 ( V_5 ) )
goto V_163;
V_53 = F_115 ( V_5 , & V_214 ) ;
if ( V_53 )
goto V_163;
V_53 = 0 ;
break;
}
default:
;
}
V_163:
return V_53 ;
}
int F_130 ( struct V_1 * V_2 ,
struct V_276 * V_277 )
{
return - V_40 ;
}
int F_131 ( struct V_1 * V_2 ,
struct V_276 * V_277 )
{
return - V_40 ;
}
int F_132 ( struct V_1 * V_2 ,
struct V_278 * V_279 )
{
return - V_40 ;
}
static int F_133 ( void )
{
if ( ! V_36 && ( V_280 < V_281 ) ) {
V_36 = F_105 ( V_197 ,
F_106 ( V_282 ) ) ;
if ( ! V_36 )
return - V_283 ;
memset ( ( void * ) V_36 , 0 , V_282 ) ;
V_45 = V_36 + V_284 ;
F_23 ( V_50 L_12 ,
V_36 , V_45 ) ;
}
return 0 ;
}
static void F_134 ( void )
{
if ( V_36 ) {
memset ( ( void * ) V_36 , 0 , V_282 ) ;
F_135 ( V_36 , F_106 ( V_282 ) ) ;
V_36 = 0 ;
V_45 = 0 ;
V_42 = 0 ;
}
}
static int F_136 ( struct V_1 * V_2 )
{
int V_12 ;
union V_285 V_286 ;
struct V_24 * V_24 = F_41 ( V_2 -> V_5 , V_2 -> V_6 . V_24 ) ;
if ( F_137 ( V_24 ) )
return F_138 ( V_24 ) ;
for ( V_12 = 0 ; V_12 < 5 ; V_12 ++ )
V_24 -> V_287 [ V_12 ] = F_139 ( V_12 ) ;
V_286 . V_288 = V_24 -> V_287 [ 3 ] ;
V_286 . V_289 = 4 ;
V_24 -> V_287 [ 3 ] = V_286 . V_288 ;
V_24 -> V_290 . V_291 = 1 ;
V_24 -> V_290 . V_292 = 1 ;
V_24 -> V_290 . V_293 = 1 ;
V_24 -> V_290 . V_294 = 1 ;
V_24 -> V_290 . V_295 = 1 ;
V_24 -> V_290 . V_296 = 1 ;
V_24 -> V_290 . V_297 = 1 ;
V_24 -> V_298 . V_299 = 1 ;
V_24 -> V_300 = V_46 ;
return 0 ;
}
static int F_140 ( struct V_1 * V_2 )
{
long V_301 ;
struct V_24 * V_24 = V_2 -> V_6 . V_24 ;
unsigned long V_302 ;
V_302 = V_186 -> V_302 ;
F_23 ( V_50 L_13 , V_2 , V_302 ) ;
V_301 = F_13 ( ( T_1 * ) V_24 , ( T_1 * ) V_302 , 0 ) ;
if ( V_301 ) {
F_23 ( V_85 L_14 ) ;
return - V_40 ;
}
return 0 ;
}
static void F_141 ( struct V_1 * V_2 )
{
T_8 V_303 = { 0 } ;
F_142 ( & V_303 ) ;
V_2 -> V_6 . V_21 = V_303 . V_304 ;
V_2 -> V_6 . V_22 [ 0 ] = V_303 . V_305 [ 0 ] ;
V_2 -> V_6 . V_22 [ 1 ] = V_303 . V_305 [ 1 ] ;
V_2 -> V_6 . V_23 [ 0 ] = V_303 . V_306 [ 0 ] ;
V_2 -> V_6 . V_23 [ 1 ] = V_303 . V_306 [ 1 ] ;
}
static void F_143 ( struct V_1 * V_2 )
{
struct V_143 * V_144 = & V_2 -> V_6 . V_145 ;
if ( F_67 ( V_144 ) )
F_144 ( V_144 , V_151 ) ;
}
static enum V_307 F_145 ( struct V_143 * V_82 )
{
struct V_1 * V_2 ;
T_9 * V_308 ;
V_2 = F_146 ( V_82 , struct V_1 , V_6 . V_145 ) ;
V_308 = & V_2 -> V_120 ;
if ( V_2 -> V_6 . V_118 != V_152 )
goto V_163;
if ( F_50 ( V_308 ) )
F_51 ( V_308 ) ;
V_163:
V_2 -> V_6 . V_309 = 1 ;
V_2 -> V_6 . V_149 = 1 ;
return V_310 ;
}
bool F_147 ( struct V_1 * V_2 )
{
return F_61 ( V_2 -> V_5 ) == ( V_2 -> V_6 . V_311 != NULL ) ;
}
int F_148 ( struct V_1 * V_2 )
{
struct V_1 * V_312 ;
int V_53 ;
int V_12 ;
long V_147 ;
struct V_5 * V_5 = V_2 -> V_5 ;
struct V_106 * V_107 = F_49 ( V_2 ) ;
union V_174 * V_313 = & V_2 -> V_6 . V_227 ;
struct V_1 * V_314 = F_149 ( V_2 -> V_5 , V_2 ) ;
if ( F_137 ( V_314 ) )
return F_138 ( V_314 ) ;
if ( F_150 ( V_2 ) ) {
V_2 -> V_6 . V_118 = V_119 ;
V_107 -> V_113 = V_315 ;
V_147 = 0UL - F_1 ( V_2 ) ;
for ( V_12 = 0 ; V_12 < V_316 ; V_12 ++ ) {
V_312 = (struct V_1 * ) ( ( char * ) V_2 +
sizeof( struct V_317 ) * V_12 ) ;
V_312 -> V_6 . V_147 = V_147 ;
V_312 -> V_6 . V_240 = 0 ;
}
} else
V_2 -> V_6 . V_118 = V_130 ;
V_53 = - V_283 ;
V_2 -> V_6 . V_311 = F_151 ( sizeof( struct V_318 ) , V_197 ) ;
if ( ! V_2 -> V_6 . V_311 )
goto V_163;
V_2 -> V_6 . V_311 -> V_2 = V_2 ;
V_313 -> V_319 [ 1 ] = 0 ;
V_313 -> V_319 [ 12 ] = ( unsigned long ) ( ( char * ) V_314 + V_320 ) ;
V_313 -> V_319 [ 13 ] = ( unsigned long ) V_314 ;
V_313 -> V_169 = 0x1008522000UL ;
V_313 -> V_321 [ 40 ] = V_322 ;
V_313 -> V_323 = 0 ;
V_313 -> V_324 = 0x0 ;
V_313 -> V_321 [ 36 ] = 0x0 ;
V_313 -> V_321 [ 19 ] = 0x0 ;
V_313 -> V_321 [ 18 ] = ( unsigned long ) V_314 +
( ( sizeof( struct V_1 ) + 15 ) & ~ 15 ) ;
V_313 -> V_321 [ 64 ] = 0x0 ;
V_313 -> V_325 [ 0 ] = 0x7e04UL ;
V_313 -> V_325 [ 2 ] = ( unsigned long ) V_186 -> V_302 ;
V_313 -> V_325 [ 8 ] = 0x3c ;
V_313 -> V_326 [ 0 ] = 0x30 ;
V_313 -> V_326 [ 1 ] = 0x30 ;
V_313 -> V_326 [ 2 ] = 0x30 ;
V_313 -> V_326 [ 3 ] = 0x30 ;
V_313 -> V_326 [ 4 ] = 0x30 ;
V_313 -> V_326 [ 5 ] = 0x30 ;
V_313 -> V_326 [ 7 ] = 0x30 ;
V_313 -> V_327 [ 0 ] = * ( unsigned long * ) V_186 -> V_328 ;
V_2 -> V_6 . V_173 = V_5 -> V_6 . V_208 ;
V_2 -> V_6 . V_204 = V_5 -> V_6 . V_204 ;
V_2 -> V_6 . V_206 = V_5 -> V_6 . V_206 ;
F_152 ( & V_2 -> V_6 . V_145 , V_329 , V_151 ) ;
V_2 -> V_6 . V_145 . V_330 = F_145 ;
V_2 -> V_6 . V_170 = - 1 ;
V_2 -> V_6 . V_24 = (struct V_24 * ) F_153 ( V_2 -> V_331 ) ;
V_2 -> V_6 . V_332 = V_42 ;
V_2 -> V_6 . V_333 = V_334 ;
V_2 -> V_6 . V_335 = F_18 ( & V_5 -> V_6 . V_336 ) ;
V_2 -> V_6 . V_337 . V_338 = (struct V_230 * ) F_154 ( V_2 -> V_331 ) ;
V_2 -> V_6 . V_339 . V_338 = (struct V_230 * ) F_155 ( V_2 -> V_331 ) ;
F_141 ( V_2 ) ;
V_53 = 0 ;
V_163:
return V_53 ;
}
static int F_156 ( struct V_1 * V_2 , int V_100 )
{
unsigned long V_169 ;
int V_53 ;
F_10 ( V_169 ) ;
V_53 = F_77 ( V_2 ) ;
F_12 ( V_169 ) ;
if ( V_53 )
goto V_340;
V_53 = F_157 ( V_2 , V_2 -> V_5 , V_100 ) ;
if ( V_53 )
goto V_340;
V_53 = F_136 ( V_2 ) ;
if ( V_53 ) {
F_23 ( V_50 L_15 ) ;
goto V_341;
}
V_53 = F_140 ( V_2 ) ;
if ( V_53 )
goto V_341;
F_78 ( V_2 ) ;
return 0 ;
V_341:
F_158 ( V_2 ) ;
V_340:
return V_53 ;
}
struct V_1 * F_159 ( struct V_5 * V_5 ,
unsigned int V_100 )
{
struct V_1 * V_2 ;
unsigned long V_165 = V_5 -> V_6 . V_165 ;
int V_53 ;
int V_131 ;
F_104 ( sizeof( struct V_1 ) > V_342 / 2 ) ;
V_53 = - V_40 ;
if ( V_100 >= V_316 ) {
F_23 ( V_85 L_16 ,
V_316 ) ;
goto V_340;
}
V_53 = - V_283 ;
if ( ! V_165 ) {
F_23 ( V_85 L_17 , V_100 ) ;
goto V_340;
}
V_2 = (struct V_1 * ) ( V_165 + F_107 ( struct V_199 ,
V_343 [ V_100 ] . V_344 ) ) ;
V_2 -> V_5 = V_5 ;
V_131 = F_160 () ;
V_53 = F_156 ( V_2 , V_100 ) ;
F_161 () ;
if ( V_53 ) {
F_23 ( V_50 L_18 ) ;
goto V_340;
}
return V_2 ;
V_340:
return F_162 ( V_53 ) ;
}
int F_163 ( struct V_1 * V_2 )
{
return 0 ;
}
int F_164 ( struct V_1 * V_2 , struct V_345 * V_346 )
{
return - V_40 ;
}
int F_165 ( struct V_1 * V_2 , struct V_345 * V_346 )
{
return - V_40 ;
}
int F_166 ( struct V_1 * V_2 ,
struct V_347 * V_348 )
{
return - V_40 ;
}
void F_167 ( struct V_5 * V_5 )
{
unsigned long V_165 = V_5 -> V_6 . V_165 ;
if ( V_165 ) {
memset ( ( void * ) V_165 , 0 , V_198 ) ;
F_135 ( V_165 , F_106 ( V_198 ) ) ;
}
}
static void F_168 ( struct V_5 * V_5 )
{
struct V_349 * V_350 ;
struct V_351 * V_352 ;
int V_13 ;
unsigned long V_353 ;
V_350 = V_349 ( V_5 ) ;
F_169 (memslot, slots) {
V_353 = V_352 -> V_353 ;
for ( V_13 = 0 ; V_13 < V_352 -> V_354 ; V_13 ++ ) {
if ( V_352 -> V_355 [ V_13 ] )
F_170 ( (struct V_356 * ) V_352 -> V_355 [ V_13 ] ) ;
}
}
}
void F_171 ( struct V_5 * V_5 )
{
}
void F_172 ( struct V_5 * V_5 )
{
F_173 ( V_5 ) ;
#ifdef F_174
F_175 ( V_5 ) ;
#endif
F_176 ( V_5 -> V_6 . V_357 ) ;
F_168 ( V_5 ) ;
}
void F_177 ( struct V_1 * V_2 )
{
}
void F_178 ( struct V_1 * V_2 , int V_131 )
{
if ( V_131 != V_2 -> V_131 ) {
V_2 -> V_131 = V_131 ;
if ( V_2 -> V_6 . V_150 )
F_143 ( V_2 ) ;
}
}
int F_179 ( struct V_1 * V_2 , struct V_218 * V_107 )
{
struct V_24 * V_24 = F_41 ( V_2 -> V_5 , V_2 -> V_6 . V_24 ) ;
int V_12 ;
F_180 ( V_2 ) ;
for ( V_12 = 0 ; V_12 < 16 ; V_12 ++ ) {
V_107 -> V_24 . V_219 [ V_12 ] = V_24 -> V_219 [ V_12 ] ;
V_107 -> V_24 . V_220 [ V_12 ] = V_24 -> V_220 [ V_12 ] ;
}
for ( V_12 = 0 ; V_12 < 128 ; V_12 ++ )
V_107 -> V_24 . V_221 [ V_12 ] = V_24 -> V_221 [ V_12 ] ;
V_107 -> V_24 . V_222 = V_24 -> V_222 ;
V_107 -> V_24 . V_223 = V_24 -> V_223 ;
V_107 -> V_24 . V_224 = V_24 -> V_224 ;
V_107 -> V_24 . V_225 = V_24 -> V_225 ;
V_107 -> V_24 . V_226 = V_24 -> V_226 ;
memcpy ( & V_107 -> V_228 , & V_2 -> V_6 . V_227 , sizeof( union V_174 ) ) ;
F_181 ( V_118 ) ;
F_181 ( V_173 ) ;
memcpy ( V_107 -> V_229 , V_2 -> V_6 . V_229 , sizeof( struct V_230 ) * V_231 ) ;
memcpy ( V_107 -> V_232 , V_2 -> V_6 . V_232 , sizeof( struct V_230 ) * V_233 ) ;
F_181 ( V_234 ) ;
F_181 ( V_235 ) ;
F_181 ( V_236 ) ;
F_181 ( V_237 ) ;
F_181 ( V_238 ) ;
F_181 ( V_149 ) ;
F_181 ( V_239 ) ;
F_181 ( V_240 ) ;
for ( V_12 = 0 ; V_12 < 8 ; V_12 ++ ) {
V_107 -> V_241 [ V_12 ] = V_2 -> V_6 . V_241 [ V_12 ] ;
V_107 -> V_242 [ V_12 ] = V_2 -> V_6 . V_242 [ V_12 ] ;
V_107 -> V_243 [ V_12 ] = V_2 -> V_6 . V_243 [ V_12 ] ;
}
for ( V_12 = 0 ; V_12 < 4 ; V_12 ++ )
V_107 -> V_244 [ V_12 ] = V_2 -> V_6 . V_244 [ V_12 ] ;
V_107 -> V_250 = V_2 -> V_6 . V_147 + F_1 ( V_2 ) ;
F_181 ( V_245 ) ;
F_181 ( V_204 ) ;
F_181 ( V_206 ) ;
F_181 ( V_246 ) ;
F_181 ( V_247 ) ;
F_181 ( V_248 ) ;
F_181 ( V_249 ) ;
F_182 ( V_2 ) ;
return 0 ;
}
int F_183 ( struct V_1 * V_2 ,
struct V_358 * V_359 )
{
memcpy ( V_359 , V_2 , sizeof( struct V_358 ) ) ;
return 0 ;
}
int F_184 ( struct V_1 * V_2 ,
struct V_358 * V_359 )
{
memcpy ( V_2 + 1 , & V_359 -> V_359 [ 0 ] + sizeof( struct V_1 ) ,
sizeof( struct V_358 ) - sizeof( struct V_1 ) ) ;
V_2 -> V_6 . V_360 = ( (struct V_1 * ) V_359 ) -> V_6 . V_360 ;
return 0 ;
}
void F_185 ( struct V_1 * V_2 )
{
F_67 ( & V_2 -> V_6 . V_145 ) ;
F_176 ( V_2 -> V_6 . V_311 ) ;
}
long F_186 ( struct V_257 * V_258 ,
unsigned int V_259 , unsigned long V_260 )
{
struct V_1 * V_2 = V_258 -> V_261 ;
void T_7 * V_262 = ( void T_7 * ) V_260 ;
struct V_358 * V_359 = NULL ;
long V_53 ;
switch ( V_259 ) {
case V_361 : {
struct V_358 T_7 * V_362 ;
void T_7 * V_363 = V_262 ;
V_53 = - V_269 ;
if ( F_122 ( & V_362 , V_363 , sizeof( void * ) ) )
goto V_163;
if ( ! F_187 ( V_364 , V_362 ,
sizeof( struct V_358 ) ) ) {
F_23 ( V_48 L_19
L_20 ) ;
goto V_163;
}
V_359 = F_151 ( sizeof( struct V_358 ) , V_197 ) ;
if ( ! V_359 ) {
V_53 = - V_283 ;
goto V_163;
}
V_53 = F_183 ( V_2 , V_359 ) ;
if ( V_53 )
goto V_163;
if ( F_129 ( V_362 , V_359 ,
sizeof( struct V_358 ) ) ) {
V_53 = - V_269 ;
goto V_163;
}
break;
}
case V_365 : {
struct V_358 T_7 * V_362 ;
void T_7 * V_363 = V_262 ;
V_53 = - V_269 ;
if ( F_122 ( & V_362 , V_363 , sizeof( void * ) ) )
goto V_163;
if ( ! F_187 ( V_366 , V_362 ,
sizeof( struct V_358 ) ) ) {
F_23 ( V_48 L_21
L_22 ) ;
goto V_163;
}
V_359 = F_188 ( sizeof( struct V_358 ) , V_197 ) ;
if ( ! V_359 ) {
V_53 = - V_283 ;
goto V_163;
}
if ( F_122 ( V_359 , V_362 ,
sizeof( struct V_358 ) ) )
goto V_163;
V_53 = F_184 ( V_2 , V_359 ) ;
break;
}
default:
V_53 = - V_40 ;
}
V_163:
F_176 ( V_359 ) ;
return V_53 ;
}
int F_189 ( struct V_1 * V_2 , struct V_367 * V_368 )
{
return V_369 ;
}
void F_190 ( struct V_351 * free ,
struct V_351 * V_370 )
{
}
int F_191 ( struct V_351 * V_35 , unsigned long V_354 )
{
return 0 ;
}
int F_192 ( struct V_5 * V_5 ,
struct V_351 * V_352 ,
struct V_351 V_371 ,
struct V_267 * V_372 ,
int V_373 )
{
unsigned long V_12 ;
unsigned long V_374 ;
int V_354 = V_352 -> V_354 ;
unsigned long V_353 = V_352 -> V_353 ;
if ( V_353 + V_354 > ( V_375 >> V_137 ) )
return - V_283 ;
for ( V_12 = 0 ; V_12 < V_354 ; V_12 ++ ) {
V_374 = F_193 ( V_5 , V_353 + V_12 ) ;
if ( ! F_194 ( V_374 ) ) {
F_109 ( V_5 , V_353 + V_12 ,
V_374 << V_137 ,
V_376 | V_377 ) ;
V_352 -> V_355 [ V_12 ] = ( unsigned long ) F_195 ( V_374 ) ;
} else {
F_109 ( V_5 , V_353 + V_12 ,
V_378 | ( V_374 << V_137 ) ,
V_379 ) ;
V_352 -> V_355 [ V_12 ] = 0 ;
}
}
return 0 ;
}
void F_196 ( struct V_5 * V_5 ,
struct V_267 * V_372 ,
struct V_351 V_371 ,
int V_373 )
{
return;
}
void F_197 ( struct V_5 * V_5 )
{
F_198 ( V_5 ) ;
}
void F_199 ( struct V_5 * V_5 ,
struct V_351 * V_35 )
{
F_197 () ;
}
long F_200 ( struct V_257 * V_258 ,
unsigned int V_259 , unsigned long V_260 )
{
return - V_40 ;
}
void F_201 ( struct V_1 * V_2 )
{
F_158 ( V_2 ) ;
}
static int F_202 ( void )
{
long V_380 = 1 , V_30 = 1 , V_381 = 1 ;
long V_301 ;
V_301 = F_203 ( & V_380 , & V_30 , & V_381 , 0 ) ;
if ( V_301 )
goto V_163;
if ( ! ( V_380 & V_382 ) )
goto V_163;
F_23 ( V_50 L_23 ) ;
V_301 = F_204 ( & V_280 , & V_383 ) ;
if ( V_301 )
goto V_163;
F_23 ( V_50 L_24 , V_280 ) ;
if ( ! ( V_383 & V_384 ) ) {
F_23 ( V_47 L_25
L_26 , V_383 ) ;
}
return 1 ;
V_163:
return 0 ;
}
static void F_205 ( struct V_186 * V_385 ,
struct V_386 * V_386 )
{
unsigned long V_387 , V_388 ;
unsigned long V_389 ;
if ( ! F_111 ( L_11 ) )
return;
V_389 = ( unsigned long ) V_386 -> V_390 ;
V_387 = V_36 + V_385 -> V_391 - V_389 ;
V_388 = V_36 + V_385 -> V_392 - V_389 ;
F_23 ( V_48 L_27
L_28 ) ;
memcpy ( ( void * ) V_387 , ( void * ) V_388 , 0x60 ) ;
}
static int F_206 ( struct V_186 * V_385 ,
struct V_386 * V_386 )
{
unsigned long V_389 ;
unsigned long V_393 ;
unsigned long V_394 , V_395 , V_396 ;
struct V_397 * V_398 ;
F_104 ( ! V_386 ) ;
if ( ! V_36 ) {
F_23 ( L_29 ) ;
return - V_269 ;
}
V_389 = ( unsigned long ) V_386 -> V_390 ;
V_393 = V_386 -> V_399 ;
if ( F_102 ( V_393 > V_282 ) )
return - V_269 ;
memcpy ( ( void * ) V_36 , ( void * ) V_389 , V_393 ) ;
F_205 ( V_385 , V_386 ) ;
F_5 ( V_36 , V_393 ) ;
V_394 = V_385 -> V_302 - V_389 ;
V_186 -> V_302 = V_38 + V_394 ;
F_23 ( V_50 L_30 ,
V_186 -> V_302 ) ;
V_396 = ( unsigned long ) V_385 -> V_328 - V_389 ;
V_186 -> V_328 = ( V_400 * ) ( V_38 +
V_396 ) ;
V_395 = * ( unsigned long * ) V_385 -> V_328 - V_389 ;
V_398 = (struct V_397 * ) ( V_36 + V_396 ) ;
V_398 -> V_401 = V_38 + V_395 ;
V_398 -> V_402 = V_38 + ( V_398 -> V_402 - V_389 ) ;
F_23 ( V_50 L_31 ,
V_38 + V_395 ) ;
V_396 = ( unsigned long ) V_385 -> V_187 - V_389 ;
V_186 -> V_187 = ( V_403 * ) ( V_38 +
V_396 ) ;
V_395 = * ( unsigned long * ) V_385 -> V_187 - V_389 ;
V_398 = (struct V_397 * ) ( V_36 + V_396 ) ;
V_398 -> V_401 = V_38 + V_395 ;
V_398 -> V_402 = V_38 + ( V_398 -> V_402 - V_389 ) ;
V_334 = V_398 -> V_402 ;
F_23 ( V_50 L_32 ,
V_186 -> V_328 ) ;
F_23 ( V_50 L_33 ,
V_38 + V_395 ) ;
return 0 ;
}
int F_207 ( void * V_404 )
{
int V_53 ;
struct V_186 * V_385 = (struct V_186 * ) V_404 ;
if ( ! F_202 () ) {
F_23 ( V_85 L_34 ) ;
V_53 = - V_405 ;
goto V_163;
}
if ( V_186 ) {
F_23 ( V_85 L_35 ) ;
V_53 = - V_406 ;
goto V_163;
}
V_53 = - V_283 ;
V_186 = F_151 ( sizeof( struct V_186 ) , V_197 ) ;
if ( ! V_186 )
goto V_163;
if ( F_133 () )
goto V_407;
V_53 = F_206 ( V_385 , V_385 -> V_386 ) ;
if ( V_53 )
goto V_408;
return 0 ;
V_408:
F_134 () ;
V_407:
F_176 ( V_186 ) ;
V_163:
return V_53 ;
}
void F_208 ( void )
{
F_134 () ;
F_176 ( V_186 ) ;
V_186 = NULL ;
}
static void F_209 ( struct V_5 * V_5 ,
struct V_351 * V_352 )
{
int V_12 ;
long V_304 ;
unsigned long V_409 ;
unsigned long * V_410 = ( unsigned long * ) ( V_5 -> V_6 . V_165 +
F_107 ( struct V_199 , V_411 ) ) ;
V_409 = F_210 ( V_352 ) ;
V_304 = V_352 -> V_353 / V_412 ;
F_20 ( & V_5 -> V_6 . V_336 ) ;
for ( V_12 = 0 ; V_12 < V_409 / sizeof( long ) ; ++ V_12 ) {
V_352 -> V_410 [ V_12 ] = V_410 [ V_304 + V_12 ] ;
V_410 [ V_304 + V_12 ] = 0 ;
}
F_22 ( & V_5 -> V_6 . V_336 ) ;
}
int F_211 ( struct V_5 * V_5 ,
struct V_413 * log )
{
int V_53 ;
unsigned long V_409 ;
struct V_351 * V_352 ;
int V_414 = 0 ;
F_126 ( & V_5 -> V_273 ) ;
V_53 = - V_40 ;
if ( log -> V_35 >= V_415 )
goto V_163;
V_352 = F_212 ( V_5 -> V_416 , log -> V_35 ) ;
V_53 = - V_417 ;
if ( ! V_352 -> V_410 )
goto V_163;
F_209 ( V_5 , V_352 ) ;
V_53 = F_213 ( V_5 , log , & V_414 ) ;
if ( V_53 )
goto V_163;
if ( V_414 ) {
F_198 ( V_5 ) ;
V_409 = F_210 ( V_352 ) ;
memset ( V_352 -> V_410 , 0 , V_409 ) ;
}
V_53 = 0 ;
V_163:
F_128 ( & V_5 -> V_273 ) ;
return V_53 ;
}
int F_214 ( void )
{
return 0 ;
}
void F_215 ( void )
{
}
int F_216 ( struct V_1 * V_2 , struct V_418 * V_255 )
{
return F_40 ( V_2 , V_255 -> V_91 ) ;
}
int F_217 ( struct V_318 * V_311 , T_10 V_419 )
{
return V_311 -> V_2 -> V_331 == V_419 ;
}
int F_218 ( struct V_318 * V_311 , T_11 V_420 )
{
return 0 ;
}
int F_219 ( struct V_1 * V_421 , struct V_1 * V_422 )
{
return V_421 -> V_6 . V_245 - V_422 -> V_6 . V_245 ;
}
int F_220 ( struct V_1 * V_2 , struct V_318 * V_423 ,
int V_424 , int V_419 , int V_425 )
{
struct V_318 * V_426 = V_2 -> V_6 . V_311 ;
return ( V_425 == 0 ) ?
F_217 ( V_426 , V_419 ) :
F_218 ( V_426 , V_419 ) ;
}
static int F_221 ( int * V_427 )
{
T_5 V_428 , V_429 ;
int V_12 ;
for ( V_12 = 7 ; V_12 >= 0 ; V_12 -- ) {
V_428 = V_427 [ V_12 ] ;
if ( V_428 ) {
V_429 = F_222 ( V_428 ) ;
return V_12 * 32 + V_429 - 1 ;
}
}
return - 1 ;
}
int F_223 ( struct V_1 * V_2 )
{
struct V_24 * V_24 = F_41 ( V_2 -> V_5 , V_2 -> V_6 . V_24 ) ;
if ( V_24 -> V_92 [ 0 ] & ( 1UL << V_430 ) )
return V_430 ;
if ( V_24 -> V_92 [ 0 ] & ( 1UL << V_431 ) )
return V_431 ;
return F_221 ( ( int * ) & V_24 -> V_92 [ 0 ] ) ;
}
int F_69 ( struct V_1 * V_2 )
{
return V_2 -> V_6 . V_309 ;
}
int F_224 ( struct V_1 * V_2 )
{
return ( V_2 -> V_6 . V_118 == V_119 ) ||
( F_223 ( V_2 ) != - 1 ) ;
}
int F_225 ( struct V_1 * V_2 )
{
return ( ! F_42 ( V_182 , & V_2 -> V_124 ) ) ;
}
int F_226 ( struct V_1 * V_2 ,
struct V_432 * V_118 )
{
V_118 -> V_118 = V_2 -> V_6 . V_118 ;
return 0 ;
}
static int F_227 ( struct V_1 * V_2 )
{
int V_53 ;
long V_169 ;
F_10 ( V_169 ) ;
V_53 = F_77 ( V_2 ) ;
F_12 ( V_169 ) ;
if ( V_53 )
goto V_340;
V_2 -> V_6 . V_112 = 0 ;
F_185 ( V_2 ) ;
V_53 = F_148 ( V_2 ) ;
if ( V_53 )
goto V_340;
F_78 ( V_2 ) ;
V_53 = 0 ;
V_340:
return V_53 ;
}
int F_228 ( struct V_1 * V_2 ,
struct V_432 * V_118 )
{
int V_53 = 0 ;
V_2 -> V_6 . V_118 = V_118 -> V_118 ;
if ( V_2 -> V_6 . V_118 == V_130 )
V_53 = F_227 ( V_2 ) ;
return V_53 ;
}
