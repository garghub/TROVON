static int F_1 ( struct V_1 * V_2 ,
struct V_3 * V_4 ,
struct V_5 * V_6 ,
unsigned int * V_7 )
{
struct V_8 * V_9 = V_2 -> V_10 ;
unsigned int V_11 = F_2 ( V_6 -> V_12 ) ;
unsigned long V_13 = V_2 -> V_14 + F_3 ( V_11 ) ;
unsigned int V_15 = 0 ;
int V_16 ;
for ( V_16 = 0 ; V_16 < V_6 -> V_17 ; V_16 ++ ) {
V_15 = V_7 [ V_16 ] ;
F_4 ( V_15 & 0x00ff , V_13 ) ;
F_4 ( ( V_15 >> 8 ) & 0x00ff , V_13 + 1 ) ;
}
V_9 -> V_18 [ V_11 ] = V_15 ;
return V_6 -> V_17 ;
}
static int F_5 ( struct V_1 * V_2 ,
struct V_3 * V_4 ,
struct V_5 * V_6 ,
unsigned int * V_7 )
{
struct V_8 * V_9 = V_2 -> V_10 ;
int V_11 = F_2 ( V_6 -> V_12 ) ;
unsigned long V_13 = V_2 -> V_14 + F_3 ( V_11 ) ;
int V_16 ;
for ( V_16 = 0 ; V_16 < V_6 -> V_17 ; V_16 ++ ) {
F_6 ( V_13 ) ;
V_7 [ V_16 ] = V_9 -> V_18 [ V_11 ] ;
}
return V_6 -> V_17 ;
}
static int F_7 ( struct V_1 * V_2 ,
struct V_19 * V_20 )
{
struct V_8 * V_9 ;
struct V_3 * V_4 ;
int V_21 ;
F_8 ( V_2 , & V_20 -> V_2 ) ;
V_2 -> V_22 = V_2 -> V_23 -> V_24 ;
V_21 = F_9 ( V_2 , sizeof( * V_9 ) ) ;
if ( V_21 )
return V_21 ;
V_9 = V_2 -> V_10 ;
V_21 = F_10 ( V_20 , V_2 -> V_22 ) ;
if ( V_21 )
return V_21 ;
V_2 -> V_14 = F_11 ( V_20 , 3 ) ;
V_21 = F_12 ( V_2 , 2 ) ;
if ( V_21 )
return V_21 ;
V_4 = & V_2 -> V_25 [ 0 ] ;
V_4 -> type = V_26 ;
V_4 -> V_27 = V_28 | V_29 ;
V_4 -> V_30 = 6 ;
V_4 -> V_31 = 0xffff ;
V_4 -> V_32 = & V_33 ;
V_4 -> V_34 = F_1 ;
V_4 -> V_35 = F_5 ;
V_4 = & V_2 -> V_25 [ 1 ] ;
V_21 = F_13 ( V_2 , V_4 , NULL ,
V_2 -> V_14 + V_36 ) ;
if ( V_21 )
return V_21 ;
F_14 ( V_2 -> V_37 , L_1 , V_2 -> V_22 ) ;
return 1 ;
}
static void F_15 ( struct V_1 * V_2 )
{
struct V_19 * V_20 = F_16 ( V_2 ) ;
if ( V_2 -> V_25 )
F_17 ( V_2 , & V_2 -> V_25 [ 1 ] ) ;
if ( V_20 ) {
if ( V_2 -> V_14 )
F_18 ( V_20 ) ;
}
}
static int T_1 F_19 ( struct V_19 * V_2 ,
const struct V_38 * V_39 )
{
return F_20 ( V_2 , & V_40 ) ;
}
static void T_2 F_21 ( struct V_19 * V_2 )
{
F_22 ( V_2 ) ;
}
