T_1 F_1 ( T_1 V_1 , int V_2 , T_1 V_3 )
{
if ( V_2 == 1 )
return V_1 - V_3 ;
return V_1 ;
}
int F_2 ( struct V_4 * V_5 ,
enum V_6 V_2 )
{
T_1 V_7 ,
V_8 ,
V_9 ;
int V_10 = FALSE ;
V_7 = ( F_3 ( V_5 , V_11 ) << 8 )
| F_3 ( V_5 , V_12 ) ;
V_7 = F_4 ( V_7 , 16 ) ;
V_8 = ( F_3 ( V_5 , V_13 ) << 8 )
| F_3 ( V_5 , V_14 ) ;
V_9 = V_5 -> V_15 [ V_2 ] / 1000 ;
V_9 += ( V_9 / 10 ) ;
V_9 = 65536 * ( V_9 / 2 ) ;
V_9 /= V_5 -> V_16 / 1000 ;
if ( V_9 > 0x4000 )
V_9 = 0x4000 ;
if ( ( V_8 > 0x2000 )
|| ( V_7 > ( 2 * V_9 ) )
|| ( V_7 < ( - 2 * V_9 ) ) )
V_10 = TRUE ;
return V_10 ;
}
static void F_5 ( struct V_4 * V_5 ,
T_1 * V_17 , T_1 * V_18 ,
T_1 * V_19 ,
enum V_6 V_2 )
{
T_1 V_20 , V_21 , V_22 , V_23 , V_9 ;
enum V_24 V_25 ;
V_23 = V_5 -> V_26 [ V_2 ] ;
V_9 = V_5 -> V_15 [ V_2 ] / 1000 ;
V_9 += V_9 / 10 ;
V_25 = V_5 -> V_27 [ V_2 ] ;
V_9 = 65536 * ( V_9 / 2 ) ;
V_9 /= V_5 -> V_16 / 1000 ;
if ( V_9 > 0x4000 )
V_9 = 0x4000 ;
V_21 = V_23 ;
V_21 /= V_5 -> V_16 >> 10 ;
V_21 = V_21 << 6 ;
switch ( V_25 ) {
case V_28 :
case V_29 :
V_21 *= 3 ;
V_20 = 20 ;
break;
case V_30 :
V_21 *= 4 ;
V_20 = 25 ;
break;
case V_31 :
default:
V_21 *= 3 ;
V_20 = 25 ;
break;
}
V_21 /= 100 ;
if ( ( V_21 > V_9 ) || ( V_21 < 0 ) )
V_21 = V_9 / 2 ;
V_20 *= 27500 ;
if ( V_23 > 0 )
V_20 /= V_23 / 1000 ;
if ( ( V_20 > 100 ) || ( V_20 < 0 ) )
V_20 = 100 ;
V_22 = ( V_9 / V_21 ) + 1 ;
if ( ( V_22 > 100 ) || ( V_22 < 0 ) ) {
V_22 = 100 ;
V_21 = V_9 / V_22 ;
}
* V_17 = V_21 ;
* V_18 = V_20 ;
* V_19 = V_22 ;
}
static int F_6 ( struct V_4 * V_5 ,
T_1 V_32 , T_1 V_33 , int V_34 ,
T_1 V_35 , enum V_6 V_2 )
{
int V_10 ,
V_36 = FALSE ;
T_1 V_37 ,
V_38 ,
V_9 ;
V_9 = V_5 -> V_15 [ V_2 ] / 1000 ;
V_9 += ( V_9 / 10 ) ;
V_9 = 65536 * ( V_9 / 2 ) ;
V_9 /= V_5 -> V_16 / 1000 ;
if ( V_9 > 0x4000 )
V_9 = 0x4000 ;
if ( V_34 == TRUE )
V_38 = 0 ;
else
V_38 = - V_9 + V_32 ;
V_37 = 0 ;
do {
F_7 ( V_5 , V_39 , 0x1c ) ;
F_7 ( V_5 , V_40 , ( V_38 / 256 ) & 0xff ) ;
F_7 ( V_5 , V_41 , V_38 & 0xff ) ;
F_7 ( V_5 , V_39 , 0x18 ) ;
F_8 ( V_5 , V_42 , 1 ) ;
if ( V_5 -> V_43 == 0x12 ) {
F_8 ( V_5 , V_44 , 1 ) ;
F_8 ( V_5 , V_44 , 0 ) ;
}
if ( V_34 == TRUE ) {
if ( V_38 >= 0 )
V_38 = - V_38 - 2 * V_32 ;
else
V_38 = - V_38 ;
} else
V_38 += + 2 * V_32 ;
V_37 ++ ;
V_36 = F_9 ( V_5 , V_2 , V_33 ) ;
V_10 = F_2 ( V_5 , V_2 ) ;
} while ( ( V_36 == FALSE )
&& ( V_10 == FALSE )
&& ( ( V_38 - V_32 ) < V_9 )
&& ( ( V_38 + V_32 ) > - V_9 )
&& ( V_37 < V_35 ) );
F_8 ( V_5 , V_42 , 0 ) ;
return V_36 ;
}
static int F_10 ( struct V_4 * V_5 ,
enum V_6 V_2 )
{
int V_36 = FALSE ,
V_10 ,
V_34 ;
T_1 V_45 ,
V_46 ,
V_47 ,
V_48 ,
V_22 ;
F_5 ( V_5 , & V_46 , & V_47 ,
& V_22 , V_2 ) ;
switch ( V_5 -> V_27 [ V_2 ] ) {
case V_28 :
case V_29 :
if ( V_5 -> V_43 >= 0x20 )
F_7 ( V_5 , V_49 , 0x3b ) ;
else
F_7 ( V_5 , V_49 , 0xef ) ;
F_7 ( V_5 , V_50 , 0x49 ) ;
V_34 = FALSE ;
break;
case V_30 :
if ( V_5 -> V_43 >= 0x20 )
F_7 ( V_5 , V_51 , 0x79 ) ;
else
F_7 ( V_5 , V_51 , 0x68 ) ;
F_7 ( V_5 , V_50 , 0x89 ) ;
V_34 = TRUE ;
break;
case V_31 :
default:
if ( V_5 -> V_43 >= 0x20 ) {
F_7 ( V_5 , V_49 , 0x3b ) ;
F_7 ( V_5 , V_51 , 0x79 ) ;
} else {
F_7 ( V_5 , V_49 , 0xef ) ;
F_7 ( V_5 , V_51 , 0x68 ) ;
}
F_7 ( V_5 , V_50 , 0xc9 ) ;
V_34 = FALSE ;
break;
}
V_48 = 0 ;
do {
V_36 = F_6 ( V_5 ,
V_46 ,
V_47 ,
V_34 ,
V_22 ,
V_2 ) ;
V_10 = F_2 ( V_5 , V_2 ) ;
V_48 ++ ;
if ( ( V_36 == TRUE )
|| ( V_10 == TRUE )
|| ( V_48 == 2 ) ) {
if ( V_5 -> V_43 >= 0x20 ) {
F_7 ( V_5 , V_49 , 0x49 ) ;
F_7 ( V_5 , V_51 , 0x9e ) ;
} else {
F_7 ( V_5 , V_49 , 0xed ) ;
F_7 ( V_5 , V_51 , 0x88 ) ;
}
if ( ( F_11 ( V_5 , V_52 ) ==
V_53 ) &&
( V_36 == TRUE ) ) {
F_12 ( V_47 ) ;
V_45 = F_11 ( V_5 , V_54 ) ;
if ( V_45 < 0xd ) {
F_12 ( V_47 ) ;
V_45 = F_11 ( V_5 ,
V_54 ) ;
}
if ( V_45 < 0xd ) {
V_36 = FALSE ;
if ( V_48 < 2 ) {
if ( V_5 -> V_43 >= 0x20 )
F_7 ( V_5 ,
V_51 ,
0x79 ) ;
else
F_7 ( V_5 ,
V_51 ,
0x68 ) ;
F_7 ( V_5 ,
V_50 ,
0x89 ) ;
}
}
}
}
} while ( ( V_36 == FALSE )
&& ( V_48 < 2 )
&& ( V_10 == FALSE ) );
return V_36 ;
}
static T_2 F_13 ( struct V_4 * V_5 ,
T_2 V_16 ,
enum V_6 V_2 )
{
T_1 V_55 , V_56 , V_57 , V_58 , V_23 ;
V_23 = ( F_11 ( V_5 , V_59 ) << 24 ) +
( F_11 ( V_5 , V_60 ) << 16 ) +
( F_11 ( V_5 , V_61 ) << 8 ) +
( F_11 ( V_5 , V_62 ) ) ;
F_14 ( L_1 ,
V_23 , F_11 ( V_5 , V_62 ) ,
F_11 ( V_5 , V_61 ) ,
F_11 ( V_5 , V_60 ) ,
F_11 ( V_5 , V_59 ) ) ;
V_57 = ( V_16 ) >> 16 ;
V_58 = ( V_23 ) >> 16 ;
V_55 = ( V_16 ) % 0x10000 ;
V_56 = ( V_23 ) % 0x10000 ;
V_23 = ( V_57 * V_58 ) +
( ( V_57 * V_56 ) >> 16 ) +
( ( V_58 * V_55 ) >> 16 ) ;
return V_23 ;
}
static void F_15 ( struct V_4 * V_5 ,
T_2 V_16 , T_2 V_23 ,
enum V_6 V_2 )
{
T_2 V_63 ;
F_14 ( L_2 , V_64 , V_16 ,
V_23 , V_2 ) ;
if ( V_23 > 60000000 ) {
V_63 = V_23 << 4 ;
V_63 /= ( V_16 >> 12 ) ;
} else if ( V_23 > 6000000 ) {
V_63 = V_23 << 6 ;
V_63 /= ( V_16 >> 10 ) ;
} else {
V_63 = V_23 << 9 ;
V_63 /= ( V_16 >> 7 ) ;
}
F_7 ( V_5 , V_65 , ( V_63 >> 8 ) & 0x7f ) ;
F_7 ( V_5 , V_65 + 1 , ( V_63 & 0xff ) ) ;
}
static void F_16 ( struct V_4 * V_5 ,
T_2 V_16 , T_2 V_23 ,
enum V_6 V_2 )
{
T_2 V_63 ;
V_23 = 105 * ( V_23 / 100 ) ;
if ( V_23 > 60000000 ) {
V_63 = V_23 << 4 ;
V_63 /= ( V_16 >> 12 ) ;
} else if ( V_23 > 6000000 ) {
V_63 = V_23 << 6 ;
V_63 /= ( V_16 >> 10 ) ;
} else {
V_63 = V_23 << 9 ;
V_63 /= ( V_16 >> 7 ) ;
}
if ( V_63 < 0x7fff ) {
F_7 ( V_5 , V_66 , ( V_63 >> 8 ) & 0x7f ) ;
F_7 ( V_5 , V_66 + 1 , ( V_63 & 0xff ) ) ;
} else {
F_7 ( V_5 , V_66 , 0x7f ) ;
F_7 ( V_5 , V_66 + 1 , 0xff ) ;
}
}
static void F_17 ( struct V_4 * V_5 ,
T_2 V_16 , T_2 V_23 ,
enum V_6 V_2 )
{
T_2 V_63 ;
V_23 = 95 * ( V_23 / 100 ) ;
if ( V_23 > 60000000 ) {
V_63 = V_23 << 4 ;
V_63 /= ( V_16 >> 12 ) ;
} else if ( V_23 > 6000000 ) {
V_63 = V_23 << 6 ;
V_63 /= ( V_16 >> 10 ) ;
} else {
V_63 = V_23 << 9 ;
V_63 /= ( V_16 >> 7 ) ;
}
F_7 ( V_5 , V_67 , ( V_63 >> 8 ) & 0xff ) ;
F_7 ( V_5 , V_67 + 1 , ( V_63 & 0xff ) ) ;
}
static T_1 F_18 ( struct V_4 * V_5 ,
T_2 V_23 ,
enum V_6 V_2 )
{
T_1 V_68 ;
V_68 = ( F_3 ( V_5 , V_69 ) << 16 ) +
( F_3 ( V_5 , V_69 + 1 ) << 8 ) +
( F_3 ( V_5 , V_69 + 2 ) ) ;
V_68 = F_4 ( V_68 , 24 ) ;
if ( V_68 == 0 )
V_68 = 1 ;
V_68 = ( ( T_1 ) V_23 * 10 ) / ( ( T_1 ) 0x1000000 / V_68 ) ;
V_68 /= 320 ;
return V_68 ;
}
static void F_19 ( struct V_4 * V_5 ,
enum V_6 V_2 )
{
T_1 V_70 ;
if ( V_5 -> V_43 == 0x10 ) {
F_8 ( V_5 , V_71 , 1 ) ;
V_70 = F_3 ( V_5 , V_72 ) & 0x03 ;
F_8 ( V_5 , V_73 , V_70 ) ;
} else if ( V_5 -> V_43 <= 0x20 )
F_8 ( V_5 , V_71 , 0 ) ;
else
F_8 ( V_5 , V_74 , 0 ) ;
}
static T_2 F_20 ( T_2 V_23 , enum V_75 V_76 )
{
T_2 V_70 ;
switch ( V_76 ) {
case V_77 :
V_70 = 20 ;
break;
case V_78 :
V_70 = 25 ;
break;
case V_79 :
default:
V_70 = 35 ;
break;
}
return V_23 + ( V_23 * V_70 ) / 100 ;
}
static int F_21 ( struct V_4 * V_5 ,
enum V_6 V_2 )
{
int V_80 = FALSE ;
T_1 V_81 ,
V_82 = 0 ;
T_3 V_83 ,
V_84 ,
V_85 ;
V_83 = F_3 ( V_5 , V_49 ) ;
V_84 = F_3 ( V_5 , V_86 ) ;
V_85 = F_3 ( V_5 , V_87 ) ;
F_7 ( V_5 , V_86 , 0x20 ) ;
F_7 ( V_5 , V_87 , 0x0 ) ;
F_8 ( V_5 , V_88 , 0 ) ;
F_7 ( V_5 , V_89 , 0x80 ) ;
F_7 ( V_5 , V_90 , 0x40 ) ;
F_7 ( V_5 , V_49 , 0x0 ) ;
F_7 ( V_5 , V_40 , 0x0 ) ;
F_7 ( V_5 , V_41 , 0x0 ) ;
F_7 ( V_5 , V_91 , 0x65 ) ;
F_7 ( V_5 , V_39 , 0x18 ) ;
F_12 ( 7 ) ;
for ( V_81 = 0 ; V_81 < 10 ; V_81 ++ ) {
if ( F_11 ( V_5 , V_92 ) >= 2 )
V_82 ++ ;
F_12 ( 1 ) ;
}
if ( V_82 >= 3 )
V_80 = TRUE ;
F_7 ( V_5 , V_91 , 0x38 ) ;
F_7 ( V_5 , V_89 , 0x88 ) ;
F_7 ( V_5 , V_90 , 0x68 ) ;
F_7 ( V_5 , V_49 , V_83 ) ;
F_7 ( V_5 , V_86 , V_84 ) ;
F_7 ( V_5 , V_87 , V_85 ) ;
return V_80 ;
}
static int F_22 ( struct V_93 * V_94 ,
T_1 V_95 )
{
struct V_96 * V_97 = V_94 -> V_98 ;
struct V_4 * V_5 = V_97 -> V_99 ;
enum V_6 V_2 = V_97 -> V_2 ;
int V_36 = FALSE ,
V_100 = V_2 ;
T_1 V_23 ,
V_101 ,
V_102 ,
V_103 ,
V_104 ,
V_105 ,
V_106 ,
V_107 ,
V_20 ,
V_108 ;
V_23 = V_5 -> V_26 [ V_100 ] ;
V_101 = V_5 -> V_15 [ V_100 ] ;
if ( V_23 >= 10000000 )
V_102 = V_95 / 3 ;
else
V_102 = V_95 / 2 ;
V_36 = F_9 ( V_5 , V_100 , V_102 ) ;
if ( V_36 != FALSE )
return V_36 ;
if ( V_23 >= 10000000 ) {
if ( F_21 ( V_5 , V_100 ) == TRUE ) {
F_7 ( V_5 , V_39 , 0x1f ) ;
F_7 ( V_5 , V_39 , 0x15 ) ;
V_36 = F_9 ( V_5 , V_100 , V_95 ) ;
} else
V_36 = FALSE ;
return V_36 ;
}
if ( V_5 -> V_43 <= 0x20 ) {
if ( V_23 <= 1000000 )
V_103 = 500 ;
else if ( V_23 <= 4000000 )
V_103 = 1000 ;
else if ( V_23 <= 7000000 )
V_103 = 2000 ;
else if ( V_23 <= 10000000 )
V_103 = 3000 ;
else
V_103 = 5000 ;
if ( V_23 >= 2000000 ) {
V_20 = ( V_95 / 3 ) ;
if ( V_20 > 1000 )
V_20 = 1000 ;
} else
V_20 = ( V_95 / 2 ) ;
} else {
V_103 = V_23 / 4000 ;
V_20 = ( V_95 * 3 ) / 4 ;
}
V_104 = ( ( V_101 / 1000 ) / V_103 ) ;
if ( ( V_104 % 2 ) != 0 )
V_104 += 1 ;
if ( V_104 <= 0 )
V_104 = 2 ;
else if ( V_104 > 12 )
V_104 = 12 ;
V_105 = 1 ;
V_106 = 1 ;
if ( V_5 -> V_43 <= 0x20 ) {
V_107 = V_5 -> V_108 [ V_100 ] ;
V_5 -> V_109 [ V_100 ] = F_20 ( V_5 -> V_26 [ V_100 ] ,
V_5 -> V_70 ) + V_5 -> V_26 [ V_100 ] ;
} else
V_107 = 0 ;
while ( ( V_105 <= V_104 ) && ( V_36 == FALSE ) ) {
if ( V_106 > 0 )
V_107 += ( V_105 * V_103 ) ;
else
V_107 -= ( V_105 * V_103 ) ;
if ( V_5 -> V_43 <= 0x20 ) {
if ( V_5 -> V_110 [ V_100 ] == 3 )
F_23 ( V_5 , V_107 ,
V_5 -> V_109 [ V_100 ] , V_2 ) ;
else
F_24 ( V_94 , V_107 , V_5 -> V_109 [ V_100 ] ) ;
F_7 ( V_5 , V_39 , 0x1c ) ;
F_7 ( V_5 , V_40 , 0 ) ;
F_7 ( V_5 , V_41 , 0 ) ;
F_7 ( V_5 , V_39 , 0x1f ) ;
F_7 ( V_5 , V_39 , 0x15 ) ;
} else {
F_7 ( V_5 , V_39 , 0x1c ) ;
V_108 = ( V_107 * 65536 ) / ( V_5 -> V_16 / 1000 ) ;
F_8 ( V_5 , V_111 , F_25 ( V_108 ) ) ;
F_8 ( V_5 , V_112 , F_26 ( V_108 ) ) ;
F_7 ( V_5 , V_39 , 0x1f ) ;
F_7 ( V_5 , V_39 , 0x05 ) ;
}
V_36 = F_9 ( V_5 , V_100 , V_20 ) ;
V_106 *= - 1 ;
V_105 ++ ;
}
return V_36 ;
}
static void F_27 ( T_1 * V_95 , T_1 * V_113 ,
T_1 V_23 ,
enum V_114 V_115 )
{
switch ( V_115 ) {
case V_116 :
if ( V_23 <= 1500000 ) {
( * V_95 ) = 1500 ;
( * V_113 ) = 400 ;
} else if ( V_23 <= 5000000 ) {
( * V_95 ) = 1000 ;
( * V_113 ) = 300 ;
} else {
( * V_95 ) = 700 ;
( * V_113 ) = 100 ;
}
break;
case V_117 :
case V_118 :
default:
if ( V_23 <= 1000000 ) {
( * V_95 ) = 3000 ;
( * V_113 ) = 1700 ;
} else if ( V_23 <= 2000000 ) {
( * V_95 ) = 2500 ;
( * V_113 ) = 1100 ;
} else if ( V_23 <= 5000000 ) {
( * V_95 ) = 1000 ;
( * V_113 ) = 550 ;
} else if ( V_23 <= 10000000 ) {
( * V_95 ) = 700 ;
( * V_113 ) = 250 ;
} else if ( V_23 <= 20000000 ) {
( * V_95 ) = 400 ;
( * V_113 ) = 130 ;
} else {
( * V_95 ) = 300 ;
( * V_113 ) = 100 ;
}
break;
}
if ( V_115 == V_118 )
( * V_95 ) /= 2 ;
}
static void F_28 ( struct V_4 * V_5 ,
enum V_6 V_2 )
{
T_1 V_119 = V_120 ;
F_14 ( L_3 , V_64 ) ;
F_7 ( V_5 , V_119 ++ , 0xd0 ) ;
F_7 ( V_5 , V_119 ++ , 0x7d ) ;
F_7 ( V_5 , V_119 ++ , 0x53 ) ;
F_7 ( V_5 , V_119 ++ , 0x2f ) ;
F_7 ( V_5 , V_119 ++ , 0x24 ) ;
F_7 ( V_5 , V_119 ++ , 0x1f ) ;
}
static void F_29 ( struct V_4 * V_5 ,
enum V_24 V_25 ,
enum V_121 V_122 ,
enum V_6 V_2 )
{
F_14 ( L_4 , V_64 ) ;
switch ( V_25 ) {
case V_31 :
F_14 ( L_5 ) ;
F_7 ( V_5 , V_123 , 0x10 ) ;
F_7 ( V_5 , V_124 , 0x3f ) ;
break;
case V_28 :
F_14 ( L_6 ) ;
F_7 ( V_5 , V_123 , 0x00 ) ;
switch ( V_122 ) {
case V_125 :
default:
F_7 ( V_5 , V_124 , 0x2f ) ;
break;
case V_126 :
F_7 ( V_5 , V_124 , 0x01 ) ;
break;
case V_127 :
F_7 ( V_5 , V_124 , 0x02 ) ;
break;
case V_128 :
F_7 ( V_5 , V_124 , 0x04 ) ;
break;
case V_129 :
F_7 ( V_5 , V_124 , 0x08 ) ;
break;
case V_130 :
F_7 ( V_5 , V_124 , 0x20 ) ;
break;
}
break;
case V_29 :
F_14 ( L_7 ) ;
F_7 ( V_5 , V_123 , 0x80 ) ;
switch ( V_122 ) {
case V_125 :
default:
F_7 ( V_5 , V_124 , 0x13 ) ;
break;
case V_126 :
F_7 ( V_5 , V_124 , 0x01 ) ;
break;
case V_127 :
F_7 ( V_5 , V_124 , 0x02 ) ;
break;
case V_131 :
F_7 ( V_5 , V_124 , 0x10 ) ;
break;
}
break;
default:
break;
}
}
static enum V_121 F_30 ( struct V_4 * V_5 ,
enum V_6 V_2 )
{
enum V_121 V_132 ;
T_1 V_133 = F_11 ( V_5 , V_134 ) ;
switch ( V_133 ) {
case 13 :
V_132 = V_126 ;
break;
case 18 :
V_132 = V_127 ;
break;
case 21 :
V_132 = V_128 ;
break;
case 24 :
V_132 = V_129 ;
break;
case 25 :
V_132 = V_131 ;
break;
case 26 :
V_132 = V_130 ;
break;
default:
V_132 = V_125 ;
break;
}
return V_132 ;
}
static void F_31 ( struct V_4 * V_5 ,
enum V_6 V_2 ,
T_2 V_23 )
{
if ( V_5 -> V_43 >= 0x30 ) {
if ( V_23 >= 15000000 ) {
F_7 ( V_5 , V_135 , 0x2b ) ;
F_7 ( V_5 , V_136 , 0x1a ) ;
} else if ( ( V_23 >= 7000000 ) && ( 15000000 > V_23 ) ) {
F_7 ( V_5 , V_135 , 0x0c ) ;
F_7 ( V_5 , V_136 , 0x1b ) ;
} else if ( V_23 < 7000000 ) {
F_7 ( V_5 , V_135 , 0x2c ) ;
F_7 ( V_5 , V_136 , 0x1c ) ;
}
} else {
F_7 ( V_5 , V_135 , 0x1a ) ;
F_7 ( V_5 , V_136 , 0x09 ) ;
}
}
static void F_32 ( struct V_93 * V_94 )
{
struct V_96 * V_97 = V_94 -> V_98 ;
struct V_4 * V_5 = V_97 -> V_99 ;
enum V_6 V_2 = V_97 -> V_2 ;
T_1 V_23 ,
V_137 ,
V_138 ,
V_139 ,
V_140 ,
V_81 = 0 ,
V_141 ,
V_142 ,
V_143 = 0 ,
V_144 ;
enum V_75 V_70 ;
enum V_145 V_146 ;
F_14 ( L_3 , V_64 ) ;
V_23 = F_13 ( V_5 , V_5 -> V_16 , V_2 ) ;
V_23 += F_18 ( V_5 , V_23 , V_2 ) ;
switch ( V_5 -> V_147 [ V_2 ] . V_25 ) {
case V_148 :
case V_149 :
F_14 ( L_8 , V_64 ) ;
if ( V_5 -> V_27 [ V_2 ] == V_31 ) {
F_8 ( V_5 , V_150 , 1 ) ;
F_8 ( V_5 , V_151 , 0 ) ;
}
F_8 ( V_5 , V_73 , V_5 -> V_70 ) ;
F_8 ( V_5 , V_71 , 1 ) ;
if ( V_5 -> V_43 < 0x30 ) {
F_7 ( V_5 , V_152 , 0x75 ) ;
break;
}
if ( F_30 ( V_5 , V_2 ) == V_126 ) {
F_7 ( V_5 , V_153 , 0x98 ) ;
F_7 ( V_5 , V_154 , 0x18 ) ;
} else {
F_7 ( V_5 , V_153 , 0x18 ) ;
F_7 ( V_5 , V_154 , 0x18 ) ;
}
F_7 ( V_5 , V_152 , 0x75 ) ;
break;
case V_155 :
F_14 ( L_9 , V_64 ) ;
F_8 ( V_5 , V_150 , 0 ) ;
F_8 ( V_5 , V_151 , 1 ) ;
F_7 ( V_5 , V_135 , 0 ) ;
F_7 ( V_5 , V_136 , 0 ) ;
if ( V_5 -> V_147 [ V_2 ] . V_156 == V_157 ) {
V_146 = F_11 ( V_5 , V_158 ) ;
V_137 = F_11 ( V_5 , V_159 ) & 0x01 ;
V_138 = F_33 ( V_23 ,
V_146 ,
V_137 ,
V_5 -> V_43 ) ;
if ( V_146 <= V_160 )
F_7 ( V_5 , V_161 , V_138 ) ;
else if ( V_146 <= V_162 ) {
F_7 ( V_5 , V_161 , 0x2a ) ;
F_7 ( V_5 , V_163 , V_138 ) ;
}
if ( ( V_5 -> V_164 == V_165 ) &&
( V_146 > V_162 ) ) {
if ( V_146 <= V_166 ) {
F_7 ( V_5 , V_161 , 0x2a ) ;
F_7 ( V_5 , V_167 ,
V_138 ) ;
} else if ( V_146 <= V_168 ) {
F_7 ( V_5 , V_161 , 0x2a ) ;
F_7 ( V_5 , V_169 ,
V_138 ) ;
}
}
} else {
V_144 = V_5 -> V_147 [ V_2 ] . V_144 ;
V_138 = F_34 ( V_23 ,
V_144 , V_5 -> V_43 ) ;
if ( V_144 == V_170 )
F_7 ( V_5 , V_161 , V_138 ) ;
else if ( V_144 == V_171 ) {
F_7 ( V_5 , V_161 , 0x2a ) ;
F_7 ( V_5 , V_163 , V_138 ) ;
} else if ( V_144 == V_172 ) {
F_7 ( V_5 , V_161 , 0x2a ) ;
F_7 ( V_5 , V_167 , V_138 ) ;
} else if ( V_144 == V_173 ) {
F_7 ( V_5 , V_161 , 0x2a ) ;
F_7 ( V_5 , V_169 , V_138 ) ;
}
}
if ( V_5 -> V_43 <= 0x11 ) {
if ( V_5 -> V_164 != V_165 )
F_35 ( V_5 , V_2 ) ;
}
F_7 ( V_5 , V_152 , 0x67 ) ;
break;
case V_174 :
default:
F_14 ( L_10 , V_64 ) ;
F_8 ( V_5 , V_150 , 1 ) ;
F_8 ( V_5 , V_151 , 1 ) ;
break;
}
V_139 = F_3 ( V_5 , V_11 ) ;
V_140 = F_3 ( V_5 , V_12 ) ;
V_70 = F_11 ( V_5 , V_175 ) ;
if ( V_5 -> V_176 [ V_2 ] == V_116 ) {
F_7 ( V_5 , V_177 , 0x00 ) ;
F_8 ( V_5 , V_178 , 0 ) ;
F_8 ( V_5 , V_88 , 0 ) ;
F_7 ( V_5 , V_179 , 0xc1 ) ;
F_15 ( V_5 , V_5 -> V_16 , V_23 , V_2 ) ;
V_143 = 1 ;
if ( V_5 -> V_147 [ V_2 ] . V_25 != V_155 )
F_31 ( V_5 , V_2 , V_23 ) ;
}
if ( V_5 -> V_43 >= 0x20 ) {
if ( ( V_5 -> V_27 [ V_2 ] == V_28 ) ||
( V_5 -> V_27 [ V_2 ] ==
V_29 ) ||
( V_5 -> V_27 [ V_2 ] ==
V_31 ) ) {
F_7 ( V_5 , V_180 , 0x0a ) ;
F_7 ( V_5 , V_181 , 0x0 ) ;
}
}
if ( V_5 -> V_43 < 0x20 )
F_7 ( V_5 , V_182 , 0x08 ) ;
if ( V_5 -> V_43 == 0x10 )
F_7 ( V_5 , V_183 , 0x0a ) ;
F_7 ( V_5 , V_91 , 0x38 ) ;
if ( ( V_5 -> V_43 >= 0x20 ) ||
( V_143 == 1 ) ||
( V_5 -> V_26 [ V_2 ] < 10000000 ) ) {
F_7 ( V_5 , V_40 , V_139 ) ;
F_7 ( V_5 , V_41 , V_140 ) ;
V_5 -> V_109 [ V_2 ] = F_20 ( V_23 ,
V_5 -> V_70 ) + 10000000 ;
if ( ( V_5 -> V_43 >= 0x20 ) || ( V_143 == 1 ) ) {
if ( V_5 -> V_176 [ V_2 ] != V_118 ) {
if ( V_5 -> V_110 [ V_2 ] == 3 )
F_23 ( V_5 ,
V_5 -> V_108 [ V_2 ] ,
V_5 -> V_109 [ V_2 ] ,
V_2 ) ;
else
F_36 ( V_94 ,
V_5 -> V_109 [ V_2 ] ) ;
}
}
if ( ( V_5 -> V_176 [ V_2 ] == V_116 ) ||
( V_5 -> V_26 [ V_2 ] < 10000000 ) )
F_12 ( 50 ) ;
else
F_12 ( 5 ) ;
F_27 ( & V_141 , & V_142 , V_23 ,
V_118 ) ;
if ( F_9 ( V_5 , V_2 , V_141 / 2 ) == FALSE ) {
F_7 ( V_5 , V_39 , 0x1f ) ;
F_7 ( V_5 , V_40 , V_139 ) ;
F_7 ( V_5 , V_41 , V_140 ) ;
F_7 ( V_5 , V_39 , 0x18 ) ;
V_81 = 0 ;
while ( ( F_9 ( V_5 ,
V_2 ,
V_141 / 2 ) == FALSE ) &&
( V_81 <= 2 ) ) {
F_7 ( V_5 , V_39 , 0x1f ) ;
F_7 ( V_5 , V_40 , V_139 ) ;
F_7 ( V_5 , V_41 , V_140 ) ;
F_7 ( V_5 , V_39 , 0x18 ) ;
V_81 ++ ;
}
}
}
if ( V_5 -> V_43 >= 0x20 )
F_7 ( V_5 , V_49 , 0x49 ) ;
if ( ( V_5 -> V_147 [ V_2 ] . V_25 == V_148 ) ||
( V_5 -> V_147 [ V_2 ] . V_25 == V_149 ) )
F_28 ( V_5 , V_2 ) ;
}
static int F_37 ( struct V_4 * V_5 ,
enum V_6 V_2 , T_1 V_184 )
{
T_1 V_185 = 0 , V_36 = 0 ;
enum V_186 V_187 ;
F_14 ( L_3 , V_64 ) ;
V_187 = F_11 ( V_5 , V_52 ) ;
while ( ( V_185 < V_184 ) && ( V_36 == 0 ) ) {
switch ( V_187 ) {
case V_188 :
case V_189 :
default:
V_36 = 0 ;
break;
case V_53 :
V_36 = F_11 ( V_5 , V_190 ) ;
break;
case V_191 :
V_36 = F_11 ( V_5 , V_192 ) ;
break;
}
if ( V_36 == 0 ) {
F_12 ( 10 ) ;
V_185 += 10 ;
}
}
if ( V_36 )
F_14 ( L_11 , V_64 ) ;
else
F_14 ( L_12 , V_64 ) ;
return V_36 ;
}
static int F_38 ( struct V_4 * V_5 ,
enum V_6 V_2 ,
T_1 V_193 , T_1 V_113 )
{
T_1 V_185 = 0 , V_36 = 0 ;
F_14 ( L_3 , V_64 ) ;
V_36 = F_9 ( V_5 , V_2 , V_193 ) ;
if ( V_36 )
V_36 = V_36 && F_37 ( V_5 , V_2 , V_113 ) ;
if ( V_36 ) {
V_36 = 0 ;
F_14 ( L_13 ,
V_64 , V_185 , V_113 ) ;
while ( ( V_185 < V_113 ) && ( V_36 == 0 ) ) {
V_36 = F_11 ( V_5 , V_194 ) ;
F_12 ( 1 ) ;
V_185 ++ ;
}
}
if ( V_36 )
F_14 ( L_14 , V_64 ) ;
else
F_14 ( L_15 , V_64 ) ;
if ( V_36 )
return TRUE ;
else
return FALSE ;
}
enum V_195 F_39 ( struct V_93 * V_94 ,
enum V_6 V_2 )
{
struct V_96 * V_97 = V_94 -> V_98 ;
struct V_4 * V_5 = V_97 -> V_99 ;
enum V_195 V_196 ;
int V_197 = F_11 ( V_5 , V_52 ) ;
switch ( V_197 ) {
case 2 :
V_196 = V_155 ;
break;
case 3 :
if ( F_11 ( V_5 , V_198 ) == 1 )
V_196 = V_149 ;
else
V_196 = V_148 ;
break;
default:
V_196 = V_174 ;
}
F_14 ( L_16 , V_64 , V_196 ) ;
return V_196 ;
}
static T_1 F_40 ( struct V_4 * V_5 , T_2 V_16 ,
enum V_6 V_2 )
{
T_1 V_199 ,
V_55 ,
V_56 ,
V_57 ,
V_58 ;
V_199 = ( F_11 ( V_5 , V_200 ) << 16 ) +
( F_11 ( V_5 , V_201 ) << 8 ) +
( F_11 ( V_5 , V_202 ) ) ;
V_199 = F_4 ( V_199 , 24 ) ;
V_57 = V_16 >> 12 ;
V_58 = V_199 >> 12 ;
V_55 = V_16 % 0x1000 ;
V_56 = V_199 % 0x1000 ;
V_199 = ( V_57 * V_58 ) +
( ( V_57 * V_56 ) >> 12 ) +
( ( V_58 * V_55 ) >> 12 ) ;
return V_199 ;
}
static T_2 F_41 ( struct V_93 * V_94 )
{
struct V_203 * V_204 = NULL ;
struct V_205 * V_206 = NULL ;
T_2 V_108 = 0 ;
if ( & V_94 -> V_207 )
V_204 = & V_94 -> V_207 ;
if ( & V_204 -> V_206 )
V_206 = & V_204 -> V_206 ;
if ( V_206 -> V_208 ) {
if ( ( V_206 -> V_208 ( V_94 , & V_108 ) ) < 0 )
F_14 ( L_17 , V_64 ) ;
else
F_14 ( L_18 , V_64 , V_108 ) ;
}
return V_108 ;
}
static enum
V_209 F_42 ( struct V_93 * V_94 )
{
struct V_96 * V_97 = V_94 -> V_98 ;
struct V_4 * V_5 = V_97 -> V_99 ;
enum V_6 V_2 = V_97 -> V_2 ;
enum V_209 V_210 = V_211 ;
struct V_212 * V_147 = & V_5 -> V_147 [ V_2 ] ;
T_1 V_213 ,
V_214 ;
int V_81 = 0 ,
V_100 = V_2 ;
T_3 V_215 ;
F_12 ( 5 ) ;
if ( V_5 -> V_176 [ V_100 ] == V_116 ) {
V_215 = F_3 ( V_5 , V_69 ) ;
V_81 = 0 ;
F_7 ( V_5 , V_177 , 0x5c ) ;
while ( ( V_81 <= 50 ) && ( V_215 != 0 ) && ( V_215 != 0xff ) ) {
V_215 = F_3 ( V_5 , V_69 ) ;
F_12 ( 5 ) ;
V_81 += 5 ;
}
}
V_147 -> V_25 = F_39 ( V_94 , V_100 ) ;
if ( V_5 -> V_110 [ V_2 ] == 3 )
V_147 -> V_216 = F_43 ( V_5 , V_100 ) ;
else
V_147 -> V_216 = F_41 ( V_94 ) ;
V_213 = F_40 ( V_5 , V_5 -> V_16 , V_100 ) / 1000 ;
V_147 -> V_216 += V_213 ;
V_147 -> V_26 = F_13 ( V_5 , V_5 -> V_16 , V_100 ) ;
V_214 = F_18 ( V_5 , V_147 -> V_26 , V_100 ) ;
V_147 -> V_26 += V_214 ;
V_147 -> V_122 = F_30 ( V_5 , V_100 ) ;
V_147 -> V_217 = F_11 ( V_5 , V_158 ) ;
V_147 -> V_218 = F_11 ( V_5 , V_159 ) & 0x01 ;
V_147 -> V_156 = ( ( T_2 ) F_11 ( V_5 , V_159 ) ) >> 1 ;
V_147 -> V_70 = F_11 ( V_5 , V_175 ) ;
F_14 ( L_19 , V_64 , V_147 -> V_217 ) ;
switch ( V_147 -> V_25 ) {
case V_155 :
V_147 -> V_219 = F_11 ( V_5 , V_220 ) ;
if ( V_147 -> V_217 <= V_160 )
V_147 -> V_144 = V_170 ;
else if ( V_147 -> V_217 <= V_162 )
V_147 -> V_144 = V_171 ;
else if ( V_147 -> V_217 <= V_166 )
V_147 -> V_144 = V_172 ;
else if ( V_147 -> V_217 <= V_168 )
V_147 -> V_144 = V_173 ;
else
V_147 -> V_144 = V_221 ;
break;
case V_148 :
case V_149 :
V_147 -> V_219 = F_11 ( V_5 , V_222 ) ;
V_147 -> V_144 = V_170 ;
break;
default:
break;
}
if ( ( V_5 -> V_176 [ V_100 ] == V_116 ) ||
( V_5 -> V_26 [ V_100 ] < 10000000 ) ) {
V_213 = V_147 -> V_216 - V_5 -> V_108 [ V_100 ] ;
if ( V_5 -> V_110 [ V_2 ] == 3 )
V_5 -> V_108 [ V_100 ] = F_43 ( V_5 , V_100 ) ;
else
V_5 -> V_108 [ V_100 ] = F_41 ( V_94 ) ;
if ( ABS ( V_213 ) <= ( ( V_5 -> V_15 [ V_100 ] / 2000 ) + 500 ) )
V_210 = V_223 ;
else if ( ABS ( V_213 ) <=
( F_20 ( V_147 -> V_26 ,
V_147 -> V_70 ) / 2000 ) )
V_210 = V_223 ;
} else if ( ABS ( V_213 ) <= ( ( V_5 -> V_15 [ V_100 ] / 2000 ) + 500 ) )
V_210 = V_223 ;
F_14 ( L_20 , V_64 , V_210 ) ;
return V_210 ;
}
static enum
V_209 F_44 ( struct V_93 * V_94 )
{
struct V_96 * V_97 = V_94 -> V_98 ;
struct V_4 * V_5 = V_97 -> V_99 ;
enum V_6 V_2 = V_97 -> V_2 ;
enum V_209 V_224 = V_225 ;
T_1 V_23 ,
V_95 ,
V_113 ,
V_139 ,
V_140 ;
V_5 -> V_147 [ V_2 ] . V_226 = FALSE ;
if ( F_11 ( V_5 , V_52 ) == V_191 ) {
V_23 = F_13 ( V_5 , V_5 -> V_16 , V_2 ) ;
V_23 += F_18 ( V_5 , V_23 , V_2 ) ;
if ( V_5 -> V_176 [ V_2 ] == V_116 )
F_15 ( V_5 , V_5 -> V_16 , V_23 , V_2 ) ;
F_27 ( & V_95 , & V_113 ,
V_23 , V_118 ) ;
V_139 = F_3 ( V_5 , V_11 ) ;
V_140 = F_3 ( V_5 , V_12 ) ;
F_8 ( V_5 , V_88 , 0 ) ;
F_8 ( V_5 , V_227 ,
V_228 ) ;
F_7 ( V_5 , V_39 , 0x1c ) ;
F_7 ( V_5 , V_40 , V_139 ) ;
F_7 ( V_5 , V_41 , V_140 ) ;
F_7 ( V_5 , V_39 , 0x18 ) ;
if ( F_38 ( V_5 , V_2 ,
V_95 , V_113 ) == TRUE ) {
V_5 -> V_147 [ V_2 ] . V_226 = TRUE ;
V_224 = F_42 ( V_94 ) ;
F_32 ( V_94 ) ;
} else {
F_8 ( V_5 , V_227 ,
V_229 ) ;
F_7 ( V_5 , V_39 , 0x1c ) ;
F_7 ( V_5 , V_40 , V_139 ) ;
F_7 ( V_5 , V_41 , V_140 ) ;
F_7 ( V_5 , V_39 , 0x18 ) ;
if ( F_38 ( V_5 , V_2 ,
V_95 , V_113 ) == TRUE ) {
V_5 -> V_147 [ V_2 ] . V_226 = TRUE ;
V_224 = F_42 ( V_94 ) ;
F_32 ( V_94 ) ;
}
}
} else
V_5 -> V_147 [ V_2 ] . V_226 = FALSE ;
return V_224 ;
}
static T_4 F_45 ( struct V_4 * V_5 ,
enum V_6 V_2 )
{
T_2 V_230 = 0xffff ,
V_231 ,
V_232 , V_233 ;
T_1 V_81 , V_234 , V_104 , V_106 ;
F_14 ( L_3 , V_64 ) ;
F_7 ( V_5 , V_91 , 0x38 ) ;
F_8 ( V_5 , V_178 , 0 ) ;
F_8 ( V_5 , V_88 , 0 ) ;
F_8 ( V_5 , V_235 , 1 ) ;
F_8 ( V_5 , V_236 , 1 ) ;
F_7 ( V_5 , V_237 , 0x0 ) ;
F_15 ( V_5 , V_5 -> V_16 , 1000000 , V_2 ) ;
V_104 = - 1 + ( V_5 -> V_15 [ V_2 ] / 1000000 ) ;
V_104 /= 2 ;
V_104 = ( 2 * V_104 ) + 1 ;
if ( V_104 < 0 )
V_104 = 1 ;
V_106 = 1 ;
V_233 = ( 1000000 << 8 ) / ( V_5 -> V_16 >> 8 ) ;
V_232 = 0 ;
for ( V_81 = 0 ; V_81 < V_104 ; V_81 ++ ) {
if ( V_106 > 0 )
V_232 = V_232 + ( V_233 * V_81 ) ;
else
V_232 = V_232 - ( V_233 * V_81 ) ;
V_106 *= - 1 ;
F_7 ( V_5 , V_39 , 0x5C ) ;
F_7 ( V_5 , V_40 , ( V_232 >> 8 ) & 0xff ) ;
F_7 ( V_5 , V_41 , V_232 & 0xff ) ;
F_7 ( V_5 , V_39 , 0x58 ) ;
F_12 ( 10 ) ;
V_231 = 0 ;
for ( V_234 = 0 ; V_234 < 10 ; V_234 ++ )
V_231 += ( F_3 ( V_5 , V_13 ) << 8 )
| F_3 ( V_5 , V_14 ) ;
V_231 /= 10 ;
if ( V_231 < V_230 )
V_230 = V_231 ;
}
return ( T_4 ) V_230 ;
}
static T_2 F_46 ( struct V_93 * V_94 )
{
struct V_96 * V_97 = V_94 -> V_98 ;
struct V_4 * V_5 = V_97 -> V_99 ;
enum V_6 V_2 = V_97 -> V_2 ;
int V_238 = FALSE ;
T_1 V_81 , V_82 = 0 ,
V_106 = 1 ,
V_104 ,
V_105 = 0 ,
V_107 ;
T_2 V_239 ,
V_240 = 0 ,
V_8 = 0 ,
V_103 = 1200 ;
if ( V_5 -> V_43 >= 0x30 )
V_239 = 0x2e00 ;
else
V_239 = 0x1f00 ;
F_8 ( V_5 , V_241 , 0x1f ) ;
F_7 ( V_5 , V_242 , 0x12 ) ;
F_7 ( V_5 , V_86 , 0xf0 ) ;
F_7 ( V_5 , V_87 , 0xe0 ) ;
F_8 ( V_5 , V_178 , 1 ) ;
F_8 ( V_5 , V_88 , 1 ) ;
F_7 ( V_5 , V_66 , 0x83 ) ;
F_7 ( V_5 , V_243 , 0xc0 ) ;
F_7 ( V_5 , V_67 , 0x82 ) ;
F_7 ( V_5 , V_244 , 0xa0 ) ;
F_7 ( V_5 , V_237 , 0x0 ) ;
F_7 ( V_5 , V_91 , 0x50 ) ;
if ( V_5 -> V_43 >= 0x30 ) {
F_7 ( V_5 , V_49 , 0x99 ) ;
F_7 ( V_5 , V_177 , 0x98 ) ;
} else if ( V_5 -> V_43 >= 0x20 ) {
F_7 ( V_5 , V_49 , 0x6a ) ;
F_7 ( V_5 , V_177 , 0x95 ) ;
} else {
F_7 ( V_5 , V_49 , 0xed ) ;
F_7 ( V_5 , V_177 , 0x73 ) ;
}
if ( V_5 -> V_26 [ V_2 ] <= 2000000 )
V_103 = 1000 ;
else if ( V_5 -> V_26 [ V_2 ] <= 5000000 )
V_103 = 2000 ;
else if ( V_5 -> V_26 [ V_2 ] <= 12000000 )
V_103 = 3000 ;
else
V_103 = 5000 ;
V_104 = - 1 + ( ( V_5 -> V_15 [ V_2 ] / 1000 ) / V_103 ) ;
V_104 /= 2 ;
V_104 = ( 2 * V_104 ) + 1 ;
if ( V_104 < 0 )
V_104 = 1 ;
else if ( V_104 > 10 ) {
V_104 = 11 ;
V_103 = ( V_5 -> V_15 [ V_2 ] / 1000 ) / 10 ;
}
V_105 = 0 ;
V_106 = 1 ;
V_107 = V_5 -> V_108 [ V_2 ] ;
while ( ( V_238 == FALSE ) && ( V_105 < V_104 ) ) {
F_7 ( V_5 , V_39 , 0x5f ) ;
F_8 ( V_5 , V_241 , 0 ) ;
F_12 ( 50 ) ;
for ( V_81 = 0 ; V_81 < 10 ; V_81 ++ ) {
if ( F_11 ( V_5 , V_92 ) >= 2 )
V_82 ++ ;
V_8 += ( F_3 ( V_5 , V_13 ) << 8 ) |
F_3 ( V_5 , V_14 ) ;
}
V_8 /= 10 ;
V_240 = F_13 ( V_5 , V_5 -> V_16 , V_2 ) ;
V_105 ++ ;
V_106 *= - 1 ;
F_14 ( L_21
L_22 ,
V_107 , V_8 , V_240 , V_82 ) ;
if ( ( V_82 >= 5 ) &&
( V_8 < V_239 ) &&
( V_240 < 55000000 ) &&
( V_240 > 850000 ) )
V_238 = TRUE ;
else if ( V_105 < V_104 ) {
if ( V_106 > 0 )
V_107 += ( V_105 * V_103 ) ;
else
V_107 -= ( V_105 * V_103 ) ;
if ( V_5 -> V_110 [ V_2 ] == 3 )
F_23 ( V_5 , V_107 ,
V_5 -> V_109 [ V_2 ] , V_2 ) ;
else
F_24 ( V_94 , V_107 ,
V_5 -> V_109 [ V_2 ] ) ;
}
}
if ( V_238 == FALSE )
V_240 = 0 ;
else
V_240 = F_13 ( V_5 , V_5 -> V_16 , V_2 ) ;
return V_240 ;
}
static T_2 F_47 ( struct V_93 * V_94 )
{
struct V_96 * V_97 = V_94 -> V_98 ;
struct V_4 * V_5 = V_97 -> V_99 ;
enum V_6 V_2 = V_97 -> V_2 ;
T_2 V_240 ,
V_245 ,
V_63 ,
V_246 ,
V_247 ,
V_248 ;
V_240 = F_13 ( V_5 , V_5 -> V_16 , V_2 ) ;
if ( V_240 > 3000000 ) {
V_246 = 13 * ( V_240 / 10 ) ;
V_246 = ( V_246 / 1000 ) * 65536 ;
V_246 /= ( V_5 -> V_16 / 1000 ) ;
V_247 = 10 * ( V_240 / 13 ) ;
V_247 = ( V_247 / 1000 ) * 65536 ;
V_247 /= ( V_5 -> V_16 / 1000 ) ;
V_63 = ( V_240 / 1000 ) * 65536 ;
V_63 /= ( V_5 -> V_16 / 1000 ) ;
} else {
V_246 = 13 * ( V_240 / 10 ) ;
V_246 = ( V_246 / 100 ) * 65536 ;
V_246 /= ( V_5 -> V_16 / 100 ) ;
V_247 = 10 * ( V_240 / 14 ) ;
V_247 = ( V_247 / 100 ) * 65536 ;
V_247 /= ( V_5 -> V_16 / 100 ) ;
V_63 = ( V_240 / 100 ) * 65536 ;
V_63 /= ( V_5 -> V_16 / 100 ) ;
}
V_248 = 13 * ( V_240 / 10 ) ;
V_245 = ( F_3 ( V_5 , V_11 ) << 8 )
| F_3 ( V_5 , V_12 ) ;
if ( V_248 < V_5 -> V_26 [ V_2 ] )
V_240 = 0 ;
else {
F_7 ( V_5 , V_39 , 0x1f ) ;
F_7 ( V_5 , V_179 , 0xc1 ) ;
F_7 ( V_5 , V_86 , 0x20 ) ;
F_7 ( V_5 , V_87 , 0x00 ) ;
F_7 ( V_5 , V_242 , 0xd2 ) ;
F_8 ( V_5 , V_88 , 0 ) ;
F_7 ( V_5 , V_91 , 0x38 ) ;
if ( V_5 -> V_43 >= 0x30 )
F_7 ( V_5 , V_49 , 0x79 ) ;
else if ( V_5 -> V_43 >= 0x20 )
F_7 ( V_5 , V_49 , 0x49 ) ;
else
F_7 ( V_5 , V_49 , 0xed ) ;
F_7 ( V_5 , V_66 , ( V_246 >> 8 ) & 0x7f ) ;
F_7 ( V_5 , V_243 , ( V_246 & 0xff ) ) ;
F_7 ( V_5 , V_67 , ( V_247 >> 8 ) & 0x7f ) ;
F_7 ( V_5 , V_244 , ( V_247 & 0xff ) ) ;
F_7 ( V_5 , V_65 , ( V_63 >> 8 ) & 0xff ) ;
F_7 ( V_5 , V_249 , ( V_63 & 0xff ) ) ;
F_7 ( V_5 , V_237 , 0x20 ) ;
F_7 ( V_5 , V_40 , ( V_245 >> 8 ) & 0xff ) ;
F_7 ( V_5 , V_41 , V_245 & 0xff ) ;
F_7 ( V_5 , V_39 , 0x15 ) ;
}
return V_240 ;
}
static int F_48 ( struct V_93 * V_94 )
{
struct V_96 * V_97 = V_94 -> V_98 ;
struct V_4 * V_5 = V_97 -> V_99 ;
enum V_6 V_2 = V_97 -> V_2 ;
T_3 V_250 ,
V_251 ,
V_252 ;
T_2 V_240 ,
V_239 ;
int V_36 = FALSE ,
V_253 = FALSE ;
T_1 V_95 = 500 ,
V_113 = 50 ,
V_254 ,
V_81 ,
V_255 ;
T_4 V_256 ;
T_3 V_257 ;
F_14 ( L_3 , V_64 ) ;
if ( V_5 -> V_43 < 0x20 ) {
V_251 = 233 ;
V_252 = 143 ;
} else {
V_251 = 110 ;
V_252 = 10 ;
}
if ( V_5 -> V_43 <= 0x20 )
V_239 = V_258 ;
else
V_239 = V_259 ;
V_256 = F_45 ( V_5 , V_2 ) ;
F_14 ( L_23 , V_64 , V_256 , V_239 ) ;
if ( V_256 > V_239 )
return FALSE ;
if ( V_5 -> V_43 == 0x10 )
F_7 ( V_5 , V_183 , 0xaa ) ;
if ( V_5 -> V_43 < 0x20 )
F_7 ( V_5 , V_182 , 0x55 ) ;
else
F_7 ( V_5 , V_182 , 0x20 ) ;
if ( V_5 -> V_43 <= 0x20 )
F_7 ( V_5 , V_260 , 0xc4 ) ;
else
F_7 ( V_5 , V_260 , 0x6 ) ;
F_7 ( V_5 , V_90 , 0x44 ) ;
if ( V_5 -> V_43 >= 0x20 ) {
F_7 ( V_5 , V_261 , 0x41 ) ;
F_7 ( V_5 , V_262 , 0x41 ) ;
F_7 ( V_5 , V_181 , 0x82 ) ;
F_7 ( V_5 , V_180 , 0x0 ) ;
}
V_250 = V_251 ;
do {
F_7 ( V_5 , V_263 , V_250 ) ;
if ( F_46 ( V_94 ) != 0 ) {
V_240 = F_47 ( V_94 ) ;
if ( V_240 != 0 ) {
F_27 ( & V_95 ,
& V_113 ,
V_240 ,
V_116 ) ;
V_36 = F_9 ( V_5 ,
V_2 ,
V_95 ) ;
} else
V_36 = FALSE ;
} else {
V_254 = 0 ;
V_255 = 0 ;
for ( V_81 = 0 ; V_81 < 10 ; V_81 ++ ) {
V_256 = ( F_3 ( V_5 , V_13 ) << 8 )
| F_3 ( V_5 , V_14 ) ;
if ( V_256 >= 0xff00 )
V_255 ++ ;
V_257 = F_3 ( V_5 , V_264 ) ;
if ( ( ( V_257 & 0x1 ) == 0x1 ) &&
( ( V_257 >> 7 ) == 1 ) )
V_254 ++ ;
}
if ( ( V_254 > 7 ) || ( V_255 > 7 ) )
V_253 = TRUE ;
V_36 = FALSE ;
}
V_250 -= 30 ;
} while ( ( V_250 >= V_252 ) &&
( V_36 == FALSE ) &&
( V_253 == FALSE ) );
return V_36 ;
}
static void F_49 ( struct V_4 * V_5 ,
enum V_6 V_2 )
{
T_1 V_119 = V_120 ;
F_14 ( L_3 , V_64 ) ;
F_7 ( V_5 , V_119 ++ , 0x96 ) ;
F_7 ( V_5 , V_119 ++ , 0x64 ) ;
F_7 ( V_5 , V_119 ++ , 0x36 ) ;
F_7 ( V_5 , V_119 ++ , 0x23 ) ;
F_7 ( V_5 , V_119 ++ , 0x1e ) ;
F_7 ( V_5 , V_119 ++ , 0x19 ) ;
}
static void F_50 ( struct V_4 * V_5 ,
enum V_6 V_2 )
{
F_14 ( L_3 , V_64 ) ;
switch ( V_5 -> V_27 [ V_2 ] ) {
case V_28 :
F_14 ( L_24 ) ;
break;
case V_29 :
F_14 ( L_25 ) ;
case V_30 :
break;
F_14 ( L_26 ) ;
case V_31 :
default:
F_14 ( L_27 ) ;
break;
}
switch ( V_5 -> V_27 [ V_2 ] ) {
case V_28 :
case V_29 :
F_8 ( V_5 , V_150 , 1 ) ;
F_8 ( V_5 , V_151 , 0 ) ;
F_8 ( V_5 , V_265 , 0 ) ;
F_31 ( V_5 ,
V_2 ,
V_5 -> V_26 [ V_2 ] ) ;
F_7 ( V_5 , V_266 , 0x22 ) ;
F_49 ( V_5 , V_2 ) ;
F_29 ( V_5 ,
V_5 -> V_27 [ V_2 ] ,
V_5 -> V_122 [ V_2 ] , V_2 ) ;
break;
case V_30 :
F_8 ( V_5 , V_150 , 0 ) ;
F_8 ( V_5 , V_151 , 1 ) ;
F_8 ( V_5 , V_265 , 1 ) ;
F_7 ( V_5 , V_135 , 0x1a ) ;
F_7 ( V_5 , V_136 , 0x09 ) ;
if ( V_5 -> V_43 <= 0x20 )
F_7 ( V_5 , V_266 , 0x26 ) ;
else
F_7 ( V_5 , V_266 , 0x66 ) ;
if ( V_5 -> V_164 != V_165 ) {
if ( V_5 -> V_43 <= 0x11 )
F_51 ( V_5 , V_2 ) ;
else
F_35 ( V_5 , V_2 ) ;
} else
F_52 ( V_5 , V_2 ) ;
F_28 ( V_5 , V_2 ) ;
break;
case V_31 :
default:
F_8 ( V_5 , V_150 , 1 ) ;
F_8 ( V_5 , V_151 , 1 ) ;
F_8 ( V_5 , V_265 , 0 ) ;
F_7 ( V_5 , V_135 , 0x1a ) ;
F_7 ( V_5 , V_136 , 0x09 ) ;
F_31 ( V_5 ,
V_2 ,
V_5 -> V_26 [ V_2 ] ) ;
if ( V_5 -> V_43 <= 0x20 )
F_7 ( V_5 , V_266 , 0x26 ) ;
else
F_7 ( V_5 , V_266 , 0x66 ) ;
if ( V_5 -> V_164 != V_165 ) {
if ( V_5 -> V_43 <= 0x11 )
F_51 ( V_5 , V_2 ) ;
else
F_35 ( V_5 , V_2 ) ;
} else
F_52 ( V_5 , V_2 ) ;
F_28 ( V_5 , V_2 ) ;
F_29 ( V_5 ,
V_5 -> V_27 [ V_2 ] ,
V_5 -> V_122 [ V_2 ] , V_2 ) ;
break;
}
}
enum V_209 F_53 ( struct V_93 * V_94 )
{
struct V_96 * V_97 = V_94 -> V_98 ;
struct V_4 * V_5 = V_97 -> V_99 ;
enum V_6 V_2 = V_97 -> V_2 ;
T_1 V_95 = 500 , V_113 = 50 ;
T_1 V_267 , V_268 , V_81 ;
int V_36 = FALSE , V_269 = FALSE ;
enum V_209 V_224 = V_270 ;
enum V_114 V_115 ;
int V_10 = FALSE ;
F_14 ( L_3 , V_64 ) ;
V_115 = V_5 -> V_176 [ V_2 ] ;
F_8 ( V_5 , V_44 , 1 ) ;
F_7 ( V_5 , V_39 , 0x5c ) ;
if ( V_5 -> V_43 >= 0x20 ) {
if ( V_5 -> V_26 [ V_2 ] > 5000000 )
F_7 ( V_5 , V_51 , 0x9e ) ;
else
F_7 ( V_5 , V_51 , 0x82 ) ;
} else
F_7 ( V_5 , V_51 , 0x88 ) ;
F_27 ( & V_95 , & V_113 ,
V_5 -> V_26 [ V_2 ] ,
V_5 -> V_176 [ V_2 ] ) ;
if ( V_5 -> V_176 [ V_2 ] == V_116 ) {
V_5 -> V_109 [ V_2 ] = 2 * 36000000 ;
F_7 ( V_5 , V_179 , 0xc0 ) ;
F_7 ( V_5 , V_271 , 0x70 ) ;
F_15 ( V_5 , V_5 -> V_16 , 1000000 , V_2 ) ;
} else {
F_7 ( V_5 , V_237 , 0x20 ) ;
F_7 ( V_5 , V_242 , 0xd2 ) ;
if ( V_5 -> V_26 [ V_2 ] < 2000000 )
F_7 ( V_5 , V_271 , 0x63 ) ;
else
F_7 ( V_5 , V_271 , 0x70 ) ;
F_7 ( V_5 , V_91 , 0x38 ) ;
V_5 -> V_109 [ V_2 ] =
F_20 ( V_5 -> V_26 [ V_2 ] ,
V_5 -> V_70 ) ;
if ( V_5 -> V_43 >= 0x20 ) {
F_7 ( V_5 , V_263 , 0x5a ) ;
if ( V_5 -> V_176 [ V_2 ] == V_117 ) {
V_5 -> V_109 [ V_2 ] += 10000000 ;
V_5 -> V_109 [ V_2 ] *= 15 ;
V_5 -> V_109 [ V_2 ] /= 10 ;
} else if ( V_5 -> V_176 [ V_2 ] == V_118 )
V_5 -> V_109 [ V_2 ] += 10000000 ;
} else {
F_7 ( V_5 , V_263 , 0xc1 ) ;
V_5 -> V_109 [ V_2 ] += 10000000 ;
V_5 -> V_109 [ V_2 ] *= 15 ;
V_5 -> V_109 [ V_2 ] /= 10 ;
}
F_7 ( V_5 , V_179 , 0xc1 ) ;
F_15 ( V_5 , V_5 -> V_16 ,
V_5 -> V_26 [ V_2 ] , V_2 ) ;
F_16 ( V_5 , V_5 -> V_16 ,
V_5 -> V_26 [ V_2 ] , V_2 ) ;
F_17 ( V_5 , V_5 -> V_16 ,
V_5 -> V_26 [ V_2 ] , V_2 ) ;
if ( V_5 -> V_26 [ V_2 ] >= 10000000 )
V_269 = FALSE ;
else
V_269 = TRUE ;
}
if ( V_5 -> V_110 [ V_2 ] == 3 )
F_23 ( V_5 , V_5 -> V_108 [ V_2 ] ,
V_5 -> V_109 [ V_2 ] , V_2 ) ;
else
F_24 ( V_94 , V_5 -> V_108 [ V_2 ] , V_5 -> V_109 [ V_2 ] ) ;
V_268 = F_54 ( F_11 ( V_5 , V_272 ) ,
F_11 ( V_5 , V_273 ) ) ;
V_267 = 0 ;
if ( V_268 == 0 ) {
for ( V_81 = 0 ; V_81 < 5 ; V_81 ++ )
V_267 += ( F_11 ( V_5 , V_274 ) +
F_11 ( V_5 , V_275 ) ) / 2 ;
V_267 /= 5 ;
}
if ( ( V_268 == 0 ) && ( V_267 < V_276 ) ) {
V_5 -> V_147 [ V_2 ] . V_226 = FALSE ;
V_224 = V_277 ;
F_14 ( L_28 , V_64 ) ;
} else {
F_8 ( V_5 , V_227 ,
V_5 -> V_278 [ V_2 ] ) ;
if ( V_5 -> V_43 <= 0x20 )
F_8 ( V_5 , V_71 , 1 ) ;
else
F_8 ( V_5 , V_74 , 1 ) ;
F_50 ( V_5 , V_2 ) ;
if ( V_5 -> V_176 [ V_2 ] != V_116 )
F_55 ( V_5 , V_2 ) ;
}
if ( V_224 == V_277 )
return V_224 ;
if ( V_5 -> V_43 == 0x12 ) {
F_8 ( V_5 , V_44 , 0 ) ;
F_12 ( 3 ) ;
F_8 ( V_5 , V_44 , 1 ) ;
F_8 ( V_5 , V_44 , 0 ) ;
}
if ( V_115 == V_116 )
V_36 = F_48 ( V_94 ) ;
else if ( V_115 == V_117 )
V_36 = F_22 ( V_94 , V_95 ) ;
else if ( V_115 == V_118 )
V_36 = F_9 ( V_5 , V_2 , V_95 ) ;
if ( ( V_36 == FALSE ) && ( V_115 == V_117 ) ) {
if ( V_269 == FALSE ) {
if ( F_21 ( V_5 , V_2 ) == TRUE )
V_36 = F_10 ( V_5 , V_2 ) ;
}
}
if ( V_36 == TRUE )
V_224 = F_42 ( V_94 ) ;
if ( ( V_36 == TRUE ) && ( V_224 == V_223 ) ) {
F_32 ( V_94 ) ;
if ( V_5 -> V_43 <= 0x11 ) {
if ( ( F_39 ( V_94 , 0 ) ==
V_148 ) &&
( F_39 ( V_94 , 1 ) ==
V_148 ) ) {
F_12 ( 20 ) ;
F_8 ( V_5 , V_44 , 0 ) ;
} else {
F_8 ( V_5 , V_44 , 0 ) ;
F_12 ( 3 ) ;
F_8 ( V_5 , V_44 , 1 ) ;
F_8 ( V_5 , V_44 , 0 ) ;
}
} else if ( V_5 -> V_43 >= 0x20 ) {
F_8 ( V_5 , V_44 , 0 ) ;
F_12 ( 3 ) ;
F_8 ( V_5 , V_44 , 1 ) ;
F_8 ( V_5 , V_44 , 0 ) ;
}
if ( F_38 ( V_5 , V_2 ,
V_113 , V_113 ) == TRUE ) {
V_36 = TRUE ;
V_5 -> V_147 [ V_2 ] . V_226 = TRUE ;
if ( V_5 -> V_147 [ V_2 ] . V_25 ==
V_155 ) {
F_19 ( V_5 , V_2 ) ;
F_8 ( V_5 , V_279 , 1 ) ;
F_8 ( V_5 , V_279 , 0 ) ;
F_7 ( V_5 , V_152 , 0x67 ) ;
} else {
F_7 ( V_5 , V_152 , 0x75 ) ;
}
F_7 ( V_5 , V_280 , 0 ) ;
F_7 ( V_5 , V_281 , 0xc1 ) ;
} else {
V_36 = FALSE ;
V_224 = V_225 ;
V_10 = F_2 ( V_5 , V_2 ) ;
V_5 -> V_147 [ V_2 ] . V_226 = FALSE ;
}
}
if ( ( V_224 != V_225 ) || ( V_10 != FALSE ) )
return V_224 ;
if ( V_5 -> V_43 > 0x11 ) {
V_5 -> V_147 [ V_2 ] . V_226 = FALSE ;
return V_224 ;
}
if ( ( F_11 ( V_5 , V_52 ) == V_191 ) &&
( V_5 -> V_278 [ V_2 ] <= V_282 ) )
V_224 = F_44 ( V_94 ) ;
return V_224 ;
}
