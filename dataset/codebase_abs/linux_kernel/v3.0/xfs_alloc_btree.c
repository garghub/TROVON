STATIC struct V_1 *
F_1 (
struct V_1 * V_2 )
{
return F_2 ( V_2 -> V_3 , V_2 -> V_4 ,
V_2 -> V_5 . V_6 . V_7 , V_2 -> V_5 . V_6 . V_8 ,
V_2 -> V_9 ) ;
}
STATIC void
F_3 (
struct V_1 * V_2 ,
union V_10 * V_11 ,
int V_12 )
{
struct V_13 * V_7 = V_2 -> V_5 . V_6 . V_7 ;
struct V_14 * V_15 = F_4 ( V_7 ) ;
T_1 V_16 = F_5 ( V_15 -> V_17 ) ;
int V_18 = V_2 -> V_9 ;
struct V_19 * V_20 = F_6 ( V_2 -> V_3 , V_16 ) ;
ASSERT ( V_11 -> V_21 != 0 ) ;
V_15 -> V_22 [ V_18 ] = V_11 -> V_21 ;
F_7 ( & V_15 -> V_23 [ V_18 ] , V_12 ) ;
V_20 -> V_24 [ V_18 ] += V_12 ;
F_8 ( V_20 ) ;
F_9 ( V_2 -> V_4 , V_7 , V_25 | V_26 ) ;
}
STATIC int
F_10 (
struct V_1 * V_2 ,
union V_10 * V_27 ,
union V_10 * V_28 ,
int V_29 ,
int * V_30 )
{
int error ;
T_2 V_31 ;
F_11 ( V_2 , V_32 ) ;
error = F_12 ( V_2 -> V_4 , V_2 -> V_5 . V_6 . V_7 ,
& V_31 , 1 ) ;
if ( error ) {
F_11 ( V_2 , V_33 ) ;
return error ;
}
if ( V_31 == V_34 ) {
F_11 ( V_2 , V_35 ) ;
* V_30 = 0 ;
return 0 ;
}
F_13 ( V_2 -> V_3 , V_2 -> V_5 . V_6 . V_8 , V_31 , 1 , false ) ;
F_14 ( V_2 -> V_4 , 1 ) ;
V_28 -> V_21 = F_15 ( V_31 ) ;
F_11 ( V_2 , V_35 ) ;
* V_30 = 1 ;
return 0 ;
}
STATIC int
F_16 (
struct V_1 * V_2 ,
struct V_13 * V_36 )
{
struct V_13 * V_7 = V_2 -> V_5 . V_6 . V_7 ;
struct V_14 * V_15 = F_4 ( V_7 ) ;
T_2 V_31 ;
int error ;
V_31 = F_17 ( V_2 -> V_3 , F_18 ( V_36 ) ) ;
error = F_19 ( V_2 -> V_4 , V_7 , NULL , V_31 , 1 ) ;
if ( error )
return error ;
F_20 ( V_2 -> V_4 , F_5 ( V_15 -> V_17 ) , V_31 , 1 ,
V_37 ) ;
F_14 ( V_2 -> V_4 , - 1 ) ;
return 0 ;
}
STATIC void
F_21 (
struct V_1 * V_2 ,
struct V_38 * V_39 ,
union V_40 * V_41 ,
int V_11 ,
int V_42 )
{
struct V_14 * V_15 = F_4 ( V_2 -> V_5 . V_6 . V_7 ) ;
T_1 V_16 = F_5 ( V_15 -> V_17 ) ;
struct V_19 * V_20 ;
T_3 V_43 ;
int V_44 ;
ASSERT ( V_2 -> V_9 == V_45 ) ;
switch ( V_42 ) {
case V_46 :
if ( V_11 != F_22 ( V_39 ) )
return;
V_43 = V_41 -> V_47 . V_48 ;
break;
case V_49 :
if ( F_5 ( V_41 -> V_47 . V_48 ) <=
F_5 ( V_15 -> V_50 ) )
return;
V_43 = V_41 -> V_47 . V_48 ;
break;
case V_51 :
V_44 = F_22 ( V_39 ) ;
if ( V_11 <= V_44 )
return;
ASSERT ( V_11 == V_44 + 1 ) ;
if ( V_44 ) {
T_4 * V_52 ;
V_52 = F_23 ( V_2 -> V_3 , V_39 , V_44 ) ;
V_43 = V_52 -> V_48 ;
} else {
V_43 = 0 ;
}
break;
default:
ASSERT ( 0 ) ;
return;
}
V_15 -> V_50 = V_43 ;
V_20 = F_6 ( V_2 -> V_3 , V_16 ) ;
V_20 -> V_53 = F_5 ( V_43 ) ;
F_8 ( V_20 ) ;
F_9 ( V_2 -> V_4 , V_2 -> V_5 . V_6 . V_7 , V_54 ) ;
}
STATIC int
F_24 (
struct V_1 * V_2 ,
int V_55 )
{
return V_2 -> V_3 -> V_56 [ V_55 != 0 ] ;
}
STATIC int
F_25 (
struct V_1 * V_2 ,
int V_55 )
{
return V_2 -> V_3 -> V_57 [ V_55 != 0 ] ;
}
STATIC void
F_26 (
union V_58 * V_59 ,
union V_40 * V_41 )
{
ASSERT ( V_41 -> V_47 . V_60 != 0 ) ;
V_59 -> V_47 . V_60 = V_41 -> V_47 . V_60 ;
V_59 -> V_47 . V_48 = V_41 -> V_47 . V_48 ;
}
STATIC void
F_27 (
union V_58 * V_59 ,
union V_40 * V_41 )
{
ASSERT ( V_59 -> V_47 . V_60 != 0 ) ;
V_41 -> V_47 . V_60 = V_59 -> V_47 . V_60 ;
V_41 -> V_47 . V_48 = V_59 -> V_47 . V_48 ;
}
STATIC void
F_28 (
struct V_1 * V_2 ,
union V_40 * V_41 )
{
ASSERT ( V_2 -> V_61 . V_6 . V_60 != 0 ) ;
V_41 -> V_47 . V_60 = F_15 ( V_2 -> V_61 . V_6 . V_60 ) ;
V_41 -> V_47 . V_48 = F_15 ( V_2 -> V_61 . V_6 . V_48 ) ;
}
STATIC void
F_29 (
struct V_1 * V_2 ,
union V_10 * V_11 )
{
struct V_14 * V_15 = F_4 ( V_2 -> V_5 . V_6 . V_7 ) ;
ASSERT ( V_2 -> V_5 . V_6 . V_8 == F_5 ( V_15 -> V_17 ) ) ;
ASSERT ( V_15 -> V_22 [ V_2 -> V_9 ] != 0 ) ;
V_11 -> V_21 = V_15 -> V_22 [ V_2 -> V_9 ] ;
}
STATIC T_5
F_30 (
struct V_1 * V_2 ,
union V_58 * V_59 )
{
T_6 * V_41 = & V_2 -> V_61 . V_6 ;
T_7 * V_62 = & V_59 -> V_47 ;
T_5 V_63 ;
if ( V_2 -> V_9 == V_64 ) {
return ( T_5 ) F_5 ( V_62 -> V_60 ) -
V_41 -> V_60 ;
}
V_63 = ( T_5 ) F_5 ( V_62 -> V_48 ) - V_41 -> V_48 ;
if ( V_63 )
return V_63 ;
return ( T_5 ) F_5 ( V_62 -> V_60 ) - V_41 -> V_60 ;
}
STATIC int
F_31 (
struct V_1 * V_2 ,
union V_58 * V_65 ,
union V_58 * V_66 )
{
if ( V_2 -> V_9 == V_64 ) {
return F_5 ( V_65 -> V_47 . V_60 ) <
F_5 ( V_66 -> V_47 . V_60 ) ;
} else {
return F_5 ( V_65 -> V_47 . V_48 ) <
F_5 ( V_66 -> V_47 . V_48 ) ||
( V_65 -> V_47 . V_48 == V_66 -> V_47 . V_48 &&
F_5 ( V_65 -> V_47 . V_60 ) <
F_5 ( V_66 -> V_47 . V_60 ) ) ;
}
}
STATIC int
F_32 (
struct V_1 * V_2 ,
union V_40 * V_67 ,
union V_40 * V_68 )
{
if ( V_2 -> V_9 == V_64 ) {
return F_5 ( V_67 -> V_47 . V_60 ) +
F_5 ( V_67 -> V_47 . V_48 ) <=
F_5 ( V_68 -> V_47 . V_60 ) ;
} else {
return F_5 ( V_67 -> V_47 . V_48 ) <
F_5 ( V_68 -> V_47 . V_48 ) ||
( V_67 -> V_47 . V_48 == V_68 -> V_47 . V_48 &&
F_5 ( V_67 -> V_47 . V_60 ) <
F_5 ( V_68 -> V_47 . V_60 ) ) ;
}
}
STATIC void
F_33 (
struct V_1 * V_2 ,
const char * V_69 ,
char * V_21 ,
int type ,
int line ,
T_8 V_70 ,
T_8 V_71 ,
T_8 V_72 ,
T_8 V_73 ,
T_8 V_74 ,
T_8 V_75 ,
T_8 V_76 ,
T_8 V_77 ,
T_8 V_78 ,
T_8 V_79 ,
T_8 V_80 )
{
F_34 ( V_81 , ( void * ) ( V_82 ) type ,
( void * ) V_69 , ( void * ) V_21 , NULL , ( void * ) V_2 ,
( void * ) V_70 , ( void * ) V_71 , ( void * ) V_72 , ( void * ) V_73 ,
( void * ) V_74 , ( void * ) V_75 , ( void * ) V_76 , ( void * ) V_77 ,
( void * ) V_78 , ( void * ) V_79 , ( void * ) V_80 ) ;
}
STATIC void
F_35 (
struct V_1 * V_2 ,
T_9 * V_83 ,
T_10 * V_84 ,
T_10 * V_85 )
{
* V_83 = V_2 -> V_5 . V_6 . V_8 ;
* V_84 = V_2 -> V_61 . V_6 . V_60 ;
* V_85 = V_2 -> V_61 . V_6 . V_48 ;
}
STATIC void
F_36 (
struct V_1 * V_2 ,
union V_58 * V_59 ,
T_10 * V_84 ,
T_10 * V_85 )
{
* V_84 = F_5 ( V_59 -> V_47 . V_60 ) ;
* V_85 = F_5 ( V_59 -> V_47 . V_48 ) ;
}
STATIC void
F_37 (
struct V_1 * V_2 ,
union V_40 * V_41 ,
T_10 * V_84 ,
T_10 * V_85 ,
T_10 * V_86 )
{
* V_84 = F_5 ( V_41 -> V_47 . V_60 ) ;
* V_85 = F_5 ( V_41 -> V_47 . V_48 ) ;
* V_86 = 0 ;
}
struct V_1 *
F_2 (
struct V_87 * V_88 ,
struct V_89 * V_90 ,
struct V_13 * V_7 ,
T_1 V_8 ,
T_11 V_18 )
{
struct V_14 * V_15 = F_4 ( V_7 ) ;
struct V_1 * V_2 ;
ASSERT ( V_18 == V_64 || V_18 == V_45 ) ;
V_2 = F_38 ( V_91 , V_92 ) ;
V_2 -> V_4 = V_90 ;
V_2 -> V_3 = V_88 ;
V_2 -> V_93 = F_5 ( V_15 -> V_23 [ V_18 ] ) ;
V_2 -> V_9 = V_18 ;
V_2 -> V_94 = V_88 -> V_95 . V_96 ;
V_2 -> V_97 = & V_98 ;
if ( V_18 == V_45 )
V_2 -> V_99 = V_100 ;
V_2 -> V_5 . V_6 . V_7 = V_7 ;
V_2 -> V_5 . V_6 . V_8 = V_8 ;
return V_2 ;
}
int
F_39 (
struct V_87 * V_88 ,
int V_101 ,
int V_102 )
{
V_101 -= F_40 ( V_88 ) ;
if ( V_102 )
return V_101 / sizeof( T_4 ) ;
return V_101 / ( sizeof( T_7 ) + sizeof( V_103 ) ) ;
}
