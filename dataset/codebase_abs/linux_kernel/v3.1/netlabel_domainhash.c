static void F_1 ( struct V_1 * V_2 )
{
struct V_3 * V_4 ;
struct V_5 * V_6 ;
struct V_5 * V_7 ;
#if F_2 ( V_8 ) || F_2 ( V_9 )
struct V_10 * V_11 ;
struct V_10 * V_12 ;
#endif
V_4 = F_3 ( V_2 , struct V_3 , V_13 ) ;
if ( V_4 -> type == V_14 ) {
F_4 (iter4, tmp4,
&ptr->type_def.addrsel->list4) {
F_5 ( V_6 ) ;
F_6 ( F_7 ( V_6 ) ) ;
}
#if F_2 ( V_8 ) || F_2 ( V_9 )
F_8 (iter6, tmp6,
&ptr->type_def.addrsel->list6) {
F_9 ( V_11 ) ;
F_6 ( F_10 ( V_11 ) ) ;
}
#endif
}
F_6 ( V_4 -> V_15 ) ;
F_6 ( V_4 ) ;
}
static T_1 F_11 ( const char * V_16 )
{
T_1 V_17 ;
T_1 V_18 ;
T_1 V_19 ;
for ( V_17 = 0 , V_18 = 0 , V_19 = strlen ( V_16 ) ; V_17 < V_19 ; V_17 ++ )
V_18 = ( V_18 << 4 | ( V_18 >> ( 8 * sizeof( T_1 ) - 4 ) ) ) ^ V_16 [ V_17 ] ;
return V_18 & ( F_12 ( V_20 ) -> V_21 - 1 ) ;
}
static struct V_3 * F_13 ( const char * V_15 )
{
T_1 V_22 ;
struct V_23 * V_24 ;
struct V_3 * V_17 ;
if ( V_15 != NULL ) {
V_22 = F_11 ( V_15 ) ;
V_24 = & F_12 ( V_20 ) -> V_25 [ V_22 ] ;
F_14 (iter, bkt_list, list)
if ( V_17 -> V_26 && strcmp ( V_17 -> V_15 , V_15 ) == 0 )
return V_17 ;
}
return NULL ;
}
static struct V_3 * F_15 ( const char * V_15 )
{
struct V_3 * V_2 ;
V_2 = F_13 ( V_15 ) ;
if ( V_2 == NULL ) {
V_2 = F_12 ( V_27 ) ;
if ( V_2 != NULL && ! V_2 -> V_26 )
V_2 = NULL ;
}
return V_2 ;
}
static void F_16 ( struct V_3 * V_2 ,
struct V_5 * V_28 ,
struct V_10 * V_29 ,
int V_30 ,
struct V_31 * V_32 )
{
struct V_33 * V_34 ;
struct V_35 * V_36 = NULL ;
T_1 type ;
V_34 = F_17 ( V_37 , V_32 ) ;
if ( V_34 != NULL ) {
F_18 ( V_34 , L_1 ,
V_2 -> V_15 ? V_2 -> V_15 : L_2 ) ;
if ( V_28 != NULL ) {
struct V_38 * V_39 ;
V_39 = F_7 ( V_28 ) ;
type = V_39 -> type ;
V_36 = V_39 -> V_40 . V_36 ;
F_19 ( V_34 , 0 , NULL ,
V_28 -> V_41 , V_28 -> V_42 ) ;
#if F_2 ( V_8 ) || F_2 ( V_9 )
} else if ( V_29 != NULL ) {
struct V_43 * V_44 ;
V_44 = F_10 ( V_29 ) ;
type = V_44 -> type ;
F_20 ( V_34 , 0 , NULL ,
& V_29 -> V_41 , & V_29 -> V_42 ) ;
#endif
} else {
type = V_2 -> type ;
V_36 = V_2 -> V_40 . V_36 ;
}
switch ( type ) {
case V_45 :
F_18 ( V_34 , L_3 ) ;
break;
case V_46 :
F_21 ( V_36 == NULL ) ;
F_18 ( V_34 ,
L_4 ,
V_36 -> V_47 ) ;
break;
}
F_18 ( V_34 , L_5 , V_30 == 0 ? 1 : 0 ) ;
F_22 ( V_34 ) ;
}
}
int T_2 F_23 ( T_1 V_21 )
{
T_1 V_17 ;
struct V_48 * V_49 ;
if ( V_21 == 0 )
return - V_50 ;
V_49 = F_24 ( sizeof( * V_49 ) , V_51 ) ;
if ( V_49 == NULL )
return - V_52 ;
V_49 -> V_21 = 1 << V_21 ;
V_49 -> V_25 = F_25 ( V_49 -> V_21 ,
sizeof( struct V_23 ) ,
V_51 ) ;
if ( V_49 -> V_25 == NULL ) {
F_6 ( V_49 ) ;
return - V_52 ;
}
for ( V_17 = 0 ; V_17 < V_49 -> V_21 ; V_17 ++ )
F_26 ( & V_49 -> V_25 [ V_17 ] ) ;
F_27 ( & V_53 ) ;
F_28 ( V_20 , V_49 ) ;
F_29 ( & V_53 ) ;
return 0 ;
}
int F_30 ( struct V_3 * V_2 ,
struct V_31 * V_32 )
{
int V_54 = 0 ;
struct V_3 * V_55 ;
struct V_5 * V_6 ;
struct V_5 * V_7 ;
#if F_2 ( V_8 ) || F_2 ( V_9 )
struct V_10 * V_11 ;
struct V_10 * V_12 ;
#endif
F_31 () ;
F_27 ( & V_53 ) ;
if ( V_2 -> V_15 != NULL )
V_55 = F_13 ( V_2 -> V_15 ) ;
else
V_55 = F_15 ( V_2 -> V_15 ) ;
if ( V_55 == NULL ) {
V_2 -> V_26 = 1 ;
if ( V_2 -> V_15 != NULL ) {
T_1 V_22 = F_11 ( V_2 -> V_15 ) ;
F_32 ( & V_2 -> V_56 ,
& F_33 ( V_20 ) -> V_25 [ V_22 ] ) ;
} else {
F_26 ( & V_2 -> V_56 ) ;
F_28 ( V_27 , V_2 ) ;
}
if ( V_2 -> type == V_14 ) {
F_34 (iter4,
&entry->type_def.addrsel->list4)
F_16 ( V_2 , V_6 , NULL ,
V_54 , V_32 ) ;
#if F_2 ( V_8 ) || F_2 ( V_9 )
F_35 (iter6,
&entry->type_def.addrsel->list6)
F_16 ( V_2 , NULL , V_11 ,
V_54 , V_32 ) ;
#endif
} else
F_16 ( V_2 , NULL , NULL ,
V_54 , V_32 ) ;
} else if ( V_55 -> type == V_14 &&
V_2 -> type == V_14 ) {
struct V_23 * V_57 ;
struct V_23 * V_58 ;
V_57 = & V_55 -> V_40 . V_59 -> V_60 ;
V_58 = & V_55 -> V_40 . V_59 -> V_61 ;
F_34 (iter4,
&entry->type_def.addrsel->list4)
if ( F_36 ( V_6 -> V_41 ,
V_6 -> V_42 ,
V_57 ) ) {
V_54 = - V_62 ;
goto V_63;
}
#if F_2 ( V_8 ) || F_2 ( V_9 )
F_35 (iter6,
&entry->type_def.addrsel->list6)
if ( F_37 ( & V_11 -> V_41 ,
& V_11 -> V_42 ,
V_58 ) ) {
V_54 = - V_62 ;
goto V_63;
}
#endif
F_4 (iter4, tmp4,
&entry->type_def.addrsel->list4) {
F_5 ( V_6 ) ;
V_6 -> V_26 = 1 ;
V_54 = F_38 ( V_6 , V_57 ) ;
F_16 ( V_55 , V_6 , NULL ,
V_54 , V_32 ) ;
if ( V_54 != 0 )
goto V_63;
}
#if F_2 ( V_8 ) || F_2 ( V_9 )
F_8 (iter6, tmp6,
&entry->type_def.addrsel->list6) {
F_9 ( V_11 ) ;
V_11 -> V_26 = 1 ;
V_54 = F_39 ( V_11 , V_58 ) ;
F_16 ( V_55 , NULL , V_11 ,
V_54 , V_32 ) ;
if ( V_54 != 0 )
goto V_63;
}
#endif
} else
V_54 = - V_50 ;
V_63:
F_29 ( & V_53 ) ;
F_40 () ;
return V_54 ;
}
int F_41 ( struct V_3 * V_2 ,
struct V_31 * V_32 )
{
return F_30 ( V_2 , V_32 ) ;
}
int F_42 ( struct V_3 * V_2 ,
struct V_31 * V_32 )
{
int V_54 = 0 ;
struct V_33 * V_34 ;
if ( V_2 == NULL )
return - V_64 ;
F_27 ( & V_53 ) ;
if ( V_2 -> V_26 ) {
V_2 -> V_26 = 0 ;
if ( V_2 != F_33 ( V_27 ) )
F_43 ( & V_2 -> V_56 ) ;
else
F_28 ( V_27 , NULL ) ;
} else
V_54 = - V_64 ;
F_29 ( & V_53 ) ;
V_34 = F_17 ( V_65 , V_32 ) ;
if ( V_34 != NULL ) {
F_18 ( V_34 ,
L_6 ,
V_2 -> V_15 ? V_2 -> V_15 : L_2 ,
V_54 == 0 ? 1 : 0 ) ;
F_22 ( V_34 ) ;
}
if ( V_54 == 0 ) {
struct V_5 * V_6 ;
struct V_38 * V_39 ;
switch ( V_2 -> type ) {
case V_14 :
F_34 (iter4,
&entry->type_def.addrsel->list4) {
V_39 = F_7 ( V_6 ) ;
F_44 ( V_39 -> V_40 . V_36 ) ;
}
break;
case V_46 :
F_44 ( V_2 -> V_40 . V_36 ) ;
break;
}
F_45 ( & V_2 -> V_13 , F_1 ) ;
}
return V_54 ;
}
int F_46 ( const char * V_15 ,
const struct V_66 * V_41 ,
const struct V_66 * V_42 ,
struct V_31 * V_32 )
{
struct V_3 * V_67 ;
struct V_5 * V_68 ;
struct V_5 * V_6 ;
#if F_2 ( V_8 ) || F_2 ( V_9 )
struct V_10 * V_11 ;
#endif
struct V_38 * V_2 ;
F_31 () ;
if ( V_15 )
V_67 = F_13 ( V_15 ) ;
else
V_67 = F_15 ( V_15 ) ;
if ( V_67 == NULL || V_67 -> type != V_14 )
goto V_69;
F_27 ( & V_53 ) ;
V_68 = F_47 ( V_41 -> V_70 , V_42 -> V_70 ,
& V_67 -> V_40 . V_59 -> V_60 ) ;
F_29 ( & V_53 ) ;
if ( V_68 == NULL )
goto V_69;
F_34 (iter4, &entry_map->type_def.addrsel->list4)
goto V_71;
#if F_2 ( V_8 ) || F_2 ( V_9 )
F_35 (iter6, &entry_map->type_def.addrsel->list6)
goto V_71;
#endif
F_42 ( V_67 , V_32 ) ;
V_71:
F_40 () ;
F_48 () ;
V_2 = F_7 ( V_68 ) ;
F_44 ( V_2 -> V_40 . V_36 ) ;
F_6 ( V_2 ) ;
return 0 ;
V_69:
F_40 () ;
return - V_64 ;
}
int F_49 ( const char * V_15 , struct V_31 * V_32 )
{
int V_54 ;
struct V_3 * V_2 ;
F_31 () ;
if ( V_15 )
V_2 = F_13 ( V_15 ) ;
else
V_2 = F_15 ( V_15 ) ;
V_54 = F_42 ( V_2 , V_32 ) ;
F_40 () ;
return V_54 ;
}
int F_50 ( struct V_31 * V_32 )
{
return F_49 ( NULL , V_32 ) ;
}
struct V_3 * F_51 ( const char * V_15 )
{
return F_15 ( V_15 ) ;
}
struct V_38 * F_52 ( const char * V_15 ,
T_3 V_41 )
{
struct V_3 * V_72 ;
struct V_5 * V_73 ;
V_72 = F_15 ( V_15 ) ;
if ( V_72 == NULL )
return NULL ;
if ( V_72 -> type != V_14 )
return NULL ;
V_73 = F_53 ( V_41 ,
& V_72 -> V_40 . V_59 -> V_60 ) ;
if ( V_73 == NULL )
return NULL ;
return F_7 ( V_73 ) ;
}
struct V_43 * F_54 ( const char * V_15 ,
const struct V_74 * V_41 )
{
struct V_3 * V_72 ;
struct V_10 * V_73 ;
V_72 = F_15 ( V_15 ) ;
if ( V_72 == NULL )
return NULL ;
if ( V_72 -> type != V_14 )
return NULL ;
V_73 = F_55 ( V_41 ,
& V_72 -> V_40 . V_59 -> V_61 ) ;
if ( V_73 == NULL )
return NULL ;
return F_10 ( V_73 ) ;
}
int F_56 ( T_1 * V_75 ,
T_1 * V_76 ,
int (* F_57) ( struct V_3 * V_2 , void * V_77 ) ,
void * V_78 )
{
int V_54 = - V_64 ;
T_1 V_79 ;
struct V_23 * V_80 ;
struct V_3 * V_81 ;
T_1 V_82 = 0 ;
F_31 () ;
for ( V_79 = * V_75 ;
V_79 < F_33 ( V_20 ) -> V_21 ;
V_79 ++ , V_82 = 0 ) {
V_80 = & F_33 ( V_20 ) -> V_25 [ V_79 ] ;
F_14 (iter_entry, iter_list, list)
if ( V_81 -> V_26 ) {
if ( V_82 ++ < * V_76 )
continue;
V_54 = F_57 ( V_81 , V_78 ) ;
if ( V_54 < 0 ) {
V_82 -- ;
goto V_83;
}
}
}
V_83:
F_40 () ;
* V_75 = V_79 ;
* V_76 = V_82 ;
return V_54 ;
}
