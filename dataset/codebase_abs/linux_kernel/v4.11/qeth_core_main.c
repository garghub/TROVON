int F_1 ( struct V_1 * V_2 )
{
return ( V_2 -> V_3 == V_4 ) ||
( V_2 -> V_3 == V_5 ) ;
}
static void F_2 ( struct V_6 * V_7 )
{
struct V_1 * V_2 ;
V_2 = F_3 ( V_7 , struct V_1 , V_8 ) ;
F_4 ( V_2 , 2 , L_1 ) ;
F_5 () ;
F_6 ( V_2 -> V_9 ) ;
F_7 () ;
F_8 ( V_2 -> V_10 ) ;
}
void F_9 ( struct V_1 * V_2 )
{
F_4 ( V_2 , 2 , L_2 ) ;
F_10 ( V_11 , & V_2 -> V_8 ) ;
}
static inline const char * F_11 ( struct V_1 * V_2 )
{
if ( V_2 -> V_12 . V_13 ) {
switch ( V_2 -> V_12 . type ) {
case V_14 :
return L_3 ;
case V_15 :
return L_4 ;
case V_16 :
return L_5 ;
case V_17 :
return L_6 ;
default:
return L_7 ;
}
} else {
switch ( V_2 -> V_12 . type ) {
case V_14 :
return L_8 ;
case V_15 :
return L_9 ;
case V_18 :
return L_10 ;
case V_16 :
return L_11 ;
case V_17 :
return L_12 ;
default:
return L_7 ;
}
}
return L_13 ;
}
const char * F_12 ( struct V_1 * V_2 )
{
if ( V_2 -> V_12 . V_13 ) {
switch ( V_2 -> V_12 . type ) {
case V_14 :
return L_14 ;
case V_15 :
return L_15 ;
case V_16 :
return L_16 ;
case V_17 :
return L_17 ;
default:
return L_18 ;
}
} else {
switch ( V_2 -> V_12 . type ) {
case V_14 :
switch ( V_2 -> V_12 . V_19 ) {
case V_20 :
return L_19 ;
case V_21 :
return L_20 ;
case V_22 :
return L_21 ;
case V_23 :
return L_22 ;
case V_24 :
return L_23 ;
case V_25 :
return L_24 ;
case V_26 :
return L_25 ;
case V_27 :
return L_26 ;
default:
return L_27 ;
}
case V_15 :
return L_28 ;
case V_18 :
return L_29 ;
case V_16 :
return L_30 ;
case V_17 :
return L_31 ;
default:
return L_18 ;
}
}
return L_32 ;
}
void F_13 ( struct V_1 * V_2 )
{
V_2 -> V_28 = V_29 ;
}
void F_14 ( struct V_1 * V_2 )
{
V_2 -> V_28 = NULL ;
}
static bool F_15 ( const struct V_1 * V_2 )
{
return V_2 -> V_28 == V_29 ;
}
void F_16 ( struct V_1 * V_2 , unsigned long V_30 ,
int V_31 )
{
unsigned long V_32 ;
F_17 ( & V_2 -> V_33 , V_32 ) ;
V_2 -> V_34 = V_30 ;
if ( V_31 )
V_2 -> V_35 &= V_30 ;
F_18 ( & V_2 -> V_33 , V_32 ) ;
F_19 ( & V_2 -> V_36 ) ;
}
int F_20 ( struct V_1 * V_2 , unsigned long V_30 )
{
unsigned long V_32 ;
int V_37 = 0 ;
F_17 ( & V_2 -> V_33 , V_32 ) ;
V_37 = ( V_2 -> V_38 & V_30 ) ;
F_18 ( & V_2 -> V_33 , V_32 ) ;
return V_37 ;
}
int F_21 ( struct V_1 * V_2 , unsigned long V_30 )
{
if ( F_15 ( V_2 ) )
return 0 ;
return F_22 ( V_2 -> V_36 ,
F_20 ( V_2 , V_30 ) == 0 ) ;
}
void F_23 ( struct V_1 * V_2 )
{
struct V_39 * V_40 , * V_41 ;
F_4 ( V_2 , 5 , L_33 ) ;
F_24 (pool_entry, tmp,
&card->qdio.in_buf_pool.entry_list, list) {
F_25 ( & V_40 -> V_42 ) ;
}
}
static int F_26 ( struct V_1 * V_2 )
{
struct V_39 * V_40 ;
void * V_43 ;
int V_44 , V_45 ;
F_4 ( V_2 , 5 , L_34 ) ;
for ( V_44 = 0 ; V_44 < V_2 -> V_46 . V_47 . V_48 ; ++ V_44 ) {
V_40 = F_27 ( sizeof( * V_40 ) , V_49 ) ;
if ( ! V_40 ) {
F_28 ( V_2 ) ;
return - V_50 ;
}
for ( V_45 = 0 ; V_45 < F_29 ( V_2 ) ; ++ V_45 ) {
V_43 = ( void * ) F_30 ( V_49 ) ;
if ( ! V_43 ) {
while ( V_45 > 0 )
F_31 ( ( unsigned long )
V_40 -> V_51 [ -- V_45 ] ) ;
F_32 ( V_40 ) ;
F_28 ( V_2 ) ;
return - V_50 ;
}
V_40 -> V_51 [ V_45 ] = V_43 ;
}
F_33 ( & V_40 -> V_52 ,
& V_2 -> V_46 . V_47 . V_53 ) ;
}
return 0 ;
}
int F_34 ( struct V_1 * V_2 , int V_54 )
{
F_4 ( V_2 , 2 , L_35 ) ;
if ( ( V_2 -> V_3 != V_55 ) &&
( V_2 -> V_3 != V_56 ) )
return - V_57 ;
F_23 ( V_2 ) ;
F_28 ( V_2 ) ;
V_2 -> V_46 . V_58 . V_48 = V_54 ;
V_2 -> V_46 . V_47 . V_48 = V_54 ;
return F_26 ( V_2 ) ;
}
static void F_35 ( struct V_59 * V_60 )
{
if ( ! V_60 )
return;
F_36 ( V_60 -> V_61 , V_62 ) ;
F_32 ( V_60 ) ;
}
static struct V_59 * F_37 ( void )
{
struct V_59 * V_60 = F_27 ( sizeof( * V_60 ) , V_49 ) ;
int V_44 ;
if ( ! V_60 )
return NULL ;
if ( F_38 ( V_60 -> V_61 , V_62 ) ) {
F_32 ( V_60 ) ;
return NULL ;
}
for ( V_44 = 0 ; V_44 < V_62 ; ++ V_44 )
V_60 -> V_63 [ V_44 ] . V_64 = V_60 -> V_61 [ V_44 ] ;
F_39 ( V_65 , 2 , & V_60 , sizeof( void * ) ) ;
return V_60 ;
}
static inline int F_40 ( struct V_1 * V_2 )
{
int V_37 ;
if ( V_2 -> V_66 . V_67 == V_68 ) {
F_41 ( V_65 , 2 , L_36 ) ;
F_42 ( V_2 -> V_46 . V_69 -> V_61 ,
V_62 ) ;
V_2 -> V_46 . V_69 -> V_70 = 127 ;
V_37 = F_43 ( F_44 ( V_2 ) , V_71 ,
V_2 -> V_46 . V_72 - 1 , 0 ,
127 ) ;
if ( V_37 ) {
F_45 ( V_65 , 2 , L_37 , V_37 ) ;
goto V_73;
}
}
V_37 = 0 ;
V_73:
return V_37 ;
}
static inline int F_46 ( struct V_1 * V_2 )
{
int V_37 ;
if ( V_2 -> V_66 . V_67 == V_68 ) {
int V_44 ;
struct V_74 * V_75 ;
F_41 ( V_65 , 2 , L_38 ) ;
V_2 -> V_46 . V_69 = F_37 () ;
if ( ! V_2 -> V_46 . V_69 ) {
V_37 = - 1 ;
goto V_76;
}
V_2 -> V_46 . V_72 = 2 ;
V_2 -> V_46 . V_77 =
F_27 ( V_2 -> V_46 . V_78 *
V_62 *
sizeof( struct V_74 ) , V_49 ) ;
V_75 = V_2 -> V_46 . V_77 ;
if ( V_75 == NULL ) {
V_37 = - 1 ;
goto V_79;
}
for ( V_44 = 0 ; V_44 < V_2 -> V_46 . V_78 ; ++ V_44 ) {
V_2 -> V_46 . V_80 [ V_44 ] -> V_81 = V_75 ;
V_75 += V_62 ;
}
} else {
F_41 ( V_65 , 2 , L_39 ) ;
V_2 -> V_46 . V_69 = NULL ;
V_2 -> V_46 . V_72 = 1 ;
}
F_45 ( V_65 , 2 , L_40 , V_2 -> V_46 . V_72 ) ;
V_37 = 0 ;
V_73:
return V_37 ;
V_79:
F_35 ( V_2 -> V_46 . V_69 ) ;
V_2 -> V_46 . V_69 = NULL ;
V_76:
F_47 ( & V_2 -> V_10 -> V_9 , L_41 ) ;
goto V_73;
}
static inline void F_48 ( struct V_1 * V_2 )
{
if ( V_2 -> V_46 . V_69 ) {
-- V_2 -> V_46 . V_72 ;
F_35 ( V_2 -> V_46 . V_69 ) ;
V_2 -> V_46 . V_69 = NULL ;
}
F_32 ( V_2 -> V_46 . V_77 ) ;
V_2 -> V_46 . V_77 = NULL ;
}
static inline enum V_82 F_49 ( int V_83 ,
int V_84 ) {
enum V_82 V_85 ;
switch ( V_83 ) {
case 0 :
V_85 = V_84 ? V_86 : V_87 ;
break;
case 4 :
case 16 :
case 17 :
case 18 :
V_85 = V_84 ? V_88 :
V_89 ;
break;
default:
V_85 = V_84 ? V_90 :
V_91 ;
break;
}
return V_85 ;
}
static inline void F_50 ( struct V_92 * V_60 ,
int V_93 , int V_94 )
{
if ( V_60 -> V_2 -> V_66 . V_67 != V_68 )
return;
if ( V_60 -> V_63 [ V_93 ] -> V_95 != NULL ) {
struct V_96 * V_97 = V_60 -> V_63 [ V_93 ] ;
struct V_96 * V_98 = V_60 -> V_63 [ V_93 ] -> V_95 ;
while ( V_98 ) {
if ( V_94 ||
F_51 ( & V_98 -> V_3 ) ==
V_99 ) {
struct V_96 * V_100 = V_98 ;
F_4 ( V_100 -> V_60 -> V_2 , 5 , L_42 ) ;
F_52 ( V_100 -> V_60 -> V_2 , 5 , L_43 , ( long ) V_100 ) ;
F_53 ( V_98 ) ;
V_98 = V_100 -> V_95 ;
F_54 ( V_97 -> V_95 != V_100 ) ;
V_97 -> V_95 = V_98 ;
F_55 ( V_101 , V_100 ) ;
} else {
V_97 = V_98 ;
V_98 = V_98 -> V_95 ;
}
}
}
if ( V_94 && ( F_51 ( & ( V_60 -> V_63 [ V_93 ] -> V_3 ) ) ==
V_99 ) ) {
V_60 -> V_63 [ V_93 ] -> V_102 = V_60 -> V_81 [ V_93 ] . V_102 ;
F_56 ( V_60 , V_93 ) ;
F_4 ( V_60 -> V_2 , 2 , L_44 ) ;
}
}
static inline void F_57 ( struct V_1 * V_2 ,
unsigned long V_103 ) {
struct V_104 * V_102 ;
struct V_96 * V_64 ;
enum V_82 V_105 ;
V_102 = (struct V_104 * ) F_58 ( V_103 ) ;
F_4 ( V_2 , 5 , L_45 ) ;
F_52 ( V_2 , 5 , L_43 , V_103 ) ;
V_64 = (struct V_96 * ) V_102 -> V_106 ;
F_52 ( V_2 , 5 , L_43 , V_102 -> V_106 ) ;
if ( F_59 ( & V_64 -> V_3 , V_107 ,
V_108 ) == V_107 ) {
V_105 = V_87 ;
} else {
F_54 ( F_51 ( & V_64 -> V_3 ) !=
V_109 ) ;
F_60 ( & V_64 -> V_3 , V_108 ) ;
V_105 = V_86 ;
}
if ( V_102 -> V_110 != 0 ) {
F_52 ( V_2 , 2 , L_46 , V_102 -> V_110 ) ;
V_105 = F_49 ( V_102 -> V_110 , 1 ) ;
}
F_61 ( V_64 -> V_60 , V_64 , V_105 ) ;
V_64 -> V_102 = NULL ;
F_62 ( V_64 -> V_60 , V_64 ,
V_99 ) ;
F_63 ( V_102 ) ;
}
static inline int F_64 ( struct V_1 * V_2 , unsigned int V_111 )
{
return V_2 -> V_66 . V_67 == V_68 &&
V_2 -> V_46 . V_69 != NULL &&
V_111 != 0 &&
V_111 == V_2 -> V_46 . V_72 - 1 ;
}
static int F_65 ( struct V_1 * V_2 )
{
int V_37 ;
struct V_112 * V_113 ;
F_4 ( V_2 , 5 , L_47 ) ;
if ( V_2 -> V_114 . V_3 != V_115 )
return - V_116 ;
V_113 = F_66 ( & V_2 -> V_114 ) ;
if ( ! V_113 ) {
F_67 ( & V_2 -> V_10 -> V_9 , L_48
L_49 ) ;
F_68 ( 2 , L_50
L_51 , F_69 ( & V_2 -> V_10 -> V_9 ) ) ;
return - V_50 ;
}
F_70 ( & V_2 -> V_114 , V_113 -> V_117 , V_118 ) ;
F_4 ( V_2 , 6 , L_52 ) ;
V_37 = F_71 ( V_2 -> V_114 . V_119 , & V_2 -> V_114 . V_120 ,
( V_121 ) V_113 , 0 , 0 ) ;
if ( V_37 ) {
F_68 ( 2 , L_53
L_54 , F_69 ( & V_2 -> V_10 -> V_9 ) , V_37 ) ;
F_60 ( & V_2 -> V_114 . V_122 , 0 ) ;
V_2 -> V_123 = 1 ;
F_72 ( V_2 ) ;
F_19 ( & V_2 -> V_36 ) ;
}
return V_37 ;
}
static struct V_124 * F_73 ( struct V_1 * V_2 )
{
struct V_124 * V_125 ;
V_125 = F_27 ( sizeof( struct V_124 ) , V_126 ) ;
if ( V_125 ) {
F_60 ( & V_125 -> V_127 , 1 ) ;
F_60 ( & V_125 -> V_128 , 0 ) ;
V_125 -> V_2 = V_2 ;
}
return V_125 ;
}
static void F_74 ( struct V_124 * V_125 )
{
F_75 ( F_51 ( & V_125 -> V_127 ) <= 0 ) ;
F_76 ( & V_125 -> V_127 ) ;
}
static void F_77 ( struct V_124 * V_125 )
{
F_75 ( F_51 ( & V_125 -> V_127 ) <= 0 ) ;
if ( F_78 ( & V_125 -> V_127 ) )
F_32 ( V_125 ) ;
}
static void F_79 ( struct V_129 * V_130 , int V_37 ,
struct V_1 * V_2 )
{
char * V_131 ;
int V_132 = V_130 -> V_133 . V_134 ;
V_131 = F_80 ( V_132 ) ;
if ( V_37 )
F_68 ( 2 , L_55
L_56 ,
V_131 , V_132 , F_69 ( & V_2 -> V_10 -> V_9 ) ,
F_81 ( V_2 ) , V_37 ,
F_82 ( V_37 ) ) ;
else
F_68 ( 5 , L_57 ,
V_131 , V_132 , F_69 ( & V_2 -> V_10 -> V_9 ) ,
F_81 ( V_2 ) ) ;
}
static struct V_129 * F_83 ( struct V_1 * V_2 ,
struct V_112 * V_113 )
{
struct V_129 * V_130 = NULL ;
F_4 ( V_2 , 5 , L_58 ) ;
if ( F_84 ( V_113 -> V_117 ) ) {
V_130 = (struct V_129 * ) F_85 ( V_113 -> V_117 ) ;
if ( F_86 ( V_130 ) ) {
if ( V_130 -> V_133 . V_134 != V_135 &&
V_130 -> V_133 . V_134 != V_136 &&
V_130 -> V_133 . V_134 != V_137 &&
V_130 -> V_133 . V_134 != V_138 )
F_79 ( V_130 ,
V_130 -> V_133 . V_139 , V_2 ) ;
return V_130 ;
} else {
switch ( V_130 -> V_133 . V_134 ) {
case V_140 :
if ( V_130 -> V_133 . V_139 ==
V_141 ) {
F_47 ( & V_2 -> V_10 -> V_9 ,
L_59
L_60
L_61 ,
F_81 ( V_2 ) ) ;
F_9 ( V_2 ) ;
} else {
F_67 ( & V_2 -> V_10 -> V_9 ,
L_62
L_63 ,
F_81 ( V_2 ) ,
V_2 -> V_12 . V_142 ) ;
F_79 ( V_130 ,
V_130 -> V_133 . V_139 , V_2 ) ;
}
V_2 -> V_143 = 0 ;
if ( V_2 -> V_9 && F_87 ( V_2 -> V_9 ) )
F_88 ( V_2 -> V_9 ) ;
return NULL ;
case V_144 :
F_89 ( & V_2 -> V_10 -> V_9 ,
L_64
L_65 ,
F_81 ( V_2 ) ,
V_2 -> V_12 . V_142 ) ;
F_90 ( V_2 -> V_9 ) ;
V_2 -> V_143 = 1 ;
if ( V_2 -> V_12 . V_145 )
V_2 -> V_12 . V_145 = 2 ;
F_72 ( V_2 ) ;
return NULL ;
case V_146 :
case V_147 :
case V_148 :
if ( V_2 -> V_149 -> V_150
( V_2 , V_130 ) )
return V_130 ;
else
return NULL ;
case V_137 :
return V_130 ;
case V_151 :
F_4 ( V_2 , 3 , L_66 ) ;
break;
case V_152 :
F_4 ( V_2 , 3 , L_67 ) ;
break;
default:
F_68 ( 2 , L_68
L_69 ) ;
break;
}
}
}
return V_130 ;
}
void F_91 ( struct V_1 * V_2 )
{
struct V_124 * V_125 , * V_153 ;
unsigned long V_32 ;
F_4 ( V_2 , 4 , L_70 ) ;
F_17 ( & V_2 -> V_154 , V_32 ) ;
F_24 (reply, r, &card->cmd_waiter_list, list) {
F_74 ( V_125 ) ;
V_125 -> V_37 = - V_116 ;
F_76 ( & V_125 -> V_128 ) ;
F_92 ( & V_125 -> V_42 ) ;
F_19 ( & V_125 -> V_36 ) ;
F_77 ( V_125 ) ;
}
F_18 ( & V_2 -> V_154 , V_32 ) ;
F_60 ( & V_2 -> V_155 . V_122 , 0 ) ;
}
static int F_93 ( struct V_1 * V_2 ,
unsigned char * V_64 )
{
if ( ! V_64 )
return 0 ;
F_39 ( V_156 , 2 , V_64 , V_157 ) ;
if ( ( V_64 [ 2 ] & 0xc0 ) == 0xc0 ) {
F_68 ( 2 , L_71
L_72 ,
V_64 [ 4 ] ,
( ( V_64 [ 4 ] == 0x22 ) ?
L_73 : L_74 ) ) ;
F_4 ( V_2 , 2 , L_75 ) ;
F_4 ( V_2 , 2 , L_76 ) ;
F_52 ( V_2 , 2 , L_77 , - V_116 ) ;
if ( V_64 [ 4 ] == 0xf6 ) {
F_47 ( & V_2 -> V_10 -> V_9 ,
L_78
L_79 ) ;
return - V_57 ;
}
return - V_116 ;
}
return 0 ;
}
static struct V_1 * F_94 ( struct V_158 * V_159 )
{
struct V_1 * V_2 = F_95 ( & ( (struct V_160 * )
F_95 ( & V_159 -> V_9 ) ) -> V_9 ) ;
return V_2 ;
}
static void F_70 ( struct V_161 * V_162 , unsigned char * V_113 ,
T_1 V_163 )
{
struct V_1 * V_2 ;
V_2 = F_94 ( V_162 -> V_119 ) ;
F_4 ( V_2 , 4 , L_80 ) ;
if ( V_162 == & V_2 -> V_114 )
memcpy ( & V_162 -> V_120 , V_164 , sizeof( struct V_165 ) ) ;
else
memcpy ( & V_162 -> V_120 , V_166 , sizeof( struct V_165 ) ) ;
V_162 -> V_120 . V_167 = V_163 ;
V_162 -> V_120 . V_168 = ( T_1 ) F_96 ( V_113 ) ;
}
static struct V_112 * F_97 ( struct V_161 * V_162 )
{
T_2 V_169 ;
F_4 ( F_94 ( V_162 -> V_119 ) , 6 , L_81 ) ;
V_169 = V_162 -> V_170 ;
do {
if ( V_162 -> V_113 [ V_169 ] . V_3 == V_171 ) {
V_162 -> V_113 [ V_169 ] . V_3 = V_172 ;
V_162 -> V_170 = ( V_162 -> V_170 + 1 ) %
V_173 ;
memset ( V_162 -> V_113 [ V_169 ] . V_117 , 0 , V_118 ) ;
return V_162 -> V_113 + V_169 ;
}
V_169 = ( V_169 + 1 ) % V_173 ;
} while ( V_169 != V_162 -> V_170 );
return NULL ;
}
void F_98 ( struct V_161 * V_162 ,
struct V_112 * V_113 )
{
unsigned long V_32 ;
F_4 ( F_94 ( V_162 -> V_119 ) , 6 , L_82 ) ;
F_17 ( & V_162 -> V_174 , V_32 ) ;
memset ( V_113 -> V_117 , 0 , V_118 ) ;
V_113 -> V_3 = V_171 ;
V_113 -> V_175 = V_176 ;
V_113 -> V_37 = 0 ;
F_18 ( & V_162 -> V_174 , V_32 ) ;
F_19 ( & V_162 -> V_36 ) ;
}
static struct V_112 * F_66 ( struct V_161 * V_162 )
{
struct V_112 * V_64 = NULL ;
unsigned long V_32 ;
F_17 ( & V_162 -> V_174 , V_32 ) ;
V_64 = F_97 ( V_162 ) ;
F_18 ( & V_162 -> V_174 , V_32 ) ;
return V_64 ;
}
struct V_112 * F_99 ( struct V_161 * V_162 )
{
struct V_112 * V_64 ;
F_100 ( V_162 -> V_36 ,
( ( V_64 = F_66 ( V_162 ) ) != NULL ) ) ;
return V_64 ;
}
void F_101 ( struct V_161 * V_162 )
{
int V_177 ;
for ( V_177 = 0 ; V_177 < V_173 ; V_177 ++ )
F_98 ( V_162 , & V_162 -> V_113 [ V_177 ] ) ;
V_162 -> V_178 = 0 ;
V_162 -> V_170 = 0 ;
}
static void V_176 ( struct V_161 * V_162 ,
struct V_112 * V_113 )
{
struct V_1 * V_2 ;
struct V_124 * V_125 , * V_153 ;
struct V_129 * V_130 ;
unsigned long V_32 ;
int V_179 ;
int V_37 = 0 ;
V_2 = F_94 ( V_162 -> V_119 ) ;
F_4 ( V_2 , 4 , L_83 ) ;
V_37 = F_93 ( V_2 , V_113 -> V_117 ) ;
switch ( V_37 ) {
case 0 :
break;
case - V_116 :
F_91 ( V_2 ) ;
F_72 ( V_2 ) ;
default:
goto V_73;
}
V_130 = F_83 ( V_2 , V_113 ) ;
if ( ( V_130 == NULL ) && ( V_2 -> V_3 != V_55 ) )
goto V_73;
if ( V_2 -> V_12 . type == V_18 &&
V_130 &&
V_130 -> V_133 . V_134 != V_144 &&
V_2 -> V_180 . V_181 != NULL ) {
V_2 -> V_180 . V_181 ( V_2 -> V_9 , V_130 ) ;
goto V_73;
}
F_17 ( & V_2 -> V_154 , V_32 ) ;
F_24 (reply, r, &card->cmd_waiter_list, list) {
if ( ( V_125 -> V_182 == V_183 ) ||
( ( V_130 ) && ( V_125 -> V_182 == V_130 -> V_133 . V_182 ) ) ) {
F_74 ( V_125 ) ;
F_92 ( & V_125 -> V_42 ) ;
F_18 ( & V_2 -> V_154 , V_32 ) ;
V_179 = 0 ;
if ( V_125 -> V_175 != NULL ) {
if ( V_130 ) {
V_125 -> V_184 = ( V_185 ) ( ( char * ) V_130 -
( char * ) V_113 -> V_117 ) ;
V_179 = V_125 -> V_175 ( V_2 ,
V_125 ,
( unsigned long ) V_130 ) ;
} else
V_179 = V_125 -> V_175 ( V_2 ,
V_125 ,
( unsigned long ) V_113 ) ;
}
if ( V_130 )
V_125 -> V_37 = ( V_186 ) V_130 -> V_133 . V_139 ;
else if ( V_113 -> V_37 )
V_125 -> V_37 = V_113 -> V_37 ;
if ( V_179 ) {
F_17 ( & V_2 -> V_154 , V_32 ) ;
F_102 ( & V_125 -> V_42 ,
& V_2 -> V_187 ) ;
F_18 ( & V_2 -> V_154 , V_32 ) ;
} else {
F_76 ( & V_125 -> V_128 ) ;
F_19 ( & V_125 -> V_36 ) ;
}
F_77 ( V_125 ) ;
goto V_73;
}
}
F_18 ( & V_2 -> V_154 , V_32 ) ;
V_73:
memcpy ( & V_2 -> V_182 . V_188 ,
F_103 ( V_113 -> V_117 ) ,
V_189 ) ;
F_98 ( V_162 , V_113 ) ;
}
static int F_104 ( struct V_161 * V_162 )
{
int V_177 ;
F_41 ( V_65 , 2 , L_84 ) ;
for ( V_177 = 0 ; V_177 < V_173 ; V_177 ++ ) {
V_162 -> V_113 [ V_177 ] . V_117 =
F_27 ( V_118 , V_190 | V_49 ) ;
if ( V_162 -> V_113 [ V_177 ] . V_117 == NULL )
break;
V_162 -> V_113 [ V_177 ] . V_3 = V_171 ;
V_162 -> V_113 [ V_177 ] . V_162 = V_162 ;
V_162 -> V_113 [ V_177 ] . V_175 = V_176 ;
V_162 -> V_113 [ V_177 ] . V_37 = 0 ;
}
if ( V_177 < V_173 ) {
while ( V_177 -- > 0 )
F_32 ( V_162 -> V_113 [ V_177 ] . V_117 ) ;
return - V_50 ;
}
V_162 -> V_178 = 0 ;
V_162 -> V_170 = 0 ;
F_60 ( & V_162 -> V_122 , 0 ) ;
F_105 ( & V_162 -> V_174 ) ;
F_106 ( & V_162 -> V_36 ) ;
return 0 ;
}
static int F_107 ( struct V_1 * V_2 ,
unsigned long V_191 )
{
unsigned long V_32 ;
F_17 ( & V_2 -> V_33 , V_32 ) ;
if ( ! ( V_2 -> V_34 & V_191 ) ||
( V_2 -> V_35 & V_191 ) ) {
F_18 ( & V_2 -> V_33 , V_32 ) ;
return - V_57 ;
}
V_2 -> V_35 |= V_191 ;
F_18 ( & V_2 -> V_33 , V_32 ) ;
return 0 ;
}
void F_108 ( struct V_1 * V_2 , unsigned long V_191 )
{
unsigned long V_32 ;
F_17 ( & V_2 -> V_33 , V_32 ) ;
V_2 -> V_35 &= ~ V_191 ;
F_18 ( & V_2 -> V_33 , V_32 ) ;
F_19 ( & V_2 -> V_36 ) ;
}
void F_109 ( struct V_1 * V_2 , unsigned long V_191 )
{
unsigned long V_32 ;
F_17 ( & V_2 -> V_33 , V_32 ) ;
V_2 -> V_38 &= ~ V_191 ;
F_18 ( & V_2 -> V_33 , V_32 ) ;
F_19 ( & V_2 -> V_36 ) ;
}
static int F_110 ( struct V_1 * V_2 , unsigned long V_191 )
{
unsigned long V_32 ;
int V_37 = 0 ;
F_17 ( & V_2 -> V_33 , V_32 ) ;
if ( V_2 -> V_35 & V_191 ) {
if ( ( V_2 -> V_34 & V_191 ) &&
! ( V_2 -> V_38 & V_191 ) ) {
V_37 = 1 ;
V_2 -> V_35 &= ~ V_191 ;
V_2 -> V_38 |= V_191 ;
} else
V_37 = - V_57 ;
}
F_18 ( & V_2 -> V_33 , V_32 ) ;
return V_37 ;
}
int F_111 ( struct V_1 * V_2 , unsigned long V_191 )
{
int V_37 = 0 ;
F_100 ( V_2 -> V_36 ,
( V_37 = F_110 ( V_2 , V_191 ) ) >= 0 ) ;
return V_37 ;
}
void F_72 ( struct V_1 * V_2 )
{
F_4 ( V_2 , 2 , L_85 ) ;
if ( F_107 ( V_2 , V_192 ) == 0 )
F_112 ( & V_2 -> V_193 ) ;
}
static int F_113 ( struct V_158 * V_159 , struct V_194 * V_194 )
{
int V_195 , V_196 ;
char * V_197 ;
struct V_1 * V_2 ;
V_197 = ( char * ) V_194 -> V_198 ;
V_196 = V_194 -> V_199 . V_130 . V_196 ;
V_195 = V_194 -> V_199 . V_130 . V_195 ;
V_2 = F_94 ( V_159 ) ;
if ( V_196 & ( V_200 | V_201 |
V_202 | V_203 |
V_204 | V_205 ) ) {
F_4 ( V_2 , 2 , L_86 ) ;
F_67 ( & V_159 -> V_9 , L_48
L_49 ) ;
F_68 ( 2 , L_87 ,
F_69 ( & V_159 -> V_9 ) , V_195 , V_196 ) ;
F_114 ( V_206 , L_88 , V_207 ,
16 , 1 , V_194 , 64 , 1 ) ;
return 1 ;
}
if ( V_195 & V_208 ) {
if ( V_197 [ V_209 ] &
V_210 ) {
F_4 ( V_2 , 2 , L_89 ) ;
return 1 ;
}
if ( V_197 [ V_211 ] &
V_212 ) {
F_4 ( V_2 , 2 , L_90 ) ;
return 1 ;
}
if ( ( V_197 [ 2 ] == 0xaf ) && ( V_197 [ 3 ] == 0xfe ) ) {
F_4 ( V_2 , 2 , L_91 ) ;
return 1 ;
}
if ( ( ! V_197 [ 0 ] ) && ( ! V_197 [ 1 ] ) && ( ! V_197 [ 2 ] ) && ( ! V_197 [ 3 ] ) ) {
F_4 ( V_2 , 2 , L_92 ) ;
return 0 ;
}
F_4 ( V_2 , 2 , L_93 ) ;
return 1 ;
}
return 0 ;
}
static long F_115 ( struct V_158 * V_159 ,
unsigned long V_213 , struct V_194 * V_194 )
{
struct V_1 * V_2 ;
V_2 = F_94 ( V_159 ) ;
if ( ! V_2 || ! F_116 ( V_194 ) )
return 0 ;
switch ( F_117 ( V_194 ) ) {
case - V_116 :
F_68 ( 2 , L_94 ,
F_69 ( & V_159 -> V_9 ) ) ;
F_4 ( V_2 , 2 , L_95 ) ;
F_52 ( V_2 , 2 , L_77 , - V_116 ) ;
break;
case - V_214 :
F_67 ( & V_159 -> V_9 , L_96
L_97 ) ;
F_4 ( V_2 , 2 , L_95 ) ;
F_52 ( V_2 , 2 , L_77 , - V_214 ) ;
if ( V_213 == V_215 ) {
if ( V_2 -> V_117 . V_119 == V_159 ) {
V_2 -> V_117 . V_3 = V_216 ;
F_19 ( & V_2 -> V_36 ) ;
}
}
break;
default:
F_68 ( 2 , L_98 ,
F_69 ( & V_159 -> V_9 ) , F_117 ( V_194 ) ) ;
F_4 ( V_2 , 2 , L_95 ) ;
F_4 ( V_2 , 2 , L_99 ) ;
}
return F_117 ( V_194 ) ;
}
static void F_118 ( struct V_158 * V_159 , unsigned long V_213 ,
struct V_194 * V_194 )
{
int V_37 ;
int V_196 , V_195 ;
struct V_112 * V_64 ;
struct V_161 * V_162 ;
struct V_1 * V_2 ;
struct V_112 * V_113 ;
T_2 V_169 ;
if ( F_115 ( V_159 , V_213 , V_194 ) )
return;
V_196 = V_194 -> V_199 . V_130 . V_196 ;
V_195 = V_194 -> V_199 . V_130 . V_195 ;
V_2 = F_94 ( V_159 ) ;
if ( ! V_2 )
return;
F_4 ( V_2 , 5 , L_100 ) ;
if ( V_2 -> V_114 . V_119 == V_159 ) {
V_162 = & V_2 -> V_114 ;
F_4 ( V_2 , 5 , L_101 ) ;
} else if ( V_2 -> V_155 . V_119 == V_159 ) {
V_162 = & V_2 -> V_155 ;
F_4 ( V_2 , 5 , L_102 ) ;
} else {
V_162 = & V_2 -> V_117 ;
F_4 ( V_2 , 5 , L_103 ) ;
}
F_60 ( & V_162 -> V_122 , 0 ) ;
if ( V_194 -> V_199 . V_130 . V_217 & ( V_218 ) )
V_162 -> V_3 = V_219 ;
if ( V_194 -> V_199 . V_130 . V_217 & ( V_220 ) )
V_162 -> V_3 = V_221 ;
if ( ( V_162 == & V_2 -> V_117 ) && ( V_213 != 0 ) &&
( V_213 != V_215 ) )
goto V_73;
if ( V_213 == V_222 ) {
F_4 ( V_2 , 6 , L_104 ) ;
V_213 = 0 ;
}
if ( V_213 == V_223 ) {
F_4 ( V_2 , 6 , L_105 ) ;
V_213 = 0 ;
}
if ( ( V_195 & V_224 ) ||
( V_195 & V_208 ) ||
( V_196 ) ) {
if ( V_194 -> V_225 . V_226 . V_227 . V_228 ) {
F_67 ( & V_162 -> V_119 -> V_9 ,
L_106
L_107 ) ;
F_68 ( 2 , L_108
L_109 ,
F_69 ( & V_162 -> V_119 -> V_9 ) , V_196 , V_195 ) ;
F_114 ( V_206 , L_88 ,
V_207 , 16 , 1 , V_194 , 32 , 1 ) ;
F_114 ( V_206 , L_110 ,
V_207 , 16 , 1 , V_194 -> V_198 , 32 , 1 ) ;
}
if ( V_213 == V_215 ) {
V_162 -> V_3 = V_216 ;
goto V_73;
}
V_37 = F_113 ( V_159 , V_194 ) ;
if ( V_37 ) {
F_91 ( V_2 ) ;
F_72 ( V_2 ) ;
goto V_73;
}
}
if ( V_213 == V_215 ) {
V_162 -> V_3 = V_229 ;
goto V_73;
}
if ( V_213 ) {
V_64 = (struct V_112 * ) F_119 ( ( V_121 ) V_213 ) ;
V_64 -> V_3 = V_230 ;
}
if ( V_162 == & V_2 -> V_117 )
return;
if ( V_162 == & V_2 -> V_114 &&
V_162 -> V_3 == V_115 )
F_65 ( V_2 ) ;
V_113 = V_162 -> V_113 ;
V_169 = V_162 -> V_178 ;
while ( V_113 [ V_169 ] . V_3 == V_230 ) {
if ( V_113 [ V_169 ] . V_175 != NULL )
V_113 [ V_169 ] . V_175 ( V_162 , V_113 + V_169 ) ;
V_169 = ( V_169 + 1 ) % V_173 ;
}
V_162 -> V_178 = V_169 ;
V_73:
F_19 ( & V_2 -> V_36 ) ;
return;
}
static void F_61 ( struct V_92 * V_60 ,
struct V_96 * V_231 ,
enum V_82 V_105 )
{
struct V_232 * V_233 ;
if ( F_120 ( & V_231 -> V_234 ) )
goto V_73;
V_233 = F_121 ( & V_231 -> V_234 ) ;
while ( V_233 ) {
F_52 ( V_60 -> V_2 , 5 , L_111 , V_105 ) ;
F_52 ( V_60 -> V_2 , 5 , L_43 , ( long ) V_233 ) ;
if ( V_233 -> V_235 == V_236 ) {
if ( V_233 -> V_237 ) {
struct V_238 * V_239 = F_122 ( V_233 -> V_237 ) ;
V_239 -> V_240 ( V_233 , V_105 ) ;
}
}
if ( F_123 ( & V_231 -> V_234 , V_233 ) )
V_233 = NULL ;
else
V_233 = F_124 ( & V_231 -> V_234 , V_233 ) ;
}
V_73:
return;
}
static void F_53 ( struct V_96 * V_231 )
{
struct V_232 * V_233 ;
struct V_238 * V_239 ;
int V_241 = 0 ;
if ( F_51 ( & V_231 -> V_3 ) == V_109 )
V_241 = 1 ;
F_54 ( F_51 ( & V_231 -> V_3 ) == V_108 ) ;
V_233 = F_125 ( & V_231 -> V_234 ) ;
while ( V_233 ) {
F_4 ( V_231 -> V_60 -> V_2 , 5 , L_112 ) ;
F_52 ( V_231 -> V_60 -> V_2 , 5 , L_43 , ( long ) V_233 ) ;
if ( V_241 && V_233 -> V_235 == V_236 ) {
if ( V_233 -> V_237 ) {
V_239 = F_122 ( V_233 -> V_237 ) ;
V_239 -> V_240 ( V_233 , V_91 ) ;
}
}
F_126 ( & V_233 -> V_242 ) ;
F_127 ( V_233 ) ;
V_233 = F_125 ( & V_231 -> V_234 ) ;
}
}
static void F_62 ( struct V_92 * V_111 ,
struct V_96 * V_231 ,
enum V_243 V_244 )
{
int V_44 ;
if ( V_231 -> V_64 -> V_245 [ 0 ] . V_246 & V_247 )
F_126 ( & V_111 -> V_248 ) ;
if ( V_244 == V_249 ) {
F_53 ( V_231 ) ;
}
for ( V_44 = 0 ; V_44 < F_29 ( V_111 -> V_2 ) ; ++ V_44 ) {
if ( V_231 -> V_64 -> V_245 [ V_44 ] . V_250 && V_231 -> V_251 [ V_44 ] )
F_55 ( V_252 ,
V_231 -> V_64 -> V_245 [ V_44 ] . V_250 ) ;
V_231 -> V_251 [ V_44 ] = 0 ;
V_231 -> V_64 -> V_245 [ V_44 ] . V_253 = 0 ;
V_231 -> V_64 -> V_245 [ V_44 ] . V_250 = NULL ;
V_231 -> V_64 -> V_245 [ V_44 ] . V_254 = 0 ;
V_231 -> V_64 -> V_245 [ V_44 ] . V_246 = 0 ;
}
V_231 -> V_64 -> V_245 [ 15 ] . V_254 = 0 ;
V_231 -> V_64 -> V_245 [ 15 ] . V_246 = 0 ;
V_231 -> V_255 = 0 ;
F_60 ( & V_231 -> V_3 , V_244 ) ;
}
static void F_128 ( struct V_92 * V_60 , int free )
{
int V_45 ;
for ( V_45 = 0 ; V_45 < V_62 ; ++ V_45 ) {
if ( ! V_60 -> V_63 [ V_45 ] )
continue;
F_50 ( V_60 , V_45 , 1 ) ;
F_62 ( V_60 , V_60 -> V_63 [ V_45 ] , V_249 ) ;
if ( free ) {
F_55 ( V_101 , V_60 -> V_63 [ V_45 ] ) ;
V_60 -> V_63 [ V_45 ] = NULL ;
}
}
}
void F_129 ( struct V_1 * V_2 )
{
int V_44 ;
F_4 ( V_2 , 2 , L_113 ) ;
for ( V_44 = 0 ; V_44 < V_2 -> V_46 . V_78 ; ++ V_44 ) {
if ( V_2 -> V_46 . V_80 [ V_44 ] ) {
F_128 ( V_2 -> V_46 . V_80 [ V_44 ] , 0 ) ;
}
}
}
static void F_28 ( struct V_1 * V_2 )
{
struct V_39 * V_40 , * V_41 ;
int V_44 = 0 ;
F_24 (pool_entry, tmp,
&card->qdio.init_pool.entry_list, init_list) {
for ( V_44 = 0 ; V_44 < F_29 ( V_2 ) ; ++ V_44 )
F_31 ( ( unsigned long ) V_40 -> V_51 [ V_44 ] ) ;
F_25 ( & V_40 -> V_52 ) ;
F_32 ( V_40 ) ;
}
}
static void F_130 ( struct V_161 * V_162 )
{
int V_177 ;
F_41 ( V_65 , 2 , L_114 ) ;
for ( V_177 = 0 ; V_177 < V_173 ; V_177 ++ )
F_32 ( V_162 -> V_113 [ V_177 ] . V_117 ) ;
}
static void F_131 ( struct V_1 * V_2 )
{
if ( ( F_51 ( & V_2 -> V_46 . V_3 ) != V_256 ) &&
( V_2 -> V_46 . V_78 == 4 ) )
F_132 ( V_2 ) ;
V_2 -> V_46 . V_78 = 1 ;
if ( V_2 -> V_46 . V_257 != 0 )
F_89 ( & V_2 -> V_10 -> V_9 , L_115 ) ;
V_2 -> V_46 . V_257 = 0 ;
}
static void F_133 ( struct V_1 * V_2 )
{
if ( ( F_51 ( & V_2 -> V_46 . V_3 ) != V_256 ) &&
( V_2 -> V_46 . V_78 == 1 ) ) {
F_132 ( V_2 ) ;
V_2 -> V_46 . V_257 = 2 ;
}
V_2 -> V_46 . V_78 = 4 ;
}
static void F_134 ( struct V_1 * V_2 )
{
struct V_158 * V_119 ;
struct V_258 * V_259 ;
F_41 ( V_65 , 2 , L_116 ) ;
V_119 = V_2 -> V_117 . V_119 ;
V_259 = F_135 ( V_119 , 0 ) ;
if ( ! V_259 )
goto V_73;
V_2 -> V_12 . V_260 = 0x4100 + V_259 -> V_261 ;
if ( V_2 -> V_12 . type == V_15 )
goto V_73;
if ( ( V_259 -> V_262 & 0x02 ) == 0x02 )
F_131 ( V_2 ) ;
else
F_133 ( V_2 ) ;
V_73:
F_32 ( V_259 ) ;
F_45 ( V_65 , 2 , L_117 , V_2 -> V_46 . V_78 ) ;
F_45 ( V_65 , 2 , L_118 , V_2 -> V_12 . V_260 ) ;
}
static void F_136 ( struct V_1 * V_2 )
{
F_41 ( V_65 , 4 , L_119 ) ;
F_60 ( & V_2 -> V_46 . V_3 , V_256 ) ;
V_2 -> V_46 . V_263 = V_264 ;
if ( V_2 -> V_12 . type == V_15 )
V_2 -> V_46 . V_47 . V_48 = V_265 ;
else
V_2 -> V_46 . V_47 . V_48 = V_266 ;
V_2 -> V_46 . V_58 . V_48 = V_2 -> V_46 . V_47 . V_48 ;
F_137 ( & V_2 -> V_46 . V_58 . V_53 ) ;
F_137 ( & V_2 -> V_46 . V_47 . V_53 ) ;
}
static void F_138 ( struct V_1 * V_2 )
{
V_2 -> V_66 . V_267 . type = V_268 ;
V_2 -> V_66 . V_269 . type = V_268 ;
V_2 -> V_66 . V_270 = 0 ;
V_2 -> V_66 . V_271 = V_272 ;
V_2 -> V_66 . V_273 = 0 ;
V_2 -> V_66 . V_274 = V_275 ;
V_2 -> V_66 . V_276 = V_277 ;
V_2 -> V_66 . V_67 = V_278 ;
}
static int F_139 ( struct V_1 * V_2 , unsigned long V_191 )
{
unsigned long V_32 ;
int V_37 = 0 ;
F_17 ( & V_2 -> V_33 , V_32 ) ;
F_52 ( V_2 , 4 , L_120 ,
( V_279 ) V_2 -> V_35 ,
( V_279 ) V_2 -> V_34 ,
( V_279 ) V_2 -> V_38 ) ;
V_37 = ( V_2 -> V_35 & V_191 ) ;
F_18 ( & V_2 -> V_33 , V_32 ) ;
return V_37 ;
}
static void F_140 ( struct V_6 * V_7 )
{
struct V_280 * V_281 ;
struct V_1 * V_2 = F_3 ( V_7 , struct V_1 ,
V_193 ) ;
F_4 ( V_2 , 2 , L_121 ) ;
if ( V_2 -> V_114 . V_3 != V_115 &&
V_2 -> V_155 . V_3 != V_115 )
return;
if ( F_139 ( V_2 , V_192 ) ) {
V_281 = F_141 ( V_2 -> V_149 -> V_282 , ( void * ) V_2 ,
L_122 ) ;
if ( F_116 ( V_281 ) ) {
F_108 ( V_2 , V_192 ) ;
F_109 ( V_2 ,
V_192 ) ;
}
}
}
static int F_142 ( struct V_1 * V_2 )
{
F_41 ( V_65 , 2 , L_123 ) ;
F_39 ( V_65 , 2 , & V_2 , sizeof( void * ) ) ;
V_2 -> V_114 . V_3 = V_216 ;
V_2 -> V_155 . V_3 = V_216 ;
V_2 -> V_117 . V_3 = V_216 ;
V_2 -> V_3 = V_55 ;
V_2 -> V_143 = 0 ;
V_2 -> V_123 = 0 ;
V_2 -> V_9 = NULL ;
F_105 ( & V_2 -> V_283 ) ;
F_105 ( & V_2 -> V_284 ) ;
F_105 ( & V_2 -> V_154 ) ;
F_105 ( & V_2 -> V_285 ) ;
F_105 ( & V_2 -> V_33 ) ;
F_143 ( & V_2 -> V_286 ) ;
F_143 ( & V_2 -> V_287 ) ;
V_2 -> V_35 = 0 ;
V_2 -> V_34 = 0 ;
V_2 -> V_38 = 0 ;
F_144 ( & V_2 -> V_193 , F_140 ) ;
F_137 ( & V_2 -> V_187 ) ;
F_106 ( & V_2 -> V_36 ) ;
F_138 ( V_2 ) ;
F_137 ( & V_2 -> V_288 . V_289 ) ;
V_2 -> V_288 . V_290 = 0 ;
V_2 -> V_288 . V_291 = 0 ;
V_2 -> V_288 . V_292 = 0 ;
F_136 ( V_2 ) ;
F_145 ( & V_2 -> V_293 , V_294 ) ;
F_144 ( & V_2 -> V_8 , F_2 ) ;
return 0 ;
}
static void F_146 ( struct V_295 * V_296 , struct V_297 * V_298 )
{
struct V_1 * V_2 = F_3 ( V_298 , struct V_1 ,
V_299 ) ;
if ( V_2 -> V_12 . V_300 [ 0 ] )
F_147 ( V_296 , L_124 ,
F_148 ( V_2 ) , V_2 -> V_12 . V_300 ) ;
}
static struct V_1 * F_149 ( void )
{
struct V_1 * V_2 ;
F_41 ( V_65 , 2 , L_125 ) ;
V_2 = F_27 ( sizeof( struct V_1 ) , V_190 | V_49 ) ;
if ( ! V_2 )
goto V_73;
F_39 ( V_65 , 2 , & V_2 , sizeof( void * ) ) ;
if ( F_104 ( & V_2 -> V_114 ) )
goto V_301;
if ( F_104 ( & V_2 -> V_155 ) )
goto V_302;
V_2 -> V_66 . V_303 = - 1 ;
V_2 -> V_299 . V_304 = F_146 ;
F_150 ( & V_2 -> V_299 ) ;
return V_2 ;
V_302:
F_130 ( & V_2 -> V_114 ) ;
V_301:
F_32 ( V_2 ) ;
V_73:
return NULL ;
}
static int F_151 ( struct V_1 * V_2 )
{
int V_44 = 0 ;
F_41 ( V_65 , 2 , L_126 ) ;
V_2 -> V_46 . V_305 = V_306 ;
V_2 -> V_46 . V_257 = V_307 ;
while ( V_308 [ V_44 ] [ V_309 ] ) {
if ( ( F_152 ( V_2 ) -> V_310 . V_311 ==
V_308 [ V_44 ] [ V_312 ] ) &&
( F_152 ( V_2 ) -> V_310 . V_313 ==
V_308 [ V_44 ] [ V_309 ] ) ) {
V_2 -> V_12 . type = V_308 [ V_44 ] [ V_309 ] ;
V_2 -> V_46 . V_78 =
V_308 [ V_44 ] [ V_314 ] ;
V_2 -> V_46 . V_72 = 1 ;
V_2 -> V_12 . V_315 =
V_308 [ V_44 ] [ V_316 ] ;
F_134 ( V_2 ) ;
return 0 ;
}
V_44 ++ ;
}
V_2 -> V_12 . type = V_317 ;
F_47 ( & V_2 -> V_10 -> V_9 , L_127
L_128 ) ;
return - V_318 ;
}
static int F_153 ( struct V_161 * V_162 )
{
unsigned long V_32 ;
struct V_1 * V_2 ;
int V_37 ;
V_2 = F_94 ( V_162 -> V_119 ) ;
F_4 ( V_2 , 3 , L_129 ) ;
F_17 ( F_154 ( V_162 -> V_119 ) , V_32 ) ;
V_37 = F_155 ( V_162 -> V_119 , V_222 ) ;
F_18 ( F_154 ( V_162 -> V_119 ) , V_32 ) ;
if ( V_37 )
return V_37 ;
V_37 = F_156 ( V_2 -> V_36 ,
V_162 -> V_3 == V_219 , V_319 ) ;
if ( V_37 == - V_320 )
return V_37 ;
if ( V_162 -> V_3 != V_219 )
return - V_321 ;
V_162 -> V_3 = V_216 ;
return 0 ;
}
static int F_157 ( struct V_161 * V_162 )
{
unsigned long V_32 ;
struct V_1 * V_2 ;
int V_37 ;
V_2 = F_94 ( V_162 -> V_119 ) ;
F_4 ( V_2 , 3 , L_130 ) ;
F_17 ( F_154 ( V_162 -> V_119 ) , V_32 ) ;
V_37 = F_158 ( V_162 -> V_119 , V_223 ) ;
F_18 ( F_154 ( V_162 -> V_119 ) , V_32 ) ;
if ( V_37 )
return V_37 ;
V_37 = F_156 ( V_2 -> V_36 ,
V_162 -> V_3 == V_221 , V_319 ) ;
if ( V_37 == - V_320 )
return V_37 ;
if ( V_162 -> V_3 != V_221 )
return - V_321 ;
return 0 ;
}
static int F_159 ( struct V_1 * V_2 )
{
int V_322 = 0 , V_323 = 0 , V_324 = 0 ;
F_4 ( V_2 , 3 , L_131 ) ;
V_322 = F_157 ( & V_2 -> V_114 ) ;
V_323 = F_157 ( & V_2 -> V_155 ) ;
V_324 = F_157 ( & V_2 -> V_117 ) ;
if ( V_322 )
return V_322 ;
if ( V_323 )
return V_323 ;
return V_324 ;
}
static int F_160 ( struct V_1 * V_2 )
{
int V_322 = 0 , V_323 = 0 , V_324 = 0 ;
F_4 ( V_2 , 3 , L_132 ) ;
V_322 = F_153 ( & V_2 -> V_114 ) ;
V_323 = F_153 ( & V_2 -> V_155 ) ;
V_324 = F_153 ( & V_2 -> V_117 ) ;
if ( V_322 )
return V_322 ;
if ( V_323 )
return V_323 ;
return V_324 ;
}
static int F_161 ( struct V_1 * V_2 , int V_325 )
{
int V_37 = 0 ;
F_4 ( V_2 , 3 , L_133 ) ;
if ( V_325 )
V_37 = F_159 ( V_2 ) ;
if ( V_37 )
return V_37 ;
return F_160 ( V_2 ) ;
}
int F_162 ( struct V_1 * V_2 , int V_326 )
{
int V_37 = 0 ;
F_4 ( V_2 , 3 , L_134 ) ;
switch ( F_59 ( & V_2 -> V_46 . V_3 , V_327 ,
V_328 ) ) {
case V_327 :
if ( V_2 -> V_12 . type == V_15 )
V_37 = F_163 ( F_44 ( V_2 ) ,
V_329 ) ;
else
V_37 = F_163 ( F_44 ( V_2 ) ,
V_330 ) ;
if ( V_37 )
F_52 ( V_2 , 3 , L_37 , V_37 ) ;
F_60 ( & V_2 -> V_46 . V_3 , V_331 ) ;
break;
case V_328 :
return V_37 ;
default:
break;
}
V_37 = F_161 ( V_2 , V_326 ) ;
if ( V_37 )
F_52 ( V_2 , 3 , L_135 , V_37 ) ;
V_2 -> V_3 = V_55 ;
return V_37 ;
}
static int F_164 ( struct V_1 * V_2 , void * * V_64 ,
int * V_253 )
{
struct V_332 * V_332 ;
char * V_333 ;
int V_334 ;
struct V_161 * V_162 = & V_2 -> V_117 ;
unsigned long V_32 ;
V_332 = F_165 ( V_162 -> V_119 , V_335 ) ;
if ( ! V_332 || V_332 -> V_130 == 0 )
return - V_336 ;
V_333 = F_27 ( V_332 -> V_167 , V_49 | V_190 ) ;
if ( ! V_333 )
return - V_50 ;
V_162 -> V_120 . V_337 = V_332 -> V_130 ;
V_162 -> V_120 . V_168 = ( T_1 ) F_96 ( V_333 ) ;
V_162 -> V_120 . V_167 = V_332 -> V_167 ;
V_162 -> V_120 . V_32 = V_338 ;
V_162 -> V_3 = V_339 ;
F_17 ( F_154 ( V_162 -> V_119 ) , V_32 ) ;
V_334 = F_166 ( V_162 -> V_119 , & V_162 -> V_120 ,
V_215 , V_340 , 0 ,
V_341 ) ;
F_18 ( F_154 ( V_162 -> V_119 ) , V_32 ) ;
if ( ! V_334 )
F_100 ( V_2 -> V_36 ,
( V_162 -> V_3 == V_229 ||
V_162 -> V_3 == V_216 ) ) ;
if ( V_162 -> V_3 == V_216 )
V_334 = - V_116 ;
else
V_162 -> V_3 = V_216 ;
if ( V_334 ) {
F_32 ( V_333 ) ;
* V_64 = NULL ;
* V_253 = 0 ;
} else {
* V_253 = V_332 -> V_167 ;
* V_64 = V_333 ;
}
return V_334 ;
}
static void F_167 ( struct V_1 * V_2 , char * V_342 )
{
F_41 ( V_65 , 2 , L_136 ) ;
V_2 -> V_12 . V_142 = V_342 [ 30 ] ;
V_2 -> V_12 . V_343 = V_342 [ 31 ] ;
V_2 -> V_12 . V_344 = V_342 [ 63 ] ;
V_2 -> V_12 . V_13 = ( ( V_342 [ 0x10 ] == V_345 [ 'V' ] ) &&
( V_342 [ 0x11 ] == V_345 [ 'M' ] ) ) ;
}
static void F_168 ( struct V_1 * V_2 , char * V_342 )
{
F_41 ( V_65 , 2 , L_137 ) ;
if ( V_342 [ 74 ] == 0xF0 && V_342 [ 75 ] == 0xF0 &&
V_342 [ 76 ] >= 0xF1 && V_342 [ 76 ] <= 0xF4 ) {
V_2 -> V_12 . V_346 . V_347 = 0 ;
V_2 -> V_12 . V_346 . V_348 = 0 ;
V_2 -> V_12 . V_346 . V_349 = 0 ;
} else {
V_2 -> V_12 . V_346 . V_347 = 250 ;
V_2 -> V_12 . V_346 . V_348 = 5 ;
V_2 -> V_12 . V_346 . V_349 = 15 ;
}
}
static void F_169 ( struct V_1 * V_2 )
{
V_2 -> V_350 . V_351 = 0x00010103UL ;
V_2 -> V_350 . V_352 = 0x00010108UL ;
V_2 -> V_350 . V_353 = 0x0001010aUL ;
V_2 -> V_350 . V_354 = 0x0001010bUL ;
V_2 -> V_350 . V_355 = 0x0001010dUL ;
}
static void F_170 ( struct V_1 * V_2 )
{
switch ( V_2 -> V_12 . type ) {
case V_15 :
V_2 -> V_12 . V_260 = V_356 ;
break;
case V_14 :
case V_18 :
V_2 -> V_12 . V_260 = V_357 ;
break;
default:
break;
}
}
static int F_171 ( struct V_161 * V_162 ,
void (* F_172)( struct V_161 * ,
struct V_112 * ) )
{
struct V_112 * V_113 ;
unsigned long V_32 ;
int V_37 ;
struct V_1 * V_2 ;
F_41 ( V_65 , 2 , L_138 ) ;
V_2 = F_94 ( V_162 -> V_119 ) ;
V_113 = F_66 ( V_162 ) ;
if ( ! V_113 )
return - V_50 ;
V_113 -> V_175 = F_172 ;
memcpy ( & V_162 -> V_120 , V_164 , sizeof( struct V_165 ) ) ;
V_162 -> V_120 . V_167 = V_118 ;
V_162 -> V_120 . V_168 = ( T_1 ) F_96 ( V_113 -> V_117 ) ;
F_100 ( V_2 -> V_36 ,
F_59 ( & V_162 -> V_122 , 0 , 1 ) == 0 ) ;
F_41 ( V_65 , 6 , L_52 ) ;
F_17 ( F_154 ( V_162 -> V_119 ) , V_32 ) ;
V_37 = F_71 ( V_162 -> V_119 ,
& V_162 -> V_120 , ( V_121 ) V_113 , 0 , 0 ) ;
F_18 ( F_154 ( V_162 -> V_119 ) , V_32 ) ;
if ( V_37 ) {
F_68 ( 2 , L_139 , V_37 ) ;
F_45 ( V_65 , 2 , L_135 , V_37 ) ;
F_60 ( & V_162 -> V_122 , 0 ) ;
F_19 ( & V_2 -> V_36 ) ;
return V_37 ;
}
V_37 = F_156 ( V_2 -> V_36 ,
V_162 -> V_3 == V_115 , V_319 ) ;
if ( V_37 == - V_320 )
return V_37 ;
if ( V_162 -> V_3 != V_115 ) {
V_37 = - V_321 ;
F_45 ( V_65 , 2 , L_140 , V_37 ) ;
F_101 ( V_162 ) ;
} else
V_37 = 0 ;
return V_37 ;
}
static int F_173 ( struct V_161 * V_162 ,
void (* F_172)( struct V_161 * ,
struct V_112 * ) )
{
struct V_1 * V_2 ;
struct V_112 * V_113 ;
unsigned long V_32 ;
V_185 V_358 ;
T_2 V_41 ;
int V_37 ;
struct V_359 V_360 ;
V_2 = F_94 ( V_162 -> V_119 ) ;
F_41 ( V_65 , 2 , L_141 ) ;
V_113 = F_66 ( V_162 ) ;
if ( ! V_113 )
return - V_50 ;
V_113 -> V_175 = F_172 ;
memcpy ( & V_162 -> V_120 , V_166 , sizeof( struct V_165 ) ) ;
V_162 -> V_120 . V_167 = V_361 ;
V_162 -> V_120 . V_168 = ( T_1 ) F_96 ( V_113 -> V_117 ) ;
if ( V_162 == & V_2 -> V_155 ) {
memcpy ( V_113 -> V_117 , V_362 , V_361 ) ;
memcpy ( F_174 ( V_113 -> V_117 ) ,
& V_2 -> V_182 . V_363 , V_189 ) ;
V_2 -> V_182 . V_363 ++ ;
} else {
memcpy ( V_113 -> V_117 , V_364 , V_361 ) ;
memcpy ( F_174 ( V_113 -> V_117 ) ,
& V_2 -> V_182 . V_363 , V_189 ) ;
}
V_41 = ( ( T_2 ) V_2 -> V_12 . V_365 ) | 0x80 ;
memcpy ( F_175 ( V_113 -> V_117 ) , & V_41 , 1 ) ;
memcpy ( F_176 ( V_113 -> V_117 ) ,
& V_2 -> V_350 . V_351 , V_366 ) ;
memcpy ( F_177 ( V_113 -> V_117 ) ,
& V_2 -> V_12 . V_260 , sizeof( V_185 ) ) ;
F_178 ( F_44 ( V_2 ) , & V_360 ) ;
memcpy ( F_179 ( V_113 -> V_117 ) , & V_360 . V_367 , 2 ) ;
V_358 = ( V_2 -> V_12 . V_344 << 8 ) + V_2 -> V_12 . V_343 ;
memcpy ( F_180 ( V_113 -> V_117 ) , & V_358 , 2 ) ;
F_100 ( V_2 -> V_36 ,
F_59 ( & V_162 -> V_122 , 0 , 1 ) == 0 ) ;
F_41 ( V_65 , 6 , L_52 ) ;
F_17 ( F_154 ( V_162 -> V_119 ) , V_32 ) ;
V_37 = F_71 ( V_162 -> V_119 ,
& V_162 -> V_120 , ( V_121 ) V_113 , 0 , 0 ) ;
F_18 ( F_154 ( V_162 -> V_119 ) , V_32 ) ;
if ( V_37 ) {
F_68 ( 2 , L_142 ,
V_37 ) ;
F_45 ( V_65 , 2 , L_37 , V_37 ) ;
F_60 ( & V_162 -> V_122 , 0 ) ;
F_19 ( & V_2 -> V_36 ) ;
return V_37 ;
}
V_37 = F_156 ( V_2 -> V_36 ,
V_162 -> V_3 == V_368 , V_319 ) ;
if ( V_37 == - V_320 )
return V_37 ;
if ( V_162 -> V_3 != V_368 ) {
F_67 ( & V_162 -> V_119 -> V_9 , L_143
L_144 ) ;
F_68 ( 2 , L_145 ,
F_69 ( & V_162 -> V_119 -> V_9 ) ) ;
F_45 ( V_65 , 2 , L_135 , - V_321 ) ;
F_101 ( V_162 ) ;
return - V_321 ;
}
return F_171 ( V_162 , F_172 ) ;
}
static int F_181 ( int V_369 )
{
if ( ( V_369 & 0xff ) == 8 )
return ( V_369 & 0xff ) + 0x400 ;
if ( ( ( V_369 >> 8 ) & 3 ) == 1 )
return ( V_369 & 0xff ) + 0x200 ;
return V_369 ;
}
static void F_182 ( struct V_161 * V_162 ,
struct V_112 * V_113 )
{
struct V_1 * V_2 ;
V_185 V_358 ;
F_41 ( V_65 , 2 , L_146 ) ;
if ( V_162 -> V_3 == V_216 ) {
V_162 -> V_3 = V_368 ;
goto V_73;
}
V_2 = F_94 ( V_162 -> V_119 ) ;
if ( ! ( F_183 ( V_113 -> V_117 ) ) ) {
if ( F_184 ( V_113 -> V_117 ) == V_370 )
F_47 ( & V_2 -> V_155 . V_119 -> V_9 ,
L_147
L_148 ) ;
else
F_68 ( 2 , L_149
L_150 ,
F_69 ( & V_2 -> V_155 . V_119 -> V_9 ) ) ;
goto V_73;
}
memcpy ( & V_358 , F_177 ( V_113 -> V_117 ) , 2 ) ;
if ( ( V_358 & ~ 0x0100 ) != F_181 ( V_2 -> V_12 . V_260 ) ) {
F_68 ( 2 , L_151
L_152
L_153 , F_69 ( & V_2 -> V_155 . V_119 -> V_9 ) ,
V_2 -> V_12 . V_260 , V_358 ) ;
goto V_73;
}
V_162 -> V_3 = V_115 ;
V_73:
F_98 ( V_162 , V_113 ) ;
}
static void F_185 ( struct V_161 * V_162 ,
struct V_112 * V_113 )
{
struct V_1 * V_2 ;
V_185 V_358 ;
F_41 ( V_65 , 2 , L_154 ) ;
if ( V_162 -> V_3 == V_216 ) {
V_162 -> V_3 = V_368 ;
goto V_73;
}
V_2 = F_94 ( V_162 -> V_119 ) ;
if ( F_93 ( V_2 , V_113 -> V_117 ) )
goto V_73;
if ( ! ( F_183 ( V_113 -> V_117 ) ) ) {
switch ( F_184 ( V_113 -> V_117 ) ) {
case V_370 :
F_47 ( & V_2 -> V_155 . V_119 -> V_9 ,
L_147
L_148 ) ;
break;
case V_371 :
case V_372 :
F_47 ( & V_2 -> V_114 . V_119 -> V_9 ,
L_155
L_156 ) ;
break;
default:
F_68 ( 2 , L_157
L_150 ,
F_69 ( & V_2 -> V_114 . V_119 -> V_9 ) ) ;
}
F_52 ( V_2 , 2 , L_158 ,
F_184 ( V_113 -> V_117 ) ) ;
goto V_73;
}
memcpy ( & V_358 , F_177 ( V_113 -> V_117 ) , 2 ) ;
if ( V_358 != F_181 ( V_2 -> V_12 . V_260 ) ) {
F_68 ( 2 , L_159
L_160 ,
F_69 ( & V_2 -> V_114 . V_119 -> V_9 ) ,
V_2 -> V_12 . V_260 , V_358 ) ;
goto V_73;
}
memcpy ( & V_2 -> V_350 . V_373 ,
F_176 ( V_113 -> V_117 ) ,
V_366 ) ;
memcpy ( & V_2 -> V_12 . V_300 [ 0 ] ,
F_186 ( V_113 -> V_117 ) , V_374 ) ;
V_162 -> V_3 = V_115 ;
V_73:
F_98 ( V_162 , V_113 ) ;
}
void F_187 ( struct V_1 * V_2 , int V_163 ,
struct V_112 * V_113 )
{
F_70 ( & V_2 -> V_155 , V_113 -> V_117 , V_163 ) ;
V_113 -> V_175 = F_98 ;
memcpy ( F_174 ( V_113 -> V_117 ) ,
& V_2 -> V_182 . V_363 , V_189 ) ;
V_2 -> V_182 . V_363 ++ ;
memcpy ( F_103 ( V_113 -> V_117 ) ,
& V_2 -> V_182 . V_375 , V_189 ) ;
V_2 -> V_182 . V_375 ++ ;
memcpy ( F_188 ( V_113 -> V_117 ) ,
& V_2 -> V_182 . V_188 , V_189 ) ;
F_39 ( V_156 , 2 , V_113 -> V_117 , V_157 ) ;
}
int F_189 ( struct V_1 * V_2 , int V_163 ,
struct V_112 * V_113 ,
int (* F_190)( struct V_1 * V_376 ,
struct V_124 * V_377 ,
unsigned long V_378 ) ,
void * V_379 )
{
int V_37 ;
unsigned long V_32 ;
struct V_124 * V_125 = NULL ;
unsigned long V_380 , V_381 ;
struct V_129 * V_130 ;
F_4 ( V_2 , 2 , L_161 ) ;
if ( V_2 -> V_123 ) {
F_98 ( V_113 -> V_162 , V_113 ) ;
return - V_116 ;
}
V_125 = F_73 ( V_2 ) ;
if ( ! V_125 ) {
return - V_50 ;
}
V_125 -> V_175 = F_190 ;
V_125 -> V_382 = V_379 ;
if ( V_2 -> V_3 == V_55 )
V_125 -> V_182 = V_183 ;
else
V_125 -> V_182 = V_2 -> V_182 . V_383 ++ ;
F_106 ( & V_125 -> V_36 ) ;
F_17 ( & V_2 -> V_154 , V_32 ) ;
F_102 ( & V_125 -> V_42 , & V_2 -> V_187 ) ;
F_18 ( & V_2 -> V_154 , V_32 ) ;
F_39 ( V_156 , 2 , V_113 -> V_117 , V_157 ) ;
while ( F_59 ( & V_2 -> V_155 . V_122 , 0 , 1 ) ) ;
F_187 ( V_2 , V_163 , V_113 ) ;
if ( F_84 ( V_113 -> V_117 ) )
V_381 = V_384 ;
else
V_381 = V_319 ;
V_380 = V_385 + V_381 ;
F_4 ( V_2 , 6 , L_52 ) ;
F_17 ( F_154 ( V_2 -> V_155 . V_119 ) , V_32 ) ;
V_37 = F_71 ( V_2 -> V_155 . V_119 , & V_2 -> V_155 . V_120 ,
( V_121 ) V_113 , 0 , 0 ) ;
F_18 ( F_154 ( V_2 -> V_155 . V_119 ) , V_32 ) ;
if ( V_37 ) {
F_68 ( 2 , L_162
L_163 ,
F_69 ( & V_2 -> V_155 . V_119 -> V_9 ) , V_37 ) ;
F_52 ( V_2 , 2 , L_164 , V_37 ) ;
F_17 ( & V_2 -> V_154 , V_32 ) ;
F_92 ( & V_125 -> V_42 ) ;
F_77 ( V_125 ) ;
F_18 ( & V_2 -> V_154 , V_32 ) ;
F_98 ( V_113 -> V_162 , V_113 ) ;
F_60 ( & V_2 -> V_155 . V_122 , 0 ) ;
F_19 ( & V_2 -> V_36 ) ;
return V_37 ;
}
V_130 = (struct V_129 * ) ( V_113 -> V_117 + V_386 ) ;
if ( ( V_130 -> V_133 . V_134 == V_387 ) &&
( V_130 -> V_133 . V_388 == V_389 ) ) {
if ( ! F_191 ( V_125 -> V_36 ,
F_51 ( & V_125 -> V_128 ) , V_381 ) )
goto V_390;
} else {
while ( ! F_51 ( & V_125 -> V_128 ) ) {
if ( F_192 ( V_385 , V_380 ) )
goto V_390;
F_193 () ;
}
}
if ( V_125 -> V_37 == - V_116 )
goto error;
V_37 = V_125 -> V_37 ;
F_77 ( V_125 ) ;
return V_37 ;
V_390:
V_125 -> V_37 = - V_321 ;
F_17 ( & V_125 -> V_2 -> V_154 , V_32 ) ;
F_92 ( & V_125 -> V_42 ) ;
F_18 ( & V_125 -> V_2 -> V_154 , V_32 ) ;
F_76 ( & V_125 -> V_128 ) ;
error:
F_60 ( & V_2 -> V_155 . V_122 , 0 ) ;
F_98 ( V_113 -> V_162 , V_113 ) ;
V_2 -> V_155 . V_178 = ( V_2 -> V_155 . V_178 + 1 ) % V_173 ;
V_37 = V_125 -> V_37 ;
F_77 ( V_125 ) ;
return V_37 ;
}
static int F_194 ( struct V_1 * V_2 , struct V_124 * V_125 ,
unsigned long V_117 )
{
struct V_112 * V_113 ;
F_41 ( V_65 , 2 , L_165 ) ;
V_113 = (struct V_112 * ) V_117 ;
memcpy ( & V_2 -> V_350 . V_391 ,
F_195 ( V_113 -> V_117 ) ,
V_366 ) ;
F_45 ( V_65 , 2 , L_77 , V_113 -> V_37 ) ;
return 0 ;
}
static int F_196 ( struct V_1 * V_2 )
{
int V_37 ;
struct V_112 * V_113 ;
F_41 ( V_65 , 2 , L_166 ) ;
V_113 = F_99 ( & V_2 -> V_155 ) ;
memcpy ( V_113 -> V_117 , V_392 , V_393 ) ;
memcpy ( F_197 ( V_113 -> V_117 ) ,
& V_2 -> V_350 . V_373 , V_366 ) ;
memcpy ( F_198 ( V_113 -> V_117 ) ,
& V_2 -> V_350 . V_352 , V_366 ) ;
V_37 = F_189 ( V_2 , V_393 , V_113 ,
F_194 , NULL ) ;
return V_37 ;
}
static int F_199 ( struct V_1 * V_2 , struct V_124 * V_125 ,
unsigned long V_117 )
{
struct V_112 * V_113 ;
F_41 ( V_65 , 2 , L_167 ) ;
V_113 = (struct V_112 * ) V_117 ;
memcpy ( & V_2 -> V_350 . V_394 ,
F_200 ( V_113 -> V_117 ) ,
V_366 ) ;
F_45 ( V_65 , 2 , L_77 , V_113 -> V_37 ) ;
return 0 ;
}
static int F_201 ( struct V_1 * V_2 )
{
int V_37 ;
struct V_112 * V_113 ;
F_41 ( V_65 , 2 , L_168 ) ;
V_113 = F_99 ( & V_2 -> V_155 ) ;
memcpy ( V_113 -> V_117 , V_395 , V_396 ) ;
memcpy ( F_202 ( V_113 -> V_117 ) ,
& V_2 -> V_350 . V_373 , V_366 ) ;
memcpy ( F_203 ( V_113 -> V_117 ) ,
& V_2 -> V_350 . V_353 , V_366 ) ;
memcpy ( F_204 ( V_113 -> V_117 ) ,
& V_2 -> V_350 . V_391 , V_366 ) ;
V_37 = F_189 ( V_2 , V_396 , V_113 ,
F_199 , NULL ) ;
return V_37 ;
}
static inline int F_205 ( struct V_1 * V_2 )
{
switch ( V_2 -> V_12 . type ) {
case V_317 :
return 1500 ;
case V_15 :
return V_2 -> V_12 . V_397 ;
case V_14 :
switch ( V_2 -> V_12 . V_19 ) {
case V_21 :
case V_25 :
return 2000 ;
default:
return V_2 -> V_66 . V_303 ? 1500 : 1492 ;
}
case V_16 :
case V_17 :
return V_2 -> V_66 . V_303 ? 1500 : 1492 ;
default:
return 1500 ;
}
}
static inline int F_206 ( int V_398 )
{
switch ( V_398 ) {
case 0x4000 :
return 8192 ;
case 0x6000 :
return 16384 ;
case 0xa000 :
return 32768 ;
case 0xffff :
return 57344 ;
default:
return 0 ;
}
}
static inline int F_207 ( struct V_1 * V_2 , int V_399 )
{
switch ( V_2 -> V_12 . type ) {
case V_14 :
case V_16 :
case V_17 :
case V_15 :
return ( ( V_399 >= 576 ) &&
( V_399 <= V_2 -> V_12 . V_397 ) ) ;
case V_18 :
case V_317 :
default:
return 1 ;
}
}
static int F_208 ( struct V_1 * V_2 , struct V_124 * V_125 ,
unsigned long V_117 )
{
V_185 V_399 , V_398 ;
V_185 V_163 ;
T_2 V_19 ;
struct V_112 * V_113 ;
F_41 ( V_65 , 2 , L_169 ) ;
V_113 = (struct V_112 * ) V_117 ;
memcpy ( & V_2 -> V_350 . V_400 ,
F_209 ( V_113 -> V_117 ) ,
V_366 ) ;
if ( V_2 -> V_12 . type == V_15 ) {
memcpy ( & V_398 , F_210 ( V_113 -> V_117 ) , 2 ) ;
V_399 = F_206 ( V_398 ) ;
if ( ! V_399 ) {
V_113 -> V_37 = - V_401 ;
F_45 ( V_65 , 2 , L_77 , V_113 -> V_37 ) ;
return 0 ;
}
if ( V_2 -> V_12 . V_402 && ( V_2 -> V_12 . V_402 != V_399 ) ) {
if ( V_2 -> V_9 &&
( ( V_2 -> V_9 -> V_399 == V_2 -> V_12 . V_402 ) ||
( V_2 -> V_9 -> V_399 > V_399 ) ) )
V_2 -> V_9 -> V_399 = V_399 ;
F_132 ( V_2 ) ;
}
V_2 -> V_12 . V_402 = V_399 ;
V_2 -> V_12 . V_397 = V_399 ;
V_2 -> V_46 . V_263 = V_399 + 2 * V_403 ;
} else {
V_2 -> V_12 . V_397 = * ( V_185 * ) F_210 (
V_113 -> V_117 ) ;
V_2 -> V_12 . V_402 = F_211 ( V_2 -> V_12 . V_397 ,
F_205 ( V_2 ) ) ;
V_2 -> V_46 . V_263 = V_264 ;
}
memcpy ( & V_163 , F_212 ( V_113 -> V_117 ) , 2 ) ;
if ( V_163 >= V_404 ) {
memcpy ( & V_19 ,
F_213 ( V_113 -> V_117 ) , 1 ) ;
V_2 -> V_12 . V_19 = V_19 ;
} else
V_2 -> V_12 . V_19 = 0 ;
F_45 ( V_65 , 2 , L_170 , V_2 -> V_12 . V_19 ) ;
F_45 ( V_65 , 2 , L_77 , V_113 -> V_37 ) ;
return 0 ;
}
static int F_214 ( struct V_1 * V_2 )
{
int V_37 ;
char V_405 ;
struct V_112 * V_113 ;
F_41 ( V_65 , 2 , L_171 ) ;
V_113 = F_99 ( & V_2 -> V_155 ) ;
memcpy ( V_113 -> V_117 , V_406 , V_407 ) ;
* ( F_215 ( V_113 -> V_117 ) ) =
( T_2 ) V_2 -> V_12 . V_365 ;
if ( V_2 -> V_66 . V_303 )
if ( V_2 -> V_12 . type == V_18 )
V_405 = V_408 ;
else
V_405 = V_409 ;
else
V_405 = V_410 ;
memcpy ( F_216 ( V_113 -> V_117 ) , & V_405 , 1 ) ;
memcpy ( F_217 ( V_113 -> V_117 ) ,
& V_2 -> V_350 . V_394 , V_366 ) ;
memcpy ( F_218 ( V_113 -> V_117 ) ,
& V_2 -> V_350 . V_354 , V_366 ) ;
V_37 = F_189 ( V_2 , V_407 , V_113 ,
F_208 , NULL ) ;
return V_37 ;
}
static int F_219 ( struct V_1 * V_2 , struct V_124 * V_125 ,
unsigned long V_117 )
{
struct V_112 * V_113 ;
F_41 ( V_65 , 2 , L_172 ) ;
V_113 = (struct V_112 * ) V_117 ;
memcpy ( & V_2 -> V_350 . V_411 ,
F_220 ( V_113 -> V_117 ) ,
V_366 ) ;
if ( ! strncmp ( L_173 , F_220 ( V_113 -> V_117 ) ,
3 ) ) {
F_41 ( V_65 , 2 , L_174 ) ;
F_47 ( & V_2 -> V_10 -> V_9 , L_175
L_176 ) ;
V_113 -> V_37 = - V_412 ;
}
F_45 ( V_65 , 2 , L_77 , V_113 -> V_37 ) ;
return 0 ;
}
static int F_221 ( struct V_1 * V_2 )
{
int V_37 ;
V_185 V_358 ;
struct V_112 * V_113 ;
struct V_359 V_413 ;
F_41 ( V_65 , 2 , L_177 ) ;
V_113 = F_99 ( & V_2 -> V_155 ) ;
memcpy ( V_113 -> V_117 , V_414 , V_415 ) ;
memcpy ( F_222 ( V_113 -> V_117 ) ,
& V_2 -> V_350 . V_394 , V_366 ) ;
memcpy ( F_223 ( V_113 -> V_117 ) ,
& V_2 -> V_350 . V_355 , V_366 ) ;
memcpy ( F_224 ( V_113 -> V_117 ) ,
& V_2 -> V_350 . V_400 , V_366 ) ;
F_178 ( F_44 ( V_2 ) , & V_413 ) ;
memcpy ( F_225 ( V_113 -> V_117 ) , & V_413 . V_367 , 2 ) ;
V_358 = ( V_2 -> V_12 . V_344 << 8 ) + V_2 -> V_12 . V_343 ;
memcpy ( F_226 ( V_113 -> V_117 ) , & V_358 , 2 ) ;
V_37 = F_189 ( V_2 , V_415 , V_113 ,
F_219 , NULL ) ;
return V_37 ;
}
static int F_56 ( struct V_92 * V_60 , int V_93 )
{
int V_37 ;
struct V_96 * V_416 ;
V_37 = 0 ;
V_416 = F_227 ( V_101 , V_126 ) ;
if ( ! V_416 ) {
V_37 = - V_50 ;
goto V_73;
}
V_416 -> V_64 = V_60 -> V_61 [ V_93 ] ;
F_228 ( & V_416 -> V_234 ) ;
F_229 ( & V_416 -> V_234 . V_154 , & V_417 ) ;
V_416 -> V_60 = V_60 ;
V_416 -> V_102 = NULL ;
V_416 -> V_95 = V_60 -> V_63 [ V_93 ] ;
F_60 ( & V_416 -> V_3 , V_249 ) ;
V_60 -> V_63 [ V_93 ] = V_416 ;
if ( V_60 -> V_81 ) {
V_60 -> V_81 [ V_93 ] . V_418 = V_416 ;
F_52 ( V_60 -> V_2 , 2 , L_178 , V_93 ) ;
F_52 ( V_60 -> V_2 , 2 , L_43 , ( long ) V_416 ) ;
F_52 ( V_60 -> V_2 , 2 , L_43 ,
( long ) V_416 -> V_95 ) ;
}
V_73:
return V_37 ;
}
static void F_230 ( struct V_92 * V_60 )
{
if ( ! V_60 )
return;
F_36 ( V_60 -> V_61 , V_62 ) ;
F_32 ( V_60 ) ;
}
static struct V_92 * F_231 ( void )
{
struct V_92 * V_60 = F_27 ( sizeof( * V_60 ) , V_49 ) ;
if ( ! V_60 )
return NULL ;
if ( F_38 ( V_60 -> V_61 , V_62 ) ) {
F_32 ( V_60 ) ;
return NULL ;
}
return V_60 ;
}
static int F_232 ( struct V_1 * V_2 )
{
int V_44 , V_45 ;
F_41 ( V_65 , 2 , L_179 ) ;
if ( F_59 ( & V_2 -> V_46 . V_3 , V_256 ,
V_331 ) != V_256 )
return 0 ;
F_41 ( V_65 , 2 , L_180 ) ;
V_2 -> V_46 . V_419 = F_37 () ;
if ( ! V_2 -> V_46 . V_419 )
goto V_420;
if ( F_26 ( V_2 ) )
goto V_421;
V_2 -> V_46 . V_80 =
F_27 ( V_2 -> V_46 . V_78 *
sizeof( struct V_92 * ) , V_49 ) ;
if ( ! V_2 -> V_46 . V_80 )
goto V_422;
for ( V_44 = 0 ; V_44 < V_2 -> V_46 . V_78 ; ++ V_44 ) {
V_2 -> V_46 . V_80 [ V_44 ] = F_231 () ;
if ( ! V_2 -> V_46 . V_80 [ V_44 ] )
goto V_423;
F_45 ( V_65 , 2 , L_181 , V_44 ) ;
F_39 ( V_65 , 2 , & V_2 -> V_46 . V_80 [ V_44 ] , sizeof( void * ) ) ;
V_2 -> V_46 . V_80 [ V_44 ] -> V_424 = V_44 ;
for ( V_45 = 0 ; V_45 < V_62 ; ++ V_45 ) {
F_75 ( V_2 -> V_46 . V_80 [ V_44 ] -> V_63 [ V_45 ] != NULL ) ;
if ( F_56 ( V_2 -> V_46 . V_80 [ V_44 ] , V_45 ) )
goto V_425;
}
}
if ( F_46 ( V_2 ) )
goto V_423;
return 0 ;
V_425:
while ( V_45 > 0 ) {
-- V_45 ;
F_55 ( V_101 ,
V_2 -> V_46 . V_80 [ V_44 ] -> V_63 [ V_45 ] ) ;
V_2 -> V_46 . V_80 [ V_44 ] -> V_63 [ V_45 ] = NULL ;
}
V_423:
while ( V_44 > 0 ) {
F_230 ( V_2 -> V_46 . V_80 [ -- V_44 ] ) ;
F_128 ( V_2 -> V_46 . V_80 [ V_44 ] , 1 ) ;
}
F_32 ( V_2 -> V_46 . V_80 ) ;
V_2 -> V_46 . V_80 = NULL ;
V_422:
F_28 ( V_2 ) ;
V_421:
F_35 ( V_2 -> V_46 . V_419 ) ;
V_2 -> V_46 . V_419 = NULL ;
V_420:
F_60 ( & V_2 -> V_46 . V_3 , V_256 ) ;
return - V_50 ;
}
static void F_132 ( struct V_1 * V_2 )
{
int V_44 , V_45 ;
if ( F_233 ( & V_2 -> V_46 . V_3 , V_256 ) ==
V_256 )
return;
F_48 ( V_2 ) ;
F_234 ( & V_2 -> V_293 ) ;
for ( V_45 = 0 ; V_45 < V_62 ; ++ V_45 ) {
if ( V_2 -> V_46 . V_419 -> V_63 [ V_45 ] . V_426 )
F_127 ( V_2 -> V_46 . V_419 -> V_63 [ V_45 ] . V_426 ) ;
}
F_35 ( V_2 -> V_46 . V_419 ) ;
V_2 -> V_46 . V_419 = NULL ;
F_28 ( V_2 ) ;
if ( V_2 -> V_46 . V_80 ) {
for ( V_44 = 0 ; V_44 < V_2 -> V_46 . V_78 ; ++ V_44 ) {
F_128 ( V_2 -> V_46 . V_80 [ V_44 ] , 1 ) ;
F_230 ( V_2 -> V_46 . V_80 [ V_44 ] ) ;
}
F_32 ( V_2 -> V_46 . V_80 ) ;
V_2 -> V_46 . V_80 = NULL ;
}
}
static void F_235 ( struct V_1 * V_2 ,
char * V_427 )
{
V_427 [ 0 ] = V_345 [ 'P' ] ;
V_427 [ 1 ] = V_345 [ 'C' ] ;
V_427 [ 2 ] = V_345 [ 'I' ] ;
V_427 [ 3 ] = V_345 [ 'T' ] ;
* ( ( unsigned int * ) ( & V_427 [ 4 ] ) ) = F_236 ( V_2 ) ;
* ( ( unsigned int * ) ( & V_427 [ 8 ] ) ) = F_237 ( V_2 ) ;
* ( ( unsigned int * ) ( & V_427 [ 12 ] ) ) = F_238 ( V_2 ) ;
}
static void F_239 ( struct V_1 * V_2 ,
char * V_427 )
{
V_427 [ 16 ] = V_345 [ 'B' ] ;
V_427 [ 17 ] = V_345 [ 'L' ] ;
V_427 [ 18 ] = V_345 [ 'K' ] ;
V_427 [ 19 ] = V_345 [ 'T' ] ;
* ( ( unsigned int * ) ( & V_427 [ 20 ] ) ) = V_2 -> V_12 . V_346 . V_347 ;
* ( ( unsigned int * ) ( & V_427 [ 24 ] ) ) = V_2 -> V_12 . V_346 . V_348 ;
* ( ( unsigned int * ) ( & V_427 [ 28 ] ) ) =
V_2 -> V_12 . V_346 . V_349 ;
}
static int F_240 ( struct V_1 * V_2 )
{
F_41 ( V_65 , 3 , L_182 ) ;
return F_241 ( F_44 ( V_2 ) ) ;
}
static int F_242 ( struct V_1 * V_2 )
{
int V_37 ;
struct V_112 * V_113 ;
F_41 ( V_65 , 2 , L_183 ) ;
V_113 = F_99 ( & V_2 -> V_155 ) ;
memcpy ( V_113 -> V_117 , V_428 , V_429 ) ;
memcpy ( F_243 ( V_113 -> V_117 ) ,
& V_2 -> V_350 . V_394 , V_366 ) ;
memcpy ( F_244 ( V_113 -> V_117 ) ,
& V_2 -> V_350 . V_411 , V_366 ) ;
V_37 = F_189 ( V_2 , V_429 , V_113 , NULL , NULL ) ;
return V_37 ;
}
static int F_245 ( struct V_1 * V_2 )
{
int V_37 ;
F_41 ( V_65 , 2 , L_184 ) ;
V_37 = F_65 ( V_2 ) ;
if ( V_37 ) {
F_45 ( V_65 , 2 , L_37 , V_37 ) ;
return V_37 ;
}
V_37 = F_196 ( V_2 ) ;
if ( V_37 ) {
F_45 ( V_65 , 2 , L_135 , V_37 ) ;
goto V_430;
}
V_37 = F_201 ( V_2 ) ;
if ( V_37 ) {
F_45 ( V_65 , 2 , L_140 , V_37 ) ;
goto V_430;
}
V_37 = F_214 ( V_2 ) ;
if ( V_37 ) {
F_45 ( V_65 , 2 , L_185 , V_37 ) ;
goto V_430;
}
V_37 = F_221 ( V_2 ) ;
if ( V_37 ) {
F_45 ( V_65 , 2 , L_186 , V_37 ) ;
goto V_430;
}
V_37 = F_232 ( V_2 ) ;
if ( V_37 ) {
F_45 ( V_65 , 2 , L_186 , V_37 ) ;
goto V_430;
}
V_37 = F_246 ( V_2 ) ;
if ( V_37 ) {
F_45 ( V_65 , 2 , L_187 , V_37 ) ;
F_132 ( V_2 ) ;
goto V_430;
}
V_37 = F_240 ( V_2 ) ;
if ( V_37 ) {
F_45 ( V_65 , 2 , L_188 , V_37 ) ;
goto V_430;
}
V_37 = F_242 ( V_2 ) ;
if ( V_37 ) {
F_45 ( V_65 , 2 , L_189 , V_37 ) ;
goto V_430;
}
return 0 ;
V_430:
F_162 ( V_2 , V_2 -> V_12 . type != V_15 ) ;
F_247 ( F_44 ( V_2 ) ) ;
return V_37 ;
}
void F_248 ( struct V_1 * V_2 )
{
switch ( V_2 -> V_12 . type ) {
case V_14 :
case V_16 :
case V_17 :
if ( ! V_2 -> V_12 . V_300 [ 0 ] ) {
sprintf ( V_2 -> V_12 . V_300 , L_190 ,
V_2 -> V_12 . V_300 [ 2 ] ,
V_2 -> V_12 . V_300 [ 3 ] ) ;
break;
}
case V_15 :
if ( ( V_2 -> V_12 . V_13 ) ||
( V_2 -> V_12 . V_300 [ 0 ] & 0x80 ) ) {
V_2 -> V_12 . V_300 [ 0 ] = ( char ) V_431 [ ( T_2 )
V_2 -> V_12 . V_300 [ 0 ] ] ;
V_2 -> V_12 . V_300 [ 1 ] = ( char ) V_431 [ ( T_2 )
V_2 -> V_12 . V_300 [ 1 ] ] ;
V_2 -> V_12 . V_300 [ 2 ] = ( char ) V_431 [ ( T_2 )
V_2 -> V_12 . V_300 [ 2 ] ] ;
V_2 -> V_12 . V_300 [ 3 ] = ( char ) V_431 [ ( T_2 )
V_2 -> V_12 . V_300 [ 3 ] ] ;
V_2 -> V_12 . V_300 [ V_374 ] = 0 ;
}
break;
default:
memset ( & V_2 -> V_12 . V_300 [ 0 ] , 0 , V_374 + 1 ) ;
}
F_89 ( & V_2 -> V_10 -> V_9 ,
L_191 ,
F_11 ( V_2 ) ,
( V_2 -> V_12 . V_300 [ 0 ] ) ? L_192 : L_74 ,
( V_2 -> V_12 . V_300 [ 0 ] ) ? V_2 -> V_12 . V_300 : L_74 ,
( V_2 -> V_12 . V_300 [ 0 ] ) ? L_193 : L_74 ,
F_12 ( V_2 ) ) ;
}
static void F_249 ( struct V_1 * V_2 )
{
struct V_39 * V_432 ;
F_4 ( V_2 , 5 , L_194 ) ;
F_250 (entry,
&card->qdio.init_pool.entry_list, init_list) {
F_251 ( V_2 , V_432 ) ;
}
}
static inline struct V_39 * F_252 (
struct V_1 * V_2 )
{
struct V_433 * V_434 ;
struct V_39 * V_432 ;
int V_44 , free ;
struct V_435 * V_435 ;
if ( F_253 ( & V_2 -> V_46 . V_58 . V_53 ) )
return NULL ;
F_254 (plh, &card->qdio.in_buf_pool.entry_list) {
V_432 = F_255 ( V_434 , struct V_39 , V_42 ) ;
free = 1 ;
for ( V_44 = 0 ; V_44 < F_29 ( V_2 ) ; ++ V_44 ) {
if ( F_256 ( F_257 ( V_432 -> V_51 [ V_44 ] ) ) > 1 ) {
free = 0 ;
break;
}
}
if ( free ) {
F_92 ( & V_432 -> V_42 ) ;
return V_432 ;
}
}
V_432 = F_255 ( V_2 -> V_46 . V_58 . V_53 . V_436 ,
struct V_39 , V_42 ) ;
for ( V_44 = 0 ; V_44 < F_29 ( V_2 ) ; ++ V_44 ) {
if ( F_256 ( F_257 ( V_432 -> V_51 [ V_44 ] ) ) > 1 ) {
V_435 = F_258 ( V_126 ) ;
if ( ! V_435 ) {
return NULL ;
} else {
F_31 ( ( unsigned long ) V_432 -> V_51 [ V_44 ] ) ;
V_432 -> V_51 [ V_44 ] = F_259 ( V_435 ) ;
if ( V_2 -> V_66 . V_273 )
V_2 -> V_437 . V_438 ++ ;
}
}
}
F_92 ( & V_432 -> V_42 ) ;
return V_432 ;
}
static int F_260 ( struct V_1 * V_2 ,
struct V_439 * V_231 )
{
struct V_39 * V_40 ;
int V_44 ;
if ( ( V_2 -> V_66 . V_67 == V_68 ) && ( ! V_231 -> V_426 ) ) {
V_231 -> V_426 = F_261 ( V_440 + V_441 ) ;
if ( ! V_231 -> V_426 )
return 1 ;
}
V_40 = F_252 ( V_2 ) ;
if ( ! V_40 )
return 1 ;
V_231 -> V_40 = V_40 ;
for ( V_44 = 0 ; V_44 < F_29 ( V_2 ) ; ++ V_44 ) {
V_231 -> V_64 -> V_245 [ V_44 ] . V_253 = V_403 ;
V_231 -> V_64 -> V_245 [ V_44 ] . V_250 = V_40 -> V_51 [ V_44 ] ;
if ( V_44 == F_29 ( V_2 ) - 1 )
V_231 -> V_64 -> V_245 [ V_44 ] . V_254 = V_442 ;
else
V_231 -> V_64 -> V_245 [ V_44 ] . V_254 = 0 ;
V_231 -> V_64 -> V_245 [ V_44 ] . V_246 = 0 ;
}
return 0 ;
}
int F_262 ( struct V_1 * V_2 )
{
int V_44 , V_45 ;
int V_37 ;
F_41 ( V_65 , 2 , L_195 ) ;
F_42 ( V_2 -> V_46 . V_419 -> V_61 ,
V_62 ) ;
F_249 ( V_2 ) ;
for ( V_44 = 0 ; V_44 < V_2 -> V_46 . V_58 . V_48 - 1 ; ++ V_44 )
F_260 ( V_2 , & V_2 -> V_46 . V_419 -> V_63 [ V_44 ] ) ;
V_2 -> V_46 . V_419 -> V_70 =
V_2 -> V_46 . V_58 . V_48 - 1 ;
V_37 = F_43 ( F_44 ( V_2 ) , V_71 , 0 , 0 ,
V_2 -> V_46 . V_58 . V_48 - 1 ) ;
if ( V_37 ) {
F_45 ( V_65 , 2 , L_37 , V_37 ) ;
return V_37 ;
}
V_37 = F_40 ( V_2 ) ;
if ( V_37 ) {
return V_37 ;
}
for ( V_44 = 0 ; V_44 < V_2 -> V_46 . V_78 ; ++ V_44 ) {
F_42 ( V_2 -> V_46 . V_80 [ V_44 ] -> V_61 ,
V_62 ) ;
for ( V_45 = 0 ; V_45 < V_62 ; ++ V_45 ) {
F_62 ( V_2 -> V_46 . V_80 [ V_44 ] ,
V_2 -> V_46 . V_80 [ V_44 ] -> V_63 [ V_45 ] ,
V_249 ) ;
}
V_2 -> V_46 . V_80 [ V_44 ] -> V_2 = V_2 ;
V_2 -> V_46 . V_80 [ V_44 ] -> V_443 = 0 ;
V_2 -> V_46 . V_80 [ V_44 ] -> V_444 = 0 ;
F_60 ( & V_2 -> V_46 . V_80 [ V_44 ] -> V_445 , 0 ) ;
F_60 ( & V_2 -> V_46 . V_80 [ V_44 ] -> V_248 , 0 ) ;
F_60 ( & V_2 -> V_46 . V_80 [ V_44 ] -> V_3 ,
V_446 ) ;
}
return 0 ;
}
static inline T_2 F_263 ( enum V_447 V_19 )
{
switch ( V_19 ) {
case V_21 :
return 2 ;
default:
return 1 ;
}
}
static void F_264 ( struct V_1 * V_2 ,
struct V_129 * V_130 , T_2 V_134 ,
enum V_448 V_449 )
{
memset ( V_130 , 0 , sizeof( struct V_129 ) ) ;
V_130 -> V_133 . V_134 = V_134 ;
V_130 -> V_133 . V_450 = V_451 ;
V_130 -> V_133 . V_182 = V_2 -> V_182 . V_383 ;
V_130 -> V_133 . V_452 = F_263 ( V_2 -> V_12 . V_19 ) ;
V_130 -> V_133 . V_453 = ( T_2 ) V_2 -> V_12 . V_365 ;
if ( V_2 -> V_66 . V_303 )
V_130 -> V_133 . V_454 = 2 ;
else
V_130 -> V_133 . V_454 = 1 ;
V_130 -> V_133 . V_455 = 1 ;
V_130 -> V_133 . V_388 = V_449 ;
V_130 -> V_133 . V_456 = 0 ;
V_130 -> V_133 . V_457 = 0 ;
}
struct V_112 * F_265 ( struct V_1 * V_2 ,
enum V_458 V_459 , enum V_448 V_449 )
{
struct V_112 * V_113 ;
struct V_129 * V_130 ;
V_113 = F_66 ( & V_2 -> V_155 ) ;
if ( V_113 ) {
V_130 = (struct V_129 * ) ( V_113 -> V_117 + V_386 ) ;
F_264 ( V_2 , V_130 , V_459 , V_449 ) ;
} else {
F_67 ( & V_2 -> V_10 -> V_9 ,
L_196 ) ;
F_68 ( 1 , L_197 ,
F_69 ( & V_2 -> V_10 -> V_9 ) ) ;
}
return V_113 ;
}
void F_266 ( struct V_1 * V_2 , struct V_112 * V_113 ,
char V_405 )
{
memcpy ( V_113 -> V_117 , V_460 , V_386 ) ;
memcpy ( F_267 ( V_113 -> V_117 ) , & V_405 , 1 ) ;
memcpy ( F_268 ( V_113 -> V_117 ) ,
& V_2 -> V_350 . V_411 , V_366 ) ;
}
int F_269 ( struct V_1 * V_2 , struct V_112 * V_113 ,
int (* F_190)( struct V_1 * , struct V_124 * ,
unsigned long ) ,
void * V_379 )
{
int V_37 ;
char V_405 ;
F_4 ( V_2 , 4 , L_198 ) ;
if ( V_2 -> V_66 . V_303 )
if ( V_2 -> V_12 . type == V_18 )
V_405 = V_408 ;
else
V_405 = V_409 ;
else
V_405 = V_410 ;
F_266 ( V_2 , V_113 , V_405 ) ;
V_37 = F_189 ( V_2 , V_461 ,
V_113 , F_190 , V_379 ) ;
if ( V_37 == - V_321 ) {
F_91 ( V_2 ) ;
F_72 ( V_2 ) ;
}
return V_37 ;
}
static int F_270 ( struct V_1 * V_2 )
{
int V_37 ;
struct V_112 * V_113 ;
F_41 ( V_65 , 2 , L_199 ) ;
V_113 = F_265 ( V_2 , V_144 , 0 ) ;
if ( ! V_113 )
return - V_50 ;
V_37 = F_269 ( V_2 , V_113 , NULL , NULL ) ;
return V_37 ;
}
static int F_271 ( struct V_1 * V_2 ,
struct V_124 * V_125 , unsigned long V_117 )
{
struct V_129 * V_130 ;
F_4 ( V_2 , 4 , L_200 ) ;
V_130 = (struct V_129 * ) V_117 ;
if ( V_130 -> V_133 . V_139 == 0 )
V_130 -> V_133 . V_139 =
V_130 -> V_117 . V_462 . V_133 . V_139 ;
return 0 ;
}
static int F_272 ( struct V_1 * V_2 ,
struct V_124 * V_125 , unsigned long V_117 )
{
struct V_129 * V_130 ;
F_4 ( V_2 , 3 , L_201 ) ;
V_130 = (struct V_129 * ) V_117 ;
if ( V_130 -> V_117 . V_462 . V_117 . V_463 . V_464 & 0x7f ) {
V_2 -> V_12 . V_19 =
V_130 -> V_117 . V_462 . V_117 . V_463 . V_464 ;
F_45 ( V_65 , 2 , L_202 , V_2 -> V_12 . V_19 ) ;
}
V_2 -> V_66 . V_465 . V_466 =
V_130 -> V_117 . V_462 . V_117 . V_463 . V_467 ;
return F_271 ( V_2 , V_125 , ( unsigned long ) V_130 ) ;
}
static struct V_112 * F_273 ( struct V_1 * V_2 ,
T_1 V_134 , T_1 V_468 )
{
struct V_112 * V_113 ;
struct V_129 * V_130 ;
V_113 = F_265 ( V_2 , V_469 ,
V_389 ) ;
if ( V_113 ) {
V_130 = (struct V_129 * ) ( V_113 -> V_117 + V_386 ) ;
V_130 -> V_117 . V_462 . V_133 . V_470 = V_468 ;
V_130 -> V_117 . V_462 . V_133 . V_471 = V_134 ;
V_130 -> V_117 . V_462 . V_133 . V_472 = 1 ;
V_130 -> V_117 . V_462 . V_133 . V_473 = 1 ;
}
return V_113 ;
}
int F_274 ( struct V_1 * V_2 )
{
int V_37 ;
struct V_112 * V_113 ;
F_4 ( V_2 , 3 , L_203 ) ;
V_113 = F_273 ( V_2 , V_474 ,
sizeof( struct V_475 ) ) ;
if ( ! V_113 )
return - V_50 ;
V_37 = F_269 ( V_2 , V_113 , F_272 , NULL ) ;
return V_37 ;
}
static int F_275 ( struct V_1 * V_2 ,
struct V_124 * V_125 , unsigned long V_117 )
{
struct V_129 * V_130 ;
F_41 ( V_65 , 2 , L_204 ) ;
V_130 = (struct V_129 * ) V_117 ;
switch ( V_130 -> V_133 . V_139 ) {
case V_476 :
case V_477 :
F_41 ( V_65 , 2 , L_205 ) ;
V_2 -> V_66 . V_478 . V_466 |= V_479 ;
V_2 -> V_66 . V_480 . V_466 |= V_479 ;
return - 0 ;
default:
if ( V_130 -> V_133 . V_139 ) {
F_68 ( 1 , L_206
L_207 ,
F_69 ( & V_2 -> V_10 -> V_9 ) ,
V_130 -> V_133 . V_139 ) ;
return 0 ;
}
}
if ( V_130 -> V_133 . V_388 == V_389 ) {
V_2 -> V_66 . V_478 . V_466 = V_130 -> V_133 . V_456 ;
V_2 -> V_66 . V_478 . V_481 = V_130 -> V_133 . V_457 ;
} else if ( V_130 -> V_133 . V_388 == V_482 ) {
V_2 -> V_66 . V_480 . V_466 = V_130 -> V_133 . V_456 ;
V_2 -> V_66 . V_480 . V_481 = V_130 -> V_133 . V_457 ;
} else
F_68 ( 1 , L_208
L_209 , F_69 ( & V_2 -> V_10 -> V_9 ) ) ;
return 0 ;
}
int F_276 ( struct V_1 * V_2 , enum V_448 V_449 )
{
int V_37 ;
struct V_112 * V_113 ;
F_45 ( V_65 , 2 , L_210 , V_449 ) ;
V_113 = F_265 ( V_2 , V_483 , V_449 ) ;
if ( ! V_113 )
return - V_50 ;
V_37 = F_269 ( V_2 , V_113 , F_275 , NULL ) ;
return V_37 ;
}
static int F_277 ( struct V_1 * V_2 ,
struct V_124 * V_125 , unsigned long V_117 )
{
struct V_129 * V_130 ;
struct V_484 * V_485 ;
struct V_486 * V_487 ;
F_4 ( V_2 , 2 , L_211 ) ;
V_130 = (struct V_129 * ) V_117 ;
V_485 = (struct V_484 * ) V_125 -> V_382 ;
if ( V_130 -> V_117 . V_462 . V_133 . V_139 == 0 ) {
V_487 = & V_130 -> V_117 . V_462 . V_117 . V_488 ;
V_485 -> V_489 = V_487 -> V_489 ;
V_485 -> V_490 = V_487 -> V_490 ;
F_52 ( V_2 , 2 , L_212 , V_485 -> V_489 ,
V_485 -> V_490 ) ;
}
F_271 ( V_2 , V_125 , ( unsigned long ) V_130 ) ;
return 0 ;
}
int V_486 ( struct V_1 * V_2 ,
struct V_484 * V_485 )
{
struct V_112 * V_113 ;
F_4 ( V_2 , 2 , L_213 ) ;
if ( ! F_278 ( V_2 , V_491 ) )
return - V_336 ;
if ( ! F_87 ( V_2 -> V_9 ) )
return - V_492 ;
V_113 = F_273 ( V_2 , V_491 ,
sizeof( struct V_493 ) ) ;
if ( ! V_113 )
return - V_50 ;
return F_269 ( V_2 , V_113 ,
F_277 , V_485 ) ;
}
static int F_279 ( struct V_1 * V_2 ,
struct V_124 * V_125 , unsigned long V_117 )
{
struct V_129 * V_130 ;
V_185 V_37 ;
V_130 = (struct V_129 * ) V_117 ;
V_37 = V_130 -> V_133 . V_139 ;
if ( V_37 )
F_52 ( V_2 , 2 , L_214 , V_37 ) ;
else
V_2 -> V_12 . V_494 = V_130 -> V_117 . V_495 . V_496 ;
return 0 ;
}
static int F_280 ( struct V_1 * V_2 )
{
struct V_112 * V_113 ;
struct V_129 * V_130 ;
F_41 ( V_65 , 2 , L_215 ) ;
V_113 = F_265 ( V_2 , V_138 , 0 ) ;
if ( ! V_113 )
return - V_50 ;
V_130 = (struct V_129 * ) ( V_113 -> V_117 + V_386 ) ;
V_130 -> V_117 . V_495 . V_497 = 16 ;
V_130 -> V_117 . V_495 . V_498 = V_499 ;
return F_269 ( V_2 , V_113 , F_279 , NULL ) ;
}
static void F_281 ( struct V_1 * V_2 , struct V_500 * V_501 )
{
unsigned long V_12 = F_282 ( V_49 ) ;
struct V_502 * V_503 = (struct V_502 * ) V_12 ;
struct V_504 * V_505 = (struct V_504 * ) V_12 ;
struct V_359 V_506 ;
int V_369 ;
V_501 -> V_142 = V_2 -> V_12 . V_142 ;
F_178 ( F_152 ( V_2 ) , & V_506 ) ;
V_501 -> V_507 = V_506 . V_507 ;
V_501 -> V_367 = V_506 . V_367 ;
if ( ! V_12 )
return;
V_369 = F_283 ( NULL , 0 , 0 , 0 ) ;
if ( ( V_369 >= 2 ) && ( F_283 ( V_503 , 2 , 2 , 2 ) == 0 ) )
V_501 -> V_508 = V_503 -> V_509 ;
if ( ( V_369 >= 3 ) && ( F_283 ( V_505 , 3 , 2 , 2 ) == 0 ) ) {
F_284 ( V_505 -> V_510 [ 0 ] . V_511 , sizeof( V_505 -> V_510 [ 0 ] . V_511 ) ) ;
memcpy ( V_501 -> V_512 , V_505 -> V_510 [ 0 ] . V_511 , sizeof( V_501 -> V_512 ) ) ;
}
F_31 ( V_12 ) ;
return;
}
static int F_285 ( struct V_1 * V_2 ,
struct V_124 * V_125 , unsigned long V_117 )
{
struct V_129 * V_130 ;
V_185 V_37 ;
V_130 = (struct V_129 * ) V_117 ;
V_37 = V_130 -> V_133 . V_139 ;
if ( V_37 )
F_52 ( V_2 , 2 , L_216 , V_37 ) ;
return 0 ;
}
int F_286 ( struct V_1 * V_2 , enum V_513 V_514 )
{
struct V_112 * V_113 ;
struct V_129 * V_130 ;
F_41 ( V_65 , 2 , L_217 ) ;
V_113 = F_265 ( V_2 , V_138 , 0 ) ;
if ( ! V_113 )
return - V_50 ;
V_130 = (struct V_129 * ) ( V_113 -> V_117 + V_386 ) ;
V_130 -> V_117 . V_495 . V_497 = 80 ;
V_130 -> V_117 . V_495 . V_498 = V_515 ;
V_130 -> V_117 . V_495 . type = 1 ;
V_130 -> V_117 . V_495 . V_514 = V_514 ;
switch ( V_514 ) {
case V_516 :
V_130 -> V_117 . V_495 . V_66 = 0x0003 ;
V_130 -> V_117 . V_495 . V_496 = 0x00010000 +
sizeof( struct V_500 ) ;
F_281 ( V_2 ,
(struct V_500 * ) V_130 -> V_117 . V_495 . V_517 ) ;
break;
case V_518 :
V_130 -> V_117 . V_495 . V_66 = 0x0001 ;
break;
case V_519 :
break;
}
return F_269 ( V_2 , V_113 , F_285 , NULL ) ;
}
int F_287 ( struct V_1 * V_2 , struct V_520 * V_231 ,
unsigned int V_521 , const char * V_522 )
{
if ( V_521 ) {
F_4 ( V_2 , 2 , V_522 ) ;
F_52 ( V_2 , 2 , L_218 ,
V_231 -> V_245 [ 15 ] . V_246 ) ;
F_52 ( V_2 , 2 , L_219 ,
V_231 -> V_245 [ 14 ] . V_246 ) ;
F_52 ( V_2 , 2 , L_220 , V_521 ) ;
if ( ( V_231 -> V_245 [ 15 ] . V_246 ) == 0x12 ) {
V_2 -> V_523 . V_524 ++ ;
return 0 ;
} else
return 1 ;
}
return 0 ;
}
static void V_294 ( struct V_6 * V_7 )
{
struct V_1 * V_2 = F_3 ( V_7 , struct V_1 ,
V_293 . V_7 ) ;
F_52 ( V_2 , 2 , L_221 , V_2 -> V_525 ) ;
F_288 ( V_2 , V_2 -> V_525 ) ;
}
void F_288 ( struct V_1 * V_2 , int V_169 )
{
struct V_59 * V_111 = V_2 -> V_46 . V_419 ;
struct V_433 * V_526 ;
int V_167 ;
int V_44 ;
int V_37 ;
int V_527 = 0 ;
V_167 = ( V_169 < V_111 -> V_70 ) ?
V_2 -> V_46 . V_58 . V_48 -
( V_111 -> V_70 - V_169 ) :
V_2 -> V_46 . V_58 . V_48 -
( V_111 -> V_70 + V_62 - V_169 ) ;
if ( V_167 >= F_289 ( V_2 ) ) {
for ( V_44 = V_111 -> V_70 ;
V_44 < V_111 -> V_70 + V_167 ; ++ V_44 ) {
if ( F_260 ( V_2 ,
& V_111 -> V_63 [ V_44 % V_62 ] ) ) {
break;
} else {
V_527 ++ ;
}
}
if ( V_527 < V_167 ) {
F_60 ( & V_2 -> V_528 , 3 ) ;
V_167 = V_527 ;
} else {
F_290 ( & V_2 -> V_528 , - 1 , 0 ) ;
}
if ( ! V_167 ) {
V_44 = 0 ;
F_254 (lh, &card->qdio.in_buf_pool.entry_list)
V_44 ++ ;
if ( V_44 == V_2 -> V_46 . V_58 . V_48 ) {
F_4 ( V_2 , 2 , L_222 ) ;
V_2 -> V_525 = V_169 ;
F_291 (
& V_2 -> V_293 ,
V_529 ) ;
}
return;
}
if ( V_2 -> V_66 . V_273 ) {
V_2 -> V_437 . V_530 ++ ;
V_2 -> V_437 . V_531 =
F_292 () ;
}
V_37 = F_43 ( F_44 ( V_2 ) , V_71 , 0 ,
V_111 -> V_70 , V_167 ) ;
if ( V_2 -> V_66 . V_273 )
V_2 -> V_437 . V_532 +=
F_292 () -
V_2 -> V_437 . V_531 ;
if ( V_37 ) {
F_4 ( V_2 , 2 , L_223 ) ;
}
V_111 -> V_70 = ( V_111 -> V_70 + V_167 ) %
V_62 ;
}
}
static int F_293 ( struct V_1 * V_2 ,
struct V_96 * V_64 , unsigned int V_533 )
{
int V_83 = V_64 -> V_64 -> V_245 [ 15 ] . V_246 ;
F_4 ( V_2 , 6 , L_224 ) ;
if ( V_2 -> V_12 . type == V_15 ) {
if ( V_83 == 0 ) {
V_533 = 0 ;
} else {
V_533 = 1 ;
}
}
F_287 ( V_2 , V_64 -> V_64 , V_533 , L_225 ) ;
if ( ! V_533 )
return V_534 ;
if ( ( V_83 >= 15 ) && ( V_83 <= 31 ) )
return V_535 ;
F_4 ( V_2 , 1 , L_226 ) ;
F_52 ( V_2 , 1 , L_227 ,
( V_186 ) V_533 , ( V_279 ) V_83 ) ;
return V_536 ;
}
static void F_294 ( struct V_92 * V_111 )
{
if ( ! V_111 -> V_444 ) {
if ( F_51 ( & V_111 -> V_445 )
>= V_537 ) {
F_4 ( V_111 -> V_2 , 6 , L_228 ) ;
if ( V_111 -> V_2 -> V_66 . V_273 )
V_111 -> V_2 -> V_437 . V_538 ++ ;
V_111 -> V_444 = 1 ;
}
}
}
static int F_295 ( struct V_92 * V_111 )
{
struct V_96 * V_64 ;
int V_539 = 0 ;
if ( V_111 -> V_444 ) {
if ( F_51 ( & V_111 -> V_445 )
<= V_540 ) {
F_4 ( V_111 -> V_2 , 6 , L_229 ) ;
if ( V_111 -> V_2 -> V_66 . V_273 )
V_111 -> V_2 -> V_437 . V_541 ++ ;
V_111 -> V_444 = 0 ;
V_64 = V_111 -> V_63 [ V_111 -> V_443 ] ;
if ( ( F_51 ( & V_64 -> V_3 ) ==
V_249 ) &&
( V_64 -> V_255 > 0 ) ) {
F_60 ( & V_64 -> V_3 ,
V_107 ) ;
V_539 ++ ;
V_111 -> V_443 =
( V_111 -> V_443 + 1 ) %
V_62 ;
}
}
}
return V_539 ;
}
static int F_296 ( struct V_92 * V_111 )
{
struct V_96 * V_64 ;
V_64 = V_111 -> V_63 [ V_111 -> V_443 ] ;
if ( ( F_51 ( & V_64 -> V_3 ) == V_249 ) &&
( V_64 -> V_255 > 0 ) ) {
F_60 ( & V_64 -> V_3 , V_107 ) ;
V_111 -> V_443 =
( V_111 -> V_443 + 1 ) % V_62 ;
return 1 ;
}
return 0 ;
}
static void F_297 ( struct V_92 * V_111 , int V_169 ,
int V_167 )
{
struct V_96 * V_231 ;
int V_37 ;
int V_44 ;
unsigned int V_542 ;
for ( V_44 = V_169 ; V_44 < V_169 + V_167 ; ++ V_44 ) {
int V_93 = V_44 % V_62 ;
V_231 = V_111 -> V_63 [ V_93 ] ;
V_231 -> V_64 -> V_245 [ V_231 -> V_255 - 1 ] . V_254 |=
V_442 ;
if ( V_111 -> V_81 )
V_111 -> V_81 [ V_93 ] . V_418 = V_231 ;
if ( V_111 -> V_2 -> V_12 . type == V_15 )
continue;
if ( ! V_111 -> V_444 ) {
if ( ( F_51 ( & V_111 -> V_445 ) >=
( V_537 -
V_543 ) ) &&
! F_51 ( & V_111 -> V_248 ) ) {
F_76 ( & V_111 -> V_248 ) ;
V_231 -> V_64 -> V_245 [ 0 ] . V_246 |= V_247 ;
}
} else {
if ( ! F_51 ( & V_111 -> V_248 ) ) {
F_76 ( & V_111 -> V_248 ) ;
V_231 -> V_64 -> V_245 [ 0 ] . V_246 |= V_247 ;
}
}
}
F_298 ( V_111 -> V_2 -> V_9 ) ;
if ( V_111 -> V_2 -> V_66 . V_273 ) {
V_111 -> V_2 -> V_437 . V_544 ++ ;
V_111 -> V_2 -> V_437 . V_545 =
F_292 () ;
}
V_542 = V_546 ;
if ( F_51 ( & V_111 -> V_248 ) )
V_542 |= V_547 ;
V_37 = F_43 ( F_44 ( V_111 -> V_2 ) , V_542 ,
V_111 -> V_424 , V_169 , V_167 ) ;
if ( V_111 -> V_2 -> V_66 . V_273 )
V_111 -> V_2 -> V_437 . V_548 +=
F_292 () -
V_111 -> V_2 -> V_437 . V_545 ;
F_299 ( V_167 , & V_111 -> V_445 ) ;
if ( V_37 ) {
V_111 -> V_2 -> V_523 . V_549 += V_167 ;
if ( V_37 == - V_550 )
return;
F_4 ( V_111 -> V_2 , 2 , L_230 ) ;
F_52 ( V_111 -> V_2 , 2 , L_231 , V_111 -> V_424 ) ;
F_52 ( V_111 -> V_2 , 2 , L_232 , V_169 ) ;
F_52 ( V_111 -> V_2 , 2 , L_233 , V_167 ) ;
F_52 ( V_111 -> V_2 , 2 , L_164 , V_37 ) ;
F_72 ( V_111 -> V_2 ) ;
return;
}
if ( V_111 -> V_2 -> V_66 . V_273 )
V_111 -> V_2 -> V_437 . V_551 += V_167 ;
}
static void F_300 ( struct V_92 * V_111 )
{
int V_169 ;
int V_552 = 0 ;
int V_553 = 0 ;
if ( ( F_51 ( & V_111 -> V_445 ) <= V_540 ) ||
! F_51 ( & V_111 -> V_248 ) ) {
if ( F_233 ( & V_111 -> V_3 , V_554 ) ==
V_446 ) {
F_301 ( V_111 -> V_2 -> V_9 ) ;
V_169 = V_111 -> V_443 ;
V_553 = V_111 -> V_444 ;
F_302 () ;
V_552 += F_295 ( V_111 ) ;
if ( ! V_552 &&
! F_51 ( & V_111 -> V_248 ) )
V_552 +=
F_296 ( V_111 ) ;
if ( V_111 -> V_2 -> V_66 . V_273 &&
V_553 )
V_111 -> V_2 -> V_437 . V_555 +=
V_552 ;
if ( V_552 )
F_297 ( V_111 , V_169 , V_552 ) ;
F_60 ( & V_111 -> V_3 , V_446 ) ;
}
}
}
void F_303 ( struct V_158 * V_119 , int V_111 ,
unsigned long V_556 )
{
struct V_1 * V_2 = (struct V_1 * ) V_556 ;
if ( V_2 -> V_9 && ( V_2 -> V_9 -> V_32 & V_557 ) )
F_304 ( & V_2 -> V_558 ) ;
}
int F_305 ( struct V_1 * V_2 , enum V_559 V_67 )
{
int V_37 ;
if ( V_2 -> V_66 . V_67 == V_560 ) {
V_37 = - 1 ;
goto V_73;
} else {
if ( V_2 -> V_66 . V_67 == V_67 ) {
V_37 = 0 ;
goto V_73;
}
if ( V_2 -> V_3 != V_55 &&
V_2 -> V_3 != V_56 ) {
V_37 = - 1 ;
goto V_73;
}
F_132 ( V_2 ) ;
V_2 -> V_66 . V_67 = V_67 ;
V_37 = 0 ;
}
V_73:
return V_37 ;
}
static void F_306 ( struct V_1 * V_2 ,
unsigned int V_533 ,
unsigned int V_111 , int V_561 , int V_167 ) {
struct V_59 * V_67 = V_2 -> V_46 . V_69 ;
int V_44 ;
int V_37 ;
if ( ! F_64 ( V_2 , V_111 ) )
goto V_73;
F_52 ( V_2 , 5 , L_234 , V_561 ) ;
F_52 ( V_2 , 5 , L_235 , V_167 ) ;
F_52 ( V_2 , 5 , L_236 , V_533 ) ;
if ( V_533 ) {
F_301 ( V_2 -> V_9 ) ;
F_72 ( V_2 ) ;
goto V_73;
}
if ( V_2 -> V_66 . V_273 ) {
V_2 -> V_437 . V_562 ++ ;
V_2 -> V_437 . V_563 = F_292 () ;
}
for ( V_44 = V_561 ; V_44 < V_561 + V_167 ; ++ V_44 ) {
int V_93 = V_44 % V_62 ;
struct V_520 * V_64 = V_67 -> V_61 [ V_93 ] ;
int V_564 ;
V_564 = 0 ;
while ( ( V_564 < V_565 ) &&
V_64 -> V_245 [ V_564 ] . V_250 ) {
unsigned long V_103 ;
V_103 = ( unsigned long ) V_64 -> V_245 [ V_564 ] . V_250 ;
F_57 ( V_2 , V_103 ) ;
V_64 -> V_245 [ V_564 ] . V_250 = NULL ;
V_64 -> V_245 [ V_564 ] . V_254 = 0 ;
V_64 -> V_245 [ V_564 ] . V_246 = 0 ;
V_64 -> V_245 [ V_564 ] . V_253 = 0 ;
++ V_564 ;
}
V_64 -> V_245 [ 15 ] . V_254 = 0 ;
V_64 -> V_245 [ 15 ] . V_246 = 0 ;
}
V_37 = F_43 ( F_44 ( V_2 ) , V_71 , V_111 ,
V_2 -> V_46 . V_69 -> V_70 ,
V_167 ) ;
if ( V_37 ) {
F_67 ( & V_2 -> V_10 -> V_9 ,
L_237 , V_37 ) ;
F_4 ( V_2 , 2 , L_238 ) ;
}
V_2 -> V_46 . V_69 -> V_70 = ( V_2 -> V_46 . V_69 -> V_70
+ V_167 ) % V_62 ;
F_307 ( V_2 -> V_9 ) ;
if ( V_2 -> V_66 . V_273 ) {
int V_566 = F_292 () ;
V_566 -= V_2 -> V_437 . V_563 ;
V_2 -> V_437 . V_567 += V_566 ;
}
V_73:
return;
}
void F_308 ( struct V_158 * V_119 , unsigned int V_533 ,
unsigned int V_111 , int V_568 , int V_167 ,
unsigned long V_556 )
{
struct V_1 * V_2 = (struct V_1 * ) V_556 ;
F_52 ( V_2 , 2 , L_239 , V_111 ) ;
F_52 ( V_2 , 2 , L_240 , V_533 ) ;
if ( F_64 ( V_2 , V_111 ) )
F_306 ( V_2 , V_533 , V_111 , V_568 , V_167 ) ;
else if ( V_533 )
F_72 ( V_2 ) ;
}
void F_309 ( struct V_158 * V_119 ,
unsigned int V_521 , int V_569 , int V_561 ,
int V_167 , unsigned long V_556 )
{
struct V_1 * V_2 = (struct V_1 * ) V_556 ;
struct V_92 * V_111 = V_2 -> V_46 . V_80 [ V_569 ] ;
struct V_96 * V_64 ;
int V_44 ;
F_4 ( V_2 , 6 , L_241 ) ;
if ( V_521 & V_570 ) {
F_4 ( V_2 , 2 , L_242 ) ;
F_301 ( V_2 -> V_9 ) ;
F_72 ( V_2 ) ;
return;
}
if ( V_2 -> V_66 . V_273 ) {
V_2 -> V_437 . V_571 ++ ;
V_2 -> V_437 . V_572 =
F_292 () ;
}
for ( V_44 = V_561 ; V_44 < ( V_561 + V_167 ) ; ++ V_44 ) {
int V_93 = V_44 % V_62 ;
V_64 = V_111 -> V_63 [ V_93 ] ;
F_293 ( V_2 , V_64 , V_521 ) ;
if ( V_111 -> V_81 &&
( V_111 -> V_81 [ V_93 ] . V_32 &
V_573 ) != 0 ) {
F_54 ( V_2 -> V_66 . V_67 != V_68 ) ;
if ( F_59 ( & V_64 -> V_3 ,
V_107 ,
V_109 ) ==
V_107 ) {
F_61 ( V_111 , V_64 ,
V_574 ) ;
}
V_64 -> V_102 = V_111 -> V_81 [ V_93 ] . V_102 ;
F_52 ( V_111 -> V_2 , 5 , L_243 , V_93 ) ;
F_4 ( V_111 -> V_2 , 5 , L_244 ) ;
F_52 ( V_111 -> V_2 , 5 , L_43 ,
F_310 ( V_64 -> V_102 ) ) ;
if ( F_56 ( V_111 , V_93 ) ) {
F_4 ( V_2 , 2 , L_245 ) ;
F_72 ( V_2 ) ;
}
} else {
if ( V_2 -> V_66 . V_67 == V_68 ) {
enum V_82 V_85 ;
V_85 = F_49 (
V_64 -> V_64 -> V_245 [ 15 ] . V_246 , 0 ) ;
F_61 ( V_111 , V_64 , V_85 ) ;
}
F_62 ( V_111 , V_64 ,
V_249 ) ;
}
F_50 ( V_111 , V_93 , 0 ) ;
}
F_311 ( V_167 , & V_111 -> V_445 ) ;
if ( V_2 -> V_12 . type != V_15 )
F_300 ( V_111 ) ;
F_307 ( V_111 -> V_2 -> V_9 ) ;
if ( V_2 -> V_66 . V_273 )
V_2 -> V_437 . V_575 += F_292 () -
V_2 -> V_437 . V_572 ;
}
static inline int F_312 ( struct V_1 * V_2 , int V_576 )
{
if ( ( V_2 -> V_12 . type == V_15 ) && ( V_576 == 3 ) )
return 2 ;
return V_576 ;
}
int F_313 ( struct V_1 * V_2 , struct V_232 * V_233 ,
int V_577 , int V_578 )
{
T_3 * V_579 ;
V_279 V_580 ;
if ( V_578 && V_2 -> V_12 . V_315 )
return V_2 -> V_12 . V_315 &
( V_2 -> V_46 . V_78 - 1 ) ;
switch ( V_2 -> V_46 . V_305 ) {
case V_581 :
case V_582 :
switch ( V_577 ) {
case 4 :
V_580 = F_314 ( F_315 ( V_233 ) ) ;
break;
case 6 :
V_580 = F_316 ( F_317 ( V_233 ) ) ;
break;
default:
return V_2 -> V_46 . V_257 ;
}
if ( V_2 -> V_46 . V_305 == V_582 )
return F_312 ( V_2 , ~ V_580 >> 6 & 3 ) ;
if ( V_580 & V_583 )
return F_312 ( V_2 , 3 ) ;
if ( V_580 & V_584 )
return 2 ;
if ( V_580 & V_585 )
return 1 ;
if ( V_580 & V_586 )
return 0 ;
break;
case V_587 :
if ( V_233 -> V_588 > 5 )
return 0 ;
return F_312 ( V_2 , ~ V_233 -> V_588 >> 1 & 3 ) ;
case V_589 :
V_579 = & ( (struct V_590 * ) V_233 -> V_117 ) -> V_591 ;
if ( * V_579 == V_592 )
return F_312 ( V_2 , ~ * ( V_579 + 1 ) >>
( V_593 + 1 ) & 3 ) ;
break;
default:
break;
}
return V_2 -> V_46 . V_257 ;
}
int F_318 ( struct V_232 * V_233 )
{
int V_177 , V_51 = 0 ;
for ( V_177 = 0 ; V_177 < F_319 ( V_233 ) -> V_594 ; V_177 ++ ) {
struct V_595 * V_596 = & F_319 ( V_233 ) -> V_597 [ V_177 ] ;
V_51 += F_320 (
( V_121 ) F_321 ( V_596 ) ,
( V_121 ) F_321 ( V_596 ) + F_322 ( V_596 ) ) ;
}
return V_51 ;
}
int F_323 ( struct V_1 * V_2 ,
struct V_232 * V_233 , int V_598 , int V_599 )
{
int V_51 = F_320 (
( V_121 ) V_233 -> V_117 + V_599 ,
( V_121 ) V_233 -> V_117 + F_324 ( V_233 ) ) +
F_318 ( V_233 ) ;
if ( ( V_51 + V_598 ) > F_29 ( V_2 ) ) {
F_68 ( 2 , L_246
L_247 ,
V_51 + V_598 , V_233 -> V_163 ) ;
return 0 ;
}
return V_51 ;
}
int F_325 ( struct V_232 * V_233 , struct V_600 * * V_133 , int V_163 )
{
int V_601 , V_602 , V_603 ;
if ( ( ( unsigned long ) V_233 -> V_117 & V_604 ) !=
( ( ( unsigned long ) V_233 -> V_117 + V_163 - 1 ) & V_604 ) ) {
V_601 = F_326 ( V_233 ) ;
V_602 = V_403 - ( ( unsigned long ) V_233 -> V_117 % V_403 ) ;
V_603 = V_163 - V_602 ;
if ( V_603 > V_601 )
return 1 ;
memmove ( V_233 -> V_117 - V_603 , V_233 -> V_117 , F_324 ( V_233 ) ) ;
V_233 -> V_117 -= V_603 ;
V_233 -> V_605 -= V_603 ;
* V_133 = (struct V_600 * ) V_233 -> V_117 ;
F_68 ( 2 , L_248 , V_163 , V_603 ) ;
}
return 0 ;
}
static inline void F_327 ( struct V_232 * V_233 ,
struct V_520 * V_64 , int V_606 , int * V_255 ,
int V_184 )
{
int V_253 = F_324 ( V_233 ) ;
int V_607 ;
int V_245 ;
char * V_117 ;
int V_608 , V_177 ;
struct V_595 * V_596 ;
V_245 = * V_255 ;
V_117 = V_233 -> V_117 ;
V_608 = ( V_606 == 0 ? 1 : 0 ) ;
if ( V_184 >= 0 ) {
V_117 = V_233 -> V_117 + V_184 ;
V_253 -= V_184 ;
V_608 = 0 ;
}
while ( V_253 > 0 ) {
V_607 = V_403 - ( ( unsigned long ) V_117 % V_403 ) ;
if ( V_253 < V_607 )
V_607 = V_253 ;
V_64 -> V_245 [ V_245 ] . V_250 = V_117 ;
V_64 -> V_245 [ V_245 ] . V_253 = V_607 ;
V_253 -= V_607 ;
if ( ! V_253 ) {
if ( V_608 )
if ( F_319 ( V_233 ) -> V_594 )
V_64 -> V_245 [ V_245 ] . V_254 =
V_609 ;
else
V_64 -> V_245 [ V_245 ] . V_254 = 0 ;
else
V_64 -> V_245 [ V_245 ] . V_254 =
V_610 ;
} else {
if ( V_608 )
V_64 -> V_245 [ V_245 ] . V_254 =
V_609 ;
else
V_64 -> V_245 [ V_245 ] . V_254 =
V_610 ;
}
V_117 += V_607 ;
V_245 ++ ;
V_608 = 0 ;
}
for ( V_177 = 0 ; V_177 < F_319 ( V_233 ) -> V_594 ; V_177 ++ ) {
V_596 = & F_319 ( V_233 ) -> V_597 [ V_177 ] ;
V_117 = ( char * ) F_328 ( F_329 ( V_596 ) ) +
V_596 -> V_611 ;
V_253 = V_596 -> V_612 ;
while ( V_253 > 0 ) {
V_607 = V_403 -
( ( unsigned long ) V_117 % V_403 ) ;
if ( V_253 < V_607 )
V_607 = V_253 ;
V_64 -> V_245 [ V_245 ] . V_250 = V_117 ;
V_64 -> V_245 [ V_245 ] . V_253 = V_607 ;
V_64 -> V_245 [ V_245 ] . V_254 =
V_610 ;
V_253 -= V_607 ;
V_117 += V_607 ;
V_245 ++ ;
}
}
if ( V_64 -> V_245 [ V_245 - 1 ] . V_254 )
V_64 -> V_245 [ V_245 - 1 ] . V_254 = V_613 ;
* V_255 = V_245 ;
}
static inline int F_330 ( struct V_92 * V_111 ,
struct V_96 * V_231 , struct V_232 * V_233 ,
struct V_600 * V_133 , int V_184 , int V_614 )
{
struct V_520 * V_64 ;
int V_552 = 0 , V_615 , V_616 = 0 ;
V_64 = V_231 -> V_64 ;
F_76 ( & V_233 -> V_242 ) ;
F_331 ( & V_231 -> V_234 , V_233 ) ;
if ( V_133 -> V_133 . V_617 . V_310 == V_618 ) {
int V_245 = V_231 -> V_255 ;
V_615 = sizeof( struct V_619 ) +
( (struct V_619 * ) V_133 ) -> V_496 . V_620 ;
V_64 -> V_245 [ V_245 ] . V_250 = V_233 -> V_117 ;
V_64 -> V_245 [ V_245 ] . V_253 = V_615 ;
V_64 -> V_245 [ V_245 ] . V_254 = V_609 ;
V_231 -> V_255 ++ ;
V_233 -> V_117 += V_615 ;
V_233 -> V_163 -= V_615 ;
V_616 = 1 ;
}
if ( V_184 >= 0 ) {
int V_245 = V_231 -> V_255 ;
V_64 -> V_245 [ V_245 ] . V_250 = V_133 ;
V_64 -> V_245 [ V_245 ] . V_253 = sizeof( struct V_600 ) +
V_614 ;
V_64 -> V_245 [ V_245 ] . V_254 = V_609 ;
V_231 -> V_251 [ V_245 ] = 1 ;
V_231 -> V_255 ++ ;
}
F_327 ( V_233 , V_64 , V_616 ,
( int * ) & V_231 -> V_255 , V_184 ) ;
if ( ! V_111 -> V_444 ) {
F_4 ( V_111 -> V_2 , 6 , L_249 ) ;
F_60 ( & V_231 -> V_3 , V_107 ) ;
V_552 = 1 ;
} else {
F_4 ( V_111 -> V_2 , 6 , L_250 ) ;
if ( V_111 -> V_2 -> V_66 . V_273 )
V_111 -> V_2 -> V_437 . V_621 ++ ;
if ( V_231 -> V_255 >=
F_29 ( V_111 -> V_2 ) ) {
F_60 ( & V_231 -> V_3 , V_107 ) ;
V_552 = 1 ;
}
}
return V_552 ;
}
int F_332 ( struct V_1 * V_2 ,
struct V_92 * V_111 , struct V_232 * V_233 ,
struct V_600 * V_133 , int V_622 ,
int V_184 , int V_614 )
{
struct V_96 * V_64 ;
int V_169 ;
while ( F_59 ( & V_111 -> V_3 , V_446 ,
V_623 ) != V_446 ) ;
V_169 = V_111 -> V_443 ;
V_64 = V_111 -> V_63 [ V_111 -> V_443 ] ;
if ( F_51 ( & V_64 -> V_3 ) != V_249 )
goto V_73;
V_111 -> V_443 = ( V_111 -> V_443 + 1 ) %
V_62 ;
F_60 ( & V_111 -> V_3 , V_446 ) ;
F_330 ( V_111 , V_64 , V_233 , V_133 , V_184 , V_614 ) ;
F_297 ( V_111 , V_169 , 1 ) ;
return 0 ;
V_73:
F_60 ( & V_111 -> V_3 , V_446 ) ;
return - V_624 ;
}
int F_333 ( struct V_1 * V_2 , struct V_92 * V_111 ,
struct V_232 * V_233 , struct V_600 * V_133 ,
int V_622 )
{
struct V_96 * V_64 ;
int V_625 ;
int V_539 = 0 ;
int V_444 = 0 ;
int V_41 ;
int V_37 = 0 ;
while ( F_59 ( & V_111 -> V_3 , V_446 ,
V_623 ) != V_446 ) ;
V_625 = V_111 -> V_443 ;
V_64 = V_111 -> V_63 [ V_111 -> V_443 ] ;
if ( F_51 ( & V_64 -> V_3 ) != V_249 ) {
F_60 ( & V_111 -> V_3 , V_446 ) ;
return - V_624 ;
}
F_294 ( V_111 ) ;
if ( V_111 -> V_444 ) {
V_444 = 1 ;
if ( ( F_29 ( V_2 ) -
V_64 -> V_255 ) < V_622 ) {
F_60 ( & V_64 -> V_3 , V_107 ) ;
V_539 ++ ;
V_111 -> V_443 =
( V_111 -> V_443 + 1 ) %
V_62 ;
V_64 = V_111 -> V_63 [ V_111 -> V_443 ] ;
if ( F_51 ( & V_64 -> V_3 ) !=
V_249 ) {
F_297 ( V_111 , V_625 ,
V_539 ) ;
F_60 ( & V_111 -> V_3 ,
V_446 ) ;
return - V_624 ;
}
}
}
V_41 = F_330 ( V_111 , V_64 , V_233 , V_133 , - 1 , 0 ) ;
V_111 -> V_443 = ( V_111 -> V_443 + V_41 ) %
V_62 ;
V_539 += V_41 ;
if ( V_539 )
F_297 ( V_111 , V_625 , V_539 ) ;
else if ( ! F_51 ( & V_111 -> V_248 ) )
F_233 ( & V_111 -> V_3 , V_554 ) ;
while ( F_334 ( & V_111 -> V_3 ) ) {
V_539 = 0 ;
V_625 = V_111 -> V_443 ;
V_539 += F_295 ( V_111 ) ;
if ( ! V_539 && ! F_51 ( & V_111 -> V_248 ) )
V_539 += F_296 ( V_111 ) ;
if ( V_539 )
F_297 ( V_111 , V_625 , V_539 ) ;
}
if ( V_111 -> V_2 -> V_66 . V_273 && V_444 )
V_111 -> V_2 -> V_437 . V_555 += V_539 ;
return V_37 ;
}
static int F_335 ( struct V_1 * V_2 ,
struct V_124 * V_125 , unsigned long V_117 )
{
struct V_129 * V_130 ;
struct V_475 * V_626 ;
F_4 ( V_2 , 4 , L_251 ) ;
V_130 = (struct V_129 * ) V_117 ;
V_626 = & ( V_130 -> V_117 . V_462 ) ;
F_271 ( V_2 , V_125 , ( unsigned long ) V_130 ) ;
if ( V_130 -> V_133 . V_139 ) {
F_52 ( V_2 , 4 , L_252 , V_130 -> V_133 . V_139 ) ;
V_626 -> V_117 . V_627 = V_628 ;
}
V_2 -> V_12 . V_629 = V_626 -> V_117 . V_627 ;
return 0 ;
}
void F_336 ( struct V_1 * V_2 )
{
enum V_630 V_627 ;
struct V_631 * V_9 = V_2 -> V_9 ;
struct V_112 * V_113 ;
struct V_129 * V_130 ;
F_4 ( V_2 , 4 , L_253 ) ;
if ( ( ( V_9 -> V_32 & V_632 ) &&
( V_2 -> V_12 . V_629 == V_633 ) ) ||
( ! ( V_9 -> V_32 & V_632 ) &&
( V_2 -> V_12 . V_629 == V_628 ) ) )
return;
V_627 = V_628 ;
if ( V_9 -> V_32 & V_632 )
V_627 = V_633 ;
F_52 ( V_2 , 4 , L_254 , V_627 ) ;
V_113 = F_273 ( V_2 , V_634 ,
sizeof( struct V_493 ) + 8 ) ;
if ( ! V_113 )
return;
V_130 = (struct V_129 * ) ( V_113 -> V_117 + V_386 ) ;
V_130 -> V_117 . V_462 . V_117 . V_627 = V_627 ;
F_269 ( V_2 , V_113 , F_335 , NULL ) ;
}
int F_337 ( struct V_631 * V_9 , int V_635 )
{
struct V_1 * V_2 ;
char V_636 [ 15 ] ;
V_2 = V_9 -> V_637 ;
F_4 ( V_2 , 4 , L_255 ) ;
sprintf ( V_636 , L_256 , V_635 ) ;
F_4 ( V_2 , 4 , V_636 ) ;
if ( ( ! F_338 ( V_2 , V_638 ) ) &&
( ! F_207 ( V_2 , V_635 ) ) )
return - V_401 ;
V_9 -> V_399 = V_635 ;
return 0 ;
}
struct V_639 * F_339 ( struct V_631 * V_9 )
{
struct V_1 * V_2 ;
V_2 = V_9 -> V_637 ;
F_4 ( V_2 , 5 , L_257 ) ;
return & V_2 -> V_523 ;
}
static int F_340 ( struct V_1 * V_2 ,
struct V_124 * V_125 , unsigned long V_117 )
{
struct V_129 * V_130 ;
F_4 ( V_2 , 4 , L_258 ) ;
V_130 = (struct V_129 * ) V_117 ;
if ( ! V_2 -> V_66 . V_303 ||
! ( V_2 -> V_12 . V_640 & V_641 ) ) {
memcpy ( V_2 -> V_9 -> V_642 ,
& V_130 -> V_117 . V_462 . V_117 . V_643 . V_250 ,
V_644 ) ;
V_2 -> V_12 . V_640 |= V_641 ;
}
F_271 ( V_2 , V_125 , ( unsigned long ) V_130 ) ;
return 0 ;
}
int F_341 ( struct V_1 * V_2 )
{
int V_37 ;
struct V_112 * V_113 ;
struct V_129 * V_130 ;
F_4 ( V_2 , 4 , L_259 ) ;
V_113 = F_273 ( V_2 , V_645 ,
sizeof( struct V_493 ) +
sizeof( struct V_646 ) ) ;
if ( ! V_113 )
return - V_50 ;
V_130 = (struct V_129 * ) ( V_113 -> V_117 + V_386 ) ;
V_130 -> V_117 . V_462 . V_117 . V_643 . V_130 = V_647 ;
V_130 -> V_117 . V_462 . V_117 . V_643 . V_648 = V_644 ;
memcpy ( & V_130 -> V_117 . V_462 . V_117 . V_643 . V_250 ,
V_2 -> V_9 -> V_642 , V_644 ) ;
V_37 = F_269 ( V_2 , V_113 , F_340 ,
NULL ) ;
return V_37 ;
}
static int F_342 ( struct V_1 * V_2 ,
struct V_124 * V_125 , unsigned long V_117 )
{
struct V_129 * V_130 ;
struct V_649 * V_650 ;
int V_651 = * ( int * ) V_125 -> V_382 ;
F_4 ( V_2 , 4 , L_260 ) ;
V_130 = (struct V_129 * ) V_117 ;
V_650 = & V_130 -> V_117 . V_462 . V_117 . V_652 ;
F_45 ( V_65 , 2 , L_260 ) ;
F_45 ( V_65 , 2 , L_261 , V_2 -> V_10 -> V_9 . V_653 . V_511 ) ;
F_45 ( V_65 , 2 , L_262 ,
V_130 -> V_117 . V_462 . V_133 . V_139 ) ;
if ( V_130 -> V_117 . V_462 . V_133 . V_139 !=
V_654 )
F_68 ( 3 , L_263 ,
V_2 -> V_10 -> V_9 . V_653 . V_511 ,
V_650 -> V_655 ,
V_130 -> V_117 . V_462 . V_133 . V_139 ) ;
switch ( V_130 -> V_117 . V_462 . V_133 . V_139 ) {
case V_654 :
if ( V_2 -> V_66 . V_276 == V_277 ) {
F_89 ( & V_2 -> V_10 -> V_9 ,
L_264 ) ;
} else {
F_89 ( & V_2 -> V_10 -> V_9 ,
L_265 ) ;
}
break;
case V_656 :
F_68 ( 2 , L_266
L_267 , F_69 ( & V_2 -> V_10 -> V_9 ) ) ;
if ( V_651 )
V_2 -> V_66 . V_276 = V_2 -> V_66 . V_657 ;
break;
case V_658 :
F_68 ( 2 , L_268
L_269 , F_69 ( & V_2 -> V_10 -> V_9 ) ) ;
if ( V_651 )
V_2 -> V_66 . V_276 = V_2 -> V_66 . V_657 ;
break;
case V_659 :
F_47 ( & V_2 -> V_10 -> V_9 , L_270
L_271 ) ;
break;
case V_660 :
F_47 ( & V_2 -> V_10 -> V_9 ,
L_272
L_273 ) ;
if ( V_651 )
V_2 -> V_66 . V_276 = V_2 -> V_66 . V_657 ;
break;
case V_661 :
F_47 ( & V_2 -> V_10 -> V_9 ,
L_274 ) ;
if ( V_651 )
V_2 -> V_66 . V_276 = V_2 -> V_66 . V_657 ;
break;
case V_662 :
F_47 ( & V_2 -> V_10 -> V_9 , L_275
L_276 ) ;
if ( V_651 )
V_2 -> V_66 . V_276 = V_2 -> V_66 . V_657 ;
break;
case V_663 :
F_47 ( & V_2 -> V_10 -> V_9 , L_277
L_278 ) ;
if ( V_651 )
V_2 -> V_66 . V_276 = V_2 -> V_66 . V_657 ;
break;
case V_664 :
F_67 ( & V_2 -> V_10 -> V_9 , L_279
L_280 ) ;
break;
default:
if ( V_651 )
V_2 -> V_66 . V_276 = V_2 -> V_66 . V_657 ;
break;
}
F_271 ( V_2 , V_125 , ( unsigned long ) V_130 ) ;
return 0 ;
}
static int F_343 ( struct V_1 * V_2 ,
enum V_665 V_276 , int V_651 )
{
int V_37 ;
struct V_112 * V_113 ;
struct V_129 * V_130 ;
struct V_649 * V_650 ;
F_4 ( V_2 , 4 , L_281 ) ;
F_45 ( V_65 , 2 , L_281 ) ;
F_45 ( V_65 , 2 , L_261 , V_2 -> V_10 -> V_9 . V_653 . V_511 ) ;
V_113 = F_273 ( V_2 , V_666 ,
sizeof( struct V_493 ) +
sizeof( struct V_649 ) ) ;
if ( ! V_113 )
return - V_50 ;
V_130 = (struct V_129 * ) ( V_113 -> V_117 + V_386 ) ;
V_650 = & V_130 -> V_117 . V_462 . V_117 . V_652 ;
V_650 -> V_655 = V_276 ;
V_37 = F_269 ( V_2 , V_113 , F_342 ,
& V_651 ) ;
F_45 ( V_65 , 2 , L_262 , V_37 ) ;
return V_37 ;
}
int F_344 ( struct V_1 * V_2 , int V_651 )
{
int V_37 = 0 ;
F_4 ( V_2 , 4 , L_282 ) ;
if ( ( V_2 -> V_12 . type == V_14 ||
V_2 -> V_12 . type == V_17 ) &&
F_278 ( V_2 , V_666 ) ) {
V_37 = F_343 ( V_2 ,
V_2 -> V_66 . V_276 , V_651 ) ;
if ( V_37 ) {
F_68 ( 3 ,
L_283 ,
V_2 -> V_10 -> V_9 . V_653 . V_511 ,
V_37 ) ;
V_37 = - V_336 ;
}
} else if ( V_2 -> V_66 . V_276 != V_277 ) {
V_2 -> V_66 . V_276 = V_277 ;
F_47 ( & V_2 -> V_10 -> V_9 , L_270
L_271 ) ;
V_37 = - V_336 ;
}
return V_37 ;
}
void F_345 ( struct V_631 * V_9 )
{
struct V_1 * V_2 ;
V_2 = V_9 -> V_637 ;
F_4 ( V_2 , 4 , L_284 ) ;
V_2 -> V_523 . V_549 ++ ;
F_72 ( V_2 ) ;
}
int F_346 ( struct V_631 * V_9 , int V_667 , int V_668 )
{
struct V_1 * V_2 = V_9 -> V_637 ;
int V_37 = 0 ;
switch ( V_668 ) {
case V_669 :
V_37 = V_670 ;
if ( ( V_2 -> V_12 . V_19 != V_22 ) &&
( V_2 -> V_12 . V_19 != V_671 ) &&
( V_2 -> V_12 . V_19 != V_23 ) )
V_37 |= V_672 ;
break;
case V_673 :
V_37 = V_674 | V_675 | V_676 |
V_677 | V_678 | V_679 | V_680 |
V_681 ;
break;
case V_682 :
V_37 = ( V_9 -> V_642 [ 0 ] << 16 ) | ( V_9 -> V_642 [ 1 ] << 8 ) |
V_9 -> V_642 [ 2 ] ;
V_37 = ( V_37 >> 5 ) & 0xFFFF ;
break;
case V_683 :
V_37 = ( V_9 -> V_642 [ 2 ] << 10 ) & 0xFFFF ;
break;
case V_684 :
V_37 = V_685 ;
break;
case V_686 :
V_37 = V_687 | V_688 | V_689 | V_690 |
V_691 | V_692 ;
break;
case V_693 :
break;
case V_694 :
break;
case V_695 :
break;
case V_696 :
break;
case V_697 :
V_37 = V_2 -> V_523 . V_698 ;
break;
case V_699 :
break;
case V_700 :
break;
case V_701 :
break;
case V_702 :
break;
case V_703 :
break;
case V_704 :
break;
case V_705 :
break;
default:
break;
}
return V_37 ;
}
static int F_347 ( struct V_1 * V_2 ,
struct V_112 * V_113 , int V_163 ,
int (* F_190)( struct V_1 * , struct V_124 * ,
unsigned long ) ,
void * V_379 )
{
V_186 V_706 , V_707 ;
F_4 ( V_2 , 4 , L_285 ) ;
memcpy ( V_113 -> V_117 , V_460 , V_386 ) ;
memcpy ( F_268 ( V_113 -> V_117 ) ,
& V_2 -> V_350 . V_411 , V_366 ) ;
V_706 = ( V_708 ) V_386 + V_163 ;
V_707 = ( V_708 ) V_163 ;
memcpy ( F_348 ( V_113 -> V_117 ) , & V_706 , 2 ) ;
memcpy ( F_349 ( V_113 -> V_117 ) , & V_707 , 2 ) ;
memcpy ( F_350 ( V_113 -> V_117 ) , & V_707 , 2 ) ;
memcpy ( F_351 ( V_113 -> V_117 ) , & V_707 , 2 ) ;
return F_189 ( V_2 , V_386 + V_163 , V_113 ,
F_190 , V_379 ) ;
}
static int F_352 ( struct V_1 * V_2 ,
struct V_124 * V_125 , unsigned long V_709 )
{
struct V_129 * V_130 ;
struct V_710 * V_711 ;
struct V_712 * V_713 ;
unsigned char * V_117 ;
V_185 V_714 ;
F_4 ( V_2 , 3 , L_286 ) ;
V_130 = (struct V_129 * ) V_709 ;
V_117 = ( unsigned char * ) ( ( char * ) V_130 - V_125 -> V_184 ) ;
V_711 = (struct V_710 * ) V_125 -> V_382 ;
V_713 = & V_130 -> V_117 . V_462 . V_117 . V_713 ;
if ( V_130 -> V_133 . V_139 ) {
F_52 ( V_2 , 4 , L_287 , V_130 -> V_133 . V_139 ) ;
return 0 ;
}
if ( V_130 -> V_117 . V_462 . V_133 . V_139 ) {
V_130 -> V_133 . V_139 =
V_130 -> V_117 . V_462 . V_133 . V_139 ;
F_52 ( V_2 , 4 , L_288 , V_130 -> V_133 . V_139 ) ;
return 0 ;
}
V_714 = * ( ( V_185 * ) F_349 ( V_117 ) ) ;
if ( V_130 -> V_117 . V_462 . V_133 . V_473 == 1 )
V_714 -= ( V_185 ) ( ( char * ) & V_713 -> V_117 - ( char * ) V_130 ) ;
else
V_714 -= ( V_185 ) ( ( char * ) & V_713 -> V_715 - ( char * ) V_130 ) ;
if ( ( V_711 -> V_716 - V_711 -> V_717 ) < V_714 ) {
F_52 ( V_2 , 4 , L_289 , - V_50 ) ;
V_130 -> V_133 . V_139 = V_718 ;
return 0 ;
}
F_52 ( V_2 , 4 , L_290 ,
V_130 -> V_117 . V_462 . V_133 . V_472 ) ;
F_52 ( V_2 , 4 , L_291 ,
V_130 -> V_117 . V_462 . V_133 . V_473 ) ;
if ( V_130 -> V_117 . V_462 . V_133 . V_473 == 1 ) {
memcpy ( V_711 -> V_719 + V_711 -> V_717 ,
( char * ) V_713 ,
V_714 + F_353 ( struct V_712 , V_117 ) ) ;
V_711 -> V_717 += F_353 ( struct V_712 , V_117 ) ;
} else {
memcpy ( V_711 -> V_719 + V_711 -> V_717 ,
( char * ) & V_713 -> V_715 , V_714 ) ;
}
V_711 -> V_717 += V_714 ;
F_52 ( V_2 , 4 , L_292 ,
V_130 -> V_117 . V_462 . V_133 . V_472 ) ;
F_52 ( V_2 , 4 , L_293 ,
V_130 -> V_117 . V_462 . V_133 . V_473 ) ;
if ( V_130 -> V_117 . V_462 . V_133 . V_473 <
V_130 -> V_117 . V_462 . V_133 . V_472 )
return 1 ;
return 0 ;
}
int F_354 ( struct V_1 * V_2 , char T_4 * V_719 )
{
struct V_112 * V_113 ;
struct V_129 * V_130 ;
struct V_720 * V_721 ;
unsigned int V_722 ;
struct V_710 V_711 = { 0 , } ;
int V_37 = 0 ;
F_4 ( V_2 , 3 , L_294 ) ;
if ( V_2 -> V_12 . V_13 )
return - V_336 ;
if ( ( ! F_278 ( V_2 , V_723 ) ) &&
( ! V_2 -> V_66 . V_303 ) ) {
return - V_336 ;
}
if ( F_355 ( & V_722 , V_719 + sizeof( int ) , sizeof( int ) ) )
return - V_724 ;
if ( V_722 > ( V_118 - V_386 -
sizeof( struct V_725 ) -
sizeof( struct V_493 ) ) )
return - V_401 ;
V_721 = F_356 ( V_719 , V_722 + sizeof( struct V_726 ) ) ;
if ( F_116 ( V_721 ) ) {
F_4 ( V_2 , 2 , L_295 ) ;
return F_117 ( V_721 ) ;
}
V_711 . V_716 = V_721 -> V_133 . V_714 ;
V_711 . V_719 = F_27 ( V_711 . V_716 , V_49 ) ;
if ( ! V_711 . V_719 ) {
F_32 ( V_721 ) ;
return - V_50 ;
}
V_711 . V_717 = sizeof( struct V_726 ) ;
V_113 = F_273 ( V_2 , V_723 ,
V_727 + V_722 ) ;
if ( ! V_113 ) {
V_37 = - V_50 ;
goto V_73;
}
V_130 = (struct V_129 * ) ( V_113 -> V_117 + V_386 ) ;
memcpy ( & V_130 -> V_117 . V_462 . V_117 . V_713 , & V_721 -> V_130 , V_722 ) ;
V_37 = F_347 ( V_2 , V_113 , V_728 + V_722 ,
F_352 , ( void * ) & V_711 ) ;
if ( V_37 )
F_68 ( 2 , L_296 ,
F_81 ( V_2 ) , V_37 ) ;
else {
if ( F_357 ( V_719 , V_711 . V_719 , V_711 . V_716 ) )
V_37 = - V_724 ;
}
V_73:
F_32 ( V_721 ) ;
F_32 ( V_711 . V_719 ) ;
return V_37 ;
}
static int F_358 ( struct V_1 * V_2 ,
struct V_124 * V_125 , unsigned long V_117 )
{
struct V_129 * V_130 ;
struct V_729 * V_730 ;
char * V_731 ;
int V_732 ;
F_4 ( V_2 , 3 , L_297 ) ;
V_130 = (struct V_129 * ) V_117 ;
V_730 = (struct V_729 * ) V_125 -> V_382 ;
V_732 = V_130 -> V_117 . V_462 . V_133 . V_470 ;
V_731 = ( char * ) V_117 + 28 ;
if ( V_732 > ( V_730 -> V_733 - V_730 -> V_734 ) ) {
V_130 -> V_133 . V_139 = V_735 ;
return 0 ;
}
memcpy ( ( V_730 -> V_64 + V_730 -> V_734 ) , V_731 ,
V_732 ) ;
V_730 -> V_734 += V_732 ;
if ( V_130 -> V_117 . V_462 . V_133 . V_473 <
V_130 -> V_117 . V_462 . V_133 . V_472 )
return 1 ;
return 0 ;
}
int F_359 ( struct V_1 * V_2 , char T_4 * V_719 )
{
int V_37 = 0 ;
struct V_112 * V_113 ;
struct V_129 * V_130 ;
struct V_736 * V_737 ;
struct V_738 V_739 ;
struct V_729 V_730 ;
void T_4 * V_41 ;
F_4 ( V_2 , 3 , L_298 ) ;
if ( ! F_278 ( V_2 , V_740 ) ) {
V_37 = - V_336 ;
goto V_73;
}
if ( F_355 ( & V_739 , V_719 ,
sizeof( struct V_738 ) ) ) {
V_37 = - V_724 ;
goto V_73;
}
V_730 . V_733 = V_739 . V_733 ;
V_730 . V_734 = 0 ;
V_730 . V_64 = F_27 ( V_739 . V_733 , V_49 ) ;
if ( ! V_730 . V_64 ) {
V_37 = - V_50 ;
goto V_73;
}
V_113 = F_273 ( V_2 , V_740 ,
sizeof( struct V_493 ) +
sizeof( struct V_736 ) ) ;
if ( ! V_113 ) {
V_37 = - V_50 ;
goto V_741;
}
V_130 = (struct V_129 * ) ( V_113 -> V_117 + V_386 ) ;
V_737 = & V_130 -> V_117 . V_462 . V_117 . V_742 ;
V_737 -> V_655 = V_739 . V_134 ;
V_37 = F_269 ( V_2 , V_113 , F_358 ,
& V_730 ) ;
if ( ! V_37 ) {
if ( F_360 () )
V_41 = F_361 ( V_739 . V_43 ) ;
else
V_41 = ( void T_4 * ) ( unsigned long ) V_739 . V_43 ;
if ( F_357 ( V_41 , V_730 . V_64 ,
V_730 . V_734 ) ) {
V_37 = - V_724 ;
goto V_741;
}
V_739 . V_734 = V_730 . V_734 ;
if ( F_357 ( V_719 , & V_739 ,
sizeof( struct V_738 ) ) )
V_37 = - V_724 ;
} else
if ( V_37 == V_735 )
V_37 = - V_724 ;
V_741:
F_32 ( V_730 . V_64 ) ;
V_73:
return V_37 ;
}
static int F_362 ( struct V_1 * V_2 ,
struct V_124 * V_125 , unsigned long V_117 )
{
struct V_129 * V_130 ;
struct V_743 * V_744 ;
struct V_745 * V_745 ;
F_4 ( V_2 , 2 , L_299 ) ;
V_745 = (struct V_745 * ) V_125 -> V_382 ;
V_130 = (struct V_129 * ) V_117 ;
V_744 = & V_130 -> V_117 . V_462 . V_117 . V_744 ;
if ( V_130 -> V_117 . V_462 . V_133 . V_139 == 0 ) {
V_745 -> V_746 = V_744 -> V_746 ;
V_745 -> V_747 = V_744 -> V_747 ;
V_745 -> V_748 = V_744 -> V_748 ;
}
F_271 ( V_2 , V_125 , ( unsigned long ) V_130 ) ;
return 0 ;
}
static int V_743 ( struct V_1 * V_2 ,
struct V_745 * V_745 )
{
struct V_112 * V_113 ;
F_4 ( V_2 , 2 , L_300 ) ;
if ( ! F_278 ( V_2 , V_749 ) )
return - V_336 ;
V_113 = F_273 ( V_2 , V_749 ,
sizeof( struct V_493 ) ) ;
if ( ! V_113 )
return - V_50 ;
return F_269 ( V_2 , V_113 , F_362 ,
( void * ) V_745 ) ;
}
static inline int F_363 ( struct V_1 * V_2 )
{
switch ( V_2 -> V_12 . type ) {
case V_15 :
return 2 ;
default:
return 0 ;
}
}
static void F_364 ( struct V_1 * V_2 )
{
int V_37 ;
int V_253 ;
char * V_342 ;
struct V_158 * V_750 ;
int V_751 = 0 ;
F_41 ( V_65 , 2 , L_301 ) ;
V_750 = F_44 ( V_2 ) ;
if ( ! V_750 -> V_752 ) {
V_751 = 1 ;
V_37 = F_365 ( V_750 ) ;
if ( V_37 ) {
F_45 ( V_65 , 2 , L_140 , V_37 ) ;
goto V_73;
}
}
V_37 = F_164 ( V_2 , ( void * * ) & V_342 , & V_253 ) ;
if ( V_37 ) {
F_68 ( 2 , L_302 ,
F_69 ( & V_2 -> V_10 -> V_9 ) , V_37 ) ;
F_45 ( V_65 , 2 , L_186 , V_37 ) ;
goto V_753;
}
F_167 ( V_2 , V_342 ) ;
if ( V_751 )
F_168 ( V_2 , V_342 ) ;
F_32 ( V_342 ) ;
V_37 = F_366 ( V_750 , & V_2 -> V_754 ) ;
if ( V_37 )
F_45 ( V_65 , 2 , L_187 , V_37 ) ;
F_45 ( V_65 , 2 , L_303 , V_2 -> V_754 . V_755 ) ;
F_45 ( V_65 , 2 , L_304 , V_2 -> V_754 . V_756 ) ;
F_45 ( V_65 , 2 , L_304 , V_2 -> V_754 . V_757 ) ;
F_45 ( V_65 , 2 , L_305 , V_2 -> V_754 . V_758 ) ;
if ( ! ( ( V_2 -> V_754 . V_755 != V_759 ) ||
( ( V_2 -> V_754 . V_756 & V_760 ) == 0 ) ||
( ( V_2 -> V_754 . V_757 & V_761 ) == 0 ) ) ) {
F_89 ( & V_2 -> V_10 -> V_9 ,
L_306 ) ;
} else {
V_2 -> V_66 . V_67 = V_560 ;
}
V_753:
if ( V_751 == 1 )
F_367 ( V_750 ) ;
V_73:
return;
}
static inline void F_368 ( struct V_1 * V_2 ,
struct V_520 * * V_762 ,
void (* * F_369) ( struct V_158 * , int , unsigned long ) ) {
int V_44 ;
if ( V_2 -> V_66 . V_67 == V_68 ) {
int V_184 = V_62 *
( V_2 -> V_46 . V_72 - 1 ) ;
V_44 = V_62 * ( V_2 -> V_46 . V_72 - 1 ) ;
for ( V_44 = 0 ; V_44 < V_62 ; ++ V_44 ) {
V_762 [ V_184 + V_44 ] = (struct V_520 * )
F_310 ( V_2 -> V_46 . V_69 -> V_63 [ V_44 ] . V_64 ) ;
}
F_369 [ V_2 -> V_46 . V_72 - 1 ] = NULL ;
}
}
static int F_246 ( struct V_1 * V_2 )
{
struct V_763 V_764 ;
char * V_765 ;
struct V_520 * * V_762 ;
void (* * F_369) ( struct V_158 * , int , unsigned long );
struct V_520 * * V_766 ;
int V_44 , V_45 , V_767 ;
int V_37 = 0 ;
F_41 ( V_65 , 2 , L_307 ) ;
V_765 = F_27 ( V_62 * sizeof( char ) ,
V_49 ) ;
if ( ! V_765 ) {
V_37 = - V_50 ;
goto V_768;
}
F_235 ( V_2 , V_765 ) ;
F_239 ( V_2 , V_765 ) ;
V_762 = F_27 ( V_2 -> V_46 . V_72 *
V_62 * sizeof( void * ) ,
V_49 ) ;
if ( ! V_762 ) {
V_37 = - V_50 ;
goto V_769;
}
for ( V_44 = 0 ; V_44 < V_62 ; ++ V_44 ) {
V_762 [ V_44 ] = (struct V_520 * )
F_310 ( V_2 -> V_46 . V_419 -> V_63 [ V_44 ] . V_64 ) ;
}
F_369 = F_27 ( sizeof( void * ) * V_2 -> V_46 . V_72 ,
V_49 ) ;
if ( ! F_369 ) {
V_37 = - V_50 ;
goto V_770;
}
for ( V_44 = 0 ; V_44 < V_2 -> V_46 . V_72 ; ++ V_44 )
F_369 [ V_44 ] = V_2 -> V_149 -> V_771 ;
F_368 ( V_2 , V_762 , F_369 ) ;
V_766 =
F_27 ( V_2 -> V_46 . V_78 * V_62 *
sizeof( void * ) , V_49 ) ;
if ( ! V_766 ) {
V_37 = - V_50 ;
goto V_772;
}
for ( V_44 = 0 , V_767 = 0 ; V_44 < V_2 -> V_46 . V_78 ; ++ V_44 )
for ( V_45 = 0 ; V_45 < V_62 ; ++ V_45 , ++ V_767 ) {
V_766 [ V_767 ] = (struct V_520 * ) F_310 (
V_2 -> V_46 . V_80 [ V_44 ] -> V_63 [ V_45 ] -> V_64 ) ;
}
memset ( & V_764 , 0 , sizeof( struct V_763 ) ) ;
V_764 . V_159 = F_44 ( V_2 ) ;
V_764 . V_773 = F_363 ( V_2 ) ;
V_764 . V_774 = 0 ;
V_764 . V_765 = V_765 ;
V_764 . V_775 = V_2 -> V_46 . V_72 ;
V_764 . V_776 = V_2 -> V_46 . V_78 ;
V_764 . V_777 = V_2 -> V_149 -> V_777 ;
V_764 . V_778 = V_2 -> V_149 -> V_778 ;
V_764 . V_779 = F_369 ;
V_764 . V_780 = ( unsigned long ) V_2 ;
V_764 . V_781 = ( void * * ) V_762 ;
V_764 . V_782 = ( void * * ) V_766 ;
V_764 . V_783 = V_2 -> V_46 . V_77 ;
V_764 . V_784 =
( V_2 -> V_12 . type == V_15 ) ? 1 : 32 ;
if ( F_59 ( & V_2 -> V_46 . V_3 , V_331 ,
V_327 ) == V_331 ) {
V_37 = F_370 ( & V_764 ) ;
if ( V_37 ) {
F_60 ( & V_2 -> V_46 . V_3 , V_331 ) ;
goto V_73;
}
V_37 = F_371 ( & V_764 ) ;
if ( V_37 ) {
F_60 ( & V_2 -> V_46 . V_3 , V_331 ) ;
F_247 ( F_44 ( V_2 ) ) ;
}
}
switch ( V_2 -> V_66 . V_67 ) {
case V_68 :
F_89 ( & V_2 -> V_10 -> V_9 , L_308 ) ;
break;
case V_278 :
F_89 ( & V_2 -> V_10 -> V_9 , L_309 ) ;
break;
default:
break;
}
V_73:
F_32 ( V_766 ) ;
V_772:
F_32 ( F_369 ) ;
V_770:
F_32 ( V_762 ) ;
V_769:
F_32 ( V_765 ) ;
V_768:
return V_37 ;
}
static void F_372 ( struct V_1 * V_2 )
{
F_41 ( V_65 , 2 , L_310 ) ;
F_39 ( V_65 , 2 , & V_2 , sizeof( void * ) ) ;
F_130 ( & V_2 -> V_114 ) ;
F_130 ( & V_2 -> V_155 ) ;
if ( V_2 -> V_9 )
F_373 ( V_2 -> V_9 ) ;
F_132 ( V_2 ) ;
F_374 ( & V_2 -> V_299 ) ;
F_32 ( V_2 ) ;
}
void F_375 ( struct V_1 * V_2 )
{
F_4 ( V_2 , 2 , L_311 ) ;
F_376 ( V_2 , 2 , & V_2 -> V_66 . V_478 , sizeof( V_2 -> V_66 . V_478 ) ) ;
F_376 ( V_2 , 2 , & V_2 -> V_66 . V_480 , sizeof( V_2 -> V_66 . V_480 ) ) ;
F_376 ( V_2 , 2 , & V_2 -> V_66 . V_465 , sizeof( V_2 -> V_66 . V_465 ) ) ;
F_376 ( V_2 , 2 , & V_2 -> V_12 . V_494 ,
sizeof( V_2 -> V_12 . V_494 ) ) ;
}
int F_377 ( struct V_1 * V_2 )
{
int V_785 = 3 ;
int V_37 ;
F_41 ( V_65 , 2 , L_312 ) ;
F_60 ( & V_2 -> V_528 , 0 ) ;
F_134 ( V_2 ) ;
V_786:
if ( V_785 < 3 )
F_68 ( 2 , L_313 ,
F_69 ( & V_2 -> V_10 -> V_9 ) ) ;
V_37 = F_162 ( V_2 , V_2 -> V_12 . type != V_15 ) ;
F_367 ( F_44 ( V_2 ) ) ;
F_367 ( F_378 ( V_2 ) ) ;
F_367 ( F_152 ( V_2 ) ) ;
F_247 ( F_44 ( V_2 ) ) ;
V_37 = F_365 ( F_152 ( V_2 ) ) ;
if ( V_37 )
goto V_787;
V_37 = F_365 ( F_378 ( V_2 ) ) ;
if ( V_37 )
goto V_787;
V_37 = F_365 ( F_44 ( V_2 ) ) ;
if ( V_37 )
goto V_787;
V_787:
if ( V_37 == - V_320 ) {
F_41 ( V_65 , 2 , L_314 ) ;
return V_37 ;
} else if ( V_37 ) {
F_45 ( V_65 , 2 , L_37 , V_37 ) ;
if ( -- V_785 < 0 )
goto V_73;
else
goto V_786;
}
F_364 ( V_2 ) ;
F_169 ( V_2 ) ;
F_170 ( V_2 ) ;
V_37 = F_173 ( & V_2 -> V_114 , F_185 ) ;
if ( V_37 == - V_320 ) {
F_41 ( V_65 , 2 , L_315 ) ;
return V_37 ;
} else if ( V_37 ) {
F_45 ( V_65 , 2 , L_140 , V_37 ) ;
if ( -- V_785 < 0 )
goto V_73;
else
goto V_786;
}
V_37 = F_173 ( & V_2 -> V_155 , F_182 ) ;
if ( V_37 == - V_320 ) {
F_41 ( V_65 , 2 , L_316 ) ;
return V_37 ;
} else if ( V_37 ) {
F_45 ( V_65 , 2 , L_185 , V_37 ) ;
if ( -- V_785 < 0 )
goto V_73;
else
goto V_786;
}
V_2 -> V_123 = 0 ;
V_37 = F_245 ( V_2 ) ;
if ( V_37 ) {
F_45 ( V_65 , 2 , L_186 , V_37 ) ;
goto V_73;
}
V_37 = F_270 ( V_2 ) ;
if ( V_37 ) {
F_45 ( V_65 , 2 , L_187 , V_37 ) ;
if ( V_37 == V_788 ) {
F_67 ( & V_2 -> V_10 -> V_9 ,
L_317 ) ;
V_2 -> V_143 = 0 ;
} else {
V_37 = - V_789 ;
goto V_73;
}
} else
V_2 -> V_143 = 1 ;
V_2 -> V_66 . V_478 . V_466 = 0 ;
V_2 -> V_66 . V_480 . V_466 = 0 ;
V_2 -> V_66 . V_465 . V_466 = 0 ;
V_2 -> V_66 . V_790 . V_466 = 0 ;
V_2 -> V_12 . V_494 = 0 ;
V_37 = F_276 ( V_2 , V_389 ) ;
if ( V_37 == - V_50 )
goto V_73;
if ( F_338 ( V_2 , V_479 ) ) {
V_37 = F_274 ( V_2 ) ;
if ( V_37 < 0 ) {
F_45 ( V_65 , 2 , L_188 , V_37 ) ;
goto V_73;
}
}
if ( F_278 ( V_2 , V_791 ) ) {
V_37 = F_280 ( V_2 ) ;
if ( V_37 < 0 ) {
F_45 ( V_65 , 2 , L_189 , V_37 ) ;
goto V_73;
}
}
return 0 ;
V_73:
F_67 ( & V_2 -> V_10 -> V_9 , L_106
L_107 ) ;
F_68 ( 2 , L_318 ,
F_69 ( & V_2 -> V_10 -> V_9 ) , V_37 ) ;
return V_37 ;
}
static inline int F_379 ( struct V_439 * V_792 ,
struct V_793 * V_245 ,
struct V_232 * * V_794 , int V_184 , int * V_795 , int V_714 )
{
struct V_435 * V_435 = F_257 ( V_245 -> V_250 ) ;
if ( * V_794 == NULL ) {
if ( V_792 -> V_426 ) {
* V_794 = V_792 -> V_426 ;
V_792 -> V_426 = NULL ;
} else {
* V_794 = F_261 ( V_440 + V_441 ) ;
if ( ! ( * V_794 ) )
return - V_50 ;
}
F_380 ( * V_794 , V_441 ) ;
if ( V_714 <= V_440 ) {
memcpy ( F_381 ( * V_794 , V_714 ) , V_245 -> V_250 + V_184 ,
V_714 ) ;
} else {
F_382 ( V_435 ) ;
memcpy ( F_381 ( * V_794 , V_440 ) ,
V_245 -> V_250 + V_184 , V_440 ) ;
F_383 ( * V_794 , * V_795 , V_435 ,
V_184 + V_440 ,
V_714 - V_440 ) ;
( * V_794 ) -> V_714 += V_714 - V_440 ;
( * V_794 ) -> V_163 += V_714 - V_440 ;
( * V_794 ) -> V_796 += V_714 - V_440 ;
( * V_795 ) ++ ;
}
} else {
F_382 ( V_435 ) ;
F_383 ( * V_794 , * V_795 , V_435 , V_184 , V_714 ) ;
( * V_794 ) -> V_714 += V_714 ;
( * V_794 ) -> V_163 += V_714 ;
( * V_794 ) -> V_796 += V_714 ;
( * V_795 ) ++ ;
}
return 0 ;
}
static inline int F_384 ( struct V_793 * V_797 )
{
return ( V_797 -> V_254 & V_442 ) ;
}
struct V_232 * F_385 ( struct V_1 * V_2 ,
struct V_439 * V_792 ,
struct V_793 * * V_798 , int * V_799 ,
struct V_600 * * V_133 )
{
struct V_793 * V_245 = * V_798 ;
struct V_520 * V_64 = V_792 -> V_64 ;
int V_184 = * V_799 ;
struct V_232 * V_233 = NULL ;
int V_800 = 0 ;
void * V_801 ;
int V_714 ;
int V_802 = 0 ;
int V_803 = 0 ;
int V_596 = 0 ;
if ( V_245 -> V_253 < V_184 + sizeof( struct V_600 ) ) {
if ( F_384 ( V_245 ) )
return NULL ;
V_245 ++ ;
V_184 = 0 ;
if ( V_245 -> V_253 < sizeof( struct V_600 ) )
return NULL ;
}
* V_133 = V_245 -> V_250 + V_184 ;
V_184 += sizeof( struct V_600 ) ;
switch ( ( * V_133 ) -> V_133 . V_804 . V_310 ) {
case V_805 :
V_800 = ( * V_133 ) -> V_133 . V_804 . V_806 ;
break;
case V_807 :
V_800 = ( * V_133 ) -> V_133 . V_617 . V_253 ;
V_802 = V_441 ;
break;
case V_808 :
V_800 = ( * V_133 ) -> V_133 . V_809 . V_810 ;
V_802 = sizeof( struct V_600 ) ;
break;
default:
break;
}
if ( ! V_800 )
return NULL ;
if ( ( ( V_800 >= V_2 -> V_66 . V_274 ) &&
( ! ( V_2 -> V_12 . type == V_18 ) ) &&
( ! F_51 ( & V_2 -> V_528 ) ) ) ||
( V_2 -> V_66 . V_67 == V_68 ) ) {
V_803 = 1 ;
} else {
V_233 = F_261 ( V_800 + V_802 ) ;
if ( ! V_233 )
goto V_811;
if ( V_802 )
F_380 ( V_233 , V_802 ) ;
}
V_801 = V_245 -> V_250 + V_184 ;
while ( V_800 ) {
V_714 = F_211 ( V_800 , ( int ) ( V_245 -> V_253 - V_184 ) ) ;
if ( V_714 ) {
if ( V_803 ) {
if ( F_379 ( V_792 , V_245 ,
& V_233 , V_184 , & V_596 , V_714 ) )
goto V_811;
} else {
memcpy ( F_381 ( V_233 , V_714 ) , V_801 ,
V_714 ) ;
}
}
V_800 -= V_714 ;
if ( V_800 ) {
if ( F_384 ( V_245 ) ) {
F_4 ( V_2 , 4 , L_319 ) ;
F_376 ( V_2 , 2 , V_64 , sizeof( void * ) ) ;
F_127 ( V_233 ) ;
V_2 -> V_523 . V_698 ++ ;
return NULL ;
}
V_245 ++ ;
V_184 = 0 ;
V_801 = V_245 -> V_250 ;
} else {
V_184 += V_714 ;
}
}
* V_798 = V_245 ;
* V_799 = V_184 ;
if ( V_803 && V_2 -> V_66 . V_273 ) {
V_2 -> V_437 . V_812 ++ ;
V_2 -> V_437 . V_813 += F_319 ( V_233 ) -> V_594 ;
}
return V_233 ;
V_811:
if ( F_386 () ) {
F_4 ( V_2 , 2 , L_320 ) ;
}
V_2 -> V_523 . V_524 ++ ;
return NULL ;
}
int F_387 ( struct V_1 * V_2 ,
struct V_124 * V_125 , unsigned long V_117 )
{
struct V_129 * V_130 ;
F_4 ( V_2 , 4 , L_200 ) ;
V_130 = (struct V_129 * ) V_117 ;
if ( V_130 -> V_133 . V_139 == 0 ) {
V_130 -> V_133 . V_139 = V_130 -> V_117 . V_814 . V_133 . V_139 ;
if ( V_130 -> V_133 . V_388 == V_389 )
V_2 -> V_66 . V_478 . V_481 = V_130 -> V_133 . V_457 ;
if ( V_130 -> V_133 . V_388 == V_482 )
V_2 -> V_66 . V_480 . V_481 = V_130 -> V_133 . V_457 ;
}
return 0 ;
}
struct V_112 * F_388 ( struct V_1 * V_2 ,
enum V_815 V_816 ,
V_185 V_337 , V_185 V_163 ,
enum V_448 V_449 )
{
struct V_112 * V_113 ;
struct V_129 * V_130 ;
F_4 ( V_2 , 4 , L_321 ) ;
V_113 = F_265 ( V_2 , V_817 , V_449 ) ;
if ( V_113 ) {
V_130 = (struct V_129 * ) ( V_113 -> V_117 + V_386 ) ;
V_130 -> V_117 . V_814 . V_133 . V_818 = V_816 ;
V_130 -> V_117 . V_814 . V_133 . V_253 = 8 + V_163 ;
V_130 -> V_117 . V_814 . V_133 . V_471 = V_337 ;
V_130 -> V_117 . V_814 . V_133 . V_139 = 0 ;
V_130 -> V_117 . V_814 . V_133 . V_473 = 0 ;
}
return V_113 ;
}
int F_389 ( struct V_1 * V_2 ,
struct V_112 * V_113 , V_185 V_163 , long V_117 ,
int (* F_190)( struct V_1 * ,
struct V_124 * , unsigned long ) ,
void * V_379 )
{
int V_37 ;
struct V_129 * V_130 ;
F_4 ( V_2 , 4 , L_322 ) ;
V_130 = (struct V_129 * ) ( V_113 -> V_117 + V_386 ) ;
if ( V_163 <= sizeof( T_1 ) )
V_130 -> V_117 . V_814 . V_117 . V_819 = ( T_1 ) V_117 ;
else
memcpy ( & V_130 -> V_117 . V_814 . V_117 , ( void * ) V_117 , V_163 ) ;
V_37 = F_269 ( V_2 , V_113 , F_190 , V_379 ) ;
return V_37 ;
}
int F_390 ( struct V_1 * V_2 ,
enum V_815 V_816 ,
V_185 V_337 , long V_117 )
{
int V_37 ;
int V_253 = 0 ;
struct V_112 * V_113 ;
F_4 ( V_2 , 4 , L_323 ) ;
if ( V_117 )
V_253 = sizeof( T_1 ) ;
V_113 = F_388 ( V_2 , V_816 , V_337 ,
V_253 , V_389 ) ;
if ( ! V_113 )
return - V_50 ;
V_37 = F_389 ( V_2 , V_113 , V_253 , V_117 ,
F_387 , NULL ) ;
return V_37 ;
}
static void F_391 ( void )
{
int V_820 ;
for ( V_820 = 0 ; V_820 < V_821 ; V_820 ++ ) {
F_392 ( V_822 [ V_820 ] . V_310 ) ;
V_822 [ V_820 ] . V_310 = NULL ;
}
}
void F_393 ( T_5 * V_310 , int V_369 , char * V_823 , ... )
{
char V_824 [ 32 ] ;
T_6 args ;
if ( ! F_394 ( V_310 , V_369 ) )
return;
va_start ( args , V_823 ) ;
vsnprintf ( V_824 , sizeof( V_824 ) , V_823 , args ) ;
va_end ( args ) ;
F_395 ( V_310 , V_369 , V_824 ) ;
}
static int F_396 ( void )
{
int V_334 ;
int V_820 ;
for ( V_820 = 0 ; V_820 < V_821 ; V_820 ++ ) {
V_822 [ V_820 ] . V_310 = F_397 ( V_822 [ V_820 ] . V_511 ,
V_822 [ V_820 ] . V_825 ,
V_822 [ V_820 ] . V_826 ,
V_822 [ V_820 ] . V_163 ) ;
if ( V_822 [ V_820 ] . V_310 == NULL ) {
F_391 () ;
return - V_50 ;
}
V_334 = F_398 ( V_822 [ V_820 ] . V_310 , V_822 [ V_820 ] . V_827 ) ;
if ( V_334 ) {
F_391 () ;
return V_334 ;
}
F_399 ( V_822 [ V_820 ] . V_310 , V_822 [ V_820 ] . V_369 ) ;
}
return 0 ;
}
int F_400 ( struct V_1 * V_2 ,
enum V_828 V_149 )
{
int V_37 = 0 ;
F_401 ( & V_829 ) ;
switch ( V_149 ) {
case V_830 :
V_2 -> V_149 = F_402 (
F_403 ( V_831 ) , L_324 ) ;
break;
case V_832 :
V_2 -> V_149 = F_402 (
F_403 ( V_833 ) , L_325 ) ;
break;
}
if ( ! V_2 -> V_149 ) {
F_47 ( & V_2 -> V_10 -> V_9 , L_326
L_327 , V_149 ) ;
V_37 = - V_401 ;
}
F_404 ( & V_829 ) ;
return V_37 ;
}
void F_405 ( struct V_1 * V_2 )
{
if ( V_2 -> V_66 . V_303 )
F_406 ( V_833 ) ;
else
F_406 ( V_831 ) ;
V_2 -> V_149 = NULL ;
}
static T_5 * F_407 ( char * V_511 )
{
struct V_834 * V_432 ;
T_5 * V_37 = NULL ;
F_401 ( & V_835 ) ;
F_250 (entry, &qeth_dbf_list, dbf_list) {
if ( strcmp ( V_432 -> V_836 , V_511 ) == 0 ) {
V_37 = V_432 -> V_837 ;
break;
}
}
F_404 ( & V_835 ) ;
return V_37 ;
}
static int F_408 ( struct V_1 * V_2 , char * V_511 )
{
struct V_834 * V_838 ;
V_2 -> V_839 = F_397 ( V_511 , 2 , 1 , 8 ) ;
if ( ! V_2 -> V_839 ) {
F_45 ( V_65 , 2 , L_261 , L_328 ) ;
goto V_840;
}
if ( F_398 ( V_2 -> V_839 , & V_841 ) )
goto V_842;
V_838 = F_27 ( sizeof( struct V_834 ) , V_49 ) ;
if ( ! V_838 )
goto V_842;
strncpy ( V_838 -> V_836 , V_511 , V_843 ) ;
V_838 -> V_837 = V_2 -> V_839 ;
F_401 ( & V_835 ) ;
F_33 ( & V_838 -> V_844 , & V_845 ) ;
F_404 ( & V_835 ) ;
return 0 ;
V_842:
F_392 ( V_2 -> V_839 ) ;
V_840:
return - V_50 ;
}
static void F_409 ( void )
{
struct V_834 * V_432 , * V_41 ;
F_401 ( & V_835 ) ;
F_24 (entry, tmp, &qeth_dbf_list, dbf_list) {
F_25 ( & V_432 -> V_844 ) ;
F_392 ( V_432 -> V_837 ) ;
F_32 ( V_432 ) ;
}
F_404 ( & V_835 ) ;
}
static int F_410 ( struct V_160 * V_10 )
{
struct V_1 * V_2 ;
struct V_846 * V_9 ;
int V_37 ;
unsigned long V_32 ;
char V_836 [ V_843 ] ;
F_41 ( V_65 , 2 , L_329 ) ;
V_9 = & V_10 -> V_9 ;
if ( ! F_411 ( V_9 ) )
return - V_789 ;
F_45 ( V_65 , 2 , L_261 , F_69 ( & V_10 -> V_9 ) ) ;
V_2 = F_149 () ;
if ( ! V_2 ) {
F_45 ( V_65 , 2 , L_37 , - V_50 ) ;
V_37 = - V_50 ;
goto V_847;
}
snprintf ( V_836 , sizeof( V_836 ) , L_330 ,
F_69 ( & V_10 -> V_9 ) ) ;
V_2 -> V_839 = F_407 ( V_836 ) ;
if ( ! V_2 -> V_839 ) {
V_37 = F_408 ( V_2 , V_836 ) ;
if ( V_37 )
goto V_848;
}
V_2 -> V_114 . V_119 = V_10 -> V_159 [ 0 ] ;
V_2 -> V_155 . V_119 = V_10 -> V_159 [ 1 ] ;
V_2 -> V_117 . V_119 = V_10 -> V_159 [ 2 ] ;
F_412 ( & V_10 -> V_9 , V_2 ) ;
V_2 -> V_10 = V_10 ;
V_10 -> V_159 [ 0 ] -> V_849 = F_118 ;
V_10 -> V_159 [ 1 ] -> V_849 = F_118 ;
V_10 -> V_159 [ 2 ] -> V_849 = F_118 ;
V_37 = F_151 ( V_2 ) ;
if ( V_37 ) {
F_45 ( V_65 , 2 , L_140 , V_37 ) ;
goto V_848;
}
V_37 = F_142 ( V_2 ) ;
if ( V_37 ) {
F_45 ( V_65 , 2 , L_135 , V_37 ) ;
goto V_848;
}
if ( V_2 -> V_12 . type == V_18 )
V_10 -> V_9 . type = & V_850 ;
else
V_10 -> V_9 . type = & V_851 ;
switch ( V_2 -> V_12 . type ) {
case V_18 :
case V_16 :
V_37 = F_400 ( V_2 , V_832 ) ;
if ( V_37 )
goto V_848;
V_37 = V_2 -> V_149 -> V_852 ( V_2 -> V_10 ) ;
if ( V_37 )
goto V_853;
case V_14 :
case V_17 :
default:
break;
}
F_413 ( & V_854 . V_855 , V_32 ) ;
F_102 ( & V_2 -> V_42 , & V_854 . V_42 ) ;
F_414 ( & V_854 . V_855 , V_32 ) ;
F_364 ( V_2 ) ;
return 0 ;
V_853:
F_405 ( V_2 ) ;
V_848:
F_372 ( V_2 ) ;
V_847:
F_415 ( V_9 ) ;
return V_37 ;
}
static void F_416 ( struct V_160 * V_10 )
{
unsigned long V_32 ;
struct V_1 * V_2 = F_95 ( & V_10 -> V_9 ) ;
F_41 ( V_65 , 2 , L_331 ) ;
if ( V_2 -> V_149 ) {
V_2 -> V_149 -> remove ( V_10 ) ;
F_405 ( V_2 ) ;
}
F_413 ( & V_854 . V_855 , V_32 ) ;
F_25 ( & V_2 -> V_42 ) ;
F_414 ( & V_854 . V_855 , V_32 ) ;
F_372 ( V_2 ) ;
F_412 ( & V_10 -> V_9 , NULL ) ;
F_415 ( & V_10 -> V_9 ) ;
return;
}
static int F_417 ( struct V_160 * V_10 )
{
struct V_1 * V_2 = F_95 ( & V_10 -> V_9 ) ;
int V_37 = 0 ;
int V_856 ;
if ( ! V_2 -> V_149 ) {
if ( V_2 -> V_12 . type == V_15 )
V_856 = V_830 ;
else
V_856 = V_832 ;
V_37 = F_400 ( V_2 , V_856 ) ;
if ( V_37 )
goto V_840;
V_37 = V_2 -> V_149 -> V_852 ( V_2 -> V_10 ) ;
if ( V_37 )
goto V_840;
}
V_37 = V_2 -> V_149 -> V_857 ( V_10 ) ;
V_840:
return V_37 ;
}
static int F_418 ( struct V_160 * V_10 )
{
struct V_1 * V_2 = F_95 ( & V_10 -> V_9 ) ;
return V_2 -> V_149 -> V_858 ( V_10 ) ;
}
static void F_419 ( struct V_160 * V_10 )
{
struct V_1 * V_2 = F_95 ( & V_10 -> V_9 ) ;
if ( V_2 -> V_149 && V_2 -> V_149 -> V_859 )
V_2 -> V_149 -> V_859 ( V_10 ) ;
}
static int F_420 ( struct V_160 * V_10 )
{
struct V_1 * V_2 = F_95 ( & V_10 -> V_9 ) ;
if ( V_2 -> V_149 && V_2 -> V_149 -> V_860 )
return V_2 -> V_149 -> V_860 ( V_10 ) ;
return 0 ;
}
static void F_421 ( struct V_160 * V_10 )
{
struct V_1 * V_2 = F_95 ( & V_10 -> V_9 ) ;
if ( V_2 -> V_149 && V_2 -> V_149 -> V_861 )
V_2 -> V_149 -> V_861 ( V_10 ) ;
}
static int F_422 ( struct V_160 * V_10 )
{
struct V_1 * V_2 = F_95 ( & V_10 -> V_9 ) ;
if ( V_2 -> V_149 && V_2 -> V_149 -> V_862 )
return V_2 -> V_149 -> V_862 ( V_10 ) ;
return 0 ;
}
static int F_423 ( struct V_160 * V_10 )
{
struct V_1 * V_2 = F_95 ( & V_10 -> V_9 ) ;
if ( V_2 -> V_149 && V_2 -> V_149 -> V_863 )
return V_2 -> V_149 -> V_863 ( V_10 ) ;
return 0 ;
}
static int F_424 ( struct V_160 * V_10 )
{
struct V_1 * V_2 = F_95 ( & V_10 -> V_9 ) ;
if ( V_2 -> V_149 && V_2 -> V_149 -> V_864 )
return V_2 -> V_149 -> V_864 ( V_10 ) ;
return 0 ;
}
static T_7 F_425 ( struct V_865 * V_866 ,
const char * V_231 , T_8 V_167 )
{
int V_840 ;
V_840 = F_426 ( V_867 ,
& V_868 , 3 , V_231 ) ;
return V_840 ? V_840 : V_167 ;
}
int F_427 ( struct V_631 * V_9 , int V_869 )
{
switch ( V_869 ) {
case V_870 :
return ( sizeof( V_871 ) / V_872 ) ;
default:
return - V_401 ;
}
}
void F_428 ( struct V_631 * V_9 ,
struct V_873 * V_523 , T_9 * V_117 )
{
struct V_1 * V_2 = V_9 -> V_637 ;
V_117 [ 0 ] = V_2 -> V_523 . V_874 -
V_2 -> V_437 . V_875 ;
V_117 [ 1 ] = V_2 -> V_437 . V_876 ;
V_117 [ 2 ] = V_2 -> V_523 . V_877 -
V_2 -> V_437 . V_878 ;
V_117 [ 3 ] = V_2 -> V_437 . V_551 ;
V_117 [ 4 ] = V_2 -> V_523 . V_877 - V_2 -> V_437 . V_878
- V_2 -> V_437 . V_621 ;
V_117 [ 5 ] = V_2 -> V_437 . V_551 - V_2 -> V_437 . V_555 ;
V_117 [ 6 ] = V_2 -> V_437 . V_621 ;
V_117 [ 7 ] = V_2 -> V_437 . V_555 ;
V_117 [ 8 ] = V_2 -> V_437 . V_879 ;
V_117 [ 9 ] = V_2 -> V_437 . V_880 ;
V_117 [ 10 ] = V_2 -> V_437 . V_812 ;
V_117 [ 11 ] = V_2 -> V_437 . V_813 ;
V_117 [ 12 ] = V_2 -> V_437 . V_438 ;
V_117 [ 13 ] = ( V_2 -> V_437 . V_881 >> 10 ) ;
V_117 [ 14 ] = V_2 -> V_437 . V_882 ;
V_117 [ 15 ] = V_2 -> V_437 . V_538 ;
V_117 [ 16 ] = V_2 -> V_437 . V_541 ;
V_117 [ 17 ] = V_540 ;
V_117 [ 18 ] = V_537 ;
V_117 [ 19 ] = F_51 ( & V_2 -> V_46 . V_80 [ 0 ] -> V_445 ) ;
V_117 [ 20 ] = ( V_2 -> V_46 . V_78 > 1 ) ?
F_51 ( & V_2 -> V_46 . V_80 [ 1 ] -> V_445 ) : 0 ;
V_117 [ 21 ] = ( V_2 -> V_46 . V_78 > 2 ) ?
F_51 ( & V_2 -> V_46 . V_80 [ 2 ] -> V_445 ) : 0 ;
V_117 [ 22 ] = ( V_2 -> V_46 . V_78 > 3 ) ?
F_51 ( & V_2 -> V_46 . V_80 [ 3 ] -> V_445 ) : 0 ;
V_117 [ 23 ] = V_2 -> V_437 . V_883 ;
V_117 [ 24 ] = V_2 -> V_437 . V_884 ;
V_117 [ 25 ] = V_2 -> V_437 . V_532 ;
V_117 [ 26 ] = V_2 -> V_437 . V_530 ;
V_117 [ 27 ] = V_2 -> V_437 . V_575 ;
V_117 [ 28 ] = V_2 -> V_437 . V_571 ;
V_117 [ 29 ] = V_2 -> V_437 . V_885 ;
V_117 [ 30 ] = V_2 -> V_437 . V_886 ;
V_117 [ 31 ] = V_2 -> V_437 . V_548 ;
V_117 [ 32 ] = V_2 -> V_437 . V_544 ;
V_117 [ 33 ] = V_2 -> V_437 . V_887 ;
V_117 [ 34 ] = V_2 -> V_437 . V_888 ;
V_117 [ 35 ] = V_2 -> V_437 . V_889 ;
V_117 [ 36 ] = V_2 -> V_437 . V_562 ;
V_117 [ 37 ] = V_2 -> V_437 . V_567 ;
}
void F_429 ( struct V_631 * V_9 , V_708 V_869 , V_279 * V_117 )
{
switch ( V_869 ) {
case V_870 :
memcpy ( V_117 , & V_871 ,
sizeof( V_871 ) ) ;
break;
default:
F_75 ( 1 ) ;
break;
}
}
void F_430 ( struct V_631 * V_9 ,
struct V_890 * V_12 )
{
struct V_1 * V_2 = V_9 -> V_637 ;
F_431 ( V_12 -> V_891 , V_2 -> V_66 . V_303 ? L_325 : L_324 ,
sizeof( V_12 -> V_891 ) ) ;
F_431 ( V_12 -> V_892 , L_332 , sizeof( V_12 -> V_892 ) ) ;
F_431 ( V_12 -> V_893 , V_2 -> V_12 . V_300 ,
sizeof( V_12 -> V_893 ) ) ;
snprintf ( V_12 -> V_894 , sizeof( V_12 -> V_894 ) , L_333 ,
F_432 ( V_2 ) , F_433 ( V_2 ) , F_434 ( V_2 ) ) ;
}
static void F_435 ( struct V_895 * V_896 ,
int V_897 , int V_898 )
{
int V_899 , V_900 , V_901 , V_902 ;
switch ( V_898 ) {
case V_903 :
V_899 = V_904 ;
V_900 = V_905 ;
break;
case V_906 :
V_899 = V_907 ;
V_900 = V_908 ;
break;
default:
V_899 = V_904 ;
V_900 = V_905 ;
F_54 ( 1 ) ;
}
V_901 = 0 ;
V_902 = 0 ;
switch ( V_897 ) {
case V_909 :
V_901 |= V_910 ;
V_902 |= V_911 ;
case V_912 :
V_901 |= V_913 | V_914 ;
V_902 |= V_915 |
V_916 ;
case V_917 :
V_901 |= V_918 | V_919 ;
V_902 |= V_920 | V_921 ;
case V_922 :
V_901 |= V_923 | V_924 ;
V_902 |= V_925 | V_926 ;
break;
default:
V_901 = V_923 | V_924 ;
V_902 = V_925 | V_926 ;
F_54 ( 1 ) ;
}
V_896 -> V_927 = V_928 | V_900 | V_902 ;
V_896 -> V_929 = V_930 | V_899 | V_901 ;
}
int F_436 ( struct V_631 * V_931 ,
struct V_895 * V_896 )
{
struct V_1 * V_2 = V_931 -> V_637 ;
enum V_447 V_19 ;
struct V_745 V_745 ;
int V_37 ;
V_708 V_932 ;
if ( ( V_2 -> V_12 . type == V_15 ) || ( V_2 -> V_12 . V_13 ) )
V_19 = V_23 ;
else
V_19 = V_2 -> V_12 . V_19 ;
V_896 -> V_933 = V_934 ;
V_896 -> V_935 = V_936 ;
V_896 -> V_937 = V_938 ;
switch ( V_19 ) {
case V_20 :
case V_24 :
F_435 ( V_896 , V_917 , V_903 ) ;
V_932 = V_917 ;
V_896 -> V_939 = V_903 ;
break;
case V_22 :
case V_26 :
F_435 ( V_896 , V_912 , V_906 ) ;
V_932 = V_912 ;
V_896 -> V_939 = V_906 ;
break;
case V_23 :
F_435 ( V_896 , V_909 , V_906 ) ;
V_932 = V_909 ;
V_896 -> V_939 = V_906 ;
break;
default:
F_435 ( V_896 , V_922 , V_903 ) ;
V_932 = V_922 ;
V_896 -> V_939 = V_903 ;
}
F_437 ( V_896 , V_932 ) ;
if ( ! F_1 ( V_2 ) )
return - V_789 ;
V_37 = V_743 ( V_2 , & V_745 ) ;
if ( V_37 == - V_336 )
return 0 ;
if ( V_37 )
return V_37 ;
F_438 ( V_931 ,
L_334 ,
V_745 . V_746 ,
V_745 . V_747 ,
V_745 . V_748 ) ;
switch ( V_745 . V_746 ) {
case V_940 :
case V_941 :
F_435 ( V_896 , V_912 , V_903 ) ;
V_896 -> V_939 = V_903 ;
break;
case V_942 :
case V_943 :
F_435 ( V_896 , V_912 , V_906 ) ;
V_896 -> V_939 = V_906 ;
break;
case V_944 :
case V_945 :
F_435 ( V_896 , V_909 , V_906 ) ;
V_896 -> V_939 = V_906 ;
break;
}
switch ( V_745 . V_747 ) {
case V_946 :
V_896 -> V_935 = V_936 ;
break;
case V_947 :
V_896 -> V_935 = V_948 ;
break;
}
switch ( V_745 . V_748 ) {
case V_949 :
V_932 = V_922 ;
break;
case V_950 :
V_932 = V_917 ;
break;
case V_951 :
V_932 = V_912 ;
break;
case V_952 :
V_932 = V_909 ;
break;
}
F_437 ( V_896 , V_932 ) ;
return 0 ;
}
static int F_439 ( struct V_1 * V_2 ,
struct V_124 * V_125 ,
unsigned long V_117 )
{
struct V_129 * V_130 = (struct V_129 * ) V_117 ;
struct V_953 * V_954 =
(struct V_953 * ) V_125 -> V_382 ;
F_4 ( V_2 , 4 , L_335 ) ;
if ( V_130 -> V_133 . V_139 )
return 0 ;
memset ( V_954 , 0 , sizeof( * V_954 ) ) ;
if ( V_130 -> V_117 . V_814 . V_133 . V_471 == V_955 ) {
V_954 -> V_929 =
V_130 -> V_117 . V_814 . V_117 . V_956 . V_929 ;
F_52 ( V_2 , 3 , L_336 , V_954 -> V_929 ) ;
}
if ( V_130 -> V_117 . V_814 . V_133 . V_471 == V_957 ) {
V_954 -> V_929 =
V_130 -> V_117 . V_814 . V_117 . V_956 . V_929 ;
V_954 -> V_290 =
V_130 -> V_117 . V_814 . V_117 . V_956 . V_290 ;
F_52 ( V_2 , 3 , L_337 , V_954 -> V_929 ) ;
F_52 ( V_2 , 3 , L_338 , V_954 -> V_290 ) ;
}
return 0 ;
}
static int F_440 ( struct V_1 * V_2 ,
enum V_815 V_816 ,
V_185 V_337 , long V_117 ,
struct V_953 * V_954 )
{
struct V_112 * V_113 ;
int V_37 = - V_50 ;
F_4 ( V_2 , 4 , L_339 ) ;
V_113 = F_388 ( V_2 , V_816 , V_337 ,
sizeof( T_1 ) , V_389 ) ;
if ( V_113 )
V_37 = F_389 ( V_2 , V_113 , sizeof( T_1 ) , V_117 ,
F_439 ,
V_954 ) ;
return V_37 ;
}
static int F_441 ( struct V_1 * V_2 , int V_958 )
{
const T_1 V_959 = V_960 |
V_961 |
V_962 ;
struct V_953 V_954 ;
int V_37 ;
V_37 = F_440 ( V_2 , V_958 , V_955 , 0 ,
& V_954 ) ;
if ( ! V_37 ) {
if ( ( V_959 & V_954 . V_929 ) !=
V_959 )
V_37 = - V_116 ;
else if ( ! ( V_963 & V_954 . V_929 ) &&
V_958 == V_964 )
F_67 ( & V_2 -> V_10 -> V_9 ,
L_340 ,
F_81 ( V_2 ) ) ;
}
if ( V_37 ) {
F_390 ( V_2 , V_958 , V_965 , 0 ) ;
F_67 ( & V_2 -> V_10 -> V_9 ,
L_341 ,
F_81 ( V_2 ) ) ;
return V_37 ;
}
V_37 = F_440 ( V_2 , V_958 , V_957 ,
V_954 . V_929 , & V_954 ) ;
if ( ! V_37 ) {
if ( ( V_959 & V_954 . V_290 ) !=
V_959 )
V_37 = - V_116 ;
}
if ( V_37 ) {
F_390 ( V_2 , V_958 , V_965 , 0 ) ;
F_67 ( & V_2 -> V_10 -> V_9 ,
L_342 ,
F_81 ( V_2 ) ) ;
return V_37 ;
}
F_89 ( & V_2 -> V_10 -> V_9 , L_343 ,
V_958 == V_964 ? L_344 : L_345 ) ;
return 0 ;
}
static int F_442 ( struct V_1 * V_2 , int V_966 , int V_958 )
{
int V_37 = ( V_966 ) ? F_441 ( V_2 , V_958 )
: F_390 ( V_2 , V_958 ,
V_965 , 0 ) ;
return V_37 ? - V_116 : 0 ;
}
static int F_443 ( struct V_1 * V_2 , int V_966 )
{
int V_37 ;
F_4 ( V_2 , 3 , L_346 ) ;
if ( V_966 ) {
V_37 = F_390 ( V_2 , V_967 ,
V_955 , 0 ) ;
if ( V_37 ) {
F_67 ( & V_2 -> V_10 -> V_9 ,
L_347 ,
F_81 ( V_2 ) ) ;
return - V_116 ;
}
F_89 ( & V_2 -> V_10 -> V_9 , L_348 ) ;
} else {
V_37 = F_390 ( V_2 , V_967 ,
V_965 , 0 ) ;
}
return V_37 ;
}
int F_444 ( struct V_631 * V_9 )
{
struct V_1 * V_2 = V_9 -> V_637 ;
T_10 V_282 = V_9 -> V_968 ;
if ( V_282 & V_969 ) {
if ( F_442 ( V_2 , 1 , V_970 ) )
V_282 ^= V_969 ;
}
if ( V_282 & V_971 ) {
if ( F_442 ( V_2 , 1 , V_964 ) )
V_282 ^= V_971 ;
}
if ( V_282 & V_972 ) {
if ( F_443 ( V_2 , 1 ) )
V_282 ^= V_972 ;
}
if ( V_282 == V_9 -> V_968 )
return 0 ;
F_67 ( & V_2 -> V_10 -> V_9 ,
L_349 ) ;
V_9 -> V_968 = V_282 ;
return - V_116 ;
}
int F_445 ( struct V_631 * V_9 , T_10 V_968 )
{
struct V_1 * V_2 = V_9 -> V_637 ;
T_10 V_973 = V_9 -> V_968 ^ V_968 ;
int V_37 = 0 ;
F_41 ( V_65 , 2 , L_350 ) ;
F_39 ( V_65 , 2 , & V_968 , sizeof( V_968 ) ) ;
if ( ( V_973 & V_969 ) ) {
V_37 = F_442 ( V_2 ,
V_968 & V_969 ? 1 : 0 ,
V_970 ) ;
if ( V_37 )
V_973 ^= V_969 ;
}
if ( ( V_973 & V_971 ) ) {
V_37 = F_442 ( V_2 ,
V_968 & V_971 ? 1 : 0 ,
V_964 ) ;
if ( V_37 )
V_973 ^= V_971 ;
}
if ( ( V_973 & V_972 ) ) {
V_37 = F_443 ( V_2 , V_968 & V_972 ? 1 : 0 ) ;
if ( V_37 )
V_973 ^= V_972 ;
}
if ( ( V_9 -> V_968 ^ V_968 ) == V_973 )
return 0 ;
V_9 -> V_968 ^= V_973 ;
return - V_116 ;
}
T_10 F_446 ( struct V_631 * V_9 ,
T_10 V_968 )
{
struct V_1 * V_2 = V_9 -> V_637 ;
F_41 ( V_65 , 2 , L_351 ) ;
if ( ! F_338 ( V_2 , V_970 ) )
V_968 &= ~ V_969 ;
if ( ! F_338 ( V_2 , V_964 ) )
V_968 &= ~ V_971 ;
if ( ! F_338 ( V_2 , V_967 ) ) {
V_968 &= ~ V_972 ;
F_89 ( & V_2 -> V_10 -> V_9 , L_352 ,
F_81 ( V_2 ) ) ;
}
if ( V_2 -> V_3 == V_55 ||
V_2 -> V_3 == V_56 )
V_968 = V_968 & ~ ( V_969 | V_971 |
V_972 ) ;
F_39 ( V_65 , 2 , & V_968 , sizeof( V_968 ) ) ;
return V_968 ;
}
static int T_11 F_447 ( void )
{
int V_37 ;
F_448 ( L_353 ) ;
F_137 ( & V_854 . V_42 ) ;
F_137 ( & V_845 ) ;
F_449 ( & V_854 . V_855 ) ;
F_143 ( & V_829 ) ;
V_11 = F_450 ( L_354 ) ;
V_37 = F_396 () ;
if ( V_37 )
goto V_974;
V_867 = F_451 ( L_355 ) ;
V_37 = F_452 ( V_867 ) ;
if ( V_37 )
goto V_975;
V_252 = F_453 ( L_356 ,
sizeof( struct V_600 ) + V_441 , 64 , 0 , NULL ) ;
if ( ! V_252 ) {
V_37 = - V_50 ;
goto V_976;
}
V_101 = F_453 ( L_357 ,
sizeof( struct V_96 ) , 0 , 0 , NULL ) ;
if ( ! V_101 ) {
V_37 = - V_50 ;
goto V_977;
}
V_37 = F_454 ( & V_978 ) ;
if ( V_37 )
goto V_979;
V_868 . V_891 . V_980 = V_981 ;
V_37 = F_455 ( & V_868 ) ;
if ( V_37 )
goto V_982;
return 0 ;
V_982:
F_456 ( & V_978 ) ;
V_979:
F_457 ( V_101 ) ;
V_977:
F_457 ( V_252 ) ;
V_976:
F_458 ( V_867 ) ;
V_975:
F_391 () ;
V_974:
F_459 ( L_358 ) ;
return V_37 ;
}
static void T_12 F_460 ( void )
{
F_409 () ;
F_461 ( V_11 ) ;
F_462 ( & V_868 ) ;
F_456 ( & V_978 ) ;
F_457 ( V_101 ) ;
F_457 ( V_252 ) ;
F_458 ( V_867 ) ;
F_391 () ;
F_448 ( L_359 ) ;
}
