static void F_1 ( unsigned long V_1 )
{
struct V_2 * V_3 = (struct V_2 * ) V_1 ;
F_2 ( L_1 , V_3 , V_3 -> V_4 ) ;
F_3 ( V_3 ) ;
V_3 -> V_5 = V_6 ;
V_3 -> V_7 ( V_3 ) ;
F_4 ( V_3 ) ;
F_5 ( V_3 ) ;
F_6 ( V_3 ) ;
}
static void F_7 ( struct V_2 * V_3 , long V_8 )
{
F_2 ( L_2 , V_3 , V_3 -> V_4 , V_8 ) ;
F_8 ( V_3 , & V_3 -> V_9 , V_10 + V_8 ) ;
}
static void F_9 ( struct V_2 * V_3 )
{
F_2 ( L_1 , V_3 , V_3 -> V_4 ) ;
F_10 ( V_3 , & V_3 -> V_9 ) ;
}
static struct V_11 * F_11 ( struct V_12 * V_13 )
{
struct V_14 * V_15 = V_13 -> V_15 ;
struct V_11 * V_16 = V_13 -> V_17 ;
if ( V_16 )
return V_16 ;
V_16 = F_12 ( sizeof( struct V_11 ) , V_18 ) ;
if ( ! V_16 )
return NULL ;
F_13 ( & V_16 -> V_19 ) ;
V_13 -> V_17 = V_16 ;
V_16 -> V_13 = V_13 ;
if ( V_15 -> V_20 > 0 )
V_16 -> V_21 = V_15 -> V_20 ;
else
V_16 -> V_21 = 60 ;
F_2 ( L_3 , V_13 , V_16 ) ;
return V_16 ;
}
static void F_14 ( struct V_2 * V_3 , int V_22 )
{
struct V_11 * V_16 ;
V_16 = F_15 ( V_3 ) -> V_16 ;
F_2 ( L_4 , V_3 , V_16 , V_22 ) ;
if ( V_16 ) {
F_16 ( V_16 ) ;
V_16 -> V_3 = NULL ;
F_15 ( V_3 ) -> V_16 = NULL ;
F_17 ( V_16 ) ;
if ( V_16 -> V_13 )
F_18 ( V_16 -> V_13 ) ;
}
V_3 -> V_4 = V_23 ;
V_3 -> V_5 = V_22 ;
V_3 -> V_7 ( V_3 ) ;
F_19 ( V_3 , V_24 ) ;
}
static int F_20 ( struct V_12 * V_13 , int V_22 )
{
struct V_11 * V_16 = V_13 -> V_17 ;
struct V_2 * V_3 ;
if ( ! V_16 )
return 0 ;
F_2 ( L_5 , V_13 , V_16 , V_22 ) ;
F_16 ( V_16 ) ;
V_3 = V_16 -> V_3 ;
F_17 ( V_16 ) ;
if ( V_3 ) {
F_3 ( V_3 ) ;
F_9 ( V_3 ) ;
F_14 ( V_3 , V_22 ) ;
F_4 ( V_3 ) ;
F_5 ( V_3 ) ;
}
V_13 -> V_17 = NULL ;
F_21 ( V_16 ) ;
return 0 ;
}
static void F_22 ( struct V_11 * V_16 , struct V_2 * V_3 , struct V_2 * V_25 )
{
F_2 ( L_6 , V_16 ) ;
F_15 ( V_3 ) -> V_16 = V_16 ;
V_16 -> V_3 = V_3 ;
if ( V_25 )
F_23 ( V_25 , V_3 ) ;
}
static int F_24 ( struct V_11 * V_16 , struct V_2 * V_3 ,
struct V_2 * V_25 )
{
int V_22 = 0 ;
F_16 ( V_16 ) ;
if ( V_16 -> V_3 )
V_22 = - V_26 ;
else
F_22 ( V_16 , V_3 , V_25 ) ;
F_17 ( V_16 ) ;
return V_22 ;
}
static int F_25 ( struct V_2 * V_3 )
{
struct V_11 * V_16 ;
struct V_12 * V_13 ;
struct V_14 * V_15 ;
int V_22 , type ;
F_2 ( L_7 , & F_15 ( V_3 ) -> V_27 , & F_15 ( V_3 ) -> V_28 ) ;
V_15 = F_26 ( & F_15 ( V_3 ) -> V_28 , & F_15 ( V_3 ) -> V_27 ) ;
if ( ! V_15 )
return - V_29 ;
F_27 ( V_15 ) ;
if ( F_28 ( V_15 ) && ! V_30 )
type = V_31 ;
else
type = V_32 ;
if ( F_15 ( V_3 ) -> V_33 == V_34 &&
( ! F_29 ( V_15 ) || ! F_28 ( V_15 ) ) ) {
V_22 = - V_35 ;
goto V_36;
}
V_13 = F_30 ( V_15 , type , & F_15 ( V_3 ) -> V_28 ,
F_15 ( V_3 ) -> V_33 ) ;
if ( F_31 ( V_13 ) ) {
V_22 = F_32 ( V_13 ) ;
goto V_36;
}
V_16 = F_11 ( V_13 ) ;
if ( ! V_16 ) {
F_18 ( V_13 ) ;
V_22 = - V_37 ;
goto V_36;
}
F_33 ( & F_15 ( V_3 ) -> V_27 , & V_13 -> V_27 ) ;
V_22 = F_24 ( V_16 , V_3 , NULL ) ;
if ( V_22 )
goto V_36;
if ( V_13 -> V_38 == V_39 ) {
F_9 ( V_3 ) ;
V_3 -> V_4 = V_39 ;
} else {
V_3 -> V_4 = V_40 ;
F_7 ( V_3 , V_3 -> V_41 ) ;
}
V_36:
F_34 ( V_15 ) ;
F_35 ( V_15 ) ;
return V_22 ;
}
static int F_36 ( struct V_2 * V_3 , struct V_42 * V_43 , int V_44 )
{
struct V_11 * V_16 = F_15 ( V_3 ) -> V_16 ;
struct V_45 * V_46 ;
int V_22 ;
if ( V_44 > V_16 -> V_21 )
return - V_47 ;
F_2 ( L_8 , V_3 , V_44 ) ;
V_46 = F_37 ( V_3 , V_44 , V_43 -> V_48 & V_49 , & V_22 ) ;
if ( ! V_46 )
return V_22 ;
if ( F_38 ( F_39 ( V_46 , V_44 ) , V_43 -> V_50 , V_44 ) ) {
F_40 ( V_46 ) ;
return - V_51 ;
}
F_41 ( V_16 -> V_13 , V_46 ) ;
return V_44 ;
}
static void F_42 ( struct V_11 * V_16 , struct V_45 * V_46 )
{
struct V_2 * V_3 ;
F_16 ( V_16 ) ;
V_3 = V_16 -> V_3 ;
F_17 ( V_16 ) ;
if ( ! V_3 )
goto V_52;
F_2 ( L_8 , V_3 , V_46 -> V_44 ) ;
if ( V_3 -> V_4 != V_39 )
goto V_52;
if ( ! F_43 ( V_3 , V_46 ) )
return;
V_52:
F_40 ( V_46 ) ;
}
static struct V_2 * F_44 ( T_1 * V_53 )
{
struct V_2 * V_3 ;
F_45 (sk, &sco_sk_list.head) {
if ( V_3 -> V_4 != V_54 )
continue;
if ( ! F_46 ( & F_15 ( V_3 ) -> V_27 , V_53 ) )
return V_3 ;
}
return NULL ;
}
static struct V_2 * F_47 ( T_1 * V_27 )
{
struct V_2 * V_3 = NULL , * V_55 = NULL ;
F_48 ( & V_56 . V_19 ) ;
F_45 (sk, &sco_sk_list.head) {
if ( V_3 -> V_4 != V_54 )
continue;
if ( ! F_46 ( & F_15 ( V_3 ) -> V_27 , V_27 ) )
break;
if ( ! F_46 ( & F_15 ( V_3 ) -> V_27 , V_57 ) )
V_55 = V_3 ;
}
F_49 ( & V_56 . V_19 ) ;
return V_3 ? V_3 : V_55 ;
}
static void F_50 ( struct V_2 * V_3 )
{
F_2 ( L_9 , V_3 ) ;
F_51 ( & V_3 -> V_58 ) ;
F_51 ( & V_3 -> V_59 ) ;
}
static void F_52 ( struct V_2 * V_25 )
{
struct V_2 * V_3 ;
F_2 ( L_10 , V_25 ) ;
while ( ( V_3 = F_53 ( V_25 , NULL ) ) ) {
F_54 ( V_3 ) ;
F_5 ( V_3 ) ;
}
V_25 -> V_4 = V_23 ;
F_19 ( V_25 , V_24 ) ;
}
static void F_5 ( struct V_2 * V_3 )
{
if ( ! F_55 ( V_3 , V_24 ) || V_3 -> V_60 )
return;
F_2 ( L_11 , V_3 , V_3 -> V_4 ) ;
F_56 ( & V_56 , V_3 ) ;
F_19 ( V_3 , V_61 ) ;
F_6 ( V_3 ) ;
}
static void F_57 ( struct V_2 * V_3 )
{
F_2 ( L_12 , V_3 , V_3 -> V_4 , V_3 -> V_60 ) ;
switch ( V_3 -> V_4 ) {
case V_54 :
F_52 ( V_3 ) ;
break;
case V_39 :
case V_62 :
if ( F_15 ( V_3 ) -> V_16 -> V_13 ) {
V_3 -> V_4 = V_63 ;
F_7 ( V_3 , V_64 ) ;
F_18 ( F_15 ( V_3 ) -> V_16 -> V_13 ) ;
F_15 ( V_3 ) -> V_16 -> V_13 = NULL ;
} else
F_14 ( V_3 , V_65 ) ;
break;
case V_66 :
case V_40 :
case V_63 :
F_14 ( V_3 , V_65 ) ;
break;
default:
F_19 ( V_3 , V_24 ) ;
break;
}
}
static void F_54 ( struct V_2 * V_3 )
{
F_9 ( V_3 ) ;
F_58 ( V_3 ) ;
F_57 ( V_3 ) ;
F_59 ( V_3 ) ;
F_5 ( V_3 ) ;
}
static void F_60 ( struct V_2 * V_3 , struct V_2 * V_25 )
{
F_2 ( L_9 , V_3 ) ;
if ( V_25 ) {
V_3 -> V_67 = V_25 -> V_67 ;
F_61 ( V_3 ) -> V_68 = F_61 ( V_25 ) -> V_68 ;
F_62 ( V_25 , V_3 ) ;
}
}
static struct V_2 * F_63 ( struct V_69 * V_69 , struct V_70 * V_2 , int V_71 , T_2 V_72 )
{
struct V_2 * V_3 ;
V_3 = F_64 ( V_69 , V_73 , V_72 , & V_74 ) ;
if ( ! V_3 )
return NULL ;
F_65 ( V_2 , V_3 ) ;
F_66 ( & F_61 ( V_3 ) -> V_75 ) ;
V_3 -> V_76 = F_50 ;
V_3 -> V_41 = V_77 ;
F_67 ( V_3 , V_24 ) ;
V_3 -> V_78 = V_71 ;
V_3 -> V_4 = V_79 ;
F_15 ( V_3 ) -> V_33 = V_80 ;
F_68 ( & V_3 -> V_9 , F_1 , ( unsigned long ) V_3 ) ;
F_69 ( & V_56 , V_3 ) ;
return V_3 ;
}
static int F_70 ( struct V_69 * V_69 , struct V_70 * V_2 , int V_81 ,
int V_82 )
{
struct V_2 * V_3 ;
F_2 ( L_13 , V_2 ) ;
V_2 -> V_38 = V_83 ;
if ( V_2 -> type != V_84 )
return - V_85 ;
V_2 -> V_86 = & V_87 ;
V_3 = F_63 ( V_69 , V_2 , V_81 , V_88 ) ;
if ( ! V_3 )
return - V_37 ;
F_60 ( V_3 , NULL ) ;
return 0 ;
}
static int F_71 ( struct V_70 * V_2 , struct V_89 * V_90 , int V_91 )
{
struct V_92 * V_93 = (struct V_92 * ) V_90 ;
struct V_2 * V_3 = V_2 -> V_3 ;
int V_22 = 0 ;
F_2 ( L_14 , V_3 , & V_93 -> V_94 ) ;
if ( ! V_90 || V_90 -> V_95 != V_96 )
return - V_47 ;
F_58 ( V_3 ) ;
if ( V_3 -> V_4 != V_79 ) {
V_22 = - V_97 ;
goto V_36;
}
if ( V_3 -> V_67 != V_84 ) {
V_22 = - V_47 ;
goto V_36;
}
F_33 ( & F_15 ( V_3 ) -> V_27 , & V_93 -> V_94 ) ;
V_3 -> V_4 = V_98 ;
V_36:
F_59 ( V_3 ) ;
return V_22 ;
}
static int F_72 ( struct V_70 * V_2 , struct V_89 * V_90 , int V_99 , int V_68 )
{
struct V_92 * V_93 = (struct V_92 * ) V_90 ;
struct V_2 * V_3 = V_2 -> V_3 ;
int V_22 ;
F_2 ( L_9 , V_3 ) ;
if ( V_99 < sizeof( struct V_92 ) ||
V_90 -> V_95 != V_96 )
return - V_47 ;
if ( V_3 -> V_4 != V_79 && V_3 -> V_4 != V_98 )
return - V_97 ;
if ( V_3 -> V_67 != V_84 )
return - V_47 ;
F_58 ( V_3 ) ;
F_33 ( & F_15 ( V_3 ) -> V_28 , & V_93 -> V_94 ) ;
V_22 = F_25 ( V_3 ) ;
if ( V_22 )
goto V_36;
V_22 = F_73 ( V_3 , V_39 ,
F_74 ( V_3 , V_68 & V_100 ) ) ;
V_36:
F_59 ( V_3 ) ;
return V_22 ;
}
static int F_75 ( struct V_70 * V_2 , int V_101 )
{
struct V_2 * V_3 = V_2 -> V_3 ;
T_1 * V_27 = & F_15 ( V_3 ) -> V_27 ;
int V_22 = 0 ;
F_2 ( L_15 , V_3 , V_101 ) ;
F_58 ( V_3 ) ;
if ( V_3 -> V_4 != V_98 ) {
V_22 = - V_97 ;
goto V_36;
}
if ( V_3 -> V_67 != V_84 ) {
V_22 = - V_47 ;
goto V_36;
}
F_76 ( & V_56 . V_19 ) ;
if ( F_44 ( V_27 ) ) {
V_22 = - V_102 ;
goto V_103;
}
V_3 -> V_104 = V_101 ;
V_3 -> V_105 = 0 ;
V_3 -> V_4 = V_54 ;
V_103:
F_77 ( & V_56 . V_19 ) ;
V_36:
F_59 ( V_3 ) ;
return V_22 ;
}
static int F_78 ( struct V_70 * V_2 , struct V_70 * V_106 , int V_68 )
{
F_79 ( V_107 , V_108 ) ;
struct V_2 * V_3 = V_2 -> V_3 , * V_109 ;
long V_110 ;
int V_22 = 0 ;
F_58 ( V_3 ) ;
V_110 = F_80 ( V_3 , V_68 & V_100 ) ;
F_2 ( L_16 , V_3 , V_110 ) ;
F_81 ( F_82 ( V_3 ) , & V_107 ) ;
while ( 1 ) {
F_83 ( V_111 ) ;
if ( V_3 -> V_4 != V_54 ) {
V_22 = - V_97 ;
break;
}
V_109 = F_53 ( V_3 , V_106 ) ;
if ( V_109 )
break;
if ( ! V_110 ) {
V_22 = - V_112 ;
break;
}
if ( F_84 ( V_108 ) ) {
V_22 = F_85 ( V_110 ) ;
break;
}
F_59 ( V_3 ) ;
V_110 = F_86 ( V_110 ) ;
F_58 ( V_3 ) ;
}
F_87 ( V_113 ) ;
F_88 ( F_82 ( V_3 ) , & V_107 ) ;
if ( V_22 )
goto V_36;
V_106 -> V_38 = V_114 ;
F_2 ( L_17 , V_109 ) ;
V_36:
F_59 ( V_3 ) ;
return V_22 ;
}
static int F_89 ( struct V_70 * V_2 , struct V_89 * V_90 , int * V_44 , int V_115 )
{
struct V_92 * V_93 = (struct V_92 * ) V_90 ;
struct V_2 * V_3 = V_2 -> V_3 ;
F_2 ( L_18 , V_2 , V_3 ) ;
V_90 -> V_95 = V_96 ;
* V_44 = sizeof( struct V_92 ) ;
if ( V_115 )
F_33 ( & V_93 -> V_94 , & F_15 ( V_3 ) -> V_28 ) ;
else
F_33 ( & V_93 -> V_94 , & F_15 ( V_3 ) -> V_27 ) ;
return 0 ;
}
static int F_90 ( struct V_116 * V_117 , struct V_70 * V_2 ,
struct V_42 * V_43 , T_3 V_44 )
{
struct V_2 * V_3 = V_2 -> V_3 ;
int V_22 ;
F_2 ( L_18 , V_2 , V_3 ) ;
V_22 = F_91 ( V_3 ) ;
if ( V_22 )
return V_22 ;
if ( V_43 -> V_48 & V_118 )
return - V_35 ;
F_58 ( V_3 ) ;
if ( V_3 -> V_4 == V_39 )
V_22 = F_36 ( V_3 , V_43 , V_44 ) ;
else
V_22 = - V_119 ;
F_59 ( V_3 ) ;
return V_22 ;
}
static void F_92 ( struct V_12 * V_16 , T_4 V_33 )
{
struct V_14 * V_15 = V_16 -> V_15 ;
F_2 ( L_6 , V_16 ) ;
V_16 -> V_38 = V_62 ;
if ( ! F_28 ( V_15 ) ) {
struct V_120 V_121 ;
F_33 ( & V_121 . V_122 , & V_16 -> V_28 ) ;
V_121 . V_123 = 0x00 ;
F_93 ( V_15 , V_124 , sizeof( V_121 ) , & V_121 ) ;
} else {
struct V_125 V_121 ;
F_33 ( & V_121 . V_122 , & V_16 -> V_28 ) ;
V_121 . V_126 = F_94 ( V_16 -> V_126 ) ;
V_121 . V_127 = F_95 ( 0x00001f40 ) ;
V_121 . V_128 = F_95 ( 0x00001f40 ) ;
V_121 . V_129 = F_94 ( V_33 ) ;
switch ( V_33 & V_130 ) {
case V_131 :
if ( V_16 -> V_126 & V_132 )
V_121 . V_133 = F_94 ( 0x0008 ) ;
else
V_121 . V_133 = F_94 ( 0x000D ) ;
V_121 . V_134 = 0x02 ;
break;
case V_135 :
V_121 . V_133 = F_94 ( 0xffff ) ;
V_121 . V_134 = 0xff ;
break;
}
F_93 ( V_15 , V_136 ,
sizeof( V_121 ) , & V_121 ) ;
}
}
static int F_96 ( struct V_116 * V_117 , struct V_70 * V_2 ,
struct V_42 * V_43 , T_3 V_44 , int V_68 )
{
struct V_2 * V_3 = V_2 -> V_3 ;
struct V_137 * V_138 = F_15 ( V_3 ) ;
F_58 ( V_3 ) ;
if ( V_3 -> V_4 == V_66 &&
F_97 ( V_139 , & F_61 ( V_3 ) -> V_68 ) ) {
F_92 ( V_138 -> V_16 -> V_13 , V_138 -> V_33 ) ;
V_3 -> V_4 = V_62 ;
F_59 ( V_3 ) ;
return 0 ;
}
F_59 ( V_3 ) ;
return F_98 ( V_117 , V_2 , V_43 , V_44 , V_68 ) ;
}
static int F_99 ( struct V_70 * V_2 , int V_140 , int V_141 , char T_5 * V_142 , unsigned int V_143 )
{
struct V_2 * V_3 = V_2 -> V_3 ;
int V_44 , V_22 = 0 ;
struct V_144 V_145 ;
T_6 V_146 ;
F_2 ( L_9 , V_3 ) ;
F_58 ( V_3 ) ;
switch ( V_141 ) {
case V_147 :
if ( V_3 -> V_4 != V_98 && V_3 -> V_4 != V_54 ) {
V_22 = - V_47 ;
break;
}
if ( F_100 ( V_146 , ( T_6 T_5 * ) V_142 ) ) {
V_22 = - V_51 ;
break;
}
if ( V_146 )
F_101 ( V_139 , & F_61 ( V_3 ) -> V_68 ) ;
else
F_102 ( V_139 , & F_61 ( V_3 ) -> V_68 ) ;
break;
case V_148 :
if ( V_3 -> V_4 != V_79 && V_3 -> V_4 != V_98 &&
V_3 -> V_4 != V_66 ) {
V_22 = - V_47 ;
break;
}
V_145 . V_33 = F_15 ( V_3 ) -> V_33 ;
V_44 = F_103 (unsigned int, sizeof(voice), optlen) ;
if ( F_104 ( ( char * ) & V_145 , V_142 , V_44 ) ) {
V_22 = - V_51 ;
break;
}
if ( V_145 . V_33 != V_34 &&
V_145 . V_33 != V_80 ) {
V_22 = - V_47 ;
break;
}
F_15 ( V_3 ) -> V_33 = V_145 . V_33 ;
break;
default:
V_22 = - V_149 ;
break;
}
F_59 ( V_3 ) ;
return V_22 ;
}
static int F_105 ( struct V_70 * V_2 , int V_141 , char T_5 * V_142 , int T_5 * V_143 )
{
struct V_2 * V_3 = V_2 -> V_3 ;
struct V_150 V_151 ;
struct V_152 V_153 ;
int V_44 , V_22 = 0 ;
F_2 ( L_9 , V_3 ) ;
if ( F_100 ( V_44 , V_143 ) )
return - V_51 ;
F_58 ( V_3 ) ;
switch ( V_141 ) {
case V_154 :
if ( V_3 -> V_4 != V_39 &&
! ( V_3 -> V_4 == V_66 &&
F_97 ( V_139 , & F_61 ( V_3 ) -> V_68 ) ) ) {
V_22 = - V_119 ;
break;
}
V_151 . V_21 = F_15 ( V_3 ) -> V_16 -> V_21 ;
F_2 ( L_19 , V_151 . V_21 ) ;
V_44 = F_103 (unsigned int, len, sizeof(opts)) ;
if ( F_106 ( V_142 , ( char * ) & V_151 , V_44 ) )
V_22 = - V_51 ;
break;
case V_155 :
if ( V_3 -> V_4 != V_39 &&
! ( V_3 -> V_4 == V_66 &&
F_97 ( V_139 , & F_61 ( V_3 ) -> V_68 ) ) ) {
V_22 = - V_119 ;
break;
}
memset ( & V_153 , 0 , sizeof( V_153 ) ) ;
V_153 . V_156 = F_15 ( V_3 ) -> V_16 -> V_13 -> V_157 ;
memcpy ( V_153 . V_158 , F_15 ( V_3 ) -> V_16 -> V_13 -> V_158 , 3 ) ;
V_44 = F_103 (unsigned int, len, sizeof(cinfo)) ;
if ( F_106 ( V_142 , ( char * ) & V_153 , V_44 ) )
V_22 = - V_51 ;
break;
default:
V_22 = - V_149 ;
break;
}
F_59 ( V_3 ) ;
return V_22 ;
}
static int F_107 ( struct V_70 * V_2 , int V_140 , int V_141 , char T_5 * V_142 , int T_5 * V_143 )
{
struct V_2 * V_3 = V_2 -> V_3 ;
int V_44 , V_22 = 0 ;
struct V_144 V_145 ;
F_2 ( L_9 , V_3 ) ;
if ( V_140 == V_159 )
return F_105 ( V_2 , V_141 , V_142 , V_143 ) ;
if ( F_100 ( V_44 , V_143 ) )
return - V_51 ;
F_58 ( V_3 ) ;
switch ( V_141 ) {
case V_147 :
if ( V_3 -> V_4 != V_98 && V_3 -> V_4 != V_54 ) {
V_22 = - V_47 ;
break;
}
if ( F_108 ( F_97 ( V_139 , & F_61 ( V_3 ) -> V_68 ) ,
( T_6 T_5 * ) V_142 ) )
V_22 = - V_51 ;
break;
case V_148 :
V_145 . V_33 = F_15 ( V_3 ) -> V_33 ;
V_44 = F_103 (unsigned int, len, sizeof(voice)) ;
if ( F_106 ( V_142 , ( char * ) & V_145 , V_44 ) )
V_22 = - V_51 ;
break;
default:
V_22 = - V_149 ;
break;
}
F_59 ( V_3 ) ;
return V_22 ;
}
static int F_109 ( struct V_70 * V_2 , int V_160 )
{
struct V_2 * V_3 = V_2 -> V_3 ;
int V_22 = 0 ;
F_2 ( L_18 , V_2 , V_3 ) ;
if ( ! V_3 )
return 0 ;
F_58 ( V_3 ) ;
if ( ! V_3 -> V_161 ) {
V_3 -> V_161 = V_162 ;
F_9 ( V_3 ) ;
F_57 ( V_3 ) ;
if ( F_55 ( V_3 , V_163 ) && V_3 -> V_164 &&
! ( V_108 -> V_68 & V_165 ) )
V_22 = F_73 ( V_3 , V_23 ,
V_3 -> V_164 ) ;
}
F_59 ( V_3 ) ;
return V_22 ;
}
static int F_110 ( struct V_70 * V_2 )
{
struct V_2 * V_3 = V_2 -> V_3 ;
int V_22 = 0 ;
F_2 ( L_18 , V_2 , V_3 ) ;
if ( ! V_3 )
return 0 ;
F_54 ( V_3 ) ;
if ( F_55 ( V_3 , V_163 ) && V_3 -> V_164 &&
! ( V_108 -> V_68 & V_165 ) ) {
F_58 ( V_3 ) ;
V_22 = F_73 ( V_3 , V_23 , V_3 -> V_164 ) ;
F_59 ( V_3 ) ;
}
F_111 ( V_3 ) ;
F_5 ( V_3 ) ;
return V_22 ;
}
static void F_112 ( struct V_11 * V_16 )
{
struct V_2 * V_25 ;
struct V_2 * V_3 = V_16 -> V_3 ;
F_2 ( L_6 , V_16 ) ;
if ( V_3 ) {
F_9 ( V_3 ) ;
F_3 ( V_3 ) ;
V_3 -> V_4 = V_39 ;
V_3 -> V_7 ( V_3 ) ;
F_4 ( V_3 ) ;
} else {
F_16 ( V_16 ) ;
V_25 = F_47 ( & V_16 -> V_13 -> V_27 ) ;
if ( ! V_25 ) {
F_17 ( V_16 ) ;
return;
}
F_3 ( V_25 ) ;
V_3 = F_63 ( F_113 ( V_25 ) , NULL ,
V_166 , V_88 ) ;
if ( ! V_3 ) {
F_4 ( V_25 ) ;
F_17 ( V_16 ) ;
return;
}
F_60 ( V_3 , V_25 ) ;
F_33 ( & F_15 ( V_3 ) -> V_27 , & V_16 -> V_13 -> V_27 ) ;
F_33 ( & F_15 ( V_3 ) -> V_28 , & V_16 -> V_13 -> V_28 ) ;
F_114 ( V_16 -> V_13 ) ;
F_22 ( V_16 , V_3 , V_25 ) ;
if ( F_97 ( V_139 , & F_61 ( V_25 ) -> V_68 ) )
V_3 -> V_4 = V_66 ;
else
V_3 -> V_4 = V_39 ;
V_25 -> V_167 ( V_25 ) ;
F_4 ( V_25 ) ;
F_17 ( V_16 ) ;
}
}
int F_115 ( struct V_14 * V_15 , T_1 * V_122 , T_7 * V_68 )
{
struct V_2 * V_3 ;
int V_168 = 0 ;
F_2 ( L_20 , V_15 -> V_169 , V_122 ) ;
F_48 ( & V_56 . V_19 ) ;
F_45 (sk, &sco_sk_list.head) {
if ( V_3 -> V_4 != V_54 )
continue;
if ( ! F_46 ( & F_15 ( V_3 ) -> V_27 , & V_15 -> V_122 ) ||
! F_46 ( & F_15 ( V_3 ) -> V_27 , V_57 ) ) {
V_168 |= V_170 ;
if ( F_97 ( V_139 , & F_61 ( V_3 ) -> V_68 ) )
* V_68 |= V_171 ;
break;
}
}
F_49 ( & V_56 . V_19 ) ;
return V_168 ;
}
void F_116 ( struct V_12 * V_13 , T_7 V_172 )
{
F_2 ( L_21 , V_13 , & V_13 -> V_28 , V_172 ) ;
if ( ! V_172 ) {
struct V_11 * V_16 ;
V_16 = F_11 ( V_13 ) ;
if ( V_16 )
F_112 ( V_16 ) ;
} else
F_20 ( V_13 , F_117 ( V_172 ) ) ;
}
void F_118 ( struct V_12 * V_13 , T_7 V_173 )
{
F_2 ( L_22 , V_13 , V_173 ) ;
F_20 ( V_13 , F_117 ( V_173 ) ) ;
}
int F_119 ( struct V_12 * V_13 , struct V_45 * V_46 )
{
struct V_11 * V_16 = V_13 -> V_17 ;
if ( ! V_16 )
goto V_52;
F_2 ( L_23 , V_16 , V_46 -> V_44 ) ;
if ( V_46 -> V_44 ) {
F_42 ( V_16 , V_46 ) ;
return 0 ;
}
V_52:
F_40 ( V_46 ) ;
return 0 ;
}
static int F_120 ( struct V_174 * V_175 , void * V_176 )
{
struct V_2 * V_3 ;
F_48 ( & V_56 . V_19 ) ;
F_45 (sk, &sco_sk_list.head) {
F_121 ( V_175 , L_24 , & F_15 ( V_3 ) -> V_27 ,
& F_15 ( V_3 ) -> V_28 , V_3 -> V_4 ) ;
}
F_49 ( & V_56 . V_19 ) ;
return 0 ;
}
static int F_122 ( struct V_177 * V_177 , struct V_178 * V_178 )
{
return F_123 ( V_178 , F_120 , V_177 -> V_179 ) ;
}
int T_8 F_124 ( void )
{
int V_22 ;
V_22 = F_125 ( & V_74 , 0 ) ;
if ( V_22 < 0 )
return V_22 ;
V_22 = F_126 ( V_166 , & V_180 ) ;
if ( V_22 < 0 ) {
F_127 ( L_25 ) ;
goto error;
}
V_22 = F_128 ( & V_181 , L_26 , & V_56 , NULL ) ;
if ( V_22 < 0 ) {
F_127 ( L_27 ) ;
F_129 ( V_166 ) ;
goto error;
}
F_130 ( L_28 ) ;
if ( F_131 ( V_182 ) )
return 0 ;
V_183 = F_132 ( L_26 , 0444 , V_182 ,
NULL , & V_184 ) ;
return 0 ;
error:
F_133 ( & V_74 ) ;
return V_22 ;
}
void T_9 F_134 ( void )
{
F_135 ( & V_181 , L_26 ) ;
F_136 ( V_183 ) ;
F_129 ( V_166 ) ;
F_133 ( & V_74 ) ;
}
