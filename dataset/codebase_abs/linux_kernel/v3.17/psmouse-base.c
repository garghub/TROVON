T_1 F_1 ( struct V_1 * V_1 )
{
struct V_2 * V_3 = V_1 -> V_3 ;
unsigned char * V_4 = V_1 -> V_4 ;
if ( V_1 -> V_5 < V_1 -> V_6 )
return V_7 ;
if ( V_1 -> type == V_8 || V_1 -> type == V_9 )
F_2 ( V_3 , V_10 , - ( signed char ) V_4 [ 3 ] ) ;
if ( V_1 -> type == V_11 ) {
switch ( V_4 [ 3 ] & 0xC0 ) {
case 0x80 :
F_2 ( V_3 , V_10 , ( int ) ( V_4 [ 3 ] & 32 ) - ( int ) ( V_4 [ 3 ] & 31 ) ) ;
break;
case 0x40 :
F_2 ( V_3 , V_12 , ( int ) ( V_4 [ 3 ] & 32 ) - ( int ) ( V_4 [ 3 ] & 31 ) ) ;
break;
case 0x00 :
case 0xC0 :
F_2 ( V_3 , V_10 , ( int ) ( V_4 [ 3 ] & 8 ) - ( int ) ( V_4 [ 3 ] & 7 ) ) ;
F_3 ( V_3 , V_13 , ( V_4 [ 3 ] >> 4 ) & 1 ) ;
F_3 ( V_3 , V_14 , ( V_4 [ 3 ] >> 5 ) & 1 ) ;
break;
}
}
if ( V_1 -> type == V_9 ) {
F_3 ( V_3 , V_13 , ( V_4 [ 0 ] >> 6 ) & 1 ) ;
F_3 ( V_3 , V_14 , ( V_4 [ 0 ] >> 7 ) & 1 ) ;
}
if ( V_1 -> type == V_15 ) {
F_3 ( V_3 , V_14 , ( V_4 [ 0 ] >> 3 ) & 1 ) ;
V_4 [ 1 ] |= ( V_4 [ 0 ] & 0x40 ) << 1 ;
}
if ( V_1 -> type == V_16 ) {
F_3 ( V_3 , V_13 , ( V_4 [ 0 ] >> 3 ) & 1 ) ;
V_4 [ 0 ] |= 0x08 ;
}
F_3 ( V_3 , V_17 , V_4 [ 0 ] & 1 ) ;
F_3 ( V_3 , V_18 , ( V_4 [ 0 ] >> 2 ) & 1 ) ;
F_3 ( V_3 , V_19 , ( V_4 [ 0 ] >> 1 ) & 1 ) ;
F_2 ( V_3 , V_20 , V_4 [ 1 ] ? ( int ) V_4 [ 1 ] - ( int ) ( ( V_4 [ 0 ] << 4 ) & 0x100 ) : 0 ) ;
F_2 ( V_3 , V_21 , V_4 [ 2 ] ? ( int ) ( ( V_4 [ 0 ] << 3 ) & 0x100 ) - ( int ) V_4 [ 2 ] : 0 ) ;
F_4 ( V_3 ) ;
return V_22 ;
}
void F_5 ( struct V_1 * V_1 , struct V_23 * V_24 ,
unsigned long V_25 )
{
F_6 ( V_26 , V_24 , V_25 ) ;
}
static inline void F_7 ( struct V_1 * V_1 , enum V_27 V_28 )
{
V_1 -> V_29 = V_28 ;
V_1 -> V_5 = V_1 -> V_30 = 0 ;
V_1 -> V_31 . V_32 = 0 ;
V_1 -> V_33 = V_34 ;
}
void F_8 ( struct V_1 * V_1 , enum V_27 V_28 )
{
F_9 ( V_1 -> V_31 . V_35 ) ;
F_7 ( V_1 , V_28 ) ;
F_10 ( V_1 -> V_31 . V_35 ) ;
}
static int F_11 ( struct V_1 * V_1 )
{
T_1 V_36 = V_1 -> V_37 ( V_1 ) ;
switch ( V_36 ) {
case V_38 :
if ( V_1 -> V_29 == V_39 ) {
F_12 ( V_1 ,
L_1 ,
V_1 -> V_40 , V_1 -> V_41 ,
V_1 -> V_5 ) ;
if ( ++ V_1 -> V_30 == V_1 -> V_42 ) {
F_7 ( V_1 , V_43 ) ;
F_13 ( V_1 ,
L_2 ) ;
F_14 ( V_1 -> V_31 . V_35 ) ;
return - 1 ;
}
}
V_1 -> V_5 = 0 ;
break;
case V_22 :
V_1 -> V_5 = 0 ;
if ( V_1 -> V_30 ) {
V_1 -> V_30 = 0 ;
F_13 ( V_1 ,
L_3 ,
V_1 -> V_40 , V_1 -> V_41 ) ;
}
break;
case V_7 :
break;
}
return 0 ;
}
static T_2 F_15 ( struct V_35 * V_35 ,
unsigned char V_44 , unsigned int V_32 )
{
struct V_1 * V_1 = F_16 ( V_35 ) ;
if ( V_1 -> V_29 == V_43 )
goto V_45;
if ( F_17 ( ( V_32 & V_46 ) ||
( ( V_32 & V_47 ) && ! V_1 -> V_48 ) ) ) {
if ( V_1 -> V_29 == V_39 )
F_12 ( V_1 ,
L_4 ,
V_32 & V_46 ? L_5 : L_6 ,
V_32 & V_47 ? L_7 : L_6 ) ;
F_18 ( & V_1 -> V_31 ) ;
goto V_45;
}
if ( F_17 ( V_1 -> V_31 . V_32 & V_49 ) )
if ( F_19 ( & V_1 -> V_31 , V_44 ) )
goto V_45;
if ( F_17 ( V_1 -> V_31 . V_32 & V_50 ) )
if ( F_20 ( & V_1 -> V_31 , V_44 ) )
goto V_45;
if ( V_1 -> V_29 <= V_51 )
goto V_45;
if ( V_1 -> V_29 == V_39 &&
V_1 -> V_5 && F_21 ( V_34 , V_1 -> V_33 + V_52 / 2 ) ) {
F_22 ( V_1 , L_8 ,
V_1 -> V_40 , V_1 -> V_41 , V_1 -> V_5 ) ;
V_1 -> V_53 = V_1 -> V_4 [ 0 ] ;
F_7 ( V_1 , V_51 ) ;
F_5 ( V_1 , & V_1 -> V_54 , 0 ) ;
goto V_45;
}
V_1 -> V_4 [ V_1 -> V_5 ++ ] = V_44 ;
if ( F_17 ( V_1 -> V_4 [ 0 ] == V_55 && V_1 -> V_5 <= 2 ) ) {
if ( V_1 -> V_5 == 1 ) {
V_1 -> V_33 = V_34 ;
goto V_45;
}
if ( V_1 -> V_4 [ 1 ] == V_56 ||
( V_1 -> type == V_57 &&
V_1 -> V_4 [ 1 ] == V_55 ) ) {
F_7 ( V_1 , V_43 ) ;
F_14 ( V_35 ) ;
goto V_45;
}
V_1 -> V_5 = 1 ;
if ( F_11 ( V_1 ) )
goto V_45;
V_1 -> V_4 [ V_1 -> V_5 ++ ] = V_44 ;
}
if ( V_1 -> V_29 == V_39 &&
V_1 -> V_5 == 1 && V_1 -> V_58 &&
F_21 ( V_34 , V_1 -> V_33 + V_1 -> V_58 * V_52 ) ) {
V_1 -> V_53 = V_1 -> V_4 [ 0 ] ;
F_7 ( V_1 , V_51 ) ;
F_5 ( V_1 , & V_1 -> V_54 , 0 ) ;
goto V_45;
}
V_1 -> V_33 = V_34 ;
F_11 ( V_1 ) ;
V_45:
return V_59 ;
}
int F_23 ( struct V_1 * V_1 , unsigned char V_60 )
{
int V_61 ;
if ( F_24 ( & V_1 -> V_31 , NULL , V_62 ) )
return - 1 ;
for ( V_61 = 6 ; V_61 >= 0 ; V_61 -= 2 ) {
unsigned char V_63 = ( V_60 >> V_61 ) & 3 ;
if ( F_24 ( & V_1 -> V_31 , & V_63 , V_64 ) )
return - 1 ;
}
return 0 ;
}
int F_25 ( struct V_1 * V_1 )
{
unsigned char V_65 [ 2 ] ;
if ( F_24 ( & V_1 -> V_31 , V_65 , V_66 ) )
return - 1 ;
if ( V_65 [ 0 ] != V_55 && V_65 [ 1 ] != V_56 )
return - 1 ;
return 0 ;
}
void F_26 ( struct V_1 * V_1 , unsigned int V_67 )
{
static const unsigned char V_68 [] = { 0 , 1 , 2 , 2 , 3 } ;
unsigned char V_69 ;
if ( V_67 == 0 || V_67 > 200 )
V_67 = 200 ;
V_69 = V_68 [ V_67 / 50 ] ;
F_24 ( & V_1 -> V_31 , & V_69 , V_64 ) ;
V_1 -> V_67 = 25 << V_69 ;
}
static void F_27 ( struct V_1 * V_1 , unsigned int V_70 )
{
static const unsigned char V_71 [] = { 200 , 100 , 80 , 60 , 40 , 20 , 10 , 0 } ;
unsigned char V_72 ;
int V_61 = 0 ;
while ( V_71 [ V_61 ] > V_70 ) V_61 ++ ;
V_72 = V_71 [ V_61 ] ;
F_24 ( & V_1 -> V_31 , & V_72 , V_73 ) ;
V_1 -> V_70 = V_72 ;
}
static int F_28 ( struct V_1 * V_1 )
{
return F_24 ( & V_1 -> V_31 , V_1 -> V_4 ,
V_74 | ( V_1 -> V_6 << 8 ) ) ;
}
static int F_29 ( struct V_1 * V_1 , bool V_75 )
{
struct V_31 * V_31 = & V_1 -> V_31 ;
unsigned char V_65 [ 4 ] ;
V_65 [ 0 ] = 3 ;
F_24 ( V_31 , V_65 , V_64 ) ;
F_24 ( V_31 , NULL , V_62 ) ;
F_24 ( V_31 , NULL , V_62 ) ;
F_24 ( V_31 , NULL , V_62 ) ;
F_24 ( V_31 , V_65 , V_76 ) ;
if ( V_65 [ 0 ] != 0x00 || V_65 [ 1 ] != 0x33 || V_65 [ 2 ] != 0x55 )
return - 1 ;
if ( V_75 ) {
F_30 ( V_18 , V_1 -> V_3 -> V_77 ) ;
F_30 ( V_14 , V_1 -> V_3 -> V_77 ) ;
F_30 ( V_13 , V_1 -> V_3 -> V_77 ) ;
F_30 ( V_10 , V_1 -> V_3 -> V_78 ) ;
V_1 -> V_79 = L_9 ;
V_1 -> V_40 = L_10 ;
V_1 -> V_6 = 4 ;
}
return 0 ;
}
static int F_31 ( struct V_1 * V_1 , bool V_75 )
{
struct V_31 * V_31 = & V_1 -> V_31 ;
unsigned char V_65 [ 2 ] ;
V_65 [ 0 ] = 200 ;
F_24 ( V_31 , V_65 , V_73 ) ;
V_65 [ 0 ] = 100 ;
F_24 ( V_31 , V_65 , V_73 ) ;
V_65 [ 0 ] = 80 ;
F_24 ( V_31 , V_65 , V_73 ) ;
F_24 ( V_31 , V_65 , V_80 ) ;
if ( V_65 [ 0 ] != 3 )
return - 1 ;
if ( V_75 ) {
F_30 ( V_18 , V_1 -> V_3 -> V_77 ) ;
F_30 ( V_10 , V_1 -> V_3 -> V_78 ) ;
if ( ! V_1 -> V_79 )
V_1 -> V_79 = L_11 ;
if ( ! V_1 -> V_40 )
V_1 -> V_40 = L_12 ;
V_1 -> V_6 = 4 ;
}
return 0 ;
}
static int F_32 ( struct V_1 * V_1 , bool V_75 )
{
struct V_31 * V_31 = & V_1 -> V_31 ;
unsigned char V_65 [ 2 ] ;
F_31 ( V_1 , 0 ) ;
V_65 [ 0 ] = 200 ;
F_24 ( V_31 , V_65 , V_73 ) ;
V_65 [ 0 ] = 200 ;
F_24 ( V_31 , V_65 , V_73 ) ;
V_65 [ 0 ] = 80 ;
F_24 ( V_31 , V_65 , V_73 ) ;
F_24 ( V_31 , V_65 , V_80 ) ;
if ( V_65 [ 0 ] != 4 )
return - 1 ;
V_65 [ 0 ] = 200 ;
F_24 ( V_31 , V_65 , V_73 ) ;
V_65 [ 0 ] = 80 ;
F_24 ( V_31 , V_65 , V_73 ) ;
V_65 [ 0 ] = 40 ;
F_24 ( V_31 , V_65 , V_73 ) ;
if ( V_75 ) {
F_30 ( V_18 , V_1 -> V_3 -> V_77 ) ;
F_30 ( V_10 , V_1 -> V_3 -> V_78 ) ;
F_30 ( V_12 , V_1 -> V_3 -> V_78 ) ;
F_30 ( V_13 , V_1 -> V_3 -> V_77 ) ;
F_30 ( V_14 , V_1 -> V_3 -> V_77 ) ;
if ( ! V_1 -> V_79 )
V_1 -> V_79 = L_11 ;
if ( ! V_1 -> V_40 )
V_1 -> V_40 = L_13 ;
V_1 -> V_6 = 4 ;
}
return 0 ;
}
static int F_33 ( struct V_1 * V_1 , bool V_75 )
{
struct V_31 * V_31 = & V_1 -> V_31 ;
unsigned char V_65 [ 2 ] ;
static const unsigned char V_81 [] = { 20 , 60 , 40 , 20 , 20 , 60 , 40 , 20 , 20 } ;
int V_61 ;
V_65 [ 0 ] = 10 ;
F_24 ( V_31 , V_65 , V_73 ) ;
V_65 [ 0 ] = 0 ;
F_24 ( V_31 , V_65 , V_64 ) ;
for ( V_61 = 0 ; V_61 < F_34 ( V_81 ) ; V_61 ++ ) {
V_65 [ 0 ] = V_81 [ V_61 ] ;
F_24 ( V_31 , V_65 , V_73 ) ;
}
F_24 ( V_31 , V_65 , V_80 ) ;
if ( V_65 [ 0 ] != 2 )
return - 1 ;
if ( V_75 ) {
F_30 ( V_18 , V_1 -> V_3 -> V_77 ) ;
F_30 ( V_14 , V_1 -> V_3 -> V_77 ) ;
V_1 -> V_79 = L_14 ;
V_1 -> V_40 = L_15 ;
}
return 0 ;
}
static int F_35 ( struct V_1 * V_1 , bool V_75 )
{
if ( V_75 ) {
if ( ! V_1 -> V_79 )
V_1 -> V_79 = L_11 ;
if ( ! V_1 -> V_40 )
V_1 -> V_40 = L_10 ;
F_30 ( V_18 , V_1 -> V_3 -> V_77 ) ;
}
return 0 ;
}
static int F_36 ( struct V_1 * V_1 , bool V_75 )
{
if ( V_75 ) {
V_1 -> V_79 = L_16 ;
V_1 -> V_40 = L_17 ;
F_30 ( V_18 , V_1 -> V_3 -> V_77 ) ;
F_30 ( V_13 , V_1 -> V_3 -> V_77 ) ;
}
return 0 ;
}
static void F_37 ( struct V_1 * V_1 )
{
struct V_2 * V_2 = V_1 -> V_3 ;
memset ( V_2 -> V_82 , 0 , sizeof( V_2 -> V_82 ) ) ;
memset ( V_2 -> V_77 , 0 , sizeof( V_2 -> V_77 ) ) ;
memset ( V_2 -> V_78 , 0 , sizeof( V_2 -> V_78 ) ) ;
memset ( V_2 -> V_83 , 0 , sizeof( V_2 -> V_83 ) ) ;
memset ( V_2 -> V_84 , 0 , sizeof( V_2 -> V_84 ) ) ;
F_30 ( V_85 , V_2 -> V_82 ) ;
F_30 ( V_86 , V_2 -> V_82 ) ;
F_30 ( V_17 , V_2 -> V_77 ) ;
F_30 ( V_19 , V_2 -> V_77 ) ;
F_30 ( V_20 , V_2 -> V_78 ) ;
F_30 ( V_21 , V_2 -> V_78 ) ;
F_30 ( V_87 , V_2 -> V_88 ) ;
V_1 -> V_89 = F_27 ;
V_1 -> V_90 = F_26 ;
V_1 -> V_91 = F_28 ;
V_1 -> V_37 = F_1 ;
V_1 -> V_6 = 3 ;
V_1 -> V_92 = NULL ;
V_1 -> V_93 = NULL ;
V_1 -> V_94 = NULL ;
V_1 -> V_95 = NULL ;
V_1 -> V_96 = NULL ;
}
static int F_38 ( int (* F_39)( struct V_1 * V_1 ,
bool V_75 ) ,
struct V_1 * V_1 , bool V_75 )
{
if ( V_75 )
F_37 ( V_1 ) ;
return F_39 ( V_1 , V_75 ) ;
}
static int F_40 ( struct V_1 * V_1 ,
unsigned int V_97 , bool V_75 )
{
bool V_98 = false ;
if ( F_38 ( V_99 , V_1 , V_75 ) == 0 ) {
if ( V_97 > V_11 ) {
if ( ! V_75 || F_41 ( V_1 ) == 0 )
return V_100 ;
}
}
if ( V_97 > V_11 &&
F_38 ( F_33 , V_1 , V_75 ) == 0 ) {
return V_15 ;
}
if ( V_97 > V_101 &&
F_38 ( V_102 , V_1 , V_75 ) == 0 ) {
V_98 = true ;
if ( V_97 > V_11 ) {
if ( F_42 () &&
( ! V_75 || F_43 ( V_1 ) == 0 ) ) {
return V_103 ;
}
V_97 = V_11 ;
}
F_44 ( V_1 ) ;
}
if ( V_97 > V_11 &&
F_45 ( V_1 , V_75 ) == 0 ) {
if ( F_46 () ) {
if ( F_47 ( V_1 ) == 0 )
return V_104 ;
return V_101 ;
}
V_97 = V_11 ;
}
if ( V_97 > V_11 ) {
F_24 ( & V_1 -> V_31 , NULL , V_105 ) ;
if ( F_38 ( V_106 ,
V_1 , V_75 ) == 0 ) {
if ( ! V_75 || F_48 ( V_1 ) == 0 )
return V_107 ;
V_97 = V_11 ;
}
}
if ( V_97 > V_11 &&
F_38 ( V_108 , V_1 , V_75 ) == 0 ) {
if ( ! V_75 || F_49 ( V_1 ) == 0 )
return V_57 ;
V_97 = V_11 ;
}
if ( V_97 > V_11 &&
F_38 ( V_109 , V_1 , V_75 ) == 0 ) {
if ( ! V_75 || F_50 ( V_1 ) == 0 )
return V_110 ;
V_97 = V_11 ;
}
if ( V_97 > V_11 ) {
if ( F_38 ( F_29 ,
V_1 , V_75 ) == 0 )
return V_9 ;
if ( F_38 ( V_111 ,
V_1 , V_75 ) == 0 )
return V_112 ;
if ( F_38 ( V_113 ,
V_1 , V_75 ) == 0 )
return V_114 ;
if ( F_38 ( V_115 ,
V_1 , V_75 ) == 0 )
return V_116 ;
}
if ( V_97 > V_11 ) {
if ( F_38 ( V_117 ,
V_1 , V_75 ) == 0 ) {
if ( ! V_75 || F_51 ( V_1 ) == 0 )
return V_118 ;
V_97 = V_11 ;
}
}
F_24 ( & V_1 -> V_31 , NULL , V_105 ) ;
F_25 ( V_1 ) ;
if ( V_97 >= V_11 &&
F_38 ( F_32 ,
V_1 , V_75 ) == 0 ) {
return V_11 ;
}
if ( V_97 >= V_8 &&
F_38 ( F_31 ,
V_1 , V_75 ) == 0 ) {
return V_8 ;
}
F_38 ( F_35 , V_1 , V_75 ) ;
if ( V_98 ) {
F_25 ( V_1 ) ;
}
return V_101 ;
}
static const struct V_119 * F_52 ( enum V_120 type )
{
int V_61 ;
for ( V_61 = 0 ; V_61 < F_34 ( V_121 ) ; V_61 ++ )
if ( V_121 [ V_61 ] . type == type )
return & V_121 [ V_61 ] ;
F_53 ( 1 ) ;
return & V_121 [ 0 ] ;
}
static const struct V_119 * F_54 ( const char * V_40 , T_3 V_122 )
{
const struct V_119 * V_69 ;
int V_61 ;
for ( V_61 = 0 ; V_61 < F_34 ( V_121 ) ; V_61 ++ ) {
V_69 = & V_121 [ V_61 ] ;
if ( ( strlen ( V_69 -> V_40 ) == V_122 && ! strncmp ( V_69 -> V_40 , V_40 , V_122 ) ) ||
( strlen ( V_69 -> V_123 ) == V_122 && ! strncmp ( V_69 -> V_123 , V_40 , V_122 ) ) )
return & V_121 [ V_61 ] ;
}
return NULL ;
}
static int F_55 ( struct V_1 * V_1 )
{
struct V_31 * V_31 = & V_1 -> V_31 ;
unsigned char V_65 [ 2 ] ;
V_65 [ 0 ] = 0xa5 ;
if ( F_24 ( V_31 , V_65 , V_80 ) )
return - 1 ;
if ( V_65 [ 0 ] != 0x00 && V_65 [ 0 ] != 0x03 &&
V_65 [ 0 ] != 0x04 && V_65 [ 0 ] != 0xff )
return - 1 ;
if ( F_24 ( V_31 , NULL , V_105 ) )
F_12 ( V_1 , L_18 ,
V_31 -> V_35 -> V_41 ) ;
return 0 ;
}
static void F_56 ( struct V_1 * V_1 )
{
if ( V_124 != V_101 ) {
V_1 -> V_89 ( V_1 , V_1 -> V_70 ) ;
V_1 -> V_90 ( V_1 , V_1 -> V_67 ) ;
F_24 ( & V_1 -> V_31 , NULL , V_62 ) ;
}
}
int F_57 ( struct V_1 * V_1 )
{
if ( F_24 ( & V_1 -> V_31 , NULL , V_125 ) ) {
F_12 ( V_1 , L_19 ,
V_1 -> V_31 . V_35 -> V_41 ) ;
return - 1 ;
}
F_8 ( V_1 , V_39 ) ;
return 0 ;
}
int F_58 ( struct V_1 * V_1 )
{
if ( F_24 ( & V_1 -> V_31 , NULL , V_126 ) ) {
F_12 ( V_1 , L_20 ,
V_1 -> V_31 . V_35 -> V_41 ) ;
return - 1 ;
}
F_8 ( V_1 , V_127 ) ;
return 0 ;
}
static void F_59 ( struct V_128 * V_24 )
{
struct V_1 * V_129 = NULL , * V_1 =
F_60 ( V_24 , struct V_1 , V_54 . V_24 ) ;
struct V_35 * V_35 = V_1 -> V_31 . V_35 ;
T_1 V_36 = V_7 ;
bool V_130 = false , V_131 = false ;
int V_61 ;
F_61 ( & V_132 ) ;
if ( V_1 -> V_29 != V_51 )
goto V_45;
if ( V_35 -> V_129 && V_35 -> V_133 . type == V_134 ) {
V_129 = F_16 ( V_35 -> V_129 ) ;
F_58 ( V_129 ) ;
}
V_1 -> V_135 ++ ;
if ( F_62 ( & V_1 -> V_31 , V_126 , 20 ) ) {
if ( V_1 -> V_135 < 3 || V_1 -> V_136 )
V_130 = true ;
} else
V_1 -> V_136 = true ;
if ( ! V_130 ) {
if ( V_1 -> V_91 ( V_1 ) )
V_130 = true ;
else {
F_8 ( V_1 , V_127 ) ;
for ( V_61 = 0 ; V_61 < V_1 -> V_6 ; V_61 ++ ) {
V_1 -> V_5 ++ ;
V_36 = V_1 -> V_37 ( V_1 ) ;
if ( V_36 != V_7 )
break;
}
if ( V_36 != V_22 )
V_130 = true ;
F_8 ( V_1 , V_51 ) ;
}
}
for ( V_61 = 0 ; V_61 < 5 ; V_61 ++ ) {
if ( ! F_24 ( & V_1 -> V_31 , NULL , V_125 ) ) {
V_131 = true ;
break;
}
F_63 ( 200 ) ;
}
if ( ! V_131 ) {
F_12 ( V_1 , L_21 ,
V_1 -> V_31 . V_35 -> V_41 ) ;
V_130 = true ;
}
if ( V_130 ) {
F_8 ( V_1 , V_43 ) ;
F_22 ( V_1 ,
L_22 ) ;
F_14 ( V_35 ) ;
} else
F_8 ( V_1 , V_39 ) ;
if ( V_129 )
F_57 ( V_129 ) ;
V_45:
F_64 ( & V_132 ) ;
}
static void F_65 ( struct V_35 * V_35 )
{
struct V_1 * V_1 = F_16 ( V_35 ) ;
struct V_1 * V_129 = NULL ;
F_61 ( & V_132 ) ;
if ( V_35 -> V_129 && V_35 -> V_133 . type == V_134 ) {
V_129 = F_16 ( V_35 -> V_129 ) ;
F_58 ( V_129 ) ;
}
F_8 ( V_1 , V_137 ) ;
if ( F_24 ( & V_1 -> V_31 , NULL , V_126 ) )
F_12 ( V_1 , L_23 ,
V_1 -> V_31 . V_35 -> V_41 ) ;
if ( V_1 -> V_94 )
V_1 -> V_94 ( V_1 ) ;
F_24 ( & V_1 -> V_31 , NULL , V_105 ) ;
F_24 ( & V_1 -> V_31 , NULL , V_125 ) ;
if ( V_129 ) {
if ( V_129 -> V_96 )
V_129 -> V_96 ( V_129 ) ;
F_57 ( V_129 ) ;
}
F_64 ( & V_132 ) ;
}
static void F_66 ( struct V_35 * V_35 )
{
struct V_1 * V_1 , * V_129 = NULL ;
V_1 = F_16 ( V_35 ) ;
F_67 ( & V_35 -> V_3 . V_138 , & V_139 ) ;
F_61 ( & V_132 ) ;
F_8 ( V_1 , V_127 ) ;
F_64 ( & V_132 ) ;
F_68 ( V_26 ) ;
F_61 ( & V_132 ) ;
if ( V_35 -> V_129 && V_35 -> V_133 . type == V_134 ) {
V_129 = F_16 ( V_35 -> V_129 ) ;
F_58 ( V_129 ) ;
}
if ( V_1 -> V_93 )
V_1 -> V_93 ( V_1 ) ;
if ( V_129 && V_129 -> V_96 )
V_129 -> V_96 ( V_129 ) ;
F_8 ( V_1 , V_43 ) ;
F_69 ( V_35 ) ;
F_70 ( V_35 , NULL ) ;
F_71 ( V_1 -> V_3 ) ;
F_72 ( V_1 ) ;
if ( V_129 )
F_57 ( V_129 ) ;
F_64 ( & V_132 ) ;
}
static int F_73 ( struct V_1 * V_1 ,
const struct V_119 * V_140 )
{
const struct V_119 * V_141 ;
struct V_2 * V_2 = V_1 -> V_3 ;
V_2 -> V_3 . V_129 = & V_1 -> V_31 . V_35 -> V_3 ;
if ( V_140 && ( V_140 -> F_39 || V_140 -> V_142 ) ) {
F_37 ( V_1 ) ;
if ( V_140 -> F_39 && V_140 -> F_39 ( V_1 , true ) < 0 )
return - 1 ;
if ( V_140 -> V_142 && V_140 -> V_142 ( V_1 ) < 0 )
return - 1 ;
V_1 -> type = V_140 -> type ;
V_141 = V_140 ;
} else {
V_1 -> type = F_40 ( V_1 ,
V_124 , true ) ;
V_141 = F_52 ( V_1 -> type ) ;
}
V_1 -> V_48 = V_141 -> V_48 ;
if ( V_1 -> V_6 == 3 )
V_1 -> V_58 = 0 ;
if ( V_1 -> V_58 && V_1 -> V_91 ( V_1 ) )
V_1 -> V_58 = 0 ;
snprintf ( V_1 -> V_143 , sizeof( V_1 -> V_143 ) , L_24 ,
V_141 -> V_40 , V_1 -> V_79 , V_1 -> V_40 ) ;
V_2 -> V_40 = V_1 -> V_143 ;
V_2 -> V_41 = V_1 -> V_41 ;
V_2 -> V_133 . V_144 = V_145 ;
V_2 -> V_133 . V_79 = 0x0002 ;
V_2 -> V_133 . V_146 = V_1 -> type ;
V_2 -> V_133 . V_147 = V_1 -> V_148 ;
return 0 ;
}
static int F_74 ( struct V_35 * V_35 , struct V_149 * V_150 )
{
struct V_1 * V_1 , * V_129 = NULL ;
struct V_2 * V_2 ;
int V_151 = 0 , error = - V_152 ;
F_61 ( & V_132 ) ;
if ( V_35 -> V_129 && V_35 -> V_133 . type == V_134 ) {
V_129 = F_16 ( V_35 -> V_129 ) ;
F_58 ( V_129 ) ;
}
V_1 = F_75 ( sizeof( struct V_1 ) , V_153 ) ;
V_2 = F_76 () ;
if ( ! V_1 || ! V_2 )
goto V_154;
F_77 ( & V_1 -> V_31 , V_35 ) ;
F_78 ( & V_1 -> V_54 , F_59 ) ;
V_1 -> V_3 = V_2 ;
snprintf ( V_1 -> V_41 , sizeof( V_1 -> V_41 ) , L_25 , V_35 -> V_41 ) ;
F_8 ( V_1 , V_137 ) ;
F_70 ( V_35 , V_1 ) ;
error = F_79 ( V_35 , V_150 ) ;
if ( error )
goto V_155;
if ( F_55 ( V_1 ) < 0 ) {
error = - V_156 ;
goto V_157;
}
V_1 -> V_70 = V_158 ;
V_1 -> V_67 = V_159 ;
V_1 -> V_42 = V_160 ;
V_1 -> V_58 = V_129 ? 0 : V_161 ;
V_1 -> V_162 = V_163 ;
F_73 ( V_1 , NULL ) ;
F_8 ( V_1 , V_127 ) ;
F_56 ( V_1 ) ;
error = F_80 ( V_1 -> V_3 ) ;
if ( error )
goto V_164;
if ( V_129 && V_129 -> V_95 )
V_129 -> V_95 ( V_129 ) ;
error = F_81 ( & V_35 -> V_3 . V_138 , & V_139 ) ;
if ( error )
goto V_165;
F_57 ( V_1 ) ;
V_45:
if ( V_129 )
F_57 ( V_129 ) ;
F_64 ( & V_132 ) ;
return V_151 ;
V_165:
if ( V_129 && V_129 -> V_96 )
V_129 -> V_96 ( V_129 ) ;
F_71 ( V_1 -> V_3 ) ;
V_2 = NULL ;
V_164:
if ( V_1 -> V_93 )
V_1 -> V_93 ( V_1 ) ;
F_8 ( V_1 , V_43 ) ;
V_157:
F_69 ( V_35 ) ;
V_155:
F_70 ( V_35 , NULL ) ;
V_154:
F_82 ( V_2 ) ;
F_72 ( V_1 ) ;
V_151 = error ;
goto V_45;
}
static int F_83 ( struct V_35 * V_35 )
{
struct V_1 * V_1 = F_16 ( V_35 ) ;
struct V_1 * V_129 = NULL ;
struct V_149 * V_150 = V_35 -> V_150 ;
unsigned char type ;
int V_36 = - 1 ;
if ( ! V_150 || ! V_1 ) {
F_84 ( V_1 ,
L_26 ) ;
return - 1 ;
}
F_61 ( & V_132 ) ;
if ( V_35 -> V_129 && V_35 -> V_133 . type == V_134 ) {
V_129 = F_16 ( V_35 -> V_129 ) ;
F_58 ( V_129 ) ;
}
F_8 ( V_1 , V_137 ) ;
if ( V_1 -> V_92 ) {
if ( V_1 -> V_92 ( V_1 ) )
goto V_45;
} else {
F_25 ( V_1 ) ;
if ( F_55 ( V_1 ) < 0 )
goto V_45;
type = F_40 ( V_1 , V_124 , false ) ;
if ( V_1 -> type != type )
goto V_45;
}
F_8 ( V_1 , V_127 ) ;
F_56 ( V_1 ) ;
if ( V_129 && V_129 -> V_95 )
V_129 -> V_95 ( V_129 ) ;
F_57 ( V_1 ) ;
V_36 = 0 ;
V_45:
if ( V_129 )
F_57 ( V_129 ) ;
F_64 ( & V_132 ) ;
return V_36 ;
}
T_4 F_85 ( struct V_166 * V_3 , struct V_167 * V_168 ,
char * V_169 )
{
struct V_35 * V_35 = F_86 ( V_3 ) ;
struct V_170 * V_171 = F_87 ( V_168 ) ;
struct V_1 * V_1 ;
V_1 = F_16 ( V_35 ) ;
return V_171 -> V_172 ( V_1 , V_171 -> V_44 , V_169 ) ;
}
T_4 F_88 ( struct V_166 * V_3 , struct V_167 * V_168 ,
const char * V_169 , T_3 V_173 )
{
struct V_35 * V_35 = F_86 ( V_3 ) ;
struct V_170 * V_171 = F_87 ( V_168 ) ;
struct V_1 * V_1 , * V_129 = NULL ;
int V_151 ;
V_151 = F_89 ( & V_132 ) ;
if ( V_151 )
goto V_45;
V_1 = F_16 ( V_35 ) ;
if ( V_171 -> V_174 ) {
if ( V_1 -> V_29 == V_43 ) {
V_151 = - V_156 ;
goto V_175;
}
if ( V_35 -> V_129 && V_35 -> V_133 . type == V_134 ) {
V_129 = F_16 ( V_35 -> V_129 ) ;
F_58 ( V_129 ) ;
}
F_58 ( V_1 ) ;
}
V_151 = V_171 -> V_176 ( V_1 , V_171 -> V_44 , V_169 , V_173 ) ;
if ( V_171 -> V_174 ) {
if ( V_151 != - V_156 )
F_57 ( V_1 ) ;
if ( V_129 )
F_57 ( V_129 ) ;
}
V_175:
F_64 ( & V_132 ) ;
V_45:
return V_151 ;
}
static T_4 F_90 ( struct V_1 * V_1 , void * V_177 , char * V_169 )
{
unsigned int * V_178 = ( unsigned int * ) ( ( char * ) V_1 + ( T_3 ) V_177 ) ;
return sprintf ( V_169 , L_27 , * V_178 ) ;
}
static T_4 F_91 ( struct V_1 * V_1 , void * V_177 , const char * V_169 , T_3 V_173 )
{
unsigned int * V_178 = ( unsigned int * ) ( ( char * ) V_1 + ( T_3 ) V_177 ) ;
unsigned int V_179 ;
int V_180 ;
V_180 = F_92 ( V_169 , 10 , & V_179 ) ;
if ( V_180 )
return V_180 ;
* V_178 = V_179 ;
return V_173 ;
}
static T_4 F_93 ( struct V_1 * V_1 , void * V_44 , char * V_169 )
{
return sprintf ( V_169 , L_28 , F_52 ( V_1 -> type ) -> V_40 ) ;
}
static T_4 F_94 ( struct V_1 * V_1 , void * V_44 , const char * V_169 , T_3 V_173 )
{
struct V_35 * V_35 = V_1 -> V_31 . V_35 ;
struct V_1 * V_129 = NULL ;
struct V_2 * V_181 , * V_182 ;
const struct V_119 * V_140 , * V_183 ;
int error ;
int V_184 = 0 ;
V_140 = F_54 ( V_169 , V_173 ) ;
if ( ! V_140 )
return - V_185 ;
if ( V_1 -> type == V_140 -> type )
return V_173 ;
V_182 = F_76 () ;
if ( ! V_182 )
return - V_152 ;
while ( ! F_95 ( & V_35 -> V_186 ) ) {
if ( ++ V_184 > 3 ) {
F_12 ( V_1 ,
L_29 ) ;
F_82 ( V_182 ) ;
return - V_187 ;
}
F_64 ( & V_132 ) ;
F_96 ( V_35 ) ;
F_61 ( & V_132 ) ;
if ( V_35 -> V_150 != & V_188 ) {
F_82 ( V_182 ) ;
return - V_156 ;
}
if ( V_1 -> type == V_140 -> type ) {
F_82 ( V_182 ) ;
return V_173 ;
}
}
if ( V_35 -> V_129 && V_35 -> V_133 . type == V_134 ) {
V_129 = F_16 ( V_35 -> V_129 ) ;
if ( V_129 -> V_96 )
V_129 -> V_96 ( V_129 ) ;
}
V_181 = V_1 -> V_3 ;
V_183 = F_52 ( V_1 -> type ) ;
if ( V_1 -> V_93 )
V_1 -> V_93 ( V_1 ) ;
F_8 ( V_1 , V_43 ) ;
V_1 -> V_3 = V_182 ;
F_8 ( V_1 , V_137 ) ;
if ( F_73 ( V_1 , V_140 ) < 0 ) {
F_25 ( V_1 ) ;
F_73 ( V_1 , & V_121 [ 0 ] ) ;
}
F_56 ( V_1 ) ;
F_8 ( V_1 , V_127 ) ;
error = F_80 ( V_1 -> V_3 ) ;
if ( error ) {
if ( V_1 -> V_93 )
V_1 -> V_93 ( V_1 ) ;
F_8 ( V_1 , V_43 ) ;
F_82 ( V_182 ) ;
V_1 -> V_3 = V_181 ;
F_8 ( V_1 , V_137 ) ;
F_73 ( V_1 , V_183 ) ;
F_56 ( V_1 ) ;
F_8 ( V_1 , V_127 ) ;
return error ;
}
F_71 ( V_181 ) ;
if ( V_129 && V_129 -> V_95 )
V_129 -> V_95 ( V_129 ) ;
return V_173 ;
}
static T_4 F_97 ( struct V_1 * V_1 , void * V_44 , const char * V_169 , T_3 V_173 )
{
unsigned int V_179 ;
int V_180 ;
V_180 = F_92 ( V_169 , 10 , & V_179 ) ;
if ( V_180 )
return V_180 ;
V_1 -> V_89 ( V_1 , V_179 ) ;
return V_173 ;
}
static T_4 F_98 ( struct V_1 * V_1 , void * V_44 , const char * V_169 , T_3 V_173 )
{
unsigned int V_179 ;
int V_180 ;
V_180 = F_92 ( V_169 , 10 , & V_179 ) ;
if ( V_180 )
return V_180 ;
V_1 -> V_90 ( V_1 , V_179 ) ;
return V_173 ;
}
static int F_99 ( const char * V_189 , const struct V_190 * V_191 )
{
const struct V_119 * V_140 ;
if ( ! V_189 )
return - V_185 ;
V_140 = F_54 ( V_189 , strlen ( V_189 ) ) ;
if ( ! V_140 || ! V_140 -> V_192 )
return - V_185 ;
* ( ( unsigned int * ) V_191 -> V_193 ) = V_140 -> type ;
return 0 ;
}
static int F_100 ( char * V_194 , const struct V_190 * V_191 )
{
int type = * ( ( unsigned int * ) V_191 -> V_193 ) ;
return sprintf ( V_194 , L_30 , F_52 ( type ) -> V_40 ) ;
}
static int T_5 F_101 ( void )
{
int V_180 ;
F_102 () ;
F_103 () ;
F_104 () ;
V_26 = F_105 ( L_31 ) ;
if ( ! V_26 ) {
F_106 ( L_32 ) ;
return - V_152 ;
}
V_180 = F_107 ( & V_188 ) ;
if ( V_180 )
F_108 ( V_26 ) ;
return V_180 ;
}
static void T_6 F_109 ( void )
{
F_110 ( & V_188 ) ;
F_108 ( V_26 ) ;
}
