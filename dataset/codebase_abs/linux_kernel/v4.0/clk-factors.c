static unsigned long F_1 ( struct V_1 * V_2 ,
unsigned long V_3 )
{
T_1 V_4 = 1 , V_5 = 0 , V_6 = 0 , V_7 = 0 ;
T_2 V_8 ;
unsigned long V_9 ;
struct V_10 * V_11 = F_2 ( V_2 ) ;
struct V_12 * V_13 = V_11 -> V_13 ;
V_8 = F_3 ( V_11 -> V_8 ) ;
if ( V_13 -> V_14 != V_15 )
V_4 = F_4 ( V_13 -> V_16 , V_13 -> V_14 , V_8 ) ;
if ( V_13 -> V_17 != V_15 )
V_5 = F_4 ( V_13 -> V_18 , V_13 -> V_17 , V_8 ) ;
if ( V_13 -> V_19 != V_15 )
V_7 = F_4 ( V_13 -> V_20 , V_13 -> V_19 , V_8 ) ;
if ( V_13 -> V_21 != V_15 )
V_6 = F_4 ( V_13 -> V_22 , V_13 -> V_21 , V_8 ) ;
V_9 = ( V_3 * ( V_4 + V_13 -> V_23 ) * ( V_5 + 1 ) >> V_6 ) / ( V_7 + 1 ) ;
return V_9 ;
}
static long F_5 ( struct V_1 * V_2 , unsigned long V_9 ,
unsigned long * V_3 )
{
struct V_10 * V_11 = F_2 ( V_2 ) ;
V_11 -> V_24 ( ( T_2 * ) & V_9 , ( T_2 ) * V_3 ,
NULL , NULL , NULL , NULL ) ;
return V_9 ;
}
static long F_6 ( struct V_1 * V_2 , unsigned long V_9 ,
unsigned long V_25 ,
unsigned long V_26 ,
unsigned long * V_27 ,
struct V_1 * * V_28 )
{
struct V_29 * V_29 = V_2 -> V_29 , * V_30 , * V_31 = NULL ;
int V_32 , V_33 ;
unsigned long V_3 , V_34 = 0 , V_35 , V_36 = 0 ;
V_33 = F_7 ( V_29 ) ;
for ( V_32 = 0 ; V_32 < V_33 ; V_32 ++ ) {
V_30 = F_8 ( V_29 , V_32 ) ;
if ( ! V_30 )
continue;
if ( F_9 ( V_29 ) & V_37 )
V_3 = F_10 ( V_30 , V_9 ) ;
else
V_3 = F_11 ( V_30 ) ;
V_35 = F_5 ( V_2 , V_9 , & V_3 ) ;
if ( V_35 <= V_9 && V_35 > V_36 ) {
V_31 = V_30 ;
V_34 = V_3 ;
V_36 = V_35 ;
}
}
if ( V_31 )
* V_28 = F_12 ( V_31 ) ;
* V_27 = V_34 ;
return V_36 ;
}
static int F_13 ( struct V_1 * V_2 , unsigned long V_9 ,
unsigned long V_3 )
{
T_1 V_4 = 0 , V_5 = 0 , V_7 = 0 , V_6 = 0 ;
T_2 V_8 ;
struct V_10 * V_11 = F_2 ( V_2 ) ;
struct V_12 * V_13 = V_11 -> V_13 ;
unsigned long V_38 = 0 ;
V_11 -> V_24 ( ( T_2 * ) & V_9 , ( T_2 ) V_3 , & V_4 , & V_5 , & V_7 , & V_6 ) ;
if ( V_11 -> V_39 )
F_14 ( V_11 -> V_39 , V_38 ) ;
V_8 = F_3 ( V_11 -> V_8 ) ;
V_8 = F_15 ( V_13 -> V_16 , V_13 -> V_14 , V_8 , V_4 ) ;
V_8 = F_15 ( V_13 -> V_18 , V_13 -> V_17 , V_8 , V_5 ) ;
V_8 = F_15 ( V_13 -> V_20 , V_13 -> V_19 , V_8 , V_7 ) ;
V_8 = F_15 ( V_13 -> V_22 , V_13 -> V_21 , V_8 , V_6 ) ;
F_16 ( V_8 , V_11 -> V_8 ) ;
F_17 ( ( V_9 >> 20 ) * 500 / 2 ) ;
if ( V_11 -> V_39 )
F_18 ( V_11 -> V_39 , V_38 ) ;
return 0 ;
}
struct V_29 * F_19 ( struct V_40 * V_41 ,
const struct V_42 * V_43 ,
T_3 * V_39 ,
void T_4 * V_8 )
{
struct V_29 * V_29 ;
struct V_10 * V_11 ;
struct V_44 * V_45 = NULL ;
struct V_46 * V_47 = NULL ;
struct V_1 * V_48 = NULL ;
struct V_1 * V_49 = NULL ;
const char * V_50 = V_41 -> V_51 ;
const char * V_52 [ V_53 ] ;
int V_32 = 0 ;
while ( V_32 < V_53 &&
( V_52 [ V_32 ] = F_20 ( V_41 , V_32 ) ) != NULL )
V_32 ++ ;
if ( V_43 -> V_51 )
V_50 = V_43 -> V_51 ;
else
F_21 ( V_41 , L_1 , & V_50 ) ;
V_11 = F_22 ( sizeof( struct V_10 ) , V_54 ) ;
if ( ! V_11 )
return NULL ;
V_11 -> V_8 = V_8 ;
V_11 -> V_13 = V_43 -> V_55 ;
V_11 -> V_24 = V_43 -> V_56 ;
V_11 -> V_39 = V_39 ;
if ( V_43 -> V_57 ) {
V_45 = F_22 ( sizeof( struct V_44 ) , V_54 ) ;
if ( ! V_45 ) {
F_23 ( V_11 ) ;
return NULL ;
}
V_45 -> V_8 = V_8 ;
V_45 -> V_58 = V_43 -> V_57 ;
V_45 -> V_39 = V_11 -> V_39 ;
V_48 = & V_45 -> V_2 ;
}
if ( V_43 -> V_47 ) {
V_47 = F_22 ( sizeof( struct V_46 ) , V_54 ) ;
if ( ! V_47 ) {
F_23 ( V_11 ) ;
F_23 ( V_45 ) ;
return NULL ;
}
V_47 -> V_8 = V_8 ;
V_47 -> V_59 = V_43 -> V_47 ;
V_47 -> V_60 = V_43 -> V_61 ;
V_47 -> V_39 = V_11 -> V_39 ;
V_49 = & V_47 -> V_2 ;
}
V_29 = F_24 ( NULL , V_50 ,
V_52 , V_32 ,
V_49 , & V_62 ,
& V_11 -> V_2 , & V_63 ,
V_48 , & V_64 , 0 ) ;
if ( ! F_25 ( V_29 ) ) {
F_26 ( V_41 , V_65 , V_29 ) ;
F_27 ( V_29 , V_50 , NULL ) ;
}
return V_29 ;
}
