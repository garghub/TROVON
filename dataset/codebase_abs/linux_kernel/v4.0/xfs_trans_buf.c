STATIC struct V_1 *
F_1 (
struct V_2 * V_3 ,
struct V_4 * V_5 ,
struct V_6 * V_7 ,
int V_8 )
{
struct V_9 * V_10 ;
struct V_11 * V_12 ;
int V_13 = 0 ;
int V_14 ;
for ( V_14 = 0 ; V_14 < V_8 ; V_14 ++ )
V_13 += V_7 [ V_14 ] . V_15 ;
F_2 (lidp, &tp->t_items, lid_trans) {
V_12 = (struct V_11 * ) V_10 -> V_16 ;
if ( V_12 -> V_17 . V_18 == V_19 &&
V_12 -> V_20 -> V_21 == V_5 &&
F_3 ( V_12 -> V_20 ) == V_7 [ 0 ] . V_22 &&
V_12 -> V_20 -> V_23 == V_13 ) {
ASSERT ( V_12 -> V_20 -> V_24 == V_8 ) ;
return V_12 -> V_20 ;
}
}
return NULL ;
}
STATIC void
F_4 (
struct V_2 * V_3 ,
struct V_1 * V_25 ,
int V_26 )
{
struct V_11 * V_27 ;
ASSERT ( V_25 -> V_28 == NULL ) ;
F_5 ( V_25 , V_3 -> V_29 ) ;
V_27 = V_25 -> V_30 ;
ASSERT ( ! ( V_27 -> V_31 & V_32 ) ) ;
ASSERT ( ! ( V_27 -> V_33 . V_34 & V_35 ) ) ;
ASSERT ( ! ( V_27 -> V_31 & V_36 ) ) ;
if ( V_26 )
V_27 -> V_37 = 0 ;
F_6 ( & V_27 -> V_38 ) ;
F_7 ( V_3 , & V_27 -> V_17 ) ;
V_25 -> V_28 = V_3 ;
}
void
F_8 (
struct V_2 * V_3 ,
struct V_1 * V_25 )
{
F_4 ( V_3 , V_25 , 0 ) ;
F_9 ( V_25 -> V_30 ) ;
}
struct V_1 *
F_10 (
struct V_2 * V_3 ,
struct V_4 * V_5 ,
struct V_6 * V_7 ,
int V_8 ,
T_1 V_39 )
{
T_2 * V_25 ;
T_3 * V_27 ;
if ( ! V_3 )
return F_11 ( V_5 , V_7 , V_8 , V_39 ) ;
V_25 = F_1 ( V_3 , V_5 , V_7 , V_8 ) ;
if ( V_25 != NULL ) {
ASSERT ( F_12 ( V_25 ) ) ;
if ( F_13 ( V_3 -> V_29 ) ) {
F_14 ( V_25 ) ;
F_15 ( V_25 ) ;
}
ASSERT ( V_25 -> V_28 == V_3 ) ;
V_27 = V_25 -> V_30 ;
ASSERT ( V_27 != NULL ) ;
ASSERT ( F_16 ( & V_27 -> V_38 ) > 0 ) ;
V_27 -> V_37 ++ ;
F_17 ( V_27 ) ;
return V_25 ;
}
V_25 = F_11 ( V_5 , V_7 , V_8 , V_39 ) ;
if ( V_25 == NULL ) {
return NULL ;
}
ASSERT ( ! V_25 -> V_40 ) ;
F_4 ( V_3 , V_25 , 1 ) ;
F_18 ( V_25 -> V_30 ) ;
return V_25 ;
}
T_2 *
F_19 ( T_4 * V_3 ,
struct V_41 * V_42 ,
int V_39 )
{
T_2 * V_25 ;
T_3 * V_27 ;
if ( V_3 == NULL )
return F_20 ( V_42 , V_39 ) ;
V_25 = V_42 -> V_43 ;
if ( V_25 -> V_28 == V_3 ) {
V_27 = V_25 -> V_30 ;
ASSERT ( V_27 != NULL ) ;
ASSERT ( F_16 ( & V_27 -> V_38 ) > 0 ) ;
V_27 -> V_37 ++ ;
F_21 ( V_27 ) ;
return V_25 ;
}
V_25 = F_20 ( V_42 , V_39 ) ;
if ( V_25 == NULL )
return NULL ;
F_4 ( V_3 , V_25 , 1 ) ;
F_22 ( V_25 -> V_30 ) ;
return V_25 ;
}
int
F_23 (
struct V_41 * V_42 ,
struct V_2 * V_3 ,
struct V_4 * V_5 ,
struct V_6 * V_7 ,
int V_8 ,
T_1 V_39 ,
struct V_1 * * V_44 ,
const struct V_45 * V_46 )
{
struct V_1 * V_25 = NULL ;
struct V_11 * V_27 ;
int error ;
* V_44 = NULL ;
if ( V_3 )
V_25 = F_1 ( V_3 , V_5 , V_7 , V_8 ) ;
if ( V_25 ) {
ASSERT ( F_12 ( V_25 ) ) ;
ASSERT ( V_25 -> V_28 == V_3 ) ;
ASSERT ( V_25 -> V_30 != NULL ) ;
ASSERT ( ! V_25 -> V_40 ) ;
ASSERT ( V_25 -> V_47 & V_48 ) ;
if ( F_13 ( V_42 ) ) {
F_24 ( V_25 , V_49 ) ;
return - V_50 ;
}
V_27 = V_25 -> V_30 ;
V_27 -> V_37 ++ ;
ASSERT ( F_16 ( & V_27 -> V_38 ) > 0 ) ;
F_25 ( V_27 ) ;
* V_44 = V_25 ;
return 0 ;
}
V_25 = F_26 ( V_5 , V_7 , V_8 , V_39 , V_46 ) ;
if ( ! V_25 ) {
if ( ! ( V_39 & V_51 ) )
return - V_52 ;
return V_3 ? 0 : - V_53 ;
}
if ( V_25 -> V_40 ) {
error = V_25 -> V_40 ;
if ( ! F_13 ( V_42 ) )
F_27 ( V_25 , V_54 ) ;
V_25 -> V_47 &= ~ V_48 ;
F_14 ( V_25 ) ;
if ( V_3 && ( V_3 -> V_55 & V_56 ) )
F_28 ( V_3 -> V_29 , V_57 ) ;
F_29 ( V_25 ) ;
if ( error == - V_58 )
error = - V_59 ;
return error ;
}
if ( F_13 ( V_42 ) ) {
F_29 ( V_25 ) ;
F_24 ( V_25 , V_49 ) ;
return - V_50 ;
}
if ( V_3 ) {
F_4 ( V_3 , V_25 , 1 ) ;
F_30 ( V_25 -> V_30 ) ;
}
* V_44 = V_25 ;
return 0 ;
}
void
F_31 ( T_4 * V_3 ,
T_2 * V_25 )
{
T_3 * V_27 ;
if ( V_3 == NULL ) {
ASSERT ( V_25 -> V_28 == NULL ) ;
F_29 ( V_25 ) ;
return;
}
ASSERT ( V_25 -> V_28 == V_3 ) ;
V_27 = V_25 -> V_30 ;
ASSERT ( V_27 -> V_17 . V_18 == V_19 ) ;
ASSERT ( ! ( V_27 -> V_31 & V_32 ) ) ;
ASSERT ( ! ( V_27 -> V_33 . V_34 & V_35 ) ) ;
ASSERT ( F_16 ( & V_27 -> V_38 ) > 0 ) ;
F_32 ( V_27 ) ;
if ( V_27 -> V_37 > 0 ) {
V_27 -> V_37 -- ;
return;
}
if ( V_27 -> V_17 . V_60 -> V_61 & V_62 )
return;
if ( V_27 -> V_31 & V_32 )
return;
ASSERT ( ! ( V_27 -> V_31 & V_36 ) ) ;
F_33 ( & V_27 -> V_17 ) ;
if ( V_27 -> V_31 & V_63 ) {
V_27 -> V_31 &= ~ V_63 ;
}
F_34 ( & V_27 -> V_38 ) ;
if ( ! F_35 ( V_27 ) ) {
ASSERT ( F_16 ( & V_27 -> V_38 ) == 0 ) ;
ASSERT ( ! ( V_27 -> V_17 . V_64 & V_65 ) ) ;
ASSERT ( ! ( V_27 -> V_31 & V_66 ) ) ;
F_36 ( V_25 ) ;
}
V_25 -> V_28 = NULL ;
F_29 ( V_25 ) ;
}
void
F_37 ( T_4 * V_3 ,
T_2 * V_25 )
{
T_3 * V_27 = V_25 -> V_30 ;
ASSERT ( V_25 -> V_28 == V_3 ) ;
ASSERT ( V_27 != NULL ) ;
ASSERT ( ! ( V_27 -> V_31 & V_32 ) ) ;
ASSERT ( ! ( V_27 -> V_33 . V_34 & V_35 ) ) ;
ASSERT ( F_16 ( & V_27 -> V_38 ) > 0 ) ;
V_27 -> V_31 |= V_63 ;
F_38 ( V_27 ) ;
}
void
F_39 ( T_4 * V_3 ,
T_2 * V_25 )
{
T_3 * V_27 = V_25 -> V_30 ;
ASSERT ( V_25 -> V_28 == V_3 ) ;
ASSERT ( V_27 != NULL ) ;
ASSERT ( ! ( V_27 -> V_31 & V_32 ) ) ;
ASSERT ( ! ( V_27 -> V_33 . V_34 & V_35 ) ) ;
ASSERT ( F_16 ( & V_27 -> V_38 ) > 0 ) ;
ASSERT ( V_27 -> V_31 & V_63 ) ;
V_27 -> V_31 &= ~ V_63 ;
F_40 ( V_27 ) ;
}
void
F_41 ( T_4 * V_3 ,
T_2 * V_25 ,
T_5 V_67 ,
T_5 V_68 )
{
T_3 * V_27 = V_25 -> V_30 ;
ASSERT ( V_25 -> V_28 == V_3 ) ;
ASSERT ( V_27 != NULL ) ;
ASSERT ( V_67 <= V_68 && V_68 < F_42 ( V_25 -> V_23 ) ) ;
ASSERT ( V_25 -> V_69 == NULL ||
V_25 -> V_69 == V_70 ) ;
F_15 ( V_25 ) ;
ASSERT ( F_16 ( & V_27 -> V_38 ) > 0 ) ;
V_25 -> V_69 = V_70 ;
V_27 -> V_17 . V_71 = V_72 ;
F_43 ( V_27 ) ;
if ( V_27 -> V_31 & V_32 ) {
V_27 -> V_31 &= ~ V_32 ;
ASSERT ( F_44 ( V_25 ) ) ;
F_45 ( V_25 ) ;
V_27 -> V_33 . V_34 &= ~ V_35 ;
}
V_3 -> V_55 |= V_56 ;
V_27 -> V_17 . V_60 -> V_61 |= V_62 ;
V_27 -> V_31 |= V_73 | V_36 ;
if ( ! ( V_27 -> V_31 & V_74 ) )
F_46 ( V_27 , V_67 , V_68 ) ;
}
void
F_47 (
T_4 * V_3 ,
T_2 * V_25 )
{
T_3 * V_27 = V_25 -> V_30 ;
int V_14 ;
ASSERT ( V_25 -> V_28 == V_3 ) ;
ASSERT ( V_27 != NULL ) ;
ASSERT ( F_16 ( & V_27 -> V_38 ) > 0 ) ;
F_48 ( V_27 ) ;
if ( V_27 -> V_31 & V_32 ) {
ASSERT ( F_44 ( V_25 ) ) ;
ASSERT ( ! ( V_27 -> V_31 & ( V_36 | V_73 ) ) ) ;
ASSERT ( ! ( V_27 -> V_33 . V_34 & V_75 ) ) ;
ASSERT ( ! ( V_27 -> V_33 . V_34 & V_76 ) ) ;
ASSERT ( V_27 -> V_33 . V_34 & V_35 ) ;
ASSERT ( V_27 -> V_17 . V_60 -> V_61 & V_62 ) ;
ASSERT ( V_3 -> V_55 & V_56 ) ;
return;
}
F_14 ( V_25 ) ;
V_27 -> V_31 |= V_32 ;
V_27 -> V_31 &= ~ ( V_77 | V_36 | V_73 ) ;
V_27 -> V_33 . V_34 &= ~ V_75 ;
V_27 -> V_33 . V_34 |= V_35 ;
V_27 -> V_33 . V_34 &= ~ V_76 ;
for ( V_14 = 0 ; V_14 < V_27 -> V_78 ; V_14 ++ ) {
memset ( V_27 -> V_79 [ V_14 ] . V_80 , 0 ,
( V_27 -> V_79 [ V_14 ] . V_81 * sizeof( T_5 ) ) ) ;
}
V_27 -> V_17 . V_60 -> V_61 |= V_62 ;
V_3 -> V_55 |= V_56 ;
}
void
F_49 (
T_4 * V_3 ,
T_2 * V_25 )
{
T_3 * V_27 = V_25 -> V_30 ;
ASSERT ( V_25 -> V_28 == V_3 ) ;
ASSERT ( V_27 != NULL ) ;
ASSERT ( F_16 ( & V_27 -> V_38 ) > 0 ) ;
V_27 -> V_31 |= V_77 ;
F_50 ( V_3 , V_25 , V_82 ) ;
}
void
F_51 (
T_4 * V_3 ,
T_2 * V_25 )
{
T_3 * V_27 = V_25 -> V_30 ;
ASSERT ( V_25 -> V_28 == V_3 ) ;
ASSERT ( V_27 != NULL ) ;
ASSERT ( F_16 ( & V_27 -> V_38 ) > 0 ) ;
V_27 -> V_31 |= V_83 ;
V_27 -> V_17 . V_71 = V_72 ;
F_50 ( V_3 , V_25 , V_82 ) ;
}
void
F_52 (
T_4 * V_3 ,
T_2 * V_25 )
{
T_3 * V_27 = V_25 -> V_30 ;
ASSERT ( V_25 -> V_28 == V_3 ) ;
ASSERT ( V_27 != NULL ) ;
ASSERT ( F_16 ( & V_27 -> V_38 ) > 0 ) ;
V_27 -> V_31 |= V_66 ;
F_50 ( V_3 , V_25 , V_82 ) ;
}
void
F_53 (
struct V_2 * V_3 ,
struct V_1 * V_25 )
{
struct V_11 * V_27 = V_25 -> V_30 ;
ASSERT ( V_25 -> V_28 == V_3 ) ;
ASSERT ( V_27 != NULL ) ;
ASSERT ( F_16 ( & V_27 -> V_38 ) > 0 ) ;
V_27 -> V_31 |= V_74 ;
F_54 ( V_27 ) ;
}
void
F_50 (
struct V_2 * V_3 ,
struct V_1 * V_25 ,
enum V_84 type )
{
struct V_11 * V_27 = V_25 -> V_30 ;
if ( ! V_3 )
return;
ASSERT ( V_25 -> V_28 == V_3 ) ;
ASSERT ( V_27 != NULL ) ;
ASSERT ( F_16 ( & V_27 -> V_38 ) > 0 ) ;
F_55 ( & V_27 -> V_33 , type ) ;
}
void
F_56 (
struct V_1 * V_85 ,
struct V_1 * V_86 )
{
struct V_11 * V_87 = V_86 -> V_30 ;
struct V_11 * V_88 = V_85 -> V_30 ;
enum V_84 type ;
type = F_57 ( & V_87 -> V_33 ) ;
F_55 ( & V_88 -> V_33 , type ) ;
}
void
F_58 (
T_4 * V_3 ,
T_2 * V_25 ,
T_5 type )
{
struct V_11 * V_27 = V_25 -> V_30 ;
ASSERT ( type == V_89 ||
type == V_90 ||
type == V_91 ) ;
V_27 -> V_33 . V_34 |= type ;
switch ( type ) {
case V_89 :
type = V_92 ;
break;
case V_90 :
type = V_93 ;
break;
case V_91 :
type = V_94 ;
break;
default:
type = V_95 ;
break;
}
F_50 ( V_3 , V_25 , type ) ;
}
