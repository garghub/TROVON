static int F_1 ( T_1 * V_1 )
{
return 0 ;
}
static int F_2 ( T_1 * V_1 )
{
return 0 ;
}
static void F_3 ( T_1 * V_1 )
{
}
static int F_4 ( T_1 * V_1 , struct V_2 * V_3 ,
union V_4 * V_4 ,
struct V_5 * V_6 )
{
T_2 * V_7 = ( T_2 * ) V_1 -> V_8 ;
int V_9 ;
if ( ( V_1 -> V_10 & ( V_11 | V_12 ) ) ==
V_11 ) {
V_4 -> V_13 . V_14 |= F_5 ( F_6 ( 1 ) ) ;
}
V_9 = F_7 ( V_7 , V_3 , V_4 , V_6 ) ;
return V_9 ;
}
static int F_8 ( T_1 * V_1 , struct V_15 * V_16 )
{
T_2 * V_7 = ( T_2 * ) V_1 -> V_8 ;
int V_9 = 0 ;
switch ( V_16 -> V_17 ) {
case V_18 :
F_9 ( L_1 ) ;
V_9 = F_10 ( V_1 , V_16 ) ;
break;
case V_19 :
F_9 ( L_2 ) ;
V_9 = F_10 ( V_1 , V_16 ) ;
break;
case V_20 :
F_9 ( L_3 ) ;
V_9 = F_11 ( V_1 , V_16 ) ;
break;
case V_21 :
F_9 ( L_4 ) ;
V_9 = F_12 ( V_1 , V_16 ) ;
break;
case V_22 :
F_9 ( L_5 ) ;
V_9 = F_13 ( V_1 , V_16 ) ;
break;
case V_23 :
F_9 ( L_6 ) ;
V_9 = F_14 ( V_1 , V_16 ) ;
break;
case V_24 :
F_9 ( L_7 ) ;
V_9 = F_15 ( V_1 , V_16 ) ;
break;
case V_25 :
F_9 ( L_8 ) ;
V_9 = F_16 ( V_1 , V_16 ) ;
break;
case V_26 :
F_9 ( L_9 ) ;
V_9 = F_17 ( V_1 , V_16 ) ;
break;
case V_27 :
F_9 ( L_10 ) ;
V_9 = F_18 ( V_1 , V_16 ) ;
break;
case V_28 :
break;
case V_29 :
{
struct V_30 * V_31 ;
F_9 ( L_11 ) ;
V_31 = (struct V_30 * ) V_16 ;
V_9 =
F_19 ( V_1 ,
V_31 -> V_32 . V_33 ) ;
V_31 -> V_34 . V_35 =
V_36 ;
V_31 -> V_34 . V_33 = V_9 ;
V_9 = 0 ;
}
break;
case V_37 :
F_9 ( L_12 ) ;
V_9 = F_20 ( V_1 , V_16 ) ;
break;
case V_38 :
F_9 ( L_13 ) ;
V_9 = F_21 ( V_1 , V_16 ) ;
break;
case V_39 : {
struct V_40 * V_41 ;
F_9 ( L_14 ) ;
V_41 = (struct V_40 * ) V_16 ;
V_41 -> V_42 . V_35 =
V_36 ;
V_41 -> V_43 . V_35 =
V_36 ;
V_41 -> V_44 . V_35 =
V_36 ;
V_41 -> V_42 . V_33 = F_22 ( V_7 -> V_45 . V_46 ) ;
V_41 -> V_43 . V_33 = F_22 ( V_7 -> V_45 . V_47 ) ;
V_41 -> V_44 . V_33 = F_22 ( V_7 -> V_45 . V_48 ) ;
V_41 -> V_49 . V_33 = V_7 -> V_49 ;
break;
}
default:
F_23 ( V_1 -> V_50 ,
L_15 ,
V_16 -> V_17 ) ;
break;
}
return V_9 ;
}
T_3 F_19 ( T_1 * V_1 , T_3 V_32 )
{
T_2 * V_7 = ( T_2 * ) V_1 -> V_8 ;
T_3 V_9 ;
V_9 = V_51 ;
F_9 ( L_16 ,
V_1 -> V_52 , V_32 ) ;
switch ( V_32 ) {
case V_53 :
switch ( V_1 -> V_52 ) {
case V_54 :
V_1 -> V_52 = V_55 ;
V_9 = F_24 ( V_7 ) ;
if ( V_9 ) {
F_25 ( V_1 -> V_50 ,
L_17 , ( int ) V_9 ) ;
V_9 =
V_51 ;
V_1 -> V_52 = V_54 ;
break;
}
V_1 -> V_52 = V_56 ;
V_9 = V_57 ;
break;
case V_56 :
F_26 ( V_7 ) ;
V_9 = V_57 ;
break;
case V_58 :
F_23 ( V_1 -> V_50 ,
L_18 ) ;
V_9 = V_59 ;
break;
case V_60 :
default:
V_9 = V_51 ;
break;
}
break;
case V_61 :
switch ( V_1 -> V_52 ) {
case V_54 :
case V_56 :
V_1 -> V_52 = V_62 ;
V_9 = F_24 ( V_7 ) ;
if ( V_9 ) {
F_25 ( V_1 -> V_50 ,
L_17 , ( int ) V_9 ) ;
V_9 =
V_51 ;
V_1 -> V_52 = V_54 ;
break;
}
V_9 = F_27 ( V_1 ) ;
if ( V_9 ) {
F_25 ( V_1 -> V_50 ,
L_19 , ( int ) V_9 ) ;
V_9 =
V_51 ;
F_28 ( V_7 ) ;
V_1 -> V_52 = V_54 ;
break;
}
V_9 = F_29 ( V_1 ) ;
if ( V_9 ) {
F_25 ( V_1 -> V_50 ,
L_20 , ( int ) V_9 ) ;
V_9 =
V_51 ;
F_28 ( V_7 ) ;
V_1 -> V_52 = V_54 ;
break;
}
V_1 -> V_52 = V_58 ;
V_7 -> V_63 = 0 ;
V_7 -> V_64 = 60 ;
V_9 = V_57 ;
break;
case V_58 :
V_9 = V_57 ;
break;
case V_60 :
default:
V_9 = V_51 ;
break;
}
break;
case V_65 :
switch ( V_1 -> V_52 ) {
case V_54 :
V_9 = V_57 ;
break;
case V_56 :
case V_58 :
V_1 -> V_52 = V_66 ;
if ( ! V_1 -> V_67 )
F_30 ( V_1 -> V_50 ) ;
F_28 ( V_7 ) ;
V_1 -> V_68 = V_69 ;
V_1 -> V_52 = V_54 ;
V_9 = V_57 ;
break;
case V_60 :
default:
V_9 = V_51 ;
break;
}
break;
default:
V_9 = V_59 ;
break;
}
return V_9 ;
}
static int F_27 ( T_1 * V_1 )
{
int V_9 = 0 ;
T_2 * V_7 = ( T_2 * ) V_1 -> V_8 ;
T_4 V_70 ;
T_5 V_71 [ V_72 ] ;
V_9 = F_31 ( V_7 , V_73 ,
& V_7 -> V_74 ,
sizeof( V_75 ) ) ;
if ( V_9 ) {
F_25 ( V_1 -> V_50 , L_21 ) ;
goto V_76;
}
V_7 -> V_74 . V_77 = F_22 ( V_7 -> V_74 . V_77 ) ;
V_7 -> V_74 . V_78 = F_22 ( V_7 -> V_74 . V_78 ) ;
V_7 -> V_74 . V_79 = F_22 ( V_7 -> V_74 . V_79 ) ;
V_7 -> V_74 . V_80 = F_22 ( V_7 -> V_74 . V_80 ) ;
F_32 ( V_1 -> V_50 , L_22 ,
V_7 -> V_74 . V_77 , V_7 -> V_74 . V_79 ,
V_7 -> V_74 . V_80 , V_7 -> V_74 . V_78 ) ;
V_9 = F_31 ( V_7 , V_81 ,
& V_7 -> V_82 ,
sizeof( V_75 ) ) ;
if ( V_9 ) {
F_25 ( V_1 -> V_50 , L_23 ) ;
goto V_76;
}
V_7 -> V_82 . V_77 = F_22 ( V_7 -> V_82 . V_77 ) ;
V_7 -> V_82 . V_78 = F_22 ( V_7 -> V_82 . V_78 ) ;
V_7 -> V_82 . V_79 = F_22 ( V_7 -> V_82 . V_79 ) ;
V_7 -> V_82 . V_80 = F_22 ( V_7 -> V_82 . V_80 ) ;
F_32 ( V_1 -> V_50 , L_24 ,
V_7 -> V_82 . V_77 , V_7 -> V_82 . V_79 ,
V_7 -> V_82 . V_80 , V_7 -> V_82 . V_78 ) ;
V_9 = F_31 ( V_7 , V_83 ,
& V_7 -> V_84 ,
sizeof( V_75 ) ) ;
if ( V_9 ) {
F_25 ( V_1 -> V_50 , L_25 ) ;
goto V_76;
}
if ( V_7 -> V_74 . V_77 < 0x8000 ) {
F_25 ( V_1 -> V_50 ,
L_26 ) ;
V_9 = - 1 ;
goto V_76;
}
V_7 -> V_84 . V_77 = F_22 ( V_7 -> V_84 . V_77 ) ;
V_7 -> V_84 . V_78 = F_22 ( V_7 -> V_84 . V_78 ) ;
V_7 -> V_84 . V_79 = F_22 ( V_7 -> V_84 . V_79 ) ;
V_7 -> V_84 . V_80 = F_22 ( V_7 -> V_84 . V_80 ) ;
V_7 -> V_85 = V_7 -> V_84 . V_78 & ( F_33 ( 14 ) | F_33 ( 15 ) ) ;
V_7 -> V_84 . V_78 &= ~ ( ( T_4 ) ( F_33 ( 14 ) | F_33 ( 15 ) ) ) ;
if ( V_7 -> V_84 . V_77 == 0x1f ) {
F_32 ( V_1 -> V_50 ,
L_27 ,
V_7 -> V_84 . V_77 , V_7 -> V_84 . V_79 ,
V_7 -> V_84 . V_80 , V_7 -> V_84 . V_78 ) ;
} else {
F_32 ( V_1 -> V_50 ,
L_28 ,
V_7 -> V_84 . V_77 , V_7 -> V_84 . V_79 ,
V_7 -> V_84 . V_80 , V_7 -> V_84 . V_78 ) ;
F_25 ( V_1 -> V_50 , L_29 ) ;
goto V_76;
}
V_9 = F_31 ( V_7 , V_86 ,
& V_7 -> V_87 ,
sizeof( V_88 ) ) ;
if ( V_9 ) {
F_25 ( V_1 -> V_50 , L_30 ) ;
goto V_76;
}
V_7 -> V_87 . V_89 = F_22 ( V_7 -> V_87 . V_89 ) ;
V_7 -> V_87 . V_77 = F_22 ( V_7 -> V_87 . V_77 ) ;
V_7 -> V_87 . V_78 = F_22 ( V_7 -> V_87 . V_78 ) ;
V_7 -> V_87 . V_90 = F_22 ( V_7 -> V_87 . V_90 ) ;
V_7 -> V_87 . V_91 = F_22 ( V_7 -> V_87 . V_91 ) ;
F_32 ( V_1 -> V_50 ,
L_31 ,
V_7 -> V_87 . V_89 , V_7 -> V_87 . V_77 ,
V_7 -> V_87 . V_78 , V_7 -> V_87 . V_90 ,
V_7 -> V_87 . V_91 ) ;
V_9 = F_31 ( V_7 , V_92 ,
& V_7 -> V_93 ,
sizeof( V_88 ) ) ;
if ( V_9 ) {
F_25 ( V_1 -> V_50 , L_32 ) ;
goto V_76;
}
V_7 -> V_93 . V_89 = F_22 ( V_7 -> V_93 . V_89 ) ;
V_7 -> V_93 . V_77 = F_22 ( V_7 -> V_93 . V_77 ) ;
V_7 -> V_93 . V_78 = F_22 ( V_7 -> V_93 . V_78 ) ;
V_7 -> V_93 . V_90 = F_22 ( V_7 -> V_93 . V_90 ) ;
V_7 -> V_93 . V_91 = F_22 ( V_7 -> V_93 . V_91 ) ;
F_32 ( V_1 -> V_50 ,
L_33 ,
V_7 -> V_93 . V_89 , V_7 -> V_93 . V_77 ,
V_7 -> V_93 . V_78 , V_7 -> V_93 . V_90 ,
V_7 -> V_93 . V_91 ) ;
V_9 = F_31 ( V_7 , V_94 ,
& V_7 -> V_95 ,
sizeof( V_88 ) ) ;
if ( V_9 ) {
F_25 ( V_1 -> V_50 , L_34 ) ;
goto V_76;
}
V_7 -> V_95 . V_89 = F_22 ( V_7 -> V_95 . V_89 ) ;
V_7 -> V_95 . V_77 = F_22 ( V_7 -> V_95 . V_77 ) ;
V_7 -> V_95 . V_78 = F_22 ( V_7 -> V_95 . V_78 ) ;
V_7 -> V_95 . V_90 = F_22 ( V_7 -> V_95 . V_90 ) ;
V_7 -> V_95 . V_91 = F_22 ( V_7 -> V_95 . V_91 ) ;
F_32 ( V_1 -> V_50 ,
L_35 ,
V_7 -> V_95 . V_89 , V_7 -> V_95 . V_77 ,
V_7 -> V_95 . V_78 , V_7 -> V_95 . V_90 ,
V_7 -> V_95 . V_91 ) ;
V_9 = F_31 ( V_7 , V_96 ,
& V_7 -> V_97 ,
sizeof( V_88 ) ) ;
if ( V_9 ) {
F_25 ( V_1 -> V_50 , L_36 ) ;
goto V_76;
}
V_7 -> V_97 . V_89 = F_22 ( V_7 -> V_97 . V_89 ) ;
V_7 -> V_97 . V_77 = F_22 ( V_7 -> V_97 . V_77 ) ;
V_7 -> V_97 . V_78 = F_22 ( V_7 -> V_97 . V_78 ) ;
V_7 -> V_97 . V_90 = F_22 ( V_7 -> V_97 . V_90 ) ;
V_7 -> V_97 . V_91 = F_22 ( V_7 -> V_97 . V_91 ) ;
if ( V_7 -> V_97 . V_77 == 0x04 ) {
F_32 ( V_1 -> V_50 ,
L_37 ,
V_7 -> V_97 . V_89 , V_7 -> V_97 . V_77 ,
V_7 -> V_97 . V_78 , V_7 -> V_97 . V_90 ,
V_7 -> V_97 . V_91 ) ;
} else {
F_32 ( V_1 -> V_50 ,
L_38 ,
V_7 -> V_97 . V_89 , V_7 -> V_97 . V_77 ,
V_7 -> V_97 . V_78 , V_7 -> V_97 . V_90 ,
V_7 -> V_97 . V_91 ) ;
}
V_9 = F_31 ( V_7 , V_98 ,
& V_7 -> V_99 ,
sizeof( V_88 ) ) ;
if ( V_9 ) {
F_25 ( V_1 -> V_50 , L_39 ) ;
goto V_76;
}
V_7 -> V_99 . V_89 = F_22 ( V_7 -> V_99 . V_89 ) ;
V_7 -> V_99 . V_77 = F_22 ( V_7 -> V_99 . V_77 ) ;
V_7 -> V_99 . V_78 = F_22 ( V_7 -> V_99 . V_78 ) ;
V_7 -> V_99 . V_90 = F_22 ( V_7 -> V_99 . V_90 ) ;
V_7 -> V_99 . V_91 = F_22 ( V_7 -> V_99 . V_91 ) ;
F_32 ( V_1 -> V_50 ,
L_40 ,
V_7 -> V_99 . V_89 , V_7 -> V_99 . V_77 ,
V_7 -> V_99 . V_78 , V_7 -> V_99 . V_90 ,
V_7 -> V_99 . V_91 ) ;
V_9 = F_31 ( V_7 , V_100 ,
& V_7 -> V_101 ,
sizeof( V_88 ) ) ;
if ( V_9 ) {
F_25 ( V_1 -> V_50 , L_41 ) ;
goto V_76;
}
V_7 -> V_101 . V_89 = F_22 ( V_7 -> V_101 . V_89 ) ;
V_7 -> V_101 . V_77 = F_22 ( V_7 -> V_101 . V_77 ) ;
V_7 -> V_101 . V_78 = F_22 ( V_7 -> V_101 . V_78 ) ;
V_7 -> V_101 . V_90 = F_22 ( V_7 -> V_101 . V_90 ) ;
V_7 -> V_101 . V_91 = F_22 ( V_7 -> V_101 . V_91 ) ;
F_32 ( V_1 -> V_50 ,
L_42 ,
V_7 -> V_101 . V_89 , V_7 -> V_101 . V_77 ,
V_7 -> V_101 . V_78 , V_7 -> V_101 . V_90 ,
V_7 -> V_101 . V_91 ) ;
V_9 = F_31 ( V_7 , V_102 ,
& V_7 -> V_103 ,
sizeof( V_88 ) ) ;
if ( V_9 ) {
F_25 ( V_1 -> V_50 , L_43 ) ;
goto V_76;
}
V_7 -> V_103 . V_89 = F_22 ( V_7 -> V_103 . V_89 ) ;
V_7 -> V_103 . V_77 = F_22 ( V_7 -> V_103 . V_77 ) ;
V_7 -> V_103 . V_78 = F_22 ( V_7 -> V_103 . V_78 ) ;
V_7 -> V_103 . V_90 = F_22 ( V_7 -> V_103 . V_90 ) ;
V_7 -> V_103 . V_91 = F_22 ( V_7 -> V_103 . V_91 ) ;
F_32 ( V_1 -> V_50 ,
L_44 ,
V_7 -> V_103 . V_89 , V_7 -> V_103 . V_77 ,
V_7 -> V_103 . V_78 , V_7 -> V_103 . V_90 ,
V_7 -> V_103 . V_91 ) ;
V_9 = F_31 ( V_7 , V_104 ,
V_71 , V_72 ) ;
if ( ! V_9 ) {
F_32 ( V_1 -> V_50 , L_45 ,
V_72 , V_71 ) ;
} else {
F_25 ( V_1 -> V_50 , L_46 ) ;
goto V_76;
}
V_9 = F_31 ( V_7 , V_105 ,
V_1 -> V_50 -> V_106 , V_107 ) ;
if ( V_9 != 0 ) {
F_25 ( V_1 -> V_50 , L_47 ) ;
goto V_76;
}
V_1 -> V_108 |= V_109 ;
F_34 ( V_7 , V_110 , & V_70 ) ;
if ( V_70 )
V_1 -> V_108 |= V_111 ;
F_34 ( V_7 , V_112 , & V_70 ) ;
V_7 -> V_113 = V_70 ;
if ( F_35 ( V_7 -> V_84 . V_79 ,
V_7 -> V_84 . V_80 ,
V_7 -> V_84 . V_78 ) <
F_35 ( 1 , 5 , 5 ) ) {
V_1 -> V_108 |= V_114 ;
}
goto V_115;
V_76:
F_25 ( V_1 -> V_50 , L_48 , V_9 ) ;
V_115:
return V_9 ;
}
static int F_29 ( T_1 * V_1 )
{
T_2 * V_7 = ( T_2 * ) V_1 -> V_8 ;
return F_36 ( V_7 , V_116 ,
V_117 ) ;
}
static int F_37 ( T_1 * V_1 , T_6 * V_118 )
{
int V_9 = 0 ;
T_2 * V_7 = ( T_2 * ) V_1 -> V_8 ;
T_4 V_119 ;
if ( V_7 -> V_120 != V_121 )
goto exit;
if ( ( V_118 -> V_122 & ( V_123 | V_124 ) ) != 0 )
V_119 = V_125 ;
else
V_119 = V_126 ;
V_9 =
F_38 ( V_7 , V_127 ,
V_119 ) ;
exit:
return V_9 ;
}
static void F_39 ( T_1 * V_1 ,
T_7 * V_128 )
{
F_9 ( L_49 ) ;
}
static void F_40 ( T_1 * V_1 ,
T_7 * V_128 )
{
T_2 * V_7 = ( T_2 * ) V_1 -> V_8 ;
T_4 * V_129 ;
T_3 * V_130 ;
T_3 * V_131 ;
int V_132 ;
int V_133 ;
V_133 = sizeof( V_134 ) / sizeof( T_3 ) ;
if ( V_128 -> V_135 > 22 ) {
V_130 = ( T_3 * ) & V_7 -> V_136 ;
V_131 = ( T_3 * ) & V_128 -> V_137 . V_138 ;
for ( V_132 = 0 ; V_132 < V_133 ; V_132 ++ , V_130 ++ , V_131 ++ )
* V_130 += F_41 ( * V_131 ) ;
} else {
V_130 = ( T_3 * ) & V_7 -> V_136 ;
V_129 = ( T_4 * ) & V_128 -> V_137 . V_139 ;
for ( V_132 = 0 ; V_132 < V_133 ; V_132 ++ , V_130 ++ , V_129 ++ )
* V_130 += F_22 ( * V_129 ) ;
}
}
static void F_42 ( T_1 * V_1 ,
T_7 * V_128 )
{
T_2 * V_7 = ( T_2 * ) V_1 -> V_8 ;
int V_140 ;
T_8 * V_141 = & ( V_128 -> V_137 . V_142 ) ;
int V_132 ;
T_9 V_143 ;
int V_9 ;
V_140 = ( V_128 -> V_135 * sizeof( T_4 ) ) -
sizeof( V_128 -> V_144 ) - sizeof( V_128 -> V_137 . V_142 . V_145 ) ;
V_140 /= sizeof( V_146 ) ;
F_9 ( L_50 ,
V_128 -> V_137 . V_142 . V_145 , V_140 ) ;
for ( V_132 = 0 ; V_132 < V_140 ; V_132 ++ ) {
F_9 ( L_51 ,
V_141 -> V_9 [ V_132 ] . V_147 ,
V_141 -> V_9 [ V_132 ] . V_148 ,
V_141 -> V_9 [ V_132 ] . V_149 , V_141 -> V_9 [ V_132 ] . V_150 ) ;
F_9 ( L_52 ,
V_141 -> V_9 [ V_132 ] . V_151 , V_141 -> V_9 [ V_132 ] . V_152 ) ;
}
V_143 . V_153 = V_141 -> V_9 [ 0 ] . V_147 ;
memcpy ( V_143 . V_154 , V_141 -> V_9 [ 0 ] . V_154 , V_155 ) ;
V_9 = F_43 ( V_7 ,
V_156 ,
& V_143 , V_157 ) ;
if ( V_9 ) {
F_25 ( V_1 -> V_50 , L_53 ,
V_9 ) ;
}
}
static void F_44 ( T_1 * V_1 ,
T_7 * V_128 )
{
T_2 * V_7 = ( T_2 * ) V_1 -> V_8 ;
int V_140 ;
V_140 = ( V_128 -> V_135 - 3 ) / 32 ;
F_9 ( L_54 , V_140 ) ;
if ( V_140 > 32 )
V_140 = 32 ;
F_45 ( V_7 -> V_158 ) ;
V_7 -> V_158 = F_46 ( V_128 , sizeof( T_7 ) , V_159 ) ;
if ( V_140 == 0 )
V_140 = - 1 ;
V_7 -> V_160 = V_140 ;
F_47 ( & V_7 -> V_161 ) ;
}
static void F_48 ( T_1 * V_1 ,
T_7 * V_128 )
{
T_2 * V_7 = ( T_2 * ) V_1 -> V_8 ;
unsigned int V_132 , V_162 ;
V_7 -> V_163 . V_164 . V_165 =
F_22 ( V_128 -> V_137 . V_166 . V_165 ) ;
for ( V_132 = 0 , V_162 = 0 ; V_132 < V_167 ; V_132 ++ ) {
T_10 * V_9 ;
T_10 * V_166 ;
int V_168 ;
if ( ! ( V_7 -> V_163 . V_164 . V_165 & ( 1 << V_132 ) ) )
continue;
V_9 = & V_128 -> V_137 . V_166 . V_9 [ V_162 ] ;
V_168 = F_22 ( V_9 -> V_147 ) - 1 ;
if ( V_168 < 0 || V_168 >= V_167 )
continue;
V_166 = & V_7 -> V_163 . V_164 . V_9 [ V_168 ] ;
V_166 -> V_147 = V_168 ;
V_166 -> V_148 = F_22 ( V_9 -> V_148 ) ;
V_166 -> V_169 = F_22 ( V_9 -> V_169 ) ;
V_166 -> V_170 = F_22 ( V_9 -> V_170 ) ;
F_9 ( L_55 ,
V_168 + 1 ,
( V_166 -> V_170 & V_171 )
? L_56 : L_57 ,
V_166 -> V_148 , V_166 -> V_169 ,
( V_166 -> V_170 & V_172 )
? 1 : 0 ) ;
V_162 ++ ;
}
F_49 ( & V_7 -> V_163 . V_115 , 2 ) ;
V_7 -> V_163 . V_173 = V_162 ;
}
void F_50 ( struct V_174 * V_33 )
{
T_2 * V_7 = F_51 ( V_33 , struct V_175 , V_176 ) ;
T_1 * V_1 = V_7 -> V_1 ;
T_11 V_177 ;
int V_9 ;
{
struct V_2 * V_3 ;
T_7 * V_128 ;
while ( ( V_3 = F_52 ( & V_7 -> V_178 ) ) ) {
V_128 = ( T_7 * ) V_3 -> V_33 ;
F_53 ( V_1 , V_128 ) ;
}
}
if ( V_7 -> V_179 == V_7 -> V_180 )
return;
V_7 -> V_179 = V_7 -> V_180 ;
switch ( V_7 -> V_179 ) {
case V_181 :
F_30 ( V_1 -> V_50 ) ;
F_32 ( V_1 -> V_50 , L_58 ) ;
break;
case V_182 :
F_54 ( V_1 -> V_50 ) ;
if ( V_7 -> V_63 == 1 )
V_7 -> V_63 = 2 ;
V_7 -> V_64 = 60 ;
if ( V_1 -> V_50 -> type == V_183 ) {
T_4 V_184 ;
F_32 ( V_1 -> V_50 , L_59 ) ;
V_9 = F_31 ( V_7 ,
V_185 ,
V_1 -> V_154 ,
V_155 ) ;
if ( V_9 ) {
F_9
( L_60 ,
V_185 , V_9 ) ;
return;
}
V_9 = F_31 ( V_7 ,
V_186 ,
& V_177 , sizeof( V_177 ) ) ;
if ( V_9 ) {
F_9
( L_60 ,
V_186 , V_9 ) ;
return;
}
F_55 ( (struct V_187 * ) & V_177 ,
( V_188 * ) &
V_1 -> V_177 ) ;
V_9 = F_34 ( V_7 ,
V_189 ,
& V_184 ) ;
if ( V_9 ) {
F_9
( L_60 ,
V_189 , V_9 ) ;
return;
}
V_1 -> V_68 =
( V_184 == V_190 ) ?
V_191 : V_192 ;
F_56 ( V_1 , V_126 ) ;
F_57 ( & V_7 -> V_193 ) ;
}
break;
case V_194 :
if ( V_1 -> V_50 -> type == V_183 )
F_32 ( V_1 -> V_50 ,
L_61 ) ;
V_1 -> V_68 = V_69 ;
F_30 ( V_1 -> V_50 ) ;
F_58 ( V_1 ) ;
break;
case V_195 :
F_32 ( V_1 -> V_50 , L_62 ) ;
V_9 = F_31 ( V_7 ,
V_185 ,
V_1 -> V_154 , V_155 ) ;
if ( V_9 ) {
F_9 ( L_60 ,
V_185 , V_9 ) ;
return;
}
V_9 = F_31 ( V_7 ,
V_186 ,
& V_177 , sizeof( V_177 ) ) ;
if ( V_9 ) {
F_9 ( L_60 ,
V_186 , V_9 ) ;
return;
}
F_55 ( (struct V_187 * ) & V_177 ,
( V_188 * ) & V_1 -> V_177 ) ;
V_7 -> V_179 = V_182 ;
F_54 ( V_1 -> V_50 ) ;
F_59 ( V_1 ) ;
break;
case V_196 :
F_32 ( V_1 -> V_50 , L_63 ) ;
F_30 ( V_1 -> V_50 ) ;
break;
case V_197 :
F_32 ( V_1 -> V_50 , L_64 ) ;
V_7 -> V_179 = V_182 ;
F_54 ( V_1 -> V_50 ) ;
break;
case V_198 :
if ( V_7 -> V_63 && -- V_7 -> V_64 > 0 ) {
T_9 V_143 ;
V_143 = V_7 -> V_143 ;
F_43 ( V_7 ,
V_156 ,
& V_143 ,
V_157 ) ;
F_32 ( V_1 -> V_50 ,
L_65 ) ;
} else {
F_32 ( V_1 -> V_50 , L_66 ) ;
}
F_30 ( V_1 -> V_50 ) ;
F_56 ( V_1 , V_125 ) ;
break;
default:
F_23 ( V_1 -> V_50 ,
L_67 , V_7 -> V_179 ) ;
return;
}
V_1 -> V_199 = ( V_7 -> V_179 == V_182 ) ;
}
static void F_60 ( T_1 * V_1 ,
T_7 * V_128 )
{
T_2 * V_7 = ( T_2 * ) V_1 -> V_8 ;
V_7 -> V_180 = F_22 ( V_128 -> V_137 . V_199 . V_199 ) ;
F_61 ( & V_7 -> V_176 ) ;
}
static void F_62 ( T_1 * V_1 ,
T_7 * V_128 )
{
T_2 * V_7 = ( T_2 * ) V_1 -> V_8 ;
T_12 V_200 ;
int V_132 ;
memcpy ( & V_200 , & V_128 -> V_137 . V_201 , sizeof( V_200 ) ) ;
V_200 . V_201 = F_22 ( V_200 . V_201 ) ;
V_200 . V_202 = F_22 ( V_200 . V_202 ) ;
for ( V_132 = 0 ; V_132 < V_7 -> V_203 . V_133 ; V_132 ++ )
if ( memcmp ( V_200 . V_204 , V_7 -> V_203 . V_205 [ V_132 ] , V_107 ) == 0 )
break;
if ( V_132 >= V_7 -> V_203 . V_133 ) {
if ( V_200 . V_201 != V_206 )
F_23 ( V_1 -> V_50 ,
L_68 ) ;
} else {
V_7 -> V_203 . V_207 [ V_132 ] =
( V_200 . V_201 == V_208 ||
V_200 . V_201 == V_209 ) ;
if ( V_200 . V_201 == V_206 )
F_23 ( V_1 -> V_50 ,
L_69 ) ;
}
}
static void F_63 ( T_1 * V_1 ,
T_7 * V_128 )
{
T_2 * V_7 = ( T_2 * ) V_1 -> V_8 ;
struct V_2 * V_3 ;
V_3 = F_64 ( sizeof( * V_128 ) ) ;
if ( V_3 ) {
F_65 ( V_3 , sizeof( * V_128 ) ) ;
memcpy ( V_3 -> V_33 , V_128 , sizeof( * V_128 ) ) ;
F_66 ( & V_7 -> V_178 , V_3 ) ;
F_61 ( & V_7 -> V_176 ) ;
}
}
static void F_53 ( T_1 * V_1 ,
T_7 * V_128 )
{
T_2 * V_7 = ( T_2 * ) V_1 -> V_8 ;
T_13 V_200 ;
int V_132 , V_210 , V_9 , V_133 ;
T_5 * V_205 ;
memcpy ( V_200 . V_211 , V_128 -> V_137 . V_212 . V_204 , V_107 ) ;
V_200 . V_35 = V_213 ;
switch ( V_7 -> V_214 ) {
case V_215 :
for ( V_132 = 0 ; V_132 < V_7 -> V_203 . V_133 ; V_132 ++ )
if ( memcmp ( V_200 . V_211 , V_7 -> V_203 . V_205 [ V_132 ] ,
V_107 ) == 0 ) {
V_200 . V_35 = V_216 ;
break;
}
break;
case V_217 :
V_200 . V_35 = V_216 ;
break;
case V_218 :
if ( V_7 -> V_219 . V_220 == 0 ) {
V_133 = V_7 -> V_219 . V_133 ;
V_205 = V_7 -> V_219 . V_205 [ 0 ] ;
} else {
V_133 = V_7 -> V_219 . V_221 ;
V_205 = V_7 -> V_219 . V_222 [ 0 ] ;
}
for ( V_132 = 0 ; V_132 < V_133 ; V_132 ++ , V_205 += V_107 )
if ( memcmp ( V_200 . V_211 , V_205 , V_107 ) == 0 ) {
V_200 . V_35 = V_216 ;
break;
}
break;
case V_223 :
if ( V_7 -> V_224 . V_220 == 0 ) {
V_133 = V_7 -> V_224 . V_133 ;
V_205 = V_7 -> V_224 . V_205 [ 0 ] ;
} else {
V_133 = V_7 -> V_224 . V_221 ;
V_205 = V_7 -> V_224 . V_222 [ 0 ] ;
}
V_200 . V_35 = V_216 ;
for ( V_132 = 0 ; V_132 < V_133 ; V_132 ++ , V_205 += V_107 )
if ( memcmp ( V_200 . V_211 , V_205 , V_107 ) == 0 ) {
V_200 . V_35 = V_213 ;
break;
}
break;
}
V_210 = 0 ;
if ( V_200 . V_35 == V_216 ) {
for ( V_132 = 0 ; V_132 < V_7 -> V_203 . V_133 ; V_132 ++ )
if ( memcmp ( V_200 . V_211 , V_7 -> V_203 . V_205 [ V_132 ] , V_107 )
== 0 )
break;
if ( V_132 >= V_7 -> V_203 . V_133 ) {
if ( V_7 -> V_203 . V_133 >= V_225 ) {
V_200 . V_35 = V_226 ;
} else {
memcpy ( V_7 -> V_203 . V_205 [ V_7 -> V_203 . V_133 ] ,
V_200 . V_211 , V_107 ) ;
V_7 -> V_203 . V_133 ++ ;
V_210 = 1 ;
}
}
}
V_200 . V_35 = F_5 ( V_200 . V_35 ) ;
V_200 . V_227 = V_128 -> V_137 . V_212 . V_227 ;
V_9 = F_43 ( V_7 , V_228 ,
& V_200 , sizeof( V_200 ) ) ;
if ( V_9 ) {
if ( V_210 )
V_7 -> V_203 . V_133 -- ;
F_25 ( V_1 -> V_50 ,
L_70 ,
V_9 ) ;
}
}
static void F_67 ( T_1 * V_1 ,
T_7 * V_128 )
{
T_2 * V_7 = ( T_2 * ) V_1 -> V_8 ;
V_7 -> V_229 = F_22 ( V_128 -> V_137 . V_230 . V_231 ) ;
}
void F_68 ( T_1 * V_1 , T_7 * V_128 )
{
V_128 -> V_144 = F_22 ( V_128 -> V_144 ) ;
switch ( V_128 -> V_144 ) {
case V_232 :
F_39 ( V_1 , V_128 ) ;
break;
case V_233 :
F_40 ( V_1 , V_128 ) ;
break;
case V_234 :
F_44 ( V_1 , V_128 ) ;
break;
case V_235 :
F_42 ( V_1 , V_128 ) ;
break;
case V_236 :
F_48 ( V_1 , V_128 ) ;
break;
case V_237 :
F_60 ( V_1 , V_128 ) ;
break;
case V_238 :
F_62 ( V_1 , V_128 ) ;
break;
case V_239 :
F_63 ( V_1 , V_128 ) ;
break;
case V_240 :
F_67 ( V_1 , V_128 ) ;
break;
case V_241 :
F_23 ( V_1 -> V_50 , L_71 ) ;
break;
case V_242 :
F_23 ( V_1 -> V_50 , L_72 ) ;
break;
case V_243 :
F_23 ( V_1 -> V_50 , L_73 ) ;
break;
default:
F_23 ( V_1 -> V_50 ,
L_74 , V_128 -> V_144 ) ;
break;
}
}
void F_69 ( T_1 * V_1 , T_4 V_35 )
{
F_9 ( L_75 , V_35 ) ;
}
void F_70 ( T_1 * V_1 , T_4 V_35 )
{
F_9 ( L_76 , V_35 ) ;
V_1 -> V_50 -> V_244 . V_245 ++ ;
}
void F_71 ( T_1 * V_1 , struct V_2 * V_3 )
{
F_72 ( V_1 , V_3 ) ;
}
void F_73 ( T_1 * V_1 )
{
F_74 ( V_1 -> V_50 ) ;
}
static T_1 * F_75 ( void )
{
T_1 * V_1 = NULL ;
T_2 * V_7 = NULL ;
V_1 = F_76 ( sizeof( T_1 ) , V_246 ) ;
V_7 = F_76 ( sizeof( T_2 ) , V_246 ) ;
if ( ! V_1 || ! V_7 ) {
F_77 ( L_77 , V_247 ) ;
F_45 ( V_1 ) ;
F_45 ( V_7 ) ;
return NULL ;
}
V_1 -> V_248 = V_247 ;
V_1 -> V_52 = V_66 ;
V_1 -> V_8 = V_7 ;
V_1 -> V_249 = F_1 ;
V_1 -> V_250 = F_2 ;
V_1 -> V_251 = F_3 ;
V_1 -> V_252 = F_4 ;
V_1 -> V_253 = F_8 ;
V_1 -> V_254 = F_37 ;
V_1 -> V_255 = V_256 ;
V_1 -> V_108 = V_257 | V_258 ;
V_7 -> V_259 = 1 ;
return V_1 ;
}
void F_57 ( struct V_174 * V_33 )
{
T_2 * V_7 = F_51 ( V_33 , struct V_175 , V_193 ) ;
T_1 * V_1 = V_7 -> V_1 ;
T_11 V_177 ;
struct V_260 V_16 ;
T_14 * V_261 = ( T_14 * )
& V_16 . V_262 . V_33 ;
int V_9 = 0 ;
if ( V_7 -> V_1 -> V_67 )
return;
if ( ( V_1 -> V_68 == V_69 ) ||
( V_1 -> V_68 == V_263 ) ) {
return;
}
if ( V_1 -> V_68 != V_191 ) {
V_9 = F_31 (
V_7 , V_264 ,
& V_7 -> V_45 , V_265 ) ;
if ( V_9 ) {
F_25 ( V_1 -> V_50 , L_78 ) ;
return;
}
F_9 ( L_79 ,
F_22 ( V_7 -> V_45 . V_46 ) ,
F_22 ( V_7 -> V_45 . V_47 ) ,
F_22 ( V_7 -> V_45 . V_48 ) ) ;
}
V_16 . V_17 = V_18 ;
V_261 -> V_266 = V_267 ;
V_9 = F_78 ( V_1 , ( T_5 * ) & V_16 ) ;
if ( V_9 ) {
F_9 ( L_80 ,
V_9 ) ;
return;
}
switch ( V_261 -> V_33 ) {
case V_268 :
V_7 -> V_49 = 10 ;
break;
case V_269 :
V_7 -> V_49 = 20 ;
break;
case V_270 :
V_7 -> V_49 = 55 ;
break;
case V_271 :
V_7 -> V_49 = 110 ;
break;
default:
F_9 ( L_81 , V_261 -> V_33 ) ;
}
V_9 = F_31 ( V_7 ,
V_185 ,
V_1 -> V_154 , V_155 ) ;
if ( V_9 ) {
F_9 ( L_60 ,
V_185 , V_9 ) ;
return;
}
V_9 = F_31 ( V_7 ,
V_186 ,
& V_177 , sizeof( V_177 ) ) ;
if ( V_9 ) {
F_9 ( L_60 ,
V_186 , V_9 ) ;
return;
}
F_55 ( (struct V_187 * ) & V_177 ,
( V_188 * ) & V_1 -> V_177 ) ;
F_79 ( & V_7 -> V_272 , V_273 + V_274 ) ;
}
void F_80 ( unsigned long V_33 )
{
T_2 * V_7 = ( T_2 * ) V_33 ;
F_61 ( & V_7 -> V_193 ) ;
}
