static inline struct V_1 * F_1 ( struct V_2 * V_3 )
{
return F_2 ( V_3 , struct V_1 , V_4 ) ;
}
static inline struct V_5 * F_3 ( struct V_6 * V_6 )
{
return F_2 ( V_6 , struct V_5 , V_4 ) ;
}
static inline struct V_6 * F_4 ( struct V_2 * V_3 )
{
struct V_1 * V_7 = F_1 ( V_3 ) ;
struct V_5 * V_5 = F_5 ( V_7 ) ;
return & V_5 -> V_4 ;
}
static inline int F_6 ( struct V_2 * V_3 )
{
return ! F_7 ( & V_3 -> V_8 ) ;
}
static inline int F_8 ( struct V_1 * V_7 , struct V_6 * V_6 )
{
struct V_2 * V_3 = & V_7 -> V_4 ;
return V_6 -> V_9 == & V_3 -> V_8 ;
}
void F_9 ( struct V_10 * V_11 , T_1 V_12 , T_1 V_13 )
{
F_10 ( & V_11 -> V_14 ) ;
V_11 -> V_15 = V_12 ;
V_11 -> V_16 = V_13 ;
}
void F_11 ( struct V_17 * V_11 )
{
F_10 ( & V_11 -> V_18 ) ;
F_12 ( & V_19 . V_14 ) ;
if ( F_13 () == V_20 )
V_11 -> V_21 = - 1 ;
else
V_11 -> V_21 = F_14 ( F_15 () , F_13 () ) ;
F_16 ( & V_19 . V_14 ) ;
V_11 -> V_22 = 0 ;
}
void F_17 ( struct V_6 * V_6 , struct V_5 * V_5 )
{
V_6 -> V_23 = V_24 ;
#ifdef F_18
V_6 -> V_25 . V_26 = V_6 -> V_25 . V_27 = 0 ;
V_6 -> V_28 = 0 ;
V_6 -> V_29 = 0 ;
V_6 -> V_30 = V_24 ;
#else
F_11 ( & V_6 -> V_17 ) ;
#endif
}
static inline int F_19 ( struct V_5 * V_5 )
{
return F_20 ( & V_5 -> V_31 -> V_32 ) ;
}
static inline void F_21 ( struct V_5 * V_5 )
{
if ( ! V_5 -> V_33 )
return;
F_22 ( V_5 -> V_34 , V_5 -> V_31 -> V_35 ) ;
F_23 () ;
F_24 ( & V_5 -> V_31 -> V_32 ) ;
}
static inline void F_25 ( struct V_5 * V_5 )
{
if ( ! V_5 -> V_33 )
return;
F_26 ( & V_5 -> V_31 -> V_32 ) ;
F_27 ( V_5 -> V_34 , V_5 -> V_31 -> V_35 ) ;
}
static void F_28 ( struct V_6 * V_6 )
{
if ( V_6 -> V_28 && V_6 -> V_36 > 1 ) {
if ( ! V_6 -> V_29 ) {
F_21 ( F_3 ( V_6 ) ) ;
V_6 -> V_29 = 1 ;
}
} else if ( V_6 -> V_29 ) {
F_25 ( F_3 ( V_6 ) ) ;
V_6 -> V_29 = 0 ;
}
}
static void F_29 ( struct V_2 * V_3 , struct V_6 * V_6 )
{
struct V_1 * V_7 = F_1 ( V_3 ) ;
if ( V_7 -> V_37 > 1 )
V_6 -> V_28 ++ ;
F_28 ( V_6 ) ;
}
static void F_30 ( struct V_2 * V_3 , struct V_6 * V_6 )
{
struct V_1 * V_7 = F_1 ( V_3 ) ;
if ( V_7 -> V_37 > 1 )
V_6 -> V_28 -- ;
F_28 ( V_6 ) ;
}
static void F_31 ( struct V_5 * V_5 , struct V_1 * V_7 )
{
struct V_6 * V_6 = & V_5 -> V_4 ;
struct V_8 * * V_38 = & V_6 -> V_30 . V_8 ;
struct V_8 * V_39 = NULL ;
struct V_1 * V_40 ;
int V_41 = 1 ;
F_32 ( ! F_7 ( & V_7 -> V_42 ) ) ;
while ( * V_38 ) {
V_39 = * V_38 ;
V_40 = F_33 ( V_39 , struct V_1 ,
V_42 ) ;
if ( F_34 ( & V_7 -> V_4 , & V_40 -> V_4 ) )
V_38 = & V_39 -> V_43 ;
else {
V_38 = & V_39 -> V_44 ;
V_41 = 0 ;
}
}
if ( V_41 )
V_6 -> V_45 = & V_7 -> V_42 ;
F_35 ( & V_7 -> V_42 , V_39 , V_38 ) ;
F_36 ( & V_7 -> V_42 , & V_6 -> V_30 ) ;
}
static void F_37 ( struct V_5 * V_5 , struct V_1 * V_7 )
{
struct V_6 * V_6 = & V_5 -> V_4 ;
if ( F_7 ( & V_7 -> V_42 ) )
return;
if ( V_6 -> V_45 == & V_7 -> V_42 ) {
struct V_8 * V_46 ;
V_46 = F_38 ( & V_7 -> V_42 ) ;
V_6 -> V_45 = V_46 ;
}
F_39 ( & V_7 -> V_42 , & V_6 -> V_30 ) ;
F_40 ( & V_7 -> V_42 ) ;
}
static inline int F_41 ( struct V_5 * V_5 )
{
return ! F_42 ( & V_5 -> V_4 . V_30 ) ;
}
static inline bool F_43 ( struct V_5 * V_5 , struct V_1 * V_47 )
{
return F_44 ( V_47 ) ;
}
static inline void F_45 ( struct V_5 * V_5 )
{
V_5 -> V_48 = F_41 ( V_5 ) ;
}
static inline
void F_31 ( struct V_5 * V_5 , struct V_1 * V_7 )
{
}
static inline
void F_37 ( struct V_5 * V_5 , struct V_1 * V_7 )
{
}
static inline
void F_29 ( struct V_2 * V_3 , struct V_6 * V_6 )
{
}
static inline
void F_30 ( struct V_2 * V_3 , struct V_6 * V_6 )
{
}
static inline bool F_43 ( struct V_5 * V_5 , struct V_1 * V_47 )
{
return false ;
}
static inline int F_46 ( struct V_5 * V_5 )
{
return 0 ;
}
static inline void F_45 ( struct V_5 * V_5 )
{
}
static inline void F_47 ( struct V_2 * V_3 ,
struct V_2 * V_49 )
{
struct V_6 * V_6 = F_4 ( V_3 ) ;
struct V_5 * V_5 = F_3 ( V_6 ) ;
F_48 ( ! V_3 -> V_50 || V_3 -> V_51 ) ;
V_3 -> V_52 = F_49 ( V_5 ) + V_49 -> V_53 ;
V_3 -> V_13 = V_49 -> V_16 ;
V_3 -> V_50 = 0 ;
}
static void F_50 ( struct V_2 * V_3 ,
struct V_2 * V_49 )
{
struct V_6 * V_6 = F_4 ( V_3 ) ;
struct V_5 * V_5 = F_3 ( V_6 ) ;
F_32 ( V_49 -> V_16 <= 0 ) ;
if ( V_3 -> V_53 == 0 ) {
V_3 -> V_52 = F_49 ( V_5 ) + V_49 -> V_53 ;
V_3 -> V_13 = V_49 -> V_16 ;
}
while ( V_3 -> V_13 <= 0 ) {
V_3 -> V_52 += V_49 -> V_15 ;
V_3 -> V_13 += V_49 -> V_16 ;
}
if ( F_51 ( V_3 -> V_52 , F_49 ( V_5 ) ) ) {
F_52 ( L_1 ) ;
V_3 -> V_52 = F_49 ( V_5 ) + V_49 -> V_53 ;
V_3 -> V_13 = V_49 -> V_16 ;
}
if ( V_3 -> V_54 )
V_3 -> V_54 = 0 ;
if ( V_3 -> V_51 )
V_3 -> V_51 = 0 ;
}
static bool F_53 ( struct V_2 * V_3 ,
struct V_2 * V_49 , T_1 V_55 )
{
T_1 V_56 , V_57 ;
V_56 = ( V_49 -> V_15 >> V_58 ) * ( V_3 -> V_13 >> V_58 ) ;
V_57 = ( ( V_3 -> V_52 - V_55 ) >> V_58 ) *
( V_49 -> V_16 >> V_58 ) ;
return F_51 ( V_57 , V_56 ) ;
}
static void F_54 ( struct V_2 * V_3 ,
struct V_2 * V_49 )
{
struct V_6 * V_6 = F_4 ( V_3 ) ;
struct V_5 * V_5 = F_3 ( V_6 ) ;
if ( V_3 -> V_50 ) {
F_47 ( V_3 , V_49 ) ;
return;
}
if ( F_51 ( V_3 -> V_52 , F_49 ( V_5 ) ) ||
F_53 ( V_3 , V_49 , F_49 ( V_5 ) ) ) {
V_3 -> V_52 = F_49 ( V_5 ) + V_49 -> V_53 ;
V_3 -> V_13 = V_49 -> V_16 ;
}
}
static int F_55 ( struct V_2 * V_3 , bool V_59 )
{
struct V_6 * V_6 = F_4 ( V_3 ) ;
struct V_5 * V_5 = F_3 ( V_6 ) ;
T_2 V_60 , V_61 ;
T_2 V_62 , V_63 ;
unsigned long V_64 ;
T_3 V_65 ;
if ( V_59 )
return 0 ;
V_61 = F_56 ( V_3 -> V_52 ) ;
V_60 = F_57 ( & V_3 -> V_66 ) ;
V_65 = F_58 ( V_60 ) - F_49 ( V_5 ) ;
V_61 = F_59 ( V_61 , V_65 ) ;
if ( F_60 ( V_61 , V_60 ) < 0 )
return 0 ;
F_61 ( & V_3 -> V_66 , V_61 ) ;
V_62 = F_62 ( & V_3 -> V_66 ) ;
V_63 = F_63 ( & V_3 -> V_66 ) ;
V_64 = F_58 ( F_64 ( V_63 , V_62 ) ) ;
F_65 ( & V_3 -> V_66 , V_62 ,
V_64 , V_67 , 0 ) ;
return F_66 ( & V_3 -> V_66 ) ;
}
static enum V_68 F_67 ( struct V_69 * V_70 )
{
struct V_2 * V_3 = F_2 ( V_70 ,
struct V_2 ,
V_66 ) ;
struct V_1 * V_7 = F_1 ( V_3 ) ;
unsigned long V_71 ;
struct V_5 * V_5 ;
V_5 = F_68 ( V_72 , & V_71 ) ;
if ( ! F_44 ( V_7 ) || V_3 -> V_50 ||
V_3 -> V_73 || ! V_3 -> V_51 )
goto V_74;
F_69 () ;
F_70 ( V_5 ) ;
if ( ! F_71 ( V_7 ) ) {
F_50 ( V_3 , V_3 ) ;
goto V_74;
}
F_72 ( V_5 , V_7 , V_75 ) ;
if ( F_44 ( V_5 -> V_26 ) )
F_73 ( V_5 , V_7 , 0 ) ;
else
F_74 ( V_5 ) ;
#ifdef F_18
if ( F_41 ( V_5 ) )
F_75 ( V_5 ) ;
#endif
V_74:
F_76 ( V_5 , V_72 , & V_71 ) ;
return V_76 ;
}
void F_77 ( struct V_2 * V_3 )
{
struct V_69 * V_70 = & V_3 -> V_66 ;
F_78 ( V_70 , V_77 , V_78 ) ;
V_70 -> V_79 = F_67 ;
}
static
int F_79 ( struct V_5 * V_5 , struct V_2 * V_3 )
{
return ( V_3 -> V_13 <= 0 ) ;
}
static void F_80 ( struct V_5 * V_5 )
{
struct V_1 * V_26 = V_5 -> V_26 ;
struct V_2 * V_3 = & V_26 -> V_4 ;
T_1 V_80 ;
if ( ! F_44 ( V_26 ) || ! F_6 ( V_3 ) )
return;
V_80 = F_81 ( V_5 ) - V_26 -> V_81 . V_82 ;
if ( F_82 ( ( T_3 ) V_80 <= 0 ) )
return;
F_83 ( V_26 -> V_81 . V_83 . V_84 ,
F_84 ( V_26 -> V_81 . V_83 . V_84 , V_80 ) ) ;
V_26 -> V_81 . V_85 += V_80 ;
F_85 ( V_26 , V_80 ) ;
V_26 -> V_81 . V_82 = F_81 ( V_5 ) ;
F_86 ( V_26 , V_80 ) ;
F_87 ( V_5 , V_80 ) ;
V_3 -> V_13 -= V_3 -> V_54 ? 0 : V_80 ;
if ( F_79 ( V_5 , V_3 ) ) {
V_3 -> V_51 = 1 ;
F_88 ( V_5 , V_26 , 0 ) ;
if ( F_82 ( ! F_55 ( V_3 , V_26 -> V_4 . V_73 ) ) )
F_72 ( V_5 , V_26 , V_75 ) ;
if ( ! F_8 ( V_26 , & V_5 -> V_4 ) )
F_74 ( V_5 ) ;
}
if ( F_89 () ) {
struct V_86 * V_86 = & V_5 -> V_87 ;
F_12 ( & V_86 -> V_88 ) ;
if ( F_90 ( V_86 ) )
V_86 -> V_89 += V_80 ;
F_16 ( & V_86 -> V_88 ) ;
}
}
static inline T_1 F_91 ( struct V_5 * V_5 )
{
struct V_1 * V_27 = F_92 ( V_5 , V_5 -> V_34 ) ;
if ( V_27 && F_93 ( V_27 -> V_90 ) )
return V_27 -> V_4 . V_52 ;
else
return 0 ;
}
static void F_94 ( struct V_6 * V_6 , T_1 V_52 )
{
struct V_5 * V_5 = F_3 ( V_6 ) ;
if ( V_6 -> V_25 . V_26 == 0 ||
F_51 ( V_52 , V_6 -> V_25 . V_26 ) ) {
V_6 -> V_25 . V_27 = V_6 -> V_25 . V_26 ;
V_6 -> V_25 . V_26 = V_52 ;
F_95 ( & V_5 -> V_31 -> V_91 , V_5 -> V_34 , V_52 , 1 ) ;
} else if ( V_6 -> V_25 . V_27 == 0 ||
F_51 ( V_52 , V_6 -> V_25 . V_27 ) ) {
V_6 -> V_25 . V_27 = F_91 ( V_5 ) ;
}
}
static void F_96 ( struct V_6 * V_6 , T_1 V_52 )
{
struct V_5 * V_5 = F_3 ( V_6 ) ;
if ( ! V_6 -> V_36 ) {
V_6 -> V_25 . V_26 = 0 ;
V_6 -> V_25 . V_27 = 0 ;
F_95 ( & V_5 -> V_31 -> V_91 , V_5 -> V_34 , 0 , 0 ) ;
} else {
struct V_8 * V_41 = V_6 -> V_9 ;
struct V_2 * V_40 ;
V_40 = F_33 ( V_41 , struct V_2 , V_8 ) ;
V_6 -> V_25 . V_26 = V_40 -> V_52 ;
V_6 -> V_25 . V_27 = F_91 ( V_5 ) ;
F_95 ( & V_5 -> V_31 -> V_91 , V_5 -> V_34 , V_40 -> V_52 , 1 ) ;
}
}
static inline void F_94 ( struct V_6 * V_6 , T_1 V_52 ) {}
static inline void F_96 ( struct V_6 * V_6 , T_1 V_52 ) {}
static inline
void F_97 ( struct V_2 * V_3 , struct V_6 * V_6 )
{
int V_90 = F_1 ( V_3 ) -> V_90 ;
T_1 V_52 = V_3 -> V_52 ;
F_48 ( ! F_93 ( V_90 ) ) ;
V_6 -> V_36 ++ ;
F_98 ( F_3 ( V_6 ) , 1 ) ;
F_94 ( V_6 , V_52 ) ;
F_29 ( V_3 , V_6 ) ;
}
static inline
void F_99 ( struct V_2 * V_3 , struct V_6 * V_6 )
{
int V_90 = F_1 ( V_3 ) -> V_90 ;
F_48 ( ! F_93 ( V_90 ) ) ;
F_48 ( ! V_6 -> V_36 ) ;
V_6 -> V_36 -- ;
F_100 ( F_3 ( V_6 ) , 1 ) ;
F_96 ( V_6 , V_3 -> V_52 ) ;
F_30 ( V_3 , V_6 ) ;
}
static void F_101 ( struct V_2 * V_3 )
{
struct V_6 * V_6 = F_4 ( V_3 ) ;
struct V_8 * * V_38 = & V_6 -> V_23 . V_8 ;
struct V_8 * V_39 = NULL ;
struct V_2 * V_40 ;
int V_41 = 1 ;
F_32 ( ! F_7 ( & V_3 -> V_8 ) ) ;
while ( * V_38 ) {
V_39 = * V_38 ;
V_40 = F_33 ( V_39 , struct V_2 , V_8 ) ;
if ( F_51 ( V_3 -> V_52 , V_40 -> V_52 ) )
V_38 = & V_39 -> V_43 ;
else {
V_38 = & V_39 -> V_44 ;
V_41 = 0 ;
}
}
if ( V_41 )
V_6 -> V_9 = & V_3 -> V_8 ;
F_35 ( & V_3 -> V_8 , V_39 , V_38 ) ;
F_36 ( & V_3 -> V_8 , & V_6 -> V_23 ) ;
F_97 ( V_3 , V_6 ) ;
}
static void F_102 ( struct V_2 * V_3 )
{
struct V_6 * V_6 = F_4 ( V_3 ) ;
if ( F_7 ( & V_3 -> V_8 ) )
return;
if ( V_6 -> V_9 == & V_3 -> V_8 ) {
struct V_8 * V_46 ;
V_46 = F_38 ( & V_3 -> V_8 ) ;
V_6 -> V_9 = V_46 ;
}
F_39 ( & V_3 -> V_8 , & V_6 -> V_23 ) ;
F_40 ( & V_3 -> V_8 ) ;
F_99 ( V_3 , V_6 ) ;
}
static void
F_103 ( struct V_2 * V_3 ,
struct V_2 * V_49 , int V_71 )
{
F_32 ( F_6 ( V_3 ) ) ;
if ( V_3 -> V_50 || V_71 & V_92 )
F_54 ( V_3 , V_49 ) ;
else if ( V_71 & V_75 )
F_50 ( V_3 , V_49 ) ;
F_101 ( V_3 ) ;
}
static void F_104 ( struct V_2 * V_3 )
{
F_102 ( V_3 ) ;
}
static void F_72 ( struct V_5 * V_5 , struct V_1 * V_7 , int V_71 )
{
struct V_1 * V_93 = F_105 ( V_7 ) ;
struct V_2 * V_49 = & V_7 -> V_4 ;
if ( V_93 && V_7 -> V_4 . V_73 && F_93 ( V_93 -> V_94 ) ) {
V_49 = & V_93 -> V_4 ;
} else if ( ! F_93 ( V_7 -> V_94 ) ) {
F_32 ( ! V_7 -> V_4 . V_73 || V_71 != V_75 ) ;
return;
}
if ( V_7 -> V_4 . V_51 && ! ( V_71 & V_75 ) )
return;
F_103 ( & V_7 -> V_4 , V_49 , V_71 ) ;
if ( ! F_106 ( V_5 , V_7 ) && V_7 -> V_37 > 1 )
F_31 ( V_5 , V_7 ) ;
}
static void F_88 ( struct V_5 * V_5 , struct V_1 * V_7 , int V_71 )
{
F_104 ( & V_7 -> V_4 ) ;
F_37 ( V_5 , V_7 ) ;
}
static void F_107 ( struct V_5 * V_5 , struct V_1 * V_7 , int V_71 )
{
F_80 ( V_5 ) ;
F_88 ( V_5 , V_7 , V_71 ) ;
}
static void F_108 ( struct V_5 * V_5 )
{
struct V_1 * V_7 = V_5 -> V_26 ;
if ( V_7 -> V_4 . V_13 > 0 ) {
V_5 -> V_26 -> V_4 . V_54 = 1 ;
V_7 -> V_4 . V_13 = 0 ;
}
F_70 ( V_5 ) ;
F_80 ( V_5 ) ;
}
static int
F_109 ( struct V_1 * V_7 , int V_34 , int V_95 , int V_71 )
{
struct V_1 * V_26 ;
struct V_5 * V_5 ;
if ( V_95 != V_96 )
goto V_97;
V_5 = F_110 ( V_34 ) ;
F_111 () ;
V_26 = F_112 ( V_5 -> V_26 ) ;
if ( F_82 ( F_44 ( V_26 ) ) &&
( V_26 -> V_37 < 2 ||
! F_34 ( & V_7 -> V_4 , & V_26 -> V_4 ) ) &&
( V_7 -> V_37 > 1 ) ) {
int V_98 = F_113 ( V_7 ) ;
if ( V_98 != - 1 )
V_34 = V_98 ;
}
F_114 () ;
V_97:
return V_34 ;
}
static void F_115 ( struct V_5 * V_5 , struct V_1 * V_7 )
{
if ( V_5 -> V_26 -> V_37 == 1 ||
F_116 ( & V_5 -> V_31 -> V_91 , V_5 -> V_26 , NULL ) == - 1 )
return;
if ( V_7 -> V_37 != 1 &&
F_116 ( & V_5 -> V_31 -> V_91 , V_7 , NULL ) != - 1 )
return;
F_74 ( V_5 ) ;
}
static void F_73 ( struct V_5 * V_5 , struct V_1 * V_7 ,
int V_71 )
{
if ( F_34 ( & V_7 -> V_4 , & V_5 -> V_26 -> V_4 ) ) {
F_74 ( V_5 ) ;
return;
}
#ifdef F_18
if ( ( V_7 -> V_4 . V_52 == V_5 -> V_26 -> V_4 . V_52 ) &&
! F_117 ( V_5 -> V_26 ) )
F_115 ( V_5 , V_7 ) ;
#endif
}
static void F_118 ( struct V_5 * V_5 , struct V_1 * V_7 )
{
F_119 ( V_5 , V_7 -> V_4 . V_13 ) ;
}
static void F_118 ( struct V_5 * V_5 , struct V_1 * V_7 )
{
}
static struct V_2 * F_120 ( struct V_5 * V_5 ,
struct V_6 * V_6 )
{
struct V_8 * V_56 = V_6 -> V_9 ;
if ( ! V_56 )
return NULL ;
return F_33 ( V_56 , struct V_2 , V_8 ) ;
}
struct V_1 * F_121 ( struct V_5 * V_5 , struct V_1 * V_47 )
{
struct V_2 * V_3 ;
struct V_1 * V_7 ;
struct V_6 * V_6 ;
V_6 = & V_5 -> V_4 ;
if ( F_43 ( V_5 , V_47 ) ) {
F_46 ( V_5 ) ;
if ( V_5 -> V_99 && F_71 ( V_5 -> V_99 ) )
return V_100 ;
}
if ( V_47 -> V_101 == & V_102 )
F_80 ( V_5 ) ;
if ( F_82 ( ! V_6 -> V_36 ) )
return NULL ;
F_122 ( V_5 , V_47 ) ;
V_3 = F_120 ( V_5 , V_6 ) ;
F_32 ( ! V_3 ) ;
V_7 = F_1 ( V_3 ) ;
V_7 -> V_81 . V_82 = F_81 ( V_5 ) ;
F_37 ( V_5 , V_7 ) ;
if ( F_123 ( V_5 ) )
F_118 ( V_5 , V_7 ) ;
F_45 ( V_5 ) ;
return V_7 ;
}
static void F_124 ( struct V_5 * V_5 , struct V_1 * V_7 )
{
F_80 ( V_5 ) ;
if ( F_6 ( & V_7 -> V_4 ) && V_7 -> V_37 > 1 )
F_31 ( V_5 , V_7 ) ;
}
static void F_125 ( struct V_5 * V_5 , struct V_1 * V_7 , int V_103 )
{
F_80 ( V_5 ) ;
if ( F_123 ( V_5 ) && V_103 && V_7 -> V_4 . V_13 > 0 &&
F_8 ( V_7 , & V_5 -> V_4 ) )
F_118 ( V_5 , V_7 ) ;
}
static void F_126 ( struct V_1 * V_7 )
{
}
static void F_127 ( struct V_1 * V_7 )
{
struct V_69 * V_70 = & V_7 -> V_4 . V_66 ;
struct V_17 * V_11 = F_128 ( F_129 ( V_7 ) ) ;
F_130 ( & V_11 -> V_18 ) ;
V_11 -> V_22 -= V_7 -> V_4 . V_17 ;
F_131 ( & V_11 -> V_18 ) ;
F_132 ( V_70 ) ;
}
static void F_133 ( struct V_5 * V_5 )
{
struct V_1 * V_7 = V_5 -> V_26 ;
V_7 -> V_81 . V_82 = F_81 ( V_5 ) ;
F_37 ( V_5 , V_7 ) ;
}
static int F_134 ( struct V_5 * V_5 , struct V_1 * V_7 , int V_34 )
{
if ( ! F_135 ( V_5 , V_7 ) &&
F_136 ( V_34 , F_137 ( V_7 ) ) )
return 1 ;
return 0 ;
}
static struct V_1 * F_92 ( struct V_5 * V_5 , int V_34 )
{
struct V_8 * V_46 = V_5 -> V_4 . V_9 ;
struct V_2 * V_3 ;
struct V_1 * V_7 = NULL ;
V_46:
V_46 = F_38 ( V_46 ) ;
if ( V_46 ) {
V_3 = F_33 ( V_46 , struct V_2 , V_8 ) ;
V_7 = F_1 ( V_3 ) ;
if ( F_134 ( V_5 , V_7 , V_34 ) )
return V_7 ;
goto V_46;
}
return NULL ;
}
static int F_113 ( struct V_1 * V_104 )
{
struct V_105 * V_106 ;
struct V_107 * V_108 = F_138 ( V_109 ) ;
int V_110 = F_139 () ;
int V_111 , V_34 = F_129 ( V_104 ) ;
if ( F_82 ( ! V_108 ) )
return - 1 ;
if ( V_104 -> V_37 == 1 )
return - 1 ;
V_111 = F_116 ( & F_5 ( V_104 ) -> V_31 -> V_91 ,
V_104 , V_108 ) ;
if ( V_111 == - 1 )
return - 1 ;
if ( F_136 ( V_34 , V_108 ) )
return V_34 ;
if ( ! F_136 ( V_110 , V_108 ) )
V_110 = - 1 ;
F_111 () ;
F_140 (cpu, sd) {
if ( V_106 -> V_71 & V_112 ) {
if ( V_110 != - 1 &&
F_136 ( V_110 , F_141 ( V_106 ) ) ) {
F_114 () ;
return V_110 ;
}
if ( V_111 < V_113 &&
F_136 ( V_111 , F_141 ( V_106 ) ) ) {
F_114 () ;
return V_111 ;
}
}
}
F_114 () ;
if ( V_110 != - 1 )
return V_110 ;
V_34 = F_142 ( V_108 ) ;
if ( V_34 < V_113 )
return V_34 ;
return - 1 ;
}
static struct V_5 * F_143 ( struct V_1 * V_104 , struct V_5 * V_5 )
{
struct V_5 * V_114 = NULL ;
int V_115 ;
int V_34 ;
for ( V_115 = 0 ; V_115 < V_116 ; V_115 ++ ) {
V_34 = F_113 ( V_104 ) ;
if ( ( V_34 == - 1 ) || ( V_34 == V_5 -> V_34 ) )
break;
V_114 = F_110 ( V_34 ) ;
if ( F_144 ( V_5 , V_114 ) ) {
if ( F_82 ( F_5 ( V_104 ) != V_5 ||
! F_136 ( V_114 -> V_34 ,
& V_104 -> V_117 ) ||
F_135 ( V_5 , V_104 ) ||
! F_71 ( V_104 ) ) ) {
F_145 ( V_5 , V_114 ) ;
V_114 = NULL ;
break;
}
}
if ( ! V_114 -> V_4 . V_36 ||
F_51 ( V_104 -> V_4 . V_52 ,
V_114 -> V_4 . V_25 . V_26 ) )
break;
F_145 ( V_5 , V_114 ) ;
V_114 = NULL ;
}
return V_114 ;
}
static struct V_1 * F_146 ( struct V_5 * V_5 )
{
struct V_1 * V_7 ;
if ( ! F_41 ( V_5 ) )
return NULL ;
V_7 = F_33 ( V_5 -> V_4 . V_45 ,
struct V_1 , V_42 ) ;
F_32 ( V_5 -> V_34 != F_129 ( V_7 ) ) ;
F_32 ( F_106 ( V_5 , V_7 ) ) ;
F_32 ( V_7 -> V_37 <= 1 ) ;
F_32 ( ! F_71 ( V_7 ) ) ;
F_32 ( ! F_44 ( V_7 ) ) ;
return V_7 ;
}
static int F_75 ( struct V_5 * V_5 )
{
struct V_1 * V_118 ;
struct V_5 * V_114 ;
int V_119 = 0 ;
if ( ! V_5 -> V_4 . V_29 )
return 0 ;
V_118 = F_146 ( V_5 ) ;
if ( ! V_118 )
return 0 ;
V_120:
if ( F_82 ( V_118 == V_5 -> V_26 ) ) {
F_48 ( 1 ) ;
return 0 ;
}
if ( F_44 ( V_5 -> V_26 ) &&
F_51 ( V_118 -> V_4 . V_52 , V_5 -> V_26 -> V_4 . V_52 ) &&
V_5 -> V_26 -> V_37 > 1 ) {
F_74 ( V_5 ) ;
return 0 ;
}
F_147 ( V_118 ) ;
V_114 = F_143 ( V_118 , V_5 ) ;
if ( ! V_114 ) {
struct V_1 * V_104 ;
V_104 = F_146 ( V_5 ) ;
if ( F_129 ( V_118 ) == V_5 -> V_34 && V_104 == V_118 ) {
goto V_97;
}
if ( ! V_104 )
goto V_97;
F_148 ( V_118 ) ;
V_118 = V_104 ;
goto V_120;
}
F_149 ( V_5 , V_118 , 0 ) ;
F_150 ( V_118 , V_114 -> V_34 ) ;
F_151 ( V_114 , V_118 , 0 ) ;
V_119 = 1 ;
F_74 ( V_114 ) ;
F_145 ( V_5 , V_114 ) ;
V_97:
F_148 ( V_118 ) ;
return V_119 ;
}
static void F_152 ( struct V_5 * V_5 )
{
while ( F_75 ( V_5 ) )
;
}
static int F_46 ( struct V_5 * V_121 )
{
int V_110 = V_121 -> V_34 , V_119 = 0 , V_34 ;
struct V_1 * V_7 ;
struct V_5 * V_122 ;
T_1 V_123 = V_124 ;
if ( F_153 ( ! F_19 ( V_121 ) ) )
return 0 ;
F_154 () ;
F_155 (cpu, this_rq->rd->dlo_mask) {
if ( V_110 == V_34 )
continue;
V_122 = F_110 ( V_34 ) ;
if ( V_121 -> V_4 . V_36 &&
F_51 ( V_121 -> V_4 . V_25 . V_26 ,
V_122 -> V_4 . V_25 . V_27 ) )
continue;
F_144 ( V_121 , V_122 ) ;
if ( V_122 -> V_4 . V_36 <= 1 )
goto V_125;
V_7 = F_92 ( V_122 , V_110 ) ;
if ( V_7 && F_51 ( V_7 -> V_4 . V_52 , V_123 ) &&
( ! V_121 -> V_4 . V_36 ||
F_51 ( V_7 -> V_4 . V_52 ,
V_121 -> V_4 . V_25 . V_26 ) ) ) {
F_48 ( V_7 == V_122 -> V_26 ) ;
F_48 ( ! F_71 ( V_7 ) ) ;
if ( F_51 ( V_7 -> V_4 . V_52 ,
V_122 -> V_26 -> V_4 . V_52 ) )
goto V_125;
V_119 = 1 ;
F_149 ( V_122 , V_7 , 0 ) ;
F_150 ( V_7 , V_110 ) ;
F_151 ( V_121 , V_7 , 0 ) ;
V_123 = V_7 -> V_4 . V_52 ;
}
V_125:
F_145 ( V_121 , V_122 ) ;
}
return V_119 ;
}
static void F_156 ( struct V_5 * V_5 )
{
F_152 ( V_5 ) ;
}
static void F_157 ( struct V_5 * V_5 , struct V_1 * V_7 )
{
if ( ! F_135 ( V_5 , V_7 ) &&
! F_117 ( V_5 -> V_26 ) &&
F_41 ( V_5 ) &&
V_7 -> V_37 > 1 &&
F_44 ( V_5 -> V_26 ) &&
( V_5 -> V_26 -> V_37 < 2 ||
! F_34 ( & V_7 -> V_4 , & V_5 -> V_26 -> V_4 ) ) ) {
F_152 ( V_5 ) ;
}
}
static void F_158 ( struct V_1 * V_7 ,
const struct V_107 * V_126 )
{
struct V_5 * V_5 ;
struct V_127 * V_128 ;
int V_129 ;
F_32 ( ! F_44 ( V_7 ) ) ;
V_5 = F_5 ( V_7 ) ;
V_128 = V_5 -> V_31 ;
if ( ! F_159 ( V_128 -> V_130 , V_126 ) ) {
struct V_17 * V_131 ;
V_131 = F_128 ( F_160 ( V_5 ) ) ;
F_12 ( & V_131 -> V_18 ) ;
F_161 ( V_131 , V_7 -> V_4 . V_17 ) ;
F_16 ( & V_131 -> V_18 ) ;
}
if ( ! F_6 ( & V_7 -> V_4 ) )
return;
V_129 = F_162 ( V_126 ) ;
if ( ( V_7 -> V_37 > 1 ) == ( V_129 > 1 ) )
return;
if ( V_129 <= 1 ) {
if ( ! F_106 ( V_5 , V_7 ) )
F_37 ( V_5 , V_7 ) ;
F_32 ( ! V_5 -> V_4 . V_28 ) ;
V_5 -> V_4 . V_28 -- ;
} else {
if ( ! F_106 ( V_5 , V_7 ) )
F_31 ( V_5 , V_7 ) ;
V_5 -> V_4 . V_28 ++ ;
}
F_28 ( & V_5 -> V_4 ) ;
}
static void F_163 ( struct V_5 * V_5 )
{
if ( V_5 -> V_4 . V_29 )
F_21 ( V_5 ) ;
F_164 ( & V_5 -> V_31 -> V_91 , V_5 -> V_34 ) ;
if ( V_5 -> V_4 . V_36 > 0 )
F_95 ( & V_5 -> V_31 -> V_91 , V_5 -> V_34 , V_5 -> V_4 . V_25 . V_26 , 1 ) ;
}
static void F_165 ( struct V_5 * V_5 )
{
if ( V_5 -> V_4 . V_29 )
F_25 ( V_5 ) ;
F_95 ( & V_5 -> V_31 -> V_91 , V_5 -> V_34 , 0 , 0 ) ;
F_166 ( & V_5 -> V_31 -> V_91 , V_5 -> V_34 ) ;
}
void F_167 ( void )
{
unsigned int V_132 ;
F_168 (i)
F_169 ( & F_170 ( V_109 , V_132 ) ,
V_133 , F_171 ( V_132 ) ) ;
}
static void F_172 ( struct V_5 * V_5 , struct V_1 * V_7 )
{
struct V_69 * V_66 = & V_7 -> V_4 . V_66 ;
F_173 ( & V_7 -> V_134 ) ;
if ( F_66 ( V_66 ) ) {
int V_119 = F_174 ( V_66 ) ;
if ( F_82 ( V_119 == - 1 ) ) {
F_16 ( & V_5 -> V_18 ) ;
F_132 ( V_66 ) ;
F_12 ( & V_5 -> V_18 ) ;
}
}
}
static void F_175 ( struct V_5 * V_5 , struct V_1 * V_7 )
{
F_172 ( V_5 , V_7 ) ;
F_176 ( V_7 ) ;
if ( ! F_71 ( V_7 ) || V_5 -> V_4 . V_36 )
return;
if ( F_46 ( V_5 ) )
F_74 ( V_5 ) ;
}
static void F_177 ( struct V_5 * V_5 , struct V_1 * V_7 )
{
int V_135 = 1 ;
if ( F_82 ( V_7 -> V_4 . V_51 ) )
return;
if ( F_71 ( V_7 ) && V_5 -> V_26 != V_7 ) {
#ifdef F_18
if ( V_7 -> V_37 > 1 && V_5 -> V_4 . V_29 &&
F_75 ( V_5 ) && V_5 != F_5 ( V_7 ) )
V_135 = 0 ;
#endif
if ( V_135 ) {
if ( F_44 ( V_5 -> V_26 ) )
F_73 ( V_5 , V_7 , 0 ) ;
else
F_74 ( V_5 ) ;
}
}
}
static void F_178 ( struct V_5 * V_5 , struct V_1 * V_7 ,
int V_136 )
{
if ( F_71 ( V_7 ) || V_5 -> V_26 == V_7 ) {
#ifdef F_18
if ( ! V_5 -> V_4 . V_29 )
F_46 ( V_5 ) ;
if ( F_51 ( V_5 -> V_4 . V_25 . V_26 , V_7 -> V_4 . V_52 ) &&
V_5 -> V_26 == V_7 )
F_74 ( V_5 ) ;
#else
F_74 ( V_5 ) ;
#endif
} else
F_177 ( V_5 , V_7 ) ;
}
void F_179 ( struct V_137 * V_138 , int V_34 )
{
F_180 ( V_138 , V_34 , & F_110 ( V_34 ) -> V_4 ) ;
}
