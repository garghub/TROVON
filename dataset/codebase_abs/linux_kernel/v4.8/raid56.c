int F_1 ( struct V_1 * V_2 )
{
struct V_3 * V_4 ;
struct V_3 * V_5 ;
struct V_6 * V_7 ;
struct V_6 * V_8 ;
int V_9 = 1 << V_10 ;
int V_11 ;
int V_12 ;
if ( V_2 -> V_13 )
return 0 ;
V_12 = sizeof( * V_4 ) + sizeof( * V_8 ) * V_9 ;
V_4 = F_2 ( V_12 , V_14 | V_15 | V_16 ) ;
if ( ! V_4 ) {
V_4 = F_3 ( V_12 ) ;
if ( ! V_4 )
return - V_17 ;
}
F_4 ( & V_4 -> V_18 ) ;
F_5 ( & V_4 -> V_19 ) ;
V_8 = V_4 -> V_4 ;
for ( V_11 = 0 ; V_11 < V_9 ; V_11 ++ ) {
V_7 = V_8 + V_11 ;
F_5 ( & V_7 -> V_20 ) ;
F_4 ( & V_7 -> V_21 ) ;
F_6 ( & V_7 -> V_22 ) ;
}
V_5 = F_7 ( & V_2 -> V_13 , NULL , V_4 ) ;
if ( V_5 )
F_8 ( V_5 ) ;
return 0 ;
}
static void F_9 ( struct V_23 * V_24 )
{
int V_11 ;
char * V_25 ;
char * V_26 ;
int V_27 ;
V_27 = F_10 ( V_24 ) ;
if ( V_27 )
return;
for ( V_11 = 0 ; V_11 < V_24 -> V_28 ; V_11 ++ ) {
if ( ! V_24 -> V_29 [ V_11 ] )
continue;
V_25 = F_11 ( V_24 -> V_29 [ V_11 ] ) ;
V_26 = F_11 ( V_24 -> V_30 [ V_11 ] ) ;
memcpy ( V_26 , V_25 , V_31 ) ;
F_12 ( V_24 -> V_29 [ V_11 ] ) ;
F_12 ( V_24 -> V_30 [ V_11 ] ) ;
F_13 ( V_24 -> V_30 [ V_11 ] ) ;
}
F_14 ( V_32 , & V_24 -> V_33 ) ;
}
static int F_15 ( struct V_23 * V_24 )
{
T_1 V_34 = V_24 -> V_35 -> V_36 [ 0 ] ;
return F_16 ( V_34 >> 16 , V_10 ) ;
}
static void F_17 ( struct V_23 * V_37 , struct V_23 * V_38 )
{
int V_11 ;
struct V_39 * V_25 ;
struct V_39 * V_26 ;
if ( ! F_18 ( V_32 , & V_37 -> V_33 ) )
return;
for ( V_11 = 0 ; V_11 < V_38 -> V_28 ; V_11 ++ ) {
V_25 = V_37 -> V_30 [ V_11 ] ;
if ( ! V_25 || ! F_19 ( V_25 ) ) {
continue;
}
V_26 = V_38 -> V_30 [ V_11 ] ;
if ( V_26 )
F_20 ( V_26 ) ;
V_38 -> V_30 [ V_11 ] = V_25 ;
V_37 -> V_30 [ V_11 ] = NULL ;
}
}
static void F_21 ( struct V_23 * V_38 ,
struct V_23 * V_40 )
{
F_22 ( & V_38 -> V_41 , & V_40 -> V_41 ) ;
V_38 -> V_42 += V_40 -> V_42 ;
V_38 -> V_43 += V_40 -> V_43 ;
F_23 ( & V_40 -> V_41 ) ;
}
static void F_24 ( struct V_23 * V_24 )
{
int V_44 = F_15 ( V_24 ) ;
struct V_3 * V_4 ;
struct V_6 * V_8 ;
int V_45 = 0 ;
if ( ! F_18 ( V_46 , & V_24 -> V_33 ) )
return;
V_4 = V_24 -> V_47 -> V_13 ;
V_8 = V_4 -> V_4 + V_44 ;
F_25 ( & V_8 -> V_21 ) ;
F_25 ( & V_24 -> V_48 ) ;
if ( F_26 ( V_46 , & V_24 -> V_33 ) ) {
F_27 ( & V_24 -> V_19 ) ;
V_4 -> V_49 -= 1 ;
V_45 = 1 ;
if ( F_28 ( & V_24 -> V_41 ) ) {
if ( ! F_29 ( & V_24 -> V_20 ) ) {
F_27 ( & V_24 -> V_20 ) ;
F_30 ( & V_24 -> V_50 ) ;
F_31 ( ! F_29 ( & V_24 -> V_51 ) ) ;
}
}
}
F_32 ( & V_24 -> V_48 ) ;
F_32 ( & V_8 -> V_21 ) ;
if ( V_45 )
F_33 ( V_24 ) ;
}
static void F_34 ( struct V_23 * V_24 )
{
struct V_3 * V_4 ;
unsigned long V_33 ;
if ( ! F_18 ( V_46 , & V_24 -> V_33 ) )
return;
V_4 = V_24 -> V_47 -> V_13 ;
F_35 ( & V_4 -> V_18 , V_33 ) ;
F_24 ( V_24 ) ;
F_36 ( & V_4 -> V_18 , V_33 ) ;
}
static void F_37 ( struct V_1 * V_2 )
{
struct V_3 * V_4 ;
unsigned long V_33 ;
struct V_23 * V_24 ;
V_4 = V_2 -> V_13 ;
F_35 ( & V_4 -> V_18 , V_33 ) ;
while ( ! F_29 ( & V_4 -> V_19 ) ) {
V_24 = F_38 ( V_4 -> V_19 . V_52 ,
struct V_23 ,
V_19 ) ;
F_24 ( V_24 ) ;
}
F_36 ( & V_4 -> V_18 , V_33 ) ;
}
void F_39 ( struct V_1 * V_2 )
{
if ( ! V_2 -> V_13 )
return;
F_37 ( V_2 ) ;
F_8 ( V_2 -> V_13 ) ;
V_2 -> V_13 = NULL ;
}
static void F_40 ( struct V_23 * V_24 )
{
struct V_3 * V_4 ;
unsigned long V_33 ;
if ( ! F_18 ( V_32 , & V_24 -> V_33 ) )
return;
V_4 = V_24 -> V_47 -> V_13 ;
F_35 ( & V_4 -> V_18 , V_33 ) ;
F_25 ( & V_24 -> V_48 ) ;
if ( ! F_41 ( V_46 , & V_24 -> V_33 ) )
F_42 ( & V_24 -> V_50 ) ;
if ( ! F_29 ( & V_24 -> V_19 ) ) {
F_43 ( & V_24 -> V_19 , & V_4 -> V_19 ) ;
} else {
F_44 ( & V_24 -> V_19 , & V_4 -> V_19 ) ;
V_4 -> V_49 += 1 ;
}
F_32 ( & V_24 -> V_48 ) ;
if ( V_4 -> V_49 > V_53 ) {
struct V_23 * V_54 ;
V_54 = F_38 ( V_4 -> V_19 . V_55 ,
struct V_23 ,
V_19 ) ;
if ( V_54 != V_24 )
F_24 ( V_54 ) ;
}
F_36 ( & V_4 -> V_18 , V_33 ) ;
}
static void F_45 ( void * * V_56 , int V_57 , T_2 V_58 )
{
int V_59 = 0 ;
int V_60 = 0 ;
void * V_38 = V_56 [ V_57 ] ;
while( V_57 > 0 ) {
V_60 = F_46 ( V_57 , V_61 ) ;
F_47 ( V_60 , V_58 , V_38 , V_56 + V_59 ) ;
V_57 -= V_60 ;
V_59 += V_60 ;
}
}
static int F_48 ( struct V_23 * V_24 )
{
unsigned long V_62 = V_24 -> V_42 ;
int V_27 = 1 ;
if ( V_62 != V_24 -> V_63 * V_24 -> V_64 )
V_27 = 0 ;
F_31 ( V_62 > V_24 -> V_63 * V_24 -> V_64 ) ;
return V_27 ;
}
static int F_49 ( struct V_23 * V_24 )
{
unsigned long V_33 ;
int V_27 ;
F_35 ( & V_24 -> V_48 , V_33 ) ;
V_27 = F_48 ( V_24 ) ;
F_36 ( & V_24 -> V_48 , V_33 ) ;
return V_27 ;
}
static int F_50 ( struct V_23 * V_65 ,
struct V_23 * V_7 )
{
if ( F_18 ( V_66 , & V_65 -> V_33 ) ||
F_18 ( V_66 , & V_7 -> V_33 ) )
return 0 ;
if ( F_18 ( V_46 , & V_65 -> V_33 ) ||
F_18 ( V_46 , & V_7 -> V_33 ) )
return 0 ;
if ( V_65 -> V_35 -> V_36 [ 0 ] !=
V_7 -> V_35 -> V_36 [ 0 ] )
return 0 ;
if ( V_65 -> V_67 != V_7 -> V_67 )
return 0 ;
if ( V_65 -> V_67 == V_68 ||
V_7 -> V_67 == V_68 )
return 0 ;
if ( V_65 -> V_67 == V_69 ||
V_7 -> V_67 == V_69 )
return 0 ;
return 1 ;
}
static int F_51 ( struct V_23 * V_24 , int V_70 ,
int V_71 )
{
return V_70 * V_24 -> V_72 + V_71 ;
}
static struct V_39 * F_52 ( struct V_23 * V_24 , int V_70 ,
int V_71 )
{
return V_24 -> V_30 [ F_51 ( V_24 , V_70 , V_71 ) ] ;
}
static struct V_39 * F_53 ( struct V_23 * V_24 , int V_71 )
{
return F_52 ( V_24 , V_24 -> V_63 , V_71 ) ;
}
static struct V_39 * F_54 ( struct V_23 * V_24 , int V_71 )
{
if ( V_24 -> V_63 + 1 == V_24 -> V_73 )
return NULL ;
return F_52 ( V_24 , V_24 -> V_63 + 1 , V_71 ) ;
}
static T_3 int F_55 ( struct V_23 * V_24 )
{
int V_44 = F_15 ( V_24 ) ;
struct V_6 * V_8 = V_24 -> V_47 -> V_13 -> V_4 + V_44 ;
struct V_23 * V_7 ;
struct V_23 * V_74 ;
unsigned long V_33 ;
F_56 ( V_22 ) ;
struct V_23 * V_45 = NULL ;
struct V_23 * V_75 = NULL ;
int V_27 = 0 ;
int V_76 = 0 ;
F_35 ( & V_8 -> V_21 , V_33 ) ;
F_57 (cur, &h->hash_list, hash_list) {
V_76 ++ ;
if ( V_7 -> V_35 -> V_36 [ 0 ] == V_24 -> V_35 -> V_36 [ 0 ] ) {
F_25 ( & V_7 -> V_48 ) ;
if ( F_28 ( & V_7 -> V_41 ) &&
F_29 ( & V_7 -> V_51 ) &&
F_18 ( V_46 , & V_7 -> V_33 ) &&
! F_18 ( V_66 , & V_7 -> V_33 ) ) {
F_27 ( & V_7 -> V_20 ) ;
F_30 ( & V_7 -> V_50 ) ;
F_17 ( V_7 , V_24 ) ;
V_75 = V_7 ;
F_32 ( & V_7 -> V_48 ) ;
goto V_77;
}
if ( F_50 ( V_7 , V_24 ) ) {
F_21 ( V_7 , V_24 ) ;
F_32 ( & V_7 -> V_48 ) ;
V_45 = V_24 ;
V_27 = 1 ;
goto V_78;
}
F_57 (pending, &cur->plug_list,
plug_list) {
if ( F_50 ( V_74 , V_24 ) ) {
F_21 ( V_74 , V_24 ) ;
F_32 ( & V_7 -> V_48 ) ;
V_45 = V_24 ;
V_27 = 1 ;
goto V_78;
}
}
F_58 ( & V_24 -> V_51 , & V_7 -> V_51 ) ;
F_32 ( & V_7 -> V_48 ) ;
V_27 = 1 ;
goto V_78;
}
}
V_77:
F_42 ( & V_24 -> V_50 ) ;
F_44 ( & V_24 -> V_20 , & V_8 -> V_20 ) ;
V_78:
F_36 ( & V_8 -> V_21 , V_33 ) ;
if ( V_75 )
F_34 ( V_75 ) ;
if ( V_45 )
F_33 ( V_45 ) ;
return V_27 ;
}
static T_3 void F_59 ( struct V_23 * V_24 )
{
int V_44 ;
struct V_6 * V_8 ;
unsigned long V_33 ;
int V_79 = 0 ;
V_44 = F_15 ( V_24 ) ;
V_8 = V_24 -> V_47 -> V_13 -> V_4 + V_44 ;
if ( F_29 ( & V_24 -> V_51 ) )
F_40 ( V_24 ) ;
F_35 ( & V_8 -> V_21 , V_33 ) ;
F_25 ( & V_24 -> V_48 ) ;
if ( ! F_29 ( & V_24 -> V_20 ) ) {
if ( F_29 ( & V_24 -> V_51 ) &&
F_18 ( V_46 , & V_24 -> V_33 ) ) {
V_79 = 1 ;
F_60 ( V_66 , & V_24 -> V_33 ) ;
F_31 ( ! F_28 ( & V_24 -> V_41 ) ) ;
goto V_80;
}
F_27 ( & V_24 -> V_20 ) ;
F_30 ( & V_24 -> V_50 ) ;
if ( ! F_29 ( & V_24 -> V_51 ) ) {
struct V_23 * V_52 ;
struct V_81 * V_82 = V_24 -> V_51 . V_52 ;
V_52 = F_38 ( V_82 , struct V_23 ,
V_51 ) ;
F_27 ( & V_24 -> V_51 ) ;
F_44 ( & V_52 -> V_20 , & V_8 -> V_20 ) ;
F_42 ( & V_52 -> V_50 ) ;
F_32 ( & V_24 -> V_48 ) ;
F_36 ( & V_8 -> V_21 , V_33 ) ;
if ( V_52 -> V_67 == V_83 )
F_61 ( V_52 ) ;
else if ( V_52 -> V_67 == V_69 ) {
F_17 ( V_24 , V_52 ) ;
F_61 ( V_52 ) ;
} else if ( V_52 -> V_67 == V_84 ) {
F_17 ( V_24 , V_52 ) ;
F_62 ( V_52 ) ;
} else if ( V_52 -> V_67 == V_68 ) {
F_17 ( V_24 , V_52 ) ;
F_63 ( V_52 ) ;
}
goto V_85;
} else if ( F_64 ( & V_8 -> V_22 ) ) {
F_32 ( & V_24 -> V_48 ) ;
F_36 ( & V_8 -> V_21 , V_33 ) ;
F_65 ( & V_8 -> V_22 ) ;
goto V_85;
}
}
V_80:
F_32 ( & V_24 -> V_48 ) ;
F_36 ( & V_8 -> V_21 , V_33 ) ;
V_85:
if ( ! V_79 )
F_34 ( V_24 ) ;
}
static void F_33 ( struct V_23 * V_24 )
{
int V_11 ;
F_66 ( F_67 ( & V_24 -> V_50 ) < 0 ) ;
if ( ! F_68 ( & V_24 -> V_50 ) )
return;
F_66 ( ! F_29 ( & V_24 -> V_19 ) ) ;
F_66 ( ! F_29 ( & V_24 -> V_20 ) ) ;
F_66 ( ! F_28 ( & V_24 -> V_41 ) ) ;
for ( V_11 = 0 ; V_11 < V_24 -> V_28 ; V_11 ++ ) {
if ( V_24 -> V_30 [ V_11 ] ) {
F_20 ( V_24 -> V_30 [ V_11 ] ) ;
V_24 -> V_30 [ V_11 ] = NULL ;
}
}
F_69 ( V_24 -> V_35 ) ;
F_70 ( V_24 ) ;
}
static void F_71 ( struct V_23 * V_24 )
{
F_59 ( V_24 ) ;
F_33 ( V_24 ) ;
}
static void F_72 ( struct V_23 * V_24 , int V_86 )
{
struct V_87 * V_7 = F_73 ( & V_24 -> V_41 ) ;
struct V_87 * V_52 ;
if ( V_24 -> V_43 )
F_74 ( V_24 -> V_47 , V_24 -> V_43 ) ;
F_71 ( V_24 ) ;
while ( V_7 ) {
V_52 = V_7 -> V_88 ;
V_7 -> V_88 = NULL ;
V_7 -> V_89 = V_86 ;
F_75 ( V_7 ) ;
V_7 = V_52 ;
}
}
static void F_76 ( struct V_87 * V_87 )
{
struct V_23 * V_24 = V_87 -> V_90 ;
int V_86 = V_87 -> V_89 ;
int V_91 ;
if ( V_86 )
F_77 ( V_24 , V_87 ) ;
F_78 ( V_87 ) ;
if ( ! F_68 ( & V_24 -> V_92 ) )
return;
V_86 = 0 ;
V_91 = ( V_24 -> V_67 == V_68 ) ?
0 : V_24 -> V_35 -> V_91 ;
if ( F_67 ( & V_24 -> error ) > V_91 )
V_86 = - V_93 ;
F_72 ( V_24 , V_86 ) ;
}
static struct V_39 * F_79 ( struct V_23 * V_24 ,
int V_71 , int V_94 , int V_95 )
{
int V_96 ;
struct V_39 * V_97 = NULL ;
V_96 = V_71 * ( V_24 -> V_64 >> V_98 ) + V_94 ;
F_80 ( & V_24 -> V_48 ) ;
V_97 = V_24 -> V_29 [ V_96 ] ;
F_81 ( & V_24 -> V_48 ) ;
if ( V_97 || V_95 )
return V_97 ;
return V_24 -> V_30 [ V_96 ] ;
}
static unsigned long F_82 ( unsigned long V_64 , int V_99 )
{
return F_83 ( V_64 , V_31 ) * V_99 ;
}
static struct V_23 * F_84 ( struct V_100 * V_101 ,
struct V_102 * V_35 , T_1 V_64 )
{
struct V_23 * V_24 ;
int V_63 = 0 ;
int V_73 = V_35 -> V_103 - V_35 -> V_104 ;
int V_105 = F_82 ( V_64 , V_73 ) ;
int V_72 = F_83 ( V_64 , V_31 ) ;
void * V_97 ;
V_24 = F_2 ( sizeof( * V_24 ) + V_105 * sizeof( struct V_39 * ) * 2 +
F_83 ( V_72 , V_106 ) *
sizeof( long ) , V_107 ) ;
if ( ! V_24 )
return F_85 ( - V_17 ) ;
F_23 ( & V_24 -> V_41 ) ;
F_5 ( & V_24 -> V_51 ) ;
F_4 ( & V_24 -> V_48 ) ;
F_5 ( & V_24 -> V_19 ) ;
F_5 ( & V_24 -> V_20 ) ;
V_24 -> V_35 = V_35 ;
V_24 -> V_47 = V_101 -> V_47 ;
V_24 -> V_64 = V_64 ;
V_24 -> V_28 = V_105 ;
V_24 -> V_73 = V_73 ;
V_24 -> V_72 = V_72 ;
V_24 -> V_108 = - 1 ;
V_24 -> V_109 = - 1 ;
F_86 ( & V_24 -> V_50 , 1 ) ;
F_86 ( & V_24 -> error , 0 ) ;
F_86 ( & V_24 -> V_92 , 0 ) ;
V_97 = V_24 + 1 ;
V_24 -> V_30 = V_97 ;
V_24 -> V_29 = V_97 + sizeof( struct V_39 * ) * V_105 ;
V_24 -> V_110 = V_97 + sizeof( struct V_39 * ) * V_105 * 2 ;
if ( V_35 -> V_111 & V_112 )
V_63 = V_73 - 1 ;
else if ( V_35 -> V_111 & V_113 )
V_63 = V_73 - 2 ;
else
F_87 () ;
V_24 -> V_63 = V_63 ;
return V_24 ;
}
static int F_10 ( struct V_23 * V_24 )
{
int V_11 ;
struct V_39 * V_39 ;
for ( V_11 = 0 ; V_11 < V_24 -> V_28 ; V_11 ++ ) {
if ( V_24 -> V_30 [ V_11 ] )
continue;
V_39 = F_88 ( V_107 | V_114 ) ;
if ( ! V_39 )
return - V_17 ;
V_24 -> V_30 [ V_11 ] = V_39 ;
}
return 0 ;
}
static int F_89 ( struct V_23 * V_24 )
{
int V_11 ;
struct V_39 * V_39 ;
V_11 = F_51 ( V_24 , V_24 -> V_63 , 0 ) ;
for (; V_11 < V_24 -> V_28 ; V_11 ++ ) {
if ( V_24 -> V_30 [ V_11 ] )
continue;
V_39 = F_88 ( V_107 | V_114 ) ;
if ( ! V_39 )
return - V_17 ;
V_24 -> V_30 [ V_11 ] = V_39 ;
}
return 0 ;
}
static int F_90 ( struct V_23 * V_24 ,
struct V_41 * V_41 ,
struct V_39 * V_39 ,
int V_115 ,
unsigned long V_116 ,
unsigned long V_117 )
{
struct V_87 * V_65 = V_41 -> V_118 ;
T_1 V_119 = 0 ;
int V_27 ;
struct V_87 * V_87 ;
struct V_120 * V_70 ;
T_1 V_121 ;
V_70 = & V_24 -> V_35 -> V_122 [ V_115 ] ;
V_121 = V_70 -> V_123 + ( V_116 << V_98 ) ;
if ( ! V_70 -> V_124 -> V_125 )
return F_91 ( V_24 , V_115 ) ;
if ( V_65 ) {
V_119 = ( T_1 ) V_65 -> V_126 . V_127 << 9 ;
V_119 += V_65 -> V_126 . V_128 ;
if ( V_119 == V_121 && V_70 -> V_124 -> V_125 &&
! V_65 -> V_89 &&
V_65 -> V_129 == V_70 -> V_124 -> V_125 ) {
V_27 = F_92 ( V_65 , V_39 , V_31 , 0 ) ;
if ( V_27 == V_31 )
return 0 ;
}
}
V_87 = F_93 ( V_107 , V_117 >> V_98 ? : 1 ) ;
if ( ! V_87 )
return - V_17 ;
V_87 -> V_126 . V_128 = 0 ;
V_87 -> V_129 = V_70 -> V_124 -> V_125 ;
V_87 -> V_126 . V_127 = V_121 >> 9 ;
F_92 ( V_87 , V_39 , V_31 , 0 ) ;
F_94 ( V_41 , V_87 ) ;
return 0 ;
}
static void F_95 ( struct V_23 * V_24 )
{
if ( V_24 -> V_108 >= 0 || V_24 -> V_109 >= 0 ) {
F_31 ( V_24 -> V_108 == V_24 -> V_73 - 1 ) ;
F_96 ( V_24 ) ;
} else {
F_97 ( V_24 ) ;
}
}
static void F_98 ( struct V_23 * V_24 )
{
struct V_87 * V_87 ;
T_1 V_130 ;
unsigned long V_131 ;
unsigned long V_116 ;
struct V_39 * V_97 ;
int V_11 ;
F_80 ( & V_24 -> V_48 ) ;
F_99 (bio, &rbio->bio_list) {
V_130 = ( T_1 ) V_87 -> V_126 . V_127 << 9 ;
V_131 = V_130 - V_24 -> V_35 -> V_36 [ 0 ] ;
V_116 = V_131 >> V_98 ;
for ( V_11 = 0 ; V_11 < V_87 -> V_132 ; V_11 ++ ) {
V_97 = V_87 -> V_133 [ V_11 ] . V_134 ;
V_24 -> V_29 [ V_116 + V_11 ] = V_97 ;
}
}
F_81 ( & V_24 -> V_48 ) ;
}
static T_3 void F_97 ( struct V_23 * V_24 )
{
struct V_102 * V_35 = V_24 -> V_35 ;
void * V_135 [ V_24 -> V_73 ] ;
int V_63 = V_24 -> V_63 ;
int V_70 ;
int V_94 ;
int V_136 = - 1 ;
int V_137 = - 1 ;
struct V_41 V_41 ;
struct V_87 * V_87 ;
int V_27 ;
F_23 ( & V_41 ) ;
if ( V_24 -> V_73 - V_24 -> V_63 == 1 ) {
V_136 = V_24 -> V_73 - 1 ;
} else if ( V_24 -> V_73 - V_24 -> V_63 == 2 ) {
V_136 = V_24 -> V_73 - 2 ;
V_137 = V_24 -> V_73 - 1 ;
} else {
F_87 () ;
}
F_80 ( & V_24 -> V_48 ) ;
F_14 ( V_66 , & V_24 -> V_33 ) ;
F_81 ( & V_24 -> V_48 ) ;
F_86 ( & V_24 -> error , 0 ) ;
F_98 ( V_24 ) ;
if ( ! F_49 ( V_24 ) )
F_9 ( V_24 ) ;
else
F_60 ( V_32 , & V_24 -> V_33 ) ;
for ( V_94 = 0 ; V_94 < V_24 -> V_72 ; V_94 ++ ) {
struct V_39 * V_97 ;
for ( V_70 = 0 ; V_70 < V_63 ; V_70 ++ ) {
V_97 = F_79 ( V_24 , V_70 , V_94 , 0 ) ;
V_135 [ V_70 ] = F_11 ( V_97 ) ;
}
V_97 = F_53 ( V_24 , V_94 ) ;
F_13 ( V_97 ) ;
V_135 [ V_70 ++ ] = F_11 ( V_97 ) ;
if ( V_137 != - 1 ) {
V_97 = F_54 ( V_24 , V_94 ) ;
F_13 ( V_97 ) ;
V_135 [ V_70 ++ ] = F_11 ( V_97 ) ;
V_138 . V_139 ( V_24 -> V_73 , V_31 ,
V_135 ) ;
} else {
memcpy ( V_135 [ V_63 ] , V_135 [ 0 ] , V_31 ) ;
F_45 ( V_135 + 1 , V_63 - 1 , V_31 ) ;
}
for ( V_70 = 0 ; V_70 < V_24 -> V_73 ; V_70 ++ )
F_12 ( F_79 ( V_24 , V_70 , V_94 , 0 ) ) ;
}
for ( V_70 = 0 ; V_70 < V_24 -> V_73 ; V_70 ++ ) {
for ( V_94 = 0 ; V_94 < V_24 -> V_72 ; V_94 ++ ) {
struct V_39 * V_39 ;
if ( V_70 < V_24 -> V_63 ) {
V_39 = F_79 ( V_24 , V_70 , V_94 , 1 ) ;
if ( ! V_39 )
continue;
} else {
V_39 = F_52 ( V_24 , V_70 , V_94 ) ;
}
V_27 = F_90 ( V_24 , & V_41 ,
V_39 , V_70 , V_94 , V_24 -> V_64 ) ;
if ( V_27 )
goto V_140;
}
}
if ( F_100 ( ! V_35 -> V_104 ) )
goto V_141;
for ( V_70 = 0 ; V_70 < V_24 -> V_73 ; V_70 ++ ) {
if ( ! V_35 -> V_142 [ V_70 ] )
continue;
for ( V_94 = 0 ; V_94 < V_24 -> V_72 ; V_94 ++ ) {
struct V_39 * V_39 ;
if ( V_70 < V_24 -> V_63 ) {
V_39 = F_79 ( V_24 , V_70 , V_94 , 1 ) ;
if ( ! V_39 )
continue;
} else {
V_39 = F_52 ( V_24 , V_70 , V_94 ) ;
}
V_27 = F_90 ( V_24 , & V_41 , V_39 ,
V_24 -> V_35 -> V_142 [ V_70 ] ,
V_94 , V_24 -> V_64 ) ;
if ( V_27 )
goto V_140;
}
}
V_141:
F_86 ( & V_24 -> V_92 , F_101 ( & V_41 ) ) ;
F_31 ( F_67 ( & V_24 -> V_92 ) == 0 ) ;
while ( 1 ) {
V_87 = F_102 ( & V_41 ) ;
if ( ! V_87 )
break;
V_87 -> V_90 = V_24 ;
V_87 -> V_143 = F_76 ;
F_103 ( V_87 , V_144 , 0 ) ;
F_104 ( V_87 ) ;
}
return;
V_140:
F_72 ( V_24 , - V_93 ) ;
}
static int F_105 ( struct V_23 * V_24 ,
struct V_87 * V_87 )
{
T_1 V_123 = V_87 -> V_126 . V_127 ;
T_1 V_145 ;
int V_11 ;
struct V_120 * V_70 ;
V_123 <<= 9 ;
for ( V_11 = 0 ; V_11 < V_24 -> V_35 -> V_103 ; V_11 ++ ) {
V_70 = & V_24 -> V_35 -> V_122 [ V_11 ] ;
V_145 = V_70 -> V_123 ;
if ( V_123 >= V_145 &&
V_123 < V_145 + V_24 -> V_64 &&
V_87 -> V_129 == V_70 -> V_124 -> V_125 ) {
return V_11 ;
}
}
return - 1 ;
}
static int F_106 ( struct V_23 * V_24 ,
struct V_87 * V_87 )
{
T_1 V_146 = V_87 -> V_126 . V_127 ;
T_1 V_145 ;
int V_11 ;
V_146 <<= 9 ;
for ( V_11 = 0 ; V_11 < V_24 -> V_63 ; V_11 ++ ) {
V_145 = V_24 -> V_35 -> V_36 [ V_11 ] ;
if ( V_146 >= V_145 &&
V_146 < V_145 + V_24 -> V_64 ) {
return V_11 ;
}
}
return - 1 ;
}
static int F_91 ( struct V_23 * V_24 , int V_147 )
{
unsigned long V_33 ;
int V_27 = 0 ;
F_35 ( & V_24 -> V_48 , V_33 ) ;
if ( V_24 -> V_108 == V_147 || V_24 -> V_109 == V_147 )
goto V_78;
if ( V_24 -> V_108 == - 1 ) {
V_24 -> V_108 = V_147 ;
F_42 ( & V_24 -> error ) ;
} else if ( V_24 -> V_109 == - 1 ) {
V_24 -> V_109 = V_147 ;
F_42 ( & V_24 -> error ) ;
} else {
V_27 = - V_93 ;
}
V_78:
F_36 ( & V_24 -> V_48 , V_33 ) ;
return V_27 ;
}
static int F_77 ( struct V_23 * V_24 ,
struct V_87 * V_87 )
{
int V_147 = F_105 ( V_24 , V_87 ) ;
if ( V_147 < 0 )
return - V_93 ;
return F_91 ( V_24 , V_147 ) ;
}
static void F_107 ( struct V_87 * V_87 )
{
int V_11 ;
struct V_39 * V_97 ;
for ( V_11 = 0 ; V_11 < V_87 -> V_132 ; V_11 ++ ) {
V_97 = V_87 -> V_133 [ V_11 ] . V_134 ;
F_13 ( V_97 ) ;
}
}
static void F_108 ( struct V_87 * V_87 )
{
struct V_23 * V_24 = V_87 -> V_90 ;
if ( V_87 -> V_89 )
F_77 ( V_24 , V_87 ) ;
else
F_107 ( V_87 ) ;
F_78 ( V_87 ) ;
if ( ! F_68 ( & V_24 -> V_92 ) )
return;
if ( F_67 ( & V_24 -> error ) > V_24 -> V_35 -> V_91 )
goto V_140;
F_95 ( V_24 ) ;
return;
V_140:
F_72 ( V_24 , - V_93 ) ;
}
static void F_62 ( struct V_23 * V_24 )
{
F_109 ( & V_24 -> V_148 , V_149 ,
V_150 , NULL , NULL ) ;
F_110 ( V_24 -> V_47 -> V_151 ,
& V_24 -> V_148 ) ;
}
static void F_61 ( struct V_23 * V_24 )
{
F_109 ( & V_24 -> V_148 , V_149 ,
V_152 , NULL , NULL ) ;
F_110 ( V_24 -> V_47 -> V_151 ,
& V_24 -> V_148 ) ;
}
static int F_111 ( struct V_23 * V_24 )
{
int V_153 = 0 ;
struct V_41 V_41 ;
int V_27 ;
int V_94 ;
int V_70 ;
struct V_87 * V_87 ;
F_23 ( & V_41 ) ;
V_27 = F_10 ( V_24 ) ;
if ( V_27 )
goto V_140;
F_98 ( V_24 ) ;
F_86 ( & V_24 -> error , 0 ) ;
for ( V_70 = 0 ; V_70 < V_24 -> V_63 ; V_70 ++ ) {
for ( V_94 = 0 ; V_94 < V_24 -> V_72 ; V_94 ++ ) {
struct V_39 * V_39 ;
V_39 = F_79 ( V_24 , V_70 , V_94 , 1 ) ;
if ( V_39 )
continue;
V_39 = F_52 ( V_24 , V_70 , V_94 ) ;
if ( F_19 ( V_39 ) )
continue;
V_27 = F_90 ( V_24 , & V_41 , V_39 ,
V_70 , V_94 , V_24 -> V_64 ) ;
if ( V_27 )
goto V_140;
}
}
V_153 = F_101 ( & V_41 ) ;
if ( ! V_153 ) {
goto V_154;
}
F_86 ( & V_24 -> V_92 , V_153 ) ;
while ( 1 ) {
V_87 = F_102 ( & V_41 ) ;
if ( ! V_87 )
break;
V_87 -> V_90 = V_24 ;
V_87 -> V_143 = F_108 ;
F_103 ( V_87 , V_155 , 0 ) ;
F_112 ( V_24 -> V_47 , V_87 ,
V_156 ) ;
F_104 ( V_87 ) ;
}
return 0 ;
V_140:
F_72 ( V_24 , - V_93 ) ;
return - V_93 ;
V_154:
F_95 ( V_24 ) ;
return 0 ;
}
static int F_113 ( struct V_23 * V_24 )
{
int V_27 ;
V_27 = F_89 ( V_24 ) ;
if ( V_27 ) {
F_33 ( V_24 ) ;
return V_27 ;
}
V_27 = F_55 ( V_24 ) ;
if ( V_27 == 0 )
F_97 ( V_24 ) ;
return 0 ;
}
static int F_114 ( struct V_23 * V_24 )
{
int V_27 ;
V_27 = F_55 ( V_24 ) ;
if ( V_27 == 0 )
F_62 ( V_24 ) ;
return 0 ;
}
static int F_115 ( struct V_23 * V_24 )
{
if ( ! F_49 ( V_24 ) )
return F_114 ( V_24 ) ;
return F_113 ( V_24 ) ;
}
static int F_116 ( void * V_157 , struct V_81 * V_158 , struct V_81 * V_159 )
{
struct V_23 * V_160 = F_117 ( V_158 , struct V_23 ,
V_51 ) ;
struct V_23 * V_161 = F_117 ( V_159 , struct V_23 ,
V_51 ) ;
T_1 V_162 = V_160 -> V_41 . V_82 -> V_126 . V_127 ;
T_1 V_163 = V_161 -> V_41 . V_82 -> V_126 . V_127 ;
if ( V_162 < V_163 )
return - 1 ;
if ( V_162 > V_163 )
return 1 ;
return 0 ;
}
static void F_118 ( struct V_164 * V_165 )
{
struct V_23 * V_7 ;
struct V_23 * V_65 = NULL ;
F_119 ( NULL , & V_165 -> V_166 , F_116 ) ;
while ( ! F_29 ( & V_165 -> V_166 ) ) {
V_7 = F_38 ( V_165 -> V_166 . V_52 ,
struct V_23 , V_51 ) ;
F_27 ( & V_7 -> V_51 ) ;
if ( F_49 ( V_7 ) ) {
F_113 ( V_7 ) ;
continue;
}
if ( V_65 ) {
if ( F_50 ( V_65 , V_7 ) ) {
F_21 ( V_65 , V_7 ) ;
F_33 ( V_7 ) ;
continue;
}
F_115 ( V_65 ) ;
}
V_65 = V_7 ;
}
if ( V_65 ) {
F_115 ( V_65 ) ;
}
F_70 ( V_165 ) ;
}
static void F_120 ( struct V_167 * V_148 )
{
struct V_164 * V_165 ;
V_165 = F_117 ( V_148 , struct V_164 , V_148 ) ;
F_118 ( V_165 ) ;
}
static void F_121 ( struct V_168 * V_169 , bool V_170 )
{
struct V_164 * V_165 ;
V_165 = F_117 ( V_169 , struct V_164 , V_169 ) ;
if ( V_170 ) {
F_109 ( & V_165 -> V_148 , V_149 ,
F_120 , NULL , NULL ) ;
F_110 ( V_165 -> V_2 -> V_151 ,
& V_165 -> V_148 ) ;
return;
}
F_118 ( V_165 ) ;
}
int F_122 ( struct V_100 * V_101 , struct V_87 * V_87 ,
struct V_102 * V_35 , T_1 V_64 )
{
struct V_23 * V_24 ;
struct V_164 * V_165 = NULL ;
struct V_168 * V_169 ;
int V_27 ;
V_24 = F_84 ( V_101 , V_35 , V_64 ) ;
if ( F_123 ( V_24 ) ) {
F_69 ( V_35 ) ;
return F_124 ( V_24 ) ;
}
F_94 ( & V_24 -> V_41 , V_87 ) ;
V_24 -> V_42 = V_87 -> V_126 . V_128 ;
V_24 -> V_67 = V_84 ;
F_125 ( V_101 -> V_47 ) ;
V_24 -> V_43 = 1 ;
if ( F_49 ( V_24 ) ) {
V_27 = F_113 ( V_24 ) ;
if ( V_27 )
F_126 ( V_101 -> V_47 ) ;
return V_27 ;
}
V_169 = F_127 ( F_121 , V_101 -> V_47 ,
sizeof( * V_165 ) ) ;
if ( V_169 ) {
V_165 = F_117 ( V_169 , struct V_164 , V_169 ) ;
if ( ! V_165 -> V_2 ) {
V_165 -> V_2 = V_101 -> V_47 ;
F_5 ( & V_165 -> V_166 ) ;
}
F_58 ( & V_24 -> V_51 , & V_165 -> V_166 ) ;
V_27 = 0 ;
} else {
V_27 = F_115 ( V_24 ) ;
if ( V_27 )
F_126 ( V_101 -> V_47 ) ;
}
return V_27 ;
}
static void F_128 ( struct V_23 * V_24 )
{
int V_94 , V_70 ;
void * * V_135 ;
int V_108 = - 1 , V_109 = - 1 ;
struct V_39 * V_39 ;
int V_86 ;
int V_11 ;
V_135 = F_129 ( V_24 -> V_73 , sizeof( void * ) , V_107 ) ;
if ( ! V_135 ) {
V_86 = - V_17 ;
goto V_171;
}
V_108 = V_24 -> V_108 ;
V_109 = V_24 -> V_109 ;
if ( V_24 -> V_67 == V_83 ||
V_24 -> V_67 == V_69 ) {
F_80 ( & V_24 -> V_48 ) ;
F_14 ( V_66 , & V_24 -> V_33 ) ;
F_81 ( & V_24 -> V_48 ) ;
}
F_98 ( V_24 ) ;
for ( V_94 = 0 ; V_94 < V_24 -> V_72 ; V_94 ++ ) {
if ( V_24 -> V_67 == V_68 &&
! F_18 ( V_94 , V_24 -> V_110 ) )
continue;
for ( V_70 = 0 ; V_70 < V_24 -> V_73 ; V_70 ++ ) {
if ( ( V_24 -> V_67 == V_83 ||
V_24 -> V_67 == V_69 ) &&
( V_70 == V_108 || V_70 == V_109 ) ) {
V_39 = F_79 ( V_24 , V_70 , V_94 , 0 ) ;
} else {
V_39 = F_52 ( V_24 , V_70 , V_94 ) ;
}
V_135 [ V_70 ] = F_11 ( V_39 ) ;
}
if ( V_24 -> V_35 -> V_111 & V_113 ) {
if ( V_109 < 0 ) {
if ( V_108 == V_24 -> V_63 ) {
V_86 = - V_93 ;
goto V_140;
}
goto V_172;
}
if ( V_108 > V_109 ) {
int V_173 = V_109 ;
V_109 = V_108 ;
V_108 = V_173 ;
}
if ( V_24 -> V_35 -> V_36 [ V_109 ] == V_174 ) {
if ( V_24 -> V_35 -> V_36 [ V_108 ] ==
V_175 ) {
V_86 = - V_93 ;
goto V_140;
}
goto V_172;
}
if ( V_24 -> V_35 -> V_36 [ V_109 ] == V_175 ) {
F_130 ( V_24 -> V_73 ,
V_31 , V_108 , V_135 ) ;
} else {
F_131 ( V_24 -> V_73 ,
V_31 , V_108 , V_109 ,
V_135 ) ;
}
} else {
void * V_97 ;
F_31 ( V_109 != - 1 ) ;
V_172:
memcpy ( V_135 [ V_108 ] ,
V_135 [ V_24 -> V_63 ] ,
V_31 ) ;
V_97 = V_135 [ V_108 ] ;
for ( V_70 = V_108 ; V_70 < V_24 -> V_63 - 1 ; V_70 ++ )
V_135 [ V_70 ] = V_135 [ V_70 + 1 ] ;
V_135 [ V_24 -> V_63 - 1 ] = V_97 ;
F_45 ( V_135 , V_24 -> V_63 - 1 , V_31 ) ;
}
if ( V_24 -> V_67 == V_84 ) {
for ( V_11 = 0 ; V_11 < V_24 -> V_72 ; V_11 ++ ) {
if ( V_108 != - 1 ) {
V_39 = F_52 ( V_24 , V_108 , V_11 ) ;
F_13 ( V_39 ) ;
}
if ( V_109 != - 1 ) {
V_39 = F_52 ( V_24 , V_109 , V_11 ) ;
F_13 ( V_39 ) ;
}
}
}
for ( V_70 = 0 ; V_70 < V_24 -> V_73 ; V_70 ++ ) {
if ( ( V_24 -> V_67 == V_83 ||
V_24 -> V_67 == V_69 ) &&
( V_70 == V_108 || V_70 == V_109 ) ) {
V_39 = F_79 ( V_24 , V_70 , V_94 , 0 ) ;
} else {
V_39 = F_52 ( V_24 , V_70 , V_94 ) ;
}
F_12 ( V_39 ) ;
}
}
V_86 = 0 ;
V_140:
F_70 ( V_135 ) ;
V_171:
if ( V_24 -> V_67 == V_83 ) {
if ( V_86 == 0 )
F_9 ( V_24 ) ;
else
F_60 ( V_32 , & V_24 -> V_33 ) ;
F_72 ( V_24 , V_86 ) ;
} else if ( V_24 -> V_67 == V_69 ) {
F_72 ( V_24 , V_86 ) ;
} else if ( V_86 == 0 ) {
V_24 -> V_108 = - 1 ;
V_24 -> V_109 = - 1 ;
if ( V_24 -> V_67 == V_84 )
F_97 ( V_24 ) ;
else if ( V_24 -> V_67 == V_68 )
F_132 ( V_24 , 0 ) ;
else
F_87 () ;
} else {
F_72 ( V_24 , V_86 ) ;
}
}
static void F_133 ( struct V_87 * V_87 )
{
struct V_23 * V_24 = V_87 -> V_90 ;
if ( V_87 -> V_89 )
F_77 ( V_24 , V_87 ) ;
else
F_107 ( V_87 ) ;
F_78 ( V_87 ) ;
if ( ! F_68 ( & V_24 -> V_92 ) )
return;
if ( F_67 ( & V_24 -> error ) > V_24 -> V_35 -> V_91 )
F_72 ( V_24 , - V_93 ) ;
else
F_128 ( V_24 ) ;
}
static int F_96 ( struct V_23 * V_24 )
{
int V_153 = 0 ;
struct V_41 V_41 ;
int V_27 ;
int V_94 ;
int V_70 ;
struct V_87 * V_87 ;
F_23 ( & V_41 ) ;
V_27 = F_10 ( V_24 ) ;
if ( V_27 )
goto V_140;
F_86 ( & V_24 -> error , 0 ) ;
for ( V_70 = 0 ; V_70 < V_24 -> V_73 ; V_70 ++ ) {
if ( V_24 -> V_108 == V_70 || V_24 -> V_109 == V_70 ) {
F_42 ( & V_24 -> error ) ;
continue;
}
for ( V_94 = 0 ; V_94 < V_24 -> V_72 ; V_94 ++ ) {
struct V_39 * V_97 ;
V_97 = F_52 ( V_24 , V_70 , V_94 ) ;
if ( F_19 ( V_97 ) )
continue;
V_27 = F_90 ( V_24 , & V_41 ,
F_52 ( V_24 , V_70 , V_94 ) ,
V_70 , V_94 , V_24 -> V_64 ) ;
if ( V_27 < 0 )
goto V_140;
}
}
V_153 = F_101 ( & V_41 ) ;
if ( ! V_153 ) {
if ( F_67 ( & V_24 -> error ) <= V_24 -> V_35 -> V_91 ) {
F_128 ( V_24 ) ;
goto V_78;
} else {
goto V_140;
}
}
F_86 ( & V_24 -> V_92 , V_153 ) ;
while ( 1 ) {
V_87 = F_102 ( & V_41 ) ;
if ( ! V_87 )
break;
V_87 -> V_90 = V_24 ;
V_87 -> V_143 = F_133 ;
F_103 ( V_87 , V_155 , 0 ) ;
F_112 ( V_24 -> V_47 , V_87 ,
V_156 ) ;
F_104 ( V_87 ) ;
}
V_78:
return 0 ;
V_140:
if ( V_24 -> V_67 == V_83 ||
V_24 -> V_67 == V_69 )
F_72 ( V_24 , - V_93 ) ;
return - V_93 ;
}
int F_134 ( struct V_100 * V_101 , struct V_87 * V_87 ,
struct V_102 * V_35 , T_1 V_64 ,
int V_176 , int V_177 )
{
struct V_23 * V_24 ;
int V_27 ;
V_24 = F_84 ( V_101 , V_35 , V_64 ) ;
if ( F_123 ( V_24 ) ) {
if ( V_177 )
F_69 ( V_35 ) ;
return F_124 ( V_24 ) ;
}
V_24 -> V_67 = V_83 ;
F_94 ( & V_24 -> V_41 , V_87 ) ;
V_24 -> V_42 = V_87 -> V_126 . V_128 ;
V_24 -> V_108 = F_106 ( V_24 , V_87 ) ;
if ( V_24 -> V_108 == - 1 ) {
F_87 () ;
if ( V_177 )
F_69 ( V_35 ) ;
F_70 ( V_24 ) ;
return - V_93 ;
}
if ( V_177 ) {
F_125 ( V_101 -> V_47 ) ;
V_24 -> V_43 = 1 ;
} else {
F_135 ( V_35 ) ;
}
if ( V_176 == 3 )
V_24 -> V_109 = V_24 -> V_73 - 2 ;
V_27 = F_55 ( V_24 ) ;
if ( V_27 == 0 )
F_96 ( V_24 ) ;
return 0 ;
}
static void V_150 ( struct V_167 * V_148 )
{
struct V_23 * V_24 ;
V_24 = F_117 ( V_148 , struct V_23 , V_148 ) ;
F_111 ( V_24 ) ;
}
static void V_152 ( struct V_167 * V_148 )
{
struct V_23 * V_24 ;
V_24 = F_117 ( V_148 , struct V_23 , V_148 ) ;
F_96 ( V_24 ) ;
}
struct V_23 *
F_136 ( struct V_100 * V_101 , struct V_87 * V_87 ,
struct V_102 * V_35 , T_1 V_64 ,
struct V_178 * V_179 ,
unsigned long * V_110 , int V_180 )
{
struct V_23 * V_24 ;
int V_11 ;
V_24 = F_84 ( V_101 , V_35 , V_64 ) ;
if ( F_123 ( V_24 ) )
return NULL ;
F_94 ( & V_24 -> V_41 , V_87 ) ;
ASSERT ( ! V_87 -> V_126 . V_128 ) ;
V_24 -> V_67 = V_68 ;
for ( V_11 = 0 ; V_11 < V_24 -> V_73 ; V_11 ++ ) {
if ( V_35 -> V_122 [ V_11 ] . V_124 == V_179 ) {
V_24 -> V_181 = V_11 ;
break;
}
}
ASSERT ( V_101 -> V_182 == V_31 ) ;
ASSERT ( V_24 -> V_72 == V_180 ) ;
F_137 ( V_24 -> V_110 , V_110 , V_180 ) ;
return V_24 ;
}
void F_138 ( struct V_23 * V_24 , struct V_39 * V_39 ,
T_1 V_146 )
{
int V_131 ;
int V_71 ;
ASSERT ( V_146 >= V_24 -> V_35 -> V_36 [ 0 ] ) ;
ASSERT ( V_146 + V_31 <= V_24 -> V_35 -> V_36 [ 0 ] +
V_24 -> V_64 * V_24 -> V_63 ) ;
V_131 = ( int ) ( V_146 - V_24 -> V_35 -> V_36 [ 0 ] ) ;
V_71 = V_131 >> V_98 ;
V_24 -> V_29 [ V_71 ] = V_39 ;
}
static int F_139 ( struct V_23 * V_24 )
{
int V_11 ;
int V_183 ;
int V_71 ;
struct V_39 * V_39 ;
F_140 (bit, rbio->dbitmap, rbio->stripe_npages) {
for ( V_11 = 0 ; V_11 < V_24 -> V_73 ; V_11 ++ ) {
V_71 = V_11 * V_24 -> V_72 + V_183 ;
if ( V_24 -> V_30 [ V_71 ] )
continue;
V_39 = F_88 ( V_107 | V_114 ) ;
if ( ! V_39 )
return - V_17 ;
V_24 -> V_30 [ V_71 ] = V_39 ;
}
}
return 0 ;
}
static T_3 void F_132 ( struct V_23 * V_24 ,
int V_184 )
{
struct V_102 * V_35 = V_24 -> V_35 ;
void * V_135 [ V_24 -> V_73 ] ;
F_141 ( V_185 , V_24 -> V_72 ) ;
int V_63 = V_24 -> V_63 ;
int V_70 ;
int V_94 ;
int V_136 = - 1 ;
int V_137 = - 1 ;
struct V_39 * V_186 = NULL ;
struct V_39 * V_187 = NULL ;
struct V_41 V_41 ;
struct V_87 * V_87 ;
int V_188 = 0 ;
int V_27 ;
F_23 ( & V_41 ) ;
if ( V_24 -> V_73 - V_24 -> V_63 == 1 ) {
V_136 = V_24 -> V_73 - 1 ;
} else if ( V_24 -> V_73 - V_24 -> V_63 == 2 ) {
V_136 = V_24 -> V_73 - 2 ;
V_137 = V_24 -> V_73 - 1 ;
} else {
F_87 () ;
}
if ( V_35 -> V_104 && V_35 -> V_142 [ V_24 -> V_181 ] ) {
V_188 = 1 ;
F_137 ( V_185 , V_24 -> V_110 , V_24 -> V_72 ) ;
}
F_60 ( V_32 , & V_24 -> V_33 ) ;
if ( ! V_184 )
goto V_189;
V_186 = F_88 ( V_107 | V_114 ) ;
if ( ! V_186 )
goto V_140;
F_13 ( V_186 ) ;
if ( V_137 != - 1 ) {
V_187 = F_88 ( V_107 | V_114 ) ;
if ( ! V_187 ) {
F_20 ( V_186 ) ;
goto V_140;
}
F_13 ( V_187 ) ;
}
F_86 ( & V_24 -> error , 0 ) ;
F_140 (pagenr, rbio->dbitmap, rbio->stripe_npages) {
struct V_39 * V_97 ;
void * V_190 ;
for ( V_70 = 0 ; V_70 < V_63 ; V_70 ++ ) {
V_97 = F_79 ( V_24 , V_70 , V_94 , 0 ) ;
V_135 [ V_70 ] = F_11 ( V_97 ) ;
}
V_135 [ V_70 ++ ] = F_11 ( V_186 ) ;
if ( V_137 != - 1 ) {
V_135 [ V_70 ++ ] = F_11 ( V_187 ) ;
V_138 . V_139 ( V_24 -> V_73 , V_31 ,
V_135 ) ;
} else {
memcpy ( V_135 [ V_63 ] , V_135 [ 0 ] , V_31 ) ;
F_45 ( V_135 + 1 , V_63 - 1 , V_31 ) ;
}
V_97 = F_52 ( V_24 , V_24 -> V_181 , V_94 ) ;
V_190 = F_11 ( V_97 ) ;
if ( memcmp ( V_190 , V_135 [ V_24 -> V_181 ] , V_31 ) )
memcpy ( V_190 , V_135 [ V_24 -> V_181 ] , V_31 ) ;
else
F_142 ( V_24 -> V_110 , V_94 , 1 ) ;
F_12 ( V_97 ) ;
for ( V_70 = 0 ; V_70 < V_24 -> V_73 ; V_70 ++ )
F_12 ( F_79 ( V_24 , V_70 , V_94 , 0 ) ) ;
}
F_20 ( V_186 ) ;
if ( V_187 )
F_20 ( V_187 ) ;
V_189:
F_140 (pagenr, rbio->dbitmap, rbio->stripe_npages) {
struct V_39 * V_39 ;
V_39 = F_52 ( V_24 , V_24 -> V_181 , V_94 ) ;
V_27 = F_90 ( V_24 , & V_41 ,
V_39 , V_24 -> V_181 , V_94 , V_24 -> V_64 ) ;
if ( V_27 )
goto V_140;
}
if ( ! V_188 )
goto V_191;
F_140 (pagenr, pbitmap, rbio->stripe_npages) {
struct V_39 * V_39 ;
V_39 = F_52 ( V_24 , V_24 -> V_181 , V_94 ) ;
V_27 = F_90 ( V_24 , & V_41 , V_39 ,
V_35 -> V_142 [ V_24 -> V_181 ] ,
V_94 , V_24 -> V_64 ) ;
if ( V_27 )
goto V_140;
}
V_191:
V_63 = F_101 ( & V_41 ) ;
if ( ! V_63 ) {
F_72 ( V_24 , 0 ) ;
return;
}
F_86 ( & V_24 -> V_92 , V_63 ) ;
while ( 1 ) {
V_87 = F_102 ( & V_41 ) ;
if ( ! V_87 )
break;
V_87 -> V_90 = V_24 ;
V_87 -> V_143 = F_76 ;
F_103 ( V_87 , V_144 , 0 ) ;
F_104 ( V_87 ) ;
}
return;
V_140:
F_72 ( V_24 , - V_93 ) ;
}
static inline int F_143 ( struct V_23 * V_24 , int V_70 )
{
if ( V_70 >= 0 && V_70 < V_24 -> V_63 )
return 1 ;
return 0 ;
}
static void F_144 ( struct V_23 * V_24 )
{
if ( F_67 ( & V_24 -> error ) > V_24 -> V_35 -> V_91 )
goto V_140;
if ( V_24 -> V_108 >= 0 || V_24 -> V_109 >= 0 ) {
int V_192 = 0 , V_193 = - 1 ;
if ( F_143 ( V_24 , V_24 -> V_108 ) )
V_192 ++ ;
else if ( F_145 ( V_24 -> V_108 ) )
V_193 = V_24 -> V_108 ;
if ( F_143 ( V_24 , V_24 -> V_109 ) )
V_192 ++ ;
else if ( F_145 ( V_24 -> V_109 ) )
V_193 = V_24 -> V_109 ;
if ( V_192 > V_24 -> V_35 -> V_91 - 1 )
goto V_140;
if ( V_192 == 0 ) {
F_132 ( V_24 , 0 ) ;
return;
}
if ( V_193 != V_24 -> V_181 )
goto V_140;
F_128 ( V_24 ) ;
} else {
F_132 ( V_24 , 1 ) ;
}
return;
V_140:
F_72 ( V_24 , - V_93 ) ;
}
static void F_146 ( struct V_87 * V_87 )
{
struct V_23 * V_24 = V_87 -> V_90 ;
if ( V_87 -> V_89 )
F_77 ( V_24 , V_87 ) ;
else
F_107 ( V_87 ) ;
F_78 ( V_87 ) ;
if ( ! F_68 ( & V_24 -> V_92 ) )
return;
F_144 ( V_24 ) ;
}
static void F_147 ( struct V_23 * V_24 )
{
int V_153 = 0 ;
struct V_41 V_41 ;
int V_27 ;
int V_94 ;
int V_70 ;
struct V_87 * V_87 ;
V_27 = F_139 ( V_24 ) ;
if ( V_27 )
goto V_140;
F_23 ( & V_41 ) ;
F_86 ( & V_24 -> error , 0 ) ;
for ( V_70 = 0 ; V_70 < V_24 -> V_73 ; V_70 ++ ) {
F_140 (pagenr, rbio->dbitmap, rbio->stripe_npages) {
struct V_39 * V_39 ;
V_39 = F_79 ( V_24 , V_70 , V_94 , 1 ) ;
if ( V_39 )
continue;
V_39 = F_52 ( V_24 , V_70 , V_94 ) ;
if ( F_19 ( V_39 ) )
continue;
V_27 = F_90 ( V_24 , & V_41 , V_39 ,
V_70 , V_94 , V_24 -> V_64 ) ;
if ( V_27 )
goto V_140;
}
}
V_153 = F_101 ( & V_41 ) ;
if ( ! V_153 ) {
goto V_154;
}
F_86 ( & V_24 -> V_92 , V_153 ) ;
while ( 1 ) {
V_87 = F_102 ( & V_41 ) ;
if ( ! V_87 )
break;
V_87 -> V_90 = V_24 ;
V_87 -> V_143 = F_146 ;
F_103 ( V_87 , V_155 , 0 ) ;
F_112 ( V_24 -> V_47 , V_87 ,
V_156 ) ;
F_104 ( V_87 ) ;
}
return;
V_140:
F_72 ( V_24 , - V_93 ) ;
return;
V_154:
F_144 ( V_24 ) ;
}
static void F_148 ( struct V_167 * V_148 )
{
struct V_23 * V_24 ;
V_24 = F_117 ( V_148 , struct V_23 , V_148 ) ;
F_147 ( V_24 ) ;
}
static void F_63 ( struct V_23 * V_24 )
{
F_109 ( & V_24 -> V_148 , V_149 ,
F_148 , NULL , NULL ) ;
F_110 ( V_24 -> V_47 -> V_151 ,
& V_24 -> V_148 ) ;
}
void F_149 ( struct V_23 * V_24 )
{
if ( ! F_55 ( V_24 ) )
F_63 ( V_24 ) ;
}
struct V_23 *
F_150 ( struct V_100 * V_101 , struct V_87 * V_87 ,
struct V_102 * V_35 , T_1 V_194 )
{
struct V_23 * V_24 ;
V_24 = F_84 ( V_101 , V_35 , V_194 ) ;
if ( F_123 ( V_24 ) )
return NULL ;
V_24 -> V_67 = V_69 ;
F_94 ( & V_24 -> V_41 , V_87 ) ;
ASSERT ( ! V_87 -> V_126 . V_128 ) ;
V_24 -> V_108 = F_106 ( V_24 , V_87 ) ;
if ( V_24 -> V_108 == - 1 ) {
F_87 () ;
F_70 ( V_24 ) ;
return NULL ;
}
return V_24 ;
}
static void F_151 ( struct V_167 * V_148 )
{
struct V_23 * V_24 ;
V_24 = F_117 ( V_148 , struct V_23 , V_148 ) ;
F_96 ( V_24 ) ;
}
static void F_152 ( struct V_23 * V_24 )
{
F_109 ( & V_24 -> V_148 , V_149 ,
F_151 , NULL , NULL ) ;
F_110 ( V_24 -> V_47 -> V_151 , & V_24 -> V_148 ) ;
}
void F_153 ( struct V_23 * V_24 )
{
if ( ! F_55 ( V_24 ) )
F_152 ( V_24 ) ;
}
