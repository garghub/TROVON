static int F_1 ( void )
{
int V_1 ;
F_2 ( L_1 ) ;
V_1 = F_3 ( & V_2 . V_3 -> V_4 ) ;
F_4 ( V_1 < 0 ) ;
if ( V_1 < 0 )
return V_1 ;
return 0 ;
}
static void F_5 ( void )
{
int V_1 ;
F_2 ( L_2 ) ;
V_1 = F_6 ( & V_2 . V_3 -> V_4 ) ;
F_4 ( V_1 < 0 && V_1 != - V_5 ) ;
}
static T_1 F_7 ( int V_6 , void * V_7 )
{
struct V_8 * V_9 = V_7 ;
T_2 V_10 ;
V_10 = F_8 ( V_9 ) ;
F_9 ( V_9 , V_10 ) ;
if ( ( V_10 & V_11 ) &&
V_10 & V_12 ) {
T_2 V_13 ;
F_10 ( V_9 , V_14 ) ;
V_13 = F_11 ( V_2 . V_15 . V_16 , V_17 ) ;
V_13 = F_12 ( V_13 , 1 , 15 , 15 ) ;
V_13 = F_12 ( V_13 , 0 , 14 , 7 ) ;
F_13 ( V_2 . V_15 . V_16 , V_17 , V_13 ) ;
F_9 ( V_9 , V_11 |
V_12 ) ;
F_10 ( V_9 , V_18 ) ;
F_14 ( V_2 . V_15 . V_16 , V_17 , 0 , 15 , 15 ) ;
} else if ( V_10 & V_11 ) {
F_10 ( V_9 , V_19 ) ;
} else if ( V_10 & V_12 ) {
F_10 ( V_9 , V_18 ) ;
}
return V_20 ;
}
static int F_15 ( void )
{
int V_1 ;
struct V_21 * V_22 ;
if ( V_2 . V_23 != NULL )
return 0 ;
V_22 = F_16 ( & V_2 . V_3 -> V_4 , L_3 ) ;
if ( F_17 ( V_22 ) ) {
F_18 ( L_4 ) ;
return F_19 ( V_22 ) ;
}
if ( F_20 ( V_22 ) ) {
V_1 = F_21 ( V_22 , 1800000 , 1800000 ) ;
if ( V_1 ) {
F_22 ( V_22 ) ;
F_23 ( L_5 ) ;
return V_1 ;
}
}
V_2 . V_23 = V_22 ;
return 0 ;
}
static int F_24 ( struct V_24 * V_25 )
{
int V_1 ;
V_1 = F_25 ( V_2 . V_23 ) ;
if ( V_1 )
return V_1 ;
V_1 = F_1 () ;
if ( V_1 )
goto V_26;
F_26 ( V_27 ) ;
V_2 . V_28 = true ;
return 0 ;
V_26:
F_27 ( V_2 . V_23 ) ;
return V_1 ;
}
static void F_28 ( struct V_24 * V_25 )
{
V_2 . V_28 = false ;
F_5 () ;
F_27 ( V_2 . V_23 ) ;
}
static int F_29 ( struct V_24 * V_25 )
{
int V_1 ;
struct V_29 * V_30 ;
struct V_31 * V_32 = V_2 . V_33 . V_34 ;
unsigned long V_15 ;
V_1 = F_24 ( V_25 ) ;
if ( V_1 )
return V_1 ;
V_30 = & V_2 . V_35 . V_36 ;
F_2 ( L_6 , V_30 -> V_37 , V_30 -> V_38 ) ;
V_15 = V_30 -> V_39 / 1000 ;
F_30 ( & V_2 . V_40 , F_31 ( V_2 . V_41 ) , V_15 ) ;
F_32 ( & V_2 . V_9 , 0xffffffff ) ;
F_9 ( & V_2 . V_9 ,
F_8 ( & V_2 . V_9 ) ) ;
V_1 = F_33 ( & V_2 . V_40 , & V_2 . V_9 ) ;
if ( V_1 ) {
F_2 ( L_7 ) ;
goto V_42;
}
V_1 = F_34 ( & V_2 . V_15 , & V_2 . V_35 ) ;
if ( V_1 ) {
F_2 ( L_8 ) ;
goto V_43;
}
V_1 = F_10 ( & V_2 . V_9 , V_18 ) ;
if ( V_1 )
goto V_44;
F_35 ( & V_2 . V_45 , & V_2 . V_9 , & V_2 . V_35 ) ;
F_36 ( 0 ) ;
F_37 ( V_32 , V_30 ) ;
V_1 = F_38 ( & V_2 . V_9 ) ;
if ( V_1 )
goto V_46;
V_1 = F_39 ( V_32 ) ;
if ( V_1 )
goto V_47;
F_40 ( & V_2 . V_9 ,
V_11 | V_12 ) ;
return 0 ;
V_47:
F_41 ( & V_2 . V_9 ) ;
V_46:
F_10 ( & V_2 . V_9 , V_14 ) ;
V_44:
V_43:
F_42 ( & V_2 . V_40 , & V_2 . V_9 ) ;
V_42:
F_28 ( V_25 ) ;
return - V_48 ;
}
static void F_43 ( struct V_24 * V_25 )
{
struct V_31 * V_32 = V_2 . V_33 . V_34 ;
F_32 ( & V_2 . V_9 , 0xffffffff ) ;
F_44 ( V_32 ) ;
F_41 ( & V_2 . V_9 ) ;
F_10 ( & V_2 . V_9 , V_14 ) ;
F_42 ( & V_2 . V_40 , & V_2 . V_9 ) ;
F_28 ( V_25 ) ;
}
static int F_45 ( struct V_24 * V_25 ,
struct V_29 * V_36 )
{
struct V_24 * V_49 = & V_2 . V_33 ;
if ( ! F_46 ( V_49 -> V_50 , V_36 ) )
return - V_51 ;
return 0 ;
}
static void F_47 ( struct V_24 * V_25 ,
struct V_29 * V_36 )
{
struct V_52 V_53 ;
const struct V_54 * V_55 ;
F_48 ( & V_2 . V_56 ) ;
V_53 = F_49 ( V_36 ) ;
V_2 . V_35 . V_53 = V_53 ;
V_55 = F_50 ( V_53 . V_57 , V_53 . V_58 ) ;
if ( V_55 != NULL ) {
V_2 . V_35 = * V_55 ;
F_51 ( V_55 -> V_36 . V_39 ) ;
} else {
V_2 . V_35 . V_36 = * V_36 ;
V_2 . V_35 . V_53 . V_58 = 0 ;
V_2 . V_35 . V_53 . V_57 = V_59 ;
F_51 ( V_36 -> V_39 ) ;
}
F_2 ( L_9 , V_2 . V_35 . V_53 . V_57 == V_59 ?
L_10 : L_11 , V_2 . V_35 . V_53 . V_58 ) ;
F_52 ( & V_2 . V_56 ) ;
}
static void F_53 ( struct V_24 * V_25 ,
struct V_29 * V_36 )
{
const struct V_54 * V_35 ;
struct V_52 V_53 = V_2 . V_35 . V_53 ;
V_35 = F_50 ( V_53 . V_57 , V_53 . V_58 ) ;
if ( V_35 == NULL )
V_35 = F_54 () ;
memcpy ( V_36 , & V_35 -> V_36 , sizeof( V_35 -> V_36 ) ) ;
}
static void F_55 ( struct V_60 * V_61 )
{
F_48 ( & V_2 . V_56 ) ;
if ( F_1 () ) {
F_52 ( & V_2 . V_56 ) ;
return;
}
F_56 ( & V_2 . V_9 , V_61 ) ;
F_57 ( & V_2 . V_40 , V_61 ) ;
F_58 ( & V_2 . V_15 , V_61 ) ;
F_59 ( & V_2 . V_45 , V_61 ) ;
F_5 () ;
F_52 ( & V_2 . V_56 ) ;
}
static int F_60 ( T_3 * V_62 , int V_63 )
{
int V_1 ;
int V_64 ;
F_48 ( & V_2 . V_56 ) ;
V_1 = F_1 () ;
F_61 ( V_1 ) ;
V_64 = F_62 ( V_2 . V_9 . V_16 , V_65 , 3 , 2 ) ;
F_14 ( V_2 . V_9 . V_16 , V_65 , 1 , 3 , 2 ) ;
V_1 = F_63 ( & V_2 . V_45 , V_62 , V_63 ) ;
F_14 ( V_2 . V_9 . V_16 , V_65 , V_64 , 3 , 2 ) ;
F_5 () ;
F_52 ( & V_2 . V_56 ) ;
return V_1 ;
}
static int F_64 ( struct V_24 * V_25 )
{
struct V_24 * V_49 = & V_2 . V_33 ;
int V_1 = 0 ;
F_2 ( L_12 ) ;
F_48 ( & V_2 . V_56 ) ;
if ( V_49 == NULL || V_49 -> V_34 == NULL ) {
F_18 ( L_13 ) ;
V_1 = - V_66 ;
goto V_67;
}
V_1 = F_29 ( V_25 ) ;
if ( V_1 ) {
F_18 ( L_14 ) ;
goto V_67;
}
F_52 ( & V_2 . V_56 ) ;
return 0 ;
V_67:
F_52 ( & V_2 . V_56 ) ;
return V_1 ;
}
static void F_65 ( struct V_24 * V_25 )
{
F_2 ( L_15 ) ;
F_48 ( & V_2 . V_56 ) ;
F_43 ( V_25 ) ;
F_52 ( & V_2 . V_56 ) ;
}
static int F_66 ( struct V_24 * V_25 )
{
int V_1 = 0 ;
F_2 ( L_16 ) ;
F_48 ( & V_2 . V_56 ) ;
V_1 = F_24 ( V_25 ) ;
if ( V_1 ) {
F_18 ( L_14 ) ;
goto V_67;
}
F_52 ( & V_2 . V_56 ) ;
return 0 ;
V_67:
F_52 ( & V_2 . V_56 ) ;
return V_1 ;
}
static void F_67 ( struct V_24 * V_25 )
{
F_2 ( L_17 ) ;
F_48 ( & V_2 . V_56 ) ;
F_28 ( V_25 ) ;
F_52 ( & V_2 . V_56 ) ;
}
static int F_68 ( struct V_68 * V_3 )
{
struct V_69 * V_69 ;
V_69 = F_69 ( & V_3 -> V_4 , L_18 ) ;
if ( F_17 ( V_69 ) ) {
F_18 ( L_19 ) ;
return F_19 ( V_69 ) ;
}
V_2 . V_41 = V_69 ;
return 0 ;
}
static int F_70 ( struct V_24 * V_25 ,
struct V_24 * V_70 )
{
struct V_31 * V_32 ;
int V_1 ;
V_1 = F_15 () ;
if ( V_1 )
return V_1 ;
V_32 = F_71 ( V_25 -> V_50 ) ;
if ( ! V_32 )
return - V_66 ;
V_1 = F_72 ( V_32 , V_25 ) ;
if ( V_1 )
return V_1 ;
V_1 = F_73 ( V_25 , V_70 ) ;
if ( V_1 ) {
F_18 ( L_20 ,
V_70 -> V_71 ) ;
F_74 ( V_32 , V_25 ) ;
return V_1 ;
}
return 0 ;
}
static void F_75 ( struct V_24 * V_25 ,
struct V_24 * V_70 )
{
F_4 ( V_70 != V_25 -> V_70 ) ;
if ( V_70 != V_25 -> V_70 )
return;
F_76 ( V_25 ) ;
if ( V_25 -> V_34 )
F_74 ( V_25 -> V_34 , V_25 ) ;
}
static int F_77 ( struct V_24 * V_25 ,
T_3 * V_72 , int V_63 )
{
bool V_73 ;
int V_1 ;
V_73 = V_2 . V_28 == false ;
if ( V_73 ) {
V_1 = F_66 ( V_25 ) ;
if ( V_1 )
return V_1 ;
}
V_1 = F_60 ( V_72 , V_63 ) ;
if ( V_73 )
F_67 ( V_25 ) ;
return V_1 ;
}
static int F_78 ( struct V_24 * V_25 )
{
int V_1 ;
F_48 ( & V_2 . V_56 ) ;
if ( ! F_79 ( V_2 . V_35 . V_53 . V_57 ) ) {
V_1 = - V_74 ;
goto V_75;
}
V_1 = F_80 ( & V_2 . V_9 , true ) ;
if ( V_1 )
goto V_75;
F_52 ( & V_2 . V_56 ) ;
return 0 ;
V_75:
F_52 ( & V_2 . V_56 ) ;
return V_1 ;
}
static void F_81 ( struct V_24 * V_25 )
{
F_80 ( & V_2 . V_9 , false ) ;
}
static int F_82 ( struct V_24 * V_25 )
{
return F_83 ( & V_2 . V_9 , true ) ;
}
static void F_84 ( struct V_24 * V_25 )
{
F_83 ( & V_2 . V_9 , false ) ;
}
static bool F_85 ( struct V_24 * V_25 )
{
bool V_1 ;
F_48 ( & V_2 . V_56 ) ;
V_1 = F_79 ( V_2 . V_35 . V_53 . V_57 ) ;
F_52 ( & V_2 . V_56 ) ;
return V_1 ;
}
static int F_86 ( struct V_24 * V_25 ,
struct V_76 * V_77 )
{
int V_1 ;
T_2 V_78 = V_2 . V_35 . V_36 . V_39 ;
F_48 ( & V_2 . V_56 ) ;
if ( ! F_79 ( V_2 . V_35 . V_53 . V_57 ) ) {
V_1 = - V_74 ;
goto V_75;
}
V_1 = F_87 ( & V_2 . V_45 , & V_2 . V_9 , V_77 , V_78 ) ;
if ( V_1 )
goto V_75;
F_52 ( & V_2 . V_56 ) ;
return 0 ;
V_75:
F_52 ( & V_2 . V_56 ) ;
return V_1 ;
}
static int F_78 ( struct V_24 * V_25 )
{
return - V_74 ;
}
static void F_81 ( struct V_24 * V_25 )
{
}
static int F_82 ( struct V_24 * V_25 )
{
return - V_74 ;
}
static void F_84 ( struct V_24 * V_25 )
{
}
static bool F_85 ( struct V_24 * V_25 )
{
return false ;
}
static int F_86 ( struct V_24 * V_25 ,
struct V_76 * V_77 )
{
return - V_74 ;
}
static void F_88 ( struct V_68 * V_3 )
{
struct V_24 * V_49 = & V_2 . V_33 ;
V_49 -> V_4 = & V_3 -> V_4 ;
V_49 -> V_79 = V_80 ;
V_49 -> V_81 = V_82 ;
V_49 -> V_71 = L_21 ;
V_49 -> V_50 = V_83 ;
V_49 -> V_84 . V_2 = & V_85 ;
V_49 -> V_86 = V_87 ;
F_89 ( V_49 ) ;
}
static void T_4 F_90 ( struct V_68 * V_3 )
{
struct V_24 * V_49 = & V_2 . V_33 ;
F_91 ( V_49 ) ;
}
static int F_92 ( struct V_68 * V_3 )
{
struct V_88 * V_89 = V_3 -> V_4 . V_90 ;
struct V_88 * V_91 ;
int V_1 ;
V_91 = F_93 ( V_89 ) ;
if ( ! V_91 )
return 0 ;
V_1 = F_94 ( V_3 , V_91 , & V_2 . V_15 ) ;
if ( V_1 )
goto V_75;
F_95 ( V_91 ) ;
return 0 ;
V_75:
F_95 ( V_91 ) ;
return V_1 ;
}
static int F_96 ( struct V_68 * V_3 )
{
int V_1 ;
int V_6 ;
V_2 . V_3 = V_3 ;
F_97 ( & V_2 . V_56 ) ;
if ( V_3 -> V_4 . V_90 ) {
V_1 = F_92 ( V_3 ) ;
if ( V_1 )
return V_1 ;
}
V_1 = F_98 ( V_3 , & V_2 . V_9 ) ;
if ( V_1 )
return V_1 ;
V_1 = F_99 ( V_3 , & V_2 . V_40 ) ;
if ( V_1 )
return V_1 ;
V_1 = F_100 ( V_3 , & V_2 . V_15 ) ;
if ( V_1 )
return V_1 ;
V_1 = F_101 ( V_3 , & V_2 . V_45 ) ;
if ( V_1 )
return V_1 ;
V_1 = F_68 ( V_3 ) ;
if ( V_1 ) {
F_18 ( L_22 ) ;
return V_1 ;
}
V_6 = F_102 ( V_3 , 0 ) ;
if ( V_6 < 0 ) {
F_18 ( L_23 ) ;
return - V_66 ;
}
V_1 = F_103 ( & V_3 -> V_4 , V_6 ,
NULL , F_7 ,
V_92 , L_24 , & V_2 . V_9 ) ;
if ( V_1 ) {
F_18 ( L_25 ) ;
return V_1 ;
}
F_104 ( & V_3 -> V_4 ) ;
F_88 ( V_3 ) ;
F_105 ( L_26 , F_55 ) ;
return 0 ;
}
static int T_4 F_106 ( struct V_68 * V_3 )
{
F_90 ( V_3 ) ;
F_107 ( & V_3 -> V_4 ) ;
return 0 ;
}
static int F_108 ( struct V_93 * V_4 )
{
F_109 ( V_2 . V_41 ) ;
F_110 () ;
return 0 ;
}
static int F_111 ( struct V_93 * V_4 )
{
int V_1 ;
V_1 = F_112 () ;
if ( V_1 < 0 )
return V_1 ;
F_113 ( V_2 . V_41 ) ;
return 0 ;
}
int T_5 F_114 ( void )
{
return F_115 ( & V_94 ) ;
}
void T_4 F_116 ( void )
{
F_117 ( & V_94 ) ;
}
