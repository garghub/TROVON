static int F_1 ( int V_1 ,
unsigned int V_2 ,
unsigned short V_3 ,
unsigned short * V_4 )
{
unsigned int V_5 = 0 ;
int V_6 = 0 ;
int V_7 ;
int V_8 ;
unsigned char V_9 [ 1 ] ;
unsigned char V_10 = 0 ;
unsigned char V_11 = 0 ;
unsigned char V_12 = 0 ;
unsigned char V_13 = 0 ;
unsigned short V_14 = 0 ;
for ( V_7 = 0 ; V_7 < V_1 ;
V_7 ++ ) {
do {
V_5 =
F_2 ( V_2 +
V_15 ) ;
V_5 = V_5 & V_16 ;
} while ( V_5 == V_16 );
for ( V_6 = 0 ; V_6 < 2 ; V_6 ++ ) {
V_12 = ( V_3 + V_6 ) % 256 ;
V_13 = ( V_3 + V_6 ) / 256 ;
F_3 ( V_17 ,
V_2 + V_15 +
3 ) ;
do {
V_5 =
F_2 ( V_2 +
V_15 ) ;
V_5 = V_5 & V_16 ;
} while ( V_5 == V_16 );
F_3 ( V_12 ,
V_2 + V_15 +
2 ) ;
do {
V_5 =
F_2 ( V_2 +
V_15 ) ;
V_5 = V_5 & V_16 ;
} while ( V_5 == V_16 );
F_3 ( V_18 ,
V_2 + V_15 +
3 ) ;
do {
V_5 =
F_2 ( V_2 +
V_15 ) ;
V_5 = V_5 & V_16 ;
} while ( V_5 == V_16 );
F_3 ( V_13 ,
V_2 + V_15 +
2 ) ;
do {
V_5 =
F_2 ( V_2 +
V_15 ) ;
V_5 = V_5 & V_16 ;
} while ( V_5 == V_16 );
F_3 ( V_19 ,
V_2 + V_15 +
3 ) ;
do {
V_5 =
F_2 ( V_2 +
V_15 ) ;
V_5 = V_5 & V_16 ;
} while ( V_5 == V_16 );
* V_9 =
F_4 ( V_2 +
V_15 + 2 ) ;
do {
V_5 =
F_2 ( V_2 +
V_15 ) ;
V_5 = V_5 & V_16 ;
} while ( V_5 == V_16 );
if ( V_6 == 0 )
V_10 = V_9 [ 0 ] ;
else
V_11 = V_9 [ 0 ] ;
F_5 ( 1 ) ;
}
V_14 =
( V_10 | ( ( ( unsigned short ) V_11 ) *
256 ) ) ;
V_4 [ V_7 ] = V_14 ;
V_3 += 2 ;
}
return 0 ;
}
static void F_6 ( unsigned int V_2 ,
struct V_20 * V_21 )
{
unsigned short V_22 ;
unsigned short V_23 ;
unsigned short V_24 = 0 ;
unsigned short V_25 [ 2 ] ;
unsigned short V_26 = 0 ;
unsigned short V_27 = 0 ;
unsigned short V_28 = 0 ;
unsigned short V_29 = 0 ;
unsigned short V_30 = 0 ;
unsigned short V_31 = 0 ;
unsigned short V_32 = 0 ;
unsigned short V_33 = 0 ;
unsigned short V_34 = 0 ;
unsigned short V_35 [ 2 ] ;
unsigned short V_36 = 0 ;
unsigned short V_37 = 0 ;
F_1 ( 1 ,
V_2 , 0x116 ,
& V_22 ) ;
V_22 = V_22 + 0x100 ;
F_1 ( 1 ,
V_2 , V_22 + 0x02 ,
& V_24 ) ;
for ( V_26 = 0 ; V_26 < V_24 ;
V_26 ++ ) {
V_23 =
V_22 +
( V_27 * V_26 ) + 0x04 ;
F_1 ( 1 ,
V_2 , V_23 ,
& V_27 ) ;
V_27 = V_27 >> 4 ;
F_1 ( 1 ,
V_2 , V_23 + 0x06 ,
& V_28 ) ;
V_28 = V_28 >> 4 ;
F_1 ( 1 ,
V_2 , V_23 + 0x08 ,
& V_29 ) ;
V_29 = ( V_29 >> 3 ) & 0x1 ;
F_1 ( 1 ,
V_2 , V_23 + 0x44 ,
& V_30 ) ;
V_30 = V_30 & 0xFF ;
V_31 =
V_23 + 0x46 +
( ( ( V_30 / 16 ) + 1 ) * 2 ) +
( 6 * V_30 ) +
( 4 * ( ( ( V_30 / 16 ) + 1 ) * 2 ) ) ;
F_1 ( 1 ,
V_2 , V_31 ,
& V_32 ) ;
V_32 = V_32 >> 4 ;
V_34 = V_23 ;
for ( V_36 = 0 ; V_36 < V_30 ;
V_36 ++ ) {
F_1 ( 1 ,
V_2 , V_23 + 70 + ( 2 * ( 1 + ( V_30 / 16 ) ) ) + ( 0x02 * V_36 ) ,
& V_37 ) ;
V_21 -> V_38 [ V_26 ] .
V_37 [ V_36 ] = V_37 ;
# ifdef F_7
F_8 ( L_1 ,
V_21 -> V_38 [ V_26 ] .
V_37 [ V_36 ] ) ;
# endif
F_1 ( 2 ,
V_2 , V_23 + 70 + ( ( 2 * V_30 ) + ( 2 * ( 1 + ( V_30 / 16 ) ) ) ) + ( 0x04 * V_36 ) ,
V_35 ) ;
V_21 -> V_38 [ V_26 ] .
V_39 [ V_36 ] =
( V_35 [ 1 ] << 16 ) +
V_35 [ 0 ] ;
# ifdef F_7
F_8 ( L_2 , V_36 ,
V_21 -> V_38 [ V_26 ] .
V_39 [ V_36 ] ) ;
# endif
}
for ( V_33 = 0 ; V_33 < V_28 ; V_33 ++ ) {
F_1 ( 2 ,
V_2 ,
( V_33 * V_32 ) +
V_31 + 0x0C , V_25 ) ;
V_21 -> V_38 [ V_26 ] .
V_40 [ V_33 ] =
( V_25 [ 0 ] +
( ( V_25 [ 1 ] & 0xFFF ) << 16 ) ) ;
# ifdef F_7
F_8 ( L_3 , V_33 ,
V_21 -> V_38 [ V_26 ] .
V_40 [ V_33 ] ) ;
# endif
}
F_1 ( 2 ,
V_2 ,
( V_33 * V_32 ) + V_31 +
0x0C , V_25 ) ;
V_21 -> V_38 [ V_26 ] .
V_41 =
( V_25 [ 0 ] +
( ( V_25 [ 1 ] & 0xFFF ) << 16 ) ) ;
# ifdef F_7
F_8 ( L_4 ,
V_21 -> V_38 [ V_26 ] .
V_41 ) ;
# endif
}
}
static int F_9 ( struct V_42 * V_43 ,
unsigned int V_44 ,
unsigned int * V_45 ,
unsigned int * V_46 ,
unsigned int * V_47 )
{
int V_48 = 0 ;
int V_49 = 0 ;
#ifdef F_7
F_8 ( L_5 , V_44 ) ;
#endif
if ( V_50 [ V_43 -> V_51 ] . V_52 == 1 ) {
if ( V_44 <= 1 )
V_48 = V_44 , V_49 = 0 ;
else if ( ( V_44 >= 2 ) && ( V_44 <= 3 ) )
V_48 = V_44 - 2 , V_49 = 1 ;
else if ( ( V_44 >= 4 ) && ( V_44 <= 5 ) )
V_48 = V_44 - 4 , V_49 = 2 ;
else if ( ( V_44 >= 6 ) && ( V_44 <= 7 ) )
V_48 = V_44 - 6 , V_49 = 3 ;
} else {
if ( ( V_44 == 0 ) || ( V_44 == 1 ) )
V_48 = 0 , V_49 = 0 ;
else if ( ( V_44 == 2 ) || ( V_44 == 3 ) )
V_48 = 1 , V_49 = 0 ;
else if ( ( V_44 == 4 ) || ( V_44 == 5 ) )
V_48 = 0 , V_49 = 1 ;
else if ( ( V_44 == 6 ) || ( V_44 == 7 ) )
V_48 = 1 , V_49 = 1 ;
else if ( ( V_44 == 8 ) || ( V_44 == 9 ) )
V_48 = 0 , V_49 = 2 ;
else if ( ( V_44 == 10 ) || ( V_44 == 11 ) )
V_48 = 1 , V_49 = 2 ;
else if ( ( V_44 == 12 ) || ( V_44 == 13 ) )
V_48 = 0 , V_49 = 3 ;
else if ( ( V_44 == 14 ) || ( V_44 == 15 ) )
V_48 = 1 , V_49 = 3 ;
}
* V_45 =
V_50 [ V_43 -> V_51 ] . V_38 [ V_49 ] . V_41 ;
#ifdef F_7
F_8 ( L_6 , * V_45 ) ;
#endif
* V_46 =
V_50 [ V_43 -> V_51 ] . V_38 [ V_49 ] .
V_40 [ V_48 ] ;
#ifdef F_7
F_8 ( L_7 , * V_46 ) ;
#endif
* V_47 =
V_50 [ V_43 -> V_51 ] . V_38 [ V_49 ] .
V_39 [ V_50 [ V_43 -> V_51 ] . V_53 ] ;
#ifdef F_7
F_8 ( L_8 , * V_47 ) ;
#endif
return 0 ;
}
static int F_10 ( struct V_42 * V_43 ,
struct V_54 * V_55 ,
struct V_56 * V_57 ,
unsigned int * V_58 )
{
struct V_59 * V_60 = V_43 -> V_61 ;
V_58 [ 1 ] = F_2 ( V_60 -> V_62 ) & 0xf ;
return V_57 -> V_63 ;
}
static int F_11 ( struct V_42 * V_43 ,
struct V_54 * V_55 ,
struct V_56 * V_57 ,
unsigned int * V_58 )
{
struct V_59 * V_60 = V_43 -> V_61 ;
V_55 -> V_64 = F_2 ( V_60 -> V_65 ) & 0xf ;
if ( F_12 ( V_55 , V_58 ) )
F_13 ( V_55 -> V_64 , V_60 -> V_65 ) ;
V_58 [ 1 ] = V_55 -> V_64 ;
return V_57 -> V_63 ;
}
static int F_14 ( struct V_42 * V_43 ,
struct V_54 * V_55 ,
struct V_56 * V_57 ,
unsigned int * V_58 )
{
struct V_59 * V_60 = V_43 -> V_61 ;
unsigned int V_66 = 0 ;
unsigned int V_67 = 0 ;
unsigned int V_68 = 0 ;
V_67 = V_50 [ V_43 -> V_51 ] . V_69 ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( 0 | V_50 [ V_43 -> V_51 ] . V_69 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 0x4 ) ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( V_50 [ V_43 -> V_51 ] . V_72 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 36 ) ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( V_50 [ V_43 -> V_51 ] . V_73 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 32 ) ;
V_68 = V_67 | ( V_67 << 8 ) | 0x80000 ;
if ( V_50 [ V_43 -> V_51 ] . V_74 == V_75 ) {
V_68 = V_68 | 0x00100000 ;
}
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( V_68 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 8 ) ;
if ( V_50 [ V_43 -> V_51 ] . V_74 == V_76 ) {
do {
V_66 = F_2 ( V_60 -> V_70 +
V_50 [ V_43 -> V_51 ] . V_71 + 20 ) & 1 ;
} while ( V_66 != 1 );
V_58 [ 0 ] =
F_2 ( V_60 -> V_70 +
V_50 [ V_43 -> V_51 ] . V_71 + 28 ) ;
}
return 0 ;
}
static int F_15 ( struct V_42 * V_43 ,
unsigned int * V_58 )
{
struct V_59 * V_60 = V_43 -> V_61 ;
unsigned int V_77 = 0 , V_66 = 0 ;
unsigned int V_68 = 0 ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( V_50 [ V_43 -> V_51 ] . V_72 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 36 ) ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( V_50 [ V_43 -> V_51 ] . V_73 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 32 ) ;
V_77 = F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 12 ) ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( ( V_77 | 0x00020000 ) ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 12 ) ;
V_68 = 0 ;
if ( V_50 [ V_43 -> V_51 ] . V_74 == V_75 ) {
V_68 = V_68 | 0x00100000 ;
}
V_68 = V_68 | 0x00080000 ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( V_68 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 8 ) ;
if ( V_50 [ V_43 -> V_51 ] . V_74 == V_76 ) {
do {
V_66 = F_2 ( V_60 -> V_70 +
V_50 [ V_43 -> V_51 ] . V_71 + 20 ) & 1 ;
} while ( V_66 != 1 );
V_58 [ 0 ] =
F_2 ( V_60 -> V_70 +
V_50 [ V_43 -> V_51 ] . V_71 + 28 ) ;
}
return 0 ;
}
static int F_16 ( struct V_42 * V_43 ,
unsigned int * V_58 )
{
struct V_59 * V_60 = V_43 -> V_61 ;
unsigned int V_66 = 0 ;
int V_68 = 0 ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( V_50 [ V_43 -> V_51 ] . V_72 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 36 ) ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( V_50 [ V_43 -> V_51 ] . V_73 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 32 ) ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( 0x00040000 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 12 ) ;
V_68 = 0 ;
if ( V_50 [ V_43 -> V_51 ] . V_74 == V_75 ) {
V_68 = V_68 | 0x00100000 ;
}
V_68 = V_68 | 0x00080000 ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( V_68 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 8 ) ;
if ( V_50 [ V_43 -> V_51 ] . V_74 == V_76 ) {
do {
V_66 = F_2 ( V_60 -> V_70 +
V_50 [ V_43 -> V_51 ] . V_71 + 20 ) & 1 ;
} while ( V_66 != 1 );
V_58 [ 0 ] =
F_2 ( V_60 -> V_70 +
V_50 [ V_43 -> V_51 ] . V_71 + 28 ) ;
}
return 0 ;
}
static int F_17 ( struct V_42 * V_43 ,
unsigned int * V_58 )
{
struct V_59 * V_60 = V_43 -> V_61 ;
unsigned int V_66 = 0 ;
int V_68 = 0 ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( V_50 [ V_43 -> V_51 ] . V_72 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 36 ) ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( V_50 [ V_43 -> V_51 ] . V_73 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 32 ) ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( 0x00000400 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 4 ) ;
V_68 = 0 ;
if ( V_50 [ V_43 -> V_51 ] . V_74 == V_75 ) {
V_68 = V_68 | 0x00100000 ;
}
V_68 = V_68 | 0x00080000 ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( V_68 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 8 ) ;
if ( V_50 [ V_43 -> V_51 ] . V_74 == V_76 ) {
do {
V_66 = F_2 ( V_60 -> V_70 +
V_50 [ V_43 -> V_51 ] . V_71 + 20 ) & 1 ;
} while ( V_66 != 1 );
V_58 [ 0 ] =
F_2 ( V_60 -> V_70 +
V_50 [ V_43 -> V_51 ] . V_71 + 28 ) ;
}
return 0 ;
}
static int F_18 ( struct V_42 * V_43 ,
unsigned int * V_58 )
{
struct V_59 * V_60 = V_43 -> V_61 ;
unsigned int V_66 = 0 ;
int V_68 = 0 ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( V_50 [ V_43 -> V_51 ] . V_72 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 36 ) ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( V_50 [ V_43 -> V_51 ] . V_73 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 32 ) ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( 0x00000400 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 4 ) ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( 0x00020000 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 12 ) ;
V_68 = 0 ;
if ( V_50 [ V_43 -> V_51 ] . V_74 == V_75 ) {
V_68 = V_68 | 0x00100000 ;
}
V_68 = V_68 | 0x00080000 ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( V_68 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 8 ) ;
if ( V_50 [ V_43 -> V_51 ] . V_74 == V_76 ) {
do {
V_66 = F_2 ( V_60 -> V_70 +
V_50 [ V_43 -> V_51 ] . V_71 + 20 ) & 1 ;
} while ( V_66 != 1 );
V_58 [ 0 ] =
F_2 ( V_60 -> V_70 +
V_50 [ V_43 -> V_51 ] . V_71 + 28 ) ;
}
return 0 ;
}
static int F_19 ( struct V_42 * V_43 ,
unsigned int * V_58 )
{
struct V_59 * V_60 = V_43 -> V_61 ;
unsigned int V_66 = 0 ;
int V_68 = 0 ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( V_50 [ V_43 -> V_51 ] . V_72 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 36 ) ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( V_50 [ V_43 -> V_51 ] . V_73 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 32 ) ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( 0x00000400 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 4 ) ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( 0x00040000 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 12 ) ;
V_68 = 0 ;
if ( V_50 [ V_43 -> V_51 ] . V_74 == V_75 ) {
V_68 = V_68 | 0x00100000 ;
}
V_68 = V_68 | 0x00080000 ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( V_68 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 8 ) ;
if ( V_50 [ V_43 -> V_51 ] . V_74 == V_76 ) {
do {
V_66 = F_2 ( V_60 -> V_70 +
V_50 [ V_43 -> V_51 ] . V_71 + 20 ) & 1 ;
} while ( V_66 != 1 );
V_58 [ 0 ] =
F_2 ( V_60 -> V_70 +
V_50 [ V_43 -> V_51 ] . V_71 + 28 ) ;
}
return 0 ;
}
static int F_20 ( struct V_42 * V_43 )
{
struct V_59 * V_60 = V_43 -> V_61 ;
int V_78 ;
unsigned int V_79 ;
V_50 [ V_43 -> V_51 ] . V_74 = 0 ;
V_50 [ V_43 -> V_51 ] . V_80 = 0 ;
V_50 [ V_43 -> V_51 ] . V_81 = 0 ;
V_50 [ V_43 -> V_51 ] . V_82 = 0 ;
V_50 [ V_43 -> V_51 ] . V_83 = 0 ;
F_13 ( 0x83838383 , V_60 -> V_84 + 0x60 ) ;
V_79 = F_2 ( V_60 -> V_84 + 0x38 ) ;
F_13 ( V_79 | 0x2000 , V_60 -> V_84 + 0x38 ) ;
F_13 ( 0 , V_60 -> V_65 ) ;
for ( V_78 = 0 ; V_78 <= 95 ; V_78 ++ ) {
V_50 [ V_43 -> V_51 ] . V_85 [ V_78 ] = 0 ;
}
for ( V_78 = 0 ; V_78 <= 192 ; ) {
while ( ( ( F_2 ( V_60 -> V_70 + V_78 + 12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( 0 , V_60 -> V_70 + V_78 + 8 ) ;
V_78 = V_78 + 64 ;
}
return 0 ;
}
static int F_21 ( struct V_42 * V_43 ,
struct V_54 * V_55 ,
struct V_56 * V_57 ,
unsigned int * V_58 )
{
unsigned int V_86 = 0 ;
int V_87 ;
int V_8 = 0 ;
if ( V_50 [ V_43 -> V_51 ] . V_80 == 0 )
{
F_20 ( V_43 ) ;
return - V_88 ;
}
#ifdef F_7
F_8 ( L_9 , V_57 -> V_89 [ 0 ] ) ;
#endif
switch ( V_57 -> V_89 [ 0 ] ) {
case 0 :
F_14 ( V_43 , V_55 , V_57 ,
& V_86 ) ;
V_50 [ V_43 -> V_51 ] .
V_85 [ V_50 [ V_43 -> V_51 ] .
V_81 + 0 ] = V_86 ;
F_9 ( V_43 ,
V_50 [ V_43 -> V_51 ] . V_44 ,
& V_50 [ V_43 -> V_51 ] .
V_85 [ V_50 [ V_43 -> V_51 ] .
V_81 + 6 ] ,
& V_50 [ V_43 -> V_51 ] .
V_85 [ V_50 [ V_43 -> V_51 ] .
V_81 + 7 ] ,
& V_50 [ V_43 -> V_51 ] .
V_85 [ V_50 [ V_43 -> V_51 ] .
V_81 + 8 ] ) ;
#ifdef F_7
F_8 ( L_10 , V_50 [ V_43 -> V_51 ] . V_85 [ V_50 [ V_43 -> V_51 ] . V_81 + 6 ] ) ;
F_8 ( L_11 , V_50 [ V_43 -> V_51 ] . V_85 [ V_50 [ V_43 -> V_51 ] . V_81 + 7 ] ) ;
F_8 ( L_12 , V_50 [ V_43 -> V_51 ] . V_85 [ V_50 [ V_43 -> V_51 ] . V_81 + 8 ] ) ;
#endif
if ( ( V_50 [ V_43 -> V_51 ] . V_90 == 2 )
&& ( V_50 [ V_43 -> V_51 ] . V_74 == FALSE )
&& ( V_50 [ V_43 -> V_51 ] . V_91 == 1 ) )
{
F_17 ( V_43 , & V_86 ) ;
V_50 [ V_43 -> V_51 ] .
V_85 [ V_50 [ V_43 ->
V_51 ] . V_81 + 3 ] = V_86 ;
}
else {
V_50 [ V_43 -> V_51 ] .
V_85 [ V_50 [ V_43 ->
V_51 ] . V_81 + 3 ] = 0 ;
}
if ( ( V_50 [ V_43 -> V_51 ] . V_92 == FALSE )
&& ( V_50 [ V_43 -> V_51 ] . V_74 == FALSE ) )
{
F_15 ( V_43 ,
& V_86 ) ;
V_50 [ V_43 -> V_51 ] .
V_85 [ V_50 [ V_43 ->
V_51 ] . V_81 + 1 ] = V_86 ;
F_16 ( V_43 ,
& V_86 ) ;
V_50 [ V_43 -> V_51 ] .
V_85 [ V_50 [ V_43 ->
V_51 ] . V_81 + 2 ] = V_86 ;
}
if ( ( V_50 [ V_43 -> V_51 ] . V_90 == 2 )
&& ( V_50 [ V_43 -> V_51 ] . V_74 == FALSE )
&& ( V_50 [ V_43 -> V_51 ] . V_91 == 1 ) )
{
if ( V_50 [ V_43 -> V_51 ] . V_93 !=
V_50 [ V_43 -> V_51 ] . V_94 )
{
V_87 = 1 ;
}
else {
if ( V_50 [ V_43 -> V_51 ] . V_95 ==
V_50 [ V_43 -> V_51 ] . V_53 )
{
V_87 = 0 ;
}
else {
V_87 = 1 ;
}
}
if ( V_87 == 1 ) {
F_18 ( V_43 ,
& V_86 ) ;
V_50 [ V_43 -> V_51 ] .
V_85 [ V_50
[ V_43 -> V_51 ] . V_81 + 4 ] =
V_86 ;
F_19 ( V_43 , & V_86 ) ;
V_50 [ V_43 -> V_51 ] .
V_85 [ V_50
[ V_43 -> V_51 ] . V_81 + 5 ] =
V_86 ;
}
else {
V_50 [ V_43 -> V_51 ] .
V_85 [ V_50
[ V_43 -> V_51 ] . V_81 + 4 ] = 0 ;
V_50 [ V_43 -> V_51 ] .
V_85 [ V_50
[ V_43 -> V_51 ] . V_81 + 5 ] = 0 ;
}
}
if ( V_50 [ V_43 -> V_51 ] . V_96 != 1 ) {
V_50 [ V_43 -> V_51 ] . V_81 = 0 ;
}
else {
V_50 [ V_43 -> V_51 ] . V_81 =
V_50 [ V_43 -> V_51 ] . V_81 + 9 ;
}
if ( ( V_50 [ V_43 -> V_51 ] . V_96 == 1 )
&& ( V_50 [ V_43 -> V_51 ] . V_74 == 1 ) ) {
V_50 [ V_43 -> V_51 ] . V_81 =
V_50 [ V_43 -> V_51 ] . V_81 - 9 ;
}
if ( V_50 [ V_43 -> V_51 ] . V_96 == 0 ) {
#ifdef F_7
F_8 ( L_13 ) ;
#endif
V_58 [ 0 ] =
V_50 [ V_43 -> V_51 ] .
V_85 [ 0 ] ;
V_58 [ 1 ] =
V_50 [ V_43 -> V_51 ] .
V_85 [ 1 ] ;
V_58 [ 2 ] =
V_50 [ V_43 -> V_51 ] .
V_85 [ 2 ] ;
V_58 [ 3 ] =
V_50 [ V_43 -> V_51 ] .
V_85 [ 3 ] ;
V_58 [ 4 ] =
V_50 [ V_43 -> V_51 ] .
V_85 [ 4 ] ;
V_58 [ 5 ] =
V_50 [ V_43 -> V_51 ] .
V_85 [ 5 ] ;
F_9 ( V_43 ,
V_50 [ V_43 -> V_51 ] . V_44 ,
& V_58 [ 6 ] , & V_58 [ 7 ] , & V_58 [ 8 ] ) ;
}
break;
case 1 :
for ( V_8 = 0 ; V_8 < V_57 -> V_63 ; V_8 ++ ) {
V_58 [ V_8 ] =
V_50 [ V_43 -> V_51 ] .
V_85 [ V_8 ] ;
}
V_50 [ V_43 -> V_51 ] . V_81 = 0 ;
V_50 [ V_43 -> V_51 ] . V_82 = 0 ;
if ( V_50 [ V_43 -> V_51 ] . V_96 == 1 ) {
V_50 [ V_43 -> V_51 ] . V_80 = 0 ;
V_50 [ V_43 -> V_51 ] . V_74 = 0 ;
}
break;
default:
F_8 ( L_14 ) ;
F_20 ( V_43 ) ;
return - V_88 ;
}
return V_57 -> V_63 ;
}
static int F_22 ( struct V_42 * V_43 ,
struct V_54 * V_55 ,
struct V_56 * V_57 ,
unsigned int * V_58 )
{
struct V_59 * V_60 = V_43 -> V_61 ;
unsigned int V_97 = 0 , V_98 = 0 ;
unsigned int V_67 = 0 ;
unsigned int V_99 = 0 ;
int V_100 = 0 ;
#ifdef F_7
int V_8 = 0 , V_101 = 0 ;
#endif
if ( V_50 [ V_43 -> V_51 ] . V_83 != 1 ) {
V_50 [ V_43 -> V_51 ] . V_91 = 1 ;
V_50 [ V_43 -> V_51 ] . V_93 = 0 ;
V_50 [ V_43 -> V_51 ] . V_95 = 2 ;
V_50 [ V_43 -> V_51 ] . V_74 = 0 ;
V_50 [ V_43 -> V_51 ] . V_92 = 0 ;
V_50 [ V_43 -> V_51 ] . V_102 = 0 ;
V_50 [ V_43 -> V_51 ] . V_82 = 0 ;
V_50 [ V_43 -> V_51 ] . V_44 = 0 ;
V_50 [ V_43 -> V_51 ] . V_81 = 0 ;
V_50 [ V_43 -> V_51 ] . V_80 = 0 ;
V_50 [ V_43 -> V_51 ] . V_83 = 1 ;
V_50 [ V_43 -> V_51 ] . V_52 = 0 ;
memset ( V_50 [ V_43 -> V_51 ] . V_38 , 0 ,
sizeof( V_50 [ V_43 -> V_51 ] . V_38 [ V_103 ] ) ) ;
F_6 ( V_60 -> V_84 ,
& V_50 [ V_43 -> V_51 ] ) ;
#ifdef F_7
for ( V_8 = 0 ; V_8 < V_103 ; V_8 ++ ) {
F_8 ( L_15 , V_8 ,
V_50 [ V_43 -> V_51 ] . V_38 [ V_8 ] .
V_41 ) ;
for ( V_101 = 0 ; V_101 < 5 ; V_101 ++ ) {
F_8 ( L_16 , V_8 , V_101 , V_50 [ V_43 -> V_51 ] . V_38 [ V_8 ] . V_40 [ V_101 ] ) ;
}
for ( V_101 = 0 ; V_101 < 8 ; V_101 ++ ) {
F_8 ( L_17 , V_8 , V_101 , V_50 [ V_43 -> V_51 ] . V_38 [ V_8 ] . V_39 [ V_101 ] ) ;
}
for ( V_101 = 0 ; V_101 < 8 ; V_101 ++ ) {
F_8 ( L_18 ,
V_8 , V_101 ,
V_50 [ V_43 -> V_51 ] . V_38 [ V_8 ] .
V_37 [ V_101 ] ) ;
}
}
#endif
}
if ( V_58 [ 0 ] != 0 && V_58 [ 0 ] != 1 && V_58 [ 0 ] != 2 ) {
F_8 ( L_19 ) ;
V_100 ++ ;
}
if ( V_58 [ 0 ] == 1 ) {
if ( V_58 [ 14 ] != 0 && V_58 [ 14 ] != 1 && V_58 [ 14 ] != 2
&& V_58 [ 14 ] != 4 ) {
F_8 ( L_20 ) ;
V_100 ++ ;
}
}
if ( V_58 [ 1 ] < 0 || V_58 [ 1 ] > 7 ) {
F_8 ( L_21 ) ;
V_100 ++ ;
}
if ( V_58 [ 2 ] != 0 && V_58 [ 2 ] != 1 ) {
F_8 ( L_22 ) ;
V_100 ++ ;
}
if ( V_58 [ 3 ] != 0 ) {
F_8 ( L_23 ) ;
V_100 ++ ;
}
if ( V_58 [ 4 ] != 0 && V_58 [ 4 ] != 1 ) {
F_8 ( L_24 ) ;
V_100 ++ ;
}
if ( V_58 [ 5 ] != 0 && V_58 [ 5 ] != 1 ) {
F_8 ( L_25 ) ;
V_100 ++ ;
}
if ( V_58 [ 8 ] != 0 && V_58 [ 8 ] != 1 && V_58 [ 2 ] != 2 ) {
F_8 ( L_26 ) ;
}
if ( V_58 [ 12 ] == 0 || V_58 [ 12 ] == 1 ) {
if ( V_58 [ 6 ] != 20 && V_58 [ 6 ] != 40 && V_58 [ 6 ] != 80
&& V_58 [ 6 ] != 160 ) {
F_8 ( L_27 ) ;
V_100 ++ ;
}
if ( V_58 [ 7 ] != 2 ) {
F_8 ( L_28 ) ;
V_100 ++ ;
}
}
if ( V_58 [ 9 ] != 0 && V_58 [ 9 ] != 1 ) {
F_8 ( L_29 ) ;
V_100 ++ ;
}
if ( V_58 [ 11 ] < 0 || V_58 [ 11 ] > 4 ) {
F_8 ( L_30 ) ;
V_100 ++ ;
}
if ( V_58 [ 12 ] < 0 || V_58 [ 12 ] > 3 ) {
F_8 ( L_31 ) ;
V_100 ++ ;
}
if ( V_58 [ 13 ] < 0 || V_58 [ 13 ] > 16 ) {
F_8 ( L_32 ) ;
V_100 ++ ;
}
V_50 [ V_43 -> V_51 ] . V_102 = V_58 [ 13 ] ;
V_50 [ V_43 -> V_51 ] . V_96 = V_58 [ 12 ] ;
V_50 [ V_43 -> V_51 ] . V_94 = V_58 [ 2 ] ;
V_50 [ V_43 -> V_51 ] . V_53 = V_58 [ 1 ] ;
V_50 [ V_43 -> V_51 ] . V_73 = V_58 [ 6 ] ;
V_50 [ V_43 -> V_51 ] . V_72 = V_58 [ 7 ] ;
V_50 [ V_43 -> V_51 ] . V_90 = V_58 [ 0 ] ;
V_50 [ V_43 -> V_51 ] . V_52 = V_58 [ 5 ] ;
memset ( V_50 [ V_43 -> V_51 ] . V_104 , 0 , ( 7 + 12 ) * sizeof( unsigned int ) ) ;
while ( V_50 [ V_43 -> V_51 ] . V_74 == 1 ) {
#ifndef F_23
F_24 ( 1 ) ;
#else
F_8 ( L_33 ) ;
#endif
}
V_67 = F_25 ( V_57 -> V_105 ) ;
V_50 [ V_43 -> V_51 ] . V_69 = V_67 ;
V_50 [ V_43 -> V_51 ] . V_44 = V_67 ;
if ( V_58 [ 5 ] == 0 ) {
if ( V_67 < 0 || V_67 > 15 ) {
F_8 ( L_34 ) ;
V_100 ++ ;
}
}
else {
if ( V_58 [ 14 ] == 2 ) {
if ( V_67 < 0 || V_67 > 3 ) {
F_8 ( L_34 ) ;
V_100 ++ ;
}
}
else {
if ( V_67 < 0 || V_67 > 7 ) {
F_8 ( L_34 ) ;
V_100 ++ ;
}
}
}
if ( V_58 [ 12 ] == 0 || V_58 [ 12 ] == 1 ) {
switch ( V_58 [ 5 ] ) {
case 0 :
if ( V_67 >= 0 && V_67 <= 3 ) {
V_50 [ V_43 -> V_51 ] . V_71 = 0 ;
}
if ( V_67 >= 4 && V_67 <= 7 ) {
V_50 [ V_43 -> V_51 ] . V_71 = 64 ;
}
if ( V_67 >= 8 && V_67 <= 11 ) {
V_50 [ V_43 -> V_51 ] . V_71 = 128 ;
}
if ( V_67 >= 12 && V_67 <= 15 ) {
V_50 [ V_43 -> V_51 ] . V_71 = 192 ;
}
break;
case 1 :
if ( V_58 [ 14 ] == 2 ) {
if ( V_67 == 0 ) {
V_50 [ V_43 -> V_51 ] . V_71 = 0 ;
}
if ( V_67 == 1 ) {
V_50 [ V_43 -> V_51 ] . V_71 = 64 ;
}
if ( V_67 == 2 ) {
V_50 [ V_43 -> V_51 ] . V_71 = 128 ;
}
if ( V_67 == 3 ) {
V_50 [ V_43 -> V_51 ] . V_71 = 192 ;
}
V_50 [ V_43 -> V_51 ] . V_69 = 0 ;
V_67 = 0 ;
break;
}
if ( V_67 >= 0 && V_67 <= 1 ) {
V_50 [ V_43 -> V_51 ] . V_71 = 0 ;
}
if ( V_67 >= 2 && V_67 <= 3 ) {
V_50 [ V_43 -> V_51 ] . V_69 =
V_50 [ V_43 -> V_51 ] . V_69 -
2 ;
V_50 [ V_43 -> V_51 ] . V_71 = 64 ;
V_67 = V_67 - 2 ;
}
if ( V_67 >= 4 && V_67 <= 5 ) {
V_50 [ V_43 -> V_51 ] . V_69 =
V_50 [ V_43 -> V_51 ] . V_69 -
4 ;
V_50 [ V_43 -> V_51 ] . V_71 = 128 ;
V_67 = V_67 - 4 ;
}
if ( V_67 >= 6 && V_67 <= 7 ) {
V_50 [ V_43 -> V_51 ] . V_69 =
V_50 [ V_43 -> V_51 ] . V_69 -
6 ;
V_50 [ V_43 -> V_51 ] . V_71 = 192 ;
V_67 = V_67 - 6 ;
}
break;
default:
F_8 ( L_35 ) ;
V_100 ++ ;
}
}
else {
switch ( V_58 [ 11 ] ) {
case 1 :
V_50 [ V_43 -> V_51 ] . V_71 = 0 ;
break;
case 2 :
V_50 [ V_43 -> V_51 ] . V_71 = 64 ;
break;
case 3 :
V_50 [ V_43 -> V_51 ] . V_71 = 128 ;
break;
case 4 :
V_50 [ V_43 -> V_51 ] . V_71 = 192 ;
break;
default:
F_8 ( L_36 ) ;
V_100 ++ ;
}
}
if ( V_100 ) {
F_20 ( V_43 ) ;
return - V_88 ;
}
if ( V_50 [ V_43 -> V_51 ] . V_96 != 1 ) {
V_50 [ V_43 -> V_51 ] . V_81 = 0 ;
V_50 [ V_43 -> V_51 ] . V_82 = 0 ;
}
V_97 =
V_58 [ 1 ] | ( V_58 [ 2 ] << 6 ) | ( V_58 [ 5 ] << 7 ) | ( V_58 [ 3 ] << 8 ) |
( V_58 [ 4 ] << 9 ) ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( 0 | V_67 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 0x4 ) ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( 0 , V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 0x0 ) ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( V_97 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 0x0 ) ;
V_98 = F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 12 ) ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( ( V_98 & 0xFFF9FFFF ) ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 12 ) ;
if ( V_58 [ 9 ] == 1 ) {
V_60 -> V_106 = V_107 ;
V_50 [ V_43 -> V_51 ] . V_74 = 1 ;
}
else {
V_50 [ V_43 -> V_51 ] . V_74 = 0 ;
}
V_50 [ V_43 -> V_51 ] . V_80 = 1 ;
if ( V_50 [ V_43 -> V_51 ] . V_96 == 1 )
{
V_50 [ V_43 -> V_51 ] . V_82 =
V_50 [ V_43 -> V_51 ] . V_82 + 1 ;
V_57 -> V_89 [ 0 ] = 0 ;
F_21 ( V_43 , V_55 , V_57 , & V_99 ) ;
}
return V_57 -> V_63 ;
}
static int F_26 ( struct V_42 * V_43 ,
struct V_54 * V_55 ,
struct V_56 * V_57 ,
unsigned int * V_58 )
{
struct V_59 * V_60 = V_43 -> V_61 ;
unsigned int V_108 = 0 ;
int V_78 ;
if ( V_50 [ V_43 -> V_51 ] . V_80 == 0 ) {
F_20 ( V_43 ) ;
return - V_88 ;
}
if ( V_58 [ 0 ] != 0 && V_58 [ 0 ] != 1 ) {
F_8 ( L_26 ) ;
F_20 ( V_43 ) ;
return - V_88 ;
}
if ( V_58 [ 0 ] == 1 )
{
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] .
V_71 + 12 ) >> 19 ) & 1 ) !=
1 ) ;
F_13 ( ( 0x00001000 | V_50 [ V_43 -> V_51 ] . V_69 ) ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
4 ) ;
V_78 = V_50 [ V_43 -> V_51 ] . V_74 ;
V_50 [ V_43 -> V_51 ] . V_74 = V_76 ;
F_14 ( V_43 , V_55 , V_57 , V_58 ) ;
if ( V_50 [ V_43 -> V_51 ] . V_92 == FALSE ) {
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] .
V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( ( 0x00001000 | V_50 [ V_43 -> V_51 ] .
V_69 ) ,
V_60 -> V_70 +
V_50 [ V_43 -> V_51 ] . V_71 + 4 ) ;
V_58 ++ ;
F_15 ( V_43 , V_58 ) ;
V_58 ++ ;
F_16 ( V_43 , V_58 ) ;
}
} else {
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] .
V_71 + 12 ) >> 19 ) & 1 ) !=
1 ) ;
F_13 ( ( 0x00000800 | V_50 [ V_43 -> V_51 ] . V_69 ) ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
4 ) ;
V_108 =
F_2 ( V_60 -> V_70 +
V_50 [ V_43 -> V_51 ] . V_71 + 0 ) ;
V_78 = V_50 [ V_43 -> V_51 ] . V_74 ;
V_50 [ V_43 -> V_51 ] . V_74 = V_76 ;
F_14 ( V_43 , V_55 , V_57 , V_58 ) ;
if ( V_50 [ V_43 -> V_51 ] . V_92 == FALSE ) {
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] .
V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( ( 0x00000800 | V_50 [ V_43 -> V_51 ] .
V_69 ) ,
V_60 -> V_70 +
V_50 [ V_43 -> V_51 ] . V_71 + 4 ) ;
V_58 ++ ;
F_15 ( V_43 , V_58 ) ;
V_58 ++ ;
F_16 ( V_43 , V_58 ) ;
}
}
V_50 [ V_43 -> V_51 ] . V_74 = V_78 ;
return V_57 -> V_63 ;
}
static int F_27 ( struct V_42 * V_43 ,
struct V_54 * V_55 ,
struct V_56 * V_57 ,
unsigned int * V_58 )
{
F_20 ( V_43 ) ;
return V_57 -> V_63 ;
}
static int F_28 ( struct V_42 * V_43 ,
struct V_54 * V_55 ,
struct V_109 * V_110 )
{
int V_111 = 0 ;
unsigned int V_112 = 0 ;
unsigned int V_113 = 0 ;
unsigned int V_114 = 0 ;
unsigned int V_115 = 0 ;
int V_116 = 0 ;
int V_117 = 0 ;
double V_118 = 0.0 ;
double V_119 = 0.0 ;
unsigned int V_120 ;
V_111 |= F_29 ( & V_110 -> V_121 , V_122 | V_123 ) ;
V_111 |= F_29 ( & V_110 -> V_124 ,
V_125 | V_126 ) ;
V_111 |= F_29 ( & V_110 -> V_127 , V_125 ) ;
V_111 |= F_29 ( & V_110 -> V_128 , V_129 ) ;
V_111 |= F_29 ( & V_110 -> V_130 , V_129 | V_131 ) ;
if ( V_50 [ V_43 -> V_51 ] . V_74 == 0 )
V_111 |= - V_88 ;
if ( V_111 ) {
F_20 ( V_43 ) ;
return 1 ;
}
V_111 |= F_30 ( & V_110 -> V_121 ) ;
V_111 |= F_30 ( & V_110 -> V_124 ) ;
V_111 |= F_30 ( & V_110 -> V_130 ) ;
if ( V_111 ) {
F_20 ( V_43 ) ;
return 2 ;
}
switch ( V_110 -> V_121 ) {
case V_122 :
V_111 |= F_31 ( & V_110 -> V_132 , 0 ) ;
break;
case V_123 :
V_120 = V_110 -> V_132 & 0xffff ;
if ( V_120 < 1 || V_120 > 3 ) {
V_110 -> V_132 &= ~ 0xffff ;
V_110 -> V_132 |= 1 ;
V_111 |= - V_88 ;
}
V_120 = V_110 -> V_132 >> 16 ;
if ( V_120 != 2 ) {
V_110 -> V_132 &= ~ ( 0xffff << 16 ) ;
V_110 -> V_132 |= ( 2 << 16 ) ;
V_111 |= - V_88 ;
}
break;
}
V_111 |= F_31 ( & V_110 -> V_133 , V_110 -> V_134 ) ;
V_50 [ V_43 -> V_51 ] . V_135 = V_110 -> V_136 [ 0 ] ;
V_50 [ V_43 -> V_51 ] . V_137 = V_110 -> V_136 [ 1 ] ;
if ( V_110 -> V_127 == V_125 ) {
V_112 = V_110 -> V_138 & 0xFFFF ;
V_113 = V_110 -> V_138 >> 16 ;
if ( V_112 != 20 && V_112 != 40
&& V_112 != 80 && V_112 != 160 )
{
F_8 ( L_27 ) ;
V_111 ++ ;
}
if ( V_113 != 2 ) {
F_8 ( L_28 ) ;
V_111 ++ ;
}
} else {
V_112 = 0 ;
V_113 = 0 ;
}
if ( V_110 -> V_124 == V_126 ) {
V_114 = 0 ;
V_115 = 0 ;
}
else {
V_114 = V_110 -> V_139 & 0xFFFF ;
V_115 = V_110 -> V_139 >> 16 ;
if ( V_115 != 2 && V_115 != 3 ) {
V_111 ++ ;
F_8 ( L_37 ) ;
}
if ( V_114 < 1 || V_114 > 1023 ) {
V_111 ++ ;
F_8 ( L_38 ) ;
}
if ( V_111 ) {
F_20 ( V_43 ) ;
return 3 ;
}
F_32 () ;
V_119 = ( double ) V_114 ;
V_116 =
V_50 [ V_43 -> V_51 ] . V_137 -
V_50 [ V_43 -> V_51 ] . V_135 + 4 ;
V_118 =
( double ) ( ( double ) V_112 /
( double ) V_116 ) ;
V_118 =
( double ) 1.0 / V_118 ;
V_113 = 3 ;
if ( V_115 <= V_113 ) {
for ( V_117 = 0 ;
V_117 < ( V_113 - V_115 ) ;
V_117 ++ ) {
V_118 =
V_118 * 1000 ;
V_118 =
V_118 + 1 ;
}
} else {
for ( V_117 = 0 ;
V_117 < ( V_115 - V_113 ) ;
V_117 ++ ) {
V_119 = V_119 * 1000 ;
}
}
if ( V_118 >= V_119 ) {
F_8 ( L_39 ) ;
V_111 ++ ;
}
F_33 () ;
}
if ( V_111 ) {
F_20 ( V_43 ) ;
return 4 ;
}
return 0 ;
}
static int F_34 ( struct V_42 * V_43 ,
struct V_54 * V_55 )
{
struct V_59 * V_60 = V_43 -> V_61 ;
unsigned int V_108 = 0 ;
V_50 [ V_43 -> V_51 ] . V_74 = 0 ;
V_50 [ V_43 -> V_51 ] . V_80 = 0 ;
V_50 [ V_43 -> V_51 ] . V_81 = 0 ;
V_50 [ V_43 -> V_51 ] . V_82 = 0 ;
V_108 =
F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 8 ) ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( ( V_108 & 0xFFE7FFFF ) ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 8 ) ;
return 0 ;
}
static int F_35 ( struct V_42 * V_43 ,
struct V_54 * V_55 )
{
struct V_59 * V_60 = V_43 -> V_61 ;
struct V_109 * V_110 = & V_55 -> V_140 -> V_110 ;
unsigned int V_108 = 0 ;
unsigned int V_141 = 0 ;
unsigned int V_142 = 0 ;
unsigned int V_143 = 0 ;
unsigned int V_144 = 0 ;
unsigned int V_112 = 0 ;
unsigned int V_113 = 0 ;
unsigned int V_114 = 0 ;
unsigned int V_115 = 0 ;
unsigned int V_145 = 0 ;
V_50 [ V_43 -> V_51 ] . V_135 = V_110 -> V_136 [ 0 ] ;
V_50 [ V_43 -> V_51 ] . V_137 = V_110 -> V_136 [ 1 ] ;
if ( V_110 -> V_121 == V_123 ) {
V_141 = 1 ;
V_142 = V_110 -> V_132 & 0xFFFF ;
V_143 = V_110 -> V_132 >> 16 ;
}
else {
V_141 = 0 ;
}
if ( V_110 -> V_130 == V_129 ) {
V_144 = 0 ;
}
else {
V_144 = 2 ;
}
if ( V_110 -> V_124 == V_126 ) {
V_114 = 0 ;
V_115 = 0 ;
V_145 = 0 ;
}
else {
V_114 = V_110 -> V_139 & 0xFFFF ;
V_115 = V_110 -> V_139 >> 16 ;
V_145 = 1 ;
}
if ( V_110 -> V_127 == V_125 ) {
V_112 = V_110 -> V_138 & 0xFFFF ;
V_113 = V_110 -> V_138 >> 16 ;
} else {
V_112 = 0 ;
V_113 = 0 ;
}
V_108 =
F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 12 ) ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( ( V_108 & 0xFFC00000 ) ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 12 ) ;
V_108 = 0 ;
V_108 =
V_50 [ V_43 -> V_51 ] . V_135 | ( V_50 [ V_43 ->
V_51 ] .
V_137 << 8 ) | 0x00100000 | ( V_141 << 24 ) |
( V_142 << 25 ) | ( V_143 << 27 ) | ( V_145
<< 18 ) | ( V_144 << 16 ) ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( V_108 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 0x8 ) ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( V_114 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 40 ) ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( V_115 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 44 ) ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( V_112 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 32 ) ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( V_113 ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 36 ) ;
V_108 =
F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 4 ) ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( ( ( V_108 & 0x1E0FF ) | 0x00002000 ) ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 4 ) ;
V_108 = 0 ;
V_108 =
F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 8 ) ;
while ( ( ( F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 +
12 ) >> 19 ) & 1 ) != 1 ) ;
F_13 ( ( V_108 | 0x00080000 ) ,
V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 8 ) ;
return 0 ;
}
static int F_36 ( struct V_42 * V_43 )
{
struct V_59 * V_60 = V_43 -> V_61 ;
struct V_54 * V_55 = V_43 -> V_146 ;
unsigned int V_147 = 0 ;
int V_63 = 0 , V_8 = 0 ;
V_147 =
F_2 ( V_60 -> V_70 + V_50 [ V_43 -> V_51 ] . V_71 + 16 ) ;
if ( ( V_147 & 0x2 ) == 0x2 ) {
V_50 [ V_43 -> V_51 ] . V_104 [ V_50 [ V_43 ->
V_51 ] . V_81 ] =
F_2 ( V_60 -> V_70 +
V_50 [ V_43 -> V_51 ] . V_71 + 28 ) ;
if ( ( V_50 [ V_43 -> V_51 ] . V_81 ==
( V_50 [ V_43 -> V_51 ] . V_137 -
V_50 [ V_43 -> V_51 ] .
V_135 + 3 ) ) ) {
V_50 [ V_43 -> V_51 ] . V_81 ++ ;
for ( V_8 = V_50 [ V_43 -> V_51 ] . V_135 ;
V_8 <= V_50 [ V_43 -> V_51 ] . V_137 ;
V_8 ++ ) {
F_9 ( V_43 , V_8 ,
& V_50 [ V_43 -> V_51 ] .
V_104 [ V_50 [ V_43 ->
V_51 ] . V_81 + ( ( V_8 -
V_50
[ V_43 -> V_51 ] .
V_135 )
* 3 ) ] ,
& V_50 [ V_43 -> V_51 ] .
V_104 [ V_50 [ V_43 ->
V_51 ] . V_81 + ( ( V_8 -
V_50
[ V_43 -> V_51 ] .
V_135 )
* 3 ) + 1 ] ,
& V_50 [ V_43 -> V_51 ] .
V_104 [ V_50 [ V_43 ->
V_51 ] . V_81 + ( ( V_8 -
V_50
[ V_43 -> V_51 ] .
V_135 )
* 3 ) + 2 ] ) ;
}
V_50 [ V_43 -> V_51 ] . V_81 = - 1 ;
V_55 -> V_140 -> V_148 |= V_149 ;
V_63 = F_37 ( V_55 ,
( 7 + 12 ) * sizeof( unsigned int ) ) ;
if ( V_63 > ( ( 7 + 12 ) * sizeof( unsigned int ) ) ) {
F_8 ( L_40 , V_63 ) ;
V_55 -> V_140 -> V_148 |= V_150 ;
}
F_38 ( V_55 , 0 ,
( unsigned int * ) V_50 [ V_43 -> V_51 ] .
V_104 , ( 7 + 12 ) * sizeof( unsigned int ) ) ;
F_39 ( V_55 ,
( 7 + 12 ) * sizeof( unsigned int ) ) ;
F_40 ( V_43 , V_55 ) ;
* /
}
V_50 [ V_43 -> V_51 ] . V_81 ++ ;
}
V_50 [ V_43 -> V_51 ] . V_74 = 0 ;
return 0 ;
}
static void F_41 ( int V_151 , void * V_152 )
{
struct V_42 * V_43 = V_152 ;
struct V_59 * V_60 = V_43 -> V_61 ;
unsigned int V_147 = 0 ;
unsigned int V_153 = 0 ;
int V_154 = 0 ;
int V_155 = 0 ;
unsigned int V_86 = 0 ;
unsigned int V_156 = 0 ;
unsigned int V_157 = 0 ;
int V_87 ;
int V_158 = 0 ;
switch ( V_50 [ V_43 -> V_51 ] . V_96 ) {
case 0 :
case 1 :
switch ( V_50 [ V_43 -> V_51 ] . V_90 ) {
case 0 :
case 1 :
V_147 =
F_2 ( V_60 -> V_70 +
V_50 [ V_43 -> V_51 ] . V_71 + 16 ) ;
if ( ( V_147 & 0x2 ) == 0x2 ) {
V_154 =
( ( F_2 ( V_60 -> V_70 +
V_50 [ V_43 ->
V_51 ] .
V_71 +
12 ) & 0x00060000 ) >>
17 ) ;
V_157 =
F_2 ( V_60 -> V_70 +
V_50 [ V_43 -> V_51 ] . V_71 + 28 ) ;
if ( V_154 == 0 ) {
V_50 [ V_43 -> V_51 ] .
V_85
[ V_50 [ V_43 -> V_51 ] .
V_81 + 0 ] = V_157 ;
F_15
( V_43 , & V_86 ) ;
}
if ( V_154 == 1 ) {
V_50 [ V_43 -> V_51 ] .
V_85
[ V_50 [ V_43 -> V_51 ] .
V_81 + 1 ] = V_157 ;
F_16 ( V_43 ,
& V_86 ) ;
}
if ( V_154 == 2 ) {
V_50 [ V_43 -> V_51 ] .
V_85
[ V_50 [ V_43 -> V_51 ] .
V_81 + 2 ] = V_157 ;
if ( V_50 [ V_43 -> V_51 ] .
V_96 == 1 ) {
V_50 [ V_43 -> V_51 ] .
V_74 = 0 ;
V_50 [ V_43 -> V_51 ] .
V_81 =
V_50 [ V_43 ->
V_51 ] . V_81 + 9 ;
}
else {
V_50 [ V_43 -> V_51 ] .
V_81 = 0 ;
}
if ( V_50 [ V_43 -> V_51 ] .
V_96 != 1 ) {
V_158 = F_42 ( V_159 , V_60 -> V_106 , 0 ) ;
}
else {
if ( V_50 [ V_43 -> V_51 ] .
V_102 ==
V_50 [ V_43 ->
V_51 ] . V_82 ) {
F_42 ( V_159 , V_60 -> V_106 , 0 ) ;
}
}
}
}
break;
case 2 :
V_147 =
F_2 ( V_60 -> V_70 +
V_50 [ V_43 -> V_51 ] . V_71 + 16 ) ;
if ( ( V_147 & 0x2 ) == 0x2 ) {
V_155 =
( ( F_2 ( V_60 -> V_70 +
V_50 [ V_43 ->
V_51 ] .
V_71 +
4 ) & 0x00000400 ) >> 10 ) ;
V_154 =
( ( F_2 ( V_60 -> V_70 +
V_50 [ V_43 ->
V_51 ] .
V_71 +
12 ) & 0x00060000 ) >>
17 ) ;
V_153 =
F_2 ( V_60 -> V_70 +
V_50 [ V_43 -> V_51 ] . V_71 + 24 ) ;
V_50 [ V_43 -> V_51 ] . V_44 =
V_153 ;
V_156 =
F_2 ( V_60 -> V_70 +
V_50 [ V_43 -> V_51 ] . V_71 + 28 ) ;
if ( ( V_154 == 0 )
&& ( V_155 == 0 ) ) {
V_50 [ V_43 -> V_51 ] .
V_85
[ V_50 [ V_43 -> V_51 ] .
V_81 + 0 ] =
V_156 ;
F_17 ( V_43 ,
& V_86 ) ;
}
if ( ( V_155 == 1 )
&& ( V_154 == 0 ) ) {
V_50 [ V_43 -> V_51 ] .
V_85
[ V_50 [ V_43 -> V_51 ] .
V_81 + 3 ] =
V_156 ;
F_15
( V_43 , & V_86 ) ;
}
if ( ( V_154 == 1 )
&& ( V_155 == 0 ) ) {
V_50 [ V_43 -> V_51 ] .
V_85
[ V_50 [ V_43 -> V_51 ] .
V_81 + 1 ] =
V_156 ;
F_16 ( V_43 ,
& V_86 ) ;
}
if ( ( V_154 == 2 )
&& ( V_155 == 0 ) ) {
V_50 [ V_43 -> V_51 ] .
V_85
[ V_50 [ V_43 -> V_51 ] .
V_81 + 2 ] =
V_156 ;
if ( V_50 [ V_43 -> V_51 ] .
V_93 !=
V_50 [ V_43 -> V_51 ] .
V_94 ) {
V_87 = 1 ;
}
else {
if ( V_50 [ V_43 -> V_51 ] .
V_95 ==
V_50 [ V_43 ->
V_51 ] .
V_53 ) {
V_87
= 0 ;
}
else {
V_87
= 1 ;
}
}
if ( V_87 == 1 ) {
F_18 ( V_43 ,
& V_86 ) ;
}
else {
V_50 [ V_43 -> V_51 ] .
V_85
[ V_50 [ V_43 ->
V_51 ] . V_81 +
4 ] = 0 ;
V_50 [ V_43 -> V_51 ] .
V_85
[ V_50 [ V_43 ->
V_51 ] . V_81 +
5 ] = 0 ;
}
}
if ( ( V_154 == 1 )
&& ( V_155 == 1 ) ) {
V_50 [ V_43 -> V_51 ] .
V_85
[ V_50 [ V_43 -> V_51 ] .
V_81 + 4 ] =
V_156 ;
F_19 ( V_43 ,
& V_86 ) ;
}
if ( ( V_154 == 2 )
&& ( V_155 == 1 ) ) {
V_50 [ V_43 -> V_51 ] .
V_85
[ V_50 [ V_43 -> V_51 ] .
V_81 + 5 ] =
V_156 ;
if ( V_50 [ V_43 -> V_51 ] .
V_96 == 1 ) {
V_50 [ V_43 -> V_51 ] .
V_74 = 0 ;
V_50 [ V_43 -> V_51 ] .
V_81 =
V_50 [ V_43 ->
V_51 ] . V_81 + 9 ;
}
else {
V_50 [ V_43 -> V_51 ] .
V_81 = 0 ;
}
if ( V_50 [ V_43 -> V_51 ] .
V_96 != 1 ) {
F_42 ( V_159 , V_60 -> V_106 , 0 ) ;
}
else {
if ( V_50 [ V_43 -> V_51 ] .
V_102 ==
V_50 [ V_43 ->
V_51 ] . V_82 ) {
F_42 ( V_159 , V_60 -> V_106 , 0 ) ;
}
}
}
}
break;
}
break;
case 2 :
case 3 :
F_36 ( V_43 ) ;
break;
}
return;
}
