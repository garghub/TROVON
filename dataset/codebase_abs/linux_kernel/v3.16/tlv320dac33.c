static inline unsigned int F_1 ( struct V_1 * V_2 ,
unsigned V_3 )
{
T_1 * V_4 = V_2 -> V_5 ;
if ( V_3 >= V_6 )
return 0 ;
return V_4 [ V_3 ] ;
}
static inline void F_2 ( struct V_1 * V_2 ,
T_1 V_3 , T_1 V_7 )
{
T_1 * V_4 = V_2 -> V_5 ;
if ( V_3 >= V_6 )
return;
V_4 [ V_3 ] = V_7 ;
}
static int F_3 ( struct V_1 * V_2 , unsigned int V_3 ,
T_1 * V_7 )
{
struct V_8 * V_9 = F_4 ( V_2 ) ;
int V_10 , V_11 = 0 ;
* V_7 = V_3 & 0xff ;
if ( V_9 -> V_12 ) {
V_10 = F_5 ( V_2 -> V_13 , V_7 [ 0 ] ) ;
if ( V_10 < 0 ) {
F_6 ( V_2 -> V_14 , L_1 , V_10 ) ;
V_7 [ 0 ] = F_1 ( V_2 , V_3 ) ;
V_11 = V_10 ;
} else {
V_7 [ 0 ] = V_10 ;
F_2 ( V_2 , V_3 , V_10 ) ;
}
} else {
V_7 [ 0 ] = F_1 ( V_2 , V_3 ) ;
}
return V_11 ;
}
static int F_7 ( struct V_1 * V_2 , unsigned int V_3 ,
unsigned int V_7 )
{
struct V_8 * V_9 = F_4 ( V_2 ) ;
T_1 V_15 [ 2 ] ;
int V_11 = 0 ;
V_15 [ 0 ] = V_3 & 0xff ;
V_15 [ 1 ] = V_7 & 0xff ;
F_2 ( V_2 , V_15 [ 0 ] , V_15 [ 1 ] ) ;
if ( V_9 -> V_12 ) {
V_11 = V_2 -> V_16 ( V_2 -> V_13 , V_15 , 2 ) ;
if ( V_11 != 2 )
F_6 ( V_2 -> V_14 , L_2 , V_11 ) ;
else
V_11 = 0 ;
}
return V_11 ;
}
static int F_8 ( struct V_1 * V_2 , unsigned int V_3 ,
unsigned int V_7 )
{
struct V_8 * V_9 = F_4 ( V_2 ) ;
int V_11 ;
F_9 ( & V_9 -> V_17 ) ;
V_11 = F_7 ( V_2 , V_3 , V_7 ) ;
F_10 ( & V_9 -> V_17 ) ;
return V_11 ;
}
static int F_11 ( struct V_1 * V_2 , unsigned int V_3 ,
unsigned int V_7 )
{
struct V_8 * V_9 = F_4 ( V_2 ) ;
T_1 V_15 [ 3 ] ;
int V_11 = 0 ;
V_15 [ 0 ] = V_3 & 0xff ;
V_15 [ 1 ] = ( V_7 >> 8 ) & 0xff ;
V_15 [ 2 ] = V_7 & 0xff ;
F_2 ( V_2 , V_15 [ 0 ] , V_15 [ 1 ] ) ;
F_2 ( V_2 , V_15 [ 0 ] + 1 , V_15 [ 2 ] ) ;
if ( V_9 -> V_12 ) {
V_15 [ 0 ] |= V_18 ;
V_11 = V_2 -> V_16 ( V_2 -> V_13 , V_15 , 3 ) ;
if ( V_11 != 3 )
F_6 ( V_2 -> V_14 , L_2 , V_11 ) ;
else
V_11 = 0 ;
}
return V_11 ;
}
static void F_12 ( struct V_1 * V_2 )
{
struct V_8 * V_9 = F_4 ( V_2 ) ;
if ( F_13 ( ! V_9 -> V_12 ) )
return;
F_7 ( V_2 , V_19 , F_14 ( 0 ) ) ;
F_7 ( V_2 , V_20 , V_21 |
V_22 ) ;
F_7 ( V_2 , V_23 , 0x00 ) ;
F_7 ( V_2 , V_24 , V_25 ) ;
F_7 ( V_2 , V_26 ,
F_1 ( V_2 , V_26 ) ) ;
F_7 ( V_2 , V_27 ,
F_1 ( V_2 , V_27 ) ) ;
F_7 ( V_2 , V_28 ,
F_1 ( V_2 , V_28 ) ) ;
F_7 ( V_2 , V_29 ,
F_1 ( V_2 , V_29 ) ) ;
F_7 ( V_2 , V_30 ,
F_1 ( V_2 , V_30 ) ) ;
F_7 ( V_2 , V_31 ,
F_1 ( V_2 , V_31 ) ) ;
F_7 ( V_2 , V_32 ,
F_1 ( V_2 , V_32 ) ) ;
}
static inline int F_15 ( struct V_1 * V_2 )
{
int V_33 , V_11 = 0 ;
T_1 V_3 ;
for ( V_33 = 0 ; V_33 < 3 ; V_33 ++ ) {
V_11 = F_3 ( V_2 , V_34 + V_33 , & V_3 ) ;
if ( V_11 < 0 )
break;
}
return V_11 ;
}
static inline void F_16 ( struct V_1 * V_2 , int V_35 )
{
T_1 V_3 ;
V_3 = F_1 ( V_2 , V_36 ) ;
if ( V_35 )
V_3 |= V_37 ;
else
V_3 &= ~ ( V_37 | V_38 |
V_39 | V_40 ) ;
F_7 ( V_2 , V_36 , V_3 ) ;
}
static inline void F_17 ( struct V_1 * V_2 )
{
T_1 V_3 ;
V_3 = F_1 ( V_2 , V_41 ) ;
V_3 &= ~ V_42 ;
F_7 ( V_2 , V_41 , V_3 ) ;
V_3 = F_1 ( V_2 , V_36 ) ;
V_3 &= ~ ( V_38 | V_39 | V_40 ) ;
F_7 ( V_2 , V_36 , V_3 ) ;
}
static int F_18 ( struct V_1 * V_2 , int V_35 )
{
struct V_8 * V_9 = F_4 ( V_2 ) ;
int V_11 = 0 ;
F_9 ( & V_9 -> V_17 ) ;
if ( F_13 ( V_35 == V_9 -> V_12 ) ) {
F_19 ( V_2 -> V_14 , L_3 ,
V_35 ? L_4 : L_5 ) ;
goto exit;
}
if ( V_35 ) {
V_11 = F_20 ( F_21 ( V_9 -> V_43 ) ,
V_9 -> V_43 ) ;
if ( V_11 != 0 ) {
F_6 ( V_2 -> V_14 ,
L_6 , V_11 ) ;
goto exit;
}
if ( V_9 -> V_44 >= 0 )
F_22 ( V_9 -> V_44 , 1 ) ;
V_9 -> V_12 = 1 ;
} else {
F_16 ( V_2 , 0 ) ;
if ( V_9 -> V_44 >= 0 )
F_22 ( V_9 -> V_44 , 0 ) ;
V_11 = F_23 ( F_21 ( V_9 -> V_43 ) ,
V_9 -> V_43 ) ;
if ( V_11 != 0 ) {
F_6 ( V_2 -> V_14 ,
L_7 , V_11 ) ;
goto exit;
}
V_9 -> V_12 = 0 ;
}
exit:
F_10 ( & V_9 -> V_17 ) ;
return V_11 ;
}
static int F_24 ( struct V_45 * V_46 ,
struct V_47 * V_48 , int V_49 )
{
struct V_8 * V_9 = F_4 ( V_46 -> V_2 ) ;
switch ( V_49 ) {
case V_50 :
if ( F_25 ( V_9 -> V_51 ) ) {
F_26 ( V_9 -> V_51 , V_46 -> V_2 ) ;
F_27 ( V_9 -> V_51 , V_46 -> V_2 ) ;
}
break;
case V_52 :
F_17 ( V_46 -> V_2 ) ;
break;
}
return 0 ;
}
static int F_28 ( struct V_47 * V_48 ,
struct V_53 * V_54 )
{
struct V_1 * V_2 = F_29 ( V_48 ) ;
struct V_8 * V_9 = F_4 ( V_2 ) ;
V_54 -> V_7 . integer . V_7 [ 0 ] = V_9 -> V_55 ;
return 0 ;
}
static int F_30 ( struct V_47 * V_48 ,
struct V_53 * V_54 )
{
struct V_1 * V_2 = F_29 ( V_48 ) ;
struct V_8 * V_9 = F_4 ( V_2 ) ;
int V_11 = 0 ;
if ( V_9 -> V_55 == V_54 -> V_7 . integer . V_7 [ 0 ] )
return 0 ;
if ( F_31 ( V_2 ) )
return - V_56 ;
if ( V_54 -> V_7 . integer . V_7 [ 0 ] < 0 ||
V_54 -> V_7 . integer . V_7 [ 0 ] >= V_57 )
V_11 = - V_58 ;
else
V_9 -> V_55 = V_54 -> V_7 . integer . V_7 [ 0 ] ;
return V_11 ;
}
static int F_32 ( struct V_1 * V_2 ,
enum V_59 V_60 )
{
int V_11 ;
switch ( V_60 ) {
case V_61 :
break;
case V_62 :
break;
case V_63 :
if ( V_2 -> V_64 . V_65 == V_66 ) {
V_11 = F_18 ( V_2 , 1 ) ;
if ( V_11 != 0 )
return V_11 ;
F_12 ( V_2 ) ;
}
break;
case V_66 :
if ( V_2 -> V_64 . V_65 == V_66 )
return 0 ;
V_11 = F_18 ( V_2 , 0 ) ;
if ( V_11 != 0 )
return V_11 ;
break;
}
V_2 -> V_64 . V_65 = V_60 ;
return 0 ;
}
static inline void F_33 ( struct V_8 * V_9 )
{
struct V_1 * V_2 = V_9 -> V_2 ;
unsigned int V_67 ;
unsigned long V_68 ;
switch ( V_9 -> V_55 ) {
case V_69 :
F_11 ( V_2 , V_70 ,
F_34 ( V_9 -> V_71 ) ) ;
F_35 ( & V_9 -> V_72 , V_68 ) ;
V_9 -> V_73 = F_36 ( F_37 () ) ;
V_9 -> V_74 = V_9 -> V_73 ;
F_38 ( & V_9 -> V_72 , V_68 ) ;
F_11 ( V_2 , V_75 ,
F_34 ( V_9 -> V_76 ) ) ;
V_67 = F_39 ( V_9 -> V_77 ,
V_9 -> V_76 ) + 1000 ;
F_40 ( V_67 , V_67 + 500 ) ;
F_7 ( V_2 , V_78 , V_79 ) ;
break;
case V_80 :
F_35 ( & V_9 -> V_72 , V_68 ) ;
V_9 -> V_74 = F_36 ( F_37 () ) ;
V_9 -> V_74 -= V_9 -> V_81 ;
F_38 ( & V_9 -> V_72 , V_68 ) ;
F_11 ( V_2 , V_75 ,
F_34 ( V_82 ) ) ;
F_7 ( V_2 , V_78 , V_83 ) ;
break;
default:
F_41 ( V_2 -> V_14 , L_8 ,
V_9 -> V_55 ) ;
break;
}
}
static inline void F_42 ( struct V_8 * V_9 )
{
struct V_1 * V_2 = V_9 -> V_2 ;
unsigned long V_68 ;
switch ( V_9 -> V_55 ) {
case V_69 :
F_35 ( & V_9 -> V_72 , V_68 ) ;
V_9 -> V_73 = F_36 ( F_37 () ) ;
F_38 ( & V_9 -> V_72 , V_68 ) ;
F_11 ( V_2 , V_70 ,
F_34 ( V_9 -> V_71 ) ) ;
break;
case V_80 :
break;
default:
F_41 ( V_2 -> V_14 , L_8 ,
V_9 -> V_55 ) ;
break;
}
}
static void F_43 ( struct V_84 * V_85 )
{
struct V_1 * V_2 ;
struct V_8 * V_9 ;
T_1 V_3 ;
V_9 = F_44 ( V_85 , struct V_8 , V_85 ) ;
V_2 = V_9 -> V_2 ;
F_9 ( & V_9 -> V_17 ) ;
switch ( V_9 -> V_86 ) {
case V_87 :
V_9 -> V_86 = V_88 ;
F_33 ( V_9 ) ;
break;
case V_88 :
F_42 ( V_9 ) ;
break;
case V_89 :
break;
case V_90 :
V_9 -> V_86 = V_89 ;
F_7 ( V_2 , V_78 , 0 ) ;
V_3 = F_1 ( V_2 , V_91 ) ;
V_3 |= V_92 ;
F_7 ( V_2 , V_91 , V_3 ) ;
break;
}
F_10 ( & V_9 -> V_17 ) ;
}
static T_2 F_45 ( int V_93 , void * V_14 )
{
struct V_1 * V_2 = V_14 ;
struct V_8 * V_9 = F_4 ( V_2 ) ;
unsigned long V_68 ;
F_35 ( & V_9 -> V_72 , V_68 ) ;
V_9 -> V_74 = F_36 ( F_37 () ) ;
F_38 ( & V_9 -> V_72 , V_68 ) ;
if ( V_9 -> V_55 != V_80 )
F_46 ( V_9 -> V_94 , & V_9 -> V_85 ) ;
return V_95 ;
}
static void F_47 ( struct V_1 * V_2 )
{
int V_96 = 60 ;
T_1 V_3 ;
do {
F_40 ( 1000 , 2000 ) ;
F_3 ( V_2 , V_97 , & V_3 ) ;
} while ( ( ( V_3 & 0x03 ) != V_98 ) && V_96 -- );
if ( ( V_3 & 0x03 ) != V_98 )
F_6 ( V_2 -> V_14 ,
L_9 ) ;
}
static int F_48 ( struct V_99 * V_51 ,
struct V_100 * V_101 )
{
struct V_1 * V_2 = V_101 -> V_2 ;
struct V_8 * V_9 = F_4 ( V_2 ) ;
V_9 -> V_51 = V_51 ;
return 0 ;
}
static void F_49 ( struct V_99 * V_51 ,
struct V_100 * V_101 )
{
struct V_1 * V_2 = V_101 -> V_2 ;
struct V_8 * V_9 = F_4 ( V_2 ) ;
V_9 -> V_51 = NULL ;
}
static int F_50 ( struct V_99 * V_51 ,
struct V_102 * V_103 ,
struct V_100 * V_101 )
{
struct V_1 * V_2 = V_101 -> V_2 ;
struct V_8 * V_9 = F_4 ( V_2 ) ;
switch ( F_51 ( V_103 ) ) {
case 44100 :
case 48000 :
break;
default:
F_6 ( V_2 -> V_14 , L_10 ,
F_51 ( V_103 ) ) ;
return - V_58 ;
}
switch ( F_52 ( V_103 ) ) {
case V_104 :
V_9 -> V_105 = V_106 ;
V_9 -> V_77 = F_53 ( V_9 -> V_107 , 32 ) ;
break;
case V_108 :
V_9 -> V_105 = V_109 ;
V_9 -> V_77 = F_53 ( V_9 -> V_107 , 64 ) ;
break;
default:
F_6 ( V_2 -> V_14 , L_11 ,
F_52 ( V_103 ) ) ;
return - V_58 ;
}
return 0 ;
}
static int F_27 ( struct V_99 * V_51 ,
struct V_1 * V_2 )
{
struct V_8 * V_9 = F_4 ( V_2 ) ;
unsigned int V_110 , V_111 , V_112 , V_113 ;
T_1 V_114 , V_115 , V_116 ;
switch ( V_51 -> V_117 -> V_118 ) {
case 44100 :
case 48000 :
V_110 = F_54 ( V_51 -> V_117 -> V_118 , V_9 -> V_119 ) ;
V_111 = F_55 ( V_51 -> V_117 -> V_118 ,
V_9 -> V_119 ) ;
break;
default:
F_6 ( V_2 -> V_14 , L_10 ,
V_51 -> V_117 -> V_118 ) ;
return - V_58 ;
}
V_114 = F_1 ( V_2 , V_120 ) ;
V_114 &= ~ ( V_121 | V_122 ) ;
V_116 = F_1 ( V_2 , V_91 ) ;
V_116 &= ~ V_92 ;
V_116 &= ~ V_123 ;
switch ( V_51 -> V_117 -> V_124 ) {
case V_104 :
V_114 |= ( V_125 | V_126 ) ;
V_116 |= V_123 ;
break;
case V_108 :
V_114 |= ( V_127 | V_128 ) ;
break;
default:
F_6 ( V_2 -> V_14 , L_11 ,
V_51 -> V_117 -> V_124 ) ;
return - V_58 ;
}
F_9 ( & V_9 -> V_17 ) ;
if ( ! V_9 -> V_12 ) {
F_10 ( & V_9 -> V_17 ) ;
return 0 ;
}
F_16 ( V_2 , 0 ) ;
F_16 ( V_2 , 1 ) ;
V_113 = F_1 ( V_2 , V_129 ) ;
F_7 ( V_2 , V_129 , V_113 ) ;
F_11 ( V_2 , V_130 , V_110 ) ;
F_7 ( V_2 , V_131 , 96 ) ;
F_7 ( V_2 , V_132 , F_56 ( 2 ) |
F_57 ( 1 ) ) ;
F_7 ( V_2 , V_133 , F_58 ( 4 ) ) ;
V_112 = F_1 ( V_2 , V_36 ) ;
V_112 |= V_38 | V_39 | V_40 ;
F_7 ( V_2 , V_36 , V_112 ) ;
F_47 ( V_2 ) ;
if ( V_9 -> V_55 ) {
F_7 ( V_2 , V_134 , F_59 ( 1 ) ) ;
F_7 ( V_2 , V_135 , 1 ) ;
F_11 ( V_2 , V_136 , V_111 ) ;
F_7 ( V_2 , V_137 , V_138 ) ;
} else {
F_7 ( V_2 , V_134 , V_139 ) ;
F_7 ( V_2 , V_135 , 0 ) ;
}
switch ( V_9 -> V_55 ) {
case V_69 :
F_7 ( V_2 , V_140 ,
F_60 ( V_141 ) ) ;
break;
case V_80 :
F_7 ( V_2 , V_142 ,
F_61 ( V_141 ) ) ;
break;
default:
break;
}
V_115 = F_1 ( V_2 , V_41 ) ;
switch ( V_9 -> V_55 ) {
case V_69 :
V_116 &= ~ V_143 ;
V_116 &= ~ V_144 ;
if ( V_9 -> V_145 )
V_115 |= V_42 ;
else
V_115 &= ~ V_42 ;
break;
case V_80 :
V_116 &= ~ V_143 ;
V_116 |= V_144 ;
if ( V_9 -> V_145 )
V_115 |= V_42 ;
else
V_115 &= ~ V_42 ;
break;
default:
V_116 |= V_143 ;
V_115 |= V_42 ;
break;
}
F_7 ( V_2 , V_91 , V_116 ) ;
F_7 ( V_2 , V_120 , V_114 ) ;
F_7 ( V_2 , V_41 , V_115 ) ;
if ( V_9 -> V_55 )
F_7 ( V_2 , V_146 ,
V_9 -> V_107 ) ;
else
if ( V_51 -> V_117 -> V_124 == V_104 )
F_7 ( V_2 , V_146 , 32 ) ;
else
F_7 ( V_2 , V_146 , 16 ) ;
switch ( V_9 -> V_55 ) {
case V_69 :
F_11 ( V_2 , V_147 ,
F_34 ( V_9 -> V_76 ) ) ;
break;
case V_80 :
F_11 ( V_2 , V_148 , F_34 ( V_9 -> V_149 ) ) ;
F_11 ( V_2 , V_150 ,
F_34 ( V_82 ) ) ;
break;
default:
break;
}
F_10 ( & V_9 -> V_17 ) ;
return 0 ;
}
static void F_26 ( struct V_99 * V_51 ,
struct V_1 * V_2 )
{
struct V_8 * V_9 = F_4 ( V_2 ) ;
unsigned int V_151 = V_51 -> V_117 -> V_151 ;
unsigned int V_118 = V_51 -> V_117 -> V_118 ;
unsigned int V_152 ;
if ( ! V_9 -> V_55 )
return;
switch ( V_9 -> V_55 ) {
case V_69 :
V_9 -> V_76 = F_62 ( V_118 ,
V_9 -> V_153 ) ;
V_152 = V_9 -> V_105 - V_9 -> V_76 ;
if ( V_151 <= V_9 -> V_76 )
V_9 -> V_71 = V_151 *
( ( V_9 -> V_76 / V_151 ) +
( V_9 -> V_76 % V_151 ?
1 : 0 ) ) ;
else if ( V_151 > V_152 )
V_9 -> V_71 = V_152 ;
else
V_9 -> V_71 = V_151 ;
V_9 -> V_154 = F_39 ( V_9 -> V_77 ,
V_9 -> V_71 ) ;
V_9 -> V_74 = 0 ;
V_9 -> V_73 = 0 ;
break;
case V_80 :
V_9 -> V_149 = F_63 ( V_151 , V_118 ,
V_9 -> V_77 ) + 9 ;
if ( V_9 -> V_149 > ( V_9 -> V_105 - V_82 ) )
V_9 -> V_149 = V_9 -> V_105 - V_82 ;
if ( V_9 -> V_149 < ( V_82 + 10 ) )
V_9 -> V_149 = ( V_82 + 10 ) ;
V_9 -> V_81 =
F_39 ( V_51 -> V_117 -> V_118 ,
V_9 -> V_149 - V_82 + 1 ) ;
V_9 -> V_74 = 0 ;
break;
default:
break;
}
}
static int F_64 ( struct V_99 * V_51 , int V_155 ,
struct V_100 * V_101 )
{
struct V_1 * V_2 = V_101 -> V_2 ;
struct V_8 * V_9 = F_4 ( V_2 ) ;
int V_11 = 0 ;
switch ( V_155 ) {
case V_156 :
case V_157 :
case V_158 :
if ( V_9 -> V_55 ) {
V_9 -> V_86 = V_87 ;
F_46 ( V_9 -> V_94 , & V_9 -> V_85 ) ;
}
break;
case V_159 :
case V_160 :
case V_161 :
if ( V_9 -> V_55 ) {
V_9 -> V_86 = V_90 ;
F_46 ( V_9 -> V_94 , & V_9 -> V_85 ) ;
}
break;
default:
V_11 = - V_58 ;
}
return V_11 ;
}
static T_3 F_65 (
struct V_99 * V_51 ,
struct V_100 * V_101 )
{
struct V_1 * V_2 = V_101 -> V_2 ;
struct V_8 * V_9 = F_4 ( V_2 ) ;
unsigned long long V_162 , V_163 , V_164 ;
unsigned int V_165 , V_149 ;
int V_166 , V_167 , V_168 ;
T_3 V_67 = 0 ;
unsigned long V_68 ;
switch ( V_9 -> V_55 ) {
case V_169 :
break;
case V_69 :
F_35 ( & V_9 -> V_72 , V_68 ) ;
V_162 = V_9 -> V_74 ;
V_163 = V_9 -> V_73 ;
F_38 ( & V_9 -> V_72 , V_68 ) ;
V_164 = F_36 ( F_37 () ) ;
if ( ! V_163 )
goto V_170;
if ( V_162 > V_163 ) {
V_165 = V_164 - V_162 ;
V_166 = V_165 ? F_62 (
V_51 -> V_117 -> V_118 ,
V_165 ) : 0 ;
if ( F_25 ( V_9 -> V_76 > V_166 ) )
V_67 = V_9 -> V_76 - V_166 ;
else
V_67 = 0 ;
} else if ( ( V_164 - V_163 ) <= V_9 -> V_154 ) {
V_165 = V_164 - V_162 ;
V_166 = V_165 ? F_62 (
V_51 -> V_117 -> V_118 ,
V_165 ) : 0 ;
V_165 = V_164 - V_163 ;
V_167 = V_165 ? F_62 (
V_9 -> V_77 ,
V_165 ) : 0 ;
V_168 = V_9 -> V_76 ;
V_168 += ( V_167 - V_166 ) ;
if ( F_25 ( V_168 > 0 ) )
V_67 = V_168 ;
else
V_67 = 0 ;
} else {
V_165 = V_164 - V_162 ;
V_166 = V_165 ? F_62 (
V_51 -> V_117 -> V_118 ,
V_165 ) : 0 ;
V_167 = V_9 -> V_71 ;
V_168 = V_9 -> V_76 ;
V_168 += ( V_167 - V_166 ) ;
if ( F_25 ( V_168 > 0 ) )
V_67 = V_168 > V_9 -> V_105 ?
V_9 -> V_105 : V_168 ;
else
V_67 = 0 ;
}
break;
case V_80 :
F_35 ( & V_9 -> V_72 , V_68 ) ;
V_162 = V_9 -> V_74 ;
V_149 = V_9 -> V_149 ;
F_38 ( & V_9 -> V_72 , V_68 ) ;
V_164 = F_36 ( F_37 () ) ;
if ( ! V_162 )
goto V_170;
if ( V_164 <= V_162 ) {
V_67 = V_149 ;
goto V_170;
}
V_165 = V_164 - V_162 ;
if ( V_165 <= V_9 -> V_81 ) {
V_166 = F_62 (
V_51 -> V_117 -> V_118 ,
V_165 ) ;
if ( F_25 ( V_149 > V_166 ) )
V_67 = V_149 - V_166 ;
else
V_67 = 0 ;
} else {
V_165 = V_165 - V_9 -> V_81 ;
V_166 = F_62 (
V_51 -> V_117 -> V_118 ,
V_165 ) ;
V_167 = F_62 (
V_9 -> V_77 ,
V_165 ) ;
V_67 = V_82 + V_167 - V_166 ;
if ( F_13 ( V_67 > V_149 ) )
V_67 = V_149 ;
}
break;
default:
F_41 ( V_2 -> V_14 , L_8 ,
V_9 -> V_55 ) ;
break;
}
V_170:
return V_67 ;
}
static int F_66 ( struct V_100 * V_171 ,
int V_172 , unsigned int V_173 , int V_174 )
{
struct V_1 * V_2 = V_171 -> V_2 ;
struct V_8 * V_9 = F_4 ( V_2 ) ;
T_1 V_175 , V_176 ;
V_175 = F_1 ( V_2 , V_129 ) ;
V_176 = F_1 ( V_2 , V_135 ) ;
switch ( V_172 ) {
case V_177 :
V_175 |= V_178 ;
V_176 |= V_179 ;
break;
case V_180 :
V_175 &= ~ V_178 ;
V_176 &= ~ V_179 ;
break;
default:
F_6 ( V_2 -> V_14 , L_12 , V_172 ) ;
break;
}
V_9 -> V_119 = V_173 ;
F_2 ( V_2 , V_129 , V_175 ) ;
F_2 ( V_2 , V_135 , V_176 ) ;
return 0 ;
}
static int F_67 ( struct V_100 * V_171 ,
unsigned int V_181 )
{
struct V_1 * V_2 = V_171 -> V_2 ;
struct V_8 * V_9 = F_4 ( V_2 ) ;
T_1 V_114 , V_115 ;
V_114 = F_1 ( V_2 , V_120 ) ;
V_115 = F_1 ( V_2 , V_41 ) ;
switch ( V_181 & V_182 ) {
case V_183 :
V_114 |= ( V_184 | V_185 ) ;
break;
case V_186 :
if ( V_9 -> V_55 ) {
F_6 ( V_2 -> V_14 , L_13 ) ;
return - V_58 ;
} else
V_114 &= ~ ( V_184 | V_185 ) ;
break;
default:
return - V_58 ;
}
V_114 &= ~ V_187 ;
switch ( V_181 & V_188 ) {
case V_189 :
V_114 |= V_190 ;
break;
case V_191 :
V_114 |= V_192 ;
V_115 &= ~ V_193 ;
V_115 |= F_68 ( 0 ) ;
break;
case V_194 :
V_114 |= V_195 ;
break;
case V_196 :
V_114 |= V_197 ;
break;
default:
F_6 ( V_2 -> V_14 , L_14 ,
V_181 & V_188 ) ;
return - V_58 ;
}
F_2 ( V_2 , V_120 , V_114 ) ;
F_2 ( V_2 , V_41 , V_115 ) ;
return 0 ;
}
static int F_69 ( struct V_1 * V_2 )
{
struct V_8 * V_9 = F_4 ( V_2 ) ;
int V_11 = 0 ;
V_2 -> V_13 = V_9 -> V_13 ;
V_2 -> V_16 = ( V_198 ) V_199 ;
V_9 -> V_2 = V_2 ;
V_11 = F_18 ( V_2 , 1 ) ;
if ( V_11 != 0 ) {
F_6 ( V_2 -> V_14 , L_15 , V_11 ) ;
goto V_200;
}
V_11 = F_15 ( V_2 ) ;
F_18 ( V_2 , 0 ) ;
if ( V_11 < 0 ) {
F_6 ( V_2 -> V_14 , L_16 , V_11 ) ;
V_11 = - V_201 ;
goto V_200;
}
if ( V_9 -> V_93 >= 0 ) {
V_11 = F_70 ( V_9 -> V_93 , F_45 ,
V_202 ,
V_2 -> V_203 , V_2 ) ;
if ( V_11 < 0 ) {
F_6 ( V_2 -> V_14 , L_17 ,
V_9 -> V_93 , V_11 ) ;
V_9 -> V_93 = - 1 ;
}
if ( V_9 -> V_93 != - 1 ) {
V_9 -> V_94 =
F_71 ( L_18 ) ;
if ( V_9 -> V_94 == NULL ) {
F_72 ( V_9 -> V_93 , V_2 ) ;
return - V_204 ;
}
F_73 ( & V_9 -> V_85 , F_43 ) ;
}
}
if ( V_9 -> V_93 >= 0 )
F_74 ( V_2 , V_205 ,
F_21 ( V_205 ) ) ;
V_200:
return V_11 ;
}
static int F_75 ( struct V_1 * V_2 )
{
struct V_8 * V_9 = F_4 ( V_2 ) ;
F_32 ( V_2 , V_66 ) ;
if ( V_9 -> V_93 >= 0 ) {
F_72 ( V_9 -> V_93 , V_9 -> V_2 ) ;
F_76 ( V_9 -> V_94 ) ;
}
return 0 ;
}
static int F_77 ( struct V_206 * V_207 ,
const struct V_208 * V_209 )
{
struct V_210 * V_211 ;
struct V_8 * V_9 ;
int V_11 , V_33 ;
if ( V_207 -> V_14 . V_212 == NULL ) {
F_6 ( & V_207 -> V_14 , L_19 ) ;
return - V_201 ;
}
V_211 = V_207 -> V_14 . V_212 ;
V_9 = F_78 ( & V_207 -> V_14 , sizeof( struct V_8 ) ,
V_213 ) ;
if ( V_9 == NULL )
return - V_204 ;
V_9 -> V_13 = V_207 ;
F_79 ( & V_9 -> V_17 ) ;
F_80 ( & V_9 -> V_72 ) ;
F_81 ( V_207 , V_9 ) ;
V_9 -> V_44 = V_211 -> V_44 ;
V_9 -> V_107 = V_211 -> V_107 ;
V_9 -> V_145 = V_211 -> V_145 ;
V_9 -> V_153 = V_211 -> V_153 ;
if ( ! V_9 -> V_153 )
V_9 -> V_153 = 10000 ;
V_9 -> V_93 = V_207 -> V_93 ;
V_9 -> V_55 = V_169 ;
if ( V_9 -> V_44 >= 0 ) {
V_11 = F_82 ( V_9 -> V_44 , L_20 ) ;
if ( V_11 < 0 ) {
F_6 ( & V_207 -> V_14 ,
L_21 ,
V_9 -> V_44 ) ;
goto V_214;
}
F_83 ( V_9 -> V_44 , 0 ) ;
}
for ( V_33 = 0 ; V_33 < F_21 ( V_9 -> V_43 ) ; V_33 ++ )
V_9 -> V_43 [ V_33 ] . V_215 = V_216 [ V_33 ] ;
V_11 = F_84 ( & V_207 -> V_14 , F_21 ( V_9 -> V_43 ) ,
V_9 -> V_43 ) ;
if ( V_11 != 0 ) {
F_6 ( & V_207 -> V_14 , L_22 , V_11 ) ;
goto V_217;
}
V_11 = F_85 ( & V_207 -> V_14 ,
& V_218 , & V_219 , 1 ) ;
if ( V_11 < 0 )
goto V_217;
return V_11 ;
V_217:
if ( V_9 -> V_44 >= 0 )
F_86 ( V_9 -> V_44 ) ;
V_214:
return V_11 ;
}
static int F_87 ( struct V_206 * V_207 )
{
struct V_8 * V_9 = F_88 ( V_207 ) ;
if ( F_13 ( V_9 -> V_12 ) )
F_18 ( V_9 -> V_2 , 0 ) ;
if ( V_9 -> V_44 >= 0 )
F_86 ( V_9 -> V_44 ) ;
F_89 ( & V_207 -> V_14 ) ;
return 0 ;
}
