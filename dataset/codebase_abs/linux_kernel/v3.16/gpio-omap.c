static int F_1 ( struct V_1 * V_2 , unsigned int V_3 )
{
return V_2 -> V_4 . V_5 + V_3 ;
}
static inline struct V_1 * F_2 ( struct V_6 * V_7 )
{
struct V_8 * V_4 = F_3 ( V_7 ) ;
return F_4 ( V_4 , struct V_1 , V_4 ) ;
}
static void F_5 ( struct V_1 * V_2 , int V_9 , int V_10 )
{
void T_1 * V_11 = V_2 -> V_5 ;
T_2 V_12 ;
V_11 += V_2 -> V_13 -> V_14 ;
V_12 = F_6 ( V_11 ) ;
if ( V_10 )
V_12 |= F_7 ( V_9 ) ;
else
V_12 &= ~ ( F_7 ( V_9 ) ) ;
F_8 ( V_12 , V_11 ) ;
V_2 -> V_15 . V_16 = V_12 ;
}
static void F_9 ( struct V_1 * V_2 , int V_9 , int V_17 )
{
void T_1 * V_11 = V_2 -> V_5 ;
T_2 V_12 = F_10 ( V_2 , V_9 ) ;
if ( V_17 ) {
V_11 += V_2 -> V_13 -> V_18 ;
V_2 -> V_15 . V_19 |= V_12 ;
} else {
V_11 += V_2 -> V_13 -> V_20 ;
V_2 -> V_15 . V_19 &= ~ V_12 ;
}
F_8 ( V_12 , V_11 ) ;
}
static void F_11 ( struct V_1 * V_2 , int V_9 , int V_17 )
{
void T_1 * V_11 = V_2 -> V_5 + V_2 -> V_13 -> V_19 ;
T_2 V_21 = F_10 ( V_2 , V_9 ) ;
T_2 V_12 ;
V_12 = F_6 ( V_11 ) ;
if ( V_17 )
V_12 |= V_21 ;
else
V_12 &= ~ V_21 ;
F_8 ( V_12 , V_11 ) ;
V_2 -> V_15 . V_19 = V_12 ;
}
static int F_12 ( struct V_1 * V_2 , int V_22 )
{
void T_1 * V_11 = V_2 -> V_5 + V_2 -> V_13 -> V_23 ;
return ( F_6 ( V_11 ) & ( F_7 ( V_22 ) ) ) != 0 ;
}
static int F_13 ( struct V_1 * V_2 , int V_22 )
{
void T_1 * V_11 = V_2 -> V_5 + V_2 -> V_13 -> V_19 ;
return ( F_6 ( V_11 ) & ( F_7 ( V_22 ) ) ) != 0 ;
}
static inline void F_14 ( void T_1 * V_5 , T_2 V_11 , T_2 V_24 , bool V_25 )
{
int V_12 = F_6 ( V_5 + V_11 ) ;
if ( V_25 )
V_12 |= V_24 ;
else
V_12 &= ~ V_24 ;
F_8 ( V_12 , V_5 + V_11 ) ;
}
static inline void F_15 ( struct V_1 * V_2 )
{
if ( V_2 -> V_26 && ! V_2 -> V_27 ) {
F_16 ( V_2 -> V_28 ) ;
V_2 -> V_27 = true ;
F_8 ( V_2 -> V_26 ,
V_2 -> V_5 + V_2 -> V_13 -> V_29 ) ;
}
}
static inline void F_17 ( struct V_1 * V_2 )
{
if ( V_2 -> V_26 && V_2 -> V_27 ) {
F_8 ( 0 , V_2 -> V_5 + V_2 -> V_13 -> V_29 ) ;
F_18 ( V_2 -> V_28 ) ;
V_2 -> V_27 = false ;
}
}
static void F_19 ( struct V_1 * V_2 , unsigned V_9 ,
unsigned V_30 )
{
void T_1 * V_11 ;
T_2 V_31 ;
T_2 V_12 ;
if ( ! V_2 -> V_32 )
return;
if ( V_30 < 32 )
V_30 = 0x01 ;
else if ( V_30 > 7936 )
V_30 = 0xff ;
else
V_30 = ( V_30 / 0x1f ) - 1 ;
V_12 = F_10 ( V_2 , V_9 ) ;
F_16 ( V_2 -> V_28 ) ;
V_11 = V_2 -> V_5 + V_2 -> V_13 -> V_30 ;
F_8 ( V_30 , V_11 ) ;
V_11 = V_2 -> V_5 + V_2 -> V_13 -> V_29 ;
V_31 = F_6 ( V_11 ) ;
if ( V_30 )
V_31 |= V_12 ;
else
V_31 &= ~ V_12 ;
V_2 -> V_26 = V_31 ;
F_8 ( V_31 , V_11 ) ;
F_18 ( V_2 -> V_28 ) ;
F_15 ( V_2 ) ;
if ( V_2 -> V_26 ) {
V_2 -> V_15 . V_30 = V_30 ;
V_2 -> V_15 . V_29 = V_31 ;
}
}
static void F_20 ( struct V_1 * V_2 , unsigned V_9 )
{
T_2 V_21 = F_10 ( V_2 , V_9 ) ;
if ( ! V_2 -> V_32 )
return;
if ( ! ( V_2 -> V_26 & V_21 ) )
return;
V_2 -> V_26 &= ~ V_21 ;
V_2 -> V_15 . V_29 &= ~ V_21 ;
F_8 ( V_2 -> V_15 . V_29 ,
V_2 -> V_5 + V_2 -> V_13 -> V_29 ) ;
if ( ! V_2 -> V_26 ) {
V_2 -> V_15 . V_30 = 0 ;
F_8 ( V_2 -> V_15 . V_30 , V_2 -> V_5 +
V_2 -> V_13 -> V_30 ) ;
F_18 ( V_2 -> V_28 ) ;
V_2 -> V_27 = false ;
}
}
static inline void F_21 ( struct V_1 * V_2 , int V_9 ,
unsigned V_33 )
{
void T_1 * V_5 = V_2 -> V_5 ;
T_2 V_21 = F_7 ( V_9 ) ;
F_14 ( V_5 , V_2 -> V_13 -> V_34 , V_21 ,
V_33 & V_35 ) ;
F_14 ( V_5 , V_2 -> V_13 -> V_36 , V_21 ,
V_33 & V_37 ) ;
F_14 ( V_5 , V_2 -> V_13 -> V_38 , V_21 ,
V_33 & V_39 ) ;
F_14 ( V_5 , V_2 -> V_13 -> V_40 , V_21 ,
V_33 & V_41 ) ;
V_2 -> V_15 . V_34 =
F_6 ( V_2 -> V_5 + V_2 -> V_13 -> V_34 ) ;
V_2 -> V_15 . V_36 =
F_6 ( V_2 -> V_5 + V_2 -> V_13 -> V_36 ) ;
V_2 -> V_15 . V_38 =
F_6 ( V_2 -> V_5 + V_2 -> V_13 -> V_38 ) ;
V_2 -> V_15 . V_40 =
F_6 ( V_2 -> V_5 + V_2 -> V_13 -> V_40 ) ;
if ( F_22 ( ! ( V_2 -> V_42 & V_21 ) ) ) {
F_14 ( V_5 , V_2 -> V_13 -> V_43 , V_21 , V_33 != 0 ) ;
V_2 -> V_15 . V_44 =
F_6 ( V_2 -> V_5 + V_2 -> V_13 -> V_43 ) ;
}
if ( ! V_2 -> V_13 -> V_45 ) {
if ( V_2 -> V_42 ) {
if ( ! ( V_2 -> V_42 & V_21 ) )
goto exit;
}
if ( V_33 & V_46 )
V_2 -> V_47 |= V_21 ;
else
V_2 -> V_47 &= ~ V_21 ;
}
exit:
V_2 -> V_48 =
F_6 ( V_2 -> V_5 + V_2 -> V_13 -> V_34 ) |
F_6 ( V_2 -> V_5 + V_2 -> V_13 -> V_36 ) ;
}
static void F_23 ( struct V_1 * V_2 , int V_9 )
{
void T_1 * V_11 = V_2 -> V_5 ;
T_2 V_12 = 0 ;
if ( ! V_2 -> V_13 -> V_45 )
return;
V_11 += V_2 -> V_13 -> V_45 ;
V_12 = F_6 ( V_11 ) ;
if ( ( V_12 >> V_9 ) & 1 )
V_12 &= ~ ( F_7 ( V_9 ) ) ;
else
V_12 |= F_7 ( V_9 ) ;
F_8 ( V_12 , V_11 ) ;
}
static void F_23 ( struct V_1 * V_2 , int V_9 ) {}
static int F_24 ( struct V_1 * V_2 , int V_9 ,
unsigned V_33 )
{
void T_1 * V_11 = V_2 -> V_5 ;
void T_1 * V_5 = V_2 -> V_5 ;
T_2 V_12 = 0 ;
if ( V_2 -> V_13 -> V_34 && V_2 -> V_13 -> V_43 ) {
F_21 ( V_2 , V_9 , V_33 ) ;
} else if ( V_2 -> V_13 -> V_45 ) {
V_11 += V_2 -> V_13 -> V_45 ;
V_12 = F_6 ( V_11 ) ;
if ( ( V_33 & V_49 ) == V_46 )
V_2 -> V_50 |= F_7 ( V_9 ) ;
if ( V_33 & V_39 )
V_12 |= F_7 ( V_9 ) ;
else if ( V_33 & V_41 )
V_12 &= ~ ( F_7 ( V_9 ) ) ;
else
return - V_51 ;
F_8 ( V_12 , V_11 ) ;
} else if ( V_2 -> V_13 -> V_52 ) {
if ( V_9 & 0x08 )
V_11 += V_2 -> V_13 -> V_53 ;
else
V_11 += V_2 -> V_13 -> V_52 ;
V_9 &= 0x07 ;
V_12 = F_6 ( V_11 ) ;
V_12 &= ~ ( 3 << ( V_9 << 1 ) ) ;
if ( V_33 & V_39 )
V_12 |= 2 << ( V_9 << 1 ) ;
if ( V_33 & V_41 )
V_12 |= F_7 ( V_9 << 1 ) ;
F_14 ( V_5 , V_2 -> V_13 -> V_43 , F_7 ( V_9 ) , V_33 ) ;
V_2 -> V_15 . V_44 =
F_6 ( V_2 -> V_5 + V_2 -> V_13 -> V_43 ) ;
F_8 ( V_12 , V_11 ) ;
}
return 0 ;
}
static void F_25 ( struct V_1 * V_2 , unsigned V_22 )
{
if ( V_2 -> V_13 -> V_54 ) {
void T_1 * V_11 = V_2 -> V_5 + V_2 -> V_13 -> V_54 ;
F_8 ( F_6 ( V_11 ) | ( F_7 ( V_22 ) ) , V_11 ) ;
}
if ( V_2 -> V_13 -> V_55 && ! F_26 ( V_2 ) ) {
void T_1 * V_11 = V_2 -> V_5 + V_2 -> V_13 -> V_55 ;
T_2 V_55 ;
V_55 = F_6 ( V_11 ) ;
V_55 &= ~ V_56 ;
F_8 ( V_55 , V_11 ) ;
V_2 -> V_15 . V_55 = V_55 ;
}
}
static void F_27 ( struct V_1 * V_2 , unsigned V_22 )
{
void T_1 * V_5 = V_2 -> V_5 ;
if ( V_2 -> V_13 -> V_43 &&
! F_28 ( V_2 -> V_57 , V_22 ) &&
! F_28 ( V_2 -> V_58 , V_22 ) ) {
F_14 ( V_5 , V_2 -> V_13 -> V_43 , F_7 ( V_22 ) , 0 ) ;
V_2 -> V_15 . V_44 =
F_6 ( V_2 -> V_5 + V_2 -> V_13 -> V_43 ) ;
}
if ( V_2 -> V_13 -> V_55 && ! F_26 ( V_2 ) ) {
void T_1 * V_11 = V_2 -> V_5 + V_2 -> V_13 -> V_55 ;
T_2 V_55 ;
V_55 = F_6 ( V_11 ) ;
V_55 |= V_56 ;
F_8 ( V_55 , V_11 ) ;
V_2 -> V_15 . V_55 = V_55 ;
}
}
static int F_29 ( struct V_1 * V_2 , int V_24 )
{
void T_1 * V_11 = V_2 -> V_5 + V_2 -> V_13 -> V_14 ;
return F_6 ( V_11 ) & V_24 ;
}
static int F_30 ( struct V_6 * V_7 , unsigned type )
{
struct V_1 * V_2 = F_2 ( V_7 ) ;
unsigned V_9 = 0 ;
int V_59 ;
unsigned long V_60 ;
unsigned V_22 ;
if ( ! F_26 ( V_2 ) )
F_31 ( V_2 -> V_61 ) ;
#ifdef F_32
if ( V_7 -> V_62 > V_63 )
V_9 = F_33 ( V_7 -> V_62 - V_63 ) ;
#endif
if ( ! V_9 )
V_9 = F_1 ( V_2 , V_7 -> V_64 ) ;
if ( type & ~ V_49 )
return - V_51 ;
if ( ! V_2 -> V_13 -> V_34 &&
( type & ( V_35 | V_37 ) ) )
return - V_51 ;
F_34 ( & V_2 -> V_65 , V_60 ) ;
V_22 = F_35 ( V_2 , V_9 ) ;
V_59 = F_24 ( V_2 , V_22 , type ) ;
if ( ! F_28 ( V_2 -> V_57 , V_22 ) ) {
F_25 ( V_2 , V_22 ) ;
F_5 ( V_2 , V_22 , 1 ) ;
} else if ( ! F_29 ( V_2 , F_7 ( V_22 ) ) ) {
F_36 ( & V_2 -> V_65 , V_60 ) ;
return - V_51 ;
}
V_2 -> V_58 |= F_7 ( F_35 ( V_2 , V_9 ) ) ;
F_36 ( & V_2 -> V_65 , V_60 ) ;
if ( type & ( V_35 | V_37 ) )
F_37 ( V_7 -> V_62 , V_66 ) ;
else if ( type & ( V_41 | V_39 ) )
F_37 ( V_7 -> V_62 , V_67 ) ;
return V_59 ;
}
static void F_38 ( struct V_1 * V_2 , int V_68 )
{
void T_1 * V_11 = V_2 -> V_5 ;
V_11 += V_2 -> V_13 -> V_69 ;
F_8 ( V_68 , V_11 ) ;
if ( V_2 -> V_13 -> V_70 ) {
V_11 = V_2 -> V_5 + V_2 -> V_13 -> V_70 ;
F_8 ( V_68 , V_11 ) ;
}
F_6 ( V_11 ) ;
}
static inline void F_39 ( struct V_1 * V_2 , int V_9 )
{
F_38 ( V_2 , F_10 ( V_2 , V_9 ) ) ;
}
static T_2 F_40 ( struct V_1 * V_2 )
{
void T_1 * V_11 = V_2 -> V_5 ;
T_2 V_12 ;
T_2 V_24 = ( F_7 ( V_2 -> V_71 ) ) - 1 ;
V_11 += V_2 -> V_13 -> V_72 ;
V_12 = F_6 ( V_11 ) ;
if ( V_2 -> V_13 -> V_73 )
V_12 = ~ V_12 ;
V_12 &= V_24 ;
return V_12 ;
}
static void F_41 ( struct V_1 * V_2 , int V_68 )
{
void T_1 * V_11 = V_2 -> V_5 ;
T_2 V_12 ;
if ( V_2 -> V_13 -> V_74 ) {
V_11 += V_2 -> V_13 -> V_74 ;
V_12 = V_68 ;
V_2 -> V_15 . V_75 |= V_68 ;
} else {
V_11 += V_2 -> V_13 -> V_72 ;
V_12 = F_6 ( V_11 ) ;
if ( V_2 -> V_13 -> V_73 )
V_12 &= ~ V_68 ;
else
V_12 |= V_68 ;
V_2 -> V_15 . V_75 = V_12 ;
}
F_8 ( V_12 , V_11 ) ;
}
static void F_42 ( struct V_1 * V_2 , int V_68 )
{
void T_1 * V_11 = V_2 -> V_5 ;
T_2 V_12 ;
if ( V_2 -> V_13 -> V_76 ) {
V_11 += V_2 -> V_13 -> V_76 ;
V_12 = V_68 ;
V_2 -> V_15 . V_75 &= ~ V_68 ;
} else {
V_11 += V_2 -> V_13 -> V_72 ;
V_12 = F_6 ( V_11 ) ;
if ( V_2 -> V_13 -> V_73 )
V_12 |= V_68 ;
else
V_12 &= ~ V_68 ;
V_2 -> V_15 . V_75 = V_12 ;
}
F_8 ( V_12 , V_11 ) ;
}
static inline void F_43 ( struct V_1 * V_2 , int V_9 , int V_17 )
{
if ( V_17 )
F_41 ( V_2 , F_10 ( V_2 , V_9 ) ) ;
else
F_42 ( V_2 , F_10 ( V_2 , V_9 ) ) ;
}
static int F_44 ( struct V_1 * V_2 , int V_9 , int V_17 )
{
T_2 V_21 = F_10 ( V_2 , V_9 ) ;
unsigned long V_60 ;
if ( V_2 -> V_42 & V_21 ) {
F_45 ( V_2 -> V_61 ,
L_1 , V_9 ) ;
return - V_51 ;
}
F_34 ( & V_2 -> V_65 , V_60 ) ;
if ( V_17 )
V_2 -> V_15 . V_44 |= V_21 ;
else
V_2 -> V_15 . V_44 &= ~ V_21 ;
F_8 ( V_2 -> V_15 . V_44 , V_2 -> V_5 + V_2 -> V_13 -> V_43 ) ;
F_36 ( & V_2 -> V_65 , V_60 ) ;
return 0 ;
}
static void F_46 ( struct V_1 * V_2 , int V_9 )
{
F_5 ( V_2 , F_35 ( V_2 , V_9 ) , 1 ) ;
F_43 ( V_2 , V_9 , 0 ) ;
F_39 ( V_2 , V_9 ) ;
F_24 ( V_2 , F_35 ( V_2 , V_9 ) , V_77 ) ;
F_20 ( V_2 , V_9 ) ;
}
static int F_47 ( struct V_6 * V_7 , unsigned int V_17 )
{
struct V_1 * V_2 = F_2 ( V_7 ) ;
unsigned int V_9 = F_1 ( V_2 , V_7 -> V_64 ) ;
return F_44 ( V_2 , V_9 , V_17 ) ;
}
static int F_48 ( struct V_8 * V_4 , unsigned V_22 )
{
struct V_1 * V_2 = F_4 ( V_4 , struct V_1 , V_4 ) ;
unsigned long V_60 ;
if ( ! F_26 ( V_2 ) )
F_31 ( V_2 -> V_61 ) ;
F_34 ( & V_2 -> V_65 , V_60 ) ;
if ( ! F_28 ( V_2 -> V_58 , V_22 ) ) {
F_24 ( V_2 , V_22 , V_77 ) ;
F_25 ( V_2 , V_22 ) ;
}
V_2 -> V_57 |= F_7 ( V_22 ) ;
F_36 ( & V_2 -> V_65 , V_60 ) ;
return 0 ;
}
static void F_49 ( struct V_8 * V_4 , unsigned V_22 )
{
struct V_1 * V_2 = F_4 ( V_4 , struct V_1 , V_4 ) ;
unsigned long V_60 ;
F_34 ( & V_2 -> V_65 , V_60 ) ;
V_2 -> V_57 &= ~ ( F_7 ( V_22 ) ) ;
F_27 ( V_2 , V_22 ) ;
F_46 ( V_2 , V_2 -> V_4 . V_5 + V_22 ) ;
F_36 ( & V_2 -> V_65 , V_60 ) ;
if ( ! F_26 ( V_2 ) )
F_50 ( V_2 -> V_61 ) ;
}
static void F_51 ( unsigned int V_62 , struct V_78 * V_79 )
{
void T_1 * V_80 = NULL ;
T_2 V_81 ;
unsigned int V_82 ;
struct V_1 * V_2 ;
int V_83 = 0 ;
struct V_84 * V_85 = F_52 ( V_79 ) ;
struct V_8 * V_4 = F_53 ( V_62 ) ;
F_54 ( V_85 , V_79 ) ;
V_2 = F_4 ( V_4 , struct V_1 , V_4 ) ;
V_80 = V_2 -> V_5 + V_2 -> V_13 -> V_69 ;
F_31 ( V_2 -> V_61 ) ;
if ( F_55 ( ! V_80 ) )
goto exit;
while ( 1 ) {
T_2 V_86 , V_48 = 0 ;
T_2 V_87 ;
V_87 = F_40 ( V_2 ) ;
V_86 = V_81 = F_6 ( V_80 ) & V_87 ;
if ( V_2 -> V_48 )
V_48 = V_2 -> V_48 & V_87 ;
F_42 ( V_2 , V_86 & ~ V_48 ) ;
F_38 ( V_2 , V_86 & ~ V_48 ) ;
F_41 ( V_2 , V_86 & ~ V_48 ) ;
if ( ! V_48 && ! V_83 ) {
V_83 = 1 ;
F_56 ( V_85 , V_79 ) ;
}
if ( ! V_81 )
break;
while ( V_81 ) {
V_82 = F_57 ( V_81 ) ;
V_81 &= ~ ( F_7 ( V_82 ) ) ;
if ( V_2 -> V_50 & ( F_7 ( V_82 ) ) )
F_23 ( V_2 , V_82 ) ;
F_58 ( F_59 ( V_2 -> V_4 . V_88 ,
V_82 ) ) ;
}
}
exit:
if ( ! V_83 )
F_56 ( V_85 , V_79 ) ;
F_50 ( V_2 -> V_61 ) ;
}
static void F_60 ( struct V_6 * V_7 )
{
struct V_1 * V_2 = F_2 ( V_7 ) ;
unsigned int V_9 = F_1 ( V_2 , V_7 -> V_64 ) ;
unsigned long V_60 ;
unsigned V_22 = F_35 ( V_2 , V_9 ) ;
F_34 ( & V_2 -> V_65 , V_60 ) ;
F_61 ( & V_2 -> V_4 , V_22 ) ;
V_2 -> V_58 &= ~ ( F_7 ( V_22 ) ) ;
F_27 ( V_2 , V_22 ) ;
F_46 ( V_2 , V_9 ) ;
F_36 ( & V_2 -> V_65 , V_60 ) ;
if ( ! F_26 ( V_2 ) )
F_50 ( V_2 -> V_61 ) ;
}
static void F_62 ( struct V_6 * V_7 )
{
struct V_1 * V_2 = F_2 ( V_7 ) ;
unsigned int V_9 = F_1 ( V_2 , V_7 -> V_64 ) ;
F_39 ( V_2 , V_9 ) ;
}
static void F_63 ( struct V_6 * V_7 )
{
struct V_1 * V_2 = F_2 ( V_7 ) ;
unsigned int V_9 = F_1 ( V_2 , V_7 -> V_64 ) ;
unsigned long V_60 ;
F_34 ( & V_2 -> V_65 , V_60 ) ;
F_43 ( V_2 , V_9 , 0 ) ;
F_24 ( V_2 , F_35 ( V_2 , V_9 ) , V_77 ) ;
F_36 ( & V_2 -> V_65 , V_60 ) ;
}
static void F_64 ( struct V_6 * V_7 )
{
struct V_1 * V_2 = F_2 ( V_7 ) ;
unsigned int V_9 = F_1 ( V_2 , V_7 -> V_64 ) ;
unsigned int V_89 = F_10 ( V_2 , V_9 ) ;
T_2 V_33 = F_65 ( V_7 ) ;
unsigned long V_60 ;
F_34 ( & V_2 -> V_65 , V_60 ) ;
if ( V_33 )
F_24 ( V_2 , F_35 ( V_2 , V_9 ) , V_33 ) ;
if ( V_2 -> V_48 & V_89 ) {
F_43 ( V_2 , V_9 , 0 ) ;
F_39 ( V_2 , V_9 ) ;
}
F_43 ( V_2 , V_9 , 1 ) ;
F_36 ( & V_2 -> V_65 , V_60 ) ;
}
static int F_66 ( struct V_90 * V_61 )
{
struct V_91 * V_92 = F_67 ( V_61 ) ;
struct V_1 * V_2 = F_68 ( V_92 ) ;
void T_1 * V_93 = V_2 -> V_5 +
V_94 / V_2 -> V_95 ;
unsigned long V_60 ;
F_34 ( & V_2 -> V_65 , V_60 ) ;
F_8 ( 0xffff & ~ V_2 -> V_15 . V_44 , V_93 ) ;
F_36 ( & V_2 -> V_65 , V_60 ) ;
return 0 ;
}
static int F_69 ( struct V_90 * V_61 )
{
struct V_91 * V_92 = F_67 ( V_61 ) ;
struct V_1 * V_2 = F_68 ( V_92 ) ;
void T_1 * V_93 = V_2 -> V_5 +
V_94 / V_2 -> V_95 ;
unsigned long V_60 ;
F_34 ( & V_2 -> V_65 , V_60 ) ;
F_8 ( V_2 -> V_15 . V_44 , V_93 ) ;
F_36 ( & V_2 -> V_65 , V_60 ) ;
return 0 ;
}
static inline void F_70 ( struct V_1 * V_2 )
{
F_71 ( & V_96 , V_2 ) ;
if ( F_72 ( & V_97 ) == 0 )
( void ) F_73 ( & V_96 ) ;
}
static int F_74 ( struct V_8 * V_4 , unsigned V_22 )
{
struct V_1 * V_2 ;
unsigned long V_60 ;
void T_1 * V_11 ;
int V_98 ;
V_2 = F_4 ( V_4 , struct V_1 , V_4 ) ;
V_11 = V_2 -> V_5 + V_2 -> V_13 -> V_14 ;
F_34 ( & V_2 -> V_65 , V_60 ) ;
V_98 = ! ! ( F_6 ( V_11 ) & F_7 ( V_22 ) ) ;
F_36 ( & V_2 -> V_65 , V_60 ) ;
return V_98 ;
}
static int F_75 ( struct V_8 * V_4 , unsigned V_22 )
{
struct V_1 * V_2 ;
unsigned long V_60 ;
V_2 = F_4 ( V_4 , struct V_1 , V_4 ) ;
F_34 ( & V_2 -> V_65 , V_60 ) ;
F_5 ( V_2 , V_22 , 1 ) ;
F_36 ( & V_2 -> V_65 , V_60 ) ;
return 0 ;
}
static int F_76 ( struct V_8 * V_4 , unsigned V_22 )
{
struct V_1 * V_2 ;
T_2 V_24 ;
V_2 = F_4 ( V_4 , struct V_1 , V_4 ) ;
V_24 = ( F_7 ( V_22 ) ) ;
if ( F_29 ( V_2 , V_24 ) )
return F_12 ( V_2 , V_22 ) ;
else
return F_13 ( V_2 , V_22 ) ;
}
static int F_77 ( struct V_8 * V_4 , unsigned V_22 , int V_99 )
{
struct V_1 * V_2 ;
unsigned long V_60 ;
V_2 = F_4 ( V_4 , struct V_1 , V_4 ) ;
F_34 ( & V_2 -> V_65 , V_60 ) ;
V_2 -> V_18 ( V_2 , V_22 , V_99 ) ;
F_5 ( V_2 , V_22 , 0 ) ;
F_36 ( & V_2 -> V_65 , V_60 ) ;
return 0 ;
}
static int F_78 ( struct V_8 * V_4 , unsigned V_22 ,
unsigned V_30 )
{
struct V_1 * V_2 ;
unsigned long V_60 ;
V_2 = F_4 ( V_4 , struct V_1 , V_4 ) ;
F_34 ( & V_2 -> V_65 , V_60 ) ;
F_19 ( V_2 , V_22 , V_30 ) ;
F_36 ( & V_2 -> V_65 , V_60 ) ;
return 0 ;
}
static void F_79 ( struct V_8 * V_4 , unsigned V_22 , int V_99 )
{
struct V_1 * V_2 ;
unsigned long V_60 ;
V_2 = F_4 ( V_4 , struct V_1 , V_4 ) ;
F_34 ( & V_2 -> V_65 , V_60 ) ;
V_2 -> V_18 ( V_2 , V_22 , V_99 ) ;
F_36 ( & V_2 -> V_65 , V_60 ) ;
}
static void T_3 F_80 ( struct V_1 * V_2 )
{
static bool V_100 ;
T_2 V_101 ;
if ( V_100 || V_2 -> V_13 -> V_102 == V_103 )
return;
V_101 = F_81 ( V_2 -> V_5 + V_2 -> V_13 -> V_102 ) ;
F_82 ( L_2 ,
( V_101 >> 4 ) & 0x0f , V_101 & 0x0f ) ;
V_100 = true ;
}
static void F_83 ( struct V_1 * V_2 )
{
void T_1 * V_5 = V_2 -> V_5 ;
T_2 V_12 = 0xffffffff ;
if ( V_2 -> V_71 == 16 )
V_12 = 0xffff ;
if ( V_2 -> V_104 ) {
F_8 ( V_12 , V_2 -> V_5 + V_2 -> V_13 -> V_72 ) ;
return;
}
F_14 ( V_5 , V_2 -> V_13 -> V_72 , V_12 , V_2 -> V_13 -> V_73 ) ;
F_14 ( V_5 , V_2 -> V_13 -> V_69 , V_12 , ! V_2 -> V_13 -> V_73 ) ;
if ( V_2 -> V_13 -> V_29 )
F_8 ( 0 , V_5 + V_2 -> V_13 -> V_29 ) ;
V_2 -> V_15 . V_16 = F_6 ( V_2 -> V_5 + V_2 -> V_13 -> V_14 ) ;
if ( V_2 -> V_13 -> V_55 )
F_8 ( 0 , V_5 + V_2 -> V_13 -> V_55 ) ;
V_2 -> V_28 = F_84 ( V_2 -> V_61 , L_3 ) ;
if ( F_85 ( V_2 -> V_28 ) )
F_45 ( V_2 -> V_61 , L_4 ) ;
}
static void
F_86 ( struct V_1 * V_2 , unsigned int V_105 ,
unsigned int V_106 )
{
struct V_107 * V_108 ;
struct V_109 * V_110 ;
V_108 = F_87 ( L_5 , 1 , V_105 , V_2 -> V_5 ,
V_111 ) ;
if ( ! V_108 ) {
F_45 ( V_2 -> V_61 , L_6 ) ;
return;
}
V_110 = V_108 -> V_112 ;
V_110 -> V_4 . V_89 = V_113 ;
V_110 -> V_4 . V_114 = V_115 ;
V_110 -> V_4 . V_116 = F_30 ;
if ( V_2 -> V_13 -> V_43 )
V_110 -> V_4 . V_117 = F_47 ;
V_110 -> V_13 . V_24 = V_118 / V_2 -> V_95 ;
F_88 ( V_108 , F_89 ( V_106 ) , V_119 ,
V_120 | V_121 , 0 ) ;
}
static int F_90 ( struct V_1 * V_2 )
{
int V_122 ;
static int V_9 ;
int V_123 = 0 ;
int V_124 ;
V_2 -> V_4 . V_125 = F_48 ;
V_2 -> V_4 . free = F_49 ;
V_2 -> V_4 . V_126 = F_74 ;
V_2 -> V_4 . V_127 = F_75 ;
V_2 -> V_4 . V_128 = F_76 ;
V_2 -> V_4 . V_129 = F_77 ;
V_2 -> V_4 . V_130 = F_78 ;
V_2 -> V_4 . V_25 = F_79 ;
if ( V_2 -> V_104 ) {
V_2 -> V_4 . V_131 = L_7 ;
if ( V_2 -> V_13 -> V_43 )
V_2 -> V_4 . V_61 = & V_96 . V_61 ;
V_2 -> V_4 . V_5 = F_33 ( 0 ) ;
} else {
V_2 -> V_4 . V_131 = L_8 ;
V_2 -> V_4 . V_5 = V_9 ;
V_9 += V_2 -> V_71 ;
}
V_2 -> V_4 . V_132 = V_2 -> V_71 ;
V_124 = F_91 ( & V_2 -> V_4 ) ;
if ( V_124 ) {
F_45 ( V_2 -> V_61 , L_9 , V_124 ) ;
return V_124 ;
}
#ifdef F_32
V_123 = F_92 ( - 1 , 0 , V_2 -> V_71 , 0 ) ;
if ( V_123 < 0 ) {
F_45 ( V_2 -> V_61 , L_10 ) ;
return - V_133 ;
}
#endif
V_124 = F_93 ( & V_2 -> V_4 , & V_134 ,
V_123 , F_51 ,
V_77 ) ;
if ( V_124 ) {
F_45 ( V_2 -> V_61 , L_11 , V_124 ) ;
V_124 = F_94 ( & V_2 -> V_4 ) ;
return - V_133 ;
}
F_95 ( & V_2 -> V_4 , & V_134 ,
V_2 -> V_62 , F_51 ) ;
for ( V_122 = 0 ; V_122 < V_2 -> V_71 ; V_122 ++ ) {
int V_62 = F_59 ( V_2 -> V_4 . V_88 , V_122 ) ;
F_96 ( V_62 , & V_135 ) ;
if ( V_2 -> V_104 ) {
F_86 ( V_2 , V_62 , V_2 -> V_71 ) ;
F_97 ( V_62 , NULL , NULL ) ;
F_98 ( V_62 , 0 ) ;
}
}
return 0 ;
}
static int F_99 ( struct V_91 * V_92 )
{
struct V_90 * V_61 = & V_92 -> V_61 ;
struct V_136 * V_137 = V_61 -> V_138 ;
const struct V_139 * V_140 ;
const struct V_141 * V_142 ;
struct V_143 * V_144 ;
struct V_1 * V_2 ;
int V_124 ;
V_140 = F_100 ( F_101 ( V_145 ) , V_61 ) ;
V_142 = V_140 ? V_140 -> V_146 : F_102 ( V_61 ) ;
if ( ! V_142 )
return - V_51 ;
V_2 = F_103 ( V_61 , sizeof( struct V_1 ) , V_147 ) ;
if ( ! V_2 ) {
F_45 ( V_61 , L_12 ) ;
return - V_148 ;
}
V_144 = F_104 ( V_92 , V_149 , 0 ) ;
if ( F_105 ( ! V_144 ) ) {
F_45 ( V_61 , L_13 ) ;
return - V_133 ;
}
V_2 -> V_62 = V_144 -> V_150 ;
V_2 -> V_61 = V_61 ;
V_2 -> V_4 . V_61 = V_61 ;
V_2 -> V_32 = V_142 -> V_32 ;
V_2 -> V_95 = V_142 -> V_151 ;
V_2 -> V_71 = V_142 -> V_152 ;
V_2 -> V_104 = V_142 -> V_104 ;
V_2 -> V_42 = V_142 -> V_42 ;
V_2 -> V_13 = V_142 -> V_13 ;
#ifdef F_106
V_2 -> V_4 . V_138 = F_107 ( V_137 ) ;
#endif
if ( V_137 ) {
if ( ! F_108 ( V_137 , L_14 ) )
V_2 -> V_153 = true ;
} else {
V_2 -> V_153 = V_142 -> V_153 ;
if ( V_2 -> V_153 )
V_2 -> V_154 =
V_142 -> V_154 ;
}
if ( V_2 -> V_13 -> V_18 && V_2 -> V_13 -> V_20 )
V_2 -> V_18 = F_9 ;
else
V_2 -> V_18 = F_11 ;
F_109 ( & V_2 -> V_65 ) ;
V_144 = F_104 ( V_92 , V_155 , 0 ) ;
V_2 -> V_5 = F_110 ( V_61 , V_144 ) ;
if ( F_85 ( V_2 -> V_5 ) ) {
F_111 ( V_2 -> V_4 . V_88 ) ;
return F_112 ( V_2 -> V_5 ) ;
}
F_71 ( V_92 , V_2 ) ;
F_113 ( V_2 -> V_61 ) ;
F_114 ( V_2 -> V_61 ) ;
F_31 ( V_2 -> V_61 ) ;
if ( V_2 -> V_104 )
F_70 ( V_2 ) ;
F_83 ( V_2 ) ;
V_124 = F_90 ( V_2 ) ;
if ( V_124 )
return V_124 ;
F_80 ( V_2 ) ;
F_50 ( V_2 -> V_61 ) ;
F_115 ( & V_2 -> V_137 , & V_156 ) ;
return 0 ;
}
static int F_116 ( struct V_90 * V_61 )
{
struct V_91 * V_92 = F_67 ( V_61 ) ;
struct V_1 * V_2 = F_68 ( V_92 ) ;
T_2 V_157 = 0 , V_158 = 0 ;
unsigned long V_60 ;
T_2 V_159 , V_160 ;
F_34 ( & V_2 -> V_65 , V_60 ) ;
V_159 = V_2 -> V_15 . V_34 & V_2 -> V_15 . V_44 ;
if ( V_159 )
F_8 ( V_159 | V_2 -> V_15 . V_40 ,
V_2 -> V_5 + V_2 -> V_13 -> V_40 ) ;
V_160 = V_2 -> V_15 . V_36 & V_2 -> V_15 . V_44 ;
if ( V_160 )
F_8 ( V_160 | V_2 -> V_15 . V_38 ,
V_2 -> V_5 + V_2 -> V_13 -> V_38 ) ;
if ( ! V_2 -> V_47 )
goto V_161;
if ( V_2 -> V_162 != V_163 ) {
V_2 -> V_162 = 0 ;
goto V_161;
}
V_2 -> V_164 = F_6 ( V_2 -> V_5 +
V_2 -> V_13 -> V_23 ) ;
V_157 = V_2 -> V_15 . V_40 ;
V_158 = V_2 -> V_15 . V_38 ;
V_157 &= ~ V_2 -> V_47 ;
V_158 &= ~ V_2 -> V_47 ;
F_8 ( V_157 , V_2 -> V_5 + V_2 -> V_13 -> V_40 ) ;
F_8 ( V_158 , V_2 -> V_5 + V_2 -> V_13 -> V_38 ) ;
V_2 -> V_165 = true ;
V_161:
if ( V_2 -> V_154 )
V_2 -> V_166 =
V_2 -> V_154 ( V_2 -> V_61 ) ;
F_17 ( V_2 ) ;
F_36 ( & V_2 -> V_65 , V_60 ) ;
return 0 ;
}
static int F_117 ( struct V_90 * V_61 )
{
struct V_91 * V_92 = F_67 ( V_61 ) ;
struct V_1 * V_2 = F_68 ( V_92 ) ;
T_2 V_12 = 0 , V_167 , V_168 , V_169 ;
unsigned long V_60 ;
int V_170 ;
F_34 ( & V_2 -> V_65 , V_60 ) ;
if ( V_2 -> V_153 && ! V_2 -> V_171 ) {
F_118 ( V_2 ) ;
if ( V_2 -> V_154 )
V_2 -> V_166 =
V_2 -> V_154 ( V_2 -> V_61 ) ;
}
F_15 ( V_2 ) ;
F_8 ( V_2 -> V_15 . V_40 ,
V_2 -> V_5 + V_2 -> V_13 -> V_40 ) ;
F_8 ( V_2 -> V_15 . V_38 ,
V_2 -> V_5 + V_2 -> V_13 -> V_38 ) ;
if ( V_2 -> V_153 ) {
if ( ! V_2 -> V_154 ) {
F_119 ( V_2 ) ;
} else {
V_170 = V_2 -> V_154 ( V_2 -> V_61 ) ;
if ( V_170 != V_2 -> V_166 ) {
F_119 ( V_2 ) ;
} else {
F_36 ( & V_2 -> V_65 , V_60 ) ;
return 0 ;
}
}
}
if ( ! V_2 -> V_165 ) {
F_36 ( & V_2 -> V_65 , V_60 ) ;
return 0 ;
}
V_12 = F_6 ( V_2 -> V_5 + V_2 -> V_13 -> V_23 ) ;
V_12 ^= V_2 -> V_164 ;
V_12 &= V_2 -> V_47 ;
V_168 = V_12 & V_2 -> V_15 . V_40 ;
V_168 &= V_2 -> V_164 ;
V_169 = V_12 & V_2 -> V_15 . V_38 ;
V_169 &= ~ ( V_2 -> V_164 ) ;
V_167 = V_12 & ( ~ ( V_2 -> V_15 . V_40 ) &
~ ( V_2 -> V_15 . V_38 ) ) ;
V_167 |= V_168 | V_169 ;
if ( V_167 ) {
T_2 V_172 , V_173 ;
V_172 = F_6 ( V_2 -> V_5 + V_2 -> V_13 -> V_34 ) ;
V_173 = F_6 ( V_2 -> V_5 + V_2 -> V_13 -> V_36 ) ;
if ( ! V_2 -> V_13 -> V_174 ) {
F_8 ( V_172 | V_167 , V_2 -> V_5 +
V_2 -> V_13 -> V_34 ) ;
F_8 ( V_173 | V_167 , V_2 -> V_5 +
V_2 -> V_13 -> V_36 ) ;
}
if ( V_2 -> V_13 -> V_174 ) {
F_8 ( V_172 | V_12 , V_2 -> V_5 +
V_2 -> V_13 -> V_34 ) ;
F_8 ( V_173 | V_12 , V_2 -> V_5 +
V_2 -> V_13 -> V_36 ) ;
}
F_8 ( V_172 , V_2 -> V_5 + V_2 -> V_13 -> V_34 ) ;
F_8 ( V_173 , V_2 -> V_5 + V_2 -> V_13 -> V_36 ) ;
}
V_2 -> V_165 = false ;
F_36 ( & V_2 -> V_65 , V_60 ) ;
return 0 ;
}
void F_120 ( int V_175 )
{
struct V_1 * V_2 ;
F_121 (bank, &omap_gpio_list, node) {
if ( ! F_26 ( V_2 ) || ! V_2 -> V_153 )
continue;
V_2 -> V_162 = V_175 ;
F_122 ( V_2 -> V_61 ) ;
}
}
void F_123 ( void )
{
struct V_1 * V_2 ;
F_121 (bank, &omap_gpio_list, node) {
if ( ! F_26 ( V_2 ) || ! V_2 -> V_153 )
continue;
F_31 ( V_2 -> V_61 ) ;
}
}
static void F_118 ( struct V_1 * V_176 )
{
struct V_177 * V_13 = V_176 -> V_13 ;
void T_1 * V_5 = V_176 -> V_5 ;
V_176 -> V_15 . V_55 = F_6 ( V_5 + V_13 -> V_55 ) ;
V_176 -> V_15 . V_16 = F_6 ( V_5 + V_13 -> V_14 ) ;
V_176 -> V_15 . V_44 = F_6 ( V_5 + V_13 -> V_43 ) ;
V_176 -> V_15 . V_34 = F_6 ( V_5 + V_13 -> V_34 ) ;
V_176 -> V_15 . V_36 = F_6 ( V_5 + V_13 -> V_36 ) ;
V_176 -> V_15 . V_38 = F_6 ( V_5 + V_13 -> V_38 ) ;
V_176 -> V_15 . V_40 = F_6 ( V_5 + V_13 -> V_40 ) ;
V_176 -> V_15 . V_75 = F_6 ( V_5 + V_13 -> V_72 ) ;
V_176 -> V_15 . V_178 = F_6 ( V_5 + V_13 -> V_178 ) ;
if ( V_13 -> V_18 && V_176 -> V_13 -> V_20 )
V_176 -> V_15 . V_19 = F_6 ( V_5 + V_13 -> V_18 ) ;
else
V_176 -> V_15 . V_19 = F_6 ( V_5 + V_13 -> V_19 ) ;
V_176 -> V_171 = true ;
}
static void F_119 ( struct V_1 * V_2 )
{
F_8 ( V_2 -> V_15 . V_44 ,
V_2 -> V_5 + V_2 -> V_13 -> V_43 ) ;
F_8 ( V_2 -> V_15 . V_55 , V_2 -> V_5 + V_2 -> V_13 -> V_55 ) ;
F_8 ( V_2 -> V_15 . V_34 ,
V_2 -> V_5 + V_2 -> V_13 -> V_34 ) ;
F_8 ( V_2 -> V_15 . V_36 ,
V_2 -> V_5 + V_2 -> V_13 -> V_36 ) ;
F_8 ( V_2 -> V_15 . V_38 ,
V_2 -> V_5 + V_2 -> V_13 -> V_38 ) ;
F_8 ( V_2 -> V_15 . V_40 ,
V_2 -> V_5 + V_2 -> V_13 -> V_40 ) ;
if ( V_2 -> V_13 -> V_18 && V_2 -> V_13 -> V_20 )
F_8 ( V_2 -> V_15 . V_19 ,
V_2 -> V_5 + V_2 -> V_13 -> V_18 ) ;
else
F_8 ( V_2 -> V_15 . V_19 ,
V_2 -> V_5 + V_2 -> V_13 -> V_19 ) ;
F_8 ( V_2 -> V_15 . V_16 , V_2 -> V_5 + V_2 -> V_13 -> V_14 ) ;
if ( V_2 -> V_26 ) {
F_8 ( V_2 -> V_15 . V_30 , V_2 -> V_5 +
V_2 -> V_13 -> V_30 ) ;
F_8 ( V_2 -> V_15 . V_29 ,
V_2 -> V_5 + V_2 -> V_13 -> V_29 ) ;
}
F_8 ( V_2 -> V_15 . V_75 ,
V_2 -> V_5 + V_2 -> V_13 -> V_72 ) ;
F_8 ( V_2 -> V_15 . V_178 ,
V_2 -> V_5 + V_2 -> V_13 -> V_178 ) ;
}
static inline void F_118 ( struct V_1 * V_176 ) {}
static int T_3 F_124 ( void )
{
return F_72 ( & V_179 ) ;
}
