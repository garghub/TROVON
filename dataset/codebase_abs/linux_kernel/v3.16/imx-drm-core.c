int F_1 ( struct V_1 * V_2 )
{
return V_2 -> V_3 ;
}
static void F_2 ( struct V_4 * V_5 )
{
#if F_3 ( V_6 )
struct V_7 * V_8 = V_5 -> V_9 ;
if ( V_8 -> V_10 )
F_4 ( V_8 -> V_10 ) ;
#endif
}
static int F_5 ( struct V_4 * V_5 )
{
#if F_3 ( V_6 )
struct V_7 * V_8 = V_5 -> V_9 ;
#endif
F_6 ( V_5 ) ;
#if F_3 ( V_6 )
if ( V_8 -> V_10 )
F_7 ( V_8 -> V_10 ) ;
#endif
F_8 ( V_5 -> V_11 , V_5 ) ;
F_9 ( V_5 ) ;
F_10 ( V_5 ) ;
return 0 ;
}
static struct V_1 * F_11 ( struct V_12 * V_2 )
{
struct V_7 * V_8 = V_2 -> V_11 -> V_9 ;
unsigned V_13 ;
for ( V_13 = 0 ; V_13 < V_14 ; V_13 ++ )
if ( V_8 -> V_2 [ V_13 ] && V_8 -> V_2 [ V_13 ] -> V_2 == V_2 )
return V_8 -> V_2 [ V_13 ] ;
return NULL ;
}
int F_12 ( struct V_15 * V_16 ,
T_1 V_17 , int V_18 , int V_19 )
{
struct V_20 * V_21 ;
struct V_1 * V_22 ;
V_22 = F_11 ( V_16 -> V_2 ) ;
if ( ! V_22 )
return - V_23 ;
V_21 = & V_22 -> V_24 ;
if ( V_21 -> V_25 )
return V_21 -> V_25 ( V_16 -> V_2 ,
V_16 -> V_26 , V_17 ,
V_18 , V_19 ) ;
return 0 ;
}
int F_13 ( struct V_15 * V_16 , T_1 V_17 )
{
return F_12 ( V_16 , V_17 , 2 , 3 ) ;
}
int F_14 ( struct V_1 * V_1 )
{
return F_15 ( V_1 -> V_2 -> V_11 , V_1 -> V_3 ) ;
}
void F_16 ( struct V_1 * V_1 )
{
F_17 ( V_1 -> V_2 -> V_11 , V_1 -> V_3 ) ;
}
void F_18 ( struct V_1 * V_1 )
{
F_19 ( V_1 -> V_2 -> V_11 , V_1 -> V_3 ) ;
}
static int F_20 ( struct V_4 * V_5 , int V_2 )
{
struct V_7 * V_8 = V_5 -> V_9 ;
struct V_1 * V_1 = V_8 -> V_2 [ V_2 ] ;
int V_27 ;
if ( ! V_1 )
return - V_23 ;
if ( ! V_1 -> V_24 . V_28 )
return - V_29 ;
V_27 = V_1 -> V_24 . V_28 (
V_1 -> V_2 ) ;
return V_27 ;
}
static void F_21 ( struct V_4 * V_5 , int V_2 )
{
struct V_7 * V_8 = V_5 -> V_9 ;
struct V_1 * V_1 = V_8 -> V_2 [ V_2 ] ;
if ( ! V_1 )
return;
if ( ! V_1 -> V_24 . V_30 )
return;
V_1 -> V_24 . V_30 ( V_1 -> V_2 ) ;
}
static void F_22 ( struct V_4 * V_5 ,
struct V_31 * V_32 )
{
int V_13 ;
if ( ! V_32 -> V_33 )
return;
for ( V_13 = 0 ; V_13 < V_14 ; V_13 ++ )
F_21 ( V_5 , V_13 ) ;
}
void F_23 ( struct V_34 * V_35 )
{
F_24 ( V_35 ) ;
F_25 ( V_35 ) ;
}
void F_26 ( struct V_15 * V_16 )
{
F_27 ( V_16 ) ;
}
static void F_28 ( struct V_4 * V_5 )
{
#if F_3 ( V_6 )
struct V_7 * V_8 = V_5 -> V_9 ;
F_29 ( V_8 -> V_10 ) ;
#endif
}
static int F_30 ( struct V_4 * V_5 , unsigned long V_36 )
{
struct V_7 * V_8 ;
struct V_34 * V_35 ;
int V_27 ;
V_8 = F_31 ( V_5 -> V_11 , sizeof( * V_8 ) , V_37 ) ;
if ( ! V_8 )
return - V_38 ;
V_8 -> V_5 = V_5 ;
V_5 -> V_9 = V_8 ;
V_5 -> V_39 = true ;
V_5 -> V_40 . V_41 = 64 ;
V_5 -> V_40 . V_42 = 64 ;
V_5 -> V_40 . V_43 = 4096 ;
V_5 -> V_40 . V_44 = 4096 ;
V_5 -> V_40 . V_45 = & V_46 ;
F_32 ( V_5 ) ;
V_27 = F_33 ( V_5 , V_14 ) ;
if ( V_27 )
goto V_47;
V_5 -> V_48 = true ;
F_34 ( V_5 -> V_49 , V_5 ) ;
V_27 = F_35 ( V_5 -> V_11 , V_5 ) ;
if ( V_27 )
goto V_50;
F_36 (connector, &drm->mode_config.connector_list, head) {
V_27 = F_37 ( V_35 ) ;
if ( V_27 ) {
F_38 ( V_5 -> V_11 ,
L_1 ,
V_35 -> V_51 . V_52 ,
V_35 -> V_53 , V_27 ) ;
goto V_54;
}
}
#if F_3 ( V_6 )
if ( V_55 != 16 && V_55 != 32 ) {
F_39 ( V_5 -> V_11 , L_2 ) ;
V_55 = 16 ;
}
V_8 -> V_10 = F_40 ( V_5 , V_55 ,
V_5 -> V_40 . V_56 , V_14 ) ;
if ( F_41 ( V_8 -> V_10 ) ) {
V_27 = F_42 ( V_8 -> V_10 ) ;
V_8 -> V_10 = NULL ;
goto V_54;
}
#endif
F_43 ( V_5 ) ;
return 0 ;
V_54:
F_8 ( V_5 -> V_11 , V_5 ) ;
V_50:
F_9 ( V_5 ) ;
V_47:
F_10 ( V_5 ) ;
return V_27 ;
}
int F_44 ( struct V_4 * V_5 , struct V_12 * V_2 ,
struct V_1 * * V_57 ,
const struct V_20 * V_24 ,
struct V_58 * V_59 )
{
struct V_7 * V_8 = V_5 -> V_9 ;
struct V_1 * V_1 ;
int V_27 ;
if ( V_8 -> V_60 >= V_14 )
return - V_23 ;
if ( V_8 -> V_5 -> V_61 )
return - V_62 ;
V_1 = F_45 ( sizeof( * V_1 ) , V_37 ) ;
if ( ! V_1 )
return - V_38 ;
V_1 -> V_24 = * V_24 ;
V_1 -> V_3 = V_8 -> V_60 ++ ;
V_1 -> V_59 = V_59 ;
V_1 -> V_2 = V_2 ;
V_8 -> V_2 [ V_1 -> V_3 ] = V_1 ;
* V_57 = V_1 ;
V_27 = F_46 ( V_1 -> V_2 , 256 ) ;
if ( V_27 )
goto V_63;
F_47 ( V_2 ,
V_1 -> V_24 . V_64 ) ;
F_48 ( V_5 , V_2 ,
V_1 -> V_24 . V_65 ) ;
return 0 ;
V_63:
V_8 -> V_2 [ V_1 -> V_3 ] = NULL ;
F_49 ( V_1 ) ;
return V_27 ;
}
int F_50 ( struct V_1 * V_1 )
{
struct V_7 * V_8 = V_1 -> V_2 -> V_11 -> V_9 ;
F_51 ( V_1 -> V_2 ) ;
V_8 -> V_2 [ V_1 -> V_3 ] = NULL ;
F_49 ( V_1 ) ;
return 0 ;
}
static T_2 F_52 ( struct V_7 * V_8 ,
struct V_58 * V_66 )
{
struct V_58 * V_59 ;
unsigned V_13 ;
V_59 = F_53 ( V_66 ) ;
if ( ! V_59 )
return 0 ;
F_54 ( V_59 ) ;
for ( V_13 = 0 ; V_13 < V_14 ; V_13 ++ ) {
struct V_1 * V_1 = V_8 -> V_2 [ V_13 ] ;
if ( V_1 && V_1 -> V_59 == V_59 )
return F_55 ( V_1 -> V_2 ) ;
}
return 0 ;
}
static struct V_58 * F_56 (
const struct V_58 * V_67 , struct V_58 * V_68 )
{
struct V_58 * V_69 = F_57 ( V_67 , V_68 ) ;
F_54 ( V_68 ) ;
return V_69 ;
}
int F_58 ( struct V_4 * V_5 ,
struct V_15 * V_16 , struct V_58 * V_70 )
{
struct V_7 * V_8 = V_5 -> V_9 ;
struct V_58 * V_71 = NULL ;
T_2 V_72 = 0 ;
int V_13 ;
for ( V_13 = 0 ; ; V_13 ++ ) {
T_1 V_73 ;
V_71 = F_56 ( V_70 , V_71 ) ;
if ( ! V_71 )
break;
V_73 = F_52 ( V_8 , V_71 ) ;
if ( V_73 == 0 )
return - V_74 ;
V_72 |= V_73 ;
}
if ( V_71 )
F_54 ( V_71 ) ;
if ( V_13 == 0 )
return - V_75 ;
V_16 -> V_76 = V_72 ;
V_16 -> V_77 = ~ 0 ;
return 0 ;
}
int F_59 ( struct V_58 * V_69 ,
struct V_15 * V_16 )
{
struct V_1 * V_22 = F_11 ( V_16 -> V_2 ) ;
struct V_58 * V_71 = NULL ;
struct V_78 V_66 ;
struct V_58 * V_59 ;
int V_27 ;
if ( ! V_69 || ! V_22 )
return - V_23 ;
do {
V_71 = F_56 ( V_69 , V_71 ) ;
if ( ! V_71 )
break;
V_59 = F_53 ( V_71 ) ;
F_54 ( V_59 ) ;
if ( V_59 == V_22 -> V_59 ) {
V_27 = F_60 ( V_71 , & V_66 ) ;
return V_27 ? V_27 : V_66 . V_59 ;
}
} while ( V_71 );
return - V_23 ;
}
static int F_61 ( struct V_79 * V_11 , void * V_80 )
{
struct V_58 * V_70 = V_80 ;
if ( F_62 ( V_70 -> V_53 , L_3 ) == 0 ) {
V_70 = F_63 ( V_70 ) ;
F_54 ( V_70 ) ;
}
return V_11 -> V_81 == V_70 ;
}
static int F_64 ( struct V_79 * V_82 , struct V_82 * V_83 )
{
struct V_84 * V_85 ;
int V_27 ;
F_36 (component, &imx_drm_components, list) {
V_27 = F_65 ( V_83 , F_61 ,
V_85 -> V_81 ) ;
if ( V_27 )
return V_27 ;
}
return 0 ;
}
static int F_66 ( struct V_79 * V_11 )
{
return F_67 ( & V_86 , F_68 ( V_11 ) ) ;
}
static void F_69 ( struct V_79 * V_11 )
{
F_70 ( F_71 ( V_11 ) ) ;
}
static struct V_84 * F_72 ( struct V_79 * V_11 ,
struct V_58 * V_69 )
{
struct V_84 * V_85 ;
F_36 (component, &imx_drm_components, list)
if ( V_85 -> V_81 == V_69 )
return V_85 ;
return NULL ;
}
static int F_73 ( struct V_79 * V_11 , struct V_58 * V_69 )
{
struct V_84 * V_85 ;
if ( F_72 ( V_11 , V_69 ) )
return 0 ;
V_85 = F_31 ( V_11 , sizeof( * V_85 ) , V_37 ) ;
if ( ! V_85 )
return - V_38 ;
V_85 -> V_81 = V_69 ;
F_74 ( & V_85 -> V_87 , & V_88 ) ;
return 0 ;
}
static int F_75 ( struct V_89 * V_90 )
{
struct V_58 * V_71 , * V_59 , * V_91 ;
int V_27 ;
int V_13 ;
for ( V_13 = 0 ; ; V_13 ++ ) {
V_59 = F_76 ( V_90 -> V_11 . V_81 , L_4 , V_13 ) ;
if ( ! V_59 )
break;
V_27 = F_73 ( & V_90 -> V_11 , V_59 ) ;
if ( V_27 < 0 )
return V_27 ;
}
if ( V_13 == 0 ) {
F_38 ( & V_90 -> V_11 , L_5 ) ;
return - V_92 ;
}
for ( V_13 = 0 ; ; V_13 ++ ) {
V_59 = F_76 ( V_90 -> V_11 . V_81 , L_4 , V_13 ) ;
if ( ! V_59 )
break;
F_77 (port, ep) {
V_91 = F_78 ( V_71 ) ;
if ( ! V_91 || ! F_79 ( V_91 ) ) {
F_54 ( V_91 ) ;
continue;
} else if ( ! F_79 ( V_91 -> V_67 ) ) {
F_39 ( & V_90 -> V_11 , L_6 ,
V_91 -> V_93 ) ;
F_54 ( V_91 ) ;
continue;
}
V_27 = F_73 ( & V_90 -> V_11 , V_91 ) ;
F_54 ( V_91 ) ;
if ( V_27 < 0 )
return V_27 ;
}
F_54 ( V_59 ) ;
}
V_27 = F_80 ( & V_90 -> V_11 , F_81 ( 32 ) ) ;
if ( V_27 )
return V_27 ;
return F_82 ( & V_90 -> V_11 , & V_94 ) ;
}
static int F_83 ( struct V_89 * V_90 )
{
F_84 ( & V_90 -> V_11 , & V_94 ) ;
return 0 ;
}
