static T_1
F_1 ( struct V_1 * V_2 )
{
T_1 V_3 ;
int V_4 = 0 ;
do {
F_2 ( 1 ) ;
if ( ++ V_4 > V_5 )
return V_6 ;
V_3 = F_3 ( V_2 , V_7 ) ;
} while ( ! F_4 ( V_3 ) );
return V_3 ;
}
void
F_5 ( struct V_1 * V_2 , struct V_8 * V_9 )
{
T_1 V_3 ;
T_1 V_10 ;
struct V_11 * V_12 = V_2 -> V_12 ;
struct V_13 * V_14 = V_2 -> V_14 ;
V_10 = F_6 ( V_14 -> V_15 ,
V_2 -> V_16 ) ;
if ( F_7 ( V_2 ) ) {
V_9 -> V_3 . V_9 = V_17 ;
return;
}
F_8 ( V_2 , V_18 , V_10 ) ;
F_8 ( V_2 , V_19 , V_9 -> V_20 . V_21 ) ;
F_8 ( V_2 , V_22 , V_9 -> V_20 . V_23 ) ;
F_8 ( V_2 , V_24 , V_9 -> V_20 . V_25 ) ;
F_8 ( V_2 , V_7 ,
F_9 ( V_9 -> V_20 . V_9 ) ) ;
V_3 = F_1 ( V_2 ) ;
if ( V_3 == V_6 ) {
F_10 ( & V_12 -> V_26 , L_1 ) ;
V_9 -> V_3 . V_9 = V_17 ;
} else if ( V_3 == V_27 ) {
V_9 -> V_3 . V_9 = F_3 ( V_2 , V_19 ) ;
F_10 ( & V_12 -> V_26 , L_2 ,
V_9 -> V_3 . V_9 ) ;
} else if ( V_3 == V_28 ) {
V_9 -> V_3 . V_9 = V_29 ;
if ( V_9 -> V_3 . V_23 )
V_9 -> V_3 . V_23 = F_3 ( V_2 ,
V_22 ) ;
if ( V_9 -> V_3 . V_25 )
V_9 -> V_3 . V_25 = F_3 ( V_2 ,
V_24 ) ;
}
if ( V_9 -> V_3 . V_21 )
V_9 -> V_3 . V_21 = F_3 ( V_2 , V_19 ) ;
F_11 ( V_2 ) ;
}
static T_2 F_12 ( T_2 * V_30 , T_3 V_31 )
{
T_4 V_32 = 0 ;
int V_33 = V_31 / sizeof( T_2 ) ;
while ( V_33 -- > 0 )
V_32 += * V_30 ++ ;
while ( V_32 >> 32 )
V_32 = ( V_32 & 0xFFFFFFFF ) + ( V_32 >> 32 ) ;
return ~ V_32 ;
}
int F_13 ( struct V_1 * V_2 )
{
int V_34 , V_35 ;
T_3 V_31 ;
void * V_36 ;
T_1 V_37 , V_38 , * V_39 , * V_40 ;
struct V_8 V_9 ;
struct V_13 * V_14 ;
struct V_41 * V_42 , * V_43 ;
T_5 V_44 = 0 ;
V_14 = V_2 -> V_14 ;
memset ( & V_9 , 0 , sizeof( V_9 ) ) ;
V_9 . V_20 . V_9 = V_45 ;
memset ( & V_9 . V_3 , 1 , sizeof( struct V_46 ) ) ;
F_5 ( V_2 , & V_9 ) ;
if ( V_9 . V_3 . V_9 != V_29 ) {
F_14 ( & V_2 -> V_12 -> V_26 ,
L_3 , V_9 . V_3 . V_9 ) ;
V_34 = - V_47 ;
return V_34 ;
}
V_31 = V_9 . V_3 . V_23 ;
V_37 = V_9 . V_3 . V_25 ;
if ( ! V_31 )
return - V_47 ;
V_36 = F_15 ( & V_2 -> V_12 -> V_26 , V_31 ,
& V_44 , V_48 ) ;
if ( ! V_36 ) {
F_10 ( & V_2 -> V_12 -> V_26 ,
L_4 ) ;
return - V_49 ;
}
memset ( & V_9 . V_3 , 0 , sizeof( struct V_46 ) ) ;
V_9 . V_20 . V_9 = V_50 ;
V_9 . V_20 . V_21 = F_16 ( V_44 ) ;
V_9 . V_20 . V_23 = F_17 ( V_44 ) ;
V_9 . V_20 . V_25 = V_31 ;
F_5 ( V_2 , & V_9 ) ;
V_34 = V_9 . V_3 . V_9 ;
if ( V_34 != V_29 ) {
F_10 ( & V_2 -> V_12 -> V_26 ,
L_5 , V_34 ) ;
V_34 = - V_47 ;
goto error;
}
V_43 = V_36 ;
V_38 = F_12 ( ( T_2 * ) V_36 , V_31 ) ;
if ( V_38 ) {
F_10 ( & V_2 -> V_12 -> V_26 ,
L_6 ) ;
V_34 = - V_47 ;
goto error;
}
V_14 -> V_51 . V_42 = F_18 ( V_31 ) ;
if ( ! V_14 -> V_51 . V_42 ) {
V_34 = - V_47 ;
goto error;
}
V_40 = V_36 ;
V_39 = ( T_1 * ) V_14 -> V_51 . V_42 ;
for ( V_35 = 0 ; V_35 < V_31 / sizeof( T_1 ) ; V_35 ++ )
* V_39 ++ = F_19 ( * V_40 ++ ) ;
V_42 = V_14 -> V_51 . V_42 ;
V_42 -> V_52 = V_53 ;
V_14 -> V_51 . V_54 = 1 ;
error:
F_20 ( & V_2 -> V_12 -> V_26 , V_31 , V_36 , V_44 ) ;
return V_34 ;
}
int
F_21 ( struct V_1 * V_2 , int V_55 )
{
struct V_8 V_9 ;
struct V_56 * V_57 = V_2 -> V_57 ;
memset ( & V_9 , 0 , sizeof( V_9 ) ) ;
V_9 . V_20 . V_9 = V_58 ;
V_9 . V_20 . V_21 = V_57 -> V_59 ;
V_9 . V_20 . V_23 = V_55 ;
V_9 . V_20 . V_25 = 0 ;
if ( V_57 -> V_60 == V_61 ) {
F_5 ( V_2 , & V_9 ) ;
if ( V_9 . V_3 . V_9 ) {
F_10 ( & V_2 -> V_12 -> V_26 , L_7 ) ;
return - V_47 ;
}
}
return 0 ;
}
static int
F_22 ( struct V_1 * V_2 )
{
void * V_62 ;
struct V_63 * V_64 ;
struct V_65 * V_66 ;
struct V_67 * V_68 ;
struct V_69 * V_70 ;
struct V_71 * V_72 ;
struct V_73 * V_74 ;
struct V_75 * V_76 ;
struct V_77 * V_78 ;
struct V_8 V_9 ;
T_5 V_79 , V_80 ;
T_6 V_81 ;
T_7 V_35 , V_82 , V_83 ;
T_8 V_84 , V_85 ;
T_1 V_86 , V_87 , V_88 , V_89 ;
int V_34 ;
struct V_56 * V_57 = V_2 -> V_57 ;
V_82 = V_2 -> V_90 ;
V_83 = V_2 -> V_91 ;
V_84 =
F_23 ( struct V_63 , V_82 ,
V_83 ) ;
V_85 =
F_24 ( struct V_65 , V_82 ,
V_83 ) ;
V_62 = F_15 ( & V_2 -> V_12 -> V_26 , V_84 ,
& V_79 , V_48 ) ;
if ( V_62 == NULL )
return - V_49 ;
V_64 = V_62 ;
V_62 = F_15 ( & V_2 -> V_12 -> V_26 , V_85 ,
& V_80 , V_48 ) ;
if ( V_62 == NULL ) {
V_34 = - V_49 ;
goto V_92;
}
V_66 = V_62 ;
V_64 -> V_93 = F_25 ( V_80 ) ;
V_86 = ( V_94 | V_95
| V_96 ) ;
V_86 |= ( V_97 | V_98 ) ;
V_64 -> V_99 = F_26 ( struct V_63 ,
V_100 ) ;
V_64 -> V_101 = V_83 - 1 ;
V_64 -> V_102 [ 0 ] = F_27 ( V_86 ) ;
V_64 -> V_103 =
F_27 ( V_104 ) ;
V_64 -> V_105 =
F_27 ( V_106 ) ;
V_64 -> V_107 = F_28 ( V_82 ) ;
V_64 -> V_108 = F_28 ( V_83 ) ;
V_64 -> V_109 = 0 ;
V_88 = F_29 ( V_64 -> V_109 ) +
( sizeof( struct V_67 ) * V_82 ) ;
V_64 -> V_110 = F_27 ( V_88 ) ;
V_68 = (struct V_67 * ) ( V_64 -> V_111 +
F_29 ( V_64 -> V_109 ) ) ;
for ( V_35 = 0 ; V_35 < V_82 ; V_35 ++ ) {
V_76 = & V_57 -> V_112 [ V_35 ] ;
V_76 -> V_113 = 0 ;
V_68 [ V_35 ] . V_114 = F_25 ( V_76 -> V_81 ) ;
V_68 [ V_35 ] . V_115 = F_27 ( V_76 -> V_116 ) ;
V_68 [ V_35 ] . V_117 = F_27 ( V_35 ) ;
V_68 [ V_35 ] . V_118 = F_25 ( V_76 -> V_119 ) ;
}
V_70 = (struct V_69 * ) ( V_64 -> V_111 +
F_29 ( V_64 -> V_110 ) ) ;
for ( V_35 = 0 ; V_35 < V_83 ; V_35 ++ ) {
V_78 = & V_57 -> V_120 [ V_35 ] ;
V_78 -> V_121 = 0 ;
memset ( V_78 -> V_122 , 0 , F_30 ( V_78 ) ) ;
V_70 [ V_35 ] . V_114 = F_25 ( V_78 -> V_81 ) ;
V_70 [ V_35 ] . V_115 = F_27 ( V_78 -> V_116 ) ;
V_70 [ V_35 ] . V_123 = F_28 ( V_35 ) ;
}
V_81 = V_79 ;
memset ( & V_9 , 0 , sizeof( V_9 ) ) ;
V_9 . V_20 . V_21 = ( T_1 ) ( V_81 >> 32 ) ;
V_9 . V_20 . V_23 = ( T_1 ) ( V_81 & 0xffffffff ) ;
V_9 . V_20 . V_25 = V_84 ;
V_9 . V_20 . V_9 = V_124 ;
F_5 ( V_2 , & V_9 ) ;
V_34 = V_9 . V_3 . V_9 ;
if ( V_34 ) {
F_10 ( & V_2 -> V_12 -> V_26 ,
L_8 , V_34 ) ;
goto V_125;
}
V_72 = ( (struct V_71 * )
& V_66 -> V_111 [ F_29 ( V_66 -> V_109 ) ] ) ;
for ( V_35 = 0 ; V_35 < F_31 ( V_66 -> V_107 ) ; V_35 ++ ) {
V_76 = & V_57 -> V_112 [ V_35 ] ;
V_87 = F_29 ( V_72 [ V_35 ] . V_126 ) ;
V_76 -> V_127 = V_2 -> V_14 -> V_128 + V_87 ;
}
V_74 = ( (struct V_73 * )
& V_66 -> V_111 [ F_29 ( V_66 -> V_110 ) ] ) ;
for ( V_35 = 0 ; V_35 < F_31 ( V_66 -> V_108 ) ; V_35 ++ ) {
V_78 = & V_57 -> V_120 [ V_35 ] ;
V_87 = F_29 ( V_74 [ V_35 ] . V_129 ) ;
V_89 = F_29 ( V_74 [ V_35 ] . V_130 ) ;
V_78 -> V_131 = V_2 -> V_14 -> V_128 + V_87 ;
V_78 -> V_132 = V_2 -> V_14 -> V_128 + V_89 ;
}
V_57 -> V_60 = F_29 ( V_66 -> V_133 ) ;
V_57 -> V_59 = F_31 ( V_66 -> V_59 ) ;
V_57 -> V_134 = V_66 -> V_134 ;
V_125:
F_20 ( & V_2 -> V_12 -> V_26 , V_85 , V_66 ,
V_80 ) ;
V_92:
F_20 ( & V_2 -> V_12 -> V_26 , V_84 , V_64 , V_79 ) ;
return V_34 ;
}
static void
F_32 ( struct V_1 * V_2 )
{
struct V_8 V_9 ;
struct V_56 * V_57 = V_2 -> V_57 ;
memset ( & V_9 , 0 , sizeof( V_9 ) ) ;
V_9 . V_20 . V_21 = V_57 -> V_59 ;
V_9 . V_20 . V_23 = V_135 ;
V_9 . V_20 . V_25 = 0 ;
V_9 . V_20 . V_9 = V_136 ;
F_5 ( V_2 , & V_9 ) ;
if ( V_9 . V_3 . V_9 )
F_10 ( & V_2 -> V_12 -> V_26 ,
L_9 ) ;
V_57 -> V_60 = V_137 ;
}
static int
F_33 ( struct V_1 * V_2 )
{
struct V_138 * V_64 ;
struct V_139 * V_140 ;
struct V_141 * V_66 ;
void * V_142 , * V_143 ;
T_8 V_84 , V_85 ;
T_1 V_144 ;
struct V_8 V_9 ;
int V_34 ;
T_6 V_81 ;
T_5 V_145 , V_146 ;
struct V_147 * V_148 = V_2 -> V_148 ;
V_148 -> V_113 = 0 ;
V_148 -> V_149 = 0 ;
* ( V_148 -> V_150 ) = 0 ;
V_84 = F_34 ( struct V_138 ) ;
V_142 = F_15 ( & V_2 -> V_12 -> V_26 , V_84 ,
& V_145 , V_48 ) ;
if ( ! V_142 )
return - V_49 ;
V_85 = F_35 ( struct V_141 ) ;
V_143 = F_15 ( & V_2 -> V_12 -> V_26 , V_85 ,
& V_146 , V_48 ) ;
if ( ! V_143 ) {
V_34 = - V_49 ;
goto V_92;
}
memset ( V_142 , 0 , V_84 ) ;
V_64 = V_142 ;
memset ( V_143 , 0 , V_85 ) ;
V_66 = V_143 ;
V_64 -> V_93 = F_25 ( V_146 ) ;
V_144 = ( V_94 | V_95 |
V_151 ) ;
V_64 -> V_102 [ 0 ] = F_27 ( V_144 ) ;
V_64 -> V_103 =
F_27 ( V_104 ) ;
V_64 -> V_152 = 0 ;
V_64 -> V_123 = 0 ;
V_64 -> V_153 = F_25 ( V_148 -> V_154 ) ;
V_140 = & V_64 -> V_155 ;
V_140 -> V_114 = F_25 ( V_148 -> V_81 ) ;
V_140 -> V_115 = F_27 ( V_148 -> V_116 ) ;
V_81 = V_145 ;
memset ( & V_9 , 0 , sizeof( V_9 ) ) ;
V_9 . V_20 . V_21 = ( T_1 ) ( V_81 >> 32 ) ;
V_9 . V_20 . V_23 = ( ( T_1 ) V_81 & 0xffffffff ) ;
V_9 . V_20 . V_25 = V_84 ;
V_9 . V_20 . V_9 = V_156 ;
F_5 ( V_2 , & V_9 ) ;
V_34 = V_9 . V_3 . V_9 ;
if ( V_34 == V_29 ) {
V_144 = F_29 ( V_66 -> V_155 . V_126 ) ;
V_148 -> V_157 = V_2 -> V_14 -> V_128 + V_144 ;
V_2 -> V_158 =
F_31 ( V_66 -> V_59 ) ;
} else {
F_10 ( & V_2 -> V_12 -> V_26 ,
L_10 , V_34 ) ;
V_34 = - V_47 ;
}
F_20 ( & V_2 -> V_12 -> V_26 , V_85 , V_143 ,
V_146 ) ;
V_92:
F_20 ( & V_2 -> V_12 -> V_26 , V_84 , V_142 , V_145 ) ;
return V_34 ;
}
static void
F_36 ( struct V_1 * V_2 )
{
struct V_8 V_9 ;
memset ( & V_9 , 0 , sizeof( V_9 ) ) ;
V_9 . V_20 . V_21 = V_2 -> V_158 ;
V_9 . V_20 . V_23 = V_135 ;
V_9 . V_20 . V_25 = 0 ;
V_9 . V_20 . V_9 = V_159 ;
F_5 ( V_2 , & V_9 ) ;
if ( V_9 . V_3 . V_9 )
F_10 ( & V_2 -> V_12 -> V_26 ,
L_11 ) ;
}
int
F_37 ( struct V_1 * V_2 , T_1 V_160 )
{
struct V_8 V_9 ;
memset ( & V_9 , 0 , sizeof( V_9 ) ) ;
V_9 . V_20 . V_21 = V_160 ;
V_9 . V_20 . V_9 = V_161 ;
F_5 ( V_2 , & V_9 ) ;
return V_9 . V_3 . V_9 ;
}
int F_38 ( struct V_1 * V_2 )
{
void * V_62 ;
int V_34 ;
int V_162 ;
struct V_56 * V_57 ;
struct V_75 * V_76 ;
struct V_77 * V_78 ;
struct V_147 * V_148 ;
struct V_11 * V_12 = V_2 -> V_12 ;
V_57 = V_2 -> V_57 ;
V_148 = V_2 -> V_148 ;
V_148 -> V_150 = ( V_163 * ) F_15 ( & V_12 -> V_26 ,
sizeof( T_1 ) , & V_148 -> V_154 , V_48 ) ;
if ( V_148 -> V_150 == NULL ) {
F_10 ( & V_12 -> V_26 , L_12 ) ;
return - V_49 ;
}
V_62 = F_15 ( & V_12 -> V_26 , F_39 ( V_148 ) ,
& V_148 -> V_81 , V_48 ) ;
if ( V_62 == NULL ) {
F_10 ( & V_12 -> V_26 , L_13 ) ;
V_34 = - V_49 ;
goto V_164;
}
V_148 -> V_122 = V_62 ;
for ( V_162 = 0 ; V_162 < V_2 -> V_90 ; V_162 ++ ) {
V_76 = & V_57 -> V_112 [ V_162 ] ;
V_62 = F_15 ( & V_2 -> V_12 -> V_26 ,
F_40 ( V_76 ) ,
& V_76 -> V_81 , V_48 ) ;
if ( V_62 == NULL ) {
F_10 ( & V_12 -> V_26 ,
L_14 , V_162 ) ;
V_34 = - V_49 ;
goto V_164;
}
V_76 -> V_122 = V_62 ;
}
for ( V_162 = 0 ; V_162 < V_2 -> V_91 ; V_162 ++ ) {
V_78 = & V_57 -> V_120 [ V_162 ] ;
V_62 = F_15 ( & V_2 -> V_12 -> V_26 ,
F_30 ( V_78 ) ,
& V_78 -> V_81 , V_48 ) ;
if ( V_62 == NULL ) {
F_10 ( & V_12 -> V_26 ,
L_15 , V_162 ) ;
V_34 = - V_49 ;
goto V_164;
}
V_78 -> V_122 = V_62 ;
}
return 0 ;
V_164:
F_41 ( V_2 ) ;
return V_34 ;
}
int F_42 ( struct V_1 * V_2 )
{
int V_34 ;
if ( V_2 -> V_165 & V_166 ) {
F_43 ( V_2 -> V_12 ) ;
V_2 -> V_165 &= ~ V_166 ;
}
V_34 = F_22 ( V_2 ) ;
if ( V_34 )
return V_34 ;
V_34 = F_33 ( V_2 ) ;
if ( V_34 ) {
F_32 ( V_2 ) ;
return V_34 ;
}
F_44 ( V_167 , & V_2 -> V_60 ) ;
return 0 ;
}
void F_45 ( struct V_1 * V_2 )
{
if ( F_46 ( V_167 , & V_2 -> V_60 ) ) {
F_32 ( V_2 ) ;
F_36 ( V_2 ) ;
F_2 ( 20 ) ;
}
}
void F_41 ( struct V_1 * V_2 )
{
struct V_56 * V_57 ;
struct V_75 * V_76 ;
struct V_77 * V_78 ;
struct V_147 * V_148 ;
int V_162 ;
V_57 = V_2 -> V_57 ;
V_148 = V_2 -> V_148 ;
if ( V_148 -> V_150 != NULL ) {
F_20 ( & V_2 -> V_12 -> V_26 ,
sizeof( T_1 ) ,
V_148 -> V_150 ,
V_148 -> V_154 ) ;
V_148 -> V_150 = NULL ;
}
if ( V_148 -> V_122 != NULL ) {
F_20 ( & V_2 -> V_12 -> V_26 ,
F_39 ( V_148 ) ,
V_148 -> V_122 , V_148 -> V_81 ) ;
V_148 -> V_122 = NULL ;
}
for ( V_162 = 0 ; V_162 < V_2 -> V_90 ; V_162 ++ ) {
V_76 = & V_57 -> V_112 [ V_162 ] ;
if ( V_76 -> V_122 != NULL ) {
F_20 ( & V_2 -> V_12 -> V_26 ,
F_40 ( V_76 ) ,
V_76 -> V_122 ,
V_76 -> V_81 ) ;
V_76 -> V_122 = NULL ;
}
}
for ( V_162 = 0 ; V_162 < V_2 -> V_91 ; V_162 ++ ) {
V_78 = & V_57 -> V_120 [ V_162 ] ;
if ( V_78 -> V_122 != NULL ) {
F_20 ( & V_2 -> V_12 -> V_26 ,
F_30 ( V_78 ) ,
V_78 -> V_122 ,
V_78 -> V_81 ) ;
V_78 -> V_122 = NULL ;
}
}
}
int F_47 ( struct V_1 * V_2 , T_7 * V_168 )
{
int V_34 ;
struct V_8 V_9 ;
memset ( & V_9 , 0 , sizeof( V_9 ) ) ;
V_9 . V_20 . V_21 = V_2 -> V_14 -> V_15 | V_169 ;
V_9 . V_20 . V_9 = V_170 ;
V_9 . V_3 . V_21 = V_9 . V_3 . V_23 = 1 ;
F_5 ( V_2 , & V_9 ) ;
V_34 = V_9 . V_3 . V_9 ;
if ( V_34 == V_29 )
F_48 ( V_2 , V_9 . V_3 . V_21 , V_9 . V_3 . V_23 , 0 , V_168 ) ;
else {
F_10 ( & V_2 -> V_12 -> V_26 ,
L_16 , V_34 ) ;
V_34 = - V_47 ;
}
return V_34 ;
}
int F_49 ( struct V_1 * V_2 ,
struct V_171 * V_172 , T_7 V_173 )
{
int V_34 ;
T_5 V_174 ;
struct V_171 * V_175 ;
void * V_176 ;
struct V_8 V_9 ;
T_8 V_177 = sizeof( struct V_171 ) ;
V_176 = F_15 ( & V_2 -> V_12 -> V_26 , V_177 ,
& V_174 , V_48 ) ;
if ( ! V_176 )
return - V_49 ;
memset ( V_176 , 0 , V_177 ) ;
V_175 = V_176 ;
memset ( & V_9 , 0 , sizeof( V_9 ) ) ;
V_9 . V_20 . V_9 = V_178 ;
V_9 . V_20 . V_21 = F_17 ( V_174 ) ;
V_9 . V_20 . V_23 = F_16 ( V_174 ) ;
V_9 . V_20 . V_25 = ( V_173 << 16 | V_177 ) ;
F_5 ( V_2 , & V_9 ) ;
V_34 = V_9 . V_3 . V_9 ;
if ( V_34 == V_29 ) {
V_172 -> V_15 = F_31 ( V_175 -> V_15 ) ;
V_172 -> V_179 = F_31 ( V_175 -> V_179 ) ;
V_172 -> V_180 = F_31 ( V_175 -> V_180 ) ;
V_172 -> V_181 = F_31 ( V_175 -> V_181 ) ;
V_172 -> V_182 = F_31 ( V_175 -> V_182 ) ;
V_172 -> V_183 = F_31 ( V_175 -> V_183 ) ;
V_172 -> V_184 = F_31 ( V_175 -> V_184 ) ;
V_172 -> V_185 = F_31 ( V_175 -> V_185 ) ;
V_172 -> V_102 = F_29 ( V_175 -> V_102 ) ;
V_172 -> V_186 = F_31 ( V_175 -> V_186 ) ;
F_14 ( & V_2 -> V_12 -> V_26 ,
L_17
L_18
L_19 ,
V_172 -> V_180 , V_172 -> V_181 ,
V_172 -> V_182 , V_172 -> V_183 ,
V_172 -> V_184 , V_172 -> V_185 ,
V_172 -> V_186 , V_172 -> V_102 ) ;
} else {
F_10 ( & V_2 -> V_12 -> V_26 ,
L_20 , V_34 ) ;
V_34 = - V_47 ;
}
F_20 ( & V_2 -> V_12 -> V_26 , V_177 , V_176 ,
V_174 ) ;
return V_34 ;
}
int F_50 ( struct V_1 * V_2 , struct V_171 * V_187 )
{
int V_34 = - V_47 ;
T_5 V_174 ;
void * V_176 ;
struct V_8 V_9 ;
struct V_171 * V_175 ;
T_8 V_177 = sizeof( struct V_171 ) ;
if ( V_2 -> V_179 != V_188 )
return V_34 ;
V_176 = F_15 ( & V_2 -> V_12 -> V_26 , V_177 ,
& V_174 , V_48 ) ;
if ( ! V_176 )
return - V_49 ;
memset ( V_176 , 0 , V_177 ) ;
V_175 = V_176 ;
V_175 -> V_15 = F_28 ( V_187 -> V_15 ) ;
V_175 -> V_179 = F_28 ( V_187 -> V_179 ) ;
V_175 -> V_180 = F_28 ( V_187 -> V_180 ) ;
V_175 -> V_181 = F_28 ( V_187 -> V_181 ) ;
V_175 -> V_102 = F_27 ( V_187 -> V_102 ) ;
V_175 -> V_189 = V_187 -> V_189 ;
V_175 -> V_182 = F_28 ( V_187 -> V_182 ) ;
V_175 -> V_183 = F_28 ( V_187 -> V_183 ) ;
V_175 -> V_184 = F_28 ( V_187 -> V_184 ) ;
V_175 -> V_185 = F_28 ( V_187 -> V_185 ) ;
memset ( & V_9 , 0 , sizeof( V_9 ) ) ;
V_9 . V_20 . V_9 = V_190 ;
V_9 . V_20 . V_21 = F_17 ( V_174 ) ;
V_9 . V_20 . V_23 = F_16 ( V_174 ) ;
V_9 . V_20 . V_25 = ( ( V_187 -> V_15 << 16 ) | V_177 ) ;
F_5 ( V_2 , & V_9 ) ;
V_34 = V_9 . V_3 . V_9 ;
if ( V_34 != V_29 ) {
F_10 ( & V_2 -> V_12 -> V_26 ,
L_21 , V_34 ) ;
V_34 = - V_47 ;
}
F_20 ( & V_2 -> V_12 -> V_26 , V_177 , V_176 ,
V_174 ) ;
return V_34 ;
}
int F_51 ( struct V_1 * V_2 ,
struct V_191 * V_192 )
{
int V_34 = 0 , V_35 ;
struct V_8 V_9 ;
T_5 V_193 ;
struct V_191 * V_194 ;
void * V_195 ;
T_8 V_196 = sizeof( struct V_191 ) ;
T_8 V_197 = V_196 * V_198 ;
V_195 = F_15 ( & V_2 -> V_12 -> V_26 , V_197 ,
& V_193 , V_48 ) ;
if ( ! V_195 )
return - V_49 ;
memset ( V_195 , 0 , V_197 ) ;
V_194 = V_195 ;
memset ( & V_9 , 0 , sizeof( V_9 ) ) ;
V_9 . V_20 . V_9 = V_199 ;
V_9 . V_20 . V_21 = F_17 ( V_193 ) ;
V_9 . V_20 . V_23 = F_16 ( V_193 ) ;
V_9 . V_20 . V_25 = V_197 ;
F_5 ( V_2 , & V_9 ) ;
V_34 = V_9 . V_3 . V_9 ;
if ( V_34 == V_29 ) {
for ( V_35 = 0 ; V_35 < V_198 ; V_35 ++ , V_194 ++ , V_192 ++ ) {
V_192 -> V_200 = F_31 ( V_194 -> V_200 ) ;
V_192 -> V_201 = F_31 ( V_194 -> V_201 ) ;
V_192 -> type = F_31 ( V_194 -> type ) ;
V_192 -> V_202 =
F_31 ( V_194 -> V_202 ) ;
V_192 -> V_203 =
F_31 ( V_194 -> V_203 ) ;
V_192 -> V_204 =
F_31 ( V_194 -> V_204 ) ;
memcpy ( V_192 -> V_168 , V_194 -> V_168 , V_205 ) ;
}
} else {
F_10 ( & V_2 -> V_12 -> V_26 ,
L_22 , V_34 ) ;
V_34 = - V_47 ;
}
F_20 ( & V_2 -> V_12 -> V_26 , V_197 , V_195 ,
V_193 ) ;
return V_34 ;
}
int F_52 ( struct V_1 * V_2 , T_7 V_200 ,
T_7 V_206 , T_7 V_15 )
{
int V_34 = - V_47 ;
T_1 V_21 ;
struct V_8 V_9 ;
if ( V_2 -> V_179 != V_188 ||
! ( V_2 -> V_207 [ V_200 ] . V_165 & V_208 ) )
return V_34 ;
V_21 = V_200 | ( V_206 ? V_209 : 0 ) ;
V_21 |= V_15 << 8 ;
memset ( & V_9 , 0 , sizeof( V_9 ) ) ;
V_9 . V_20 . V_9 = V_210 ;
V_9 . V_20 . V_21 = V_21 ;
F_5 ( V_2 , & V_9 ) ;
V_34 = V_9 . V_3 . V_9 ;
if ( V_34 != V_29 ) {
F_10 ( & V_2 -> V_12 -> V_26 ,
L_23 ,
V_15 , V_200 ) ;
} else {
F_14 ( & V_2 -> V_12 -> V_26 ,
L_24 ,
V_200 , V_15 ) ;
}
return V_34 ;
}
int F_53 ( struct V_1 * V_2 , const T_7 V_211 ,
const T_7 V_212 , struct V_213 * V_214 ) {
T_8 V_215 = sizeof( struct V_213 ) ;
struct V_213 * V_216 ;
T_5 V_217 ;
void * V_218 ;
T_1 V_21 ;
struct V_8 V_9 ;
int V_34 ;
if ( V_214 == NULL )
return - V_49 ;
if ( V_2 -> V_179 != V_188 &&
V_211 != V_2 -> V_14 -> V_15 ) {
F_10 ( & V_2 -> V_12 -> V_26 ,
L_25 , V_211 ) ;
return - V_47 ;
}
V_218 = F_15 ( & V_2 -> V_12 -> V_26 , V_215 ,
& V_217 , V_48 ) ;
if ( ! V_218 ) {
F_10 ( & V_2 -> V_12 -> V_26 , L_26 ) ;
return - V_49 ;
}
memset ( V_218 , 0 , V_215 ) ;
V_21 = V_211 | V_219 << 8 | V_220 << 12 ;
V_21 |= V_212 << 15 | V_215 << 16 ;
memset ( & V_9 , 0 , sizeof( V_9 ) ) ;
V_9 . V_20 . V_9 = V_221 ;
V_9 . V_20 . V_21 = V_21 ;
V_9 . V_20 . V_23 = F_17 ( V_217 ) ;
V_9 . V_20 . V_25 = F_16 ( V_217 ) ;
F_5 ( V_2 , & V_9 ) ;
V_34 = V_9 . V_3 . V_9 ;
if ( ! V_34 ) {
V_216 = V_218 ;
V_214 -> V_59 = F_31 ( V_216 -> V_59 ) ;
V_214 -> V_37 = F_31 ( V_216 -> V_37 ) ;
V_214 -> V_222 = F_31 ( V_216 -> V_222 ) ;
V_214 -> V_223 =
F_54 ( V_216 -> V_223 ) ;
V_214 -> V_224 =
F_54 ( V_216 -> V_224 ) ;
V_214 -> V_225 = F_54 ( V_216 -> V_225 ) ;
V_214 -> V_226 = F_54 ( V_216 -> V_226 ) ;
V_214 -> V_227 = F_54 ( V_216 -> V_227 ) ;
V_214 -> V_228 = F_54 ( V_216 -> V_228 ) ;
V_214 -> V_229 = F_54 ( V_216 -> V_229 ) ;
}
F_20 ( & V_2 -> V_12 -> V_26 , V_215 , V_218 ,
V_217 ) ;
return V_34 ;
}
int F_55 ( struct V_1 * V_2 ,
struct V_230 * V_231 )
{
struct V_230 * V_216 ;
struct V_8 V_9 ;
T_8 V_215 = sizeof( struct V_230 ) ;
T_5 V_217 ;
void * V_218 ;
int V_34 ;
V_218 = F_15 ( & V_2 -> V_12 -> V_26 , V_215 ,
& V_217 , V_48 ) ;
if ( ! V_218 ) {
F_10 ( & V_2 -> V_12 -> V_26 ,
L_27 , V_232 ) ;
return - V_49 ;
}
memset ( V_218 , 0 , V_215 ) ;
memset ( & V_9 , 0 , sizeof( V_9 ) ) ;
V_9 . V_20 . V_9 = V_233 ;
V_9 . V_20 . V_21 = V_215 << 16 ;
V_9 . V_20 . V_23 = F_17 ( V_217 ) ;
V_9 . V_20 . V_25 = F_16 ( V_217 ) ;
F_5 ( V_2 , & V_9 ) ;
V_34 = V_9 . V_3 . V_9 ;
if ( ! V_34 ) {
V_216 = V_218 ;
V_231 -> V_234 = F_54 ( V_216 -> V_234 ) ;
V_231 -> V_235 = F_54 ( V_216 -> V_235 ) ;
V_231 -> V_236 =
F_54 ( V_216 -> V_236 ) ;
V_231 -> V_237 =
F_54 ( V_216 -> V_237 ) ;
V_231 -> V_238 = F_54 ( V_216 -> V_238 ) ;
V_231 -> V_239 = F_54 ( V_216 -> V_239 ) ;
V_231 -> V_240 =
F_54 ( V_216 -> V_240 ) ;
V_231 -> V_241 =
F_54 ( V_216 -> V_241 ) ;
V_231 -> V_242 =
F_54 ( V_216 -> V_242 ) ;
V_231 -> V_243 =
F_54 ( V_216 -> V_243 ) ;
V_231 -> V_244 = F_54 ( V_216 -> V_244 ) ;
V_231 -> V_245 = F_54 ( V_216 -> V_245 ) ;
V_231 -> V_246 = F_54 ( V_216 -> V_246 ) ;
} else {
F_14 ( & V_2 -> V_12 -> V_26 ,
L_28 , V_232 , V_34 ) ;
}
F_20 ( & V_2 -> V_12 -> V_26 , V_215 , V_218 ,
V_217 ) ;
return V_34 ;
}
int F_56 ( struct V_1 * V_2 , const T_7 V_207 ,
const T_7 V_212 , struct V_213 * V_214 ) {
struct V_213 V_247 ;
T_7 V_35 ;
int V_248 = - V_47 ;
if ( V_214 == NULL )
return - V_49 ;
if ( V_2 -> V_179 != V_188 )
return - V_47 ;
if ( V_2 -> V_249 == NULL )
return - V_47 ;
memset ( V_214 , 0 , sizeof( T_6 ) ) ;
V_214 -> V_225 = V_250 ;
V_214 -> V_223 = V_250 ;
V_214 -> V_224 = V_250 ;
V_214 -> V_226 = V_250 ;
V_214 -> V_228 = V_250 ;
V_214 -> V_227 = V_250 ;
V_214 -> V_229 = V_250 ;
V_214 -> V_59 = V_207 ;
for ( V_35 = 0 ; V_35 < V_198 ; V_35 ++ ) {
if ( V_2 -> V_249 [ V_35 ] . V_251 != V_207 )
continue;
memset ( & V_247 , 0 , sizeof( struct V_213 ) ) ;
if ( F_53 ( V_2 , V_35 , V_212 , & V_247 ) )
continue;
V_214 -> V_222 = V_247 . V_222 ;
V_214 -> V_37 = V_247 . V_37 ;
F_57 ( V_214 -> V_225 ,
V_247 . V_225 ) ;
F_57 ( V_214 -> V_223 ,
V_247 . V_223 ) ;
F_57 ( V_214 -> V_224 ,
V_247 . V_224 ) ;
F_57 ( V_214 -> V_226 ,
V_247 . V_226 ) ;
F_57 ( V_214 -> V_228 ,
V_247 . V_228 ) ;
F_57 ( V_214 -> V_227 ,
V_247 . V_227 ) ;
F_57 ( V_214 -> V_229 ,
V_247 . V_229 ) ;
V_248 = 0 ;
}
return V_248 ;
}
int F_58 ( struct V_1 * V_2 , const T_7 V_252 ,
const T_7 V_253 , const T_7 V_212 )
{
T_1 V_21 ;
struct V_8 V_9 ;
if ( V_2 -> V_179 != V_188 )
return - V_47 ;
if ( V_252 == V_220 ) {
if ( V_253 >= V_198 )
goto V_254;
} else if ( V_252 == V_255 ) {
if ( V_253 >= V_256 )
goto V_254;
} else {
goto V_254;
}
if ( V_212 > V_257 )
goto V_254;
V_21 = V_253 | V_219 << 8 | V_252 << 12 ;
V_21 |= V_258 | V_212 << 15 ;
memset ( & V_9 , 0 , sizeof( V_9 ) ) ;
V_9 . V_20 . V_9 = V_221 ;
V_9 . V_20 . V_21 = V_21 ;
F_5 ( V_2 , & V_9 ) ;
return V_9 . V_3 . V_9 ;
V_254:
F_10 ( & V_2 -> V_12 -> V_26 , L_29
L_30 , V_252 , V_253 , V_212 ) ;
return - V_47 ;
}
static int
F_59 ( struct V_1 * V_2 ,
T_1 * V_21 , T_1 * V_23 )
{
int V_34 = - V_47 ;
struct V_8 V_9 ;
T_7 V_15 ;
V_15 = ( * V_21 >> 8 ) ;
V_9 . V_20 . V_9 = V_259 ;
V_9 . V_20 . V_21 = * V_21 ;
V_9 . V_3 . V_21 = V_9 . V_3 . V_23 = 1 ;
F_5 ( V_2 , & V_9 ) ;
* V_21 = V_9 . V_3 . V_21 ;
* V_23 = V_9 . V_3 . V_23 ;
V_34 = V_9 . V_3 . V_9 ;
if ( V_34 == V_29 ) {
F_14 ( & V_2 -> V_12 -> V_26 ,
L_31 , V_15 ) ;
} else {
F_10 ( & V_2 -> V_12 -> V_26 ,
L_32 ,
V_15 ) ;
}
return V_34 ;
}
int F_60 ( struct V_1 * V_2 ,
struct V_260 * V_261 )
{
int V_34 = - V_47 ;
T_1 V_21 , V_23 = 0 ;
struct V_8 V_9 ;
T_7 V_15 ;
if ( V_2 -> V_179 != V_188 )
return V_34 ;
V_15 = V_261 -> V_15 ;
V_21 = ( V_2 -> V_249 [ V_15 ] . V_251 & V_262 ) ;
V_21 |= ( V_15 << 8 ) ;
if ( F_59 ( V_2 , & V_21 , & V_23 ) )
return V_34 ;
V_21 &= ~ ( 0x0ff << 8 ) ;
V_21 |= ( V_15 << 8 ) ;
V_21 &= ~ ( V_263 | V_264 ) ;
switch ( V_261 -> V_179 ) {
case V_265 :
V_21 |= ( V_209 | V_266 | V_267 ) ;
V_23 |= ( V_262 | V_268 ) ;
if ( V_2 -> V_102 & V_269 )
V_23 |= ( V_263 | V_264 ) ;
if ( ! ( V_261 -> V_270 ) )
V_21 &= ~ V_209 ;
if ( ! ( V_261 -> V_271 ) )
V_21 &= ~ V_266 ;
if ( ! ( V_261 -> V_272 ) )
V_21 &= ~ V_267 ;
if ( ! ( V_261 -> V_273 ) )
V_23 &= ~ V_262 ;
if ( ! ( V_261 -> V_274 & V_262 ) )
V_23 &= ~ ( V_268 | V_263 | V_264 ) ;
if ( ! ( V_261 -> V_274 & V_268 ) )
V_23 &= ~ V_263 ;
if ( ! ( V_261 -> V_274 & V_263 ) )
V_23 &= ~ V_264 ;
break;
case V_275 :
V_21 |= ( V_263 | V_276 ) ;
V_21 |= ( V_261 -> V_277 << 16 ) ;
break;
case V_278 :
V_21 |= ( V_264 | V_276 ) ;
V_21 &= ~ ( 0x0ffff << 16 ) ;
break;
default:
return V_34 ;
}
memset ( & V_9 , 0 , sizeof( V_9 ) ) ;
V_9 . V_20 . V_9 = V_279 ;
V_9 . V_20 . V_21 = V_21 ;
V_9 . V_20 . V_23 = V_23 ;
F_5 ( V_2 , & V_9 ) ;
V_34 = V_9 . V_3 . V_9 ;
if ( V_34 != V_29 ) {
F_10 ( & V_2 -> V_12 -> V_26 ,
L_33 , V_15 ) ;
} else {
F_14 ( & V_2 -> V_12 -> V_26 ,
L_34 , V_15 ) ;
}
return V_34 ;
}
int
F_61 ( struct V_1 * V_2 ,
struct V_260 * V_261 )
{
T_1 V_21 , V_23 ;
T_7 V_251 ;
if ( V_2 -> V_179 == V_188 )
V_251 = V_2 -> V_249 [ V_261 -> V_15 ] . V_251 ;
else
V_251 = V_2 -> V_280 ;
V_21 = V_251 ;
V_21 |= ( V_261 -> V_15 << 8 ) ;
if ( F_59 ( V_2 , & V_21 , & V_23 ) )
return - V_47 ;
V_261 -> V_270 = ! ! ( V_21 & V_209 ) ;
V_261 -> V_281 = ! ! ( V_21 & V_276 ) ;
V_261 -> V_271 = ! ! ( V_21 & V_266 ) ;
V_261 -> V_272 = ! ! ( V_21 & V_267 ) ;
V_261 -> V_277 = F_62 ( V_21 >> 16 ) ;
V_261 -> V_273 = ( V_23 & 0x1 ) ;
V_261 -> V_274 = ( ( V_23 >> 1 ) & 0x7 ) ;
return 0 ;
}
