static int F_1 ( struct V_1 * V_2 , T_1 V_3 , T_1 * V_4 )
{
int V_5 ;
V_5 = F_2 ( V_2 , V_3 ) ;
if ( V_5 < 0 )
return V_5 ;
* V_4 = V_5 ;
return 0 ;
}
static int F_3 ( struct V_1 * V_2 , T_1 V_3 , T_1 V_4 )
{
return F_4 ( V_2 , V_3 , V_4 ) ;
}
static int F_5 ( struct V_1 * V_2 , T_1 V_6 , T_2 V_7 )
{
T_1 V_8 ;
T_1 V_9 ;
int V_10 ;
if ( V_6 == V_11 )
V_8 = V_12 ;
else if ( V_6 == V_13 )
V_8 = V_14 ;
else
return - V_15 ;
if ( V_7 > V_16 )
return - V_15 ;
V_9 = ( V_7 * 255 ) / V_16 ;
V_10 = F_3 ( V_2 , V_8 , V_9 ) ;
return V_10 ;
}
static int F_6 ( struct V_1 * V_2 , T_1 V_6 ,
T_1 V_17 )
{
T_1 V_18 ;
T_1 V_19 ;
int V_10 ;
if ( V_6 == V_11 )
V_18 = V_20 ;
else if ( V_6 == V_13 )
V_18 = V_21 ;
else
return - V_15 ;
if ( V_17 > V_22 )
return - V_15 ;
V_19 = ( V_17 * 255 ) / V_22 ;
V_10 = F_3 ( V_2 , V_18 , V_19 ) ;
return V_10 ;
}
static int F_7 ( struct V_23 * V_24 , T_1 V_25 )
{
struct V_26 * V_27 = F_8 ( V_24 -> V_2 ) ;
T_1 V_28 = V_24 -> V_28 ;
T_1 V_3 ;
T_1 V_29 = 0 ;
int V_10 ;
F_9 ( & V_24 -> V_2 -> V_30 , L_1 ,
V_31 , V_24 -> V_32 . V_33 , V_25 ) ;
switch ( V_28 ) {
case V_34 :
case V_35 :
case V_36 :
case V_37 :
V_3 = V_38 ;
break;
case V_39 :
case V_40 :
case V_41 :
case V_42 :
V_28 -= V_39 ;
V_3 = V_43 ;
break;
default:
return - V_15 ;
}
if ( V_25 > V_44 )
return - V_15 ;
if ( V_24 -> type == V_45 && V_25 < 2 )
V_25 = 1 - V_25 ;
F_10 ( & V_27 -> V_46 ) ;
F_1 ( V_24 -> V_2 , V_3 , & V_29 ) ;
V_29 &= ~ ( V_47 << ( V_28 << 1 ) ) ;
V_29 |= ( V_25 << ( V_28 << 1 ) ) ;
F_9 ( & V_24 -> V_2 -> V_30 , L_2 ,
V_31 , V_24 -> V_32 . V_33 , V_3 , V_28 , V_25 , V_29 ) ;
V_10 = F_3 ( V_24 -> V_2 , V_3 , V_29 ) ;
F_11 ( & V_27 -> V_46 ) ;
return V_10 ;
}
static int F_12 ( struct V_48 * V_49 ,
unsigned long * V_50 ,
unsigned long * V_51 )
{
struct V_23 * V_24 = F_13 ( V_49 ) ;
T_2 V_7 ;
T_1 V_17 ;
int V_10 ;
if ( * V_50 + * V_51 > V_16 )
return - V_15 ;
if ( * V_50 == 0 && * V_51 == 0 ) {
* V_50 = 500 ;
* V_51 = 500 ;
}
V_7 = ( * V_50 ) + ( * V_51 ) ;
V_17 = 100 * ( * V_50 ) / V_7 ;
if ( V_24 -> type == V_45 )
V_17 = 100 - V_17 ;
V_10 = F_5 ( V_24 -> V_2 , V_11 , V_7 ) ;
if ( V_10 )
return V_10 ;
V_10 = F_6 ( V_24 -> V_2 , V_11 , V_17 ) ;
if ( V_10 )
return V_10 ;
F_9 ( & V_24 -> V_2 -> V_30 , L_3 ,
V_31 ) ;
F_7 ( V_24 , V_52 ) ;
return 0 ;
}
static int F_14 ( struct V_48 * V_49 ,
enum V_53 V_54 )
{
struct V_23 * V_24 = F_13 ( V_49 ) ;
F_9 ( & V_24 -> V_2 -> V_30 , L_4 ,
V_31 , V_49 -> V_33 , V_54 ) ;
return F_7 ( V_24 , ! ! V_54 ) ;
}
static int F_15 ( struct V_1 * V_2 ,
struct V_26 * V_27 ,
struct V_55 * V_56 )
{
int V_57 , V_10 = 0 ;
for ( V_57 = 0 ; V_57 < V_56 -> V_58 ; V_57 ++ ) {
struct V_59 * V_60 = & V_56 -> V_61 [ V_57 ] ;
struct V_23 * V_24 = & V_27 -> V_61 [ V_57 ] ;
V_24 -> V_2 = V_2 ;
V_24 -> V_28 = V_57 ;
switch ( V_60 -> type ) {
case V_62 :
case V_45 :
V_24 -> type = V_60 -> type ;
V_24 -> V_32 . V_33 = V_60 -> V_33 ;
V_24 -> V_32 . V_63 = 1 ;
V_24 -> V_32 . V_64 =
F_14 ;
V_24 -> V_32 . V_65 = F_12 ;
V_24 -> V_32 . V_66 = V_67 ;
V_10 = F_16 ( & V_2 -> V_30 , & V_24 -> V_32 ) ;
if ( V_10 < 0 ) {
F_17 ( & V_2 -> V_30 ,
L_5 ,
V_24 -> V_32 . V_33 ) ;
goto exit;
}
V_24 -> V_32 . V_54 =
(enum V_53 ) V_60 -> V_25 ;
V_10 = F_7 ( V_24 , V_60 -> V_25 ) ;
if ( V_10 < 0 ) {
F_17 ( & V_2 -> V_30 ,
L_6 ,
V_24 -> V_32 . V_33 , V_60 -> V_25 ) ;
goto exit;
}
break;
case V_68 :
default:
break;
}
}
return 0 ;
exit:
if ( V_57 > 0 )
for ( V_57 = V_57 - 1 ; V_57 >= 0 ; V_57 -- )
switch ( V_56 -> V_61 [ V_57 ] . type ) {
case V_62 :
case V_45 :
F_18 ( & V_27 -> V_61 [ V_57 ] . V_32 ) ;
break;
case V_68 :
default:
break;
}
return V_10 ;
}
static int F_19 ( struct V_1 * V_2 ,
const struct V_69 * V_28 )
{
struct V_55 * V_70 =
F_20 ( & V_2 -> V_30 ) ;
struct V_26 * V_27 ;
int V_10 ;
if ( V_70 == NULL ) {
F_17 ( & V_2 -> V_30 , L_7 ) ;
return - V_15 ;
}
if ( ! F_21 ( V_2 -> V_71 ,
V_72 ) ) {
F_17 ( & V_2 -> V_30 , L_8 ) ;
return - V_73 ;
}
V_27 = F_22 ( & V_2 -> V_30 , sizeof( struct V_26 ) ,
V_74 ) ;
if ( ! V_27 )
return - V_75 ;
V_27 -> V_2 = V_2 ;
F_23 ( V_2 , V_27 ) ;
F_24 ( & V_27 -> V_46 ) ;
V_10 = F_15 ( V_2 , V_27 , V_70 ) ;
if ( V_10 < 0 )
return V_10 ;
F_25 ( & V_2 -> V_30 , L_9 ) ;
return 0 ;
}
static int F_26 ( struct V_1 * V_2 )
{
struct V_55 * V_56 = F_20 ( & V_2 -> V_30 ) ;
struct V_26 * V_27 = F_8 ( V_2 ) ;
int V_57 ;
for ( V_57 = 0 ; V_57 < V_56 -> V_58 ; V_57 ++ )
switch ( V_27 -> V_61 [ V_57 ] . type ) {
case V_62 :
case V_45 :
F_18 ( & V_27 -> V_61 [ V_57 ] . V_32 ) ;
break;
case V_68 :
default:
break;
}
return 0 ;
}
