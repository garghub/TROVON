static bool F_1 ( T_1 V_1 )
{
if ( V_1 == V_2 || V_1 == V_3 )
return true ;
return false ;
}
static int F_2 ( const void * V_4 , const T_2 V_5 ,
enum V_6 V_7 ,
struct V_8 * V_9 )
{
T_1 * V_10 , * V_11 ;
T_2 V_12 ;
switch ( V_7 ) {
case V_13 :
V_12 = V_14 + 1 ;
break;
case V_15 :
V_12 = V_5 + 1 ;
break;
default:
V_12 = V_5 ;
}
V_10 = F_3 ( V_12 , V_16 ) ;
if ( ! V_10 )
return - V_17 ;
memcpy ( V_10 , V_4 , V_5 ) ;
if ( V_7 == V_13 || V_7 == V_15 ) {
for ( V_11 = V_10 ; V_11 - V_10 < V_5 ; V_11 ++ )
if ( * V_11 == ' ' )
* V_11 = '_' ;
}
V_9 -> V_4 = V_10 ;
V_9 -> V_18 = V_12 ;
return 0 ;
}
static void F_4 ( struct V_19 * V_20 ,
enum V_21 V_22 ,
enum V_6 V_7 ,
struct V_8 * V_9 )
{
T_1 * V_11 = V_9 -> V_4 , V_12 = V_9 -> V_18 ;
switch ( V_7 ) {
case V_23 :
V_11 = F_5 ( V_9 -> V_4 , V_12 , ':' ) ;
if ( V_11 != V_9 -> V_4 )
F_6 ( V_20 , L_1 , V_9 -> V_4 ) ;
V_11 += 2 ;
V_12 -= V_11 - V_9 -> V_4 ;
case V_24 :
case V_25 :
if ( ! V_12 )
break;
F_7 ( V_20 , V_11 , V_12 ) ;
break;
case V_15 :
F_6 ( V_20 , L_1 , V_11 ) ;
break;
default:
break;
}
}
static void F_8 ( struct V_19 * V_20 ,
enum V_21 V_22 ,
enum V_6 V_7 ,
struct V_8 * V_9 )
{
if ( V_22 != V_26 )
F_9 ( V_20 , & V_9 -> V_18 , sizeof( T_2 ) ) ;
if ( ! V_9 -> V_18 )
return;
F_9 ( V_20 , V_9 -> V_4 , V_9 -> V_18 ) ;
}
static void F_10 ( struct V_19 * V_20 ,
enum V_21 V_22 ,
enum V_6 V_7 ,
struct V_8 * V_9 )
{
switch ( V_22 ) {
case V_27 :
F_4 ( V_20 , V_22 , V_7 , V_9 ) ;
break;
case V_28 :
case V_26 :
F_8 ( V_20 , V_22 , V_7 , V_9 ) ;
break;
default:
break;
}
}
void F_11 ( struct V_19 * V_20 , enum V_21 V_22 ,
struct V_8 * V_9 )
{
F_10 ( V_20 , V_22 , V_24 , V_9 ) ;
}
void F_12 ( struct V_19 * V_20 , enum V_21 V_22 ,
struct V_8 * V_9 )
{
F_10 ( V_20 , V_22 , V_23 ,
V_9 ) ;
}
void F_13 ( struct V_19 * V_20 , enum V_21 V_22 ,
struct V_8 * V_9 )
{
F_10 ( V_20 , V_22 , V_15 , V_9 ) ;
}
void F_14 ( struct V_19 * V_20 , enum V_21 V_22 ,
struct V_8 * V_9 )
{
F_10 ( V_20 , V_22 , V_25 , V_9 ) ;
}
static int F_15 ( T_1 * V_29 , T_2 V_30 , T_1 V_31 ,
struct V_8 * V_9 ,
bool V_32 )
{
T_1 V_33 [ V_34 + 2 + V_35 ] = { 0 } ;
enum V_6 V_36 = V_24 ;
T_2 V_37 = 0 ;
if ( ! V_32 ) {
V_36 = V_23 ;
if ( V_31 < V_38 )
V_37 += snprintf ( V_33 , V_34 + 1 ,
L_1 , V_39 [ V_31 ] ) ;
V_33 [ V_37 ] = ':' ;
V_37 += 2 ;
}
if ( V_29 )
memcpy ( V_33 + V_37 , V_29 , V_30 ) ;
else
V_37 += V_40 ;
return F_2 ( V_33 , V_37 + V_30 ,
V_36 , V_9 ) ;
}
int F_16 ( struct V_41 * V_42 , struct V_43 * V_43 ,
const unsigned char * V_44 ,
struct V_45 * V_46 , int V_47 ,
struct V_8 * V_9 )
{
struct {
struct V_48 V_49 ;
char V_29 [ V_35 ] ;
} V_50 ;
T_1 * V_51 = NULL ;
T_2 V_52 = 0 ;
struct V_53 * V_53 ;
int V_54 ;
memset ( & V_50 , 0 , sizeof( V_50 ) ) ;
if ( ! V_42 )
goto V_55;
if ( F_1 ( V_42 -> V_56 -> V_1 ) ) {
V_51 = V_42 -> V_56 -> V_29 ;
V_52 = V_42 -> V_56 -> V_57 ;
goto V_55;
}
if ( ! V_43 )
return - V_58 ;
V_53 = F_17 ( V_43 ) ;
V_50 . V_49 . V_1 = F_1 ( V_59 ) ?
V_59 : V_2 ;
V_54 = F_18 ( V_43 , & V_50 . V_49 ) ;
if ( V_54 ) {
F_19 ( V_60 , V_53 ,
V_44 , L_2 ,
L_3 , V_54 , 0 ) ;
return V_54 ;
}
V_51 = V_50 . V_49 . V_29 ;
V_52 = V_50 . V_49 . V_57 ;
V_55:
return F_15 ( V_51 , V_52 , - 1 ,
V_9 , true ) ;
}
int F_20 ( struct V_41 * V_42 ,
struct V_43 * V_43 , const unsigned char * V_44 ,
struct V_45 * V_46 ,
int V_47 , struct V_8 * V_9 )
{
T_1 * V_51 = NULL , V_31 = V_38 ;
T_2 V_52 = 0 ;
if ( ! V_42 )
goto V_55;
V_51 = V_42 -> V_56 -> V_29 ;
V_52 = V_42 -> V_56 -> V_57 ;
V_31 = V_42 -> V_56 -> V_1 ;
V_55:
return F_15 ( V_51 , V_52 ,
V_31 , V_9 , false ) ;
}
static int F_21 ( struct V_41 * V_42 ,
struct V_43 * V_43 ,
const unsigned char * V_44 ,
struct V_8 * V_9 ,
bool V_32 )
{
const char * V_61 = NULL ;
T_2 V_62 = 0 ;
enum V_6 V_36 = V_32 ?
V_13 : V_15 ;
F_22 ( V_44 == NULL && V_43 == NULL ) ;
if ( V_44 ) {
V_61 = V_44 ;
V_62 = strlen ( V_44 ) ;
if ( ! V_32 || V_62 <= V_14 )
goto V_55;
}
if ( V_43 ) {
V_61 = V_43 -> V_63 -> V_64 . V_65 ;
V_62 = strlen ( V_61 ) ;
} else
V_62 = V_14 ;
V_55:
return F_2 ( V_61 , V_62 ,
V_36 , V_9 ) ;
}
int F_23 ( struct V_41 * V_42 , struct V_43 * V_43 ,
const unsigned char * V_44 ,
struct V_45 * V_46 , int V_47 ,
struct V_8 * V_9 )
{
return F_21 ( V_42 , V_43 , V_44 ,
V_9 , true ) ;
}
int F_24 ( struct V_41 * V_42 , struct V_43 * V_43 ,
const unsigned char * V_44 ,
struct V_45 * V_46 , int V_47 ,
struct V_8 * V_9 )
{
return F_21 ( V_42 , V_43 , V_44 ,
V_9 , false ) ;
}
int F_25 ( struct V_41 * V_42 , struct V_43 * V_43 ,
const unsigned char * V_44 ,
struct V_45 * V_46 , int V_47 ,
struct V_8 * V_9 )
{
enum V_6 V_36 = V_25 ;
int V_66 = 0 ;
if ( ( ! V_46 ) || ( V_46 -> type != V_67 ) )
goto V_55;
V_66 = F_2 ( V_46 , V_47 , V_36 ,
V_9 ) ;
V_55:
return V_66 ;
}
