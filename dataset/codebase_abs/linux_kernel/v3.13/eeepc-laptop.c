static int F_1 ( T_1 V_1 , const char * V_2 , int V_3 )
{
T_2 V_4 ;
V_4 = F_2 ( V_1 , ( char * ) V_2 , V_3 ) ;
return ( V_4 == V_5 ? 0 : - 1 ) ;
}
static int F_3 ( T_1 V_1 , const char * V_2 , int * V_3 )
{
T_2 V_4 ;
unsigned long long V_6 ;
V_4 = F_4 ( V_1 , ( char * ) V_2 , NULL , & V_6 ) ;
if ( F_5 ( V_4 ) ) {
* V_3 = - 1 ;
return - 1 ;
} else {
* V_3 = V_6 ;
return 0 ;
}
}
static int F_6 ( struct V_7 * V_8 , int V_9 , int V_10 )
{
const char * V_2 = V_11 [ V_9 ] ;
if ( V_2 == NULL )
return - V_12 ;
if ( ( V_8 -> V_13 & ( 0x1 << V_9 ) ) == 0 )
return - V_12 ;
if ( F_1 ( V_8 -> V_1 , V_2 , V_10 ) )
F_7 ( L_1 , V_2 ) ;
return 0 ;
}
static int F_8 ( struct V_7 * V_8 , int V_9 )
{
const char * V_2 = V_14 [ V_9 ] ;
int V_10 ;
if ( V_2 == NULL )
return - V_12 ;
if ( ( V_8 -> V_13 & ( 0x1 << V_9 ) ) == 0 )
return - V_12 ;
if ( F_3 ( V_8 -> V_1 , V_2 , & V_10 ) )
F_7 ( L_2 , V_2 ) ;
return V_10 ;
}
static int F_9 ( struct V_7 * V_8 , int V_9 ,
T_1 * V_1 )
{
const char * V_2 = V_11 [ V_9 ] ;
T_2 V_4 ;
if ( V_2 == NULL )
return - V_12 ;
if ( ( V_8 -> V_13 & ( 0x1 << V_9 ) ) == 0 )
return - V_12 ;
V_4 = F_10 ( V_8 -> V_1 , ( char * ) V_2 ,
V_1 ) ;
if ( V_4 != V_5 ) {
F_7 ( L_3 , V_2 ) ;
return - V_12 ;
}
return 0 ;
}
static int F_11 ( const char * V_15 , unsigned long V_16 , int * V_3 )
{
if ( ! V_16 )
return 0 ;
if ( sscanf ( V_15 , L_4 , V_3 ) != 1 )
return - V_17 ;
return V_16 ;
}
static T_3 F_12 ( struct V_18 * V_19 , int V_9 ,
const char * V_15 , T_4 V_16 )
{
struct V_7 * V_8 = F_13 ( V_19 ) ;
int V_20 , V_10 ;
V_20 = F_11 ( V_15 , V_16 , & V_10 ) ;
if ( V_20 > 0 )
V_10 = F_6 ( V_8 , V_9 , V_10 ) ;
if ( V_10 < 0 )
return - V_21 ;
return V_20 ;
}
static T_3 F_14 ( struct V_18 * V_19 , int V_9 , char * V_15 )
{
struct V_7 * V_8 = F_13 ( V_19 ) ;
int V_10 = F_8 ( V_8 , V_9 ) ;
if ( V_10 < 0 )
return - V_21 ;
return sprintf ( V_15 , L_5 , V_10 ) ;
}
static int F_15 ( struct V_7 * V_8 , struct V_22 * V_23 )
{
V_23 -> V_24 = F_8 ( V_8 , V_25 ) ;
V_23 -> V_26 = ( V_23 -> V_24 >> 8 ) & 0xff ;
V_23 -> V_24 &= 0xff ;
if ( V_23 -> V_24 < 0 || V_23 -> V_26 <= 0 || V_23 -> V_26 > 12 )
return - V_12 ;
return 0 ;
}
static T_3 F_16 ( struct V_18 * V_19 ,
struct V_27 * V_28 ,
char * V_15 )
{
struct V_7 * V_8 = F_13 ( V_19 ) ;
struct V_22 V_23 ;
int V_29 ;
T_3 V_30 = 0 ;
if ( F_15 ( V_8 , & V_23 ) )
return - V_12 ;
for ( V_29 = 0 ; V_29 < V_23 . V_26 ; V_29 ++ )
V_30 += sprintf ( V_15 + V_30 , L_6 , V_29 ) ;
V_30 += sprintf ( V_15 + V_30 , L_7 ) ;
return V_30 ;
}
static T_3 F_17 ( struct V_18 * V_19 ,
struct V_27 * V_28 ,
char * V_15 )
{
struct V_7 * V_8 = F_13 ( V_19 ) ;
struct V_22 V_23 ;
if ( F_15 ( V_8 , & V_23 ) )
return - V_12 ;
return sprintf ( V_15 , L_8 , ( V_23 . V_26 << 8 ) | V_23 . V_24 ) ;
}
static T_3 F_18 ( struct V_18 * V_19 ,
struct V_27 * V_28 ,
const char * V_15 , T_4 V_16 )
{
struct V_7 * V_8 = F_13 ( V_19 ) ;
struct V_22 V_23 ;
int V_20 , V_10 ;
if ( V_8 -> V_31 )
return - V_32 ;
if ( F_15 ( V_8 , & V_23 ) )
return - V_12 ;
V_20 = F_11 ( V_15 , V_16 , & V_10 ) ;
if ( V_20 < 0 )
return V_20 ;
if ( ! V_20 || V_10 < 0 || V_10 >= V_23 . V_26 )
return - V_17 ;
F_6 ( V_8 , V_25 , V_10 ) ;
return V_20 ;
}
static T_3 F_19 ( struct V_18 * V_19 ,
struct V_27 * V_28 ,
char * V_15 )
{
struct V_7 * V_8 = F_13 ( V_19 ) ;
return sprintf ( V_15 , L_5 , V_8 -> V_31 ) ;
}
static T_3 F_20 ( struct V_18 * V_19 ,
struct V_27 * V_28 ,
const char * V_15 , T_4 V_16 )
{
struct V_7 * V_8 = F_13 ( V_19 ) ;
int V_20 , V_10 ;
V_20 = F_11 ( V_15 , V_16 , & V_10 ) ;
if ( V_20 < 0 )
return V_20 ;
switch ( V_10 ) {
case 0 :
if ( V_8 -> V_31 )
F_7 ( L_9
L_10 ) ;
V_8 -> V_31 = false ;
return V_20 ;
case 1 :
return - V_32 ;
default:
return - V_17 ;
}
}
static int F_21 ( struct V_7 * V_8 )
{
int V_6 ;
V_8 -> V_33 = F_22 ( V_34 , - 1 ) ;
if ( ! V_8 -> V_33 )
return - V_35 ;
F_23 ( V_8 -> V_33 , V_8 ) ;
V_6 = F_24 ( V_8 -> V_33 ) ;
if ( V_6 )
goto V_36;
V_6 = F_25 ( & V_8 -> V_33 -> V_19 . V_37 ,
& V_38 ) ;
if ( V_6 )
goto V_39;
return 0 ;
V_39:
F_26 ( V_8 -> V_33 ) ;
V_36:
F_27 ( V_8 -> V_33 ) ;
return V_6 ;
}
static void F_28 ( struct V_7 * V_8 )
{
F_29 ( & V_8 -> V_33 -> V_19 . V_37 ,
& V_38 ) ;
F_30 ( V_8 -> V_33 ) ;
}
static void F_31 ( struct V_40 * V_41 )
{
struct V_7 * V_8 ;
V_8 = F_32 ( V_41 , struct V_7 , V_42 ) ;
F_6 ( V_8 , V_43 , V_8 -> V_44 ) ;
}
static void F_33 ( struct V_45 * V_46 ,
enum V_47 V_10 )
{
struct V_7 * V_8 ;
V_8 = F_32 ( V_46 , struct V_7 , V_48 ) ;
V_8 -> V_44 = ( V_10 > 0 ) ? 1 : 0 ;
F_34 ( V_8 -> V_49 , & V_8 -> V_42 ) ;
}
static enum V_47 F_35 ( struct V_45 * V_46 )
{
struct V_7 * V_8 ;
V_8 = F_32 ( V_46 , struct V_7 , V_48 ) ;
return F_8 ( V_8 , V_43 ) ;
}
static int F_36 ( struct V_7 * V_8 )
{
int V_20 ;
if ( F_8 ( V_8 , V_43 ) == - V_12 )
return 0 ;
V_8 -> V_49 = F_37 ( L_11 ) ;
if ( ! V_8 -> V_49 )
return - V_35 ;
F_38 ( & V_8 -> V_42 , F_31 ) ;
V_8 -> V_48 . V_50 = L_12 ;
V_8 -> V_48 . V_51 = F_33 ;
if ( F_8 ( V_8 , V_43 ) >= 0 )
V_8 -> V_48 . V_52 = F_35 ;
V_8 -> V_48 . V_53 = 1 ;
V_20 = F_39 ( & V_8 -> V_33 -> V_19 ,
& V_8 -> V_48 ) ;
if ( V_20 ) {
F_40 ( V_8 -> V_49 ) ;
return V_20 ;
}
return 0 ;
}
static void F_41 ( struct V_7 * V_8 )
{
if ( ! F_42 ( V_8 -> V_48 . V_19 ) )
F_43 ( & V_8 -> V_48 ) ;
if ( V_8 -> V_49 )
F_40 ( V_8 -> V_49 ) ;
}
static bool F_44 ( struct V_7 * V_8 )
{
if ( F_8 ( V_8 , V_54 ) == 1 )
return false ;
return true ;
}
static void F_45 ( struct V_7 * V_8 , T_1 V_1 )
{
struct V_55 * V_56 ;
struct V_55 * V_19 ;
struct V_57 * V_58 ;
bool V_59 = F_44 ( V_8 ) ;
bool V_60 ;
T_5 V_61 ;
if ( V_8 -> V_62 )
F_46 ( V_8 -> V_62 , V_59 ) ;
F_47 ( & V_8 -> V_63 ) ;
if ( V_8 -> V_64 ) {
V_56 = F_48 ( V_1 ) ;
if ( ! V_56 ) {
F_49 ( L_13 ) ;
goto V_65;
}
V_58 = V_56 -> V_66 ;
if ( ! V_58 ) {
F_7 ( L_14 ) ;
goto V_67;
}
if ( F_50 ( V_58 , 0 , V_68 , & V_61 ) ) {
F_51 ( L_15 ) ;
goto V_67;
}
V_60 = ( V_61 == 0xffffffff ) ;
if ( V_59 != V_60 ) {
F_7 ( L_16
L_17 ,
V_59 ? L_18 : L_19 ,
V_60 ? L_20 : L_21 ) ;
F_7 ( L_22
L_23 ) ;
goto V_67;
}
if ( ! V_59 ) {
V_19 = F_52 ( V_58 , 0 ) ;
if ( V_19 ) {
F_53 ( V_19 ) ;
goto V_67;
}
V_19 = F_54 ( V_58 , 0 ) ;
if ( V_19 ) {
F_55 ( V_58 ) ;
if ( F_56 ( V_19 ) )
F_51 ( L_24 ) ;
}
} else {
V_19 = F_52 ( V_58 , 0 ) ;
if ( V_19 ) {
F_57 ( V_19 ) ;
F_53 ( V_19 ) ;
}
}
V_67:
F_53 ( V_56 ) ;
}
V_65:
F_58 ( & V_8 -> V_63 ) ;
}
static void F_59 ( struct V_7 * V_8 , char * V_69 )
{
T_2 V_4 = V_5 ;
T_1 V_1 ;
V_4 = F_10 ( NULL , V_69 , & V_1 ) ;
if ( F_60 ( V_4 ) )
F_45 ( V_8 , V_1 ) ;
}
static void F_61 ( T_1 V_1 , T_5 V_70 , void * V_71 )
{
struct V_7 * V_8 = V_71 ;
if ( V_70 != V_72 )
return;
F_45 ( V_8 , V_1 ) ;
}
static int F_62 ( struct V_7 * V_8 ,
char * V_69 )
{
T_2 V_4 ;
T_1 V_1 ;
V_4 = F_10 ( NULL , V_69 , & V_1 ) ;
if ( F_60 ( V_4 ) ) {
V_4 = F_63 ( V_1 ,
V_73 ,
F_61 ,
V_8 ) ;
if ( F_5 ( V_4 ) )
F_7 ( L_25 , V_69 ) ;
F_45 ( V_8 , V_1 ) ;
} else
return - V_12 ;
return 0 ;
}
static void F_64 ( struct V_7 * V_8 ,
char * V_69 )
{
T_2 V_4 = V_5 ;
T_1 V_1 ;
V_4 = F_10 ( NULL , V_69 , & V_1 ) ;
if ( F_60 ( V_4 ) ) {
V_4 = F_65 ( V_1 ,
V_73 ,
F_61 ) ;
if ( F_5 ( V_4 ) )
F_51 ( L_26 ,
V_69 ) ;
F_45 ( V_8 , V_1 ) ;
}
}
static int F_66 ( struct V_64 * V_64 ,
T_6 * V_10 )
{
struct V_7 * V_8 = V_64 -> V_74 ;
int V_3 = F_8 ( V_8 , V_54 ) ;
if ( V_3 == 1 || V_3 == 0 )
* V_10 = V_3 ;
else
return - V_17 ;
return 0 ;
}
static void F_67 ( struct V_64 * V_64 )
{
F_68 ( V_64 -> V_75 ) ;
F_68 ( V_64 ) ;
}
static int F_69 ( struct V_7 * V_8 )
{
int V_76 = - V_35 ;
struct V_57 * V_58 = F_70 ( 0 , 1 ) ;
if ( ! V_58 ) {
F_51 ( L_27 ) ;
return - V_12 ;
}
V_8 -> V_64 = F_71 ( sizeof( struct V_64 ) , V_77 ) ;
if ( ! V_8 -> V_64 )
goto V_78;
V_8 -> V_64 -> V_75 = F_71 ( sizeof( struct V_79 ) ,
V_77 ) ;
if ( ! V_8 -> V_64 -> V_75 )
goto V_80;
V_8 -> V_64 -> V_74 = V_8 ;
V_8 -> V_64 -> V_81 = & F_67 ;
V_8 -> V_64 -> V_82 = & V_83 ;
F_66 ( V_8 -> V_64 ,
& V_8 -> V_64 -> V_75 -> V_84 ) ;
V_76 = F_72 ( V_8 -> V_64 , V_58 , 0 , L_28 ) ;
if ( V_76 ) {
F_51 ( L_29 , V_76 ) ;
goto V_85;
}
return 0 ;
V_85:
F_68 ( V_8 -> V_64 -> V_75 ) ;
V_80:
F_68 ( V_8 -> V_64 ) ;
V_8 -> V_64 = NULL ;
V_78:
return V_76 ;
}
static int F_73 ( void * V_71 , bool V_59 )
{
T_1 V_1 = V_71 ;
return F_1 ( V_1 , NULL , ! V_59 ) ;
}
static int F_74 ( struct V_7 * V_8 ,
struct V_86 * * V_86 ,
const char * V_50 ,
enum V_87 type , int V_9 )
{
T_1 V_1 ;
int V_6 ;
V_6 = F_9 ( V_8 , V_9 , & V_1 ) ;
if ( V_6 < 0 )
return V_6 ;
* V_86 = F_75 ( V_50 , & V_8 -> V_33 -> V_19 , type ,
& V_88 , V_1 ) ;
if ( ! * V_86 )
return - V_17 ;
F_76 ( * V_86 , F_8 ( V_8 , V_9 ) != 1 ) ;
V_6 = F_77 ( * V_86 ) ;
if ( V_6 ) {
F_78 ( * V_86 ) ;
* V_86 = NULL ;
return V_6 ;
}
return 0 ;
}
static void F_79 ( struct V_7 * V_8 )
{
F_64 ( V_8 , L_30 ) ;
F_64 ( V_8 , L_31 ) ;
F_64 ( V_8 , L_32 ) ;
if ( V_8 -> V_62 ) {
F_80 ( V_8 -> V_62 ) ;
F_78 ( V_8 -> V_62 ) ;
V_8 -> V_62 = NULL ;
}
if ( V_8 -> V_64 )
F_81 ( V_8 -> V_64 ) ;
if ( V_8 -> V_89 ) {
F_80 ( V_8 -> V_89 ) ;
F_78 ( V_8 -> V_89 ) ;
V_8 -> V_89 = NULL ;
}
if ( V_8 -> V_90 ) {
F_80 ( V_8 -> V_90 ) ;
F_78 ( V_8 -> V_90 ) ;
V_8 -> V_90 = NULL ;
}
if ( V_8 -> V_91 ) {
F_80 ( V_8 -> V_91 ) ;
F_78 ( V_8 -> V_91 ) ;
V_8 -> V_91 = NULL ;
}
}
static int F_82 ( struct V_7 * V_8 )
{
int V_6 = 0 ;
F_83 ( & V_8 -> V_63 ) ;
V_6 = F_74 ( V_8 , & V_8 -> V_62 ,
L_33 , V_92 ,
V_54 ) ;
if ( V_6 && V_6 != - V_12 )
goto exit;
V_6 = F_74 ( V_8 , & V_8 -> V_89 ,
L_34 , V_93 ,
V_94 ) ;
if ( V_6 && V_6 != - V_12 )
goto exit;
V_6 = F_74 ( V_8 , & V_8 -> V_90 ,
L_35 , V_95 ,
V_96 ) ;
if ( V_6 && V_6 != - V_12 )
goto exit;
V_6 = F_74 ( V_8 , & V_8 -> V_91 ,
L_36 , V_97 ,
V_98 ) ;
if ( V_6 && V_6 != - V_12 )
goto exit;
if ( V_8 -> V_99 )
return 0 ;
V_6 = F_69 ( V_8 ) ;
if ( V_6 == - V_100 )
V_6 = 0 ;
F_62 ( V_8 , L_30 ) ;
F_62 ( V_8 , L_31 ) ;
F_62 ( V_8 , L_32 ) ;
exit:
if ( V_6 && V_6 != - V_12 )
F_79 ( V_8 ) ;
return V_6 ;
}
static int F_84 ( struct V_18 * V_18 )
{
struct V_7 * V_8 = F_13 ( V_18 ) ;
if ( V_8 -> V_62 ) {
bool V_101 ;
V_101 = F_8 ( V_8 , V_54 ) ;
F_6 ( V_8 , V_54 , V_101 ) ;
}
return 0 ;
}
static int F_85 ( struct V_18 * V_18 )
{
struct V_7 * V_8 = F_13 ( V_18 ) ;
if ( V_8 -> V_62 ) {
F_59 ( V_8 , L_30 ) ;
F_59 ( V_8 , L_31 ) ;
F_59 ( V_8 , L_32 ) ;
}
if ( V_8 -> V_89 )
F_46 ( V_8 -> V_89 ,
F_8 ( V_8 , V_94 ) != 1 ) ;
if ( V_8 -> V_90 )
F_46 ( V_8 -> V_90 ,
F_8 ( V_8 , V_96 ) != 1 ) ;
if ( V_8 -> V_91 )
F_46 ( V_8 -> V_91 ,
F_8 ( V_8 , V_98 ) != 1 ) ;
return 0 ;
}
static int F_86 ( void )
{
T_6 V_10 = 0 ;
F_87 ( V_102 , & V_10 ) ;
return V_10 * 255 / 100 ;
}
static void F_88 ( int V_10 )
{
V_10 = F_89 ( V_10 , 0 , 255 ) ;
V_10 = V_10 * 100 / 255 ;
F_90 ( V_102 , V_10 ) ;
}
static int F_91 ( void )
{
T_6 V_103 = 0 ;
T_6 V_104 = 0 ;
F_87 ( V_105 , & V_103 ) ;
F_87 ( V_106 , & V_104 ) ;
return V_103 << 8 | V_104 ;
}
static int F_92 ( void )
{
T_6 V_10 = 0 ;
F_87 ( V_107 , & V_10 ) ;
if ( V_10 & 0x02 )
return 1 ;
else
return 2 ;
}
static void F_93 ( int V_108 )
{
T_6 V_10 = 0 ;
F_87 ( V_107 , & V_10 ) ;
if ( V_108 == 1 )
V_10 |= 0x02 ;
else
V_10 &= ~ 0x02 ;
F_90 ( V_107 , V_10 ) ;
}
static T_3 F_94 ( void (* F_95)( int ) , const char * V_15 , T_4 V_16 )
{
int V_20 , V_10 ;
V_20 = F_11 ( V_15 , V_16 , & V_10 ) ;
if ( V_20 > 0 )
F_95 ( V_10 ) ;
return V_20 ;
}
static T_3 F_96 ( int (* F_97)( void ) , char * V_15 )
{
return sprintf ( V_15 , L_5 , F_97 () ) ;
}
static T_3
F_98 ( struct V_18 * V_19 , struct V_27 * V_28 , char * V_15 )
{
return sprintf ( V_15 , L_37 ) ;
}
static void F_99 ( struct V_7 * V_8 )
{
struct V_18 * V_109 ;
V_109 = V_8 -> V_110 ;
if ( ! V_109 )
return;
F_29 ( & V_109 -> V_37 ,
& V_111 ) ;
F_100 ( V_109 ) ;
V_8 -> V_110 = NULL ;
}
static int F_101 ( struct V_7 * V_8 )
{
struct V_18 * V_109 ;
int V_6 ;
V_109 = F_102 ( & V_8 -> V_33 -> V_19 ) ;
if ( F_103 ( V_109 ) ) {
F_51 ( L_38 ) ;
V_8 -> V_110 = NULL ;
return F_104 ( V_109 ) ;
}
V_8 -> V_110 = V_109 ;
V_6 = F_25 ( & V_109 -> V_37 ,
& V_111 ) ;
if ( V_6 )
F_99 ( V_8 ) ;
return V_6 ;
}
static int F_105 ( struct V_112 * V_113 )
{
struct V_7 * V_8 = F_106 ( V_113 ) ;
return F_8 ( V_8 , V_114 ) ;
}
static int F_107 ( struct V_112 * V_113 , int V_10 )
{
struct V_7 * V_8 = F_106 ( V_113 ) ;
return F_6 ( V_8 , V_114 , V_10 ) ;
}
static int F_108 ( struct V_112 * V_113 )
{
return F_107 ( V_113 , V_113 -> V_115 . V_116 ) ;
}
static int F_109 ( struct V_7 * V_8 )
{
struct V_112 * V_113 = V_8 -> V_112 ;
int V_117 = V_113 -> V_115 . V_116 ;
F_110 ( V_113 , V_118 ) ;
return V_117 ;
}
static int F_111 ( struct V_7 * V_8 )
{
struct V_119 V_115 ;
struct V_112 * V_113 ;
memset ( & V_115 , 0 , sizeof( struct V_119 ) ) ;
V_115 . type = V_120 ;
V_115 . V_53 = 15 ;
V_113 = F_112 ( V_34 ,
& V_8 -> V_33 -> V_19 , V_8 ,
& V_121 , & V_115 ) ;
if ( F_103 ( V_113 ) ) {
F_51 ( L_39 ) ;
V_8 -> V_112 = NULL ;
return F_104 ( V_113 ) ;
}
V_8 -> V_112 = V_113 ;
V_113 -> V_115 . V_116 = F_105 ( V_113 ) ;
V_113 -> V_115 . V_122 = V_123 ;
F_113 ( V_113 ) ;
return 0 ;
}
static void F_114 ( struct V_7 * V_8 )
{
if ( V_8 -> V_112 )
F_115 ( V_8 -> V_112 ) ;
V_8 -> V_112 = NULL ;
}
static int F_116 ( struct V_7 * V_8 )
{
struct V_124 * V_125 ;
int error ;
V_125 = F_117 () ;
if ( ! V_125 )
return - V_35 ;
V_125 -> V_50 = L_40 ;
V_125 -> V_126 = V_34 L_41 ;
V_125 -> V_127 . V_128 = V_129 ;
V_125 -> V_19 . V_130 = & V_8 -> V_33 -> V_19 ;
error = F_118 ( V_125 , V_131 , NULL ) ;
if ( error ) {
F_51 ( L_42 ) ;
goto V_132;
}
error = F_119 ( V_125 ) ;
if ( error ) {
F_51 ( L_43 ) ;
goto V_133;
}
V_8 -> V_134 = V_125 ;
return 0 ;
V_133:
F_120 ( V_125 ) ;
V_132:
F_121 ( V_125 ) ;
return error ;
}
static void F_122 ( struct V_7 * V_8 )
{
if ( V_8 -> V_134 ) {
F_120 ( V_8 -> V_134 ) ;
F_123 ( V_8 -> V_134 ) ;
}
V_8 -> V_134 = NULL ;
}
static void F_124 ( struct V_7 * V_8 , int V_70 )
{
if ( ! V_8 -> V_134 )
return ;
if ( ! F_125 ( V_8 -> V_134 , V_70 , 1 , true ) )
F_126 ( L_44 , V_70 ) ;
}
static void F_127 ( struct V_135 * V_18 , T_5 V_70 )
{
struct V_7 * V_8 = F_128 ( V_18 ) ;
T_7 V_16 ;
if ( V_70 > V_136 )
return;
V_16 = V_8 -> V_137 [ V_70 % 128 ] ++ ;
F_129 ( V_18 -> V_138 . V_139 ,
F_130 ( & V_18 -> V_19 ) , V_70 ,
V_16 ) ;
if ( V_70 >= V_140 && V_70 <= V_141 ) {
if ( V_8 -> V_112 != NULL ) {
int V_142 , V_143 ;
V_142 = F_109 ( V_8 ) ;
V_143 = V_70 - V_140 ;
if ( V_143 < V_142 ) {
V_70 = V_140 ;
} else if ( V_143 > V_142 ) {
V_70 = V_141 ;
} else {
}
F_124 ( V_8 , V_70 ) ;
}
} else {
F_124 ( V_8 , V_70 ) ;
}
}
static void F_131 ( struct V_7 * V_8 )
{
const char * V_144 ;
V_144 = F_132 ( V_145 ) ;
if ( ! V_144 )
return;
if ( strcmp ( V_144 , L_45 ) == 0 || strcmp ( V_144 , L_46 ) == 0 ) {
V_8 -> V_31 = true ;
F_126 ( L_47
L_48 , V_144 ) ;
F_126 ( L_49 ) ;
}
if ( strcmp ( V_144 , L_50 ) == 0 || strcmp ( V_144 , L_51 ) == 0 ||
strcmp ( V_144 , L_52 ) == 0 ) {
V_8 -> V_99 = true ;
F_126 ( L_53 ) ;
}
}
static void F_133 ( struct V_7 * V_8 , int V_9 , const char * V_50 )
{
int V_146 ;
if ( ! ( V_8 -> V_13 & ( 1 << V_9 ) )
&& ! F_3 ( V_8 -> V_1 , V_14 [ V_9 ] , & V_146 ) ) {
F_126 ( L_54
L_55 , V_50 , 1 << V_9 ) ;
V_8 -> V_13 |= 1 << V_9 ;
}
}
static void F_134 ( struct V_7 * V_8 )
{
F_133 ( V_8 , V_147 , L_56 ) ;
F_133 ( V_8 , V_148 , L_57 ) ;
F_133 ( V_8 , V_149 , L_58 ) ;
F_133 ( V_8 , V_43 , L_59 ) ;
}
static int F_135 ( struct V_7 * V_8 )
{
unsigned int V_150 ;
int V_6 ;
V_6 = F_136 ( V_8 -> V_18 ) ;
if ( V_6 )
return V_6 ;
if ( ! V_8 -> V_18 -> V_4 . V_151 ) {
F_51 ( L_60 ) ;
return - V_12 ;
}
V_150 = V_152 | V_153 ;
F_137 ( L_61 , V_150 ) ;
if ( F_1 ( V_8 -> V_1 , L_62 , V_150 ) ) {
F_51 ( L_63 ) ;
return - V_12 ;
}
if ( F_3 ( V_8 -> V_1 , L_64 , & V_8 -> V_13 ) ) {
F_51 ( L_65 ) ;
return - V_12 ;
}
F_134 ( V_8 ) ;
F_126 ( L_66 , V_8 -> V_13 ) ;
return 0 ;
}
static void F_138 ( struct V_7 * V_8 )
{
if ( F_8 ( V_8 , V_154 ) == 0 )
F_6 ( V_8 , V_154 , 1 ) ;
}
static int F_139 ( struct V_135 * V_18 )
{
struct V_7 * V_8 ;
int V_6 ;
F_137 ( V_155 L_7 ) ;
V_8 = F_71 ( sizeof( struct V_7 ) , V_77 ) ;
if ( ! V_8 )
return - V_35 ;
V_8 -> V_1 = V_18 -> V_1 ;
strcpy ( F_140 ( V_18 ) , V_156 ) ;
strcpy ( F_141 ( V_18 ) , V_157 ) ;
V_18 -> V_158 = V_8 ;
V_8 -> V_18 = V_18 ;
V_8 -> V_99 = V_99 ;
F_131 ( V_8 ) ;
V_6 = F_135 ( V_8 ) ;
if ( V_6 )
goto V_159;
F_138 ( V_8 ) ;
V_6 = F_21 ( V_8 ) ;
if ( V_6 )
goto V_159;
if ( ! F_142 () ) {
V_6 = F_111 ( V_8 ) ;
if ( V_6 )
goto V_160;
} else
F_126 ( L_67 ) ;
V_6 = F_116 ( V_8 ) ;
if ( V_6 )
goto V_161;
V_6 = F_101 ( V_8 ) ;
if ( V_6 )
goto V_162;
V_6 = F_36 ( V_8 ) ;
if ( V_6 )
goto V_163;
V_6 = F_82 ( V_8 ) ;
if ( V_6 )
goto V_164;
V_165 = true ;
return 0 ;
V_164:
F_41 ( V_8 ) ;
V_163:
F_99 ( V_8 ) ;
V_162:
F_122 ( V_8 ) ;
V_161:
F_114 ( V_8 ) ;
V_160:
F_28 ( V_8 ) ;
V_159:
F_68 ( V_8 ) ;
return V_6 ;
}
static int F_143 ( struct V_135 * V_18 )
{
struct V_7 * V_8 = F_128 ( V_18 ) ;
F_114 ( V_8 ) ;
F_79 ( V_8 ) ;
F_122 ( V_8 ) ;
F_99 ( V_8 ) ;
F_41 ( V_8 ) ;
F_28 ( V_8 ) ;
F_68 ( V_8 ) ;
return 0 ;
}
static int T_8 F_144 ( void )
{
int V_6 ;
V_6 = F_145 ( & V_166 ) ;
if ( V_6 < 0 )
return V_6 ;
V_6 = F_146 ( & V_167 ) ;
if ( V_6 < 0 )
goto V_168;
if ( ! V_165 ) {
V_6 = - V_12 ;
goto V_169;
}
return 0 ;
V_169:
F_147 ( & V_167 ) ;
V_168:
F_148 ( & V_166 ) ;
return V_6 ;
}
static void T_9 F_149 ( void )
{
F_147 ( & V_167 ) ;
F_148 ( & V_166 ) ;
}
