static struct V_1 * F_1 ( struct V_2 * V_3 ,
T_1 V_4 )
{
struct V_1 * V_5 = V_6 ;
while ( ( V_5 != NULL ) &&
( ( V_5 -> V_7 -> V_3 != V_3 ) ||
( V_5 -> V_7 -> V_4 != V_4 ) ) ) {
V_5 = V_5 -> V_8 ;
}
return V_5 ;
}
static void F_2 ( struct V_9 * V_7 )
{
struct V_1 * V_10 = V_6 ;
struct V_1 * V_11 = F_1 ( V_7 -> V_3 ,
V_7 -> V_4 ) ;
if ( V_11 != NULL ) {
if ( V_11 == V_6 ) {
V_6 = V_11 -> V_8 ;
} else {
while ( V_10 -> V_8 != V_11 )
V_10 = V_10 -> V_8 ;
V_10 -> V_8 = V_11 -> V_8 ;
}
F_3 ( V_11 ) ;
}
}
static struct V_1 * F_4 ( struct V_9 * V_7 )
{
struct V_1 * V_12 ;
struct V_1 * V_5 ;
V_12 = F_5 ( sizeof( struct V_1 ) , V_13 ) ;
if ( V_12 != NULL ) {
V_12 -> V_7 = V_7 ;
V_12 -> V_8 = NULL ;
if ( V_6 == NULL ) {
V_6 = V_12 ;
} else {
V_5 = V_6 ;
while ( V_5 -> V_8 != NULL )
V_5 = V_5 -> V_8 ;
V_5 -> V_8 = V_12 ;
}
}
return V_12 ;
}
static inline T_2 F_6 ( T_2 V_14 , T_2 V_15 )
{
if ( V_15 == 32 )
return V_14 ;
else
return ( V_14 >= ( 1 << ( V_15 - 1 ) ) ) ? ( V_14 - ( 1 << V_15 ) ) : V_14 ;
}
static int F_7 ( struct V_16 * V_17 , unsigned int V_18 )
{
const struct V_19 * V_20 = V_17 -> V_20 ;
int V_21 ;
T_1 V_22 [] = { V_18 >> 8 , V_18 & 0xff } ;
T_1 V_23 ;
struct V_24 V_25 [] = {
{ . V_26 = V_20 -> V_27 , . V_28 = 0 , . V_23 = V_22 , . V_29 = 2 } ,
{ . V_26 = V_20 -> V_27 , . V_28 = V_30 , . V_23 = & V_23 , . V_29 = 1 }
} ;
V_21 = F_8 ( V_17 -> V_31 , V_25 , 2 ) ;
if ( V_21 != 2 ) {
if ( V_21 != - V_32 )
F_9 ( V_33 , 1 ,
L_1 ,
V_18 , V_21 ) ;
return V_21 < 0 ? V_21 : - V_34 ;
}
if ( F_10 ( * V_17 -> V_35 >= V_36 ) )
F_9 ( V_33 , 1 , L_2 ,
V_18 , V_23 ) ;
return ( unsigned int ) V_23 ;
}
static int F_11 ( struct V_16 * V_17 , unsigned int V_18 , T_1 * V_37 , T_3 V_38 )
{
const struct V_19 * V_20 = V_17 -> V_20 ;
int V_21 ;
T_1 V_23 [ V_39 ] ;
struct V_24 V_24 = { . V_26 = V_20 -> V_27 , . V_28 = 0 , . V_23 = V_23 , . V_29 = 2 + V_38 } ;
if ( 2 + V_38 > sizeof( V_23 ) ) {
F_12 ( V_40
L_3 ,
V_41 , V_18 , V_38 ) ;
return - V_42 ;
}
V_23 [ 0 ] = V_18 >> 8 ;
V_23 [ 1 ] = V_18 & 0xff ;
memcpy ( & V_23 [ 2 ] , V_37 , V_38 ) ;
if ( F_10 ( * V_17 -> V_35 >= V_36 ) ) {
int V_43 ;
F_12 ( V_44 L_4 , V_45 , V_18 ) ;
for ( V_43 = 0 ; V_43 < V_38 ; V_43 ++ )
F_12 ( L_5 , V_37 [ V_43 ] ) ;
F_12 ( L_6 ) ;
}
V_21 = F_8 ( V_17 -> V_31 , & V_24 , 1 ) ;
if ( V_21 != 1 ) {
if ( V_21 != - V_32 )
F_9 ( V_33 , 1 , L_7 ,
V_18 , V_37 [ 0 ] , V_38 , V_21 ) ;
return V_21 < 0 ? V_21 : - V_34 ;
}
return 0 ;
}
static int F_13 ( struct V_16 * V_17 , unsigned int V_18 , T_1 V_37 )
{
return F_11 ( V_17 , V_18 , & V_37 , 1 ) ;
}
static int F_14 ( struct V_16 * V_17 , int V_46 )
{
T_3 V_18 ;
if ( V_46 ) {
if ( V_17 -> V_20 -> V_47 )
V_17 -> V_20 -> V_47 ( & V_17 -> V_48 , 1 ) ;
else
F_15 ( & V_17 -> V_7 -> V_49 ) ;
}
V_18 = F_16 ( V_17 , V_50 ) ;
if ( V_46 ) {
F_9 ( V_51 , 1 , L_8 ) ;
F_17 ( V_18 , V_52 , 1 ) ;
if ( F_18 ( V_17 , V_50 , V_18 ) < 0 )
goto V_53;
} else {
F_9 ( V_51 , 1 , L_9 ) ;
F_17 ( V_18 , V_52 , 0 ) ;
if ( ( F_18 ( V_17 , V_50 , V_18 ) ) < 0 )
goto V_53;
}
if ( ! V_46 ) {
if ( V_17 -> V_20 -> V_47 )
V_17 -> V_20 -> V_47 ( & V_17 -> V_48 , 0 ) ;
else
F_19 ( & V_17 -> V_7 -> V_49 ) ;
}
return 0 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
if ( V_17 -> V_20 -> V_47 )
V_17 -> V_20 -> V_47 ( & V_17 -> V_48 , 0 ) ;
else
F_19 ( & V_17 -> V_7 -> V_49 ) ;
return - 1 ;
}
static void F_20 ( struct V_16 * V_17 )
{
switch ( V_17 -> V_54 ) {
case V_55 :
F_9 ( V_51 , 1 , L_11 ) ;
if ( V_17 -> V_56 <= 1500000 ) {
V_17 -> V_57 = 1500 ;
V_17 -> V_58 = 400 ;
} else if ( V_17 -> V_56 <= 5000000 ) {
V_17 -> V_57 = 1000 ;
V_17 -> V_58 = 300 ;
} else {
V_17 -> V_57 = 700 ;
V_17 -> V_58 = 100 ;
}
break;
case V_59 :
case V_60 :
default:
F_9 ( V_51 , 1 , L_12 ) ;
if ( V_17 -> V_56 <= 1000000 ) {
V_17 -> V_57 = 4500 ;
V_17 -> V_58 = 1700 ;
} else if ( V_17 -> V_56 <= 2000000 ) {
V_17 -> V_57 = 2500 ;
V_17 -> V_58 = 1100 ;
} else if ( V_17 -> V_56 <= 5000000 ) {
V_17 -> V_57 = 1000 ;
V_17 -> V_58 = 550 ;
} else if ( V_17 -> V_56 <= 10000000 ) {
V_17 -> V_57 = 700 ;
V_17 -> V_58 = 250 ;
} else if ( V_17 -> V_56 <= 20000000 ) {
V_17 -> V_57 = 400 ;
V_17 -> V_58 = 130 ;
} else {
V_17 -> V_57 = 300 ;
V_17 -> V_58 = 100 ;
}
break;
}
if ( V_17 -> V_54 == V_60 )
V_17 -> V_57 /= 2 ;
}
static int F_21 ( struct V_16 * V_17 , T_3 V_56 )
{
T_3 V_61 ;
if ( V_56 > 60000000 ) {
V_61 = ( V_56 << 4 ) ;
V_61 /= ( V_17 -> V_7 -> V_62 >> 12 ) ;
} else if ( V_56 > 6000000 ) {
V_61 = ( V_56 << 6 ) ;
V_61 /= ( V_17 -> V_7 -> V_62 >> 10 ) ;
} else {
V_61 = ( V_56 << 9 ) ;
V_61 /= ( V_17 -> V_7 -> V_62 >> 7 ) ;
}
if ( F_18 ( V_17 , V_63 , ( V_61 >> 8 ) & 0x7f ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_64 , ( V_61 & 0xff ) ) < 0 )
goto V_53;
return 0 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static int F_22 ( struct V_16 * V_17 , T_3 V_65 , T_3 V_56 )
{
T_3 V_61 ;
V_56 = 105 * ( V_56 / 100 ) ;
if ( V_56 > 60000000 ) {
V_61 = ( V_56 << 4 ) ;
V_61 /= ( V_17 -> V_7 -> V_62 >> 12 ) ;
} else if ( V_56 > 6000000 ) {
V_61 = ( V_56 << 6 ) ;
V_61 /= ( V_17 -> V_7 -> V_62 >> 10 ) ;
} else {
V_61 = ( V_56 << 9 ) ;
V_61 /= ( V_17 -> V_7 -> V_62 >> 7 ) ;
}
if ( V_61 < 0x7fff ) {
if ( F_18 ( V_17 , V_66 , ( V_61 >> 8 ) & 0x7f ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_67 , V_61 & 0xff ) < 0 )
goto V_53;
} else {
if ( F_18 ( V_17 , V_66 , 0x7f ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_67 , 0xff ) < 0 )
goto V_53;
}
return 0 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static int F_23 ( struct V_16 * V_17 , T_3 V_65 , T_3 V_56 )
{
T_3 V_61 ;
V_56 = 95 * ( V_56 / 100 ) ;
if ( V_56 > 60000000 ) {
V_61 = ( V_56 << 4 ) ;
V_61 /= ( V_17 -> V_7 -> V_62 >> 12 ) ;
} else if ( V_56 > 6000000 ) {
V_61 = ( V_56 << 6 ) ;
V_61 /= ( V_17 -> V_7 -> V_62 >> 10 ) ;
} else {
V_61 = ( V_56 << 9 ) ;
V_61 /= ( V_17 -> V_7 -> V_62 >> 7 ) ;
}
if ( F_18 ( V_17 , V_68 , ( ( V_61 >> 8 ) & 0x7f ) ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_69 , ( V_61 & 0xff ) ) < 0 )
goto V_53;
return 0 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static T_3 F_24 ( T_3 V_56 , enum V_70 V_71 )
{
T_3 V_72 ;
switch ( V_71 ) {
case V_73 :
V_72 = 20 ;
break;
case V_74 :
V_72 = 25 ;
break;
case V_75 :
default:
V_72 = 35 ;
break;
}
return V_56 + ( V_56 * V_72 ) / 100 ;
}
static int F_25 ( struct V_16 * V_17 )
{
if ( F_18 ( V_17 , V_76 , 0x96 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_77 , 0x64 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_78 , 0x36 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_79 , 0x23 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_80 , 0x1e ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_81 , 0x19 ) < 0 )
goto V_53;
return 0 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static int F_26 ( struct V_16 * V_17 )
{
if ( F_18 ( V_17 , V_76 , 0xd0 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_77 , 0x7d ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_78 , 0x53 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_79 , 0x2f ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_80 , 0x24 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_81 , 0x1f ) < 0 )
goto V_53;
return 0 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static int F_27 ( struct V_16 * V_17 )
{
switch ( V_17 -> V_82 ) {
case V_83 :
if ( F_18 ( V_17 , V_84 , 0x10 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_85 , 0x3f ) < 0 )
goto V_53;
break;
case V_86 :
if ( F_18 ( V_17 , V_84 , 0x00 ) < 0 )
goto V_53;
switch ( V_17 -> V_87 ) {
case V_88 :
if ( F_18 ( V_17 , V_85 , 0x01 ) < 0 )
goto V_53;
break;
case V_89 :
if ( F_18 ( V_17 , V_85 , 0x02 ) < 0 )
goto V_53;
break;
case V_90 :
if ( F_18 ( V_17 , V_85 , 0x04 ) < 0 )
goto V_53;
break;
case V_91 :
if ( F_18 ( V_17 , V_85 , 0x08 ) < 0 )
goto V_53;
break;
case V_92 :
if ( F_18 ( V_17 , V_85 , 0x20 ) < 0 )
goto V_53;
break;
default:
if ( F_18 ( V_17 , V_85 , 0x2f ) < 0 )
goto V_53;
break;
}
break;
case V_93 :
if ( F_18 ( V_17 , V_84 , 0x80 ) < 0 )
goto V_53;
switch ( V_17 -> V_87 ) {
case V_88 :
if ( F_18 ( V_17 , V_85 , 0x01 ) < 0 )
goto V_53;
break;
case V_89 :
if ( F_18 ( V_17 , V_85 , 0x02 ) < 0 )
goto V_53;
break;
case V_94 :
if ( F_18 ( V_17 , V_85 , 0x10 ) < 0 )
goto V_53;
break;
default:
if ( F_18 ( V_17 , V_85 , 0x13 ) < 0 )
goto V_53;
break;
}
break;
default:
break;
}
return 0 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static int F_28 ( struct V_16 * V_17 )
{
if ( F_18 ( V_17 , V_95 , 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_96 , 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_97 , 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_98 , 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_99 , 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_100 , 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_101 , 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_102 , 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_103 , 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_104 , 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_105 , 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_106 , 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_107 , 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_108 , 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_109 , 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_110 , 0xff ) < 0 )
goto V_53;
return 0 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static int F_29 ( struct V_16 * V_17 )
{
if ( F_18 ( V_17 , V_95 , 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_96 , 0xfc ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_97 , 0xcc ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_98 , 0xcc ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_99 , 0xcc ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_100 , 0xcc ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_101 , 0xcc ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_102 , 0xcc ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_103 , 0xcc ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_104 , 0xcc ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_105 , 0xcc ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_106 , 0xcc ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_107 , 0xcc ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_108 , 0xcc ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_109 , 0xcc ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_110 , 0xcf ) < 0 )
goto V_53;
return 0 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static int F_30 ( struct V_16 * V_17 )
{
if ( F_18 ( V_17 , V_95 , 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_96 , 0xf0 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_97 , 0x00 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_98 , 0x00 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_99 , 0x00 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_100 , 0x00 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_101 , 0x00 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_102 , 0x00 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_103 , 0x00 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_104 , 0x00 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_105 , 0x00 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_106 , 0x00 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_107 , 0x00 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_108 , 0x00 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_109 , 0x00 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_110 , 0x0f ) < 0 )
goto V_53;
return 0 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static int F_31 ( struct V_16 * V_17 , int V_46 )
{
T_3 V_18 ;
switch ( V_17 -> V_111 ) {
case V_112 :
F_15 ( & V_17 -> V_7 -> V_113 ) ;
V_18 = F_7 ( V_17 , V_114 ) ;
F_32 ( V_18 , V_115 , V_46 ) ;
if ( F_13 ( V_17 , V_114 , V_18 ) < 0 )
goto V_53;
F_19 ( & V_17 -> V_7 -> V_113 ) ;
break;
case V_116 :
F_15 ( & V_17 -> V_7 -> V_113 ) ;
V_18 = F_7 ( V_17 , V_114 ) ;
F_32 ( V_18 , V_117 , V_46 ) ;
if ( F_13 ( V_17 , V_114 , V_18 ) < 0 )
goto V_53;
F_19 ( & V_17 -> V_7 -> V_113 ) ;
break;
default:
F_9 ( V_33 , 1 , L_13 ) ;
break;
}
return 0 ;
V_53:
F_19 ( & V_17 -> V_7 -> V_113 ) ;
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static int F_33 ( struct V_16 * V_17 )
{
if ( V_17 -> V_7 -> V_118 >= 0x30 ) {
if ( V_17 -> V_56 >= 15000000 ) {
if ( F_18 ( V_17 , V_119 , 0x2b ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_120 , 0x1a ) < 0 )
goto V_53;
} else if ( ( V_17 -> V_56 >= 7000000 ) && ( 15000000 > V_17 -> V_56 ) ) {
if ( F_18 ( V_17 , V_119 , 0x0c ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_120 , 0x1b ) < 0 )
goto V_53;
} else if ( V_17 -> V_56 < 7000000 ) {
if ( F_18 ( V_17 , V_119 , 0x2c ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_120 , 0x1c ) < 0 )
goto V_53;
}
} else {
if ( F_18 ( V_17 , V_119 , 0x1a ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_120 , 0x09 ) < 0 )
goto V_53;
}
return 0 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static int F_34 ( struct V_16 * V_17 )
{
T_3 V_18 ;
switch ( V_17 -> V_82 ) {
case V_86 :
case V_93 :
V_18 = F_16 ( V_17 , V_121 ) ;
F_17 ( V_18 , V_122 , 1 ) ;
F_17 ( V_18 , V_123 , 0 ) ;
if ( F_18 ( V_17 , V_121 , V_18 ) < 0 )
goto V_53;
if ( F_31 ( V_17 , 0 ) < 0 )
goto V_53;
if ( F_33 ( V_17 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_124 , 0x22 ) < 0 )
goto V_53;
if ( F_25 ( V_17 ) < 0 )
goto V_53;
if ( F_27 ( V_17 ) < 0 )
goto V_53;
break;
case V_125 :
V_18 = F_16 ( V_17 , V_121 ) ;
F_17 ( V_18 , V_122 , 0 ) ;
F_17 ( V_18 , V_123 , 0 ) ;
if ( F_18 ( V_17 , V_121 , V_18 ) < 0 )
goto V_53;
F_17 ( V_18 , V_122 , 1 ) ;
F_17 ( V_18 , V_123 , 1 ) ;
if ( F_18 ( V_17 , V_121 , V_18 ) < 0 )
goto V_53;
if ( F_31 ( V_17 , 1 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_119 , 0x1a ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_120 , 0x09 ) < 0 )
goto V_53;
if ( V_17 -> V_7 -> V_118 <= 0x20 ) {
if ( F_18 ( V_17 , V_124 , 0x26 ) < 0 )
goto V_53;
} else {
if ( F_18 ( V_17 , V_124 , 0x66 ) < 0 )
goto V_53;
}
if ( V_17 -> V_126 != V_127 ) {
if ( F_29 ( V_17 ) < 0 )
goto V_53;
} else {
if ( F_30 ( V_17 ) < 0 )
goto V_53;
}
if ( F_26 ( V_17 ) < 0 )
goto V_53;
break;
case V_83 :
default:
V_18 = F_16 ( V_17 , V_121 ) ;
F_17 ( V_18 , V_122 , 0 ) ;
F_17 ( V_18 , V_123 , 0 ) ;
if ( F_18 ( V_17 , V_121 , V_18 ) < 0 )
goto V_53;
F_17 ( V_18 , V_122 , 1 ) ;
F_17 ( V_18 , V_123 , 1 ) ;
if ( F_18 ( V_17 , V_121 , V_18 ) < 0 )
goto V_53;
if ( F_31 ( V_17 , 0 ) < 0 )
goto V_53;
if ( F_33 ( V_17 ) < 0 )
goto V_53;
if ( V_17 -> V_7 -> V_118 <= 0x20 ) {
if ( F_18 ( V_17 , V_124 , 0x26 ) < 0 )
goto V_53;
} else {
if ( F_18 ( V_17 , V_124 , 0x66 ) < 0 )
goto V_53;
}
if ( V_17 -> V_126 != V_127 ) {
if ( F_29 ( V_17 ) < 0 )
goto V_53;
} else {
if ( F_30 ( V_17 ) < 0 )
goto V_53;
}
if ( F_25 ( V_17 ) < 0 )
goto V_53;
if ( F_27 ( V_17 ) < 0 )
goto V_53;
break;
}
return 0 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static int F_35 ( struct V_16 * V_17 )
{
T_3 V_18 , V_128 ;
T_4 V_129 ;
V_18 = F_16 ( V_17 , V_130 ) ;
F_17 ( V_18 , V_131 , 0x1f ) ;
if ( F_18 ( V_17 , V_130 , V_18 ) < 0 )
goto V_53;
if ( V_17 -> V_7 -> V_118 <= 0x20 ) {
if ( V_17 -> V_56 <= 5000000 ) {
if ( F_18 ( V_17 , V_132 , 0x44 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_133 , 0x0f ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_134 , 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_135 , 0xf0 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_136 , 0x00 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_137 , 0x68 ) < 0 )
goto V_53;
} else {
if ( F_18 ( V_17 , V_132 , 0xc4 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_137 , 0x44 ) < 0 )
goto V_53;
}
} else {
if ( V_17 -> V_56 <= 5000000 ) {
F_18 ( V_17 , V_137 , 0x68 ) ;
} else {
F_18 ( V_17 , V_137 , 0x44 ) ;
}
F_18 ( V_17 , V_132 , 0x46 ) ;
if ( V_17 -> V_54 == V_60 ) {
V_128 = 1000 << 16 ;
V_128 /= ( V_17 -> V_7 -> V_62 / 1000 ) ;
V_129 = ( T_4 ) V_128 ;
} else {
V_128 = ( V_17 -> V_138 / 2000 ) + 600 ;
V_128 = V_128 << 16 ;
V_128 /= ( V_17 -> V_7 -> V_62 / 1000 ) ;
V_129 = ( T_4 ) V_128 ;
}
if ( F_18 ( V_17 , V_133 , F_36 ( V_129 ) ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_134 , F_37 ( V_129 ) ) < 0 )
goto V_53;
V_129 *= - 1 ;
if ( F_18 ( V_17 , V_135 , F_36 ( V_129 ) ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_136 , F_37 ( V_129 ) ) < 0 )
goto V_53;
}
if ( F_18 ( V_17 , V_139 , 0 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_140 , 0 ) < 0 )
goto V_53;
if ( V_17 -> V_7 -> V_118 >= 0x20 ) {
if ( F_18 ( V_17 , V_141 , 0x41 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_142 , 0x41 ) < 0 )
goto V_53;
if ( ( V_17 -> V_82 == V_86 ) ||
( V_17 -> V_82 == V_93 ) ||
( V_17 -> V_82 == V_83 ) ) {
if ( F_18 ( V_17 , V_143 , 0x82 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_144 , 0x00 ) < 0 )
goto V_53;
}
}
if ( F_18 ( V_17 , V_145 , 0x00 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_146 , 0xe0 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_147 , 0xc0 ) < 0 )
goto V_53;
V_18 = F_16 ( V_17 , V_121 ) ;
F_17 ( V_18 , V_148 , 0 ) ;
F_17 ( V_18 , V_149 , 0 ) ;
if ( F_18 ( V_17 , V_121 , V_18 ) < 0 )
goto V_53;
V_18 = F_16 ( V_17 , V_150 ) ;
F_17 ( V_18 , V_151 , 0x0 ) ;
if ( F_18 ( V_17 , V_150 , V_18 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_152 , 0x88 ) < 0 )
goto V_53;
if ( V_17 -> V_7 -> V_118 >= 0x20 ) {
if ( V_17 -> V_56 < 2000000 ) {
if ( V_17 -> V_7 -> V_118 <= 0x20 ) {
if ( F_18 ( V_17 , V_153 , 0x39 ) < 0 )
goto V_53;
} else {
if ( F_18 ( V_17 , V_153 , 0x89 ) < 0 )
goto V_53;
}
if ( F_18 ( V_17 , V_154 , 0x40 ) < 0 )
goto V_53;
} else if ( V_17 -> V_56 < 10000000 ) {
if ( F_18 ( V_17 , V_153 , 0x4c ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_154 , 0x20 ) < 0 )
goto V_53;
} else {
if ( F_18 ( V_17 , V_153 , 0x4b ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_154 , 0x20 ) < 0 )
goto V_53;
}
} else {
if ( V_17 -> V_56 < 10000000 ) {
if ( F_18 ( V_17 , V_153 , 0xef ) < 0 )
goto V_53;
} else {
if ( F_18 ( V_17 , V_153 , 0xed ) < 0 )
goto V_53;
}
}
switch ( V_17 -> V_54 ) {
case V_60 :
if ( F_18 ( V_17 , V_130 , 0x1f ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_130 , 0x18 ) < 0 )
goto V_53;
break;
case V_59 :
if ( F_18 ( V_17 , V_130 , 0x1f ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_130 , 0x15 ) < 0 )
goto V_53;
break;
default:
break;
}
return 0 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static int F_38 ( struct V_16 * V_17 )
{
T_3 V_155 = 0xffff , V_156 = 0 , V_157 , V_158 , V_18 ;
T_2 V_43 , V_159 , V_160 , V_161 ;
if ( F_18 ( V_17 , V_162 , 0x38 ) < 0 )
goto V_53;
V_18 = F_16 ( V_17 , V_121 ) ;
F_17 ( V_18 , V_148 , 0 ) ;
F_17 ( V_18 , V_149 , 0 ) ;
if ( F_18 ( V_17 , V_121 , V_18 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_66 , 0x83 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_67 , 0xc0 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_68 , 0x82 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_69 , 0xa0 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_163 , 0x00 ) < 0 )
goto V_53;
if ( F_21 ( V_17 , 1000000 ) < 0 )
goto V_53;
V_160 = V_17 -> V_138 / 1000000 ;
if ( V_160 <= 0 )
V_160 = 1 ;
V_161 = 1 ;
V_158 = ( 1000000 * 256 ) / ( V_17 -> V_7 -> V_62 / 256 ) ;
V_157 = 0 ;
for ( V_43 = 0 ; V_43 < V_160 ; V_43 ++ ) {
if ( V_161 > 0 )
V_157 = V_157 + ( V_158 * V_43 ) ;
else
V_157 = V_157 - ( V_158 * V_43 ) ;
V_161 *= - 1 ;
if ( F_18 ( V_17 , V_130 , 0x5c ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_139 , ( V_157 >> 8 ) & 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_140 , V_157 & 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_130 , 0x58 ) < 0 )
goto V_53;
F_39 ( 10 ) ;
V_156 = 0 ;
for ( V_159 = 0 ; V_159 < 10 ; V_159 ++ ) {
V_156 += ( F_16 ( V_17 , V_164 ) << 8 ) |
F_16 ( V_17 , V_165 ) ;
}
V_156 /= 10 ;
if ( V_156 < V_155 )
V_155 = V_156 ;
}
return V_155 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static T_3 F_40 ( struct V_16 * V_17 , T_3 V_65 )
{
T_1 V_166 , V_167 , V_168 , V_169 ;
T_2 V_56 , V_170 , V_171 , V_172 , V_173 ;
V_166 = F_16 ( V_17 , V_174 ) ;
V_167 = F_16 ( V_17 , V_175 ) ;
V_168 = F_16 ( V_17 , V_176 ) ;
V_169 = F_16 ( V_17 , V_177 ) ;
V_56 = ( ( V_166 << 24 ) | ( V_167 << 16 ) | ( V_168 << 8 ) | V_169 ) ;
V_170 = V_65 >> 16 ;
V_171 = V_56 >> 16 ;
V_172 = V_65 % 0x10000 ;
V_173 = V_56 % 0x10000 ;
V_56 = ( V_170 * V_171 ) +
( ( V_170 * V_173 ) >> 16 ) +
( ( V_171 * V_172 ) >> 16 ) ;
return V_56 ;
}
static T_3 F_41 ( struct V_16 * V_17 )
{
struct V_178 * V_179 = & V_17 -> V_48 ;
int V_180 = 0 , V_43 ;
T_2 V_181 = 0 , V_161 = 1 , V_160 , V_182 = 0 , V_129 ;
T_3 V_183 = 0 , V_156 = 0 , V_184 = 1200 , V_18 ;
T_3 V_185 ;
if ( V_17 -> V_7 -> V_118 >= 0x30 )
V_185 = 0x2e00 ;
else
V_185 = 0x1f00 ;
V_18 = F_16 ( V_17 , V_130 ) ;
F_17 ( V_18 , V_131 , 0x1f ) ;
if ( F_18 ( V_17 , V_130 , V_18 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_186 , 0x12 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_187 , 0xc0 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_146 , 0xf0 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_147 , 0xe0 ) < 0 )
goto V_53;
V_18 = F_16 ( V_17 , V_121 ) ;
F_17 ( V_18 , V_148 , 1 ) ;
F_17 ( V_18 , V_149 , 0 ) ;
if ( F_18 ( V_17 , V_121 , V_18 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_66 , 0x83 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_67 , 0xc0 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_68 , 0x82 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_69 , 0xa0 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_163 , 0x00 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_162 , 0x50 ) < 0 )
goto V_53;
if ( V_17 -> V_7 -> V_118 >= 0x30 ) {
if ( F_18 ( V_17 , V_153 , 0x99 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_145 , 0x98 ) < 0 )
goto V_53;
} else if ( V_17 -> V_7 -> V_118 >= 0x20 ) {
if ( F_18 ( V_17 , V_153 , 0x6a ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_145 , 0x95 ) < 0 )
goto V_53;
}
if ( V_17 -> V_56 <= 2000000 )
V_184 = 1000 ;
else if ( V_17 -> V_56 <= 5000000 )
V_184 = 2000 ;
else if ( V_17 -> V_56 <= 12000000 )
V_184 = 3000 ;
else
V_184 = 5000 ;
V_160 = - 1 + ( ( V_17 -> V_138 / 1000 ) / V_184 ) ;
V_160 /= 2 ;
V_160 = ( 2 * V_160 ) + 1 ;
if ( V_160 < 0 )
V_160 = 1 ;
else if ( V_160 > 10 ) {
V_160 = 11 ;
V_184 = ( V_17 -> V_138 / 1000 ) / 10 ;
}
V_182 = 0 ;
V_161 = 1 ;
V_129 = V_17 -> V_188 ;
while ( ( ! V_180 ) && ( V_182 < V_160 ) ) {
if ( F_18 ( V_17 , V_130 , 0x5f ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_139 , 0x00 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_140 , 0x00 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_63 , 0x00 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_64 , 0x00 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_130 , 0x40 ) < 0 )
goto V_53;
F_39 ( 50 ) ;
for ( V_43 = 0 ; V_43 < 10 ; V_43 ++ ) {
V_18 = F_16 ( V_17 , V_189 ) ;
if ( F_42 ( V_18 , V_190 ) >= 2 )
V_181 ++ ;
V_156 += ( F_16 ( V_17 , V_164 ) << 8 ) |
F_16 ( V_17 , V_165 ) ;
}
V_156 /= 10 ;
V_183 = F_40 ( V_17 , V_17 -> V_7 -> V_62 ) ;
V_182 ++ ;
V_161 *= - 1 ;
if ( ( V_181 >= 5 ) && ( V_156 < V_185 ) &&
( V_183 < 50000000 ) && ( V_183 > 850000 ) )
V_180 = 1 ;
else if ( V_182 < V_160 ) {
if ( V_161 > 0 )
V_129 += V_182 * V_184 ;
else
V_129 -= V_182 * V_184 ;
if ( F_14 ( V_17 , 1 ) < 0 )
goto V_53;
if ( V_17 -> V_20 -> V_191 ) {
if ( V_17 -> V_20 -> V_191 ( V_179 , V_129 ) < 0 )
goto V_192;
}
if ( V_17 -> V_20 -> V_193 ) {
if ( V_17 -> V_20 -> V_193 ( V_179 , V_17 -> V_194 ) < 0 )
goto V_192;
}
if ( F_14 ( V_17 , 0 ) < 0 )
goto V_53;
F_39 ( 50 ) ;
if ( F_14 ( V_17 , 1 ) < 0 )
goto V_53;
if ( V_17 -> V_20 -> V_195 ) {
if ( V_17 -> V_20 -> V_195 ( V_179 , & V_18 ) < 0 )
goto V_192;
}
if ( V_18 )
F_9 ( V_51 , 1 , L_14 ) ;
else
F_9 ( V_51 , 1 , L_15 ) ;
if ( F_14 ( V_17 , 0 ) < 0 )
goto V_53;
}
}
if ( ! V_180 )
V_183 = 0 ;
else
V_183 = F_40 ( V_17 , V_17 -> V_7 -> V_62 ) ;
return V_183 ;
V_192:
F_14 ( V_17 , 0 ) ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static T_3 F_43 ( struct V_16 * V_17 )
{
T_3 V_183 , V_196 , V_61 , V_18 ;
V_183 = F_40 ( V_17 , V_17 -> V_7 -> V_62 ) ;
V_196 = F_16 ( V_17 , V_197 ) << 8 ;
V_196 |= F_16 ( V_17 , V_198 ) ;
V_61 = 13 * ( V_183 / 10 ) ;
if ( V_61 < V_17 -> V_56 )
V_183 = 0 ;
else {
if ( F_18 ( V_17 , V_130 , 0x1f ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_187 , 0xc1 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_146 , 0x20 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_147 , 0x00 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_186 , 0xd2 ) < 0 )
goto V_53;
V_18 = F_16 ( V_17 , V_121 ) ;
F_17 ( V_18 , V_149 , 0x00 ) ;
if ( F_18 ( V_17 , V_121 , V_18 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_162 , 0x38 ) < 0 )
goto V_53;
if ( V_17 -> V_7 -> V_118 >= 0x30 ) {
if ( F_18 ( V_17 , V_153 , 0x79 ) < 0 )
goto V_53;
} else if ( V_17 -> V_7 -> V_118 >= 0x20 ) {
if ( F_18 ( V_17 , V_153 , 0x49 ) < 0 )
goto V_53;
}
if ( V_183 > 3000000 ) {
V_61 = 13 * ( V_183 / 10 ) ;
V_61 = ( V_61 / 1000 ) * 65536 ;
V_61 /= ( V_17 -> V_7 -> V_62 / 1000 ) ;
if ( F_18 ( V_17 , V_66 , ( V_61 >> 8 ) & 0x7f ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_67 , V_61 & 0xff ) < 0 )
goto V_53;
V_61 = 10 * ( V_183 / 13 ) ;
V_61 = ( V_61 / 1000 ) * 65536 ;
V_61 /= ( V_17 -> V_7 -> V_62 / 1000 ) ;
if ( F_18 ( V_17 , V_68 , ( V_61 >> 8 ) & 0x7f ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_69 , V_61 & 0xff ) < 0 )
goto V_53;
V_61 = ( V_183 / 1000 ) * 65536 ;
V_61 /= ( V_17 -> V_7 -> V_62 / 1000 ) ;
if ( F_18 ( V_17 , V_63 , ( V_61 >> 8 ) & 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_64 , V_61 & 0xff ) < 0 )
goto V_53;
} else {
V_61 = 13 * ( V_183 / 10 ) ;
V_61 = ( V_61 / 100 ) * 65536 ;
V_61 /= ( V_17 -> V_7 -> V_62 / 100 ) ;
if ( F_18 ( V_17 , V_66 , ( V_61 >> 8 ) & 0x7f ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_67 , V_61 & 0xff ) < 0 )
goto V_53;
V_61 = 10 * ( V_183 / 14 ) ;
V_61 = ( V_61 / 100 ) * 65536 ;
V_61 /= ( V_17 -> V_7 -> V_62 / 100 ) ;
if ( F_18 ( V_17 , V_68 , ( V_61 >> 8 ) & 0x7f ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_69 , V_61 & 0xff ) < 0 )
goto V_53;
V_61 = ( V_183 / 100 ) * 65536 ;
V_61 /= ( V_17 -> V_7 -> V_62 / 100 ) ;
if ( F_18 ( V_17 , V_63 , ( V_61 >> 8 ) & 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_64 , V_61 & 0xff ) < 0 )
goto V_53;
}
if ( F_18 ( V_17 , V_163 , 0x20 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_139 , ( V_196 >> 8 ) & 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_140 , V_196 & 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_130 , 0x15 ) < 0 )
goto V_53;
}
return V_183 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static int F_44 ( struct V_16 * V_17 , T_2 V_199 )
{
T_2 V_200 = 0 , V_201 = 0 ;
T_3 V_18 ;
T_1 V_202 ;
while ( ( V_200 < V_199 ) && ( ! V_201 ) ) {
V_18 = F_16 ( V_17 , V_203 ) ;
V_202 = F_42 ( V_18 , V_204 ) ;
switch ( V_202 ) {
case 0 :
case 1 :
default:
F_9 ( V_51 , 1 , L_16 ) ;
V_201 = 0 ;
break;
case 2 :
case 3 :
V_18 = F_16 ( V_17 , V_189 ) ;
V_201 = F_42 ( V_18 , V_205 ) ;
break;
}
if ( ! V_201 )
F_39 ( 10 ) ;
else
F_9 ( V_51 , 1 , L_17 ) ;
V_200 += 10 ;
}
return V_201 ;
}
static int F_45 ( struct V_16 * V_17 )
{
T_3 V_156 , V_18 , V_183 ;
T_2 V_206 , V_207 , V_43 ;
T_1 V_208 , V_209 , V_210 ;
int V_211 = 0 ;
int V_201 ;
V_209 = 110 ;
V_210 = 10 ;
V_156 = F_38 ( V_17 ) ;
if ( V_156 > F_46 ( V_17 -> V_7 -> V_118 ) ) {
V_201 = 0 ;
} else {
if ( V_17 -> V_7 -> V_118 <= 0x20 ) {
if ( F_18 ( V_17 , V_132 , 0xc4 ) < 0 )
goto V_53;
} else {
if ( F_18 ( V_17 , V_132 , 0x06 ) < 0 )
goto V_53;
}
if ( F_18 ( V_17 , V_137 , 0x44 ) < 0 )
goto V_53;
if ( V_17 -> V_7 -> V_118 >= 0x20 ) {
if ( F_18 ( V_17 , V_141 , 0x41 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_142 , 0x41 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_143 , 0x82 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_144 , 0x00 ) < 0 )
goto V_53;
}
V_208 = V_209 ;
do {
if ( F_18 ( V_17 , V_212 , V_208 ) < 0 )
goto V_53;
if ( F_41 ( V_17 ) != 0 ) {
V_183 = F_43 ( V_17 ) ;
if ( V_183 != 0 ) {
F_20 ( V_17 ) ;
V_201 = F_44 ( V_17 ,
V_17 -> V_57 ) ;
} else {
V_201 = 0 ;
}
} else {
V_206 = 0 ;
V_207 = 0 ;
for ( V_43 = 0 ; V_43 < 10 ; V_43 ++ ) {
V_156 += ( F_16 ( V_17 , V_164 ) << 8 ) |
F_16 ( V_17 , V_165 ) ;
if ( V_156 >= 0xff00 )
V_207 ++ ;
V_18 = F_16 ( V_17 , V_213 ) ;
if ( ( F_42 ( V_18 , V_214 ) == 0x01 ) &&
( F_42 ( V_18 , V_215 ) == 0x01 ) )
V_206 ++ ;
}
if ( ( V_206 > 7 ) || ( V_207 > 7 ) )
V_211 = 1 ;
V_201 = 0 ;
}
V_208 -= 20 ;
} while ( ( V_208 >= V_210 ) && ( ! V_201 ) && ( ! V_211 ) );
}
return V_201 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static int F_47 ( struct V_16 * V_17 )
{
T_3 V_18 ;
T_2 V_181 = 0 , V_43 ;
T_1 V_129 , V_216 , V_217 ;
int V_180 = 0 ;
V_129 = F_16 ( V_17 , V_153 ) ;
V_216 = F_16 ( V_17 , V_146 ) ;
V_217 = F_16 ( V_17 , V_147 ) ;
if ( F_18 ( V_17 , V_146 , 0x20 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_147 , 0x00 ) < 0 )
goto V_53;
V_18 = F_16 ( V_17 , V_121 ) ;
F_17 ( V_18 , V_149 , 0x00 ) ;
if ( F_18 ( V_17 , V_121 , V_18 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_152 , 0x80 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_137 , 0x40 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_153 , 0x00 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_139 , 0x00 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_140 , 0x00 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_162 , 0x65 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_130 , 0x18 ) < 0 )
goto V_53;
F_39 ( 10 ) ;
for ( V_43 = 0 ; V_43 < 10 ; V_43 ++ ) {
V_18 = F_16 ( V_17 , V_189 ) ;
if ( F_42 ( V_18 , V_190 ) >= 2 )
V_181 ++ ;
F_39 ( 1 ) ;
}
if ( V_181 >= 3 )
V_180 = 1 ;
if ( F_18 ( V_17 , V_162 , 0x38 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_152 , 0x88 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_137 , 0x68 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_153 , V_129 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_146 , V_216 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_147 , V_217 ) < 0 )
goto V_53;
return V_180 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static int F_48 ( struct V_16 * V_17 , T_2 V_218 )
{
struct V_178 * V_179 = & V_17 -> V_48 ;
T_3 V_18 ;
T_2 V_184 , V_160 , V_182 , V_161 , V_129 , V_219 ;
int V_201 = 0 ;
if ( V_17 -> V_56 >= 10000000 )
V_219 = V_218 / 3 ;
else
V_219 = V_218 / 2 ;
V_201 = F_44 ( V_17 , V_219 ) ;
if ( ! V_201 ) {
if ( V_17 -> V_56 >= 10000000 ) {
if ( F_47 ( V_17 ) ) {
if ( F_18 ( V_17 , V_130 , 0x1f ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_130 , 0x15 ) < 0 )
goto V_53;
V_201 = F_44 ( V_17 , V_218 ) ;
} else {
V_201 = 0 ;
}
} else {
if ( V_17 -> V_56 <= 4000000 )
V_184 = 1000 ;
else if ( V_17 -> V_56 <= 7000000 )
V_184 = 2000 ;
else if ( V_17 -> V_56 <= 10000000 )
V_184 = 3000 ;
else
V_184 = 5000 ;
V_160 = ( V_17 -> V_138 / 1000 ) / V_184 ;
V_160 /= 2 ;
V_160 = 2 * ( V_160 + 1 ) ;
if ( V_160 < 0 )
V_160 = 2 ;
else if ( V_160 > 12 )
V_160 = 12 ;
V_182 = 1 ;
V_161 = 1 ;
if ( ! V_201 ) {
V_129 = V_17 -> V_188 ;
V_17 -> V_194 = F_24 ( V_17 -> V_56 , V_17 -> V_71 ) + V_17 -> V_56 ;
while ( ( V_182 <= V_160 ) && ( ! V_201 ) ) {
if ( V_161 > 0 )
V_129 += V_182 * V_184 ;
else
V_129 -= V_182 * V_184 ;
if ( F_14 ( V_17 , 1 ) < 0 )
goto V_53;
if ( V_17 -> V_20 -> V_191 ) {
if ( V_17 -> V_20 -> V_191 ( V_179 , V_129 ) < 0 )
goto V_192;
}
if ( V_17 -> V_20 -> V_193 ) {
if ( V_17 -> V_20 -> V_193 ( V_179 , V_17 -> V_194 ) < 0 )
goto V_192;
}
if ( F_14 ( V_17 , 0 ) < 0 )
goto V_53;
F_39 ( 50 ) ;
if ( F_14 ( V_17 , 1 ) < 0 )
goto V_53;
if ( V_17 -> V_20 -> V_195 ) {
if ( V_17 -> V_20 -> V_195 ( V_179 , & V_18 ) < 0 )
goto V_192;
}
if ( V_18 )
F_9 ( V_51 , 1 , L_14 ) ;
else
F_9 ( V_51 , 1 , L_15 ) ;
if ( F_14 ( V_17 , 0 ) < 0 )
goto V_53;
F_18 ( V_17 , V_130 , 0x1c ) ;
if ( F_18 ( V_17 , V_139 , 0x00 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_140 , 0x00 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_130 , 0x1f ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_130 , 0x15 ) < 0 )
goto V_53;
V_201 = F_44 ( V_17 , ( V_218 / 3 ) ) ;
V_161 *= - 1 ;
V_182 ++ ;
}
}
}
}
return V_201 ;
V_192:
F_14 ( V_17 , 0 ) ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static int F_49 ( struct V_16 * V_17 , T_2 * V_220 , T_2 * V_221 , T_2 * V_160 )
{
T_2 V_199 , V_222 , V_223 , V_56 , V_224 ;
V_56 = V_17 -> V_56 ;
V_224 = V_17 -> V_138 / 1000 ;
V_224 += V_224 / 10 ;
V_224 = 65536 * ( V_224 / 2 ) ;
V_224 /= ( V_17 -> V_7 -> V_62 / 1000 ) ;
if ( V_224 > 0x4000 )
V_224 = 0x4000 ;
V_222 = V_56 ;
V_222 /= V_17 -> V_7 -> V_62 / 1000 ;
V_222 *= 256 ;
V_222 *= 256 ;
V_222 /= 1000 ;
switch ( V_17 -> V_82 ) {
case V_86 :
case V_93 :
V_222 *= 3 ;
V_199 = 20 ;
break;
case V_125 :
V_222 *= 4 ;
V_199 = 25 ;
break;
case V_83 :
default:
V_222 *= 3 ;
V_199 = 25 ;
break;
}
V_222 /= 100 ;
if ( ( V_222 > V_224 ) || ( V_222 < 0 ) )
V_222 = V_224 / 2 ;
V_199 *= 27500 ;
if ( V_56 > 0 )
V_199 /= ( V_56 / 1000 ) ;
if ( ( V_199 > 100 ) || ( V_199 < 0 ) )
V_199 = 100 ;
V_223 = ( V_224 / V_222 ) + 1 ;
if ( ( V_223 > 100 ) || ( V_223 < 0 ) ) {
V_223 = 100 ;
V_222 = V_224 / V_223 ;
}
* V_220 = V_222 ;
* V_221 = V_199 ;
* V_160 = V_223 ;
return 0 ;
}
static int F_50 ( struct V_16 * V_17 )
{
T_2 V_225 , V_156 , V_224 ;
int V_226 ;
V_225 = F_16 ( V_17 , V_197 ) << 8 ;
V_225 |= F_16 ( V_17 , V_198 ) ;
V_225 = F_6 ( V_225 , 16 ) ;
V_156 = F_16 ( V_17 , V_164 ) << 8 ;
V_156 |= F_16 ( V_17 , V_165 ) ;
V_224 = V_17 -> V_138 / 1000 ;
V_224 += ( V_224 / 10 ) ;
V_224 = ( 65536 * V_224 / 2 ) ;
V_224 /= V_17 -> V_7 -> V_62 / 1000 ;
if ( V_224 > 0x4000 )
V_224 = 0x4000 ;
if ( ( V_156 > 0x2000 ) || ( V_225 > 2 * V_224 ) || ( V_225 < - 2 * V_224 ) ) {
V_226 = 1 ;
F_9 ( V_51 , 1 , L_18 ) ;
} else {
V_226 = 0 ;
F_9 ( V_51 , 1 , L_19 ) ;
}
return V_226 ;
}
static int F_51 ( struct V_16 * V_17 , T_2 V_222 , T_2 V_199 , int V_227 , T_2 V_223 )
{
int V_226 , V_201 = 0 ;
T_2 V_228 = 0 , V_229 , V_224 ;
T_3 V_18 ;
V_224 = V_17 -> V_138 / 1000 ;
V_224 += ( V_224 / 10 ) ;
V_224 = ( 65536 * V_224 / 2 ) ;
V_224 /= ( V_17 -> V_7 -> V_62 / 1000 ) ;
if ( V_224 > 0x4000 )
V_224 = 0x4000 ;
if ( V_227 )
V_229 = 0 ;
else
V_229 = - V_224 + V_222 ;
do {
if ( F_18 ( V_17 , V_130 , 0x1c ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_139 , ( ( V_229 / 256 ) & 0xff ) ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_140 , V_229 & 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_130 , 0x18 ) < 0 )
goto V_53;
V_18 = F_16 ( V_17 , V_230 ) ;
F_17 ( V_18 , V_231 , 0x1 ) ;
if ( F_18 ( V_17 , V_230 , V_18 ) < 0 )
goto V_53;
if ( V_227 ) {
if ( V_229 >= 0 )
V_229 = - V_229 - 2 * V_222 ;
else
V_229 = - V_229 ;
} else {
V_229 += 2 * V_222 ;
}
V_228 ++ ;
V_201 = F_44 ( V_17 , V_199 ) ;
V_226 = F_50 ( V_17 ) ;
} while ( ( ! V_201 ) &&
( ! V_226 ) &&
( ( V_229 - V_222 ) < V_224 ) &&
( ( V_229 + V_222 ) > - V_224 ) &&
( V_228 < V_223 ) );
V_18 = F_16 ( V_17 , V_230 ) ;
F_17 ( V_18 , V_231 , 0 ) ;
if ( F_18 ( V_17 , V_230 , V_18 ) < 0 )
goto V_53;
return V_201 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static int F_52 ( struct V_16 * V_17 )
{
int V_226 , V_227 , V_201 = 0 ;
T_3 V_18 ;
T_2 V_232 ;
T_2 V_222 , V_233 , V_234 , V_223 ;
F_49 ( V_17 , & V_222 , & V_233 , & V_223 ) ;
switch ( V_17 -> V_82 ) {
case V_86 :
case V_93 :
if ( V_17 -> V_7 -> V_118 >= 0x20 ) {
if ( F_18 ( V_17 , V_153 , 0x3B ) < 0 )
goto V_53;
}
if ( F_18 ( V_17 , V_121 , 0x49 ) < 0 )
goto V_53;
V_227 = 0 ;
break;
case V_125 :
if ( V_17 -> V_7 -> V_118 >= 0x20 ) {
if ( F_18 ( V_17 , V_235 , 0x79 ) < 0 )
goto V_53;
}
if ( F_18 ( V_17 , V_121 , 0x89 ) < 0 )
goto V_53;
V_227 = 1 ;
break;
case V_83 :
default:
if ( V_17 -> V_7 -> V_118 >= 0x20 ) {
if ( F_18 ( V_17 , V_153 , 0x3b ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_235 , 0x79 ) < 0 )
goto V_53;
}
if ( F_18 ( V_17 , V_121 , 0xc9 ) < 0 )
goto V_53;
V_227 = 0 ;
break;
}
V_234 = 0 ;
do {
V_201 = F_51 ( V_17 , V_222 , V_233 , V_227 , V_223 ) ;
V_226 = F_50 ( V_17 ) ;
V_234 ++ ;
if ( V_201 || V_226 || ( V_234 == 2 ) ) {
if ( V_17 -> V_7 -> V_118 >= 0x20 ) {
if ( F_18 ( V_17 , V_153 , 0x49 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_235 , 0x9e ) < 0 )
goto V_53;
}
V_18 = F_16 ( V_17 , V_203 ) ;
if ( ( V_201 ) && ( F_42 ( V_18 , V_204 ) == V_236 ) ) {
F_39 ( V_233 ) ;
V_18 = F_16 ( V_17 , V_237 ) ;
V_232 = F_42 ( V_18 , V_238 ) ;
if ( V_232 < 0xd ) {
F_39 ( V_233 ) ;
V_18 = F_16 ( V_17 , V_237 ) ;
V_232 = F_42 ( V_18 , V_238 ) ;
}
if ( V_232 < 0xd ) {
V_201 = 0 ;
if ( V_234 < 2 ) {
if ( V_17 -> V_7 -> V_118 >= 0x20 ) {
if ( F_18 ( V_17 , V_235 , 0x79 ) < 0 )
goto V_53;
}
if ( F_18 ( V_17 , V_121 , 0x89 ) < 0 )
goto V_53;
}
}
}
}
} while ( ( ! V_201 ) && ( V_234 < 2 ) && ( ! V_226 ) );
return V_201 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static enum V_239 F_53 ( struct V_16 * V_17 )
{
T_3 V_18 ;
enum V_239 V_240 ;
V_18 = F_16 ( V_17 , V_203 ) ;
if ( F_42 ( V_18 , V_204 ) == 2 )
V_240 = V_236 ;
else if ( F_42 ( V_18 , V_204 ) == 3 ) {
V_18 = F_16 ( V_17 , V_84 ) ;
if ( F_42 ( V_18 , V_241 ) == 1 )
V_240 = V_242 ;
else
V_240 = V_243 ;
} else {
V_240 = V_244 ;
}
return V_240 ;
}
static T_2 F_54 ( struct V_16 * V_17 , T_3 V_62 )
{
T_2 V_245 , V_170 , V_171 , V_172 , V_173 ;
V_245 = F_16 ( V_17 , V_197 ) << 16 ;
V_245 |= F_16 ( V_17 , V_198 ) << 8 ;
V_245 |= F_16 ( V_17 , V_246 ) ;
V_245 = F_6 ( V_245 , 24 ) ;
V_170 = V_62 >> 12 ;
V_171 = V_245 >> 12 ;
V_172 = V_62 % 0x1000 ;
V_173 = V_245 % 0x1000 ;
V_245 = ( V_170 * V_171 ) +
( ( V_170 * V_173 ) >> 12 ) +
( ( V_171 * V_172 ) >> 12 ) ;
return V_245 ;
}
static int F_55 ( struct V_16 * V_17 )
{
T_3 V_18 , V_247 ;
V_18 = F_16 ( V_17 , V_248 ) ;
V_247 = F_42 ( V_18 , V_249 ) ;
switch ( V_247 ) {
case 13 :
V_17 -> V_87 = V_88 ;
break;
case 18 :
V_17 -> V_87 = V_89 ;
break;
case 21 :
V_17 -> V_87 = V_90 ;
break;
case 24 :
V_17 -> V_87 = V_91 ;
break;
case 25 :
V_17 -> V_87 = V_94 ;
break;
case 26 :
V_17 -> V_87 = V_92 ;
break;
default:
V_17 -> V_87 = V_250 ;
break;
}
return 0 ;
}
static enum V_251 F_56 ( struct V_16 * V_17 )
{
struct V_178 * V_179 = & V_17 -> V_48 ;
T_1 V_252 ;
T_3 V_18 ;
T_2 V_43 = 0 , V_229 ;
F_39 ( 5 ) ;
if ( V_17 -> V_54 == V_55 ) {
V_252 = F_16 ( V_17 , V_253 ) ;
F_18 ( V_17 , V_145 , 0x5c ) ;
while ( ( V_43 <= 50 ) && ( V_252 != 0 ) && ( V_252 != 0xff ) ) {
V_252 = F_16 ( V_17 , V_253 ) ;
F_39 ( 5 ) ;
V_43 += 5 ;
}
}
V_17 -> V_240 = F_53 ( V_17 ) ;
if ( F_14 ( V_17 , 1 ) < 0 )
goto V_53;
if ( V_17 -> V_20 -> V_254 ) {
if ( V_17 -> V_20 -> V_254 ( V_179 , & V_17 -> V_188 ) < 0 )
goto V_192;
}
if ( F_14 ( V_17 , 0 ) < 0 )
goto V_53;
V_229 = F_54 ( V_17 , V_17 -> V_7 -> V_62 ) / 1000 ;
V_17 -> V_188 += V_229 ;
if ( F_55 ( V_17 ) < 0 )
goto V_53;
V_18 = F_16 ( V_17 , V_255 ) ;
V_17 -> V_256 = F_42 ( V_18 , V_257 ) ;
V_17 -> V_258 = F_42 ( V_18 , V_259 ) & 0x01 ;
V_17 -> V_260 = F_42 ( V_18 , V_259 ) >> 1 ;
V_18 = F_16 ( V_17 , V_261 ) ;
V_17 -> V_71 = F_42 ( V_18 , V_262 ) ;
V_18 = F_16 ( V_17 , V_84 ) ;
V_17 -> V_263 = F_42 ( V_18 , V_264 ) ;
if ( ( V_17 -> V_54 == V_55 ) || ( V_17 -> V_56 < 10000000 ) ) {
if ( F_14 ( V_17 , 1 ) < 0 )
goto V_53;
if ( V_17 -> V_20 -> V_254 ) {
if ( V_17 -> V_20 -> V_254 ( V_179 , & V_17 -> V_188 ) < 0 )
goto V_192;
}
if ( F_14 ( V_17 , 0 ) < 0 )
goto V_53;
if ( abs ( V_229 ) <= ( ( V_17 -> V_138 / 2000 ) + 500 ) )
return V_265 ;
else if ( abs ( V_229 ) <= ( F_24 ( V_17 -> V_56 , V_17 -> V_71 ) / 2000 ) )
return V_265 ;
else
return V_266 ;
} else {
if ( abs ( V_229 ) <= ( ( V_17 -> V_138 / 2000 ) + 500 ) )
return V_265 ;
else
return V_266 ;
}
return V_266 ;
V_192:
F_14 ( V_17 , 0 ) ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static T_3 F_57 ( struct V_16 * V_17 , T_3 V_56 )
{
T_2 V_267 ;
V_267 = F_16 ( V_17 , V_253 ) << 16 ;
V_267 |= F_16 ( V_17 , V_268 ) << 8 ;
V_267 |= F_16 ( V_17 , V_269 ) ;
V_267 = F_6 ( V_267 , 24 ) ;
if ( ! V_267 )
V_267 = 1 ;
V_267 = ( ( T_2 ) V_56 * 10 ) / ( ( T_2 ) 0x1000000 / V_267 ) ;
V_267 /= 320 ;
return V_267 ;
}
static T_1 F_58 ( struct V_16 * V_17 , enum V_270 V_256 , T_2 V_258 )
{
T_1 V_271 = 0x29 ;
T_2 V_43 ;
struct V_272 * V_273 , * V_274 , * V_275 ;
if ( V_17 -> V_7 -> V_118 == 0x20 ) {
V_273 = V_276 ;
V_274 = V_277 ;
V_275 = V_278 ;
} else {
V_273 = V_279 ;
V_274 = V_280 ;
V_275 = V_281 ;
}
if ( V_256 < V_282 ) {
V_43 = 0 ;
while ( ( V_43 < 3 ) && ( V_256 != V_274 [ V_43 ] . V_256 ) )
V_43 ++ ;
if ( V_43 >= 3 )
V_43 = 2 ;
} else {
V_43 = 0 ;
while ( ( V_43 < 14 ) && ( V_256 != V_273 [ V_43 ] . V_256 ) )
V_43 ++ ;
if ( V_43 >= 14 ) {
V_43 = 0 ;
while ( ( V_43 < 11 ) && ( V_256 != V_275 [ V_43 ] . V_256 ) )
V_43 ++ ;
if ( V_43 >= 11 )
V_43 = 10 ;
}
}
if ( V_256 <= V_283 ) {
if ( V_258 ) {
if ( V_17 -> V_56 <= 3000000 )
V_271 = V_274 [ V_43 ] . V_284 ;
else if ( V_17 -> V_56 <= 7000000 )
V_271 = V_274 [ V_43 ] . V_285 ;
else if ( V_17 -> V_56 <= 15000000 )
V_271 = V_274 [ V_43 ] . V_286 ;
else if ( V_17 -> V_56 <= 25000000 )
V_271 = V_274 [ V_43 ] . V_287 ;
else
V_271 = V_274 [ V_43 ] . V_288 ;
} else {
if ( V_17 -> V_56 <= 3000000 )
V_271 = V_274 [ V_43 ] . V_289 ;
else if ( V_17 -> V_56 <= 7000000 )
V_271 = V_274 [ V_43 ] . V_290 ;
else if ( V_17 -> V_56 <= 15000000 )
V_271 = V_274 [ V_43 ] . V_291 ;
else if ( V_17 -> V_56 <= 25000000 )
V_271 = V_274 [ V_43 ] . V_292 ;
else
V_271 = V_274 [ V_43 ] . V_293 ;
}
} else if ( V_256 <= V_294 ) {
if ( V_258 ) {
if ( V_17 -> V_56 <= 3000000 )
V_271 = V_273 [ V_43 ] . V_284 ;
else if ( V_17 -> V_56 <= 7000000 )
V_271 = V_273 [ V_43 ] . V_285 ;
else if ( V_17 -> V_56 <= 15000000 )
V_271 = V_273 [ V_43 ] . V_286 ;
else if ( V_17 -> V_56 <= 25000000 )
V_271 = V_273 [ V_43 ] . V_287 ;
else
V_271 = V_273 [ V_43 ] . V_288 ;
} else {
if ( V_17 -> V_56 <= 3000000 )
V_271 = V_273 [ V_43 ] . V_289 ;
else if ( V_17 -> V_56 <= 7000000 )
V_271 = V_273 [ V_43 ] . V_290 ;
else if ( V_17 -> V_56 <= 15000000 )
V_271 = V_273 [ V_43 ] . V_291 ;
else if ( V_17 -> V_56 <= 25000000 )
V_271 = V_273 [ V_43 ] . V_292 ;
else
V_271 = V_273 [ V_43 ] . V_293 ;
}
} else {
if ( V_17 -> V_56 <= 3000000 )
V_271 = V_275 [ V_43 ] . V_284 ;
else if ( V_17 -> V_56 <= 7000000 )
V_271 = V_275 [ V_43 ] . V_285 ;
else if ( V_17 -> V_56 <= 15000000 )
V_271 = V_275 [ V_43 ] . V_286 ;
else if ( V_17 -> V_56 <= 25000000 )
V_271 = V_275 [ V_43 ] . V_287 ;
else
V_271 = V_275 [ V_43 ] . V_288 ;
}
return V_271 ;
}
static T_1 F_59 ( struct V_16 * V_17 )
{
struct V_295 * V_296 = NULL ;
T_2 V_297 = 0 ;
T_1 V_271 = 0x0b ;
switch ( V_17 -> V_298 ) {
case V_299 :
default:
V_297 = 0 ;
break;
case V_300 :
V_297 = 1 ;
break;
case V_301 :
V_297 = 2 ;
break;
case V_302 :
V_297 = 3 ;
break;
}
if ( V_17 -> V_7 -> V_118 >= 0x30 ) {
V_296 = V_303 ;
} else {
V_296 = V_304 ;
}
if ( V_17 -> V_56 <= 3000000 )
V_271 = V_296 [ V_297 ] . V_305 ;
else if ( V_17 -> V_56 <= 7000000 )
V_271 = V_296 [ V_297 ] . V_306 ;
else if ( V_17 -> V_56 <= 15000000 )
V_271 = V_296 [ V_297 ] . V_307 ;
else if ( V_17 -> V_56 <= 25000000 )
V_271 = V_296 [ V_297 ] . V_308 ;
else
V_271 = V_296 [ V_297 ] . V_309 ;
return V_271 ;
}
static int F_60 ( struct V_16 * V_17 )
{
struct V_178 * V_179 = & V_17 -> V_48 ;
enum V_270 V_256 ;
T_2 V_56 , V_258 , V_271 , V_310 , V_311 , V_43 = 0 , V_312 = 0 ;
T_3 V_18 ;
V_56 = F_40 ( V_17 , V_17 -> V_7 -> V_62 ) ;
V_56 += F_57 ( V_17 , V_56 ) ;
switch ( V_17 -> V_240 ) {
case V_243 :
case V_242 :
if ( V_17 -> V_82 == V_83 ) {
V_18 = F_16 ( V_17 , V_121 ) ;
F_17 ( V_18 , V_122 , 1 ) ;
F_17 ( V_18 , V_123 , 0 ) ;
if ( F_18 ( V_17 , V_121 , V_18 ) < 0 )
goto V_53;
}
V_18 = F_16 ( V_17 , V_313 ) ;
F_17 ( V_18 , V_314 , V_17 -> V_71 ) ;
F_17 ( V_18 , V_315 , 0x01 ) ;
if ( F_18 ( V_17 , V_313 , V_18 ) < 0 )
goto V_53;
if ( V_17 -> V_7 -> V_118 >= 0x30 ) {
if ( F_55 ( V_17 ) < 0 )
goto V_53;
if ( V_17 -> V_87 == V_88 ) {
if ( F_18 ( V_17 , V_316 , 0x98 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_317 , 0x18 ) < 0 )
goto V_53;
} else {
if ( F_18 ( V_17 , V_316 , 0x18 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_317 , 0x18 ) < 0 )
goto V_53;
}
}
if ( F_18 ( V_17 , V_318 , 0x75 ) < 0 )
goto V_53;
break;
case V_236 :
V_18 = F_16 ( V_17 , V_121 ) ;
F_17 ( V_18 , V_122 , 0 ) ;
F_17 ( V_18 , V_123 , 1 ) ;
if ( F_18 ( V_17 , V_121 , V_18 ) < 0 )
goto V_53;
if ( V_17 -> V_7 -> V_118 >= 0x30 ) {
if ( F_18 ( V_17 , V_119 , 0 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_120 , 0 ) < 0 )
goto V_53;
}
if ( V_17 -> V_260 == V_319 ) {
V_18 = F_16 ( V_17 , V_255 ) ;
V_256 = F_42 ( V_18 , V_257 ) ;
V_258 = F_42 ( V_18 , V_259 ) & 0x01 ;
V_271 = F_58 ( V_17 , V_256 , V_258 ) ;
if ( V_256 <= V_320 ) {
F_18 ( V_17 , V_321 , V_271 ) ;
} else if ( V_256 <= V_294 ) {
if ( F_18 ( V_17 , V_321 , 0x2a ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_322 , V_271 ) < 0 )
goto V_53;
}
if ( ( V_17 -> V_126 == V_127 ) && ( V_256 > V_294 ) ) {
if ( V_256 <= V_323 ) {
if ( F_18 ( V_17 , V_321 , 0x2a ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_324 , V_271 ) < 0 )
goto V_53;
} else {
if ( F_18 ( V_17 , V_321 , 0x2a ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_325 , V_271 ) < 0 )
goto V_53;
}
}
} else {
V_271 = F_59 ( V_17 ) ;
if ( V_17 -> V_298 == V_299 ) {
if ( F_18 ( V_17 , V_321 , V_271 ) < 0 )
goto V_53;
} else if ( V_17 -> V_298 == V_300 ) {
if ( F_18 ( V_17 , V_321 , 0x2a ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_322 , V_271 ) < 0 )
goto V_53;
} else if ( V_17 -> V_298 == V_301 ) {
if ( F_18 ( V_17 , V_321 , 0x2a ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_324 , V_271 ) < 0 )
goto V_53;
} else if ( V_17 -> V_298 == V_302 ) {
if ( F_18 ( V_17 , V_321 , 0x2a ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_325 , V_271 ) < 0 )
goto V_53;
}
}
F_18 ( V_17 , V_318 , 0x67 ) ;
break;
case V_244 :
default:
V_18 = F_16 ( V_17 , V_121 ) ;
F_17 ( V_18 , V_122 , 1 ) ;
F_17 ( V_18 , V_123 , 1 ) ;
if ( F_18 ( V_17 , V_121 , V_18 ) < 0 )
goto V_53;
break;
}
V_310 = F_16 ( V_17 , V_197 ) ;
V_311 = F_16 ( V_17 , V_198 ) ;
V_18 = F_16 ( V_17 , V_261 ) ;
if ( V_17 -> V_54 == V_55 ) {
F_18 ( V_17 , V_145 , 0x00 ) ;
V_18 = F_16 ( V_17 , V_121 ) ;
F_17 ( V_18 , V_148 , 0x00 ) ;
F_17 ( V_18 , V_149 , 0x00 ) ;
if ( F_18 ( V_17 , V_121 , V_18 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_187 , 0xc1 ) < 0 )
goto V_53;
if ( F_21 ( V_17 , V_56 ) < 0 )
goto V_53;
V_312 = 1 ;
if ( F_33 ( V_17 ) < 0 )
goto V_53;
}
if ( V_17 -> V_7 -> V_118 >= 0x20 ) {
if ( ( V_17 -> V_82 == V_86 ) ||
( V_17 -> V_82 == V_93 ) ||
( V_17 -> V_82 == V_83 ) ) {
if ( F_18 ( V_17 , V_144 , 0x0a ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_143 , 0x00 ) < 0 )
goto V_53;
}
}
if ( F_18 ( V_17 , V_162 , 0x38 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_66 , 0x80 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_68 , 0x80 ) < 0 )
goto V_53;
if ( ( V_17 -> V_7 -> V_118 >= 0x20 ) || ( V_312 == 1 ) ||
( V_17 -> V_56 < 10000000 ) ) {
if ( F_18 ( V_17 , V_139 , V_310 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_140 , V_311 ) < 0 )
goto V_53;
V_17 -> V_194 = F_24 ( V_56 , V_17 -> V_71 ) + 10000000 ;
if ( ( V_17 -> V_7 -> V_118 >= 0x20 ) || ( V_312 == 1 ) ) {
if ( V_17 -> V_54 != V_60 ) {
if ( F_14 ( V_17 , 1 ) < 0 )
goto V_53;
if ( V_17 -> V_20 -> V_193 ) {
if ( V_17 -> V_20 -> V_193 ( V_179 , V_17 -> V_194 ) < 0 )
goto V_192;
}
if ( F_14 ( V_17 , 0 ) < 0 )
goto V_53;
}
}
if ( ( V_17 -> V_54 == V_55 ) || ( V_17 -> V_56 < 10000000 ) )
F_39 ( 50 ) ;
else
F_39 ( 5 ) ;
F_20 ( V_17 ) ;
if ( ! ( F_44 ( V_17 , ( V_17 -> V_57 / 2 ) ) ) ) {
if ( F_18 ( V_17 , V_130 , 0x1f ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_139 , V_310 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_140 , V_311 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_130 , 0x18 ) < 0 )
goto V_53;
V_43 = 0 ;
while ( ( ! ( F_44 ( V_17 , ( V_17 -> V_57 / 2 ) ) ) ) && ( V_43 <= 2 ) ) {
if ( F_18 ( V_17 , V_130 , 0x1f ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_139 , V_310 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_140 , V_311 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_130 , 0x18 ) < 0 )
goto V_53;
V_43 ++ ;
}
}
}
if ( V_17 -> V_7 -> V_118 >= 0x20 ) {
if ( F_18 ( V_17 , V_153 , 0x49 ) < 0 )
goto V_53;
}
if ( ( V_17 -> V_240 == V_243 ) || ( V_17 -> V_240 == V_242 ) )
F_26 ( V_17 ) ;
return 0 ;
V_192:
F_14 ( V_17 , 0 ) ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static int F_61 ( struct V_16 * V_17 , T_2 V_199 )
{
T_2 V_200 = 0 , V_201 = 0 , V_202 ;
T_3 V_18 ;
while ( ( V_200 < V_199 ) && ( ! V_201 ) ) {
V_18 = F_16 ( V_17 , V_203 ) ;
V_202 = F_42 ( V_18 , V_204 ) ;
switch ( V_202 ) {
case 0 :
case 1 :
default:
V_201 = 0 ;
break;
case 2 :
V_18 = F_16 ( V_17 , V_326 ) ;
V_201 = F_42 ( V_18 , V_327 ) ;
break;
case 3 :
V_18 = F_16 ( V_17 , V_328 ) ;
V_201 = F_42 ( V_18 , V_329 ) ;
break;
}
if ( ! V_201 ) {
F_39 ( 10 ) ;
V_200 += 10 ;
}
}
return V_201 ;
}
static int F_62 ( struct V_16 * V_17 , T_2 V_218 , T_2 V_330 )
{
T_3 V_18 ;
T_2 V_200 = 0 ;
int V_201 ;
V_201 = F_44 ( V_17 , V_218 ) ;
if ( V_201 )
V_201 = F_61 ( V_17 , V_330 ) ;
if ( V_201 ) {
V_201 = 0 ;
while ( ( V_200 < V_330 ) && ( ! V_201 ) ) {
V_18 = F_16 ( V_17 , V_331 ) ;
V_201 = F_42 ( V_18 , V_332 ) ;
F_39 ( 1 ) ;
V_200 ++ ;
}
}
return V_201 ;
}
static int F_63 ( struct V_16 * V_17 )
{
T_3 V_18 ;
if ( V_17 -> V_7 -> V_118 <= 0x20 ) {
V_18 = F_16 ( V_17 , V_313 ) ;
F_17 ( V_18 , V_315 , 0x00 ) ;
if ( F_18 ( V_17 , V_313 , V_18 ) < 0 )
goto V_53;
} else {
V_18 = F_16 ( V_17 , V_313 ) ;
F_17 ( V_18 , V_333 , 0x00 ) ;
if ( F_18 ( V_17 , V_313 , V_18 ) < 0 )
goto V_53;
}
return 0 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static enum V_251 F_64 ( struct V_16 * V_17 )
{
struct V_178 * V_179 = & V_17 -> V_48 ;
enum V_251 V_334 = V_335 ;
T_3 V_18 ;
T_2 V_336 , V_337 = 0 , V_43 ;
int V_201 = 0 , V_338 = 0 ;
V_18 = F_16 ( V_17 , V_339 ) ;
F_17 ( V_18 , V_340 , 1 ) ;
if ( F_18 ( V_17 , V_339 , V_18 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_130 , 0x5c ) < 0 )
goto V_53;
if ( V_17 -> V_7 -> V_118 >= 0x20 ) {
if ( V_17 -> V_56 > 5000000 ) {
if ( F_18 ( V_17 , V_235 , 0x9e ) < 0 )
goto V_53;
} else {
if ( F_18 ( V_17 , V_235 , 0x82 ) < 0 )
goto V_53;
}
}
F_20 ( V_17 ) ;
if ( V_17 -> V_54 == V_55 ) {
V_17 -> V_194 = 2 * 36000000 ;
if ( F_18 ( V_17 , V_187 , 0xc0 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_341 , 0x70 ) < 0 )
goto V_53;
if ( F_21 ( V_17 , 1000000 ) < 0 )
goto V_53;
} else {
if ( F_18 ( V_17 , V_163 , 0x20 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_186 , 0xd2 ) < 0 )
goto V_53;
if ( V_17 -> V_56 < 2000000 ) {
if ( F_18 ( V_17 , V_341 , 0x63 ) < 0 )
goto V_53;
} else {
if ( F_18 ( V_17 , V_341 , 0x70 ) < 0 )
goto V_53;
}
if ( F_18 ( V_17 , V_162 , 0x38 ) < 0 )
goto V_53;
if ( V_17 -> V_7 -> V_118 >= 0x20 ) {
if ( F_18 ( V_17 , V_212 , 0x5a ) < 0 )
goto V_53;
if ( V_17 -> V_54 == V_59 )
V_17 -> V_194 = ( 15 * ( F_24 ( V_17 -> V_56 , V_17 -> V_71 ) + 10000000 ) ) / 10 ;
else if ( V_17 -> V_54 == V_60 )
V_17 -> V_194 = F_24 ( V_17 -> V_56 , V_17 -> V_71 ) + 10000000 ;
}
if ( F_18 ( V_17 , V_187 , 0xc1 ) < 0 )
goto V_53;
if ( F_21 ( V_17 , V_17 -> V_56 ) < 0 )
goto V_53;
if ( F_22 ( V_17 , V_17 -> V_7 -> V_62 ,
V_17 -> V_56 ) < 0 )
goto V_53;
if ( F_23 ( V_17 , V_17 -> V_7 -> V_62 ,
V_17 -> V_56 ) < 0 )
goto V_53;
if ( V_17 -> V_56 >= 10000000 )
V_338 = 0 ;
else
V_338 = 1 ;
}
if ( F_14 ( V_17 , 1 ) < 0 )
goto V_53;
if ( V_17 -> V_20 -> V_342 ) {
V_18 = V_17 -> V_20 -> V_343 ;
if ( V_18 == 0 )
V_18 = 10 ;
if ( V_17 -> V_20 -> V_342 ( V_179 , V_18 ) < 0 )
goto V_192;
}
if ( V_17 -> V_20 -> V_191 ) {
if ( V_17 -> V_20 -> V_191 ( V_179 , V_17 -> V_188 ) < 0 )
goto V_192;
}
if ( V_17 -> V_20 -> V_193 ) {
if ( V_17 -> V_20 -> V_193 ( V_179 , V_17 -> V_194 ) < 0 )
goto V_192;
}
if ( F_14 ( V_17 , 0 ) < 0 )
goto V_53;
F_39 ( 50 ) ;
if ( V_17 -> V_20 -> V_195 ) {
if ( F_14 ( V_17 , 1 ) < 0 )
goto V_53;
if ( V_17 -> V_20 -> V_195 ( V_179 , & V_18 ) < 0 )
goto V_192;
if ( F_14 ( V_17 , 0 ) < 0 )
goto V_53;
if ( V_18 )
F_9 ( V_51 , 1 , L_14 ) ;
else {
F_9 ( V_51 , 1 , L_15 ) ;
return V_335 ;
}
}
F_39 ( 10 ) ;
V_336 = F_65 ( F_16 ( V_17 , V_344 ) ,
F_16 ( V_17 , V_345 ) ) ;
if ( V_336 == 0 ) {
for ( V_43 = 0 ; V_43 < 5 ; V_43 ++ ) {
V_337 += ( F_16 ( V_17 , V_346 ) +
F_16 ( V_17 , V_347 ) ) >> 1 ;
}
V_337 /= 5 ;
}
if ( ( V_336 == 0 ) && ( V_337 < V_348 ) ) {
F_9 ( V_33 , 1 , L_20 , V_337 ) ;
V_201 = 0 ;
V_334 = V_349 ;
} else {
V_18 = F_16 ( V_17 , V_313 ) ;
F_17 ( V_18 , V_350 , V_17 -> V_263 ) ;
if ( V_17 -> V_7 -> V_118 <= 0x20 ) {
F_17 ( V_18 , V_315 , 1 ) ;
} else {
F_17 ( V_18 , V_333 , 1 ) ;
}
if ( F_18 ( V_17 , V_313 , V_18 ) < 0 )
goto V_53;
if ( F_34 ( V_17 ) < 0 )
goto V_53;
if ( V_17 -> V_54 != V_55 ) {
if ( F_35 ( V_17 ) < 0 )
goto V_53;
}
}
if ( V_334 == V_349 )
return V_334 ;
if ( V_17 -> V_54 == V_55 )
V_201 = F_45 ( V_17 ) ;
else if ( V_17 -> V_54 == V_59 )
V_201 = F_48 ( V_17 , V_17 -> V_57 ) ;
else if ( V_17 -> V_54 == V_60 )
V_201 = F_44 ( V_17 , V_17 -> V_57 ) ;
if ( ( ! V_201 ) && ( V_17 -> V_54 == V_59 ) ) {
if ( ! V_338 ) {
if ( F_47 ( V_17 ) )
V_201 = F_52 ( V_17 ) ;
}
}
if ( V_201 )
V_334 = F_56 ( V_17 ) ;
if ( ( V_201 ) && ( V_334 == V_265 ) ) {
F_60 ( V_17 ) ;
if ( V_17 -> V_7 -> V_118 >= 0x20 ) {
V_18 = F_16 ( V_17 , V_339 ) ;
F_17 ( V_18 , V_340 , 0 ) ;
if ( F_18 ( V_17 , V_339 , V_18 ) < 0 )
goto V_53;
F_39 ( 3 ) ;
F_17 ( V_18 , V_340 , 1 ) ;
if ( F_18 ( V_17 , V_339 , V_18 ) < 0 )
goto V_53;
F_17 ( V_18 , V_340 , 0 ) ;
if ( F_18 ( V_17 , V_339 , V_18 ) < 0 )
goto V_53;
}
V_201 = F_62 ( V_17 , V_17 -> V_58 ,
V_17 -> V_58 ) ;
if ( V_201 ) {
if ( V_17 -> V_240 == V_236 ) {
F_63 ( V_17 ) ;
V_18 = F_16 ( V_17 , V_351 ) ;
F_17 ( V_18 , V_352 , 1 ) ;
if ( F_18 ( V_17 , V_351 , V_18 ) < 0 )
goto V_53;
V_18 = F_16 ( V_17 , V_351 ) ;
F_17 ( V_18 , V_352 , 0 ) ;
if ( F_18 ( V_17 , V_351 , V_18 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_318 , 0x67 ) < 0 )
goto V_53;
} else {
if ( F_18 ( V_17 , V_318 , 0x75 ) < 0 )
goto V_53;
}
if ( F_18 ( V_17 , V_353 , 0x00 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_354 , 0xc1 ) < 0 )
goto V_53;
} else {
V_334 = V_355 ;
F_50 ( V_17 ) ;
}
}
return V_334 ;
V_192:
F_14 ( V_17 , 0 ) ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static int F_66 ( struct V_16 * V_17 , int V_356 )
{
T_3 V_18 ;
if ( V_356 < 0 || V_356 > 255 ) {
F_9 ( V_51 , 1 , L_21 ) ;
V_18 = F_16 ( V_17 , V_230 ) ;
F_17 ( V_18 , V_357 , 0x00 ) ;
if ( F_18 ( V_17 , V_230 , V_18 ) < 0 )
goto V_53;
} else {
F_9 ( V_51 , 1 , L_22 , V_356 ) ;
V_18 = F_16 ( V_17 , V_230 ) ;
F_17 ( V_18 , V_357 , 0x01 ) ;
if ( F_18 ( V_17 , V_230 , V_18 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_358 , V_356 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_359 , 0xff ) < 0 )
goto V_53;
}
return 0 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static enum V_360 F_67 ( struct V_178 * V_179 )
{
struct V_16 * V_17 = V_179 -> V_361 ;
struct V_362 * V_363 = & V_179 -> V_364 ;
if ( V_363 -> V_188 == 0 )
return V_365 ;
V_17 -> V_240 = V_363 -> V_366 ;
V_17 -> V_188 = V_363 -> V_188 ;
V_17 -> V_56 = V_363 -> V_367 ;
V_17 -> V_82 = V_83 ;
V_17 -> V_54 = V_59 ;
V_17 -> V_87 = V_250 ;
if ( V_17 -> V_56 > 10000000 ) {
F_9 ( V_51 , 1 , L_23 ) ;
V_17 -> V_138 = 10000000 ;
} else {
F_9 ( V_51 , 1 , L_24 ) ;
V_17 -> V_138 = 5000000 ;
}
F_66 ( V_17 , V_363 -> V_368 ) ;
if ( F_64 ( V_17 ) == V_265 ) {
F_9 ( V_51 , 1 , L_25 ) ;
return V_369 ;
} else {
F_9 ( V_51 , 1 , L_26 ) ;
return V_370 ;
}
return V_371 ;
}
static int F_68 ( struct V_178 * V_179 , enum V_372 * V_373 )
{
struct V_16 * V_17 = V_179 -> V_361 ;
T_3 V_18 , V_374 ;
T_1 V_375 ;
* V_373 = 0 ;
V_374 = F_16 ( V_17 , V_189 ) ;
if ( F_42 ( V_374 , V_376 ) )
* V_373 |= V_377 | V_378 ;
V_18 = F_16 ( V_17 , V_203 ) ;
V_375 = F_42 ( V_18 , V_204 ) ;
switch ( V_375 ) {
case 0 :
case 1 :
default:
F_9 ( V_51 , 1 , L_27 ) ;
break;
case 2 :
F_9 ( V_51 , 1 , L_28 ) ;
if ( F_42 ( V_374 , V_205 ) ) {
V_18 = F_16 ( V_17 , V_326 ) ;
if ( F_42 ( V_18 , V_327 ) ) {
* V_373 |= V_379 ;
V_18 = F_16 ( V_17 , V_331 ) ;
if ( F_42 ( V_18 , V_332 ) )
* V_373 |= V_380 | V_381 ;
}
}
break;
case 3 :
F_9 ( V_51 , 1 , L_29 ) ;
if ( F_42 ( V_374 , V_205 ) ) {
V_18 = F_16 ( V_17 , V_328 ) ;
if ( F_42 ( V_18 , V_329 ) ) {
* V_373 |= V_379 ;
V_18 = F_16 ( V_17 , V_331 ) ;
if ( F_42 ( V_18 , V_332 ) )
* V_373 |= V_380 | V_381 ;
}
}
break;
}
return 0 ;
}
static int F_69 ( struct V_178 * V_179 , T_3 * V_382 )
{
struct V_16 * V_17 = V_179 -> V_361 ;
T_2 V_383 , V_384 , V_385 , V_386 , V_387 , V_38 ;
T_3 V_18 , V_388 , V_389 , V_390 ;
enum V_372 V_373 ;
F_68 ( V_179 , & V_373 ) ;
if ( ! ( V_373 & V_381 ) ) {
* V_382 = 1 << 23 ;
} else {
V_18 = F_16 ( V_17 , V_391 ) ;
V_388 = F_42 ( V_18 , V_392 ) ;
V_18 = F_16 ( V_17 , V_393 ) ;
V_389 = F_42 ( V_18 , V_394 ) ;
V_18 = F_16 ( V_17 , V_395 ) ;
V_390 = F_42 ( V_18 , V_396 ) ;
* V_382 = ( ( V_388 << 16 ) | ( V_389 << 8 ) | V_390 ) ;
V_383 = F_16 ( V_17 , V_353 ) ;
V_384 = F_16 ( V_17 , V_397 ) ;
V_385 = F_16 ( V_17 , V_398 ) ;
V_386 = F_16 ( V_17 , V_399 ) ;
V_387 = F_16 ( V_17 , V_400 ) ;
if ( ( ! V_383 ) && ( ! V_384 ) ) {
V_38 = ( V_385 & 0xff ) << 16 ;
V_38 |= ( V_386 & 0xff ) << 8 ;
V_38 |= V_387 & 0xff ;
} else {
V_38 = 1 << 24 ;
}
if ( V_38 == 0 )
* V_382 = 1 ;
}
if ( F_18 ( V_17 , V_353 , 0 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_354 , 0xc1 ) < 0 )
goto V_53;
return 0 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static int F_70 ( const struct V_401 * V_402 , int V_403 , int V_404 )
{
int V_405 = 0 ;
int V_406 = 0 , V_407 ;
if ( ( V_404 >= V_402 [ V_406 ] . V_408 && V_404 < V_402 [ V_403 ] . V_408 ) ||
( V_404 >= V_402 [ V_403 ] . V_408 && V_404 < V_402 [ V_406 ] . V_408 ) ) {
while ( ( V_403 - V_406 ) > 1 ) {
V_407 = ( V_403 + V_406 ) / 2 ;
if ( ( V_404 >= V_402 [ V_406 ] . V_408 && V_404 < V_402 [ V_407 ] . V_408 ) ||
( V_404 >= V_402 [ V_407 ] . V_408 && V_404 < V_402 [ V_406 ] . V_408 ) )
V_403 = V_407 ;
else
V_406 = V_407 ;
}
V_405 = ( ( V_404 - V_402 [ V_406 ] . V_408 ) *
( V_402 [ V_403 ] . V_409 - V_402 [ V_406 ] . V_409 ) /
( V_402 [ V_403 ] . V_408 - V_402 [ V_406 ] . V_408 ) ) +
V_402 [ V_406 ] . V_409 ;
} else {
if ( V_402 [ V_406 ] . V_408 < V_402 [ V_403 ] . V_408 ) {
if ( V_404 < V_402 [ V_406 ] . V_408 )
V_405 = V_402 [ V_406 ] . V_409 ;
else if ( V_404 >= V_402 [ V_403 ] . V_408 )
V_405 = V_402 [ V_403 ] . V_409 ;
} else {
if ( V_404 >= V_402 [ V_406 ] . V_408 )
V_405 = V_402 [ V_406 ] . V_409 ;
else if ( V_404 < V_402 [ V_403 ] . V_408 )
V_405 = V_402 [ V_403 ] . V_409 ;
}
}
return V_405 ;
}
static int F_71 ( struct V_178 * V_179 , T_5 * V_410 )
{
struct V_16 * V_17 = V_179 -> V_361 ;
T_3 V_18 ;
T_2 V_411 , V_412 , V_413 ;
T_2 V_414 ;
V_18 = F_16 ( V_17 , V_344 ) ;
V_412 = F_42 ( V_18 , V_415 ) ;
V_18 = F_16 ( V_17 , V_345 ) ;
V_411 = F_42 ( V_18 , V_415 ) ;
V_413 = F_65 ( V_412 , V_411 ) ;
V_414 = F_70 ( V_416 ,
F_72 ( V_416 ) - 1 , V_413 ) ;
if ( V_413 > V_416 [ 0 ] . V_408 )
V_414 = 0 ;
else if ( V_413 < V_416 [ F_72 ( V_416 ) - 1 ] . V_408 )
V_414 = - 100 ;
* V_410 = ( V_414 + 100 ) * 0xFFFF / 100 ;
return 0 ;
}
static int F_73 ( struct V_178 * V_179 , T_5 * V_417 )
{
struct V_16 * V_17 = V_179 -> V_361 ;
T_3 V_418 , V_419 , V_18 , V_43 ;
T_2 V_420 , V_421 , V_404 = 0 ;
T_1 V_422 ;
T_2 div ;
T_3 V_423 ;
switch ( V_17 -> V_240 ) {
case V_236 :
V_18 = F_16 ( V_17 , V_189 ) ;
V_422 = F_42 ( V_18 , V_205 ) ;
if ( V_422 ) {
F_39 ( 5 ) ;
for ( V_43 = 0 ; V_43 < 16 ; V_43 ++ ) {
V_419 = F_16 ( V_17 , V_424 ) ;
V_421 = F_42 ( V_419 , V_425 ) ;
V_418 = F_16 ( V_17 , V_426 ) ;
V_420 = F_42 ( V_418 , V_425 ) ;
V_404 += F_65 ( V_421 , V_420 ) ;
F_39 ( 1 ) ;
}
V_404 /= 16 ;
V_423 = F_72 ( V_427 ) - 1 ;
div = V_427 [ 0 ] . V_408 -
V_427 [ V_423 ] . V_408 ;
* V_417 = 0xFFFF - ( ( V_404 * 0xFFFF ) / div ) ;
}
break;
case V_243 :
case V_242 :
V_18 = F_16 ( V_17 , V_189 ) ;
V_422 = F_42 ( V_18 , V_205 ) ;
if ( V_422 ) {
F_39 ( 5 ) ;
for ( V_43 = 0 ; V_43 < 16 ; V_43 ++ ) {
V_419 = F_16 ( V_17 , V_428 ) ;
V_421 = F_42 ( V_419 , V_429 ) ;
V_418 = F_16 ( V_17 , V_430 ) ;
V_420 = F_42 ( V_418 , V_429 ) ;
V_404 += F_65 ( V_421 , V_420 ) ;
F_39 ( 1 ) ;
}
V_404 /= 16 ;
V_423 = F_72 ( V_431 ) - 1 ;
div = V_431 [ 0 ] . V_408 -
V_431 [ V_423 ] . V_408 ;
* V_417 = 0xFFFF - ( ( V_404 * 0xFFFF ) / div ) ;
}
break;
default:
break;
}
return 0 ;
}
static int F_74 ( struct V_178 * V_179 , T_6 V_432 )
{
struct V_16 * V_17 = V_179 -> V_361 ;
T_3 V_18 ;
V_18 = F_16 ( V_17 , V_433 ) ;
switch ( V_432 ) {
case V_434 :
F_17 ( V_18 , V_435 , 0 ) ;
F_17 ( V_18 , V_436 , 1 ) ;
if ( F_18 ( V_17 , V_433 , V_18 ) < 0 )
goto V_53;
F_17 ( V_18 , V_436 , 0 ) ;
if ( F_18 ( V_17 , V_433 , V_18 ) < 0 )
goto V_53;
break;
case V_437 :
F_17 ( V_18 , V_435 , 0 ) ;
F_17 ( V_18 , V_436 , 1 ) ;
if ( F_18 ( V_17 , V_433 , V_18 ) < 0 )
goto V_53;
break;
default:
return - V_42 ;
}
return 0 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static enum V_438 F_75 ( struct V_178 * V_179 )
{
return V_439 ;
}
static int F_76 ( struct V_178 * V_179 , struct V_440 * V_441 )
{
struct V_16 * V_17 = V_179 -> V_361 ;
T_3 V_18 , V_442 = 0 , V_443 = 1 ;
int V_43 ;
V_18 = F_16 ( V_17 , V_433 ) ;
F_17 ( V_18 , V_435 ,
( V_17 -> V_20 -> V_444 ) ? 4 : 2 ) ;
F_17 ( V_18 , V_436 , 1 ) ;
if ( F_18 ( V_17 , V_433 , V_18 ) < 0 )
goto V_53;
F_17 ( V_18 , V_436 , 0 ) ;
if ( F_18 ( V_17 , V_433 , V_18 ) < 0 )
goto V_53;
F_17 ( V_18 , V_445 , 1 ) ;
if ( F_18 ( V_17 , V_433 , V_18 ) < 0 )
goto V_53;
for ( V_43 = 0 ; V_43 < V_441 -> V_446 ; V_43 ++ ) {
while ( V_443 ) {
V_18 = F_16 ( V_17 , V_447 ) ;
V_443 = F_42 ( V_18 , V_448 ) ;
}
if ( F_18 ( V_17 , V_449 , V_441 -> V_25 [ V_43 ] ) < 0 )
goto V_53;
}
V_18 = F_16 ( V_17 , V_433 ) ;
F_17 ( V_18 , V_445 , 0 ) ;
if ( F_18 ( V_17 , V_433 , V_18 ) < 0 )
goto V_53;
V_43 = 0 ;
while ( ( ! V_442 ) && ( V_43 < 10 ) ) {
V_18 = F_16 ( V_17 , V_447 ) ;
V_442 = F_42 ( V_18 , V_450 ) ;
F_39 ( 10 ) ;
V_43 ++ ;
}
return 0 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static int F_77 ( struct V_178 * V_179 , T_7 V_451 )
{
struct V_16 * V_17 = V_179 -> V_361 ;
T_3 V_18 , V_442 = 0 , V_443 = 1 ;
T_1 V_452 , V_453 ;
int V_43 ;
V_18 = F_16 ( V_17 , V_433 ) ;
if ( V_451 == V_454 ) {
V_452 = ( V_17 -> V_20 -> V_444 ) ? 5 : 3 ;
V_453 = 0x00 ;
} else {
V_452 = ( V_17 -> V_20 -> V_444 ) ? 4 : 2 ;
V_453 = 0xFF ;
}
F_17 ( V_18 , V_435 , V_452 ) ;
F_17 ( V_18 , V_436 , 1 ) ;
if ( F_18 ( V_17 , V_433 , V_18 ) < 0 )
goto V_53;
F_17 ( V_18 , V_436 , 0 ) ;
if ( F_18 ( V_17 , V_433 , V_18 ) < 0 )
goto V_53;
F_17 ( V_18 , V_445 , 1 ) ;
if ( F_18 ( V_17 , V_433 , V_18 ) < 0 )
goto V_53;
while ( V_443 ) {
V_18 = F_16 ( V_17 , V_447 ) ;
V_443 = F_42 ( V_18 , V_448 ) ;
}
if ( F_18 ( V_17 , V_449 , V_453 ) < 0 )
goto V_53;
V_18 = F_16 ( V_17 , V_433 ) ;
F_17 ( V_18 , V_445 , 0 ) ;
if ( F_18 ( V_17 , V_433 , V_18 ) < 0 )
goto V_53;
V_43 = 0 ;
while ( ( ! V_442 ) && ( V_43 < 10 ) ) {
V_18 = F_16 ( V_17 , V_447 ) ;
V_442 = F_42 ( V_18 , V_450 ) ;
F_39 ( 10 ) ;
V_43 ++ ;
}
return 0 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static int F_78 ( struct V_178 * V_179 , struct V_455 * V_456 )
{
struct V_16 * V_17 = V_179 -> V_361 ;
T_3 V_18 = 0 , V_43 = 0 , V_457 = 0 ;
while ( ( V_457 != 1 ) && ( V_43 < 10 ) ) {
F_39 ( 10 ) ;
V_43 ++ ;
V_18 = F_16 ( V_17 , V_458 ) ;
V_457 = F_42 ( V_18 , V_459 ) ;
}
if ( V_457 ) {
V_456 -> V_446 = F_42 ( V_18 , V_460 ) ;
for ( V_43 = 0 ; V_43 < V_456 -> V_446 ; V_43 ++ )
V_456 -> V_25 [ V_43 ] = F_16 ( V_17 , V_461 ) ;
}
return 0 ;
}
static int F_79 ( struct V_178 * V_179 )
{
struct V_16 * V_17 = V_179 -> V_361 ;
T_3 V_18 ;
T_1 V_462 = 0 ;
if ( F_14 ( V_17 , 1 ) < 0 )
goto V_53;
if ( V_17 -> V_20 -> V_463 ) {
if ( V_17 -> V_20 -> V_463 ( V_179 ) < 0 )
goto V_192;
}
if ( F_14 ( V_17 , 0 ) < 0 )
goto V_53;
F_9 ( V_51 , 1 , L_30 ,
V_17 -> V_464 == V_465 ? L_31 : L_32 ,
V_17 -> V_111 ) ;
F_15 ( & V_17 -> V_7 -> V_113 ) ;
switch ( V_17 -> V_111 ) {
case V_112 :
V_18 = F_7 ( V_17 , V_466 ) ;
F_32 ( V_18 , V_467 , 0 ) ;
if ( F_13 ( V_17 , V_466 , V_18 ) < 0 )
goto V_468;
V_18 = F_7 ( V_17 , V_469 ) ;
F_32 ( V_18 , V_470 , 0 ) ;
if ( F_13 ( V_17 , V_469 , V_18 ) < 0 )
goto V_468;
V_18 = F_7 ( V_17 , V_471 ) ;
if ( F_80 ( V_18 , V_472 ) == 0 )
V_462 = 1 ;
V_18 = F_7 ( V_17 , V_473 ) ;
F_32 ( V_18 , V_474 , 1 ) ;
F_32 ( V_18 , V_475 , 1 ) ;
if ( V_462 )
F_32 ( V_18 , V_476 , 1 ) ;
if ( F_13 ( V_17 , V_473 , V_18 ) < 0 )
goto V_468;
V_18 = F_7 ( V_17 , V_114 ) ;
F_32 ( V_18 , V_477 , 1 ) ;
F_32 ( V_18 , V_115 , 1 ) ;
if ( V_462 )
F_32 ( V_18 , V_478 , 1 ) ;
if ( F_13 ( V_17 , V_114 , V_18 ) < 0 )
goto V_468;
break;
case V_116 :
V_18 = F_7 ( V_17 , V_471 ) ;
F_32 ( V_18 , V_472 , 0 ) ;
if ( F_13 ( V_17 , V_471 , V_18 ) < 0 )
goto V_468;
V_18 = F_7 ( V_17 , V_479 ) ;
F_32 ( V_18 , V_480 , 0 ) ;
if ( F_13 ( V_17 , V_479 , V_18 ) < 0 )
goto V_468;
V_18 = F_7 ( V_17 , V_466 ) ;
if ( F_80 ( V_18 , V_467 ) == 0 )
V_462 = 1 ;
V_18 = F_7 ( V_17 , V_473 ) ;
F_32 ( V_18 , V_481 , 1 ) ;
F_32 ( V_18 , V_482 , 1 ) ;
if ( V_462 )
F_32 ( V_18 , V_476 , 1 ) ;
if ( F_13 ( V_17 , V_473 , V_18 ) < 0 )
goto V_468;
V_18 = F_7 ( V_17 , V_114 ) ;
F_32 ( V_18 , V_483 , 1 ) ;
F_32 ( V_18 , V_117 , 1 ) ;
if ( V_462 )
F_32 ( V_18 , V_478 , 1 ) ;
if ( F_13 ( V_17 , V_114 , V_18 ) < 0 )
goto V_468;
break;
default:
F_9 ( V_33 , 1 , L_13 ) ;
break;
}
if ( V_462 ) {
V_18 = F_7 ( V_17 , V_484 ) ;
F_32 ( V_18 , V_485 , 0x01 ) ;
if ( F_13 ( V_17 , V_484 , V_18 ) < 0 )
goto V_468;
}
F_19 ( & V_17 -> V_7 -> V_113 ) ;
return 0 ;
V_192:
F_14 ( V_17 , 0 ) ;
goto V_53;
V_468:
F_19 ( & V_17 -> V_7 -> V_113 ) ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static int F_81 ( struct V_178 * V_179 )
{
struct V_16 * V_17 = V_179 -> V_361 ;
T_3 V_18 ;
F_9 ( V_51 , 1 , L_33 ,
V_17 -> V_464 == V_465 ? L_31 : L_32 ,
V_17 -> V_111 ) ;
F_15 ( & V_17 -> V_7 -> V_113 ) ;
V_18 = F_7 ( V_17 , V_484 ) ;
F_32 ( V_18 , V_485 , 0x00 ) ;
if ( F_13 ( V_17 , V_484 , V_18 ) < 0 )
goto V_53;
switch ( V_17 -> V_111 ) {
case V_112 :
V_18 = F_7 ( V_17 , V_466 ) ;
F_32 ( V_18 , V_467 , 1 ) ;
if ( F_13 ( V_17 , V_466 , V_18 ) < 0 )
goto V_53;
V_18 = F_7 ( V_17 , V_469 ) ;
F_32 ( V_18 , V_470 , 1 ) ;
if ( F_13 ( V_17 , V_469 , V_18 ) < 0 )
goto V_53;
V_18 = F_7 ( V_17 , V_473 ) ;
F_32 ( V_18 , V_474 , 0 ) ;
F_32 ( V_18 , V_475 , 0 ) ;
F_32 ( V_18 , V_476 , 0 ) ;
if ( F_13 ( V_17 , V_473 , V_18 ) < 0 )
goto V_53;
V_18 = F_7 ( V_17 , V_114 ) ;
F_32 ( V_18 , V_477 , 0 ) ;
F_32 ( V_18 , V_115 , 0 ) ;
F_32 ( V_18 , V_478 , 0 ) ;
if ( F_13 ( V_17 , V_114 , V_18 ) < 0 )
goto V_53;
break;
case V_116 :
V_18 = F_7 ( V_17 , V_471 ) ;
F_32 ( V_18 , V_472 , 1 ) ;
if ( F_13 ( V_17 , V_471 , V_18 ) < 0 )
goto V_53;
V_18 = F_7 ( V_17 , V_479 ) ;
F_32 ( V_18 , V_480 , 1 ) ;
if ( F_13 ( V_17 , V_479 , V_18 ) < 0 )
goto V_53;
V_18 = F_7 ( V_17 , V_473 ) ;
F_32 ( V_18 , V_481 , 0 ) ;
F_32 ( V_18 , V_482 , 0 ) ;
F_32 ( V_18 , V_476 , 0 ) ;
if ( F_13 ( V_17 , V_473 , V_18 ) < 0 )
goto V_53;
V_18 = F_7 ( V_17 , V_114 ) ;
F_32 ( V_18 , V_483 , 0 ) ;
F_32 ( V_18 , V_117 , 0 ) ;
F_32 ( V_18 , V_478 , 0 ) ;
if ( F_13 ( V_17 , V_114 , V_18 ) < 0 )
goto V_53;
break;
default:
F_9 ( V_33 , 1 , L_13 ) ;
break;
}
F_19 ( & V_17 -> V_7 -> V_113 ) ;
return 0 ;
V_53:
F_19 ( & V_17 -> V_7 -> V_113 ) ;
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static void F_82 ( struct V_178 * V_179 )
{
struct V_16 * V_17 = V_179 -> V_361 ;
V_17 -> V_7 -> V_486 -- ;
if ( V_17 -> V_7 -> V_486 <= 0 ) {
F_9 ( V_33 , 1 , L_34 ) ;
F_2 ( V_17 -> V_7 ) ;
F_3 ( V_17 -> V_7 ) ;
}
F_3 ( V_17 ) ;
}
static int F_83 ( struct V_16 * V_17 , enum V_487 V_488 )
{
T_3 V_18 = 0 ;
V_18 = F_7 ( V_17 , V_489 ) ;
switch ( V_488 ) {
case V_490 :
default:
if ( ( V_17 -> V_126 != V_490 ) || ( F_80 ( V_18 , V_491 ) != 1 ) ) {
if ( F_13 ( V_17 , V_489 , 0x1d ) < 0 )
goto V_53;
V_17 -> V_126 = V_490 ;
V_18 = F_7 ( V_17 , V_492 ) ;
F_32 ( V_18 , V_493 , 0x1 ) ;
if ( F_13 ( V_17 , V_492 , V_18 ) < 0 )
goto V_53;
F_32 ( V_18 , V_493 , 0x0 ) ;
if ( F_13 ( V_17 , V_492 , V_18 ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_95 , 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_96 , 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_97 , 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_98 , 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_99 , 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_100 , 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_101 , 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_102 , 0xcc ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_103 , 0xcc ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_104 , 0xcc ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_105 , 0xcc ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_106 , 0xcc ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_107 , 0xcc ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_108 , 0xcc ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_109 , 0xff ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_110 , 0xcf ) < 0 )
goto V_53;
}
break;
case V_127 :
if ( F_28 ( V_17 ) < 0 )
goto V_53;
if ( F_30 ( V_17 ) < 0 )
goto V_53;
if ( V_17 -> V_111 == V_116 ) {
if ( F_13 ( V_17 , V_489 , 0x06 ) < 0 )
goto V_53;
} else {
if ( F_13 ( V_17 , V_489 , 0x04 ) < 0 )
goto V_53;
}
V_18 = F_7 ( V_17 , V_492 ) ;
F_32 ( V_18 , V_493 , 0x1 ) ;
if ( F_13 ( V_17 , V_492 , V_18 ) < 0 )
goto V_53;
F_32 ( V_18 , V_493 , 0x0 ) ;
if ( F_13 ( V_17 , V_492 , V_18 ) < 0 )
goto V_53;
V_18 = F_16 ( V_17 , V_230 ) ;
F_17 ( V_18 , V_231 , 0x01 ) ;
if ( F_18 ( V_17 , V_230 , V_18 ) < 0 )
goto V_53;
F_17 ( V_18 , V_231 , 0x00 ) ;
if ( F_18 ( V_17 , V_230 , V_18 ) < 0 )
goto V_53;
break;
}
return 0 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static T_3 F_84 ( struct V_16 * V_17 )
{
const struct V_19 * V_20 = V_17 -> V_20 ;
T_3 div , V_18 ;
T_1 V_494 ;
div = F_7 ( V_17 , V_495 ) ;
V_18 = F_7 ( V_17 , V_484 ) ;
V_494 = F_80 ( V_18 , V_496 ) ? 4 : 6 ;
return ( div + 1 ) * V_20 -> V_497 / V_494 ;
}
static int F_85 ( struct V_16 * V_17 , T_3 V_62 , T_3 V_65 )
{
const struct V_19 * V_20 = V_17 -> V_20 ;
T_3 V_18 , div , V_498 ;
V_18 = F_7 ( V_17 , V_484 ) ;
V_498 = ( ( F_80 ( V_18 , V_496 ) == 1 ) ? 4 : 6 ) ;
div = ( ( V_498 * V_62 ) / V_20 -> V_497 ) - 1 ;
V_18 = F_7 ( V_17 , V_495 ) ;
F_32 ( V_18 , V_499 , div ) ;
if ( F_13 ( V_17 , V_495 , V_18 ) < 0 )
goto V_53;
V_17 -> V_7 -> V_62 = F_84 ( V_17 ) ;
div = V_17 -> V_7 -> V_62 / 704000 ;
if ( F_18 ( V_17 , V_500 , div ) < 0 )
goto V_53;
if ( F_18 ( V_17 , V_501 , div ) < 0 )
goto V_53;
return 0 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static int F_86 ( struct V_16 * V_17 )
{
T_3 V_18 ;
if ( V_17 -> V_7 -> V_118 >= 0x20 ) {
switch ( V_17 -> V_20 -> V_502 ) {
case V_503 :
case V_504 :
switch ( V_17 -> V_20 -> V_505 ) {
case V_506 :
case V_507 :
default:
F_13 ( V_17 , V_508 , 0x00 ) ;
break;
case V_503 :
case V_504 :
if ( F_13 ( V_17 , V_508 , 0x06 ) < 0 )
goto V_53;
V_18 = F_7 ( V_17 , V_509 ) ;
F_17 ( V_18 , V_510 , 3 ) ;
if ( F_13 ( V_17 , V_509 , V_18 ) < 0 )
goto V_53;
V_18 = F_7 ( V_17 , V_511 ) ;
F_17 ( V_18 , V_510 , 3 ) ;
if ( F_13 ( V_17 , V_511 , V_18 ) < 0 )
goto V_53;
if ( F_13 ( V_17 , V_512 , 0x14 ) < 0 )
goto V_53;
if ( F_13 ( V_17 , V_513 , 0x28 ) < 0 )
goto V_53;
break;
}
break;
case V_506 :
case V_507 :
default:
switch ( V_17 -> V_20 -> V_505 ) {
case V_506 :
case V_507 :
default:
if ( F_13 ( V_17 , V_508 , 0x0c ) < 0 )
goto V_53;
break;
case V_503 :
case V_504 :
if ( F_13 ( V_17 , V_508 , 0x0a ) < 0 )
goto V_53;
break;
}
break;
}
} else {
switch ( V_17 -> V_20 -> V_502 ) {
case V_503 :
case V_504 :
switch ( V_17 -> V_20 -> V_505 ) {
case V_506 :
case V_507 :
default:
F_13 ( V_17 , V_514 , 0x10 ) ;
break;
case V_503 :
case V_504 :
F_13 ( V_17 , V_514 , 0x16 ) ;
V_18 = F_7 ( V_17 , V_509 ) ;
F_17 ( V_18 , V_510 , 3 ) ;
if ( F_13 ( V_17 , V_509 , V_18 ) < 0 )
goto V_53;
V_18 = F_7 ( V_17 , V_509 ) ;
F_17 ( V_18 , V_510 , 0 ) ;
if ( F_13 ( V_17 , V_509 , V_18 ) < 0 )
goto V_53;
if ( F_13 ( V_17 , V_512 , 0x14 ) < 0 )
goto V_53;
if ( F_13 ( V_17 , V_513 , 0x28 ) < 0 )
goto V_53;
break;
}
break;
case V_506 :
case V_507 :
default:
switch ( V_17 -> V_20 -> V_505 ) {
case V_506 :
case V_507 :
default:
F_13 ( V_17 , V_514 , 0x14 ) ;
break;
case V_503 :
case V_504 :
F_13 ( V_17 , V_514 , 0x12 ) ;
break;
}
break;
}
}
switch ( V_17 -> V_20 -> V_502 ) {
case V_503 :
V_18 = F_7 ( V_17 , V_515 ) ;
F_17 ( V_18 , V_516 , V_17 -> V_20 -> V_517 ) ;
F_17 ( V_18 , V_518 , 0x00 ) ;
F_17 ( V_18 , V_519 , 0x00 ) ;
if ( F_13 ( V_17 , V_515 , V_18 ) < 0 )
goto V_53;
break;
case V_504 :
V_18 = F_7 ( V_17 , V_515 ) ;
F_17 ( V_18 , V_516 , V_17 -> V_20 -> V_517 ) ;
F_17 ( V_18 , V_518 , 0x00 ) ;
F_17 ( V_18 , V_519 , 0x01 ) ;
if ( F_13 ( V_17 , V_515 , V_18 ) < 0 )
goto V_53;
break;
case V_506 :
V_18 = F_7 ( V_17 , V_515 ) ;
F_17 ( V_18 , V_516 , V_17 -> V_20 -> V_517 ) ;
F_17 ( V_18 , V_518 , 0x01 ) ;
F_17 ( V_18 , V_519 , 0x00 ) ;
if ( F_13 ( V_17 , V_515 , V_18 ) < 0 )
goto V_53;
break;
case V_507 :
V_18 = F_7 ( V_17 , V_515 ) ;
F_17 ( V_18 , V_516 , V_17 -> V_20 -> V_517 ) ;
F_17 ( V_18 , V_518 , 0x01 ) ;
F_17 ( V_18 , V_519 , 0x01 ) ;
if ( F_13 ( V_17 , V_515 , V_18 ) < 0 )
goto V_53;
break;
default:
break;
}
switch ( V_17 -> V_20 -> V_505 ) {
case V_503 :
V_18 = F_7 ( V_17 , V_520 ) ;
F_17 ( V_18 , V_516 , V_17 -> V_20 -> V_521 ) ;
F_17 ( V_18 , V_518 , 0x00 ) ;
F_17 ( V_18 , V_519 , 0x00 ) ;
if ( F_13 ( V_17 , V_520 , V_18 ) < 0 )
goto V_53;
break;
case V_504 :
V_18 = F_7 ( V_17 , V_520 ) ;
F_17 ( V_18 , V_516 , V_17 -> V_20 -> V_521 ) ;
F_17 ( V_18 , V_518 , 0x00 ) ;
F_17 ( V_18 , V_519 , 0x01 ) ;
if ( F_13 ( V_17 , V_520 , V_18 ) < 0 )
goto V_53;
break;
case V_506 :
V_18 = F_7 ( V_17 , V_520 ) ;
F_17 ( V_18 , V_516 , V_17 -> V_20 -> V_521 ) ;
F_17 ( V_18 , V_518 , 0x01 ) ;
F_17 ( V_18 , V_519 , 0x00 ) ;
if ( F_13 ( V_17 , V_520 , V_18 ) < 0 )
goto V_53;
break;
case V_507 :
V_18 = F_7 ( V_17 , V_520 ) ;
F_17 ( V_18 , V_516 , V_17 -> V_20 -> V_521 ) ;
F_17 ( V_18 , V_518 , 0x01 ) ;
F_17 ( V_18 , V_519 , 0x01 ) ;
if ( F_13 ( V_17 , V_520 , V_18 ) < 0 )
goto V_53;
break;
default:
break;
}
if ( V_17 -> V_20 -> V_522 > 0 ) {
T_3 V_523 ;
switch ( V_17 -> V_20 -> V_502 ) {
case V_503 :
case V_504 :
default:
V_523 = V_17 -> V_7 -> V_62 /
( V_17 -> V_20 -> V_522 / 4 ) ;
if ( V_523 < 0x08 )
V_523 = 0x08 ;
if ( V_523 > 0xFF )
V_523 = 0xFF ;
break;
case V_506 :
case V_507 :
V_523 = V_17 -> V_7 -> V_62 /
( V_17 -> V_20 -> V_522 / 32 ) ;
if ( V_523 < 0x20 )
V_523 = 0x20 ;
if ( V_523 > 0xFF )
V_523 = 0xFF ;
break;
}
V_18 = F_7 ( V_17 , V_509 ) ;
F_17 ( V_18 , V_510 , 3 ) ;
if ( F_13 ( V_17 , V_509 , V_18 ) < 0 )
goto V_53;
if ( F_13 ( V_17 , V_512 , V_523 ) < 0 )
goto V_53;
}
if ( V_17 -> V_20 -> V_524 > 0 ) {
T_3 V_523 ;
switch ( V_17 -> V_20 -> V_505 ) {
case V_503 :
case V_504 :
default:
V_523 = V_17 -> V_7 -> V_62 /
( V_17 -> V_20 -> V_524 / 4 ) ;
if ( V_523 < 0x08 )
V_523 = 0x08 ;
if ( V_523 > 0xFF )
V_523 = 0xFF ;
break;
case V_506 :
case V_507 :
V_523 = V_17 -> V_7 -> V_62 /
( V_17 -> V_20 -> V_524 / 32 ) ;
if ( V_523 < 0x20 )
V_523 = 0x20 ;
if ( V_523 > 0xFF )
V_523 = 0xFF ;
break;
}
V_18 = F_7 ( V_17 , V_511 ) ;
F_17 ( V_18 , V_510 , 3 ) ;
if ( F_13 ( V_17 , V_511 , V_18 ) < 0 )
goto V_53;
if ( F_13 ( V_17 , V_513 , V_523 ) < 0 )
goto V_53;
}
V_18 = F_7 ( V_17 , V_520 ) ;
F_17 ( V_18 , V_340 , 0x01 ) ;
if ( F_13 ( V_17 , V_520 , V_18 ) < 0 )
goto V_53;
F_17 ( V_18 , V_340 , 0x00 ) ;
if ( F_13 ( V_17 , V_520 , V_18 ) < 0 )
goto V_53;
V_18 = F_7 ( V_17 , V_515 ) ;
F_17 ( V_18 , V_340 , 0x01 ) ;
if ( F_13 ( V_17 , V_515 , V_18 ) < 0 )
goto V_53;
F_17 ( V_18 , V_340 , 0x00 ) ;
if ( F_13 ( V_17 , V_515 , V_18 ) < 0 )
goto V_53;
return 0 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static int F_87 ( struct V_16 * V_17 )
{
T_3 V_18 ;
if ( V_17 -> V_7 -> V_118 >= 0x20 ) {
switch ( V_17 -> V_20 -> V_502 ) {
case V_503 :
case V_504 :
F_13 ( V_17 , V_508 , 0x00 ) ;
break;
case V_506 :
case V_507 :
default:
F_13 ( V_17 , V_508 , 0x0c ) ;
break;
}
} else {
switch ( V_17 -> V_20 -> V_502 ) {
case V_503 :
case V_504 :
F_13 ( V_17 , V_514 , 0x10 ) ;
break;
case V_506 :
case V_507 :
default:
F_13 ( V_17 , V_514 , 0x14 ) ;
break;
}
}
switch ( V_17 -> V_20 -> V_502 ) {
case V_503 :
V_18 = F_7 ( V_17 , V_515 ) ;
F_17 ( V_18 , V_518 , 0x00 ) ;
F_17 ( V_18 , V_519 , 0x00 ) ;
if ( F_13 ( V_17 , V_515 , V_18 ) < 0 )
goto V_53;
break;
case V_504 :
V_18 = F_7 ( V_17 , V_515 ) ;
F_17 ( V_18 , V_518 , 0x00 ) ;
F_17 ( V_18 , V_519 , 0x01 ) ;
if ( F_13 ( V_17 , V_515 , V_18 ) < 0 )
goto V_53;
break;
case V_506 :
V_18 = F_7 ( V_17 , V_515 ) ;
F_17 ( V_18 , V_518 , 0x01 ) ;
F_17 ( V_18 , V_519 , 0x00 ) ;
if ( F_13 ( V_17 , V_515 , V_18 ) < 0 )
goto V_53;
break;
case V_507 :
V_18 = F_7 ( V_17 , V_515 ) ;
F_17 ( V_18 , V_518 , 0x01 ) ;
F_17 ( V_18 , V_519 , 0x01 ) ;
if ( F_13 ( V_17 , V_515 , V_18 ) < 0 )
goto V_53;
break;
default:
break;
}
if ( V_17 -> V_20 -> V_522 > 0 ) {
T_3 V_523 ;
switch ( V_17 -> V_20 -> V_502 ) {
case V_503 :
case V_504 :
default:
V_523 = V_17 -> V_7 -> V_62 /
( V_17 -> V_20 -> V_522 / 4 ) ;
if ( V_523 < 0x08 )
V_523 = 0x08 ;
if ( V_523 > 0xFF )
V_523 = 0xFF ;
break;
case V_506 :
case V_507 :
V_523 = V_17 -> V_7 -> V_62 /
( V_17 -> V_20 -> V_522 / 32 ) ;
if ( V_523 < 0x20 )
V_523 = 0x20 ;
if ( V_523 > 0xFF )
V_523 = 0xFF ;
break;
}
V_18 = F_7 ( V_17 , V_509 ) ;
F_17 ( V_18 , V_510 , 3 ) ;
if ( F_13 ( V_17 , V_509 , V_18 ) < 0 )
goto V_53;
if ( F_13 ( V_17 , V_512 , V_523 ) < 0 )
goto V_53;
}
V_18 = F_7 ( V_17 , V_515 ) ;
F_17 ( V_18 , V_340 , 0x01 ) ;
if ( F_13 ( V_17 , V_515 , V_18 ) < 0 )
goto V_53;
F_17 ( V_18 , V_340 , 0x00 ) ;
if ( F_13 ( V_17 , V_515 , V_18 ) < 0 )
goto V_53;
return 0 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static int F_88 ( struct V_178 * V_179 )
{
struct V_16 * V_17 = V_179 -> V_361 ;
const struct V_19 * V_20 = V_17 -> V_20 ;
T_3 V_18 ;
if ( V_17 -> V_7 -> V_62 == 0 ) {
if ( F_14 ( V_17 , 1 ) < 0 )
goto V_53;
if ( V_20 -> V_525 ) {
if ( V_20 -> V_525 ( V_179 ) < 0 )
goto V_192;
}
if ( F_14 ( V_17 , 0 ) < 0 )
goto V_53;
F_85 ( V_17 , 135000000 , V_20 -> V_497 ) ;
F_39 ( 5 ) ;
if ( F_13 ( V_17 , V_484 ,
0x20 | V_20 -> V_526 ) < 0 )
goto V_53;
F_84 ( V_17 ) ;
}
if ( F_81 ( V_179 ) < 0 ) {
F_9 ( V_33 , 1 , L_35 ) ;
goto V_53;
}
if ( F_83 ( V_17 , V_17 -> V_126 ) < 0 )
goto V_53;
V_18 = F_16 ( V_17 , V_527 ) ;
F_17 ( V_18 , V_528 , V_17 -> V_263 ) ;
if ( F_18 ( V_17 , V_527 , V_18 ) < 0 )
goto V_53;
V_18 = F_16 ( V_17 , V_313 ) ;
F_17 ( V_18 , V_314 , V_17 -> V_71 ) ;
if ( F_18 ( V_17 , V_313 , V_18 ) < 0 )
goto V_53;
if ( F_14 ( V_17 , 1 ) < 0 )
goto V_53;
if ( V_20 -> V_529 ) {
if ( V_20 -> V_529 ( V_179 , V_530 ) < 0 )
goto V_192;
}
if ( V_20 -> V_525 ) {
if ( V_20 -> V_525 ( V_179 ) < 0 )
goto V_192;
}
if ( F_14 ( V_17 , 0 ) < 0 )
goto V_53;
if ( V_17 -> V_464 == V_465 ) {
if ( F_86 ( V_17 ) < 0 )
goto V_53;
} else {
if ( F_87 ( V_17 ) < 0 )
goto V_53;
}
return 0 ;
V_192:
F_14 ( V_17 , 0 ) ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
static int F_89 ( struct V_178 * V_179 )
{
struct V_16 * V_17 = V_179 -> V_361 ;
const struct V_19 * V_20 = V_17 -> V_20 ;
const struct V_531 * V_532 = NULL ;
const struct V_531 * V_533 = NULL ;
unsigned long V_534 = 0 , V_535 = 0 ;
T_3 V_18 = 0 ;
int V_43 ;
if ( V_17 -> V_464 == V_465 ) {
F_9 ( V_51 , 1 , L_36 ) ;
V_532 = V_536 ;
V_534 = F_72 ( V_536 ) ;
V_533 = V_537 ;
V_535 = F_72 ( V_537 ) ;
} else if ( V_17 -> V_464 == V_538 ) {
F_9 ( V_51 , 1 , L_37 ) ;
V_532 = V_539 ;
V_534 = F_72 ( V_539 ) ;
V_533 = V_540 ;
V_535 = F_72 ( V_540 ) ;
}
if ( F_13 ( V_17 , V_541 , 0x5c ) < 0 )
goto V_53;
if ( V_17 -> V_464 == V_465 )
if ( F_13 ( V_17 , V_542 , 0x5c ) < 0 )
goto V_53;
F_39 ( 5 ) ;
if ( F_13 ( V_17 , V_543 , 0x6c ) < 0 )
goto V_53;
if ( V_17 -> V_464 == V_465 )
if ( F_13 ( V_17 , V_544 , 0x6c ) < 0 )
goto V_53;
F_17 ( V_18 , V_545 , V_20 -> V_546 ) ;
if ( F_13 ( V_17 , V_547 , V_18 ) < 0 )
goto V_53;
if ( V_17 -> V_464 == V_465 )
if ( F_13 ( V_17 , V_548 , V_18 ) < 0 )
goto V_53;
if ( F_13 ( V_17 , V_495 , 0x13 ) < 0 )
goto V_53;
F_39 ( 5 ) ;
if ( F_13 ( V_17 , V_549 , 0x08 ) < 0 )
goto V_53;
if ( F_13 ( V_17 , V_484 , 0x20 | V_20 -> V_526 ) < 0 )
goto V_53;
F_39 ( 5 ) ;
F_9 ( V_51 , 1 , L_38 ) ;
for ( V_43 = 0 ; V_43 < V_534 ; V_43 ++ ) {
if ( F_13 ( V_17 , V_532 [ V_43 ] . V_26 , V_532 [ V_43 ] . V_37 ) < 0 )
goto V_53;
}
V_17 -> V_7 -> V_118 = F_7 ( V_17 , V_550 ) ;
if ( V_17 -> V_7 -> V_118 >= 0x20 ) {
if ( F_13 ( V_17 , V_508 , 0x0c ) < 0 )
goto V_53;
F_9 ( V_51 , 1 , L_39 ) ;
for ( V_43 = 0 ; V_43 < V_535 ; V_43 ++ ) {
if ( F_13 ( V_17 , V_533 [ V_43 ] . V_26 , V_533 [ V_43 ] . V_37 ) < 0 )
goto V_53;
}
} else if ( V_17 -> V_7 -> V_118 < 0x20 ) {
F_9 ( V_33 , 1 , L_40 ,
V_17 -> V_7 -> V_118 ) ;
goto V_53;
} else if ( V_17 -> V_7 -> V_118 > 0x30 ) {
F_9 ( V_33 , 1 , L_41 ,
V_17 -> V_7 -> V_118 ) ;
}
V_18 = F_7 ( V_17 , V_466 ) ;
F_32 ( V_18 , V_551 ,
( V_20 -> V_552 == V_553 ) ? 0 : 1 ) ;
if ( F_13 ( V_17 , V_466 , V_18 ) < 0 )
goto V_53;
V_18 = F_7 ( V_17 , V_471 ) ;
F_32 ( V_18 , V_554 ,
( V_20 -> V_555 == V_553 ) ? 0 : 1 ) ;
if ( F_13 ( V_17 , V_471 , V_18 ) < 0 )
goto V_53;
if ( F_13 ( V_17 , V_492 , 0x80 ) < 0 )
goto V_53;
if ( F_13 ( V_17 , V_492 , 0x00 ) < 0 )
goto V_53;
return 0 ;
V_53:
F_9 ( V_33 , 1 , L_10 ) ;
return - 1 ;
}
int F_90 ( struct V_178 * V_179 , T_1 V_556 , T_1 V_161 , T_1 V_453 ,
T_1 V_557 )
{
struct V_16 * V_17 = V_179 -> V_361 ;
T_1 V_18 = 0 ;
F_32 ( V_18 , V_558 , V_161 ) ;
F_32 ( V_18 , V_559 , V_453 ) ;
F_32 ( V_18 , V_560 , V_557 ) ;
return F_13 ( V_17 , F_91 ( V_556 ) , V_18 ) ;
}
struct V_178 * F_92 ( const struct V_19 * V_20 ,
struct V_2 * V_31 ,
enum V_561 V_111 )
{
struct V_16 * V_17 = NULL ;
struct V_1 * V_562 ;
V_17 = F_93 ( sizeof ( struct V_16 ) , V_13 ) ;
if ( V_17 == NULL )
goto error;
V_17 -> V_35 = & V_35 ;
V_17 -> V_20 = V_20 ;
V_17 -> V_31 = V_31 ;
V_17 -> V_48 . V_563 = V_564 ;
V_17 -> V_48 . V_361 = V_17 ;
V_17 -> V_111 = V_111 ;
V_17 -> V_126 = V_20 -> V_126 ;
V_17 -> V_464 = V_20 -> V_464 ;
V_17 -> V_71 = V_75 ;
V_562 = F_1 ( V_17 -> V_31 ,
V_17 -> V_20 -> V_27 ) ;
if ( ( V_562 != NULL ) && ( V_17 -> V_126 == V_490 ) ) {
V_17 -> V_7 = V_562 -> V_7 ;
V_17 -> V_7 -> V_486 ++ ;
F_9 ( V_565 , 1 , L_42 ) ;
} else {
V_17 -> V_7 = F_5 ( sizeof( struct V_9 ) ,
V_13 ) ;
if ( ! V_17 -> V_7 )
goto error;
V_562 = F_4 ( V_17 -> V_7 ) ;
if ( ! V_562 ) {
F_3 ( V_17 -> V_7 ) ;
goto error;
}
V_17 -> V_7 -> V_486 = 1 ;
V_17 -> V_7 -> V_62 = 0 ;
V_17 -> V_7 -> V_118 = 0 ;
V_17 -> V_7 -> V_3 = V_17 -> V_31 ;
V_17 -> V_7 -> V_4 = V_17 -> V_20 -> V_27 ;
F_9 ( V_565 , 1 , L_43 ) ;
F_94 ( & V_17 -> V_7 -> V_113 ) ;
F_94 ( & V_17 -> V_7 -> V_49 ) ;
if ( F_89 ( & V_17 -> V_48 ) < 0 ) {
F_9 ( V_33 , 1 , L_44 ) ;
goto V_566;
}
}
if ( V_17 -> V_7 -> V_118 >= 0x30 )
V_17 -> V_48 . V_563 . V_567 . V_568 |= V_569 ;
if ( V_20 -> V_444 )
F_77 ( & V_17 -> V_48 , V_454 ) ;
F_9 ( V_33 , 1 , L_45 ,
V_17 -> V_464 == V_465 ? L_31 : L_32 ,
V_111 ,
V_17 -> V_7 -> V_118 ) ;
return & V_17 -> V_48 ;
V_566:
F_2 ( V_17 -> V_7 ) ;
F_3 ( V_17 -> V_7 ) ;
error:
F_3 ( V_17 ) ;
return NULL ;
}
