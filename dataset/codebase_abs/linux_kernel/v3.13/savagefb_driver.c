static void F_1 ( struct V_1 * V_2 , int V_3 )
{
if ( V_3 )
F_2 ( 0x00 , 0x01 , V_2 ) ;
else
F_2 ( 0x00 , 0x03 , V_2 ) ;
}
static void F_3 ( struct V_1 * V_2 , int V_4 )
{
unsigned char V_5 ;
if ( V_4 ) {
V_5 = F_4 ( 0x01 , V_2 ) ;
F_1 ( V_2 , 1 ) ;
F_2 ( 0x01 , V_5 | 0x20 , V_2 ) ;
F_5 ( V_2 ) ;
} else {
V_5 = F_4 ( 0x01 , V_2 ) ;
F_2 ( 0x01 , V_5 & ~ 0x20 , V_2 ) ;
F_1 ( V_2 , 0 ) ;
F_6 ( V_2 ) ;
}
}
static void F_7 ( struct V_1 * V_2 , struct V_6 * V_7 )
{
int V_8 ;
F_8 ( V_7 -> V_9 , V_2 ) ;
for ( V_8 = 1 ; V_8 < 5 ; V_8 ++ )
F_2 ( V_8 , V_7 -> V_10 [ V_8 ] , V_2 ) ;
F_9 ( 17 , V_7 -> V_11 [ 17 ] & ~ 0x80 , V_2 ) ;
for ( V_8 = 0 ; V_8 < 25 ; V_8 ++ )
F_9 ( V_8 , V_7 -> V_11 [ V_8 ] , V_2 ) ;
for ( V_8 = 0 ; V_8 < 9 ; V_8 ++ )
F_10 ( V_8 , V_7 -> V_12 [ V_8 ] , V_2 ) ;
F_5 ( V_2 ) ;
for ( V_8 = 0 ; V_8 < 21 ; V_8 ++ )
F_11 ( V_8 , V_7 -> V_13 [ V_8 ] , V_2 ) ;
F_6 ( V_2 ) ;
}
static void F_12 ( struct V_14 * V_15 ,
struct V_1 * V_2 ,
struct V_16 * V_17 ,
struct V_6 * V_7 )
{
V_7 -> V_9 = 0x23 ;
if ( ! ( V_17 -> V_18 & V_19 ) )
V_7 -> V_9 |= 0x40 ;
if ( ! ( V_17 -> V_18 & V_20 ) )
V_7 -> V_9 |= 0x80 ;
V_7 -> V_10 [ 0x00 ] = 0x00 ;
V_7 -> V_10 [ 0x01 ] = 0x01 ;
V_7 -> V_10 [ 0x02 ] = 0x0F ;
V_7 -> V_10 [ 0x03 ] = 0x00 ;
V_7 -> V_10 [ 0x04 ] = 0x0E ;
V_7 -> V_11 [ 0x00 ] = ( V_17 -> V_21 >> 3 ) - 5 ;
V_7 -> V_11 [ 0x01 ] = ( V_17 -> V_22 >> 3 ) - 1 ;
V_7 -> V_11 [ 0x02 ] = ( V_17 -> V_23 >> 3 ) - 1 ;
V_7 -> V_11 [ 0x03 ] = ( ( ( V_17 -> V_24 >> 3 ) - 1 ) & 0x1f ) | 0x80 ;
V_7 -> V_11 [ 0x04 ] = ( V_17 -> V_23 >> 3 ) ;
V_7 -> V_11 [ 0x05 ] = ( ( ( ( V_17 -> V_24 >> 3 ) - 1 ) & 0x20 ) << 2 ) |
( ( ( V_17 -> V_24 >> 3 ) ) & 0x1f ) ;
V_7 -> V_11 [ 0x06 ] = ( V_17 -> V_25 - 2 ) & 0xFF ;
V_7 -> V_11 [ 0x07 ] = ( ( ( V_17 -> V_25 - 2 ) & 0x100 ) >> 8 ) |
( ( ( V_17 -> V_26 - 1 ) & 0x100 ) >> 7 ) |
( ( V_17 -> V_27 & 0x100 ) >> 6 ) |
( ( ( V_17 -> V_27 - 1 ) & 0x100 ) >> 5 ) |
0x10 |
( ( ( V_17 -> V_25 - 2 ) & 0x200 ) >> 4 ) |
( ( ( V_17 -> V_26 - 1 ) & 0x200 ) >> 3 ) |
( ( V_17 -> V_27 & 0x200 ) >> 2 ) ;
V_7 -> V_11 [ 0x08 ] = 0x00 ;
V_7 -> V_11 [ 0x09 ] = ( ( ( V_17 -> V_27 - 1 ) & 0x200 ) >> 4 ) | 0x40 ;
if ( V_17 -> V_28 )
V_7 -> V_11 [ 0x09 ] |= 0x80 ;
V_7 -> V_11 [ 0x0a ] = 0x00 ;
V_7 -> V_11 [ 0x0b ] = 0x00 ;
V_7 -> V_11 [ 0x0c ] = 0x00 ;
V_7 -> V_11 [ 0x0d ] = 0x00 ;
V_7 -> V_11 [ 0x0e ] = 0x00 ;
V_7 -> V_11 [ 0x0f ] = 0x00 ;
V_7 -> V_11 [ 0x10 ] = V_17 -> V_27 & 0xff ;
V_7 -> V_11 [ 0x11 ] = ( V_17 -> V_29 & 0x0f ) | 0x20 ;
V_7 -> V_11 [ 0x12 ] = ( V_17 -> V_26 - 1 ) & 0xff ;
V_7 -> V_11 [ 0x13 ] = V_15 -> V_30 >> 4 ;
V_7 -> V_11 [ 0x14 ] = 0x00 ;
V_7 -> V_11 [ 0x15 ] = ( V_17 -> V_27 - 1 ) & 0xff ;
V_7 -> V_11 [ 0x16 ] = ( V_17 -> V_29 - 1 ) & 0xff ;
V_7 -> V_11 [ 0x17 ] = 0xc3 ;
V_7 -> V_11 [ 0x18 ] = 0xff ;
V_7 -> V_12 [ 0x00 ] = 0x00 ;
V_7 -> V_12 [ 0x01 ] = 0x00 ;
V_7 -> V_12 [ 0x02 ] = 0x00 ;
V_7 -> V_12 [ 0x03 ] = 0x00 ;
V_7 -> V_12 [ 0x04 ] = 0x00 ;
V_7 -> V_12 [ 0x05 ] = 0x40 ;
V_7 -> V_12 [ 0x06 ] = 0x05 ;
V_7 -> V_12 [ 0x07 ] = 0x0F ;
V_7 -> V_12 [ 0x08 ] = 0xFF ;
V_7 -> V_13 [ 0x00 ] = 0x00 ;
V_7 -> V_13 [ 0x01 ] = 0x01 ;
V_7 -> V_13 [ 0x02 ] = 0x02 ;
V_7 -> V_13 [ 0x03 ] = 0x03 ;
V_7 -> V_13 [ 0x04 ] = 0x04 ;
V_7 -> V_13 [ 0x05 ] = 0x05 ;
V_7 -> V_13 [ 0x06 ] = 0x06 ;
V_7 -> V_13 [ 0x07 ] = 0x07 ;
V_7 -> V_13 [ 0x08 ] = 0x08 ;
V_7 -> V_13 [ 0x09 ] = 0x09 ;
V_7 -> V_13 [ 0x0a ] = 0x0A ;
V_7 -> V_13 [ 0x0b ] = 0x0B ;
V_7 -> V_13 [ 0x0c ] = 0x0C ;
V_7 -> V_13 [ 0x0d ] = 0x0D ;
V_7 -> V_13 [ 0x0e ] = 0x0E ;
V_7 -> V_13 [ 0x0f ] = 0x0F ;
V_7 -> V_13 [ 0x10 ] = 0x41 ;
V_7 -> V_13 [ 0x11 ] = 0xFF ;
V_7 -> V_13 [ 0x12 ] = 0x0F ;
V_7 -> V_13 [ 0x13 ] = 0x00 ;
V_7 -> V_13 [ 0x14 ] = 0x00 ;
}
static void
F_13 ( struct V_1 * V_2 , int V_31 )
{
int V_32 = V_33 - V_31 ;
while ( ( F_14 ( 0x48C00 , V_2 ) & 0x0000ffff ) > V_32 ) ;
}
static void
F_15 ( struct V_1 * V_2 , int V_31 )
{
int V_32 = V_33 - V_31 ;
while ( ( F_14 ( 0x48C60 , V_2 ) & 0x001fffff ) > V_32 ) ;
}
static void
F_16 ( struct V_1 * V_2 , int V_31 )
{
int V_32 = V_33 - V_31 ;
while ( ( F_14 ( 0x48C60 , V_2 ) & 0x0000ffff ) > V_32 ) ;
}
static void
F_17 ( struct V_1 * V_2 )
{
while ( ( F_14 ( 0x48C00 , V_2 ) & 0x0008ffff ) != 0x80000 ) ;
}
static void
F_18 ( struct V_1 * V_2 )
{
while ( ( F_14 ( 0x48C60 , V_2 ) & 0x00a00000 ) != 0x00a00000 ) ;
}
static void
F_19 ( struct V_1 * V_2 )
{
while ( ( F_14 ( 0x48C60 , V_2 ) & 0x009fffff ) ) ;
}
static void
F_20 ( struct V_1 * V_2 )
{
unsigned long V_34 ;
V_34 = 1 | 8 | V_35 ;
F_21 ( V_34 , V_2 -> V_36 ) ;
F_22 ( V_34 , V_2 -> V_37 ) ;
switch( V_2 -> V_38 ) {
case V_39 :
case V_40 :
F_23 ( 0x48C18 , F_14 ( 0x48C18 , V_2 ) & 0x3FF0 , V_2 ) ;
F_23 ( 0x48C14 ,
( V_2 -> V_41 >> 11 ) | ( V_2 -> V_42 << 29 ) ,
V_2 ) ;
F_23 ( 0x48C10 , 0x78207220 , V_2 ) ;
F_23 ( 0x48C0C , 0 , V_2 ) ;
F_23 ( 0x48C18 , F_14 ( 0x48C18 , V_2 ) | 0x0C , V_2 ) ;
break;
case V_43 :
case V_44 :
case V_45 :
case V_46 :
case V_47 :
F_23 ( 0x48C18 , F_14 ( 0x48C18 , V_2 ) & 0x3FF0 , V_2 ) ;
F_23 ( 0x48C10 , 0x00700040 , V_2 ) ;
F_23 ( 0x48C0C , 0 , V_2 ) ;
F_23 ( 0x48C18 , F_14 ( 0x48C18 , V_2 ) | 0x08 , V_2 ) ;
break;
case V_48 :
F_23 ( 0x48C18 , 0 , V_2 ) ;
F_23 ( 0x48C18 ,
( V_2 -> V_41 >> 7 ) | ( V_2 -> V_42 ) ,
V_2 ) ;
F_23 ( 0x48A30 , 0 , V_2 ) ;
F_23 ( 0x48C18 , F_14 ( 0x48C18 , V_2 ) | 0x00280000 ,
V_2 ) ;
break;
default:
break;
}
F_24 ( 0x3d4 , 0x31 , V_2 ) ;
F_24 ( 0x3d5 , 0x0c , V_2 ) ;
F_24 ( 0x3d4 , 0x50 , V_2 ) ;
F_24 ( 0x3d5 , F_25 ( 0x3d5 , V_2 ) | 0xC1 , V_2 ) ;
F_24 ( 0x3d4 , 0x40 , V_2 ) ;
F_24 ( 0x3d5 , 0x01 , V_2 ) ;
F_23 ( V_49 , ~ 0 , V_2 ) ;
F_23 ( V_50 , ~ 0 , V_2 ) ;
F_23 ( 0x8128 , ~ 0 , V_2 ) ;
F_23 ( 0x812C , ~ 0 , V_2 ) ;
F_26 ( 0x8134 , 0x27 , V_2 ) ;
F_26 ( 0x8136 , 0x07 , V_2 ) ;
V_2 -> V_51 = 0 ;
V_2 -> V_52 ( V_2 , 4 ) ;
F_27 ( V_53 | ( 1 << 16 ) | V_54 ) ;
F_27 ( 0 ) ;
F_27 ( V_53 | ( 1 << 16 ) | V_55 ) ;
F_27 ( V_34 ) ;
V_2 -> V_51 = 0 ;
V_2 -> V_52 ( V_2 , 4 ) ;
F_27 ( V_53 | ( 1 << 16 ) | V_54 ) ;
F_27 ( 0 ) ;
F_27 ( V_53 | ( 1 << 16 ) | V_55 ) ;
F_27 ( V_34 ) ;
}
static void F_28 ( struct V_56 * V_57 )
{
struct V_1 * V_2 = V_57 -> V_2 ;
int V_58 ;
V_58 = V_59 | V_60 ;
V_2 -> V_51 = 0 ;
V_2 -> V_52 ( V_2 , 3 ) ;
F_27 ( V_58 ) ;
F_27 ( F_29 ( 0 , 0 ) ) ;
F_27 ( F_30 ( 0xfff , 0xfff ) ) ;
}
static void F_20 ( struct V_1 * V_2 ) {}
static void F_31 ( long V_61 , int V_62 , int V_63 , int V_64 ,
int V_65 , int V_66 , long V_67 ,
long V_68 , unsigned int * V_69 ,
unsigned int * V_70 , unsigned int * V_71 )
{
long V_72 , V_73 ;
unsigned int V_74 ;
unsigned char V_75 , V_76 , V_77 = 16 + 2 , V_78 = 2 , V_79 = 125 + 2 ;
if ( V_61 < V_67 / ( 1 << V_66 ) ) {
F_32 ( V_80 L_1 , V_61 ) ;
V_61 = V_67 / ( 1 << V_66 ) ;
}
if ( V_61 > V_68 / ( 1 << V_65 ) ) {
F_32 ( V_80 L_1 , V_61 ) ;
V_61 = V_68 / ( 1 << V_65 ) ;
}
V_73 = V_61 ;
for ( V_76 = V_65 ; V_76 <= V_66 ; V_76 ++ ) {
for ( V_75 = V_63 + 2 ; V_75 <= V_64 + 2 ; V_75 ++ ) {
V_74 = ( V_61 * V_75 * ( 1 << V_76 ) + V_81 ) /
V_82 ;
if ( V_74 < V_62 + 2 || V_74 > 127 + 2 )
continue;
if ( ( V_74 * V_82 >= V_67 * V_75 ) &&
( V_74 * V_82 <= V_68 * V_75 ) ) {
V_72 = V_61 * ( 1 << V_76 ) * V_75 - V_82 * V_74 ;
if ( V_72 < 0 )
V_72 = - V_72 ;
if ( V_72 < V_73 ) {
V_73 = V_72 ;
V_79 = V_74 ;
V_77 = V_75 ;
V_78 = V_76 ;
}
}
}
}
* V_70 = V_77 - 2 ;
* V_71 = V_78 ;
* V_69 = V_79 - 2 ;
}
static int F_33 ( long V_61 , int V_62 , int V_63 , int V_64 ,
int V_65 , int V_66 , long V_67 ,
long V_68 , unsigned char * V_69 ,
unsigned char * V_70 )
{
long V_72 , V_73 ;
unsigned int V_74 ;
unsigned char V_75 , V_76 ;
unsigned char V_77 = 16 + 2 , V_78 = 2 , V_79 = 125 + 2 ;
V_73 = V_61 ;
for ( V_76 = V_65 ; V_76 <= V_66 ; V_76 ++ ) {
for ( V_75 = V_63 + 2 ; V_75 <= V_64 + 2 ; V_75 ++ ) {
V_74 = ( V_61 * V_75 * ( 1 << V_76 ) + V_81 ) /
V_82 ;
if ( V_74 < V_62 + 2 || V_74 > 127 + 2 )
continue;
if ( ( V_74 * V_82 >= V_67 * V_75 ) &&
( V_74 * V_82 <= V_68 * V_75 ) ) {
V_72 = V_61 * ( 1 << V_76 ) * V_75 - V_82 * V_74 ;
if ( V_72 < 0 )
V_72 = - V_72 ;
if ( V_72 < V_73 ) {
V_73 = V_72 ;
V_79 = V_74 ;
V_77 = V_75 ;
V_78 = V_76 ;
}
}
}
}
if ( V_64 == 63 )
* V_70 = ( V_77 - 2 ) | ( V_78 << 6 ) ;
else
* V_70 = ( V_77 - 2 ) | ( V_78 << 5 ) ;
* V_69 = V_79 - 2 ;
return 0 ;
}
static void F_34 ( struct V_1 * V_2 )
{
unsigned char V_8 ;
int V_83 = 0x3d4 ;
int V_84 = 0x3d5 ;
F_32 ( V_85 L_2
L_3 ) ;
for ( V_8 = 0 ; V_8 < 0x70 ; V_8 ++ ) {
if ( ! ( V_8 % 16 ) )
F_32 ( V_85 L_4 , V_8 >> 4 ) ;
F_24 ( 0x3c4 , V_8 , V_2 ) ;
F_32 ( V_85 L_5 , F_25 ( 0x3c5 , V_2 ) ) ;
}
F_32 ( V_85 L_6
L_7 ) ;
for ( V_8 = 0 ; V_8 < 0xB7 ; V_8 ++ ) {
if ( ! ( V_8 % 16 ) )
F_32 ( V_85 L_8 , V_8 >> 4 ) ;
F_24 ( V_83 , V_8 , V_2 ) ;
F_32 ( V_85 L_5 , F_25 ( V_84 , V_2 ) ) ;
}
F_32 ( V_85 L_9 ) ;
}
static void F_35 ( struct V_1 * V_2 , struct V_6 * V_7 )
{
unsigned char V_86 , V_87 , V_88 ;
F_36 ( 0x3d4 , 0x4838 , V_2 ) ;
F_36 ( 0x3d4 , 0xa039 , V_2 ) ;
F_36 ( 0x3c4 , 0x0608 , V_2 ) ;
F_24 ( 0x3d4 , 0x66 , V_2 ) ;
V_88 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d5 , V_88 | 0x80 , V_2 ) ;
F_24 ( 0x3d4 , 0x3a , V_2 ) ;
V_86 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d5 , V_86 | 0x80 , V_2 ) ;
F_24 ( 0x3d4 , 0x53 , V_2 ) ;
V_87 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d5 , V_87 & 0x7f , V_2 ) ;
F_24 ( 0x3d4 , 0x66 , V_2 ) ;
F_24 ( 0x3d5 , V_88 , V_2 ) ;
F_24 ( 0x3d4 , 0x3a , V_2 ) ;
F_24 ( 0x3d5 , V_86 , V_2 ) ;
F_24 ( 0x3d4 , 0x66 , V_2 ) ;
F_24 ( 0x3d5 , V_88 , V_2 ) ;
F_24 ( 0x3d4 , 0x3a , V_2 ) ;
F_24 ( 0x3d5 , V_86 , V_2 ) ;
F_24 ( 0x3c4 , 0x08 , V_2 ) ;
V_7 -> V_89 = F_25 ( 0x3c5 , V_2 ) ;
F_24 ( 0x3c5 , 0x06 , V_2 ) ;
F_24 ( 0x3d4 , 0x31 , V_2 ) ;
V_7 -> V_90 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x32 , V_2 ) ;
V_7 -> V_91 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x34 , V_2 ) ;
V_7 -> V_92 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x36 , V_2 ) ;
V_7 -> V_93 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x3a , V_2 ) ;
V_7 -> V_94 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x40 , V_2 ) ;
V_7 -> V_95 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x42 , V_2 ) ;
V_7 -> V_96 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x45 , V_2 ) ;
V_7 -> V_97 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x50 , V_2 ) ;
V_7 -> V_98 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x51 , V_2 ) ;
V_7 -> V_99 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x53 , V_2 ) ;
V_7 -> V_100 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x58 , V_2 ) ;
V_7 -> V_101 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x60 , V_2 ) ;
V_7 -> V_102 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x66 , V_2 ) ;
V_7 -> V_103 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x67 , V_2 ) ;
V_7 -> V_104 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x68 , V_2 ) ;
V_7 -> V_105 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x69 , V_2 ) ;
V_7 -> V_106 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x6f , V_2 ) ;
V_7 -> V_107 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x33 , V_2 ) ;
V_7 -> V_108 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x86 , V_2 ) ;
V_7 -> V_109 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x88 , V_2 ) ;
V_7 -> V_110 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x90 , V_2 ) ;
V_7 -> V_111 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x91 , V_2 ) ;
V_7 -> V_112 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0xb0 , V_2 ) ;
V_7 -> V_113 = F_25 ( 0x3d5 , V_2 ) | 0x80 ;
F_24 ( 0x3d4 , 0x3b , V_2 ) ;
V_7 -> V_114 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x3c , V_2 ) ;
V_7 -> V_115 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x43 , V_2 ) ;
V_7 -> V_116 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x5d , V_2 ) ;
V_7 -> V_117 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x5e , V_2 ) ;
V_7 -> V_118 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x65 , V_2 ) ;
V_7 -> V_119 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3c4 , 0x0e , V_2 ) ;
V_7 -> V_120 = F_25 ( 0x3c5 , V_2 ) ;
F_24 ( 0x3c4 , 0x0f , V_2 ) ;
V_7 -> V_121 = F_25 ( 0x3c5 , V_2 ) ;
F_24 ( 0x3c4 , 0x10 , V_2 ) ;
V_7 -> V_122 = F_25 ( 0x3c5 , V_2 ) ;
F_24 ( 0x3c4 , 0x11 , V_2 ) ;
V_7 -> V_123 = F_25 ( 0x3c5 , V_2 ) ;
F_24 ( 0x3c4 , 0x12 , V_2 ) ;
V_7 -> V_124 = F_25 ( 0x3c5 , V_2 ) ;
F_24 ( 0x3c4 , 0x13 , V_2 ) ;
V_7 -> V_125 = F_25 ( 0x3c5 , V_2 ) ;
F_24 ( 0x3c4 , 0x29 , V_2 ) ;
V_7 -> V_126 = F_25 ( 0x3c5 , V_2 ) ;
F_24 ( 0x3c4 , 0x15 , V_2 ) ;
V_7 -> V_127 = F_25 ( 0x3c5 , V_2 ) ;
F_24 ( 0x3c4 , 0x30 , V_2 ) ;
V_7 -> V_128 = F_25 ( 0x3c5 , V_2 ) ;
F_24 ( 0x3c4 , 0x18 , V_2 ) ;
V_7 -> V_129 = F_25 ( 0x3c5 , V_2 ) ;
if ( V_2 -> V_38 == V_40 ) {
int V_8 ;
for ( V_8 = 0 ; V_8 < 8 ; V_8 ++ ) {
F_24 ( 0x3c4 , 0x54 + V_8 , V_2 ) ;
V_7 -> V_130 [ V_8 ] = F_25 ( 0x3c5 , V_2 ) ;
}
}
F_24 ( 0x3d4 , 0x66 , V_2 ) ;
V_88 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d5 , V_88 | 0x80 , V_2 ) ;
F_24 ( 0x3d4 , 0x3a , V_2 ) ;
V_86 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d5 , V_86 | 0x80 , V_2 ) ;
if ( V_2 -> V_38 != V_40 ) {
V_7 -> V_131 = F_14 ( V_132 , V_2 ) ;
V_7 -> V_133 = F_14 ( V_134 , V_2 ) ;
V_7 -> V_135 = F_14 ( V_136 , V_2 ) ;
V_7 -> V_137 = F_14 ( V_138 , V_2 ) ;
}
F_24 ( 0x3d4 , 0x3a , V_2 ) ;
F_24 ( 0x3d5 , V_86 , V_2 ) ;
F_24 ( 0x3d4 , 0x66 , V_2 ) ;
F_24 ( 0x3d5 , V_88 , V_2 ) ;
}
static void F_37 ( struct V_1 * V_2 ,
struct V_6 * V_7 )
{
unsigned char V_86 , V_87 , V_88 ;
F_36 ( 0x3d4 , 0x4838 , V_2 ) ;
F_36 ( 0x3d4 , 0xa039 , V_2 ) ;
F_36 ( 0x3c4 , 0x0608 , V_2 ) ;
F_24 ( 0x3d4 , 0x66 , V_2 ) ;
V_88 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d5 , V_88 | 0x80 , V_2 ) ;
F_24 ( 0x3d4 , 0x3a , V_2 ) ;
V_86 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d5 , V_86 | 0x80 , V_2 ) ;
F_24 ( 0x3d4 , 0x53 , V_2 ) ;
V_87 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d5 , V_87 & 0x7f , V_2 ) ;
F_24 ( 0x3d4 , 0x66 , V_2 ) ;
F_24 ( 0x3d5 , V_88 , V_2 ) ;
F_24 ( 0x3d4 , 0x3a , V_2 ) ;
F_24 ( 0x3d5 , V_86 , V_2 ) ;
F_24 ( 0x3d4 , 0x66 , V_2 ) ;
F_24 ( 0x3d5 , V_88 , V_2 ) ;
F_24 ( 0x3d4 , 0x3a , V_2 ) ;
F_24 ( 0x3d5 , V_86 , V_2 ) ;
F_24 ( 0x3c4 , 0x08 , V_2 ) ;
F_24 ( 0x3c5 , V_7 -> V_89 , V_2 ) ;
F_24 ( 0x3c5 , 0x06 , V_2 ) ;
F_24 ( 0x3d4 , 0x31 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_90 , V_2 ) ;
F_24 ( 0x3d4 , 0x32 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_91 , V_2 ) ;
F_24 ( 0x3d4 , 0x34 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_92 , V_2 ) ;
F_24 ( 0x3d4 , 0x36 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_93 , V_2 ) ;
F_24 ( 0x3d4 , 0x3a , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_94 , V_2 ) ;
F_24 ( 0x3d4 , 0x40 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_95 , V_2 ) ;
F_24 ( 0x3d4 , 0x42 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_96 , V_2 ) ;
F_24 ( 0x3d4 , 0x45 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_97 , V_2 ) ;
F_24 ( 0x3d4 , 0x50 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_98 , V_2 ) ;
F_24 ( 0x3d4 , 0x51 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_99 , V_2 ) ;
F_24 ( 0x3d4 , 0x53 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_100 , V_2 ) ;
F_24 ( 0x3d4 , 0x58 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_101 , V_2 ) ;
F_24 ( 0x3d4 , 0x60 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_102 , V_2 ) ;
F_24 ( 0x3d4 , 0x66 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_103 , V_2 ) ;
F_24 ( 0x3d4 , 0x67 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_104 , V_2 ) ;
F_24 ( 0x3d4 , 0x68 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_105 , V_2 ) ;
F_24 ( 0x3d4 , 0x69 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_106 , V_2 ) ;
F_24 ( 0x3d4 , 0x6f , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_107 , V_2 ) ;
F_24 ( 0x3d4 , 0x33 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_108 , V_2 ) ;
F_24 ( 0x3d4 , 0x86 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_109 , V_2 ) ;
F_24 ( 0x3d4 , 0x88 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_110 , V_2 ) ;
F_24 ( 0x3d4 , 0x90 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_111 , V_2 ) ;
F_24 ( 0x3d4 , 0x91 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_112 , V_2 ) ;
F_24 ( 0x3d4 , 0xb0 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_113 , V_2 ) ;
F_24 ( 0x3d4 , 0x3b , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_114 , V_2 ) ;
F_24 ( 0x3d4 , 0x3c , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_115 , V_2 ) ;
F_24 ( 0x3d4 , 0x43 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_116 , V_2 ) ;
F_24 ( 0x3d4 , 0x5d , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_117 , V_2 ) ;
F_24 ( 0x3d4 , 0x5e , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_118 , V_2 ) ;
F_24 ( 0x3d4 , 0x65 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_119 , V_2 ) ;
F_24 ( 0x3c4 , 0x0e , V_2 ) ;
F_24 ( 0x3c5 , V_7 -> V_120 , V_2 ) ;
F_24 ( 0x3c4 , 0x0f , V_2 ) ;
F_24 ( 0x3c5 , V_7 -> V_121 , V_2 ) ;
F_24 ( 0x3c4 , 0x10 , V_2 ) ;
F_24 ( 0x3c5 , V_7 -> V_122 , V_2 ) ;
F_24 ( 0x3c4 , 0x11 , V_2 ) ;
F_24 ( 0x3c5 , V_7 -> V_123 , V_2 ) ;
F_24 ( 0x3c4 , 0x12 , V_2 ) ;
F_24 ( 0x3c5 , V_7 -> V_124 , V_2 ) ;
F_24 ( 0x3c4 , 0x13 , V_2 ) ;
F_24 ( 0x3c5 , V_7 -> V_125 , V_2 ) ;
F_24 ( 0x3c4 , 0x29 , V_2 ) ;
F_24 ( 0x3c5 , V_7 -> V_126 , V_2 ) ;
F_24 ( 0x3c4 , 0x15 , V_2 ) ;
F_24 ( 0x3c5 , V_7 -> V_127 , V_2 ) ;
F_24 ( 0x3c4 , 0x30 , V_2 ) ;
F_24 ( 0x3c5 , V_7 -> V_128 , V_2 ) ;
F_24 ( 0x3c4 , 0x18 , V_2 ) ;
F_24 ( 0x3c5 , V_7 -> V_129 , V_2 ) ;
if ( V_2 -> V_38 == V_40 ) {
int V_8 ;
for ( V_8 = 0 ; V_8 < 8 ; V_8 ++ ) {
F_24 ( 0x3c4 , 0x54 + V_8 , V_2 ) ;
F_24 ( 0x3c5 , V_7 -> V_130 [ V_8 ] , V_2 ) ;
}
}
F_24 ( 0x3d4 , 0x66 , V_2 ) ;
V_88 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d5 , V_88 | 0x80 , V_2 ) ;
F_24 ( 0x3d4 , 0x3a , V_2 ) ;
V_86 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d5 , V_86 | 0x80 , V_2 ) ;
if ( V_2 -> V_38 != V_40 ) {
F_23 ( V_132 , V_7 -> V_131 , V_2 ) ;
F_23 ( V_134 , V_7 -> V_133 , V_2 ) ;
F_23 ( V_136 , V_7 -> V_135 , V_2 ) ;
F_23 ( V_138 , V_7 -> V_137 , V_2 ) ;
}
F_24 ( 0x3d4 , 0x3a , V_2 ) ;
F_24 ( 0x3d5 , V_86 , V_2 ) ;
F_24 ( 0x3d4 , 0x66 , V_2 ) ;
F_24 ( 0x3d5 , V_88 , V_2 ) ;
}
static void F_38 ( struct V_14 * V_15 ,
const struct V_139 * V_140 )
{
V_15 -> V_141 = V_15 -> V_30 = V_140 -> V_141 ;
V_15 -> V_142 = V_140 -> V_142 ;
if ( V_15 -> V_143 < V_15 -> V_142 )
V_15 -> V_143 = V_15 -> V_142 ;
V_15 -> V_144 = V_15 -> V_145 = 0 ;
V_15 -> V_146 = V_140 -> V_146 ;
V_15 -> V_147 = V_140 -> V_147 ;
V_15 -> V_148 = V_140 -> V_148 ;
V_15 -> V_149 = V_140 -> V_149 ;
V_15 -> V_150 = V_140 -> V_150 ;
V_15 -> V_151 = V_140 -> V_151 ;
V_15 -> V_152 = V_140 -> V_152 ;
V_15 -> V_18 = V_140 -> V_18 ;
V_15 -> V_153 = V_140 -> V_153 ;
}
static int F_39 ( struct V_14 * V_15 ,
struct V_56 * V_57 )
{
struct V_1 * V_2 = V_57 -> V_2 ;
int V_154 , V_155 , V_156 = 0 ;
F_40 ( L_10 ) ;
V_15 -> V_157 . V_158 = 0 ;
V_15 -> V_157 . V_159 = 0 ;
switch ( V_15 -> V_160 ) {
case 8 :
V_15 -> V_161 . V_158 = V_15 -> V_162 . V_158 =
V_15 -> V_163 . V_158 = 0 ;
V_15 -> V_161 . V_159 = V_15 -> V_162 . V_159 =
V_15 -> V_163 . V_159 = V_15 -> V_160 ;
break;
case 16 :
V_15 -> V_161 . V_158 = 11 ;
V_15 -> V_161 . V_159 = 5 ;
V_15 -> V_162 . V_158 = 5 ;
V_15 -> V_162 . V_159 = 6 ;
V_15 -> V_163 . V_158 = 0 ;
V_15 -> V_163 . V_159 = 5 ;
break;
case 32 :
V_15 -> V_157 . V_158 = 24 ;
V_15 -> V_157 . V_159 = 8 ;
V_15 -> V_161 . V_158 = 16 ;
V_15 -> V_161 . V_159 = 8 ;
V_15 -> V_162 . V_158 = 8 ;
V_15 -> V_162 . V_159 = 8 ;
V_15 -> V_163 . V_158 = 0 ;
V_15 -> V_163 . V_159 = 8 ;
break;
default:
return - V_164 ;
}
if ( ! V_57 -> V_165 . V_166 || ! V_57 -> V_165 . V_167 ||
! V_57 -> V_165 . V_168 || ! F_41 ( V_15 , V_57 ) )
V_156 = 1 ;
if ( ! V_156 && V_57 -> V_165 . V_169 ) {
if ( ! F_42 ( V_170 , 0 , V_15 , V_57 ) )
V_156 = 1 ;
}
if ( ! V_156 ) {
const struct V_139 * V_171 ;
V_171 = F_43 ( V_15 , & V_57 -> V_172 ) ;
if ( V_171 ) {
F_38 ( V_15 , V_171 ) ;
V_156 = 1 ;
}
}
if ( ! V_156 && V_57 -> V_165 . V_173 )
return - V_164 ;
if ( V_2 -> V_174 &&
( V_15 -> V_141 > V_2 -> V_174 ||
V_15 -> V_142 > V_2 -> V_175 ) ) {
F_32 ( V_176 L_11
L_12 , V_15 -> V_141 , V_15 -> V_142 ,
V_2 -> V_174 ,
V_2 -> V_175 ) ;
return - 1 ;
}
if ( V_15 -> V_143 < V_15 -> V_142 )
V_15 -> V_143 = V_15 -> V_142 ;
if ( V_15 -> V_30 < V_15 -> V_141 )
V_15 -> V_30 = V_15 -> V_141 ;
V_155 = V_57 -> V_177 . V_178 ;
V_154 = V_15 -> V_30 * V_15 -> V_160 *
V_15 -> V_143 / 8 ;
if ( V_154 > V_155 ) {
V_15 -> V_143 = V_155 * 8 /
( V_15 -> V_30 * V_15 -> V_160 ) ;
V_154 = V_15 -> V_30 * V_15 -> V_160 *
V_15 -> V_143 / 8 ;
}
if ( V_15 -> V_143 < V_15 -> V_142 )
V_15 -> V_142 = V_15 -> V_143 ;
if ( V_15 -> V_30 < V_15 -> V_141 )
V_15 -> V_141 = V_15 -> V_30 ;
if ( V_15 -> V_144 + V_15 -> V_141 > V_15 -> V_30 )
V_15 -> V_144 = V_15 -> V_30 - V_15 -> V_141 ;
if ( V_15 -> V_145 + V_15 -> V_142 > V_15 -> V_143 )
V_15 -> V_145 = V_15 -> V_143 - V_15 -> V_142 ;
return 0 ;
}
static int F_44 ( struct V_14 * V_15 ,
struct V_1 * V_2 ,
struct V_6 * V_7 )
{
struct V_16 V_17 ;
int V_179 , V_180 , V_8 , V_181 ;
unsigned int V_74 , V_182 , V_71 ;
unsigned char V_5 = 0 ;
unsigned int V_146 = V_15 -> V_146 ;
F_40 ( L_13 ) ;
memset ( & V_17 , 0 , sizeof( V_17 ) ) ;
if ( ! V_146 ) V_146 = 10000 ;
V_17 . Clock = 1000000000 / V_146 ;
if ( V_17 . Clock < 1 ) V_17 . Clock = 1 ;
V_17 . V_28 = V_15 -> V_153 & V_183 ;
V_17 . V_184 = V_15 -> V_153 & V_185 ;
V_17 . V_22 = V_15 -> V_141 ;
V_17 . V_23 = V_17 . V_22 + V_15 -> V_148 ;
V_17 . V_24 = V_17 . V_23 + V_15 -> V_151 ;
V_17 . V_21 = V_17 . V_24 + V_15 -> V_147 ;
V_17 . V_26 = V_15 -> V_142 ;
V_17 . V_27 = V_17 . V_26 + V_15 -> V_150 ;
V_17 . V_29 = V_17 . V_27 + V_15 -> V_152 ;
V_17 . V_25 = V_17 . V_29 + V_15 -> V_149 ;
V_17 . V_18 = V_15 -> V_18 ;
V_2 -> V_36 = V_15 -> V_160 ;
V_2 -> V_37 = V_15 -> V_30 ;
if ( V_15 -> V_160 == 16 && V_2 -> V_38 == V_39 ) {
V_17 . V_22 *= 2 ;
V_17 . V_23 *= 2 ;
V_17 . V_24 *= 2 ;
V_17 . V_21 *= 2 ;
}
F_12 ( V_15 , V_2 , & V_17 , V_7 ) ;
V_180 = V_17 . Clock ;
V_7 -> V_104 = 0x00 ;
switch( V_15 -> V_160 ) {
case 8 :
if ( ( V_2 -> V_38 == V_48 ) && ( V_180 >= 230000 ) )
V_7 -> V_104 = 0x10 ;
else
V_7 -> V_104 = 0x00 ;
break;
case 15 :
if ( F_45 ( V_2 -> V_38 ) ||
( ( V_2 -> V_38 == V_48 ) && ( V_180 >= 230000 ) ) )
V_7 -> V_104 = 0x30 ;
else
V_7 -> V_104 = 0x20 ;
break;
case 16 :
if ( F_45 ( V_2 -> V_38 ) ||
( ( V_2 -> V_38 == V_48 ) && ( V_180 >= 230000 ) ) )
V_7 -> V_104 = 0x50 ;
else
V_7 -> V_104 = 0x40 ;
break;
case 24 :
V_7 -> V_104 = 0x70 ;
break;
case 32 :
V_7 -> V_104 = 0xd0 ;
break;
}
F_24 ( 0x3d4 , 0x3a , V_2 ) ;
V_5 = F_25 ( 0x3d5 , V_2 ) ;
if ( 1 )
V_7 -> V_94 = ( V_5 & 0x7f ) | 0x15 ;
else
V_7 -> V_94 = V_5 | 0x95 ;
V_7 -> V_100 = 0x00 ;
V_7 -> V_90 = 0x8c ;
V_7 -> V_103 = 0x89 ;
F_24 ( 0x3d4 , 0x58 , V_2 ) ;
V_7 -> V_101 = F_25 ( 0x3d5 , V_2 ) & 0x80 ;
V_7 -> V_101 |= 0x13 ;
V_7 -> V_127 = 0x03 | 0x80 ;
V_7 -> V_129 = 0x00 ;
V_7 -> V_116 = V_7 -> V_97 = V_7 -> V_119 = 0x00 ;
F_24 ( 0x3d4 , 0x40 , V_2 ) ;
V_7 -> V_95 = F_25 ( 0x3d5 , V_2 ) & ~ 0x01 ;
V_7 -> V_131 = 0x010400 ;
V_7 -> V_133 = 0x00 ;
V_7 -> V_135 = 0x0808 ;
V_7 -> V_137 = 0x08080810 ;
F_31 ( V_180 , 1 , 1 , 127 , 0 , 4 , 180000 , 360000 , & V_74 , & V_182 , & V_71 ) ;
if ( V_2 -> V_186 <= 0 ) {
V_7 -> V_122 = 255 ;
V_7 -> V_123 = 255 ;
} else {
F_33 ( V_2 -> V_186 , 1 , 1 , 31 , 0 , 3 , 135000 , 270000 ,
& V_7 -> V_123 , & V_7 -> V_122 ) ;
}
V_7 -> V_124 = ( V_71 << 6 ) | ( V_182 & 0x3f ) ;
V_7 -> V_125 = V_74 & 0xff ;
V_7 -> V_126 = ( V_71 & 4 ) | ( V_74 & 0x100 ) >> 5 | ( V_182 & 0x40 ) >> 2 ;
if ( V_15 -> V_160 < 24 )
V_7 -> V_131 -= 0x8000 ;
else
V_7 -> V_131 -= 0x4000 ;
if ( V_17 . V_184 )
V_7 -> V_96 = 0x20 ;
else
V_7 -> V_96 = 0x00 ;
V_7 -> V_92 = 0x10 ;
V_8 = ( ( ( ( V_17 . V_21 >> 3 ) - 5 ) & 0x100 ) >> 8 ) |
( ( ( ( V_17 . V_22 >> 3 ) - 1 ) & 0x100 ) >> 7 ) |
( ( ( ( V_17 . V_23 >> 3 ) - 1 ) & 0x100 ) >> 6 ) |
( ( V_17 . V_23 & 0x800 ) >> 7 ) ;
if ( ( V_17 . V_24 >> 3 ) - ( V_17 . V_23 >> 3 ) > 64 )
V_8 |= 0x08 ;
if ( ( V_17 . V_24 >> 3 ) - ( V_17 . V_23 >> 3 ) > 32 )
V_8 |= 0x20 ;
V_181 = ( V_7 -> V_11 [ 0 ] + ( ( V_8 & 0x01 ) << 8 ) +
V_7 -> V_11 [ 4 ] + ( ( V_8 & 0x10 ) << 4 ) + 1 ) / 2 ;
if ( V_181 - ( V_7 -> V_11 [ 4 ] + ( ( V_8 & 0x10 ) << 4 ) ) < 4 ) {
if ( V_7 -> V_11 [ 4 ] + ( ( V_8 & 0x10 ) << 4 ) + 4 <=
V_7 -> V_11 [ 0 ] + ( ( V_8 & 0x01 ) << 8 ) )
V_181 = V_7 -> V_11 [ 4 ] + ( ( V_8 & 0x10 ) << 4 ) + 4 ;
else
V_181 = V_7 -> V_11 [ 0 ] + ( ( V_8 & 0x01 ) << 8 ) + 1 ;
}
V_7 -> V_114 = V_181 & 0xff ;
V_8 |= ( V_181 & 0x100 ) >> 2 ;
V_7 -> V_115 = ( V_7 -> V_11 [ 0 ] + ( ( V_8 & 0x01 ) << 8 ) ) / 2 ;
V_7 -> V_117 = V_8 ;
V_7 -> V_118 = ( ( ( V_17 . V_25 - 2 ) & 0x400 ) >> 10 ) |
( ( ( V_17 . V_26 - 1 ) & 0x400 ) >> 9 ) |
( ( ( V_17 . V_27 ) & 0x400 ) >> 8 ) |
( ( ( V_17 . V_27 ) & 0x400 ) >> 6 ) | 0x40 ;
V_179 = ( V_15 -> V_30 * ( ( V_15 -> V_160 + 7 ) / 8 ) ) >> 3 ;
V_7 -> V_112 = V_7 -> V_11 [ 19 ] = 0xff & V_179 ;
V_7 -> V_99 = ( 0x300 & V_179 ) >> 4 ;
V_7 -> V_111 = 0x80 | ( V_179 >> 8 ) ;
V_7 -> V_9 |= 0x0c ;
if ( V_15 -> V_160 <= 8 )
V_7 -> V_98 = 0 ;
else if ( V_15 -> V_160 <= 16 )
V_7 -> V_98 = 0x10 ;
else
V_7 -> V_98 = 0x30 ;
if ( V_15 -> V_30 <= 640 )
V_7 -> V_98 |= 0x40 ;
else if ( V_15 -> V_30 == 800 )
V_7 -> V_98 |= 0x80 ;
else if ( V_15 -> V_30 == 1024 )
V_7 -> V_98 |= 0x00 ;
else if ( V_15 -> V_30 == 1152 )
V_7 -> V_98 |= 0x01 ;
else if ( V_15 -> V_30 == 1280 )
V_7 -> V_98 |= 0xc0 ;
else if ( V_15 -> V_30 == 1600 )
V_7 -> V_98 |= 0x81 ;
else
V_7 -> V_98 |= 0xc1 ;
if ( V_2 -> V_38 == V_48 )
V_7 -> V_108 = 0x08 ;
else
V_7 -> V_108 = 0x20 ;
V_7 -> V_11 [ 0x17 ] = 0xeb ;
V_7 -> V_104 |= 1 ;
F_24 ( 0x3d4 , 0x36 , V_2 ) ;
V_7 -> V_93 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x68 , V_2 ) ;
V_7 -> V_105 = F_25 ( 0x3d5 , V_2 ) ;
V_7 -> V_106 = 0 ;
F_24 ( 0x3d4 , 0x6f , V_2 ) ;
V_7 -> V_107 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x86 , V_2 ) ;
V_7 -> V_109 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d4 , 0x88 , V_2 ) ;
V_7 -> V_110 = F_25 ( 0x3d5 , V_2 ) | 0x08 ;
F_24 ( 0x3d4 , 0xb0 , V_2 ) ;
V_7 -> V_113 = F_25 ( 0x3d5 , V_2 ) | 0x80 ;
return 0 ;
}
static int F_46 ( unsigned V_187 ,
unsigned V_161 ,
unsigned V_162 ,
unsigned V_163 ,
unsigned V_157 ,
struct V_56 * V_57 )
{
struct V_1 * V_2 = V_57 -> V_2 ;
if ( V_187 >= V_188 )
return - V_164 ;
V_2 -> V_189 [ V_187 ] . V_161 = V_161 ;
V_2 -> V_189 [ V_187 ] . V_162 = V_162 ;
V_2 -> V_189 [ V_187 ] . V_163 = V_163 ;
V_2 -> V_189 [ V_187 ] . V_157 = V_157 ;
switch ( V_57 -> V_15 . V_160 ) {
case 8 :
F_24 ( 0x3c8 , V_187 , V_2 ) ;
F_24 ( 0x3c9 , V_161 >> 10 , V_2 ) ;
F_24 ( 0x3c9 , V_162 >> 10 , V_2 ) ;
F_24 ( 0x3c9 , V_163 >> 10 , V_2 ) ;
break;
case 16 :
if ( V_187 < 16 )
( ( V_190 * ) V_57 -> V_191 ) [ V_187 ] =
( ( V_161 & 0xf800 ) ) |
( ( V_162 & 0xfc00 ) >> 5 ) |
( ( V_163 & 0xf800 ) >> 11 ) ;
break;
case 24 :
if ( V_187 < 16 )
( ( V_190 * ) V_57 -> V_191 ) [ V_187 ] =
( ( V_161 & 0xff00 ) << 8 ) |
( ( V_162 & 0xff00 ) ) |
( ( V_163 & 0xff00 ) >> 8 ) ;
break;
case 32 :
if ( V_187 < 16 )
( ( V_190 * ) V_57 -> V_191 ) [ V_187 ] =
( ( V_157 & 0xff00 ) << 16 ) |
( ( V_161 & 0xff00 ) << 8 ) |
( ( V_162 & 0xff00 ) ) |
( ( V_163 & 0xff00 ) >> 8 ) ;
break;
default:
return 1 ;
}
return 0 ;
}
static void F_47 ( struct V_1 * V_2 , struct V_6 * V_7 )
{
unsigned char V_5 , V_86 , V_88 , V_192 ;
F_40 ( L_14 ) ;
V_2 -> V_193 ( V_2 ) ;
F_24 ( 0x3c2 , 0x23 , V_2 ) ;
F_36 ( 0x3d4 , 0x4838 , V_2 ) ;
F_36 ( 0x3d4 , 0xa539 , V_2 ) ;
F_36 ( 0x3c4 , 0x0608 , V_2 ) ;
F_3 ( V_2 , 1 ) ;
F_48 ( V_2 ) ;
F_24 ( 0x3d4 , 0x67 , V_2 ) ;
V_192 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d5 , V_192 & ~ 0x0c , V_2 ) ;
F_24 ( 0x3d4 , 0x23 , V_2 ) ;
F_24 ( 0x3d5 , 0x00 , V_2 ) ;
F_24 ( 0x3d4 , 0x26 , V_2 ) ;
F_24 ( 0x3d5 , 0x00 , V_2 ) ;
F_24 ( 0x3d4 , 0x66 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_103 , V_2 ) ;
F_24 ( 0x3d4 , 0x3a , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_94 , V_2 ) ;
F_24 ( 0x3d4 , 0x31 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_90 , V_2 ) ;
F_24 ( 0x3d4 , 0x32 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_91 , V_2 ) ;
F_24 ( 0x3d4 , 0x58 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_101 , V_2 ) ;
F_24 ( 0x3d4 , 0x53 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_100 & 0x7f , V_2 ) ;
F_36 ( 0x3c4 , 0x0608 , V_2 ) ;
F_24 ( 0x3c4 , 0x0e , V_2 ) ;
F_24 ( 0x3c5 , V_7 -> V_120 , V_2 ) ;
F_24 ( 0x3c4 , 0x0f , V_2 ) ;
F_24 ( 0x3c5 , V_7 -> V_121 , V_2 ) ;
F_24 ( 0x3c4 , 0x29 , V_2 ) ;
F_24 ( 0x3c5 , V_7 -> V_126 , V_2 ) ;
F_24 ( 0x3c4 , 0x15 , V_2 ) ;
F_24 ( 0x3c5 , V_7 -> V_127 , V_2 ) ;
if ( V_2 -> V_38 == V_40 ) {
int V_8 ;
for ( V_8 = 0 ; V_8 < 8 ; V_8 ++ ) {
F_24 ( 0x3c4 , 0x54 + V_8 , V_2 ) ;
F_24 ( 0x3c5 , V_7 -> V_130 [ V_8 ] , V_2 ) ;
}
}
F_7 ( V_2 , V_7 ) ;
F_24 ( 0x3d4 , 0x53 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_100 , V_2 ) ;
F_24 ( 0x3d4 , 0x5d , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_117 , V_2 ) ;
F_24 ( 0x3d4 , 0x5e , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_118 , V_2 ) ;
F_24 ( 0x3d4 , 0x3b , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_114 , V_2 ) ;
F_24 ( 0x3d4 , 0x3c , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_115 , V_2 ) ;
F_24 ( 0x3d4 , 0x43 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_116 , V_2 ) ;
F_24 ( 0x3d4 , 0x65 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_119 , V_2 ) ;
F_24 ( 0x3d4 , 0x67 , V_2 ) ;
V_192 = F_25 ( 0x3d5 , V_2 ) & 0xf ;
F_24 ( 0x3d5 , 0x50 | V_192 , V_2 ) ;
F_49 ( 10 ) ;
F_24 ( 0x3d4 , 0x67 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_104 & ~ 0x0c , V_2 ) ;
F_24 ( 0x3d4 , 0x34 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_92 , V_2 ) ;
F_24 ( 0x3d4 , 0x40 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_95 , V_2 ) ;
F_24 ( 0x3d4 , 0x42 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_96 , V_2 ) ;
F_24 ( 0x3d4 , 0x45 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_97 , V_2 ) ;
F_24 ( 0x3d4 , 0x50 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_98 , V_2 ) ;
F_24 ( 0x3d4 , 0x51 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_99 , V_2 ) ;
F_24 ( 0x3d4 , 0x36 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_93 , V_2 ) ;
F_24 ( 0x3d4 , 0x60 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_102 , V_2 ) ;
F_24 ( 0x3d4 , 0x68 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_105 , V_2 ) ;
F_24 ( 0x3d4 , 0x69 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_106 , V_2 ) ;
F_24 ( 0x3d4 , 0x6f , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_107 , V_2 ) ;
F_24 ( 0x3d4 , 0x33 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_108 , V_2 ) ;
F_24 ( 0x3d4 , 0x86 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_109 , V_2 ) ;
F_24 ( 0x3d4 , 0x88 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_110 , V_2 ) ;
F_24 ( 0x3d4 , 0x90 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_111 , V_2 ) ;
F_24 ( 0x3d4 , 0x91 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_112 , V_2 ) ;
if ( V_2 -> V_38 == V_43 ) {
F_24 ( 0x3d4 , 0xb0 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_113 , V_2 ) ;
}
F_24 ( 0x3d4 , 0x32 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_91 , V_2 ) ;
F_24 ( 0x3c4 , 0x08 , V_2 ) ;
F_24 ( 0x3c5 , 0x06 , V_2 ) ;
if ( V_7 -> V_122 != 255 ) {
F_24 ( 0x3c4 , 0x10 , V_2 ) ;
F_24 ( 0x3c5 , V_7 -> V_122 , V_2 ) ;
F_24 ( 0x3c4 , 0x11 , V_2 ) ;
F_24 ( 0x3c5 , V_7 -> V_123 , V_2 ) ;
}
F_24 ( 0x3c4 , 0x0e , V_2 ) ;
F_24 ( 0x3c5 , V_7 -> V_120 , V_2 ) ;
F_24 ( 0x3c4 , 0x0f , V_2 ) ;
F_24 ( 0x3c5 , V_7 -> V_121 , V_2 ) ;
F_24 ( 0x3c4 , 0x12 , V_2 ) ;
F_24 ( 0x3c5 , V_7 -> V_124 , V_2 ) ;
F_24 ( 0x3c4 , 0x13 , V_2 ) ;
F_24 ( 0x3c5 , V_7 -> V_125 , V_2 ) ;
F_24 ( 0x3c4 , 0x29 , V_2 ) ;
F_24 ( 0x3c5 , V_7 -> V_126 , V_2 ) ;
F_24 ( 0x3c4 , 0x18 , V_2 ) ;
F_24 ( 0x3c5 , V_7 -> V_129 , V_2 ) ;
F_24 ( 0x3c4 , 0x15 , V_2 ) ;
V_5 = F_25 ( 0x3c5 , V_2 ) & ~ 0x21 ;
F_24 ( 0x3c5 , V_5 | 0x03 , V_2 ) ;
F_24 ( 0x3c5 , V_5 | 0x23 , V_2 ) ;
F_24 ( 0x3c5 , V_5 | 0x03 , V_2 ) ;
F_24 ( 0x3c5 , V_7 -> V_127 , V_2 ) ;
F_50 ( 100 ) ;
F_24 ( 0x3c4 , 0x30 , V_2 ) ;
F_24 ( 0x3c5 , V_7 -> V_128 , V_2 ) ;
F_24 ( 0x3c4 , 0x08 , V_2 ) ;
F_24 ( 0x3c5 , V_7 -> V_89 , V_2 ) ;
F_48 ( V_2 ) ;
F_24 ( 0x3d4 , 0x67 , V_2 ) ;
F_24 ( 0x3d5 , V_7 -> V_104 , V_2 ) ;
F_24 ( 0x3d4 , 0x66 , V_2 ) ;
V_88 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d5 , V_88 | 0x80 , V_2 ) ;
F_24 ( 0x3d4 , 0x3a , V_2 ) ;
V_86 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d5 , V_86 | 0x80 , V_2 ) ;
if ( V_2 -> V_38 != V_40 ) {
F_48 ( V_2 ) ;
F_23 ( V_132 , V_7 -> V_131 , V_2 ) ;
V_2 -> V_193 ( V_2 ) ;
F_23 ( V_134 , V_7 -> V_133 , V_2 ) ;
V_2 -> V_193 ( V_2 ) ;
F_23 ( V_136 , V_7 -> V_135 , V_2 ) ;
V_2 -> V_193 ( V_2 ) ;
F_23 ( V_138 , V_7 -> V_137 , V_2 ) ;
}
F_24 ( 0x3d4 , 0x66 , V_2 ) ;
F_24 ( 0x3d5 , V_88 , V_2 ) ;
F_24 ( 0x3d4 , 0x3a , V_2 ) ;
F_24 ( 0x3d5 , V_86 , V_2 ) ;
F_20 ( V_2 ) ;
F_3 ( V_2 , 0 ) ;
}
static void F_51 ( struct V_1 * V_2 , int V_194 )
{
F_36 ( 0x3d4 , ( V_194 & 0x00ff00 ) | 0x0c , V_2 ) ;
F_36 ( 0x3d4 , ( ( V_194 & 0x00ff ) << 8 ) | 0x0d , V_2 ) ;
F_24 ( 0x3d4 , 0x69 , V_2 ) ;
F_24 ( 0x3d5 , ( V_194 & 0x7f0000 ) >> 16 , V_2 ) ;
}
static void F_52 ( struct V_56 * V_57 )
{
V_57 -> V_177 . V_195 = V_57 -> V_15 . V_30 *
V_57 -> V_15 . V_160 / 8 ;
if ( V_57 -> V_15 . V_160 == 8 ) {
V_57 -> V_177 . V_196 = V_197 ;
V_57 -> V_177 . V_198 = 4 ;
} else {
V_57 -> V_177 . V_196 = V_199 ;
V_57 -> V_177 . V_198 = 2 ;
}
}
static int F_53 ( struct V_56 * V_57 )
{
struct V_1 * V_2 = V_57 -> V_2 ;
struct V_14 * V_15 = & V_57 -> V_15 ;
int V_200 ;
F_40 ( L_15 ) ;
V_200 = F_44 ( V_15 , V_2 , & V_2 -> V_201 ) ;
if ( V_200 )
return V_200 ;
if ( V_2 -> V_202 <= 0 ) {
if ( V_15 -> V_160 > 24 )
V_2 -> V_202 = V_2 -> clock [ 3 ] ;
else if ( V_15 -> V_160 >= 24 )
V_2 -> V_202 = V_2 -> clock [ 2 ] ;
else if ( ( V_15 -> V_160 > 8 ) && ( V_15 -> V_160 < 24 ) )
V_2 -> V_202 = V_2 -> clock [ 1 ] ;
else if ( V_15 -> V_160 <= 8 )
V_2 -> V_202 = V_2 -> clock [ 0 ] ;
}
V_2 -> V_203 = V_2 -> V_202 ;
V_2 -> V_204 = 10000 ;
F_47 ( V_2 , & V_2 -> V_201 ) ;
F_54 ( & V_57 -> V_205 , V_57 ) ;
F_52 ( V_57 ) ;
F_28 ( V_57 ) ;
F_34 ( V_2 ) ;
return 0 ;
}
static int F_55 ( struct V_14 * V_15 ,
struct V_56 * V_57 )
{
struct V_1 * V_2 = V_57 -> V_2 ;
int V_194 ;
V_194 = ( V_15 -> V_145 * V_57 -> V_177 . V_195
+ ( V_15 -> V_144 & ~ 1 ) * ( ( V_57 -> V_15 . V_160 + 7 ) / 8 ) ) >> 2 ;
F_51 ( V_2 , V_194 ) ;
return 0 ;
}
static int F_56 ( int V_206 , struct V_56 * V_57 )
{
struct V_1 * V_2 = V_57 -> V_2 ;
T_1 V_207 = 0 , V_208 = 0 ;
if ( V_2 -> V_209 == V_210 ) {
F_24 ( 0x3c4 , 0x08 , V_2 ) ;
V_207 = F_25 ( 0x3c5 , V_2 ) ;
V_207 |= 0x06 ;
F_24 ( 0x3c5 , V_207 , V_2 ) ;
F_24 ( 0x3c4 , 0x0d , V_2 ) ;
V_208 = F_25 ( 0x3c5 , V_2 ) ;
V_208 &= 0x50 ;
switch ( V_206 ) {
case V_211 :
case V_212 :
break;
case V_213 :
V_208 |= 0x10 ;
break;
case V_214 :
V_208 |= 0x40 ;
break;
case V_215 :
V_208 |= 0x50 ;
break;
}
F_24 ( 0x3c4 , 0x0d , V_2 ) ;
F_24 ( 0x3c5 , V_208 , V_2 ) ;
}
if ( V_2 -> V_209 == V_216 ||
V_2 -> V_209 == V_217 ) {
switch( V_206 ) {
case V_211 :
case V_212 :
F_24 ( 0x3c4 , 0x31 , V_2 ) ;
F_24 ( 0x3c5 , F_25 ( 0x3c5 , V_2 ) | 0x10 , V_2 ) ;
break;
case V_213 :
case V_214 :
case V_215 :
F_24 ( 0x3c4 , 0x31 , V_2 ) ;
F_24 ( 0x3c5 , F_25 ( 0x3c5 , V_2 ) & ~ 0x10 , V_2 ) ;
break;
}
}
return ( V_206 == V_212 ) ? 1 : 0 ;
}
static int F_57 ( struct V_56 * V_57 , int V_218 )
{
struct V_1 * V_2 = V_57 -> V_2 ;
F_58 ( & V_2 -> V_219 ) ;
if ( ! V_2 -> V_220 ) {
memset ( & V_2 -> V_221 , 0 , sizeof( V_2 -> V_221 ) ) ;
V_2 -> V_221 . V_222 = V_223 | V_224 |
V_225 ;
V_2 -> V_221 . V_226 = V_2 -> V_227 . V_228 + 0x8000 ;
F_59 ( & V_2 -> V_221 ) ;
F_35 ( V_2 , & V_2 -> V_229 ) ;
}
V_2 -> V_220 ++ ;
F_60 ( & V_2 -> V_219 ) ;
return 0 ;
}
static int F_61 ( struct V_56 * V_57 , int V_218 )
{
struct V_1 * V_2 = V_57 -> V_2 ;
F_58 ( & V_2 -> V_219 ) ;
if ( V_2 -> V_220 == 1 ) {
F_37 ( V_2 , & V_2 -> V_229 ) ;
F_62 ( & V_2 -> V_221 ) ;
}
V_2 -> V_220 -- ;
F_60 ( & V_2 -> V_219 ) ;
return 0 ;
}
static void F_63 ( struct V_1 * V_2 )
{
unsigned char V_230 ;
F_40 ( L_16 ) ;
V_230 = F_25 ( 0x3c3 , V_2 ) ;
F_24 ( 0x3c3 , V_230 | 0x01 , V_2 ) ;
V_230 = F_25 ( 0x3cc , V_2 ) ;
F_24 ( 0x3c2 , V_230 | 0x01 , V_2 ) ;
if ( V_2 -> V_38 >= V_43 ) {
F_24 ( 0x3d4 , 0x40 , V_2 ) ;
V_230 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d5 , V_230 | 1 , V_2 ) ;
}
}
static void F_64 ( struct V_1 * V_2 )
{
unsigned char V_230 ;
F_40 ( L_17 ) ;
if ( V_2 -> V_38 >= V_43 ) {
F_24 ( 0x3d4 , 0x40 , V_2 ) ;
V_230 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d5 , V_230 | 1 , V_2 ) ;
}
}
static int F_65 ( struct V_56 * V_57 )
{
struct V_1 * V_2 = V_57 -> V_2 ;
F_40 ( L_18 ) ;
if ( F_66 ( V_2 -> V_38 ) )
V_2 -> V_227 . V_231 = F_67 ( V_2 -> V_232 , 0 ) +
V_233 ;
else
V_2 -> V_227 . V_231 = F_67 ( V_2 -> V_232 , 0 ) +
V_234 ;
V_2 -> V_227 . V_235 = V_236 ;
V_2 -> V_227 . V_228 = F_68 ( V_2 -> V_227 . V_231 , V_2 -> V_227 . V_235 ) ;
if ( ! V_2 -> V_227 . V_228 ) {
F_32 ( L_19 ) ;
return - V_237 ;
} else
F_32 ( V_176 L_20 ,
V_2 -> V_227 . V_228 ) ;
V_57 -> V_177 . V_238 = V_2 -> V_227 . V_231 ;
V_57 -> V_177 . V_239 = V_2 -> V_227 . V_235 ;
V_2 -> V_240 = ( V_190 V_241 * ) ( V_2 -> V_227 . V_228 + V_242 ) ;
V_2 -> V_51 = 0 ;
F_63 ( V_2 ) ;
return 0 ;
}
static void F_69 ( struct V_56 * V_57 )
{
struct V_1 * V_2 = V_57 -> V_2 ;
F_40 ( L_21 ) ;
F_64 ( V_2 ) ;
if ( V_2 -> V_227 . V_228 ) {
F_70 ( V_2 -> V_227 . V_228 ) ;
V_2 -> V_227 . V_228 = NULL ;
}
}
static int F_71 ( struct V_56 * V_57 , int V_243 )
{
struct V_1 * V_2 = V_57 -> V_2 ;
int V_244 ;
F_40 ( L_22 ) ;
if ( F_66 ( V_2 -> V_38 ) )
V_244 = 0 ;
else
V_244 = 1 ;
V_2 -> V_245 . V_231 = F_67 ( V_2 -> V_232 , V_244 ) ;
V_2 -> V_245 . V_235 = V_243 ;
V_2 -> V_245 . V_228 = F_68 ( V_2 -> V_245 . V_231 , V_2 -> V_245 . V_235 ) ;
if ( ! V_2 -> V_245 . V_228 ) {
F_32 ( L_23 ) ;
return - V_237 ;
} else
F_32 ( V_176 L_24
L_25 , V_2 -> V_245 . V_228 , V_2 -> V_245 . V_231 ) ;
V_57 -> V_177 . V_246 = V_2 -> V_245 . V_231 ;
V_57 -> V_177 . V_178 = V_2 -> V_245 . V_235 - V_2 -> V_247 ;
V_57 -> V_248 = V_2 -> V_245 . V_228 ;
#ifdef F_72
V_2 -> V_245 . V_249 = F_73 ( V_2 -> V_245 . V_231 , V_243 ,
V_250 , 1 ) ;
#endif
F_74 ( V_2 -> V_245 . V_228 , 0 , V_2 -> V_245 . V_235 ) ;
return 0 ;
}
static void F_75 ( struct V_56 * V_57 )
{
struct V_1 * V_2 = V_57 -> V_2 ;
F_40 ( L_26 ) ;
if ( V_2 -> V_245 . V_228 ) {
#ifdef F_72
F_76 ( V_2 -> V_245 . V_249 , V_2 -> V_245 . V_231 , V_2 -> V_245 . V_235 ) ;
#endif
F_70 ( V_2 -> V_245 . V_228 ) ;
V_2 -> V_245 . V_228 = NULL ;
V_57 -> V_248 = NULL ;
}
}
static int F_77 ( struct V_1 * V_2 )
{
unsigned char V_251 , V_74 , V_182 , V_75 , V_76 , V_207 , V_252 , V_88 = 0 , V_5 ;
static unsigned char V_253 [] = { 8 , 4 , 4 , 2 } ;
static unsigned char V_254 [] = { 2 , 4 , 8 , 12 , 16 , 32 , 64 , 32 } ;
static unsigned char V_255 [] = { 2 , 8 , 4 , 16 , 8 , 16 , 4 , 16 } ;
static unsigned char V_256 [] = { 0 , 2 , 4 , 8 , 16 , 32 , 2 , 2 } ;
int V_257 , V_258 , V_259 ;
F_40 ( L_27 ) ;
F_24 ( 0x3d4 , 0x11 , V_2 ) ;
V_5 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d5 , V_5 & 0x7f , V_2 ) ;
F_36 ( 0x3d4 , 0x4838 , V_2 ) ;
F_36 ( 0x3d4 , 0xa039 , V_2 ) ;
F_36 ( 0x3c4 , 0x0608 , V_2 ) ;
F_24 ( 0x3d4 , 0x40 , V_2 ) ;
V_5 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d5 , V_5 & ~ 0x01 , V_2 ) ;
F_24 ( 0x3d4 , 0x38 , V_2 ) ;
F_24 ( 0x3d5 , 0x48 , V_2 ) ;
F_36 ( 0x3d4 , 0x4838 , V_2 ) ;
F_24 ( 0x3d4 , 0x36 , V_2 ) ;
V_251 = F_25 ( 0x3d5 , V_2 ) ;
switch ( V_2 -> V_38 ) {
case V_39 :
V_257 = V_253 [ ( V_251 & 0xC0 ) >> 6 ] * 1024 ;
break;
case V_43 :
F_24 ( 0x3d4 , 0x68 , V_2 ) ;
if ( ( F_25 ( 0x3d5 , V_2 ) & 0xC0 ) == ( 0x01 << 6 ) )
V_254 [ 1 ] = 8 ;
case V_48 :
V_257 = V_254 [ ( V_251 & 0xE0 ) >> 5 ] * 1024 ;
break;
case V_40 :
case V_47 :
V_257 = V_255 [ ( V_251 & 0x0E ) >> 1 ] * 1024 ;
break;
case V_45 :
case V_46 :
case V_44 :
V_257 = V_256 [ ( V_251 & 0xE0 ) >> 5 ] * 1024 ;
break;
default:
V_257 = 0 ;
break;
}
V_258 = V_257 * 1024 ;
F_32 ( V_176 L_28 , V_257 ) ;
F_24 ( 0x3d4 , 0x66 , V_2 ) ;
V_88 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d5 , V_88 | 0x02 , V_2 ) ;
F_49 ( 10 ) ;
F_24 ( 0x3d4 , 0x66 , V_2 ) ;
F_24 ( 0x3d5 , V_88 & ~ 0x02 , V_2 ) ;
F_49 ( 10 ) ;
F_24 ( 0x3d4 , 0x3f , V_2 ) ;
V_252 = F_25 ( 0x3d5 , V_2 ) ;
F_24 ( 0x3d5 , V_252 | 0x08 , V_2 ) ;
F_49 ( 10 ) ;
F_24 ( 0x3d4 , 0x3f , V_2 ) ;
F_24 ( 0x3d5 , V_252 & ~ 0x08 , V_2 ) ;
F_49 ( 10 ) ;
V_2 -> V_260 = 4 ;
V_2 -> clock [ 0 ] = 250000 ;
V_2 -> clock [ 1 ] = 250000 ;
V_2 -> clock [ 2 ] = 220000 ;
V_2 -> clock [ 3 ] = 220000 ;
F_24 ( 0x3c4 , 0x08 , V_2 ) ;
V_207 = F_25 ( 0x3c5 , V_2 ) ;
F_24 ( 0x3c5 , 0x06 , V_2 ) ;
F_24 ( 0x3c4 , 0x10 , V_2 ) ;
V_182 = F_25 ( 0x3c5 , V_2 ) ;
F_24 ( 0x3c4 , 0x11 , V_2 ) ;
V_74 = F_25 ( 0x3c5 , V_2 ) ;
F_24 ( 0x3c4 , 0x08 , V_2 ) ;
F_24 ( 0x3c5 , V_207 , V_2 ) ;
V_74 &= 0x7f ;
V_75 = V_182 & 0x1f ;
V_76 = ( V_182 >> 5 ) & 0x03 ;
V_2 -> V_186 = ( ( 1431818 * ( V_74 + 2 ) ) / ( V_75 + 2 ) / ( 1 << V_76 ) + 50 ) / 100 ;
F_32 ( V_176 L_29 ,
V_2 -> V_186 ) ;
V_259 = 0 ;
if ( V_2 -> V_38 == V_43 ) {
unsigned char V_261 = 0x00 ;
F_24 ( 0x3c4 , 0x30 , V_2 ) ;
F_24 ( 0x3c5 , F_25 ( 0x3c5 , V_2 ) & ~ 0x02 , V_2 ) ;
V_261 = F_25 ( 0x3c5 , V_2 ) ;
if ( V_261 & 0x02 ) {
V_259 = 1 ;
F_32 ( L_30 ) ;
}
}
if ( ( F_45 ( V_2 -> V_38 ) ||
F_78 ( V_2 -> V_38 ) ) && ! V_2 -> V_262 )
V_2 -> V_209 = V_216 ;
else if ( V_259 || ( V_2 -> V_38 == V_43 && V_2 -> V_259 ) )
V_2 -> V_209 = V_217 ;
else
V_2 -> V_209 = V_210 ;
if ( V_2 -> V_209 == V_216 ) {
unsigned char V_263 = F_79 ( 0x6b , V_2 ) ;
int V_264 = ( F_4 ( 0x61 , V_2 ) +
( ( F_4 ( 0x66 , V_2 ) & 0x02 ) << 7 ) + 1 ) * 8 ;
int V_265 = ( F_4 ( 0x69 , V_2 ) +
( ( F_4 ( 0x6e , V_2 ) & 0x70 ) << 4 ) + 1 ) ;
char * V_266 = L_31 ;
enum T_2 {
V_267 = 0x01 ,
V_268 = 0x02 ,
V_269 = 0x04 ,
V_270 = 0x20 ,
V_271 = 0x80
};
if ( ( F_4 ( 0x39 , V_2 ) & 0x03 ) == 0 ) {
V_266 = L_32 ;
} else if ( ( F_4 ( 0x30 , V_2 ) & 0x01 ) == 0 ) {
V_266 = L_33 ;
} else {
V_266 = L_34 ;
}
F_32 ( V_176 L_35 ,
V_264 , V_265 , V_266 ,
V_263 & V_268 ? L_36 : L_37 ) ;
if ( V_263 & V_268 ) {
F_32 ( V_176 L_38
L_39 , V_264 , V_265 ) ;
V_2 -> V_174 = V_264 ;
V_2 -> V_175 = V_265 ;
} else
V_2 -> V_209 = V_210 ;
}
F_35 ( V_2 , & V_2 -> V_201 ) ;
V_2 -> V_272 = V_2 -> V_201 ;
if ( F_80 ( V_2 -> V_38 ) ) {
V_2 -> V_42 = 2 ;
V_2 -> V_247 = 0x8000 << V_2 -> V_42 ;
V_2 -> V_41 = V_258 ;
} else {
V_2 -> V_42 = 7 ;
V_2 -> V_247 = 0x400 << V_2 -> V_42 ;
V_2 -> V_41 = V_258 - V_2 -> V_247 ;
}
return V_258 ;
}
static int F_81 ( struct V_56 * V_57 , struct V_273 * V_274 ,
const struct V_275 * V_276 )
{
struct V_1 * V_2 = V_57 -> V_2 ;
int V_200 = 0 ;
V_2 -> V_232 = V_274 ;
V_57 -> V_177 . type = V_277 ;
V_57 -> V_177 . V_278 = 0 ;
V_57 -> V_177 . V_279 = 1 ;
V_57 -> V_177 . V_280 = 0 ;
V_57 -> V_177 . V_281 = V_276 -> V_282 ;
switch ( V_57 -> V_177 . V_281 ) {
case V_283 :
V_2 -> V_38 = V_47 ;
snprintf ( V_57 -> V_177 . V_276 , 16 , L_40 ) ;
break;
case V_284 :
V_2 -> V_38 = V_43 ;
snprintf ( V_57 -> V_177 . V_276 , 16 , L_41 ) ;
break;
case V_285 :
V_2 -> V_38 = V_39 ;
snprintf ( V_57 -> V_177 . V_276 , 16 , L_42 ) ;
break;
case V_286 :
V_2 -> V_38 = V_39 ;
snprintf ( V_57 -> V_177 . V_276 , 16 , L_43 ) ;
break;
case V_287 :
V_2 -> V_38 = V_48 ;
snprintf ( V_57 -> V_177 . V_276 , 16 , L_44 ) ;
break;
case V_288 :
V_2 -> V_38 = V_40 ;
snprintf ( V_57 -> V_177 . V_276 , 16 , L_45 ) ;
break;
case V_289 :
V_2 -> V_38 = V_40 ;
snprintf ( V_57 -> V_177 . V_276 , 16 , L_46 ) ;
break;
case V_290 :
V_2 -> V_38 = V_40 ;
snprintf ( V_57 -> V_177 . V_276 , 16 , L_47 ) ;
break;
case V_291 :
V_2 -> V_38 = V_40 ;
snprintf ( V_57 -> V_177 . V_276 , 16 , L_48 ) ;
break;
case V_292 :
V_2 -> V_38 = V_45 ;
snprintf ( V_57 -> V_177 . V_276 , 16 , L_49 ) ;
break;
case V_293 :
V_2 -> V_38 = V_45 ;
snprintf ( V_57 -> V_177 . V_276 , 16 , L_50 ) ;
break;
case V_294 :
V_2 -> V_38 = V_44 ;
snprintf ( V_57 -> V_177 . V_276 , 16 , L_51 ) ;
break;
case V_295 :
V_2 -> V_38 = V_44 ;
snprintf ( V_57 -> V_177 . V_276 , 16 , L_52 ) ;
break;
case V_296 :
V_2 -> V_38 = V_46 ;
snprintf ( V_57 -> V_177 . V_276 , 16 , L_53 ) ;
break;
case V_297 :
V_2 -> V_38 = V_46 ;
snprintf ( V_57 -> V_177 . V_276 , 16 , L_54 ) ;
break;
}
if ( F_66 ( V_2 -> V_38 ) ) {
V_2 -> V_193 = F_17 ;
V_2 -> V_52 = F_13 ;
} else if ( F_80 ( V_2 -> V_38 ) ||
V_47 == V_2 -> V_38 ) {
V_2 -> V_193 = F_18 ;
V_2 -> V_52 = F_15 ;
} else {
V_2 -> V_193 = F_19 ;
V_2 -> V_52 = F_16 ;
}
V_57 -> V_15 . V_298 = 0 ;
V_57 -> V_15 . V_299 = V_300 ;
V_57 -> V_15 . V_179 = - 1 ;
V_57 -> V_15 . V_301 = - 1 ;
V_57 -> V_15 . V_302 = 0 ;
V_57 -> V_303 = & V_304 ;
V_57 -> V_222 = V_305 |
V_306 |
V_307 ;
V_57 -> V_191 = V_2 -> V_191 ;
#if F_82 ( V_308 )
V_57 -> V_309 . V_310 = F_83 ( 8 , 1024 , V_311 ) ;
V_200 = - V_237 ;
if ( V_57 -> V_309 . V_310 ) {
V_57 -> V_309 . V_312 = 8 * 1024 ;
V_57 -> V_309 . V_313 = 4 ;
V_57 -> V_309 . V_314 = 4 ;
V_57 -> V_309 . V_315 = 32 ;
V_200 = F_84 ( & V_57 -> V_205 , V_188 , 0 ) ;
if ( ! V_200 )
V_57 -> V_222 |= V_316 |
V_317 |
V_318 ;
}
#endif
return V_200 ;
}
static int F_85 ( struct V_273 * V_274 , const struct V_275 * V_276 )
{
struct V_56 * V_57 ;
struct V_1 * V_2 ;
T_3 V_319 , V_320 ;
int V_200 , V_321 ;
int V_243 ;
F_40 ( L_55 ) ;
V_57 = F_86 ( sizeof( struct V_1 ) , & V_274 -> V_274 ) ;
if ( ! V_57 )
return - V_237 ;
V_2 = V_57 -> V_2 ;
F_87 ( & V_2 -> V_219 ) ;
V_200 = F_88 ( V_274 ) ;
if ( V_200 )
goto V_322;
if ( ( V_200 = F_89 ( V_274 , L_56 ) ) ) {
F_32 ( V_80 L_57 ) ;
goto V_322;
}
V_200 = - V_237 ;
if ( ( V_200 = F_81 ( V_57 , V_274 , V_276 ) ) )
goto V_323;
V_200 = F_65 ( V_57 ) ;
if ( V_200 )
goto V_324;
V_243 = F_77 ( V_2 ) ;
if ( V_243 < 0 ) {
V_200 = V_243 ;
goto V_324;
}
V_200 = F_71 ( V_57 , V_243 ) ;
if ( V_200 )
goto V_325;
F_90 ( & V_57 -> V_172 ) ;
#if F_82 ( V_326 )
F_91 ( V_57 ) ;
F_92 ( V_57 , & V_2 -> V_327 ) ;
F_93 ( V_2 -> V_327 , & V_57 -> V_165 ) ;
F_94 ( V_2 -> V_327 ) ;
F_95 ( V_57 -> V_165 . V_140 ,
V_57 -> V_165 . V_173 ,
& V_57 -> V_172 ) ;
#endif
V_57 -> V_15 = V_328 ;
if ( V_2 -> V_174 ) {
struct V_139 V_329 ;
memset ( & V_329 , 0 , sizeof( V_329 ) ) ;
V_329 . V_141 = V_2 -> V_174 ;
V_329 . V_142 = V_2 -> V_175 ;
V_329 . V_330 = 60 ;
if ( F_96 ( & V_329 , 0 , 0 ) )
F_32 ( V_331 L_58 ) ;
else if ( F_97 ( & V_57 -> V_15 , V_57 , NULL , NULL , 0 ,
& V_329 , 0 ) != 3 )
V_57 -> V_15 = V_328 ;
}
if ( V_332 ) {
F_97 ( & V_57 -> V_15 , V_57 , V_332 ,
V_57 -> V_165 . V_140 , V_57 -> V_165 . V_173 ,
NULL , 8 ) ;
} else if ( V_57 -> V_165 . V_140 != NULL ) {
const struct V_139 * V_171 ;
V_171 = F_98 ( & V_57 -> V_165 , & V_57 -> V_172 ) ;
F_38 ( & V_57 -> V_15 , V_171 ) ;
}
V_321 = V_57 -> V_15 . V_30 * ( ( V_57 -> V_15 . V_160 + 7 ) >> 3 ) ;
V_57 -> V_15 . V_143 = V_57 -> V_177 . V_178 / V_321 ;
if ( V_57 -> V_15 . V_143 < V_57 -> V_15 . V_142 ) {
V_200 = - V_237 ;
goto V_333;
}
#if F_82 ( V_308 )
if ( V_57 -> V_15 . V_143 > 0x1000 )
V_57 -> V_15 . V_143 = 0x1000 ;
if ( V_57 -> V_15 . V_30 > 0x1000 )
V_57 -> V_15 . V_30 = 0x1000 ;
#endif
F_39 ( & V_57 -> V_15 , V_57 ) ;
F_52 ( V_57 ) ;
V_319 = 1953125000 / V_57 -> V_15 . V_146 ;
V_319 = V_319 * 512 / ( V_57 -> V_15 . V_141 + V_57 -> V_15 . V_147 +
V_57 -> V_15 . V_148 +
V_57 -> V_15 . V_151 ) ;
V_320 = V_319 / ( V_57 -> V_15 . V_142 + V_57 -> V_15 . V_149 +
V_57 -> V_15 . V_150 + V_57 -> V_15 . V_152 ) ;
F_32 ( V_176 L_59 V_334 L_60
L_61 ,
V_57 -> V_177 . V_178 >> 10 ,
V_57 -> V_15 . V_141 , V_57 -> V_15 . V_142 ,
V_319 / 1000 , V_319 % 1000 , V_320 ) ;
F_99 ( V_57 -> V_165 . V_140 ) ;
V_57 -> V_165 . V_140 = NULL ;
V_200 = F_100 ( V_57 ) ;
if ( V_200 < 0 )
goto V_333;
F_32 ( V_176 L_62 ,
V_57 -> V_177 . V_276 ) ;
F_101 ( V_274 , V_57 ) ;
return 0 ;
V_333:
#ifdef V_326
F_102 ( V_57 ) ;
#endif
F_84 ( & V_57 -> V_205 , 0 , 0 ) ;
F_75 ( V_57 ) ;
V_325:
F_69 ( V_57 ) ;
V_324:
F_94 ( V_57 -> V_309 . V_310 ) ;
V_323:
F_103 ( V_274 ) ;
V_322:
F_104 ( V_57 ) ;
return V_200 ;
}
static void F_105 ( struct V_273 * V_274 )
{
struct V_56 * V_57 = F_106 ( V_274 ) ;
F_40 ( L_63 ) ;
if ( V_57 ) {
if ( F_107 ( V_57 ) )
F_32 ( V_331 L_64
L_65 ) ;
#ifdef V_326
F_102 ( V_57 ) ;
#endif
F_84 ( & V_57 -> V_205 , 0 , 0 ) ;
F_75 ( V_57 ) ;
F_69 ( V_57 ) ;
F_94 ( V_57 -> V_309 . V_310 ) ;
F_103 ( V_274 ) ;
F_104 ( V_57 ) ;
}
}
static int F_108 ( struct V_273 * V_274 , T_4 V_335 )
{
struct V_56 * V_57 = F_106 ( V_274 ) ;
struct V_1 * V_2 = V_57 -> V_2 ;
F_40 ( L_66 ) ;
if ( V_335 . V_336 == V_337 )
V_335 . V_336 = V_338 ;
V_2 -> V_339 = V_335 . V_336 ;
V_274 -> V_274 . V_340 . V_341 = V_335 ;
if ( V_335 . V_336 == V_338 )
return 0 ;
F_109 () ;
F_110 ( V_57 , 1 ) ;
if ( V_57 -> V_303 -> V_342 )
V_57 -> V_303 -> V_342 ( V_57 ) ;
F_56 ( V_215 , V_57 ) ;
F_37 ( V_2 , & V_2 -> V_272 ) ;
F_64 ( V_2 ) ;
F_111 ( V_274 ) ;
F_112 ( V_274 ) ;
F_113 ( V_274 , F_114 ( V_274 , V_335 ) ) ;
F_115 () ;
return 0 ;
}
static int F_116 ( struct V_273 * V_274 )
{
struct V_56 * V_57 = F_106 ( V_274 ) ;
struct V_1 * V_2 = V_57 -> V_2 ;
int V_343 = V_2 -> V_339 ;
F_40 ( L_67 ) ;
V_2 -> V_339 = V_344 ;
if ( V_343 == V_338 ) {
F_113 ( V_274 , V_345 ) ;
return 0 ;
}
F_109 () ;
F_113 ( V_274 , V_345 ) ;
F_117 ( V_274 ) ;
if ( F_88 ( V_274 ) )
F_40 ( L_68 ) ;
F_118 ( V_274 ) ;
F_63 ( V_2 ) ;
F_77 ( V_2 ) ;
F_53 ( V_57 ) ;
F_110 ( V_57 , 0 ) ;
F_56 ( V_211 , V_57 ) ;
F_115 () ;
return 0 ;
}
static void T_5 F_119 ( void )
{
F_40 ( L_69 ) ;
F_120 ( & V_346 ) ;
}
static int T_6 F_121 ( char * V_347 )
{
#ifndef F_122
char * V_348 ;
if ( ! V_347 || ! * V_347 )
return 0 ;
while ( ( V_348 = F_123 ( & V_347 , L_70 ) ) != NULL ) {
V_332 = V_348 ;
}
#endif
return 0 ;
}
static int T_6 F_124 ( void )
{
char * V_349 ;
F_40 ( L_71 ) ;
if ( F_125 ( L_56 , & V_349 ) )
return - V_350 ;
F_121 ( V_349 ) ;
return F_126 ( & V_346 ) ;
}
