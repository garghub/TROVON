static struct V_1 * F_1 ( unsigned int V_2 )
{
unsigned int V_3 ;
for ( V_3 = 0 ; V_3 < V_4 ; V_3 ++ )
if ( V_5 [ V_3 ] . V_2 == V_2 )
return V_5 + V_3 ;
return NULL ;
}
static void F_2 ( struct V_6 * V_7 , struct V_8 * V_9 )
{
F_3 ( L_1 , V_9 -> V_10 ) ;
V_7 -> V_11 = V_9 ;
V_7 -> V_12 . V_13 = V_9 -> V_14 ;
V_7 -> V_15 . V_13 = V_9 -> V_14 ;
V_7 -> V_12 . V_16 = V_9 -> V_17 - V_9 -> V_14 + 1 ;
V_7 -> V_15 . V_16 = V_9 -> V_17 - V_9 -> V_14 + 1 ;
V_7 -> V_12 . V_18 = ( V_9 -> V_19 + 1 ) * 2 ;
V_7 -> V_15 . V_18 = V_9 -> V_20 * 2 ;
V_7 -> V_12 . V_21 = ( ( V_9 -> V_22 & V_23 ) ? 524 : 624 )
- V_7 -> V_12 . V_18 ;
V_7 -> V_15 . V_21 = ( V_9 -> V_24 - V_9 -> V_20 + 1 ) * 2 ;
V_7 -> V_25 = V_7 -> V_15 ;
F_4 ( V_7 ) ;
}
static void F_5 ( struct V_6 * V_7 , int V_26 )
{
F_3 ( L_2 , V_26 , F_6 ( V_7 , V_26 ) . V_10 ) ;
V_7 -> V_27 = V_26 ;
F_2 ( V_7 , V_7 -> V_11 ) ;
F_7 ( V_7 , & F_6 ( V_7 , V_26 ) ) ;
}
static void F_8 ( struct V_6 * V_7 )
{
int V_28 , V_29 , V_30 , V_31 ;
struct V_8 * V_9 = V_7 -> V_11 ;
V_31 = F_6 ( V_7 , V_7 -> V_27 ) . V_32 ;
V_28 = V_9 -> V_28 ;
V_29 = V_9 -> V_29 ;
V_30 = V_9 -> V_30 ;
if ( V_31 > 5 )
V_28 |= 0x80 ;
if ( V_33 || V_7 -> V_34 )
V_29 |= 0x20 ;
V_29 |= V_35 ;
V_30 |= V_36 ;
V_30 &= ~ V_37 ;
V_28 &= ~ V_38 ;
F_9 ( V_39 , 0x08 ) ;
F_9 ( V_40 , 0xc0 | V_31 ) ;
F_9 ( V_41 , 0x00 ) ;
F_9 ( V_42 , 0x90 ) ;
F_9 ( V_43 , 0x90 ) ;
F_9 ( V_44 , 0xeb ) ;
F_9 ( V_45 , 0xe0 ) ;
F_9 ( V_46 , V_9 -> V_47 ) ;
F_9 ( V_48 , V_29 ) ;
F_9 ( V_49 , V_28 ) ;
F_9 ( V_50 , V_7 -> V_51 ) ;
F_9 ( V_52 ,
V_7 -> V_53 ? - V_7 -> V_54 : V_7 -> V_54 ) ;
F_9 ( V_55 ,
V_7 -> V_53 ? - V_7 -> V_56 : V_7 -> V_56 ) ;
F_9 ( V_57 , V_7 -> V_58 ) ;
F_9 ( V_59 , V_30 ) ;
F_9 ( V_60 , V_9 -> V_61 ) ;
F_9 ( V_62 , V_9 -> V_63 ) ;
F_9 ( V_64 , 0x00 ) ;
F_9 ( V_65 , 0x01 ) ;
F_9 ( V_66 , 0x11 ) ;
F_9 ( V_67 , 0xfe ) ;
F_9 ( V_68 , V_9 -> V_69 ) ;
F_9 ( V_70 , 0x40 ) ;
F_9 ( V_71 , 0x80 ) ;
}
void F_4 ( struct V_6 * V_7 )
{
F_8 ( V_7 ) ;
if ( F_6 ( V_7 , V_7 -> V_27 ) . V_72 )
F_10 ( V_7 , V_73 , V_74 , V_7 -> V_11 -> V_22 ) ;
F_11 ( V_7 , V_73 , V_74 , V_7 -> V_11 -> V_22 ) ;
}
static void F_12 ( struct V_6 * V_7 , int V_75 , int V_76 )
{
static const struct {
int V_77 ;
int V_78 ;
int V_79 ;
int V_80 ;
int V_81 ;
} V_82 [] = {
{ 1 , 0 , 0 , 0 , 0 } ,
{ 2 , 2 , 1 , 2 , 2 } ,
{ 3 , 4 , 1 , 3 , 2 } ,
{ 4 , 8 , 1 , 4 , 2 } ,
{ 5 , 8 , 1 , 4 , 2 } ,
{ 6 , 8 , 1 , 4 , 3 } ,
{ 7 , 8 , 1 , 4 , 3 } ,
{ 8 , 15 , 0 , 4 , 3 } ,
{ 9 , 15 , 0 , 4 , 3 } ,
{ 10 , 16 , 1 , 5 , 3 } ,
} ;
static const int V_83 = F_13 ( V_82 ) ;
int V_3 ;
for ( V_3 = 0 ; V_3 < V_83 ; V_3 ++ )
if ( V_82 [ V_3 ] . V_77 == V_76 )
break;
if ( V_3 == V_83 )
return;
F_9 ( F_14 ( V_75 ) , V_82 [ V_3 ] . V_77 ) ;
F_9 ( F_15 ( V_75 ) , V_82 [ V_3 ] . V_78 ) ;
F_9 ( F_16 ( V_75 ) ,
( V_82 [ V_3 ] . V_79 << 3 ) | ( V_82 [ V_3 ] . V_80 ) ) ;
F_17 ( F_18 ( V_75 ) , 0x0f ,
( V_82 [ V_3 ] . V_81 << 2 ) | V_82 [ V_3 ] . V_81 ) ;
}
static void F_19 ( struct V_6 * V_7 , int V_75 , int V_84 )
{
int V_85 , V_86 ;
F_9 ( F_20 ( V_75 ) , V_84 & 0xff ) ;
F_9 ( F_21 ( V_75 ) , V_84 >> 8 ) ;
V_86 = ( V_7 -> V_87 ) ? 0x02 : 0x00 ;
if ( V_84 < 2048 ) {
F_3 ( L_3 , V_84 ) ;
F_9 ( F_22 ( V_75 ) , 0x00 | V_86 ) ;
F_9 ( F_23 ( V_75 ) , 0x40 ) ;
F_9 ( F_24 ( V_75 ) , 0x40 ) ;
} else {
V_85 = 0x40 * 1024 / V_84 ;
F_3 ( L_4 , V_84 , V_85 ) ;
F_9 ( F_22 ( V_75 ) , 0x01 | V_86 ) ;
F_9 ( F_23 ( V_75 ) , V_85 ) ;
F_9 ( F_24 ( V_75 ) , V_85 ) ;
}
F_9 ( F_25 ( V_75 ) , 0x80 ) ;
}
static void F_26 ( struct V_6 * V_7 , int V_75 ,
int V_16 , int V_21 , int V_88 )
{
int V_76 , V_89 , V_84 , V_90 , V_91 ;
int V_14 , V_17 , V_92 , V_93 ;
int div = V_88 ? 2 : 1 ;
V_14 = V_7 -> V_25 . V_13 ;
V_92 = V_7 -> V_25 . V_18 / 2 ;
V_17 = ( V_7 -> V_25 . V_13 + V_7 -> V_25 . V_16 - 1 ) ;
V_93 = ( V_7 -> V_25 . V_18 + V_7 -> V_25 . V_21 - 1 ) / 2 ;
F_9 ( F_27 ( V_75 ) , V_14 & 0xff ) ;
F_9 ( F_28 ( V_75 ) , V_14 >> 8 ) ;
F_9 ( F_29 ( V_75 ) , V_17 & 0xff ) ;
F_9 ( F_30 ( V_75 ) , V_17 >> 8 ) ;
F_9 ( F_31 ( V_75 ) , V_92 & 0xff ) ;
F_9 ( F_32 ( V_75 ) , V_92 >> 8 ) ;
F_9 ( F_33 ( V_75 ) , V_93 & 0xff ) ;
F_9 ( F_34 ( V_75 ) , V_93 >> 8 ) ;
V_76 = V_7 -> V_25 . V_16 / V_16 ;
if ( 0 == V_76 )
V_76 = 1 ;
V_89 = 1024 * V_7 -> V_25 . V_16 / V_76 / V_16 ;
V_84 = 512 * div * V_7 -> V_25 . V_21 / V_21 ;
F_3 ( L_5 , V_76 , V_89 , V_84 ) ;
F_12 ( V_7 , V_75 , V_76 ) ;
F_9 ( F_35 ( V_75 ) , V_89 & 0xff ) ;
F_9 ( F_36 ( V_75 ) , V_89 >> 8 ) ;
F_19 ( V_7 , V_75 , V_84 ) ;
F_9 ( F_37 ( V_75 ) , V_16 & 0xff ) ;
F_9 ( F_38 ( V_75 ) , V_16 >> 8 ) ;
F_9 ( F_39 ( V_75 ) , V_21 / div & 0xff ) ;
F_9 ( F_40 ( V_75 ) , V_21 / div >> 8 ) ;
V_91 = V_7 -> V_94 ;
V_90 = V_7 -> V_95 ;
F_9 ( F_41 ( V_75 ) , V_91 ) ;
F_9 ( F_42 ( V_75 ) , V_90 ) ;
F_9 ( F_43 ( V_75 ) , V_91 ) ;
F_9 ( F_44 ( V_75 ) , V_90 ) ;
}
static void F_45 ( struct V_6 * V_7 , int V_96 ,
struct V_97 * V_98 , int V_99 , char * V_10 )
{
T_1 V_100 = 0 ;
int V_3 ;
for ( V_3 = 0 ; V_3 < V_99 ; V_3 ++ ) {
V_100 |= V_98 [ V_3 ] . V_101 ;
V_100 &= ~ V_98 [ V_3 ] . V_102 ;
if ( V_3 < 15 && V_98 [ V_3 ] . V_103 == V_98 [ V_3 + 1 ] . V_103 )
continue;
F_9 ( V_96 + 0 , V_100 ) ;
F_9 ( V_96 + 2 , V_98 [ V_3 ] . V_103 & 0xff ) ;
F_9 ( V_96 + 3 , V_98 [ V_3 ] . V_103 >> 8 ) ;
F_3 ( L_6 ,
V_10 , V_100 , V_98 [ V_3 ] . V_103 ) ;
V_96 += 8 ;
}
for (; V_96 < 0x400 ; V_96 += 8 ) {
F_9 ( V_96 + 0 , 0 ) ;
F_9 ( V_96 + 1 , 0 ) ;
F_9 ( V_96 + 2 , 0 ) ;
F_9 ( V_96 + 3 , 0 ) ;
}
}
static int F_46 ( int V_85 )
{
if ( V_85 < 0 )
V_85 = 0 ;
return V_85 ;
}
static int F_47 ( const void * V_104 , const void * V_105 )
{
const struct V_97 * V_106 = V_104 ;
const struct V_97 * V_107 = V_105 ;
if ( V_106 -> V_103 < V_107 -> V_103 )
return - 1 ;
if ( V_106 -> V_103 > V_107 -> V_103 )
return 1 ;
return 0 ;
}
static int F_48 ( struct V_6 * V_7 , struct V_108 * V_109 ,
int V_110 , int V_88 )
{
struct V_97 V_111 [ 16 ] , V_112 [ 16 ] ;
int V_113 = 0 , V_114 = 0 , V_3 ;
int div = V_88 ? 2 : 1 ;
memset ( V_111 , 0 , sizeof( V_111 ) ) ;
memset ( V_112 , 0 , sizeof( V_112 ) ) ;
for ( V_3 = 0 ; V_3 < V_110 && V_3 < 8 ; V_3 ++ ) {
V_111 [ V_113 ] . V_103 = F_46 ( V_109 [ V_3 ] . V_115 . V_13 ) ;
V_111 [ V_113 ] . V_101 = ( 1 << V_3 ) ;
V_113 ++ ;
V_111 [ V_113 ] . V_103 = F_46 ( V_109 [ V_3 ] . V_115 . V_13 + V_109 [ V_3 ] . V_115 . V_16 ) ;
V_111 [ V_113 ] . V_102 = ( 1 << V_3 ) ;
V_113 ++ ;
V_112 [ V_114 ] . V_103 = F_46 ( V_109 [ V_3 ] . V_115 . V_18 / div ) ;
V_112 [ V_114 ] . V_101 = ( 1 << V_3 ) ;
V_114 ++ ;
V_112 [ V_114 ] . V_103 = F_46 ( ( V_109 [ V_3 ] . V_115 . V_18 + V_109 [ V_3 ] . V_115 . V_21 )
/ div ) ;
V_112 [ V_114 ] . V_102 = ( 1 << V_3 ) ;
V_114 ++ ;
}
F_49 ( V_111 , V_113 , sizeof V_111 [ 0 ] , F_47 , NULL ) ;
F_49 ( V_112 , V_114 , sizeof V_112 [ 0 ] , F_47 , NULL ) ;
F_45 ( V_7 , 0x380 , V_111 , V_113 , L_7 ) ;
F_45 ( V_7 , 0x384 , V_112 , V_114 , L_8 ) ;
return 0 ;
}
static int F_50 ( struct V_6 * V_7 , struct V_116 * V_117 , bool V_118 )
{
enum V_119 V_120 ;
int V_121 , V_122 ;
if ( ! V_118 && ( V_7 -> V_123 . V_124 == NULL || V_7 -> V_125 == NULL ) )
return - V_126 ;
if ( V_117 -> V_127 . V_16 < 48 )
V_117 -> V_127 . V_16 = 48 ;
if ( V_117 -> V_127 . V_21 < 32 )
V_117 -> V_127 . V_21 = 32 ;
if ( V_117 -> V_128 > 8 )
V_117 -> V_128 = 8 ;
V_117 -> V_129 = 0 ;
V_117 -> V_130 = 0 ;
V_120 = V_117 -> V_120 ;
V_121 = V_7 -> V_25 . V_16 ;
V_122 = V_7 -> V_25 . V_21 ;
if ( V_131 == V_120 ) {
V_120 = ( V_117 -> V_127 . V_21 > V_122 / 2 )
? V_132
: V_133 ;
}
switch ( V_120 ) {
case V_133 :
case V_134 :
V_122 = V_122 / 2 ;
break;
default:
V_120 = V_132 ;
break;
}
V_117 -> V_120 = V_120 ;
if ( V_117 -> V_127 . V_16 > V_121 )
V_117 -> V_127 . V_16 = V_121 ;
if ( V_117 -> V_127 . V_21 > V_122 )
V_117 -> V_127 . V_21 = V_122 ;
return 0 ;
}
static int F_51 ( struct V_6 * V_7 )
{
unsigned long V_124 , V_135 , V_136 ;
int V_137 ;
V_137 = F_50 ( V_7 , & V_7 -> V_117 , false ) ;
if ( 0 != V_137 )
return V_137 ;
V_7 -> V_138 = V_7 -> V_117 . V_120 ;
F_3 ( L_9 ,
V_7 -> V_117 . V_127 . V_16 , V_7 -> V_117 . V_127 . V_21 ,
V_7 -> V_117 . V_127 . V_13 , V_7 -> V_117 . V_127 . V_18 ,
V_7 -> V_125 -> V_10 , V_139 [ V_7 -> V_138 ] ) ;
F_26 ( V_7 , V_140 , V_7 -> V_117 . V_127 . V_16 , V_7 -> V_117 . V_127 . V_21 ,
F_52 ( V_7 -> V_138 ) ) ;
F_48 ( V_7 , V_7 -> V_109 , V_7 -> V_110 ,
F_52 ( V_7 -> V_138 ) ) ;
if ( V_7 -> V_125 -> V_141 )
F_17 ( F_53 ( V_140 ) , 0x3f , 0x03 ) ;
else
F_17 ( F_53 ( V_140 ) , 0x3f , 0x01 ) ;
F_9 ( V_142 , V_7 -> V_125 -> V_143 | 0x20 ) ;
V_124 = ( unsigned long ) V_7 -> V_123 . V_124 ;
V_124 += V_7 -> V_123 . V_144 . V_145 * V_7 -> V_117 . V_127 . V_18 ;
V_124 += V_7 -> V_125 -> V_146 / 8 * V_7 -> V_117 . V_127 . V_13 ;
V_136 = V_7 -> V_123 . V_144 . V_145 ;
V_135 = V_147 ;
if ( V_7 -> V_125 -> V_148 )
V_135 |= V_149 ;
if ( V_7 -> V_125 -> V_150 )
V_135 |= V_151 ;
if ( F_52 ( V_7 -> V_138 ) ) {
F_54 ( F_55 ( 1 ) , V_124 ) ;
F_54 ( F_56 ( 1 ) , V_124 + V_136 ) ;
F_54 ( F_57 ( 1 ) , V_136 * 2 ) ;
F_54 ( F_58 ( 1 ) , V_135 ) ;
} else {
F_54 ( F_55 ( 1 ) , V_124 ) ;
F_54 ( F_56 ( 1 ) , V_124 ) ;
F_54 ( F_57 ( 1 ) , V_136 ) ;
F_54 ( F_58 ( 1 ) , V_135 ) ;
}
V_7 -> V_152 = 1 ;
F_59 ( V_7 ) ;
return 0 ;
}
static int F_60 ( struct V_6 * V_7 )
{
V_7 -> V_152 = 0 ;
F_59 ( V_7 ) ;
return 0 ;
}
static int F_61 ( struct V_6 * V_7 ,
struct V_153 * V_154 ,
struct V_153 * V_155 )
{
struct V_156 * V_157 = V_154 -> V_158 . V_159 -> V_160 ;
unsigned long V_124 , V_135 , V_136 ;
unsigned long V_161 , V_162 , V_163 , V_164 , V_165 ;
F_3 ( L_10 , V_154 ) ;
V_154 -> V_166 = 0 ;
F_26 ( V_7 , V_167 , V_7 -> V_16 , V_7 -> V_21 ,
F_52 ( V_7 -> V_120 ) ) ;
if ( V_7 -> V_144 -> V_141 )
F_17 ( F_53 ( V_167 ) , 0x3f , 0x03 ) ;
else
F_17 ( F_53 ( V_167 ) , 0x3f , 0x01 ) ;
F_9 ( V_168 , V_7 -> V_144 -> V_143 ) ;
V_124 = F_62 ( V_154 ) ;
if ( V_7 -> V_144 -> V_169 )
V_136 = V_7 -> V_16 ;
else
V_136 = ( V_7 -> V_16 * V_7 -> V_144 -> V_146 ) / 8 ;
V_135 = V_147 |
V_170 |
( V_157 -> V_171 . V_172 >> 12 ) ;
if ( V_7 -> V_144 -> V_148 )
V_135 |= V_149 ;
if ( V_7 -> V_144 -> V_150 )
V_135 |= V_151 ;
if ( F_52 ( V_7 -> V_120 ) ) {
F_54 ( F_55 ( 0 ) , V_124 ) ;
F_54 ( F_56 ( 0 ) , V_124 + V_136 ) ;
F_54 ( F_57 ( 0 ) , V_136 * 2 ) ;
} else {
F_54 ( F_55 ( 0 ) , V_124 ) ;
F_54 ( F_56 ( 0 ) , V_124 ) ;
F_54 ( F_57 ( 0 ) , V_136 ) ;
}
F_54 ( F_58 ( 0 ) , V_135 ) ;
if ( V_7 -> V_144 -> V_169 ) {
V_161 = V_136 >> V_7 -> V_144 -> V_173 ;
V_162 = V_7 -> V_21 >> V_7 -> V_144 -> V_174 ;
V_163 = V_124 + V_136 * V_7 -> V_21 ;
V_164 = V_163 + V_161 * V_162 ;
if ( V_7 -> V_144 -> V_175 )
V_165 = V_163 , V_163 = V_164 , V_164 = V_165 ;
F_3 ( L_11 ,
V_161 , V_162 , V_163 , V_164 ) ;
if ( F_52 ( V_7 -> V_120 ) ) {
F_54 ( F_55 ( 4 ) , V_163 ) ;
F_54 ( F_56 ( 4 ) , V_163 + V_161 ) ;
F_54 ( F_57 ( 4 ) , V_161 * 2 ) ;
F_54 ( F_55 ( 5 ) , V_164 ) ;
F_54 ( F_56 ( 5 ) , V_164 + V_161 ) ;
F_54 ( F_57 ( 5 ) , V_161 * 2 ) ;
} else {
F_54 ( F_55 ( 4 ) , V_163 ) ;
F_54 ( F_56 ( 4 ) , V_163 ) ;
F_54 ( F_57 ( 4 ) , V_161 ) ;
F_54 ( F_55 ( 5 ) , V_164 ) ;
F_54 ( F_56 ( 5 ) , V_164 ) ;
F_54 ( F_57 ( 5 ) , V_161 ) ;
}
F_54 ( F_58 ( 4 ) , V_135 ) ;
F_54 ( F_58 ( 5 ) , V_135 ) ;
}
F_59 ( V_7 ) ;
F_63 ( & V_157 -> V_176 , V_177 + V_178 ) ;
return 0 ;
}
static int F_64 ( struct V_179 * V_158 )
{
struct V_156 * V_157 = V_158 -> V_159 -> V_160 ;
struct V_153 * V_154 = F_65 ( V_158 , struct V_153 , V_158 ) ;
V_157 -> V_180 = NULL ;
V_154 -> V_181 = F_61 ;
return 0 ;
}
static int F_66 ( struct V_179 * V_158 )
{
struct V_156 * V_157 = V_158 -> V_159 -> V_160 ;
struct V_6 * V_7 = V_157 -> V_7 ;
struct V_153 * V_154 = F_65 ( V_158 , struct V_153 , V_158 ) ;
struct V_182 * V_172 = F_67 ( & V_154 -> V_158 , 0 ) ;
unsigned int V_183 ;
if ( V_172 -> V_184 -> V_185 ) {
F_68 ( L_12 ) ;
return - V_126 ;
}
V_183 = ( V_7 -> V_16 * V_7 -> V_21 * V_7 -> V_144 -> V_146 ) >> 3 ;
if ( F_69 ( V_158 , 0 ) < V_183 )
return - V_126 ;
F_70 ( V_158 , 0 , V_183 ) ;
V_158 -> V_186 . V_120 = V_7 -> V_120 ;
return F_71 ( V_7 -> V_187 , & V_157 -> V_171 , V_172 -> V_184 , V_172 -> V_188 ,
F_72 ( V_154 ) ) ;
}
static int F_73 ( struct V_159 * V_189 , const struct V_190 * V_144 ,
unsigned int * V_191 , unsigned int * V_192 ,
unsigned int V_193 [] , void * V_194 [] )
{
struct V_156 * V_157 = V_189 -> V_160 ;
struct V_6 * V_7 = V_157 -> V_7 ;
int V_183 = V_7 -> V_144 -> V_146 * V_7 -> V_16 * V_7 -> V_21 >> 3 ;
if ( V_7 -> V_16 < 48 ||
V_7 -> V_21 < 32 ||
V_7 -> V_16 / 4 > V_7 -> V_25 . V_16 ||
V_7 -> V_21 / 4 > V_7 -> V_25 . V_21 ||
V_7 -> V_16 > V_7 -> V_12 . V_16 ||
V_7 -> V_21 > V_7 -> V_12 . V_21 )
return - V_126 ;
* V_191 = F_74 ( V_183 , * V_191 ) ;
* V_192 = 1 ;
V_193 [ 0 ] = V_183 ;
V_194 [ 0 ] = V_7 -> V_195 ;
return 0 ;
}
void F_75 ( struct V_179 * V_196 )
{
struct V_156 * V_157 = V_196 -> V_159 -> V_160 ;
struct V_6 * V_7 = V_157 -> V_7 ;
struct V_153 * V_154 = F_65 ( V_196 , struct V_153 , V_158 ) ;
F_76 ( V_7 , V_157 , V_154 ) ;
}
int F_77 ( struct V_159 * V_197 , unsigned int V_83 )
{
struct V_156 * V_157 = V_197 -> V_160 ;
struct V_6 * V_7 = V_157 -> V_7 ;
if ( F_78 ( V_7 ) && F_79 ( & V_7 -> V_198 ) &&
V_157 == & V_7 -> V_199 && V_7 -> V_144 -> V_169 ) {
struct V_153 * V_154 , * V_165 ;
F_80 (buf, tmp, &dmaq->queue, entry) {
F_81 ( & V_154 -> V_200 ) ;
F_82 ( & V_154 -> V_158 , V_201 ) ;
}
if ( V_157 -> V_180 ) {
F_82 ( & V_157 -> V_180 -> V_158 , V_201 ) ;
V_157 -> V_180 = NULL ;
}
return - V_202 ;
}
if ( ( V_157 == & V_7 -> V_199 && ! F_83 ( & V_7 -> V_203 ) ) ||
( V_157 == & V_7 -> V_204 && ! F_83 ( & V_7 -> V_205 ) ) )
F_84 ( & V_7 -> V_206 ,
V_207 , 20 ) ;
V_157 -> V_208 = 0 ;
return 0 ;
}
void F_85 ( struct V_159 * V_197 )
{
struct V_156 * V_157 = V_197 -> V_160 ;
struct V_6 * V_7 = V_157 -> V_7 ;
F_86 ( V_7 , V_157 ) ;
if ( ( V_157 == & V_7 -> V_199 && ! F_83 ( & V_7 -> V_203 ) ) ||
( V_157 == & V_7 -> V_204 && ! F_83 ( & V_7 -> V_205 ) ) )
F_87 ( & V_7 -> V_206 ) ;
}
static int F_88 ( struct V_209 * V_210 )
{
struct V_6 * V_7 = F_65 ( V_210 -> V_211 , struct V_6 , V_212 ) ;
unsigned long V_213 ;
int V_214 = 0 ;
switch ( V_210 -> V_22 ) {
case V_215 :
V_7 -> V_51 = V_210 -> V_85 ;
F_9 ( V_50 , V_210 -> V_85 ) ;
break;
case V_216 :
V_7 -> V_58 = V_210 -> V_85 ;
F_9 ( V_57 , V_210 -> V_85 ) ;
break;
case V_217 :
V_7 -> V_54 = V_210 -> V_85 ;
F_9 ( V_52 ,
V_7 -> V_53 ? - V_7 -> V_54 : V_7 -> V_54 ) ;
break;
case V_218 :
V_7 -> V_56 = V_210 -> V_85 ;
F_9 ( V_55 ,
V_7 -> V_53 ? - V_7 -> V_56 : V_7 -> V_56 ) ;
break;
case V_219 :
V_7 -> V_220 = V_210 -> V_85 ;
F_89 ( V_7 ) ;
break;
case V_221 :
V_7 -> V_222 = V_210 -> V_85 ;
F_90 ( V_7 , V_7 -> V_222 ) ;
break;
case V_223 :
V_7 -> V_53 = V_210 -> V_85 ;
F_9 ( V_52 ,
V_7 -> V_53 ? - V_7 -> V_54 : V_7 -> V_54 ) ;
F_9 ( V_55 ,
V_7 -> V_53 ? - V_7 -> V_56 : V_7 -> V_56 ) ;
break;
case V_224 :
V_7 -> V_87 = V_210 -> V_85 ;
V_214 = 1 ;
break;
case V_225 :
V_7 -> V_95 = V_210 -> V_85 ;
V_214 = 1 ;
break;
case V_226 :
V_7 -> V_94 = V_210 -> V_85 ;
V_214 = 1 ;
break;
case V_227 :
{
struct V_228 V_229 ;
V_229 . V_230 = V_231 ;
V_229 . V_232 = & V_7 -> V_233 ;
V_7 -> V_234 = V_210 -> V_85 ;
if ( V_7 -> V_233 ) {
if ( V_7 -> V_234 )
V_7 -> V_233 |= V_235 ;
else
V_7 -> V_233 &= ~ V_235 ;
F_10 ( V_7 , V_230 , V_236 , & V_229 ) ;
}
break;
}
default:
return - V_126 ;
}
if ( V_214 && V_7 -> V_237 ) {
F_91 ( & V_7 -> V_238 , V_213 ) ;
F_60 ( V_7 ) ;
F_51 ( V_7 ) ;
F_92 ( & V_7 -> V_238 , V_213 ) ;
}
return 0 ;
}
static inline struct V_159 * F_93 ( struct V_239 * V_239 )
{
return F_94 ( V_239 ) -> V_240 ;
}
static int F_95 ( struct V_239 * V_239 )
{
struct V_241 * V_242 = F_94 ( V_239 ) ;
struct V_6 * V_7 = F_96 ( V_239 ) ;
int V_243 = F_97 ( V_239 ) ;
if ( V_243 < 0 )
return V_243 ;
F_98 ( & V_7 -> V_244 ) ;
if ( V_242 -> V_245 == V_246 ) {
F_7 ( V_7 , & F_99 ( V_7 ) . V_247 ) ;
F_10 ( V_7 , V_230 , V_248 ) ;
} else {
F_5 ( V_7 , V_7 -> V_27 ) ;
}
F_100 ( & V_7 -> V_244 ) ;
return 0 ;
}
static int F_101 ( struct V_239 * V_239 )
{
struct V_241 * V_242 = F_94 ( V_239 ) ;
struct V_6 * V_7 = F_96 ( V_239 ) ;
struct V_249 * V_250 = V_239 -> V_251 ;
struct V_252 V_253 ;
unsigned long V_213 ;
F_98 ( & V_7 -> V_244 ) ;
F_102 ( V_7 ) ;
if ( V_250 == V_7 -> V_237 ) {
F_91 ( & V_7 -> V_238 , V_213 ) ;
F_60 ( V_7 ) ;
F_92 ( & V_7 -> V_238 , V_213 ) ;
V_7 -> V_237 = NULL ;
}
if ( V_242 -> V_245 == V_246 )
F_103 ( V_239 ) ;
else
F_104 ( V_239 , NULL ) ;
F_17 ( V_168 , 0x1f , 0 ) ;
F_17 ( V_142 , 0x1f , 0 ) ;
F_17 ( V_254 , 0x1f , 0 ) ;
F_17 ( V_255 , 0x1f , 0 ) ;
F_10 ( V_7 , V_256 , V_257 , 0 ) ;
if ( V_242 -> V_245 == V_246 )
F_10 ( V_7 , V_256 , V_258 , V_259 , & V_253 ) ;
F_100 ( & V_7 -> V_244 ) ;
return 0 ;
}
static T_2 F_105 ( struct V_239 * V_239 , char T_3 * V_260 ,
T_4 V_83 , T_5 * V_261 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
struct V_252 V_253 ;
V_253 . V_262 = V_83 / 3 ;
V_253 . V_263 = V_239 -> V_264 & V_265 ;
V_253 . V_266 = V_260 ;
V_253 . V_267 = V_239 ;
V_253 . V_268 = - V_269 ;
F_98 ( & V_7 -> V_244 ) ;
F_10 ( V_7 , V_256 , V_258 , V_270 , & V_253 ) ;
F_100 ( & V_7 -> V_244 ) ;
return V_253 . V_268 ;
}
static unsigned int F_106 ( struct V_239 * V_239 , T_6 * V_271 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
struct V_252 V_253 ;
unsigned int V_272 = F_107 ( V_239 , V_271 ) ;
V_253 . V_267 = V_239 ;
V_253 . V_273 = V_271 ;
V_253 . V_268 = 0 ;
F_98 ( & V_7 -> V_244 ) ;
F_10 ( V_7 , V_256 , V_258 , V_274 , & V_253 ) ;
F_100 ( & V_7 -> V_244 ) ;
return V_272 | V_253 . V_268 ;
}
static int F_108 ( struct V_239 * V_239 , void * V_232 ,
struct V_190 * V_275 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
struct V_8 * V_9 = V_7 -> V_11 ;
memset ( & V_275 -> V_144 . V_276 . V_277 , 0 , sizeof( V_275 -> V_144 . V_276 . V_277 ) ) ;
V_275 -> V_144 . V_276 . V_278 = 6750000 * 4 ;
V_275 -> V_144 . V_276 . V_279 = 2048 ;
V_275 -> V_144 . V_276 . V_280 = V_281 ;
V_275 -> V_144 . V_276 . V_185 = 64 * 4 ;
V_275 -> V_144 . V_276 . V_282 [ 0 ] = V_9 -> V_283 ;
V_275 -> V_144 . V_276 . V_83 [ 0 ] = V_9 -> V_19 - V_9 -> V_283 + 1 ;
V_275 -> V_144 . V_276 . V_282 [ 1 ] = V_9 -> V_284 ;
V_275 -> V_144 . V_276 . V_83 [ 1 ] = V_275 -> V_144 . V_276 . V_83 [ 0 ] ;
V_275 -> V_144 . V_276 . V_213 = 0 ;
return 0 ;
}
static int F_109 ( struct V_239 * V_239 , void * V_232 ,
struct V_190 * V_275 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
V_275 -> V_144 . V_285 . V_16 = V_7 -> V_16 ;
V_275 -> V_144 . V_285 . V_21 = V_7 -> V_21 ;
V_275 -> V_144 . V_285 . V_120 = V_7 -> V_120 ;
V_275 -> V_144 . V_285 . V_286 = V_7 -> V_144 -> V_2 ;
V_275 -> V_144 . V_285 . V_145 =
( V_275 -> V_144 . V_285 . V_16 * V_7 -> V_144 -> V_146 ) >> 3 ;
V_275 -> V_144 . V_285 . V_287 =
V_275 -> V_144 . V_285 . V_21 * V_275 -> V_144 . V_285 . V_145 ;
V_275 -> V_144 . V_285 . V_288 = V_289 ;
return 0 ;
}
static int F_110 ( struct V_239 * V_239 , void * V_232 ,
struct V_190 * V_275 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
struct V_108 T_3 * V_109 = V_275 -> V_144 . V_117 . V_109 ;
T_7 V_128 = V_275 -> V_144 . V_117 . V_128 ;
int V_137 = 0 ;
int V_3 ;
if ( V_290 > 0 ) {
F_111 ( V_291 L_13 ) ;
return - V_126 ;
}
V_275 -> V_144 . V_117 = V_7 -> V_117 ;
V_275 -> V_144 . V_117 . V_109 = V_109 ;
if ( V_109 == NULL )
V_128 = 0 ;
if ( V_7 -> V_110 < V_128 )
V_128 = V_7 -> V_110 ;
V_275 -> V_144 . V_117 . V_128 = V_128 ;
for ( V_3 = 0 ; ! V_137 && V_3 < V_128 ; V_3 ++ ) {
if ( F_112 ( & V_275 -> V_144 . V_117 . V_109 [ V_3 ] . V_115 , & V_7 -> V_109 [ V_3 ] . V_115 ,
sizeof( struct V_292 ) ) )
V_137 = - V_293 ;
}
return V_137 ;
}
static int F_113 ( struct V_239 * V_239 , void * V_232 ,
struct V_190 * V_275 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
struct V_1 * V_144 ;
enum V_119 V_120 ;
unsigned int V_121 , V_122 ;
V_144 = F_1 ( V_275 -> V_144 . V_285 . V_286 ) ;
if ( NULL == V_144 )
return - V_126 ;
V_120 = V_275 -> V_144 . V_285 . V_120 ;
V_121 = F_114 ( V_7 -> V_25 . V_16 * 4 , V_7 -> V_12 . V_16 ) ;
V_122 = F_114 ( V_7 -> V_25 . V_21 * 4 , V_7 -> V_12 . V_21 ) ;
if ( V_131 == V_120 ) {
V_120 = ( V_275 -> V_144 . V_285 . V_21 > V_122 / 2 )
? V_132
: V_134 ;
}
switch ( V_120 ) {
case V_133 :
case V_134 :
V_122 = V_122 / 2 ;
break;
default:
V_120 = V_132 ;
break;
}
V_275 -> V_144 . V_285 . V_120 = V_120 ;
if ( V_275 -> V_144 . V_285 . V_16 < 48 )
V_275 -> V_144 . V_285 . V_16 = 48 ;
if ( V_275 -> V_144 . V_285 . V_21 < 32 )
V_275 -> V_144 . V_285 . V_21 = 32 ;
if ( V_275 -> V_144 . V_285 . V_16 > V_121 )
V_275 -> V_144 . V_285 . V_16 = V_121 ;
if ( V_275 -> V_144 . V_285 . V_21 > V_122 )
V_275 -> V_144 . V_285 . V_21 = V_122 ;
V_275 -> V_144 . V_285 . V_16 &= ~ 0x03 ;
V_275 -> V_144 . V_285 . V_145 =
( V_275 -> V_144 . V_285 . V_16 * V_144 -> V_146 ) >> 3 ;
V_275 -> V_144 . V_285 . V_287 =
V_275 -> V_144 . V_285 . V_21 * V_275 -> V_144 . V_285 . V_145 ;
V_275 -> V_144 . V_285 . V_288 = V_289 ;
return 0 ;
}
static int F_115 ( struct V_239 * V_239 , void * V_232 ,
struct V_190 * V_275 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
if ( V_290 > 0 ) {
F_111 ( V_291 L_13 ) ;
return - V_126 ;
}
if ( V_275 -> V_144 . V_117 . V_109 == NULL )
V_275 -> V_144 . V_117 . V_128 = 0 ;
return F_50 ( V_7 , & V_275 -> V_144 . V_117 , true ) ;
}
static int F_116 ( struct V_239 * V_239 , void * V_232 ,
struct V_190 * V_275 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
int V_137 ;
V_137 = F_113 ( V_239 , V_232 , V_275 ) ;
if ( 0 != V_137 )
return V_137 ;
V_7 -> V_144 = F_1 ( V_275 -> V_144 . V_285 . V_286 ) ;
V_7 -> V_16 = V_275 -> V_144 . V_285 . V_16 ;
V_7 -> V_21 = V_275 -> V_144 . V_285 . V_21 ;
V_7 -> V_120 = V_275 -> V_144 . V_285 . V_120 ;
return 0 ;
}
static int F_117 ( struct V_239 * V_239 , void * V_232 ,
struct V_190 * V_275 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
int V_137 ;
unsigned long V_213 ;
if ( V_290 > 0 ) {
F_111 ( V_291 L_13 ) ;
return - V_126 ;
}
if ( V_275 -> V_144 . V_117 . V_109 == NULL )
V_275 -> V_144 . V_117 . V_128 = 0 ;
V_137 = F_50 ( V_7 , & V_275 -> V_144 . V_117 , true ) ;
if ( 0 != V_137 )
return V_137 ;
V_7 -> V_117 = V_275 -> V_144 . V_117 ;
V_7 -> V_110 = V_275 -> V_144 . V_117 . V_128 ;
if ( F_118 ( V_7 -> V_109 , V_275 -> V_144 . V_117 . V_109 ,
sizeof( struct V_108 ) * V_7 -> V_110 ) )
return - V_293 ;
if ( V_232 == V_7 -> V_237 ) {
F_91 ( & V_7 -> V_238 , V_213 ) ;
F_60 ( V_7 ) ;
F_51 ( V_7 ) ;
F_92 ( & V_7 -> V_238 , V_213 ) ;
}
return 0 ;
}
int F_119 ( struct V_239 * V_239 , void * V_232 , struct V_294 * V_3 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
unsigned int V_295 ;
V_295 = V_3 -> V_296 ;
if ( V_295 >= V_297 )
return - V_126 ;
if ( NULL == F_6 ( V_7 , V_3 -> V_296 ) . V_10 )
return - V_126 ;
V_3 -> V_296 = V_295 ;
V_3 -> type = V_298 ;
strcpy ( V_3 -> V_10 , F_6 ( V_7 , V_295 ) . V_10 ) ;
if ( F_6 ( V_7 , V_295 ) . V_72 )
V_3 -> type = V_299 ;
if ( V_295 == V_7 -> V_27 ) {
int V_300 = F_120 ( V_301 ) ;
int V_302 = F_120 ( V_303 ) ;
if ( 0 != ( V_300 & 0x40 ) )
V_3 -> V_304 |= V_305 ;
if ( 0 != ( V_302 & 0x40 ) )
V_3 -> V_304 |= V_306 ;
if ( 0 != ( V_302 & 0x0e ) )
V_3 -> V_304 |= V_307 ;
}
V_3 -> V_308 = V_309 ;
return 0 ;
}
int F_121 ( struct V_239 * V_239 , void * V_232 , unsigned int * V_3 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
* V_3 = V_7 -> V_27 ;
return 0 ;
}
int F_122 ( struct V_239 * V_239 , void * V_232 , unsigned int V_3 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
if ( V_3 >= V_297 )
return - V_126 ;
if ( NULL == F_6 ( V_7 , V_3 ) . V_10 )
return - V_126 ;
F_5 ( V_7 , V_3 ) ;
return 0 ;
}
int F_123 ( struct V_239 * V_239 , void * V_232 ,
struct V_310 * V_311 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
struct V_241 * V_242 = F_94 ( V_239 ) ;
T_7 V_312 , V_313 , V_314 ;
unsigned int V_315 = V_7 -> V_315 ;
strcpy ( V_311 -> V_316 , L_14 ) ;
F_124 ( V_311 -> F_99 , V_317 [ V_7 -> V_318 ] . V_10 ,
sizeof( V_311 -> F_99 ) ) ;
sprintf ( V_311 -> V_319 , L_15 , F_125 ( V_7 -> V_187 ) ) ;
V_311 -> V_320 = V_321 | V_322 ;
if ( ( V_315 != V_323 ) && ( V_315 != V_324 ) )
V_311 -> V_320 |= V_325 ;
V_312 = V_326 ;
if ( V_7 -> V_327 )
V_312 |= V_328 ;
V_313 = V_329 ;
if ( V_290 <= 0 && ! F_126 ( V_239 ) )
V_313 |= V_330 ;
V_314 = V_331 ;
switch ( V_242 -> V_245 ) {
case V_246 :
V_311 -> V_320 |= V_312 ;
break;
case V_332 :
V_311 -> V_320 |= V_313 ;
break;
case V_333 :
V_311 -> V_320 |= V_314 ;
break;
}
V_311 -> V_334 = V_312 | V_313 | V_314 |
V_311 -> V_320 | V_335 ;
if ( V_242 -> V_245 == V_246 ) {
V_311 -> V_320 &= ~ V_322 ;
if ( ! V_7 -> V_327 )
V_311 -> V_320 &= ~ V_321 ;
}
return 0 ;
}
int F_127 ( struct V_239 * V_239 , void * V_232 , T_8 V_22 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
struct V_249 * V_250 = V_232 ;
unsigned long V_213 ;
unsigned int V_3 ;
T_8 V_336 ;
if ( F_126 ( V_239 ) && V_7 -> V_237 ) {
return - V_202 ;
}
for ( V_3 = 0 ; V_3 < V_337 ; V_3 ++ )
if ( V_22 == V_338 [ V_3 ] . V_22 )
break;
if ( V_3 == V_337 )
for ( V_3 = 0 ; V_3 < V_337 ; V_3 ++ )
if ( V_22 & V_338 [ V_3 ] . V_22 )
break;
if ( V_3 == V_337 )
return - V_126 ;
if ( ( V_22 & V_339 ) && ( V_340 [ 0 ] != '-' ) ) {
if ( V_340 [ 0 ] == 'L' || V_340 [ 0 ] == 'l' ) {
if ( V_340 [ 1 ] == 'C' || V_340 [ 1 ] == 'c' )
V_336 = V_341 ;
else
V_336 = V_342 ;
} else {
if ( V_340 [ 0 ] == 'D' || V_340 [ 0 ] == 'd' )
V_336 = V_343 ;
else
V_336 = V_339 ;
}
for ( V_3 = 0 ; V_3 < V_337 ; V_3 ++ ) {
if ( V_336 == V_338 [ V_3 ] . V_22 )
break;
}
if ( V_3 == V_337 )
return - V_126 ;
}
V_22 = V_338 [ V_3 ] . V_22 ;
if ( ! F_126 ( V_239 ) && V_250 == V_7 -> V_237 ) {
F_91 ( & V_7 -> V_238 , V_213 ) ;
F_60 ( V_7 ) ;
F_92 ( & V_7 -> V_238 , V_213 ) ;
F_2 ( V_7 , & V_338 [ V_3 ] ) ;
F_91 ( & V_7 -> V_238 , V_213 ) ;
F_51 ( V_7 ) ;
F_92 ( & V_7 -> V_238 , V_213 ) ;
} else
F_2 ( V_7 , & V_338 [ V_3 ] ) ;
F_128 ( V_7 ) ;
return 0 ;
}
int F_129 ( struct V_239 * V_239 , void * V_232 , T_8 * V_22 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
* V_22 = V_7 -> V_11 -> V_22 ;
return 0 ;
}
static T_8 F_130 ( struct V_6 * V_7 )
{
static T_8 V_344 [] = {
V_345 ,
V_346 ,
V_347 ,
V_339 } ;
T_8 V_268 = 0 ;
T_9 V_348 = F_120 ( V_301 ) ;
T_9 V_349 = F_120 ( V_303 ) ;
if ( ! ( V_349 & 0x1 ) )
V_268 = V_345 ;
else
V_268 = V_344 [ V_348 & 0x03 ] ;
return V_268 ;
}
int F_131 ( struct V_239 * V_239 , void * V_232 , T_8 * V_308 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
* V_308 &= F_130 ( V_7 ) ;
return 0 ;
}
static int F_132 ( struct V_239 * V_239 , void * V_232 ,
struct V_350 * V_311 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
if ( V_311 -> type != V_351 &&
V_311 -> type != V_352 )
return - V_126 ;
V_311 -> V_353 = V_7 -> V_12 ;
V_311 -> V_354 = V_7 -> V_15 ;
V_311 -> V_355 . V_356 = 1 ;
V_311 -> V_355 . V_357 = 1 ;
if ( V_7 -> V_11 -> V_22 & V_23 ) {
V_311 -> V_355 . V_356 = 11 ;
V_311 -> V_355 . V_357 = 10 ;
}
if ( V_7 -> V_11 -> V_22 & V_358 ) {
V_311 -> V_355 . V_356 = 54 ;
V_311 -> V_355 . V_357 = 59 ;
}
return 0 ;
}
static int F_133 ( struct V_239 * V_239 , void * V_275 , struct V_359 * V_360 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
if ( V_360 -> type != V_351 &&
V_360 -> type != V_352 )
return - V_126 ;
V_360 -> V_115 = V_7 -> V_25 ;
return 0 ;
}
static int F_134 ( struct V_239 * V_239 , void * V_275 , const struct V_359 * V_360 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
struct V_292 * V_105 = & V_7 -> V_12 ;
struct V_292 * V_115 = & V_7 -> V_25 ;
if ( V_360 -> type != V_351 &&
V_360 -> type != V_352 )
return - V_126 ;
if ( V_7 -> V_237 )
return - V_202 ;
if ( F_83 ( & V_7 -> V_205 ) )
return - V_202 ;
* V_115 = V_360 -> V_115 ;
if ( V_115 -> V_18 < V_105 -> V_18 )
V_115 -> V_18 = V_105 -> V_18 ;
if ( V_115 -> V_18 > V_105 -> V_18 + V_105 -> V_21 )
V_115 -> V_18 = V_105 -> V_18 + V_105 -> V_21 ;
if ( V_115 -> V_21 > V_105 -> V_18 - V_115 -> V_18 + V_105 -> V_21 )
V_115 -> V_21 = V_105 -> V_18 - V_115 -> V_18 + V_105 -> V_21 ;
if ( V_115 -> V_13 < V_105 -> V_13 )
V_115 -> V_13 = V_105 -> V_13 ;
if ( V_115 -> V_13 > V_105 -> V_13 + V_105 -> V_16 )
V_115 -> V_13 = V_105 -> V_13 + V_105 -> V_16 ;
if ( V_115 -> V_16 > V_105 -> V_13 - V_115 -> V_13 + V_105 -> V_16 )
V_115 -> V_16 = V_105 -> V_13 - V_115 -> V_13 + V_105 -> V_16 ;
return 0 ;
}
int F_135 ( struct V_239 * V_239 , void * V_232 ,
struct V_361 * V_362 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
int V_295 ;
if ( 0 != V_362 -> V_296 )
return - V_126 ;
memset ( V_362 , 0 , sizeof( * V_362 ) ) ;
for ( V_295 = 0 ; V_295 < V_297 ; V_295 ++ ) {
if ( F_6 ( V_7 , V_295 ) . V_72 )
break;
}
if ( V_295 == V_297 )
return - V_126 ;
if ( NULL != F_6 ( V_7 , V_295 ) . V_10 ) {
strcpy ( V_362 -> V_10 , L_16 ) ;
V_362 -> type = V_363 ;
F_10 ( V_7 , V_230 , V_364 , V_362 ) ;
V_362 -> V_365 = V_366 |
V_367 |
V_368 |
V_369 ;
V_362 -> V_370 = F_136 ( V_7 ) ;
V_362 -> V_371 = F_137 ( V_362 -> V_370 ) ;
}
if ( 0 != ( F_120 ( V_301 ) & 0x03 ) )
V_362 -> signal = 0xffff ;
return 0 ;
}
int F_138 ( struct V_239 * V_239 , void * V_232 ,
const struct V_361 * V_362 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
int V_372 , V_373 ;
if ( 0 != V_362 -> V_296 )
return - V_126 ;
V_373 = V_7 -> V_374 . V_373 ;
if ( V_324 == V_373 ) {
V_372 = F_136 ( V_7 ) ;
V_373 = F_137 ( V_372 ) ;
}
if ( V_373 != V_362 -> V_371 )
V_7 -> V_374 . V_373 = V_362 -> V_371 ;
return 0 ;
}
int F_139 ( struct V_239 * V_239 , void * V_232 ,
struct V_375 * V_275 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
if ( 0 != V_275 -> V_230 )
return - V_126 ;
F_10 ( V_7 , V_230 , V_376 , V_275 ) ;
return 0 ;
}
int F_140 ( struct V_239 * V_239 , void * V_232 ,
const struct V_375 * V_275 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
if ( 0 != V_275 -> V_230 )
return - V_126 ;
F_10 ( V_7 , V_230 , V_377 , V_275 ) ;
F_128 ( V_7 ) ;
return 0 ;
}
static int F_141 ( struct V_239 * V_239 , void * V_232 ,
struct V_378 * V_275 )
{
if ( V_275 -> V_296 >= V_4 )
return - V_126 ;
F_124 ( V_275 -> V_379 , V_5 [ V_275 -> V_296 ] . V_10 ,
sizeof( V_275 -> V_379 ) ) ;
V_275 -> V_286 = V_5 [ V_275 -> V_296 ] . V_2 ;
return 0 ;
}
static int F_142 ( struct V_239 * V_239 , void * V_232 ,
struct V_378 * V_275 )
{
if ( V_290 > 0 ) {
F_111 ( V_291 L_13 ) ;
return - V_126 ;
}
if ( ( V_275 -> V_296 >= V_4 ) || V_5 [ V_275 -> V_296 ] . V_169 )
return - V_126 ;
F_124 ( V_275 -> V_379 , V_5 [ V_275 -> V_296 ] . V_10 ,
sizeof( V_275 -> V_379 ) ) ;
V_275 -> V_286 = V_5 [ V_275 -> V_296 ] . V_2 ;
return 0 ;
}
static int F_143 ( struct V_239 * V_239 , void * V_275 ,
struct V_380 * V_381 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
* V_381 = V_7 -> V_123 ;
V_381 -> V_365 = V_382 ;
return 0 ;
}
static int F_144 ( struct V_239 * V_239 , void * V_275 ,
const struct V_380 * V_381 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
struct V_1 * V_144 ;
if ( ! F_145 ( V_383 ) &&
! F_145 ( V_384 ) )
return - V_385 ;
V_144 = F_1 ( V_381 -> V_144 . V_286 ) ;
if ( NULL == V_144 )
return - V_126 ;
V_7 -> V_123 = * V_381 ;
V_7 -> V_125 = V_144 ;
if ( 0 == V_7 -> V_123 . V_144 . V_145 )
V_7 -> V_123 . V_144 . V_145 =
V_7 -> V_123 . V_144 . V_16 * V_144 -> V_146 / 8 ;
return 0 ;
}
static int F_146 ( struct V_239 * V_239 , void * V_232 , unsigned int V_386 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
unsigned long V_213 ;
if ( V_386 ) {
if ( V_290 > 0 ) {
F_3 ( L_17 ) ;
return - V_126 ;
}
if ( V_7 -> V_237 && V_232 != V_7 -> V_237 )
return - V_202 ;
V_7 -> V_237 = V_232 ;
F_91 ( & V_7 -> V_238 , V_213 ) ;
F_51 ( V_7 ) ;
F_92 ( & V_7 -> V_238 , V_213 ) ;
}
if ( ! V_386 ) {
if ( V_232 != V_7 -> V_237 )
return - V_126 ;
F_91 ( & V_7 -> V_238 , V_213 ) ;
F_60 ( V_7 ) ;
F_92 ( & V_7 -> V_238 , V_213 ) ;
V_7 -> V_237 = NULL ;
}
return 0 ;
}
static int F_147 ( struct V_239 * V_239 , void * V_232 ,
struct V_387 * V_96 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
V_96 -> V_85 = F_120 ( V_96 -> V_96 & 0xffffff ) ;
V_96 -> V_183 = 1 ;
return 0 ;
}
static int F_148 ( struct V_239 * V_239 , void * V_232 ,
const struct V_387 * V_96 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
F_9 ( V_96 -> V_96 & 0xffffff , V_96 -> V_85 ) ;
return 0 ;
}
static int F_149 ( struct V_239 * V_239 , void * V_232 ,
struct V_361 * V_362 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
if ( 0 != V_362 -> V_296 )
return - V_126 ;
strcpy ( V_362 -> V_10 , L_18 ) ;
F_10 ( V_7 , V_230 , V_364 , V_362 ) ;
V_362 -> V_371 &= V_388 | V_389 ;
if ( V_7 -> V_26 -> V_390 == V_391 ) {
V_362 -> signal = 0xf800 - ( ( F_120 ( 0x581 ) & 0x1f ) << 11 ) ;
V_362 -> V_370 = ( F_120 ( 0x529 ) & 0x08 ) ?
V_392 : V_393 ;
}
return 0 ;
}
static int F_150 ( struct V_239 * V_239 , void * V_232 ,
const struct V_361 * V_362 )
{
struct V_6 * V_7 = F_96 ( V_239 ) ;
if ( 0 != V_362 -> V_296 )
return - V_126 ;
F_10 ( V_7 , V_230 , V_394 , V_362 ) ;
return 0 ;
}
int F_151 ( struct V_6 * V_7 )
{
struct V_395 * V_396 = & V_7 -> V_212 ;
struct V_159 * V_189 ;
int V_243 ;
if ( V_397 < 2 || V_397 > V_398 )
V_397 = 2 ;
if ( V_399 > V_400 )
V_399 = V_400 ;
V_399 = ( V_399 + V_401 - 1 ) & V_402 ;
F_152 ( V_396 , 11 ) ;
F_153 ( V_396 , & V_403 ,
V_215 , 0 , 255 , 1 , 128 ) ;
F_153 ( V_396 , & V_403 ,
V_217 , 0 , 127 , 1 , 68 ) ;
F_153 ( V_396 , & V_403 ,
V_218 , 0 , 127 , 1 , 64 ) ;
F_153 ( V_396 , & V_403 ,
V_216 , - 128 , 127 , 1 , 0 ) ;
F_153 ( V_396 , & V_403 ,
V_224 , 0 , 1 , 1 , 0 ) ;
F_153 ( V_396 , & V_403 ,
V_219 , 0 , 1 , 1 , 0 ) ;
F_153 ( V_396 , & V_403 ,
V_221 , - 15 , 15 , 1 , 0 ) ;
F_154 ( V_396 , & V_404 , NULL ) ;
F_154 ( V_396 , & V_405 , NULL ) ;
F_154 ( V_396 , & V_406 , NULL ) ;
F_154 ( V_396 , & V_407 , NULL ) ;
if ( V_396 -> error )
return V_396 -> error ;
if ( F_155 ( V_7 ) ) {
V_396 = & V_7 -> V_408 ;
F_152 ( V_396 , 2 ) ;
F_156 ( V_396 , & V_7 -> V_212 ,
V_409 ) ;
if ( V_396 -> error )
return V_396 -> error ;
}
V_7 -> V_220 = 1 ;
if ( V_7 -> V_233 && V_407 . V_410 )
V_7 -> V_233 |= V_235 ;
V_7 -> V_411 = 0 ;
F_157 ( & V_7 -> V_199 . V_240 ) ;
F_158 ( & V_7 -> V_199 . V_176 ) ;
V_7 -> V_199 . V_176 . V_412 = V_413 ;
V_7 -> V_199 . V_176 . V_260 = ( unsigned long ) ( & V_7 -> V_199 ) ;
V_7 -> V_199 . V_7 = V_7 ;
V_7 -> V_144 = F_1 ( V_414 ) ;
V_7 -> V_16 = 720 ;
V_7 -> V_21 = 576 ;
V_7 -> V_120 = V_132 ;
V_7 -> V_117 . V_127 . V_16 = V_7 -> V_16 ;
V_7 -> V_117 . V_127 . V_21 = V_7 -> V_21 ;
V_7 -> V_117 . V_120 = V_132 ;
V_7 -> V_123 . V_144 . V_16 = V_7 -> V_16 ;
V_7 -> V_123 . V_144 . V_21 = V_7 -> V_21 ;
V_7 -> V_123 . V_144 . V_286 = V_7 -> V_144 -> V_2 ;
V_7 -> V_123 . V_144 . V_288 = V_289 ;
if ( V_317 [ V_7 -> V_318 ] . V_415 )
F_159 ( V_7 ) ;
V_189 = & V_7 -> V_205 ;
V_189 -> type = V_351 ;
V_189 -> V_416 = V_417 | V_418 ;
if ( V_419 )
V_189 -> V_416 |= V_420 ;
V_189 -> V_160 = & V_7 -> V_199 ;
V_189 -> V_421 = & V_422 ;
V_189 -> V_423 = V_424 ;
V_189 -> V_425 = & V_426 ;
V_189 -> V_427 = sizeof( struct V_153 ) ;
V_189 -> V_428 = V_429 ;
V_189 -> V_244 = & V_7 -> V_244 ;
V_243 = F_160 ( V_189 ) ;
if ( V_243 )
return V_243 ;
F_161 ( V_7 -> V_187 , & V_7 -> V_199 . V_171 ) ;
V_189 = & V_7 -> V_203 ;
V_189 -> type = V_430 ;
V_189 -> V_416 = V_417 | V_418 ;
if ( V_419 )
V_189 -> V_416 |= V_420 ;
V_189 -> V_160 = & V_7 -> V_204 ;
V_189 -> V_421 = & V_431 ;
V_189 -> V_423 = V_424 ;
V_189 -> V_425 = & V_426 ;
V_189 -> V_427 = sizeof( struct V_153 ) ;
V_189 -> V_428 = V_429 ;
V_189 -> V_244 = & V_7 -> V_244 ;
V_243 = F_160 ( V_189 ) ;
if ( V_243 )
return V_243 ;
F_161 ( V_7 -> V_187 , & V_7 -> V_204 . V_171 ) ;
return 0 ;
}
void F_162 ( struct V_6 * V_7 )
{
F_163 ( & V_7 -> V_205 ) ;
F_164 ( V_7 -> V_187 , & V_7 -> V_199 . V_171 ) ;
F_163 ( & V_7 -> V_203 ) ;
F_164 ( V_7 -> V_187 , & V_7 -> V_204 . V_171 ) ;
F_165 ( & V_7 -> V_212 ) ;
if ( F_155 ( V_7 ) )
F_165 ( & V_7 -> V_408 ) ;
}
int F_159 ( struct V_6 * V_7 )
{
int V_432 = V_317 [ V_7 -> V_318 ] . V_415 ;
int V_433 ;
unsigned int V_434 = V_317 [ V_7 -> V_318 ] . V_434 ;
F_9 ( V_435 , V_415 [ V_432 ] [ 0 ] ) ;
V_433 = V_415 [ V_432 ] [ 1 ] ;
if ( V_434 & V_436 )
V_433 &= ~ V_437 ;
F_9 ( V_438 , V_433 ) ;
F_9 ( V_439 , V_415 [ V_432 ] [ 2 ] ) ;
F_9 ( V_440 , V_415 [ V_432 ] [ 4 ] ) ;
V_433 = V_415 [ V_432 ] [ 5 ] ;
if ( V_434 & V_441 )
V_433 &= ~ V_442 ;
if ( V_434 & V_443 )
V_433 |= V_444 ;
F_9 ( V_445 , V_433 ) ;
V_433 = V_415 [ V_432 ] [ 6 ] ;
if ( V_434 & V_446 ) {
V_433 &= ~ V_447 ;
V_433 |= V_448 ;
}
F_9 ( V_449 , V_433 ) ;
F_9 ( V_450 , V_415 [ V_432 ] [ 7 ] ) ;
F_9 ( V_451 , V_415 [ V_432 ] [ 8 ] ) ;
F_9 ( V_452 , V_415 [ V_432 ] [ 3 ] ) ;
return 0 ;
}
int F_166 ( struct V_6 * V_7 )
{
F_2 ( V_7 , & V_338 [ 0 ] ) ;
F_5 ( V_7 , 0 ) ;
F_167 ( & V_7 -> V_212 ) ;
F_89 ( V_7 ) ;
F_90 ( V_7 , V_7 -> V_222 ) ;
return 0 ;
}
void F_168 ( struct V_6 * V_7 )
{
static const char * V_453 [] = {
L_19 , L_20 , L_21 , L_22 } ;
T_7 V_348 , V_349 ;
V_348 = F_120 ( V_301 ) ;
V_349 = F_120 ( V_303 ) ;
F_3 ( L_23 ,
( V_348 & 0x40 ) ? L_24 : L_25 ,
( V_349 & 0x40 ) ? L_26 : L_27 ,
V_453 [ V_348 & 0x03 ] ) ;
V_7 -> V_34 = ( V_348 & 0x40 ) || ( V_349 & 0x40 ) || ! ( V_349 & 0x1 ) ;
if ( V_7 -> V_34 ) {
if ( V_7 -> V_234 )
V_7 -> V_411 = 1 ;
F_89 ( V_7 ) ;
} else {
F_128 ( V_7 ) ;
}
if ( ( V_349 & 0x80 ) && ! V_33 && ! V_7 -> V_34 )
F_169 ( V_48 , 0x20 ) ;
else
F_170 ( V_48 , 0x20 ) ;
if ( V_7 -> V_454 && V_7 -> V_454 -> V_455 )
V_7 -> V_454 -> V_455 ( V_7 ) ;
}
void F_171 ( struct V_6 * V_7 , unsigned long V_304 )
{
enum V_119 V_120 ;
F_172 ( & V_7 -> V_238 ) ;
if ( V_7 -> V_199 . V_180 ) {
V_120 = V_7 -> V_120 ;
if ( F_52 ( V_120 ) ) {
if ( ( V_304 & 0x10 ) == 0x00 ) {
V_7 -> V_199 . V_180 -> V_166 = 1 ;
goto V_456;
}
if ( ! V_7 -> V_199 . V_180 -> V_166 )
goto V_456;
} else if ( V_120 == V_133 ) {
if ( ( V_304 & 0x10 ) != 0x10 )
goto V_456;
} else if ( V_120 == V_134 ) {
if ( ( V_304 & 0x10 ) != 0x00 )
goto V_456;
}
F_173 ( V_7 , & V_7 -> V_199 , V_457 ) ;
}
F_174 ( V_7 , & V_7 -> V_199 ) ;
V_456:
F_175 ( & V_7 -> V_238 ) ;
}
