static int T_1 F_1 ( char * V_1 )
{
char * V_2 ;
V_3 = 0 ;
while ( ( V_2 = F_2 ( & V_1 , L_1 ) ) != NULL ) {
switch ( V_2 [ 0 ] ) {
case 'a' :
if ( strcmp ( V_2 , L_2 ) == 0 )
V_3 |= - 1 ;
break;
case 'b' :
if ( strncmp ( V_2 , L_3 , 3 ) == 0 )
V_3 |= V_4 ;
if ( strncmp ( V_2 , L_4 , 3 ) == 0 )
V_3 |= V_5 ;
break;
case 'c' :
V_3 |= V_6 ;
break;
case 'e' :
V_3 |= V_7 ;
break;
case 'm' :
V_3 |= V_8 ;
break;
case 'n' :
if ( strcmp ( V_2 , L_5 ) == 0 )
V_3 = 0 ;
break;
case 's' :
V_3 |= V_9 ;
break;
}
}
return 1 ;
}
static inline unsigned char F_3 ( T_2 * V_10 , unsigned int V_11 )
{
unsigned int V_12 = V_11 << V_10 -> V_13 . V_14 ;
return F_4 ( V_10 -> V_13 . V_15 + V_12 ) ;
}
static inline void F_5 ( T_2 * V_10 , unsigned int V_11 , unsigned int V_16 )
{
unsigned int V_12 = V_11 << V_10 -> V_13 . V_14 ;
F_6 ( V_16 , V_10 -> V_13 . V_15 + V_12 ) ;
}
static void F_7 ( T_2 * V_10 )
{
unsigned char V_17 , V_18 , V_19 ;
V_17 = F_3 ( V_10 , V_20 ) ;
V_18 = F_3 ( V_10 , V_21 ) ;
V_19 = F_3 ( V_10 , V_22 ) ;
F_8 ( L_6
L_7 ,
F_3 ( V_10 , V_23 ) ,
F_3 ( V_10 , V_24 ) ,
F_3 ( V_10 , V_25 ) , V_18 , V_19 , V_17 ,
F_3 ( V_10 , V_26 ) ) ;
F_8 ( L_8 ,
F_3 ( V_10 , V_27 ) ,
F_3 ( V_10 , V_28 ) ,
F_3 ( V_10 , V_29 ) ,
F_3 ( V_10 , V_30 ) ) ;
}
static void F_9 ( struct V_31 * V_32 , const char * V_33 , const char * V_34 )
{
F_8 ( L_9 ,
V_33 , V_32 -> V_35 , V_32 -> V_36 , V_32 -> V_37 ,
V_32 -> V_38 , V_34 ) ;
}
static void F_10 ( T_2 * V_10 )
{
static int V_39 = 0 ;
int V_40 ;
if ( V_39 ++ )
return;
F_8 ( L_10 ) ;
F_8 ( L_11 ,
V_10 -> V_41 , V_10 -> V_42 , V_10 -> V_43 ,
V_10 -> V_44 ) ;
F_8 ( L_12 ,
V_10 -> V_13 . V_14 , V_10 -> V_13 . V_45 ,
V_10 -> V_13 . V_46 [ 0 ] , V_10 -> V_13 . V_46 [ 1 ] , V_10 -> V_13 . V_46 [ 2 ] ,
V_10 -> V_13 . V_46 [ 3 ] ) ;
F_8 ( L_13 ,
V_10 -> V_13 . type , V_10 -> V_13 . V_47 ) ;
F_9 ( & V_10 -> V_13 . V_32 , L_14 , L_15 ) ;
F_8 ( L_16 ,
V_10 -> V_13 . V_48 ,
V_10 -> V_13 . V_49 , V_10 -> V_13 . V_50 ) ;
F_8 ( L_17
L_18 ,
V_10 -> V_51 . V_52 , V_10 -> V_51 . V_53 , V_10 -> V_51 . V_54 ,
V_10 -> V_51 . V_55 , V_10 -> V_51 . V_56 , V_10 -> V_51 . V_57 ,
V_10 -> V_51 . V_58 , V_10 -> V_51 . V_59 , V_10 -> V_51 . V_60 ,
V_10 -> V_51 . V_61 ) ;
F_8 ( L_19 ,
V_10 -> V_62 . V_63 , V_10 -> V_62 . V_64 ,
V_10 -> V_62 . V_65 , V_10 -> V_62 . V_66 ) ;
for ( V_40 = 0 ; V_40 < 8 ; V_40 ++ ) {
F_8 ( L_20 ,
V_40 , V_10 -> V_67 [ V_40 ] , V_40 ,
V_10 -> V_68 [ V_40 ] . V_69 , V_10 -> V_68 [ V_40 ] . V_70 ,
V_10 -> V_68 [ V_40 ] . V_71 , V_10 -> V_68 [ V_40 ] . V_72 ) ;
}
F_8 ( L_21 ,
V_10 -> V_73 . V_74 , V_10 -> V_73 . V_75 ,
V_10 -> V_73 . V_76 , V_10 -> V_73 . V_77 ) ;
F_8 ( L_22 ,
V_10 -> V_78 , V_10 -> V_79 ) ;
}
static void F_11 ( T_2 * V_10 , const char * V_80 )
{
int V_81 = 0 ;
if ( V_10 -> V_41 != V_82 ) {
F_8 ( V_83 L_23 ) ;
V_81 ++ ;
}
if ( V_10 -> V_79 != V_82 ) {
F_8 ( V_83 L_24 ) ;
V_81 ++ ;
}
if ( V_81 ) {
F_10 ( V_10 ) ;
F_12 ( L_25 , V_80 ) ;
}
}
static const char * F_13 ( int V_18 )
{
static const char * V_84 [] = {
L_26 , L_27 ,
L_28 , L_29 ,
L_30 , L_31 ,
L_32 , L_33
} ;
return V_84 [ V_18 & V_85 ] ;
}
static const char * F_14 ( T_2 * V_10 )
{
static const char * V_84 [] = {
[ V_86 ] = L_34 ,
[ V_87 ] = L_35 ,
[ V_88 ] = L_36 ,
[ V_89 ] = L_37 ,
[ V_90 ] = L_38 ,
[ V_91 ] = L_39 ,
[ V_92 ] = L_40 ,
[ V_93 ] = L_41 ,
[ V_94 ] = L_42 ,
[ V_95 ] = L_43 ,
[ V_96 ] = L_44 ,
} ;
if ( V_10 -> V_13 . V_47 < F_15 ( V_84 ) &&
V_84 [ V_10 -> V_13 . V_47 ] )
return V_84 [ V_10 -> V_13 . V_47 ] ;
return L_45 ;
}
static char F_16 ( T_2 * V_10 )
{
if ( V_10 -> V_43 )
return '0' + V_10 -> V_43 -> V_68 -> V_97 ;
else
return 'H' ;
}
static void
F_17 ( T_2 * V_10 , char V_98 , char * V_99 , T_3 V_100 )
{
static char V_101 [ 1024 ] ;
vsnprintf ( V_101 , sizeof( V_101 ) , V_99 , V_100 ) ;
F_8 ( L_46 , V_10 -> V_42 -> V_102 , V_98 , V_101 ) ;
}
static void F_18 ( T_2 * V_10 , int V_103 ,
struct V_104 * V_43 , char * V_99 , ... )
{
T_3 args ;
if ( V_103 != 0 && ! ( V_103 & V_3 ) )
return;
va_start ( args , V_99 ) ;
F_17 ( V_10 , '0' + V_43 -> V_68 -> V_97 , V_99 , args ) ;
va_end ( args ) ;
F_8 ( L_47 ) ;
F_19 ( V_43 -> V_105 ) ;
}
static void
F_20 ( T_2 * V_10 , int V_103 , int V_98 , char * V_99 , ... )
{
T_3 args ;
if ( V_103 != 0 && ! ( V_103 & V_3 ) )
return;
if ( V_98 < 0 )
V_98 = 'H' ;
else
V_98 += '0' ;
va_start ( args , V_99 ) ;
F_17 ( V_10 , V_98 , V_99 , args ) ;
va_end ( args ) ;
F_8 ( L_48 ) ;
}
static void F_21 ( T_2 * V_10 , int V_103 , char * V_99 , ... )
{
T_3 args ;
if ( V_103 != 0 && ! ( V_103 & V_3 ) )
return;
va_start ( args , V_99 ) ;
F_17 ( V_10 , F_16 ( V_10 ) , V_99 , args ) ;
va_end ( args ) ;
F_8 ( L_48 ) ;
}
static void F_22 ( int V_18 , int V_106 , int V_107 , int V_108 )
{
V_109 [ V_110 ] . V_18 = V_18 ;
V_109 [ V_110 ] . V_106 = V_106 ;
V_109 [ V_110 ] . V_107 = V_107 ;
V_109 [ V_110 ] . V_108 = V_108 ;
V_110 = ( V_110 + 1 ) & ( V_111 - 1 ) ;
}
static void F_23 ( T_2 * V_10 , unsigned int V_112 )
{
V_113 [ V_114 ] . V_112 = V_112 ;
V_113 [ V_114 ] . V_115 = F_24 ( 0 ) ;
V_114 = ( V_114 + 1 ) & 7 ;
F_5 ( V_10 , V_25 , V_112 ) ;
}
static void F_25 ( void )
{
int V_40 ;
V_40 = V_110 ;
F_8 ( V_116 L_49 ) ;
do {
F_8 ( L_50 ,
V_109 [ V_40 ] . V_18 , V_109 [ V_40 ] . V_106 ,
V_109 [ V_40 ] . V_107 , V_109 [ V_40 ] . V_108 ) ;
V_40 = ( V_40 + 1 ) & ( V_111 - 1 ) ;
if ( ( ( V_40 ^ V_110 ) & 7 ) == 0 )
F_8 ( L_48 ) ;
} while ( V_40 != V_110 );
if ( ( V_40 ^ V_110 ) & 7 )
F_8 ( L_48 ) ;
V_40 = V_114 ;
F_8 ( V_116 L_51 ) ;
do {
F_8 ( L_52 , V_113 [ V_40 ] . V_112 , V_113 [ V_40 ] . V_115 ) ;
V_40 = ( V_40 + 1 ) & 7 ;
} while ( V_40 != V_114 );
F_8 ( L_48 ) ;
}
static inline unsigned short
F_26 ( T_2 * V_10 , int V_117 )
{
unsigned short V_118 = V_119 ;
struct V_120 * V_121 ;
int V_122 = 0 ;
while ( ( V_121 = F_27 ( & V_10 -> V_13 . V_123 , V_122 ++ ) ) != NULL ) {
if ( V_117 >= V_121 -> V_124 )
break;
}
if ( V_121 ) {
if ( V_121 -> V_121 [ 0 ] == V_125 )
V_118 = V_125 | V_121 -> V_121 [ 2 ] << 8 ;
else
V_118 = V_121 -> V_121 [ 0 ] ;
}
F_21 ( V_10 , V_8 ,
L_53 , V_118 , V_117 ) ;
return V_118 ;
}
static int F_28 ( T_2 * V_10 , int V_126 )
{
int V_127 = ( V_10 -> V_62 . V_63 * V_126 ) / 1000 ;
F_29 ( V_10 ) ;
if ( V_127 < 4 )
V_127 = 4 ;
else if ( V_127 > 35 )
V_127 = 35 ;
return V_127 & 31 ;
}
static void F_30 ( T_2 * V_10 , int V_98 )
{
unsigned int V_128 ;
F_5 ( V_10 , V_129 , V_10 -> V_68 [ V_98 ] . V_71 ) ;
F_5 ( V_10 , V_130 , V_10 -> V_68 [ V_98 ] . V_70 ) ;
V_128 = V_10 -> V_13 . V_46 [ 2 ] ;
if ( V_10 -> V_68 [ V_98 ] . V_131 >= ( 200 / 4 ) )
V_128 = V_128 & ~ V_132 ;
F_5 ( V_10 , V_29 , V_128 ) ;
}
static void F_31 ( T_2 * V_10 , char * V_121 )
{
struct V_133 * V_134 = & V_10 -> V_68 [ V_10 -> V_43 -> V_68 -> V_97 ] ;
enum { V_135 , V_136 , V_137 , V_138 } V_139 = V_137 ;
#ifdef F_32
switch ( V_121 [ 0 ] ) {
case V_140 :
if ( V_134 -> V_72 == V_141 ) {
V_134 -> V_72 = V_142 ;
V_139 = V_136 ;
}
break;
case V_125 :
switch ( V_134 -> V_72 ) {
case V_142 :
V_139 = V_138 ;
break;
default:
F_23 ( V_10 , V_143 ) ;
if ( V_121 [ 4 ] > V_10 -> V_62 . V_66 )
V_121 [ 4 ] = V_10 -> V_62 . V_66 ;
if ( V_121 [ 3 ] < 1000 / V_10 -> V_62 . V_63 )
V_121 [ 3 ] = 1000 / V_10 -> V_62 . V_63 ;
F_33 ( & V_10 -> V_13 . V_123 ) ;
F_34 ( & V_10 -> V_13 . V_123 , 5 ,
V_125 , 3 , V_144 ,
V_121 [ 3 ] , V_121 [ 4 ] ) ;
V_10 -> V_13 . V_47 = V_93 ;
V_134 -> V_72 = V_145 ;
V_139 = V_135 ;
break;
case V_141 :
V_139 = V_138 ;
if ( V_121 [ 4 ] <= V_10 -> V_62 . V_66 &&
V_121 [ 3 ] >= 1000 / V_10 -> V_62 . V_63 ) {
V_134 -> V_72 = V_146 ;
V_139 = V_135 ;
}
break;
}
}
#else
V_139 = V_138 ;
#endif
switch ( V_139 ) {
case V_135 :
V_134 -> V_131 = V_121 [ 3 ] ;
V_134 -> V_71 = V_121 [ 4 ] ;
V_134 -> V_70 = F_28 ( V_10 , V_121 [ 3 ] * 4 ) ;
F_30 ( V_10 , V_10 -> V_43 -> V_68 -> V_97 ) ;
break;
case V_138 :
F_23 ( V_10 , V_143 ) ;
F_33 ( & V_10 -> V_13 . V_123 ) ;
F_34 ( & V_10 -> V_13 . V_123 , 1 , V_140 ) ;
V_10 -> V_13 . V_47 = V_93 ;
case V_136 :
V_134 -> V_131 = V_10 -> V_62 . V_65 / 4 ;
V_134 -> V_71 = 0 ;
V_134 -> V_70 = V_10 -> V_13 . V_48 ;
F_30 ( V_10 , V_10 -> V_43 -> V_68 -> V_97 ) ;
break;
case V_137 :
break;
}
}
static void F_35 ( T_2 * V_10 , int V_147 )
{
struct V_31 * V_32 = & V_10 -> V_13 . V_32 ;
F_29 ( V_10 ) ;
F_36 ( V_147 < 0 ) ;
V_32 -> V_47 -= V_147 ;
while ( V_147 != 0 ) {
if ( V_32 -> V_36 > V_147 )
break;
V_147 -= V_32 -> V_36 ;
if ( ! F_37 ( V_32 ) && V_147 ) {
F_8 ( V_148 L_54 ,
V_10 -> V_42 -> V_102 , '0' + V_10 -> V_43 -> V_68 -> V_97 ) ;
return;
}
}
V_32 -> V_36 -= V_147 ;
if ( V_32 -> V_36 )
V_32 -> V_35 += V_147 ;
else
V_32 -> V_35 = NULL ;
}
static void F_38 ( T_2 * V_10 , T_4 V_149 )
{
struct V_31 * V_32 = & V_10 -> V_13 . V_32 ;
F_29 ( V_10 ) ;
if ( V_149 == V_150 )
F_5 ( V_10 , V_151 , F_39 ( V_32 ) ) ;
else
F_40 ( V_32 , F_3 ( V_10 , V_151 ) ) ;
if ( V_32 -> V_36 == 0 )
F_37 ( V_32 ) ;
}
static void F_41 ( T_2 * V_10 , unsigned int V_152 )
{
F_5 ( V_10 , V_153 , V_152 ) ;
F_5 ( V_10 , V_154 , V_152 >> 8 ) ;
F_5 ( V_10 , V_155 , V_152 >> 16 ) ;
}
static unsigned int F_42 ( T_2 * V_10 )
{
return F_3 ( V_10 , V_23 ) +
( F_3 ( V_10 , V_24 ) << 8 ) +
( F_3 ( V_10 , V_30 ) << 16 ) ;
}
static void F_43 ( T_2 * V_10 )
{
unsigned long V_156 , V_157 , V_124 ;
T_5 V_158 = V_10 -> V_73 . V_74 ;
V_10 -> V_73 . V_74 = V_159 ;
if ( V_158 == V_160 || V_158 == V_159 )
return;
if ( V_158 == V_161 )
V_156 = V_10 -> V_13 . V_32 . V_47 ;
else
V_156 = V_10 -> V_13 . V_32 . V_36 ;
V_157 = F_42 ( V_10 ) ;
V_124 = F_3 ( V_10 , V_26 ) & V_162 ;
F_21 ( V_10 , V_5 , L_55
L_56 ,
V_156 , V_157 , V_124 ) ;
if ( V_10 -> V_13 . V_47 == V_89 )
V_157 += V_124 ;
F_35 ( V_10 , V_156 - V_157 ) ;
}
static void F_44 ( T_2 * V_10 )
{
T_4 V_149 ;
T_5 V_158 ;
F_21 ( V_10 , V_5 ,
L_57 ,
V_10 -> V_13 . V_32 . V_35 , V_10 -> V_13 . V_32 . V_36 ,
V_10 -> V_13 . V_32 . V_47 ) ;
if ( ! V_10 -> V_13 . V_32 . V_35 ) {
F_21 ( V_10 , V_7 , L_58
L_59 ) ;
F_9 ( & V_10 -> V_13 . V_32 , L_60 , L_48 ) ;
F_9 ( & V_10 -> V_43 -> V_32 , L_61 , L_48 ) ;
return;
}
if ( V_10 -> V_68 [ V_10 -> V_43 -> V_68 -> V_97 ] . V_71 )
V_158 = V_161 ;
else
V_158 = V_160 ;
if ( V_10 -> V_13 . V_47 == V_89 )
V_149 = V_150 ;
else
V_149 = V_163 ;
if ( V_10 -> V_73 . V_75 )
V_158 = V_10 -> V_73 . V_75 ( V_10 -> V_42 , & V_10 -> V_13 . V_32 ,
V_149 , V_158 ) ;
V_10 -> V_73 . V_74 = V_158 ;
if ( V_158 == V_161 )
F_41 ( V_10 , V_10 -> V_13 . V_32 . V_47 ) ;
else
F_41 ( V_10 , V_10 -> V_13 . V_32 . V_36 ) ;
switch ( V_158 ) {
case V_160 :
F_21 ( V_10 , V_5 , L_62 ) ;
F_5 ( V_10 , V_129 , 0 ) ;
F_5 ( V_10 , V_130 , V_10 -> V_13 . V_48 ) ;
F_23 ( V_10 , V_164 ) ;
F_38 ( V_10 , V_149 ) ;
break;
case V_165 :
F_21 ( V_10 , V_5 , L_63 ) ;
F_23 ( V_10 , V_164 | V_166 ) ;
V_10 -> V_73 . V_76 ( V_10 -> V_42 , & V_10 -> V_13 . V_32 ,
V_149 , V_10 -> V_43 -> V_167 ) ;
break;
case V_168 :
F_21 ( V_10 , V_5 , L_64 ) ;
F_23 ( V_10 , V_164 | V_166 ) ;
break;
case V_161 :
F_21 ( V_10 , V_5 , L_65 ) ;
F_23 ( V_10 , V_164 | V_166 ) ;
break;
default:
F_21 ( V_10 , V_5 | V_7 ,
L_66 ) ;
break;
}
}
static void F_45 ( T_2 * V_10 )
{
F_29 ( V_10 ) ;
if ( V_10 -> V_73 . V_74 == V_161 ||
V_10 -> V_73 . V_74 == V_168 )
V_10 -> V_73 . V_77 ( V_10 -> V_42 , & V_10 -> V_13 . V_32 ) ;
F_43 ( V_10 ) ;
if ( V_10 -> V_13 . V_47 == V_90 ) {
unsigned int V_124 ;
V_124 = F_3 ( V_10 , V_26 ) & V_162 ;
while ( V_124 && V_10 -> V_13 . V_32 . V_35 ) {
* V_10 -> V_13 . V_32 . V_35 = F_3 ( V_10 , V_151 ) ;
F_35 ( V_10 , 1 ) ;
V_124 -- ;
}
} else {
F_23 ( V_10 , V_169 ) ;
}
}
static void F_46 ( T_2 * V_10 )
{
F_29 ( V_10 ) ;
if ( V_10 -> V_73 . V_74 == V_161 ||
V_10 -> V_73 . V_74 == V_168 )
V_10 -> V_73 . V_77 ( V_10 -> V_42 , & V_10 -> V_13 . V_32 ) ;
V_10 -> V_73 . V_74 = V_159 ;
F_23 ( V_10 , V_169 ) ;
}
static void F_47 ( T_2 * V_10 )
{
unsigned long V_170 ;
F_29 ( V_10 ) ;
F_21 ( V_10 , V_6 , L_67 ,
V_10 -> V_13 . V_47 ) ;
F_33 ( & V_10 -> V_13 . V_123 ) ;
switch ( V_10 -> V_13 . V_47 ) {
case V_87 :
case V_171 :
F_48 ( V_10 , V_172 ) ;
break;
case V_92 :
V_10 -> V_13 . V_49 = 1 ;
V_10 -> V_13 . V_47 = V_86 ;
V_10 -> V_51 . V_58 += 1 ;
F_49 ( & V_10 -> V_173 , V_170 ) ;
if ( V_10 -> V_13 . V_47 == V_86 )
F_50 ( V_10 ) ;
F_51 ( & V_10 -> V_173 , V_170 ) ;
break;
case V_96 :
F_48 ( V_10 , V_174 ) ;
break;
case V_94 :
if ( F_26 ( V_10 , V_10 -> V_13 . V_175 ) == ABORT ) {
V_10 -> V_13 . V_50 = 0 ;
F_48 ( V_10 , V_176 ) ;
break;
}
default:
F_8 ( V_116 L_68 ,
V_10 -> V_42 -> V_102 , F_16 ( V_10 ) , F_14 ( V_10 ) ) ;
F_25 () ;
F_45 ( V_10 ) ;
F_48 ( V_10 , V_177 ) ;
break;
}
}
static void
F_52 ( T_2 * V_10 )
{
unsigned int V_178 , V_40 ;
unsigned char V_121 [ 4 ] ;
unsigned char V_98 , V_179 , V_180 ;
F_29 ( V_10 ) ;
F_53 ( V_10 -> V_13 . V_47 == V_87 ||
V_10 -> V_13 . V_47 == V_171 ) ;
V_178 = F_3 ( V_10 , V_26 ) ;
F_21 ( V_10 , V_6 , L_69 ,
V_10 -> V_13 . V_47 , V_178 ) ;
V_178 &= V_162 ;
if ( V_178 < 2 || V_178 > 4 ) {
F_8 ( V_116 L_70 ,
V_10 -> V_42 -> V_102 ) ;
goto V_181;
}
for ( V_40 = 0 ; V_40 < V_178 ; V_40 ++ )
V_121 [ V_40 ] = F_3 ( V_10 , V_151 ) ;
if ( ! ( V_121 [ 0 ] & ( 1 << V_10 -> V_42 -> V_182 ) ) ||
! ( V_121 [ 1 ] & 0x80 ) )
goto V_183;
V_98 = V_121 [ 0 ] & ~ ( 1 << V_10 -> V_42 -> V_182 ) ;
V_98 = F_54 ( V_98 ) - 1 ;
V_179 = V_121 [ 1 ] & 7 ;
V_180 = 0 ;
if ( V_178 >= 3 ) {
if ( V_121 [ 2 ] != V_184 )
goto V_183;
V_180 = V_121 [ 3 ] ;
}
F_5 ( V_10 , V_185 , V_98 ) ;
F_30 ( V_10 , V_98 ) ;
F_33 ( & V_10 -> V_13 . V_123 ) ;
F_21 ( V_10 , V_6 , L_71 ,
V_98 , V_179 , V_180 ) ;
if ( V_10 -> V_13 . V_49 && V_10 -> V_43 ) {
V_10 -> V_13 . V_49 = 0 ;
if ( V_10 -> V_43 -> V_68 -> V_97 == V_98 &&
V_10 -> V_43 -> V_68 -> V_179 == V_179 &&
V_10 -> V_43 -> V_180 == V_180 ) {
F_21 ( V_10 , V_6 , L_72 ) ;
} else {
F_55 ( & V_10 -> V_52 . V_186 , V_10 -> V_43 ) ;
F_21 ( V_10 , V_6 , L_73 ) ;
V_10 -> V_43 = NULL ;
}
}
if ( ! V_10 -> V_43 ) {
V_10 -> V_43 = F_56 ( & V_10 -> V_52 . V_186 ,
V_98 , V_179 , V_180 ) ;
F_21 ( V_10 , V_6 , L_74 ) ;
}
if ( V_10 -> V_43 ) {
V_10 -> V_13 . V_32 = V_10 -> V_43 -> V_32 ;
F_21 ( V_10 , V_6 , L_75 ,
V_10 -> V_13 . V_32 . V_35 , V_10 -> V_13 . V_32 . V_36 ) ;
V_10 -> V_13 . V_47 = V_91 ;
} else {
F_23 ( V_10 , V_143 ) ;
#if 0
if (tag)
msgqueue_addmsg(&info->scsi.msgs, 2, ABORT_TAG, tag);
else
#endif
F_34 ( & V_10 -> V_13 . V_123 , 1 , ABORT ) ;
V_10 -> V_13 . V_47 = V_93 ;
V_10 -> V_13 . V_50 = 1 ;
}
F_23 ( V_10 , V_187 ) ;
return;
V_183:
F_8 ( V_116 L_76 ,
V_10 -> V_42 -> V_102 ) ;
for ( V_40 = 0 ; V_40 < V_178 ; V_40 ++ )
F_8 ( L_77 , V_121 [ V_40 ] ) ;
F_8 ( L_48 ) ;
V_181:
F_23 ( V_10 , V_143 ) ;
F_33 ( & V_10 -> V_13 . V_123 ) ;
F_34 ( & V_10 -> V_13 . V_123 , 1 , V_188 ) ;
V_10 -> V_13 . V_47 = V_93 ;
F_23 ( V_10 , V_187 ) ;
}
static void F_57 ( T_2 * V_10 , unsigned char * V_120 , int V_189 )
{
int V_40 ;
switch ( V_120 [ 0 ] ) {
case V_190 :
if ( V_189 != 1 )
goto V_191;
F_8 ( V_116 L_78
L_79 ,
V_10 -> V_42 -> V_102 , F_16 ( V_10 ) ) ;
break;
case V_192 :
if ( V_189 != 1 )
goto V_191;
V_10 -> V_43 -> V_32 = V_10 -> V_13 . V_32 ;
V_10 -> V_43 -> V_32 . V_193 = 0 ;
F_21 ( V_10 , V_6 | V_8 | V_5 ,
L_80 ,
V_10 -> V_13 . V_32 . V_35 , V_10 -> V_13 . V_32 . V_36 ) ;
break;
case V_194 :
if ( V_189 != 1 )
goto V_191;
V_10 -> V_13 . V_32 = V_10 -> V_43 -> V_32 ;
F_21 ( V_10 , V_6 | V_8 | V_5 ,
L_81 ,
V_10 -> V_13 . V_32 . V_35 , V_10 -> V_13 . V_32 . V_36 ) ;
break;
case V_195 :
if ( V_189 != 1 )
goto V_191;
V_10 -> V_13 . V_47 = V_92 ;
break;
case V_140 :
if ( V_189 != 1 )
goto V_191;
switch ( F_26 ( V_10 , V_10 -> V_13 . V_175 ) ) {
case V_125 | V_144 << 8 :
F_31 ( V_10 , V_120 ) ;
break;
default:
F_21 ( V_10 , 0 , L_82 ,
F_26 ( V_10 , V_10 -> V_13 . V_175 ) ) ;
}
break;
case V_119 :
break;
case V_125 :
if ( V_189 < 3 )
goto V_191;
switch ( V_120 [ 2 ] ) {
case V_144 :
F_31 ( V_10 , V_120 ) ;
break;
default:
goto V_191;
}
break;
default:
goto V_191;
}
return;
V_191:
F_21 ( V_10 , 0 , L_83 ) ;
F_8 ( L_84 , V_10 -> V_42 -> V_102 , F_16 ( V_10 ) ) ;
for ( V_40 = 0 ; V_40 < V_189 ; V_40 ++ )
F_8 ( L_85 , V_40 & 31 ? L_86 : L_87 , V_120 [ V_40 ] ) ;
F_8 ( L_48 ) ;
F_23 ( V_10 , V_196 ) ;
F_7 ( V_10 ) ;
F_23 ( V_10 , V_143 ) ;
F_33 ( & V_10 -> V_13 . V_123 ) ;
F_34 ( & V_10 -> V_13 . V_123 , 1 , V_140 ) ;
V_10 -> V_13 . V_47 = V_93 ;
F_7 ( V_10 ) ;
}
static int F_58 ( T_2 * V_10 , int V_197 )
{
int V_198 ;
int V_18 ;
F_23 ( V_10 , V_197 ) ;
for ( V_198 = 1000 ; V_198 ; V_198 -= 1 ) {
V_18 = F_3 ( V_10 , V_21 ) ;
if ( V_18 & ( V_199 | V_200 ) )
break;
F_59 ( 1 ) ;
}
return V_18 ;
}
static int F_60 ( T_2 * V_10 )
{
unsigned int V_18 = F_58 ( V_10 , V_187 ) ;
if ( ( V_18 & V_199 ) == 0 )
goto V_201;
if ( ( V_18 & V_85 ) != V_202 )
goto V_203;
F_3 ( V_10 , V_22 ) ;
V_18 = F_58 ( V_10 , V_164 ) ;
if ( ( V_18 & V_199 ) == 0 )
goto V_201;
if ( V_18 & V_200 )
goto V_204;
if ( ( V_18 & V_85 ) != V_202 )
goto V_203;
F_3 ( V_10 , V_22 ) ;
return F_3 ( V_10 , V_151 ) ;
V_201:
F_21 ( V_10 , V_7 , L_88 ) ;
return - 1 ;
V_203:
F_21 ( V_10 , V_7 , L_89 , V_18 ) ;
return - 2 ;
V_204:
F_21 ( V_10 , V_7 , L_90 ) ;
return - 3 ;
}
static void F_61 ( T_2 * V_10 )
{
unsigned char * V_120 = V_10 -> V_13 . V_120 ;
unsigned int V_189 = 1 ;
int V_205 = 0 ;
F_29 ( V_10 ) ;
V_120 [ 0 ] = F_3 ( V_10 , V_151 ) ;
if ( V_120 [ 0 ] == V_125 ) {
V_205 = F_60 ( V_10 ) ;
if ( V_205 >= 0 ) {
V_120 [ 1 ] = V_205 ;
for ( V_189 = 2 ; V_189 < V_120 [ 1 ] + 2 ; V_189 ++ ) {
V_205 = F_60 ( V_10 ) ;
if ( V_205 >= 0 )
V_120 [ V_189 ] = V_205 ;
else
break;
}
}
}
if ( V_205 == - 3 )
goto V_204;
#ifdef F_62
{
int V_40 ;
F_8 ( L_91 ,
V_10 -> V_42 -> V_102 , F_16 ( V_10 ) ) ;
for ( V_40 = 0 ; V_40 < V_189 ; V_40 ++ )
F_8 ( L_92 , V_120 [ V_40 ] ) ;
F_8 ( L_48 ) ;
}
#endif
F_57 ( V_10 , V_120 , V_189 ) ;
F_23 ( V_10 , V_187 ) ;
return;
V_204:
F_23 ( V_10 , V_143 ) ;
F_33 ( & V_10 -> V_13 . V_123 ) ;
F_34 ( & V_10 -> V_13 . V_123 , 1 , V_206 ) ;
V_10 -> V_13 . V_47 = V_93 ;
F_23 ( V_10 , V_187 ) ;
return;
}
static void F_63 ( T_2 * V_10 )
{
int V_40 ;
F_29 ( V_10 ) ;
F_23 ( V_10 , V_196 | V_166 ) ;
F_23 ( V_10 , V_169 ) ;
for ( V_40 = V_10 -> V_13 . V_32 . V_193 ; V_40 < V_10 -> V_43 -> V_207 ; V_40 ++ )
F_5 ( V_10 , V_151 , V_10 -> V_43 -> V_105 [ V_40 ] ) ;
F_23 ( V_10 , V_164 ) ;
V_10 -> V_13 . V_47 = V_88 ;
}
static void F_64 ( T_2 * V_10 , int V_208 )
{
unsigned int V_209 = F_65 ( & V_10 -> V_13 . V_123 ) ;
F_29 ( V_10 ) ;
F_23 ( V_10 , V_169 ) ;
if ( V_209 ) {
struct V_120 * V_121 ;
int V_122 = 0 ;
while ( ( V_121 = F_27 ( & V_10 -> V_13 . V_123 , V_122 ++ ) ) != NULL ) {
int V_40 ;
for ( V_40 = V_208 ; V_40 < V_121 -> V_152 ; V_40 ++ )
F_5 ( V_10 , V_151 , V_121 -> V_121 [ V_40 ] ) ;
V_121 -> V_124 = V_209 - ( F_3 ( V_10 , V_26 ) & V_162 ) ;
V_208 = 0 ;
}
} else
F_5 ( V_10 , V_151 , V_119 ) ;
F_23 ( V_10 , V_164 ) ;
V_10 -> V_13 . V_47 = V_94 ;
}
static void F_66 ( T_2 * V_10 , unsigned int V_18 , unsigned int V_17 )
{
F_29 ( V_10 ) ;
F_21 ( V_10 , V_4 ,
L_93 ,
V_18 , V_17 , V_10 -> V_13 . V_47 ) ;
switch ( V_10 -> V_13 . V_47 ) {
case V_87 :
if ( ( V_17 & V_210 ) != V_211 )
goto V_212;
break;
case V_171 :
switch ( V_17 & V_210 ) {
case V_213 :
case V_211 :
goto V_212;
case V_214 :
case V_215 :
if ( ( V_18 & V_85 ) == V_202 )
break;
goto V_212;
case V_216 :
break;
}
default:
break;
}
F_23 ( V_10 , V_196 ) ;
#define F_67 ( T_6 , V_108 ) ((ph) << 3 | (st))
switch ( F_67 ( V_18 & V_85 , V_10 -> V_13 . V_47 ) ) {
case F_67 ( V_217 , V_171 ) :
case F_67 ( V_217 , V_94 ) :
case F_67 ( V_217 , V_88 ) :
case F_67 ( V_217 , V_91 ) :
V_10 -> V_13 . V_47 = V_90 ;
F_44 ( V_10 ) ;
return;
case F_67 ( V_217 , V_90 ) :
case F_67 ( V_218 , V_89 ) :
F_43 ( V_10 ) ;
F_44 ( V_10 ) ;
return;
case F_67 ( V_218 , V_171 ) :
case F_67 ( V_218 , V_94 ) :
case F_67 ( V_218 , V_88 ) :
case F_67 ( V_218 , V_91 ) :
F_23 ( V_10 , V_169 ) ;
V_10 -> V_13 . V_47 = V_89 ;
F_44 ( V_10 ) ;
return;
case F_67 ( V_219 , V_89 ) :
case F_67 ( V_219 , V_90 ) :
F_45 ( V_10 ) ;
case F_67 ( V_219 , V_171 ) :
case F_67 ( V_219 , V_94 ) :
case F_67 ( V_219 , V_88 ) :
case F_67 ( V_219 , V_91 ) :
F_23 ( V_10 , V_220 ) ;
V_10 -> V_13 . V_47 = V_95 ;
return;
case F_67 ( V_202 , V_89 ) :
case F_67 ( V_202 , V_90 ) :
F_45 ( V_10 ) ;
case F_67 ( V_202 , V_88 ) :
case F_67 ( V_202 , V_171 ) :
case F_67 ( V_202 , V_94 ) :
V_10 -> V_13 . V_175 = F_3 ( V_10 , V_26 ) & V_162 ;
F_23 ( V_10 , V_169 ) ;
F_23 ( V_10 , V_164 ) ;
V_10 -> V_13 . V_47 = V_91 ;
return;
case F_67 ( V_202 , V_91 ) :
V_10 -> V_13 . V_175 = F_3 ( V_10 , V_26 ) & V_162 ;
F_23 ( V_10 , V_164 ) ;
return;
case F_67 ( V_221 , V_94 ) :
case F_67 ( V_221 , V_91 ) :
F_63 ( V_10 ) ;
V_10 -> V_13 . V_47 = V_88 ;
return;
case F_67 ( V_222 , V_87 ) :
F_64 ( V_10 , 1 ) ;
return;
case F_67 ( V_222 , V_171 ) :
case F_67 ( V_222 , V_94 ) :
if ( V_10 -> V_68 [ V_10 -> V_43 -> V_68 -> V_97 ] . V_223 ) {
V_10 -> V_68 [ V_10 -> V_43 -> V_68 -> V_97 ] . V_223 = 0 ;
V_10 -> V_68 [ V_10 -> V_43 -> V_68 -> V_97 ] . V_224 = 1 ;
F_5 ( V_10 , V_27 , V_10 -> V_13 . V_46 [ 0 ] ) ;
}
if ( F_65 ( & V_10 -> V_13 . V_123 ) > 1 )
F_23 ( V_10 , V_143 ) ;
case F_67 ( V_222 , V_93 ) :
F_64 ( V_10 , 0 ) ;
return;
case F_67 ( V_221 , V_88 ) :
F_8 ( V_116 L_94
L_95 ,
V_10 -> V_42 -> V_102 , F_16 ( V_10 ) ) ;
F_23 ( V_10 , V_143 ) ;
F_41 ( V_10 , 15 ) ;
F_23 ( V_10 , V_225 | V_166 ) ;
F_33 ( & V_10 -> V_13 . V_123 ) ;
F_34 ( & V_10 -> V_13 . V_123 , 1 , V_188 ) ;
V_10 -> V_13 . V_47 = V_93 ;
return;
}
if ( V_10 -> V_13 . V_47 == V_92 ) {
F_8 ( V_116 L_96 ,
V_10 -> V_42 -> V_102 , F_16 ( V_10 ) ,
F_13 ( V_18 ) ) ;
F_33 ( & V_10 -> V_13 . V_123 ) ;
F_23 ( V_10 , V_143 ) ;
F_34 ( & V_10 -> V_13 . V_123 , 1 , V_188 ) ;
V_10 -> V_13 . V_47 = V_93 ;
V_10 -> V_13 . V_50 = 1 ;
F_23 ( V_10 , V_164 ) ;
return;
}
F_8 ( V_116 L_97 ,
V_10 -> V_42 -> V_102 , F_16 ( V_10 ) ,
F_13 ( V_18 ) ,
F_14 ( V_10 ) ) ;
F_25 () ;
return;
V_212:
F_21 ( V_10 , 0 , L_98 , V_17 & V_210 ) ;
F_7 ( V_10 ) ;
F_25 () ;
F_48 ( V_10 , V_177 ) ;
}
static void F_68 ( T_2 * V_10 , unsigned int V_18 , unsigned int V_17 )
{
unsigned int V_226 = F_3 ( V_10 , V_26 ) & V_162 ;
F_29 ( V_10 ) ;
F_21 ( V_10 , V_9 ,
L_99 ,
V_18 , V_17 , V_10 -> V_13 . V_47 ) ;
switch ( V_10 -> V_13 . V_47 ) {
case V_95 :
if ( V_226 != 2 ) {
F_21 ( V_10 , 0 , L_100 , V_226 ) ;
}
V_10 -> V_13 . V_32 . V_227 = F_3 ( V_10 , V_151 ) ;
V_10 -> V_13 . V_32 . V_228 = F_3 ( V_10 , V_151 ) ;
V_10 -> V_13 . V_47 = V_96 ;
F_23 ( V_10 , V_187 ) ;
break;
case V_86 :
case V_87 :
case V_171 :
break;
case V_91 :
if ( ( V_18 & V_85 ) == V_202 ) {
V_10 -> V_13 . V_175 = V_226 ;
F_61 ( V_10 ) ;
break;
}
default:
F_21 ( V_10 , 0 , L_101
L_102 ,
F_16 ( V_10 ) , F_14 ( V_10 ) ) ;
}
}
static void F_69 ( T_2 * V_10 )
{
T_7 V_72 ;
int V_40 ;
F_33 ( & V_10 -> V_13 . V_123 ) ;
V_72 = V_142 ;
#ifdef F_32
if ( V_10 -> V_62 . V_229 & ( V_230 | V_231 ) )
V_72 = V_232 ;
#endif
V_10 -> V_13 . V_47 = V_86 ;
V_10 -> V_43 = NULL ;
memset ( & V_10 -> V_13 . V_32 , 0 , sizeof( V_10 -> V_13 . V_32 ) ) ;
for ( V_40 = 0 ; V_40 < 8 ; V_40 ++ ) {
V_10 -> V_68 [ V_40 ] . V_69 = V_10 -> V_62 . V_69 ;
V_10 -> V_68 [ V_40 ] . V_72 = V_72 ;
V_10 -> V_68 [ V_40 ] . V_131 = V_10 -> V_62 . V_65 / 4 ;
V_10 -> V_68 [ V_40 ] . V_70 = V_10 -> V_13 . V_48 ;
V_10 -> V_68 [ V_40 ] . V_71 = 0 ;
V_10 -> V_68 [ V_40 ] . V_233 = 0 ;
}
V_10 -> V_234 = 1 ;
F_70 ( & V_10 -> V_235 ) ;
}
T_8 F_71 ( T_2 * V_10 )
{
unsigned char V_19 , V_17 , V_18 ;
int V_236 = V_237 ;
F_29 ( V_10 ) ;
V_18 = F_3 ( V_10 , V_21 ) ;
V_17 = F_3 ( V_10 , V_20 ) ;
V_19 = F_3 ( V_10 , V_22 ) ;
F_22 ( V_18 , V_17 , V_19 , V_10 -> V_13 . V_47 ) ;
if ( V_18 & V_199 ) {
if ( V_19 & V_238 ) {
F_21 ( V_10 , 0 , L_103 ) ;
F_69 ( V_10 ) ;
F_72 ( V_10 -> V_42 , 0 ) ;
} else if ( V_19 & V_239 ) {
F_21 ( V_10 , V_7 , L_104 ) ;
F_7 ( V_10 ) ;
F_25 () ;
} else if ( V_19 & V_240 )
F_47 ( V_10 ) ;
else if ( V_19 & V_241 )
F_52 ( V_10 ) ;
else if ( V_19 & V_242 )
F_66 ( V_10 , V_18 , V_17 ) ;
else if ( V_19 & V_243 )
F_68 ( V_10 , V_18 , V_17 ) ;
else
F_21 ( V_10 , 0 , L_105
L_106 ,
F_14 ( V_10 ) , V_19 , V_17 , V_18 ) ;
V_236 = V_244 ;
}
return V_236 ;
}
static void F_73 ( T_2 * V_10 , struct V_104 * V_43 )
{
int V_209 ;
F_41 ( V_10 , 0 ) ;
F_23 ( V_10 , V_196 | V_166 ) ;
F_23 ( V_10 , V_169 ) ;
F_5 ( V_10 , V_185 , F_74 ( V_43 -> V_68 -> V_97 ) ) ;
F_5 ( V_10 , V_245 , V_10 -> V_62 . V_64 ) ;
F_30 ( V_10 , V_43 -> V_68 -> V_97 ) ;
V_209 = F_65 ( & V_10 -> V_13 . V_123 ) ;
#ifdef F_62
{
struct V_120 * V_121 ;
int V_122 = 0 , V_40 ;
F_8 ( L_107 ,
V_10 -> V_42 -> V_102 , '0' + V_43 -> V_68 -> V_97 ) ;
while ( ( V_121 = F_27 ( & V_10 -> V_13 . V_123 , V_122 ++ ) ) != NULL ) {
F_8 ( L_108 ) ;
for ( V_40 = 0 ; V_40 < V_121 -> V_152 ; V_40 ++ )
F_8 ( L_109 , V_121 -> V_121 [ V_40 ] ) ;
F_8 ( L_110 ) ;
}
F_8 ( L_48 ) ;
}
#endif
if ( V_209 == 1 || V_209 == 3 ) {
struct V_120 * V_121 ;
int V_122 = 0 , V_40 ;
V_10 -> V_13 . V_47 = V_171 ;
while ( ( V_121 = F_27 ( & V_10 -> V_13 . V_123 , V_122 ++ ) ) != NULL ) {
for ( V_40 = 0 ; V_40 < V_121 -> V_152 ; V_40 ++ )
F_5 ( V_10 , V_151 , V_121 -> V_121 [ V_40 ] ) ;
V_121 -> V_124 = V_209 - ( F_3 ( V_10 , V_26 ) & V_162 ) ;
}
for ( V_40 = 0 ; V_40 < V_43 -> V_207 ; V_40 ++ )
F_5 ( V_10 , V_151 , V_43 -> V_105 [ V_40 ] ) ;
if ( V_209 == 1 )
F_23 ( V_10 , V_246 ) ;
else
F_23 ( V_10 , V_247 ) ;
} else {
struct V_120 * V_121 = F_27 ( & V_10 -> V_13 . V_123 , 0 ) ;
F_5 ( V_10 , V_151 , V_121 -> V_121 [ 0 ] ) ;
V_121 -> V_124 = 1 ;
F_23 ( V_10 , V_248 ) ;
}
}
static int F_75 ( T_2 * V_10 , int V_98 )
{
#if 0
if (target == 3) {
info->device[target].parity_check = 0;
return 1;
}
#endif
return V_10 -> V_68 [ V_98 ] . V_223 ;
}
static void F_76 ( T_2 * V_10 , struct V_104 * V_43 )
{
int V_69 ;
V_10 -> V_13 . V_47 = V_87 ;
V_10 -> V_13 . V_32 = V_43 -> V_32 ;
V_10 -> V_43 = V_43 ;
V_10 -> V_73 . V_74 = V_159 ;
if ( F_75 ( V_10 , V_43 -> V_68 -> V_97 ) )
F_5 ( V_10 , V_27 , V_10 -> V_13 . V_46 [ 0 ] | V_249 ) ;
else
F_5 ( V_10 , V_27 , V_10 -> V_13 . V_46 [ 0 ] ) ;
V_69 = V_43 -> V_105 [ 0 ] != V_250 &&
V_10 -> V_68 [ V_43 -> V_68 -> V_97 ] . V_69 ;
F_33 ( & V_10 -> V_13 . V_123 ) ;
F_34 ( & V_10 -> V_13 . V_123 , 1 , F_77 ( V_69 , V_43 -> V_68 -> V_179 ) ) ;
if ( V_43 -> V_180 )
F_34 ( & V_10 -> V_13 . V_123 , 2 , V_184 , V_43 -> V_180 ) ;
do {
#ifdef F_32
if ( ( V_10 -> V_68 [ V_43 -> V_68 -> V_97 ] . V_72 == V_232 ||
V_10 -> V_68 [ V_43 -> V_68 -> V_97 ] . V_72 == V_146 ) &&
( V_43 -> V_105 [ 0 ] == V_250 ||
V_43 -> V_105 [ 0 ] == V_251 ) ) {
V_10 -> V_68 [ V_43 -> V_68 -> V_97 ] . V_72 = V_141 ;
F_34 ( & V_10 -> V_13 . V_123 , 5 ,
V_125 , 3 , V_144 ,
1000 / V_10 -> V_62 . V_63 ,
V_10 -> V_62 . V_66 ) ;
break;
}
#endif
} while ( 0 );
F_73 ( V_10 , V_43 ) ;
}
static void F_78 ( T_2 * V_10 , struct V_104 * V_43 )
{
#ifdef F_79
if ( V_43 -> V_68 -> V_252 && V_43 -> V_105 [ 0 ] != V_250 &&
V_43 -> V_105 [ 0 ] != V_251 ) {
V_43 -> V_68 -> V_253 += 1 ;
if ( V_43 -> V_68 -> V_253 == 0 )
V_43 -> V_68 -> V_253 = 1 ;
V_43 -> V_180 = V_43 -> V_68 -> V_253 ;
} else
#endif
F_80 ( V_43 -> V_68 -> V_97 * 8 + V_43 -> V_68 -> V_179 , V_10 -> V_67 ) ;
V_10 -> V_51 . V_53 += 1 ;
switch ( V_43 -> V_105 [ 0 ] ) {
case V_254 :
case V_255 :
case V_256 :
V_10 -> V_51 . V_56 += 1 ;
break;
case V_257 :
case V_258 :
case V_259 :
V_10 -> V_51 . V_55 += 1 ;
break;
default:
V_10 -> V_51 . V_57 += 1 ;
break;
}
}
static void F_81 ( T_2 * V_10 ,
struct V_104 * V_43 )
{
struct V_120 * V_121 ;
V_10 -> V_13 . V_47 = V_87 ;
V_10 -> V_13 . V_32 = V_43 -> V_32 ;
V_10 -> V_43 = V_43 ;
V_10 -> V_73 . V_74 = V_159 ;
F_21 ( V_10 , V_7 , L_111 ) ;
F_33 ( & V_10 -> V_13 . V_123 ) ;
F_34 ( & V_10 -> V_13 . V_123 , 1 , V_260 ) ;
F_41 ( V_10 , 0 ) ;
F_23 ( V_10 , V_196 | V_166 ) ;
F_23 ( V_10 , V_169 ) ;
F_5 ( V_10 , V_185 , F_74 ( V_43 -> V_68 -> V_97 ) ) ;
F_5 ( V_10 , V_245 , V_10 -> V_62 . V_64 ) ;
F_30 ( V_10 , V_43 -> V_68 -> V_97 ) ;
V_121 = F_27 ( & V_10 -> V_13 . V_123 , 0 ) ;
F_5 ( V_10 , V_151 , V_260 ) ;
V_121 -> V_124 = 1 ;
F_23 ( V_10 , V_248 ) ;
}
static void F_50 ( T_2 * V_10 )
{
struct V_104 * V_43 = NULL ;
#define F_82 0
#define F_83 1
#define F_84 2
int V_261 = F_82 ;
F_29 ( V_10 ) ;
do {
if ( V_10 -> V_262 ) {
V_43 = V_10 -> V_262 ;
V_261 = F_83 ;
break;
}
if ( V_10 -> V_263 ) {
V_43 = V_10 -> V_263 ;
V_10 -> V_263 = NULL ;
break;
}
if ( V_10 -> V_44 ) {
V_43 = V_10 -> V_44 ;
V_10 -> V_44 = NULL ;
break;
}
if ( ! V_43 ) {
V_43 = F_85 ( & V_10 -> V_52 . V_264 ,
V_10 -> V_67 ) ;
V_261 = F_84 ;
break;
}
} while ( 0 );
if ( ! V_43 ) {
F_23 ( V_10 , V_265 ) ;
return;
}
F_23 ( V_10 , V_266 ) ;
if ( V_10 -> V_13 . V_49 && V_10 -> V_43 ) {
F_21 ( V_10 , V_6 ,
L_112 ,
V_10 -> V_43 -> V_68 -> V_97 ) ;
F_55 ( & V_10 -> V_52 . V_186 , V_10 -> V_43 ) ;
V_10 -> V_13 . V_49 = 0 ;
V_10 -> V_43 = NULL ;
}
F_18 ( V_10 , V_6 | V_8 , V_43 ,
L_113 ) ;
switch ( V_261 ) {
case F_84 :
F_78 ( V_10 , V_43 ) ;
case F_82 :
F_76 ( V_10 , V_43 ) ;
break;
case F_83 :
F_81 ( V_10 , V_43 ) ;
break;
}
F_21 ( V_10 , V_6 , L_114 ,
V_10 -> V_13 . V_32 . V_35 , V_10 -> V_13 . V_32 . V_36 ) ;
}
static void F_86 ( T_2 * V_10 , struct V_104 * V_43 ,
unsigned int V_267 )
{
F_21 ( V_10 , V_7 , L_115 ) ;
V_10 -> V_262 = NULL ;
V_10 -> V_268 = 1 ;
F_70 ( & V_10 -> V_235 ) ;
}
static void F_87 ( T_2 * V_10 , struct V_104 * V_43 ,
unsigned int V_267 )
{
F_20 ( V_10 , V_6 , V_43 -> V_68 -> V_97 ,
L_116 ,
V_267 , V_43 -> V_32 . V_228 , V_43 -> V_32 . V_227 ) ;
if ( V_267 != V_174 || V_43 -> V_32 . V_227 != V_269 )
memset ( V_43 -> V_270 , 0 , sizeof( V_43 -> V_270 ) ) ;
F_88 ( V_43 , & V_10 -> V_271 ) ;
V_43 -> V_272 ( V_43 ) ;
}
static void
F_89 ( T_2 * V_10 , struct V_104 * V_43 , unsigned int V_267 )
{
V_10 -> V_51 . V_54 += 1 ;
V_43 -> V_267 = V_267 << 16 | V_10 -> V_13 . V_32 . V_228 << 8 |
V_10 -> V_13 . V_32 . V_227 ;
F_18 ( V_10 , V_6 , V_43 ,
L_117 , V_43 -> V_267 ) ;
if ( F_90 ( V_43 -> V_267 ) != V_174 ||
F_91 ( V_43 -> V_267 ) != V_190 )
goto V_273;
if ( F_92 ( V_43 -> V_267 ) == V_274 ||
F_92 ( V_43 -> V_267 ) == V_275 )
goto V_276;
if ( F_92 ( V_43 -> V_267 ) != V_269 )
goto V_273;
if ( V_10 -> V_13 . V_32 . V_35 ) {
switch ( V_43 -> V_105 [ 0 ] ) {
case V_251 :
case V_277 :
case V_278 :
break;
default:
F_8 ( V_116 L_118
L_119 ,
V_10 -> V_42 -> V_102 , '0' + V_43 -> V_68 -> V_97 ,
V_43 -> V_267 , V_10 -> V_13 . V_32 . V_35 ,
V_10 -> V_13 . V_32 . V_36 ) ;
F_19 ( V_43 -> V_105 ) ;
V_43 -> V_267 &= ~ ( 255 << 16 ) ;
V_43 -> V_267 |= V_279 << 16 ;
goto V_276;
}
}
V_273:
if ( V_43 -> V_272 ) {
V_43 -> V_272 ( V_43 ) ;
return;
}
F_12 ( L_120 ,
V_10 -> V_42 -> V_102 ) ;
V_276:
if ( V_43 -> V_105 [ 0 ] == V_250 )
goto V_273;
F_93 ( V_43 , & V_10 -> V_271 , NULL , 0 , ~ 0 ) ;
F_20 ( V_10 , V_6 , V_43 -> V_68 -> V_97 ,
L_121 ) ;
F_94 ( V_43 ) ;
V_43 -> V_32 . V_228 = 0 ;
V_43 -> V_32 . V_227 = 0 ;
V_43 -> V_180 = 0 ;
V_43 -> V_280 = ( void * ) F_87 ;
if ( V_10 -> V_263 )
F_8 ( V_148 L_122 ,
V_10 -> V_42 -> V_102 , '0' + V_43 -> V_68 -> V_97 ) ;
V_10 -> V_263 = V_43 ;
}
static void F_48 ( T_2 * V_10 , unsigned int V_267 )
{
void (* F_95)( T_2 * , struct V_104 * , unsigned int );
struct V_104 * V_43 ;
unsigned long V_170 ;
F_29 ( V_10 ) ;
if ( ! V_10 -> V_43 )
goto V_281;
V_43 = V_10 -> V_43 ;
V_10 -> V_43 = NULL ;
V_10 -> V_13 . V_47 = V_86 ;
if ( V_10 -> V_13 . V_50 ) {
F_21 ( V_10 , 0 , L_123 ) ;
V_267 = V_176 ;
V_10 -> V_13 . V_50 = 0 ;
}
if ( V_10 -> V_13 . V_32 . V_35 && V_10 -> V_13 . V_32 . V_36 == 0 ) {
F_8 ( L_124
L_125 ,
V_10 -> V_42 -> V_102 , '0' + V_43 -> V_68 -> V_97 ,
V_10 -> V_13 . V_32 . V_35 , V_10 -> V_13 . V_32 . V_36 ) ;
V_10 -> V_13 . V_32 . V_35 = NULL ;
F_19 ( V_43 -> V_105 ) ;
}
V_10 -> V_68 [ V_43 -> V_68 -> V_97 ] . V_223 = 0 ;
F_96 ( V_43 -> V_68 -> V_97 * 8 + V_43 -> V_68 -> V_179 , V_10 -> V_67 ) ;
F_95 = ( void (*) ( T_2 * , struct V_104 * , unsigned int ) ) V_43 -> V_280 ;
F_95 ( V_10 , V_43 , V_267 ) ;
if ( V_10 -> V_13 . V_45 ) {
F_49 ( & V_10 -> V_173 , V_170 ) ;
if ( V_10 -> V_13 . V_47 == V_86 )
F_50 ( V_10 ) ;
F_51 ( & V_10 -> V_173 , V_170 ) ;
}
return;
V_281:
F_12 ( L_126 ,
V_10 -> V_42 -> V_102 ) ;
}
static int F_97 ( struct V_104 * V_43 ,
void (* V_273)( struct V_104 * ) )
{
T_2 * V_10 = ( T_2 * ) V_43 -> V_68 -> V_42 -> V_282 ;
int V_267 ;
F_29 ( V_10 ) ;
F_18 ( V_10 , V_6 , V_43 ,
L_127 , V_43 ) ;
V_43 -> V_272 = V_273 ;
V_43 -> V_280 = ( void * ) F_89 ;
V_43 -> V_267 = 0 ;
F_94 ( V_43 ) ;
V_10 -> V_51 . V_52 += 1 ;
V_43 -> V_180 = 0 ;
F_98 ( & V_10 -> V_173 ) ;
V_267 = ! F_99 ( & V_10 -> V_52 . V_264 , V_43 ) ;
if ( V_267 == 0 && V_10 -> V_13 . V_47 == V_86 )
F_50 ( V_10 ) ;
F_100 ( & V_10 -> V_173 ) ;
F_20 ( V_10 , V_6 , - 1 , L_128 ,
V_267 ? L_129 : L_130 ) ;
return V_267 ;
}
static void F_101 ( struct V_104 * V_43 )
{
T_2 * V_10 = ( T_2 * ) V_43 -> V_68 -> V_42 -> V_282 ;
F_29 ( V_10 ) ;
V_10 -> V_78 = 1 ;
}
static int F_102 ( struct V_104 * V_43 ,
void (* V_273)( struct V_104 * ) )
{
T_2 * V_10 = ( T_2 * ) V_43 -> V_68 -> V_42 -> V_282 ;
F_29 ( V_10 ) ;
F_36 ( V_10 -> V_13 . V_45 ) ;
V_10 -> V_78 = 0 ;
F_97 ( V_43 , F_101 ) ;
F_103 ( V_10 -> V_42 -> V_173 ) ;
while ( ! V_10 -> V_78 ) {
if ( F_3 ( V_10 , V_21 ) & V_199 ) {
F_104 ( V_10 -> V_42 -> V_173 ) ;
F_71 ( V_10 ) ;
F_103 ( V_10 -> V_42 -> V_173 ) ;
}
}
F_104 ( V_10 -> V_42 -> V_173 ) ;
V_273 ( V_43 ) ;
return 0 ;
}
static void F_105 ( unsigned long V_283 )
{
T_2 * V_10 = ( T_2 * ) V_283 ;
F_21 ( V_10 , V_7 , L_131 ) ;
F_106 ( & V_10 -> V_284 ) ;
if ( V_10 -> V_234 == 0 )
V_10 -> V_234 = - 1 ;
if ( V_10 -> V_268 == 0 )
V_10 -> V_268 = - 1 ;
F_70 ( & V_10 -> V_235 ) ;
}
static enum V_285 F_107 ( T_2 * V_10 ,
struct V_104 * V_43 )
{
enum V_285 V_139 = V_286 ;
if ( F_108 ( & V_10 -> V_52 . V_264 , V_43 ) ) {
F_8 ( L_132 ) ;
V_139 = V_287 ;
} else if ( F_108 ( & V_10 -> V_52 . V_186 , V_43 ) ) {
F_8 ( L_133 ) ;
V_139 = V_288 ;
} else if ( V_10 -> V_43 == V_43 ) {
F_8 ( L_134 ) ;
switch ( V_10 -> V_13 . V_47 ) {
case V_86 :
if ( V_10 -> V_13 . V_49 ) {
V_10 -> V_13 . V_49 = 0 ;
V_10 -> V_43 = NULL ;
V_139 = V_288 ;
}
break;
default:
break;
}
} else if ( V_10 -> V_44 == V_43 ) {
V_10 -> V_44 = NULL ;
F_96 ( V_43 -> V_68 -> V_97 * 8 + V_43 -> V_68 -> V_179 , V_10 -> V_67 ) ;
F_8 ( L_135 ) ;
V_139 = V_287 ;
} else
F_8 ( L_136 ) ;
return V_139 ;
}
int F_109 ( struct V_104 * V_43 )
{
T_2 * V_10 = ( T_2 * ) V_43 -> V_68 -> V_42 -> V_282 ;
int V_267 = V_289 ;
F_29 ( V_10 ) ;
V_10 -> V_51 . V_59 += 1 ;
F_8 ( V_148 L_137 , V_10 -> V_42 -> V_102 ) ;
F_19 ( V_43 -> V_105 ) ;
F_25 () ;
F_7 ( V_10 ) ;
F_8 ( V_148 L_138 , V_10 -> V_42 -> V_102 , V_43 ) ;
switch ( F_107 ( V_10 , V_43 ) ) {
case V_287 :
F_8 ( L_139 ) ;
V_267 = V_290 ;
break;
case V_288 :
default:
case V_286 :
F_8 ( L_140 ) ;
break;
}
return V_267 ;
}
int F_110 ( struct V_104 * V_43 )
{
T_2 * V_10 = ( T_2 * ) V_43 -> V_68 -> V_42 -> V_282 ;
unsigned long V_170 ;
int V_40 , V_139 = V_289 , V_98 = V_43 -> V_68 -> V_97 ;
F_21 ( V_10 , V_7 , L_141 , V_98 ) ;
F_49 ( & V_10 -> V_173 , V_170 ) ;
do {
if ( V_10 -> V_43 && ! V_10 -> V_13 . V_49 &&
V_10 -> V_43 -> V_68 -> V_97 == V_43 -> V_68 -> V_97 )
break;
F_111 ( & V_10 -> V_52 . V_264 , V_98 ) ;
F_111 ( & V_10 -> V_52 . V_186 , V_98 ) ;
if ( V_10 -> V_44 && V_10 -> V_44 -> V_68 -> V_97 == V_98 )
V_10 -> V_44 = NULL ;
if ( V_10 -> V_263 && V_10 -> V_263 -> V_68 -> V_97 == V_98 )
V_10 -> V_263 = NULL ;
for ( V_40 = 0 ; V_40 < 8 ; V_40 ++ )
F_96 ( V_98 * 8 + V_40 , V_10 -> V_67 ) ;
V_43 -> V_280 = ( void * ) F_86 ;
V_10 -> V_268 = 0 ;
V_10 -> V_262 = V_43 ;
if ( V_10 -> V_13 . V_47 == V_86 )
F_50 ( V_10 ) ;
F_112 ( & V_10 -> V_284 , V_291 + 30 * V_292 ) ;
F_51 ( & V_10 -> V_173 , V_170 ) ;
F_113 ( V_10 -> V_235 , V_10 -> V_268 ) ;
F_114 ( & V_10 -> V_284 ) ;
F_49 ( & V_10 -> V_173 , V_170 ) ;
V_10 -> V_262 = NULL ;
if ( V_10 -> V_268 == 1 )
V_139 = V_290 ;
} while ( 0 );
V_43 -> V_280 = NULL ;
F_51 ( & V_10 -> V_173 , V_170 ) ;
F_21 ( V_10 , V_7 , L_142 ,
V_139 == V_290 ? L_130 : L_143 ) ;
return V_139 ;
}
int F_115 ( struct V_104 * V_43 )
{
T_2 * V_10 = ( T_2 * ) V_43 -> V_68 -> V_42 -> V_282 ;
unsigned long V_170 ;
struct V_293 * V_294 ;
F_29 ( V_10 ) ;
F_21 ( V_10 , V_7 , L_144 ) ;
V_10 -> V_51 . V_60 += 1 ;
F_49 ( & V_10 -> V_173 , V_170 ) ;
F_46 ( V_10 ) ;
F_5 ( V_10 , V_29 , V_10 -> V_13 . V_46 [ 2 ] ) ;
while ( F_3 ( V_10 , V_21 ) & V_199 )
F_3 ( V_10 , V_22 ) ;
V_10 -> V_234 = 0 ;
F_116 (SDpnt, info->host) {
int V_40 ;
if ( V_294 -> V_295 )
continue;
F_111 ( & V_10 -> V_52 . V_264 , V_294 -> V_97 ) ;
F_111 ( & V_10 -> V_52 . V_186 , V_294 -> V_97 ) ;
if ( V_10 -> V_44 && V_10 -> V_44 -> V_68 -> V_97 == V_294 -> V_97 )
V_10 -> V_44 = NULL ;
if ( V_10 -> V_263 && V_10 -> V_263 -> V_68 -> V_97 == V_294 -> V_97 )
V_10 -> V_263 = NULL ;
V_10 -> V_43 = NULL ;
for ( V_40 = 0 ; V_40 < 8 ; V_40 ++ )
F_96 ( V_294 -> V_97 * 8 + V_40 , V_10 -> V_67 ) ;
}
V_10 -> V_13 . V_47 = V_86 ;
F_23 ( V_10 , V_296 ) ;
F_112 ( & V_10 -> V_284 , V_291 + V_292 ) ;
F_51 ( & V_10 -> V_173 , V_170 ) ;
F_113 ( V_10 -> V_235 , V_10 -> V_234 ) ;
F_114 ( & V_10 -> V_284 ) ;
F_21 ( V_10 , V_7 , L_145 ,
V_10 -> V_234 == 1 ? L_130 : L_143 ) ;
return V_10 -> V_234 == 1 ? V_290 : V_289 ;
}
static void F_117 ( T_2 * V_10 )
{
unsigned int clock = ( ( V_10 -> V_62 . V_63 - 1 ) / 5 + 1 ) & 7 ;
F_5 ( V_10 , V_297 , clock ) ;
F_5 ( V_10 , V_27 , V_10 -> V_13 . V_46 [ 0 ] ) ;
F_5 ( V_10 , V_28 , V_10 -> V_13 . V_46 [ 1 ] ) ;
F_5 ( V_10 , V_29 , V_10 -> V_13 . V_46 [ 2 ] ) ;
F_5 ( V_10 , V_245 , V_10 -> V_62 . V_64 ) ;
F_5 ( V_10 , V_129 , 0 ) ;
F_5 ( V_10 , V_130 , V_10 -> V_13 . V_48 ) ;
F_5 ( V_10 , V_27 , V_10 -> V_13 . V_46 [ 0 ] ) ;
}
int F_118 ( struct V_104 * V_43 )
{
T_2 * V_10 = ( T_2 * ) V_43 -> V_68 -> V_42 -> V_282 ;
F_104 ( V_10 -> V_42 -> V_173 ) ;
F_29 ( V_10 ) ;
F_8 ( L_146 ,
V_10 -> V_42 -> V_102 , '0' + V_43 -> V_68 -> V_97 , V_298 ) ;
F_23 ( V_10 , V_299 ) ;
F_103 ( V_10 -> V_42 -> V_173 ) ;
F_119 ( 50 * 1000 / 100 ) ;
F_104 ( V_10 -> V_42 -> V_173 ) ;
F_23 ( V_10 , V_196 ) ;
F_117 ( V_10 ) ;
F_103 ( V_10 -> V_42 -> V_173 ) ;
return V_290 ;
}
static int F_120 ( T_2 * V_10 )
{
int V_300 , V_301 ;
F_5 ( V_10 , V_25 , V_299 ) ;
F_59 ( 50 ) ;
F_5 ( V_10 , V_25 , V_196 ) ;
F_5 ( V_10 , V_29 , 0 ) ;
F_5 ( V_10 , V_28 , V_302 ) ;
if ( ( F_3 ( V_10 , V_28 ) & ( ~ 0xe0 ) ) != V_302 )
return V_303 ;
F_5 ( V_10 , V_28 , 0 ) ;
F_5 ( V_10 , V_29 , 0 ) ;
F_5 ( V_10 , V_29 , 5 ) ;
if ( F_3 ( V_10 , V_29 ) != 5 )
return V_304 ;
F_5 ( V_10 , V_29 , 0 ) ;
F_5 ( V_10 , V_29 , V_305 ) ;
F_5 ( V_10 , V_29 , 0 ) ;
F_5 ( V_10 , V_25 , V_299 ) ;
F_59 ( 50 ) ;
F_5 ( V_10 , V_25 , V_166 | V_196 ) ;
F_5 ( V_10 , V_28 , V_306 ) ;
F_5 ( V_10 , V_25 , V_299 ) ;
F_59 ( 50 ) ;
F_5 ( V_10 , V_25 , V_196 ) ;
V_301 = F_3 ( V_10 , V_307 ) ;
V_300 = V_301 >> 3 ;
V_301 &= 7 ;
switch ( V_300 ) {
case 0x01 :
if ( V_301 == 4 )
return V_308 ;
break;
case 0x02 :
switch ( V_301 ) {
case 2 :
return V_309 ;
case 3 :
return V_310 ;
}
break;
default:
break;
}
F_8 ( L_147 , V_300 , V_301 ) ;
return V_311 ;
}
static void F_121 ( T_2 * V_10 )
{
int V_40 ;
F_29 ( V_10 ) ;
F_69 ( V_10 ) ;
memset ( V_10 -> V_67 , 0 , sizeof( V_10 -> V_67 ) ) ;
V_10 -> V_13 . V_49 = 0 ;
V_10 -> V_13 . V_50 = 0 ;
for ( V_40 = 0 ; V_40 < 8 ; V_40 ++ ) {
V_10 -> V_68 [ V_40 ] . V_224 = 0 ;
V_10 -> V_68 [ V_40 ] . V_223 = 1 ;
}
while ( F_122 ( & V_10 -> V_52 . V_186 ) != NULL ) ;
V_10 -> V_43 = NULL ;
V_10 -> V_263 = NULL ;
V_10 -> V_262 = NULL ;
V_10 -> V_44 = NULL ;
}
int F_123 ( struct V_312 * V_42 )
{
T_2 * V_10 = ( T_2 * ) V_42 -> V_282 ;
V_10 -> V_41 = V_82 ;
V_10 -> V_79 = V_82 ;
V_10 -> V_42 = V_42 ;
V_10 -> V_13 . V_46 [ 0 ] = V_42 -> V_182 | V_313 ;
V_10 -> V_13 . V_46 [ 1 ] = V_306 | V_302 ;
V_10 -> V_13 . V_46 [ 2 ] = V_10 -> V_62 . V_128 |
V_305 | V_314 | V_315 | V_316 ;
V_10 -> V_13 . V_48 = F_28 ( V_10 , V_10 -> V_62 . V_65 ) ;
V_10 -> V_268 = - 1 ;
V_10 -> V_234 = - 1 ;
F_124 ( & V_10 -> V_235 ) ;
F_125 ( & V_10 -> V_284 ) ;
V_10 -> V_284 . V_283 = ( unsigned long ) V_10 ;
V_10 -> V_284 . V_317 = F_105 ;
F_126 ( & V_10 -> V_173 ) ;
memset ( & V_10 -> V_51 , 0 , sizeof( V_10 -> V_51 ) ) ;
F_127 ( & V_10 -> V_13 . V_123 ) ;
if ( ! F_128 ( & V_10 -> V_52 . V_264 ) )
return - V_318 ;
if ( ! F_128 ( & V_10 -> V_52 . V_186 ) ) {
F_129 ( & V_10 -> V_52 . V_264 ) ;
return - V_318 ;
}
return 0 ;
}
int F_130 ( struct V_312 * V_42 , struct V_68 * V_134 )
{
T_2 * V_10 = ( T_2 * ) V_42 -> V_282 ;
int type , V_319 ;
if ( V_10 -> V_62 . V_63 <= 10 || V_10 -> V_62 . V_63 > 40 ) {
F_8 ( V_83 L_148 ,
V_10 -> V_62 . V_63 ) ;
return - V_320 ;
}
F_121 ( V_10 ) ;
type = F_120 ( V_10 ) ;
V_10 -> V_13 . type = V_321 [ type ] ;
F_59 ( 300 ) ;
F_117 ( V_10 ) ;
F_5 ( V_10 , V_27 , V_10 -> V_13 . V_46 [ 0 ] | V_322 ) ;
F_5 ( V_10 , V_25 , V_296 ) ;
F_103 ( V_10 -> V_42 -> V_173 ) ;
F_119 ( 100 * 1000 / 100 ) ;
F_104 ( V_10 -> V_42 -> V_173 ) ;
F_5 ( V_10 , V_27 , V_10 -> V_13 . V_46 [ 0 ] ) ;
F_3 ( V_10 , V_22 ) ;
F_29 ( V_10 ) ;
V_319 = F_131 ( V_42 , V_134 ) ;
if ( V_319 )
F_5 ( V_10 , V_25 , V_299 ) ;
else
F_132 ( V_42 ) ;
return V_319 ;
}
void F_133 ( struct V_312 * V_42 )
{
T_2 * V_10 = ( T_2 * ) V_42 -> V_282 ;
F_29 ( V_10 ) ;
F_134 ( V_42 ) ;
F_5 ( V_10 , V_25 , V_299 ) ;
F_135 ( V_42 ) ;
}
void F_136 ( struct V_312 * V_42 )
{
T_2 * V_10 = ( T_2 * ) V_42 -> V_282 ;
F_129 ( & V_10 -> V_52 . V_186 ) ;
F_129 ( & V_10 -> V_52 . V_264 ) ;
}
int F_137 ( T_2 * V_10 , char * V_37 )
{
return sprintf ( V_37 ,
L_48
L_149
L_150
L_151
L_152 ,
V_10 -> V_13 . type , V_10 -> V_13 . V_15 ,
V_10 -> V_13 . V_45 , V_10 -> V_13 . V_73 ) ;
}
int F_138 ( T_2 * V_10 , char * V_37 )
{
char * V_323 = V_37 ;
V_323 += sprintf ( V_323 , L_48
L_153
L_154
L_155
L_156
L_157
L_158
L_159
L_160
L_161
L_162
L_163 ,
V_10 -> V_51 . V_52 , V_10 -> V_51 . V_53 ,
V_10 -> V_51 . V_54 , V_10 -> V_51 . V_55 ,
V_10 -> V_51 . V_56 , V_10 -> V_51 . V_57 ,
V_10 -> V_51 . V_58 , V_10 -> V_51 . V_59 ,
V_10 -> V_51 . V_60 , V_10 -> V_51 . V_61 ) ;
return V_323 - V_37 ;
}
int F_139 ( T_2 * V_10 , char * V_37 )
{
struct V_133 * V_134 ;
struct V_293 * V_324 ;
char * V_323 = V_37 ;
V_323 += sprintf ( V_323 , L_164 ) ;
F_116 (scd, info->host) {
V_134 = & V_10 -> V_68 [ V_324 -> V_97 ] ;
V_323 += sprintf ( V_323 , L_165 , V_324 -> V_97 , V_324 -> V_179 ) ;
if ( V_324 -> V_325 )
V_323 += sprintf ( V_323 , L_166 ,
V_324 -> V_252 ? L_167 : L_168 ,
V_324 -> V_253 ) ;
else
V_323 += sprintf ( V_323 , L_169 ) ;
V_323 += sprintf ( V_323 , L_170 , V_134 -> V_224 ? L_167 : L_168 ) ;
if ( V_134 -> V_71 )
V_323 += sprintf ( V_323 , L_171 ,
V_134 -> V_71 , V_134 -> V_131 * 4 ) ;
else
V_323 += sprintf ( V_323 , L_172 ) ;
}
return V_323 - V_37 ;
}
