void
F_1 (
T_1 * V_1 )
{
ASSERT ( F_2 ( & V_1 -> V_2 ) ) ;
F_3 ( & V_1 -> V_3 ) ;
F_4 ( V_4 , V_1 ) ;
F_5 ( V_5 ) ;
}
void
F_6 (
T_2 * V_6 ,
T_3 * V_7 )
{
T_4 * V_8 = V_6 -> V_9 ;
ASSERT ( V_7 -> V_10 ) ;
if ( V_8 -> V_11 && ! V_7 -> V_12 )
V_7 -> V_12 = F_7 ( V_8 -> V_11 ) ;
if ( V_8 -> V_13 && ! V_7 -> V_14 )
V_7 -> V_14 = F_7 ( V_8 -> V_13 ) ;
if ( V_8 -> V_15 && ! V_7 -> V_16 )
V_7 -> V_16 = F_7 ( V_8 -> V_15 ) ;
if ( V_8 -> V_17 && ! V_7 -> V_18 )
V_7 -> V_18 = F_7 ( V_8 -> V_17 ) ;
if ( V_8 -> V_19 && ! V_7 -> V_20 )
V_7 -> V_20 = F_7 ( V_8 -> V_19 ) ;
if ( V_8 -> V_21 && ! V_7 -> V_22 )
V_7 -> V_22 = F_7 ( V_8 -> V_21 ) ;
}
void
F_8 (
T_2 * V_6 ,
T_3 * V_7 )
{
ASSERT ( V_7 -> V_10 ) ;
#ifdef F_9
if ( V_7 -> V_14 )
ASSERT ( F_10 ( V_7 -> V_12 ) <=
F_10 ( V_7 -> V_14 ) ) ;
if ( V_7 -> V_18 )
ASSERT ( F_10 ( V_7 -> V_16 ) <=
F_10 ( V_7 -> V_18 ) ) ;
if ( V_7 -> V_22 )
ASSERT ( F_10 ( V_7 -> V_20 ) <=
F_10 ( V_7 -> V_22 ) ) ;
#endif
if ( ! V_7 -> V_23 ) {
if ( ( V_7 -> V_12 &&
( F_10 ( V_7 -> V_24 ) >
F_10 ( V_7 -> V_12 ) ) ) ||
( V_7 -> V_14 &&
( F_10 ( V_7 -> V_24 ) >
F_10 ( V_7 -> V_14 ) ) ) ) {
V_7 -> V_23 = F_11 ( F_12 () +
V_6 -> V_9 -> V_25 ) ;
} else {
V_7 -> V_26 = 0 ;
}
} else {
if ( ( ! V_7 -> V_12 ||
( F_10 ( V_7 -> V_24 ) <=
F_10 ( V_7 -> V_12 ) ) ) &&
( ! V_7 -> V_14 ||
( F_10 ( V_7 -> V_24 ) <=
F_10 ( V_7 -> V_14 ) ) ) ) {
V_7 -> V_23 = 0 ;
}
}
if ( ! V_7 -> V_27 ) {
if ( ( V_7 -> V_16 &&
( F_10 ( V_7 -> V_28 ) >
F_10 ( V_7 -> V_16 ) ) ) ||
( V_7 -> V_18 &&
( F_10 ( V_7 -> V_28 ) >
F_10 ( V_7 -> V_18 ) ) ) ) {
V_7 -> V_27 = F_11 ( F_12 () +
V_6 -> V_9 -> V_29 ) ;
} else {
V_7 -> V_30 = 0 ;
}
} else {
if ( ( ! V_7 -> V_16 ||
( F_10 ( V_7 -> V_28 ) <=
F_10 ( V_7 -> V_16 ) ) ) &&
( ! V_7 -> V_18 ||
( F_10 ( V_7 -> V_28 ) <=
F_10 ( V_7 -> V_18 ) ) ) ) {
V_7 -> V_27 = 0 ;
}
}
if ( ! V_7 -> V_31 ) {
if ( ( V_7 -> V_20 &&
( F_10 ( V_7 -> V_32 ) >
F_10 ( V_7 -> V_20 ) ) ) ||
( V_7 -> V_22 &&
( F_10 ( V_7 -> V_32 ) >
F_10 ( V_7 -> V_22 ) ) ) ) {
V_7 -> V_31 = F_11 ( F_12 () +
V_6 -> V_9 -> V_33 ) ;
} else {
V_7 -> V_34 = 0 ;
}
} else {
if ( ( ! V_7 -> V_20 ||
( F_10 ( V_7 -> V_32 ) <=
F_10 ( V_7 -> V_20 ) ) ) &&
( ! V_7 -> V_22 ||
( F_10 ( V_7 -> V_32 ) <=
F_10 ( V_7 -> V_22 ) ) ) ) {
V_7 -> V_31 = 0 ;
}
}
}
STATIC void
F_13 (
T_5 * V_35 ,
T_2 * V_6 ,
T_6 V_36 ,
T_7 type ,
T_8 * V_37 )
{
struct V_38 * V_8 = V_6 -> V_9 ;
T_9 * V_7 ;
int V_39 , V_40 ;
ASSERT ( V_35 ) ;
ASSERT ( F_14 ( V_37 ) ) ;
V_7 = V_37 -> V_41 ;
V_39 = V_36 - ( V_36 % V_8 -> V_42 ) ;
ASSERT ( V_39 >= 0 ) ;
memset ( V_7 , 0 , F_15 ( V_8 -> V_43 ) ) ;
for ( V_40 = 0 ; V_40 < V_8 -> V_42 ; V_40 ++ , V_7 ++ , V_39 ++ ) {
V_7 -> V_44 . V_45 = F_16 ( V_46 ) ;
V_7 -> V_44 . V_47 = V_48 ;
V_7 -> V_44 . V_10 = F_11 ( V_39 ) ;
V_7 -> V_44 . V_49 = type ;
}
F_17 ( V_35 , V_37 ,
( type & V_50 ? V_51 :
( ( type & V_52 ) ? V_53 :
V_54 ) ) ) ;
F_18 ( V_35 , V_37 , 0 , F_15 ( V_8 -> V_43 ) - 1 ) ;
}
STATIC int
F_19 (
T_5 * * V_55 ,
T_2 * V_6 ,
T_1 * V_1 ,
T_10 * V_56 ,
T_11 V_57 ,
T_8 * * V_58 )
{
T_12 V_59 ;
T_13 V_60 ;
T_14 V_61 ;
int V_62 , error , V_63 ;
T_8 * V_37 ;
T_5 * V_35 = * V_55 ;
ASSERT ( V_35 != NULL ) ;
F_20 ( V_1 ) ;
F_21 ( & V_60 , & V_59 ) ;
F_22 ( V_56 , V_64 ) ;
if ( ! F_23 ( V_1 -> V_65 , V_1 -> V_66 ) ) {
F_24 ( V_56 , V_64 ) ;
return ( V_67 ) ;
}
F_25 ( V_35 , V_56 , V_64 ) ;
V_62 = 1 ;
error = F_26 ( V_35 , V_56 , V_57 ,
V_68 , V_69 ,
& V_59 , F_27 ( V_6 ) ,
& V_61 , & V_62 , & V_60 ) ;
if ( error )
goto V_70;
ASSERT ( V_61 . V_71 == V_68 ) ;
ASSERT ( V_62 == 1 ) ;
ASSERT ( ( V_61 . V_72 != V_73 ) &&
( V_61 . V_72 != V_74 ) ) ;
V_1 -> V_75 = F_28 ( V_6 , V_61 . V_72 ) ;
V_37 = F_29 ( V_35 , V_6 -> V_76 ,
V_1 -> V_75 ,
V_6 -> V_9 -> V_43 ,
0 ) ;
error = F_30 ( V_37 ) ;
if ( error )
goto V_77;
F_13 ( V_35 , V_6 , F_31 ( V_1 -> V_78 . V_10 ) ,
V_1 -> V_66 & V_79 , V_37 ) ;
F_32 ( V_35 , V_37 ) ;
if ( ( error = F_33 ( V_55 , & V_60 , & V_63 ) ) ) {
goto V_77;
}
if ( V_63 ) {
V_35 = * V_55 ;
F_34 ( V_35 , V_37 ) ;
} else {
F_35 ( V_35 , V_37 ) ;
}
* V_58 = V_37 ;
return 0 ;
V_77:
F_36 ( & V_60 ) ;
V_70:
F_24 ( V_56 , V_64 ) ;
return ( error ) ;
}
STATIC int
F_37 (
T_5 * * V_55 ,
T_1 * V_1 ,
T_3 * * V_80 ,
T_8 * * V_58 ,
T_7 V_81 )
{
T_14 V_61 ;
int V_62 = 1 , error ;
T_8 * V_37 ;
T_10 * V_56 = F_38 ( V_1 ) ;
T_2 * V_6 = V_1 -> V_65 ;
T_3 * V_82 ;
T_6 V_36 = F_31 ( V_1 -> V_78 . V_10 ) ;
T_5 * V_35 = ( V_55 ? * V_55 : NULL ) ;
V_1 -> V_83 = ( T_11 ) V_36 / V_6 -> V_9 -> V_42 ;
F_22 ( V_56 , V_84 ) ;
if ( ! F_23 ( V_1 -> V_65 , V_1 -> V_66 ) ) {
F_24 ( V_56 , V_84 ) ;
return V_67 ;
}
error = F_39 ( V_56 , V_1 -> V_83 ,
V_68 , & V_61 , & V_62 , 0 ) ;
F_24 ( V_56 , V_84 ) ;
if ( error )
return error ;
ASSERT ( V_62 == 1 ) ;
ASSERT ( V_61 . V_71 == 1 ) ;
V_1 -> V_85 = ( V_36 % V_6 -> V_9 -> V_42 ) *
sizeof( T_9 ) ;
ASSERT ( V_61 . V_72 != V_73 ) ;
if ( V_61 . V_72 == V_74 ) {
if ( ! ( V_81 & V_86 ) )
return V_87 ;
ASSERT ( V_35 ) ;
error = F_19 ( V_55 , V_6 , V_1 , V_56 ,
V_1 -> V_83 , & V_37 ) ;
if ( error )
return error ;
V_35 = * V_55 ;
} else {
F_40 ( V_1 ) ;
V_1 -> V_75 = F_28 ( V_6 , V_61 . V_72 ) ;
error = F_41 ( V_6 , V_35 , V_6 -> V_76 ,
V_1 -> V_75 ,
V_6 -> V_9 -> V_43 ,
0 , & V_37 ) ;
if ( error || ! V_37 )
return F_42 ( error ) ;
}
ASSERT ( F_14 ( V_37 ) ) ;
V_82 = V_37 -> V_41 + V_1 -> V_85 ;
error = F_43 ( V_6 , V_82 , V_36 , V_1 -> V_66 & V_79 ,
V_81 & ( V_88 | V_89 ) ,
L_1 ) ;
if ( error ) {
if ( ! ( V_81 & V_88 ) ) {
F_44 ( V_35 , V_37 ) ;
return F_42 ( V_90 ) ;
}
}
* V_58 = V_37 ;
* V_80 = V_82 ;
return ( 0 ) ;
}
int
F_45 (
struct V_91 * V_6 ,
T_6 V_36 ,
T_7 type ,
T_7 V_81 ,
struct V_92 * * V_93 )
{
struct V_92 * V_1 ;
struct V_94 * V_95 ;
struct V_96 * V_37 ;
struct V_97 * V_35 = NULL ;
int error ;
int V_98 = 0 ;
V_1 = F_46 ( V_4 , V_99 ) ;
V_1 -> V_66 = type ;
V_1 -> V_78 . V_10 = F_11 ( V_36 ) ;
V_1 -> V_65 = V_6 ;
F_47 ( & V_1 -> V_2 ) ;
F_48 ( & V_1 -> V_3 ) ;
F_49 ( & V_1 -> V_100 ) ;
F_50 ( & V_1 -> V_101 ) ;
F_51 ( & V_1 -> V_101 ) ;
if ( ! ( type & V_50 ) )
F_52 ( & V_1 -> V_3 , & V_102 ) ;
F_53 ( V_5 ) ;
F_54 ( V_1 ) ;
if ( V_81 & V_86 ) {
V_35 = F_55 ( V_6 , V_103 ) ;
error = F_56 ( V_35 , F_27 ( V_6 ) ,
F_57 ( V_6 ) +
F_15 ( V_6 -> V_9 -> V_43 ) - 1 + 128 ,
0 ,
V_104 ,
V_105 ) ;
if ( error )
goto V_77;
V_98 = V_106 ;
}
error = F_37 ( & V_35 , V_1 , & V_95 , & V_37 , V_81 ) ;
if ( error ) {
F_58 ( V_1 ) ;
V_98 |= V_107 ;
goto V_77;
}
memcpy ( & V_1 -> V_78 , V_95 , sizeof( T_3 ) ) ;
F_59 ( V_1 ) ;
V_1 -> V_108 = F_10 ( V_95 -> V_24 ) ;
V_1 -> V_109 = F_10 ( V_95 -> V_28 ) ;
V_1 -> V_110 = F_10 ( V_95 -> V_32 ) ;
F_60 ( V_37 , V_111 ) ;
ASSERT ( F_14 ( V_37 ) ) ;
F_44 ( V_35 , V_37 ) ;
if ( V_35 ) {
error = F_61 ( V_35 , V_106 ) ;
if ( error )
goto V_70;
}
* V_93 = V_1 ;
return error ;
V_77:
if ( V_35 )
F_62 ( V_35 , V_98 ) ;
V_70:
F_1 ( V_1 ) ;
* V_93 = NULL ;
return error ;
}
int
F_63 (
T_2 * V_6 ,
T_10 * V_112 ,
T_6 V_36 ,
T_7 type ,
T_7 V_81 ,
T_1 * * V_93 )
{
struct V_38 * V_113 = V_6 -> V_9 ;
struct V_114 * V_115 = F_64 ( V_113 , type ) ;
struct V_92 * V_1 ;
int error ;
ASSERT ( F_65 ( V_6 ) ) ;
if ( ( ! F_66 ( V_6 ) && type == V_50 ) ||
( ! F_67 ( V_6 ) && type == V_52 ) ||
( ! F_68 ( V_6 ) && type == V_116 ) ) {
return ( V_67 ) ;
}
#ifdef F_9
if ( V_117 ) {
if ( ( V_118 == V_6 -> V_76 ) &&
( V_119 ++ % V_120 ) == 0 ) {
F_69 ( V_6 , L_2 ) ;
return ( V_90 ) ;
}
}
ASSERT ( type == V_50 ||
type == V_52 ||
type == V_116 ) ;
if ( V_112 ) {
ASSERT ( F_70 ( V_112 , V_64 ) ) ;
ASSERT ( F_71 ( V_112 , type ) == NULL ) ;
}
#endif
V_121:
F_72 ( & V_113 -> V_122 ) ;
V_1 = F_73 ( V_115 , V_36 ) ;
if ( V_1 ) {
F_74 ( V_1 ) ;
if ( V_1 -> V_66 & V_123 ) {
F_75 ( V_1 ) ;
F_76 ( & V_113 -> V_122 ) ;
F_77 ( V_1 ) ;
F_78 ( 1 ) ;
goto V_121;
}
V_1 -> V_124 ++ ;
F_76 ( & V_113 -> V_122 ) ;
F_79 ( V_1 ) ;
F_53 ( V_125 ) ;
* V_93 = V_1 ;
return 0 ;
}
F_76 ( & V_113 -> V_122 ) ;
F_53 ( V_126 ) ;
if ( V_112 )
F_24 ( V_112 , V_64 ) ;
error = F_45 ( V_6 , V_36 , type , V_81 , & V_1 ) ;
if ( V_112 )
F_22 ( V_112 , V_64 ) ;
if ( error )
return error ;
if ( V_112 ) {
if ( F_23 ( V_6 , type ) ) {
struct V_92 * V_127 ;
V_127 = F_71 ( V_112 , type ) ;
if ( V_127 ) {
F_1 ( V_1 ) ;
V_1 = V_127 ;
F_74 ( V_1 ) ;
goto V_128;
}
} else {
F_1 ( V_1 ) ;
return F_42 ( V_67 ) ;
}
}
F_72 ( & V_113 -> V_122 ) ;
error = - F_80 ( V_115 , V_36 , V_1 ) ;
if ( F_81 ( error ) ) {
F_82 ( error != V_129 ) ;
F_76 ( & V_113 -> V_122 ) ;
F_83 ( V_1 ) ;
F_1 ( V_1 ) ;
F_53 ( V_130 ) ;
goto V_121;
}
F_74 ( V_1 ) ;
V_1 -> V_124 = 1 ;
V_113 -> V_131 ++ ;
F_76 ( & V_113 -> V_122 ) ;
V_128:
ASSERT ( ( V_112 == NULL ) || F_70 ( V_112 , V_64 ) ) ;
F_84 ( V_1 ) ;
* V_93 = V_1 ;
return ( 0 ) ;
}
STATIC void
F_85 (
struct V_92 * V_1 )
{
struct V_38 * V_113 = V_1 -> V_65 -> V_9 ;
struct V_92 * V_132 ;
F_86 ( V_1 ) ;
F_72 ( & V_113 -> V_133 ) ;
if ( F_2 ( & V_1 -> V_2 ) ) {
F_87 ( & V_1 -> V_2 , & V_113 -> V_134 ) ;
V_113 -> V_135 ++ ;
F_53 ( V_136 ) ;
}
F_76 ( & V_113 -> V_133 ) ;
V_132 = V_1 -> V_137 ;
if ( V_132 ) {
F_74 ( V_132 ) ;
V_1 -> V_137 = NULL ;
}
F_75 ( V_1 ) ;
if ( V_132 )
F_88 ( V_132 ) ;
}
void
F_88 (
struct V_92 * V_1 )
{
ASSERT ( V_1 -> V_124 > 0 ) ;
ASSERT ( F_89 ( V_1 ) ) ;
F_90 ( V_1 ) ;
if ( -- V_1 -> V_124 > 0 )
F_75 ( V_1 ) ;
else
F_85 ( V_1 ) ;
}
void
F_91 (
T_1 * V_1 )
{
if ( ! V_1 )
return;
F_92 ( V_1 ) ;
F_74 ( V_1 ) ;
F_88 ( V_1 ) ;
}
STATIC void
F_93 (
struct V_96 * V_37 ,
struct V_138 * V_139 )
{
T_15 * V_140 = (struct V_141 * ) V_139 ;
T_1 * V_1 = V_140 -> V_142 ;
struct V_143 * V_144 = V_139 -> V_145 ;
if ( ( V_139 -> V_146 & V_147 ) &&
V_139 -> V_148 == V_140 -> V_149 ) {
F_94 ( & V_144 -> V_150 ) ;
if ( V_139 -> V_148 == V_140 -> V_149 )
F_95 ( V_144 , V_139 ) ;
else
F_96 ( & V_144 -> V_150 ) ;
}
F_97 ( V_1 ) ;
}
int
F_98 (
T_1 * V_1 ,
T_7 V_81 )
{
struct V_91 * V_6 = V_1 -> V_65 ;
struct V_96 * V_37 ;
struct V_94 * V_95 ;
int error ;
ASSERT ( F_89 ( V_1 ) ) ;
ASSERT ( ! F_99 ( & V_1 -> V_101 ) ) ;
F_100 ( V_1 ) ;
if ( ! F_101 ( V_1 ) ||
( ( V_81 & V_151 ) && F_102 ( & V_1 -> V_152 ) > 0 ) ) {
F_97 ( V_1 ) ;
return 0 ;
}
F_103 ( V_1 ) ;
if ( F_104 ( V_6 ) ) {
V_1 -> V_66 &= ~ V_153 ;
F_97 ( V_1 ) ;
return F_42 ( V_90 ) ;
}
error = F_41 ( V_6 , NULL , V_6 -> V_76 , V_1 -> V_75 ,
V_6 -> V_9 -> V_43 , 0 , & V_37 ) ;
if ( error ) {
ASSERT ( error != V_87 ) ;
F_97 ( V_1 ) ;
return error ;
}
V_95 = V_37 -> V_41 + V_1 -> V_85 ;
error = F_43 ( V_6 , & V_1 -> V_78 , F_31 ( V_95 -> V_10 ) , 0 ,
V_89 , L_3 ) ;
if ( error ) {
F_105 ( V_37 ) ;
F_97 ( V_1 ) ;
F_106 ( V_6 , V_154 ) ;
return F_42 ( V_90 ) ;
}
memcpy ( V_95 , & V_1 -> V_78 , sizeof( T_3 ) ) ;
V_1 -> V_66 &= ~ V_153 ;
F_107 ( V_6 -> V_155 , & V_1 -> V_156 . V_149 ,
& V_1 -> V_156 . V_157 . V_148 ) ;
F_108 ( V_37 , F_93 ,
& V_1 -> V_156 . V_157 ) ;
if ( F_109 ( V_37 ) ) {
F_110 ( V_1 ) ;
F_111 ( V_6 , 0 ) ;
}
if ( V_81 & V_158 )
error = F_112 ( V_37 ) ;
else
F_113 ( V_37 ) ;
F_105 ( V_37 ) ;
F_114 ( V_1 ) ;
return error ;
}
void
F_115 (
T_1 * V_159 ,
T_1 * V_160 )
{
if ( V_159 && V_160 ) {
ASSERT ( V_159 != V_160 ) ;
if ( F_31 ( V_159 -> V_78 . V_10 ) >
F_31 ( V_160 -> V_78 . V_10 ) ) {
F_72 ( & V_160 -> V_3 ) ;
F_116 ( & V_159 -> V_3 , V_161 ) ;
} else {
F_72 ( & V_159 -> V_3 ) ;
F_116 ( & V_160 -> V_3 , V_161 ) ;
}
} else if ( V_159 ) {
F_72 ( & V_159 -> V_3 ) ;
} else if ( V_160 ) {
F_72 ( & V_160 -> V_3 ) ;
}
}
void
F_117 (
T_1 * V_1 )
{
T_2 * V_6 = V_1 -> V_65 ;
T_8 * V_37 ;
V_37 = F_118 ( V_6 -> V_76 , V_1 -> V_75 ,
V_6 -> V_9 -> V_43 , V_162 ) ;
if ( ! V_37 )
goto V_163;
if ( F_119 ( V_37 ) ) {
if ( F_109 ( V_37 ) )
F_111 ( V_6 , 0 ) ;
F_120 ( V_37 ) ;
F_121 ( V_37 -> V_164 -> V_165 ) ;
}
F_105 ( V_37 ) ;
V_163:
F_122 ( V_1 ) ;
}
int T_16
F_123 ( void )
{
V_4 =
F_124 ( sizeof( struct V_92 ) , L_4 ) ;
if ( ! V_4 )
goto V_166;
V_167 =
F_124 ( sizeof( struct V_168 ) , L_5 ) ;
if ( ! V_167 )
goto V_169;
return 0 ;
V_169:
F_125 ( V_4 ) ;
V_166:
return - V_170 ;
}
void
F_126 ( void )
{
F_125 ( V_167 ) ;
F_125 ( V_4 ) ;
}
