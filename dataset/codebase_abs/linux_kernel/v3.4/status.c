void F_1 ( struct V_1 * V_2 ,
struct V_3 * V_4 )
{
struct V_5 * V_6 = F_2 ( V_2 ) ;
struct V_7 * V_8 = F_3 ( V_4 ) ;
int V_9 ;
V_4 -> V_10 = V_11 ;
F_4 ( V_8 -> V_12 & V_13 ?
& V_6 -> V_14 : & V_6 -> V_15 , V_4 ) ;
V_9 = F_5 ( & V_6 -> V_14 ) +
F_5 ( & V_6 -> V_15 ) ;
while ( V_9 > V_16 &&
( V_4 = F_6 ( & V_6 -> V_15 ) ) ) {
F_7 ( V_4 ) ;
V_9 -- ;
F_8 ( V_6 -> V_17 ) ;
}
F_9 ( & V_6 -> V_18 ) ;
}
static void F_10 ( struct V_5 * V_6 ,
struct V_19 * V_20 ,
struct V_3 * V_4 )
{
struct V_7 * V_8 = F_3 ( V_4 ) ;
struct V_21 * V_22 = ( void * ) V_4 -> V_23 ;
int V_24 ;
memset ( & V_8 -> V_25 , 0 , sizeof( V_8 -> V_25 ) ) ;
V_8 -> V_25 . V_26 = V_26 ;
V_8 -> V_25 . V_27 = & V_20 -> V_28 -> V_27 ;
V_8 -> V_12 |= V_29 |
V_30 ;
V_8 -> V_12 &= ~ V_31 ;
V_20 -> V_32 ++ ;
if ( V_22 -> V_33 & F_11 ( V_34 ) )
V_22 -> V_33 &= ~ F_11 ( V_34 ) ;
if ( F_12 ( V_22 -> V_33 ) ) {
T_1 * V_35 = F_13 ( V_22 ) ;
int V_36 = * V_35 & V_37 ;
if ( * V_35 & V_38 )
* V_35 &= ~ V_38 ;
V_24 = V_39 [ V_36 & 7 ] ;
} else {
V_24 = V_40 ;
}
F_14 ( V_20 , V_41 ) ;
if ( F_15 ( V_20 , V_42 ) &&
F_5 ( & V_20 -> V_43 [ V_24 ] ) < V_44 ) {
F_4 ( & V_20 -> V_43 [ V_24 ] , V_4 ) ;
F_16 ( V_20 ) ;
if ( ! F_17 ( & V_6 -> V_45 ) )
F_18 ( & V_6 -> V_45 ,
F_19 ( V_26 +
V_46 ) ) ;
return;
}
if ( ! F_15 ( V_20 , V_42 ) &&
! ( V_8 -> V_12 & V_47 ) ) {
V_8 -> V_12 |= V_47 ;
F_20 ( V_6 , V_4 ) ;
return;
}
#ifdef F_21
if ( F_22 () )
F_23 ( V_6 -> V_2 . V_48 ,
L_1 ,
F_5 ( & V_20 -> V_43 [ V_24 ] ) ,
! ! F_15 ( V_20 , V_42 ) , V_26 ) ;
#endif
F_24 ( V_4 ) ;
}
static void F_25 ( struct V_19 * V_20 , T_1 * V_49 , T_1 V_36 )
{
struct V_50 * V_51 ;
V_51 = F_26 ( V_20 -> V_52 . V_51 [ V_36 ] ) ;
if ( ! V_51 || ! V_51 -> V_53 )
return;
V_51 -> V_53 = false ;
F_27 ( & V_20 -> V_28 -> V_27 , V_49 , V_36 , V_51 -> V_54 ) ;
}
static void F_28 ( struct V_19 * V_20 , struct V_3 * V_4 )
{
struct V_55 * V_56 = ( void * ) V_4 -> V_23 ;
struct V_5 * V_6 = V_20 -> V_6 ;
struct V_57 * V_28 = V_20 -> V_28 ;
if ( F_12 ( V_56 -> V_33 ) ) {
struct V_21 * V_22 = ( void * ) V_4 -> V_23 ;
T_1 * V_58 = F_13 ( V_22 ) ;
T_2 V_36 = V_58 [ 0 ] & 0xf ;
F_25 ( V_20 , V_22 -> V_59 , V_36 ) ;
}
if ( F_29 ( V_56 -> V_33 ) &&
V_28 -> V_27 . type == V_60 &&
V_56 -> V_61 . V_62 . V_63 == V_64 &&
V_56 -> V_61 . V_62 . V_61 . V_65 . V_62 == V_66 ) {
switch ( V_56 -> V_61 . V_62 . V_61 . V_65 . V_67 ) {
case V_68 :
V_20 -> V_28 -> V_61 . V_69 . V_70 = V_71 ;
break;
case V_72 :
V_20 -> V_28 -> V_61 . V_69 . V_70 = V_73 ;
break;
case V_74 :
default:
V_20 -> V_28 -> V_61 . V_69 . V_70 = V_75 ;
break;
}
F_30 ( & V_6 -> V_2 , & V_6 -> V_76 ) ;
}
}
static void F_31 ( struct V_19 * V_20 , T_1 V_36 , T_2 V_77 )
{
struct V_50 * V_51 ;
V_51 = F_26 ( V_20 -> V_52 . V_51 [ V_36 ] ) ;
if ( ! V_51 )
return;
V_51 -> V_54 = V_77 ;
V_51 -> V_53 = true ;
}
static int F_32 ( struct V_7 * V_8 )
{
int V_78 = sizeof( struct V_79 ) ;
if ( V_8 -> V_80 . V_81 [ 0 ] . V_82 >= 0 &&
! ( V_8 -> V_80 . V_81 [ 0 ] . V_12 & V_83 ) )
V_78 += 2 ;
V_78 += 2 ;
V_78 += 1 ;
if ( V_8 -> V_80 . V_81 [ 0 ] . V_82 >= 0 &&
V_8 -> V_80 . V_81 [ 0 ] . V_12 & V_83 )
V_78 += 3 ;
return V_78 ;
}
static void F_33 ( struct V_84
* V_85 , struct V_3 * V_4 ,
int V_86 , int V_87 )
{
struct V_7 * V_8 = F_3 ( V_4 ) ;
struct V_21 * V_22 = (struct V_21 * ) V_4 -> V_23 ;
struct V_79 * V_88 ;
unsigned char * V_89 ;
T_2 V_90 ;
V_88 = (struct V_79 * ) F_34 ( V_4 , V_87 ) ;
memset ( V_88 , 0 , V_87 ) ;
V_88 -> V_91 = F_11 ( V_87 ) ;
V_88 -> V_92 =
F_35 ( ( 1 << V_93 ) |
( 1 << V_94 ) ) ;
V_89 = ( unsigned char * ) ( V_88 + 1 ) ;
if ( V_8 -> V_80 . V_81 [ 0 ] . V_82 >= 0 &&
! ( V_8 -> V_80 . V_81 [ 0 ] . V_12 & V_83 ) ) {
V_88 -> V_92 |= F_35 ( 1 << V_95 ) ;
* V_89 = V_85 -> V_96 [ V_8 -> V_80 . V_81 [ 0 ] . V_82 ] . V_97 / 5 ;
V_89 += 2 ;
}
V_90 = 0 ;
if ( ! ( V_8 -> V_12 & V_98 ) &&
! F_36 ( V_22 -> V_59 ) )
V_90 |= V_99 ;
if ( ( V_8 -> V_80 . V_81 [ 0 ] . V_12 & V_100 ) ||
( V_8 -> V_80 . V_81 [ 0 ] . V_12 & V_101 ) )
V_90 |= V_102 ;
else if ( V_8 -> V_80 . V_81 [ 0 ] . V_12 & V_100 )
V_90 |= V_103 ;
F_37 ( V_90 , V_89 ) ;
V_89 += 2 ;
* V_89 = V_86 ;
V_89 ++ ;
if ( V_8 -> V_80 . V_81 [ 0 ] . V_82 >= 0 &&
V_8 -> V_80 . V_81 [ 0 ] . V_12 & V_83 ) {
V_88 -> V_92 |= F_35 ( 1 << V_104 ) ;
V_89 [ 0 ] = V_105 |
V_106 |
V_107 ;
if ( V_8 -> V_80 . V_81 [ 0 ] . V_12 & V_108 )
V_89 [ 1 ] |= V_109 ;
if ( V_8 -> V_80 . V_81 [ 0 ] . V_12 & V_110 )
V_89 [ 1 ] |= V_111 ;
if ( V_8 -> V_80 . V_81 [ 0 ] . V_12 & V_112 )
V_89 [ 1 ] |= V_113 ;
V_89 [ 2 ] = V_8 -> V_80 . V_81 [ 0 ] . V_82 ;
V_89 += 3 ;
}
}
void F_38 ( struct V_1 * V_2 , struct V_3 * V_4 )
{
struct V_3 * V_114 ;
struct V_21 * V_22 = (struct V_21 * ) V_4 -> V_23 ;
struct V_5 * V_6 = F_2 ( V_2 ) ;
struct V_7 * V_8 = F_3 ( V_4 ) ;
T_3 V_115 ;
struct V_84 * V_85 ;
struct V_57 * V_28 ;
struct V_116 * V_117 = NULL ;
struct V_19 * V_20 , * V_9 ;
int V_86 = - 1 , V_118 ;
int V_119 = - 1 ;
bool V_120 ;
bool V_121 ;
struct V_122 * V_123 ;
int V_87 ;
for ( V_118 = 0 ; V_118 < V_124 ; V_118 ++ ) {
if ( V_8 -> V_80 . V_81 [ V_118 ] . V_82 < 0 ) {
break;
} else if ( V_118 >= V_2 -> V_125 ) {
V_8 -> V_80 . V_81 [ V_118 ] . V_82 = - 1 ;
V_8 -> V_80 . V_81 [ V_118 ] . V_126 = 0 ;
break;
}
V_86 += V_8 -> V_80 . V_81 [ V_118 ] . V_126 ;
}
V_119 = V_118 - 1 ;
if ( V_86 < 0 )
V_86 = 0 ;
F_39 () ;
V_85 = V_6 -> V_2 . V_48 -> V_127 [ V_8 -> V_128 ] ;
V_115 = V_22 -> V_33 ;
F_40 (local, hdr->addr1, sta, tmp) {
if ( F_41 ( V_22 -> V_129 , V_20 -> V_28 -> V_27 . V_49 ) )
continue;
if ( V_8 -> V_12 & V_130 )
F_42 ( V_20 , V_131 ) ;
V_121 = ! ! ( V_8 -> V_12 & V_98 ) ;
if ( ! V_121 && F_15 ( V_20 , V_42 ) ) {
F_10 ( V_6 , V_20 , V_4 ) ;
F_43 () ;
return;
}
if ( ( V_6 -> V_2 . V_12 & V_132 ) &&
( V_119 != - 1 ) )
V_20 -> V_133 = V_8 -> V_80 . V_81 [ V_119 ] ;
if ( ( V_8 -> V_12 & V_134 ) &&
( F_12 ( V_115 ) ) ) {
T_2 V_36 , V_77 ;
T_1 * V_58 ;
V_58 = F_13 ( V_22 ) ;
V_36 = V_58 [ 0 ] & 0xf ;
V_77 = ( ( F_44 ( V_22 -> V_135 ) + 0x10 )
& V_136 ) ;
F_27 ( & V_20 -> V_28 -> V_27 , V_22 -> V_59 ,
V_36 , V_77 ) ;
}
if ( ! V_121 && F_45 ( V_115 ) ) {
T_2 V_36 , V_25 ;
V_123 = (struct V_122 * ) V_4 -> V_23 ;
V_25 = F_44 ( V_123 -> V_25 ) ;
if ( ! ( V_25 & V_137 ) ) {
T_2 V_77 = F_44 ( V_123 -> V_138 ) ;
V_36 = ( V_25 &
V_139 ) >>
V_140 ;
F_31 ( V_20 , V_36 , V_77 ) ;
}
}
if ( V_8 -> V_12 & V_141 ) {
F_10 ( V_6 , V_20 , V_4 ) ;
F_43 () ;
return;
} else {
if ( ! V_121 )
V_20 -> V_142 ++ ;
V_20 -> V_143 += V_86 ;
}
F_46 ( V_6 , V_85 , V_20 , V_4 ) ;
if ( F_47 ( & V_20 -> V_28 -> V_27 ) )
F_48 ( V_6 , V_20 , V_4 ) ;
if ( ! ( V_8 -> V_12 & V_144 ) && V_121 )
F_28 ( V_20 , V_4 ) ;
if ( ( V_20 -> V_28 -> V_27 . type == V_60 ) &&
( V_6 -> V_2 . V_12 & V_145 ) )
F_49 ( V_20 -> V_28 , ( void * ) V_4 -> V_23 , V_121 ) ;
if ( V_6 -> V_2 . V_12 & V_145 ) {
if ( V_8 -> V_12 & V_98 ) {
if ( V_20 -> V_146 )
V_20 -> V_146 = 0 ;
} else if ( ++ V_20 -> V_146 >= V_147 ) {
F_50 ( V_20 -> V_28 -> V_148 ,
V_20 -> V_20 . V_49 ,
V_20 -> V_146 ,
V_149 ) ;
V_20 -> V_146 = 0 ;
}
}
}
F_43 () ;
F_51 ( V_6 , 0 ) ;
if ( V_8 -> V_12 & V_98 ) {
if ( F_52 ( V_22 -> V_135 ) ) {
V_6 -> V_150 ++ ;
if ( F_36 ( V_22 -> V_59 ) )
V_6 -> V_151 ++ ;
if ( V_86 > 0 )
V_6 -> V_152 ++ ;
if ( V_86 > 1 )
V_6 -> V_153 ++ ;
}
if ( ! F_36 ( V_22 -> V_59 ) ||
F_53 ( V_115 ) ||
F_54 ( V_115 ) )
V_6 -> V_154 ++ ;
} else {
if ( F_52 ( V_22 -> V_135 ) )
V_6 -> V_155 ++ ;
}
if ( F_55 ( V_115 ) && F_56 ( V_115 ) &&
( V_6 -> V_2 . V_12 & V_145 ) &&
! ( V_8 -> V_12 & V_144 ) &&
V_6 -> V_156 && ! ( V_6 -> V_157 ) ) {
if ( V_8 -> V_12 & V_98 ) {
V_6 -> V_156 -> V_61 . V_69 . V_12 |=
V_158 ;
} else
F_18 ( & V_6 -> V_159 , V_26 +
F_57 ( 10 ) ) ;
}
if ( V_8 -> V_12 & V_160 ) {
T_4 V_161 = ( unsigned long ) V_4 ;
if ( F_55 ( V_22 -> V_33 ) ||
F_58 ( V_22 -> V_33 ) ) {
V_121 = V_8 -> V_12 & V_98 ;
F_59 ( V_4 -> V_148 , V_22 -> V_59 ,
V_161 , V_121 , V_149 ) ;
} else {
struct V_162 * V_163 ;
F_39 () ;
F_60 (wk, &local->work_list, list) {
if ( V_163 -> type != V_164 )
continue;
if ( V_163 -> V_165 . V_166 != V_4 )
continue;
V_163 -> V_165 . V_80 = true ;
break;
}
F_43 () ;
if ( V_6 -> V_167 == V_4 ) {
V_161 = V_6 -> V_168 ^ 2 ;
V_6 -> V_167 = NULL ;
}
F_61 (
V_4 -> V_148 , V_161 , V_4 -> V_23 , V_4 -> V_78 ,
! ! ( V_8 -> V_12 & V_98 ) ,
V_149 ) ;
}
}
if ( F_62 ( V_8 -> V_169 ) ) {
struct V_3 * V_170 ;
unsigned long V_12 ;
F_63 ( & V_6 -> V_171 , V_12 ) ;
V_170 = F_64 ( & V_6 -> V_172 ,
V_8 -> V_169 ) ;
if ( V_170 )
F_65 ( & V_6 -> V_172 ,
V_8 -> V_169 ) ;
F_66 ( & V_6 -> V_171 , V_12 ) ;
if ( V_170 )
F_67 ( V_170 ,
V_8 -> V_12 & V_98 ) ;
}
F_68 ( V_4 ) ;
V_120 = ! ! ( V_8 -> V_12 & V_144 ) ||
! ( F_53 ( V_115 ) ) ;
if ( ! V_6 -> V_173 && ( ! V_120 || ! V_6 -> V_174 ) ) {
F_24 ( V_4 ) ;
return;
}
V_87 = F_32 ( V_8 ) ;
if ( F_69 ( F_70 ( V_4 ) < V_87 ) ) {
F_71 ( V_175 L_2 ) ;
F_24 ( V_4 ) ;
return;
}
F_33 ( V_85 , V_4 , V_86 , V_87 ) ;
F_72 ( V_4 , 0 ) ;
V_4 -> V_176 = V_177 ;
V_4 -> V_10 = V_178 ;
V_4 -> V_179 = F_73 ( V_180 ) ;
memset ( V_4 -> V_181 , 0 , sizeof( V_4 -> V_181 ) ) ;
F_39 () ;
F_60 (sdata, &local->interfaces, list) {
if ( V_28 -> V_27 . type == V_182 ) {
if ( ! F_74 ( V_28 ) )
continue;
if ( ( V_28 -> V_61 . V_183 & V_184 ) &&
! V_120 )
continue;
if ( V_117 ) {
V_114 = F_75 ( V_4 , V_149 ) ;
if ( V_114 ) {
V_114 -> V_148 = V_117 ;
F_76 ( V_114 ) ;
}
}
V_117 = V_28 -> V_148 ;
}
}
if ( V_117 ) {
V_4 -> V_148 = V_117 ;
F_76 ( V_4 ) ;
V_4 = NULL ;
}
F_43 () ;
F_24 ( V_4 ) ;
}
void F_77 ( struct V_185 * V_186 , T_5 V_187 )
{
struct V_19 * V_20 = F_78 ( V_186 , struct V_19 , V_20 ) ;
F_50 ( V_20 -> V_28 -> V_148 , V_20 -> V_20 . V_49 ,
V_187 , V_149 ) ;
}
void F_79 ( struct V_1 * V_2 , struct V_3 * V_4 )
{
struct V_5 * V_6 = F_2 ( V_2 ) ;
struct V_7 * V_8 = F_3 ( V_4 ) ;
if ( F_62 ( V_8 -> V_169 ) ) {
struct V_3 * V_170 ;
unsigned long V_12 ;
F_63 ( & V_6 -> V_171 , V_12 ) ;
V_170 = F_64 ( & V_6 -> V_172 ,
V_8 -> V_169 ) ;
if ( V_170 )
F_65 ( & V_6 -> V_172 ,
V_8 -> V_169 ) ;
F_66 ( & V_6 -> V_171 , V_12 ) ;
if ( V_170 )
F_80 ( V_170 ) ;
}
F_80 ( V_4 ) ;
}
