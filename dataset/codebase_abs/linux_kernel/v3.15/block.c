static int T_1 F_1 ( const char * V_1 ,
const struct V_2 * V_3 )
{
int V_4 , V_5 ;
T_2 V_6 ;
struct V_7 * V_8 ;
char V_9 [ V_10 ] ;
char * V_11 = & V_9 [ 0 ] ;
char * V_12 [ V_13 ] ;
if ( ! V_1 )
return - V_14 ;
V_6 = F_2 ( V_1 , V_10 ) ;
if ( V_6 == 0 ) {
F_3 ( L_1 ) ;
return 0 ;
}
if ( V_6 == V_10 ) {
F_4 ( L_2 ,
V_1 , V_10 ) ;
return - V_14 ;
}
strcpy ( V_9 , V_1 ) ;
if ( V_9 [ V_6 - 1 ] == '\n' )
V_9 [ V_6 - 1 ] = '\0' ;
for ( V_4 = 0 ; V_4 < V_13 ; V_4 ++ )
V_12 [ V_4 ] = F_5 ( & V_11 , L_3 ) ;
V_8 = & V_7 [ V_15 ] ;
if ( V_12 [ 1 ] ) {
V_5 = F_6 ( V_12 [ 0 ] , 10 , & V_8 -> V_16 ) ;
if ( V_5 < 0 )
return - V_14 ;
V_5 = F_6 ( V_12 [ 1 ] , 10 , & V_8 -> V_17 ) ;
if ( V_5 < 0 ) {
V_8 -> V_17 = - 1 ;
strcpy ( V_8 -> V_18 , V_12 [ 1 ] ) ;
}
} else {
strcpy ( V_8 -> V_18 , V_12 [ 0 ] ) ;
V_8 -> V_16 = - 1 ;
V_8 -> V_17 = - 1 ;
}
V_15 ++ ;
return 0 ;
}
static struct V_19 * F_7 ( int V_16 , int V_17 )
{
struct V_19 * V_20 ;
F_8 (dev, &ubiblock_devices, list)
if ( V_20 -> V_16 == V_16 && V_20 -> V_17 == V_17 )
return V_20 ;
return NULL ;
}
static int F_9 ( struct V_19 * V_20 , char * V_21 ,
int V_22 , int V_23 , int V_6 )
{
int V_5 ;
V_5 = F_10 ( V_20 -> V_24 , V_22 , V_21 , V_23 , V_6 ) ;
if ( V_5 ) {
F_4 ( L_4 ,
V_20 -> V_25 -> V_26 , V_5 ) ;
return V_5 ;
}
return 0 ;
}
static int F_11 ( struct V_19 * V_20 , char * V_21 ,
T_3 V_27 , int V_6 )
{
int V_5 , V_22 , V_23 ;
int V_28 = V_6 ;
int V_29 = V_6 ;
T_4 V_30 = V_27 << 9 ;
V_23 = F_12 ( V_30 , V_20 -> V_31 ) ;
V_22 = V_30 ;
while ( V_28 ) {
if ( V_23 + V_29 > V_20 -> V_31 )
V_29 = V_20 -> V_31 - V_23 ;
V_5 = F_9 ( V_20 , V_21 , V_22 , V_23 , V_29 ) ;
if ( V_5 )
return V_5 ;
V_21 += V_29 ;
V_28 -= V_29 ;
V_29 = V_28 ;
V_22 += 1 ;
V_23 = 0 ;
}
return 0 ;
}
static int F_13 ( struct V_19 * V_20 , struct V_32 * V_33 )
{
int V_6 , V_5 ;
T_3 V_27 ;
if ( V_33 -> V_34 != V_35 )
return - V_36 ;
if ( F_14 ( V_33 ) + F_15 ( V_33 ) >
F_16 ( V_33 -> V_37 ) )
return - V_36 ;
if ( F_17 ( V_33 ) != V_38 )
return - V_39 ;
V_27 = F_14 ( V_33 ) ;
V_6 = F_18 ( V_33 ) ;
F_19 ( & V_20 -> V_40 ) ;
V_5 = F_11 ( V_20 , V_33 -> V_21 , V_27 , V_6 ) ;
F_20 ( & V_20 -> V_40 ) ;
return V_5 ;
}
static void F_21 ( struct V_41 * V_42 )
{
struct V_19 * V_20 =
F_22 ( V_42 , struct V_19 , V_42 ) ;
struct V_43 * V_44 = V_20 -> V_44 ;
struct V_32 * V_33 ;
int V_45 ;
F_23 ( V_44 -> V_46 ) ;
V_33 = F_24 ( V_44 ) ;
while ( V_33 ) {
F_25 ( V_44 -> V_46 ) ;
V_45 = F_13 ( V_20 , V_33 ) ;
F_23 ( V_44 -> V_46 ) ;
if ( ! F_26 ( V_33 , V_45 ) )
V_33 = F_24 ( V_44 ) ;
}
F_25 ( V_44 -> V_46 ) ;
}
static void F_27 ( struct V_43 * V_44 )
{
struct V_19 * V_20 ;
struct V_32 * V_33 ;
V_20 = V_44 -> V_47 ;
if ( ! V_20 )
while ( ( V_33 = F_24 ( V_44 ) ) != NULL )
F_28 ( V_33 , - V_48 ) ;
else
F_29 ( V_20 -> V_49 , & V_20 -> V_42 ) ;
}
static int F_30 ( struct V_50 * V_51 , T_5 V_52 )
{
struct V_19 * V_20 = V_51 -> V_53 -> V_54 ;
int V_5 ;
F_19 ( & V_20 -> V_40 ) ;
if ( V_20 -> V_55 > 0 ) {
goto V_56;
}
if ( V_52 & V_57 ) {
V_5 = - V_58 ;
goto V_59;
}
V_20 -> V_24 = F_31 ( V_20 -> V_16 , V_20 -> V_17 , V_60 ) ;
if ( F_32 ( V_20 -> V_24 ) ) {
F_4 ( L_5 ,
V_20 -> V_25 -> V_26 , V_20 -> V_16 , V_20 -> V_17 ) ;
V_5 = F_33 ( V_20 -> V_24 ) ;
V_20 -> V_24 = NULL ;
goto V_59;
}
V_56:
V_20 -> V_55 ++ ;
F_20 ( & V_20 -> V_40 ) ;
return 0 ;
V_59:
F_20 ( & V_20 -> V_40 ) ;
return V_5 ;
}
static void F_34 ( struct V_61 * V_25 , T_5 V_52 )
{
struct V_19 * V_20 = V_25 -> V_54 ;
F_19 ( & V_20 -> V_40 ) ;
V_20 -> V_55 -- ;
if ( V_20 -> V_55 == 0 ) {
F_35 ( V_20 -> V_24 ) ;
V_20 -> V_24 = NULL ;
}
F_20 ( & V_20 -> V_40 ) ;
}
static int F_36 ( struct V_50 * V_51 , struct V_62 * V_63 )
{
V_63 -> V_64 = 1 ;
V_63 -> V_65 = 1 ;
V_63 -> V_66 = F_16 ( V_51 -> V_53 ) ;
V_63 -> V_67 = 0 ;
return 0 ;
}
int F_37 ( struct V_68 * V_69 )
{
struct V_19 * V_20 ;
struct V_61 * V_25 ;
int V_70 ;
int V_5 ;
F_19 ( & V_71 ) ;
if ( F_7 ( V_69 -> V_16 , V_69 -> V_17 ) ) {
F_20 ( & V_71 ) ;
return - V_72 ;
}
F_20 ( & V_71 ) ;
V_20 = F_38 ( sizeof( struct V_19 ) , V_73 ) ;
if ( ! V_20 )
return - V_74 ;
F_39 ( & V_20 -> V_40 ) ;
V_20 -> V_16 = V_69 -> V_16 ;
V_20 -> V_17 = V_69 -> V_17 ;
V_20 -> V_31 = V_69 -> V_75 ;
V_25 = F_40 ( 1 ) ;
if ( ! V_25 ) {
F_4 ( L_6 ) ;
V_5 = - V_48 ;
goto V_76;
}
V_25 -> V_77 = & V_78 ;
V_25 -> V_79 = V_80 ;
V_25 -> V_81 = V_20 -> V_16 * V_82 + V_20 -> V_17 ;
V_25 -> V_54 = V_20 ;
sprintf ( V_25 -> V_26 , L_7 , V_20 -> V_16 , V_20 -> V_17 ) ;
V_70 = ( V_69 -> V_83 * V_69 -> V_75 ) >> 9 ;
F_41 ( V_25 , V_70 ) ;
V_20 -> V_25 = V_25 ;
F_42 ( & V_20 -> V_46 ) ;
V_20 -> V_44 = F_43 ( F_27 , & V_20 -> V_46 ) ;
if ( ! V_20 -> V_44 ) {
F_4 ( L_8 ) ;
V_5 = - V_48 ;
goto V_84;
}
V_20 -> V_44 -> V_47 = V_20 ;
V_20 -> V_25 -> V_85 = V_20 -> V_44 ;
V_20 -> V_49 = F_44 ( L_9 , 0 , 0 , V_25 -> V_26 ) ;
if ( ! V_20 -> V_49 )
goto V_86;
F_45 ( & V_20 -> V_42 , F_21 ) ;
F_19 ( & V_71 ) ;
F_46 ( & V_20 -> V_87 , & V_88 ) ;
F_20 ( & V_71 ) ;
F_47 ( V_20 -> V_25 ) ;
F_48 ( L_10 ,
V_20 -> V_25 -> V_26 , V_20 -> V_16 , V_20 -> V_17 , V_69 -> V_18 ) ;
return 0 ;
V_86:
F_49 ( V_20 -> V_44 ) ;
V_84:
F_50 ( V_20 -> V_25 ) ;
V_76:
F_51 ( V_20 ) ;
return V_5 ;
}
static void F_52 ( struct V_19 * V_20 )
{
F_53 ( V_20 -> V_25 ) ;
F_49 ( V_20 -> V_44 ) ;
F_48 ( L_11 , V_20 -> V_25 -> V_26 ) ;
F_50 ( V_20 -> V_25 ) ;
}
int F_54 ( struct V_68 * V_69 )
{
struct V_19 * V_20 ;
F_19 ( & V_71 ) ;
V_20 = F_7 ( V_69 -> V_16 , V_69 -> V_17 ) ;
if ( ! V_20 ) {
F_20 ( & V_71 ) ;
return - V_48 ;
}
F_19 ( & V_20 -> V_40 ) ;
if ( V_20 -> V_55 > 0 ) {
F_20 ( & V_20 -> V_40 ) ;
F_20 ( & V_71 ) ;
return - V_89 ;
}
F_55 ( & V_20 -> V_87 ) ;
F_20 ( & V_71 ) ;
F_56 ( V_20 -> V_49 ) ;
F_52 ( V_20 ) ;
F_20 ( & V_20 -> V_40 ) ;
F_51 ( V_20 ) ;
return 0 ;
}
static void F_57 ( struct V_68 * V_69 )
{
struct V_19 * V_20 ;
int V_70 ;
F_19 ( & V_71 ) ;
V_20 = F_7 ( V_69 -> V_16 , V_69 -> V_17 ) ;
if ( ! V_20 ) {
F_20 ( & V_71 ) ;
return;
}
F_19 ( & V_20 -> V_40 ) ;
V_70 = ( V_69 -> V_83 * V_69 -> V_75 ) >> 9 ;
F_41 ( V_20 -> V_25 , V_70 ) ;
F_48 ( L_12 , V_20 -> V_25 -> V_26 , V_69 -> V_83 ) ;
F_20 ( & V_20 -> V_40 ) ;
F_20 ( & V_71 ) ;
}
static int F_58 ( struct V_90 * V_91 ,
unsigned long V_92 , void * V_93 )
{
struct V_94 * V_95 = V_93 ;
switch ( V_92 ) {
case V_96 :
break;
case V_97 :
F_54 ( & V_95 -> V_69 ) ;
break;
case V_98 :
F_57 ( & V_95 -> V_69 ) ;
break;
default:
break;
}
return V_99 ;
}
static struct V_100 * T_1
F_59 ( const char * V_18 , int V_16 , int V_17 )
{
if ( V_16 == - 1 )
return F_60 ( V_18 , V_60 ) ;
else if ( V_17 == - 1 )
return F_61 ( V_16 , V_18 , V_60 ) ;
else
return F_31 ( V_16 , V_17 , V_60 ) ;
}
static int T_1 F_62 ( void )
{
int V_4 , V_5 ;
struct V_7 * V_101 ;
struct V_100 * V_24 ;
struct V_68 V_69 ;
for ( V_4 = 0 ; V_4 < V_15 ; V_4 ++ ) {
V_101 = & V_7 [ V_4 ] ;
V_24 = F_59 ( V_101 -> V_18 , V_101 -> V_16 , V_101 -> V_17 ) ;
if ( F_32 ( V_24 ) ) {
F_4 ( L_13 ,
F_33 ( V_24 ) ) ;
V_5 = F_33 ( V_24 ) ;
break;
}
F_63 ( V_24 , & V_69 ) ;
F_35 ( V_24 ) ;
V_5 = F_37 ( & V_69 ) ;
if ( V_5 ) {
F_4 ( L_14 ,
V_69 . V_18 , V_5 ) ;
break;
}
}
return V_5 ;
}
static void F_64 ( void )
{
struct V_19 * V_102 ;
struct V_19 * V_20 ;
F_65 (dev, next, &ubiblock_devices, list) {
F_56 ( V_20 -> V_49 ) ;
F_66 ( V_20 -> V_24 ) ;
F_55 ( & V_20 -> V_87 ) ;
F_52 ( V_20 ) ;
F_51 ( V_20 ) ;
}
}
int T_1 F_67 ( void )
{
int V_5 ;
V_80 = F_68 ( 0 , L_15 ) ;
if ( V_80 < 0 )
return V_80 ;
V_5 = F_62 () ;
if ( V_5 )
goto V_103;
V_5 = F_69 ( & V_104 , 1 ) ;
if ( V_5 )
goto V_105;
return 0 ;
V_105:
F_70 ( V_80 , L_15 ) ;
V_103:
F_64 () ;
return V_5 ;
}
void T_6 F_71 ( void )
{
F_72 ( & V_104 ) ;
F_64 () ;
F_70 ( V_80 , L_15 ) ;
}
