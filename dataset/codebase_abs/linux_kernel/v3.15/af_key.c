static inline struct V_1 * F_1 ( struct V_2 * V_3 )
{
return (struct V_1 * ) V_3 ;
}
static int F_2 ( const struct V_2 * V_3 )
{
if ( 3 * F_3 ( & V_3 -> V_4 ) <= 2 * V_3 -> V_5 )
return 1 ;
return 0 ;
}
static void F_4 ( struct V_1 * V_6 )
{
if ( V_6 -> V_7 . V_7 ) {
if ( V_6 -> V_7 . V_8 ) {
F_5 ( V_6 -> V_7 . V_8 ) ;
V_6 -> V_7 . V_8 = NULL ;
}
V_6 -> V_7 . V_9 ( V_6 ) ;
V_6 -> V_7 . V_7 = NULL ;
V_6 -> V_7 . V_9 = NULL ;
}
}
static void F_6 ( struct V_2 * V_3 )
{
struct V_10 * V_10 = F_7 ( V_3 ) ;
struct V_11 * V_12 = F_8 ( V_10 , V_13 ) ;
F_4 ( F_1 ( V_3 ) ) ;
F_9 ( & V_3 -> V_14 ) ;
if ( ! F_10 ( V_3 , V_15 ) ) {
F_11 ( L_1 , V_3 ) ;
return;
}
F_12 ( F_3 ( & V_3 -> V_4 ) ) ;
F_12 ( F_3 ( & V_3 -> V_16 ) ) ;
F_13 ( & V_12 -> V_17 ) ;
}
static void F_14 ( struct V_2 * V_3 )
{
struct V_10 * V_10 = F_7 ( V_3 ) ;
struct V_11 * V_12 = F_8 ( V_10 , V_13 ) ;
F_15 ( & V_18 ) ;
F_16 ( V_3 , & V_12 -> V_19 ) ;
F_17 ( & V_18 ) ;
}
static void F_18 ( struct V_2 * V_3 )
{
F_15 ( & V_18 ) ;
F_19 ( V_3 ) ;
F_17 ( & V_18 ) ;
}
static int F_20 ( struct V_10 * V_10 , struct V_20 * V_2 , int V_21 ,
int V_22 )
{
struct V_11 * V_12 = F_8 ( V_10 , V_13 ) ;
struct V_2 * V_3 ;
int V_23 ;
if ( ! F_21 ( V_10 -> V_24 , V_25 ) )
return - V_26 ;
if ( V_2 -> type != V_27 )
return - V_28 ;
if ( V_21 != V_29 )
return - V_30 ;
V_23 = - V_31 ;
V_3 = F_22 ( V_10 , V_32 , V_33 , & V_34 ) ;
if ( V_3 == NULL )
goto V_35;
V_2 -> V_36 = & V_37 ;
F_23 ( V_2 , V_3 ) ;
V_3 -> V_38 = V_32 ;
V_3 -> V_39 = F_6 ;
F_24 ( & V_12 -> V_17 ) ;
F_14 ( V_3 ) ;
return 0 ;
V_35:
return V_23 ;
}
static int F_25 ( struct V_20 * V_2 )
{
struct V_2 * V_3 = V_2 -> V_3 ;
if ( ! V_3 )
return 0 ;
F_18 ( V_3 ) ;
F_26 ( V_3 ) ;
V_2 -> V_3 = NULL ;
F_9 ( & V_3 -> V_40 ) ;
F_27 () ;
F_28 ( V_3 ) ;
return 0 ;
}
static int F_29 ( struct V_41 * V_8 , struct V_41 * * V_42 ,
T_1 V_43 , struct V_2 * V_3 )
{
int V_23 = - V_44 ;
F_30 ( V_3 ) ;
if ( * V_42 == NULL ) {
if ( F_3 ( & V_8 -> V_45 ) != 1 ) {
* V_42 = F_31 ( V_8 , V_43 ) ;
} else {
* V_42 = V_8 ;
F_24 ( & V_8 -> V_45 ) ;
}
}
if ( * V_42 != NULL ) {
if ( F_3 ( & V_3 -> V_4 ) <= V_3 -> V_5 ) {
F_32 ( * V_42 , V_3 ) ;
F_33 ( & V_3 -> V_14 , * V_42 ) ;
V_3 -> V_46 ( V_3 ) ;
* V_42 = NULL ;
V_23 = 0 ;
}
}
F_28 ( V_3 ) ;
return V_23 ;
}
static int F_34 ( struct V_41 * V_8 , T_1 V_43 ,
int V_47 , struct V_2 * V_48 ,
struct V_10 * V_10 )
{
struct V_11 * V_12 = F_8 ( V_10 , V_13 ) ;
struct V_2 * V_3 ;
struct V_41 * V_42 = NULL ;
int V_23 = - V_49 ;
if ( ! V_8 )
return - V_31 ;
F_35 () ;
F_36 (sk, &net_pfkey->table) {
struct V_1 * V_6 = F_1 ( V_3 ) ;
int V_50 ;
if ( V_6 -> V_51 )
F_29 ( V_8 , & V_42 , V_43 , V_3 ) ;
if ( V_3 == V_48 )
continue;
if ( V_47 != V_52 ) {
if ( V_47 & V_53 )
continue;
if ( ( V_47 & V_54 ) &&
! V_6 -> V_55 )
continue;
if ( V_47 & V_56 )
continue;
}
V_50 = F_29 ( V_8 , & V_42 , V_43 , V_3 ) ;
if ( ( V_47 & V_54 ) && V_23 )
V_23 = V_50 ;
}
F_37 () ;
if ( V_48 != NULL )
V_23 = F_29 ( V_8 , & V_42 , V_43 , V_48 ) ;
F_5 ( V_42 ) ;
F_5 ( V_8 ) ;
return V_23 ;
}
static int F_38 ( struct V_1 * V_6 )
{
struct V_57 * V_58 ;
int V_59 ;
V_59 = V_6 -> V_7 . V_7 ( V_6 ) ;
if ( V_59 == - V_44 )
return 0 ;
if ( V_6 -> V_7 . V_8 ) {
if ( ! F_2 ( & V_6 -> V_3 ) )
return 0 ;
V_58 = (struct V_57 * ) V_6 -> V_7 . V_8 -> V_60 ;
V_58 -> V_61 = 0 ;
V_58 -> V_62 = V_59 ;
F_34 ( V_6 -> V_7 . V_8 , V_63 , V_56 ,
& V_6 -> V_3 , F_7 ( & V_6 -> V_3 ) ) ;
V_6 -> V_7 . V_8 = NULL ;
}
F_4 ( V_6 ) ;
return V_59 ;
}
static inline void F_39 ( struct V_57 * V_64 ,
const struct V_57 * V_65 )
{
* V_64 = * V_65 ;
}
static int F_40 ( const struct V_57 * V_65 , int V_23 , struct V_2 * V_3 )
{
struct V_41 * V_8 = F_41 ( sizeof( struct V_57 ) + 16 , V_33 ) ;
struct V_57 * V_58 ;
if ( ! V_8 )
return - V_44 ;
V_23 = - V_23 ;
if ( V_23 == V_66 ||
V_23 == V_67 ||
V_23 == V_68 )
V_23 = V_69 ;
if ( V_23 >= 512 )
V_23 = V_70 ;
F_42 ( V_23 <= 0 || V_23 >= 256 ) ;
V_58 = (struct V_57 * ) F_43 ( V_8 , sizeof( struct V_57 ) ) ;
F_39 ( V_58 , V_65 ) ;
V_58 -> V_62 = ( V_71 ) V_23 ;
V_58 -> V_72 = ( sizeof( struct V_57 ) /
sizeof( V_73 ) ) ;
F_34 ( V_8 , V_33 , V_56 , V_3 , F_7 ( V_3 ) ) ;
return 0 ;
}
static int F_44 ( const void * V_74 )
{
const struct V_75 * V_76 = V_74 ;
const struct V_77 * V_78 = ( const struct V_77 * ) ( V_76 + 1 ) ;
const struct V_79 * sin ;
#if F_45 ( V_80 )
const struct V_81 * V_82 ;
#endif
int V_83 ;
switch ( V_78 -> V_84 ) {
case V_85 :
V_83 = F_46 ( sizeof( * V_76 ) + sizeof( * sin ) , sizeof( V_73 ) ) ;
if ( V_76 -> V_86 != V_83 ||
V_76 -> V_87 > 32 )
return - V_70 ;
break;
#if F_45 ( V_80 )
case V_88 :
V_83 = F_46 ( sizeof( * V_76 ) + sizeof( * V_82 ) , sizeof( V_73 ) ) ;
if ( V_76 -> V_86 != V_83 ||
V_76 -> V_87 > 128 )
return - V_70 ;
break;
#endif
default:
return - V_70 ;
break;
}
return 0 ;
}
static inline int F_47 ( const struct V_89 * V_90 )
{
return F_46 ( sizeof( struct V_89 ) +
V_90 -> V_91 ,
sizeof( V_73 ) ) ;
}
static inline int F_48 ( const void * V_74 )
{
const struct V_89 * V_90 = V_74 ;
int V_83 = V_90 -> V_91 ;
if ( V_83 > V_92 )
return - V_70 ;
V_83 = F_47 ( V_90 ) ;
if ( V_90 -> V_93 != V_83 )
return - V_70 ;
return 0 ;
}
static inline struct V_94 * F_49 ( const struct V_89 * V_90 ,
T_1 V_95 )
{
struct V_94 * V_96 = NULL ;
int V_97 = V_90 -> V_91 ;
V_96 = F_50 ( ( sizeof( * V_96 ) + V_97 ) , V_95 ) ;
if ( ! V_96 )
return NULL ;
V_96 -> V_83 = F_47 ( V_90 ) ;
V_96 -> V_98 = V_90 -> V_99 ;
V_96 -> V_100 = V_90 -> V_101 ;
V_96 -> V_102 = V_90 -> V_103 ;
V_96 -> V_104 = V_90 -> V_91 ;
memcpy ( V_96 + 1 , V_90 + 1 ,
V_96 -> V_104 ) ;
return V_96 ;
}
static int F_51 ( const struct V_75 * V_105 ,
const struct V_75 * V_106 )
{
const struct V_77 * V_107 , * V_108 ;
if ( ! V_105 || ! V_106 )
return 0 ;
V_107 = ( const struct V_77 * ) ( V_105 + 1 ) ;
V_108 = ( const struct V_77 * ) ( V_106 + 1 ) ;
if ( V_107 -> V_84 != V_108 -> V_84 )
return 0 ;
if ( V_107 -> V_84 != V_85
#if F_45 ( V_80 )
&& V_107 -> V_84 != V_88
#endif
)
return 0 ;
return 1 ;
}
static int F_52 ( struct V_41 * V_8 , const struct V_57 * V_58 , void * * V_109 )
{
const char * V_74 = ( char * ) V_58 ;
int V_83 = V_8 -> V_83 ;
V_83 -= sizeof( * V_58 ) ;
V_74 += sizeof( * V_58 ) ;
while ( V_83 > 0 ) {
const struct V_110 * V_111 = ( const struct V_110 * ) V_74 ;
T_2 V_112 ;
int V_113 ;
V_113 = V_111 -> V_114 ;
V_113 *= sizeof( V_73 ) ;
V_112 = V_111 -> V_115 ;
if ( V_113 < sizeof( V_73 ) ||
V_113 > V_83 ||
V_112 == V_116 )
return - V_70 ;
if ( V_112 <= V_117 ) {
int V_118 = ( int ) V_119 [ V_112 ] ;
if ( V_113 < V_118 )
return - V_70 ;
if ( V_109 [ V_112 - 1 ] != NULL )
return - V_70 ;
if ( V_112 == V_120 ||
V_112 == V_121 ||
V_112 == V_122 ||
V_112 == V_123 ) {
if ( F_44 ( V_74 ) )
return - V_70 ;
}
if ( V_112 == V_124 ) {
if ( F_48 ( V_74 ) )
return - V_70 ;
}
V_109 [ V_112 - 1 ] = ( void * ) V_74 ;
}
V_74 += V_113 ;
V_83 -= V_113 ;
}
return 0 ;
}
static T_2
F_53 ( V_71 V_125 )
{
switch ( V_125 ) {
case V_126 :
return V_127 ;
case V_128 :
return V_129 ;
case V_130 :
return V_131 ;
case V_132 :
return V_133 ;
break;
default:
return 0 ;
}
}
static V_71
F_54 ( T_2 V_134 )
{
switch ( V_134 ) {
case V_129 :
return V_128 ;
case V_131 :
return V_130 ;
case V_133 :
return V_132 ;
break;
default:
return 0 ;
}
}
static V_71 F_55 ( V_71 V_134 )
{
return V_134 == V_127 ? 0 : V_134 ;
}
static V_71 F_56 ( V_71 V_134 )
{
return V_134 ? V_134 : V_127 ;
}
static inline int F_57 ( T_3 V_135 )
{
switch ( V_135 ) {
case V_85 :
return sizeof( struct V_79 ) ;
#if F_45 ( V_80 )
case V_88 :
return sizeof( struct V_81 ) ;
#endif
}
return 0 ;
}
static
int F_58 ( const struct V_77 * V_136 , T_4 * V_137 )
{
switch ( V_136 -> V_84 ) {
case V_85 :
V_137 -> V_138 =
( (struct V_79 * ) V_136 ) -> V_139 . V_107 ;
return V_85 ;
#if F_45 ( V_80 )
case V_88 :
memcpy ( V_137 -> V_140 ,
& ( (struct V_81 * ) V_136 ) -> V_141 ,
sizeof( struct V_142 ) ) ;
return V_88 ;
#endif
}
return 0 ;
}
static
int F_59 ( const struct V_75 * V_78 , T_4 * V_137 )
{
return F_58 ( (struct V_77 * ) ( V_78 + 1 ) ,
V_137 ) ;
}
static struct V_143 * F_60 ( struct V_10 * V_10 , const struct V_57 * V_58 , void * const * V_109 )
{
const struct V_144 * V_136 ;
const struct V_75 * V_78 ;
T_2 V_134 ;
unsigned short V_135 ;
T_4 * V_137 ;
V_136 = V_109 [ V_145 - 1 ] ;
if ( V_136 == NULL )
return NULL ;
V_134 = F_53 ( V_58 -> V_146 ) ;
if ( V_134 == 0 )
return NULL ;
V_78 = V_109 [ V_121 - 1 ] ;
if ( V_78 == NULL )
return NULL ;
V_135 = ( ( const struct V_77 * ) ( V_78 + 1 ) ) -> V_84 ;
switch ( V_135 ) {
case V_85 :
V_137 = ( T_4 * ) & ( ( const struct V_79 * ) ( V_78 + 1 ) ) -> V_139 ;
break;
#if F_45 ( V_80 )
case V_88 :
V_137 = ( T_4 * ) & ( ( const struct V_81 * ) ( V_78 + 1 ) ) -> V_141 ;
break;
#endif
default:
V_137 = NULL ;
}
if ( ! V_137 )
return NULL ;
return F_61 ( V_10 , V_147 , V_137 , V_136 -> V_148 , V_134 , V_135 ) ;
}
static int
F_62 ( T_3 V_135 )
{
return F_63 ( F_57 ( V_135 ) ) ;
}
static inline int F_64 ( int V_149 )
{
switch( V_149 ) {
case V_150 :
return V_151 ;
case V_152 :
return V_153 ;
case V_154 :
return V_155 ;
default:
return - 1 ;
}
}
static inline int F_65 ( int V_149 )
{
switch( V_149 ) {
case V_156 :
case V_151 :
return V_150 ;
case V_153 :
return V_152 ;
case V_155 :
return V_154 ;
default:
return - 1 ;
}
}
static unsigned int F_66 ( const T_4 * V_137 , T_5 V_157 ,
struct V_77 * V_136 ,
unsigned short V_135 )
{
switch ( V_135 ) {
case V_85 :
{
struct V_79 * sin = (struct V_79 * ) V_136 ;
sin -> V_158 = V_85 ;
sin -> V_159 = V_157 ;
sin -> V_139 . V_107 = V_137 -> V_138 ;
memset ( sin -> V_160 , 0 , sizeof( sin -> V_160 ) ) ;
return 32 ;
}
#if F_45 ( V_80 )
case V_88 :
{
struct V_81 * V_82 = (struct V_81 * ) V_136 ;
V_82 -> V_161 = V_88 ;
V_82 -> V_162 = V_157 ;
V_82 -> V_163 = 0 ;
V_82 -> V_141 = * (struct V_142 * ) V_137 -> V_140 ;
V_82 -> V_164 = 0 ;
return 128 ;
}
#endif
}
return 0 ;
}
static struct V_41 * F_67 ( const struct V_143 * V_165 ,
int V_166 , int V_167 )
{
struct V_41 * V_8 ;
struct V_57 * V_58 ;
struct V_144 * V_136 ;
struct V_168 * V_169 ;
struct V_75 * V_78 ;
struct V_170 * V_171 ;
struct V_172 * V_173 ;
struct V_89 * V_90 ;
struct V_174 * V_175 ;
int V_97 = 0 ;
int V_176 ;
int V_177 = 0 ;
int V_178 = 0 ;
int V_179 ;
struct V_180 * V_181 = NULL ;
int V_149 ;
V_179 = F_62 ( V_165 -> V_182 . V_135 ) ;
if ( ! V_179 )
return F_68 ( - V_70 ) ;
V_176 = sizeof( struct V_57 ) + sizeof( struct V_144 ) +
sizeof( struct V_168 ) +
( ( V_167 & 1 ) ? sizeof( struct V_168 ) : 0 ) +
( ( V_167 & 2 ) ? sizeof( struct V_168 ) : 0 ) +
sizeof( struct V_75 ) * 2 +
V_179 * 2 +
sizeof( struct V_172 ) ;
if ( ( V_175 = V_165 -> V_183 ) ) {
V_97 = F_63 ( V_175 -> V_104 ) ;
V_176 += sizeof( struct V_89 ) + V_97 ;
}
if ( ! F_69 ( & V_165 -> V_184 . V_185 , & V_165 -> V_182 . V_185 , V_165 -> V_182 . V_135 ) )
V_176 += sizeof( struct V_75 ) + V_179 ;
if ( V_166 ) {
if ( V_165 -> V_186 && V_165 -> V_186 -> V_187 ) {
V_177 =
F_63 ( ( V_165 -> V_186 -> V_187 + 7 ) / 8 ) ;
V_176 += sizeof( struct V_170 ) + V_177 ;
}
if ( V_165 -> V_188 && V_165 -> V_188 -> V_187 ) {
V_178 =
F_63 ( ( V_165 -> V_188 -> V_187 + 7 ) / 8 ) ;
V_176 += sizeof( struct V_170 ) + V_178 ;
}
}
if ( V_165 -> V_189 )
V_181 = V_165 -> V_189 ;
if ( V_181 && V_181 -> V_190 ) {
V_176 += sizeof( struct V_191 ) ;
V_176 += sizeof( struct V_192 ) ;
V_176 += sizeof( struct V_192 ) ;
}
V_8 = F_41 ( V_176 + 16 , V_63 ) ;
if ( V_8 == NULL )
return F_68 ( - V_44 ) ;
V_58 = (struct V_57 * ) F_43 ( V_8 , sizeof( struct V_57 ) ) ;
memset ( V_58 , 0 , V_176 ) ;
V_58 -> V_72 = V_176 / sizeof( V_73 ) ;
V_136 = (struct V_144 * ) F_43 ( V_8 , sizeof( struct V_144 ) ) ;
V_136 -> V_193 = sizeof( struct V_144 ) / sizeof( V_73 ) ;
V_136 -> V_194 = V_145 ;
V_136 -> V_148 = V_165 -> V_195 . V_196 ;
V_136 -> V_197 = V_165 -> V_182 . V_198 ;
switch ( V_165 -> V_199 . V_200 ) {
case V_201 :
V_136 -> V_202 = V_165 -> V_199 . V_203 ?
V_204 : V_205 ;
break;
case V_206 :
V_136 -> V_202 = V_207 ;
break;
default:
V_136 -> V_202 = V_208 ;
break;
}
V_136 -> V_209 = 0 ;
if ( V_165 -> V_186 ) {
struct V_210 * V_211 = F_70 ( V_165 -> V_186 -> V_212 , 0 ) ;
V_136 -> V_209 = ( V_211 && V_211 -> V_213 ) ?
V_211 -> V_214 . V_215 : 0 ;
}
V_136 -> V_216 = 0 ;
F_42 ( V_165 -> V_188 && V_165 -> V_217 ) ;
if ( V_165 -> V_188 ) {
struct V_210 * V_211 = F_71 ( V_165 -> V_188 -> V_212 , 0 ) ;
V_136 -> V_216 = ( V_211 && V_211 -> V_213 ) ?
V_211 -> V_214 . V_215 : 0 ;
}
if ( V_165 -> V_217 ) {
struct V_210 * V_211 = F_72 ( V_165 -> V_217 -> V_212 , 0 ) ;
V_136 -> V_216 = ( V_211 && V_211 -> V_213 ) ?
V_211 -> V_214 . V_215 : 0 ;
}
V_136 -> V_218 = 0 ;
if ( V_165 -> V_182 . V_219 & V_220 )
V_136 -> V_218 |= V_221 ;
if ( V_165 -> V_182 . V_219 & V_222 )
V_136 -> V_218 |= V_223 ;
if ( V_165 -> V_182 . V_219 & V_224 )
V_136 -> V_218 |= V_225 ;
if ( V_167 & 2 ) {
V_169 = (struct V_168 * ) F_43 ( V_8 ,
sizeof( struct V_168 ) ) ;
V_169 -> V_226 =
sizeof( struct V_168 ) / sizeof( V_73 ) ;
V_169 -> V_227 = V_228 ;
V_169 -> V_229 = F_73 ( V_165 -> V_230 . V_231 ) ;
V_169 -> V_232 = F_73 ( V_165 -> V_230 . V_233 ) ;
V_169 -> V_234 = V_165 -> V_230 . V_235 ;
V_169 -> V_236 = V_165 -> V_230 . V_237 ;
}
if ( V_167 & 1 ) {
V_169 = (struct V_168 * ) F_43 ( V_8 ,
sizeof( struct V_168 ) ) ;
V_169 -> V_226 =
sizeof( struct V_168 ) / sizeof( V_73 ) ;
V_169 -> V_227 = V_238 ;
V_169 -> V_229 = F_73 ( V_165 -> V_230 . V_239 ) ;
V_169 -> V_232 = F_73 ( V_165 -> V_230 . V_240 ) ;
V_169 -> V_234 = V_165 -> V_230 . V_241 ;
V_169 -> V_236 = V_165 -> V_230 . V_242 ;
}
V_169 = (struct V_168 * ) F_43 ( V_8 ,
sizeof( struct V_168 ) ) ;
V_169 -> V_226 =
sizeof( struct V_168 ) / sizeof( V_73 ) ;
V_169 -> V_227 = V_243 ;
V_169 -> V_229 = V_165 -> V_244 . V_245 ;
V_169 -> V_232 = V_165 -> V_244 . V_246 ;
V_169 -> V_234 = V_165 -> V_244 . V_247 ;
V_169 -> V_236 = V_165 -> V_244 . V_248 ;
V_78 = (struct V_75 * ) F_43 ( V_8 ,
sizeof( struct V_75 ) + V_179 ) ;
V_78 -> V_86 =
( sizeof( struct V_75 ) + V_179 ) /
sizeof( V_73 ) ;
V_78 -> V_249 = V_120 ;
V_78 -> V_250 = 0 ;
V_78 -> V_251 = 0 ;
V_78 -> V_87 =
F_66 ( & V_165 -> V_182 . V_185 , 0 ,
(struct V_77 * ) ( V_78 + 1 ) ,
V_165 -> V_182 . V_135 ) ;
if ( ! V_78 -> V_87 )
F_74 () ;
V_78 = (struct V_75 * ) F_43 ( V_8 ,
sizeof( struct V_75 ) + V_179 ) ;
V_78 -> V_86 =
( sizeof( struct V_75 ) + V_179 ) /
sizeof( V_73 ) ;
V_78 -> V_249 = V_121 ;
V_78 -> V_250 = 0 ;
V_78 -> V_251 = 0 ;
V_78 -> V_87 =
F_66 ( & V_165 -> V_195 . V_252 , 0 ,
(struct V_77 * ) ( V_78 + 1 ) ,
V_165 -> V_182 . V_135 ) ;
if ( ! V_78 -> V_87 )
F_74 () ;
if ( ! F_69 ( & V_165 -> V_184 . V_185 , & V_165 -> V_182 . V_185 ,
V_165 -> V_182 . V_135 ) ) {
V_78 = (struct V_75 * ) F_43 ( V_8 ,
sizeof( struct V_75 ) + V_179 ) ;
V_78 -> V_86 =
( sizeof( struct V_75 ) + V_179 ) /
sizeof( V_73 ) ;
V_78 -> V_249 = V_122 ;
V_78 -> V_250 =
F_56 ( V_165 -> V_184 . V_134 ) ;
V_78 -> V_87 = V_165 -> V_184 . V_253 ;
V_78 -> V_251 = 0 ;
F_66 ( & V_165 -> V_184 . V_185 , V_165 -> V_184 . V_254 ,
(struct V_77 * ) ( V_78 + 1 ) ,
V_165 -> V_182 . V_135 ) ;
}
if ( V_166 && V_177 ) {
V_171 = (struct V_170 * ) F_43 ( V_8 ,
sizeof( struct V_170 ) + V_177 ) ;
V_171 -> V_255 = ( sizeof( struct V_170 ) + V_177 ) /
sizeof( V_73 ) ;
V_171 -> V_256 = V_257 ;
V_171 -> V_258 = V_165 -> V_186 -> V_187 ;
V_171 -> V_259 = 0 ;
memcpy ( V_171 + 1 , V_165 -> V_186 -> V_260 , ( V_165 -> V_186 -> V_187 + 7 ) / 8 ) ;
}
if ( V_166 && V_178 ) {
V_171 = (struct V_170 * ) F_43 ( V_8 ,
sizeof( struct V_170 ) + V_178 ) ;
V_171 -> V_255 = ( sizeof( struct V_170 ) +
V_178 ) / sizeof( V_73 ) ;
V_171 -> V_256 = V_261 ;
V_171 -> V_258 = V_165 -> V_188 -> V_187 ;
V_171 -> V_259 = 0 ;
memcpy ( V_171 + 1 , V_165 -> V_188 -> V_260 ,
( V_165 -> V_188 -> V_187 + 7 ) / 8 ) ;
}
V_173 = (struct V_172 * ) F_43 ( V_8 , sizeof( struct V_172 ) ) ;
V_173 -> V_262 = sizeof( struct V_172 ) / sizeof( V_73 ) ;
V_173 -> V_263 = V_264 ;
if ( ( V_149 = F_64 ( V_165 -> V_182 . V_149 ) ) < 0 ) {
F_5 ( V_8 ) ;
return F_68 ( - V_70 ) ;
}
V_173 -> V_265 = V_149 ;
V_173 -> V_266 = 0 ;
V_173 -> V_267 = 0 ;
V_173 -> V_268 = 0 ;
V_173 -> V_269 = V_165 -> V_182 . V_270 ;
if ( V_181 && V_181 -> V_190 ) {
struct V_191 * V_271 ;
struct V_192 * V_272 ;
V_271 = (struct V_191 * ) F_43 ( V_8 , sizeof( * V_271 ) ) ;
V_271 -> V_273 = sizeof( * V_271 ) / sizeof( V_73 ) ;
V_271 -> V_274 = V_275 ;
V_271 -> V_276 = V_181 -> V_190 ;
V_271 -> V_277 [ 0 ] = 0 ;
V_271 -> V_277 [ 1 ] = 0 ;
V_271 -> V_277 [ 2 ] = 0 ;
V_272 = (struct V_192 * ) F_43 ( V_8 , sizeof ( * V_272 ) ) ;
V_272 -> V_278 = sizeof( * V_272 ) / sizeof( V_73 ) ;
V_272 -> V_279 = V_280 ;
V_272 -> V_281 = V_181 -> V_282 ;
V_272 -> V_283 = 0 ;
V_272 = (struct V_192 * ) F_43 ( V_8 , sizeof ( * V_272 ) ) ;
V_272 -> V_278 = sizeof( * V_272 ) / sizeof( V_73 ) ;
V_272 -> V_279 = V_284 ;
V_272 -> V_281 = V_181 -> V_285 ;
V_272 -> V_283 = 0 ;
}
if ( V_175 ) {
V_90 = (struct V_89 * ) F_43 ( V_8 ,
sizeof( struct V_89 ) + V_97 ) ;
V_90 -> V_93 =
( sizeof( struct V_89 ) + V_97 ) / sizeof( V_73 ) ;
V_90 -> V_99 = V_124 ;
V_90 -> V_101 = V_175 -> V_100 ;
V_90 -> V_103 = V_175 -> V_102 ;
V_90 -> V_91 = V_175 -> V_104 ;
memcpy ( V_90 + 1 , V_175 -> V_286 ,
V_175 -> V_104 ) ;
}
return V_8 ;
}
static inline struct V_41 * F_75 ( const struct V_143 * V_165 )
{
struct V_41 * V_8 ;
V_8 = F_67 ( V_165 , 1 , 3 ) ;
return V_8 ;
}
static inline struct V_41 * F_76 ( const struct V_143 * V_165 ,
int V_167 )
{
return F_67 ( V_165 , 0 , V_167 ) ;
}
static struct V_143 * F_77 ( struct V_10 * V_10 ,
const struct V_57 * V_58 ,
void * const * V_109 )
{
struct V_143 * V_165 ;
const struct V_168 * V_169 ;
const struct V_144 * V_136 ;
const struct V_170 * V_171 ;
const struct V_89 * V_90 ;
T_2 V_134 ;
int V_23 ;
V_136 = V_109 [ V_145 - 1 ] ;
if ( ! V_136 ||
! F_51 ( V_109 [ V_120 - 1 ] ,
V_109 [ V_121 - 1 ] ) )
return F_68 ( - V_70 ) ;
if ( V_58 -> V_146 == V_130 &&
! V_109 [ V_261 - 1 ] )
return F_68 ( - V_70 ) ;
if ( V_58 -> V_146 == V_128 &&
! V_109 [ V_257 - 1 ] )
return F_68 ( - V_70 ) ;
if ( ! ! V_109 [ V_228 - 1 ] !=
! ! V_109 [ V_238 - 1 ] )
return F_68 ( - V_70 ) ;
V_134 = F_53 ( V_58 -> V_146 ) ;
if ( V_134 == 0 )
return F_68 ( - V_70 ) ;
V_23 = - V_44 ;
if ( V_136 -> V_209 > V_287 ||
( V_58 -> V_146 == V_132 &&
V_136 -> V_216 > V_288 ) ||
V_136 -> V_216 > V_289 )
return F_68 ( - V_70 ) ;
V_171 = V_109 [ V_257 - 1 ] ;
if ( V_171 != NULL &&
V_136 -> V_209 != V_290 &&
( ( V_171 -> V_258 + 7 ) / 8 == 0 ||
( V_171 -> V_258 + 7 ) / 8 > V_171 -> V_255 * sizeof( V_73 ) ) )
return F_68 ( - V_70 ) ;
V_171 = V_109 [ V_261 - 1 ] ;
if ( V_171 != NULL &&
V_136 -> V_216 != V_291 &&
( ( V_171 -> V_258 + 7 ) / 8 == 0 ||
( V_171 -> V_258 + 7 ) / 8 > V_171 -> V_255 * sizeof( V_73 ) ) )
return F_68 ( - V_70 ) ;
V_165 = F_78 ( V_10 ) ;
if ( V_165 == NULL )
return F_68 ( - V_44 ) ;
V_165 -> V_195 . V_134 = V_134 ;
V_165 -> V_195 . V_196 = V_136 -> V_148 ;
V_165 -> V_182 . V_198 = F_79 (unsigned int, sa->sadb_sa_replay,
(sizeof(x->replay.bitmap) * 8)) ;
if ( V_136 -> V_218 & V_221 )
V_165 -> V_182 . V_219 |= V_220 ;
if ( V_136 -> V_218 & V_223 )
V_165 -> V_182 . V_219 |= V_222 ;
if ( V_136 -> V_218 & V_225 )
V_165 -> V_182 . V_219 |= V_224 ;
V_169 = V_109 [ V_228 - 1 ] ;
if ( V_169 != NULL ) {
V_165 -> V_230 . V_231 = F_80 ( V_169 -> V_229 ) ;
V_165 -> V_230 . V_233 = F_80 ( V_169 -> V_232 ) ;
V_165 -> V_230 . V_235 = V_169 -> V_234 ;
V_165 -> V_230 . V_237 = V_169 -> V_236 ;
}
V_169 = V_109 [ V_238 - 1 ] ;
if ( V_169 != NULL ) {
V_165 -> V_230 . V_239 = F_80 ( V_169 -> V_229 ) ;
V_165 -> V_230 . V_240 = F_80 ( V_169 -> V_232 ) ;
V_165 -> V_230 . V_241 = V_169 -> V_234 ;
V_165 -> V_230 . V_242 = V_169 -> V_236 ;
}
V_90 = V_109 [ V_124 - 1 ] ;
if ( V_90 != NULL ) {
struct V_94 * V_96 = F_49 ( V_90 , V_33 ) ;
if ( ! V_96 )
goto V_35;
V_23 = F_81 ( V_165 , V_96 ) ;
F_82 ( V_96 ) ;
if ( V_23 )
goto V_35;
}
V_171 = V_109 [ V_257 - 1 ] ;
if ( V_136 -> V_209 ) {
int V_292 = 0 ;
struct V_210 * V_211 = F_83 ( V_136 -> V_209 ) ;
if ( ! V_211 || ! V_211 -> V_213 ) {
V_23 = - V_293 ;
goto V_35;
}
if ( V_171 )
V_292 = ( V_171 -> V_258 + 7 ) / 8 ;
V_165 -> V_186 = F_50 ( sizeof( * V_165 -> V_186 ) + V_292 , V_33 ) ;
if ( ! V_165 -> V_186 )
goto V_35;
strcpy ( V_165 -> V_186 -> V_212 , V_211 -> V_294 ) ;
V_165 -> V_186 -> V_187 = 0 ;
if ( V_171 ) {
V_165 -> V_186 -> V_187 = V_171 -> V_258 ;
memcpy ( V_165 -> V_186 -> V_260 , V_171 + 1 , V_292 ) ;
}
V_165 -> V_186 -> V_295 = V_211 -> V_296 . V_297 . V_298 ;
V_165 -> V_182 . V_299 = V_136 -> V_209 ;
}
if ( V_136 -> V_216 ) {
if ( V_58 -> V_146 == V_132 ) {
struct V_210 * V_211 = F_84 ( V_136 -> V_216 ) ;
if ( ! V_211 || ! V_211 -> V_213 ) {
V_23 = - V_293 ;
goto V_35;
}
V_165 -> V_217 = F_50 ( sizeof( * V_165 -> V_217 ) , V_33 ) ;
if ( ! V_165 -> V_217 )
goto V_35;
strcpy ( V_165 -> V_217 -> V_212 , V_211 -> V_294 ) ;
V_165 -> V_182 . V_300 = V_136 -> V_216 ;
} else {
int V_292 = 0 ;
struct V_210 * V_211 = F_85 ( V_136 -> V_216 ) ;
if ( ! V_211 || ! V_211 -> V_213 ) {
V_23 = - V_293 ;
goto V_35;
}
V_171 = (struct V_170 * ) V_109 [ V_261 - 1 ] ;
if ( V_171 )
V_292 = ( V_171 -> V_258 + 7 ) / 8 ;
V_165 -> V_188 = F_50 ( sizeof( * V_165 -> V_188 ) + V_292 , V_33 ) ;
if ( ! V_165 -> V_188 )
goto V_35;
strcpy ( V_165 -> V_188 -> V_212 , V_211 -> V_294 ) ;
V_165 -> V_188 -> V_187 = 0 ;
if ( V_171 ) {
V_165 -> V_188 -> V_187 = V_171 -> V_258 ;
memcpy ( V_165 -> V_188 -> V_260 , V_171 + 1 , V_292 ) ;
}
V_165 -> V_182 . V_301 = V_136 -> V_216 ;
}
}
V_165 -> V_182 . V_135 = F_59 ( (struct V_75 * ) V_109 [ V_120 - 1 ] ,
& V_165 -> V_182 . V_185 ) ;
F_59 ( (struct V_75 * ) V_109 [ V_121 - 1 ] ,
& V_165 -> V_195 . V_252 ) ;
if ( V_109 [ V_264 - 1 ] ) {
const struct V_172 * V_173 = V_109 [ V_264 - 1 ] ;
int V_149 = F_65 ( V_173 -> V_265 ) ;
if ( V_149 < 0 ) {
V_23 = - V_70 ;
goto V_35;
}
V_165 -> V_182 . V_149 = V_149 ;
V_165 -> V_182 . V_270 = V_173 -> V_269 ;
}
if ( V_109 [ V_122 - 1 ] ) {
const struct V_75 * V_78 = V_109 [ V_122 - 1 ] ;
V_165 -> V_184 . V_135 = F_59 ( V_78 , & V_165 -> V_184 . V_185 ) ;
V_165 -> V_184 . V_253 = V_78 -> V_87 ;
}
if ( ! V_165 -> V_184 . V_135 )
V_165 -> V_184 . V_135 = V_165 -> V_182 . V_135 ;
if ( V_109 [ V_275 - 1 ] ) {
const struct V_191 * V_271 ;
struct V_180 * V_181 ;
V_165 -> V_189 = F_50 ( sizeof( * V_165 -> V_189 ) , V_33 ) ;
if ( ! V_165 -> V_189 )
goto V_35;
V_181 = V_165 -> V_189 ;
V_271 = V_109 [ V_275 - 1 ] ;
V_181 -> V_190 = V_271 -> V_276 ;
if ( V_109 [ V_280 - 1 ] ) {
const struct V_192 * V_272 =
V_109 [ V_280 - 1 ] ;
V_181 -> V_282 = V_272 -> V_281 ;
}
if ( V_109 [ V_284 - 1 ] ) {
const struct V_192 * V_272 =
V_109 [ V_284 - 1 ] ;
V_181 -> V_285 = V_272 -> V_281 ;
}
memset ( & V_181 -> V_302 , 0 , sizeof( V_181 -> V_302 ) ) ;
}
V_23 = F_86 ( V_165 ) ;
if ( V_23 )
goto V_35;
V_165 -> V_199 . V_303 = V_58 -> V_61 ;
return V_165 ;
V_35:
V_165 -> V_199 . V_200 = V_304 ;
F_87 ( V_165 ) ;
return F_68 ( V_23 ) ;
}
static int F_88 ( struct V_2 * V_3 , struct V_41 * V_8 , const struct V_57 * V_58 , void * const * V_109 )
{
return - V_305 ;
}
static int F_89 ( struct V_2 * V_3 , struct V_41 * V_8 , const struct V_57 * V_58 , void * const * V_109 )
{
struct V_10 * V_10 = F_7 ( V_3 ) ;
struct V_41 * V_306 ;
struct V_172 * V_173 ;
struct V_75 * V_185 , * V_252 ;
struct V_57 * V_307 ;
struct V_308 * V_309 ;
struct V_143 * V_165 = NULL ;
int V_149 ;
int V_23 ;
T_6 V_310 , V_311 ;
T_6 V_270 ;
T_7 V_134 ;
unsigned short V_135 ;
T_4 * V_312 = NULL , * V_313 = NULL ;
if ( ! F_51 ( V_109 [ V_120 - 1 ] ,
V_109 [ V_121 - 1 ] ) )
return - V_70 ;
V_134 = F_53 ( V_58 -> V_146 ) ;
if ( V_134 == 0 )
return - V_70 ;
if ( ( V_173 = V_109 [ V_264 - 1 ] ) != NULL ) {
V_149 = F_65 ( V_173 -> V_265 ) ;
if ( V_149 < 0 )
return - V_70 ;
V_270 = V_173 -> V_269 ;
} else {
V_149 = 0 ;
V_270 = 0 ;
}
V_185 = V_109 [ V_120 - 1 ] ;
V_252 = V_109 [ V_121 - 1 ] ;
V_135 = ( (struct V_77 * ) ( V_185 + 1 ) ) -> V_84 ;
switch ( V_135 ) {
case V_85 :
V_313 = ( T_4 * ) & ( (struct V_79 * ) ( V_252 + 1 ) ) -> V_139 . V_107 ;
V_312 = ( T_4 * ) & ( (struct V_79 * ) ( V_185 + 1 ) ) -> V_139 . V_107 ;
break;
#if F_45 ( V_80 )
case V_88 :
V_313 = ( T_4 * ) & ( (struct V_81 * ) ( V_252 + 1 ) ) -> V_141 ;
V_312 = ( T_4 * ) & ( (struct V_81 * ) ( V_185 + 1 ) ) -> V_141 ;
break;
#endif
}
if ( V_58 -> V_61 ) {
V_165 = F_90 ( V_10 , V_147 , V_58 -> V_61 ) ;
if ( V_165 && ! F_69 ( & V_165 -> V_195 . V_252 , V_313 , V_135 ) ) {
F_87 ( V_165 ) ;
V_165 = NULL ;
}
}
if ( ! V_165 )
V_165 = F_91 ( V_10 , & V_314 , V_149 , V_270 , V_134 , V_313 , V_312 , 1 , V_135 ) ;
if ( V_165 == NULL )
return - V_315 ;
V_310 = 0x100 ;
V_311 = 0x0fffffff ;
V_309 = V_109 [ V_316 - 1 ] ;
if ( V_309 ) {
V_310 = V_309 -> V_317 ;
V_311 = V_309 -> V_318 ;
}
V_23 = F_92 ( V_165 -> V_195 . V_134 , V_310 , V_311 ) ;
if ( V_23 ) {
F_87 ( V_165 ) ;
return V_23 ;
}
V_23 = F_93 ( V_165 , V_310 , V_311 ) ;
V_306 = V_23 ? F_68 ( V_23 ) : F_75 ( V_165 ) ;
if ( F_94 ( V_306 ) ) {
F_87 ( V_165 ) ;
return F_95 ( V_306 ) ;
}
V_307 = (struct V_57 * ) V_306 -> V_60 ;
V_307 -> V_319 = V_58 -> V_319 ;
V_307 -> V_320 = V_321 ;
V_307 -> V_146 = F_54 ( V_134 ) ;
V_307 -> V_62 = 0 ;
V_307 -> V_322 = 0 ;
V_307 -> V_61 = V_58 -> V_61 ;
V_307 -> V_323 = V_58 -> V_323 ;
F_87 ( V_165 ) ;
F_34 ( V_306 , V_33 , V_56 , V_3 , V_10 ) ;
return 0 ;
}
static int F_96 ( struct V_2 * V_3 , struct V_41 * V_8 , const struct V_57 * V_58 , void * const * V_109 )
{
struct V_10 * V_10 = F_7 ( V_3 ) ;
struct V_143 * V_165 ;
if ( V_58 -> V_72 != sizeof( struct V_57 ) / 8 )
return - V_305 ;
if ( V_58 -> V_61 == 0 || V_58 -> V_62 == 0 )
return 0 ;
V_165 = F_90 ( V_10 , V_147 , V_58 -> V_61 ) ;
if ( V_165 == NULL )
return 0 ;
F_97 ( & V_165 -> V_324 ) ;
if ( V_165 -> V_199 . V_200 == V_206 )
V_165 -> V_199 . V_200 = V_325 ;
F_98 ( & V_165 -> V_324 ) ;
F_87 ( V_165 ) ;
return 0 ;
}
static inline int F_99 ( int V_326 )
{
switch ( V_326 ) {
case V_327 :
return V_328 ;
case V_329 :
return V_330 ;
case V_331 :
return V_332 ;
case V_333 :
default:
F_11 ( L_2 , V_326 ) ;
break;
}
return 0 ;
}
static inline int F_100 ( int V_326 )
{
switch ( V_326 ) {
case V_334 :
return V_335 ;
case V_336 :
return V_337 ;
case V_338 :
return V_339 ;
case V_340 :
return V_341 ;
default:
F_11 ( L_3 , V_326 ) ;
break;
}
return 0 ;
}
static int F_101 ( struct V_143 * V_165 , const struct V_342 * V_343 )
{
struct V_41 * V_8 ;
struct V_57 * V_58 ;
V_8 = F_75 ( V_165 ) ;
if ( F_94 ( V_8 ) )
return F_95 ( V_8 ) ;
V_58 = (struct V_57 * ) V_8 -> V_60 ;
V_58 -> V_319 = V_29 ;
V_58 -> V_320 = F_100 ( V_343 -> V_326 ) ;
V_58 -> V_146 = F_54 ( V_165 -> V_195 . V_134 ) ;
V_58 -> V_62 = 0 ;
V_58 -> V_322 = 0 ;
V_58 -> V_61 = V_343 -> V_303 ;
V_58 -> V_323 = V_343 -> V_344 ;
F_34 ( V_8 , V_63 , V_52 , NULL , F_102 ( V_165 ) ) ;
return 0 ;
}
static int F_103 ( struct V_2 * V_3 , struct V_41 * V_8 , const struct V_57 * V_58 , void * const * V_109 )
{
struct V_10 * V_10 = F_7 ( V_3 ) ;
struct V_143 * V_165 ;
int V_23 ;
struct V_342 V_343 ;
V_165 = F_77 ( V_10 , V_58 , V_109 ) ;
if ( F_94 ( V_165 ) )
return F_95 ( V_165 ) ;
F_104 ( V_165 ) ;
if ( V_58 -> V_320 == V_337 )
V_23 = F_105 ( V_165 ) ;
else
V_23 = F_106 ( V_165 ) ;
F_107 ( V_165 , V_23 ? 0 : 1 ,
F_108 ( V_345 ) ,
F_109 ( V_345 ) , 0 ) ;
if ( V_23 < 0 ) {
V_165 -> V_199 . V_200 = V_304 ;
F_110 ( V_165 ) ;
goto V_35;
}
if ( V_58 -> V_320 == V_337 )
V_343 . V_326 = V_336 ;
else
V_343 . V_326 = V_338 ;
V_343 . V_303 = V_58 -> V_61 ;
V_343 . V_344 = V_58 -> V_323 ;
F_111 ( V_165 , & V_343 ) ;
V_35:
F_87 ( V_165 ) ;
return V_23 ;
}
static int F_112 ( struct V_2 * V_3 , struct V_41 * V_8 , const struct V_57 * V_58 , void * const * V_109 )
{
struct V_10 * V_10 = F_7 ( V_3 ) ;
struct V_143 * V_165 ;
struct V_342 V_343 ;
int V_23 ;
if ( ! V_109 [ V_145 - 1 ] ||
! F_51 ( V_109 [ V_120 - 1 ] ,
V_109 [ V_121 - 1 ] ) )
return - V_70 ;
V_165 = F_60 ( V_10 , V_58 , V_109 ) ;
if ( V_165 == NULL )
return - V_49 ;
if ( ( V_23 = F_113 ( V_165 ) ) )
goto V_35;
if ( F_114 ( V_165 ) ) {
V_23 = - V_26 ;
goto V_35;
}
V_23 = F_115 ( V_165 ) ;
if ( V_23 < 0 )
goto V_35;
V_343 . V_303 = V_58 -> V_61 ;
V_343 . V_344 = V_58 -> V_323 ;
V_343 . V_326 = V_334 ;
F_111 ( V_165 , & V_343 ) ;
V_35:
F_116 ( V_165 , V_23 ? 0 : 1 ,
F_108 ( V_345 ) ,
F_109 ( V_345 ) , 0 ) ;
F_87 ( V_165 ) ;
return V_23 ;
}
static int F_117 ( struct V_2 * V_3 , struct V_41 * V_8 , const struct V_57 * V_58 , void * const * V_109 )
{
struct V_10 * V_10 = F_7 ( V_3 ) ;
T_8 V_134 ;
struct V_41 * V_346 ;
struct V_57 * V_307 ;
struct V_143 * V_165 ;
if ( ! V_109 [ V_145 - 1 ] ||
! F_51 ( V_109 [ V_120 - 1 ] ,
V_109 [ V_121 - 1 ] ) )
return - V_70 ;
V_165 = F_60 ( V_10 , V_58 , V_109 ) ;
if ( V_165 == NULL )
return - V_49 ;
V_346 = F_75 ( V_165 ) ;
V_134 = V_165 -> V_195 . V_134 ;
F_87 ( V_165 ) ;
if ( F_94 ( V_346 ) )
return F_95 ( V_346 ) ;
V_307 = (struct V_57 * ) V_346 -> V_60 ;
V_307 -> V_319 = V_58 -> V_319 ;
V_307 -> V_320 = V_347 ;
V_307 -> V_146 = F_54 ( V_134 ) ;
V_307 -> V_62 = 0 ;
V_307 -> V_322 = 0 ;
V_307 -> V_61 = V_58 -> V_61 ;
V_307 -> V_323 = V_58 -> V_323 ;
F_34 ( V_346 , V_63 , V_56 , V_3 , F_7 ( V_3 ) ) ;
return 0 ;
}
static struct V_41 * F_118 ( const struct V_57 * V_65 ,
T_1 V_43 )
{
struct V_41 * V_8 ;
struct V_57 * V_58 ;
int V_83 , V_348 , V_349 , V_350 ;
V_348 = F_119 () ;
if ( V_348 ) {
V_348 *= sizeof( struct V_351 ) ;
V_348 += sizeof( struct V_352 ) ;
}
V_349 = F_120 () ;
if ( V_349 ) {
V_349 *= sizeof( struct V_351 ) ;
V_349 += sizeof( struct V_352 ) ;
}
V_83 = V_349 + V_348 + sizeof( struct V_57 ) ;
V_8 = F_41 ( V_83 + 16 , V_43 ) ;
if ( ! V_8 )
goto V_353;
V_58 = (struct V_57 * ) F_43 ( V_8 , sizeof( * V_58 ) ) ;
F_39 ( V_58 , V_65 ) ;
V_58 -> V_62 = 0 ;
V_58 -> V_72 = V_83 / sizeof( V_73 ) ;
if ( V_348 ) {
struct V_352 * V_76 ;
struct V_351 * V_354 ;
V_76 = (struct V_352 * ) F_43 ( V_8 , V_348 ) ;
V_354 = (struct V_351 * ) ( V_76 + 1 ) ;
V_76 -> V_355 = V_348 / sizeof( V_73 ) ;
V_76 -> V_356 = V_357 ;
for ( V_350 = 0 ; ; V_350 ++ ) {
struct V_210 * V_186 = F_121 ( V_350 ) ;
if ( ! V_186 )
break;
if ( ! V_186 -> V_213 )
continue;
if ( V_186 -> V_358 )
* V_354 ++ = V_186 -> V_214 ;
}
}
if ( V_349 ) {
struct V_352 * V_76 ;
struct V_351 * V_354 ;
V_76 = (struct V_352 * ) F_43 ( V_8 , V_349 ) ;
V_354 = (struct V_351 * ) ( V_76 + 1 ) ;
V_76 -> V_355 = V_349 / sizeof( V_73 ) ;
V_76 -> V_356 = V_359 ;
for ( V_350 = 0 ; ; V_350 ++ ) {
struct V_210 * V_188 = F_122 ( V_350 ) ;
if ( ! V_188 )
break;
if ( ! V_188 -> V_213 )
continue;
if ( V_188 -> V_358 )
* V_354 ++ = V_188 -> V_214 ;
}
}
V_353:
return V_8 ;
}
static int F_123 ( struct V_2 * V_3 , struct V_41 * V_8 , const struct V_57 * V_58 , void * const * V_109 )
{
struct V_1 * V_6 = F_1 ( V_3 ) ;
struct V_41 * V_360 ;
if ( V_58 -> V_146 > V_361 )
return - V_70 ;
if ( V_58 -> V_146 != V_126 ) {
if ( V_6 -> V_55 & ( 1 << V_58 -> V_146 ) )
return - V_362 ;
V_6 -> V_55 |= ( 1 << V_58 -> V_146 ) ;
}
F_124 () ;
V_360 = F_118 ( V_58 , V_33 ) ;
if ( ! V_360 ) {
if ( V_58 -> V_146 != V_126 )
V_6 -> V_55 &= ~ ( 1 << V_58 -> V_146 ) ;
return - V_44 ;
}
F_34 ( V_360 , V_33 , V_54 , V_3 , F_7 ( V_3 ) ) ;
return 0 ;
}
static int F_125 ( struct V_2 * V_3 , const struct V_57 * V_363 )
{
struct V_41 * V_8 ;
struct V_57 * V_58 ;
V_8 = F_41 ( sizeof( struct V_57 ) + 16 , V_63 ) ;
if ( ! V_8 )
return - V_44 ;
V_58 = (struct V_57 * ) F_43 ( V_8 , sizeof( struct V_57 ) ) ;
memcpy ( V_58 , V_363 , sizeof( struct V_57 ) ) ;
V_58 -> V_62 = ( V_71 ) 0 ;
V_58 -> V_72 = ( sizeof( struct V_57 ) / sizeof( V_73 ) ) ;
return F_34 ( V_8 , V_63 , V_56 , V_3 , F_7 ( V_3 ) ) ;
}
static int F_126 ( const struct V_342 * V_343 )
{
struct V_41 * V_8 ;
struct V_57 * V_58 ;
V_8 = F_41 ( sizeof( struct V_57 ) + 16 , V_63 ) ;
if ( ! V_8 )
return - V_44 ;
V_58 = (struct V_57 * ) F_43 ( V_8 , sizeof( struct V_57 ) ) ;
V_58 -> V_146 = F_54 ( V_343 -> V_60 . V_134 ) ;
V_58 -> V_320 = V_364 ;
V_58 -> V_61 = V_343 -> V_303 ;
V_58 -> V_323 = V_343 -> V_344 ;
V_58 -> V_319 = V_29 ;
V_58 -> V_62 = ( V_71 ) 0 ;
V_58 -> V_72 = ( sizeof( struct V_57 ) / sizeof( V_73 ) ) ;
V_58 -> V_322 = 0 ;
F_34 ( V_8 , V_63 , V_52 , NULL , V_343 -> V_10 ) ;
return 0 ;
}
static int F_127 ( struct V_2 * V_3 , struct V_41 * V_8 , const struct V_57 * V_58 , void * const * V_109 )
{
struct V_10 * V_10 = F_7 ( V_3 ) ;
unsigned int V_134 ;
struct V_342 V_343 ;
struct V_365 V_366 ;
int V_23 , V_50 ;
V_134 = F_53 ( V_58 -> V_146 ) ;
if ( V_134 == 0 )
return - V_70 ;
V_366 . V_367 = F_108 ( V_345 ) ;
V_366 . V_368 = F_109 ( V_345 ) ;
V_366 . V_369 = 0 ;
V_23 = F_128 ( V_10 , V_134 , & V_366 ) ;
V_50 = F_125 ( V_3 , V_58 ) ;
if ( V_23 || V_50 ) {
if ( V_23 == - V_49 )
V_23 = 0 ;
return V_23 ? V_23 : V_50 ;
}
V_343 . V_60 . V_134 = V_134 ;
V_343 . V_303 = V_58 -> V_61 ;
V_343 . V_344 = V_58 -> V_323 ;
V_343 . V_326 = V_370 ;
V_343 . V_10 = V_10 ;
F_111 ( NULL , & V_343 ) ;
return 0 ;
}
static int F_129 ( struct V_143 * V_165 , int V_371 , void * V_372 )
{
struct V_1 * V_6 = V_372 ;
struct V_41 * V_346 ;
struct V_57 * V_307 ;
if ( ! F_2 ( & V_6 -> V_3 ) )
return - V_44 ;
V_346 = F_75 ( V_165 ) ;
if ( F_94 ( V_346 ) )
return F_95 ( V_346 ) ;
V_307 = (struct V_57 * ) V_346 -> V_60 ;
V_307 -> V_319 = V_6 -> V_7 . V_373 ;
V_307 -> V_320 = V_374 ;
V_307 -> V_146 = F_54 ( V_165 -> V_195 . V_134 ) ;
V_307 -> V_62 = 0 ;
V_307 -> V_322 = 0 ;
V_307 -> V_61 = V_371 + 1 ;
V_307 -> V_323 = V_6 -> V_7 . V_375 ;
if ( V_6 -> V_7 . V_8 )
F_34 ( V_6 -> V_7 . V_8 , V_63 , V_56 ,
& V_6 -> V_3 , F_7 ( & V_6 -> V_3 ) ) ;
V_6 -> V_7 . V_8 = V_346 ;
return 0 ;
}
static int F_130 ( struct V_1 * V_6 )
{
struct V_10 * V_10 = F_7 ( & V_6 -> V_3 ) ;
return F_131 ( V_10 , & V_6 -> V_7 . V_376 . V_200 , F_129 , ( void * ) V_6 ) ;
}
static void F_132 ( struct V_1 * V_6 )
{
struct V_10 * V_10 = F_7 ( & V_6 -> V_3 ) ;
F_133 ( & V_6 -> V_7 . V_376 . V_200 , V_10 ) ;
}
static int F_134 ( struct V_2 * V_3 , struct V_41 * V_8 , const struct V_57 * V_58 , void * const * V_109 )
{
T_7 V_134 ;
struct V_377 * V_378 = NULL ;
struct V_1 * V_6 = F_1 ( V_3 ) ;
if ( V_6 -> V_7 . V_7 != NULL )
return - V_379 ;
V_134 = F_53 ( V_58 -> V_146 ) ;
if ( V_134 == 0 )
return - V_70 ;
if ( V_109 [ V_380 - 1 ] ) {
struct V_381 * V_382 = V_109 [ V_380 - 1 ] ;
V_378 = F_50 ( sizeof( * V_378 ) , V_33 ) ;
if ( V_378 == NULL )
return - V_31 ;
memcpy ( & V_378 -> V_185 , & V_382 -> V_383 ,
sizeof( T_4 ) ) ;
memcpy ( & V_378 -> V_252 , & V_382 -> V_384 ,
sizeof( T_4 ) ) ;
V_378 -> V_135 = V_382 -> V_385 ;
V_378 -> V_386 = V_382 -> V_387 ;
V_378 -> V_388 = V_382 -> V_389 ;
}
V_6 -> V_7 . V_373 = V_58 -> V_319 ;
V_6 -> V_7 . V_375 = V_58 -> V_323 ;
V_6 -> V_7 . V_7 = F_130 ;
V_6 -> V_7 . V_9 = F_132 ;
F_135 ( & V_6 -> V_7 . V_376 . V_200 , V_134 , V_378 ) ;
return F_38 ( V_6 ) ;
}
static int F_136 ( struct V_2 * V_3 , struct V_41 * V_8 , const struct V_57 * V_58 , void * const * V_109 )
{
struct V_1 * V_6 = F_1 ( V_3 ) ;
int V_125 = V_58 -> V_146 ;
bool V_390 = false ;
if ( V_58 -> V_72 == ( sizeof( * V_58 ) / sizeof( V_73 ) ) ) {
V_390 = true ;
if ( V_125 != 0 && V_125 != 1 )
return - V_70 ;
V_6 -> V_51 = V_125 ;
}
if ( V_390 && F_137 ( V_8 ) )
V_8 = F_138 ( V_8 , V_33 ) ;
else
V_8 = F_31 ( V_8 , V_33 ) ;
if ( V_390 && V_8 ) {
struct V_57 * V_391 = (struct V_57 * ) V_8 -> V_60 ;
V_391 -> V_62 = 0 ;
}
F_34 ( V_8 , V_33 , V_52 , NULL , F_7 ( V_3 ) ) ;
return 0 ;
}
static int F_139 ( struct V_392 * V_393 , int V_394 , int V_371 , void * V_372 )
{
int V_350 ;
T_6 V_270 = * ( T_6 * ) V_372 ;
for ( V_350 = 0 ; V_350 < V_393 -> V_395 ; V_350 ++ ) {
if ( V_393 -> V_396 [ V_350 ] . V_270 == V_270 )
return - V_362 ;
}
return 0 ;
}
static T_6 F_140 ( struct V_10 * V_10 )
{
struct V_397 V_398 ;
T_6 V_399 ;
int V_59 ;
static T_6 V_270 = V_400 ;
V_399 = V_270 ;
do {
++ V_270 ;
if ( V_270 == 0 )
V_270 = V_400 + 1 ;
F_141 ( & V_398 , V_401 ) ;
V_59 = V_397 ( V_10 , & V_398 , F_139 , ( void * ) & V_270 ) ;
F_142 ( & V_398 , V_10 ) ;
if ( V_59 != - V_362 )
return V_270 ;
} while ( V_270 != V_399 );
return 0 ;
}
static int
F_143 ( struct V_392 * V_393 , struct V_402 * V_403 )
{
struct V_10 * V_10 = F_144 ( V_393 ) ;
struct V_404 * V_405 = V_393 -> V_396 + V_393 -> V_395 ;
int V_149 ;
if ( V_393 -> V_395 >= V_406 )
return - V_407 ;
if ( V_403 -> V_408 == 0 )
return - V_70 ;
V_405 -> V_195 . V_134 = V_403 -> V_409 ;
if ( ( V_149 = F_65 ( V_403 -> V_408 ) ) < 0 )
return - V_70 ;
V_405 -> V_149 = V_149 ;
if ( V_403 -> V_410 == V_411 )
V_405 -> V_412 = 1 ;
else if ( V_403 -> V_410 == V_413 ) {
V_405 -> V_270 = V_403 -> V_414 ;
if ( V_405 -> V_270 > V_400 )
V_405 -> V_270 = 0 ;
if ( ! V_405 -> V_270 && ! ( V_405 -> V_270 = F_140 ( V_10 ) ) )
return - V_44 ;
}
if ( V_405 -> V_149 == V_152 ) {
T_7 * V_136 = ( T_7 * ) ( V_403 + 1 ) ;
int V_135 , V_415 ;
V_135 = F_58 ( (struct V_77 * ) V_136 ,
& V_405 -> V_185 ) ;
if ( ! V_135 )
return - V_70 ;
V_415 = F_57 ( V_135 ) ;
if ( F_58 ( (struct V_77 * ) ( V_136 + V_415 ) ,
& V_405 -> V_195 . V_252 ) != V_135 )
return - V_70 ;
V_405 -> V_416 = V_135 ;
} else
V_405 -> V_416 = V_393 -> V_135 ;
V_405 -> V_417 = 1 ;
V_393 -> V_395 ++ ;
return 0 ;
}
static int
F_145 ( struct V_392 * V_393 , struct V_418 * V_419 )
{
int V_23 ;
int V_83 = V_419 -> V_420 * 8 - sizeof( struct V_418 ) ;
struct V_402 * V_403 = ( void * ) ( V_419 + 1 ) ;
if ( V_419 -> V_420 * 8 < sizeof( struct V_418 ) )
return - V_70 ;
while ( V_83 >= sizeof( struct V_402 ) ) {
if ( ( V_23 = F_143 ( V_393 , V_403 ) ) < 0 )
return V_23 ;
V_83 -= V_403 -> V_421 ;
V_403 = ( void * ) ( ( T_7 * ) V_403 + V_403 -> V_421 ) ;
}
return 0 ;
}
static inline int F_146 ( const struct V_392 * V_393 )
{
struct V_174 * V_175 = V_393 -> V_183 ;
if ( V_175 ) {
int V_83 = sizeof( struct V_89 ) ;
V_83 += V_175 -> V_104 ;
return F_63 ( V_83 ) ;
}
return 0 ;
}
static int F_147 ( const struct V_392 * V_393 )
{
const struct V_404 * V_405 ;
int V_179 = F_62 ( V_393 -> V_135 ) ;
int V_415 = 0 ;
int V_350 ;
for ( V_350 = 0 ; V_350 < V_393 -> V_395 ; V_350 ++ ) {
V_405 = V_393 -> V_396 + V_350 ;
V_415 += F_57 ( V_405 -> V_416 ) ;
}
return sizeof( struct V_57 ) +
( sizeof( struct V_168 ) * 3 ) +
( sizeof( struct V_75 ) * 2 ) +
( V_179 * 2 ) +
sizeof( struct V_418 ) +
( V_393 -> V_395 * sizeof( struct V_402 ) ) +
( V_415 * 2 ) +
F_146 ( V_393 ) ;
}
static struct V_41 * F_148 ( const struct V_392 * V_393 )
{
struct V_41 * V_8 ;
int V_176 ;
V_176 = F_147 ( V_393 ) ;
V_8 = F_41 ( V_176 + 16 , V_63 ) ;
if ( V_8 == NULL )
return F_68 ( - V_44 ) ;
return V_8 ;
}
static int F_149 ( struct V_41 * V_8 , const struct V_392 * V_393 , int V_394 )
{
struct V_57 * V_58 ;
struct V_75 * V_78 ;
struct V_168 * V_169 ;
struct V_418 * V_419 ;
struct V_89 * V_90 ;
struct V_174 * V_175 ;
int V_350 ;
int V_176 ;
int V_179 = F_62 ( V_393 -> V_135 ) ;
int V_415 = F_57 ( V_393 -> V_135 ) ;
V_176 = F_147 ( V_393 ) ;
V_58 = (struct V_57 * ) F_43 ( V_8 , sizeof( struct V_57 ) ) ;
memset ( V_58 , 0 , V_176 ) ;
V_78 = (struct V_75 * ) F_43 ( V_8 ,
sizeof( struct V_75 ) + V_179 ) ;
V_78 -> V_86 =
( sizeof( struct V_75 ) + V_179 ) /
sizeof( V_73 ) ;
V_78 -> V_249 = V_120 ;
V_78 -> V_250 = F_56 ( V_393 -> V_422 . V_134 ) ;
V_78 -> V_87 = V_393 -> V_422 . V_253 ;
V_78 -> V_251 = 0 ;
if ( ! F_66 ( & V_393 -> V_422 . V_185 ,
V_393 -> V_422 . V_254 ,
(struct V_77 * ) ( V_78 + 1 ) ,
V_393 -> V_135 ) )
F_74 () ;
V_78 = (struct V_75 * ) F_43 ( V_8 ,
sizeof( struct V_75 ) + V_179 ) ;
V_78 -> V_86 =
( sizeof( struct V_75 ) + V_179 ) /
sizeof( V_73 ) ;
V_78 -> V_249 = V_121 ;
V_78 -> V_250 = F_56 ( V_393 -> V_422 . V_134 ) ;
V_78 -> V_87 = V_393 -> V_422 . V_423 ;
V_78 -> V_251 = 0 ;
F_66 ( & V_393 -> V_422 . V_252 , V_393 -> V_422 . V_424 ,
(struct V_77 * ) ( V_78 + 1 ) ,
V_393 -> V_135 ) ;
V_169 = (struct V_168 * ) F_43 ( V_8 ,
sizeof( struct V_168 ) ) ;
V_169 -> V_226 =
sizeof( struct V_168 ) / sizeof( V_73 ) ;
V_169 -> V_227 = V_228 ;
V_169 -> V_229 = F_73 ( V_393 -> V_230 . V_231 ) ;
V_169 -> V_232 = F_73 ( V_393 -> V_230 . V_233 ) ;
V_169 -> V_234 = V_393 -> V_230 . V_235 ;
V_169 -> V_236 = V_393 -> V_230 . V_237 ;
V_169 = (struct V_168 * ) F_43 ( V_8 ,
sizeof( struct V_168 ) ) ;
V_169 -> V_226 =
sizeof( struct V_168 ) / sizeof( V_73 ) ;
V_169 -> V_227 = V_238 ;
V_169 -> V_229 = F_73 ( V_393 -> V_230 . V_239 ) ;
V_169 -> V_232 = F_73 ( V_393 -> V_230 . V_240 ) ;
V_169 -> V_234 = V_393 -> V_230 . V_241 ;
V_169 -> V_236 = V_393 -> V_230 . V_242 ;
V_169 = (struct V_168 * ) F_43 ( V_8 ,
sizeof( struct V_168 ) ) ;
V_169 -> V_226 =
sizeof( struct V_168 ) / sizeof( V_73 ) ;
V_169 -> V_227 = V_243 ;
V_169 -> V_229 = V_393 -> V_244 . V_245 ;
V_169 -> V_232 = V_393 -> V_244 . V_246 ;
V_169 -> V_234 = V_393 -> V_244 . V_247 ;
V_169 -> V_236 = V_393 -> V_244 . V_248 ;
V_419 = (struct V_418 * ) F_43 ( V_8 , sizeof( struct V_418 ) ) ;
V_419 -> V_420 = sizeof( struct V_418 ) / sizeof( V_73 ) ;
V_419 -> V_425 = V_426 ;
V_419 -> V_427 = V_428 ;
if ( V_393 -> V_429 == V_430 ) {
if ( V_393 -> V_395 )
V_419 -> V_427 = V_431 ;
else
V_419 -> V_427 = V_432 ;
}
V_419 -> V_433 = V_394 + 1 ;
V_419 -> V_434 = 0 ;
V_419 -> V_435 = V_393 -> V_436 ;
V_419 -> V_437 = V_393 -> V_438 ;
for ( V_350 = 0 ; V_350 < V_393 -> V_395 ; V_350 ++ ) {
const struct V_404 * V_405 = V_393 -> V_396 + V_350 ;
struct V_402 * V_403 ;
int V_439 ;
int V_149 ;
V_439 = sizeof( struct V_402 ) ;
if ( V_405 -> V_149 == V_152 ) {
V_415 = F_57 ( V_405 -> V_416 ) ;
V_439 += V_415 * 2 ;
} else {
V_176 -= 2 * V_415 ;
}
V_403 = ( void * ) F_43 ( V_8 , V_439 ) ;
V_419 -> V_420 += V_439 / 8 ;
memset ( V_403 , 0 , sizeof( * V_403 ) ) ;
V_403 -> V_421 = V_439 ;
V_403 -> V_409 = V_405 -> V_195 . V_134 ;
if ( ( V_149 = F_64 ( V_405 -> V_149 ) ) < 0 )
return - V_70 ;
V_403 -> V_408 = V_149 ;
V_403 -> V_410 = V_440 ;
if ( V_405 -> V_270 )
V_403 -> V_410 = V_413 ;
if ( V_405 -> V_412 )
V_403 -> V_410 = V_411 ;
V_403 -> V_414 = V_405 -> V_270 ;
if ( V_405 -> V_149 == V_152 ) {
T_7 * V_136 = ( void * ) ( V_403 + 1 ) ;
F_66 ( & V_405 -> V_185 , 0 ,
(struct V_77 * ) V_136 ,
V_405 -> V_416 ) ;
F_66 ( & V_405 -> V_195 . V_252 , 0 ,
(struct V_77 * ) ( V_136 + V_415 ) ,
V_405 -> V_416 ) ;
}
}
if ( ( V_175 = V_393 -> V_183 ) ) {
int V_97 = F_146 ( V_393 ) ;
V_90 = (struct V_89 * ) F_43 ( V_8 , V_97 ) ;
V_90 -> V_93 = V_97 / sizeof( V_73 ) ;
V_90 -> V_99 = V_124 ;
V_90 -> V_101 = V_175 -> V_100 ;
V_90 -> V_103 = V_175 -> V_102 ;
V_90 -> V_91 = V_175 -> V_104 ;
memcpy ( V_90 + 1 , V_175 -> V_286 ,
V_175 -> V_104 ) ;
}
V_58 -> V_72 = V_176 / sizeof( V_73 ) ;
V_58 -> V_322 = F_3 ( & V_393 -> V_441 ) ;
return 0 ;
}
static int F_150 ( struct V_392 * V_393 , int V_394 , const struct V_342 * V_343 )
{
struct V_41 * V_346 ;
struct V_57 * V_307 ;
int V_23 ;
V_346 = F_148 ( V_393 ) ;
if ( F_94 ( V_346 ) )
return F_95 ( V_346 ) ;
V_23 = F_149 ( V_346 , V_393 , V_394 ) ;
if ( V_23 < 0 )
return V_23 ;
V_307 = (struct V_57 * ) V_346 -> V_60 ;
V_307 -> V_319 = V_29 ;
if ( V_343 -> V_60 . V_442 && V_343 -> V_326 == V_327 )
V_307 -> V_320 = V_443 ;
else
V_307 -> V_320 = F_99 ( V_343 -> V_326 ) ;
V_307 -> V_62 = 0 ;
V_307 -> V_61 = V_343 -> V_303 ;
V_307 -> V_323 = V_343 -> V_344 ;
F_34 ( V_346 , V_63 , V_52 , NULL , F_144 ( V_393 ) ) ;
return 0 ;
}
static int F_151 ( struct V_2 * V_3 , struct V_41 * V_8 , const struct V_57 * V_58 , void * const * V_109 )
{
struct V_10 * V_10 = F_7 ( V_3 ) ;
int V_23 = 0 ;
struct V_168 * V_169 ;
struct V_75 * V_136 ;
struct V_418 * V_419 ;
struct V_392 * V_393 ;
struct V_342 V_343 ;
struct V_89 * V_90 ;
if ( ! F_51 ( V_109 [ V_120 - 1 ] ,
V_109 [ V_121 - 1 ] ) ||
! V_109 [ V_426 - 1 ] )
return - V_70 ;
V_419 = V_109 [ V_426 - 1 ] ;
if ( V_419 -> V_427 > V_431 )
return - V_70 ;
if ( ! V_419 -> V_433 || V_419 -> V_433 >= V_444 )
return - V_70 ;
V_393 = F_152 ( V_10 , V_33 ) ;
if ( V_393 == NULL )
return - V_44 ;
V_393 -> V_429 = ( V_419 -> V_427 == V_428 ?
V_445 : V_430 ) ;
V_393 -> V_438 = V_419 -> V_437 ;
V_136 = V_109 [ V_120 - 1 ] ;
V_393 -> V_135 = F_59 ( V_136 , & V_393 -> V_422 . V_185 ) ;
V_393 -> V_422 . V_135 = V_393 -> V_135 ;
V_393 -> V_422 . V_253 = V_136 -> V_87 ;
V_393 -> V_422 . V_134 = F_55 ( V_136 -> V_250 ) ;
V_393 -> V_422 . V_254 = ( (struct V_79 * ) ( V_136 + 1 ) ) -> V_159 ;
if ( V_393 -> V_422 . V_254 )
V_393 -> V_422 . V_446 = F_153 ( 0xffff ) ;
V_136 = V_109 [ V_121 - 1 ] ;
F_59 ( V_136 , & V_393 -> V_422 . V_252 ) ;
V_393 -> V_422 . V_423 = V_136 -> V_87 ;
V_393 -> V_422 . V_134 = F_55 ( V_136 -> V_250 ) ;
V_393 -> V_422 . V_424 = ( (struct V_79 * ) ( V_136 + 1 ) ) -> V_159 ;
if ( V_393 -> V_422 . V_424 )
V_393 -> V_422 . V_447 = F_153 ( 0xffff ) ;
V_90 = V_109 [ V_124 - 1 ] ;
if ( V_90 != NULL ) {
struct V_94 * V_96 = F_49 ( V_90 , V_33 ) ;
if ( ! V_96 ) {
V_23 = - V_44 ;
goto V_35;
}
V_23 = F_154 ( & V_393 -> V_183 , V_96 , V_33 ) ;
F_82 ( V_96 ) ;
if ( V_23 )
goto V_35;
}
V_393 -> V_230 . V_240 = V_448 ;
V_393 -> V_230 . V_233 = V_448 ;
V_393 -> V_230 . V_239 = V_448 ;
V_393 -> V_230 . V_231 = V_448 ;
if ( ( V_169 = V_109 [ V_228 - 1 ] ) != NULL ) {
V_393 -> V_230 . V_231 = F_80 ( V_169 -> V_229 ) ;
V_393 -> V_230 . V_233 = F_80 ( V_169 -> V_232 ) ;
V_393 -> V_230 . V_235 = V_169 -> V_234 ;
V_393 -> V_230 . V_237 = V_169 -> V_236 ;
}
if ( ( V_169 = V_109 [ V_238 - 1 ] ) != NULL ) {
V_393 -> V_230 . V_239 = F_80 ( V_169 -> V_229 ) ;
V_393 -> V_230 . V_240 = F_80 ( V_169 -> V_232 ) ;
V_393 -> V_230 . V_241 = V_169 -> V_234 ;
V_393 -> V_230 . V_242 = V_169 -> V_236 ;
}
V_393 -> V_395 = 0 ;
if ( V_419 -> V_427 == V_431 &&
( V_23 = F_145 ( V_393 , V_419 ) ) < 0 )
goto V_35;
V_23 = F_155 ( V_419 -> V_433 - 1 , V_393 ,
V_58 -> V_320 != V_332 ) ;
F_156 ( V_393 , V_23 ? 0 : 1 ,
F_108 ( V_345 ) ,
F_109 ( V_345 ) , 0 ) ;
if ( V_23 )
goto V_35;
if ( V_58 -> V_320 == V_332 )
V_343 . V_326 = V_331 ;
else
V_343 . V_326 = V_329 ;
V_343 . V_303 = V_58 -> V_61 ;
V_343 . V_344 = V_58 -> V_323 ;
F_157 ( V_393 , V_419 -> V_433 - 1 , & V_343 ) ;
F_158 ( V_393 ) ;
return 0 ;
V_35:
V_393 -> V_398 . V_449 = 1 ;
F_159 ( V_393 ) ;
return V_23 ;
}
static int F_160 ( struct V_2 * V_3 , struct V_41 * V_8 , const struct V_57 * V_58 , void * const * V_109 )
{
struct V_10 * V_10 = F_7 ( V_3 ) ;
int V_23 ;
struct V_75 * V_136 ;
struct V_418 * V_419 ;
struct V_392 * V_393 ;
struct V_450 V_184 ;
struct V_342 V_343 ;
struct V_89 * V_90 ;
struct V_174 * V_451 = NULL ;
if ( ! F_51 ( V_109 [ V_120 - 1 ] ,
V_109 [ V_121 - 1 ] ) ||
! V_109 [ V_426 - 1 ] )
return - V_70 ;
V_419 = V_109 [ V_426 - 1 ] ;
if ( ! V_419 -> V_433 || V_419 -> V_433 >= V_444 )
return - V_70 ;
memset ( & V_184 , 0 , sizeof( V_184 ) ) ;
V_136 = V_109 [ V_120 - 1 ] ;
V_184 . V_135 = F_59 ( V_136 , & V_184 . V_185 ) ;
V_184 . V_253 = V_136 -> V_87 ;
V_184 . V_134 = F_55 ( V_136 -> V_250 ) ;
V_184 . V_254 = ( (struct V_79 * ) ( V_136 + 1 ) ) -> V_159 ;
if ( V_184 . V_254 )
V_184 . V_446 = F_153 ( 0xffff ) ;
V_136 = V_109 [ V_121 - 1 ] ;
F_59 ( V_136 , & V_184 . V_252 ) ;
V_184 . V_423 = V_136 -> V_87 ;
V_184 . V_134 = F_55 ( V_136 -> V_250 ) ;
V_184 . V_424 = ( (struct V_79 * ) ( V_136 + 1 ) ) -> V_159 ;
if ( V_184 . V_424 )
V_184 . V_447 = F_153 ( 0xffff ) ;
V_90 = V_109 [ V_124 - 1 ] ;
if ( V_90 != NULL ) {
struct V_94 * V_96 = F_49 ( V_90 , V_33 ) ;
if ( ! V_96 )
return - V_31 ;
V_23 = F_154 ( & V_451 , V_96 , V_33 ) ;
F_82 ( V_96 ) ;
if ( V_23 )
return V_23 ;
}
V_393 = F_161 ( V_10 , V_147 , V_401 ,
V_419 -> V_433 - 1 , & V_184 , V_451 ,
1 , & V_23 ) ;
F_162 ( V_451 ) ;
if ( V_393 == NULL )
return - V_315 ;
F_163 ( V_393 , V_23 ? 0 : 1 ,
F_108 ( V_345 ) ,
F_109 ( V_345 ) , 0 ) ;
if ( V_23 )
goto V_35;
V_343 . V_303 = V_58 -> V_61 ;
V_343 . V_344 = V_58 -> V_323 ;
V_343 . V_60 . V_442 = 0 ;
V_343 . V_326 = V_327 ;
F_157 ( V_393 , V_419 -> V_433 - 1 , & V_343 ) ;
V_35:
F_158 ( V_393 ) ;
if ( V_23 == 0 )
F_164 ( V_10 ) ;
return V_23 ;
}
static int F_165 ( struct V_2 * V_3 , struct V_392 * V_393 , const struct V_57 * V_58 , int V_394 )
{
int V_23 ;
struct V_41 * V_346 ;
struct V_57 * V_307 ;
V_23 = 0 ;
V_346 = F_148 ( V_393 ) ;
if ( F_94 ( V_346 ) ) {
V_23 = F_95 ( V_346 ) ;
goto V_35;
}
V_23 = F_149 ( V_346 , V_393 , V_394 ) ;
if ( V_23 < 0 )
goto V_35;
V_307 = (struct V_57 * ) V_346 -> V_60 ;
V_307 -> V_319 = V_58 -> V_319 ;
V_307 -> V_320 = V_58 -> V_320 ;
V_307 -> V_146 = 0 ;
V_307 -> V_62 = 0 ;
V_307 -> V_61 = V_58 -> V_61 ;
V_307 -> V_323 = V_58 -> V_323 ;
F_34 ( V_346 , V_63 , V_56 , V_3 , F_144 ( V_393 ) ) ;
V_23 = 0 ;
V_35:
return V_23 ;
}
static int F_166 ( T_3 V_135 )
{
return F_63 ( F_57 ( V_135 ) * 2 ) ;
}
static int F_167 ( struct V_77 * V_136 , int V_113 ,
T_4 * V_185 , T_4 * V_252 ,
T_9 * V_135 )
{
int V_452 , V_415 ;
if ( V_113 < F_166 ( V_136 -> V_84 ) )
return - V_70 ;
V_452 = F_58 ( V_136 , V_185 ) ;
if ( ! V_452 )
return - V_70 ;
V_415 = F_57 ( V_452 ) ;
if ( F_58 ( (struct V_77 * ) ( ( ( T_7 * ) V_136 ) + V_415 ) ,
V_252 ) != V_452 )
return - V_70 ;
* V_135 = V_452 ;
return 0 ;
}
static int F_168 ( struct V_402 * V_453 , int V_83 ,
struct V_454 * V_455 )
{
int V_23 ;
struct V_402 * V_456 ;
int V_149 ;
if ( V_83 <= sizeof( struct V_402 ) ||
V_83 < V_453 -> V_421 )
return - V_70 ;
V_23 = F_167 ( (struct V_77 * ) ( V_453 + 1 ) ,
V_453 -> V_421 ,
& V_455 -> V_457 , & V_455 -> V_458 ,
& V_455 -> V_459 ) ;
if ( V_23 )
return V_23 ;
V_456 = (struct V_402 * ) ( ( T_7 * ) V_453 + V_453 -> V_421 ) ;
V_83 -= V_453 -> V_421 ;
if ( V_83 <= sizeof( struct V_402 ) ||
V_83 < V_456 -> V_421 )
return - V_70 ;
V_23 = F_167 ( (struct V_77 * ) ( V_456 + 1 ) ,
V_456 -> V_421 ,
& V_455 -> V_460 , & V_455 -> V_461 ,
& V_455 -> V_462 ) ;
if ( V_23 )
return V_23 ;
if ( V_453 -> V_409 != V_456 -> V_409 ||
V_453 -> V_408 != V_456 -> V_408 ||
V_453 -> V_414 != V_456 -> V_414 )
return - V_70 ;
V_455 -> V_134 = V_453 -> V_409 ;
if ( ( V_149 = F_65 ( V_453 -> V_408 ) ) < 0 )
return - V_70 ;
V_455 -> V_149 = V_149 ;
V_455 -> V_270 = V_453 -> V_414 ;
return ( ( int ) ( V_453 -> V_421 +
V_456 -> V_421 ) ) ;
}
static int F_169 ( struct V_2 * V_3 , struct V_41 * V_8 ,
const struct V_57 * V_58 , void * const * V_109 )
{
int V_350 , V_83 , V_463 , V_23 = - V_70 ;
T_7 V_394 ;
struct V_75 * V_136 ;
struct V_464 * V_465 ;
struct V_418 * V_419 ;
struct V_402 * V_403 ;
struct V_450 V_184 ;
struct V_454 V_455 [ V_406 ] ;
struct V_466 V_467 ;
struct V_10 * V_10 = F_7 ( V_3 ) ;
if ( ! F_51 ( V_109 [ V_120 - 1 ] ,
V_109 [ V_121 - 1 ] ) ||
! V_109 [ V_426 - 1 ] ) {
V_23 = - V_70 ;
goto V_35;
}
V_465 = V_109 [ V_468 - 1 ] ;
V_419 = V_109 [ V_426 - 1 ] ;
if ( V_419 -> V_433 >= V_444 ) {
V_23 = - V_70 ;
goto V_35;
}
if ( V_465 ) {
V_467 . V_469 = V_465 -> V_470 ;
V_463 = F_167 ( (struct V_77 * ) ( V_465 + 1 ) ,
8 * ( V_465 -> V_471 ) - sizeof( * V_465 ) ,
& V_467 . V_472 , & V_467 . V_473 , & V_467 . V_135 ) ;
if ( V_463 < 0 ) {
V_23 = V_463 ;
goto V_35;
}
}
V_394 = V_419 -> V_433 - 1 ;
memset ( & V_184 , 0 , sizeof( V_184 ) ) ;
V_136 = V_109 [ V_120 - 1 ] ;
V_184 . V_135 = F_59 ( V_136 , & V_184 . V_185 ) ;
V_184 . V_253 = V_136 -> V_87 ;
V_184 . V_134 = F_55 ( V_136 -> V_250 ) ;
V_184 . V_254 = ( (struct V_79 * ) ( V_136 + 1 ) ) -> V_159 ;
if ( V_184 . V_254 )
V_184 . V_446 = F_153 ( 0xffff ) ;
V_136 = V_109 [ V_121 - 1 ] ,
F_59 ( V_136 , & V_184 . V_252 ) ;
V_184 . V_423 = V_136 -> V_87 ;
V_184 . V_134 = F_55 ( V_136 -> V_250 ) ;
V_184 . V_424 = ( (struct V_79 * ) ( V_136 + 1 ) ) -> V_159 ;
if ( V_184 . V_424 )
V_184 . V_447 = F_153 ( 0xffff ) ;
V_403 = (struct V_402 * ) ( V_419 + 1 ) ;
V_350 = 0 ;
V_83 = V_419 -> V_420 * 8 - sizeof( struct V_418 ) ;
while ( V_83 > 0 && V_350 < V_406 ) {
V_463 = F_168 ( V_403 , V_83 , & V_455 [ V_350 ] ) ;
if ( V_463 < 0 ) {
V_23 = V_463 ;
goto V_35;
} else {
V_403 = (struct V_402 * ) ( ( T_7 * ) V_403 + V_463 ) ;
V_83 -= V_463 ;
V_350 ++ ;
}
}
if ( ! V_350 || V_83 > 0 ) {
V_23 = - V_70 ;
goto V_35;
}
return V_454 ( & V_184 , V_394 , V_401 , V_455 , V_350 ,
V_465 ? & V_467 : NULL , V_10 ) ;
V_35:
return V_23 ;
}
static int F_169 ( struct V_2 * V_3 , struct V_41 * V_8 ,
const struct V_57 * V_58 , void * const * V_109 )
{
return - V_474 ;
}
static int F_170 ( struct V_2 * V_3 , struct V_41 * V_8 , const struct V_57 * V_58 , void * const * V_109 )
{
struct V_10 * V_10 = F_7 ( V_3 ) ;
unsigned int V_394 ;
int V_23 = 0 , V_475 ;
struct V_418 * V_419 ;
struct V_392 * V_393 ;
struct V_342 V_343 ;
if ( ( V_419 = V_109 [ V_426 - 1 ] ) == NULL )
return - V_70 ;
V_394 = F_171 ( V_419 -> V_435 ) ;
if ( V_394 >= V_476 )
return - V_70 ;
V_475 = ( V_58 -> V_320 == V_443 ) ;
V_393 = F_172 ( V_10 , V_147 , V_401 ,
V_394 , V_419 -> V_435 , V_475 , & V_23 ) ;
if ( V_393 == NULL )
return - V_315 ;
if ( V_475 ) {
F_163 ( V_393 , V_23 ? 0 : 1 ,
F_108 ( V_345 ) ,
F_109 ( V_345 ) , 0 ) ;
if ( V_23 )
goto V_35;
V_343 . V_303 = V_58 -> V_61 ;
V_343 . V_344 = V_58 -> V_323 ;
V_343 . V_60 . V_442 = 1 ;
V_343 . V_326 = V_327 ;
F_157 ( V_393 , V_394 , & V_343 ) ;
} else {
V_23 = F_165 ( V_3 , V_393 , V_58 , V_394 ) ;
}
V_35:
F_158 ( V_393 ) ;
if ( V_475 && V_23 == 0 )
F_164 ( V_10 ) ;
return V_23 ;
}
static int F_173 ( struct V_392 * V_393 , int V_394 , int V_371 , void * V_372 )
{
struct V_1 * V_6 = V_372 ;
struct V_41 * V_346 ;
struct V_57 * V_307 ;
int V_23 ;
if ( ! F_2 ( & V_6 -> V_3 ) )
return - V_44 ;
V_346 = F_148 ( V_393 ) ;
if ( F_94 ( V_346 ) )
return F_95 ( V_346 ) ;
V_23 = F_149 ( V_346 , V_393 , V_394 ) ;
if ( V_23 < 0 )
return V_23 ;
V_307 = (struct V_57 * ) V_346 -> V_60 ;
V_307 -> V_319 = V_6 -> V_7 . V_373 ;
V_307 -> V_320 = V_477 ;
V_307 -> V_146 = V_126 ;
V_307 -> V_62 = 0 ;
V_307 -> V_61 = V_371 + 1 ;
V_307 -> V_323 = V_6 -> V_7 . V_375 ;
if ( V_6 -> V_7 . V_8 )
F_34 ( V_6 -> V_7 . V_8 , V_63 , V_56 ,
& V_6 -> V_3 , F_7 ( & V_6 -> V_3 ) ) ;
V_6 -> V_7 . V_8 = V_346 ;
return 0 ;
}
static int F_174 ( struct V_1 * V_6 )
{
struct V_10 * V_10 = F_7 ( & V_6 -> V_3 ) ;
return V_397 ( V_10 , & V_6 -> V_7 . V_376 . V_478 , F_173 , ( void * ) V_6 ) ;
}
static void F_175 ( struct V_1 * V_6 )
{
struct V_10 * V_10 = F_7 ( (struct V_2 * ) V_6 ) ;
F_142 ( & V_6 -> V_7 . V_376 . V_478 , V_10 ) ;
}
static int F_176 ( struct V_2 * V_3 , struct V_41 * V_8 , const struct V_57 * V_58 , void * const * V_109 )
{
struct V_1 * V_6 = F_1 ( V_3 ) ;
if ( V_6 -> V_7 . V_7 != NULL )
return - V_379 ;
V_6 -> V_7 . V_373 = V_58 -> V_319 ;
V_6 -> V_7 . V_375 = V_58 -> V_323 ;
V_6 -> V_7 . V_7 = F_174 ;
V_6 -> V_7 . V_9 = F_175 ;
F_141 ( & V_6 -> V_7 . V_376 . V_478 , V_401 ) ;
return F_38 ( V_6 ) ;
}
static int F_177 ( const struct V_342 * V_343 )
{
struct V_41 * V_479 ;
struct V_57 * V_58 ;
V_479 = F_41 ( sizeof( struct V_57 ) + 16 , V_63 ) ;
if ( ! V_479 )
return - V_44 ;
V_58 = (struct V_57 * ) F_43 ( V_479 , sizeof( struct V_57 ) ) ;
V_58 -> V_320 = V_480 ;
V_58 -> V_61 = V_343 -> V_303 ;
V_58 -> V_323 = V_343 -> V_344 ;
V_58 -> V_319 = V_29 ;
V_58 -> V_62 = ( V_71 ) 0 ;
V_58 -> V_146 = V_126 ;
V_58 -> V_72 = ( sizeof( struct V_57 ) / sizeof( V_73 ) ) ;
V_58 -> V_322 = 0 ;
F_34 ( V_479 , V_63 , V_52 , NULL , V_343 -> V_10 ) ;
return 0 ;
}
static int F_178 ( struct V_2 * V_3 , struct V_41 * V_8 , const struct V_57 * V_58 , void * const * V_109 )
{
struct V_10 * V_10 = F_7 ( V_3 ) ;
struct V_342 V_343 ;
struct V_365 V_366 ;
int V_23 , V_50 ;
V_366 . V_367 = F_108 ( V_345 ) ;
V_366 . V_368 = F_109 ( V_345 ) ;
V_366 . V_369 = 0 ;
V_23 = F_179 ( V_10 , V_401 , & V_366 ) ;
V_50 = F_125 ( V_3 , V_58 ) ;
if ( V_23 || V_50 ) {
if ( V_23 == - V_49 )
return 0 ;
return V_23 ;
}
V_343 . V_60 . type = V_401 ;
V_343 . V_326 = V_481 ;
V_343 . V_344 = V_58 -> V_323 ;
V_343 . V_303 = V_58 -> V_61 ;
V_343 . V_10 = V_10 ;
F_157 ( NULL , 0 , & V_343 ) ;
return 0 ;
}
static int F_180 ( struct V_2 * V_3 , struct V_41 * V_8 , const struct V_57 * V_58 )
{
void * V_109 [ V_117 ] ;
int V_23 ;
F_34 ( F_31 ( V_8 , V_33 ) , V_33 ,
V_53 , NULL , F_7 ( V_3 ) ) ;
memset ( V_109 , 0 , sizeof( V_109 ) ) ;
V_23 = F_52 ( V_8 , V_58 , V_109 ) ;
if ( ! V_23 ) {
V_23 = - V_305 ;
if ( V_482 [ V_58 -> V_320 ] )
V_23 = V_482 [ V_58 -> V_320 ] ( V_3 , V_8 , V_58 , V_109 ) ;
}
return V_23 ;
}
static struct V_57 * F_181 ( struct V_41 * V_8 , int * V_483 )
{
struct V_57 * V_58 = NULL ;
if ( V_8 -> V_83 < sizeof( * V_58 ) ) {
* V_483 = - V_484 ;
} else {
V_58 = (struct V_57 * ) V_8 -> V_60 ;
if ( V_58 -> V_319 != V_29 ||
V_58 -> V_322 != 0 ||
( V_58 -> V_320 <= V_485 ||
V_58 -> V_320 > V_486 ) ) {
V_58 = NULL ;
* V_483 = - V_70 ;
} else if ( V_58 -> V_72 != ( V_8 -> V_83 /
sizeof( V_73 ) ) ||
V_58 -> V_72 < ( sizeof( struct V_57 ) /
sizeof( V_73 ) ) ) {
V_58 = NULL ;
* V_483 = - V_484 ;
} else {
* V_483 = 0 ;
}
}
return V_58 ;
}
static inline int F_182 ( const struct V_404 * V_405 ,
const struct V_210 * V_487 )
{
unsigned int V_195 = V_487 -> V_214 . V_215 ;
if ( V_195 >= sizeof( V_405 -> V_488 ) * 8 )
return 0 ;
return ( V_405 -> V_488 >> V_195 ) & 1 ;
}
static inline int F_183 ( const struct V_404 * V_405 ,
const struct V_210 * V_487 )
{
unsigned int V_195 = V_487 -> V_214 . V_215 ;
if ( V_195 >= sizeof( V_405 -> V_489 ) * 8 )
return 0 ;
return ( V_405 -> V_489 >> V_195 ) & 1 ;
}
static int F_184 ( const struct V_404 * V_405 )
{
int V_350 , V_490 = 0 ;
for ( V_350 = 0 ; ; V_350 ++ ) {
const struct V_210 * V_186 = F_121 ( V_350 ) ;
if ( ! V_186 )
break;
if ( ! V_186 -> V_213 )
continue;
if ( F_182 ( V_405 , V_186 ) && V_186 -> V_358 )
V_490 += sizeof( struct V_491 ) ;
}
return V_490 + sizeof( struct V_492 ) ;
}
static int F_185 ( const struct V_404 * V_405 )
{
int V_350 , V_467 , V_490 = 0 ;
for ( V_350 = 0 ; ; V_350 ++ ) {
const struct V_210 * V_188 = F_122 ( V_350 ) ;
if ( ! V_188 )
break;
if ( ! V_188 -> V_213 )
continue;
if ( ! ( F_183 ( V_405 , V_188 ) && V_188 -> V_358 ) )
continue;
for ( V_467 = 1 ; ; V_467 ++ ) {
const struct V_210 * V_186 = F_121 ( V_467 ) ;
if ( ! V_186 )
break;
if ( ! V_186 -> V_213 )
continue;
if ( F_182 ( V_405 , V_186 ) && V_186 -> V_358 )
V_490 += sizeof( struct V_491 ) ;
}
}
return V_490 + sizeof( struct V_492 ) ;
}
static void F_186 ( struct V_41 * V_8 , const struct V_404 * V_405 )
{
struct V_492 * V_74 ;
int V_350 ;
V_74 = (struct V_492 * ) F_43 ( V_8 , sizeof( struct V_492 ) ) ;
V_74 -> V_493 = sizeof( struct V_492 ) / 8 ;
V_74 -> V_494 = V_495 ;
V_74 -> V_496 = 32 ;
memset ( V_74 -> V_497 , 0 , sizeof( V_74 -> V_497 ) ) ;
for ( V_350 = 0 ; ; V_350 ++ ) {
const struct V_210 * V_186 = F_121 ( V_350 ) ;
if ( ! V_186 )
break;
if ( ! V_186 -> V_213 )
continue;
if ( F_182 ( V_405 , V_186 ) && V_186 -> V_358 ) {
struct V_491 * V_343 ;
V_343 = (struct V_491 * ) F_43 ( V_8 , sizeof( struct V_491 ) ) ;
memset ( V_343 , 0 , sizeof( * V_343 ) ) ;
V_74 -> V_493 += sizeof( struct V_491 ) / 8 ;
V_343 -> V_498 = V_186 -> V_214 . V_215 ;
V_343 -> V_499 = V_186 -> V_214 . V_500 ;
V_343 -> V_501 = V_186 -> V_214 . V_502 ;
V_343 -> V_503 = 24 * 60 * 60 ;
V_343 -> V_504 = 20 * 60 * 60 ;
V_343 -> V_505 = 8 * 60 * 60 ;
V_343 -> V_506 = 7 * 60 * 60 ;
}
}
}
static void F_187 ( struct V_41 * V_8 , const struct V_404 * V_405 )
{
struct V_492 * V_74 ;
int V_350 , V_467 ;
V_74 = (struct V_492 * ) F_43 ( V_8 , sizeof( struct V_492 ) ) ;
V_74 -> V_493 = sizeof( struct V_492 ) / 8 ;
V_74 -> V_494 = V_495 ;
V_74 -> V_496 = 32 ;
memset ( V_74 -> V_497 , 0 , sizeof( V_74 -> V_497 ) ) ;
for ( V_350 = 0 ; ; V_350 ++ ) {
const struct V_210 * V_188 = F_122 ( V_350 ) ;
if ( ! V_188 )
break;
if ( ! V_188 -> V_213 )
continue;
if ( ! ( F_183 ( V_405 , V_188 ) && V_188 -> V_358 ) )
continue;
for ( V_467 = 1 ; ; V_467 ++ ) {
struct V_491 * V_343 ;
const struct V_210 * V_186 = F_121 ( V_467 ) ;
if ( ! V_186 )
break;
if ( ! V_186 -> V_213 )
continue;
if ( ! ( F_182 ( V_405 , V_186 ) && V_186 -> V_358 ) )
continue;
V_343 = (struct V_491 * ) F_43 ( V_8 , sizeof( struct V_491 ) ) ;
memset ( V_343 , 0 , sizeof( * V_343 ) ) ;
V_74 -> V_493 += sizeof( struct V_491 ) / 8 ;
V_343 -> V_498 = V_186 -> V_214 . V_215 ;
V_343 -> V_499 = V_186 -> V_214 . V_500 ;
V_343 -> V_501 = V_186 -> V_214 . V_502 ;
V_343 -> V_507 = V_188 -> V_214 . V_215 ;
V_343 -> V_508 = V_188 -> V_214 . V_500 ;
V_343 -> V_509 = V_188 -> V_214 . V_502 ;
V_343 -> V_503 = 24 * 60 * 60 ;
V_343 -> V_504 = 20 * 60 * 60 ;
V_343 -> V_505 = 8 * 60 * 60 ;
V_343 -> V_506 = 7 * 60 * 60 ;
}
}
}
static int F_188 ( struct V_392 * V_393 , const struct V_342 * V_343 )
{
return 0 ;
}
static int F_189 ( struct V_143 * V_165 , const struct V_342 * V_343 )
{
struct V_41 * V_346 ;
struct V_57 * V_307 ;
int V_510 ;
int V_167 ;
V_510 = V_343 -> V_60 . V_510 ;
if ( V_510 )
V_167 = 2 ;
else
V_167 = 1 ;
V_346 = F_76 ( V_165 , V_167 ) ;
if ( F_94 ( V_346 ) )
return F_95 ( V_346 ) ;
V_307 = (struct V_57 * ) V_346 -> V_60 ;
V_307 -> V_319 = V_29 ;
V_307 -> V_320 = V_341 ;
V_307 -> V_146 = F_54 ( V_165 -> V_195 . V_134 ) ;
V_307 -> V_62 = 0 ;
V_307 -> V_322 = 0 ;
V_307 -> V_61 = 0 ;
V_307 -> V_323 = 0 ;
F_34 ( V_346 , V_63 , V_54 , NULL , F_102 ( V_165 ) ) ;
return 0 ;
}
static int F_190 ( struct V_143 * V_165 , const struct V_342 * V_343 )
{
struct V_10 * V_10 = V_165 ? F_102 ( V_165 ) : V_343 -> V_10 ;
struct V_11 * V_12 = F_8 ( V_10 , V_13 ) ;
if ( F_3 ( & V_12 -> V_17 ) == 0 )
return 0 ;
switch ( V_343 -> V_326 ) {
case V_340 :
return F_189 ( V_165 , V_343 ) ;
case V_334 :
case V_336 :
case V_338 :
return F_101 ( V_165 , V_343 ) ;
case V_370 :
return F_126 ( V_343 ) ;
case V_511 :
break;
default:
F_11 ( L_3 , V_343 -> V_326 ) ;
break;
}
return 0 ;
}
static int F_191 ( struct V_392 * V_393 , int V_394 , const struct V_342 * V_343 )
{
if ( V_393 && V_393 -> type != V_401 )
return 0 ;
switch ( V_343 -> V_326 ) {
case V_333 :
return F_188 ( V_393 , V_343 ) ;
case V_327 :
case V_329 :
case V_331 :
return F_150 ( V_393 , V_394 , V_343 ) ;
case V_481 :
if ( V_343 -> V_60 . type != V_401 )
break;
return F_177 ( V_343 ) ;
default:
F_11 ( L_2 , V_343 -> V_326 ) ;
break;
}
return 0 ;
}
static T_6 F_192 ( void )
{
T_6 V_512 ;
static T_10 V_513 ;
do {
V_512 = F_193 ( & V_513 ) ;
} while ( ! V_512 );
return V_512 ;
}
static bool F_194 ( const struct V_342 * V_343 )
{
struct V_11 * V_12 = F_8 ( V_343 -> V_10 , V_13 ) ;
struct V_2 * V_3 ;
bool V_514 = false ;
F_35 () ;
F_36 (sk, &net_pfkey->table) {
if ( F_1 ( V_3 ) -> V_55 ) {
V_514 = true ;
break;
}
}
F_37 () ;
return V_514 ;
}
static int F_195 ( struct V_143 * V_165 , struct V_404 * V_405 , struct V_392 * V_393 )
{
struct V_41 * V_8 ;
struct V_57 * V_58 ;
struct V_75 * V_78 ;
struct V_418 * V_419 ;
int V_179 ;
int V_176 ;
struct V_89 * V_90 ;
struct V_174 * V_175 ;
int V_97 = 0 ;
V_179 = F_62 ( V_165 -> V_182 . V_135 ) ;
if ( ! V_179 )
return - V_70 ;
V_176 = sizeof( struct V_57 ) +
( sizeof( struct V_75 ) * 2 ) +
( V_179 * 2 ) +
sizeof( struct V_418 ) ;
if ( V_165 -> V_195 . V_134 == V_129 )
V_176 += F_184 ( V_405 ) ;
else if ( V_165 -> V_195 . V_134 == V_131 )
V_176 += F_185 ( V_405 ) ;
if ( ( V_175 = V_165 -> V_183 ) ) {
V_97 = F_63 ( V_175 -> V_104 ) ;
V_176 += sizeof( struct V_89 ) + V_97 ;
}
V_8 = F_41 ( V_176 + 16 , V_63 ) ;
if ( V_8 == NULL )
return - V_31 ;
V_58 = (struct V_57 * ) F_43 ( V_8 , sizeof( struct V_57 ) ) ;
V_58 -> V_319 = V_29 ;
V_58 -> V_320 = V_515 ;
V_58 -> V_146 = F_54 ( V_165 -> V_195 . V_134 ) ;
V_58 -> V_72 = V_176 / sizeof( V_73 ) ;
V_58 -> V_62 = 0 ;
V_58 -> V_322 = 0 ;
V_58 -> V_61 = V_165 -> V_199 . V_303 = F_192 () ;
V_58 -> V_323 = 0 ;
V_78 = (struct V_75 * ) F_43 ( V_8 ,
sizeof( struct V_75 ) + V_179 ) ;
V_78 -> V_86 =
( sizeof( struct V_75 ) + V_179 ) /
sizeof( V_73 ) ;
V_78 -> V_249 = V_120 ;
V_78 -> V_250 = 0 ;
V_78 -> V_251 = 0 ;
V_78 -> V_87 =
F_66 ( & V_165 -> V_182 . V_185 , 0 ,
(struct V_77 * ) ( V_78 + 1 ) ,
V_165 -> V_182 . V_135 ) ;
if ( ! V_78 -> V_87 )
F_74 () ;
V_78 = (struct V_75 * ) F_43 ( V_8 ,
sizeof( struct V_75 ) + V_179 ) ;
V_78 -> V_86 =
( sizeof( struct V_75 ) + V_179 ) /
sizeof( V_73 ) ;
V_78 -> V_249 = V_121 ;
V_78 -> V_250 = 0 ;
V_78 -> V_251 = 0 ;
V_78 -> V_87 =
F_66 ( & V_165 -> V_195 . V_252 , 0 ,
(struct V_77 * ) ( V_78 + 1 ) ,
V_165 -> V_182 . V_135 ) ;
if ( ! V_78 -> V_87 )
F_74 () ;
V_419 = (struct V_418 * ) F_43 ( V_8 , sizeof( struct V_418 ) ) ;
V_419 -> V_420 = sizeof( struct V_418 ) / sizeof( V_73 ) ;
V_419 -> V_425 = V_426 ;
V_419 -> V_427 = V_431 ;
V_419 -> V_433 = V_516 + 1 ;
V_419 -> V_434 = 0 ;
V_419 -> V_435 = V_393 -> V_436 ;
V_419 -> V_437 = V_393 -> V_438 ;
if ( V_165 -> V_195 . V_134 == V_129 )
F_186 ( V_8 , V_405 ) ;
else if ( V_165 -> V_195 . V_134 == V_131 )
F_187 ( V_8 , V_405 ) ;
if ( V_175 ) {
V_90 = (struct V_89 * ) F_43 ( V_8 ,
sizeof( struct V_89 ) + V_97 ) ;
V_90 -> V_93 =
( sizeof( struct V_89 ) + V_97 ) / sizeof( V_73 ) ;
V_90 -> V_99 = V_124 ;
V_90 -> V_101 = V_175 -> V_100 ;
V_90 -> V_103 = V_175 -> V_102 ;
V_90 -> V_91 = V_175 -> V_104 ;
memcpy ( V_90 + 1 , V_175 -> V_286 ,
V_175 -> V_104 ) ;
}
return F_34 ( V_8 , V_63 , V_54 , NULL , F_102 ( V_165 ) ) ;
}
static struct V_392 * F_196 ( struct V_2 * V_3 , int V_517 ,
T_7 * V_60 , int V_83 , int * V_394 )
{
struct V_10 * V_10 = F_7 ( V_3 ) ;
struct V_392 * V_393 ;
struct V_418 * V_419 = (struct V_418 * ) V_60 ;
struct V_89 * V_90 ;
switch ( V_3 -> V_38 ) {
case V_85 :
if ( V_517 != V_518 ) {
* V_394 = - V_305 ;
return NULL ;
}
break;
#if F_45 ( V_80 )
case V_88 :
if ( V_517 != V_519 ) {
* V_394 = - V_305 ;
return NULL ;
}
break;
#endif
default:
* V_394 = - V_70 ;
return NULL ;
}
* V_394 = - V_70 ;
if ( V_83 < sizeof( struct V_418 ) ||
V_419 -> V_420 * 8 > V_83 ||
V_419 -> V_427 > V_520 ||
( ! V_419 -> V_433 || V_419 -> V_433 > V_521 ) )
return NULL ;
V_393 = F_152 ( V_10 , V_63 ) ;
if ( V_393 == NULL ) {
* V_394 = - V_44 ;
return NULL ;
}
V_393 -> V_429 = ( V_419 -> V_427 == V_428 ?
V_445 : V_430 ) ;
V_393 -> V_230 . V_240 = V_448 ;
V_393 -> V_230 . V_233 = V_448 ;
V_393 -> V_230 . V_239 = V_448 ;
V_393 -> V_230 . V_231 = V_448 ;
V_393 -> V_135 = V_3 -> V_38 ;
V_393 -> V_395 = 0 ;
if ( V_419 -> V_427 == V_431 &&
( * V_394 = F_145 ( V_393 , V_419 ) ) < 0 )
goto V_35;
if ( V_83 >= ( V_419 -> V_420 * 8 +
sizeof( struct V_89 ) ) ) {
char * V_74 = ( char * ) V_419 ;
struct V_94 * V_96 ;
V_74 += V_419 -> V_420 * 8 ;
V_90 = (struct V_89 * ) V_74 ;
if ( V_83 < V_419 -> V_420 * 8 +
V_90 -> V_93 ) {
* V_394 = - V_70 ;
goto V_35;
}
if ( ( * V_394 = F_48 ( V_74 ) ) )
goto V_35;
V_96 = F_49 ( V_90 , V_63 ) ;
* V_394 = F_154 ( & V_393 -> V_183 , V_96 , V_63 ) ;
F_82 ( V_96 ) ;
if ( * V_394 )
goto V_35;
}
* V_394 = V_419 -> V_433 - 1 ;
return V_393 ;
V_35:
V_393 -> V_398 . V_449 = 1 ;
F_159 ( V_393 ) ;
return NULL ;
}
static int F_197 ( struct V_143 * V_165 , T_4 * V_522 , T_5 V_254 )
{
struct V_41 * V_8 ;
struct V_57 * V_58 ;
struct V_144 * V_136 ;
struct V_75 * V_78 ;
struct V_192 * V_272 ;
int V_179 ;
int V_176 ;
T_8 V_125 = ( V_165 -> V_195 . V_134 == V_131 ? V_130 : 0 ) ;
struct V_180 * V_181 = NULL ;
V_179 = F_62 ( V_165 -> V_182 . V_135 ) ;
if ( ! V_179 )
return - V_70 ;
if ( ! V_125 )
return - V_70 ;
if ( ! V_165 -> V_189 )
return - V_70 ;
V_181 = V_165 -> V_189 ;
V_176 = sizeof( struct V_57 ) +
sizeof( struct V_144 ) +
( sizeof( struct V_75 ) * 2 ) +
( V_179 * 2 ) +
( sizeof( struct V_192 ) * 2 ) ;
V_8 = F_41 ( V_176 + 16 , V_63 ) ;
if ( V_8 == NULL )
return - V_31 ;
V_58 = (struct V_57 * ) F_43 ( V_8 , sizeof( struct V_57 ) ) ;
V_58 -> V_319 = V_29 ;
V_58 -> V_320 = V_523 ;
V_58 -> V_146 = V_125 ;
V_58 -> V_72 = V_176 / sizeof( V_73 ) ;
V_58 -> V_62 = 0 ;
V_58 -> V_322 = 0 ;
V_58 -> V_61 = V_165 -> V_199 . V_303 = F_192 () ;
V_58 -> V_323 = 0 ;
V_136 = (struct V_144 * ) F_43 ( V_8 , sizeof( struct V_144 ) ) ;
V_136 -> V_193 = sizeof( struct V_144 ) / sizeof( V_73 ) ;
V_136 -> V_194 = V_145 ;
V_136 -> V_148 = V_165 -> V_195 . V_196 ;
V_136 -> V_197 = 0 ;
V_136 -> V_202 = 0 ;
V_136 -> V_209 = 0 ;
V_136 -> V_216 = 0 ;
V_136 -> V_218 = 0 ;
V_78 = (struct V_75 * )
F_43 ( V_8 , sizeof( struct V_75 ) + V_179 ) ;
V_78 -> V_86 =
( sizeof( struct V_75 ) + V_179 ) /
sizeof( V_73 ) ;
V_78 -> V_249 = V_120 ;
V_78 -> V_250 = 0 ;
V_78 -> V_251 = 0 ;
V_78 -> V_87 =
F_66 ( & V_165 -> V_182 . V_185 , 0 ,
(struct V_77 * ) ( V_78 + 1 ) ,
V_165 -> V_182 . V_135 ) ;
if ( ! V_78 -> V_87 )
F_74 () ;
V_272 = (struct V_192 * ) F_43 ( V_8 , sizeof ( * V_272 ) ) ;
V_272 -> V_278 = sizeof( * V_272 ) / sizeof( V_73 ) ;
V_272 -> V_279 = V_280 ;
V_272 -> V_281 = V_181 -> V_282 ;
V_272 -> V_283 = 0 ;
V_78 = (struct V_75 * )
F_43 ( V_8 , sizeof( struct V_75 ) + V_179 ) ;
V_78 -> V_86 =
( sizeof( struct V_75 ) + V_179 ) /
sizeof( V_73 ) ;
V_78 -> V_249 = V_121 ;
V_78 -> V_250 = 0 ;
V_78 -> V_251 = 0 ;
V_78 -> V_87 =
F_66 ( V_522 , 0 ,
(struct V_77 * ) ( V_78 + 1 ) ,
V_165 -> V_182 . V_135 ) ;
if ( ! V_78 -> V_87 )
F_74 () ;
V_272 = (struct V_192 * ) F_43 ( V_8 , sizeof ( * V_272 ) ) ;
V_272 -> V_278 = sizeof( * V_272 ) / sizeof( V_73 ) ;
V_272 -> V_279 = V_284 ;
V_272 -> V_281 = V_254 ;
V_272 -> V_283 = 0 ;
return F_34 ( V_8 , V_63 , V_54 , NULL , F_102 ( V_165 ) ) ;
}
static int F_198 ( struct V_41 * V_8 , int V_524 , int type ,
const struct V_450 * V_184 )
{
struct V_75 * V_78 ;
V_78 = (struct V_75 * ) F_43 ( V_8 , sizeof( struct V_75 ) + V_524 ) ;
V_78 -> V_86 = ( sizeof( struct V_75 ) + V_524 ) / 8 ;
V_78 -> V_249 = type ;
V_78 -> V_250 = V_184 -> V_134 ;
V_78 -> V_251 = 0 ;
switch ( type ) {
case V_120 :
V_78 -> V_87 = V_184 -> V_253 ;
F_66 ( & V_184 -> V_185 , 0 ,
(struct V_77 * ) ( V_78 + 1 ) ,
V_184 -> V_135 ) ;
break;
case V_121 :
V_78 -> V_87 = V_184 -> V_423 ;
F_66 ( & V_184 -> V_252 , 0 ,
(struct V_77 * ) ( V_78 + 1 ) ,
V_184 -> V_135 ) ;
break;
default:
return - V_70 ;
}
return 0 ;
}
static int F_199 ( struct V_41 * V_8 , const struct V_466 * V_467 )
{
struct V_464 * V_465 ;
T_7 * V_136 ;
int V_135 = V_467 -> V_135 ;
int V_415 = F_57 ( V_135 ) ;
int V_525 ;
V_525 = ( sizeof( struct V_464 ) +
F_166 ( V_135 ) ) ;
V_465 = (struct V_464 * ) F_43 ( V_8 , V_525 ) ;
memset ( V_465 , 0 , V_525 ) ;
V_465 -> V_471 = V_525 / 8 ;
V_465 -> V_526 = V_468 ;
V_465 -> V_470 = V_467 -> V_469 ;
V_136 = ( T_7 * ) ( V_465 + 1 ) ;
if ( ! F_66 ( & V_467 -> V_472 , 0 , (struct V_77 * ) V_136 , V_135 ) ||
! F_66 ( & V_467 -> V_473 , 0 , (struct V_77 * ) ( V_136 + V_415 ) , V_135 ) )
return - V_70 ;
return 0 ;
}
static int F_200 ( struct V_41 * V_8 ,
V_71 V_134 , V_71 V_149 , int V_527 ,
T_11 V_270 , V_71 V_135 ,
const T_4 * V_105 , const T_4 * V_106 )
{
struct V_402 * V_403 ;
T_7 * V_136 ;
int V_415 = F_57 ( V_135 ) ;
int V_525 ;
V_525 = sizeof( struct V_402 ) +
F_166 ( V_135 ) ;
V_403 = (struct V_402 * ) F_43 ( V_8 , V_525 ) ;
memset ( V_403 , 0 , V_525 ) ;
V_403 -> V_421 = V_525 ;
V_403 -> V_409 = V_134 ;
V_403 -> V_408 = V_149 ;
V_403 -> V_410 = V_527 ;
V_403 -> V_414 = V_270 ;
V_136 = ( T_7 * ) ( V_403 + 1 ) ;
if ( ! F_66 ( V_105 , 0 , (struct V_77 * ) V_136 , V_135 ) ||
! F_66 ( V_106 , 0 , (struct V_77 * ) ( V_136 + V_415 ) , V_135 ) )
return - V_70 ;
return 0 ;
}
static int F_201 ( const struct V_450 * V_184 , T_7 V_394 , T_7 type ,
const struct V_454 * V_455 , int V_528 ,
const struct V_466 * V_467 )
{
int V_350 ;
int V_529 ;
int V_176 = 0 ;
int V_530 = 0 ;
struct V_41 * V_8 ;
struct V_57 * V_58 ;
struct V_418 * V_419 ;
const struct V_454 * V_531 ;
if ( type != V_401 )
return 0 ;
if ( V_528 <= 0 || V_528 > V_406 )
return - V_70 ;
if ( V_467 != NULL ) {
V_176 += F_63 ( sizeof( struct V_464 ) +
F_166 ( V_467 -> V_135 ) ) ;
}
V_529 = F_62 ( V_184 -> V_135 ) ;
if ( ! V_529 )
return - V_70 ;
V_176 += ( sizeof( struct V_75 ) + V_529 ) * 2 ;
V_530 += sizeof( struct V_418 ) ;
for ( V_350 = 0 , V_531 = V_455 ; V_350 < V_528 ; V_350 ++ , V_531 ++ ) {
V_530 += sizeof( struct V_402 ) +
F_166 ( V_531 -> V_459 ) ;
V_530 += sizeof( struct V_402 ) +
F_166 ( V_531 -> V_462 ) ;
}
V_176 += sizeof( struct V_57 ) + V_530 ;
V_8 = F_41 ( V_176 , V_63 ) ;
if ( V_8 == NULL )
return - V_31 ;
V_58 = (struct V_57 * ) F_43 ( V_8 , sizeof( struct V_57 ) ) ;
V_58 -> V_319 = V_29 ;
V_58 -> V_320 = V_532 ;
V_58 -> V_146 = F_54 ( V_455 -> V_134 ) ;
V_58 -> V_72 = V_176 / 8 ;
V_58 -> V_62 = 0 ;
V_58 -> V_322 = 0 ;
V_58 -> V_61 = 0 ;
V_58 -> V_323 = 0 ;
if ( V_467 != NULL && ( F_199 ( V_8 , V_467 ) < 0 ) )
goto V_23;
F_198 ( V_8 , V_529 , V_120 , V_184 ) ;
F_198 ( V_8 , V_529 , V_121 , V_184 ) ;
V_419 = (struct V_418 * ) F_43 ( V_8 , sizeof( struct V_418 ) ) ;
V_419 -> V_420 = V_530 / 8 ;
V_419 -> V_425 = V_426 ;
V_419 -> V_427 = V_431 ;
V_419 -> V_433 = V_394 + 1 ;
V_419 -> V_434 = 0 ;
V_419 -> V_435 = 0 ;
V_419 -> V_437 = 0 ;
for ( V_350 = 0 , V_531 = V_455 ; V_350 < V_528 ; V_350 ++ , V_531 ++ ) {
int V_149 = F_64 ( V_531 -> V_149 ) ;
if ( V_149 < 0 )
goto V_23;
if ( F_200 ( V_8 , V_531 -> V_134 , V_149 ,
( V_531 -> V_270 ? V_413 : V_440 ) ,
V_531 -> V_270 , V_531 -> V_459 ,
& V_531 -> V_457 , & V_531 -> V_458 ) < 0 )
goto V_23;
if ( F_200 ( V_8 , V_531 -> V_134 , V_149 ,
( V_531 -> V_270 ? V_413 : V_440 ) ,
V_531 -> V_270 , V_531 -> V_462 ,
& V_531 -> V_460 , & V_531 -> V_461 ) < 0 )
goto V_23;
}
F_34 ( V_8 , V_63 , V_52 , NULL , & V_533 ) ;
return 0 ;
V_23:
F_5 ( V_8 ) ;
return - V_70 ;
}
static int F_201 ( const struct V_450 * V_184 , T_7 V_394 , T_7 type ,
const struct V_454 * V_455 , int V_528 ,
const struct V_466 * V_467 )
{
return - V_474 ;
}
static int F_202 ( struct V_534 * V_534 ,
struct V_20 * V_2 , struct V_535 * V_536 , T_12 V_83 )
{
struct V_2 * V_3 = V_2 -> V_3 ;
struct V_41 * V_8 = NULL ;
struct V_57 * V_58 = NULL ;
int V_23 ;
struct V_10 * V_10 = F_7 ( V_3 ) ;
V_23 = - V_305 ;
if ( V_536 -> V_537 & V_538 )
goto V_35;
V_23 = - V_484 ;
if ( ( unsigned int ) V_83 > V_3 -> V_539 - 32 )
goto V_35;
V_23 = - V_44 ;
V_8 = F_41 ( V_83 , V_33 ) ;
if ( V_8 == NULL )
goto V_35;
V_23 = - V_540 ;
if ( F_203 ( F_43 ( V_8 , V_83 ) , V_536 -> V_541 , V_83 ) )
goto V_35;
V_58 = F_181 ( V_8 , & V_23 ) ;
if ( ! V_58 )
goto V_35;
F_15 ( & V_10 -> V_542 . V_543 ) ;
V_23 = F_180 ( V_3 , V_8 , V_58 ) ;
F_17 ( & V_10 -> V_542 . V_543 ) ;
V_35:
if ( V_23 && V_58 && F_40 ( V_58 , V_23 , V_3 ) == 0 )
V_23 = 0 ;
F_5 ( V_8 ) ;
return V_23 ? : V_83 ;
}
static int F_204 ( struct V_534 * V_534 ,
struct V_20 * V_2 , struct V_535 * V_536 , T_12 V_83 ,
int V_219 )
{
struct V_2 * V_3 = V_2 -> V_3 ;
struct V_1 * V_6 = F_1 ( V_3 ) ;
struct V_41 * V_8 ;
int V_544 , V_23 ;
V_23 = - V_70 ;
if ( V_219 & ~ ( V_545 | V_546 | V_547 | V_548 ) )
goto V_35;
V_8 = F_205 ( V_3 , V_219 , V_219 & V_546 , & V_23 ) ;
if ( V_8 == NULL )
goto V_35;
V_544 = V_8 -> V_83 ;
if ( V_544 > V_83 ) {
V_536 -> V_537 |= V_547 ;
V_544 = V_83 ;
}
F_206 ( V_8 ) ;
V_23 = F_207 ( V_8 , 0 , V_536 -> V_541 , V_544 ) ;
if ( V_23 )
goto V_549;
F_208 ( V_536 , V_3 , V_8 ) ;
V_23 = ( V_219 & V_547 ) ? V_8 -> V_83 : V_544 ;
if ( V_6 -> V_7 . V_7 != NULL &&
3 * F_3 ( & V_3 -> V_4 ) <= V_3 -> V_5 )
F_38 ( V_6 ) ;
V_549:
F_209 ( V_3 , V_8 ) ;
V_35:
return V_23 ;
}
static int F_210 ( struct V_550 * V_551 , void * V_552 )
{
struct V_2 * V_553 = F_211 ( V_552 ) ;
if ( V_552 == V_554 )
F_212 ( V_551 , L_4 ) ;
else
F_212 ( V_551 , L_5 ,
V_553 ,
F_3 ( & V_553 -> V_555 ) ,
F_213 ( V_553 ) ,
F_214 ( V_553 ) ,
F_215 ( F_216 ( V_551 ) , F_217 ( V_553 ) ) ,
F_218 ( V_553 )
) ;
return 0 ;
}
static void * F_219 ( struct V_550 * V_551 , T_13 * V_556 )
__acquires( T_14 )
{
struct V_10 * V_10 = F_220 ( V_551 ) ;
struct V_11 * V_12 = F_8 ( V_10 , V_13 ) ;
F_35 () ;
return F_221 ( & V_12 -> V_19 , * V_556 ) ;
}
static void * F_222 ( struct V_550 * V_551 , void * V_552 , T_13 * V_556 )
{
struct V_10 * V_10 = F_220 ( V_551 ) ;
struct V_11 * V_12 = F_8 ( V_10 , V_13 ) ;
return F_223 ( V_552 , & V_12 -> V_19 , V_556 ) ;
}
static void F_224 ( struct V_550 * V_551 , void * V_552 )
__releases( T_14 )
{
F_37 () ;
}
static int F_225 ( struct V_557 * V_557 , struct V_558 * V_558 )
{
return F_226 ( V_557 , V_558 , & V_559 ,
sizeof( struct V_560 ) ) ;
}
static int T_15 F_227 ( struct V_10 * V_10 )
{
struct V_561 * V_562 ;
V_562 = F_228 ( L_6 , 0 , V_10 -> V_563 , & V_564 ) ;
if ( V_562 == NULL )
return - V_31 ;
return 0 ;
}
static void T_16 F_229 ( struct V_10 * V_10 )
{
F_230 ( L_6 , V_10 -> V_563 ) ;
}
static inline int F_227 ( struct V_10 * V_10 )
{
return 0 ;
}
static inline void F_229 ( struct V_10 * V_10 )
{
}
static int T_15 F_231 ( struct V_10 * V_10 )
{
struct V_11 * V_12 = F_8 ( V_10 , V_13 ) ;
int V_565 ;
F_232 ( & V_12 -> V_19 ) ;
F_233 ( & V_12 -> V_17 , 0 ) ;
V_565 = F_227 ( V_10 ) ;
return V_565 ;
}
static void T_16 F_234 ( struct V_10 * V_10 )
{
struct V_11 * V_12 = F_8 ( V_10 , V_13 ) ;
F_229 ( V_10 ) ;
F_42 ( ! F_235 ( & V_12 -> V_19 ) ) ;
}
static void T_17 F_236 ( void )
{
F_237 ( & V_566 ) ;
F_238 ( V_32 ) ;
F_239 ( & V_567 ) ;
F_240 ( & V_34 ) ;
}
static int T_18 F_241 ( void )
{
int V_23 = F_242 ( & V_34 , 0 ) ;
if ( V_23 != 0 )
goto V_35;
V_23 = F_243 ( & V_567 ) ;
if ( V_23 != 0 )
goto V_568;
V_23 = F_244 ( & V_569 ) ;
if ( V_23 != 0 )
goto V_570;
V_23 = F_245 ( & V_566 ) ;
if ( V_23 != 0 )
goto V_571;
V_35:
return V_23 ;
V_571:
F_238 ( V_32 ) ;
V_570:
F_239 ( & V_567 ) ;
V_568:
F_240 ( & V_34 ) ;
goto V_35;
}
