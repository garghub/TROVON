static void F_1 ( struct V_1 * V_2 , T_1 * V_3 , T_2 V_4 )
{
T_2 V_5 , V_6 ;
T_2 V_7 ;
T_2 V_8 ;
T_1 * V_9 ;
T_2 V_10 ;
V_9 = V_2 -> V_9 ;
V_2 -> V_11 = 0 ;
V_2 -> V_12 = 0 ;
for ( V_10 = 0 ; V_10 < 256 ; V_10 ++ )
V_9 [ V_10 ] = ( T_1 ) V_10 ;
V_7 = 0 ;
V_8 = 0 ;
for ( V_10 = 0 ; V_10 < 256 ; V_10 ++ )
{
V_5 = V_9 [ V_10 ] ;
V_8 = ( V_8 + V_3 [ V_7 ] + V_5 ) & 0xff ;
V_6 = V_9 [ V_8 ] ;
V_9 [ V_8 ] = ( T_1 ) V_5 ;
V_9 [ V_10 ] = ( T_1 ) V_6 ;
if ( ++ V_7 >= V_4 )
V_7 = 0 ;
}
}
static T_2 F_2 ( struct V_1 * V_2 )
{
T_2 V_11 ;
T_2 V_12 ;
T_2 V_13 , V_14 ;
T_1 * V_9 ;
V_9 = V_2 -> V_9 ;
V_11 = ( V_2 -> V_11 + 1 ) & 0xff ;
V_13 = V_9 [ V_11 ] ;
V_12 = ( V_13 + V_2 -> V_12 ) & 0xff ;
V_14 = V_9 [ V_12 ] ;
V_2 -> V_11 = V_11 ;
V_2 -> V_12 = V_12 ;
V_9 [ V_12 ] = ( T_1 ) V_13 ;
V_9 [ V_11 ] = ( T_1 ) V_14 ;
return V_9 [ ( V_13 + V_14 ) & 0xff ] ;
}
static void F_3 ( struct V_1 * V_2 ,
T_1 * V_15 ,
T_1 * V_16 ,
T_2 V_17 )
{
T_2 V_18 ;
for ( V_18 = 0 ; V_18 < V_17 ; V_18 ++ )
V_15 [ V_18 ] = V_16 [ V_18 ] ^ ( unsigned char ) F_2 ( V_2 ) ;
}
static T_1 F_4 ( T_1 V_19 )
{
T_1 V_20 = ( ( V_19 << 7 ) & 0x80 ) | ( ( V_19 << 5 ) & 0x40 ) |
( ( V_19 << 3 ) & 0x20 ) | ( ( V_19 << 1 ) & 0x10 ) |
( ( V_19 >> 1 ) & 0x08 ) | ( ( V_19 >> 3 ) & 0x04 ) |
( ( V_19 >> 5 ) & 0x02 ) | ( ( V_19 >> 7 ) & 0x01 ) ;
return V_20 ;
}
static void F_5 ( void )
{
if ( V_21 == 1 )
return;
else{
int V_18 , V_22 ;
T_2 V_23 ;
T_1 * V_24 = ( T_1 * ) & V_23 , * V_25 ;
T_1 V_26 ;
V_23 = 0x12340000 ;
for ( V_18 = 0 ; V_18 < 256 ; ++ V_18 )
{
V_26 = F_4 ( ( T_1 ) V_18 ) ;
for ( V_23 = ( ( T_2 ) V_26 ) << 24 , V_22 = 8 ; V_22 > 0 ; -- V_22 ) {
V_23 = V_23 & 0x80000000 ? ( V_23 << 1 ) ^ V_27 : ( V_23 << 1 ) ;
}
V_25 = ( T_1 * ) & V_28 [ V_18 ] ;
V_25 [ 0 ] = F_4 ( V_24 [ 3 ] ) ;
V_25 [ 1 ] = F_4 ( V_24 [ 2 ] ) ;
V_25 [ 2 ] = F_4 ( V_24 [ 1 ] ) ;
V_25 [ 3 ] = F_4 ( V_24 [ 0 ] ) ;
}
V_21 = 1 ;
}
}
static T_2 F_6 ( T_1 * V_29 , int V_17 )
{
T_1 * V_24 ;
T_2 V_30 ;
if ( V_21 == 0 ) F_5 () ;
V_30 = 0xffffffff ;
for ( V_24 = V_29 ; V_17 > 0 ; ++ V_24 , -- V_17 )
V_30 = V_28 [ ( V_30 ^ * V_24 ) & 0xff ] ^ ( V_30 >> 8 ) ;
return ~ V_30 ;
}
void F_7 ( struct V_31 * V_32 ,
struct V_33 * V_34 )
{
unsigned char V_30 [ 4 ] ;
struct V_1 V_35 ;
int V_36 , V_37 , V_38 ;
T_2 V_39 ;
T_1 * V_40 , * V_41 , * V_42 ;
T_1 V_43 [ 16 ] ;
T_1 V_44 = 0 ;
struct V_45 * V_46 = & V_34 -> V_47 ;
struct V_48 * V_49 = & V_32 -> V_50 ;
struct V_51 * V_52 = & V_32 -> V_53 ;
if ( ! V_34 -> V_54 )
return;
V_44 = V_55 ;
V_40 = V_34 -> V_54 + V_44 ;
if ( ( V_46 -> V_56 != V_57 ) && ( V_46 -> V_56 != V_58 ) )
return;
V_38 = V_49 -> V_59 ;
V_39 = V_49 -> V_60 [ V_38 ] ;
for ( V_36 = 0 ; V_36 < V_46 -> V_61 ; V_36 ++ ) {
V_42 = V_40 + V_46 -> V_62 ;
memcpy ( & V_43 [ 0 ] , V_42 , 3 ) ;
memcpy ( & V_43 [ 3 ] , & V_49 -> V_63 [ V_38 ] . V_64 [ 0 ] ,
V_39 ) ;
V_41 = V_40 + V_46 -> V_65 + V_46 -> V_62 ;
if ( ( V_36 + 1 ) == V_46 -> V_61 ) {
V_37 = V_46 -> V_66 - V_46 -> V_62 -
V_46 -> V_65 - V_46 -> V_67 ;
* ( ( T_2 * ) V_30 ) = F_8 ( F_6 ( V_41 , V_37 ) ) ;
F_1 ( & V_35 , V_43 , 3 + V_39 ) ;
F_3 ( & V_35 , V_41 , V_41 , V_37 ) ;
F_3 ( & V_35 , V_41 + V_37 , V_30 , 4 ) ;
} else {
V_37 = V_52 -> V_68 - V_46 -> V_62 -
V_46 -> V_65 - V_46 -> V_67 ;
* ( ( T_2 * ) V_30 ) = F_8 ( F_6 ( V_41 , V_37 ) ) ;
F_1 ( & V_35 , V_43 , 3 + V_39 ) ;
F_3 ( & V_35 , V_41 , V_41 , V_37 ) ;
F_3 ( & V_35 , V_41 + V_37 , V_30 , 4 ) ;
V_40 += V_52 -> V_68 ;
V_40 = F_9 ( V_40 , 4 ) ;
}
}
}
void F_10 ( struct V_31 * V_32 ,
struct V_69 * V_70 )
{
T_1 V_30 [ 4 ] ;
struct V_1 V_35 ;
int V_37 ;
T_2 V_39 ;
T_1 * V_40 , * V_41 , * V_42 , V_43 [ 16 ] ;
T_1 V_7 ;
struct V_71 * V_72 = & V_70 -> V_47 ;
struct V_48 * V_49 = & V_32 -> V_50 ;
struct V_73 * V_74 = V_70 -> V_75 ;
V_40 = V_74 -> V_19 ;
if ( ( V_72 -> V_56 != V_57 ) && ( V_72 -> V_56 != V_58 ) )
return;
V_42 = V_40 + V_72 -> V_62 ;
V_7 = V_72 -> V_76 ;
V_39 = V_49 -> V_60 [ V_7 ] ;
memcpy ( & V_43 [ 0 ] , V_42 , 3 ) ;
memcpy ( & V_43 [ 3 ] , & V_49 -> V_63 [ V_7 ] . V_64 [ 0 ] ,
V_39 ) ;
V_37 = V_74 -> V_17 - V_72 -> V_62 - V_72 -> V_65 ;
V_41 = V_40 + V_72 -> V_65 + V_72 -> V_62 ;
F_1 ( & V_35 , V_43 , 3 + V_39 ) ;
F_3 ( & V_35 , V_41 , V_41 , V_37 ) ;
* ( ( T_2 * ) V_30 ) = F_11 ( F_6 ( V_41 , V_37 - 4 ) ) ;
if ( V_30 [ 3 ] != V_41 [ V_37 - 1 ] || V_30 [ 2 ] != V_41 [ V_37 - 2 ] ||
V_30 [ 1 ] != V_41 [ V_37 - 3 ] || V_30 [ 0 ] != V_41 [ V_37 - 4 ] ) {
F_12 ( V_77 , V_78 ,
( L_1
L_2
L_3
L_4 ,
V_30 [ 3 ] , V_41 [ V_37 - 1 ] ,
V_30 [ 2 ] , V_41 [ V_37 - 2 ] ,
V_30 [ 1 ] , V_41 [ V_37 - 3 ] ,
V_30 [ 0 ] , V_41 [ V_37 - 4 ] ) ) ;
}
return;
}
static T_2 F_13 ( T_1 * V_24 )
{
T_3 V_18 ;
T_2 V_79 = 0 ;
for ( V_18 = 0 ; V_18 < 4 ; V_18 ++ )
{
V_79 |= ( ( T_2 ) ( * V_24 ++ ) ) << ( 8 * V_18 ) ;
}
return V_79 ;
}
static void F_14 ( T_1 * V_24 , T_2 V_80 )
{
long V_18 ;
for ( V_18 = 0 ; V_18 < 4 ; V_18 ++ )
{
* V_24 ++ = ( T_1 ) ( V_80 & 0xff ) ;
V_80 >>= 8 ;
}
}
static void F_15 ( struct V_81 * V_82 )
{
V_82 -> V_83 = V_82 -> V_84 ;
V_82 -> V_85 = V_82 -> V_86 ;
V_82 -> V_87 = 0 ;
V_82 -> V_88 = 0 ;
}
void F_16 ( struct V_81 * V_82 , T_1 * V_3 )
{
V_82 -> V_84 = F_13 ( V_3 ) ;
V_82 -> V_86 = F_13 ( V_3 + 4 ) ;
F_15 ( V_82 ) ;
}
void F_17 ( struct V_81 * V_82 , T_1 V_89 )
{
V_82 -> V_88 |= ( ( unsigned long ) V_89 ) << ( 8 * V_82 -> V_87 ) ;
V_82 -> V_87 ++ ;
if ( V_82 -> V_87 >= 4 )
{
V_82 -> V_83 ^= V_82 -> V_88 ;
V_82 -> V_85 ^= F_18 ( V_82 -> V_83 , 17 ) ;
V_82 -> V_83 += V_82 -> V_85 ;
V_82 -> V_85 ^= ( ( V_82 -> V_83 & 0xff00ff00 ) >> 8 ) | ( ( V_82 -> V_83 & 0x00ff00ff ) << 8 ) ;
V_82 -> V_83 += V_82 -> V_85 ;
V_82 -> V_85 ^= F_18 ( V_82 -> V_83 , 3 ) ;
V_82 -> V_83 += V_82 -> V_85 ;
V_82 -> V_85 ^= F_19 ( V_82 -> V_83 , 2 ) ;
V_82 -> V_83 += V_82 -> V_85 ;
V_82 -> V_88 = 0 ;
V_82 -> V_87 = 0 ;
}
}
void F_20 ( struct V_81 * V_82 , T_1 * V_16 , T_2 V_90 )
{
while( V_90 > 0 )
{
F_17 ( V_82 , * V_16 ++ ) ;
V_90 -- ;
}
}
void F_21 ( struct V_81 * V_82 , T_1 * V_91 )
{
F_17 ( V_82 , 0x5a ) ;
F_17 ( V_82 , 0 ) ;
F_17 ( V_82 , 0 ) ;
F_17 ( V_82 , 0 ) ;
F_17 ( V_82 , 0 ) ;
while( V_82 -> V_87 != 0 )
{
F_17 ( V_82 , 0 ) ;
}
F_14 ( V_91 , V_82 -> V_83 ) ;
F_14 ( V_91 + 4 , V_82 -> V_85 ) ;
F_15 ( V_82 ) ;
}
void F_22 ( T_1 * V_3 , T_1 * V_92 , T_1 * V_19 , T_2 V_93 , T_1 * V_94 , T_1 V_95 )
{
struct V_81 V_96 ;
T_1 V_97 [ 4 ] = { 0x0 , 0x0 , 0x0 , 0x0 } ;
F_16 ( & V_96 , V_3 ) ;
V_97 [ 0 ] = V_95 ;
if ( V_92 [ 1 ] & 1 ) {
F_20 ( & V_96 , & V_92 [ 16 ] , 6 ) ;
if ( V_92 [ 1 ] & 2 )
F_20 ( & V_96 , & V_92 [ 24 ] , 6 ) ;
else
F_20 ( & V_96 , & V_92 [ 10 ] , 6 ) ;
}
else{
F_20 ( & V_96 , & V_92 [ 4 ] , 6 ) ;
if ( V_92 [ 1 ] & 2 )
F_20 ( & V_96 , & V_92 [ 16 ] , 6 ) ;
else
F_20 ( & V_96 , & V_92 [ 10 ] , 6 ) ;
}
F_20 ( & V_96 , & V_97 [ 0 ] , 4 ) ;
F_20 ( & V_96 , V_19 , V_93 ) ;
F_21 ( & V_96 , V_94 ) ;
}
static void F_23 ( T_4 * V_98 , const T_1 * V_99 , const T_1 * V_100 , T_2 V_101 )
{
int V_18 ;
V_98 [ 0 ] = F_24 ( V_101 ) ;
V_98 [ 1 ] = F_25 ( V_101 ) ;
V_98 [ 2 ] = F_26 ( V_100 [ 1 ] , V_100 [ 0 ] ) ;
V_98 [ 3 ] = F_26 ( V_100 [ 3 ] , V_100 [ 2 ] ) ;
V_98 [ 4 ] = F_26 ( V_100 [ 5 ] , V_100 [ 4 ] ) ;
for ( V_18 = 0 ; V_18 < V_102 ; V_18 ++ )
{
V_98 [ 0 ] += F_27 ( V_98 [ 4 ] ^ F_28 ( ( V_18 & 1 ) + 0 ) ) ;
V_98 [ 1 ] += F_27 ( V_98 [ 0 ] ^ F_28 ( ( V_18 & 1 ) + 2 ) ) ;
V_98 [ 2 ] += F_27 ( V_98 [ 1 ] ^ F_28 ( ( V_18 & 1 ) + 4 ) ) ;
V_98 [ 3 ] += F_27 ( V_98 [ 2 ] ^ F_28 ( ( V_18 & 1 ) + 6 ) ) ;
V_98 [ 4 ] += F_27 ( V_98 [ 3 ] ^ F_28 ( ( V_18 & 1 ) + 0 ) ) ;
V_98 [ 4 ] += ( unsigned short ) V_18 ;
}
}
static void F_29 ( T_1 * V_103 , const T_1 * V_99 , const T_4 * V_98 , T_4 V_104 )
{
int V_18 ;
T_4 V_105 [ 6 ] ;
for ( V_18 = 0 ; V_18 < 5 ; V_18 ++ ) V_105 [ V_18 ] = V_98 [ V_18 ] ;
V_105 [ 5 ] = V_98 [ 4 ] + V_104 ;
V_105 [ 0 ] += F_27 ( V_105 [ 5 ] ^ F_28 ( 0 ) ) ;
V_105 [ 1 ] += F_27 ( V_105 [ 0 ] ^ F_28 ( 1 ) ) ;
V_105 [ 2 ] += F_27 ( V_105 [ 1 ] ^ F_28 ( 2 ) ) ;
V_105 [ 3 ] += F_27 ( V_105 [ 2 ] ^ F_28 ( 3 ) ) ;
V_105 [ 4 ] += F_27 ( V_105 [ 3 ] ^ F_28 ( 4 ) ) ;
V_105 [ 5 ] += F_27 ( V_105 [ 4 ] ^ F_28 ( 5 ) ) ;
V_105 [ 0 ] += F_30 ( V_105 [ 5 ] ^ F_28 ( 6 ) ) ;
V_105 [ 1 ] += F_30 ( V_105 [ 0 ] ^ F_28 ( 7 ) ) ;
V_105 [ 2 ] += F_30 ( V_105 [ 1 ] ) ;
V_105 [ 3 ] += F_30 ( V_105 [ 2 ] ) ;
V_105 [ 4 ] += F_30 ( V_105 [ 3 ] ) ;
V_105 [ 5 ] += F_30 ( V_105 [ 4 ] ) ;
V_103 [ 0 ] = F_31 ( V_104 ) ;
V_103 [ 1 ] = ( F_31 ( V_104 ) | 0x20 ) & 0x7F ;
V_103 [ 2 ] = F_32 ( V_104 ) ;
V_103 [ 3 ] = F_32 ( ( V_105 [ 5 ] ^ F_28 ( 0 ) ) >> 1 ) ;
for ( V_18 = 0 ; V_18 < 6 ; V_18 ++ )
{
V_103 [ 4 + 2 * V_18 ] = F_32 ( V_105 [ V_18 ] ) ;
V_103 [ 5 + 2 * V_18 ] = F_31 ( V_105 [ V_18 ] ) ;
}
}
T_2 F_33 ( struct V_31 * V_32 ,
struct V_33 * V_34 )
{
T_4 V_106 ;
T_2 V_107 ;
T_1 V_103 [ 16 ] ;
T_1 V_108 [ 16 ] ;
T_1 V_30 [ 4 ] ;
T_1 V_44 = 0 ;
struct V_1 V_35 ;
int V_36 , V_37 ;
T_2 V_109 ;
T_1 * V_40 , * V_41 , * V_42 , * V_110 ;
union V_111 V_112 ;
struct V_113 * V_114 ;
struct V_45 * V_46 = & V_34 -> V_47 ;
struct V_48 * V_49 = & V_32 -> V_50 ;
struct V_51 * V_52 = & V_32 -> V_53 ;
T_2 V_79 = V_115 ;
if ( ! V_34 -> V_54 )
return V_116 ;
V_44 = V_55 ;
V_40 = V_34 -> V_54 + V_44 ;
if ( V_46 -> V_56 == V_117 ) {
if ( V_46 -> V_118 )
{
V_114 = V_46 -> V_118 ;
}
else
{
F_34 ( L_5 , V_119 ) ;
V_114 = F_35 ( & V_32 -> V_120 ,
& V_46 -> V_121 [ 0 ] ) ;
}
if ( V_114 != NULL ) {
if ( ! ( V_114 -> V_9 & V_122 ) )
{
F_34 ( L_6 , V_119 , V_114 -> V_9 ) ;
return V_116 ;
}
F_12 ( V_77 , V_78 , ( L_7 ) ) ;
if ( F_36 ( V_46 -> V_121 ) )
V_110 = V_49 -> V_123 [ V_49 -> V_124 ] . V_64 ;
else
V_110 = & V_114 -> V_125 . V_64 [ 0 ] ;
V_109 = 16 ;
for ( V_36 = 0 ; V_36 < V_46 -> V_61 ; V_36 ++ ) {
V_42 = V_40 + V_46 -> V_62 ;
V_41 = V_40 + V_46 -> V_65 + V_46 -> V_62 ;
F_37 ( V_42 , V_112 ) ;
V_106 = ( T_4 ) ( V_112 . V_80 ) ;
V_107 = ( T_2 ) ( V_112 . V_80 >> 16 ) ;
F_23 ( ( T_4 * ) & V_108 [ 0 ] , V_110 , & V_46 -> V_100 [ 0 ] , V_107 ) ;
F_29 ( & V_103 [ 0 ] , V_110 , ( T_4 * ) & V_108 [ 0 ] , V_106 ) ;
if ( ( V_36 + 1 ) == V_46 -> V_61 ) {
V_37 = V_46 -> V_66 - V_46 -> V_62 - V_46 -> V_65 - V_46 -> V_67 ;
F_12 ( V_77 , V_126 , ( L_8 , V_46 -> V_65 , V_46 -> V_67 ) ) ;
* ( ( T_2 * ) V_30 ) = F_8 ( F_6 ( V_41 , V_37 ) ) ;
F_1 ( & V_35 , V_103 , 16 ) ;
F_3 ( & V_35 , V_41 , V_41 , V_37 ) ;
F_3 ( & V_35 , V_41 + V_37 , V_30 , 4 ) ;
}
else{
V_37 = V_52 -> V_68 - V_46 -> V_62 - V_46 -> V_65 - V_46 -> V_67 ;
* ( ( T_2 * ) V_30 ) = F_8 ( F_6 ( V_41 , V_37 ) ) ;
F_1 ( & V_35 , V_103 , 16 ) ;
F_3 ( & V_35 , V_41 , V_41 , V_37 ) ;
F_3 ( & V_35 , V_41 + V_37 , V_30 , 4 ) ;
V_40 += V_52 -> V_68 ;
V_40 = F_9 ( V_40 , 4 ) ;
}
}
}
else{
F_12 ( V_77 , V_78 , ( L_9 ) ) ;
F_34 ( L_10 , V_119 ) ;
V_79 = V_116 ;
}
}
return V_79 ;
}
T_2 F_38 ( struct V_31 * V_32 ,
struct V_69 * V_70 )
{
T_4 V_106 ;
T_2 V_107 ;
T_1 V_103 [ 16 ] ;
T_1 V_108 [ 16 ] ;
T_1 V_30 [ 4 ] ;
struct V_1 V_35 ;
int V_37 ;
T_2 V_109 ;
T_1 * V_40 , * V_41 , * V_42 , * V_110 ;
union V_111 V_112 ;
struct V_113 * V_114 ;
struct V_71 * V_72 = & V_70 -> V_47 ;
struct V_48 * V_49 = & V_32 -> V_50 ;
struct V_73 * V_74 = V_70 -> V_75 ;
T_2 V_79 = V_115 ;
V_40 = V_74 -> V_19 ;
if ( V_72 -> V_56 == V_117 ) {
V_114 = F_35 ( & V_32 -> V_120 ,
& V_72 -> V_100 [ 0 ] ) ;
if ( V_114 != NULL ) {
if ( F_36 ( V_72 -> V_121 ) ) {
if ( V_49 -> V_127 == false ) {
V_79 = V_116 ;
F_34 ( L_11 , V_119 ) ;
goto exit;
}
V_110 = V_49 -> V_123 [ V_72 -> V_76 ] . V_64 ;
V_109 = 16 ;
} else {
F_12 ( V_77 , V_78 , ( L_12 ) ) ;
V_110 = & V_114 -> V_125 . V_64 [ 0 ] ;
V_109 = 16 ;
}
V_42 = V_40 + V_72 -> V_62 ;
V_41 = V_40 + V_72 -> V_65 + V_72 -> V_62 ;
V_37 = V_74 -> V_17 - V_72 -> V_62 - V_72 -> V_65 ;
F_37 ( V_42 , V_112 ) ;
V_106 = ( T_4 ) ( V_112 . V_80 ) ;
V_107 = ( T_2 ) ( V_112 . V_80 >> 16 ) ;
F_23 ( ( T_4 * ) & V_108 [ 0 ] , V_110 , & V_72 -> V_100 [ 0 ] , V_107 ) ;
F_29 ( & V_103 [ 0 ] , V_110 , ( unsigned short * ) & V_108 [ 0 ] , V_106 ) ;
F_1 ( & V_35 , V_103 , 16 ) ;
F_3 ( & V_35 , V_41 , V_41 , V_37 ) ;
* ( ( T_2 * ) V_30 ) = F_11 ( F_6 ( V_41 , V_37 - 4 ) ) ;
if ( V_30 [ 3 ] != V_41 [ V_37 - 1 ] || V_30 [ 2 ] != V_41 [ V_37 - 2 ] || V_30 [ 1 ] != V_41 [ V_37 - 3 ] || V_30 [ 0 ] != V_41 [ V_37 - 4 ] )
{
F_12 ( V_77 , V_78 , ( L_13 ,
V_30 [ 3 ] , V_41 [ V_37 - 1 ] , V_30 [ 2 ] , V_41 [ V_37 - 2 ] , V_30 [ 1 ] , V_41 [ V_37 - 3 ] , V_30 [ 0 ] , V_41 [ V_37 - 4 ] ) ) ;
V_79 = V_116 ;
}
} else {
F_12 ( V_77 , V_78 , ( L_14 ) ) ;
V_79 = V_116 ;
}
}
exit:
return V_79 ;
}
static void F_39 ( T_1 * V_128 , T_1 * V_89 , T_1 * V_129 )
{
int V_18 ;
for ( V_18 = 0 ; V_18 < 16 ; V_18 ++ )
V_129 [ V_18 ] = V_128 [ V_18 ] ^ V_89 [ V_18 ] ;
}
static void F_40 ( T_1 * V_128 , T_1 * V_89 , T_1 * V_129 )
{
int V_18 ;
for ( V_18 = 0 ; V_18 < 4 ; V_18 ++ )
V_129 [ V_18 ] = V_128 [ V_18 ] ^ V_89 [ V_18 ] ;
}
static T_1 F_41 ( T_1 V_128 )
{
return V_130 [ ( int ) V_128 ] ;
}
static void F_42 ( T_1 * V_3 , int V_131 )
{
T_1 V_132 ;
T_1 V_133 [ 4 ] ;
T_1 V_134 [ 12 ] =
{
0x01 , 0x02 , 0x04 , 0x08 , 0x10 , 0x20 , 0x40 , 0x80 ,
0x1b , 0x36 , 0x36 , 0x36
} ;
V_133 [ 0 ] = F_41 ( V_3 [ 13 ] ) ;
V_133 [ 1 ] = F_41 ( V_3 [ 14 ] ) ;
V_133 [ 2 ] = F_41 ( V_3 [ 15 ] ) ;
V_133 [ 3 ] = F_41 ( V_3 [ 12 ] ) ;
V_132 = V_134 [ V_131 ] ;
F_40 ( & V_3 [ 0 ] , V_133 , & V_3 [ 0 ] ) ;
V_3 [ 0 ] = V_3 [ 0 ] ^ V_132 ;
F_40 ( & V_3 [ 4 ] , & V_3 [ 0 ] , & V_3 [ 4 ] ) ;
F_40 ( & V_3 [ 8 ] , & V_3 [ 4 ] , & V_3 [ 8 ] ) ;
F_40 ( & V_3 [ 12 ] , & V_3 [ 8 ] , & V_3 [ 12 ] ) ;
}
static void F_43 ( T_1 * V_135 , T_1 * V_129 )
{
int V_18 ;
for ( V_18 = 0 ; V_18 < 16 ; V_18 ++ )
{
V_129 [ V_18 ] = F_41 ( V_135 [ V_18 ] ) ;
}
}
static void F_44 ( T_1 * V_135 , T_1 * V_129 )
{
V_129 [ 0 ] = V_135 [ 0 ] ;
V_129 [ 1 ] = V_135 [ 5 ] ;
V_129 [ 2 ] = V_135 [ 10 ] ;
V_129 [ 3 ] = V_135 [ 15 ] ;
V_129 [ 4 ] = V_135 [ 4 ] ;
V_129 [ 5 ] = V_135 [ 9 ] ;
V_129 [ 6 ] = V_135 [ 14 ] ;
V_129 [ 7 ] = V_135 [ 3 ] ;
V_129 [ 8 ] = V_135 [ 8 ] ;
V_129 [ 9 ] = V_135 [ 13 ] ;
V_129 [ 10 ] = V_135 [ 2 ] ;
V_129 [ 11 ] = V_135 [ 7 ] ;
V_129 [ 12 ] = V_135 [ 12 ] ;
V_129 [ 13 ] = V_135 [ 1 ] ;
V_129 [ 14 ] = V_135 [ 6 ] ;
V_129 [ 15 ] = V_135 [ 11 ] ;
}
static void F_45 ( T_1 * V_135 , T_1 * V_129 )
{
int V_18 ;
T_1 V_136 [ 4 ] ;
T_1 V_137 [ 4 ] ;
T_1 V_138 [ 4 ] ;
T_1 V_139 [ 4 ] ;
T_1 V_140 [ 4 ] ;
T_1 V_141 [ 4 ] ;
T_1 V_142 [ 4 ] ;
T_1 V_143 [ 4 ] ;
for ( V_18 = 0 ; V_18 < 4 ; V_18 ++ )
{
if ( ( V_135 [ V_18 ] & 0x80 ) == 0x80 )
V_136 [ V_18 ] = 0x1b ;
else
V_136 [ V_18 ] = 0x00 ;
}
V_139 [ 0 ] = V_135 [ 2 ] ;
V_139 [ 1 ] = V_135 [ 3 ] ;
V_139 [ 2 ] = V_135 [ 0 ] ;
V_139 [ 3 ] = V_135 [ 1 ] ;
V_138 [ 0 ] = V_135 [ 3 ] ;
V_138 [ 1 ] = V_135 [ 0 ] ;
V_138 [ 2 ] = V_135 [ 1 ] ;
V_138 [ 3 ] = V_135 [ 2 ] ;
V_140 [ 0 ] = V_135 [ 0 ] & 0x7f ;
V_140 [ 1 ] = V_135 [ 1 ] & 0x7f ;
V_140 [ 2 ] = V_135 [ 2 ] & 0x7f ;
V_140 [ 3 ] = V_135 [ 3 ] & 0x7f ;
for ( V_18 = 3 ; V_18 > 0 ; V_18 -- )
{
V_140 [ V_18 ] = V_140 [ V_18 ] << 1 ;
if ( ( V_140 [ V_18 - 1 ] & 0x80 ) == 0x80 )
{
V_140 [ V_18 ] = ( V_140 [ V_18 ] | 0x01 ) ;
}
}
V_140 [ 0 ] = V_140 [ 0 ] << 1 ;
V_140 [ 0 ] = V_140 [ 0 ] & 0xfe ;
F_40 ( V_136 , V_140 , V_137 ) ;
F_40 ( V_135 , V_137 , V_141 ) ;
V_142 [ 0 ] = V_141 [ 0 ] ;
V_141 [ 0 ] = V_141 [ 1 ] ;
V_141 [ 1 ] = V_141 [ 2 ] ;
V_141 [ 2 ] = V_141 [ 3 ] ;
V_141 [ 3 ] = V_142 [ 0 ] ;
F_40 ( V_137 , V_141 , V_142 ) ;
F_40 ( V_139 , V_138 , V_143 ) ;
F_40 ( V_142 , V_143 , V_129 ) ;
}
static void F_46 ( T_1 * V_3 , T_1 * V_19 , T_1 * V_144 )
{
int V_131 ;
int V_18 ;
T_1 V_145 [ 16 ] ;
T_1 V_146 [ 16 ] ;
T_1 V_147 [ 16 ] ;
for ( V_18 = 0 ; V_18 < 16 ; V_18 ++ ) V_147 [ V_18 ] = V_3 [ V_18 ] ;
for ( V_131 = 0 ; V_131 < 11 ; V_131 ++ )
{
if ( V_131 == 0 )
{
F_39 ( V_147 , V_19 , V_144 ) ;
F_42 ( V_147 , V_131 ) ;
}
else if ( V_131 == 10 )
{
F_43 ( V_144 , V_145 ) ;
F_44 ( V_145 , V_146 ) ;
F_39 ( V_146 , V_147 , V_144 ) ;
}
else
{
F_43 ( V_144 , V_145 ) ;
F_44 ( V_145 , V_146 ) ;
F_45 ( & V_146 [ 0 ] , & V_145 [ 0 ] ) ;
F_45 ( & V_146 [ 4 ] , & V_145 [ 4 ] ) ;
F_45 ( & V_146 [ 8 ] , & V_145 [ 8 ] ) ;
F_45 ( & V_146 [ 12 ] , & V_145 [ 12 ] ) ;
F_39 ( V_145 , V_147 , V_144 ) ;
F_42 ( V_147 , V_131 ) ;
}
}
}
static void F_47 ( T_1 * V_148 , int V_149 , int V_150 , T_1 * V_151 ,
T_5 V_152 , T_1 * V_153 )
{
int V_18 ;
V_148 [ 0 ] = 0x59 ;
if ( V_149 && V_150 )
V_148 [ 1 ] = V_151 [ 30 ] & 0x0f ;
if ( V_149 && ! V_150 )
V_148 [ 1 ] = V_151 [ 24 ] & 0x0f ;
if ( ! V_149 )
V_148 [ 1 ] = 0x00 ;
for ( V_18 = 2 ; V_18 < 8 ; V_18 ++ )
V_148 [ V_18 ] = V_151 [ V_18 + 8 ] ;
for ( V_18 = 8 ; V_18 < 14 ; V_18 ++ )
V_148 [ V_18 ] = V_153 [ 13 - V_18 ] ;
V_148 [ 14 ] = ( unsigned char ) ( V_152 / 256 ) ;
V_148 [ 15 ] = ( unsigned char ) ( V_152 % 256 ) ;
}
static void F_48 ( T_1 * V_154 , int V_155 , T_1 * V_151 )
{
V_154 [ 0 ] = ( T_1 ) ( ( V_155 - 2 ) / 256 ) ;
V_154 [ 1 ] = ( T_1 ) ( ( V_155 - 2 ) % 256 ) ;
V_154 [ 2 ] = V_151 [ 0 ] & 0xcf ;
V_154 [ 3 ] = V_151 [ 1 ] & 0xc7 ;
V_154 [ 4 ] = V_151 [ 4 ] ;
V_154 [ 5 ] = V_151 [ 5 ] ;
V_154 [ 6 ] = V_151 [ 6 ] ;
V_154 [ 7 ] = V_151 [ 7 ] ;
V_154 [ 8 ] = V_151 [ 8 ] ;
V_154 [ 9 ] = V_151 [ 9 ] ;
V_154 [ 10 ] = V_151 [ 10 ] ;
V_154 [ 11 ] = V_151 [ 11 ] ;
V_154 [ 12 ] = V_151 [ 12 ] ;
V_154 [ 13 ] = V_151 [ 13 ] ;
V_154 [ 14 ] = V_151 [ 14 ] ;
V_154 [ 15 ] = V_151 [ 15 ] ;
}
static void F_49 (
T_1 * V_156 ,
T_1 * V_151 ,
int V_150 ,
int V_149
)
{
int V_18 ;
for ( V_18 = 0 ; V_18 < 16 ; V_18 ++ ) V_156 [ V_18 ] = 0x00 ;
V_156 [ 0 ] = V_151 [ 16 ] ;
V_156 [ 1 ] = V_151 [ 17 ] ;
V_156 [ 2 ] = V_151 [ 18 ] ;
V_156 [ 3 ] = V_151 [ 19 ] ;
V_156 [ 4 ] = V_151 [ 20 ] ;
V_156 [ 5 ] = V_151 [ 21 ] ;
V_156 [ 6 ] = 0x00 ;
V_156 [ 7 ] = 0x00 ;
if ( ! V_149 && V_150 )
{
for ( V_18 = 0 ; V_18 < 6 ; V_18 ++ ) V_156 [ 8 + V_18 ] = V_151 [ 24 + V_18 ] ;
}
if ( V_149 && ! V_150 )
{
V_156 [ 8 ] = V_151 [ 24 ] & 0x0f ;
V_156 [ 9 ] = V_151 [ 25 ] & 0x00 ;
}
if ( V_149 && V_150 )
{
for ( V_18 = 0 ; V_18 < 6 ; V_18 ++ ) V_156 [ 8 + V_18 ] = V_151 [ 24 + V_18 ] ;
V_156 [ 14 ] = V_151 [ 30 ] & 0x0f ;
V_156 [ 15 ] = V_151 [ 31 ] & 0x00 ;
}
}
static void F_50 ( T_1 * V_157 , int V_150 , int V_149 ,
T_1 * V_151 , T_1 * V_153 , int V_23 )
{
int V_18 = 0 ;
for ( V_18 = 0 ; V_18 < 16 ; V_18 ++ ) V_157 [ V_18 ] = 0x00 ;
V_18 = 0 ;
V_157 [ 0 ] = 0x01 ;
if ( V_149 && V_150 )
V_157 [ 1 ] = V_151 [ 30 ] & 0x0f ;
if ( V_149 && ! V_150 )
V_157 [ 1 ] = V_151 [ 24 ] & 0x0f ;
for ( V_18 = 2 ; V_18 < 8 ; V_18 ++ )
V_157 [ V_18 ] = V_151 [ V_18 + 8 ] ;
for ( V_18 = 8 ; V_18 < 14 ; V_18 ++ )
V_157 [ V_18 ] = V_153 [ 13 - V_18 ] ;
V_157 [ 14 ] = ( unsigned char ) ( V_23 / 256 ) ;
V_157 [ 15 ] = ( unsigned char ) ( V_23 % 256 ) ;
}
static void F_51 ( T_1 * V_158 , T_1 * V_159 , T_1 * V_129 )
{
int V_18 ;
for ( V_18 = 0 ; V_18 < 16 ; V_18 ++ )
V_129 [ V_18 ] = V_158 [ V_18 ] ^ V_159 [ V_18 ] ;
}
static int F_52 ( T_1 * V_3 , T_5 V_62 , T_1 * V_40 , T_5 V_160 )
{
T_5 V_149 , V_150 , V_18 , V_22 , V_161 ,
V_162 , V_163 ;
T_1 V_153 [ 6 ] ;
T_1 V_148 [ 16 ] ;
T_1 V_154 [ 16 ] ;
T_1 V_156 [ 16 ] ;
T_1 V_157 [ 16 ] ;
T_1 V_164 [ 16 ] ;
T_1 V_165 [ 16 ] ;
T_1 V_166 [ 16 ] ;
T_1 V_167 [ 8 ] ;
struct V_168 * V_169 = (struct V_168 * ) V_40 ;
T_4 V_170 = F_53 ( V_169 -> V_171 ) & V_172 ;
memset ( ( void * ) V_148 , 0 , 16 ) ;
memset ( ( void * ) V_154 , 0 , 16 ) ;
memset ( ( void * ) V_156 , 0 , 16 ) ;
memset ( ( void * ) V_157 , 0 , 16 ) ;
memset ( ( void * ) V_164 , 0 , 16 ) ;
memset ( ( void * ) V_165 , 0 , 16 ) ;
memset ( ( void * ) V_166 , 0 , 16 ) ;
if ( ( V_62 == sizeof( struct V_173 ) ||
( V_62 == sizeof( struct V_174 ) ) ) )
V_150 = 0 ;
else
V_150 = 1 ;
if ( F_54 ( V_169 -> V_171 ) ) {
if ( ( V_170 == V_175 ) ||
( V_170 == V_176 ) ||
( V_170 == V_177 ) ) {
V_149 = 1 ;
if ( V_62 != sizeof( struct V_174 ) )
V_62 += 2 ;
} else if ( ( V_170 == V_178 ) ||
( V_170 == V_179 ) ||
( V_170 == V_180 ) ||
( V_170 == V_181 ) ) {
if ( V_62 != sizeof( struct V_174 ) )
V_62 += 2 ;
V_149 = 1 ;
} else {
V_149 = 0 ;
}
} else {
V_149 = 0 ;
}
V_153 [ 0 ] = V_40 [ V_62 ] ;
V_153 [ 1 ] = V_40 [ V_62 + 1 ] ;
V_153 [ 2 ] = V_40 [ V_62 + 4 ] ;
V_153 [ 3 ] = V_40 [ V_62 + 5 ] ;
V_153 [ 4 ] = V_40 [ V_62 + 6 ] ;
V_153 [ 5 ] = V_40 [ V_62 + 7 ] ;
F_47 ( V_148 , V_149 , V_150 , V_40 , V_160 , V_153 ) ;
F_48 ( V_154 , V_62 , V_40 ) ;
F_49 ( V_156 , V_40 , V_150 , V_149 ) ;
V_161 = V_160 % 16 ;
V_162 = V_160 / 16 ;
V_163 = ( V_62 + 8 ) ;
F_46 ( V_3 , V_148 , V_165 ) ;
F_51 ( V_165 , V_154 , V_164 ) ;
F_46 ( V_3 , V_164 , V_165 ) ;
F_51 ( V_165 , V_156 , V_164 ) ;
F_46 ( V_3 , V_164 , V_165 ) ;
for ( V_18 = 0 ; V_18 < V_162 ; V_18 ++ ) {
F_51 ( V_165 , & V_40 [ V_163 ] , V_164 ) ;
V_163 += 16 ;
F_46 ( V_3 , V_164 , V_165 ) ;
}
if ( V_161 > 0 ) {
for ( V_22 = 0 ; V_22 < 16 ; V_22 ++ )
V_166 [ V_22 ] = 0x00 ;
for ( V_22 = 0 ; V_22 < V_161 ; V_22 ++ )
V_166 [ V_22 ] = V_40 [ V_163 ++ ] ;
F_51 ( V_165 , V_166 , V_164 ) ;
F_46 ( V_3 , V_164 , V_165 ) ;
}
for ( V_22 = 0 ; V_22 < 8 ; V_22 ++ )
V_167 [ V_22 ] = V_165 [ V_22 ] ;
for ( V_22 = 0 ; V_22 < 8 ; V_22 ++ )
V_40 [ V_163 + V_22 ] = V_167 [ V_22 ] ;
V_163 = V_62 + 8 ;
for ( V_18 = 0 ; V_18 < V_162 ; V_18 ++ ) {
F_50 ( V_157 , V_150 , V_149 ,
V_40 , V_153 , V_18 + 1 ) ;
F_46 ( V_3 , V_157 , V_165 ) ;
F_51 ( V_165 , & V_40 [ V_163 ] , V_164 ) ;
for ( V_22 = 0 ; V_22 < 16 ; V_22 ++ )
V_40 [ V_163 ++ ] = V_164 [ V_22 ] ;
}
if ( V_161 > 0 ) {
F_50 ( V_157 , V_150 , V_149 , V_40 ,
V_153 , V_162 + 1 ) ;
for ( V_22 = 0 ; V_22 < 16 ; V_22 ++ )
V_166 [ V_22 ] = 0x00 ;
for ( V_22 = 0 ; V_22 < V_161 ; V_22 ++ )
V_166 [ V_22 ] = V_40 [ V_163 + V_22 ] ;
F_46 ( V_3 , V_157 , V_165 ) ;
F_51 ( V_165 , V_166 , V_164 ) ;
for ( V_22 = 0 ; V_22 < V_161 ; V_22 ++ )
V_40 [ V_163 ++ ] = V_164 [ V_22 ] ;
}
F_50 ( V_157 , V_150 , V_149 , V_40 ,
V_153 , 0 ) ;
for ( V_22 = 0 ; V_22 < 16 ; V_22 ++ )
V_166 [ V_22 ] = 0x00 ;
for ( V_22 = 0 ; V_22 < 8 ; V_22 ++ )
V_166 [ V_22 ] = V_40 [ V_22 + V_62 + 8 + V_160 ] ;
F_46 ( V_3 , V_157 , V_165 ) ;
F_51 ( V_165 , V_166 , V_164 ) ;
for ( V_22 = 0 ; V_22 < 8 ; V_22 ++ )
V_40 [ V_163 ++ ] = V_164 [ V_22 ] ;
return V_115 ;
}
T_2 F_55 ( struct V_31 * V_32 , struct V_33 * V_34 )
{
int V_36 , V_37 ;
T_2 V_109 ;
T_1 * V_40 , * V_110 ;
T_1 V_44 = 0 ;
struct V_113 * V_114 ;
struct V_45 * V_46 = & V_34 -> V_47 ;
struct V_48 * V_49 = & V_32 -> V_50 ;
struct V_51 * V_52 = & V_32 -> V_53 ;
T_2 V_79 = V_115 ;
if ( ! V_34 -> V_54 )
return V_116 ;
V_44 = V_55 ;
V_40 = V_34 -> V_54 + V_44 ;
if ( V_46 -> V_56 != V_182 )
return V_116 ;
if ( V_46 -> V_118 ) {
V_114 = V_46 -> V_118 ;
} else {
F_34 ( L_15 , V_119 ) ;
V_114 = F_35 ( & V_32 -> V_120 , & V_46 -> V_121 [ 0 ] ) ;
}
if ( ! V_114 ) {
F_12 ( V_77 , V_78 ,
( L_16 ) ) ;
F_34 ( L_10 , V_119 ) ;
V_79 = V_116 ;
goto V_129;
}
if ( ! ( V_114 -> V_9 & V_122 ) ) {
F_34 ( L_6 ,
V_119 , V_114 -> V_9 ) ;
return V_116 ;
}
F_12 ( V_77 , V_78 ,
( L_17 ) ) ;
if ( F_36 ( V_46 -> V_121 ) )
V_110 = V_49 -> V_123 [ V_49 -> V_124 ] . V_64 ;
else
V_110 = & V_114 -> V_125 . V_64 [ 0 ] ;
V_109 = 16 ;
for ( V_36 = 0 ; V_36 < V_46 -> V_61 ; V_36 ++ ) {
if ( ( V_36 + 1 ) == V_46 -> V_61 ) {
V_37 = V_46 -> V_66 -
V_46 -> V_62 - V_46 -> V_65 -
V_46 -> V_67 ;
F_52 ( V_110 , V_46 -> V_62 , V_40 , V_37 ) ;
} else {
V_37 = V_52 -> V_68 - V_46 -> V_62 -
V_46 -> V_65 - V_46 -> V_67 ;
F_52 ( V_110 , V_46 -> V_62 , V_40 , V_37 ) ;
V_40 += V_52 -> V_68 ;
V_40 = F_9 ( V_40 , 4 ) ;
}
}
V_129:
return V_79 ;
}
static int F_56 ( T_1 * V_3 , T_5 V_62 ,
T_1 * V_40 , T_5 V_160 )
{
static T_1 V_183 [ V_184 ] ;
T_5 V_149 , V_150 , V_18 , V_22 , V_161 ,
V_162 , V_163 ;
int V_79 = V_115 ;
T_1 V_153 [ 6 ] ;
T_1 V_148 [ 16 ] ;
T_1 V_154 [ 16 ] ;
T_1 V_156 [ 16 ] ;
T_1 V_157 [ 16 ] ;
T_1 V_164 [ 16 ] ;
T_1 V_165 [ 16 ] ;
T_1 V_166 [ 16 ] ;
T_1 V_167 [ 8 ] ;
struct V_168 * V_169 = (struct V_168 * ) V_40 ;
T_4 V_170 = F_53 ( V_169 -> V_171 ) & V_172 ;
memset ( ( void * ) V_148 , 0 , 16 ) ;
memset ( ( void * ) V_154 , 0 , 16 ) ;
memset ( ( void * ) V_156 , 0 , 16 ) ;
memset ( ( void * ) V_157 , 0 , 16 ) ;
memset ( ( void * ) V_164 , 0 , 16 ) ;
memset ( ( void * ) V_165 , 0 , 16 ) ;
memset ( ( void * ) V_166 , 0 , 16 ) ;
V_162 = ( V_160 - 8 ) / 16 ;
V_161 = ( V_160 - 8 ) % 16 ;
V_153 [ 0 ] = V_40 [ V_62 ] ;
V_153 [ 1 ] = V_40 [ V_62 + 1 ] ;
V_153 [ 2 ] = V_40 [ V_62 + 4 ] ;
V_153 [ 3 ] = V_40 [ V_62 + 5 ] ;
V_153 [ 4 ] = V_40 [ V_62 + 6 ] ;
V_153 [ 5 ] = V_40 [ V_62 + 7 ] ;
if ( ( V_62 == sizeof( struct V_173 ) ||
( V_62 == sizeof( struct V_174 ) ) ) )
V_150 = 0 ;
else
V_150 = 1 ;
if ( F_54 ( V_169 -> V_171 ) ) {
if ( ( V_170 == V_175 ) ||
( V_170 == V_176 ) ||
( V_170 == V_177 ) ) {
V_149 = 1 ;
if ( V_62 != sizeof( struct V_173 ) )
V_62 += 2 ;
} else if ( ( V_170 == V_178 ) ||
( V_170 == V_179 ) ||
( V_170 == V_180 ) ||
( V_170 == V_181 ) ) {
if ( V_62 != sizeof( struct V_173 ) )
V_62 += 2 ;
V_149 = 1 ;
} else {
V_149 = 0 ;
}
} else {
V_149 = 0 ;
}
V_163 = V_62 + 8 ;
for ( V_18 = 0 ; V_18 < V_162 ; V_18 ++ ) {
F_50 ( V_157 , V_150 , V_149 ,
V_40 , V_153 , V_18 + 1 ) ;
F_46 ( V_3 , V_157 , V_165 ) ;
F_51 ( V_165 , & V_40 [ V_163 ] , V_164 ) ;
for ( V_22 = 0 ; V_22 < 16 ; V_22 ++ )
V_40 [ V_163 ++ ] = V_164 [ V_22 ] ;
}
if ( V_161 > 0 ) {
F_50 ( V_157 , V_150 , V_149 , V_40 ,
V_153 , V_162 + 1 ) ;
for ( V_22 = 0 ; V_22 < 16 ; V_22 ++ )
V_166 [ V_22 ] = 0x00 ;
for ( V_22 = 0 ; V_22 < V_161 ; V_22 ++ )
V_166 [ V_22 ] = V_40 [ V_163 + V_22 ] ;
F_46 ( V_3 , V_157 , V_165 ) ;
F_51 ( V_165 , V_166 , V_164 ) ;
for ( V_22 = 0 ; V_22 < V_161 ; V_22 ++ )
V_40 [ V_163 ++ ] = V_164 [ V_22 ] ;
}
if ( ( V_62 + V_160 + 8 ) <= V_184 )
memcpy ( V_183 , V_40 , ( V_62 + V_160 + 8 ) ) ;
V_153 [ 0 ] = V_40 [ V_62 ] ;
V_153 [ 1 ] = V_40 [ V_62 + 1 ] ;
V_153 [ 2 ] = V_40 [ V_62 + 4 ] ;
V_153 [ 3 ] = V_40 [ V_62 + 5 ] ;
V_153 [ 4 ] = V_40 [ V_62 + 6 ] ;
V_153 [ 5 ] = V_40 [ V_62 + 7 ] ;
F_47 ( V_148 , V_149 , V_150 , V_183 ,
V_160 - 8 , V_153 ) ;
F_48 ( V_154 , V_62 , V_183 ) ;
F_49 ( V_156 , V_183 , V_150 , V_149 ) ;
V_161 = ( V_160 - 8 ) % 16 ;
V_162 = ( V_160 - 8 ) / 16 ;
V_163 = ( V_62 + 8 ) ;
F_46 ( V_3 , V_148 , V_165 ) ;
F_51 ( V_165 , V_154 , V_164 ) ;
F_46 ( V_3 , V_164 , V_165 ) ;
F_51 ( V_165 , V_156 , V_164 ) ;
F_46 ( V_3 , V_164 , V_165 ) ;
for ( V_18 = 0 ; V_18 < V_162 ; V_18 ++ ) {
F_51 ( V_165 , & V_183 [ V_163 ] , V_164 ) ;
V_163 += 16 ;
F_46 ( V_3 , V_164 , V_165 ) ;
}
if ( V_161 > 0 ) {
for ( V_22 = 0 ; V_22 < 16 ; V_22 ++ )
V_166 [ V_22 ] = 0x00 ;
for ( V_22 = 0 ; V_22 < V_161 ; V_22 ++ )
V_166 [ V_22 ] = V_183 [ V_163 ++ ] ;
F_51 ( V_165 , V_166 , V_164 ) ;
F_46 ( V_3 , V_164 , V_165 ) ;
}
for ( V_22 = 0 ; V_22 < 8 ; V_22 ++ )
V_167 [ V_22 ] = V_165 [ V_22 ] ;
for ( V_22 = 0 ; V_22 < 8 ; V_22 ++ )
V_183 [ V_163 + V_22 ] = V_167 [ V_22 ] ;
V_163 = V_62 + 8 ;
for ( V_18 = 0 ; V_18 < V_162 ; V_18 ++ ) {
F_50 ( V_157 , V_150 , V_149 ,
V_183 , V_153 , V_18 + 1 ) ;
F_46 ( V_3 , V_157 , V_165 ) ;
F_51 ( V_165 , & V_183 [ V_163 ] , V_164 ) ;
for ( V_22 = 0 ; V_22 < 16 ; V_22 ++ )
V_183 [ V_163 ++ ] = V_164 [ V_22 ] ;
}
if ( V_161 > 0 ) {
F_50 ( V_157 , V_150 , V_149 ,
V_183 , V_153 , V_162 + 1 ) ;
for ( V_22 = 0 ; V_22 < 16 ; V_22 ++ )
V_166 [ V_22 ] = 0x00 ;
for ( V_22 = 0 ; V_22 < V_161 ; V_22 ++ )
V_166 [ V_22 ] = V_183 [ V_163 + V_22 ] ;
F_46 ( V_3 , V_157 , V_165 ) ;
F_51 ( V_165 , V_166 , V_164 ) ;
for ( V_22 = 0 ; V_22 < V_161 ; V_22 ++ )
V_183 [ V_163 ++ ] = V_164 [ V_22 ] ;
}
F_50 ( V_157 , V_150 , V_149 , V_183 ,
V_153 , 0 ) ;
for ( V_22 = 0 ; V_22 < 16 ; V_22 ++ )
V_166 [ V_22 ] = 0x00 ;
for ( V_22 = 0 ; V_22 < 8 ; V_22 ++ )
V_166 [ V_22 ] = V_183 [ V_22 + V_62 + 8 + V_160 - 8 ] ;
F_46 ( V_3 , V_157 , V_165 ) ;
F_51 ( V_165 , V_166 , V_164 ) ;
for ( V_22 = 0 ; V_22 < 8 ; V_22 ++ )
V_183 [ V_163 ++ ] = V_164 [ V_22 ] ;
for ( V_18 = 0 ; V_18 < 8 ; V_18 ++ ) {
if ( V_40 [ V_62 + 8 + V_160 - 8 + V_18 ] != V_183 [ V_62 + 8 + V_160 - 8 + V_18 ] ) {
F_12 ( V_77 , V_78 ,
( L_18 ,
V_18 , V_40 [ V_62 + 8 + V_160 - 8 + V_18 ] , V_183 [ V_62 + 8 + V_160 - 8 + V_18 ] ) ) ;
F_34 ( L_18 ,
V_18 , V_40 [ V_62 + 8 + V_160 - 8 + V_18 ] , V_183 [ V_62 + 8 + V_160 - 8 + V_18 ] ) ;
V_79 = V_116 ;
}
}
return V_79 ;
}
T_2 F_57 ( struct V_31 * V_32 , struct V_69 * V_70 )
{
struct V_113 * V_114 ;
struct V_71 * V_72 = & V_70 -> V_47 ;
struct V_48 * V_49 = & V_32 -> V_50 ;
struct V_73 * V_74 = V_70 -> V_75 ;
int V_37 ;
T_1 * V_40 , * V_110 ;
T_2 V_79 = V_115 ;
V_40 = V_74 -> V_19 ;
if ( V_72 -> V_56 != V_182 )
return V_116 ;
V_114 = F_35 ( & V_32 -> V_120 , & V_72 -> V_100 [ 0 ] ) ;
if ( ! V_114 ) {
F_12 ( V_77 , V_78 ,
( L_16 ) ) ;
V_79 = V_116 ;
goto exit;
}
F_12 ( V_77 , V_78 ,
( L_19 ) ) ;
if ( F_36 ( V_72 -> V_121 ) ) {
if ( ! V_49 -> V_127 ) {
V_79 = V_116 ;
F_34 ( L_20
L_21 , V_119 ) ;
goto exit;
}
V_110 = V_49 -> V_123 [ V_72 -> V_76 ] . V_64 ;
if ( V_49 -> V_124 != V_72 -> V_76 ) {
F_34 ( L_22
L_23 , V_72 -> V_76 ,
V_49 -> V_124 ) ;
V_79 = V_116 ;
goto exit;
}
} else {
V_110 = & V_114 -> V_125 . V_64 [ 0 ] ;
}
V_37 = V_74 -> V_17 - V_72 -> V_62 - V_72 -> V_65 ;
V_79 = F_56 ( V_110 , V_72 -> V_62 , V_40 , V_37 ) ;
exit:
return V_79 ;
}
void F_58 ( void * V_185 )
{
struct V_31 * V_32 = (struct V_31 * ) V_185 ;
F_12 ( V_77 , V_78 , ( L_24 ) ) ;
V_32 -> V_50 . V_186 = true ;
F_12 ( V_77 , V_78 ,
( L_25 ,
V_32 -> V_50 . V_186 ) ) ;
}
