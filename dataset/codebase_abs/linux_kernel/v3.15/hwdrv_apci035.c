static int F_1 ( struct V_1 * V_2 ,
struct V_3 * V_4 ,
struct V_5 * V_6 ,
unsigned int * V_7 )
{
struct V_8 * V_9 = V_2 -> V_10 ;
unsigned int V_11 ;
unsigned int V_12 ;
unsigned int V_13 ;
V_14 = 0 ;
V_9 -> V_15 = V_16 ;
V_9 -> V_17 = V_7 [ 0 ] ;
V_18 = V_7 [ 1 ] ;
if ( V_7 [ 0 ] == 0 )
V_13 = 2 ;
else
V_13 = 0 ;
V_12 = 0 ;
F_2 ( V_12 , V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) + 12 ) ;
V_12 = F_3 ( V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) + 12 ) ;
F_2 ( V_7 [ 3 ] , V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) + 4 ) ;
F_2 ( V_7 [ 2 ] , V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) + 8 ) ;
if ( V_7 [ 0 ] == V_20 ) {
V_12 =
( V_12 & 0xFFF719E2UL ) | V_13 << 13UL | 0x10UL ;
} else if ( V_7 [ 0 ] == V_21 ) {
V_12 = V_12 & 0xFFF819E2UL ;
} else {
F_4 ( V_2 -> V_22 , L_1 ) ;
return - V_23 ;
}
F_2 ( V_12 , V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) + 12 ) ;
V_12 = F_3 ( V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) + 12 ) ;
V_12 = V_12 & 0xFFFFF89FUL ;
if ( V_7 [ 4 ] == V_24 ) {
V_12 = V_12 | ( V_7 [ 5 ] << 5 ) ;
}
F_2 ( V_12 , V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) + 12 ) ;
V_12 = F_3 ( V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) + 12 ) ;
V_12 = V_12 & 0xFFFFF87FUL ;
if ( V_7 [ 6 ] == V_24 ) {
V_12 = V_12 | ( V_7 [ 7 ] << 7 ) ;
}
F_2 ( V_12 , V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) + 12 ) ;
V_12 = F_3 ( V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) + 12 ) ;
V_12 = V_12 & 0xFFFFF9FBUL ;
V_12 = V_12 | ( V_7 [ 8 ] << 2 ) ;
F_2 ( V_12 , V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) + 12 ) ;
if ( V_7 [ 9 ] == V_24 ) {
F_2 ( V_7 [ 11 ] ,
V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) + 24 ) ;
F_2 ( V_7 [ 10 ] ,
V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) + 28 ) ;
}
V_12 = F_3 ( V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) + 12 ) ;
V_12 = V_12 & 0xFFFFF9F7UL ;
V_12 = V_12 | ( V_7 [ 12 ] << 3 ) ;
F_2 ( V_12 , V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) + 12 ) ;
V_12 = F_3 ( V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) + 12 ) ;
V_11 = F_3 ( V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) + 16 ) ;
V_12 = ( V_12 & 0xFFFFF9FDUL ) | ( V_7 [ 13 ] << 1 ) ;
F_2 ( V_12 , V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) + 12 ) ;
return V_6 -> V_25 ;
}
static int F_5 ( struct V_1 * V_2 ,
struct V_3 * V_4 ,
struct V_5 * V_6 ,
unsigned int * V_7 )
{
struct V_8 * V_9 = V_2 -> V_10 ;
unsigned int V_12 ;
int V_26 ;
if ( V_7 [ 0 ] == 1 ) {
V_12 =
F_3 ( V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) + 12 ) ;
V_12 = ( V_12 & 0xFFFFF9FFUL ) | 0x1UL ;
F_2 ( V_12 ,
V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) + 12 ) ;
}
if ( V_7 [ 0 ] == 2 ) {
V_12 =
F_3 ( V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) + 12 ) ;
V_12 = ( V_12 & 0xFFFFF9FFUL ) | 0x200UL ;
F_2 ( V_12 ,
V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) + 12 ) ;
}
if ( V_7 [ 0 ] == 0 ) {
V_12 = 0 ;
F_2 ( V_12 ,
V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) + 12 ) ;
}
if ( V_7 [ 0 ] == 3 ) {
V_12 = 0 ;
for ( V_26 = 1 ; V_26 <= 4 ; V_26 ++ ) {
if ( V_9 -> V_17 == V_21 )
V_12 = 0x2UL ;
else
V_12 = 0x10UL ;
V_18 = V_26 ;
F_2 ( V_12 ,
V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) +
0 ) ;
}
}
if ( V_7 [ 0 ] == 4 ) {
V_12 = 0 ;
for ( V_26 = 1 ; V_26 <= 4 ; V_26 ++ ) {
if ( V_9 -> V_17 == V_21 )
V_12 = 0x1UL ;
else
V_12 = 0x8UL ;
V_18 = V_26 ;
F_2 ( V_12 ,
V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) +
0 ) ;
}
}
if ( V_7 [ 0 ] == 5 ) {
V_12 = 0 ;
for ( V_26 = 1 ; V_26 <= 4 ; V_26 ++ ) {
if ( V_9 -> V_17 == V_21 )
V_12 = 0x4UL ;
else
V_12 = 0x20UL ;
V_18 = V_26 ;
F_2 ( V_12 ,
V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) +
0 ) ;
}
V_14 = 1 ;
}
return V_6 -> V_25 ;
}
static int F_6 ( struct V_1 * V_2 ,
struct V_3 * V_4 ,
struct V_5 * V_6 ,
unsigned int * V_7 )
{
struct V_8 * V_9 = V_2 -> V_10 ;
unsigned int V_11 ;
V_18 = V_6 -> V_27 [ 0 ] ;
V_11 = F_3 ( V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) + 16 ) ;
V_7 [ 0 ] = ( ( V_11 >> 1 ) & 1 ) ;
V_7 [ 1 ] = ( ( V_11 >> 2 ) & 1 ) ;
V_7 [ 2 ] = ( ( V_11 >> 3 ) & 1 ) ;
V_7 [ 3 ] = ( ( V_11 >> 0 ) & 1 ) ;
if ( V_9 -> V_17 == V_20 )
V_7 [ 4 ] = F_3 ( V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) + 0 ) ;
return V_6 -> V_25 ;
}
static int F_7 ( struct V_1 * V_2 ,
struct V_3 * V_4 ,
struct V_5 * V_6 ,
unsigned int * V_7 )
{
struct V_8 * V_9 = V_2 -> V_10 ;
V_9 -> V_15 = V_16 ;
F_2 ( 0x200 | 0 , V_9 -> V_19 + 128 + 0x4 ) ;
F_2 ( 0 , V_9 -> V_19 + 128 + 0 ) ;
F_2 ( 0x300 | 0 , V_9 -> V_19 + 128 + 0x4 ) ;
F_2 ( ( V_7 [ 0 ] << 8 ) , V_9 -> V_19 + 128 + 0 ) ;
F_2 ( 0x200000UL , V_9 -> V_19 + 128 + 12 ) ;
return V_6 -> V_25 ;
}
static int F_8 ( struct V_1 * V_2 ,
struct V_3 * V_4 ,
struct V_5 * V_6 ,
unsigned int * V_7 )
{
struct V_8 * V_9 = V_2 -> V_10 ;
unsigned int V_28 ;
V_28 = 0x80000 ;
F_2 ( V_28 , V_9 -> V_19 + 128 + 8 ) ;
V_7 [ 0 ] = F_3 ( V_9 -> V_19 + 128 + 28 ) ;
return V_6 -> V_25 ;
}
static int F_9 ( struct V_1 * V_2 )
{
struct V_8 * V_9 = V_2 -> V_10 ;
int V_26 ;
for ( V_26 = 1 ; V_26 <= 4 ; V_26 ++ ) {
V_18 = V_26 ;
F_2 ( 0x0 , V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) + 0 ) ;
}
F_2 ( 0x0 , V_9 -> V_19 + 128 + 12 ) ;
return 0 ;
}
static void F_10 ( int V_29 , void * V_30 )
{
struct V_1 * V_2 = V_30 ;
struct V_8 * V_9 = V_2 -> V_10 ;
unsigned int V_31 ;
unsigned int V_32 ;
unsigned int V_33 ;
unsigned int V_34 ;
unsigned int V_35 ;
if ( V_14 == 1 ) {
V_18 = V_36 ;
V_36 = V_36 + 1 ;
}
V_31 = F_3 ( V_9 -> V_19 + 128 + 16 ) ;
V_32 =
F_3 ( V_9 -> V_19 + ( ( V_18 - 1 ) * 32 ) + 20 ) ;
if ( ( ( ( V_31 ) & 0x8 ) == 0x8 ) ) {
V_33 = F_3 ( V_9 -> V_19 + 128 + 12 ) ;
V_33 = V_33 & 0xFFDF0000UL ;
F_2 ( V_33 , V_9 -> V_19 + 128 + 12 ) ;
V_34 = F_3 ( V_9 -> V_19 + 128 + 60 ) ;
V_35 = F_3 ( V_9 -> V_19 + 128 + 60 ) ;
F_11 ( V_37 , V_9 -> V_15 , 0 ) ;
} else if ( ( V_32 & 0x1 ) == 0x1 ) {
F_11 ( V_37 , V_9 -> V_15 , 0 ) ;
}
return;
}
