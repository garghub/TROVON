static inline int F_1 ( struct V_1 * V_2 )
{
return V_3 - F_2 ( V_2 ) - 1 ;
}
static void F_3 ( struct V_1 * V_4 ,
struct V_5 * V_6 )
{
struct V_7 * V_8 ;
unsigned long V_9 ;
T_1 V_10 ;
T_2 V_11 ;
V_10 = F_4 ( V_6 -> V_12 , 1000000000 , & V_11 ) ;
F_5 ( & V_4 -> V_13 , V_9 ) ;
V_8 = & V_4 -> V_14 [ V_4 -> V_15 ] ;
V_8 -> V_16 = V_6 -> V_16 ;
V_8 -> V_17 . V_18 = V_10 ;
V_8 -> V_17 . V_19 = V_11 ;
if ( ! F_1 ( V_4 ) )
V_4 -> V_20 = ( V_4 -> V_20 + 1 ) % V_3 ;
V_4 -> V_15 = ( V_4 -> V_15 + 1 ) % V_3 ;
F_6 ( & V_4 -> V_13 , V_9 ) ;
}
static T_3 F_7 ( long V_21 )
{
T_1 V_22 = 1 + V_21 ;
V_22 *= 125 ;
V_22 >>= 13 ;
return ( T_3 ) V_22 ;
}
static int F_8 ( struct V_23 * V_24 , struct V_25 * V_26 )
{
V_26 -> V_27 = 0 ;
V_26 -> V_28 = 1 ;
return 0 ;
}
static int F_9 ( struct V_23 * V_24 , const struct V_25 * V_26 )
{
struct V_29 * V_30 = F_10 ( V_24 , struct V_29 , clock ) ;
return V_30 -> V_31 -> V_32 ( V_30 -> V_31 , V_26 ) ;
}
static int F_11 ( struct V_23 * V_24 , struct V_25 * V_26 )
{
struct V_29 * V_30 = F_10 ( V_24 , struct V_29 , clock ) ;
return V_30 -> V_31 -> V_33 ( V_30 -> V_31 , V_26 ) ;
}
static int F_12 ( struct V_23 * V_24 , struct V_34 * V_35 )
{
struct V_29 * V_30 = F_10 ( V_24 , struct V_29 , clock ) ;
struct V_36 * V_37 ;
int V_38 = - V_39 ;
V_37 = V_30 -> V_31 ;
if ( V_35 -> V_40 & V_41 ) {
struct V_25 V_42 ;
T_4 V_43 ;
T_1 V_44 ;
V_42 . V_27 = V_35 -> time . V_27 ;
V_42 . V_28 = V_35 -> time . V_45 ;
if ( ! ( V_35 -> V_40 & V_46 ) )
V_42 . V_28 *= 1000 ;
if ( ( unsigned long ) V_42 . V_28 >= V_47 )
return - V_48 ;
V_43 = F_13 ( V_42 ) ;
V_44 = F_14 ( V_43 ) ;
V_38 = V_37 -> V_49 ( V_37 , V_44 ) ;
} else if ( V_35 -> V_40 & V_50 ) {
V_38 = V_37 -> V_51 ( V_37 , F_7 ( V_35 -> V_52 ) ) ;
V_30 -> V_53 = V_35 -> V_52 ;
} else if ( V_35 -> V_40 == 0 ) {
V_35 -> V_52 = V_30 -> V_53 ;
V_38 = 0 ;
}
return V_38 ;
}
static void F_15 ( struct V_23 * V_24 )
{
struct V_29 * V_30 = F_10 ( V_24 , struct V_29 , clock ) ;
F_16 ( & V_30 -> V_54 ) ;
F_16 ( & V_30 -> V_55 ) ;
F_17 ( & V_56 , V_30 -> V_16 ) ;
F_18 ( V_30 ) ;
}
struct V_29 * F_19 ( struct V_36 * V_31 ,
struct V_57 * V_58 )
{
struct V_29 * V_30 ;
int V_38 = 0 , V_16 , V_59 = F_20 ( V_60 ) ;
if ( V_31 -> V_61 > V_62 )
return F_21 ( - V_48 ) ;
V_38 = - V_63 ;
V_30 = F_22 ( sizeof( struct V_29 ) , V_64 ) ;
if ( V_30 == NULL )
goto V_65;
V_16 = F_23 ( & V_56 , 0 , V_66 + 1 , V_64 ) ;
if ( V_16 < 0 ) {
V_38 = V_16 ;
goto V_67;
}
V_30 -> clock . V_37 = V_68 ;
V_30 -> clock . V_69 = F_15 ;
V_30 -> V_31 = V_31 ;
V_30 -> V_70 = F_24 ( V_59 , V_16 ) ;
V_30 -> V_16 = V_16 ;
F_25 ( & V_30 -> V_71 . V_13 ) ;
F_26 ( & V_30 -> V_54 ) ;
F_26 ( & V_30 -> V_55 ) ;
F_27 ( & V_30 -> V_72 ) ;
V_30 -> V_73 = F_28 ( V_74 , V_58 , V_30 -> V_70 , V_30 ,
L_1 , V_30 -> V_16 ) ;
if ( F_29 ( V_30 -> V_73 ) )
goto V_75;
F_30 ( V_30 -> V_73 , V_30 ) ;
V_38 = F_31 ( V_30 ) ;
if ( V_38 )
goto V_76;
if ( V_31 -> V_77 ) {
struct V_78 V_77 ;
memset ( & V_77 , 0 , sizeof( V_77 ) ) ;
snprintf ( V_77 . V_79 , V_80 , L_1 , V_16 ) ;
V_77 . V_81 = V_82 ;
V_77 . V_83 = V_31 -> V_83 ;
V_30 -> V_84 = F_32 ( & V_77 , V_85 ) ;
if ( ! V_30 -> V_84 ) {
F_33 ( L_2 ) ;
goto V_86;
}
}
V_38 = F_34 ( & V_30 -> clock , V_30 -> V_70 ) ;
if ( V_38 ) {
F_33 ( L_3 ) ;
goto V_87;
}
return V_30 ;
V_87:
if ( V_30 -> V_84 )
F_35 ( V_30 -> V_84 ) ;
V_86:
F_36 ( V_30 ) ;
V_76:
F_37 ( V_74 , V_30 -> V_70 ) ;
V_75:
F_16 ( & V_30 -> V_54 ) ;
F_16 ( & V_30 -> V_55 ) ;
V_67:
F_18 ( V_30 ) ;
V_65:
return F_21 ( V_38 ) ;
}
int F_38 ( struct V_29 * V_30 )
{
V_30 -> V_88 = 1 ;
F_39 ( & V_30 -> V_72 ) ;
if ( V_30 -> V_84 )
F_35 ( V_30 -> V_84 ) ;
F_36 ( V_30 ) ;
F_37 ( V_74 , V_30 -> V_70 ) ;
F_40 ( & V_30 -> clock ) ;
return 0 ;
}
void V_5 ( struct V_29 * V_30 , struct V_5 * V_89 )
{
struct V_90 V_91 ;
switch ( V_89 -> type ) {
case V_92 :
break;
case V_93 :
F_3 ( & V_30 -> V_71 , V_89 ) ;
F_39 ( & V_30 -> V_72 ) ;
break;
case V_94 :
F_41 ( & V_91 ) ;
F_42 ( V_30 -> V_84 , & V_91 , V_95 , NULL ) ;
break;
case V_96 :
F_42 ( V_30 -> V_84 , & V_89 -> V_97 ,
V_95 , NULL ) ;
break;
}
}
int F_43 ( struct V_29 * V_30 )
{
return V_30 -> V_16 ;
}
int F_44 ( struct V_29 * V_30 ,
enum V_98 V_99 , unsigned int V_100 )
{
struct V_101 * V_102 = NULL ;
int V_103 ;
F_45 ( & V_30 -> V_55 ) ;
for ( V_103 = 0 ; V_103 < V_30 -> V_31 -> V_104 ; V_103 ++ ) {
if ( V_30 -> V_31 -> V_105 [ V_103 ] . V_99 == V_99 &&
V_30 -> V_31 -> V_105 [ V_103 ] . V_100 == V_100 ) {
V_102 = & V_30 -> V_31 -> V_105 [ V_103 ] ;
break;
}
}
F_46 ( & V_30 -> V_55 ) ;
return V_102 ? V_103 : - 1 ;
}
static void T_5 F_47 ( void )
{
F_48 ( V_74 ) ;
F_49 ( V_60 , V_66 + 1 ) ;
F_50 ( & V_56 ) ;
}
static int T_6 F_51 ( void )
{
int V_38 ;
V_74 = F_52 ( V_106 , L_4 ) ;
if ( F_29 ( V_74 ) ) {
F_33 ( L_5 ) ;
return F_53 ( V_74 ) ;
}
V_38 = F_54 ( & V_60 , 0 , V_66 + 1 , L_4 ) ;
if ( V_38 < 0 ) {
F_33 ( L_6 ) ;
goto V_107;
}
V_74 -> V_108 = V_109 ;
F_55 ( L_7 ) ;
return 0 ;
V_107:
F_48 ( V_74 ) ;
return V_38 ;
}
