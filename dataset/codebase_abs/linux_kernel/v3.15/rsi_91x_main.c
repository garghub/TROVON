void F_1 ( T_1 V_1 , const char * V_2 , ... )
{
struct V_3 V_4 ;
T_2 args ;
va_start ( args , V_2 ) ;
V_4 . V_2 = V_2 ;
V_4 . V_5 = & args ;
if ( V_1 & V_6 )
F_2 ( L_1 , & V_4 ) ;
va_end ( args ) ;
}
static struct V_7 * F_3 ( struct V_8 * V_9 ,
T_3 * V_10 ,
T_1 V_11 ,
T_3 V_12 )
{
struct V_13 * V_14 ;
struct V_15 * V_16 ;
struct V_7 * V_17 = NULL ;
T_3 V_18 ;
if ( F_4 ( ! V_11 , L_2 , V_19 ) )
return NULL ;
if ( V_11 > ( V_20 * 4 ) ) {
F_1 ( V_21 , L_3 ,
V_19 , V_11 ) ;
V_11 = V_20 * 4 ;
}
V_11 -= V_12 ;
V_17 = F_5 ( V_11 + V_22 ) ;
if ( V_17 == NULL )
return NULL ;
V_18 = ( V_12 + V_22 ) ;
F_6 ( V_17 , V_11 ) ;
memcpy ( ( V_17 -> V_23 ) , ( V_10 + V_18 ) , V_17 -> V_24 ) ;
V_14 = F_7 ( V_17 ) ;
V_16 = (struct V_15 * ) V_14 -> V_25 ;
V_16 -> V_26 = F_8 ( V_10 ) ;
V_16 -> V_27 = F_9 ( V_9 -> V_28 ) ;
return V_17 ;
}
int F_10 ( struct V_8 * V_9 , T_4 V_29 )
{
T_3 * V_30 = NULL , V_12 = 0 ;
T_1 V_31 , V_32 = 0 , V_33 = 0 ;
T_5 V_34 = 0 , V_35 ;
struct V_7 * V_17 = NULL ;
V_31 = 0 ;
do {
V_30 = & V_9 -> V_36 [ V_31 ] ;
V_34 = * ( T_5 * ) & V_30 [ 0 ] ;
V_35 = * ( T_5 * ) & V_30 [ 2 ] ;
V_33 = F_11 ( V_30 , V_35 ) ;
V_32 = F_12 ( V_30 , V_35 ) ;
V_12 = F_13 ( V_30 , V_35 ) ;
switch ( V_33 ) {
case V_37 :
V_17 = F_3 ( V_9 ,
( V_30 + V_35 ) ,
V_32 ,
V_12 ) ;
if ( V_17 == NULL )
goto V_38;
F_14 ( V_9 , V_17 ) ;
break;
case V_39 :
F_15 ( V_9 , ( V_30 + V_35 ) ) ;
break;
default:
F_1 ( V_21 , L_4 ,
V_19 , V_33 ) ;
goto V_38;
}
V_31 += V_34 ;
V_29 -= V_34 ;
} while ( V_29 > 0 );
return 0 ;
V_38:
return - V_40 ;
}
static void F_16 ( struct V_8 * V_9 )
{
struct V_41 * V_42 = V_9 -> V_28 ;
T_1 V_43 = V_44 ;
do {
if ( V_42 -> V_45 )
V_43 = V_42 -> V_45 ( V_42 ) ;
F_17 ( & V_9 -> V_46 . V_47 , V_43 ) ;
F_18 ( & V_9 -> V_46 . V_47 ) ;
if ( V_9 -> V_48 )
F_19 ( V_9 ) ;
} while ( F_20 ( & V_9 -> V_46 . V_49 ) == 0 );
F_21 ( & V_9 -> V_46 . V_50 , 0 ) ;
}
struct V_41 * F_22 ( void )
{
struct V_41 * V_42 = NULL ;
struct V_8 * V_9 = NULL ;
T_3 V_51 = 0 ;
V_42 = F_23 ( sizeof( * V_42 ) , V_52 ) ;
if ( ! V_42 )
return NULL ;
V_42 -> V_28 = F_23 ( sizeof( * V_9 ) , V_52 ) ;
if ( V_42 -> V_28 == NULL ) {
F_1 ( V_21 , L_5 ,
V_19 ) ;
F_24 ( V_42 ) ;
return NULL ;
} else {
V_9 = V_42 -> V_28 ;
V_9 -> V_28 = V_42 ;
}
for ( V_51 = 0 ; V_51 < V_53 ; V_51 ++ )
F_25 ( & V_9 -> V_54 [ V_51 ] ) ;
F_26 ( & V_9 -> V_46 . V_47 ) ;
F_27 ( & V_9 -> V_55 ) ;
F_27 ( & V_9 -> V_56 ) ;
if ( F_28 ( V_9 ,
& V_9 -> V_46 ,
F_16 ,
L_6 ) ) {
F_1 ( V_21 , L_7 , V_19 ) ;
goto V_57;
}
V_9 -> V_48 = true ;
return V_42 ;
V_57:
F_24 ( V_9 ) ;
F_24 ( V_42 ) ;
return NULL ;
}
void F_29 ( struct V_41 * V_42 )
{
struct V_8 * V_9 = V_42 -> V_28 ;
T_3 V_51 ;
F_1 ( V_58 , L_8 , V_19 ) ;
F_30 ( & V_9 -> V_46 ) ;
for ( V_51 = 0 ; V_51 < V_53 ; V_51 ++ )
F_31 ( & V_9 -> V_54 [ V_51 ] ) ;
V_9 -> V_48 = false ;
F_24 ( V_9 ) ;
F_24 ( V_42 -> V_59 ) ;
F_24 ( V_42 ) ;
}
static int F_32 ( void )
{
F_1 ( V_60 , L_9 , V_19 ) ;
return 0 ;
}
static void F_33 ( void )
{
F_1 ( V_60 , L_10 , V_19 ) ;
}
