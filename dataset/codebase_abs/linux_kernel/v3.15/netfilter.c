int F_1 ( struct V_1 * V_2 )
{
struct V_3 * V_3 = F_2 ( F_3 ( V_2 ) -> V_4 ) ;
const struct V_5 * V_6 = F_4 ( V_2 ) ;
unsigned int V_7 ;
struct V_8 * V_9 ;
struct V_10 V_11 = {
. V_12 = V_2 -> V_13 ? V_2 -> V_13 -> V_14 : 0 ,
. V_15 = V_2 -> V_16 ,
. V_17 = V_6 -> V_17 ,
. V_18 = V_6 -> V_18 ,
} ;
int V_19 ;
V_9 = F_5 ( V_3 , V_2 -> V_13 , & V_11 ) ;
V_19 = V_9 -> error ;
if ( V_19 ) {
F_6 ( V_3 , F_7 ( V_9 ) , V_20 ) ;
F_8 ( V_21 L_1 ) ;
F_9 ( V_9 ) ;
return V_19 ;
}
F_10 ( V_2 ) ;
F_11 ( V_2 , V_9 ) ;
#ifdef F_12
if ( ! ( F_13 ( V_2 ) -> V_22 & V_23 ) &&
F_14 ( V_2 , F_15 ( & V_11 ) , V_24 ) == 0 ) {
F_11 ( V_2 , NULL ) ;
V_9 = F_16 ( V_3 , V_9 , F_15 ( & V_11 ) , V_2 -> V_13 , 0 ) ;
if ( F_17 ( V_9 ) )
return F_18 ( V_9 ) ;
F_11 ( V_2 , V_9 ) ;
}
#endif
V_7 = F_3 ( V_2 ) -> V_4 -> V_25 ;
if ( F_19 ( V_2 ) < V_7 &&
F_20 ( V_2 , F_21 ( V_7 - F_19 ( V_2 ) ) ,
0 , V_26 ) )
return - V_27 ;
return 0 ;
}
static void F_22 ( const struct V_1 * V_2 ,
struct V_28 * V_29 )
{
struct V_30 * V_31 = F_23 ( V_29 ) ;
if ( V_29 -> V_32 == V_33 ) {
const struct V_5 * V_6 = F_4 ( V_2 ) ;
V_31 -> V_17 = V_6 -> V_17 ;
V_31 -> V_18 = V_6 -> V_18 ;
V_31 -> V_16 = V_2 -> V_16 ;
}
}
static int F_24 ( struct V_1 * V_2 ,
const struct V_28 * V_29 )
{
struct V_30 * V_31 = F_23 ( V_29 ) ;
if ( V_29 -> V_32 == V_33 ) {
const struct V_5 * V_6 = F_4 ( V_2 ) ;
if ( ! F_25 ( & V_6 -> V_17 , & V_31 -> V_17 ) ||
! F_25 ( & V_6 -> V_18 , & V_31 -> V_18 ) ||
V_2 -> V_16 != V_31 -> V_16 )
return F_1 ( V_2 ) ;
}
return 0 ;
}
static int F_26 ( struct V_3 * V_3 , struct V_8 * * V_9 ,
struct V_34 * V_35 , bool V_36 )
{
static const struct V_37 V_38 ;
static const struct V_39 V_40 = {
. V_13 . V_14 = 1 ,
. V_41 = (struct V_37 * ) & V_38 ,
} ;
const void * V_13 = V_36 ? & V_40 : NULL ;
struct V_8 * V_42 ;
int V_19 ;
V_42 = F_5 ( V_3 , V_13 , & V_35 -> V_43 . V_44 ) ;
V_19 = V_42 -> error ;
if ( V_19 )
F_9 ( V_42 ) ;
else
* V_9 = V_42 ;
return V_19 ;
}
T_1 F_27 ( struct V_1 * V_2 , unsigned int V_32 ,
unsigned int V_45 , T_2 V_46 )
{
const struct V_5 * V_47 = F_4 ( V_2 ) ;
T_1 V_48 = 0 ;
switch ( V_2 -> V_49 ) {
case V_50 :
if ( V_32 != V_51 && V_32 != V_52 )
break;
if ( ! F_28 ( & V_47 -> V_18 , & V_47 -> V_17 ,
V_2 -> V_53 - V_45 , V_46 ,
F_29 ( V_2 -> V_48 ,
F_30 ( V_2 , 0 ,
V_45 , 0 ) ) ) ) {
V_2 -> V_49 = V_54 ;
break;
}
case V_55 :
V_2 -> V_48 = ~ F_31 (
F_28 ( & V_47 -> V_18 , & V_47 -> V_17 ,
V_2 -> V_53 - V_45 ,
V_46 ,
F_29 ( 0 ,
F_30 ( V_2 , 0 ,
V_45 , 0 ) ) ) ) ;
V_48 = F_32 ( V_2 ) ;
}
return V_48 ;
}
static T_1 F_33 ( struct V_1 * V_2 , unsigned int V_32 ,
unsigned int V_45 , unsigned int V_53 ,
T_2 V_46 )
{
const struct V_5 * V_47 = F_4 ( V_2 ) ;
T_3 V_56 ;
T_1 V_48 = 0 ;
switch ( V_2 -> V_49 ) {
case V_50 :
if ( V_53 == V_2 -> V_53 - V_45 )
return F_27 ( V_2 , V_32 , V_45 , V_46 ) ;
case V_55 :
V_56 = F_30 ( V_2 , 0 , V_45 , 0 ) ;
V_2 -> V_48 = ~ F_31 ( F_28 ( & V_47 -> V_18 ,
& V_47 -> V_17 ,
V_2 -> V_53 - V_45 ,
V_46 ,
F_29 ( 0 , V_56 ) ) ) ;
V_2 -> V_49 = V_55 ;
return F_34 ( V_2 , V_45 + V_53 ) ;
}
return V_48 ;
}
int T_4 F_35 ( void )
{
F_36 ( V_57 , & V_58 ) ;
return F_37 ( & V_59 ) ;
}
void F_38 ( void )
{
F_36 ( V_57 , NULL ) ;
F_39 ( & V_59 ) ;
}
