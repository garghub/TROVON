static void T_1 F_1 ( char * V_1 , int * V_2 )
{
if ( V_2 [ 0 ] == 3 )
F_2 ( V_3 L_1 , V_2 [ 1 ] , V_2 [ 2 ] , V_2 [ 3 ] , V_1 ) ;
else if ( V_2 [ 0 ] == 4 )
F_2 ( V_3 L_2 , V_2 [ 1 ] , V_2 [ 2 ] , V_2 [ 3 ] , V_2 [ 4 ] , V_1 ) ;
else
F_2 ( V_3 L_3 , V_2 [ 1 ] , V_2 [ 2 ] , V_2 [ 3 ] , V_2 [ 4 ] , V_2 [ 5 ] , V_1 ) ;
}
static int T_1 F_3 ( char * V_4 )
{
static short V_5 ;
short V_6 ;
int V_2 [ 6 ] ;
( void ) F_4 ( V_4 , F_5 ( V_2 ) , V_2 ) ;
if ( V_5 >= V_7 ) {
F_2 ( V_3 L_4 L_5 , V_8 ) ;
return 0 ;
}
if ( ( V_2 [ 0 ] < 3 ) || ( V_2 [ 0 ] > 5 ) ) {
F_2 ( V_3 L_6 L_7 L_8 , V_8 ) ;
} else {
for ( V_6 = 0 ; V_6 < V_9 ; V_6 ++ )
if ( V_2 [ 1 ] == V_10 [ V_6 ] )
break;
if ( V_6 == V_9 ) {
F_1 ( L_9 , V_2 ) ;
return 0 ;
} else
V_11 [ V_5 ] . V_12 = V_2 [ 1 ] ;
for ( V_6 = 0 ; V_6 < V_13 ; V_6 ++ )
if ( V_2 [ 2 ] == V_14 [ V_6 ] )
break;
if ( V_6 == V_13 ) {
F_1 ( L_10 , V_2 ) ;
return 0 ;
} else
V_11 [ V_5 ] . V_15 = V_2 [ 2 ] ;
for ( V_6 = 0 ; V_6 < V_16 ; V_6 ++ )
if ( V_2 [ 3 ] == V_17 [ V_6 ] )
break;
if ( V_6 == V_16 ) {
F_1 ( L_11 , V_2 ) ;
return 0 ;
} else
V_11 [ V_5 ] . V_18 = V_2 [ 3 ] ;
if ( V_2 [ 0 ] > 3 ) {
if ( ( V_2 [ 4 ] < 500 ) || ( V_2 [ 4 ] > 31875 ) ) {
F_1 ( L_12 L_13 , V_2 ) ;
V_11 [ V_5 ] . V_19 = V_20 ;
} else
V_11 [ V_5 ] . V_19 = V_2 [ 4 ] / 125 ;
} else
V_11 [ V_5 ] . V_19 = V_20 ;
if ( V_2 [ 0 ] > 4 ) {
if ( ( V_2 [ 5 ] < 500 ) || ( V_2 [ 5 ] > 31875 ) ) {
F_1 ( L_14 L_13 , V_2 ) ;
V_11 [ V_5 ] . V_21 = V_22 ;
} else
V_11 [ V_5 ] . V_21 = V_2 [ 5 ] / 125 ;
} else
V_11 [ V_5 ] . V_21 = V_22 ;
if ( V_5 ) {
for ( V_6 = 0 ; V_6 < ( V_5 - 1 ) ; V_6 ++ ) {
int V_23 = V_6 + 1 ;
for (; V_23 < V_5 ; V_23 ++ )
if ( V_11 [ V_6 ] . V_12 == V_11 [ V_23 ] . V_12 ) {
F_1 ( L_15 , V_2 ) ;
return 0 ;
}
if ( V_11 [ V_6 ] . V_15 == V_11 [ V_23 ] . V_15 ) {
F_1 ( L_16 L_17 , V_2 ) ;
return 0 ;
}
if ( V_11 [ V_6 ] . V_18 == V_11 [ V_23 ] . V_18 ) {
F_1 ( L_18 L_19 , V_2 ) ;
return 0 ;
}
}
}
F_6 ( V_24 L_20
L_21 , V_11 [ V_5 ] . V_12 , V_11 [ V_5 ] . V_15 , V_11 [ V_5 ] . V_18 , V_11 [ V_5 ] . V_19 * 125 , V_11 [ V_5 ] . V_21 * 125 ) ;
V_5 ++ ;
}
return 1 ;
}
static inline void F_7 ( T_2 * V_25 , int V_26 )
{
* V_25 ++ = ( unsigned ) V_26 >> 16 ;
* V_25 ++ = ( unsigned ) V_26 >> 8 ;
* V_25 ++ = V_26 ;
}
static inline int F_8 ( T_2 * V_25 )
{
return ( V_25 [ 0 ] << 16 ) | ( V_25 [ 1 ] << 8 ) | V_25 [ 2 ] ;
}
static inline void F_9 ( T_3 * V_27 )
{
V_27 -> V_28 |= V_29 ;
F_10 ( V_27 -> V_28 , V_27 -> V_18 + V_30 ) ;
}
static inline void F_11 ( T_3 * V_27 )
{
unsigned long V_31 ;
V_27 -> V_28 |= V_32 ;
F_10 ( V_27 -> V_28 , V_27 -> V_18 + V_30 ) ;
V_31 = F_12 () ;
F_13 ( V_27 -> V_15 , V_33 ) ;
F_14 ( V_27 -> V_15 ) ;
F_15 ( V_31 ) ;
}
static inline short F_16 ( unsigned V_34 , unsigned V_35 , unsigned V_36 , unsigned V_37 )
{
unsigned V_38 ;
unsigned long V_39 = V_40 + V_41 ;
while ( F_17 ( V_40 , V_39 ) ) {
V_38 = F_18 ( V_34 ) & V_35 ;
if ( ( ( V_38 & V_36 ) == V_36 ) && ( ( V_38 & V_37 ) == 0 ) )
return ( 0 ) ;
}
return ( 1 ) ;
}
static inline int F_19 ( T_3 * V_27 , T_2 * V_42 , int V_43 )
{
if ( ! F_16 ( V_27 -> V_18 + V_44 , V_45 , V_46 , 0 ) ) {
while ( V_43 -- ) {
do {
F_10 ( * V_42 , V_27 -> V_18 + V_47 ) ;
F_16 ( V_27 -> V_18 + V_44 , V_45 , V_46 , 0 ) ;
} while ( F_18 ( V_27 -> V_18 + V_44 ) & V_48 );
V_42 ++ ;
}
return ( 1 ) ;
}
F_2 ( V_49 L_22 , V_43 + 1 ) ;
return ( 0 ) ;
}
static inline T_4 * F_20 ( struct V_50 * V_27 , int V_51 )
{
T_4 * V_52 , * V_53 = NULL ;
unsigned long V_31 ;
unsigned long V_54 = V_40 + V_41 ;
unsigned long V_55 ;
int V_6 ;
if ( V_51 <= 0 )
return ( NULL ) ;
F_21 ( V_27 -> V_56 ) ;
V_57:
while ( V_58 < V_51 ) {
V_54 = V_40 + V_41 ;
do {
for ( V_55 = V_40 ; V_55 == V_40 ; )
F_22 () ;
} while ( V_58 < V_51 && F_17 ( V_40 , V_54 ) );
if ( V_58 < V_51 ) {
F_2 ( V_3 L_23 ) ;
return ( NULL ) ;
}
}
F_23 ( & V_59 , V_31 ) ;
if ( V_58 < V_51 ) {
F_24 ( & V_59 , V_31 ) ;
goto V_57;
}
V_52 = V_60 ;
V_58 -= V_51 ;
for ( V_6 = 0 ; V_6 < V_51 ; V_6 ++ ) {
V_53 = V_60 ;
V_60 = V_53 -> V_61 ;
}
V_53 -> V_61 = NULL ;
F_24 ( & V_59 , V_31 ) ;
F_25 ( V_27 -> V_56 ) ;
return ( V_52 ) ;
}
static inline void F_26 ( T_4 * V_52 )
{
unsigned long V_31 ;
F_23 ( & V_59 , V_31 ) ;
memset ( V_52 , 0 , sizeof( T_4 ) ) ;
V_52 -> V_61 = V_60 ;
V_60 = V_52 ;
V_58 ++ ;
F_24 ( & V_59 , V_31 ) ;
}
static inline void F_27 ( void )
{
int V_6 ;
F_28 ( & V_59 ) ;
V_60 = & ( V_62 [ 0 ] ) ;
memset ( V_62 , 0 , sizeof( V_62 ) ) ;
for ( V_6 = 0 ; V_6 < V_63 - 1 ; V_6 ++ ) {
V_62 [ V_6 ] . V_61 = & ( V_62 [ V_6 + 1 ] ) ;
V_62 [ V_6 ] . V_64 = NULL ;
}
V_62 [ V_63 - 1 ] . V_61 = NULL ;
V_62 [ V_63 - 1 ] . V_64 = NULL ;
}
static int F_29 ( T_3 * V_27 , T_4 * V_65 )
{
int V_6 , V_66 ;
unsigned long V_31 ;
T_2 V_67 ;
T_5 * V_68 = V_27 -> V_69 . V_66 ;
int * V_70 = & ( V_27 -> V_70 ) ;
F_6 ( L_24 , ( long ) V_65 ) ;
F_23 ( V_27 -> V_71 -> V_56 , V_31 ) ;
V_66 = * V_70 ;
for ( V_6 = 0 ; V_6 < V_72 ; V_6 ++ ) {
if ( V_68 [ V_66 ] . V_73 == 0 ) {
F_6 ( L_25 , V_66 ) ;
V_68 [ V_66 ] . V_73 = 1 ;
F_7 ( ( T_2 * ) V_68 [ V_66 ] . V_65 , ( int ) V_65 ) ;
* V_70 = ( V_66 + 1 ) % V_72 ;
break;
} else
V_66 = ( V_66 + 1 ) % V_72 ;
}
F_24 ( V_27 -> V_71 -> V_56 , V_31 ) ;
F_6 ( L_26 , ( long ) V_65 ) ;
if ( V_6 >= V_72 ) {
F_6 ( L_27 ) ;
return ( 0 ) ;
}
F_9 ( V_27 ) ;
V_67 = V_74 | V_66 ;
F_19 ( V_27 , & V_67 , 1 ) ;
F_6 ( L_28 ) ;
return ( 1 ) ;
}
static int F_30 ( unsigned V_75 , unsigned V_76 )
{
#ifdef F_31
int V_77 = V_75 ;
#endif
switch ( ( V_75 >> 8 ) & 0xff ) {
case 0 :
V_75 = V_78 ;
break;
case 1 :
V_75 = V_79 ;
break;
case 2 :
V_75 = V_79 ;
break;
case 4 :
V_75 = V_80 ;
break;
case 5 :
V_75 = V_81 ;
break;
case 6 :
V_75 = V_82 ;
break;
case 80 :
case 81 :
V_75 = V_83 ;
break;
case 82 :
V_75 = V_84 ;
break;
case 83 :
case 84 :
V_75 = V_81 ;
break;
default:
V_75 = V_78 ;
}
#ifdef F_31
if ( V_76 || V_75 )
F_6 ( L_29 , V_76 , V_77 , V_75 ) ;
#endif
return ( V_76 | ( V_75 << 16 ) ) ;
}
static T_6 F_32 ( int V_12 , void * V_85 )
{
T_3 * V_27 = ( T_3 * ) V_85 ;
int V_86 , V_87 , V_88 , V_89 ;
int V_90 , V_91 ;
T_4 * V_52 ;
T_7 * V_92 ;
struct V_93 * V_64 ;
T_5 * V_94 = V_27 -> V_69 . V_87 ;
unsigned long V_31 ;
F_23 ( V_27 -> V_71 -> V_56 , V_31 ) ;
V_27 -> V_95 ++ ;
F_6 ( L_30 , V_12 , ( long ) V_27 ) ;
V_86 = F_18 ( V_27 -> V_18 + V_96 ) ;
F_6 ( L_31 , V_86 ) ;
if ( ! ( F_18 ( V_27 -> V_18 + V_44 ) & V_97 ) ) {
F_6 ( L_32 ) ;
goto V_98;
}
if ( ! ( V_86 & V_99 ) )
goto V_98;
if ( ! ( V_86 & V_100 ) ) {
F_6 ( L_33 ) ;
goto V_98;
}
V_87 = V_86 & V_101 ;
V_89 = V_94 [ V_87 ] . V_73 ;
if ( V_89 & 0x80 ) {
F_6 ( L_34 , V_89 ) ;
goto V_98;
}
V_52 = F_33 ( F_8 ( ( T_2 * ) V_94 [ V_87 ] . V_65 ) ) ;
V_94 [ V_87 ] . V_73 = 0 ;
if ( V_52 -> V_102 & V_103 ) {
V_92 = ( T_7 * ) V_52 ;
V_92 -> V_73 = V_89 ;
V_92 -> V_104 = 0 ;
goto V_98;
}
V_64 = V_52 -> V_64 ;
if ( -- ( V_64 -> V_105 . V_104 ) <= 0 ) {
V_90 = V_52 -> V_106 | ( V_89 << 8 ) ;
V_91 = V_52 -> V_73 ;
V_88 = F_30 ( V_90 , V_91 ) ;
V_64 -> V_107 = V_88 ;
F_26 ( V_52 ) ;
V_64 -> V_108 ( V_64 ) ;
}
V_98:
F_6 ( L_35 ) ;
F_34 ( V_27 ) ;
F_24 ( V_27 -> V_71 -> V_56 , V_31 ) ;
return V_109 ;
}
static int F_35 ( struct V_93 * V_64 ,
void (* F_36)( struct V_93 * ) )
{
T_4 * V_52 ;
T_8 * V_110 ;
T_2 * V_111 = ( T_2 * ) V_64 -> V_112 ;
T_2 V_113 ;
short V_114 ;
int V_115 ;
T_3 * V_27 = ( T_3 * ) V_64 -> V_116 -> V_27 -> V_117 ;
V_114 = V_64 -> V_118 ;
V_113 = ( ( V_64 -> V_116 -> V_119 << 5 ) & 0xe0 ) | ( V_64 -> V_116 -> V_120 & 7 ) ;
V_64 -> V_108 = F_36 ;
V_64 -> V_105 . V_104 = 1 ;
V_52 = F_20 ( V_64 -> V_116 -> V_27 , 1 ) ;
V_52 -> V_113 = V_113 ;
memcpy ( V_52 -> V_111 , V_111 , V_114 ) ;
V_52 -> V_121 = 0x40 ;
V_52 -> V_64 = V_64 ;
V_64 -> V_122 = ( T_2 * ) V_52 ;
V_52 -> V_27 = V_27 ;
V_115 = F_37 ( V_64 ) ;
if ( V_115 > 1 ) {
struct V_123 * V_124 ;
unsigned V_6 ;
F_6 ( L_36 , V_115 ) ;
V_110 = V_52 -> V_110 ;
V_52 -> V_102 = 1 ;
F_7 ( V_52 -> V_125 , ( int ) V_110 ) ;
F_7 ( V_52 -> V_126 , V_115 * sizeof( T_8 ) ) ;
F_38 (SCpnt, sg, nseg, i) {
F_7 ( V_110 [ V_6 ] . V_127 , F_39 ( F_40 ( V_124 ) ) + V_124 -> V_128 ) ;
F_7 ( V_110 [ V_6 ] . V_43 , V_124 -> V_129 ) ;
}
} else {
V_52 -> V_102 = 0 ;
if ( V_115 ) {
struct V_123 * V_124 = F_41 ( V_64 ) ;
F_7 ( V_52 -> V_125 , F_39 ( F_40 ( V_124 ) ) + V_124 -> V_128 ) ;
}
F_7 ( V_52 -> V_126 , F_42 ( V_64 ) ) ;
}
while ( ! F_29 ( V_27 , V_52 ) )
F_22 () ;
return 0 ;
}
int F_43 ( T_3 * V_27 )
{
T_9 V_130 = {
V_131 ,
7 ,
V_27 -> V_19 ,
V_27 -> V_21 ,
0 ,
{ 0 , 0 , 0 } ,
V_72 ,
V_132
} ;
int V_133 ;
F_10 ( V_134 , V_27 -> V_18 + V_30 ) ;
F_44 ( 40 ) ;
F_10 ( 0 , V_27 -> V_18 + V_30 ) ;
V_27 -> V_28 = 0 ;
if ( F_16 ( V_27 -> V_18 + V_44 , V_45 , V_46 , 0 ) ) {
F_2 ( V_3 L_37 ) ;
return - 1 ;
}
if ( ( V_133 = F_18 ( V_27 -> V_18 + V_96 ) ) != 1 ) {
F_2 ( L_38 ) ;
switch ( V_133 ) {
case 2 :
F_2 ( V_3 L_39 ) ;
break;
case 3 :
F_2 ( V_3 L_40 ) ;
break;
case 4 :
F_2 ( V_3 L_41 ) ;
break;
case 5 :
F_2 ( V_3 L_42 ) ;
break;
case 6 :
F_2 ( V_3 L_43 ) ;
break;
case 7 :
F_2 ( V_3 L_44 ) ;
break;
default:
F_2 ( V_3 L_45 , V_133 ) ;
}
return - 1 ;
}
memset ( & ( V_27 -> V_69 ) , 0 , sizeof( V_27 -> V_69 ) ) ;
F_7 ( ( T_2 * ) & ( V_130 . V_135 ) , ( int ) & ( V_27 -> V_69 ) ) ;
if ( ! F_19 ( V_27 , ( T_2 * ) & V_130 , sizeof( V_130 ) ) ) {
F_2 ( V_3 L_46 ) ;
return - 1 ;
}
if ( F_16 ( V_27 -> V_18 + V_44 , V_45 , V_136 , 0 ) ) {
F_2 ( L_47 ) ;
return - 1 ;
}
return 0 ;
}
static int F_45 ( T_3 * V_27 )
{
if ( F_43 ( V_27 ) == - 1 )
return 0 ;
if ( F_46 ( V_27 -> V_12 , F_32 , 0 , L_48 , V_27 ) ) {
F_2 ( L_49 , V_27 -> V_12 ) ;
return ( 0 ) ;
}
if ( F_47 ( V_27 -> V_15 , L_48 ) ) {
F_2 ( L_50 , V_27 -> V_15 ) ;
F_48 ( V_27 -> V_12 , V_27 ) ;
return ( 0 ) ;
}
F_11 ( V_27 ) ;
F_9 ( V_27 ) ;
if ( ! F_49 ( V_27 , V_137 ) ) {
F_50 ( V_27 -> V_15 ) ;
F_48 ( V_27 -> V_12 , NULL ) ;
return ( 0 ) ;
}
return ( 1 ) ;
}
static void F_51 ( T_3 * V_27 )
{
static T_10 V_92 = { V_138 } ;
V_92 . V_104 = 1 ;
F_29 ( V_27 , (struct V_52 * ) & V_92 ) ;
while ( V_92 . V_104 ) {
F_22 () ;
F_52 () ;
}
V_27 -> V_139 = V_92 . V_140 ;
V_27 -> V_141 = V_92 . V_142 ;
}
static int F_53 ( struct V_50 * V_27 , char * V_143 , int V_129 )
{
F_6 ( L_51 , V_129 , V_143 , V_129 ) ;
F_6 ( L_52 ) ;
return ( V_129 ) ;
}
static int F_54 ( struct V_144 * V_145 , struct V_50 * V_27 )
{
T_3 * V_146 = ( T_3 * ) V_27 -> V_117 ;
unsigned long V_31 ;
#ifdef F_31
T_5 * V_68 , * V_94 ;
short V_147 ;
#endif
F_23 ( V_27 -> V_56 , V_31 ) ;
SPRINTF ( L_53 , V_27 -> V_148 , V_146 -> V_139 , V_146 -> V_141 ) ;
SPRINTF ( L_54 , V_146 -> V_18 ) ;
SPRINTF ( L_55 , V_146 -> V_12 ) ;
SPRINTF ( L_56 , V_146 -> V_15 ) ;
SPRINTF ( L_57 , V_146 -> V_95 ) ;
SPRINTF ( L_58 , V_146 -> V_19 * 125 ) ;
SPRINTF ( L_59 , V_146 -> V_21 * 125 ) ;
#ifdef F_31
V_68 = V_146 -> V_69 . V_66 ;
V_94 = V_146 -> V_69 . V_87 ;
SPRINTF ( L_60 , V_146 -> V_28 ) ;
SPRINTF ( L_61 ) ;
SPRINTF ( L_62 , V_132 ) ;
SPRINTF ( L_63 ) ;
for ( V_6 = V_147 = 0 ; V_6 < V_132 ; V_6 ++ )
if ( V_94 [ V_6 ] . V_73 ) {
V_147 ++ ;
SPRINTF ( L_64 , V_6 ) ;
}
SPRINTF ( V_147 ? L_65 : L_66 ) ;
SPRINTF ( L_67 ) ;
SPRINTF ( L_62 , V_72 ) ;
SPRINTF ( L_68 , V_146 -> V_70 ) ;
SPRINTF ( L_63 ) ;
for ( V_6 = V_147 = 0 ; V_6 < V_72 ; V_6 ++ )
if ( V_68 [ V_6 ] . V_73 ) {
V_147 ++ ;
SPRINTF ( L_64 , V_6 ) ;
}
SPRINTF ( V_147 ? L_65 : L_66 ) ;
#endif
F_24 ( V_27 -> V_56 , V_31 ) ;
return 0 ;
}
static T_1 int F_55 ( struct V_149 * V_150 )
{
short V_151 = 0 , V_152 , V_153 , V_6 , V_154 ;
short V_155 [ V_7 ] ;
unsigned V_18 ;
T_3 * V_27 = NULL ;
struct V_50 * V_71 ;
int V_156 = 0 ;
F_6 ( L_69 ) ;
#ifdef F_56
if ( V_157 )
F_3 ( V_157 ) ;
#endif
for ( V_6 = 0 ; V_6 < V_158 ; V_159 [ V_6 ++ ] = NULL ) ;
for ( V_6 = 0 ; V_6 < V_7 ; V_155 [ V_6 ++ ] = - 1 ) ;
V_150 -> V_160 = L_48 ;
V_150 -> V_161 = & F_54 ;
V_150 -> V_162 = F_53 ;
F_27 () ;
for ( V_154 = 0 ; V_154 < V_7 ; V_154 ++ ) {
for ( V_152 = 0 ; V_152 < V_163 ; V_152 ++ )
for ( V_153 = 0 ; V_153 < V_164 ; V_153 ++ ) {
for ( V_6 = 0 ; V_6 < V_154 ; V_6 ++ )
if ( V_155 [ V_6 ] == V_152 )
break;
if ( V_6 == V_154 ) {
void T_11 * V_165 = F_57 ( V_166 [ V_152 ] + V_167 [ V_153 ] . V_168 ,
V_167 [ V_153 ] . V_43 ) ;
short V_169 = 1 ;
if ( V_165 )
V_169 = F_58 ( V_165 , V_167 [ V_153 ] . V_170 , V_167 [ V_153 ] . V_43 ) ;
F_59 ( V_165 ) ;
if ( V_169 )
goto V_171;
}
}
V_171:
#ifdef F_31
F_6 ( L_70 , V_154 + 1 ) ;
if ( V_152 == V_163 )
F_6 ( L_71 ) ;
else
F_6 ( L_72 , V_166 [ V_152 ] ) ;
#endif
if ( V_11 [ V_154 ] . V_12 < 0 )
continue;
if ( V_156 == V_158 )
continue;
V_18 = V_11 [ V_154 ] . V_18 ;
F_6 ( L_73 , V_18 ) ;
if ( F_60 ( V_18 , 4 , L_48 ) ) {
F_6 ( L_74 , V_18 ) ;
F_10 ( V_134 , V_18 + V_30 ) ;
F_61 ( 10 ) ;
F_10 ( 0 , V_18 + V_30 ) ;
if ( F_16 ( V_18 + V_44 , V_45 , V_46 , 0 ) ) {
F_6 ( L_75 ) ;
goto V_172;
} else
F_6 ( L_76 ) ;
if ( F_18 ( V_18 + V_96 ) == 1 ) {
V_71 = F_62 ( V_150 , sizeof( T_3 ) ) ;
if ( V_71 == NULL )
goto V_172;
V_27 = ( T_3 * ) V_71 -> V_117 ;
F_6 ( L_77 , ( int ) V_27 ) ;
memset ( V_27 , 0 , sizeof( T_3 ) ) ;
V_27 -> V_12 = V_11 [ V_154 ] . V_12 ;
V_27 -> V_15 = V_11 [ V_154 ] . V_15 ;
V_27 -> V_18 = V_18 ;
V_27 -> V_95 = 0 ;
V_27 -> V_19 = V_11 [ V_154 ] . V_19 ;
V_27 -> V_21 = V_11 [ V_154 ] . V_21 ;
V_27 -> V_71 = V_159 [ V_156 ] = V_71 ;
V_156 ++ ;
F_6 ( L_78 L_79 , V_27 -> V_18 , V_27 -> V_12 , V_27 -> V_15 ) ;
if ( ! F_45 ( V_27 ) )
goto V_173;
F_51 ( V_27 ) ;
if ( V_27 -> V_139 < 6 )
V_71 -> V_174 = 1 ;
V_151 ++ ;
if ( V_152 != V_163 )
V_155 [ V_154 ] = V_152 ;
F_2 ( V_175 L_80 , V_27 -> V_139 , V_27 -> V_141 ) ;
F_2 ( L_81 , V_27 -> V_18 , V_27 -> V_12 , V_27 -> V_15 ) ;
F_2 ( L_82 , V_27 -> V_19 * 125 , V_27 -> V_21 * 125 ) ;
}
} else
F_6 ( L_83 , V_18 ) ;
continue;
V_173:
F_63 ( V_71 ) ;
V_172:
F_64 ( V_18 , 4 ) ;
}
if ( ! V_151 )
F_2 ( L_84 ) ;
return ( V_151 ) ;
}
static int F_65 ( struct V_50 * V_176 )
{
if ( V_176 -> V_12 )
F_48 ( V_176 -> V_12 , NULL ) ;
if ( V_176 -> V_177 && V_176 -> V_178 )
F_64 ( V_176 -> V_177 , V_176 -> V_178 ) ;
F_63 ( V_176 ) ;
return 0 ;
}
static int F_66 ( struct V_93 * V_64 )
{
T_3 * V_27 = ( T_3 * ) V_64 -> V_116 -> V_27 -> V_117 ;
F_25 ( V_64 -> V_116 -> V_27 -> V_56 ) ;
if ( F_43 ( V_27 ) < 0 ) {
F_21 ( V_64 -> V_116 -> V_27 -> V_56 ) ;
return V_179 ;
}
F_9 ( V_27 ) ;
F_21 ( V_64 -> V_116 -> V_27 -> V_56 ) ;
return V_180 ;
}
static int F_67 ( struct V_181 * V_182 ,
struct V_183 * V_184 , T_12 V_185 , int * V_186 )
{
char V_187 [ V_188 ] ;
F_6 ( L_85 ,
F_68 ( V_184 , V_187 ) , V_185 ) ;
( void ) V_187 ;
V_186 [ 0 ] = 64 ;
V_186 [ 1 ] = 32 ;
V_186 [ 2 ] = V_185 >> 11 ;
if ( V_186 [ 2 ] >= 1024 ) {
int V_189 [ 3 ] ;
if ( ( F_69 ( V_184 , V_185 , V_189 ) < 0 ) || ! ( ( ( V_189 [ 0 ] == 64 ) && ( V_189 [ 1 ] == 32 ) ) || ( ( V_189 [ 0 ] == 255 ) && ( V_189 [ 1 ] == 63 ) ) ) ) {
F_2 ( L_86 L_87 ) ;
V_186 [ 0 ] = 255 ;
V_186 [ 1 ] = 63 ;
V_186 [ 2 ] = ( unsigned long ) V_185 / ( 255 * 63 ) ;
} else {
V_186 [ 0 ] = V_189 [ 0 ] ;
V_186 [ 1 ] = V_189 [ 1 ] ;
V_186 [ 2 ] = V_189 [ 2 ] ;
if ( V_189 [ 0 ] == 255 )
F_2 ( V_175 L_88 L_89 , V_8 ) ;
}
}
F_6 ( L_90 , V_186 [ 0 ] , V_186 [ 1 ] , V_186 [ 2 ] ) ;
F_6 ( L_91 ) ;
return ( 0 ) ;
}
