void F_1 ( void )
{
T_1 V_1 ;
V_1 = F_2 ( V_2 + V_3 ) ;
V_1 &= ~ ( V_4 |
V_5 ) ;
V_1 |= V_6 |
V_7 ;
F_3 ( V_1 , V_2 + V_3 ) ;
}
void F_4 ( void )
{
T_1 V_1 ;
V_1 = F_2 ( V_2 + V_3 ) ;
V_1 |= V_8 ;
F_3 ( V_1 , V_2 + V_3 ) ;
}
void F_5 ( void )
{
T_1 V_1 ;
V_1 = F_2 ( V_2 + V_9 ) ;
V_1 &= ~ V_10 ;
V_1 |= V_11 |
V_12 ;
F_3 ( V_1 , V_2 + V_9 ) ;
}
void F_6 ( void )
{
T_1 V_1 ;
V_1 = F_2 ( V_2 + V_9 ) ;
V_1 |= V_13 ;
F_3 ( V_1 , V_2 + V_9 ) ;
}
static inline void F_7 ( void T_2 * V_14 ,
struct V_15 * V_16 ,
T_3 V_17 , T_1 V_18 , T_1 V_19 )
{
T_1 V_20 = F_2 ( V_14 + V_16 -> V_21 [ V_17 ] ) ;
V_20 &= V_19 ;
V_18 &= V_19 ;
if ( V_20 != V_18 ) {
F_8 ( L_1 ,
V_17 , V_20 , V_18 ) ;
F_8 ( L_2 , V_19 ) ;
V_16 -> V_22 = false ;
}
}
static void F_9 ( struct V_15 * V_16 )
{
T_1 V_18 ;
V_18 = V_23 & ( ~ V_24 ) ;
F_7 ( V_2 , V_16 , 0 , V_18 ,
V_25 ) ;
V_18 = V_26 & ( ~ V_27 ) ;
F_7 ( V_2 , V_16 , 1 , V_18 ,
V_28 ) ;
V_18 = V_29 ;
F_7 ( V_2 , V_16 , 2 , V_18 ,
V_30 ) ;
V_18 = V_31 ;
F_7 ( V_2 , V_16 , 3 , V_18 ,
V_32 ) ;
}
static void F_10 ( const char * V_33 ,
struct V_34 * V_35 )
{
V_35 -> V_16 -> V_22 = true ;
if ( F_2 ( V_2 + V_35 -> V_16 -> V_36 ) &
V_37 ) {
F_9 ( V_35 -> V_16 ) ;
F_8 ( L_3 ,
V_33 ) ;
return;
}
F_3 ( V_23 ,
V_2 + V_35 -> V_16 -> V_21 [ 0 ] ) ;
F_3 ( V_26 ,
V_2 + V_35 -> V_16 -> V_21 [ 1 ] ) ;
F_3 ( V_29 ,
V_2 + V_35 -> V_16 -> V_21 [ 2 ] ) ;
F_3 ( V_31 ,
V_2 + V_35 -> V_16 -> V_21 [ 3 ] ) ;
F_11 ( 1 ) ;
}
static void F_12 ( struct V_34 * V_35 )
{
F_10 ( L_4 , V_35 ) ;
}
static void F_13 ( struct V_34 * V_35 )
{
F_10 ( L_5 , V_35 ) ;
}
static void F_14 ( struct V_34 * V_35 )
{
F_10 ( L_6 , V_35 ) ;
}
static void F_15 ( struct V_34 * V_35 )
{
F_10 ( L_7 , V_35 ) ;
}
static void F_16 ( struct V_34 * V_38 )
{
T_1 V_19 ;
T_1 V_1 = F_2 ( V_2 + V_38 -> V_16 -> V_36 ) ;
V_38 -> V_16 -> V_22 = true ;
if ( V_1 & V_37 ) {
if ( V_1 & V_39 ) {
F_8 ( L_8 ) ;
V_38 -> V_16 -> V_22 = false ;
}
F_8 ( L_9 ) ;
V_1 = V_40 ;
V_19 = V_41 | V_42 ;
F_7 ( V_2 , V_38 -> V_16 , 0 , V_1 ,
~ V_19 & V_43 ) ;
V_1 = V_44 ;
F_7 ( V_2 , V_38 -> V_16 , 2 , V_1 ,
V_45 ) ;
V_1 = F_2 ( V_2 + V_38 -> V_16 -> V_21 [ 0 ] ) ;
V_1 &= ~ V_19 ;
V_1 |= V_40 & V_19 ;
F_3 ( V_1 , V_2 + V_38 -> V_16 -> V_21 [ 0 ] ) ;
F_11 ( 1 ) ;
return;
}
V_1 |= V_39 ;
F_3 ( V_1 , V_2 + V_38 -> V_16 -> V_36 ) ;
F_3 ( V_40 ,
V_2 + V_38 -> V_16 -> V_21 [ 0 ] ) ;
F_3 ( V_44 ,
V_2 + V_38 -> V_16 -> V_21 [ 2 ] ) ;
F_11 ( 1 ) ;
}
static void F_17 ( struct V_34 * V_46 )
{
T_1 V_1 ;
T_1 V_19 = 0xffff ;
V_46 -> V_16 -> V_22 = true ;
if ( F_2 ( V_2 + V_46 -> V_16 -> V_36 ) &
V_37 ) {
F_8 ( L_10 ) ;
V_1 = V_47 ;
F_7 ( V_2 , V_46 -> V_16 , 1 ,
V_1 , V_48 ) ;
V_1 = V_49 & ( ~ V_50 ) ;
V_19 |= V_51 | V_52 |
V_53 | V_54 ;
F_7 ( V_2 , V_46 -> V_16 , 0 , V_1 ,
~ V_19 & V_55 ) ;
V_19 = V_52 | V_53 ;
V_1 = F_2 ( V_2 + V_46 -> V_16 -> V_21 [ 0 ] ) ;
V_1 &= ~ V_19 ;
V_1 |= V_49 & V_19 ;
F_3 ( V_1 , V_2 + V_46 -> V_16 -> V_21 [ 0 ] ) ;
F_11 ( 1 ) ;
return;
}
V_1 = F_2 ( V_2 + V_46 -> V_16 -> V_21 [ 0 ] ) ;
V_1 &= V_51 ;
V_1 |= V_49 ;
F_3 ( V_1 , V_2 + V_46 -> V_16 -> V_21 [ 0 ] ) ;
F_3 ( V_47 , V_2 +
V_46 -> V_16 -> V_21 [ 1 ] ) ;
F_11 ( 1 ) ;
}
static void F_18 ( const char * V_56 , struct V_34 * V_57 ,
T_1 V_58 , T_1 V_59 , T_1 V_60 , T_1 V_61 )
{
T_1 V_18 ;
T_1 V_1 = F_2 ( V_2 + V_57 -> V_16 -> V_36 ) ;
V_57 -> V_16 -> V_22 = true ;
if ( V_1 & V_37 ) {
F_8 ( L_3 ,
V_56 ) ;
if ( V_1 & V_62 ) {
F_8 ( L_11 ) ;
V_57 -> V_16 -> V_22 = false ;
}
V_18 = V_58 ;
F_7 ( V_2 , V_57 -> V_16 , 0 , V_18 ,
V_63 &
( ~ V_64 ) ) ;
if ( V_57 -> V_16 -> V_65 ) {
V_18 = V_59 ;
F_7 ( V_2 , V_57 -> V_16 , 1 ,
V_18 , V_66 ) ;
V_18 = V_60 ;
F_7 ( V_2 , V_57 -> V_16 , 2 ,
V_18 , V_67 ) ;
V_18 = V_61 ;
F_7 ( V_2 , V_57 -> V_16 , 3 ,
V_18 , V_68 ) ;
} else if ( V_57 -> V_16 -> V_21 [ 1 ] ) {
V_18 = V_59 ;
F_7 ( V_2 , V_57 -> V_16 , 1 ,
V_18 , V_66 &
( ~ V_69 ) ) ;
}
if ( V_1 & V_70 ) {
V_1 &= ~ V_70 ;
F_3 ( V_1 , V_2 +
V_57 -> V_16 -> V_36 ) ;
}
V_1 = F_2 ( V_2 + V_57 -> V_16 -> V_21 [ 0 ] ) ;
V_1 &= ~ V_64 ;
V_1 |= V_58 & V_64 ;
F_3 ( V_1 , V_2 + V_57 -> V_16 -> V_21 [ 0 ] ) ;
F_11 ( 1 ) ;
return;
}
V_1 |= V_62 ;
V_1 &= ~ V_70 ;
F_3 ( V_1 , V_2 + V_57 -> V_16 -> V_36 ) ;
if ( ! V_57 -> V_16 -> V_21 [ 1 ] ) {
F_3 ( V_58 , V_2 +
V_57 -> V_16 -> V_21 [ 0 ] ) ;
F_11 ( 1 ) ;
return;
}
F_3 ( V_58 , V_2 +
V_57 -> V_16 -> V_21 [ 0 ] ) ;
F_3 ( V_59 & ( ~ V_71 ) ,
V_2 + V_57 -> V_16 -> V_21 [ 1 ] ) ;
F_3 ( V_60 , V_2 + V_57 -> V_16 -> V_21 [ 2 ] ) ;
F_3 ( V_61 , V_2 + V_57 -> V_16 -> V_21 [ 3 ] ) ;
F_11 ( 1 ) ;
}
static void F_19 ( struct V_34 * V_72 )
{
F_18 ( L_12 , V_72 , V_73 ,
V_74 ,
V_75 ,
V_76 ) ;
}
static void F_20 ( struct V_34 * V_77 )
{
F_18 ( L_13 , V_77 , V_78 ,
V_79 ,
V_80 ,
V_81 ) ;
}
static void F_21 ( struct V_34 * V_82 )
{
F_18 ( L_14 , V_82 , V_83 , 0 , 0 , 0 ) ;
}
static void F_22 ( struct V_34 * V_84 )
{
T_1 V_19 ;
T_1 V_1 = F_2 ( V_2 + V_84 -> V_16 -> V_36 ) ;
V_84 -> V_16 -> V_22 = true ;
if ( V_1 & V_37 ) {
F_8 ( L_15 ) ;
V_1 &= V_85 ;
if ( V_1 != V_86 ) {
F_8 ( L_16 ,
V_1 , V_86 ) ;
F_8 ( L_17 ,
V_85 ) ;
V_84 -> V_16 -> V_22 = false ;
}
V_1 = V_87 & ( ~ V_88 ) ;
V_19 = V_89 | V_90 ;
F_7 ( V_2 , V_84 -> V_16 , 0 , V_1 ,
~ V_19 & V_91 ) ;
V_1 = F_2 ( V_2 + V_84 -> V_16 -> V_21 [ 0 ] ) ;
V_1 &= ~ V_19 ;
V_1 |= V_87 & V_19 ;
F_3 ( V_1 , V_2 + V_84 -> V_16 -> V_21 [ 0 ] ) ;
F_11 ( 1 ) ;
return;
}
V_1 &= ~ V_85 ;
V_1 |= V_86 & V_85 ;
F_3 ( V_1 , V_2 + V_84 -> V_16 -> V_36 ) ;
F_3 ( V_87 ,
V_2 + V_84 -> V_16 -> V_21 [ 0 ] ) ;
F_11 ( 1 ) ;
}
static void F_23 ( struct V_92 * V_93 , T_1 * V_94 , T_1 * V_95 )
{
unsigned long V_96 ;
if ( ! F_24 ( V_93 -> V_97 ) )
V_96 = F_25 ( F_26 ( V_93 ) ) ;
else
V_96 = 38400000 ;
V_96 /= F_27 ( V_93 , V_96 ) ;
switch ( V_96 ) {
case 12000000 :
case 12800000 :
case 13000000 :
* V_94 = 0x2B ;
* V_95 = 0x0B ;
return;
case 19200000 :
* V_94 = 0x12 ;
* V_95 = 0x08 ;
return;
case 38400000 :
* V_94 = 0x04 ;
* V_95 = 0x05 ;
return;
default:
F_28 ( L_18 ,
V_98 , V_96 ) ;
F_29 () ;
}
}
static void F_30 ( struct V_34 * V_99 )
{
T_1 V_18 ;
V_18 = V_100 ;
F_7 ( V_2 , V_99 -> V_16 , 0 , V_18 ,
V_101 & ( ~ V_102 ) ) ;
V_18 = V_103 ;
F_7 ( V_2 , V_99 -> V_16 , 1 , V_18 ,
V_104 ) ;
V_18 = V_105 ;
F_7 ( V_2 , V_99 -> V_16 , 2 ,
V_18 , V_106 ) ;
V_18 = V_107 & ( ~ V_108 ) ;
F_7 ( V_2 , V_99 -> V_16 , 3 , V_18 ,
V_109 ) ;
V_18 = V_110 ;
F_7 ( V_2 , V_99 -> V_16 , 4 , V_18 ,
V_111 ) ;
V_18 = V_112 ;
F_7 ( V_2 , V_99 -> V_16 , 5 , V_18 ,
V_113 ) ;
}
static void F_31 ( struct V_34 * V_114 )
{
T_1 V_1 ;
T_1 V_94 , V_95 ;
V_114 -> V_16 -> V_22 = true ;
F_23 ( & V_114 -> V_93 , & V_94 , & V_95 ) ;
V_1 = V_105 & ( ~ V_115 ) &
( ~ V_116 ) ;
V_1 |= V_94 << V_117 ;
V_1 |= V_95 << V_118 ;
if ( F_2 ( V_2 + V_114 -> V_16 -> V_36 ) & V_37 ) {
F_8 ( L_19 ) ;
F_30 ( V_114 ) ;
F_3 ( V_1 , V_2 + V_114 -> V_16 -> V_21 [ 2 ] ) ;
V_1 = F_2 ( V_2 + V_114 -> V_16 -> V_21 [ 0 ] ) ;
V_1 &= ~ V_102 ;
V_1 |= V_100 & V_102 ;
F_3 ( V_1 , V_2 + V_114 -> V_16 -> V_21 [ 0 ] ) ;
F_11 ( 1 ) ;
return;
}
F_3 ( V_100 , V_2 +
V_114 -> V_16 -> V_21 [ 0 ] ) ;
F_3 ( V_103 , V_2 +
V_114 -> V_16 -> V_21 [ 1 ] ) ;
F_3 ( V_1 , V_2 + V_114 -> V_16 -> V_21 [ 2 ] ) ;
F_3 ( V_107 , V_2 +
V_114 -> V_16 -> V_21 [ 3 ] ) ;
F_3 ( V_110 , V_2 +
V_114 -> V_16 -> V_21 [ 4 ] ) ;
F_3 ( V_112 , V_2 +
V_114 -> V_16 -> V_21 [ 5 ] ) ;
F_11 ( 1 ) ;
}
static void F_32 ( struct V_34 * V_119 )
{
T_1 V_19 , V_1 = F_2 ( V_2 + V_119 -> V_16 -> V_36 ) ;
V_119 -> V_16 -> V_22 = true ;
if ( V_1 & V_37 ) {
F_8 ( L_20 ) ;
V_1 = V_120 & ( ~ V_121 ) ;
V_19 = V_122 | V_123 ;
F_7 ( V_2 , V_119 -> V_16 , 0 , V_1 ,
~ V_19 & V_124 ) ;
V_1 = F_2 ( V_2 + V_119 -> V_16 -> V_21 [ 0 ] ) ;
V_1 &= ~ V_19 ;
V_1 |= V_120 & V_19 ;
F_3 ( V_1 , V_2 + V_119 -> V_16 -> V_21 [ 0 ] ) ;
F_11 ( 1 ) ;
return;
}
F_3 ( V_120 ,
V_2 + V_119 -> V_16 -> V_21 [ 0 ] ) ;
F_11 ( 1 ) ;
}
static void F_33 ( struct V_34 * V_99 , bool V_125 )
{
T_1 V_1 , V_19 ;
V_1 = V_126 & ( ~ V_127 ) ;
V_19 = V_128 | V_129 ;
if ( ! V_125 )
V_19 |= V_127 ;
F_7 ( V_2 , V_99 -> V_16 , 0 , V_1 ,
~ V_19 & V_130 ) ;
V_1 = V_131 ;
V_19 = V_132 | V_133 ;
F_7 ( V_2 , V_99 -> V_16 , 1 , V_1 ,
~ V_19 & V_134 ) ;
}
static void F_34 ( struct V_34 * V_135 )
{
T_1 V_19 ;
T_1 V_1 = F_2 ( V_2 + V_135 -> V_16 -> V_36 ) ;
V_135 -> V_16 -> V_22 = true ;
if ( V_1 & V_37 ) {
F_8 ( L_21 ) ;
F_33 ( V_135 , true ) ;
V_1 = F_2 ( V_2 + V_135 -> V_16 -> V_21 [ 0 ] ) ;
V_19 = V_128 | V_129 ;
V_1 &= ~ V_19 ;
V_1 |= V_126 & V_19 ;
F_3 ( V_1 , V_2 + V_135 -> V_16 -> V_21 [ 0 ] ) ;
F_11 ( 1 ) ;
return;
}
F_3 ( V_126 ,
V_2 + V_135 -> V_16 -> V_21 [ 0 ] ) ;
V_1 = F_2 ( V_2 + V_135 -> V_16 -> V_21 [ 1 ] ) ;
V_19 = V_132 | V_133 ;
V_1 &= V_19 ;
V_1 |= ~ V_19 & V_131 ;
F_3 ( V_1 , V_2 + V_135 -> V_16 -> V_21 [ 1 ] ) ;
F_11 ( 1 ) ;
}
static void F_35 ( struct V_34 * V_99 , bool V_136 )
{
T_1 V_1 , V_19 ;
V_1 = V_137 & ( ~ V_138 ) ;
V_19 = V_139 | ( V_136 ? V_138 : 0 ) ;
F_7 ( V_2 , V_99 -> V_16 , 0 , V_1 ,
~ V_19 & V_140 ) ;
V_1 = V_141 ;
V_19 = V_142 ;
F_7 ( V_2 , V_99 -> V_16 , 1 , V_1 ,
~ V_19 & V_143 ) ;
}
static void F_36 ( struct V_34 * V_144 )
{
T_1 V_1 = F_2 ( V_2 + V_144 -> V_16 -> V_36 ) ;
V_144 -> V_16 -> V_22 = true ;
if ( V_1 & V_37 ) {
F_8 ( L_22 ) ;
F_35 ( V_144 , false ) ;
V_1 = F_2 ( V_2 + V_144 -> V_16 -> V_21 [ 0 ] ) ;
V_1 &= ~ V_139 ;
V_1 |= V_137 & V_139 ;
F_3 ( V_1 , V_2 + V_144 -> V_16 -> V_21 [ 0 ] ) ;
V_1 = F_2 ( V_2 + V_144 -> V_16 -> V_21 [ 1 ] ) ;
V_1 &= ~ V_142 ;
V_1 |= V_141 & V_142 ;
F_3 ( V_1 , V_2 + V_144 -> V_16 -> V_21 [ 1 ] ) ;
F_11 ( 1 ) ;
return;
}
F_3 ( V_137 ,
V_2 + V_144 -> V_16 -> V_21 [ 0 ] ) ;
F_3 ( V_141 ,
V_2 + V_144 -> V_16 -> V_21 [ 1 ] ) ;
F_11 ( 1 ) ;
}
static int F_37 ( struct V_34 * V_99 ,
T_1 V_145 , T_1 V_19 )
{
int V_146 ;
T_1 V_1 = 0 ;
for ( V_146 = 0 ; V_146 < V_99 -> V_16 -> V_147 / V_148 + 1 ; V_146 ++ ) {
F_11 ( V_148 ) ;
V_1 = F_2 ( V_2 + V_145 ) ;
if ( ( V_1 & V_19 ) == V_19 ) {
F_11 ( V_148 ) ;
return 0 ;
}
}
return - V_149 ;
}
static int F_38 ( struct V_34 * V_114 ,
struct V_150 * V_151 )
{
T_1 V_1 , V_14 , V_152 ;
V_152 = ( F_39 ( V_114 ) >> V_114 -> V_16 -> V_153 -> V_154 )
<< V_155 ;
V_1 = F_2 ( V_2 + V_114 -> V_16 -> V_21 [ 2 ] ) ;
V_1 &= ( ~ V_152 ) ;
V_1 |= V_151 -> V_156 << V_155 ;
F_3 ( V_1 , V_2 + V_114 -> V_16 -> V_21 [ 2 ] ) ;
F_11 ( 1 ) ;
V_1 = F_2 ( V_2 + V_114 -> V_16 -> V_21 [ 2 ] ) ;
V_1 |= V_106 ;
F_3 ( V_1 , V_2 + V_114 -> V_16 -> V_21 [ 2 ] ) ;
F_11 ( 1 ) ;
F_37 ( V_114 , V_114 -> V_16 -> V_21 [ 2 ] ,
V_157 ) ;
V_14 = F_2 ( V_2 + V_114 -> V_16 -> V_36 ) &
( ~ F_40 ( V_114 ) ) ;
V_14 |= V_151 -> V_156 << V_114 -> V_16 -> V_153 -> V_154 ;
F_3 ( V_14 , V_2 + V_114 -> V_16 -> V_36 ) ;
F_11 ( 1 ) ;
V_1 &= ~ V_106 ;
F_3 ( V_1 , V_2 + V_114 -> V_16 -> V_21 [ 2 ] ) ;
F_11 ( 1 ) ;
F_41 ( L_23 ,
F_42 ( V_114 -> V_93 . V_97 ) , V_151 -> V_158 , V_151 -> V_156 , V_151 -> V_159 ,
V_151 -> V_96 / V_151 -> V_158 * V_151 -> V_156 /
V_114 -> V_16 -> V_160 [ V_151 -> V_159 ] . V_161 / 1000 ) ;
return 0 ;
}
static int F_43 ( struct V_92 * V_93 ,
struct V_150 * V_151 ,
unsigned long V_162 , unsigned long V_96 )
{
struct V_34 * V_99 = F_44 ( V_93 ) ;
struct V_15 * V_16 = V_99 -> V_16 ;
int V_159 ;
unsigned long V_163 , V_164 ;
T_1 V_161 ;
if ( ! V_162 )
return - V_165 ;
if ( ! ( V_16 -> V_166 & V_167 ) ) {
V_159 = F_45 ( V_16 -> V_168 , V_162 ) ;
V_159 = V_16 -> V_169 ( V_159 , & V_161 ) ;
} else {
V_159 = V_162 >= V_16 -> V_168 ? 1 : - V_165 ;
}
if ( V_159 < 0 )
return - V_165 ;
V_151 -> V_158 = F_27 ( V_93 , V_96 ) ;
V_151 -> V_159 = V_159 ;
V_151 -> V_159 = F_46 ( V_99 , V_151 -> V_159 ) ;
V_164 = V_162 * V_159 ;
if ( V_164 > V_16 -> V_170 )
V_164 = V_16 -> V_170 ;
V_163 = V_96 / V_151 -> V_158 ;
V_151 -> V_156 = V_164 / V_163 ;
V_151 -> V_171 = 0 ;
if ( V_16 -> V_172 ) {
unsigned long V_173 = V_164 - V_163 * V_151 -> V_156 ;
if ( V_173 || V_16 -> V_174 ) {
T_4 V_175 = V_173 * V_176 ;
F_47 ( V_175 , V_163 ) ;
V_175 -= V_176 / 2 ;
V_151 -> V_171 = F_48 ( V_175 ) ;
}
}
V_151 -> V_96 = V_96 ;
V_151 -> V_177 = V_162 ;
return 0 ;
}
static void F_49 ( struct V_150 * V_151 )
{
V_151 -> V_156 = V_151 -> V_156 * V_176 + V_176 / 2 +
F_50 ( V_151 -> V_171 ) ;
V_151 -> V_158 *= V_176 ;
}
static unsigned long
F_51 ( struct V_15 * V_16 ,
unsigned long V_178 )
{
unsigned long V_168 = V_16 -> V_168 ;
V_16 -> V_168 += F_45 ( V_178 , V_176 ) ;
V_168 = F_52 ( V_168 , V_16 -> V_168 ) ;
return V_168 ;
}
static T_1 F_53 ( T_1 V_159 , T_1 * V_161 )
{
int V_146 ;
if ( V_159 ) {
for ( V_146 = 0 ; V_146 <= V_179 ; V_146 ++ ) {
if ( V_159 <= V_180 [ V_146 ] . V_161 ) {
if ( V_161 )
* V_161 = V_146 ;
return V_180 [ V_146 ] . V_161 ;
}
}
}
return - V_165 ;
}
static T_1 F_54 ( T_1 V_159 , T_1 * V_161 )
{
if ( V_159 ) {
T_1 V_146 = F_55 ( V_159 ) ;
if ( V_146 == F_56 ( V_159 ) )
V_146 -- ;
if ( V_146 <= V_181 ) {
if ( V_161 )
* V_161 = V_146 ;
return 1 << V_146 ;
}
}
return - V_165 ;
}
static void F_57 ( void T_2 * V_2 )
{
T_1 V_145 ;
int V_146 ;
for ( V_146 = 0 ; V_146 < F_58 ( V_182 ) ; V_146 ++ ) {
if ( V_183 == V_182 [ V_146 ] . V_184 )
break;
}
if ( V_146 >= F_58 ( V_182 ) ) {
F_28 ( L_24 , V_98 ,
V_183 ) ;
return;
}
V_145 = F_2 ( V_2 + V_185 ) ;
V_145 |= V_186 |
V_187 |
V_188 ;
V_145 &= ~ ( V_189 |
V_190 ) ;
F_3 ( V_145 , V_2 + V_185 ) ;
V_145 = F_2 ( V_2 + V_185 ) ;
V_145 |= V_191 ;
F_3 ( V_145 , V_2 + V_185 ) ;
F_11 ( 1 ) ;
V_145 = F_2 ( V_2 + V_192 ) ;
V_145 &= ~ V_193 ;
F_3 ( V_145 , V_2 + V_192 ) ;
V_145 = F_2 ( V_2 + V_194 ) ;
V_145 &= ~ V_195 ;
F_3 ( V_145 , V_2 + V_194 ) ;
F_11 ( 10 ) ;
V_145 = F_2 ( V_2 + V_196 ) ;
V_145 &= ~ F_59 ( ~ 0 ) ;
V_145 |= F_59 ( V_182 [ V_146 ] . V_197 ) ;
V_145 &= ~ F_60 ( ~ 0 ) ;
V_145 |= F_60 ( V_182 [ V_146 ] .
V_198 ) ;
F_3 ( V_145 , V_2 + V_196 ) ;
V_145 = F_2 ( V_2 + V_199 ) ;
V_145 &= ~ F_61 ( ~ 0 ) ;
V_145 |= F_61 ( V_182 [ V_146 ] .
V_200 ) ;
V_145 &= ~ F_62 ( ~ 0 ) ;
V_145 |= F_62 ( V_182 [ V_146 ] .
V_201 ) ;
V_145 |= V_202 ;
F_3 ( V_145 , V_2 + V_199 ) ;
V_145 = F_2 ( V_2 + V_199 ) ;
V_145 &= ~ V_203 ;
V_145 |= V_204 ;
F_3 ( V_145 , V_2 + V_199 ) ;
F_11 ( 1 ) ;
V_145 = F_2 ( V_2 + V_196 ) ;
V_145 |= V_205 ;
V_145 |= V_206 ;
V_145 |= V_207 ;
V_145 &= ~ V_208 ;
V_145 &= ~ V_209 ;
V_145 &= ~ V_210 ;
F_3 ( V_145 , V_2 + V_196 ) ;
V_145 = F_2 ( V_2 + V_199 ) ;
V_145 &= ~ V_203 ;
V_145 &= ~ V_204 ;
F_3 ( V_145 , V_2 + V_199 ) ;
V_145 = F_2 ( V_2 + V_194 ) ;
V_145 |= V_211 ;
V_145 &= ~ V_212 ;
F_3 ( V_145 , V_2 + V_194 ) ;
F_11 ( 1 ) ;
V_145 = F_2 ( V_2 + V_213 ) ;
V_145 &= ~ V_214 ;
F_3 ( V_145 , V_2 + V_213 ) ;
F_11 ( 1 ) ;
V_145 = F_2 ( V_2 + V_194 ) ;
V_145 |= V_215 ;
F_3 ( V_145 , V_2 + V_194 ) ;
}
static T_5 void F_63 ( void T_2 * V_2 ,
void T_2 * V_216 )
{
struct V_97 * V_97 ;
V_97 = F_64 ( NULL , L_25 , L_26 , 0 ,
1 , 2 ) ;
V_217 [ V_218 ] = V_97 ;
V_97 = F_65 ( L_27 , L_28 , 0 , V_2 ,
1 , 17 , 181 ) ;
V_217 [ V_219 ] = V_97 ;
V_97 = F_65 ( L_29 , L_28 , 0 , V_2 ,
1 , 17 , 207 ) ;
V_217 [ V_220 ] = V_97 ;
V_97 = F_65 ( L_30 , L_28 , 0 , V_2 ,
1 , 17 , 222 ) ;
V_217 [ V_221 ] = V_97 ;
V_97 = F_66 ( NULL , L_31 , L_32 , 0 ,
V_2 + V_222 , 21 , 0 , & V_223 ) ;
V_217 [ V_224 ] = V_97 ;
V_97 = F_67 ( L_33 , L_31 , 0 ,
V_2 , 0 , 48 ,
V_225 ) ;
V_217 [ V_226 ] = V_97 ;
V_97 = F_67 ( L_34 , L_31 , 0 ,
V_2 , 0 , 82 ,
V_225 ) ;
V_217 [ V_227 ] = V_97 ;
V_97 = F_68 ( NULL , L_35 , V_228 ,
F_58 ( V_228 ) , 0 ,
V_2 + V_229 ,
29 , 3 , 0 , & V_230 ) ;
V_97 = F_69 ( L_36 , L_35 , V_2 + V_229 ,
& V_230 ) ;
V_217 [ V_231 ] = V_97 ;
V_97 = F_66 ( NULL , L_37 , L_38 , 0 , V_2 + V_232 ,
0 , 0 , & V_233 ) ;
F_70 ( V_97 , L_37 , NULL ) ;
V_217 [ V_234 ] = V_97 ;
V_97 = F_66 ( NULL , L_39 , L_38 , 0 , V_2 + V_232 ,
1 , 0 , & V_233 ) ;
F_70 ( V_97 , L_39 , NULL ) ;
V_217 [ V_235 ] = V_97 ;
F_71 ( V_2 , V_216 , V_236 , & V_237 ) ;
}
static void T_5 F_72 ( void T_2 * V_2 ,
void T_2 * V_238 )
{
T_1 V_1 ;
struct V_97 * V_97 ;
V_97 = F_73 ( L_40 , L_41 , V_2 ,
V_238 , 0 , & V_239 , NULL ) ;
if ( ! F_74 ( F_75 ( V_97 ) ) )
F_70 ( V_97 , L_40 , NULL ) ;
V_217 [ V_240 ] = V_97 ;
V_97 = F_76 ( L_42 , L_40 ,
V_2 + V_241 , 0 , V_242 ,
8 , 8 , 1 , NULL ) ;
V_97 = F_77 ( L_43 , L_42 ,
V_2 + V_241 , 1 , 0 ,
V_243 , 0 , NULL ) ;
F_70 ( V_97 , L_43 , NULL ) ;
V_217 [ V_244 ] = V_97 ;
V_97 = F_64 ( NULL , L_44 , L_40 ,
V_243 , 1 , 1 ) ;
F_70 ( V_97 , L_44 , NULL ) ;
V_217 [ V_245 ] = V_97 ;
V_97 = F_78 ( L_45 , L_41 , V_2 ,
V_238 , 0 , & V_246 , NULL ) ;
F_70 ( V_97 , L_45 , NULL ) ;
V_217 [ V_247 ] = V_97 ;
V_97 = F_78 ( L_46 , L_41 , V_2 ,
V_238 , 0 , & V_248 , NULL ) ;
F_70 ( V_97 , L_46 , NULL ) ;
V_217 [ V_249 ] = V_97 ;
V_97 = F_79 ( L_47 , L_48 , V_2 , V_238 ,
V_250 , & V_251 , NULL ) ;
F_70 ( V_97 , L_47 , NULL ) ;
V_217 [ V_252 ] = V_97 ;
V_97 = F_80 ( L_49 , L_48 , V_2 , V_238 ,
V_250 , & V_253 , NULL ) ;
F_70 ( V_97 , L_49 , NULL ) ;
V_217 [ V_254 ] = V_97 ;
F_70 ( V_97 , L_50 , NULL ) ;
V_217 [ V_255 ] = V_97 ;
V_97 = F_64 ( NULL , L_51 , L_47 ,
V_243 , 1 , 1 ) ;
F_70 ( V_97 , L_51 , NULL ) ;
V_217 [ V_256 ] = V_97 ;
V_1 = F_81 ( V_2 + V_257 . V_36 ) ;
V_1 &= ~ V_258 ;
F_82 ( V_1 , V_2 + V_257 . V_36 ) ;
V_97 = F_83 ( L_52 , L_41 , V_2 , V_238 ,
0 , & V_257 , & V_259 , V_260 ) ;
F_70 ( V_97 , L_52 , NULL ) ;
V_217 [ V_261 ] = V_97 ;
V_97 = F_84 ( NULL , L_53 , L_52 , 0 ,
V_2 + V_192 , 16 , 4 , 0 ,
V_262 , NULL ) ;
F_70 ( V_97 , L_53 , NULL ) ;
V_217 [ V_263 ] = V_97 ;
V_97 = F_76 ( L_54 , L_53 ,
V_2 + V_264 , 0 ,
V_242 ,
8 , 8 , 1 , & V_259 ) ;
V_97 = F_77 ( L_55 , L_54 ,
V_2 + V_264 , 1 , 0 ,
V_243 , 0 , & V_259 ) ;
F_70 ( V_97 , L_55 , NULL ) ;
V_217 [ V_265 ] = V_97 ;
V_97 = F_76 ( L_56 , L_53 ,
V_2 + V_264 , 0 ,
V_242 ,
24 , 8 , 1 , & V_259 ) ;
V_97 = F_77 ( L_57 , L_56 ,
V_2 + V_264 , 17 , 16 ,
V_243 , 0 , & V_259 ) ;
F_70 ( V_97 , L_57 , NULL ) ;
V_217 [ V_266 ] = V_97 ;
F_57 ( V_2 ) ;
V_97 = F_66 ( NULL , L_58 , L_52 ,
V_243 , V_2 + V_192 ,
22 , 0 , & V_259 ) ;
F_70 ( V_97 , L_58 , NULL ) ;
V_217 [ V_267 ] = V_97 ;
V_97 = F_66 ( NULL , L_59 , L_57 ,
V_243 , V_2 + V_192 ,
23 , 0 , NULL ) ;
F_70 ( V_97 , L_59 , NULL ) ;
V_217 [ V_268 ] = V_97 ;
V_97 = F_66 ( NULL , L_60 , L_55 ,
V_243 , V_2 + V_192 ,
25 , 0 , NULL ) ;
F_70 ( V_97 , L_60 , NULL ) ;
V_217 [ V_269 ] = V_97 ;
V_97 = F_85 ( L_61 , L_41 , V_2 , V_238 , 0 ,
& V_270 , & V_223 ) ;
F_70 ( V_97 , L_61 , NULL ) ;
V_217 [ V_271 ] = V_97 ;
V_97 = F_64 ( NULL , L_32 , L_61 ,
V_243 , 1 , 2 ) ;
F_70 ( V_97 , L_32 , NULL ) ;
V_217 [ V_272 ] = V_97 ;
V_97 = F_86 ( L_62 , L_41 ,
V_2 , V_238 , 0 ,
& V_273 ,
& V_274 , V_260 ) ;
F_70 ( V_97 , L_62 , NULL ) ;
V_217 [ V_275 ] = V_97 ;
V_97 = F_84 ( NULL , L_63 , L_62 , 0 ,
V_2 + V_276 , 16 , 5 , 0 ,
V_262 , & V_274 ) ;
F_70 ( V_97 , L_63 , NULL ) ;
V_217 [ V_277 ] = V_97 ;
V_97 = F_76 ( L_64 , L_62 ,
V_2 + V_278 , 0 ,
V_242 ,
8 , 8 , 1 , NULL ) ;
V_97 = F_77 ( L_65 , L_64 ,
V_2 + V_278 , 1 , 0 ,
V_243 , 0 , NULL ) ;
V_217 [ V_279 ] = V_97 ;
V_97 = F_87 ( L_38 , L_41 ,
V_2 , 0 , & V_280 , NULL ) ;
F_70 ( V_97 , L_38 , NULL ) ;
V_217 [ V_281 ] = V_97 ;
V_97 = F_83 ( L_66 , L_41 , V_2 , V_238 ,
0 , & V_282 , NULL , V_260 ) ;
F_70 ( V_97 , L_66 , NULL ) ;
V_217 [ V_283 ] = V_97 ;
V_97 = F_84 ( NULL , L_67 , L_66 , 0 ,
V_2 + V_284 , 19 , 4 , 0 ,
V_262 , NULL ) ;
F_70 ( V_97 , L_67 , NULL ) ;
V_217 [ V_285 ] = V_97 ;
V_97 = F_64 ( NULL , L_68 , L_66 ,
V_243 , 1 , 3 ) ;
F_70 ( V_97 , L_68 , NULL ) ;
V_217 [ V_286 ] = V_97 ;
V_97 = F_64 ( NULL , L_69 , L_66 ,
V_243 , 1 , 5 ) ;
F_70 ( V_97 , L_69 , NULL ) ;
V_217 [ V_287 ] = V_97 ;
V_97 = F_76 ( L_70 , L_67 ,
V_2 + V_288 , 0 , V_242 ,
8 , 8 , 1 , NULL ) ;
V_97 = F_77 ( L_71 , L_70 ,
V_2 + V_288 , 1 , 0 ,
V_243 , 0 , NULL ) ;
F_70 ( V_97 , L_71 , NULL ) ;
V_217 [ V_289 ] = V_97 ;
V_97 = F_88 ( L_72 , L_41 , V_2 ,
0 , & V_290 , NULL ) ;
F_70 ( V_97 , L_72 , NULL ) ;
V_217 [ V_291 ] = V_97 ;
V_97 = F_88 ( L_73 , L_41 , V_2 ,
0 , & V_292 , NULL ) ;
F_70 ( V_97 , L_73 , NULL ) ;
V_217 [ V_293 ] = V_97 ;
V_97 = F_64 ( NULL , L_74 , L_73 ,
V_243 , 1 , 1 ) ;
F_70 ( V_97 , L_74 , NULL ) ;
V_217 [ V_294 ] = V_97 ;
V_97 = F_64 ( NULL , L_75 , L_28 ,
V_243 , 1 , 2 ) ;
F_70 ( V_97 , L_75 , NULL ) ;
V_217 [ V_295 ] = V_97 ;
}
static void F_89 ( T_1 V_296 )
{
unsigned int V_145 ;
do {
V_145 = F_81 ( V_2 + V_297 ) ;
F_90 () ;
} while ( ! ( V_145 & ( 1 << V_296 ) ) );
}
static void F_91 ( T_1 V_296 )
{
}
static void F_92 ( void )
{
V_298 . V_299 =
F_81 ( V_2 + V_300 ) ;
F_82 ( 3 << 30 , V_2 + V_300 ) ;
}
static void F_93 ( void )
{
F_82 ( V_298 . V_299 ,
V_2 + V_300 ) ;
}
static void T_5 F_94 ( void )
{
F_95 ( V_301 , V_217 , V_302 ) ;
}
static void T_5 F_96 ( struct V_303 * V_304 )
{
struct V_303 * V_305 ;
T_1 V_306 , V_307 ;
V_2 = F_97 ( V_304 , 0 ) ;
if ( ! V_2 ) {
F_28 ( L_76 ) ;
return;
}
V_305 = F_98 ( NULL , V_308 ) ;
if ( ! V_305 ) {
F_28 ( L_77 ) ;
F_74 ( 1 ) ;
return;
}
V_216 = F_97 ( V_305 , 0 ) ;
if ( ! V_216 ) {
F_28 ( L_78 ) ;
F_74 ( 1 ) ;
return;
}
V_217 = F_99 ( V_2 , V_302 ,
V_309 ) ;
if ( ! V_217 )
return;
V_306 = F_100 ( V_2 + V_310 ) >> V_311 ;
V_307 = ( V_306 & V_312 ) + 1 ;
if ( F_101 ( V_2 , V_236 , V_313 ,
F_58 ( V_313 ) , V_307 ,
& V_183 , & V_260 ) < 0 )
return;
F_102 ( V_236 ) ;
F_72 ( V_2 , V_216 ) ;
F_63 ( V_2 , V_216 ) ;
F_103 ( V_2 , V_216 , V_236 ,
V_314 ,
F_58 ( V_314 ) ) ;
F_104 ( V_216 , V_236 ) ;
V_306 = F_100 ( V_2 + V_315 ) ;
V_306 &= ~ F_105 ( 25 ) ;
F_106 ( V_306 , V_2 + V_315 ) ;
V_316 = F_94 ;
F_107 ( V_2 , V_216 , V_236 ,
& V_317 ) ;
F_108 ( V_304 ) ;
F_109 ( V_318 , F_58 ( V_318 ) ) ;
V_319 = & V_320 ;
}
