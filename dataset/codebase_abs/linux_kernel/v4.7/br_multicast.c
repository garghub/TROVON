static inline int F_1 ( const struct V_1 * V_2 , const struct V_1 * V_3 )
{
if ( V_2 -> V_4 != V_3 -> V_4 )
return 0 ;
if ( V_2 -> V_5 != V_3 -> V_5 )
return 0 ;
switch ( V_2 -> V_4 ) {
case F_2 ( V_6 ) :
return V_2 -> V_7 . V_8 == V_3 -> V_7 . V_8 ;
#if F_3 ( V_9 )
case F_2 ( V_10 ) :
return F_4 ( & V_2 -> V_7 . V_11 , & V_3 -> V_7 . V_11 ) ;
#endif
}
return 0 ;
}
static inline int F_5 ( struct V_12 * V_13 , T_1 V_14 ,
T_2 V_5 )
{
return F_6 ( ( V_15 V_16 ) V_14 , V_5 , V_13 -> V_17 ) & ( V_13 -> V_18 - 1 ) ;
}
static inline int F_7 ( struct V_12 * V_13 ,
const struct V_19 * V_14 ,
T_2 V_5 )
{
return F_6 ( F_8 ( V_14 ) , V_5 ,
V_13 -> V_17 ) & ( V_13 -> V_18 - 1 ) ;
}
static inline int F_9 ( struct V_12 * V_13 ,
struct V_1 * V_14 )
{
switch ( V_14 -> V_4 ) {
case F_2 ( V_6 ) :
return F_5 ( V_13 , V_14 -> V_7 . V_8 , V_14 -> V_5 ) ;
#if F_3 ( V_9 )
case F_2 ( V_10 ) :
return F_7 ( V_13 , & V_14 -> V_7 . V_11 , V_14 -> V_5 ) ;
#endif
}
return 0 ;
}
static struct V_20 * F_10 (
struct V_12 * V_13 , struct V_1 * V_21 , int V_22 )
{
struct V_20 * V_23 ;
F_11 (mp, &mdb->mhash[hash], hlist[mdb->ver]) {
if ( F_1 ( & V_23 -> V_24 , V_21 ) )
return V_23 ;
}
return NULL ;
}
struct V_20 * F_12 ( struct V_12 * V_13 ,
struct V_1 * V_21 )
{
if ( ! V_13 )
return NULL ;
return F_10 ( V_13 , V_21 , F_9 ( V_13 , V_21 ) ) ;
}
static struct V_20 * F_13 (
struct V_12 * V_13 , T_1 V_21 , T_2 V_5 )
{
struct V_1 V_25 ;
V_25 . V_7 . V_8 = V_21 ;
V_25 . V_4 = F_2 ( V_6 ) ;
V_25 . V_5 = V_5 ;
return F_12 ( V_13 , & V_25 ) ;
}
static struct V_20 * F_14 (
struct V_12 * V_13 , const struct V_19 * V_21 ,
T_2 V_5 )
{
struct V_1 V_25 ;
V_25 . V_7 . V_11 = * V_21 ;
V_25 . V_4 = F_2 ( V_10 ) ;
V_25 . V_5 = V_5 ;
return F_12 ( V_13 , & V_25 ) ;
}
struct V_20 * F_15 ( struct V_26 * V_27 ,
struct V_28 * V_29 , T_3 V_5 )
{
struct V_12 * V_13 = F_16 ( V_27 -> V_13 ) ;
struct V_1 V_14 ;
if ( V_27 -> V_30 )
return NULL ;
if ( F_17 ( V_29 ) -> V_31 )
return NULL ;
V_14 . V_4 = V_29 -> V_32 ;
V_14 . V_5 = V_5 ;
switch ( V_29 -> V_32 ) {
case F_2 ( V_6 ) :
V_14 . V_7 . V_8 = F_18 ( V_29 ) -> V_33 ;
break;
#if F_3 ( V_9 )
case F_2 ( V_10 ) :
V_14 . V_7 . V_11 = F_19 ( V_29 ) -> V_33 ;
break;
#endif
default:
return NULL ;
}
return F_12 ( V_13 , & V_14 ) ;
}
static void F_20 ( struct V_34 * V_35 )
{
struct V_12 * V_13 =
F_21 ( V_35 , struct V_12 , V_36 ) ;
struct V_12 * V_37 = V_13 -> V_37 ;
V_13 -> V_37 = NULL ;
F_22 ( V_37 -> V_38 ) ;
F_22 ( V_37 ) ;
}
static int F_23 ( struct V_12 * V_39 ,
struct V_12 * V_37 ,
int V_40 )
{
struct V_20 * V_23 ;
int V_41 ;
int V_42 ;
int V_43 ;
for ( V_43 = 0 ; V_43 < V_37 -> V_18 ; V_43 ++ )
F_24 (mp, &old->mhash[i], hlist[old->ver])
F_25 ( & V_23 -> V_44 [ V_39 -> V_45 ] ,
& V_39 -> V_38 [ F_9 ( V_39 , & V_23 -> V_24 ) ] ) ;
if ( ! V_40 )
return 0 ;
V_41 = 0 ;
for ( V_43 = 0 ; V_43 < V_39 -> V_18 ; V_43 ++ ) {
V_42 = 0 ;
F_24 (mp, &new->mhash[i], hlist[new->ver])
V_42 ++ ;
if ( V_42 > V_41 )
V_41 = V_42 ;
}
return V_41 > V_40 ? - V_46 : 0 ;
}
void F_26 ( struct V_34 * V_35 )
{
struct V_47 * V_48 =
F_21 ( V_35 , struct V_47 , V_36 ) ;
F_22 ( V_48 ) ;
}
static void F_27 ( struct V_34 * V_35 )
{
struct V_20 * V_23 =
F_21 ( V_35 , struct V_20 , V_36 ) ;
F_22 ( V_23 ) ;
}
static void F_28 ( unsigned long V_49 )
{
struct V_20 * V_23 = ( void * ) V_49 ;
struct V_26 * V_27 = V_23 -> V_27 ;
struct V_12 * V_13 ;
F_29 ( & V_27 -> V_50 ) ;
if ( ! F_30 ( V_27 -> V_51 ) || F_31 ( & V_23 -> V_52 ) )
goto V_53;
V_23 -> V_54 = false ;
if ( V_23 -> V_55 )
goto V_53;
V_13 = F_32 ( V_27 -> V_13 , V_27 ) ;
F_33 ( & V_23 -> V_44 [ V_13 -> V_45 ] ) ;
V_13 -> V_56 -- ;
F_34 ( & V_23 -> V_36 , F_27 ) ;
V_53:
F_35 ( & V_27 -> V_50 ) ;
}
static void F_36 ( struct V_26 * V_27 ,
struct V_47 * V_57 )
{
struct V_12 * V_13 ;
struct V_20 * V_23 ;
struct V_47 * V_48 ;
struct V_47 T_4 * * V_58 ;
V_13 = F_32 ( V_27 -> V_13 , V_27 ) ;
V_23 = F_12 ( V_13 , & V_57 -> V_24 ) ;
if ( F_37 ( ! V_23 ) )
return;
for ( V_58 = & V_23 -> V_55 ;
( V_48 = F_32 ( * V_58 , V_27 ) ) != NULL ;
V_58 = & V_48 -> V_59 ) {
if ( V_48 != V_57 )
continue;
F_38 ( * V_58 , V_48 -> V_59 ) ;
F_39 ( & V_48 -> V_54 ) ;
F_40 ( & V_48 -> V_52 ) ;
F_41 ( V_27 -> V_51 , V_48 -> V_60 , & V_57 -> V_24 , V_61 ,
V_48 -> V_62 ) ;
F_34 ( & V_48 -> V_36 , F_26 ) ;
if ( ! V_23 -> V_55 && ! V_23 -> V_54 &&
F_30 ( V_27 -> V_51 ) )
F_42 ( & V_23 -> V_52 , V_63 ) ;
return;
}
F_37 ( 1 ) ;
}
static void F_43 ( unsigned long V_49 )
{
struct V_47 * V_57 = ( void * ) V_49 ;
struct V_26 * V_27 = V_57 -> V_60 -> V_27 ;
F_29 ( & V_27 -> V_50 ) ;
if ( ! F_30 ( V_27 -> V_51 ) || F_31 ( & V_57 -> V_52 ) ||
F_44 ( & V_57 -> V_54 ) || V_57 -> V_62 & V_64 )
goto V_53;
F_36 ( V_27 , V_57 ) ;
V_53:
F_35 ( & V_27 -> V_50 ) ;
}
static int F_45 ( struct V_12 T_4 * * V_65 , int V_18 ,
int V_40 )
{
struct V_12 * V_37 = F_46 ( * V_65 , 1 ) ;
struct V_12 * V_13 ;
int V_66 ;
V_13 = F_47 ( sizeof( * V_13 ) , V_67 ) ;
if ( ! V_13 )
return - V_68 ;
V_13 -> V_18 = V_18 ;
V_13 -> V_37 = V_37 ;
V_13 -> V_38 = F_48 ( V_18 * sizeof( * V_13 -> V_38 ) , V_67 ) ;
if ( ! V_13 -> V_38 ) {
F_22 ( V_13 ) ;
return - V_68 ;
}
V_13 -> V_56 = V_37 ? V_37 -> V_56 : 0 ;
V_13 -> V_45 = V_37 ? V_37 -> V_45 ^ 1 : 0 ;
if ( ! V_37 || V_40 )
F_49 ( & V_13 -> V_17 , sizeof( V_13 -> V_17 ) ) ;
else
V_13 -> V_17 = V_37 -> V_17 ;
if ( ! V_37 )
goto V_53;
V_66 = F_23 ( V_13 , V_37 , V_40 ) ;
if ( V_66 ) {
F_22 ( V_13 -> V_38 ) ;
F_22 ( V_13 ) ;
return V_66 ;
}
V_69 ++ ;
F_34 ( & V_13 -> V_36 , F_20 ) ;
V_53:
F_38 ( * V_65 , V_13 ) ;
return 0 ;
}
static struct V_28 * F_50 ( struct V_26 * V_27 ,
T_1 V_70 )
{
struct V_28 * V_29 ;
struct V_71 * V_72 ;
struct V_73 * V_74 ;
struct V_75 * V_76 ;
V_29 = F_51 ( V_27 -> V_51 , sizeof( * V_74 ) + sizeof( * V_76 ) +
sizeof( * V_72 ) + 4 ) ;
if ( ! V_29 )
goto V_53;
V_29 -> V_32 = F_2 ( V_6 ) ;
F_52 ( V_29 ) ;
V_74 = F_53 ( V_29 ) ;
F_54 ( V_74 -> V_77 , V_27 -> V_51 -> V_78 ) ;
V_74 -> V_79 [ 0 ] = 1 ;
V_74 -> V_79 [ 1 ] = 0 ;
V_74 -> V_79 [ 2 ] = 0x5e ;
V_74 -> V_79 [ 3 ] = 0 ;
V_74 -> V_79 [ 4 ] = 0 ;
V_74 -> V_79 [ 5 ] = 1 ;
V_74 -> V_80 = F_2 ( V_6 ) ;
F_55 ( V_29 , sizeof( * V_74 ) ) ;
F_56 ( V_29 , V_29 -> V_42 ) ;
V_76 = F_18 ( V_29 ) ;
V_76 -> V_81 = 4 ;
V_76 -> V_82 = 6 ;
V_76 -> V_83 = 0xc0 ;
V_76 -> V_84 = F_2 ( sizeof( * V_76 ) + sizeof( * V_72 ) + 4 ) ;
V_76 -> V_85 = 0 ;
V_76 -> V_86 = F_2 ( V_87 ) ;
V_76 -> V_88 = 1 ;
V_76 -> V_32 = V_89 ;
V_76 -> V_90 = V_27 -> V_91 ?
F_57 ( V_27 -> V_51 , 0 , V_92 ) : 0 ;
V_76 -> V_33 = F_58 ( V_93 ) ;
( ( V_94 * ) & V_76 [ 1 ] ) [ 0 ] = V_95 ;
( ( V_94 * ) & V_76 [ 1 ] ) [ 1 ] = 4 ;
( ( V_94 * ) & V_76 [ 1 ] ) [ 2 ] = 0 ;
( ( V_94 * ) & V_76 [ 1 ] ) [ 3 ] = 0 ;
F_59 ( V_76 ) ;
F_55 ( V_29 , 24 ) ;
F_60 ( V_29 , V_29 -> V_42 ) ;
V_72 = F_61 ( V_29 ) ;
V_72 -> type = V_96 ;
V_72 -> V_97 = ( V_70 ? V_27 -> V_98 :
V_27 -> V_99 ) /
( V_100 / V_101 ) ;
V_72 -> V_70 = V_70 ;
V_72 -> V_102 = 0 ;
V_72 -> V_102 = F_62 ( ( void * ) V_72 , sizeof( struct V_71 ) ) ;
F_55 ( V_29 , sizeof( * V_72 ) ) ;
F_63 ( V_29 , sizeof( * V_74 ) ) ;
V_53:
return V_29 ;
}
static struct V_28 * F_64 ( struct V_26 * V_27 ,
const struct V_19 * V_70 )
{
struct V_28 * V_29 ;
struct V_103 * V_104 ;
struct V_105 * V_106 ;
struct V_73 * V_74 ;
V_94 * V_107 ;
unsigned long V_108 ;
V_29 = F_51 ( V_27 -> V_51 , sizeof( * V_74 ) + sizeof( * V_104 ) +
8 + sizeof( * V_106 ) ) ;
if ( ! V_29 )
goto V_53;
V_29 -> V_32 = F_2 ( V_10 ) ;
F_52 ( V_29 ) ;
V_74 = F_53 ( V_29 ) ;
F_54 ( V_74 -> V_77 , V_27 -> V_51 -> V_78 ) ;
V_74 -> V_80 = F_2 ( V_10 ) ;
F_55 ( V_29 , sizeof( * V_74 ) ) ;
F_56 ( V_29 , V_29 -> V_42 ) ;
V_104 = F_19 ( V_29 ) ;
* ( V_15 T_1 * ) V_104 = F_58 ( 0x60000000 ) ;
V_104 -> V_109 = F_2 ( 8 + sizeof( * V_106 ) ) ;
V_104 -> V_110 = V_111 ;
V_104 -> V_112 = 1 ;
F_65 ( & V_104 -> V_33 , F_58 ( 0xff020000 ) , 0 , 0 , F_58 ( 1 ) ) ;
if ( F_66 ( F_67 ( V_27 -> V_51 ) , V_27 -> V_51 , & V_104 -> V_33 , 0 ,
& V_104 -> V_90 ) ) {
F_68 ( V_29 ) ;
V_27 -> V_113 = 0 ;
return NULL ;
}
V_27 -> V_113 = 1 ;
F_69 ( & V_104 -> V_33 , V_74 -> V_79 ) ;
V_107 = ( V_94 * ) ( V_104 + 1 ) ;
V_107 [ 0 ] = V_114 ;
V_107 [ 1 ] = 0 ;
V_107 [ 2 ] = V_115 ;
V_107 [ 3 ] = 2 ;
V_107 [ 4 ] = 0 ;
V_107 [ 5 ] = 0 ;
V_107 [ 6 ] = V_116 ;
V_107 [ 7 ] = V_116 ;
F_55 ( V_29 , sizeof( * V_104 ) + 8 ) ;
F_60 ( V_29 , V_29 -> V_42 ) ;
V_106 = (struct V_105 * ) F_70 ( V_29 ) ;
V_108 = F_71 ( V_70 ) ?
V_27 -> V_99 :
V_27 -> V_98 ;
V_106 -> V_117 = V_118 ;
V_106 -> V_119 = 0 ;
V_106 -> V_120 = 0 ;
V_106 -> V_121 = F_2 ( ( T_3 ) F_72 ( V_108 ) ) ;
V_106 -> V_122 = 0 ;
V_106 -> V_123 = * V_70 ;
V_106 -> V_120 = F_73 ( & V_104 -> V_90 , & V_104 -> V_33 ,
sizeof( * V_106 ) , V_114 ,
F_74 ( V_106 ,
sizeof( * V_106 ) , 0 ) ) ;
F_55 ( V_29 , sizeof( * V_106 ) ) ;
F_63 ( V_29 , sizeof( * V_74 ) ) ;
V_53:
return V_29 ;
}
static struct V_28 * F_75 ( struct V_26 * V_27 ,
struct V_1 * V_24 )
{
switch ( V_24 -> V_4 ) {
case F_2 ( V_6 ) :
return F_50 ( V_27 , V_24 -> V_7 . V_8 ) ;
#if F_3 ( V_9 )
case F_2 ( V_10 ) :
return F_64 ( V_27 , & V_24 -> V_7 . V_11 ) ;
#endif
}
return NULL ;
}
static struct V_20 * F_76 (
struct V_26 * V_27 , struct V_124 * V_60 ,
struct V_1 * V_70 , int V_22 )
{
struct V_12 * V_13 ;
struct V_20 * V_23 ;
unsigned int V_125 = 0 ;
unsigned int V_18 ;
int V_40 ;
int V_66 ;
V_13 = F_46 ( V_27 -> V_13 , 1 ) ;
F_24 (mp, &mdb->mhash[hash], hlist[mdb->ver]) {
V_125 ++ ;
if ( F_77 ( F_1 ( V_70 , & V_23 -> V_24 ) ) )
return V_23 ;
}
V_40 = 0 ;
V_18 = V_13 -> V_18 ;
if ( F_77 ( V_125 > V_27 -> V_126 && V_125 ) ) {
if ( F_78 () )
F_79 ( V_27 , L_1
L_2 ,
V_60 ? V_60 -> V_51 -> V_127 : V_27 -> V_51 -> V_127 ) ;
V_40 = V_27 -> V_126 ;
}
if ( V_13 -> V_56 >= V_18 ) {
V_18 *= 2 ;
if ( F_77 ( V_18 > V_27 -> V_128 ) ) {
F_80 ( V_27 , L_3
L_4 ,
V_27 -> V_128 ,
V_60 ? V_60 -> V_51 -> V_127 : V_27 -> V_51 -> V_127 ) ;
V_66 = - V_129 ;
V_130:
V_27 -> V_30 = 1 ;
goto V_66;
}
}
if ( V_18 > V_13 -> V_18 || V_40 ) {
if ( V_13 -> V_37 ) {
if ( F_78 () )
F_79 ( V_27 , L_1
L_5 ,
V_60 ? V_60 -> V_51 -> V_127 : V_27 -> V_51 -> V_127 ) ;
V_66 = - V_131 ;
goto V_66;
}
V_66 = F_45 ( & V_27 -> V_13 , V_18 , V_40 ) ;
if ( V_66 ) {
F_80 ( V_27 , L_6
L_7 ,
V_60 ? V_60 -> V_51 -> V_127 : V_27 -> V_51 -> V_127 ,
V_13 -> V_56 , V_66 ) ;
goto V_130;
}
V_66 = - V_132 ;
goto V_66;
}
return NULL ;
V_66:
V_23 = F_81 ( V_66 ) ;
return V_23 ;
}
struct V_20 * F_82 ( struct V_26 * V_27 ,
struct V_124 * V_60 , struct V_1 * V_70 )
{
struct V_12 * V_13 ;
struct V_20 * V_23 ;
int V_22 ;
int V_66 ;
V_13 = F_46 ( V_27 -> V_13 , 1 ) ;
if ( ! V_13 ) {
V_66 = F_45 ( & V_27 -> V_13 , V_133 , 0 ) ;
if ( V_66 )
return F_81 ( V_66 ) ;
goto V_134;
}
V_22 = F_9 ( V_13 , V_70 ) ;
V_23 = F_76 ( V_27 , V_60 , V_70 , V_22 ) ;
switch ( F_83 ( V_23 ) ) {
case 0 :
break;
case - V_132 :
V_134:
V_13 = F_46 ( V_27 -> V_13 , 1 ) ;
V_22 = F_9 ( V_13 , V_70 ) ;
break;
default:
goto V_53;
}
V_23 = F_48 ( sizeof( * V_23 ) , V_67 ) ;
if ( F_77 ( ! V_23 ) )
return F_81 ( - V_68 ) ;
V_23 -> V_27 = V_27 ;
V_23 -> V_24 = * V_70 ;
F_84 ( & V_23 -> V_52 , F_28 ,
( unsigned long ) V_23 ) ;
F_85 ( & V_23 -> V_44 [ V_13 -> V_45 ] , & V_13 -> V_38 [ V_22 ] ) ;
V_13 -> V_56 ++ ;
V_53:
return V_23 ;
}
struct V_47 * F_86 (
struct V_124 * V_60 ,
struct V_1 * V_70 ,
struct V_47 T_4 * V_59 ,
unsigned char V_62 )
{
struct V_47 * V_48 ;
V_48 = F_48 ( sizeof( * V_48 ) , V_67 ) ;
if ( F_77 ( ! V_48 ) )
return NULL ;
V_48 -> V_24 = * V_70 ;
V_48 -> V_60 = V_60 ;
V_48 -> V_62 = V_62 ;
F_38 ( V_48 -> V_59 , V_59 ) ;
F_25 ( & V_48 -> V_54 , & V_60 -> V_54 ) ;
F_84 ( & V_48 -> V_52 , F_43 ,
( unsigned long ) V_48 ) ;
return V_48 ;
}
static int F_87 ( struct V_26 * V_27 ,
struct V_124 * V_60 ,
struct V_1 * V_70 )
{
struct V_20 * V_23 ;
struct V_47 * V_48 ;
struct V_47 T_4 * * V_58 ;
unsigned long V_135 = V_63 ;
int V_66 ;
F_29 ( & V_27 -> V_50 ) ;
if ( ! F_30 ( V_27 -> V_51 ) ||
( V_60 && V_60 -> V_136 == V_137 ) )
goto V_53;
V_23 = F_82 ( V_27 , V_60 , V_70 ) ;
V_66 = F_83 ( V_23 ) ;
if ( F_88 ( V_23 ) )
goto V_66;
if ( ! V_60 ) {
V_23 -> V_54 = true ;
F_42 ( & V_23 -> V_52 , V_135 + V_27 -> V_138 ) ;
goto V_53;
}
for ( V_58 = & V_23 -> V_55 ;
( V_48 = F_32 ( * V_58 , V_27 ) ) != NULL ;
V_58 = & V_48 -> V_59 ) {
if ( V_48 -> V_60 == V_60 )
goto V_139;
if ( ( unsigned long ) V_48 -> V_60 < ( unsigned long ) V_60 )
break;
}
V_48 = F_86 ( V_60 , V_70 , * V_58 , 0 ) ;
if ( F_77 ( ! V_48 ) )
goto V_66;
F_38 ( * V_58 , V_48 ) ;
F_41 ( V_27 -> V_51 , V_60 , V_70 , V_140 , 0 ) ;
V_139:
F_42 ( & V_48 -> V_52 , V_135 + V_27 -> V_138 ) ;
V_53:
V_66 = 0 ;
V_66:
F_35 ( & V_27 -> V_50 ) ;
return V_66 ;
}
static int F_89 ( struct V_26 * V_27 ,
struct V_124 * V_60 ,
T_1 V_70 ,
T_2 V_5 )
{
struct V_1 V_141 ;
if ( F_90 ( V_70 ) )
return 0 ;
V_141 . V_7 . V_8 = V_70 ;
V_141 . V_4 = F_2 ( V_6 ) ;
V_141 . V_5 = V_5 ;
return F_87 ( V_27 , V_60 , & V_141 ) ;
}
static int F_91 ( struct V_26 * V_27 ,
struct V_124 * V_60 ,
const struct V_19 * V_70 ,
T_2 V_5 )
{
struct V_1 V_141 ;
if ( F_92 ( V_70 ) )
return 0 ;
V_141 . V_7 . V_11 = * V_70 ;
V_141 . V_4 = F_2 ( V_10 ) ;
V_141 . V_5 = V_5 ;
return F_87 ( V_27 , V_60 , & V_141 ) ;
}
static void F_93 ( unsigned long V_49 )
{
struct V_124 * V_60 = ( void * ) V_49 ;
struct V_26 * V_27 = V_60 -> V_27 ;
F_29 ( & V_27 -> V_50 ) ;
if ( V_60 -> V_142 == V_143 ||
V_60 -> V_142 == V_144 ||
F_31 ( & V_60 -> V_145 ) ||
F_44 ( & V_60 -> V_146 ) )
goto V_53;
F_94 ( & V_60 -> V_146 ) ;
F_95 ( V_27 -> V_51 , V_60 , V_61 ) ;
if ( V_60 -> V_142 == V_147 )
V_60 -> V_142 = V_148 ;
V_53:
F_35 ( & V_27 -> V_50 ) ;
}
static void F_96 ( unsigned long V_49 )
{
}
static void F_97 ( struct V_26 * V_27 ,
struct V_149 * V_150 )
{
F_29 ( & V_27 -> V_50 ) ;
if ( ! F_30 ( V_27 -> V_51 ) || V_27 -> V_30 )
goto V_53;
F_98 ( V_27 , V_150 ) ;
V_53:
F_35 ( & V_27 -> V_50 ) ;
}
static void F_99 ( unsigned long V_49 )
{
struct V_26 * V_27 = ( void * ) V_49 ;
F_97 ( V_27 , & V_27 -> V_151 ) ;
}
static void F_100 ( unsigned long V_49 )
{
struct V_26 * V_27 = ( void * ) V_49 ;
F_97 ( V_27 , & V_27 -> V_152 ) ;
}
static void F_101 ( struct V_26 * V_27 ,
struct V_1 * V_14 ,
struct V_28 * V_29 )
{
if ( V_14 -> V_4 == F_2 ( V_6 ) )
V_27 -> V_153 . V_24 . V_7 . V_8 = F_18 ( V_29 ) -> V_90 ;
#if F_3 ( V_9 )
else
V_27 -> V_154 . V_24 . V_7 . V_11 = F_19 ( V_29 ) -> V_90 ;
#endif
}
static void F_102 ( struct V_26 * V_27 ,
struct V_124 * V_60 ,
struct V_1 * V_14 )
{
struct V_28 * V_29 ;
V_29 = F_75 ( V_27 , V_14 ) ;
if ( ! V_29 )
return;
if ( V_60 ) {
V_29 -> V_51 = V_60 -> V_51 ;
F_103 ( V_155 , V_156 ,
F_67 ( V_60 -> V_51 ) , NULL , V_29 , NULL , V_29 -> V_51 ,
V_157 ) ;
} else {
F_101 ( V_27 , V_14 , V_29 ) ;
F_104 ( V_29 ) ;
}
}
static void F_105 ( struct V_26 * V_27 ,
struct V_124 * V_60 ,
struct V_149 * V_158 )
{
unsigned long time ;
struct V_1 V_141 ;
struct V_159 * V_160 = NULL ;
if ( ! F_30 ( V_27 -> V_51 ) || V_27 -> V_30 ||
! V_27 -> V_161 )
return;
memset ( & V_141 . V_7 , 0 , sizeof( V_141 . V_7 ) ) ;
if ( V_60 ? ( V_158 == & V_60 -> V_151 ) :
( V_158 == & V_27 -> V_151 ) ) {
V_160 = & V_27 -> V_162 ;
V_141 . V_4 = F_2 ( V_6 ) ;
#if F_3 ( V_9 )
} else {
V_160 = & V_27 -> V_163 ;
V_141 . V_4 = F_2 ( V_10 ) ;
#endif
}
if ( ! V_160 || F_31 ( & V_160 -> V_52 ) )
return;
F_102 ( V_27 , V_60 , & V_141 ) ;
time = V_63 ;
time += V_158 -> V_164 < V_27 -> V_165 ?
V_27 -> V_166 :
V_27 -> V_167 ;
F_42 ( & V_158 -> V_52 , time ) ;
}
static void
F_106 ( struct V_124 * V_60 ,
struct V_149 * V_150 )
{
struct V_26 * V_27 = V_60 -> V_27 ;
F_29 ( & V_27 -> V_50 ) ;
if ( V_60 -> V_136 == V_137 ||
V_60 -> V_136 == V_168 )
goto V_53;
if ( V_150 -> V_164 < V_27 -> V_165 )
V_150 -> V_164 ++ ;
F_105 ( V_60 -> V_27 , V_60 , V_150 ) ;
V_53:
F_35 ( & V_27 -> V_50 ) ;
}
static void F_107 ( unsigned long V_49 )
{
struct V_124 * V_60 = ( void * ) V_49 ;
F_106 ( V_60 , & V_60 -> V_151 ) ;
}
static void F_108 ( unsigned long V_49 )
{
struct V_124 * V_60 = ( void * ) V_49 ;
F_106 ( V_60 , & V_60 -> V_152 ) ;
}
void F_109 ( struct V_124 * V_60 )
{
V_60 -> V_142 = V_148 ;
F_84 ( & V_60 -> V_145 , F_93 ,
( unsigned long ) V_60 ) ;
F_84 ( & V_60 -> V_151 . V_52 ,
F_107 , ( unsigned long ) V_60 ) ;
#if F_3 ( V_9 )
F_84 ( & V_60 -> V_152 . V_52 ,
F_108 , ( unsigned long ) V_60 ) ;
#endif
}
void F_110 ( struct V_124 * V_60 )
{
struct V_26 * V_27 = V_60 -> V_27 ;
struct V_47 * V_57 ;
struct V_169 * V_170 ;
F_111 ( & V_27 -> V_50 ) ;
F_112 (pg, n, &port->mglist, mglist)
F_36 ( V_27 , V_57 ) ;
F_113 ( & V_27 -> V_50 ) ;
F_114 ( & V_60 -> V_145 ) ;
}
static void F_115 ( struct V_149 * V_150 )
{
V_150 -> V_164 = 0 ;
if ( F_116 ( & V_150 -> V_52 ) >= 0 ||
F_40 ( & V_150 -> V_52 ) )
F_42 ( & V_150 -> V_52 , V_63 ) ;
}
void F_117 ( struct V_124 * V_60 )
{
struct V_26 * V_27 = V_60 -> V_27 ;
F_29 ( & V_27 -> V_50 ) ;
if ( V_27 -> V_30 || ! F_30 ( V_27 -> V_51 ) )
goto V_53;
F_115 ( & V_60 -> V_151 ) ;
#if F_3 ( V_9 )
F_115 ( & V_60 -> V_152 ) ;
#endif
if ( V_60 -> V_142 == V_144 &&
F_44 ( & V_60 -> V_146 ) )
F_118 ( V_27 , V_60 ) ;
V_53:
F_35 ( & V_27 -> V_50 ) ;
}
void F_119 ( struct V_124 * V_60 )
{
struct V_26 * V_27 = V_60 -> V_27 ;
struct V_47 * V_57 ;
struct V_169 * V_170 ;
F_29 ( & V_27 -> V_50 ) ;
F_112 (pg, n, &port->mglist, mglist)
if ( ! ( V_57 -> V_62 & V_64 ) )
F_36 ( V_27 , V_57 ) ;
if ( ! F_44 ( & V_60 -> V_146 ) ) {
F_94 ( & V_60 -> V_146 ) ;
F_95 ( V_27 -> V_51 , V_60 , V_61 ) ;
if ( V_60 -> V_142 == V_147 )
V_60 -> V_142 = V_148 ;
}
F_40 ( & V_60 -> V_145 ) ;
F_40 ( & V_60 -> V_151 . V_52 ) ;
#if F_3 ( V_9 )
F_40 ( & V_60 -> V_152 . V_52 ) ;
#endif
F_35 ( & V_27 -> V_50 ) ;
}
static int F_120 ( struct V_26 * V_27 ,
struct V_124 * V_60 ,
struct V_28 * V_29 ,
T_3 V_5 )
{
struct V_171 * V_72 ;
struct V_172 * V_173 ;
int V_43 ;
int V_42 ;
int V_174 ;
int type ;
int V_66 = 0 ;
T_1 V_70 ;
V_72 = F_121 ( V_29 ) ;
V_174 = F_122 ( V_72 -> V_175 ) ;
V_42 = F_123 ( V_29 ) + sizeof( * V_72 ) ;
for ( V_43 = 0 ; V_43 < V_174 ; V_43 ++ ) {
V_42 += sizeof( * V_173 ) ;
if ( ! F_124 ( V_29 , V_42 ) )
return - V_46 ;
V_173 = ( void * ) ( V_29 -> V_49 + V_42 - sizeof( * V_173 ) ) ;
V_70 = V_173 -> V_176 ;
type = V_173 -> V_177 ;
V_42 += F_122 ( V_173 -> V_178 ) * 4 ;
if ( ! F_124 ( V_29 , V_42 ) )
return - V_46 ;
switch ( type ) {
case V_179 :
case V_180 :
case V_181 :
case V_182 :
case V_183 :
case V_184 :
break;
default:
continue;
}
if ( ( type == V_181 ||
type == V_179 ) &&
F_122 ( V_173 -> V_178 ) == 0 ) {
F_125 ( V_27 , V_60 , V_70 , V_5 ) ;
} else {
V_66 = F_89 ( V_27 , V_60 , V_70 , V_5 ) ;
if ( V_66 )
break;
}
}
return V_66 ;
}
static int F_126 ( struct V_26 * V_27 ,
struct V_124 * V_60 ,
struct V_28 * V_29 ,
T_3 V_5 )
{
struct V_185 * V_186 ;
struct V_187 * V_173 ;
int V_43 ;
int V_42 ;
int V_174 ;
int V_66 = 0 ;
if ( ! F_124 ( V_29 , sizeof( * V_186 ) ) )
return - V_46 ;
V_186 = F_70 ( V_29 ) ;
V_174 = F_122 ( V_186 -> V_188 . V_189 [ 1 ] ) ;
V_42 = F_123 ( V_29 ) + sizeof( * V_186 ) ;
for ( V_43 = 0 ; V_43 < V_174 ; V_43 ++ ) {
T_5 * V_190 , V_191 ;
V_190 = F_127 ( V_29 ,
V_42 + F_128 ( struct V_187 ,
V_178 ) ,
sizeof( V_191 ) , & V_191 ) ;
if ( ! V_190 )
return - V_46 ;
if ( ! F_124 ( V_29 ,
V_42 + sizeof( * V_173 ) +
sizeof( struct V_19 ) * F_122 ( * V_190 ) ) )
return - V_46 ;
V_173 = (struct V_187 * ) ( V_29 -> V_49 + V_42 ) ;
V_42 += sizeof( * V_173 ) +
sizeof( struct V_19 ) * F_122 ( * V_190 ) ;
switch ( V_173 -> V_177 ) {
case V_192 :
case V_193 :
case V_194 :
case V_195 :
case V_196 :
case V_197 :
break;
default:
continue;
}
if ( ( V_173 -> V_177 == V_194 ||
V_173 -> V_177 == V_192 ) &&
F_122 ( * V_190 ) == 0 ) {
F_129 ( V_27 , V_60 , & V_173 -> V_176 ,
V_5 ) ;
} else {
V_66 = F_91 ( V_27 , V_60 ,
& V_173 -> V_176 , V_5 ) ;
if ( ! V_66 )
break;
}
}
return V_66 ;
}
static bool F_130 ( struct V_26 * V_27 ,
struct V_124 * V_60 ,
T_1 V_90 )
{
if ( ! F_31 ( & V_27 -> V_151 . V_52 ) &&
! F_31 ( & V_27 -> V_162 . V_52 ) )
goto V_198;
if ( ! V_27 -> V_153 . V_24 . V_7 . V_8 )
goto V_198;
if ( F_131 ( V_90 ) <= F_131 ( V_27 -> V_153 . V_24 . V_7 . V_8 ) )
goto V_198;
return false ;
V_198:
V_27 -> V_153 . V_24 . V_7 . V_8 = V_90 ;
F_38 ( V_27 -> V_153 . V_60 , V_60 ) ;
return true ;
}
static bool F_132 ( struct V_26 * V_27 ,
struct V_124 * V_60 ,
struct V_19 * V_90 )
{
if ( ! F_31 ( & V_27 -> V_152 . V_52 ) &&
! F_31 ( & V_27 -> V_163 . V_52 ) )
goto V_198;
if ( F_133 ( V_90 , & V_27 -> V_154 . V_24 . V_7 . V_11 ) <= 0 )
goto V_198;
return false ;
V_198:
V_27 -> V_154 . V_24 . V_7 . V_11 = * V_90 ;
F_38 ( V_27 -> V_154 . V_60 , V_60 ) ;
return true ;
}
static bool F_134 ( struct V_26 * V_27 ,
struct V_124 * V_60 ,
struct V_1 * V_90 )
{
switch ( V_90 -> V_4 ) {
case F_2 ( V_6 ) :
return F_130 ( V_27 , V_60 , V_90 -> V_7 . V_8 ) ;
#if F_3 ( V_9 )
case F_2 ( V_10 ) :
return F_132 ( V_27 , V_60 , & V_90 -> V_7 . V_11 ) ;
#endif
}
return false ;
}
static void
F_135 ( struct V_26 * V_27 ,
struct V_159 * V_150 ,
unsigned long V_199 )
{
if ( ! F_31 ( & V_150 -> V_52 ) )
V_150 -> V_200 = V_63 + V_199 ;
F_42 ( & V_150 -> V_52 , V_63 + V_27 -> V_201 ) ;
}
static void F_118 ( struct V_26 * V_27 ,
struct V_124 * V_60 )
{
struct V_124 * V_48 ;
struct V_169 * V_202 = NULL ;
if ( ! F_44 ( & V_60 -> V_146 ) )
return;
F_24 (p, &br->router_list, rlist) {
if ( ( unsigned long ) V_60 >= ( unsigned long ) V_48 )
break;
V_202 = & V_48 -> V_146 ;
}
if ( V_202 )
F_136 ( & V_60 -> V_146 , V_202 ) ;
else
F_85 ( & V_60 -> V_146 , & V_27 -> V_203 ) ;
F_95 ( V_27 -> V_51 , V_60 , V_140 ) ;
}
static void F_137 ( struct V_26 * V_27 ,
struct V_124 * V_60 )
{
unsigned long V_135 = V_63 ;
if ( ! V_60 ) {
if ( V_27 -> V_142 == V_148 )
F_42 ( & V_27 -> V_145 ,
V_135 + V_27 -> V_201 ) ;
return;
}
if ( V_60 -> V_142 == V_143 ||
V_60 -> V_142 == V_144 )
return;
F_118 ( V_27 , V_60 ) ;
F_42 ( & V_60 -> V_145 ,
V_135 + V_27 -> V_201 ) ;
}
static void F_138 ( struct V_26 * V_27 ,
struct V_124 * V_60 ,
struct V_159 * V_150 ,
struct V_1 * V_90 ,
unsigned long V_199 )
{
if ( ! F_134 ( V_27 , V_60 , V_90 ) )
return;
F_135 ( V_27 , V_150 , V_199 ) ;
F_137 ( V_27 , V_60 ) ;
}
static int F_139 ( struct V_26 * V_27 ,
struct V_124 * V_60 ,
struct V_28 * V_29 ,
T_3 V_5 )
{
const struct V_75 * V_76 = F_18 ( V_29 ) ;
struct V_71 * V_72 = F_61 ( V_29 ) ;
struct V_20 * V_23 ;
struct V_204 * V_205 ;
struct V_47 * V_48 ;
struct V_47 T_4 * * V_58 ;
struct V_1 V_90 ;
unsigned long V_199 ;
unsigned long V_135 = V_63 ;
unsigned int V_206 = F_123 ( V_29 ) ;
T_1 V_70 ;
int V_66 = 0 ;
F_29 ( & V_27 -> V_50 ) ;
if ( ! F_30 ( V_27 -> V_51 ) ||
( V_60 && V_60 -> V_136 == V_137 ) )
goto V_53;
V_70 = V_72 -> V_70 ;
if ( V_29 -> V_42 == V_206 + sizeof( * V_72 ) ) {
V_199 = V_72 -> V_97 * ( V_100 / V_101 ) ;
if ( ! V_199 ) {
V_199 = 10 * V_100 ;
V_70 = 0 ;
}
} else if ( V_29 -> V_42 >= V_206 + sizeof( * V_205 ) ) {
V_205 = F_140 ( V_29 ) ;
if ( V_205 -> V_190 )
goto V_53;
V_199 = V_205 -> V_97 ?
F_141 ( V_205 -> V_97 ) * ( V_100 / V_101 ) : 1 ;
} else {
goto V_53;
}
if ( ! V_70 ) {
V_90 . V_4 = F_2 ( V_6 ) ;
V_90 . V_7 . V_8 = V_76 -> V_90 ;
F_138 ( V_27 , V_60 , & V_27 -> V_162 ,
& V_90 , V_199 ) ;
goto V_53;
}
V_23 = F_13 ( F_32 ( V_27 -> V_13 , V_27 ) , V_70 , V_5 ) ;
if ( ! V_23 )
goto V_53;
V_199 *= V_27 -> V_207 ;
if ( V_23 -> V_54 &&
( F_31 ( & V_23 -> V_52 ) ?
F_142 ( V_23 -> V_52 . V_208 , V_135 + V_199 ) :
F_116 ( & V_23 -> V_52 ) >= 0 ) )
F_42 ( & V_23 -> V_52 , V_135 + V_199 ) ;
for ( V_58 = & V_23 -> V_55 ;
( V_48 = F_32 ( * V_58 , V_27 ) ) != NULL ;
V_58 = & V_48 -> V_59 ) {
if ( F_31 ( & V_48 -> V_52 ) ?
F_142 ( V_48 -> V_52 . V_208 , V_135 + V_199 ) :
F_116 ( & V_48 -> V_52 ) >= 0 )
F_42 ( & V_48 -> V_52 , V_135 + V_199 ) ;
}
V_53:
F_35 ( & V_27 -> V_50 ) ;
return V_66 ;
}
static int F_143 ( struct V_26 * V_27 ,
struct V_124 * V_60 ,
struct V_28 * V_29 ,
T_3 V_5 )
{
const struct V_103 * V_104 = F_19 ( V_29 ) ;
struct V_105 * V_209 ;
struct V_20 * V_23 ;
struct V_210 * V_211 ;
struct V_47 * V_48 ;
struct V_47 T_4 * * V_58 ;
struct V_1 V_90 ;
unsigned long V_199 ;
unsigned long V_135 = V_63 ;
unsigned int V_206 = F_123 ( V_29 ) ;
const struct V_19 * V_70 = NULL ;
bool V_212 ;
int V_66 = 0 ;
F_29 ( & V_27 -> V_50 ) ;
if ( ! F_30 ( V_27 -> V_51 ) ||
( V_60 && V_60 -> V_136 == V_137 ) )
goto V_53;
if ( V_29 -> V_42 == V_206 + sizeof( * V_209 ) ) {
if ( ! F_124 ( V_29 , V_206 + sizeof( * V_209 ) ) ) {
V_66 = - V_46 ;
goto V_53;
}
V_209 = (struct V_105 * ) F_70 ( V_29 ) ;
V_199 = F_144 ( F_122 ( V_209 -> V_121 ) ) ;
if ( V_199 )
V_70 = & V_209 -> V_123 ;
} else {
if ( ! F_124 ( V_29 , V_206 + sizeof( * V_211 ) ) ) {
V_66 = - V_46 ;
goto V_53;
}
V_211 = (struct V_210 * ) F_70 ( V_29 ) ;
if ( ! V_211 -> V_213 )
V_70 = & V_211 -> V_214 ;
V_199 = V_18 ( F_144 ( F_145 ( V_211 ) ) , 1UL ) ;
}
V_212 = V_70 && F_71 ( V_70 ) ;
if ( V_212 ) {
V_90 . V_4 = F_2 ( V_10 ) ;
V_90 . V_7 . V_11 = V_104 -> V_90 ;
F_138 ( V_27 , V_60 , & V_27 -> V_163 ,
& V_90 , V_199 ) ;
goto V_53;
} else if ( ! V_70 ) {
goto V_53;
}
V_23 = F_14 ( F_32 ( V_27 -> V_13 , V_27 ) , V_70 , V_5 ) ;
if ( ! V_23 )
goto V_53;
V_199 *= V_27 -> V_207 ;
if ( V_23 -> V_54 &&
( F_31 ( & V_23 -> V_52 ) ?
F_142 ( V_23 -> V_52 . V_208 , V_135 + V_199 ) :
F_116 ( & V_23 -> V_52 ) >= 0 ) )
F_42 ( & V_23 -> V_52 , V_135 + V_199 ) ;
for ( V_58 = & V_23 -> V_55 ;
( V_48 = F_32 ( * V_58 , V_27 ) ) != NULL ;
V_58 = & V_48 -> V_59 ) {
if ( F_31 ( & V_48 -> V_52 ) ?
F_142 ( V_48 -> V_52 . V_208 , V_135 + V_199 ) :
F_116 ( & V_48 -> V_52 ) >= 0 )
F_42 ( & V_48 -> V_52 , V_135 + V_199 ) ;
}
V_53:
F_35 ( & V_27 -> V_50 ) ;
return V_66 ;
}
static void
F_146 ( struct V_26 * V_27 ,
struct V_124 * V_60 ,
struct V_1 * V_70 ,
struct V_159 * V_160 ,
struct V_149 * V_158 )
{
struct V_12 * V_13 ;
struct V_20 * V_23 ;
struct V_47 * V_48 ;
unsigned long V_135 ;
unsigned long time ;
F_29 ( & V_27 -> V_50 ) ;
if ( ! F_30 ( V_27 -> V_51 ) ||
( V_60 && V_60 -> V_136 == V_137 ) )
goto V_53;
V_13 = F_32 ( V_27 -> V_13 , V_27 ) ;
V_23 = F_12 ( V_13 , V_70 ) ;
if ( ! V_23 )
goto V_53;
if ( V_60 && ( V_60 -> V_62 & V_215 ) ) {
struct V_47 T_4 * * V_58 ;
for ( V_58 = & V_23 -> V_55 ;
( V_48 = F_32 ( * V_58 , V_27 ) ) != NULL ;
V_58 = & V_48 -> V_59 ) {
if ( V_48 -> V_60 != V_60 )
continue;
F_38 ( * V_58 , V_48 -> V_59 ) ;
F_39 ( & V_48 -> V_54 ) ;
F_40 ( & V_48 -> V_52 ) ;
F_34 ( & V_48 -> V_36 , F_26 ) ;
F_41 ( V_27 -> V_51 , V_60 , V_70 , V_61 ,
V_48 -> V_62 ) ;
if ( ! V_23 -> V_55 && ! V_23 -> V_54 &&
F_30 ( V_27 -> V_51 ) )
F_42 ( & V_23 -> V_52 , V_63 ) ;
}
goto V_53;
}
if ( F_31 ( & V_160 -> V_52 ) )
goto V_53;
if ( V_27 -> V_161 ) {
F_102 ( V_27 , V_60 , & V_23 -> V_24 ) ;
time = V_63 + V_27 -> V_207 *
V_27 -> V_98 ;
F_42 ( & V_158 -> V_52 , time ) ;
for ( V_48 = F_32 ( V_23 -> V_55 , V_27 ) ;
V_48 != NULL ;
V_48 = F_32 ( V_48 -> V_59 , V_27 ) ) {
if ( V_48 -> V_60 != V_60 )
continue;
if ( ! F_44 ( & V_48 -> V_54 ) &&
( F_31 ( & V_48 -> V_52 ) ?
F_142 ( V_48 -> V_52 . V_208 , time ) :
F_116 ( & V_48 -> V_52 ) >= 0 ) ) {
F_42 ( & V_48 -> V_52 , time ) ;
}
break;
}
}
V_135 = V_63 ;
time = V_135 + V_27 -> V_207 *
V_27 -> V_98 ;
if ( ! V_60 ) {
if ( V_23 -> V_54 &&
( F_31 ( & V_23 -> V_52 ) ?
F_142 ( V_23 -> V_52 . V_208 , time ) :
F_116 ( & V_23 -> V_52 ) >= 0 ) ) {
F_42 ( & V_23 -> V_52 , time ) ;
}
goto V_53;
}
for ( V_48 = F_32 ( V_23 -> V_55 , V_27 ) ;
V_48 != NULL ;
V_48 = F_32 ( V_48 -> V_59 , V_27 ) ) {
if ( V_48 -> V_60 != V_60 )
continue;
if ( ! F_44 ( & V_48 -> V_54 ) &&
( F_31 ( & V_48 -> V_52 ) ?
F_142 ( V_48 -> V_52 . V_208 , time ) :
F_116 ( & V_48 -> V_52 ) >= 0 ) ) {
F_42 ( & V_48 -> V_52 , time ) ;
}
break;
}
V_53:
F_35 ( & V_27 -> V_50 ) ;
}
static void F_125 ( struct V_26 * V_27 ,
struct V_124 * V_60 ,
T_1 V_70 ,
T_2 V_5 )
{
struct V_1 V_141 ;
struct V_149 * V_158 ;
if ( F_90 ( V_70 ) )
return;
V_158 = V_60 ? & V_60 -> V_151 : & V_27 -> V_151 ;
V_141 . V_7 . V_8 = V_70 ;
V_141 . V_4 = F_2 ( V_6 ) ;
V_141 . V_5 = V_5 ;
F_146 ( V_27 , V_60 , & V_141 , & V_27 -> V_162 ,
V_158 ) ;
}
static void F_129 ( struct V_26 * V_27 ,
struct V_124 * V_60 ,
const struct V_19 * V_70 ,
T_2 V_5 )
{
struct V_1 V_141 ;
struct V_149 * V_158 ;
if ( F_92 ( V_70 ) )
return;
V_158 = V_60 ? & V_60 -> V_152 : & V_27 -> V_152 ;
V_141 . V_7 . V_11 = * V_70 ;
V_141 . V_4 = F_2 ( V_10 ) ;
V_141 . V_5 = V_5 ;
F_146 ( V_27 , V_60 , & V_141 , & V_27 -> V_163 ,
V_158 ) ;
}
static int F_147 ( struct V_26 * V_27 ,
struct V_124 * V_60 ,
struct V_28 * V_29 ,
T_3 V_5 )
{
struct V_28 * V_216 = NULL ;
struct V_71 * V_72 ;
int V_66 ;
V_66 = F_148 ( V_29 , & V_216 ) ;
if ( V_66 == - V_217 ) {
if ( ! F_90 ( F_18 ( V_29 ) -> V_33 ) )
F_17 ( V_29 ) -> V_218 = 1 ;
return 0 ;
} else if ( V_66 < 0 ) {
return V_66 ;
}
F_17 ( V_29 ) -> V_31 = 1 ;
V_72 = F_61 ( V_29 ) ;
switch ( V_72 -> type ) {
case V_219 :
case V_220 :
F_17 ( V_29 ) -> V_218 = 1 ;
V_66 = F_89 ( V_27 , V_60 , V_72 -> V_70 , V_5 ) ;
break;
case V_221 :
V_66 = F_120 ( V_27 , V_60 , V_216 , V_5 ) ;
break;
case V_96 :
V_66 = F_139 ( V_27 , V_60 , V_216 , V_5 ) ;
break;
case V_222 :
F_125 ( V_27 , V_60 , V_72 -> V_70 , V_5 ) ;
break;
}
if ( V_216 && V_216 != V_29 )
F_68 ( V_216 ) ;
return V_66 ;
}
static int F_149 ( struct V_26 * V_27 ,
struct V_124 * V_60 ,
struct V_28 * V_29 ,
T_3 V_5 )
{
struct V_28 * V_216 = NULL ;
struct V_105 * V_209 ;
int V_66 ;
V_66 = F_150 ( V_29 , & V_216 ) ;
if ( V_66 == - V_217 ) {
if ( ! F_92 ( & F_19 ( V_29 ) -> V_33 ) )
F_17 ( V_29 ) -> V_218 = 1 ;
return 0 ;
} else if ( V_66 < 0 ) {
return V_66 ;
}
F_17 ( V_29 ) -> V_31 = 1 ;
V_209 = (struct V_105 * ) F_151 ( V_29 ) ;
switch ( V_209 -> V_117 ) {
case V_223 :
F_17 ( V_29 ) -> V_218 = 1 ;
V_66 = F_91 ( V_27 , V_60 , & V_209 -> V_123 , V_5 ) ;
break;
case V_224 :
V_66 = F_126 ( V_27 , V_60 , V_216 , V_5 ) ;
break;
case V_118 :
V_66 = F_143 ( V_27 , V_60 , V_216 , V_5 ) ;
break;
case V_225 :
F_129 ( V_27 , V_60 , & V_209 -> V_123 , V_5 ) ;
break;
}
if ( V_216 && V_216 != V_29 )
F_68 ( V_216 ) ;
return V_66 ;
}
int F_152 ( struct V_26 * V_27 , struct V_124 * V_60 ,
struct V_28 * V_29 , T_3 V_5 )
{
F_17 ( V_29 ) -> V_31 = 0 ;
F_17 ( V_29 ) -> V_218 = 0 ;
if ( V_27 -> V_30 )
return 0 ;
switch ( V_29 -> V_32 ) {
case F_2 ( V_6 ) :
return F_147 ( V_27 , V_60 , V_29 , V_5 ) ;
#if F_3 ( V_9 )
case F_2 ( V_10 ) :
return F_149 ( V_27 , V_60 , V_29 , V_5 ) ;
#endif
}
return 0 ;
}
static void F_153 ( struct V_26 * V_27 ,
struct V_149 * V_150 ,
struct V_226 * V_227 )
{
F_29 ( & V_27 -> V_50 ) ;
if ( V_150 -> V_164 < V_27 -> V_165 )
V_150 -> V_164 ++ ;
F_154 ( V_227 -> V_60 , NULL ) ;
F_105 ( V_27 , NULL , V_150 ) ;
F_35 ( & V_27 -> V_50 ) ;
}
static void F_155 ( unsigned long V_49 )
{
struct V_26 * V_27 = ( void * ) V_49 ;
F_153 ( V_27 , & V_27 -> V_151 , & V_27 -> V_153 ) ;
}
static void F_156 ( unsigned long V_49 )
{
struct V_26 * V_27 = ( void * ) V_49 ;
F_153 ( V_27 , & V_27 -> V_152 , & V_27 -> V_154 ) ;
}
void F_157 ( struct V_26 * V_27 )
{
V_27 -> V_126 = 4 ;
V_27 -> V_128 = 512 ;
V_27 -> V_142 = V_148 ;
V_27 -> V_161 = 0 ;
V_27 -> V_91 = 0 ;
V_27 -> V_207 = 2 ;
V_27 -> V_165 = 2 ;
V_27 -> V_98 = V_100 ;
V_27 -> V_99 = 10 * V_100 ;
V_27 -> V_166 = 125 * V_100 / 4 ;
V_27 -> V_167 = 125 * V_100 ;
V_27 -> V_201 = 255 * V_100 ;
V_27 -> V_138 = 260 * V_100 ;
V_27 -> V_162 . V_200 = 0 ;
V_27 -> V_153 . V_60 = NULL ;
#if F_3 ( V_9 )
V_27 -> V_163 . V_200 = 0 ;
V_27 -> V_154 . V_60 = NULL ;
#endif
V_27 -> V_113 = 1 ;
F_158 ( & V_27 -> V_50 ) ;
F_84 ( & V_27 -> V_145 ,
F_96 , 0 ) ;
F_84 ( & V_27 -> V_162 . V_52 ,
F_99 , ( unsigned long ) V_27 ) ;
F_84 ( & V_27 -> V_151 . V_52 , F_155 ,
( unsigned long ) V_27 ) ;
#if F_3 ( V_9 )
F_84 ( & V_27 -> V_163 . V_52 ,
F_100 , ( unsigned long ) V_27 ) ;
F_84 ( & V_27 -> V_152 . V_52 , F_156 ,
( unsigned long ) V_27 ) ;
#endif
}
static void F_159 ( struct V_26 * V_27 ,
struct V_149 * V_150 )
{
V_150 -> V_164 = 0 ;
if ( V_27 -> V_30 )
return;
F_42 ( & V_150 -> V_52 , V_63 ) ;
}
void F_160 ( struct V_26 * V_27 )
{
F_159 ( V_27 , & V_27 -> V_151 ) ;
#if F_3 ( V_9 )
F_159 ( V_27 , & V_27 -> V_152 ) ;
#endif
}
void F_161 ( struct V_26 * V_27 )
{
F_114 ( & V_27 -> V_145 ) ;
F_114 ( & V_27 -> V_162 . V_52 ) ;
F_114 ( & V_27 -> V_151 . V_52 ) ;
#if F_3 ( V_9 )
F_114 ( & V_27 -> V_163 . V_52 ) ;
F_114 ( & V_27 -> V_152 . V_52 ) ;
#endif
}
void F_162 ( struct V_26 * V_27 )
{
struct V_12 * V_13 ;
struct V_20 * V_23 ;
struct V_169 * V_170 ;
V_16 V_45 ;
int V_43 ;
F_111 ( & V_27 -> V_50 ) ;
V_13 = F_32 ( V_27 -> V_13 , V_27 ) ;
if ( ! V_13 )
goto V_53;
V_27 -> V_13 = NULL ;
V_45 = V_13 -> V_45 ;
for ( V_43 = 0 ; V_43 < V_13 -> V_18 ; V_43 ++ ) {
F_112 (mp, n, &mdb->mhash[i],
hlist[ver]) {
F_40 ( & V_23 -> V_52 ) ;
F_34 ( & V_23 -> V_36 , F_27 ) ;
}
}
if ( V_13 -> V_37 ) {
F_113 ( & V_27 -> V_50 ) ;
F_163 () ;
F_111 ( & V_27 -> V_50 ) ;
F_37 ( V_13 -> V_37 ) ;
}
V_13 -> V_37 = V_13 ;
F_34 ( & V_13 -> V_36 , F_20 ) ;
V_53:
F_113 ( & V_27 -> V_50 ) ;
}
int F_164 ( struct V_26 * V_27 , unsigned long V_228 )
{
int V_66 = - V_46 ;
F_111 ( & V_27 -> V_50 ) ;
switch ( V_228 ) {
case V_143 :
case V_144 :
F_40 ( & V_27 -> V_145 ) ;
case V_148 :
V_27 -> V_142 = V_228 ;
V_66 = 0 ;
break;
}
F_113 ( & V_27 -> V_50 ) ;
return V_66 ;
}
static void F_165 ( struct V_124 * V_48 )
{
if ( F_44 ( & V_48 -> V_146 ) )
return;
F_94 ( & V_48 -> V_146 ) ;
F_95 ( V_48 -> V_27 -> V_51 , V_48 , V_61 ) ;
}
int F_166 ( struct V_124 * V_48 , unsigned long V_228 )
{
struct V_26 * V_27 = V_48 -> V_27 ;
unsigned long V_135 = V_63 ;
int V_66 = - V_46 ;
F_29 ( & V_27 -> V_50 ) ;
if ( V_48 -> V_142 == V_228 ) {
if ( V_48 -> V_142 == V_147 )
F_42 ( & V_48 -> V_145 ,
V_135 + V_27 -> V_201 ) ;
V_66 = 0 ;
goto V_229;
}
switch ( V_228 ) {
case V_143 :
V_48 -> V_142 = V_143 ;
F_165 ( V_48 ) ;
F_40 ( & V_48 -> V_145 ) ;
break;
case V_148 :
V_48 -> V_142 = V_148 ;
F_165 ( V_48 ) ;
break;
case V_144 :
V_48 -> V_142 = V_144 ;
F_40 ( & V_48 -> V_145 ) ;
F_118 ( V_27 , V_48 ) ;
break;
case V_147 :
V_48 -> V_142 = V_147 ;
F_137 ( V_27 , V_48 ) ;
break;
default:
goto V_229;
}
V_66 = 0 ;
V_229:
F_35 ( & V_27 -> V_50 ) ;
return V_66 ;
}
static void F_98 ( struct V_26 * V_27 ,
struct V_149 * V_150 )
{
struct V_124 * V_60 ;
F_159 ( V_27 , V_150 ) ;
F_167 (port, &br->port_list, list) {
if ( V_60 -> V_136 == V_137 ||
V_60 -> V_136 == V_168 )
continue;
if ( V_150 == & V_27 -> V_151 )
F_115 ( & V_60 -> V_151 ) ;
#if F_3 ( V_9 )
else
F_115 ( & V_60 -> V_152 ) ;
#endif
}
}
int F_168 ( struct V_26 * V_27 , unsigned long V_228 )
{
int V_66 = 0 ;
struct V_12 * V_13 ;
F_111 ( & V_27 -> V_50 ) ;
if ( V_27 -> V_30 == ! V_228 )
goto V_229;
V_27 -> V_30 = ! V_228 ;
if ( V_27 -> V_30 )
goto V_229;
if ( ! F_30 ( V_27 -> V_51 ) )
goto V_229;
V_13 = F_32 ( V_27 -> V_13 , V_27 ) ;
if ( V_13 ) {
if ( V_13 -> V_37 ) {
V_66 = - V_131 ;
V_230:
V_27 -> V_30 = ! ! V_228 ;
goto V_229;
}
V_66 = F_45 ( & V_27 -> V_13 , V_13 -> V_18 ,
V_27 -> V_126 ) ;
if ( V_66 )
goto V_230;
}
F_98 ( V_27 , & V_27 -> V_151 ) ;
#if F_3 ( V_9 )
F_98 ( V_27 , & V_27 -> V_152 ) ;
#endif
V_229:
F_113 ( & V_27 -> V_50 ) ;
return V_66 ;
}
int F_169 ( struct V_26 * V_27 , unsigned long V_228 )
{
unsigned long V_199 ;
V_228 = ! ! V_228 ;
F_111 ( & V_27 -> V_50 ) ;
if ( V_27 -> V_161 == V_228 )
goto V_229;
V_27 -> V_161 = V_228 ;
if ( ! V_228 )
goto V_229;
V_199 = V_27 -> V_99 ;
if ( ! F_31 ( & V_27 -> V_162 . V_52 ) )
V_27 -> V_162 . V_200 = V_63 + V_199 ;
F_98 ( V_27 , & V_27 -> V_151 ) ;
#if F_3 ( V_9 )
if ( ! F_31 ( & V_27 -> V_163 . V_52 ) )
V_27 -> V_163 . V_200 = V_63 + V_199 ;
F_98 ( V_27 , & V_27 -> V_152 ) ;
#endif
V_229:
F_113 ( & V_27 -> V_50 ) ;
return 0 ;
}
int F_170 ( struct V_26 * V_27 , unsigned long V_228 )
{
int V_66 = - V_46 ;
V_16 V_37 ;
struct V_12 * V_13 ;
F_111 ( & V_27 -> V_50 ) ;
if ( ! F_171 ( V_228 ) )
goto V_229;
V_13 = F_32 ( V_27 -> V_13 , V_27 ) ;
if ( V_13 && V_228 < V_13 -> V_56 )
goto V_229;
V_66 = 0 ;
V_37 = V_27 -> V_128 ;
V_27 -> V_128 = V_228 ;
if ( V_13 ) {
if ( V_13 -> V_37 ) {
V_66 = - V_131 ;
V_230:
V_27 -> V_128 = V_37 ;
goto V_229;
}
V_66 = F_45 ( & V_27 -> V_13 , V_27 -> V_128 ,
V_27 -> V_126 ) ;
if ( V_66 )
goto V_230;
}
V_229:
F_113 ( & V_27 -> V_50 ) ;
return V_66 ;
}
int F_172 ( struct V_231 * V_51 ,
struct V_232 * V_233 )
{
struct V_26 * V_27 ;
struct V_124 * V_60 ;
struct V_47 * V_70 ;
struct V_233 * V_234 ;
int V_125 = 0 ;
F_173 () ;
if ( ! V_233 || ! F_174 ( V_51 ) )
goto V_229;
V_60 = F_175 ( V_51 ) ;
if ( ! V_60 || ! V_60 -> V_27 )
goto V_229;
V_27 = V_60 -> V_27 ;
F_176 (port, &br->port_list, list) {
if ( ! V_60 -> V_51 || V_60 -> V_51 == V_51 )
continue;
F_11 (group, &port->mglist, mglist) {
V_234 = F_47 ( sizeof( * V_234 ) , V_67 ) ;
if ( ! V_234 )
goto V_229;
V_234 -> V_24 = V_70 -> V_24 ;
F_177 ( & V_234 -> V_235 , V_233 ) ;
V_125 ++ ;
}
}
V_229:
F_178 () ;
return V_125 ;
}
bool F_179 ( struct V_231 * V_51 , int V_4 )
{
struct V_26 * V_27 ;
struct V_124 * V_60 ;
struct V_73 V_74 ;
bool V_236 = false ;
F_173 () ;
if ( ! F_174 ( V_51 ) )
goto V_229;
V_60 = F_175 ( V_51 ) ;
if ( ! V_60 || ! V_60 -> V_27 )
goto V_229;
V_27 = V_60 -> V_27 ;
memset ( & V_74 , 0 , sizeof( V_74 ) ) ;
V_74 . V_80 = F_2 ( V_4 ) ;
V_236 = F_180 ( V_27 , & V_74 ) ;
V_229:
F_178 () ;
return V_236 ;
}
bool F_181 ( struct V_231 * V_51 , int V_4 )
{
struct V_26 * V_27 ;
struct V_124 * V_60 ;
bool V_236 = false ;
F_173 () ;
if ( ! F_174 ( V_51 ) )
goto V_229;
V_60 = F_175 ( V_51 ) ;
if ( ! V_60 || ! V_60 -> V_27 )
goto V_229;
V_27 = V_60 -> V_27 ;
switch ( V_4 ) {
case V_6 :
if ( ! F_31 ( & V_27 -> V_162 . V_52 ) ||
F_16 ( V_27 -> V_153 . V_60 ) == V_60 )
goto V_229;
break;
#if F_3 ( V_9 )
case V_10 :
if ( ! F_31 ( & V_27 -> V_163 . V_52 ) ||
F_16 ( V_27 -> V_154 . V_60 ) == V_60 )
goto V_229;
break;
#endif
default:
goto V_229;
}
V_236 = true ;
V_229:
F_178 () ;
return V_236 ;
}
