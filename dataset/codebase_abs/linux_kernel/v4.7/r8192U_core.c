double F_1 ( int V_1 )
{
return V_1 ;
}
unsigned int F_2 ( double V_2 )
{
return V_2 ;
}
double F_3 ( double V_3 , double V_4 )
{
return V_3 + V_4 ;
}
double F_4 ( float V_3 , float V_4 )
{
return V_3 + V_4 ;
}
double F_5 ( double V_3 , double V_4 )
{
return V_3 - V_4 ;
}
double F_6 ( float V_3 )
{
return V_3 ;
}
static void F_7 ( T_1 V_5 , struct V_6 * V_7 )
{
int V_1 , V_8 = - 1 , V_9 = - 1 ;
struct V_10 * V_11 = V_7 -> V_12 ;
switch ( V_5 ) {
case V_13 :
case V_14 :
case V_15 :
case V_16 :
case V_17 :
case V_18 :
case V_19 :
case V_20 :
case V_21 :
case V_22 :
F_8 ( V_11 ) ;
V_11 -> V_23 = false ;
if ( ( V_7 -> V_24 == V_25 ) || ( V_7 -> V_24 == V_26 ) ) {
V_9 = 1 ;
V_8 = 14 ;
} else {
F_9 ( V_27 ,
L_1 ,
V_28 ) ;
}
if ( V_29 [ V_5 ] . V_30 != 0 ) {
memset ( F_10 ( V_11 ) -> V_31 , 0 ,
sizeof( F_10 ( V_11 ) -> V_31 ) ) ;
for ( V_1 = 0 ; V_1 < V_29 [ V_5 ] . V_30 ; V_1 ++ ) {
if ( V_29 [ V_5 ] . V_32 [ V_1 ] < V_9 || V_29 [ V_5 ] . V_32 [ V_1 ] > V_8 )
break;
F_10 ( V_11 ) -> V_31 [ V_29 [ V_5 ] . V_32 [ V_1 ] ] = 1 ;
}
}
break;
case V_33 :
F_10 ( V_11 ) -> V_34 = 0 ;
F_11 ( V_11 ) ;
V_11 -> V_23 = true ;
break;
default:
break;
}
}
static void F_12 ( struct V_35 * V_36 )
{
T_2 V_37 = 0 ;
V_37 |= F_13 ( 31 ) | F_13 ( 30 ) ;
F_14 ( V_36 , V_38 , V_37 ) ;
}
void F_15 ( struct V_35 * V_36 , T_1 V_39 , T_2 V_40 )
{
F_14 ( V_36 , V_41 , V_40 ) ;
F_14 ( V_36 , V_38 , F_13 ( 31 ) | F_13 ( 16 ) | ( V_39 & 0xff ) ) ;
}
T_2 F_16 ( struct V_35 * V_36 , T_1 V_39 )
{
T_2 V_40 ;
F_14 ( V_36 , V_38 , 0x80000000 | ( V_39 & 0xff ) ) ;
F_17 ( V_36 , 0xa8 , & V_40 ) ;
return V_40 ;
}
void F_18 ( struct V_35 * V_36 , int V_42 , T_1 V_40 )
{
int V_43 ;
struct V_6 * V_7 = (struct V_6 * ) F_19 ( V_36 ) ;
struct V_44 * V_45 = V_7 -> V_45 ;
T_1 * V_46 = F_20 ( sizeof( V_40 ) , V_47 ) ;
if ( ! V_46 )
return;
* V_46 = V_40 ;
V_43 = F_21 ( V_45 , F_22 ( V_45 , 0 ) ,
V_48 , V_49 ,
V_42 | 0xfe00 , 0 , V_46 , 1 , V_50 / 2 ) ;
F_23 ( V_46 ) ;
if ( V_43 < 0 )
F_24 ( V_36 , L_2 ,
V_43 ) ;
}
int F_25 ( struct V_35 * V_36 , int V_42 , T_1 * V_40 )
{
int V_43 ;
struct V_6 * V_7 = (struct V_6 * ) F_19 ( V_36 ) ;
struct V_44 * V_45 = V_7 -> V_45 ;
T_1 * V_46 = F_20 ( sizeof( T_1 ) , V_47 ) ;
if ( ! V_46 )
return - V_51 ;
V_43 = F_21 ( V_45 , F_26 ( V_45 , 0 ) ,
V_52 , V_53 ,
V_42 | 0xfe00 , 0 , V_46 , 1 , V_50 / 2 ) ;
* V_40 = * V_46 ;
F_23 ( V_46 ) ;
if ( V_43 < 0 ) {
F_24 ( V_36 , L_3 , V_28 , V_43 ) ;
return V_43 ;
}
return 0 ;
}
void F_27 ( struct V_35 * V_36 , int V_42 , T_1 V_40 )
{
int V_43 ;
struct V_6 * V_7 = (struct V_6 * ) F_19 ( V_36 ) ;
struct V_44 * V_45 = V_7 -> V_45 ;
T_1 * V_46 = F_20 ( sizeof( V_40 ) , V_47 ) ;
if ( ! V_46 )
return;
* V_46 = V_40 ;
V_43 = F_21 ( V_45 , F_22 ( V_45 , 0 ) ,
V_48 , V_49 ,
( V_42 & 0xff ) | 0xff00 , ( V_42 >> 8 ) & 0x0f ,
V_46 , 1 , V_50 / 2 ) ;
F_23 ( V_46 ) ;
if ( V_43 < 0 )
F_24 ( V_36 , L_4 , V_43 ) ;
}
void F_28 ( struct V_35 * V_36 , int V_42 , T_3 V_40 )
{
int V_43 ;
struct V_6 * V_7 = (struct V_6 * ) F_19 ( V_36 ) ;
struct V_44 * V_45 = V_7 -> V_45 ;
T_3 * V_46 = F_20 ( sizeof( V_40 ) , V_47 ) ;
if ( ! V_46 )
return;
* V_46 = V_40 ;
V_43 = F_21 ( V_45 , F_22 ( V_45 , 0 ) ,
V_48 , V_49 ,
( V_42 & 0xff ) | 0xff00 , ( V_42 >> 8 ) & 0x0f ,
V_46 , 2 , V_50 / 2 ) ;
F_23 ( V_46 ) ;
if ( V_43 < 0 )
F_24 ( V_36 , L_5 , V_43 ) ;
}
void F_14 ( struct V_35 * V_36 , int V_42 , T_2 V_40 )
{
int V_43 ;
struct V_6 * V_7 = (struct V_6 * ) F_19 ( V_36 ) ;
struct V_44 * V_45 = V_7 -> V_45 ;
T_2 * V_46 = F_20 ( sizeof( V_40 ) , V_47 ) ;
if ( ! V_46 )
return;
* V_46 = V_40 ;
V_43 = F_21 ( V_45 , F_22 ( V_45 , 0 ) ,
V_48 , V_49 ,
( V_42 & 0xff ) | 0xff00 , ( V_42 >> 8 ) & 0x0f ,
V_46 , 4 , V_50 / 2 ) ;
F_23 ( V_46 ) ;
if ( V_43 < 0 )
F_24 ( V_36 , L_6 ,
V_43 ) ;
}
int F_29 ( struct V_35 * V_36 , int V_42 , T_1 * V_40 )
{
int V_43 ;
struct V_6 * V_7 = (struct V_6 * ) F_19 ( V_36 ) ;
struct V_44 * V_45 = V_7 -> V_45 ;
T_1 * V_46 = F_20 ( sizeof( T_1 ) , V_47 ) ;
if ( ! V_46 )
return - V_51 ;
V_43 = F_21 ( V_45 , F_26 ( V_45 , 0 ) ,
V_52 , V_53 ,
( V_42 & 0xff ) | 0xff00 , ( V_42 >> 8 ) & 0x0f ,
V_46 , 1 , V_50 / 2 ) ;
* V_40 = * V_46 ;
F_23 ( V_46 ) ;
if ( V_43 < 0 ) {
F_24 ( V_36 , L_3 , V_28 , V_43 ) ;
return V_43 ;
}
return 0 ;
}
int F_30 ( struct V_35 * V_36 , int V_42 , T_3 * V_40 )
{
int V_43 ;
struct V_6 * V_7 = (struct V_6 * ) F_19 ( V_36 ) ;
struct V_44 * V_45 = V_7 -> V_45 ;
T_3 * V_46 = F_20 ( sizeof( T_3 ) , V_47 ) ;
if ( ! V_46 )
return - V_51 ;
V_43 = F_21 ( V_45 , F_26 ( V_45 , 0 ) ,
V_52 , V_53 ,
( V_42 & 0xff ) | 0xff00 , ( V_42 >> 8 ) & 0x0f ,
V_46 , 2 , V_50 / 2 ) ;
* V_40 = * V_46 ;
F_23 ( V_46 ) ;
if ( V_43 < 0 ) {
F_24 ( V_36 , L_3 , V_28 , V_43 ) ;
return V_43 ;
}
return 0 ;
}
static int F_31 ( struct V_35 * V_36 , int V_42 , T_3 * V_40 )
{
int V_43 ;
struct V_6 * V_7 = (struct V_6 * ) F_19 ( V_36 ) ;
struct V_44 * V_45 = V_7 -> V_45 ;
T_3 * V_46 = F_20 ( sizeof( T_3 ) , V_47 ) ;
if ( ! V_46 )
return - V_51 ;
V_43 = F_21 ( V_45 , F_26 ( V_45 , 0 ) ,
V_52 , V_53 ,
V_42 | 0xfe00 , 0 , V_46 , 2 , V_50 / 2 ) ;
* V_40 = * V_46 ;
F_23 ( V_46 ) ;
if ( V_43 < 0 ) {
F_24 ( V_36 , L_3 , V_28 , V_43 ) ;
return V_43 ;
}
return 0 ;
}
int F_17 ( struct V_35 * V_36 , int V_42 , T_2 * V_40 )
{
int V_43 ;
struct V_6 * V_7 = (struct V_6 * ) F_19 ( V_36 ) ;
struct V_44 * V_45 = V_7 -> V_45 ;
T_2 * V_46 = F_20 ( sizeof( T_2 ) , V_47 ) ;
if ( ! V_46 )
return - V_51 ;
V_43 = F_21 ( V_45 , F_26 ( V_45 , 0 ) ,
V_52 , V_53 ,
( V_42 & 0xff ) | 0xff00 , ( V_42 >> 8 ) & 0x0f ,
V_46 , 4 , V_50 / 2 ) ;
* V_40 = * V_46 ;
F_23 ( V_46 ) ;
if ( V_43 < 0 ) {
F_24 ( V_36 , L_3 , V_28 , V_43 ) ;
return V_43 ;
}
return 0 ;
}
inline void F_32 ( struct V_35 * V_36 )
{
}
static int F_33 ( struct V_54 * V_55 , void * V_56 )
{
struct V_35 * V_36 = V_55 -> V_57 ;
struct V_6 * V_7 = (struct V_6 * ) F_19 ( V_36 ) ;
struct V_10 * V_11 = V_7 -> V_12 ;
struct V_58 * V_59 ;
F_34 (target, &ieee->network_list, list) {
const char * V_60 = L_7 ;
if ( V_59 -> V_61 > 0 || V_59 -> V_62 > 0 )
V_60 = L_8 ;
F_35 ( V_55 , L_9 , V_59 -> V_63 , V_60 ) ;
}
return 0 ;
}
static int F_36 ( struct V_54 * V_55 , void * V_56 )
{
struct V_35 * V_36 = V_55 -> V_57 ;
int V_1 , V_64 , V_65 = 0xff ;
T_1 V_66 ;
F_37 ( V_55 , L_10 ) ;
for ( V_64 = 0 ; V_64 <= V_65 ; ) {
F_35 ( V_55 , L_11 , V_64 ) ;
for ( V_1 = 0 ; V_1 < 16 && V_64 <= V_65 ; V_1 ++ , V_64 ++ ) {
F_29 ( V_36 , 0x000 | V_64 , & V_66 ) ;
F_35 ( V_55 , L_12 , V_66 ) ;
}
}
F_37 ( V_55 , L_13 ) ;
for ( V_64 = 0 ; V_64 <= V_65 ; ) {
F_35 ( V_55 , L_11 , V_64 ) ;
for ( V_1 = 0 ; V_1 < 16 && V_64 <= V_65 ; V_1 ++ , V_64 ++ ) {
F_29 ( V_36 , 0x100 | V_64 , & V_66 ) ;
F_35 ( V_55 , L_12 , V_66 ) ;
}
}
F_37 ( V_55 , L_14 ) ;
for ( V_64 = 0 ; V_64 <= V_65 ; ) {
F_35 ( V_55 , L_11 , V_64 ) ;
for ( V_1 = 0 ; V_1 < 16 && V_64 <= V_65 ; V_1 ++ , V_64 ++ ) {
F_29 ( V_36 , 0x300 | V_64 , & V_66 ) ;
F_35 ( V_55 , L_12 , V_66 ) ;
}
}
F_38 ( V_55 , '\n' ) ;
return 0 ;
}
static int F_39 ( struct V_54 * V_55 , void * V_56 )
{
struct V_35 * V_36 = V_55 -> V_57 ;
struct V_6 * V_7 = (struct V_6 * ) F_19 ( V_36 ) ;
F_35 ( V_55 ,
L_15
L_16
L_17
L_18
L_19
L_20
L_21
L_22
L_23
L_24
L_25
L_26
L_27
L_28
L_29
L_30
L_31
L_32
L_33
L_34
L_35
L_36
L_37
L_38 ,
V_7 -> V_67 . V_68 ,
V_7 -> V_67 . V_69 ,
V_7 -> V_67 . V_70 ,
V_7 -> V_67 . V_71 ,
V_7 -> V_67 . V_72 ,
V_7 -> V_67 . V_73 ,
V_7 -> V_67 . V_74 ,
V_7 -> V_67 . V_75 ,
V_7 -> V_67 . V_76 ,
V_7 -> V_67 . V_77 ,
V_7 -> V_67 . V_78 ,
V_7 -> V_67 . V_79 ,
V_7 -> V_67 . V_80 ,
F_40 ( V_36 ) ,
V_7 -> V_67 . V_81 ,
F_41 ( & ( V_7 -> V_82 [ V_83 ] ) ) ,
F_41 ( & ( V_7 -> V_82 [ V_84 ] ) ) ,
F_41 ( & ( V_7 -> V_82 [ V_85 ] ) ) ,
F_41 ( & ( V_7 -> V_82 [ V_86 ] ) ) ,
V_7 -> V_67 . V_87 ,
V_7 -> V_67 . V_88 ,
V_7 -> V_67 . V_89 ,
V_7 -> V_67 . V_90 ,
V_7 -> V_67 . V_91
) ;
return 0 ;
}
static int F_42 ( struct V_54 * V_55 , void * V_56 )
{
struct V_35 * V_36 = V_55 -> V_57 ;
struct V_6 * V_7 = (struct V_6 * ) F_19 ( V_36 ) ;
F_35 ( V_55 ,
L_39
L_40
L_41 ,
V_7 -> V_67 . V_92 ,
V_7 -> V_67 . V_93 ,
V_7 -> V_67 . V_94 ) ;
return 0 ;
}
static void F_43 ( void )
{
F_9 ( V_95 , L_42 ) ;
V_96 = F_44 ( V_97 , V_98 . V_99 ) ;
}
static int F_45 ( struct V_100 * V_100 , struct V_101 * V_101 )
{
struct V_35 * V_36 = F_46 ( V_100 ) ;
int (* F_47)( struct V_54 * , void * ) = F_48 ( V_100 ) ;
return F_49 ( V_101 , F_47 , V_36 ) ;
}
static void F_50 ( struct V_35 * V_36 )
{
const struct V_102 * V_103 ;
struct V_104 * V_105 ;
if ( V_96 ) {
V_105 = F_51 ( V_36 -> V_106 , 0 , V_96 , V_36 ) ;
if ( ! V_105 ) {
F_9 ( V_27 ,
L_43 ,
V_36 -> V_106 ) ;
return;
}
for ( V_103 = V_107 ; V_103 -> V_106 [ 0 ] ; V_103 ++ ) {
if ( ! F_52 ( V_103 -> V_106 , V_108 | V_109 , V_105 ,
& V_110 , V_103 -> F_47 ) ) {
F_9 ( V_27 ,
L_44 ,
V_36 -> V_106 , V_103 -> V_106 ) ;
return;
}
}
}
}
static void F_53 ( struct V_35 * V_36 )
{
F_54 ( V_36 -> V_106 , V_96 ) ;
}
short F_55 ( struct V_35 * V_36 , int V_111 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
int V_112 = F_41 ( & V_7 -> V_82 [ V_111 ] ) ;
return ( V_112 < V_113 ) ;
}
static void F_56 ( struct V_35 * V_36 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
F_57 ( & V_7 -> V_114 ) ;
}
void F_58 ( struct V_35 * V_36 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
T_1 V_115 ;
F_29 ( V_36 , V_116 , & V_115 ) ;
V_115 &= ~ V_117 ;
if ( V_7 -> V_12 -> V_118 == V_119 ) {
if ( V_7 -> V_12 -> V_120 == V_121 )
V_115 |= ( V_122 << V_123 ) ;
else if ( V_7 -> V_12 -> V_120 == V_124 )
V_115 |= ( V_125 << V_123 ) ;
else if ( V_7 -> V_12 -> V_120 == V_126 )
V_115 |= ( V_127 << V_123 ) ;
} else {
V_115 |= ( V_128 << V_123 ) ;
}
F_27 ( V_36 , V_116 , V_115 ) ;
}
void F_59 ( struct V_35 * V_36 , short V_129 )
{
struct V_6 * V_7 = (struct V_6 * ) F_19 ( V_36 ) ;
F_9 ( V_130 , L_45 , V_28 , V_129 ) ;
V_7 -> V_131 = V_129 ;
if ( V_7 -> V_132 )
V_7 -> V_132 ( V_36 , V_7 -> V_131 ) ;
F_60 ( 10 ) ;
}
static T_2 F_61 ( struct V_133 * V_134 )
{
return ( sizeof( V_135 ) + V_134 -> V_136
+ V_134 -> V_137 ) ;
}
static int F_62 ( struct V_35 * V_36 )
{
struct V_6 * V_7 = (struct V_6 * ) F_19 ( V_36 ) ;
struct V_138 * V_139 ;
struct V_140 * V_141 ;
struct V_142 * V_143 ;
while ( F_63 ( & V_7 -> V_144 ) < V_145 ) {
V_141 = F_64 ( V_146 , V_47 ) ;
if ( ! V_141 )
break;
V_139 = F_65 ( 0 , V_47 ) ;
if ( ! V_139 ) {
F_66 ( V_141 ) ;
break;
}
F_67 ( V_139 , V_7 -> V_45 ,
F_68 ( V_7 -> V_45 , 3 ) ,
F_69 ( V_141 ) ,
V_146 , V_147 , V_141 ) ;
V_143 = (struct V_142 * ) V_141 -> V_148 ;
V_143 -> V_138 = V_139 ;
V_143 -> V_36 = V_36 ;
V_143 -> V_149 = 3 ;
F_70 ( & V_7 -> V_144 , V_141 ) ;
F_71 ( V_139 , V_47 ) ;
}
while ( F_63 ( & V_7 -> V_144 ) < V_145 + 3 ) {
V_141 = F_64 ( V_146 , V_47 ) ;
if ( ! V_141 )
break;
V_139 = F_65 ( 0 , V_47 ) ;
if ( ! V_139 ) {
F_66 ( V_141 ) ;
break;
}
F_67 ( V_139 , V_7 -> V_45 ,
F_68 ( V_7 -> V_45 , 9 ) ,
F_69 ( V_141 ) ,
V_146 , V_147 , V_141 ) ;
V_143 = (struct V_142 * ) V_141 -> V_148 ;
V_143 -> V_138 = V_139 ;
V_143 -> V_36 = V_36 ;
V_143 -> V_149 = 9 ;
F_70 ( & V_7 -> V_144 , V_141 ) ;
F_71 ( V_139 , V_47 ) ;
}
return 0 ;
}
void F_72 ( struct V_35 * V_36 )
{
struct V_6 * V_7 = (struct V_6 * ) F_19 ( V_36 ) ;
T_2 V_150 ;
F_17 ( V_36 , V_151 , & V_150 ) ;
V_150 = V_150 & ~ V_152 ;
V_150 = V_150 | V_153 ;
V_150 = V_150 | V_154 ;
V_150 = V_150 | V_155 ;
V_150 = V_150 | V_156 ;
if ( V_36 -> V_157 & V_158 )
F_73 ( L_46 ) ;
if ( V_7 -> V_12 -> V_120 == V_159 ||
V_36 -> V_157 & V_158 ) {
V_150 = V_150 | V_160 ;
} else {
V_150 = V_150 | V_161 ;
V_150 = V_150 | V_162 ;
}
if ( V_7 -> V_12 -> V_120 == V_159 ) {
V_150 = V_150 | V_163 ;
V_150 = V_150 | V_164 ;
}
if ( V_7 -> V_165 == 1 && V_7 -> V_12 -> V_120 == V_159 )
V_150 = V_150 | V_166 ;
V_150 = V_150 & ~ V_167 ;
V_150 = V_150 | ( V_168 << V_169 ) ;
V_150 = V_150 & ~ V_170 ;
V_150 = V_150 | ( ( T_2 ) 7 << V_171 ) ;
V_150 = V_150 | V_172 ;
F_14 ( V_36 , V_151 , V_150 ) ;
}
void F_74 ( struct V_35 * V_36 )
{
F_62 ( V_36 ) ;
}
void F_75 ( struct V_35 * V_36 )
{
}
void F_76 ( struct V_35 * V_36 )
{
T_1 V_173 ;
struct V_6 * V_7 = F_19 ( V_36 ) ;
struct V_140 * V_141 ;
struct V_142 * V_143 ;
F_29 ( V_36 , V_174 , & V_173 ) ;
F_27 ( V_36 , V_174 , V_173 & ~ ( V_175 | V_176 ) ) ;
F_32 ( V_36 ) ;
F_60 ( 10 ) ;
while ( ( V_141 = F_77 ( & V_7 -> V_144 ) ) ) {
V_143 = (struct V_142 * ) V_141 -> V_148 ;
if ( ! V_143 -> V_138 )
continue;
F_78 ( V_143 -> V_138 ) ;
F_66 ( V_141 ) ;
}
if ( F_63 ( & V_7 -> V_177 ) )
F_79 ( V_36 , L_47 ) ;
F_80 ( & V_7 -> V_177 ) ;
}
inline T_3 F_81 ( int V_178 )
{
switch ( V_178 ) {
case 10 :
return 0 ;
case 20 :
return 1 ;
case 55 :
return 2 ;
case 110 :
return 3 ;
case 60 :
return 4 ;
case 90 :
return 5 ;
case 120 :
return 6 ;
case 180 :
return 7 ;
case 240 :
return 8 ;
case 360 :
return 9 ;
case 480 :
return 10 ;
case 540 :
return 11 ;
default:
return 3 ;
}
}
inline T_3 F_82 ( short V_178 )
{
if ( V_178 > 11 )
return 0 ;
return V_179 [ V_178 ] ;
}
static void V_147 ( struct V_138 * V_138 )
{
struct V_140 * V_141 = (struct V_140 * ) V_138 -> V_180 ;
struct V_142 * V_143 = (struct V_142 * ) V_141 -> V_148 ;
struct V_35 * V_36 = V_143 -> V_36 ;
struct V_6 * V_7 = F_19 ( V_36 ) ;
int V_149 = V_143 -> V_149 ;
int V_181 ;
if ( ! V_7 -> V_182 )
return;
if ( F_83 ( V_138 -> V_43 ) ) {
V_143 -> V_138 = NULL ;
V_7 -> V_67 . V_93 ++ ;
V_7 -> V_12 -> V_67 . V_183 ++ ;
F_84 ( V_138 ) ;
return;
}
F_85 ( V_141 , & V_7 -> V_144 ) ;
F_86 ( V_141 , V_138 -> V_184 ) ;
F_70 ( & V_7 -> V_177 , V_141 ) ;
F_87 ( & V_7 -> V_185 ) ;
V_141 = F_88 ( V_146 ) ;
if ( F_83 ( ! V_141 ) ) {
F_84 ( V_138 ) ;
F_24 ( V_36 , L_48 , V_28 ) ;
return;
}
F_67 ( V_138 , V_7 -> V_45 ,
F_68 ( V_7 -> V_45 , V_149 ) ,
F_69 ( V_141 ) ,
V_146 , V_147 , V_141 ) ;
V_143 = (struct V_142 * ) V_141 -> V_148 ;
V_143 -> V_138 = V_138 ;
V_143 -> V_36 = V_36 ;
V_143 -> V_149 = V_149 ;
V_138 -> V_186 = F_69 ( V_141 ) ;
V_138 -> V_180 = V_141 ;
F_70 ( & V_7 -> V_144 , V_141 ) ;
V_181 = F_71 ( V_138 , V_187 ) ;
if ( V_181 && V_181 != V_188 )
F_24 ( V_36 ,
L_49 ,
V_181 , V_138 -> V_43 ) ;
}
static T_2 F_89 ( struct V_35 * V_36 ,
struct V_133 * V_134 )
{
T_2 V_43 ;
V_43 = F_90 ( V_36 , V_134 ) ;
if ( V_43 )
F_73 ( L_50 ) ;
return V_43 ;
}
static void F_91 ( struct V_35 * V_36 )
{
}
static void F_92 ( struct V_35 * V_36 )
{
}
static void F_93 ( struct V_140 * V_141 , struct V_35 * V_36 ,
int V_178 )
{
struct V_6 * V_7 = (struct V_6 * ) F_19 ( V_36 ) ;
int V_189 ;
unsigned long V_157 ;
T_4 * V_190 = ( T_4 * ) ( V_141 -> V_148 + V_191 ) ;
T_1 V_111 = V_190 -> V_111 ;
F_94 ( V_111 != V_192 ) ;
F_95 ( & V_7 -> V_193 , V_157 ) ;
* (struct V_35 * * ) ( V_141 -> V_148 ) = V_36 ;
V_190 -> V_194 = 1 ;
F_96 ( V_141 , V_7 -> V_12 -> V_195 ) ;
V_189 = F_97 ( V_36 , V_141 ) ;
F_98 ( & V_7 -> V_193 , V_157 ) ;
}
static int F_99 ( struct V_140 * V_141 , struct V_35 * V_36 )
{
struct V_6 * V_7 = (struct V_6 * ) F_19 ( V_36 ) ;
int V_189 ;
unsigned long V_157 ;
T_4 * V_190 = ( T_4 * ) ( V_141 -> V_148 + V_191 ) ;
T_1 V_111 = V_190 -> V_111 ;
F_95 ( & V_7 -> V_193 , V_157 ) ;
memcpy ( ( unsigned char * ) ( V_141 -> V_148 ) , & V_36 , sizeof( V_36 ) ) ;
if ( V_111 == V_192 ) {
F_96 ( V_141 , V_196 ) ;
F_100 ( V_36 , V_141 ) ;
V_189 = 1 ;
} else {
F_96 ( V_141 , V_7 -> V_12 -> V_195 ) ;
V_189 = F_97 ( V_36 , V_141 ) ;
}
F_98 ( & V_7 -> V_193 , V_157 ) ;
return V_189 ;
}
static void F_101 ( struct V_138 * V_197 )
{
struct V_140 * V_141 = (struct V_140 * ) V_197 -> V_180 ;
struct V_35 * V_36 ;
struct V_6 * V_7 = NULL ;
T_4 * V_190 ;
T_1 V_111 ;
if ( ! V_141 )
return;
V_36 = * (struct V_35 * * ) ( V_141 -> V_148 ) ;
V_190 = ( T_4 * ) ( V_141 -> V_148 + V_191 ) ;
V_111 = V_190 -> V_111 ;
V_7 = F_19 ( V_36 ) ;
if ( V_190 -> V_111 != V_192 ) {
if ( V_197 -> V_43 == 0 ) {
F_102 ( V_36 ) ;
V_7 -> V_67 . V_198 ++ ;
V_7 -> V_12 -> V_199 . V_200 ++ ;
V_7 -> V_67 . V_201 +=
( V_141 -> V_202 - V_7 -> V_12 -> V_195 ) ;
} else {
V_7 -> V_12 -> V_67 . V_203 ++ ;
}
}
F_103 ( V_141 ) ;
F_84 ( V_197 ) ;
F_104 ( & V_7 -> V_82 [ V_111 ] ) ;
if ( V_111 != V_204 ) {
if ( ( F_63 ( & V_7 -> V_12 -> V_205 [ V_111 ] ) != 0 ) &&
( ! ( V_7 -> V_12 -> V_206 ) ) ) {
V_141 = F_105 ( & ( V_7 -> V_12 -> V_205 [ V_111 ] ) ) ;
if ( V_141 )
V_7 -> V_12 -> V_207 ( V_141 ,
V_36 ) ;
return;
}
}
}
static void F_106 ( struct V_35 * V_36 , T_3 * V_208 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
struct V_58 * V_209 ;
T_1 V_1 = 0 , V_210 = 0 ;
V_209 = & V_7 -> V_12 -> V_211 ;
for ( V_1 = 0 ; V_1 < V_209 -> V_212 ; V_1 ++ ) {
V_210 = V_209 -> V_213 [ V_1 ] & 0x7f ;
switch ( V_210 ) {
case V_214 :
* V_208 |= V_215 ;
break;
case V_216 :
* V_208 |= V_217 ;
break;
case V_218 :
* V_208 |= V_219 ;
break;
case V_220 :
* V_208 |= V_221 ;
break;
case V_222 :
* V_208 |= V_223 ;
break;
case V_224 :
* V_208 |= V_225 ;
break;
case V_226 :
* V_208 |= V_227 ;
break;
case V_228 :
* V_208 |= V_229 ;
break;
case V_230 :
* V_208 |= V_231 ;
break;
case V_232 :
* V_208 |= V_233 ;
break;
case V_234 :
* V_208 |= V_235 ;
break;
case V_236 :
* V_208 |= V_237 ;
break;
}
}
for ( V_1 = 0 ; V_1 < V_209 -> V_238 ; V_1 ++ ) {
V_210 = V_209 -> V_239 [ V_1 ] & 0x7f ;
switch ( V_210 ) {
case V_214 :
* V_208 |= V_215 ;
break;
case V_216 :
* V_208 |= V_217 ;
break;
case V_218 :
* V_208 |= V_219 ;
break;
case V_220 :
* V_208 |= V_221 ;
break;
case V_222 :
* V_208 |= V_223 ;
break;
case V_224 :
* V_208 |= V_225 ;
break;
case V_226 :
* V_208 |= V_227 ;
break;
case V_228 :
* V_208 |= V_229 ;
break;
case V_230 :
* V_208 |= V_231 ;
break;
case V_232 :
* V_208 |= V_233 ;
break;
case V_234 :
* V_208 |= V_235 ;
break;
case V_236 :
* V_208 |= V_237 ;
break;
}
}
}
static void F_107 ( struct V_35 * V_36 , T_3 V_240 )
{
T_2 V_241 = 0 ;
struct V_6 * V_7 = F_19 ( V_36 ) ;
struct V_58 * V_209 = & V_7 -> V_12 -> V_211 ;
V_7 -> V_242 = V_240 & V_243 ;
V_241 = V_7 -> V_210 ;
if ( V_7 -> V_242 )
V_241 |= V_244 ;
F_14 ( V_36 , V_245 , V_241 ) ;
if ( V_209 -> V_246 & ( V_247 | V_248 ) ) {
T_1 V_249 = 0 ;
if ( ( V_240 & V_250 ) &&
( ! V_7 -> V_12 -> V_251 -> V_252 ) )
V_249 = V_253 ;
else
V_249 = V_254 ;
V_7 -> V_249 = V_249 ;
F_27 ( V_36 , V_255 , V_249 ) ;
}
}
static void F_108 ( struct V_35 * V_36 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
struct V_58 * V_209 ;
T_3 V_256 = 0 , V_257 = 6 , V_258 = 0xf ;
T_3 V_208 = 0 ;
V_209 = & V_7 -> V_12 -> V_211 ;
F_106 ( V_36 , & V_208 ) ;
V_7 -> V_210 = V_208 & 0x15f ;
F_14 ( V_36 , V_259 , ( ( T_2 * ) V_209 -> V_260 ) [ 0 ] ) ;
F_28 ( V_36 , V_259 + 4 , ( ( T_3 * ) V_209 -> V_260 ) [ 2 ] ) ;
F_58 ( V_36 ) ;
if ( V_7 -> V_12 -> V_120 == V_124 ) {
F_28 ( V_36 , V_261 , 2 ) ;
F_28 ( V_36 , V_262 , 1023 ) ;
F_28 ( V_36 , V_263 , V_209 -> V_264 ) ;
F_28 ( V_36 , V_265 , 1 ) ;
F_27 ( V_36 , V_266 , 100 ) ;
V_256 |= ( V_257 << V_267 ) ;
V_256 |= V_258 << V_268 ;
F_28 ( V_36 , V_269 , V_256 ) ;
}
}
void F_109 ( struct V_35 * V_36 , T_3 V_270 )
{
}
inline T_1 F_110 ( T_3 V_178 )
{
if ( ( ( V_178 <= 110 ) && ( V_178 != 60 ) && ( V_178 != 90 ) ) || ( V_178 == 220 ) )
return 1 ;
else
return 0 ;
}
short F_100 ( struct V_35 * V_36 , struct V_140 * V_141 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
int V_43 ;
struct V_138 * V_197 ;
unsigned int V_271 ;
T_5 * V_272 = ( T_5 * ) V_141 -> V_40 ;
T_4 * V_190 = ( T_4 * ) ( V_141 -> V_148 + V_191 ) ;
T_1 V_111 = V_190 -> V_111 ;
F_111 ( & V_7 -> V_82 [ V_111 ] ) ;
V_197 = F_65 ( 0 , V_187 ) ;
if ( ! V_197 ) {
F_112 ( V_141 ) ;
return - V_51 ;
}
memset ( V_272 , 0 , V_196 ) ;
V_272 -> V_273 = 1 ;
V_272 -> V_274 = 1 ;
V_272 -> V_275 = V_190 -> V_276 ;
V_272 -> V_277 = V_190 -> V_278 ;
V_272 -> V_279 = 1 ;
V_272 -> V_280 = V_190 -> V_281 ;
V_271 = 0x04 ;
F_67 ( V_197 , V_7 -> V_45 ,
F_113 ( V_7 -> V_45 , V_271 ) ,
V_141 -> V_40 , V_141 -> V_202 , F_101 , V_141 ) ;
V_43 = F_71 ( V_197 , V_187 ) ;
if ( ! V_43 )
return 0 ;
F_114 ( L_51 , V_43 ) ;
return - 1 ;
}
static T_1 F_115 ( T_1 V_282 )
{
T_1 V_283 = 0x0 ;
switch ( V_282 ) {
case V_284 :
V_283 = V_285 ;
break;
case V_286 :
V_283 = V_287 ;
break;
case V_288 :
V_283 = V_289 ;
break;
case V_290 :
V_283 = V_291 ;
break;
case V_292 :
V_283 = V_293 ;
break;
case V_204 :
V_283 = V_294 ;
break;
case V_192 :
V_283 = V_295 ;
break;
case V_296 :
V_283 = V_297 ;
break;
default:
F_9 ( V_27 ,
L_52 ,
V_282 ) ;
break;
}
return V_283 ;
}
static T_1 F_116 ( T_1 V_178 )
{
T_1 V_189 = V_298 ;
switch ( V_178 ) {
case V_214 :
V_189 = V_298 ;
break;
case V_216 :
V_189 = V_299 ;
break;
case V_218 :
V_189 = V_300 ;
break;
case V_220 :
V_189 = V_301 ;
break;
case V_222 :
V_189 = V_302 ;
break;
case V_224 :
V_189 = V_303 ;
break;
case V_226 :
V_189 = V_304 ;
break;
case V_228 :
V_189 = V_305 ;
break;
case V_230 :
V_189 = V_306 ;
break;
case V_232 :
V_189 = V_307 ;
break;
case V_234 :
V_189 = V_308 ;
break;
case V_236 :
V_189 = V_309 ;
break;
case V_310 :
V_189 = V_311 ;
break;
case V_312 :
V_189 = V_313 ;
break;
case V_314 :
V_189 = V_315 ;
break;
case V_316 :
V_189 = V_317 ;
break;
case V_318 :
V_189 = V_319 ;
break;
case V_320 :
V_189 = V_321 ;
break;
case V_322 :
V_189 = V_323 ;
break;
case V_324 :
V_189 = V_325 ;
break;
case V_326 :
V_189 = V_327 ;
break;
case V_328 :
V_189 = V_329 ;
break;
case V_330 :
V_189 = V_331 ;
break;
case V_332 :
V_189 = V_333 ;
break;
case V_334 :
V_189 = V_335 ;
break;
case V_336 :
V_189 = V_337 ;
break;
case V_338 :
V_189 = V_339 ;
break;
case V_340 :
V_189 = V_341 ;
break;
case ( 0x80 | 0x20 ) :
V_189 = V_342 ;
break;
default:
break;
}
return V_189 ;
}
static T_1 F_117 ( T_1 V_343 , T_1 V_344 , T_4 * V_190 )
{
T_1 V_345 ;
V_345 = ( V_343 == 1 ) ?
( ( V_190 -> V_346 ) ? 1 : 0 ) :
( ( V_190 -> V_347 ) ? 1 : 0 ) ;
if ( V_343 == 1 && V_344 != V_341 )
V_345 = 0 ;
return V_345 ;
}
static void F_118 ( struct V_138 * V_197 )
{
}
short F_97 ( struct V_35 * V_36 , struct V_140 * V_141 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
T_4 * V_190 = ( T_4 * ) ( V_141 -> V_148 + V_191 ) ;
T_6 * V_348 = ( T_6 * ) V_141 -> V_40 ;
T_7 * V_349 =
( T_7 * ) ( V_141 -> V_40 + V_196 ) ;
struct V_44 * V_45 = V_7 -> V_45 ;
int V_350 ;
int V_43 ;
struct V_138 * V_197 = NULL , * V_351 = NULL ;
unsigned int V_271 ;
V_350 = F_41 ( & V_7 -> V_82 [ V_190 -> V_111 ] ) ;
if ( V_350 > V_113 ) {
F_119 ( V_36 , L_53 ) ;
F_103 ( V_141 ) ;
return - 1 ;
}
V_197 = F_65 ( 0 , V_187 ) ;
if ( ! V_197 ) {
F_103 ( V_141 ) ;
return - V_51 ;
}
memset ( V_349 , 0 , sizeof( T_7 ) ) ;
V_349 -> V_343 = ( V_190 -> V_352 & 0x80 ) ? 1 : 0 ;
V_349 -> V_344 = F_116 ( V_190 -> V_352 ) ;
V_349 -> V_353 = V_190 -> V_194 ;
V_349 -> Short = F_117 ( V_349 -> V_343 , V_349 -> V_344 ,
V_190 ) ;
if ( V_190 -> V_354 ) {
V_349 -> V_355 = 1 ;
V_349 -> V_356 = V_190 -> V_357 ;
V_349 -> V_358 = V_190 -> V_359 & 0x07 ;
} else {
V_349 -> V_355 = 0 ;
V_349 -> V_356 = 0 ;
V_349 -> V_358 = 0 ;
}
V_349 -> V_360 = ( V_190 -> V_361 ) ? 1 : 0 ;
V_349 -> V_362 = ( V_190 -> V_363 ) ? 1 : 0 ;
V_349 -> V_364 = ( V_190 -> V_365 ) ? 1 : 0 ;
V_349 -> V_366 = ( V_190 -> V_367 & 0x80 ) ? 1 : 0 ;
V_349 -> V_368 = F_116 ( ( T_1 ) V_190 -> V_367 ) ;
V_349 -> V_369 = ( V_349 -> V_366 == 0 ) ? ( V_190 -> V_370 ) : 0 ;
V_349 -> V_371 = ( V_349 -> V_366 == 1 ) ? ( ( V_190 -> V_372 ) ? 1 : 0 ) : 0 ;
V_349 -> V_373 = ( V_349 -> V_366 == 0 ) ? ( V_190 -> V_374 ? 1 : 0 ) :
( V_190 -> V_375 ? 1 : 0 ) ;
if ( V_7 -> V_376 == V_377 ) {
if ( V_190 -> V_378 ) {
V_349 -> V_379 = 1 ;
V_349 -> V_380 = 0 ;
} else {
V_349 -> V_379 = 0 ;
V_349 -> V_380 = V_7 -> V_381 ;
}
} else {
V_349 -> V_379 = 0 ;
V_349 -> V_380 = 0 ;
}
memset ( V_348 , 0 , sizeof( T_6 ) ) ;
V_348 -> V_280 = 0 ;
V_348 -> V_275 = 1 ;
V_348 -> V_382 = sizeof( T_7 ) + 8 ;
V_348 -> V_383 = ( V_141 -> V_202 - V_384 ) & 0xffff ;
V_348 -> V_385 = 0 ;
V_348 -> V_386 = V_190 -> V_387 ;
V_348 -> V_388 = 1 ;
V_348 -> V_389 = 0x0 ;
if ( V_190 -> V_390 ) {
switch ( V_7 -> V_12 -> V_391 ) {
case V_392 :
case V_393 :
V_348 -> V_389 = 0x1 ;
V_348 -> V_388 = 0 ;
break;
case V_394 :
V_348 -> V_389 = 0x2 ;
V_348 -> V_388 = 0 ;
break;
case V_395 :
V_348 -> V_389 = 0x3 ;
V_348 -> V_388 = 0 ;
break;
case V_396 :
V_348 -> V_389 = 0x0 ;
V_348 -> V_388 = 1 ;
break;
}
}
V_348 -> V_283 = F_115 ( V_190 -> V_111 ) ;
V_348 -> V_397 = sizeof( T_7 ) ;
V_348 -> V_398 = V_190 -> V_399 ;
V_348 -> V_400 = V_190 -> V_401 ;
V_348 -> V_273 = 1 ;
V_348 -> V_274 = 1 ;
V_348 -> V_279 = 1 ;
V_348 -> V_277 = ( T_2 ) ( V_141 -> V_202 - V_196 ) ;
V_271 = 0x5 ;
F_67 ( V_197 , V_45 ,
F_113 ( V_45 , V_271 ) , V_141 -> V_40 ,
V_141 -> V_202 , F_101 , V_141 ) ;
V_43 = F_71 ( V_197 , V_187 ) ;
if ( ! V_43 ) {
bool V_402 = false ;
T_1 V_403 = 0 ;
if ( V_45 -> V_404 == V_405 ) {
if ( V_141 -> V_202 > 0 && V_141 -> V_202 % 512 == 0 )
V_402 = true ;
} else {
if ( V_141 -> V_202 > 0 && V_141 -> V_202 % 64 == 0 )
V_402 = true ;
}
if ( V_402 ) {
V_351 = F_65 ( 0 , V_187 ) ;
if ( ! V_351 ) {
F_9 ( V_27 ,
L_54 ) ;
return - V_51 ;
}
F_67 ( V_351 , V_45 ,
F_113 ( V_45 , V_271 ) ,
& V_403 , 0 , F_118 , V_36 ) ;
V_43 = F_71 ( V_351 , V_187 ) ;
if ( V_43 ) {
F_9 ( V_27 ,
L_55 ,
F_41 ( & V_7 -> V_82 [ V_190 -> V_111 ] ) ,
V_43 ) ;
return - 1 ;
}
}
F_102 ( V_36 ) ;
F_111 ( & V_7 -> V_82 [ V_190 -> V_111 ] ) ;
return 0 ;
}
F_9 ( V_27 , L_56 ,
F_41 ( & V_7 -> V_82 [ V_190 -> V_111 ] ) ,
V_43 ) ;
return - 1 ;
}
static short F_120 ( struct V_35 * V_36 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
V_7 -> V_406 = F_121 ( sizeof( struct V_138 * ) * ( V_145 + 1 ) ,
V_47 ) ;
if ( ! V_7 -> V_406 )
return - V_51 ;
#ifndef F_122
for ( V_1 = 0 ; V_1 < ( V_145 + 1 ) ; V_1 ++ ) {
V_7 -> V_406 [ V_1 ] = F_65 ( 0 , V_47 ) ;
V_7 -> V_406 [ V_1 ] -> V_186 =
F_121 ( V_146 , V_47 ) ;
V_7 -> V_406 [ V_1 ] -> V_407 = V_146 ;
}
#endif
#ifdef F_123
{
long V_408 = 0 ;
void * V_409 , * V_410 ;
V_7 -> V_406 [ 16 ] = F_65 ( 0 , V_47 ) ;
V_7 -> V_409 = F_121 ( 16 , V_47 ) ;
V_409 = V_7 -> V_409 ;
V_408 = ( ( long ) V_409 ) & 3 ;
if ( V_408 ) {
V_410 = V_409 + 4 - V_408 ;
V_7 -> V_406 [ 16 ] -> V_407 = 16 - 4 + V_408 ;
} else {
V_410 = V_409 ;
V_7 -> V_406 [ 16 ] -> V_407 = 16 ;
}
V_7 -> V_406 [ 16 ] -> V_186 = V_410 ;
}
#endif
memset ( V_7 -> V_406 , 0 , sizeof( struct V_138 * ) * V_145 ) ;
V_7 -> V_411 = F_124 ( V_145 , sizeof( struct V_140 * ) ,
V_47 ) ;
if ( ! V_7 -> V_411 ) {
F_23 ( V_7 -> V_406 ) ;
V_7 -> V_411 = NULL ;
V_7 -> V_406 = NULL ;
F_114 ( L_57 ) ;
return - V_51 ;
}
F_119 ( V_36 , L_58 ) ;
return 0 ;
}
static void F_125 ( struct V_35 * V_36 )
{
int V_1 ;
struct V_6 * V_7 = F_19 ( V_36 ) ;
if ( V_7 -> V_406 ) {
for ( V_1 = 0 ; V_1 < ( V_145 + 1 ) ; V_1 ++ ) {
F_78 ( V_7 -> V_406 [ V_1 ] ) ;
F_84 ( V_7 -> V_406 [ V_1 ] ) ;
}
F_23 ( V_7 -> V_406 ) ;
V_7 -> V_406 = NULL ;
}
F_23 ( V_7 -> V_409 ) ;
V_7 -> V_409 = NULL ;
F_23 ( V_7 -> V_411 ) ;
V_7 -> V_411 = NULL ;
}
void F_125 ( struct V_35 * V_36 )
{
int V_1 ;
struct V_6 * V_7 = F_19 ( V_36 ) ;
#ifndef F_122
if ( V_7 -> V_406 ) {
for ( V_1 = 0 ; V_1 < ( V_145 + 1 ) ; V_1 ++ ) {
F_78 ( V_7 -> V_406 [ V_1 ] ) ;
F_23 ( V_7 -> V_406 [ V_1 ] -> V_186 ) ;
F_84 ( V_7 -> V_406 [ V_1 ] ) ;
}
F_23 ( V_7 -> V_406 ) ;
V_7 -> V_406 = NULL ;
}
#else
F_23 ( V_7 -> V_406 ) ;
V_7 -> V_406 = NULL ;
F_23 ( V_7 -> V_409 ) ;
V_7 -> V_409 = NULL ;
F_23 ( V_7 -> V_411 ) ;
V_7 -> V_411 = 0 ;
#endif
}
static void F_126 ( struct V_35 * V_36 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
struct V_10 * V_11 = V_7 -> V_12 ;
if ( V_11 -> V_118 == V_119 ) {
F_108 ( V_36 ) ;
F_127 ( V_36 ) ;
if ( V_392 == V_11 -> V_391 ||
V_393 == V_11 -> V_391 )
F_128 ( V_36 ) ;
}
if ( V_11 -> V_120 == V_121 || V_11 -> V_120 == V_124 ) {
T_2 V_412 = 0 ;
F_17 ( V_36 , V_151 , & V_412 ) ;
if ( V_7 -> V_12 -> V_118 == V_119 )
V_7 -> V_413 = V_412 |= V_162 ;
else
V_7 -> V_413 = V_412 &= ~ V_162 ;
F_14 ( V_36 , V_151 , V_412 ) ;
}
}
static void F_129 ( struct V_414 * V_415 )
{
struct V_6 * V_7 = F_130 ( V_415 , struct V_6 ,
V_416 . V_415 ) ;
struct V_35 * V_36 = V_7 -> V_12 -> V_36 ;
struct V_10 * V_11 = V_7 -> V_12 ;
struct V_58 * V_209 = & V_11 -> V_211 ;
if ( V_11 -> V_251 -> V_417 )
F_131 ( V_11 , V_209 ) ;
V_11 -> V_251 -> V_252 =
V_209 -> V_418 . V_419 ;
F_107 ( V_36 , V_209 -> V_420 ) ;
}
static void F_132 ( struct V_414 * V_415 )
{
struct V_6 * V_7 = F_130 ( V_415 , struct V_6 ,
V_421 ) ;
struct V_35 * V_36 = V_7 -> V_12 -> V_36 ;
struct V_422 * V_423 =
& V_7 -> V_12 -> V_211 . V_424 . V_425 ;
T_1 V_246 = V_7 -> V_12 -> V_211 . V_246 ;
T_2 V_426 ;
T_2 V_427 ;
T_2 V_428 ;
T_2 V_429 ;
T_2 V_430 ;
int V_1 ;
F_133 ( & V_7 -> V_431 ) ;
if ( V_7 -> V_12 -> V_118 != V_119 )
goto V_432;
F_9 ( V_433 ,
L_59 ) ;
for ( V_1 = 0 ; V_1 < V_434 ; V_1 ++ ) {
V_426 = V_423 -> V_435 [ V_1 ] * ( ( V_246 & ( V_247 | V_248 ) ) ? 9 : 20 ) + V_436 ;
V_426 <<= V_437 ;
V_428 = ( T_2 ) F_134 ( V_423 -> V_438 [ V_1 ] ) ;
V_428 <<= V_439 ;
V_429 = ( T_2 ) F_134 ( V_423 -> V_429 [ V_1 ] ) ;
V_429 <<= V_440 ;
V_430 = ( T_2 ) F_134 ( V_423 -> V_430 [ V_1 ] ) ;
V_430 <<= V_441 ;
V_427 = V_428 | V_429 | V_430 | V_426 ;
F_14 ( V_36 , V_442 [ V_1 ] , V_427 ) ;
}
V_432:
F_135 ( & V_7 -> V_431 ) ;
}
static int F_136 ( struct V_6 * V_7 ,
int V_443 ,
struct V_58 * V_444 )
{
int V_189 = 0 ;
T_2 V_445 = sizeof( struct V_422 ) ;
if ( V_7 -> V_12 -> V_118 != V_119 )
return V_189 ;
if ( V_7 -> V_12 -> V_120 != V_121 )
return V_189 ;
if ( V_444 -> V_157 & V_446 ) {
if ( V_443 &&
( V_444 -> V_157 & V_447 ) )
V_444 -> V_424 . V_448 = V_444 -> V_424 . V_449 ;
if ( ( V_444 -> V_424 . V_448 == 1 ) && ( V_443 == 1 ) &&
( V_444 -> V_157 & V_447 ) &&
( V_444 -> V_424 . V_450 !=
V_444 -> V_424 . V_451 ) ) {
V_444 -> V_424 . V_450 =
V_444 -> V_424 . V_451 ;
F_57 ( & V_7 -> V_421 ) ;
F_9 ( V_433 ,
L_60 ) ;
}
} else {
memcpy ( & V_7 -> V_12 -> V_211 . V_424 . V_425 ,
& V_452 , V_445 ) ;
if ( ( V_444 -> V_424 . V_448 == 1 ) && ( V_443 == 1 ) ) {
F_57 ( & V_7 -> V_421 ) ;
F_9 ( V_433 ,
L_61 ) ;
}
V_444 -> V_424 . V_448 = 0 ;
V_444 -> V_424 . V_449 = 0 ;
}
return 0 ;
}
static int F_137 ( struct V_35 * V_36 ,
struct V_453 * V_454 ,
struct V_58 * V_444 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
F_136 ( V_7 , 1 , V_444 ) ;
F_138 ( & V_7 -> V_416 , 0 ) ;
return 0 ;
}
static int F_139 ( struct V_6 * V_7 ,
struct V_58 * V_444 )
{
unsigned long V_157 ;
T_2 V_445 = sizeof( struct V_422 ) ;
int V_455 = 0 ;
if ( ! V_7 || ! V_444 )
return 0 ;
if ( V_7 -> V_12 -> V_118 != V_119 )
return 0 ;
if ( V_7 -> V_12 -> V_120 != V_121 )
return 0 ;
F_95 ( & V_7 -> V_12 -> V_456 , V_157 ) ;
if ( V_444 -> V_157 & V_447 ) {
memcpy ( & V_7 -> V_12 -> V_211 . V_424 . V_425 ,
& V_444 -> V_424 . V_425 ,
sizeof( struct V_422 ) ) ;
V_7 -> V_12 -> V_211 . V_424 . V_448 = 1 ;
V_455 = 1 ;
V_7 -> V_12 -> V_211 . V_424 . V_450 =
V_7 -> V_12 -> V_211 . V_424 . V_451 ;
V_7 -> V_12 -> V_211 . V_424 . V_451 =
V_444 -> V_424 . V_451 ;
} else {
memcpy ( & V_7 -> V_12 -> V_211 . V_424 . V_425 ,
& V_452 , V_445 ) ;
V_7 -> V_12 -> V_211 . V_424 . V_448 = 0 ;
V_7 -> V_12 -> V_211 . V_424 . V_449 = 0 ;
V_455 = 1 ;
}
F_98 ( & V_7 -> V_12 -> V_456 , V_157 ) ;
F_9 ( V_433 , L_62 , V_28 ,
V_444 -> V_157 ,
V_7 -> V_12 -> V_211 . V_424 . V_448 ) ;
if ( V_455 == 1 )
F_57 ( & V_7 -> V_421 ) ;
return 0 ;
}
static int F_140 (
struct V_35 * V_36 ,
struct V_457 * V_458 ,
struct V_58 * V_444 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
F_139 ( V_7 , V_444 ) ;
return 0 ;
}
static void F_127 ( struct V_35 * V_36 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
struct V_10 * V_11 = V_7 -> V_12 ;
T_1 * V_459 = V_11 -> V_460 ;
T_2 V_461 = 0 ;
T_1 V_462 = 0 ;
F_106 ( V_36 , ( T_3 * ) ( & V_461 ) ) ;
V_461 |= ( * ( T_3 * ) ( V_459 ) ) << 12 ;
switch ( V_11 -> V_246 ) {
case V_463 :
V_461 &= 0x00000FF0 ;
break;
case V_464 :
V_461 &= 0x0000000F ;
break;
case V_247 :
V_461 &= 0x00000FF7 ;
break;
case V_248 :
case V_465 :
if ( V_11 -> V_251 -> V_466 == 0 ) {
V_461 &= 0x0007F007 ;
} else {
if ( V_7 -> V_467 == V_468 )
V_461 &= 0x000FF007 ;
else
V_461 &= 0x0F81F007 ;
}
break;
default:
break;
}
V_461 &= 0x0FFFFFFF ;
if ( V_11 -> V_251 -> V_469 && V_11 -> V_251 -> V_470 )
V_461 |= 0x80000000 ;
else if ( ! V_11 -> V_251 -> V_469 &&
V_11 -> V_251 -> V_471 )
V_461 |= 0x80000000 ;
F_14 ( V_36 , V_472 + V_462 * 4 , V_461 ) ;
F_27 ( V_36 , V_473 , 1 ) ;
}
static bool F_141 ( struct V_35 * V_36 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
struct V_10 * V_11 = V_7 -> V_12 ;
struct V_58 * V_444 = & V_11 -> V_211 ;
int V_61 = V_11 -> V_61 ;
struct V_474 * V_475 ;
int V_476 ;
V_475 = V_11 -> V_475 [ V_11 -> V_477 ] ;
V_476 = ( V_444 -> V_420 & V_478 ) ||
( V_11 -> V_479 && V_475 && V_475 -> V_480 &&
( 0 == strcmp ( V_475 -> V_480 -> V_106 , L_63 ) ) ) ;
if ( V_476 && ( V_61 == 0 ) ) {
return false ;
} else if ( ( V_61 != 0 ) ) {
if ( ( ( V_11 -> V_481 [ 0 ] == 0xdd ) && ( ! memcmp ( & ( V_11 -> V_481 [ 14 ] ) , V_482 , 4 ) ) ) || ( ( V_11 -> V_481 [ 0 ] == 0x30 ) && ( ! memcmp ( & V_11 -> V_481 [ 10 ] , V_483 , 4 ) ) ) )
return true ;
else
return false ;
} else {
return true ;
}
return true ;
}
static bool F_142 ( struct V_35 * V_36 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
return V_7 -> V_12 -> V_484 ;
}
static void F_143 ( struct V_6 * V_7 )
{
struct V_10 * V_11 = V_7 -> V_12 ;
if ( V_11 -> V_246 == V_485 ||
V_11 -> V_246 == V_486 )
memcpy ( V_11 -> V_487 ,
V_11 -> V_488 , 16 ) ;
else
memset ( V_11 -> V_487 , 0 , 16 ) ;
}
static T_1 F_144 ( struct V_35 * V_36 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
T_1 V_189 = 0 ;
switch ( V_7 -> V_24 ) {
case V_25 :
case V_26 :
case V_489 :
V_189 = V_485 | V_490 | V_491 ;
break;
case V_492 :
V_189 = V_493 | V_486 ;
break;
default:
V_189 = V_491 ;
break;
}
return V_189 ;
}
static void F_145 ( struct V_35 * V_36 , T_1 V_494 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
T_1 V_495 = F_144 ( V_36 ) ;
if ( V_494 == V_496 ||
( V_494 & V_495 ) == 0 ) {
if ( V_495 & V_485 ) {
V_494 = V_485 ;
} else if ( V_495 & V_486 ) {
V_494 = V_486 ;
} else if ( ( V_495 & V_493 ) ) {
V_494 = V_493 ;
} else if ( ( V_495 & V_490 ) ) {
V_494 = V_490 ;
} else if ( ( V_495 & V_491 ) ) {
V_494 = V_491 ;
} else {
F_9 ( V_27 ,
L_64 ,
V_28 , V_495 ) ;
V_494 = V_491 ;
}
}
#ifdef F_146
F_147 (
V_497 , V_498 -> V_499 ,
& V_497 -> V_500 . V_501 . V_502 ) ;
#endif
V_7 -> V_12 -> V_246 = V_494 ;
if ( V_494 == V_485 ||
V_494 == V_486 )
V_7 -> V_12 -> V_251 -> V_503 = 1 ;
else
V_7 -> V_12 -> V_251 -> V_503 = 0 ;
F_9 ( V_95 , L_65 , V_494 ) ;
F_143 ( V_7 ) ;
}
static void F_148 ( struct V_35 * V_36 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
T_1 V_1 ;
V_7 -> V_504 = V_505 ;
V_7 -> V_131 = 1 ;
V_7 -> V_12 -> V_246 = V_496 ;
V_7 -> V_12 -> V_120 = V_121 ;
V_7 -> V_12 -> V_506 = 0 ;
V_7 -> V_507 = V_508 ;
V_7 -> V_509 = V_510 ;
V_7 -> V_12 -> V_511 = V_512 ;
V_7 -> V_12 -> V_178 = 110 ;
V_7 -> V_12 -> V_513 = 1 ;
V_7 -> V_514 = ( V_36 -> V_157 & V_158 ) ? 1 : 0 ;
V_7 -> V_515 = 6 ;
V_7 -> V_516 = 1 ;
V_7 -> V_517 = V_518 ;
V_7 -> V_519 = false ;
V_7 -> V_520 = false ;
V_7 -> V_521 = false ;
V_7 -> V_12 -> V_522 = 0 ;
V_7 -> V_12 -> V_211 . V_264 =
V_523 ;
V_7 -> V_12 -> V_524 = V_525 |
V_526 | V_527 |
V_528 | V_529 |
V_530 ;
V_7 -> V_12 -> V_531 = 1 ;
V_7 -> V_12 -> V_532 =
V_533 | V_534 ;
V_7 -> V_12 -> V_479 = 1 ;
V_7 -> V_12 -> V_535 = 1 ;
V_7 -> V_12 -> V_536 = NULL ;
V_7 -> V_12 -> V_537 = NULL ;
V_7 -> V_12 -> V_207 = F_99 ;
V_7 -> V_12 -> V_538 = F_59 ;
V_7 -> V_12 -> V_539 = F_126 ;
V_7 -> V_12 -> V_540 = F_93 ;
V_7 -> V_12 -> V_541 = F_91 ;
V_7 -> V_12 -> V_542 = F_92 ;
V_7 -> V_12 -> V_543 = 0 ;
V_7 -> V_12 -> V_544 = V_545 ;
V_7 -> V_12 -> F_55 = F_55 ;
V_7 -> V_12 -> V_195 = V_384 ;
V_7 -> V_12 -> V_546 = 1 ;
V_7 -> V_12 -> V_547 = V_548 ;
V_7 -> V_12 -> V_549 = F_140 ;
V_7 -> V_12 -> V_550 = F_137 ;
V_7 -> V_12 -> V_551 = F_141 ;
V_7 -> V_12 -> V_552 =
F_142 ;
V_7 -> V_12 -> V_553 = F_145 ;
V_7 -> V_12 -> V_554 = V_555 ;
V_7 -> V_556 = V_557 ;
#ifdef F_146
if ( V_558 -> V_559 ) {
V_498 -> V_560 = 7 ;
V_498 -> V_561 = 7 ;
}
#endif
V_7 -> V_560 = 0x30 ;
V_7 -> V_561 = 0x30 ;
V_7 -> V_562 = 7 ;
V_7 -> V_563 = 0 ;
V_7 -> V_564 =
( V_565 << V_566 ) |
( V_7 -> V_560 << V_567 ) |
( V_7 -> V_561 << V_568 ) |
( false ? V_569 : 0 ) ;
#ifdef F_146
if ( V_558 -> V_559 )
V_498 -> V_413 =
V_498 -> V_570 |
V_153 | V_154 |
V_571 |
V_155 | V_156 | V_161 |
V_163 | V_166 |
( ( T_2 ) 7 << V_171 ) |
( V_498 -> V_562 << V_572 ) |
( V_498 -> V_562 == 7 ? V_573 : 0 ) ;
else
#endif
V_7 -> V_413 =
V_153 | V_154 |
V_571 |
V_155 | V_156 | V_161 |
( ( T_2 ) 7 << V_171 ) |
( V_7 -> V_562 << V_169 ) |
( V_7 -> V_562 == 7 ? V_172 : 0 ) ;
V_7 -> V_574 = 0 ;
V_7 -> V_575 = F_20 ( sizeof( V_576 ) , V_47 ) ;
F_149 ( & V_7 -> V_144 ) ;
F_149 ( & V_7 -> V_177 ) ;
for ( V_1 = 0 ; V_1 < V_577 ; V_1 ++ )
F_149 ( & V_7 -> V_12 -> V_205 [ V_1 ] ) ;
for ( V_1 = 0 ; V_1 < V_577 ; V_1 ++ )
F_149 ( & V_7 -> V_12 -> V_578 [ V_1 ] ) ;
for ( V_1 = 0 ; V_1 < V_577 ; V_1 ++ )
F_149 ( & V_7 -> V_12 -> V_579 [ V_1 ] ) ;
V_7 -> V_132 = V_580 ;
}
static void F_150 ( struct V_6 * V_7 )
{
F_151 ( & V_7 -> V_193 ) ;
F_151 ( & V_7 -> V_581 ) ;
F_152 ( & V_7 -> V_582 , 1 ) ;
F_152 ( & V_7 -> V_583 , 1 ) ;
F_153 ( & V_7 -> V_431 ) ;
}
static void F_154 ( struct V_35 * V_36 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
F_155 ( & V_7 -> V_114 , V_584 ) ;
F_156 ( & V_7 -> V_585 ,
V_586 ) ;
F_156 ( & V_7 -> V_587 ,
V_588 ) ;
F_156 ( & V_7 -> V_589 ,
V_590 ) ;
F_156 ( & V_7 -> V_416 ,
F_129 ) ;
F_156 ( & V_7 -> V_591 ,
V_592 ) ;
F_155 ( & V_7 -> V_421 , F_132 ) ;
F_157 ( & V_7 -> V_185 ,
( void (*) ( unsigned long ) ) V_593 ,
( unsigned long ) V_7 ) ;
}
static void F_158 ( struct V_35 * V_36 )
{
T_3 V_594 = 0 ;
struct V_6 * V_7 = F_19 ( V_36 ) ;
F_9 ( V_595 , L_66 , V_28 ) ;
F_31 ( V_36 , V_596 , & V_594 ) ;
F_9 ( V_595 ,
L_67 , V_596 , V_594 ) ;
V_7 -> V_597 =
( V_594 & V_598 ) ? V_599 : V_600 ;
F_9 ( V_595 ,
L_68 , V_28 , V_7 -> V_597 ) ;
}
static inline T_3 F_159 ( T_3 * V_40 )
{
T_3 V_241 = * V_40 ;
* V_40 = ( V_241 >> 8 ) | ( V_241 << 8 ) ;
return * V_40 ;
}
static void F_160 ( struct V_35 * V_36 )
{
T_3 V_601 = 0 ;
T_1 V_602 [ 6 ] = { 0x00 , 0xe0 , 0x4c , 0x00 , 0x00 , 0x02 } ;
T_1 V_603 = false ;
struct V_6 * V_7 = F_19 ( V_36 ) ;
T_3 V_604 = 0 ;
int V_1 ;
F_9 ( V_595 , L_66 , V_28 ) ;
V_601 = F_161 ( V_36 , 0 ) ;
F_9 ( V_595 , L_69 , V_601 ) ;
if ( V_601 != V_605 )
F_9 ( V_27 ,
L_70 ,
V_601 , V_605 ) ;
else
V_603 = true ;
if ( V_603 ) {
V_604 = F_161 ( V_36 , V_606 >> 1 ) ;
V_7 -> V_607 = F_159 ( & V_604 ) ;
V_7 -> V_608 = F_161 ( V_36 , V_609 >> 1 ) ;
V_604 = F_161 ( V_36 , V_610 >> 1 ) ;
V_7 -> V_611 = ( V_604 & 0xff00 ) >> 8 ;
V_7 -> V_612 = true ;
V_7 -> V_613 =
F_161 ( V_36 , ( V_614 >> 1 ) ) >> 8 ;
} else {
V_7 -> V_607 = 0 ;
V_7 -> V_608 = 0 ;
V_7 -> V_615 = V_616 ;
V_7 -> V_611 = 0 ;
V_7 -> V_613 = 0 ;
}
F_9 ( V_595 ,
L_71 ,
V_7 -> V_607 , V_7 -> V_608 , V_7 -> V_613 ,
V_7 -> V_611 ) ;
V_7 -> V_29 = V_7 -> V_611 ;
if ( V_603 ) {
int V_1 ;
for ( V_1 = 0 ; V_1 < 6 ; V_1 += 2 ) {
T_3 V_241 = 0 ;
V_241 = F_161 ( V_36 , ( T_3 ) ( ( V_617 + V_1 ) >> 1 ) ) ;
* ( T_3 * ) ( & V_36 -> V_618 [ V_1 ] ) = V_241 ;
}
} else {
memcpy ( V_36 -> V_618 , V_602 , 6 ) ;
}
F_9 ( V_595 , L_72 , V_36 -> V_618 ) ;
V_7 -> V_467 = V_619 ;
V_7 -> V_24 = V_26 ;
if ( V_7 -> V_615 == ( T_1 ) V_620 ) {
if ( V_603 )
V_7 -> V_621 = ( F_161 ( V_36 , ( V_622 >> 1 ) ) & 0xff00 ) >> 8 ;
else
V_7 -> V_621 = V_623 ;
F_9 ( V_595 , L_73 , V_7 -> V_621 ) ;
if ( V_603 )
V_7 -> V_624 = ( T_1 ) ( F_161 ( V_36 , ( V_625 >> 1 ) ) & 0x00ff ) ;
else
V_7 -> V_624 = V_626 ;
F_9 ( V_595 , L_74 , V_7 -> V_624 ) ;
V_7 -> V_627 = V_7 -> V_624 * 100 ;
if ( V_603 )
V_7 -> V_628 = ( F_161 ( V_36 , ( V_629 >> 1 ) ) & 0x0f00 ) >> 8 ;
else
V_7 -> V_628 = V_630 ;
F_9 ( V_595 , L_75 , V_7 -> V_628 ) ;
if ( V_603 )
V_7 -> V_631 = ( F_161 ( V_36 , ( V_632 >> 1 ) ) & 0x0f ) ;
else
V_7 -> V_631 = V_633 ;
F_9 ( V_595 , L_76 , V_7 -> V_631 ) ;
if ( V_603 )
V_7 -> V_634 = ( F_161 ( V_36 , ( V_635 >> 1 ) ) & 0xff00 ) >> 8 ;
else
V_7 -> V_634 = 1 ;
F_9 ( V_595 , L_77 , V_7 -> V_634 ) ;
if ( V_7 -> V_634 == 0 ) {
int V_1 ;
if ( V_603 )
V_7 -> V_636 = ( F_161 ( V_36 , ( V_637 >> 1 ) ) & 0xff ) >> 8 ;
else
V_7 -> V_636 = 0x10 ;
F_9 ( V_595 , L_78 , V_7 -> V_636 ) ;
for ( V_1 = 0 ; V_1 < 3 ; V_1 ++ ) {
if ( V_603 ) {
V_604 = F_161 ( V_36 , ( V_638 + V_1 ) >> 1 ) ;
if ( ( ( V_638 + V_1 ) % 2 ) == 0 )
V_604 = V_604 & 0x00ff ;
else
V_604 = ( V_604 & 0xff00 ) >> 8 ;
} else {
V_604 = 0x10 ;
}
V_7 -> V_639 [ V_1 ] = ( T_1 ) V_604 ;
F_9 ( V_595 , L_79 , V_1 , V_7 -> V_636 ) ;
}
} else if ( V_7 -> V_634 == 1 ) {
if ( V_603 ) {
V_604 = F_161 ( V_36 ,
V_640 >> 1 ) ;
V_604 = ( V_604 & 0xff00 ) >> 8 ;
} else {
V_604 = 0x10 ;
}
V_7 -> V_641 [ 0 ] = ( T_1 ) V_604 ;
if ( V_603 )
V_604 = F_161 ( V_36 , ( V_640 + 2 ) >> 1 ) ;
else
V_604 = 0x1010 ;
* ( ( T_3 * ) ( & V_7 -> V_641 [ 1 ] ) ) = V_604 ;
if ( V_603 )
V_604 = F_161 ( V_36 ,
V_642 >> 1 ) ;
else
V_604 = 0x1010 ;
* ( ( T_3 * ) ( & V_7 -> V_639 [ 0 ] ) ) = V_604 ;
if ( V_603 )
V_604 = F_161 ( V_36 , ( V_642 + 2 ) >> 1 ) ;
else
V_604 = 0x10 ;
V_7 -> V_639 [ 2 ] = ( T_1 ) V_604 ;
}
for ( V_1 = 0 ; V_1 < 14 ; V_1 ++ ) {
if ( V_1 <= 3 )
V_7 -> V_643 [ V_1 ] = V_7 -> V_639 [ 0 ] ;
else if ( V_1 >= 4 && V_1 <= 9 )
V_7 -> V_643 [ V_1 ] = V_7 -> V_639 [ 1 ] ;
else
V_7 -> V_643 [ V_1 ] = V_7 -> V_639 [ 2 ] ;
}
for ( V_1 = 0 ; V_1 < 14 ; V_1 ++ ) {
if ( V_7 -> V_634 == 0 ) {
if ( V_1 <= 3 )
V_7 -> V_644 [ V_1 ] = V_7 -> V_639 [ 0 ] + ( V_7 -> V_636 - V_7 -> V_639 [ 1 ] ) ;
else if ( V_1 >= 4 && V_1 <= 9 )
V_7 -> V_644 [ V_1 ] = V_7 -> V_636 ;
else
V_7 -> V_644 [ V_1 ] = V_7 -> V_639 [ 2 ] + ( V_7 -> V_636 - V_7 -> V_639 [ 1 ] ) ;
} else if ( V_7 -> V_634 == 1 ) {
if ( V_1 <= 3 )
V_7 -> V_644 [ V_1 ] = V_7 -> V_641 [ 0 ] ;
else if ( V_1 >= 4 && V_1 <= 9 )
V_7 -> V_644 [ V_1 ] = V_7 -> V_641 [ 1 ] ;
else
V_7 -> V_644 [ V_1 ] = V_7 -> V_641 [ 2 ] ;
}
}
V_7 -> V_645 = V_7 -> V_628 ;
V_7 -> V_646 [ 0 ] = ( V_7 -> V_621 & 0xf ) ;
V_7 -> V_646 [ 1 ] =
( V_7 -> V_621 & 0xf0 ) >> 4 ;
V_7 -> V_647 = V_7 -> V_631 ;
V_7 -> V_648 [ 0 ] = V_7 -> V_624 ;
}
switch ( V_7 -> V_613 ) {
case V_649 :
V_7 -> V_650 = V_651 ;
break;
case V_652 :
V_7 -> V_650 = V_653 ;
break;
default:
V_7 -> V_650 = V_654 ;
break;
}
switch ( V_7 -> V_650 ) {
case V_651 :
V_7 -> V_655 = V_656 ;
break;
case V_653 :
V_7 -> V_655 = V_657 ;
break;
default:
V_7 -> V_655 = V_658 ;
break;
}
if ( V_7 -> V_467 == V_468 )
F_9 ( V_595 , L_80 ) ;
else
F_9 ( V_595 , L_81 ) ;
F_162 ( V_36 ) ;
F_9 ( V_595 , L_82 , V_28 ) ;
}
static short F_163 ( struct V_35 * V_36 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
if ( V_7 -> V_29 > V_33 ) {
F_24 ( V_36 ,
L_83 ) ;
V_7 -> V_29 = 0 ;
}
F_9 ( V_95 , L_84 , V_7 -> V_29 ) ;
F_7 ( V_7 -> V_29 , V_7 ) ;
return 0 ;
}
static short F_164 ( struct V_35 * V_36 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
memset ( & ( V_7 -> V_67 ) , 0 , sizeof( struct V_659 ) ) ;
memset ( V_7 -> V_660 , 0 , 9 ) ;
#ifdef F_165
{
int V_1 = 0 ;
T_1 V_661 [] = { 3 , 2 , 1 , 0 , 4 , 8 , 7 , 6 , 5 } ;
memcpy ( V_7 -> V_660 , V_661 , 9 ) ;
}
#else
{
T_1 V_661 [] = { 3 , 2 , 1 , 0 , 4 , 4 , 0 , 4 , 4 } ;
memcpy ( V_7 -> V_660 , V_661 , 9 ) ;
}
#endif
F_148 ( V_36 ) ;
F_150 ( V_7 ) ;
F_154 ( V_36 ) ;
F_158 ( V_36 ) ;
F_160 ( V_36 ) ;
F_163 ( V_36 ) ;
F_166 ( V_36 ) ;
F_167 ( & V_7 -> V_662 , V_663 ,
( unsigned long ) V_36 ) ;
if ( F_120 ( V_36 ) != 0 ) {
F_73 ( L_85 ) ;
return - V_51 ;
}
return 0 ;
}
static void F_168 ( struct V_35 * V_36 )
{
T_2 V_664 = 0 , V_665 = 0 ;
T_1 V_666 = 0 , V_667 = 0 ;
struct V_6 * V_7 = F_19 ( V_36 ) ;
T_2 V_461 = 0 ;
switch ( V_7 -> V_12 -> V_246 ) {
case V_491 :
V_666 = V_668 ;
V_664 = V_669 ;
V_665 = V_669 ;
break;
case V_493 :
V_666 = V_670 | V_668 ;
V_664 = V_671 ;
V_665 = V_671 ;
break;
case V_490 :
V_666 = V_668 ;
V_664 = V_669 | V_671 ;
V_665 = V_669 | V_671 ;
break;
case V_496 :
#ifdef F_146
if ( V_558 -> V_559 ) {
V_666 = V_668 ;
V_664 = V_669 | V_671 ;
V_665 = V_669 | V_671 ;
} else
#endif
{
V_666 = V_668 ;
V_664 = V_669 | V_671 |
V_672 | V_673 ;
V_665 = V_669 | V_671 ;
}
break;
case V_485 :
V_666 = V_668 ;
V_664 = V_669 | V_671 |
V_672 | V_673 ;
V_665 = V_669 | V_671 ;
break;
case V_486 :
V_666 = V_670 ;
V_664 = V_671 | V_672 |
V_673 ;
V_665 = V_671 ;
break;
}
F_27 ( V_36 , V_674 , V_666 ) ;
V_461 = V_664 ;
if ( V_7 -> V_467 == V_468 )
V_461 &= ~ ( V_673 ) ;
F_14 ( V_36 , V_472 , V_461 ) ;
F_27 ( V_36 , V_473 , 1 ) ;
F_29 ( V_36 , 0x313 , & V_667 ) ;
V_665 = ( ( V_667 ) << 24 ) | ( V_665 & 0x00ffffff ) ;
F_14 ( V_36 , V_245 , V_665 ) ;
F_28 ( V_36 , V_675 ,
V_7 -> V_560 << V_676 |
V_7 -> V_561 << V_677 ) ;
}
static bool F_169 ( struct V_35 * V_36 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
T_2 V_678 = 0 ;
bool V_679 = true ;
T_1 V_680 = 0x0 ;
T_1 V_241 ;
F_9 ( V_95 , L_86 , V_28 ) ;
V_7 -> V_681 = V_682 ;
F_18 ( V_36 , 0x5f , 0x80 ) ;
F_60 ( 50 ) ;
F_18 ( V_36 , 0x5f , 0xf0 ) ;
F_18 ( V_36 , 0x5d , 0x00 ) ;
F_18 ( V_36 , 0x5e , 0x80 ) ;
F_27 ( V_36 , 0x17 , 0x37 ) ;
F_60 ( 10 ) ;
V_7 -> V_575 -> V_683 = V_684 ;
F_17 ( V_36 , V_685 , & V_678 ) ;
if ( V_7 -> V_575 -> V_683 == V_684 )
V_678 |= V_686 ;
else if ( V_7 -> V_575 -> V_683 == V_687 )
V_678 |= V_688 ;
else
F_9 ( V_27 ,
L_87 ,
V_28 , V_7 -> V_575 -> V_683 ) ;
F_14 ( V_36 , V_685 , V_678 ) ;
F_170 ( V_36 ) ;
V_7 -> V_689 = V_690 ;
F_17 ( V_36 , V_685 , & V_678 ) ;
if ( V_7 -> V_689 == V_690 )
V_678 = ( V_678 & V_691 ) |
V_692 ;
else if ( V_7 -> V_689 == V_693 )
V_678 |= V_694 ;
else
F_9 ( V_27 ,
L_88 ,
V_28 , V_7 -> V_689 ) ;
F_14 ( V_36 , V_685 , V_678 ) ;
F_171 ( 500 ) ;
F_25 ( V_36 , 0x5f , & V_241 ) ;
F_18 ( V_36 , 0x5f , V_241 | 0x20 ) ;
F_168 ( V_36 ) ;
F_27 ( V_36 , V_174 , V_176 | V_175 ) ;
F_14 ( V_36 , V_695 , ( ( T_2 * ) V_36 -> V_618 ) [ 0 ] ) ;
F_28 ( V_36 , V_696 , ( ( T_3 * ) ( V_36 -> V_618 + 4 ) ) [ 0 ] ) ;
F_14 ( V_36 , V_151 , V_7 -> V_413 ) ;
F_14 ( V_36 , V_697 ,
V_698 << V_699 |
V_700 << V_701 |
V_702 << V_703 |
V_704 << V_705 ) ;
F_14 ( V_36 , V_706 ,
V_707 << V_708 |
V_709 << V_710 ) ;
F_14 ( V_36 , V_711 ,
V_712 |
V_713 << V_714 ) ;
F_14 ( V_36 , V_472 + 4 * 7 , ( V_671 | V_669 ) ) ;
F_27 ( V_36 , V_715 , 0x30 ) ;
if ( V_7 -> V_517 == V_518 )
F_145 ( V_36 , V_7 -> V_12 -> V_246 ) ;
if ( V_7 -> V_517 == V_518 ) {
F_12 ( V_36 ) ;
V_680 |= V_716 ;
V_680 |= V_717 ;
V_680 |= V_718 ;
F_27 ( V_36 , V_719 , V_680 ) ;
}
F_28 ( V_36 , V_261 , 2 ) ;
F_28 ( V_36 , V_263 , 100 ) ;
#define F_172 0x005e4332
{
int V_1 ;
for ( V_1 = 0 ; V_1 < V_434 ; V_1 ++ )
F_14 ( V_36 , V_442 [ V_1 ] , F_172 ) ;
}
F_173 ( V_36 ) ;
if ( V_7 -> V_615 == ( T_1 ) V_620 ) {
F_174 ( V_36 ) ;
F_175 ( V_36 , V_7 -> V_131 ) ;
}
V_679 = F_176 ( V_36 ) ;
if ( ! V_679 ) {
F_9 ( V_27 , L_89 ,
V_28 ) ;
return V_679 ;
}
F_9 ( V_95 , L_90 , V_28 ) ;
#ifdef F_146
if ( V_558 -> V_517 == V_518 ) {
if ( V_720 -> V_721 ) {
F_9 ( ( V_95 | V_722 ) , V_723 ,
( L_91 ) ) ;
F_177 ( V_558 , V_724 , V_725 ) ;
for ( V_726 = 0 ; V_726 < V_498 -> V_727 ; V_726 ++ )
F_178 ( V_558 ,
( V_728 ) V_726 ,
0x4 , 0xC00 , 0x0 ) ;
} else if ( V_720 -> V_729 > V_730 ) {
F_9 ( ( V_95 | V_722 ) , V_723 ,
( L_92 ,
V_720 -> V_729 ) ) ;
F_177 ( V_558 ,
V_724 ,
V_720 -> V_729 ) ;
} else {
V_498 -> V_731 = V_732 ;
V_720 -> V_729 = 0 ;
F_9 ( ( V_95 | V_722 ) , V_723 ,
( L_93 ) ) ;
}
} else {
if ( V_498 -> V_731 == V_724 ) {
F_177 ( V_558 ,
V_724 ,
V_720 -> V_729 ) ;
for ( V_726 = 0 ; V_726 < V_498 -> V_727 ; V_726 ++ )
F_178 ( V_558 ,
( V_728 ) V_726 ,
0x4 , 0xC00 , 0x0 ) ;
}
}
#endif
if ( V_7 -> V_517 == V_518 ) {
F_179 ( V_36 ) ;
F_9 ( V_95 , L_94 , V_28 ) ;
}
if ( V_7 -> V_12 -> V_522 )
V_7 -> V_681 = V_733 ;
else
V_7 -> V_681 = V_682 ;
F_180 ( V_36 ) ;
F_181 ( V_36 , V_734 , V_735 , 0x1 ) ;
F_181 ( V_36 , V_734 , V_736 , 0x1 ) ;
if ( V_7 -> V_517 == V_518 ) {
T_1 V_737 ;
F_29 ( V_36 , 0x301 , & V_737 ) ;
if ( V_737 == 0x03 ) {
V_7 -> V_738 = true ;
F_9 ( V_739 , L_95 ) ;
} else {
V_7 -> V_738 = false ;
F_9 ( V_739 , L_96 ) ;
}
F_182 ( V_36 ) ;
if ( V_7 -> V_738 ) {
T_2 V_1 , V_740 ;
T_2 V_741 = F_183 ( V_36 ,
V_742 ,
V_743 ) ;
for ( V_1 = 0 ; V_1 < V_744 ; V_1 ++ ) {
if ( V_741 == V_7 -> V_745 [ V_1 ] . V_746 ) {
V_7 -> V_747 = ( T_1 ) V_1 ;
V_7 -> V_748 =
( T_1 ) V_1 ;
V_7 -> V_749 =
V_7 -> V_747 ;
break;
}
}
V_740 = F_183 ( V_36 ,
V_750 ,
V_751 ) ;
for ( V_1 = 0 ; V_1 < V_752 ; V_1 ++ ) {
if ( V_740 == V_7 -> V_753 [ V_1 ] . V_754 [ 0 ] ) {
V_7 -> V_755 = ( T_1 ) V_1 ;
break;
}
}
V_7 -> V_756 = 0 ;
V_7 -> V_757 = 0 ;
V_7 -> V_758 =
V_7 -> V_755 ;
}
}
F_27 ( V_36 , 0x87 , 0x0 ) ;
return V_679 ;
}
static struct V_759 * F_184 ( struct V_35 * V_36 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
return & V_7 -> V_12 -> V_67 ;
}
static bool F_185 ( struct V_35 * V_36 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
T_3 V_760 ;
bool V_761 = false ;
F_30 ( V_36 , 0x128 , & V_760 ) ;
F_9 ( V_762 ,
L_97 , V_28 ,
V_760 , V_7 -> V_763 ) ;
if ( V_7 -> V_763 == V_760 )
V_761 = true ;
V_7 -> V_763 = V_760 ;
return V_761 ;
}
static T_8 F_186 ( struct V_35 * V_36 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
T_1 V_282 ;
bool V_764 = false ;
for ( V_282 = 0 ; V_282 <= V_204 ; V_282 ++ ) {
if ( V_282 == V_192 )
continue;
if ( ( F_63 ( & V_7 -> V_12 -> V_205 [ V_282 ] ) == 0 ) && ( F_63 ( & V_7 -> V_12 -> V_578 [ V_282 ] ) == 0 ) )
continue;
V_764 = true ;
}
if ( V_764 ) {
if ( F_185 ( V_36 ) ) {
F_9 ( V_762 ,
L_98 ) ;
return V_765 ;
}
}
return V_518 ;
}
static bool F_187 ( struct V_35 * V_36 )
{
T_3 V_766 ;
struct V_6 * V_7 = F_19 ( V_36 ) ;
bool V_761 = false ;
static T_1 V_767 ;
F_30 ( V_36 , 0x130 , & V_766 ) ;
F_9 ( V_762 ,
L_99 , V_28 ,
V_766 , V_7 -> V_768 ) ;
V_767 ++ ;
if ( V_7 -> V_769 >= ( V_770 + 5 ) ) {
V_767 = 0 ;
} else if ( V_7 -> V_769 < ( V_770 + 5 ) &&
( ( V_7 -> V_376 != V_771 && V_7 -> V_769 >= V_772 ) ||
( V_7 -> V_376 == V_771 && V_7 -> V_769 >= V_773 ) ) ) {
if ( V_767 < 2 )
return V_761 ;
V_767 = 0 ;
} else if ( ( ( V_7 -> V_376 != V_771 && V_7 -> V_769 < V_772 ) ||
( V_7 -> V_376 == V_771 && V_7 -> V_769 < V_773 ) ) &&
V_7 -> V_769 >= V_774 ) {
if ( V_767 < 4 )
return V_761 ;
V_767 = 0 ;
} else {
if ( V_767 < 8 )
return V_761 ;
V_767 = 0 ;
}
if ( V_7 -> V_768 == V_766 )
V_761 = true ;
V_7 -> V_768 = V_766 ;
return V_761 ;
}
static T_8 F_188 ( struct V_35 * V_36 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
bool V_775 = false ;
if ( V_7 -> V_516 > 1 )
V_775 = true ;
if ( V_775 ) {
if ( F_187 ( V_36 ) ) {
F_9 ( V_762 , L_100 ) ;
return V_765 ;
}
}
return V_518 ;
}
static T_8 F_189 ( struct V_35 * V_36 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
T_8 V_776 = V_518 ;
T_8 V_777 = V_518 ;
T_9 V_778 ;
V_778 = V_7 -> V_12 -> V_731 ;
V_776 = F_186 ( V_36 ) ;
if ( V_778 != V_724 ||
( V_7 -> V_12 -> V_120 != V_124 ) ) {
V_777 = F_188 ( V_36 ) ;
}
if ( V_776 == V_779 ||
V_777 == V_779 ) {
return V_779 ;
} else if ( V_776 == V_765 ||
V_777 == V_765 ) {
F_9 ( V_762 , L_101 , V_28 ) ;
return V_765 ;
} else {
return V_518 ;
}
}
static void F_190 ( struct V_35 * V_36 )
{
T_1 V_780 = 0 ;
struct V_6 * V_7 = F_19 ( V_36 ) ;
T_1 * V_781 = V_7 -> V_12 -> V_211 . V_260 ;
static T_1 V_782 [ 4 ] [ 6 ] = {
{ 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 } ,
{ 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x01 } ,
{ 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x02 } ,
{ 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x03 } } ;
static T_1 V_783 [] = {
0xff , 0xff , 0xff , 0xff , 0xff , 0xff } ;
F_9 ( V_784 , L_102 ) ;
if ( ( V_7 -> V_12 -> V_391 == V_392 ) ||
( V_7 -> V_12 -> V_391 == V_393 ) ) {
for ( V_780 = 0 ; V_780 < 4 ; V_780 ++ ) {
V_781 = V_782 [ V_780 ] ;
F_191 ( V_36 , V_780 , V_780 ,
V_7 -> V_12 -> V_391 ,
V_781 , 0 , NULL ) ;
}
} else if ( V_7 -> V_12 -> V_391 == V_394 ) {
if ( V_7 -> V_12 -> V_120 == V_124 )
F_191 ( V_36 , 4 , 0 , V_7 -> V_12 -> V_391 ,
( T_1 * ) V_36 -> V_618 , 0 , NULL ) ;
else
F_191 ( V_36 , 4 , 0 , V_7 -> V_12 -> V_391 ,
V_781 , 0 , NULL ) ;
} else if ( V_7 -> V_12 -> V_391 == V_395 ) {
if ( V_7 -> V_12 -> V_120 == V_124 )
F_191 ( V_36 , 4 , 0 , V_7 -> V_12 -> V_391 ,
( T_1 * ) V_36 -> V_618 , 0 , NULL ) ;
else
F_191 ( V_36 , 4 , 0 , V_7 -> V_12 -> V_391 ,
V_781 , 0 , NULL ) ;
}
if ( V_7 -> V_12 -> V_785 == V_394 ) {
V_781 = V_783 ;
for ( V_780 = 1 ; V_780 < 4 ; V_780 ++ ) {
F_191 ( V_36 , V_780 , V_780 ,
V_7 -> V_12 -> V_785 ,
V_781 , 0 , NULL ) ;
}
if ( V_7 -> V_12 -> V_120 == V_124 )
F_191 ( V_36 , 0 , 0 , V_7 -> V_12 -> V_785 ,
V_782 [ 0 ] , 0 , NULL ) ;
} else if ( V_7 -> V_12 -> V_785 == V_395 ) {
V_781 = V_783 ;
for ( V_780 = 1 ; V_780 < 4 ; V_780 ++ ) {
F_191 ( V_36 , V_780 , V_780 ,
V_7 -> V_12 -> V_785 ,
V_781 , 0 , NULL ) ;
}
if ( V_7 -> V_12 -> V_120 == V_124 )
F_191 ( V_36 , 0 , 0 , V_7 -> V_12 -> V_785 ,
V_782 [ 0 ] , 0 , NULL ) ;
}
}
static void F_192 ( struct V_35 * V_36 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
T_1 V_786 = 0 ;
int V_787 = 0 ;
struct V_10 * V_11 = V_7 -> V_12 ;
if ( V_7 -> V_517 == V_518 ) {
V_788:
F_9 ( V_762 , L_103 ) ;
V_7 -> V_517 = V_765 ;
F_193 ( & V_7 -> V_582 ) ;
if ( V_7 -> V_182 == 0 ) {
F_9 ( V_27 ,
L_104 ,
V_28 ) ;
V_182 ( & V_7 -> V_582 ) ;
return;
}
V_7 -> V_182 = 0 ;
F_9 ( V_762 ,
L_105 ,
V_28 ) ;
F_76 ( V_36 ) ;
F_194 ( V_7 ) ;
F_195 ( V_36 ) ;
F_196 ( & V_7 -> V_662 ) ;
V_11 -> V_789 = 1 ;
if ( V_11 -> V_118 == V_119 ) {
F_193 ( & V_11 -> V_582 ) ;
F_119 ( V_36 , L_106 ) ;
F_197 ( V_7 -> V_12 ) ;
F_196 ( & V_11 -> V_790 ) ;
F_198 ( & V_11 -> V_791 ) ;
F_199 ( V_11 ) ;
F_200 ( V_36 ) ;
V_182 ( & V_11 -> V_582 ) ;
} else {
F_119 ( V_36 , L_107 ) ;
F_201 ( V_7 -> V_12 ) ;
}
V_182 ( & V_7 -> V_582 ) ;
F_9 ( V_762 ,
L_108 ,
V_28 ) ;
F_9 ( V_762 ,
L_109 ,
V_28 ) ;
V_787 = F_202 ( V_36 ) ;
F_9 ( V_762 ,
L_110 ,
V_28 ) ;
if ( V_787 == - V_792 ) {
if ( V_786 < 3 ) {
V_786 ++ ;
goto V_788;
} else {
F_9 ( V_27 ,
L_111 ,
V_28 ) ;
}
}
V_11 -> V_793 = 1 ;
F_128 ( V_36 ) ;
if ( V_11 -> V_118 == V_119 &&
V_11 -> V_120 == V_121 ) {
V_11 -> V_538 ( V_11 -> V_36 ,
V_11 -> V_211 . V_794 ) ;
F_203 ( V_11 -> V_795 , & V_11 -> V_796 ) ;
} else if ( V_11 -> V_118 == V_119 &&
V_11 -> V_120 == V_124 ) {
V_11 -> V_538 ( V_11 -> V_36 ,
V_11 -> V_211 . V_794 ) ;
V_11 -> V_539 ( V_11 -> V_36 ) ;
F_204 ( V_11 ) ;
if ( V_11 -> V_542 )
V_11 -> V_542 ( V_11 -> V_36 ) ;
F_205 ( V_11 -> V_36 ) ;
}
F_190 ( V_36 ) ;
V_7 -> V_517 = V_518 ;
V_7 -> V_797 ++ ;
V_7 -> V_519 = false ;
V_7 -> V_798 = false ;
F_27 ( V_36 , V_473 , 1 ) ;
F_9 ( V_762 ,
L_112 ,
V_7 -> V_797 ) ;
}
}
static void F_206 ( struct V_6 * V_7 , T_2 * V_799 ,
T_2 * V_800 )
{
T_3 V_801 ;
T_1 V_1 ;
* V_799 = 0 ;
* V_800 = 0 ;
V_801 = ( V_7 -> V_12 -> V_199 . V_801 ++ ) %
( V_7 -> V_12 -> V_199 . V_802 ) ;
V_7 -> V_12 -> V_199 . V_803 [ V_801 ] =
V_7 -> V_12 -> V_199 . V_804 ;
V_7 -> V_12 -> V_199 . V_805 [ V_801 ] =
V_7 -> V_12 -> V_199 . V_806 ;
for ( V_1 = 0 ; V_1 < V_7 -> V_12 -> V_199 . V_802 ; V_1 ++ ) {
* V_799 += V_7 -> V_12 -> V_199 . V_803 [ V_1 ] ;
* V_800 += V_7 -> V_12 -> V_199 . V_805 [ V_1 ] ;
}
}
static void V_586 ( struct V_414 * V_415 )
{
struct V_807 * V_808 = F_207 ( V_415 ) ;
struct V_6 * V_7 = F_130 ( V_808 ,
struct V_6 , V_585 ) ;
struct V_35 * V_36 = V_7 -> V_12 -> V_36 ;
struct V_10 * V_11 = V_7 -> V_12 ;
T_8 V_809 = V_518 ;
static T_1 V_810 ;
bool V_811 = false ;
T_2 V_799 = 0 ;
T_2 V_800 = 0 ;
if ( ! V_7 -> V_182 )
return;
F_208 ( V_36 ) ;
if ( V_11 -> V_118 == V_119 ) {
if ( V_11 -> V_199 . V_812 > 666 ||
V_11 -> V_199 . V_200 > 666 ) {
V_811 = true ;
}
V_11 -> V_199 . V_812 = 0 ;
V_11 -> V_199 . V_200 = 0 ;
V_11 -> V_199 . V_811 = V_811 ;
}
if ( V_7 -> V_12 -> V_118 == V_119 &&
V_7 -> V_12 -> V_120 == V_121 ) {
F_206 ( V_7 , & V_799 , & V_800 ) ;
if ( ( V_799 + V_800 ) == 0 ) {
#ifdef F_209
if ( V_778 == V_724 )
F_9 ( V_27 , L_113 , V_28 ) ;
#endif
F_119 ( V_36 ,
L_114 ,
V_28 ) ;
V_7 -> V_12 -> V_118 = V_813 ;
F_210 ( V_7 -> V_12 ) ;
F_211 ( V_7 -> V_12 ,
V_7 -> V_12 -> V_211 . V_260 ) ;
V_7 -> V_12 -> V_539 ( V_36 ) ;
F_203 ( V_7 -> V_12 -> V_795 ,
& V_7 -> V_12 -> V_814 ) ;
}
}
V_7 -> V_12 -> V_199 . V_804 = 0 ;
V_7 -> V_12 -> V_199 . V_806 = 0 ;
if ( V_810 ++ >= 3 ) {
V_809 = F_189 ( V_36 ) ;
V_810 = 3 ;
}
if ( ( V_7 -> V_521 ) || ( V_7 -> V_517 == V_518 &&
( V_7 -> V_519 ||
( ! V_7 -> V_520 && V_809 == V_765 ) ) ) ) {
F_9 ( V_762 ,
L_115 ,
V_28 , V_7 -> V_521 , V_7 -> V_517 ,
V_7 -> V_519 ,
V_7 -> V_520 , V_809 ) ;
F_192 ( V_36 ) ;
}
V_7 -> V_521 = false ;
V_7 -> V_519 = false ;
V_7 -> V_798 = false ;
F_9 ( V_815 , L_116 ) ;
}
static void V_663 ( unsigned long V_40 )
{
struct V_6 * V_7 = F_19 ( (struct V_35 * ) V_40 ) ;
F_138 ( & V_7 -> V_585 , 0 ) ;
F_212 ( & V_7 -> V_662 ,
V_816 + F_213 ( V_817 ) ) ;
}
static int F_202 ( struct V_35 * V_36 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
int V_679 = 0 ;
V_7 -> V_182 = 1 ;
V_7 -> V_12 -> V_506 = 1 ;
F_9 ( V_95 , L_117 ) ;
V_679 = F_169 ( V_36 ) ;
if ( ! V_679 ) {
F_9 ( V_27 , L_118 ,
V_28 ) ;
V_7 -> V_182 = V_7 -> V_12 -> V_506 = 0 ;
return - V_792 ;
}
F_9 ( V_95 , L_119 ) ;
F_74 ( V_36 ) ;
if ( V_7 -> V_12 -> V_118 != V_119 )
F_214 ( V_7 -> V_12 ) ;
F_215 ( V_7 -> V_12 ) ;
V_663 ( ( unsigned long ) V_36 ) ;
if ( ! F_40 ( V_36 ) )
F_216 ( V_36 ) ;
else
F_217 ( V_36 ) ;
return 0 ;
}
static int F_218 ( struct V_35 * V_36 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
int V_189 ;
F_193 ( & V_7 -> V_582 ) ;
V_189 = F_219 ( V_36 ) ;
V_182 ( & V_7 -> V_582 ) ;
return V_189 ;
}
int F_219 ( struct V_35 * V_36 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
if ( V_7 -> V_182 == 1 )
return - 1 ;
return F_202 ( V_36 ) ;
}
static int F_220 ( struct V_35 * V_36 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
int V_189 ;
F_193 ( & V_7 -> V_582 ) ;
V_189 = F_221 ( V_36 ) ;
V_182 ( & V_7 -> V_582 ) ;
return V_189 ;
}
int F_221 ( struct V_35 * V_36 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
int V_1 ;
if ( V_7 -> V_182 == 0 )
return - 1 ;
V_7 -> V_182 = 0 ;
V_7 -> V_12 -> V_506 = 0 ;
F_9 ( V_818 , L_120 , V_28 ) ;
if ( ! F_40 ( V_36 ) )
F_222 ( V_36 ) ;
F_76 ( V_36 ) ;
for ( V_1 = 0 ; V_1 < V_577 ; V_1 ++ )
F_80 ( & V_7 -> V_12 -> V_205 [ V_1 ] ) ;
for ( V_1 = 0 ; V_1 < V_577 ; V_1 ++ )
F_80 ( & V_7 -> V_12 -> V_578 [ V_1 ] ) ;
for ( V_1 = 0 ; V_1 < V_577 ; V_1 ++ )
F_80 ( & V_7 -> V_12 -> V_579 [ V_1 ] ) ;
F_194 ( V_7 ) ;
F_195 ( V_36 ) ;
F_196 ( & V_7 -> V_662 ) ;
F_201 ( V_7 -> V_12 ) ;
memset ( & V_7 -> V_12 -> V_211 , 0 ,
F_223 ( struct V_58 , V_819 ) ) ;
F_9 ( V_818 , L_121 , V_28 ) ;
return 0 ;
}
void F_224 ( struct V_35 * V_36 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
int V_787 = 0 ;
if ( V_7 -> V_182 == 0 )
return;
V_7 -> V_182 = 0 ;
F_194 ( V_7 ) ;
F_196 ( & V_7 -> V_662 ) ;
F_201 ( V_7 -> V_12 ) ;
F_76 ( V_36 ) ;
V_787 = F_202 ( V_36 ) ;
}
static void V_584 ( struct V_414 * V_415 )
{
struct V_6 * V_7 = F_130 ( V_415 , struct V_6 ,
V_114 ) ;
struct V_35 * V_36 = V_7 -> V_12 -> V_36 ;
F_193 ( & V_7 -> V_582 ) ;
F_224 ( V_36 ) ;
V_182 ( & V_7 -> V_582 ) ;
}
static void F_225 ( struct V_35 * V_36 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
short V_514 ;
V_514 = ( V_36 -> V_157 & V_158 ) ? 1 : 0 ;
if ( V_514 != V_7 -> V_514 )
V_7 -> V_514 = V_514 ;
}
static int F_226 ( struct V_35 * V_36 , void * V_820 )
{
struct V_6 * V_7 = F_19 ( V_36 ) ;
struct V_821 * V_39 = V_820 ;
F_193 ( & V_7 -> V_582 ) ;
F_227 ( V_36 -> V_618 , V_39 -> V_822 ) ;
F_57 ( & V_7 -> V_114 ) ;
V_182 ( & V_7 -> V_582 ) ;
return 0 ;
}
static int F_228 ( struct V_35 * V_36 , struct V_823 * V_824 , int V_173 )
{
struct V_6 * V_7 = (struct V_6 * ) F_19 ( V_36 ) ;
struct V_825 * V_826 = (struct V_825 * ) V_824 ;
int V_189 = - 1 ;
struct V_10 * V_11 = V_7 -> V_12 ;
T_2 V_827 [ 4 ] ;
T_1 V_828 [ 6 ] = { 0xff , 0xff , 0xff , 0xff , 0xff , 0xff } ;
struct V_829 * V_830 = & V_826 -> V_831 . V_40 ;
struct V_832 * V_833 = NULL ;
F_193 ( & V_7 -> V_582 ) ;
if ( V_830 -> V_834 < sizeof( struct V_832 ) || ! V_830 -> V_835 ) {
V_189 = - V_836 ;
goto V_837;
}
V_833 = F_229 ( V_830 -> V_835 , V_830 -> V_834 ) ;
if ( F_230 ( V_833 ) ) {
V_189 = F_231 ( V_833 ) ;
goto V_837;
}
switch ( V_173 ) {
case V_838 :
if ( V_833 -> V_173 == V_839 ) {
if ( V_833 -> V_831 . V_475 . V_840 ) {
if ( strcmp ( V_833 -> V_831 . V_475 . V_841 , L_122 ) == 0 ) {
V_11 -> V_391 = V_395 ;
} else if ( strcmp ( V_833 -> V_831 . V_475 . V_841 , L_123 ) == 0 ) {
V_11 -> V_391 = V_394 ;
} else if ( strcmp ( V_833 -> V_831 . V_475 . V_841 , L_63 ) == 0 ) {
if ( V_833 -> V_831 . V_475 . V_842 == 13 )
V_11 -> V_391 = V_393 ;
else if ( V_833 -> V_831 . V_475 . V_842 == 5 )
V_11 -> V_391 = V_392 ;
} else {
V_11 -> V_391 = V_396 ;
}
if ( V_11 -> V_391 ) {
memcpy ( ( T_1 * ) V_827 , V_833 -> V_831 . V_475 . V_827 , 16 ) ;
F_128 ( V_36 ) ;
F_191 ( V_36 , 4 , V_833 -> V_831 . V_475 . V_843 ,
V_11 -> V_391 ,
( T_1 * ) V_11 -> V_844 ,
0 , V_827 ) ;
if ( V_11 -> V_845 != 2 )
F_191 ( V_36 , V_833 -> V_831 . V_475 . V_843 ,
V_833 -> V_831 . V_475 . V_843 ,
V_11 -> V_391 ,
( T_1 * ) V_11 -> V_844 ,
0 , V_827 ) ;
}
} else {
memcpy ( ( T_1 * ) V_827 , V_833 -> V_831 . V_475 . V_827 , 16 ) ;
if ( strcmp ( V_833 -> V_831 . V_475 . V_841 , L_122 ) == 0 ) {
V_11 -> V_785 = V_395 ;
} else if ( strcmp ( V_833 -> V_831 . V_475 . V_841 , L_123 ) == 0 ) {
V_11 -> V_785 = V_394 ;
} else if ( strcmp ( V_833 -> V_831 . V_475 . V_841 , L_63 ) == 0 ) {
if ( V_833 -> V_831 . V_475 . V_842 == 13 )
V_11 -> V_785 = V_393 ;
else if ( V_833 -> V_831 . V_475 . V_842 == 5 )
V_11 -> V_785 = V_392 ;
} else {
V_11 -> V_785 = V_396 ;
}
if ( V_11 -> V_785 ) {
F_191 ( V_36 , V_833 -> V_831 . V_475 . V_843 ,
V_833 -> V_831 . V_475 . V_843 ,
V_11 -> V_785 ,
V_828 ,
0 ,
V_827 ) ;
}
}
}
V_189 = F_232 ( V_7 -> V_12 ,
& V_826 -> V_831 . V_40 ) ;
break;
default:
V_189 = - V_846 ;
break;
}
F_23 ( V_833 ) ;
V_833 = NULL ;
V_837:
V_182 ( & V_7 -> V_582 ) ;
return V_189 ;
}
static T_1 F_233 ( bool V_847 , T_1 V_178 )
{
T_1 V_848 = 0xff ;
if ( ! V_847 ) {
switch ( V_178 ) {
case V_298 :
V_848 = V_214 ;
break;
case V_299 :
V_848 = V_216 ;
break;
case V_300 :
V_848 = V_218 ;
break;
case V_301 :
V_848 = V_220 ;
break;
case V_302 :
V_848 = V_222 ;
break;
case V_303 :
V_848 = V_224 ;
break;
case V_304 :
V_848 = V_226 ;
break;
case V_305 :
V_848 = V_228 ;
break;
case V_306 :
V_848 = V_230 ;
break;
case V_307 :
V_848 = V_232 ;
break;
case V_308 :
V_848 = V_234 ;
break;
case V_309 :
V_848 = V_236 ;
break;
default:
V_848 = 0xff ;
F_9 ( V_849 ,
L_124 ,
V_178 , V_847 ) ;
break;
}
} else {
switch ( V_178 ) {
case V_311 :
V_848 = V_310 ;
break;
case V_313 :
V_848 = V_312 ;
break;
case V_315 :
V_848 = V_314 ;
break;
case V_317 :
V_848 = V_316 ;
break;
case V_319 :
V_848 = V_318 ;
break;
case V_321 :
V_848 = V_320 ;
break;
case V_323 :
V_848 = V_322 ;
break;
case V_325 :
V_848 = V_324 ;
break;
case V_327 :
V_848 = V_326 ;
break;
case V_329 :
V_848 = V_328 ;
break;
case V_331 :
V_848 = V_330 ;
break;
case V_333 :
V_848 = V_332 ;
break;
case V_335 :
V_848 = V_334 ;
break;
case V_337 :
V_848 = V_336 ;
break;
case V_339 :
V_848 = V_338 ;
break;
case V_341 :
V_848 = V_340 ;
break;
case V_342 :
V_848 = 0x80 | 0x20 ;
break;
default:
V_848 = 0xff ;
F_9 ( V_849 ,
L_124 ,
V_178 , V_847 ) ;
break;
}
}
return V_848 ;
}
static void F_234 ( struct V_35 * V_36 ,
struct V_133 * V_67 )
{
struct V_6 * V_7 = (struct V_6 * ) F_19 ( V_36 ) ;
if ( V_67 -> V_850 && ! V_67 -> V_851 ) {
V_67 -> V_852 [ 0 ] = V_7 -> V_853 ;
V_67 -> V_852 [ 1 ] = V_7 -> V_854 ;
} else {
V_7 -> V_853 = V_67 -> V_852 [ 0 ] ;
V_7 -> V_854 = V_67 -> V_852 [ 1 ] ;
}
}
static long F_235 ( T_1 V_855 )
{
long V_856 ;
V_856 = ( long ) ( ( V_855 + 1 ) >> 1 ) ;
V_856 -= 95 ;
return V_856 ;
}
static void F_236 ( struct V_6 * V_7 , T_1 * V_857 ,
struct V_133 * V_858 ,
struct V_133 * V_859 )
{
bool V_860 = false ;
T_1 V_861 ;
T_2 V_862 , V_863 ;
static T_2 V_864 , V_865 ;
static T_2 V_866 , V_867 ;
static T_2 V_868 , V_869 ;
static T_2 V_870 ;
static T_2 V_871 ;
static T_2 V_872 ;
struct V_873 * V_874 ;
T_3 V_875 ;
unsigned int V_876 , V_877 ;
V_874 = (struct V_873 * ) V_857 ;
V_875 = F_134 ( V_874 -> V_878 ) ;
V_876 = F_237 ( V_875 ) ;
V_877 = F_238 ( V_875 ) ;
V_859 -> V_879 = V_877 ;
if ( ! V_858 -> V_850 ) {
V_860 = true ;
}
if ( V_865 ++ >= V_880 ) {
V_865 = V_880 ;
V_868 = V_7 -> V_67 . V_881 [ V_864 ] ;
V_7 -> V_67 . V_882 -= V_868 ;
}
V_7 -> V_67 . V_882 += V_858 -> V_883 ;
V_7 -> V_67 . V_881 [ V_864 ++ ] =
V_858 -> V_883 ;
if ( V_864 >= V_880 )
V_864 = 0 ;
V_863 = V_7 -> V_67 . V_882 / V_865 ;
V_7 -> V_67 . V_884 = F_235 ( ( T_1 ) V_863 ) ;
V_859 -> V_885 = V_7 -> V_67 . V_884 ;
if ( ! V_858 -> V_886 ) {
if ( ! V_858 -> V_887 )
return;
}
if ( ! V_860 )
return;
V_7 -> V_67 . V_888 ++ ;
if ( ! V_858 -> V_889 &&
( V_858 -> V_890 || V_858 -> V_887 ) ) {
for ( V_861 = V_891 ; V_861 < V_7 -> V_727 ; V_861 ++ ) {
if ( ! F_239 (
V_7 -> V_12 -> V_36 , V_861 ) )
continue;
if ( V_7 -> V_67 . V_892 [ V_861 ] == 0 )
V_7 -> V_67 . V_892 [ V_861 ] =
V_858 -> V_893 [ V_861 ] ;
if ( V_858 -> V_893 [ V_861 ] > V_7 -> V_67 . V_892 [ V_861 ] ) {
V_7 -> V_67 . V_892 [ V_861 ] =
( ( V_7 -> V_67 . V_892 [ V_861 ] * ( V_894 - 1 ) ) +
( V_858 -> V_893 [ V_861 ] ) ) / ( V_894 ) ;
V_7 -> V_67 . V_892 [ V_861 ] = V_7 -> V_67 . V_892 [ V_861 ] + 1 ;
} else {
V_7 -> V_67 . V_892 [ V_861 ] =
( ( V_7 -> V_67 . V_892 [ V_861 ] * ( V_894 - 1 ) ) +
( V_858 -> V_893 [ V_861 ] ) ) / ( V_894 ) ;
}
F_9 ( V_895 ,
L_125 ,
V_7 -> V_67 . V_892 [ V_861 ] ) ;
}
}
F_9 ( V_896 , L_126 ,
V_858 -> V_889 ? L_127 : L_128 ,
V_858 -> V_897 ) ;
if ( V_858 -> V_898 ) {
if ( V_871 ++ >= V_899 ) {
V_871 = V_899 ;
V_872 = V_7 -> V_67 . V_900 [ V_870 ] ;
V_7 -> V_67 . V_901 -= V_872 ;
}
V_7 -> V_67 . V_901 += V_858 -> V_897 ;
V_7 -> V_67 . V_900 [ V_870 ] = V_858 -> V_897 ;
V_870 ++ ;
if ( V_870 >= V_899 )
V_870 = 0 ;
V_858 -> V_897 = V_7 -> V_67 . V_901 / V_871 ;
if ( V_858 -> V_897 >= 3 )
V_858 -> V_897 -= 3 ;
}
F_9 ( V_896 , L_126 ,
V_858 -> V_889 ? L_127 : L_128 ,
V_858 -> V_897 ) ;
if ( V_858 -> V_890 ||
V_858 -> V_898 ||
V_858 -> V_887 ) {
if ( V_7 -> V_769 < 0 )
V_7 -> V_769 =
V_858 -> V_897 ;
if ( V_858 -> V_897 > ( T_2 ) V_7 -> V_769 ) {
V_7 -> V_769 =
( ( ( V_7 -> V_769 ) * ( V_894 - 1 ) ) +
( V_858 -> V_897 ) ) / ( V_894 ) ;
V_7 -> V_769 = V_7 -> V_769 + 1 ;
} else {
V_7 -> V_769 =
( ( ( V_7 -> V_769 ) * ( V_894 - 1 ) ) +
( V_858 -> V_897 ) ) / ( V_894 ) ;
}
}
if ( V_858 -> V_902 ) {
if ( V_858 -> V_890 ||
V_858 -> V_898 ||
V_858 -> V_887 ) {
if ( V_867 ++ >= V_880 ) {
V_867 = V_880 ;
V_869 = V_7 -> V_67 . V_903 [ V_866 ] ;
V_7 -> V_67 . V_904 -= V_869 ;
}
V_7 -> V_67 . V_904 +=
V_858 -> V_902 ;
V_7 -> V_67 . V_903 [ V_866 ++ ] =
V_858 -> V_902 ;
if ( V_866 >= V_880 )
V_866 = 0 ;
V_863 = V_7 -> V_67 . V_904 /
V_867 ;
V_7 -> V_67 . V_905 = V_863 ;
V_7 -> V_67 . V_906 = V_863 ;
}
if ( V_858 -> V_890 ||
V_858 -> V_898 ||
V_858 -> V_887 ) {
for ( V_862 = 0 ; V_862 < 2 ; V_862 ++ ) {
if ( V_858 -> V_907 [ V_862 ] != - 1 ) {
if ( V_7 -> V_67 . V_908 [ V_862 ] == 0 )
V_7 -> V_67 . V_908 [ V_862 ] = V_858 -> V_907 [ V_862 ] ;
V_7 -> V_67 . V_908 [ V_862 ] =
( ( V_7 -> V_67 . V_908 [ V_862 ] * ( V_894 - 1 ) ) +
( V_858 -> V_907 [ V_862 ] * 1 ) ) / ( V_894 ) ;
}
}
}
}
}
static T_1 F_240 ( char V_909 )
{
if ( ( V_909 <= - 100 ) || ( V_909 >= 20 ) )
return 0 ;
else if ( V_909 >= 0 )
return 100 ;
else
return 100 + V_909 ;
}
static T_1 F_241 ( char V_910 )
{
char V_911 ;
V_911 = V_910 ;
if ( V_911 >= 0 )
V_911 = 0 ;
if ( V_911 <= - 33 )
V_911 = - 33 ;
V_911 = 0 - V_911 ;
V_911 *= 3 ;
if ( V_911 == 99 )
V_911 = 100 ;
return V_911 ;
}
static long F_242 ( long V_912 )
{
long V_913 ;
if ( V_912 >= 61 && V_912 <= 100 )
V_913 = 90 + ( ( V_912 - 60 ) / 4 ) ;
else if ( V_912 >= 41 && V_912 <= 60 )
V_913 = 78 + ( ( V_912 - 40 ) / 2 ) ;
else if ( V_912 >= 31 && V_912 <= 40 )
V_913 = 66 + ( V_912 - 30 ) ;
else if ( V_912 >= 21 && V_912 <= 30 )
V_913 = 54 + ( V_912 - 20 ) ;
else if ( V_912 >= 5 && V_912 <= 20 )
V_913 = 42 + ( ( ( V_912 - 5 ) * 2 ) / 3 ) ;
else if ( V_912 == 4 )
V_913 = 36 ;
else if ( V_912 == 3 )
V_913 = 27 ;
else if ( V_912 == 2 )
V_913 = 18 ;
else if ( V_912 == 1 )
V_913 = 9 ;
else
V_913 = V_912 ;
return V_913 ;
}
static inline bool F_243 ( struct V_914 * V_915 )
{
if ( V_915 -> V_916 )
return false ;
switch ( V_915 -> V_917 ) {
case V_298 :
case V_299 :
case V_300 :
case V_301 :
return true ;
default:
return false ;
}
}
static void F_244 ( struct V_6 * V_7 ,
struct V_133 * V_134 ,
V_914 * V_915 ,
struct V_133 * V_918 ,
bool V_919 ,
bool V_920 ,
bool V_898 ,
bool V_887 )
{
T_10 * V_921 ;
T_11 * V_922 ;
T_12 * V_923 ;
T_1 * V_924 ;
T_1 V_1 , V_925 , V_926 , V_927 , V_928 ;
char V_929 [ 4 ] , V_930 = 0 ;
char V_931 , V_932 ;
T_1 V_933 , V_934 ;
T_2 V_935 , V_936 = 0 ;
T_1 V_937 = 0 ;
T_1 V_938 = 0 ;
T_1 V_939 ;
V_7 -> V_67 . V_940 ++ ;
V_937 = F_243 ( V_915 ) ;
memset ( V_918 , 0 , sizeof( struct V_133 ) ) ;
V_134 -> V_886 =
V_918 -> V_886 = V_919 ;
V_134 -> V_890 = V_918 -> V_890 = V_920 ;
V_134 -> V_889 = V_918 -> V_889 = V_937 ;
V_134 -> V_898 = V_918 -> V_898 = V_898 ;
V_134 -> V_887 = V_918 -> V_887 = V_887 ;
V_924 = ( T_1 * ) V_915 ;
V_924 += sizeof( V_914 ) ;
V_922 = ( T_11 * ) V_924 ;
V_921 = ( T_10 * ) V_924 ;
V_134 -> V_907 [ 0 ] = - 1 ;
V_134 -> V_907 [ 1 ] = - 1 ;
V_918 -> V_907 [ 0 ] = - 1 ;
V_918 -> V_907 [ 1 ] = - 1 ;
if ( V_937 ) {
T_1 V_941 ;
V_7 -> V_67 . V_942 ++ ;
if ( ! V_7 -> V_943 ) {
V_941 = V_922 -> V_944 & 0xc0 ;
V_941 >>= 6 ;
switch ( V_941 ) {
case 0x3 :
V_930 = - 35 - ( V_922 -> V_944 & 0x3e ) ;
break;
case 0x2 :
V_930 = - 23 - ( V_922 -> V_944 & 0x3e ) ;
break;
case 0x1 :
V_930 = - 11 - ( V_922 -> V_944 & 0x3e ) ;
break;
case 0x0 :
V_930 = 6 - ( V_922 -> V_944 & 0x3e ) ;
break;
}
} else {
V_941 = V_922 -> V_944 & 0x60 ;
V_941 >>= 5 ;
switch ( V_941 ) {
case 0x3 :
V_930 = - 35 - ( ( V_922 -> V_944 & 0x1f ) << 1 ) ;
break;
case 0x2 :
V_930 = - 23 - ( ( V_922 -> V_944 & 0x1f ) << 1 ) ;
break;
case 0x1 :
V_930 = - 11 - ( ( V_922 -> V_944 & 0x1f ) << 1 ) ;
break;
case 0x0 :
V_930 = 6 - ( ( V_922 -> V_944 & 0x1f ) << 1 ) ;
break;
}
}
V_934 = F_240 ( V_930 ) ;
V_134 -> V_897 = V_918 -> V_897 = V_934 ;
V_134 -> V_945 = V_934 ;
if ( V_134 -> V_897 > 40 ) {
V_939 = 100 ;
} else {
V_939 = V_922 -> V_946 ;
if ( V_922 -> V_946 > 64 )
V_939 = 0 ;
else if ( V_922 -> V_946 < 20 )
V_939 = 100 ;
else
V_939 = ( ( 64 - V_939 ) * 100 ) / 44 ;
}
V_134 -> V_902 = V_918 -> V_902 = V_939 ;
V_134 -> V_907 [ 0 ] =
V_918 -> V_907 [ 0 ] = V_939 ;
V_134 -> V_907 [ 1 ] =
V_918 -> V_907 [ 1 ] = - 1 ;
} else {
V_7 -> V_67 . V_947 ++ ;
for ( V_1 = V_891 ; V_1 < V_7 -> V_727 ; V_1 ++ ) {
if ( V_7 -> V_948 [ V_1 ] )
V_938 ++ ;
else
continue;
if ( ! F_239 (
V_7 -> V_12 -> V_36 , V_1 ) )
continue;
V_929 [ V_1 ] =
( ( V_921 -> V_949 [ V_1 ] & 0x3F ) * 2 ) - 106 ;
V_926 = V_921 -> V_950 [ V_1 ] ;
V_931 = ( char ) ( V_926 ) ;
V_931 /= 2 ;
V_7 -> V_67 . V_951 [ V_1 ] = ( long ) V_931 ;
V_935 = F_240 ( V_929 [ V_1 ] ) ;
V_936 += V_935 ;
V_134 -> V_893 [ V_1 ] = ( T_1 ) V_935 ;
V_918 -> V_893 [ V_1 ] = ( T_1 ) V_935 ;
}
V_930 = ( ( ( V_921 -> V_934 ) >> 1 ) & 0x7f ) - 106 ;
V_934 = F_240 ( V_930 ) ;
V_134 -> V_897 = V_918 -> V_897 = V_934 ;
V_134 -> V_952 = V_918 -> V_952 = V_930 ;
if ( V_915 -> V_916 && V_915 -> V_917 >= V_327 &&
V_915 -> V_917 <= V_341 )
V_925 = 2 ;
else
V_925 = 1 ;
for ( V_1 = 0 ; V_1 < V_925 ; V_1 ++ ) {
V_927 = V_921 -> V_953 [ V_1 ] ;
V_932 = ( char ) ( V_927 ) ;
V_932 /= 2 ;
V_933 = F_241 ( V_932 ) ;
if ( V_1 == 0 )
V_134 -> V_902 =
V_918 -> V_902 =
( T_1 ) ( V_933 & 0xff ) ;
V_134 -> V_907 [ V_1 ] =
V_918 -> V_907 [ V_1 ] =
( T_1 ) ( V_933 & 0xff ) ;
}
V_928 = V_921 -> V_928 ;
V_923 = ( T_12 * )
& V_928 ;
if ( V_915 -> V_954 )
V_7 -> V_67 . V_955 [ 1 + V_923 -> V_956 ] ++ ;
else
V_7 -> V_67 . V_955 [ 0 ] ++ ;
}
if ( V_937 ) {
V_134 -> V_883 =
V_918 -> V_883 =
( T_1 ) ( F_242 ( ( long ) V_934 ) ) ;
} else {
if ( V_938 != 0 ) {
V_134 -> V_883 =
V_918 -> V_883 =
( T_1 ) ( F_242 ( ( long ) ( V_936 /= V_938 ) ) ) ;
}
}
}
static void F_245 (
struct V_133 * V_957 ,
struct V_133 * V_958 )
{
V_958 -> V_850 = V_957 -> V_850 ;
V_958 -> V_851 = V_957 -> V_851 ;
V_958 -> V_879 = V_957 -> V_879 ;
}
static void F_246 ( struct V_140 * V_141 ,
struct V_133 * V_134 ,
V_914 * V_915 )
{
struct V_142 * V_143 = (struct V_142 * ) V_141 -> V_148 ;
struct V_35 * V_36 = V_143 -> V_36 ;
struct V_6 * V_7 = (struct V_6 * ) F_19 ( V_36 ) ;
bool V_919 , V_920 ;
bool V_898 = false , V_887 = false ;
static struct V_133 V_959 ;
struct V_873 * V_874 ;
T_3 V_960 , type ;
T_1 * V_961 ;
T_1 * V_962 ;
V_961 = ( T_1 * ) V_141 -> V_40 ;
V_874 = (struct V_873 * ) V_961 ;
V_960 = F_134 ( V_874 -> V_963 ) ;
type = F_247 ( V_960 ) ;
V_962 = V_874 -> V_964 ;
V_919 = ( V_965 != type ) &&
( F_248 ( V_7 -> V_12 -> V_211 . V_260 , ( V_960 & V_966 ) ? V_874 -> V_964 : ( V_960 & V_967 ) ? V_874 -> V_968 : V_874 -> V_969 ) )
&& ( ! V_134 -> V_970 ) && ( ! V_134 -> V_971 ) && ( ! V_134 -> V_972 ) ;
V_920 = V_919 &
( F_248 ( V_962 , V_7 -> V_12 -> V_36 -> V_618 ) ) ;
if ( F_249 ( V_960 ) == V_973 )
V_898 = true ;
if ( F_249 ( V_960 ) == V_974 ) {
if ( ( F_248 ( V_962 , V_36 -> V_618 ) ) )
V_887 = true ;
}
if ( V_919 )
V_7 -> V_67 . V_975 ++ ;
if ( V_920 )
V_7 -> V_67 . V_976 ++ ;
F_236 ( V_7 , V_961 , & V_959 , V_134 ) ;
F_244 ( V_7 , V_134 , V_915 , & V_959 ,
V_919 , V_920 ,
V_898 , V_887 ) ;
F_245 ( V_134 , & V_959 ) ;
}
static void
F_250 ( struct V_35 * V_36 ,
struct V_133 * V_67 )
{
struct V_6 * V_7 = (struct V_6 * ) F_19 ( V_36 ) ;
T_2 V_977 = 1 ;
T_2 V_978 ;
T_2 V_979 ;
if ( V_67 -> V_971 )
V_977 = 2 ;
else if ( V_67 -> V_972 )
V_977 = 3 ;
if ( V_67 -> V_980 )
V_979 = 1 ;
else
V_979 = 0 ;
switch ( V_67 -> V_178 ) {
case V_214 :
V_978 = 0 ;
break;
case V_216 :
V_978 = 1 ;
break;
case V_218 :
V_978 = 2 ;
break;
case V_220 :
V_978 = 3 ;
break;
case V_222 :
V_978 = 4 ;
break;
case V_224 :
V_978 = 5 ;
break;
case V_226 :
V_978 = 6 ;
break;
case V_228 :
V_978 = 7 ;
break;
case V_230 :
V_978 = 8 ;
break;
case V_232 :
V_978 = 9 ;
break;
case V_234 :
V_978 = 10 ;
break;
case V_236 :
V_978 = 11 ;
break;
case V_310 :
V_978 = 12 ;
break;
case V_312 :
V_978 = 13 ;
break;
case V_314 :
V_978 = 14 ;
break;
case V_316 :
V_978 = 15 ;
break;
case V_318 :
V_978 = 16 ;
break;
case V_320 :
V_978 = 17 ;
break;
case V_322 :
V_978 = 18 ;
break;
case V_324 :
V_978 = 19 ;
break;
case V_326 :
V_978 = 20 ;
break;
case V_328 :
V_978 = 21 ;
break;
case V_330 :
V_978 = 22 ;
break;
case V_332 :
V_978 = 23 ;
break;
case V_334 :
V_978 = 24 ;
break;
case V_336 :
V_978 = 25 ;
break;
case V_338 :
V_978 = 26 ;
break;
case V_340 :
V_978 = 27 ;
break;
default:
V_978 = 28 ;
break;
}
V_7 -> V_67 . V_981 [ V_979 ] [ V_978 ] ++ ;
V_7 -> V_67 . V_982 [ 0 ] [ V_978 ] ++ ;
V_7 -> V_67 . V_982 [ V_977 ] [ V_978 ] ++ ;
}
static void F_251 ( struct V_140 * V_141 ,
struct V_133 * V_67 ,
bool V_983 )
{
struct V_142 * V_143 = (struct V_142 * ) V_141 -> V_148 ;
struct V_35 * V_36 = V_143 -> V_36 ;
struct V_6 * V_7 = (struct V_6 * ) F_19 ( V_36 ) ;
V_914 * V_984 = NULL ;
V_135 * V_985 = ( V_135 * ) V_141 -> V_40 ;
V_67 -> V_986 = V_985 -> V_986 ;
V_67 -> V_136 = V_985 -> V_136 ;
V_67 -> V_137 = 0 ;
V_67 -> V_972 = V_985 -> V_987 ;
V_67 -> V_971 = V_985 -> V_988 ;
V_67 -> V_970 = V_67 -> V_971 | V_67 -> V_972 ;
V_67 -> V_989 = ! V_985 -> V_990 ;
if ( ( V_7 -> V_12 -> V_251 -> V_417 ) &&
( V_7 -> V_12 -> V_391 == V_395 ) )
V_67 -> V_970 = false ;
else
V_67 -> V_970 = V_67 -> V_971 | V_67 -> V_972 ;
if ( V_67 -> V_986 < 24 || V_67 -> V_986 > V_991 )
V_67 -> V_970 |= 1 ;
if ( V_67 -> V_136 != 0 ) {
V_984 = ( V_914 * ) (
V_141 -> V_40
+ sizeof( V_135 )
+ V_67 -> V_137
) ;
if ( ! V_67 -> V_970 ) {
T_1 V_848 ;
V_848 = F_233 ( V_984 -> V_916 ,
V_984 -> V_917 ) ;
if ( V_848 == 0xff ) {
V_67 -> V_970 = 1 ;
V_67 -> V_178 = V_214 ;
} else {
V_67 -> V_178 = V_848 ;
}
} else {
V_67 -> V_178 = 0x02 ;
}
V_67 -> V_980 = V_984 -> V_992 ;
F_250 ( V_36 , V_67 ) ;
V_67 -> V_850 = ( V_984 -> V_993 == 1 ) ;
V_67 -> V_851 = ( V_984 -> V_993 == 1 ) &&
( V_984 -> V_994 == 1 ) ;
V_67 -> V_995 = V_984 -> V_996 ;
F_234 ( V_36 , V_67 ) ;
if ( V_984 -> V_994 == 1 || V_984 -> V_993 == 1 )
F_9 ( V_896 ,
L_129 ,
V_984 -> V_994 , V_984 -> V_993 ) ;
}
F_252 ( V_141 , sizeof( V_135 ) ) ;
if ( ( V_67 -> V_137 + V_67 -> V_136 ) > 0 ) {
V_67 -> V_997 = 1 ;
F_252 ( V_141 , V_67 -> V_137 + V_67 -> V_136 ) ;
}
if ( V_984 ) {
V_67 -> V_998 = V_984 -> V_954 ;
F_246 ( V_141 , V_67 , V_984 ) ;
}
}
static void F_253 ( struct V_140 * V_141 )
{
struct V_142 * V_143 = (struct V_142 * ) V_141 -> V_148 ;
struct V_35 * V_36 = V_143 -> V_36 ;
struct V_6 * V_7 = (struct V_6 * ) F_19 ( V_36 ) ;
struct V_133 V_67 = {
. signal = 0 ,
. V_999 = 0x100 - 98 ,
. V_178 = 0 ,
. V_1000 = V_1001 ,
} ;
T_2 V_1002 = 0 ;
struct V_1003 * V_1004 = NULL ;
bool V_1005 = false ;
if ( ( V_141 -> V_202 >= ( 20 + sizeof( V_135 ) ) ) && ( V_141 -> V_202 < V_146 ) ) {
F_251 ( V_141 , & V_67 , false ) ;
F_254 ( V_141 , V_141 -> V_202 - 4 ) ;
V_1002 = V_141 -> V_202 ;
V_1004 = (struct V_1003 * ) V_141 -> V_40 ;
V_1005 = false ;
if ( F_255 ( V_1004 -> V_964 ) ) {
} else if ( F_256 ( V_1004 -> V_964 ) ) {
} else {
V_1005 = true ;
}
if ( ! F_257 ( V_7 -> V_12 , V_141 , & V_67 ) ) {
F_103 ( V_141 ) ;
} else {
V_7 -> V_67 . V_92 ++ ;
if ( V_1005 )
V_7 -> V_67 . V_1006 += V_1002 ;
}
} else {
V_7 -> V_67 . V_94 ++ ;
F_119 ( V_36 , L_130 , V_141 -> V_202 ) ;
F_103 ( V_141 ) ;
}
}
static void F_258 (
struct V_35 * V_36 ,
struct V_133 * V_134 )
{
T_1 * V_1007 ;
T_3 V_1008 = 0 ;
struct V_6 * V_7 = F_19 ( V_36 ) ;
V_134 -> V_1009 += F_61 ( V_134 ) ;
V_1007 = V_134 -> V_1009 ;
V_1008 = V_134 -> V_1010 ;
#ifdef F_209
if ( ! V_558 -> V_559 )
F_259 ( V_558 , V_1011 ) ;
#endif
#ifdef F_260
T_9 V_1012 ;
V_558 -> V_1013 . V_1014 ( V_558 , V_1015 ,
( T_1 * ) ( & V_1012 ) ) ;
if ( V_1012 == V_724 )
return;
#endif
V_7 -> V_67 . V_1016 ++ ;
#ifdef F_209
F_261 ( V_558 , V_1011 ) ;
#endif
if ( F_89 ( V_36 , V_134 ) )
return;
#ifdef F_262
F_263 () ;
#endif
}
static void F_264 ( struct V_140 * V_141 ,
struct V_133 * V_67 )
{
V_135 * V_985 = ( V_135 * ) V_141 -> V_40 ;
V_67 -> V_1009 = ( T_1 * ) V_141 -> V_40 ;
V_67 -> V_986 = V_985 -> V_986 ;
V_67 -> V_136 = 0 ;
V_67 -> V_137 = 0 ;
V_67 -> V_1010 = V_67 -> V_986 - V_1017 ;
V_67 -> V_1018 = V_67 -> V_1010 ;
V_67 -> V_1019 = 0 ;
V_67 -> V_1020 = 1 ;
}
static void F_265 ( struct V_140 * V_141 )
{
struct V_142 * V_143 = (struct V_142 * ) V_141 -> V_148 ;
struct V_35 * V_36 = V_143 -> V_36 ;
struct V_133 V_67 = {
. signal = 0 ,
. V_999 = 0x100 - 98 ,
. V_178 = 0 ,
. V_1000 = V_1001 ,
} ;
if ( ( V_141 -> V_202 >= ( 20 + sizeof( V_135 ) ) ) && ( V_141 -> V_202 < V_146 ) ) {
F_264 ( V_141 , & V_67 ) ;
F_258 ( V_36 , & V_67 ) ;
F_103 ( V_141 ) ;
}
}
static void V_593 ( struct V_6 * V_7 )
{
struct V_140 * V_141 ;
struct V_142 * V_143 ;
while ( NULL != ( V_141 = F_105 ( & V_7 -> V_177 ) ) ) {
V_143 = (struct V_142 * ) V_141 -> V_148 ;
switch ( V_143 -> V_149 ) {
case 3 :
V_7 -> V_516 -- ;
F_253 ( V_141 ) ;
break;
case 9 :
F_9 ( V_849 , L_131 ,
V_143 -> V_149 ) ;
F_265 ( V_141 ) ;
break;
default:
F_9 ( V_27 , L_132 ,
V_143 -> V_149 ) ;
F_112 ( V_141 ) ;
break;
}
}
}
static int F_266 ( struct V_1021 * V_1022 ,
const struct V_1023 * V_1024 )
{
struct V_35 * V_36 = NULL ;
struct V_6 * V_7 = NULL ;
struct V_44 * V_45 = F_267 ( V_1022 ) ;
int V_189 ;
F_9 ( V_95 , L_133 ) ;
V_36 = F_268 ( sizeof( struct V_6 ) ) ;
if ( ! V_36 )
return - V_51 ;
F_269 ( V_1022 , V_36 ) ;
F_270 ( V_36 , & V_1022 -> V_36 ) ;
V_7 = F_19 ( V_36 ) ;
V_7 -> V_12 = F_271 ( V_36 ) ;
V_7 -> V_45 = V_45 ;
V_36 -> V_1025 = & V_1026 ;
V_36 -> V_1027 =
(struct V_1028 * ) & V_1029 ;
V_36 -> type = V_1030 ;
V_36 -> V_1031 = V_50 * 3 ;
if ( F_272 ( V_36 , V_1032 ) < 0 ) {
F_9 ( V_95 ,
L_134 ) ;
V_1032 = L_135 ;
F_272 ( V_36 , V_1032 ) ;
}
F_9 ( V_95 , L_136 ) ;
if ( F_164 ( V_36 ) != 0 ) {
F_9 ( V_27 , L_137 ) ;
V_189 = - V_1033 ;
goto V_1034;
}
F_200 ( V_36 ) ;
F_222 ( V_36 ) ;
V_189 = F_273 ( V_36 ) ;
if ( V_189 )
goto V_1035;
F_9 ( V_95 , L_138 , V_36 -> V_106 ) ;
F_50 ( V_36 ) ;
F_9 ( V_95 , L_139 ) ;
return 0 ;
V_1035:
F_221 ( V_36 ) ;
F_23 ( V_7 -> V_575 ) ;
V_7 -> V_575 = NULL ;
F_125 ( V_36 ) ;
F_60 ( 10 ) ;
V_1034:
F_274 ( V_36 ) ;
F_9 ( V_27 , L_140 ) ;
return V_189 ;
}
static void F_194 ( struct V_6 * V_7 )
{
F_275 ( & V_7 -> V_114 ) ;
F_198 ( & V_7 -> V_585 ) ;
F_198 ( & V_7 -> V_416 ) ;
F_275 ( & V_7 -> V_421 ) ;
}
static void F_276 ( struct V_1021 * V_1022 )
{
struct V_35 * V_36 = F_277 ( V_1022 ) ;
struct V_6 * V_7 = F_19 ( V_36 ) ;
if ( V_36 ) {
F_278 ( V_36 ) ;
F_9 ( V_818 ,
L_141 ) ;
F_53 ( V_36 ) ;
F_221 ( V_36 ) ;
F_23 ( V_7 -> V_575 ) ;
V_7 -> V_575 = NULL ;
F_125 ( V_36 ) ;
F_60 ( 10 ) ;
}
F_274 ( V_36 ) ;
F_9 ( V_818 , L_142 ) ;
}
static int T_13 F_279 ( void )
{
int V_189 ;
#ifdef F_280
V_189 = F_281 () ;
if ( V_189 ) {
F_282 ( L_143 , V_189 ) ;
return V_189 ;
}
#endif
V_189 = F_283 () ;
if ( V_189 ) {
F_282 ( L_144 , V_189 ) ;
return V_189 ;
}
V_189 = F_284 () ;
if ( V_189 ) {
F_282 ( L_145 , V_189 ) ;
return V_189 ;
}
V_189 = F_285 () ;
if ( V_189 ) {
F_282 ( L_146 , V_189 ) ;
return V_189 ;
}
V_189 = F_286 () ;
if ( V_189 ) {
F_282 ( L_147 , V_189 ) ;
return V_189 ;
}
F_287 ( L_148 ) ;
F_287 ( L_149 ) ;
F_9 ( V_95 , L_150 ) ;
F_9 ( V_95 , L_151 , V_1036 ) ;
F_43 () ;
return F_288 ( & V_1037 ) ;
}
static void T_14 F_289 ( void )
{
F_290 ( & V_1037 ) ;
F_9 ( V_818 , L_152 ) ;
}
void F_128 ( struct V_35 * V_36 )
{
T_1 V_680 = 0x0 ;
struct V_6 * V_7 = (struct V_6 * ) F_19 ( V_36 ) ;
struct V_10 * V_11 = V_7 -> V_12 ;
V_680 = V_716 | V_717 ;
if ( ( ( V_392 == V_11 -> V_391 ) || ( V_393 == V_11 -> V_391 ) ) && ( V_7 -> V_12 -> V_845 != 2 ) ) {
V_680 |= V_1038 ;
V_680 |= V_1039 ;
} else if ( ( V_11 -> V_120 == V_124 ) && ( V_11 -> V_391 & ( V_395 | V_394 ) ) ) {
V_680 |= V_1038 ;
V_680 |= V_1039 ;
}
V_11 -> V_1040 = 1 ;
if ( ( V_11 -> V_251 -> V_1041 & V_1042 ) || ! V_1043 ) {
V_11 -> V_1040 = 0 ;
V_680 &= ~ V_717 ;
}
F_9 ( V_784 , L_153 ,
V_28 , V_11 -> V_1040 , V_11 -> V_391 ,
V_680 ) ;
F_27 ( V_36 , V_719 , V_680 ) ;
}
void F_191 ( struct V_35 * V_36 , T_1 V_1044 , T_1 V_1045 , T_3 V_1046 ,
T_1 * V_781 , T_1 V_1047 , T_2 * V_1048 )
{
T_2 V_1049 = 0 ;
T_2 V_1050 = 0 ;
T_3 V_1051 = 0 ;
T_1 V_1 ;
if ( V_1044 >= V_1052 )
F_9 ( V_27 , L_154 ) ;
F_9 ( V_784 ,
L_155 ,
V_36 , V_1044 , V_1045 , V_1046 , V_781 ) ;
if ( V_1047 )
V_1051 |= F_13 ( 15 ) | ( V_1046 << 2 ) ;
else
V_1051 |= F_13 ( 15 ) | ( V_1046 << 2 ) | V_1045 ;
for ( V_1 = 0 ; V_1 < V_1053 ; V_1 ++ ) {
V_1049 = V_1 + V_1053 * V_1044 ;
V_1049 |= F_13 ( 31 ) | F_13 ( 16 ) ;
if ( V_1 == 0 ) {
V_1050 = ( T_2 ) ( * ( V_781 + 0 ) ) << 16 |
( T_2 ) ( * ( V_781 + 1 ) ) << 24 |
( T_2 ) V_1051 ;
F_14 ( V_36 , V_41 , V_1050 ) ;
F_14 ( V_36 , V_38 , V_1049 ) ;
} else if ( V_1 == 1 ) {
V_1050 = ( T_2 ) ( * ( V_781 + 2 ) ) |
( T_2 ) ( * ( V_781 + 3 ) ) << 8 |
( T_2 ) ( * ( V_781 + 4 ) ) << 16 |
( T_2 ) ( * ( V_781 + 5 ) ) << 24 ;
F_14 ( V_36 , V_41 , V_1050 ) ;
F_14 ( V_36 , V_38 , V_1049 ) ;
} else {
if ( V_1048 ) {
F_14 ( V_36 , V_41 , ( T_2 ) ( * ( V_1048 + V_1 - 2 ) ) ) ;
F_14 ( V_36 , V_38 , V_1049 ) ;
}
}
}
}
