static int
F_1 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_2 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
3 , NULL , FALSE , 0 , NULL ) ;
return T_3 ;
}
static int
F_3 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_4 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
0U , 255U , & V_2 , FALSE ) ;
#line 64 "../../asn1/sbc-ap/sbc-ap.cnf"
if ( F_5 ( T_5 -> V_3 -> V_4 , V_5 ) )
F_6 ( T_5 -> V_3 -> V_4 , V_5 , L_1 ,
F_7 ( V_2 , V_6 ,
L_2 ) ) ;
return T_3 ;
}
static int
F_8 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_4 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
0U , 65535U , & V_7 , FALSE ) ;
return T_3 ;
}
static int
F_9 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_4 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
0U , 65535U , & V_8 , FALSE ) ;
#line 47 "../../asn1/sbc-ap/sbc-ap.cnf"
if ( T_7 ) {
F_10 ( F_11 ( T_5 -> V_9 , 2 ) , L_3 , F_7 ( V_8 , F_12 ( V_10 ) , L_4 ) ) ;
}
return T_3 ;
}
static int
F_13 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_2 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
4 , NULL , FALSE , 0 , NULL ) ;
return T_3 ;
}
static int
F_14 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_15 ( T_2 , T_3 , T_5 , T_7 , T_8 , V_11 ) ;
return T_3 ;
}
static int
F_16 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_17 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
V_12 , V_13 ) ;
return T_3 ;
}
static int
F_18 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_19 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
V_14 , V_15 ,
0 , V_16 , FALSE ) ;
return T_3 ;
}
static int
F_20 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_15 ( T_2 , T_3 , T_5 , T_7 , T_8 , V_17 ) ;
return T_3 ;
}
static int
F_21 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_17 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
V_18 , V_19 ) ;
return T_3 ;
}
static int
F_22 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_19 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
V_20 , V_21 ,
1 , V_22 , FALSE ) ;
return T_3 ;
}
static int
F_23 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_4 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
0U , 255U , NULL , FALSE ) ;
return T_3 ;
}
static int
F_24 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_25 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
28 , 28 , FALSE , NULL ) ;
return T_3 ;
}
static int
F_26 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_2 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
1 , NULL , FALSE , 0 , NULL ) ;
return T_3 ;
}
static int
F_27 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_2 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
2 , NULL , TRUE , 0 , NULL ) ;
return T_3 ;
}
static int
F_28 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_17 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
V_23 , V_24 ) ;
return T_3 ;
}
static int
F_29 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_19 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
V_25 , V_26 ,
1 , V_27 , FALSE ) ;
return T_3 ;
}
static int
F_30 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_17 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
V_28 , V_29 ) ;
return T_3 ;
}
static int
F_31 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_25 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
8 , 8 , FALSE , NULL ) ;
return T_3 ;
}
static int
F_32 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
#line 102 "../../asn1/sbc-ap/sbc-ap.cnf"
T_1 * V_30 = NULL ;
T_3 = F_33 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
3 , 3 , FALSE , & V_30 ) ;
if( F_34 ( T_2 ) == 0 )
return T_3 ;
if ( ! V_30 )
return T_3 ;
F_35 ( V_30 , T_5 -> V_3 , T_7 , 0 , FALSE ) ;
return T_3 ;
}
static int
F_36 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_17 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
V_31 , V_32 ) ;
return T_3 ;
}
static int
F_37 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_19 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
V_33 , V_34 ,
1 , V_35 , FALSE ) ;
return T_3 ;
}
static int
F_38 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_33 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
3 , 3 , FALSE , NULL ) ;
return T_3 ;
}
static int
F_39 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_19 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
V_36 , V_37 ,
1 , V_38 , FALSE ) ;
return T_3 ;
}
static int
F_40 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_4 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
4096U , 131071U , NULL , FALSE ) ;
return T_3 ;
}
static int
F_41 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_33 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
2 , 2 , FALSE , NULL ) ;
return T_3 ;
}
static int
F_42 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_17 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
V_39 , V_40 ) ;
return T_3 ;
}
static int
F_43 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_17 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
V_41 , V_42 ) ;
return T_3 ;
}
static int
F_44 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_19 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
V_43 , V_44 ,
1 , V_45 , FALSE ) ;
return T_3 ;
}
static int
F_45 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_25 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
16 , 16 , FALSE , NULL ) ;
return T_3 ;
}
static int
F_46 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_4 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
0U , 65535U , NULL , FALSE ) ;
return T_3 ;
}
static int
F_47 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_33 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
1 , 20 , FALSE , NULL ) ;
return T_3 ;
}
static int
F_48 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_4 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
0U , 4096U , NULL , FALSE ) ;
return T_3 ;
}
static int
F_49 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_25 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
16 , 16 , FALSE , NULL ) ;
return T_3 ;
}
static int
F_50 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_19 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
V_46 , V_47 ,
1 , V_48 , FALSE ) ;
return T_3 ;
}
static int
F_51 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_52 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
V_49 , V_50 ,
NULL ) ;
return T_3 ;
}
static int
F_53 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_33 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
1 , 9600 , FALSE , NULL ) ;
return T_3 ;
}
static int
F_54 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_33 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
50 , 50 , FALSE , NULL ) ;
return T_3 ;
}
static int
F_55 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_33 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
2 , 2 , FALSE , NULL ) ;
return T_3 ;
}
static int
F_56 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_17 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
V_51 , V_52 ) ;
return T_3 ;
}
static int
F_57 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_17 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
V_53 , V_54 ) ;
return T_3 ;
}
static int
F_58 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_17 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
V_55 , V_56 ) ;
return T_3 ;
}
static int
F_59 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_17 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
V_57 , V_58 ) ;
return T_3 ;
}
static int
F_60 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_15 ( T_2 , T_3 , T_5 , T_7 , T_8 , V_59 ) ;
return T_3 ;
}
static int
F_61 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_17 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
V_60 , V_61 ) ;
return T_3 ;
}
static int
F_62 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_15 ( T_2 , T_3 , T_5 , T_7 , T_8 , V_62 ) ;
return T_3 ;
}
static int
F_63 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_17 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
V_63 , V_64 ) ;
return T_3 ;
}
static int
F_64 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_15 ( T_2 , T_3 , T_5 , T_7 , T_8 , V_65 ) ;
return T_3 ;
}
static int
F_65 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_17 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
V_66 , V_67 ) ;
return T_3 ;
}
static int
F_66 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_52 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
V_68 , V_69 ,
NULL ) ;
return T_3 ;
}
static int F_67 ( T_1 * T_2 V_1 , T_9 * V_3 V_1 , T_6 * T_7 V_1 , void * T_10 V_1 ) {
int T_3 = 0 ;
T_4 V_70 ;
F_68 ( & V_70 , V_71 , TRUE , V_3 ) ;
T_3 = F_23 ( T_2 , T_3 , & V_70 , T_7 , V_72 ) ;
T_3 += 7 ; T_3 >>= 3 ;
return T_3 ;
}
static int F_69 ( T_1 * T_2 V_1 , T_9 * V_3 V_1 , T_6 * T_7 V_1 , void * T_10 V_1 ) {
int T_3 = 0 ;
T_4 V_70 ;
F_68 ( & V_70 , V_71 , TRUE , V_3 ) ;
T_3 = F_26 ( T_2 , T_3 , & V_70 , T_7 , V_73 ) ;
T_3 += 7 ; T_3 >>= 3 ;
return T_3 ;
}
static int F_70 ( T_1 * T_2 V_1 , T_9 * V_3 V_1 , T_6 * T_7 V_1 , void * T_10 V_1 ) {
int T_3 = 0 ;
T_4 V_70 ;
F_68 ( & V_70 , V_71 , TRUE , V_3 ) ;
T_3 = F_30 ( T_2 , T_3 , & V_70 , T_7 , V_74 ) ;
T_3 += 7 ; T_3 >>= 3 ;
return T_3 ;
}
static int F_71 ( T_1 * T_2 V_1 , T_9 * V_3 V_1 , T_6 * T_7 V_1 , void * T_10 V_1 ) {
int T_3 = 0 ;
T_4 V_70 ;
F_68 ( & V_70 , V_71 , TRUE , V_3 ) ;
T_3 = F_31 ( T_2 , T_3 , & V_70 , T_7 , V_75 ) ;
T_3 += 7 ; T_3 >>= 3 ;
return T_3 ;
}
static int F_72 ( T_1 * T_2 V_1 , T_9 * V_3 V_1 , T_6 * T_7 V_1 , void * T_10 V_1 ) {
int T_3 = 0 ;
T_4 V_70 ;
F_68 ( & V_70 , V_71 , TRUE , V_3 ) ;
T_3 = F_40 ( T_2 , T_3 , & V_70 , T_7 , V_76 ) ;
T_3 += 7 ; T_3 >>= 3 ;
return T_3 ;
}
static int F_73 ( T_1 * T_2 V_1 , T_9 * V_3 V_1 , T_6 * T_7 V_1 , void * T_10 V_1 ) {
int T_3 = 0 ;
T_4 V_70 ;
F_68 ( & V_70 , V_71 , TRUE , V_3 ) ;
T_3 = F_44 ( T_2 , T_3 , & V_70 , T_7 , V_77 ) ;
T_3 += 7 ; T_3 >>= 3 ;
return T_3 ;
}
static int F_74 ( T_1 * T_2 V_1 , T_9 * V_3 V_1 , T_6 * T_7 V_1 , void * T_10 V_1 ) {
int T_3 = 0 ;
T_4 V_70 ;
F_68 ( & V_70 , V_71 , TRUE , V_3 ) ;
T_3 = F_45 ( T_2 , T_3 , & V_70 , T_7 , V_78 ) ;
T_3 += 7 ; T_3 >>= 3 ;
return T_3 ;
}
static int F_75 ( T_1 * T_2 V_1 , T_9 * V_3 V_1 , T_6 * T_7 V_1 , void * T_10 V_1 ) {
int T_3 = 0 ;
T_4 V_70 ;
F_68 ( & V_70 , V_71 , TRUE , V_3 ) ;
T_3 = F_46 ( T_2 , T_3 , & V_70 , T_7 , V_79 ) ;
T_3 += 7 ; T_3 >>= 3 ;
return T_3 ;
}
static int F_76 ( T_1 * T_2 V_1 , T_9 * V_3 V_1 , T_6 * T_7 V_1 , void * T_10 V_1 ) {
int T_3 = 0 ;
T_4 V_70 ;
F_68 ( & V_70 , V_71 , TRUE , V_3 ) ;
T_3 = F_47 ( T_2 , T_3 , & V_70 , T_7 , V_80 ) ;
T_3 += 7 ; T_3 >>= 3 ;
return T_3 ;
}
static int F_77 ( T_1 * T_2 V_1 , T_9 * V_3 V_1 , T_6 * T_7 V_1 , void * T_10 V_1 ) {
int T_3 = 0 ;
T_4 V_70 ;
F_68 ( & V_70 , V_71 , TRUE , V_3 ) ;
T_3 = F_48 ( T_2 , T_3 , & V_70 , T_7 , V_81 ) ;
T_3 += 7 ; T_3 >>= 3 ;
return T_3 ;
}
static int F_78 ( T_1 * T_2 V_1 , T_9 * V_3 V_1 , T_6 * T_7 V_1 , void * T_10 V_1 ) {
int T_3 = 0 ;
T_4 V_70 ;
F_68 ( & V_70 , V_71 , TRUE , V_3 ) ;
T_3 = F_49 ( T_2 , T_3 , & V_70 , T_7 , V_82 ) ;
T_3 += 7 ; T_3 >>= 3 ;
return T_3 ;
}
static int F_79 ( T_1 * T_2 V_1 , T_9 * V_3 V_1 , T_6 * T_7 V_1 , void * T_10 V_1 ) {
int T_3 = 0 ;
T_4 V_70 ;
F_68 ( & V_70 , V_71 , TRUE , V_3 ) ;
T_3 = F_51 ( T_2 , T_3 , & V_70 , T_7 , V_83 ) ;
T_3 += 7 ; T_3 >>= 3 ;
return T_3 ;
}
static int F_80 ( T_1 * T_2 V_1 , T_9 * V_3 V_1 , T_6 * T_7 V_1 , void * T_10 V_1 ) {
int T_3 = 0 ;
T_4 V_70 ;
F_68 ( & V_70 , V_71 , TRUE , V_3 ) ;
T_3 = F_53 ( T_2 , T_3 , & V_70 , T_7 , V_84 ) ;
T_3 += 7 ; T_3 >>= 3 ;
return T_3 ;
}
static int F_81 ( T_1 * T_2 V_1 , T_9 * V_3 V_1 , T_6 * T_7 V_1 , void * T_10 V_1 ) {
int T_3 = 0 ;
T_4 V_70 ;
F_68 ( & V_70 , V_71 , TRUE , V_3 ) ;
T_3 = F_54 ( T_2 , T_3 , & V_70 , T_7 , V_85 ) ;
T_3 += 7 ; T_3 >>= 3 ;
return T_3 ;
}
static int F_82 ( T_1 * T_2 V_1 , T_9 * V_3 V_1 , T_6 * T_7 V_1 , void * T_10 V_1 ) {
int T_3 = 0 ;
T_4 V_70 ;
F_68 ( & V_70 , V_71 , TRUE , V_3 ) ;
T_3 = F_55 ( T_2 , T_3 , & V_70 , T_7 , V_86 ) ;
T_3 += 7 ; T_3 >>= 3 ;
return T_3 ;
}
static int F_83 ( T_1 * T_2 V_1 , T_9 * V_3 V_1 , T_6 * T_7 V_1 , void * T_10 V_1 ) {
int T_3 = 0 ;
T_4 V_70 ;
F_68 ( & V_70 , V_71 , TRUE , V_3 ) ;
T_3 = F_56 ( T_2 , T_3 , & V_70 , T_7 , V_87 ) ;
T_3 += 7 ; T_3 >>= 3 ;
return T_3 ;
}
static int F_84 ( T_1 * T_2 V_1 , T_9 * V_3 V_1 , T_6 * T_7 V_1 , void * T_10 V_1 ) {
int T_3 = 0 ;
T_4 V_70 ;
F_68 ( & V_70 , V_71 , TRUE , V_3 ) ;
T_3 = F_57 ( T_2 , T_3 , & V_70 , T_7 , V_88 ) ;
T_3 += 7 ; T_3 >>= 3 ;
return T_3 ;
}
static int F_85 ( T_1 * T_2 V_1 , T_9 * V_3 V_1 , T_6 * T_7 V_1 , void * T_10 V_1 ) {
int T_3 = 0 ;
T_4 V_70 ;
F_68 ( & V_70 , V_71 , TRUE , V_3 ) ;
T_3 = F_58 ( T_2 , T_3 , & V_70 , T_7 , V_89 ) ;
T_3 += 7 ; T_3 >>= 3 ;
return T_3 ;
}
static int F_86 ( T_1 * T_2 V_1 , T_9 * V_3 V_1 , T_6 * T_7 V_1 , void * T_10 V_1 ) {
int T_3 = 0 ;
T_4 V_70 ;
F_68 ( & V_70 , V_71 , TRUE , V_3 ) ;
T_3 = F_59 ( T_2 , T_3 , & V_70 , T_7 , V_90 ) ;
T_3 += 7 ; T_3 >>= 3 ;
return T_3 ;
}
static int F_87 ( T_1 * T_2 V_1 , T_9 * V_3 V_1 , T_6 * T_7 V_1 , void * T_10 V_1 ) {
int T_3 = 0 ;
T_4 V_70 ;
F_68 ( & V_70 , V_71 , TRUE , V_3 ) ;
T_3 = F_66 ( T_2 , T_3 , & V_70 , T_7 , V_91 ) ;
T_3 += 7 ; T_3 >>= 3 ;
return T_3 ;
}
static int V_11 ( T_1 * T_2 , T_9 * V_3 , T_6 * T_7 , void * T_10 V_1 )
{
return ( F_88 ( V_92 , V_8 , T_2 , V_3 , T_7 ) ) ? F_34 ( T_2 ) : 0 ;
}
static int V_17 ( T_1 * T_2 , T_9 * V_3 , T_6 * T_7 , void * T_10 V_1 )
{
return ( F_88 ( V_93 , V_7 , T_2 , V_3 , T_7 ) ) ? F_34 ( T_2 ) : 0 ;
}
static int V_59 ( T_1 * T_2 , T_9 * V_3 , T_6 * T_7 , void * T_10 V_1 )
{
return ( F_88 ( V_94 , V_2 , T_2 , V_3 , T_7 ) ) ? F_34 ( T_2 ) : 0 ;
}
static int V_62 ( T_1 * T_2 , T_9 * V_3 , T_6 * T_7 , void * T_10 V_1 )
{
return ( F_88 ( V_95 , V_2 , T_2 , V_3 , T_7 ) ) ? F_34 ( T_2 ) : 0 ;
}
static int V_65 ( T_1 * T_2 , T_9 * V_3 , T_6 * T_7 , void * T_10 V_1 )
{
return ( F_88 ( V_96 , V_2 , T_2 , V_3 , T_7 ) ) ? F_34 ( T_2 ) : 0 ;
}
static void
F_89 ( T_1 * T_2 , T_9 * V_3 , T_6 * T_7 )
{
T_11 * V_97 = NULL ;
T_6 * V_98 = NULL ;
if ( F_5 ( V_3 -> V_4 , V_99 ) )
F_90 ( V_3 -> V_4 , V_99 , V_100 ) ;
if ( T_7 ) {
V_97 = F_91 ( T_7 , V_101 , T_2 , 0 , - 1 , V_102 ) ;
V_98 = F_92 ( V_97 , V_103 ) ;
F_87 ( T_2 , V_3 , V_98 , NULL ) ;
}
}
void F_93 ( void ) {
static T_12 V_104 [] = {
#line 1 "../../asn1/sbc-ap/packet-sbc-ap-hfarr.c"
{ & V_72 ,
{ L_5 , L_6 ,
V_105 , V_106 , F_12 ( V_107 ) , 0 ,
NULL , V_108 } } ,
{ & V_73 ,
{ L_7 , L_8 ,
V_105 , V_106 , F_12 ( V_109 ) , 0 ,
NULL , V_108 } } ,
{ & V_74 ,
{ L_9 , L_10 ,
V_110 , V_111 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_75 ,
{ L_11 , L_12 ,
V_112 , V_111 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_76 ,
{ L_13 , L_14 ,
V_105 , V_106 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_77 ,
{ L_15 , L_16 ,
V_105 , V_106 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_78 ,
{ L_17 , L_18 ,
V_112 , V_111 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_79 ,
{ L_19 , L_20 ,
V_105 , V_106 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_80 ,
{ L_21 , L_22 ,
V_112 , V_111 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_81 ,
{ L_23 , L_24 ,
V_105 , V_106 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_82 ,
{ L_25 , L_26 ,
V_112 , V_111 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_83 ,
{ L_27 , L_28 ,
V_105 , V_106 , F_12 ( V_113 ) , 0 ,
NULL , V_108 } } ,
{ & V_84 ,
{ L_29 , L_30 ,
V_112 , V_111 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_85 ,
{ L_31 , L_32 ,
V_112 , V_111 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_86 ,
{ L_33 , L_34 ,
V_112 , V_111 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_87 ,
{ L_35 , L_36 ,
V_110 , V_111 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_88 ,
{ L_37 , L_38 ,
V_110 , V_111 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_89 ,
{ L_39 , L_40 ,
V_110 , V_111 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_90 ,
{ L_41 , L_42 ,
V_110 , V_111 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_91 ,
{ L_43 , L_44 ,
V_105 , V_106 , F_12 ( V_114 ) , 0 ,
NULL , V_108 } } ,
{ & V_115 ,
{ L_45 , L_46 ,
V_110 , V_111 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_116 ,
{ L_47 , L_48 ,
V_105 , V_106 , F_12 ( V_10 ) , 0 ,
L_49 , V_108 } } ,
{ & V_117 ,
{ L_50 , L_51 ,
V_105 , V_106 , F_12 ( V_118 ) , 0 ,
NULL , V_108 } } ,
{ & V_119 ,
{ L_52 , L_53 ,
V_110 , V_111 , NULL , 0 ,
L_54 , V_108 } } ,
{ & V_120 ,
{ L_55 , L_56 ,
V_110 , V_111 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_121 ,
{ L_47 , L_48 ,
V_122 , V_106 , F_12 ( V_10 ) , 0 ,
L_57 , V_108 } } ,
{ & V_123 ,
{ L_58 , L_59 ,
V_110 , V_111 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_124 ,
{ L_60 , L_61 ,
V_105 , V_106 , F_12 ( V_6 ) , 0 ,
NULL , V_108 } } ,
{ & V_125 ,
{ L_62 , L_63 ,
V_105 , V_106 , F_12 ( V_126 ) , 0 ,
NULL , V_108 } } ,
{ & V_127 ,
{ L_64 , L_65 ,
V_105 , V_106 , F_12 ( V_118 ) , 0 ,
L_66 , V_108 } } ,
{ & V_128 ,
{ L_67 , L_68 ,
V_105 , V_106 , NULL , 0 ,
L_69 , V_108 } } ,
{ & V_129 ,
{ L_70 , L_71 ,
V_105 , V_106 , NULL , 0 ,
L_72 , V_108 } } ,
{ & V_130 ,
{ L_73 , L_74 ,
V_110 , V_111 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_131 ,
{ L_75 , L_76 ,
V_105 , V_106 , F_12 ( V_118 ) , 0 ,
L_66 , V_108 } } ,
{ & V_132 ,
{ L_77 , L_78 ,
V_105 , V_106 , F_12 ( V_10 ) , 0 ,
L_49 , V_108 } } ,
{ & V_133 ,
{ L_79 , L_80 ,
V_105 , V_106 , F_12 ( V_134 ) , 0 ,
NULL , V_108 } } ,
{ & V_135 ,
{ L_81 , L_82 ,
V_110 , V_111 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_136 ,
{ L_83 , L_84 ,
V_112 , V_111 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_137 ,
{ L_85 , L_86 ,
V_112 , V_111 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_138 ,
{ L_87 , L_88 ,
V_112 , V_111 , NULL , 0 ,
L_89 , V_108 } } ,
{ & V_139 ,
{ L_90 , L_91 ,
V_110 , V_111 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_140 ,
{ L_92 , L_93 ,
V_110 , V_111 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_141 ,
{ L_94 , L_95 ,
V_110 , V_111 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_142 ,
{ L_96 , L_97 ,
V_112 , V_111 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_143 ,
{ L_98 , L_99 ,
V_105 , V_106 , NULL , 0 ,
L_100 , V_108 } } ,
{ & V_144 ,
{ L_101 , L_102 ,
V_105 , V_106 , NULL , 0 ,
L_103 , V_108 } } ,
{ & V_145 ,
{ L_104 , L_105 ,
V_105 , V_106 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_146 ,
{ L_106 , L_107 ,
V_105 , V_106 , NULL , 0 ,
L_108 , V_108 } } ,
{ & V_147 ,
{ L_109 , L_110 ,
V_105 , V_106 , NULL , 0 ,
L_72 , V_108 } } ,
{ & V_148 ,
{ L_111 , L_112 ,
V_110 , V_111 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_149 ,
{ L_113 , L_114 ,
V_110 , V_111 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_150 ,
{ L_115 , L_116 ,
V_110 , V_111 , NULL , 0 ,
NULL , V_108 } } ,
{ & V_151 ,
{ L_52 , L_53 ,
V_110 , V_111 , NULL , 0 ,
L_117 , V_108 } } ,
{ & V_152 ,
{ L_52 , L_53 ,
V_110 , V_111 , NULL , 0 ,
L_118 , V_108 } } ,
{ & V_153 ,
{ L_52 , L_53 ,
V_110 , V_111 , NULL , 0 ,
L_119 , V_108 } } ,
#line 155 "../../asn1/sbc-ap/packet-sbc-ap-template.c"
} ;
static T_13 * V_154 [] = {
& V_103 ,
#line 1 "../../asn1/sbc-ap/packet-sbc-ap-ettarr.c"
& V_14 ,
& V_12 ,
& V_20 ,
& V_18 ,
& V_28 ,
& V_25 ,
& V_23 ,
& V_33 ,
& V_36 ,
& V_31 ,
& V_43 ,
& V_41 ,
& V_46 ,
& V_39 ,
& V_49 ,
& V_51 ,
& V_53 ,
& V_55 ,
& V_57 ,
& V_68 ,
& V_60 ,
& V_63 ,
& V_66 ,
#line 161 "../../asn1/sbc-ap/packet-sbc-ap-template.c"
} ;
V_101 = F_94 ( V_100 , V_155 , V_156 ) ;
F_95 ( V_101 , V_104 , F_96 ( V_104 ) ) ;
F_97 ( V_154 , F_96 ( V_154 ) ) ;
V_92 = F_98 ( L_120 , L_121 , V_105 , V_106 ) ;
V_93 = F_98 ( L_122 , L_123 , V_105 , V_106 ) ;
V_94 = F_98 ( L_124 , L_125 , V_105 , V_106 ) ;
V_95 = F_98 ( L_126 , L_127 , V_105 , V_106 ) ;
V_96 = F_98 ( L_128 , L_129 , V_105 , V_106 ) ;
}
void
F_99 ( void )
{
static T_14 V_157 = FALSE ;
static T_15 V_158 ;
if( ! V_157 ) {
V_159 = F_100 ( F_89 , V_101 ) ;
F_101 ( L_130 , V_160 , V_159 ) ;
V_157 = TRUE ;
#line 1 "../../asn1/sbc-ap/packet-sbc-ap-dis-tab.c"
F_101 ( L_120 , V_161 , F_102 ( F_67 , V_101 ) ) ;
F_101 ( L_120 , V_162 , F_102 ( F_70 , V_101 ) ) ;
F_101 ( L_120 , V_163 , F_102 ( F_71 , V_101 ) ) ;
F_101 ( L_120 , V_164 , F_102 ( F_74 , V_101 ) ) ;
F_101 ( L_120 , V_165 , F_102 ( F_75 , V_101 ) ) ;
F_101 ( L_120 , V_166 , F_102 ( F_77 , V_101 ) ) ;
F_101 ( L_120 , V_167 , F_102 ( F_78 , V_101 ) ) ;
F_101 ( L_120 , V_168 , F_102 ( F_73 , V_101 ) ) ;
F_101 ( L_120 , V_169 , F_102 ( F_79 , V_101 ) ) ;
F_101 ( L_120 , V_170 , F_102 ( F_80 , V_101 ) ) ;
F_101 ( L_120 , V_171 , F_102 ( F_81 , V_101 ) ) ;
F_101 ( L_120 , V_172 , F_102 ( F_82 , V_101 ) ) ;
F_101 ( L_120 , V_173 , F_102 ( F_76 , V_101 ) ) ;
F_101 ( L_120 , V_174 , F_102 ( F_69 , V_101 ) ) ;
F_101 ( L_120 , V_175 , F_102 ( F_72 , V_101 ) ) ;
F_101 ( L_124 , V_176 , F_102 ( F_83 , V_101 ) ) ;
F_101 ( L_126 , V_176 , F_102 ( F_84 , V_101 ) ) ;
F_101 ( L_124 , V_177 , F_102 ( F_85 , V_101 ) ) ;
F_101 ( L_126 , V_177 , F_102 ( F_86 , V_101 ) ) ;
#line 194 "../../asn1/sbc-ap/packet-sbc-ap-template.c"
} else {
if ( V_158 != 0 ) {
F_103 ( L_131 , V_158 , V_159 ) ;
}
}
V_158 = V_178 ;
if ( V_158 != 0 ) {
F_101 ( L_131 , V_158 , V_159 ) ;
}
}
