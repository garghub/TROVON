static int
F_1 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_2 ( T_2 , T_7 , T_9 , T_4 , T_5 , T_10 ,
NULL ) ;
return T_5 ;
}
static int
F_3 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_2 ( T_2 , T_7 , T_9 , T_4 , T_5 , T_10 ,
NULL ) ;
return T_5 ;
}
static int
F_4 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_5 ( T_2 , T_7 , T_9 , T_4 , T_5 ,
V_2 , T_10 , V_3 ) ;
return T_5 ;
}
static int
F_6 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_7 ( T_2 , T_7 , T_9 , T_4 , T_5 , T_10 ,
NULL ) ;
return T_5 ;
}
static int
F_8 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_9 ( T_2 , T_7 , T_9 , T_4 , T_5 , T_10 , NULL ) ;
return T_5 ;
}
static int
F_10 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
#line 99 "../../asn1/t125/t125.cnf"
T_3 * V_4 = NULL ;
T_5 = F_7 ( T_2 , T_7 , T_9 , T_4 , T_5 , T_10 ,
& V_4 ) ;
if( V_4 )
F_11 ( V_5 , V_4 ,
T_7 -> V_6 , V_7 , NULL ) ;
return T_5 ;
}
static int
F_12 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_5 ( T_2 , T_7 , T_9 , T_4 , T_5 ,
V_8 , T_10 , V_9 ) ;
return T_5 ;
}
static int
F_13 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_14 ( T_2 , T_7 , T_9 , T_4 , T_5 ,
T_10 , V_10 , 101 , TRUE , F_12 ) ;
return T_5 ;
}
static int
F_15 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_2 ( T_2 , T_7 , T_9 , T_4 , T_5 , T_10 ,
NULL ) ;
return T_5 ;
}
static int
F_16 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
#line 108 "../../asn1/t125/t125.cnf"
T_3 * V_4 = NULL ;
T_5 = F_7 ( T_2 , T_7 , T_9 , T_4 , T_5 , T_10 ,
& V_4 ) ;
if( V_4 )
F_11 ( V_5 , V_4 ,
T_7 -> V_6 , V_7 , NULL ) ;
return T_5 ;
}
static int
F_17 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_5 ( T_2 , T_7 , T_9 , T_4 , T_5 ,
V_11 , T_10 , V_12 ) ;
return T_5 ;
}
static int
F_18 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_14 ( T_2 , T_7 , T_9 , T_4 , T_5 ,
T_10 , V_10 , 102 , TRUE , F_17 ) ;
return T_5 ;
}
static int
F_19 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_5 ( T_2 , T_7 , T_9 , T_4 , T_5 ,
V_13 , T_10 , V_14 ) ;
return T_5 ;
}
static int
F_20 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_14 ( T_2 , T_7 , T_9 , T_4 , T_5 ,
T_10 , V_10 , 103 , TRUE , F_19 ) ;
return T_5 ;
}
static int
F_21 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_5 ( T_2 , T_7 , T_9 , T_4 , T_5 ,
V_15 , T_10 , V_16 ) ;
return T_5 ;
}
static int
F_22 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_14 ( T_2 , T_7 , T_9 , T_4 , T_5 ,
T_10 , V_10 , 104 , TRUE , F_21 ) ;
return T_5 ;
}
static int
F_23 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_24 ( T_7 , T_9 , T_4 , T_5 ,
V_17 , T_10 , V_18 ,
NULL ) ;
return T_5 ;
}
static int F_25 ( T_3 * T_4 V_1 , T_11 * V_6 V_1 , T_8 * T_9 V_1 , void * T_12 V_1 ) {
int T_5 = 0 ;
T_6 V_19 ;
F_26 ( & V_19 , V_20 , TRUE , V_6 ) ;
T_5 = F_23 ( FALSE , T_4 , T_5 , & V_19 , T_9 , V_21 ) ;
return T_5 ;
}
static int
F_27 ( T_3 * T_4 , T_11 * V_6 , T_8 * V_22 , void * T_12 V_1 )
{
T_13 * V_23 = NULL ;
T_8 * T_9 = NULL ;
T_14 V_24 ;
T_1 V_25 ;
T_15 V_26 ;
V_7 = V_22 ;
F_28 ( V_6 -> V_27 , V_28 , L_1 ) ;
F_29 ( V_6 -> V_27 , V_29 ) ;
V_23 = F_30 ( V_22 , V_30 , T_4 , 0 , F_31 ( T_4 ) , V_31 ) ;
T_9 = F_32 ( V_23 , V_32 ) ;
F_33 ( T_4 , 0 , & V_24 , & V_25 , & V_26 ) ;
if ( ( V_24 == V_10 ) && ( V_26 >= 101 ) && ( V_26 <= 104 ) ) {
F_25 ( T_4 , V_6 , T_9 , NULL ) ;
} else {
F_34 ( V_7 ) ;
F_35 ( T_4 , V_6 , T_9 ) ;
}
return F_31 ( T_4 ) ;
}
static T_1
F_36 ( T_3 * T_4 , T_11 * V_6 , T_8 * V_22 , void * T_12 V_1 )
{
T_14 V_24 ;
T_1 V_25 ;
T_15 V_26 ;
T_16 V_33 = 100 ;
T_6 V_19 ;
T_1 V_34 ;
F_26 ( & V_19 , V_35 , TRUE , V_6 ) ;
V_34 = FALSE ;
F_37 {
F_33 ( T_4 , 0 , & V_24 , & V_25 , & V_26 ) ;
} F_38 {
V_34 = TRUE ;
} V_36 ;
if ( ! V_34 && ( ( V_24 == V_10 ) && ( ( V_26 >= 101 ) && ( V_26 <= 104 ) ) ) ) {
F_27 ( T_4 , V_6 , V_22 , NULL ) ;
return TRUE ;
}
V_34 = FALSE ;
F_37 {
F_39 ( T_4 , 0 , & V_19 ,
NULL , V_37 , 0 , 42 ,
& V_33 , FALSE ) ;
} F_38 {
V_34 = TRUE ;
} V_36 ;
if ( ! V_34 && ( V_33 <= 42 ) ) {
F_27 ( T_4 , V_6 , V_22 , NULL ) ;
return TRUE ;
}
return FALSE ;
}
void F_40 ( void ) {
static T_17 V_38 [] = {
{ & V_39 ,
{ L_2 , L_3 ,
V_40 , V_41 , NULL , 0 ,
NULL , V_42 } } ,
{ & V_37 ,
{ L_4 , L_5 ,
V_43 , V_44 , NULL , 0 ,
NULL , V_42 } } ,
#line 1 "../../asn1/t125/packet-t125-hfarr.c"
{ & V_21 ,
{ L_6 , L_7 ,
V_43 , V_44 , F_41 ( V_45 ) , 0 ,
NULL , V_42 } } ,
{ & V_46 ,
{ L_8 , L_9 ,
V_43 , V_44 , NULL , 0 ,
L_10 , V_42 } } ,
{ & V_47 ,
{ L_11 , L_12 ,
V_43 , V_44 , NULL , 0 ,
L_10 , V_42 } } ,
{ & V_48 ,
{ L_13 , L_14 ,
V_43 , V_44 , NULL , 0 ,
L_10 , V_42 } } ,
{ & V_49 ,
{ L_15 , L_16 ,
V_43 , V_44 , NULL , 0 ,
L_10 , V_42 } } ,
{ & V_50 ,
{ L_17 , L_18 ,
V_43 , V_44 , NULL , 0 ,
L_10 , V_42 } } ,
{ & V_51 ,
{ L_19 , L_20 ,
V_43 , V_44 , NULL , 0 ,
L_10 , V_42 } } ,
{ & V_52 ,
{ L_21 , L_22 ,
V_43 , V_44 , NULL , 0 ,
L_10 , V_42 } } ,
{ & V_53 ,
{ L_23 , L_24 ,
V_43 , V_44 , NULL , 0 ,
L_10 , V_42 } } ,
{ & V_54 ,
{ L_25 , L_26 ,
V_55 , V_41 , NULL , 0 ,
L_27 , V_42 } } ,
{ & V_56 ,
{ L_28 , L_29 ,
V_55 , V_41 , NULL , 0 ,
L_27 , V_42 } } ,
{ & V_57 ,
{ L_30 , L_31 ,
V_58 , V_41 , NULL , 0 ,
L_32 , V_42 } } ,
{ & V_59 ,
{ L_33 , L_34 ,
V_40 , V_41 , NULL , 0 ,
L_35 , V_42 } } ,
{ & V_60 ,
{ L_36 , L_37 ,
V_40 , V_41 , NULL , 0 ,
L_35 , V_42 } } ,
{ & V_61 ,
{ L_38 , L_39 ,
V_40 , V_41 , NULL , 0 ,
L_35 , V_42 } } ,
{ & V_62 ,
{ L_40 , L_41 ,
V_55 , V_41 , NULL , 0 ,
NULL , V_42 } } ,
{ & V_63 ,
{ L_42 , L_43 ,
V_43 , V_44 , F_41 ( V_64 ) , 0 ,
NULL , V_42 } } ,
{ & V_65 ,
{ L_44 , L_45 ,
V_43 , V_44 , NULL , 0 ,
L_10 , V_42 } } ,
{ & V_66 ,
{ L_46 , L_47 ,
V_40 , V_41 , NULL , 0 ,
NULL , V_42 } } ,
{ & V_67 ,
{ L_40 , L_41 ,
V_55 , V_41 , NULL , 0 ,
L_48 , V_42 } } ,
{ & V_68 ,
{ L_49 , L_50 ,
V_43 , V_44 , F_41 ( V_69 ) , 0 ,
NULL , V_42 } } ,
{ & V_70 ,
{ L_51 , L_52 ,
V_40 , V_41 , NULL , 0 ,
NULL , V_42 } } ,
{ & V_71 ,
{ L_53 , L_54 ,
V_40 , V_41 , NULL , 0 ,
NULL , V_42 } } ,
{ & V_72 ,
{ L_55 , L_56 ,
V_40 , V_41 , NULL , 0 ,
NULL , V_42 } } ,
{ & V_73 ,
{ L_57 , L_58 ,
V_40 , V_41 , NULL , 0 ,
NULL , V_42 } } ,
#line 160 "../../asn1/t125/packet-t125-template.c"
} ;
static T_18 * V_74 [] = {
& V_32 ,
#line 1 "../../asn1/t125/packet-t125-ettarr.c"
& V_3 ,
& V_9 ,
& V_12 ,
& V_14 ,
& V_16 ,
& V_18 ,
#line 166 "../../asn1/t125/packet-t125-template.c"
} ;
V_30 = F_42 ( V_75 , V_76 , V_77 ) ;
F_43 ( V_30 , V_38 , F_44 ( V_38 ) ) ;
F_45 ( V_74 , F_44 ( V_74 ) ) ;
F_46 ( L_59 , & V_5 ) ;
F_47 ( L_59 , F_27 , V_30 ) ;
}
void F_48 ( void ) {
F_49 ( L_60 , F_36 , V_30 ) ;
F_49 ( L_61 , F_36 , V_30 ) ;
}
