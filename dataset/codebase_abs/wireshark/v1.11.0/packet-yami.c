static int
F_1 ( T_1 * V_1 , T_2 * V_2 , int V_3 , T_3 * V_4 )
{
const int V_5 = V_3 ;
T_2 * V_6 ;
T_3 * V_7 ;
char * V_8 ;
int V_9 ;
T_4 V_10 ;
T_4 type ;
V_7 = F_2 ( V_2 , & V_11 , V_1 , V_3 , 0 , V_12 ) ;
V_6 = F_3 ( V_7 , V_13 ) ;
V_9 = V_3 ;
V_10 = F_4 ( V_1 , V_3 ) ;
V_3 += 4 ;
V_8 = F_5 ( F_6 () , V_1 , V_3 , V_10 , V_14 | V_12 ) ;
F_7 ( V_7 , L_1 , V_8 ) ;
F_7 ( V_4 , L_2 , V_8 ) ;
V_3 += ( V_10 + 3 ) & ~ 3 ;
F_8 ( V_6 , & V_15 , V_1 , V_9 , V_3 - V_9 , V_8 ) ;
type = F_4 ( V_1 , V_3 ) ;
F_2 ( V_6 , & V_16 , V_1 , V_3 , 4 , V_17 ) ;
V_3 += 4 ;
switch ( type ) {
case V_18 :
{
T_4 V_19 = F_4 ( V_1 , V_3 ) ;
F_7 ( V_7 , L_3 , V_19 ? L_4 : L_5 ) ;
F_2 ( V_6 , & V_20 , V_1 , V_3 , 4 , V_17 ) ;
V_3 += 4 ;
break;
}
case V_21 :
{
T_5 V_19 = F_4 ( V_1 , V_3 ) ;
F_7 ( V_7 , L_6 , V_19 ) ;
F_2 ( V_6 , & V_22 , V_1 , V_3 , 4 , V_17 ) ;
V_3 += 4 ;
break;
}
case V_23 :
{
T_6 V_19 = F_9 ( V_1 , V_3 ) ;
F_7 ( V_7 , L_7 V_24 L_8 , V_19 ) ;
F_2 ( V_6 , & V_25 , V_1 , V_3 , 8 , V_17 ) ;
V_3 += 8 ;
break;
}
case V_26 :
{
T_7 V_19 = F_10 ( V_1 , V_3 ) ;
F_7 ( V_7 , L_9 , V_19 ) ;
F_2 ( V_6 , & V_27 , V_1 , V_3 , 8 , V_17 ) ;
V_3 += 8 ;
break;
}
case V_28 :
{
const int V_29 = V_3 ;
T_4 V_30 ;
char * V_19 ;
V_30 = F_4 ( V_1 , V_3 ) ;
V_3 += 4 ;
V_19 = F_5 ( F_6 () , V_1 , V_3 , V_30 , V_14 | V_12 ) ;
F_7 ( V_7 , L_10 , V_19 ) ;
V_3 += ( V_30 + 3 ) & ~ 3 ;
F_8 ( V_6 , & V_31 , V_1 , V_29 , V_3 - V_29 , V_19 ) ;
break;
}
case V_32 :
{
const int V_29 = V_3 ;
T_4 V_30 ;
const T_8 * V_19 ;
char * V_33 ;
V_30 = F_4 ( V_1 , V_3 ) ;
V_3 += 4 ;
V_19 = F_11 ( V_1 , V_3 , V_30 ) ;
V_33 = F_12 ( V_19 , V_30 ) ;
F_7 ( V_7 , L_11 , V_33 ) ;
V_3 += ( V_30 + 3 ) & ~ 3 ;
F_13 ( V_6 , V_34 . V_35 , V_1 , V_29 , V_3 - V_29 , V_19 , L_12 , V_33 ) ;
break;
}
case V_36 :
{
T_4 V_37 ;
T_9 V_38 ;
int V_39 ;
V_37 = F_4 ( V_1 , V_3 ) ;
F_2 ( V_6 , & V_40 , V_1 , V_3 , 4 , V_17 ) ;
V_3 += 4 ;
F_7 ( V_7 , L_13 , V_37 ) ;
for ( V_38 = 0 ; V_38 < V_37 / 32 ; V_38 ++ ) {
T_4 V_19 = F_4 ( V_1 , V_3 ) ;
for ( V_39 = 0 ; V_39 < 32 ; V_39 ++ ) {
int V_41 = ! ! ( V_19 & ( 1 << V_39 ) ) ;
F_7 ( V_7 , L_2 , V_41 ? L_14 : L_15 ) ;
F_14 ( V_6 , & V_20 , V_1 , V_3 + ( V_39 / 8 ) , 1 , V_41 ) ;
}
V_3 += 4 ;
}
if ( V_37 % 32 ) {
T_4 V_19 = F_4 ( V_1 , V_3 ) ;
int V_42 = V_37 % 32 ;
for ( V_39 = 0 ; V_39 < V_42 ; V_39 ++ ) {
int V_41 = ! ! ( V_19 & ( 1 << V_39 ) ) ;
F_7 ( V_7 , L_2 , V_41 ? L_14 : L_15 ) ;
F_14 ( V_6 , & V_20 , V_1 , V_3 + ( V_39 / 8 ) , 1 , V_41 ) ;
}
V_3 += 4 ;
}
F_7 ( V_7 , L_16 ) ;
break;
}
case V_43 :
{
T_4 V_37 ;
T_9 V_38 ;
V_37 = F_4 ( V_1 , V_3 ) ;
F_2 ( V_6 , & V_40 , V_1 , V_3 , 4 , V_17 ) ;
V_3 += 4 ;
F_7 ( V_7 , L_17 , V_37 ) ;
for ( V_38 = 0 ; V_38 < V_37 ; V_38 ++ ) {
T_5 V_19 = F_4 ( V_1 , V_3 ) ;
F_7 ( V_7 , L_18 , V_19 ) ;
F_2 ( V_6 , & V_22 , V_1 , V_3 , 4 , V_17 ) ;
V_3 += 4 ;
}
F_7 ( V_7 , L_16 ) ;
break;
}
case V_44 :
{
T_4 V_37 ;
T_9 V_38 ;
V_37 = F_4 ( V_1 , V_3 ) ;
F_2 ( V_6 , & V_40 , V_1 , V_3 , 4 , V_17 ) ;
V_3 += 4 ;
F_7 ( V_7 , L_19 , V_37 ) ;
for ( V_38 = 0 ; V_38 < V_37 ; V_38 ++ ) {
T_6 V_19 = F_9 ( V_1 , V_3 ) ;
F_7 ( V_7 , L_20 V_24 L_21 , V_19 ) ;
F_2 ( V_6 , & V_25 , V_1 , V_3 , 8 , V_17 ) ;
V_3 += 8 ;
}
F_7 ( V_7 , L_16 ) ;
break;
}
case V_45 :
{
T_4 V_37 ;
T_9 V_38 ;
V_37 = F_4 ( V_1 , V_3 ) ;
F_2 ( V_6 , & V_40 , V_1 , V_3 , 4 , V_17 ) ;
V_3 += 4 ;
F_7 ( V_7 , L_22 , V_37 ) ;
for ( V_38 = 0 ; V_38 < V_37 ; V_38 ++ ) {
T_7 V_19 = F_10 ( V_1 , V_3 ) ;
F_7 ( V_7 , L_23 , V_19 ) ;
F_2 ( V_6 , & V_27 , V_1 , V_3 , 8 , V_17 ) ;
V_3 += 8 ;
}
F_7 ( V_7 , L_16 ) ;
break;
}
case V_46 :
{
T_4 V_37 ;
T_9 V_38 ;
V_37 = F_4 ( V_1 , V_3 ) ;
F_2 ( V_6 , & V_40 , V_1 , V_3 , 4 , V_17 ) ;
V_3 += 4 ;
F_7 ( V_7 , L_24 , V_37 ) ;
for ( V_38 = 0 ; V_38 < V_37 ; V_38 ++ ) {
const int V_29 = V_3 ;
T_4 V_30 ;
char * V_19 ;
V_30 = F_4 ( V_1 , V_3 ) ;
V_3 += 4 ;
V_19 = F_5 ( F_6 () , V_1 , V_3 , V_30 , V_14 | V_12 ) ;
F_7 ( V_7 , L_25 , V_19 ) ;
F_8 ( V_6 , & V_31 , V_1 , V_29 , V_3 - V_29 , V_19 ) ;
V_3 += ( V_30 + 3 ) & ~ 3 ;
}
F_7 ( V_7 , L_16 ) ;
break;
}
case V_47 :
{
T_4 V_37 ;
T_9 V_38 ;
V_37 = F_4 ( V_1 , V_3 ) ;
F_2 ( V_6 , & V_40 , V_1 , V_3 , 4 , V_17 ) ;
V_3 += 4 ;
F_7 ( V_7 , L_26 , V_37 ) ;
for ( V_38 = 0 ; V_38 < V_37 ; V_38 ++ ) {
const int V_29 = V_3 ;
T_4 V_30 ;
const T_8 * V_19 ;
char * V_33 ;
V_30 = F_4 ( V_1 , V_3 ) ;
V_3 += 4 ;
V_19 = F_11 ( V_1 , V_3 , V_30 ) ;
V_33 = F_12 ( V_19 , V_30 ) ;
F_7 ( V_7 , L_2 , V_33 ) ;
V_3 += ( V_30 + 3 ) & ~ 3 ;
F_13 ( V_6 , V_34 . V_35 , V_1 , V_29 , V_3 - V_29 , V_19 , L_12 , V_33 ) ;
}
F_7 ( V_7 , L_16 ) ;
break;
}
case V_48 :
{
T_4 V_37 ;
T_9 V_38 ;
V_37 = F_4 ( V_1 , V_3 ) ;
F_2 ( V_6 , & V_49 , V_1 , V_3 , 4 , V_17 ) ;
V_3 += 4 ;
F_7 ( V_7 , L_27 , V_37 ) ;
for ( V_38 = 0 ; V_38 < V_37 ; V_38 ++ ) {
V_3 = F_1 ( V_1 , V_6 , V_3 , V_7 ) ;
if ( V_3 == - 1 )
return - 1 ;
}
break;
}
default:
F_7 ( V_7 , L_28 , type ) ;
return - 1 ;
}
F_15 ( V_7 , V_3 - V_5 ) ;
return V_3 ;
}
static int
F_16 ( T_1 * V_1 , T_10 V_50 , T_2 * V_2 , int V_3 )
{
const int V_5 = V_3 ;
T_2 * V_51 ;
T_3 * V_7 ;
T_4 V_37 ;
T_9 V_38 ;
V_7 = F_2 ( V_2 , ( V_50 ) ? & V_52 : & V_53 , V_1 , V_3 , 0 , V_12 ) ;
V_51 = F_3 ( V_7 , ( V_50 ) ? V_54 : V_55 ) ;
V_37 = F_4 ( V_1 , V_3 ) ;
F_2 ( V_51 , & V_49 , V_1 , V_3 , 4 , V_17 ) ;
V_3 += 4 ;
F_7 ( V_7 , L_29 , V_37 ) ;
for ( V_38 = 0 ; V_38 < V_37 ; V_38 ++ ) {
V_3 = F_1 ( V_1 , V_51 , V_3 , V_7 ) ;
if ( V_3 == - 1 )
return - 1 ;
}
F_15 ( V_7 , V_3 - V_5 ) ;
return V_3 ;
}
static void
F_17 ( T_1 * V_1 , T_11 * V_56 , T_2 * V_2 )
{
T_2 * V_57 = NULL ;
T_3 * V_7 ;
T_12 V_58 ;
T_12 V_59 ;
T_12 V_60 ;
T_12 V_61 ;
int V_3 ;
F_18 ( V_56 -> V_62 , V_63 , L_30 ) ;
F_19 ( V_56 -> V_62 , V_64 ) ;
if ( V_2 ) {
V_7 = F_2 ( V_2 , V_65 , V_1 , 0 , - 1 , V_12 ) ;
V_57 = F_3 ( V_7 , V_66 ) ;
}
V_3 = 0 ;
F_2 ( V_57 , & V_67 , V_1 , V_3 , 4 , V_17 ) ;
V_3 += 4 ;
V_58 = F_4 ( V_1 , V_3 ) ;
V_7 = F_2 ( V_57 , & V_68 , V_1 , V_3 , 4 , V_17 ) ;
if( V_58 < 0 )
F_7 ( V_7 , L_12 , L_31 ) ;
V_3 += 4 ;
V_59 = F_4 ( V_1 , V_3 ) ;
F_2 ( V_57 , & V_69 , V_1 , V_3 , 4 , V_17 ) ;
if ( V_59 < 4 ) {
}
V_3 += 4 ;
V_60 = F_4 ( V_1 , V_3 ) ;
V_7 = F_2 ( V_57 , & V_70 , V_1 , V_3 , 4 , V_17 ) ;
V_61 = V_60 + 16 ;
F_7 ( V_7 , L_32 , V_61 ) ;
V_3 += 4 ;
if ( V_58 == 1 || V_58 == - 1 ) {
if ( V_59 <= V_60 ) {
const int V_5 = V_3 ;
V_3 = F_16 ( V_1 , FALSE , V_57 , V_3 ) ;
if ( V_3 != V_5 + V_59 ) {
V_3 = V_5 + V_59 ;
}
F_16 ( V_1 , TRUE , V_57 , V_3 ) ;
}
}
}
static T_9
F_20 ( T_11 * V_56 V_71 , T_1 * V_1 , int V_3 )
{
T_4 V_72 = F_4 ( V_1 , V_3 + 12 ) ;
return V_72 + V_73 ;
}
static int
F_21 ( T_1 * V_1 , T_11 * V_56 , T_2 * V_2 , void * V_50 V_71 )
{
F_22 ( V_1 , V_56 , V_2 , V_74 , V_73 , F_20 , F_17 ) ;
return F_23 ( V_1 ) ;
}
void
F_24 ( void )
{
static T_13 * V_75 [] = {
& V_67 ,
& V_68 ,
& V_69 ,
& V_70 ,
& V_53 ,
& V_52 ,
& V_11 ,
& V_15 ,
& V_16 ,
& V_20 ,
& V_22 ,
& V_25 ,
& V_27 ,
& V_31 ,
& V_34 ,
& V_49 ,
& V_40 ,
} ;
static T_12 * V_76 [] = {
& V_66 ,
& V_55 ,
& V_54 ,
& V_13
} ;
T_14 * V_77 ;
int V_78 ;
V_78 = F_25 ( L_33 , L_30 , L_34 ) ;
V_65 = F_26 ( V_78 ) ;
F_27 ( V_78 , V_75 , F_28 ( V_75 ) ) ;
F_29 ( V_76 , F_28 ( V_76 ) ) ;
V_77 = F_30 ( V_78 , V_79 ) ;
F_31 ( V_77 , L_35 , L_36 , L_37 , 10 , & V_80 ) ;
F_31 ( V_77 , L_38 , L_39 , L_40 , 10 , & V_81 ) ;
F_32 ( V_77 , L_41 ,
L_42 ,
L_43
L_44 ,
& V_74 ) ;
V_82 = F_33 ( F_21 , V_78 ) ;
}
void
V_79 ( void )
{
static int V_83 = FALSE ;
static T_9 V_84 , V_85 ;
if( V_83 == FALSE ) {
V_83 = TRUE ;
V_84 = V_80 ;
V_85 = V_81 ;
} else{
F_34 ( L_35 , V_84 , V_82 ) ;
F_34 ( L_38 , V_85 , V_82 ) ;
}
V_84 = V_80 ;
V_85 = V_81 ;
F_35 ( L_35 , V_84 , V_82 ) ;
F_35 ( L_38 , V_85 , V_82 ) ;
}
