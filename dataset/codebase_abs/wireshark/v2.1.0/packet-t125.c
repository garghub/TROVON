static int
F_1 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_2 ( T_2 , T_7 , T_9 , T_4 , T_5 , T_10 ,
NULL ) ;
return T_5 ;
}
static int
F_3 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_2 ( T_2 , T_7 , T_9 , T_4 , T_5 , T_10 ,
NULL ) ;
return T_5 ;
}
static int
F_4 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_5 ( T_2 , T_7 , T_9 , T_4 , T_5 ,
V_2 , T_10 , V_3 ) ;
return T_5 ;
}
static int
F_6 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_7 ( T_2 , T_7 , T_9 , T_4 , T_5 , T_10 ,
NULL ) ;
return T_5 ;
}
static int
F_8 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_9 ( T_2 , T_7 , T_9 , T_4 , T_5 , T_10 , NULL ) ;
return T_5 ;
}
static int
F_10 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
#line 98 "./asn1/t125/t125.cnf"
T_3 * V_4 = NULL ;
T_11 * V_5 ;
T_5 = F_7 ( T_2 , T_7 , T_9 , T_4 , T_5 , T_10 ,
& V_4 ) ;
if( V_4 )
F_11 ( V_6 , V_4 ,
T_7 -> V_7 , V_8 , & V_5 , NULL ) ;
return T_5 ;
}
static int
F_12 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_5 ( T_2 , T_7 , T_9 , T_4 , T_5 ,
V_9 , T_10 , V_10 ) ;
return T_5 ;
}
static int
F_13 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_14 ( T_2 , T_7 , T_9 , T_4 , T_5 ,
T_10 , V_11 , 101 , TRUE , F_12 ) ;
return T_5 ;
}
static int
F_15 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_2 ( T_2 , T_7 , T_9 , T_4 , T_5 , T_10 ,
NULL ) ;
return T_5 ;
}
static int
F_16 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
#line 108 "./asn1/t125/t125.cnf"
T_3 * V_4 = NULL ;
T_11 * V_5 ;
T_5 = F_7 ( T_2 , T_7 , T_9 , T_4 , T_5 , T_10 ,
& V_4 ) ;
if( V_4 )
F_11 ( V_6 , V_4 ,
T_7 -> V_7 , V_8 , & V_5 , NULL ) ;
return T_5 ;
}
static int
F_17 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_5 ( T_2 , T_7 , T_9 , T_4 , T_5 ,
V_12 , T_10 , V_13 ) ;
return T_5 ;
}
static int
F_18 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_14 ( T_2 , T_7 , T_9 , T_4 , T_5 ,
T_10 , V_11 , 102 , TRUE , F_17 ) ;
return T_5 ;
}
static int
F_19 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_5 ( T_2 , T_7 , T_9 , T_4 , T_5 ,
V_14 , T_10 , V_15 ) ;
return T_5 ;
}
static int
F_20 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_14 ( T_2 , T_7 , T_9 , T_4 , T_5 ,
T_10 , V_11 , 103 , TRUE , F_19 ) ;
return T_5 ;
}
static int
F_21 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_5 ( T_2 , T_7 , T_9 , T_4 , T_5 ,
V_16 , T_10 , V_17 ) ;
return T_5 ;
}
static int
F_22 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_14 ( T_2 , T_7 , T_9 , T_4 , T_5 ,
T_10 , V_11 , 104 , TRUE , F_21 ) ;
return T_5 ;
}
static int
F_23 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_24 ( T_7 , T_9 , T_4 , T_5 ,
V_18 , T_10 , V_19 ,
NULL ) ;
return T_5 ;
}
static int F_25 ( T_3 * T_4 V_1 , T_12 * V_7 V_1 , T_8 * T_9 V_1 , void * T_13 V_1 ) {
int T_5 = 0 ;
T_6 V_20 ;
F_26 ( & V_20 , V_21 , TRUE , V_7 ) ;
T_5 = F_23 ( FALSE , T_4 , T_5 , & V_20 , T_9 , V_22 ) ;
return T_5 ;
}
static int
F_27 ( T_3 * T_4 , T_12 * V_7 , T_8 * V_23 , void * T_13 V_1 )
{
T_14 * V_24 = NULL ;
T_8 * T_9 = NULL ;
T_15 V_25 ;
T_1 V_26 ;
T_16 V_27 ;
V_8 = V_23 ;
F_28 ( V_7 -> V_28 , V_29 , L_1 ) ;
F_29 ( V_7 -> V_28 , V_30 ) ;
V_24 = F_30 ( V_23 , V_31 , T_4 , 0 , F_31 ( T_4 ) , V_32 ) ;
T_9 = F_32 ( V_24 , V_33 ) ;
F_33 ( T_4 , 0 , & V_25 , & V_26 , & V_27 ) ;
if ( ( V_25 == V_11 ) && ( V_27 >= 101 ) && ( V_27 <= 104 ) ) {
F_25 ( T_4 , V_7 , T_9 , NULL ) ;
} else {
F_34 ( V_8 ) ;
F_35 ( T_4 , V_7 , T_9 ) ;
}
return F_31 ( T_4 ) ;
}
static T_1
F_36 ( T_3 * T_4 , T_12 * V_7 , T_8 * V_23 , void * T_13 V_1 )
{
T_15 V_25 ;
T_1 V_26 ;
T_16 V_27 ;
volatile T_1 V_34 ;
V_34 = FALSE ;
F_37 {
F_33 ( T_4 , 0 , & V_25 , & V_26 , & V_27 ) ;
} F_38 {
V_34 = TRUE ;
} V_35 ;
if ( ! V_34 && ( ( V_25 == V_11 ) && ( ( V_27 >= 101 ) && ( V_27 <= 104 ) ) ) ) {
F_27 ( T_4 , V_7 , V_23 , NULL ) ;
return TRUE ;
}
return FALSE ;
}
void F_39 ( void ) {
static T_17 V_36 [] = {
#line 1 "./asn1/t125/packet-t125-hfarr.c"
{ & V_22 ,
{ L_2 , L_3 ,
V_37 , V_38 , F_40 ( V_39 ) , 0 ,
NULL , V_40 } } ,
{ & V_41 ,
{ L_4 , L_5 ,
V_37 , V_38 , NULL , 0 ,
L_6 , V_40 } } ,
{ & V_42 ,
{ L_7 , L_8 ,
V_37 , V_38 , NULL , 0 ,
L_6 , V_40 } } ,
{ & V_43 ,
{ L_9 , L_10 ,
V_37 , V_38 , NULL , 0 ,
L_6 , V_40 } } ,
{ & V_44 ,
{ L_11 , L_12 ,
V_37 , V_38 , NULL , 0 ,
L_6 , V_40 } } ,
{ & V_45 ,
{ L_13 , L_14 ,
V_37 , V_38 , NULL , 0 ,
L_6 , V_40 } } ,
{ & V_46 ,
{ L_15 , L_16 ,
V_37 , V_38 , NULL , 0 ,
L_6 , V_40 } } ,
{ & V_47 ,
{ L_17 , L_18 ,
V_37 , V_38 , NULL , 0 ,
L_6 , V_40 } } ,
{ & V_48 ,
{ L_19 , L_20 ,
V_37 , V_38 , NULL , 0 ,
L_6 , V_40 } } ,
{ & V_49 ,
{ L_21 , L_22 ,
V_50 , V_51 , NULL , 0 ,
L_23 , V_40 } } ,
{ & V_52 ,
{ L_24 , L_25 ,
V_50 , V_51 , NULL , 0 ,
L_23 , V_40 } } ,
{ & V_53 ,
{ L_26 , L_27 ,
V_54 , V_51 , NULL , 0 ,
L_28 , V_40 } } ,
{ & V_55 ,
{ L_29 , L_30 ,
V_56 , V_51 , NULL , 0 ,
L_31 , V_40 } } ,
{ & V_57 ,
{ L_32 , L_33 ,
V_56 , V_51 , NULL , 0 ,
L_31 , V_40 } } ,
{ & V_58 ,
{ L_34 , L_35 ,
V_56 , V_51 , NULL , 0 ,
L_31 , V_40 } } ,
{ & V_59 ,
{ L_36 , L_37 ,
V_50 , V_51 , NULL , 0 ,
NULL , V_40 } } ,
{ & V_60 ,
{ L_38 , L_39 ,
V_37 , V_38 , F_40 ( V_61 ) , 0 ,
NULL , V_40 } } ,
{ & V_62 ,
{ L_40 , L_41 ,
V_37 , V_38 , NULL , 0 ,
L_6 , V_40 } } ,
{ & V_63 ,
{ L_42 , L_43 ,
V_56 , V_51 , NULL , 0 ,
NULL , V_40 } } ,
{ & V_64 ,
{ L_36 , L_37 ,
V_50 , V_51 , NULL , 0 ,
L_44 , V_40 } } ,
{ & V_65 ,
{ L_45 , L_46 ,
V_37 , V_38 , F_40 ( V_66 ) , 0 ,
NULL , V_40 } } ,
{ & V_67 ,
{ L_47 , L_48 ,
V_56 , V_51 , NULL , 0 ,
NULL , V_40 } } ,
{ & V_68 ,
{ L_49 , L_50 ,
V_56 , V_51 , NULL , 0 ,
NULL , V_40 } } ,
{ & V_69 ,
{ L_51 , L_52 ,
V_56 , V_51 , NULL , 0 ,
NULL , V_40 } } ,
{ & V_70 ,
{ L_53 , L_54 ,
V_56 , V_51 , NULL , 0 ,
NULL , V_40 } } ,
#line 126 "./asn1/t125/packet-t125-template.c"
} ;
static T_18 * V_71 [] = {
& V_33 ,
#line 1 "./asn1/t125/packet-t125-ettarr.c"
& V_3 ,
& V_10 ,
& V_13 ,
& V_15 ,
& V_17 ,
& V_19 ,
#line 132 "./asn1/t125/packet-t125-template.c"
} ;
V_31 = F_41 ( V_72 , V_73 , V_74 ) ;
F_42 ( V_31 , V_36 , F_43 ( V_36 ) ) ;
F_44 ( V_71 , F_43 ( V_71 ) ) ;
V_6 = F_45 ( L_55 , V_31 ) ;
F_46 ( L_55 , F_27 , V_31 ) ;
}
void F_47 ( void ) {
F_48 ( L_56 , F_36 , L_57 , L_58 , V_31 , V_75 ) ;
F_48 ( L_59 , F_36 , L_60 , L_61 , V_31 , V_75 ) ;
}
