static T_1 F_1 ( T_2 * V_1 , T_3 * V_2 , T_1 V_3 , T_4 V_4 , T_5 * V_5 )
{
T_4 V_6 = 0 , V_7 ;
T_4 V_8 , V_9 , V_10 , V_11 , V_12 , V_13 ;
T_6 V_14 ;
struct V_15 * V_16 ;
struct V_17 * V_18 ;
V_16 = ( V_15 * ) F_2 ( F_3 () , V_2 , V_19 , 0 ) ;
V_18 = ( V_17 * ) F_2 ( F_3 () , V_2 , V_20 , 0 ) ;
if ( ! V_16 ) {
V_16 = F_4 ( F_3 () , struct V_15 ) ;
F_5 ( F_3 () , V_2 , V_19 , 0 , V_16 ) ;
}
if ( ! V_18 ) {
V_18 = F_4 ( F_3 () , struct V_17 ) ;
F_5 ( F_3 () , V_2 , V_20 , 0 , V_18 ) ;
}
while ( V_6 < V_4 ) {
V_14 = F_6 ( V_1 , V_3 ) ;
V_7 = F_7 ( V_1 , V_3 + 4 ) ;
V_8 = V_7 & 0x3 ;
V_9 = ( V_7 >> 2 ) & 0x3 ;
V_10 = V_7 >> 4 ;
V_7 = F_7 ( V_1 , V_3 + 5 ) ;
V_10 |= ( V_7 & 0x01 ) << 4 ;
V_11 = ( V_7 >> 1 ) & 0x1 ;
V_12 = ( V_7 >> 2 ) & 0x1 ;
V_13 = ( V_7 >> 3 ) & 0x1 ;
if ( V_6 >= V_21 ) {
F_8 ( V_5 , V_2 , & V_22 , V_1 , V_3 , - 1 ,
L_1 ,
V_4 , V_21 ) ;
return - 1 ;
}
if ( V_6 >= V_23 ) {
F_8 ( V_5 , V_2 , & V_24 , V_1 , V_3 , - 1 ,
L_2 ,
V_4 , V_23 ) ;
return - 1 ;
}
V_18 -> V_25 [ V_6 ] = V_8 ;
V_18 -> V_26 [ V_6 ] = V_10 ;
V_18 -> V_14 [ V_6 ] = V_14 ;
V_18 -> V_12 [ V_6 ] = V_12 ;
V_18 -> V_13 [ V_6 ] = V_13 ;
V_18 -> V_27 [ V_6 ] = V_28 ;
V_16 -> V_11 [ V_6 ] = V_11 ? TRUE : FALSE ;
switch ( V_9 ) {
case V_29 :
V_16 -> V_9 [ V_6 ] = V_30 ;
break;
case V_31 :
V_16 -> V_9 [ V_6 ] = V_32 ;
break;
case V_33 :
V_16 -> V_9 [ V_6 ] = V_34 ;
break;
default:
V_16 -> V_9 [ V_6 ] = V_35 ;
}
if ( V_5 ) {
T_5 * V_36 ;
T_7 * V_37 ;
V_37 = F_9 ( V_5 , V_38 , V_1 , V_3 , 8 , V_39 ) ;
V_36 = F_10 ( V_37 , V_40 ) ;
if ( V_14 )
F_11 ( V_36 , V_41 , V_1 , V_3 , 4 , V_14 ) ;
F_12 ( V_36 , V_42 , V_1 , ( V_3 + 4 ) * 8 + 4 , 2 , V_43 ) ;
F_12 ( V_36 , V_44 , V_1 , ( V_3 + 4 ) * 8 + 6 , 2 , V_43 ) ;
F_9 ( V_36 , V_45 , V_1 , ( V_3 + 4 ) , 2 , V_43 ) ;
F_13 ( V_36 , V_46 , V_1 , V_3 + 5 , 1 , V_11 ) ;
F_13 ( V_36 , V_47 , V_1 , V_3 + 5 , 1 , V_12 ) ;
F_13 ( V_36 , V_48 , V_1 , V_3 + 5 , 1 , V_13 ) ;
}
V_3 += 8 ;
V_6 ++ ;
}
return V_3 ;
}
static void F_14 ( T_2 * V_1 , T_3 * V_2 V_49 , T_1 V_3 , T_8 * V_50 , T_5 * V_5 V_49 )
{
T_4 V_51 ;
T_1 V_52 , V_53 ;
const T_4 * V_54 ;
V_50 -> V_55 = V_56 ;
V_54 = F_15 ( V_1 , V_3 , 4 ) ;
V_52 = V_54 [ 0 ] | ( ( V_54 [ 1 ] & 0x01 ) << 8 ) ;
V_53 = ( V_54 [ 1 ] >> 1 ) | ( ( V_54 [ 2 ] & 0x3f ) << 7 ) ;
V_51 = ( V_54 [ 2 ] >> 6 ) | ( ( V_54 [ 3 ] & 0x01 ) << 2 ) ;
switch ( V_51 ) {
case V_57 :
V_50 -> V_58 = 18 ;
break;
case V_59 :
V_50 -> V_58 = 36 ;
break;
case V_60 :
V_50 -> V_58 = 72 ;
break;
case V_61 :
V_50 -> V_58 = 144 ;
break;
default:
V_50 -> V_58 = 0 ;
}
V_50 -> V_62 = 1 ;
V_50 -> V_63 [ 0 ] = V_53 ;
V_50 -> V_64 [ 0 ] = V_52 ;
}
static void F_16 ( T_2 * V_1 , T_3 * V_2 V_49 , T_1 V_3 , T_8 * V_50 , T_5 * V_5 )
{
const T_4 * V_54 ;
T_4 V_4 ;
T_1 V_52 , V_53 ;
V_50 -> V_55 = V_65 ;
V_54 = F_15 ( V_1 , V_3 , 4 ) ;
V_52 = V_54 [ 0 ] | ( ( V_54 [ 1 ] & 0x01 ) << 8 ) ;
V_53 = ( V_54 [ 1 ] >> 1 ) | ( ( V_54 [ 2 ] & 0x3f ) << 7 ) ;
V_50 -> V_62 = 1 ;
V_50 -> V_63 [ 0 ] = V_53 ;
V_50 -> V_64 [ 0 ] = V_52 ;
V_3 += 4 ;
V_4 = F_7 ( V_1 , V_3 ) ; V_3 ++ ;
if ( V_4 > 0 )
F_1 ( V_1 , V_2 , V_3 , V_4 , V_5 ) ;
}
static void F_17 ( T_2 * V_1 , T_3 * V_2 , T_1 V_3 , T_8 * V_50 , T_5 * V_5 )
{
T_4 V_66 , V_4 ;
T_1 V_67 , V_68 ;
T_6 V_69 , V_6 = 0 ;
const T_4 * V_54 ;
T_5 * V_36 ;
T_7 * V_37 ;
V_50 -> V_55 = V_70 ;
V_69 = F_7 ( V_1 , V_3 ) ; V_3 ++ ;
if ( V_5 )
F_11 ( V_5 , V_71 , V_1 , V_3 - 1 , 1 , V_69 ) ;
V_50 -> V_62 = V_69 ;
V_50 -> V_72 = 1 ;
while ( V_6 < V_69 ) {
V_37 = F_9 ( V_5 , V_73 , V_1 , V_3 , 4 , V_39 ) ;
V_36 = F_10 ( V_37 , V_40 ) ;
V_54 = F_15 ( V_1 , V_3 , 4 ) ;
V_66 = ( V_54 [ 0 ] & 0x1f ) + 1 ;
V_67 = ( ( V_54 [ 1 ] & 0x3f ) << 3 ) | ( V_54 [ 0 ] >> 5 ) ;
V_68 = ( ( V_54 [ 3 ] & 0x07 ) << 10 ) | ( V_54 [ 2 ] << 2 ) | ( ( V_54 [ 1 ] & 0xc0 ) >> 6 ) ;
V_68 = V_68 == 0x1fff ? 0 : V_68 ;
V_50 -> V_63 [ V_6 ] = V_68 ;
V_50 -> V_64 [ V_6 ] = V_67 ;
if ( V_36 ) {
F_11 ( V_36 , V_74 , V_1 , V_3 , 1 , V_66 ) ;
F_11 ( V_36 , V_75 , V_1 , V_3 , 2 , V_67 ) ;
if ( V_68 )
F_11 ( V_36 , V_76 , V_1 , V_3 + 1 , 3 , V_68 ) ;
}
V_3 += 4 ;
if ( V_6 > V_77 ) {
F_8 ( V_5 , V_2 , & V_78 , V_1 , V_3 , - 1 ,
L_3 ,
V_77 ) ;
return;
}
V_6 ++ ;
}
V_4 = F_7 ( V_1 , V_3 ) ; V_3 ++ ;
if ( V_4 > 0 )
F_1 ( V_1 , V_2 , V_3 , V_4 , V_5 ) ;
}
static void F_18 ( T_2 * V_1 , T_3 * V_2 , T_1 V_3 , T_8 * V_50 , T_5 * V_5 )
{
const T_4 * V_54 ;
T_4 V_4 ;
T_1 V_52 , V_53 ;
V_50 -> V_55 = V_79 ;
V_54 = F_15 ( V_1 , V_3 , 4 ) ;
V_52 = V_54 [ 0 ] | ( ( V_54 [ 1 ] & 0x01 ) << 8 ) ;
V_53 = ( V_54 [ 1 ] >> 1 ) | ( ( V_54 [ 2 ] & 0x3f ) << 7 ) ;
V_50 -> V_62 = 1 ;
V_50 -> V_63 [ 0 ] = V_53 ;
V_50 -> V_64 [ 0 ] = V_52 ;
V_3 += 4 ;
V_4 = F_7 ( V_1 , V_3 ) ; V_3 ++ ;
if ( V_4 > 0 )
F_1 ( V_1 , V_2 , V_3 , V_4 , V_5 ) ;
}
static void F_19 ( T_2 * V_1 , T_3 * V_2 , T_1 V_3 , T_8 * V_50 , T_5 * V_5 )
{
T_4 V_4 , V_80 ;
V_80 = F_7 ( V_1 , V_3 ) ;
V_50 -> V_81 = V_80 & 0x08 ? V_82 : V_83 ;
V_50 -> V_55 = V_84 ;
if ( V_5 ) {
F_12 ( V_5 , V_85 , V_1 ,
V_3 * 8 + 4 , 1 , V_43 ) ;
F_12 ( V_5 , V_86 , V_1 ,
V_3 * 8 + 5 , 3 , V_43 ) ;
}
V_3 ++ ;
V_4 = F_7 ( V_1 , V_3 ) ; V_3 ++ ;
if ( V_4 > 0 )
F_1 ( V_1 , V_2 , V_3 , V_4 , V_5 ) ;
}
static void F_20 ( T_2 * V_1 , T_3 * V_2 , T_1 V_3 , T_8 * V_50 , T_5 * V_5 )
{
T_4 V_4 , V_87 , V_88 , V_6 = 0 ;
T_4 V_89 , V_90 ;
T_1 V_91 ;
T_7 * V_37 ;
T_5 * V_36 = NULL ;
V_50 -> V_55 = V_92 ;
V_87 = F_7 ( V_1 , V_3 ) ;
if ( V_5 ) {
F_11 ( V_5 , V_86 , V_1 , V_3 , 1 , V_87 ) ;
}
V_3 ++ ;
V_88 = F_7 ( V_1 , V_3 ) ; V_3 ++ ;
V_50 -> V_93 = V_88 ;
while ( V_6 < V_88 ) {
V_90 = F_7 ( V_1 , V_3 ++ ) ;
V_89 = F_7 ( V_1 , V_3 ++ ) ;
V_91 = F_21 ( V_1 , V_3 ) ;
V_3 += 2 ;
V_50 -> V_94 [ V_6 ] = V_90 ;
V_50 -> V_95 [ V_6 ] = V_91 ;
if ( V_5 ) {
V_37 = F_9 ( V_5 , V_96 , V_1 , V_3 - 4 , 4 , V_39 ) ;
V_36 = F_10 ( V_37 , V_97 ) ;
F_11 ( V_36 , V_98 , V_1 , V_3 - 4 , 1 , V_90 ) ;
F_11 ( V_36 , V_99 , V_1 , V_3 - 3 , 1 , V_89 ) ;
F_11 ( V_36 , V_100 , V_1 , V_3 - 2 , 2 , V_91 ) ;
}
V_6 ++ ;
if ( V_6 >= V_101 ) {
F_8 ( V_5 , V_2 , & V_78 , V_1 , V_3 , - 1 ,
L_3 ,
V_77 ) ;
return;
}
}
V_4 = F_7 ( V_1 , V_3 ) ; V_3 ++ ;
if ( V_4 > 0 )
F_1 ( V_1 , V_2 , V_3 , V_4 , V_5 ) ;
}
static void F_22 ( T_2 * V_1 , T_3 * V_2 , T_1 V_3 , T_4 V_102 , T_4 V_103 , T_5 * V_5 )
{
T_8 * V_50 ;
V_50 = ( T_8 * ) F_2 ( F_3 () , V_2 , V_104 , 0 ) ;
if ( ! V_50 ) {
V_50 = F_4 ( F_3 () , T_8 ) ;
F_5 ( F_3 () , V_2 , V_104 , 0 , V_50 ) ;
}
V_50 -> V_105 = V_2 -> V_106 == V_107 ;
V_50 -> V_108 = 7 ;
V_50 -> V_109 = 2008 ;
V_50 -> V_110 = 9 ;
V_50 -> V_72 = 1 ;
switch ( V_103 ) {
case V_111 :
V_50 -> V_112 = V_113 ;
break;
case V_114 :
V_50 -> V_112 = V_115 ;
break;
default:
V_50 -> V_112 = V_116 ;
}
V_50 -> V_117 = V_118 ;
V_50 -> V_119 = V_120 ;
switch ( V_102 ) {
case V_121 :
F_14 ( V_1 , V_2 , V_3 , V_50 , V_5 ) ;
break;
case V_122 :
F_16 ( V_1 , V_2 , V_3 , V_50 , V_5 ) ;
break;
case V_123 :
F_18 ( V_1 , V_2 , V_3 , V_50 , V_5 ) ;
break;
case V_124 :
F_17 ( V_1 , V_2 , V_3 , V_50 , V_5 ) ;
break;
case V_125 :
F_19 ( V_1 , V_2 , V_3 , V_50 , V_5 ) ;
break;
case V_126 :
F_20 ( V_1 , V_2 , V_3 , V_50 , V_5 ) ;
break;
default:
V_50 -> V_55 = 0 ;
}
}
static void F_23 ( T_2 * V_1 , T_3 * V_2 , T_5 * V_5 )
{
T_4 V_103 , V_102 ;
T_1 V_127 ;
T_6 V_128 , V_129 ;
T_2 * V_130 ;
T_9 * V_131 ;
T_7 * V_132 ;
T_5 * V_133 = NULL ;
F_24 ( V_2 -> V_134 , V_135 , L_4 ) ;
V_127 = F_21 ( V_1 , 0 ) ;
V_103 = F_7 ( V_1 , 2 ) ;
V_102 = F_7 ( V_1 , 3 ) ;
if ( V_5 ) {
V_132 = F_9 ( V_5 , V_136 , V_1 , 0 , V_127 , V_39 ) ;
V_133 = F_10 ( V_132 , V_137 ) ;
F_11 ( V_133 , V_138 , V_1 , 2 , 1 , V_103 ) ;
F_11 ( V_133 , V_139 , V_1 , 3 , 1 , V_102 ) ;
}
F_22 ( V_1 , V_2 , 4 , V_102 , V_103 , V_133 ) ;
switch ( V_103 ) {
case V_111 :
V_129 = F_25 ( V_1 , V_127 ) ; V_127 += 4 ;
V_128 = F_25 ( V_1 , V_127 ) ; V_127 += 4 ;
memset ( & V_2 -> V_140 -> V_141 , 0 , sizeof( V_2 -> V_140 -> V_141 ) ) ;
V_2 -> V_140 -> V_141 . V_142 = V_143 ;
V_2 -> V_140 -> V_141 . V_144 = V_145 ;
V_2 -> V_140 -> V_141 . V_146 = ( ( V_128 & 0x0ff00000 ) >> 20 ) ;
V_2 -> V_140 -> V_141 . V_147 = ( ( V_128 & 0x000ffff0 ) >> 4 ) ;
V_2 -> V_140 -> V_141 . V_148 = V_129 & 0x000000ff ;
V_2 -> V_140 -> V_141 . type = V_149 ;
V_131 = & V_150 ;
break;
case V_114 :
V_131 = & V_151 ;
break;
default:
V_131 = & V_152 ;
}
V_130 = F_26 ( V_1 , V_127 ) ;
F_27 ( * V_131 , V_130 , V_2 , V_5 ) ;
}
void
F_28 ( void )
{
static T_10 V_153 [] = {
{ & V_138 , { L_5 , L_6 , V_154 , V_155 , F_29 ( V_156 ) , 0x0 , NULL , V_157 } } ,
{ & V_139 , { L_7 , L_8 , V_154 , V_155 , F_29 ( V_158 ) , 0x0 , NULL , V_157 } } ,
{ & V_71 , { L_9 , L_10 , V_154 , V_159 , NULL , 0 , NULL , V_157 } } ,
{ & V_74 , { L_11 , L_12 , V_154 , V_159 , NULL , 0 , NULL , V_157 } } ,
{ & V_86 , { L_13 , L_14 , V_154 , V_159 , NULL , 0 , NULL , V_157 } } ,
{ & V_85 , { L_15 , L_16 , V_160 , V_161 , NULL , 0 , NULL , V_157 } } ,
{ & V_73 , { L_17 , L_18 , V_162 , V_161 , NULL , 0 , NULL , V_157 } } ,
{ & V_75 , { L_19 , L_20 , V_163 , V_159 , NULL , 0 , NULL , V_157 } } ,
{ & V_76 , { L_21 , L_22 , V_164 , V_159 , NULL , 0 , NULL , V_157 } } ,
{ & V_96 , { L_23 , L_24 , V_162 , V_161 , NULL , 0 , NULL , V_157 } } ,
{ & V_98 , { L_25 , L_26 , V_154 , V_159 , NULL , 0 , NULL , V_157 } } ,
{ & V_99 , { L_27 , L_28 , V_154 , V_159 , NULL , 0 , NULL , V_157 } } ,
{ & V_100 , { L_21 , L_29 , V_163 , V_159 , NULL , 0 , NULL , V_157 } } ,
{ & V_38 , { L_30 , L_31 , V_162 , V_161 , NULL , 0 , NULL , V_157 } } ,
{ & V_41 , { L_32 , L_33 , V_164 , V_155 , NULL , 0x0 , NULL , V_157 } } ,
{ & V_42 , { L_34 , L_35 , V_154 , V_159 , F_29 ( V_165 ) , 0 , NULL , V_157 } } ,
{ & V_44 , { L_36 , L_37 , V_154 , V_159 , F_29 ( V_166 ) , 0 , NULL , V_157 } } ,
{ & V_45 , { L_38 , L_39 , V_163 , V_159 , NULL , 0x01f0 , NULL , V_157 } } ,
{ & V_46 , { L_40 , L_41 , V_160 , V_161 , F_30 ( & V_167 ) , 0 , L_42 , V_157 } } ,
{ & V_47 , { L_43 , L_44 , V_160 , V_161 , F_30 ( & V_168 ) , 0 , L_45 , V_157 } } ,
{ & V_48 , { L_46 , L_47 , V_160 , V_161 , F_30 ( & V_169 ) , 0 , L_48 , V_157 } }
} ;
static T_11 * V_170 [] = {
& V_137 ,
& V_40 ,
& V_97 ,
& V_171
} ;
static T_12 V_172 [] = {
{ & V_22 , { L_49 , V_173 , V_174 , L_50 , V_175 } } ,
{ & V_24 , { L_51 , V_173 , V_174 , L_52 , V_175 } } ,
{ & V_78 , { L_53 , V_173 , V_174 , L_54 , V_175 } } ,
} ;
T_13 * V_176 ;
V_136 = F_31 ( L_4 , L_4 , L_55 ) ;
F_32 ( L_55 , F_23 , V_136 ) ;
F_33 ( V_136 , V_153 , F_34 ( V_153 ) ) ;
F_35 ( V_170 , F_34 ( V_170 ) ) ;
V_176 = F_36 ( V_136 ) ;
F_37 ( V_176 , V_172 , F_34 ( V_172 ) ) ;
}
void
F_38 ( void )
{
V_150 = F_39 ( L_56 ) ;
V_152 = F_39 ( L_57 ) ;
V_151 = F_39 ( L_58 ) ;
}
