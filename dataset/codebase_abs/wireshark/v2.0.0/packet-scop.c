static int
F_1 ( T_1 * V_1 , T_2 * V_2 , T_3 * V_3 , void * T_4 V_4 )
{
T_1 * V_5 ;
T_5 * V_6 ;
T_3 * V_7 ;
T_6 V_8 = 0 ;
T_7 V_9 ;
memset ( & V_9 , 0 , sizeof( V_9 ) ) ;
F_2 ( V_2 -> V_10 , V_11 , L_1 ) ;
F_3 ( V_2 -> V_10 , V_12 ) ;
V_6 = F_4 ( V_3 , V_13 , V_1 , 0 , F_5 ( V_1 ) ,
L_2 ) ;
V_7 = F_6 ( V_6 , V_14 ) ;
V_9 . V_15 = F_7 ( V_1 , V_8 ) ;
F_8 ( V_7 , V_16 , V_1 , V_8 , 1 , V_9 . V_15 ) ;
V_8 += 1 ;
V_9 . V_17 = F_7 ( V_1 , V_8 ) ;
F_8 ( V_7 , V_18 , V_1 , V_8 , 1 , V_9 . V_17 ) ;
V_8 += 1 ;
V_9 . V_19 = F_9 ( V_1 , V_8 ) ;
F_8 ( V_7 , V_20 , V_1 , V_8 , 2 , V_9 . V_19 ) ;
V_8 += 2 ;
if ( ( V_9 . V_15 == V_21 )
|| ( V_9 . V_15 == V_22 ) ) {
return V_8 ;
}
V_5 = V_1 ;
V_9 . V_23 = F_7 ( V_5 , V_8 ) ;
F_8 ( V_7 , V_24 , V_5 , V_8 , 1 , V_9 . V_23 ) ;
V_8 += 1 ;
switch ( V_9 . V_23 ) {
case V_25 :
F_10 ( F_11 ( V_5 , V_8 ) , V_2 , V_7 ) ;
break;
case V_26 :
F_12 ( F_11 ( V_5 , V_8 ) , V_2 , V_7 ) ;
break;
case V_27 :
default:
F_13 ( V_28 , F_11 ( V_5 , V_8 ) , V_2 , V_3 ) ;
break;
}
return F_5 ( V_1 ) ;
}
static T_6
F_14 ( T_2 * V_2 V_4 , T_1 * V_1 , int V_8 , void * T_4 V_4 )
{
return F_9 ( V_1 , V_8 + V_29 ) ;
}
static int
F_15 ( T_1 * V_1 , T_2 * V_2 , T_3 * V_3 , void * T_4 )
{
F_16 ( V_1 , V_2 , V_3 , TRUE , V_30 , F_14 , F_1 , T_4 ) ;
return F_5 ( V_1 ) ;
}
static void
F_10 ( T_1 * V_1 , T_2 * V_2 , T_3 * V_3 )
{
T_6 V_8 = 0 ;
T_8 type = F_7 ( V_1 , V_8 ) ;
T_9 V_31 ;
F_8 ( V_3 , V_32 , V_1 , V_8 , 1 , type ) ;
F_17 ( V_3 , L_3 , F_18 ( type , V_33 , L_4 ) ) ;
F_2 ( V_2 -> V_10 , V_12 , F_18 ( type , V_33 , L_4 ) ) ;
V_8 += 2 ;
if ( type == V_34 ) {
V_31 = F_9 ( V_1 , 1 ) ;
F_19 ( V_3 , V_35 , V_1 , V_8 , 2 , V_31 , L_5 , ( V_31 == 0x0000 ) ? L_6 : L_7 ) ;
V_8 += 2 ;
}
if ( V_8 < F_20 ( V_1 ) ) {
T_1 * V_36 = F_11 ( V_1 , V_8 ) ;
T_3 * V_37 = F_21 ( V_3 ) ;
F_13 ( V_28 , V_36 , V_2 , V_37 ) ;
}
}
static void
F_12 ( T_1 * V_1 , T_2 * V_2 , T_3 * V_3 )
{
F_13 ( V_38 , V_1 , V_2 , F_21 ( V_3 ) ) ;
}
void F_22 ( void )
{
T_10 * V_39 ;
static T_11 V_40 [] = {
{ & V_16 ,
{ L_8 , L_9 , V_41 , V_42 , F_23 ( V_43 ) , 0x0 ,
L_10 , V_44 } } ,
{ & V_18 ,
{ L_11 , L_12 , V_41 , V_42 , NULL , 0x0 ,
L_13 , V_44 } } ,
{ & V_20 ,
{ L_14 , L_15 , V_45 , V_42 , NULL , 0x0 ,
NULL , V_44 } } ,
{ & V_24 ,
{ L_16 , L_17 , V_41 , V_42 , F_23 ( V_46 ) , 0x0 ,
NULL , V_44 } } ,
{ & V_32 ,
{ L_18 , L_19 , V_41 , V_42 , F_23 ( V_33 ) , 0x0 ,
L_20 , V_44 } } ,
{ & V_35 ,
{ L_21 , L_22 , V_45 , V_47 , NULL , 0x0 ,
L_23 , V_44 } }
} ;
static T_12 * V_48 [] = {
& V_14
} ;
V_13 = F_24 ( L_2 , L_1 , L_24 ) ;
F_25 ( V_13 , V_40 , F_26 ( V_40 ) ) ;
F_27 ( V_48 , F_26 ( V_48 ) ) ;
V_39 = F_28 ( V_13 , V_49 ) ;
F_29 ( V_39 , L_25 , L_26 ,
L_27 ,
10 , & V_50 ) ;
F_29 ( V_39 , L_28 , L_29 ,
L_30 ,
10 , & V_51 ) ;
F_30 ( L_31 , F_1 , V_13 ) ;
F_30 ( L_32 , F_15 , V_13 ) ;
}
void V_49 ( void )
{
static T_13 V_52 = FALSE ;
static T_14 V_53 ;
static T_14 V_54 ;
static T_15 V_55 ;
static T_15 V_56 ;
if ( ! V_52 ) {
V_55 = F_31 ( L_31 ) ;
V_56 = F_31 ( L_32 ) ;
V_38 = F_31 ( L_33 ) ;
V_28 = F_31 ( L_34 ) ;
V_52 = TRUE ;
} else {
F_32 ( L_35 , V_53 , V_55 ) ;
F_32 ( L_36 , V_53 , V_56 ) ;
F_32 ( L_35 , V_54 , V_55 ) ;
F_32 ( L_36 , V_54 , V_56 ) ;
}
F_33 ( L_35 , V_50 , V_55 ) ;
F_33 ( L_36 , V_50 , V_56 ) ;
F_33 ( L_35 , V_51 , V_55 ) ;
F_33 ( L_36 , V_51 , V_56 ) ;
V_53 = V_50 ;
V_54 = V_51 ;
}
