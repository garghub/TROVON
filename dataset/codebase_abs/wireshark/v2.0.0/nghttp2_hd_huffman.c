static T_1 F_1 ( T_2 * V_1 , T_3 * V_2 ,
T_3 V_3 , const T_4 * V_4 ) {
int V_5 ;
T_3 V_6 = V_4 -> V_6 ;
T_5 V_7 = V_4 -> V_7 ;
if ( V_3 > V_6 ) {
F_2 ( V_1 , ( V_8 ) ( V_7 << ( V_3 - V_6 ) ) ) ;
return ( T_1 ) ( V_3 - V_6 ) ;
}
if ( V_3 == V_6 ) {
F_3 ( V_1 , ( V_8 ) V_7 ) ;
-- * V_2 ;
return 8 ;
}
F_3 ( V_1 , ( V_8 ) ( V_7 >> ( V_6 - V_3 ) ) ) ;
-- * V_2 ;
V_6 -= V_3 ;
if ( V_6 & 0x7 ) {
V_7 <<= 8 - ( V_6 & 0x7 ) ;
}
if ( * V_2 < ( V_6 + 7 ) / 8 ) {
V_5 = F_4 ( V_1 ) ;
if ( V_5 != 0 ) {
return V_5 ;
}
* V_2 = F_5 ( V_1 ) ;
assert ( * V_2 >= 3 ) ;
}
if ( V_6 < 8 ) {
F_6 ( V_1 , ( V_8 ) V_7 ) ;
* V_2 = F_5 ( V_1 ) ;
return ( T_1 ) ( 8 - V_6 ) ;
}
if ( V_6 > 24 ) {
F_7 ( V_1 , ( V_8 ) ( V_7 >> 24 ) ) ;
V_6 -= 8 ;
}
if ( V_6 > 16 ) {
F_7 ( V_1 , ( V_8 ) ( V_7 >> 16 ) ) ;
V_6 -= 8 ;
}
if ( V_6 > 8 ) {
F_7 ( V_1 , ( V_8 ) ( V_7 >> 8 ) ) ;
V_6 -= 8 ;
}
if ( V_6 == 8 ) {
F_7 ( V_1 , ( V_8 ) V_7 ) ;
* V_2 = F_5 ( V_1 ) ;
return 8 ;
}
F_6 ( V_1 , ( V_8 ) V_7 ) ;
* V_2 = F_5 ( V_1 ) ;
return ( T_1 ) ( 8 - V_6 ) ;
}
T_3 F_8 ( const V_8 * V_9 , T_3 V_10 ) {
T_3 V_11 ;
T_3 V_6 = 0 ;
for ( V_11 = 0 ; V_11 < V_10 ; ++ V_11 ) {
V_6 += V_12 [ V_9 [ V_11 ] ] . V_6 ;
}
return ( V_6 + 7 ) / 8 ;
}
int F_9 ( T_2 * V_1 , const V_8 * V_9 ,
T_3 V_13 ) {
int V_5 ;
T_1 V_3 = 8 ;
T_3 V_11 ;
T_3 V_14 ;
V_14 = F_5 ( V_1 ) ;
for ( V_11 = 0 ; V_11 < V_13 ; ++ V_11 ) {
const T_4 * V_4 = & V_12 [ V_9 [ V_11 ] ] ;
if ( V_3 == 8 ) {
if ( V_14 ) {
F_6 ( V_1 , 0 ) ;
} else {
V_5 = F_10 ( V_1 , 0 ) ;
if ( V_5 != 0 ) {
return V_5 ;
}
V_14 = F_5 ( V_1 ) ;
}
}
V_3 = F_1 ( V_1 , & V_14 , ( T_3 ) V_3 , V_4 ) ;
if ( V_3 < 0 ) {
return ( int ) V_3 ;
}
}
if ( V_3 < 8 ) {
const T_4 * V_4 = & V_12 [ 256 ] ;
assert ( V_14 ) ;
F_3 ( V_1 , ( V_8 ) ( V_4 -> V_7 >> ( V_4 -> V_6 - V_3 ) ) ) ;
}
return 0 ;
}
void F_11 ( T_6 * V_15 ) {
V_15 -> V_16 = 0 ;
V_15 -> V_17 = 1 ;
}
T_1 F_12 ( T_6 * V_15 ,
T_2 * V_1 , const V_8 * V_9 ,
T_3 V_13 , int V_18 ) {
T_3 V_11 ;
int V_5 ;
T_3 V_14 ;
V_14 = F_5 ( V_1 ) ;
for ( V_11 = 0 ; V_11 < V_13 ; ++ V_11 ) {
const T_7 * V_19 ;
V_19 = & V_20 [ V_15 -> V_16 ] [ V_9 [ V_11 ] >> 4 ] ;
if ( V_19 -> V_21 & V_22 ) {
return V_23 ;
}
if ( V_19 -> V_21 & V_24 ) {
F_13 ( V_1 , V_19 -> V_4 , V_14 ) ;
}
V_19 = & V_20 [ V_19 -> V_16 ] [ V_9 [ V_11 ] & 0xf ] ;
if ( V_19 -> V_21 & V_22 ) {
return V_23 ;
}
if ( V_19 -> V_21 & V_24 ) {
F_13 ( V_1 , V_19 -> V_4 , V_14 ) ;
}
V_15 -> V_16 = V_19 -> V_16 ;
V_15 -> V_17 = ( V_19 -> V_21 & V_25 ) != 0 ;
}
if ( V_18 && ! V_15 -> V_17 ) {
return V_23 ;
}
return ( T_1 ) V_11 ;
}
