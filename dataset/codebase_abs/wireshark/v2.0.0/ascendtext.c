static T_1 F_1 ( T_2 * V_1 , int * V_2 , T_3 * * V_3 )
{
int V_4 ;
T_1 V_5 = - 1 , V_6 , V_7 ;
T_4 V_8 [ V_9 ] ;
T_5 V_10 = 0 , type = 0 ;
T_5 V_11 = 262144 ;
memset ( & V_8 , 0 , sizeof( V_8 ) ) ;
while ( ( ( V_4 = F_2 ( V_1 -> V_12 ) ) != V_13 ) ) {
V_11 -- ;
if ( ! V_11 ) {
* V_2 = 0 ;
return - 1 ;
}
for ( V_10 = 0 ; V_10 < V_9 ; V_10 ++ ) {
const T_3 * V_14 = V_15 [ V_10 ] . V_14 ;
T_4 V_16 = strlen ( V_14 ) ;
if ( V_4 == * ( V_14 + V_8 [ V_10 ] ) ) {
V_8 [ V_10 ] ++ ;
if ( V_8 [ V_10 ] >= V_16 ) {
V_6 = F_3 ( V_1 -> V_12 ) ;
if ( V_6 == - 1 ) {
* V_2 = F_4 ( V_1 -> V_12 , V_3 ) ;
return - 1 ;
}
if ( strcmp ( V_14 , V_17 ) == 0 ) {
V_5 = V_6 - V_16 ;
} else {
if ( V_5 == - 1 ) {
V_7 = V_6 - V_16 ;
} else {
V_7 = V_5 ;
}
type = V_15 [ V_10 ] . type ;
goto V_18;
}
}
} else {
V_8 [ V_10 ] = 0 ;
}
}
}
* V_2 = F_4 ( V_1 -> V_12 , V_3 ) ;
return - 1 ;
V_18:
if ( F_5 ( V_1 -> V_12 , V_7 , V_19 , V_2 ) == - 1 )
return - 1 ;
V_1 -> V_20 . V_21 . V_22 . type = type ;
return V_7 ;
}
T_6 F_6 ( T_2 * V_1 , int * V_2 , T_3 * * V_3 )
{
T_1 V_23 ;
T_7 V_24 ;
T_8 * V_22 ;
V_1 -> V_25 = NULL ;
V_23 = F_1 ( V_1 , V_2 , V_3 ) ;
if ( V_23 == - 1 ) {
if ( * V_2 != 0 && * V_2 != V_26 )
return V_27 ;
return V_28 ;
}
F_7 () ;
if ( ! F_8 ( V_1 -> V_12 , & V_1 -> V_20 ) ) {
return V_28 ;
}
V_1 -> V_29 = V_30 ;
switch( V_1 -> V_20 . V_21 . V_22 . type ) {
case V_31 :
case V_32 :
V_1 -> V_33 = V_34 ;
break;
case V_35 :
V_1 -> V_33 = V_36 ;
break;
default:
V_1 -> V_33 = V_37 ;
}
V_1 -> V_38 = V_39 ;
V_1 -> V_40 = V_41 ;
V_1 -> V_42 = V_43 ;
V_22 = ( T_8 * ) F_9 ( sizeof( T_8 ) ) ;
V_1 -> V_25 = ( void * ) V_22 ;
V_22 -> V_44 = V_23 ;
if ( F_10 ( V_1 , & V_24 , V_2 ) == - 1 ) {
return V_27 ;
}
V_22 -> V_45 = V_24 . V_46 ;
V_22 -> V_47 = FALSE ;
V_1 -> V_48 = V_49 ;
F_7 () ;
return V_50 ;
}
static T_9 V_41 ( T_2 * V_1 , int * V_2 , T_3 * * V_3 ,
T_1 * V_51 )
{
T_8 * V_22 = ( T_8 * ) V_1 -> V_25 ;
T_1 V_23 ;
if ( F_5 ( V_1 -> V_12 , V_22 -> V_44 ,
V_19 , V_2 ) == - 1 )
return FALSE ;
V_23 = F_1 ( V_1 , V_2 , V_3 ) ;
if ( V_23 == - 1 )
return FALSE ;
if ( F_11 ( V_22 , V_1 -> V_12 , & V_1 -> V_20 , V_1 -> V_52 ,
V_1 -> V_38 ) != V_53 ) {
* V_2 = V_54 ;
* V_3 = F_12 ( ( V_55 != NULL ) ? V_55 : L_1 ) ;
return FALSE ;
}
* V_51 = V_23 ;
return TRUE ;
}
static T_9 V_43 ( T_2 * V_1 , T_1 V_56 ,
struct V_57 * V_20 , T_10 * V_58 ,
int * V_2 , T_3 * * V_3 )
{
T_8 * V_22 = ( T_8 * ) V_1 -> V_25 ;
if ( F_5 ( V_1 -> V_59 , V_56 , V_19 , V_2 ) == - 1 )
return FALSE ;
if ( F_11 ( V_22 , V_1 -> V_59 , V_20 , V_58 ,
V_1 -> V_38 ) != V_53 ) {
* V_2 = V_54 ;
* V_3 = F_12 ( ( V_55 != NULL ) ? V_55 : L_1 ) ;
return FALSE ;
}
return TRUE ;
}
