static int
F_1 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_2 ( T_2 , T_7 , T_9 , T_4 , T_5 , T_10 ,
NULL ) ;
return T_5 ;
}
static int
F_3 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_4 ( T_2 , V_2 ,
T_7 , T_9 , T_4 , T_5 , T_10 ,
NULL ) ;
return T_5 ;
}
static int
F_5 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_6 ( T_2 , T_7 , T_9 , T_4 , T_5 ,
V_3 , T_10 , V_4 ) ;
return T_5 ;
}
static int
F_7 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_8 ( T_2 , T_7 , T_9 , T_4 , T_5 ,
V_5 , T_10 , V_6 ) ;
return T_5 ;
}
static int
F_9 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_6 ( T_2 , T_7 , T_9 , T_4 , T_5 ,
V_7 , T_10 , V_8 ) ;
return T_5 ;
}
static int
F_10 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_8 ( T_2 , T_7 , T_9 , T_4 , T_5 ,
V_9 , T_10 , V_10 ) ;
return T_5 ;
}
static int
F_11 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_12 ( T_7 , T_9 , T_4 , T_5 ,
V_11 , T_10 , V_12 ,
NULL ) ;
return T_5 ;
}
static int
F_13 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_14 ( T_2 , T_7 , T_9 , T_4 , T_5 , T_10 ) ;
return T_5 ;
}
static int
F_15 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_4 ( T_2 , V_13 ,
T_7 , T_9 , T_4 , T_5 , T_10 ,
NULL ) ;
return T_5 ;
}
static int
F_16 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_2 ( T_2 , T_7 , T_9 , T_4 , T_5 , T_10 ,
NULL ) ;
return T_5 ;
}
static int
F_17 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_12 ( T_7 , T_9 , T_4 , T_5 ,
V_14 , T_10 , V_15 ,
NULL ) ;
return T_5 ;
}
static int
F_18 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_6 ( T_2 , T_7 , T_9 , T_4 , T_5 ,
V_16 , T_10 , V_17 ) ;
return T_5 ;
}
static int
F_19 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_8 ( T_2 , T_7 , T_9 , T_4 , T_5 ,
V_18 , T_10 , V_19 ) ;
return T_5 ;
}
static int
F_20 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_2 ( T_2 , T_7 , T_9 , T_4 , T_5 , T_10 ,
NULL ) ;
return T_5 ;
}
static int
F_21 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_12 ( T_7 , T_9 , T_4 , T_5 ,
V_20 , T_10 , V_21 ,
NULL ) ;
return T_5 ;
}
static int
F_22 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_8 ( T_2 , T_7 , T_9 , T_4 , T_5 ,
V_22 , T_10 , V_23 ) ;
return T_5 ;
}
static int
F_23 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_12 ( T_7 , T_9 , T_4 , T_5 ,
V_24 , T_10 , V_25 ,
NULL ) ;
return T_5 ;
}
static int
F_24 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_12 ( T_7 , T_9 , T_4 , T_5 ,
V_26 , T_10 , V_27 ,
NULL ) ;
return T_5 ;
}
static int
F_25 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_8 ( T_2 , T_7 , T_9 , T_4 , T_5 ,
V_28 , T_10 , V_29 ) ;
return T_5 ;
}
static int
F_26 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
#line 18 "../../asn1/goose/goose.cnf"
T_11 V_30 ;
T_12 * V_31 ;
T_11 V_32 ;
T_11 V_33 ;
T_11 V_34 ;
T_13 V_35 ;
T_14 * V_36 ;
V_30 = F_27 ( T_4 , T_5 ) ;
if( V_30 != 8 )
{
V_31 = F_28 ( T_9 , T_4 , T_5 , V_30 ,
L_1
L_2 ) ;
F_29 ( V_31 , V_37 , V_38 ) ;
F_30 ( T_7 -> V_39 , V_31 , V_37 , V_38 , L_3 ) ;
if( T_10 >= 0 )
{
F_31 ( T_9 , T_10 , T_4 , T_5 , V_30 , L_4 ) ;
}
return T_5 ;
}
V_32 = F_32 ( T_4 , T_5 ) ;
V_33 = F_33 ( T_4 , T_5 + 4 ) * 0x100 ;
V_34 = ( T_11 ) ( ( ( V_40 ) V_33 * F_34 ( 1000000000U ) ) / F_34 ( 0x100000000U ) ) ;
V_35 . V_41 = V_32 ;
V_35 . V_42 = V_34 ;
V_36 = F_35 ( & V_35 , V_43 , TRUE ) ;
if( T_10 >= 0 )
{
F_31 ( T_9 , T_10 , T_4 , T_5 , V_30 , V_36 ) ;
}
return T_5 ;
return T_5 ;
}
static int
F_36 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_37 ( T_2 , T_7 , T_9 , T_4 , T_5 , T_10 , NULL ) ;
return T_5 ;
}
static int
F_38 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_6 ( T_2 , T_7 , T_9 , T_4 , T_5 ,
V_44 , T_10 , V_45 ) ;
return T_5 ;
}
static int
F_39 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_40 ( T_2 , T_7 , T_9 , T_4 , T_5 ,
NULL , T_10 , - 1 ,
NULL ) ;
return T_5 ;
}
static int
F_41 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_42 ( T_2 , T_7 , T_9 , T_4 , T_5 , T_10 ,
NULL ) ;
return T_5 ;
}
static int
F_43 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_44 ( T_2 , T_7 , T_9 , T_4 , T_5 , T_10 ,
NULL ) ;
return T_5 ;
}
static int
F_45 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_42 ( T_2 , T_7 , T_9 , T_4 , T_5 , T_10 ,
NULL ) ;
return T_5 ;
}
static int
F_46 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_42 ( T_2 , T_7 , T_9 , T_4 , T_5 , T_10 ,
NULL ) ;
return T_5 ;
}
static int
F_47 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_48 ( T_2 , T_7 , T_9 , T_4 , T_5 , T_10 , NULL ) ;
return T_5 ;
}
static int
F_49 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_4 ( T_2 , V_46 ,
T_7 , T_9 , T_4 , T_5 , T_10 ,
NULL ) ;
return T_5 ;
}
static int
F_50 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_12 ( T_7 , T_9 , T_4 , T_5 ,
V_47 , T_10 , V_48 ,
NULL ) ;
return T_5 ;
}
static int
F_51 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_8 ( T_2 , T_7 , T_9 , T_4 , T_5 ,
V_49 , T_10 , V_50 ) ;
return T_5 ;
}
static int
F_52 ( T_1 T_2 V_1 , T_3 * T_4 V_1 , int T_5 V_1 , T_6 * T_7 V_1 , T_8 * T_9 V_1 , int T_10 V_1 ) {
T_5 = F_12 ( T_7 , T_9 , T_4 , T_5 ,
V_51 , T_10 , V_52 ,
NULL ) ;
return T_5 ;
}
static void
F_53 ( T_3 * T_4 , T_15 * V_39 , T_8 * V_53 )
{
int T_5 = 0 ;
int V_54 ;
T_16 V_55 ;
T_12 * V_56 = NULL ;
T_8 * T_9 = NULL ;
T_6 V_57 ;
F_54 ( & V_57 , V_58 , TRUE , V_39 ) ;
F_55 ( V_39 -> V_59 , V_60 , V_61 ) ;
F_56 ( V_39 -> V_59 , V_62 ) ;
if ( V_53 ) {
V_56 = F_57 ( V_53 , V_63 , T_4 , 0 , - 1 , V_64 ) ;
T_9 = F_58 ( V_56 , V_65 ) ;
F_57 ( T_9 , V_66 , T_4 , T_5 , 2 , V_67 ) ;
V_55 = F_59 ( T_4 , T_5 + 2 ) ;
F_57 ( T_9 , V_68 , T_4 , T_5 + 2 , 2 , V_67 ) ;
F_57 ( T_9 , V_69 , T_4 , T_5 + 4 , 2 , V_67 ) ;
F_57 ( T_9 , V_70 , T_4 , T_5 + 6 , 2 , V_67 ) ;
T_5 = 8 ;
while ( T_5 < V_55 ) {
V_54 = T_5 ;
T_5 = F_52 ( FALSE , T_4 , T_5 , & V_57 , T_9 , - 1 ) ;
if ( T_5 == V_54 ) {
F_28 ( T_9 , T_4 , T_5 , - 1 , L_5 ) ;
return;
}
}
}
}
void F_60 ( void ) {
static T_17 V_71 [] =
{
{ & V_66 ,
{ L_6 , L_7 , V_72 , V_73 , NULL , 0x0 , NULL , V_74 } } ,
{ & V_68 ,
{ L_8 , L_9 , V_72 , V_75 , NULL , 0x0 , NULL , V_74 } } ,
{ & V_69 ,
{ L_10 , L_11 , V_72 , V_73 , NULL , 0x0 , NULL , V_74 } } ,
{ & V_70 ,
{ L_12 , L_13 , V_72 , V_73 , NULL , 0x0 , NULL , V_74 } } ,
#line 1 "../../asn1/goose/packet-goose-hfarr.c"
{ & V_76 ,
{ L_14 , L_15 ,
V_77 , V_78 , NULL , 0 ,
NULL , V_74 } } ,
{ & V_79 ,
{ L_16 , L_17 ,
V_77 , V_78 , NULL , 0 ,
L_18 , V_74 } } ,
{ & V_80 ,
{ L_19 , L_20 ,
V_81 , V_75 , NULL , 0 ,
L_21 , V_74 } } ,
{ & V_82 ,
{ L_22 , L_23 ,
V_83 , V_75 , F_61 ( V_84 ) , 0 ,
L_24 , V_74 } } ,
{ & V_85 ,
{ L_25 , L_26 ,
V_83 , V_75 , F_61 ( V_86 ) , 0 ,
L_27 , V_74 } } ,
{ & V_87 ,
{ L_28 , L_29 ,
V_83 , V_75 , F_61 ( V_88 ) , 0 ,
L_30 , V_74 } } ,
{ & V_89 ,
{ L_31 , L_32 ,
V_77 , V_78 , NULL , 0 ,
L_33 , V_74 } } ,
{ & V_90 ,
{ L_34 , L_35 ,
V_77 , V_78 , NULL , 0 ,
L_36 , V_74 } } ,
{ & V_91 ,
{ L_37 , L_38 ,
V_77 , V_78 , NULL , 0 ,
L_33 , V_74 } } ,
{ & V_92 ,
{ L_39 , L_40 ,
V_77 , V_78 , NULL , 0 ,
L_36 , V_74 } } ,
{ & V_93 ,
{ L_41 , L_42 ,
V_77 , V_78 , NULL , 0 ,
NULL , V_74 } } ,
{ & V_94 ,
{ L_31 , L_32 ,
V_77 , V_78 , NULL , 0 ,
L_43 , V_74 } } ,
{ & V_95 ,
{ L_34 , L_35 ,
V_77 , V_78 , NULL , 0 ,
L_43 , V_74 } } ,
{ & V_96 ,
{ L_37 , L_38 ,
V_77 , V_78 , NULL , 0 ,
L_43 , V_74 } } ,
{ & V_97 ,
{ L_39 , L_40 ,
V_77 , V_78 , NULL , 0 ,
L_43 , V_74 } } ,
{ & V_98 ,
{ L_44 , L_45 ,
V_99 , V_78 , NULL , 0 ,
L_46 , V_74 } } ,
{ & V_100 ,
{ L_47 , L_48 ,
V_83 , V_75 , NULL , 0 ,
L_49 , V_74 } } ,
{ & V_101 ,
{ L_50 , L_51 ,
V_81 , V_75 , NULL , 0 ,
L_21 , V_74 } } ,
{ & V_102 ,
{ L_52 , L_53 ,
V_83 , V_75 , NULL , 0 ,
NULL , V_74 } } ,
{ & V_103 ,
{ L_54 , L_55 ,
V_99 , V_78 , NULL , 0 ,
L_46 , V_74 } } ,
{ & V_104 ,
{ L_56 , L_57 ,
V_81 , V_75 , NULL , 0 ,
L_21 , V_74 } } ,
{ & V_105 ,
{ L_58 , L_59 ,
V_83 , V_75 , F_61 ( V_106 ) , 0 ,
L_60 , V_74 } } ,
{ & V_107 ,
{ L_61 , L_62 ,
V_77 , V_78 , NULL , 0 ,
NULL , V_74 } } ,
{ & V_108 ,
{ L_63 , L_64 ,
V_99 , V_78 , NULL , 0 ,
L_46 , V_74 } } ,
{ & V_109 ,
{ L_65 , L_66 ,
V_83 , V_75 , NULL , 0 ,
L_67 , V_74 } } ,
{ & V_110 ,
{ L_68 , L_69 ,
V_83 , V_75 , F_61 ( V_111 ) , 0 ,
NULL , V_74 } } ,
{ & V_112 ,
{ L_70 , L_71 ,
V_81 , V_75 , F_61 ( V_113 ) , 0 ,
L_72 , V_74 } } ,
{ & V_114 ,
{ L_47 , L_48 ,
V_81 , V_75 , NULL , 0 ,
L_21 , V_74 } } ,
{ & V_115 ,
{ L_73 , L_74 ,
V_99 , V_78 , NULL , 0 ,
L_75 , V_74 } } ,
{ & V_116 ,
{ L_76 , L_77 ,
V_81 , V_75 , F_61 ( V_117 ) , 0 ,
L_78 , V_74 } } ,
{ & V_118 ,
{ L_79 , L_80 ,
V_99 , V_78 , NULL , 0 ,
L_46 , V_74 } } ,
{ & V_119 ,
{ L_81 , L_82 ,
V_81 , V_75 , NULL , 0 ,
L_21 , V_74 } } ,
{ & V_120 ,
{ L_83 , L_84 ,
V_99 , V_78 , NULL , 0 ,
L_46 , V_74 } } ,
{ & V_121 ,
{ L_85 , L_86 ,
V_99 , V_78 , NULL , 0 ,
L_87 , V_74 } } ,
{ & V_122 ,
{ L_88 , L_89 ,
V_81 , V_75 , NULL , 0 ,
L_21 , V_74 } } ,
{ & V_123 ,
{ L_90 , L_91 ,
V_81 , V_75 , NULL , 0 ,
L_21 , V_74 } } ,
{ & V_124 ,
{ L_92 , L_93 ,
V_125 , V_78 , NULL , 0 ,
L_94 , V_74 } } ,
{ & V_126 ,
{ L_95 , L_96 ,
V_125 , V_78 , NULL , 0 ,
L_94 , V_74 } } ,
{ & V_127 ,
{ L_97 , L_98 ,
V_81 , V_75 , NULL , 0 ,
L_21 , V_74 } } ,
{ & V_128 ,
{ L_99 , L_100 ,
V_83 , V_75 , NULL , 0 ,
L_101 , V_74 } } ,
{ & V_129 ,
{ L_102 , L_103 ,
V_83 , V_75 , F_61 ( V_130 ) , 0 ,
NULL , V_74 } } ,
{ & V_131 ,
{ L_104 , L_105 ,
V_83 , V_75 , NULL , 0 ,
L_101 , V_74 } } ,
{ & V_132 ,
{ L_102 , L_103 ,
V_83 , V_75 , F_61 ( V_130 ) , 0 ,
NULL , V_74 } } ,
{ & V_133 ,
{ L_106 , L_107 ,
V_83 , V_75 , NULL , 0 ,
L_101 , V_74 } } ,
{ & V_134 ,
{ L_102 , L_103 ,
V_83 , V_75 , F_61 ( V_130 ) , 0 ,
NULL , V_74 } } ,
{ & V_135 ,
{ L_108 , L_109 ,
V_125 , V_78 , NULL , 0 ,
NULL , V_74 } } ,
{ & V_136 ,
{ L_110 , L_111 ,
V_137 , V_78 , NULL , 0 ,
NULL , V_74 } } ,
{ & V_138 ,
{ L_112 , L_113 ,
V_81 , V_75 , NULL , 0 ,
NULL , V_74 } } ,
{ & V_139 ,
{ L_114 , L_115 ,
V_81 , V_75 , NULL , 0 ,
L_21 , V_74 } } ,
{ & V_140 ,
{ L_116 , L_117 ,
V_137 , V_78 , NULL , 0 ,
L_118 , V_74 } } ,
{ & V_141 ,
{ L_119 , L_120 ,
V_142 , V_78 , NULL , 0 ,
NULL , V_74 } } ,
{ & V_143 ,
{ L_121 , L_122 ,
V_137 , V_78 , NULL , 0 ,
NULL , V_74 } } ,
{ & V_144 ,
{ L_123 , L_124 ,
V_99 , V_78 , NULL , 0 ,
L_46 , V_74 } } ,
{ & V_145 ,
{ L_125 , L_126 ,
V_137 , V_78 , NULL , 0 ,
L_127 , V_74 } } ,
{ & V_146 ,
{ L_128 , L_129 ,
V_81 , V_75 , NULL , 0 ,
L_21 , V_74 } } ,
{ & V_147 ,
{ L_130 , L_131 ,
V_137 , V_78 , NULL , 0 ,
L_132 , V_74 } } ,
{ & V_148 ,
{ L_133 , L_134 ,
V_149 , V_78 , NULL , 0 ,
L_135 , V_74 } } ,
{ & V_150 ,
{ L_136 , L_137 ,
V_99 , V_78 , NULL , 0 ,
NULL , V_74 } } ,
{ & V_151 ,
{ L_138 , L_139 ,
V_99 , V_78 , NULL , 0 ,
L_87 , V_74 } } ,
#line 127 "../../asn1/goose/packet-goose-template.c"
} ;
static T_18 * V_152 [] = {
& V_65 ,
#line 1 "../../asn1/goose/packet-goose-ettarr.c"
& V_52 ,
& V_29 ,
& V_27 ,
& V_12 ,
& V_25 ,
& V_6 ,
& V_4 ,
& V_10 ,
& V_8 ,
& V_23 ,
& V_21 ,
& V_19 ,
& V_17 ,
& V_15 ,
& V_50 ,
& V_45 ,
& V_48 ,
#line 133 "../../asn1/goose/packet-goose-template.c"
} ;
V_63 = F_62 ( V_61 , V_153 , V_154 ) ;
F_63 ( L_140 , F_53 , V_63 ) ;
F_64 ( V_63 , V_71 , F_65 ( V_71 ) ) ;
F_66 ( V_152 , F_65 ( V_152 ) ) ;
}
void F_67 ( void ) {
T_19 V_155 ;
V_155 = F_68 ( L_140 ) ;
F_69 ( L_141 , V_156 , V_155 ) ;
}
