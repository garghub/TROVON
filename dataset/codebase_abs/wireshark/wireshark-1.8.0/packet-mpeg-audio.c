static int
F_1 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_2 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
11 , 11 , FALSE , NULL ) ;
return T_3 ;
}
static int
F_3 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_4 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
4 , NULL , FALSE , 0 , NULL ) ;
return T_3 ;
}
static int
F_5 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_4 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
4 , NULL , FALSE , 0 , NULL ) ;
return T_3 ;
}
static int
F_6 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_4 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
2 , NULL , FALSE , 0 , NULL ) ;
return T_3 ;
}
static int
F_7 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_8 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
0U , 15U , NULL , FALSE ) ;
return T_3 ;
}
static int
F_9 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_8 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
0U , 3U , NULL , FALSE ) ;
return T_3 ;
}
static int
F_10 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_11 ( T_2 , T_3 , T_5 , T_7 , T_8 , NULL ) ;
return T_3 ;
}
static int
F_12 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_4 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
4 , NULL , FALSE , 0 , NULL ) ;
return T_3 ;
}
static int
F_13 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_4 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
4 , NULL , FALSE , 0 , NULL ) ;
return T_3 ;
}
static int
F_14 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_15 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
V_2 , V_3 ) ;
return T_3 ;
}
static int
F_16 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_17 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
3 , 3 , FALSE , NULL ) ;
return T_3 ;
}
static int
F_18 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_17 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
30 , 30 , FALSE , NULL ) ;
return T_3 ;
}
static int
F_19 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_17 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
4 , 4 , FALSE , NULL ) ;
return T_3 ;
}
static int
F_20 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_17 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
28 , 28 , FALSE , NULL ) ;
return T_3 ;
}
static int
F_21 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_8 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
0U , 255U , NULL , FALSE ) ;
return T_3 ;
}
static int
F_22 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_8 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
0U , 255U , NULL , FALSE ) ;
return T_3 ;
}
static int
F_23 ( T_1 * T_2 V_1 , int T_3 V_1 , T_4 * T_5 V_1 , T_6 * T_7 V_1 , int T_8 V_1 ) {
T_3 = F_15 ( T_2 , T_3 , T_5 , T_7 , T_8 ,
V_4 , V_5 ) ;
return T_3 ;
}
static T_9
F_24 ( T_1 * T_2 , T_10 * V_6 , T_6 * T_7 )
{
T_11 V_7 ;
struct V_8 V_8 ;
int V_9 = 0 ;
T_4 V_10 ;
int T_3 = 0 ;
static const char * V_11 [] = { L_1 , L_2 , L_3 } ;
if ( ! F_25 ( T_2 , 0 , 4 ) )
return FALSE ;
V_7 = F_26 ( T_2 , 0 ) ;
F_27 ( & V_8 , V_7 ) ;
if ( ! F_28 ( & V_8 ) )
return FALSE ;
if ( ! F_29 ( & V_8 ) )
return FALSE ;
if ( ! F_30 ( & V_8 ) )
return FALSE ;
F_31 ( V_6 -> V_12 , V_13 ,
L_4 , V_11 [ F_32 ( & V_8 ) ] ) ;
F_31 ( V_6 -> V_12 , V_14 ,
L_5 , F_33 ( & V_8 ) + 1 ) ;
if ( F_34 ( & V_8 ) && F_35 ( & V_8 ) ) {
V_9 = ( int ) ( F_36 ( & V_8 ) - sizeof V_8 ) ;
if ( F_37 ( V_6 -> V_12 , V_15 ) ) {
F_38 ( & V_6 -> V_16 , V_17 , 0 , NULL ) ;
F_31 ( V_6 -> V_12 , V_15 ,
L_6 , F_39 ( & V_8 ) / 1000 ) ;
}
if ( F_37 ( V_6 -> V_12 , V_18 ) ) {
F_38 ( & V_6 -> V_19 , V_17 , 0 , NULL ) ;
F_31 ( V_6 -> V_12 , V_18 ,
L_7 , F_40 ( & V_8 ) / ( float ) 1000 ) ;
}
}
if ( T_7 == NULL )
return TRUE ;
F_41 ( & V_10 , V_20 , TRUE , V_6 ) ;
T_3 = F_14 ( T_2 , T_3 , & V_10 ,
T_7 , V_21 ) ;
if ( V_9 > 0 ) {
unsigned int V_22 ;
F_42 ( T_7 , V_23 , T_2 ,
T_3 / 8 , V_9 , V_24 ) ;
T_3 += V_9 * 8 ;
V_22 = F_43 ( & V_8 ) ;
if ( V_22 > 0 ) {
F_42 ( T_7 , V_25 , T_2 ,
T_3 / 8 , V_22 , V_24 ) ;
}
}
return TRUE ;
}
static void
F_44 ( T_1 * T_2 , T_10 * V_6 , T_6 * T_7 )
{
T_4 V_10 ;
F_45 ( V_6 -> V_12 , V_13 , L_8 ) ;
F_46 ( V_6 -> V_12 , V_14 ) ;
if ( T_7 == NULL )
return;
F_41 ( & V_10 , V_20 , TRUE , V_6 ) ;
F_23 ( T_2 , 0 , & V_10 ,
T_7 , V_26 ) ;
}
static void
F_47 ( T_1 * T_2 , T_10 * V_6 , T_6 * T_7 )
{
F_45 ( V_6 -> V_12 , V_13 , L_9 ) ;
F_46 ( V_6 -> V_12 , V_14 ) ;
F_42 ( T_7 , V_27 , T_2 ,
0 , - 1 , V_24 ) ;
}
static T_9
F_48 ( T_1 * T_2 , T_10 * V_6 , T_6 * T_7 )
{
int V_28 ;
if ( ! F_25 ( T_2 , 0 , 3 ) )
return FALSE ;
V_28 = F_49 ( T_2 , 0 ) ;
switch ( V_28 ) {
case 0x544147 :
F_44 ( T_2 , V_6 , T_7 ) ;
return TRUE ;
case 0x494433 :
F_47 ( T_2 , V_6 , T_7 ) ;
return TRUE ;
default:
return F_24 ( T_2 , V_6 , T_7 ) ;
}
}
void
F_50 ( void )
{
static T_12 V_29 [] = {
#line 1 "../../asn1/mpeg-audio/packet-mpeg-audio-hfarr.c"
{ & V_30 ,
{ L_10 , L_11 ,
V_31 , V_32 , NULL , 0 ,
L_12 , V_33 } } ,
{ & V_34 ,
{ L_13 , L_14 ,
V_35 , V_36 , F_51 ( V_37 ) , 0 ,
NULL , V_33 } } ,
{ & V_38 ,
{ L_15 , L_16 ,
V_35 , V_36 , F_51 ( V_39 ) , 0 ,
NULL , V_33 } } ,
{ & V_40 ,
{ L_17 , L_18 ,
V_35 , V_36 , F_51 ( V_41 ) , 0 ,
NULL , V_33 } } ,
{ & V_42 ,
{ L_19 , L_20 ,
V_35 , V_36 , NULL , 0 ,
L_21 , V_33 } } ,
{ & V_43 ,
{ L_22 , L_23 ,
V_35 , V_36 , NULL , 0 ,
L_24 , V_33 } } ,
{ & V_44 ,
{ L_25 , L_26 ,
V_45 , V_32 , NULL , 0 ,
L_27 , V_33 } } ,
{ & V_46 ,
{ L_28 , L_29 ,
V_45 , V_32 , NULL , 0 ,
L_27 , V_33 } } ,
{ & V_47 ,
{ L_30 , L_31 ,
V_35 , V_36 , F_51 ( V_48 ) , 0 ,
NULL , V_33 } } ,
{ & V_49 ,
{ L_32 , L_33 ,
V_35 , V_36 , NULL , 0 ,
L_24 , V_33 } } ,
{ & V_50 ,
{ L_34 , L_35 ,
V_45 , V_32 , NULL , 0 ,
L_27 , V_33 } } ,
{ & V_51 ,
{ L_36 , L_37 ,
V_45 , V_32 , NULL , 0 ,
L_27 , V_33 } } ,
{ & V_52 ,
{ L_38 , L_39 ,
V_35 , V_36 , F_51 ( V_53 ) , 0 ,
NULL , V_33 } } ,
{ & V_54 ,
{ L_40 , L_41 ,
V_55 , V_32 , NULL , 0 ,
L_42 , V_33 } } ,
{ & V_56 ,
{ L_43 , L_44 ,
V_55 , V_32 , NULL , 0 ,
L_45 , V_33 } } ,
{ & V_57 ,
{ L_46 , L_47 ,
V_55 , V_32 , NULL , 0 ,
L_45 , V_33 } } ,
{ & V_58 ,
{ L_48 , L_49 ,
V_55 , V_32 , NULL , 0 ,
L_45 , V_33 } } ,
{ & V_59 ,
{ L_50 , L_51 ,
V_55 , V_32 , NULL , 0 ,
L_52 , V_33 } } ,
{ & V_60 ,
{ L_53 , L_54 ,
V_55 , V_32 , NULL , 0 ,
L_55 , V_33 } } ,
{ & V_61 ,
{ L_56 , L_57 ,
V_35 , V_36 , NULL , 0 ,
L_58 , V_33 } } ,
{ & V_62 ,
{ L_59 , L_60 ,
V_35 , V_36 , NULL , 0 ,
L_58 , V_33 } } ,
{ & V_63 ,
{ L_61 , L_62 ,
V_35 , V_36 , F_51 ( V_64 ) , 0 ,
NULL , V_33 } } ,
#line 159 "../../asn1/mpeg-audio/packet-mpeg-audio-template.c"
{ & V_23 ,
{ L_63 , L_64 ,
V_31 , V_32 , NULL , 0 , NULL , V_33 } } ,
{ & V_25 ,
{ L_65 , L_66 ,
V_31 , V_32 , NULL , 0 , NULL , V_33 } } ,
{ & V_26 ,
{ L_8 , L_67 ,
V_65 , V_32 , NULL , 0 , NULL , V_33 } } ,
{ & V_27 ,
{ L_9 , L_68 ,
V_65 , V_32 , NULL , 0 , NULL , V_33 } } ,
} ;
static T_13 * V_66 [] = {
#line 1 "../../asn1/mpeg-audio/packet-mpeg-audio-ettarr.c"
& V_2 ,
& V_4 ,
#line 176 "../../asn1/mpeg-audio/packet-mpeg-audio-template.c"
} ;
V_21 = F_52 (
L_69 , L_70 , L_71 ) ;
F_53 ( V_21 , V_29 , F_54 ( V_29 ) ) ;
F_55 ( V_66 , F_54 ( V_66 ) ) ;
}
void
F_56 ( void )
{
F_57 ( L_72 , F_48 , V_21 ) ;
}
