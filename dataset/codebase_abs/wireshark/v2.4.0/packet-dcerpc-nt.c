int
F_1 ( T_1 * V_1 , int V_2 , T_2 * V_3 ,
T_3 * V_4 , T_4 * V_5 , T_5 * V_6 , int V_7 ,
int V_8 )
{
T_6 * V_9 ;
T_7 V_10 ;
T_3 * V_11 ;
V_11 = F_2 ( V_4 , V_1 , V_2 , 0 , V_12 , & V_9 ,
F_3 ( V_7 ) ) ;
if ( V_8 ) {
V_10 = F_4 ( V_1 , V_2 ) ;
} else {
V_2 = F_5 ( V_1 , V_2 , V_3 , V_11 , V_5 , V_6 ,
V_13 , & V_10 ) ;
}
F_6 ( V_11 , V_14 , V_1 , V_2 , ( int ) V_10 , V_15 ) ;
V_2 += ( int ) V_10 ;
return V_2 ;
}
int
F_7 ( T_1 * V_1 , int V_2 ,
T_2 * V_3 V_16 , T_3 * V_4 ,
T_5 * V_6 V_16 , int V_7 , int T_8 V_16 )
{
T_9 V_10 ;
V_10 = F_8 ( V_1 , V_2 ) ;
F_6 ( V_4 , V_7 , V_1 , V_2 , V_10 , V_17 | V_15 ) ;
return V_2 + V_10 ;
}
int
F_9 ( T_1 * V_1 , int V_2 ,
T_2 * V_3 V_16 , T_3 * V_4 ,
T_5 * V_6 V_16 , int V_7 , int T_8 V_16 )
{
T_9 V_10 ;
V_10 = F_10 ( V_1 , V_2 ) ;
F_6 ( V_4 , V_7 , V_1 , V_2 , V_10 , V_18 | V_19 ) ;
return V_2 + V_10 ;
}
int
F_11 ( T_1 * V_1 , int V_2 ,
T_2 * V_3 , T_3 * V_4 ,
T_4 * V_5 , T_5 * V_6 , int V_7 ,
T_10 * V_20 ,
void * V_21 )
{
T_11 V_10 , V_22 ;
V_23 ;
if ( V_5 -> V_24 )
return V_2 ;
V_2 = F_12 ( V_1 , V_2 , V_3 , V_4 , V_5 , V_6 ,
V_25 , & V_10 ) ;
V_2 = F_12 ( V_1 , V_2 , V_3 , V_4 , V_5 , V_6 ,
V_26 , & V_22 ) ;
V_2 = F_13 ( V_1 , V_2 , V_3 , V_4 , V_5 , V_6 ,
V_27 , V_28 ,
L_1 , V_7 , V_20 , V_21 ) ;
if ( V_5 -> V_29 -> V_30 & V_31 ) {
V_23 ;
}
return V_2 ;
}
static int
F_14 ( T_1 * V_1 , int V_2 ,
T_2 * V_3 , T_3 * V_4 ,
T_4 * V_5 , T_5 * V_6 , int V_7 , int T_8 ,
T_12 V_32 )
{
T_6 * V_9 ;
T_3 * V_11 = V_4 ;
if ( V_32 ) {
V_11 = F_2 (
V_4 , V_1 , V_2 , 0 , V_33 , & V_9 ,
F_3 ( V_7 ) ) ;
}
return F_11 (
V_1 , V_2 , V_3 , V_11 , V_5 , V_6 , V_7 ,
V_34 , F_15 ( 2 + T_8 ) ) ;
}
int
F_16 ( T_1 * V_1 , int V_2 ,
T_2 * V_3 , T_3 * V_4 ,
T_4 * V_5 , T_5 * V_6 , int V_7 , int T_8 )
{
return F_14 (
V_1 , V_2 , V_3 , V_4 , V_5 , V_6 , V_7 , T_8 , TRUE ) ;
}
int
F_17 ( T_1 * V_1 , int V_2 ,
T_2 * V_3 , T_3 * V_4 ,
T_4 * V_5 , T_5 * V_6 )
{
return F_14 (
V_1 , V_2 , V_3 , V_4 , V_5 , V_6 , V_5 -> V_7 , 0 , FALSE ) ;
}
int
F_18 ( T_1 * V_1 , int V_2 ,
T_2 * V_3 , T_3 * V_4 ,
T_4 * V_5 , T_5 * V_6 , int V_7 ,
T_10 * V_20 ,
void * V_21 )
{
T_6 * V_9 ;
T_3 * V_11 ;
T_11 V_10 , V_22 ;
V_23 ;
if ( V_5 -> V_24 )
return V_2 ;
V_11 = F_2 ( V_4 , V_1 , V_2 , 0 , V_35 , & V_9 ,
F_3 ( V_7 ) ) ;
V_2 = F_12 ( V_1 , V_2 , V_3 , V_11 , V_5 , V_6 ,
V_25 , & V_10 ) ;
V_2 = F_12 ( V_1 , V_2 , V_3 , V_11 , V_5 , V_6 ,
V_26 , & V_22 ) ;
V_2 = F_13 ( V_1 , V_2 , V_3 , V_11 , V_5 , V_6 ,
V_36 , V_28 ,
L_2 , V_7 , V_20 , V_21 ) ;
if ( V_5 -> V_29 -> V_30 & V_31 ) {
V_23 ;
}
return V_2 ;
}
static void F_19 ( T_2 * V_3 , T_3 * V_4 V_16 ,
T_6 * V_9 , T_4 * V_5 V_16 , T_1 * V_1 ,
int V_37 , int V_38 ,
void * V_21 )
{
T_13 V_39 = F_20 ( V_21 ) ;
T_13 T_8 = F_21 ( V_39 ) ;
char * V_40 ;
if ( V_37 % 4 )
V_37 += 4 - ( V_37 % 4 ) ;
if ( ( V_38 - V_37 ) <= 12 )
return;
V_40 = F_22 ( F_23 () , V_1 , V_37 + 12 , ( V_38 - V_37 - 12 ) ) ;
if ( V_39 & V_41 ) {
F_24 ( V_3 -> V_42 , V_43 , L_3 , V_40 ) ;
}
if ( T_8 > 0 && V_9 && V_40 && V_40 [ 0 ] ) {
F_25 ( V_9 , L_4 , V_40 ) ;
V_9 = F_26 ( V_9 ) ;
T_8 -- ;
if ( T_8 > 0 ) {
F_25 ( V_9 , L_4 , V_40 ) ;
V_9 = F_26 ( V_9 ) ;
T_8 -- ;
while ( T_8 > 0 ) {
F_25 ( V_9 , L_5 , V_40 ) ;
V_9 = F_26 ( V_9 ) ;
T_8 -- ;
}
}
}
}
int
F_27 ( T_1 * V_1 , int V_2 ,
T_2 * V_3 , T_3 * V_4 ,
T_4 * V_5 , T_5 * V_6 , int V_7 , int T_8 )
{
return F_18 (
V_1 , V_2 , V_3 , V_4 , V_5 , V_6 , V_7 , F_19 , F_15 ( 2 + T_8 ) ) ;
}
int
F_28 ( T_1 * V_1 , int V_2 ,
T_2 * V_3 , T_3 * V_4 ,
T_4 * V_5 , T_5 * V_6 , int V_7 ,
T_10 * V_20 ,
void * V_21 )
{
T_6 * V_9 ;
T_3 * V_11 ;
T_11 V_10 , V_22 ;
V_23 ;
if ( V_5 -> V_24 )
return V_2 ;
V_11 = F_2 ( V_4 , V_1 , V_2 , 0 , V_44 , & V_9 ,
F_3 ( V_7 ) ) ;
V_2 = F_12 ( V_1 , V_2 , V_3 , V_11 , V_5 , V_6 ,
V_25 , & V_10 ) ;
V_2 = F_12 ( V_1 , V_2 , V_3 , V_11 , V_5 , V_6 ,
V_26 , & V_22 ) ;
V_2 = F_13 ( V_1 , V_2 , V_3 , V_11 , V_5 , V_6 ,
V_36 , V_28 ,
L_6 , V_7 , V_20 , V_21 ) ;
if ( V_5 -> V_29 -> V_30 & V_31 ) {
V_23 ;
}
return V_2 ;
}
int
F_29 ( T_1 * V_1 , int V_2 ,
T_2 * V_3 , T_3 * V_4 ,
T_4 * V_5 , T_5 * V_6 , int V_7 , int T_8 )
{
return F_28 (
V_1 , V_2 , V_3 , V_4 , V_5 , V_6 , V_7 , V_45 , F_15 ( 2 + T_8 ) ) ;
}
int
F_30 ( T_1 * V_1 , int V_2 ,
T_2 * V_3 , T_3 * V_4 ,
T_4 * V_5 , T_5 * V_6 )
{
V_2 = F_31 ( V_1 , V_2 , V_3 , V_4 , V_5 , V_6 , V_46 , NULL ) ;
return V_2 ;
}
int
F_32 ( T_1 * V_1 , int V_2 , T_2 * V_3 , T_3 * V_47 , T_4 * V_5 , T_5 * V_6 , T_14 V_48 , int V_49 )
{
T_6 * V_9 = NULL ;
T_3 * V_4 = NULL ;
int V_50 ;
T_15 * V_51 ;
V_23 ;
V_50 = V_2 ;
V_51 = F_33 ( V_49 ) ;
if ( V_47 ) {
V_4 = F_34 ( V_47 , V_1 , V_2 , 0 , V_52 , & V_9 , L_7 , V_51 -> V_53 ) ;
}
V_2 = F_35 ( V_1 , V_2 , V_3 , V_4 , V_5 , V_6 , V_54 , 0 ) ;
V_2 = F_35 ( V_1 , V_2 , V_3 , V_4 , V_5 , V_6 , V_55 , 0 ) ;
V_2 = F_13 (
V_1 , V_2 , V_3 , V_4 , V_5 , V_6 ,
V_27 , V_28 ,
V_51 -> V_53 , V_49 , V_34 ,
F_15 ( V_48 ) ) ;
F_36 ( V_9 , V_2 - V_50 ) ;
if ( V_5 -> V_29 -> V_30 & V_31 ) {
V_23 ;
}
return V_2 ;
}
int
F_37 ( T_1 * V_1 , int V_2 ,
T_2 * V_3 V_16 , T_3 * V_4 ,
T_4 * V_5 , T_5 * V_6 V_16 , int V_7 )
{
if( V_5 -> V_24 ) {
return V_2 ;
}
V_56 ;
V_2 = F_38 ( V_1 , V_4 , V_2 , V_7 ) ;
return V_2 ;
}
int
F_39 ( T_1 * V_1 , int V_2 ,
T_2 * V_3 V_16 , T_3 * V_4 ,
T_4 * V_5 , T_5 * V_6 V_16 , int V_7 , T_12 V_57 )
{
if( V_5 -> V_24 ) {
return V_2 ;
}
V_58 ;
V_2 = F_40 ( V_1 , V_4 , V_2 , V_7 , V_57 ) ;
return V_2 ;
}
static T_9 F_41 ( T_16 V_59 )
{
const T_17 * V_60 = ( const T_17 * ) V_59 ;
return V_60 -> V_61 [ 4 ] + ( V_60 -> V_61 [ 5 ] << 8 ) +
( V_60 -> V_61 [ 6 ] << 16 ) + ( V_60 -> V_61 [ 7 ] << 24 ) ;
}
static T_12 F_42 ( T_18 * V_61 )
{
static T_5 V_62 [ 20 ] ;
return memcmp ( V_61 , V_62 , 20 ) == 0 ;
}
static T_13 F_43 ( T_16 V_63 , T_16 V_64 )
{
const T_17 * V_65 = ( const T_17 * ) V_63 ;
const T_17 * V_66 = ( const T_17 * ) V_64 ;
return memcmp ( V_65 -> V_61 , V_66 -> V_61 ,
sizeof( V_65 -> V_61 ) ) == 0 ;
}
static T_19 * F_44 ( T_18 * V_61 , T_14 V_67 ,
T_20 * * V_68 )
{
T_17 V_60 ;
T_19 * V_69 ;
memcpy ( & V_60 . V_61 , V_61 , sizeof( V_60 . V_61 ) ) ;
if ( ( * V_68 = ( T_20 * ) F_45 ( V_70 , & V_60 ) ) ) {
for ( V_69 = ( * V_68 ) -> V_71 ; V_69 != NULL ; V_69 = V_69 -> V_72 ) {
if ( V_69 -> V_73 <= V_67 &&
( V_69 -> V_74 == 0 ||
V_69 -> V_74 >= V_67 ) )
break;
}
return V_69 ;
} else {
return NULL ;
}
}
static void F_46 ( T_18 * V_61 , T_14 V_67 ,
T_19 * V_69 , T_20 * V_75 )
{
T_17 * V_60 ;
T_19 * V_76 , * V_77 ;
if ( V_75 == NULL ) {
V_75 = ( T_20 * ) F_47 ( F_48 () , sizeof( T_20 ) ) ;
V_75 -> V_71 = V_69 ;
V_69 -> V_72 = NULL ;
V_60 = ( T_17 * ) F_47 ( F_48 () , sizeof( T_17 ) ) ;
memcpy ( & V_60 -> V_61 , V_61 , sizeof( V_60 -> V_61 ) ) ;
F_49 ( V_70 , V_60 , V_75 ) ;
} else {
for ( V_77 = V_75 -> V_71 , V_76 = NULL ;
V_77 != NULL && V_77 -> V_73 <= V_67 ;
V_76 = V_77 , V_77 = V_77 -> V_72 )
;
if ( V_76 == NULL )
V_75 -> V_71 = V_69 ;
else
V_76 -> V_72 = V_69 ;
V_69 -> V_72 = V_77 ;
}
}
void F_50 ( T_18 * V_61 , T_2 * V_3 ,
T_12 V_78 , T_12 V_79 )
{
T_20 * V_75 ;
T_19 * V_69 ;
if ( V_3 -> V_80 -> V_30 . V_81 )
return;
if ( F_42 ( V_61 ) )
return;
V_69 = F_44 ( V_61 , V_3 -> V_82 , & V_75 ) ;
if ( V_69 != NULL ) {
if ( V_78 ) {
if ( V_69 -> V_73 == V_3 -> V_82 &&
V_69 -> V_74 == 0 )
return;
V_69 -> V_74 = V_3 -> V_82 ;
V_69 = NULL ;
} else {
if ( V_79 ) {
V_69 -> V_83 = V_3 -> V_82 ;
V_69 -> V_74 = V_3 -> V_82 ;
}
return;
}
}
V_69 = ( T_19 * ) F_47 ( F_48 () , sizeof( T_19 ) ) ;
V_69 -> V_84 = V_78 ? V_3 -> V_82 : 0 ;
V_69 -> V_83 = V_79 ? V_3 -> V_82 : 0 ;
V_69 -> V_73 = V_3 -> V_82 ;
V_69 -> V_74 = V_69 -> V_83 ;
V_69 -> type = 0 ;
V_69 -> V_53 = NULL ;
F_46 ( V_61 , V_3 -> V_82 , V_69 , V_75 ) ;
}
static void F_51 ( T_18 * V_61 , T_2 * V_3 ,
T_14 type )
{
T_20 * V_75 ;
T_19 * V_69 ;
if ( V_3 -> V_80 -> V_30 . V_81 )
return;
if ( F_42 ( V_61 ) )
return;
V_69 = F_44 ( V_61 , V_3 -> V_82 , & V_75 ) ;
if ( V_69 != NULL ) {
V_69 -> type = type ;
}
}
void F_52 ( T_18 * V_61 , T_2 * V_3 ,
const char * V_53 )
{
T_20 * V_75 ;
T_19 * V_69 ;
if ( V_3 -> V_80 -> V_30 . V_81 )
return;
if ( F_42 ( V_61 ) )
return;
V_69 = F_44 ( V_61 , V_3 -> V_82 , & V_75 ) ;
if ( V_69 != NULL ) {
if ( V_69 -> V_53 && V_53 ) {
#ifdef F_53
if ( strcmp ( V_69 -> V_53 , V_53 ) != 0 )
F_54 ( L_8 , V_75 -> V_53 , V_53 ) ;
#endif
}
V_69 -> V_53 = F_55 ( F_48 () , V_53 ) ;
return;
}
V_69 = ( T_19 * ) F_47 ( F_48 () , sizeof( T_19 ) ) ;
V_69 -> V_84 = 0 ;
V_69 -> V_83 = 0 ;
V_69 -> V_73 = V_3 -> V_82 ;
V_69 -> V_74 = 0 ;
V_69 -> type = 0 ;
if ( V_53 )
V_69 -> V_53 = F_55 ( F_48 () , V_53 ) ;
else
V_69 -> V_53 = F_55 ( F_48 () , L_9 ) ;
F_46 ( V_61 , V_3 -> V_82 , V_69 , V_75 ) ;
}
T_12 F_56 ( T_18 * V_61 ,
char * * V_53 , T_14 * type ,
T_14 * V_84 , T_14 * V_83 ,
T_14 V_85 )
{
T_20 * V_75 ;
T_19 * V_69 ;
if ( V_53 )
* V_53 = NULL ;
if ( type )
* type = 0 ;
if ( V_84 )
* V_84 = 0 ;
if ( V_83 )
* V_83 = 0 ;
V_69 = F_44 ( V_61 , V_85 , & V_75 ) ;
if ( V_69 ) {
if ( V_53 )
* V_53 = V_69 -> V_53 ;
if ( type )
* type = V_69 -> type ;
if ( V_84 )
* V_84 = V_69 -> V_84 ;
if ( V_83 )
* V_83 = V_69 -> V_83 ;
}
return V_69 != NULL ;
}
int
F_57 ( T_1 * V_1 , T_13 V_2 , T_2 * V_3 ,
T_3 * V_4 , T_4 * V_5 , T_5 * V_6 ,
int V_49 , T_14 * V_86 )
{
T_14 V_87 ;
V_2 = F_58 ( V_1 , V_2 , V_3 , V_4 , V_5 , V_6 ,
V_49 , & V_87 ) ;
if ( V_87 != 0 )
F_24 ( V_3 -> V_42 , V_43 , L_3 ,
F_59 ( V_87 , & V_88 ,
L_10 ) ) ;
if ( V_86 )
* V_86 = V_87 ;
return V_2 ;
}
int
F_60 ( T_1 * V_1 , T_13 V_2 , T_2 * V_3 ,
T_3 * V_4 , T_4 * V_5 , T_5 * V_6 ,
int V_49 , T_14 * V_86 )
{
T_14 V_87 ;
V_2 = F_58 ( V_1 , V_2 , V_3 , V_4 , V_5 , V_6 ,
V_49 , & V_87 ) ;
if ( V_87 != 0 )
F_24 ( V_3 -> V_42 , V_43 , L_3 ,
F_59 ( V_87 , & V_89 ,
L_10 ) ) ;
if ( V_86 )
* V_86 = V_87 ;
return V_2 ;
}
static int
F_61 ( T_1 * V_1 , T_13 V_2 , T_2 * V_3 ,
T_3 * V_4 , T_4 * V_5 , T_5 * V_6 , int V_49 ,
T_18 * V_86 , T_6 * * V_90 ,
T_12 V_78 , T_12 V_79 , T_21 type )
{
T_6 * V_9 = NULL ;
T_3 * V_11 ;
T_18 V_91 ;
T_14 V_84 = 0 , V_83 = 0 ;
char * V_53 ;
int V_50 = V_2 ;
if( V_5 -> V_24 ) {
return V_2 ;
}
switch( type ) {
case V_92 :
if ( ! V_5 -> V_93 && ( V_2 % 4 ) ) {
V_2 += 4 - ( V_2 % 4 ) ;
}
V_11 = F_2 ( V_4 , V_1 , V_2 , sizeof( T_18 ) ,
V_94 , & V_9 , L_11 ) ;
V_2 = F_62 ( V_1 , V_2 , V_3 , V_11 , V_5 , V_6 ,
V_49 , & V_91 ) ;
break;
case V_95 :
V_11 = F_2 ( V_4 , V_1 , V_2 , 16 ,
V_94 , & V_9 , L_12 ) ;
V_91 . V_96 = 0 ;
V_2 = F_31 ( V_1 , V_2 , V_3 , V_11 , V_5 , V_6 , V_49 , & V_91 . V_97 ) ;
break;
default:
F_63 () ;
return V_2 ;
}
F_50 ( & V_91 , V_3 , V_78 , V_79 ) ;
if ( F_56 ( & V_91 , & V_53 , NULL , & V_84 ,
& V_83 , V_3 -> V_82 ) ) {
if ( V_84 ) {
T_6 * V_98 ;
V_98 = F_64 (
V_11 , V_99 , V_1 ,
V_50 , sizeof( T_18 ) , V_84 ) ;
F_65 ( V_98 ) ;
}
if ( V_83 ) {
T_6 * V_98 ;
V_98 = F_64 (
V_11 , V_100 , V_1 ,
V_50 , sizeof( T_18 ) , V_83 ) ;
F_65 ( V_98 ) ;
}
if ( V_53 != NULL && V_90 == NULL )
F_25 ( V_9 , L_4 , V_53 ) ;
}
if ( V_86 )
* V_86 = V_91 ;
if ( V_90 )
* V_90 = V_9 ;
return V_2 ;
}
int
F_66 ( T_1 * V_1 , T_13 V_2 , T_2 * V_3 ,
T_3 * V_4 , T_4 * V_5 , T_5 * V_6 , int V_49 ,
T_18 * V_86 , T_6 * * V_90 ,
T_12 V_78 , T_12 V_79 )
{
V_2 = F_61 ( V_1 , V_2 , V_3 ,
V_4 , V_5 , V_6 , V_49 ,
V_86 , V_90 ,
V_78 , V_79 , V_92 ) ;
return V_2 ;
}
int
F_67 ( T_1 * V_1 , T_13 V_2 , T_2 * V_3 ,
T_3 * V_4 , T_4 * V_5 , T_5 * V_6 , int V_49 ,
T_14 V_48 )
{
T_18 V_61 ;
V_2 = F_61 ( V_1 , V_2 , V_3 ,
V_4 , V_5 , V_6 , V_49 ,
& V_61 , NULL ,
V_48 & V_101 , V_48 & V_102 ,
V_92 ) ;
if( ( V_48 & V_101 )
&& ! V_3 -> V_80 -> V_30 . V_81
&& ! V_5 -> V_24 ) {
char * V_103 = NULL ;
const char * V_104 = NULL ;
T_22 * V_105 ;
V_105 = ( T_22 * ) V_5 -> V_29 ;
V_104 = ( const char * ) V_105 -> V_106 ;
if( ! V_104 ) {
V_104 = L_13 ;
}
V_103 = F_68 ( F_23 () , L_14 , V_5 -> V_107 , V_104 ) ;
F_52 ( & V_61 , V_3 , V_103 ) ;
F_51 ( & V_61 , V_3 , V_48 & V_108 ) ;
}
if( ! V_3 -> V_80 -> V_30 . V_81
&& ! V_5 -> V_24 ) {
T_22 * V_105 ;
V_105 = ( T_22 * ) V_5 -> V_29 ;
if( ! V_105 -> V_69 ) {
V_105 -> V_69 = ( T_18 * ) F_69 ( F_48 () , & V_61 , sizeof( T_18 ) ) ;
}
}
return V_2 ;
}
int
F_70 ( T_1 * V_1 , T_13 V_2 , T_2 * V_3 ,
T_3 * V_4 , T_4 * V_5 , T_5 * V_6 , int V_49 ,
T_18 * V_86 , T_6 * * V_90 ,
T_12 V_78 , T_12 V_79 )
{
V_2 = F_61 ( V_1 , V_2 , V_3 ,
V_4 , V_5 , V_6 , V_49 ,
V_86 , V_90 ,
V_78 , V_79 , V_95 ) ;
return V_2 ;
}
int
F_71 ( T_1 * V_1 , T_13 V_2 , T_2 * V_3 V_16 ,
T_3 * V_4 , T_4 * V_5 V_16 , T_5 * V_6 V_16 , int V_49 ,
int V_109 , const T_5 * * V_86 )
{
const T_5 * V_110 ;
V_110 = ( const T_5 * ) F_72 ( V_1 , V_2 , V_109 ) ;
if ( V_4 ) {
F_6 ( V_4 , V_49 , V_1 , V_2 , V_109 , V_15 ) ;
}
if ( V_86 )
* V_86 = V_110 ;
return V_2 + V_109 ;
}
int
F_73 ( T_1 * V_1 , T_13 V_2 , T_2 * V_3 ,
T_3 * V_4 , T_4 * V_5 , T_5 * V_6 ,
int V_49 , int V_109 , const T_5 * * V_86 )
{
if( V_5 -> V_24 ) {
return V_2 ;
}
return F_71 ( V_1 , V_2 , V_3 ,
V_4 , V_5 , V_6 , V_49 , V_109 , V_86 ) ;
}
int
F_74 ( T_1 * V_1 , T_13 V_2 , T_2 * V_3 V_16 ,
T_3 * V_4 , T_5 * V_6 , int V_49 ,
int V_109 )
{
if ( V_4 ) {
F_6 ( V_4 , V_49 , V_1 , V_2 , V_109 * 2 , F_75 ( V_6 ) ) ;
}
return V_2 + V_109 * 2 ;
}
int
F_76 ( T_1 * V_1 , T_13 V_2 , T_2 * V_3 ,
T_3 * V_4 , T_4 * V_5 , T_5 * V_6 ,
int V_49 , int V_109 )
{
if( V_5 -> V_24 ) {
return V_2 ;
}
if ( V_2 % 2 )
V_2 ++ ;
return F_74 ( V_1 , V_2 , V_3 ,
V_4 , V_6 , V_49 , V_109 ) ;
}
void V_34 ( T_2 * V_3 , T_3 * V_4 V_16 ,
T_6 * V_9 , T_4 * V_5 , T_1 * V_1 ,
int V_37 , int V_38 ,
void * V_21 )
{
T_13 V_39 = F_20 ( V_21 ) ;
T_13 T_8 = F_21 ( V_39 ) ;
char * V_40 ;
if ( V_37 % 4 )
V_37 += 4 - ( V_37 % 4 ) ;
if ( ( V_38 - V_37 ) <= 12 )
return;
V_40 = F_77 ( F_23 () ,
V_1 , V_37 + 12 , V_38 - V_37 - 12 ,
V_18 | V_19 ) ;
if ( V_39 & V_41 ) {
F_24 ( V_3 -> V_42 , V_43 , L_3 , V_40 ) ;
}
if ( T_8 > 0 && V_9 && V_40 && V_40 [ 0 ] ) {
F_25 ( V_9 , L_4 , V_40 ) ;
V_9 = F_26 ( V_9 ) ;
T_8 -- ;
if ( V_9 && T_8 > 0 ) {
F_25 ( V_9 , L_4 , V_40 ) ;
V_9 = F_26 ( V_9 ) ;
T_8 -- ;
while ( V_9 && T_8 > 0 ) {
F_25 ( V_9 , L_5 , V_40 ) ;
V_9 = F_26 ( V_9 ) ;
T_8 -- ;
}
}
}
if ( V_39 & V_111 ) {
T_22 * V_105 = ( T_22 * ) V_5 -> V_29 ;
V_105 -> V_106 = F_55 ( F_48 () , V_40 ) ;
}
}
void V_45 ( T_2 * V_3 , T_3 * V_4 V_16 ,
T_6 * V_9 , T_4 * V_5 , T_1 * V_1 ,
int V_37 , int V_38 ,
void * V_21 )
{
T_13 V_39 = F_20 ( V_21 ) ;
T_13 T_8 = F_21 ( V_39 ) ;
T_5 * V_40 ;
if ( V_37 % 4 )
V_37 += 4 - ( V_37 % 4 ) ;
if ( ( V_38 - V_37 ) <= 12 )
return;
V_40 = F_77 ( F_23 () ,
V_1 , V_37 + 12 , ( V_38 - V_37 - 12 ) , V_17 ) ;
if ( V_39 & V_41 ) {
F_24 ( V_3 -> V_42 , V_43 , L_3 , V_40 ) ;
}
if ( T_8 > 0 && V_9 && V_40 && V_40 [ 0 ] ) {
F_25 ( V_9 , L_4 , V_40 ) ;
V_9 = F_26 ( V_9 ) ;
T_8 -- ;
if ( T_8 > 0 ) {
F_25 ( V_9 , L_4 , V_40 ) ;
V_9 = F_26 ( V_9 ) ;
T_8 -- ;
while ( T_8 > 0 ) {
F_25 ( V_9 , L_5 , V_40 ) ;
V_9 = F_26 ( V_9 ) ;
T_8 -- ;
}
}
}
if ( V_39 & V_111 ) {
T_22 * V_105 = ( T_22 * ) V_5 -> V_29 ;
V_105 -> V_106 = F_55 ( F_48 () , V_40 ) ;
}
}
int F_78 ( T_1 * V_1 , T_13 V_2 ,
T_2 * V_3 , T_3 * V_4 ,
T_4 * V_5 , T_5 * V_6 , int type , const char * V_112 ,
int V_7 , int T_8 )
{
return F_13 (
V_1 , V_2 , V_3 , V_4 , V_5 , V_6 ,
V_27 , type , V_112 , V_7 ,
V_34 , F_15 ( T_8 + 1 ) ) ;
}
int
F_79 ( T_1 * V_1 , int V_2 , T_2 * V_3 ,
T_3 * V_4 , T_4 * V_5 , T_5 * V_6 V_16 )
{
T_6 * V_9 ;
T_22 * V_105 = ( T_22 * ) V_5 -> V_29 ;
char * V_113 = NULL ;
const char * V_53 ;
int V_114 ;
if( V_5 -> V_7 != - 1 ) {
V_53 = F_3 ( V_5 -> V_7 ) ;
} else {
V_53 = L_15 ;
}
if( V_5 -> V_24 ) {
return V_2 ;
}
V_114 = F_80 ( V_1 , V_2 , V_4 , V_53 , & V_113 ,
V_115 ) ;
if ( ( V_114 - V_2 ) > 28 ) {
V_9 = F_81 ( V_4 ? V_4 -> V_116 : NULL ) ;
F_82 ( V_3 , V_9 , & V_117 ) ;
return V_114 ;
}
if ( V_4 ) {
V_9 = F_81 ( V_4 -> V_116 ) ;
F_36 ( V_9 , 28 ) ;
}
V_2 += 28 ;
if( V_105 ) {
V_105 -> V_106 = F_55 ( F_48 () , V_113 ) ;
}
return V_2 ;
}
int
F_83 ( T_1 * V_1 , int V_2 , T_2 * V_3 ,
T_3 * V_4 , T_4 * V_5 , T_5 * V_6 )
{
T_22 * V_105 = ( T_22 * ) V_5 -> V_29 ;
char * V_113 = NULL ;
const char * V_53 ;
if( V_5 -> V_7 != - 1 ) {
V_53 = F_3 ( V_5 -> V_7 ) ;
} else {
V_53 = L_15 ;
}
if( V_5 -> V_24 ) {
return V_2 ;
}
V_2 = F_5 ( V_1 , V_2 , V_3 , V_4 , V_5 , V_6 ,
V_118 , NULL ) ;
V_2 = F_80 ( V_1 , V_2 , V_4 , V_53 , & V_113 ,
V_115 ) ;
if( V_105 ) {
V_105 -> V_106 = F_55 ( F_48 () , V_113 ) ;
}
return V_2 ;
}
int
F_84 ( T_1 * V_1 , int V_2 , T_2 * V_3 , T_3 * V_4 , T_4 * V_5 , T_5 * V_6 , T_14 V_39 )
{
T_22 * V_105 = ( T_22 * ) V_5 -> V_29 ;
T_13 T_8 = F_21 ( V_39 ) ;
V_2 = F_83 ( V_1 , V_2 , V_3 , V_4 , V_5 , V_6 ) ;
if( V_105 && V_105 -> V_106 ) {
char * V_40 = ( char * ) V_105 -> V_106 ;
T_6 * V_9 = ( T_6 * ) V_4 ;
if ( ( V_39 & V_41 ) && ( ! V_5 -> V_24 ) ) {
F_24 ( V_3 -> V_42 , V_43 , L_3 , V_40 ) ;
}
if ( T_8 > 0 && V_9 && V_40 && V_40 [ 0 ] ) {
F_25 ( V_9 , L_4 , V_40 ) ;
V_9 = F_26 ( V_9 ) ;
T_8 -- ;
if ( T_8 > 0 ) {
F_25 ( V_9 , L_4 , V_40 ) ;
V_9 = F_26 ( V_9 ) ;
T_8 -- ;
while ( T_8 > 0 ) {
F_25 ( V_9 , L_5 , V_40 ) ;
V_9 = F_26 ( V_9 ) ;
T_8 -- ;
}
}
}
}
return V_2 ;
}
static int
F_85 ( T_1 * V_1 , int V_2 , T_2 * V_3 ,
T_3 * V_4 , T_4 * V_5 , T_5 * V_6 )
{
V_2 = F_83 ( V_1 , V_2 , V_3 , V_4 , V_5 , V_6 ) ;
return V_2 ;
}
int
F_86 ( T_1 * V_1 , int V_2 ,
T_2 * V_3 , T_3 * V_47 ,
T_4 * V_5 , T_5 * V_6 )
{
T_6 * V_9 = NULL ;
T_3 * V_4 = NULL ;
int V_50 = V_2 ;
if( V_47 ) {
V_4 = F_2 ( V_47 , V_1 , V_2 , - 1 ,
V_119 , & V_9 , L_16 ) ;
}
V_2 = F_87 ( V_1 , V_2 , V_3 , V_4 , V_5 , V_6 ,
F_85 , V_28 ,
L_17 , V_115 ) ;
F_36 ( V_9 , V_2 - V_50 ) ;
return V_2 ;
}
int
F_88 ( T_1 * V_1 , int V_2 , T_2 * V_3 ,
T_3 * V_47 , T_4 * V_5 , T_5 * V_6 )
{
T_14 V_120 ;
static const int * V_30 [] = {
& V_121 ,
& V_122 ,
& V_123 ,
& V_124 ,
& V_125 ,
& V_126 ,
& V_127 ,
& V_128 ,
& V_129 ,
& V_130 ,
& V_131 ,
NULL
} ;
V_2 = F_58 ( V_1 , V_2 , V_3 , NULL , V_5 , V_6 , - 1 , & V_120 ) ;
F_89 ( V_47 , V_1 , V_2 - 4 , V_132 ,
V_133 , V_30 , V_120 , V_134 ) ;
return V_2 ;
}
static int
F_90 ( T_1 * V_1 , int V_2 ,
T_2 * V_3 , T_3 * V_4 ,
T_4 * V_5 , T_5 * V_6 )
{
V_2 = F_91 ( V_1 , V_2 , V_3 , V_4 , V_5 , V_6 ,
V_135 , NULL ) ;
return V_2 ;
}
static int
F_92 ( T_1 * V_1 , int V_2 ,
T_2 * V_3 , T_3 * V_47 ,
T_4 * V_5 , T_5 * V_6 )
{
T_6 * V_9 = NULL ;
T_3 * V_4 = NULL ;
int V_50 = V_2 ;
if( V_47 ) {
V_4 = F_2 ( V_47 , V_1 , V_2 , - 1 ,
V_136 , & V_9 , L_18 ) ;
}
V_2 = F_93 ( V_1 , V_2 , V_3 , V_4 , V_5 , V_6 ,
F_90 ) ;
F_36 ( V_9 , V_2 - V_50 ) ;
return V_2 ;
}
int
F_94 ( T_1 * V_1 , int V_2 ,
T_2 * V_3 , T_3 * V_47 ,
T_4 * V_5 , T_5 * V_6 )
{
T_6 * V_9 = NULL ;
T_3 * V_4 = NULL ;
int V_50 = V_2 ;
V_56 ;
if( V_47 ) {
V_4 = F_2 ( V_47 , V_1 , V_2 , - 1 ,
V_137 , & V_9 , L_18 ) ;
}
V_2 = F_12 ( V_1 , V_2 , V_3 , V_4 , V_5 , V_6 ,
V_138 , NULL ) ;
V_2 = F_87 ( V_1 , V_2 , V_3 , V_4 , V_5 , V_6 ,
F_92 , V_28 ,
L_19 , - 1 ) ;
F_36 ( V_9 , V_2 - V_50 ) ;
return V_2 ;
}
static int
F_95 ( T_1 * V_1 , int V_2 ,
T_2 * V_3 , T_3 * V_47 ,
T_4 * V_5 , T_5 * V_6 )
{
V_2 = F_86 ( V_1 , V_2 , V_3 , V_47 , V_5 , V_6 ) ;
return V_2 ;
}
static int
F_96 ( T_1 * V_1 , int V_2 ,
T_2 * V_3 , T_3 * V_4 ,
T_4 * V_5 , T_5 * V_6 )
{
V_2 = F_97 ( V_1 , V_2 , V_3 , V_4 , V_5 , V_6 ,
F_95 ) ;
return V_2 ;
}
int
F_98 ( T_1 * V_1 , int V_2 ,
T_2 * V_3 , T_3 * V_47 ,
T_4 * V_5 , T_5 * V_6 )
{
T_14 V_139 ;
T_6 * V_9 = NULL ;
T_3 * V_4 = NULL ;
int V_50 = V_2 ;
if( V_47 ) {
V_4 = F_2 ( V_47 , V_1 , V_2 , - 1 ,
V_140 , & V_9 , L_20 ) ;
}
V_23 ;
V_2 = F_58 ( V_1 , V_2 , V_3 , V_4 , V_5 , V_6 ,
V_118 , & V_139 ) ;
V_2 = F_87 ( V_1 , V_2 , V_3 , V_4 , V_5 , V_6 ,
F_96 , V_28 ,
L_21 , - 1 ) ;
F_36 ( V_9 , V_2 - V_50 ) ;
if ( V_5 -> V_29 -> V_30 & V_31 ) {
V_23 ;
}
return V_2 ;
}
int
F_99 ( T_1 * V_1 , int V_2 ,
T_2 * V_3 , T_3 * V_47 ,
T_4 * V_5 , T_5 * V_6 )
{
T_6 * V_9 = NULL ;
T_3 * V_4 = NULL ;
if( V_47 ) {
V_4 = F_2 ( V_47 , V_1 , V_2 , 0 ,
V_141 , & V_9 , L_22 ) ;
}
V_2 = F_86 ( V_1 , V_2 , V_3 , V_4 , V_5 , V_6 ) ;
V_2 = F_58 ( V_1 , V_2 , V_3 , V_4 , V_5 , V_6 ,
V_142 , NULL ) ;
return V_2 ;
}
int
F_100 ( T_1 * V_1 , int V_2 ,
T_2 * V_3 , T_3 * V_47 ,
T_4 * V_5 , T_5 * V_6 )
{
T_6 * V_9 = NULL ;
T_3 * V_4 = NULL ;
int V_50 = V_2 ;
if( V_47 ) {
V_4 = F_2 ( V_47 , V_1 , V_2 , 0 ,
V_143 , & V_9 , L_23 ) ;
}
V_2 = F_97 ( V_1 , V_2 , V_3 , V_4 , V_5 , V_6 ,
F_99 ) ;
F_36 ( V_9 , V_2 - V_50 ) ;
return V_2 ;
}
int
F_101 ( T_3 * V_47 , T_1 * V_1 , int V_2 , T_5 * V_6 )
{
T_3 * V_4 ;
T_5 V_144 ;
V_4 = F_2 ( V_47 , V_1 , V_2 , 16 , V_145 , NULL , L_24 ) ;
F_6 ( V_4 , V_146 , V_1 , V_2 , 1 , V_19 ) ;
V_2 ++ ;
V_144 = F_102 ( V_1 , V_2 ) ;
F_64 ( V_4 , V_147 , V_1 , V_2 , 1 , V_144 >> 4 ) ;
V_2 ++ ;
if ( V_6 ) {
* V_6 = V_144 ;
}
F_6 ( V_4 , V_148 , V_1 , V_2 , 2 , V_19 ) ;
V_2 += 2 ;
F_6 ( V_4 , V_149 , V_1 , V_2 , 4 , V_19 ) ;
V_2 += 4 ;
F_6 ( V_4 , V_150 , V_1 , V_2 , 8 , V_19 ) ;
V_2 += 8 ;
return V_2 ;
}
void F_103 ( int V_151 )
{
T_23 * V_152 ;
static T_24 V_153 [] = {
{ & V_26 ,
{ L_25 , L_26 , V_154 , V_155 ,
NULL , 0x0 , L_27 ,
V_156 } } ,
{ & V_25 ,
{ L_28 , L_29 , V_154 , V_155 ,
NULL , 0x0 , L_30 ,
V_156 } } ,
{ & V_46 ,
{ L_31 , L_32 , V_157 , V_158 ,
NULL , 0x0 , L_33 , V_156 } } ,
{ & V_99 ,
{ L_34 , L_35 ,
V_159 , V_158 , NULL , 0x0 ,
NULL , V_156 } } ,
{ & V_100 ,
{ L_36 , L_37 ,
V_159 , V_158 , NULL , 0x0 ,
NULL , V_156 } } ,
{ & V_132 ,
{ L_38 , L_39 , V_160 , V_161 ,
NULL , 0x0 , NULL , V_156 } } ,
{ & V_131 ,
{ L_40 , L_41 , V_162 , 32 ,
F_104 ( & V_163 ) , 0x0001 ,
L_42 , V_156 } } ,
{ & V_130 ,
{ L_43 , L_44 , V_162 , 32 ,
F_104 ( & V_164 ) , 0x0002 ,
L_45 , V_156 } } ,
{ & V_129 ,
{ L_46 , L_47 , V_162 , 32 ,
F_104 ( & V_165 ) , 0x0004 ,
L_48 , V_156 } } ,
{ & V_128 ,
{ L_49 , L_50 , V_162 , 32 ,
F_104 ( & V_166 ) , 0x0008 ,
L_51 , V_156 } } ,
{ & V_127 ,
{ L_52 , L_53 , V_162 , 32 ,
F_104 ( & V_167 ) , 0x0010 ,
L_54 , V_156 } } ,
{ & V_126 ,
{ L_55 , L_56 , V_162 , 32 ,
F_104 ( & V_168 ) , 0x0020 ,
NULL , V_156 } } ,
{ & V_125 ,
{ L_57 , L_58 , V_162 , 32 ,
F_104 ( & V_169 ) , 0x0040 ,
NULL , V_156 } } ,
{ & V_124 ,
{ L_59 , L_60 , V_162 , 32 ,
F_104 ( & V_170 ) , 0x0080 ,
NULL , V_156 } } ,
{ & V_123 ,
{ L_61 , L_62 , V_162 , 32 ,
F_104 ( & V_171 ) , 0x0100 ,
NULL , V_156 } } ,
{ & V_122 ,
{ L_63 , L_64 , V_162 , 32 ,
F_104 ( & V_172 ) , 0x0200 ,
L_65 , V_156 } } ,
{ & V_121 ,
{ L_66 , L_67 , V_162 , 32 ,
F_104 ( & V_173 ) , 0x0400 ,
L_68 , V_156 } } ,
{ & V_174 ,
{ L_69 , L_70 ,
V_175 , V_158 , NULL , 0x0 ,
L_71 , V_156 } } ,
{ & V_115 ,
{ L_72 , L_73 ,
V_175 , V_158 , NULL , 0x0 ,
L_74 , V_156 } } ,
{ & V_118 ,
{ L_75 , L_76 ,
V_160 , V_155 , NULL , 0x0 ,
L_77 , V_156 } } ,
{ & V_138 ,
{ L_78 , L_79 ,
V_154 , V_155 , NULL , 0 ,
L_80 , V_156 } } ,
{ & V_135 ,
{ L_81 , L_82 ,
V_176 , V_161 , NULL , 0x0 ,
L_83 , V_156 } } ,
{ & V_142 ,
{ L_84 , L_85 ,
V_160 , V_161 , NULL , 0x0 , NULL , V_156 } } ,
{ & V_54 ,
{ L_86 , L_87 ,
V_154 , V_155 , NULL , 0 , NULL , V_156 } } ,
{ & V_55 ,
{ L_88 , L_89 ,
V_154 , V_155 , NULL , 0 , NULL , V_156 } } ,
{ & V_13 ,
{ L_90 , L_91 ,
V_160 , V_155 , NULL , 0 , NULL , V_156 } } ,
{ & V_14 ,
{ L_92 , L_93 ,
V_177 , V_158 , NULL , 0 , NULL , V_156 } } ,
{ & V_150 , {
L_94 , L_95 , V_178 , V_155 ,
NULL , 0 , L_96 , V_156 } } ,
{ & V_149 , {
L_97 , L_98 , V_160 , V_161 ,
NULL , 0 , L_99 , V_156 } } ,
{ & V_146 , {
L_100 , L_101 , V_176 , V_155 ,
NULL , 0 , L_102 , V_156 } } ,
{ & V_148 , {
L_103 , L_104 , V_154 , V_155 ,
NULL , 0 , L_105 , V_156 } } ,
} ;
static T_13 * V_179 [] = {
& V_12 ,
& V_33 ,
& V_35 ,
& V_94 ,
& V_119 ,
& V_133 ,
& V_137 ,
& V_136 ,
& V_140 ,
& V_143 ,
& V_141 ,
& V_44 ,
& V_52 ,
& V_145 ,
} ;
static T_25 V_180 [] = {
{ & V_117 , { L_106 , V_181 , V_182 , L_107 , V_183 } } ,
} ;
F_105 ( V_179 , F_106 ( V_179 ) ) ;
F_107 ( V_151 , V_153 , F_106 ( V_153 ) ) ;
V_152 = F_108 ( V_151 ) ;
F_109 ( V_152 , V_180 , F_106 ( V_180 ) ) ;
V_70 = F_110 ( F_111 () , F_48 () , F_41 , F_43 ) ;
}
