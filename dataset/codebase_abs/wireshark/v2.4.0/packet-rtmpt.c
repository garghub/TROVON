static void F_1 ( const char * V_1 , ... )
{
T_1 args ;
va_start ( args , V_1 ) ;
vprintf ( V_1 , args ) ;
}
static void F_1 ( const char * V_1 , ... ) { ( void ) V_1 ; }
static T_2 F_2 ( T_2 V_2 )
{
switch ( V_2 & 0x3f ) {
case 0 : return 2 ;
case 1 : return 3 ;
default: return 1 ;
}
}
static T_2 F_3 ( T_2 V_2 )
{
switch ( ( V_2 >> 6 ) & 3 ) {
case 0 : return 11 ;
case 1 : return 7 ;
case 2 : return 3 ;
default: return 0 ;
}
}
static T_2
F_4 ( T_3 * V_3 , T_2 V_4 )
{
T_4 V_5 ;
T_2 V_6 = F_5 ( V_3 , V_4 ) ;
T_5 V_7 = 0 ;
T_2 V_8 = 0 ;
T_2 V_9 = 0 ;
while ( V_9 == 0 || V_7 > 0 ) {
if ( V_7 > 0 ) {
if ( V_6 - V_9 < 2 )
return V_6 ;
V_8 = F_6 ( V_3 , V_4 + V_9 ) + 2 ;
if ( V_6 - V_9 < V_8 + 1 )
return V_6 ;
V_9 += V_8 ;
}
if ( V_6 - V_9 < 1 )
return V_6 ;
V_5 = F_7 ( V_3 , V_4 + V_9 ) ;
if ( V_7 > 0 && V_8 == 2 && V_5 == V_10 ) {
V_9 ++ ;
V_7 -- ;
continue;
}
switch ( V_5 ) {
case V_11 :
V_8 = 9 ;
break;
case V_12 :
V_8 = 2 ;
break;
case V_13 :
if ( V_6 - V_9 < 3 )
return V_6 ;
V_8 = F_6 ( V_3 , V_4 + V_9 + 1 ) + 3 ;
break;
case V_14 :
case V_15 :
case V_16 :
V_8 = 1 ;
break;
case V_17 :
V_8 = 11 ;
break;
case V_18 :
case V_19 :
if ( V_6 - V_9 < 5 )
return V_6 ;
V_8 = F_8 ( V_3 , V_4 + V_9 + 1 ) + 5 ;
break;
case V_20 :
V_8 = 9 ;
break;
case V_21 :
V_8 = 1 ;
V_7 ++ ;
break;
case V_22 :
V_8 = 5 ;
V_7 ++ ;
break;
default:
return V_6 ;
}
if ( V_6 - V_9 < V_8 )
return V_6 ;
V_9 += V_8 ;
}
return V_9 ;
}
static T_6 *
F_9 ( T_3 * V_3 , T_2 V_4 , T_2 V_23 , const T_6 * V_24 )
{
T_5 V_6 = F_5 ( V_3 , V_4 ) ;
T_5 V_8 ;
T_5 V_25 ;
while ( V_6 > 0 && V_23 > 0 ) {
V_8 = F_4 ( V_3 , V_4 ) ;
V_4 += V_8 ;
V_6 -= V_8 ;
V_23 -- ;
}
if ( V_6 > 0 && V_23 == 0 ) {
T_4 V_5 = F_7 ( V_3 , V_4 ) ;
if ( ! V_24 && V_5 == V_13 && V_6 >= 3 ) {
V_25 = F_6 ( V_3 , V_4 + 1 ) ;
if ( V_6 >= V_25 + 3 ) {
return F_10 ( F_11 () , V_3 , V_4 + 3 , V_25 , V_26 ) ;
}
}
if ( V_24 && V_5 == V_21 ) {
V_4 ++ ;
V_6 -- ;
while ( V_6 > 2 ) {
T_5 V_27 = F_6 ( V_3 , V_4 ) ;
if ( V_6 < 2 + V_27 + 3 )
break;
if ( F_12 ( V_3 , V_4 + 2 , V_24 , strlen ( V_24 ) ) == 0 ) {
if ( F_7 ( V_3 , V_4 + 2 + V_27 ) != V_13 )
break;
V_25 = F_6 ( V_3 , V_4 + 2 + V_27 + 1 ) ;
if ( V_6 < 2 + V_27 + 3 + V_25 )
break;
return F_10 ( F_11 () , V_3 , V_4 + 2 + V_27 + 3 , V_25 , V_26 ) ;
}
V_8 = F_4 ( V_3 , V_4 + 2 + V_27 ) ;
V_4 += 2 + V_27 + V_8 ;
V_6 -= 2 + V_27 + V_8 ;
}
}
}
return NULL ;
}
static T_5
F_13 ( T_3 * V_3 , T_2 V_4 )
{
T_5 V_6 = F_5 ( V_3 , V_4 ) ;
if ( V_6 > 0 ) {
T_5 V_8 = F_4 ( V_3 , V_4 ) ;
if ( V_6 < V_8 )
return 0 ;
V_4 += V_8 ;
V_6 -= V_8 ;
}
if ( V_6 >= 9 ) {
T_4 V_5 = F_7 ( V_3 , V_4 ) ;
if ( V_5 == V_11 ) {
return ( T_5 ) F_14 ( V_3 , V_4 + 1 ) ;
}
}
return 0 ;
}
static T_6 *
F_15 ( T_3 * V_3 , T_5 V_4 , T_5 V_6 , T_7 * V_28 , int V_29 , T_8 * V_30 , T_2 * V_31 )
{
if ( V_30 -> V_32 == V_33 || V_30 -> V_32 == V_34 ||
V_30 -> V_32 == V_35 || V_30 -> V_32 == V_36 ) {
if ( V_30 -> V_37 >= 4 && V_6 >= 4 ) {
* V_31 = TRUE ;
return F_16 ( F_11 () , L_1 ,
F_17 ( V_30 -> V_32 , V_38 , L_2 ) ,
F_8 ( V_3 , V_4 ) ) ;
}
} else if ( V_30 -> V_32 == V_39 ) {
if ( V_30 -> V_37 >= 5 && V_6 >= 5 ) {
* V_31 = TRUE ;
return F_16 ( F_11 () , L_3 ,
F_17 ( V_30 -> V_32 , V_38 , L_2 ) ,
F_8 ( V_3 , V_4 ) ,
F_17 ( F_7 ( V_3 , V_4 + 4 ) , V_40 , L_4 ) ) ;
}
} else if ( V_30 -> V_32 == V_41 ) {
T_9 V_42 = - 1 ;
const T_6 * V_43 ;
const T_6 * V_44 = L_5 ;
if ( V_30 -> V_37 < 2 || V_6 < 2 )
return NULL ;
V_42 = F_6 ( V_3 , V_4 ) ;
V_43 = F_18 ( V_42 , V_45 ) ;
if ( V_43 == NULL ) {
* V_31 = TRUE ;
V_43 = F_16 ( F_11 () , L_6 , V_42 ) ;
}
if ( V_42 == V_46 || V_42 == V_47 ||
V_42 == V_48 || V_42 == V_49 ) {
if ( V_30 -> V_37 >= 6 && V_6 >= 6 ) {
V_44 = F_16 ( F_11 () , L_7 , F_8 ( V_3 , V_4 + 2 ) ) ;
}
} else if ( V_42 == V_50 ) {
if ( V_30 -> V_37 >= 10 && V_6 >= 10 ) {
V_44 = F_16 ( F_11 () , L_8 ,
F_8 ( V_3 , V_4 + 2 ) ,
F_8 ( V_3 , V_4 + 6 ) ) ;
}
}
return F_16 ( F_11 () , L_9 , V_43 , V_44 ) ;
} else if ( V_30 -> V_32 == V_51 || V_30 -> V_32 == V_52 ||
V_30 -> V_32 == V_53 || V_30 -> V_32 == V_54 ) {
T_5 V_55 = 0 ;
T_5 V_56 = 0 ;
T_6 * V_43 = NULL ;
T_6 * V_44 = NULL ;
if ( V_30 -> V_32 == V_52 || V_30 -> V_32 == V_54 ) {
V_56 = 1 ;
}
if ( V_30 -> V_37 >= 3 + V_56 && V_6 >= 3 + V_56 ) {
V_55 = F_6 ( V_3 , V_4 + 1 + V_56 ) ;
}
if ( V_55 > 0 ) {
V_43 = F_10 ( F_11 () , V_3 , V_4 + 3 + V_56 , V_55 , V_26 ) ;
F_19 ( L_10 , V_43 ) ;
if ( strcmp ( V_43 , L_11 ) == 0 ) {
V_44 = F_9 ( V_3 , V_4 + V_56 , 2 , L_12 ) ;
} else if ( strcmp ( V_43 , L_13 ) == 0 ) {
V_44 = F_9 ( V_3 , V_4 + V_56 , 3 , NULL ) ;
} else if ( strcmp ( V_43 , L_14 ) == 0 ) {
V_44 = F_9 ( V_3 , V_4 + V_56 , 3 , L_15 ) ;
} else if ( strcmp ( V_43 , L_16 ) == 0 ) {
V_44 = F_9 ( V_3 , V_4 + V_56 , 3 , NULL ) ;
} else if ( strcmp ( V_43 , L_17 ) == 0 ) {
V_44 = F_9 ( V_3 , V_4 + V_56 , 3 , NULL ) ;
} else if ( strcmp ( V_43 , L_18 ) == 0 ) {
V_44 = F_9 ( V_3 , V_4 + V_56 , 3 , NULL ) ;
} else if ( strcmp ( V_43 , L_19 ) == 0 ) {
if ( V_30 -> V_32 == V_51 || V_30 -> V_32 == V_52 ) {
V_44 = F_9 ( V_3 , V_4 + V_56 , 3 , L_20 ) ;
} else {
V_44 = F_9 ( V_3 , V_4 + V_56 , 1 , L_20 ) ;
}
} else if ( strcmp ( V_43 , L_21 ) == 0 ) {
V_44 = F_9 ( V_3 , V_4 + V_56 , 1 , L_20 ) ;
} else if ( strcmp ( V_43 , L_22 ) == 0 ) {
V_44 = F_9 ( V_3 , V_4 + V_56 , 3 , L_20 ) ;
V_30 -> V_57 = TRUE ;
} else if ( strcmp ( V_43 , L_23 ) == 0 ) {
V_44 = F_9 ( V_3 , V_4 + V_56 , 3 , L_20 ) ;
V_30 -> V_57 = TRUE ;
}
if ( V_30 -> V_58 != 0 && V_30 -> V_59 == 0 ) {
V_30 -> V_59 = F_20 ( F_21 ( V_28 -> V_60 [ V_29 ^ 1 ] , V_30 -> V_58 ) ) ;
if ( V_30 -> V_59 ) {
F_19 ( L_24 , V_30 -> V_59 ) ;
}
}
}
if ( V_43 ) {
if ( V_44 ) {
return F_16 ( F_11 () , L_25 , V_43 , V_44 ) ;
} else {
return F_16 ( F_11 () , L_26 , V_43 ) ;
}
}
}
return NULL ;
}
static void
F_22 ( T_3 * V_3 , T_2 V_4 , T_10 * V_61 , T_11 V_62 )
{
switch ( V_62 ) {
case V_33 :
F_23 ( V_61 , V_63 , V_3 , V_4 , 4 , V_64 ) ;
break;
case V_34 :
F_23 ( V_61 , V_65 , V_3 , V_4 , 4 , V_64 ) ;
break;
case V_35 :
F_23 ( V_61 , V_66 , V_3 , V_4 , 4 , V_64 ) ;
break;
case V_41 :
F_23 ( V_61 , V_67 , V_3 , V_4 , 2 , V_64 ) ;
break;
case V_36 :
F_23 ( V_61 , V_68 , V_3 , V_4 , 4 , V_64 ) ;
break;
case V_39 :
F_23 ( V_61 , V_68 , V_3 , V_4 , 4 , V_64 ) ;
F_23 ( V_61 , V_69 , V_3 , V_4 + 4 , 1 , V_64 ) ;
break;
}
}
static T_2
F_24 ( T_3 * V_3 , T_2 V_4 , T_10 * V_70 , T_11 * V_71 , T_12 * V_72 )
{
T_13 * V_73 ;
T_10 * V_74 ;
T_10 * V_75 ;
T_11 V_25 ;
T_6 * V_76 ;
T_11 V_77 = 0 ;
for (; ; ) {
V_25 = F_6 ( V_3 , V_4 ) ;
if ( V_25 == 0 &&
F_7 ( V_3 , V_4 + 2 ) == V_10 )
break;
V_77 ++ ;
V_76 = F_10 ( F_11 () , V_3 , V_4 + 2 , V_25 , V_26 ) ;
V_74 = F_25 ( V_70 , V_3 , V_4 , - 1 ,
V_78 , & V_73 , L_27 ,
V_76 ) ;
V_75 = F_25 ( V_74 , V_3 ,
V_4 , 2 + V_25 ,
V_79 , NULL , L_28 , V_76 ) ;
F_26 ( V_75 , V_80 , V_3 , V_4 , 2 , V_25 ) ;
V_4 += 2 ;
F_23 ( V_75 , V_81 , V_3 , V_4 , V_25 , V_82 | V_83 ) ;
V_4 += V_25 ;
V_4 = F_27 ( V_3 , V_4 , V_74 , V_72 , V_73 ) ;
F_28 ( V_73 , V_3 , V_4 ) ;
}
F_23 ( V_70 , V_84 , V_3 , V_4 , 3 , V_83 ) ;
V_4 += 3 ;
* V_71 = V_77 ;
return V_4 ;
}
static T_2
F_27 ( T_3 * V_3 , T_2 V_4 , T_10 * V_70 , T_12 * V_72 , T_13 * V_85 )
{
T_4 V_5 ;
T_13 * V_86 ;
T_10 * V_87 ;
T_2 V_88 = V_4 ;
T_5 V_89 ;
double V_90 ;
T_12 V_91 ;
T_11 V_25 ;
T_6 * V_76 ;
T_11 V_92 ;
T_11 V_93 ;
T_14 V_94 ;
T_15 V_95 ;
T_11 V_77 ;
V_5 = F_7 ( V_3 , V_4 ) ;
if ( V_85 != NULL )
F_29 ( V_85 , L_29 ,
F_30 ( V_5 , V_96 , L_30 ) ) ;
switch ( V_5 ) {
case V_21 :
V_86 = F_23 ( V_70 , V_97 , V_3 , V_4 , - 1 , V_83 ) ;
V_87 = F_31 ( V_86 , V_98 ) ;
break;
case V_22 :
V_86 = F_23 ( V_70 , V_99 , V_3 , V_4 , - 1 , V_83 ) ;
V_87 = F_31 ( V_86 , V_98 ) ;
break;
case V_100 :
V_86 = F_23 ( V_70 , V_101 , V_3 , V_4 , - 1 , V_83 ) ;
V_87 = F_31 ( V_86 , V_98 ) ;
break;
default:
V_87 = F_32 ( V_70 , V_3 , V_4 , - 1 , V_98 , & V_86 ,
F_30 ( V_5 , V_96 , L_30 ) ) ;
break;
}
F_26 ( V_87 , V_102 , V_3 , V_88 , 1 , V_5 ) ;
V_88 ++ ;
switch ( V_5 ) {
case V_11 :
V_90 = F_14 ( V_3 , V_88 ) ;
F_33 ( V_87 , V_103 , V_3 , V_88 , 8 , V_90 ) ;
V_88 += 8 ;
F_29 ( V_86 , L_31 F_34 ( V_104 ) L_32 , V_90 ) ;
if ( V_85 != NULL )
F_29 ( V_85 , L_31 F_34 ( V_104 ) L_32 , V_90 ) ;
break;
case V_12 :
V_91 = F_7 ( V_3 , V_88 ) ;
F_35 ( V_87 , V_105 , V_3 , V_88 , 1 , V_91 ) ;
V_88 += 1 ;
F_29 ( V_86 , V_91 ? L_33 : L_34 ) ;
if ( V_85 != NULL )
F_29 ( V_85 , V_91 ? L_33 : L_34 ) ;
break;
case V_13 :
V_25 = F_6 ( V_3 , V_88 ) ;
F_26 ( V_87 , V_80 , V_3 , V_88 , 2 , V_25 ) ;
V_88 += 2 ;
V_76 = F_10 ( F_11 () , V_3 , V_88 , V_25 , V_82 | V_83 ) ;
if ( V_25 != 0 )
F_36 ( V_87 , V_81 , V_3 , V_88 , V_25 , V_76 ) ;
V_88 += V_25 ;
F_29 ( V_86 , L_35 , V_76 ) ;
if ( V_85 != NULL )
F_29 ( V_85 , L_35 , V_76 ) ;
break;
case V_21 :
V_88 = F_24 ( V_3 , V_88 , V_87 , & V_77 , V_72 ) ;
F_29 ( V_86 , L_36 , V_77 ) ;
break;
case V_14 :
case V_15 :
break;
case V_106 :
V_89 = F_6 ( V_3 , V_88 ) ;
F_26 ( V_87 , V_107 , V_3 , V_88 , 2 , V_89 ) ;
V_88 += 2 ;
F_29 ( V_86 , L_7 , V_89 ) ;
break;
case V_22 :
V_92 = F_8 ( V_3 , V_88 ) ;
F_26 ( V_87 , V_108 , V_3 , V_88 , 4 , V_92 ) ;
V_88 += 4 ;
V_88 = F_24 ( V_3 , V_88 , V_87 , & V_77 , V_72 ) ;
F_29 ( V_86 , L_36 , V_77 ) ;
break;
case V_10 :
F_23 ( V_70 , V_84 , V_3 , V_88 , 3 , V_83 ) ;
V_88 += 3 ;
break;
case V_100 :
V_92 = F_8 ( V_3 , V_88 ) ;
F_26 ( V_87 , V_108 , V_3 , V_88 , 4 , V_92 ) ;
V_88 += 4 ;
for ( V_93 = 0 ; V_93 < V_92 ; V_93 ++ )
V_88 = F_27 ( V_3 , V_88 , V_87 , V_72 , NULL ) ;
F_29 ( V_86 , L_36 , V_92 ) ;
break;
case V_17 :
V_90 = F_14 ( V_3 , V_88 ) ;
V_94 . V_109 = ( V_110 ) ( V_90 / 1000 ) ;
V_94 . V_111 = ( int ) ( ( V_90 - 1000 * ( double ) V_94 . V_109 ) * 1000000 ) ;
F_37 ( V_87 , V_112 , V_3 , V_88 , 8 , & V_94 ) ;
V_88 += 8 ;
F_29 ( V_86 , L_29 , F_38 ( F_11 () , & V_94 , V_113 , TRUE ) ) ;
if ( V_85 != NULL )
F_29 ( V_85 , L_29 , F_38 ( F_11 () , & V_94 , V_113 , TRUE ) ) ;
V_88 += 2 ;
break;
case V_18 :
case V_19 :
V_25 = F_8 ( V_3 , V_88 ) ;
F_26 ( V_87 , V_80 , V_3 , V_88 , 2 , V_25 ) ;
V_88 += 4 ;
V_76 = F_10 ( F_11 () , V_3 , V_88 , V_25 , V_82 | V_83 ) ;
if ( V_25 != 0 )
F_36 ( V_87 , ( V_5 == V_19 ) ? V_114 : V_115 , V_3 , V_88 , V_25 , V_76 ) ;
V_88 += V_25 ;
F_29 ( V_86 , L_35 , V_76 ) ;
if ( V_85 != NULL )
F_29 ( V_85 , L_35 , V_76 ) ;
break;
case V_16 :
break;
case V_116 :
V_25 = F_6 ( V_3 , V_88 ) ;
F_26 ( V_87 , V_80 , V_3 , V_88 , 2 , V_25 ) ;
V_88 += 2 ;
V_76 = F_10 ( F_11 () , V_3 , V_88 , V_25 , V_82 | V_83 ) ;
F_36 ( V_87 , V_81 , V_3 , V_88 , V_25 , V_76 ) ;
V_88 += V_25 ;
V_88 = F_24 ( V_3 , V_88 , V_87 , & V_77 , V_72 ) ;
break;
case V_117 :
* V_72 = TRUE ;
break;
case V_20 :
V_95 = F_39 ( V_3 , V_88 ) ;
F_40 ( V_87 , V_118 , V_3 , V_88 , 8 , V_95 ) ;
V_88 += 8 ;
F_29 ( V_86 , L_37 V_119 L_38 , V_95 ) ;
if ( V_85 != NULL )
F_29 ( V_85 , L_37 V_119 L_38 , V_95 ) ;
break;
default:
V_88 = F_41 ( V_3 ) ;
break;
}
F_28 ( V_86 , V_3 , V_88 ) ;
return V_88 ;
}
static T_5
F_42 ( T_3 * V_3 , int V_4 , T_11 * V_120 )
{
T_11 V_37 = 0 ;
T_4 V_121 ;
T_5 V_122 ;
V_121 = F_7 ( V_3 , V_4 ) ;
V_122 = ( V_121 & 0x7F ) ;
V_4 ++ ;
V_37 ++ ;
if ( ! ( V_121 & 0x80 ) ) {
* V_120 = V_37 ;
return V_122 ;
}
V_121 = F_7 ( V_3 , V_4 ) ;
V_122 = ( V_122 << 7 ) | ( V_121 & 0x7F ) ;
V_4 ++ ;
V_37 ++ ;
if ( ! ( V_121 & 0x80 ) ) {
* V_120 = V_37 ;
return V_122 ;
}
V_121 = F_7 ( V_3 , V_4 ) ;
V_122 = ( V_122 << 7 ) | ( V_121 & 0x7F ) ;
V_4 ++ ;
V_37 ++ ;
if ( ! ( V_121 & 0x80 ) ) {
* V_120 = V_37 ;
return V_122 ;
}
V_121 = F_7 ( V_3 , V_4 ) ;
V_122 = ( V_122 << 8 ) | V_121 ;
V_37 ++ ;
* V_120 = V_37 ;
return V_122 ;
}
static T_2
F_43 ( T_3 * V_3 , T_2 V_4 , T_10 * V_70 , T_13 * V_85 )
{
T_4 V_5 ;
T_13 * V_86 ;
T_10 * V_87 ;
T_2 V_88 = V_4 ;
T_11 V_123 ;
T_5 V_89 ;
double V_90 ;
T_11 V_25 ;
T_6 * V_76 ;
T_11 V_92 ;
T_13 * V_124 ;
T_10 * V_125 ;
T_11 V_93 ;
T_12 V_126 ;
T_11 V_127 ;
T_13 * V_128 ;
T_10 * V_129 ;
T_10 * V_75 ;
T_10 * V_130 ;
T_4 * V_131 ;
V_5 = F_7 ( V_3 , V_4 ) ;
if ( V_85 != NULL )
F_29 ( V_85 , L_29 ,
F_30 ( V_5 , V_132 , L_30 ) ) ;
switch ( V_5 ) {
case V_133 :
V_86 = F_23 ( V_70 , V_134 , V_3 , V_4 , - 1 , V_83 ) ;
V_87 = F_31 ( V_86 , V_98 ) ;
break;
case V_135 :
V_86 = F_23 ( V_70 , V_97 , V_3 , V_4 , - 1 , V_83 ) ;
V_87 = F_31 ( V_86 , V_98 ) ;
break;
default:
V_87 = F_32 ( V_70 , V_3 , V_4 , - 1 , V_98 , & V_86 ,
F_30 ( V_5 , V_132 , L_30 ) ) ;
break;
}
F_26 ( V_87 , V_136 , V_3 , V_88 , 1 , V_5 ) ;
V_88 ++ ;
switch ( V_5 ) {
case V_137 :
case V_138 :
break;
case V_139 :
F_35 ( V_87 , V_105 , V_3 , 0 , 0 , FALSE ) ;
F_29 ( V_86 , L_34 ) ;
break;
case V_140 :
F_35 ( V_87 , V_105 , V_3 , 0 , 0 , TRUE ) ;
F_29 ( V_86 , L_33 ) ;
break;
case V_141 :
V_89 = F_42 ( V_3 , V_88 , & V_123 ) ;
F_26 ( V_87 , V_142 , V_3 , V_88 , V_123 , V_89 ) ;
F_29 ( V_86 , L_39 , V_89 ) ;
if ( V_85 != NULL )
F_29 ( V_85 , L_39 , V_89 ) ;
V_88 += V_123 ;
break;
case V_143 :
V_90 = F_14 ( V_3 , V_88 ) ;
F_33 ( V_87 , V_103 , V_3 , V_88 , 8 , V_90 ) ;
V_88 += 8 ;
F_29 ( V_86 , L_31 F_34 ( V_104 ) L_32 , V_90 ) ;
if ( V_85 != NULL )
F_29 ( V_85 , L_31 F_34 ( V_104 ) L_32 , V_90 ) ;
break;
case V_144 :
V_89 = F_42 ( V_3 , V_88 , & V_123 ) ;
if ( V_89 & 0x00000001 ) {
V_25 = V_89 >> 1 ;
F_26 ( V_87 , V_80 , V_3 , V_88 , V_123 , V_25 ) ;
V_88 += V_123 ;
V_76 = F_10 ( F_11 () , V_3 , V_88 , V_25 , V_82 | V_83 ) ;
if ( V_25 != 0 )
F_36 ( V_87 , V_81 , V_3 , V_88 , V_25 , V_76 ) ;
V_88 += V_25 ;
F_29 ( V_86 , L_35 , V_76 ) ;
if ( V_85 != NULL )
F_29 ( V_85 , L_35 , V_76 ) ;
} else {
F_26 ( V_87 , V_145 , V_3 , V_88 , V_123 , V_89 >> 1 ) ;
V_88 += V_123 ;
F_29 ( V_86 , L_40 , V_89 >> 1 ) ;
if ( V_85 != NULL )
F_29 ( V_85 , L_40 , V_89 >> 1 ) ;
}
break;
case V_146 :
V_89 = F_42 ( V_3 , V_88 , & V_123 ) ;
if ( V_89 & 0x00000001 ) {
T_14 V_94 ;
V_88 += V_123 ;
V_90 = F_14 ( V_3 , V_88 ) ;
V_94 . V_109 = ( V_110 ) ( V_90 / 1000 ) ;
V_94 . V_111 = ( int ) ( ( V_90 - 1000 * ( double ) V_94 . V_109 ) * 1000000 ) ;
F_37 ( V_87 , V_112 , V_3 , V_88 , 8 , & V_94 ) ;
V_88 += 8 ;
F_29 ( V_86 , L_41 , F_38 ( F_11 () , & V_94 , V_113 , TRUE ) ) ;
if ( V_85 != NULL )
F_29 ( V_85 , L_41 , F_38 ( F_11 () , & V_94 , V_113 , TRUE ) ) ;
} else {
F_26 ( V_87 , V_107 , V_3 , V_88 , V_123 , V_89 >> 1 ) ;
V_88 += V_123 ;
F_29 ( V_86 , L_42 , V_89 >> 1 ) ;
if ( V_85 != NULL )
F_29 ( V_85 , L_42 , V_89 >> 1 ) ;
}
break;
case V_133 :
V_89 = F_42 ( V_3 , V_88 , & V_123 ) ;
if ( V_89 & 0x00000001 ) {
V_92 = V_89 >> 1 ;
F_26 ( V_87 , V_147 , V_3 , V_88 , V_123 , V_92 ) ;
V_88 += V_123 ;
for (; ; ) {
V_89 = F_42 ( V_3 , V_88 , & V_123 ) ;
if ( V_89 & 0x00000001 ) {
V_25 = V_89 >> 1 ;
if ( V_25 == 0 ) {
F_23 ( V_87 , V_148 , V_3 , V_88 , V_123 , V_83 ) ;
V_88 += V_123 ;
break;
}
V_76 = F_10 ( F_11 () , V_3 , V_88 + V_123 , V_25 , V_82 | V_83 ) ;
V_125 = F_32 ( V_87 , V_3 , V_88 , V_25 ,
V_149 , & V_124 , V_76 ) ;
F_26 ( V_125 , V_80 , V_3 , V_88 , V_123 , V_25 ) ;
V_88 += V_123 ;
F_36 ( V_125 , V_81 , V_3 , V_88 , V_25 , V_76 ) ;
} else {
V_125 = F_25 ( V_87 , V_3 , V_88 , V_123 ,
V_149 , & V_124 , L_43 , V_89 >> 1 ) ;
F_26 ( V_125 , V_145 , V_3 , V_88 , V_123 , V_89 >> 1 ) ;
}
V_5 = F_7 ( V_3 , V_4 ) ;
F_29 ( V_124 , L_41 ,
F_30 ( V_5 , V_132 , L_30 ) ) ;
V_88 = F_43 ( V_3 , V_88 , V_125 , V_124 ) ;
}
for ( V_93 = 0 ; V_93 < V_92 ; V_93 ++ )
V_88 = F_43 ( V_3 , V_88 , V_87 , NULL ) ;
F_28 ( V_86 , V_3 , V_88 ) ;
} else {
F_26 ( V_87 , V_107 , V_3 , V_88 , V_123 , V_89 >> 1 ) ;
F_29 ( V_86 , L_40 , V_89 >> 1 ) ;
if ( V_85 != NULL )
F_29 ( V_85 , L_40 , V_89 >> 1 ) ;
}
break;
case V_135 :
V_89 = F_42 ( V_3 , V_88 , & V_123 ) ;
if ( V_89 & 0x00000001 ) {
if ( V_89 & 0x00000002 ) {
if ( V_89 & 0x00000004 ) {
V_88 += V_123 ;
} else {
V_126 = ( V_89 & 0x00000008 ) ? TRUE : FALSE ;
V_127 = V_89 >> 4 ;
F_26 ( V_87 , V_150 , V_3 , V_88 , V_123 , V_127 ) ;
V_88 += V_123 ;
V_89 = F_42 ( V_3 , V_88 , & V_123 ) ;
if ( V_89 & 0x00000001 ) {
V_25 = V_89 >> 1 ;
V_76 = F_10 ( F_11 () , V_3 , V_88 + V_123 , V_25 , V_82 | V_83 ) ;
V_129 = F_25 ( V_87 , V_3 , V_88 , - 1 ,
V_151 , & V_128 , L_44 , V_76 , V_127 ) ;
V_75 = F_25 ( V_129 , V_3 ,
V_88 ,
V_123 + V_25 ,
V_79 , NULL , L_45 ,
V_76 ) ;
F_26 ( V_75 , V_152 , V_3 , V_88 , V_123 , V_25 ) ;
V_88 += V_123 ;
F_36 ( V_75 , V_153 , V_3 , V_88 , V_25 , V_76 ) ;
V_88 += V_25 ;
} else {
V_129 = F_25 ( V_87 , V_3 , V_88 , V_123 ,
V_151 , & V_128 , L_46 , V_89 >> 1 ) ;
F_26 ( V_129 , V_145 , V_3 , V_88 , V_123 , V_89 >> 1 ) ;
V_88 += V_123 ;
}
for ( V_93 = 0 ; V_93 < V_127 ; V_93 ++ ) {
V_89 = F_42 ( V_3 , V_88 , & V_123 ) ;
if ( V_89 & 0x00000001 ) {
V_25 = V_89 >> 1 ;
V_76 = F_10 ( F_11 () , V_3 , V_88 + V_123 , V_25 , V_82 | V_83 ) ;
V_130 = F_25 ( V_129 , V_3 , V_88 , V_123 + V_25 ,
V_154 , NULL , L_47 , V_76 ) ;
F_26 ( V_130 , V_155 , V_3 , V_88 , V_123 , V_25 ) ;
V_88 += V_123 ;
F_36 ( V_130 , V_156 , V_3 , V_88 , V_25 , V_76 ) ;
V_88 += V_25 ;
} else {
F_26 ( V_129 , V_145 , V_3 , V_88 , V_123 , V_89 >> 1 ) ;
V_88 += V_123 ;
}
}
for ( V_93 = 0 ; V_93 < V_127 ; V_93 ++ )
V_88 = F_43 ( V_3 , V_88 , V_129 , NULL ) ;
if ( V_126 ) {
for (; ; ) {
V_89 = F_42 ( V_3 , V_88 , & V_123 ) ;
if ( V_89 & 0x00000001 ) {
V_25 = V_89 >> 1 ;
if ( V_25 == 0 ) {
F_23 ( V_129 , V_157 , V_3 , V_88 , V_123 , V_83 ) ;
V_88 += V_123 ;
break;
}
V_76 = F_10 ( F_11 () , V_3 , V_88 + V_123 , V_25 , V_82 | V_83 ) ;
V_125 = F_25 ( V_129 , V_3 , V_88 , - 1 ,
V_149 , & V_124 , L_48 , V_76 ) ;
V_75 = F_25 ( V_125 , V_3 ,
V_88 ,
V_123 + V_25 ,
V_79 , NULL , L_49 ,
V_76 ) ;
F_26 ( V_75 , V_155 , V_3 , V_88 , V_123 , V_25 ) ;
V_88 += V_123 ;
F_36 ( V_75 , V_156 , V_3 , V_88 , V_25 , V_76 ) ;
V_88 += V_25 ;
} else {
V_125 = F_25 ( V_129 , V_3 , V_88 , V_123 ,
V_149 , & V_124 , L_43 , V_89 >> 1 ) ;
F_26 ( V_125 , V_145 , V_3 , V_88 , V_123 , V_89 >> 1 ) ;
V_88 += V_123 ;
}
V_88 = F_43 ( V_3 , V_88 , V_125 , V_124 ) ;
F_28 ( V_124 , V_3 , V_88 ) ;
}
}
F_28 ( V_128 , V_3 , V_88 ) ;
}
} else {
F_26 ( V_87 , V_158 , V_3 , V_88 , V_123 , V_89 >> 2 ) ;
V_88 += V_123 ;
}
} else {
F_26 ( V_87 , V_107 , V_3 , V_88 , V_123 , V_89 >> 1 ) ;
F_29 ( V_86 , L_40 , V_89 >> 1 ) ;
if ( V_85 != NULL )
F_29 ( V_85 , L_40 , V_89 >> 1 ) ;
}
break;
case V_159 :
V_89 = F_42 ( V_3 , V_88 , & V_123 ) ;
if ( V_89 & 0x00000001 ) {
V_25 = V_89 >> 1 ;
F_26 ( V_87 , V_160 , V_3 , V_88 , V_123 , V_25 ) ;
V_88 += V_123 ;
F_23 ( V_87 , V_161 , V_3 , V_88 , V_25 , V_82 | V_83 ) ;
} else {
F_26 ( V_87 , V_107 , V_3 , V_88 , V_123 , V_89 >> 1 ) ;
F_29 ( V_86 , L_40 , V_89 >> 1 ) ;
if ( V_85 != NULL )
F_29 ( V_85 , L_40 , V_89 >> 1 ) ;
}
break;
case V_162 :
V_89 = F_42 ( V_3 , V_88 , & V_123 ) ;
if ( V_89 & 0x00000001 ) {
V_92 = V_89 >> 1 ;
F_26 ( V_87 , V_163 , V_3 , V_88 , V_123 , V_92 ) ;
V_88 += V_123 ;
V_131 = ( T_4 * ) F_44 ( F_11 () , V_3 , V_88 , V_92 ) ;
F_45 ( V_87 , V_164 , V_3 , V_88 , V_92 , V_131 ) ;
F_29 ( V_86 , L_29 , F_46 ( F_11 () , V_131 , V_92 ) ) ;
if ( V_85 != NULL )
F_29 ( V_85 , L_29 , F_46 ( F_11 () , V_131 , V_92 ) ) ;
} else {
F_26 ( V_87 , V_107 , V_3 , V_88 , V_123 , V_89 >> 1 ) ;
F_29 ( V_86 , L_40 , V_89 >> 1 ) ;
if ( V_85 != NULL )
F_29 ( V_85 , L_40 , V_89 >> 1 ) ;
}
break;
default:
V_88 = F_41 ( V_3 ) ;
break;
}
F_28 ( V_86 , V_3 , V_88 ) ;
return V_88 ;
}
static T_2
F_47 ( T_3 * V_3 , T_2 V_4 , T_10 * V_61 , T_12 V_165 )
{
T_12 V_72 = FALSE ;
if ( V_165 ) {
V_4 ++ ;
}
while ( F_5 ( V_3 , V_4 ) > 0 )
{
if ( V_72 )
V_4 = F_43 ( V_3 , V_4 , V_61 , NULL ) ;
else
V_4 = F_27 ( V_3 , V_4 , V_61 , & V_72 , NULL ) ;
}
return V_4 ;
}
static void
F_48 ( T_3 * V_3 , T_2 V_4 , T_10 * V_61 )
{
T_4 V_166 ;
T_13 * V_167 ;
T_10 * V_168 ;
V_166 = F_7 ( V_3 , V_4 ) ;
V_167 = F_49 ( V_61 , V_169 , V_3 , V_4 , 1 , V_166 ,
L_50 , V_166 ,
F_30 ( ( V_166 & 0xf0 ) >> 4 , V_170 , L_51 ) ,
F_30 ( ( V_166 & 0x0c ) >> 2 , V_171 , L_52 ) ,
F_30 ( ( V_166 & 0x02 ) >> 1 , V_172 , L_53 ) ,
F_30 ( V_166 & 0x01 , V_173 , L_54 ) ) ;
V_168 = F_31 ( V_167 , V_174 ) ;
F_26 ( V_168 , V_175 , V_3 , V_4 , 1 , V_166 ) ;
F_26 ( V_168 , V_176 , V_3 , V_4 , 1 , V_166 ) ;
F_26 ( V_168 , V_177 , V_3 , V_4 , 1 , V_166 ) ;
F_26 ( V_168 , V_178 , V_3 , V_4 , 1 , V_166 ) ;
F_23 ( V_61 , V_179 , V_3 , V_4 + 1 , - 1 , V_83 ) ;
}
static void
F_50 ( T_3 * V_3 , T_2 V_4 , T_10 * V_61 )
{
T_4 V_166 ;
T_13 * V_180 ;
T_10 * V_181 ;
V_166 = F_7 ( V_3 , V_4 ) ;
V_180 = F_49 ( V_61 , V_182 , V_3 , V_4 , 1 , V_166 ,
L_55 , V_166 ,
F_30 ( ( V_166 & 0xf0 ) >> 4 , V_183 , L_56 ) ,
F_30 ( V_166 & 0x0f , V_184 , L_51 ) ) ;
V_181 = F_31 ( V_180 , V_185 ) ;
F_26 ( V_181 , V_186 , V_3 , V_4 , 1 , V_166 ) ;
F_26 ( V_181 , V_187 , V_3 , V_4 , 1 , V_166 ) ;
F_23 ( V_61 , V_188 , V_3 , V_4 + 1 , - 1 , V_83 ) ;
}
static void
F_51 ( T_3 * V_3 , T_2 V_4 , T_10 * V_61 )
{
T_10 * V_189 ;
T_10 * V_190 ;
while ( F_5 ( V_3 , V_4 ) > 0 ) {
T_4 V_191 ;
T_11 V_192 ;
V_191 = F_7 ( V_3 , V_4 + 0 ) ;
V_192 = F_52 ( V_3 , V_4 + 1 ) ;
V_189 = F_32 ( V_61 , V_3 , V_4 , 11 + V_192 + 4 , V_193 , NULL ,
F_30 ( V_191 , V_194 , L_57 ) ) ;
F_23 ( V_189 , V_195 , V_3 , V_4 + 0 , 1 , V_64 ) ;
F_23 ( V_189 , V_196 , V_3 , V_4 + 1 , 3 , V_64 ) ;
F_23 ( V_189 , V_197 , V_3 , V_4 + 4 , 3 , V_64 ) ;
F_23 ( V_189 , V_198 , V_3 , V_4 + 7 , 1 , V_64 ) ;
F_23 ( V_189 , V_199 , V_3 , V_4 + 8 , 3 , V_64 ) ;
V_190 = F_32 ( V_189 , V_3 , V_4 + 11 , V_192 , V_200 , NULL , L_58 ) ;
switch ( V_191 ) {
case 8 :
F_48 ( V_3 , V_4 + 11 , V_190 ) ;
break;
case 9 :
F_50 ( V_3 , V_4 + 11 , V_190 ) ;
break;
case 18 :
F_47 ( V_3 , V_4 + 11 , V_190 , FALSE ) ;
break;
default:
break;
}
F_23 ( V_189 , V_201 , V_3 , V_4 + 11 + V_192 , 4 , V_64 ) ;
V_4 += 11 + V_192 + 4 ;
}
}
static void
F_53 ( T_3 * V_3 , T_16 * V_202 , T_10 * V_70 , T_7 * V_28 , int V_29 , T_8 * V_30 )
{
T_2 V_4 = 0 ;
T_6 * V_203 = NULL ;
T_2 V_31 = FALSE ;
T_12 V_204 = FALSE ;
T_5 V_205 = 0 ;
T_5 V_206 = 0 ;
F_54 ( V_202 -> V_207 , V_208 , L_59 ) ;
F_19 ( L_60 ,
V_202 -> V_209 , V_202 -> V_210 -> V_211 . V_212 ,
F_5 ( V_3 , V_4 ) , V_70 ) ;
F_55 ( V_202 -> V_207 , V_213 ) ;
if ( F_5 ( V_3 , V_4 ) < 1 ) return;
if ( V_30 -> V_2 <= V_214 ) {
if ( V_30 -> V_1 < 3
&& F_5 ( V_3 , V_4 ) >= V_30 -> V_215 + 3
&& F_52 ( V_3 , V_4 + V_30 -> V_215 ) == 0xffffff ) {
V_204 = TRUE ;
}
V_205 = V_4 + V_30 -> V_215 + V_30 -> V_216 ;
V_206 = F_5 ( V_3 , V_205 ) ;
if ( V_30 -> V_32 == V_33 && V_30 -> V_37 >= 4 && V_206 >= 4 ) {
T_5 V_217 = F_8 ( V_3 , V_205 ) ;
if ( V_217 < V_218 ) {
F_56 ( V_28 -> V_219 [ V_29 ] , V_30 -> V_220 , F_57 ( V_217 ) ) ;
}
}
if ( ! F_58 ( V_202 ) ) {
if ( V_30 -> V_32 == V_51 || V_30 -> V_32 == V_52 ||
V_30 -> V_32 == V_53 || V_30 -> V_32 == V_54 ) {
T_5 V_56 = 0 ;
if ( V_30 -> V_32 == V_52 || V_30 -> V_32 == V_54 ) {
V_56 = 1 ;
}
V_30 -> V_58 = F_13 ( V_3 , V_205 + V_56 ) ;
if ( V_30 -> V_58 != 0 ) {
F_19 ( L_61 , V_30 -> V_58 ) ;
F_56 ( V_28 -> V_60 [ V_29 ] , V_30 -> V_58 , F_57 ( V_202 -> V_209 ) ) ;
}
}
}
}
if ( V_30 -> V_2 <= V_214 )
{
V_203 = F_15 ( V_3 , V_205 , V_206 , V_28 , V_29 , V_30 , & V_31 ) ;
}
if ( V_30 -> V_2 > V_214 ) {
F_59 ( V_202 -> V_207 , V_213 , L_62 ,
F_17 ( V_30 -> V_2 , V_221 , L_2 ) ) ;
F_60 ( V_202 -> V_207 , V_213 ) ;
} else if ( V_203 ) {
F_59 ( V_202 -> V_207 , V_213 , L_62 , V_203 ) ;
F_60 ( V_202 -> V_207 , V_213 ) ;
} else {
F_59 ( V_202 -> V_207 , V_213 , L_62 ,
F_17 ( V_30 -> V_32 , V_38 , L_2 ) ) ;
F_60 ( V_202 -> V_207 , V_213 ) ;
}
if ( V_70 )
{
T_10 * V_61 = NULL ;
T_10 * V_222 = NULL ;
T_13 * V_86 ;
V_86 = F_23 ( V_70 , V_223 , V_3 , V_4 , - 1 , V_83 ) ;
if ( V_30 -> V_2 > V_214 ) {
F_29 ( V_86 , L_63 ,
F_17 ( V_30 -> V_2 , V_221 , L_2 ) ) ;
V_222 = F_31 ( V_86 , V_224 ) ;
V_61 = F_32 ( V_222 , V_3 , V_4 , - 1 , V_225 , NULL ,
F_17 ( V_30 -> V_2 , V_221 , L_2 ) ) ;
if ( V_30 -> V_2 == V_226 )
{
F_23 ( V_61 , V_227 , V_3 , 0 , 1 , V_83 ) ;
F_23 ( V_61 , V_228 , V_3 , 1 , 1536 , V_83 ) ;
}
else if ( V_30 -> V_2 == V_229 )
{
F_23 ( V_61 , V_230 , V_3 , 0 , 1 , V_83 ) ;
F_23 ( V_61 , V_231 , V_3 , 1 , 1536 , V_83 ) ;
F_23 ( V_61 , V_232 , V_3 , 1537 , 1536 , V_83 ) ;
}
else if ( V_30 -> V_2 == V_233 )
{
F_23 ( V_61 , V_234 , V_3 , 0 , 1536 , V_83 ) ;
}
return;
}
if ( V_203 && V_31 ) {
F_29 ( V_86 , L_63 , V_203 ) ;
} else if ( V_203 ) {
F_29 ( V_86 , L_64 ,
F_17 ( V_30 -> V_32 , V_38 , L_2 ) , V_203 ) ;
} else {
F_29 ( V_86 , L_63 ,
F_17 ( V_30 -> V_32 , V_38 , L_2 ) ) ;
}
V_222 = F_31 ( V_86 , V_224 ) ;
if ( V_30 -> V_59 != 0 ) {
F_26 ( V_222 ,
V_30 -> V_57 ? V_235 : V_236 ,
V_3 , V_4 , V_30 -> V_215 + V_30 -> V_216 + V_30 -> V_37 ,
V_30 -> V_59 ) ;
}
V_61 = F_32 ( V_222 , V_3 , V_4 , V_30 -> V_215 + V_30 -> V_216 , V_237 , NULL , V_238 ) ;
if ( V_30 -> V_1 <= 3 ) F_23 ( V_61 , V_239 , V_3 , V_4 + 0 , 1 , V_64 ) ;
if ( V_30 -> V_1 <= 3 ) F_23 ( V_61 , V_240 , V_3 , V_4 + 0 , V_30 -> V_215 , V_64 ) ;
if ( V_30 -> V_1 <= 2 ) {
if ( V_30 -> V_1 > 0 ) {
F_23 ( V_61 , V_241 , V_3 , V_4 + V_30 -> V_215 , 3 , V_64 ) ;
} else {
F_23 ( V_61 , V_242 , V_3 , V_4 + V_30 -> V_215 , 3 , V_64 ) ;
}
if ( V_204 ) {
F_23 ( V_61 , V_243 , V_3 , V_4 + V_30 -> V_215 + V_30 -> V_216 - 4 , 4 , V_64 ) ;
}
}
if ( ( V_30 -> V_1 > 0 && ! V_204 ) || V_30 -> V_1 == 3 ) {
F_61 ( V_61 , V_242 , V_3 , V_4 + V_30 -> V_215 , 0 , V_30 -> V_244 , L_65 , V_30 -> V_244 ) ;
}
if ( V_30 -> V_1 <= 1 ) F_23 ( V_61 , V_245 , V_3 , V_4 + V_30 -> V_215 + 3 , 3 , V_64 ) ;
if ( V_30 -> V_1 <= 1 ) F_23 ( V_61 , V_246 , V_3 , V_4 + V_30 -> V_215 + 6 , 1 , V_64 ) ;
if ( V_30 -> V_1 <= 0 ) F_23 ( V_61 , V_247 , V_3 , V_4 + V_30 -> V_215 + 7 , 4 , V_248 ) ;
if ( V_30 -> V_37 == 0 ) return;
V_4 = V_205 ;
V_61 = F_32 ( V_222 , V_3 , V_4 , - 1 , V_249 , NULL , V_250 ) ;
switch ( V_30 -> V_32 ) {
case V_33 :
case V_34 :
case V_35 :
case V_41 :
case V_36 :
case V_39 :
F_22 ( V_3 , V_4 , V_61 , V_30 -> V_32 ) ;
break;
case V_51 :
case V_53 :
F_47 ( V_3 , V_4 , V_61 , FALSE ) ;
break;
case V_52 :
case V_54 :
F_47 ( V_3 , V_4 , V_61 , TRUE ) ;
break;
case V_251 :
F_48 ( V_3 , V_4 , V_61 ) ;
break;
case V_252 :
F_50 ( V_3 , V_4 , V_61 ) ;
break;
case V_253 :
F_51 ( V_3 , V_4 , V_61 ) ;
break;
}
}
}
static void
F_62 ( T_3 * V_3 , T_16 * V_202 , T_10 * V_70 , T_7 * V_28 , int V_29 , T_5 V_254 , T_5 V_255 )
{
int V_4 = 0 ;
int V_6 ;
int V_256 ;
T_4 V_257 ;
int V_258 ;
int V_259 ;
T_5 V_2 ;
T_5 V_244 = 0 ;
T_5 V_260 = 0 ;
int V_261 ;
T_4 V_32 ;
T_5 V_262 ;
int V_263 ;
T_5 V_264 = 0 ;
T_17 * V_265 ;
T_18 * V_86 ;
T_8 * V_30 ;
T_3 * V_266 ;
V_6 = F_41 ( V_3 ) ;
if ( ! V_6 )
return;
F_19 ( L_66 , V_29 , V_254 , V_254 + V_6 - 1 ) ;
if ( V_202 -> V_210 -> V_211 . V_212 ) {
T_19 * V_267 ;
V_267 = F_63 ( F_11 () ) ;
F_64 ( V_267 , 0 ) ;
V_30 = ( T_8 * ) F_65 ( V_28 -> V_267 [ V_29 ] , V_254 + V_6 - 1 ) ;
while ( V_30 && V_30 -> V_220 >= V_254 && V_30 -> V_220 >= V_264 ) {
F_64 ( V_267 , V_30 ) ;
V_264 = V_30 -> V_220 + 1 ;
V_30 = ( T_8 * ) F_65 ( V_28 -> V_267 [ V_29 ] , V_30 -> V_220 - 1 ) ;
}
while ( ( V_30 = ( T_8 * ) F_66 ( V_267 ) ) != NULL ) {
if ( V_30 -> V_268 ) {
V_266 = F_67 ( V_3 , V_30 -> V_269 . V_270 , V_30 -> V_271 , V_30 -> V_271 ) ;
F_68 ( V_202 , V_266 , L_67 ) ;
} else {
V_266 = F_69 ( V_3 , V_30 -> V_269 . V_4 , V_30 -> V_271 ) ;
}
F_53 ( V_266 , V_202 , V_70 , V_28 , V_29 , V_30 ) ;
}
return;
}
while ( V_6 > 0 ) {
V_265 = NULL ;
V_86 = NULL ;
V_30 = NULL ;
if ( V_4 == 0 ) {
V_265 = ( T_17 * ) F_65 ( V_28 -> V_272 [ V_29 ] , V_254 + V_4 - 1 ) ;
if ( V_265 ) {
F_19 ( L_68 , V_265 -> V_254 , V_265 -> V_220 , V_265 -> V_271 , V_265 -> V_37 ) ;
if ( V_265 -> V_271 >= V_265 -> V_37 || V_254 + V_4 < V_265 -> V_254 || V_254 + V_4 > V_265 -> V_220 + V_265 -> V_37 - V_265 -> V_271 ) {
V_265 = NULL ;
} else if ( ! V_265 -> V_273 ) {
V_86 = ( T_18 * ) F_21 ( V_28 -> V_274 [ V_29 ] , V_265 -> V_275 . V_2 ) ;
if ( V_86 ) {
V_30 = ( T_8 * ) F_65 ( V_86 -> V_267 , V_254 + V_4 - 1 ) ;
}
if ( V_30 && V_30 -> V_276 ) {
goto V_277;
}
V_265 = NULL ;
V_86 = NULL ;
V_30 = NULL ;
}
if ( V_265 ) {
V_256 = V_265 -> V_37 - V_265 -> V_271 ;
if ( V_6 < V_256 )
V_256 = V_6 ;
F_70 ( V_3 , V_265 -> V_275 . V_278 + V_265 -> V_271 , V_4 , V_256 ) ;
V_2 = V_265 -> V_275 . V_278 [ 0 ] ;
V_257 = ( V_2 >> 6 ) & 3 ;
V_258 = F_2 ( V_2 ) ;
if ( ( V_257 < 3 ) && ( V_265 -> V_271 < ( V_258 + 3 ) ) && ( V_265 -> V_271 + V_256 >= ( V_258 + 3 ) ) ) {
if ( F_71 ( V_265 -> V_275 . V_278 + V_258 ) == 0xffffff ) {
V_265 -> V_37 += 4 ;
}
}
V_265 -> V_271 += V_256 ;
V_265 -> V_220 = V_254 + V_256 - 1 ;
V_6 -= V_256 ;
V_4 += V_256 ;
if ( V_265 -> V_271 < V_265 -> V_37 ) {
return;
}
}
}
}
if ( ! V_265 ) {
V_2 = F_7 ( V_3 , V_4 ) ;
if ( V_2 == V_279 && V_254 + V_4 == V_280 ) {
V_257 = 4 ;
V_258 = 1 ;
V_259 = 0 ;
V_2 = V_255 == 1 ? V_226 : V_229 ;
} else if ( V_254 + V_4 == V_281 ) {
V_257 = 4 ;
V_258 = 0 ;
V_259 = 0 ;
V_2 = V_233 ;
} else {
V_257 = ( V_2 >> 6 ) & 3 ;
V_258 = F_2 ( V_2 ) ;
V_259 = F_3 ( V_2 ) ;
if ( ( V_257 < 3 ) && ( V_6 >= ( V_258 + 3 ) ) ) {
if ( F_52 ( V_3 , V_4 + V_258 ) == 0xffffff ) {
V_259 += 4 ;
}
}
if ( V_6 < ( V_258 + V_259 ) ) {
V_265 = F_72 ( F_73 () , T_17 ) ;
V_265 -> V_273 = 1 ;
V_265 -> V_254 = V_254 + V_4 ;
V_265 -> V_220 = V_265 -> V_254 + V_6 - 1 ;
V_265 -> V_37 = V_258 + V_259 ;
F_70 ( V_3 , V_265 -> V_275 . V_278 , V_4 , V_6 ) ;
V_265 -> V_271 = V_6 ;
F_56 ( V_28 -> V_272 [ V_29 ] , V_254 + V_4 , V_265 ) ;
return;
}
V_2 = V_2 & 0x3f ;
if ( V_2 == 0 )
V_2 = F_7 ( V_3 , V_4 + 1 ) + 64 ;
else if ( V_2 == 1 )
V_2 = F_74 ( V_3 , V_4 + 1 ) + 64 ;
}
} else {
V_2 = V_265 -> V_275 . V_278 [ 0 ] ;
V_257 = ( V_2 >> 6 ) & 3 ;
V_258 = F_2 ( V_2 ) ;
V_259 = V_265 -> V_37 - V_258 ;
V_2 = V_2 & 0x3f ;
if ( V_2 == 0 )
V_2 = V_265 -> V_275 . V_278 [ 1 ] + 64 ;
else if ( V_2 == 1 )
V_2 = F_75 ( V_265 -> V_275 . V_278 + 1 ) + 64 ;
}
if ( V_2 <= V_214 )
V_86 = ( T_18 * ) F_21 ( V_28 -> V_274 [ V_29 ] , V_2 ) ;
if ( V_86 )
V_30 = ( T_8 * ) F_65 ( V_86 -> V_267 , V_254 + V_4 - 1 ) ;
if ( V_257 == 0 )
V_262 = V_265 ? F_76 ( V_265 -> V_275 . V_278 + V_258 + 7 ) : F_8 ( V_3 , V_4 + V_258 + 7 ) ;
else if ( V_86 )
V_262 = V_86 -> V_262 ;
else V_262 = 0 ;
if ( V_257 < 2 )
V_32 = V_265 ? V_265 -> V_275 . V_278 [ V_258 + 6 ] : F_7 ( V_3 , V_4 + V_258 + 6 ) ;
else if ( V_86 )
V_32 = V_86 -> V_32 ;
else
V_32 = 0 ;
if ( V_2 > V_214 ) {
if ( V_2 == V_226 )
V_263 = V_261 = 1536 ;
else if ( V_2 == V_229 )
V_263 = V_261 = 3072 ;
else
V_263 = V_261 = 1536 ;
} else {
V_263 = F_20 ( F_65 ( V_28 -> V_219 [ V_29 ] , V_254 + V_4 - 1 ) ) ;
if ( ! V_263 )
V_263 = V_282 ;
if ( V_257 < 2 )
V_261 = V_265 ? F_71 ( V_265 -> V_275 . V_278 + V_258 + 3 ) : F_52 ( V_3 , V_4 + V_258 + 3 ) ;
else if ( V_86 )
V_261 = V_86 -> V_37 ;
else
V_261 = V_263 ;
if ( V_261 > ( T_2 ) V_218 ) {
return;
}
}
if ( ! V_86 || ! V_30 || V_257 < 3 || V_30 -> V_271 == V_30 -> V_256 || V_30 -> V_283 != V_30 -> V_276 ) {
F_19 ( L_69 ,
V_29 , V_254 + V_4 ,
V_86 , V_30 , V_257 , V_258 + V_259 , V_2 , V_30 ? V_30 -> V_271 : 0 , V_30 ? V_30 -> V_256 : 0 , V_261 , V_263 ) ;
if ( ! V_86 ) {
V_86 = F_72 ( F_73 () , T_18 ) ;
V_86 -> V_267 = F_77 ( F_73 () ) ;
V_86 -> V_244 = 0 ;
V_86 -> V_260 = 0 ;
F_56 ( V_28 -> V_274 [ V_29 ] , V_2 , V_86 ) ;
}
if ( V_257 == 0 ) {
V_244 = V_265 ? F_71 ( V_265 -> V_275 . V_278 + V_258 ) : F_52 ( V_3 , V_4 + V_258 ) ;
if ( V_244 == 0xffffff ) {
V_244 = V_265 ? F_76 ( V_265 -> V_275 . V_278 + V_258 + 11 ) : F_8 ( V_3 , V_4 + V_258 + 11 ) ;
}
V_260 = V_244 - V_86 -> V_244 ;
} else if ( V_257 < 3 ) {
V_260 = V_265 ? F_71 ( V_265 -> V_275 . V_278 + V_258 ) : F_52 ( V_3 , V_4 + V_258 ) ;
if ( V_260 == 0xffffff ) {
V_244 = V_265 ? F_76 ( V_265 -> V_275 . V_278 + V_258 + V_259 - 4 ) : F_8 ( V_3 , V_4 + V_258 + V_259 - 4 ) ;
V_260 = V_86 -> V_260 ;
} else {
V_244 = V_86 -> V_244 + V_260 ;
}
} else {
V_244 = V_86 -> V_244 + V_86 -> V_260 ;
V_260 = V_86 -> V_260 ;
}
V_30 = F_72 ( F_73 () , T_8 ) ;
V_30 -> V_254 = V_30 -> V_220 = V_265 ? V_265 -> V_254 : V_254 + V_4 ;
V_30 -> V_271 = 0 ;
V_30 -> V_256 = V_258 + V_259 + V_261 ;
V_30 -> V_276 = 0 ;
V_30 -> V_283 = 0 ;
V_30 -> V_215 = V_258 ;
V_30 -> V_216 = V_259 ;
V_30 -> V_1 = V_257 ;
V_30 -> V_2 = V_2 ;
V_30 -> V_244 = V_244 ;
V_30 -> V_37 = V_261 ;
if ( V_2 > V_214 )
V_30 -> V_32 = V_2 ;
else
V_30 -> V_32 = V_32 & 0x7f ;
V_30 -> V_262 = V_262 ;
V_30 -> V_58 = 0 ;
V_30 -> V_57 = FALSE ;
V_30 -> V_59 = 0 ;
V_86 -> V_244 = V_244 ;
V_86 -> V_260 = V_260 ;
V_86 -> V_37 = V_261 ;
V_86 -> V_32 = V_32 ;
V_86 -> V_262 = V_262 ;
F_56 ( V_86 -> V_267 , V_30 -> V_254 , V_30 ) ;
if ( ! V_265 && V_261 <= V_263 && V_30 -> V_256 <= V_6 ) {
V_30 -> V_268 = FALSE ;
V_30 -> V_269 . V_4 = V_4 ;
V_30 -> V_220 = V_254 + V_4 + V_30 -> V_256 - 1 ;
V_30 -> V_271 = V_30 -> V_256 ;
F_56 ( V_28 -> V_267 [ V_29 ] , V_30 -> V_220 , V_30 ) ;
V_266 = F_69 ( V_3 , V_30 -> V_269 . V_4 , V_30 -> V_271 ) ;
F_53 ( V_266 , V_202 , V_70 , V_28 , V_29 , V_30 ) ;
V_4 += V_30 -> V_256 ;
V_6 -= V_30 -> V_256 ;
continue;
} else {
V_30 -> V_268 = TRUE ;
V_30 -> V_269 . V_270 = ( T_4 * ) F_78 ( F_73 () , V_30 -> V_215 + V_30 -> V_216 + V_30 -> V_37 ) ;
if ( V_265 && V_265 -> V_273 ) {
memcpy ( V_30 -> V_269 . V_270 , V_265 -> V_275 . V_278 , V_265 -> V_37 ) ;
} else {
F_70 ( V_3 , V_30 -> V_269 . V_270 , V_4 , V_258 + V_259 ) ;
V_4 += V_258 + V_259 ;
V_6 -= V_258 + V_259 ;
}
V_30 -> V_220 = V_254 + V_4 - 1 ;
V_30 -> V_271 = V_258 + V_259 ;
if ( V_30 -> V_271 == V_30 -> V_256 ) {
F_56 ( V_28 -> V_267 [ V_29 ] , V_30 -> V_220 , V_30 ) ;
V_266 = F_67 ( V_3 , V_30 -> V_269 . V_270 , V_30 -> V_271 , V_30 -> V_271 ) ;
F_68 ( V_202 , V_266 , L_67 ) ;
F_53 ( V_266 , V_202 , V_70 , V_28 , V_29 , V_30 ) ;
continue;
}
V_30 -> V_276 = V_263 ;
if ( V_30 -> V_276 > V_30 -> V_256 - V_30 -> V_271 )
V_30 -> V_276 = V_30 -> V_256 - V_30 -> V_271 ;
}
} else {
F_19 ( L_70 ,
V_29 , V_254 + V_4 ,
V_86 , V_30 , V_258 + V_259 , V_2 , V_30 ? V_30 -> V_271 : 0 , V_30 ? V_30 -> V_256 : 0 , V_261 , V_263 ) ;
V_30 -> V_276 = V_263 ;
if ( V_30 -> V_276 > V_30 -> V_256 - V_30 -> V_271 )
V_30 -> V_276 = V_30 -> V_256 - V_30 -> V_271 ;
V_4 += V_258 + V_259 ;
V_6 -= V_258 + V_259 ;
}
V_265 = NULL ;
V_277:
V_256 = V_30 -> V_276 - V_30 -> V_283 ;
if ( V_256 > V_6 )
V_256 = V_6 ;
F_19 ( L_71 , V_30 -> V_276 , V_30 -> V_283 , V_6 , V_256 ) ;
F_70 ( V_3 , V_30 -> V_269 . V_270 + V_30 -> V_271 , V_4 , V_256 ) ;
if ( V_265 ) {
V_265 -> V_271 += V_256 ;
V_265 -> V_220 = V_254 + V_4 + V_256 - 1 ;
}
V_30 -> V_220 = V_254 + V_4 + V_256 - 1 ;
V_30 -> V_271 += V_256 ;
V_30 -> V_283 += V_256 ;
V_4 += V_256 ;
V_6 -= V_256 ;
if ( V_30 -> V_283 == V_30 -> V_276 ) {
V_30 -> V_283 = 0 ;
V_30 -> V_276 = 0 ;
}
if ( V_30 -> V_271 == V_30 -> V_256 ) {
F_56 ( V_28 -> V_267 [ V_29 ] , V_30 -> V_220 , V_30 ) ;
V_266 = F_67 ( V_3 , V_30 -> V_269 . V_270 , V_30 -> V_271 , V_30 -> V_271 ) ;
F_68 ( V_202 , V_266 , L_67 ) ;
F_53 ( V_266 , V_202 , V_70 , V_28 , V_29 , V_30 ) ;
} else if ( V_30 -> V_283 < V_30 -> V_276 ) {
T_17 * V_284 = F_72 ( F_73 () , T_17 ) ;
V_284 -> V_273 = 0 ;
V_284 -> V_254 = V_254 + V_4 - V_256 ;
V_284 -> V_220 = V_284 -> V_254 + V_6 - 1 + V_256 ;
V_284 -> V_271 = V_30 -> V_283 ;
V_284 -> V_37 = V_30 -> V_276 ;
V_284 -> V_275 . V_2 = V_30 -> V_2 ;
F_19 ( L_72 , V_254 + V_4 - V_256 - 1 ) ;
F_56 ( V_28 -> V_272 [ V_29 ] , V_254 + V_4 - V_256 - 1 , V_284 ) ;
}
}
}
static T_7 *
F_79 ( T_20 * V_285 )
{
T_7 * V_28 = F_72 ( F_73 () , T_7 ) ;
F_80 ( V_285 , V_223 , V_28 ) ;
V_28 -> V_286 [ 0 ] = F_77 ( F_73 () ) ;
V_28 -> V_286 [ 1 ] = F_77 ( F_73 () ) ;
V_28 -> V_272 [ 0 ] = F_77 ( F_73 () ) ;
V_28 -> V_272 [ 1 ] = F_77 ( F_73 () ) ;
V_28 -> V_274 [ 0 ] = F_77 ( F_73 () ) ;
V_28 -> V_274 [ 1 ] = F_77 ( F_73 () ) ;
V_28 -> V_267 [ 0 ] = F_77 ( F_73 () ) ;
V_28 -> V_267 [ 1 ] = F_77 ( F_73 () ) ;
V_28 -> V_219 [ 0 ] = F_77 ( F_73 () ) ;
V_28 -> V_219 [ 1 ] = F_77 ( F_73 () ) ;
V_28 -> V_60 [ 0 ] = F_77 ( F_73 () ) ;
V_28 -> V_60 [ 1 ] = F_77 ( F_73 () ) ;
return V_28 ;
}
static int
F_81 ( T_3 * V_3 , T_16 * V_202 , T_10 * V_70 , void * V_269 )
{
T_20 * V_285 ;
T_7 * V_28 ;
int V_29 ;
struct V_287 * V_287 ;
if ( V_269 == NULL ) {
return 0 ;
}
V_287 = (struct V_287 * ) V_269 ;
V_285 = F_82 ( V_202 ) ;
V_28 = ( T_7 * ) F_83 ( V_285 , V_223 ) ;
if ( ! V_28 ) {
V_28 = F_79 ( V_285 ) ;
}
V_29 = ( F_84 ( & V_285 -> V_288 -> V_289 , & V_202 -> V_262 ) &&
F_84 ( & V_285 -> V_288 -> V_290 , & V_202 -> V_291 ) &&
V_285 -> V_288 -> V_292 == V_202 -> V_293 &&
V_285 -> V_288 -> V_294 == V_202 -> V_295 ) ? 0 : 1 ;
F_62 ( V_3 , V_202 , V_70 , V_28 , V_29 , V_287 -> V_254 , V_287 -> V_255 ) ;
return F_41 ( V_3 ) ;
}
static int
F_85 ( T_3 * V_3 , T_16 * V_202 , T_10 * V_70 , void * V_269 V_296 )
{
T_20 * V_285 ;
T_7 * V_28 ;
int V_29 ;
T_5 V_254 ;
T_5 V_255 ;
T_5 V_4 ;
T_2 V_6 ;
V_4 = 0 ;
V_6 = F_5 ( V_3 , 0 ) ;
V_29 = V_202 -> V_293 == V_202 -> V_297 ;
if ( V_29 ) {
V_285 = F_86 ( V_202 -> V_209 , & V_202 -> V_291 , & V_202 -> V_262 , V_202 -> V_298 , 0 , V_202 -> V_293 , 0 ) ;
if ( ! V_285 ) {
F_19 ( L_73 ) ;
V_285 = F_87 ( V_202 -> V_209 , & V_202 -> V_291 , & V_202 -> V_262 , V_202 -> V_298 , 0 , V_202 -> V_293 , 0 ) ;
}
} else {
V_285 = F_86 ( V_202 -> V_209 , & V_202 -> V_262 , & V_202 -> V_291 , V_202 -> V_298 , 0 , V_202 -> V_295 , 0 ) ;
if ( ! V_285 ) {
F_19 ( L_73 ) ;
V_285 = F_87 ( V_202 -> V_209 , & V_202 -> V_262 , & V_202 -> V_291 , V_202 -> V_298 , 0 , V_202 -> V_295 , 0 ) ;
}
}
V_28 = ( T_7 * ) F_83 ( V_285 , V_223 ) ;
if ( ! V_28 ) {
V_28 = F_79 ( V_285 ) ;
}
V_255 = F_20 ( F_65 ( V_28 -> V_286 [ V_29 ^ 1 ] , V_202 -> V_209 ) ) + 1 ;
if ( V_29 == 1 && V_255 < 2 && V_6 == 17 ) {
V_4 += 17 ;
V_6 -= 17 ;
} else if ( V_29 || V_6 == 1 ) {
V_4 ++ ;
V_6 -- ;
}
V_254 = F_20 ( F_21 ( V_28 -> V_286 [ V_29 ] , V_202 -> V_209 ) ) ;
if ( V_254 == 0 ) {
V_254 = F_20 ( F_65 ( V_28 -> V_286 [ V_29 ] , V_202 -> V_209 ) ) ;
V_254 += V_6 ;
F_56 ( V_28 -> V_286 [ V_29 ] , V_202 -> V_209 , F_57 ( V_254 ) ) ;
}
V_254 -= V_6 - 1 ;
F_19 ( L_74 , V_202 -> V_209 , V_29 , V_254 , V_255 , V_6 ) ;
if ( V_6 < 1 )
return V_4 ;
if ( V_4 > 0 ) {
T_3 * V_299 = F_69 ( V_3 , V_4 , V_6 ) ;
F_62 ( V_299 , V_202 , V_70 , V_28 , V_29 , V_254 , V_255 ) ;
} else {
F_62 ( V_3 , V_202 , V_70 , V_28 , V_29 , V_254 , V_255 ) ;
}
return F_88 ( V_3 ) ;
}
static T_12
F_89 ( T_3 * V_3 , T_16 * V_202 , T_10 * V_70 , void * V_269 )
{
T_20 * V_300 ;
if ( F_41 ( V_3 ) >= 12 )
{
struct V_287 * V_287 = (struct V_287 * ) V_269 ;
if ( V_287 -> V_255 == V_281
&& V_287 -> V_254 == V_280
&& F_7 ( V_3 , 0 ) == V_279 )
{
V_300 = F_82 ( V_202 ) ;
F_90 ( V_300 , V_301 ) ;
F_81 ( V_3 , V_202 , V_70 , V_269 ) ;
return TRUE ;
}
}
return FALSE ;
}
static int
F_91 ( T_3 * V_3 , T_16 * V_202 V_296 , T_10 * V_70 , void * V_269 V_296 )
{
T_13 * V_86 ;
T_10 * V_302 , * V_303 , * V_304 ;
int V_4 ;
T_11 V_305 , V_306 , V_93 ;
T_11 V_307 ;
T_11 V_308 , V_309 ;
T_12 V_72 = FALSE ;
V_86 = F_23 ( V_70 , V_310 , V_3 , 0 , - 1 , V_83 ) ;
V_302 = F_31 ( V_86 , V_311 ) ;
V_4 = 0 ;
F_23 ( V_302 , V_312 , V_3 , V_4 , 2 , V_64 ) ;
V_4 += 2 ;
V_305 = F_6 ( V_3 , V_4 ) ;
F_26 ( V_302 , V_313 , V_3 , V_4 , 2 , V_305 ) ;
V_4 += 2 ;
if ( V_305 != 0 ) {
V_303 = F_32 ( V_302 , V_3 , V_4 , - 1 , V_314 , NULL , L_75 ) ;
for ( V_93 = 0 ; V_93 < V_305 ; V_93 ++ ) {
V_307 = F_6 ( V_3 , V_4 ) ;
F_23 ( V_303 , V_315 , V_3 , V_4 , 2 , V_82 | V_64 ) ;
V_4 += 2 + V_307 ;
F_23 ( V_303 , V_316 , V_3 , V_4 , 1 , V_83 ) ;
V_4 += 1 ;
V_308 = F_8 ( V_3 , V_4 ) ;
if ( V_308 == 0xFFFFFFFF )
F_61 ( V_303 , V_317 , V_3 , V_4 , 4 , V_308 , L_30 ) ;
else
F_26 ( V_303 , V_317 , V_3 , V_4 , 4 , V_308 ) ;
V_4 += 4 ;
if ( V_72 )
V_4 = F_43 ( V_3 , V_4 , V_303 , NULL ) ;
else
V_4 = F_27 ( V_3 , V_4 , V_303 , & V_72 , NULL ) ;
}
}
V_306 = F_6 ( V_3 , V_4 ) ;
F_26 ( V_302 , V_318 , V_3 , V_4 , 2 , V_306 ) ;
V_4 += 2 ;
if ( V_306 != 0 ) {
V_304 = F_32 ( V_302 , V_3 , V_4 , - 1 , V_319 , NULL , L_76 ) ;
for ( V_93 = 0 ; V_93 < V_306 ; V_93 ++ ) {
V_307 = F_6 ( V_3 , V_4 ) ;
F_23 ( V_304 , V_320 , V_3 , V_4 , 2 , V_82 | V_64 ) ;
V_4 += 2 + V_307 ;
V_307 = F_6 ( V_3 , V_4 ) ;
F_23 ( V_304 , V_321 , V_3 , V_4 , 2 , V_82 | V_64 ) ;
V_4 += 2 + V_307 ;
V_309 = F_8 ( V_3 , V_4 ) ;
if ( V_309 == 0xFFFFFFFF )
F_61 ( V_304 , V_322 , V_3 , V_4 , 4 , V_309 , L_30 ) ;
else
F_26 ( V_304 , V_322 , V_3 , V_4 , 4 , V_309 ) ;
V_4 += 4 ;
V_4 = F_47 ( V_3 , V_4 , V_304 , FALSE ) ;
}
}
return F_88 ( V_3 ) ;
}
void
F_92 ( void )
{
static T_21 V_323 [] = {
{ & V_227 ,
{ L_77 , L_78 , V_324 , V_325 ,
NULL , 0x0 , L_79 , V_326 } } ,
{ & V_230 ,
{ L_77 , L_80 , V_324 , V_325 ,
NULL , 0x0 , L_81 , V_326 } } ,
{ & V_228 ,
{ L_82 , L_83 , V_324 , V_325 ,
NULL , 0x0 , L_84 , V_326 } } ,
{ & V_231 ,
{ L_82 , L_85 , V_324 , V_325 ,
NULL , 0x0 , L_86 , V_326 } } ,
{ & V_234 ,
{ L_82 , L_87 , V_324 , V_325 ,
NULL , 0x0 , L_88 , V_326 } } ,
{ & V_232 ,
{ L_82 , L_89 , V_324 , V_325 ,
NULL , 0x0 , L_90 , V_326 } } ,
{ & V_239 ,
{ L_91 , L_92 , V_327 , V_328 ,
NULL , 0xC0 , L_93 , V_326 } } ,
{ & V_240 ,
{ L_94 , L_95 , V_327 , V_328 ,
NULL , 0x3F , L_96 , V_326 } } ,
{ & V_242 ,
{ L_97 , L_98 , V_329 , V_328 ,
NULL , 0x0 , L_99 , V_326 } } ,
{ & V_241 ,
{ L_100 , L_101 , V_329 , V_328 ,
NULL , 0x0 , L_102 , V_326 } } ,
{ & V_245 ,
{ L_103 , L_104 , V_329 , V_328 ,
NULL , 0x0 , L_105 , V_326 } } ,
{ & V_246 ,
{ L_106 , L_107 , V_327 , V_330 ,
F_93 ( V_38 ) , 0x0 , L_108 , V_326 } } ,
{ & V_247 ,
{ L_109 , L_110 , V_331 , V_328 ,
NULL , 0x0 , L_111 , V_326 } } ,
{ & V_243 ,
{ L_112 , L_113 , V_329 , V_328 ,
NULL , 0x0 , L_114 , V_326 } } ,
{ & V_63 ,
{ L_115 , L_116 , V_331 , V_328 ,
NULL , 0x0 , L_117 , V_326 } } ,
{ & V_65 ,
{ L_118 , L_119 , V_331 , V_328 ,
NULL , 0x0 , L_120 , V_326 } } ,
{ & V_66 ,
{ L_121 , L_122 , V_331 , V_328 ,
NULL , 0x0 , L_123 , V_326 } } ,
{ & V_68 ,
{ L_124 , L_125 , V_331 , V_328 ,
NULL , 0x0 , L_126 , V_326 } } ,
{ & V_69 ,
{ L_127 , L_128 , V_327 , V_328 ,
F_93 ( V_40 ) , 0x0 , L_126 , V_326 } } ,
{ & V_67 ,
{ L_129 , L_130 , V_332 , V_328 ,
F_93 ( V_45 ) , 0x0 , L_131 , V_326 } } ,
{ & V_236 ,
{ L_132 , L_133 , V_333 , V_325 ,
NULL , 0x0 , L_134 , V_326 } } ,
{ & V_235 ,
{ L_135 , L_136 , V_333 , V_325 ,
NULL , 0x0 , L_137 , V_326 } } ,
{ & V_169 ,
{ L_138 , L_139 , V_327 , V_330 ,
NULL , 0x0 , L_140 , V_326 } } ,
{ & V_175 ,
{ L_91 , L_141 , V_327 , V_328 ,
F_93 ( V_170 ) , 0xf0 , L_142 , V_326 } } ,
{ & V_176 ,
{ L_143 , L_144 , V_327 , V_328 ,
F_93 ( V_171 ) , 0x0c , L_145 , V_326 } } ,
{ & V_177 ,
{ L_146 , L_147 , V_327 , V_328 ,
F_93 ( V_172 ) , 0x02 , L_148 , V_326 } } ,
{ & V_178 ,
{ L_149 , L_150 , V_327 , V_328 ,
F_93 ( V_173 ) , 0x01 , L_151 , V_326 } } ,
{ & V_179 ,
{ L_152 , L_153 , V_324 , V_325 ,
NULL , 0x0 , L_154 , V_326 } } ,
{ & V_182 ,
{ L_155 , L_156 , V_327 , V_330 ,
NULL , 0x0 , L_157 , V_326 } } ,
{ & V_186 ,
{ L_158 , L_159 , V_327 , V_328 ,
F_93 ( V_183 ) , 0xf0 , L_160 , V_326 } } ,
{ & V_187 ,
{ L_91 , L_161 , V_327 , V_328 ,
F_93 ( V_184 ) , 0x0f , L_162 , V_326 } } ,
{ & V_188 ,
{ L_163 , L_164 , V_324 , V_325 ,
NULL , 0x0 , L_165 , V_326 } } ,
{ & V_195 ,
{ L_158 , L_166 , V_327 , V_328 ,
F_93 ( V_194 ) , 0x0 , L_167 , V_326 } } ,
{ & V_196 ,
{ L_168 , L_169 , V_329 , V_328 ,
NULL , 0x0 , L_170 , V_326 } } ,
{ & V_197 ,
{ L_97 , L_171 , V_329 , V_328 ,
NULL , 0x0 , L_172 , V_326 } } ,
{ & V_198 ,
{ L_173 , L_174 , V_327 , V_328 ,
NULL , 0x0 , L_175 , V_326 } } ,
{ & V_199 ,
{ L_109 , L_176 , V_329 , V_328 ,
NULL , 0x0 , L_177 , V_326 } } ,
{ & V_201 ,
{ L_178 , L_179 , V_331 , V_328 ,
NULL , 0x0 , L_180 , V_326 } }
} ;
static T_2 * V_334 [] = {
& V_224 ,
& V_225 ,
& V_237 ,
& V_249 ,
& V_335 ,
& V_174 ,
& V_185 ,
& V_193 ,
& V_200
} ;
T_22 * V_336 ;
V_223 = F_94 ( L_181 , L_182 , L_183 ) ;
F_95 ( V_223 , V_323 , F_96 ( V_323 ) ) ;
F_97 ( V_334 , F_96 ( V_334 ) ) ;
V_336 = F_98 ( V_223 , NULL ) ;
F_99 ( V_336 , L_184 ,
L_185 ,
L_186
L_187
L_188 ,
& V_337 ) ;
F_100 ( V_336 , L_189 ,
L_190 ,
L_191 ,
10 , & V_218 ) ;
}
void
F_101 ( void )
{
static T_21 V_323 [] = {
{ & V_312 ,
{ L_192 , L_193 , V_332 , V_328 ,
NULL , 0x0 , NULL , V_326 } } ,
{ & V_313 ,
{ L_194 , L_195 , V_332 , V_328 ,
NULL , 0x0 , NULL , V_326 } } ,
{ & V_315 ,
{ L_196 , L_197 , V_338 , V_325 ,
NULL , 0x0 , NULL , V_326 } } ,
{ & V_316 ,
{ L_198 , L_199 , V_339 , V_325 ,
NULL , 0x0 , NULL , V_326 } } ,
{ & V_317 ,
{ L_200 , L_201 , V_331 , V_328 ,
NULL , 0x0 , NULL , V_326 } } ,
#if 0
{ &hf_amf_header_value_type,
{ "Value type", "amf.header.value_type", FT_UINT32, BASE_HEX,
VALS(rtmpt_type_vals), 0x0, NULL, HFILL }},
#endif
{ & V_318 ,
{ L_202 , L_203 , V_332 , V_328 ,
NULL , 0x0 , NULL , V_326 } } ,
{ & V_320 ,
{ L_204 , L_205 , V_338 , V_325 ,
NULL , 0x0 , NULL , V_326 } } ,
{ & V_321 ,
{ L_206 , L_207 , V_338 , V_325 ,
NULL , 0x0 , NULL , V_326 } } ,
{ & V_322 ,
{ L_200 , L_208 , V_331 , V_328 ,
NULL , 0x0 , NULL , V_326 } } ,
{ & V_102 ,
{ L_209 , L_210 , V_327 , V_330 ,
F_93 ( V_96 ) , 0x0 , NULL , V_326 } } ,
{ & V_136 ,
{ L_211 , L_212 , V_327 , V_330 ,
F_93 ( V_132 ) , 0x0 , NULL , V_326 } } ,
{ & V_103 ,
{ L_213 , L_214 , V_340 , V_325 ,
NULL , 0x0 , L_215 , V_326 } } ,
{ & V_142 ,
{ L_216 , L_217 , V_331 , V_328 ,
NULL , 0x0 , L_218 , V_326 } } ,
{ & V_105 ,
{ L_219 , L_220 , V_339 , V_325 ,
NULL , 0x0 , L_221 , V_326 } } ,
{ & V_80 ,
{ L_222 , L_223 , V_331 , V_328 ,
NULL , 0x0 , L_224 , V_326 } } ,
{ & V_81 ,
{ L_225 , L_226 , V_341 , V_325 ,
NULL , 0x0 , L_227 , V_326 } } ,
{ & V_145 ,
{ L_228 , L_229 , V_331 , V_328 ,
NULL , 0x0 , L_230 , V_326 } } ,
{ & V_107 ,
{ L_231 , L_232 , V_331 , V_328 ,
NULL , 0x0 , L_233 , V_326 } } ,
{ & V_112 ,
{ L_234 , L_235 , V_342 , V_113 ,
NULL , 0x0 , L_236 , V_326 } } ,
#if 0
{ &hf_amf_longstringlength,
{ "String length", "amf.longstringlength", FT_UINT32, BASE_DEC,
NULL, 0x0, "AMF long string length", HFILL }},
#endif
{ & V_115 ,
{ L_237 , L_238 , V_341 , V_325 ,
NULL , 0x0 , L_239 , V_326 } } ,
{ & V_114 ,
{ L_240 , L_241 , V_341 , V_325 ,
NULL , 0x0 , L_242 , V_326 } } ,
{ & V_160 ,
{ L_243 , L_244 , V_331 , V_328 ,
NULL , 0x0 , L_245 , V_326 } } ,
{ & V_161 ,
{ L_246 , L_247 , V_341 , V_325 ,
NULL , 0x0 , L_248 , V_326 } } ,
{ & V_118 ,
{ L_249 , L_250 , V_343 , V_328 ,
NULL , 0x0 , L_251 , V_326 } } ,
{ & V_163 ,
{ L_252 , L_253 , V_331 , V_328 ,
NULL , 0x0 , L_254 , V_326 } } ,
{ & V_164 ,
{ L_255 , L_256 , V_324 , V_325 ,
NULL , 0x0 , L_257 , V_326 } } ,
{ & V_97 ,
{ L_258 , L_259 , V_344 , V_325 ,
NULL , 0x0 , L_260 , V_326 } } ,
{ & V_150 ,
{ L_261 , L_262 , V_331 , V_328 ,
NULL , 0x0 , L_263 , V_326 } } ,
{ & V_152 ,
{ L_264 , L_265 , V_331 , V_328 ,
NULL , 0x0 , L_266 , V_326 } } ,
{ & V_153 ,
{ L_267 , L_268 , V_341 , V_325 ,
NULL , 0x0 , L_269 , V_326 } } ,
{ & V_155 ,
{ L_270 , L_271 , V_331 , V_328 ,
NULL , 0x0 , L_272 , V_326 } } ,
{ & V_156 ,
{ L_273 , L_274 , V_341 , V_325 ,
NULL , 0x0 , L_275 , V_326 } } ,
{ & V_158 ,
{ L_276 , L_277 , V_331 , V_328 ,
NULL , 0x0 , L_278 , V_326 } } ,
{ & V_99 ,
{ L_279 , L_280 , V_344 , V_325 ,
NULL , 0x0 , L_281 , V_326 } } ,
{ & V_101 ,
{ L_282 , L_283 , V_344 , V_325 ,
NULL , 0x0 , L_284 , V_326 } } ,
{ & V_134 ,
{ L_285 , L_286 , V_344 , V_325 ,
NULL , 0x0 , L_287 , V_326 } } ,
{ & V_108 ,
{ L_288 , L_289 , V_331 , V_328 ,
NULL , 0x0 , L_290 , V_326 } } ,
{ & V_147 ,
{ L_291 , L_292 , V_331 , V_328 ,
NULL , 0x0 , L_293 , V_326 } } ,
{ & V_84 ,
{ L_294 , L_295 , V_344 , V_325 ,
NULL , 0x0 , NULL , V_326 } } ,
{ & V_148 ,
{ L_296 , L_297 , V_344 , V_325 ,
NULL , 0x0 , NULL , V_326 } } ,
{ & V_157 ,
{ L_298 , L_299 , V_344 , V_325 ,
NULL , 0x0 , NULL , V_326 } } ,
} ;
static T_2 * V_334 [] = {
& V_311 ,
& V_314 ,
& V_319 ,
& V_98 ,
& V_78 ,
& V_79 ,
& V_149 ,
& V_151 ,
& V_154 ,
} ;
V_310 = F_94 ( L_300 , L_301 , L_302 ) ;
F_95 ( V_310 , V_323 , F_96 ( V_323 ) ) ;
F_97 ( V_334 , F_96 ( V_334 ) ) ;
}
void
F_102 ( void )
{
T_23 V_345 ;
F_103 ( L_303 , F_89 , L_304 , L_305 , V_223 , V_346 ) ;
V_301 = F_104 ( F_81 , V_223 ) ;
F_105 ( L_306 , V_347 , V_301 ) ;
V_348 = F_104 ( F_85 , V_223 ) ;
F_106 ( L_307 , L_308 , V_348 ) ;
V_345 = F_104 ( F_91 , V_310 ) ;
F_106 ( L_307 , L_309 , V_345 ) ;
}
